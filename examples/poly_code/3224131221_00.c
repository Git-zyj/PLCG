...
void kernel_3224131221_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_P, PB_M); i_2++) {
                B[i_2][i_2] = C[i_2][i_0][i_1] - C[i_1+i_0][i_2][i_1] - C[i_1][i_2+i_0][i_1+1] - C[i_2][i_0][i_1] * 5;
            }
        }
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                B[i_3][i_3] = C[i_0][i_4][i_3+1] * B[i_3-2][i_3-2] + B[i_3-1][i_3-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3224131221_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
