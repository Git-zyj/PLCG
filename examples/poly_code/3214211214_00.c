...
void kernel_3214211214_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_P-1; i++) {
        for (int j = 0; j < PB_P-2; j++) {
            for (int k = 1; k < j; k++) {
                A[i][j+1][j+1] = C[j+1][k][i-1] * A[k-1][j+2][j+2] + A[i-1][j+1][j+1] + A[j+1][i+1][i+1] + 1;
            }
        }
        B[i] = D[i] - B[i-1] - 4;
    }
...
kernel_3214211214_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
