...
void kernel_3224212113_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N-2; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_M-1, i_0); i_2++) {
                A[i_1+2][i_1][i_2+1] = C[i_2][i_0][0] + 2;
            }
        }
        for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                A[i_3+2][i_3][i_4+1] = A[i_3+2][i_3][i_4+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3224212113_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
