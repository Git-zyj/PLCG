...
void kernel_3214311212_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int k = 2; k < PB_L-2; k++) {
        B[k-1][k+1] = D[k] * B[k-2][k] * B[k][k+2] * 6;
    }
#pragma endscop
...
kernel_3214311212_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
