...
void kernel_3114311212_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 2; i < PB_L; i++) {
        for (int j = 1; j < min(i,PB_M-1); j++) {
            A[j][i-1] = B[j+1][j] - A[j][i-2] - A[j-1][i] * 2;
        }
    }
#pragma endscop
...
kernel_3114311212_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
