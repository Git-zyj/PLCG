...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_N); i_1++) {
            A[i_1-1][i_0] = C[i_1-1] + D[i_1][i_0-1] - E[i_1][i_0][i_1] - C[i_1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, PB_P-1); i_3++) {
            for (int i_4 = 2; i_4 < PB_P-1; i_4++) {
                B[i_2][i_4][i_3] = E[i_2-1][i_3-1][i_4+1] * E[i_4][i_2][i_3+1] + B[i_2][i_4][i_3-1] - B[i_2+2][i_4-2][i_3] - 3;
            }
        }
    }
#pragma endscop
