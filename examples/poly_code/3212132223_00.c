...
void kernel_3212132223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_N, PB_N-1); i_1++) {
            A[i_1][i_1+i_0] = C[i_1+1][i_0] * C[i_1][i_0] - C[i_1+i_0][i_0] + A[i_1+2][i_1+i_0+2] + A[i_1][i_1+i_0+2] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < min(min(-i_2+PB_P-1, PB_N-2), i_2); i_3++) {
            A[i_3][i_3+i_2+1] = C[i_3][i_2] + C[i_3+2][i_2+1] - 3;
        }
    }
#pragma endscop
...
kernel_3212132223_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
