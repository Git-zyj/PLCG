...
void kernel_2122212121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_P+1); i_1++) {
            A[i_0][i_1+i_0-1][0] = B[i_1-1][i_0][i_1+1] + A[i_1+2][i_1+i_0-1][0] - 2;
        }
    }
#pragma endscop
...
kernel_2122212121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
