...
void kernel_3224212111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[i_1+2][i_0] = C[i_0] + A[i_1+1][i_0] + 2;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_N, i_0); i_2++) {
            for (int i_3 = 0; i_3 < min(-i_0+PB_L, i_0); i_3++) {
                B[i_2+i_0][i_2][i_3+i_0] = 1;
            }
        }
    }
#pragma endscop
...
kernel_3224212111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
