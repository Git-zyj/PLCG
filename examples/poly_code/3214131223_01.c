...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L-1; i_1++) {
            A[i_0+1][i_1] = C[0] * D[i_1-1][i_0] - A[i_0-1][i_1] - A[i_0+1][i_1-2] - A[i_0+1][i_1+1] * 1;
        }
        for (int i_2 = 1; i_2 < i_0; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_0+PB_L, -i_2+PB_L+1); i_3++) {
                B[i_3+i_2-1][i_0] = E[i_3][i_0][i_2] - E[i_2][i_3+i_0][i_3] + F[i_2][i_3+1][i_0][1] + B[i_3+i_0-1][i_2+1] + 1;
            }
        }
    }
#pragma endscop
