...
void kernel_3234311214_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_L; i++) {
        for (int j = 0; j < min(i,PB_M-1); j++) {
            A[j][i] = C[j] * C[j] - A[j+1][i] + 4;
        }
...
kernel_3234311214_00(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
