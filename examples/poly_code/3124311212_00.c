...
void kernel_3124311212_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_L; i++) {
        for (int j = 0; j < PB_M-2; j++) {
            A[j+1][j+1] = B[j][i] * A[j][j] - A[j+2][j+2] + 4;
        }
    }
#pragma endscop
...
kernel_3124311212_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
