...
void kernel_3224211223_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 3; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_0+PB_M, PB_P), -i_1+PB_M-2); i_2++) {
                A[i_1+i_0][i_2-1][i_0] = C[i_0][i_2][i_1] - A[i_1+i_0-1][i_2-1][i_0] + A[i_2+i_0][i_1-3][i_0] * A[i_2+i_1+2][i_0-1][i_1] * 6;
            }
        }
        B[i_0] = B[i_0+1] * 3;
    }
...
kernel_3224211223_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
