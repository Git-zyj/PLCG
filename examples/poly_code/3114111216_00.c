...
void kernel_3114111216_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M; i++) {
        for (int j = 1; j < i; j++) {
            A[j-1][i] = B[i][j] - A[j][i] - A[j-1][i-1] + 3;
        }
    }
#pragma endscop
...
kernel_3114111216_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
