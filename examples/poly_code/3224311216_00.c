...
void kernel_3224311216_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M-1; i++) {
        A[i] = C[i] + A[i-1] * A[i+1] + 6;
    }
    for (int j = 0; j < PB_L; j++) {
        for (int k = 0; k < PB_M-2; k++) {
            for (int l = 1; l < min(j,PB_L-1); l++) {
                B[k+1][k][l] = D[k][l+1][j] - B[k+1][k][l-1] - B[k+2][k+1][j] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3224311216_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
