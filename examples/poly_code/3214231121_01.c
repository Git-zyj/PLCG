...
void kernel_3214231121_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0] = C[i_0] * C[i_0+1] + D[i_0] * A[i_0+1] + 1;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            A[i_2] = E[i_2+i_1][i_1] * E[i_2-1][i_1+1] - 5;
        }
    }
#pragma endscop
...
kernel_3214231121_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
