...
void kernel_3224111216_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_P-1; i++) {
        for (int j = 1; j < PB_L; j++) {
            A[j][i] = C[j][i+1] * A[j-1][i-1] - 1;
        }
    }
    for (int k = 0; k < PB_P-1; k++) {
        for (int l = 0; l < PB_L-1; l++) {
            A[l+1][k+1] = C[l+1][k] + A[l][k+1] * 6;
        }
    }
#pragma endscop
...
kernel_3224111216_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
