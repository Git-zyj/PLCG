...
void kernel_3222132221_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        B[i_0][0] = C[i_0] * C[i_0] - B[i_0-1][0] - B[i_0+1][0] + 2;
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            B[i_0][0] = D[i_1+1][i_0] - D[i_1+2][i_0] + D[i_0][i_1] - D[i_0-2][i_0+2] + 4;
        }
    }
#pragma endscop
...
kernel_3222132221_01(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
