...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_N+1, PB_M-1), i_0); i_1++) {
            A[i_1][i_0] = C[i_0-1][i_1] + C[i_1+i_0-1][i_0-1] * A[i_1+1][i_0+1] - A[i_1+1][i_0] + 5;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            A[i_2][i_0-1] = C[i_2][1] - C[i_2-1][i_0] * D[i_2] + D[i_2] - 5;
        }
    }
#pragma endscop
