...
void kernel_2214212211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_P-2); i_1++) {
            A[i_1+i_0][i_1+1] = A[i_1+i_0][i_1+2] - A[i_1+i_0-1][i_1+1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_P, -i_2+PB_L+1); i_3++) {
            A[i_3+i_2-1][i_3] = C[0][i_2] + 2;
        }
    }
#pragma endscop
...
kernel_2214212211_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
