...
void kernel_3114111214_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_L+1; i++) {
        for (int j = 2; j < PB_L; j++) {
            A[j-1] = B[j][i-1] * A[j] - A[j-2] * 4;
        }
    }
#pragma endscop
...
kernel_3114111214_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
