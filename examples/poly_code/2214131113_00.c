...
void kernel_2214131113_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            B[i_1-2] = C[i_1+i_0] - C[i_0-1] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        B[i_2-1] = C[i_2] * C[i_2] + C[i_2] + B[i_2-2] * 5;
    }
#pragma endscop
...
kernel_2214131113_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
