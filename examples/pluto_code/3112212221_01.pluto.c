...
/* Start of CLooG code */
if ((PB_L >= 4) && (PB_M >= 2) && (PB_P >= 5)) {
  for (t2=max(ceild(-PB_L-29,32),ceild(-PB_P-28,32));t2<=-1;t2++) {
    lbp=0;
    ubp=min(min(floord(PB_L-4,32),floord(PB_M-2,32)),floord(32*t2+PB_L+29,32));
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t5=max(max(32*t2,-PB_P+3),32*t4-PB_L+2);t5<=min(-2,32*t2+31);t5++) {
        for (t7=32*t4;t7<=min(min(PB_M-2,32*t4+31),t5+PB_L-2);t7++) {
          A[t7][t7+-t5] = B[t7][-t5+2] - A[t7][t7+-t5-2] * A[t7+1][t7+-t5+1] - 5;;
        }
      }
    }
  }
