...
void kernel_3124212221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(2,32*t1-32*t2);t3<=min(-32*t2+PB_L,32*t1-32*t2+31);t3++) {
        for (t4=32*t2;t4<=min(32*t2+31,-t3+PB_L);t4++) {
          A[t4+t3-1][t3+1] = B[t3][t4] - A[t4+t3-2][t3] - A[t4+t3-2][t3+1] + 6;;
        }
      }
    }
  }
...
kernel_3124212221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
