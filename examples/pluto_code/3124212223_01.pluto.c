...
void kernel_3124212223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(2,32*t1-32*t2);t3<=(min(PB_L-1,32*t1-32*t2+31))-7;t3+=8) {
        for (t4=max(1,32*t2);t4<=min(PB_M-1,32*t2+31);t4++) {
          A[t3][t4] = B[t4] * A[t3-2][t4-1] * A[t3][t4+2] + 2;;
          A[(t3+1)][t4] = B[t4] * A[(t3+1)-2][t4-1] * A[(t3+1)][t4+2] + 2;;
          A[(t3+2)][t4] = B[t4] * A[(t3+2)-2][t4-1] * A[(t3+2)][t4+2] + 2;;
          A[(t3+3)][t4] = B[t4] * A[(t3+3)-2][t4-1] * A[(t3+3)][t4+2] + 2;;
          A[(t3+4)][t4] = B[t4] * A[(t3+4)-2][t4-1] * A[(t3+4)][t4+2] + 2;;
          A[(t3+5)][t4] = B[t4] * A[(t3+5)-2][t4-1] * A[(t3+5)][t4+2] + 2;;
          A[(t3+6)][t4] = B[t4] * A[(t3+6)-2][t4-1] * A[(t3+6)][t4+2] + 2;;
          A[(t3+7)][t4] = B[t4] * A[(t3+7)-2][t4-1] * A[(t3+7)][t4+2] + 2;;
        }
      }
      for (;t3<=min(PB_L-1,32*t1-32*t2+31);t3++) {
        for (t4=max(1,32*t2);t4<=min(PB_M-1,32*t2+31);t4++) {
          A[t3][t4] = B[t4] * A[t3-2][t4-1] * A[t3][t4+2] + 2;;
        }
      }
...
kernel_3124212223_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
