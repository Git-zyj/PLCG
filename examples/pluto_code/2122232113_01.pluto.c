...
void kernel_2122232113_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(-32*t1+PB_M-2,32);t2++) {
      for (t3=32*t1;t3<=min(32*t1+31,-32*t2+PB_M-2);t3++) {
        for (t4=32*t2;t4<=min(32*t2+31,-t3+PB_M-2);t4++) {
          A[t4][t3+t4][t3+1] = B[t3][t4] + B[t4][t4] * C[t3][t4] * A[t4+1][t3+t4+1][t3+1] + 3;;
        }
      }
    }
  }
...
kernel_2122232113_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
