...
void kernel_3112112111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_P-1,32);t2++) {
      for (t3=max(1,32*t1);t3<=min(min(PB_P-2,32*t1+31),32*t2+30);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(PB_P-1,32*t2+31);t4++) {
          A[t3-1][t4] = B[t3][t4][0] - A[t3-1][t4-1] * 3;;
        }
      }
    }
  }
...
kernel_3112112111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
