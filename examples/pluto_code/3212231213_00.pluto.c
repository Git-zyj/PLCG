...
void kernel_3212231213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    for (t2=max(0,ceild(32*t1-PB_M+2,16));t2<=min(floord(PB_M-1,16),floord(32*t1+PB_M+30,32));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=1;t3<=min(15,PB_M-2);t3++) {
          B[t3] = C[t3][1] * D[t3+1][0] - E[t3] - F[t3+1] + F[t3] - B[t3-1] + B[t3+1] * 3;;
          for (t4=2*t3;t4<=min(31,t3+PB_M-1);t4++) {
            A[(-t3+t4)][t3] = C[(-t3+t4)][t3] * A[(-t3+t4)+1][t3-1] + A[(-t3+t4)+1][t3] - 5;;
          }
        }
      }
      if (t2 >= 1) {
        for (t3=max(max(1,32*t1),32*t2-PB_M+1);t3<=min(min(PB_M-1,32*t1+31),16*t2+15);t3++) {
          for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,t3+PB_M-1);t4++) {
            A[(-t3+t4)][t3] = C[(-t3+t4)][t3] * A[(-t3+t4)+1][t3-1] + A[(-t3+t4)+1][t3] - 5;;
          }
        }
      }
      if ((PB_M <= 16) && (t1 == 0) && (t2 == 0)) {
        A[(PB_M-1)][(PB_M-1)] = C[(PB_M-1)][(PB_M-1)] * A[(PB_M-1)+1][(PB_M-1)-1] + A[(PB_M-1)+1][(PB_M-1)] - 5;;
      }
      if (t2 == 0) {
...
kernel_3212231213_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
