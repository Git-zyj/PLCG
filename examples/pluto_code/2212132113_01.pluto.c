...
void kernel_2212132113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_M-1,32);t2++) {
      for (t3=32*t1;t3<=min(min(PB_M-3,32*t1+31),-32*t2+PB_M-1);t3++) {
        for (t4=max(2,32*t2);t4<=min(32*t2+31,-t3+PB_M-1);t4++) {
          A[t3][t4-2] = C[t4][0] + C[t4][t3+2] + D[t3] * C[t4][t3+1] + A[t3][t4-1] - 3;;
          A[t3][t4-2] = C[t3+t4][t3] * C[t3][t4] * 4;;
        }
        for (t4=-t3+PB_M;t4<=min(PB_M-1,32*t2+31);t4++) {
          A[t3][t4-2] = C[t4][0] + C[t4][t3+2] + D[t3] * C[t4][t3+1] + A[t3][t4-1] - 3;;
        }
      }
      for (t3=max(32*t1,-32*t2+PB_M);t3<=min(PB_M-1,32*t1+31);t3++) {
        for (t4=32*t2;t4<=min(PB_M-1,32*t2+31);t4++) {
          A[t3][t4-2] = C[t4][0] + C[t4][t3+2] + D[t3] * C[t4][t3+1] + A[t3][t4-1] - 3;;
        }
      }
      if (t2 == 0) {
        for (t3=max(32*t1,PB_M-2);t3<=min(PB_M-1,32*t1+31);t3++) {
          for (t4=2;t4<=min(31,PB_M-1);t4++) {
            A[t3][t4-2] = C[t4][0] + C[t4][t3+2] + D[t3] * C[t4][t3+1] + A[t3][t4-1] - 3;;
          }
        }
      }
...
kernel_2212132113_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
