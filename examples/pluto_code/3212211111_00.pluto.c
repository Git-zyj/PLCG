...
void kernel_3212211111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 3) {
  if (PB_P >= 1) {
    for (t2=1;t2<=PB_M-2;t2++) {
      A[t2+1] = C[t2-1] * A[t2] * 4;;
      for (t4=-PB_P+1;t4<=0;t4++) {
        for (t6=max(2,t2);t6<=PB_M;t6++) {
          B[t6-1][t6] = B[t6-2][t6-1] - 6;;
        }
      }
    }
  }
...
kernel_3212211111_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
