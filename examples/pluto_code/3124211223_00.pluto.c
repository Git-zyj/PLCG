  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_M-1,32);t2++) {
      for (t3=0;t3<=min(floord(PB_M-2,32),t2);t3++) {
        for (t4=32*t1;t4<=min(min(PB_M-2,32*t1+31),32*t2+30);t4++) {
          for (t5=max(max(max(4,32*t2),32*t3+1),t4+1);t5<=min(PB_M-1,32*t2+31);t5++) {
            lbv=max(3,32*t3);
            ubv=min(32*t3+31,t5-1);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t5][t4][t6-1] = B[t4][t6][t5] * A[t5-1][t4][t6-1] + A[t5-1][t4][t6-3] + 3;;
            }
          }
        }
      }
