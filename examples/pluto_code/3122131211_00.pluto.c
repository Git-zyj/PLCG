...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(1,32*t1-32*t2);t3<=min(PB_L-1,32*t1-32*t2+31);t3++) {
        for (t4=max(2,32*t2);t4<=min(2*PB_L-3,32*t2+31);t4++) {
          lbv=max(1,t4-PB_L+2);
          ubv=min(PB_L-1,t4-1);
#pragma ivdep
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            A[t5-1][(t4-t5)][t3-1][t5] = B[t3][t5][(t4-t5)+1] - B[(t4-t5)][t3][t3] - B[t5-1][t3][(t4-t5)+1] + A[t5-1][(t4-t5)+1][t3-1][t5] * A[(t4-t5)-1][t5][t3][(t4-t5)] + 6;;
          }
        }
      }
    }
