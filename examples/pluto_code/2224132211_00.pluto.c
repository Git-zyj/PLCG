...
void kernel_2224132211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
/* Start of CLooG code */
if (PB_L >= 4) {
  for (t1=2;t1<=min(PB_L-4,PB_M-1);t1++) {
    for (t2=0;t2<=t1-1;t2++) {
      A[t1][t2+2] = C[t1-2][t2] - A[t1][t2+3] - 6;;
    }
    for (t2=t1;t2<=PB_L-4;t2++) {
      B[t1+1][t1-2] = C[t2-1][t1] + D[t2] + C[t2+2][t1] - D[t2+1] - C[t2][t1] - A[t1][t2+3] * B[t1+2][t1-1] - B[t2+2][t2-1] - 6;;
    }
  }
  for (t1=max(2,PB_L-3);t1<=PB_M-1;t1++) {
    for (t2=0;t2<=PB_L-4;t2++) {
      A[t1][t2+2] = C[t1-2][t2] - A[t1][t2+3] - 6;;
    }
  }
  for (t1=max(2,PB_M);t1<=PB_L-4;t1++) {
    for (t2=t1;t2<=PB_L-4;t2++) {
      B[t1+1][t1-2] = C[t2-1][t1] + D[t2] + C[t2+2][t1] - D[t2+1] - C[t2][t1] - A[t1][t2+3] * B[t1+2][t1-1] - B[t2+2][t2-1] - 6;;
    }
  }
...
kernel_2224132211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
