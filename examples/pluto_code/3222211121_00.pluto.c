...
void kernel_3222211121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
if ((PB_L >= 0) && (PB_N >= max(0,-3*PB_L+3))) {
  for (t1=0;t1<=min(floord(PB_N+PB_L-1,32),floord(PB_N+2*PB_L-2,32));t1++) {
    if ((PB_N >= 3) && (PB_P <= -1) && (t1 <= floord(PB_N-2,32))) {
      for (t4=max(1,32*t1);t4<=min(PB_N-2,32*t1+31);t4++) {
        A[t4] = C[t4] - A[t4+1] - A[t4-1] + 1;;
      }
    }
    for (t2=0;t2<=min(min(floord(PB_P,32),floord(-32*t1+PB_N+PB_L+PB_P-2,32)),floord(PB_N+3*PB_P-3,96));t2++) {
      if ((PB_M <= -1) && (PB_N >= 3) && (t1 <= floord(PB_N-2,32)) && (t2 == 0)) {
        for (t4=max(1,32*t1);t4<=min(PB_N-2,32*t1+31);t4++) {
          A[t4] = C[t4] - A[t4+1] - A[t4-1] + 1;;
        }
      }
      if ((PB_N >= ceild(-3*PB_M+6,2)) && (t1 <= floord(2*PB_N+2*PB_L+PB_M-4,64))) {
        for (t3=0;t3<=min(min(floord(PB_M,32),floord(-32*t1+PB_N+PB_L+PB_M-2,32)),floord(PB_N+3*PB_M-3,96));t3++) {
          if ((PB_M <= 1) && (t2 == 0) && (t3 == 0)) {
            for (t4=max(1,32*t1);t4<=min(PB_N-2,32*t1+31);t4++) {
              A[t4] = C[t4] - A[t4+1] - A[t4-1] + 1;;
            }
          }
          if ((PB_M >= 2) && (PB_P == 0) && (t2 == 0) && (t3 == 0)) {
            for (t4=max(1,32*t1);t4<=min(PB_N-2,32*t1+31);t4++) {
              A[t4] = C[t4] - A[t4+1] - A[t4-1] + 1;;
            }
          }
          if ((PB_M >= 2) && (t2 <= floord(PB_P-1,32)) && (t2 >= -t3+1) && (t3 <= floord(PB_M-1,32))) {
...
#pragma vector always
                for (t6=lbv;t6<=ubv;t6++) {
                  B[t4][t5][t6-1] = D[t6][t4][t5] * 1;;
                }
              }
            }
          }
          if ((PB_L >= 1) && (PB_M >= 2) && (PB_P >= 1) && (t1 == 0) && (t2 == 0) && (t3 == 0)) {
            for (t5=0;t5<=min(31,PB_P-1);t5++) {
              lbv=1;
              ubv=min(31,PB_M-1);
#pragma ivdep
#pragma vector always
              for (t6=lbv;t6<=ubv;t6++) {
                B[0][t5][t6-1] = D[t6][0][t5] * 1;;
              }
            }
          }
          if ((PB_M >= 2) && (PB_P >= 1) && (t2 == 0) && (t3 == 0)) {
            for (t4=max(1,32*t1);t4<=min(min(PB_L-1,PB_N-2),32*t1+31);t4++) {
              A[t4] = C[t4] - A[t4+1] - A[t4-1] + 1;;
              for (t5=0;t5<=min(31,PB_P-1);t5++) {
                lbv=1;
                ubv=min(31,PB_M-1);
...
#pragma vector always
          ...
          if ((PB_M >= 2) && (PB_P >= 1) && (t2 == 0) && (t3 == 0)) {
            for (t4=max(max(1,PB_L),32*t1);t4<=min(PB_N-2,32*t1+31);t4++) {
              A[t4] = C[t4] - A[t4+1] - A[t4-1] + 1;;
            }
          }
          if ((PB_M >= 2) && (PB_P >= 1) && (t2 == 0) && (t3 == 0)) {
            for (t4=max(max(1,32*t1),PB_N-1);t4<=min(PB_L-1,32*t1+31);t4++) {
              for (t5=0;t5<=min(31,PB_P-1);t5++) {
                lbv=1;
                ubv=min(31,PB_M-1);
#pragma ivdep
...
kernel_3222211121_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
