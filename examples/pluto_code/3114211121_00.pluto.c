...
void kernel_3114211121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_L-2,32),t1);t2++) {
      for (t3=max(max(2,32*t1),32*t2+1);t3<=min(PB_L-1,32*t1+31);t3++) {
        lbv=max(1,32*t2);
        ubv=min(32*t2+31,t3-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4] = B[t3] - A[t4-1][t3] * 5;;
        }
      }
    }
  }
...
kernel_3114211121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
