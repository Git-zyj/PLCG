...
void kernel_2222132111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_M >= 0) && (PB_N >= max(0,-PB_M+4))) {
  for (t1=0;t1<=min(floord(PB_N+PB_M-2,32),floord(4*PB_N+5*PB_M-12,128));t1++) {
    if ((PB_M >= 4) && (t1 <= floord(PB_M-2,32))) {
      for (t4=max(2,32*t1);t4<=min(PB_M-2,32*t1+31);t4++) {
        B[1] = D[t4] + D[t4+1] + D[t4-2] + D[t4] + B[1] - B[1] - 1;;
      }
    }
    if ((PB_N >= 4) && (PB_P >= 2) && (t1 <= floord(PB_N-3,32))) {
      for (t2=0;t2<=min(floord(PB_P-1,32),t1);t2++) {
        for (t3=max(1,32*t2);t3<=min(min(PB_N-3,PB_P-1),32*t2+31);t3++) {
          for (t4=max(32*t1,t3);t4<=min(PB_N-3,32*t1+31);t4++) {
            A[(-t3+t4)+t3][(-t3+t4)+t3+2] = C[(-t3+t4)][t3-1] + C[t3+2][t3] - 4;;
          }
        }
      }
    }
...
kernel_2222132111_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
