...
void kernel_2222132111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=t2;t3<=floord(PB_M-3,32);t3++) {
        for (t4=max(2,32*t2);t4<=min(min(PB_M-4,32*t2+31),32*t3+30);t4++) {
          for (t5=max(32*t3,t4+1);t5<=min(PB_M-3,32*t3+31);t5++) {
            A[t4-1] = D[t4-2][t5-2] * D[t5+2][t4+2] + D[t4][t5] - C[t4] * 1;;
          }
        }
      }
    }
...
kernel_2222132111_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
