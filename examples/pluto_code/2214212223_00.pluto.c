...
void kernel_2214212223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_L >= 5) {
  for (t1=0;t1<=floord(PB_L-3,32);t1++) {
    for (t2=0;t2<=min(floord(PB_L-4,32),t1);t2++) {
      if (t2 >= 1) {
...
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t4][t3+2] = C[t3][t4-2] * C[t3][t4+2] + 5;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        B[2] = B[2 +2] + B[2 -1] * B[2 -2] - B[2 +1] - 6;;
      }
      if (t2 == 0) {
        for (t3=max(3,32*t1);t3<=min(PB_L-3,32*t1+31);t3++) {
          B[t3] = B[t3+2] + B[t3-1] * B[t3-2] - B[t3+1] - 6;;
          lbv=2;
          ubv=min(31,t3-1);
#pragma ivdep
#pragma vector always
...
kernel_2214212223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
