...
void kernel_2214212223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 3) {
  for (t1=0;t1<=floord(PB_M,32);t1++) {
    for (t2=0;t2<=min(floord(3*PB_M,32),floord(64*t1+PB_M+62,32));t2++) {
      if ((PB_M >= 4) && (t1 == 0) && (t2 == 0)) {
        for (t3=3;t3<=min(13,PB_M);t3++) {
          A[t3] = C[t3-1] + A[t3-1] + A[t3+1] - 4;;
          for (t4=2*t3+4;t4<=min(31,2*t3+PB_M);t4++) {
            B[t3-1][(-2*t3+t4)-2] = B[t3-3][(-2*t3+t4)-4] + B[(-2*t3+t4)-1][t3-1] + 5;;
          }
        }
      }
      if ((PB_M == 3) && (t1 == 0) && (t2 == 0)) {
        A[3] = C[3 -1] + A[3 -1] + A[3 +1] - 4;;
      }
      if (t2 == 0) {
        for (t3=max(14,32*t1);t3<=min(PB_M,32*t1+31);t3++) {
          A[t3] = C[t3-1] + A[t3-1] + A[t3+1] - 4;;
        }
      }
      if (t2 >= 1) {
        for (t3=max(max(3,ceild(32*t2-PB_M,2)),32*t1);t3<=min(min(PB_M,32*t1+31),16*t2+13);t3++) {
          for (t4=max(32*t2,2*t3+4);t4<=min(32*t2+31,2*t3+PB_M);t4++) {
            B[t3-1][(-2*t3+t4)-2] = B[t3-3][(-2*t3+t4)-4] + B[(-2*t3+t4)-1][t3-1] + 5;;
          }
        }
      }
...
kernel_2214212223_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
