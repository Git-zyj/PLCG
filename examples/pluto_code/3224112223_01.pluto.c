...
void kernel_3224112223_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
  ...
  for (t1=0;t1<=floord(PB_M-3,32);t1++) {
    if (PB_P <= -1) {
      for (t4=32*t1;t4<=min(PB_M-3,32*t1+31);t4++) {
        B[t4+2] = C[t4] * B[t4+1] * 1;;
      }
    }
    for (t2=0;t2<=floord(PB_P,32);t2++) {
      for (t3=0;t3<=floord(PB_P,32);t3++) {
        if ((PB_P >= 3) && (t2 == 0) && (t3 == 0)) {
          for (t4=32*t1;t4<=min(PB_M-3,32*t1+31);t4++) {
            A[t4][0 +2][0] = A[t4+1][0 +2][0] - A[t4+1][0 +2][0 +1] * 4;;
            B[t4+2] = C[t4] * B[t4+1] * 1;;
            lbv=1;
            ubv=min(31,PB_P-2);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][0 +2][t6] = A[t4+1][0 +2][t6] - A[t4+1][0 +2][t6+1] * 4;;
            }
...
#pragma vector always
              for (t6=lbv;t6<=ubv;t6++) {
                A[t4][t5+2][t6] = A[t4+1][t5+2][t6] - A[t4+1][t5+2][t6+1] * 4;;
              }
            }
          }
        }
        if ((t2 <= floord(PB_P-3,32)) && (t2 >= -t3+1) && (t3 <= floord(PB_P-2,32))) {
          for (t4=32*t1;t4<=min(PB_M-3,32*t1+31);t4++) {
...
kernel_3224112223_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
