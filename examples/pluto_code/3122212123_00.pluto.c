...
void kernel_3122212123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_M-2,32);t2++) {
      for (t3=max(1,32*t1);t3<=(min(PB_M,32*t1+31))-7;t3+=8) {
        for (t4=32*t2;t4<=min(PB_M-2,32*t2+31);t4++) {
          A[t3-1][t4] = B[0] - A[t3-1][t4+1] - 3;;
          A[(t3+1)-1][t4] = B[0] - A[(t3+1)-1][t4+1] - 3;;
          A[(t3+2)-1][t4] = B[0] - A[(t3+2)-1][t4+1] - 3;;
          A[(t3+3)-1][t4] = B[0] - A[(t3+3)-1][t4+1] - 3;;
          A[(t3+4)-1][t4] = B[0] - A[(t3+4)-1][t4+1] - 3;;
          A[(t3+5)-1][t4] = B[0] - A[(t3+5)-1][t4+1] - 3;;
          A[(t3+6)-1][t4] = B[0] - A[(t3+6)-1][t4+1] - 3;;
          A[(t3+7)-1][t4] = B[0] - A[(t3+7)-1][t4+1] - 3;;
        }
      }
      for (;t3<=min(PB_M,32*t1+31);t3++) {
        for (t4=32*t2;t4<=min(PB_M-2,32*t2+31);t4++) {
          A[t3-1][t4] = B[0] - A[t3-1][t4+1] - 3;;
        }
      }
...
kernel_3122212123_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
