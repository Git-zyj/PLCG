...
void kernel_3122212123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_P-1,32);t2++) {
      for (t3=t2;t3<=min(floord(PB_P+PB_L-3,32),floord(32*t2+PB_L+29,32));t3++) {
        for (t4=max(32*t2,32*t3-PB_L+2);t4<=min(PB_P-1,32*t2+31);t4++) {
...
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[(-t4+t5)][t6] = B[t6][(-t4+t5)] * A[(-t4+t5)+1][t6] - 6;;
            }
          }
        }
      }
...
kernel_3122212123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
