...
/* Start of CLooG code */
if ((PB_M >= 0) && (PB_Q >= 0)) {
  for (t1=max(2,ceild(-PB_Q+9,3));t1<=PB_Q+PB_M-1;t1++) {
    if ((t1 >= 3) && (t1 <= PB_M-1)) {
      B[t1-1] = D[t1] * D[1] + D[t1] * D[t1] - B[t1] * B[t1-2] * B[t1-3] * 1;;
    }
    if ((PB_L >= 3) && (t1 <= PB_Q-1)) {
      for (t2=0;t2<=floord(5*PB_L-10,32);t2++) {
        lbp=max(ceild(3*t2,4),ceild(32*t2-PB_L+2,32));
        ubp=min(min(floord(PB_L-2,8),floord(96*t2+PB_L+92,128)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
        for (t3=lbp;t3<=ubp;t3++) {
          for (t4=max(max(1,ceild(32*t3-PB_L+1,3)),32*t2-32*t3);t4<=min(min(floord(32*t3+30,3),PB_L-2),32*t2-32*t3+31);t4++) {
            for (t5=max(32*t3,3*t4+1);t5<=min(32*t3+31,3*t4+PB_L-1);t5++) {
              A[t4-1][(-3*t4+t5)][t1] = C[t4+1][(-3*t4+t5)][t1] - C[t1-1][(-3*t4+t5)][t4] * A[(-3*t4+t5)-1][t1-2][t4] + 5;;
            }
          }
        }
      }
