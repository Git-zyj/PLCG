...
void kernel_2212212213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (t2=0;t2<=floord(PB_M-2,32);t2++) {
      if ((PB_M >= 3) && (t2 == 0)) {
        for (t3=max(2,32*t1);t3<=min(PB_M,32*t1+31);t3++) {
          B[t3][0 +2] = C[0 +2][t3] * C[0][t3] - B[t3][0 +1] + B[t3+1][0 +2] + B[t3+1][0 +3] * 6;;
          A[t3-1] = A[t3-2] - 1;;
          for (t4=1;t4<=min(31,PB_M-3);t4++) {
            B[t3][t4+2] = C[t4+2][t3] * C[t4][t3] - B[t3][t4+1] + B[t3+1][t4+2] + B[t3+1][t4+3] * 6;;
          }
        }
      }
      if ((PB_M == 2) && (t1 == 0) && (t2 == 0)) {
...
kernel_2212212213_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
