...
void kernel_3214231211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_L >= 1) {
  for (t1=1;t1<=PB_L-2;t1++) {
    A[t1] = C[0] * D[t1+1] + D[t1] * D[t1] - A[t1-1] + A[t1+1] - 6;;
    A[(t1-1)] = C[(t1-1)+1] + C[(t1-1)] * 4;;
  }
  for (t1=max(1,PB_L-1);t1<=PB_L;t1++) {
    A[(t1-1)] = C[(t1-1)+1] + C[(t1-1)] * 4;;
  }
}
/* End of CLooG code */
...
kernel_3214231211_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
