  ...
  for (t1=0;t1<=floord(PB_N-1,32);t1++) {
    if (PB_L <= -1) {
      for (t3=max(1,32*t1);t3<=min(PB_N-1,32*t1+31);t3++) {
        A[t3+2] = C[t3-1] - D[t3] + C[t3+1] + 1;;
      }
    }
    if (PB_L >= 0) {
      for (t2=0;t2<=min(min(floord(64*t1+PB_L+61,32),floord(128*t1+63*PB_L,64)),floord(96*t1+61*PB_N+32*PB_L-122,1024));t2++) {
        if ((PB_L <= 1) && (t2 == 0)) {
          for (t3=max(1,32*t1);t3<=min(PB_N-1,32*t1+31);t3++) {
            A[t3+2] = C[t3-1] - D[t3] + C[t3+1] + 1;;
          }
        ...
        if ((PB_L >= 2) && (t2 >= 1)) {
          for (t3=max(ceild(32*t2-PB_L+1,2),32*t1);t3<=min(min(PB_N-3,32*t1+31),16*t2+15);t3++) {
            for (t4=max(32*t2,2*t3+1);t4<=min(32*t2+31,2*t3+PB_L-1);t4++) {
              B[(-2*t3+t4)+1] = C[(-2*t3+t4)] + E[t3+2][(-2*t3+t4)-1] + F[(-2*t3+t4)][t3+2] * B[(-2*t3+t4)+2] + B[(-2*t3+t4)] * B[(-2*t3+t4)+3] - 6;;
            }
          }
        }
        if ((PB_L >= 2) && (PB_N >= 3) && (t1 == 0) && (t2 == 0)) {
          for (t4=1;t4<=min(31,PB_L-1);t4++) {
            B[t4+1] = C[t4] + E[0 +2][t4-1] + F[t4][0 +2] * B[t4+2] + B[t4] * B[t4+3] - 6;;
          }
        }
        if ((PB_L >= 2) && (t1 == 0) && (t2 == 0)) {
          for (t3=1;t3<=min(15,PB_N-3);t3++) {
            A[t3+2] = C[t3-1] - D[t3] + C[t3+1] + 1;;
        ...
        if ((PB_L >= 2) && (t1 == 0) && (t2 == 0)) {
          for (t3=max(1,PB_N-2);t3<=min(15,PB_N-1);t3++) {
            A[t3+2] = C[t3-1] - D[t3] + C[t3+1] + 1;;
          }
        }
        if ((PB_L >= 2) && (t2 == 0)) {
          for (t3=max(16,32*t1);t3<=min(PB_N-1,32*t1+31);t3++) {
            A[t3+2] = C[t3-1] - D[t3] + C[t3+1] + 1;;
          }
        }
      }
