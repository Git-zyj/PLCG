...
/* Start of CLooG code */
if ((PB_L >= 4) && (PB_M >= 3)) {
  for (t1=0;t1<=min(floord(PB_L-2,16),floord(PB_L+PB_M-2,32));t1++) {
    lbp=max(0,ceild(32*t1-PB_L+1,32));
    ubp=min(floord(t1,2),floord(PB_M-1,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(2,32*t2);t3<=min(min(min(PB_L-2,PB_M-1),32*t2+31),32*t1-32*t2+30);t3++) {
        for (t4=max(32*t1-32*t2,t3+1);t4<=min(PB_L-1,32*t1-32*t2+31);t4++) {
          A[t3-1][t4] = B[t3][t4] - A[t3-2][t4] - A[t3-1][t4-1] - 1;;
        }
      }
    }
  }
