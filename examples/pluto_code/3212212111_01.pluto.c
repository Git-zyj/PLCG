...
void kernel_3212212111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_L-1,32);t2++) {
      for (t3=0;t3<=floord(PB_L-1,32);t3++) {
          ...
          for (t4=max(1,32*t1);t4<=min(PB_L-1,32*t1+31);t4++) {
            for (t5=max(32*t2,t4);t5<=min(PB_L-1,32*t2+31);t5++) {
              for (t6=32*t2;t6<=t5-1;t6++) {
                B[t4][t5][0] = D[t5][t6][t4] + A[t4][t6] - 3;;
              }
              A[t4][t5] = C[t5][t4-1] + 4;;
              B[t4][t5][0] = D[t5][t5][t4] + A[t4][t5] - 3;;
              for (t6=t5+1;t6<=min(PB_L-1,32*t2+31);t6++) {
                B[t4][t5][0] = D[t5][t6][t4] + A[t4][t6] - 3;;
              }
            }
          }
        ...
        if (t2 >= t3+1) {
          for (t4=max(1,32*t1);t4<=min(PB_L-1,32*t1+31);t4++) {
            for (t5=max(32*t2,t4);t5<=(min(PB_L-1,32*t2+31))-7;t5+=8) {
              for (t6=32*t3;t6<=32*t3+31;t6++) {
                B[t4][t5][0] = D[t5][t6][t4] + A[t4][t6] - 3;;
                B[t4][(t5+1)][0] = D[(t5+1)][t6][t4] + A[t4][t6] - 3;;
                B[t4][(t5+2)][0] = D[(t5+2)][t6][t4] + A[t4][t6] - 3;;
                B[t4][(t5+3)][0] = D[(t5+3)][t6][t4] + A[t4][t6] - 3;;
                B[t4][(t5+4)][0] = D[(t5+4)][t6][t4] + A[t4][t6] - 3;;
                B[t4][(t5+5)][0] = D[(t5+5)][t6][t4] + A[t4][t6] - 3;;
                B[t4][(t5+6)][0] = D[(t5+6)][t6][t4] + A[t4][t6] - 3;;
                B[t4][(t5+7)][0] = D[(t5+7)][t6][t4] + A[t4][t6] - 3;;
              }
            }
        ...
        if (t2 <= t3-1) {
          for (t4=max(1,32*t1);t4<=32*t1+31;t4++) {
            for (t5=max(32*t2,t4);t5<=32*t2+31-7;t5+=8) {
...
kernel_3212212111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
