...
void kernel_2222112113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3+2][t4-2] = C[t4-1][t3+1] * 2;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=min(2,PB_P-2);t3++) {
          B[t3][t3] = B[t3+1][t3+1] + 6;;
        }
      }
      if (t2 == 0) {
...
kernel_2222112113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
