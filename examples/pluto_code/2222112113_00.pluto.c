...
void kernel_2222112113_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma vector always
  ...
  if (PB_P >= 1) {
    for (t2=0;t2<=floord(PB_M+2*PB_P-3,32);t2++) {
      lbp=max(ceild(t2,2),ceild(32*t2-PB_P+1,32));
      ubp=min(min(floord(PB_M+PB_P-2,32),floord(32*t2+PB_M+30,64)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(32*t2-32*t3,32*t3-PB_M+1);t4<=min(PB_P-1,32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4);t5<=min(32*t3+31,t4+PB_M-1);t5++) {
            B[(-t4+t5)] = D[(-t4+t5)][t4] * B[(-t4+t5)+1] + 1;;
          }
        }
      }
    }
...
kernel_2222112113_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
