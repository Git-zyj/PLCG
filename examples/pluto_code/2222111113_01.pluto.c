...
void kernel_2222111113_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,ceild(32*t1-PB_L-31,32));t2<=floord(t1,2);t2++) {
      for (t3=max(max(1,32*t2),32*t1-PB_L);t3<=min(min(PB_L-1,16*t1+15),32*t2+31);t3++) {
        if (t1 <= floord(t3,16)) {
          B[t3-1][t3-1][t3] = C[t3-1][t3] * B[t3][t3][t3-1] - 2;;
        }
        lbv=max(32*t1,2*t3+1);
        ubv=min(32*t1+31,t3+PB_L);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          B[(-t3+t4-1)][(-t3+t4-1)][(t3-1)+1] = C[(t3-1)][(-t3+t4-1)+1] * 1;;
          B[t3-1][t3-1][(-t3+t4)] = C[(-t3+t4)-1][t3] * B[t3][t3][(-t3+t4)-1] - 2;;
        }
      }
    }
  }
...
kernel_2222111113_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
