...
void kernel_3112231221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
/* Start of CLooG code */
if ((PB_M >= 2) && (PB_P >= 3)) {
  for (t1=0;t1<=floord(4*PB_P+PB_M-5,32);t1++) {
    lbp=max(ceild(3*t1,4),ceild(32*t1-PB_P+1,32));
    ubp=min(min(floord(3*PB_P+PB_M-4,32),floord(96*t1+PB_M+92,128)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,ceild(32*t2-PB_M+1,3)),32*t1-32*t2);t3<=min(min(floord(32*t2+30,3),PB_P-1),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,3*t3+1);t4<=min(32*t2+31,3*t3+PB_M-1);t4++) {
          A[t3-1][(-3*t3+t4)] = B[(-3*t3+t4)-1][t3] * C[(-3*t3+t4)][t3] - D[(-3*t3+t4)] * A[t3+1][(-3*t3+t4)] + A[(-3*t3+t4)-1][t3-2] - 3;;
        }
      }
    }
  }
...
kernel_3112231221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
