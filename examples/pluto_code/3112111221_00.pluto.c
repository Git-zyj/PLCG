...
/* Start of CLooG code */
if ((PB_L >= 4) && (PB_M >= 4) && (PB_N >= 2)) {
  for (t1=0;t1<=min(floord(PB_M-1,32),floord(PB_N+PB_L-3,32));t1++) {
    lbp=max(max(0,ceild(32*t1-PB_M+4,32)),ceild(32*t1-PB_N+2,32));
    ubp=min(floord(PB_L-1,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t1-32*t2;t3<=min(min(min(PB_M-4,PB_N-2),32*t1-32*t2+31),-32*t2+PB_M-1);t3++) {
        for (t4=max(3,32*t2);t4<=min(min(PB_L-1,32*t2+31),-t3+PB_M-1);t4++) {
          A[t3+1][t4-1] = B[t4+t3][t4] - A[t3][t4-1] + A[t3+1][t4-3] * 5;;
        }
      }
    }
  }
