...
void kernel_2124212211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t5=max(32*t2,-PB_L+3);t5<=min(-2,32*t2+31);t5++) {
        for (t7=max(1,32*t4);t7<=min(32*t4+31,-t5-1);t7++) {
          A[-t5][t7-1] = B[t7+1][-t5+2] * A[-t5][t7] + A[-t5-1][t7-1] - 6;;
        }
      }
    }
  }
...
kernel_2124212211_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
