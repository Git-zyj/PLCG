...
void kernel_2212131123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 3) {
  for (t1=1;t1<=PB_M-3;t1++) {
    A[t1+1] = C[t1] - C[t1] + C[t1-1] * 3;;
    B[t1] = C[1] * C[t1+1] - C[t1-1] + B[t1+2] * B[t1+1] + 3;;
  }
  A[(PB_M-2)+1] = C[(PB_M-2)] - C[(PB_M-2)] + C[(PB_M-2)-1] * 3;;
}
/* End of CLooG code */
...
kernel_2212131123_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
