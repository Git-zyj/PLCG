...
void kernel_3224211223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
  ...
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    if ((PB_L <= -1) && (PB_M >= 4) && (t1 <= floord(PB_M-3,32))) {
      for (t4=max(1,32*t1);t4<=min(PB_M-3,32*t1+31);t4++) {
        A[t4+1] = C[t4+1] * A[t4+2] + A[t4] * A[t4-1] + 3;;
      }
    }
    if ((PB_M >= -3*PB_L+4) && (t1 <= floord(PB_M+2*PB_L-3,32))) {
...
#pragma vector always
          ...
          if ((PB_L >= 1) && (t2 == 0) && (t3 == 0)) {
            for (t4=max(max(1,32*t1),PB_M-2);t4<=min(PB_M-1,32*t1+31);t4++) {
              for (t5=1;t5<=min(31,PB_L);t5++) {
                lbv=t5;
                ubv=min(31,PB_L);
#pragma ivdep
...
kernel_3224211223_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
