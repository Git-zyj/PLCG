...
void kernel_3124232223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if (PB_M >= 3) {
  for (t1=-1;t1<=min(floord(PB_M-18,16),floord(PB_M-3,32));t1++) {
    lbp=max(2*t1,t1+1);
    ubp=min(floord(PB_M-2,16),floord(32*t1+PB_M+30,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(0,32*t1),32*t2-PB_M+1);t3<=min(min(min(PB_M-3,-32*t1+32*t2),32*t1+62),16*t2+15);t3++) {
        lbv=max(max(32*t2,2*t3),-32*t1+32*t2+t3-31);
        ubv=min(min(32*t2+31,-32*t1+32*t2+t3),t3+PB_M-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+1][(-t3+t4)-1] = B[(-t3+t4)][t3] - B[(-t3+t4)][t3] - C[t3][(-t3+t4)] + A[t3][(-t3+t4)] - A[t3+2][(-t3+t4)-1] * 1;;
        }
      }
    }
  }
...
kernel_3124232223_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
