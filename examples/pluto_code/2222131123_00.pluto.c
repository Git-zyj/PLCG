...
void kernel_2222131123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  for (t1=1;t1<=PB_M-2;t1++) {
    A[t1] = C[t1] + C[t1] - C[t1-1] + C[0] + 3;;
    B[t1] = C[t1+1] * C[t1+1] * B[t1-1] - B[t1+1] + 3;;
  }
  A[(PB_M-1)] = C[(PB_M-1)] + C[(PB_M-1)] - C[(PB_M-1)-1] + C[0] + 3;;
}
/* End of CLooG code */
...
kernel_2222131123_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
