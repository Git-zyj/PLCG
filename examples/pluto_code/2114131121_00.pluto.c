...
void kernel_2114131121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 3) {
  for (t1=2;t1<=PB_M-1;t1++) {
    A[t1] = B[t1+1][0] * C[t1] * C[t1] + A[t1-2] + 2;;
  }
}
/* End of CLooG code */
...
kernel_2114131121_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
