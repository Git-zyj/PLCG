...
void kernel_2122131111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_Q >= 1)) {
  for (t1=0;t1<=min(floord(3*PB_L-6,32),floord(2*PB_Q+PB_L-1,32));t1++) {
    lbp=max(ceild(2*t1,3),ceild(32*t1-PB_Q,32));
    ubp=min(min(min(floord(PB_L-2,16),floord(PB_Q+PB_L-1,32)),floord(32*t1+PB_L+30,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,32*t1-32*t2),32*t2-PB_L+1);t3<=min(min(min(PB_Q,PB_L-2),16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3+1);t4<=min(32*t2+31,t3+PB_L-1);t4++) {
          A[(-2*t3+t4)+t3] = B[t3-1][(-2*t3+t4)][t3-1] * C[(-2*t3+t4)-1][t3] * C[(-2*t3+t4)][t3] + A[(-2*t3+t4)+t3-1] * 4;;
        }
      }
    }
  }
...
kernel_2122131111_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
