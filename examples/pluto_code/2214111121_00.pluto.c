...
void kernel_2214111121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 1) {
  ...
  for (t1=1;t1<=min(PB_P,PB_M-1);t1++) {
    A[0][t1][t1-1] = C[0 +t1] * 1;;
    for (t2=1;t2<=-t1+PB_M-1;t2++) {
      A[t2][t1][t1-1] = C[t2+t1] * 1;;
      B[t1] = B[t2-1] - 3;;
    }
    for (t2=-t1+PB_M;t2<=PB_M;t2++) {
      B[t1] = B[t2-1] - 3;;
    }
  }
  for (t1=max(1,PB_P+1);t1<=PB_M-1;t1++) {
    for (t2=1;t2<=PB_M;t2++) {
      B[t1] = B[t2-1] - 3;;
    }
  }
...
kernel_2214111121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
