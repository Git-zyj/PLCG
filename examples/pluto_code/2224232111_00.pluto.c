...
void kernel_2224232111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
if (PB_M >= 3) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    for (t2=0;t2<=floord(PB_M-3,32);t2++) {
      if ((t1 <= floord(PB_M-2,32)) && (t2 == 0)) {
        if (t1 == 0) {
          A[0 +2][0] = C[0][0] + 4;;
        }
        for (t4=max(1,32*t1);t4<=min(PB_M-2,32*t1+31);t4++) {
          A[0 +2][t4] = C[0][t4] + 4;;
          B[t4] = D[t4] + D[t4] - E[t4-1] - E[t4] - D[t4+1] + B[t4+1] * B[t4-1] - 1;;
        }
        if (t1 >= ceild(PB_M-32,32)) {
          A[0 +2][(PB_M-1)] = C[0][(PB_M-1)] + 4;;
        }
      }
...
kernel_2224232111_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
