...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 5)) {
  for (t1=max(-1,ceild(-PB_L-28,32));t1<=min(floord(PB_L-2,32),floord(PB_M-4,32));t1++) {
    lbp=max(0,t1);
    ubp=min(floord(PB_L-2,32),floord(32*t1+PB_L+28,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(1,32*t1);t3<=min(min(min(min(PB_L-2,PB_M-4),32*t1+62),32*t2+31),32*t1-32*t2+PB_L+29);t3++) {
        lbv=max(max(32*t2,t3),-32*t1+32*t2+t3-31);
        ubv=min(min(PB_L-2,32*t2+31),-32*t1+32*t2+t3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+1][(-t3+t4)+t3+1] = B[(-t3+t4)][t3] * C[t3] * D[t3+1][t3+1] - A[t3-1][(-t3+t4)+t3-1] * A[t3+3][(-t3+t4)+t3+1] * 5;;
        }
      }
    }
  }
