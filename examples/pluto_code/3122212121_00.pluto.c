...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L+PB_P-4,32);t2++) {
      for (t3=max(0,ceild(32*t2-PB_L-30,32));t3<=min(floord(PB_P-3,32),t2);t3++) {
        for (t4=32*t1;t4<=min(PB_L-1,32*t1+31);t4++) {
          for (t5=max(max(3,32*t2),32*t3+2);t5<=min(min(32*t2+31,PB_L+PB_P-4),32*t3+PB_L+30);t5++) {
            for (t6=max(max(1,32*t3),t5-PB_L+1);t6<=min(min(PB_P-3,32*t3+31),t5-2);t6++) {
              A[t4][(t5-t6)][t6] = B[(t5-t6)][t4][t6-1] + A[t4][(t5-t6)-2][t6+2] * 2;;
            }
          }
        }
      }
