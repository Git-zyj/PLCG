...
void kernel_2212231221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_N >= max(0,-PB_Q+4)) && (PB_Q >= 0)) {
  for (t1=0;t1<=min(floord(PB_N+PB_Q-2,32),floord(2*PB_N+3*PB_Q-8,64));t1++) {
    for (t2=0;t2<=min(min(floord(PB_Q,32),floord(-32*t1+PB_N+3*PB_Q-4,64)),floord(PB_N+4*PB_Q-4,128));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=min(1,PB_N-4);t3++) {
          A[t3+1] = C[t3] * C[t3] * C[t3] + A[t3+2] + A[t3+3] * 2;;
        }
      }
      if ((t2 >= 1) && (t2 <= floord(PB_Q-1,32))) {
        for (t3=max(2,32*t1);t3<=min(PB_Q-2,32*t1+31);t3++) {
          lbv=32*t2;
          ubv=min(PB_Q-1,32*t2+31);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t3][t4-1] = D[t4-1][t3+1] + E[t4-1][t3-1] - D[t3-1][t4] + B[t3-2][t4-1] + B[t3-1][t4-1] + 4;;
          }
        }
      }
      if (t2 == 0) {
        for (t3=max(2,32*t1);t3<=min(min(PB_N-4,PB_Q-2),32*t1+31);t3++) {
          A[t3+1] = C[t3] * C[t3] * C[t3] + A[t3+2] + A[t3+3] * 2;;
          lbv=1;
          ubv=min(31,PB_Q-1);
#pragma ivdep
#pragma vector always
      ...
      if (t2 == 0) {
        for (t3=max(max(2,32*t1),PB_Q-1);t3<=min(PB_N-4,32*t1+31);t3++) {
          A[t3+1] = C[t3] * C[t3] * C[t3] + A[t3+2] + A[t3+3] * 2;;
        }
      }
      if (t2 == 0) {
        for (t3=max(max(2,32*t1),PB_N-3);t3<=min(PB_Q-2,32*t1+31);t3++) {
          lbv=1;
          ubv=min(31,PB_Q-1);
#pragma ivdep
#pragma vector always
...
if (PB_Q <= -1) {
  for (t1=0;t1<=floord(PB_N-4,32);t1++) {
    for (t3=32*t1;t3<=min(PB_N-4,32*t1+31);t3++) {
      A[t3+1] = C[t3] * C[t3] * C[t3] + A[t3+2] + A[t3+3] * 2;;
    }
  }
}
...
kernel_2212231221_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
