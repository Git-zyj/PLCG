...
void kernel_2214132221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if (PB_M >= 4) {
  for (t2=3;t2<=PB_M-1;t2++) {
    A[2][t2-1] = C[t2-1][2 -2] + C[2 +1][t2-1] * B[2 +1][2 +2] * B[t2][t2+1] - 2;;
  }
  for (t1=3;t1<=PB_M-2;t1++) {
    for (t2=2;t2<=t1-1;t2++) {
      B[t1][t1+1] = C[t2][0] + C[t1][t2] * C[t1-1][t2-1] * C[t2][t1+2] + B[t2-2][t2-1] - B[t1-1][t1] - 3;;
    }
    for (t2=t1+1;t2<=PB_M-1;t2++) {
      A[t1][t2-1] = C[t2-1][t1-2] + C[t1+1][t2-1] * B[t1+1][t1+2] * B[t2][t2+1] - 2;;
    }
  }
  for (t2=2;t2<=PB_M-2;t2++) {
    B[(PB_M-1)][(PB_M-1)+1] = C[t2][0] + C[(PB_M-1)][t2] * C[(PB_M-1)-1][t2-1] * C[t2][(PB_M-1)+2] + B[t2-2][t2-1] - B[(PB_M-1)-1][(PB_M-1)] - 3;;
  }
}
/* End of CLooG code */
...
kernel_2214132221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
