...
void kernel_2214132221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_M >= 4) {
  for (t1=0;t1<=min(1,PB_M-4);t1++) {
    B[t1+2] = C[t1+2] + C[t1] * C[t1+2] - C[t1] + B[t1+1] * B[t1+3] - 3;;
  }
  for (t1=2;t1<=PB_M-4;t1++) {
    A[t1] = C[t1] + C[t1] + A[t1+1] - A[t1-2] * 6;;
    B[t1+2] = C[t1+2] + C[t1] * C[t1+2] - C[t1] + B[t1+1] * B[t1+3] - 3;;
  }
  for (t1=max(2,PB_M-3);t1<=PB_M-2;t1++) {
    A[t1] = C[t1] + C[t1] + A[t1+1] - A[t1-2] * 6;;
  }
}
/* End of CLooG code */
...
kernel_2214132221_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
