...
void kernel_2214211111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (t2=0;t2<=floord(PB_M-1,32);t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=min(31,PB_M-2);t3++) {
          B[t3+1] = C[t3] * 4;;
          for (t4=max(1,t3);t4<=min(31,PB_M-1);t4++) {
            A[t3][t4-1] = C[t4] * A[t3][t4] + 2;;
          }
        }
      }
      if (t2 >= 1) {
...
kernel_2214211111_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
