...
void kernel_2214211111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
/* Start of CLooG code */
if ((PB_L >= 1) && (PB_N >= 1)) {
  for (t1=1;t1<=min(PB_L,PB_N);t1++) {
    for (t2=t1;t2<=PB_L;t2++) {
      A[t1][t2] = C[t1-1][t2-1] * B[t1-1][1] * B[t1][1] * 4;;
      B[t1][1] = D[t2-1] + 6;;
    }
  }
}
...
kernel_2214211111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
