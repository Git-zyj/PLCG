...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,-t1-1);t2<=min(floord(PB_M-1,32),floord(-32*t1+PB_P-3,32));t2++) {
      for (t3=0;t3<=floord(PB_L-2,32);t3++) {
        for (t4=max(max(0,32*t1+1),32*t1+32*t2);t4<=min(min(PB_P-3,32*t1+PB_M+30),32*t1+32*t2+62);t4++) {
          for (t5=max(1,32*t3);t5<=min(PB_L-2,32*t3+31);t5++) {
            lbv=max(max(1,32*t2),-32*t1+t4-31);
            ubv=min(min(PB_M-1,-32*t1+t4),32*t2+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t5-1][t4+1][t6-1] = B[t6][t5][t4] + A[t5+1][t4+2][t6] - 1;;
            }
          }
        }
      }
