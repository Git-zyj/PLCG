...
if (PB_M >= 3) {
  if (PB_Q >= 0) {
    for (t1=0;t1<=floord(PB_M+PB_Q-2,32);t1++) {
      for (t2=0;t2<=min(min(floord(PB_M-1,32),floord(PB_M+PB_Q-3,32)),floord(-32*t1+PB_M+PB_Q-2,32));t2++) {
        for (t3=0;t3<=min(min(min(floord(PB_M-2,32),floord(-32*t1+PB_M+PB_Q-2,32)),floord(2*PB_M+PB_Q-6,64)),floord(-32*t1+2*PB_M+PB_Q-5,64));t3++) {
          if ((t2 == 0) && (t3 == 0)) {
            for (t4=max(1,32*t1);t4<=min(min(PB_M-2,PB_Q-1),32*t1+31);t4++) {
              B[t4] = B[t4+1] - B[t4-1] * 2;;
              for (t5=1;t5<=min(31,PB_M-1);t5++) {
                for (t6=1;t6<=min(31,PB_M-2);t6++) {
                  A[t5][t4][t4] = C[t5-1][t6+1][t4] * C[t4+1][t5][t4-1] - 3;;
                }
              }
            }
          }
          if (t2 >= -t3+1) {
            for (t4=max(1,32*t1);t4<=min(PB_Q-1,32*t1+31);t4++) {
              for (t5=max(1,32*t2);t5<=min(PB_M-1,32*t2+31);t5++) {
                for (t6=max(1,32*t3);t6<=min(PB_M-2,32*t3+31);t6++) {
                  A[t5][t4][t4] = C[t5-1][t6+1][t4] * C[t4+1][t5][t4-1] - 3;;
                }
              }
            }
          ...
          if ((t2 == 0) && (t3 == 0)) {
            for (t4=max(32*t1,PB_M-1);t4<=min(PB_Q-1,32*t1+31);t4++) {
              for (t5=1;t5<=min(31,PB_M-1);t5++) {
                for (t6=1;t6<=min(31,PB_M-2);t6++) {
                  A[t5][t4][t4] = C[t5-1][t6+1][t4] * C[t4+1][t5][t4-1] - 3;;
                }
          ...
          if ((t2 == 0) && (t3 == 0)) {
            for (t4=max(max(1,PB_Q),32*t1);t4<=min(PB_M-2,32*t1+31);t4++) {
              B[t4] = B[t4+1] - B[t4-1] * 2;;
            }
          }
        }
