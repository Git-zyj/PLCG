...
if (PB_M >= 2) {
  if (PB_N >= 0) {
    for (t1=0;t1<=min(floord(PB_N+PB_M-2,32),floord(PB_N+2*PB_M-5,32));t1++) {
      for (t2=max(0,ceild(32*t1-PB_M+2,32));t2<=min(min(min(floord(PB_N+PB_M-2,32),floord(32*t1+PB_M+30,32)),floord(-32*t1+2*PB_N+PB_M-2,32)),floord(-32*t1+3*PB_N+2*PB_M-5,64));t2++) {
        if (t2 >= 1) {
          for (t3=max(32*t1,32*t2-PB_M+1);t3<=min(min(PB_N-1,32*t1+31),32*t2+30);t3++) {
            for (t4=max(32*t2,t3+1);t4<=min(32*t2+31,t3+PB_M-1);t4++) {
              A[(-t3+t4)][(-t3+t4)] = C[(-t3+t4)-1][1] - D[t3][(-t3+t4)] * D[t3][(-t3+t4)] - C[(-t3+t4)][(-t3+t4)-1] + A[(-t3+t4)-1][(-t3+t4)-1] * 2;;
            }
          }
        }
        if ((PB_N >= 1) && (t1 == 0) && (t2 == 0)) {
          for (t4=1;t4<=min(31,PB_M-1);t4++) {
            A[t4][t4] = C[t4-1][1] - D[0][t4] * D[0][t4] - C[t4][t4-1] + A[t4-1][t4-1] * 2;;
          }
        }
        if ((t1 == 0) && (t2 == 0)) {
          for (t3=1;t3<=min(min(30,PB_M-2),PB_N-1);t3++) {
            B[t3] = E[t3+1] * F[t3] * B[t3-1] * 6;;
        ...
        if ((t1 == 0) && (t2 == 0)) {
          for (t3=PB_M-1;t3<=min(30,PB_N-1);t3++) {
            for (t4=t3+1;t4<=min(31,t3+PB_M-1);t4++) {
              A[(-t3+t4)][(-t3+t4)] = C[(-t3+t4)-1][1] - D[t3][(-t3+t4)] * D[t3][(-t3+t4)] - C[(-t3+t4)][(-t3+t4)-1] + A[(-t3+t4)-1][(-t3+t4)-1] * 2;;
            }
          }
        ...
        if ((t1 == 0) && (t2 == 0)) {
          for (t3=max(1,PB_N);t3<=min(30,PB_M-2);t3++) {
            B[t3] = E[t3+1] * F[t3] * B[t3-1] * 6;;
          }
        }
        if (t2 == 0) {
