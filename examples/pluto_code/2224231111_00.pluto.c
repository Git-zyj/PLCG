...
if (PB_N >= 3) {
  for (t1=0;t1<=floord(PB_N-2,32);t1++) {
    ...
    for (t2=0;t2<=floord(PB_L,32);t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=1;t3<=min(min(31,PB_L-2),PB_N-2);t3++) {
          A[t3] = C[t3] + A[t3-1] + 3;;
          lbv=t3;
          ubv=min(31,PB_L-2);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t3][t4] = D[t3][t4] - E[t3+1][0] - D[t3][t4+1] * D[t3][t4] - D[t4][t3] - B[t3+1][t4] - 2;;
          }
        }
      }
      if (t2 == 0) {
        for (t3=max(32,32*t1);t3<=min(PB_N-2,32*t1+31);t3++) {
          A[t3] = C[t3] + A[t3-1] + 3;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(1,PB_L-1);t3<=min(31,PB_N-2);t3++) {
          A[t3] = C[t3] + A[t3-1] + 3;;
        }
      }
      if ((t2 >= 1) && (t2 <= floord(PB_L-2,32))) {
        lbp=max(1,32*t1);
        ubp=min(min(min(PB_L-2,PB_N-2),32*t1+31),32*t2+31);
