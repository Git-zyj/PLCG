...
void kernel_3124131221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_L >= 5) && (PB_P >= 4)) {
  for (t1=0;t1<=min(floord(PB_L-2,16),floord(PB_P,32));t1++) {
    lbp=max(ceild(t1,2),ceild(64*t1-PB_P+1,64));
    ubp=min(floord(PB_L-1,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(4,32*t2),32*t1-32*t2+1);t3<=min(min(PB_L-1,32*t2+31),-32*t1+32*t2+PB_P);t3++) {
        lbv=32*t1-32*t2;
        ubv=min(min(-t3+PB_P,t3-1),32*t1-32*t2+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4+t3-1] = B[t4+t3][t4] + B[t3][t4+1] - B[t4][t3] - A[t3-1][t4+t3-2] * A[t3-2][t4+t3-4] + 4;;
        }
      }
    }
  }
...
kernel_3124131221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
