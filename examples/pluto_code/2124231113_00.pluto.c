...
void kernel_2124231113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 1) && (PB_M >= 3) && (PB_Q >= 2)) {
  lbp=0;
  ubp=min(floord(PB_L,32),floord(PB_M-2,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(floord(PB_M-2,32),floord(PB_Q-1,32)),floord(-32*t1+PB_M-1,32));t2++) {
      for (t3=max(1,32*t1);t3<=min(min(min(PB_L,PB_M-2),32*t1+31),-32*t2+PB_M-1);t3++) {
        for (t4=max(1,32*t2);t4<=min(min(PB_Q-1,32*t2+31),-t3+PB_M-1);t4++) {
          A[t3-1][t4] = B[t4][t4] + B[t4+t3][t4] - C[t3][t4] * A[t3-1][t4-1] + 1;;
        }
      }
    }
  }
...
kernel_2124231113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
