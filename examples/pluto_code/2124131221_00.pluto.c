...
void kernel_2124131221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_L >= 4) {
  for (t1=1;t1<=PB_L-3;t1++) {
    for (t2=0;t2<=PB_L-1;t2++) {
      A[t1+1][0] = B[t1+1][t2] * B[t2][t1] - B[t1+1][t1] - A[t1-1][0] - A[t1+2][0] - 6;;
    }
  }
}
/* End of CLooG code */
...
kernel_2124131221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
