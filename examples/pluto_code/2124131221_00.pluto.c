...
void kernel_2124131221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t2;t3<=min(PB_M-3,32*t2+31);t3++) {
        for (t4=32*t1-32*t2;t4<=min(PB_M-2,32*t1-32*t2+31);t4++) {
          A[t3][t4] = B[t4+1][t3][t3+1] * C[t4][t3] - C[t4+1][t3+1] - A[t3][t4+1] - A[t3+2][t4] + 6;;
        }
      }
    }
  }
...
kernel_2124131221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
