...
void kernel_2112232121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_L >= 5) {
  for (t1=2;t1<=PB_L-3;t1++) {
    for (t2=0;t2<=PB_L-3;t2++) {
      A[t1][t1] = B[t2] + C[t1-2][t2+2] * C[t2+2][0] - A[t1+2][t1+2] - 4;;
    }
  }
}
/* End of CLooG code */
...
kernel_2112232121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
