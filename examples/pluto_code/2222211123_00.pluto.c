...
void kernel_2222211123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma vector always
      ...
      for (t3=max(2,32*t2);t3<=min(PB_N,32*t2+31);t3++) {
        lbv=max(1,32*t1);
        ubv=min(PB_L-1,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3-1][t4] = B[t3-1][t4] + 1;;
          B[(t3-1)-1][t4] = C[t4-1][(t3-1)] - C[t4+1][(t3-1)] + A[(t3-1)+1][t4] + 5;;
        }
      }
      if (t2 >= ceild(PB_N-30,32)) {
        lbv=max(1,32*t1);
        ubv=min(PB_L-1,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          B[PB_N-1][t4] = C[t4-1][PB_N] - C[t4+1][PB_N] + A[PB_N+1][t4] + 5;;
        }
      }
    }
  }
...
kernel_2222211123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
