...
void kernel_3224211213_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
if (PB_N >= 3) {
  if (PB_M >= 1) {
    for (t2=0;t2<=PB_N-2;t2++) {
      for (t4=0;t4<=PB_M-1;t4++) {
        B[t2+1][1] = B[t2+1][1] + 4;;
      }
    }
  }
  for (t1=1;t1<=min(PB_M-1,PB_N-3);t1++) {
    for (t2=0;t2<=t1;t2++) {
      for (t4=t1;t4<=PB_M-1;t4++) {
        B[t2+1][1] = B[t2+1][1] + 4;;
      }
    }
    for (t2=t1+1;t2<=min(2*t1,PB_N-2);t2++) {
      for (t4=t1;t4<=PB_M-1;t4++) {
        B[t2+1][1] = B[t2+1][1] + 4;;
      }
      B[t1+1][1] = C[t1][(-t1+t2-1)+1][0] - B[t1+2][1] * 3;;
    }
    for (t2=2*t1+1;t2<=PB_N-2;t2++) {
      for (t4=t1;t4<=PB_M-1;t4++) {
        B[t2+1][1] = B[t2+1][1] + 4;;
      }
    }
    for (t2=PB_N-1;t2<=2*t1;t2++) {
      B[t1+1][1] = C[t1][(-t1+t2-1)+1][0] - B[t1+2][1] * 3;;
    }
  }
  for (t1=max(1,PB_M);t1<=PB_N-3;t1++) {
...
kernel_3224211213_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
