...
void kernel_2214232221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
if (PB_M >= 5) {
  for (t1=0;t1<=floord(PB_M-3,32);t1++) {
    for (t2=0;t2<=min(floord(PB_M-3,16),floord(32*t1+PB_M+28,32));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(31,PB_M-3);t3++) {
          A[t3] = C[t3] * C[t3-1] * 6;;
          for (t4=t3;t4<=min(31,t3+PB_M-3);t4++) {
            B[t3][(-t3+t4)] = D[(-t3+t4)][t3-2] + D[(-t3+t4)+2][t3] - D[(-t3+t4)][t3] - E[(-t3+t4)][t3] + B[t3-1][(-t3+t4)+1] * B[t3-1][(-t3+t4)] - B[t3][(-t3+t4)+1] - B[t3+2][(-t3+t4)+2] + 2;;
          }
        }
      }
      if (t2 == 0) {
      ...
      if (t2 >= 1) {
        for (t3=max(max(2,32*t1),32*t2-PB_M+3);t3<=min(min(PB_M-3,32*t1+31),32*t2+31);t3++) {
          for (t4=max(32*t2,t3);t4<=min(32*t2+31,t3+PB_M-3);t4++) {
            B[t3][(-t3+t4)] = D[(-t3+t4)][t3-2] + D[(-t3+t4)+2][t3] - D[(-t3+t4)][t3] - E[(-t3+t4)][t3] + B[t3-1][(-t3+t4)+1] * B[t3-1][(-t3+t4)] - B[t3][(-t3+t4)+1] - B[t3+2][(-t3+t4)+2] + 2;;
          }
        }
      }
...
kernel_2214232221_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
