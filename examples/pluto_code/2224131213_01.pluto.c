...
void kernel_2224131213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 1) {
  for (t1=2;t1<=PB_L-1;t1++) {
    A[t1][t1-1] = C[t1+1] + C[t1] + C[t1+1] + C[t1] + A[t1+1][t1] + A[t1-1][t1-2] - 5;;
    A[(t1-1)][(t1-1)-1] = C[(t1-1)] + 3;;
  }
  for (t1=max(2,PB_L);t1<=PB_L+1;t1++) {
    A[(t1-1)][(t1-1)-1] = C[(t1-1)] + 3;;
  }
}
/* End of CLooG code */
...
kernel_2224131213_01(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
