...
void kernel_3222131113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      if (32*t1 == 32*t2+PB_L) {
        for (t3=max(1,32*t1-PB_L);t3<=32*t1-PB_L+31;t3++) {
          if (PB_L%32 == 0) {
            B[t3][PB_L-1] = C[t3][PB_L-1] + C[PB_L-1][t3] + B[t3-1][PB_L-2] * 5;;
          }
        }
      }
...
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          B[1][t4-1] = C[1][t4-1] + C[t4-1][1] + B[1 -1][t4-2] * 5;;
        }
      }
...
#pragma vector always
            for (t4=lbv;t4<=ubv;t4++) {
              A[t4][t3-1] = C[t3][t4] + C[t4][t3] - C[t3][t4] + A[t4][t3-2] * 3;;
            }
          }
...
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t4][t3-1] = C[t3][t4] + C[t4][t3] - C[t3][t4] + A[t4][t3-2] * 3;;
            B[t3][t4-1] = C[t3][t4-1] + C[t4-1][t3] + B[t3-1][t4-2] * 5;;
          }
          if (t1 >= ceild(32*t2+PB_L-31,32)) {
            B[t3][PB_L-1] = C[t3][PB_L-1] + C[PB_L-1][t3] + B[t3-1][PB_L-2] * 5;;
          }
        }
      }
...
kernel_3222131113_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
