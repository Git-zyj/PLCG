...
void kernel_2224212121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_L >= 2) {
  for (t1=0;t1<=floord(PB_L-2,8);t1++) {
    ...
    ubp=min(floord(3*t1+2,4),floord(3*PB_L-4,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(32*t2+1,3),32*t1-32*t2);t3<=min(min(min(32*t2,PB_L-1),16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,3*t3-1);t4++) {
          B[(-2*t3+t4)] = D[(-2*t3+t4)][t3] - B[(-2*t3+t4)+1] - B[(-2*t3+t4)+2] * 4;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=1;t3<=min(2,PB_L-1);t3++) {
          for (t4=2*t3;t4<=3*t3-1;t4++) {
            B[(-2*t3+t4)] = D[(-2*t3+t4)][t3] - B[(-2*t3+t4)+1] - B[(-2*t3+t4)+2] * 4;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=3;t3<=min(15,PB_L-1);t3++) {
          for (t4=2;t4<=t3-1;t4++) {
            A[t3][t4] = C[t4-2] - 4;;
          }
      ...
      for (t3=max(max(32*t1-32*t2,16*t2+16),32*t2+1);t3<=min(PB_L-1,32*t1-32*t2+31);t3++) {
        for (t4=max(2,32*t2);t4<=min(32*t2+31,t3-1);t4++) {
          A[t3][t4] = C[t4-2] - 4;;
        }
      }
    }
...
kernel_2224212121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
