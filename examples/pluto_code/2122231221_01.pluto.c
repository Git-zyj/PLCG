...
/* Start of CLooG code */
if (PB_M >= 6) {
  for (t1=0;t1<=floord(5*PB_M-20,32);t1++) {
    lbp=ceild(4*t1,5);
    ubp=min(min(floord(PB_M-4,8),floord(96*t1+PB_M+89,128)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,ceild(32*t2-PB_M+4,3)),32*t1-32*t2);t3<=min(min(PB_M-4,8*t2+7),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,4*t3);t4<=min(32*t2+31,3*t3+PB_M-4);t4++) {
          A[(-4*t3+t4)+t3+1] = B[t3+1][(-4*t3+t4)] * B[(-4*t3+t4)+t3+1][(-4*t3+t4)] - C[(-4*t3+t4)][t3] - A[(-4*t3+t4)+t3-2] + A[(-4*t3+t4)+t3+3] * 5;;
        }
      }
    }
  }
