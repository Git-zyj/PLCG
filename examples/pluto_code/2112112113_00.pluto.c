...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,ceild(32*t1-PB_L-31,32));t2<=min(floord(PB_L+1,32),t1);t2++) {
      for (t3=max(max(2,32*t2),32*t1-PB_L);t3<=min(min(PB_L+1,32*t1+29),32*t2+31);t3++) {
        lbv=max(32*t1,t3+2);
        ubv=min(32*t1+31,t3+PB_L);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-t3+t4)-2] = B[(-t3+t4)-1][t3-2] * A[t3-1][(-t3+t4)-1] + 6;;
        }
      }
    }
  }
