...
void kernel_2212111111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
  int t1;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
for (t1=1;t1<=min(PB_M,PB_N-1);t1++) {
  A[t1-1] = 3;;
  B[t1] = C[t1+1] * B[t1-1] * 4;;
}
for (t1=max(1,PB_N);t1<=PB_M;t1++) {
  A[t1-1] = 3;;
}
for (t1=max(1,PB_M+1);t1<=PB_N-1;t1++) {
  B[t1] = C[t1+1] * B[t1-1] * 4;;
}
/* End of CLooG code */
...
kernel_2212111111_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
