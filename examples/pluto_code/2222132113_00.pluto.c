...
void kernel_2222132113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_L >= 4) {
  for (t1=0;t1<=floord(PB_L-2,32);t1++) {
    for (t2=max(0,ceild(32*t1-PB_L+4,16));t2<=min(floord(3*PB_L-6,32),floord(64*t1+PB_L+60,32));t2++) {
      if (t2 >= 1) {
        for (t3=max(max(1,ceild(32*t2-PB_L+2,2)),32*t1);t3<=min(min(PB_L-2,32*t1+31),16*t2+14);t3++) {
          for (t4=max(32*t2,2*t3+2);t4<=min(32*t2+31,2*t3+PB_L-2);t4++) {
            B[t3+1][(-2*t3+t4)-2] = D[(-2*t3+t4)][t3] * B[(-2*t3+t4)+1][t3-1] * 1;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t4=4;t4<=min(31,PB_L);t4++) {
          B[1 +1][(t4-2)-2] = D[(t4-2)][1] * B[(t4-2)+1][1 -1] * 1;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(14,PB_L-4);t3++) {
          A[t3][t3+2] = C[t3-2] * C[t3+2] + C[t3] * C[t3] - C[t3] * A[t3+1][t3+3] - 2;;
          for (t4=2*t3+2;t4<=min(31,2*t3+PB_L-2);t4++) {
            B[t3+1][(-2*t3+t4)-2] = D[(-2*t3+t4)][t3] * B[(-2*t3+t4)+1][t3-1] * 1;;
          }
        }
      }
      if (t2 == 0) {
        for (t3=max(15,32*t1);t3<=min(PB_L-4,32*t1+31);t3++) {
          A[t3][t3+2] = C[t3-2] * C[t3+2] + C[t3] * C[t3] - C[t3] * A[t3+1][t3+3] - 2;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(2,PB_L-3);t3<=min(14,PB_L-2);t3++) {
          for (t4=2*t3+2;t4<=min(31,2*t3+PB_L-2);t4++) {
            B[t3+1][(-2*t3+t4)-2] = D[(-2*t3+t4)][t3] * B[(-2*t3+t4)+1][t3-1] * 1;;
          }
        }
...
kernel_2222132113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
