...
void kernel_3224211113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_L >= 3) {
  for (t1=1;t1<=PB_L-2;t1++) {
    A[t1][t1] = C[t1+1] * 3;;
    A[t1+1][t1+1] = D[t1] + A[t1-1][t1-1] - 3;;
  }
}
/* End of CLooG code */
...
kernel_3224211113_00(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
