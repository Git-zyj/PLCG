...
void kernel_3124111212_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
for (t1=lbp;t1<=ubp;t1++) {
  for (t2=0;t2<=floord(PB_P,32);t2++) {
    for (t3=t2;t3<=min(floord(PB_P+PB_M-1,32),floord(32*t2+PB_M+30,32));t3++) {
      for (t4=max(max(2,32*t2),32*t3-PB_M+1);t4<=min(min(PB_P,32*t2+31),32*t3+30);t4++) {
        for (t5=max(32*t3,t4+1);t5<=min(32*t3+31,t4+PB_M-1);t5++) {
          lbv=32*t1;
          ubv=min(PB_M-1,32*t1+31);
#pragma ivdep
#pragma vector always
          for (t6=lbv;t6<=ubv;t6++) {
            A[t6][(-t4+t5)-1][t4-1] = B[(-t4+t5)][t4-1][t6] * A[t6][(-t4+t5)][t4-2] * A[t6][(-t4+t5)-1][t4-2] * 3;;
          }
        }
      }
    }
...
kernel_3124111212_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
