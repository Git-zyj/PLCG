...
void kernel_2224131113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (t2=0;t2<=floord(PB_M-1,32);t2++) {
      if (t2 == 0) {
        for (t3=32*t1;t3<=min(PB_M-2,32*t1+31);t3++) {
          A[t3][1] = C[t3] + C[t3] + D[t3+1][0] - C[t3] + A[t3+1][1] * 6;;
          B[0][t3] = D[t3][0] + D[0][t3] * 5;;
          lbv=1;
          ubv=min(31,PB_M-1);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t4][t3] = D[t3][t4] + D[t4][t3] * 5;;
          }
        }
      }
      if (t2 >= 1) {
...
#pragma vector always
      ...
      if ((t1 >= ceild(PB_M-32,32)) && (t2 == 0)) {
        lbv=0;
        ubv=min(31,PB_M-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          B[t4][(PB_M-1)] = D[(PB_M-1)][t4] + D[t4][(PB_M-1)] * 5;;
        }
      }
    }
  }
...
kernel_2224131113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
