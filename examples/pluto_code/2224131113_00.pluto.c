...
void kernel_2224131113_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma vector always
            for (t4=lbv;t4<=ubv;t4++) {
              B[t3][(t4-1)] = C[t3][(t4-1)] * C[t3][(t4-1)] + C[(t4-1)][t3] * 5;;
            }
          }
        }
        if ((PB_M >= 2) && (t1 <= floord(PB_L,32)) && (t1 >= ceild(PB_M,32))) {
          for (t3=32*t2;t3<=min(PB_L-1,32*t2+31);t3++) {
            lbv=32*t1;
            ubv=min(PB_L,32*t1+31);
#pragma ivdep
#pragma vector always
        ...
        if ((PB_M >= 2) && (t1 <= min(floord(PB_L,32),floord(PB_M-1,32))) && (t2 == 0)) {
          lbv=max(1,32*t1);
          ubv=min(PB_L,32*t1+31);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[0][(t4-1)] = C[0][(t4-1)] * C[0][(t4-1)] + C[(t4-1)][0] * 5;;
          }
        }
        if ((t1 <= floord(PB_M-1,32)) && (t1 >= ceild(PB_L+1,32))) {
          for (t3=max(1,32*t2);t3<=min(PB_L-1,32*t2+31);t3++) {
            lbv=32*t1;
            ubv=min(PB_M-1,32*t1+31);
#pragma ivdep
#pragma vector always
            for (t4=lbv;t4<=ubv;t4++) {
              B[t3][t4-1] = C[t4][t3] * C[t4][0] + D[t3] + B[t3-1][t4-1] - 5;;
            }
          }
        }
        if ((PB_M >= 2) && (t1 <= min(floord(PB_L,32),floord(PB_M-1,32)))) {
          for (t3=max(1,32*t2);t3<=min(PB_L-1,32*t2+31);t3++) {
            lbv=max(1,32*t1);
            ubv=min(min(PB_L,PB_M-1),32*t1+31);
#pragma ivdep
#pragma vector always
            for (t4=lbv;t4<=ubv;t4++) {
              B[t3][(t4-1)] = C[t3][(t4-1)] * C[t3][(t4-1)] + C[(t4-1)][t3] * 5;;
...
kernel_2224131113_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
