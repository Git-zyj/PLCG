...
if (PB_L >= 2) {
  for (t1=0;t1<=floord(PB_L-1,32);t1++) {
    ...
    for (t2=0;t2<=floord(PB_M,32);t2++) {
      if (t2 == 0) {
        for (t3=max(1,32*t1);t3<=min(min(PB_L-1,PB_M-2),32*t1+31);t3++) {
          B[0 +t3][t3] = C[0 +1][t3] * B[0 +t3-1][t3-1] - B[0 +t3+1][t3] * 1;;
          A[t3] = A[t3+1] * 3;;
          for (t4=1;t4<=min(31,-t3+PB_M-2);t4++) {
            B[t4+t3][t3] = C[t4+1][t3] * B[t4+t3-1][t3-1] - B[t4+t3+1][t3] * 1;;
          }
        }
      }
      if (t2 == 0) {
        for (t3=max(max(1,32*t1),PB_M-1);t3<=min(PB_L-1,32*t1+31);t3++) {
          A[t3] = A[t3+1] * 3;;
        }
      }
      if (t2 >= 1) {
        for (t3=max(1,32*t1);t3<=min(min(PB_L-1,32*t1+31),-32*t2+PB_M-2);t3++) {
