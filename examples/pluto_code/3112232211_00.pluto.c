...
void kernel_3112232211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_L >= 3) {
  for (t1=3;t1<=PB_L;t1++) {
    A[t1-2] = B[t1] + C[t1-1][0] * D[t1] * A[t1-3] - A[t1-1] * 2;;
  }
}
/* End of CLooG code */
...
kernel_3112232211_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
