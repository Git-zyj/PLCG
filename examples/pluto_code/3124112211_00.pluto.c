...
void kernel_3124112211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_L >= 3) && (PB_M >= 3)) {
  for (t1=2;t1<=min(PB_L-1,PB_M-1);t1++) {
    for (t2=0;t2<=-t1+PB_M-1;t2++) {
      A[t1-1][1] = B[t2+t1][0] * A[t1][1] * A[t1-2][1] + 3;;
    }
  }
}
/* End of CLooG code */
...
kernel_3124112211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
