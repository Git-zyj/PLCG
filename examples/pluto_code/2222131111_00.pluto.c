...
void kernel_2222131111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
    ...
    for (t2=0;t2<=floord(PB_M-2,32);t2++) {
      if (t1 == 0) {
        for (t4=max(1,32*t2);t4<=min(PB_M-2,32*t2+31);t4++) {
          B[t4][0] = D[0 +t4][t4][0] * E[0 +t4] - C[0][t4] + C[0 +1][t4] - C[0][t4-1] - B[t4+1][0] - 6;;
        }
      }
      for (t3=max(1,32*t1);t3<=min(min(PB_M-2,32*t1+31),-32*t2+PB_M-1);t3++) {
        for (t4=max(1,32*t2);t4<=min(32*t2+31,-t3+PB_M-1);t4++) {
          A[t4-1][t3] = C[t3-1][t4] - A[t4][t3] + 1;;
          B[t4][t3] = D[t3+t4][t4][0] * E[t3+t4] - C[t3][t4] + C[t3+1][t4] - C[t3][t4-1] - B[t4+1][t3] - 6;;
        }
        for (t4=-t3+PB_M;t4<=min(PB_M-2,32*t2+31);t4++) {
          A[t4-1][t3] = C[t3-1][t4] - A[t4][t3] + 1;;
        }
      }
      if ((t1 <= floord(PB_M-1,32)) && (t1 >= ceild(PB_M-32,32)) && (t2 == 0)) {
        for (t4=1;t4<=min(31,PB_M-2);t4++) {
          A[t4-1][(PB_M-1)] = C[(PB_M-1)-1][t4] - A[t4][(PB_M-1)] + 1;;
        }
      }
      for (t3=max(32*t1,-32*t2+PB_M);t3<=min(PB_M,32*t1+31);t3++) {
        for (t4=max(1,32*t2);t4<=min(PB_M-2,32*t2+31);t4++) {
          A[t4-1][t3] = C[t3-1][t4] - A[t4][t3] + 1;;
        }
      }
...
kernel_2222131111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
