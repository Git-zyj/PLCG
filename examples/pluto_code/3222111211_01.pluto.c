...
void kernel_3222111211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
for (t1=2;t1<=min(PB_M-2,PB_N-1);t1++) {
  A[t1-1] = A[t1-2] + A[t1] * 2;;
  B[t1] = C[t1-1] + B[t1+1] - 2;;
}
for (t1=max(2,PB_M-1);t1<=PB_N-1;t1++) {
  A[t1-1] = A[t1-2] + A[t1] * 2;;
}
for (t1=max(2,PB_N);t1<=PB_M-2;t1++) {
  B[t1] = C[t1-1] + B[t1+1] - 2;;
}
/* End of CLooG code */
...
kernel_3222111211_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
