...
void kernel_2112212111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
    ...
    for (t2=0;t2<=floord(PB_M-2,32);t2++) {
      for (t3=32*t1;t3<=(min(PB_M-1,32*t1+31))-7;t3+=8) {
        for (t4=max(2,32*t2);t4<=min(PB_M-2,32*t2+31);t4++) {
          A[t3][t4] = B[t4-2] + A[t3][t4+1] + 3;;
          A[(t3+1)][t4] = B[t4-2] + A[(t3+1)][t4+1] + 3;;
          A[(t3+2)][t4] = B[t4-2] + A[(t3+2)][t4+1] + 3;;
          A[(t3+3)][t4] = B[t4-2] + A[(t3+3)][t4+1] + 3;;
          A[(t3+4)][t4] = B[t4-2] + A[(t3+4)][t4+1] + 3;;
          A[(t3+5)][t4] = B[t4-2] + A[(t3+5)][t4+1] + 3;;
          A[(t3+6)][t4] = B[t4-2] + A[(t3+6)][t4+1] + 3;;
          A[(t3+7)][t4] = B[t4-2] + A[(t3+7)][t4+1] + 3;;
        }
      }
...
kernel_2112212111_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
