...
void kernel_3114132111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if (PB_M >= 3) {
  lbp=ceild(-PB_M-28,32);
  ubp=floord(PB_M-3,32);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,-t1-1);t2<=floord(-32*t1+PB_M-3,64);t2++) {
      for (t3=max(0,32*t1+32*t2);t3<=min(min(floord(32*t1+PB_M+28,2),32*t1+32*t2+62),-32*t2+PB_M-3);t3++) {
        lbv=max(32*t2,-32*t1+t3-31);
        ubv=min(min(-32*t1+t3,32*t2+31),-t3+PB_M-3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4+t3] = B[t3][t4] - B[t4][t3+2] * B[t4][0] + A[t3+1][t4+t3+2] + 6;;
        }
      }
    }
  }
...
kernel_3114132111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
