...
void kernel_3114132111_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_P-1,32);t2++) {
      for (t3=t2;t3<=min(floord(PB_P-1,16),floord(32*t2+PB_P+30,32));t3++) {
        for (t4=max(2,32*t1);t4<=min(PB_P-1,32*t1+31);t4++) {
          for (t5=max(32*t2,32*t3-PB_P+1);t5<=min(min(PB_P-1,32*t2+31),32*t3+30);t5++) {
            for (t6=max(32*t3,t5+1);t6<=min(32*t3+31,t5+PB_P-1);t6++) {
              A[t4+1][t4][(-t5+t6)] = B[t4-1][t5][t5] - B[t5][t4-2][(-t5+t6)] + B[t4][t4][(-t5+t6)] * A[t4+1][t4][(-t5+t6)-1] - 2;;
            }
          }
        }
      }
...
kernel_3114132111_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
