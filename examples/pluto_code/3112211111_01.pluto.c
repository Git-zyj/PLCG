...
void kernel_3112211111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_M >= 3) {
  for (t1=0;t1<=floord(3*PB_M-9,32);t1++) {
    lbp=ceild(2*t1,3);
    ubp=min(min(floord(PB_M-3,16),floord(32*t1+PB_M+29,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_M+2);t3<=min(min(PB_M-3,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3+1);t4<=min(32*t2+31,t3+PB_M-2);t4++) {
          A[(-2*t3+t4)+t3][0] = B[(-2*t3+t4)-1][t3] + A[(-2*t3+t4)+t3+1][0] - 4;;
        }
      }
    }
  }
...
kernel_3112211111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
