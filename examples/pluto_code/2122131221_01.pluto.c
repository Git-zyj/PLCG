...
void kernel_2122131221_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 3)) {
  for (t1=0;t1<=min(floord(PB_L-2,16),floord(PB_M-1,32));t1++) {
    lbp=max(max(0,ceild(32*t1-PB_L+1,32)),ceild(32*t1-PB_M+2,32));
    ubp=min(min(floord(PB_L-2,32),floord(PB_M-2,32)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(1,32*t1-32*t2);t3<=min(min(min(PB_L-1,PB_M-2),32*t1-32*t2+31),-32*t2+PB_M-1);t3++) {
        for (t4=max(1,32*t2);t4<=min(min(PB_L-2,32*t2+31),-t3+PB_M-1);t4++) {
          A[t4+t3][t3][t4] = B[t3][t4+1] - C[t3] * B[t4][t3] + A[t4+t3-1][t3][t4-1] - A[t4+t3-1][t3-1][t4] - 3;;
        }
      }
    }
  }
...
kernel_2122131221_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
