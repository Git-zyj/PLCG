...
void kernel_3214212211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=ceild(t2-1,2);t3<=min(floord(PB_M-2,32),t2);t3++) {
        for (t4=max(max(3,32*t3),16*t2+1);t4<=min(min(PB_M-2,32*t2+29),32*t3+31);t4++) {
          lbv=max(32*t2,t4+2);
          ubv=min(32*t2+31,2*t4-1);
#pragma ivdep
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            B[t4][(-t4+t5)-1] = A[(-t4+t5)] + B[(-t4+t5)-1][t4-1] - B[t4+1][(-t4+t5)-2] - 5;;
          }
        }
      }
    }
...
kernel_3214212211_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
