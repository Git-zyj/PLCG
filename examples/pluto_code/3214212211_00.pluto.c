...
void kernel_3214212211_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
if (PB_M >= 2) {
  if ((PB_L >= 2) && (PB_Q <= 3)) {
    for (t1=1;t1<=PB_M-1;t1++) {
      for (t2=0;t2<=PB_L-2;t2++) {
        A[t2][t1][t2] = C[t2][t1] * A[t2][t1-1][t2] + A[t2+1][t1][t2+1] + 3;;
      }
    }
  }
  if ((PB_L >= 2) && (PB_Q >= 4)) {
  ...
  if ((PB_L <= 1) && (PB_Q >= 4)) {
    for (t1=3;t1<=PB_M-1;t1++) {
      for (t2=0;t2<=PB_Q-4;t2++) {
        for (t3=0;t3<=PB_Q-4;t3++) {
          B[t1-2][t2][t2+2] = D[t1][t2][t3] * B[t1-3][t3+1][t3+3] * B[t1-2][t2+1][t2+3] - 6;;
        }
      }
    }
  }
  if ((PB_L >= 2) && (PB_Q >= 4)) {
    for (t1=3;t1<=PB_M-1;t1++) {
      for (t2=0;t2<=min(PB_L-2,PB_Q-4);t2++) {
        A[t2][t1][t2] = C[t2][t1] * A[t2][t1-1][t2] + A[t2+1][t1][t2+1] + 3;;
        for (t3=0;t3<=PB_Q-4;t3++) {
          B[t1-2][t2][t2+2] = D[t1][t2][t3] * B[t1-3][t3+1][t3+3] * B[t1-2][t2+1][t2+3] - 6;;
        }
      }
      for (t2=PB_Q-3;t2<=PB_L-2;t2++) {
        A[t2][t1][t2] = C[t2][t1] * A[t2][t1-1][t2] + A[t2+1][t1][t2+1] + 3;;
      }
      for (t2=PB_L-1;t2<=PB_Q-4;t2++) {
        for (t3=0;t3<=PB_Q-4;t3++) {
          B[t1-2][t2][t2+2] = D[t1][t2][t3] * B[t1-3][t3+1][t3+3] * B[t1-2][t2+1][t2+3] - 6;;
        }
      }
...
kernel_3214212211_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
