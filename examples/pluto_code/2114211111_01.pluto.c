...
void kernel_2114211111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
    ...
    for (t2=0;t2<=floord(PB_L-1,32);t2++) {
      for (t3=32*t1;t3<=min(PB_L-1,32*t1+31);t3++) {
        for (t4=max(1,32*t2);t4<=min(PB_L-1,32*t2+31);t4++) {
          A[t3][t4] = B[t4][t3][t4] * A[t3][t4-1] + 6;;
        }
      }
    }
  }
...
kernel_2114211111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
