...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_P-4,32);t2++) {
      for (t3=0;t3<=min(floord(PB_P-5,32),t2);t3++) {
        for (t4=max(1,32*t1);t4<=min(PB_L-1,32*t1+31);t4++) {
          for (t5=max(1,32*t3);t5<=min(min(PB_P-5,32*t2+30),32*t3+31);t5++) {
            lbv=max(32*t2,t5+1);
            ubv=min(PB_P-4,32*t2+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t6+1][t4][t5+2] = B[t6][t5][t4] + C[t5][t4][t5-1] * D[t4][t4] - A[t5+2][t4-1][t6+3] + 5;;
            }
          }
        }
      }
