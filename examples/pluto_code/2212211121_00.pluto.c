...
void kernel_2212211121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
  ...
  for (t2=lbp;t2<=ubp;t2++) {
    for (t4=0;t4<=floord(PB_L-3,32);t4++) {
      for (t5=32*t2;t5<=min(PB_L-1,32*t2+31);t5++) {
        for (t7=max(1,32*t4);t7<=min(PB_L-3,32*t4+31);t7++) {
          A[t5][t7] = C[t7-1][t5] - A[t5][t7+2] - 6;;
        }
      }
    }
  }
...
if (PB_N >= 2) {
  for (t2=0;t2<=PB_N-1;t2++) {
    for (t4=-PB_N+1;t4<=-1;t4++) {
      B[-t4][-t4] = B[-t4-1][-t4-1] * 5;;
    }
  }
}
/* End of CLooG code */
...
kernel_2212211121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
