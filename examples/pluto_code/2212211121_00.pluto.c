...
void kernel_2212211121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_M >= 1) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    for (t2=0;t2<=min(floord(PB_M-1,16),floord(32*t1+PB_M+30,32));t2++) {
      if (t2 >= 1) {
        for (t3=max(max(2,32*t1),32*t2-PB_M+1);t3<=min(min(PB_M-1,32*t1+31),32*t2+29);t3++) {
          lbp=max(32*t2,t3+2);
          ubp=min(32*t2+31,t3+PB_M-1);
#pragma omp parallel for private(lbv,ubv)
          for (t4=lbp;t4<=ubp;t4++) {
            A[t3][(-t3+t4)] = C[(-t3+t4)][t3] * A[t3-2][(-t3+t4)-2] + A[t3-2][(-t3+t4)+1] - 5;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
      ...
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(29,PB_M-1);t3++) {
          B[t3] = D[t3] + 4;;
          lbp=t3+2;
          ubp=min(31,t3+PB_M-1);
#pragma omp parallel for private(lbv,ubv)
          for (t4=lbp;t4<=ubp;t4++) {
            A[t3][(-t3+t4)] = C[(-t3+t4)][t3] * A[t3-2][(-t3+t4)-2] + A[t3-2][(-t3+t4)+1] - 5;;
          }
      ...
      if (t2 == 0) {
        for (t3=max(30,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
          B[t3] = D[t3] + 4;;
        }
      }
    }
...
kernel_2212211121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
