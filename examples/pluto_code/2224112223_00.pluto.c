...
void kernel_2224112223_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if (PB_M >= 2) {
  for (t2=0;t2<=min(PB_L-1,PB_M-1);t2++) {
    for (t3=max(1,t2);t3<=PB_M-1;t3++) {
      B[t2] = C[t3][t2] - B[t3-1] - 5;;
    }
  }
}
...
kernel_2224112223_00(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
