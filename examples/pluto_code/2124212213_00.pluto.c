...
void kernel_2124212213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_P >= 5)) {
  for (t1=0;t1<=min(floord(PB_P-4,16),floord(PB_P+PB_L-4,32));t1++) {
    lbp=max(0,ceild(32*t1-PB_P+3,32));
    ubp=min(floord(t1,2),floord(PB_L-1,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t1-32*t2),32*t2+1);t3<=min(PB_P-3,32*t1-32*t2+31);t3++) {
        for (t4=max(1,32*t2);t4<=min(min(PB_L-1,32*t2+31),t3-1);t4++) {
          A[t3][t4] = B[t4][t3+2] * A[t3][t4-1] + A[t3-1][t4] + 2;;
        }
      }
    }
  }
...
kernel_2124212213_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
