...
void kernel_2124212213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
/* Start of CLooG code */
if (PB_N >= 4) {
  for (t1=3;t1<=PB_N-1;t1++) {
    A[t1-2] = B[t1] - A[t1-3] + A[t1-1] + 4;;
  }
}
/* End of CLooG code */
...
kernel_2124212213_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
