...
if (PB_N >= 2) {
  if (PB_M >= 2) {
    for (t2=0;t2<=min(floord(PB_M-1,16),floord(PB_N+PB_M-2,32));t2++) {
...
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(1,32*t3);t4<=min(PB_M-1,32*t3+31);t4++) {
          for (t5=max(1,32*t2-32*t3);t5<=min(min(t4,PB_N-1),32*t2-32*t3+31);t5++) {
            A[t4][t5] = C[t4+1][t5] * A[t4][t5-1] - A[t4][t5+1] + B[t4] * 4;;
          }
        }
      }
    }
