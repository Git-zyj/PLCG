...
void kernel_3214112111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    for (t2=0;t2<=min(floord(PB_M-1,32),floord(-64*t1+3*PB_M-5,32));t2++) {
      if ((PB_M >= 3) && (t1 <= floord(PB_M-2,32)) && (t2 == 0)) {
        ...
        for (t4=max(2,32*t1);t4<=min(PB_M-2,32*t1+31);t4++) {
          A[t4+1] = 6;;
          B[t4] = C[t4] * C[t4-2] - B[t4-1] + 1;;
        }
        if (t1 >= ceild(PB_M-32,32)) {
          B[(PB_M-1)] = C[(PB_M-1)] * C[(PB_M-1)-2] - B[(PB_M-1)-1] + 1;;
        }
      }
      if ((PB_M == 2) && (t1 == 0) && (t2 == 0)) {
        A[0 +1] = 6;;
      ...
      if (t1 <= floord(PB_M-2,32)) {
        for (t3=max(1,32*t2);t3<=(min(PB_M-1,32*t2+31))-7;t3+=8) {
...
kernel_3214112111_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
