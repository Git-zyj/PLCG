...
void kernel_3214231121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_L >= 2) {
  if ((PB_L >= 4) && (PB_P <= 3)) {
    for (t1=2;t1<=PB_L-1;t1++) {
      for (t2=1;t2<=min(PB_L-3,t1-1);t2++) {
        for (t3=1;t3<=PB_L-1;t3++) {
          A[t1+1][t1][t2+1] = C[t2][t1][t3-1] - C[t3][t1][t2] + C[t2-1][t1][t3] * A[t2+3][t2+2][t3+1] - 1;;
        }
      }
    }
  }
  if ((PB_L <= 3) && (PB_P >= 4)) {
    for (t1=1;t1<=PB_L-1;t1++) {
      for (t2=2;t2<=PB_P-2;t2++) {
        B[t2][t1-1] = D[t2][t1] - D[t1][t2+1] * D[t2][t1] * B[t2-2][t1-1] + 3;;
      }
    }
  }
  if ((PB_L >= 4) && (PB_P >= 4)) {
    for (t2=2;t2<=PB_P-2;t2++) {
      B[t2][1 -1] = D[t2][1] - D[1][t2+1] * D[t2][1] * B[t2-2][1 -1] + 3;;
    }
  }
  if ((PB_L >= 4) && (PB_P >= 4)) {
    for (t1=2;t1<=PB_L-1;t1++) {
      for (t3=1;t3<=PB_L-1;t3++) {
        A[t1+1][t1][1 +1] = C[1][t1][t3-1] - C[t3][t1][1] + C[1 -1][t1][t3] * A[1 +3][1 +2][t3+1] - 1;;
      }
      for (t2=2;t2<=min(min(PB_L-3,PB_P-2),t1-1);t2++) {
        B[t2][t1-1] = D[t2][t1] - D[t1][t2+1] * D[t2][t1] * B[t2-2][t1-1] + 3;;
        for (t3=1;t3<=PB_L-1;t3++) {
          A[t1+1][t1][t2+1] = C[t2][t1][t3-1] - C[t3][t1][t2] + C[t2-1][t1][t3] * A[t2+3][t2+2][t3+1] - 1;;
        }
      }
      for (t2=PB_P-1;t2<=min(PB_L-3,t1-1);t2++) {
        for (t3=1;t3<=PB_L-1;t3++) {
          A[t1+1][t1][t2+1] = C[t2][t1][t3-1] - C[t3][t1][t2] + C[t2-1][t1][t3] * A[t2+3][t2+2][t3+1] - 1;;
        }
      }
      if ((PB_L <= PB_P) && (t1 == PB_L-1)) {
        B[(PB_L-2)][(PB_L-1)-1] = D[(PB_L-2)][(PB_L-1)] - D[(PB_L-1)][(PB_L-2)+1] * D[(PB_L-2)][(PB_L-1)] * B[(PB_L-2)-2][(PB_L-1)-1] + 3;;
      }
      for (t2=t1;t2<=PB_P-2;t2++) {
        B[t2][t1-1] = D[t2][t1] - D[t1][t2+1] * D[t2][t1] * B[t2-2][t1-1] + 3;;
      }
    }
  }
...
kernel_3214231121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
