...
/* Start of CLooG code */
if (PB_M >= 6) {
  for (t2=ceild(-PB_M-27,32);t2<=-1;t2++) {
    lbp=0;
    ubp=min(floord(PB_M-6,32),floord(32*t2+PB_M+27,32));
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t5=max(32*t2,32*t4-PB_M+4);t5<=min(-2,32*t2+31);t5++) {
        for (t7=32*t4;t7<=min(32*t4+31,t5+PB_M-4);t7++) {
          A[t7+-t5+2][-t5] = B[t7+-t5][-t5] - C[t7][t7+-t5-2] + B[-t5][t7+1] + A[t7+-t5+3][-t5+1] - A[t7+-t5+1][-t5] * 2;;
        }
      }
    }
  }
