...
void kernel_3122132121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_M >= 3) && (PB_P >= 4)) {
  for (t2=max(ceild(-PB_M-32,32),ceild(-PB_P-29,32));t2<=-1;t2++) {
    lbp=0;
    ubp=min(floord(PB_M-3,32),floord(32*t2+PB_M+32,32));
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t5=max(max(32*t2,-PB_P+2),32*t4-PB_M-1);t5<=min(-2,32*t2+31);t5++) {
        lbv=32*t4;
        ubv=min(min(PB_M-3,32*t4+31),t5+PB_M+1);
#pragma ivdep
#pragma vector always
        for (t7=lbv;t7<=ubv;t7++) {
          A[-t5][t7] = B[t7+-t5-2][-t5] - B[t7][-t5] * B[t7+2][-t5+1] * A[-t5-1][t7] + 2;;
        }
      }
    }
  }
...
kernel_3122132121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
