...
void kernel_3124212211_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5, t6;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 4) && (PB_M >= 3) && (PB_N >= 1)) {
  lbp=0;
      ...
      for (t3=0;t3<=min(floord(PB_L-4,32),t2);t3++) {
        for (t4=32*t1;t4<=min(PB_M-3,32*t1+31);t4++) {
          for (t5=32*t2;t5<=min(PB_N-1,32*t2+31);t5++) {
            for (t6=32*t3;t6<=min(min(t5,PB_L-4),32*t3+31);t6++) {
              A[t5][t4+2][t6+2] = B[t4][t6][t5] - A[t5][t4+2][t6+1] * A[t5][t4+2][t6+3] * 3;;
            }
          }
        }
      }
...
kernel_3124212211_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
