...
void kernel_2124112123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if (PB_M >= 3) {
  for (t1=0;t1<=floord(5*PB_M-15,64);t1++) {
    lbp=ceild(4*t1,5);
    ubp=min(min(floord(PB_M-3,16),floord(64*t1+PB_M+59,96)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(32*t2-PB_M+3,2),32*t1-32*t2);t3<=min(min(floord(PB_M-3,2),8*t2+7),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,4*t3);t4<=min(32*t2+31,2*t3+PB_M-3);t4++) {
          A[(-3*t3+t4)+t3][1] = B[(-3*t3+t4)][t3] - A[(-3*t3+t4)+t3+2][1] * 2;;
        }
      }
    }
  }
...
kernel_2124112123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
