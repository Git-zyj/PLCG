...
void kernel_2124112123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=ceild(t1-1,2);t2<=min(floord(PB_M-1,32),t1);t2++) {
      for (t3=max(max(2,32*t2),16*t1+1);t3<=min(PB_M-1,32*t2+31);t3++) {
        lbv=max(32*t1,t3);
        ubv=min(32*t1+31,2*t3-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-t3+t4)+2] = B[(-t3+t4)][t3][(-t3+t4)+2] * A[t3-2][(-t3+t4)+4] * 6;;
        }
      }
    }
  }
...
kernel_2124112123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
