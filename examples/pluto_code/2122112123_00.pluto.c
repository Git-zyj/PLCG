...
void kernel_2122112123_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if (PB_M >= 1) {
  lbp=ceild(-PB_M-30,32);
  ubp=floord(PB_M-1,16);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,-t1-1);t2<=min(floord(-16*t1+PB_M-1,16),floord(PB_M-1,32));t2++) {
      for (t3=max(0,16*t1+16*t2);t3<=min(min(floord(32*t1+PB_M+30,2),PB_M-1),16*t1+16*t2+31);t3++) {
        lbv=max(32*t2,-32*t1+2*t3-31);
        ubv=min(min(PB_M-1,-32*t1+2*t3),32*t2+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4][0] = B[t3][t4] - A[t3+1][t4+2][0] - 5;;
        }
      }
    }
  }
...
kernel_2122112123_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
