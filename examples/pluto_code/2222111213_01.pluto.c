...
void kernel_2222111213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
  int t1, t2;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
for (t1=1;t1<=min(2,PB_L-2);t1++) {
  A[t1] = A[t1-1] - A[t1+1] - 5;;
}
for (t1=3;t1<=min(PB_L-2,PB_M+2);t1++) {
  A[t1] = A[t1-1] - A[t1+1] - 5;;
  A[(t1-2)+1] = C[(t1-2)-1] - 1;;
}
for (t1=max(3,PB_M+3);t1<=PB_L-2;t1++) {
  A[t1] = A[t1-1] - A[t1+1] - 5;;
}
for (t1=max(3,PB_L-1);t1<=PB_M+2;t1++) {
  A[(t1-2)+1] = C[(t1-2)-1] - 1;;
}
/* End of CLooG code */
...
kernel_2222111213_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
