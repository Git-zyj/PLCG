...
void kernel_2112211111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,32*t1-32*t2),32*t2-PB_L+1);t3<=min(min(PB_L-1,32*t2+29),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+2);t4<=min(32*t2+31,t3+PB_L-1);t4++) {
          A[(-t3+t4)-1][1] = B[t3-1] + A[(-t3+t4)-2][1] + 1;;
        }
      }
    }
  }
...
kernel_2112211111_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
