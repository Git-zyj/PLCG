...
void kernel_2124111213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_L >= 3) {
  for (t1=0;t1<=floord(PB_L-2,8);t1++) {
    lbp=max(ceild(2*t1,3),ceild(32*t1-PB_L+2,32));
    ubp=min(min(floord(3*PB_L-5,32),floord(64*t1+PB_L+61,96)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,ceild(32*t2-PB_L+1,2)),32*t1-32*t2);t3<=min(min(PB_L-2,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3+1);t4<=min(32*t2+31,2*t3+PB_L-1);t4++) {
          A[(-2*t3+t4)][t3-1] = B[(-2*t3+t4)][t3-1] - A[(-2*t3+t4)-1][t3-1] + A[t3+1][(-2*t3+t4)-1] + 6;;
        }
      }
    }
  }
...
kernel_2124111213_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
