...
void kernel_2212232123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_M >= 3) {
  if (PB_L >= 0) {
    for (t1=0;t1<=floord(PB_L+PB_M-3,32);t1++) {
      for (t2=0;t2<=min(floord(PB_M-3,32),floord(-32*t1+PB_L+PB_M-3,32));t2++) {
        if ((t1 <= min(floord(PB_L-1,32),floord(PB_M-3,32))) && (t2 == 0)) {
          for (t4=32*t1;t4<=min(min(PB_L-1,PB_M-3),32*t1+31);t4++) {
            A[0 +1][t4] = C[0 +2][t4] * 3;;
            B[t4] = D[t4+1] * D[t4] + D[t4+2] - D[t4] - D[t4] + B[t4+2] - 4;;
          }
          for (t4=PB_M-2;t4<=min(PB_L-1,32*t1+31);t4++) {
            A[0 +1][t4] = C[0 +2][t4] * 3;;
          }
          for (t4=PB_L;t4<=min(PB_M-3,32*t1+31);t4++) {
            B[t4] = D[t4+1] * D[t4] + D[t4+2] - D[t4] - D[t4] + B[t4+2] - 4;;
          }
        }
        if ((t1 <= floord(PB_L-1,32)) && (t1 >= ceild(PB_M-2,32)) && (t2 == 0)) {
          for (t4=32*t1;t4<=min(PB_L-1,32*t1+31);t4++) {
            A[0 +1][t4] = C[0 +2][t4] * 3;;
          }
        }
        if ((t1 <= floord(PB_M-3,32)) && (t1 >= ceild(PB_L,32)) && (t2 == 0)) {
...
kernel_2212232123_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
