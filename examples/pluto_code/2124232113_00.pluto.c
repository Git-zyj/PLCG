...
void kernel_2124232113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (t2=0;t2<=floord(PB_L-2,32);t2++) {
      for (t3=max(1,32*t1);t3<=min(PB_L-1,32*t1+31);t3++) {
        for (t4=max(2,32*t2);t4<=min(PB_L-2,32*t2+31);t4++) {
          A[t3][t4] = B[t4-2][t3] * B[t3-1][t4] - C[t3][t4] + A[t3][t4+1] * 2;;
        }
      }
    }
  }
...
kernel_2124232113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
