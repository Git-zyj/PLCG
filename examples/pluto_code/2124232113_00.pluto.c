...
void kernel_2124232113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=PB_M-2;t1++) {
    A[t1][t1] = B[t1] + C[t1] * B[t1+1] - A[t1+1][t1+1] + 3;;
  }
}
/* End of CLooG code */
...
kernel_2124232113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
