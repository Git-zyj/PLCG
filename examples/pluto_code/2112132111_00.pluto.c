...
void kernel_2112132111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_L >= 3) {
  for (t1=2;t1<=PB_L-1;t1++) {
    for (t2=0;t2<=-t1+PB_L-1;t2++) {
      A[t1][t1-1] = B[t2+t1] + C[t2][t1] + C[t1][t2+1] - A[t1-1][t1-2] * 2;;
    }
  }
}
/* End of CLooG code */
...
kernel_2112132111_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
