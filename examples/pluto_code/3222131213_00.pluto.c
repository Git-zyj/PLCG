...
void kernel_3222131213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_L >= 2) {
  for (t2=2;t2<=PB_L;t2++) {
    A[t2-1] = C[t2+1] - C[t2-1] - C[t2] - A[t2-2] * 4;;
  }
  if (PB_L >= 3) {
    for (t2=0;t2<=floord(PB_L-1,8);t2++) {
      lbp=max(ceild(2*t2,3),ceild(32*t2-PB_L+1,32));
      ubp=min(min(floord(3*PB_L-3,32),floord(64*t2+PB_L+61,96)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(ceild(32*t3-PB_L+1,2),32*t2-32*t3);t4<=min(min(PB_L-1,16*t3+14),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,2*t4+2);t5<=min(32*t3+31,2*t4+PB_L-1);t5++) {
            A[(-2*t4+t5)-2] = C[0] - C[(-2*t4+t5)-1] - D[(-2*t4+t5)][t4] - A[(-2*t4+t5)] - 2;;
          }
        }
      }
    }
...
kernel_3222131213_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
