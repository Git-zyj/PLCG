...
void kernel_3124132221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_Q >= 5)) {
  for (t1=0;t1<=floord(2*PB_Q+PB_L-3,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_Q+1,32));
    ubp=min(min(floord(PB_Q+PB_L-2,32),floord(32*t1+PB_L+30,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(4,32*t1-32*t2),32*t2-PB_L+1);t3<=min(min(PB_Q-1,32*t2+29),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+2);t4<=min(32*t2+31,t3+PB_L-1);t4++) {
          for (t5=1;t5<=PB_Q-2;t5++) {
            A[t3-2][(-t3+t4)][(-t3+t4)] = B[t3-2][t5-1][0] * C[(-t3+t4)][t5-1] + B[(-t3+t4)][t3-2][t5+1] + A[t3-2][(-t3+t4)-2][(-t3+t4)-2] - A[t3-4][(-t3+t4)+1][(-t3+t4)+1] + 4;;
          }
        }
      }
    }
...
kernel_3124132221_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
