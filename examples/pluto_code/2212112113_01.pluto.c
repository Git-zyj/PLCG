...
void kernel_2212112113_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if ((PB_M >= 0) && (PB_N >= 0)) {
  for (t1=0;t1<=min(floord(PB_M+PB_N-1,32),floord(4*PB_M+PB_N-4,32));t1++) {
    for (t2=0;t2<=min(min(floord(PB_M,32),floord(-32*t1+4*PB_M+PB_N-4,96)),floord(4*PB_M+PB_N-4,128));t2++) {
      if (t2 == 0) {
        for (t3=32*t1;t3<=min(min(PB_M-1,PB_N-4),32*t1+31);t3++) {
          A[t3] = C[0][t3+2] + 6;;
          B[t3+2][t3] = B[t3+3][t3+1] * 1;;
      ...
      if (t2 == 0) {
        for (t3=max(32*t1,PB_N-3);t3<=min(PB_M-1,32*t1+31);t3++) {
          for (t4=0;t4<=min(31,PB_M-1);t4++) {
            A[t3] = C[t4][t3+2] + 6;;
          }
        }
      }
      if (t2 == 0) {
        for (t3=max(PB_M,32*t1);t3<=min(PB_N-4,32*t1+31);t3++) {
          B[t3+2][t3] = B[t3+3][t3+1] * 1;;
        }
      }
    }
...
kernel_2212112113_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
