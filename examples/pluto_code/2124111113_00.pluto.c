...
void kernel_2124111113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if ((PB_M >= 2) && (PB_N >= 3)) {
  lbp=0;
  ubp=min(floord(PB_M-1,32),floord(PB_N-2,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_M-1,32);t2++) {
      for (t3=max(1,32*t1);t3<=min(min(PB_M-1,PB_N-2),32*t1+31);t3++) {
        for (t4=max(32*t2,t3);t4<=min(PB_M-1,32*t2+31);t4++) {
          A[t3+1][t4] = B[t3-1] - A[t3+1][t4-1] * 4;;
        }
      }
    }
  }
...
kernel_2124111113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
