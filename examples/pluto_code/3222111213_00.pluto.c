...
for (t2=1;t2<=PB_N-1;t2++) {
  A[t2] = C[t2] * C[t2-1] - A[t2+1] + A[t2-1] + 2;;
}
if (PB_M >= 4) {
  lbp=0;
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t3=0;t3<=floord(PB_M-2,32);t3++) {
      for (t4=32*t2;t4<=(min(PB_M-1,32*t2+31))-7;t4+=8) {
        for (t5=max(2,32*t3);t5<=min(PB_M-2,32*t3+31);t5++) {
          B[t4][t5-1] = A[t5+1] + B[t4][t5-2] + 5;;
          B[(t4+1)][t5-1] = A[t5+1] + B[(t4+1)][t5-2] + 5;;
          B[(t4+2)][t5-1] = A[t5+1] + B[(t4+2)][t5-2] + 5;;
          B[(t4+3)][t5-1] = A[t5+1] + B[(t4+3)][t5-2] + 5;;
          B[(t4+4)][t5-1] = A[t5+1] + B[(t4+4)][t5-2] + 5;;
          B[(t4+5)][t5-1] = A[t5+1] + B[(t4+5)][t5-2] + 5;;
          B[(t4+6)][t5-1] = A[t5+1] + B[(t4+6)][t5-2] + 5;;
          B[(t4+7)][t5-1] = A[t5+1] + B[(t4+7)][t5-2] + 5;;
        }
      }
