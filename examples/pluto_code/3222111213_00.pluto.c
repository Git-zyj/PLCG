...
if (PB_M >= 1) {
  if (PB_M >= 3) {
    for (t2=0;t2<=floord(PB_M-2,8);t2++) {
      ...
      ubp=min(min(floord(3*PB_M-4,32),floord(64*t2+PB_M+60,96)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5,t6,t7)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(1,ceild(32*t3-PB_M+2,2)),32*t2-32*t3);t4<=min(min(PB_M-1,16*t3+15),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,2*t4+1);t5<=min(32*t3+31,2*t4+PB_M-2);t5++) {
            A[(-2*t4+t5)][t4] = C[(-2*t4+t5)][t4] * C[(-2*t4+t5)-1][t4-1] - A[t4][(-2*t4+t5)+1] * A[(-2*t4+t5)+1][t4-1] * 2;;
          }
        }
      }
    }
    ...
    for (t3=0;t3<=floord(PB_M-1,32);t3++) {
      for (t4=0;t4<=floord(PB_L-1,32);t4++) {
        for (t5=32*t3;t5<=min(PB_M-1,32*t3+31);t5++) {
          for (t6=32*t4;t6<=(min(PB_L-1,32*t4+31))-7;t6+=8) {
            lbv=32*t2;
