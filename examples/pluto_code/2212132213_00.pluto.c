...
/* Start of CLooG code */
if (PB_L >= 4) {
  A[1 +2] = C[1 +1] + C[1 +1] - C[1] - C[1 +2] + A[1 +1] * B[1 -1] + 4;;
}
for (t1=2;t1<=min(PB_L-3,PB_M-2);t1++) {
  A[t1+2] = C[t1+1] + C[t1+1] - C[t1] - C[t1+2] + A[t1+1] * B[t1-1] + 4;;
  B[(t1-1)] = C[(t1-1)-1] + C[(t1-1)] + A[(t1-1)+2] * B[(t1-1)-1] + 2;;
}
for (t1=max(2,PB_M-1);t1<=PB_L-3;t1++) {
  A[t1+2] = C[t1+1] + C[t1+1] - C[t1] - C[t1+2] + A[t1+1] * B[t1-1] + 4;;
}
for (t1=max(2,PB_L-2);t1<=PB_M-2;t1++) {
  B[(t1-1)] = C[(t1-1)-1] + C[(t1-1)] + A[(t1-1)+2] * B[(t1-1)-1] + 2;;
}
/* End of CLooG code */
