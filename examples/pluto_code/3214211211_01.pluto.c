...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      if (t1 == 2*t2) {
        for (t3=max(1,16*t1);t3<=min(PB_N-3,16*t1+31);t3++) {
          for (t4=max(1,16*t1);t4<=t3-1;t4++) {
            if (t1%2 == 0) {
              B[t4][t3+1][t3] = C[t3+1][t4+1] - A[t3+1] + B[t4-1][t3+1][t3] * 3;;
            }
          }
          if (t1%2 == 0) {
            A[t3+1] = A[t3+2] - A[t3] + 2;;
          }
          if (t1%2 == 0) {
            B[t3][t3+1][t3] = C[t3+1][t3+1] - A[t3+1] + B[t3-1][t3+1][t3] * 3;;
          }
        }
      }
      if (t1 >= 2*t2+1) {
      ...
      if ((t1 == 2*t2) && (t1 >= ceild(PB_N-33,16))) {
        for (t4=max(1,16*t1);t4<=PB_N-3;t4++) {
          if (t1%2 == 0) {
            B[t4][(PB_N-2)+1][(PB_N-2)] = C[(PB_N-2)+1][t4+1] - A[(PB_N-2)+1] + B[t4-1][(PB_N-2)+1][(PB_N-2)] * 3;;
          }
        }
      }
    }
