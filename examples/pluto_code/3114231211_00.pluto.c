...
if ((PB_L >= 4) && (PB_M >= 2)) {
  for (t1=1;t1<=PB_L-1;t1++) {
    for (t2=0;t2<=floord(PB_L+2*PB_M-4,32);t2++) {
      lbp=max(ceild(t2,2),ceild(32*t2-PB_M+1,32));
      ubp=min(min(floord(PB_L+PB_M-3,32),floord(32*t2+PB_L+29,64)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(1,32*t2-32*t3),32*t3-PB_L+2);t4<=min(min(PB_M-1,32*t3+29),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4+2);t5<=min(32*t3+31,t4+PB_L-2);t5++) {
            A[(-t4+t5)-1][t1][0] = B[t1][(-t4+t5)+1][t1] * C[t4][(-t4+t5)][t1] + B[t4][t1-1][t1] * A[(-t4+t5)-2][t1][0] + A[(-t4+t5)-1][t4-1][0] + 2;;
          }
        }
      }
    }
