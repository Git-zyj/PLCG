...
void kernel_3224131111_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            B[t4][t5][t4] = C[t4+1][t5] * C[t4+1][t5+1] - 3;;
          }
        }
      }
    }
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t3=t2;t3<=floord(PB_L-2,32);t3++) {
      for (t4=t3;t4<=min(floord(PB_L-2,16),floord(32*t3+PB_L+30,32));t4++) {
        for (t5=max(max(32*t3,32*t2+1),32*t4-PB_L+1);t5<=min(min(PB_L-2,32*t3+31),32*t4+30);t5++) {
          for (t6=max(32*t4,t5+1);t6<=min(32*t4+31,t5+PB_L-1);t6++) {
            lbv=32*t2;
            ubv=min(32*t2+31,t5-1);
#pragma ivdep
#pragma vector always
            for (t7=lbv;t7<=ubv;t7++) {
              B[t7][(-t5+t6)][t7] = D[t7+1][t5-1][t5] * D[t7+1][(-t5+t6)][t5] - D[(-t5+t6)-1][t5+1][t7+1] - D[(-t5+t6)-1][t5-1][t7] * B[t7][(-t5+t6)+1][t7] + 3;;
            }
          }
        }
      }
...
kernel_3224131111_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
