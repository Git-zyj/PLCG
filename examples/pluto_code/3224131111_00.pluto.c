...
void kernel_3224131111_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_M >= 3) {
  for (t1=1;t1<=PB_M-2;t1++) {
    B[t1] = D[t1+1] - D[t1] * D[t1-1] * D[t1] * B[t1-1] + 5;;
    if (PB_L >= 2) {
      for (t2=0;t2<=floord(3*PB_M+PB_L-4,32);t2++) {
        lbp=max(ceild(2*t2,3),ceild(32*t2-PB_M+1,32));
        ubp=min(min(floord(2*PB_M+PB_L-3,32),floord(64*t2+PB_L+61,96)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
        for (t3=lbp;t3<=ubp;t3++) {
          for (t4=max(max(1,ceild(32*t3-PB_L+1,2)),32*t2-32*t3);t4<=min(min(PB_M-1,16*t3+15),32*t2-32*t3+31);t4++) {
            for (t5=max(32*t3,2*t4+1);t5<=min(32*t3+31,2*t4+PB_L-1);t5++) {
              A[t1][t4-1][(-2*t4+t5)] = C[t4][(-2*t4+t5)][t4] + A[t4][(-2*t4+t5)-1][t1-1] + 4;;
            }
          }
        }
      }
...
kernel_3224131111_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
