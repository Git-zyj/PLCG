...
void kernel_2222132223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_N >= 3) {
  if (PB_M >= 2) {
    for (t2=0;t2<=min(floord(PB_M-1,8),floord(PB_N-1,16));t2++) {
      lbp=ceild(3*t2,4);
      ubp=min(min(min(min(floord(3*PB_M-3,32),floord(3*PB_N-3,64)),floord(32*t2+PB_N+30,64)),floord(64*t2+PB_M+61,96)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(max(1,ceild(32*t3-PB_M+1,2)),32*t2-32*t3),32*t3-PB_N+1);t4<=min(min(min(floord(PB_N-1,2),floord(32*t3+31,3)),PB_M-1),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,3*t4);t5<=min(min(32*t3+31,t4+PB_N-1),2*t4+PB_M-1);t5++) {
            A[(-2*t4+t5)][(-2*t4+t5)] = C[(-2*t4+t5)] - C[(-2*t4+t5)+t4] + D[(-2*t4+t5)][t4-1] - A[(-2*t4+t5)-1][(-2*t4+t5)-1] * A[(-2*t4+t5)+1][(-2*t4+t5)+1] - A[(-2*t4+t5)+2][(-2*t4+t5)+2] - 4;;
          }
        }
      }
    }
...
#pragma vector always
    for (t3=lbv;t3<=ubv;t3++) {
      B[t3] = C[t3] - C[t3-1] - C[t3] - A[t3-2][t3-2] - 4;;
    }
  }
}
/* End of CLooG code */
...
kernel_2222132223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
