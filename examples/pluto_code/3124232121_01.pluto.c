...
void kernel_3124232121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 3)) {
  for (t1=0;t1<=min(min(floord(PB_M-3,8),floord(3*PB_L-7,32)),floord(2*PB_M+PB_L-7,32));t1++) {
    lbp=max(ceild(2*t1,3),ceild(32*t1-PB_M+3,32));
    ubp=min(min(min(min(min(floord(PB_L-2,16),floord(3*PB_M-7,32)),floord(PB_M+PB_L-4,32)),floord(32*t1+PB_L+30,64)),floord(64*t1+PB_M+61,96)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(ceild(32*t2-PB_M+1,2),32*t1-32*t2),32*t2-PB_L+1);t3<=min(min(min(PB_L-3,PB_M-3),16*t2+14),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3+2);t4<=min(min(32*t2+31,t3+PB_L-1),2*t3+PB_M-1);t4++) {
          A[t3][(-2*t3+t4)] = B[(-2*t3+t4)-2] * C[t3+2][(-2*t3+t4)+t3] + B[(-2*t3+t4)] + A[(-2*t3+t4)-1][t3] * 5;;
        }
      }
    }
  }
...
kernel_3124232121_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
