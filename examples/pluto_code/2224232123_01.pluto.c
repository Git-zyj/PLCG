...
void kernel_2224232123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
if (PB_M >= 4) {
  for (t1=-1;t1<=floord(PB_M-2,8);t1++) {
    lbp=ceild(t1,2);
    ubp=min(min(floord(3*PB_M-3,32),floord(16*t1+PB_M+15,32)),floord(32*t1+PB_M+26,32));
    ...
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=8*t1;t3<=min(min(min(PB_M-2,8*t1+15),16*t2+14),32*t1-32*t2-1);t3++) {
        for (t4=max(max(32*t2,2*t3+2),-32*t1+32*t2+4*t3-29);t4<=min(32*t2+31,-32*t1+32*t2+4*t3+2);t4++) {
          A[(-2*t3+t4-2)] = C[(-2*t3+t4-2)] + E[(-2*t3+t4-2)][t3+2] * D[(-2*t3+t4-2)] + 5;;
        }
      }
      for (t3=max(max(2,ceild(32*t2-PB_M-1,2)),8*t1);t3<=min(floord(32*t2-1,3),32*t1-32*t2+31);t3++) {
        for (t4=32*t2;t4<=min(2*t3+PB_M+1,-32*t1+32*t2+4*t3+2);t4++) {
          A[(-2*t3+t4-2)] = C[(-2*t3+t4-2)] + E[(-2*t3+t4-2)][t3+2] * D[(-2*t3+t4-2)] + 5;;
        }
      }
      for (t3=max(max(2,ceild(32*t2,3)),32*t1-32*t2);t3<=min(min(floord(32*t2+31,3),PB_M-2),32*t1-32*t2+31);t3++) {
        if ((t1 <= floord(t3-14,8)) && (3*t1 == 4*t2-2)) {
          if ((t1+2)%4 == 0) {
            A[t3] = C[t3] + D[t3] - C[t3+1] * A[t3-2] + 1;;
          }
        }
        ...
        if (t1 >= ceild(32*t2+t3-29,32)) {
          A[t3] = C[t3] + D[t3] - C[t3+1] * A[t3-2] + 1;;
          A[(t3-2)] = C[(t3-2)] + E[(t3-2)][t3+2] * D[(t3-2)] + 5;;
        }
        for (t4=max(3*t3+1,-32*t1+32*t2+4*t3-29);t4<=min(min(32*t2+31,2*t3+PB_M+1),-32*t1+32*t2+4*t3+2);t4++) {
          A[(-2*t3+t4-2)] = C[(-2*t3+t4-2)] + E[(-2*t3+t4-2)][t3+2] * D[(-2*t3+t4-2)] + 5;;
        }
      }
      for (t3=max(max(max(2,ceild(32*t2-PB_M-1,2)),8*t1),32*t1-32*t2+32);t3<=min(floord(32*t1-32*t2+PB_M+30,2),8*t1+15);t3++) {
...
kernel_2224232123_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
