...
void kernel_3214211211_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
/* Start of CLooG code */
if (PB_M >= 2) {
  if (PB_L >= max(0,-PB_M+3)) {
    for (t1=0;t1<=min(floord(PB_L+PB_M-1,32),floord(PB_L+2*PB_M-4,32));t1++) {
      for (t2=0;t2<=min(min(min(floord(PB_M-2,32),floord(PB_L+PB_M-3,32)),floord(-32*t1+PB_L+PB_M-1,32)),floord(-32*t1+PB_L+2*PB_M-4,64));t2++) {
        if ((PB_M >= 3) && (PB_P <= -1) && (t1 <= floord(PB_M-1,32)) && (t2 == 0)) {
          for (t4=max(2,32*t1);t4<=min(PB_M-1,32*t1+31);t4++) {
            B[t4-1] = C[t4-1] * D[t4] * B[t4-2] + B[t4] + B[t4-2] - B[t4-2] + 1;;
          }
        }
        for (t3=0;t3<=min(floord(PB_P,32),floord(-32*t2+PB_M+PB_P-3,32));t3++) {
          if ((t2 >= -t3+1) && (t3 <= floord(PB_P-1,32))) {
            lbp=max(1,32*t1);
            ubp=min(PB_L,32*t1+31);
...
#pragma vector always
          ...
          if ((PB_L >= 1) && (PB_P >= 1) && (t1 == 0) && (t2 == 0) && (t3 == 0)) {
...
#pragma vector always
          ...
          if ((PB_P >= 1) && (t2 == 0) && (t3 == 0)) {
            for (t4=max(2,32*t1);t4<=min(min(PB_L,PB_M-1),32*t1+31);t4++) {
              A[0 +1][t4-1][0] = 1;;
              B[t4-1] = C[t4-1] * D[t4] * B[t4-2] + B[t4] + B[t4-2] - B[t4-2] + 1;;
              lbv=1;
              ubv=min(31,PB_P-1);
...
kernel_3214211211_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
