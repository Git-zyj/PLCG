...
void kernel_3112212213_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if ((PB_L >= 3) && (PB_M >= 2)) {
  for (t1=0;t1<=PB_M-2;t1++) {
    for (t2=0;t2<=floord(PB_L,8);t2++) {
      lbp=ceild(3*t2,4);
      ubp=min(min(min(floord(3*PB_L,32),floord(64*t2+PB_L+62,96)),floord(96*t2+PB_M+92,128)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(max(3,ceild(32*t3-PB_L,2)),ceild(32*t3-PB_M+1,3)),32*t2-32*t3);t4<=min(min(floord(32*t3+31,3),PB_L),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,3*t4);t5<=min(min(32*t3+31,2*t4+PB_L),3*t4+PB_M-1);t5++) {
            A[t1][(-3*t4+t5)+t4-1][t1] = B[(-3*t4+t5)][t1][t1] + A[t1][(-3*t4+t5)+t4-3][t1] + A[t1+1][(-3*t4+t5)+t4-1][t1+1] + 4;;
          }
        }
      }
    }
...
kernel_3112212213_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
