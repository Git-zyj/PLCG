...
void kernel_3124112213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(3,32*t1-32*t2),16*t2+1);t3<=min(min(PB_L-1,32*t2+29),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+2);t4<=min(32*t2+31,2*t3-1);t4++) {
          A[(-t3+t4)-1] = B[t3-2][2] + A[(-t3+t4)] - A[(-t3+t4)-2] + 3;;
        }
      }
    }
  }
...
kernel_3124112213_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
