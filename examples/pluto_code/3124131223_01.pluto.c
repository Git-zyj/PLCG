...
void kernel_3124131223_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=min(floord(-32*t1+32*t2+PB_M-1,32),t2);t3++) {
        for (t4=32*t1-32*t2;t4<=min(min(PB_M-2,32*t1-32*t2+31),-32*t3+PB_M-1);t4++) {
          for (t5=32*t2;t5<=min(PB_M-2,32*t2+31);t5++) {
            for (t6=32*t3;t6<=min(min(t5,32*t3+31),-t4+PB_M-1);t6++) {
              A[t5][t6+t4+1][t6] = B[t5][t4+1][t6+1] + B[t6+t4][t5][t5] * B[t5+1][t4][t5+t6] * A[t5+1][t6+t4+2][t6] + A[t5][t6+t4+3][t6+2] * 5;;
            }
          }
        }
      }
...
kernel_3124131223_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
