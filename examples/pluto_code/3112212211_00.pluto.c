...
void kernel_3112212211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t6=max(32*t4,-PB_L+1);t6<=min(0,32*t4+31);t6++) {
        for (t7=max(1,32*t2);t7<=min(PB_L-2,32*t2+31);t7++) {
          A[t7][-t6] = B[t7][-t6] - A[t7-1][-t6] * A[-t6][t7+1] * 2;;
        }
      }
    }
  }
...
kernel_3112212211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
