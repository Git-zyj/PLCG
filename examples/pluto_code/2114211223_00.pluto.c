...
void kernel_2114211223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(3,32*t1-32*t2);t3<=min(PB_M-2,32*t1-32*t2+31);t3++) {
        for (t4=max(2,32*t2);t4<=min(PB_L-2,32*t2+31);t4++) {
          A[t3-1][t4] = B[t3+1][t4+1][t3] - A[t3-3][t4] + A[t3-1][t4-2] + 1;;
        }
      }
    }
  }
...
kernel_2114211223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
