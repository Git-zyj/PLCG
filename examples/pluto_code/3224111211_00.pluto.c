...
void kernel_3224111211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 2) {
  for (t1=0;t1<=PB_L-2;t1++) {
    A[t1+1] = C[t1+1] + C[t1] - A[t1+2] - 6;;
    if (t1 <= PB_L-3) {
      for (t2=0;t2<=floord(-t1+PB_L-2,8);t2++) {
        lbp=max(ceild(2*t2,3),ceild(t1+32*t2-PB_L+1,32));
        ubp=min(min(floord(-3*t1+3*PB_L-4,32),floord(-t1+64*t2+PB_L+60,96)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
        for (t3=lbp;t3<=ubp;t3++) {
          for (t4=max(max(1,ceild(t1+32*t3-PB_L+2,2)),32*t2-32*t3);t4<=min(min(16*t3+15,-t1+PB_L-1),32*t2-32*t3+31);t4++) {
            for (t5=max(32*t3,2*t4+1);t5<=min(32*t3+31,-t1+2*t4+PB_L-2);t5++) {
              B[t4+t1][(-2*t4+t5)][(-2*t4+t5)+1] = B[(-2*t4+t5)+t1+1][t4][t4+1] * B[t4+t1][(-2*t4+t5)-1][(-2*t4+t5)] - B[(-2*t4+t5)+t4-1][t1][t1+1] - 1;;
            }
          }
        }
      }
...
kernel_3224111211_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
