...
void kernel_2212112121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t6=max(32*t4,-PB_M+1);t6<=min(-2,32*t4+31);t6++) {
        for (t7=max(1,32*t2);t7<=min(PB_M,32*t2+31);t7++) {
          A[t7-1][-t6+1] = C[-t6-2] + A[t7-1][-t6+2] + 6;;
          B[-t6] = 4;;
        }
      }
    }
...
kernel_2212112121_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
