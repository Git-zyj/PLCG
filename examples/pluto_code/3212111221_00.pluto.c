...
void kernel_3212111221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 1) {
  for (t1=0;t1<=floord(PB_M,32);t1++) {
    if ((PB_L <= -1) && (PB_M >= 5) && (t1 <= floord(PB_M-3,32))) {
      for (t4=max(2,32*t1);t4<=min(PB_M-3,32*t1+31);t4++) {
        A[t4] = C[t4] * C[t4] * A[t4-2] + A[t4+1] - A[t4+2] * 6;;
      }
    }
    for (t2=0;t2<=min(min(floord(PB_L,32),floord(-32*t1+PB_M+3*PB_L-3,96)),floord(PB_M+4*PB_L-5,128));t2++) {
      for (t3=0;t3<=min(min(floord(PB_L,32),floord(-32*t1+PB_M+3*PB_L-3,96)),floord(PB_M+4*PB_L-5,128));t3++) {
        if ((PB_L == 0) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(2,32*t1);t4<=min(PB_M-3,32*t1+31);t4++) {
            A[t4] = C[t4] * C[t4] * A[t4-2] + A[t4+1] - A[t4+2] * 6;;
          }
        }
        if ((t2 <= floord(PB_L-1,32)) && (t2 >= -t3+1) && (t3 <= floord(PB_L-1,32))) {
...
#pragma vector always
        ...
        if ((PB_L >= 1) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(max(2,32*t1),PB_M-2);t4<=min(PB_M,32*t1+31);t4++) {
            for (t5=0;t5<=(min(31,PB_L-1))-7;t5+=8) {
              lbv=0;
              ubv=min(31,PB_L-1);
#pragma ivdep
...
kernel_3212111221_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
