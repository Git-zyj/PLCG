...
void kernel_3212111221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 1) {
  if (PB_L >= 4) {
    for (t1=1;t1<=PB_M;t1++) {
      A[t1+1] = A[0] - A[0 +3] - 5;;
      B[t1-1] = C[t1] + C[t1] + A[t1+3] - A[t1-1] - 6;;
      for (t2=1;t2<=PB_L-4;t2++) {
        A[t1+1] = A[t2] - A[t2+3] - 5;;
      }
    }
  }
  if (PB_L <= 3) {
...
kernel_3212111221_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
