...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
    for (t1=lbp;t1<=ubp;t1++) {
      for (t2=0;t2<=min(min(min(floord(PB_M-2,32),floord(PB_M+PB_L-4,32)),floord(-32*t1+PB_M+PB_L-2,32)),floord(-64*t1+3*PB_M+2*PB_L-8,96));t2++) {
        if (t2 >= 1) {
          for (t3=max(1,32*t1);t3<=min(PB_L-1,32*t1+31);t3++) {
            for (t4=32*t2;t4<=min(PB_M-2,32*t2+31);t4++) {
              A[t4-1][t3][t3-1] = C[t3-1] + A[t4-2][t3][t3-1] * 6;;
            }
          }
        }
        if ((PB_L >= 2) && (t1 == 0) && (t2 == 0)) {
          for (t4=2;t4<=min(31,PB_M-2);t4++) {
            A[t4-1][1][1 -1] = C[1 -1] + A[t4-2][1][1 -1] * 6;;
          }
        }
        if (t2 == 0) {
          for (t3=max(2,32*t1);t3<=min(min(PB_L-1,PB_M-2),32*t1+31);t3++) {
            B[t3] = D[t3] * D[t3] + C[t3+1] - D[t3] - 5;;
            for (t4=2;t4<=min(31,PB_M-2);t4++) {
              A[t4-1][t3][t3-1] = C[t3-1] + A[t4-2][t3][t3-1] * 6;;
            }
          }
