...
void kernel_3112231111_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_M-1,32),floord(PB_P-1,32));t2++) {
      for (t3=t2;t3<=floord(PB_P-1,32);t3++) {
        for (t4=max(1,32*t2);t4<=min(min(PB_M-1,PB_P-1),32*t2+31);t4++) {
          for (t5=max(32*t3,t4);t5<=min(PB_P-1,32*t3+31);t5++) {
            lbv=32*t1;
            ubv=min(PB_P-2,32*t1+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4-1][t5][t6+1] = B[1][t4][t5] + C[t4-1][t6+1][t5] + B[t6][t4][t5] * A[t4][t5+1][t6+1] * 2;;
            }
          }
        }
      }
...
kernel_3112231111_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
