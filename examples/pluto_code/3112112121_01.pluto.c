...
void kernel_3112112121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_M >= 1) && (PB_P >= 1) && (PB_Q >= 3)) {
  for (t1=0;t1<=PB_P-1;t1++) {
    for (t2=0;t2<=floord(3*PB_Q+PB_M-10,32);t2++) {
      lbp=max(ceild(2*t2,3),ceild(32*t2-PB_Q+3,32));
      ubp=min(min(floord(2*PB_Q+PB_M-7,32),floord(64*t2+PB_M+61,96)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(ceild(32*t3-PB_M+1,2),32*t2-32*t3);t4<=min(min(PB_Q-3,16*t3+15),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,2*t4);t5<=min(32*t3+31,2*t4+PB_M-1);t5++) {
            A[t4][(-2*t4+t5)+2] = B[t4][t1][t4+2] - A[(-2*t4+t5)][t4+3] - 3;;
          }
        }
      }
    }
...
kernel_3112112121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
