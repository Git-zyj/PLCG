...
/* Start of CLooG code */
if ((PB_L >= 5) && (PB_Q >= 1)) {
  for (t1=0;t1<=floord(PB_Q+PB_L-3,32);t1++) {
    lbp=max(0,ceild(32*t1-PB_Q,32));
    ubp=min(floord(PB_L-3,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(1,32*t1-32*t2);t3<=min(PB_Q,32*t1-32*t2+31);t3++) {
        for (t4=max(2,32*t2);t4<=min(PB_L-3,32*t2+31);t4++) {
          A[t3+2][t4-2] = B[t4+2][t3-1] + A[t3+2][t4-1] - A[t3+3][t4-2] * 6;;
        }
      }
    }
  }
