...
void kernel_3124111121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
      ...
      for (t3=0;t3<=floord(PB_M-1,32);t3++) {
        for (t4=max(1,32*t1);t4<=min(PB_M,32*t1+31);t4++) {
          for (t5=32*t2;t5<=min(PB_L-1,32*t2+31);t5++) {
            for (t6=max(2,32*t3);t6<=min(PB_M-1,32*t3+31);t6++) {
              A[t4-1][t5][t6] = B[t6][t5][t4] - A[t4-1][t5][t6-2] * 4;;
            }
          }
        }
      }
...
kernel_3124111121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
