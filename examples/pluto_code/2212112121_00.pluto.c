...
void kernel_2212112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
for (t1=2;t1<=min(PB_L-1,PB_M-1);t1++) {
  A[t1][t1] = C[t1] + A[t1-2][t1-2] - 4;;
  B[t1] = C[t1] + B[t1-2] + 5;;
}
for (t1=max(2,PB_L);t1<=PB_M-1;t1++) {
  A[t1][t1] = C[t1] + A[t1-2][t1-2] - 4;;
}
for (t1=max(2,PB_M);t1<=PB_L-1;t1++) {
  B[t1] = C[t1] + B[t1-2] + 5;;
}
/* End of CLooG code */
...
kernel_2212112121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
