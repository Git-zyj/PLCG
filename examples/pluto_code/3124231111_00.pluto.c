...
void kernel_3124231111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_M >= 2) {
  for (t1=1;t1<=PB_M-1;t1++) {
    A[t1] = B[t1] + C[t1] * D[t1-1][0] * A[t1-1] - 5;;
  }
}
/* End of CLooG code */
...
kernel_3124231111_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
