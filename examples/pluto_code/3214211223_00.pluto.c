...
void kernel_3214211223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (t2=0;t2<=min(floord(PB_M-1,32),t1);t2++) {
      if (t2 == 0) {
        for (t3=max(2,32*t1);t3<=min(PB_M,32*t1+31);t3++) {
          A[t3-1][0] = A[t3-2][0] + 2;;
          B[t3] = C[t3-1] + B[t3-2] * B[t3-1] + B[t3+1] + 2;;
          lbv=1;
          ubv=min(31,t3-1);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3-1][t4] = A[t3-2][t4] + 2;;
          }
        }
      }
      if (t2 >= 1) {
...
kernel_3214211223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
