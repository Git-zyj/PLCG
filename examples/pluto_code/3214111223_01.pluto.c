...
void kernel_3214111223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
      ...
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=1;t3<=min(15,PB_M-3);t3++) {
          ...
          for (t4=2*t3;t4<=min(31,t3+PB_M-1);t4++) {
            A[(-2*t3+t4)+t3][(-2*t3+t4)] = C[(-2*t3+t4)][t3] + A[(-2*t3+t4)+t3-1][(-2*t3+t4)] * A[(-2*t3+t4)+t3-1][t3-1] * 5;;
          }
        }
      }
      if (t2 >= 1) {
        for (t3=max(max(1,32*t1),32*t2-PB_M+1);t3<=min(min(PB_M-3,32*t1+31),16*t2+15);t3++) {
...
kernel_3214111223_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
