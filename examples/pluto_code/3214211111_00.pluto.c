...
void kernel_3214211111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M-2,32);t1++) {
    for (t2=0;t2<=min(floord(PB_M-2,16),floord(32*t1+PB_M+30,32));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=min(30,PB_M-2);t3++) {
          B[t3] = C[t3] - C[t3] - B[t3+1] - 1;;
      ...
      if (t2 >= 1) {
        for (t3=max(32*t1,32*t2-PB_M+1);t3<=min(min(PB_M-2,32*t1+31),32*t2+30);t3++) {
...
kernel_3214211111_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
