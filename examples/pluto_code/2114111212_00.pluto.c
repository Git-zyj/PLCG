...
void kernel_2114111212_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t3=max(max(1,32*t1-32*t2),32*t2-PB_M+2);t3<=min(PB_L-1,32*t1-32*t2+31);t3++) {
      for (t4=max(32*t2,t3);t4<=min(32*t2+31,t3+PB_M-2);t4++) {
        A[(-t3+t4)][t3][(-t3+t4)] = B[t3][(-t3+t4)] + A[(-t3+t4)+1][t3][(-t3+t4)+1] - A[(-t3+t4)+1][t3-1][(-t3+t4)+1] * 1;;
      }
    }
  }
}
...
kernel_2114111212_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
