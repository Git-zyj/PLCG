...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_L-4,32),floord(-32*t1+PB_L-2,32));t2++) {
      for (t3=t1;t3<=floord(-32*t2+PB_L-1,32);t3++) {
        for (t4=32*t1;t4<=min(min(min(PB_M-2,32*t1+31),32*t3+30),-32*t2+PB_L-2);t4++) {
          for (t5=max(max(3,32*t3),t4+1);t5<=min(32*t3+31,-32*t2+PB_L-1);t5++) {
            lbv=32*t2;
            ubv=min(32*t2+31,-t5+PB_L-1);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t5-2][t4][t6+t5] = B[t4+1][t6][t5] + A[t5-3][t4][t6+t5-1] * 6;;
            }
          }
        }
      }
