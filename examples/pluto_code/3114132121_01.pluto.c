...
/* Start of CLooG code */
if (PB_N >= 2) {
  for (t1=0;t1<=floord(3*PB_N-3,64);t1++) {
    lbp=max(0,ceild(64*t1-PB_N+1,64));
    ubp=min(floord(PB_N-1,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=t1-t2;t3<=floord(-32*t1+32*t2+PB_N-1,32);t3++) {
        for (t4=32*t1-32*t2;t4<=min(min(floord(PB_N-1,2),32*t1-32*t2+31),-32*t3+PB_N-1);t4++) {
          for (t5=max(32*t3,t4);t5<=min(32*t3+31,-t4+PB_N-1);t5++) {
            lbv=max(1,32*t2);
            ubv=min(PB_N-1,32*t2+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t5][t4+2][t6-1] = B[t4][t6][t5] * B[t4][t5+t4][t6] + B[t4][t5][2] + A[t4+2][t5+2][t6+1] - 4;;
            }
          }
        }
      }
