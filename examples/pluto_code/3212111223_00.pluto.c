...
void kernel_3212111223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if ((PB_L >= 4) && (PB_P >= 2)) {
  for (t1=1;t1<=PB_L-3;t1++) {
    for (t2=0;t2<=PB_P-2;t2++) {
      A[t1+1][t2] = C[t2+1][t1-1] + C[t2][t1+1] - B[t1][t1][t2+1] * B[t1+1][t1+1][t2+1] + 6;;
      if (PB_P >= 3) {
        lbp=0;
...
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t1][t1][t4+1] = A[t4-1][t2] + B[t1+2][t1+2][t4+1] + 3;;
          }
        }
      }
    }
...
kernel_3212111223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
