...
void kernel_2224131111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_L >= 2) {
  if (PB_L >= 4) {
    lbp=0;
    ubp=floord(PB_L-2,64);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=t2;t3<=min(floord(PB_L-2,32),floord(-32*t2+PB_L-1,32));t3++) {
        for (t4=max(1,32*t2);t4<=min(min(min(floord(PB_L-2,2),32*t2+31),32*t3+30),-32*t3+PB_L-1);t4++) {
          for (t5=max(32*t3,t4+1);t5<=min(32*t3+31,-t4+PB_L-1);t5++) {
            A[t4+1][t4-1] = C[t4+t5][t4+t5] - D[t5] + C[t4][t5] - B[t4+1] * 6;;
          }
        }
      }
    }
  ...
  for (t2=1;t2<=PB_L-1;t2++) {
    B[t2+1] = D[t2] - C[t2][t2-1] * C[t2-1][0] - B[t2] - 1;;
  }
}
/* End of CLooG code */
...
kernel_2224131111_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
