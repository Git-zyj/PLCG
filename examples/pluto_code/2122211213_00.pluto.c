...
void kernel_2122211213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_M >= 3)) {
  for (t1=0;t1<=floord(PB_L+PB_M-5,32);t1++) {
...
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+1][t4] = B[t3] * A[t3][t4] - A[t3+2][t4+1] - 6;;
        }
      }
    }
  }
...
kernel_2122211213_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
