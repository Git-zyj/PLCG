...
void kernel_3212111223_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if ((PB_M >= 0) && (PB_N >= 0)) {
  for (t1=0;t1<=PB_N+PB_M-2;t1++) {
    if (t1 <= PB_M-2) {
      B[t1+1] = C[t1+1] + 6;;
    }
    if ((PB_L >= 4) && (t1 <= PB_N-2)) {
      for (t2=0;t2<=floord(3*PB_N+PB_L-5,32);t2++) {
        lbp=max(ceild(2*t2,3),ceild(32*t2-PB_N+1,32));
        ubp=min(min(floord(2*PB_N+PB_L-4,32),floord(64*t2+PB_L+60,96)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
        for (t3=lbp;t3<=ubp;t3++) {
          for (t4=max(max(1,ceild(32*t3-PB_L+2,2)),32*t2-32*t3);t4<=min(min(PB_N-1,16*t3+14),32*t2-32*t3+31);t4++) {
            for (t5=max(32*t3,2*t4+2);t5<=min(32*t3+31,2*t4+PB_L-2);t5++) {
              A[t1+1][(-2*t4+t5)][(-2*t4+t5)] = A[t4-1][(-2*t4+t5)][(-2*t4+t5)] + A[t1+1][(-2*t4+t5)+1][(-2*t4+t5)+1] * A[t4][(-2*t4+t5)+1][(-2*t4+t5)+1] - A[t1+1][(-2*t4+t5)-2][(-2*t4+t5)-2] + 6;;
            }
          }
        }
      }
...
kernel_3212111223_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
