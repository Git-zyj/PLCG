...
void kernel_3212132113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 3) {
  for (t1=1;t1<=PB_L-2;t1++) {
    A[t1][t1] = C[t1+1] - C[t1] * C[t1-1] + C[t1] + C[t1] - A[t1-1][t1-1] * 1;;
    A[t1-1][t1-1] = C[t1] + 6;;
  }
}
/* End of CLooG code */
...
kernel_3212132113_01(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
