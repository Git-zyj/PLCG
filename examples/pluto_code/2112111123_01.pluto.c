...
void kernel_2112111123_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(-32*t1+PB_M-1,32);t2++) {
      for (t3=32*t1;t3<=min(min(floord(PB_M-1,2),32*t1+31),-32*t2+PB_M-1);t3++) {
        for (t4=max(32*t2,t3);t4<=min(32*t2+31,-t3+PB_M-1);t4++) {
          A[t3][t4] = B[t4+t3] - A[t3][t4+2] - 4;;
        }
      }
    }
  }
...
kernel_2112111123_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
