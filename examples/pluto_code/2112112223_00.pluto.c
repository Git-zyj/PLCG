...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(floord(PB_M-1,32),floord(PB_P-1,32)),t1);t2++) {
      for (t3=max(2,32*t2);t3<=min(min(min(PB_M-1,PB_P-1),32*t1+30),32*t2+31);t3++) {
        lbv=max(32*t1,t3+1);
        ubv=min(PB_P,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t4+1][t3][t3] = B[t3][t4-1] * A[t4+1][t3-1][t3-1] + A[t4+1][t3-2][t3-2] * 6;;
        }
      }
    }
  }
