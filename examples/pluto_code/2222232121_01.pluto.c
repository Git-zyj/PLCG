...
void kernel_2222232121_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,ceild(32*t1-PB_P+1,32));t2<=floord(PB_P+1,32);t2++) {
      for (t3=32*t1;t3<=min(min(PB_P-3,32*t1+31),32*t2+30);t3++) {
        if (t1 == t2) {
          for (t4=max(2,32*t1);t4<=t3;t4++) {
            B[t3+2][t3+2] = D[(t4-1)-1] - C[t3][t3] - C[(t4-1)][t3+1] - E[(t4-1)][t3] - A[(t4-1)+1][t3][(t4-1)-1] + 6;;
          }
        }
        for (t4=max(max(2,32*t2),t3+1);t4<=min(PB_P+1,32*t2+31);t4++) {
          A[t4][t3][t4-2] = C[t3][t4-1] - 3;;
          B[t3+2][t3+2] = D[(t4-1)-1] - C[t3][t3] - C[(t4-1)][t3+1] - E[(t4-1)][t3] - A[(t4-1)+1][t3][(t4-1)-1] + 6;;
        }
      }
      for (t3=max(32*t1,PB_P-2);t3<=min(min(PB_P,32*t1+31),32*t2+30);t3++) {
        for (t4=max(max(2,32*t2),t3+1);t4<=min(PB_P+1,32*t2+31);t4++) {
          A[t4][t3][t4-2] = C[t3][t4-1] - 3;;
        }
      }
      for (t3=max(32*t1,32*t2+31);t3<=min(PB_P-3,32*t1+31);t3++) {
...
kernel_2222232121_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
