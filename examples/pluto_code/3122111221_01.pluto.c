...
void kernel_3122111221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t1-32*t2),32*t2-PB_M+1);t3<=min(min(PB_M-1,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,t3+PB_M-1);t4++) {
          A[(-2*t3+t4)+t3-1][t3] = B[(-2*t3+t4)][t3] - A[(-2*t3+t4)+t3][(-2*t3+t4)+1] - A[(-2*t3+t4)+t3-2][t3-1] + 1;;
        }
      }
    }
  }
...
kernel_3122111221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
