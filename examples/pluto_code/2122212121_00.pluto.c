...
if (PB_P >= 2) {
  for (t1=0;t1<=floord(PB_P-2,8);t1++) {
    ...
    ubp=min(min(floord(3*PB_P-4,32),floord(64*t1+PB_P+60,96)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,ceild(32*t2-PB_P+2,2)),32*t1-32*t2);t3<=min(min(PB_P-1,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,2*t3+PB_P-2);t4++) {
          A[(-2*t3+t4)][t3] = B[(-2*t3+t4)][t3-1] - A[t3][(-2*t3+t4)+1] + 3;;
        }
      }
    }
  }
