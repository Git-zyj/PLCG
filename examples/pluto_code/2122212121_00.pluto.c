...
if ((PB_P >= 3) && (PB_Q >= 3)) {
  lbp=0;
  ubp=min(floord(PB_Q-2,16),floord(PB_P,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=ceild(t1-1,2);t2<=min(min(floord(PB_P-1,32),floord(PB_Q-1,32)),t1);t2++) {
      for (t3=max(max(3,32*t1),32*t2+1);t3<=min(min(min(PB_P,2*PB_Q-3),32*t1+31),64*t2+61);t3++) {
        for (t4=max(ceild(t3+1,2),32*t2);t4<=min(min(PB_Q-1,32*t2+31),t3-1);t4++) {
          A[t4][(t3-t4)+t4-1][0] = B[(t3-t4)-1][t4][(t3-t4)+1] + A[(t3-t4)+2][(t3-t4)+t4-1][0] - 2;;
        }
      }
    }
  }
