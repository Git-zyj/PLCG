...
void kernel_3114212113_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_M-1,32);t2++) {
      for (t3=2*t2;t3<=min(floord(3*PB_M-4,32),floord(64*t2+PB_M+60,32));t3++) {
        for (t4=32*t1;t4<=min(PB_M-1,32*t1+31);t4++) {
          for (t5=max(max(2,ceild(32*t3-PB_M+2,2)),32*t2);t5<=min(min(PB_M-1,32*t2+31),16*t3+14);t5++) {
            for (t6=max(32*t3,2*t5+2);t6<=min(32*t3+31,2*t5+PB_M-2);t6++) {
              A[t5-2][t4][(-2*t5+t6)+1] = B[(-2*t5+t6)][t4] - A[(-2*t5+t6)-2][t4][t5] - 2;;
            }
          }
        }
      }
...
kernel_3114212113_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
