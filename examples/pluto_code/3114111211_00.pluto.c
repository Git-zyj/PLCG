...
if ((PB_L >= 2) && (PB_N >= 3)) {
  for (t1=0;t1<=floord(PB_L-2,16);t1++) {
    lbp=ceild(t1,2);
    ubp=min(min(floord(PB_L-1,32),floord(32*t1+PB_N+29,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_N+2);t3<=min(min(PB_L-2,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(min(PB_L-1,32*t2+31),t3+PB_N-2);t4++) {
          A[(-t3+t4)][(-t3+t4)] = B[(-t3+t4)][(-t3+t4)+t3] + A[(-t3+t4)-1][(-t3+t4)-1] * A[(-t3+t4)+1][(-t3+t4)+1] * 2;;
        }
      }
    }
  }
