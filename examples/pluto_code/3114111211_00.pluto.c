...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=t1-t2;t3<=min(floord(PB_P+PB_L-2,32),floord(32*t1-32*t2+PB_L+30,32));t3++) {
        for (t4=max(max(1,32*t1-32*t2),32*t3-PB_L+1);t4<=min(min(PB_P-1,32*t3+30),32*t1-32*t2+31);t4++) {
          for (t5=max(1,32*t2);t5<=min(PB_P-1,32*t2+31);t5++) {
            for (t6=max(32*t3,t4+1);t6<=min(32*t3+31,t4+PB_L-1);t6++) {
              A[t5][(-t4+t6)-1][t4] = B[(-t4+t6)][t5][t4] + A[t5-1][(-t4+t6)][t4-1] + A[t5][(-t4+t6)][t4] + 2;;
            }
          }
        }
      }
