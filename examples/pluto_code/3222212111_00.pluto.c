...
void kernel_3222212111_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-3,32);t2++) {
      for (t3=t2;t3<=min(floord(PB_L-3,16),floord(32*t2+PB_L+29,32));t3++) {
        if (t1 == 0) {
          for (t4=0;t4<=min(1,PB_M-1);t4++) {
            for (t5=max(max(1,32*t2),32*t3-PB_L+2);t5<=min(PB_L-3,32*t2+31);t5++) {
              for (t6=max(32*t3,t5);t6<=min(32*t3+31,t5+PB_L-2);t6++) {
                B[t4][(-t5+t6)] = B[t4][(-t5+t6)+1] - 1;;
              }
            }
          }
        }
        for (t4=max(2,32*t1);t4<=min(PB_M-1,32*t1+31);t4++) {
          for (t5=max(max(1,32*t2),32*t3-PB_L+2);t5<=min(min(PB_L-3,32*t2+31),32*t3+30);t5++) {
            if (t2 == t3) {
              B[t4][0] = B[t4][0 +1] - 1;;
            }
            for (t6=max(32*t3,t5+1);t6<=min(32*t3+31,t5+PB_L-2);t6++) {
              A[t5+2][t4+1][t4-2] = C[t4][(-t5+t6)][1] + D[(-t5+t6)-1][t5-1][t4] - B[t4][(-t5+t6)+1] + 2;;
              B[t4][(-t5+t6)] = B[t4][(-t5+t6)+1] - 1;;
            }
          }
          if ((t2 == t3) && (t2 <= floord(PB_L-34,32))) {
            B[t4][0] = B[t4][0 +1] - 1;;
          }
        }
      }
...
kernel_3222212111_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
