...
void kernel_3222212111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_M >= 1) && (PB_N >= 1) && (PB_N >= -PB_M+3)) {
  lbp=0;
  ubp=min(floord(PB_N-1,32),floord(PB_N+PB_M-3,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(-32*t1+PB_N+PB_M-1,32);t2++) {
      for (t3=max(2,32*t2);t3<=min(min(PB_M,32*t1),32*t2+31);t3++) {
        lbv=32*t1;
        ubv=min(PB_N-1,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          B[t4][t3-2][t3+1] = C[t3][t3] + B[t4][t3-1][t3+2] - 5;;
        }
      }
      for (t3=max(max(2,32*t2),32*t1+1);t3<=min(min(PB_M,PB_N),32*t2+31);t3++) {
        lbv=32*t1;
        ubv=min(32*t1+31,t3-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3-1][t4] = A[t3-2][t4] * 6;;
          B[t4][t3-2][t3+1] = C[t3][t3] + B[t4][t3-1][t3+2] - 5;;
        }
        if (t1 == t2) {
          lbv=t3;
...
#pragma vector always
      ...
      if (t1 <= floord(PB_M-1,32)) {
        for (t3=max(32*t2,PB_M+1);t3<=min(PB_N,32*t2+31);t3++) {
          lbv=32*t1;
...
#pragma vector always
      ...
      for (t3=max(32*t2,PB_N+1);t3<=min(PB_M,32*t2+31);t3++) {
        lbv=32*t1;
        ubv=min(PB_N-1,32*t1+31);
#pragma ivdep
#pragma vector always
...
kernel_3222212111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
