...
void kernel_2114132223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_M >= 3) {
  for (t2=-PB_M+3;t2<=0;t2++) {
    A[-t2] = B[-t2] * B[-t2] * B[-t2+1] + A[-t2+2] + A[-t2+1] * 1;;
  }
}
/* End of CLooG code */
...
kernel_2114132223_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
