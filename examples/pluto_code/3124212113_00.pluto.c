...
void kernel_3124212113_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-2,32);t2++) {
      for (t3=0;t3<=min(min(min(floord(PB_L-3,32),floord(-32*t1+PB_P-3,32)),floord(-32*t2+PB_L-1,32)),t1);t3++) {
        for (t4=max(2,32*t1);t4<=min(min(PB_P-4,32*t1+31),-32*t3+PB_P-3);t4++) {
          for (t5=max(2,32*t2);t5<=min(min(PB_L-2,32*t2+31),-32*t3+PB_L-1);t5++) {
            for (t6=max(1,32*t3);t6<=min(min(min(t4,32*t3+31),-t4+PB_P-3),-t5+PB_L-1);t6++) {
              A[t4-2][t5+t6][t6] = B[t4][t5-2][t4+t6+2] - A[t4-2][t5+t6-1][t6-1] - 2;;
            }
          }
        }
      }
...
kernel_3124212113_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
