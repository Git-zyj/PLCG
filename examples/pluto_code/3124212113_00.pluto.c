...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-3,32);t2++) {
      for (t3=2*t2;t3<=min(min(min(floord(16*t1+PB_L+12,16),floord(PB_M+2*PB_L-8,32)),floord(64*t2+PB_M+60,32)),t1+2*t2+2);t3++) {
        for (t4=max(max(max(1,32*t1),-64*t2+32*t3-61),32*t3-2*PB_L+7);t4<=min(PB_M-1,32*t1+31);t4++) {
          for (t5=max(ceild(32*t3-t4+1,2),32*t2);t5<=min(min(PB_L-3,32*t2+31),16*t3+15);t5++) {
            for (t6=max(32*t3,2*t5);t6<=min(32*t3+31,t4+2*t5-1);t6++) {
              A[t4][(-2*t5+t6)][t5] = B[(-2*t5+t6)][t4+1][t5+2] + A[t4][t5][(-2*t5+t6)+1] * 6;;
            }
          }
        }
      }
