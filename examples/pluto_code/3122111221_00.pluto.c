...
void kernel_3122111221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
/* Start of CLooG code */
if ((PB_L >= 1) && (PB_M >= 3)) {
  for (t1=0;t1<=floord(3*PB_L+PB_M-6,32);t1++) {
    lbp=max(ceild(2*t1,3),ceild(32*t1-PB_L+1,32));
    ubp=min(min(floord(2*PB_L+PB_M-5,32),floord(64*t1+PB_M+59,96)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(32*t2-PB_M+3,2),32*t1-32*t2);t3<=min(min(PB_L-1,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,2*t3+PB_M-3);t4++) {
          A[(-2*t3+t4)] = B[t3] + A[(-2*t3+t4)+2] + A[(-2*t3+t4)+1] - 5;;
        }
      }
    }
  }
...
kernel_3122111221_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
