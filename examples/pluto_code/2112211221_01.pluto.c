...
void kernel_2112211221_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 1) && (PB_P >= 4)) {
  lbp=0;
  ubp=min(floord(PB_M,32),floord(PB_P-3,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(floord(PB_L-1,32),floord(PB_P-2,32)),floord(-32*t1+PB_P-1,32));t2++) {
      for (t3=max(1,32*t1);t3<=min(min(min(PB_M,PB_P-3),32*t1+31),-32*t2+PB_P-1);t3++) {
        for (t4=max(2,32*t2);t4<=min(min(PB_L-1,32*t2+31),-t3+PB_P-1);t4++) {
          A[t3-1][t4][t3+t4] = B[t3] * A[t3-1][t4-2][t3+t4-2] + A[t3-1][t4-1][t3+t4-1] + 5;;
        }
      }
    }
  }
...
kernel_2112211221_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
