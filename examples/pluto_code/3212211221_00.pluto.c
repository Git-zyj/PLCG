...
void kernel_3212211221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
        ...
        if ((PB_L >= 2) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(2,32*t1);t4<=min(PB_P-1,32*t1+31);t4++) {
            ...
            for (t5=0;t5<=min(31,PB_L-2);t5++) {
              for (t6=2;t6<=min(31,PB_L);t6++) {
                A[t4][t5][t6-1] = C[t4][t5+1][t4] + A[t4-2][t5][t6-1] + A[t4][t5][t6-2] + 2;;
              }
            }
          }
        }
        if ((t2 <= floord(PB_L-2,32)) && (t2 >= -t3+1)) {
          for (t4=max(2,32*t1);t4<=min(PB_P-1,32*t1+31);t4++) {
            for (t5=32*t2;t5<=min(PB_L-2,32*t2+31);t5++) {
              for (t6=max(2,32*t3);t6<=min(PB_L,32*t3+31);t6++) {
                A[t4][t5][t6-1] = C[t4][t5+1][t4] + A[t4-2][t5][t6-1] + A[t4][t5][t6-2] + 2;;
              }
            }
          }
...
kernel_3212211221_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
