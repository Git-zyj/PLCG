...
void kernel_3212211221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
if (PB_P >= 1) {
  for (t1=0;t1<=floord(PB_P-1,32);t1++) {
    ...
    if ((PB_Q >= 3) && (t1 <= min(floord(PB_L-3,32),floord(PB_Q-1,32)))) {
      lbp=0;
      ubp=floord(-32*t1+PB_L-2,32);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
      for (t2=lbp;t2<=ubp;t2++) {
        for (t3=t1;t3<=floord(PB_Q-1,32);t3++) {
          for (t4=max(1,32*t2);t4<=min(32*t2+31,-32*t1+PB_L-2);t4++) {
            for (t5=max(2,32*t3);t5<=min(PB_Q-1,32*t3+31);t5++) {
              for (t6=32*t1;t6<=min(min(min(t5,PB_P-1),32*t1+31),-t4+PB_L-2);t6++) {
                B[t5][t4-1][t4+t6] = C[t4][t5][0] * D[t5][t6][t5] - B[t5+2][t4-1][t4+t6+1] + B[t5-2][t4-1][t4+t6] - B[t5+2][t4-1][t4+t6] - 3;;
              }
            }
          }
        }
...
kernel_3212211221_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
