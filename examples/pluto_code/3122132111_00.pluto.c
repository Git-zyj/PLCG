...
void kernel_3122132111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_M-2,32);t2++) {
      for (t3=max(2,32*t2);t3<=min(PB_M-2,32*t2+31);t3++) {
        lbv=max(2,32*t1);
        ubv=min(t3,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t4][t3] = B[t3][t4-2] - B[t3][t4-2] + B[t3][t4-1] * A[t4][t3+1] - 6;;
        }
      }
    }
  }
...
kernel_3122132111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
