...
void kernel_3122231221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_M-1,32);t2++) {
      for (t3=3*t2;t3<=min(floord(PB_M-1,8),floord(96*t2+PB_M+92,32));t3++) {
        for (t4=32*t1;t4<=min(PB_M-1,32*t1+31);t4++) {
          for (t5=max(max(2,ceild(32*t3-PB_M+1,3)),32*t2);t5<=min(min(floord(32*t3+31,3),PB_M-1),32*t2+31);t5++) {
            for (t6=max(32*t3,3*t5);t6<=min(32*t3+31,3*t5+PB_M-1);t6++) {
              A[t4][t5][(-3*t5+t6)] = B[0][(-3*t5+t6)][t5] + C[0][t4] - D[t5][t4][t5-1] - A[t4][(-3*t5+t6)+1][t5] + A[t4][(-3*t5+t6)][t5-2] * 6;;
            }
          }
        }
      }
...
kernel_3122231221_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
