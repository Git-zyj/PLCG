...
void kernel_3212212113_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=floord(PB_M-1,32);t3++) {
        for (t4=0;t4<=floord(PB_L-3,32);t4++) {
          for (t5=32*t3;t5<=min(PB_M-1,32*t3+31);t5++) {
            for (t6=32*t4;t6<=(min(PB_L-3,32*t4+31))-7;t6+=8) {
              lbv=32*t2;
              ubv=min(PB_P-1,32*t2+31);
#pragma ivdep
#pragma vector always
              for (t7=lbv;t7<=ubv;t7++) {
                B[t5][t7] = C[t5][t6+2][t7] * 3;;
                B[t5][t7] = C[t5][(t6+1)+2][t7] * 3;;
                B[t5][t7] = C[t5][(t6+2)+2][t7] * 3;;
                B[t5][t7] = C[t5][(t6+3)+2][t7] * 3;;
                B[t5][t7] = C[t5][(t6+4)+2][t7] * 3;;
                B[t5][t7] = C[t5][(t6+5)+2][t7] * 3;;
                B[t5][t7] = C[t5][(t6+6)+2][t7] * 3;;
                B[t5][t7] = C[t5][(t6+7)+2][t7] * 3;;
              }
            }
            for (;t6<=min(PB_L-3,32*t4+31);t6++) {
              lbv=32*t2;
              ubv=min(PB_P-1,32*t2+31);
#pragma ivdep
#pragma vector always
    ...
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=floord(PB_P-1,32);t3++) {
        for (t4=32*t2;t4<=min(PB_M-1,32*t2+31);t4++) {
          for (t5=max(1,32*t3);t5<=min(PB_P-1,32*t3+31);t5++) {
            B[t4][t5] = B[t4][t5-1] * 6;;
          }
        }
      }
...
kernel_3212212113_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
