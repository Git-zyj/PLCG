...
void kernel_3224112213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
if (PB_M >= 4) {
  for (t1=0;t1<=floord(PB_M-3,32);t1++) {
    for (t2=0;t2<=min(floord(PB_M-2,16),floord(32*t1+PB_M+30,32));t2++) {
      for (t3=max(max(0,ceild(32*t2-PB_M-13,16)),ceild(32*t1+32*t2-PB_M-13,32));t3<=min(min(min(floord(PB_M-2,16),floord(-32*t1+32*t2+PB_M-2,16)),floord(32*t1+PB_M+30,32)),floord(-32*t1+64*t2+PB_M+29,32));t3++) {
        if ((t1 == 0) && (t2 == 0) && (t3 == 0)) {
          A[1] = C[1 +2] - A[1 -1] + A[1 +1] - 6;;
        }
        if (t3 >= 1) {
          for (t4=max(max(max(2,32*t1),32*t2-PB_M+1),32*t3-PB_M+1);t4<=min(min(min(min(PB_L-1,PB_M-3),32*t1+31),32*t2+30),16*t3+14);t4++) {
            for (t5=max(32*t2,t4+1);t5<=min(32*t2+31,t4+PB_M-1);t5++) {
              lbp=max(32*t3,2*t4+2);
              ubp=min(32*t3+31,t4+PB_M-1);
#pragma omp parallel for private(lbv,ubv)
              for (t6=lbp;t6<=ubp;t6++) {
                B[(-2*t4+t6)+t4][(-t4+t5)][t4-2] = D[(-2*t4+t6)][(-t4+t5)][t4] * B[(-2*t4+t6)+t4-1][(-t4+t5)-1][(-2*t4+t6)-2] + 6;;
              }
            }
          }
        }
        if ((t1 == 0) && (t2 >= 1) && (t3 == 0)) {
          for (t4=max(2,32*t2-PB_M+1);t4<=min(14,PB_L-1);t4++) {
...
#pragma omp parallel for private(lbv,ubv)
        ...
        if ((t1 == 0) && (t2 == 0) && (t3 == 0)) {
          for (t4=2;t4<=min(min(14,PB_L-1),PB_M-3);t4++) {
            A[t4] = C[t4+2] - A[t4-1] + A[t4+1] - 6;;
            for (t5=t4+1;t5<=min(31,t4+PB_M-1);t5++) {
              lbp=2*t4+2;
              ubp=min(31,t4+PB_M-1);
#pragma omp parallel for private(lbv,ubv)
              for (t6=lbp;t6<=ubp;t6++) {
                B[(-2*t4+t6)+t4][(-t4+t5)][t4-2] = D[(-2*t4+t6)][(-t4+t5)][t4] * B[(-2*t4+t6)+t4-1][(-t4+t5)-1][(-2*t4+t6)-2] + 6;;
              }
        ...
        if ((t1 == 0) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(2,PB_L);t4<=min(14,PB_M-3);t4++) {
            A[t4] = C[t4+2] - A[t4-1] + A[t4+1] - 6;;
          }
        }
        if ((t2 == 0) && (t3 == 0)) {
          for (t4=max(15,32*t1);t4<=min(PB_M-3,32*t1+31);t4++) {
            A[t4] = C[t4+2] - A[t4-1] + A[t4+1] - 6;;
          }
        }
      }
...
kernel_3224112213_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
