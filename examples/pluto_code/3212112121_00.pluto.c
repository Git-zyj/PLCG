...
if (PB_M >= 1) {
  for (t1=0;t1<=floord(PB_M-1,8);t1++) {
    ...
    ubp=min(min(floord(3*t1+6*PB_M-6,68),floord(192*t1+3*PB_M+124,288)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      ...
      if (t2 <= floord(PB_M-1,32)) {
        for (t3=max(2,32*t1-32*t2);t3<=min(floord(32*t2,3),32*t1-32*t2+31);t3++) {
      ...
      if (t2 >= ceild(PB_M,32)) {
        for (t3=max(ceild(32*t2+1,3),32*t1-32*t2);t3<=min(min(PB_M-1,16*t2+15),32*t1-32*t2+31);t3++) {
          for (t4=max(32*t2,2*t3+1);t4<=min(32*t2+31,3*t3-1);t4++) {
            B[(-2*t3+t4)-1] = D[t3][(-2*t3+t4)] * B[(-2*t3+t4)+1] - B[(-2*t3+t4)] - 2;;
          }
        }
      }
      if (t2 <= floord(PB_M-1,32)) {
        for (t3=max(max(2,ceild(32*t2+1,3)),32*t1-32*t2);t3<=min(min(PB_M-1,16*t2+15),32*t1-32*t2+31);t3++) {
          for (t4=32*t2;t4<=min(2*t3,PB_M-1);t4++) {
            A[t4] = C[t4] * 3;;
          }
          for (t4=max(32*t2,2*t3+1);t4<=min(min(PB_M-1,32*t2+31),3*t3-1);t4++) {
            A[t4] = C[t4] * 3;;
            B[(-2*t3+t4)-1] = D[t3][(-2*t3+t4)] * B[(-2*t3+t4)+1] - B[(-2*t3+t4)] - 2;;
          }
          for (t4=3*t3;t4<=min(PB_M-1,32*t2+31);t4++) {
            A[t4] = C[t4] * 3;;
          }
          for (t4=max(PB_M,2*t3+1);t4<=min(32*t2+31,3*t3-1);t4++) {
            B[(-2*t3+t4)-1] = D[t3][(-2*t3+t4)] * B[(-2*t3+t4)+1] - B[(-2*t3+t4)] - 2;;
          }
        }
      }
