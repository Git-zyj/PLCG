...
void kernel_2112131121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_M >= 4) {
  for (t1=1;t1<=PB_M-3;t1++) {
    A[t1+1][t1-1] = B[t1] + B[t1] * B[t1] * A[t1+2][t1] + 4;;
  }
}
/* End of CLooG code */
...
kernel_2112131121_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
