...
void kernel_3222132121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
/* Start of CLooG code */
if (PB_L >= 4) {
  for (t1=max(0,ceild(-33*PB_L+198,67));t1<=floord(33*PB_L-99,32);t1++) {
    lbp=max(max(0,ceild(32*t1-PB_L+3,32)),ceild(32*t1-PB_L+5,34));
    ubp=min(floord(16*t1+15,16),floord(32736*t1+33*PB_L+31645,33792));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      if ((t1 <= floord(33*t2,32)) && (t1 >= ceild(33*t2-31,32)) && (t2 >= 1)) {
        A[t2+2][t2+2] = C[0] * C[t2] + C[t2-1] - A[t2][t2] + 3;;
      }
      if ((PB_Q >= 3) && (t2 <= floord(PB_Q-1,32))) {
        for (t3=max(max(max(3,32*t1),32*t2+1),32*t1-32*t2+2);t3<=min(min(PB_L-3,32*t1+62),32*t1-32*t2+PB_Q+30);t3++) {
          lbv=max(max(2,32*t2),-32*t1+32*t2+t3-31);
          ubv=min(min(min(PB_Q-1,32*t2+31),t3-1),-32*t1+32*t2+t3);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t3][t4] = D[t3][t4][t4] - C[t4-2] - C[t4+1] * B[t3+2][t4+2] - 2;;
          }
        }
      }
    }
...
kernel_3222132121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
