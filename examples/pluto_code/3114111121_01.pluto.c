...
void kernel_3114111121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
  ...
  for (t1=1;t1<=PB_L;t1++) {
    for (t2=0;t2<=PB_M-1;t2++) {
      for (t3=2;t3<=PB_M+1;t3++) {
        A[t2][t3] = B[t2][t3-1] + A[t3-2][t1-1] * 1;;
      }
    }
  }
}
...
kernel_3114111121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
