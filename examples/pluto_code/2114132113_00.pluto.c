...
if ((PB_L >= 3) && (PB_P >= 2)) {
  ...
  ubp=min(0,floord(PB_L-4,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,-t1-1);t2<=min(floord(PB_P,32),floord(-32*t1+PB_L-2,32));t2++) {
      for (t3=max(max(1,32*t1+2),32*t1+32*t2);t3<=min(min(min(min(PB_P,PB_L-2),32*t2+31),32*t1+PB_P+31),32*t1+32*t2+62);t3++) {
        lbv=max(max(32*t2,t3),-32*t1+t3-31);
        ubv=min(min(PB_P,-32*t1+t3),32*t2+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4-1] = B[t4] - C[t4][t3] * C[t3+1][t4] * A[t3-1][t4-2] * 5;;
        }
      }
    }
  }
