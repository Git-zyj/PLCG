...
void kernel_3112211213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_L >= 2) {
  for (t1=1;t1<=PB_L-1;t1++) {
    A[t1+1] = B[t1-1][t1] * A[t1] + A[t1+2] + 3;;
  }
}
/* End of CLooG code */
...
kernel_3112211213_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
