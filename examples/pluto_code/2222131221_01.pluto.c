...
/* Start of CLooG code */
if (PB_N >= 2) {
  for (t2=3;t2<=PB_N-1;t2++) {
    A[t2][t2-1] = C[t2+1] * C[t2-1] + C[t2+1] - C[t2] - C[t2] - A[t2-2][t2-3] - B[t2] * 1;;
  }
  if (PB_L >= 1) {
    lbp=0;
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=floord(PB_L,32);t3++) {
        for (t4=max(1,32*t2);t4<=(min(PB_N-1,32*t2+31))-7;t4+=8) {
          for (t5=max(1,32*t3);t5<=min(PB_L,32*t3+31);t5++) {
            B[t4] = A[t5][t5-1] * A[t4][t4-1] - 4;;
            B[(t4+1)] = A[t5][t5-1] * A[(t4+1)][(t4+1)-1] - 4;;
            B[(t4+2)] = A[t5][t5-1] * A[(t4+2)][(t4+2)-1] - 4;;
            B[(t4+3)] = A[t5][t5-1] * A[(t4+3)][(t4+3)-1] - 4;;
            B[(t4+4)] = A[t5][t5-1] * A[(t4+4)][(t4+4)-1] - 4;;
            B[(t4+5)] = A[t5][t5-1] * A[(t4+5)][(t4+5)-1] - 4;;
            B[(t4+6)] = A[t5][t5-1] * A[(t4+6)][(t4+6)-1] - 4;;
            B[(t4+7)] = A[t5][t5-1] * A[(t4+7)][(t4+7)-1] - 4;;
          }
        }
        for (;t4<=min(PB_N-1,32*t2+31);t4++) {
          for (t5=max(1,32*t3);t5<=min(PB_L,32*t3+31);t5++) {
            B[t4] = A[t5][t5-1] * A[t4][t4-1] - 4;;
          }
        }
