...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    ...
    for (t2=0;t2<=floord(PB_P,32);t2++) {
      for (t3=0;t3<=min(min(min(floord(PB_L-2,32),floord(-32*t2+PB_P,32)),t1),t2);t3++) {
        if ((t1 == 0) && (t2 == 0) && (t3 == 0)) {
          for (t4=1;t4<=min(3,PB_L-1);t4++) {
            A[t4] = C[t4-1] * 2;;
          }
        }
        if (t2 >= 1) {
          for (t4=max(max(4,32*t1),32*t3+1);t4<=min(min(PB_L-1,32*t1+31),-32*t2+PB_P-1);t4++) {
            for (t5=max(3,32*t3);t5<=min(min(32*t3+31,t4-1),-t4+PB_P-1);t5++) {
              for (t6=max(32*t2,t5);t6<=min(32*t2+31,-t4+PB_P-1);t6++) {
                B[t4][t5-1][t6-1] = D[t5][t6-1][t4-1] - D[t5][t6+t4][t4] * D[t5][t4][0] * D[t5][t4-1][t6] - B[t4][t5-1][t6] - B[t4][t5-3][t6-1] - 3;;
              }
            }
          }
        }
        if ((t2 == 0) && (t3 == 0)) {
          for (t4=max(4,32*t1);t4<=min(min(PB_L-1,PB_P-4),32*t1+31);t4++) {
            A[t4] = C[t4-1] * 2;;
            for (t5=3;t5<=min(min(31,t4-1),-t4+PB_P-1);t5++) {
              for (t6=t5;t6<=min(31,-t4+PB_P-1);t6++) {
                B[t4][t5-1][t6-1] = D[t5][t6-1][t4-1] - D[t5][t6+t4][t4] * D[t5][t4][0] * D[t5][t4-1][t6] - B[t4][t5-1][t6] - B[t4][t5-3][t6-1] - 3;;
              }
            }
          }
        ...
        if ((t2 == 0) && (t3 == 0)) {
          for (t4=max(max(4,32*t1),PB_P-3);t4<=min(PB_L-1,32*t1+31);t4++) {
            A[t4] = C[t4-1] * 2;;
          }
        }
      }
