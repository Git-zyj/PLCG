...
void kernel_2222232213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
if (PB_L >= 2) {
  for (t2=2;t2<=PB_L-1;t2++) {
    A[t2-1] = C[t2-2] + D[t2] - C[t2-1] - C[t2+2] - A[t2] + 3;;
  }
  if (PB_P >= 3) {
    for (t2=0;t2<=min(min(floord(PB_L,16),floord(PB_L+PB_P,32)),floord(3*PB_P-3,32));t2++) {
      lbp=max(ceild(t2,2),ceild(32*t2-PB_P+1,32));
      ubp=min(min(min(floord(PB_P-1,16),floord(PB_L+1,32)),floord(32*t2+PB_P+30,64)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(2,32*t2-32*t3),32*t3-PB_P+1);t4<=min(min(min(PB_L,PB_P-1),32*t3+30),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4+1);t5<=min(min(PB_L+1,32*t3+31),t4+PB_P-1);t5++) {
            B[(-t4+t5)+1] = E[(-t4+t5)+t4-2][0] - F[(-t4+t5)][t4] + A[(-t4+t5)-1] * B[(-t4+t5)] - B[(-t4+t5)+2] * 1;;
          }
        }
      }
    }
...
kernel_2222232213_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
