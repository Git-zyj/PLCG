...
void kernel_2112131123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_P >= 3) {
  for (t1=0;t1<=floord(3*PB_P-6,32);t1++) {
    ...
    ubp=min(min(floord(PB_P-2,16),floord(32*t1+PB_P+29,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_P+2);t3<=min(min(PB_P-2,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(32*t2+31,t3+PB_P-2);t4++) {
          A[(-t3+t4)-1] = B[t3+1][(-t3+t4)+1] * B[(-t3+t4)][t3] * B[(-t3+t4)][t3+1] - A[(-t3+t4)] - 6;;
        }
      }
    }
  }
...
kernel_2112131123_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
