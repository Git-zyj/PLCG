...
void kernel_2122132113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_L >= 3) && (PB_M >= 1)) {
  lbp=0;
  ubp=min(floord(PB_L-2,16),floord(PB_M-1,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,ceild(32*t1-PB_L-28,32));t2<=min(floord(PB_L-1,32),t1);t2++) {
      for (t3=max(32*t2,32*t1-PB_L+3);t3<=min(min(PB_L-1,PB_M-1),32*t2+31);t3++) {
        lbv=max(32*t1,t3);
        ubv=min(min(PB_M-1,32*t1+31),t3+PB_L-3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-t3+t4)+1] = B[(-t3+t4)][t3] * B[1][(-t3+t4)+2] * B[(-t3+t4)+t3][(-t3+t4)] * A[t3+1][(-t3+t4)] + 1;;
        }
      }
    }
  }
...
kernel_2122132113_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
