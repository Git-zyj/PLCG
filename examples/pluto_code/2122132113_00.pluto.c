...
void kernel_2122132113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if (PB_L >= 4) {
  lbp=ceild(-PB_L-27,32);
  ubp=floord(PB_L-3,32);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,-t1-1);t2<=min(floord(PB_L-1,32),floord(-32*t1+PB_L-1,32));t2++) {
      for (t3=max(max(3,32*t1+2),32*t1+32*t2);t3<=min(min(PB_L-1,32*t1+PB_L+30),32*t1+32*t2+62);t3++) {
        lbv=max(max(2,32*t2),-32*t1+t3-31);
        ubv=min(min(PB_L-1,-32*t1+t3),32*t2+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3-2][t4] = B[t3][t4] + B[0][t4-2] - B[t3][t4] - A[t3-3][t4-1] * 2;;
        }
      }
    }
  }
...
kernel_2122132113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
