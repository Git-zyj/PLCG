...
void kernel_2214132211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 3) {
  if (PB_M >= 4) {
    A[0 +2] = C[0] + C[0] + A[0 +3] * A[0 +1] - 5;;
  }
  for (t1=1;t1<=PB_M-4;t1++) {
    A[t1+2] = C[t1] + C[t1] + A[t1+3] * A[t1+1] - 5;;
    B[t1] = C[t1] + C[t1] - C[t1-1] - C[t1] + B[t1+1] + 1;;
  }
  for (t1=max(1,PB_M-3);t1<=PB_M-2;t1++) {
    B[t1] = C[t1] + C[t1] - C[t1-1] - C[t1] + B[t1+1] + 1;;
  }
}
/* End of CLooG code */
...
kernel_2214132211_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
