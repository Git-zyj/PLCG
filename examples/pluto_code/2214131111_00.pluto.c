...
void kernel_2214131111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_M >= 2) {
  A[0] = C[0] - C[0] * C[0] + C[0 +1] - A[0 +1] * 3;;
  for (t1=1;t1<=PB_M-2;t1++) {
    A[t1] = C[t1] - C[t1] * C[t1] + C[t1+1] - A[t1+1] * 3;;
    B[t1] = C[t1] - C[t1-1] * B[t1-1] * 1;;
  }
  B[(PB_M-1)] = C[(PB_M-1)] - C[(PB_M-1)-1] * B[(PB_M-1)-1] * 1;;
}
/* End of CLooG code */
...
kernel_2214131111_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
