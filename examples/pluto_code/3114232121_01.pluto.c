...
void kernel_3114232121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_L-1,32),t1);t2++) {
      for (t3=t2;t3<=min(min(floord(PB_M-1,32),floord(-32*t1+PB_L+PB_M-2,32)),floord(-32*t1+32*t2+PB_M+30,32));t3++) {
        for (t4=32*t1;t4<=min(min(min(PB_M-3,32*t1+31),32*t2-32*t3+PB_M+30),-32*t3+PB_L+PB_M-2);t4++) {
          for (t5=max(32*t2,32*t3+t4-PB_M+1);t5<=min(min(min(t4,PB_L-1),32*t2+31),32*t3+29);t5++) {
            for (t6=max(32*t3,t5+2);t6<=min(32*t3+31,-t4+t5+PB_M-1);t6++) {
              A[t4][(-t5+t6)] = B[t4+1][(-t5+t6)][t5] + C[(-t5+t6)][t5][(-t5+t6)-2] + B[(-t5+t6)+t4][(-t5+t6)+2][t5] + A[t4][(-t5+t6)-1] - 5;;
            }
          }
        }
      }
...
kernel_3114232121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
