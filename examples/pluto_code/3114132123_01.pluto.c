...
void kernel_3114132123_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
/* Start of CLooG code */
if ((PB_M >= 2) && (PB_Q >= 3)) {
  for (t1=0;t1<=min(floord(PB_M-1,16),floord(PB_Q+PB_M-2,32));t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_Q+1,32));
    ubp=min(floord(PB_M-1,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=t1-t2;t3<=min(min(floord(PB_Q-1,16),floord(PB_Q+PB_M-2,32)),floord(32*t1-32*t2+PB_Q+30,32));t3++) {
        for (t4=max(max(1,32*t1-32*t2),32*t3-PB_Q+1);t4<=min(min(min(PB_M-1,PB_Q-1),32*t3+29),32*t1-32*t2+31);t4++) {
          for (t5=max(32*t2,t4);t5<=min(PB_M-1,32*t2+31);t5++) {
            lbv=max(32*t3,t4+2);
            ubv=min(32*t3+31,t4+PB_Q-1);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][(-t4+t6)+1] = B[t5][t4][(-t4+t6)] + B[t5][(-t4+t6)-2][t4-1] - B[t5][t4][t4] + A[t5+2][(-t4+t6)-1] + 2;;
            }
          }
        }
      }
...
kernel_3114132123_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
