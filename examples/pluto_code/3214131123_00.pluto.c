...
void kernel_3214131123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma omp parallel for private(lbv,ubv)
        for (t4=lbp;t4<=ubp;t4++) {
          A[t3-1][t4+1] = C[0][t4] * C[t3][t3-1] * C[t3][t4+1] * A[t3][t4+2] - 4;;
        }
      }
    }
  }
...
kernel_3214131123_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
