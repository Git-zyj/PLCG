...
void kernel_2122131213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_M >= 3) {
  for (t1=1;t1<=PB_M-2;t1++) {
    A[t1] = B[0] + B[t1] * B[t1] - A[t1+1] + A[t1-1] - 3;;
  }
}
/* End of CLooG code */
...
kernel_2122131213_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
