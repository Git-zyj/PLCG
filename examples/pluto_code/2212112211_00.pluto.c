...
void kernel_2212112211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_M >= 3) {
  for (t1=0;t1<=floord(PB_M-2,32);t1++) {
    for (t2=max(0,ceild(32*t1-PB_M+3,32));t2<=floord(PB_M-2,32);t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=min(31,PB_M-3);t3++) {
          A[t3+1] = A[t3] + A[t3+2] * 1;;
          lbv=max(1,t3);
          ubv=min(31,PB_M-2);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t3][t4] = C[t4-1][t3] * B[t3+1][t4] * B[t4+1][t3] + 2;;
          }
        }
      }
      if (t2 == 0) {
...
#pragma vector always
      ...
      if ((PB_M <= 33) && (t1 == 0) && (t2 == 0)) {
        B[(PB_M-2)][(PB_M-2)] = C[(PB_M-2)-1][(PB_M-2)] * B[(PB_M-2)+1][(PB_M-2)] * B[(PB_M-2)+1][(PB_M-2)] + 2;;
      }
    }
  }
}
...
kernel_2212112211_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
