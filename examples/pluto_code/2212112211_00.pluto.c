...
void kernel_2212112211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_P >= 2) {
  B[0] = A[0][0 +1] - 3;;
  for (t1=2;t1<=PB_P-2;t1++) {
    A[t1-1][t1] = C[1] * C[t1+1] + A[t1-2][t1-1] * A[t1][t1+1] * A[t1][t1+1] - 1;;
    B[(t1-1)] = A[(t1-1)][(t1-1)+1] - 3;;
  }
  if (PB_P >= 3) {
    B[(PB_P-2)] = A[(PB_P-2)][(PB_P-2)+1] - 3;;
  }
}
/* End of CLooG code */
...
kernel_2212112211_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
