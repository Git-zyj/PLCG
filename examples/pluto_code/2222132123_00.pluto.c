...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    ...
    for (t2=0;t2<=floord(PB_L,32);t2++) {
      if ((t2 >= 1) && (t2 <= floord(PB_L-1,32))) {
        for (t3=max(2,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
          for (t4=32*t2;t4<=min(PB_L-1,32*t2+31);t4++) {
            A[t3-2][t4] = C[t4-2] - C[t3] - D[t4][t3+1] * A[t3][t4] + A[t3-2][t4-2] - 5;;
          }
        }
      }
      if ((PB_L <= 2) && (t2 == 0)) {
      ...
      if ((PB_L >= 3) && (PB_M >= 3) && (t1 >= ceild(PB_M-32,32)) && (t2 == 0)) {
        B[(PB_M-1)] = C[(PB_M-1)-1] * C[(PB_M-1)] + C[(PB_M-1)] - 1;;
      }
    }
  }
}
