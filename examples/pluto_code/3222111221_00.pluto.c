...
void kernel_3222111221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
if (PB_M >= 5) {
  for (t1=0;t1<=floord(PB_M-3,32);t1++) {
    for (t2=0;t2<=min(floord(PB_M-4,32),t1);t2++) {
      if ((PB_L <= -1) && (t2 == 0)) {
        for (t4=max(2,32*t1);t4<=min(PB_M-3,32*t1+31);t4++) {
          B[t4] = B[t4+2] - B[t4-2] + 4;;
        }
      }
      if (t2 <= floord(3*PB_M+PB_L-15,96)) {
        for (t3=0;t3<=min(floord(PB_L,32),floord(-32*t2+PB_M+PB_L-5,32));t3++) {
          if ((PB_L >= 3) && (PB_Q >= 2) && (t2 == 0) && (t3 == 0)) {
            for (t4=max(2,32*t1);t4<=min(PB_M-3,32*t1+31);t4++) {
              B[t4] = B[t4+2] - B[t4-2] + 4;;
              for (t5=2;t5<=min(31,PB_L-1);t5++) {
                lbp=1;
                ubp=min(min(31,PB_Q-1),t4-1);
#pragma omp parallel for private(lbv,ubv)
                for (t6=lbp;t6<=ubp;t6++) {
                  A[t4+1][t5][t6] = C[t6][t4][t5] * A[t4+1][t5-2][t6-1] * 4;;
                }
              }
            }
          }
          if ((PB_L >= 3) && (PB_Q >= 2) && (t2 <= floord(PB_Q-1,32)) && (t2 >= -t3+1) && (t3 <= floord(PB_L-1,32))) {
            for (t4=max(max(2,32*t1),32*t2+1);t4<=min(PB_M-3,32*t1+31);t4++) {
              for (t5=max(2,32*t3);t5<=min(PB_L-1,32*t3+31);t5++) {
                lbp=max(1,32*t2);
                ubp=min(min(PB_Q-1,32*t2+31),t4-1);
#pragma omp parallel for private(lbv,ubv)
                for (t6=lbp;t6<=ubp;t6++) {
                  A[t4+1][t5][t6] = C[t6][t4][t5] * A[t4+1][t5-2][t6-1] * 4;;
                }
...
kernel_3222111221_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
