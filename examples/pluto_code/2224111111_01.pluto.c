...
void kernel_2224111111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M-2,32);t1++) {
    for (t2=0;t2<=min(floord(2*t1+PB_M-2,17),2*t1+1);t2++) {
      if ((t1 == 0) && (t2 == 0)) {
      ...
      if (t2 >= 1) {
        lbp=max(32*t1,16*t2+1);
        ubp=min(min(PB_M-2,32*t1+31),32*t2+30);
#pragma omp parallel for private(lbv,ubv,t4)
        for (t3=lbp;t3<=ubp;t3++) {
          lbv=max(32*t2,t3+1);
          ubv=min(32*t2+31,2*t3-1);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t3][(-t3+t4)] = C[(-t3+t4)] + B[t3+1][(-t3+t4)-1] + 1;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(30,PB_M-2);t3++) {
          A[t3] = A[t3+1] - 5;;
          lbv=t3+1;
          ubv=min(31,2*t3-1);
#pragma ivdep
...
kernel_2224111111_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
