...
void kernel_2114111113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_L >= 2) && (PB_N >= 2)) {
  lbp=0;
  ubp=min(floord(PB_N-2,32),floord(PB_L-2,64));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(-32*t1+PB_L-1,32);t2++) {
      for (t3=32*t1;t3<=min(min(min(min(floord(PB_L-2,2),PB_N-2),32*t1+31),32*t2+30),-32*t2+PB_L-1);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(32*t2+31,-t3+PB_L-1);t4++) {
          A[t3+1][t4+1] = B[t3][t3+t4] + A[t3+1][t4+2] - 3;;
        }
      }
    }
  }
...
kernel_2114111113_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
