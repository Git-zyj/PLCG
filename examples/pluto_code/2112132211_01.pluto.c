...
void kernel_2112132211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_M >= 3) {
  for (t2=-PB_M+2;t2<=-1;t2++) {
    A[-t2] = B[-t2] - B[-t2-1] * B[-t2] * A[-t2-1] + A[-t2+1] - 6;;
  }
}
/* End of CLooG code */
...
kernel_2112132211_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
