...
void kernel_2222131113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_N >= 3) {
  for (t1=0;t1<=floord(PB_N-3,32);t1++) {
    for (t2=0;t2<=min(floord(-32*t1+PB_N-3,32),t1);t2++) {
      if (t2 >= 1) {
        for (t3=max(32*t1,32*t2+1);t3<=min(32*t1+31,-32*t2+PB_N-3);t3++) {
          lbp=32*t2;
          ubp=min(min(32*t2+31,t3-1),-t3+PB_N-3);
#pragma omp parallel for private(lbv,ubv)
          for (t4=lbp;t4<=ubp;t4++) {
            A[t4+t3][t3] = C[t4+t3][1] - D[t3] - A[t4+t3+1][t3+1] - A[t4+t3+2][t3+1] + 5;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        B[0] = D[0] + C[0][0] - D[0] * C[0][0] - 1;;
      }
      if (t2 == 0) {
        for (t3=max(1,32*t1);t3<=min(PB_N-3,32*t1+31);t3++) {
          A[0 +t3][t3] = C[0 +t3][1] - D[t3] - A[0 +t3+1][t3+1] - A[0 +t3+2][t3+1] + 5;;
          B[t3] = D[t3] + C[t3][t3] - D[t3] * C[t3][t3] - 1;;
          lbp=1;
          ubp=min(min(31,t3-1),-t3+PB_N-3);
#pragma omp parallel for private(lbv,ubv)
          for (t4=lbp;t4<=ubp;t4++) {
            A[t4+t3][t3] = C[t4+t3][1] - D[t3] - A[t4+t3+1][t3+1] - A[t4+t3+2][t3+1] + 5;;
          }
...
kernel_2222131113_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
