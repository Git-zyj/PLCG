...
void kernel_3212212213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 1) && (PB_P >= 1) && (PB_P >= -2*PB_L+5)) {
  for (t1=max(0,-PB_L+2);t1<=min(PB_P-1,PB_P+PB_L-3);t1++) {
    if ((PB_M >= 0) && (t1 <= floord(2*PB_P+PB_M-4,2)) && (t1 >= ceild(-PB_M+2,2))) {
      for (t2=0;t2<=min(min(floord(PB_L+PB_M-1,32),floord(t1+PB_L+PB_M-2,32)),floord(-t1+PB_P+PB_L+PB_M-3,32));t2++) {
        if ((t1 >= 1) && (t1 <= PB_P-2) && (t2 <= floord(PB_L-1,32))) {
          for (t5=32*t2;t5<=min(PB_L-1,32*t2+31);t5++) {
            A[t1+1][t1] = A[t1][t1-1] - A[t1+2][t1+1] + 4;;
          }
        }
        if ((PB_L >= 2) && (PB_M >= 2) && (t2 <= floord(PB_M,32))) {
          for (t3=0;t3<=floord(PB_L+1,32);t3++) {
            for (t4=max(3,32*t3);t4<=min(PB_L+1,32*t3+31);t4++) {
              for (t5=max(2,32*t2);t5<=min(PB_M,32*t2+31);t5++) {
                B[t4][t5-1][t4-2] = C[t4-1][t1][t1] + B[t4-1][t5-2][t4-3] - 2;;
              }
            }
          }
        }
...
kernel_3212212213_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
