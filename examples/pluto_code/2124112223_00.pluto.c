...
void kernel_2124112223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(32*t2-PB_M+3,2),32*t1-32*t2);t3<=min(min(PB_M-1,16*t2+14),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3+2);t4<=min(32*t2+31,2*t3+PB_M-3);t4++) {
          A[(-2*t3+t4)][(-2*t3+t4)] = B[(-2*t3+t4)] - A[(-2*t3+t4)-2][(-2*t3+t4)-2] - A[(-2*t3+t4)+2][(-2*t3+t4)+2] * 1;;
        }
      }
    }
  }
...
kernel_2124112223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
