...
for (t1=1;t1<=min(PB_L-3,PB_M-3);t1++) {
  A[t1+1] = C[t1] * C[t1-1] + A[t1-1] + A[t1+2] + 6;;
  B[t1] = B[t1+2] + B[t1+1] * 5;;
}
for (t1=max(1,PB_M-2);t1<=PB_L-3;t1++) {
  A[t1+1] = C[t1] * C[t1-1] + A[t1-1] + A[t1+2] + 6;;
}
for (t1=max(1,PB_L-2);t1<=PB_M-3;t1++) {
  B[t1] = B[t1+2] + B[t1+1] * 5;;
}
/* End of CLooG code */
