...
void kernel_2114131223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if ((PB_L >= 4) && (PB_M >= 6)) {
  for (t1=0;t1<=floord(PB_M-4,32);t1++) {
    ...
    ubp=min(min(floord(PB_L-3,32),floord(PB_M-5,32)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(max(2,32*t1),32*t2+1),32*t1-32*t2+1);t3<=min(min(PB_M-4,32*t1+62),32*t1-32*t2+PB_L+28);t3++) {
        lbv=max(max(1,32*t2),-32*t1+32*t2+t3-31);
        ubv=min(min(min(PB_L-3,32*t2+31),t3-1),-32*t1+32*t2+t3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+1][t4] = B[t4-1] - C[t3-1][t4-1] + C[t4][t3] + A[t3+3][t4] + A[t3+3][t4+2] * 4;;
        }
      }
    }
  }
...
kernel_2114131223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
