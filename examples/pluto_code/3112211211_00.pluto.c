...
void kernel_3112211211_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
/* Start of CLooG code */
if ((PB_L >= 4) && (PB_N >= 3)) {
  for (t1=0;t1<=floord(2*PB_L+PB_N-8,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_L+3,32));
    ubp=min(min(floord(PB_L+PB_N-5,32),floord(32*t1+PB_N+29,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,32*t1-32*t2),32*t2-PB_N+2);t3<=min(min(PB_L-3,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=0;t4<=PB_L-1-7;t4+=8) {
          lbv=max(32*t2,t3+1);
          ubv=min(32*t2+31,t3+PB_N-2);
#pragma ivdep
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            A[t3][t3+1][(-t3+t5)] = B[(-t3+t5)+1][t3][1] - A[t3+1][t3+2][(-t3+t5)-1] + A[t3-1][t3][(-t3+t5)] * 4;;
            A[t3][t3+1][(-t3+t5)] = B[(-t3+t5)+1][t3][1] - A[t3+1][t3+2][(-t3+t5)-1] + A[t3-1][t3][(-t3+t5)] * 4;;
            A[t3][t3+1][(-t3+t5)] = B[(-t3+t5)+1][t3][1] - A[t3+1][t3+2][(-t3+t5)-1] + A[t3-1][t3][(-t3+t5)] * 4;;
            A[t3][t3+1][(-t3+t5)] = B[(-t3+t5)+1][t3][1] - A[t3+1][t3+2][(-t3+t5)-1] + A[t3-1][t3][(-t3+t5)] * 4;;
            A[t3][t3+1][(-t3+t5)] = B[(-t3+t5)+1][t3][1] - A[t3+1][t3+2][(-t3+t5)-1] + A[t3-1][t3][(-t3+t5)] * 4;;
...
kernel_3112211211_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
