...
void kernel_3112211211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_M >= 2) && (PB_Q >= 1)) {
  for (t1=1;t1<=PB_Q;t1++) {
    for (t2=0;t2<=floord(2*PB_M+PB_L-6,32);t2++) {
      lbp=max(ceild(t2,2),ceild(32*t2-PB_M+2,32));
      ubp=min(min(floord(PB_M+PB_L-4,32),floord(32*t2+PB_L+29,64)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(32*t2-32*t3,32*t3-PB_L+2);t4<=min(PB_M-2,32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4);t5<=min(32*t3+31,t4+PB_L-2);t5++) {
            A[t1][(-t4+t5)] = B[(-t4+t5)][t4][t1-1] - A[t4][(-t4+t5)+1] + A[t4+1][(-t4+t5)+1] - 4;;
          }
        }
      }
    }
...
kernel_3112211211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
