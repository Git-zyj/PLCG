...
void kernel_2114232221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_M >= 6) {
  for (t2=-PB_M+4;t2<=-2;t2++) {
    A[-t2+2] = B[0] + B[-t2-2] + C[-t2] * A[-t2] - A[-t2+3] + 2;;
  }
}
/* End of CLooG code */
...
kernel_2114232221_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
