...
void kernel_2214212211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_N >= 3)) {
  for (t1=0;t1<=floord(PB_N+PB_L-3,32);t1++) {
    lbp=max(max(0,ceild(32*t1-PB_N+2,32)),ceild(32*t1-PB_N-PB_L+5,64));
    ubp=min(floord(PB_L-1,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      if (PB_L >= 3) {
        for (t3=max(1,32*t1-32*t2);t3<=min(min(min(PB_L-1,PB_N-2),32*t2+31),32*t1-32*t2+31);t3++) {
          if ((t1 == 0) && (t2 == 0) && (t3 == 1)) {
            A[1][1] = C[1][1 +2] + D[1][1] * A[1 +1][1] * 4;;
          }
          if (t1 == 2*t2) {
            for (t4=max(2,16*t1);t4<=t3-1;t4++) {
              if (t1%2 == 0) {
                B[t3][t4-1] = B[t3-1][t4-1] - B[t3-1][t4-2] * B[t3][t4] + 4;;
              }
            }
          }
          for (t4=max(max(2,32*t2),t3);t4<=min(PB_L-1,32*t2+31);t4++) {
            A[t3][t4] = C[t4][t3+2] + D[t4][t4] * A[t3+1][t4] * 4;;
            B[t3][t4-1] = B[t3-1][t4-1] - B[t3-1][t4-2] * B[t3][t4] + 4;;
          }
        }
      }
      ...
      if ((PB_L >= 3) && (t1 == 2*t2)) {
        for (t3=PB_L;t3<=min(PB_N-2,16*t1+31);t3++) {
          for (t4=max(2,16*t1);t4<=PB_L-1;t4++) {
            if (t1%2 == 0) {
              B[t3][t4-1] = B[t3-1][t4-1] - B[t3-1][t4-2] * B[t3][t4] + 4;;
            }
          }
...
kernel_2214212211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
