...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,32*t2),-32*t1+32*t2-30);t3<=min(min(PB_L-1,32*t2+31),-32*t1+32*t2+PB_L-2);t3++) {
        for (t4=max(1,32*t1-32*t2+t3);t4<=min(PB_L-2,32*t1-32*t2+t3+31);t4++) {
          A[t4][t3] = B[t3][t4+1] + C[t3+1][t4] - B[t3][0] * A[t4-1][t3-1] * A[t4-1][t3] * 2;;
        }
      }
    }
  }
