...
void kernel_3114132223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(2,32*t1-32*t2);t3<=min(min(min(floord(PB_M-1,2),PB_L-1),32*t1-32*t2+31),-32*t2+PB_M-1);t3++) {
        for (t4=max(32*t2,t3);t4<=min(32*t2+31,-t3+PB_M-1);t4++) {
          A[t3-2][t4] = B[t3][t4][t3] - C[t4+t3][t4] - C[t4][t3] * A[t3-2][t4-2] + A[t3][t4] * 1;;
        }
      }
    }
  }
...
kernel_3114132223_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
