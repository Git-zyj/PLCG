...
void kernel_3212111121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma omp parallel for private(lbv,ubv,t7,t8)
        for (t6=lbp;t6<=ubp;t6++) {
          lbv=max(1,32*t6);
          ubv=min(PB_L-3,32*t6+31);
#pragma ivdep
#pragma vector always
          for (t7=lbv;t7<=ubv;t7++) {
            A[t2][t7][t4] = C[t4][t2][t4+1] - C[t4+1][t7-1][t2] + A[t2+1][t7+2][t4] * B[t7][t2+1] - 2;;
          }
        }
      }
      B[t2][t4+1] = 4;;
...
kernel_3212111121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
