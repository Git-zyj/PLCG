...
void kernel_3122112111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_L >= 2) && (PB_M >= 3)) {
  lbp=0;
  ubp=min(floord(PB_L-2,32),floord(PB_M-3,32));
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t4=ceild(-PB_M-28,32);t4<=-t2;t4++) {
      for (t6=max(32*t4,-PB_M+3);t6<=min(-32*t2,32*t4+31);t6++) {
        lbv=32*t2;
        ubv=min(min(-t6,PB_L-2),32*t2+31);
#pragma ivdep
#pragma vector always
        for (t7=lbv;t7<=ubv;t7++) {
          A[t7][-t6+2] = B[-t6][t7+1] - A[t7][-t6+1] + 1;;
        }
      }
    }
  }
...
kernel_3122112111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
