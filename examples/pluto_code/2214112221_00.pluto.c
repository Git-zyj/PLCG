...
void kernel_2214112221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_L >= 3) {
  for (t1=0;t1<=PB_L-3;t1++) {
    if (t1 <= PB_L-4) {
      lbp=0;
      ubp=floord(-t1+PB_L-3,32);
#pragma omp parallel for private(lbv,ubv,t4)
      for (t3=lbp;t3<=ubp;t3++) {
        lbv=max(1,32*t3);
        ubv=min(32*t3+31,-t1+PB_L-3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t4-1][t4+t1+2] = C[t4][t1+2] - A[t4-1][t4+t1+1] + B[t4+2] * B[t1] - 5;;
        }
      }
    }
    B[t1] = D[t1] - 1;;
...
kernel_2214112221_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
