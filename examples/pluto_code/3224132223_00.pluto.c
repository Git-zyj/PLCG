...
void kernel_3224132223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
if (PB_L >= 3) {
  for (t1=0;t1<=floord(PB_L-1,32);t1++) {
    for (t2=0;t2<=floord(PB_L+1,32);t2++) {
      for (t3=0;t3<=min(floord(PB_L-2,32),t1);t3++) {
        if ((PB_L >= 6) && (t1 == 0) && (t2 == 0) && (t3 == 0)) {
          A[1 +2] = C[1 -1] - C[1 +2] + C[1] + C[1] * A[1 +3] - A[1] - A[1 +4] - A[1 +1] - 6;;
        }
        if (t2 >= -t3+1) {
          for (t4=max(max(2,32*t1),32*t3+1);t4<=min(PB_L-1,32*t1+31);t4++) {
            for (t5=max(2,32*t2);t5<=min(PB_L+1,32*t2+31);t5++) {
              for (t6=max(1,32*t3);t6<=min(32*t3+31,t4-1);t6++) {
                B[t4][t5-2][t4] = D[t5][t4][t6] * D[t4+1][t6-1][t5] - 3;;
              }
            }
          }
        }
        if ((t2 == 0) && (t3 == 0)) {
          for (t4=max(2,32*t1);t4<=min(PB_L-5,32*t1+31);t4++) {
            A[t4+2] = C[t4-1] - C[t4+2] + C[t4] + C[t4] * A[t4+3] - A[t4] - A[t4+4] - A[t4+1] - 6;;
        ...
        if ((t2 == 0) && (t3 == 0)) {
          for (t4=max(max(2,32*t1),PB_L-4);t4<=min(PB_L-1,32*t1+31);t4++) {
            for (t5=2;t5<=min(31,PB_L+1);t5++) {
              for (t6=1;t6<=min(31,t4-1);t6++) {
                B[t4][t5-2][t4] = D[t5][t4][t6] * D[t4+1][t6-1][t5] - 3;;
              }
...
kernel_3224132223_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
