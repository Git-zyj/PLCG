...
void kernel_2212132213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 3) {
  if (PB_M >= 2) {
    for (t2=0;t2<=floord(2*PB_L+PB_M-2,32);t2++) {
      lbp=max(ceild(t2,2),ceild(32*t2-PB_L+1,32));
      ubp=min(min(floord(PB_L+PB_M-1,32),floord(32*t2+PB_M+31,64)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(2,32*t2-32*t3),32*t3-PB_M);t4<=min(min(PB_L-1,32*t3+29),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4+2);t5<=min(32*t3+31,t4+PB_M);t5++) {
            A[(-t4+t5)-2] = C[t4] + A[(-t4+t5)-1] * B[t4-2][t4] * 2;;
          }
        }
      }
    }
  }
  for (t2=3;t2<=PB_L-2;t2++) {
    B[t2-2][t2] = C[t2] + C[t2+1] + C[t2] - C[t2+1] + B[t2-3][t2-1] * B[t2-1][t2+1] + 6;;
  }
}
/* End of CLooG code */
...
kernel_2212132213_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
