...
void kernel_3112212113_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
    ...
    for (t2=0;t2<=floord(PB_Q-1,32);t2++) {
      for (t3=0;t3<=floord(PB_L,32);t3++) {
        for (t4=max(2,32*t1);t4<=min(PB_L-1,32*t1+31);t4++) {
          for (t5=max(2,32*t3);t5<=(min(PB_L,32*t3+31))-7;t5+=8) {
            lbv=max(2,32*t2);
            ubv=min(PB_Q-1,32*t2+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][t5-1][t6-2] = B[t4-2][t4][t6] * A[t4][t5-2][t6-2] * 2;;
              A[t4][(t5+1)-1][t6-2] = B[t4-2][t4][t6] * A[t4][(t5+1)-2][t6-2] * 2;;
              A[t4][(t5+2)-1][t6-2] = B[t4-2][t4][t6] * A[t4][(t5+2)-2][t6-2] * 2;;
              A[t4][(t5+3)-1][t6-2] = B[t4-2][t4][t6] * A[t4][(t5+3)-2][t6-2] * 2;;
              A[t4][(t5+4)-1][t6-2] = B[t4-2][t4][t6] * A[t4][(t5+4)-2][t6-2] * 2;;
              A[t4][(t5+5)-1][t6-2] = B[t4-2][t4][t6] * A[t4][(t5+5)-2][t6-2] * 2;;
              A[t4][(t5+6)-1][t6-2] = B[t4-2][t4][t6] * A[t4][(t5+6)-2][t6-2] * 2;;
              A[t4][(t5+7)-1][t6-2] = B[t4-2][t4][t6] * A[t4][(t5+7)-2][t6-2] * 2;;
            }
          }
          for (;t5<=min(PB_L,32*t3+31);t5++) {
            lbv=max(2,32*t2);
            ubv=min(PB_Q-1,32*t2+31);
#pragma ivdep
#pragma vector always
...
kernel_3112212113_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
