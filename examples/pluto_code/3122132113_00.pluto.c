...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_M-1,32),t1);t2++) {
      for (t3=3*t2;t3<=min(min(floord(PB_M-1,8),floord(PB_M+3*PB_Q-7,32)),floord(96*t2+PB_M+92,32));t3++) {
        for (t4=max(max(1,ceild(32*t3-PB_M+1,3)),32*t2);t4<=min(min(min(floord(32*t3+30,3),PB_M-1),PB_Q-2),32*t2+31);t4++) {
          for (t5=max(32*t3,3*t4+1);t5<=min(32*t3+31,3*t4+PB_M-1);t5++) {
            lbv=max(32*t1,t4);
            ubv=min(PB_Q-2,32*t1+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[(-3*t4+t5)][t4][t6] = B[(-3*t4+t5)][t6][t4] * B[t4][(-3*t4+t5)][t6+1] * B[(-3*t4+t5)][t4-1][0] * A[t4-1][(-3*t4+t5)-1][t6] - 6;;
            }
          }
        }
      }
