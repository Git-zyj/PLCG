...
void kernel_3122131223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_L >= 1) && (PB_M >= 1)) {
  for (t2=max(-PB_L+1,-PB_M+1);t2<=0;t2++) {
    for (t4=0;t4<=min(PB_L-1,t2+PB_M-1);t4++) {
      A[-t2][0] = B[t4][-t2] * B[t4+-t2][-t2] + B[-t2][t4] - A[t4+2][0] * A[-t2+2][0] * 1;;
    }
  }
}
/* End of CLooG code */
...
kernel_3122131223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
