...
void kernel_2114112211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_M >= 4)) {
  for (t1=0;t1<=floord(PB_M+PB_L-5,32);t1++) {
    ...
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t1-32*t2;t3<=min(PB_M-4,32*t1-32*t2+31);t3++) {
        for (t4=max(1,32*t2);t4<=min(PB_L-1,32*t2+31);t4++) {
          A[t3+2][t4] = B[t3][t4] * A[t3+3][t4] + A[t3+2][t4-1] - 3;;
        }
      }
    }
  }
...
kernel_2114112211_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
