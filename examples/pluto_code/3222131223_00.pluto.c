...
/* Start of CLooG code */
if (PB_M >= 1) {
  for (t2=0;t2<=min(min(floord(PB_L-2,8),floord(5*PB_M-5,32)),floord(3*PB_M+PB_L-5,32));t2++) {
    lbp=max(ceild(3*t2,4),ceild(32*t2-PB_M+1,32));
    ubp=min(min(min(min(min(floord(PB_M-1,8),floord(3*PB_L-6,32)),floord(2*PB_M+PB_L-4,32)),floord(64*t2+PB_L+60,96)),floord(96*t2+PB_M+92,128)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
    for (t3=lbp;t3<=ubp;t3++) {
      for (t4=max(max(ceild(32*t3-PB_L+2,2),ceild(32*t3-PB_M+1,3)),32*t2-32*t3);t4<=min(min(min(floord(32*t3+31,3),PB_L-2),PB_M-1),32*t2-32*t3+31);t4++) {
        for (t5=max(32*t3,3*t4);t5<=min(min(32*t3+31,2*t4+PB_L-2),3*t4+PB_M-1);t5++) {
          A[t4][(-3*t4+t5)] = C[(-3*t4+t5)+t4+1][t4] + A[(-3*t4+t5)][t4+2] + B[(-3*t4+t5)] + 6;;
        }
      }
    }
  }
  for (t2=1;t2<=PB_M-3;t2++) {
    B[t2] = D[t2-1] * D[t2] + D[t2] - D[t2] * B[t2+2] + B[t2-1] * 4;;
  }
}
/* End of CLooG code */
