...
void kernel_3222211223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    if ((PB_L <= -1) && (PB_M >= 4)) {
      for (t3=max(3,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
        B[t3-1] = B[t3-3] - B[t3] - 5;;
      }
    }
    if (PB_M >= -PB_L+4) {
      for (t2=0;t2<=min(floord(PB_L,32),floord(PB_M+2*PB_L-4,64));t2++) {
        if ((t2 >= 1) && (t2 <= floord(PB_L-1,32))) {
          lbp=32*t1;
          ubp=min(PB_M-2,32*t1+31);
...
#pragma vector always
            for (t4=lbv;t4<=ubv;t4++) {
              A[t3][t4-1] = C[t3][t4-1][0] + A[t3+1][t4-1] * 5;;
            }
          }
        }
        if ((PB_L >= 2) && (t1 == 0) && (t2 == 0)) {
...
#pragma vector always
        ...
        if ((PB_L >= 2) && (PB_M >= 4) && (t1 >= ceild(PB_M-32,32)) && (t2 == 0)) {
          B[(PB_M-1)-1] = B[(PB_M-1)-3] - B[(PB_M-1)] - 5;;
        }
      }
    }
  }
...
kernel_3222211223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
