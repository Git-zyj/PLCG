...
void kernel_2212212221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    ...
    for (t2=0;t2<=floord(PB_L,32);t2++) {
      ...
      if ((PB_L >= 5) && (t1 == 0) && (t2 == 0)) {
        B[1 +1] = C[1] + B[1 -1] + B[1 +2] + 1;;
      }
      if ((PB_L >= 5) && (t2 == 0)) {
        for (t3=max(2,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
          B[t3+1] = C[t3] + B[t3-1] + B[t3+2] + 1;;
          for (t4=2;t4<=min(31,PB_L-3);t4++) {
            A[t3-2][t4] = C[t3] * A[t3-2][t4+2] - A[t3-2][t4-2] + 4;;
          }
        }
      }
...
kernel_2212212221_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
