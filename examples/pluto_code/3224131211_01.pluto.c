...
void kernel_3224131211_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_M >= 3) {
  for (t1=0;t1<=floord(PB_M-2,32);t1++) {
    for (t2=0;t2<=min(floord(PB_M-3,32),t1);t2++) {
      for (t3=0;t3<=min(floord(PB_M-3,32),t1);t3++) {
        if (t2 >= -t3+1) {
          for (t4=max(max(32*t1,32*t2+1),32*t3+1);t4<=min(min(min(PB_L-2,PB_M-2),32*t1+31),-32*t3+PB_L-1);t4++) {
            for (t5=max(1,32*t2);t5<=min(32*t2+31,t4-1);t5++) {
              for (t6=max(1,32*t3);t6<=min(min(32*t3+31,t4-1),-t4+PB_L-1);t6++) {
                A[t4][t5][t6+t5] = C[t6][t4][t5] - C[t4][t6-1][t5-1] - A[t4][t5][t6+t5-1] * A[t5+1][t6][t6+t4] * 5;;
              }
            }
          }
        }
        if ((t1 == 0) && (t2 == 0) && (t3 == 0)) {
          B[1][1] = D[1 +1] + D[1 -1] + D[1 -1] + B[1 +1][1] * 6;;
        }
        if ((t2 == 0) && (t3 == 0)) {
          for (t4=max(2,32*t1);t4<=min(min(PB_L-2,PB_M-2),32*t1+31);t4++) {
            B[t4][1] = D[t4+1] + D[t4-1] + D[t4-1] + B[t4+1][1] * 6;;
            for (t5=1;t5<=min(31,t4-1);t5++) {
              for (t6=1;t6<=min(min(31,t4-1),-t4+PB_L-1);t6++) {
                A[t4][t5][t6+t5] = C[t6][t4][t5] - C[t4][t6-1][t5-1] - A[t4][t5][t6+t5-1] * A[t5+1][t6][t6+t4] * 5;;
              }
            }
          }
        ...
        if ((t2 == 0) && (t3 == 0)) {
          for (t4=max(max(2,32*t1),PB_L-1);t4<=min(PB_M-2,32*t1+31);t4++) {
            B[t4][1] = D[t4+1] + D[t4-1] + D[t4-1] + B[t4+1][1] * 6;;
          }
        }
      }
...
kernel_3224131211_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
