...
void kernel_3212212121_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
for (t1=2;t1<=min(PB_L-3,PB_M-1);t1++) {
  A[t1] = C[t1-1][0] + D[t1+2] - A[t1-2] + 6;;
  B[t1] = 4;;
}
for (t1=max(2,PB_M);t1<=PB_L-3;t1++) {
  A[t1] = C[t1-1][0] + D[t1+2] - A[t1-2] + 6;;
}
for (t1=max(2,PB_L-2);t1<=PB_M-1;t1++) {
  B[t1] = 4;;
}
/* End of CLooG code */
...
kernel_3212212121_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
