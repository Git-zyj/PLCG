...
void kernel_2114232213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_M >= 4)) {
  for (t1=0;t1<=floord(PB_M+PB_L-4,32);t1++) {
    lbp=max(0,ceild(32*t1-PB_M+4,32));
    ubp=min(floord(PB_L,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t1-32*t2;t3<=min(PB_M-4,32*t1-32*t2+31);t3++) {
        for (t4=max(2,32*t2);t4<=min(PB_L,32*t2+31);t4++) {
          A[t3+2][t4-2] = B[t3] + B[0] * B[t4] * A[t3+2][t4-1] - A[t3+3][t4-2] + 1;;
        }
      }
    }
  }
...
kernel_2114232213_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
