...
void kernel_2122211111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_M >= 2) {
  for (t1=1;t1<=PB_M-1;t1++) {
    for (t2=0;t2<=-t1+PB_M-1;t2++) {
      A[t2][t2+1] = B[t1][t2+t1] + A[t1-1][t1] + 1;;
    }
  }
}
/* End of CLooG code */
...
kernel_2122211111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
