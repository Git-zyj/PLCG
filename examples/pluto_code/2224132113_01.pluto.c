...
void kernel_2224132113_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_M >= 2) {
  if (PB_M >= 4) {
    for (t2=0;t2<=floord(3*PB_M-3,64);t2++) {
      lbp=ceild(2*t2,3);
      ubp=min(floord(PB_M-1,32),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(1,32*t2-32*t3),32*t3-PB_M+3);t4<=min(min(floord(PB_M-1,2),16*t3+15),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,2*t4);t5<=min(min(PB_M-1,32*t3+31),t4+PB_M-3);t5++) {
            A[(-t4+t5)-1] = C[(-t4+t5)][t4-1] * C[(-t4+t5)+1][t4] * C[(-t4+t5)][t4] - D[(-t4+t5)+2] * C[(-t4+t5)][(-t4+t5)+t4] - A[(-t4+t5)] + 4;;
          }
        }
      }
    }
...
kernel_2224132113_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
