...
void kernel_3114131123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-1,32);t2++) {
      for (t3=32*t1;t3<=min(PB_L-1,32*t1+31);t3++) {
        for (t4=max(3,32*t2);t4<=min(PB_L-1,32*t2+31);t4++) {
          A[t3][t4-1] = B[t3] * B[t4] * C[t3][t4] + A[t3][t4-3] * 3;;
        }
      }
    }
  }
...
kernel_3114131123_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
