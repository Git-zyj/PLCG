...
void kernel_2122212111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_M >= 1) && (PB_N >= 2)) {
  for (t1=0;t1<=floord(2*PB_M+PB_N-3,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_M+1,32));
    ubp=min(min(floord(PB_M+PB_N-2,32),floord(32*t1+PB_N+30,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_N+1);t3<=min(min(PB_M-1,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(32*t2+31,t3+PB_N-1);t4++) {
          A[(-t3+t4)] = B[(-t3+t4)][0] + A[(-t3+t4)-1] - 3;;
        }
      }
    }
  }
...
kernel_2122212111_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
