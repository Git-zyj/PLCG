...
/* Start of CLooG code */
if (PB_N >= 5) {
  for (t1=-1;t1<=floord(PB_N-3,16);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_N+4,32));
    ubp=min(floord(PB_N-1,32),floord(32*t1+PB_N+26,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,16*t1),32*t1-32*t2+1);t3<=min(min(min(floord(32*t1-32*t2+PB_N+30,2),PB_N-3),16*t1+31),32*t2+30);t3++) {
        lbv=max(max(32*t2,t3+1),-32*t1+32*t2+2*t3-31);
        ubv=min(min(PB_N-1,32*t2+31),-32*t1+32*t2+2*t3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4] = B[t3][t4] + B[t3-2][t4+1] + C[t3+2][t4] - A[t3+1][t4] - A[t3+1][t4+2] + 5;;
        }
      }
    }
  }
