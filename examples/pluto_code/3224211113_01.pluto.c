...
if (PB_L >= 2) {
  if (PB_M >= 0) {
    lbp=0;
    ubp=min(floord(PB_M+PB_L-1,32),floord(PB_M+3*PB_L-7,32));
      ...
      for (t2=0;t2<=floord(PB_L,32);t2++) {
        ...
        if (32*t2 == PB_L) {
          for (t3=max(2,32*t1);t3<=min(PB_L-1,32*t1+31);t3++) {
            if (PB_L%32 == 0) {
              A[(t3-1)+1][PB_L-1] = C[PB_L+1][(t3-1)+1] * D[PB_L+1][(t3-1)-1] * 2;;
            }
          }
        }
        if (t2 <= floord(PB_L-1,32)) {
          for (t3=max(2,32*t1);t3<=min(min(PB_L-1,PB_M-1),32*t1+31);t3++) {
            for (t4=max(1,32*t2);t4<=min(PB_L-1,32*t2+31);t4++) {
              A[t3][t4] = A[t3][t4-1] + 5;;
              A[(t3-1)+1][t4-1] = C[t4+1][(t3-1)+1] * D[t4+1][(t3-1)-1] * 2;;
            }
        ...
        if (t2 <= floord(PB_L-1,32)) {
          for (t3=max(max(2,PB_M),32*t1);t3<=min(PB_L-1,32*t1+31);t3++) {
