...
void kernel_2122112111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
    ...
    for (t2=0;t2<=floord(PB_M-2,32);t2++) {
      for (t3=32*t1;t3<=min(PB_M-1,32*t1+31);t3++) {
        for (t4=32*t2;t4<=min(PB_M-2,32*t2+31);t4++) {
          A[t3+1][t4+1] = B[t4][t3] * A[t3+1][t4] + 5;;
        }
      }
    }
  }
...
kernel_2122112111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
