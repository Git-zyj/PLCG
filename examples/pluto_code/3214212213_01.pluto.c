...
void kernel_3214212213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_M >= 4) {
  for (t2=1;t2<=PB_M-3;t2++) {
    A[t2][t2+1] = A[t2-1][t2] * A[t2+1][t2+2] - 2;;
  }
  lbp=0;
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t3=t2;t3<=floord(PB_M-2,32);t3++) {
      for (t4=max(1,32*t2);t4<=min(min(PB_M-3,32*t2+31),32*t3+30);t4++) {
        for (t5=max(32*t3,t4+1);t5<=min(PB_M-2,32*t3+31);t5++) {
          A[t4-1][t4] = C[t5+1][t4-1] + 5;;
        }
      }
    }
  }
...
kernel_3214212213_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
