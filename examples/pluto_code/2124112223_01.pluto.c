...
void kernel_2124112223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(3*PB_M-3,32);t1++) {
    lbp=ceild(2*t1,3);
    ubp=min(min(floord(3*t1+2,4),floord(PB_M-1,16)),floord(32*t1+PB_M+30,64));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(ceild(32*t2+1,3),32*t1-32*t2),32*t2-PB_M+1);t3<=min(min(PB_M-1,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3);t4<=min(min(32*t2+31,3*t3-1),t3+PB_M-1);t4++) {
          A[(-2*t3+t4)+t3-1] = B[(-2*t3+t4)][t3] * A[(-2*t3+t4)+t3] * A[(-2*t3+t4)+t3] + 1;;
        }
      }
    }
  }
...
kernel_2124112223_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
