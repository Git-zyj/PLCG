...
void kernel_3112111213_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if ((PB_M >= 4) && (PB_Q >= 1)) {
  for (t1=1;t1<=PB_M-3;t1++) {
    for (t2=0;t2<=floord(3*PB_M+PB_Q-7,32);t2++) {
      lbp=max(ceild(2*t2,3),ceild(32*t2-PB_M+2,32));
      ubp=min(min(floord(2*PB_M+PB_Q-5,32),floord(64*t2+PB_Q+61,96)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(ceild(32*t3-PB_Q+1,2),32*t2-32*t3);t4<=min(min(PB_M-2,16*t3+15),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,2*t4);t5<=min(32*t3+31,2*t4+PB_Q-1);t5++) {
            A[t1+1][t4][(-2*t4+t5)+1] = B[t4][t1-1][(-2*t4+t5)] - A[t1+2][(-2*t4+t5)][t4+2] - A[t4+1][(-2*t4+t5)][t1] + 1;;
          }
        }
      }
    }
...
kernel_3112111213_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
