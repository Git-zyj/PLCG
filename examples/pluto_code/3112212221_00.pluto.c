...
void kernel_3112212221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if (PB_M >= 4) {
  for (t2=0;t2<=PB_M-1;t2++) {
    for (t4=-PB_M+2;t4<=0;t4++) {
      for (t6=1;t6<=min(PB_M-3,t4+PB_M-1);t6++) {
        A[t6+1][t6] = B[t6+-t4][t6+2][0] + A[t6+2][t6+1] + A[t6][t6-1] * 5;;
      }
    }
  }
}
...
kernel_3112212221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
