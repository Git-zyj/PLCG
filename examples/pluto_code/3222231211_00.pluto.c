...
void kernel_3222231211_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5, t6;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_M >= max(0,ceild(-3*PB_N+6,2))) && (PB_N >= 0)) {
  for (t1=0;t1<=min(floord(PB_M+PB_N-1,32),floord(2*PB_M+3*PB_N-4,64));t1++) {
    for (t2=0;t2<=min(min(floord(PB_M,32),floord(2*PB_M+3*PB_N-6,64)),t1);t2++) {
      if ((PB_L <= -1) && (PB_N >= 2) && (t1 <= floord(PB_N-1,32)) && (t2 == 0)) {
        for (t4=max(1,32*t1);t4<=min(PB_N-1,32*t1+31);t4++) {
          B[t4] = D[t4] - E[t4] - E[0] + D[t4] - B[t4+1] + B[t4-1] * B[t4-1] + B[t4-1] + 5;;
        }
      }
      for (t3=0;t3<=min(floord(PB_L,32),floord(2*PB_L+PB_N-2,64));t3++) {
        if ((PB_L >= 1) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(1,32*t1);t4<=min(min(PB_M-2,PB_N-1),32*t1+31);t4++) {
            A[0][t4][0] = C[t4+1][0] * 2;;
            B[t4] = D[t4] - E[t4] - E[0] + D[t4] - B[t4+1] + B[t4-1] * B[t4-1] + B[t4-1] + 5;;
            lbv=1;
            ubv=min(31,PB_L-1);
...
#pragma vector always
        ...
        if ((PB_L >= 1) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(max(1,PB_N),32*t1);t4<=min(PB_M-2,32*t1+31);t4++) {
...
#pragma vector always
        ...
        if ((PB_L >= 1) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(max(1,32*t1),PB_M-1);t4<=min(PB_N-1,32*t1+31);t4++) {
            B[t4] = D[t4] - E[t4] - E[0] + D[t4] - B[t4+1] + B[t4-1] * B[t4-1] + B[t4-1] + 5;;
          }
        }
      }
...
kernel_3222231211_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
