...
void kernel_2222111111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
/* Start of CLooG code */
if (PB_M >= 2) {
  B[0][0] = D[0] * B[0 +1][0 +1] * 3;;
  for (t1=1;t1<=PB_M-2;t1++) {
    A[t1][t1] = C[t1][1] + A[t1+1][t1+1] * 1;;
    B[t1][t1] = D[0] * B[t1+1][t1+1] * 3;;
    for (t2=1;t2<=t1-1;t2++) {
      A[t1][t1] = C[t1][1] + A[t1+1][t1+1] * 1;;
    }
  }
...
kernel_2222111111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
