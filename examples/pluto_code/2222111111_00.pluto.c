...
void kernel_2222111111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  if (PB_L >= 0) {
    for (t1=0;t1<=floord(PB_L+PB_M-2,32);t1++) {
      for (t2=max(0,ceild(32*t1-PB_M+2,32));t2<=min(min(floord(32*t1+PB_M+29,32),floord(-32*t1+2*PB_L+PB_M-2,32)),floord(32*t1+31*PB_L+32*PB_M-64,1024));t2++) {
        if ((t1 == 0) && (t2 == 0)) {
          for (t3=0;t3<=min(min(31,PB_L-1),PB_M-2);t3++) {
            if (t3 == 0) {
              A[0][1] = A[0 +1][1] * 1;;
              B[0 +1] = C[0] + C[0] + B[0] * 6;;
            }
            ...
            for (t4=max(1,t3);t4<=min(31,t3+PB_M-2);t4++) {
              A[(-t3+t4)][1] = A[(-t3+t4)+1][1] * 1;;
            }
          }
        }
        if (t2 >= 1) {
          for (t3=max(32*t1,32*t2-PB_M+2);t3<=min(min(PB_L-1,32*t1+31),32*t2+31);t3++) {
        ...
        if ((t1 == 0) && (t2 == 0)) {
          for (t3=PB_M-1;t3<=min(31,PB_L-1);t3++) {
        ...
        if ((t1 == 0) && (t2 == 0)) {
          for (t3=PB_L;t3<=min(31,PB_M-2);t3++) {
            B[t3+1] = C[t3] + C[t3] + B[t3] * 6;;
          }
        }
      }
...
kernel_2222111111_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
