...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=floord(PB_P-1,32);t3++) {
        if ((PB_P >= 2) && (t3 == 0)) {
          for (t4=32*t1-32*t2;t4<=min(32*t1-32*t2+31,-32*t2+PB_N-3);t4++) {
            for (t5=32*t2;t5<=min(min(PB_P-1,32*t2+31),-t4+PB_N-3);t5++) {
              B[t5][t5+t4+1] = D[t5+t4][t4+1] * B[t5][t5+t4+2] * B[t5][t5+t4] + B[t5+1][t5+t4+2] + 4;;
              for (t6=1;t6<=min(31,PB_P-1);t6++) {
                A[t6][t4][t5] = C[t5+1][t4+1][t6] - A[t6-1][t4][t5] + 2;;
              }
            }
          }
        }
        ...
        if ((PB_P == 1) && (t2 == 0) && (t3 == 0)) {
          for (t4=32*t1;t4<=min(PB_N-3,32*t1+31);t4++) {
            B[0][0 +t4+1] = D[0 +t4][t4+1] * B[0][0 +t4+2] * B[0][0 +t4] + B[0 +1][0 +t4+2] + 4;;
          }
        }
      }
    }
