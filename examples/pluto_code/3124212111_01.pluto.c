...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(-32*t1+PB_M,32);t2++) {
      for (t3=32*t1;t3<=min(min(min(floord(PB_M-1,2),32*t1+31),-32*t2+PB_M),32*t2+30);t3++) {
        for (t4=max(max(2,32*t2),t3+1);t4<=min(32*t2+31,-t3+PB_M);t4++) {
          A[t3+t4-2][t3] = B[t3] + A[t3+t4-1][t3] + 4;;
        }
      }
    }
  }
