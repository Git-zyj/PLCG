...
void kernel_2224132213_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 1) {
  for (t1=0;t1<=PB_L-1;t1++) {
    B[t1+2] = C[t1] - C[t1] + B[t1+1] - B[t1] - 3;;
    B[t1+1] = C[t1] - C[t1] + C[1] - C[t1] * 1;;
  }
}
/* End of CLooG code */
...
kernel_2224132213_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
