...
void kernel_2222132221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
  ...
  for (t1=0;t1<=floord(PB_L-1,32);t1++) {
    if (PB_M <= -1) {
      for (t3=max(2,32*t1);t3<=min(PB_L-1,32*t1+31);t3++) {
        B[t3] = D[t3] - D[t3] + B[t3-2] * 3;;
      }
    }
    if (PB_M >= 0) {
      for (t2=0;t2<=min(min(min(floord(10*t1+2*PB_M,5),floord(2*PB_L+PB_M-5,32)),floord(64*t1+PB_M+59,32)),floord(5*PB_L+3*PB_M-15,80));t2++) {
        if ((PB_M >= 5) && (t1 == 0) && (t2 == 0)) {
          for (t3=2;t3<=min(14,PB_L-1);t3++) {
            B[t3] = D[t3] - D[t3] + B[t3-2] * 3;;
            for (t4=2*t3+2;t4<=min(31,2*t3+PB_M-3);t4++) {
              A[t3][(-2*t3+t4)] = C[(-2*t3+t4)+2][t3] * C[(-2*t3+t4)+1][t3+2] * C[t3-1][0] * C[(-2*t3+t4)][0] + A[t3][(-2*t3+t4)+2] * A[t3][(-2*t3+t4)-2] - A[(-2*t3+t4)-1][t3+2] + 6;;
            }
          }
        }
        if ((PB_M >= 5) && (t2 >= 1)) {
          for (t3=max(max(2,ceild(32*t2-PB_M+3,2)),32*t1);t3<=min(min(PB_L-1,32*t1+31),16*t2+14);t3++) {
            for (t4=max(32*t2,2*t3+2);t4<=min(32*t2+31,2*t3+PB_M-3);t4++) {
              A[t3][(-2*t3+t4)] = C[(-2*t3+t4)+2][t3] * C[(-2*t3+t4)+1][t3+2] * C[t3-1][0] * C[(-2*t3+t4)][0] + A[t3][(-2*t3+t4)+2] * A[t3][(-2*t3+t4)-2] - A[(-2*t3+t4)-1][t3+2] + 6;;
            }
          }
        }
...
kernel_2222132221_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
