...
void kernel_2222132221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_P >= 3) {
  for (t1=2;t1<=PB_P-1;t1++) {
    A[t1-1] = C[t1] - C[t1] - C[t1+2] + B[t1] + B[t1-2] + 5;;
    B[t1] = D[t1][t1] - C[t1] + A[t1-1] - B[t1-2] - 1;;
  }
}
/* End of CLooG code */
...
kernel_2222132221_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
