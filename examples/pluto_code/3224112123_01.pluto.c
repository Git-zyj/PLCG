...
void kernel_3224112123_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=floord(PB_L-1,32);t3++) {
        for (t4=0;t4<=min(floord(PB_P-3,32),t2);t4++) {
          for (t5=max(1,32*t2);t5<=min(PB_L-1,32*t2+31);t5++) {
            for (t6=max(1,32*t3);t6<=min(PB_L-1,32*t3+31);t6++) {
              lbv=32*t4;
              ubv=min(min(t5,PB_P-3),32*t4+31);
#pragma ivdep
#pragma vector always
              for (t7=lbv;t7<=ubv;t7++) {
                B[t6][t5][t7+2] = C[t7][t6][t6] - A[t6][t5-1] - B[t6-1][t5][t7] * 5;;
              }
            }
          }
        }
...
kernel_3224112123_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
