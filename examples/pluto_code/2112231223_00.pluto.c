...
void kernel_2112231223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_L >= 4) {
  for (t1=0;t1<=floord(5*PB_L-5,32);t1++) {
    ...
    ubp=min(min(floord(PB_L-1,8),floord(96*t1+PB_L+92,128)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,ceild(32*t2-PB_L+1,3)),32*t1-32*t2);t3<=min(min(floord(32*t2+28,3),PB_L-1),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,3*t3+3);t4<=min(32*t2+31,3*t3+PB_L-1);t4++) {
          A[t3-1][(-3*t3+t4)] = B[(-3*t3+t4)][t3] * B[t3-1][(-3*t3+t4)-1] * C[t3+1][(-3*t3+t4)-1] + A[(-3*t3+t4)-3][t3] + A[(-3*t3+t4)-2][t3] + 2;;
        }
      }
    }
  }
...
kernel_2112231223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
