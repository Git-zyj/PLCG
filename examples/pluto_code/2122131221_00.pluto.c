...
void kernel_2122131221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 3) {
  for (t1=1;t1<=PB_M-2;t1++) {
    for (t2=0;t2<=min(PB_M-3,-t1+PB_M-1);t2++) {
      A[t2] = B[t2+t1][0] - B[t2+t1-1][t2] + C[t2] + A[t2+2] + A[t1+1] - 1;;
    }
  }
}
/* End of CLooG code */
...
kernel_2122131221_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
