...
void kernel_3114212111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_L >= 2) && (PB_P >= 5)) {
  lbp=0;
  ubp=min(floord(PB_L+1,32),floord(PB_P-3,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(floord(PB_L-1,32),floord(PB_P-4,32)),floord(-32*t1+PB_P-2,32));t2++) {
      for (t3=max(1,32*t2);t3<=min(min(min(PB_L-1,PB_P-4),32*t2+31),-32*t1+PB_P-2);t3++) {
        lbv=max(2,32*t1);
        ubv=min(min(PB_L+1,32*t1+31),-t3+PB_P-2);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3-1][t4] = B[t4-2][t4+t3+1] * A[t3][t4] * 4;;
        }
      }
    }
  }
...
kernel_3114212111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
