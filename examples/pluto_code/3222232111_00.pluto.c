...
void kernel_3222232111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
if (PB_N >= 4) {
  if (PB_M >= 0) {
    for (t1=max(ceild(-PB_M-11,16),ceild(-PB_M-43,32));t1<=min(floord(PB_N-3,16),floord(6*PB_N+PB_M-21,96));t1++) {
      lbp=max(max(ceild(-PB_M-11,16),ceild(32*t1-PB_N+3,16)),ceild(352*t1-11*PB_N-15*PB_M+33,416));
      ubp=min(t1,2*t1+2);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
      for (t2=lbp;t2<=ubp;t2++) {
        if (t2 <= -1) {
          for (t3=max(32*t1-32*t2,-16*t2-14);t3<=min(min(PB_M-3,32*t1-32*t2+31),-16*t2+PB_N-3);t3++) {
            for (t4=max(1,16*t2+t3);t4<=min(PB_N-3,16*t2+t3+15);t4++) {
              A[t3][t4] = C[t3+2][t4] + C[t3][t4] * D[t4+2][t3] * B[t3+1] + 6;;
            }
          }
        }
        if (t2 >= 1) {
          for (t3=32*t1-32*t2;t3<=min(min(PB_M-3,32*t1-32*t2+31),-16*t2+PB_N-3);t3++) {
            for (t4=16*t2+t3;t4<=min(PB_N-3,16*t2+t3+15);t4++) {
              A[t3][t4] = C[t3+2][t4] + C[t3][t4] * D[t4+2][t3] * B[t3+1] + 6;;
            }
          }
        }
        if ((PB_M >= 3) && (t1 == 0) && (t2 == 0)) {
          for (t4=1;t4<=min(15,PB_N-3);t4++) {
            A[0][t4] = C[0 +2][t4] + C[0][t4] * D[t4+2][0] * B[0 +1] + 6;;
          }
        }
        if (t2 == 0) {
          for (t3=max(1,32*t1);t3<=min(min(PB_M-3,PB_N-3),32*t1+31);t3++) {
            A[t3][t3] = C[t3+2][t3] + C[t3][t3] * D[t3+2][t3] * B[t3+1] + 6;;
            B[t3+1] = E[t3-1] - F[t3] * F[t3] - B[t3] - 1;;
            for (t4=t3+1;t4<=min(PB_N-3,t3+15);t4++) {
              A[t3][t4] = C[t3+2][t4] + C[t3][t4] * D[t4+2][t3] * B[t3+1] + 6;;
            }
          }
        }
        if (t2 == 0) {
          for (t3=max(max(1,32*t1),PB_M-2);t3<=min(PB_N-3,32*t1+31);t3++) {
            B[t3+1] = E[t3-1] - F[t3] * F[t3] - B[t3] - 1;;
          }
        }
      }
...
kernel_3222232111_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
