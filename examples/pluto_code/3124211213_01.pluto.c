...
void kernel_3124211213_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if ((PB_L >= 3) && (PB_M >= 4)) {
  lbp=0;
  ubp=min(floord(PB_L-2,32),floord(PB_M-2,64));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=min(floord(PB_M-3,32),floord(-32*t1+PB_M-2,32));t2++) {
      for (t3=0;t3<=min(floord(PB_L-2,32),floord(-32*t1+PB_L-1,32));t3++) {
        for (t4=max(1,32*t1);t4<=min(min(min(min(floord(PB_M-2,2),PB_L-2),32*t1+31),-32*t2+PB_M-2),-32*t3+PB_L-1);t4++) {
          for (t5=max(1,32*t3);t5<=min(32*t3+31,-t4+PB_L-1);t5++) {
            for (t6=max(32*t2,t4);t6<=min(32*t2+31,-t4+PB_M-2);t6++) {
              A[t6+t4+1][t5+1][t5+t4] = B[t5-1][t4-1][t6] - A[t6+t4][t5+1][t5+t4] + A[t6+t4+1][t5][t5+t4-1] + 1;;
            }
          }
        }
      }
...
kernel_3124211213_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
