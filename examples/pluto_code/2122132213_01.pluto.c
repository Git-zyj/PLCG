...
void kernel_2122132213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t1-32*t2),32*t2-PB_M+2);t3<=min(min(PB_M-2,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,t3+PB_M-2);t4++) {
          A[(-t3+t4)][(-t3+t4)] = B[t3-1] + C[(-t3+t4)][t3] * C[(-t3+t4)][t3-2] + A[(-t3+t4)-1][(-t3+t4)-1] - A[(-t3+t4)+1][(-t3+t4)+1] + 3;;
        }
      }
    }
  }
...
kernel_2122132213_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
