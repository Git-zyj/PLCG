...
void kernel_3114131123_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(floord(PB_M-2,32),floord(PB_N-1,32)),floord(-32*t1+PB_M-1,32));t2++) {
      for (t3=max(1,32*t1);t3<=min(min(PB_N-1,32*t1+31),-32*t2+PB_M-1);t3++) {
        for (t4=32*t2;t4<=min(min(PB_N-1,32*t2+31),-t3+PB_M-1);t4++) {
          A[t3-1][t4+1] = B[t4][t3] + B[t3][t4+t3] + B[t4][t3] + A[t3-1][t4+2] * 5;;
        }
      }
    }
  }
...
kernel_3114131123_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
