...
void kernel_3114211223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
  ...
  for (t1=0;t1<=PB_L-1;t1++) {
    for (t2=1;t2<=PB_M-3;t2++) {
      for (t3=0;t3<=PB_L-1;t3++) {
        A[t1][0] = B[t3][t2] + A[t2-1][0] * A[t2+2][0] - 2;;
      }
    }
  }
}
...
kernel_3114211223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
