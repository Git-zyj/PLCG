...
void kernel_2214132223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=floord(PB_M-3,32);t3++) {
        for (t4=max(2,32*t3);t4<=min(PB_M-3,32*t3+31);t4++) {
          lbv=max(2,32*t2);
          ubv=min(PB_L-2,32*t2+31);
#pragma ivdep
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            A[t5][t4] = C[t4+2][t5+1] - C[t4-2][t5] + C[t5][t4] - C[t4-2][t5-2] - C[t4][t5] - B[t4] * 3;;
          }
        }
      }
    }
...
kernel_2214132223_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
