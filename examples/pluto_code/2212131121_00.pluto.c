...
void kernel_2212131121_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 1) {
  ...
  for (t1=1;t1<=PB_M-1;t1++) {
    B[t1] = C[t1] - C[t1] - 3;;
    B[(t1-1)] = C[(t1-1)] - C[(t1-1)] * C[(t1-1)+1] - B[(t1-1)+1] + 5;;
  }
}
/* End of CLooG code */
...
kernel_2212131121_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
