...
void kernel_2112111121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if (PB_M >= 2) {
  lbp=0;
  ubp=floord(PB_M-2,64);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(-32*t1+PB_M-2,32);t2++) {
      for (t3=32*t1;t3<=min(min(floord(PB_M-2,2),32*t1+31),-32*t2+PB_M-2);t3++) {
        for (t4=max(32*t2,t3);t4<=min(32*t2+31,-t3+PB_M-2);t4++) {
          A[t4+t3][t4] = B[t3+1][t4][t4] * A[t4+t3+1][t4+1] + 4;;
        }
      }
    }
  }
...
kernel_2112111121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
