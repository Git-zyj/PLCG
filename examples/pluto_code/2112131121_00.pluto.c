...
void kernel_2112131121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_N-2,32);t2++) {
      for (t3=32*t1;t3<=min(PB_N-1,32*t1+31);t3++) {
        for (t4=max(1,32*t2);t4<=min(PB_N-2,32*t2+31);t4++) {
          A[t3][t4+1] = B[t3][t4+1] - B[t4+1][t3] + B[t4][t3] * A[t3][t4-1] - 3;;
        }
      }
    }
  }
...
kernel_2112131121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
