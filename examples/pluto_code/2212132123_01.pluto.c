...
void kernel_2212132123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
/* Start of CLooG code */
if (PB_M >= 3) {
  for (t1=4;t1<=PB_M+1;t1++) {
    A[t1-2] = C[t1+1] - C[t1] + A[t1-4] * A[t1-3] - 4;;
    B[t1] = D[t1-2][t1-1] - C[t1] + C[t1] - C[t1] + 3;;
  }
}
/* End of CLooG code */
...
kernel_2212132123_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
