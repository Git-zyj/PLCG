...
void kernel_2222111121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(2,32*t2);t3<=(min(PB_M-1,32*t2+31))-7;t3+=8) {
        for (t4=max(1,32*t1-32*t2);t4<=min(PB_M-2,32*t1-32*t2+31);t4++) {
          A[t4-1] = C[0] - C[t4+1] * 1;;
          B[t3][t4] = B[t3-2][t4] * B[t3][t4-1] - 1;;
          A[t4-1] = C[0] - C[t4+1] * 1;;
          B[(t3+1)][t4] = B[(t3+1)-2][t4] * B[(t3+1)][t4-1] - 1;;
          A[t4-1] = C[0] - C[t4+1] * 1;;
          B[(t3+2)][t4] = B[(t3+2)-2][t4] * B[(t3+2)][t4-1] - 1;;
          A[t4-1] = C[0] - C[t4+1] * 1;;
          B[(t3+3)][t4] = B[(t3+3)-2][t4] * B[(t3+3)][t4-1] - 1;;
          A[t4-1] = C[0] - C[t4+1] * 1;;
          B[(t3+4)][t4] = B[(t3+4)-2][t4] * B[(t3+4)][t4-1] - 1;;
          A[t4-1] = C[0] - C[t4+1] * 1;;
          B[(t3+5)][t4] = B[(t3+5)-2][t4] * B[(t3+5)][t4-1] - 1;;
          A[t4-1] = C[0] - C[t4+1] * 1;;
          B[(t3+6)][t4] = B[(t3+6)-2][t4] * B[(t3+6)][t4-1] - 1;;
          A[t4-1] = C[0] - C[t4+1] * 1;;
          B[(t3+7)][t4] = B[(t3+7)-2][t4] * B[(t3+7)][t4-1] - 1;;
        }
      }
...
kernel_2222111121_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
