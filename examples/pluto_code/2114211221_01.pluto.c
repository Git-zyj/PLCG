...
void kernel_2114211221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M-2,8);t1++) {
    ...
    ubp=min(min(floord(3*PB_M-4,32),floord(64*t1+PB_M+62,96)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(32*t2-PB_M,2),32*t1-32*t2);t3<=min(min(PB_M-2,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3+1);t4<=min(32*t2+31,2*t3+PB_M);t4++) {
          A[(-2*t3+t4)] = B[t3+1][(-2*t3+t4)-1] * A[(-2*t3+t4)-1] * A[(-2*t3+t4)+2] + 1;;
        }
      }
    }
  }
...
kernel_2114211221_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
