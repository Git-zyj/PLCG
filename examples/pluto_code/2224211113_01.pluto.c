...
void kernel_2224211113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_L >= 1) {
  if (PB_M >= 0) {
    for (t1=0;t1<=min(floord(PB_M+PB_L-1,32),floord(PB_M+2*PB_L-3,32));t1++) {
      for (t2=0;t2<=min(min(floord(PB_L-1,32),floord(-32*t1+PB_M+PB_L-1,32)),floord(-32*t1+PB_M+2*PB_L-3,64));t2++) {
        if (t2 >= 1) {
          lbp=32*t1;
          ubp=min(PB_M-1,32*t1+31);
...
#pragma vector always
        ...
        if (t2 == 0) {
          for (t3=max(1,32*t1);t3<=min(min(PB_L-1,PB_M-1),32*t1+31);t3++) {
            A[t3][0] = 4;;
            B[t3+1] = C[t3-1] + D[t3][0] - B[t3] - 3;;
            lbv=1;
            ubv=min(31,PB_L-1);
#pragma ivdep
#pragma vector always
...
#pragma vector always
        ...
        if (t2 == 0) {
          for (t3=max(max(1,PB_M),32*t1);t3<=min(PB_L-1,32*t1+31);t3++) {
            B[t3+1] = C[t3-1] + D[t3][0] - B[t3] - 3;;
          }
        }
      }
...
kernel_2224211113_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
