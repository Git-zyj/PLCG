...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=t1;t2++) {
      for (t3=t2;t3<=min(floord(PB_M+PB_L-3,32),floord(32*t2+PB_L+29,32));t3++) {
        for (t4=max(max(1,32*t1),32*t3-PB_L+2);t4<=min(PB_M-1,32*t1+31);t4++) {
          for (t5=max(max(1,32*t2),32*t3-PB_L+2);t5<=min(min(t4,32*t2+31),32*t3+30);t5++) {
            for (t6=max(32*t3,t5+1);t6<=min(32*t3+31,t5+PB_L-2);t6++) {
              A[(t4-t5)+t5][(-t5+t6)][(-t5+t6)] = B[(t4-t5)][(-t5+t6)-1][t5-1] + A[(t4-t5)+t5][(-t5+t6)-1][(-t5+t6)-1] + A[(t4-t5)+t5][(-t5+t6)+1][(-t5+t6)+1] + 6;;
            }
          }
        }
      }
