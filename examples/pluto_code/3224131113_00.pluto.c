...
void kernel_3224131113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_L >= 2) {
  if (PB_M >= 0) {
    for (t1=0;t1<=floord(2*PB_L+PB_M-2,32);t1++) {
      ...
      ubp=min(floord(PB_L+PB_M,32),t1);
...
#pragma vector always
            for (t4=lbv;t4<=ubv;t4++) {
              A[t3][t4] = C[t3][t4+1] * D[t4] + 5;;
            }
          }
        }
        for (t3=max(2,32*t2);t3<=min(min(PB_L,PB_M-1),32*t2+31);t3++) {
          lbv=32*t1-32*t2;
...
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3][t4] = C[t3][t4+1] * D[t4] + 5;;
            B[t4+1][t3-1] = C[t3+1][t4+1] + C[t3][t4] - C[t3][t4] * C[t3][t4] - A[t3][t4] - B[t4][t3-2] + 4;;
          }
        }
...
#pragma vector always
        ...
        for (t3=max(max(2,PB_M),32*t2);t3<=min(PB_L,32*t2+31);t3++) {
          lbv=32*t1-32*t2;
          ubv=min(PB_L-2,32*t1-32*t2+31);
#pragma ivdep
#pragma vector always
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
      for (t2=lbp;t2<=ubp;t2++) {
        for (t3=max(2,32*t2);t3<=min(PB_L,32*t2+31);t3++) {
          lbv=32*t1-32*t2;
          ubv=min(PB_L-2,32*t1-32*t2+31);
#pragma ivdep
#pragma vector always
...
kernel_3224131113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
