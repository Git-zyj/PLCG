...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(3,32*t1-32*t2),16*t2+1);t3<=min(min(PB_L-2,32*t2+29),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+2);t4<=min(32*t2+31,2*t3-1);t4++) {
          A[(-t3+t4)][(-t3+t4)] = B[t3][t3] - B[(-t3+t4)-2][t3] - B[(-t3+t4)][t3+1] + A[(-t3+t4)-1][(-t3+t4)-1] + A[(-t3+t4)+1][(-t3+t4)+1] * 2;;
        }
      }
    }
  }
