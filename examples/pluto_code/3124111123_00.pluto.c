...
void kernel_3124111123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_M >= 1) && (PB_P >= 2)) {
  lbp=0;
  ubp=min(floord(PB_M-1,32),floord(PB_P-1,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_P-1,32);t2++) {
      for (t3=32*t1;t3<=min(min(PB_M-1,PB_P-1),32*t1+31);t3++) {
        for (t4=max(max(1,32*t2),t3);t4<=min(PB_P-1,32*t2+31);t4++) {
          A[t3][t4] = B[t3][t4+1] * A[t3][t4-1] * 4;;
        }
      }
    }
  }
...
kernel_3124111123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
