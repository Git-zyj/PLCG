...
void kernel_3114131213_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if ((PB_L >= 2) && (PB_Q >= 3)) {
  lbp=0;
  ubp=min(floord(PB_L-1,32),floord(PB_Q-1,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(min(floord(PB_L-2,32),floord(-32*t1+PB_Q-1,32)),floord(PB_Q-2,64)),t1);t2++) {
      for (t3=0;t3<=floord(PB_Q-2,32);t3++) {
        for (t4=32*t2;t4<=min(min(min(min(floord(PB_Q-2,2),PB_L-2),32*t1+30),32*t2+31),-32*t1+PB_Q-1);t4++) {
          for (t5=max(1,32*t3);t5<=min(PB_Q-2,32*t3+31);t5++) {
            lbv=max(32*t1,t4+1);
            ubv=min(min(PB_L-1,32*t1+31),-t4+PB_Q-1);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t6][t4][t5] = B[t4][t5-1][1] * B[t4][t5][t4+t6] * B[t4][t5][t6] - A[t6][t4+1][t5] * A[t6][t4][t5+1] - 2;;
            }
          }
        }
      }
...
kernel_3114131213_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
