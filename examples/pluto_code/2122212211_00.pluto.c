...
void kernel_2122212211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
/* Start of CLooG code */
if ((PB_N >= 2) && (PB_P >= 2)) {
  for (t1=-1;t1<=min(floord(PB_N-1,32),floord(PB_N+PB_P-34,32));t1++) {
    lbp=t1+1;
    ubp=min(floord(PB_N+PB_P-2,32),floord(32*t1+PB_P+30,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,32*t1),32*t2-PB_P+1);t3<=min(PB_N-1,32*t1+62);t3++) {
        lbv=max(32*t2,-32*t1+32*t2+t3-31);
        ubv=min(min(32*t2+31,-32*t1+32*t2+t3),t3+PB_P-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-t3+t4)] = B[t3] + A[t3-1][(-t3+t4)] - A[t3+1][(-t3+t4)-1] + 2;;
        }
      }
    }
  }
...
kernel_2122212211_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
