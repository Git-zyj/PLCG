...
void kernel_2112111213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if ((PB_L >= 3) && (PB_M >= 3) && (PB_Q >= 1)) {
  for (t1=1;t1<=min(PB_L-2,PB_M-2);t1++) {
    for (t2=0;t2<=min(PB_Q-1,-t1+PB_L-2);t2++) {
      A[t1] = B[t2+t1+1][t2][0] - A[t1-1] * A[t1+1] - 4;;
    }
  }
}
/* End of CLooG code */
...
kernel_2112111213_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
