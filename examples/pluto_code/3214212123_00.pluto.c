...
#pragma vector always
              for (t6=lbv;t6<=ubv;t6++) {
                A[t4][t5+2][t6] = C[t4][t5][t6+1] - 5;;
              }
            }
          }
        }
        if ((t2 <= floord(PB_L-3,32)) && (t2 >= -t3+1) && (t3 <= floord(PB_P-1,32))) {
          lbp=max(2,32*t1);
