...
void kernel_3214212113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
if (PB_P >= 2) {
  for (t1=0;t1<=floord(PB_P-1,16);t1++) {
    lbp=max(max(0,ceild(32*t1-PB_P+1,32)),ceild(32*t1-PB_P+2,64));
    ubp=min(floord(PB_P-1,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=min(floord(PB_P-1,32),floord(-32*t1+64*t2+PB_P-1,32));t3++) {
        if (t3 == 0) {
          for (t4=max(32*t1-32*t2,32*t2+32);t4<=min(PB_P-2,32*t1-32*t2+31);t4++) {
            for (t5=32*t2;t5<=32*t2+31;t5++) {
              A[t5+2][t4] = C[t5][t5] * A[t5+1][t4] + A[t5+2][t4+1] - 4;;
            }
          }
        }
        for (t4=32*t1-32*t2;t4<=min(min(min(min(PB_M-1,PB_P-1),32*t2+31),32*t2+31*t3),32*t1-32*t2+31);t4++) {
          for (t5=max(32*t2,t4);t5<=min(PB_P-1,32*t2+31);t5++) {
            lbv=max(1,32*t3);
            ubv=min(PB_P-1,32*t3+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              B[t4][t5][t6] = D[t6-1][t4][t5] - 5;;
            }
          }
        }
        if ((t1 == 2*t2) && (t3 == 0)) {
          for (t4=16*t1+1;t4<=min(min(PB_M-1,PB_P-2),16*t1+31);t4++) {
            for (t5=16*t1;t5<=t4-1;t5++) {
              if (t1%2 == 0) {
                A[t5+2][t4] = C[t5][t5] * A[t5+1][t4] + A[t5+2][t4+1] - 4;;
              }
            }
            for (t5=t4;t5<=min(PB_P-1,16*t1+31);t5++) {
              lbv=1;
              ubv=min(31,PB_P-1);
#pragma ivdep
#pragma vector always
        ...
        if ((t1 == 2*t2) && (t3 == 0)) {
          for (t4=max(PB_M,16*t1+1);t4<=min(PB_P-2,16*t1+31);t4++) {
            for (t5=16*t1;t5<=t4-1;t5++) {
              if (t1%2 == 0) {
                A[t5+2][t4] = C[t5][t5] * A[t5+1][t4] + A[t5+2][t4+1] - 4;;
              }
        ...
        if ((PB_P <= PB_M) && (t1 == 2*t2) && (t1 <= floord(PB_P-2,16)) && (t1 >= ceild(PB_P-32,16)) && (t3 == 0)) {
          lbv=1;
          ubv=min(31,PB_P-1);
#pragma ivdep
#pragma vector always
          for (t6=lbv;t6<=ubv;t6++) {
            if (t1%2 == 0) {
              B[(PB_P-1)][(PB_P-1)][t6] = D[t6-1][(PB_P-1)][(PB_P-1)] - 5;;
            }
          }
        }
      }
...
kernel_3214212113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
