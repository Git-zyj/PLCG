...
void kernel_2222131121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    if (PB_M >= 0) {
      for (t2=0;t2<=min(floord(PB_L+PB_M-2,32),floord(5*PB_L+PB_M-14,32));t2++) {
        if ((PB_M >= 4) && (t2 == 0)) {
          lbv=max(1,32*t1);
...
#pragma vector always
        ...
        for (t3=max(1,32*t2);t3<=min(min(PB_L-2,PB_M-4),32*t2+31);t3++) {
          lbv=max(1,32*t1);
          ubv=min(PB_L-1,32*t1+31);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3+1][t4] = A[t3+3][t4] + 6;;
            B[t3][t4-1] = C[t3][t4] * C[t4][t3] + D[t3] - C[t4-1][t3-1] - C[t4][t3] + B[t3+1][t4-1] - 6;;
          }
        }
        for (t3=max(32*t2,PB_L-1);t3<=min(PB_M-4,32*t2+31);t3++) {
          lbv=max(1,32*t1);
          ubv=min(PB_L-1,32*t1+31);
#pragma ivdep
#pragma vector always
        ...
        for (t3=max(max(1,32*t2),PB_M-3);t3<=min(PB_L-2,32*t2+31);t3++) {
          lbv=max(1,32*t1);
          ubv=min(PB_L-1,32*t1+31);
#pragma ivdep
#pragma vector always
...
kernel_2222131121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
