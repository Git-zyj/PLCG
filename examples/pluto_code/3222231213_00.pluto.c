...
void kernel_3222231213_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5, t6;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= max(0,ceild(-4*PB_M+12,3))) && (PB_M >= 0)) {
  for (t1=0;t1<=min(floord(PB_L+PB_M-2,32),floord(3*PB_L+4*PB_M-9,96));t1++) {
    if ((PB_M >= 3) && (t1 <= floord(PB_M-2,32))) {
      for (t6=max(1,32*t1);t6<=min(PB_M-2,32*t1+31);t6++) {
        B[t6] = E[t6] * E[0] * B[t6-1] * B[t6+1] + 4;;
      }
    }
    if ((PB_L >= 4) && (t1 <= floord(PB_L-3,32))) {
      lbp=0;
      ubp=floord(PB_L-2,32);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
      for (t2=lbp;t2<=ubp;t2++) {
        for (t3=0;t3<=floord(PB_L-1,32);t3++) {
          for (t4=max(1,32*t2);t4<=min(PB_L-2,32*t2+31);t4++) {
            for (t5=32*t3;t5<=min(PB_L-1,32*t3+31);t5++) {
              for (t6=max(1,32*t1);t6<=min(PB_L-3,32*t1+31);t6++) {
                A[t5][t4+1][t6+1] = C[t4+1][t5+1][t6] + C[t4-1][t5][t6] - D[t6][t4][t5] + C[t5][t4][t6-1] + A[t5][t4+1][t6+2] * A[t5][t4+1][t6] - 2;;
              }
            }
          }
        }
...
kernel_3222231213_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
