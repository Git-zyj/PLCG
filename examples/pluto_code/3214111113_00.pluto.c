...
/* Start of CLooG code */
if ((PB_N >= 0) && (PB_P >= max(0,-PB_N+2))) {
  for (t1=0;t1<=floord(PB_P+PB_N-1,32);t1++) {
    if ((PB_N >= 2) && (t1 <= floord(PB_N-1,32))) {
      for (t6=max(1,32*t1);t6<=min(PB_N-1,32*t1+31);t6++) {
        B[t6] = B[t6-1] + 1;;
      }
    }
    if ((PB_L >= 1) && (PB_P >= 2) && (t1 <= floord(PB_P-1,32))) {
      lbp=0;
      ubp=floord(PB_L,32);
        ...
        for (t3=0;t3<=floord(PB_L-1,32);t3++) {
          for (t4=max(1,32*t2);t4<=min(PB_L,32*t2+31);t4++) {
            for (t5=32*t3;t5<=min(PB_L-1,32*t3+31);t5++) {
              for (t6=max(1,32*t1);t6<=min(PB_P-1,32*t1+31);t6++) {
                A[t5][t4-1][t6] = C[t5][t4][t6] + C[t5][t6][t4] + A[t5][t4-1][t6-1] + 1;;
              }
            }
          }
        }
