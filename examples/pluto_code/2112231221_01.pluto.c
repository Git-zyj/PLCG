...
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_M >= 5)) {
  for (t1=0;t1<=floord(3*PB_L+PB_M-9,32);t1++) {
    lbp=max(ceild(2*t1,3),ceild(32*t1-PB_L+2,32));
    ubp=min(min(floord(2*PB_L+PB_M-7,32),floord(64*t1+PB_M+59,96)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(32*t2-PB_M+3,2),32*t1-32*t2);t3<=min(min(PB_L-2,16*t2+14),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3+2);t4<=min(32*t2+31,2*t3+PB_M-3);t4++) {
          A[(-2*t3+t4)][(-2*t3+t4)] = B[(-2*t3+t4)][t3] + C[(-2*t3+t4)+1] * D[(-2*t3+t4)][t3+1] + A[(-2*t3+t4)+2][(-2*t3+t4)+2] + A[(-2*t3+t4)-2][(-2*t3+t4)-2] - 1;;
        }
      }
    }
  }
