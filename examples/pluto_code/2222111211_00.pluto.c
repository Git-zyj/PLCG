...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_M+2);t3<=min(16*t2,32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(32*t2+31,t3+PB_M-2);t4++) {
          B[(-t3+t4)] = B[(-t3+t4)+1] - B[(-t3+t4)-1] + 2;;
        }
      }
      for (t3=max(32*t1-32*t2,16*t2+1);t3<=min(min(PB_M-2,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3);t4<=2*t3-1;t4++) {
          A[(-t3+t4)+1] = C[0] + A[(-t3+t4)+2] + 6;;
        }
        for (t4=2*t3;t4<=min(32*t2+31,t3+PB_M-2);t4++) {
          B[(-t3+t4)] = B[(-t3+t4)+1] - B[(-t3+t4)-1] + 2;;
        }
      }
      ...
      if ((t1 >= ceild(32*t2+PB_M-32,32)) && (t2 >= ceild(PB_M-16,16))) {
        for (t4=max(32*t2,PB_M-1);t4<=2*PB_M-4;t4++) {
          A[(t4-PB_M+1)+1] = C[0] + A[(t4-PB_M+1)+2] + 6;;
        }
      }
    }
  }
