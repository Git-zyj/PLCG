...
if (PB_P >= 3) {
  if (PB_M >= 0) {
    lbp=0;
    ubp=floord(PB_P+PB_M-2,32);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
    for (t1=lbp;t1<=ubp;t1++) {
      for (t2=max(0,ceild(32*t1-PB_M,32));t2<=floord(PB_P-1,32);t2++) {
        for (t3=32*t1;t3<=min(min(min(PB_M-1,PB_P-2),32*t1+31),32*t2+30);t3++) {
          if (t1 == t2) {
          ...
          for (t4=max(max(2,32*t2),t3+1);t4<=min(PB_P-1,32*t2+31);t4++) {
            A[t3][t4-1] = C[t3][t4] * A[t3][t4-2] + A[t3][t4] + 1;;
            A[t3][t4-1] = C[t4][0] + 2;;
          }
        }
        for (t3=max(PB_M,32*t1);t3<=min(min(PB_P-2,32*t1+31),32*t2+30);t3++) {
          for (t4=max(max(2,32*t2),t3+1);t4<=min(PB_P-1,32*t2+31);t4++) {
            A[t3][t4-1] = C[t3][t4] * A[t3][t4-2] + A[t3][t4] + 1;;
          }
        }
        if (t1 == t2) {
          for (t3=PB_P-1;t3<=min(PB_M-1,32*t1+30);t3++) {
            for (t4=max(2,32*t1);t4<=PB_P-1;t4++) {
              A[t3][t4-1] = C[t4][0] + 2;;
            }
          }
        }
