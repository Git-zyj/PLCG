...
void kernel_2124232121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_N-3,32),floord(-32*t1+PB_N-1,32));t2++) {
      for (t3=32*t2;t3<=min(min(PB_N-3,32*t2+31),-32*t1+PB_N-1);t3++) {
        lbv=max(2,32*t1);
        ubv=min(min(PB_M-1,32*t1+31),-t3+PB_N-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+t4][t4-1] = B[t4-2][t3] - C[t3][t4+2] - B[t3][t4] - A[t3+t4-2][t4-1] - 5;;
        }
      }
    }
  }
...
kernel_2124232121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
