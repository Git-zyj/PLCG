...
void kernel_3212131223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
        ...
        for (t2=0;t2<=floord(PB_N-1,32);t2++) {
          for (t3=max(1,32*t2);t3<=min(PB_N-1,32*t2+31);t3++) {
            for (t4=32*t1;t4<=min(PB_L-3,32*t1+31);t4++) {
              A[t3+1][t4+1] = C[t4][t3+1] - C[t3+1][t4] * C[t4][t3] - A[t3+1][t4+2] - A[t3+3][t4+1] - A[t3+1][t4] * 2;;
            }
          }
        }
      }
...
kernel_3212131223_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
