...
/* Start of CLooG code */
if (PB_L >= 3) {
  for (t1=0;t1<=floord(PB_L,16);t1++) {
    lbp=max(0,ceild(32*t1-PB_L+1,32));
    ubp=min(floord(PB_L+1,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      if (t2 <= floord(PB_L-1,32)) {
        for (t3=max(2,32*t1-32*t2);t3<=min(min(PB_L-1,32*t2+31),32*t1-32*t2+31);t3++) {
          if (t1 == 2*t2) {
            for (t4=max(2,16*t1);t4<=t3-1;t4++) {
              if (t1%2 == 0) {
                B[t3+1][t4-2] = E[t3] * B[t3+3][t4-2] + 3;;
              }
            }
          }
          for (t4=max(32*t2,t3);t4<=min(PB_L-1,32*t2+31);t4++) {
            A[t3][t4] = C[t4-2][t3] - D[t4][t3-1] - D[t3][t4+2] + C[t3][0] + C[t3][t4] + A[t3-2][t4] + A[t3+1][t4] * A[t3][t4-2] * 4;;
            B[t3+1][t4-2] = E[t3] * B[t3+3][t4-2] + 3;;
          }
          for (t4=PB_L;t4<=min(PB_L+1,32*t2+31);t4++) {
            B[t3+1][t4-2] = E[t3] * B[t3+3][t4-2] + 3;;
          }
        }
      }
