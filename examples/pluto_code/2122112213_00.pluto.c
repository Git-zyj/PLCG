...
void kernel_2122112213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_N >= 2) {
  for (t1=0;t1<=PB_N-2;t1++) {
    for (t2=t1;t2<=PB_N-2;t2++) {
      A[t1][t1] = B[t2][t1+1] + A[t2+1][t2+1] - A[t1+1][t1+1] * 2;;
    }
  }
}
/* End of CLooG code */
...
kernel_2122112213_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
