...
void kernel_2214232211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
if (PB_M >= 3) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    for (t2=0;t2<=min(floord(PB_M-2,16),floord(32*t1+PB_M+28,32));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(min(31,PB_L-2),PB_M-1);t3++) {
          A[t3-2] = C[t3] - D[t3] - A[t3-1] * 3;;
          for (t4=t3;t4<=min(min(31,PB_L-2),t3+PB_M-3);t4++) {
            B[t3][(-t3+t4)] = E[(-t3+t4)+2][t3+2] + E[t3][(-t3+t4)+t3+1] - D[t3] + B[t3][(-t3+t4)+1] - B[t3-1][(-t3+t4)+1] + 1;;
          }
        }
      }
      if (t2 == 0) {
      ...
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(2,PB_L-1);t3<=min(31,PB_M-1);t3++) {
          A[t3-2] = C[t3] - D[t3] - A[t3-1] * 3;;
        }
      }
      if ((t2 >= 1) && (t2 <= floord(PB_L-2,32))) {
        for (t3=max(max(2,32*t1),32*t2-PB_M+3);t3<=min(min(min(PB_L-2,PB_M-1),32*t1+31),32*t2+31);t3++) {
          for (t4=max(32*t2,t3);t4<=min(min(PB_L-2,32*t2+31),t3+PB_M-3);t4++) {
            B[t3][(-t3+t4)] = E[(-t3+t4)+2][t3+2] + E[t3][(-t3+t4)+t3+1] - D[t3] + B[t3][(-t3+t4)+1] - B[t3-1][(-t3+t4)+1] + 1;;
          }
        }
      }
...
kernel_2214232211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
