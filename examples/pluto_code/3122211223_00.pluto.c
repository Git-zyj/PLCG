...
void kernel_3122211223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
/* Start of CLooG code */
if (PB_N >= 4) {
  for (t1=0;t1<=floord(PB_N-1,8);t1++) {
    lbp=ceild(3*t1,4);
    ubp=min(min(floord(3*PB_N-3,32),floord(64*t1+PB_N+61,96)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(32*t2-PB_N+1,2),32*t1-32*t2);t3<=min(min(floord(32*t2+31,3),PB_N-1),32*t1-32*t2+31);t3++) {
        for (t4=max(max(32*t2,3*t3),2*t3+3);t4<=min(32*t2+31,2*t3+PB_N-1);t4++) {
          A[(-2*t3+t4)][(-2*t3+t4)-1] = B[(-2*t3+t4)] + A[(-2*t3+t4)-1][(-2*t3+t4)-2] - A[(-2*t3+t4)-2][(-2*t3+t4)-3] * 5;;
        }
      }
    }
  }
...
kernel_3122211223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
