...
void kernel_3212232121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
if (PB_M >= 3) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    for (t2=0;t2<=min(floord(PB_M-3,32),floord(-32*t1+PB_M-1,32));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(31,floord(PB_M-1,2));t3++) {
          B[t3-2] = E[t3] * F[t3] + 3;;
          for (t4=t3;t4<=min(31,-t3+PB_M-1);t4++) {
            A[t3][t4+t3] = C[t4+t3][t4-1] - D[t4][t3] * D[t3][0] + A[t3-2][t4+t3-2] * A[t3][t4+t3-2] + 2;;
          }
        }
      }
      if (t2 >= 1) {
        for (t3=max(2,32*t1);t3<=min(min(min(floord(PB_M-1,2),32*t1+31),32*t2+31),-32*t2+PB_M-1);t3++) {
      ...
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=ceild(PB_M,2);t3<=min(31,PB_M-1);t3++) {
          B[t3-2] = E[t3] * F[t3] + 3;;
        }
      }
    }
...
kernel_3212232121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
