...
void kernel_3212232121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
...
if (PB_M >= 3) {
  if (PB_P >= 0) {
    for (t1=0;t1<=floord(PB_M+PB_P-3,32);t1++) {
      for (t2=0;t2<=floord(PB_P,32);t2++) {
        for (t3=0;t3<=min(min(min(min(floord(t1,2),floord(PB_M-1,32)),floord(-64*t1+3*PB_M+2*PB_P-9,32)),floord(-64*t2+PB_M+2*PB_P-3,32)),t2);t3++) {
          if ((t1 == 0) && (t2 == 0) && (t3 == 0)) {
            for (t4=0;t4<=min(3,PB_M-3);t4++) {
              A[t4+2] = C[t4+2] + D[t4] - D[t4] - C[1] * A[t4+1] * 2;;
            }
          }
          if ((t2 >= 1) && (t2 <= floord(PB_P-1,32))) {
            for (t4=max(4,32*t1);t4<=min(min(min(2*PB_P-6,32*t1+31),PB_M+PB_P-4),32*t3+PB_P+28);t4++) {
              for (t5=max(max(2,32*t3),t4-PB_P+3);t5<=min(min(floord(t4,2),PB_M-1),32*t3+31);t5++) {
                lbv=max(32*t2,t5);
                ubv=min(PB_P-1,32*t2+31);
#pragma ivdep
#pragma vector always
                for (t6=lbv;t6<=ubv;t6++) {
                  B[t5][(t4-t5)][t6] = E[t6+2][t5][(t4-t5)] - E[(t4-t5)][t5][t6] * B[t5-2][(t4-t5)+2][t6] - 6;;
                }
              }
            }
          }
          if ((t2 == 0) && (t3 == 0)) {
            for (t4=max(4,32*t1);t4<=min(min(min(PB_M-3,PB_P+28),2*PB_P-6),32*t1+31);t4++) {
              A[t4+2] = C[t4+2] + D[t4] - D[t4] - C[1] * A[t4+1] * 2;;
              for (t5=max(2,t4-PB_P+3);t5<=min(31,floord(t4,2));t5++) {
                lbv=t5;
                ubv=min(31,PB_P-1);
#pragma ivdep
#pragma vector always
          ...
          if ((t2 == 0) && (t3 == 0)) {
            for (t4=max(max(4,32*t1),2*PB_P-5);t4<=min(min(PB_M-3,PB_P+28),32*t1+31);t4++) {
              A[t4+2] = C[t4+2] + D[t4] - D[t4] - C[1] * A[t4+1] * 2;;
            }
          }
          if ((t2 == 0) && (t3 == 0)) {
            for (t4=max(max(4,32*t1),PB_M-2);t4<=min(min(min(PB_P+28,2*PB_P-6),32*t1+31),PB_M+PB_P-4);t4++) {
              for (t5=max(2,t4-PB_P+3);t5<=min(min(31,floord(t4,2)),PB_M-1);t5++) {
                lbv=t5;
                ubv=min(31,PB_P-1);
#pragma ivdep
#pragma vector always
          ...
          if ((t2 == 0) && (t3 == 0)) {
            for (t4=max(32*t1,PB_P+29);t4<=min(PB_M-3,32*t1+31);t4++) {
              A[t4+2] = C[t4+2] + D[t4] - D[t4] - C[1] * A[t4+1] * 2;;
            }
          }
        }
...
kernel_3212232121_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
