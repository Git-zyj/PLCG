...
void kernel_2114112213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_L >= 2) && (PB_P >= 2)) {
  for (t2=0;t2<=floord(PB_L,32);t2++) {
    lbp=ceild(-PB_P-31,32);
    ubp=-1;
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t6=max(-PB_P,32*t4);t6<=min(-2,32*t4+31);t6++) {
        lbv=max(2,32*t2);
        ubv=min(PB_L,32*t2+31);
#pragma ivdep
#pragma vector always
        for (t7=lbv;t7<=ubv;t7++) {
          A[t7-1][-t6-2] = B[-t6][t7-2] * A[t7-1][-t6-1] * A[t7-2][-t6-1] * 6;;
        }
      }
    }
  }
...
kernel_2114112213_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
