...
void kernel_3212112211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 3) {
  if (PB_N >= 2) {
    for (t2=2;t2<=PB_L-1;t2++) {
      A[t2-1] = C[t2] - A[t2-2] * A[t2] - A[t2-2] + 2;;
      for (t4=-PB_N+2;t4<=0;t4++) {
        B[-t4][1] = B[-t4+1][1] - 5;;
      }
    }
  }
...
kernel_3212112211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
