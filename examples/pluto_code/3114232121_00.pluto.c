...
void kernel_3114232121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_N >= 2) {
  for (t1=0;t1<=PB_N-2;t1++) {
    A[t1] = B[t1] + C[t1+1] - C[t1] * A[t1+1] - 5;;
  }
}
/* End of CLooG code */
...
kernel_3114232121_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
