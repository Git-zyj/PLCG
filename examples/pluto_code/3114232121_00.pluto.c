...
void kernel_3114232121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][t5][t6] = B[t4+1][0][t5] + C[t5][t6][t4] + B[t6][t4+1][t5] + A[t4][t5-1][t6] * 4;;
            }
          }
        }
      }
...
kernel_3114232121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
