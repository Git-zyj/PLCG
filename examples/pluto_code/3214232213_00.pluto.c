...
void kernel_3214232213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
    ...
    for (t2=0;t2<=floord(PB_M-3,32);t2++) {
      if ((PB_M >= 5) && (t2 == 0)) {
        for (t3=max(2,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
          B[t3] = D[t3] - D[t3] + D[t3-1] + B[t3+1] + 2;;
          for (t4=2;t4<=min(31,PB_M-3);t4++) {
            A[t4-2][t3+2] = C[t3][t4+1] * C[t3-2][t3] + C[t3][t4+2] * A[t4-2][t3+1] + A[t4-1][t3+2] + A[t4-2][t3+3] * 3;;
          }
        }
      }
      if (t2 >= 1) {
        for (t3=max(2,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
          for (t4=32*t2;t4<=min(PB_M-3,32*t2+31);t4++) {
            A[t4-2][t3+2] = C[t3][t4+1] * C[t3-2][t3] + C[t3][t4+2] * A[t4-2][t3+1] + A[t4-1][t3+2] + A[t4-2][t3+3] * 3;;
          }
        }
      }
      if ((PB_M == 4) && (t1 == 0) && (t2 == 0)) {
        B[2] = D[2] - D[2] + D[2 -1] + B[2 +1] + 2;;
      }
    }
  }
}
...
kernel_3214232213_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
