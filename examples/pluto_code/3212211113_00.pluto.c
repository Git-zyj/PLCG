...
void kernel_3212211113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_L >= 2) {
  if (PB_P >= 0) {
    for (t1=max(ceild(-PB_P-15,16),ceild(-PB_P-47,32));t1<=floord(PB_L-2,16);t1++) {
      lbp=max(max(ceild(-PB_P-15,16),ceild(32*t1-PB_L+2,16)),ceild(32*t1-PB_L-PB_P+2,32));
      ubp=min(t1,2*t1+2);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
      for (t2=lbp;t2<=ubp;t2++) {
        if ((PB_P >= 1) && (t1 <= floord(32*t2+PB_P,32)) && (t2 >= 1)) {
          for (t3=max(32*t1-16*t2,16*t2+1);t3<=min(min(PB_L-2,32*t1-16*t2+46),16*t2+PB_P+15);t3++) {
            lbv=max(max(1,32*t1-32*t2),-16*t2+t3-15);
            ubv=min(min(PB_P,-16*t2+t3),32*t1-32*t2+31);
#pragma ivdep
#pragma vector always
            for (t4=lbv;t4<=ubv;t4++) {
              A[t3][t4-1] = C[0] - D[t3+1][t3+1] - B[t4+1] - 5;;
            }
          }
        }
        if ((PB_P == 0) && (t2 == 0)) {
          for (t3=32*t1;t3<=(min(PB_L-2,32*t1+31))-7;t3+=8) {
            B[t3+1] = B[t3+2] * 1;;
            B[(t3+1)+1] = B[(t3+1)+2] * 1;;
            B[(t3+2)+1] = B[(t3+2)+2] * 1;;
            B[(t3+3)+1] = B[(t3+3)+2] * 1;;
          ...
          for (;t3<=min(PB_L-2,32*t1+31);t3++) {
            B[t3+1] = B[t3+2] * 1;;
          }
        }
        if ((PB_P >= 1) && (t1 >= ceild(PB_P+1,32)) && (t2 == 0)) {
          for (t3=32*t1;t3<=(min(PB_L-2,32*t1+31))-7;t3+=8) {
            B[t3+1] = B[t3+2] * 1;;
            B[(t3+1)+1] = B[(t3+1)+2] * 1;;
            B[(t3+2)+1] = B[(t3+2)+2] * 1;;
        ...
        if ((PB_P >= 1) && (t1 <= floord(PB_P,32)) && (t2 == 0)) {
          for (t3=max(1,32*t1);t3<=min(min(PB_L-2,PB_P+15),32*t1+31);t3++) {
...
#pragma vector always
        ...
        if (t1 <= floord(32*t2+PB_P,32)) {
          for (t3=max(max(0,32*t1-16*t2),32*t1+16*t2+32);t3<=min(min(PB_L-2,32*t1-16*t2+46),16*t2+PB_P+15);t3++) {
...
#pragma vector always
        ...
        if ((PB_P >= 1) && (t1 <= floord(PB_P,32)) && (t2 == 0)) {
          for (t3=PB_P+16;t3<=(min(PB_L-2,32*t1+31))-7;t3+=8) {
            B[t3+1] = B[t3+2] * 1;;
            B[(t3+1)+1] = B[(t3+1)+2] * 1;;
            B[(t3+2)+1] = B[(t3+2)+2] * 1;;
            B[(t3+3)+1] = B[(t3+3)+2] * 1;;
...
kernel_3212211113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
