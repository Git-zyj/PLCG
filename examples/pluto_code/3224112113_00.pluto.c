...
void kernel_3224112113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
  ...
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    if ((PB_M >= 4) && (t1 <= floord(PB_M-2,32))) {
      for (t6=max(2,32*t1);t6<=min(PB_M-2,32*t1+31);t6++) {
        A[t6] = C[t6-2] + C[0] + A[t6+1] + A[t6-1] + 5;;
      }
    }
    if (t1 <= floord(PB_P-2,32)) {
      lbp=t1;
      ubp=floord(PB_M-1,32);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
      for (t2=lbp;t2<=ubp;t2++) {
        for (t3=t1;t3<=floord(PB_P-1,32);t3++) {
          for (t4=32*t2;t4<=min(PB_M-1,32*t2+31);t4++) {
            for (t5=max(32*t3,32*t1+1);t5<=min(PB_P-1,32*t3+31);t5++) {
              for (t6=32*t1;t6<=min(min(t4,32*t1+31),t5-1);t6++) {
                B[t4][t6] = 5;;
              }
            }
          }
...
kernel_3224112113_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
