...
void kernel_3224112113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 1) {
  if ((PB_M >= ceild(-PB_P+4,2)) && (PB_P >= 0)) {
    for (t1=0;t1<=min(floord(PB_M+PB_P-1,32),floord(2*PB_M+3*PB_P-4,64));t1++) {
      ...
      if ((PB_P >= 2) && (t1 <= floord(PB_P,32))) {
        lbp=0;
        ubp=min(min(floord(PB_M-1,32),floord(PB_P-1,32)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
        for (t2=lbp;t2<=ubp;t2++) {
          for (t3=0;t3<=min(floord(PB_P-1,32),t1);t3++) {
            for (t4=32*t2;t4<=min(min(min(PB_M-1,PB_P-1),32*t1+30),32*t2+31);t4++) {
              for (t5=max(1,32*t3);t5<=min(min(PB_P-1,32*t1+30),32*t3+31);t5++) {
                for (t6=max(max(32*t1,t4+1),t5+1);t6<=min(PB_P,32*t1+31);t6++) {
                  B[t4][t5][t6-1] = B[t4][t5-1][t6-1] * 4;;
                }
              }
            }
          }
...
kernel_3224112113_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
