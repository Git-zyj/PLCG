...
void kernel_2112132223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(2,32*t1-32*t2);t3<=min(min(PB_L-5,32*t1-32*t2+31),-32*t2+PB_L-3);t3++) {
        for (t4=max(2,32*t2);t4<=min(min(t3,32*t2+31),-t3+PB_L-3);t4++) {
          A[t3+t4+2][t3] = B[t4-2][t3+t4+2] + B[t3+t4][0] - B[t3][t4] * A[t3+t4][t3] * A[t3+t4+1][t3-1] + 1;;
        }
      }
    }
  }
...
kernel_2112132223_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
