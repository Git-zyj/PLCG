...
if ((PB_L >= 3) && (PB_M >= 4)) {
  lbp=0;
  ubp=min(floord(PB_L-3,32),floord(PB_M-4,64));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=min(floord(PB_L-2,32),floord(-32*t1+PB_M-3,32));t2++) {
      for (t3=max(32*t2,32*t1+1);t3<=min(min(PB_L-2,32*t2+31),-32*t1+PB_M-3);t3++) {
        lbv=32*t1;
        ubv=min(min(32*t1+31,t3-1),-t3+PB_M-3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t4+t3+1][t3] = B[t4+t3][t3] + B[t3+1][t4] - B[t4+t3][t3+1] - A[t4+t3+2][t3+1] * 1;;
        }
      }
    }
  }
