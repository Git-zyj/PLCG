...
void kernel_2222232113_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
if (PB_L >= 1) {
  ...
  for (t1=1;t1<=PB_L-1;t1++) {
    A[t1+1] = C[t1] * A[t1+2] * 2;;
    A[t1+1] = D[2] * E[t1+2][t1] - C[t1] * D[t1-1] - 2;;
  }
}
/* End of CLooG code */
...
kernel_2222232113_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
