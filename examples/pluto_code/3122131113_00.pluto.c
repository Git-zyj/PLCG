...
void kernel_3122131113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_M >= 2) {
  for (t1=0;t1<=floord(3*PB_M-2,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_M,32));
    ubp=min(min(floord(PB_M-1,16),floord(32*t1+PB_M+30,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,32*t1-32*t2),32*t2-PB_M+1);t3<=min(min(PB_M,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(32*t2+31,t3+PB_M-1);t4++) {
          A[(-t3+t4)] = B[t3-1][(-t3+t4)] * B[(-t3+t4)][t3] + C[(-t3+t4)-1] * A[(-t3+t4)-1] - 3;;
        }
      }
    }
  }
...
kernel_3122131113_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
