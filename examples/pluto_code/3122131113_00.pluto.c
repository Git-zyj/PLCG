...
void kernel_3122131113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t4=ceild(-PB_N-29,32);t4<=-1;t4++) {
      for (t6=max(32*t4,-PB_N+2);t6<=32*t4+31;t6++) {
        lbv=32*t2;
        ubv=min(PB_N-1,32*t2+31);
#pragma ivdep
#pragma vector always
        for (t7=lbv;t7<=ubv;t7++) {
          A[-t6][t7+1] = B[-t6-1][t7] - B[t7][-t6] * B[-t6+1][t7] + A[-t6-1][t7+1] + 2;;
        }
      }
    }
  }
...
kernel_3122131113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
