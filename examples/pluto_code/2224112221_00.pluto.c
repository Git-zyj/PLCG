...
void kernel_2224112221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
      for (t2=lbp;t2<=ubp;t2++) {
        for (t3=32*t1-32*t2;t3<=min(min(min(PB_L-3,PB_M-2),32*t2+31),32*t1-32*t2+31);t3++) {
          if (t1 == 2*t2) {
            for (t4=16*t1;t4<=t3-1;t4++) {
              if (t1%2 == 0) {
                B[t3+1][t4] = D[t4] - B[t3+1][t4+2] * 4;;
              }
            }
          }
          for (t4=max(32*t2,t3);t4<=min(PB_L-3,32*t2+31);t4++) {
            A[t3][t4] = C[t4][t3+1] * A[t4][t3+1] - A[t3+1][t4+2] - A[t3][t4+1] + 6;;
            B[t3+1][t4] = D[t4] - B[t3+1][t4+2] * 4;;
          }
        }
        for (t3=max(PB_M-1,32*t1-32*t2);t3<=min(min(PB_L-3,32*t2+31),32*t1-32*t2+31);t3++) {
          for (t4=max(32*t2,t3);t4<=min(PB_L-3,32*t2+31);t4++) {
            A[t3][t4] = C[t4][t3+1] * A[t4][t3+1] - A[t3+1][t4+2] - A[t3][t4+1] + 6;;
          }
        }
        ...
        if (t1 == 2*t2) {
          for (t3=PB_L-2;t3<=min(PB_M-2,16*t1+31);t3++) {
            for (t4=16*t1;t4<=PB_L-3;t4++) {
              if (t1%2 == 0) {
                B[t3+1][t4] = D[t4] - B[t3+1][t4+2] * 4;;
              }
            }
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
      for (t2=lbp;t2<=ubp;t2++) {
        for (t3=32*t1-32*t2;t3<=min(PB_L-3,32*t1-32*t2+31);t3++) {
          for (t4=max(32*t2,t3);t4<=min(PB_L-3,32*t2+31);t4++) {
            A[t3][t4] = C[t4][t3+1] * A[t4][t3+1] - A[t3+1][t4+2] - A[t3][t4+1] + 6;;
          }
        }
...
kernel_2224112221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
