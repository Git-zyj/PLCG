...
void kernel_3222132113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
for (t1=1;t1<=min(PB_L-2,PB_M-1);t1++) {
  A[t1] = C[t1] + C[t1] + C[t1-1] - C[t1] - 5;;
  B[t1] = D[t1][t1+1] - B[t1-1] + 3;;
}
for (t1=max(1,PB_L-1);t1<=PB_M-1;t1++) {
  A[t1] = C[t1] + C[t1] + C[t1-1] - C[t1] - 5;;
}
for (t1=max(1,PB_M);t1<=PB_L-2;t1++) {
  B[t1] = D[t1][t1+1] - B[t1-1] + 3;;
}
/* End of CLooG code */
...
kernel_3222132113_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
