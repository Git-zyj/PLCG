...
void kernel_2124211212_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
for (t1=0;t1<=min(floord(PB_M-1,16),floord(PB_P+PB_M-1,32));t1++) {
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t3=max(1,32*t2);t3<=min(PB_M-1,32*t2+31);t3++) {
      for (t4=max(1,32*t1-32*t2);t4<=min(min(PB_P,t3),32*t1-32*t2+31);t4++) {
        A[t3][t4+1] = B[t4][t3-1][t4-1] + A[t3-1][t4+1] + A[t3][t4] - 4;;
      }
    }
  }
}
...
kernel_2124211212_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
