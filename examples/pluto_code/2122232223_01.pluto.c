...
void kernel_2122232223_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,ceild(32*t2-PB_M+1,3)),32*t1-32*t2);t3<=min(min(floord(32*t2+29,3),PB_M-1),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,3*t3+2);t4<=min(32*t2+31,3*t3+PB_M-1);t4++) {
          A[t3][(-3*t3+t4)][t3] = B[(-3*t3+t4)-2][t3] + C[(-3*t3+t4)][t3-1] + C[(-3*t3+t4)-2][t3] * A[t3][(-3*t3+t4)+2][t3] * A[(-3*t3+t4)-2][t3][(-3*t3+t4)-2] + 3;;
        }
      }
    }
  }
...
kernel_2122232223_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
