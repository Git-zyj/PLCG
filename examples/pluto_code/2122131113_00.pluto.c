...
void kernel_2122131113_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t6=max(32*t4,-32*t2-31);t6<=min(PB_M-1,32*t4+31);t6++) {
        for (t7=max(32*t2,-t6);t7<=32*t2+31;t7++) {
          A[t6-1][-t7][-t7] = B[t6] - C[t6][0] - B[-t7] + A[t6-1][-t7-1][-t7-1] - 1;;
        }
      }
    }
  }
...
kernel_2122131113_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
