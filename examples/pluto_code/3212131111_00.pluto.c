...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_M-1,32);t2++) {
      if ((PB_M >= 3) && (PB_P <= -1) && (t1 >= t2) && (t2 <= floord(PB_M-2,32))) {
        for (t4=max(max(2,32*t1),32*t2+1);t4<=min(PB_M-1,32*t1+31);t4++) {
          for (t5=max(1,32*t2);t5<=min(32*t2+31,t4-1);t5++) {
            A[t5-1][t4] = C[t4][t5] * D[t5] * C[t4][t5-1] + 2;;
          }
        }
      }
      if ((PB_P >= 0) && (t2 <= floord(2*PB_M+PB_P-4,64))) {
        for (t3=max(0,-2*t1+2*t2);t3<=min(min(min(floord(2*PB_M+PB_P-3,32),floord(64*t2+PB_P+61,32)),floord(128*t2+63*PB_P,64)),floord(4*PB_M+5*PB_P-12,64));t3++) {
          if ((PB_P <= 1) && (t3 == 0)) {
            for (t4=max(max(2,32*t1),32*t2+1);t4<=min(PB_M-1,32*t1+31);t4++) {
              for (t5=max(1,32*t2);t5<=min(32*t2+31,t4-1);t5++) {
                A[t5-1][t4] = C[t4][t5] * D[t5] * C[t4][t5-1] + 2;;
          ...
          if ((PB_P >= 2) && (t2 <= floord(t3,2)) && (t3 >= 1)) {
            for (t4=32*t1;t4<=min(PB_M-1,32*t1+31);t4++) {
              for (t5=max(ceild(32*t3-PB_P+1,2),32*t2);t5<=min(min(PB_M-1,32*t2+31),16*t3+15);t5++) {
                for (t6=max(32*t3,2*t5+1);t6<=min(32*t3+31,2*t5+PB_P-1);t6++) {
                  B[(-2*t5+t6)-1][t4][t5] = E[t4][t5+1][(-2*t5+t6)-1] - E[t4][t5][(-2*t5+t6)] + B[t5][t4][(-2*t5+t6)] * 5;;
                }
              }
            }
          }
          ...
          if ((PB_P >= 2) && (t2 == 0) && (t3 == 0)) {
            for (t4=max(2,32*t1);t4<=min(PB_M-1,32*t1+31);t4++) {
              for (t6=1;t6<=min(31,PB_P-1);t6++) {
                B[t6-1][t4][0] = E[t4][0 +1][t6-1] - E[t4][0][t6] + B[0][t4][t6] * 5;;
              }
              ...
              for (t5=16;t5<=min(31,t4-1);t5++) {
                A[t5-1][t4] = C[t4][t5] * D[t5] * C[t4][t5-1] + 2;;
              }
              if (t1 == 0) {
                for (t5=t4;t5<=min(15,PB_M-1);t5++) {
                  for (t6=2*t5+1;t6<=min(31,2*t5+PB_P-1);t6++) {
                    B[(-2*t5+t6)-1][t4][t5] = E[t4][t5+1][(-2*t5+t6)-1] - E[t4][t5][(-2*t5+t6)] + B[t5][t4][(-2*t5+t6)] * 5;;
                  }
                }
