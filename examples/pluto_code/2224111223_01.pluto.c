...
void kernel_2224111223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_M >= 1) {
  for (t1=0;t1<=floord(PB_M,32);t1++) {
    lbp=max(1,32*t1);
    ubp=min(PB_M,32*t1+31);
#pragma omp parallel for private(lbv,ubv)
    for (t4=lbp;t4<=ubp;t4++) {
      A[t4-1] = 6;;
    }
    if ((PB_M >= 5) && (t1 <= floord(PB_M-3,32))) {
      for (t2=t1;t2<=floord(PB_M-2,32);t2++) {
        for (t3=max(max(3,32*t2),32*t1+1);t3<=min(PB_M-2,32*t2+31);t3++) {
          lbp=max(2,32*t1);
          ubp=min(32*t1+31,t3-1);
#pragma omp parallel for private(lbv,ubv)
          for (t4=lbp;t4<=ubp;t4++) {
            B[t3][t4-1] = C[t3-1][t4] * C[t3][t4] + B[t3-2][t4-1] - B[t3+1][t4-1] + B[t3-1][t4-2] - 5;;
          }
        }
      }
    }
...
kernel_2224111223_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
