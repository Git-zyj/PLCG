...
void kernel_2212212213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  ...
  for (t1=3;t1<=PB_M-3;t1++) {
    A[t1-1] = A[t1] - 1;;
    B[t1-2] = C[t1+2] * B[t1-1] * B[t1-3] - 1;;
  }
  for (t1=max(3,PB_M-2);t1<=PB_M-1;t1++) {
    A[t1-1] = A[t1] - 1;;
  }
}
/* End of CLooG code */
...
kernel_2212212213_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
