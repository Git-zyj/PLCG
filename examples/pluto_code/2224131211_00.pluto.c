...
void kernel_2224131211_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_L >= 1) {
  for (t1=1;t1<=min(2,PB_L);t1++) {
    B[t1-1] = C[t1+1] - C[t1] + C[t1] * 5;;
  }
  for (t1=3;t1<=PB_L;t1++) {
    B[t1-1] = C[t1+1] - C[t1] + C[t1] * 5;;
    B[(t1-1)-1] = C[(t1-1)] + C[(t1-1)] + B[(t1-1)] + B[(t1-1)-2] + 5;;
  }
}
/* End of CLooG code */
...
kernel_2224131211_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
