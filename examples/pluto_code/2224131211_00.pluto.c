...
void kernel_2224131211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 2) {
  for (t1=2;t1<=PB_L-1;t1++) {
    A[t1-1] = C[t1] * A[t1] - A[t1-2] + 4;;
    A[(t1-1)-1] = C[(t1-1)] - C[(t1-1)] + C[(t1-1)] - C[(t1-1)] * C[(t1-1)] + 5;;
  }
  A[(PB_L-1)-1] = C[(PB_L-1)] - C[(PB_L-1)] + C[(PB_L-1)] - C[(PB_L-1)] * C[(PB_L-1)] + 5;;
}
/* End of CLooG code */
...
kernel_2224131211_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
