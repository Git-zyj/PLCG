...
void kernel_3122232113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(-32*t1+PB_M-1,32),t1);t2++) {
      for (t3=max(2,32*t1);t3<=min(min(min(PB_L-1,PB_M-3),32*t1+31),-32*t2+PB_M-1);t3++) {
        for (t4=max(2,32*t2);t4<=min(min(t3,32*t2+31),-t3+PB_M-1);t4++) {
          A[t3][t4] = B[t3+2][t4] - C[t3][t4-2] + B[t3+t4][t3] + A[t3][t4+1] * 5;;
        }
      }
    }
  }
...
kernel_3122232113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
