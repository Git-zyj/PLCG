...
void kernel_2222132221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_N >= 4) {
  for (t1=0;t1<=PB_N-5;t1++) {
    A[t1+2] = C[t1] + A[t1+3] + B[t1+4] + 5;;
    B[t1+2] = C[t1] - C[t1+1] + C[t1+2] * C[t1+2] + A[t1+2] - B[t1+3] * 3;;
  }
  B[(PB_N-4)+2] = C[(PB_N-4)] - C[(PB_N-4)+1] + C[(PB_N-4)+2] * C[(PB_N-4)+2] + A[(PB_N-4)+2] - B[(PB_N-4)+3] * 3;;
}
/* End of CLooG code */
...
kernel_2222132221_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
