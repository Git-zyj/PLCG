...
void kernel_3224132123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_L >= 1) {
  for (t1=0;t1<=floord(PB_L-1,32);t1++) {
    for (t2=0;t2<=floord(PB_L-1,32);t2++) {
      if (t2 == 0) {
        for (t3=32*t1;t3<=min(min(PB_L-3,PB_P-1),32*t1+31);t3++) {
          A[t3][0 +t3] = C[0 +2][t3] + C[t3][0] + C[0 +t3][0] - 2;;
          B[t3] = D[t3] + D[t3+1] * B[t3+2] + 2;;
          lbv=1;
          ubv=min(min(31,PB_L-1),-t3+PB_P-1);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3][t4+t3] = C[t4+2][t3] + C[t3][t4] + C[t4+t3][t4] - 2;;
          }
        }
      }
      if (t2 >= 1) {
        lbp=32*t1;
        ubp=min(min(PB_L-1,32*t1+31),-32*t2+PB_P-1);
#pragma omp parallel for private(lbv,ubv,t4)
        for (t3=lbp;t3<=ubp;t3++) {
          lbv=32*t2;
          ubv=min(min(PB_L-1,32*t2+31),-t3+PB_P-1);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3][t4+t3] = C[t4+2][t3] + C[t3][t4] + C[t4+t3][t4] - 2;;
      ...
      if (t2 == 0) {
        lbp=max(32*t1,PB_L-2);
        ubp=min(min(PB_L-1,PB_P-1),32*t1+31);
#pragma omp parallel for private(lbv,ubv,t4)
        for (t3=lbp;t3<=ubp;t3++) {
          lbv=0;
          ubv=min(min(31,PB_L-1),-t3+PB_P-1);
...
#pragma vector always
      ...
      if (t2 == 0) {
        for (t3=max(PB_P,32*t1);t3<=min(PB_L-3,32*t1+31);t3++) {
          B[t3] = D[t3] + D[t3+1] * B[t3+2] + 2;;
        }
      }
    }
...
kernel_3224132123_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
