...
void kernel_2222212111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if ((PB_M >= 0) && (PB_N >= 0)) {
  lbp=0;
  ubp=min(floord(PB_N+PB_M-1,32),floord(PB_N+3*PB_M-3,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_N,32),floord(-32*t1+PB_N+PB_M-1,32));t2++) {
      if (t2 == 0) {
        for (t3=32*t1;t3<=min(min(PB_M-1,PB_N-3),32*t1+31);t3++) {
          B[t3] = 1;;
          for (t4=2;t4<=min(31,-t3+PB_N-1);t4++) {
            A[t4-1][t3] = C[t3][0] * C[t3+t4][t4] + A[t4-2][t3] * 4;;
          }
        }
      }
      if (t2 >= 1) {
        for (t3=32*t1;t3<=min(32*t1+31,-32*t2+PB_N-1);t3++) {
      ...
      if (t2 == 0) {
        for (t3=max(PB_M,32*t1);t3<=min(PB_N-3,32*t1+31);t3++) {
          for (t4=2;t4<=min(31,-t3+PB_N-1);t4++) {
            A[t4-1][t3] = C[t3][0] * C[t3+t4][t4] + A[t4-2][t3] * 4;;
          }
        }
      ...
      if (t2 == 0) {
        for (t3=max(32*t1,PB_N-2);t3<=min(PB_M-1,32*t1+31);t3++) {
          B[t3] = 1;;
        }
      }
    }
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(-32*t1+PB_N-1,32);t2++) {
      for (t3=32*t1;t3<=min(min(PB_N-3,32*t1+31),-32*t2+PB_N-1);t3++) {
        for (t4=max(2,32*t2);t4<=min(32*t2+31,-t3+PB_N-1);t4++) {
          A[t4-1][t3] = C[t3][0] * C[t3+t4][t4] + A[t4-2][t3] * 4;;
        }
      }
    }
...
kernel_2222212111_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
