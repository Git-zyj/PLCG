...
void kernel_2224212123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    if ((PB_N <= -1) && (t1 <= floord(PB_M-2,32))) {
      for (t4=32*t1;t4<=min(PB_M-2,32*t1+31);t4++) {
        B[t4] = D[t4] - B[t4+1] - 4;;
      }
    }
    for (t2=0;t2<=min(floord(PB_N,32),floord(-96*t1+PB_N+3*PB_M-6,32));t2++) {
      if ((PB_M >= 3) && (PB_N >= 3) && (t1 <= floord(PB_M-2,32)) && (t2 == 0)) {
        if (t1 == 0) {
          for (t4=0;t4<=1;t4++) {
            B[t4] = D[t4] - B[t4+1] - 4;;
          }
        ...
        for (t4=max(2,32*t1);t4<=min(PB_M-2,32*t1+31);t4++) {
          A[0][t4-2] = C[t4][0] - A[0 +2][t4-2] * 6;;
          B[t4] = D[t4] - B[t4+1] - 4;;
        }
        if (t1 >= ceild(PB_M-32,32)) {
          A[0][(PB_M-1)-2] = C[(PB_M-1)][0] - A[0 +2][(PB_M-1)-2] * 6;;
        }
      }
      if ((32*t1 == PB_M-1) && (t2 == 0)) {
        if ((PB_M+31)%32 == 0) {
          A[0][(PB_M-1)-2] = C[(PB_M-1)][0] - A[0 +2][(PB_M-1)-2] * 6;;
        }
      }
      ...
      if ((PB_M >= 3) && (PB_N <= 2) && (t2 == 0)) {
        for (t4=32*t1;t4<=min(PB_M-2,32*t1+31);t4++) {
          B[t4] = D[t4] - B[t4+1] - 4;;
        }
      }
      if (PB_M >= 3) {
        for (t3=max(1,32*t2);t3<=min(PB_N-3,32*t2+31);t3++) {
          for (t4=max(2,32*t1);t4<=min(PB_M-1,32*t1+31);t4++) {
            A[t3][t4-2] = C[t4][t3] - A[t3+2][t4-2] * 6;;
          }
        }
      }
    }
...
kernel_2224212123_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
