...
void kernel_2114212223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
/* Start of CLooG code */
if (PB_L >= 6) {
  for (t1=-1;t1<=floord(PB_L-2,16);t1++) {
    lbp=ceild(t1,2);
    ubp=min(floord(PB_L-2,32),floord(32*t1+PB_L+21,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(4,16*t1);t3<=min(min(floord(32*t1-32*t2+PB_L+29,2),PB_L-2),16*t1+31);t3++) {
        lbv=max(max(32*t2,t3),-32*t1+32*t2+2*t3-31);
        ubv=min(min(PB_L-2,32*t2+31),-32*t1+32*t2+2*t3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-t3+t4)+t3] = B[t3+2][(-t3+t4)][(-t3+t4)+t3+1] * A[t3+2][(-t3+t4)+t3+1] + A[t3-2][(-t3+t4)+t3-4] * 1;;
        }
      }
    }
  }
...
kernel_2114212223_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
