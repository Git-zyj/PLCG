...
void kernel_3114131221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if ((PB_M >= 3) && (PB_N >= 4)) {
  for (t1=1;t1<=PB_M-2;t1++) {
    for (t2=0;t2<=floord(4*t1+PB_N-6,32);t2++) {
      lbp=max(ceild(3*t2,4),ceild(-t1+32*t2+1,32));
      ubp=min(min(floord(3*t1+PB_N-5,32),floord(96*t2+PB_N+91,128)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(ceild(32*t3-PB_N+2,3),32*t2-32*t3);t4<=min(min(floord(32*t3+29,3),t1-1),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,3*t4+2);t5<=min(32*t3+31,3*t4+PB_N-2);t5++) {
            A[t4][(-3*t4+t5)] = B[(-3*t4+t5)-1][t1+1][t4] + B[t4][t1-1][t4] * B[(-3*t4+t5)+1][t4][t1-1] + A[t4][(-3*t4+t5)-2] * A[(-3*t4+t5)-2][t4] - 6;;
          }
        }
      }
    }
...
kernel_3114131221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
