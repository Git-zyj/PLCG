...
void kernel_3224232211_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
if (PB_M >= 2) {
  if (PB_P >= 0) {
    for (t1=0;t1<=min(floord(PB_P+PB_M-1,32),floord(PB_P+5*PB_M-13,32));t1++) {
      if ((PB_M >= 3) && (t1 <= floord(PB_M-1,32))) {
        for (t6=max(2,32*t1);t6<=min(PB_M-1,32*t1+31);t6++) {
          B[t6-1] = E[t6] * E[t6+1] - E[t6] + E[t6+2] + B[t6] * B[t6-2] - B[t6] * 3;;
        }
      }
          ...
          for (t3=0;t3<=floord(PB_M-1,32);t3++) {
            for (t4=32*t2;t4<=min(PB_M-1,32*t2+31);t4++) {
              for (t5=max(1,32*t3);t5<=min(PB_M-1,32*t3+31);t5++) {
                for (t6=32*t1;t6<=min(PB_P-3,32*t1+31);t6++) {
                  A[t6][t4][t5] = C[t4][t5][t6] - D[t4][t5+2][t6+2] + A[t6][t4][t5-1] + 3;;
                }
              }
            }
          }
...
kernel_3224232211_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
