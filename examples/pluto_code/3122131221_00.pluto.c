...
void kernel_3122131221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_M-1,32),t1);t2++) {
      for (t3=0;t3<=min(floord(PB_M-2,32),t1);t3++) {
        for (t4=32*t2;t4<=min(min(PB_M-1,32*t1+30),32*t2+31);t4++) {
          for (t5=max(2,32*t3);t5<=min(min(PB_M-2,32*t1+30),32*t3+31);t5++) {
            lbv=max(max(32*t1,t4+1),t5+1);
            ubv=min(PB_M,32*t1+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t5][t4][t6] = B[t6][t4][t5] + B[0][t4][t5] * B[t4][t6-1][t5-1] - A[t5+1][t4][t6] * A[t5-2][t4][t6] + 1;;
            }
          }
        }
      }
...
kernel_3122131221_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
