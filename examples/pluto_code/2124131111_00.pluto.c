...
void kernel_2124131111_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if ((PB_L >= 2) && (PB_M >= 2) && (PB_P >= 1)) {
  lbp=0;
  ubp=min(floord(PB_P,32),floord(PB_M-1,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(floord(PB_L-2,32),floord(PB_M-2,32)),floord(-32*t1+PB_M-1,32));t2++) {
      for (t3=max(1,32*t1);t3<=min(min(PB_P,32*t1+31),-32*t2+PB_M-1);t3++) {
        for (t4=32*t2;t4<=min(min(PB_L-2,32*t2+31),-t3+PB_M-1);t4++) {
          A[t3+1][t4][t3-1] = B[t3+1] * C[t3][t4+1] - C[t3+t4][t3+1] * A[t3+1][t4+1][t3-1] * 5;;
        }
      }
    }
  }
...
kernel_2124131111_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
