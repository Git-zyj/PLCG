...
void kernel_3122231121_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_M-3,32),floord(-32*t1+PB_M-2,32));t2++) {
      for (t3=0;t3<=min(floord(-32*t1+PB_L,32),t1);t3++) {
        for (t4=32*t2;t4<=min(min(PB_M-3,32*t2+31),-32*t1+PB_M-2);t4++) {
          for (t5=max(1,32*t3);t5<=min(min(min(floord(PB_L,2),-32*t1+PB_L),32*t3+31),-t4+PB_M-2);t5++) {
            lbv=max(32*t1,t5);
            ubv=min(min(32*t1+31,-t5+PB_L),-t4+PB_M-2);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t5+1][t4][t4+t6] = B[t4+t6+1][t6] + C[t4][t6+t5-1][t5] - D[t4][t5][t6] * A[t5-1][t4][t4+t6] + 2;;
            }
          }
        }
      }
...
kernel_3122231121_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
