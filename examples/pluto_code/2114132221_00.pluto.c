...
void kernel_2114132221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t2;t3<=min(PB_L-3,32*t2+31);t3++) {
        lbv=max(2,32*t1-32*t2);
        ubv=min(PB_L,32*t1-32*t2+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+2][t4] = B[t3] + C[t4][t4-1] * B[t3] * A[t3][t4-2] - A[t3+1][t4] - 1;;
        }
      }
    }
  }
...
kernel_2114132221_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
