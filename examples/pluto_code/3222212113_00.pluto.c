...
void kernel_3222212113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(1,32*t2);t3<=min(min(PB_M-1,96*t1-96*t2),32*t2+31);t3++) {
        for (t4=32*t1-32*t2;t4<=min(PB_M-1,32*t1-32*t2+31);t4++) {
          B[t3][t4] = C[t3][t4+2] - D[t3][t3] * B[t3][t4-1] + 2;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
      ...
      for (t3=max(max(4,32*t2),96*t1-96*t2+1);t3<=min(PB_M-1,32*t2+31);t3++) {
        for (t4=max(1,32*t1-32*t2);t4<=min(floord(t3-1,3),32*t1-32*t2+31);t4++) {
          A[(t3-3*t4)][t4] = A[t4-1][(t3-3*t4)-1] + 5;;
          B[t3][t4] = C[t3][t4+2] - D[t3][t3] * B[t3][t4-1] + 2;;
        }
        for (t4=ceild(t3,3);t4<=min(PB_M-1,32*t1-32*t2+31);t4++) {
          B[t3][t4] = C[t3][t4+2] - D[t3][t3] * B[t3][t4-1] + 2;;
        }
      }
      for (t3=max(max(max(4,PB_M),32*t2),96*t1-96*t2+1);t3<=min(min(4*PB_M-4,32*t2+31),96*t1-96*t2+PB_M+92);t3++) {
        for (t4=max(ceild(t3-PB_M+1,3),32*t1-32*t2);t4<=min(min(floord(t3-1,3),PB_M-1),32*t1-32*t2+31);t4++) {
          A[(t3-3*t4)][t4] = A[t4-1][(t3-3*t4)-1] + 5;;
        }
      }
    }
...
kernel_3222212113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
