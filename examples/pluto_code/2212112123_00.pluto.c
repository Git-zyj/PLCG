...
void kernel_2212112123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
  int t1, t2;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
for (t1=0;t1<=min(1,PB_N-1);t1++) {
  for (t2=t1;t2<=PB_N-1;t2++) {
    A[2] = 3;;
  }
}
for (t1=2;t1<=min(PB_M-3,PB_N-1);t1++) {
  for (t2=t1;t2<=min(PB_M-2,PB_N-1);t2++) {
    A[2] = 3;;
    B[t1][t2] = C[t1+2][t2+1] - C[t1-2][t2-1] * B[t1][t2+1] + B[t1][t2-2] - 5;;
  }
  for (t2=PB_M-1;t2<=PB_N-1;t2++) {
    A[2] = 3;;
  }
  for (t2=PB_N;t2<=PB_M-2;t2++) {
    B[t1][t2] = C[t1+2][t2+1] - C[t1-2][t2-1] * B[t1][t2+1] + B[t1][t2-2] - 5;;
  }
}
for (t1=max(2,PB_M-2);t1<=PB_N-1;t1++) {
  for (t2=t1;t2<=PB_N-1;t2++) {
    A[2] = 3;;
  }
}
for (t1=max(2,PB_N);t1<=PB_M-3;t1++) {
...
kernel_2212112123_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
