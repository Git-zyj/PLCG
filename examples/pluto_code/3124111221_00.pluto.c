...
void kernel_3124111221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5, t6;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_N >= 3) && (PB_P >= 3)) {
  lbp=0;
    ...
    for (t2=0;t2<=floord(PB_P-1,32);t2++) {
      for (t3=0;t3<=floord(PB_N-1,32);t3++) {
        for (t4=max(2,32*t2);t4<=min(PB_P-1,32*t2+31);t4++) {
          for (t5=max(2,32*t3);t5<=min(PB_N-1,32*t3+31);t5++) {
            lbv=32*t1;
            ubv=min(PB_L-2,32*t1+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t5][t4][t6+1] = B[t5-1][t4][t6] + A[t5-2][t4][t6+1] - A[t5][t4-2][t6+1] + 4;;
            }
          }
        }
      }
...
kernel_3124111221_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
