...
void kernel_2112232211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  for (t1=1;t1<=PB_M-1;t1++) {
    A[t1] = B[t1] - B[t1] + C[0] - A[t1+1] * A[t1-1] * 6;;
  }
}
/* End of CLooG code */
...
kernel_2112232211_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
