...
void kernel_2112112111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_M-3,32);t2++) {
      for (t3=32*t2;t3<=min(PB_M-3,32*t2+31);t3++) {
        lbv=max(1,32*t1);
        ubv=min(PB_L-1,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+1][t4-1] = B[t3+2][t4] - A[t3][t4-1] * 4;;
        }
      }
    }
  }
...
kernel_2112112111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
