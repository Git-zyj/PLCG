...
void kernel_3222212211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
if (PB_M >= 2) {
  for (t1=1;t1<=PB_M-1;t1++) {
    A[t1] = C[t1] + A[t1-1] + 5;;
    if (t1 >= 3) {
      for (t2=0;t2<=floord(2*t1+PB_M-3,32);t2++) {
        lbp=max(ceild(t2,2),ceild(-t1+32*t2+1,32));
        ubp=min(min(floord(t1+PB_M-2,32),floord(32*t2+PB_M+30,64)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
        for (t3=lbp;t3<=ubp;t3++) {
          for (t4=max(max(2,32*t2-32*t3),32*t3-PB_M+1);t4<=min(min(t1-1,32*t3+28),32*t2-32*t3+31);t4++) {
            for (t5=max(32*t3,t4+3);t5<=min(32*t3+31,t4+PB_M-1);t5++) {
              B[1][(-t4+t5)-2][t1] = D[(-t4+t5)-1][t1][t4-2] - B[1][(-t4+t5)-3][t1-1] + B[1][(-t4+t5)-3][t1] + 6;;
            }
          }
        }
      }
...
kernel_3222212211_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
