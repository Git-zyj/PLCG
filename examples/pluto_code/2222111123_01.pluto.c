...
void kernel_2222111123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-1,32);t2++) {
      for (t3=32*t2;t3<=(min(32*t1,32*t2+31))-7;t3+=8) {
        lbv=max(1,32*t1);
...
#pragma vector always
      ...
      for (t3=max(32*t2,32*t1+1);t3<=min(min(PB_L-4,32*t2+31),-32*t1+PB_M-1);t3++) {
        if (t1 == 0) {
          B[t3+1][0] = C[0 +t3][0] - B[t3+3][0] - 6;;
        }
        lbv=max(1,32*t1);
        ubv=min(min(32*t1+31,t3-1),-t3+PB_M-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t4-1] = 3;;
          B[t3+1][t4] = C[t4+t3][0] - B[t3+3][t4] - 6;;
        }
        lbv=-t3+PB_M;
...
#pragma vector always
      ...
      for (t3=max(max(32*t2,PB_L-3),32*t1+1);t3<=min(min(PB_L-1,32*t2+31),-32*t1+PB_M-1);t3++) {
        lbv=max(1,32*t1);
        ubv=min(PB_L,32*t1+31);
#pragma ivdep
#pragma vector always
      ...
      for (t3=max(max(32*t2,-32*t1+PB_M),32*t1+1);t3<=min(PB_L-1,32*t2+31);t3++) {
        lbv=max(1,32*t1);
        ubv=min(PB_L,32*t1+31);
#pragma ivdep
#pragma vector always
...
kernel_2222111123_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
