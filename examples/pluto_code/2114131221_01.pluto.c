...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(min(floord(PB_L-2,32),floord(-32*t1+PB_P-1,32)),floord(PB_P-2,64)),t1);t2++) {
      for (t3=max(1,32*t2);t3<=min(min(min(min(floord(PB_P-2,2),PB_L-2),32*t1+30),32*t2+31),-32*t1+PB_P-1);t3++) {
        lbv=max(32*t1,t3+1);
        ubv=min(min(PB_L-1,32*t1+31),-t3+PB_P-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t4][t3-1] = B[0] * B[t3] * C[t4][t3+t4] - A[t3][t4] + A[t3][t4-2] - 5;;
        }
      }
    }
  }
