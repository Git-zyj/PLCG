...
void kernel_2112231213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t2;t3<=min(min(PB_M-2,32*t2+31),-32*t1+32*t2+PB_M-1);t3++) {
        for (t4=32*t1-32*t2;t4<=min(min(PB_M-2,32*t1-32*t2+31),-t3+PB_M-1);t4++) {
          A[t3][t4] = B[t3+t4][0] + B[t4][t3] * C[t3][t4+1] + A[t3+1][t4] + A[t3][t4+1] * 2;;
        }
      }
    }
  }
...
kernel_2112231213_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
