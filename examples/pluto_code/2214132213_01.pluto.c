...
void kernel_2214132213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_L >= 4) {
  if (PB_L >= 5) {
    for (t2=0;t2<=floord(3*PB_L-9,32);t2++) {
      lbp=ceild(2*t2,3);
      ubp=min(min(floord(PB_L-3,16),floord(32*t2+PB_L+28,64)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(2,32*t2-32*t3),32*t3-PB_L+3);t4<=min(min(PB_L-3,16*t3+15),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,2*t4);t5<=min(32*t3+31,t4+PB_L-3);t5++) {
            A[(-t4+t5)+1] = C[(-t4+t5)][t4] * A[(-t4+t5)] + A[(-t4+t5)+2] * 2;;
          }
        }
      }
    }
  ...
  for (t2=2;t2<=PB_L-2;t2++) {
    B[t2] = D[t2] + D[t2-2] - D[t2+1] * D[t2] * D[t2] * A[t2+1] - B[t2+1] + 5;;
  }
}
/* End of CLooG code */
...
kernel_2214132213_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
