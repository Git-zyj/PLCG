  ...
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    if ((PB_M >= 3) && (PB_P <= -1) && (t1 <= floord(PB_M-2,32))) {
      for (t4=max(1,32*t1);t4<=min(PB_M-2,32*t1+31);t4++) {
        A[t4] = C[t4-1] + A[t4+1] + 5;;
      }
    }
    for (t2=0;t2<=min(min(floord(PB_P,32),floord(-32*t1+PB_M+PB_P-2,32)),floord(PB_M+2*PB_P-3,64));t2++) {
      for (t3=0;t3<=min(floord(PB_M-1,32),floord(-64*t2+PB_M+2*PB_P-3,32));t3++) {
        if ((PB_P == 0) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(1,32*t1);t4<=min(PB_M-2,32*t1+31);t4++) {
            A[t4] = C[t4-1] + A[t4+1] + 5;;
          }
        ...
        if ((PB_M >= 2) && (PB_P >= 1) && (t1 >= ceild(PB_M-32,32)) && (t2 == 0) && (t3 == 0)) {
          for (t5=0;t5<=min(31,PB_P-1);t5++) {
            for (t6=0;t6<=min(31,PB_M-1);t6++) {
              B[(PB_M-1)][t5][t5] = 4;;
            }
