...
void kernel_2124232211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_N >= 5) {
  for (t1=2;t1<=PB_N-3;t1++) {
    A[t1+1] = B[t1-2] - B[t1] - B[t1+2] * A[t1+2] + A[t1] * 6;;
  }
}
/* End of CLooG code */
...
kernel_2124232211_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
