...
void kernel_2112131221_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if (PB_L >= 5) {
  for (t2=-PB_L+1;t2<=-3;t2++) {
    for (t4=2;t4<=min(PB_L-3,-t2-1);t4++) {
      A[t4-1][-t2][-t2] = B[-t2] + C[-t2+1][0] + C[t4][-t2] * A[t4-2][-t2][-t2] - A[-t2-1][t4+2][t4+2] - 1;;
    }
  }
}
/* End of CLooG code */
...
kernel_2112131221_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
