...
#pragma vector always
    for (t3=lbv;t3<=ubv;t3++) {
      A[t3-2] = C[t3] + C[t3] - C[t3] * D[t3][0] - B[t3-1] - 4;;
    }
  }
  if ((PB_M >= 7) && (PB_N >= 4)) {
    for (t2=0;t2<=min(min(floord(PB_M-3,16),floord(3*PB_N-5,32)),floord(PB_N+PB_M-3,32));t2++) {
      lbp=max(max(ceild(t2,2),ceild(32*t2-PB_M+4,32)),ceild(32*t2-PB_N+1,32));
      ubp=min(min(floord(2*t2+1,3),floord(PB_N-2,16)),floord(PB_M-2,32));
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(3,32*t2-32*t3),16*t3+1);t4<=min(min(min(PB_M-4,PB_N-1),32*t3+29),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4+2);t5<=min(min(PB_M-2,32*t3+31),2*t4-1);t5++) {
            B[(-t4+t5)-1] = C[(-t4+t5)+t4+1] - C[t4+2] + A[(-t4+t5)-2] - B[(-t4+t5)-2] + 5;;
          }
        }
      }
    }
