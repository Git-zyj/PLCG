...
if ((PB_L >= 1) && (PB_M >= 5)) {
  for (t1=2;t1<=PB_M-3;t1++) {
    for (t2=0;t2<=floord(PB_M+4*PB_L-6,32);t2++) {
      lbp=max(ceild(3*t2,4),ceild(32*t2-PB_L+1,32));
      ubp=min(min(floord(PB_M+3*PB_L-5,32),floord(96*t2+PB_M+91,128)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(ceild(32*t3-PB_M+2,3),32*t2-32*t3);t4<=min(min(floord(32*t3+31,3),PB_L-1),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,3*t4);t5<=min(32*t3+31,3*t4+PB_M-2);t5++) {
            A[t1][(-3*t4+t5)][(-3*t4+t5)] = B[(-3*t4+t5)][t1-2][t4] * A[t1-1][(-3*t4+t5)+1][(-3*t4+t5)+1] * A[(-3*t4+t5)+1][t1+2][t1+2] - 5;;
          }
        }
      }
    }
