...
void kernel_3124132113_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if (PB_L >= 3) {
  for (t1=0;t1<=PB_L-3;t1++) {
    for (t2=0;t2<=PB_L-1;t2++) {
      A[0] = B[t1][1] - B[t2][t1] + B[t2][t1+2] - A[0] * 5;;
    }
  }
}
/* End of CLooG code */
...
kernel_3124132113_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
