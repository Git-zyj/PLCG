...
void kernel_3222132111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_M >= 3) {
  A[0 +2] = C[0] - C[0] - C[0] - C[0] * C[0] * 1;;
  for (t1=1;t1<=PB_M-3;t1++) {
    A[t1+2] = C[t1] - C[t1] - C[t1] - C[t1] * C[t1] * 1;;
    B[t1][t1] = C[1] * B[t1+1][t1+1] - B[t1-1][t1-1] - 5;;
  }
  B[(PB_M-2)][(PB_M-2)] = C[1] * B[(PB_M-2)+1][(PB_M-2)+1] - B[(PB_M-2)-1][(PB_M-2)-1] - 5;;
}
/* End of CLooG code */
...
kernel_3222132111_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
