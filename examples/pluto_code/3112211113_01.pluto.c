...
void kernel_3112211113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_L >= 3) {
  for (t1=1;t1<=PB_L-2;t1++) {
    for (t2=t1;t2<=PB_L-1;t2++) {
      A[t1][t1+1] = B[t1-1][t2] + A[t2-1][t2] - 1;;
    }
  }
}
/* End of CLooG code */
...
kernel_3112211113_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
