...
void kernel_3114212123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_L >= 3) && (PB_M >= 2)) {
  for (t1=2;t1<=min(PB_M,PB_L-1);t1++) {
    for (t2=t1;t2<=PB_M;t2++) {
      A[t1][t1] = B[t2-1][t1] * A[t1-2][t1-2] + 5;;
    }
  }
}
/* End of CLooG code */
...
kernel_3114212123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
