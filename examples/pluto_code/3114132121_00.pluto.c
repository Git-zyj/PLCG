...
/* Start of CLooG code */
if ((PB_M >= 4) && (PB_P >= 4)) {
  for (t1=0;t1<=min(floord(PB_M-2,16),floord(PB_P-2,32));t1++) {
    lbp=ceild(t1,2);
    ubp=min(floord(PB_M-2,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t1-32*t2;t3<=min(min(min(min(floord(PB_P-2,2),PB_M-2),PB_P-4),32*t1-32*t2+31),-32*t2+PB_P-2);t3++) {
        for (t4=max(max(2,32*t2),t3);t4<=min(min(PB_M-2,32*t2+31),-t3+PB_P-2);t4++) {
          A[t3][t4+t3] = B[t4-2][t3][t4] - C[t4][t3] - C[t3][t4+1] + A[t4][t4+t3+1] * 4;;
        }
      }
    }
  }
