...
void kernel_2224211212_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
B[1] = B[1 -1] * B[1 +1] + 3;;
for (t1=2;t1<=min(PB_L-2,PB_N-1);t1++) {
  A[t1-1] = C[t1] * C[t1-1] - A[t1-2] - A[t1] - 3;;
  B[t1] = B[t1-1] * B[t1+1] + 3;;
}
for (t1=PB_L-1;t1<=PB_N-1;t1++) {
  A[t1-1] = C[t1] * C[t1-1] - A[t1-2] - A[t1] - 3;;
}
for (t1=PB_N;t1<=PB_L-2;t1++) {
  B[t1] = B[t1-1] * B[t1+1] + 3;;
}
/* End of CLooG code */
...
kernel_2224211212_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
