...
void kernel_2112232223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_L >= 3) {
  for (t2=-PB_L+1;t2<=-2;t2++) {
    A[-t2] = B[-t2] - C[-t2] + B[-t2-2] - A[-t2-2] * A[-t2-1] * 2;;
  }
}
/* End of CLooG code */
...
kernel_2112232223_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
