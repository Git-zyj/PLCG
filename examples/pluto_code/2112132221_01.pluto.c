...
/* Start of CLooG code */
if ((PB_L >= 6) && (PB_M >= 3)) {
  for (t1=0;t1<=floord(3*PB_L-11,32);t1++) {
    lbp=max(ceild(2*t1,3),ceild(32*t1-PB_L+5,32));
    ubp=min(min(min(floord(PB_L-3,16),floord(32*t1+PB_L+30,64)),floord(64*t1+PB_M+63,96)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(max(1,ceild(32*t2-PB_M-1,2)),32*t1-32*t2),32*t2-PB_L+1);t3<=min(min(PB_L-5,16*t2+13),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3+4);t4<=min(min(32*t2+31,t3+PB_L-1),2*t3+PB_M+1);t4++) {
          A[(-2*t3+t4)-2] = B[(-2*t3+t4)][t3] * B[(-2*t3+t4)][t3-1] - C[(-2*t3+t4)+t3] - A[(-2*t3+t4)-3] + A[(-2*t3+t4)-4] + 2;;
        }
      }
    }
  }
