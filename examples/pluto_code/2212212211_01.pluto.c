...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t3=0;t3<=floord(PB_M-3,32);t3++) {
      for (t4=32*t3;t4<=(min(PB_M-3,32*t3+31))-7;t4+=8) {
        lbv=32*t2;
        ubv=min(PB_M-1,32*t2+31);
#pragma ivdep
#pragma vector always
        for (t5=lbv;t5<=ubv;t5++) {
          A[t5] = C[t4+2][t5] * D[t5] + 1;;
          A[t5] = C[(t4+1)+2][t5] * D[t5] + 1;;
          A[t5] = C[(t4+2)+2][t5] * D[t5] + 1;;
          A[t5] = C[(t4+3)+2][t5] * D[t5] + 1;;
          A[t5] = C[(t4+4)+2][t5] * D[t5] + 1;;
          A[t5] = C[(t4+5)+2][t5] * D[t5] + 1;;
          A[t5] = C[(t4+6)+2][t5] * D[t5] + 1;;
          A[t5] = C[(t4+7)+2][t5] * D[t5] + 1;;
        }
      }
      for (;t4<=min(PB_M-3,32*t3+31);t4++) {
        lbv=32*t2;
        ubv=min(PB_M-1,32*t2+31);
#pragma ivdep
#pragma vector always
