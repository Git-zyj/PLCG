...
void kernel_3122132213_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_M-3,32),floord(PB_Q-1,32));t2++) {
      for (t3=2*t2;t3<=min(min(min(min(floord(PB_M-3,16),floord(3*PB_Q-3,32)),floord(PB_Q+PB_M-4,32)),floord(32*t2+PB_M+28,32)),floord(64*t2+PB_Q+61,32));t3++) {
        for (t4=max(2,32*t1);t4<=min(PB_Q-1,32*t1+31);t4++) {
          for (t5=max(max(ceild(32*t3-PB_Q+1,2),32*t2),32*t3-PB_M+3);t5<=min(min(min(PB_M-3,PB_Q-1),32*t2+31),16*t3+15);t5++) {
            for (t6=max(32*t3,2*t5);t6<=min(min(32*t3+31,t5+PB_M-3),2*t5+PB_Q-1);t6++) {
              A[t5+2][t4-2][(-2*t5+t6)] = B[t5][(-2*t5+t6)][t4][0] + C[(-2*t5+t6)+t5+2][t4][(-2*t5+t6)] - D[(-2*t5+t6)][t4] * A[(-2*t5+t6)+1][t4-2][t5] + A[t5+2][t4-2][(-2*t5+t6)+1] - 5;;
            }
          }
        }
      }
...
kernel_3122132213_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
