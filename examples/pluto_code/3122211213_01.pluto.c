...
void kernel_3122211213_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_M-3,32);t2++) {
      for (t3=0;t3<=min(min(floord(PB_M-3,32),floord(-32*t1+PB_M-2,32)),floord(-32*t2+PB_M-2,32));t3++) {
        for (t4=max(1,32*t1);t4<=min(min(PB_M-3,32*t1+31),-32*t3+PB_M-2);t4++) {
          for (t5=max(1,32*t3);t5<=min(min(32*t3+31,-32*t2+PB_M-2),-t4+PB_M-2);t5++) {
            lbv=max(1,32*t2);
            ubv=min(32*t2+31,-t5+PB_M-2);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t5+t6][t5+t4][t4] = B[t5-1][t4-1][0] + A[t5+t6+1][t5+t4+1][t4] * A[t5+t6-1][t5+t4-1][t4] * 4;;
            }
          }
        }
      }
...
kernel_3122211213_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
