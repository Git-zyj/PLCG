...
void kernel_3212112123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
      ...
      if ((PB_L >= 3) && (t2 == 0)) {
        for (t3=32*t1;t3<=min(PB_N-1,32*t1+31);t3++) {
          ...
          for (t4=1;t4<=min(31,PB_L-2);t4++) {
            for (t5=0;t5<=t4-1;t5++) {
              B[t3][t3][t4] = D[t5+2][t4][t3] * B[t3][t3][t4+1] + 6;;
            }
          }
        }
      }
...
kernel_3212112123_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
