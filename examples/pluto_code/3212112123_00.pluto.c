...
void kernel_3212112123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_M >= 4) {
  if (PB_L >= 0) {
    for (t1=0;t1<=floord(PB_M+PB_L-3,32);t1++) {
      for (t2=0;t2<=floord(PB_L,32);t2++) {
        for (t3=0;t3<=min(min(min(floord(PB_M-3,32),floord(-32*t1+PB_M+PB_L-3,32)),floord(-32*t2+PB_M+PB_L-4,32)),floord(-32*t1+3*PB_M+PB_L-11,96));t3++) {
          if ((t2 <= floord(PB_L-1,32)) && (t2 >= -t3+1)) {
            for (t4=32*t1;t4<=min(PB_L-2,32*t1+31);t4++) {
              for (t5=32*t2;t5<=min(PB_L-1,32*t2+31);t5++) {
                for (t6=max(1,32*t3);t6<=min(PB_M-3,32*t3+31);t6++) {
                  B[t4][t5] = C[t5+1][t4+1] + 1;;
                }
              }
            }
          }
          if ((PB_L >= 2) && (t1 == 0) && (t2 == 0) && (t3 == 0)) {
            for (t5=0;t5<=min(31,PB_L-1);t5++) {
              for (t6=1;t6<=min(31,PB_M-3);t6++) {
                B[0][t5] = C[t5+1][0 +1] + 1;;
              }
            }
          }
          if ((t2 == 0) && (t3 == 0)) {
            for (t4=max(1,32*t1);t4<=min(min(PB_L-2,PB_M-3),32*t1+31);t4++) {
              A[t4] = A[t4+2] * A[t4-1] + 5;;
              for (t5=0;t5<=min(31,PB_L-1);t5++) {
                for (t6=1;t6<=min(31,PB_M-3);t6++) {
                  B[t4][t5] = C[t5+1][t4+1] + 1;;
                }
          ...
          if ((t2 == 0) && (t3 == 0)) {
            for (t4=max(max(1,32*t1),PB_L-1);t4<=min(PB_M-3,32*t1+31);t4++) {
              A[t4] = A[t4+2] * A[t4-1] + 5;;
            }
          }
          if ((t2 == 0) && (t3 == 0)) {
            for (t4=max(32*t1,PB_M-2);t4<=min(PB_L-2,32*t1+31);t4++) {
              for (t5=0;t5<=min(31,PB_L-1);t5++) {
                for (t6=1;t6<=min(31,PB_M-3);t6++) {
                  B[t4][t5] = C[t5+1][t4+1] + 1;;
                }
  ...
  if (PB_L <= -1) {
    for (t1=0;t1<=floord(PB_M-3,32);t1++) {
      for (t4=max(1,32*t1);t4<=min(PB_M-3,32*t1+31);t4++) {
        A[t4] = A[t4+2] * A[t4-1] + 5;;
      }
    }
  }
...
kernel_3212112123_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
