...
if (PB_L >= 1) {
  if ((PB_L >= 2) && (PB_M >= 6)) {
    for (t2=max(-1,ceild(-PB_L-29,32));t2<=floord(PB_M-4,32);t2++) {
      lbp=max(0,t2);
      ubp=min(min(min(min(floord(PB_M-5,16),floord(PB_M+PB_L-6,32)),floord(32*t2+PB_L+29,32)),floord(32*t2+PB_M+26,32)),2*t2+2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(max(max(2,32*t2),16*t3+1),-32*t2+32*t3-30),32*t3-PB_L+2);t4<=min(min(PB_M-4,32*t2+62),32*t3+31);t4++) {
          lbv=max(max(32*t3,t4),-32*t2+32*t3+t4-31);
          ubv=min(min(min(32*t3+31,2*t4-1),-32*t2+32*t3+t4),t4+PB_L-2);
#pragma ivdep
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            A[t4+2][(-t4+t5)] = C[(-t4+t5)+2][t4-2] + C[t4][(-t4+t5)] + D[t4][(-t4+t5)+1] - A[t4+1][(-t4+t5)+1] * A[t4+3][(-t4+t5)] - B[t4+1] - 3;;
          }
        }
      }
    }
