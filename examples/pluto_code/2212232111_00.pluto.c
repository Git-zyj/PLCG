...
/* Start of CLooG code */
if (PB_M >= 4) {
  B[0 +2] = D[0 +2] * D[0] * D[0] + B[0 +3] * 4;;
}
for (t1=1;t1<=min(PB_L-2,PB_M-4);t1++) {
  A[t1] = C[t1][t1+1] * D[t1] + C[t1][t1-1] + B[t1+2] - 6;;
  B[t1+2] = D[t1+2] * D[t1] * D[t1] + B[t1+3] * 4;;
}
for (t1=max(1,PB_M-3);t1<=PB_L-2;t1++) {
  A[t1] = C[t1][t1+1] * D[t1] + C[t1][t1-1] + B[t1+2] - 6;;
}
for (t1=max(1,PB_L-1);t1<=PB_M-4;t1++) {
  B[t1+2] = D[t1+2] * D[t1] * D[t1] + B[t1+3] * 4;;
}
/* End of CLooG code */
