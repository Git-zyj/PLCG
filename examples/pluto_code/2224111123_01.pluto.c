...
void kernel_2224111123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_N >= 1) {
  for (t1=2;t1<=PB_N-1;t1++) {
    A[(t1-2)] = C[(t1-2)] * A[(t1-2)+2] - 6;;
    A[t1-2] = C[0] * 6;;
  }
  for (t1=max(2,PB_N);t1<=PB_N+1;t1++) {
    A[t1-2] = C[0] * 6;;
  }
}
/* End of CLooG code */
...
kernel_2224111123_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
