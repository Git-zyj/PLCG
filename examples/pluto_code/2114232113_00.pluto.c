...
void kernel_2114232113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if ((PB_L >= 3) && (PB_M >= 3)) {
  ...
  ubp=min(min(0,floord(PB_L-4,32)),floord(PB_M-4,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(t1,-t1-1);t2<=min(min(floord(PB_M-1,32),floord(-32*t1+PB_L-2,32)),floord(-32*t1+PB_M,64));t2++) {
      for (t3=max(max(1,32*t1+2),32*t1+32*t2);t3<=min(min(min(min(min(floord(PB_M,2),floord(32*t1+PB_M+31,2)),PB_L-2),-32*t2+PB_M),32*t2+31),32*t1+32*t2+62);t3++) {
        lbv=max(max(32*t2,t3),-32*t1+t3-31);
        ubv=min(min(-32*t1+t3,32*t2+31),-t3+PB_M);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+1][t4] = B[t4-1][t3] * B[t4+t3-1][0] - C[t4-2][t3] * A[t3][t4-1] * 1;;
        }
      }
    }
  }
...
kernel_2114232113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
