...
void kernel_2122131123_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (t2=0;t2<=floord(PB_L-1,32);t2++) {
      for (t3=32*t1;t3<=min(PB_L-1,32*t1+31);t3++) {
        for (t4=max(2,32*t2);t4<=min(PB_L-1,32*t2+31);t4++) {
          A[t3][t4] = B[t3+1] + C[t3][t4-1] + C[t4-1][t3] + A[t3][t4-2] - 3;;
        }
      }
    }
  }
...
kernel_2122131123_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
