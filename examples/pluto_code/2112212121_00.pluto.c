...
void kernel_2112212121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_N-5,32);t2++) {
      for (t3=32*t2;t3<=(min(PB_N-5,32*t2+31))-7;t3+=8) {
        lbv=32*t1;
        ubv=min(PB_N-1,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+2][t4] = B[t4] * A[t3+4][t4] * 1;;
          A[(t3+1)+2][t4] = B[t4] * A[(t3+1)+4][t4] * 1;;
          A[(t3+2)+2][t4] = B[t4] * A[(t3+2)+4][t4] * 1;;
          A[(t3+3)+2][t4] = B[t4] * A[(t3+3)+4][t4] * 1;;
          A[(t3+4)+2][t4] = B[t4] * A[(t3+4)+4][t4] * 1;;
          A[(t3+5)+2][t4] = B[t4] * A[(t3+5)+4][t4] * 1;;
          A[(t3+6)+2][t4] = B[t4] * A[(t3+6)+4][t4] * 1;;
          A[(t3+7)+2][t4] = B[t4] * A[(t3+7)+4][t4] * 1;;
        }
      }
      for (;t3<=min(PB_N-5,32*t2+31);t3++) {
        lbv=32*t1;
        ubv=min(PB_N-1,32*t1+31);
#pragma ivdep
#pragma vector always
...
kernel_2112212121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
