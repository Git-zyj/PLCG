...
void kernel_2222111113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= max(0,ceild(-2*PB_M+6,3))) && (PB_M >= 0)) {
  for (t1=0;t1<=floord(PB_L+PB_M-2,32);t1++) {
    for (t2=0;t2<=min(floord(PB_L,32),floord(3*PB_L+PB_M-3,96));t2++) {
      if ((PB_M >= 3) && (t1 <= min(floord(PB_L-2,32),floord(PB_M-2,32))) && (t2 == 0)) {
        if (t1 == 0) {
          A[0][0 +1] = C[0 +1] * 5;;
        }
        for (t4=max(1,32*t1);t4<=min(min(PB_L-2,PB_M-2),32*t1+31);t4++) {
          A[0][t4+1] = C[t4+1] * 5;;
          B[t4] = C[t4+1] * B[t4-1] - B[t4+1] * 1;;
        }
        for (t4=PB_M-1;t4<=min(PB_L-2,32*t1+31);t4++) {
          A[0][t4+1] = C[t4+1] * 5;;
        }
        for (t4=PB_L-1;t4<=min(PB_M-2,32*t1+31);t4++) {
          B[t4] = C[t4+1] * B[t4-1] - B[t4+1] * 1;;
        }
      }
      if ((PB_M <= 2) && (t1 <= floord(PB_L-2,32)) && (t2 == 0)) {
        for (t4=32*t1;t4<=min(PB_L-2,32*t1+31);t4++) {
          A[0][t4+1] = C[t4+1] * 5;;
        }
      }
      if ((PB_M >= 3) && (t1 <= floord(PB_L-2,32)) && (t1 >= ceild(PB_M-1,32)) && (t2 == 0)) {
        for (t4=32*t1;t4<=min(PB_L-2,32*t1+31);t4++) {
          A[0][t4+1] = C[t4+1] * 5;;
        }
      }
      if ((PB_M >= 3) && (t1 <= floord(PB_M-2,32)) && (t1 >= ceild(PB_L-1,32)) && (t2 == 0)) {
      ...
      if (t1 <= floord(PB_L-2,32)) {
        for (t3=max(1,32*t2);t3<=(min(PB_L-1,32*t2+31))-7;t3+=8) {
          for (t4=32*t1;t4<=min(PB_L-2,32*t1+31);t4++) {
            A[t3][t4+1] = C[t4+1] * 5;;
            A[(t3+1)][t4+1] = C[t4+1] * 5;;
            A[(t3+2)][t4+1] = C[t4+1] * 5;;
            A[(t3+3)][t4+1] = C[t4+1] * 5;;
            A[(t3+4)][t4+1] = C[t4+1] * 5;;
            A[(t3+5)][t4+1] = C[t4+1] * 5;;
            A[(t3+6)][t4+1] = C[t4+1] * 5;;
            A[(t3+7)][t4+1] = C[t4+1] * 5;;
          }
        }
...
kernel_2222111113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
