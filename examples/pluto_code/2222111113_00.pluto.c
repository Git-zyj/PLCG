...
void kernel_2222111113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (t2=0;t2<=floord(PB_L-1,32);t2++) {
      for (t3=32*t1;t3<=min(32*t2,32*t1+31);t3++) {
        for (t4=max(1,32*t2);t4<=min(PB_L-1,32*t2+31);t4++) {
          A[t3][t4-1] = C[t4][t3] + C[t3][t4] * A[t3][t4] * 3;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t4=1;t4<=min(31,PB_L-1);t4++) {
          A[1][t4-1] = C[t4][1] + C[1][t4] * A[1][t4] * 3;;
        }
      }
      for (t3=max(32*t1,32*t2+32);t3<=min(PB_L-1,32*t1+31);t3++) {
        for (t4=max(1,32*t2);t4<=32*t2+31;t4++) {
          A[t3][t4-1] = 1;;
        }
      }
      if (t1 == t2) {
        for (t3=max(2,32*t1+1);t3<=min(PB_L-1,32*t1+31);t3++) {
          for (t4=max(1,32*t1);t4<=t3-1;t4++) {
            A[t3][t4-1] = 1;;
          }
          for (t4=t3;t4<=min(PB_L-1,32*t1+31);t4++) {
            A[t3][t4-1] = C[t4][t3] + C[t3][t4] * A[t3][t4] * 3;;
          }
        }
      }
...
kernel_2222111113_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
