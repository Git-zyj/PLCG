...
/* Start of CLooG code */
if ((PB_L >= 0) && (PB_N >= max(0,ceild(-PB_L+5,5)))) {
  for (t1=0;t1<=min(floord(PB_N+PB_L+1,32),floord(7*PB_N+PB_L-5,32));t1++) {
    if ((PB_M <= -1) && (t1 <= floord(PB_L-5,32))) {
      for (t3=32*t1;t3<=min(PB_L-5,32*t1+31);t3++) {
        B[t3+2] = C[t3+1] + B[t3+4] * B[t3+1] + 4;;
      }
    }
    for (t2=0;t2<=min(min(floord(PB_M,32),floord(5*PB_M+PB_L-5,160)),floord(-32*t1+PB_N+6*PB_M+PB_L-5,192));t2++) {
...
#pragma vector always
      ...
      if ((PB_M >= 1) && (t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=min(1,PB_L-5);t3++) {
          B[t3+2] = C[t3+1] + B[t3+4] * B[t3+1] + 4;;
        }
      }
      if ((PB_M >= 1) && (t2 == 0)) {
        for (t3=max(2,32*t1);t3<=min(min(PB_L-5,PB_N+1),32*t1+31);t3++) {
          A[t3-2][0] = 5;;
          B[t3+2] = C[t3+1] + B[t3+4] * B[t3+1] + 4;;
          lbv=1;
          ubv=min(31,PB_M-1);
...
#pragma vector always
      ...
      if ((PB_M >= 1) && (t2 == 0)) {
        for (t3=max(max(2,32*t1),PB_L-4);t3<=min(PB_N+1,32*t1+31);t3++) {
          lbv=0;
          ubv=min(31,PB_M-1);
#pragma ivdep
#pragma vector always
      ...
      if ((PB_M >= 1) && (t2 == 0)) {
        for (t3=max(32*t1,PB_N+2);t3<=min(PB_L-5,32*t1+31);t3++) {
          B[t3+2] = C[t3+1] + B[t3+4] * B[t3+1] + 4;;
        }
      }
    }
...
if (PB_N <= -1) {
  for (t1=0;t1<=floord(PB_L-5,32);t1++) {
    for (t3=32*t1;t3<=min(PB_L-5,32*t1+31);t3++) {
      B[t3+2] = C[t3+1] + B[t3+4] * B[t3+1] + 4;;
    }
  }
