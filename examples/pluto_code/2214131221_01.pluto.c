...
void kernel_2214131221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
/* Start of CLooG code */
if (PB_P >= 5) {
  for (t1=0;t1<=floord(3*PB_P-5,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_P+1,32));
    ubp=min(floord(2*t1+1,3),floord(PB_P-2,16));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(4,32*t1-32*t2),16*t2+1);t3<=min(min(PB_P-1,32*t2+28),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+3);t4<=min(32*t2+31,2*t3-1);t4++) {
          A[t3-1][(-t3+t4)-1] = C[(-t3+t4)][t3] + D[t3+1][(-t3+t4)][t3] - C[(-t3+t4)][(-t3+t4)] - A[t3-3][(-t3+t4)-3] + A[t3+1][(-t3+t4)-3] * 3;;
          B[(-t3+t4)][(-t3+t4)+1] = C[t3][(-t3+t4)] + E[(-t3+t4)] * B[(-t3+t4)+1][(-t3+t4)+2] * 5;;
        }
      }
    }
  }
...
kernel_2214131221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
