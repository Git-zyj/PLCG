...
void kernel_2122112223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
    ...
    for (t2=0;t2<=floord(PB_M-1,32);t2++) {
      for (t3=32*t1;t3<=(min(PB_N-1,32*t1+31))-7;t3+=8) {
        for (t4=max(1,32*t2);t4<=min(PB_M-1,32*t2+31);t4++) {
          A[t3][t4] = B[t4][0] * A[t3][t4+2] - A[t3][t4-1] - 5;;
          A[(t3+1)][t4] = B[t4][0] * A[(t3+1)][t4+2] - A[(t3+1)][t4-1] - 5;;
          A[(t3+2)][t4] = B[t4][0] * A[(t3+2)][t4+2] - A[(t3+2)][t4-1] - 5;;
          A[(t3+3)][t4] = B[t4][0] * A[(t3+3)][t4+2] - A[(t3+3)][t4-1] - 5;;
          A[(t3+4)][t4] = B[t4][0] * A[(t3+4)][t4+2] - A[(t3+4)][t4-1] - 5;;
          A[(t3+5)][t4] = B[t4][0] * A[(t3+5)][t4+2] - A[(t3+5)][t4-1] - 5;;
          A[(t3+6)][t4] = B[t4][0] * A[(t3+6)][t4+2] - A[(t3+6)][t4-1] - 5;;
          A[(t3+7)][t4] = B[t4][0] * A[(t3+7)][t4+2] - A[(t3+7)][t4-1] - 5;;
        }
      }
...
kernel_2122112223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
