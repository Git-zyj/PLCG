...
void kernel_2122112221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_M >= 1) {
  for (t2=2;t2<=PB_M+1;t2++) {
    for (t4=-PB_M+1;t4<=0;t4++) {
      A[-t4][t2] = B[-t4][t2] * A[-t4][t2+1] + A[t2-2][-t4] - 4;;
    }
  }
}
/* End of CLooG code */
...
kernel_2122112221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
