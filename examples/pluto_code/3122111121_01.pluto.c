...
if ((PB_L >= 4) && (PB_M >= 2)) {
  lbp=0;
  ubp=min(floord(PB_L-3,32),floord(PB_M-1,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(min(floord(PB_M-2,32),floord(-32*t1+PB_L-3,32)),floord(PB_L-4,64)),t1);t2++) {
      for (t3=max(32*t1,32*t2+1);t3<=min(min(PB_M-1,32*t1+31),-32*t2+PB_L-3);t3++) {
        for (t4=32*t2;t4<=min(min(32*t2+31,t3-1),-t3+PB_L-3);t4++) {
          A[t3][t4+t3] = B[t4][t3+1] + A[t3][t4+t3+2] + 2;;
        }
      }
    }
  }
