...
if (PB_L >= 4) {
  for (t1=0;t1<=floord(PB_L-3,32);t1++) {
    for (t2=0;t2<=floord(PB_L-2,32);t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=1;t3<=min(30,PB_L-3);t3++) {
          A[t3+2] = C[t3] + 5;;
          for (t4=t3+1;t4<=min(31,PB_L-2);t4++) {
            B[(-t3+t4)+t3][(-t3+t4)] = C[(-t3+t4)-1] * D[(-t3+t4)][t3+1] * C[(-t3+t4)] - E[(-t3+t4)][t3][t3] + C[t3+2] + B[(-t3+t4)+t3+1][(-t3+t4)+1] * B[(-t3+t4)+t3-1][(-t3+t4)-1] * B[(-t3+t4)+t3][(-t3+t4)+1] - 3;;
          }
        }
      }
      if (t2 == 0) {
        for (t3=max(31,32*t1);t3<=min(PB_L-3,32*t1+31);t3++) {
          A[t3+2] = C[t3] + 5;;
        }
      }
      if (t2 >= 1) {
