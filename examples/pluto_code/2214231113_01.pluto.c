...
void kernel_2214231113_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
  int t1, t2;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
for (t1=1;t1<=min(PB_M,PB_L-1);t1++) {
  A[t1] = C[t1] * D[t1] - C[t1] + C[0] + A[t1-1] + 1;;
  A[(t1-1)] = C[0] - 5;;
}
for (t1=max(1,PB_M+1);t1<=PB_L-1;t1++) {
  A[t1] = C[t1] * D[t1] - C[t1] + C[0] + A[t1-1] + 1;;
}
for (t1=max(1,PB_L);t1<=PB_M;t1++) {
  A[(t1-1)] = C[0] - 5;;
}
/* End of CLooG code */
...
kernel_2214231113_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
