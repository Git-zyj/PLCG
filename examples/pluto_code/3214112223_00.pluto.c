...
void kernel_3214112223_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
  ...
  for (t1=0;t1<=floord(PB_L-1,32);t1++) {
    if (PB_P <= -1) {
      for (t4=max(4,32*t1);t4<=min(PB_L-1,32*t1+31);t4++) {
        B[t4] = C[t4] - C[t4] * B[t4-2] + 1;;
      }
    }
    for (t2=0;t2<=floord(PB_P,32);t2++) {
      for (t3=0;t3<=min(min(floord(PB_L-2,32),floord(-96*t2+PB_L+3*PB_P-5,32)),t1);t3++) {
        if ((PB_P >= 1) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(4,32*t1);t4<=min(PB_L-1,32*t1+31);t4++) {
            A[t4-2][0][0] = A[t4][0][0 +2] - A[t4-4][0][0] - 3;;
            B[t4] = C[t4] - C[t4] * B[t4-2] + 1;;
            lbv=1;
            ubv=min(31,t4-1);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4-2][0][t6] = A[t4][0][t6+2] - A[t4-4][0][t6] - 3;;
            }
            for (t5=1;t5<=min(31,PB_P-1);t5++) {
              lbv=0;
              ubv=min(31,t4-1);
#pragma ivdep
#pragma vector always
              for (t6=lbv;t6<=ubv;t6++) {
                A[t4-2][t5][t6] = A[t4][t5][t6+2] - A[t4-4][t5][t6] - 3;;
              }
            }
          }
        }
        if ((t2 <= floord(PB_P-1,32)) && (t2 >= -t3+1)) {
          for (t4=max(max(4,32*t1),32*t3+1);t4<=min(PB_L-1,32*t1+31);t4++) {
...
kernel_3214112223_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
