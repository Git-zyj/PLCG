...
void kernel_2124112113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_P,32);t2++) {
      for (t3=32*t1;t3<=min(PB_L-1,32*t1+31);t3++) {
        for (t4=max(2,32*t2);t4<=min(PB_P,32*t2+31);t4++) {
          A[t3][t4-2] = B[t3+1] * A[t3][t4-1] * 3;;
        }
      }
    }
  }
...
kernel_2124112113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
