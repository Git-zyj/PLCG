...
void kernel_3112211113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(floord(PB_M-2,32),floord(PB_P-3,32)),t1);t2++) {
      for (t3=32*t2;t3<=min(min(min(PB_M-2,PB_P-3),32*t1+30),32*t2+31);t3++) {
        lbv=max(32*t1,t3+1);
        ubv=min(PB_P-2,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4+1] = B[t3+1] - A[t3+1][t4+1] - 5;;
        }
      }
    }
  }
...
kernel_3112211113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
