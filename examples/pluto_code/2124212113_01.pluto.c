...
void kernel_2124212113_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(2,32*t2);t3<=min(PB_L-2,32*t2+31);t3++) {
        lbv=32*t1-32*t2;
        ubv=min(t3,32*t1-32*t2+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t4] = B[t3-2][t4] - A[t3+1] * 3;;
        }
      }
    }
  }
...
kernel_2124212113_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
