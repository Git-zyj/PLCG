...
void kernel_3222112213_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  ...
  for (t1=3;t1<=PB_M-1;t1++) {
    B[t1-1] = C[t1] - C[t1] + 4;;
    B[(t1-2)] = B[(t1-2)+1] - B[(t1-2)-1] - 6;;
  }
  if (PB_M >= 3) {
    B[(PB_M-2)] = B[(PB_M-2)+1] - B[(PB_M-2)-1] - 6;;
  }
}
/* End of CLooG code */
...
kernel_3222112213_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
