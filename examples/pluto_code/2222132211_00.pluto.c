...
void kernel_2222132211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_L >= 2) {
  A[0] = C[0] * C[0] * A[0 +1] + B[0 +1] + 4;;
  for (t1=1;t1<=PB_L-2;t1++) {
    A[t1] = C[t1] * C[t1] * A[t1+1] + B[t1+1] + 4;;
    B[t1] = C[t1] * C[t1] - C[t1] + B[t1-1] * 2;;
  }
  B[(PB_L-1)] = C[(PB_L-1)] * C[(PB_L-1)] - C[(PB_L-1)] + B[(PB_L-1)-1] * 2;;
}
/* End of CLooG code */
...
kernel_2222132211_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
