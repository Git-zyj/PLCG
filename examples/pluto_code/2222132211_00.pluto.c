...
void kernel_2222132211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
  ...
  for (t1=0;t1<=floord(PB_M-2,32);t1++) {
    if (PB_P <= -1) {
      for (t3=max(1,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
        B[t3] = C[t3] * B[t3+1] * B[t3-1] * B[t3-1] - 6;;
      }
    }
    if (PB_P >= 0) {
      for (t2=0;t2<=min(min(floord(32*t1+PB_P+29,32),floord(128*t1+33*PB_P,128)),floord(64*t1+29*PB_M+31*PB_P-87,992));t2++) {
        if ((PB_P >= 4) && (t2 >= 1)) {
          for (t3=max(32*t1,32*t2-PB_P+2);t3<=min(min(PB_M-3,32*t1+31),32*t2+29);t3++) {
            for (t4=max(32*t2,t3+2);t4<=min(32*t2+31,t3+PB_P-2);t4++) {
              A[(-t3+t4)+1][(-t3+t4)] = C[(-t3+t4)+1] * C[t3+2] - D[t3][(-t3+t4)] * D[t3][t3] - D[t3][(-t3+t4)-2] - A[(-t3+t4)+2][(-t3+t4)+1] - 4;;
            }
          }
        }
        if ((PB_P >= 4) && (t1 == 0) && (t2 == 0)) {
          for (t4=2;t4<=min(31,PB_P-2);t4++) {
            A[t4+1][t4] = C[t4+1] * C[0 +2] - D[0][t4] * D[0][0] - D[0][t4-2] - A[t4+2][t4+1] - 4;;
          }
        }
        if ((PB_P <= 3) && (t2 == 0)) {
          for (t3=max(1,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
        ...
        if ((PB_P >= 4) && (t1 == 0) && (t2 == 0)) {
          for (t3=1;t3<=min(29,PB_M-3);t3++) {
            B[t3] = C[t3] * B[t3+1] * B[t3-1] * B[t3-1] - 6;;
            for (t4=t3+2;t4<=min(31,t3+PB_P-2);t4++) {
              A[(-t3+t4)+1][(-t3+t4)] = C[(-t3+t4)+1] * C[t3+2] - D[t3][(-t3+t4)] * D[t3][t3] - D[t3][(-t3+t4)-2] - A[(-t3+t4)+2][(-t3+t4)+1] - 4;;
            }
          }
        }
        if ((PB_P >= 4) && (t2 == 0)) {
          for (t3=max(30,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
            B[t3] = C[t3] * B[t3+1] * B[t3-1] * B[t3-1] - 6;;
          }
        }
        if ((PB_M <= 31) && (PB_P >= 4) && (t1 == 0) && (t2 == 0)) {
          B[(PB_M-2)] = C[(PB_M-2)] * B[(PB_M-2)+1] * B[(PB_M-2)-1] * B[(PB_M-2)-1] - 6;;
        }
      }
    }
  }
...
kernel_2222132211_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
