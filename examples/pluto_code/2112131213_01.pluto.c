...
void kernel_2112131213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
/* Start of CLooG code */
if (PB_M >= 3) {
  for (t2=-PB_M+2;t2<=-1;t2++) {
    A[-t2] = B[-t2-1] + B[-t2] * B[-t2] - A[-t2+1] - A[-t2-1] * 3;;
  }
}
/* End of CLooG code */
...
kernel_2112131213_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
