...
void kernel_3122111123_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
    ...
    for (t2=0;t2<=floord(PB_L-2,32);t2++) {
      for (t3=max(1,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
        for (t4=max(1,32*t2);t4<=min(PB_L-2,32*t2+31);t4++) {
          A[t3-1][t4+1][t4] = B[t3][t4] * A[t3-1][t4][t4-1] - 3;;
        }
      }
    }
  }
...
kernel_3122111123_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
