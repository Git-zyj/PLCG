...
void kernel_3114211213_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5, t6;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_M >= 1) && (PB_P >= 2)) {
  lbp=0;
  ubp=floord(PB_M-1,32);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-2,32);t2++) {
      for (t3=0;t3<=min(floord(PB_P-2,32),floord(-32*t1+PB_M-1,32));t3++) {
        for (t4=32*t1;t4<=min(32*t1+31,-32*t3+PB_M-1);t4++) {
          for (t5=32*t2;t5<=min(PB_L-2,32*t2+31);t5++) {
            for (t6=32*t3;t6<=min(min(PB_P-2,32*t3+31),-t4+PB_M-1);t6++) {
              A[t4][t5][t6] = B[t6+t4][t5] + A[t4][t5+1][t6+1] + A[t4][t5][t6+1] * 4;;
            }
          }
        }
      }
...
kernel_3114211213_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
