...
void kernel_2214232213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
/* Start of CLooG code */
if (PB_L >= 2) {
  for (t1=1;t1<=min(2,PB_L-1);t1++) {
    A[t1] = C[t1] + A[t1+1] - A[t1-1] * 3;;
  }
  for (t1=3;t1<=PB_L-1;t1++) {
    A[t1] = C[t1] + A[t1+1] - A[t1-1] * 3;;
    A[(t1-1)] = D[(t1-1)] * C[(t1-1)] - C[(t1-1)-2] + C[(t1-1)] + C[(t1-1)] * 3;;
  }
  if (PB_L >= 3) {
    A[(PB_L-1)] = D[(PB_L-1)] * C[(PB_L-1)] - C[(PB_L-1)-2] + C[(PB_L-1)] + C[(PB_L-1)] * 3;;
  }
}
/* End of CLooG code */
...
kernel_2214232213_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
