...
void kernel_3114211211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_N >= 5) {
  lbp=0;
  ubp=floord(PB_N-2,64);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=min(floord(PB_N-3,32),floord(-32*t1+PB_N-2,32));t2++) {
      for (t3=max(2,32*t2);t3<=min(min(PB_N-3,32*t2+31),-32*t1+PB_N-2);t3++) {
        lbv=max(1,32*t1);
        ubv=min(min(t3,32*t1+31),-t3+PB_N-2);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+t4][t3-1] = B[t3][t4] * A[t3+t4+1][t3] + A[t3+t4-1][t3-2] * 4;;
        }
      }
    }
  }
...
kernel_3114211211_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
