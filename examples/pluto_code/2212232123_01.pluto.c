...
void kernel_2212232123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
/* Start of CLooG code */
if ((PB_L >= 0) && (PB_M >= 0)) {
  for (t1=0;t1<=min(floord(PB_M+PB_L-2,32),floord(2*PB_M+3*PB_L-6,64));t1++) {
    for (t2=0;t2<=floord(PB_L,32);t2++) {
      if (t2 == 0) {
        for (t3=32*t1;t3<=min(min(PB_L-2,PB_M-3),32*t1+31);t3++) {
          A[t3] = C[t3] + C[t3] * D[t3] + A[t3+2] + 2;;
          lbv=2;
          ubv=min(31,PB_L);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t3][t4-2] = E[t3][t4-1] - E[t4+2][t3] + E[t4+1][t3+1] - B[t3+2][t4-2] - 5;;
          }
        }
      }
      if (t2 == 0) {
        for (t3=max(32*t1,PB_L-1);t3<=min(PB_M-3,32*t1+31);t3++) {
          A[t3] = C[t3] + C[t3] * D[t3] + A[t3+2] + 2;;
        }
      }
      if (t2 >= 1) {
...
#pragma vector always
      ...
      if (t2 == 0) {
        for (t3=max(32*t1,PB_M-2);t3<=min(PB_L-2,32*t1+31);t3++) {
          lbv=2;
          ubv=min(31,PB_L);
#pragma ivdep
#pragma vector always
...
kernel_2212232123_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
