...
void kernel_2212111123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_N >= 5) {
  for (t1=0;t1<=floord(PB_N-2,32);t1++) {
    for (t2=0;t2<=min(floord(3*PB_N-4,32),floord(64*t1+PB_N+62,32));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=3;t3<=min(14,PB_N-2);t3++) {
          A[t3] = 4;;
          lbp=2*t3+3;
          ubp=min(31,2*t3+PB_N);
#pragma omp parallel for private(lbv,ubv)
          for (t4=lbp;t4<=ubp;t4++) {
            B[t3-1][(-2*t3+t4)-1] = C[(-2*t3+t4)-1][t3] - C[t3+1][(-2*t3+t4)] * B[t3-3][(-2*t3+t4)] - B[t3][(-2*t3+t4)-3] - 6;;
          }
        }
      }
      if (t2 == 0) {
      ...
      if (t2 >= 1) {
        for (t3=max(max(3,ceild(32*t2-PB_N,2)),32*t1);t3<=min(min(PB_N-2,32*t1+31),16*t2+14);t3++) {
          lbp=max(32*t2,2*t3+3);
          ubp=min(32*t2+31,2*t3+PB_N);
#pragma omp parallel for private(lbv,ubv)
          for (t4=lbp;t4<=ubp;t4++) {
            B[t3-1][(-2*t3+t4)-1] = C[(-2*t3+t4)-1][t3] - C[t3+1][(-2*t3+t4)] * B[t3-3][(-2*t3+t4)] - B[t3][(-2*t3+t4)-3] - 6;;
          }
...
kernel_2212111123_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
