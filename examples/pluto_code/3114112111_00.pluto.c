...
void kernel_3114112111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 3)) {
  for (t1=0;t1<=min(floord(PB_M-2,16),floord(PB_L+PB_M-4,32));t1++) {
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(1,32*t2);t3<=min(PB_M-2,32*t2+31);t3++) {
        for (t4=max(1,32*t1-32*t2);t4<=min(min(t3,PB_L-2),32*t1-32*t2+31);t4++) {
          A[t3] = B[t3+1][t4+1] - A[t4-1] * 5;;
        }
      }
    }
  }
...
kernel_3114112111_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
