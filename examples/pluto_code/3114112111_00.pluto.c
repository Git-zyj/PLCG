...
void kernel_3114112111_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_M-1,32);t2++) {
      for (t3=0;t3<=floord(PB_M-2,32);t3++) {
        for (t4=32*t1;t4<=min(min(PB_L-2,PB_M-1),32*t1+31);t4++) {
...
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][t5-1][t6] = B[t6][t5+1][t4+1] - A[t4][t5-2][t6] - 5;;
            }
          }
        }
      }
...
kernel_3114112111_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
