...
void kernel_2114231213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if ((PB_L >= 1) && (PB_M >= 3)) {
  for (t1=0;t1<=min(floord(3*PB_M-6,32),floord(2*PB_L+PB_M-4,32));t1++) {
    lbp=max(ceild(2*t1,3),ceild(32*t1-PB_L+1,32));
    ubp=min(min(min(floord(PB_M-2,16),floord(PB_L+PB_M-3,32)),floord(32*t1+PB_M+29,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_M+2);t3<=min(min(min(PB_L-1,PB_M-2),16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(max(32*t2,2*t3),t3+1);t4<=min(32*t2+31,t3+PB_M-2);t4++) {
          A[(-t3+t4)] = B[(-t3+t4)] * C[(-t3+t4)+1] + C[(-t3+t4)] + A[(-t3+t4)-1] * A[(-t3+t4)+1] * 2;;
        }
      }
    }
  }
...
kernel_2114231213_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
