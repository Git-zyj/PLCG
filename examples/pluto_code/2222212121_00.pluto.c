...
void kernel_2222212121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_L >= 3) {
  if (PB_M >= 0) {
    for (t1=0;t1<=floord(PB_L+PB_M-1,32);t1++) {
      for (t2=0;t2<=min(min(min(floord(PB_L-1,32),floord(PB_L+PB_M-3,32)),floord(-32*t1+PB_L+PB_M-1,32)),floord(-32*t1+2*PB_L+PB_M-4,64));t2++) {
        if (t2 >= 1) {
          for (t3=32*t1;t3<=min(PB_M-2,32*t1+31);t3++) {
            for (t4=32*t2;t4<=min(PB_L-1,32*t2+31);t4++) {
              B[t3] = C[t3+1][2] - D[t3][t4] - 3;;
            }
          }
        }
        if ((t1 == 0) && (t2 == 0)) {
          for (t3=0;t3<=min(1,PB_M-2);t3++) {
            for (t4=2;t4<=min(31,PB_L-1);t4++) {
              B[t3] = C[t3+1][2] - D[t3][t4] - 3;;
            }
          }
        }
        if (t2 == 0) {
          for (t3=max(2,32*t1);t3<=min(min(PB_L-1,PB_M-2),32*t1+31);t3++) {
            A[t3-1] = A[t3-2] - 1;;
            for (t4=2;t4<=min(31,PB_L-1);t4++) {
              B[t3] = C[t3+1][2] - D[t3][t4] - 3;;
            }
        ...
        if (t2 == 0) {
          for (t3=max(max(2,32*t1),PB_M-1);t3<=min(PB_L-1,32*t1+31);t3++) {
            A[t3-1] = A[t3-2] - 1;;
          }
        }
        if (t2 == 0) {
          for (t3=max(PB_L,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
            for (t4=2;t4<=min(31,PB_L-1);t4++) {
              B[t3] = C[t3+1][2] - D[t3][t4] - 3;;
            }
          }
...
kernel_2222212121_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
