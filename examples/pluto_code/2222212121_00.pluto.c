...
void kernel_2222212121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_L-3,32),t1);t2++) {
      if (t1 <= floord(PB_L-3,32)) {
        for (t3=32*t2;t3<=min(min(PB_L-3,32*t1+30),32*t2+31);t3++) {
          if (t1 == t2) {
            A[t3][t3][0] = C[t3][t3+2] + 2;;
          }
          lbv=max(32*t1,t3+1);
          ubv=min(PB_L-3,32*t1+31);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t4][t3][0] = C[t3][t4+2] + 2;;
            B[(t4-1)] = C[t3][(t4-1)] - A[t3][(t4-1)+1][0] + A[(t4-1)+1][t3+2][0] * 1;;
          }
          if (t1 >= ceild(PB_L-33,32)) {
            B[(PB_L-3)] = C[t3][(PB_L-3)] - A[t3][(PB_L-3)+1][0] + A[(PB_L-3)+1][t3+2][0] * 1;;
          }
        }
      }
      if ((t1 == t2) && (t1 <= floord(PB_L-34,32))) {
        A[(32*t1+31)][(32*t1+31)][0] = C[(32*t1+31)][(32*t1+31)+2] + 2;;
      }
      if (32*t1 == PB_L-2) {
        for (t3=32*t2;t3<=32*t2+31;t3++) {
          if ((PB_L+30)%32 == 0) {
            B[(PB_L-3)] = C[t3][(PB_L-3)] - A[t3][(PB_L-3)+1][0] + A[(PB_L-3)+1][t3+2][0] * 1;;
          }
        }
...
kernel_2222212121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
