    ...
    for (t2=0;t2<=min(floord(PB_M-1,16),floord(32*t1+PB_M+30,32));t2++) {
      if (t2 == 0) {
        for (t3=max(32,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
          A[t3] = C[t3] * A[t3+1] - A[t3-1] + A[t3-1] - 4;;
        }
      }
      if (t2 >= 1) {
        for (t3=max(32*t1,32*t2-PB_M+1);t3<=min(min(PB_M-1,32*t1+31),32*t2+31);t3++) {
          for (t4=max(32*t2,t3);t4<=min(32*t2+31,t3+PB_M-1);t4++) {
            B[(-t3+t4)] = C[t3] * C[(-t3+t4)] + C[(-t3+t4)] + C[(-t3+t4)] * B[(-t3+t4)+1] - 3;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t4=0;t4<=min(31,PB_M-1);t4++) {
          B[t4] = C[0] * C[t4] + C[t4] + C[t4] * B[t4+1] - 3;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
      ...
      if ((PB_M >= 2) && (PB_M <= 32) && (t1 == 0) && (t2 == 0)) {
        for (t4=PB_M-1;t4<=min(31,2*PB_M-2);t4++) {
          B[(t4-PB_M+1)] = C[(PB_M-1)] * C[(t4-PB_M+1)] + C[(t4-PB_M+1)] + C[(t4-PB_M+1)] * B[(t4-PB_M+1)+1] - 3;;
        }
      }
    }
  }
