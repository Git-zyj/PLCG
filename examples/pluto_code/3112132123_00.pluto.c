...
void kernel_3112132123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if ((PB_M >= 3) && (PB_Q >= 3)) {
  for (t1=0;t1<=floord(2*PB_Q+PB_M-3,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_Q+1,32));
    ubp=min(min(floord(PB_Q+PB_M-2,32),floord(32*t1+PB_M+30,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t1-32*t2),32*t2-PB_M+1);t3<=min(min(PB_Q-1,32*t2+29),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+2);t4<=min(32*t2+31,t3+PB_M-1);t4++) {
          A[(-t3+t4)] = B[t3][(-t3+t4)][t3-2] * B[t3][(-t3+t4)-2][t3] - C[t3] + A[(-t3+t4)-1] + 2;;
        }
      }
    }
  }
...
kernel_3112132123_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
