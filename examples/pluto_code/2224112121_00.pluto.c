...
void kernel_2224112121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 0) && (PB_M >= max(0,-5*PB_L+5))) {
  for (t1=0;t1<=min(floord(PB_M+PB_L-1,32),floord(PB_M+3*PB_L-3,32));t1++) {
    if ((PB_M >= 5) && (PB_P <= -1) && (t1 <= floord(PB_M-3,32))) {
      for (t3=max(2,32*t1);t3<=min(PB_M-3,32*t1+31);t3++) {
        A[t3] = C[t3] - C[t3-2] + A[t3+2] - A[t3+1] - 2;;
      }
    }
    for (t2=0;t2<=min(min(floord(PB_P,32),floord(-32*t1+PB_M+PB_L+2*PB_P-3,64)),floord(PB_M+5*PB_P-5,160));t2++) {
      if ((PB_P == 0) && (t2 == 0)) {
        for (t3=max(2,32*t1);t3<=min(PB_M-3,32*t1+31);t3++) {
          A[t3] = C[t3] - C[t3-2] + A[t3+2] - A[t3+1] - 2;;
        }
...
#pragma vector always
      ...
      if ((PB_P >= 1) && (t2 == 0)) {
        for (t3=max(2,32*t1);t3<=min(min(PB_L-1,PB_M-3),32*t1+31);t3++) {
          B[t3][0] = 5;;
          A[t3] = C[t3] - C[t3-2] + A[t3+2] - A[t3+1] - 2;;
          lbv=1;
          ubv=min(31,PB_P-1);
...
#pragma vector always
      ...
      if ((PB_P >= 1) && (t2 == 0)) {
        for (t3=max(max(2,PB_L),32*t1);t3<=min(PB_M-3,32*t1+31);t3++) {
          A[t3] = C[t3] - C[t3-2] + A[t3+2] - A[t3+1] - 2;;
        }
      }
      if ((PB_P >= 1) && (t2 == 0)) {
...
kernel_2224112121_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
