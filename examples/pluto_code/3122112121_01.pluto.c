...
void kernel_3122112121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if (PB_L >= 2) {
  for (t2=ceild(-PB_L-30,32);t2<=-1;t2++) {
    lbp=0;
    ubp=floord(32*t2+PB_L+30,32);
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t5=max(32*t2,32*t4-PB_L+1);t5<=32*t2+31;t5++) {
        lbv=32*t4;
        ubv=min(32*t4+31,t5+PB_L-1);
#pragma ivdep
#pragma vector always
        for (t7=lbv;t7<=ubv;t7++) {
          A[t7][t7+-t5] = B[t7+-t5+1][-t5] - A[t7][t7+-t5-1] - 3;;
        }
      }
    }
  }
...
kernel_3122112121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
