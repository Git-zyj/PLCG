...
if (PB_M >= 2) {
  for (t2=0;t2<=PB_M-2;t2++) {
    A[t2] = C[t2] * C[t2+1] + D[t2] * A[t2+1] + 1;;
  }
  if (PB_M >= 4) {
    lbp=0;
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=t2;t3<=min(floord(PB_M-2,32),floord(-32*t2+PB_M-1,32));t3++) {
        for (t4=max(1,32*t2);t4<=min(min(min(floord(PB_M-2,2),32*t2+31),32*t3+30),-32*t3+PB_M-1);t4++) {
          for (t5=max(32*t3,t4+1);t5<=min(32*t3+31,-t4+PB_M-1);t5++) {
            A[t4] = E[t4+t5][t5] * E[t4-1][t5+1] - 5;;
          }
        }
      }
    }
