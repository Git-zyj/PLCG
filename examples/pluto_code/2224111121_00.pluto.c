...
void kernel_2224111121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if ((PB_M >= 2) && (PB_N >= 1)) {
  lbp=0;
  ubp=min(floord(PB_N+PB_M-2,32),floord(2*PB_N+PB_M-4,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,ceild(32*t1-PB_M-31,32));t2<=min(floord(32*t1+PB_M-2,32),floord(-32*t1+2*PB_N+PB_M-4,32));t2++) {
      if (t1 == t2) {
        for (t3=32*t1;t3<=min(min(PB_M-2,PB_N-2),32*t1+29);t3++) {
          lbv=32*t1;
          ubv=t3;
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t4][t3] = D[t3] - B[t4][t3+1] * 5;;
          }
          lbv=t3+2;
          ubv=min(32*t1+31,t3+PB_M);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3][(-t3+t4)-1] = C[(-t3+t4)+1][t3] + A[t3+1][(-t3+t4)-2] + 6;;
          }
        }
      }
      for (t3=max(32*t2,32*t1-PB_M);t3<=min(min(PB_N-2,32*t1-1),32*t2+31);t3++) {
        lbv=max(32*t1,t3+2);
        ubv=min(32*t1+31,t3+PB_M);
#pragma ivdep
#pragma vector always
      ...
      if (t1 == t2) {
        for (t3=max(32*t1,PB_M-1);t3<=min(PB_N-2,32*t1+29);t3++) {
          lbv=t3+2;
          ubv=min(32*t1+31,t3+PB_M);
#pragma ivdep
#pragma vector always
      ...
      if ((t1 == t2) && (t1 <= floord(PB_N-1,32))) {
        for (t3=PB_N-1;t3<=min(PB_M-2,32*t1+29);t3++) {
          lbv=32*t1;
          ubv=PB_N-1;
#pragma ivdep
#pragma vector always
      ...
      if (t1 <= floord(PB_N-1,32)) {
        for (t3=max(32*t2,32*t1+30);t3<=min(PB_M-2,32*t2+31);t3++) {
          lbv=32*t1;
...
kernel_2224111121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
