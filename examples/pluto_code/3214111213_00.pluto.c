...
if (PB_L >= 3) {
  for (t1=0;t1<=floord(PB_L-1,32);t1++) {
    for (t2=0;t2<=min(floord(PB_L-2,16),floord(32*t1+PB_L+30,32));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(15,PB_L-2);t3++) {
          B[t3-1] = C[t3] * 6;;
          for (t4=2*t3+1;t4<=min(31,t3+PB_L-1);t4++) {
            A[t3][(-2*t3+t4)+t3-1] = A[(-2*t3+t4)][(-2*t3+t4)+t3] + A[(-2*t3+t4)-1][(-2*t3+t4)+t3-2] - A[t3][(-2*t3+t4)+t3] + 5;;
          }
        }
      }
      if (t2 >= 1) {
        for (t3=max(max(2,32*t1),32*t2-PB_L+1);t3<=min(min(PB_L-2,32*t1+31),16*t2+15);t3++) {
      ...
      if ((PB_L <= 16) && (t1 == 0) && (t2 == 0)) {
        B[(PB_L-1)-1] = C[(PB_L-1)] * 6;;
      }
    }
  }
}
