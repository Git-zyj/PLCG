...
/* Start of CLooG code */
if (PB_L >= 1) {
  if ((PB_L >= -2*PB_M+3) && (PB_M >= 0)) {
    for (t1=0;t1<=floord(PB_L+PB_M-2,32);t1++) {
      for (t2=0;t2<=min(min(min(floord(PB_L,32),floord(PB_L+3*PB_M-3,32)),floord(-32*t1+PB_L+PB_M-1,32)),floord(-32*t1+PB_L+2*PB_M-2,32));t2++) {
        if (t2 >= 1) {
          lbp=32*t1;
...
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[0][t4+1] = D[t4-1][0] + E[0][t4] + 3;;
          }
        }
        if (t2 == 0) {
          for (t3=max(1,32*t1);t3<=min(min(PB_L-2,PB_M-1),32*t1+31);t3++) {
            A[t3+1] = C[t3] - C[t3-1] * C[t3+1] * A[t3] - A[t3+2] - 2;;
            lbv=1;
            ubv=min(31,PB_L);
#pragma ivdep
#pragma vector always
