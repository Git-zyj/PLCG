...
void kernel_3114131211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if (PB_L >= 4) {
  for (t1=0;t1<=floord(3*PB_L-3,64);t1++) {
    lbp=ceild(2*t1,3);
    ubp=min(floord(PB_L-1,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(1,32*t1-32*t2);t3<=min(min(floord(PB_L-1,2),16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(max(32*t2,2*t3),t3+2);t4<=min(PB_L-1,32*t2+31);t4++) {
          A[(-t3+t4)-1] = B[(-t3+t4)-1][t3] + B[t3-1][(-t3+t4)+t3] + B[(-t3+t4)][t3] * A[(-t3+t4)-2] - A[(-t3+t4)] + 2;;
        }
      }
    }
  }
...
kernel_3114131211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
