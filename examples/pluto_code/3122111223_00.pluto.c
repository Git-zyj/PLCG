...
void kernel_3122111223_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
  ...
  for (t1=0;t1<=PB_M-2;t1++) {
    for (t2=0;t2<=PB_L-3;t2++) {
      for (t3=1;t3<=PB_L-1;t3++) {
        A[t3+1][t2][t2] = B[t2][t1+1][t3-1] - A[t1][t3][t3] - A[t3+2][t2+2][t2+2] - 2;;
      }
    }
  }
}
...
kernel_3122111223_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
