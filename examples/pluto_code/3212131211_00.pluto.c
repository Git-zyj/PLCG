...
void kernel_3212131211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_L-3,32),t1);t2++) {
      for (t3=32*t2;t3<=min(min(PB_L-3,PB_P-1),32*t2+31);t3++) {
        lbv=max(32*t1,t3);
        ubv=min(PB_P-1,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4] = C[t4] - D[t3][0] - D[t3][t4] + C[t3] + B[t3+2][t4] + B[t3+1][t4] - 4;;
          B[t3+1][t4] = C[t4] * D[t3][t4] - A[t4+1][t3] - A[t3+1][t4] - 2;;
        }
      }
    }
  }
...
kernel_3212131211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
