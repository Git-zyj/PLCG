...
void kernel_3214211121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
if (PB_P >= 4) {
  lbp=0;
  ubp=floord(PB_P-1,16);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_P-2,32);t2++) {
      if (t1 <= floord(PB_P-1,32)) {
        for (t3=max(2,32*t2);t3<=min(PB_P-2,32*t2+31);t3++) {
          lbv=32*t1;
          ubv=min(PB_P-1,32*t1+31);
#pragma ivdep
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            A[0][t5][t3] = A[t5][0][t3-2] - 2;;
          }
          for (t4=1;t4<=min(PB_P-1,32*t1+31);t4++) {
            lbv=32*t1;
            ubv=t4-1;
#pragma ivdep
#pragma vector always
            for (t5=lbv;t5<=ubv;t5++) {
              B[t5][t3-1][t3] = C[t3+1][t3][t4] * C[t4-1][t3][t5] - B[t5][t3][t3+1] - 5;;
            }
            lbv=max(32*t1,t4);
            ubv=min(PB_P-1,32*t1+31);
#pragma ivdep
#pragma vector always
            for (t5=lbv;t5<=ubv;t5++) {
              A[t4][(-t4+t5)][t3] = A[(-t4+t5)][t4][t3-2] - 2;;
              B[t5][t3-1][t3] = C[t3+1][t3][t4] * C[t4-1][t3][t5] - B[t5][t3][t3+1] - 5;;
            }
            lbv=PB_P;
            ubv=min(32*t1+31,t4+PB_P-1);
#pragma ivdep
#pragma vector always
            for (t5=lbv;t5<=ubv;t5++) {
              A[t4][(-t4+t5)][t3] = A[(-t4+t5)][t4][t3-2] - 2;;
...
#pragma vector always
      ...
      if (t1 >= ceild(PB_P,32)) {
        for (t3=max(2,32*t2);t3<=min(PB_P-2,32*t2+31);t3++) {
          for (t4=32*t1-PB_P+1;t4<=PB_P-1;t4++) {
            lbv=32*t1;
            ubv=min(32*t1+31,t4+PB_P-1);
#pragma ivdep
#pragma vector always
...
kernel_3214211121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
