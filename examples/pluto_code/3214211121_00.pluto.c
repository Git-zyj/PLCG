...
void kernel_3214211121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
  ...
  for (t1=0;t1<=floord(PB_M-2,32);t1++) {
    for (t2=0;t2<=floord(PB_M-2,32);t2++) {
      ...
      if ((t2 >= 1) && (t2 <= floord(PB_M-3,32))) {
        for (t3=max(1,32*t1);t3<=min(min(PB_M-3,32*t1+31),32*t2+31);t3++) {
          lbp=max(32*t2,t3);
          ubp=min(PB_M-3,32*t2+31);
#pragma omp parallel for private(lbv,ubv)
          for (t4=lbp;t4<=ubp;t4++) {
            B[t3-1][t4] = B[t3+1][t4+2] * 3;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
...
#pragma omp parallel for private(lbv,ubv)
      ...
      if ((PB_M >= 3) && (PB_M <= 33) && (t1 == 0) && (t2 == 0)) {
        A[(PB_M-2)+1] = C[(PB_M-2)+1] + 1;;
      }
    }
  }
}
...
kernel_3214211121_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
