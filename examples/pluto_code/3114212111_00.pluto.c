...
/* Start of CLooG code */
if ((PB_L >= 1) && (PB_M >= 1) && (PB_P >= 2)) {
  lbp=0;
  ubp=floord(PB_L-1,32);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_M-1,32);t2++) {
      for (t3=2*t2;t3<=min(floord(2*PB_M+PB_P-4,32),floord(64*t2+PB_P+60,32));t3++) {
        for (t4=32*t1;t4<=min(PB_L-1,32*t1+31);t4++) {
          for (t5=max(ceild(32*t3-PB_P+2,2),32*t2);t5<=min(min(PB_M-1,32*t2+31),16*t3+15);t5++) {
            for (t6=max(32*t3,2*t5);t6<=min(32*t3+31,2*t5+PB_P-2);t6++) {
              A[t4][t5][(-2*t5+t6)] = B[t5][t4][(-2*t5+t6)] * A[t4][(-2*t5+t6)+1][t5] * 5;;
            }
          }
        }
      }
