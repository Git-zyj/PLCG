...
void kernel_2122232121_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 3) {
  for (t1=0;t1<=PB_M-3;t1++) {
    A[t1] = B[t1+1] + C[t1] - C[t1+1] - A[t1+2] * 4;;
  }
}
/* End of CLooG code */
...
kernel_2122232121_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
