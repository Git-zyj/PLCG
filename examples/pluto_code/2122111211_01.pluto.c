...
void kernel_2122111211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 4)) {
  for (t1=0;t1<=min(floord(PB_L-1,16),floord(PB_M-1,32));t1++) {
    lbp=max(max(0,ceild(32*t1-PB_L+1,32)),ceild(32*t1-PB_M+2,32));
    ubp=min(min(floord(PB_L,32),floord(PB_M-3,32)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(2,32*t1-32*t2);t3<=min(min(min(PB_L-1,PB_M-2),32*t1-32*t2+31),-32*t2+PB_M-1);t3++) {
        for (t4=max(1,32*t2);t4<=min(min(PB_L,32*t2+31),-t3+PB_M-1);t4++) {
          A[t4+t3][t3] = B[t4+t3][t4-1] + A[t4+t3-2][t3-1] * A[t4+t3-1][t3] - 1;;
        }
      }
    }
  }
...
kernel_2122111211_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
