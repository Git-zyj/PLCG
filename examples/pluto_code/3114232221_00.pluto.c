...
void kernel_3114232221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_M >= 3) {
  for (t1=2;t1<=PB_M-1;t1++) {
    A[t1] = B[t1] - B[t1] * B[t1] * A[t1-2] * A[t1+2] * 1;;
  }
}
/* End of CLooG code */
...
kernel_3114232221_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
