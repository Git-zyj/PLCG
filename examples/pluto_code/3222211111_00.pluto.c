...
void kernel_3222211111_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 2) {
  if (PB_P >= 1) {
    for (t1=0;t1<=PB_L-2;t1++) {
      B[t1][t1] = C[t1] + B[t1+1][t1+1] - 6;;
      for (t2=1;t2<=PB_P;t2++) {
        for (t3=1;t3<=PB_L-1;t3++) {
          A[t1][t1+1][t3-1] = A[t3-1][t3][t2-1] - 4;;
        }
      }
    }
  }
...
kernel_3222211111_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
