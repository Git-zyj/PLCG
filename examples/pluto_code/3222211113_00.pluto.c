...
void kernel_3222211113_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_L >= 2) {
  if (PB_M >= 0) {
    for (t1=0;t1<=floord(3*PB_L+PB_M-3,32);t1++) {
      lbp=max(max(ceild(t1,2),ceild(32*t1-PB_L+1,32)),ceild(128*t1-2*PB_L-PB_M+4,192));
      ubp=min(min(floord(3*t1+1,3),floord(2*PB_L+PB_M-2,32)),floord(64*t1+PB_M+32,96));
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
      for (t2=lbp;t2<=ubp;t2++) {
        for (t3=max(max(1,32*t1-32*t2),32*t2-PB_M+1);t3<=min(min(PB_L-1,16*t2-1),32*t1-32*t2+31);t3++) {
          for (t4=32*t2;t4<=min(32*t2+31,t3+PB_M-1);t4++) {
            B[(-t3+t4)-1] = C[(-t3+t4)-1] + D[(-t3+t4)+1][t3] * B[(-t3+t4)] * 5;;
          }
        }
        if (PB_M <= 1) {
        ...
        if (PB_M >= 2) {
          for (t3=max(16*t2,32*t1-32*t2);t3<=min(min(min(PB_L-1,16*t2+15),32*t2-PB_M),32*t1-32*t2+31);t3++) {
            B[t3-1] = 2;;
          }
        }
        if (PB_M >= 2) {
          for (t3=max(max(max(1,16*t2),32*t1-32*t2),32*t2-PB_M+1);t3<=min(min(PB_L-1,16*t2+15),32*t1-32*t2+31);t3++) {
            for (t4=max(32*t2,t3+1);t4<=min(2*t3-1,t3+PB_M-1);t4++) {
              B[(-t3+t4)-1] = C[(-t3+t4)-1] + D[(-t3+t4)+1][t3] * B[(-t3+t4)] * 5;;
            }
            if (t3 >= PB_M) {
              B[t3-1] = 2;;
            ...
            if (t3 <= PB_M-1) {
              B[t3-1] = C[t3-1] + D[t3+1][t3] * B[t3] * 5;;
              B[t3-1] = 2;;
            }
        ...
        if (PB_M >= 2) {
          for (t3=max(max(32*t1-32*t2,16*t2+16),32*t2-PB_M+1);t3<=min(min(PB_L-1,32*t2+30),32*t1-32*t2+31);t3++) {
...
kernel_3222211113_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
