...
void kernel_2224112123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma vector always
      ...
      if (t1 <= floord(PB_L-1,32)) {
        for (t2=0;t2<=floord(PB_N,32);t2++) {
          for (t3=max(1,32*t2);t3<=min(PB_N,32*t2+31);t3++) {
            lbv=32*t1;
            ubv=min(PB_L-1,32*t1+31);
#pragma ivdep
#pragma vector always
            for (t4=lbv;t4<=ubv;t4++) {
              A[t3][t4] = C[t3-1][0] * A[t3+2][t4] + A[t3-1][t4] + 5;;
            }
          }
        }
      }
...
kernel_2224112123_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
