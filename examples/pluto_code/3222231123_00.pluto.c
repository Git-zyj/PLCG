...
if (PB_L >= 3) {
  if (PB_P >= 0) {
    for (t1=0;t1<=floord(2*PB_L+PB_P-6,32);t1++) {
      lbp=max(max(0,ceild(32*t1-PB_L-PB_P+2,32)),ceild(32*t1-PB_P-32,64));
      ubp=min(floord(PB_L-3,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
      for (t2=lbp;t2<=ubp;t2++) {
        for (t3=max(1,32*t1-32*t2);t3<=min(min(32*t2,PB_P-1),32*t1-32*t2+31);t3++) {
          for (t4=32*t2;t4<=min(PB_L-3,32*t2+31);t4++) {
            B[t3][t3] = D[t3] * E[t3][t4] - A[t3-1] - 4;;
          }
        }
        for (t3=max(32*t1-32*t2,32*t2+1);t3<=min(min(min(PB_L-2,PB_P-1),32*t2+32),32*t1-32*t2+31);t3++) {
          for (t4=32*t2;t4<=t3-2;t4++) {
            B[t3][t3] = D[t3] * E[t3][t4] - A[t3-1] - 4;;
          }
          A[(t3-1)] = C[(t3-1)+1] * C[(t3-1)] * C[(t3-1)] * D[(t3-1)+1] + A[(t3-1)+2] + 4;;
          B[t3][t3] = D[t3] * E[t3][(t3-1)] - A[t3-1] - 4;;
          if (t1 == 2*t2) {
            for (t4=t3;t4<=min(PB_L-3,16*t1+31);t4++) {
              if (t1%2 == 0) {
                B[t3][t3] = D[t3] * E[t3][t4] - A[t3-1] - 4;;
              }
            }
        ...
        for (t3=max(max(PB_P,32*t1-32*t2),32*t2+1);t3<=min(min(PB_L-2,32*t2+32),32*t1-32*t2+31);t3++) {
          A[(t3-1)] = C[(t3-1)+1] * C[(t3-1)] * C[(t3-1)] * D[(t3-1)+1] + A[(t3-1)+2] + 4;;
        }
        for (t3=max(PB_L-1,32*t1-32*t2);t3<=min(min(PB_P-1,32*t2+32),32*t1-32*t2+31);t3++) {
          for (t4=32*t2;t4<=PB_L-3;t4++) {
            B[t3][t3] = D[t3] * E[t3][t4] - A[t3-1] - 4;;
          }
        }
        for (t3=max(32*t1-32*t2,32*t2+33);t3<=min(PB_P-1,32*t1-32*t2+31);t3++) {
          for (t4=32*t2;t4<=min(PB_L-3,32*t2+31);t4++) {
            B[t3][t3] = D[t3] * E[t3][t4] - A[t3-1] - 4;;
          }
        }
  ...
  if (PB_P <= -1) {
    for (t1=0;t1<=floord(PB_L-3,16);t1++) {
      t2 = max(ceild(t1-1,2),ceild(32*t1-PB_L+2,32));
      if (t2 <= floord(t1,2)) {
        for (t3=max(32*t1-32*t2,32*t2+1);t3<=min(min(PB_L-2,32*t2+32),32*t1-32*t2+31);t3++) {
          A[(t3-1)] = C[(t3-1)+1] * C[(t3-1)] * C[(t3-1)] * D[(t3-1)+1] + A[(t3-1)+2] + 4;;
        }
      }
    }
