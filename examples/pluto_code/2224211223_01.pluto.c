...
void kernel_2224211223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 1) {
  for (t2=1;t2<=PB_M-2;t2++) {
    A[t2] = C[t2-1] * C[t2-1] * A[t2+1] * A[t2-1] + 4;;
  }
  for (t2=ceild(-PB_M-30,32);t2<=0;t2++) {
    lbp=0;
    ubp=floord(PB_M-1,32);
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t5=max(32*t2,-PB_M+1);t5<=(min(0,32*t2+31))-7;t5+=8) {
        lbv=32*t4;
...
kernel_2224211223_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
