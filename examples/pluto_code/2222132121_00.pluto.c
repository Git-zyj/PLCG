...
void kernel_2222132121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
  ...
  for (t1=lbp;t1<=ubp;t1++) {
    if ((PB_L <= -1) && (PB_M >= 4) && (t1 <= floord(PB_M-2,32))) {
      for (t3=max(2,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
        B[t3-2] = C[t3] - C[t3+1] + C[t3] + C[t3] - 3;;
      }
    }
    if ((PB_M >= ceild(-PB_L+8,2)) && (t1 <= floord(2*PB_M+PB_L-4,64))) {
      for (t2=0;t2<=min(min(floord(PB_L,32),floord(PB_M+PB_L-4,32)),floord(-32*t1+PB_M+PB_L-2,32));t2++) {
        if ((PB_L >= 2) && (t2 == 0)) {
          for (t3=max(2,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
            B[t3-2] = C[t3] - C[t3+1] + C[t3] + C[t3] - 3;;
            for (t4=1;t4<=min(31,PB_L-1);t4++) {
              A[t3-2][t4] = C[t3] - A[t3-2][t4-1] - 4;;
            }
          }
        }
        ...
        if ((PB_L >= 2) && (t1 >= ceild(PB_M-32,32)) && (t2 == 0)) {
          for (t4=1;t4<=min(31,PB_L-1);t4++) {
            A[(PB_M-1)-2][t4] = C[(PB_M-1)] - A[(PB_M-1)-2][t4-1] - 4;;
          }
        }
        if ((PB_L <= 1) && (t2 == 0)) {
          for (t3=max(2,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
...
kernel_2222132121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
