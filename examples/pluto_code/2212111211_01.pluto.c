...
void kernel_2212111211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 1) {
  for (t1=0;t1<=floord(PB_M,32);t1++) {
    if ((PB_L <= -1) && (PB_M >= 3) && (t1 <= floord(PB_M-1,32))) {
      for (t3=max(2,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
        A[t3-1] = C[t3] - A[t3] * A[t3-2] * 4;;
      }
    }
    for (t2=0;t2<=min(min(floord(PB_L,32),floord(PB_M+PB_L-3,32)),floord(-64*t1+2*PB_M+PB_L-2,32));t2++) {
      if ((PB_L <= 1) && (t2 == 0)) {
        for (t3=max(2,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
          A[t3-1] = C[t3] - A[t3] * A[t3-2] * 4;;
        }
      ...
      if ((t2 >= 1) && (t2 <= floord(PB_L-2,32))) {
        for (t3=max(1,32*t1);t3<=min(PB_M,32*t1+31);t3++) {
          for (t4=32*t2;t4<=min(PB_L-2,32*t2+31);t4++) {
            B[t3-1][t4+1] = B[t3-1][t4] * 6;;
          }
        }
      }
      if ((PB_L >= 2) && (t1 == 0) && (t2 == 0)) {
        for (t4=0;t4<=min(31,PB_L-2);t4++) {
          B[1 -1][t4+1] = B[1 -1][t4] * 6;;
        }
      }
      if ((PB_L >= 2) && (t2 == 0)) {
        for (t3=max(2,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
          B[t3-1][0 +1] = B[t3-1][0] * 6;;
          A[t3-1] = C[t3] - A[t3] * A[t3-2] * 4;;
      ...
      if ((PB_L >= 2) && (PB_M >= 2) && (t1 >= ceild(PB_M-31,32)) && (t2 == 0)) {
        for (t4=0;t4<=min(31,PB_L-2);t4++) {
          B[PB_M-1][t4+1] = B[PB_M-1][t4] * 6;;
        }
      }
    }
  }
...
kernel_2212111211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
