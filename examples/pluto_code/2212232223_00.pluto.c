...
void kernel_2212232223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M-2,32);t1++) {
    for (t2=0;t2<=min(floord(32*t1+PB_M+30,32),floord(32*t1+33*PB_M-66,544));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=min(13,PB_M-5);t3++) {
          B[t3] = C[t3] * C[t3] + D[t3][1] * B[t3+1] - 1;;
          for (t4=2*t3+4;t4<=min(31,t3+PB_M-1);t4++) {
            A[(-2*t3+t4)-2][t3] = C[t3] + C[(-2*t3+t4)+2] - D[(-2*t3+t4)+t3][t3] * A[t3][(-2*t3+t4)-1] * A[(-2*t3+t4)-4][t3] * 2;;
          }
        }
      }
      if (t2 >= 1) {
        for (t3=max(32*t1,32*t2-PB_M+1);t3<=min(min(PB_M-5,32*t1+31),16*t2+13);t3++) {
          for (t4=max(32*t2,2*t3+4);t4<=min(32*t2+31,t3+PB_M-1);t4++) {
            A[(-2*t3+t4)-2][t3] = C[t3] + C[(-2*t3+t4)+2] - D[(-2*t3+t4)+t3][t3] * A[t3][(-2*t3+t4)-1] * A[(-2*t3+t4)-4][t3] * 2;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(0,PB_M-4);t3<=min(13,PB_M-2);t3++) {
          B[t3] = C[t3] * C[t3] + D[t3][1] * B[t3+1] - 1;;
        }
      }
      if (t2 == 0) {
        for (t3=max(14,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
          B[t3] = C[t3] * C[t3] + D[t3][1] * B[t3+1] - 1;;
        }
      }
    }
...
kernel_2212232223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
