...
void kernel_3114212213_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
/* Start of CLooG code */
if ((PB_M >= 2) && (PB_P >= 2)) {
  for (t1=0;t1<=min(floord(PB_P-2,16),floord(PB_M-2,32));t1++) {
    lbp=max(0,ceild(32*t1-PB_P+1,32));
    ubp=min(floord(PB_P-2,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=t2;t3<=min(floord(PB_P-2,32),floord(-32*t1+32*t2+PB_M-2,32));t3++) {
        for (t4=32*t1-32*t2;t4<=min(min(PB_P-1,32*t1-32*t2+31),-32*t3+PB_M-2);t4++) {
          for (t5=32*t3;t5<=min(min(PB_P-2,32*t3+31),-t4+PB_M-2);t5++) {
            lbv=32*t2;
            ubv=min(t5,32*t2+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t6+t4][t4][t6] = B[t5+t4][t6][t4] + A[t5+t4+1][t4][t5+1] * A[t5+t4+1][t4+1][t5] - 1;;
            }
          }
        }
      }
...
kernel_3114212213_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
