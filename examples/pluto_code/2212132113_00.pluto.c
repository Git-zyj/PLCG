...
void kernel_2212132113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_M >= 3) {
  for (t2=0;t2<=PB_M-3;t2++) {
    for (t4=1;t4<=-t2+PB_M-2;t4++) {
      A[t4+t2][t4-1] = C[t2][2] + A[t4+t2+1][t2] - 3;;
    }
  }
  for (t2=-PB_M+2;t2<=-2;t2++) {
    B[-t2] = D[-t2-2] + D[-t2] * D[-t2] * D[-t2] + B[-t2+1] * 5;;
  }
}
/* End of CLooG code */
...
kernel_2212132113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
