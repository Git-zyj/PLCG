...
void kernel_3222112121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 1) {
  if (PB_M >= 0) {
    for (t1=0;t1<=min(floord(PB_L+PB_M-1,32),floord(4*PB_L+PB_M-7,32));t1++) {
      for (t2=0;t2<=min(min(floord(PB_L-1,32),floord(-32*t1+PB_L+PB_M-1,32)),floord(-32*t1+4*PB_L+PB_M-7,128));t2++) {
        if (t2 >= 1) {
          lbp=32*t1;
...
#pragma vector always
        ...
        if (t2 == 0) {
          for (t3=max(1,32*t1);t3<=min(min(PB_L-1,PB_M-3),32*t1+31);t3++) {
            B[t3+2][0] = 4;;
            A[t3+1] = C[t3-1] + C[t3] * A[t3-1] * A[t3+3] * 3;;
            lbv=1;
            ubv=min(31,PB_L-1);
#pragma ivdep
#pragma vector always
        ...
        if (t2 == 0) {
          for (t3=max(max(1,32*t1),PB_M-2);t3<=min(PB_L-1,32*t1+31);t3++) {
            A[t3+1] = C[t3-1] + C[t3] * A[t3-1] * A[t3+3] * 3;;
          }
        }
        if (t2 == 0) {
...
kernel_3222112121_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
