...
void kernel_2122232123_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 3) {
  for (t1=2;t1<=PB_M-1;t1++) {
    A[t1][t1] = B[t1-2] - C[t1-2] + B[t1] + A[t1-2][t1-2] * 5;;
  }
}
/* End of CLooG code */
...
kernel_2122232123_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
