...
void kernel_2222131211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_L >= 2) {
  A[0 +1][0 +1] = C[0 +1] - C[0] - C[0] * B[0] * 6;;
  for (t1=1;t1<=PB_L-2;t1++) {
    A[t1+1][t1+1] = C[t1+1] - C[t1] - C[t1] * B[t1] * 6;;
    B[t1] = C[t1-1] + C[1] * C[t1] + A[t1+1][t1+1] * B[t1+1] * B[t1-1] - 2;;
  }
}
/* End of CLooG code */
...
kernel_2222131211_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
