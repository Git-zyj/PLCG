...
void kernel_3212231211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
...
/* Start of CLooG code */
if (PB_P >= 2) {
  for (t1=0;t1<=floord(PB_P,32);t1++) {
    if (PB_M <= -1) {
      for (t4=max(2,32*t1);t4<=min(PB_P,32*t1+31);t4++) {
        A[t4] = C[t4] * D[t4] * D[t4+1] * A[t4+1] - A[t4-1] - A[t4+1] * 1;;
      }
    }
    for (t2=0;t2<=floord(PB_M,32);t2++) {
      ...
      for (t3=0;t3<=floord(PB_L,32);t3++) {
        if ((PB_M >= 3) && (t1 == 0) && (t2 == 0) && (t3 == 0)) {
          for (t4=2;t4<=min(min(31,PB_P),PB_L-1);t4++) {
            A[t4] = C[t4] * D[t4] * D[t4+1] * A[t4+1] - A[t4-1] - A[t4+1] * 1;;
            for (t5=t4;t5<=min(31,PB_L-1);t5++) {
              lbv=1;
              ubv=min(31,PB_M-2);
#pragma ivdep
#pragma vector always
              for (t6=lbv;t6<=ubv;t6++) {
                B[t5][t4-1][t6-1] = E[t5][t6+1][t4+1] - E[t5-1][t6][t4] + B[t5-1][t4-2][t6-1] + 3;;
              }
            }
          }
        }
        ...
        if ((PB_M >= 3) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(32,32*t1);t4<=min(PB_P,32*t1+31);t4++) {
            A[t4] = C[t4] * D[t4] * D[t4+1] * A[t4+1] - A[t4-1] - A[t4+1] * 1;;
          }
        }
        if ((PB_M >= 3) && (t1 == 0) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(2,PB_L);t4<=min(31,PB_P);t4++) {
            A[t4] = C[t4] * D[t4] * D[t4+1] * A[t4+1] - A[t4-1] - A[t4+1] * 1;;
          }
        }
        if ((PB_M >= 3) && (t2 <= floord(PB_M-2,32)) && (t3 >= 1) && (t3 <= floord(PB_L-1,32))) {
          for (t4=max(2,32*t1);t4<=min(min(min(PB_P,PB_L-1),32*t1+31),32*t3+31);t4++) {
            for (t5=max(32*t3,t4);t5<=min(PB_L-1,32*t3+31);t5++) {
              lbv=max(1,32*t2);
...
#pragma vector always
        ...
        if ((t1 == 0) && (t2 >= 1) && (t2 <= floord(PB_M-2,32)) && (t3 == 0)) {
          for (t4=2;t4<=min(min(31,PB_P),PB_L-1);t4++) {
            for (t5=t4;t5<=min(31,PB_L-1);t5++) {
              lbv=32*t2;
              ubv=min(PB_M-2,32*t2+31);
...
kernel_3212231211_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
