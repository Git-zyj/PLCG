...
void kernel_2114112221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if ((PB_L >= 3) && (PB_M >= 3)) {
  for (t1=-1;t1<=floord(PB_M-1,16);t1++) {
    lbp=ceild(t1,2);
    ubp=min(min(floord(PB_M+PB_L-4,32),floord(16*t1+PB_L+12,32)),floord(32*t1+PB_L+26,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,16*t1),32*t2-PB_L+3);t3<=min(min(PB_M-1,16*t1+31),32*t1-32*t2+PB_L+28);t3++) {
        lbv=max(max(32*t2,t3),-32*t1+32*t2+2*t3-31);
        ubv=min(min(32*t2+31,-32*t1+32*t2+2*t3),t3+PB_L-3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3-1][(-t3+t4)+1] = B[(-t3+t4)+1] * A[t3-2][(-t3+t4)+2] + A[t3][(-t3+t4)+2] - 4;;
        }
      }
    }
  }
...
kernel_2114112221_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
