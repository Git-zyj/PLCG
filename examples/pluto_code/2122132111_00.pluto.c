...
void kernel_2122132111_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_M-1,32);t2++) {
      for (t3=32*t2;t3<=min(PB_M-1,32*t2+31);t3++) {
        lbv=max(2,32*t1);
        ubv=min(PB_P,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4-1][0] = B[t3][t4-1] - B[t3][t4+2] + B[t3][t4-2] * A[t3+1][t4-1][0] + 4;;
        }
      }
    }
  }
...
kernel_2122132111_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
