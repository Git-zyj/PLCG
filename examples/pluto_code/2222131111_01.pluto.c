...
void kernel_2222131111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
for (t1=0;t1<=min(PB_M-2,PB_N-2);t1++) {
  A[t1] = C[t1][t1+1] - D[t1] + D[t1] * 5;;
  B[t1] = D[0] * D[t1] - D[t1] * B[t1+1] + 3;;
}
for (t1=max(0,PB_N-1);t1<=PB_M-2;t1++) {
  A[t1] = C[t1][t1+1] - D[t1] + D[t1] * 5;;
}
...
kernel_2222131111_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
