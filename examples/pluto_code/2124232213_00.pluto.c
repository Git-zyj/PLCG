...
void kernel_2124232213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_N >= 2)) {
  for (t1=0;t1<=floord(2*PB_L+PB_N-3,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_L+1,32));
    ubp=min(min(floord(PB_L+PB_N-2,32),floord(32*t1+PB_N+30,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,32*t1-32*t2),32*t2-PB_N+1);t3<=min(min(PB_L-1,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(32*t2+31,t3+PB_N-1);t4++) {
          A[t3-1][(-t3+t4)] = B[(-t3+t4)] * C[(-t3+t4)] * D[(-t3+t4)][t3] - A[t3-1][(-t3+t4)+1] - A[t3][(-t3+t4)-1] * 2;;
        }
      }
    }
  }
...
kernel_2124232213_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
