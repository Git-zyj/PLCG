...
void kernel_2114232211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_L >= 2) {
  for (t1=0;t1<=floord(3*PB_L+1,32);t1++) {
    ...
    ubp=min(floord(2*t1+1,3),floord(PB_L,16));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t1-32*t2),16*t2+1);t3<=min(min(PB_L+1,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(min(32*t2+31,2*t3-1),t3+PB_L-1);t4++) {
          A[(-t3+t4)] = B[(-t3+t4)][t3-2] * B[t3][(-t3+t4)] + C[t3][(-t3+t4)] + A[(-t3+t4)+1] + A[(-t3+t4)-1] + 4;;
        }
      }
    }
  }
...
kernel_2114232211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
