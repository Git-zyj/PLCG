...
void kernel_2222132223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_M >= 3) {
  for (t2=-PB_M+3;t2<=-2;t2++) {
    A[-t2] = C[-t2] - C[-t2] * C[-t2-1] * C[-t2-2] - C[-t2] * C[-t2] * A[-t2+2] * A[-t2+1] - 6;;
  }
  for (t2=2;t2<=PB_M-1;t2++) {
    B[t2] = B[t2-1] - B[t2-2] + 4;;
  }
}
/* End of CLooG code */
...
kernel_2222132223_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
