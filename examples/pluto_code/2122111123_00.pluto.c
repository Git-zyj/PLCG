...
void kernel_2122111123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t4=ceild(-PB_L-30,32);t4<=-1;t4++) {
      for (t6=max(32*t4,-PB_L+1);t6<=min(-2,32*t4+31);t6++) {
        lbv=max(1,32*t2);
        ubv=min(PB_P-1,32*t2+31);
#pragma ivdep
#pragma vector always
        for (t7=lbv;t7<=ubv;t7++) {
          A[t7-1][-t6] = B[1][-t6][t7] + A[t7-1][-t6-2] + 4;;
        }
      }
    }
  }
...
kernel_2122111123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
