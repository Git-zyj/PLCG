...
void kernel_2114231223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4-1] = B[t3][t3+1] - B[t3][t4-1] * C[t4+1][t3] + A[t4][t3+1] + A[t4][t3] - 4;;
        }
      }
    }
  }
...
kernel_2114231223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
