...
void kernel_2224211221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (t2=0;t2<=min(floord(PB_M-3,32),t1);t2++) {
      if (t2 == 0) {
        for (t3=max(2,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
          B[t3][0] = B[t3][0 +1] * B[t3-2][0] + 5;;
          A[t3+1] = C[t3] * C[0] - A[t3] * A[t3-1] + 4;;
          for (t4=1;t4<=min(31,t3-1);t4++) {
            B[t3][t4] = B[t3][t4+1] * B[t3-2][t4] + 5;;
          }
        }
      }
      if (t2 >= 1) {
        for (t3=max(32*t1,32*t2+1);t3<=min(PB_M-2,32*t1+31);t3++) {
...
kernel_2224211221_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
