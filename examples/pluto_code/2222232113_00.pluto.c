...
void kernel_2222232113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
    ...
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=floord(PB_L-2,32);t3++) {
        for (t4=max(1,32*t2);t4<=min(PB_N-1,32*t2+31);t4++) {
          for (t5=max(2,32*t3);t5<=min(PB_L-2,32*t3+31);t5++) {
            B[t4][t5+1] = D[t5-2][t4] * D[t5][t4+1] - E[t4][t4] - D[t4-1][t5] + A[t5-2] - B[t4][t5] * 5;;
          }
        }
      }
    }
...
kernel_2222232113_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
