...
void kernel_2224112111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_M >= max(0,-2*PB_P+2)) && (PB_P >= 0)) {
  for (t1=0;t1<=floord(PB_M+PB_P-1,32);t1++) {
    if ((PB_L <= -1) && (PB_M >= 2) && (t1 <= floord(PB_M-1,32))) {
      for (t3=max(1,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
        A[t3] = C[t3] * C[t3] + A[t3-1] + 2;;
      }
    }
    for (t2=0;t2<=min(floord(PB_L,32),floord(PB_M+2*PB_L-2,64));t2++) {
      if ((PB_L == 0) && (t2 == 0)) {
        for (t3=max(1,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
          A[t3] = C[t3] * C[t3] + A[t3-1] + 2;;
        }
...
#pragma vector always
      ...
      if ((PB_L >= 1) && (t2 == 0)) {
        for (t3=max(1,32*t1);t3<=min(min(PB_M-1,PB_P-1),32*t1+31);t3++) {
          B[t3+2][0][0] = 5;;
          A[t3] = C[t3] * C[t3] + A[t3-1] + 2;;
          lbv=1;
          ubv=min(31,PB_L-1);
...
#pragma vector always
      ...
      if ((PB_L >= 1) && (t2 == 0)) {
        for (t3=max(max(1,PB_P),32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
          A[t3] = C[t3] * C[t3] + A[t3-1] + 2;;
        }
      }
      if ((PB_L >= 1) && (t2 == 0)) {
        for (t3=max(max(1,PB_M),32*t1);t3<=min(PB_P-1,32*t1+31);t3++) {
          lbv=0;
          ubv=min(31,PB_L-1);
#pragma ivdep
#pragma vector always
...
kernel_2224112111_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
