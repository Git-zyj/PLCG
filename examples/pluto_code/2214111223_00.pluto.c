...
void kernel_2214111223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if (PB_L >= 6) {
  for (t1=2;t1<=PB_L-4;t1++) {
    for (t2=0;t2<=PB_L-1;t2++) {
      A[t1][t1+1] = A[t1+2][t1+3] - A[t1-2][t1-1] + 4;;
      A[t1][t1+1] = C[t1][t2] + 4;;
    }
  }
}
...
kernel_2214111223_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
