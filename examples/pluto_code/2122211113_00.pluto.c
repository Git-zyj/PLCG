...
if ((PB_L >= 2) && (PB_N >= 3)) {
  lbp=0;
  ubp=min(floord(PB_N-3,32),floord(PB_L-2,64));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=min(floord(PB_N-2,32),floord(-32*t1+PB_L-1,32));t2++) {
      for (t3=max(32*t2,32*t1+1);t3<=min(min(PB_N-2,32*t2+31),-32*t1+PB_L-1);t3++) {
        lbv=32*t1;
        ubv=min(min(32*t1+31,t3-1),-t3+PB_L-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4+t3] = B[t3+1][t4][t3-1] * A[t3-1][t4+t3-1] * 2;;
        }
      }
    }
  }
