...
void kernel_3112231211_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
  int t1, t2, t3;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 3) && (PB_P >= 2)) {
  for (t1=0;t1<=min(min(floord(PB_M-2,2),PB_L-2),PB_P-2);t1++) {
    for (t2=max(1,t1);t2<=min(PB_L-2,-t1+PB_M-2);t2++) {
      for (t3=2;t3<=PB_L;t3++) {
        A[t1][t2-1][t1+1] = B[t3][t2+t1+1] + C[t2+1][t3-1][t1] - D[t2][t1][t3-1] - A[t1][t3-2][t1+1] * A[t2-1][t3-1][t2] - 3;;
      }
    }
  }
}
...
kernel_3112231211_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
