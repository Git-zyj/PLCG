...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_P-2,32);t2++) {
      for (t3=32*t1;t3<=min(min(PB_M-2,PB_P-2),32*t1+31);t3++) {
        for (t4=max(max(1,32*t2),t3);t4<=min(PB_P-2,32*t2+31);t4++) {
          A[t3][t4-1] = B[t4][t3] + C[t3+1][t4-1] + B[t4+1][t4] * A[t3][t4+1] + 5;;
        }
      }
    }
  }
