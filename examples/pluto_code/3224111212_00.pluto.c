...
void kernel_3224111212_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
for (t1=1;t1<=min(PB_L-1,PB_M-2);t1++) {
  A[t1] = C[t1] - A[t1-1] * A[t1+1] * 5;;
  B[t1] = B[t1-1] - 4;;
}
for (t1=PB_L;t1<=PB_M-2;t1++) {
  A[t1] = C[t1] - A[t1-1] * A[t1+1] * 5;;
}
...
kernel_3224111212_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
