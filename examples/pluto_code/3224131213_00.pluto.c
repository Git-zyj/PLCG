...
for (t2=-PB_M+1;t2<=-2;t2++) {
  A[-t2-1] = C[-t2] - A[-t2-2] * 2;;
}
if (PB_L >= 3) {
  for (t2=1;t2<=min(floord(PB_L,2),PB_L-2);t2++) {
    for (t4=t2;t4<=min(PB_L-2,-t2+PB_L);t4++) {
      B[t2][t4] = D[t4][t2+1] - E[t2][t4+1][t2] - D[t4+t2-1][t2] - D[t4][t2] - B[t2+1][t4-1] + B[t2][t4-1] * B[t4+1][t2+1] + 2;;
    }
  }
}
/* End of CLooG code */
