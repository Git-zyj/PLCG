...
void kernel_2222132121_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_M >= 0) && (PB_N >= max(0,ceild(-4*PB_M+12,3)))) {
  for (t1=0;t1<=min(floord(PB_N+PB_M-1,32),floord(3*PB_N+4*PB_M-6,96));t1++) {
    for (t2=max(max(0,ceild(32*t1-PB_N,16)),ceild(96*t1-3*PB_N-2*PB_M+6,48));t2<=min(min(min(min(floord(PB_M,16),floord(-32*t1+PB_N+2*PB_M-2,16)),floord(32*t1+PB_M+31,32)),floord(2*t1+2*PB_M,33)),floord(PB_N+4*PB_M-4,64));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(min(15,PB_M-1),PB_N-2);t3++) {
          A[t3] = C[t3] * C[t3+1] * C[t3-2] - A[t3-1] + 3;;
          for (t4=2*t3;t4<=min(31,t3+PB_M);t4++) {
            B[(-2*t3+t4)+t3-1] = C[t3] + D[t3][(-2*t3+t4)+t3+2] - D[(-2*t3+t4)][0] - B[(-2*t3+t4)+t3-2] - 3;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(2,PB_M);t3<=min(15,PB_N-2);t3++) {
          A[t3] = C[t3] * C[t3+1] * C[t3-2] - A[t3-1] + 3;;
        }
      }
      if (t2 >= 1) {
        for (t3=max(max(2,32*t1),32*t2-PB_M);t3<=min(min(PB_M-1,32*t1+31),16*t2+15);t3++) {
          for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,t3+PB_M);t4++) {
            B[(-2*t3+t4)+t3-1] = C[t3] + D[t3][(-2*t3+t4)+t3+2] - D[(-2*t3+t4)][0] - B[(-2*t3+t4)+t3-2] - 3;;
          }
        }
      }
...
if ((PB_M <= -1) && (PB_N >= 4)) {
  for (t1=0;t1<=floord(PB_N-2,32);t1++) {
    for (t3=max(2,32*t1);t3<=min(PB_N-2,32*t1+31);t3++) {
      A[t3] = C[t3] * C[t3+1] * C[t3-2] - A[t3-1] + 3;;
    }
  }
}
if ((PB_M >= 3) && (PB_N <= -1)) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    for (t2=2*t1;t2<=min(floord(PB_M-1,16),floord(32*t1+PB_M+31,32));t2++) {
      for (t3=max(max(2,32*t1),32*t2-PB_M);t3<=min(min(PB_M-1,32*t1+31),16*t2+15);t3++) {
        for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,t3+PB_M);t4++) {
          B[(-2*t3+t4)+t3-1] = C[t3] + D[t3][(-2*t3+t4)+t3+2] - D[(-2*t3+t4)][0] - B[(-2*t3+t4)+t3-2] - 3;;
        }
...
kernel_2222132121_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
