...
void kernel_3124212223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
    ...
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t1-32*t2;t3<=min(PB_N-2,32*t1-32*t2+31);t3++) {
        for (t4=max(2,32*t2);t4<=min(PB_M-1,32*t2+31);t4++) {
          A[t3][t4] = B[t4][t3] - A[t3+1][t4] + A[t3][t4-2] + 6;;
        }
      }
    }
  }
...
kernel_3124212223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
