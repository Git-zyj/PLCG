...
void kernel_3122232123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_M >= 4) {
  for (t2=-PB_M+2;t2<=-2;t2++) {
    for (t4=0;t4<=t2+PB_M-2;t4++) {
      A[t4+-t2-1] = B[t4][-t2] + B[t4][-t2-2] - B[-t2][t4+-t2] * A[t4+-t2+1] - 4;;
    }
  }
}
/* End of CLooG code */
...
kernel_3122232123_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
