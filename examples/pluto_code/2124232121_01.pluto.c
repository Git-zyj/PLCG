...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(-32*t1+PB_M-2,32),t1);t2++) {
      for (t3=max(1,32*t1);t3<=min(min(PB_M-3,32*t1+31),-32*t2+PB_M-2);t3++) {
        for (t4=max(1,32*t2);t4<=min(min(t3,32*t2+31),-t3+PB_M-2);t4++) {
          A[t3+t4][t3] = B[t4-1][t3+2] + B[t3+2][t4+1] * C[t3][t4] - A[t3+t4+1][t3] * 1;;
        }
      }
    }
  }
