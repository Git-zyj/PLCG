...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(-PB_M-30,32),-t1+t2-1);t3<=min(floord(PB_L-5,32),floord(-32*t1+32*t2+PB_L-4,32));t3++) {
        for (t4=max(max(1,32*t1-32*t2),-32*t3-31);t4<=min(min(PB_M-1,32*t1-32*t2+31),-32*t3+PB_L-4);t4++) {
          for (t5=max(0,32*t3+t4);t5<=min(PB_L-4,32*t3+t4+31);t5++) {
            lbv=max(1,32*t2);
            ubv=min(PB_M-2,32*t2+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][t5+1][t6] = B[t6+1][t5] + A[t4-1][t5][t6-1] - A[t4][t5+3][t6] * 3;;
            }
          }
        }
      }
