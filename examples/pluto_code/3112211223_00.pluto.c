...
void kernel_3112211223_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if ((PB_M >= 3) && (PB_P >= 1)) {
  for (t1=0;t1<=floord(PB_M-3,8);t1++) {
    ...
    ubp=min(min(floord(3*PB_M-8,32),floord(64*t1+PB_M+60,96)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(32*t2-PB_M+2,2),32*t1-32*t2);t3<=min(min(PB_M-3,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,2*t3+PB_M-2);t4++) {
          for (t5=0;t5<=PB_P-1;t5++) {
            A[t3][(-2*t3+t4)][0] = B[t3][t5][(-2*t3+t4)] - A[t3+2][(-2*t3+t4)][0] - A[(-2*t3+t4)+1][t3][0] - 1;;
          }
        }
      }
    }
...
kernel_3112211223_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
