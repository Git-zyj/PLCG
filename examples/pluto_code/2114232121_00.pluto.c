...
void kernel_2114232121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
/* Start of CLooG code */
if (PB_M >= 4) {
  for (t1=0;t1<=floord(3*PB_M-5,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_M+1,32));
    ubp=min(floord(2*t1+1,3),floord(PB_M-2,16));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(3,32*t1-32*t2),16*t2+1);t3<=min(min(PB_M-1,32*t2+29),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+2);t4<=min(32*t2+31,2*t3-1);t4++) {
          A[(-t3+t4)] = B[(-t3+t4)] - C[(-t3+t4)] - D[(-t3+t4)-2][(-t3+t4)] * A[(-t3+t4)-1] * 5;;
        }
      }
    }
  }
...
kernel_2114232121_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
