...
void kernel_2114232121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,ceild(32*t2-PB_L+1,3)),32*t1-32*t2);t3<=min(min(floord(32*t2+31,3),PB_L-1),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,3*t3);t4<=min(32*t2+31,3*t3+PB_L-1);t4++) {
          A[t3-2][(-3*t3+t4)] = B[0][t3-2][t3] * C[(-3*t3+t4)][t3-2] * C[(-3*t3+t4)+1][t3+2] + A[(-3*t3+t4)][t3] - 3;;
        }
      }
    }
  }
...
kernel_2114232121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
