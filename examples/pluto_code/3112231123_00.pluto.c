...
void kernel_3112231123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t4=ceild(-PB_M-30,32);t4<=min(-1,-t2);t4++) {
      for (t6=max(32*t4,-PB_M+1);t6<=min(min(-2,-32*t2),32*t4+31);t6++) {
        lbv=max(1,32*t2);
        ubv=min(min(-t6,PB_L-1),32*t2+31);
#pragma ivdep
#pragma vector always
        for (t7=lbv;t7<=ubv;t7++) {
          A[-t6-1][t7] = B[t7-1] + C[-t6][0] * B[-t6] + A[-t6-2][t7] - 2;;
        }
      }
    }
  }
...
kernel_3112231123_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
