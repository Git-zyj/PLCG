...
if ((PB_M >= 3) && (PB_N >= 3)) {
  lbp=0;
  ubp=min(floord(PB_M-3,32),floord(PB_N-1,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(floord(PB_N-3,32),floord(-32*t1+PB_M-3,32)),t1);t2++) {
      for (t3=32*t2;t3<=min(min(min(floord(PB_M-3,2),PB_N-3),32*t2+31),-32*t1+PB_M-3);t3++) {
        lbv=max(32*t1,t3);
        ubv=min(min(PB_N-1,32*t1+31),-t3+PB_M-3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4+t3] = B[t4][t3] * A[t3+2][t4+t3+2] * 4;;
        }
      }
    }
  }
