...
void kernel_3214112123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_N >= 1) {
  for (t1=0;t1<=floord(PB_N-1,32);t1++) {
    for (t2=0;t2<=min(floord(64*t1+PB_N+59,32),floord(64*t1+31*PB_N-31,352));t2++) {
      if ((PB_N >= 4) && (t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=min(min(10,PB_M-1),PB_N-3);t3++) {
          A[t3+1] = C[t3] * C[t3] * 2;;
          for (t4=max(3*t3,2*t3+1);t4<=min(31,2*t3+PB_N-3);t4++) {
            B[(-2*t3+t4)] = B[(-2*t3+t4)-1] - B[(-2*t3+t4)+2] - 5;;
          }
        }
      }
      if ((PB_N <= 3) && (t1 == 0) && (t2 == 0)) {
      ...
      if (t2 == 0) {
        for (t3=max(11,32*t1);t3<=min(min(PB_M-1,PB_N-3),32*t1+31);t3++) {
          A[t3+1] = C[t3] * C[t3] * 2;;
        }
      }
      if (t2 >= 1) {
        for (t3=max(ceild(32*t2-PB_N+3,2),32*t1);t3<=min(min(min(floord(32*t2+31,3),PB_M-1),PB_N-3),32*t1+31);t3++) {
          for (t4=max(32*t2,3*t3);t4<=min(32*t2+31,2*t3+PB_N-3);t4++) {
            B[(-2*t3+t4)] = B[(-2*t3+t4)-1] - B[(-2*t3+t4)+2] - 5;;
          }
        }
      }
      if ((PB_N >= 4) && (t2 == 0)) {
        for (t3=max(32*t1,PB_N-2);t3<=min(min(PB_M-1,PB_N-1),32*t1+31);t3++) {
          A[t3+1] = C[t3] * C[t3] * 2;;
        }
      }
      if ((PB_N >= 4) && (t2 == 0)) {
        for (t3=max(PB_M,32*t1);t3<=min(PB_N-1,32*t1+31);t3++) {
          A[t3+1] = C[t3] * C[t3] * 2;;
        }
      }
    }
...
kernel_3214112123_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
