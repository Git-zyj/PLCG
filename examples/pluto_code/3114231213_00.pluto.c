...
void kernel_3114231213_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_L-1,32);t2++) {
      for (t3=t2;t3<=min(floord(PB_L-1,16),floord(32*t2+PB_L+30,32));t3++) {
        for (t4=max(max(1,32*t2),32*t3-PB_L+1);t4<=min(min(PB_L-1,32*t2+31),32*t3+29);t4++) {
          for (t5=max(32*t3,t4+2);t5<=min(32*t3+31,t4+PB_L-1);t5++) {
            lbv=max(1,32*t1);
            ubv=min(t4,32*t1+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[(-t4+t5)-1][t6-1][t4] = B[t4][(-t4+t5)][t6] - C[t4][(-t4+t5)-1] - D[(-t4+t5)][t4][t6+1] - A[(-t4+t5)][t6-1][t4-1] * A[(-t4+t5)-2][t6-1][t4] + 2;;
            }
          }
        }
      }
...
kernel_3114231213_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
