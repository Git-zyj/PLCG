...
void kernel_3222132223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_L >= 4) {
  for (t1=0;t1<=floord(PB_L,32);t1++) {
    for (t2=0;t2<=min(floord(PB_L-2,16),floord(32*t1+PB_L+28,32));t2++) {
      if (t2 >= 1) {
        for (t3=max(32*t1,32*t2-PB_L+3);t3<=min(min(PB_L-1,32*t1+31),32*t2+29);t3++) {
          for (t4=max(32*t2,t3+2);t4<=min(32*t2+31,t3+PB_L-3);t4++) {
            A[(-t3+t4)-1] = C[(-t3+t4)][t3] + C[t3+2][(-t3+t4)+1] * C[t3][(-t3+t4)+2] + C[t3][(-t3+t4)] * C[t3][(-t3+t4)] * A[(-t3+t4)-2] - 5;;
          }
        }
      }
      if ((PB_L >= 5) && (t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=3;t3++) {
          for (t4=t3+2;t4<=min(31,t3+PB_L-3);t4++) {
            A[(-t3+t4)-1] = C[(-t3+t4)][t3] + C[t3+2][(-t3+t4)+1] * C[t3][(-t3+t4)+2] + C[t3][(-t3+t4)] * C[t3][(-t3+t4)] * A[(-t3+t4)-2] - 5;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=4;t3<=min(29,PB_L-1);t3++) {
          B[t3-2] = D[t3-1] + B[t3-4] * B[t3-3] - 6;;
          for (t4=t3+2;t4<=min(31,t3+PB_L-3);t4++) {
            A[(-t3+t4)-1] = C[(-t3+t4)][t3] + C[t3+2][(-t3+t4)+1] * C[t3][(-t3+t4)+2] + C[t3][(-t3+t4)] * C[t3][(-t3+t4)] * A[(-t3+t4)-2] - 5;;
          }
        }
      ...
      if (t2 == 0) {
        for (t3=max(30,32*t1);t3<=min(PB_L,32*t1+31);t3++) {
          B[t3-2] = D[t3-1] + B[t3-4] * B[t3-3] - 6;;
        }
      }
      if ((PB_L <= 29) && (t1 == 0) && (t2 == 0)) {
        B[PB_L-2] = D[PB_L-1] + B[PB_L-4] * B[PB_L-3] - 6;;
      }
    }
  }
}
...
kernel_3222132223_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
