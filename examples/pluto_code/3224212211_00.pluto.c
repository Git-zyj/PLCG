...
void kernel_3224212211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_N >= 3) {
  ...
  for (t1=2;t1<=PB_N-2;t1++) {
    A[t1] = A[t1+1] * B[t1-1] + 3;;
    B[t1-1] = C[t1] - B[t1-2] * B[t1] + 6;;
  }
  B[(PB_N-1)-1] = C[(PB_N-1)] - B[(PB_N-1)-2] * B[(PB_N-1)] + 6;;
}
/* End of CLooG code */
...
kernel_3224212211_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
