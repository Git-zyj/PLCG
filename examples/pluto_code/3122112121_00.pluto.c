...
void kernel_3122112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_L >= 5) {
  for (t1=0;t1<=floord(PB_L-3,8);t1++) {
    ...
    ubp=min(floord(3*t1+2,4),floord(3*PB_L-7,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(3,ceild(32*t2+1,3)),32*t1-32*t2);t3<=min(min(PB_L-2,16*t2+14),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3+2);t4<=min(32*t2+31,3*t3-1);t4++) {
          A[(-2*t3+t4)][0] = B[(-2*t3+t4)][t3+1] + A[(-2*t3+t4)-2][0] - 2;;
        }
      }
    }
  }
...
kernel_3122112121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
