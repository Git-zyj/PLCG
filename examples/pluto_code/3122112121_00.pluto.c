...
void kernel_3122112121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5, t6;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_M >= 1) && (PB_P >= 1) && (PB_Q >= 2)) {
  lbp=0;
  ubp=floord(PB_Q-1,32);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_P-1,32);t2++) {
      for (t3=t2;t3<=min(floord(PB_P+PB_M-2,32),floord(32*t2+PB_M+30,32));t3++) {
        for (t4=max(1,32*t1);t4<=min(PB_Q-1,32*t1+31);t4++) {
          for (t5=max(32*t2,32*t3-PB_M+1);t5<=min(PB_P-1,32*t2+31);t5++) {
            for (t6=max(32*t3,t5);t6<=min(32*t3+31,t5+PB_M-1);t6++) {
              A[t4-1][t4][(-t5+t6)] = B[(-t5+t6)][t4+2][t5] * A[t4-1][t4][(-t5+t6)+1] + 6;;
            }
          }
        }
      }
...
kernel_3122112121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
