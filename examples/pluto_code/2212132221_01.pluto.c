...
void kernel_2212132221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 4) && (PB_M >= 6) && (PB_N >= 3)) {
  lbp=0;
  ubp=min(floord(PB_M-4,32),floord(PB_N-1,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(floord(PB_L-3,32),floord(PB_M-5,32)),floord(-32*t1+PB_M-3,32));t2++) {
      for (t3=max(2,32*t1);t3<=min(min(min(PB_M-4,PB_N-1),32*t1+31),-32*t2+PB_M-3);t3++) {
        for (t4=max(1,32*t2);t4<=min(min(PB_L-3,32*t2+31),-t3+PB_M-3);t4++) {
          A[t3+t4][t4] = C[t3] + D[t4][t3] + B[t3+t4+2][t4] + 4;;
          B[t3+t4+2][t4] = D[t3-2][t4] + D[t3+t4+1][t4] * C[t3-1] * A[t3+t4][t4] - A[t3+t4-1][t4-1] * A[t3+t4+2][t4+2] * 3;;
        }
      }
    }
  }
...
kernel_2212132221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
