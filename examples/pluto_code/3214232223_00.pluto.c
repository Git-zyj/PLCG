...
void kernel_3214232223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_L >= 3) {
  for (t1=2;t1<=PB_L-1;t1++) {
    A[t1] = C[t1+1] - D[t1-1] - D[t1] + C[t1+2] + D[t1] + 2;;
    A[t1-2] = C[t1-1] + A[t1-1] + A[t1] - 5;;
  }
}
/* End of CLooG code */
...
kernel_3214232223_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
