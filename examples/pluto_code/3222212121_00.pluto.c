...
if (PB_N >= 6) {
  if (PB_L >= 0) {
    for (t1=0;t1<=floord(PB_N+PB_L-4,32);t1++) {
      if (t1 <= floord(PB_N-4,32)) {
        for (t4=max(2,32*t1);t4<=min(PB_N-4,32*t1+31);t4++) {
          A[t4+2] = C[t4-1] * C[t4-2] - A[t4+1] - A[t4+3] * 2;;
        }
      }
      if (t1 <= floord(PB_L-2,32)) {
        for (t2=0;t2<=floord(PB_N-4,32);t2++) {
          for (t3=max(2,32*t2);t3<=(min(PB_N-4,32*t2+31))-7;t3+=8) {
