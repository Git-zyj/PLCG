...
void kernel_3112212123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if ((PB_L >= 5) && (PB_M >= 1)) {
  lbp=0;
  ubp=min(floord(PB_M+1,32),floord(PB_L-1,64));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=min(min(floord(PB_L-3,32),floord(PB_M+1,32)),floord(-32*t1+PB_L-1,32));t2++) {
      for (t3=max(2,32*t2);t3<=min(min(min(PB_L-3,PB_M+1),32*t2+31),-32*t1+PB_L-1);t3++) {
        lbv=max(2,32*t1);
        ubv=min(min(t3,32*t1+31),-t3+PB_L-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+t4][t4] = B[t3-2] * A[t3+t4-2][t4] - 6;;
        }
      }
    }
  }
...
kernel_3112212123_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
