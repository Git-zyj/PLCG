...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
      for (t2=lbp;t2<=ubp;t2++) {
        for (t3=0;t3<=floord(PB_L,32);t3++) {
          for (t4=32*t2;t4<=(min(PB_L-1,32*t2+31))-7;t4+=8) {
            for (t5=max(1,32*t3);t5<=min(PB_L,32*t3+31);t5++) {
              A[t1+2][t4][t4] = A[t1+3][t5-1][t5-1] + 2;;
              A[t1+2][(t4+1)][(t4+1)] = A[t1+3][t5-1][t5-1] + 2;;
              A[t1+2][(t4+2)][(t4+2)] = A[t1+3][t5-1][t5-1] + 2;;
              A[t1+2][(t4+3)][(t4+3)] = A[t1+3][t5-1][t5-1] + 2;;
              A[t1+2][(t4+4)][(t4+4)] = A[t1+3][t5-1][t5-1] + 2;;
              A[t1+2][(t4+5)][(t4+5)] = A[t1+3][t5-1][t5-1] + 2;;
              A[t1+2][(t4+6)][(t4+6)] = A[t1+3][t5-1][t5-1] + 2;;
              A[t1+2][(t4+7)][(t4+7)] = A[t1+3][t5-1][t5-1] + 2;;
            }
          }
          for (;t4<=min(PB_L-1,32*t2+31);t4++) {
            for (t5=max(1,32*t3);t5<=min(PB_L,32*t3+31);t5++) {
              A[t1+2][t4][t4] = A[t1+3][t5-1][t5-1] + 2;;
            }
          }
