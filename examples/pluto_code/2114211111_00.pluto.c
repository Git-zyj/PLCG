...
/* Start of CLooG code */
if (PB_L >= 3) {
  lbp=ceild(-PB_L-28,32);
  ubp=0;
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,-t1-1);t2<=min(floord(PB_L-2,32),floord(-32*t1+PB_L-1,64));t2++) {
      for (t3=max(1,32*t1+32*t2);t3<=min(min(min(min(floord(PB_L-1,2),floord(32*t1+PB_L+30,2)),32*t2+31),32*t1+32*t2+62),-32*t2+PB_L-1);t3++) {
        lbv=max(max(32*t2,t3),-32*t1+t3-31);
        ubv=min(min(-32*t1+t3,32*t2+31),-t3+PB_L-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4] = B[t3][t4+t3] * A[t3-1][t4-1] * 3;;
        }
      }
    }
  }
