...
void kernel_2212212223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 0) && (PB_M >= max(0,-PB_L+4))) {
  for (t1=0;t1<=min(floord(PB_M+PB_L-1,32),floord(4*PB_M+5*PB_L-8,128));t1++) {
    for (t2=0;t2<=min(min(floord(PB_M,32),floord(2*PB_M+PB_L-4,64)),t1);t2++) {
      if (t2 >= 1) {
        for (t3=32*t1;t3<=min(PB_M-2,32*t1+31);t3++) {
          for (t4=32*t2;t4<=min(t3,32*t2+31);t4++) {
            A[(t3-t4)+t4][t4] = A[(t3-t4)+t4][t4-2] + A[(t3-t4)+t4+1][t4+1] * 4;;
          }
        }
      }
      if ((PB_M >= 4) && (t1 == 0) && (t2 == 0)) {
        A[0 +2][2] = A[0 +2][2 -2] + A[0 +2 +1][2 +1] * 4;;
      }
      if (t2 == 0) {
        for (t3=max(3,32*t1);t3<=min(min(PB_L-1,PB_M-2),32*t1+31);t3++) {
          B[t3-2] = C[t3][0] + D[t3] - B[t3-3] * 5;;
      ...
      if (t2 == 0) {
        for (t3=max(max(3,PB_L),32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
          for (t4=2;t4<=min(31,t3);t4++) {
            A[(t3-t4)+t4][t4] = A[(t3-t4)+t4][t4-2] + A[(t3-t4)+t4+1][t4+1] * 4;;
          }
        }
      ...
      if (t2 == 0) {
        for (t3=max(max(3,32*t1),PB_M-1);t3<=min(PB_L-1,32*t1+31);t3++) {
          B[t3-2] = C[t3][0] + D[t3] - B[t3-3] * 5;;
        }
      }
    }
  ...
  for (t1=0;t1<=floord(PB_M-2,32);t1++) {
    for (t2=0;t2<=t1;t2++) {
      for (t3=max(2,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
        for (t4=max(2,32*t2);t4<=min(t3,32*t2+31);t4++) {
          A[(t3-t4)+t4][t4] = A[(t3-t4)+t4][t4-2] + A[(t3-t4)+t4+1][t4+1] * 4;;
        }
      }
    }
...
kernel_2212212223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
