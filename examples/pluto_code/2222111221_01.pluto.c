...
void kernel_2222111221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
  int t1;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
for (t1=1;t1<=min(PB_N,PB_M-1);t1++) {
  A[t1+1] = C[t1-1] - A[t1+2] + A[t1] * A[t1+3] * 6;;
  B[t1] = B[t1-1] - 1;;
}
for (t1=max(1,PB_M);t1<=PB_N;t1++) {
  A[t1+1] = C[t1-1] - A[t1+2] + A[t1] * A[t1+3] * 6;;
}
...
kernel_2222111221_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
