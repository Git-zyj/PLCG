...
void kernel_3112232121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-3,32);t2++) {
      for (t3=max(2,32*t2);t3<=min(PB_L-3,32*t2+31);t3++) {
        lbv=32*t1;
        ubv=min(PB_P-1,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4] = B[t3] - C[t4][t3+2] - C[t4][t3] * A[t3-2][t4] * 4;;
        }
      }
    }
  }
...
kernel_3112232121_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
