...
void kernel_2114131211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_M >= 4) {
  for (t1=0;t1<=floord(PB_M-2,16);t1++) {
    lbp=ceild(t1,2);
    ubp=min(floord(2*t1+1,3),floord(PB_M-1,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t1-32*t2),16*t2+1);t3<=min(min(PB_M-2,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(min(PB_M-1,32*t2+31),2*t3-1);t4++) {
          A[(-t3+t4)] = B[(-t3+t4)][t3] - B[(-t3+t4)-1][(-t3+t4)+1] * B[(-t3+t4)+t3][1] * A[(-t3+t4)+1] - A[(-t3+t4)-1] - 3;;
        }
      }
    }
  }
...
kernel_2114131211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
