...
void kernel_2214231121_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_M >= max(0,ceild(-3*PB_P+6,2))) && (PB_P >= 0)) {
  for (t1=0;t1<=floord(PB_M+PB_P-1,32);t1++) {
    for (t2=0;t2<=min(floord(PB_P,32),floord(PB_M+3*PB_P-3,96));t2++) {
...
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t3-1][t4] = E[t3][t4] - E[t4][t3] * E[t4-1][t3] + 6;;
          }
        }
      }
      if ((PB_P >= 2) && (t1 == 0) && (t2 == 0)) {
...
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          B[1 -1][t4] = E[1][t4] - E[t4][1] * E[t4-1][1] + 6;;
        }
      }
      if (t2 == 0) {
        for (t3=max(2,32*t1);t3<=min(min(PB_M-1,PB_P-1),32*t1+31);t3++) {
          A[t3] = C[t3] * C[t3] * D[t3] + A[t3-1] + A[t3-2] - 2;;
          lbv=1;
          ubv=min(31,PB_P-1);
#pragma ivdep
#pragma vector always
      ...
      if (t2 == 0) {
        for (t3=max(max(2,PB_P),32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
          A[t3] = C[t3] * C[t3] * D[t3] + A[t3-1] + A[t3-2] - 2;;
        }
      }
      if (t2 == 0) {
        for (t3=max(max(2,PB_M),32*t1);t3<=min(PB_P-1,32*t1+31);t3++) {
          lbv=1;
          ubv=min(31,PB_P-1);
#pragma ivdep
#pragma vector always
...
kernel_2214231121_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
