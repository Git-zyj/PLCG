...
void kernel_2224111123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 1) {
  ...
  for (t1=max(2,PB_M);t1<=PB_M+1;t1++) {
    A[(t1-2)] = C[(t1-2)] * 2;;
  }
}
/* End of CLooG code */
...
kernel_2224111123_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
