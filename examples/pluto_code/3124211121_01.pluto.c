...
void kernel_3124211121_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5, t6, t7, t8, t9, t10;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_M >= 1) && (PB_Q >= 5)) {
  for (t2=0;t2<=min(floord(PB_L-1,32),floord(PB_Q-3,64));t2++) {
    lbp=0;
    ubp=min(min(floord(PB_L-2,32),floord(PB_M-1,32)),floord(-32*t2+PB_L-1,32));
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8,t9,t10)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t5=max(1,32*t2);t5<=min(min(floord(PB_Q-3,2),32*t2+31),-32*t4+PB_L-1);t5++) {
        for (t8=t5-PB_Q+3;t8<=-t5;t8++) {
          lbv=32*t4;
          ubv=min(min(PB_M-1,32*t4+31),-t5+PB_L-1);
#pragma ivdep
#pragma vector always
          for (t9=lbv;t9<=ubv;t9++) {
            A[t9][t9+1][-t8+t5] = B[-t8][t5-1][t9+t5] + A[t9][t9+1][-t8+t5+2] * 6;;
          }
        }
      }
    }
...
kernel_3124211121_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
