...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,ceild(32*t2-PB_M+1,3)),32*t1-32*t2);t3<=min(min(floord(32*t2+30,3),PB_M-2),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,3*t3+1);t4<=min(32*t2+31,3*t3+PB_M-1);t4++) {
          A[(-3*t3+t4)-1][t3] = B[(-3*t3+t4)][t3] - C[(-3*t3+t4)-1][t3] - D[(-3*t3+t4)] + A[t3-2][(-3*t3+t4)-1] * A[(-3*t3+t4)-1][t3+1] * 2;;
        }
      }
    }
  }
