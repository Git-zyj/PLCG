...
void kernel_2114112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_L >= 1) && (PB_M >= 6)) {
  lbp=0;
  ubp=min(floord(PB_L+1,32),floord(PB_M-4,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_M-3,32);t2++) {
      for (t3=max(max(3,32*t2),32*t1+1);t3<=min(PB_M-3,32*t2+31);t3++) {
        lbv=max(2,32*t1);
        ubv=min(min(PB_L+1,32*t1+31),t3-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4-2] = B[t4+2][t3] + A[t3+2][t4-2] * 4;;
        }
      }
    }
  }
...
kernel_2114112121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
