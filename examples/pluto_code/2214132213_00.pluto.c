...
void kernel_2214132213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 3)) {
  for (t1=0;t1<=min(floord(PB_M-1,16),floord(PB_L+PB_M-3,32));t1++) {
    ...
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(2,32*t2);t3<=min(PB_M-1,32*t2+31);t3++) {
        for (t4=max(1,32*t1-32*t2);t4<=min(min(t3,PB_L-2),32*t1-32*t2+31);t4++) {
          A[t3][t4] = C[t3][t4] * D[t3-2] * C[t3][t4] * 3;;
          A[t3-1][t4-1] = D[t3] * C[t4+1][t3+1] - C[t4-1][t4] - A[t3][t4] * A[t3-1][t4] - 4;;
        }
      }
    }
  }
...
kernel_2214132213_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
