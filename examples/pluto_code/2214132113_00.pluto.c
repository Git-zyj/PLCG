...
void kernel_2214132113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_N >= 1) {
  for (t2=2;t2<=PB_N;t2++) {
    A[t2-1] = C[t2] - D[t2+1][t2] + C[t2] - A[t2-2] + 5;;
  }
  if (PB_M >= 1) {
    lbp=0;
    ubp=floord(PB_N,32);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=min(floord(PB_M-1,32),t2);t3++) {
        for (t4=max(1,32*t2);t4<=min(PB_N,32*t2+31);t4++) {
          for (t5=32*t3;t5<=min(min(t4,PB_M-1),32*t3+31);t5++) {
            A[t4-1] = D[t4][t5+1] * D[t5][t4] * D[t4][t5+1] * 5;;
          }
        }
      }
    }
...
kernel_2214132113_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
