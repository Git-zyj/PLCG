...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_M+1);t3<=min(min(32*t2,PB_M-1),32*t1-32*t2+31);t3++) {
        for (t4=32*t2;t4<=min(32*t2+31,t3+PB_M-1);t4++) {
          A[t3][(-t3+t4)+2] = C[(-t3+t4)][t3] + A[t3+1][(-t3+t4)+1] + 4;;
        }
      }
      if ((PB_M >= 2) && (t1 == 0) && (t2 == 0)) {
        for (t4=1;t4<=min(31,PB_M);t4++) {
          A[1][(t4-1)+2] = C[(t4-1)][1] + A[1 +1][(t4-1)+1] + 4;;
        }
      }
      for (t3=max(32*t1-32*t2,32*t2+32);t3<=min(PB_M-3,32*t1-32*t2+31);t3++) {
        for (t4=max(1,32*t2);t4<=32*t2+31;t4++) {
          B[t4+2][t3-1][t4] = C[t3+2][t4-1] + B[t4+2][t3-2][t4] - B[t4+3][t3-1][t4+1] + B[t3+2][t4][t3] * 2;;
        }
      }
      if (t1 == 2*t2) {
        for (t3=max(2,16*t1+1);t3<=min(PB_M-3,16*t1+31);t3++) {
      ...
      if (t1 == 2*t2) {
        for (t3=max(max(2,PB_M-2),16*t1+1);t3<=min(PB_M-1,16*t1+31);t3++) {
          for (t4=t3;t4<=min(16*t1+31,t3+PB_M-1);t4++) {
            if (t1%2 == 0) {
              A[t3][(-t3+t4)+2] = C[(-t3+t4)][t3] + A[t3+1][(-t3+t4)+1] + 4;;
            }
