...
void kernel_2224111111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
for (t1=max(1,PB_L);t1<=PB_M-2;t1++) {
  B[t1+1] = C[t1] * 2;;
}
/* End of CLooG code */
...
kernel_2224111111_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
