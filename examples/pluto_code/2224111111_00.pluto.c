...
void kernel_2224111111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      ...
      if (32*t1 == 32*t2+PB_M) {
        for (t3=max(1,32*t1-PB_M);t3<=32*t1-PB_M+31;t3++) {
          if (PB_M%32 == 0) {
            B[t3][PB_M] = C[PB_M-1][t3] * B[t3-1][PB_M] - B[t3][PB_M-1] * 2;;
          }
        }
      }
      if (t1 <= floord(32*t2+PB_M-1,32)) {
        for (t3=max(1,32*t2);t3<=min(min(PB_M-1,32*t2+31),32*t1-32*t2+30);t3++) {
          if (t1 == 2*t2) {
          ...
          for (t4=max(32*t1-32*t2,t3+1);t4<=min(PB_M-1,32*t1-32*t2+31);t4++) {
            A[t4][t3] = C[t3][t4+1] + 4;;
            B[t3][t4] = C[t4-1][t3] * B[t3-1][t4] - B[t3][t4-1] * 2;;
          }
          if (t1 >= ceild(32*t2+PB_M-31,32)) {
            B[t3][PB_M] = C[PB_M-1][t3] * B[t3-1][PB_M] - B[t3][PB_M-1] * 2;;
          }
        }
      }
      for (t3=max(32*t2,32*t1-32*t2+31);t3<=min(PB_M-1,32*t2+31);t3++) {
...
kernel_2224111111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
