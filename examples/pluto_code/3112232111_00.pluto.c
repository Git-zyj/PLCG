...
void kernel_3112232111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
/* Start of CLooG code */
if ((PB_M >= 3) && (PB_Q >= 3)) {
  for (t1=1;t1<=PB_Q-2;t1++) {
    for (t2=0;t2<=min(floord(PB_M-3,8),floord(t1+3*PB_M-7,32));t2++) {
      lbp=max(ceild(2*t2,3),ceild(32*t2-PB_M+2,32));
      ubp=min(min(min(min(floord(3*PB_M-7,32),floord(t1+2*PB_M-5,32)),floord(t1+64*t2+61,96)),floord(64*t2+PB_M+59,96)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(max(1,ceild(-t1+32*t3+1,2)),ceild(32*t3-PB_M+3,2)),32*t2-32*t3);t4<=min(min(PB_M-2,16*t3+15),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,2*t4);t5<=min(min(32*t3+31,t1+2*t4-1),2*t4+PB_M-3);t5++) {
            A[t4][(-2*t4+t5)+2] = B[t4-1][(-2*t4+t5)][1] + C[(-2*t4+t5)][t4][t1+1] + C[t4][t1][(-2*t4+t5)] + A[(-2*t4+t5)][t4+1] + 2;;
          }
        }
      }
    }
...
kernel_3112232111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
