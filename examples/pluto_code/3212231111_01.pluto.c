...
void kernel_3212231111_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
if (PB_L >= 2) {
  for (t1=0;t1<=floord(PB_L-1,32);t1++) {
    for (t2=0;t2<=min(floord(PB_L-2,32),t1);t2++) {
      if (t2 >= 1) {
        for (t3=max(32*t1,32*t2+1);t3<=min(PB_L-1,32*t1+31);t3++) {
          for (t4=32*t2;t4<=(min(32*t2+31,t3-1))-7;t4+=8) {
            for (t5=0;t5<=PB_L-1;t5++) {
              A[t3][0][t5] = C[t3][t5][t4] + A[t3][0][t4-1] + 4;;
              A[t3][0][t5] = C[t3][t5][(t4+1)] + A[t3][0][(t4+1)-1] + 4;;
              A[t3][0][t5] = C[t3][t5][(t4+2)] + A[t3][0][(t4+2)-1] + 4;;
              A[t3][0][t5] = C[t3][t5][(t4+3)] + A[t3][0][(t4+3)-1] + 4;;
              A[t3][0][t5] = C[t3][t5][(t4+4)] + A[t3][0][(t4+4)-1] + 4;;
              A[t3][0][t5] = C[t3][t5][(t4+5)] + A[t3][0][(t4+5)-1] + 4;;
              A[t3][0][t5] = C[t3][t5][(t4+6)] + A[t3][0][(t4+6)-1] + 4;;
              A[t3][0][t5] = C[t3][t5][(t4+7)] + A[t3][0][(t4+7)-1] + 4;;
            }
          }
          for (;t4<=min(32*t2+31,t3-1);t4++) {
            for (t5=0;t5<=PB_L-1;t5++) {
              A[t3][0][t5] = C[t3][t5][t4] + A[t3][0][t4-1] + 4;;
            }
          }
      ...
      if ((t1 == 0) && (t2 == 0)) {
        B[1] = D[1] * E[1] * E[1 +1] * E[1] - E[1 -1] - B[1 +1] - 5;;
      }
      if (t2 == 0) {
        for (t3=max(2,32*t1);t3<=min(PB_L-1,32*t1+31);t3++) {
          B[t3] = D[t3] * E[t3] * E[t3+1] * E[1] - E[t3-1] - B[t3+1] - 5;;
          for (t4=1;t4<=(min(31,t3-1))-7;t4+=8) {
            for (t5=0;t5<=PB_L-1;t5++) {
              A[t3][0][t5] = C[t3][t5][t4] + A[t3][0][t4-1] + 4;;
              A[t3][0][t5] = C[t3][t5][(t4+1)] + A[t3][0][(t4+1)-1] + 4;;
              A[t3][0][t5] = C[t3][t5][(t4+2)] + A[t3][0][(t4+2)-1] + 4;;
...
kernel_3212231111_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
