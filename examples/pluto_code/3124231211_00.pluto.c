...
/* Start of CLooG code */
if ((PB_M >= 2) && (PB_P >= 1)) {
  for (t2=ceild(-PB_M-29,32);t2<=0;t2++) {
    lbp=0;
    ubp=min(min(floord(PB_M-1,32),floord(PB_P-1,32)),floord(32*t2+PB_M+30,32));
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t5=max(max(32*t2,-PB_M+2),32*t4-PB_M+1);t5<=min(0,32*t2+31);t5++) {
        for (t7=32*t4;t7<=min(min(PB_P-1,32*t4+31),t5+PB_M-1);t7++) {
          A[-t5+1][t7+1] = B[-t5][t7] - C[-t5][t7] - C[t7+-t5][t7] + A[-t5+1][t7+2] - A[t7][-t5+1] - 2;;
        }
      }
    }
  }
