...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_P >= 2)) {
  for (t1=0;t1<=min(floord(PB_L-2,16),floord(PB_L+PB_P-4,32));t1++) {
    lbp=max(0,ceild(32*t1-PB_L+2,32));
    ubp=min(min(floord(PB_L-1,32),floord(PB_P-2,32)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=t2;t3<=floord(PB_P-2,32);t3++) {
        for (t4=max(1,32*t1-32*t2);t4<=min(PB_L-2,32*t1-32*t2+31);t4++) {
          for (t5=32*t2;t5<=min(min(PB_L-1,PB_P-2),32*t2+31);t5++) {
            for (t6=max(32*t3,t5);t6<=min(PB_P-2,32*t3+31);t6++) {
              A[t5+1][t4][t6] = B[t6][t5][t4] + C[t5][t6][t4] - B[t5][t6][t4-1] - A[t5+2][t4+1][t6] - A[t5+1][t4][t6+1] + 6;;
            }
          }
        }
      }
