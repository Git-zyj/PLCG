...
void kernel_3212112111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 1) {
  if (PB_N >= 0) {
    for (t1=0;t1<=min(floord(PB_N+PB_L-1,32),floord(PB_N+2*PB_L-3,32));t1++) {
      for (t2=0;t2<=min(min(floord(PB_L-1,32),floord(-32*t1+PB_N+PB_L-1,32)),floord(-32*t1+PB_N+2*PB_L-3,64));t2++) {
        if (t2 >= 1) {
          lbp=32*t1;
          ubp=min(PB_N-1,32*t1+31);
...
#pragma vector always
        ...
        if (t2 == 0) {
          for (t3=max(1,32*t1);t3<=min(min(PB_L-1,PB_N-1),32*t1+31);t3++) {
            A[t3][0] = 1;;
            B[t3] = C[t3-1] + C[t3] - B[t3+1] + B[t3-1] * 2;;
            lbv=1;
            ubv=min(31,PB_L-1);
#pragma ivdep
#pragma vector always
...
#pragma vector always
        ...
        if (t2 == 0) {
          for (t3=max(max(1,PB_N),32*t1);t3<=min(PB_L-1,32*t1+31);t3++) {
            B[t3] = C[t3-1] + C[t3] - B[t3+1] + B[t3-1] * 2;;
          }
        }
      }
...
kernel_3212112111_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
