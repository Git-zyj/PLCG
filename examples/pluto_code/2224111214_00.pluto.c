...
void kernel_2224111214_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
for (t1=0;t1<=floord(PB_N-1,32);t1++) {
  for (t2=max(0,ceild(2848*t1-89*PB_N+178,32));t2<=min(floord(PB_N+PB_M-3,32),floord(32*t1+PB_M+29,32));t2++) {
    if ((t1 == 0) && (t2 == 0)) {
      for (t3=0;t3<=30;t3++) {
        B[t3+1] = C[0] + B[t3] - 2;;
        for (t4=t3+1;t4<=31;t4++) {
          A[(-t3+t4)][(-t3+t4)] = A[(-t3+t4)-1][(-t3+t4)-1] - A[(-t3+t4)+1][(-t3+t4)+1] - A[(-t3+t4)-1][(-t3+t4)-1] * 4;;
        }
      }
    }
    if (t2 >= 1) {
      for (t3=max(32*t1,32*t2-PB_M+2);t3<=min(min(PB_N-1,32*t1+31),32*t2+30);t3++) {
    ...
    if (t2 == 0) {
      for (t3=max(31,32*t1);t3<=min(PB_N-2,32*t1+31);t3++) {
        B[t3+1] = C[0] + B[t3] - 2;;
      }
    }
  }
...
kernel_2224111214_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
