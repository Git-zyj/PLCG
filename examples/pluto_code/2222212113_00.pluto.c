...
void kernel_2222212113_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      ...
      for (t3=32*t2;t3<=min(min(PB_L-1,32*t2+31),32*t1-32*t2-1);t3++) {
        lbv=32*t1-32*t2;
...
#pragma vector always
      ...
      if (PB_P >= 3) {
        for (t3=max(max(1,32*t2),32*t1-32*t2);t3<=min(PB_L-1,32*t2+31);t3++) {
          if (t1 == t2) {
            B[t3][0 +1] = C[0][t3] - 5;;
          }
          lbv=max(1,32*t1-32*t2);
          ubv=min(min(t3,PB_P-2),32*t1-32*t2+31);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t3][t4+1] = C[t4][t3] - 5;;
            B[(t3-1)][(t4-1)+1] = C[(t4-1)][(t3-1)] * B[(t3-1)+1][(t4-1)+2] * 4;;
          }
          if (t1 == 2*t2) {
            lbv=t3+1;
...
kernel_2222212113_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
