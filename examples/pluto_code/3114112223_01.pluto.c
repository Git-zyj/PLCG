...
/* Start of CLooG code */
if (PB_M >= 3) {
  for (t1=0;t1<=floord(3*PB_M,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_M,32));
    ubp=min(min(floord(PB_M,16),floord(32*t1+PB_M+31,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,32*t1-32*t2),32*t2-PB_M);t3<=min(min(PB_M,32*t2+28),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+3);t4<=min(32*t2+31,t3+PB_M);t4++) {
          A[(-t3+t4)-2] = B[t3-1] + A[(-t3+t4)-1] - A[(-t3+t4)-3] * 3;;
        }
      }
    }
  }
