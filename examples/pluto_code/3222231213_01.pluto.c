...
void kernel_3222231213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma vector always
  ...
  if (PB_M >= 4) {
    for (t2=0;t2<=min(min(floord(PB_M-3,16),floord(3*PB_P-6,32)),floord(PB_P+PB_M-4,32));t2++) {
      lbp=max(ceild(t2,2),ceild(32*t2-PB_P+2,32));
      ubp=min(min(min(floord(PB_P-2,16),floord(PB_M-2,32)),floord(32*t2+PB_P+30,64)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(2,32*t3),32*t2-32*t3+1);t4<=min(min(min(PB_M-2,2*PB_P-3),32*t3+31),32*t2-32*t3+PB_P+30);t4++) {
          for (t5=max(max(1,32*t2-32*t3),t4-PB_P+1);t5<=min(min(PB_P-2,t4-1),32*t2-32*t3+31);t5++) {
            B[(t4-t5)+t5][t5] = D[(t4-t5)+t5][(t4-t5)-1] - D[(t4-t5)+1][t5+1] + D[1][t5] + E[(t4-t5)+t5-1][0] + A[t5+1] + B[(t4-t5)+t5-1][t5] + B[(t4-t5)+t5][t5+1] * B[(t4-t5)+t5+1][t5+1] + 2;;
          }
        }
      }
    }
...
kernel_3222231213_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
