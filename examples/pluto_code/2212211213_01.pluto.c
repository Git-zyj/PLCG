...
void kernel_2212211213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M,32);t1++) {
    if ((PB_L <= -1) && (PB_M >= 3) && (t1 <= floord(PB_M-2,32))) {
      for (t3=max(1,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
        B[t3] = C[t3-1] - B[t3+1] * 6;;
      }
    }
    if ((PB_M >= ceild(-PB_L+9,3)) && (t1 <= floord(3*PB_M+2*PB_L-6,96))) {
      for (t2=0;t2<=min(min(floord(PB_L,32),floord(2*PB_M+PB_L-6,32)),floord(-32*t1+PB_M+PB_L-2,32));t2++) {
        if ((t2 >= 1) && (t2 <= floord(PB_L-2,32))) {
          for (t3=max(2,32*t1);t3<=min(PB_M,32*t1+31);t3++) {
            for (t4=32*t2;t4<=min(PB_L-2,32*t2+31);t4++) {
              A[t3-1][t4] = A[t3-2][t4-1] + A[t3-1][t4+1] - 2;;
            }
          }
        }
        if ((PB_L <= 2) && (t2 == 0)) {
        ...
        if ((PB_L >= 3) && (t2 == 0)) {
          for (t3=max(max(2,32*t1),PB_M-1);t3<=min(PB_M,32*t1+31);t3++) {
            for (t4=1;t4<=min(31,PB_L-2);t4++) {
              A[t3-1][t4] = A[t3-2][t4-1] + A[t3-1][t4+1] - 2;;
            }
          }
...
kernel_2212211213_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
