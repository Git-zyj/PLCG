...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=floord(PB_L-2,32);t3++) {
        for (t4=max(2,32*t1-32*t2);t4<=min(min(min(PB_L+1,PB_M-2),-32*t2+PB_M),32*t1-32*t2+31);t4++) {
          for (t5=max(2,32*t2);t5<=min(min(PB_M-3,32*t2+31),-t4+PB_M);t5++) {
            lbv=32*t3;
            ubv=min(PB_L-2,32*t3+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][t5+2][t6+1] = B[t5-2][t4-2][t6] + B[t5+t4-1][t6][2] * B[t4][t6][t5] * A[t4-1][t5+2][t6] + A[t4][t5+1][t6+1] - 1;;
            }
          }
        }
      }
