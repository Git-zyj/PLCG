...
void kernel_2122211111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(floord(-32*t1+PB_N-1,32),floord(PB_N-2,64)),t1);t2++) {
      for (t3=max(32*t1,32*t2+1);t3<=min(32*t1+31,-32*t2+PB_N-1);t3++) {
        lbv=32*t2;
        ubv=min(min(32*t2+31,t3-1),-t3+PB_N-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t4][t3] = B[t4+t3][0] - A[t3+1][t4] + 6;;
        }
      }
    }
  }
...
kernel_2122211111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
