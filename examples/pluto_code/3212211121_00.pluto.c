...
void kernel_3212211121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_M >= 1) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    ...
    for (t2=0;t2<=min(floord(PB_L,32),floord(-64*t1+2*PB_M+PB_L-4,32));t2++) {
      if ((PB_L >= 2) && (t1 <= floord(PB_M-2,32)) && (t2 == 0)) {
        for (t4=32*t1;t4<=min(PB_M-2,32*t1+31);t4++) {
          A[0][t4] = C[0][t4] + A[0 +1][t4] - 6;;
          B[t4+1] = B[t4] * 6;;
        }
        if (t1 >= ceild(PB_M-32,32)) {
          A[0][(PB_M-1)] = C[0][(PB_M-1)] + A[0 +1][(PB_M-1)] - 6;;
        }
      }
...
kernel_3212211121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
