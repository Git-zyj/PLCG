...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=floord(PB_L-3,32);t3++) {
        for (t4=max(3,32*t3);t4<=min(PB_L-3,32*t3+31);t4++) {
          lbv=32*t2;
          ubv=min(PB_P-1,32*t2+31);
#pragma ivdep
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            B[t4-2][t4+2][t5] = C[t4][t5] - B[t4-3][t4+1][t5] * 4;;
          }
        }
      }
    }
    ...
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=floord(PB_L-1,32);t3++) {
        for (t4=0;t4<=floord(PB_M-1,32);t4++) {
          for (t5=max(2,32*t2);t5<=min(PB_L-3,32*t2+31);t5++) {
            for (t6=max(2,32*t3);t6<=min(PB_L-1,32*t3+31);t6++) {
              for (t7=max(2,32*t4);t7<=min(PB_M-1,32*t4+31);t7++) {
                B[t5-2][t5+2][t6] = D[t5-2][t7-2][t6+1] - D[t6][t7-1][t5] - D[t5][t6+1][t7] - C[t7][t6] - D[t6-2][t5+2][t7] * 4;;
              }
            }
          }
        }
