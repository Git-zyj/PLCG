...
void kernel_3112131221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 5)) {
  for (t1=2;t1<=min(PB_L-1,PB_M-3);t1++) {
    for (t2=1;t2<=min(t1-1,-t1+PB_M-2);t2++) {
      A[t1] = B[t2+t1][t1] * B[t2+1][t1] - B[t2+t1+1][t2] + A[t1-1] + A[t2-1] * 3;;
    }
  }
}
/* End of CLooG code */
...
kernel_3112131221_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
