...
void kernel_3112231113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(1,32*t1-32*t2);t3<=min(PB_M-1,32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3);t4<=min(PB_M,32*t2+31);t4++) {
          A[(-t3+t4)] = B[(-t3+t4)+1][t3] + B[t3][(-t3+t4)] - C[(-t3+t4)+t3][(-t3+t4)+t3-1][(-t3+t4)] * A[(-t3+t4)+1] + 6;;
        }
      }
    }
  }
...
kernel_3112231113_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
