...
void kernel_3122211211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_P >= 2)) {
  for (t1=max(-1,ceild(-PB_P-28,32));t1<=floord(PB_L-1,32);t1++) {
    lbp=max(0,ceild(32*t1-PB_L+2,32));
    ubp=min(floord(PB_P-1,32),floord(32*t1+PB_P+28,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t1),32*t1-32*t2+1);t3<=min(min(PB_L-1,32*t1+62),32*t1-32*t2+PB_P+30);t3++) {
        lbv=max(max(1,32*t2),-32*t1+32*t2+t3-31);
        ubv=min(min(PB_P-1,32*t2+31),-32*t1+32*t2+t3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3-1][t4] = B[t3-1][t4] - A[t3][t4] - A[t3-2][t4-1] - 3;;
        }
      }
    }
  }
...
kernel_3122211211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
