...
void kernel_3114112211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_N >= 4) {
  for (t1=2;t1<=PB_N-1;t1++) {
    for (t2=max(3,t1);t2<=PB_N-1;t2++) {
      A[t1-2][0] = B[t2] - A[t1-1][0] + A[t2-3][0] + 4;;
    }
  }
}
/* End of CLooG code */
...
kernel_3114112211_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
