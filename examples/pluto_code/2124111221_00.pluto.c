...
void kernel_2124111221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_M >= 5) {
  for (t1=3;t1<=PB_M-1;t1++) {
    for (t2=3;t2<=PB_M-2;t2++) {
      A[t2+1][t2-1] = B[t2][t1] - A[t2-1][t2-3] * A[t1-1][t1-3] + 1;;
    }
  }
}
/* End of CLooG code */
...
kernel_2124111221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
