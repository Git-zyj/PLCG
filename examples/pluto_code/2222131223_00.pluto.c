...
void kernel_2222131223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_N >= 3) {
  for (t1=1;t1<=min(2,PB_N-2);t1++) {
    A[t1+1] = C[t1] + C[t1] * A[t1] - A[t1-1] + 3;;
  }
  for (t1=3;t1<=PB_N-2;t1++) {
    A[t1+1] = C[t1] + C[t1] * A[t1] - A[t1-1] + 3;;
    A[(t1-2)+1] = C[(t1-2)] * C[0] + C[(t1-2)] - C[(t1-2)-1] * 1;;
  }
  for (t1=max(3,PB_N-1);t1<=PB_N;t1++) {
    A[(t1-2)+1] = C[(t1-2)] * C[0] + C[(t1-2)] - C[(t1-2)-1] * 1;;
  }
}
/* End of CLooG code */
...
kernel_2222131223_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
