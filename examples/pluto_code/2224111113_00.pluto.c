...
void kernel_2224111113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_N >= 2) {
  if (PB_P >= 3) {
    lbp=0;
    ubp=min(floord(PB_N-1,32),floord(PB_P-2,32));
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=t2;t3<=floord(PB_P-1,32);t3++) {
        for (t4=max(1,32*t2);t4<=min(min(min(PB_N-1,PB_P-2),32*t2+31),32*t3+30);t4++) {
          for (t5=max(32*t3,t4+1);t5<=min(PB_P-1,32*t3+31);t5++) {
            A[t4-1][t5] = A[t4-1][t5-1] + B[t4] - 1;;
          }
        }
      }
    }
...
kernel_2224111113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
