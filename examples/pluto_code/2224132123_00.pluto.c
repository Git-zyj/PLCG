...
void kernel_2224132123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_P >= 7) {
  for (t1=0;t1<=floord(PB_P-3,32);t1++) {
    for (t2=0;t2<=floord(PB_P-1,32);t2++) {
      if (t2 == 0) {
        for (t3=max(4,32*t1);t3<=min(min(PB_M,PB_P-3),32*t1+31);t3++) {
          A[0 +t3+1][t3] = C[0] - D[0][t3+2] - C[0] * D[0 +t3-1][0] * 1;;
          B[t3][t3-2] = C[t3] * B[t3-2][t3-4] + 5;;
          lbv=1;
          ubv=min(min(31,PB_P-1),-t3+PB_M);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t4+t3+1][t3] = C[t4] - D[t4][t3+2] - C[t4] * D[t4+t3-1][t4] * 1;;
          }
        }
      }
      if (t2 >= 1) {
        lbp=max(4,32*t1);
        ubp=min(min(PB_P-3,32*t1+31),-32*t2+PB_M);
#pragma omp parallel for private(lbv,ubv,t4)
        for (t3=lbp;t3<=ubp;t3++) {
          lbv=32*t2;
          ubv=min(min(PB_P-1,32*t2+31),-t3+PB_M);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t4+t3+1][t3] = C[t4] - D[t4][t3+2] - C[t4] * D[t4+t3-1][t4] * 1;;
      ...
      if (t2 == 0) {
        for (t3=max(max(4,32*t1),PB_M+1);t3<=(min(PB_P-3,32*t1+31))-7;t3+=8) {
          B[t3][t3-2] = C[t3] * B[t3-2][t3-4] + 5;;
          B[(t3+1)][(t3+1)-2] = C[(t3+1)] * B[(t3+1)-2][(t3+1)-4] + 5;;
          B[(t3+2)][(t3+2)-2] = C[(t3+2)] * B[(t3+2)-2][(t3+2)-4] + 5;;
          B[(t3+3)][(t3+3)-2] = C[(t3+3)] * B[(t3+3)-2][(t3+3)-4] + 5;;
          B[(t3+4)][(t3+4)-2] = C[(t3+4)] * B[(t3+4)-2][(t3+4)-4] + 5;;
          B[(t3+5)][(t3+5)-2] = C[(t3+5)] * B[(t3+5)-2][(t3+5)-4] + 5;;
          B[(t3+6)][(t3+6)-2] = C[(t3+6)] * B[(t3+6)-2][(t3+6)-4] + 5;;
          B[(t3+7)][(t3+7)-2] = C[(t3+7)] * B[(t3+7)-2][(t3+7)-4] + 5;;
        }
        for (;t3<=min(PB_P-3,32*t1+31);t3++) {
          B[t3][t3-2] = C[t3] * B[t3-2][t3-4] + 5;;
        }
      }
    }
...
kernel_2224132123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
