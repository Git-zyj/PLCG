...
void kernel_2222211121_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-2,32);t2++) {
      for (t3=max(1,32*t1);t3<=min(32*t1+31,-32*t2+PB_L-1);t3++) {
        for (t4=32*t2;t4<=min(32*t2+31,-t3+PB_L-1);t4++) {
          A[t3-1] = C[t3][t4+1] * C[t3][t4] - 3;;
          B[t4+1][t4+t3-1] = B[t4+2][t4+t3] + 2;;
        }
        for (t4=-t3+PB_L;t4<=min(PB_L-2,32*t2+31);t4++) {
          A[t3-1] = C[t3][t4+1] * C[t3][t4] - 3;;
        }
      }
      for (t3=max(32*t1,-32*t2+PB_L);t3<=min(PB_L-1,32*t1+31);t3++) {
...
kernel_2222211121_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
