...
void kernel_3222111223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 1) {
  for (t1=0;t1<=floord(PB_M-1,32);t1++) {
    for (t2=0;t2<=min(floord(3*PB_M-3,32),floord(64*t1+PB_M+61,32));t2++) {
      if (t2 >= 1) {
        for (t3=max(max(1,ceild(32*t2-PB_M+1,2)),32*t1);t3<=min(min(PB_M-1,32*t1+31),16*t2+15);t3++) {
          for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,2*t3+PB_M-1);t4++) {
            A[t3+1][(-2*t3+t4)] = A[(-2*t3+t4)+2][t3] * A[t3-1][(-2*t3+t4)] - 5;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
...
kernel_3222111223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
