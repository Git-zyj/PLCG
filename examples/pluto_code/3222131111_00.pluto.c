...
void kernel_3222131111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_M >= 4) {
  for (t1=0;t1<=floord(PB_M-3,32);t1++) {
    for (t2=0;t2<=min(floord(PB_M-3,16),floord(32*t1+PB_M+28,32));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=1;t3<=min(31,PB_M-3);t3++) {
          A[t3+1] = C[t3] - C[t3] + C[t3] + C[t3] - C[t3-1] * A[t3+2] * 3;;
          for (t4=t3;t4<=min(31,t3+PB_M-3);t4++) {
            B[(-t3+t4)+1] = D[(-t3+t4)][t3] - B[(-t3+t4)+2] - 3;;
          }
        }
      }
      if (t2 == 0) {
      ...
      if (t2 >= 1) {
        for (t3=max(max(1,32*t1),32*t2-PB_M+3);t3<=min(min(PB_M-3,32*t1+31),32*t2+31);t3++) {
...
kernel_3222131111_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
