...
void kernel_3114132123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_M-3,32),t1);t2++) {
      for (t3=max(2,32*t2);t3<=min(min(PB_M-3,PB_N-1),32*t2+31);t3++) {
        lbv=max(32*t1,t3);
        ubv=min(PB_N-1,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4-2] = B[t4][t3][t4+1] - C[t4][t3-2] * C[t3][t4-2] * A[t3+2][t4-2] + 1;;
        }
      }
    }
  }
...
kernel_3114132123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
