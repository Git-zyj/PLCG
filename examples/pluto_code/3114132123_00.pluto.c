...
void kernel_3114132123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
if ((PB_L >= 1) && (PB_M >= 2)) {
  lbp=0;
  ubp=min(floord(PB_L,32),floord(PB_M-1,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(min(floord(PB_M-2,32),floord(-32*t1+PB_L,32)),floord(PB_L-1,64)),t1);t2++) {
      for (t3=max(32*t1,32*t2+1);t3<=min(min(PB_M-1,32*t1+31),-32*t2+PB_L);t3++) {
        for (t4=32*t2;t4<=min(min(32*t2+31,-t3+PB_L),t3-1);t4++) {
          A[t3][t4] = B[t3][t4+2] + C[t4] + D[t3][t4][t4+t3-1] + A[t3][t4+1] * 3;;
        }
      }
    }
  }
...
kernel_3114132123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
