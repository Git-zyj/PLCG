...
void kernel_2224231221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_M >= 7) {
  for (t1=0;t1<=floord(3*PB_M-10,32);t1++) {
    lbp=ceild(2*t1,3);
    ubp=min(min(floord(3*t1+2,4),floord(PB_M-3,16)),floord(32*t1+PB_M+29,64));
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(max(3,ceild(32*t2+1,3)),32*t1-32*t2),32*t2-PB_M+2);t3<=min(min(PB_M-4,16*t2+14),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3+2);t4<=min(min(32*t2+31,3*t3-1),t3+PB_M-2);t4++) {
          A[t3][(-2*t3+t4)+t3+1] = C[(-2*t3+t4)-1][t3] - D[t3][(-2*t3+t4)-1] + C[(-2*t3+t4)][t3+1] * D[(-2*t3+t4)][t3+1] + B[(-2*t3+t4)-2][(-2*t3+t4)-2] - B[(-2*t3+t4)][(-2*t3+t4)] * 6;;
          B[(-2*t3+t4)][(-2*t3+t4)] = D[t3][0] - C[(-2*t3+t4)][t3] + B[(-2*t3+t4)-2][(-2*t3+t4)-2] - 1;;
        }
      }
    }
  }
...
kernel_2224231221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
