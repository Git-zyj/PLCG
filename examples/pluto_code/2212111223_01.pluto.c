...
if (PB_Q >= 3) {
  if (PB_L >= 1) {
    for (t2=0;t2<=floord(PB_Q+PB_L-3,32);t2++) {
      lbp=max(0,ceild(32*t2-PB_Q+2,32));
      ubp=min(floord(PB_L-1,32),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(1,32*t2-32*t3);t4<=min(PB_Q-2,32*t2-32*t3+31);t4++) {
          lbv=32*t3;
          ubv=min(PB_L-1,32*t3+31);
#pragma ivdep
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            A[t5][t4][t4-1] = C[t4-1][t4] + C[t4][t5] + A[t5][t4+2][t4+1] + B[t4+1] + 6;;
          }
        }
      }
    }
