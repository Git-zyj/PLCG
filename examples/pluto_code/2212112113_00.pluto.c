...
void kernel_2212112113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 1) {
  if (PB_L >= 0) {
    for (t1=0;t1<=min(floord(PB_L+PB_M-2,32),floord(PB_L+3*PB_M-6,32));t1++) {
      for (t2=0;t2<=min(min(floord(PB_M-1,32),floord(-32*t1+PB_L+PB_M-2,32)),floord(-32*t1+PB_L+3*PB_M-6,96));t2++) {
        if ((t1 <= min(floord(PB_L-3,32),floord(PB_M-2,32))) && (t2 == 0)) {
          for (t4=32*t1;t4<=min(min(PB_L-3,PB_M-2),32*t1+31);t4++) {
            A[0][t4+2] = C[0] - 5;;
            B[t4] = B[t4+1] * 3;;
          }
          for (t4=PB_M-1;t4<=min(PB_L-3,32*t1+31);t4++) {
            A[0][t4+2] = C[0] - 5;;
          }
          for (t4=PB_L-2;t4<=min(PB_M-2,32*t1+31);t4++) {
            B[t4] = B[t4+1] * 3;;
          }
        }
        if ((t1 <= floord(PB_L-3,32)) && (t1 >= ceild(PB_M-1,32)) && (t2 == 0)) {
          for (t4=32*t1;t4<=min(PB_L-3,32*t1+31);t4++) {
            A[0][t4+2] = C[0] - 5;;
          }
        }
        if ((t1 <= floord(PB_M-2,32)) && (t1 >= ceild(PB_L-2,32)) && (t2 == 0)) {
...
kernel_2212112113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
