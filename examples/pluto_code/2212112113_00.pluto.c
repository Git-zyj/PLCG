...
void kernel_2212112113_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_P-1,32);t2++) {
      for (t3=max(1,32*t2);t3<=min(min(PB_P-1,32*t1+31),32*t2+31);t3++) {
        if (t1 == t2) {
          lbv=32*t1;
          ubv=t3-1;
#pragma ivdep
...
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          B[t3-1][t4] = C[t4][t3] * 4;;
          B[t3-1][t4] = C[t3][t4] + B[t3][t4] * 3;;
        }
      }
      for (t3=max(32*t2,32*t1+32);t3<=min(PB_P-1,32*t2+31);t3++) {
        lbv=32*t1;
        ubv=32*t1+31;
#pragma ivdep
#pragma vector always
...
kernel_2212112113_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
