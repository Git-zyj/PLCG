...
void kernel_3124132111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
/* Start of CLooG code */
if ((PB_N >= 3) && (PB_P >= 3)) {
  for (t1=0;t1<=floord(2*PB_P+PB_N-4,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_P+1,32));
    ubp=min(min(floord(PB_P+PB_N-3,32),floord(32*t1+PB_N+29,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t1-32*t2),32*t2-PB_N+2);t3<=min(min(PB_P-1,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(32*t2+31,t3+PB_N-2);t4++) {
          A[(-t3+t4)] = B[(-t3+t4)][t3] + C[(-t3+t4)][t3-1][t3-2] + B[(-t3+t4)-1][t3] * A[(-t3+t4)+1] + 4;;
        }
      }
    }
  }
...
kernel_3124132111_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
