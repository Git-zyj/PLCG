...
void kernel_3124132111_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
    ...
    for (t2=0;t2<=floord(PB_L-3,32);t2++) {
      for (t3=t2;t3<=floord(PB_L-2,32);t3++) {
        for (t4=32*t1;t4<=min(PB_L-3,32*t1+31);t4++) {
          for (t5=max(32*t3,32*t2+1);t5<=min(PB_L-2,32*t3+31);t5++) {
            lbv=32*t2;
            ubv=min(32*t2+31,t5-1);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][t5+1][t6] = B[t6][t4+1][t5] + B[0][t6][t4+2] + B[t5-1][t4][t6] + A[t4][t5][t6] + 3;;
            }
          }
        }
      }
...
kernel_3124132111_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
