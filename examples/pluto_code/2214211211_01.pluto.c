...
void kernel_2214211211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_M >= 2) {
  if (PB_L >= 4) {
    for (t2=0;t2<=min(floord(3*PB_L-8,32),floord(2*PB_L+PB_M-5,32));t2++) {
      lbp=max(ceild(t2,2),ceild(32*t2-PB_L+2,32));
      ubp=min(min(min(floord(2*t2+1,3),floord(PB_L-3,16)),floord(PB_L+PB_M-3,32)),floord(32*t2+PB_M+30,64));
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(max(2,32*t2-32*t3),16*t3+1),32*t3-PB_M+1);t4<=min(min(PB_L-2,32*t3+30),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4+1);t5<=min(min(32*t3+31,2*t4-1),t4+PB_M-1);t5++) {
            A[(-t4+t5)] = C[(-t4+t5)][t4] + A[(-t4+t5)-1] + A[(-t4+t5)+1] * A[t4+1] + 6;;
          }
        }
      }
    }
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t3=0;t3<=floord(-32*t2+PB_M-1,32);t3++) {
      for (t4=max(1,32*t3);t4<=min(32*t3+31,-32*t2+PB_M-1);t4++) {
        lbv=32*t2;
        ubv=min(32*t2+31,-t4+PB_M-1);
#pragma ivdep
#pragma vector always
        for (t5=lbv;t5<=ubv;t5++) {
          B[t4+t5][t4-1][t5] = A[t4] * 3;;
        }
      }
    }
  }
...
kernel_2214211211_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
