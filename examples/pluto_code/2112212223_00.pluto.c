...
void kernel_2112212223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_N >= 2) && (PB_P >= 2)) {
  for (t1=0;t1<=min(floord(PB_P-1,16),floord(PB_P+PB_N-1,32));t1++) {
    lbp=max(0,ceild(32*t1-PB_P,32));
    ubp=min(floord(t1,2),floord(PB_N-1,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(1,32*t2);t3<=min(min(min(PB_N-1,PB_P-1),32*t2+31),32*t1-32*t2+30);t3++) {
        for (t4=max(32*t1-32*t2,t3+1);t4<=min(PB_P,32*t1-32*t2+31);t4++) {
          A[t4+2][t3] = B[t3][t3] * A[t3+4][t4-1] * A[t4+2][t3-1] + 1;;
        }
      }
    }
  }
...
kernel_2112212223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
