...
void kernel_2212111113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(-32*t1+PB_L-1,32);t2++) {
      for (t3=max(1,32*t2);t3<=min(32*t2+31,-32*t1+PB_L-2);t3++) {
        lbv=32*t1;
        ubv=min(32*t1+31,-t3+PB_L-2);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4+t3] = C[t4+t3-1] + A[t3+1][t4+t3+1] + 1;;
          A[(t3-1)+1][t4+(t3-1)+1] = C[t4+1] + 4;;
        }
        if (t1 >= ceild(-t3+PB_L-32,32)) {
          A[(t3-1)+1][(-t3+PB_L-1)+(t3-1)+1] = C[(-t3+PB_L-1)+1] + 4;;
        }
      }
      if (t1 >= ceild(-32*t2+PB_L-32,32)) {
        A[(-32*t1+PB_L-2)+1][32*t1+(-32*t1+PB_L-2)+1] = C[32*t1+1] + 4;;
      }
    }
  }
}
...
kernel_2212111113_00(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
