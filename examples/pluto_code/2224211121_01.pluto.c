...
void kernel_2224211121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (t2=0;t2<=floord(PB_L-1,32);t2++) {
      ...
      if (t2 == 0) {
        for (t3=max(2,32*t1);t3<=min(PB_L-2,32*t1+31);t3++) {
          B[t3+1] = 6;;
          A[t3+1][t3] = C[t3] - C[t3] + A[t3-1][t3-2] - A[t3][t3-1] * 2;;
...
kernel_2224211121_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
