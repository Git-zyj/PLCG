...
if ((PB_L >= max(0,ceild(-2*PB_M+6,3))) && (PB_M >= 0)) {
  for (t1=0;t1<=floord(PB_L+PB_M-2,32);t1++) {
    for (t2=max(0,ceild(32*t1-PB_M,32));t2<=min(floord(PB_L,32),floord(3*PB_L+PB_M-3,96));t2++) {
      if ((t2 >= 1) && (t2 <= floord(PB_L-1,32))) {
        for (t3=32*t1;t3<=min(min(PB_L-2,32*t1+31),32*t2+31);t3++) {
          lbv=max(32*t2,t3);
          ubv=min(PB_L-1,32*t2+31);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3][t4] = C[t3][t4] + C[t4][t3+1] - C[t4][0] * C[t4-1][t3] + C[t4-1][t3] - A[t3+1][t4] - 5;;
          }
        }
      }
      if ((PB_L >= 2) && (t1 == 0) && (t2 == 0)) {
...
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[0][t4] = C[0][t4] + C[t4][0 +1] - C[t4][0] * C[t4-1][0] + C[t4-1][0] - A[0 +1][t4] - 5;;
        }
      }
      if (t2 == 0) {
      ...
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=1;t3<=min(min(31,PB_L-2),PB_M-2);t3++) {
          B[t3] = D[t3-1] * B[t3+1] * 5;;
          lbv=t3;
          ubv=min(31,PB_L-1);
#pragma ivdep
#pragma vector always
      ...
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(1,PB_M-1);t3<=min(31,PB_L-2);t3++) {
          lbv=t3;
          ubv=min(31,PB_L-1);
#pragma ivdep
#pragma vector always
      ...
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(1,PB_L-1);t3<=min(31,PB_M-2);t3++) {
          B[t3] = D[t3-1] * B[t3+1] * 5;;
        }
      }
    }
