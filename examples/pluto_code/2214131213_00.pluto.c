...
void kernel_2214131213_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma omp parallel for private(lbv,ubv,t4,t5)
    for (t3=lbp;t3<=ubp;t3++) {
      for (t4=32*t3;t4<=min(min(PB_M-2,32*t3+31),32*t2-32*t3+30);t4++) {
        for (t5=max(32*t2-32*t3,t4+1);t5<=min(PB_M-1,32*t2-32*t3+31);t5++) {
          B[t5] = D[t4][t5] + C[t4+1] + D[t4][t5] - B[t4+1] + 4;;
        }
      }
    }
  }
...
kernel_2214131213_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
