...
void kernel_3122231113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
/* Start of CLooG code */
if (PB_N >= 3) {
  for (t2=-PB_N+1;t2<=-2;t2++) {
    A[-t2-1] = B[-t2-1] + B[-t2] * B[-t2-1] * A[-t2-2] * 4;;
  }
}
/* End of CLooG code */
...
kernel_3122231113_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
