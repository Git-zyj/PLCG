...
void kernel_3114111223_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_P-2,32);t2++) {
      for (t3=2*t2;t3<=min(floord(PB_L+2*PB_P-8,32),floord(64*t2+PB_L+58,32));t3++) {
        for (t4=max(ceild(32*t3-PB_L+4,2),32*t2);t4<=min(min(PB_P-2,32*t2+31),16*t3+15);t4++) {
          for (t5=max(32*t3,2*t4);t5<=min(32*t3+31,2*t4+PB_L-4);t5++) {
            lbv=32*t1;
            ubv=min(PB_L-2,32*t1+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[(-2*t4+t5)+1][t4+1][t6] = B[(-2*t4+t5)][t6+1][t4+1] + A[(-2*t4+t5)+3][t4][t6] - A[(-2*t4+t5)][t4+1][t6] - 2;;
            }
          }
        }
      }
...
kernel_3114111223_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
