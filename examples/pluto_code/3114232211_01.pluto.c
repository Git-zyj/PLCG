...
void kernel_3114232211_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(floord(PB_L-2,32),floord(PB_P-1,32)),t1);t2++) {
      for (t3=2*t2;t3<=min(min(floord(PB_L-2,16),floord(PB_P+PB_L-3,32)),floord(32*t2+PB_L+29,32));t3++) {
        for (t4=max(max(2,32*t1),32*t3-PB_L+2);t4<=min(PB_L-1,32*t1+31);t4++) {
          for (t5=max(max(2,32*t2),32*t3-PB_L+2);t5<=min(min(min(min(t4,PB_L-2),PB_P-1),32*t2+31),16*t3+15);t5++) {
            for (t6=max(32*t3,2*t5);t6<=min(32*t3+31,t5+PB_L-2);t6++) {
              A[(-t5+t6)][t4][t5] = B[t4+2][(-t5+t6)-2][t5-1] - C[(-t5+t6)][t4][t5-2] - B[t4][t5-2][0] + A[(-t5+t6)+1][t4][t5-1] - A[(-t5+t6)-1][t4][t5] * 2;;
            }
          }
        }
      }
...
kernel_3114232211_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
