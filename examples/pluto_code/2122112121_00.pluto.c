...
void kernel_2122112121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
    ...
    for (t2=0;t2<=floord(PB_N,32);t2++) {
      for (t3=max(1,32*t1);t3<=min(PB_L,32*t1+31);t3++) {
        for (t4=max(1,32*t2);t4<=min(PB_N,32*t2+31);t4++) {
          A[t4][t3][t3-1] = B[t4-1][t3][t4] * A[t4+2][t3][t3-1] + 4;;
        }
      }
    }
  }
...
kernel_2122112121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
