...
void kernel_2222231121_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
if (PB_M >= 2) {
  for (t1=0;t1<=floord(PB_M-2,32);t1++) {
    for (t2=0;t2<=min(floord(3*PB_M-6,32),floord(64*t1+PB_M+60,32));t2++) {
      if (t2 >= 1) {
        for (t3=max(ceild(32*t2-PB_M+2,2),32*t1);t3<=min(min(floord(32*t2+31,3),PB_M-2),32*t1+31);t3++) {
          for (t4=max(32*t2,3*t3);t4<=min(32*t2+31,2*t3+PB_M-2);t4++) {
            B[(-2*t3+t4)] = E[(-2*t3+t4)][t3] + F[(-2*t3+t4)][t3+1] + F[t3][(-2*t3+t4)+1] * F[(-2*t3+t4)+1][t3+1] + B[(-2*t3+t4)+2] * 3;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t4=0;t4<=min(31,PB_M-2);t4++) {
          B[t4] = E[t4][0] + F[t4][0 +1] + F[0][t4+1] * F[t4+1][0 +1] + B[t4+2] * 3;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=1;t3<=min(10,PB_M-2);t3++) {
          A[t3+1] = C[t3-1] * D[t3+1] + 2;;
          for (t4=3*t3;t4<=min(31,2*t3+PB_M-2);t4++) {
            B[(-2*t3+t4)] = E[(-2*t3+t4)][t3] + F[(-2*t3+t4)][t3+1] + F[t3][(-2*t3+t4)+1] * F[(-2*t3+t4)+1][t3+1] + B[(-2*t3+t4)+2] * 3;;
          }
        }
      }
      if (t2 == 0) {
        for (t3=max(11,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
          A[t3+1] = C[t3-1] * D[t3+1] + 2;;
        }
      }
    }
...
kernel_2222231121_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
