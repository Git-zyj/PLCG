...
void kernel_3224131211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_L >= 3) {
  A[1 +1] = C[1 -1] - C[1] * C[1 -1] * C[1 +1] - C[1 -1] + B[1] - B[1 -1] - 4;;
  for (t1=2;t1<=PB_L-2;t1++) {
    A[t1+1] = C[t1-1] - C[t1] * C[t1-1] * C[t1+1] - C[t1-1] + B[t1] - B[t1-1] - 4;;
    B[(t1-1)] = C[(t1-1)] - A[(t1-1)+1] - B[(t1-1)-1] + 5;;
  }
  B[(PB_L-2)] = C[(PB_L-2)] - A[(PB_L-2)+1] - B[(PB_L-2)-1] + 5;;
}
/* End of CLooG code */
...
kernel_3224131211_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
