...
void kernel_2112231223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_N >= 2) {
  for (t1=1;t1<=PB_N-1;t1++) {
    A[t1] = B[t1] - C[t1] * B[t1-1] * A[t1+2] + A[t1+1] - 6;;
  }
}
/* End of CLooG code */
...
kernel_2112231223_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
