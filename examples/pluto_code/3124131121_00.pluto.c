  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_M-1,32);t2++) {
      for (t3=0;t3<=floord(-32*t1+PB_M-1,32);t3++) {
        for (t4=32*t1;t4<=min(min(PB_M-4,32*t1+31),-32*t3+PB_M-1);t4++) {
          for (t5=max(3,32*t3);t5<=min(32*t3+31,-t4+PB_M-1);t5++) {
            lbv=max(2,32*t2);
            ubv=min(PB_M-1,32*t2+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][t5-1][t6-1] = B[t6][t5][t4] + B[t5][t6][t4+1] + B[t4+t5][t4+1][t6] * A[t4][t5-3][t6-2] - 2;;
            }
          }
        }
      }
