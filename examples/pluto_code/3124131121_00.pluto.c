...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-2,32);t2++) {
      for (t3=t2;t3<=min(floord(PB_L-2,16),floord(32*t2+PB_L+30,32));t3++) {
        for (t4=max(max(1,32*t2),32*t3-PB_L+1);t4<=min(min(PB_L-2,32*t2+31),32*t3+30);t4++) {
...
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[(-t4+t5)][t6] = B[t6+1][t4][t6] + B[(-t4+t5)][t4+1][1] * C[(-t4+t5)][t4-1] - A[(-t4+t5)-1][t6] * 3;;
            }
          }
        }
      }
