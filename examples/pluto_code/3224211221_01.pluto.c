...
void kernel_3224211221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
  int t1;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
for (t1=2;t1<=min(PB_L-2,PB_M-1);t1++) {
  A[t1] = C[t1] + A[t1-2] + 4;;
  B[t1] = B[t1-2] * B[t1-1] - B[t1+1] - 2;;
}
for (t1=max(2,PB_L-1);t1<=PB_M-1;t1++) {
  A[t1] = C[t1] + A[t1-2] + 4;;
}
for (t1=max(2,PB_M);t1<=PB_L-2;t1++) {
  B[t1] = B[t1-2] * B[t1-1] - B[t1+1] - 2;;
}
/* End of CLooG code */
...
kernel_3224211221_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
