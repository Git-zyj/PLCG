...
/* Start of CLooG code */
if ((PB_M >= 0) && (PB_N >= max(0,-PB_M+3))) {
  for (t1=0;t1<=min(floord(PB_N+PB_M-1,32),floord(3*PB_N+5*PB_M-9,96));t1++) {
    for (t2=max(max(0,ceild(32*t1-PB_M,32)),ceild(96*t1-PB_N-3*PB_M+3,64));t2<=min(floord(PB_N,32),floord(3*PB_N+PB_M-3,96));t2++) {
      if ((t2 >= 1) && (t2 <= floord(PB_N-1,32))) {
        for (t3=32*t1;t3<=min(min(PB_N-3,32*t1+31),32*t2+31);t3++) {
          lbv=max(32*t2,t3);
          ubv=min(PB_N-1,32*t2+31);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3][t4] = C[t4][t3+1] * C[t3+2][t4-1] * C[t3][t4] - C[t3+1][t4-2] - 6;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=min(1,PB_N-3);t3++) {
          lbv=2;
          ubv=min(31,PB_N-1);
#pragma ivdep
#pragma vector always
      ...
      if (t2 == 0) {
        for (t3=max(32,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
          B[t3] = D[t3-2] + D[t3] + B[t3+1] * B[t3-1] * 3;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(min(31,PB_M-1),PB_N-3);t3++) {
          B[t3] = D[t3-2] + D[t3] + B[t3+1] * B[t3-1] * 3;;
          lbv=t3;
          ubv=min(31,PB_N-1);
#pragma ivdep
#pragma vector always
      ...
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(2,PB_M);t3<=min(31,PB_N-3);t3++) {
          lbv=t3;
          ubv=min(31,PB_N-1);
#pragma ivdep
#pragma vector always
      ...
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(2,PB_N-2);t3<=min(31,PB_M-1);t3++) {
          B[t3] = D[t3-2] + D[t3] + B[t3+1] * B[t3-1] * 3;;
        }
      }
    }
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t1;t3<=min(PB_N-3,32*t1+31);t3++) {
        lbv=max(max(2,32*t2),t3);
        ubv=min(PB_N-1,32*t2+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
