...
void kernel_3114232123_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
if ((PB_M >= 3) && (PB_Q >= 7)) {
  lbp=0;
  ubp=min(floord(PB_M-1,32),floord(PB_Q-2,64));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_M-3,32),floord(-32*t1+PB_M-1,32));t2++) {
      for (t3=t1;t3<=min(floord(PB_Q-3,32),floord(-32*t1+PB_Q-1,32));t3++) {
        for (t4=32*t2;t4<=min(min(PB_M-3,32*t2+31),-32*t1+PB_M-1);t4++) {
          for (t5=max(max(4,32*t3),32*t1+1);t5<=min(min(PB_Q-3,32*t3+31),-32*t1+PB_Q-1);t5++) {
            lbv=max(2,32*t1);
            ubv=min(min(min(32*t1+31,t5-1),-t4+PB_M-1),-t5+PB_Q-1);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][t6+t5-2][t6] = B[t4][t5-2][t6+t5] * B[t4][t5+1][t6-2] * C[t4+t6][t5][t6] - A[t4][t6+t5-4][t6] + 5;;
            }
          }
        }
      }
...
kernel_3114232123_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
