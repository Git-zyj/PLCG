...
if ((PB_L >= 4) && (PB_P >= 2)) {
  lbp=0;
  ubp=min(floord(PB_L-3,32),floord(PB_P-1,32));
  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_L-2,32);t2++) {
      for (t3=2*t2;t3<=min(floord(3*PB_L-5,32),floord(64*t2+PB_L+61,32));t3++) {
        for (t4=max(1,32*t1);t4<=min(min(min(min(PB_L-3,PB_P-1),32*t1+31),32*t2+30),16*t3+13);t4++) {
          for (t5=max(max(ceild(32*t3-PB_L+1,2),32*t2),t4+1);t5<=min(min(PB_L-2,32*t2+31),16*t3+14);t5++) {
            for (t6=max(32*t3,2*t5+2);t6<=min(32*t3+31,2*t5+PB_L-1);t6++) {
              A[(-2*t5+t6)][t4] = B[t4][(-2*t5+t6)][t4] - B[t4-1][t4][t5+1] - C[t4-1][t5][0] + A[(-2*t5+t6)-2][t4] + 1;;
            }
          }
        }
      }
