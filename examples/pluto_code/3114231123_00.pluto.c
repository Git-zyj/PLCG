...
if (PB_N >= 3) {
  for (t1=0;t1<=floord(PB_N-2,8);t1++) {
    lbp=ceild(3*t1,4);
    ubp=min(min(floord(3*PB_N-6,32),floord(64*t1+PB_N+60,96)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,ceild(32*t2-PB_N+2,2)),32*t1-32*t2);t3<=min(min(floord(32*t2+31,3),PB_N-2),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,3*t3);t4<=min(32*t2+31,2*t3+PB_N-2);t4++) {
          A[(-3*t3+t4)+1][t3] = B[(-3*t3+t4)][t3-1] * B[(-3*t3+t4)+t3+1][(-3*t3+t4)] * C[(-3*t3+t4)+1][t3] + A[t3-1][(-3*t3+t4)] - 5;;
        }
      }
    }
  }
