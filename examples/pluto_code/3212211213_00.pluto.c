...
if (PB_M >= 1) {
  for (t1=0;t1<=PB_M-1;t1++) {
    if (t1 >= 1) {
      A[t1] = C[t1] - C[t1] + A[t1+1] - A[t1-1] + 5;;
    }
    if (PB_P >= 2) {
      for (t2=ceild(t1-30,16);t2<=floord(2*PB_M+PB_P-1,32);t2++) {
        lbp=max(ceild(t2,2),ceild(32*t2-PB_M,32));
        ubp=min(min(floord(PB_M+PB_P-1,32),floord(-t1+32*t2+31,32)),floord(32*t2+PB_P+30,64));
#pragma omp parallel for private(lbv,ubv,t4,t5)
        for (t3=lbp;t3<=ubp;t3++) {
          for (t4=max(max(max(1,t1),32*t2-32*t3),32*t3-PB_P+1);t4<=min(min(PB_M,32*t3+30),32*t2-32*t3+31);t4++) {
            for (t5=max(32*t3,t4+1);t5<=min(32*t3+31,t4+PB_P-1);t5++) {
              B[t1][t1][(-t4+t5)] = B[t4-1][t4-1][(-t4+t5)-1] * B[t1][t1][(-t4+t5)-1] - 4;;
            }
          }
        }
      }
