...
void kernel_3124131111_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if ((PB_M >= 3) && (PB_N >= 3)) {
  lbp=0;
  ubp=min(floord(PB_M-2,32),floord(PB_N-2,64));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_N-2,32);t2++) {
      for (t3=t1;t3<=min(floord(PB_M-1,32),floord(-32*t1+PB_N-1,32));t3++) {
        for (t4=32*t1;t4<=min(min(min(min(floord(PB_N-2,2),PB_M-2),32*t1+31),32*t3+30),-32*t3+PB_N-1);t4++) {
          for (t5=32*t2;t5<=min(PB_N-2,32*t2+31);t5++) {
            for (t6=max(max(2,32*t3),t4+1);t6<=min(min(PB_M-1,32*t3+31),-t4+PB_N-1);t6++) {
              A[t6-1][t4][t5] = B[t4][t5][t6] + B[t5+1][1][t4] * B[t4+t6][t5][t6] * A[t6-2][t4][t5] * 4;;
            }
          }
        }
      }
...
kernel_3124131111_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
