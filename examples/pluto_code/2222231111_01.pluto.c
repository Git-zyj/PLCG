...
void kernel_2222231111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_L-1,32),t1);t2++) {
      for (t3=max(2,32*t2);t3<=min(16*t1-1,32*t2+31);t3++) {
        lbv=32*t1;
        ubv=min(PB_L,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-t3+t4)-1] = C[0] * A[t3-1][(-t3+t4)] - B[(-t3+t4)+t3-2][(-t3+t4)+t3-1] + 4;;
        }
      }
      for (t3=max(max(2,16*t1),32*t2);t3<=min(min(PB_L-1,32*t1+30),32*t2+31);t3++) {
        lbv=max(32*t1,t3+1);
        ubv=min(min(PB_L,2*t3),32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-t3+t4)-1] = C[0] * A[t3-1][(-t3+t4)] - B[(-t3+t4)+t3-2][(-t3+t4)+t3-1] + 4;;
          B[(-t3+t4-1)+t3-1][(-t3+t4-1)+t3] = C[t3] * D[(-t3+t4-1)][t3-1] * C[(-t3+t4-1)] - E[t3+1] * 4;;
        }
        lbv=2*t3+1;
        ubv=min(PB_L,32*t1+31);
#pragma ivdep
...
kernel_2222231111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
