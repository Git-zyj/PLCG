...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-2,32);t2++) {
      for (t3=0;t3<=min(min(floord(PB_P-3,32),floord(-32*t1+PB_P-2,32)),floord(-32*t2+PB_L-1,32));t3++) {
        for (t4=max(1,32*t1);t4<=min(min(PB_P-3,32*t1+31),-32*t3+PB_P-2);t4++) {
          for (t5=32*t2;t5<=min(min(PB_L-2,32*t2+31),-32*t3+PB_L-1);t5++) {
            for (t6=max(1,32*t3);t6<=min(min(32*t3+31,-t4+PB_P-2),-t5+PB_L-1);t6++) {
              A[t4][t5][t6] = B[t4-1][t5+t6][t6+t4+1] * B[t5][t4-1][t6+t4] + C[t5][t6+1] * A[t4][t5][t6-1] + 5;;
            }
          }
        }
      }
