...
void kernel_3124111223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if (PB_M >= 6) {
  for (t2=-PB_M+3;t2<=-3;t2++) {
    for (t4=2;t4<=min(-t2-1,t2+PB_M-1);t4++) {
      A[t4] = B[t4+-t2][0] - A[t4-2] + A[t4+1] * 2;;
    }
  }
}
/* End of CLooG code */
...
kernel_3124111223_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
