...
void kernel_2222231121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_L >= 4) {
  for (t2=2;t2<=PB_L-2;t2++) {
    A[t2] = C[t2] * C[t2] - A[t2-2] * B[t2+1] + 2;;
  }
  lbp=0;
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t3=t2;t3<=floord(PB_L-2,32);t3++) {
      for (t4=max(1,32*t2);t4<=min(min(PB_L-3,32*t2+31),32*t3+30);t4++) {
        for (t5=max(32*t3,t4+1);t5<=min(PB_L-2,32*t3+31);t5++) {
          B[t4] = D[t4-1][t5-1] + D[t4-1][t5] + D[t5][t4] + C[t4] * 4;;
        }
      }
    }
  }
...
kernel_2222231121_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
