...
void kernel_2112131211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if ((PB_L >= 4) && (PB_M >= 2)) {
  lbp=0;
  ubp=min(floord(PB_L-4,32),floord(PB_M-2,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_M-1,32),floord(-32*t1+PB_L-3,32));t2++) {
      for (t3=max(1,32*t2);t3<=min(min(PB_M-1,32*t2+31),-32*t1+PB_L-3);t3++) {
        lbv=32*t1;
        ubv=min(min(PB_M-2,32*t1+31),-t3+PB_L-3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t4+t3+1][t4] = B[t3-1] * C[t4+1][t3] + C[t3][0] + A[t4+t3][t4] + A[t4+t3+2][t4] - 4;;
        }
      }
    }
  }
...
kernel_2112131211_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
