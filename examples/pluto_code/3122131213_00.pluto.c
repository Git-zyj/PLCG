...
void kernel_3122131213_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_Q-2,32),t1);t2++) {
      for (t3=0;t3<=floord(PB_Q-1,32);t3++) {
        for (t4=max(1,32*t2);t4<=min(min(PB_P-2,PB_Q-2),32*t2+31);t4++) {
...
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][t5][t6+1] = B[t4][t6][t5] + B[t6][t5+1][t4] - B[t5][t6-1][t4+1] - A[t4][t5-1][t6+1] * A[t4-1][t5][t6+1] + 5;;
            }
          }
        }
      }
...
kernel_3122131213_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
