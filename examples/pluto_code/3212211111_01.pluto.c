...
void kernel_3212211111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3][t4+1] = C[t4+1][t3] + 6;;
          }
        }
      }
      if ((t2 >= 1) && (t2 <= floord(PB_M-2,32))) {
        lbp=max(1,32*t1);
        ubp=min(PB_L-1,32*t1+31);
...
kernel_3212211111_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
