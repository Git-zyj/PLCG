...
void kernel_3124132121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_M-3,32);t2++) {
      for (t3=max(2,32*t1);t3<=min(PB_M-3,32*t1+31);t3++) {
        for (t4=max(32*t2,t3);t4<=min(PB_M-3,32*t2+31);t4++) {
          A[t3][t4] = B[t3][t4] + B[t4][t3] * B[t4+1][t3-2] - A[t3][t4+2] * 5;;
        }
      }
    }
  }
...
kernel_3124132121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
