...
void kernel_2212131223_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 2) {
  for (t1=0;t1<=min(1,PB_L-2);t1++) {
    B[t1+1] = C[t1] * C[t1+1] + 1;;
  }
  for (t1=2;t1<=PB_L-2;t1++) {
    B[t1+1] = C[t1] * C[t1+1] + 1;;
    B[(t1-1)] = C[(t1-1)] + C[(t1-1)] - C[(t1-1)] - C[0] * B[(t1-1)+2] - B[(t1-1)-1] * 5;;
  }
}
/* End of CLooG code */
...
kernel_2212131223_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
