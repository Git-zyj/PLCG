...
void kernel_2224112211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
  int t1, t2, t3, t4;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_M >= 0) && (PB_N >= max(0,ceild(-3*PB_M+6,2)))) {
  for (t1=0;t1<=floord(PB_N+PB_M-2,32);t1++) {
    if ((PB_N >= 3) && (PB_P <= -1) && (t1 <= floord(PB_N-2,32))) {
      for (t3=max(1,32*t1);t3<=min(PB_N-2,32*t1+31);t3++) {
        A[t3] = C[t3] - C[t3-1] * A[t3+1] - 1;;
      }
    }
    for (t2=0;t2<=min(floord(PB_P,32),floord(2*PB_N+3*PB_P-6,96));t2++) {
      ...
      if ((t2 >= 1) && (t2 <= floord(PB_P-2,32))) {
        for (t3=32*t1;t3<=min(PB_M-2,32*t1+31);t3++) {
          for (t4=32*t2;t4<=min(PB_P-2,32*t2+31);t4++) {
            B[t3][t4] = B[t3][t4+1] * B[t3+1][t4] + 3;;
          }
        }
      }
      if ((PB_M >= 2) && (PB_P >= 2) && (t1 == 0) && (t2 == 0)) {
        for (t4=0;t4<=min(31,PB_P-2);t4++) {
          B[0][t4] = B[0][t4+1] * B[0 +1][t4] + 3;;
        }
      }
      if ((PB_P >= 2) && (t2 == 0)) {
        for (t3=max(1,32*t1);t3<=min(min(PB_M-2,PB_N-2),32*t1+31);t3++) {
          B[t3][0] = B[t3][0 +1] * B[t3+1][0] + 3;;
          A[t3] = C[t3] - C[t3-1] * A[t3+1] - 1;;
      ...
      if ((PB_P >= 2) && (t2 == 0)) {
        for (t3=max(max(1,32*t1),PB_M-1);t3<=min(PB_N-2,32*t1+31);t3++) {
          A[t3] = C[t3] - C[t3-1] * A[t3+1] - 1;;
        }
      }
      if ((PB_P >= 2) && (t2 == 0)) {
        for (t3=max(max(1,32*t1),PB_N-1);t3<=min(PB_M-2,32*t1+31);t3++) {
          for (t4=0;t4<=min(31,PB_P-2);t4++) {
            B[t3][t4] = B[t3][t4+1] * B[t3+1][t4] + 3;;
          }
        }
...
kernel_2224112211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
