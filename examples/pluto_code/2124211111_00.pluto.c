...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_L-2,32),floord(-32*t1+PB_N-1,32));t2++) {
      for (t3=32*t2;t3<=min(min(PB_L-2,32*t2+31),-32*t1+PB_N-1);t3++) {
        lbv=32*t1;
        ubv=min(min(PB_M-1,32*t1+31),-t3+PB_N-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4+1] = B[t4+t3][t3][t4] * A[t3+1][t4+1] + 2;;
        }
      }
    }
  }
