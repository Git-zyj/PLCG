...
void kernel_2212211223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (t1=0;t1<=floord(PB_L+PB_M-1,32);t1++) {
      if (t1 <= floord(PB_L-1,32)) {
        for (t4=max(2,32*t1);t4<=min(PB_L-1,32*t1+31);t4++) {
          B[t4] = B[t4+1] * B[t4+2] * B[t4-1] * B[t4-2] - 4;;
        }
      }
      if ((PB_M >= 4) && (t1 <= floord(PB_M-2,32))) {
        for (t2=0;t2<=min(floord(PB_L-1,32),t1);t2++) {
          for (t3=max(2,32*t2);t3<=min(min(PB_L-1,PB_M-2),32*t2+31);t3++) {
            for (t4=max(32*t1,t3);t4<=min(PB_M-2,32*t1+31);t4++) {
              A[(-t3+t4)+t3+1][0] = C[t3][(-t3+t4)] * 3;;
            }
          }
        }
      }
...
kernel_2212211223_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
