...
void kernel_3112132223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(3,32*t2);t3<=min(PB_N-1,32*t2+31);t3++) {
        for (t4=max(1,32*t1-32*t2);t4<=min(PB_M-1,32*t1-32*t2+31);t4++) {
          A[t3-1][t4] = B[t3] + C[t4][t3] - C[t4][t3-1] * A[t3-1][t4-1] + A[t3-3][t4] + 6;;
        }
      }
    }
  }
...
kernel_3112132223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
