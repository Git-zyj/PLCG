...
void kernel_3112132223_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_P-2,32);t2++) {
      for (t3=0;t3<=min(floord(PB_M-4,32),floord(-32*t1+PB_M-3,32));t3++) {
        for (t4=max(1,32*t1);t4<=min(32*t1+31,-32*t3+PB_M-3);t4++) {
          for (t5=max(1,32*t2);t5<=min(PB_P-2,32*t2+31);t5++) {
            for (t6=32*t3;t6<=min(32*t3+31,-t4+PB_M-3);t6++) {
              A[t4+t6][t5][t6] = B[t5+t4][t5+1][t6] * B[t6][t5][t4] - B[t4][t5-1][t5+t4-1] + A[t4+t6+1][t5][t6+1] - A[t4+t6+2][t5][t6+2] - 1;;
            }
          }
        }
      }
...
kernel_3112132223_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
