...
#pragma vector always
  ...
  if ((PB_M >= 2) && (PB_Q >= 4)) {
    for (t2=0;t2<=floord(PB_M+2*PB_Q-5,32);t2++) {
      lbp=max(ceild(t2,2),ceild(32*t2-PB_Q+2,32));
      ubp=min(min(floord(PB_M+PB_Q-3,32),floord(32*t2+PB_M+30,64)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(2,32*t2-32*t3),32*t3-PB_M+1);t4<=min(min(PB_Q-2,32*t3+30),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4+1);t5<=min(32*t3+31,t4+PB_M-1);t5++) {
            B[(-t4+t5)] = D[(-t4+t5)][t4+1] * D[(-t4+t5)-1][t4] - D[(-t4+t5)-1][t4] * D[t4][0] * D[(-t4+t5)+2][t4-2] + B[(-t4+t5)-1] * 4;;
          }
        }
      }
    }
