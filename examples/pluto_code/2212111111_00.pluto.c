...
void kernel_2212111111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    if (PB_L <= -1) {
      for (t3=max(1,32*t1);t3<=(min(PB_M-1,32*t1+31))-7;t3+=8) {
        A[t3-1] = 6;;
        A[(t3+1)-1] = 6;;
        A[(t3+2)-1] = 6;;
        A[(t3+3)-1] = 6;;
    ...
    for (t2=0;t2<=floord(PB_L,32);t2++) {
      if ((PB_L >= 2) && (t2 == 0)) {
        for (t3=max(1,32*t1);t3<=(min(PB_M-1,32*t1+31))-7;t3+=8) {
          B[t3][0] = C[0] * B[t3][0 +1] + 1;;
          A[t3-1] = 6;;
          B[(t3+1)][0] = C[0] * B[(t3+1)][0 +1] + 1;;
          A[(t3+1)-1] = 6;;
          B[(t3+2)][0] = C[0] * B[(t3+2)][0 +1] + 1;;
          A[(t3+2)-1] = 6;;
          B[(t3+3)][0] = C[0] * B[(t3+3)][0 +1] + 1;;
          A[(t3+3)-1] = 6;;
          B[(t3+4)][0] = C[0] * B[(t3+4)][0 +1] + 1;;
          A[(t3+4)-1] = 6;;
          B[(t3+5)][0] = C[0] * B[(t3+5)][0 +1] + 1;;
          A[(t3+5)-1] = 6;;
          B[(t3+6)][0] = C[0] * B[(t3+6)][0 +1] + 1;;
          A[(t3+6)-1] = 6;;
          B[(t3+7)][0] = C[0] * B[(t3+7)][0 +1] + 1;;
          A[(t3+7)-1] = 6;;
          for (t4=1;t4<=min(31,PB_L-2);t4++) {
            B[t3][t4] = C[t4] * B[t3][t4+1] + 1;;
            B[(t3+1)][t4] = C[t4] * B[(t3+1)][t4+1] + 1;;
            B[(t3+2)][t4] = C[t4] * B[(t3+2)][t4+1] + 1;;
            B[(t3+3)][t4] = C[t4] * B[(t3+3)][t4+1] + 1;;
            B[(t3+4)][t4] = C[t4] * B[(t3+4)][t4+1] + 1;;
            B[(t3+5)][t4] = C[t4] * B[(t3+5)][t4+1] + 1;;
            B[(t3+6)][t4] = C[t4] * B[(t3+6)][t4+1] + 1;;
            B[(t3+7)][t4] = C[t4] * B[(t3+7)][t4+1] + 1;;
          }
        }
        for (;t3<=min(PB_M-1,32*t1+31);t3++) {
          B[t3][0] = C[0] * B[t3][0 +1] + 1;;
...
kernel_2212111111_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
