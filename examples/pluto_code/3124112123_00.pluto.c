...
void kernel_3124112123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,-t1-1);t2<=min(floord(PB_M-1,32),floord(-32*t1+PB_M-1,32));t2++) {
      for (t3=max(max(1,32*t1+1),32*t1+32*t2);t3<=min(min(PB_M-1,32*t1+PB_M+30),32*t1+32*t2+62);t3++) {
        lbv=max(max(1,32*t2),-32*t1+t3-31);
        ubv=min(min(PB_M-1,-32*t1+t3),32*t2+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4] = B[t3][t4] - A[t3-1][t4-1] - 3;;
        }
      }
    }
  }
...
kernel_3124112123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
