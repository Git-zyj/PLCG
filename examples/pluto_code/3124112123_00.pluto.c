...
void kernel_3124112123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(3,ceild(32*t2+1,3)),32*t1-32*t2);t3<=min(min(PB_L-1,16*t2+14),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3+2);t4<=min(32*t2+31,3*t3-1);t4++) {
          A[(-2*t3+t4)] = B[(-2*t3+t4)][t3] - A[(-2*t3+t4)-2] - 5;;
        }
      }
    }
  }
...
kernel_3124112123_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
