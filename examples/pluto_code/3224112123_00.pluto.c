...
void kernel_3224112123_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_Q >= 3) {
  lbp=0;
  ubp=floord(PB_Q-3,32);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    if (PB_L <= -1) {
      for (t4=32*t1;t4<=min(PB_Q-3,32*t1+31);t4++) {
        B[t4] = D[t4] - 1;;
      }
    }
    for (t2=0;t2<=floord(PB_L,32);t2++) {
      ...
      for (t3=0;t3<=floord(PB_M,32);t3++) {
        if ((PB_L >= 5) && (PB_M >= 5) && (t2 == 0) && (t3 == 0)) {
          for (t4=32*t1;t4<=min(PB_Q-3,32*t1+31);t4++) {
            B[t4] = D[t4] - 1;;
            for (t5=0;t5<=min(31,PB_L-5);t5++) {
              for (t6=2;t6<=min(31,PB_M-3);t6++) {
                A[t4][t5+2][t6] = C[t6+2][t5+2][t4+2] + A[t4][t5+4][t6] + A[t4][t5+2][t6-2] * 3;;
              }
            }
          }
        }
        if ((PB_M >= 5) && (t2 <= floord(PB_L-5,32)) && (t2 >= -t3+1) && (t3 <= floord(PB_M-3,32))) {
          for (t4=32*t1;t4<=min(PB_Q-3,32*t1+31);t4++) {
            for (t5=32*t2;t5<=min(PB_L-5,32*t2+31);t5++) {
              for (t6=max(2,32*t3);t6<=min(PB_M-3,32*t3+31);t6++) {
                A[t4][t5+2][t6] = C[t6+2][t5+2][t4+2] + A[t4][t5+4][t6] + A[t4][t5+2][t6-2] * 3;;
              }
            }
          }
...
kernel_3224112123_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
