...
if (PB_L >= 1) {
  for (t1=0;t1<=floord(PB_L-1,32);t1++) {
    for (t2=0;t2<=min(floord(PB_L-1,8),floord(96*t1+PB_L+92,32));t2++) {
      if ((PB_L >= 3) && (t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=min(9,PB_L-1);t3++) {
          B[t3+1] = D[t3] - D[t3] * D[t3+1] - B[t3+2] - B[t3] + 1;;
          for (t4=3*t3+2;t4<=min(31,3*t3+PB_L-1);t4++) {
            A[t3][(-3*t3+t4)] = C[t3+1][(-3*t3+t4)] + C[(-3*t3+t4)][t3] - C[(-3*t3+t4)-1][t3] * A[(-3*t3+t4)-2][t3] * A[t3][(-3*t3+t4)-1] - 6;;
          }
        }
      }
      if (t2 >= 1) {
        for (t3=max(ceild(32*t2-PB_L+1,3),32*t1);t3<=min(min(floord(32*t2+29,3),PB_L-1),32*t1+31);t3++) {
      ...
      if (t2 == 0) {
        for (t3=max(10,32*t1);t3<=min(PB_L-1,32*t1+31);t3++) {
          B[t3+1] = D[t3] - D[t3] * D[t3+1] - B[t3+2] - B[t3] + 1;;
        }
      }
    }
