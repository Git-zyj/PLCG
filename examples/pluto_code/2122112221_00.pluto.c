...
void kernel_2122112221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_M >= 4) {
  for (t1=0;t1<=floord(3*PB_M-6,32);t1++) {
    lbp=max(max(0,ceild(16*t1-PB_M+2,16)),ceild(32*t1-PB_M-30,64));
    ubp=min(floord(t1,2),floord(PB_M-2,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t2),32*t1-32*t2-PB_M+1);t3<=min(min(PB_M-2,32*t2+31),32*t1-32*t2+29);t3++) {
        lbv=max(32*t1-32*t2,t3+2);
        ubv=min(32*t1-32*t2+31,t3+PB_M-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-t3+t4)-2] = B[(-t3+t4)][t3] + A[t3+1][(-t3+t4)-2] * A[t3-2][(-t3+t4)] + 5;;
        }
      }
    }
  }
...
kernel_2122112221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
