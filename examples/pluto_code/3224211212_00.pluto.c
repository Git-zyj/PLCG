...
void kernel_3224211212_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
for (t1=0;t1<=floord(PB_M-1,32);t1++) {
  for (t2=0;t2<=min(floord(PB_M-2,16),2*t1+1);t2++) {
    if (t2 >= 1) {
      for (t3=max(32*t1,16*t2+1);t3<=min(min(PB_M-1,32*t1+31),32*t2+30);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(min(32*t2+31,2*t3-1),t3+PB_M-3);t4++) {
          A[(-t3+t4)+1][t3] = C[t3][(-t3+t4)-1][t3-1] - D[(-t3+t4)+1][t3] + A[(-t3+t4)][t3+1] * A[(-t3+t4)+2][t3] - A[(-t3+t4)+2][t3+1] - 6;;
        }
      }
    }
    if ((t1 == 0) && (t2 == 0)) {
    ...
    if ((t1 == 0) && (t2 == 0)) {
      for (t3=2;t3<=30;t3++) {
        B[t3] = B[t3-1] - 2;;
        for (t4=t3+1;t4<=min(31,2*t3-1);t4++) {
          A[(-t3+t4)+1][t3] = C[t3][(-t3+t4)-1][t3-1] - D[(-t3+t4)+1][t3] + A[(-t3+t4)][t3+1] * A[(-t3+t4)+2][t3] - A[(-t3+t4)+2][t3+1] - 6;;
        }
      }
    }
...
kernel_3224211212_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
