...
void kernel_3114212221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_L-3,32);t2++) {
      for (t3=max(1,32*t1);t3<=min(min(PB_L-4,32*t1+31),32*t2+30);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(PB_L-3,32*t2+31);t4++) {
          A[t3-1][t4+1] = B[t3][t4] + A[t3-1][t4-1] + A[t3-1][t4+2] + 5;;
        }
      }
    }
  }
...
kernel_3114212221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
