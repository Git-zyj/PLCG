...
void kernel_2212211113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
      ...
      if (t2 >= 1) {
        for (t3=32*t1;t3<=min(PB_M-1,32*t1+31);t3++) {
          for (t4=32*t2;t4<=min(PB_M-2,32*t2+31);t4++) {
            A[t4][t3] = C[t3][t4][t3] - A[t4+1][t3] + 5;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t4=0;t4<=min(31,PB_M-2);t4++) {
          A[t4][0] = C[0][t4][0] - A[t4+1][0] + 5;;
        }
      }
      if (t2 == 0) {
        for (t3=max(1,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
          A[0][t3] = C[t3][0][t3] - A[0 +1][t3] + 5;;
          B[t3] = D[t3][0] * B[t3-1] - 5;;
...
kernel_2212211113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
