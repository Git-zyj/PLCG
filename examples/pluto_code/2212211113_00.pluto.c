...
void kernel_2212211113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_M-2,32),floord(-32*t1+PB_M-1,32));t2++) {
      for (t3=32*t2;t3<=min(min(32*t1,32*t2+31),-32*t1+PB_M-2);t3++) {
        lbv=32*t1;
        ubv=min(32*t1+31,-t3+PB_M-2);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4+t3] = C[t4][t3] * D[t4+t3] + A[t3+1][t4+t3+1] - 3;;
        }
      }
      for (t3=max(32*t2,32*t1+1);t3<=min(32*t2+31,-32*t1+PB_M-2);t3++) {
        lbv=32*t1;
        ubv=min(min(32*t1+31,t3-1),-t3+PB_M-2);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4+t3] = C[t4][t3] * D[t4+t3] + A[t3+1][t4+t3+1] - 3;;
...
#pragma vector always
        ...
        if ((t1 >= ceild(-t3+PB_M-32,32)) && (t3 >= ceild(PB_M,2))) {
          A[t3][(-t3+PB_M-1)+t3] = 2;;
        }
      }
      if ((t1 <= floord(PB_M-2,64)) && (t1 >= max(1,ceild(-32*t2+PB_M-32,32)))) {
        A[(-32*t1+PB_M-1)][32*t1+(-32*t1+PB_M-1)] = 2;;
      }
    }
  }
}
...
kernel_2212211113_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
