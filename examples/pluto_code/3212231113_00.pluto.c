...
void kernel_3212231113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_M >= 3) {
  for (t1=0;t1<=floord(PB_M-2,32);t1++) {
    for (t2=0;t2<=min(floord(3*PB_M-5,32),floord(64*t1+PB_M+61,32));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=1;t3<=min(15,PB_M-2);t3++) {
          B[t3] = D[t3+1] + D[t3+1] - D[t3] * B[t3-1] + 3;;
          for (t4=2*t3;t4<=min(31,2*t3+PB_M-1);t4++) {
            A[t3+1][(-2*t3+t4)] = C[(-2*t3+t4)][t3+1] - D[(-2*t3+t4)] + D[(-2*t3+t4)] - A[(-2*t3+t4)][t3] - 1;;
          }
        }
      }
      if (t2 >= 1) {
        for (t3=max(max(1,ceild(32*t2-PB_M+1,2)),32*t1);t3<=min(min(PB_M-2,32*t1+31),16*t2+15);t3++) {
          for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,2*t3+PB_M-1);t4++) {
            A[t3+1][(-2*t3+t4)] = C[(-2*t3+t4)][t3+1] - D[(-2*t3+t4)] + D[(-2*t3+t4)] - A[(-2*t3+t4)][t3] - 1;;
          }
        }
      }
...
kernel_3212231113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
