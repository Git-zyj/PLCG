...
void kernel_3212231113_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (t2=0;t2<=floord(PB_M-2,32);t2++) {
      ...
      if ((t1 == 0) && (t2 == 0)) {
        for (t4=1;t4<=min(31,PB_M-2);t4++) {
          B[0] = C[0] + C[0] + D[0][t4-1] + 4;;
        }
      }
      if (t1 == t2) {
        for (t3=max(1,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
          for (t4=max(1,32*t1);t4<=t3-1;t4++) {
            B[t3] = C[t3] + C[t3] + D[t3][t4-1] + 4;;
          }
          B[t3] = C[t3] * C[t3+1] - C[t3] + B[t3] - 4;;
          B[t3] = C[t3] + C[t3] + D[t3][t3-1] + 4;;
          for (t4=t3+1;t4<=min(PB_M-2,32*t1+31);t4++) {
            B[t3] = C[t3] + C[t3] + D[t3][t4-1] + 4;;
          }
        }
      }
      if ((t1 == t2) && (t1 >= ceild(PB_M-32,32))) {
        for (t4=max(1,32*t1);t4<=PB_M-2;t4++) {
          B[(PB_M-1)] = C[(PB_M-1)] + C[(PB_M-1)] + D[(PB_M-1)][t4-1] + 4;;
        }
      }
    }
  }
...
kernel_3212231113_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
