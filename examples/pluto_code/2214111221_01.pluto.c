...
void kernel_2214111221_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if (PB_L >= 5) {
  for (t2=0;t2<=floord(PB_L-3,64);t2++) {
    lbp=max(ceild(-PB_L-27,32),ceild(32*t2-PB_L-28,32));
    ubp=min(-1,-t2);
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
    for (t4=lbp;t4<=ubp;t4++) {
      for (t5=max(1,32*t2);t5<=min(min(min(floord(PB_L-3,2),-32*t4),32*t2+31),32*t4+PB_L+28);t5++) {
        for (t7=max(32*t4,t5-PB_L+3);t7<=min(-t5,32*t4+31);t7++) {
          A[-t7+t5][t5-1][-t7+1] = A[-t7+t5+1][t5-1][-t7+2] - A[-t7+t5+2][t5-1][-t7+3] - A[-t7+t5+1][t5][-t7+1] + 5;;
        }
      }
    }
  }
...
for (t2=0;t2<=floord(PB_M-1,32);t2++) {
  lbp=0;
  ubp=floord(-32*t2+PB_M-1,32);
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8)
  for (t4=lbp;t4<=ubp;t4++) {
    for (t5=32*t2;t5<=min(32*t2+31,-32*t4+PB_M-1);t5++) {
      lbv=32*t4;
      ubv=min(32*t4+31,-t5+PB_M-1);
#pragma ivdep
...
kernel_2214111221_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
