...
void kernel_2122231223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 5) {
  for (t1=3;t1<=PB_M-2;t1++) {
    A[t1-1] = B[t1] * C[0] - B[t1] * A[t1-3] + A[t1+1] - 4;;
  }
}
/* End of CLooG code */
...
kernel_2122231223_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
