...
void kernel_3222112123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
if (PB_L >= 3) {
  for (t1=2;t1<=PB_L-1;t1++) {
    for (t2=0;t2<=floord(PB_L-1,32);t2++) {
      ...
      if (PB_M >= 0) {
        for (t3=0;t3<=min(min(min(floord(3*PB_L+PB_M-5,32),floord(96*t2+PB_M+91,32)),floord(144*t2+47*PB_M,48)),floord(9*PB_L+7*PB_M-27,96));t3++) {
          if ((PB_M >= 3) && (t2 == 0) && (t3 == 0)) {
            for (t4=0;t4<=min(10,PB_L-1);t4++) {
              A[t1+2][t4] = 4;;
              for (t5=3*t4+1;t5<=min(31,3*t4+PB_M-2);t5++) {
                B[(-3*t4+t5)][t1][t4] = C[t4][(-3*t4+t5)-1][t1] - C[(-3*t4+t5)][(-3*t4+t5)][t1] + B[(-3*t4+t5)+1][t1][t4] + B[t4][(-3*t4+t5)][t1-2] * 6;;
              }
            }
          }
          if ((PB_M <= 2) && (t3 == 0)) {
          ...
          if ((PB_M >= 3) && (t3 == 0)) {
            for (t4=max(11,32*t2);t4<=min(PB_L-1,32*t2+31);t4++) {
              A[t1+2][t4] = 4;;
            }
          }
          if ((PB_M >= 3) && (t3 >= 1)) {
            for (t4=max(ceild(32*t3-PB_M+2,3),32*t2);t4<=min(min(floord(32*t3+30,3),PB_L-1),32*t2+31);t4++) {
              for (t5=max(32*t3,3*t4+1);t5<=min(32*t3+31,3*t4+PB_M-2);t5++) {
                B[(-3*t4+t5)][t1][t4] = C[t4][(-3*t4+t5)-1][t1] - C[(-3*t4+t5)][(-3*t4+t5)][t1] + B[(-3*t4+t5)+1][t1][t4] + B[t4][(-3*t4+t5)][t1-2] * 6;;
              }
            }
          }
...
kernel_3222112123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
