...
void kernel_3114132221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-2,32);t2++) {
      for (t3=2*t2;t3<=min(floord(PB_L-2,16),floord(32*t2+PB_L+29,32));t3++) {
        for (t4=max(2,32*t1);t4<=min(PB_M-3,32*t1+31);t4++) {
          for (t5=max(max(2,32*t2),32*t3-PB_L+2);t5<=min(min(PB_L-2,32*t2+31),16*t3+15);t5++) {
            for (t6=max(32*t3,2*t5);t6<=min(32*t3+31,t5+PB_L-2);t6++) {
              A[t4+2][(-t5+t6)] = B[t4-2][(-t5+t6)-1][t5+1] + C[(-t5+t6)][(-t5+t6)] * B[t4][t5-2][(-t5+t6)-2] - A[t4+2][(-t5+t6)+1] * A[t4+2][(-t5+t6)-1] * 1;;
            }
          }
        }
      }
...
kernel_3114132221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
