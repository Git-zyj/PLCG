...
void kernel_2114231213_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_Q >= 4) {
  for (t1=0;t1<=floord(PB_Q-2,32);t1++) {
    lbp=max(0,ceild(32*t1-PB_Q+3,32));
    ubp=min(floord(PB_Q-3,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(1,32*t2);t3<=min(min(PB_Q-3,32*t2+31),-32*t1+32*t2+PB_Q-2);t3++) {
        for (t4=max(1,32*t1-32*t2);t4<=min(32*t1-32*t2+31,-t3+PB_Q-2);t4++) {
          A[t4][t3][t3+t4] = B[t3+1][t4+1] * B[t3+t4+1][t4-1] + C[t3] - A[t4+1][t3][t3+t4+1] - A[t4][t3-1][t3+t4-1] * 6;;
        }
      }
    }
  }
...
kernel_2114231213_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
