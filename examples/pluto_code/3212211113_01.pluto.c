...
if (PB_P >= 3) {
  for (t1=0;t1<=floord(PB_P-2,32);t1++) {
    ...
    for (t2=0;t2<=floord(PB_L,32);t2++) {
      for (t3=0;t3<=min(floord(PB_P-3,32),floord(-32*t1+PB_P-2,32));t3++) {
        if ((PB_L >= 1) && (t2 == 0) && (t3 == 0)) {
          for (t6=max(1,32*t1);t6<=min(PB_P-2,32*t1+31);t6++) {
            A[0][0][t6] = C[0][0 +t6+1][t6] - 5;;
            B[t6] = B[t6-1] * 3;;
          }
          for (t5=1;t5<=min(min(31,PB_P-3),-32*t1+PB_P-2);t5++) {
            for (t6=max(1,32*t1);t6<=min(32*t1+31,-t5+PB_P-2);t6++) {
              A[0][t5][t6] = C[0][t5+t6+1][t6] - 5;;
            }
          }
        }
        if ((PB_L >= 1) && (t2 == 0) && (t3 >= 1)) {
          for (t5=32*t3;t5<=min(min(PB_P-3,32*t3+31),-32*t1+PB_P-2);t5++) {
            for (t6=max(1,32*t1);t6<=min(32*t1+31,-t5+PB_P-2);t6++) {
              A[0][t5][t6] = C[0][t5+t6+1][t6] - 5;;
            }
          }
