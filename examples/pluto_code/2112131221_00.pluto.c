...
void kernel_2112131221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_M >= 5) {
  for (t1=1;t1<=PB_M;t1++) {
    for (t2=1;t2<=PB_M-4;t2++) {
      A[t2+1] = B[0][t2+1] * B[t2-1][t1] + B[t2-1][t1] * A[t1-1] + A[t2+3] - 2;;
    }
  }
}
/* End of CLooG code */
...
kernel_2112131221_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
