...
void kernel_2112211123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_M+2);t3<=min(PB_L-1,32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3);t4<=min(32*t2+31,t3+PB_M-2);t4++) {
          A[(-t3+t4)] = B[(-t3+t4)+1] * A[(-t3+t4)+1] + 2;;
        }
      }
    }
  }
...
kernel_2112211123_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
