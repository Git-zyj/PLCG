...
void kernel_3122111213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(32*t2-32*t3,32*t3-PB_L+1);t4<=min(PB_L-1,32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4);t5<=min(32*t3+31,t4+PB_L-1);t5++) {
            A[(-t4+t5)+1][0] = B[t1+1][t4][(-t4+t5)] + A[(-t4+t5)+2][0] + A[(-t4+t5)][0] * 3;;
          }
        }
      }
    }
...
kernel_3122111213_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
