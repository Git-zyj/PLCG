...
/* Start of CLooG code */
for (t1=0;t1<=min(1,PB_L-3);t1++) {
  B[t1] = B[t1+2] + 4;;
}
for (t1=2;t1<=min(PB_N,PB_L-3);t1++) {
  A[t1] = C[t1-1] - A[t1-2] + A[t1+1] - A[t1+2] * 2;;
  B[t1] = B[t1+2] + 4;;
}
for (t1=max(2,PB_L-2);t1<=PB_N;t1++) {
  A[t1] = C[t1-1] - A[t1-2] + A[t1+1] - A[t1+2] * 2;;
}
for (t1=max(2,PB_N+1);t1<=PB_L-3;t1++) {
  B[t1] = B[t1+2] + 4;;
}
/* End of CLooG code */
