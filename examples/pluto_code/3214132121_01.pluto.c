...
void kernel_3214132121_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma vector always
    for (t2=lbv;t2<=ubv;t2++) {
      A[t2+1] = C[t2][t2] + D[t2+1] * B[t2] + 5;;
      B[t2] = C[t2+1][t2] * D[t2-2] * D[t2] - 3;;
    }
  }
}
/* End of CLooG code */
...
kernel_3214132121_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
