...
void kernel_3112112221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if ((PB_M >= 2) && (PB_N >= 1)) {
  for (t1=1;t1<=PB_M-1;t1++) {
    for (t2=0;t2<=min(PB_N-1,t1-1);t2++) {
      A[2] = B[t2] * A[2] + A[2] * 1;;
    }
  }
}
...
kernel_3112112221_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
