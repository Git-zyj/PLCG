...
void kernel_3214232221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
/* Start of CLooG code */
if (PB_M >= 3) {
  for (t1=0;t1<=min(3,PB_M-3);t1++) {
    A[t1] = C[1] * C[t1+1] + A[t1+1] * A[t1+2] - 6;;
  }
  for (t1=4;t1<=PB_M-3;t1++) {
    A[t1] = C[1] * C[t1+1] + A[t1+1] * A[t1+2] - 6;;
    A[(t1-2)+2] = C[(t1-2)] + C[(t1-2)] + D[(t1-2)-2][(t1-2)+2] - 5;;
  }
  for (t1=max(4,PB_M-2);t1<=PB_M-1;t1++) {
    A[(t1-2)+2] = C[(t1-2)] + C[(t1-2)] + D[(t1-2)-2][(t1-2)+2] - 5;;
  }
}
/* End of CLooG code */
...
kernel_3214232221_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
