...
void kernel_2114212113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_M >= 2) {
  for (t1=1;t1<=PB_M;t1++) {
    for (t2=0;t2<=PB_M-2;t2++) {
      A[t2][t2] = B[t2+1][t2] - A[t1-1][t1-1] + 2;;
    }
  }
}
/* End of CLooG code */
...
kernel_2114212113_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
