...
void kernel_2214211121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_L >= 3) {
  for (t1=0;t1<=floord(PB_L-2,32);t1++) {
    for (t2=0;t2<=min(floord(3*PB_L-5,32),floord(64*t1+PB_L+61,32));t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=1;t3<=min(15,PB_L-2);t3++) {
          A[t3] = A[t3-1] - 6;;
          for (t4=2*t3+1;t4<=min(31,2*t3+PB_L-1);t4++) {
            B[(-2*t3+t4)+1][(-2*t3+t4)+1] = C[(-2*t3+t4)][(-2*t3+t4)] * C[(-2*t3+t4)+1][t3+1] - B[(-2*t3+t4)-1][(-2*t3+t4)-1] * 3;;
          }
        }
      }
      if (t2 == 0) {
      ...
      if (t2 >= 1) {
        for (t3=max(max(1,ceild(32*t2-PB_L+1,2)),32*t1);t3<=min(min(PB_L-2,32*t1+31),16*t2+15);t3++) {
          for (t4=max(32*t2,2*t3+1);t4<=min(32*t2+31,2*t3+PB_L-1);t4++) {
            B[(-2*t3+t4)+1][(-2*t3+t4)+1] = C[(-2*t3+t4)][(-2*t3+t4)] * C[(-2*t3+t4)+1][t3+1] - B[(-2*t3+t4)-1][(-2*t3+t4)-1] * 3;;
          }
        }
      }
...
kernel_2214211121_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
