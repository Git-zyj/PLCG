...
void kernel_3124131121_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_M >= 3) {
  for (t2=-PB_M+1;t2<=-2;t2++) {
    A[-t2-1] = B[-t2] + B[-t2] + B[-t2] + A[-t2-2] * 1;;
  }
}
/* End of CLooG code */
...
kernel_3124131121_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
