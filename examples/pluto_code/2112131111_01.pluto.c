...
void kernel_2112131111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_M >= 3) {
  for (t1=1;t1<=PB_M-2;t1++) {
    A[t1+1] = B[t1-1] - B[t1] + B[t1+1] * A[t1] + 4;;
  }
}
/* End of CLooG code */
...
kernel_2112131111_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
