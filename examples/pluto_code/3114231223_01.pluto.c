...
void kernel_3114231223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_L >= 5) {
  for (t1=3;t1<=PB_L-2;t1++) {
    A[t1-1] = B[t1] - C[t1+1] - B[t1+1] - A[t1+1] + A[t1-3] + 3;;
  }
}
/* End of CLooG code */
...
kernel_3114231223_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
