...
void kernel_3112231113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_P-1,32),t1);t2++) {
      for (t3=max(1,32*t2);t3<=min(min(PB_L-1,PB_P-1),32*t2+31);t3++) {
        lbv=max(32*t1,t3);
        ubv=min(PB_L-1,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4] = B[t3-1][t4] + C[t4][t4+1] * C[t4][t3] - A[t3-1][t4] + 5;;
        }
      }
    }
  }
...
kernel_3112231113_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
