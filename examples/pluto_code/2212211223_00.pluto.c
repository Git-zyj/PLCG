...
void kernel_2212211223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 2) {
  if (PB_L >= 0) {
    for (t1=0;t1<=floord(PB_M+PB_L-1,32);t1++) {
      for (t2=max(0,ceild(32*t1-PB_M+1,16));t2<=min(min(min(min(min(min(floord(32*t1+31*PB_M-62,16),floord(3*PB_L,32)),floord(64*t1+PB_M+59,32)),floord(-32*t1+PB_M+3*PB_L-1,32)),floord(6*t1+3*PB_L,34)),floord(96*t1+32*PB_M+61*PB_L-64,1024)),3*t1+2);t2++) {
        if ((t1 == 0) && (t2 == 0)) {
          A[1] = C[1 -1] - A[1 -1] * 1;;
        }
        if ((PB_M >= 3) && (t2 >= 1)) {
          for (t3=max(max(ceild(32*t2-PB_M+3,2),ceild(32*t2+1,3)),32*t1);t3<=min(min(PB_L-1,32*t1+31),16*t2+15);t3++) {
            for (t4=max(32*t2,2*t3);t4<=min(min(32*t2+31,3*t3-1),2*t3+PB_M-3);t4++) {
              B[(-2*t3+t4)][t3] = B[(-2*t3+t4)+2][t3] - B[(-2*t3+t4)][t3-2] - B[(-2*t3+t4)+2][t3-1] - 1;;
            }
          }
        }
        if ((t1 == 0) && (t2 == 0)) {
          for (t3=2;t3<=min(min(15,PB_L-1),PB_M-1);t3++) {
            A[t3] = C[t3-1] - A[t3-1] * 1;;
        ...
        if ((t1 == 0) && (t2 == 0)) {
          for (t3=max(2,PB_L);t3<=min(15,PB_M-1);t3++) {
            A[t3] = C[t3-1] - A[t3-1] * 1;;
          }
        }
        if ((PB_M >= 3) && (t1 == 0) && (t2 == 0)) {
          for (t3=PB_M;t3<=min(15,PB_L-1);t3++) {
            for (t4=2*t3;t4<=min(31,2*t3+PB_M-3);t4++) {
              B[(-2*t3+t4)][t3] = B[(-2*t3+t4)+2][t3] - B[(-2*t3+t4)][t3-2] - B[(-2*t3+t4)+2][t3-1] - 1;;
            }
          }
        }
...
kernel_2212211223_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
