...
void kernel_2224132121_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t1-32*t2),32*t2-PB_M+3);t3<=min(16*t2-1,32*t1-32*t2+31);t3++) {
        for (t4=32*t2;t4<=min(32*t2+31,t3+PB_M-3);t4++) {
          B[(-t3+t4-1)+2] = D[t3+1][(-t3+t4-1)] * D[(-t3+t4-1)][t3] + D[(-t3+t4-1)+2][t3] * B[(-t3+t4-1)+3] - 2;;
        }
      }
      for (t3=max(max(2,16*t2),32*t1-32*t2);t3<=min(min(PB_M-3,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=2*t3-1;t4++) {
          B[(-t3+t4-1)+2] = D[t3+1][(-t3+t4-1)] * D[(-t3+t4-1)][t3] + D[(-t3+t4-1)+2][t3] * B[(-t3+t4-1)+3] - 2;;
        }
        B[t3+2] = C[t3-1] + C[t3+1] + C[t3-2] * 1;;
        B[(t3-1)+2] = D[t3+1][(t3-1)] * D[(t3-1)][t3] + D[(t3-1)+2][t3] * B[(t3-1)+3] - 2;;
        for (t4=2*t3+1;t4<=min(32*t2+31,t3+PB_M-3);t4++) {
          B[(-t3+t4-1)+2] = D[t3+1][(-t3+t4-1)] * D[(-t3+t4-1)][t3] + D[(-t3+t4-1)+2][t3] * B[(-t3+t4-1)+3] - 2;;
        }
      }
      for (t3=max(32*t1-32*t2,16*t2+16);t3<=min(min(PB_M-3,32*t2+30),32*t1-32*t2+31);t3++) {
...
kernel_2224132121_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
