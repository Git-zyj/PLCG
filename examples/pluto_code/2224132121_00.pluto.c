...
void kernel_2224132121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
  ...
  for (t1=0;t1<=floord(PB_M-3,32);t1++) {
    if (PB_M >= 5) {
      for (t4=max(2,32*t1);t4<=min(PB_M-3,32*t1+31);t4++) {
        B[t4-1] = D[t4+2] + D[t4] * D[t4] - D[t4] - B[t4-2] * 6;;
      }
    }
    lbp=0;
    ubp=min(floord(PB_M-2,32),floord(-32*t1+PB_M-1,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(2,32*t2);t3<=min(min(PB_M-2,32*t2+31),-32*t1+PB_M-1);t3++) {
        for (t4=32*t1;t4<=min(32*t1+31,-t3+PB_M-1);t4++) {
          A[t3+t4][t3-2] = C[t3+t4][t3+1] * C[t3][t4] - 1;;
        }
      }
    }
  }
...
kernel_2224132121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
