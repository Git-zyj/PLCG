...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_L-2,32),floord(PB_P-1,32));t2++) {
      for (t3=0;t3<=min(floord(-32*t2+PB_L-1,32),t1);t3++) {
        for (t4=max(2,32*t1);t4<=min(PB_P-1,32*t1+31);t4++) {
          for (t5=max(1,32*t3);t5<=min(min(t4,32*t3+31),-32*t2+PB_L-1);t5++) {
            lbv=32*t2;
            ubv=min(min(PB_P-1,32*t2+31),-t5+PB_L-1);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t5][t4-2][t6] = B[t4][t6+t5][1] - A[t5-1][t4-2][t6] - 2;;
            }
          }
        }
      }
