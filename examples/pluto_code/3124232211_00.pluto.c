...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 1) && (PB_N >= 3)) {
  lbp=0;
  ubp=min(floord(PB_M-1,32),floord(PB_N-3,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_L-1,32),floord(-32*t1+PB_N-1,32));t2++) {
      for (t3=32*t1;t3<=min(min(min(PB_M-1,PB_N-3),32*t1+31),-32*t2+PB_N-1);t3++) {
        for (t4=max(2,32*t2);t4<=min(min(PB_L-1,32*t2+31),-t3+PB_N-1);t4++) {
          A[t3][t4-1] = B[t4+t3][t3] - B[t3][t4] * B[t3+2][t4] + A[t3][t4-2] + A[t3][t4] * 4;;
        }
      }
    }
  }
