...
void kernel_3114211113_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 3)) {
  for (t1=1;t1<=min(PB_L-1,PB_M-2);t1++) {
    for (t2=max(2,t1);t2<=PB_L-1;t2++) {
      for (t3=0;t3<=PB_L-2;t3++) {
        A[t1-1][t2][t2] = B[t1+1][t3][t2] + A[t2-2][t3+1][t3+1] * 6;;
      }
    }
  }
}
...
kernel_3114211113_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
