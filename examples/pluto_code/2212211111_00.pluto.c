...
void kernel_2212211111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_M+2);t3<=min(PB_L-2,32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3);t4<=min(32*t2+31,t3+PB_M-2);t4++) {
          A[(-t3+t4)] = C[(-t3+t4)+1][t3+1] - 5;;
          B[t3+1][(-t3+t4)] = C[(-t3+t4)][t3] - A[(-t3+t4)] * A[(-t3+t4)+1] - 1;;
        }
      }
    }
  }
...
kernel_2212211111_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
