...
if ((PB_L >= 3) && (PB_P >= 3)) {
  ...
  ubp=min(floord(PB_L-2,32),floord(PB_P-2,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,-t1-1);t2<=min(min(floord(PB_L-3,32),floord(-32*t1+PB_P-2,32)),floord(-32*t1+PB_L-2,64));t2++) {
      for (t3=max(32*t2,-32*t1-30);t3<=min(min(min(floord(-32*t1+PB_L-2,2),PB_L-3),32*t2+31),-32*t1+PB_P-2);t3++) {
        lbv=max(1,32*t1+t3);
        ubv=min(min(PB_P-2,32*t1+t3+31),-t3+PB_L-2);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3+t4-1][t4] = B[t3][t4+1] + B[t3][t4] + C[t4-1][t3] * A[t3+t4+1][t4+1] + 5;;
        }
      }
    }
  }
