...
if (PB_M >= 3) {
  for (t2=1;t2<=PB_M-2;t2++) {
    A[t2] = C[t2] + D[t2-1] * A[t2-1] * A[t2+1] + 2;;
  }
...
#pragma omp parallel for private(lbv,ubv,t4,t5)
    for (t3=lbp;t3<=ubp;t3++) {
      for (t4=max(32*t2-32*t3,16*t3+1);t4<=min(PB_M-2,32*t2-32*t3+31);t4++) {
        for (t5=max(32*t3,t4);t5<=min(32*t3+31,2*t4-1);t5++) {
          B[(-t4+t5)+1] = E[(-t4+t5)][t4] + F[t4-1][(-t4+t5)+1] * D[t4] * F[(-t4+t5)][t4-1] * A[(-t4+t5)] + B[(-t4+t5)+2] + 5;;
        }
      }
    }
  }
