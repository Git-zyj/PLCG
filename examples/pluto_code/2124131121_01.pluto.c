...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,-t1-1);t2<=min(floord(PB_L-3,32),floord(-32*t1+PB_L-3,32));t2++) {
      for (t3=max(1,32*t1+32*t2);t3<=min(min(PB_L-3,32*t1+PB_L+28),32*t1+32*t2+62);t3++) {
        lbv=max(32*t2,-32*t1+t3-31);
        ubv=min(min(PB_L-3,-32*t1+t3),32*t2+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4] = B[t3-1] * C[t3][0] * C[t3][t4] + A[t3+2][t4+2] + 1;;
        }
      }
    }
  }
