...
if (PB_L >= 4) {
  for (t1=0;t1<=floord(PB_L-2,16);t1++) {
    ...
    ubp=min(floord(PB_L-4,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t2;t3<=min(PB_L-4,32*t2+31);t3++) {
        for (t4=max(1,32*t1-32*t2);t4<=min(PB_L,32*t1-32*t2+31);t4++) {
          A[t4+1][t3+1][t4+1] = B[t3][t4-1] + A[t4+1][t3+3][t4+1] + A[t4+2][t3+1][t4+2] * 1;;
        }
      }
    }
  }
