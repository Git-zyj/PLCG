...
void kernel_3114212223_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if (PB_L >= 3) {
  for (t1=0;t1<=floord(3*PB_L-5,32);t1++) {
    ...
    ubp=min(floord(PB_L-2,16),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(2,32*t1-32*t2);t3<=min(PB_L-1,32*t1-32*t2+31);t3++) {
        for (t4=32*t2;t4<=min(2*PB_L-3,32*t2+31);t4++) {
          lbv=max(0,t4-PB_L+2);
          ubv=min(t4,PB_L-1);
#pragma ivdep
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            A[t3][(t4-t5)][t5+1] = B[t5][t3][t3-2] * A[t3][(t4-t5)+1][t5+1] - A[t3-1][t5][(t4-t5)+1] - 2;;
          }
        }
      }
    }
...
kernel_3114212223_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
