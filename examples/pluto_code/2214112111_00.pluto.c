...
void kernel_2214112111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
    ...
    for (t3=0;t3<=floord(PB_M-1,32);t3++) {
      for (t4=max(2,32*t2);t4<=min(PB_M,32*t2+31);t4++) {
        for (t5=32*t3;t5<=min(PB_M-1,32*t3+31);t5++) {
          A[t4-1][t5] = C[t5][t4-2] - D[t5] + A[t4-1][t5+1] * B[t4+1] * 5;;
        }
      }
    }
  }
...
kernel_2214112111_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
