...
void kernel_3114212113_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
  int t1, t2, t3, t4, t5, t6;
 int lb, ub, lbp, ubp, lb2, ub2;
 register int lbv, ubv;
/* Start of CLooG code */
if ((PB_M >= 2) && (PB_P >= 5) && (PB_Q >= 1)) {
  lbp=0;
        ...
        for (t4=max(1,32*t1);t4<=min(PB_M-1,32*t1+31);t4++) {
          for (t5=max(2,32*t2);t5<=min(PB_P-3,32*t2+31);t5++) {
            for (t6=32*t3;t6<=min(PB_Q-1,32*t3+31);t6++) {
              A[t6][t5-2][t4-1][t5-1] = B[t4][t5+2][t6] - A[t6+1][t5-2][t4-1][t5-1] + 5;;
            }
          }
        }
      }
...
kernel_3114212113_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
