...
void kernel_2112132211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if (PB_N >= 3) {
  for (t1=0;t1<=floord(PB_N-2,16);t1++) {
    lbp=max(0,ceild(32*t1-PB_N+2,32));
    ubp=min(floord(PB_N-1,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(1,32*t1-32*t2);t3<=min(PB_N-2,32*t1-32*t2+31);t3++) {
        for (t4=32*t2;t4<=min(PB_N-1,32*t2+31);t4++) {
          A[t3][t4] = B[t3][t4][t4] * C[t4][t3] + C[t3-1][t4] + A[t3+1][t4+1] - A[t3][t4+1] - 1;;
        }
      }
    }
  }
...
kernel_2112132211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
