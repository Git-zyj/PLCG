...
void kernel_2112132211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
/* Start of CLooG code */
if (PB_M >= 5) {
  for (t2=-PB_M+4;t2<=-1;t2++) {
    A[-t2+2] = B[1] * B[-t2-1] - B[-t2] * A[-t2+1] - A[-t2+3] * 2;;
  }
}
/* End of CLooG code */
...
kernel_2112132211_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
