...
/* Start of CLooG code */
if (PB_L >= 3) {
  for (t1=2;t1<=min(3,PB_L-1);t1++) {
    for (t2=t1;t2<=PB_L-1;t2++) {
      B[t1][t2-1] = C[t2][t1] + C[t1][1] - C[t2][t1] + C[t2][t1-2] * B[t1][t2] - 4;;
    }
  }
  for (t1=4;t1<=min(PB_L-1,PB_M-4);t1++) {
    for (t2=2;t2<=min(t1-1,-t1+PB_M-2);t2++) {
      A[t1+2][t1-2] = C[t2+t1+1][0] - D[t2-2] + A[t2+3][t2-1] + A[t1][t1-4] * 1;;
    }
    for (t2=t1;t2<=min(PB_L-1,-t1+PB_M-2);t2++) {
      A[t1+2][t1-2] = C[t2+t1+1][0] - D[t2-2] + A[t2+3][t2-1] + A[t1][t1-4] * 1;;
      B[t1][t2-1] = C[t2][t1] + C[t1][1] - C[t2][t1] + C[t2][t1-2] * B[t1][t2] - 4;;
    }
    for (t2=max(t1,-t1+PB_M-1);t2<=PB_L-1;t2++) {
      B[t1][t2-1] = C[t2][t1] + C[t1][1] - C[t2][t1] + C[t2][t1-2] * B[t1][t2] - 4;;
    }
  }
  for (t1=max(4,PB_M-3);t1<=PB_L-1;t1++) {
    for (t2=t1;t2<=PB_L-1;t2++) {
      B[t1][t2-1] = C[t2][t1] + C[t1][1] - C[t2][t1] + C[t2][t1-2] * B[t1][t2] - 4;;
    }
  }
