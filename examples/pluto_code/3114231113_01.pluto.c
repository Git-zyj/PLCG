...
void kernel_3114231113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
/* Start of CLooG code */
if (PB_L >= 4) {
  for (t1=1;t1<=PB_L-3;t1++) {
    for (t2=0;t2<=floord(t1+PB_L-3,32);t2++) {
      lbp=max(ceild(t2,2),ceild(-t1+32*t2+1,32));
      ubp=min(min(floord(PB_L-2,32),floord(-t1+32*t2+PB_L+30,64)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(32*t2-32*t3,t1+32*t3-PB_L+1);t4<=min(min(t1-1,32*t3+29),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4+2);t5<=min(32*t3+31,-t1+t4+PB_L-1);t5++) {
            A[(-t4+t5)-1][(-t4+t5)-1] = B[(-t4+t5)][t1][t4] * B[t1][(-t4+t5)][(-t4+t5)] - C[t4][t1][(-t4+t5)+t1] - A[(-t4+t5)-2][(-t4+t5)-2] + 3;;
          }
        }
      }
    }
...
kernel_3114231113_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
