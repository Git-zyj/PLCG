...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_M,32),t1);t2++) {
      for (t3=max(1,32*t2);t3<=min(min(PB_L-2,PB_M-1),32*t2+31);t3++) {
        lbv=max(32*t1,t3);
        ubv=min(PB_L-2,32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          B[t3][t4+1] = C[t4][t3-1] + B[t3-1][t4+1] + 1;;
          B[t3-1][t4+1] = 6;;
        }
      }
      if ((PB_M <= PB_L-2) && (t2 >= ceild(PB_M-31,32))) {
        lbv=max(PB_M,32*t1);
        ubv=min(PB_L-2,32*t1+31);
#pragma ivdep
#pragma vector always
