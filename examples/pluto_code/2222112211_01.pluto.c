...
void kernel_2222112211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_M >= 2) {
  if ((PB_L >= 4) && (PB_M == 2)) {
    for (t2=1;t2<=PB_L-3;t2++) {
      A[1][t2+2] = C[t2-1][1 +2] * D[t2+1] * A[1 -1][t2+2] - 1;;
    }
  }
  if ((PB_L <= 3) && (PB_M >= 3)) {
    for (t1=0;t1<=PB_M-1;t1++) {
      for (t2=1;t2<=PB_M-2;t2++) {
        B[t1] = B[t2-1] * B[t2+1] + 4;;
      }
    }
  }
  if ((PB_L >= 4) && (PB_M >= 3)) {
  ...
  if ((PB_L >= 4) && (PB_M >= 3)) {
    for (t1=1;t1<=PB_M-1;t1++) {
      for (t2=1;t2<=min(PB_L-3,PB_M-2);t2++) {
        A[t1][t2+2] = C[t2-1][t1+2] * D[t2+1] * A[t1-1][t2+2] - 1;;
        B[t1] = B[t2-1] * B[t2+1] + 4;;
      }
      for (t2=PB_M-1;t2<=PB_L-3;t2++) {
        A[t1][t2+2] = C[t2-1][t1+2] * D[t2+1] * A[t1-1][t2+2] - 1;;
      }
      for (t2=PB_L-2;t2<=PB_M-2;t2++) {
        B[t1] = B[t2-1] * B[t2+1] + 4;;
      }
    }
  }
...
kernel_2222112211_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
