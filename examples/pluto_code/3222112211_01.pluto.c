...
void kernel_3222112211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_M >= 5) {
  for (t2=0;t2<=floord(3*PB_M-7,32);t2++) {
    lbp=max(max(0,ceild(16*t2-PB_M+2,16)),ceild(32*t2-PB_M-30,64));
    ubp=min(floord(t2,2),floord(PB_M-3,32));
#pragma omp parallel for private(lbv,ubv,t4,t5,t6,t7)
    for (t3=lbp;t3<=ubp;t3++) {
      for (t4=max(ceild(32*t2-32*t3-PB_M-28,32),t3);t4<=min(floord(PB_M-1,32),t2-t3);t4++) {
        for (t5=max(max(2,32*t3),32*t2-32*t3-PB_M+1);t5<=min(min(min(PB_M-3,32*t3+31),32*t4+29),32*t2-32*t3+28);t5++) {
          for (t6=max(32*t4,t5+2);t6<=min(min(PB_M-1,32*t4+31),32*t2-32*t3+30);t6++) {
            lbv=max(32*t2-32*t3,t6+1);
            ubv=min(32*t2-32*t3+31,t5+PB_M-1);
#pragma ivdep
#pragma vector always
            for (t7=lbv;t7<=ubv;t7++) {
              B[(-t6+t7)+(-t5+t6)][(-t5+t6)][t5] = A[(-t5+t6)-2][(-t6+t7)] * A[t5-2][(-t6+t7)-1] + B[(-t5+t6)+t5][(-t5+t6)+1][(-t6+t7)] - B[(-t6+t7)+(-t5+t6)-1][(-t5+t6)-1][t5-1] + 2;;
            }
          }
        }
      }
...
kernel_3222112211_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
