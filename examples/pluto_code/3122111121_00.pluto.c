...
void kernel_3122111121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-1,32);t2++) {
      for (t3=2*t2;t3<=min(floord(2*PB_L+PB_M-5,32),floord(64*t2+PB_M+59,32));t3++) {
        for (t4=max(ceild(32*t3-PB_M+3,2),32*t2);t4<=min(min(PB_L-1,32*t2+31),16*t3+15);t4++) {
...
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[(-2*t4+t5)][(-2*t4+t5)][t6-1] = B[t6][(-2*t4+t5)+1] - A[(-2*t4+t5)+2][(-2*t4+t5)+2][t6-1] + 3;;
            }
          }
        }
      }
...
kernel_3122111121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
