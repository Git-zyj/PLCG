...
void kernel_2112131211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_L >= 2) && (PB_M >= 3)) {
  for (t1=-1;t1<=min(floord(PB_L-1,32),floord(PB_L+PB_M-34,32));t1++) {
    lbp=t1+1;
    ubp=min(floord(PB_L+PB_M-2,32),floord(32*t1+PB_M+30,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,32*t1),32*t2-PB_M+1);t3<=min(min(PB_L-1,32*t1+62),32*t2+29);t3++) {
        lbv=max(max(32*t2,t3+2),-32*t1+32*t2+t3-31);
        ubv=min(min(32*t2+31,-32*t1+32*t2+t3),t3+PB_M-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[(-t3+t4)-1][t3-1] = B[(-t3+t4)][t3-1] + B[(-t3+t4)][t3] - B[(-t3+t4)-1][t3] + A[(-t3+t4)-1][t3] - A[(-t3+t4)-2][t3] + 5;;
        }
      }
    }
  }
...
kernel_2112131211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
