...
void kernel_2112131211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if (PB_L >= 3) {
  for (t1=max(-1,ceild(-PB_L-28,32));t1<=floord(PB_L-1,32);t1++) {
    lbp=max(0,ceild(32*t1-PB_L+2,32));
    ubp=min(floord(PB_L-2,32),floord(32*t1+PB_L+28,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,32*t2),-32*t1+32*t2-30);t3<=min(min(PB_L-2,32*t2+31),-32*t1+32*t2+PB_L-1);t3++) {
        for (t4=max(1,32*t1-32*t2+t3);t4<=min(PB_L-1,32*t1-32*t2+t3+31);t4++) {
          A[t3][t4-1] = B[t3+1] - C[t4][t3-1] - C[t4-1][t4] * A[t3+1][t4] + A[t3][t4] + 1;;
        }
      }
    }
  }
...
kernel_2112131211_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
