...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,ceild(32*t2-PB_L+2,2)),32*t1-32*t2);t3<=min(min(PB_M-1,16*t2+15),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,2*t3+PB_L-2);t4++) {
          A[t3][(-2*t3+t4)] = B[(-2*t3+t4)] + A[(-2*t3+t4)][t3-1] - A[t3-1][(-2*t3+t4)+1] * 2;;
        }
      }
    }
  }
