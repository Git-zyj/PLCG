...
void kernel_3124232221_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=min(floord(PB_Q-1,32),t1-t2);t3++) {
        for (t4=max(2,32*t1-32*t2);t4<=min(PB_M-1,32*t1-32*t2+31);t4++) {
          for (t5=32*t2;t5<=min(PB_M-1,32*t2+31);t5++) {
            lbv=max(2,32*t3);
            ubv=min(min(t4,PB_Q-1),32*t3+31);
#pragma ivdep
#pragma vector always
            for (t6=lbv;t6<=ubv;t6++) {
              A[t4][t5+1][t6] = B[t5][t4] + C[t5][t6][t4] - D[t4][t5][t6] + A[t4][t5][t6-2] * A[t4-1][t5+1][t6] + 6;;
            }
          }
        }
      }
...
kernel_3124232221_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
