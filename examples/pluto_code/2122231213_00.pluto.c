...
void kernel_2122231213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
/* Start of CLooG code */
if (PB_M >= 3) {
  for (t1=-1;t1<=min(floord(PB_M-17,16),floord(PB_M-1,32));t1++) {
    lbp=t1+1;
    ubp=min(floord(PB_M-1,16),floord(32*t1+PB_M+30,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t1),32*t2-PB_M+1);t3<=min(PB_M-1,32*t1+62);t3++) {
        lbv=max(32*t2,-32*t1+32*t2+t3-31);
        ubv=min(min(32*t2+31,-32*t1+32*t2+t3),t3+PB_M-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3-1][(-t3+t4)] = B[(-t3+t4)][t3] - C[(-t3+t4)-1][t3+1] - D[(-t3+t4)] - A[t3-2][(-t3+t4)+1] + A[t3][(-t3+t4)] + 5;;
        }
      }
    }
  }
...
kernel_2122231213_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
