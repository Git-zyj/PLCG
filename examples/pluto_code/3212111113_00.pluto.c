...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=floord(PB_L,32);t3++) {
        for (t4=max(1,32*t3);t4<=(min(PB_L,32*t3+31))-7;t4+=8) {
          lbv=max(1,32*t2);
...
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            A[t5-1] = D[t4][t4-1] + 5;;
            A[t5-1] = D[(t4+1)][(t4+1)-1] + 5;;
            A[t5-1] = D[(t4+2)][(t4+2)-1] + 5;;
            A[t5-1] = D[(t4+3)][(t4+3)-1] + 5;;
            A[t5-1] = D[(t4+4)][(t4+4)-1] + 5;;
            A[t5-1] = D[(t4+5)][(t4+5)-1] + 5;;
            A[t5-1] = D[(t4+6)][(t4+6)-1] + 5;;
            A[t5-1] = D[(t4+7)][(t4+7)-1] + 5;;
          }
        }
        for (;t4<=min(PB_L,32*t3+31);t4++) {
          lbv=max(1,32*t2);
          ubv=min(PB_M,32*t2+31);
#pragma ivdep
#pragma vector always
