...
void kernel_3212111123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma vector always
        ...
        if ((PB_L == 0) && (PB_P >= 1) && (t2 == 0) && (t3 == 0)) {
          for (t4=max(3,32*t1);t4<=min(PB_M-1,32*t1+31);t4++) {
            A[t4-1] = C[t4] + A[t4-3] + A[t4] - 5;;
          }
        }
        if ((t2 <= floord(PB_P-1,32)) && (t2 >= -t3+1) && (t3 <= floord(PB_L-1,32))) {
          lbp=max(3,32*t1);
...
kernel_3212111123_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
