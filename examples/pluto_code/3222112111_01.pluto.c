...
void kernel_3222112111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_M >= 1) {
  if ((PB_L >= 0) && (PB_M >= ceild(-2*PB_L+9,3))) {
    for (t1=0;t1<=min(floord(PB_M+PB_L-2,32),floord(2*PB_M+PB_L-5,32));t1++) {
      if ((PB_M >= 3) && (t1 <= floord(PB_M-2,32))) {
        for (t4=max(1,32*t1);t4<=min(PB_M-2,32*t1+31);t4++) {
          A[t4] = C[t4] - C[t4+1] * A[t4-1] - 3;;
        }
      }
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
        for (t2=lbp;t2<=ubp;t2++) {
          for (t3=max(1,32*t2);t3<=min(PB_M,32*t2+31);t3++) {
...
kernel_3222112111_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
