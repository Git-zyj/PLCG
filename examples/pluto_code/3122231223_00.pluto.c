...
void kernel_3122231223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if ((PB_L >= 3) && (PB_M >= 3)) {
  for (t1=0;t1<=floord(2*PB_L+PB_M-3,32);t1++) {
    lbp=max(max(0,ceild(32*t1-PB_L-PB_M+2,32)),ceild(32*t1-PB_M-30,64));
    ubp=min(floord(t1,2),floord(PB_L-1,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t2),32*t1-32*t2-PB_M+1);t3<=min(min(PB_L-1,32*t2+31),32*t1-32*t2+29);t3++) {
        for (t4=max(32*t1-32*t2,t3+2);t4<=min(32*t1-32*t2+31,t3+PB_M-1);t4++) {
          A[t3][(-t3+t4)-1] = B[(-t3+t4)] + C[(-t3+t4)][t3] + C[(-t3+t4)+1][t3] - A[t3][(-t3+t4)-2] + A[t3-2][(-t3+t4)+1] + 5;;
        }
      }
    }
  }
...
kernel_3122231223_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
