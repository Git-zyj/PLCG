...
/* Start of CLooG code */
if ((PB_L >= max(0,-2*PB_M+6)) && (PB_M >= 0)) {
  for (t1=0;t1<=min(floord(PB_L+PB_M-1,32),floord(3*PB_L+5*PB_M-9,96));t1++) {
    for (t2=0;t2<=min(min(floord(PB_L,32),floord(3*PB_L+4*PB_M-12,96)),t1);t2++) {
      if (t2 >= 1) {
        for (t3=max(32*t1,32*t2+1);t3<=min(PB_L-3,32*t1+31);t3++) {
          for (t4=32*t2;t4<=min(32*t2+31,t3-1);t4++) {
            A[t3][t4-1] = C[t4][t3+2] * A[t3][t4-2] * A[t3-1][t4-1] + 4;;
          }
        }
      }
      if ((PB_M >= 3) && (t1 == 0) && (t2 == 0)) {
        B[2] = D[2] * B[2 -2] - B[2 -1] - 3;;
      }
      if (t2 == 0) {
        for (t3=max(3,32*t1);t3<=min(min(PB_L-3,PB_M-1),32*t1+31);t3++) {
          B[t3] = D[t3] * B[t3-2] - B[t3-1] - 3;;
      ...
      if (t2 == 0) {
        for (t3=max(max(3,PB_M),32*t1);t3<=min(PB_L-3,32*t1+31);t3++) {
          for (t4=2;t4<=min(31,t3-1);t4++) {
            A[t3][t4-1] = C[t4][t3+2] * A[t3][t4-2] * A[t3-1][t4-1] + 4;;
          }
        }
      ...
      if (t2 == 0) {
        for (t3=max(max(3,32*t1),PB_L-2);t3<=min(PB_M-1,32*t1+31);t3++) {
          B[t3] = D[t3] * B[t3-2] - B[t3-1] - 3;;
        }
      }
    }
...
if ((PB_L >= 6) && (PB_M <= -1)) {
  for (t1=0;t1<=floord(PB_L-3,32);t1++) {
    for (t2=0;t2<=min(floord(PB_L-4,32),t1);t2++) {
      for (t3=max(max(3,32*t1),32*t2+1);t3<=min(PB_L-3,32*t1+31);t3++) {
