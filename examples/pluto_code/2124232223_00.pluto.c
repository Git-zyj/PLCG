...
void kernel_2124232223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
/* Start of CLooG code */
if (PB_M >= 4) {
  for (t1=0;t1<=floord(3*PB_M-8,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_M+2,32));
    ubp=min(min(floord(PB_M-3,16),floord(32*t1+PB_M+28,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_M+3);t3<=min(min(PB_M-2,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(32*t2+31,t3+PB_M-3);t4++) {
          A[(-t3+t4)+1][(-t3+t4)] = B[(-t3+t4)+2][t3] * C[t3][(-t3+t4)] + D[t3+1] + A[(-t3+t4)+2][(-t3+t4)+1] * A[(-t3+t4)][(-t3+t4)-1] - 5;;
        }
      }
    }
  }
...
kernel_2124232223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
