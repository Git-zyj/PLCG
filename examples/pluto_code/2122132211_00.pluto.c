...
void kernel_2122132211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_M >= 4) {
  for (t1=0;t1<=PB_M-4;t1++) {
    A[t1+2][t1+1] = B[t1] - B[t1] + C[t1][t1] - A[t1+3][t1+2] * A[t1+1][t1] + 1;;
  }
}
/* End of CLooG code */
...
kernel_2122132211_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
