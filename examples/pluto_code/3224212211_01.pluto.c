...
void kernel_3224212211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma vector always
      ...
      if (t2 == 0) {
        for (t3=max(3,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
          B[t3][0] = C[t3] * B[t3+1][0] + 1;;
          A[t3-2] = A[t3-3] * A[t3-1] - A[t3-3] - 6;;
          lbv=1;
          ubv=min(31,t3-1);
#pragma ivdep
#pragma vector always
      ...
      if (t2 == 0) {
        for (t3=max(max(3,32*t1),PB_M-1);t3<=min(PB_M,32*t1+31);t3++) {
          A[t3-2] = A[t3-3] * A[t3-1] - A[t3-3] - 6;;
        }
      }
    }
...
kernel_3224212211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
