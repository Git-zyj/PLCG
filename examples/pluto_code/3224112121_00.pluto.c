...
void kernel_3224112121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_L >= 2) {
  for (t1=0;t1<=floord(PB_L,8);t1++) {
    ...
    ubp=min(floord(3*t1+2,4),floord(3*PB_L+1,32));
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=min(min(min(min(floord(3*t1-4*t2+2,2),floord(PB_L,32)),floord(-32*t2+3*PB_L+1,64)),floord(-32*t2+4*PB_L-1,96)),t1-t2);t3++) {
        if ((t1 == 0) && (t2 == 0) && (t3 == 0)) {
          for (t4=2;t4<=min(15,PB_L+1);t4++) {
            for (t5=1;t5<=min(PB_L-1,t4-1);t5++) {
              lbv=0;
              ubv=t4-1;
#pragma ivdep
#pragma vector always
              for (t6=lbv;t6<=ubv;t6++) {
                A[t4][t5][t6] = C[t6][t5] - 3;;
              }
            }
            for (t5=2*t4+1;t5<=min(min(31,3*t4-1),2*t4+PB_L-1);t5++) {
              B[(-2*t4+t5)] = C[(-2*t4+t5)][t4-2] - B[(-2*t4+t5)+2] - B[(-2*t4+t5)-1] * 3;;
            }
          }
        }
        if ((t2 >= ceild(PB_L,32)) && (t3 == 0)) {
          for (t4=max(ceild(32*t2+1,3),32*t1-32*t2);t4<=min(min(PB_L+1,16*t2+15),32*t1-32*t2+31);t4++) {
            for (t5=max(32*t2,2*t4+1);t5<=min(min(32*t2+31,3*t4-1),2*t4+PB_L-1);t5++) {
              B[(-2*t4+t5)] = C[(-2*t4+t5)][t4-2] - B[(-2*t4+t5)+2] - B[(-2*t4+t5)-1] * 3;;
            }
          }
        }
        ...
        if (t2 <= floord(PB_L-1,32)) {
          for (t4=max(max(max(32*t1-32*t2,16*t2+16),32*t2+1),32*t3+1);t4<=min(PB_L+1,32*t1-32*t2+31);t4++) {
...
kernel_3224112121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
