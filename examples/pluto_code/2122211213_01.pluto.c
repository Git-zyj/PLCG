...
void kernel_2122211213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if (PB_L >= 2) {
  for (t1=1;t1<=PB_L;t1++) {
    for (t2=0;t2<=PB_L-2;t2++) {
      A[t2][t2] = B[t1][t2+1] * A[t1-1][t1-1] * A[t2+1][t2+1] + 1;;
    }
  }
}
/* End of CLooG code */
...
kernel_2122211213_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
