...
void kernel_2122111223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if ((PB_M >= 3) && (PB_N >= 4)) {
  lbp=0;
  ubp=min(floord(PB_M-1,32),floord(PB_N-4,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(PB_M-1,32),floord(-32*t1+PB_N-2,32));t2++) {
      for (t3=32*t1;t3<=min(min(min(PB_M-1,PB_N-4),32*t1+31),-32*t2+PB_N-2);t3++) {
        for (t4=max(2,32*t2);t4<=min(min(PB_M-1,32*t2+31),-t3+PB_N-2);t4++) {
          A[t3+t4][t4-1] = B[1] - A[t3+t4-1][t4-2] - A[t3+t4+1][t4] * 2;;
        }
      }
    }
  }
...
kernel_2122111223_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
