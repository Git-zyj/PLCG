...
void kernel_2212231211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
if (PB_P >= 2) {
  for (t1=0;t1<=floord(3*PB_P-3,32);t1++) {
    ...
    ubp=min(min(floord(3*t1+1,3),floord(PB_P-1,16)),floord(32*t1+PB_P+30,64));
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      if ((PB_P == 2) && (t1 == 0) && (t2 == 0)) {
        A[1] = C[1 +1] + D[1 +1] + C[1 +1] * 1;;
      }
      for (t3=max(max(1,32*t1-32*t2),32*t2-PB_P+1);t3<=min(16*t2-1,32*t1-32*t2+31);t3++) {
        for (t4=32*t2;t4<=min(32*t2+31,t3+PB_P-1);t4++) {
          B[t3-1][(-t3+t4)-1] = E[t3][(-t3+t4)] * E[(-t3+t4)+1][t3] - A[(-t3+t4)] - B[t3][(-t3+t4)-2] + B[t3-1][(-t3+t4)-2] - 3;;
        }
      }
      if (PB_P >= 3) {
        for (t3=max(max(1,16*t2),32*t1-32*t2);t3<=min(min(PB_P-1,16*t2+15),32*t1-32*t2+31);t3++) {
          if ((t1 == 0) && (t2 == 0) && (t3 == 1)) {
            A[1] = C[1 +1] + D[1 +1] + C[1 +1] * 1;;
          }
          ...
          if (t3 >= 2) {
            A[t3] = C[t3+1] + D[t3+1] + C[t3+1] * 1;;
            B[t3-1][t3-1] = E[t3][t3] * E[t3+1][t3] - A[t3] - B[t3][t3-2] + B[t3-1][t3-2] - 3;;
          }
          for (t4=2*t3+1;t4<=min(32*t2+31,t3+PB_P-1);t4++) {
            B[t3-1][(-t3+t4)-1] = E[t3][(-t3+t4)] * E[(-t3+t4)+1][t3] - A[(-t3+t4)] - B[t3][(-t3+t4)-2] + B[t3-1][(-t3+t4)-2] - 3;;
          }
        }
      }
      for (t3=max(32*t1-32*t2,16*t2+16);t3<=min(min(PB_P-1,32*t2+29),32*t1-32*t2+31);t3++) {
...
kernel_2212231211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
