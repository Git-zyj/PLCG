...
void kernel_2224112113_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=min(floord(PB_P-1,32),t2);t3++) {
        for (t4=max(2,32*t2);t4<=min(PB_M-1,32*t2+31);t4++) {
          for (t5=32*t3;t5<=min(min(t4,PB_P-1),32*t3+31);t5++) {
            B[t4-1] = C[t4-2] * D[t4][t5] * 5;;
          }
        }
      }
    }
...
kernel_2224112113_01(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
