...
void kernel_2224212213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma vector always
  ...
  for (t2=-1;t2<=floord(PB_M-2,16);t2++) {
    lbp=max(0,t2);
    ubp=min(floord(16*t2+PB_M+15,32),floord(32*t2+PB_M+29,32));
#pragma omp parallel for private(lbv,ubv,t4,t5)
    for (t3=lbp;t3<=ubp;t3++) {
      for (t4=max(max(2,16*t2),32*t3-PB_M);t4<=min(min(min(PB_M-2,16*t2+31),16*t3+15),32*t2-32*t3+PB_M+31);t4++) {
        lbv=max(max(32*t3,2*t4),-32*t2+32*t3+2*t4-31);
        ubv=min(min(32*t3+31,t4+PB_M),-32*t2+32*t3+2*t4);
#pragma ivdep
#pragma vector always
        for (t5=lbv;t5<=ubv;t5++) {
          B[t4][(-t4+t5)-1] = C[(-t4+t5)-1][t4] + A[t4-2] - B[t4+1][(-t4+t5)] - B[t4-1][(-t4+t5)] + 4;;
        }
      }
    }
  }
...
kernel_2224212213_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
