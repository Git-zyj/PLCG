...
void kernel_3124132113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_P >= 3) {
  for (t1=2;t1<=PB_P-1;t1++) {
    A[t1-2][2] = B[t1] + B[t1] + B[t1] - A[t1-1][2] - 3;;
  }
}
/* End of CLooG code */
...
kernel_3124132113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
