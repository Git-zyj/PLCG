...
if (PB_L >= 1) {
  for (t1=0;t1<=floord(PB_L-1,32);t1++) {
    ...
    if (PB_N >= 0) {
      for (t2=0;t2<=min(min(floord(64*t1+31*PB_N,32),floord(64*t1+PB_N+60,32)),floord(64*t1+60*PB_L+31*PB_N-60,992));t2++) {
        if ((PB_N >= 2) && (t2 >= 1)) {
          for (t3=max(max(2,ceild(32*t2-PB_N+2,2)),32*t1);t3<=min(min(PB_L-1,32*t1+31),16*t2+15);t3++) {
            for (t4=max(32*t2,2*t3);t4<=min(32*t2+31,2*t3+PB_N-2);t4++) {
              A[(-2*t3+t4)][t3] = C[(-2*t3+t4)][t3] - C[t3-2][(-2*t3+t4)] * D[(-2*t3+t4)][t3] + C[(-2*t3+t4)+1][t3+2] + A[(-2*t3+t4)][t3+1] + A[t3-1][(-2*t3+t4)] - 5;;
            }
          }
        }
        if ((PB_N <= 1) && (t2 == 0)) {
        ...
        if ((PB_N >= 2) && (t1 == 0) && (t2 == 0)) {
          for (t3=2;t3<=min(15,PB_L-1);t3++) {
            B[t3] = E[1] * F[t3] + 2;;
            for (t4=2*t3;t4<=min(31,2*t3+PB_N-2);t4++) {
              A[(-2*t3+t4)][t3] = C[(-2*t3+t4)][t3] - C[t3-2][(-2*t3+t4)] * D[(-2*t3+t4)][t3] + C[(-2*t3+t4)+1][t3+2] + A[(-2*t3+t4)][t3+1] + A[t3-1][(-2*t3+t4)] - 5;;
            }
          }
        }
