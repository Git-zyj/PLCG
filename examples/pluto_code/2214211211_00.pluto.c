...
void kernel_2214211211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if (PB_N >= 2) {
  for (t1=1;t1<=min(PB_L-1,PB_N-2);t1++) {
    if (t1 == 1) {
      A[1] = C[1][1 -1] + A[1 +1] - A[1 -1] - 4;;
    }
    for (t2=2;t2<=t1-1;t2++) {
      B[t1+1][t1-1] = C[t2-1][t1] * B[t1+2][t1] + B[t2][t2-2] * 4;;
    }
    for (t2=max(2,t1);t2<=PB_N-2;t2++) {
      A[t2] = C[t2][t1-1] + A[t2+1] - A[t2-1] - 4;;
      B[t1+1][t1-1] = C[t2-1][t1] * B[t1+2][t1] + B[t2][t2-2] * 4;;
    }
    for (t2=PB_N-1;t2<=PB_N;t2++) {
      B[t1+1][t1-1] = C[t2-1][t1] * B[t1+2][t1] + B[t2][t2-2] * 4;;
    }
  }
  for (t1=max(1,PB_L);t1<=PB_N-2;t1++) {
    for (t2=t1;t2<=PB_N-2;t2++) {
      A[t2] = C[t2][t1-1] + A[t2+1] - A[t2-1] - 4;;
    }
  }
  for (t1=PB_N-1;t1<=PB_L-1;t1++) {
    for (t2=2;t2<=PB_N;t2++) {
      B[t1+1][t1-1] = C[t2-1][t1] * B[t1+2][t1] + B[t2][t2-2] * 4;;
    }
  }
...
kernel_2214211211_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
