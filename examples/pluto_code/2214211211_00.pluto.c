...
void kernel_2214211211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_M >= 2) {
  for (t1=1;t1<=PB_M-2;t1++) {
    for (t2=0;t2<=t1-1;t2++) {
      A[t1][1] = C[t2+1][t1+1] - A[t1-1][1] + 4;;
    }
    for (t2=t1;t2<=PB_M-1;t2++) {
      B[t2][t1] = D[t2-1][t1] * B[t1+1][t2-1] - B[t2-1][t1] - B[t2-1][t1-1] + 1;;
    }
  }
  for (t2=0;t2<=PB_M-2;t2++) {
    A[(PB_M-1)][1] = C[t2+1][(PB_M-1)+1] - A[(PB_M-1)-1][1] + 4;;
  }
}
/* End of CLooG code */
...
kernel_2214211211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
