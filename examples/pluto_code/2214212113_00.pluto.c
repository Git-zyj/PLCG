...
void kernel_2214212113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_L >= 1) {
  for (t1=0;t1<=floord(PB_L,32);t1++) {
    if (t1 <= floord(PB_L-4,32)) {
      for (t4=32*t1;t4<=min(PB_L-4,32*t1+31);t4++) {
        A[t4+2] = A[t4+3] + 2;;
      }
    }
    if (t1 <= floord(PB_P-3,32)) {
      lbp=t1;
      ubp=floord(PB_L,32);
#pragma omp parallel for private(lbv,ubv,t3,t4)
      for (t2=lbp;t2<=ubp;t2++) {
        for (t3=max(1,32*t2);t3<=min(PB_L,32*t2+31);t3++) {
          for (t4=32*t1;t4<=min(min(t3,PB_P-3),32*t1+31);t4++) {
            B[t3-1][t4+2] = C[t3+1][t4] - 3;;
          }
        }
      }
    }
...
kernel_2214212113_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
