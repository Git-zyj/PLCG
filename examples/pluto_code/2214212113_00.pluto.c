...
void kernel_2214212113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
for (t1=1;t1<=min(PB_L-1,PB_N-2);t1++) {
  A[t1] = 6;;
  B[t1] = C[t1] * C[t1] - B[t1-1] - B[t1+1] * 5;;
}
for (t1=max(1,PB_N-1);t1<=PB_L-1;t1++) {
  A[t1] = 6;;
}
...
kernel_2214212113_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
