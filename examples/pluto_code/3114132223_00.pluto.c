...
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(2,ceild(32*t3-PB_N+1,2)),32*t2-32*t3);t4<=min(min(floord(32*t3+31,3),PB_N-2),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,3*t4);t5<=min(32*t3+31,2*t4+PB_N-1);t5++) {
            A[(-2*t4+t5)][t4] = B[(-2*t4+t5)][t4][t1] + B[t4-2][(-2*t4+t5)][t1+2] - B[t4+1][t1][t4+1] - A[t4][(-2*t4+t5)+1] + A[(-2*t4+t5)+2][t1-1] - 2;;
          }
        }
      }
    }
