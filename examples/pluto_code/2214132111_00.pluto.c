...
void kernel_2214132111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      ...
      for (t3=max(32*t1-32*t2,32*t2-PB_M+1);t3<=min(16*t2-1,32*t1-32*t2+31);t3++) {
        for (t4=32*t2;t4<=min(32*t2+31,t3+PB_M-1);t4++) {
          A[(-t3+t4)] = D[(-t3+t4)][t3] * D[(-t3+t4)-1][t3] * A[(-t3+t4)-1] + 2;;
        }
      }
      if (PB_M >= 2) {
        for (t3=max(16*t2,32*t1-32*t2);t3<=min(min(PB_M-1,16*t2+15),32*t1-32*t2+31);t3++) {
          if ((t1 == 0) && (t2 == 0) && (t3 == 0)) {
            A[0] = C[0] * C[0] + C[0] - 6;;
          }
          ...
          if (t3 >= 1) {
            A[t3] = C[t3] * C[t3] + C[t3] - 6;;
            A[t3] = D[t3][t3] * D[t3-1][t3] * A[t3-1] + 2;;
          }
          for (t4=2*t3+1;t4<=min(32*t2+31,t3+PB_M-1);t4++) {
            A[(-t3+t4)] = D[(-t3+t4)][t3] * D[(-t3+t4)-1][t3] * A[(-t3+t4)-1] + 2;;
          }
        }
      }
...
kernel_2214132111_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
