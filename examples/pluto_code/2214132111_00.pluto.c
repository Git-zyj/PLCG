...
void kernel_2214132111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-2,32);t2++) {
      if (t1 == 0) {
        for (t4=max(2,32*t2);t4<=min(PB_L-2,32*t2+31);t4++) {
          A[2][t4-2] = C[2][t4+1] - C[t4][2 -2] - C[2][2] + C[2 +1][t4-1] + C[t4][2] * 3;;
        }
      }
      for (t3=max(3,32*t1);t3<=min(PB_P-3,32*t1+31);t3++) {
        for (t4=max(2,32*t2);t4<=min(PB_L-2,32*t2+31);t4++) {
          A[t3][t4-2] = C[t3][t4+1] - C[t4][t3-2] - C[t3][2] + C[t3+1][t4-1] + C[t4][t3] * 3;;
          B[t4+1][(t3-1)] = C[t4-2][(t3-1)+2] - A[(t3-1)+1][t4-1] - B[t4][(t3-1)] - 3;;
        }
      }
      if (t1 >= ceild(PB_P-33,32)) {
        for (t4=max(2,32*t2);t4<=min(PB_L-2,32*t2+31);t4++) {
          B[t4+1][(PB_P-3)] = C[t4-2][(PB_P-3)+2] - A[(PB_P-3)+1][t4-1] - B[t4][(PB_P-3)] - 3;;
        }
      }
    }
  }
...
kernel_2214132111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
