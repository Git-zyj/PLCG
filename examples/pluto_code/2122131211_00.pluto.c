...
void kernel_2122131211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_N >= 3) {
  for (t1=1;t1<=PB_N-2;t1++) {
    A[t1] = B[t1+1] - B[t1-1] - B[t1+1] - A[t1+1] * A[t1-1] + 2;;
  }
}
/* End of CLooG code */
...
kernel_2122131211_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
