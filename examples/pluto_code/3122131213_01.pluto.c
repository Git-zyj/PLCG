...
void kernel_3122131213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if ((PB_M >= 2) && (PB_N >= 3)) {
  for (t1=0;t1<=floord(PB_N+PB_M-4,32);t1++) {
    lbp=max(0,ceild(32*t1-PB_N+3,32));
    ubp=min(floord(PB_M-1,32),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t1-32*t2;t3<=min(PB_N-3,32*t1-32*t2+31);t3++) {
        for (t4=max(1,32*t2);t4<=min(PB_M-1,32*t2+31);t4++) {
          A[t3+1][t4] = B[t4] * C[t4][t3] * C[t4-1][t3] * A[t3+2][t4] * A[t3+1][t4-1] + 3;;
        }
      }
    }
  }
...
kernel_3122131213_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
