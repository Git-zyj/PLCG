...
void kernel_3212132113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_L >= 5) {
  for (t1=0;t1<=floord(PB_L-2,32);t1++) {
    for (t2=0;t2<=floord(PB_L-1,32);t2++) {
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=3;t3<=min(31,PB_L-2);t3++) {
          B[t3-2] = C[t3+1] + C[t3] + C[t3+1] + B[t3-3] - 4;;
          lbv=t3;
          ubv=min(31,PB_L-1);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3+1][t4] = C[t3-1] - D[t4+1][t3+1] - D[t4][t3] * A[t3+2][t4] * 5;;
          }
        }
      }
      if (t2 >= 1) {
        lbp=max(3,32*t1);
        ubp=min(min(PB_L-2,32*t1+31),32*t2+31);
...
kernel_3212132113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
