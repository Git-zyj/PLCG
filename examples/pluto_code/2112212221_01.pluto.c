...
void kernel_2112212221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_M >= 6) && (PB_N >= 4)) {
  lbp=0;
  ubp=min(floord(PB_N,32),floord(PB_M-3,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(min(min(floord(PB_N-2,32),floord(-32*t1+PB_M-1,32)),floord(PB_M-2,64)),t1);t2++) {
      for (t3=max(2,32*t2);t3<=min(min(min(min(floord(PB_M-2,2),PB_N-2),32*t1+30),32*t2+31),-32*t1+PB_M-1);t3++) {
        lbv=max(32*t1,t3+1);
        ubv=min(min(PB_N,32*t1+31),-t3+PB_M-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3-1][t4] = B[t3-2][t3+t4] * A[t4-1][t3-2] + A[t3+1][t4] - 1;;
        }
      }
    }
  }
...
kernel_2112212221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
