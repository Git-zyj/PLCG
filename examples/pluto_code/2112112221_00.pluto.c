...
void kernel_2112112221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
/* Start of CLooG code */
if ((PB_L >= 4) && (PB_M >= 5)) {
  for (t1=0;t1<=floord(2*PB_M+PB_L-9,32);t1++) {
    lbp=max(max(0,ceild(32*t1-PB_M-PB_L+6,32)),ceild(32*t1-PB_L-28,64));
    ubp=min(floord(t1,2),floord(PB_M-3,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t2),32*t1-32*t2-PB_L+3);t3<=min(min(PB_M-3,32*t2+31),32*t1-32*t2+30);t3++) {
        lbv=max(32*t1-32*t2,t3+1);
        ubv=min(32*t1-32*t2+31,t3+PB_L-3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-t3+t4)] = B[t3][(-t3+t4)] * A[t3-2][(-t3+t4)+2] + A[t3+2][(-t3+t4)-1] * 5;;
        }
      }
    }
  }
...
kernel_2112112221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
