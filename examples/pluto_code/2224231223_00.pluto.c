...
if (PB_L >= 2) {
  if (PB_N >= 4) {
    lbp=0;
    ubp=min(floord(PB_L-1,32),floord(PB_N-3,32));
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=min(min(floord(PB_L-1,32),floord(PB_N-3,32)),floord(-32*t2+PB_N-2,32));t3++) {
        for (t4=max(1,32*t2);t4<=min(min(min(PB_L-1,PB_N-3),32*t2+31),-32*t3+PB_N-2);t4++) {
          for (t5=max(1,32*t3);t5<=min(min(PB_L-1,32*t3+31),-t4+PB_N-2);t5++) {
            A[t5+t4][t5-1] = C[t4][0] + D[t4+1][t5] * C[t5][t4] * A[t5+t4+1][t5] - B[t4+1][t4] + 4;;
          }
        }
      }
    }
  ...
  for (t2=1;t2<=PB_L-1;t2++) {
    B[t2+1][t2] = E[t2-1] - E[t2-1] - B[t2][t2-1] - 1;;
  }
}
/* End of CLooG code */
