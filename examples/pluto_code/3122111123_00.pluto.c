...
void kernel_3122111123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(ceild(t1-2,3),ceild(32*t1-PB_M-31,32));t2<=min(floord(t1,2),floord(PB_M-2,32));t2++) {
      for (t3=max(max(max(3,ceild(32*t1+1,3)),32*t2),32*t1-PB_M);t3<=min(min(PB_M-2,16*t1+14),32*t2+31);t3++) {
        lbv=max(32*t1,2*t3+2);
        ubv=min(min(32*t1+31,t3+PB_M),3*t3-1);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-2*t3+t4)] = B[(-2*t3+t4)+t3-1] - A[t3+1][(-2*t3+t4)-2] - 1;;
        }
      }
    }
  }
...
kernel_3122111123_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
