...
void kernel_3112231123_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=max(0,ceild(32*t1-PB_M-29,32));t2<=floord(t1,2);t2++) {
      for (t3=max(32*t2,32*t1-PB_M+2);t3<=min(min(PB_M-2,16*t1+15),32*t2+31);t3++) {
        lbv=max(max(32*t1,2*t3),t3+1);
        ubv=min(32*t1+31,t3+PB_M-2);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-t3+t4)] = B[(-t3+t4)][(-t3+t4)-1] - B[(-t3+t4)][(-t3+t4)] * C[(-t3+t4)+1][t3+1] * A[t3+1][(-t3+t4)-1] * 6;;
        }
      }
    }
  }
...
kernel_3112231123_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
