...
if ((PB_L >= max(0,ceild(-PB_N+4,2))) && (PB_N >= 0)) {
  for (t1=0;t1<=min(floord(PB_L+PB_N+1,32),floord(2*PB_L+3*PB_N-2,64));t1++) {
    for (t2=max(max(0,ceild(32*t1-PB_L,32)),ceild(128*t1-4*PB_L-PB_N+4,128));t2<=min(floord(PB_N,16),floord(32*t1+PB_N+30,32));t2++) {
      if ((PB_N <= 3) && (t2 == 0)) {
        for (t3=max(1,32*t1);t3<=min(PB_L-1,32*t1+31);t3++) {
          A[t3] = C[t3] + A[t3-1] * A[t3+1] * A[t3-1] + 1;;
        }
      }
      if ((PB_L >= 2) && (PB_N >= 4) && (t1 == 0) && (t2 == 0)) {
        A[1] = C[1] + A[1 -1] * A[1 +1] * A[1 -1] + 1;;
      }
      if (t2 >= 1) {
        for (t3=max(max(2,32*t1),32*t2-PB_N+1);t3<=min(min(PB_N+1,32*t1+31),32*t2+28);t3++) {
          lbv=max(32*t2,t3+3);
          ubv=min(32*t2+31,t3+PB_N-1);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[(-t3+t4)-2][t3-1] = D[t3][t3] * D[(-t3+t4)][t3-2] * E[t3-2][(-t3+t4)] - E[(-t3+t4)][t3] - B[(-t3+t4)-3][t3] - 5;;
          }
        }
      }
      if ((PB_N >= 4) && (t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(min(28,PB_L-1),PB_N+1);t3++) {
          A[t3] = C[t3] + A[t3-1] * A[t3+1] * A[t3-1] + 1;;
          lbv=t3+3;
          ubv=min(31,t3+PB_N-1);
#pragma ivdep
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[(-t3+t4)-2][t3-1] = D[t3][t3] * D[(-t3+t4)][t3-2] * E[t3-2][(-t3+t4)] - E[(-t3+t4)][t3] - B[(-t3+t4)-3][t3] - 5;;
      ...
      if ((PB_N >= 4) && (t1 == 0) && (t2 == 0)) {
        for (t3=PB_N+2;t3<=min(28,PB_L-1);t3++) {
          A[t3] = C[t3] + A[t3-1] * A[t3+1] * A[t3-1] + 1;;
        }
      }
      if ((PB_N >= 4) && (t1 == 0) && (t2 == 0)) {
        for (t3=max(2,PB_L);t3<=min(28,PB_N+1);t3++) {
          lbv=t3+3;
          ubv=min(31,t3+PB_N-1);
#pragma ivdep
#pragma vector always
      ...
      if ((PB_N >= 4) && (t2 == 0)) {
        for (t3=max(29,32*t1);t3<=min(PB_L-1,32*t1+31);t3++) {
          A[t3] = C[t3] + A[t3-1] * A[t3+1] * A[t3-1] + 1;;
        }
      }
    }
...
if ((PB_L <= -1) && (PB_N >= 4)) {
  for (t1=0;t1<=floord(PB_N+1,32);t1++) {
    lbp=t1;
    ubp=min(floord(PB_N,16),floord(32*t1+PB_N+30,32));
