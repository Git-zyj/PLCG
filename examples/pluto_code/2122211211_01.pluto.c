...
void kernel_2122211211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
/* Start of CLooG code */
if (PB_P >= 3) {
  for (t1=max(-1,ceild(-PB_P-59,64));t1<=floord(PB_P-2,32);t1++) {
    lbp=max(0,t1);
    ubp=min(min(floord(PB_P-2,32),floord(64*t1+PB_P+59,64)),2*t1+2);
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(32*t1,16*t2+1),-32*t1+32*t2-30);t3<=min(min(min(PB_P-2,32*t1+62),32*t2+31),32*t1-32*t2+PB_P+29);t3++) {
        lbv=max(max(32*t2,t3),-32*t1+32*t2+t3-31);
        ubv=min(min(min(PB_P-2,32*t2+31),2*t3-1),-32*t1+32*t2+t3);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][(-t3+t4)+t3] = B[t3+1] * A[t3+1][(-t3+t4)+t3+1] - A[t3-1][(-t3+t4)+t3] - 5;;
        }
      }
    }
  }
...
kernel_2122211211_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
