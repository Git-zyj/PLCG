...
void kernel_3224231113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            A[t3][t4] = C[t4+1][t3] * C[t3][t4] + D[t3][t4] - A[t3+1][t4] - 6;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
...
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[0][t4] = C[t4+1][0] * C[0][t4] + D[0][t4] - A[0 +1][t4] - 6;;
        }
      }
      if (t2 == 0) {
        for (t3=max(1,32*t1);t3<=min(PB_M-2,32*t1+31);t3++) {
          A[t3][0] = C[0 +1][t3] * C[t3][0] + D[t3][0] - A[t3+1][0] - 6;;
          B[t3] = E[t3] - D[t3+1][1] + F[t3] - B[t3-1] + 2;;
          lbv=1;
          ubv=min(31,PB_M-2);
#pragma ivdep
#pragma vector always
...
kernel_3224231113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
