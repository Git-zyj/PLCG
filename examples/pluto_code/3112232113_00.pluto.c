...
void kernel_3112232113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
if (PB_L >= 4) {
  for (t1=2;t1<=PB_L-2;t1++) {
    A[t1][t1] = B[1] - C[t1-2] - C[t1] - A[t1+1][t1+1] * 4;;
  }
}
/* End of CLooG code */
...
kernel_3112232113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
