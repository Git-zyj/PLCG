...
void kernel_3112232113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_L+1);t3<=min(min(PB_L-1,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(32*t2+31,t3+PB_L-1);t4++) {
          A[(-t3+t4)][(-t3+t4)] = B[(-t3+t4)+1][0] * B[t3][t3] - B[(-t3+t4)][(-t3+t4)-1] + A[(-t3+t4)-1][(-t3+t4)-1] * 4;;
        }
      }
    }
  }
...
kernel_3112232113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
