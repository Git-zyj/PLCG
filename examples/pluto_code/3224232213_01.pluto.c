...
void kernel_3224232213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=t2;t3<=min(floord(PB_N-2,32),floord(-32*t2+PB_N,32));t3++) {
        for (t4=max(2,32*t2);t4<=min(min(floord(PB_N,2),32*t2+31),-32*t3+PB_N);t4++) {
          for (t5=max(32*t3,t4);t5<=min(32*t3+31,-t4+PB_N);t5++) {
            A[t5+t4-2][t4] = A[t5+t4-1][t5+1] - A[t5+t4-1][t4] + 6;;
          }
        }
      }
    }
  ...
  if (PB_L >= 5) {
    lbp=0;
    ubp=min(floord(PB_L-3,32),floord(PB_N-1,32));
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=0;t3<=min(min(min(floord(PB_L-4,32),floord(-32*t2+PB_N,32)),floord(PB_N-1,64)),t2);t3++) {
        for (t4=0;t4<=min(min(min(floord(PB_L-3,32),floord(PB_N-1,32)),floord(-32*t2+PB_L-1,32)),floord(-32*t3+PB_L-2,32));t4++) {
          for (t5=max(max(2,32*t2),32*t3+1);t5<=min(min(min(min(PB_L-3,PB_N-1),32*t2+31),-32*t3+PB_N),-32*t4+PB_L-1);t5++) {
            for (t6=max(1,32*t3);t6<=min(min(32*t3+31,-t5+PB_N),t5-1);t6++) {
              for (t7=max(2,32*t4);t7<=min(min(PB_N-1,32*t4+31),-t5+PB_L-1);t7++) {
                A[t6+t5-1][t6+1] = C[t6][t6][t5] + D[t6+2][t7] + E[t6][t5][t6+t5+2] + E[t7][t6][t5] * C[t6+1][t7-2][t5] - E[t6-1][t5][t7+t5] + 4;;
              }
            }
          }
        }
...
kernel_3224232213_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
