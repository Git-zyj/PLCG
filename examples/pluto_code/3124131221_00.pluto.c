...
/* Start of CLooG code */
if ((PB_L >= 3) && (PB_M >= 2)) {
  for (t1=0;t1<=floord(2*PB_L+PB_M-8,32);t1++) {
    lbp=max(ceild(t1,2),ceild(32*t1-PB_L+3,32));
    ubp=min(min(floord(PB_L+PB_M-5,32),floord(32*t1+PB_M+29,64)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_M+2);t3<=min(PB_L-3,32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3);t4<=min(32*t2+31,t3+PB_M-2);t4++) {
          for (t5=0;t5<=PB_L-2;t5++) {
            A[(-t3+t4)+1][t5+1] = B[t5][(-t3+t4)][t3] * B[(-t3+t4)][(-t3+t4)][t5] - B[t5][t3][(-t3+t4)] + A[(-t3+t4)+1][t3+2] * A[(-t3+t4)][t5+1] + 2;;
          }
        }
      }
    }
