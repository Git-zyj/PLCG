...
void kernel_2122231111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
  ...
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=min(floord(-32*t1+PB_L-1,32),t1);t2++) {
      for (t3=max(1,32*t1);t3<=min(min(PB_L-2,32*t1+31),-32*t2+PB_L-1);t3++) {
        for (t4=max(1,32*t2);t4<=min(min(t3,32*t2+31),-t3+PB_L-1);t4++) {
          A[t4][t3+t4-1] = B[t3+1][t4] - C[t3+t4-1][t3] * B[t3-1][t4-1] * A[t4+1][t3+t4] * 5;;
        }
      }
    }
  }
...
kernel_2122231111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
