...
void kernel_3214112223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
/* Start of CLooG code */
if (PB_M >= 3) {
  for (t1=0;t1<=min(1,PB_M-3);t1++) {
    B[t1] = B[t1+1] * B[t1+2] + 5;;
  }
  for (t1=2;t1<=PB_M-3;t1++) {
    A[t1] = C[t1][1] * D[t1] * A[t1-2] * A[t1+1] - 4;;
    B[t1] = B[t1+1] * B[t1+2] + 5;;
  }
  if (PB_M >= 4) {
    A[(PB_M-2)] = C[(PB_M-2)][1] * D[(PB_M-2)] * A[(PB_M-2)-2] * A[(PB_M-2)+1] - 4;;
  }
}
/* End of CLooG code */
...
kernel_3214112223_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
