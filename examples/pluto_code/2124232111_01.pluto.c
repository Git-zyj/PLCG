...
void kernel_2124232111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_L >= 2) && (PB_N >= 3)) {
  lbp=0;
  ubp=min(floord(PB_L-2,32),floord(PB_N-2,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_N-2,32);t2++) {
      for (t3=max(1,32*t2);t3<=min(PB_N-2,32*t2+31);t3++) {
        lbv=32*t1;
        ubv=min(min(t3,PB_L-2),32*t1+31);
#pragma ivdep
#pragma vector always
        for (t4=lbv;t4<=ubv;t4++) {
          A[t3][t4+1] = B[t3][t4] * B[t3-1][0] - B[t4+1][t4+2] - A[t3+1][t4+1] + 5;;
        }
      }
    }
  }
...
kernel_2124232111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
