...
if ((PB_L >= 3) && (PB_P >= 3)) {
  for (t1=1;t1<=PB_P-2;t1++) {
    for (t2=0;t2<=floord(t1+3*PB_L-7,32);t2++) {
      lbp=max(ceild(2*t2,3),ceild(32*t2-PB_L+2,32));
      ubp=min(min(floord(t1+2*PB_L-5,32),floord(t1+64*t2+61,96)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(1,ceild(-t1+32*t3+1,2)),32*t2-32*t3);t4<=min(min(PB_L-2,16*t3+15),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,2*t4);t5<=min(32*t3+31,t1+2*t4-1);t5++) {
            A[t4][(-2*t4+t5)] = B[t1-1][t1][(-2*t4+t5)+1] + C[t4][(-2*t4+t5)][t1+1] * C[t1][t4+1][t1+1] + A[(-2*t4+t5)+1][t4] + A[t4-1][(-2*t4+t5)] - 4;;
          }
        }
      }
    }
