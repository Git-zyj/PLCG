...
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(1,32*t2-32*t3),32*t3-PB_M+2);t4<=min(min(PB_M-1,32*t3+30),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4+1);t5<=min(32*t3+31,t4+PB_M-2);t5++) {
            A[t1][(-t4+t5)][0] = B[t1-1][(-t4+t5)-1][t4] + B[t4-1][t1][(-t4+t5)] + B[t4][(-t4+t5)-1][t1+1] * A[t1][(-t4+t5)+1][0] * A[t1+1][(-t4+t5)][0] * 4;;
          }
        }
      }
    }
