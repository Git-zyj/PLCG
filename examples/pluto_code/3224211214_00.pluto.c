...
void kernel_3224211214_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
for (t2=lbp;t2<=ubp;t2++) {
  for (t3=0;t3<=min(floord(PB_M-3,32),t2);t3++) {
    for (t4=max(32*t2,32*t3+1);t4<=min(PB_M-2,32*t2+31);t4++) {
      lbv=32*t3;
...
for (t2=0;t2<=floord(2*PB_M+PB_Q-5,32);t2++) {
  lbp=max(ceild(t2,2),ceild(32*t2-PB_M+2,32));
  ubp=min(min(floord(PB_M+PB_Q-3,32),floord(32*t2+PB_Q+30,64)),t2);
#pragma omp parallel for private(lbv,ubv,t4,t5,t6)
  for (t3=lbp;t3<=ubp;t3++) {
    for (t4=max(max(1,32*t2-32*t3),32*t3-PB_Q+1);t4<=min(PB_M-2,32*t2-32*t3+31);t4++) {
      for (t5=max(32*t3,t4);t5<=min(32*t3+31,t4+PB_Q-1);t5++) {
        for (t6=0;t6<=PB_M-2;t6++) {
          B[t4+1][t4][(-t4+t5)+1] = C[t4+1][t6][(-t4+t5)] - B[t6+1][t6][(-t4+t5)+2] * B[t4][t4-1][(-t4+t5)+1] + 3;;
        }
      }
    }
  }
...
kernel_3224211214_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
