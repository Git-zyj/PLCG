...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      if (t1 == 2*t2) {
        for (t3=max(1,16*t1);t3<=min(PB_M-1,16*t1+31);t3++) {
          if (t1%2 == 0) {
            A[t3] = C[t3] + 1;;
          }
      ...
      if (t1 <= 2*t2-1) {
        for (t3=max(1,32*t1-32*t2);t3<=32*t1-32*t2+31-7;t3+=8) {
          for (t4=32*t2;t4<=min(PB_M-1,32*t2+31);t4++) {
            A[t4] = A[t3] - A[t3-1] * 2;;
            A[t4] = A[(t3+1)] - A[(t3+1)-1] * 2;;
            A[t4] = A[(t3+2)] - A[(t3+2)-1] * 2;;
            A[t4] = A[(t3+3)] - A[(t3+3)-1] * 2;;
            A[t4] = A[(t3+4)] - A[(t3+4)-1] * 2;;
            A[t4] = A[(t3+5)] - A[(t3+5)-1] * 2;;
            A[t4] = A[(t3+6)] - A[(t3+6)-1] * 2;;
            A[t4] = A[(t3+7)] - A[(t3+7)-1] * 2;;
          }
        }
