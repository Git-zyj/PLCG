...
void kernel_2224112123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma vector always
          for (t4=lbv;t4<=ubv;t4++) {
            B[t3+2][t4] = C[t3] + B[t3+4][t4] + 3;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        B[1 +2][0] = C[1] + B[1 +4][0] + 3;;
      }
      if (t2 == 0) {
        for (t3=max(2,32*t1);t3<=min(PB_M-1,32*t1+31);t3++) {
          B[t3+2][0] = C[t3] + B[t3+4][0] + 3;;
          A[t3] = A[t3-2] + 5;;
          lbv=1;
          ubv=min(31,t3-1);
#pragma ivdep
...
kernel_2224112123_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
