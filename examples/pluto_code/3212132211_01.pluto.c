...
void kernel_3212132211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
if (PB_M >= 3) {
  for (t2=1;t2<=PB_M-2;t2++) {
    B[1] = C[t2-1][t2] + C[t2][1 -1] * C[t2][t2] - B[t2+1] - 1;;
  }
  for (t1=2;t1<=PB_M-3;t1++) {
    B[t1] = C[1 -1][1] + C[1][t1-1] * C[1][1] - B[1 +1] - 1;;
    for (t2=2;t2<=-t1+PB_M-1;t2++) {
      A[t2+t1][t2] = C[t2-2][t1] - C[t2][t2] * C[t2][0] * A[t2+t1-1][t2] * A[t2+t1-2][t2-1] * 1;;
      B[t1] = C[t2-1][t2] + C[t2][t1-1] * C[t2][t2] - B[t2+1] - 1;;
    }
    for (t2=-t1+PB_M;t2<=PB_M-2;t2++) {
      B[t1] = C[t2-1][t2] + C[t2][t1-1] * C[t2][t2] - B[t2+1] - 1;;
    }
  }
  for (t1=max(2,PB_M-2);t1<=PB_M-1;t1++) {
    for (t2=1;t2<=PB_M-2;t2++) {
      B[t1] = C[t2-1][t2] + C[t2][t1-1] * C[t2][t2] - B[t2+1] - 1;;
    }
  }
...
kernel_3212132211_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
