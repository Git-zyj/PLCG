...
void kernel_2222211223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_L-1,32);t2++) {
      ...
      if ((t1 == 0) && (t2 <= floord(PB_L-3,32))) {
        for (t4=max(1,32*t2);t4<=min(PB_L-1,32*t2+31);t4++) {
          A[1 -1][(t4-1)+1] = 4;;
        }
      }
      if (t2 <= floord(PB_L-3,32)) {
        for (t3=max(2,32*t1);t3<=min(PB_M,32*t1+31);t3++) {
          if (t2 == 0) {
            A[(t3-1)][0] = C[(t3-1)-1][0] * D[(t3-1)-1][0] + A[(t3-1)][0 +1] - A[(t3-1)][0 +2] + 5;;
          }
          for (t4=max(1,32*t2);t4<=min(PB_L-3,32*t2+31);t4++) {
            A[(t3-1)][t4] = C[(t3-1)-1][t4] * D[(t3-1)-1][t4] + A[(t3-1)][t4+1] - A[(t3-1)][t4+2] + 5;;
            A[t3-1][(t4-1)+1] = 4;;
          }
          for (t4=PB_L-2;t4<=min(PB_L-1,32*t2+31);t4++) {
            A[t3-1][(t4-1)+1] = 4;;
          }
        }
      }
...
kernel_2222211223_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
