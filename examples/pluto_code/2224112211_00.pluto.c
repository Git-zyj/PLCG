...
void kernel_2224112211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if (PB_Q >= 3) {
  if (PB_L >= 0) {
    for (t1=0;t1<=floord(2*PB_Q+PB_L-2,32);t1++) {
      lbp=max(ceild(t1,2),ceild(32*t1-PB_Q+1,32));
      ubp=min(floord(32*t1+PB_L,64),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
      for (t2=lbp;t2<=ubp;t2++) {
        if (t1 == 2*t2) {
          for (t3=max(2,16*t1);t3<=min(min(PB_L-2,PB_Q-1),16*t1+31);t3++) {
            for (t4=max(2,16*t1);t4<=t3-1;t4++) {
              if (t1%2 == 0) {
                B[t3] = D[t3+1][t4] + A[t3-2] * A[t4-2] - A[t3-1] - 2;;
              }
            }
            if (t1%2 == 0) {
              A[t3-2] = C[t3] + B[t3] - 1;;
            ...
            if (t1%2 == 0) {
              B[t3] = D[t3+1][t3] + A[t3-2] * A[t3-2] - A[t3-1] - 2;;
            }
          }
        }
        if (t1 == 2*t2) {
          for (t3=max(max(2,16*t1),PB_L-1);t3<=min(PB_Q-1,16*t1+31);t3++) {
            if (t1%2 == 0) {
              A[t3-2] = C[t3] + B[t3] - 1;;
            }
          }
        ...
        if (t1 <= 2*t2-1) {
          for (t3=32*t2;t3<=min(PB_L-2,32*t2+31);t3++) {
            for (t4=max(2,32*t1-32*t2);t4<=min(PB_Q-1,32*t1-32*t2+31);t4++) {
              B[t3] = D[t3+1][t4] + A[t3-2] * A[t4-2] - A[t3-1] - 2;;
            }
          }
        }
        if (t1 == 2*t2) {
          for (t3=PB_Q;t3<=min(PB_L-2,16*t1+31);t3++) {
            for (t4=max(2,16*t1);t4<=PB_Q-1;t4++) {
              if (t1%2 == 0) {
                B[t3] = D[t3+1][t4] + A[t3-2] * A[t4-2] - A[t3-1] - 2;;
              }
            }
  ...
  if (PB_L <= -1) {
    for (t1=0;t1<=floord(PB_Q-1,16);t1+=2) {
      for (t3=max(2,16*t1);t3<=min(PB_Q-1,16*t1+31);t3++) {
        A[t3-2] = C[t3] + B[t3] - 1;;
      }
    }
  }
...
kernel_2224112211_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
