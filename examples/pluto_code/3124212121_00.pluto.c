...
void kernel_3124212121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (t4=32*t1;t4<=(min(PB_L-1,32*t1+31))-7;t4+=8) {
          for (t5=32*t2;t5<=min(PB_L-3,32*t2+31);t5++) {
            for (t6=max(2,32*t3);t6<=min(PB_L-1,32*t3+31);t6++) {
              A[t4][t5+2][t6] = B[t6][t5][1] * A[t4][t5+2][t6-2] * 1;;
              A[(t4+1)][t5+2][t6] = B[t6][t5][1] * A[(t4+1)][t5+2][t6-2] * 1;;
              A[(t4+2)][t5+2][t6] = B[t6][t5][1] * A[(t4+2)][t5+2][t6-2] * 1;;
              A[(t4+3)][t5+2][t6] = B[t6][t5][1] * A[(t4+3)][t5+2][t6-2] * 1;;
              A[(t4+4)][t5+2][t6] = B[t6][t5][1] * A[(t4+4)][t5+2][t6-2] * 1;;
              A[(t4+5)][t5+2][t6] = B[t6][t5][1] * A[(t4+5)][t5+2][t6-2] * 1;;
              A[(t4+6)][t5+2][t6] = B[t6][t5][1] * A[(t4+6)][t5+2][t6-2] * 1;;
              A[(t4+7)][t5+2][t6] = B[t6][t5][1] * A[(t4+7)][t5+2][t6-2] * 1;;
            }
          }
        }
...
kernel_3124212121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
