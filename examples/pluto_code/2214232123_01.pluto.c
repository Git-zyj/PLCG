...
void kernel_2214232123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_N >= 1) {
  for (t1=0;t1<=PB_N-2;t1++) {
    A[t1] = C[t1+1] - D[t1+1] + A[t1+1] * 1;;
    A[t1] = C[t1] - C[t1] - C[t1] * 1;;
  }
  A[(PB_N-1)] = C[(PB_N-1)] - C[(PB_N-1)] - C[(PB_N-1)] * 1;;
}
/* End of CLooG code */
...
kernel_2214232123_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
