...
void kernel_2212131221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
if (PB_Q >= 4) {
  for (t1=0;t1<=floord(PB_Q-2,32);t1++) {
    for (t2=0;t2<=min(floord(PB_Q-2,32),floord(-64*t1+3*PB_Q-8,32));t2++) {
      if (t2 >= 1) {
        for (t3=max(1,32*t1);t3<=min(min(PB_Q-3,32*t1+31),32*t2+30);t3++) {
          for (t4=max(32*t2,t3+1);t4<=min(PB_Q-2,32*t2+31);t4++) {
            A[t4][t3] = C[t3-1][0] * D[t3] * C[t3][t4+1] - C[t3][t4-1] * C[t4][t3] + A[t4-2][t3] + A[t4+2][t3] - A[t3][t4+2] + 1;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t4=2;t4<=min(31,PB_Q-2);t4++) {
          A[t4][1] = C[1 -1][0] * D[1] * C[1][t4+1] - C[1][t4-1] * C[t4][1] + A[t4-2][1] + A[t4+2][1] - A[1][t4+2] + 1;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(30,PB_Q-3);t3++) {
          B[t3] = D[t3-1] + B[t3-1] * 5;;
          for (t4=t3+1;t4<=min(31,PB_Q-2);t4++) {
            A[t4][t3] = C[t3-1][0] * D[t3] * C[t3][t4+1] - C[t3][t4-1] * C[t4][t3] + A[t4-2][t3] + A[t4+2][t3] - A[t3][t4+2] + 1;;
          }
        }
      }
      if (t2 == 0) {
        for (t3=max(31,32*t1);t3<=min(PB_Q-2,32*t1+31);t3++) {
          B[t3] = D[t3-1] + B[t3-1] * 5;;
        }
      }
      if ((PB_Q <= 32) && (t1 == 0) && (t2 == 0)) {
        B[(PB_Q-2)] = D[(PB_Q-2)-1] + B[(PB_Q-2)-1] * 5;;
      }
    }
  }
}
...
kernel_2212131221_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
