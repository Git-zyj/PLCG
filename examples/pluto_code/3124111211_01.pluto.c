...
/* Start of CLooG code */
if (PB_M >= 2) {
  for (t1=max(-1,ceild(-PB_M-29,32));t1<=floord(PB_M-1,32);t1++) {
    lbp=max(0,t1);
    ubp=min(floord(PB_M-1,32),floord(32*t1+PB_M+29,32));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(1,32*t2),-32*t1+32*t2-30);t3<=min(PB_M-1,32*t2+31);t3++) {
        for (t4=max(1,32*t1-32*t2+t3);t4<=min(t3,32*t1-32*t2+t3+31);t4++) {
          A[t3][t4] = B[t3][t4] * A[t3][t4-1] + A[t3-1][t4-1] * 4;;
        }
      }
    }
  }
