...
void kernel_2214132121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
/* Start of CLooG code */
if (PB_P >= 2) {
  for (t1=0;t1<=min(PB_M-5,PB_P-3);t1++) {
    for (t3=0;t3<=-t1+PB_M-5;t3++) {
      B[t3+t1] = C[t3][t1] * A[t1][t1] + B[t3+t1+4] - 4;;
    }
    for (t3=t1+1;t3<=PB_P-2;t3++) {
      A[t1][t1] = C[0][t3] - C[t1+2][t3] * C[t1][t3+1] - C[2][t3] * 5;;
    }
  }
  for (t1=max(0,PB_M-4);t1<=PB_P-3;t1++) {
    for (t3=t1+1;t3<=PB_P-2;t3++) {
      A[t1][t1] = C[0][t3] - C[t1+2][t3] * C[t1][t3+1] - C[2][t3] * 5;;
    }
  }
  if (PB_P <= PB_M-3) {
    for (t3=0;t3<=-PB_P+PB_M-3;t3++) {
      B[t3+(PB_P-2)] = C[t3][(PB_P-2)] * A[(PB_P-2)][(PB_P-2)] + B[t3+(PB_P-2)+4] - 4;;
    }
  }
}
/* End of CLooG code */
...
kernel_2214132121_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
