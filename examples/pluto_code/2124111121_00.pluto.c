...
void kernel_2124111121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if ((PB_L >= 3) && (PB_M >= 1)) {
  for (t1=0;t1<=PB_L-3;t1++) {
    for (t2=0;t2<=PB_M-1;t2++) {
      A[t1][t1] = B[t2][t1][0] + A[t1+2][t1+2] * 1;;
    }
  }
}
/* End of CLooG code */
...
kernel_2124111121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
