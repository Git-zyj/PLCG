...
void kernel_2124111121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
if ((PB_N >= 2) && (PB_P >= 3)) {
  lbp=0;
  ubp=min(floord(PB_N-1,32),floord(PB_P-1,32));
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=t1;t2<=floord(PB_P-1,32);t2++) {
      for (t3=max(1,32*t1);t3<=min(min(PB_N-1,PB_P-1),32*t1+31);t3++) {
        for (t4=max(max(2,32*t2),t3);t4<=min(PB_P-1,32*t2+31);t4++) {
          A[t3][t4] = B[t3-1][t4] - A[t3][t4-2] - 6;;
        }
      }
    }
  }
...
kernel_2124111121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
