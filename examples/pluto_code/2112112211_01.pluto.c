...
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=32*t2;t3<=min(32*t2+31,-32*t1+32*t2+PB_M-2);t3++) {
        for (t4=32*t1-32*t2;t4<=min(min(PB_P-1,32*t1-32*t2+31),-t3+PB_M-2);t4++) {
          A[t3+t4][t3] = B[t3][t4] + A[t3+t4+1][t3+1] - A[t3+t4+1][t3] * 4;;
        }
      }
    }
  }
