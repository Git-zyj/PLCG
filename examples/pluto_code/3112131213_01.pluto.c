...
void kernel_3112131213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
if (PB_N >= 2) {
  for (t1=1;t1<=PB_N-1;t1++) {
    A[t1] = B[t1] + B[t1-1] + B[t1] * A[t1-1] - A[t1+1] - 3;;
  }
}
/* End of CLooG code */
...
kernel_3112131213_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
