...
void kernel_3124211121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(PB_M-1,32);t2++) {
      for (t3=32*t2;t3<=min(PB_M-1,32*t2+31);t3++) {
...
#pragma vector always
          for (t5=lbv;t5<=ubv;t5++) {
            A[t3][t5][t5] = B[t3][t4][t5] - A[t3+2][t5][t5] - 2;;
            A[t3][t5][t5] = B[t3][(t4+1)][t5] - A[t3+2][t5][t5] - 2;;
            A[t3][t5][t5] = B[t3][(t4+2)][t5] - A[t3+2][t5][t5] - 2;;
            A[t3][t5][t5] = B[t3][(t4+3)][t5] - A[t3+2][t5][t5] - 2;;
            A[t3][t5][t5] = B[t3][(t4+4)][t5] - A[t3+2][t5][t5] - 2;;
            A[t3][t5][t5] = B[t3][(t4+5)][t5] - A[t3+2][t5][t5] - 2;;
            A[t3][t5][t5] = B[t3][(t4+6)][t5] - A[t3+2][t5][t5] - 2;;
            A[t3][t5][t5] = B[t3][(t4+7)][t5] - A[t3+2][t5][t5] - 2;;
          }
        }
        for (;t4<=PB_L-1;t4++) {
          lbv=32*t1;
...
kernel_3124211121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
