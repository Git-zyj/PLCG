...
/* Start of CLooG code */
if (PB_L >= 2) {
  for (t1=0;t1<=min(1,PB_L-2);t1++) {
    for (t2=0;t2<=PB_L-1;t2++) {
      B[t1+1][t1] = B[t1+2][t1+1] + 2;;
    }
  }
  for (t1=2;t1<=min(floord(PB_M-1,2),PB_L-2);t1++) {
    for (t2=0;t2<=t1-1;t2++) {
      B[t1+1][t1] = B[t1+2][t1+1] + 2;;
    }
    for (t2=t1;t2<=min(PB_L-1,-t1+PB_M-1);t2++) {
      A[t2][t1+1] = C[t2][t1-1] - C[t2+t1][t2-1] * D[t2+1] - D[t2] - C[t2+1][t1] * A[t1-2][t2+1] * 6;;
      B[t1+1][t1] = B[t1+2][t1+1] + 2;;
    }
    if (t1 <= -PB_L+PB_M-1) {
      A[PB_L][t1+1] = C[PB_L][t1-1] - C[PB_L+t1][PB_L-1] * D[PB_L+1] - D[PB_L] - C[PB_L+1][t1] * A[t1-2][PB_L+1] * 6;;
    }
    for (t2=-t1+PB_M;t2<=PB_L-1;t2++) {
      B[t1+1][t1] = B[t1+2][t1+1] + 2;;
    }
  }
  if ((PB_L >= 3) && (PB_L <= floord(PB_M+1,2))) {
    for (t2=PB_L-1;t2<=min(PB_L,-PB_L+PB_M);t2++) {
      A[t2][(PB_L-1)+1] = C[t2][(PB_L-1)-1] - C[t2+(PB_L-1)][t2-1] * D[t2+1] - D[t2] - C[t2+1][(PB_L-1)] * A[(PB_L-1)-2][t2+1] * 6;;
    }
  }
  for (t1=max(2,ceild(PB_M,2));t1<=PB_L-2;t1++) {
    for (t2=0;t2<=PB_L-1;t2++) {
      B[t1+1][t1] = B[t1+2][t1+1] + 2;;
    }
  }
