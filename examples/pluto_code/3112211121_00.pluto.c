...
void kernel_3112211121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
...
if (PB_L >= 2) {
  for (t1=0;t1<=PB_L-2;t1++) {
    for (t2=ceild(3*t1-61,32);t2<=floord(PB_L,8);t2++) {
      lbp=max(max(ceild(2*t2,3),ceild(t1-15,16)),ceild(32*t2-PB_L-1,32));
      ubp=min(min(floord(3*PB_L+1,32),floord(-t1+32*t2+31,32)),floord(64*t2+PB_L+61,96));
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(max(2,ceild(32*t3-PB_L+1,2)),t1),32*t2-32*t3);t4<=min(min(PB_L+1,16*t3+15),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,2*t4);t5<=min(32*t3+31,2*t4+PB_L-1);t5++) {
            A[t4][(-2*t4+t5)] = B[t4][t1+1][t4+1] * A[(-2*t4+t5)+1][t4-2] + 5;;
          }
        }
      }
    }
...
kernel_3112211121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
