...
void kernel_2214232123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(2,32*t1-32*t2),32*t2-PB_L+1);t3<=min(min(PB_L-1,32*t2+30),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+1);t4<=min(32*t2+31,t3+PB_L-1);t4++) {
          A[(-t3+t4)][2] = C[(-t3+t4)+2][t3] - D[t3-1][1] - A[(-t3+t4)-1][2] * B[(-t3+t4)][t3-1] - 1;;
          B[(-t3+t4)][t3-1] = D[(-t3+t4)][t3+1] - D[t3-2][(-t3+t4)] + D[t3-1][(-t3+t4)+2] + 6;;
        }
      }
    }
  }
...
kernel_2214232123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
