...
void kernel_2224131213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma omp parallel for private(lbv,ubv,t4,t5)
      for (t3=lbp;t3<=ubp;t3++) {
        for (t4=max(max(1,32*t2-32*t3),32*t3-PB_L+1);t4<=min(min(PB_L-2,32*t3+30),32*t2-32*t3+31);t4++) {
          for (t5=max(32*t3,t4+1);t5<=min(32*t3+31,t4+PB_L-1);t5++) {
            A[(-t4+t5)-1] = C[(-t4+t5)-1][t4+1] + C[t4][0] - C[(-t4+t5)][t4-1] + A[(-t4+t5)] + B[(-t4+t5)] * 4;;
          }
        }
      }
    }
  ...
  for (t2=1;t2<=PB_L;t2++) {
    B[t2] = D[t2] * D[t2+1] - D[t2] * A[t2-1] - B[t2+1] + 4;;
  }
}
/* End of CLooG code */
...
kernel_2224131213_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
