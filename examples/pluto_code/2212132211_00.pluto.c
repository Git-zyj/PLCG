...
void kernel_2212132211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
if ((PB_L >= 0) && (PB_M >= max(0,-PB_L+3))) {
  for (t1=0;t1<=min(min(floord(2*PB_M+PB_L-1,64),floord(5*PB_M+3*PB_L-9,96)),floord(5*PB_M+3*PB_L-5,160));t1++) {
    for (t2=max(max(0,ceild(32*t1-PB_M,16)),ceild(96*t1-3*PB_M-PB_L+3,32));t2<=min(floord(PB_L,32),floord(PB_M+3*PB_L-3,96));t2++) {
      if ((t2 >= 1) && (t2 <= floord(PB_L-1,32))) {
        for (t3=32*t1;t3<=min(min(floord(PB_L-1,2),32*t1+31),16*t2+15);t3++) {
          for (t4=max(32*t2,2*t3);t4<=min(PB_L-1,32*t2+31);t4++) {
            B[(-t3+t4)] = D[t3][(-t3+t4)-2] - D[(-t3+t4)][t3] + D[t3][(-t3+t4)-1] * D[(-t3+t4)+t3][0] - B[(-t3+t4)-1] * 6;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=0;t3<=min(1,PB_L-3);t3++) {
          for (t4=t3+2;t4<=min(31,PB_L-1);t4++) {
            B[(-t3+t4)] = D[t3][(-t3+t4)-2] - D[(-t3+t4)][t3] + D[t3][(-t3+t4)-1] * D[(-t3+t4)+t3][0] - B[(-t3+t4)-1] * 6;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=2;t3<=min(min(15,floord(PB_L-1,2)),PB_M-1);t3++) {
          A[t3] = C[t3-2] + C[t3] * A[t3-1] * A[t3+1] + 6;;
          for (t4=2*t3;t4<=min(31,PB_L-1);t4++) {
            B[(-t3+t4)] = D[t3][(-t3+t4)-2] - D[(-t3+t4)][t3] + D[t3][(-t3+t4)-1] * D[(-t3+t4)+t3][0] - B[(-t3+t4)-1] * 6;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(2,ceild(PB_L,2));t3<=min(15,PB_M-1);t3++) {
          A[t3] = C[t3-2] + C[t3] * A[t3-1] * A[t3+1] + 6;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(2,PB_M);t3<=min(15,floord(PB_L-1,2));t3++) {
          for (t4=2*t3;t4<=min(31,PB_L-1);t4++) {
            B[(-t3+t4)] = D[t3][(-t3+t4)-2] - D[(-t3+t4)][t3] + D[t3][(-t3+t4)-1] * D[(-t3+t4)+t3][0] - B[(-t3+t4)-1] * 6;;
          }
        }
...
if (PB_M <= -1) {
  for (t1=0;t1<=min(floord(PB_L-3,32),floord(PB_L-1,64));t1++) {
    for (t2=2*t1;t2<=floord(PB_L-1,32);t2++) {
      for (t3=32*t1;t3<=min(min(min(floord(PB_L-1,2),PB_L-3),32*t1+31),16*t2+15);t3++) {
...
kernel_2212132211_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
