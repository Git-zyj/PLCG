...
void kernel_3114132211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(32*t1-32*t2,32*t2-PB_M+1);t3<=min(min(PB_L-1,32*t2+29),32*t1-32*t2+31);t3++) {
        for (t4=max(32*t2,t3+2);t4<=min(32*t2+31,t3+PB_M-1);t4++) {
          for (t5=2;t5<=min(PB_L-1,-t3+t4);t5++) {
            A[t5-1][(-t3+t4-t5)+1] = B[t5+2][(-t3+t4-t5)][t3] - B[(-t3+t4-t5)+t5][t3][t5] + C[t5-2][(-t3+t4-t5)] * A[(-t3+t4-t5)][t5] - A[t5-1][(-t3+t4-t5)] + 1;;
          }
        }
      }
    }
...
kernel_3114132211_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
