...
void kernel_2214112113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma omp parallel for private(lbv,ubv,t2,t3,t4)
  for (t1=lbp;t1<=ubp;t1++) {
    ...
    for (t2=max(0,ceild(32*t1-PB_N+1,32));t2<=min(floord(PB_L,32),floord(-128*t1+4*PB_N+PB_L-4,32));t2++) {
      if ((t2 >= 1) && (t2 <= floord(PB_L-4,32))) {
        for (t3=max(1,32*t1);t3<=min(min(min(PB_N,PB_L-4),32*t1+31),32*t2+31);t3++) {
          for (t4=max(32*t2,t3);t4<=min(PB_L-4,32*t2+31);t4++) {
            A[t3][t4+2] = C[t3-1] - A[t3][t4+3] + 5;;
          }
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
      ...
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=1;t3<=min(min(31,PB_L-4),PB_N-1);t3++) {
          B[t3] = C[t3] + 5;;
          for (t4=t3;t4<=min(31,PB_L-4);t4++) {
            A[t3][t4+2] = C[t3-1] - A[t3][t4+3] + 5;;
          }
        }
      }
      if ((PB_N <= min(31,PB_L-4)) && (t1 == 0) && (t2 == 0)) {
        for (t4=PB_N;t4<=min(31,PB_L-4);t4++) {
          A[PB_N][t4+2] = C[PB_N-1] - A[PB_N][t4+3] + 5;;
        }
      }
      if ((t1 == 0) && (t2 == 0)) {
        for (t3=max(1,PB_L-3);t3<=min(31,PB_N-1);t3++) {
          B[t3] = C[t3] + 5;;
        }
      }
    }
...
kernel_2214112113_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
