...
void kernel_2114231223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
/* Start of CLooG code */
if (PB_M >= 5) {
  for (t2=-PB_M+2;t2<=-3;t2++) {
    A[-t2][-t2-1] = B[-t2] - B[-t2] - C[-t2-1] - A[-t2+1][-t2] - A[-t2-2][-t2-3] + 5;;
  }
}
/* End of CLooG code */
...
kernel_2114231223_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
