...
void kernel_32342112161_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i = 2; i < PB_M; i++) {
        A[i-1] = A[i] + A[i-2] * C[i] * 6;
    }
    for (int j = 1; j < PB_M; j++) {
        for (int k = 1; k < min(j,PB_L); k++) {
            A[k-1] = A[j] * A[j-1] + D[1][k] + 1;
        }
    }
#pragma endscop
...
kernel_32342112161_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
