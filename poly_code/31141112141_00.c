...
void kernel_31141112141_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 2; i < PB_M-1; i++) {
        for (int j = 1; j < min(i,PB_L); j++) {
            A[j][i-1] = A[j-1][i-2] + A[j-1][i-1] - B[i+1][j] - 4;
        }
    }
#pragma endscop
...
kernel_31141112141_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
