...
void kernel_31242112161_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M; i++)
        for (int j = 1; j < min(i,PB_L); j++)
            A[i-1][j] = A[i-1][j-1] + A[i][j] + B[j] - 2;
#pragma endscop
...
kernel_31242112161_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
