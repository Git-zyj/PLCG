...
void kernel_32343112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i = 2; i < PB_N+1; i++)
        for (int j = 0; j < PB_M; j++)
            A[i-1][j] = C[j][i-1] - B[i-1] + 5;
        B[i-1] = D[i] * B[i-2] + B[i] * B[i-2] - 4;
#pragma endscop
...
kernel_32343112121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
