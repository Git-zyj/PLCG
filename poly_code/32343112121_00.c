...
void kernel_32343112121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_Q-2; i++)
        for (int j = 0; j < i; j++)
            for (int k = 0; k < PB_P; k++)
                A[j+1][0][i+1] = A[j+2][0][i+1] + A[j+1][0][i+2] - C[i-1][k][k] - 1;
    for (int l = 0; l < PB_P; l++)
        for (int m = 0; m < PB_Q; m++)
            for (int n = 0; n < m; n++)
                A[m+1][0][n] = A[m+2][0][0] - A[m+1][0][0] + D[n][l][m] * 2;
#pragma endscop
...
kernel_32343112121_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
