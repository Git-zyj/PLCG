...
void kernel_35341112161_00(int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_Q-1; i++)
        for (int j = 2; j < min(i,PB_M+2); j++)
            E[j-2] = E[2] + E[2] - E[2] * F[j][i+1] * 5;
    for (int k = 0; k < PB_M; k++)
        for (int l = 1; l < PB_N; l++)
            for (int m = 0; m < PB_M; m++)
                B[k][l-1][k] = B[k][m][k] + G[l][m][k] + H[m][l+1][m][k] + 6;
        D[k][k] = D[k][k] * I[k] + 6;
    for (int n = 1; n < PB_M-1; n++)
        for (int o = 2; o < PB_M+1; o++)
            D[n][n] = D[n+1][n+1] * E[o-2] * E[o-1] - F[o][n] * 1;
        E[n-1] = D[n][n] + D[n-1][n-1] * 3;
#pragma endscop
...
init_array(xe,POLYBENCH_ARRAY(E), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_35341112161_00(xe,POLYBENCH_ARRAY(E), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xe,POLYBENCH_ARRAY(E), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
