...
void kernel_31342112141_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_L+1; i++)
        for (int j = 1; j < PB_M; j++)
            for (int k = 1; k < i; k++)
                A[i+1][j][k-1] = B[j-1][j][k-1] * A[k+2][j][i-1] + A[i+2][j][k-1] - 1;
#pragma endscop
...
kernel_31342112141_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
