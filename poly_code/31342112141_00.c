...
void kernel_31342112141_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_L-1; i++) {
        for (int j = 0; j < PB_L-1; j++) {
            A[i][j] = A[i][j+1] * A[j][i+1] + B[i][j+1] + 6;
        }
    }
#pragma endscop
...
kernel_31342112141_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
