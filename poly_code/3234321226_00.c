...
void kernel_3234321226_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa)){
polybench_start_instruments;
#pragma scop
for (int i = 1; i < PB_N; i++) {
    A[i] = A[i] * A[i-1] - 5;
}
for (int j = 2; j < PB_N-1; j++) {
    A[j] = A[j+1] - A[j-2] - A[j] * A[j+1] + 5;
}
#pragma endscop
...
kernel_3234321226_00(xa,POLYBENCH_ARRAY(A));
