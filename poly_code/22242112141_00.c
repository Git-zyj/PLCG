...
void kernel_22242112141_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M-1; i++) {
        for (int j = 0; j < i; j++) {
            A[j][i] = B[i] + A[j-1][i+1] * A[j+1][i+1] * C[i][j+1] * 2;
        }
        B[i] = B[i-1] * D[i] + 5;
    }
#pragma endscop
...
kernel_22242112141_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
