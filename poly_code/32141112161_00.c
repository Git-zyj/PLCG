...
void kernel_32141112161_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_L; i++)
        for (int j = 0; j < PB_L; j++)
            for (int k = 0; k < j; k++)
                A[i][j][k] = C[j][i-1][k] - C[i][k][j+1] * A[i+1][j][k] * A[i-1][j][k] + 1;
...
kernel_32141112161_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
