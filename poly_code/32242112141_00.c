...
void kernel_32242112141_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_M-1; i++)
        for (int j = 1; j < i; j++)
            for (int k = 2; k < PB_N; k++)
                A[k-1][i+1][j] = A[k-1][i+1][j+1] - A[k-2][i+1][j] - A[k][i+1][j] + A[k-1][i+1][j-1] * 5;
    for (int l = 0; l < PB_M-1; l++)
        for (int m = 1; m < PB_M; m++)
            for (int n = 2; n < l; n++)
                A[n-2][l+1][m] = C[l][n][m] * C[m][n-1][m-1] + 2;
#pragma endscop
...
kernel_32242112141_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
