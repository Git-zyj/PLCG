...
void kernel_31142112161_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M; i++) {
        for (int j = 1; j < PB_M; j++) {
            for (int k = 1; k < PB_P+1; k++) {
                A[i][j][k-1] = A[j-1][i][k-1] * A[j][k-1][i-1] - B[k-1][k] * 6;
            }
        }
    }
#pragma endscop
...
kernel_31142112161_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
