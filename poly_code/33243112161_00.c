...
void kernel_33243112161_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_L+1; i++) {
        for (int j = 1; j < PB_L; j++) {
            A[i][j] = A[j-1][i-1] * A[i][j-1] + D[j+1] * 6;
        }
    }
    for (int k = 0; k < PB_N-1; k++) {
        B[k+1] = B[k+2] - B[k] - E[k] - E[k+1] - 4;
        for (int l = 1; l < k; l++) {
            C[l][k] = A[k][l] * A[k][l-1] + 1;
        }
    }
#pragma endscop
...
kernel_33243112161_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
