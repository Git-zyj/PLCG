...
void kernel_31142112141_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_L; i++) {
        for (int j = 0; j < min(i,PB_M-1); j++) {
            for (int k = 1; k < PB_P; k++) {
                A[k+1][j] = A[k+1][i] + A[j+1][k-1] + B[i-1][k][k] - 6;
            }
        }
    }
#pragma endscop
...
kernel_31142112141_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
