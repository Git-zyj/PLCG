...
void kernel_32142112141_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_P; i++) {
        A[i] = A[i-1] - A[i+1] + C[i] - C[i] + 3;
    }
    for (int j = 1; j < PB_P+1; j++) {
        for (int k = 0; k < min(j,PB_P); k++) {
            B[k][j-1] = B[k+1][j-1] + B[j+1][k] - 5;
        }
    }
#pragma endscop
...
kernel_32142112141_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
