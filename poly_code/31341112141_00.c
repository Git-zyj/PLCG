...
void kernel_31341112141_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_N; i++)
        for (int j = 2; j < PB_M; j++)
            for (int k = 1; k < j; k++)
                A[i][j-1][k-1] = A[i][j-2][k-1] * A[i][j-1][k] + B[i+1][j] + 6;
#pragma endscop
...
kernel_31341112141_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
