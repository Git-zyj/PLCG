...
for (int i = 0; i < PB_P; i++) {
    for (int j = 1; j < PB_M-2; j++) {
        A[j][j-1] = A[j+2][j] - A[i][i] + A[i][j+1] + 5;
    }
    for (int k = -1; k < PB_M-1; k++) {
        for (int l = 1; l < min(i,PB_M-1); l++) {
            B[k+1][i][l] = B[l+1][l-1][i] - 4;
        }
    }
}
for (int m = 1; m < PB_M; m++) {
    for (int n = 0; n < m; n++) {
        A[n][m-1] = A[n][m-1] - A[m][n] * 1;
    }
}
for (int o = 1; o < PB_P; o++) {
    for (int p = 1; p < o; p++) {
        for (int q = 2; q < PB_P; q++) {
            B[o-1][p-1][q] = B[o-1][p-1][q-2] * B[o-1][p-1][q] - B[q][q][o] + 6;
        }
    }
}
#pragma endscop
