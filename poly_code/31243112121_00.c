...
void kernel_31243112121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 2; i < PB_M; i++) {
        for (int j = 1; j < i; j++) {
            for (int k = 1; k < PB_M+1; k++) {
                A[i-1][k-1][k+1] = A[i][j-1][j+1] + A[i-2][j-1][j+1] * B[k-1][i][k-1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_31243112121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
