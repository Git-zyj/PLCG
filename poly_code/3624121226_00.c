...
void kernel_3624121226_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
for (int i = 0; i < PB_Q-2; i++) {
    ...
    for (int l = 1; l < i; l++) {
        for (int m = -1; m < PB_L-1; m++) {
            A[m+1][l-1][i] = A[m+1][l-1][i+1] + 5;
        }
    }
    for (int n = 2; n < PB_Q; n++) {
        for (int o = 1; o < PB_P-1; o++) {
            A[o-1][n-1][i+1] = A[o-1][n-2][i+2] - A[o-1][n-1][i+1] * A[i][o+1][n] + A[o][n-1][i] * 2;
        }
    }
}
for (int p = 0; p < PB_N-1; p++) {
    B[p] = B[p+1] + B[p+1] + B[p+1] - 4;
}
for (int q = 1; q < PB_M-1; q++) {
    for (int r = 1; r < PB_P-1; r++) {
        for (int s = -1; s < PB_L-1; s++) {
            A[q-1][r+1][r-1] = A[q][r+1][r-1] + A[q-1][r+1][r+1] + A[s+1][r-1][q-1] * 5;
        }
    }
    for (int t = 0; t < q; t++) {
        C[q][t] = C[q][t+1] * C[q+1][t] + 2;
    }
}
#pragma endscop
...
kernel_3624121226_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
