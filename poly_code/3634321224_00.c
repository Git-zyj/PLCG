...
for (int i = 0; i < PB_L-1; i++) {
    for (int j = 1; j < i; j++) {
        A[j][i] = B[i+1][j-1] * A[i][j+1] * 2;
    }
}
for (int k = 1; k < PB_L; k++) {
    for (int l = 1; l < k; l++) {
        B[k-1][l] = C[l] - D[l-1][k] * 5;
    }
}
for (int m = 1; m < PB_L-3; m++) {
    for (int n = 0; n < PB_Q; n++) {
        B[m][n] = 2;
    }
    for (int o = 0; o < m; o++) {
        B[m+1][m] = B[m-1][m] - B[m+3][m] - 6;
    }
}
for (int p = 0; p < PB_L; p++) {
    C[p] = C[p] * E[p] + 4;
    for (int q = 0; q < min(p,PB_L-1); q++) {
        D[q][p] = C[p] * D[q][p] + B[q+1][q] + E[p+1] + 6;
    }
}
#pragma endscop
