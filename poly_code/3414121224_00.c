...
for (int i = 1; i < PB_M-2; i++) {
    for (int j = 0; j < i; j++) {
        for (int k = 1; k < i; k++) {
            A[i+1][k-1][i] = A[i+1][k-1][i+2] - A[i+2][k-1][i] * A[j+1][k][i] + B[j][i-1] * 6;
        }
        B[j][i] = B[j][i] - C[j] - B[j][i-1] - 2;
    }
    for (int l = 1; l < PB_M; l++) {
        C[l-1] = C[l-1] * C[i] * C[l] * 1;
    }
}
for (int m = 1; m < PB_M; m++) {
    for (int n = 1; n < PB_M; n++) {
        for (int o = 3; o < PB_M; o++) {
            A[n][o-1][m] = A[n][o-3][m] + A[n][o-1][m+1] * A[n-1][m][n-1] - A[n-1][o][m-1] - 3;
        }
    }
}
#pragma endscop
