...
void kernel_32141112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_L-1; i++) {
        for (int j = 1; j < PB_L-1; j++) {
            A[i][j] = B[i][j] - A[j][i+1] + A[j-1][i] - C[i-1][j+1] - 1;
            B[i][j] = A[j+1][i+1] * C[i][j] - 5;
        }
    }
#pragma endscop
...
kernel_32141112121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
