...
void kernel_32342112141_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_L-1; i++)
        for (int j = 1; j < PB_L; j++)
            A[j] = C[j][i] * A[i+1] + A[j-1] * 1;
    for (int k = 1; k < PB_L; k++)
        B[k] = D[k] + B[k-1] + B[k+1] + 6;
#pragma endscop
...
kernel_32342112141_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
