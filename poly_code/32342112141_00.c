...
void kernel_32342112141_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M-1; i++) {
        A[i] = A[i+1] + A[i-1] * C[i-1][i] - 5;
    }
    for (int j = 1; j < PB_P; j++) {
        for (int k = 1; k < PB_P+1; k++) {
            B[k][j-1] = B[k-1][j] - B[j+1][k-1] - D[k][j] + 2;
        }
    }
#pragma endscop
...
kernel_32342112141_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
