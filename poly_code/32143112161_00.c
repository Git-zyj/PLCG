...
void kernel_32143112161_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i = 2; i < PB_M-1; i++)
        for (int j = 0; j < i; j++)
            for (int k = 0; k < PB_N; k++)
                A[k][i][j] = C[i+1][k][j] * D[k][i][j] - A[k][i+1][j+1] + 5;
        B[i-1] = B[i-2] + B[i] * B[i-2] * 6;
#pragma endscop
...
kernel_32143112161_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
