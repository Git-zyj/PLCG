...
void kernel_32143112161_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M-1; i++) {
        A[i] = A[i-1] * A[i+1] - A[i-1] - 5;
        A[i] = A[i] - C[i] * C[i] * 1;
    }
#pragma endscop
...
kernel_32143112161_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
