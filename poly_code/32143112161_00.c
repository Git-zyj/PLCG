...
void kernel_32143112161_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_P; i++) {
        for (int j = 0; j < PB_N; j++) {
            for (int k = 0; k < PB_M-1; k++) {
                A[k][k+1][i] = A[k][k+1][i] + 6;
            }
        }
        B[i+1] = D[i] * 2;
    }
...
kernel_32143112161_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
