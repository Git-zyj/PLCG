...
void kernel_32242112121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M; i++)
        for (int j = 2; j < min(i,PB_P-1); j++)
            for (int k = 2; k < PB_P+1; k++)
                A[k][j][j-1] = A[k][j+1][j] + A[j][k-1][k-2] * A[0][j-1][j-2] - C[i][k][i] + C[0][i][j] + 1;
            A[i][j][j-1] = A[i-1][j][j-1] + 5;
#pragma endscop
...
kernel_32242112121_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
