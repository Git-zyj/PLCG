...
void kernel_32242112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_P-2; i++) {
        for (int j = 1; j < PB_P-2; j++) {
            A[i][j+1] = A[i][j+2] + A[j-1][i+1] * A[j][i+2] + C[j][i] - D[i][j] + 1;
        }
        for (int k = 0; k < i; k++) {
            B[i+1][k] = B[i+2][k] + 2;
        }
...
kernel_32242112121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
