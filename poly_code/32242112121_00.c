...
void kernel_32242112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_L-1; i++)
        for (int j = 1; j < PB_M-1; j++)
            for (int k = 0; k < j; k++)
                A[j+1][k] = A[j][k+1] * A[i+2][k] * A[k+1][j-1] - 3;
        B[i] = C[i+1] * C[i] - B[i+1] * 4;
#pragma endscop
...
kernel_32242112121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
