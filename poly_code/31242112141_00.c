...
void kernel_31242112141_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_M-1; i++) {
        for (int j = 1; j < PB_P; j++) {
            A[i+1][i+1][j] = A[i+1][i+1][j-1] - A[i][i][j] + B[j][i] * 1;
        }
    }
#pragma endscop
...
kernel_31242112141_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
