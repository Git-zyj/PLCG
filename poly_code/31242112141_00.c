...
void kernel_31242112141_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_N; i++) {
        for (int j = 0; j < PB_N-1; j++) {
            for (int k = 0; k < PB_N; k++) {
                A[k][j][i] = A[j-1][i][k+1] * A[i][k+1][j] - B[j][i][k+1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_31242112141_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
