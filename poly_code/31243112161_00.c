...
void kernel_31243112161_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M-1; i++)
        for (int j = 0; j < PB_N; j++)
            A[i][i] = A[i-1][i-1] + A[i+1][i+1] + B[j][i-1] + 6;
#pragma endscop
...
kernel_31243112161_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
