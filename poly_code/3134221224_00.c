...
void kernel_3134221224_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
for (int i = 0; i < PB_P; i++) {
    for (int j = 2; j < i; j++) {
        A[j-1] = A[j-2] + A[j-1] - B[j][i] - B[i+1][i] * 1;
    }
}
#pragma endscop
...
kernel_3134221224_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
