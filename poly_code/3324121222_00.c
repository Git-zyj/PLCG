...
void kernel_3324121222_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa)){
polybench_start_instruments;
#pragma scop
for (int i = 2; i < PB_L-1; i++) {
    A[i] = A[i] + A[i] + 2;
    A[i] = A[i-2] + A[i-1] * A[i+1] * A[i] + 6;
}
...
kernel_3324121222_00(xa,POLYBENCH_ARRAY(A));
