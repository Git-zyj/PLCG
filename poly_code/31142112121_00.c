...
void kernel_31142112121_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_Q-1; i++)
        for (int j = 1; j < PB_Q+1; j++)
            for (int k = 1; k < PB_Q+1; k++)
                A[i][k][j-1][i] = B[i][k+1] + A[i+1][j][k-1][i+1] + A[i][j-1][k-1][i] * 2;
#pragma endscop
...
kernel_31142112121_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
