...
void kernel_32241112141_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M-1; i++) {
        for (int j = 0; j < i; j++) {
            A[i][j] = A[i+1][j] - A[i-1][j] - 3;
        }
        B[i] = B[i+1] * B[i-1] - C[i] * C[i-1] - 4;
    }
#pragma endscop
...
kernel_32241112141_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
