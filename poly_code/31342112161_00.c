...
void kernel_31342112161_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_N-1; i++)
        for (int j = 1; j < i; j++)
            for (int k = 2; k < PB_M; k++)
                A[i][k-1][j] = B[k][k-1][j-1] * A[i-1][k][j+1] - A[i+1][k-2][j] + 3;
#pragma endscop
...
kernel_31342112161_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
