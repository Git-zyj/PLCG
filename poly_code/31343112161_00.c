...
void kernel_31343112161_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_L-1; i++)
        for (int j = 1; j < PB_L-1; j++)
            A[j][i] = B[j-1][0] + A[i][j+1] + A[j+1][i+1] + 5;
#pragma endscop
...
kernel_31343112161_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
