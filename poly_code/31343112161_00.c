...
void kernel_31343112161_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_L-2; i++) {
        for (int j = 0; j < PB_P; j++) {
            for (int k = 1; k < PB_P; k++) {
                A[k][j+1][i+1] = A[k-1][i+2][j+1] + A[i][j][k+2] * B[i][j][k] - 4;
            }
        }
    }
#pragma endscop
...
kernel_31343112161_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
