...
void kernel_3114121226_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
for (int i = 0; i < PB_P; i++) {
    for (int j = 1; j < i; j++) {
        for (int k = 1; k < PB_P; k++) {
            A[j-1][k] = A[j-1][k] - A[j-1][k] - B[j+1][k-1][i] - B[i][j][k] - 1;
        }
    }
}
#pragma endscop
...
kernel_3114121226_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
