...
for (int i = 2; i < PB_M-1; i++) {
    for (int j = 1; j < PB_M-1; j++) {
        for (int k = 1; k < PB_M; k++) {
            A[i][k][j+1] = A[i-1][k-1][j+1] + A[j+1][k-1][k-1] - A[j][i-1][j-1] * 6;
            A[j][k-1][k] = A[i-2][k+1][j+1] + A[k][i][j+1] + A[k-1][j+1][i] - 3;
        }
    }
    B[i-1] = B[i-2] * C[i+1] + 5;
}
for (int l = 1; l < PB_M-1; l++) {
    for (int m = 0; m < l; m++) {
        D[l][l] = E[l+1][m] - F[l][l] - 3;
    }
    for (int n = 0; n < l; n++) {
        for (int o = 3; o < PB_L+1; o++) {
            D[o-1][l+1] = D[o-1][l-1] - D[o-3][l+1] + 6;
        }
    }
    C[l] = 5;
}
