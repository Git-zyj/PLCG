...
void kernel_3334221224_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
for (int j = -1; j < PB_M-1; j++) {
    for (int k = 0; k < min(j,PB_L); k++) {
        for (int l = 1; l < PB_L-1; l++) {
            B[l-1][j+1][k] = B[l][k][l+1] + 6;
        }
    }
}
for (int m = -1; m < PB_L-1; m++) {
    for (int n = 3; n < PB_Q+1; n++) {
        for (int o = 0; o < min(n,PB_P-2); o++) {
            C[m+1][o][n-1] = C[m+1][o+2][n-3] - C[m+1][o][n-1] + 1;
        }
    }
}
#pragma endscop
...
kernel_3334221224_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
