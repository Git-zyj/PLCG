...
void kernel_32343112161_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M; i++)
        A[i] = A[i-1] * C[i-1] - C[i-1] + 6;
    for (int j = 0; j < PB_M-1; j++)
        for (int k = 1; k < min(j,PB_L-1); k++)
            for (int l = 1; l < k; l++)
                B[j][k-1][l] = B[j][k][l-1] + B[j][l-1][k+1] - B[j+1][k][l-1] * 4;
#pragma endscop
...
kernel_32343112161_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
