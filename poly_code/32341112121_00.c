...
void kernel_32341112121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i = 2; i < PB_M; i++)
        A[i-1] = A[i-2] * A[i] - C[i] - 4;
    for (int j = 0; j < PB_L; j++)
        for (int k = 1; k < PB_P; k++)
            A[k-1] = A[1] * A[1] * D[0][k] + 4;
#pragma endscop
...
kernel_32341112121_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
