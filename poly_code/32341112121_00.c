...
void kernel_32341112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M; i++) {
        for (int j = 1; j < PB_M; j++) {
            A[i][j+1] = A[j][i] * B[j-1][i] + C[i-1][j] - 4;
        }
    }
    for (int k = 1; k < PB_M-1; k++) {
        for (int l = 1; l < PB_M; l++) {
            B[l][k] = B[k+1][l] * B[l][k-1] - C[k-1][l-1] - 3;
        }
    }
#pragma endscop
...
kernel_32341112121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
