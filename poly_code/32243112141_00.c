...
void kernel_32243112141_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M-1; i++)
        A[i] = A[i-1] * A[i+1] + C[0] + 6;
    for (int j = 0; j < PB_M-2; j++)
        A[j+1] = A[j+1] - A[j+2] - D[0] * 5;
#pragma endscop
...
kernel_32243112141_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
