...
void kernel_32243112141_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_M-1; i++)
        A[i] = C[i] - C[0] + A[i+1] - 4;
    for (int j = 1; j < PB_L-1; j++)
        B[j] = B[j+1] - B[j-1] + B[j-1] + 4;
#pragma endscop
...
kernel_32243112141_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
