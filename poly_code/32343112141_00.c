...
void kernel_32343112141_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int j = 0; j < PB_M; j++) {
        for (int k = 1; k < j; k++) {
            for (int l = 1; l < PB_M; l++) {
                B[l-1][j+1][k] = B[l-1][j+1][k-1] + B[k-1][j+2][l] + B[l][k][j] * C[j][l-1][k] * D[k][j][l-1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_32343112141_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
