...
void kernel_31243112141_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_M; i++)
        for (int j = 2; j < min(i,PB_M-1); j++)
            for (int k = 0; k < PB_M; k++)
                A[j][j-1] = A[j-1][j-2] * A[j+1][j] + B[k][i][i+1] + 1;
#pragma endscop
...
kernel_31243112141_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
