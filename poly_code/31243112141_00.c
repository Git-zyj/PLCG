...
void kernel_31243112141_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_L; i++) {
        for (int j = 1; j < PB_Q; j++) {
            for (int k = 1; k < min(i,PB_L-1); k++) {
                A[i][j][k] = A[k+1][j+1][i] * A[k-1][i][j] + B[i+1][j-1][0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_31243112141_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
