...
void kernel_31341112161_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_P-1; i++) {
        for (int j = 1; j < PB_P; j++) {
            for (int k = 1; k < j; k++) {
                A[k][i-1][j][k-1] = A[k][j][i][k-1] - A[k][j-1][i+1][k-1] - B[i-1][j][j] - 4;
            }
        }
    }
#pragma endscop
...
kernel_31341112161_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
