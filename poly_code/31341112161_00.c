...
void kernel_31341112161_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_M-2; i++)
        for (int j = 1; j < PB_M-2; j++)
            A[i+1][j] = B[j][j-1] * A[j+2][i] * A[i+2][j-1] + 1;
#pragma endscop
...
kernel_31341112161_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
