...
void kernel_31241112121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M; i++) {
        for (int j = 0; j < i; j++) {
            for (int k = 2; k < min(j,PB_P); k++) {
                A[j][k][k-1] = A[i][k-1][k-2] * A[j][k+1][k] - B[i-1][k][i-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_31241112121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
