...
void kernel_31241112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_Q; i++)
        for (int j = 2; j < i; j++)
            A[i-1][j-1] = A[i-1][j-2] + A[i][j-1] * B[j+1][i] + 6;
#pragma endscop
...
kernel_31241112121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
