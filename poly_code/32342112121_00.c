...
void kernel_32342112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_N-1; i++)
        for (int j = 1; j < i; j++)
            A[i][0] = A[i-1][0] + A[i+1][0] - C[i+1][j] * D[j-1][i+1] - 1;
    for (int k = 2; k < PB_M; k++)
        A[k-1][0] = A[k-2][0] * A[k][0] + 5;
#pragma endscop
...
kernel_32342112121_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
