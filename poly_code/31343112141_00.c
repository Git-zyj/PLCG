...
void kernel_31343112141_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 2; i < PB_M; i++)
        for (int j = 1; j < PB_Q-1; j++)
            for (int k = 2; k < j; k++)
                A[k][i-1][j][k-1] = A[k][i-1][j+1][k-1] - A[k-1][i-2][j][k-2] - B[j-1][i][k] - 4;
#pragma endscop
...
kernel_31343112141_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
