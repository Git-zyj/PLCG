...
void kernel_31343112141_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M+1; i++) {
        for (int j = 1; j < PB_M; j++) {
            A[j][i] = A[i-1][j] + A[j-1][i] * B[j-1] * 4;
        }
    }
#pragma endscop
...
kernel_31343112141_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
