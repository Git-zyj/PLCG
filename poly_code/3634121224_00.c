...
for (int i = 0; i < PB_L-1; i++) {
    A[i] = 1;
    for (int j = 0; j < PB_Q-1; j++) {
        for (int k = 0; k < PB_Q-1; k++) {
            B[i+1][k][i] = B[k][j][i+1] * B[k][k+1][j+1] - 1;
        }
    }
}
for (int l = 0; l < PB_M-1; l++) {
    for (int m = 1; m < PB_M; m++) {
        A[l+1] = A[m-1] * A[m-1] + C[l][m] * C[l][m] + 4;
        A[m-1] = A[m-1] + 6;
    }
}
for (int n = 1; n < PB_M-2; n++) {
    for (int o = 1; o < PB_M; o++) {
        for (int p = 0; p < n; p++) {
            B[o-1][n][o] = B[o-1][n][o-1] * 4;
        }
        C[n][n-1] = C[n+2][n-1] + C[n][n-1] * C[n-1][o] - A[o] * 6;
    }
}
#pragma endscop
