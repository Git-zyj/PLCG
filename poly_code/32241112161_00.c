...
void kernel_32241112161_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_N-2; i++) {
        for (int j = 1; j < PB_P; j++) {
            for (int k = 1; k < PB_P; k++) {
                A[i+1][k-1][j] = A[i][j][k] * B[j][i][k] - A[i+2][j-1][k] - C[k][j][j] - 3;
            }
        }
    }
    for (int l = 1; l < PB_M; l++) {
        for (int m = 0; m < PB_L-1; m++) {
            for (int n = 0; n < PB_P; n++) {
                B[l][m+1][n] = B[l-1][m+1][n] * C[m][l][n] * 6;
            }
        }
    }
#pragma endscop
...
kernel_32241112161_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
