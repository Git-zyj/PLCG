...
void kernel_23242212242_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
// 预计算通用边界
const int sqrt_PB_L = (int)sqrt(PB_L);
const int i_max = min(sqrt_PB_L, 7);
const int m_max = min(sqrt_PB_L, 7);

// 修改后的代码
for (int i = i_max; i >= 0; i--) {
    for (int j = 0; j <= (PB_L - 1 - i) / (1 + i); j++) {
        if (j > 10) {
            A[i + j + i*j][i*i] = A[i + j + i*j][i*i] + A[i][j*j] * A[j][j + j*j] * 6; // 保证不越界
        }
    }
    for (int k = 0; k < i; k++) {
        B[i][i + k + k*k] = 1; // 保证不越界（i≤7时i + k + k*k ≤49）
    }
}
for (int l = 0; l < PB_L; l++) {
    for (int m = 0; m <= m_max; m++) {
        A[m + m*m][m] = 4; // 保证不越界（m≤7时m + m*m ≤56）
    }
}
#pragma endscop
...
kernel_23242212242_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
