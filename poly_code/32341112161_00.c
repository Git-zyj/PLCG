...
void kernel_32341112161_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i = 0; i < PB_L-1; i++) {
        for (int j = 0; j < i; j++) {
            for (int k = 1; k < PB_M+1; k++) {
                A[i][j][k] = A[j][i+1][k] + A[k-1][i][j] - C[i][i+1][k-1] - 1;
            }
        }
    }
    for (int l = 0; l < PB_L; l++) {
        for (int m = 0; m < l; m++) {
            B[m][m] = B[l][l] + B[l][l] + D[m][l] * 6;
        }
    }
#pragma endscop
...
kernel_32341112161_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
