...
void kernel_35141112141_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M-1; i++)
        A[i] = 5;
        for (int j = 1; j < PB_P-1; j++)
            B[i][j] = F[i+1][1] * A[i] - B[i][j-1] + B[i+1][j+1] + B[i-1][j] * B[i][j+1] - 5;
        for (int k = 0; k < i; k++)
            for (int l = 0; l < i; l++)
                A[l] = F[l][0] + F[l][k] + F[l+1][k] * 1;
...
kernel_35141112141_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
