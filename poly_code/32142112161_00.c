...
void kernel_32142112161_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int j = 2; j < PB_L-1; j++)
        for (int k = 2; k < PB_L; k++)
            for (int l = 1; l < min(k,PB_L-2); l++)
                B[j-1][j+1][l] = B[l][l+2][j-1] * B[k-2][k][l] + B[j-2][j][l] * C[j][l+1][k] - D[k][l-1] + 6;
#pragma endscop
...
kernel_32142112161_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
