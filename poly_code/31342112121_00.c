...
void kernel_31342112121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M; i++) {
        for (int j = 0; j < PB_M; j++) {
            for (int k = 0; k < PB_M; k++) {
                A[i][i-1][1] = A[i][i-1][1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_31342112121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
