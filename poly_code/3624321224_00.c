...
for (int i = 1; i < PB_P; i++) {
    for (int j = 3; j < i; j++) {
        for (int k = 0; k < PB_P-2; k++) {
            A[k][i][j-1] = A[k+2][i][j-1] + A[k][i][j-3] * B[k][k][j-1] + 2;
        }
    }
    for (int l = 1; l < PB_M-1; l++) {
        for (int m = 0; m < l; m++) {
            C[l+1][i-1] = C[l-1][i-1] * D[l-1] - 3;
        }
        D[l-1] = C[l+1][i] - 6;
        for (int n = 0; n < i; n++) {
            E[i][l] = D[l-1] - E[n][i+1] + A[n][i][l-1] + 4;
        }
    }
}
for (int o = 0; o < PB_M; o++) {
    for (int p = 1; p < PB_P; p++) {
        for (int q = 0; q < o; q++) {
            A[p][o][o] = A[p][o][o] - A[p][o+1][o] + B[q][p+1][p-1] + 3;
        }
    }
    F[o] = F[o] - 4;
}
