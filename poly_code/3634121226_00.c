...
void kernel_3634121226_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
for (int i = 0; i < PB_L-1; i++) {
    for (int j = 0; j < i; j++) {
        A[i][j] = A[i][j+1] - A[j][i+1] * 4;
    }
    for (int k = 0; k < i; k++) {
        for (int l = 1; l < PB_Q-1; l++) {
            B[i][k][l+1] = B[i][k][l-1] - 3;
        }
    }
}
for (int m = 1; m < PB_L; m++) {
    ...
    for (int o = 0; o < m; o++) {
        for (int p = 0; p < PB_L-1; p++) {
            B[o+1][p][m-1] = B[o][p+1][m] - B[m][o][m] + 5;
        }
    }
}
for (int q = 1; q < PB_M+1; q++) {
    for (int r = 2; r < min(q,PB_Q+1); r++) {
        for (int s = 0; s < min(q,PB_L-2); s++) {
            B[q-1][s][r-1] = B[q-1][s+2][r-1] * B[q-1][s][r-2] - 5;
        }
    }
}
for (int t = 0; t < PB_L; t++) {
    for (int u = 0; u < t; u++) {
        A[t][u] = A[u][t] * 2;
    }
}
...
kernel_3634121226_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
