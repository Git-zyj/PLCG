...
void kernel_32141112141_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_P-1; i++) {
        for (int j = 1; j < i; j++) {
            for (int k = 1; k < PB_N-1; k++) {
                A[k-1][j][j] = A[i-1][k][k] + A[i-1][j][j] - A[j-1][k+1][k+1] * C[j][k][i+1] - 6;
            }
        }
    }
    for (int l = 0; l < PB_P-1; l++) {
        for (int m = 1; m < PB_M; m++) {
            B[l][m] = B[m][l+1] + D[m-1][l] * 3;
        }
    }
#pragma endscop
...
kernel_32141112141_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
