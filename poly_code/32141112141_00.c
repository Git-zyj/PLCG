...
void kernel_32141112141_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i = 1; i < PB_M-1; i++)
        for (int j = 1; j < PB_M; j++)
            for (int k = 1; k < j; k++)
                A[i+1][k+1][j] = A[i][k+1][j+1] - A[i+1][j+1][k-1] + A[i+1][k+2][j-1] * C[j][i][k] * 5;
        for (int l = 0; l < PB_M; l++)
            for (int m = 1; m < i; m++)
                A[l][m+1][i-1] = A[m][l+1][i] * C[l][m-1][i] - 3;
#pragma endscop
...
kernel_32141112141_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
