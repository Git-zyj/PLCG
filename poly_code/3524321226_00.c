...
for (int i = 0; i < PB_L-1; i++) {
    for (int j = 0; j < min(i,PB_P-1); j++) {
        for (int k = 0; k < PB_M; k++) {
            A[k][i+1][j] = A[k][i+1][j+1] - 6;
        }
    }
    B[i] = B[i] * 5;
}
for (int l = 2; l < PB_P-2; l++) {
    for (int m = 0; m < PB_L; m++) {
        for (int n = 1; n < PB_P+1; n++) {
            C[l][n-1][l] = C[l][n-1][l] - C[l-2][n-1][l] * C[m][n-1][l+1] - 4;
        }
    }
    for (int o = 2; o < PB_L; o++) {
        for (int p = 1; p < PB_M+1; p++) {
            A[p-1][o][l] = A[p-1][o-2][l] + A[p-1][o][l+2] + 5;
        }
    }
}
for (int q = 1; q < PB_N-1; q++) {
    for (int r = 0; r < PB_M-2; r++) {
        D[q+1][r] = D[q-1][r+2] * E[r][r] - 2;
    }
}
#pragma endscop
