...
for (int l = 2; l < PB_M-1; l++) {
    for (int m = 0; m < min(l,PB_M-2); m++) {
        B[l][m] = B[l-1][m+2] - C[l-1] * 5;
    }
    for (int n = 1; n < l; n++) {
        D[n-1] = D[n-1] * D[n-1] + B[l][l+1] * 2;
        for (int o = 1; o < PB_Q+1; o++) {
            E[n-1][l][o-1] = D[n-1] + E[n][l-2][o-1] + 1;
        }
    }
}
for (int p = 1; p < PB_M; p++) {
    ...
    for (int r = 1; r < p; r++) {
        for (int s = 0; s < PB_M-1; s++) {
            E[s+1][p][r-1] = E[s+1][p+2][r-1] - A[p-1][p] - F[r][r][s] + 3;
        }
    }
}
#pragma endscop
