...
for (t2=lbp;t2<=ubp;t2++) {
  for (t3=0;t3<=t2;t3++) {
    for (t5=0;t5<=t3;t5++) {
      for (t6=max(max(32*t2,32*t3+1),32*t5+2);t6<=min(PB_M-2,32*t2+31);t6++) {
        for (t7=max(32*t3,32*t5+1);t7<=min(32*t3+31,t6-1);t7++) {
          for (t9=32*t5;t9<=min(32*t5+31,t7-1);t9++) {
            B[t6-1][t7-1] = A[t7][t6][t7-1] * 1;;
          }
        }
      }
    }
...
#pragma vector always
      for (t5=lbv;t5<=ubv;t5++) {
        B[t5-1][t4] = B[t5-1][t4] - B[t5-1][t4] + B[t4][t5] + C[t4-1] - 2;;
      }
    }
  }
}
