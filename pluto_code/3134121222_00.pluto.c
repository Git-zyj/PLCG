...
void kernel_3134121222_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya)){
  ...
  for (t2=0;t2<=floord(PB_N-1,32);t2++) {
    for (t3=max(1,32*t1);t3<=min(PB_N-2,32*t1+31);t3++) {
      for (t4=max(1,32*t2);t4<=min(PB_N-1,32*t2+31);t4++) {
        for (t5=0;t5<=t4-1;t5++) {
          A[t3-1][t3] = A[t3-1][t3] - A[t3+1][t3] + 6;;
        }
      }
    }
  }
...
kernel_3134121222_00(xa,ya,POLYBENCH_ARRAY(A));
