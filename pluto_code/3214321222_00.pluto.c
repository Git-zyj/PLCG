...
for (t1=lbp;t1<=ubp;t1++) {
  for (t2=0;t2<=floord(PB_M-1,32);t2++) {
    for (t3=0;t3<=min(min(min(floord(PB_Q-1,32),floord(-800*t1+25*PB_M-25,198)),floord(-800*t1+25*PB_P+2500,202)),floord(-3192*t1+25*PB_Q+29900,800));t3++) {
      if (t3 == 0) {
        for (t4=32*t1;t4<=min(PB_P-1,32*t1+31);t4++) {
          for (t5=max(1,32*t2);t5<=min(PB_M-1,32*t2+31);t5++) {
            B[t5-1][t4+1][t4] = C[t5][t4][0] + 6;;
            A[t5][t4] = A[t5][t4] * A[t5-1][t4] - 2;;
            for (t6=1;t6<=31;t6++) {
              B[t5-1][t4+1][t4] = C[t5][t4][t6] + 6;;
            }
          }
        }
      }
      if (t3 == 0) {
        for (t4=max(PB_P,32*t1);t4<=min(PB_M-1,32*t1+31);t4++) {
          for (t5=max(1,32*t2);t5<=min(PB_M-1,32*t2+31);t5++) {
            A[t5][t4] = A[t5][t4] * A[t5-1][t4] - 2;;
          }
        }
