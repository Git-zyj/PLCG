...
void kernel_3124121222_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
for (t1=lbp;t1<=ubp;t1++) {
  for (t2=0;t2<=floord(PB_L-2,32);t2++) {
    for (t3=max(t1,t2);t3<=floord(PB_P-1,32);t3++) {
      for (t4=32*t1;t4<=min(min(PB_L-1,32*t1+31),32*t3+30);t4++) {
        for (t5=max(1,32*t2);t5<=min(min(PB_L-2,32*t2+31),32*t3+30);t5++) {
          for (t6=max(max(32*t3,t4+1),t5+1);t6<=min(PB_P-1,32*t3+31);t6++) {
            A[t5+1][t5-1][t4] = A[t5][t5-1][t4] + A[t4][t4][t5-1] - B[t4][t6] - 6;;
          }
        }
      }
    }
...
kernel_3124121222_00(xa,ya,za,POLYBENCH_ARRAY(A),xb,yb,POLYBENCH_ARRAY(B));
