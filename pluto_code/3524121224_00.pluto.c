...
void kernel_3524121224_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
for (t2=2;t2<=PB_L-3;t2++) {
  A[t2] = A[t2-2] * A[t2-2] - 2;;
  A[t2] = A[t2+2] + 2;;
  A[t2] = A[t2] + A[t2] * A[t2] + 1;;
}
lbp=0;
...
kernel_3524121224_00(xa,POLYBENCH_ARRAY(A),xb,yb,POLYBENCH_ARRAY(B));
