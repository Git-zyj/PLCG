...
void kernel_3534121226_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
for (t2=lbp;t2<=ubp;t2++) {
  for (t3=0;t3<=min(floord(PB_P-1,32),t2);t3++) {
    for (t4=max(32*t2,32*t3+1);t4<=min(PB_N-3,32*t2+31);t4++) {
      for (t5=32*t3;t5<=min(min(PB_P-1,32*t3+31),t4-1);t5++) {
        C[t4+2] = A[t4+1][t5] - 2;;
      }
    }
  }
...
for (t2=lbp;t2<=ubp;t2++) {
  for (t3=0;t3<=t2;t3++) {
    for (t4=0;t4<=floord(PB_M-2,32);t4++) {
      for (t5=max(max(3,32*t2),32*t3+1);t5<=min(PB_P-1,32*t2+31);t5++) {
...
for (t2=0;t2<=floord(3*PB_P-17,32);t2++) {
  lbp=max(ceild(t2,2),ceild(32*t2-PB_P+5,32));
  ubp=floord(2*t2+1,3);
#pragma omp parallel for private(lbv,ubv,t4,t5,t6,t7)
  for (t3=lbp;t3<=ubp;t3++) {
    for (t4=max(max(2,32*t2-32*t3),16*t3+1);t4<=min(min(PB_P-5,32*t3+30),32*t2-32*t3+31);t4++) {
      for (t5=max(32*t3,t4+1);t5<=min(32*t3+31,2*t4-1);t5++) {
        A[(-t4+t5)+1][t4+2] = A[(-t4+t5)+1][t4+4] * A[(-t4+t5)-1][t4-1] - 4;;
        A[(-t4+t5)][t4] = A[(-t4+t5)-1][t4+2] - 6;;
      }
    }
  }
}
...
kernel_3534121226_00(xa,ya,POLYBENCH_ARRAY(A),xb,yb,zb,POLYBENCH_ARRAY(B),xc,POLYBENCH_ARRAY(C));
