...
void kernel_3414121226_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
...
for (t2=1;t2<=PB_M-2;t2++) {
  for (t3=0;t3<=t2-1;t3++) {
    B[t2-1][t3+1] = A[t2] + B[t3][t2] * 2;;
  }
  for (t3=t2;t3<=2*t2-1;t3++) {
    A[t2] = A[t2] + A[t2-1] - A[(-t2+t3)] - A[t2] + 5;;
  }
  A[t2] = A[t2+1] * 3;;
}
for (t3=0;t3<=min(PB_L-2,PB_M-2);t3++) {
  B[(PB_M-1)-1][t3+1] = A[(PB_M-1)] + B[t3][(PB_M-1)] * 2;;
}
/* End of CLooG code */
...
kernel_3414121226_00(xa,POLYBENCH_ARRAY(A),xb,yb,POLYBENCH_ARRAY(B));
