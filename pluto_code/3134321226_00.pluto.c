...
void kernel_3134321226_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
...
for (t1=lbp;t1<=ubp;t1++) {
  for (t2=0;t2<=t1;t2++) {
    for (t3=max(32*t1,32*t2+1);t3<=min(PB_N-1,32*t1+31);t3++) {
      for (t4=32*t2;t4<=min(32*t2+31,t3-1);t4++) {
        A[t3][t3] = A[t3][t3] - A[t3][t3] * B[t4] + A[t3][t4+1] * 1;;
      }
    }
  }
}
...
kernel_3134321226_00(xa,ya,POLYBENCH_ARRAY(A),xb,POLYBENCH_ARRAY(B));
