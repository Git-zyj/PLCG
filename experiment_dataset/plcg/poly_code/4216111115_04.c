...
void kernel_4216111115_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_N-1; i_3++) {
            B[i_2][i_3-1] = C[i_3+i_2+1][i_3+1] + B[i_2+1][i_3-1] - 6;
        }
    }
#pragma endscop
...
kernel_4216111115_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
