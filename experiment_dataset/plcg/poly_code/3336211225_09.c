...
void kernel_3336211225_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                B[i_2][i_3] = B[i_3+1][i_2] + B[i_4][i_3-1] + B[i_2-2][i_3] * 6;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
            C[i_6] = D[i_5][i_6] * C[i_6+1] + C[i_6-1] * 5;
        }
    }
#pragma endscop
...
kernel_3336211225_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
