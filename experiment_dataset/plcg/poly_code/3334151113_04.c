...
void kernel_3334151113_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_M-1); i_1++) {
            A[i_1+1] = D[i_1] - D[i_1] * E[i_1-1][i_0] * E[i_1][i_0] + E[i_1+1][i_0-1] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_L, i_2); i_3++) {
            B[i_2][i_3] = E[i_2][i_3] + E[i_3][i_2-1] * E[i_2][i_3+1] - E[i_2][i_3+i_2] * D[i_3] + 6;
        }
        C[i_2-1] = D[0] + D[i_2-1] * D[i_2] + D[i_2+1] + A[i_2+1] - 1;
    }
#pragma endscop
...
kernel_3334151113_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
