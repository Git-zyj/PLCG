...
void kernel_2426332211_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-3; i_0++) {
        A[i_0+2] = E[i_0] - E[i_0-1] - F[i_0] * A[i_0+3] - A[i_0+1] + 4;
    }
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        B[i_1+1] = G[i_1][0] * H[i_1+2] + A[i_1+2] - 3;
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        C[i_2+2] = E[i_2] - I[i_2][0] * E[i_2+1] * E[i_2-1] + D[i_2] * D[i_2-1] * D[i_2+1] * 1;
        D[i_2] = H[i_2] + F[i_2] + F[i_2] - 6;
    }
#pragma endscop
...
kernel_2426332211_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
