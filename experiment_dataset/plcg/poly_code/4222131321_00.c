...
void kernel_4222131321_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_0][i_2][i_1] = C[i_1][i_0][i_2+1] - C[i_2][0][i_0] * C[i_0-1][i_2][i_1] * C[i_2][i_0][i_1-1] + 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-3; i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                B[i_5][i_4+1][i_3] = C[i_4][i_3][i_5-1] - B[i_3-1][i_4+2][i_5+1] + B[i_5][i_3-1][i_4] + B[i_5][i_4+2][i_3-2] - B[i_5][i_4+3][i_3] - B[i_3][i_4][i_5] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4222131321_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
