...
void kernel_3324252125_01(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M+1; i_2++) {
                C[i_2][i_0] = D[i_1-2][i_2+i_0-1][i_1] - E[i_0][i_1][i_2][i_1] - F[i_2][i_1][i_0+1] + G[i_2][i_1-1] + 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                C[i_5+1][i_4] = H[i_4][i_5-1] - G[i_3][i_4] * F[i_3+1][i_4][i_5] - H[i_3][i_4-2] + H[i_5][i_4] - 4;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_M; i_6++) {
        for (int i_7 = i_6; i_7 < -i_6+PB_M; i_7++) {
            C[i_6][i_7] = D[i_6][i_7][i_6-2] + G[i_7-1][i_6] - H[i_7+i_6][0] + G[i_6][i_7] - H[i_6][i_7+i_6] * C[i_6][i_7-2] - 6;
        }
    }
#pragma endscop
...
kernel_3324252125_01(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
