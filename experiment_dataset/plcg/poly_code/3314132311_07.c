...
void kernel_3314132311_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_Q-2, -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
                A[i_0][i_1+i_0][i_1+2] = D[i_0][i_1][i_2] - D[i_2][i_1][i_0+1] - E[i_2+2][i_0] + A[i_0-1][i_1+i_0-1][i_1+2] * 2;
            }
        }
        for (int i_3 = 1; i_3 < i_0; i_3++) {
            for (int i_4 = 0; i_4 < PB_Q-1; i_4++) {
                B[i_4+1][i_3][i_0] = D[i_3][i_0][i_4+1] * E[i_4][i_3-1] + B[i_0+1][i_3][i_4+1] * B[i_4+1][i_3-1][i_0] + 3;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = 1; i_6 < PB_M-2; i_6++) {
            for (int i_7 = 2; i_7 < -i_5+PB_M; i_7++) {
                C[i_6][i_5][i_7+1] = E[i_5][i_7] * D[i_6+2][i_7+i_5][i_5] * D[i_5][i_7-2][i_6] * C[i_6][i_5+1][i_7+1] + C[i_5-1][i_6-1][i_7+1] + C[i_7][i_6][i_5] * C[i_6-1][i_5][i_7+1] + C[i_6][i_5][i_7] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3314132311_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
