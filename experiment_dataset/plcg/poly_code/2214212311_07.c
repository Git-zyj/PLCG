...
void kernel_2214212311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
        B[i_1][i_1+1] = A[i_1+1] * B[i_1+1][i_1+2] + B[i_1-1][i_1] - 6;
    }
#pragma endscop
...
kernel_2214212311_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
