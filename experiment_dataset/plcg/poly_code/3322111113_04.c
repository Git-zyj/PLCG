...
void kernel_3322111113_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                A[i_1-1][i_0-1] = D[i_2][i_0] + A[i_2-2][i_0-1] - 5;
            }
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            A[i_3-1][i_0-2] = D[i_3+1][i_0+1] + 6;
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                C[i_5][i_6][i_4] = E[i_4][i_5][0] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3322111113_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
