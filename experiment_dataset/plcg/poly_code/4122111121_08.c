...
void kernel_4122111121_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 1; i_3 < PB_M; i_3++) {
                    A[i_3][i_1][i_2][i_0] = B[i_3][i_1][i_1] - A[i_3-1][i_1][i_2][i_0-2] * 6;
                }
            }
        }
    }
...
kernel_4122111121_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
