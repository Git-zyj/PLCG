...
void kernel_4236212321_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_0][i_2][i_1] = A[i_2][i_0][i_1-1] * A[i_0][i_2][i_1+1] * A[i_0][i_2][i_1-1] * 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < PB_N; i_5++) {
                for (int i_6 = 0; i_6 < min(min(PB_P-2, -i_5+PB_N), i_3); i_6++) {
                    B[i_3][i_5][i_6+2] = C[i_6+i_5][i_3-1][i_4] + A[i_6][i_4][i_5] * B[i_6][i_5][i_4] * 6;
                }
            }
        }
    }
...
kernel_4236212321_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
