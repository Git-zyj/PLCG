...
void kernel_4232111213_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0-1] = A[i_0] * A[i_0-2] * A[i_0] * 5;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            for (int i_3 = 1; i_3 < PB_M; i_3++) {
                for (int i_4 = 0; i_4 < min(min(-i_2+PB_L+1, PB_P), -i_3+PB_L+1); i_4++) {
                    B[i_4+i_2-1][i_3][i_3] = C[i_2][i_3][0] - B[i_4+i_3-1][i_1+1][i_1+1] - 3;
                }
            }
        }
    }
...
kernel_4232111213_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
