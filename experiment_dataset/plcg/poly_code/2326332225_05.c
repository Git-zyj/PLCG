...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        ...
        B[i_0] = D[i_0] + D[i_0] - E[i_0] * B[i_0-1] * 2;
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_N, -i_2+PB_M+1); i_3++) {
            B[i_3] = F[i_2][i_3] - G[i_3][i_2] + H[i_2][i_2] + I[i_2][i_3+1][i_2] - G[i_3+i_2-1][1] - 2;
        }
    }
#pragma endscop
...
kernel_2326332225_05(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
