...
void kernel_3322112321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 4; i_1 < PB_M-2; i_1++) {
        B[i_1-2] = D[i_1+2] + D[i_1-2] - B[i_1-4] + B[i_1-1] + 2;
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            C[i_3][i_2] = D[i_2+1] - B[i_3-1] + C[i_3][i_2-1] + C[i_3+2][i_2] * 3;
        }
    }
#pragma endscop
...
kernel_3322112321_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
