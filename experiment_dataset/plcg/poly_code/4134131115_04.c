...
void kernel_4134131115_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(min(PB_M, i_0), -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_L+1, PB_M-1); i_2++) {
                A[i_1][i_0][i_2+i_1] = B[i_2][i_0][i_1+i_0] - C[i_1][i_1] - B[i_2+1][i_2+i_0-1][i_1] - A[i_1][i_0][i_2+i_1-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4134131115_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
