...
void kernel_3224211321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_L; i_1++) {
            A[i_0-1][i_1-1] = A[i_0-1][i_1-3] - A[i_0][i_1-1] + B[i_0][i_1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 1; i_4 < PB_M; i_4++) {
                B[i_4][i_3] = C[i_4][i_3+1] - A[i_3-1][i_2-1] * B[i_4-1][i_3] - B[i_3+2][i_4+2] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3224211321_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
