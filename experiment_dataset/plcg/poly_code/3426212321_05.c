...
void kernel_3426212321_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
    ...
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                B[i_2-1][i_1+1] = B[i_1-1][i_2-1] - B[i_0-3][i_1-1] * B[i_2-2][i_1+1] * B[i_2][i_1+1] - 3;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            D[i_3-2] = E[i_4][i_3-1] + D[i_4-2] * 1;
        }
        D[i_3-2] = D[i_3] * D[i_3-3] + 5;
    }
#pragma endscop
...
kernel_3426212321_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
