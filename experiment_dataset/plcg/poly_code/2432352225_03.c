...
void kernel_2432352225_03(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            C[i_1][i_0] = E[i_0-2][i_1+2][0] * F[i_1+i_0][0] + G[i_1][i_1] * H[i_1][i_0-1] + H[i_0][i_1] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_P; i_3++) {
            C[i_2+1][i_3+2] = I[i_3] - G[i_2+1][i_3+i_2] + H[i_2-2][i_3] * H[i_3][i_2] * J[i_3] * 5;
        }
        for (int i_4 = 2; i_4 < PB_P+2; i_4++) {
            C[i_2][i_4] = I[i_4] + F[i_4][i_2] * G[i_2][i_4-2] * F[i_4][i_2] * C[i_2][i_4-2] - C[i_2-2][i_4] * 5;
        }
    }
    for (int i_5 = 0; i_5 < PB_P-2; i_5++) {
        D[i_5] = I[i_5] - I[i_5] * G[i_5+1][i_5+2] - D[i_5+2] + 6;
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2432352225_03(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
