...
void kernel_4322352111_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0] - E[i_0] * D[i_0] + F[i_0-2] + A[i_0-1] + C[i_0] * 3;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            C[i_2] = G[i_1][i_2] * H[i_2][i_1-2] - I[i_2+i_1][i_1][i_2] - G[i_2][i_1-1] - 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        C[i_3] = D[i_3-2] * D[i_3] + F[0] + D[i_3] - D[i_3] + D[i_3] + 3;
    }
#pragma endscop
...
kernel_4322352111_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
