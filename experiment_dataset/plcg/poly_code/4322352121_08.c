...
void kernel_4322352121_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        A[i_0+2] = D[i_0] * E[i_0-1] * E[i_0+2] + E[i_0] + E[i_0+2] - B[i_0-2] + 5;
        B[i_0-2] = D[i_0] + F[i_0+2] - E[i_0] - F[i_0-2] * F[i_0] - A[i_0+2] + 2;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < min(PB_Q, -i_1+PB_L); i_2++) {
            for (int i_3 = 0; i_3 < min(min(-i_1+PB_M, PB_P-2), -i_2+PB_M-1); i_3++) {
                C[i_3+i_1][i_2][i_3+2] = G[i_3+i_2+1][i_1][1] + G[i_3+1][i_2][i_1] - H[0][i_2-2] + I[i_3][i_2+i_1][i_2] * J[i_1][i_2-1][i_3] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4322352121_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
