...
void kernel_3222211115_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                A[i_1-1][i_0-1][i_2] = A[i_2-1][i_0-1][i_1-1] + 3;
            }
        }
        for (int i_3 = 1; i_3 < min(PB_M, -i_0+PB_L); i_3++) {
            B[i_3] = C[i_3+i_0][i_3] * B[i_3-1] * 6;
        }
    }
#pragma endscop
...
kernel_3222211115_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
