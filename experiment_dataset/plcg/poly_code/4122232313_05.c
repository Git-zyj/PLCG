...
void kernel_4122232313_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < -i_0+PB_L+1; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_L; i_2++) {
                A[i_0-2][i_1][i_2][i_0] = B[i_2][i_2][i_1] * B[i_2+i_0][i_1][i_2] + B[i_1+i_0-1][i_2][i_0+2] * A[i_1-3][i_0-1][i_2][i_1-1] - A[i_0-2][i_1-1][i_2-1][i_0] * A[i_0-2][i_1+1][i_2][i_0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4122232313_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
