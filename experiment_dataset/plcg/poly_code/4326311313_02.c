...
void kernel_4326311313_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_1+1][i_2+1][i_0] = D[i_2][i_1][i_0-1] * A[i_0+2][i_2+2][i_1+1] - A[i_1][i_2+1][i_0] + 5;
            }
            for (int i_3 = 0; i_3 < i_0; i_3++) {
                for (int i_4 = 1; i_4 < min(-i_0+PB_Q, PB_P-1); i_4++) {
                    B[i_1-1][i_0][i_4][i_3] = E[i_1-1][i_3+1][i_4+1][i_4+i_0] - F[i_3][i_0][i_4+1][i_1+1] * B[i_4-1][i_0][i_1][i_3+1] - B[i_3][i_0][i_4][i_1] * B[i_1-1][i_0+1][i_4][i_3+1] * B[i_1-2][i_0-1][i_4][i_3] + 4;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        for (int i_6 = 1; i_6 < min(i_5, PB_M-1); i_6++) {
            C[i_6][i_5-1] = C[i_6+1][i_5-1] - C[i_6][i_5] - C[i_6-1][i_5-1] * 1;
        }
    }
#pragma endscop
...
kernel_4326311313_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
