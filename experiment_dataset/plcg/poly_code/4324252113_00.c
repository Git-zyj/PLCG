...
void kernel_4324252113_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(-i_0+PB_M-2, PB_L-2); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_P-2, i_0); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_L, PB_L-2); i_3++) {
                    A[i_1+i_0+1][i_2][i_3][i_1-1] = D[i_1][i_3][i_2+1][i_0] + E[i_2-1][i_3+2][i_1][i_0] - F[i_0][i_1+2][i_0][i_3][i_2] - G[i_3+1][i_2][i_1] * E[i_3+i_0][i_1][i_2+2][i_2] + D[i_1][i_2][i_0][i_0] - A[i_1+i_0+2][i_2-1][i_3][i_1] + A[i_1+i_0][i_2][i_3+1][i_1-2] * 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            B[i_4+2][i_4] = H[i_5][0] + I[i_4] - J[i_5][i_4] - C[i_5] - 5;
        }
    }
    for (int i_6 = 0; i_6 < PB_L; i_6++) {
        C[i_6] = K[i_6] - K[0] * I[i_6] * I[i_6] + I[0] + I[i_6] - 6;
    }
#pragma endscop
...
kernel_4324252113_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
