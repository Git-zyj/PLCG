...
void kernel_2412352221_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0] * F[i_0] * G[i_0][i_0] - E[i_0-1] + H[i_0+2] + F[i_0] + A[i_0-1] + A[i_0+1] - 2;
    }
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_L, PB_L-2); i_2++) {
            B[i_2-1][i_1+2] = I[i_2][i_1] - I[i_2][i_2] + J[i_1][i_2] + I[i_2-1][i_2+i_1] * 4;
            D[i_2] = G[i_2][i_1] * J[i_2-2][i_1] * H[0] - J[i_2+2][i_1+1] + B[i_1+1][i_2+2] + 1;
        }
        D[i_1] = E[i_1+2] + H[i_1+1] - F[i_1] * D[i_1+1] * D[i_1-1] - D[i_1+2] + 5;
    }
#pragma endscop
...
kernel_2412352221_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
