...
void kernel_4224351115_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                for (int i_3 = 0; i_3 < min(PB_Q-1, -i_2+PB_M); i_3++) {
                    A[i_0][i_1-1][i_2][i_3] = C[i_2][i_3][i_1+i_0][i_3+1] - D[i_3][i_3][i_0-1][i_2+1] * E[i_3+i_2][i_1-1][i_1] + 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
        B[i_4] = F[i_4] - G[i_4-1] - H[i_4] + G[i_4+1] * F[i_4] - G[0] * G[i_4+1] * B[i_4-1] - 2;
    }
#pragma endscop
...
kernel_4224351115_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
