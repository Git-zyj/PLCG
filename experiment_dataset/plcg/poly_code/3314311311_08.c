...
void kernel_3314311311_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_1][i_0] = A[i_1+1][i_0] * A[i_1-1][i_0] - A[i_1-1][i_0+1] + A[i_0+1][i_1] + 5;
        }
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            for (int i_3 = max(1, i_0); i_3 < PB_P; i_3++) {
                B[i_0+1][i_2+1][i_3] = D[i_2][i_3-1][i_0+1] * B[i_0][i_2+2][i_3] + B[i_3+1][i_2][i_0] - 4;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M+1; i_5++) {
            C[i_5-1][i_4+1] = C[i_5-1][i_4] - 5;
        }
    }
#pragma endscop
...
kernel_3314311311_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
