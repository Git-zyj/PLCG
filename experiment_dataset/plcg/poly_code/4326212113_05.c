...
void kernel_4326212113_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        A[i_0] = D[i_0+2][0] * E[i_0] - 6;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 2; i_2 < PB_P; i_2++) {
            for (int i_3 = i_1; i_3 < PB_P; i_3++) {
                C[i_2][i_3+1] = D[i_1+1][0] + 2;
            }
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                C[i_1-1][i_4] = C[i_2-2][i_4] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4326212113_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
