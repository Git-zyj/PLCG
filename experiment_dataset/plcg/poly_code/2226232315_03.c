...
void kernel_2226232315_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = C[0] + A[i_0+2] + A[i_0] - A[i_0+2] - 3;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_N, -i_1+PB_L); i_2++) {
            A[i_2+2] = D[i_2][i_1+2] - C[i_2] + E[i_1][1] - D[i_1][i_2+i_1] - 5;
        }
    }
#pragma endscop
...
kernel_2226232315_03(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
