...
#endif
static void init_array(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int seed)
{
srand(seed);
int i_0, i_1, i_2, i_3;
...
void kernel_4426311315_08(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_Q-1; i_3++) {
                    D[i_0+1][i_3][i_2+1][i_3+i_1] = E[i_2][i_3-1][i_1] * D[i_0+1][i_3-1][i_2][i_3+i_1+1] - 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        B[i_4] = B[i_4-1] * B[i_4+1] - B[i_4+1] - B[i_4+1] * B[i_4+1] - 3;
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = 0; i_6 < i_5; i_6++) {
            C[i_5][i_6+1] = F[i_5-1][i_6] - C[i_5][i_6+2] + C[i_5-1][i_6+2] - 1;
        }
        for (int i_7 = 0; i_7 < PB_P-1; i_7++) {
            for (int i_8 = 0; i_8 < min(i_5, PB_M-1); i_8++) {
                for (int i_9 = 1; i_9 < min(-i_7+PB_Q-2, -i_8+PB_M-1); i_9++) {
                    D[i_5][i_9][i_8+1][i_9+i_7+1] = G[i_9+i_8+1][i_9-1][i_5-1][i_7+1] - D[i_5][i_9+1][i_8][i_9+i_7+2] * 1;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4426311315_08(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
