...
void kernel_3314312323_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(min(PB_L, -i_1+PB_P-2), -i_0+PB_P); i_2++) {
                B[i_0][i_2+i_1][i_2] = D[i_2][i_0][i_1+1] * B[i_0][i_2+i_1][i_2] + B[i_0][i_2+i_1+2][i_2] - 6;
                B[i_0][i_2+i_1][i_2] = B[i_1-2][i_2+i_0][i_2-1] - 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            C[i_3][i_4] = C[i_3+1][i_4] - C[i_3-1][i_4] - 2;
        }
    }
#pragma endscop
...
kernel_3314312323_07(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
