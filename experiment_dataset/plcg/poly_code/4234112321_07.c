...
void kernel_4234112321_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-4; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                A[i_0][i_2+2][i_1] = C[i_2+i_0][i_1+2][i_0][i_1] * A[i_0-2][i_2+2][i_1-2] - A[i_1-2][i_0+4][i_2] + A[i_0][i_2+2][i_1+2] * A[i_0+2][i_2+2][i_1] * 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_N; i_4++) {
            B[i_3][i_3] = D[i_4][i_3] - B[i_3+2][i_3+2] + 3;
        }
    }
#pragma endscop
...
kernel_4234112321_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
