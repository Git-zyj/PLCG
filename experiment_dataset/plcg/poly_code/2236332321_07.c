...
void kernel_2236332321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_0+1][i_1+i_0+1] = C[i_1][i_0-2] + D[i_1+i_0][i_0] - E[i_1] - A[i_0+1][i_1+i_0] * A[i_0-1][i_1+i_0-3] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(min(-i_2+PB_L-2, i_2), PB_M-2); i_3++) {
            B[i_2][i_3] = D[i_2-1][i_3] + F[i_3][i_2-1] * A[i_3+2][i_3+i_2+2] + B[i_2][i_3-1] * B[i_3][i_2-2] - 2;
        }
    }
#pragma endscop
...
kernel_2236332321_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
