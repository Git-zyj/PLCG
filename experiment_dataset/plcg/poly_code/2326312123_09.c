...
void kernel_2326312123_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = min(-i_1+PB_M, PB_N+2) - 1; i_2 >= 2; i_2--) {
            B[i_2+2][i_1] = E[i_2-2][i_2+i_1] + D[i_1] + C[i_2] - 4;
            C[i_2] = C[i_2+2] * 2;
        }
    }
#pragma endscop
...
kernel_2326312123_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
