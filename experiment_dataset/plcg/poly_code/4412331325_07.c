...
void kernel_4412331325_07(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                    D[i_1-1][i_0+1][i_3] = E[i_3][i_1][i_1+i_0][i_2] * F[i_3][i_1][i_0] - E[i_2][i_0-1][i_3+1][i_2+i_1] * E[i_1+i_0-1][i_2][i_3][1] * 5;
                }
            }
            for (int i_4 = i_1; i_4 < PB_P; i_4++) {
                D[i_1-1][i_0+2][i_4] = G[i_0][i_4-1] + 4;
            }
        }
        for (int i_5 = 1; i_5 < -i_0+PB_L; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
                D[i_5+1][i_6+1][i_0-1] = G[i_5+i_0][i_5] - H[i_6][i_0][i_5] - I[i_0][i_5][1] - D[i_0-2][i_6+1][i_5] + 3;
                D[i_0-1][i_5+1][i_6] = I[i_6+1][i_5][i_5] - G[i_6][i_0] * I[i_5+1][i_0][i_6] - D[i_5-1][i_6+2][i_0-2] + D[i_0-2][i_5+1][i_6+1] * 2;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4412331325_07(xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
