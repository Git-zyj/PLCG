...
void kernel_3216131125_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_Q-1, i_0); i_2++) {
                A[i_1][i_0][i_1] = C[i_2+1][i_0][i_1] + C[i_1][i_2+1][i_0] + C[1][i_1][i_2+1] + 6;
            }
            for (int i_3 = 0; i_3 < min(-i_1+PB_M, i_1); i_3++) {
                A[i_3][i_1-2][i_3] = C[i_3+i_1][i_0][i_3] - C[i_1][i_0][i_3] - A[i_3][i_1][i_3] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3216131125_03(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
