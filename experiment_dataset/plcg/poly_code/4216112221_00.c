...
void kernel_4216112221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N+2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_0-2][i_1] = A[i_0-3][i_1-2] - 2;
        }
    }
    for (int i_2 = 3; i_2 < PB_L+3; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L, i_2); i_3++) {
            for (int i_4 = 3; i_4 < PB_L-1; i_4++) {
                B[i_3][i_4-1] = C[i_3][i_4+1] * B[i_3][i_4] * B[i_4][i_2-3] + B[i_3-2][i_4-3] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4216112221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
