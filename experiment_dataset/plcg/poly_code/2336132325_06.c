...
void kernel_2336132325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            A[i_1+i_0][2] = D[i_1-1] * E[i_1+i_0][i_1] + E[i_1][i_0] - E[i_1+2][i_0-1] * D[i_0] - A[i_1+i_0+1][2] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L; i_3++) {
            A[i_3+i_2-2][2] = E[i_3+1][0] + E[i_2+2][i_3+i_2] * 6;
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < PB_L-2; i_5++) {
            C[i_4][i_5] = C[i_4+1][i_5] - C[i_4][i_5-1] * C[i_5+2][i_4] - C[i_4][i_5+2] * 4;
        }
    }
#pragma endscop
...
kernel_2336132325_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
