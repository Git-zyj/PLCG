...
void kernel_3234152325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0-1] = C[i_0+2] - C[i_0] + A[i_0-2] + A[i_0] - A[i_0+1] - 1;
    }
    for (int i_1 = 3; i_1 < PB_N+3; i_1++) {
        A[i_1-3] = C[i_1] * C[i_1-1] - C[i_1] + C[i_1-2] + D[i_1+1][i_1-1] * C[i_1+1] - C[i_1+1] - C[i_1-1] * 4;
    }
#pragma endscop
...
kernel_3234152325_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
