...
void kernel_4332352323_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < min(-i_0+PB_Q, PB_L-2); i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_Q-1; i_2++) {
                for (int i_3 = 3; i_3 < min(-i_1+PB_L-1, -i_0+PB_P-1); i_3++) {
                    A[i_1][i_3-1][i_2+2][i_3+i_0] = D[i_3+i_2][i_2][i_2][i_1+i_0] + E[i_3][i_0][i_1+2][i_2] - F[i_1][i_2][i_3][2] * E[i_1][i_3][i_2][i_0] - G[i_2][i_1][i_3+i_0+1] * F[i_2][i_3][i_1][i_0] * F[i_3+i_1+1][i_0][i_2+2][i_3] * E[i_1-2][i_3+i_2-1][i_0][i_2+2] + A[i_1+1][i_3-3][i_2+2][i_3+i_0-2] - A[i_3][i_1-1][i_2][i_1+i_0] * A[i_2][i_1-3][i_3+2][i_1+i_0-2] - A[i_1][i_0][i_3+2][i_2+i_0+1] + 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        B[i_4] = H[i_4] * H[i_4] - B[i_4+1] + B[i_4+2] * 4;
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
                for (int i_7 = 0; i_7 < min(-i_6+PB_M, -i_4+PB_Q); i_7++) {
                    A[i_5][i_4-1][i_6+2][i_7+i_4] = D[i_6+2][i_4+1][i_6+1][i_5] - D[i_6][i_4-1][i_7][0] * E[i_7+i_6][i_4][i_4][i_5+2] + 2;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4332352323_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
