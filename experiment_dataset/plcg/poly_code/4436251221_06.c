...
void kernel_4436251221_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q; i_0++) {
        A[i_0-1] = E[i_0] - F[i_0] * G[i_0][i_0] + F[i_0] * F[i_0] * F[i_0] * A[i_0] - A[i_0-3] * A[i_0-2] + 6;
        for (int i_1 = 1; i_1 < min(PB_Q-1, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 2; i_3 < PB_Q; i_3++) {
                    B[i_1+1][i_3][i_2][i_3-1] = H[i_2][i_0][i_3][i_1] + I[i_3][i_1-1][i_2] + J[i_0][i_2+1][i_1+1][1] - K[i_3-1][i_1][0] * B[i_0+1][i_3-1][i_1-1][i_3-2] + 6;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_Q-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_Q; i_5++) {
            for (int i_6 = i_5; i_6 < PB_Q-1; i_6++) {
                C[i_4-1][i_5][i_6] = I[i_5][i_4][i_6+1] * I[i_6][i_5-1][i_4+1] * I[i_4][i_6-1][i_5] * C[i_4-2][i_5][i_6] - C[i_4-3][i_5][i_6] + 6;
            }
        }
        for (int i_7 = i_4; i_7 < PB_Q-1; i_7++) {
            for (int i_8 = i_7; i_8 < PB_L-1; i_8++) {
                for (int i_9 = 0; i_9 < -i_4+PB_L+1; i_9++) {
                    D[i_9+i_4][i_7][i_4][i_8] = H[i_9+1][i_4+1][i_8][i_7+1] - J[i_4][i_7-1][i_8][i_9+1] - J[i_9][i_8][i_7+1][i_9+1] - H[i_8+1][i_9+i_4-1][i_7][i_7] * D[i_9+i_4-1][i_8][i_4][i_7] + 2;
                }
            }
        }
    }
...
kernel_4436251221_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
