...
void kernel_3236212323_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P-2, i_0); i_1++) {
            for (int i_2 = 3; i_2 < PB_P+1; i_2++) {
                A[i_0][i_1][i_2-1] = A[i_0-2][i_1][i_2-1] * A[i_0-2][i_1+2][i_2-1] * A[i_0][i_1][i_2-3] + 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            for (int i_5 = max(i_4, 1); i_5 < -i_4+PB_M; i_5++) {
                B[i_3+1][i_4][i_5+1] = C[i_5+i_4][i_3][i_3] * B[i_3+1][i_4][i_5-1] + B[i_3+1][i_4][i_5] * B[i_3+1][i_4][i_5+2] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3236212323_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
