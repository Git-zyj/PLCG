...
void kernel_3324352215_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0+2] - E[i_0-2] + F[i_0] * D[i_0+1] * A[i_0+1] + A[i_0-1] - A[i_0-1] - 5;
    }
    for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
        A[i_1] = D[0] - E[i_1] + D[i_1+1] - F[i_1] * E[0] + 6;
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            for (int i_3 = 0; i_3 < -i_1+PB_N-1; i_3++) {
                C[i_3][i_2][i_1] = G[i_2][i_3][i_1] * H[i_2+i_1][i_3][i_2] * H[i_3][i_2][i_2-1] * I[i_3+i_1+1][i_2-1][i_2] * G[i_3+i_2+1][i_3][i_1] - J[i_2][i_1-2] * C[i_1][i_3][i_2+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3324352215_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
