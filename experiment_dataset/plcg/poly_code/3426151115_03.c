...
void kernel_3426151115_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_Q; i_1++) {
        B[i_1] = E[i_1] - F[i_1+1][1] + 2;
        for (int i_2 = 1; i_2 < PB_Q; i_2++) {
            for (int i_3 = i_1; i_3 < min(PB_Q-1, -i_2+PB_M); i_3++) {
                C[i_1][i_2][i_3-1] = G[i_2][i_3][i_1] * G[i_2+1][i_1][i_3+1] - F[i_3+i_2][i_1-1] * G[i_2-1][i_3][i_2] - G[i_3-1][i_1+1][i_2-1] + F[i_3][i_2-1] - F[i_3+1][i_2] - C[i_1][i_2-1][i_3-1] + 5;
            }
        }
        D[i_1] = E[i_1] - E[i_1] * E[i_1] + E[i_1+1] - F[i_1-1][i_1] + E[i_1] * B[i_1] * 2;
    }
#pragma endscop
...
kernel_3426151115_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
