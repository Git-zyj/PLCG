...
void kernel_4232111321_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < min(PB_P, i_0); i_3++) {
                    A[i_1][i_2][i_3] = A[i_3-1][i_2+1][i_1] * 6;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 2; i_5 < PB_M; i_5++) {
            B[i_5] = C[i_5][i_4+1] * B[i_5-2] * B[i_4-2] + B[i_5-1] + B[i_5+1] * 1;
        }
    }
#pragma endscop
...
kernel_4232111321_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
