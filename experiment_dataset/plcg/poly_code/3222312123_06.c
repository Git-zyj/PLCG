...
void kernel_3222312123_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L; i_2++) {
                A[i_0][i_0][i_2+i_1] = C[i_2][0] - A[i_0][i_0][i_2+i_1-1] * 2;
            }
        }
        for (int i_3 = 3; i_3 < i_0; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_Q, -i_0+PB_L); i_4++) {
                B[i_4+i_0][i_3-2][i_4] = B[i_4+i_0][i_3-3][i_4] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3222312123_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
