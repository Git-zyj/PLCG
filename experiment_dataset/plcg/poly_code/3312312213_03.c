...
void kernel_3312312213_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        ...
        for (int i_3 = 0; i_3 < min(PB_M-3, -i_1+PB_N); i_3++) {
            C[i_3+2] = E[i_3][0] + E[i_3+i_1][i_3] + C[i_3+3] - 1;
        }
    }
#pragma endscop
...
kernel_3312312213_03(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
