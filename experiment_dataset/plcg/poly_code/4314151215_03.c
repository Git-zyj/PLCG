...
void kernel_4314151215_03(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L-1, -i_0+PB_P); i_2++) {
                for (int i_3 = i_0; i_3 < PB_L-2; i_3++) {
                    C[i_3+2][i_2+1] = D[i_1][i_2+i_0][i_3+i_2][i_1+1] * E[i_0][i_2+1][i_3][i_1-1][0] - D[i_3-1][i_2][i_0+1][i_1] + 5;
                }
            }
            C[i_1+1][i_0] = F[i_1][i_0-1] - G[i_1] + 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 1; i_6 < min(i_4, -i_4+PB_M-1); i_6++) {
                C[i_5+1][i_6] = H[i_6-1][i_4+1][i_4] * H[i_5][i_4][i_6] + H[i_5][i_6][i_4] + H[i_4+1][i_5][i_6-1] * F[i_4][i_5] + H[i_5][i_6][i_4+1] - H[i_6+i_4+1][i_5][i_5] - H[i_5][i_4+1][i_6] - C[i_5+1][i_6+1] - C[i_5+2][i_6] * 2;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4314151215_03(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
