...
void kernel_4422252115_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
        ...
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_L-1; i_3++) {
                    A[i_0][i_2+i_1-1][i_1][i_3+2] = E[i_3][i_2][i_1+1][i_0] + F[i_1][i_3][i_3+1] * E[i_0-1][i_3+i_1+1][i_2][i_3] * E[i_3-1][i_1][i_0][i_2+2] * A[i_0][i_2+i_1][i_1+1][i_3+2] + 1;
                }
            }
        }
        for (int i_4 = 0; i_4 < min(-i_0+PB_M, i_0); i_4++) {
            D[i_4] = G[i_4][i_0] - H[i_4+i_0][1] - G[i_4][i_0] * H[i_4][i_0] * G[i_4][i_0-2] - I[i_0] - 6;
        }
    }
    for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 2; i_6 < PB_N; i_6++) {
            for (int i_7 = i_5; i_7 < min(-i_5+PB_N, PB_L-2); i_7++) {
                D[i_7] = H[i_6-2][i_7] * H[i_5+2][i_7+2] - H[i_5][i_7] - J[i_7+i_5][i_6][i_7] - 1;
            }
        }
    }
    for (int i_8 = 0; i_8 < PB_N-2; i_8++) {
        D[i_8] = I[i_8] - I[i_8] * G[i_8][i_8] * I[i_8+2] - 1;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4422252115_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
