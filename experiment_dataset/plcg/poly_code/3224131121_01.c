...
void kernel_3224131121_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        ...
        for (int i_3 = i_0; i_3 < min(PB_P, -i_0+PB_L-2); i_3++) {
            B[i_3+i_0+1][1] = C[i_3][i_0] * C[i_3-1][i_0-1] * C[i_0][i_0] * C[i_3-1][i_0] + C[i_0][i_3-1] + B[i_3+i_0+2][1] + 5;
        }
    }
#pragma endscop
...
kernel_3224131121_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
