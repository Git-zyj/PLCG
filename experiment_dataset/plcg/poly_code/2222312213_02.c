...
void kernel_2222312213_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N+1; i_1++) {
            A[i_1][i_1] = C[i_1-1][0] - 1;
        }
    }
    for (int i_2 = 3; i_2 < PB_M+1; i_2++) {
        B[i_2-2] = B[i_2-1] * B[i_2-3] * B[i_2-1] * 3;
    }
#pragma endscop
...
kernel_2222312213_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
