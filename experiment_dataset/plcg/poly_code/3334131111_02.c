...
void kernel_3334131111_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_N); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_1+i_0][i_0][i_2] = D[i_1][i_0] + 5;
            }
        }
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < min(min(i_0, -i_3+PB_Q), PB_L-1); i_4++) {
                B[i_0][i_3][i_4+i_3] = E[i_4+1][i_0][i_3] * E[i_4][i_0][i_3] + E[i_3][i_0][0] * E[i_0][i_3+1][i_3] * 3;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 1; i_6 < -i_5+PB_M-1; i_6++) {
            for (int i_7 = i_5; i_7 < PB_L; i_7++) {
                C[i_5][i_7][i_6] = E[i_5][i_7][i_6-1] - E[i_5][i_6][i_7] - E[i_6][i_7][i_5] - D[i_6+i_5+1][i_7] * C[i_5-1][i_7][i_6-1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3334131111_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
