...
void kernel_4212112113_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0][i_0+2] = C[i_1] * A[i_0+1][i_0+3] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 1; i_4 < PB_M; i_4++) {
                B[i_4][i_3][i_2] = B[i_4-1][i_3][i_2-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4212112113_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
