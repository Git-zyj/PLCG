...
void kernel_3412112211_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1+i_0] = E[i_1][i_0] * A[i_0-1][i_1+i_0-2] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_2+2][i_3] = D[i_2] + D[i_3-1] + 5;
            for (int i_4 = i_3; i_4 < -i_2+PB_M; i_4++) {
                C[i_4+i_2-1][i_3][i_4] = A[i_4][i_4+i_2] * C[i_4+i_2][i_3-1][i_4] * 6;
            }
        }
        D[i_2] = 2;
    }
...
kernel_3412112211_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
