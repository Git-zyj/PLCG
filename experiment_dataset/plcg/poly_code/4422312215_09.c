...
static void print_array(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4422312215_09(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L+1; i_1++) {
            ...
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = i_1; i_3 < min(PB_P-1, -i_1+PB_L+1); i_3++) {
                    C[i_2][i_3+i_1-1][i_3+1] = E[i_2][i_0-1][i_1-2][i_3-2] * 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_L-1; i_5++) {
            for (int i_6 = min(-i_4+PB_L+1, PB_P-1) - 1; i_6 >= 0; i_6--) {
                C[i_6][i_5+i_4][i_5+1] = F[i_5-1][i_4][i_6] + G[i_6][i_5] + C[i_6][i_5+i_4+1][i_5+1] * C[i_5][i_6+i_4-1][i_6+1] + 5;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_N-1; i_7++) {
        D[i_7] = H[i_7] * D[i_7-1] - D[i_7+1] - 4;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422312215_09(xd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
