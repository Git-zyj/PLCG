...
void kernel_2216152111_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L-1; i_1++) {
            A[i_1-2][i_1-1] = C[i_0][i_1+1] + D[i_1+2] - D[i_1] * D[2] * C[i_0][i_1] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 3; i_3 < min(-i_2+PB_L, PB_L-1); i_3++) {
            B[i_3-2][i_2][i_2] = E[i_2][i_3][i_3+i_2] - C[i_2][i_3+1] + C[i_3][i_2] - D[i_3-1] - A[i_3-1][i_3] * B[i_3-3][i_2][i_2] - 6;
        }
    }
#pragma endscop
...
kernel_2216152111_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
