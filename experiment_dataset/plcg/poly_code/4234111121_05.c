...
void kernel_4234111121_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
            for (int i_4 = 3; i_4 < PB_L+3; i_4++) {
                for (int i_5 = max(1, i_2); i_5 < PB_P-1; i_5++) {
                    B[i_2][i_5-1][i_3+1] = B[i_2][i_4-3][i_5+1] + 5;
                }
            }
        }
    }
...
kernel_4234111121_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
