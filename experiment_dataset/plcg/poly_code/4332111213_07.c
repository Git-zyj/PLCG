...
void kernel_4332111213_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M+1, PB_L-1); i_1++) {
            A[i_1+1][i_1+1][0] = D[i_1][i_0] * B[i_1+i_0-1][i_0] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M; i_3++) {
            B[i_3+i_2-1][i_3] = B[i_3+i_2][i_3+1] + B[i_3+i_2-2][i_3] + B[i_3+i_2][i_3] * 5;
        }
        for (int i_4 = 0; i_4 < PB_Q; i_4++) {
            for (int i_5 = 1; i_5 < PB_P; i_5++) {
                C[i_2+1][i_5][0] = C[i_2+2][i_5-1][0] - C[i_2+2][i_5][0] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4332111213_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
