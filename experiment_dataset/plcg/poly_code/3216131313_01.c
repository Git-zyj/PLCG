...
void kernel_3216131313_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_1] = C[i_1+1] - D[i_1][i_0] * D[i_0][i_1+1] + A[i_1-1] * 3;
            for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
                B[i_2][i_1+1][i_0][i_0-1] = E[i_1+i_0+1][i_2][i_1-1] * E[i_1][i_0][i_2+1] - A[i_1] - A[i_2+1] + B[i_2-1][i_0+1][i_1][i_1-1] * B[i_1+1][i_2][i_0-1][i_0-2] - B[i_2-1][i_1+2][i_0][i_0-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3216131313_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
