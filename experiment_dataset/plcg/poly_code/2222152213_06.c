...
void kernel_2222152213_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_L, PB_M-2); i_2++) {
            B[i_1][i_2] = D[i_2-2][i_2+2] - C[i_2+2] * D[i_1-2][i_2+2] + D[i_1][i_2] + D[i_2+2][i_1] + D[i_1+1][i_2+i_1] - A[i_1][i_1] + A[i_2][i_2] + 4;
        }
    }
#pragma endscop
...
kernel_2222152213_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
