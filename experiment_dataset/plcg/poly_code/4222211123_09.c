...
void kernel_4222211123_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                    A[i_0][i_2+i_1+1][i_3+1][i_3+i_2] = 3;
                }
            }
        }
        B[i_0-1] = C[i_0] - B[i_0+1] + B[i_0-3] * 5;
    }
#pragma endscop
...
kernel_4222211123_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
