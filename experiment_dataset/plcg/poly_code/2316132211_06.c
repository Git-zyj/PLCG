...
void kernel_2316132211_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_N+1; i_1++) {
            A[i_0][i_1-1] = A[i_0-1][i_1-1] + A[i_0+1][i_1] * A[i_1-1][i_0-1] - 3;
        }
        C[i_0][i_0] = D[0] * D[i_0] * D[i_0] - D[i_0] * 5;
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_L, i_2); i_3++) {
            C[i_3][i_3] = E[i_3+i_2][i_3] + E[i_2+2][i_3] - E[i_3+i_2][2] * C[i_3-1][i_3-1] * 4;
        }
    }
#pragma endscop
...
kernel_2316132211_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
