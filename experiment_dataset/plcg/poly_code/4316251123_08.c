...
void kernel_4316251123_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
        ...
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_Q, PB_Q-1), i_0); i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_P; i_3++) {
                    A[i_0][i_3][i_2][i_0+1] = D[i_1+1][i_2][i_3+i_0][i_2+1] + D[i_3][i_3][i_1][i_2+i_0] + 5;
                }
            }
        }
        B[i_0] = E[i_0] + F[i_0] + F[i_0] + F[i_0] * E[i_0] + B[i_0+1] - 3;
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < -i_4+PB_M; i_5++) {
            B[i_5-2] = G[i_5+1][i_4] - G[i_5+i_4][1] - H[i_5+1][i_4] - H[i_5][i_5] + G[i_4][0] * G[i_5][i_4] * H[i_4-1][i_5-1] * H[i_5][i_4] - B[i_5] - 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4316251123_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
