...
void kernel_3212151223_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M - 1; i_0 >= 1; i_0--) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_N, -i_0+PB_L-1); i_2++) {
                B[i_2] = C[i_1][i_2][i_0] * C[i_1][i_0-1][0] * C[i_1][i_2+i_0+1][1] - D[i_2+i_1][1] - B[i_1+1] - 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 3; i_4 < -i_3+PB_N; i_4++) {
            B[i_4-1] = D[i_3][i_3] * D[i_3][i_4-1] - D[i_3][i_4-1] - C[i_4][i_3][1] * D[i_4+i_3][i_4] + D[i_3-1][0] * B[i_4-3] * 2;
        }
    }
#pragma endscop
...
kernel_3212151223_02(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
