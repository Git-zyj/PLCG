...
void kernel_2224312125_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            B[i_2][i_3] = D[i_3+1][i_2+1] + B[i_2+2][i_3] - 5;
        }
    }
#pragma endscop
...
kernel_2224312125_06(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
