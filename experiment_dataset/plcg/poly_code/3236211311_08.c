...
void kernel_3236211311_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_1] = C[i_1][i_0][i_0] + C[i_2][i_0][i_1] + 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            B[i_4+1] = B[i_4] - B[i_4+2] + B[i_4] * 4;
        }
    }
#pragma endscop
...
kernel_3236211311_08(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
