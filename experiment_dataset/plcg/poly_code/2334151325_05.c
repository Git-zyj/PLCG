...
void kernel_2334151325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0] = D[i_0] * D[0] * D[i_0-1] * D[i_0+1] + D[0] * D[i_0] * D[i_0] - A[i_0+1] * A[i_0+2] - 4;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_M-1; i_2++) {
            B[i_2][i_1] = E[i_2-1][i_1] - E[i_2][i_1] * E[i_2][0] * E[i_1][i_2] - D[i_2+1] + B[i_2-2][i_1+1] * B[i_2][i_1+1] - B[i_2+1][i_1+1] - B[i_2-2][i_1] * 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            A[i_4-2] = E[i_4][i_3+1] - 1;
        }
    }
#pragma endscop
...
kernel_2334151325_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
