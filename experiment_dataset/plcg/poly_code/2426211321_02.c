...
void kernel_2426211321_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        ...
        for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
            D[i_4][i_5] = C[i_4] * D[i_4+2][i_5-2] + D[i_4+1][i_5-1] * D[i_4+1][i_5] + D[i_4-1][i_5] * D[i_4-1][i_5+1] + 1;
        }
    }
#pragma endscop
...
kernel_2426211321_02(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
