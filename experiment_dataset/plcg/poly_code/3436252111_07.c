...
void kernel_3436252111_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        A[i_0+1] = E[i_0] - F[i_0-2] + F[0] + A[i_0+2] * C[i_0] + 5;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        B[i_1] = E[0] - E[i_1+2] * F[i_1] - B[i_1+1] * 6;
    }
    for (int i_2 = 2; i_2 < PB_Q; i_2++) {
        C[i_2+1] = F[i_2] - G[i_2][0] + F[i_2-1] * G[i_2][i_2] + F[i_2-2] + F[i_2] * E[0] + 3;
        for (int i_3 = i_2; i_3 < PB_Q; i_3++) {
            for (int i_4 = 2; i_4 < min(-i_3+PB_M, PB_L-2); i_4++) {
                D[i_2][i_4-2][i_3] = H[i_3+2][i_2][i_3] * I[i_4][i_2][i_3] + I[i_3][0][i_2-2] * H[i_3+2][i_4+2][i_2] + I[i_4+i_3][i_2][i_3] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3436252111_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
