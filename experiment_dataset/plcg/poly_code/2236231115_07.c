...
void kernel_2236231115_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = PB_M-1 - 1; i_1 >= 1; i_1--) {
        A[i_1-1] = C[i_1][i_1] + D[i_1+1] + D[i_1] + D[i_1+1] * D[i_1] - 3;
    }
#pragma endscop
...
kernel_2236231115_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
