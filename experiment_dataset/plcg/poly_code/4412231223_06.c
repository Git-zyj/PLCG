...
void kernel_4412231223_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] + E[i_0] - E[i_0] * 4;
        for (int i_1 = 2; i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
                    B[i_0][i_1][i_3+1][i_2] = F[i_0][i_3+1][i_2][i_1-1] - G[i_2-1][i_0][i_1][1] + G[i_3+1][i_2][i_1+i_0][0] + B[i_1-2][i_3+2][i_0-1][i_2] * 4;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 1; i_5 < PB_P-2; i_5++) {
                C[i_4][i_0] = H[i_4-1][i_0][i_5+1] - I[i_5-1][i_4][i_0-1] - J[i_4][i_5] - C[i_5][i_4-1] * C[i_5-1][i_4] - C[i_4][i_5+2] * 1;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_Q-2; i_6++) {
        for (int i_7 = 0; i_7 < min(i_6, PB_P-3); i_7++) {
            for (int i_8 = 0; i_8 < min(i_6, -i_6+PB_P+1); i_8++) {
                for (int i_9 = 0; i_9 < -i_8+PB_P; i_9++) {
                    D[i_8][i_7+1][i_6][i_9] = F[i_7][i_6][i_9+i_8][i_6+1] * G[i_9][i_7][i_8+i_6-1][i_7+1] * D[i_8][i_7+2][i_6][i_9] + D[i_8][i_7+3][i_6+2][i_9] + 6;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4412231223_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
