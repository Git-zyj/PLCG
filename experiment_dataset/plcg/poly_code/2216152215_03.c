...
void kernel_2216152215_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = C[1] - C[i_0] * D[i_0][i_0] * 5;
        for (int i_1 = max(1, i_0); i_1 < -i_0+PB_M; i_1++) {
            A[i_1] = D[i_1+i_0][i_1] - D[i_0][i_1] * C[i_1] + D[i_1][i_0] - D[i_1][i_0] * C[i_1] * A[i_1] + A[i_1-1] * 2;
        }
    }
#pragma endscop
...
kernel_2216152215_03(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
