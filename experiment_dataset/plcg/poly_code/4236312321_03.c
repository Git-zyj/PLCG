...
void kernel_4236312321_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M-3, i_0); i_1++) {
            A[i_1+1] = A[i_1+2] * A[i_1+3] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, PB_L-1); i_3++) {
            B[i_2][i_3] = C[1] - B[i_2-1][i_3] * B[i_2][i_3-2] + B[i_2][i_3-1] - B[i_2][i_3+1] - 4;
        }
    }
#pragma endscop
...
kernel_4236312321_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
