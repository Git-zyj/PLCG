...
void kernel_2322351125_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        A[i_0-1] = D[i_0+1][i_0] - E[i_0-1] - D[i_0][i_0-1] - F[i_0] - G[i_0][i_0-1] * A[i_0-3] + 2;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            A[i_2-1] = D[i_2][i_1] + H[i_1][i_1] * D[i_1][i_2] + E[i_2-1] + H[i_1][i_1-1] * G[i_1][i_2] * 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        A[i_3-1] = E[i_3] - I[i_3] - F[i_3-1] * I[i_3] + 4;
    }
#pragma endscop
...
kernel_2322351125_04(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
