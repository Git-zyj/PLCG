...
void kernel_3224131225_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_0] = C[i_1][0] + C[i_1][i_0] * C[i_1][i_0-1] * C[i_1][i_0] * C[i_1-1][i_0-1] * 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L, i_2); i_3++) {
            B[i_3] = C[i_3][i_3-1] + B[i_3+1] + B[i_3-2] + 1;
        }
    }
#pragma endscop
...
kernel_3224131225_08(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
