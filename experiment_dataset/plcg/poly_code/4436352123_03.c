...
void kernel_4436352123_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
        ...
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                    A[i_1+2][i_0+2][i_3+1][i_3] = E[i_2+1][i_0][i_3][i_1] + F[i_2+2][i_1+1][i_3][i_0][i_0] + G[i_3][i_2-1][i_0-1][i_2] * E[i_2][i_0+1][i_3][i_1-1] * 2;
                }
            }
        }
        B[i_0] = H[i_0-1] * I[i_0-1] + 4;
    }
    for (int i_4 = 2; i_4 < PB_N-2; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_N+1); i_5++) {
            C[i_4][i_5] = J[i_5][i_4] - K[i_5+i_4-1][i_5] + L[i_4][i_5-1] - L[i_4-1][i_5] + L[i_4+2][i_4-2] * J[i_4-2][i_5+2] * K[0][i_5] * C[i_4][i_5+1] * 2;
        }
        for (int i_6 = 0; i_6 < i_4; i_6++) {
            B[i_6] = J[i_6][i_4-2] - K[i_6][i_4] + L[i_4][i_6] * J[i_4][i_6] + L[i_6+2][i_4] * L[i_6][i_4+2] - L[i_6][i_4] + 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4436352123_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
