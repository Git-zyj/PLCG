...
void kernel_3234212325_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P-2; i_1++) {
            B[i_0][i_1+2] = B[i_0-1][i_1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_P; i_3++) {
            B[i_2][i_3] = C[i_3][i_2] * B[i_2][i_3-2] - B[i_2][i_3-1] - 4;
        }
    }
#pragma endscop
...
kernel_3234212325_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
