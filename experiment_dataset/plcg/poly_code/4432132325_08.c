...
void kernel_4432132325_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            for (int i_2 = i_0; i_2 < min(-i_1+PB_Q+2, PB_L-2); i_2++) {
                for (int i_3 = 2; i_3 < -i_1+PB_L; i_3++) {
                    B[i_2-2][i_1+2][i_0-2] = E[i_3-2][i_0][i_2+i_1-2][i_1] * F[i_2][i_3+i_1][i_2+2] - 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = 3; i_5 < PB_L; i_5++) {
            for (int i_6 = 1; i_6 < PB_L-3; i_6++) {
                B[i_5][i_6+2][i_4-2] = F[i_4][i_6][i_5] - G[i_5][i_6] - F[i_5][i_6-1][i_5] + F[i_6][i_4][i_5-2] - B[i_5-2][i_6+2][i_4-2] + B[i_4][i_6+2][i_5-3] + B[i_5-2][i_6+3][i_4-1] + 4;
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_M-2; i_7++) {
        C[i_7+1] = H[i_7] * H[i_7] - H[i_7+1] + H[i_7] * H[i_7] * 2;
        C[i_7+1] = H[i_7] + C[i_7+1] + C[i_7+2] - C[i_7] - 6;
    }
#pragma endscop
...
kernel_4432132325_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
