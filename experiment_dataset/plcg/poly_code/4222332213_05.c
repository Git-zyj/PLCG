...
void kernel_4222332213_05(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_1][i_2][0][i_0-2] = C[i_1][i_2][i_0] - C[i_1-1][0][i_0] + 5;
            }
        }
    }
        ...
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            for (int i_5 = 2; i_5 < i_4; i_5++) {
                for (int i_6 = 2; i_6 < -i_5+PB_L; i_6++) {
                    B[i_6][i_4][0][i_5-2] = D[i_6-2][i_3][i_4-2][i_5] + E[i_6+i_5][i_4][i_3][i_3+1] + F[i_4-2][i_3][i_6][0] * B[i_3][i_6+1][0][i_5-2] + B[i_6+1][i_3][0][i_4-2] - 6;
                }
            }
        }
    }
...
kernel_4222332213_05(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
