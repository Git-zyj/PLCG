...
void kernel_2134132113_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1+2] = B[i_1-1][i_0+2][i_1] + C[i_0][i_1] * C[i_0-2][i_1] - A[i_1+3] - 1;
        }
    }
#pragma endscop
...
kernel_2134132113_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
