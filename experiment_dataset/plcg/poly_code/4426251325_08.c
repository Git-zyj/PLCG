...
void kernel_4426251325_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L; i_1++) {
            A[i_1][i_1] = E[i_0+1] + F[i_0][0] * F[0][i_0] * G[i_1][i_0] * A[i_0+2][i_0+2] + A[i_1-2][i_1-2] * 5;
            for (int i_2 = 0; i_2 < min(-i_0+PB_M, i_0); i_2++) {
                B[i_0][i_1][i_2] = H[i_0][i_2+i_1][i_2+1] * F[i_2+i_0][i_1] + H[i_0][i_2+i_1][0] * I[i_0+1][i_2][i_1] - F[i_0][i_2+i_1] - I[i_2][0][i_1+i_0] * F[i_2][i_1] * A[i_1-2][i_1-2] + B[i_1][i_2][i_0-2] - B[i_0+2][i_1][i_2] - 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L; i_4++) {
            A[i_3-2][i_3-2] = F[i_4+1][0] * F[i_4][i_3] * F[i_3][i_3] + 5;
        }
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        for (int i_6 = i_5; i_6 < PB_L-1; i_6++) {
            A[i_6][i_6] = F[i_5-1][i_6] + F[i_6+1][i_5] - F[i_5+1][i_6] - I[i_5][i_5-1][i_6+1] * F[i_6][i_5] - F[i_6-1][i_5] * 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4426251325_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
