...
void kernel_3212132311_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M+2, PB_M-1); i_2++) {
                A[i_2+1][i_1+1] = C[i_1][i_2] + D[i_1-2][i_0][i_2] - D[i_1][i_0-2][i_2] - C[i_2][0] - C[i_2+i_0-2][i_1-2] - A[i_1+2][i_0+2] * 1;
            }
        }
        B[i_0][i_0] = E[i_0] - B[i_0-1][i_0-1] + B[i_0+1][i_0+1] + B[i_0-1][i_0-1] * B[i_0-1][i_0-1] * 5;
    }
#pragma endscop
...
kernel_3212132311_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
