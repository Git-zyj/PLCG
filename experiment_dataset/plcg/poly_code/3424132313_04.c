...
void kernel_3424132313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
    ...
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 2; i_3 < min(-i_1+PB_L-2, -i_2+PB_L); i_3++) {
                B[i_2][i_3+i_1+1][0] = F[i_3-2][i_2] - G[i_3][i_1][i_2] * H[i_1][i_2+1][i_3+i_2][i_1-2] + G[i_1-1][i_3][i_3] * H[i_3][i_2+1][i_1][i_1] * B[i_2][i_3+i_1+2][0] + B[i_2+1][i_3+i_1+1][0] + 2;
            }
        }
        A[i_1] = E[i_1] * 3;
        A[i_1+1] = E[i_1] * E[i_1] - E[i_1+2] + A[i_1] + 3;
    }
#pragma endscop
...
kernel_3424132313_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
