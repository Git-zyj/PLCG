...
void kernel_3222111325_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
                A[i_0][i_1][i_2+1] = A[i_0-2][i_1][i_2+1] * A[i_0-2][i_1-2][i_2+1] - A[i_2-1][i_0-2][i_1+1] * 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_P, PB_P-1); i_5++) {
                A[i_3][i_4+1][i_5+1] = C[i_4][i_5+i_3][i_3] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3222111325_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
