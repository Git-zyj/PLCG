...
void kernel_3232232125_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_2][i_1] = C[i_1+1][i_2][i_0] * C[i_0+1][i_2-1][i_1] + A[i_1][i_2-1] - 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_P-1, i_3); i_4++) {
            for (int i_5 = max(i_4, 1); i_5 < min(-i_3+PB_L, PB_M); i_5++) {
                B[i_5][0][i_4-1] = D[i_5][i_5-1][i_4+1] - C[i_3][i_4][i_4] * D[i_4][i_5+i_3][0] - B[i_5-1][0][i_4-1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3232232125_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
