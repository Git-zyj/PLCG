...
void kernel_4432151123_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = E[i_0+1] - E[i_0] - E[i_0] + E[1] * E[i_0] + 6;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
                B[i_3][i_2-1][i_1] = F[i_3][i_2][i_1-1] - F[i_2][i_3-1][i_1] - F[i_3+i_2][i_1][i_1+1] + G[i_3][i_2] + F[i_1+1][i_3+1][i_2+1] * B[i_3-1][i_2-1][i_1] + 3;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        A[i_4-1] = E[i_4-1] * E[i_4] + 1;
        for (int i_5 = 1; i_5 < PB_Q; i_5++) {
            for (int i_6 = 1; i_6 < PB_M-3; i_6++) {
                for (int i_7 = 0; i_7 < min(min(PB_Q-2, -i_5+PB_L), -i_4+PB_L); i_7++) {
                    D[i_6+1][i_7+i_5][i_4+1][i_7] = H[i_7][i_4][i_5][i_6] - H[i_5-1][i_4+1][i_7+1][i_5] * H[i_7][i_4][i_5][i_6] - H[i_6+1][i_7+i_4][i_5][i_7] * H[i_7][i_6][i_4-1][i_6-1] - D[i_6+3][i_7+i_5][i_4+1][i_7+2] - 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4432151123_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
