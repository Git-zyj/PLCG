...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1][i_1-1] = G[i_1] + G[i_1] - F[i_1+1] - B[i_1+1][i_1] + D[i_1] - D[i_1+1] * 4;
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            D[i_2-1] = H[i_3][i_2+2] - H[i_3][i_2] + I[i_3][i_2-2][0] * A[i_3] + 6;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        D[i_4] = F[i_4] + A[i_4-1] + B[i_4][i_4-1] * D[i_4-1] - 1;
    }
#pragma endscop
...
kernel_3436332311_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
