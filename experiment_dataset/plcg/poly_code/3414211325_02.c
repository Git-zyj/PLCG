...
void kernel_3414211325_02(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                D[i_1+1][i_1+1][i_0-2] = 1;
                D[i_1][i_1][i_0] = E[i_1-1][i_0][i_1+1] + 3;
            }
        }
    }
    for (int i_3 = 4; i_3 < PB_P; i_3++) {
        for (int i_4 = 3; i_4 < -i_3+PB_P; i_4++) {
            C[i_4-1][i_4+i_3] = C[i_4-1][i_4+i_3-1] + C[i_4-3][i_4+i_3-4] * C[i_4-1][i_4+i_3-2] + 5;
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                D[i_3][i_3][i_4] = E[i_3][i_4][i_5] + D[i_3+1][i_3+1][i_4-1] * D[i_3+1][i_3+1][i_4+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3414211325_02(xd,yd,zd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E));
