...
void kernel_4224212121_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            B[i_0][i_0][i_1] = C[i_1][i_0-2] - C[i_1][i_0] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 2; i_4 < PB_P; i_4++) {
                B[i_2][i_2][i_4] = B[i_2][i_2][i_4-2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4224212121_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
