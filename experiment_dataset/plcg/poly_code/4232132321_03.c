...
void kernel_4232132321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_P-2; i_1++) {
            A[i_1][i_1] = C[i_1] * D[i_0][i_1] * C[i_0] + A[i_1-2][i_1-2] * A[i_1+2][i_1+2] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2 - 1; i_3 >= 2; i_3--) {
            B[i_2][i_3+2] = C[i_3] * D[i_2-2][i_3] * D[i_3-2][i_2] + B[i_2][i_3+3] - B[i_2+1][i_3+2] + B[i_2][i_3+4] - 2;
        }
    }
#pragma endscop
...
kernel_4232132321_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
