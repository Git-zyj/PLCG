...
void kernel_4214111225_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M+2; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_Q-1, -i_1+PB_P), i_1); i_2++) {
                for (int i_3 = 2; i_3 < min(-i_0+PB_M, -i_1+PB_M+1); i_3++) {
                    A[i_3+i_1-1][i_0-1][i_2+i_0+1][i_3+i_2] = A[i_1+i_0-2][i_3-2][i_3+i_2+1][i_2+i_0+1] * A[i_3+i_0][i_1-1][i_2+i_1][i_2+i_0-1] + 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, PB_L-2); i_5++) {
            B[i_5] = C[i_4-1][i_5+1] + C[i_5][i_4+1] - B[i_5+2] - 6;
        }
    }
#pragma endscop
...
kernel_4214111225_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
