...
void kernel_3326212123_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
        for (int i_2 = 1; i_2 < min(i_1, PB_L+1); i_2++) {
            B[i_1+1][i_2] = D[0][i_2-1] + B[i_1+2][i_2] + 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_N; i_4++) {
            C[i_3][i_3] = C[i_3-1][i_3-1] - 6;
        }
    }
#pragma endscop
...
kernel_3326212123_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
