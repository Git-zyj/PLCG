...
void kernel_4222212221_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
        ...
        for (int i_1 = PB_M-1 - 1; i_1 >= 1; i_1--) {
            for (int i_2 = i_1; i_2 < -i_1+PB_M-1; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_1+PB_M, PB_L-2); i_3++) {
                    A[i_3+i_1][i_2][i_3] = A[i_2+i_1+1][i_3+2][i_1] * A[i_2+i_1-1][i_3][i_2-1] + A[i_2+i_1][i_3-2][i_2] - 3;
                }
            }
        }
        B[i_0] = C[i_0-2] - D[i_0] + 3;
    }
#pragma endscop
...
kernel_4222212221_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
