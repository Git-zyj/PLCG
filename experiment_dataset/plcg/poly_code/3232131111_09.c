...
void kernel_3232131111_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_P+1; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_L, -i_1+PB_Q); i_2++) {
                B[i_0-1][i_2][i_2+i_1] = C[i_2][i_1] - D[i_0][i_2-1][i_1+1] - C[i_2][0] + D[i_1][i_2][i_0+1] * D[i_2][i_1+i_0-1][i_2+1] - 6;
            }
            for (int i_3 = 1; i_3 < min(PB_P-1, -i_1+PB_Q); i_3++) {
                B[i_0][i_3][i_3+i_1] = D[i_3-1][i_3+1][i_0] - B[i_0+1][i_3][i_3+i_1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3232131111_09(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
