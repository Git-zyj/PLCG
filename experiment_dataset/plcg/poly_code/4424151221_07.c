...
void kernel_4424151221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = E[i_0+1] + E[i_0+1] - E[i_0] * E[i_0] * A[i_0] + A[i_0-1] - 4;
    }
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        C[i_1+2][i_1+2] = E[i_1-1] - C[i_1][i_1] - 6;
        for (int i_2 = 1; i_2 < min(PB_L, -i_1+PB_M+1); i_2++) {
            C[i_1][i_1] = E[i_2-1] + F[i_2][i_1+1] + G[i_2+1][i_1][i_1] - F[i_1][i_2] * F[i_1+1][i_2-1] * E[i_2+i_1-1] - 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_Q; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_Q; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_4+PB_M-1, -i_3+PB_Q); i_5++) {
                for (int i_6 = 2; i_6 < -i_3+PB_Q; i_6++) {
                    D[i_6][i_4][i_5+i_3][i_5+i_3] = H[i_4][i_5+1][i_3-1][0] + H[i_6][i_5+i_4-1][i_3][i_6] * H[i_5+i_3-1][i_6][i_4][0] * H[i_5+i_4+1][i_6+i_3][i_3+1][i_4] + H[i_4+1][i_6+i_5][i_3][0] + G[i_6][i_5][i_4] - D[i_6][i_5-1][i_4+i_3][i_4+i_3] - D[i_6-2][i_4+2][i_5+i_3][i_5+i_3] - D[i_4+1][i_5-2][i_6+i_3][i_6+i_3] + 2;
                }
            }
        }
    }
...
kernel_4424151221_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
