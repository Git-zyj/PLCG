...
void kernel_4232211213_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_N; i_2++) {
                A[i_0][i_1][i_2] = C[i_2][i_1] - A[i_2][i_1+1][i_0] + 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = i_3; i_4 < PB_N; i_4++) {
            for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
                B[i_4][i_5+1] = B[i_4][i_5+2] * B[i_4][i_5] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4232211213_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
