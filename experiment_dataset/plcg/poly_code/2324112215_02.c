...
void kernel_2324112215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        B[i_2] = D[i_2-2] - D[i_2] - 6;
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            B[i_2] = A[i_3] + B[i_3+1] - 4;
        }
    }
#pragma endscop
...
kernel_2324112215_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
