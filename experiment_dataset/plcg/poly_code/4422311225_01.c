...
void kernel_4422311225_01(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
...
#pragma scop
    ...
    for (int i_4 = 1; i_4 < PB_P-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
            B[i_4-1][i_5-1] = B[i_4-1][i_5-2] - 6;
            for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 0; i_7 < -i_6+PB_Q+1; i_7++) {
                    D[i_4+2][i_5+2][i_4+2][i_7+i_6-1] = E[i_6+1][i_4][i_5+1][i_4-1] * 2;
                }
            }
        }
        for (int i_8 = 0; i_8 < -i_4+PB_Q-2; i_8++) {
            for (int i_9 = 2; i_9 < min(PB_L, -i_4+PB_Q); i_9++) {
                for (int j_0 = 0; j_0 < PB_P-2; j_0++) {
                    D[i_4][i_9][i_4][j_0+i_8] = F[j_0][i_8][i_9+i_4][i_9] * G[i_8+1][i_4-1][i_9-1] * D[j_0+2][i_9-2][j_0+2][i_8+i_4+2] + D[i_4][i_8][i_4][j_0+i_9-2] - 2;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4422311225_01(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
