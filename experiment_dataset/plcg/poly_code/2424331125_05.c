...
void kernel_2424331125_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M-1; i_2++) {
            B[i_2+i_1+1][i_1] = F[i_2] * G[i_2-1][i_1] * G[i_1][i_2] * A[i_2] * 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        A[i_3+1] = E[i_3+1] - H[i_3-1][1] - 4;
        A[i_3] = E[i_3+1] + G[i_3][i_3+1] + F[i_3-1] * 6;
    }
#pragma endscop
...
kernel_2424331125_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
