...
void kernel_3336311111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        ...
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            C[i_2+1][i_2-1] = D[0] - C[i_2][i_2-2] * 5;
        }
    }
#pragma endscop
...
kernel_3336311111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
