...
void kernel_2214132211_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0][i_1-2] = C[i_1-2][i_0] * C[i_1+2][i_0] * A[i_1+i_0-1][i_1-2] * B[i_1-1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 3; i_3 < i_2; i_3++) {
            B[i_3-2] = C[i_3][i_2+1] + D[i_2] - C[i_2][i_3] * B[i_3-1] + B[i_3-3] * 5;
        }
    }
#pragma endscop
...
kernel_2214132211_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
