...
void kernel_2312211225_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        B[i_1] = D[i_1+1] + B[i_1+2] - B[i_1-2] - B[i_1+1] * 5;
...
kernel_2312211225_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
