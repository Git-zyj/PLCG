...
void kernel_3232211121_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = i_0; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 0; i_3 < -i_0+PB_Q; i_3++) {
                B[i_3][i_2][i_3+i_0] = C[i_2+1][i_0][i_0] * D[i_3][i_2-1][0] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3232211121_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
