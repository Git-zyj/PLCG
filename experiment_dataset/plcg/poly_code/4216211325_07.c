...
void kernel_4216211325_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_M; i_1++) {
            A[i_1-1][i_0] = C[i_1][i_0] - D[i_1][i_0] * A[i_1-3][i_0] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            A[i_3-1][i_2] = A[i_3-1][i_2] * A[i_3][i_2-2] - 6;
        }
    }
#pragma endscop
...
kernel_4216211325_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
