...
void kernel_2322112111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_L; i_2++) {
            B[i_1-2][i_2] = D[i_2+i_1][1] - 2;
        }
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            C[i_1][i_3] = D[i_1][i_3-1] - D[i_1][i_1] * 6;
        }
    }
#pragma endscop
...
kernel_2322112111_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
