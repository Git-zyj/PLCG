...
void kernel_3232132225_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            B[i_1][i_0] = C[i_0][i_1] - B[i_1][i_0] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            B[i_2][i_3] = C[i_2][i_3+1] - C[i_3+1][i_2] - C[i_2][i_3] + C[i_3][0] * B[i_2+1][i_3] - 4;
        }
    }
#pragma endscop
...
kernel_3232132225_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
