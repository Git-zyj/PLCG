...
void kernel_3232152321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_M-3); i_1++) {
            A[i_1][i_1+2] = C[i_1-2][i_0] + C[i_1+i_0][2] + C[i_0+2][i_1+2] * C[i_0-1][0] * C[i_1][i_0] + A[i_1+1][i_1+3] * A[i_1-1][i_1+1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
            for (int i_4 = 0; i_4 < min(min(-i_3+PB_N, PB_M-2), i_3); i_4++) {
                B[i_2][i_3][i_4] = D[i_3+2][i_4+1][i_2-1] - D[i_4+i_3][i_2][2] + C[i_4][0] + D[i_2][i_4][i_3] * D[i_2][i_4][i_3+1] - A[i_4][i_4+2] + B[i_4][i_3][i_2+1] + B[i_2-1][i_3][i_4] + B[i_2][i_3-1][i_4] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3232152321_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
