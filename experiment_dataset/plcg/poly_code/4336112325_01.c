...
void kernel_4336112325_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, i_0); i_1++) {
            for (int i_2 = 2; i_2 < PB_Q; i_2++) {
                A[i_2][i_1][0] = A[i_2-2][i_1-1][0] + C[i_1][i_0+1][i_2] + C[i_1][i_0][i_2] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_Q; i_3++) {
        B[i_3+1] = B[i_3+2] * B[i_3+3] + 1;
        for (int i_4 = 1; i_4 < PB_Q-1; i_4++) {
            for (int i_5 = i_4; i_5 < PB_Q-1; i_5++) {
                C[i_3][i_5+1][i_4] = D[i_5][i_3+2][i_4] * C[i_3][i_4][i_5+1] + C[i_3][i_5+1][i_4-1] - C[i_3][i_5+3][i_4+1] + C[i_4+2][i_5][i_3] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4336112325_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
