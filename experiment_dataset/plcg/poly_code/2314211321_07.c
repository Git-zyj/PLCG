...
void kernel_2314211321_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        B[i_1] = D[i_1] - C[i_1+2][i_1+2] + C[i_1][i_1] - C[i_1-2][i_1-2] + 3;
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            C[i_1][i_1] = B[i_2] * B[i_2+1] - 6;
        }
    }
#pragma endscop
...
kernel_2314211321_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
