...
void kernel_3326252311_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0] = D[i_1] * E[i_1][i_0] - D[i_1] * E[i_1+1][i_0] + F[i_1+i_0][i_1+1] * E[i_1][i_0-2] - A[i_0+1] * A[i_1+1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            A[i_2] = F[i_2][i_3] + E[i_3][i_2] - A[i_3-1] * 2;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, PB_P); i_5++) {
            for (int i_6 = 2; i_6 < PB_P-2; i_6++) {
                A[i_6] = G[i_4][i_6][i_5] - G[i_5][i_4+2][i_6] * H[i_6-2][i_4][i_5] + F[i_5][i_6] + H[i_6][i_4+2][i_6+2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3326252311_06(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
