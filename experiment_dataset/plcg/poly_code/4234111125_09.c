...
void kernel_4234111125_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < min(i_1, -i_0+PB_L-1); i_2++) {
                B[i_2+1][i_2+i_0+1] = B[i_1+1][i_1+i_0+1] + 6;
            }
        }
        for (int i_3 = i_0; i_3 < -i_0+PB_L-1; i_3++) {
            B[i_0+1][i_3+i_0+1] = C[i_3][i_0] * 6;
        }
    }
#pragma endscop
...
kernel_4234111125_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
