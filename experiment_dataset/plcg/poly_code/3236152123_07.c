...
void kernel_3236152123_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            B[i_1][i_0-1] = C[i_1] * D[i_1-2][i_1-2] + D[i_1][i_1+i_0] - D[i_0][i_1] - D[i_1][i_0] + D[i_1][i_0] + D[i_1][i_0] + B[i_1][i_0+1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_3][i_2-1] = D[i_2][i_3] * D[i_2-1][i_3] * D[i_3][i_2+2] + 3;
        }
    }
#pragma endscop
...
kernel_3236152123_07(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
