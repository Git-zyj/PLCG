...
void kernel_3124332111_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_P-2, i_1); i_2++) {
                A[i_0][i_2+i_1] = B[i_1][i_0][i_1] + C[i_1][i_0][i_1+1] * D[i_1+i_0-1][i_2+i_0+2][i_1] + A[i_0][i_2+i_1-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3124332111_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
