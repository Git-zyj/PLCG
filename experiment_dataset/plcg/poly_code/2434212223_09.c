...
void kernel_2434212223_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-3; i_0++) {
        A[i_0+1][i_0+1] = E[i_0] * A[i_0+3][i_0+3] * 2;
    }
    for (int i_1 = 3; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 3; i_2 < min(PB_N-3, i_1); i_2++) {
            B[i_1-1][i_2+1] = B[i_1-3][i_2+1] + B[i_1-1][i_2+3] - B[i_2-3][i_1] + B[i_1+1][i_2+1] * B[i_2-1][i_1] + B[i_1-1][i_2-1] + B[i_1+1][i_2] * 1;
        }
    }
...
kernel_2434212223_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
