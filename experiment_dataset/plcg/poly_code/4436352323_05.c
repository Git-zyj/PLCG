...
void kernel_4436352323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = E[i_0] + E[0] * E[i_0+2] + F[i_0+2] + G[i_0][i_0+2] * E[i_0+2] + A[i_0-2] * 1;
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                for (int i_3 = 1; i_3 < min(min(-i_0+PB_M, -i_1+PB_P-2), i_2); i_3++) {
                    B[i_1+i_0][i_3][i_1] = H[i_1+2][i_3-1][i_0][i_2] + H[i_1][i_3+1][i_0][i_2] * H[i_1+2][i_0][i_3+1][0] * I[i_1-2][i_3][i_1][i_0-1] - I[i_1+1][i_2+2][i_3][i_1+i_0] - H[i_2][i_3][i_0-1][i_2] + J[i_1-1][i_2][i_0+2][i_3] - I[i_3+i_0][i_3+i_1+2][i_2][i_1-1] - B[i_2+i_1][i_3+1][i_2] * 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_M; i_5++) {
            C[i_5][i_4] = G[i_5][i_4] + K[i_5-2] * G[i_4][i_4+1] * A[i_5] + C[i_5-1][i_4+1] * C[i_5][i_4+2] * C[i_5][i_4+1] * C[i_5-2][i_4] + 3;
        }
    }
    for (int i_6 = 4; i_6 < PB_L; i_6++) {
        for (int i_7 = 3; i_7 < PB_M; i_7++) {
            D[i_6-2][i_7-1] = F[i_6-2] - L[i_7][i_6] - M[i_7][i_6] - D[i_6][i_7-1] * D[i_6-2][i_7-3] + D[i_6-3][i_7-2] + D[i_6-4][i_7-1] + 5;
        }
    }
#pragma endscop
...
kernel_4436352323_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M));
