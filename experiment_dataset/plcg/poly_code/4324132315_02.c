...
void kernel_4324132315_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_L, -i_1+PB_L); i_2++) {
                A[i_1][i_2+i_1][1] = D[i_1][i_2+i_0][i_2+i_1-1][i_1] + E[i_1][i_2] * A[i_1][i_2+i_1-1][1] - 6;
            }
        }
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 2; i_4 < i_0; i_4++) {
                B[i_4][i_0][i_3] = E[i_4-2][i_3] * F[i_3+1][i_4][0] * B[i_4][i_0+1][i_3] - B[i_3][i_0][i_4-1] * 6;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_N-1; i_5++) {
        for (int i_6 = 0; i_6 < PB_L; i_6++) {
            for (int i_7 = max(i_6, 1); i_7 < -i_5+PB_P; i_7++) {
                for (int i_8 = 2; i_8 < min(min(min(min(-i_6+PB_L, PB_N-1), i_6), -i_5+PB_L), -i_7+PB_L); i_8++) {
                    C[i_7][i_7][i_8+i_7] = D[i_8-2][i_6][i_7+i_5][i_8] * D[i_8][i_5][i_6][i_7] - F[i_6][i_8][i_7+i_5-2] - D[i_7][i_5][i_8][i_6] - A[i_5+1][i_8+i_5][1] - A[i_8-1][i_8+i_7-1][1] * C[i_7-1][i_7-1][i_8+i_7-1] * C[i_8+1][i_8+1][i_8+i_6] + 2;
                }
            }
        }
    }
...
kernel_4324132315_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
