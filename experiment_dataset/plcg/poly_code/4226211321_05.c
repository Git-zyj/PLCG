...
void kernel_4226211321_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = PB_P-1 - 1; i_1 >= i_0; i_1--) {
            for (int i_2 = max(2, i_0); i_2 < PB_P+1; i_2++) {
                for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
                    A[i_0][i_3-1][i_2-1][i_1+1] = A[i_3+2][i_1-1][i_2-2][i_0+1] + A[i_0][i_3-1][i_2-2][i_1+1] * 5;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            B[i_0][i_4] = C[i_0+1] * B[i_0][i_4-1] + B[i_0+1][i_4] * B[i_0-1][i_4] * 1;
        }
    }
#pragma endscop
...
kernel_4226211321_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
