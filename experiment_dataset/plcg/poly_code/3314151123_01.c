...
void kernel_3314151123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0] = D[i_1] * D[i_1] - D[i_1+1] * D[i_1] * A[i_1+i_0-1] * 3;
        }
        B[i_0][1] = E[i_0][0] * D[i_0] * D[i_0] + B[i_0-1][1] + 2;
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M; i_3++) {
            A[i_3+i_2] = D[0] + D[i_3] * E[i_2+1][i_3+1] * E[i_3][i_2+1] + D[i_3] * D[i_3] * E[i_2-1][i_3] - 5;
        }
    }
#pragma endscop
...
kernel_3314151123_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
