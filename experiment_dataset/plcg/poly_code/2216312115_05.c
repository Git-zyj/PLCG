...
void kernel_2216312115_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 3; i_1 < PB_N; i_1++) {
            A[0][i_1-2] = C[i_1+1][i_0] + A[0][i_1-3] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        for (int i_3 = i_2; i_3 < PB_N; i_3++) {
            A[0][i_2-2] = D[i_3][i_2] - 5;
        }
    }
#pragma endscop
...
kernel_2216312115_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
