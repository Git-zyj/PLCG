...
void kernel_3226112221_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = C[i_0-1][0] + A[i_0+1] * A[i_0+4] + 2;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            B[i_1][i_2] = C[i_2][i_1] * B[i_1+2][i_2] * 2;
        }
    }
#pragma endscop
...
kernel_3226112221_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
