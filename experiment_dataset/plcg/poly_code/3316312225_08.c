...
void kernel_3316312225_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 3; i_2 < min(PB_M+3, i_1); i_2++) {
                A[i_0-1][i_2][i_1] = D[i_2][i_0][i_2] * A[i_2-3][i_0-1][i_1] * 3;
            }
        }
        B[i_0+2] = E[i_0] * 3;
    }
    for (int i_3 = 3; i_3 < PB_M+3; i_3++) {
        for (int i_4 = i_3; i_4 < PB_Q-2; i_4++) {
            for (int i_5 = 1; i_5 < -i_3+PB_P-2; i_5++) {
                C[i_5][i_5+i_3+2][i_4] = A[i_3-3][i_5-1][i_4] * C[i_5][i_5+i_3+2][i_4-1] * C[i_5][i_5+i_3+2][i_4+2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3316312225_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
