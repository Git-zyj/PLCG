...
void kernel_3434351213_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_N+1, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_M-1; i_2++) {
                A[i_2][i_1] = E[i_2+i_1-1][i_1][i_0] + E[i_2][i_0+1][i_1] - F[i_0-1][i_2] * G[i_2+i_1+1][i_0-1] * H[i_1][i_0][i_2-1] + H[i_2][i_0][i_1] * I[i_2-1][i_0][i_1+1] * F[i_1+i_0-1][i_2] - E[i_1][i_2][i_0-1] + A[i_2-1][i_1] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M, PB_M-1); i_4++) {
            for (int i_5 = 1; i_5 < -i_3+PB_M; i_5++) {
                B[i_5-1][i_4-1][i_3] = I[i_3+1][i_5+1][i_5] + E[i_5][i_4][i_3] + H[i_4+i_3][i_3+1][i_4] * I[i_5+i_3][i_5][i_4-1] - A[i_5][i_4+1] * 5;
            }
            for (int i_6 = 1; i_6 < PB_P; i_6++) {
                C[i_6+1][i_4-1] = I[i_3+1][i_4][i_4] + H[i_3+1][i_6][i_4] * G[i_6-1][i_4-1] + A[i_4][i_6] - A[i_4][i_6+1] + A[i_4+1][i_6+1] - 1;
            }
        }
        for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
            D[i_3+1][i_7] = J[i_7+1] - G[i_3+1][i_7-1] * K[i_7][i_3] - G[i_7][i_3] + 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3434351213_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
