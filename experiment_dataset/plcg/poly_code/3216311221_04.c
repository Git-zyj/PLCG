...
void kernel_3216311221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < PB_Q; i_3++) {
            for (int i_4 = 3; i_4 < min(i_3, -i_2+PB_P); i_4++) {
                B[i_4-1][i_3][i_2] = C[i_3][i_4+i_2][i_4+1] - B[i_4-3][i_3][i_2-1] * B[i_3-2][i_4][i_2] * B[i_4-3][i_3-1][i_2] - B[i_4-1][i_3-2][i_2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3216311221_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
