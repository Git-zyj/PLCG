...
void kernel_2432232223_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] * A[i_0+2] - B[i_0] * 1;
        B[i_0] = E[i_0] * F[0] * F[i_0] + F[i_0] - B[i_0-2] + B[i_0+2] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            A[i_2] = G[i_1][i_2] - H[i_1][i_2] * H[i_2][i_1+1] - 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        D[i_3] = E[i_3] * F[i_3-1] + E[i_3+1] - A[i_3] - D[i_3+2] - 5;
    }
#pragma endscop
...
kernel_2432232223_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
