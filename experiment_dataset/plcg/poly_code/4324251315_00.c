...
void kernel_4324251315_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,int vj,int wj,DATA_TYPE POLYBENCH_5D(J,xJ,yJ,zJ,vJ,wJ,xj,yj,zj,vj,wj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 2; i_3 < min(min(-i_2+PB_N+1, -i_0+PB_L), i_2); i_3++) {
                    A[i_0][i_2][i_1+1][i_3-1] = D[i_2+1][i_0-1][i_3][i_1-1] - D[i_2][i_0][i_3+i_1-1][i_0-1] * E[i_3][i_2][i_1+1][i_0] + D[i_2+1][i_1+1][i_3+i_0][i_2] + D[i_3+i_2-1][i_2][i_1+i_0][i_3] + D[i_1][i_0][i_2][i_3] - F[i_0][i_1][i_2][i_3][i_0-1] - E[i_2][i_1][i_3][i_0] + A[i_0][i_2+1][i_1][i_3] * A[i_0][i_2][i_1+1][i_3] - A[i_3][i_2+1][i_1+2][i_0] - A[i_0][i_2][i_1+1][i_3-2] + 2;
                }
            }
        }
        B[i_0] = G[i_0] - G[i_0] * H[i_0][i_0] * G[i_0] + I[i_0] * I[i_0] * B[i_0-1] - B[i_0+1] + 2;
    }
    for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < PB_L; i_5++) {
            for (int i_6 = 0; i_6 < PB_Q-1; i_6++) {
                for (int i_7 = 0; i_7 < PB_L; i_7++) {
                    A[i_7][i_4+1][i_6+1][i_5-1] = J[i_5][i_4][i_7][i_5][1] - 6;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4324251315_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J)));
