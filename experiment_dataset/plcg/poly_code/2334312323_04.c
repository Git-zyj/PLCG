...
void kernel_2334312323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L-1; i_4++) {
            C[i_3][i_4] = D[i_4] + C[i_4+1][i_3+1] * C[i_3][i_4+1] - C[i_3][i_4-1] + C[i_3-2][i_4] + 3;
        }
    }
#pragma endscop
...
kernel_2334312323_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
