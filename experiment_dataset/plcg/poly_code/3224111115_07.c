...
void kernel_3224111115_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(PB_L-1, -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_L-1, -i_0+PB_P); i_2++) {
                B[i_0][i_2+i_1+1][i_2+i_0] = 1;
            }
            for (int i_3 = 0; i_3 < min(i_0, -i_1+PB_L); i_3++) {
                B[i_0][i_3+i_1][i_1+i_0] = C[i_3][i_1] - B[i_0][i_3+i_1-2][i_1+i_0-1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3224111115_07(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
