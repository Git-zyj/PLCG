...
void kernel_2324151325_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] * D[i_0] + D[i_0] + D[i_0] - D[i_0-1] * D[i_0] - D[i_0+1] - D[i_0] + A[i_0+1] - A[i_0+2] - 3;
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            B[i_0][i_1] = E[i_0][i_1] - E[i_0][i_1+i_0] + A[i_1] + B[i_1][i_0-1] + B[i_0][i_1+1] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_M-1; i_3++) {
            A[i_3+1] = D[i_3] + E[i_3+1][i_3] + E[i_3-1][i_3] + 6;
        }
    }
#pragma endscop
...
kernel_2324151325_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
