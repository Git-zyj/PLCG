...
static void print_array(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn),int xo,int yo,int zo,DATA_TYPE POLYBENCH_3D(O,xO,yO,zO,xo,yo,zo))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4434351215_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn),int xo,int yo,int zo,DATA_TYPE POLYBENCH_3D(O,xO,yO,zO,xo,yo,zo)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[i_1][i_1+i_0] = E[i_1][i_0] * F[i_1][i_0] - F[i_1-1][i_0-1] * G[i_1+1][i_0] * G[i_0+1][i_1] + E[i_1][i_0] * F[i_0][i_0] - G[i_1][i_0] + C[0] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M-1, i_3); i_4++) {
                for (int i_5 = 0; i_5 < min(-i_3+PB_N+1, -i_2+PB_P); i_5++) {
                    D[i_5+i_3-1][i_2][i_2+1][1] = H[i_3+1][i_5][i_2-1][i_5] * I[i_4+i_3+1][i_5][i_5+i_2] - 1;
                }
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
        C[0] = J[i_6] - J[0] + K[i_6] + K[i_6-1] - L[i_6] + K[i_6+1] - C[0] * C[0] * 4;
        for (int i_7 = i_6; i_7 < PB_M; i_7++) {
            for (int i_8 = 2; i_8 < PB_M; i_8++) {
                for (int i_9 = 0; i_9 < min(i_6, -i_8+PB_N); i_9++) {
                    D[i_9+i_8-1][i_6][i_6+1][1] = M[0][i_6][i_7][i_9] + N[i_9][i_7][i_6] - O[i_9][i_7][i_8] * D[i_9+i_8][i_6][i_6+1][1] * D[i_9+i_8-2][i_6][i_6+1][1] * D[i_9+i_8-1][i_6][i_6+1][1] - 5;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N), xo,yo,zo,POLYBENCH_ARRAY(O), INIT_SEED);
kernel_4434351215_02(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N), xo,yo,zo,POLYBENCH_ARRAY(O));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N), xo,yo,zo,POLYBENCH_ARRAY(O)));
