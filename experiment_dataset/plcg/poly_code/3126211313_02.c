...
void kernel_3126211313_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_0][i_1][i_2+1] = B[i_1][i_2+1] + A[i_0][i_1+1][i_2+1] - A[i_0+1][i_1-1][i_2+1] + A[i_2][i_1-1][i_0] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3126211313_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
