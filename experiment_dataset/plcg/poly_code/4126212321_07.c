...
void kernel_4126212321_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 0; i_1 < min(min(PB_L-3, i_0), -i_0+PB_P); i_1++) {
            for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
                for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
                    A[i_0][i_1+2][i_2][i_3+1] = B[i_2-1][i_1][i_1+i_0] + A[i_3+2][i_1+2][i_2+2][i_0+1] * A[i_0][i_1+2][i_2][i_3+2] * A[i_3-2][i_1+3][i_2][i_0+1] - 3;
                }
            }
        }
    }
...
kernel_4126212321_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
