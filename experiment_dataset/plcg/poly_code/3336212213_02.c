...
void kernel_3336212213_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_P-1); i_1++) {
            A[i_0][i_1] = D[i_1][i_0] - A[i_0+1][i_1] * A[i_0-1][i_1-1] - A[i_0][i_1+1] + 2;
        }
    }
    ...
    for (int i_5 = 2; i_5 < PB_M+2; i_5++) {
        for (int i_6 = i_5; i_6 < min(-i_5+PB_L, PB_M+2); i_6++) {
            C[i_5-2][i_6] = D[i_6+i_5][i_6] + C[i_6-2][i_5-1] * 5;
        }
    }
#pragma endscop
...
kernel_3336212213_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
