...
void kernel_2226312221_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = max(1, i_0); i_2 < PB_M; i_2++) {
            B[i_2][i_0] = C[i_2][i_0] * B[i_2-1][i_0] * B[i_0][i_2+2] - 4;
        }
    }
#pragma endscop
...
kernel_2226312221_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
