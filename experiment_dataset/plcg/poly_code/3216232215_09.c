...
void kernel_3216232215_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_L; i_1++) {
            A[i_1-2] = C[i_1] - A[i_1-3] * A[i_1-1] * A[i_1-1] - 1;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_0][i_2] = D[i_0][i_2] - E[i_2][i_0] - E[i_2][i_0] * D[i_2][i_0] - B[i_0-1][i_2-1] + 2;
        }
    }
#pragma endscop
...
kernel_3216232215_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
