...
void kernel_2232151321_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_L, -i_0+PB_M+1); i_1++) {
            B[i_1+i_0-1][i_1-1] = C[i_1][0] * C[i_0][i_1-1] - C[i_1][0] * C[i_1+i_0-1][i_0] + C[i_1][i_0+1] * C[i_1][i_0-1] + 5;
        }
    }
    for (int i_2 = 3; i_2 < PB_L; i_2++) {
        for (int i_3 = 3; i_3 < -i_2+PB_L; i_3++) {
            B[i_3+i_2-1][i_3-1] = D[i_3] - D[i_3] + C[i_3][i_3+i_2] + D[i_3] * B[i_3+i_2-2][i_3-2] + B[i_3+i_2-3][i_2-1] - B[i_3+i_2-3][i_3-3] - 3;
        }
    }
#pragma endscop
...
kernel_2232151321_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
