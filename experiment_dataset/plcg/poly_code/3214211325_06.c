...
void kernel_3214211325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+i_0][i_1-1] = C[i_0][i_1+i_0+1] - D[i_0-1] * A[i_1+i_0+2][i_1-1] * B[i_0][i_1+3] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < -i_2+PB_M; i_3++) {
            B[i_2][i_3+1] = A[i_3+i_2][i_3-1] - A[i_3+i_2-2][i_3-3] * B[i_2][i_3-1] + 3;
        }
    }
#pragma endscop
...
kernel_3214211325_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
