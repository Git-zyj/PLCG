...
void kernel_3232312215_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_P+2; i_1++) {
            for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
                A[i_2][1][i_2+1] = C[i_1][1][i_1-2] + A[i_2+1][1][i_2+2] * A[i_2-1][1][i_2] + 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N+2; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_N+2, PB_N-2); i_5++) {
                A[i_5][1][i_5+1] = D[i_5+2][i_5+i_3-2][i_4] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3232312215_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
