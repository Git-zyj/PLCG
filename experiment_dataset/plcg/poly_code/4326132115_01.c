...
void kernel_4326132115_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = i_1; i_3 < PB_Q+1; i_3++) {
                    A[i_2][i_1][i_0][i_1] = D[i_1-1][i_1][i_3-2][i_0] + D[i_2][i_1][i_0][i_3-1] * 3;
                }
            }
        }
        B[i_0-2] = E[i_0] + E[i_0+1] - E[i_0] - B[i_0-1] - 4;
    }
    for (int i_4 = 2; i_4 < PB_P; i_4++) {
        for (int i_5 = i_4; i_5 < -i_4+PB_L; i_5++) {
            for (int i_6 = 0; i_6 < min(i_4, -i_5+PB_L); i_6++) {
                A[i_5][i_4-1][i_6+1][i_4-1] = F[i_6+i_5][i_4][i_4-2] * F[i_6][i_5+i_4][0] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4326132115_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
