...
void kernel_3324312213_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 3; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            B[i_3+2] = D[i_2-1][i_3] * D[0][i_2+1] + C[i_3-2] + C[i_3-1] + 5;
        }
...
kernel_3324312213_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
