...
void kernel_3222211323_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q+2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_Q, -i_0+PB_L); i_1++) {
            A[i_0+1][i_1+i_0][i_1] = C[i_0] * A[i_1+1][i_1+i_0-2][i_0-2] - A[i_0+1][i_1+i_0-2][i_1-2] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
        B[i_2] = B[i_2+2] - B[i_2-1] + B[i_2+1] * B[i_2-2] * 5;
    }
#pragma endscop
...
kernel_3222211323_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
