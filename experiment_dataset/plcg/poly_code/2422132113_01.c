...
void kernel_2422132113_01(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        C[i_0-1] = E[i_0+2] * E[i_0] * 2;
    }
    for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
        for (int i_2 = min(-i_1+PB_P+1, PB_L-1) - 1; i_2 >= 0; i_2--) {
            B[i_2+1][i_2+i_1-2] = B[i_1+2][i_2+i_1-1] + 5;
        }
        C[i_1] = E[i_1] * E[i_1+1] * E[i_1] * E[i_1+1] + 6;
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        C[i_3] = E[i_3] * E[i_3] - E[i_3-1] - 3;
    }
#pragma endscop
...
kernel_2422132113_01(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E));
