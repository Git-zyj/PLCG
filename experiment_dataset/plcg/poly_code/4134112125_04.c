...
void kernel_4134112125_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_Q+2; i_3++) {
                    A[i_2][i_3+2][i_1] = B[i_2][i_1-1][i_3+i_0-2] - A[i_1][i_3+2][i_2+2] * 2;
                }
            }
        }
    }
...
kernel_4134112125_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
