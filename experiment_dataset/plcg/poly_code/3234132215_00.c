...
void kernel_3234132215_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, PB_L); i_2++) {
                A[i_2+i_0][i_1][i_1] = C[i_0-1][i_1] * D[i_1][i_0][i_1] - C[i_2][i_2] * A[i_1+i_0][i_2-1][i_2-1] - A[i_2+i_0-1][i_1][i_1] + 4;
            }
        }
        for (int i_3 = i_0; i_3 < -i_0+PB_L-2; i_3++) {
            for (int i_4 = 1; i_4 < PB_M; i_4++) {
                B[i_3-1][i_0][i_4] = D[i_3][i_4-1][i_3+i_0+2] - C[i_4][i_3+i_0] - D[i_3][i_4][i_0] * B[i_0-2][i_3][i_4] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3234132215_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
