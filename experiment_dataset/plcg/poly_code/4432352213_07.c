...
void kernel_4432352213_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        A[i_0] = E[i_0] - F[i_0-2] * F[i_0] * E[i_0-1] - 6;
        for (int i_1 = 2; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_N; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_P-2; i_3++) {
                    B[i_2][i_3][i_0+2][i_1] = G[i_1][i_2+2][i_0-2][i_3+2] + H[i_3][i_0-2][i_3][i_2] + I[i_3+i_2][i_0][i_1-2][i_1] + G[i_2][i_0-2][i_3+i_1+2][i_1+1] - G[i_3+1][i_2][i_0][i_1] * I[i_3][i_2+i_0][i_1][i_0+2] * B[i_2+1][i_3][i_0+2][i_1] - B[i_2+1][i_3][i_0+1][i_1+1] * B[i_2+1][i_3][i_0+1][i_1] - 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
            C[0][i_5] = J[i_5-2] * F[i_5] - K[i_5-1][i_4] + L[i_5+2][i_4+1] + K[i_5-2][i_4-2] * F[i_5] * C[0][i_5+1] * 4;
        }
        for (int i_6 = 1; i_6 < -i_4+PB_N; i_6++) {
            D[i_4+1][i_6] = F[i_6+1] + M[i_6+i_4][i_4] - L[i_6+2][i_4] + K[i_6][i_4] * D[i_4+1][i_6-1] * D[i_4+2][i_6] * 2;
        }
    }
#pragma endscop
