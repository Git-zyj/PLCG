...
void kernel_4216311123_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        ...
        for (int i_3 = 0; i_3 < min(PB_N, i_0); i_3++) {
            B[i_0][i_3] = C[i_3][i_0+1] + B[i_0][i_3+2] * B[i_0+2][i_3+2] - 1;
        }
    }
#pragma endscop
...
kernel_4216311123_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
