...
void kernel_2214131225_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L, -i_0+PB_M+1); i_1++) {
            B[i_0-2][i_1] = C[i_1+i_0-1] + D[i_0+1][i_1] * 2;
        }
        for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
            B[i_2][i_0] = D[i_0-1][i_2-1] + D[i_0][0] + D[i_0][i_2-1] * B[i_2][i_0-2] + B[i_2+1][i_0+1] * 4;
        }
    }
#pragma endscop
...
kernel_2214131225_09(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
