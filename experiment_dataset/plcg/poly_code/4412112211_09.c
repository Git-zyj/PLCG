...
void kernel_4412112211_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
    ...
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_Q+2; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_Q-2; i_2++) {
                B[i_0][i_2+2][i_1-1] = F[i_2+1][i_1+i_0-2][i_2+i_0+2] + B[i_0][i_2+1][i_1-1] - 5;
            }
        }
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
                for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
                    C[i_3][i_4][i_5] = G[i_5+1][i_4+i_3][i_0][i_0+2] * B[i_4][i_5+1][i_3-2] * C[i_5][i_4-1][i_3] + C[i_3][i_4-1][i_5] + 4;
                }
                for (int i_6 = 2; i_6 < PB_L; i_6++) {
                    C[i_4][i_6][i_3-1] = G[i_4-1][i_3][i_6-2][0] * 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4412112211_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
