...
void kernel_3336331215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0] - E[i_0-1] * B[i_0-1] - 3;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        B[i_1-1] = F[i_1] + E[i_1] - F[i_1] - A[i_1] * B[i_1] - 6;
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_2-2] = G[i_2][i_3] * H[i_2][1] + F[i_2] * H[i_2][i_3] - 3;
        }
    }
#pragma endscop
...
kernel_3336331215_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
