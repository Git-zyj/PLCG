...
void kernel_2422351325_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M+1; i_1++) {
            A[i_0][i_1] = E[i_0] * F[i_0+1][i_1] + G[i_1][i_0] * G[i_1][i_0-1] * H[i_1+i_0-1][0] + 2;
        }
        D[i_0-2] = I[1] + I[i_0] + E[i_0] * J[i_0] * C[i_0+1] + 1;
        C[i_0] = I[i_0] - E[0] + I[i_0+1] + J[i_0+1] - I[0] * C[i_0+2] - 2;
        for (int i_2 = i_0; i_2 < min(-i_0+PB_M, PB_N); i_2++) {
            D[i_2] = H[i_2][0] * G[i_0][i_2-1] * G[i_2-1][i_0-1] - G[i_2+i_0][i_2-1] - F[i_0][i_2+1] * H[i_2][i_0] + A[i_0+2][i_2] - A[i_0+1][i_2+2] + A[i_0+1][i_2] + A[i_2-2][i_0] - A[i_0-2][i_2+1] * 2;
        }
    }
#pragma endscop
...
kernel_2422351325_01(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
