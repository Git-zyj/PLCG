...
void kernel_2434132311_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_0] = E[i_0] * F[i_1][i_0-1] + A[i_0-1][i_0-1] + A[i_0+1][i_0+1] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
            B[i_2][i_2] = G[i_3-2][i_2][i_2] * F[i_3+2][i_2-2] + B[i_2-1][i_2-1] - C[i_2+1][i_3+1] - 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < min(-i_4+PB_M+1, PB_M-1); i_5++) {
            C[i_4+1][i_5] = F[i_5+i_4-1][i_4-1] * E[i_4] + C[i_4+1][i_5+1] - 1;
        }
    }
    for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
        for (int i_7 = 2; i_7 < min(-i_6+PB_M, PB_L-2); i_7++) {
            D[i_6][i_7] = F[i_7+i_6][i_6] + F[i_7+i_6-2][i_7+2] + E[i_7-2] + F[i_7][i_6] * D[i_6-1][i_7] + D[i_6][i_7-1] - D[i_6+1][i_7+1] * D[i_7][i_6+1] - 3;
        }
    }
#pragma endscop
...
kernel_2434132311_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
