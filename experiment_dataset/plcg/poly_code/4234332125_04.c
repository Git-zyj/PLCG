...
void kernel_4234332125_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_N); i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_N; i_2++) {
                B[i_1+i_0][i_2-1] = C[i_1+1][i_0-2][i_1-1] - D[i_2+i_0][i_0+1][i_1] - D[i_1-2][i_2+2][i_0] - D[i_1][i_2-1][i_0] + 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = i_3; i_4 < min(PB_N, -i_3+PB_Q); i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_N, -i_4+PB_N); i_5++) {
                B[i_5+i_4][i_3-1] = C[i_3][i_5][i_4+i_3] * C[i_5][i_4][i_3+1] * B[i_5+i_3][i_4] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4234332125_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
