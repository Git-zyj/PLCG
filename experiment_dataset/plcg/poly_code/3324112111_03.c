...
void kernel_3324112111_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            B[i_3+1][i_4] = D[i_4][i_3-1] - D[i_3][0] * 3;
        }
        for (int i_5 = i_3; i_5 < PB_L-3; i_5++) {
            for (int i_6 = 0; i_6 < min(-i_3+PB_M+1, i_5); i_6++) {
                C[i_6+i_3-1][i_5+2][i_6+1] = C[i_6+i_3-2][i_5+3][i_6+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3324112111_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
