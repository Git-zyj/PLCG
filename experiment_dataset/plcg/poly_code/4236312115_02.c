...
void kernel_4236312115_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_P-2, i_1); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_L-1, -i_0+PB_P); i_3++) {
                    A[i_3][i_1][i_2+2] = C[i_1][i_2+2][i_3+i_0][i_1] * B[i_2+1][i_1][i_3+i_2+1] * 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < min(min(-i_4+PB_L+1, PB_P), i_4); i_5++) {
            for (int i_6 = 0; i_6 < min(PB_N-2, -i_4+PB_L); i_6++) {
                B[i_4][i_5][i_6+i_4] = D[i_6+2][i_4][i_5] + B[i_4-1][i_6][i_5+i_4-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4236312115_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
