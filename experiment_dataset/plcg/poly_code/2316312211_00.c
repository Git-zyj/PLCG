...
void kernel_2316312211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = 4;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_1] = A[i_0] * A[i_0-1] - 3;
        }
    }
    for (int i_2 = PB_L-1 - 1; i_2 >= 0; i_2--) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            C[i_3] = D[i_3+1] * C[i_3-1] - C[i_2+1] * 3;
        }
    }
#pragma endscop
...
kernel_2316312211_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
