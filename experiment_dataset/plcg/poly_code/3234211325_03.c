...
void kernel_3234211325_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                A[i_1][i_1][i_2-1] = C[i_0][i_1][i_2+1] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_M-2, i_3); i_4++) {
            for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                A[i_5+1][i_5+1][i_3-1] = A[i_4][i_4][i_5+1] - A[i_4+2][i_4+2][i_5-1] + A[i_3+1][i_3+1][i_5-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3234211325_03(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
