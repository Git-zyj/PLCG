...
void kernel_4424212123_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                A[i_2][i_1+1][i_0] = A[i_0+2][i_1+1][i_2+1] + 1;
            }
            for (int i_3 = i_0; i_3 < PB_M-1; i_3++) {
                for (int i_4 = 2; i_4 < min(PB_Q, -i_1+PB_L); i_4++) {
                    B[i_1][i_4-2][i_3] = E[i_3+1][i_4+i_1][i_0+2][i_1+1][0] + 1;
                }
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_N-1; i_5++) {
        ...
        for (int i_7 = 0; i_7 < min(PB_M, -i_5+PB_L+2); i_7++) {
            for (int i_8 = 1; i_8 < i_7; i_8++) {
                for (int i_9 = max(0, -i_8+1); i_9 < min(PB_M, -i_8+PB_L+1); i_9++) {
                    D[i_8+2][i_7][i_9][i_7+i_5-2] = D[i_5+1][i_9][i_7][i_9+i_8-1] + 4;
                }
            }
        }
    }
...
kernel_4424212123_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E));
