...
void kernel_3326152311_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = D[i_0] - D[i_0+2] + D[i_0] - D[i_0] * D[i_0+1] + D[i_0] - D[i_0+2] - D[i_0] * A[i_0+1] + A[i_0-1] * 5;
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            B[i_0+2][i_1-1] = E[i_0-1][i_1] * E[i_0-2][i_1] * E[i_1][i_0+2] * E[i_1+1][i_0+1] - E[i_1-2][i_0] * A[i_1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            A[i_3-1] = E[i_3-1][i_2] * A[i_3-1] - A[i_3+1] + 3;
        }
    }
#pragma endscop
...
kernel_3326152311_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
