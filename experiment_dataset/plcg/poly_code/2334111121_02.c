...
void kernel_2334111121_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 3; i_5 < PB_P+1; i_5++) {
            C[i_4][i_5-1] = D[i_5][i_4] - C[i_4][i_5-3] + 4;
        }
    }
#pragma endscop
...
kernel_2334111121_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
