...
void kernel_3422311313_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_0][i_1][i_1+1] = B[i_1][i_0+1] * B[i_1+1][i_0+1] - 6;
            for (int i_2 = i_1; i_2 < PB_L-1; i_2++) {
                B[i_1][i_2+1] = A[i_1-1][i_2+1][i_2+2] + A[i_0][i_1+1][i_1+2] + 4;
            }
        }
        C[i_0] = E[i_0] + C[i_0+1] - C[i_0-1] * 1;
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_M; i_4++) {
            D[i_4-1][i_3+1] = F[i_3] + C[i_4] - D[i_4-2][i_3+2] * D[i_4][i_3+2] - D[i_4-1][i_3+2] * 3;
        }
    }
#pragma endscop
...
kernel_3422311313_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
