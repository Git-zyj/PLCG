...
void kernel_3412352225_09(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        D[i_0-1] = E[i_0-1] * E[i_0] - E[i_0+2] * E[i_0] * E[i_0] * 1;
        for (int i_1 = 1; i_1 < min(PB_Q, -i_0+PB_P); i_1++) {
            for (int i_2 = 2; i_2 < min(min(PB_Q, -i_1+PB_L), -i_0+PB_L+1); i_2++) {
                B[i_2][i_1][i_0] = F[i_2][i_0][i_1] * F[i_1][i_0][i_2] * G[i_1][i_2][i_0+1] * F[i_2-2][i_0][i_1] * F[i_2+i_1][i_0+2][i_1] + F[i_2-1][i_2+1][i_1] * F[i_2+i_0-1][i_2][i_1-1] - F[i_2][i_1+i_0][i_0] * B[i_2][i_1+1][i_0] * 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_Q+2; i_3++) {
        for (int i_4 = 0; i_4 < PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                B[i_5][i_3][i_4] = H[i_5][i_4] * F[i_4+1][i_5][i_3-2] * 1;
            }
        }
        D[i_3] = I[i_3] * E[i_3+2] * J[i_3] - D[i_3+2] * D[i_3-1] - 4;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3412352225_09(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
