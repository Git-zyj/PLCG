...
void kernel_4316152321_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < min(PB_N-2, i_0); i_1++) {
            for (int i_2 = PB_M-2 - 1; i_2 >= 2; i_2--) {
                for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
                    A[i_0][i_1][i_2-2][i_1-1] = D[i_1-2][i_2][i_3+1] + E[i_3][i_0][i_1][i_2] * E[i_1+2][i_2][i_0][i_3-1] - D[i_2+2][i_0][i_1+1] * E[i_1][i_3+i_2][i_2][i_1-1] - A[i_0-1][i_1][i_2-2][i_1-1] * A[i_2][i_1][i_0][i_1-1] + 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 1; i_5 < PB_L-2; i_5++) {
            B[i_5][i_4] = F[i_5-1][i_4] * G[i_5] * F[i_5][i_4+2] - F[i_4+2][i_5+2] + B[i_5-1][i_4] * C[i_4+1][i_4+1] - 2;
            C[i_4][i_4] = G[i_5+1] * F[i_5][i_4] * G[i_5] + F[i_5][i_4] + G[i_4] + F[i_5][i_4] * C[i_4-2][i_4-2] + C[i_4+2][i_4+2] + C[i_4+1][i_4+1] + 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4316152321_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G)));
