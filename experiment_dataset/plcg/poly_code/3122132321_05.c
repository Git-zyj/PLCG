...
void kernel_3122132321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-4 - 1; i_0 >= 1; i_0--) {
        for (int i_1 = 0; i_1 < min(PB_Q, -i_0+PB_L-2); i_1++) {
            A[i_1+2][i_0+2][i_0] = B[i_1+i_0+2][i_0][i_1] + C[i_1][i_0] + C[i_1][0] - A[i_1+3][i_0+4][i_0+2] * A[i_1][i_0+4][i_0+2] + A[i_1+1][i_0+1][i_0-1] - 6;
        }
    }
#pragma endscop
...
kernel_3122132321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
