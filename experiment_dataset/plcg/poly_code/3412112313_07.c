...
void kernel_3412112313_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            A[i_1][i_0-1] = A[i_1][i_0] - A[i_1-1][i_0-1] + A[i_1][i_0-2] + 4;
        }
        for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 3; i_3 < PB_M-1; i_3++) {
                B[i_2][i_3-2] = B[i_2-1][i_3-2] * B[i_3+1][i_2-2] - B[i_2+1][i_3-3] - 3;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
        ...
        for (int i_5 = 1; i_5 < PB_N-2; i_5++) {
            D[i_4+1][i_5] = D[i_4+1][i_5+1] * D[i_4+1][i_5-1] + D[i_5+2][i_4] + 5;
        }
    }
#pragma endscop
...
kernel_3412112313_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
