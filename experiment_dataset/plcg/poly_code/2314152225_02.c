...
void kernel_2314152225_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        A[i_0-2] = D[i_0] + D[i_0] * D[i_0] - E[i_0-1][0] - A[i_0-3] * A[i_0-1] - 3;
    }
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            B[i_1][i_2] = D[i_1+2] * E[i_2][i_1] - E[i_2][i_1] * E[i_2][i_1] - E[i_2][i_1] - E[i_2][i_1-2] * A[i_2-2] - B[i_1][i_2-1] - 6;
        }
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            B[i_1][i_3] = D[i_3] * E[i_3][i_1] * E[i_3+1][i_1-1] - 6;
        }
    }
#pragma endscop
...
kernel_2314152225_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
