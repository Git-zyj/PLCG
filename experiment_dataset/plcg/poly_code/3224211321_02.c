...
void kernel_3224211321_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                A[i_1][i_2][1] = A[i_2][i_0-1][1] * A[i_1+1][i_2][1] * 5;
            }
        }
        for (int i_3 = 0; i_3 < PB_N-3; i_3++) {
            for (int i_4 = 2; i_4 < min(-i_3+PB_N, PB_L-2); i_4++) {
                B[i_0+1][i_4][i_0] = C[i_3][i_4][i_0] * C[i_4+i_3][i_0][0] * B[i_0-1][i_4][i_0-2] * B[i_3+3][i_4-2][i_3+2] * B[i_0][i_4+2][i_0-1] - B[i_0-1][i_3][i_0-2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3224211321_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
