...
#endif
static void init_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int seed)
{
srand(seed);
int i_0, i_1, i_2, i_3;
...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4424351215_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = i_1; i_2 < min(-i_1+PB_L-1, -i_0+PB_M-3); i_2++) {
                for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
                    A[i_2+i_0+1][i_1][i_3+1][i_1] = E[i_2+i_1][0][i_3+1][i_0+1] + E[i_3][i_1+1][i_0][i_1] * F[i_2][i_0][i_0-1][i_1] + F[i_0+1][i_3][i_2+i_1+1][i_3-1] - A[i_2+i_0+3][i_1][i_3+1][i_1] - 1;
                }
            }
        }
        for (int i_4 = 1; i_4 < -i_0+PB_L-1; i_4++) {
            for (int i_5 = i_0; i_5 < min(-i_0+PB_Q, PB_M-1); i_5++) {
                B[i_5][i_0] = G[i_5-1][i_4+i_0+1] - H[i_4+1][i_0][i_5+i_0] * H[i_4][i_0-1][i_5+1] + I[i_0][i_5][i_4] * J[i_5][i_0][i_4+1] * J[i_5+1][i_4][i_0] + B[i_5][i_4-1] - B[i_5-1][i_4] + B[i_5+1][i_4+1] * 3;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_P; i_6++) {
        for (int i_7 = 1; i_7 < min(i_6, PB_M-1); i_7++) {
            for (int i_8 = 1; i_8 < PB_M; i_8++) {
                B[i_7-1][i_8] = I[i_8][i_6][i_7-1] * G[i_8][i_6] - J[i_7+1][i_8-1][i_6] * 5;
            }
        }
    }
    for (int i_9 = 0; i_9 < PB_L-1; i_9++) {
        for (int j_0 = i_9; j_0 < PB_M-1; j_0++) {
            B[j_0][i_9] = K[j_0][i_9] * L[j_0] * K[j_0+1][i_9] * M[i_9][j_0] * G[j_0+1][i_9+1] + M[i_9][j_0] - G[i_9][0] - 1;
        }
    }
#pragma endscop
...
kernel_4424351215_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M));
