...
void kernel_3212152313_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_M-2; i_1++) {
            B[i_1-1][i_0-1][i_1+1] = C[i_1-2][i_1+1] * C[i_1+2][i_0+1] + C[i_1][i_0] + C[i_1+i_0+2][i_1+i_0] + C[i_1+1][i_0] * C[i_1-1][i_0] - 1;
        }
        for (int i_2 = max(3, i_0); i_2 < PB_M; i_2++) {
            B[i_2-2][i_0][i_2] = C[i_2][i_0-1] - C[i_0+2][i_0] * C[i_2-1][i_0+1] - C[i_2-2][i_0] - B[i_2-2][i_0-1][i_2] - B[i_2-2][i_0+1][i_2] * B[i_2-3][i_0-1][i_2-1] - 4;
        }
    }
#pragma endscop
...
kernel_3212152313_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
