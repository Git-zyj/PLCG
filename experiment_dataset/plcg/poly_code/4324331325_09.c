...
void kernel_4324331325_09(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_0+PB_M, -i_2+PB_P); i_3++) {
                    C[i_3][i_1+i_0+2][i_2][i_0-1] = D[i_3][i_0][i_1-1][i_2] * E[i_2+1][i_0][i_1-1][i_3-1] + E[i_3][i_0][i_1][i_3+i_2] * F[i_1+i_0+1][i_3][i_2][i_2] - D[i_1][i_3+i_0][i_0][i_2+1] + C[i_1][i_3+i_0][i_2][i_0-1] - 4;
                }
            }
        }
    }
        ...
        for (int i_5 = i_4; i_5 < PB_L; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 1; i_7 < min(PB_Q, -i_5+PB_M); i_7++) {
                    C[i_6+1][i_7+i_5][i_4][i_7-1] = F[i_5][i_6][i_7][i_4+1] * 1;
                }
            }
        }
    }
        ...
        for (int i_9 = 1; i_9 < -i_8+PB_M; i_9++) {
            for (int j_0 = i_8; j_0 < PB_L; j_0++) {
                for (int j_1 = 0; j_1 < i_9; j_1++) {
                    C[j_0][i_9+i_8][j_1][i_8-1] = F[j_0][i_8][j_1][i_8] * C[j_0+1][j_1+i_9][i_8][i_9-1] * C[j_1][j_0+i_9][i_8+2][i_9-1] + 4;
                }
            }
        }
    }
...
kernel_4324331325_09(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
