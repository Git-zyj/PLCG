...
void kernel_4422331315_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
        ...
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_N; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_M-1; i_3++) {
                    A[i_0-1][i_0-1][i_2][i_1] = E[i_3+i_1+1][i_2][i_0][i_2+1] + F[i_2+i_1][i_1][i_3][i_0-1] + F[i_3][i_3][i_2][i_1+1] - G[i_0][i_1][i_2][i_1+1][i_2-1] - A[i_0-2][i_0-2][i_2][i_1+1] + A[i_0][i_0][i_2][i_1] - 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_M; i_6++) {
                B[i_6][i_4-1][0] = H[i_6+i_5][i_6] - I[i_6][i_5] + J[i_6][i_4-1][i_4] + A[i_5-1][i_5-1][i_6][i_4] - B[i_6-1][i_4-1][0] + 2;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-2; i_7++) {
        for (int i_8 = 2; i_8 < PB_M-1; i_8++) {
            C[i_8][i_8-1] = J[i_7+1][i_8+1][i_7] + I[i_7-1][i_8] - C[i_8+1][i_8] * C[i_8-1][i_8-2] * D[i_7+1] + 5;
        }
        D[i_7+1] = K[i_7] + C[i_7][i_7-1] - D[i_7+2] - D[i_7] * 2;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4422331315_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
