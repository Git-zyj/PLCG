...
void kernel_4234212125_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < -i_2+PB_M; i_3++) {
                    A[i_0+1][i_3+i_2][i_1][i_2] = 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        B[i_4-1] = C[i_4-1] + B[i_4] + 3;
    }
#pragma endscop
...
kernel_4234212125_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
