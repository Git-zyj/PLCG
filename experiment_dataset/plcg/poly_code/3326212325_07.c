...
void kernel_3326212325_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_L, i_1); i_2++) {
                A[i_1][i_0-2][i_0] = D[i_1][i_2+i_1][i_0] - A[i_1+1][i_0-2][i_0] - A[i_1][i_0-3][i_0-1] + 6;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 2; i_4 < i_3; i_4++) {
            for (int i_5 = 0; i_5 < PB_P; i_5++) {
                A[i_4][i_3-3][i_3-1] = A[i_3+2][i_4-2][i_4] * 2;
            }
        }
        for (int i_6 = 0; i_6 < PB_M; i_6++) {
            for (int i_7 = max(4, i_3); i_7 < PB_P+2; i_7++) {
                A[i_6][i_7-4][i_7-2] = 2;
            }
        }
    }
...
kernel_3326212325_07(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
