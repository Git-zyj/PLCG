...
void kernel_4332211121_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            A[i_0+1][i_1-1] = A[i_0][i_1-1] - 2;
        }
    }
    ...
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 1; i_6 < min(-i_4+PB_L+1, PB_P-1); i_6++) {
                C[i_5][i_4][i_6+1] = E[i_6-1][i_4][i_5] + E[i_5+1][i_6+i_4-1][i_6] + C[i_5][i_4][i_6-1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4332211121_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
