...
void kernel_2234152313_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        B[i_0-1][i_0+1] = B[i_0-1][i_0+1] + 5;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 3; i_2 < i_1; i_2++) {
            B[i_1-2][i_1] = C[i_2][i_1-2] - D[i_1+2] + D[0] - D[i_2-2] + C[i_2+2][i_1] - C[i_1-1][i_2+1] * D[i_2] - C[i_1-1][i_2] * C[i_1][i_2+2] + B[i_1-1][i_1+1] * B[i_2-3][i_2-1] + 6;
        }
    }
#pragma endscop
...
kernel_2234152313_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
