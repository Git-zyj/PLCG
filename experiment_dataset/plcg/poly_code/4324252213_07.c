...
void kernel_4324252213_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 2; i_1 < PB_M+1; i_1++) {
            for (int i_2 = i_1; i_2 < min(PB_M-2, -i_1+PB_Q); i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_M+1; i_3++) {
                    A[i_2][i_1][i_1][i_3] = D[i_2][i_3][i_0+1][i_1-2] * E[i_2][i_3+i_1-1][i_0-2][i_2-2][i_1] * D[i_3][i_2+2][i_0][i_1] * D[i_1][i_3][i_1][i_0] - D[i_1][i_2][i_3][i_2] - F[i_0][i_3+2][i_2+i_1] * A[i_2][i_0-1][i_0-1][i_3] + 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        B[i_4] = G[i_4] + H[i_4] * G[i_4] - H[i_4] * G[i_4] + H[i_4+1] * B[i_4-1] + B[i_4+1] * 2;
        for (int i_5 = 0; i_5 < min(i_4, PB_M-2); i_5++) {
            for (int i_6 = 0; i_6 < PB_M; i_6++) {
                A[i_6][i_4+1][i_4+1][i_5] = F[i_5+2][i_6][i_4+2] * F[i_5][i_6+1][i_4] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4324252213_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
