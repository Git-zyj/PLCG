...
void kernel_2226152125_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_N-2, i_0); i_1++) {
            A[i_0][i_1] = C[i_0] - D[i_0-2][i_1+2] - C[i_1+2] - D[i_1][i_0] * D[i_0+1][i_1] + D[i_0][i_1-1] * D[i_1-1][i_0] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_N; i_3++) {
            B[i_2][i_3] = C[i_3] + D[i_3][i_3-2] * B[i_2][i_3-2] * 4;
        }
    }
#pragma endscop
...
kernel_2226152125_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
