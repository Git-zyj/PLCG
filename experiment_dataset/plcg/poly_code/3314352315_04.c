...
void kernel_3314352315_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(min(i_0, -i_0+PB_N), PB_M-2); i_1++) {
            B[i_1-2] = D[i_0][i_1] + E[i_1+1][2] * F[i_1+2] - F[i_1] - E[i_0][i_1] + G[i_1+i_0][i_1-2] + 1;
        }
    }
    for (int i_2 = 3; i_2 < PB_M-2; i_2++) {
        B[i_2-2] = H[i_2+2] + H[0] + F[i_2] + I[i_2+1] - F[i_2] * H[i_2] - B[i_2-1] + B[i_2-3] - 4;
        B[i_2-2] = H[i_2] * I[i_2] - I[i_2] - B[i_2-2] * 1;
    }
#pragma endscop
...
kernel_3314352315_04(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
