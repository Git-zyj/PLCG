...
void kernel_4136312315_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_Q-1, -i_1+PB_L+1); i_2++) {
                A[i_1][i_0][i_2] = B[i_0][i_2+i_1-1] - A[i_1][i_0-1][i_2] + A[i_1][i_0][i_2+1] * A[i_1][i_0+1][i_2] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4136312315_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
