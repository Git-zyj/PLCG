...
void kernel_3336112325_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][1] = A[i_0-2][1] - A[i_0+1][1] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_N-1; i_3++) {
            B[i_3+i_2][i_3+i_2] = D[i_3][i_2] - B[i_3+i_2+1][i_3+i_2+1] - B[i_3+i_2][i_3+i_2] * B[i_3+i_2+1][i_3+i_2+1] * 6;
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_N; i_5++) {
            C[i_5][i_4] = A[i_4][1] * A[i_4+1][1] * C[i_5][i_4-2] * 5;
        }
    }
#pragma endscop
...
kernel_3336112325_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
