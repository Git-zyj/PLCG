...
void kernel_3216212111_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_N-1, i_0); i_1++) {
            B[i_1][i_0][0] = C[i_1+1][i_0] - D[i_1][i_0][0] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                B[i_2][i_3][0] = B[i_3-1][i_4][0] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3216212111_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
