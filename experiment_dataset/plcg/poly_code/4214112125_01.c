...
void kernel_4214112125_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, PB_L+1); i_2++) {
                B[i_1-1][i_2+i_0][i_2-1] = 3;
            }
        }
        for (int i_3 = 1; i_3 < min(-i_0+PB_M, PB_L); i_3++) {
            for (int i_4 = max(1, i_0); i_4 < PB_M-2; i_4++) {
                B[i_4-1][i_3+i_0][i_3-1] = C[i_0+2][i_4+2][i_3] * B[i_4+1][i_3+i_0][i_3-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4214112125_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
