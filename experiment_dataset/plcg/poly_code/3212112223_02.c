...
void kernel_3212112223_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_0 = 0; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_Q, -i_0+PB_L); i_1++) {
            for (int i_2 = max(2, i_1); i_2 < min(-i_0+PB_Q-2, -i_1+PB_Q); i_2++) {
                A[i_0][i_1+1][i_2+i_1] = C[i_2][i_1+i_0][i_2][i_1] - A[i_1][i_2+1][i_2+i_0+2] - B[i_2-2][i_1] - B[i_0][i_2] * 6;
            }
        }
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            B[i_0][i_3] = D[i_3][i_0] + 5;
        }
    }
#pragma endscop
...
kernel_3212112223_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
