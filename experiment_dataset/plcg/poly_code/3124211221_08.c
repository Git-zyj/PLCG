...
void kernel_3124211221_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = i_0; i_2 < PB_L-2; i_2++) {
                A[i_0][i_2][i_2][i_0-1] = B[i_2][i_1+1] + A[i_0][i_2-1][i_2-1][i_0-1] - A[i_0-1][i_2+2][i_2+2][i_0-2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3124211221_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
