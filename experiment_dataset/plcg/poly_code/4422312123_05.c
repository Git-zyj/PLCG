...
void kernel_4422312123_05(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_P; i_2++) {
                for (int i_3 = 1; i_3 < min(i_1, -i_2+PB_M); i_3++) {
                    B[i_2-1][i_2+i_1][i_0][i_3-1] = E[i_3+i_2][i_0][i_1][i_2+i_1-2] * 3;
                }
            }
        }
        for (int i_4 = 2; i_4 < PB_P; i_4++) {
            for (int i_5 = i_4; i_5 < -i_0+PB_L; i_5++) {
                for (int i_6 = 1; i_6 < min(-i_4+PB_P, -i_0+PB_P-1); i_6++) {
                    B[i_6-1][i_6+i_4][i_0][i_5-1] = F[i_4-2][i_5+i_0][i_6][i_4] + B[i_6-1][i_6+i_0+1][i_4-2][i_5-1] + 1;
                }
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_L-2; i_7++) {
        ...
        for (int i_8 = 0; i_8 < PB_L; i_8++) {
            for (int i_9 = 0; i_9 < PB_P-2; i_9++) {
                for (int j_0 = 0; j_0 < min(PB_Q, i_8); j_0++) {
                    D[i_7][i_9][j_0][i_7] = G[i_9][i_8][i_9+2] + 1;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4422312123_05(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
