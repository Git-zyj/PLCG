...
void kernel_4326312313_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L-2; i_1++) {
            A[i_1+1][i_1] = D[i_0][i_0+1] + A[i_1][i_1-1] - A[i_0+2][i_0+1] + A[i_1+2][i_1+1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_2][i_3] = B[i_2][i_3+1] - B[i_2-1][i_3-1] + B[i_2+1][i_3] * 1;
        }
        for (int i_4 = 0; i_4 < min(PB_Q-1, i_2); i_4++) {
            for (int i_5 = 1; i_5 < PB_Q-1; i_5++) {
                C[i_5][i_4+1] = C[i_5+1][i_4] - C[i_2-1][i_5+1] * C[i_5-1][i_4+1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_4326312313_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
