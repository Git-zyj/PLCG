...
void kernel_3426131113_01(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                C[i_2][i_1][i_0] = E[i_2+i_1][i_0][1] * E[i_1-1][i_0][i_2] + E[i_2][i_0][i_1] - E[i_1][i_2+1][i_0-1] + 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        D[i_3] = F[i_3] * F[0] + F[i_3] + D[i_3] * 4;
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < i_3; i_5++) {
                C[i_3][i_4][i_5] = E[i_5][i_3-1][i_4] - E[i_3][i_5-1][i_4] + C[i_3-1][i_4][i_5] * 1;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_N; i_6++) {
        D[i_6] = F[i_6] * F[i_6] * F[i_6+1] - 2;
    }
#pragma endscop
...
kernel_3426131113_01(xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
