...
void kernel_3322312321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                A[i_2+i_0][i_1][i_2] = A[i_1+i_0-2][i_2+1][i_1] + A[i_1+i_0+2][i_2][i_0] - 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        B[i_3] = D[i_3] - B[i_3+1] - B[i_3-2] - 2;
    }
    for (int i_4 = 0; i_4 < PB_L-3; i_4++) {
        for (int i_5 = 2; i_5 < PB_L; i_5++) {
            for (int i_6 = 1; i_6 < PB_Q-1; i_6++) {
                C[i_4+2][i_5][i_6] = C[i_4+2][i_5-1][i_6+1] * C[i_4+3][i_5-2][i_6] - C[i_4+2][i_5][i_6-1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3322312321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
