...
void kernel_2434131123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            A[i_0] = E[i_1][i_0] - E[i_1+1][i_0] * 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_M-1, -i_2+PB_M); i_3++) {
            B[i_3-1] = E[i_3][i_2+1] + E[0][i_3+1] - E[i_2+1][i_3+i_2] - B[i_3-2] - 4;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        C[i_4] = E[i_4][0] * C[i_4+1] * 3;
    }
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 1; i_6 < min(i_5, -i_5+PB_L-1); i_6++) {
            D[i_5][i_6+i_5] = F[i_6] + E[i_6-1][i_5] * E[i_6][i_5-1] - E[i_6][i_5] - D[i_5][i_6+i_5+1] * 3;
        }
    }
#pragma endscop
...
kernel_2434131123_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
