...
void kernel_4322331323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        A[i_1-1] = D[i_1] * D[i_1] * D[i_1] + E[i_1-1] - 5;
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            A[i_2-1] = F[i_2][i_1-1][0] + D[i_1] + G[i_1+1][i_2-1] * H[i_2][i_1+1] + 4;
        }
    }
#pragma endscop
...
kernel_4322331323_07(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
