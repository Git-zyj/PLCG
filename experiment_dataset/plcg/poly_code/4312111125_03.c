...
void kernel_4312111125_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_2+1][i_1][i_0] = D[i_1-1][i_0][0] + A[i_2+1][i_1+1][i_0] * A[i_0+1][i_2][i_1-1] + 4;
            }
        }
        B[i_0] = 2;
    }
        ...
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < PB_M+1; i_5++) {
                for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
                    C[i_5-1][i_6][i_6+1] = C[i_5-1][i_6-1][i_6] + 1;
                }
            }
        }
    }
...
kernel_4312111125_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
