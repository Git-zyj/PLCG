...
void kernel_2414251225_07(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        D[i_0] = E[i_0] - F[i_0-1] - F[i_0+1] * 5;
        for (int i_1 = i_0; i_1 < min(PB_P, -i_0+PB_L-2); i_1++) {
            B[i_0][i_1+i_0+1] = G[i_1][i_0-1] + H[i_1][i_0] * H[i_1-1][i_0+1] * H[i_1][i_0] - G[i_1-1][i_1] + B[i_0][i_1+i_0] * B[i_0+1][i_1+i_0+2] - B[i_0][i_1+i_0-1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < min(PB_P-1, -i_2+PB_M); i_3++) {
            D[i_2+1] = H[i_3+i_2][i_3+i_2] * G[i_2][i_3] - G[i_2][i_3-1] * H[i_3][i_2+1] + G[i_2-1][i_3+1] - H[i_2-1][i_3] - I[i_3+i_2][i_3][i_2] * D[i_3-2] + 2;
        }
        D[i_2] = F[i_2] * E[i_2-1] * 6;
    }
#pragma endscop
...
kernel_2414251225_07(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
