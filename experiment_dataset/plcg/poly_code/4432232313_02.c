...
void kernel_4432232313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = E[i_0+2] * E[i_0-2] - F[i_0-1] - A[i_0+1] - A[i_0-1] - 2;
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = i_1; i_2 < PB_P-2; i_2++) {
                for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
                    B[i_1-1][i_0][i_2][i_3+2] = G[i_3][i_3-1][i_1][i_2+1] + H[i_3][i_0][i_1+1][i_2] + B[i_1][i_0][i_3][i_2+2] * B[i_1-1][i_0][i_2-1][i_3+2] * B[i_1-1][i_0-1][i_2-1][i_3+2] * 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_P-2; i_4++) {
        A[i_4+1] = F[i_4] - F[0] + E[i_4] + A[i_4] - 5;
        for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
            for (int i_6 = 2; i_6 < PB_Q; i_6++) {
                for (int i_7 = i_4; i_7 < -i_5+PB_Q; i_7++) {
                    B[i_5-2][i_6][i_7][i_4+2] = H[i_6][i_4][i_7+i_5][0] * G[i_7][i_5+1][i_4+1][i_7+1] * H[i_7+2][i_7][i_6-2][i_6] + 6;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4432232313_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
