...
void kernel_3334251213_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+1; i_0++) {
        A[i_0-1] = D[i_0-1] + E[i_0] + D[i_0-1] * E[i_0] * D[i_0] * A[i_0-2] * 5;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L; i_2++) {
            for (int i_3 = i_1; i_3 < PB_M-1; i_3++) {
                B[i_2-1][i_3] = F[i_2-1][i_1] - G[i_3][i_1+1][i_1] * F[i_3][i_2] * H[i_1-1][i_2-1] + I[i_2][i_3-1][i_1] - I[i_2+1][i_3+1][i_1] * B[i_2-1][i_3-1] - B[i_1-1][i_3-1] * B[i_2-1][i_1+1] * 4;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
            B[i_5-2][i_4-1] = E[i_5] * H[i_5+1][i_4] * F[i_5][i_4] + H[i_5][i_4+1] + 3;
        }
    }
#pragma endscop
...
kernel_3334251213_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
