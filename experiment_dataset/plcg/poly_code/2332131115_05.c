...
void kernel_2332131115_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            A[i_0][i_1-1] = D[i_1-1] - E[i_1+i_0][i_0] * E[i_1][i_0] * E[i_0][i_1] * E[i_1-1][i_0+1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        ...
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            C[i_2][i_4] = D[i_4] + D[i_4+1] - A[i_2][i_4] - 6;
        }
    }
#pragma endscop
...
kernel_2332131115_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
