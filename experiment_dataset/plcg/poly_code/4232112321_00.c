...
void kernel_4232112321_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 3; i_2 < min(i_1, PB_N+1); i_2++) {
                A[i_2-1][i_0][i_1+2] = C[i_2+2][i_1][i_0+1] - A[i_0+1][i_2][i_1+2] + A[i_2-3][i_0][i_1+2] + 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_M; i_4++) {
            for (int i_5 = 1; i_5 < min(PB_M, -i_4+PB_L); i_5++) {
                B[i_4][i_5] = C[i_4][i_5+i_4][0] * B[i_5][i_4+2] - B[i_4][i_5-1] + B[i_4-1][i_5] + B[i_5][i_4-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4232112321_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
