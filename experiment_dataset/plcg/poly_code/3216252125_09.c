...
void kernel_3216252125_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_M-2); i_1++) {
            for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
                B[i_2+2] = C[i_1][i_2][i_0] + D[i_0][i_1][i_2+i_1+2] - C[i_2][i_0][i_1] * B[i_1+2] - 2;
            }
            B[i_1+2] = E[i_1+i_0][0] * E[0][i_1+i_0-1] * F[i_1+i_0][i_1-1] + F[i_1][i_0] * E[i_1-1][i_0-1] + E[i_1][i_0] - 3;
        }
    }
#pragma endscop
...
kernel_3216252125_09(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
