...
void kernel_2414252225_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = E[i_0] * E[i_0] * E[i_0] * E[i_0] + A[i_0+3] - A[i_0+4] - 5;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        C[i_1] = F[i_1-1] * E[i_1] + E[0] + 6;
        C[i_1] = E[i_1] * F[i_1] - G[i_1][i_1] + F[i_1] - F[i_1] * F[i_1] * F[i_1] * C[i_1+2] + C[i_1+1] * 2;
        for (int i_2 = max(2, i_1); i_2 < min(PB_M, -i_1+PB_Q); i_2++) {
            C[i_1-1] = H[i_2-2][i_1] * I[0][i_2][i_1+2] + H[i_1+2][i_2] - E[0] * G[i_2-1][i_2+i_1] + 5;
        }
    }
#pragma endscop
...
kernel_2414252225_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
