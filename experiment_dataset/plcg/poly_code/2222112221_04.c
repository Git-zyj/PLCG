...
void kernel_2222112221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_1][i_0-2] = C[i_0] + A[i_1][i_0-3] * A[i_0+1][i_1-2] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        B[i_2] = C[i_2] - B[i_2+1] + 6;
    }
#pragma endscop
...
kernel_2222112221_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
