...
void kernel_2332312325_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_0-4][i_1] = D[i_1+1][i_0] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        for (int i_3 = 3; i_3 < PB_L-2; i_3++) {
            B[i_3-2][i_2] = B[i_3-2][i_2-1] * B[i_3-3][i_2-2] * C[i_3] * C[i_3+2] * 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-4; i_4++) {
        C[i_4+2] = C[i_4+4] - C[i_4+1] - 4;
    }
#pragma endscop
...
kernel_2332312325_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
