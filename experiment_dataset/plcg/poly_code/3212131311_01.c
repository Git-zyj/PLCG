...
void kernel_3212131311_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        B[i_0+1][i_0+1] = C[i_0+1][i_0-1] - B[i_0+1][i_0+1] - 2;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            B[i_1][i_1] = C[i_1][i_2+1] + C[1][i_1] + C[i_2][i_1] - C[i_2+1][i_1] + D[i_2] - B[i_1-1][i_1-1] - B[i_1+1][i_1+1] * 1;
        }
    }
#pragma endscop
...
kernel_3212131311_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
