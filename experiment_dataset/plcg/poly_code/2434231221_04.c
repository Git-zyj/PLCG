...
void kernel_2434231221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[i_1+1][i_0] = E[i_1+1] - F[i_0][i_1+1] + G[i_1][i_0+1] + A[i_1+1][i_0+2] - A[i_1+2][i_0] * 3;
        }
    }
    for (int i_2 = 3; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_3][i_2-1] = F[i_3][i_2+1] - F[i_3][i_2-1] - B[i_3][i_2-3] + 2;
        }
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            C[i_2-1][i_4] = C[i_2-1][i_4+1] + 4;
        }
        for (int i_5 = i_2; i_5 < PB_M; i_5++) {
            D[i_2][i_5+1] = F[i_5][i_2] - F[i_2-1][i_5] + F[i_5-1][i_2-1] * F[i_2+1][i_5+1] + D[i_2+2][i_5+1] * D[i_2+1][i_5-1] + 2;
        }
    }
#pragma endscop
...
kernel_2434231221_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
