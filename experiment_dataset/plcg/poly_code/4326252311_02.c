...
void kernel_4326252311_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                A[i_0-1][i_1+1][i_2-2] = D[i_1+1][i_0-1][i_2-1][i_2] - E[1][i_0][i_2] - E[i_1][i_2][i_0] * F[i_1+1][i_2][0] + F[i_2][i_0][i_1] * E[i_0][i_2][0] - E[i_1][i_2][0] * A[i_0-2][i_1+1][i_2-1] * A[i_0][i_1][i_2-2] + A[i_0-2][i_1+1][i_2-2] * A[i_0-1][i_1][i_2-2] * 1;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = max(1, i_3); i_5 < PB_Q-2; i_5++) {
                for (int i_6 = 2; i_6 < PB_L-2; i_6++) {
                    A[i_4-1][i_5+1][i_6-2] = D[i_3][i_6][i_4][i_5+2] * G[i_6][i_3][i_6][i_5] * G[i_3][i_5][1][i_6+2] * G[i_5-1][i_5][i_6-2][i_3] - 3;
                }
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_M-2; i_7++) {
        for (int i_8 = -i_7+PB_M-2 - 1; i_8 >= max(3, i_7); i_8--) {
            C[i_8-2] = H[i_8+i_7+2][0] - I[i_8][i_7] + H[i_8-2][i_7] + H[i_7][0] - C[i_8-3] - C[i_8-1] - 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4326252311_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
