...
void kernel_4226312311_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 3; i_2 < PB_L+1; i_2++) {
                A[i_2-2][i_1] = C[i_1+i_0+1][i_2][i_1] + A[i_2-3][i_1] + A[i_2-1][i_1] * A[i_2-2][i_1-1] + A[i_2-3][i_1-1] - 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_Q-3; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 2; i_5 < PB_P+1; i_5++) {
                B[i_4][i_5-2][i_3+2] = B[i_4+1][i_5-2][i_3+2] + B[i_4][i_5-1][i_3+3] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4226312311_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
