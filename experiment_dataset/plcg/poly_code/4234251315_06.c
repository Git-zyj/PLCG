...
void kernel_4234251315_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_M-2; i_2++) {
                for (int i_3 = 1; i_3 < min(min(PB_Q, -i_1+PB_L), -i_0+PB_P); i_3++) {
                    A[i_1+i_0+1][i_3][i_2][i_3-1] = C[i_2][i_3][i_1+1][1] * C[i_1][i_3-1][i_3+i_0][i_2-1] + C[i_0][i_1+1][i_3][i_2-1] * D[i_2][i_0][i_3][i_1-1] + E[i_2][i_3+i_1][i_2] * D[i_2][i_0][i_3][i_1-1] * A[i_2+i_1+2][i_0][i_3][i_0-1] + A[i_1+i_0+1][i_2+1][i_3][i_2] + 3;
                }
            }
        }
        B[i_0+1] = F[i_0-1] + G[i_0+1][i_0] - F[i_0-1] * F[i_0] - B[i_0] - B[i_0+2] * B[i_0] + 5;
    }
#pragma endscop
...
kernel_4234251315_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
