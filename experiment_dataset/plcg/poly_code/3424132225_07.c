...
void kernel_3424132225_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] - E[i_0] + E[i_0] * E[i_0-2] * E[i_0+2] * A[i_0+2] - 4;
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            B[i_0][i_1-1] = A[i_1] - B[i_0-1][i_1-1] * B[i_0+2][i_1+1] * 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q; i_2++) {
        A[i_2] = E[i_2-2] * E[i_2] * 4;
        for (int i_3 = 0; i_3 < PB_Q; i_3++) {
            for (int i_4 = 2; i_4 < -i_2+PB_P; i_4++) {
                D[i_4-2][i_3+2][i_2] = F[i_3+2][i_4][i_2-2] - F[i_2+2][i_4+i_2][i_3] * G[i_4][i_3] - B[i_3][i_4-1] * 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3424132225_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
