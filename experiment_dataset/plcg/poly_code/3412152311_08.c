...
void kernel_3412152311_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-1; i_0++) {
        A[i_0-2][i_0] = E[i_0] - A[i_0-1][i_0+1] + 3;
    }
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_N-2, -i_1+PB_N+1); i_2++) {
            for (int i_3 = 1; i_3 < min(-i_1+PB_M-1, -i_2+PB_M+2); i_3++) {
                B[i_2][i_3-1] = F[i_3+i_1+1][i_2][i_2-1] - F[i_1-2][i_3][i_2] * F[i_3][i_1-2][i_2+1] - F[i_3][i_1+2][i_2] - F[i_3][0][i_1] + F[i_3+i_2-2][i_1][i_2+i_1] + F[i_2-2][i_3][i_1-1] * B[i_3][i_1] * D[i_1][i_2] * D[i_2][i_3-1] * 2;
            }
            C[i_2+1][i_1] = G[i_2+1][i_1] + G[i_2+i_1-1][i_2] * G[i_2+1][i_1-1] - G[i_2][i_1] - G[i_2+1][i_1+1] * F[i_2][i_1+2][i_2] * G[i_2][i_1-1] + E[i_1] - C[i_1+1][i_2+1] + D[i_1][i_2] - 3;
            D[i_1][i_2] = G[i_1-2][i_2] * G[i_2+2][i_1] + G[i_2][0] - G[i_2][i_1] + B[i_2-1][i_1] - B[i_2][i_1-1] - D[i_1-1][i_2] - D[i_2][i_1-1] * 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3412152311_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
