...
void kernel_2412252125_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        B[i_0] = E[i_0-1] * F[i_0] + E[i_0+2] + F[i_0+2] + 2;
    }
    for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
        B[i_1] = F[i_1] * F[i_1] * G[i_1][0] + G[i_1+2][2] + F[i_1] - 1;
        for (int i_2 = i_1; i_2 < PB_L-1; i_2++) {
            C[i_1][i_2+1] = G[i_2][i_1] + H[i_2][i_1] - H[i_2][i_1-2] - F[i_1] - 4;
        }
        D[i_1] = E[i_1] + F[i_1] + F[1] + E[i_1-1] + E[i_1] * E[i_1+1] + B[i_1+1] - 6;
    }
#pragma endscop
...
kernel_2412252125_02(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
