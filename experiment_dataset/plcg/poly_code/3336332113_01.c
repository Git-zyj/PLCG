...
void kernel_3336332113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        A[i_0+2][i_0-1] = D[i_0] - E[i_0] * D[i_0] - D[i_0] * A[i_0+3][i_0] * 2;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        A[i_1+1][i_1-2] = F[i_1][i_1] * E[i_1] * 4;
        for (int i_2 = 1; i_2 < -i_1+PB_L+2; i_2++) {
            A[i_2+2][i_2-1] = G[i_2+i_1-2][i_2] * G[i_2][i_1] - H[i_2+1][i_1] + 2;
        }
    }
#pragma endscop
...
kernel_3336332113_01(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
