...
void kernel_4222232311_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < PB_M-1; i_2++) {
                for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
                    A[i_1][i_1+2][i_3][i_2] = C[i_2+1][i_3][i_0-2][i_1] * C[i_3][i_2][i_1][i_0] * D[i_0][i_1][i_2-2][i_3-1] - A[i_1][i_1+2][i_3][i_2+1] - A[i_3][i_3+2][i_0][i_1-1] + 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < PB_N-1; i_5++) {
            B[i_5] = E[i_5][i_5] + F[i_5][i_4][i_5-2] - B[i_5+1] * B[i_5-1] + B[i_5-1] * 3;
        }
    }
#pragma endscop
...
kernel_4222232311_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
