...
void kernel_4312311211_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            A[i_1+1] = D[i_0][i_1] - D[i_0-1][i_1+1] - A[i_1] - A[i_1+2] + 1;
        }
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            for (int i_3 = 0; i_3 < PB_P; i_3++) {
                for (int i_4 = 0; i_4 < PB_L; i_4++) {
                    B[i_2][i_0][i_4][i_4] = E[0][i_4][i_2] - B[i_4][i_0+1][i_3][i_3] - 2;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        C[i_5] = A[i_5+1] * C[i_5-1] + C[i_5+1] + 2;
    }
#pragma endscop
...
kernel_4312311211_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
