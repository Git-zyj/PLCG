...
void kernel_3234231325_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, -i_0+PB_L-2); i_1++) {
            A[i_0][i_1+i_0+1] = C[i_1][i_1-1] - C[i_0][i_1+1] * C[i_1-1][i_0] * D[i_1-1][1] - A[i_1-1][i_1+i_0+1] * A[i_0+1][i_1+i_0+2] - A[i_0-1][i_1+i_0] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_L+1, PB_M); i_3++) {
            A[i_3][i_3+i_2-1] = C[i_2][i_3+i_2-1] * C[i_3][i_2] * 3;
        }
    }
#pragma endscop
...
kernel_3234231325_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
