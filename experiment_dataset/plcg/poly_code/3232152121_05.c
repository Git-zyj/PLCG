...
void kernel_3232152121_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_M-2, i_0), PB_P-1); i_1++) {
            A[i_1+i_0+2][i_1+i_0] = C[i_0-1][i_1+1] + C[i_1][i_0] + D[i_0] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 3; i_3 < min(PB_P-2, i_2); i_3++) {
            B[i_3-1] = C[i_2][i_3] + C[i_3+2][i_2+1] + D[i_3] - C[i_3][i_2+1] - C[i_3][i_2+1] - C[i_2][i_3+2] - B[i_3-3] - B[i_3+1] * 6;
        }
    }
#pragma endscop
...
kernel_3232152121_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
