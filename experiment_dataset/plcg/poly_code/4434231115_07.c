...
void kernel_4434231115_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = E[i_0] * E[i_0+1] + F[i_0] - A[i_0] + 6;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0-1] = G[i_1][i_0] - H[i_1][i_0-1] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_M; i_3++) {
            A[i_3-2] = G[i_2+1][i_3+1] + G[i_3][i_2-1] - H[i_3+i_2][i_3] + G[i_2][i_3+1] + 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M; i_5++) {
            for (int i_6 = 0; i_6 < min(i_4, PB_M-1); i_6++) {
                D[i_4-1][i_6][i_5-1] = G[i_6+1][i_5-1] + I[i_6][i_5][i_4] - I[i_5][i_6][i_4-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4434231115_07(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
