...
void kernel_4226252211_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = max(3, i_1); i_2 < PB_M-1; i_2++) {
            B[i_2-2] = E[i_1][i_2+2] * D[i_1] * F[i_2+1][i_1] - F[i_1][0] - E[i_1+1][i_1] * D[i_2] - F[i_1-1][i_2-2] * B[i_2-3] * B[i_2-1] * 6;
        }
    }
#pragma endscop
...
kernel_4226252211_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
