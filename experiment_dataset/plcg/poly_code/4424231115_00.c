...
void kernel_4424231115_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_0][i_1] = E[i_1-1][i_0] * E[i_0][i_1-1] * F[i_1-1] * A[i_0+1][i_1+1] + 4;
        }
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            B[i_0-1][i_2-1] = G[i_2][i_0] * G[i_2+1][i_0] * 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L-1; i_4++) {
            B[i_4-1][i_3-1] = F[i_3-1] + E[i_3-1][i_4-1] * E[i_3][i_4+1] - 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_P; i_5++) {
        for (int i_6 = 1; i_6 < -i_5+PB_P+1; i_6++) {
            D[i_6] = G[i_5+1][i_6+i_5-1] * G[i_6][i_5] * F[i_6] - D[i_6-1] + 5;
        }
    }
#pragma endscop
...
kernel_4424231115_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
