...
void kernel_4314312313_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        ...
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            C[i_2][i_3] = E[i_3][i_2][i_2] * B[i_3+1] + C[i_2+1][i_3] * C[i_2][i_3+1] - C[i_2-1][i_3] - 1;
        }
    }
#pragma endscop
...
kernel_4314312313_02(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
