...
void kernel_2412212323_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_1] = E[i_0][i_0] * B[i_0-1] * B[i_0+1] - B[i_1] - 5;
        }
        B[i_0] = 4;
    }
    for (int i_2 = 0; i_2 < PB_M-4; i_2++) {
        ...
        for (int i_3 = i_2; i_3 < PB_N-1; i_3++) {
            D[i_3+1][i_2] = C[i_2+2] * C[i_2+3] - C[i_2+4] - C[i_3+4] + C[i_2+1] - D[i_3+1][i_2+1] * 6;
        }
    }
#pragma endscop
...
kernel_2412212323_06(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
