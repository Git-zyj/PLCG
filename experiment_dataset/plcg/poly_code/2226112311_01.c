...
void kernel_2226112311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L-1; i_1++) {
            A[i_1] = C[i_1+1] + A[i_1-1] + A[i_1+1] + A[i_1-1] + A[i_1-1] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-3; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N-3; i_3++) {
            B[i_3+i_2+1][i_3+i_2] = B[i_3+i_2+3][i_3+i_2+2] + 4;
        }
    }
#pragma endscop
...
kernel_2226112311_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
