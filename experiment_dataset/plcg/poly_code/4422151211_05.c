...
void kernel_4422151211_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1][i_0] = E[i_0] + E[i_0] - E[i_0] * E[i_0-1] - E[i_0] - E[i_0+1] - E[i_0] * A[i_0+2][i_0+1] + A[i_0][i_0-1] - B[i_0][i_0+1] * 4;
        for (int i_1 = 1; i_1 < -i_0+PB_M-1; i_1++) {
            B[i_0][i_0+1] = E[i_0] * F[i_0][i_1-1] - F[i_1+i_0+1][i_1] * E[i_1+i_0+1] + E[i_1] - A[i_0+1][i_0] - 4;
        }
    }
        ...
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 1; i_4 < PB_M+1; i_4++) {
                for (int i_5 = i_4; i_5 < PB_L-1; i_5++) {
                    D[i_3][i_4][i_5+2] = G[i_5-1][i_4-1][i_3+1][i_2] - G[i_2][i_3][i_5+1][i_4-1] - 6;
                }
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
        for (int i_7 = 0; i_7 < -i_6+PB_L-1; i_7++) {
            for (int i_8 = 0; i_8 < -i_6+PB_L; i_8++) {
                D[i_6][i_7][i_8+1] = H[i_7][i_8+i_6][i_6] + H[i_8][i_7+i_6+1][i_6-1] - H[i_8+i_7+1][i_6][i_8] + H[i_7][i_8][i_7] * B[i_7+1][i_7+2] + D[i_6+1][i_7][i_8+1] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422151211_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
