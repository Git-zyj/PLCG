...
void kernel_2314112125_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        ...
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            B[i_0][i_2-1] = D[i_0-1][i_0] + D[i_2][i_0] - B[i_0+2][i_2-2] * 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M-1; i_4++) {
            C[i_4+i_3-2][i_3] = C[i_4+i_3+1][i_3+2] + C[i_4+i_3][i_3+1] + 6;
        }
    }
#pragma endscop
...
kernel_2314112125_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
