...
void kernel_4232231211_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                A[i_1][i_0][i_2] = C[i_2][i_0][i_1] - A[i_1][i_0][i_2-1] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_M, PB_L-1); i_5++) {
                B[i_3][i_3][i_5+1] = D[i_3][i_5] * C[i_5][i_4][i_3-1] + E[i_5][i_3][i_4-1] - C[i_4][i_5+i_3][i_4] + E[i_3+1][i_4+1][i_3-1] - A[i_5][i_3][i_4+1] - B[i_3-1][i_3-1][i_5+1] - B[i_3][i_3][i_5] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4232231211_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
