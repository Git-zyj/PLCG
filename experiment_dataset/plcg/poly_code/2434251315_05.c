...
void kernel_2434251315_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
    ...
    for (int i_1 = PB_L-1 - 1; i_1 >= 1; i_1--) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            A[i_2+1] = F[i_1] + E[i_2] - G[i_2][i_1-1] * H[i_2][i_1-1] * H[i_2+i_1][1] - G[0][i_2] + G[i_2+i_1+1][0] * G[i_2][i_1-1] * H[i_2][i_1+1] + H[i_1+1][i_2] - G[i_2][i_2+i_1] - F[i_2] - 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            A[i_4] = H[i_4+1][i_3] - H[i_4+1][i_3] + H[i_3-1][i_4] + I[i_3+1][i_4][i_3] - G[i_4-1][i_3+1] - A[i_4+2] + 4;
        }
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            A[i_3+2] = F[i_5] + H[i_3+1][i_5] - 2;
        }
    }
#pragma endscop
...
kernel_2434251315_05(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
