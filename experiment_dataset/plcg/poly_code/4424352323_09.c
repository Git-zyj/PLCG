...
void kernel_4424352323_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_Q-2; i_2++) {
                for (int i_3 = 2; i_3 < min(i_1, -i_2+PB_Q-2); i_3++) {
                    A[i_0-1][i_1][i_3+i_2][0] = E[i_2-1][i_0+2][i_1+2][i_2] - F[i_1+1][i_3-2][i_2-1] + G[0][i_3][i_1][i_2] * F[i_1][i_3+2][i_0+2] - A[i_0-1][i_1-2][i_3+i_2][0] + A[i_0+1][i_1+2][i_3+i_2+2][0] - A[i_0-1][i_1][i_3+i_2-2][0] - A[i_0+1][i_1+2][i_3+i_2+1][0] + 1;
                }
            }
        }
        for (int i_4 = 2; i_4 < PB_P-2; i_4++) {
            for (int i_5 = 2; i_5 < PB_L; i_5++) {
                B[i_4][i_5] = F[i_0][i_4][i_5] - H[i_4][i_0][i_5] - I[i_4-2][i_0][i_5] + J[i_5-2][i_4] * I[i_0-2][i_5][i_4+2] - K[i_5][i_4] - H[i_5-2][i_0+1][i_5] - B[i_4+1][i_5+2] * 3;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_L-2; i_6++) {
        C[i_6] = L[i_6+2] - L[i_6+1] + C[i_6+2] * C[i_6-1] + C[i_6-2] - D[i_6] + 3;
    }
    for (int i_7 = 2; i_7 < PB_L-2; i_7++) {
        D[i_7] = M[i_7-2] * N[i_7] + L[i_7] - L[0] + L[i_7+2] * L[0] * L[i_7] - D[i_7+1] + D[i_7-2] + D[i_7-1] - 2;
    }
#pragma endscop
...
kernel_4424352323_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N));
