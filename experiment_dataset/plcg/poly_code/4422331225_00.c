...
void kernel_4422331225_00(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    D[i_0+1][i_1][i_3][i_2] = E[i_3][i_1][i_3][i_2] - E[i_0][i_2][i_3][i_1] * F[i_2+1][i_3][i_3][i_1-1] + G[i_2+1][i_1][i_3] * 5;
                }
            }
        }
        for (int i_4 = max(2, i_0); i_4 < -i_0+PB_P; i_4++) {
            for (int i_5 = 1; i_5 < -i_0+PB_M; i_5++) {
                for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
                    D[i_0][i_5][i_6][i_4-2] = E[i_4][i_5][i_0][i_6] + F[i_0][i_6+1][i_4-1][i_5+1] + E[i_5+i_0][i_6-1][i_4][i_4] - G[1][i_4][i_5-1] + 3;
                }
            }
            for (int i_7 = 1; i_7 < min(-i_0+PB_Q-1, -i_4+PB_L); i_7++) {
                C[i_4+1][i_7+i_0][i_4-1] = F[i_0][i_7+i_4][i_0-1][i_7+i_0+1] - F[i_4][i_7-1][i_4+i_0][0] - G[i_0][i_7][i_4] * C[i_4+1][i_7+i_0+1][i_4-1] - 5;
            }
        }
    }
        ...
        for (int i_9 = 2; i_9 < PB_Q; i_9++) {
            for (int j_0 = 1; j_0 < PB_M-1; j_0++) {
                for (int j_1 = 0; j_1 < min(-i_8+PB_L, PB_M-1); j_1++) {
                    D[i_9][i_8][j_0][j_1] = F[j_0-1][j_1+i_8][i_8+1][i_9] - D[i_9][j_0+1][i_8][j_1] * D[j_0][j_1+1][i_9-2][i_8-1] * 5;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4422331225_00(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
