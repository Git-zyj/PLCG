...
void kernel_4224152111_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_M, i_0), -i_0+PB_M+1); i_2++) {
                B[i_1][i_2+1][i_2+3] = C[i_2+i_1][i_0][i_2] - C[i_1-2][i_0][i_2] * D[i_1][i_0] - D[i_2+i_0-1][i_1] + 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_3+PB_L, PB_M); i_5++) {
                B[i_3][i_5][i_5+2] = D[i_5-1][i_4] - E[i_3+2][i_5][i_4][i_5+i_3] * D[i_3+1][i_4+1] * C[i_3][i_5][i_4] * D[i_5][i_4+1] * B[i_3][i_5+1][i_5+3] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4224152111_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
