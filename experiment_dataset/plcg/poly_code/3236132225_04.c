...
void kernel_3236132225_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P-3, i_0); i_1++) {
            for (int i_2 = max(3, i_1); i_2 < PB_M; i_2++) {
                A[i_0-1][i_1+1] = C[i_2][0] + A[i_2-3][i_1+3] - A[i_0][i_1+1] - 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N-2; i_3++) {
        B[i_3] = C[i_3][i_3] + D[i_3] + D[i_3] + D[i_3] + B[i_3+2] + B[i_3-2] - 5;
    }
#pragma endscop
...
kernel_3236132225_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
