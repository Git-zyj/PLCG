        ...
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = i_0; i_2 < -i_1+PB_L-2; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_1+PB_M, -i_0+PB_L-1); i_3++) {
                    A[0][i_2+i_1+2][i_3] = D[i_3+i_1][i_2-1][i_0][i_2] + A[0][i_3+i_0+1][i_2] - A[0][i_2+i_1+1][i_3] - 6;
                }
            }
        }
        B[i_0+2] = E[i_0-1] - E[i_0-1] + F[i_0] * F[i_0] * E[0] - F[i_0+1] - 3;
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_L; i_5++) {
            B[i_4+2] = G[i_5-2][i_4] + H[i_4][i_5][i_5] + 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4334232115_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
