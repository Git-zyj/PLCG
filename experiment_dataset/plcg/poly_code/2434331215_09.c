...
void kernel_2434331215_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_0][i_1-1] = E[i_1][i_0] * F[i_1+1] - G[i_1][0] - A[i_0][i_1-2] * B[i_0-1] * 5;
        }
        B[i_0-1] = H[i_0+1] - I[i_0+1] * F[i_0] + B[i_0-2] - 4;
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        B[i_2] = F[i_2] - F[i_2] - B[i_2-1] - 4;
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_3-1] = J[i_3][i_2] + H[i_2] + 5;
        }
    }
#pragma endscop
...
kernel_2434331215_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
