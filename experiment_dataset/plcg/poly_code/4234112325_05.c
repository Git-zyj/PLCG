...
void kernel_4234112325_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                B[i_2][i_2][i_1-1] = C[i_2][i_0][i_0+1] - C[i_0][i_2+1][i_1] + 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 4; i_4 < PB_Q+4; i_4++) {
            for (int i_5 = 2; i_5 < PB_L; i_5++) {
                B[i_3][i_3][i_5-2] = B[i_3-2][i_3-2][i_5-2] * B[i_5][i_5][i_4-4] + B[i_3+1][i_3+1][i_5-1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4234112325_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
