...
void kernel_4316331225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0-1] - D[i_0+1] * E[i_0] - F[i_0-1] - F[i_0] - D[i_0-1] + A[i_0-2] - 6;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            A[i_2+1] = G[i_1][i_2+1] * 5;
        }
        for (int i_3 = i_1; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 2; i_4 < min(min(-i_3+PB_M, PB_L-2), i_3); i_4++) {
                C[i_1][i_4][0] = H[i_4+i_3][i_1][0] + I[i_4][i_1][i_3+1] + C[i_1+2][i_4-1][0] * C[i_1-2][i_4+2][0] + C[i_1][i_4-2][0] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4316331225_04(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
