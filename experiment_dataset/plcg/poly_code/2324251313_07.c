...
void kernel_2324251313_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_N-1; i_1++) {
            A[i_0][i_1] = D[i_1+i_0-1] * E[i_1+i_0+1][i_1-1] * A[i_0-1][i_1] * A[i_0][i_1+1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_N-1; i_3++) {
            B[i_3][i_2] = E[i_3+1][i_2+1] * E[i_3+1][i_2-1] * E[i_3+1][i_2] - F[i_2] - B[i_2+1][i_3] + B[i_3-1][i_2+1] + B[i_3-1][i_2] + 3;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            B[i_5-1][i_4] = D[i_5] * G[i_5][i_4] * G[i_4][i_5-1] - G[i_5][1] + D[i_4+1] + G[i_5][i_4] - G[i_5-1][i_4-1] * 1;
        }
    }
#pragma endscop
...
kernel_2324251313_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
