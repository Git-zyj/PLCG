...
void kernel_4322131211_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0][i_1] = D[i_1][i_0] - D[i_1+i_0-1][1] - A[i_1+i_0-1][i_1] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                C[i_3+1][i_2+1][i_2+1] = E[i_3-1][i_4][i_2] * E[i_3+1][i_2][i_4] - 1;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
            for (int i_7 = 1; i_7 < -i_6+PB_P; i_7++) {
                C[i_5][i_6][i_6] = D[i_6][i_7] + D[i_7][i_6] * E[i_6+1][i_5-1][i_7+i_6] + D[i_6-1][i_7-1] - C[i_5+1][i_6+1][i_6+1] - C[i_5+1][i_7][i_7] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4322131211_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
