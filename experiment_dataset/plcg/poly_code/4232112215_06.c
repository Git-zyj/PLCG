...
void kernel_4232112215_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L; i_2++) {
                B[i_1][i_2+1] = C[i_2][i_1][i_2+i_0] - C[i_2][i_1][i_2+i_0] - 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
            for (int i_5 = 0; i_5 < i_3; i_5++) {
                B[i_4][i_5+1] = B[i_4+1][i_5+1] - B[i_4][i_5] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4232112215_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
