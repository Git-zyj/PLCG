...
void kernel_4322151111_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
    ...
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_Q, -i_2+PB_M); i_4++) {
                B[i_4+i_2][i_3][i_4] = E[i_4][i_2][i_3] - E[i_3][i_4][i_3] - E[i_3][i_2+1][i_3] * D[i_4+i_2-1][i_3] + E[i_4][i_3+1][i_2+1] + B[i_4+i_2][i_3-1][i_4] - 4;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
            C[i_6] = D[i_5+1][i_6] * D[i_6+1][i_5] * D[i_6][i_5] * D[i_5][i_5] - D[i_5][i_6] * D[i_6][i_5] * D[i_6-1][i_5] - C[i_6+1] - C[i_6-1] + 5;
        }
    }
#pragma endscop
...
kernel_4322151111_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
