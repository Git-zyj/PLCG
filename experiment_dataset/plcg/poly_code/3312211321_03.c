...
void kernel_3312211321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0+1][i_1] = D[i_1][i_0] + A[i_1][i_0+1] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_Q, -i_2+PB_N); i_3++) {
            for (int i_4 = max(2, i_3); i_4 < -i_3+PB_Q; i_4++) {
                B[i_3][i_4+1][i_2-1] = E[i_3][i_2][i_4+i_3] + F[i_3+i_2][i_4+1][i_4] - B[i_3][i_4+3][i_2-1] + B[i_3+1][i_2+1][i_4-2] - 4;
            }
        }
        for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
            C[i_2][i_5] = C[i_2][i_5+1] * C[i_2+1][i_5] + C[i_2-2][i_5] - C[i_2-1][i_5] * C[i_2][i_5-1] - 6;
        }
    }
#pragma endscop
...
kernel_3312211321_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
