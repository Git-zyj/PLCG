...
void kernel_3226312323_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(PB_P, -i_0+PB_M-2); i_1++) {
            B[i_1+i_0+2][i_1-1][1] = C[i_1][i_0] - 4;
        }
    }
    for (int i_2 = 3; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_M-2); i_3++) {
            for (int i_4 = i_2; i_4 < min(-i_2+PB_M-3, PB_P); i_4++) {
                B[i_3+i_2+1][i_2-2][1] = B[i_3+i_2][i_2-3][1] + B[i_4+i_2+3][i_2][1] + B[i_3+i_2+2][i_2-2][1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3226312323_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
