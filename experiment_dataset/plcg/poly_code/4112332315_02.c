...
void kernel_4112332315_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                A[i_1][i_0+1][i_2][i_0-1] = B[i_2][1] - B[i_2][i_1+2] * C[i_2][i_1+1][i_0+2] * A[i_1][i_0+1][i_2-1][i_0-1] + A[i_1-1][i_0+1][i_2+1][i_0-1] * A[i_0+1][i_2+1][i_1][i_2-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4112332315_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
