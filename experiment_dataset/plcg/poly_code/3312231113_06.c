...
void kernel_3312231113_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_N+1; i_1++) {
            A[i_1-1][i_0] = A[i_1-2][i_0+1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_P-1; i_3++) {
            A[i_3-2][i_2-1] = D[i_3][i_2+1] * D[i_3][i_2] + E[i_2][i_3][i_2] * F[i_3-1][i_2-1] + F[i_3][i_2] * 6;
            for (int i_4 = 1; i_4 < PB_P; i_4++) {
                A[i_4-1][i_2] = E[i_3][i_4-1][i_4] * E[i_2+1][i_3+1][i_3+1] + G[i_3][i_2][i_4] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3312231113_06(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
