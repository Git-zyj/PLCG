...
void kernel_4312111323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N-2; i_1++) {
            A[i_1+1] = D[0] + B[i_1] + B[i_1+2] * 4;
        }
        B[i_0] = D[i_0] - D[i_0] - A[i_0+1] * A[i_0-1] + 2;
        for (int i_2 = 0; i_2 < PB_N; i_2++) {
            B[i_2] = A[i_2+1] + B[i_0] - 6;
        }
    }
#pragma endscop
...
kernel_4312111323_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
