...
void kernel_3222112323_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        for (int i_1 = 4; i_1 < PB_M+4; i_1++) {
            A[i_0+2][i_1+2] = C[i_0] + A[i_0+3][i_1+2] - A[i_0+2][i_1+4] - A[i_0+4][i_1+2] + B[i_1-4] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        B[i_2-2] = C[i_2] * B[i_2] + 4;
    }
#pragma endscop
...
kernel_3222112323_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
