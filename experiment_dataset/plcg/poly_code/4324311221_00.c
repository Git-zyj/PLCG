...
void kernel_4324311221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < i_0; i_2++) {
                A[i_1][i_0][i_2] = A[i_1][i_0][i_2-2] * A[i_1-1][i_2][i_0] * 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_Q+1; i_3++) {
        for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
                B[i_3][i_4][i_5-1] = B[i_3-2][i_4+1][i_5-1] * B[i_5+2][i_4][i_3-1] + B[i_3][i_4-2][i_5-1] * 3;
            }
        }
    }
        ...
        for (int i_7 = 1; i_7 < PB_P-1; i_7++) {
            for (int i_8 = 0; i_8 < i_6; i_8++) {
                for (int i_9 = 1; i_9 < -i_6+PB_P; i_9++) {
                    C[i_7-1][i_6][i_9-1] = D[i_8][i_9][i_7][i_6] + E[i_8][i_7+1][i_9+i_6][i_8] * C[i_7][i_9][i_8] + 3;
                }
            }
        }
    }
...
kernel_4324311221_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
