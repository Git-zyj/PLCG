...
void kernel_3434251115_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                A[i_2-1][i_1][i_0] = E[i_1][i_2][i_1+i_0] - F[i_1][i_0][i_2] + F[i_2+1][i_0][i_2+1] - E[i_1+i_0][i_2][i_1] - A[i_2-1][i_1][i_0+1] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
            B[i_4][i_3] = G[i_4][i_3-1] * H[i_4] + I[i_4][i_3+1] + I[i_4][i_3+1] - G[i_4-1][i_4+i_3] - 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
            for (int i_7 = PB_M - 1; i_7 >= i_6; i_7--) {
                A[i_7-1][i_6][i_5] = E[i_7][i_5][i_6] + E[i_7][i_6-1][i_5] + F[i_7][i_6-1][i_5] - 4;
            }
            for (int i_8 = 1; i_8 < -i_5+PB_N; i_8++) {
                A[i_6-1][i_5+1][i_8] = I[i_8][i_6+1] - F[i_8][i_6][i_5+1] - E[i_8+i_5][i_6][i_5-1] + E[i_8-1][i_6][i_5] + F[i_8][i_6][i_5] - E[i_6][i_8][i_5+1] * I[i_6][i_8] + G[i_8][i_6] * 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3434251115_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
