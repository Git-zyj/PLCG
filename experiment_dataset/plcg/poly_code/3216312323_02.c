...
void kernel_3216312323_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_N, i_0); i_1++) {
            for (int i_2 = 2; i_2 < min(min(i_0, PB_M+2), -i_0+PB_L-1); i_2++) {
                B[i_1][i_2-2][i_2+i_0+1] = 3;
            }
        }
        for (int i_3 = 2; i_3 < PB_N-2; i_3++) {
            for (int i_4 = max(3, i_0); i_4 < min(min(-i_3+PB_N, PB_M), -i_0+PB_L-3); i_4++) {
                B[i_3][i_4-2][i_4+i_0] = C[i_4+i_3][i_4][i_3-1] - B[i_3][i_4-3][i_4+i_0-1] * B[i_3+2][i_4-2][i_4+i_0] - B[i_3-2][i_4][i_4+i_0+3] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3216312323_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
