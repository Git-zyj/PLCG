...
void kernel_2322252323_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P+2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_P+1, -i_0+PB_L-2); i_1++) {
            A[i_1][i_1-2] = D[i_1+i_0+1][i_0] + D[i_1-1][i_0-1] - E[i_1+i_0+2] * F[i_0][i_1][i_0+1] - D[i_1][i_0] * A[i_1+1][i_1-1] + 2;
        }
        for (int i_2 = 2; i_2 < min(-i_0+PB_M-2, PB_P+2); i_2++) {
            B[i_0-1][i_2] = G[i_0][i_2] + G[i_2-1][i_0+2] - H[i_2] * D[i_2][i_0] * G[i_2][i_0] + H[i_2+i_0+2] - A[i_0][i_0-2] + A[i_2][i_2-2] * B[i_0+1][i_2] * B[i_0-1][i_2-1] + 6;
        }
    }
    for (int i_3 = PB_L - 1; i_3 >= 3; i_3--) {
        for (int i_4 = 4; i_4 < min(PB_P+4, i_3); i_4++) {
            C[i_4+2][i_4] = D[i_4][i_3] * E[2] + G[i_4][i_3] - G[i_3+1][i_4] + A[i_4-2][i_4-4] - B[i_3-3][i_4] - C[i_4+4][i_4+2] - C[i_4+3][i_4+1] - 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_2322252323_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
