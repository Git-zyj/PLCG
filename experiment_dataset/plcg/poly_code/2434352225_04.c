...
void kernel_2434352225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0+1] + F[i_0+1] * G[i_0][0] - F[i_0-2] * E[i_0] * E[i_0] + A[i_0-2] + 4;
        A[i_0] = E[i_0+1] - H[i_0-1] + F[i_0+1] + F[i_0] + H[i_0-2] * F[i_0] * 3;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        A[i_1-1] = F[i_1] * H[i_1] - 6;
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            D[i_1-2][i_2] = E[i_2] * G[i_2][i_2-1] + I[i_2-2][i_1] + D[i_1-1][i_2] * D[i_1-2][i_2-2] * 5;
        }
    }
#pragma endscop
...
kernel_2434352225_04(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
