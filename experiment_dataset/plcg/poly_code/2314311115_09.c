...
void kernel_2314311115_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            B[i_0] = B[i_1] - C[i_1+1] * 4;
        }
        B[i_0] = 6;
        for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
            C[i_2] = D[i_2+1][0] + E[i_2-1][i_0] + 2;
        }
    }
#pragma endscop
...
kernel_2314311115_09(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
