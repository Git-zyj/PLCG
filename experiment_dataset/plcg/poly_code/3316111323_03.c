...
void kernel_3316111323_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            B[i_1] = 4;
            B[i_1] = B[i_1+1] + B[i_1-2] * B[i_1-1] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_Q; i_3++) {
            for (int i_4 = i_2; i_4 < PB_L; i_4++) {
                B[i_4+1] = D[i_4-1][i_2][i_3] + D[i_4][i_4-1][i_2+1] + D[i_2-1][i_4][i_3] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3316111323_03(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D));
