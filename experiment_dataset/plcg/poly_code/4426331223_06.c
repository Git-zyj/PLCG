...
void kernel_4426331223_06(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        D[i_0+1] = E[i_0] - E[i_0] - E[i_0] * F[i_0][i_0] + 3;
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = i_1; i_2 < PB_Q-1; i_2++) {
                B[i_0][i_1][i_2] = G[i_1-1][i_0] - H[i_1][i_2][i_0-1] * H[i_2][i_0][i_1] * B[i_0+2][i_1+1][i_2+1] * 4;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < min(PB_N, i_3); i_4++) {
            for (int i_5 = 0; i_5 < PB_N; i_5++) {
                for (int i_6 = 0; i_6 < min(i_5, -i_4+PB_Q); i_6++) {
                    B[i_4][i_5+2][i_6] = I[i_5][i_5][i_3][i_6+i_4] + I[i_4][i_3-1][i_6][i_5] + 3;
                }
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_L-2; i_7++) {
        D[i_7] = J[1] + K[i_7] - D[i_7+2] - D[i_7+1] + 3;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4426331223_06(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K)));
