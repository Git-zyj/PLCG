...
void kernel_4424311115_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_P; i_1++) {
            A[i_1][i_1+i_0] = E[i_1-1][i_0] + F[i_0][i_1] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < PB_N; i_3++) {
            ...
            for (int i_5 = 0; i_5 < -i_3+PB_P; i_5++) {
                B[i_2][i_3][i_5+1] = A[i_5][i_5+i_3] * 5;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_P; i_6++) {
        for (int i_7 = 0; i_7 < i_6; i_7++) {
            for (int i_8 = 1; i_8 < PB_L; i_8++) {
                B[i_8-1][i_7][i_6+1] = G[i_8][i_8][1] * H[i_8][i_8][i_6] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4424311115_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
