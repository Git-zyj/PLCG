...
void kernel_4222211111_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        ...
        for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_N-1, i_0); i_2++) {
                B[i_2][i_1+1] = B[i_2+1][i_1+2] + B[i_2-1][i_1+1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4222211111_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
