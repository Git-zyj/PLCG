...
void kernel_2416311215_09(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = 1; i_2 < min(i_0, PB_L+1); i_2++) {
            B[i_0][i_2-1] = B[i_0+1][i_2-1] - B[i_0-1][i_2-1] + C[i_2] * 5;
        }
        C[i_0] = E[i_0] + 4;
    }
...
kernel_2416311215_09(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E));
