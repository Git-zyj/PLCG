...
void kernel_2332132111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0] - D[i_0] * D[i_0-1] * E[i_0][i_0] - D[i_0] - A[i_0-1] + 6;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            B[i_1][i_2] = E[i_1+2][i_2] - 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        C[i_3] = D[i_3] - D[i_3-1] + D[i_3] - 3;
    }
#pragma endscop
...
kernel_2332132111_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
