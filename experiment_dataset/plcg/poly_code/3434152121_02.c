...
void kernel_3434152121_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0+2] = E[i_0] * E[i_0] + E[i_0-2] + E[i_0-2] - F[i_0][i_0-2] + E[i_0-2] + F[i_0][i_0] - E[i_0] - 3;
    }
    for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
        B[i_1+2] = E[i_1] + E[i_1] - E[i_1] + B[i_1+3] * 5;
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            for (int i_4 = max(1, i_2); i_4 < -i_3+PB_M; i_4++) {
                C[i_2][i_3][i_4] = G[i_4][i_2][0] * F[i_3][i_4-1] + G[i_4+i_3][i_3-1][i_2] + F[i_4][i_3] - C[i_2][i_3][i_4+2] + 6;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M; i_5++) {
        D[i_5] = E[i_5] - E[i_5] + A[i_5+3] * D[i_5-2] + 1;
    }
#pragma endscop
...
kernel_3434152121_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
