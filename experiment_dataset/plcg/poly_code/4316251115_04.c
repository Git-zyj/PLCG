...
void kernel_4316251115_04(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < min(min(-i_0+PB_Q+1, i_0), PB_M-1); i_3++) {
                    B[i_0][i_2+1][i_1][i_3-1] = D[i_1][i_0-1][i_2-1][i_3+i_0-1] * D[i_3][i_1][i_2][i_0] * E[i_3-1][i_0][i_2][i_1] + E[i_3+1][i_0][i_1][i_2-1] * D[i_3-1][i_1][i_2][i_0-1] * 1;
                }
            }
        }
    }
        ...
        for (int i_5 = 1; i_5 < -i_4+PB_M+1; i_5++) {
            for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 0; i_7 < i_4; i_7++) {
                    B[i_6-1][i_5+1][i_4][i_7] = D[i_6+1][i_5][i_7][i_4-1] + E[i_5+i_4-1][i_7][i_6][i_5] - D[i_6][i_4][i_7][i_5+1] - E[i_7][i_6+1][i_5-1][i_4-1] + E[i_5][i_7][i_6][i_4] * B[i_6-1][i_5+1][i_4+1][i_7+1] - 1;
                }
                C[i_6][i_4] = F[i_6+i_5][0][i_4] * G[i_4][i_6][i_5-1] - G[i_5][i_4][i_6] * C[i_4][i_6+1] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4316251115_04(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
