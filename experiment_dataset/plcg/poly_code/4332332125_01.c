...
void kernel_4332332125_01(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
        ...
        for (int i_1 = max(2, i_0); i_1 < min(-i_0+PB_P, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_M-3, i_0); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_M, -i_1+PB_M+2); i_3++) {
                    B[i_0+1][i_2+1][i_3+i_1-2][i_2] = D[i_3][i_1+i_0][i_3][i_2] + E[i_1+1][i_2-1][i_3][i_2][i_0+1] - B[i_1+1][i_2+3][i_3+i_0][i_2+2] + 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < PB_Q-2; i_5++) {
            for (int i_6 = 2; i_6 < PB_M; i_6++) {
                for (int i_7 = 0; i_7 < min(min(-i_6+PB_M, i_4), -i_5+PB_M+1); i_7++) {
                    B[i_4+1][i_5+1][i_7+i_6][i_5] = F[i_5][i_4][i_6][0] + G[i_4][i_6-2][i_5+2][i_7][0] + D[i_7+i_5-1][0][i_4][i_6] * 4;
                }
            }
            for (int i_8 = 1; i_8 < PB_M-1; i_8++) {
                for (int i_9 = 2; i_9 < -i_8+PB_M-1; i_9++) {
                    B[i_4+1][i_5][i_9+i_8+1][i_5-1] = H[i_5][i_4+2][i_9][i_8] + H[i_9-2][i_5][i_4][i_8-1] * 5;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4332332125_01(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
