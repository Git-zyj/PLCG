...
void kernel_2226212115_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M+1; i_1++) {
            B[i_0-1][i_1] = C[i_1+i_0-1][0] * 1;
        }
    }
    for (int i_2 = PB_N - 1; i_2 >= 1; i_2--) {
        for (int i_3 = 1; i_3 < min(PB_M, i_2); i_3++) {
            B[i_2-1][i_3] = B[i_2][i_3-1] * 3;
        }
    }
#pragma endscop
...
kernel_2226212115_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
