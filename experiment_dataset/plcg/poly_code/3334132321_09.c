...
void kernel_3334132321_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0][i_1+1] = D[i_1][i_0] - D[i_1-1][i_0] + D[i_1][i_0+2] * A[i_1-2][i_0][i_1-1] * A[i_1][i_0+2][i_1+1] - 5;
        }
        for (int i_2 = 2; i_2 < min(-i_0+PB_P+1, PB_M-2); i_2++) {
            for (int i_3 = 0; i_3 < PB_M; i_3++) {
                B[i_2][i_0-2][i_2] = E[i_0+1][i_2+i_0-1][i_3+i_2-2] * E[i_3][i_0-1][i_2] * B[i_2+2][i_0-2][i_2+2] - B[i_0+1][i_2-2][i_0+1] - B[i_2][i_0][i_2] + 3;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 2; i_5 < -i_4+PB_M-2; i_5++) {
            for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
                C[i_6][i_5+2][i_5] = E[i_6][i_5+i_4][i_4] * E[i_5+i_4+2][i_6+1][i_5] * E[i_5][i_4-1][i_4+1] * C[i_6][i_5+1][i_5-1] - C[i_6][i_5][i_5-2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3334132321_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
