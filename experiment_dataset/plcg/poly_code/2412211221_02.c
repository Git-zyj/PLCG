...
void kernel_2412211221_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        A[i_0] = A[i_0-2] - 1;
        for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
            B[i_1][i_0] = A[i_1] - B[i_0][i_1+2] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        C[i_2] = 6;
        for (int i_3 = 0; i_3 < -i_2+PB_L-2; i_3++) {
            D[i_3+1][i_3+i_2] = E[i_2][i_3] - A[i_3] * D[i_2+1][i_3+i_2+2] - 4;
        }
    }
#pragma endscop
...
kernel_2412211221_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
