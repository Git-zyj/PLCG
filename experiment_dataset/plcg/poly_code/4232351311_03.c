...
void kernel_4232351311_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0][i_1-1] = C[i_0][i_0] - C[i_1][i_0-1] * D[i_0][i_1+1] - E[i_1] + F[i_1] - F[i_1] * A[i_1+i_0-1][i_1-1] - A[i_1+i_0-1][i_1-2] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, i_3); i_4++) {
                B[i_2][i_3][i_4] = G[i_4+i_3][i_2][i_2] * D[i_2][i_4] - H[i_4][i_2-1][0] * B[i_2-1][i_3][i_4] - B[i_2+1][i_3][i_4] * B[i_2][i_3][i_4+1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4232351311_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
