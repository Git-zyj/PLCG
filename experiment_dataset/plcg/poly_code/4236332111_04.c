...
void kernel_4236332111_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 2; i_2 < i_1; i_2++) {
                B[i_1-2][i_1][0] = C[i_1-2][i_2+i_1][i_0-2] - C[i_2][i_1+i_0][i_2+i_1] * C[i_2][i_0-2][i_1] + C[i_2-2][i_0-2][i_1+1] - C[i_1-2][i_0][i_2] - 1;
            }
        }
        for (int i_3 = 2; i_3 < PB_P-1; i_3++) {
            for (int i_4 = 0; i_4 < i_3; i_4++) {
                B[i_3-2][i_3][0] = C[i_4][i_0][i_3] - B[i_3-1][i_3+1][0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4236332111_04(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
