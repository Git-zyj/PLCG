...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_N-2; i_1++) {
        B[i_1] = E[i_1] + F[i_1-1] - B[i_1+2] + B[i_1-1] - B[i_1-2] + B[i_1+1] + 1;
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_M+1, i_2); i_3++) {
            B[i_3-1] = G[i_3+i_2-1][1] * H[i_3][i_2] * G[i_3-1][i_2-1] + F[i_2] * 3;
        }
    }
#pragma endscop
...
kernel_2336331225_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
