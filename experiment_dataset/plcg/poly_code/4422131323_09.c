...
void kernel_4422131323_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = i_0; i_3 < min(-i_1+PB_M-1, -i_0+PB_P); i_3++) {
                    A[i_3+i_1][i_2-1][i_0][i_2] = E[i_0+1][i_3][i_1+1][i_1] - E[i_1][i_3][i_0][i_2] - E[i_2][i_3][i_0][i_1+1] * E[i_1+1][i_2][i_3+i_0][0] - A[i_3+i_1-1][i_2-1][i_0-2][i_2] * A[i_3+i_1+1][i_2-1][i_0][i_2] - A[i_3+i_1-1][i_0-1][i_2][i_0] - 3;
                }
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 3; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 1; i_7 < PB_P; i_7++) {
                    B[i_5][i_7][i_7][i_6-1] = F[i_6+1][i_5][i_4+1][i_4][i_6] + B[i_5][i_7-1][i_7-1][i_6-3] * B[i_5+1][i_7][i_7][i_6+1] - 5;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_P; i_8++) {
        for (int i_9 = 0; i_9 < PB_M; i_9++) {
            for (int j_0 = 0; j_0 < PB_M; j_0++) {
                for (int j_1 = 0; j_1 < min(min(min(-j_0+PB_M, -i_8+PB_P), i_8), -i_9+PB_P); j_1++) {
                    A[j_1+j_0][i_9][i_8][i_9+1] = F[i_9][j_0][j_1+i_8][i_8-1][0] * E[j_0][i_8][j_1+i_9][j_1] + 1;
                }
            }
        }
        for (int j_2 = 2; j_2 < PB_M-2; j_2++) {
            D[j_2] = G[j_2][i_8] * G[j_2][j_2] * H[j_2] + H[j_2+1] + G[i_8-1][j_2] + D[j_2-2] - D[j_2+2] - D[j_2+1] + D[j_2-1] * 2;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422131323_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
