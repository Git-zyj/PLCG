...
void kernel_4234331321_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
        ...
        for (int i_1 = 1; i_1 < PB_Q-3; i_1++) {
            for (int i_2 = i_0; i_2 < -i_1+PB_Q; i_2++) {
                for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
                    A[i_2+1][i_3][i_1+1][i_3] = C[i_2][i_0][i_1-1][1] + D[i_0+1][i_3][i_1-1][i_2+i_1] * E[i_2+i_1][i_0][i_3-1][i_1][i_2] * A[i_2+1][i_3+1][i_1+3][i_3+1] + A[i_2+1][i_3+2][i_1+1][i_3+2] * A[i_2+2][i_3-1][i_1+1][i_3-1] - A[i_3+1][i_0-2][i_1+1][i_0-2] + 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        B[i_4] = F[i_4+1] * G[i_4+1] - B[i_4-1] + B[i_4-2] + 4;
    }
#pragma endscop
...
kernel_4234331321_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
