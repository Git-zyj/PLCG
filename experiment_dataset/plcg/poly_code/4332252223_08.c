...
void kernel_4332252223_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_M-1; i_1++) {
            B[i_0+2][i_1] = D[i_0] + E[i_1-2] * F[i_0][i_1+i_0+1] + F[i_1][i_1-2] * F[i_1][i_0] * F[i_1][i_0] * G[i_1-1][i_0+1] * F[i_1][i_0+2] * 1;
        }
        for (int i_2 = 1; i_2 < min(-i_0+PB_N, PB_L-1); i_2++) {
            B[i_0][i_2] = G[i_2][i_0] - G[i_2+i_0][0] - H[i_2+1][i_0][i_2] - F[i_0][i_2-1] + B[i_0][i_2+1] * B[i_0+1][i_2] * 5;
        }
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 2; i_4 < PB_L; i_4++) {
                B[i_3+2][i_4-2] = I[i_4][i_3][i_0] + H[i_4][i_0][i_3+2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4332252223_08(xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
