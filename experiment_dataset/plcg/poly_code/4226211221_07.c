...
void kernel_4226211221_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 2; i_3 < -i_1+PB_P; i_3++) {
                    A[i_0][i_2+1][i_3+i_1][i_3] = C[i_1][i_3][i_0+1] - A[i_2][i_0+1][i_3+i_1-2][i_3-2] * 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_Q; i_4++) {
        for (int i_5 = 2; i_5 < PB_P-3; i_5++) {
            B[i_5][i_5+1] = B[i_5-1][i_5] + B[i_5+2][i_5+3] * B[i_5-2][i_5-1] * 1;
        }
    }
#pragma endscop
...
kernel_4226211221_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
