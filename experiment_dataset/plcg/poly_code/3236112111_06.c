...
void kernel_3236112111_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_0][i_0][i_2] = A[i_2][i_2][i_1-1] * B[i_0+2][i_2][i_2] + 4;
            }
        }
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            B[i_0+2][i_3][i_3] = C[i_3+1] + 4;
        }
    }
#pragma endscop
...
kernel_3236112111_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
