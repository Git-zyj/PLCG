...
void kernel_3334112213_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_3 = 1; i_3 < PB_N; i_3++) {
            B[i_0][i_3] = D[i_0][i_3] + B[i_0+1][i_3+1] + B[i_0][i_3-1] + 6;
        }
    }
...
kernel_3334112213_05(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
