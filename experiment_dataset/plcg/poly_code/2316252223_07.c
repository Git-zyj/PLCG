...
void kernel_2316252223_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_L-1; i_0++) {
        A[i_0-2] = D[i_0] - E[i_0-2] + E[1] - A[i_0-4] + A[i_0] + 4;
        for (int i_1 = min(PB_L, -i_0+PB_N) - 1; i_1 >= 2; i_1--) {
            A[i_1-2] = F[i_1][i_1] - G[i_1-1][i_1+1][i_0+2] * H[i_1+2][i_0] - D[i_1] + H[i_1+i_0][i_1+1] + F[i_1+1][i_0+1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_L-2; i_3++) {
            C[i_3][i_2] = F[i_3][i_2] + I[i_3][i_3+i_2+2][i_2] - H[i_3][i_2] * H[i_2][i_3] - H[i_3][i_2-1] * A[i_3-1] - C[i_3][i_2+2] - 1;
        }
    }
#pragma endscop
...
kernel_2316252223_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
