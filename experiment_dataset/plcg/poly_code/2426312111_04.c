...
void kernel_2426312111_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_2+2][i_3] = E[i_2][i_3] * B[i_2+2][i_3+1] + 3;
        }
        for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
            C[i_2][i_4] = F[i_4-2] * C[i_2][i_4+1] * 5;
        }
        for (int i_5 = i_2; i_5 < -i_2+PB_L-2; i_5++) {
            D[i_5+i_2+1][i_2-2][0] = D[i_5+i_2+2][i_2-1][0] + 4;
        }
    }
#pragma endscop
...
kernel_2426312111_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
