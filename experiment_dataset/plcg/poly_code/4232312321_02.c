...
void kernel_4232312321_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            for (int i_4 = max(1, i_2); i_4 < PB_M-1; i_4++) {
                A[i_3][i_4+1] = A[i_4-1][i_3+1] - A[i_3+1][i_4+1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4232312321_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
