...
void kernel_4424252111_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
        ...
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L-2; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_P-1; i_3++) {
                    A[i_3+i_2+1][i_1-1][i_2][i_3+i_1] = E[i_0-2][i_3+2][i_2+1][i_2] + F[i_1][i_0-1][i_2][i_3] + F[i_2][i_3][i_0][i_1+1] + G[i_2][i_2+i_0+2][1][i_3][i_0+1] + H[i_3][i_1-2][i_2] + A[i_3+i_2+3][i_1-1][i_2+1][i_3+i_1+1] - 6;
                }
            }
        }
        B[i_0] = I[0] + I[i_0] * J[i_0+1] * J[i_0+1] * I[i_0] * 2;
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_M+1; i_5++) {
            for (int i_6 = 0; i_6 < min(-i_5+PB_M, -i_4+PB_L); i_6++) {
                C[i_6+i_5][i_6+i_4] = H[i_5-1][i_4-2][i_5] + H[i_5][i_6+i_4][i_5] + H[i_4+2][i_6][i_5] - C[i_6+i_4-1][i_5+i_4-1] + 3;
            }
        }
        D[i_4] = J[i_4] * I[i_4+1] + K[i_4][i_4] * I[i_4] * I[i_4-1] - I[i_4-2] - I[i_4] * 6;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4424252111_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
