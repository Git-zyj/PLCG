...
void kernel_2234132111_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            B[i_1][i_2][i_2+i_1] = D[i_2+i_1][i_2][i_1+2] * 5;
        }
    }
#pragma endscop
...
kernel_2234132111_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
