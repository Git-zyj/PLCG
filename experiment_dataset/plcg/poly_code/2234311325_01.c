...
void kernel_2234311325_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_1+i_0-1][i_0][i_1] = C[i_1][i_0] * A[i_1+i_0][i_0+1][i_1] + A[i_1+i_0][i_0+2][i_1-1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_M+1, i_2); i_3++) {
            B[i_2][i_3+1] = A[i_3+i_2-1][i_2][i_3] + B[i_2-1][i_3+1] * B[i_2][i_3] + 6;
        }
    }
#pragma endscop
...
kernel_2234311325_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
