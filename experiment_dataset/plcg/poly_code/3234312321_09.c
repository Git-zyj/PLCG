...
void kernel_3234312321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = i_1; i_2 < PB_N-2; i_2++) {
            for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
                B[i_2][i_3+2] = B[i_2+2][i_3+2] * B[i_2][i_3+1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3234312321_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
