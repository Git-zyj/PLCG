...
void kernel_4336211115_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        ...
        for (int i_2 = max(2, i_1); i_2 < PB_L; i_2++) {
            C[i_2-1][i_1] = C[i_2-2][i_1] - C[i_2][i_1] + 1;
        }
    }
#pragma endscop
...
kernel_4336211115_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
