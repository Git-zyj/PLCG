...
void kernel_3222332321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P; i_0++) {
        A[i_0-2] = C[i_0-2] + D[i_0][i_0] * A[i_0-3] - B[i_0+2][0] + 5;
        for (int i_1 = 4; i_1 < min(PB_P, -i_0+PB_L); i_1++) {
            B[i_0][0] = E[i_1-2][i_1-2] + E[i_1][i_1+i_0] + D[i_0][i_1] + A[i_0-2] + A[i_1-2] - A[i_1-4] * 3;
        }
    }
#pragma endscop
...
kernel_3222332321_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
