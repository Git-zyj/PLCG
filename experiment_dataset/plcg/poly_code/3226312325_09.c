...
void kernel_3226312325_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(i_0, 4); i_1 < PB_L; i_1++) {
            A[i_0][i_1-2][i_1] = A[i_0][i_1-1][i_1+1] - A[i_0][i_1][i_1+2] * A[i_0+1][i_1-2][i_1] + A[i_0][i_1-4][i_1-2] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_3+1] = C[i_3][i_2-1] + B[i_2] - B[i_3-1] * 1;
        }
    }
#pragma endscop
...
kernel_3226312325_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
