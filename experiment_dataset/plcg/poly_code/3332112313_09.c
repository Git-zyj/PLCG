...
void kernel_3332112313_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_0] = A[i_0+1] - A[i_0-1] - A[i_1-1] * A[i_1+1] - A[i_0-1] - 5;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_M-2; i_2++) {
            for (int i_3 = 0; i_3 < i_2; i_3++) {
                B[i_2+i_0][i_0][i_3] = D[i_2][i_3][i_0+1] * B[i_2+i_0-1][i_0][i_3+1] + B[i_2+i_0-2][i_0-1][i_3+1] * B[i_2+i_0+2][i_0+1][i_3+1] - 3;
            }
        }
        for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
                C[i_0+1][i_4][i_4-2] = D[i_0-2][i_5+1][i_4] - C[i_0+1][i_4+1][i_4-1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3332112313_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
