...
void kernel_2332312125_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M+1, PB_M-2); i_1++) {
            A[i_0][i_1+2] = D[i_1+i_0-1][i_1] - D[i_1+1][i_0] * 6;
        }
    }
    for (int i_2 = 4; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_M, i_2); i_3++) {
            C[i_2-1][i_3-2] = D[i_3][i_2] - 3;
        }
        for (int i_4 = 2; i_4 < PB_M+1; i_4++) {
            C[i_2-1][i_4-2] = C[i_4-1][i_2-4] * 6;
        }
    }
#pragma endscop
...
kernel_2332312125_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
