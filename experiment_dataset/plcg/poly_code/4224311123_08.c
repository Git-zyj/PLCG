...
void kernel_4224311123_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
        ...
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 1; i_4 < PB_Q; i_4++) {
                for (int i_5 = 0; i_5 < -i_3+PB_P+1; i_5++) {
                    B[i_2][i_3+1][i_4-1][i_5+i_3-1] = C[i_3][i_5+1][i_2][i_4] - 4;
                }
            }
        }
    }
...
kernel_4224311123_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
