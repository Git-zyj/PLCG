...
void kernel_3232312221_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            A[i_1-1][i_0][0] = A[i_1-1][i_0+1][0] + A[i_1+1][i_0][0] - A[i_1-1][i_0-1][0] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, PB_P-2); i_4++) {
                B[i_2-1][i_3][i_4] = C[i_4+i_3][i_2][i_2+2] - B[i_2-1][i_3][i_4+2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3232312221_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
