...
void kernel_2312331321_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            A[i_1-1][i_0] = D[i_1][i_0] + A[i_1-2][i_0-1] * A[i_1-1][i_0-2] + A[i_1+1][i_0] * A[i_1][i_0] + A[i_0-1][i_1-1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        B[i_2][i_2-1] = E[i_2] * F[i_2] - G[i_2+1][0] * E[i_2] - B[i_2+2][i_2+1] * 3;
        for (int i_3 = 1; i_3 < min(-i_2+PB_L, PB_L-2); i_3++) {
            C[i_3][i_2] = E[i_3+i_2] + G[i_2][i_3] + D[i_3-1][i_2] - C[i_2][i_3+2] * C[i_3][i_2+2] * C[i_3][i_2+1] + 4;
        }
    }
#pragma endscop
...
kernel_2312331321_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
