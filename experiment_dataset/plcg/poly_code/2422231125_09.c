...
void kernel_2422231125_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        B[i_0] = 4;
        B[i_0] = E[i_0] * E[i_0+1] * F[i_0-1] * B[i_0+1] + 6;
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            C[i_0][i_1] = G[i_1][i_0] * C[i_0][i_1+2] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            B[i_3-2] = H[i_2][i_3-1] * G[i_2-1][i_3] * G[i_3+1][i_2] - I[i_2][i_3][i_2-1] + G[i_3][i_2] + 4;
        }
    }
#pragma endscop
...
kernel_2422231125_09(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
