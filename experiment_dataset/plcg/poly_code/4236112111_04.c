...
void kernel_4236112111_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_M-3, -i_1+PB_L); i_2++) {
            B[i_1][i_1+2] = D[i_2+i_1][i_1] - B[i_2+1][i_2+3] - 6;
        }
    }
#pragma endscop
...
kernel_4236112111_04(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
