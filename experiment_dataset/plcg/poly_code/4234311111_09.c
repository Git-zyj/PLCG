...
void kernel_4234311111_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
        ...
        for (int i_5 = 0; i_5 < PB_Q; i_5++) {
            for (int i_6 = i_4; i_6 < PB_P-1; i_6++) {
                for (int i_7 = 1; i_7 < PB_L; i_7++) {
                    B[i_6][i_7][i_6+1][i_4+1] = C[i_7][i_4-1][i_7+1][i_6] + A[i_4+1][i_6-1][i_7-1] * 5;
                }
            }
        }
    }
...
kernel_4234311111_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
