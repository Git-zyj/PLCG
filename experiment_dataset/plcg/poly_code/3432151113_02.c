...
void kernel_3432151113_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0-1] = E[i_0-1] - E[i_0] + E[i_0] - A[i_0] * 5;
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_L, -i_0+PB_P-1); i_2++) {
                C[i_2][i_1][i_1] = F[i_2][i_2+i_0+1][i_2] + G[i_1][i_0] * F[i_1][i_0][i_2] + F[i_2-1][i_0][i_2+1] + 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 0; i_5 < min(PB_L, -i_4+PB_Q); i_5++) {
                C[i_3+1][i_5][i_5] = F[i_3][i_3][i_5+i_4] * F[i_3][i_5][i_4] + F[i_3+1][i_5][i_3-1] + F[i_3-1][i_5][i_3] + F[i_5][i_3][i_4-1] * G[i_5][i_4+1] * D[i_5+1] * 3;
            }
            D[i_4] = G[i_3][i_4] + E[i_4] + G[i_4][i_3-1] - F[i_4][i_3+1][i_4] - G[i_4][i_3] - E[i_4+1] + D[i_4-1] - 3;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3432151113_02(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
