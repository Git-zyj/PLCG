...
void kernel_2434152213_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0-2][i_1-1] = E[i_1][i_0] + F[i_1] + E[i_0][i_0-2] - E[i_1+i_0][2] + E[i_0][i_1+i_0] - E[i_1][i_0] * E[i_0][i_1-1] - E[i_1][i_0] + D[i_0+1][i_0][i_1] + 4;
        }
    }
    for (int i_2 = 3; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_M; i_3++) {
            B[i_2][i_2-2] = E[i_3][i_2+2] * E[i_3][i_2] - E[i_3+i_2][i_3+i_2] * E[i_3+i_2-2][i_3-2] + B[i_2-1][i_2-3] - B[i_2+1][i_2-1] + 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < -i_4+PB_M; i_5++) {
            C[i_5][i_4] = E[i_5-2][i_4-1] + E[i_5+i_4][2] - C[i_5+1][i_4] - C[i_4+1][i_5] * 5;
        }
    }
    for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
        for (int i_7 = i_6; i_7 < PB_M-2; i_7++) {
            D[i_6+1][i_6][i_7] = E[0][i_6+1] + E[i_6][i_7+1] * E[i_7][i_6+1] + E[i_7][i_6] * D[i_7+2][i_7+1][i_6] * 4;
        }
    }
#pragma endscop
...
kernel_2434152213_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
