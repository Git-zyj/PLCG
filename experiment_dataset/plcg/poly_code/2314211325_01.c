...
void kernel_2314211325_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            A[i_1-1][i_1+1] = A[i_1+1][i_1+3] + A[i_1][i_1+2] + A[i_1-2][i_1] - 3;
        }
    }
    for (int i_2 = 3; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < PB_N; i_3++) {
            B[i_2][i_3] = D[i_2][i_3] - 4;
        }
        for (int i_4 = i_2; i_4 < PB_M-2; i_4++) {
            C[i_4] = A[i_2-3][i_2-1] - C[i_2-1] + C[i_4-2] * C[i_2+1] * C[i_4+2] + 3;
        }
    }
#pragma endscop
...
kernel_2314211325_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
