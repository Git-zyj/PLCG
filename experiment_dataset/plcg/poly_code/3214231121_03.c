...
void kernel_3214231121_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M; i_1++) {
            A[i_0][i_1+1] = C[i_1-1][i_0+1] + D[i_1][i_1] + E[i_0] * C[i_1][i_0] * 2;
        }
        for (int i_2 = 3; i_2 < PB_M; i_2++) {
            for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_M); i_3++) {
                B[i_3+i_2][i_2-1] = F[i_3+1][i_0][i_3] * B[i_3+i_2-3][i_2-3] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3214231121_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
