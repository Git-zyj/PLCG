...
void kernel_2416231325_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_1] = E[i_0+1][i_1] + A[i_0+1][i_1] + D[i_1+1] * 3;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
            A[i_0][i_2] = F[i_2][i_0] + F[i_2][i_0] * G[i_2+i_0-1][i_2][0] * E[i_2][i_0] * 2;
            A[i_0][i_2-1] = F[i_2][i_0+1] - F[i_2+i_0][1] * E[i_0][i_2-1] + H[i_2] - F[i_0][i_2] * 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-3; i_3++) {
        D[i_3+1] = I[i_3] - I[i_3] + D[i_3-1] - D[i_3+2] * D[i_3] - D[i_3+3] - 5;
    }
#pragma endscop
...
kernel_2416231325_02(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
