...
void kernel_3232151113_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0+1][i_1] = C[i_1] * D[i_1][i_0-1] * D[i_1-1][i_0+1] + D[1][i_0] - A[i_0+1][i_1-1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        B[i_2-1] = C[i_2] - C[i_2] * C[1] - C[i_2] * C[i_2] - 5;
    }
#pragma endscop
...
kernel_3232151113_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
