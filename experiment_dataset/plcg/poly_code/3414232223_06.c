...
void kernel_3414232223_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            A[i_0-2][i_1] = E[i_1] - F[i_0][i_1-2] + F[i_0][i_1] + F[i_1][i_1] - F[i_1][i_0-2] - A[i_0-2][i_1-2] * A[i_0-2][i_1+1] + 4;
            C[i_1+1] = G[i_1][i_0] - 6;
        }
        C[i_0] = E[i_0] + E[i_0] + C[i_0+1] - 5;
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_N, PB_L-2); i_3++) {
            for (int i_4 = 0; i_4 < i_3; i_4++) {
                D[i_3][i_4] = H[i_3-1][i_4][0] + H[i_4+i_3][i_3+i_2][i_2+1] + D[i_3+2][i_4] + D[i_3][i_2+1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3414232223_06(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
