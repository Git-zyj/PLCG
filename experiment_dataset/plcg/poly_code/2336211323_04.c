...
void kernel_2336211323_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            A[i_0-1][i_1] = A[i_0-3][i_1] * A[i_0+1][i_1-2] - 2;
        }
    }
    ...
    for (int i_3 = PB_M+1 - 1; i_3 >= 1; i_3--) {
        for (int i_4 = max(2, i_3); i_4 < PB_M-1; i_4++) {
            C[i_4-1] = D[i_3-1][i_4] - A[i_3-1][i_4] * C[i_4-2] + C[i_4+1] - 5;
        }
    }
#pragma endscop
...
kernel_2336211323_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
