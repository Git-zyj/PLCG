...
void kernel_3426311125_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        ...
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            A[i_0][i_0] = E[i_0][i_2] - 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = i_3; i_5 < PB_P; i_5++) {
                C[i_3][i_4-1][i_5] = F[i_4-1][i_3][i_4] + C[i_3][i_4][i_5] + 3;
            }
        }
        A[i_3+2][i_3+2] = G[i_3] - 6;
    }
#pragma endscop
...
kernel_3426311125_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
