...
void kernel_3226231221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_L+3; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                A[i_1][i_2-1] = C[i_2-1][i_0-1][i_1] - C[i_2][i_2][i_1] * D[i_2+1][i_0][i_1] - A[i_2][i_1-3] * A[i_1][i_2+1] + A[i_2][i_0+1] + 5;
            }
        }
        for (int i_3 = 0; i_3 < min(PB_L, -i_0+PB_P-1); i_3++) {
            for (int i_4 = i_3; i_4 < -i_3+PB_L; i_4++) {
                B[i_0-1][i_4+i_3][i_0] = C[i_3][i_4][i_0] * C[i_3][i_4][i_0+1] - D[i_4][i_3+i_0+1][i_0] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3226231221_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
