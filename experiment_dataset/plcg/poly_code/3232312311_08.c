...
void kernel_3232312311_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_L; i_2++) {
            B[i_1][0] = C[i_2] - D[i_2+i_1][i_2+i_1] + B[i_1-1][0] - B[i_1+1][0] * B[i_1+1][0] * 3;
        }
    }
#pragma endscop
...
kernel_3232312311_08(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
