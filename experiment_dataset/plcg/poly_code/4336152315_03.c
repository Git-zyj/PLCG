...
void kernel_4336152315_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N-2; i_1++) {
            A[i_0+1][i_1] = D[i_1+i_0][i_1] + D[i_0+2][i_1+i_0+2] + D[i_1][i_0] + D[i_0-2][i_1] + D[i_1][i_0] - E[i_1] + A[i_0+2][i_1] + A[i_0][i_1] + A[i_0+1][i_1+1] + 2;
        }
    }
        ...
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
                for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
                    A[i_5][i_3] = F[i_5][i_2][i_4] * G[i_5+1][i_4][i_3][i_2-2] * G[i_4-2][i_2+2][i_3+1][i_2-1] + H[i_3][i_4-1][i_2-1][i_5-2][i_4] - G[i_2+2][i_4+1][i_5][i_3] - 6;
                }
            }
        }
        for (int i_6 = 0; i_6 < -i_2+PB_L; i_6++) {
            for (int i_7 = 2; i_7 < i_6; i_7++) {
                A[i_6+1][i_7] = F[i_6+1][i_7+i_2-1][i_7] * F[i_7-2][i_6+i_2][i_2+1] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4336152315_03(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H)));
