...
void kernel_4224312225_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
                B[i_1][i_2+1][i_0+1] = C[i_1-1][i_0][i_2] - 5;
                for (int i_3 = 1; i_3 < PB_L; i_3++) {
                    B[i_3][i_1][i_2+1] = B[i_3][i_2+2][i_1+1] * B[i_3-1][i_1][i_0+1] + 2;
                }
            }
        }
    }
...
kernel_4224312225_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
