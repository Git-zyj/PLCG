...
void kernel_4436331115_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            B[i_0+1][i_1] = E[i_1][i_1] + E[i_1][i_0+1] * F[i_1] - G[i_0][i_0] - E[i_1+1][i_1+i_0] * 3;
            for (int i_2 = 0; i_2 < -i_1+PB_P; i_2++) {
                B[i_2+1][i_1] = H[i_0][i_2+i_1][i_1+i_0-1] - B[i_2][i_1] + 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_L, PB_P-1); i_5++) {
                for (int i_6 = 1; i_6 < min(PB_P, -i_4+PB_Q); i_6++) {
                    D[i_4][i_3][i_5+1][i_6] = I[i_3][i_6+1][i_4][i_5] * J[i_5+1][i_3][i_3+1][i_6+i_4] * J[i_4][i_5+i_3][i_6-1][0] + J[i_3][i_5][i_6][1] + 1;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
        for (int i_8 = 1; i_8 < i_7; i_8++) {
            for (int i_9 = 0; i_9 < min(-i_8+PB_P, -i_7+PB_Q-1); i_9++) {
                for (int j_0 = 1; j_0 < -i_7+PB_M-1; j_0++) {
                    D[i_7+1][i_8][i_9+1][j_0+1] = J[i_7][j_0-1][i_9+i_8][i_7-1] + K[j_0+i_7+1][i_8-1][i_9+i_7+1][i_8] * 1;
                }
            }
        }
    }
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4436331115_02(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K)));
