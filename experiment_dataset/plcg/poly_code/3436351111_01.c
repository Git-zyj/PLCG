...
void kernel_3436351111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-1] * E[i_0-1] * F[i_0-1] - F[i_0] * A[i_0+1] + A[i_0-1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 2; i_2 < min(PB_Q, -i_1+PB_M+1); i_2++) {
            B[i_1][i_2-1][i_1] = G[i_1-1][i_2-1] - H[i_1][i_2] * G[i_2][i_2+i_1-1] + B[i_1-1][i_2][i_1-1] + B[i_1-1][i_2-2][i_1-1] - 5;
        }
        for (int i_3 = 1; i_3 < -i_1+PB_N; i_3++) {
            C[i_3+i_1+1][i_1] = E[i_3] - G[i_3-1][i_1+1] * G[i_3+1][i_1-1] + I[i_3][i_1-1] - G[i_3+i_1][0] - 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        D[i_4-1] = E[i_4-1] - J[i_4] * F[i_4] * E[i_4] + E[i_4] + 4;
    }
#pragma endscop
...
kernel_3436351111_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
