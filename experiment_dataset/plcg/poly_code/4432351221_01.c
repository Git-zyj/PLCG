...
void kernel_4432351221_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q-1, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_Q, -i_1+PB_M), PB_M-1); i_2++) {
                for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                    A[i_2+i_1][i_0][i_2][i_3] = E[i_2][i_3][i_1] * F[i_2][i_3][0] - G[i_2+1][i_3+1][i_1+i_0+1][1] + H[i_2][i_3][i_1][i_0+1] + G[i_1+1][i_3][i_0+1][i_2+i_0] * 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_M-1; i_5++) {
            B[i_4][i_5+i_4-1] = I[i_5][i_5+i_4+1] - J[i_4][i_5+i_4+1] - K[i_5+i_4] + J[i_5][i_4+1] * D[i_4-2] * D[i_5-1] * 3;
            for (int i_6 = 0; i_6 < -i_4+PB_M; i_6++) {
                C[i_6][i_4] = F[i_6+i_4][i_4][i_5] - C[i_6][i_5+1] - 2;
            }
        }
        D[i_4-1] = L[i_4-1] + K[i_4+1] + L[i_4] * M[i_4+1] - M[i_4] + M[i_4] + L[i_4-1] - L[i_4] + D[i_4] + D[i_4+1] + 2;
    }
#pragma endscop
