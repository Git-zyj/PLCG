...
void kernel_4334351113_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] * E[i_0] + D[i_0] * E[i_0] * F[i_0] * E[i_0] * D[i_0] + D[i_0] + A[i_0+1] - 1;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < min(i_1, -i_1+PB_Q-1); i_2++) {
            for (int i_3 = 1; i_3 < PB_P; i_3++) {
                for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
                    B[i_3+1][i_4][i_2][i_1] = G[i_1][i_4][i_2-1][i_3-1] * G[i_3][i_4][i_2+i_1+1][i_4+i_2] - H[i_3][i_1+1][i_2][i_4] + G[i_4+1][i_3][i_2+i_1][1] - I[i_2][i_4][i_1+1][0] + 2;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        for (int i_6 = i_5; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 0; i_7 < min(-i_6+PB_M, PB_P); i_7++) {
                C[i_6][i_7] = J[i_7+i_6][i_5][0] * K[i_6-1][i_5-1][i_6] + C[i_6+1][i_7] + 2;
            }
        }
    }
#pragma endscop
