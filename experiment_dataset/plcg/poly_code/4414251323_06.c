...
#endif
static void init_array(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int seed)
{
srand(seed);
int i_0, i_1, i_2, i_3;
...
static void print_array(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4414251323_06(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 3; i_2 < min(min(-i_1+PB_N, -i_0+PB_L), i_0); i_2++) {
                C[i_0][i_2+i_1-2][i_2-3][i_1+1] = E[i_1][i_2][i_2+1][i_0] * F[i_2+i_1][i_0][i_2+i_0] + G[i_2+1][i_0+1][i_1+1] * F[i_2][i_0][i_1] * G[i_0+1][i_2][i_1] + H[i_2][i_1] - G[i_2+i_0][i_1][1] * B[i_2-1] + 3;
            }
        }
        for (int i_3 = 1; i_3 < -i_0+PB_L; i_3++) {
            B[i_3-1] = H[i_3][i_0] * I[i_0][i_3] * I[i_3+1][i_0+1] * G[i_0][i_3+1][0] * B[i_3+1] + B[i_3] * 5;
            for (int i_4 = i_0; i_4 < PB_M; i_4++) {
                for (int i_5 = 1; i_5 < -i_3+PB_L-1; i_5++) {
                    C[i_5][i_3+i_0][i_0-1][i_3+1] = J[i_4][i_5][i_3+1][i_0] * J[i_5][i_5+1][i_0][i_4] + J[i_0][i_3-1][i_4][i_5] - C[i_0-2][i_5+i_3+1][i_5-1][i_3+2] + 1;
                }
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M; i_6++) {
        for (int i_7 = 1; i_7 < PB_L; i_7++) {
            B[i_7-1] = H[i_6][i_7] * K[i_7-1] - I[i_7][0] + I[i_7][i_6+1] + B[i_6-1] + 5;
        }
    }
#pragma endscop
...
init_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4414251323_06(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K)));
