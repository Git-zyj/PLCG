...
void kernel_2324351215_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = D[i_0+1] * E[i_0+1] + D[i_0] - E[i_0] - A[i_0-1] - 4;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_L; i_2++) {
            A[i_2] = F[i_1][i_2] + G[i_2][i_1-1] + H[i_1][i_2+1] - H[i_1][i_2+i_1] - H[i_1][i_2] + I[1] * I[i_2] + A[i_2] - 6;
            A[i_2-1] = G[i_1][i_2+i_1] - F[i_1][i_2] - F[i_2+i_1][0] + E[i_1] + 3;
        }
    }
#pragma endscop
...
kernel_2324351215_07(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
