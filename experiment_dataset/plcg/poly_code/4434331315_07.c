...
void kernel_4434331315_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] - F[i_0-1] * A[i_0-1] * 4;
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_M+1); i_1++) {
            B[i_1+1][i_0] = G[i_1+i_0-1][i_1] - B[i_1+1][i_0-1] - B[i_1+1][i_0+1] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_L; i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_P; i_4++) {
                B[i_3+1][i_2] = H[i_2][i_4+i_3][i_3+1] * I[i_3][i_2-1][i_4] + I[i_3][i_2][i_4] * J[i_4][i_3+1][i_2-1][i_2] * K[i_4+1][i_3][i_2+1] * K[i_3][i_4-1][i_2+1] * 3;
            }
            for (int i_5 = 0; i_5 < PB_Q-1; i_5++) {
                for (int i_6 = 0; i_6 < PB_Q-1; i_6++) {
                    D[i_6][i_3][i_2-1][i_5] = L[i_6+1][i_2-1][i_3][i_5] * H[i_3+1][i_5][i_2] + M[i_5][i_2-1][i_6][i_2] - D[i_2][i_5][i_3-2][i_6+1] * D[i_6][i_3][i_2-1][i_5+1] + D[i_6+1][i_3-1][i_2-1][i_5+1] - D[i_6][i_3][i_2][i_5] + 1;
                }
            }
        }
    }
...
kernel_4434331315_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M));
