...
void kernel_2324311321_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[0] = D[i_0] + B[i_0] + B[i_0+1] + B[i_0-1] * 2;
        B[i_0] = B[i_0-2] * 1;
    }
    for (int i_1 = PB_L - 1; i_1 >= 2; i_1--) {
        for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
            C[i_1][i_2] = C[i_1-2][i_2] * C[i_1+1][i_2] + C[i_2-2][i_1] - C[i_1+2][i_2] + C[i_1][i_2+2] + 5;
        }
    }
#pragma endscop
...
kernel_2324311321_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
