...
void kernel_2432212111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
            B[i_1+2][i_2+1] = B[i_1+1][i_2+1] + 3;
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        C[i_3] = C[i_3+1] * 3;
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            D[i_3+2][i_4] = E[i_3+1][i_4] * 1;
        }
    }
#pragma endscop
...
kernel_2432212111_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
