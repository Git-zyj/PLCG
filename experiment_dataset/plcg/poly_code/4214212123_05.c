...
void kernel_4214212123_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_P-3; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_2+1][i_0][i_1+i_0+2] = C[i_2][i_1-2][i_0+2] * A[i_2+1][i_0+1][i_1+i_0+3] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_L, -i_3+PB_M-1); i_4++) {
            B[i_4+i_3][i_4] = B[i_4+i_3+1][i_4] * 2;
        }
    }
#pragma endscop
...
kernel_4214212123_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
