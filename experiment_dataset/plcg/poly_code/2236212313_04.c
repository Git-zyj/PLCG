...
void kernel_2236212313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        A[i_0+2] = C[i_0] - A[i_0+3] - 1;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_1+2] = B[i_1+1] + B[i_1+3] + B[i_1+1] * B[i_1+3] + 4;
        }
    }
#pragma endscop
...
kernel_2236212313_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
