...
void kernel_4224232321_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_P, i_1); i_2++) {
                A[i_2][i_0][i_1] = C[i_2][i_0][i_1] + D[i_0][i_1] * C[i_1][i_2][i_2+i_1] * D[i_2-1][i_1] * D[i_0][i_2+2] + A[i_2-1][i_0][i_1] * A[i_0-2][i_1-1][i_2] + A[i_2][i_0-2][i_1] * A[i_0+2][i_1][i_2-2] * A[i_1+2][i_2][i_0+1] - 2;
            }
        }
        for (int i_3 = i_0; i_3 < min(PB_L-3, -i_0+PB_N); i_3++) {
            B[i_3+1] = E[i_3+i_0][i_3+2] * B[i_3+3] * 2;
        }
    }
#pragma endscop
...
kernel_4224232321_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
