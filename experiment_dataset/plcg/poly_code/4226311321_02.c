...
void kernel_4226311321_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 3; i_2 < min(-i_0+PB_L, PB_M+3); i_2++) {
                A[i_1-1][i_2] = C[i_2+i_0][i_1-1] * A[i_1-1][i_2-2] + A[i_2-3][i_1] + 3;
            }
        }
        B[i_0] = D[1] * B[i_0+1] * B[i_0+2] - B[i_0-1] + 3;
    }
#pragma endscop
...
kernel_4226311321_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
