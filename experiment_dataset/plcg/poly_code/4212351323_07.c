...
void kernel_4212351323_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = i_1; i_2 < min(-i_1+PB_P, PB_M-1); i_2++) {
                for (int i_3 = 1; i_3 < min(min(min(-i_0+PB_P+1, -i_2+PB_M), PB_Q-3), -i_1+PB_Q); i_3++) {
                    A[i_0][i_2][i_1-1][i_3+1] = C[i_2+1][i_3+i_1-1][i_0][i_0-1] * D[i_3][i_1][i_2][i_0-1] * D[i_1][i_0][i_2-1][i_0-1] + E[i_2-1][i_1][i_3+1][i_0] + F[i_3][i_2+1][i_1][i_0][i_1-1] - E[i_3+i_2][i_0][i_1][i_3-1] * D[i_1][i_0][0][i_2] + E[i_2+1][i_0][i_3+i_0-1][i_1] - D[i_3][i_0][i_2+i_1][i_3+i_1] * A[i_0][i_2+1][i_3-1][i_1+1] - A[i_0][i_2-1][i_1-1][i_3+1] - A[i_0][i_2][i_1+1][i_3+3] - A[i_2+1][i_0][i_1-1][i_3] * A[i_0-2][i_2][i_1-1][i_3+1] - 4;
                }
            }
        }
        B[i_0] = B[i_0-1] + 6;
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4212351323_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F)));
