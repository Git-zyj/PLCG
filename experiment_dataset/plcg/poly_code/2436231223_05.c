...
void kernel_2436231223_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] + E[i_0-1] + E[i_0] - F[i_0] + F[i_0+1] - 3;
    }
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            D[i_2][i_1+2] = F[i_2] - G[i_2][i_2] - H[i_1][i_2][0] + D[i_2][i_1] - 3;
        }
        A[i_1] = E[i_1+1] + E[0] * 1;
    }
    for (int i_3 = 2; i_3 < PB_P+1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            D[i_4][i_3] = G[i_4][i_3-1] - F[0] - D[i_3+2][i_4] + D[i_4-1][i_3-2] + D[i_4+2][i_3] - 6;
        }
    }
#pragma endscop
...
kernel_2436231223_05(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
