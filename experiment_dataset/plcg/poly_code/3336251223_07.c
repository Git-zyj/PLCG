...
void kernel_3336251223_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] * E[i_0+1] + D[1] - E[i_0] * A[i_0-2] - 6;
    }
    for (int i_1 = 3; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_Q+1, i_1); i_2++) {
            for (int i_3 = 1; i_3 < min(PB_L-1, -i_1+PB_M+2); i_3++) {
                C[i_3+i_1-2][i_2][i_1-3] = F[i_2][i_3][1] * G[i_2+1][i_1][i_1+1] + G[i_1][i_3+1][i_2+i_1-1] - G[i_1][i_3-1][i_3] * 5;
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < min(min(-i_4+PB_M-2, -i_5+PB_M), i_5); i_6++) {
                C[i_6+i_4][i_5][i_4-1] = G[i_6][i_6][i_4] * G[i_5][i_4][i_5-1] * G[i_6][i_6][i_4] * H[i_4+1][i_6] + F[i_5][i_4+1][i_6-1] - F[i_6+1][i_4][i_5] + G[i_4][i_6-1][i_5] - C[i_6+i_5][i_4-1][i_5-1] + C[i_6+i_4-2][i_5][i_4-3] - C[i_6+i_4+2][i_5+1][i_4-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3336251223_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
