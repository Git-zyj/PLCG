...
void kernel_3336152315_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N-1; i_1++) {
            A[i_1] = D[i_0][i_1] * D[i_1][i_0+2] - D[i_1-2][i_0] * E[i_0] * D[i_1-1][i_0] - D[i_1][i_0] * D[i_1][i_0-2] - A[i_1-1] + A[i_1+1] * A[i_1-1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_N-2; i_3++) {
            C[i_2+2][i_3+1] = D[i_2+1][i_3-2] + D[i_3+2][i_2-1] + E[i_3] + 5;
        }
    }
    for (int i_4 = 2; i_4 < PB_N+2; i_4++) {
        for (int i_5 = 2; i_5 < PB_N; i_5++) {
            C[i_4+2][i_5] = D[i_4-2][i_5+2] + D[i_5][i_5-2] - D[i_5][i_4-2] - D[i_5][i_4-2] - A[i_5-1] - C[i_5+2][i_4+1] * C[i_4+2][i_5+1] + 2;
        }
    }
#pragma endscop
...
kernel_3336152315_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
