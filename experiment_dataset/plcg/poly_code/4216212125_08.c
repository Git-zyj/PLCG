...
void kernel_4216212125_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-1); i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                B[i_2][i_1+1] = C[i_2-2][i_1][i_0] - 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            B[i_4][i_3] = B[i_4+2][i_3+2] * 5;
        }
    }
#pragma endscop
...
kernel_4216212125_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
