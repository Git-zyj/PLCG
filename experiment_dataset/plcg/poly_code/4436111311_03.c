...
void kernel_4436111311_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
                    A[i_1][i_2][i_0-1] = A[i_1][i_3-1][i_2-1] + A[i_3][i_2][i_0] - A[i_2][i_3+1][i_1] * 2;
                }
            }
        }
        B[i_0] = 4;
        for (int i_4 = 0; i_4 < i_0; i_4++) {
            C[i_0][i_4] = E[i_4] - F[i_4][i_0] - B[i_4] + C[i_0+1][i_4] * C[i_0-1][i_4] * 3;
        }
    }
        ...
        for (int i_6 = max(2, i_5); i_6 < PB_L-1; i_6++) {
            for (int i_7 = 1; i_7 < min(i_6, PB_M-1); i_7++) {
                for (int i_8 = 2; i_8 < PB_M; i_8++) {
                    D[i_7][i_6][i_5][i_8-1] = D[i_7-1][i_6][i_5][i_8-2] * D[i_7-1][i_5][i_8][i_6-2] * D[i_8][i_6-1][i_5][i_7-1] + D[i_7+1][i_6+1][i_5-1][i_8-1] * D[i_7][i_6][i_5][i_8-2] + 3;
                }
            }
        }
    }
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4436111311_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F)));
