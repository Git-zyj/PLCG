...
void kernel_3236132123_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_P; i_2++) {
                A[i_0][i_1][i_2+i_1] = C[i_0+2][i_2-1][i_1] + C[i_0][i_2][i_1] * 6;
            }
        }
        for (int i_3 = max(3, i_0); i_3 < min(-i_0+PB_Q, PB_M); i_3++) {
            for (int i_4 = i_3; i_4 < -i_0+PB_M; i_4++) {
                B[i_3-2][i_0+1][i_0-1] = C[i_3][i_4][i_3+i_0] + C[i_4-2][i_0][i_0] * D[i_4+i_0][i_3] + D[i_3][i_4] * B[i_3-2][i_0+3][i_0+1] + B[i_4-2][i_3-1][i_3-3] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3236132123_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
