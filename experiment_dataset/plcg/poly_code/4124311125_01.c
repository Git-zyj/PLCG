...
void kernel_4124311125_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_0+PB_M, -i_2+PB_L), -i_1+PB_L); i_3++) {
                    A[i_3+i_1][i_1-1][i_0] = B[i_3+i_0][i_1][i_1] - A[i_3+i_2][i_2-1][i_1+1] - 2;
                }
            }
        }
    }
...
kernel_4124311125_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
