...
void kernel_4432312321_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = i_0; i_1 < -i_0+PB_N+1; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_N+2; i_3++) {
                    A[i_1+i_0-1][i_3][i_2][i_2] = E[i_3-1][i_1][i_0][i_2] * A[i_1+i_0-2][i_3-1][i_2+1][i_2+1] + A[i_3+i_1-2][i_0][i_2+1][i_2+1] + A[i_1+i_0-3][i_3][i_2-1][i_2-1] - 3;
                }
            }
        }
    }
    for (int i_4 = 4; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < -i_4+PB_L; i_5++) {
            for (int i_6 = 0; i_6 < min(PB_M-2, -i_4+PB_L+2); i_6++) {
                B[i_5][i_6+i_4-2] = B[i_6+2][i_5+i_4-4] - B[i_6][i_5+i_4] * 6;
            }
        }
    }
    for (int i_7 = 3; i_7 < PB_P-2; i_7++) {
        for (int i_8 = 0; i_8 < -i_7+PB_L-2; i_8++) {
            for (int i_9 = 0; i_9 < -i_7+PB_L; i_9++) {
                C[i_8][i_9+i_7] = F[i_9+i_8][i_8+i_7][i_7] + B[i_9][i_8+i_7-2] + B[i_9][i_8+i_7-1] - C[i_8+2][i_9+i_7-2] * C[i_9][i_8+i_7] - C[i_9][i_8+i_7+2] - 3;
            }
        }
        for (int j_0 = i_7; j_0 < PB_L; j_0++) {
            D[i_7-2][i_7] = G[i_7][j_0] * D[i_7-3][i_7-1] + D[i_7][i_7+2] - 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4432312321_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
