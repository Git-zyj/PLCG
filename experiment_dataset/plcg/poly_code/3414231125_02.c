...
void kernel_3414231125_02(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        D[i_0-2] = E[i_0] + F[i_0+1] - F[i_0] - D[i_0-1] * 2;
        for (int i_1 = 1; i_1 < min(PB_Q, -i_0+PB_P); i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                B[i_2-1][i_0][i_1] = G[i_0-1][i_2+1][i_1+1] - D[i_2] + 4;
            }
            D[i_1] = H[i_1-1][i_1+i_0] + H[i_0-1][0] - F[i_1+1] * 4;
            D[i_0-1] = H[i_1+i_0-1][i_0] + I[i_0][0] - I[i_1][i_0-1] + I[i_1][i_0] + 2;
        }
    }
#pragma endscop
...
kernel_3414231125_02(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
