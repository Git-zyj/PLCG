...
void kernel_2312212325_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = A[i_0+1] * A[i_0+4] * A[i_0+3] * 5;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            C[i_1+2][i_1] = C[i_1+2][i_1] + 3;
        }
        C[i_1+2][i_1] = D[i_1] + C[i_1+1][i_1-1] + 5;
    }
#pragma endscop
...
kernel_2312212325_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
