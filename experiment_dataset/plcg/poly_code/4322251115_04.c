...
void kernel_4322251115_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P, -i_0+PB_N); i_1++) {
            A[i_0] = D[1][i_1+i_0-1] * D[i_1][1] * E[i_1] - F[i_1+i_0][0] - D[i_0+1][i_1] - F[i_1][i_1] * 5;
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_M, -i_0+PB_N-1), PB_L-1); i_2++) {
                A[i_2] = G[i_2][i_1][i_0] - D[i_2+i_0+1][i_1] + H[i_0][i_2+1][i_1] + H[i_2+i_1][i_1][i_0] * A[i_2] * 2;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_P; i_3++) {
            for (int i_4 = i_0; i_4 < min(-i_0+PB_M, PB_P); i_4++) {
                A[i_4-1] = H[i_4+i_0][i_3+1][0] * G[i_4][i_3+1][i_3+i_0] + H[i_4][i_0-1][i_3] - G[i_3][i_4+1][i_0] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4322251115_04(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
