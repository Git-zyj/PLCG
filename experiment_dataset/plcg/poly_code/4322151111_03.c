...
void kernel_4322151111_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M+1, PB_L-1); i_1++) {
            A[i_1][i_0+1] = D[i_1+i_0-1][i_1+1] - D[i_1][i_0+1] - D[i_0+1][i_1] - E[i_0+1] + E[i_0] + D[i_1+1][i_0] - A[i_0-1][i_1+2] - A[i_1-1][i_0+1] - 5;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_L); i_2++) {
            B[i_2+i_0][0] = D[i_2][i_0] + D[i_0+1][i_2] - 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M, PB_L-1); i_4++) {
            C[i_4][i_3-1] = E[i_4] + D[i_3][i_4] - D[i_4][i_3-1] + D[i_4+i_3][i_4+1] * F[i_3][i_4][i_4] + D[i_4][i_3-1] * D[i_4][i_3] * C[i_4+1][i_3-1] + 4;
        }
    }
#pragma endscop
...
kernel_4322151111_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
