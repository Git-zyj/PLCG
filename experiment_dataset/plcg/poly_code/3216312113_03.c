...
void kernel_3216312113_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_N, i_3); i_4++) {
            for (int i_5 = max(1, i_3); i_5 < min(min(-i_3+PB_M, PB_P), -i_4+PB_N); i_5++) {
                B[i_4][i_5][i_3] = C[i_5+i_4][i_5+i_3][i_4] + B[i_4+1][i_5-1][i_3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3216312113_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
