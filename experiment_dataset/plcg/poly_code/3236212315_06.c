...
void kernel_3236212315_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0+2][i_0] = A[i_0+3][i_0+1] * A[i_0+1][i_0-1] - A[i_1+3][i_1+1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < i_2; i_4++) {
                A[i_2+1][i_2-1] = C[i_2+1][i_3+2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3236212315_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
