...
void kernel_4214212121_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
                    A[i_0][i_3+2][i_2][i_0-2] = 2;
                }
            }
        }
        B[i_0-1] = C[i_0-1] * C[i_0] - B[i_0] - 1;
    }
#pragma endscop
...
kernel_4214212121_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
