...
void kernel_2314351313_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        B[i_0] = D[i_0+1] + D[i_0-1] * E[i_0+1] - E[i_0] * 3;
        B[i_0] = F[i_0] - D[i_0] * D[i_0] - F[i_0-1] + G[i_0][i_0+1] + B[i_0+1] * B[i_0-1] + 6;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            B[i_2] = H[i_2-1][i_1] * I[i_2-1][i_1-1] - G[i_2-1][i_1] * D[i_2] * J[i_2][i_1][1] - B[i_2+1] - 5;
        }
    }
#pragma endscop
...
kernel_2314351313_04(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
