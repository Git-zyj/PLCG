...
void kernel_3416312325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_M-2; i_2++) {
            B[i_1][i_2+i_1] = B[i_1][i_2+i_1-1] - B[i_2-2][i_2+i_1-2] + 1;
            A[i_2] = B[i_2][i_2+i_1+2] * 6;
        }
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            D[2] = E[i_3+2][i_1] * D[2] - D[2] - D[2] - 2;
        }
    }
#pragma endscop
...
kernel_3416312325_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
