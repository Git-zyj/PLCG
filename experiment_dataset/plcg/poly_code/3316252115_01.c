...
void kernel_3316252115_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0] + D[i_0] * E[i_0-1] + F[i_0][i_0] - E[i_0] + D[i_0] + A[i_0-1] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        A[i_1] = D[i_1] + E[i_1+1] + D[i_1+1] + E[i_1] - 4;
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_1+PB_M, i_1); i_3++) {
                C[i_1+2][i_3-1][i_2+1] = G[i_2][i_1] - H[i_3+i_1][i_3][i_2-2] * H[i_1+2][i_3][i_2] * G[i_2-2][i_3] - I[i_2][i_3][i_1][i_2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3316252115_01(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
