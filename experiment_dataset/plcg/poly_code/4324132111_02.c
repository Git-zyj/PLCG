...
void kernel_4324132111_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < min(i_0, -i_2+PB_L-1); i_3++) {
                    A[i_0][i_1][i_2-1][i_0] = D[i_1+2][i_3-1][i_2-1][i_3+1] * D[i_1][i_0][i_3][i_2+1] + D[i_3+2][i_1][i_3][i_0] + D[i_2][i_3+i_2+1][i_1-2][i_0+1] * 3;
                }
            }
        }
    }
        ...
        for (int i_5 = max(i_4, 1); i_5 < -i_4+PB_M-1; i_5++) {
            for (int i_6 = 2; i_6 < -i_4+PB_M-2; i_6++) {
                for (int i_7 = 0; i_7 < PB_L; i_7++) {
                    B[i_5+i_4+1][i_4+1][i_6][i_7] = E[i_6][i_4+2][i_5-1][i_7][i_5] + D[i_6-2][i_4][i_7][i_5] + D[i_5][i_7][i_6-1][i_4+1] * B[i_6+i_4+2][i_4+2][i_7][i_5] + 4;
                }
            }
        }
    }
        ...
        for (int i_9 = 0; i_9 < PB_M; i_9++) {
            for (int j_0 = 1; j_0 < PB_P-1; j_0++) {
                for (int j_1 = max(i_9, 1); j_1 < PB_L; j_1++) {
                    A[i_9][j_0+1][j_1-1][i_9] = F[i_9][j_1][j_0-1] * A[j_0][i_9][j_1-1][j_0] - 4;
                }
            }
        }
    }
...
kernel_4324132111_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
