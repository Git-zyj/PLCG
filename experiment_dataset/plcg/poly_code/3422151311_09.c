...
void kernel_3422151311_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0] - E[i_0] * E[i_0] - E[i_0] - E[i_0] - A[i_0-1] - A[i_0+1] - 1;
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < PB_Q-2; i_2++) {
                B[i_0][i_1][i_2+1] = F[i_0][i_2][i_0-1] * G[i_2-1][i_1+i_0] * G[i_2-1][i_1] * F[i_2][0][i_1-1] * B[i_0+1][i_1][i_2+2] * B[i_0][i_1-1][i_2+1] - B[i_0][i_1+1][i_2+2] - B[i_0][i_1][i_2] * 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
            C[i_3][i_4+i_3-1] = G[i_4+1][i_4+i_3] + G[i_4+1][0] * G[i_3][i_4] + G[i_4][i_3] - A[i_4+1] - 5;
        }
        for (int i_5 = 1; i_5 < i_3; i_5++) {
            for (int i_6 = 1; i_6 < min(-i_5+PB_M, PB_L-1); i_6++) {
                D[i_3][i_5][i_6+1][i_5] = H[i_6][i_5][i_5][i_3+1] + F[i_6][i_3][i_5] - F[i_3][i_6][i_6-1] * G[i_5][0] * F[i_3][i_6+i_5][i_3] * F[i_5][i_3][i_3] + D[i_3-1][i_5][i_6+1][i_5] + D[i_3][i_5-1][i_6][i_5-1] + D[i_5+1][i_3][i_6+1][i_3] - D[i_5-1][i_6+1][i_3+1][i_6+1] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3422151311_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
