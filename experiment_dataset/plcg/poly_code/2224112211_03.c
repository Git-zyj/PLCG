...
void kernel_2224112211_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
        B[i_1] = C[i_1-2] + D[i_1-1][i_1] * B[i_1+1] + B[i_1-1] * B[i_1+1] + 1;
    }
#pragma endscop
...
kernel_2224112211_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
