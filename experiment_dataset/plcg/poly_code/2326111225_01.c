...
void kernel_2326111225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-1; i_1++) {
            A[i_1+1][i_1-1] = D[i_0] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            C[i_2-2][i_2-2] = C[i_2][i_2] - C[i_2+1][i_2+1] - C[i_2+2][i_2+2] - 4;
        }
        C[i_2][i_2] = D[1] - A[i_2+1][i_2-1] * 5;
    }
#pragma endscop
...
kernel_2326111225_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
