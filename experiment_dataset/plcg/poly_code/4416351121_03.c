...
void kernel_4416351121_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        A[i_0][i_0-1] = E[i_0] * F[i_0] * E[i_0-1] * F[i_0-1] * F[i_0+1] - A[i_0-2][i_0-3] * 3;
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_Q-1, PB_M-1), -i_1+PB_L+1); i_2++) {
                B[i_1+1][i_2+1] = G[i_1+i_0][i_2+i_1-1][0] - H[i_1-1][0][i_2+i_0+1] - G[i_2+1][i_0][i_1] - I[i_1-1][i_0][i_2] * H[i_2][i_1][i_0] - 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        C[i_3][i_3] = J[i_3] + E[i_3+1] - F[i_3] - J[i_3] - F[i_3-1] + 4;
        for (int i_4 = 0; i_4 < min(-i_3+PB_N, PB_L-1); i_4++) {
            D[i_3][i_3] = J[i_4+i_3] * K[i_3][i_4+1] + K[i_4][i_3] - L[i_4][i_3+1] + D[i_3-2][i_3-2] + 4;
        }
    }
#pragma endscop
