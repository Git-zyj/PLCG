...
void kernel_2432352123_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0] + F[i_0+2] * G[i_0][i_0] + F[i_0+2] * E[i_0] * H[i_0+1] - A[i_0+2] * A[i_0-1] - 1;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_N; i_2++) {
            A[i_2] = I[i_2][i_1] + G[i_1-1][2] + 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        C[i_3+2] = E[i_3] + E[i_3+1] - H[0] - F[i_3+1] * H[i_3] - F[i_3] - 5;
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 2; i_5 < min(min(i_4, PB_N-2), -i_4+PB_L-1); i_5++) {
            A[i_5-1] = I[i_5][i_4+2] * F[i_4] * F[i_5] * E[i_5-2] + G[i_4][i_5+i_4+1] - I[i_5+2][i_4] * 3;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2432352123_06(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
