...
void kernel_3422212215_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N-2; i_0++) {
        A[i_0-2] = E[i_0+2] + A[i_0-1] - A[i_0-3] * 6;
    }
    ...
    for (int i_3 = 1; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 2; i_4 < i_3; i_4++) {
            D[i_4+1] = D[i_4+2] * D[i_4+1] + 5;
            D[i_4+1] = F[i_3+1][i_4+2] * G[i_3-1][i_4][i_3+2] - A[i_4-2] + D[i_4+2] + 1;
        }
    }
#pragma endscop
...
kernel_3422212215_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
