...
void kernel_4312151321_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
                A[i_1][i_0][i_2] = D[i_2][i_0][i_1-1] - D[i_2+1][i_0][i_1] - A[i_1+1][i_0][i_2] - A[i_1-1][i_0][i_2-2] + 2;
            }
        }
    }
        ...
        for (int i_4 = 3; i_4 < -i_3+PB_L; i_4++) {
            for (int i_5 = 0; i_5 < -i_3+PB_Q; i_5++) {
                for (int i_6 = 1; i_6 < PB_Q-1; i_6++) {
                    B[i_4-1][i_6][i_5][i_5] = D[i_6][i_4+1][i_5] + E[i_6+1][i_3][i_5][i_4+1] + E[i_3][i_5][i_6][i_5] * E[i_4][i_5+1][i_6][i_5+i_3] * E[i_4+1][i_5][i_6][i_3] - E[i_5][i_4+1][i_6-1][i_5+i_3] * E[i_4-1][i_5][i_4+i_3][i_6+1] - D[i_5+1][i_4][i_6] * E[i_6-1][i_4+i_3][i_6-1][i_3] * B[i_4-2][i_6][i_5][i_5] - B[i_4-1][i_5+1][i_6][i_6] - B[i_4-3][i_6][i_5][i_5] + 5;
                }
            }
        }
        for (int i_7 = 1; i_7 < PB_L; i_7++) {
            C[i_7][i_3-1] = F[i_3][i_7] - F[i_7+1][i_3+1] + C[i_3+1][i_7-1] - C[i_7-1][i_3-3] * C[i_7-1][i_3-1] * C[i_7][i_3+1] - 6;
        }
    }
#pragma endscop
...
kernel_4312151321_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
