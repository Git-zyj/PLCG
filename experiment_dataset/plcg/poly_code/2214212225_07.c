...
void kernel_2214212225_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 4; i_1 < PB_L+2; i_1++) {
            A[i_0][i_1-2] = A[i_0+2][i_1-3] + A[i_0][i_1-4] + 4;
        }
        for (int i_2 = i_0; i_2 < PB_M-2; i_2++) {
            A[i_2+2][i_0-1] = C[i_2+1] * 2;
        }
    }
#pragma endscop
...
kernel_2214212225_07(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
