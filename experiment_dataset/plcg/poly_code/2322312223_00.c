...
void kernel_2322312223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-4; i_0++) {
        A[i_0][i_0+2] = 1;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0][i_0+2] = D[i_0] + D[i_0-1] - A[i_0][i_0+2] * A[i_0+2][i_0+4] - A[i_0-1][i_0+1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
            C[i_2-1][i_3] = C[i_2-1][i_3+1] - 1;
        }
    }
#pragma endscop
...
kernel_2322312223_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
