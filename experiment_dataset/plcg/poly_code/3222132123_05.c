...
void kernel_3222132123_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
    ...
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
            for (int i_3 = 0; i_3 < min(PB_M, -i_0+PB_P-1); i_3++) {
                A[i_3][i_2+2] = D[i_0][i_2][i_3] - D[2][i_2][i_0] * E[i_2+2][i_3][i_0][i_0] + D[i_2-2][i_3+i_0+1][i_2] - E[i_0][i_2][i_3][0] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3222132123_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
