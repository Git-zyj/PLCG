...
void kernel_4222111325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 2; i_3 < PB_L-3; i_3++) {
                for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
                    B[i_2][i_4][i_3+1] = B[i_3-2][i_4+2][i_2+1] - B[i_2][i_4+2][i_3+3] + B[i_2][i_4+2][i_3] + 1;
                }
            }
        }
    }
...
kernel_4222111325_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
