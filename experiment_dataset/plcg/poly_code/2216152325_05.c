...
void kernel_2216152325_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        B[i_0][i_0-2] = C[i_0] * C[i_0-2] * C[i_0+1] - C[i_0-1] + C[i_0-1] - C[i_0] + B[i_0][i_0-2] - 5;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 4; i_2 < min(PB_L, i_1); i_2++) {
            B[i_2][i_2-2] = D[i_2+2][i_1] + D[i_1][i_2] - D[i_2][i_1] - D[i_2][i_1] - B[i_2-2][i_2-4] * B[i_2+2][i_2] + 4;
        }
    }
#pragma endscop
...
kernel_2216152325_05(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
