...
void kernel_4232151211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-1, PB_P-1); i_1++) {
            A[i_1+1][i_1] = C[i_0-1][0] - D[i_1] * C[i_1][i_0] * D[i_1] - D[i_0] * C[i_0][i_1] + C[i_1+i_0+1][i_0] + C[i_1][i_0] + A[i_0+2][i_0+1] - A[i_1+2][i_1+1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_2] = C[i_3][i_2] * D[i_3+1] - B[i_2+1] + 3;
        }
    }
#pragma endscop
...
kernel_4232151211_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
