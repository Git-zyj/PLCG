...
void kernel_2222112315_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        A[0] = A[0] * 2;
        for (int i_1 = min(i_0, -i_0+PB_M-3) - 1; i_1 >= 0; i_1--) {
            B[i_1+i_0+2][0] = C[i_1][i_0-2] - A[0] + A[0] + B[i_1+i_0+2][0] * B[i_1+i_0+3][0] * B[i_1+i_0+3][0] * 3;
        }
    }
#pragma endscop
...
kernel_2222112315_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
