...
void kernel_3126352125_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_Q; i_2++) {
                A[i_1][i_0-1][i_2+2] = B[i_1][i_2][i_0] - C[i_0][i_2+i_1][i_0] + D[i_2][i_1] - E[i_0+1][i_2][i_1-2] + C[i_1][i_2][i_0-1] * A[i_2+2][i_0-1][i_1+2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3126352125_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
