...
void kernel_4422131113_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
            for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
                for (int i_5 = 1; i_5 < min(min(-i_2+PB_M, -i_4+PB_N-1), i_3); i_5++) {
                    B[i_5+i_2][i_3-1][i_4-1][i_2] = E[i_3+1][i_5+1][i_3][0] + E[i_5+i_4+1][i_3+i_2][i_2+1][i_4] * F[i_5-1][i_4][i_2] * 6;
                }
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M; i_6++) {
        for (int i_7 = 1; i_7 < min(PB_M, -i_6+PB_L); i_7++) {
            D[i_7-1] = G[i_6][i_7] + G[i_7][i_6] * G[i_7+i_6][i_7+1] - G[i_6][i_7] - D[i_7] - 2;
        }
        D[i_6] = H[i_6] + H[i_6] + H[i_6] + 6;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422131113_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
