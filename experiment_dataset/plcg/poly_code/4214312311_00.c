...
void kernel_4214312311_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < i_0; i_2++) {
                A[i_0+1][i_1][i_2] = C[i_0][i_1][i_2-2] - A[i_1][i_0][i_2] + A[i_0+1][i_1][i_2-1] - A[i_0+1][i_1+1][i_2] - 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_P; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-3; i_5++) {
                B[i_5+2][i_4+1][i_3+1][i_4] = D[i_5][i_3+1] * A[i_3+1][i_5][i_4] * B[i_5+3][i_4+2][i_3+2][i_4+1] - B[i_5+1][i_4][i_3+1][i_4-1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4214312311_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
