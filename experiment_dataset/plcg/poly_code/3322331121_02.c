...
void kernel_3322331121_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0-1] - E[0] - E[i_0-1] - E[i_0] - F[i_0] - F[i_0] * A[i_0-2] - 3;
    }
    for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
        for (int i_2 = 0; i_2 < min(min(i_1, PB_L-2), -i_1+PB_L+1); i_2++) {
            B[i_1][i_2] = G[i_1][i_2+i_1-1] - B[i_1+1][i_2+2] + 4;
        }
        for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
            B[i_1][i_3-2] = H[i_3-1][i_1] * 6;
        }
    }
#pragma endscop
...
kernel_3322331121_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
