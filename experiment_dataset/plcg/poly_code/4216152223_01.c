...
void kernel_4216152223_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            B[i_0-1][i_1-2] = C[i_1] * C[i_0-2] * C[i_1] - D[i_1+i_0][0] * D[i_1-2][i_0] * D[i_1][i_0+1] - B[i_0-1][i_1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            B[i_2-1][i_3] = D[i_2+2][i_3+1] + C[i_3-2] * D[i_3][i_2] - D[0][i_3] + B[i_2-1][i_3-1] + 2;
        }
    }
#pragma endscop
...
kernel_4216152223_01(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
