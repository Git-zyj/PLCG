...
void kernel_3236111311_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                A[i_2][i_1-1][i_0] = A[i_2][i_1-2][i_0] + A[i_2][i_1][i_0] * A[i_2+1][i_0-1][i_1+1] + 3;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L; i_3++) {
            for (int i_4 = 2; i_4 < i_3; i_4++) {
                A[i_0+1][i_4-2][i_3] = C[i_4][i_0-1][i_0] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3236111311_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
