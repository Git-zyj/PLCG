...
void kernel_4434332325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] - A[i_0-2] - A[i_0+2] - A[i_0+1] - A[i_0-1] * A[i_0+2] * 4;
        for (int i_1 = i_0; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_M; i_3++) {
                    C[i_3][i_1+1][i_2] = F[i_1][i_3+1][i_0+2][i_2][2] + G[i_2][i_3-1][i_1] - H[i_3+i_1][i_0][i_2][i_0+2] + H[i_2][i_3][i_0][i_1+1] * G[i_3][i_1][i_2] + I[i_0][i_2-1][i_3][i_1+2] * H[i_1][i_0][i_3+i_1+1][i_2] + 2;
                }
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < -i_4+PB_P; i_5++) {
            for (int i_6 = max(i_4, 2); i_6 < PB_M; i_6++) {
                for (int i_7 = 0; i_7 < PB_Q; i_7++) {
                    C[i_6][i_5][i_7] = H[i_4-1][i_6][i_5][i_5-2] + H[i_6][i_4][i_5][i_7] * H[i_6-2][i_7][i_5+i_4][i_6] * 2;
                }
            }
        }
        D[i_4+1] = E[i_4] + A[i_4] * D[i_4+3] * D[i_4-1] + 3;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4434332325_03(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
