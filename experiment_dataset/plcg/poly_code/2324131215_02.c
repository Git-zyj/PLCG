...
void kernel_2324131215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0-1] - D[i_0] - D[i_0] - D[i_0] + E[i_0][i_0] - A[i_0+1] * A[i_0-1] * 4;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < PB_N; i_2++) {
            B[i_1][i_2] = E[i_2][i_1-1] + B[i_1-1][i_2] + 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_N-1, i_3); i_4++) {
            C[i_4] = E[i_3-1][i_4] - B[i_3][i_4+1] - C[i_4-1] * C[i_4+1] + 2;
        }
    }
#pragma endscop
...
kernel_2324131215_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
