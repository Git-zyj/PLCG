...
void kernel_3212312321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            A[i_1+2] = A[i_1+3] * A[i_1+1] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_P, -i_2+PB_M); i_3++) {
            B[i_3][2] = C[i_3+i_2][i_2+2][i_3] * B[i_3-2][2] * B[i_3+1][2] - B[i_3+2][2] + B[i_3-1][2] - 4;
        }
    }
#pragma endscop
...
kernel_3212312321_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
