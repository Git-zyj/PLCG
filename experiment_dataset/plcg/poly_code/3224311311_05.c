...
void kernel_3224311311_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_Q-2, -i_0+PB_Q-3); i_2++) {
                A[i_0-1][i_1][i_2+i_0+1] = C[i_2+i_0][i_1][i_0] - A[i_0][i_1][i_2+i_0+3] * A[i_1-1][i_0][i_2+i_1+2] * A[i_0][i_1][i_2+i_0+2] + A[i_1][i_0+1][i_2+i_1+2] - 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            B[i_4][i_3-1] = D[i_3] - B[i_4][i_3-2] * B[i_4+1][i_3-1] - 2;
        }
    }
#pragma endscop
...
kernel_3224311311_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
