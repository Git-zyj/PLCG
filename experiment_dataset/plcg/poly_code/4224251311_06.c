...
void kernel_4224251311_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_P-1, i_1); i_2++) {
                for (int i_3 = 1; i_3 < min(min(-i_2+PB_M, PB_P-1), -i_1+PB_L+1); i_3++) {
                    A[i_0-1][i_2][i_0][i_3] = C[i_3+i_2][i_3+i_1-1][i_0][0] * D[i_3-1][i_0+1][i_2+1][i_1] + E[i_3+i_2][i_3][i_0][i_1-1][i_1] * C[i_1][i_2][i_3+1][i_0] * A[i_0-2][i_2-1][i_0-1][i_3] * 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        B[i_4-1] = F[i_4+1] - G[i_4] + F[i_4] - F[i_4-1] + F[i_4] - G[i_4] * B[i_4] - B[i_4-2] * B[i_4-2] + B[i_4-2] - 1;
    }
#pragma endscop
...
kernel_4224251311_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
