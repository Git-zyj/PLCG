...
void kernel_3316352315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        A[i_0] = D[0] * E[i_0] - F[i_0-2] - D[i_0] * A[i_0-1] * 4;
        for (int i_1 = 0; i_1 < min(PB_N, -i_0+PB_M-1); i_1++) {
            B[i_0+2][i_1] = E[0] - G[i_1][i_0][i_0] - H[i_1][i_0] * E[i_1+2] + I[i_1+1][i_1] - I[i_1+i_0+1][i_0-2] * A[i_1] + B[i_0+3][i_1+1] + B[i_1+2][i_0-1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_2+2][i_3] = J[i_2][i_3-1] + I[i_2+1][i_2] - J[i_2][i_3] + B[i_2+1][i_3] + 4;
        }
    }
#pragma endscop
...
kernel_3316352315_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
