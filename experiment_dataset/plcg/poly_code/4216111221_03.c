...
void kernel_4216111221_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        A[i_0+1] = A[i_0+3] * A[i_0+2] * A[i_0-1] * 1;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L-2; i_2++) {
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                B[i_1][i_2+1] = C[i_3][0] * B[i_3-1][i_2+2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4216111221_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
