...
void kernel_3332111111_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
            B[i_2][i_3] = D[i_2][i_3+1] - B[i_2+1][i_3] - B[i_2][i_3+1] + B[i_2-1][i_3] - 5;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        C[i_4][i_4-1] = E[i_4-1] + 6;
    }
#pragma endscop
...
kernel_3332111111_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
