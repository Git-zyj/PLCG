...
void kernel_2224112325_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M-4, i_0); i_1++) {
            A[i_1+2][i_0] = C[i_1][i_0] - C[i_1][i_1] - A[i_1+4][i_0+2] * A[i_1+2][i_0-1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            A[i_2+2][i_3] = A[i_2+2][i_3] - 5;
        }
    }
#pragma endscop
...
kernel_2224112325_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
