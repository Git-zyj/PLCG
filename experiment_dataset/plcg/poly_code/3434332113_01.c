...
void kernel_3434332113_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        A[i_0] = E[i_0+1] + F[i_0] - E[i_0+2] + G[i_0] + D[i_0+2] - 3;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_1][i_0] = H[i_1][i_0] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        D[i_2+1] = H[i_2-1][i_2+1] + E[i_2] + F[i_2-2] - F[i_2] + D[i_2+2] * 1;
        D[i_2+2] = I[i_2-2][i_2] * G[i_2+2] * G[0] * 6;
    }
#pragma endscop
...
kernel_3434332113_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
