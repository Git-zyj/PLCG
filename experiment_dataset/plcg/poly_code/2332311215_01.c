...
void kernel_2332311215_01(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        C[i_0+2] = D[i_0-1] + E[i_0] + 4;
        C[i_0] = E[i_0-1] + 2;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        C[i_1+1] = C[i_1] + C[i_1+2] + 2;
    }
#pragma endscop
...
kernel_2332311215_01(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
