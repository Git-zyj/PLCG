...
void kernel_4336351215_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = D[0] + D[i_0] * E[i_0] - D[i_0] * E[i_0-1] + A[i_0] - A[i_0+2] - A[i_0] - 1;
    }
        ...
        for (int i_2 = max(1, i_1); i_2 < PB_L; i_2++) {
            for (int i_3 = 1; i_3 < min(PB_N-1, i_2); i_3++) {
                for (int i_4 = i_1; i_4 < PB_L; i_4++) {
                    C[i_3+1][i_2-1][i_2] = F[i_4][i_1][i_2][i_3] - G[i_1][i_4][i_2][i_4] * G[i_3-1][i_1][i_2+1][0] - F[i_3][i_1+1][i_2][0] * 4;
                }
            }
        }
    }
        ...
        for (int i_6 = 0; i_6 < min(i_5, -i_5+PB_P+1); i_6++) {
            for (int i_7 = 0; i_7 < PB_N-1; i_7++) {
                for (int i_8 = 1; i_8 < PB_Q-1; i_8++) {
                    C[i_7+1][i_8-1][i_8] = F[i_8+i_6][i_7][i_5-1][1] - F[i_5+1][i_7][i_6][i_8+1] + H[i_7][i_8][i_5][i_6+i_5-1] * G[i_8][i_5][i_6][i_7] - C[i_7][i_8-1][i_8] - 1;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4336351215_09(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
