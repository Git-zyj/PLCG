...
void kernel_4124232113_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[i_2-2][i_1][i_0] = B[i_1-2][i_2][i_0] * C[i_1][i_2][i_0] * D[i_1][i_2] - A[i_2-2][i_1-1][i_0] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4124232113_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
