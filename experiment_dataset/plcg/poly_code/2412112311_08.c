...
void kernel_2412112311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 3; i_1 < PB_M; i_1++) {
            A[i_1-2] = E[i_1][i_0] - A[i_1-3] + A[i_1-1] * A[i_1-1] + 5;
        }
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            B[i_0-2][i_2+2] = B[i_0-3][i_2+2] - 1;
        }
    }
    for (int i_3 = 3; i_3 < PB_N-1; i_3++) {
        C[i_3-2] = C[i_3-3] * C[i_3-1] + 2;
        for (int i_4 = 2; i_4 < PB_N-1; i_4++) {
            D[i_3][i_4] = A[i_4-2] - A[i_4-1] - D[i_4-1][i_3+1] * D[i_3+1][i_4+1] - 4;
        }
    }
#pragma endscop
...
kernel_2412112311_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
