...
void kernel_3226212111_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M+1; i_3++) {
            for (int i_4 = i_2; i_4 < PB_M; i_4++) {
                B[i_4][i_3-2] = C[i_3-1][i_4][i_3-1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3226212111_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
