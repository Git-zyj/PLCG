...
void kernel_3136112323_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_M-4, -i_0+PB_L); i_1++) {
            for (int i_2 = 1; i_2 < PB_M-4; i_2++) {
                A[i_2+2][i_2] = B[i_2-1][i_1+i_0][i_0] * A[i_1+4][i_1+2] + A[i_2+4][i_2+2] - A[i_2+3][i_2+1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3136112323_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
