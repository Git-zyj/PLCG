...
void kernel_3436151323_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        B[i_0+2] = E[i_0] * E[i_0] - E[i_0] + E[i_0] * E[i_0] + E[i_0] - E[i_0] * E[i_0-1] * E[1] * 4;
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            B[i_1] = E[i_0] - F[i_0][i_1+1] * E[i_0-1] * G[i_0][i_1][1] - F[0][i_0] + C[i_0-2][i_1] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_L+1; i_3++) {
            C[i_2][i_3] = E[i_3-1] * E[i_3-1] + F[i_3][i_2] * F[i_3+1][i_2] * F[i_3][i_2] - C[i_2][i_3-2] - C[i_2][i_3+1] - C[i_2][i_3+2] + 2;
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 3; i_5 < PB_M-1; i_5++) {
            D[i_5-1] = B[i_5+1] + D[i_5] * D[i_5-3] * D[i_5-2] - D[i_5+1] + 1;
        }
    }
#pragma endscop
...
kernel_3436151323_06(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
