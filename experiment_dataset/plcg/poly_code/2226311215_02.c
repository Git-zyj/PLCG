...
void kernel_2226311215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = PB_M - 1; i_1 >= 0; i_1--) {
        for (int i_2 = max(1, i_1); i_2 < PB_M-1; i_2++) {
            A[i_2-1] = C[i_1] - A[i_2+1] + A[i_2-1] - 3;
        }
    }
#pragma endscop
...
kernel_2226311215_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
