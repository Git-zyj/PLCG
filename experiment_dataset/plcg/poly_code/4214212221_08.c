...
void kernel_4214212221_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_Q-4, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-4; i_2++) {
                for (int i_3 = max(1, i_1); i_3 < PB_P; i_3++) {
                    A[i_1][i_3][i_1+2] = A[i_2][i_1-1][i_2+2] * A[i_1+2][i_3][i_1+4] - A[i_2+2][i_3-1][i_2+4] * 2;
                }
            }
        }
    }
        ...
        for (int i_5 = max(i_4, 1); i_5 < PB_M; i_5++) {
            for (int i_6 = max(i_4, 2); i_6 < PB_L-2; i_6++) {
                for (int i_7 = 1; i_7 < PB_P; i_7++) {
                    B[i_5][i_6][i_7] = C[i_7-1][i_6+2][i_5] - B[i_5-1][i_7][i_6-2] - 2;
                }
            }
        }
    }
...
kernel_4214212221_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
