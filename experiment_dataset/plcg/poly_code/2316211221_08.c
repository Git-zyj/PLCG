...
void kernel_2316211221_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_M-1; i_1++) {
        ...
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            C[i_1-1][i_2] = B[i_2] - B[i_2+1] * C[i_1-1][i_2+1] - C[i_1-3][i_2] - C[i_1-1][i_2-1] + 2;
        }
    }
#pragma endscop
...
kernel_2316211221_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
