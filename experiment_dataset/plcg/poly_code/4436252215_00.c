...
void kernel_4436252215_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + F[i_0] + F[i_0+2] - E[i_0] + F[i_0-2] + F[i_0] * F[i_0] * G[i_0][i_0+1] + F[i_0] - A[i_0-1] - A[i_0+1] - 2;
    }
    for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < -i_1+PB_L; i_2++) {
            for (int i_3 = 0; i_3 < PB_M; i_3++) {
                for (int i_4 = 0; i_4 < min(min(-i_3+PB_P, -i_2+PB_L), -i_1+PB_P-1); i_4++) {
                    B[i_1][i_4+i_2][i_4+i_3][i_2-2] = H[i_3][i_1][i_4+2][i_2] - I[i_3][i_2][i_4+1][i_3] + B[i_3][i_2+i_1][i_4+i_1+1][i_2-2] - 3;
                }
            }
        }
        for (int i_5 = 2; i_5 < PB_L; i_5++) {
            C[i_1+1][i_5] = G[i_5-2][i_1] * J[i_1][i_1] - J[i_5][i_1-1] * C[i_1][i_5] + 1;
        }
    }
    for (int i_6 = 2; i_6 < PB_Q-2; i_6++) {
        for (int i_7 = i_6; i_7 < PB_M-1; i_7++) {
            for (int i_8 = 1; i_8 < -i_7+PB_M-1; i_8++) {
                D[i_8][i_7][i_6] = K[i_8+i_7+1][i_6-1][i_7] * L[i_8][i_7-2][i_6+2] + K[i_8][i_7][i_6-2] * I[i_8][i_7][i_6][i_6+1] * K[i_7][i_6][i_8] - D[i_8][i_7-1][i_6+1] - D[i_8-1][i_7][i_6] * 2;
            }
        }
    }
#pragma endscop
