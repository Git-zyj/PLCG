...
void kernel_3314151215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[0] = D[i_0] + D[i_0-1] * D[i_0] - D[i_0] * D[i_0] - B[i_0] + 4;
        B[i_0] = D[i_0] - D[i_0] * D[i_0+1] * A[0] + A[0] - B[i_0+1] - 4;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        A[0] = D[i_1] * D[i_1-1] * D[i_1] - D[i_1] * D[i_1-1] + 4;
    }
#pragma endscop
...
kernel_3314151215_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
