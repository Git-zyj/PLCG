...
void kernel_2422351313_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_N-1, PB_L-1); i_1++) {
            A[i_0-1][i_1+1] = E[0] - F[i_1][i_0+1] + F[i_1+1][i_0] + G[i_1+i_0+1][1] - H[i_1][i_0-1] + A[i_0][i_1] + A[i_1-2][i_0+2] + A[i_0-1][i_1] + A[i_0][i_1+1] - A[i_0][i_1+2] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_L+1, -i_2+PB_M-1); i_3++) {
            A[i_2-1][i_3+1] = H[i_3+i_2+1][i_2-1] + I[i_3-1][i_2][i_3] * F[i_3-1][i_2-1] * G[i_3+1][i_2-1] * F[i_2][i_3] * H[i_3+1][i_2] - G[i_3-1][i_2] * E[i_2] + H[i_3][i_2] + 6;
        }
        C[0] = J[i_2+1] - K[i_2+1] - J[i_2-1] - J[i_2] - C[0] - 3;
    }
...
kernel_2422351313_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
