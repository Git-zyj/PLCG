...
void kernel_4322312123_05(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
        ...
        for (int i_1 = 0; i_1 < -i_0+PB_L-3; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < PB_M; i_3++) {
                    B[i_1+i_0+3][i_3-1][i_2][i_0+2] = D[i_3][i_2][0][i_2] + 3;
                }
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < PB_L; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 0; i_7 < min(-i_5+PB_L, -i_6+PB_L-1); i_7++) {
                    B[i_7+i_6+1][i_5-1][i_4][i_7] = D[i_5-2][i_6+1][i_5-2][i_7] + E[i_7+i_5][i_6+2][i_4][i_5] * 6;
                }
            }
        }
        for (int i_8 = 0; i_8 < PB_M; i_8++) {
            C[i_8][i_4] = C[i_4+2][i_8] * 5;
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), INIT_SEED);
kernel_4322312123_05(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E)));
