...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4424351123_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 3; i_1 < -i_0+PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < min(min(min(-i_2+PB_L, i_0), PB_M-1), -i_1+PB_Q); i_3++) {
                    A[i_2][i_1-1][i_2-1][i_3] = E[i_3+1][i_2][i_1+i_0+1] * F[i_2+1][i_3][i_1-1][i_0-1] - G[i_1-1][i_3+i_2][i_1] + H[i_0+1][i_3+i_1][i_2+1][i_2] - A[i_2][i_1-3][i_2-1][i_3] - 2;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            B[i_5-1] = I[i_5-1][i_4][1] - J[i_5-1][i_4] - I[i_5][i_4+1][i_5] + J[i_5][i_4] * K[i_5][i_4] - B[i_5+1] * 4;
        }
        for (int i_6 = max(i_4, 1); i_6 < min(-i_4+PB_M, PB_M-1); i_6++) {
            B[i_6-1] = K[i_6+1][i_4] * L[i_6+1] + E[i_6+i_4][i_6+i_4][1] + L[i_6] - M[i_6][i_4] + J[i_4][i_6] - K[i_6-1][i_6+i_4] * 4;
        }
    }
    for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
        for (int i_8 = 2; i_8 < PB_L-1; i_8++) {
            for (int i_9 = 0; i_9 < min(-i_7+PB_L, -i_8+PB_L-1); i_9++) {
                D[i_9+i_7-1][i_8+1][i_8+1][0] = I[i_9][i_7][i_8] - E[i_8][i_9+i_7][1] + G[i_9][i_9+i_8+1][i_7-1] + D[i_9+i_8-2][i_7+1][i_7+1][0] * 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4424351123_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M)));
