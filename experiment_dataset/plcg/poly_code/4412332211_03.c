...
void kernel_4412332211_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < PB_L; i_1++) {
            A[i_1+2][i_1-2] = E[i_1] * F[i_1+1][2] + F[i_1-2][i_0-2] - G[i_1][i_0] * A[i_1+1][i_1-3] + A[i_1+3][i_1-1] + 2;
        }
        for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                A[i_2+2][i_2-2] = H[i_2-2][i_3][i_0] - G[i_3-1][i_2+1] + H[i_0][0][i_3] - F[i_3][i_2-1] * 5;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_P; i_5++) {
            for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
                A[i_5+3][i_5-1] = I[i_4][i_6+1][i_5] - 2;
            }
        }
        D[i_4][i_4] = D[i_4+1][i_4+1] * D[i_4-1][i_4-1] - 6;
    }
#pragma endscop
...
kernel_4412332211_03(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
