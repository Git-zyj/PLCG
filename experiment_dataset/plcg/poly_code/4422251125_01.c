...
void kernel_4422251125_01(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_N-1, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                D[i_0+1][i_2+1][i_2+2][i_1-1] = E[i_1+i_0-1][i_2] + F[i_1+1][i_0][i_1+1] - F[i_1][i_2+1][i_0-1] * F[i_1][i_2-1][i_0] + G[i_2][i_1-1] * 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        B[i_3-1] = H[i_3] + H[i_3-1] - H[i_3-1] + I[i_3] + I[i_3] + I[0] + G[i_3+1][1] + 6;
        for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
            B[i_4+1] = E[i_3][i_4] - H[i_4-1] + E[i_4+i_3][i_4] - E[i_4+1][i_3-1] + G[i_4][i_3] * 6;
            for (int i_5 = i_4; i_5 < -i_3+PB_N-1; i_5++) {
                for (int i_6 = 0; i_6 < -i_4+PB_L; i_6++) {
                    D[i_3+1][i_6][i_6+1][i_5] = J[i_5+i_3+1][i_4][i_6][i_3] * J[i_5-1][i_6][i_4][i_3] + K[i_5+1][i_6+i_4][i_5][i_3-1] + D[i_3+1][i_6+1][i_6+2][i_4-1] * D[i_6+1][i_3+2][i_3+3][i_5] + 1;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4422251125_01(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K)));
