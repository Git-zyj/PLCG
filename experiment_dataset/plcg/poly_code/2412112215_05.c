...
void kernel_2412112215_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M+1; i_1++) {
            A[i_0] = E[i_0][i_1-2] * A[i_0+1] * A[i_1-1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        A[i_2] = 4;
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            C[i_3] = C[i_3+1] + D[i_3-1][i_2+1] * 1;
            D[i_3][i_2] = 3;
        }
    }
#pragma endscop
...
kernel_2412112215_05(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
