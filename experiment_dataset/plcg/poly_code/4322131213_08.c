...
void kernel_4322131213_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_Q-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                for (int i_3 = i_2; i_3 < min(-i_0+PB_M-1, -i_2+PB_M); i_3++) {
                    A[i_3+i_2][i_0][i_1][i_1+i_0+1] = D[i_1][i_0][i_3+i_2][i_3-1] + D[i_3-1][i_2][i_0-1][i_1] + D[i_1][i_0][i_3][i_2] * A[i_3+i_0+1][i_1][i_2-1][i_2+i_1] * 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
            B[i_5+1] = E[i_5-1][i_4] - E[i_5][i_4] * B[i_5] + B[i_5+2] + 3;
        }
        for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
            B[i_6+1] = F[i_6] + E[i_6+1][i_4+1] * B[i_6+1] + 3;
        }
    }
#pragma endscop
...
kernel_4322131213_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
