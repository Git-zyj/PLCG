...
void kernel_4314312215_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        A[i_0-2] = 3;
        A[i_0-1] = D[i_0+1] + A[i_0-2] * A[i_0-3] + 4;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < PB_P; i_3++) {
                for (int i_4 = max(1, i_3); i_4 < PB_M; i_4++) {
                    C[i_4][i_2-1][i_3][i_1] = C[i_4][i_2][i_3][i_1] + C[i_4-1][i_2-1][i_1][i_3] * 4;
                }
            }
        }
    }
...
kernel_4314312215_06(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
