...
void kernel_4224211221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L, i_0); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_P+1; i_2++) {
                A[i_0][i_1][i_2+i_0-1] = 2;
            }
        }
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_N+1; i_4++) {
                for (int i_5 = 1; i_5 < -i_3+PB_L; i_5++) {
                    B[i_3][i_5][i_4][i_0] = C[i_4][i_0][i_5+1][i_3-1] - C[i_4+i_3-1][i_0][i_5+i_3][i_4+1] * B[i_3-2][i_5][i_4+2][i_0] * B[i_5+1][i_3][i_0][i_4] + B[i_5-1][i_4][i_0][i_3-2] * 1;
                }
            }
        }
    }
...
kernel_4224211221_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
