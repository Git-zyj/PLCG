...
void kernel_3234131215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1-1][i_1-1] = C[i_0][i_1] - C[i_1][i_0] * A[i_0-2][i_0-2] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            for (int i_4 = i_3; i_4 < PB_Q-1; i_4++) {
                B[i_2][i_3][i_4+1] = D[i_2][0][i_3-1] - D[i_3][i_4][i_2] * D[i_3][i_4][i_3] + A[i_4-1][i_4-1] - B[i_2+1][i_3][i_4+1] * B[i_2][i_3+1][i_4] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3234131215_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
