...
void kernel_2226111123_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P+2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_P+1, i_0); i_1++) {
            B[i_0-1][i_1-2][i_0-2] = B[i_1][i_0+2][i_1-1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = PB_P+1 - 1; i_3 >= 1; i_3--) {
            B[i_3][i_2][i_3-1] = C[i_2][i_3+1] * 4;
        }
    }
#pragma endscop
...
kernel_2226111123_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
