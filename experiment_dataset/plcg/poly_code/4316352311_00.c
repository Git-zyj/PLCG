...
void kernel_4316352311_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_M, -i_0+PB_P); i_2++) {
                for (int i_3 = 3; i_3 < min(-i_0+PB_M, -i_1+PB_M-2); i_3++) {
                    B[i_1-2][i_3][i_1] = D[i_1][i_3+i_0][i_1][2] + D[i_3][i_3][i_1][i_0] - E[i_1][i_2][i_0+2][i_0] * F[i_3][i_3+i_1+2][i_0][i_0+1][i_2] + G[i_2+i_1][i_0][i_3-2][0] * E[i_1][i_3-1][i_2+i_0][i_3+i_1+1] - 2;
                    B[i_3-2][i_1][i_3] = H[i_2][i_1][i_3-1] + H[i_2][i_3][i_2+i_1] + I[i_3][i_1-2][i_2] + E[i_3+i_2+1][i_0][i_3+i_1][0] - E[i_2+2][i_0][i_3][i_1-2] + E[i_3-2][i_2-2][i_1][i_0-1] + B[i_3-2][i_1-1][i_3] - B[i_3-3][i_1][i_3-1] - B[i_3-3][i_1-1][i_3-1] - 4;
                }
            }
        }
        for (int i_4 = i_0; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_M; i_5++) {
                B[i_5-1][i_4+1][i_5+1] = H[i_5+i_4][i_0][i_4] * I[i_5][i_0-2][i_4+1] - J[i_4][i_0][i_5-1] - 5;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4316352311_00(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
