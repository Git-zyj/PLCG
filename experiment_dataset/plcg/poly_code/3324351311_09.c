...
void kernel_3324351311_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = D[i_0] * D[i_0] + E[i_0] * D[i_0] + A[i_0+1] * C[i_0-1] * 6;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L; i_2++) {
            for (int i_3 = 1; i_3 < min(PB_N, -i_2+PB_M); i_3++) {
                B[i_1][i_1][i_2][i_3] = F[i_3][i_1][i_2] + F[i_1][i_3+i_2][i_1-1] - G[i_1][i_3+1] + H[i_3][i_1] + B[i_3][i_3][i_2-1][i_1] * B[i_2][i_2][i_3+1][i_1] + B[i_1+1][i_1+1][i_2][i_3-1] + B[i_1-1][i_1-1][i_2][i_3] - 2;
            }
        }
        C[i_1] = E[i_1+1] + D[i_1] * I[1] - I[i_1] + D[i_1] * C[i_1+1] * 6;
    }
#pragma endscop
...
kernel_3324351311_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
