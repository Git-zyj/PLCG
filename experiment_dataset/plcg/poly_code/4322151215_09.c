...
void kernel_4322151215_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < min(-i_0+PB_Q, PB_Q-1); i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < min(PB_L-2, -i_2+PB_M); i_3++) {
                    B[i_2][i_3+2] = D[i_1][i_2][i_0][i_3] + D[i_3][i_2+1][i_0][i_1] * E[i_0][i_3][i_0][i_1+1][i_2+i_1] + D[i_3+i_2][i_0][i_1-1][i_0] - D[i_2-1][i_3+1][i_1-1][i_1+i_0] + 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < min(-i_4+PB_L+1, PB_L-2); i_5++) {
            B[i_4][i_5+1] = F[i_5][i_4-1] + G[i_5] + F[i_4][i_5-1] - H[i_5][i_5-1][i_4] * F[i_5][i_4] + H[i_5+i_4-1][i_5+i_4-1][1] * B[i_4+1][i_5+2] * B[i_4][i_5] - 2;
        }
    }
    for (int i_6 = 0; i_6 < PB_L; i_6++) {
        for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
            for (int i_8 = 0; i_8 < PB_L; i_8++) {
                B[i_8][i_7+1] = F[i_8][i_6] * F[i_8][i_7] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4322151215_09(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
