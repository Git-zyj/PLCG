...
void kernel_3214212221_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[i_1+i_0] = A[i_1+i_0-1] + A[i_1+i_0-1] - B[i_1+i_0][0] - 3;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_L); i_2++) {
            B[i_2+i_0][0] = C[i_2][i_0-2] - 4;
        }
    }
#pragma endscop
...
kernel_3214212221_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
