...
void kernel_2432151311_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            A[i_0] = E[i_1+1][i_0] * F[i_1] * E[i_1][i_0] * E[i_1+i_0][i_1+1] - E[i_1][i_0] + B[i_1] - D[i_0+1][i_1-1] * D[i_0][i_1-1] * D[i_0+1][i_1] + 1;
        }
    }
    ...
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        C[i_3] = F[i_3] + F[i_3] - F[i_3] * F[i_3] + F[i_3] * F[i_3] + F[i_3-1] * F[i_3] - B[i_3+1] - C[i_3+1] - C[i_3-1] - 2;
        for (int i_4 = 1; i_4 < min(-i_3+PB_L, PB_M-1); i_4++) {
            D[i_4+1][i_3-1] = F[i_3-1] + E[i_4+i_3][i_3] * E[i_4][0] - E[i_4-1][i_3] + B[i_3-1] * C[i_4] + 1;
        }
    }
#pragma endscop
...
kernel_2432151311_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
