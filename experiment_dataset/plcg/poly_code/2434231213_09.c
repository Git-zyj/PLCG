...
void kernel_2434231213_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0+1] = E[i_0] * E[i_0] + F[i_0-1] + E[i_0+1] + F[i_0] - E[i_0+1] - A[i_0] * 6;
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_0] = G[i_1-1][i_0+1] - A[i_1+1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            C[i_3-1] = G[i_3][i_2] + H[i_2][i_3] + H[i_3+1][i_2] * E[i_3+1] + H[i_3][i_2] - C[i_3-2] - 1;
        }
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            D[i_2][i_4] = D[i_2][i_4-1] + 1;
        }
    }
#pragma endscop
...
kernel_2434231213_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
