...
void kernel_2316211313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = i_0; i_2 < PB_M; i_2++) {
            C[i_0][i_2] = D[i_2-1] * A[i_2] - C[i_0][i_2-1] - 3;
        }
    }
#pragma endscop
...
kernel_2316211313_00(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
