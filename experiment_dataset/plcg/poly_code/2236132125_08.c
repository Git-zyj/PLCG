...
void kernel_2236132125_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            B[i_1+1][i_0] = C[i_1][i_0-1] - D[i_0] * D[i_1] + D[i_0] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = i_2; i_3 < PB_N; i_3++) {
            B[i_3+1][i_2] = C[i_3][i_2+2] + D[i_2+2] * B[i_3+1][i_2-2] + 3;
        }
    }
#pragma endscop
...
kernel_2236132125_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
