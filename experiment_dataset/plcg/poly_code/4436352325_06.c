...
static void print_array(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4436352325_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_1+PB_M, -i_2+PB_P); i_3++) {
                    A[i_1][0][i_2] = E[i_0][i_3+i_2][i_1][i_0+2] + F[i_3+i_1][i_2][i_0-2][i_1] - F[i_1][i_2][i_3][i_1] + G[i_2-1][i_1+2][i_3][i_0] + H[i_2][i_0-2][i_0][i_3+1][i_1-1] - G[i_1][i_0][i_3][i_2+1] + A[i_2][0][i_0-1] * A[i_2][0][i_3-2] * A[i_2][0][i_3+1] + A[i_2-2][0][i_3-2] - A[i_3-2][0][i_0] * A[i_1][0][i_2-1] * 2;
                }
            }
        }
        for (int i_4 = 0; i_4 < min(i_0, -i_0+PB_P); i_4++) {
            for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
                for (int i_6 = 0; i_6 < min(-i_0+PB_M, -i_5+PB_L-1); i_6++) {
                    A[i_5+2][0][i_6+2] = G[i_6+i_4][i_5][i_0+2][i_0+2] - F[i_6+i_0][i_6][i_5+i_4][i_4] - G[i_6][i_0][i_5-2][i_4] + F[i_5][i_6+i_5+1][i_4+1][i_0] - I[i_5+1][i_6+2][i_4] * F[i_5][i_0][i_6][i_4] - E[i_5][i_4+i_0][i_6][0] + 2;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-2; i_7++) {
        C[i_7][i_7] = J[i_7] * K[i_7] * L[i_7+1] - J[i_7] + 1;
        for (int i_8 = 0; i_8 < -i_7+PB_M-2; i_8++) {
            D[i_8+i_7][i_8+1] = M[i_8][i_7] - N[i_7][i_7] * N[i_8][i_7] - C[i_7-1][i_7-1] * D[i_8+i_7+2][i_8+3] - D[i_8+i_7-1][i_8] * 1;
        }
    }
#pragma endscop
