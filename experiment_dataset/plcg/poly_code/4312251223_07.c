...
void kernel_4312251223_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0] = D[i_0-1] - E[i_0] - D[i_0] + E[i_0-1] - D[i_0+1] + D[i_0] * A[i_0+2] * A[i_0+1] - 1;
    }
    for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_P-2; i_2++) {
            for (int i_3 = i_1; i_3 < PB_M; i_3++) {
                C[i_3][i_2][i_2+i_1+2] = F[i_3+1][i_2-1] - G[i_2+1][i_3][i_1] + G[i_1][i_2][i_3] + 1;
            }
        }
        for (int i_4 = 1; i_4 < min(PB_P-1, -i_1+PB_Q); i_4++) {
            for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
                for (int i_6 = 0; i_6 < min(-i_5+PB_Q, -i_4+PB_P-1); i_6++) {
                    C[i_5-1][i_4][i_6+i_4] = H[i_6][i_5-1][i_1][i_4-1] - I[i_4+1][i_5][i_6][i_5+1] + I[i_4][i_1][i_4][i_5] - I[i_4-1][i_1+1][0][i_1-1] * G[i_5+1][i_4+i_1][i_6+i_4+1] + G[i_1][i_6+i_5][i_4-1] * C[i_5-1][i_4][i_6+i_4+1] - 1;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4312251223_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
