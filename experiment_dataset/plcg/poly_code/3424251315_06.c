...
void kernel_3424251315_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] * F[i_0] * F[0] + F[i_0-1] * E[i_0] + E[i_0] - F[i_0] - E[i_0+1] + G[i_0][i_0-1] - A[i_0+1] * C[i_0] + 4;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        B[i_1] = F[i_1] - F[i_1] * F[0] + B[i_1+1] - 5;
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            C[i_2] = H[i_2][i_1] - H[i_2][i_1+1] * H[i_1+1][0] + G[i_2][i_1] * A[i_2-1] + B[i_2] * C[i_2+1] * 5;
        }
        for (int i_3 = 1; i_3 < -i_1+PB_L-1; i_3++) {
            B[i_3] = H[i_3+i_1][i_3+i_1] - H[i_3+i_1+1][i_3-1] + H[i_3+1][i_1] - H[i_3][i_1-1] - A[i_3-1] + B[i_3+1] * 1;
        }
    }
#pragma endscop
...
kernel_3424251315_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
