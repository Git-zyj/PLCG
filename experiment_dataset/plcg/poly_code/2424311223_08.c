...
void kernel_2424311223_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        ...
        for (int i_3 = 2; i_3 < min(i_1, PB_M-2); i_3++) {
            C[i_1][i_3] = F[i_1][i_3] - C[i_1-1][i_3-2] + C[i_1+1][i_3] + C[i_3+2][i_1-2] * 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = i_4; i_5 < min(-i_4+PB_N, PB_L-1); i_5++) {
            D[i_4+1][i_5-1] = F[i_5+i_4][0] - D[i_4+3][i_5+1] - D[i_4-1][i_5-1] - D[i_4+3][i_5-1] - 6;
        }
    }
#pragma endscop
...
kernel_2424311223_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
