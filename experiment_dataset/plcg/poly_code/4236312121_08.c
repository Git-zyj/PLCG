...
void kernel_4236312121_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        ...
        for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
            for (int i_3 = i_2; i_3 < -i_2+PB_N; i_3++) {
                B[i_0+2][i_2][i_3] = C[i_3+i_2][i_0] * B[i_0+2][i_2+2][i_3+2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4236312121_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
