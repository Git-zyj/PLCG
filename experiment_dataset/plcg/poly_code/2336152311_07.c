...
void kernel_2336152311_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_N; i_1++) {
            A[i_1+i_0-1][i_0+2] = D[i_1][0][i_1] - E[i_1-2][i_0] - E[i_1+i_0][i_0] + F[i_1] * E[i_1+2][i_0+1] * F[i_0] - A[i_1+i_0][i_0+2] * 4;
        }
    }
    for (int i_2 = 3; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 3; i_3 < -i_2+PB_L-2; i_3++) {
            B[i_3+i_2][i_3] = E[i_3][0] + E[i_3][i_2] + E[i_3][i_2] * B[i_3+i_2][i_3-1] - B[i_3+i_2+2][i_3+1] + C[i_2-2][i_3+i_2] - C[i_3-3][i_3+i_2-1] - C[i_2-3][i_3+i_2-2] + 6;
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = i_4; i_5 < -i_4+PB_L-1; i_5++) {
            C[i_4-2][i_5+i_4] = E[i_5][i_4] - E[i_5][i_5] * E[i_4][i_5] * F[i_5] - E[i_4][i_5] - D[i_4][i_5][i_4] - B[i_5+i_4][i_4+1] + B[i_5+i_4+1][i_5+1] - B[i_5+i_4][i_5+1] - 1;
        }
    }
#pragma endscop
...
kernel_2336152311_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
