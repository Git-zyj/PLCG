...
void kernel_3222131225_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_P, -i_0+PB_M-2); i_1++) {
            A[i_1+i_0][i_1] = C[i_1][i_0] * C[i_0][i_1] + C[i_1][i_0-1] - A[i_1+i_0-2][i_1-2] - A[i_1+i_0+2][i_0] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_P; i_3++) {
            B[i_3+1][i_3-1] = C[i_2][i_3] * C[i_3][i_3+i_2] * C[i_3][i_2] - B[i_3][i_3-2] - 4;
        }
    }
#pragma endscop
...
kernel_3222131225_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
