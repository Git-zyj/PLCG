...
void kernel_2422351313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[0] - E[i_0] - F[i_0-1] * F[i_0] - F[i_0] + A[i_0+1] * A[i_0-1] + A[i_0-1] - 6;
    }
    for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_N; i_2++) {
            C[i_1-1][i_2] = E[i_2] - E[i_2] + G[i_2+i_1][0] + G[i_1][1] * G[i_2+1][i_1] + G[i_1-1][i_1] - 3;
        }
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            C[i_1-1][i_3] = H[i_3][i_1+1] - I[i_3][i_1] - G[i_1][i_3] + A[i_1] * C[i_1-1][i_3-1] + C[i_1][i_3] + 4;
        }
        A[i_1] = I[i_1][i_1-1] * F[i_1] * J[i_1] * F[i_1] * G[i_1+1][i_1] * F[i_1] + 6;
    }
#pragma endscop
...
kernel_2422351313_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
