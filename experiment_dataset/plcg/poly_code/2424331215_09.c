...
void kernel_2424331215_09(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        D[i_0-1] = E[i_0] - E[i_0-1] - F[0] + D[i_0-2] * 4;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_L); i_2++) {
            B[i_1+1][i_2-1] = G[i_2-1][i_1] - H[i_2+i_1][1] + H[i_1+1][i_2] * I[i_1+1][i_2] + J[i_2-1] - 6;
        }
        for (int i_3 = 1; i_3 < min(PB_L, -i_1+PB_N+1); i_3++) {
            D[i_3-1] = I[i_3+i_1-1][1] + D[i_3-1] + 6;
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        D[i_4-1] = J[i_4] * J[i_4] * D[i_4] - D[i_4-2] + 3;
    }
#pragma endscop
...
kernel_2424331215_09(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
