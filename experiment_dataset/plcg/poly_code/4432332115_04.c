...
void kernel_4432332115_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = E[1] + E[i_0] + F[i_0] * E[i_0] - 1;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
            A[i_2] = G[i_2+2][2] - H[i_2+2][i_2] - 6;
        }
        A[i_1+1] = I[i_1] * 5;
        for (int i_3 = 0; i_3 < min(-i_1+PB_N, PB_M-1); i_3++) {
            A[i_3+1] = G[i_1][1] * G[i_3][i_1] + G[i_1][i_3+i_1] - J[i_1][i_3] + J[i_3+i_1][0] - A[i_3+1] + 5;
        }
    }
#pragma endscop
...
kernel_4432332115_04(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
