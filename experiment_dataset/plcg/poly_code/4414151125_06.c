...
void kernel_4414151125_06(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = max(1, i_1); i_3 < -i_0+PB_L; i_3++) {
                    D[i_3-1][i_1][i_2] = E[i_2][i_1][i_3+i_0][i_0] - 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_Q-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            B[i_5] = F[i_4][i_5] * F[i_5+1][i_4] * F[i_4-1][i_5+1] * G[i_5-1] * B[i_5-1] + 2;
        }
        B[i_4] = G[i_4] + G[i_4] * G[i_4+1] + G[i_4-1] - G[i_4] * 4;
        for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 2; i_7 < min(-i_4+PB_Q-1, -i_6+PB_Q); i_7++) {
                D[i_7-1][i_6][i_4] = H[i_6+1][i_7+i_4+1][i_4] * F[i_7][i_6] - H[i_4][i_6][0] - F[i_6][i_4-1] + H[i_6][i_4][i_7-1] - H[i_6][i_7+1][i_4] * H[i_4+1][i_7+i_6][i_6] + H[i_7-1][i_4-1][i_6] * D[i_7-2][i_6+1][i_4+1] * 4;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414151125_06(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
