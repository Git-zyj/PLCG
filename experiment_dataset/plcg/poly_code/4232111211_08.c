...
void kernel_4232111211_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_0][i_1+i_0] = C[i_1] * A[i_0][i_1+i_0+1] * A[i_0+1][i_1+i_0+2] - 6;
        }
    }
        ...
        for (int i_3 = 0; i_3 < PB_Q-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                    B[i_5][i_3][i_2+1] = B[i_5+1][i_3][i_2+1] - B[i_5+1][i_4][i_3+1] * 1;
                }
            }
        }
    }
...
kernel_4232111211_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
