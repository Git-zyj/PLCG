...
void kernel_4232211221_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            for (int i_4 = i_3; i_4 < min(-i_3+PB_L, PB_M-3); i_4++) {
                for (int i_5 = 0; i_5 < -i_3+PB_L; i_5++) {
                    B[i_2+1][i_4+i_3][i_5] = B[i_4][i_5+i_3-1][i_2+2] - B[i_5+1][i_4+i_3-2][i_2] * B[i_4+3][i_5+i_3][i_2-2] - 3;
                }
            }
        }
    }
...
kernel_4232211221_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
