...
void kernel_3436131115_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0+1] - E[i_0-1] - A[i_0-1] * B[i_0][i_0] + 5;
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            B[i_0][i_0] = F[i_1][i_0] - E[i_0] + F[i_1][i_0] + F[i_1-1][i_1] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
            for (int i_4 = i_2; i_4 < min(-i_3+PB_L, PB_M); i_4++) {
                B[i_4][i_4] = G[i_2+1][i_3][i_4] + G[i_2][i_4][i_3+1] - F[i_2][i_3] + H[i_2][i_4+i_3][i_4][i_3] + 5;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_P; i_5++) {
        for (int i_6 = 0; i_6 < min(i_5, PB_M-1); i_6++) {
            for (int i_7 = 0; i_7 < PB_M-1; i_7++) {
                B[i_7+1][i_7+1] = G[i_6][i_7+1][1] - F[i_5-1][i_6+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3436131115_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
