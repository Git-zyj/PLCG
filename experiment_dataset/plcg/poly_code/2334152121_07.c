...
void kernel_2334152121_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_N-1 - 1; i_0 >= 2; i_0--) {
        A[i_0-2] = D[i_0+1] * D[i_0] * D[i_0] + D[i_0] - D[i_0] - D[i_0] + D[i_0-2] - 5;
    }
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        B[i_1] = D[i_1] - D[i_1] * D[i_1] + B[i_1-2] - 3;
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        C[i_2][i_2] = D[i_2-1] - D[i_2] * D[i_2+2] * D[i_2] - D[i_2+1] * 1;
    }
#pragma endscop
...
kernel_2334152121_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
