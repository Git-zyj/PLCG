...
void kernel_4216212223_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
            A[i_0-2][i_1] = A[i_0-2][i_1+2] + A[i_0-2][i_1-1] * A[i_0-2][i_1+1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_Q; i_3++) {
            for (int i_4 = max(2, i_3); i_4 < PB_N; i_4++) {
                for (int i_5 = 0; i_5 < min(-i_4+PB_N, -i_2+PB_P); i_5++) {
                    B[i_5+i_4][i_3][i_5+i_2][i_3] = C[i_3-1][i_4-2][i_2][i_2] * 5;
                }
            }
        }
    }
...
kernel_4216212223_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
