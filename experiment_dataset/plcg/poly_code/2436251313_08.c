...
void kernel_2436251313_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0][i_0] = E[i_0] * F[i_0] + F[i_0-1] - F[i_0+1] - A[i_0+1][i_0+1] + A[i_0-1][i_0-1] + A[i_0-1][i_0-1] * A[i_0-1][i_0-1] * 4;
    }
    for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            B[i_2+1][i_1+1] = G[i_2-1][i_1] * B[i_2+2][i_1+1] - B[i_2+1][i_1] + B[i_2+1][i_1+2] + B[i_2][i_1+2] - 2;
        }
        for (int i_3 = 0; i_3 < i_1; i_3++) {
            C[i_1+1][i_3] = G[i_3+1][i_1] - H[i_1][i_3] - G[i_1][i_3] - H[i_3][i_1+1] - H[i_3][i_1] - E[i_1+1] - C[i_1+2][i_3] * 5;
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < min(PB_L, -i_4+PB_M); i_5++) {
            D[i_5][i_4] = G[i_5][i_4-1] * H[i_5][i_4-1] - G[i_5+i_4][i_4-1] - G[i_4+1][i_5] * G[i_4][i_5] * E[i_5+i_4] + H[i_5+1][i_4] - E[i_5] * C[i_5+1][i_4] * D[i_5][i_4+1] - 2;
        }
    }
#pragma endscop
...
kernel_2436251313_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
