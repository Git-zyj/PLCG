...
void kernel_2222132115_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            B[i_0-2][i_1] = C[i_1+i_0] * C[i_1] - B[i_0-3][i_1-1] * 2;
        }
        for (int i_2 = 0; i_2 < min(PB_L, i_0); i_2++) {
            B[i_0-2][i_2] = C[i_2+1] - D[i_0][i_2] - C[i_2] * 1;
        }
    }
#pragma endscop
...
kernel_2222132115_04(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
