...
void kernel_2312311221_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        A[1] = A[1] * A[1] * 2;
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            B[i_1] = D[i_1][i_0+1] - A[1] - 6;
        }
        for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
            C[i_0][i_2] = C[i_0][i_2+1] - C[i_0][i_2+2] + C[i_0][i_2-2] - 3;
        }
    }
#pragma endscop
...
kernel_2312311221_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
