...
void kernel_2326212313_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0][i_0] = A[i_0+1][i_0+1] - B[i_0][i_0] - B[i_0-1][i_0-1] + 2;
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_0][i_0] = A[i_0-1][i_0-1] - B[i_0+1][i_0+1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_N, i_2); i_3++) {
            C[i_3+i_2-1][0] = D[i_2][i_3] + B[i_2+1][i_2+1] - C[i_3+i_2][0] + 1;
        }
    }
#pragma endscop
...
kernel_2326212313_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
