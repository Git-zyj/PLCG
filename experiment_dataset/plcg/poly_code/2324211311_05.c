...
void kernel_2324211311_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            A[i_0][i_1-1] = D[i_0][i_1] + A[i_0-1][i_1-1] * A[i_0][i_1] + A[i_0+1][i_1-1] - A[i_1][i_0] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        ...
        for (int i_4 = PB_N-1 - 1; i_4 >= 1; i_4--) {
            C[i_2][i_4] = D[i_4+1][i_2-1] - B[i_4+1][i_2+1] + C[i_2+1][i_4] - C[i_2+1][i_4-1] + 1;
        }
    }
#pragma endscop
...
kernel_2324211311_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
