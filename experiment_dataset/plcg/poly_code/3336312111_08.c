...
void kernel_3336312111_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_2 = 0; i_2 < PB_Q-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                B[i_3][i_4] = D[i_3][i_4-1][i_2+2] * C[i_3-2] + 4;
            }
            C[i_3-2] = 6;
        }
    }
...
kernel_3336312111_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
