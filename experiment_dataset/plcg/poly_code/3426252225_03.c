...
void kernel_3426252225_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + E[i_0+2] * F[i_0] - F[i_0] * F[i_0-2] + E[i_0] - 1;
        A[i_0] = E[i_0+1] - E[i_0] + F[i_0+1] - G[i_0+1][i_0+2] + A[i_0] + 1;
    }
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            D[i_1-2][i_2+2] = F[i_1+1] * G[i_2][i_1+1] * E[i_2-1] + G[i_2][i_1+1] - H[i_2][i_1+2] - 4;
            D[i_1][i_2+1] = E[i_2+1] * G[i_1][i_2] + E[i_2] + I[i_2-2][i_2+i_1][i_1] - H[i_2+i_1][i_1-1] + D[i_1][i_2+3] * D[i_1][i_2+2] + D[i_1][i_2-1] * 3;
        }
    }
#pragma endscop
...
kernel_3426252225_03(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
