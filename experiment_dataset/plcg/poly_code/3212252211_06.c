...
void kernel_3212252211_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0+1][i_1] = C[i_1][i_0] - C[i_0+2][i_1-1] + D[i_0][0] - D[i_0][i_1] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-3; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_3][i_2+2] = D[i_3][i_2] + C[i_3][i_2-1] + C[i_2+2][i_3] * D[i_2][2] - D[i_3][i_2-1] - C[i_3][i_3+2] + B[i_3][i_2+3] - B[i_2][i_3+1] * B[i_3-1][i_2+2] * B[i_3][i_2+1] + 2;
        }
    }
#pragma endscop
...
kernel_3212252211_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
