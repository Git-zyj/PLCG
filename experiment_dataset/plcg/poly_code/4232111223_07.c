...
void kernel_4232111223_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
                A[i_2][i_1+1][i_0] = C[i_1-1][i_0][i_2-1] - A[i_2][i_1][i_0] - A[i_1-2][i_2+1][i_0] + 1;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
            for (int i_5 = max(2, i_3); i_5 < PB_P; i_5++) {
                for (int i_6 = 0; i_6 < PB_N-2; i_6++) {
                    A[i_5-2][i_6+2][i_4+2] = C[i_4][i_5][i_3-1] - 2;
                }
            }
        }
    }
...
kernel_4232111223_07(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
