...
void kernel_4226231315_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            B[i_1+1][i_0-1] = C[i_1][i_0] * C[i_1+1][i_0] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < min(-i_2+PB_M+1, PB_P); i_3++) {
            B[i_2+1][i_3-1] = C[i_2-1][0] * D[i_2] - C[i_3+i_2-1][i_2] * B[i_2+2][i_3-1] - B[i_2+1][i_3-2] - B[i_2+2][i_3-2] * 4;
        }
    }
#pragma endscop
...
kernel_4226231315_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
