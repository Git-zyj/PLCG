...
void kernel_2322132215_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[0] = D[i_1][i_0+2] - E[i_0] - D[i_1][i_0+1] * A[0] + A[0] - 6;
        }
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            A[0] = D[i_0][i_2+2] - D[i_0][i_2] + 4;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_L-1, -i_3+PB_M-2); i_4++) {
            C[i_3+1][i_4] = D[i_4+i_3+2][i_3] - D[i_3][i_4] - D[i_4+i_3][i_4] + C[i_3+1][i_4+1] - 3;
        }
    }
#pragma endscop
...
kernel_2322132215_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
