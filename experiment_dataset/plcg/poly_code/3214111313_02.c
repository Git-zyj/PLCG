...
void kernel_3214111313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            A[i_1+1] = C[i_1] + A[i_0+2] + A[i_1] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_N+1; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_L-3, PB_N+1); i_3++) {
            for (int i_4 = 0; i_4 < min(min(PB_N, -i_3+PB_L-1), -i_2+PB_L-1); i_4++) {
                B[i_2-1][i_4][i_4+i_3+1] = B[i_3-1][i_2][i_4+i_2] * B[i_4][i_3+1][i_3+i_2+3] * B[i_2-2][i_4][i_4+i_3+1] - B[i_3-2][i_2][i_4+i_2+1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3214111313_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
