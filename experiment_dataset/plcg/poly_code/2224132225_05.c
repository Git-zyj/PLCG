...
void kernel_2224132225_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_N+1; i_1++) {
            A[i_0][i_0+2] = C[i_1-1] - B[i_1+2][i_0] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            B[i_3][i_2] = D[i_2][i_3] - D[i_3][i_2] - D[i_3][i_2-2] + C[i_3+1] + B[i_2+2][i_3] + B[i_3-2][i_2] + 4;
        }
    }
#pragma endscop
...
kernel_2224132225_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
