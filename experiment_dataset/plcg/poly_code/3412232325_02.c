...
void kernel_3412232325_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            C[i_0+1] = E[0][i_0+2] + F[i_0][i_1] * G[i_0][i_1][i_0] + 6;
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                C[i_2+1] = G[i_0][i_2][i_1] * 2;
            }
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            C[i_3+1] = E[i_3][i_0] * C[i_0] * C[i_3-1] + 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-3; i_4++) {
        C[i_4+3] = H[i_4-1] * H[i_4+1] * I[i_4] * I[i_4] - I[0] * I[i_4] - C[i_4+1] - 4;
    }
#pragma endscop
...
kernel_3412232325_02(xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
