...
void kernel_3426152113_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[1] - E[i_0] + E[i_0] + E[i_0-1] * E[i_0] - 3;
    }
    for (int i_1 = 0; i_1 < PB_Q-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            for (int i_3 = max(1, i_2); i_3 < PB_L; i_3++) {
                D[i_2] = F[i_3-1][i_2] + G[i_3][i_1][i_2] * G[i_3][i_1+2][i_2+1] * H[i_3][i_2][i_1+1][i_1] * 6;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        C[i_4+1] = E[i_4] - E[i_4-2] * E[i_4] - E[i_4] - E[i_4] * 4;
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            D[i_5] = F[i_5][i_4] * F[i_5][i_4-2] - G[i_4+2][i_5][i_4+1] + C[i_4+1] + 3;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3426152113_08(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
