...
void kernel_3214311323_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N-1; i_1++) {
            B[i_1+1][i_0+1] = C[i_0+1][i_1] - 6;
        }
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            for (int i_3 = i_0; i_3 < PB_L-3; i_3++) {
                B[i_2+1][i_3+1] = B[i_3+3][i_2+1] * B[i_3+1][i_2+2] + B[i_2+1][i_3+3] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3214311323_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
