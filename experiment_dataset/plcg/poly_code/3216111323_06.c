...
void kernel_3216111323_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_N; i_1++) {
            A[i_1] = C[i_0-1] + A[i_1-2] + 5;
        }
        for (int i_2 = i_0; i_2 < PB_N; i_2++) {
            for (int i_3 = i_2; i_3 < PB_N-2; i_3++) {
                A[i_3] = A[i_3+2] + A[i_3] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3216111323_06(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
