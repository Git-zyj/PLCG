...
void kernel_4414252225_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        B[i_0-2] = E[i_0] + F[i_0-2] - F[i_0+2] * E[i_0] - 3;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            B[i_2] = F[i_2] - F[i_1] - G[i_2][i_1] - H[i_2-2][i_1] - B[i_2-2] - 5;
        }
        for (int i_3 = 0; i_3 < i_1; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_M, -i_3+PB_L-2); i_4++) {
                B[i_3] = I[i_3][i_4][i_1] * J[i_3][i_1][i_4] * I[i_3+1][i_1+2][i_4+i_3+2] + J[i_4-1][i_1-2][i_3+2] - 4;
            }
        }
        B[i_1-1] = F[i_1] * F[i_1+2] + E[i_1] * F[i_1] - E[i_1] - F[i_1] + B[i_1-1] + 5;
    }
#pragma endscop
...
kernel_4414252225_03(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
