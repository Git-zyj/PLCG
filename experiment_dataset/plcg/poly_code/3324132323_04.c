...
void kernel_3324132323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        A[i_0-1] = D[i_0][0] + 4;
        A[i_0-1] = E[i_0] * E[i_0] + E[i_0-1] + E[i_0] * A[i_0-1] * 3;
    }
    for (int i_1 = 0; i_1 < PB_L-4; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_M+1; i_2++) {
            C[i_1+2][i_2] = D[i_2][i_1] + D[i_2-2][i_1] + D[i_2][i_1] - D[i_2][i_1] * A[i_2-1] + C[i_1+1][i_2] + C[i_1][i_2] * C[i_1+4][i_2+1] - 5;
        }
    }
#pragma endscop
...
kernel_3324132323_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
