...
void kernel_3336252111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P-1; i_1++) {
            A[i_0][i_1+i_0+1] = D[i_1][i_0] * E[i_0+1] * F[i_1+i_0+2][i_0-1] + E[i_0-2] + F[i_1][i_0+2] - F[i_0-1][i_1] + G[i_0] * G[i_1] - F[i_0][i_1] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_P-2, PB_L-1); i_3++) {
            B[i_2][i_3] = G[i_2] + G[i_3] + D[i_3][0] + A[i_3+1][i_3+i_2+2] + B[i_2][i_3-1] * B[i_2][i_3+1] - 6;
        }
        for (int i_4 = i_2; i_4 < PB_L-2; i_4++) {
            for (int i_5 = 1; i_5 < PB_Q; i_5++) {
                C[i_5-1][i_4+2] = H[i_4][i_2][i_2] * F[i_4][i_5] + H[i_4+2][i_2+1][i_5] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3336252111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
