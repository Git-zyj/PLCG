...
void kernel_2322111321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M+1; i_1++) {
            A[i_0][i_1] = D[i_1-1][i_0] - 4;
        }
        B[i_0-1] = B[i_0-3] * B[i_0-2] - B[i_0] + B[i_0+1] - B[i_0-3] * B[i_0-3] - 2;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
            C[i_3-1] = A[i_2][i_3] - A[i_2][i_3-2] - C[i_3+1] + 4;
        }
    }
#pragma endscop
...
kernel_2322111321_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
