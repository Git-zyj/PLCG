...
void kernel_4224132225_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, i_0); i_2++) {
                A[i_1][i_2] = C[i_2+i_1][i_0] + D[i_1][i_0-2][i_0] - A[i_1-1][i_2] + A[i_1][i_2+2] - A[i_1-2][i_2+1] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        B[i_3+1][i_3+2] = E[i_3] + E[i_3] + C[i_3][i_3] - E[i_3+2] * 4;
    }
#pragma endscop
...
kernel_4224132225_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
