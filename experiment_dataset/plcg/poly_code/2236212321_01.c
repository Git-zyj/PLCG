...
void kernel_2236212321_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = C[i_0+1] * A[i_0-1] - A[i_0-2] - A[i_0+1] + A[i_0+2] + 1;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < -i_1+PB_M-1; i_2++) {
            B[i_2+i_1+1][i_2] = C[i_2+1] * B[i_2+i_1+1][i_2-2] * 6;
        }
    }
#pragma endscop
...
kernel_2236212321_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
