...
void kernel_4434332225_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = E[i_1] + F[i_1+1][i_0-2] + G[i_1][i_0-1] + F[i_1-1][i_0] * A[i_1][i_0-2] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
            A[i_2-1][i_3-1] = G[i_3-1][i_2] * 5;
        }
    }
        ...
        for (int i_5 = 0; i_5 < PB_L-3; i_5++) {
            for (int i_6 = 2; i_6 < -i_4+PB_L; i_6++) {
                for (int i_7 = 1; i_7 < PB_N-2; i_7++) {
                    C[i_6][i_5+1][i_7][i_4] = H[i_5][i_6-2][i_7][i_4+1] * I[i_7+2][i_5+1][i_6+i_4][i_7] + I[i_6+1][i_4][i_7][2] + I[i_5][i_6][i_7][i_4-2] - C[i_6][i_5+1][i_7-1][i_4] * C[i_6][i_5+3][i_7][i_4] * C[i_6][i_5+1][i_7-1][i_4+2] * 2;
                }
            }
        }
        for (int i_8 = 0; i_8 < PB_M; i_8++) {
            A[i_8][i_4+2] = G[i_4][i_8] - 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4434332225_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
