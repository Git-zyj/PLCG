...
void kernel_2334112325_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_L+1; i_1++) {
            A[i_0][i_1-1] = A[i_0][i_1-2] + A[i_0-1][i_1-1] * A[i_0+1][i_1-1] - A[i_0+1][i_1-3] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-3; i_2++) {
        C[i_2+3] = D[0] * C[i_2+1] * 4;
        C[i_2+1] = D[i_2] + 3;
    }
#pragma endscop
...
kernel_2334112325_05(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
