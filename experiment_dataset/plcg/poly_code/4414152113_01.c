...
void kernel_4414152113_01(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
    ...
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_P; i_2++) {
                B[0][i_1+i_0][i_2] = F[i_0][i_1] + G[i_1][i_2-2][1] - G[i_1][i_2+i_0][i_1+i_0] - G[i_1][i_2][2] + B[0][i_1+i_0-1][i_2] + 3;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < -i_3+PB_L-2; i_4++) {
            for (int i_5 = max(i_4, 1); i_5 < -i_3+PB_M-1; i_5++) {
                for (int i_6 = 2; i_6 < -i_5+PB_M+1; i_6++) {
                    B[0][i_6+i_5-1][i_4] = H[i_5+i_3+1][i_6][i_4][i_4] + H[i_5][i_6][i_3-2][0] - H[i_6][i_3][i_4][i_5+1] + H[i_4+1][i_6+1][i_4+i_3+2][i_5] + H[2][i_5][i_6-2][i_3-2] * H[i_6][i_3][i_4][i_5] + 1;
                }
            }
        }
        D[i_3] = E[i_3+2] + E[i_3] + E[i_3] - E[i_3] - E[i_3] + E[i_3-1] - E[i_3] * 1;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414152113_01(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
