...
void kernel_4222132225_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            A[i_1] = C[i_1][i_0+2] - D[0] * A[i_1+1] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_N-1, -i_2+PB_N); i_3++) {
            A[i_3+1] = C[i_3+1][i_2] + C[i_3+i_2][i_2] - C[i_3][i_2] * D[i_3-2] - A[i_3] + 4;
        }
    }
#pragma endscop
...
kernel_4222132225_03(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
