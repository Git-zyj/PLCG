...
void kernel_2322111211_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_P-1; i_2++) {
            B[i_1][i_2] = D[i_2-1][i_1] + B[i_1-1][i_2+1] - 5;
        }
        for (int i_3 = 0; i_3 < min(-i_1+PB_L-1, i_1); i_3++) {
            C[i_1][i_3] = D[i_3][i_1-1] + D[i_3][i_3+i_1+1] - B[i_3+1][i_1] * 1;
        }
    }
#pragma endscop
...
kernel_2322111211_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
