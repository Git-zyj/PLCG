...
void kernel_2436252323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] * E[i_0+2] + F[i_0+2] - F[i_0+2] + A[i_0-1] - A[i_0-2] + 6;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = 2; i_2 < min(i_1, PB_M-1); i_2++) {
            B[i_1][i_2-2] = F[i_2+1] + G[i_2-2][i_2-2] + A[i_2] - B[i_1][i_2-1] - 2;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M, i_3); i_4++) {
            A[i_4+2] = F[i_4-2] - G[i_4-1][i_3] + E[i_4] * H[i_4+2][i_3-2] - G[i_4+i_3][i_4] + 5;
        }
    }
    for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
        A[i_5-1] = F[i_5] + F[i_5] * F[i_5-2] * E[i_5+2] + F[i_5-1] + F[i_5+2] - F[i_5] * E[i_5] + E[0] * A[i_5] * 6;
    }
#pragma endscop
...
kernel_2436252323_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
