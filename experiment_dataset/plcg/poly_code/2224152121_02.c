...
void kernel_2224152121_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_0-1][i_1] = C[i_1][i_0][i_0] - D[i_1][i_0] - D[i_1+2][i_0+2] - D[i_0][i_1] * D[i_0][i_1+1] - D[i_1][i_0] + 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_N+2; i_2++) {
        for (int i_3 = 2; i_3 < PB_N+2; i_3++) {
            B[i_2-2][i_3] = D[i_3][i_2] - D[i_2][i_3+2] + E[i_3] * B[i_3-2][i_2-1] - 1;
        }
    }
#pragma endscop
...
kernel_2224152121_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
