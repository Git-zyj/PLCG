...
void kernel_3414231123_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            A[i_0-1][i_1-1] = E[i_1+1] - F[i_1][i_0+1] - G[i_0+1][i_1] - A[i_0-2][i_1-1] * A[i_0-3][i_1-1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        B[i_2-1] = H[i_2] + H[i_2] * 1;
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            B[i_2-1] = H[i_3+1] * 3;
            B[i_3-1] = F[i_3][i_2+1] * E[i_3-1] - F[i_2-1][i_3] - I[i_2][i_3][i_2] * F[i_2][i_3] * 1;
        }
    }
#pragma endscop
...
kernel_3414231123_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
