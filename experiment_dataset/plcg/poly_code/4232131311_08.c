...
void kernel_4232131311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            A[i_1] = C[i_0] - D[i_0][i_1] - A[i_1+1] * A[i_1-1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
            A[i_3+1] = D[0][i_3+1] - D[i_3][i_2] + C[i_3] * A[i_3+1] + 5;
        }
    }
#pragma endscop
...
kernel_4232131311_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
