...
void kernel_3434351215_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] + F[i_0+1] + G[i_0+1][0] + F[i_0] + A[i_0-1] - A[i_0+1] * B[i_0-1] - 4;
        B[i_0] = H[i_0] + H[i_0] * I[i_0+1][i_0] + H[i_0] - F[i_0-1] + 5;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            A[i_2+1] = F[i_2-1] - J[i_2][i_1-1] - E[1] + I[i_1][i_2] * I[i_2][0] * F[i_2] + J[i_2][i_1+1] - A[i_2+1] - 5;
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            A[i_3] = J[i_3][1] - E[i_3] - 1;
        }
    }
#pragma endscop
...
kernel_3434351215_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
