...
void kernel_4336232225_00(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_L-2, PB_M-1); i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    C[i_3][i_2+1][i_2+i_1+2] = D[i_1][i_0+2][i_3][i_2] - E[i_3][i_2][i_0][i_1+1] + 3;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N-2; i_4++) {
        for (int i_5 = max(i_4, 3); i_5 < -i_4+PB_N-2; i_5++) {
            B[i_5-1] = F[i_4][i_5] - F[i_5+i_4][0] + G[i_5+i_4+2][i_5-1] * B[i_5+1] * B[i_5-3] * 2;
        }
    }
    for (int i_6 = 0; i_6 < PB_P; i_6++) {
        for (int i_7 = max(i_6, 2); i_7 < PB_M; i_7++) {
            for (int i_8 = 0; i_8 < PB_Q-2; i_8++) {
                for (int i_9 = 1; i_9 < min(-i_8+PB_L-2, PB_M-1); i_9++) {
                    C[i_7][i_9+1][i_9+i_8+2] = H[i_9-1][i_8][i_7] + D[i_7][i_8][i_6][i_8+2] + C[i_7-2][i_8][i_9+i_8+1] * 6;
                }
            }
        }
    }
...
init_array(xc,yc,zc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4336232225_00(xc,yc,zc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,yc,zc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
