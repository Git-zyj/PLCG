...
void kernel_3212312111_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M+1; i_2++) {
                B[i_2+i_1-1][i_1+1] = C[i_1][i_0][i_2] + 3;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_M-1; i_3++) {
            B[i_3+i_0][i_0+1] = B[i_3+i_0+1][i_0+2] + 2;
        }
    }
#pragma endscop
...
kernel_3212312111_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
