...
void kernel_3222131211_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_Q, -i_0+PB_L); i_2++) {
                A[i_0][i_1][i_2] = C[i_2][i_1][i_0] * C[i_1][i_2+i_0][i_0] * D[i_2][i_1] * A[i_0+1][i_1][i_2-1] * 6;
            }
        }
        for (int i_3 = 1; i_3 < PB_Q; i_3++) {
            for (int i_4 = 1; i_4 < min(min(-i_0+PB_M, -i_3+PB_M), PB_Q); i_4++) {
                B[i_3][i_0][i_4-1][0] = C[i_3+1][i_4][i_0] + C[i_4+i_0][i_3][i_3] * C[i_4+i_3][i_0][i_0] - B[i_3-1][i_4][i_0-1][0] * B[i_4+1][i_3+1][i_0-1][0] - B[i_3][i_0-1][i_4-1][0] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3222131211_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
