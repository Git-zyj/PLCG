...
void kernel_4436151213_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = min(PB_P-1, -i_1+PB_L) - 1; i_2 >= 1; i_2--) {
                for (int i_3 = 1; i_3 < min(min(PB_N-1, -i_1+PB_L), i_1); i_3++) {
                    A[i_1][i_3+i_1][i_2+1] = E[i_1-1][i_0+1][i_3][i_2-1] * F[i_1-1][i_2-1][i_0][i_3][i_2] * E[i_2][i_3+1][i_1-1][i_0+1] * E[i_0][i_3][i_2][i_1-1] * E[i_2][i_3][i_0+1][i_2+i_1] * E[i_3-1][i_0][1][i_2] * E[i_1][i_0][i_2-1][i_3] - E[i_2][i_0+1][i_1+1][i_3] - E[i_2-1][i_3][i_3][i_1] * A[i_1-1][i_3+i_1][i_2+1] * 2;
                }
            }
        }
        B[i_0+1] = G[i_0][0] * H[i_0] - H[i_0] - H[i_0] * H[i_0-1] + H[i_0] - B[i_0+2] * B[i_0] * 3;
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
            B[i_5+1] = G[i_5][i_4-1] * G[i_4][i_5+1] - H[i_4-1] * I[i_5][i_4][i_5-1] - 2;
        }
        B[i_4+1] = 3;
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4436151213_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
