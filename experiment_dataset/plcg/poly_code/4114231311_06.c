...
void kernel_4114231311_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M-1; i_1++) {
            for (int i_2 = i_0; i_2 < min(-i_0+PB_L, PB_P-1); i_2++) {
                A[i_1-1][i_0][i_2] = B[i_2][i_1][i_1-1] - C[i_2][0] - B[i_1+1][i_2+i_0][i_0] - A[i_1-1][i_0][i_2+1] - A[i_1-2][i_0+1][i_2+1] - A[i_1-1][i_0+1][i_2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4114231311_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
