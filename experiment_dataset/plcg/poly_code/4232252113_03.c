...
void kernel_4232252113_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 3; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
                A[i_0][i_1-2][i_0-1] = C[i_1][i_2][i_0] + D[i_0+1][i_1-2][i_2] - E[i_2][i_1+i_0][i_2+i_1+1][1] + D[i_1][i_0-2][i_2-1] + F[i_1+1][i_2] + D[i_1][i_0][i_2+2] - F[i_0][i_1+1] - 6;
            }
            for (int i_3 = 0; i_3 < PB_M; i_3++) {
                B[i_3][i_0][i_3+1] = C[i_3][i_0][i_1+1] * D[i_3][i_0][i_1] - D[i_0][i_3][i_1+2] * A[i_0][i_1-3][i_0-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4232252113_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
