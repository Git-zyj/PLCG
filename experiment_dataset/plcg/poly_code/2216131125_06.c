...
void kernel_2216131125_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            A[i_1-1] = C[i_1+i_0][1] + C[i_0+1][i_1] + A[i_1-3] - 6;
        }
        for (int i_2 = 2; i_2 < min(-i_0+PB_M, i_0); i_2++) {
            A[i_2-2] = C[i_2][i_0-1] + D[i_2+i_0] - C[i_0][i_2] - 2;
        }
    }
#pragma endscop
...
kernel_2216131125_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
