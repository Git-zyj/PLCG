...
void kernel_4216111125_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < min(PB_Q, i_0); i_2++) {
            for (int i_3 = i_0; i_3 < -i_2+PB_Q; i_3++) {
                B[i_0][0][i_2] = C[i_0][i_2][i_3+i_2] - B[i_0][0][i_2+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4216111125_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
