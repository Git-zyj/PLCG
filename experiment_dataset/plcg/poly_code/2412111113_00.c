...
void kernel_2412111113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q, PB_L); i_1++) {
            A[i_1+1][i_1+i_0] = E[i_0+1][i_1] * E[i_1][i_0+1] - 6;
        }
        B[i_0+1][i_0] = F[i_0] * 5;
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        ...
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            D[i_2][i_4][i_2+1] = D[i_2+1][i_4-1][i_2+2] * 5;
        }
    }
#pragma endscop
...
kernel_2412111113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
