...
void kernel_4324152315_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 4; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_M-1, -i_2+PB_P+2); i_3++) {
                    A[i_2][i_1][i_3+i_2-2][i_1+1] = D[i_3][i_2][i_0][i_1] + E[i_1][i_3+i_0][i_2] + E[i_1][i_2][i_3] * E[i_3+i_0+1][i_1+2][i_3] + A[i_2-1][i_1][i_3+i_2-2][i_1+1] * A[i_2-1][i_1][i_3+i_2-4][i_1+1] + 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        B[i_4-1] = F[i_4+2] + F[i_4+2] * F[i_4] - F[i_4+2] + F[i_4-1] - F[i_4-1] - G[i_4][i_4+1] * F[i_4] - B[i_4-2] - B[i_4] + B[i_4] + 4;
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_P+2); i_5++) {
            for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
                A[i_4][i_6+1][i_5+i_4-2][i_6+2] = G[i_4][i_6] - G[i_5][i_5+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4324152315_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
