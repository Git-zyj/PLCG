...
void kernel_2314312313_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0][i_0] = D[i_0] * A[i_0-1][i_0-1] * A[i_0+1][i_0+1] - A[i_0-1][i_0-1] - A[i_0-1][i_0-1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        B[i_1] = D[i_1+1] * B[i_1+1] + 3;
        for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_M-1); i_2++) {
            C[i_2+i_1][i_2] = B[i_2] + C[i_2+i_1+1][i_2] * C[i_2+i_1-1][i_2] * 3;
        }
    }
#pragma endscop
...
kernel_2314312313_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
