...
void kernel_4416311223_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = i_1; i_3 < PB_L; i_3++) {
                    A[i_0][i_3][i_1][i_1] = E[i_3+1][i_2][i_1+1][i_0+1] * A[i_0+2][i_3][i_1][i_1] - 5;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_M-1; i_4++) {
        B[i_4-1][i_4] = F[i_4] - B[i_4-2][i_4-1] * B[i_4-3][i_4-2] - 1;
        for (int i_5 = 0; i_5 < min(i_4, PB_L-1); i_5++) {
            for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
                A[i_4][i_5+1][i_6][i_6] = G[i_6][i_4-1][i_6+1] - 4;
            }
        }
        for (int i_7 = 0; i_7 < PB_M; i_7++) {
            B[i_4][i_4+1] = H[i_7][i_4][0] * 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4416311223_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
