...
void kernel_4322312111_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_N-2; i_2++) {
                A[i_2+i_1+1][i_2] = A[i_2+i_1+2][i_2] * 3;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 1; i_4 < min(i_0, -i_3+PB_M+2); i_4++) {
                B[i_4][i_0-2] = D[i_4+i_3-2][i_0][i_0+1] + B[i_3+1][i_4-1] * B[i_3-1][i_0-2] - 6;
            }
        }
    }
...
kernel_4322312111_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
