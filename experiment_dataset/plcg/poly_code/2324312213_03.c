...
void kernel_2324312213_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-1; i_1++) {
            B[i_1+i_0-1][i_1+i_0+1] = D[i_1+i_0+1][i_1] * C[i_0-1] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M-2; i_3++) {
            B[i_3+i_2][i_3+i_2+2] = B[i_3+i_2][i_3+i_2+2] - 2;
        }
    }
...
kernel_2324312213_03(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
