...
void kernel_4232231311_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_M-1); i_2++) {
                A[i_2][i_0][i_1+1] = C[i_2+i_1][i_0-1][1] + C[i_0][i_2][i_0] - A[i_2+1][i_0][i_1] - A[i_2][i_0+1][i_1+1] - A[i_2+1][i_0][i_1+1] - A[i_2][i_0][i_1] * 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_L+1, PB_M-2); i_4++) {
            B[i_4+1] = D[i_4][i_4+1] * D[i_4+i_3-1][i_3+1] + E[i_3-1][i_4] - E[i_4][i_3+1] + B[i_4+2] * 3;
        }
    }
#pragma endscop
...
kernel_4232231311_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
