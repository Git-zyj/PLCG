...
void kernel_3134231321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_0+PB_M, PB_P+1), -i_1+PB_M-2); i_2++) {
                A[i_2+i_1+1][0] = B[i_1][i_2][i_0] + C[i_0][i_1] + D[0][i_1+i_0][i_2-1] + A[i_2+i_0][0] - A[i_2+i_1+2][0] * A[i_2+i_1+2][0] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3134231321_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
