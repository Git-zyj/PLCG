...
void kernel_2212252211_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_M-3, -i_0+PB_L); i_1++) {
            A[i_0][i_1+2] = C[i_0][i_1] * C[i_1+2][i_1+i_0] * D[i_1][i_0-1] - A[i_0-1][i_1+2] + A[i_0][i_1+3] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_L, PB_P); i_3++) {
            A[i_3][i_2+3] = C[i_2][i_3-1] * C[i_3+2][i_2+2] - D[i_3][i_2-1] - D[i_3+i_2][1] * C[i_3][i_2] - C[i_3][i_3+i_2] * 3;
        }
    }
#pragma endscop
...
kernel_2212252211_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
