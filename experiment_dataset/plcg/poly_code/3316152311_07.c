...
void kernel_3316152311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        A[i_0] = D[i_0] * D[i_0] - D[i_0] * D[i_0] - A[i_0-1] + C[i_0] + 3;
        C[i_0] = D[i_0] - D[i_0-2] * D[0] + A[i_0-1] * C[i_0] * 5;
        for (int i_1 = 0; i_1 < min(PB_N, -i_0+PB_M-2); i_1++) {
            C[i_1] = E[i_1+i_0+2][i_1+i_0][2] + F[i_0][i_1] - F[i_1][i_0] * F[i_1][i_0] + F[i_0+2][i_0] * E[i_0][i_1][i_0] * A[i_0] - 2;
        }
    }
#pragma endscop
...
kernel_3316152311_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
