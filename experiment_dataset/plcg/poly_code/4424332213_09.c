...
void kernel_4424332213_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0+1] - F[i_0] * F[i_0-2] + E[i_0] - 2;
        for (int i_1 = 2; i_1 < -i_0+PB_L+1; i_1++) {
            B[i_0][i_1+i_0-2] = G[i_1][i_0] - H[i_0][i_1] + E[i_1] - I[i_1][i_1][i_0] - F[i_1-2] - B[i_0+1][i_1+i_0-2] + B[i_0+1][i_1+i_0-1] + 6;
        }
        A[i_0] = F[i_0] - A[i_0+1] * A[i_0] + 2;
    }
    for (int i_2 = 1; i_2 < PB_Q; i_2++) {
        for (int i_3 = 3; i_3 < PB_L+2; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_M-2, i_3); i_4++) {
                for (int i_5 = 2; i_5 < PB_Q-1; i_5++) {
                    D[i_4+2][i_5-1][i_2][i_3-2] = D[i_4+2][i_3-2][i_2-1][i_5-2] * D[i_4+2][i_2-1][i_5+1][i_3-3] + 3;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4424332213_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
