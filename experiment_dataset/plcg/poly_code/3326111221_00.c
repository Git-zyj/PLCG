...
void kernel_3326111221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_L-1; i_1++) {
            A[i_1-1][i_1+1] = A[i_1-3][i_1-1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_M-1); i_3++) {
            for (int i_4 = 0; i_4 < PB_P-3; i_4++) {
                B[i_3+i_2][1][i_4+1] = B[i_3+i_2+1][1][i_4+3] - 5;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_L; i_5++) {
        for (int i_6 = i_5; i_6 < PB_L-2; i_6++) {
            for (int i_7 = 0; i_7 < -i_5+PB_M+1; i_7++) {
                C[i_7+i_5-1][i_6][i_5] = D[i_5][i_7][i_5] - C[i_7+i_5-2][i_6+1][i_5+1] + C[i_7+i_5-1][i_6+2][i_5] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3326111221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
