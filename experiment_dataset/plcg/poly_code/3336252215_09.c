...
void kernel_3336252215_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        B[i_0-3] = D[i_0+1] - D[0] + E[i_0-1] - E[i_0-1] - 4;
        for (int i_1 = 1; i_1 < min(PB_L, -i_0+PB_N); i_1++) {
            B[i_0-2] = E[i_0-2] * E[i_1+i_0] + F[i_0][i_1] * F[i_0+2][i_1] * F[i_0][i_1] - G[i_1][i_0] + F[i_1][i_0-1] + B[i_1-1] * B[i_0-3] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_L-1; i_3++) {
            for (int i_4 = i_3; i_4 < PB_P; i_4++) {
                C[i_2][i_3][i_4] = H[i_4][i_2][i_3-2] + H[i_3][i_2-1][i_4] * I[i_4][i_3-1][i_2] * C[i_2+1][i_3+1][i_4] * C[i_3+1][i_4-1][i_2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3336252215_09(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
