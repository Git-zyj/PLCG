...
void kernel_3234212121_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_2 = 4; i_2 < PB_M+2; i_2++) {
        for (int i_3 = 2; i_3 < PB_P; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_2+PB_L+1, i_3); i_4++) {
                B[i_4][i_3][i_3-2] = D[i_4][i_4+i_2-1][i_3] - B[i_4-1][i_2-2][i_2-4] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3234212121_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
