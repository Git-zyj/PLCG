...
void kernel_2322251323_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_M-1); i_1++) {
            B[i_1] = D[i_0-1] * E[i_0-1][i_0] + D[i_0] - E[i_1+1][i_0] - E[i_0][i_1+i_0] + B[i_1] - 5;
        }
        B[i_0] = D[0] + D[i_0] * F[i_0+1] + D[i_0] + F[i_0] * F[i_0-1] - B[i_0-2] * 2;
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        B[i_2+2] = F[i_2+1] * F[i_2-1] - D[i_2] + D[i_2+1] * B[i_2+1] - 5;
    }
#pragma endscop
...
kernel_2322251323_04(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
