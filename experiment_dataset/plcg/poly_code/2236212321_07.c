...
void kernel_2236212321_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_M-1); i_1++) {
            A[i_0-1] = A[i_0-2] + A[i_1+1] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        A[i_2-1] = C[i_2] + C[i_2-1] + A[i_2+1] + 4;
    }
#pragma endscop
...
kernel_2236212321_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
