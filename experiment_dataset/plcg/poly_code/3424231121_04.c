...
void kernel_3424231121_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-1] + E[i_0-1] + 3;
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_0] = F[i_0+1][i_1] + E[i_1-1] * F[i_1][i_0] + F[i_0-1][i_1] * 6;
        }
        for (int i_2 = 1; i_2 < min(i_0, PB_L-1); i_2++) {
            for (int i_3 = 1; i_3 < min(PB_P, i_0); i_3++) {
                D[i_0+1][i_2+1][i_3] = F[i_3-1][i_2-1] - 3;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P; i_4++) {
        for (int i_5 = 0; i_5 < min(PB_P-1, -i_4+PB_L); i_5++) {
            for (int i_6 = 0; i_6 < PB_P; i_6++) {
                D[i_4+1][i_5][i_6] = G[i_6+1][i_4][i_5+1] * H[i_4-1][i_6][i_4] * G[i_6+i_5][i_5+i_4][0] - D[i_4+2][i_5][i_6] * D[i_4][i_5][i_6] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3424231121_04(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
