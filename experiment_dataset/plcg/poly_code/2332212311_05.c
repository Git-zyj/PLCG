...
void kernel_2332212311_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_1] = D[i_0+1][i_1] + A[i_1+1] * C[i_0+2] - C[i_0+3] + 3;
        }
    }
    ...
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-3; i_4++) {
            C[i_4+2] = C[i_4+1] * C[i_4+3] - C[i_4+3] - 2;
        }
    }
#pragma endscop
...
kernel_2332212311_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
