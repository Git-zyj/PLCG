...
void kernel_3312351225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = D[i_0+1] - D[i_0] * D[i_0] - E[i_0] - D[i_0] - D[i_0] - D[i_0] - E[i_0-1] - A[i_0-2] * A[i_0-1] * 5;
        for (int i_1 = 1; i_1 < -i_0+PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_2][i_1+i_0+1][i_1+i_0] = F[i_1][i_2][0] + G[i_1-1][i_2] * F[i_1][i_2][i_0] * H[i_0][i_1][i_2] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        C[i_3] = I[i_3-1] + A[i_3] + C[i_3+2] * 2;
    }
#pragma endscop
...
kernel_3312351225_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
