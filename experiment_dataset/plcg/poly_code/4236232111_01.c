...
void kernel_4236232111_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                A[i_0][i_1][i_2] = C[i_2][i_0][i_1-1] + A[i_0][i_1][i_2+1] - 6;
            }
        }
        B[i_0+2] = D[i_0] - E[i_0-1] - D[i_0-2] + E[i_0] * 2;
    }
#pragma endscop
...
kernel_4236232111_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
