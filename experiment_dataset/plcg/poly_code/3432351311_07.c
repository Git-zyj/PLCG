...
void kernel_3432351311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = E[i_0] - F[i_0] + F[i_0+1] * F[i_0+1] - F[i_0] + A[i_0] - A[i_0-2] + A[i_0] - 6;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_P); i_2++) {
            B[i_1+1][i_1] = G[i_2+1][i_1] + F[i_2] - H[i_2][0] + I[i_2][i_2] * G[i_1][i_2] - I[i_2+i_1][i_2] * B[i_1][i_1-1] * D[i_1][i_2-1] + D[i_2][i_1] - 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
        C[i_3] = F[i_3] * J[i_3] * H[i_3-1][i_3-1] - E[i_3] * E[i_3+1] + F[i_3] * J[i_3] + C[i_3-1] * 1;
        for (int i_4 = 1; i_4 < min(-i_3+PB_M, PB_P); i_4++) {
            D[i_3][i_4-1] = I[i_4+i_3][i_4] + G[i_3][i_4] * A[i_4-1] - C[i_4+1] * 1;
        }
    }
#pragma endscop
...
kernel_3432351311_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
