...
void kernel_4224312311_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < -i_0+PB_M+2; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
                A[i_2-2][i_1+i_0-2][0] = C[i_0][i_2+1][i_1] * A[i_0-1][i_2+i_1-3][0] * A[i_2-1][i_1+i_0-2][0] - A[i_2-1][i_1+i_0-4][0] - 1;
            }
        }
        B[i_0-2] = B[i_0-1] - B[i_0-3] * B[i_0-3] * 3;
    }
#pragma endscop
...
kernel_4224312311_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
