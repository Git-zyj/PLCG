...
void kernel_3236212311_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-3; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                A[i_0][i_1+1][i_0+2] = A[i_1+1][i_2+1][i_1+3] - A[i_2][i_1][i_2+2] * A[i_0][i_2][i_0+2] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        B[i_3] = C[i_3+1] + B[i_3-1] * B[i_3+1] * B[i_3-1] + 5;
    }
#pragma endscop
...
kernel_3236212311_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
