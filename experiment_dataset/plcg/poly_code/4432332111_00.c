...
void kernel_4432332111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
            A[i_0][i_1] = E[i_0][i_1+1] * F[i_1+2][i_0] - G[i_0][i_1] + A[i_0][i_1+1] - 3;
        }
        for (int i_2 = 0; i_2 < PB_Q-2; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_0+PB_L+1, PB_M-2); i_3++) {
                B[i_3+2][i_2+1][i_3+i_0-1] = H[i_3][i_0][i_2+2] * 4;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 0; i_5 < -i_4+PB_M; i_5++) {
            for (int i_6 = 2; i_6 < PB_M; i_6++) {
                C[i_5][i_4+2] = I[i_6][i_5][i_4] + E[i_4][i_5] * J[i_5+i_4-2][i_6][1] - E[i_5+i_4][i_6-2] + C[i_5][i_4+1] + 6;
            }
        }
    }
        ...
        for (int i_8 = 0; i_8 < -i_7+PB_P-2; i_8++) {
            for (int i_9 = 0; i_9 < PB_M; i_9++) {
                for (int j_0 = 0; j_0 < PB_M; j_0++) {
                    D[i_9][i_8+i_7][i_8][j_0] = K[j_0][i_8+2][j_0][i_7] - K[i_9][i_8+i_7+2][j_0][i_9] + L[j_0][i_7][i_8][i_9] - 4;
                }
            }
        }
    }
