...
void kernel_4414252325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-5; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_1+1] = E[i_1] * F[i_1][i_0-1] - F[i_1][i_0] + A[i_1][i_0] + A[i_0+2][i_1+1] * A[i_0-2][i_1+1] + 3;
        }
        for (int i_2 = i_0; i_2 < -i_0+PB_N-5; i_2++) {
            for (int i_3 = 1; i_3 < -i_0+PB_P; i_3++) {
                for (int i_4 = 2; i_4 < min(min(-i_3+PB_M, -i_2+PB_L), PB_L-1); i_4++) {
                    B[i_2+i_0+1][i_4][1][i_4] = G[i_4+1][i_3][i_0+2][i_3+i_0] + G[i_4+i_3][i_0][2][i_2] * G[i_4][i_0+1][i_2-2][i_3+1] * G[i_4-2][i_3][i_4+i_2][2] - G[i_2][i_3][i_4+1][i_0] - H[i_2][i_0][i_4][i_3-1] + B[i_2+i_0+5][i_4][1][i_4] - B[i_2+i_0+3][i_4][1][i_4] + 5;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_N-2; i_5++) {
        for (int i_6 = 2; i_6 < min(PB_M-3, -i_5+PB_L); i_6++) {
            A[i_5][i_6] = F[i_6][i_6] * I[i_6-1][i_5+2] + I[i_6+2][i_6+i_5] - F[i_6+1][i_5] - J[i_5-1] + 2;
            A[i_5][i_6+3] = I[i_6-2][i_5] - J[i_6] * F[i_5+2][0] * 2;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4414252325_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
