...
void kernel_4316111125_05(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_L+1); i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                B[i_2-1][i_1-1][i_0][i_1] = D[i_0][i_1][i_2] + 3;
            }
            for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
                for (int i_4 = 0; i_4 < PB_P; i_4++) {
                    B[i_0-1][i_3-1][i_4][i_3] = B[i_0][i_1-1][i_4][i_1] + 6;
                }
            }
            C[i_1+1][i_0][i_1+i_0-1] = 1;
        }
    }
#pragma endscop
...
kernel_4316111125_05(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
