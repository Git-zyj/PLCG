...
void kernel_4336132225_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] * D[i_0] - D[i_0-2] * D[i_0+1] - D[0] - E[i_0][i_0] + A[i_0+2] + A[i_0-2] + 1;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 4; i_2 < min(i_1, PB_L+1); i_2++) {
            B[i_2-2][i_1] = D[i_1] - B[i_2-1][i_1] + B[i_2-4][i_1] * 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            B[i_3-2][i_4] = E[i_3][i_4] * 2;
        }
    }
#pragma endscop
...
kernel_4336132225_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
