...
void kernel_2224351325_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            A[0][i_1+1] = C[i_1][i_1] * D[i_1][i_0] + D[i_0][i_1+1] * E[i_1] + D[i_1][i_1] + C[i_1][i_0] - A[0][i_1] * A[0][i_1-1] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_N; i_3++) {
            B[i_3][i_3-1] = E[i_3] + F[i_2+1][i_3] * D[i_2+1][i_3] - C[i_3-1][i_2] * A[0][i_3+1] + A[0][i_3+2] * B[i_3+1][i_3] - B[i_3-1][i_3-2] * 3;
        }
    }
#pragma endscop
...
kernel_2224351325_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
