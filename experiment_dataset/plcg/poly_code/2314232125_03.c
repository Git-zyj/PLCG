...
void kernel_2314232125_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_M+2; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M+2; i_2++) {
            C[i_2+i_1-2] = E[i_2][i_1+1] * 3;
        }
        for (int i_3 = 0; i_3 < -i_1+PB_M+2; i_3++) {
            C[i_3+i_1-2] = E[i_1-2][i_3] + E[i_3][i_1-1] * D[i_3] * E[i_3][i_1] + C[i_3+i_1-3] * C[i_3+i_1-3] - 2;
        }
    }
#pragma endscop
...
kernel_2314232125_03(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
