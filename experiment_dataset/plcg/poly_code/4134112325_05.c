...
void kernel_4134112325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-2); i_1++) {
            A[i_1] = B[0] + A[i_0-2] - A[i_1+2] * A[i_1+1] - 6;
        }
    }
#pragma endscop
...
kernel_4134112325_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
