...
void kernel_4216311211_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                B[i_0][i_4][i_4] = C[i_5-1][i_4-1] - B[i_4+1][i_5][i_5] - B[i_4+1][i_0+1][i_0+1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4216311211_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
