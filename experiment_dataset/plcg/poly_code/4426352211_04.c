...
void kernel_4426352211_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,int vm,int wm,DATA_TYPE POLYBENCH_5D(M,xM,yM,zM,vM,wM,xm,ym,zm,vm,wm),int xn,int yn,int zn,int vn,int wn,DATA_TYPE POLYBENCH_5D(N,xN,yN,zN,vN,wN,xn,yn,zn,vn,wn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0][i_1] = E[i_0-2][i_1+i_0] * F[i_1][i_0] + G[i_1-2] - F[i_1][i_0] + H[i_0][i_1+2] + C[i_0][i_1-1] * 1;
        }
        for (int i_2 = i_0; i_2 < PB_N-2; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_0+PB_L, i_0); i_3++) {
                for (int i_4 = 1; i_4 < min(-i_0+PB_Q+2, PB_M-1); i_4++) {
                    B[i_2][i_4][i_3][i_4] = I[i_2+2][i_3][i_0-1][i_4+i_0-2] - J[i_2][i_4][i_0][i_3] + J[i_4+1][i_3+i_0][i_3][i_2-1] * K[i_4][i_0][i_2][i_2] - B[i_2][i_0+1][i_4+1][i_0+1] - B[i_2][i_0][i_3+1][i_0] * B[i_3+1][i_4-1][i_0][i_4-1] * 3;
                }
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_N; i_5++) {
        for (int i_6 = i_5; i_6 < PB_M; i_6++) {
            C[i_5][i_6] = F[i_5-2][i_6] * E[i_6][i_5] + 6;
        }
    }
    for (int i_7 = 1; i_7 < PB_L-2; i_7++) {
        for (int i_8 = 2; i_8 < i_7; i_8++) {
            for (int i_9 = 0; i_9 < PB_M-2; i_9++) {
                for (int j_0 = 2; j_0 < min(-i_8+PB_P+1, PB_Q-2); j_0++) {
                    D[i_8][i_7+1][i_9][j_0+2] = J[i_7][i_8-1][j_0-1][i_8] + J[j_0][i_7][i_9][i_8] + L[i_9][i_8][i_9] + J[i_7][i_9][j_0][i_8+2] + L[i_7-1][j_0+i_8-1][0] + J[i_8-1][i_9][j_0][i_7] * J[i_9+2][i_8-1][j_0-2][i_7] + M[i_7-1][i_8-2][j_0][i_9][0] + N[j_0][i_8][i_9][i_7][i_7] + D[j_0][i_8][i_9][i_7+2] * D[i_8+1][i_7+2][i_9][j_0+1] - D[i_8-1][i_7][i_9][j_0+1] + 6;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,wn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4426352211_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,wn,POLYBENCH_ARRAY(N));
