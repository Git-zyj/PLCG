...
void kernel_2336112313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_M-1); i_1++) {
            A[i_1] = D[i_1][i_0+2] + A[i_1-1] - A[i_1+1] * A[i_1+1] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-3; i_3++) {
            C[i_3+1] = D[i_2+1][i_3] - C[i_3+2] - C[i_3+3] - 2;
        }
        C[i_2+2] = A[i_2] - 4;
    }
#pragma endscop
...
kernel_2336112313_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
