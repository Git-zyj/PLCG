...
void kernel_2224311115_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = C[i_0] - 3;
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            B[i_1-1][i_0][1] = B[i_1-1][i_0+1][1] - 1;
        }
    }
#pragma endscop
...
kernel_2224311115_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
