...
void kernel_2434352223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0-2][i_1+1] = E[i_1+1] - F[i_1][i_0-2] + G[i_1][i_0] + G[i_1+i_0][i_0] * H[i_1][i_0] + G[i_1][i_0] + A[i_0-1][i_1+1] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-3; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            B[i_2][i_3] = F[i_3][i_2] - H[i_3-2][i_2+2] - H[i_2][i_3] * C[i_3] + 4;
        }
        C[i_2] = E[i_2] * E[i_2] * C[i_2+2] - C[i_2-2] + D[i_2+3] * 1;
    }
    for (int i_4 = 2; i_4 < PB_N-2; i_4++) {
        D[i_4+2] = I[i_4] + F[i_4][i_4] - J[i_4-2] * J[i_4+2] - J[0] * J[i_4] * D[i_4] - 5;
    }
#pragma endscop
...
kernel_2434352223_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
