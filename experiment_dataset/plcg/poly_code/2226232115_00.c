...
void kernel_2226232115_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = C[i_0] + C[i_0-1] + D[i_0][i_0] + A[i_0+1] - 1;
        for (int i_1 = min(i_0, PB_M-1) - 1; i_1 >= 0; i_1--) {
            A[i_1] = C[i_1+1] * E[i_1][i_0+1] * 1;
        }
    }
#pragma endscop
...
kernel_2226232115_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
