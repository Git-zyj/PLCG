...
void kernel_4232152113_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                B[i_1][i_2-2][i_0-1] = C[i_2][i_0][i_1] + C[i_2-1][i_0-2][0] - C[i_2][i_0][i_1-1] + C[i_1][i_0][i_2] * C[i_1][i_2][i_0+1] - C[i_1-2][i_2][i_1+i_0] - C[i_1][i_0][i_2] * 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                B[i_5][i_3-2][i_4] = C[i_5][i_3][i_4] + C[i_4][i_3+1][i_3] + B[i_5][i_3-2][i_4-1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4232152113_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
