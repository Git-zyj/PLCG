...
void kernel_4434351325_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_1+2][i_0+1] = E[i_1] - F[i_1][i_0] * F[i_0][i_1] + E[i_0] * F[i_0-1][i_1] * G[i_1][i_0] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 2; i_3 < min(-i_2+PB_L-1, PB_M-1); i_3++) {
            for (int i_4 = max(2, i_2); i_4 < -i_3+PB_M-1; i_4++) {
                B[i_3][i_4] = H[i_2][i_4+1][i_3] - F[i_4+i_3+1][i_3+i_2+1] * I[i_4+i_3-1][i_2][i_2-1] + F[i_4+i_3-1][i_3] + B[i_3-1][i_4+1] - B[i_4-2][i_2+2] + B[i_3-2][i_4-2] + 3;
            }
        }
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 0; i_6 < PB_M; i_6++) {
                B[i_6][i_2-1] = H[i_6][0][i_5+1] * I[i_5-1][i_5+1][i_5] + J[i_5+1][i_2][i_6] * K[i_5][i_6][i_2][i_5-1] - 5;
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_M; i_7++) {
        for (int i_8 = 1; i_8 < min(i_7, PB_M-1); i_8++) {
            for (int i_9 = 0; i_9 < PB_M-1; i_9++) {
                B[i_7][i_9] = J[i_8][i_9][i_7] * I[i_8+1][i_7+1][i_9+1] * F[i_9+1][i_8-1] - J[i_8][i_9+1][i_7] + J[i_8+1][1][i_9] * I[i_8][i_7+1][i_9] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4434351325_03(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K)));
