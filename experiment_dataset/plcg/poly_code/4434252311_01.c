...
void kernel_4434252311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        A[i_0+2] = E[i_0] - F[i_0] - F[i_0] + F[i_0] * F[i_0] - F[i_0] * A[i_0+3] * A[i_0+1] + A[i_0+3] - A[i_0+3] * 1;
        for (int i_1 = max(1, i_0); i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_0+PB_Q-2, -i_1+PB_M-2); i_3++) {
                    B[i_2][i_3+i_1-1][i_0+1][i_3] = G[i_3-1][i_2][i_0][i_1-1] * H[i_2][i_1][i_0][i_3+i_0+2] - G[i_3+i_1+2][i_0][i_2][0] - B[i_2][i_3+i_1+1][i_0+1][i_3+1] * 5;
                }
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < -i_4+PB_Q; i_5++) {
            for (int i_6 = 0; i_6 < PB_Q-2; i_6++) {
                for (int i_7 = 2; i_7 < -i_6+PB_M; i_7++) {
                    C[i_4][i_7-1][i_6][i_7] = G[i_7+i_6][i_7-1][i_5][i_4] * H[i_6][i_7][i_5+i_4][i_6+2] + H[i_5-2][i_7][i_4][i_6] + G[i_5][i_6][i_4][i_7-2] * C[i_7][i_5-2][i_6][i_5-1] + 6;
                }
            }
        }
        for (int i_8 = 1; i_8 < PB_N-2; i_8++) {
            for (int i_9 = i_8; i_9 < min(-i_8+PB_M, -i_4+PB_P); i_9++) {
                C[i_4][i_9-1][i_8][i_9] = I[i_9+i_8][i_4][0] - J[i_8+2][i_9+2] + I[i_9+i_8-1][i_4+1][0] + K[i_8][i_4][i_9] - L[i_8][i_9+i_4] - C[i_4-1][i_9-1][i_8][i_9] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4434252311_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
