...
void kernel_4214352213_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_Q, PB_P-1); i_2++) {
                A[i_0+2][i_1][i_2] = C[i_2][i_0][i_1-1] * D[i_2][i_1+i_0][i_2][i_1] + E[i_0][i_0][i_2][i_1+2] * C[i_2][i_0][i_2] + F[i_0][i_2][i_1-1] + F[i_1][i_2+i_0][0] * A[i_1+2][i_0-1][i_2-1] * A[i_1+2][i_2-1][i_0-1] * 6;
                B[i_0][i_0][i_2] = G[i_2-1][i_1] + C[i_0][i_1+1][i_2] - F[i_0][i_2][i_1-1] + A[i_0+1][i_1][i_2+1] + A[i_0+2][i_1][i_2-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4214352213_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
