...
void kernel_3224112321_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M; i_2++) {
                A[i_1][i_1] = C[i_2][i_1] + A[i_1-1][i_1-1] - A[i_1-2][i_1-2] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_L, -i_3+PB_N-2); i_4++) {
            B[i_4+i_3] = C[i_4+i_3][2] - A[i_4][i_4] * B[i_4+i_3+1] + B[i_4+i_3-1] + B[i_4+i_3+2] * 6;
        }
    }
#pragma endscop
...
kernel_3224112321_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
