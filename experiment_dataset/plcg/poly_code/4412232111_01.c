...
void kernel_4412232111_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < PB_L; i_3++) {
                    A[i_0][i_2][i_3-1][i_1] = E[i_0][i_1][i_3-1][i_2] - F[i_2][i_3][i_1+1] + G[i_1][i_2-1][i_0][i_3] * A[i_2-1][i_0+1][i_3-1][i_1] - 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 1; i_5 < min(-i_4+PB_N-2, PB_M-2); i_5++) {
            for (int i_6 = 2; i_6 < PB_L; i_6++) {
                for (int i_7 = 1; i_7 < PB_L; i_7++) {
                    B[i_6][i_5+i_4+2][i_5+1][i_7] = G[i_7][i_6-2][i_5+2][i_4+2] + H[i_5][i_6-1][i_7][i_4+2][i_6+i_5-1] - B[i_7-1][i_5+i_4+2][i_5+1][i_6] - 5;
                }
            }
            for (int i_8 = max(i_4, 2); i_8 < -i_4+PB_N-1; i_8++) {
                C[i_4][i_5+i_4+2][i_8] = I[i_5+i_4][i_8-2][i_5] * C[i_4][i_8+i_4+1][i_5] - 2;
            }
        }
        D[i_4] = J[i_4] * K[i_4][i_4+1] * L[i_4+2] + 3;
    }
#pragma endscop
...
kernel_4412232111_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
