...
void kernel_4436331121_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_0+1][i_1] = E[i_1] * F[i_1][i_0] + E[i_0] * A[i_0+1][i_1-1] - 1;
        }
    }
        ...
        for (int i_3 = 0; i_3 < min(PB_Q-1, -i_2+PB_M); i_3++) {
            for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
                for (int i_5 = 0; i_5 < PB_P; i_5++) {
                    B[i_5][i_2-1][i_4+1][i_3+1] = G[i_3][i_5][i_4+1] + H[i_2][i_5][i_3][i_4] * I[i_3+i_2][i_4][i_4][i_5][i_2] - J[i_3][i_4][i_2][i_2] * 6;
                }
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 1; i_7 < i_6; i_7++) {
            for (int i_8 = 0; i_8 < PB_M; i_8++) {
                C[i_7][i_8] = K[i_6][i_7-1][i_8] + 3;
            }
        }
        for (int i_9 = 0; i_9 < i_6; i_9++) {
            for (int j_0 = 1; j_0 < min(i_9, -i_9+PB_L); j_0++) {
                D[j_0][i_6][j_0+i_9] = K[i_9][i_6+1][j_0-1] * L[i_9][i_6] - D[j_0][i_6-1][j_0+i_9] + 4;
            }
        }
    }
#pragma endscop
