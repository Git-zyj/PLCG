...
void kernel_2234152325_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-3; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_P-3, PB_L-1); i_1++) {
            B[i_1+1][i_1+i_0+3] = C[i_1] - D[i_0][i_1] * D[i_1+1][i_0+2] * D[i_0][i_1+1] - D[i_1][i_0+1] - D[i_0][i_1+1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-4; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_L, -i_2+PB_P-4); i_3++) {
            B[i_3][i_3+i_2+2] = D[i_3][i_3] - D[i_3-1][i_3+i_2] - C[i_3] * B[i_3][i_3+i_2+1] + B[i_3][i_3+i_2+3] + B[i_3][i_3+i_2+4] * 2;
        }
    }
#pragma endscop
...
kernel_2234152325_02(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
