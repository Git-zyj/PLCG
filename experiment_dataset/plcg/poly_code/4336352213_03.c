...
void kernel_4336352213_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] * E[i_0] + F[i_0-1] * E[2] * G[i_0][i_0] - F[i_0] * D[i_0] + 4;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_L; i_2++) {
            B[i_2][i_1] = H[i_2-1][i_1] + I[i_1][0] - D[i_1] + G[i_2][i_2+i_1] * B[i_2][i_1-1] * B[i_2+1][i_1] + B[i_2][i_1+1] * 4;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        A[i_3] = D[i_3] * F[i_3] - D[i_3] - 6;
    }
#pragma endscop
...
kernel_4336352213_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
