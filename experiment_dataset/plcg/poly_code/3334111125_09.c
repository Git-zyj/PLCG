...
void kernel_3334111125_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0][i_1][i_0] = D[i_0] - A[i_1+i_0-1][i_1-1][i_0] * 2;
        }
        B[i_0-1] = D[i_0-1] * 5;
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_N; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                C[i_3][i_4][i_2] = C[i_3-2][i_4][i_2] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3334111125_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
