...
void kernel_4222131323_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
        ...
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(PB_Q-2, -i_2+PB_L); i_3++) {
                    A[i_0][i_1][i_1][i_2] = C[i_2][i_1+1][i_0+1] - C[i_3+i_2][i_1][i_0] - A[i_0-1][i_3][i_3][i_2] * A[i_0][i_2][i_2][i_3+2] + A[i_1][i_0-2][i_0-2][i_2] - 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = PB_M-1 - 1; i_5 >= 3; i_5--) {
            B[i_4][i_5-1] = D[i_4-1][i_5] * D[i_4][i_4] - D[i_5+1][0] + B[i_4+2][i_5-2] * B[i_4+2][i_5-1] - B[i_4][i_5-3] + 5;
        }
    }
#pragma endscop
...
kernel_4222131323_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
