...
void kernel_3224212223_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
        for (int i_4 = max(3, i_3); i_4 < min(-i_3+PB_M+1, PB_L+3); i_4++) {
            for (int i_5 = 1; i_5 < PB_L+1; i_5++) {
                B[i_3-2][i_4+1] = C[i_4-1][i_5][i_4+i_3-1] - B[i_4-3][i_5+1] - B[i_5-1][i_4+2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3224212223_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
