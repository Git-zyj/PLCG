...
void kernel_3336131313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_N); i_1++) {
            A[i_1+i_0-1][i_0] = D[i_1][i_0] * A[i_1+i_0-3][i_0-1] + A[i_1+i_0][i_0] - A[i_1+i_0-2][i_0] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_M-1; i_3++) {
            B[i_3+i_2][i_3-1] = D[i_2][i_3+1] * D[i_3-1][i_2-1] * D[i_2][i_3+1] * E[i_3+i_2] * B[i_3+i_2+1][i_3-1] - B[i_3+i_2-1][i_3-2] * B[i_3+i_2+1][i_3] + B[i_3+i_2-1][i_3-1] - B[i_3+i_2][i_3-2] - 1;
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_M-1); i_5++) {
            C[i_4][i_5] = D[i_5][i_4+1] - E[0] - E[i_5+1] - C[i_4][i_5-1] * 6;
        }
    }
#pragma endscop
...
kernel_3336131313_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
