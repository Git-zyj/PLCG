...
void kernel_2236132115_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P, i_0); i_1++) {
            B[i_0][i_1] = C[i_1][i_0] + C[1][i_0-1] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            B[i_2][i_3] = C[i_3-1][i_2] + C[i_3+i_2][i_3] - C[i_3][i_2] * C[i_3][i_2] - B[i_3+1][i_2] - 3;
        }
    }
#pragma endscop
...
kernel_2236132115_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
