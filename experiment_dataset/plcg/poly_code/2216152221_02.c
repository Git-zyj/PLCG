...
void kernel_2216152221_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-4; i_0++) {
        for (int i_1 = 2; i_1 < PB_N; i_1++) {
            A[i_1][i_0+2] = C[i_1][i_0-2] * C[i_1][i_1] * C[i_1-1][i_0] - C[i_1-2][i_0-2] - A[i_1-2][i_0+2] * A[i_1][i_0+4] - A[i_1-1][i_0+1] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_N-1, i_2); i_3++) {
            B[i_2][i_3] = C[i_3][i_2] - C[i_3][i_2] + C[i_3+1][i_2] * C[i_2][i_3] + C[i_3-2][i_2+2] + 2;
        }
    }
#pragma endscop
...
kernel_2216152221_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
