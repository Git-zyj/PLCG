...
void kernel_3222231321_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1+1] = C[i_1][i_0+1] * D[i_1][i_0] - D[i_1][i_0-1] * C[i_1][i_0] * C[i_1-1][i_0] + A[i_1] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            B[i_2-1][i_3] = D[i_3-1][i_2] - A[i_3+1] * A[i_3+2] * B[i_2-1][i_3-1] - B[i_2-1][i_3+1] * B[i_2][i_3+2] * 1;
        }
    }
#pragma endscop
...
kernel_3222231321_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
