...
void kernel_2416352125_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] * F[i_0] * F[i_0-1] - E[i_0] - F[i_0] + F[i_0] - E[i_0] * E[i_0-1] - A[i_0+1] + 1;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M; i_2++) {
            C[i_1] = G[i_2][i_1+1] * H[i_2][i_1+1] + 5;
        }
        C[i_1] = E[i_1] + F[i_1] + E[i_1] - I[i_1+1] - F[i_1] * A[i_1] - 1;
        C[i_1+2] = F[i_1] + F[i_1+1] - F[0] + F[i_1+2] - E[i_1] + 4;
    }
#pragma endscop
...
kernel_2416352125_06(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
