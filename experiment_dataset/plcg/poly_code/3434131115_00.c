...
void kernel_3434131115_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = i_1; i_2 < -i_0+PB_P-1; i_2++) {
                A[i_0][i_2+i_0][i_1] = E[i_1][i_0][i_2+i_0+1] * 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            B[i_3][i_4] = F[1] + G[i_4][i_3] + F[i_4] + B[i_3+1][i_4-1] + 4;
        }
        for (int i_5 = 1; i_5 < -i_3+PB_L+1; i_5++) {
            for (int i_6 = 1; i_6 < -i_3+PB_L; i_6++) {
                C[i_6][i_3][i_5+i_3-1] = E[i_3+1][i_5-1][i_6-1] - E[i_6][i_3][i_6] * E[i_3-1][i_6][i_5] * E[i_6+1][i_3-1][i_5] + G[i_5][i_6] * C[i_5][i_3+1][i_6+i_3] * 4;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_L; i_7++) {
        for (int i_8 = i_7; i_8 < -i_7+PB_M; i_8++) {
            B[i_7][i_8] = G[i_8+i_7][1] + G[i_8+i_7][0] + B[i_7-1][i_8] + 5;
        }
    }
#pragma endscop
...
kernel_3434131115_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
