...
void kernel_4324232115_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_P; i_2++) {
                for (int i_3 = 0; i_3 < min(min(min(-i_2+PB_P-1, -i_0+PB_L+2), -i_1+PB_P-1), PB_L-2); i_3++) {
                    A[i_1][i_1][i_2][i_3+i_2+1] = D[i_0][i_2][i_3+i_1+1][i_2-1] - E[i_1][i_3+i_0-2][i_2][i_0] - E[i_3+2][i_2][i_1][i_0] - F[i_0][i_1][i_3+i_2] * A[i_3][i_3][i_2][i_2+i_0] * 5;
                }
            }
        }
        for (int i_4 = 2; i_4 < min(min(-i_0+PB_L, i_0), PB_M-2); i_4++) {
            for (int i_5 = i_4; i_5 < -i_0+PB_P-1; i_5++) {
                A[i_4-1][i_4-1][i_0][i_5+i_0+1] = F[i_5+1][i_0][i_4-2] + G[i_4+2][i_4+i_0] + 4;
            }
        }
    }
        ...
        for (int i_7 = 0; i_7 < PB_Q; i_7++) {
            for (int i_8 = 1; i_8 < min(-i_7+PB_P, PB_Q); i_8++) {
                for (int i_9 = 0; i_9 < PB_Q; i_9++) {
                    A[i_6][i_6][i_8-1][i_8+i_7] = E[i_9+i_8][i_7][i_6][0] * 5;
                }
            }
        }
    }
...
kernel_4324232115_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
