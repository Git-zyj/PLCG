...
void kernel_4412232313_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0] * F[i_0] * F[i_0-1] - E[i_0] - F[i_0] - E[i_0] * A[i_0-1] - A[i_0+1] * 1;
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 2; i_3 < PB_Q-1; i_3++) {
                    C[i_3-2][i_0][i_2][i_1-2] = G[i_0][i_0+1][i_1-2][i_3+1] + 2;
                }
            }
            for (int i_4 = 2; i_4 < PB_Q-1; i_4++) {
                for (int i_5 = 3; i_5 < min(PB_Q, -i_0+PB_M+1); i_5++) {
                    C[i_5-2][i_0][i_1][i_4-2] = H[i_5+i_0-1][i_1][i_4+1][i_0][0] - C[i_5-2][i_0-1][i_1][i_4-2] + C[i_5-1][i_0][i_1-1][i_4-2] + C[i_4-2][i_0][i_1][i_5-3] - 4;
                }
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
        A[i_6] = F[i_6] + E[i_6] - F[i_6+2] * 2;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4412232313_09(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H)));
