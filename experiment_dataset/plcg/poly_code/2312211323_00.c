...
void kernel_2312211323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = PB_L - 1; i_2 >= 2; i_2--) {
        for (int i_3 = 2; i_3 < min(PB_L, -i_2+PB_M+2); i_3++) {
            B[i_3][i_2] = D[i_3][i_2+1] + C[i_3-2][i_2] + C[i_3][i_2+2] + 6;
            C[i_3][i_2] = A[i_3+i_2-2] - B[i_3][i_2] - B[i_3][i_2-1] * B[i_3+1][i_2] - 4;
        }
    }
#pragma endscop
...
kernel_2312211323_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
