...
void kernel_4232212121_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_3 = 4; i_3 < PB_L; i_3++) {
        for (int i_4 = 3; i_4 < -i_3+PB_L; i_4++) {
            B[i_4+i_3][i_4-1] = C[i_3-2][i_4] + B[i_4+i_3-4][i_4-3] - 1;
        }
    }
#pragma endscop
...
kernel_4232212121_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
