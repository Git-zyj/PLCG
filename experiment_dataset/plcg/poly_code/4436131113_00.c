...
void kernel_4436131113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,int wc,DATA_TYPE POLYBENCH_5D(C,xC,yC,zC,vC,wC,xc,yc,zc,vc,wc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] + F[i_0][i_0-1] * E[i_0] - 2;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
            A[i_2-1] = F[i_2][i_1] + F[i_2][i_2] + 5;
        }
        for (int i_3 = 1; i_3 < min(PB_Q-1, -i_1+PB_L); i_3++) {
            for (int i_4 = i_1; i_4 < PB_Q-1; i_4++) {
                for (int i_5 = 0; i_5 < min(i_4, PB_M-1); i_5++) {
                    C[i_4][i_1-1][i_5][i_5][i_4] = G[i_5+1][i_1][i_3+1][i_4+1] + H[0][i_3+i_1][0][i_3+1][i_4] + C[i_4][i_3-1][i_5+1][i_5+1][i_4] - 6;
                }
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_L; i_6++) {
        for (int i_7 = i_6; i_7 < PB_Q-1; i_7++) {
            for (int i_8 = 0; i_8 < PB_N-1; i_8++) {
                C[i_8][i_6][i_7+1][i_7+1][i_8] = I[i_7][i_8][i_6-1] + I[i_8+1][i_7][i_6] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,wc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4436131113_00(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,wc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,wc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
