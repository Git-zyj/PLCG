...
void kernel_4426352225_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,int vm,int wm,DATA_TYPE POLYBENCH_5D(M,xM,yM,zM,vM,wM,xm,ym,zm,vm,wm)){
...
#pragma scop
        ...
        for (int i_1 = max(3, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < -i_1+PB_L; i_2++) {
                for (int i_3 = 2; i_3 < PB_L; i_3++) {
                    A[i_3][i_1][i_0-2][i_2+i_1] = E[i_1+1][i_3-1][i_0][0] + A[i_3-2][i_1][i_0-2][i_2+i_1] * A[i_3][i_1-1][i_0][i_2+i_1-3] * A[i_3-2][i_1][i_0-1][i_2+i_1] * 1;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_N; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_L); i_5++) {
            B[i_4][i_5] = F[i_5+i_4][i_4] + F[i_5+i_4][1] + G[i_5+1][i_4-1] * H[i_5+i_4][i_4] * B[i_4-2][i_5] + 3;
        }
    }
        ...
        for (int i_7 = 2; i_7 < i_6; i_7++) {
            for (int i_8 = i_7; i_8 < PB_M-2; i_8++) {
                for (int i_9 = 2; i_9 < -i_8+PB_L; i_9++) {
                    A[i_6-2][i_9][i_7-2][i_9+i_8] = I[i_9][i_6-2][i_7][i_9+i_8] * J[i_9][i_7][i_6+2][i_9+2] * J[i_9][i_6][i_8][i_7-2] * I[i_7-2][i_9][i_6][i_8] + K[i_8+2][i_9-2][i_6-2][i_7][i_8-1] + I[i_9][i_9][i_7-1][i_6] * 1;
                }
            }
        }
        for (int j_0 = 2; j_0 < i_6; j_0++) {
            for (int j_1 = i_6; j_1 < PB_P; j_1++) {
                for (int j_2 = 1; j_2 < -j_1+PB_P; j_2++) {
                    A[i_6-2][j_1][j_0-2][j_2+j_1-1] = E[j_2][j_2][j_0-2][i_6] + L[j_1-2][i_6+2][j_2+j_1] * M[i_6-1][j_2][j_1][j_0][j_1] - J[j_0-1][i_6-1][j_1][j_2+1] + I[j_2][j_0-1][i_6][j_1-2] - J[j_1][i_6][j_0][j_2-1] + 1;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4426352225_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M));
