...
void kernel_3336112321_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(min(PB_Q, i_0), -i_0+PB_M+2); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M-3; i_2++) {
                A[i_1][i_2+i_1][2] = D[i_0-2][i_2][i_1] + A[i_1-2][i_1+i_0-2][2] + A[i_1+1][i_2+i_1+3][2] + A[i_1][i_2+i_1-2][2] - 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N+1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_N; i_4++) {
            B[i_4-2][i_3-2] = B[i_4][i_3-2] - B[i_3-1][i_4-2] - 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 2; i_6 < min(i_5, PB_L-2); i_6++) {
            for (int i_7 = 2; i_7 < PB_N; i_7++) {
                C[i_5+1][i_6][i_7] = B[i_6-2][i_7] - C[i_5+1][i_6+2][i_7] - C[i_5-1][i_6][i_7-2] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3336112321_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
