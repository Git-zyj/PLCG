...
void kernel_3322211325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = A[i_0-1] * A[i_0+1] * A[i_0+2] - A[i_0-2] - A[i_0+2] + 1;
    }
    for (int i_1 = 2; i_1 < PB_P-3; i_1++) {
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
                B[i_1][i_3][i_1+1] = B[i_1-2][i_3][i_1-1] + B[i_1][i_3-1][i_1+1] + B[i_1+2][i_3+1][i_1+3] - 2;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_Q-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 0; i_6 < min(-i_5+PB_Q-1, -i_4+PB_Q-1); i_6++) {
                C[0][i_5][i_6+i_4+1] = D[i_6+i_5][i_6+1][i_4] * D[i_6][i_6][i_4] - C[0][i_4+2][i_6+i_5+1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3322211325_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
