...
void kernel_3422232115_02(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                D[i_1-3][i_2] = E[i_1+i_0][i_2+1] - F[i_2+1][i_1] * G[i_2][i_1][i_0] - 3;
            }
        }
    }
    ...
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        B[i_4] = I[i_4] - 4;
        for (int i_5 = i_4; i_5 < min(PB_L-1, -i_4+PB_N-1); i_5++) {
            D[i_4-2][i_5] = F[i_5+i_4+1][i_5] * F[i_5+i_4+1][0] - E[i_4][0] + F[i_5+i_4][i_5-1] + D[i_4-2][i_5+1] + 2;
        }
    }
#pragma endscop
...
kernel_3422232115_02(xd,yd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
