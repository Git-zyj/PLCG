...
void kernel_4316132325_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
        ...
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < PB_L; i_2++) {
                for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
                    A[i_1][i_3][i_0][i_2-1] = D[i_1+1][i_3-2][i_2][i_0] * D[i_0+2][i_3][i_1][i_0] - D[i_3][i_2-2][i_0][i_1] * E[i_3][i_2][i_1] * A[i_1-1][i_3][i_0+2][i_2-1] - A[i_1][i_3][i_0-2][i_2+1] + A[i_1+2][i_3][i_0][i_2-1] + A[i_1][i_0-2][i_3+1][i_2-1] * A[i_1-1][i_3][i_0+1][i_2-1] - 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        B[i_4] = F[i_4] * F[i_4] * B[i_4+2] - 3;
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            for (int i_6 = 0; i_6 < i_4; i_6++) {
                for (int i_7 = 0; i_7 < i_6; i_7++) {
                    A[i_6][i_7][i_5][i_4-1] = E[i_5-1][i_7+1][0] + 2;
                }
            }
        }
    }
...
kernel_4316132325_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
