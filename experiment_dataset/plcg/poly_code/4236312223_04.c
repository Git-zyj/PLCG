...
void kernel_4236312223_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_Q+1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_P-2; i_4++) {
            B[i_4][i_3][0] = C[i_3][i_4+2][i_3-1] - B[i_4+2][i_3+2][0] + B[i_4-2][i_3][0] + 5;
        }
    }
#pragma endscop
...
kernel_4236312223_04(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
