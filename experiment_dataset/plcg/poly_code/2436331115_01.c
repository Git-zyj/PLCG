...
void kernel_2436331115_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        A[i_0+1] = E[i_0] + F[i_0] - 3;
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_L-1); i_1++) {
            B[i_1+1][i_1+i_0] = E[0] + G[i_1+1][i_0] + F[i_1] + H[i_1][1] * A[i_0+1] + 1;
            D[i_1+1] = H[i_0][i_1] * H[i_0][i_0] * B[i_1+2][i_1+i_0+1] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        D[i_2+1] = E[i_2] - E[i_2] * E[i_2] * F[i_2] * D[i_2] - 1;
    }
#pragma endscop
...
kernel_2436331115_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
