...
void kernel_3214311211_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            A[i_0][i_1-1][i_1] = A[i_0][i_1-2][i_1-1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < min(PB_P, -i_2+PB_M-1); i_3++) {
            for (int i_4 = 2; i_4 < PB_P; i_4++) {
                B[i_3+i_2+1][i_4-1][1] = C[i_3-1][i_2+1][i_4] - B[i_3+i_2+1][i_4-2][1] * B[i_3+i_2][i_4][1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3214311211_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
