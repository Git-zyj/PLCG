...
void kernel_3326112111_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
            A[i_0-1] = D[i_0][i_1] * A[i_1-2] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_M+2; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_M, i_2); i_3++) {
            for (int i_4 = 3; i_4 < min(-i_3+PB_M, i_3); i_4++) {
                B[i_4-2][i_4+i_3] = D[i_4][i_3+2] * B[i_4-3][i_4+i_3-1] - 2;
            }
        }
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            C[i_2-2][i_5] = D[i_5][i_2] * 1;
        }
    }
#pragma endscop
...
kernel_3326112111_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
