...
void kernel_3236212223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[0] = C[i_0][i_0] + A[0] + A[0] * 1;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(i_1, PB_M-1); i_2++) {
            for (int i_3 = 2; i_3 < PB_N+2; i_3++) {
                B[i_3-2][i_2+1][i_1] = B[i_3-2][i_2+1][i_1-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3236212223_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
