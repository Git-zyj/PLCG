...
void kernel_2336151315_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0-1][i_0] = D[1] * E[1][i_0] * 5;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_N-1, -i_1+PB_L-1); i_2++) {
            C[i_1+1][i_2+i_1+1] = E[i_2+1][i_1-1] + E[i_1][i_2-1] * E[i_1][i_2-1] + E[i_1][i_2] + E[i_2][i_1] * C[i_1+1][i_2+i_1+1] * 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < min(min(PB_N-1, -i_3+PB_L-2), i_3); i_4++) {
            C[i_3+1][i_4+i_3+1] = E[i_4][i_3-1] - E[i_3][i_4+i_3] * E[i_3][0] - E[i_4][i_3] + E[i_4+1][i_3+1] - E[i_4][i_4] + C[i_3+1][i_4+i_3+2] - C[i_3+2][i_4+i_3+2] * C[i_3+1][i_4+i_3] * C[i_3][i_4+i_3] - 1;
        }
    }
#pragma endscop
...
kernel_2336151315_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
