...
void kernel_4412332221_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L; i_1++) {
            A[i_1+i_0][i_0][i_1-2] = E[i_0][i_1+1] - 3;
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                B[i_1][i_0] = F[i_2][i_0+1][i_1] * C[i_1][i_2+2] - 2;
            }
            C[i_1][i_0] = E[i_1][i_0] * G[i_1][i_0-2] + B[i_1][i_0] + B[i_1+1][i_0] * 6;
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_Q; i_5++) {
                for (int i_6 = max(i_4, 2); i_6 < -i_4+PB_M; i_6++) {
                    D[i_3][i_3-1][i_5][i_5+i_4] = H[i_3][i_6][i_6][i_4] - I[i_4][i_3][i_6+i_4][i_5] - I[i_5][i_4][i_3][i_3] + J[i_5-1][i_6-2][i_4] - K[i_6-2][i_5][i_4][i_3] - L[i_3][i_4+1][i_6+i_5] * D[i_3+1][i_3][i_5][i_5+i_4] - D[i_3+1][i_3][i_6-2][i_6+i_4] + 3;
                }
            }
        }
    }
