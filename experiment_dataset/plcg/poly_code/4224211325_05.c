...
void kernel_4224211325_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(min(PB_M-3, -i_0+PB_L-2), i_0), -i_1+PB_L); i_2++) {
                A[i_1+1][i_2+i_1][i_0] = A[i_2+3][i_2+i_0+2][i_1] * 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M-1; i_4++) {
            B[i_4+i_3][0] = C[i_3] - B[i_4+i_3][0] * B[i_4+i_3+1][0] * B[i_4+i_3+1][0] - B[i_4+i_3-2][0] * B[i_4+i_3-1][0] - 2;
        }
    }
#pragma endscop
...
kernel_4224211325_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
