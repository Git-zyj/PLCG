...
void kernel_4314312213_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < PB_L; i_1++) {
            A[i_1-2][i_0] = D[i_0][i_1] - A[i_1-3][i_0-1] + A[i_1-2][i_0-1] * C[i_1-2] * 5;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M, i_0); i_2++) {
            for (int i_3 = 0; i_3 < PB_Q; i_3++) {
                B[i_0][i_2+i_0][i_3] = E[i_3][i_0][0] + 6;
            }
        }
    }
...
kernel_4314312213_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
