...
void kernel_4236212313_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_L-2; i_3++) {
            for (int i_4 = max(2, i_3); i_4 < PB_L-3; i_4++) {
                B[i_3][i_4+2] = C[i_3][i_2][i_3] - C[i_4-2][i_2+1][i_3+2] - B[i_3][i_4+3] + B[i_3-1][i_4+3] * B[i_3][i_4+1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4236212313_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
