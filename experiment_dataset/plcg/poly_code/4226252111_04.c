...
void kernel_4226252111_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < PB_P-2; i_1++) {
            for (int i_2 = i_0; i_2 < PB_P-2; i_2++) {
                A[i_2][i_1-2] = C[i_2][i_0-2][i_1+2] * D[i_2][0] * D[i_2][i_1] + E[i_1+1][i_2+2][i_0+2] + E[i_2][i_2][i_0] * A[i_2][i_1-3] + 1;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_P-2; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L; i_4++) {
            A[i_4][i_3-3] = F[i_4][i_3+1] + D[i_4-2][i_3] - D[i_3][i_4] + F[i_4-2][0] * F[i_4-2][i_3+2] * 4;
        }
    }
#pragma endscop
...
kernel_4226252111_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
