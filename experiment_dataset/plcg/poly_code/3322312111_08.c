...
void kernel_3322312111_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                B[i_0][i_1][i_2+1] = B[i_1][i_0][i_2+2] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
        for (int i_4 = 1; i_4 < PB_M+1; i_4++) {
            for (int i_5 = 2; i_5 < PB_M+2; i_5++) {
                C[i_5-2][i_4][i_4] = D[i_4][i_4][i_3-1] - C[i_4-1][i_5][i_5] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3322312111_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
