...
void kernel_3334311211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_2][i_2] = D[i_3][0] - C[i_3] + C[i_2+1] * 5;
        }
        C[i_2] = 2;
    }
#pragma endscop
...
kernel_3334311211_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
