...
void kernel_3134212313_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
    ...
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(min(PB_L, -i_0+PB_M+1), -i_1+PB_M+1); i_2++) {
                A[i_2+i_1-2][i_2] = B[i_1][i_0][i_0+2] + A[i_2+i_1-3][i_2] - A[i_2+i_0-1][i_2] - A[i_2+i_1-1][i_1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3134212313_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
