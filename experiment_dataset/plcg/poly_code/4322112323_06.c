...
void kernel_4322112323_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            A[i_0][i_0] = D[i_0-1][1] + 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            C[i_3][i_3] = D[i_4][i_3-2] * C[i_3-1][i_3-1] - C[i_3+2][i_3+2] + C[i_3-2][i_3-2] - 1;
        }
    }
#pragma endscop
...
kernel_4322112323_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
