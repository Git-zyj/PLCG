...
void kernel_3322232121_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[i_1][i_0-2] = A[i_1+2][i_0-2] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            B[i_2][i_3+2] = D[i_3-1][i_2-2] - E[i_3] * D[i_3+2][i_2] + D[i_3][i_2-2] - 1;
        }
    }
    for (int i_4 = 0; i_4 < PB_Q; i_4++) {
        for (int i_5 = 1; i_5 < min(PB_Q, -i_4+PB_M); i_5++) {
            C[i_5][i_5] = F[i_4+2][i_5] - G[i_5+i_4][i_5+2][0] * E[i_5-1] * C[i_5-1][i_5-1] * 1;
        }
    }
#pragma endscop
...
kernel_3322232121_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
