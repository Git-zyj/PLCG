...
void kernel_4222111123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1][i_0-1] = C[i_0][i_1-1] * C[i_1][i_0] + 6;
        }
    }
        ...
        for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                for (int i_5 = 0; i_5 < min(-i_3+PB_P-2, i_3); i_5++) {
                    B[i_3-1][1][i_5+i_3] = B[i_3-1][1][i_5+i_3+2] * 6;
                }
            }
        }
    }
...
kernel_4222111123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
