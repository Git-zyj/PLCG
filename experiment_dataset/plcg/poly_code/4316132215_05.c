...
void kernel_4316132215_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                B[i_2-1][i_1-1] = D[i_1-1][i_2] * 6;
            }
        }
        for (int i_3 = i_0; i_3 < PB_N-2; i_3++) {
            B[i_3-1][i_0] = D[i_3][i_0] - D[i_0][i_0] * D[i_3+2][i_0+1] - B[i_0-1][i_3+1] * 1;
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_N+1; i_5++) {
            C[i_4][i_5-1][i_5] = D[i_5+1][i_4] - E[i_5] * D[i_5+i_4-1][i_4] - D[i_5][i_4] - E[1] * C[i_4][i_5][i_5+1] - C[i_5-1][i_4-2][i_4-1] * 5;
        }
    }
#pragma endscop
...
kernel_4316132215_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
