...
void kernel_3212111311_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1][i_1][i_0] = A[i_1-1][i_1-1][i_0] + A[i_1-1][i_1-1][i_0+1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_2+1][1] = C[i_2][i_3] + C[i_2+1][i_3] - B[i_2+2][1] - B[i_2][1] - B[i_2][1] - B[i_2+2][1] + 3;
        }
    }
#pragma endscop
...
kernel_3212111311_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
