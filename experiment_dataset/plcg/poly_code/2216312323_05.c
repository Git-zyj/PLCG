...
void kernel_2216312323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[0] = C[i_1] - A[0] + B[i_1+2] - B[i_0+2] + B[i_0+4] - 5;
        }
        B[i_0+2] = A[0] + 2;
    }
#pragma endscop
...
kernel_2216312323_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
