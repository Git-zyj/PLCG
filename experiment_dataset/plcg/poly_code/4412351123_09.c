...
void kernel_4412351123_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_1+PB_M, -i_0+PB_M+1), -i_2+PB_M); i_3++) {
                    A[i_1][i_2-1][i_0][i_3] = E[i_3][i_2][i_0][i_1] * E[i_2][i_3+i_0-1][i_1][i_2] + F[i_2][i_1+i_0][i_3+1][i_0] + G[i_0][i_1][i_3+1][i_1][i_2] - H[i_3][i_3+i_1][i_0+1][i_1][i_2] - I[i_2][i_2][i_3][i_0+1] - I[i_3+i_2][i_0+1][i_1][i_2+1] + 4;
                }
            }
        }
        B[i_0+1] = J[i_0] + K[i_0] + K[i_0] - 5;
        C[i_0] = L[i_0] - L[i_0] - J[i_0] + B[i_0+1] + 3;
        for (int i_4 = -i_0+PB_M - 1; i_4 >= 0; i_4--) {
            for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
                for (int i_6 = i_0; i_6 < PB_M-1; i_6++) {
                    A[i_5-1][i_4][i_0+2][i_6] = E[i_6][i_0][i_4][i_5] * E[i_6-1][i_4][i_5][i_0-1] + I[i_6+1][i_0][i_4][i_5+1] + M[i_5][i_4][i_6+1] * F[i_4][i_6+1][i_6+1][i_5] - F[i_4+i_0][i_5][i_4][i_6] - 5;
                }
            }
        }
    }
