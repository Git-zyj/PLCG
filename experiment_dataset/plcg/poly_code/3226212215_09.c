...
void kernel_3226212215_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-3; i_2++) {
            B[i_2+2] = C[i_2][i_1] - B[i_2+1] - B[i_2+3] + 1;
        }
    }
#pragma endscop
...
kernel_3226212215_09(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
