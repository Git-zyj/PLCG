...
void kernel_4326232225_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
                    A[i_3+2][i_1][i_2][i_2-1] = D[i_2+2][i_0][i_1][i_0-2] - A[i_3+2][i_1][i_2-1][i_2-2] * A[i_3+2][i_1][i_2+1][i_2] + 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_N-2; i_4++) {
        for (int i_5 = 3; i_5 < PB_L; i_5++) {
            B[i_4-1] = E[i_4][i_5] * F[i_4+2] * G[i_4][0] + G[i_5+1][i_4+2] - E[i_5][i_4-2] - E[i_5-2][i_4-2] + B[i_5-3] - 1;
        }
        B[i_4-2] = H[i_4] + H[i_4+2] + B[i_4-1] * 4;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4326232225_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
