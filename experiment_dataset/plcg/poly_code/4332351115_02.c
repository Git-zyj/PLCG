...
void kernel_4332351115_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        B[i_0-2] = D[0] + D[i_0] + E[i_0] + F[i_0-1][i_0] * 1;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M+1, i_1); i_2++) {
            B[i_1-1] = G[i_1][1] - G[i_2-1][i_1] - F[i_2][i_1] + H[i_2][i_1+1][i_2] + C[i_2+2][i_1] - 1;
            C[i_2+1][i_1+1] = F[i_1-1][i_2+1] * F[i_2+i_1-1][0] + E[i_2] - F[i_2][i_1] + G[i_2+i_1+1][i_1] - C[i_2][i_1+1] * 2;
        }
    }
#pragma endscop
...
kernel_4332351115_02(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
