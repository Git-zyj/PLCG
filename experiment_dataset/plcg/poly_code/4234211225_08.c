...
void kernel_4234211225_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q+1, i_0); i_1++) {
            for (int i_2 = i_0; i_2 < PB_Q-1; i_2++) {
                B[i_1+1][i_2+1][i_0] = C[i_0][i_2+1][i_1+i_0-1] * C[i_1][i_0][i_1] * 1;
            }
        }
    }
        ...
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < min(i_4, PB_L-2); i_5++) {
                for (int i_6 = 0; i_6 < i_5; i_6++) {
                    B[i_4][i_5][i_3] = B[i_6][i_5][i_4-2] - B[i_6+1][i_5+2][i_4] * 1;
                }
            }
        }
    }
...
kernel_4234211225_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
