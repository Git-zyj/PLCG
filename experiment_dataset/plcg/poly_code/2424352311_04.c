...
void kernel_2424352311_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0] = E[i_0] * F[i_0-2] - A[i_0-1] * A[i_0+1] + B[i_0-1] + 4;
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            B[i_1-1] = G[i_1][i_0] * H[i_1-2][i_0+2] * G[i_1][i_0+1] + H[i_0+2][i_1+1] + G[i_1][i_0] * A[i_1-1] - A[i_1+1] - B[i_1] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            D[i_3] = G[i_2][i_3] * I[i_2+2][i_3] * H[i_2][i_3] + H[i_3+i_2][i_3+1] - G[i_3][i_2-2] + D[i_3] - 6;
        }
        D[i_2] = J[i_2] + E[i_2] + J[i_2] - J[i_2] - J[i_2] - E[i_2+2] + D[i_2-1] * D[i_2+1] * 4;
    }
#pragma endscop
...
kernel_2424352311_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
