...
void kernel_2224232225_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0][i_0] = C[i_0-2][i_1] - D[i_1-1][i_1] * 3;
        }
        for (int i_2 = 3; i_2 < PB_M; i_2++) {
            B[i_2-2][i_0+2] = C[i_2][i_0] + E[i_0-2] + E[1] - B[i_2-3][i_0] + B[i_2-3][i_0+2] + B[i_2-2][i_0+4] * 1;
        }
    }
#pragma endscop
...
kernel_2224232225_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
