...
void kernel_3214211315_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_M-2); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M-2; i_2++) {
                B[i_2+i_1+2][1] = C[i_0][0][i_1] - 1;
            }
        }
    }
    for (int i_3 = PB_M-1 - 1; i_3 >= 1; i_3--) {
        for (int i_4 = 0; i_4 < -i_3+PB_M-1; i_4++) {
            B[i_4+i_3][1] = B[i_4+i_3-1][1] - B[i_4+i_3+1][1] + B[i_4+i_3+1][1] + 6;
        }
    }
#pragma endscop
...
kernel_3214211315_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
