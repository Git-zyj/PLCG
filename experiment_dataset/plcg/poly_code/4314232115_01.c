...
void kernel_4314232115_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            B[i_1] = D[i_1][i_0-2][2] * E[i_1+2] * B[i_1-2] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        B[i_2-1] = E[i_2] - F[i_2] - 2;
        for (int i_3 = 1; i_3 < min(min(PB_M-2, i_2), -i_2+PB_P); i_3++) {
            for (int i_4 = 2; i_4 < PB_Q; i_4++) {
                for (int i_5 = 0; i_5 < -i_2+PB_M; i_5++) {
                    C[i_4-1][i_5][i_3+i_2][i_2] = G[i_5+1][i_4+1][i_5+i_3][i_2] * H[i_3-1][i_4-2][i_2-1][i_5+1] + D[i_5+i_2][i_4-1][i_4] + G[i_3+2][i_4][i_2][i_3+2] - 1;
                }
            }
        }
    }
...
init_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4314232115_01(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
