...
void kernel_4416231125_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M+1; i_1++) {
            A[i_1+i_0-1][i_1+1] = E[i_1] * A[i_1+i_0-2][i_1] + 1;
        }
        for (int i_2 = i_0; i_2 < -i_0+PB_L; i_2++) {
            for (int i_3 = 1; i_3 < min(i_0, PB_L-1); i_3++) {
                for (int i_4 = 1; i_4 < PB_L; i_4++) {
                    D[i_2+1][i_4-1][i_3][i_3+1] = F[i_2-1][i_0][i_4][i_3] + F[i_4][i_3+1][i_2+1][i_0] + G[i_3-1][i_2+i_0][i_4][i_2-1] - G[i_3][0][i_4-1][i_2] - 5;
                }
            }
            A[i_2+i_0+1][i_0+1] = H[i_2+i_0-1][1] + I[i_2-1][i_0] * 1;
        }
    }
        ...
        for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 0; i_7 < -i_6+PB_L; i_7++) {
                for (int i_8 = 1; i_8 < i_5; i_8++) {
                    D[i_8+1][i_6-1][i_7][i_7+1] = F[i_8-1][i_8][i_6+1][i_6+1] + F[i_6][i_7][i_8][i_5] * G[i_6][i_7][i_8][i_5-1] - F[i_5][i_8][i_7+i_6][i_8-1] - F[i_6][i_8][i_5][0] - D[i_8-1][i_6-1][i_7][i_7+1] * 1;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4416231125_06(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
