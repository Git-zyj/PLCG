...
void kernel_4222232123_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_Q; i_1++) {
        for (int i_2 = 4; i_2 < min(PB_Q, -i_1+PB_L); i_2++) {
            B[i_2-2] = D[i_2+i_1][2] + E[i_1-1][2] * D[i_2][i_1] + D[i_1][i_2-1] + D[i_1-2][0] - B[i_2-4] + 4;
        }
    }
#pragma endscop
...
kernel_4222232123_01(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
