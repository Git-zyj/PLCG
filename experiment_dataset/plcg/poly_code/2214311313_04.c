...
void kernel_2214311313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0+1] = C[i_0] * A[i_0+2] + A[i_0] * B[i_0-1] * 1;
    }
    for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            B[i_2] = B[i_1+1] * B[i_2+1] * 3;
        }
    }
#pragma endscop
...
kernel_2214311313_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
