...
void kernel_4234311121_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_1+1][i_2] = A[i_2+1][i_1-1] - 6;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_Q; i_4++) {
            for (int i_5 = 1; i_5 < -i_3+PB_L-1; i_5++) {
                for (int i_6 = 2; i_6 < PB_Q-1; i_6++) {
                    B[i_5+i_3+1][i_6][i_6+1][i_4] = C[i_3][i_5-1][i_4][i_6] * B[i_5+i_3+1][i_6-2][i_6-1][i_4] * 4;
                }
            }
        }
    }
...
kernel_4234311121_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
