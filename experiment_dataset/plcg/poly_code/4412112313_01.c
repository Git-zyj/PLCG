...
void kernel_4412112313_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-3; i_1++) {
            A[i_0][i_1+2] = E[i_1-2][i_0] - A[i_0-1][i_1+2] - A[i_0+1][i_1+1] * 6;
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                A[i_0][i_1+3] = F[i_2+i_0][i_1+2][0] - 6;
            }
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                for (int i_5 = i_3; i_5 < PB_Q-1; i_5++) {
                    C[i_4][i_5][i_3][i_0+2] = C[i_4][i_5][i_3+1][i_0+1] * C[i_4][i_0][i_3][i_5+1] - 2;
                }
            }
            for (int i_6 = 1; i_6 < PB_L; i_6++) {
                for (int i_7 = 1; i_7 < PB_L; i_7++) {
                    D[i_6][i_3][i_0][i_7] = D[i_6][i_3-1][i_0-1][i_7-1] + D[i_6-1][i_3][i_0][i_7-1] + D[i_0+1][i_3+1][i_7][i_6] + 3;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4412112313_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
