...
void kernel_2222152111_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0+1][i_0] = C[i_0] * C[i_0-1] + C[i_0] - C[i_0] * A[i_0][i_0-1] * 2;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        B[i_1] = C[i_1] - C[i_1-2] * C[i_1-2] + C[i_1] + C[i_1+2] * A[i_1+2][i_1+1] * 6;
    }
#pragma endscop
...
kernel_2222152111_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
