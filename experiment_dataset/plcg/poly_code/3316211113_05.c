...
void kernel_3316211113_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        ...
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            C[i_1+i_0+1][i_1+1] = D[i_1-1] + C[i_1+i_0][i_1+1] + 4;
        }
    }
#pragma endscop
...
kernel_3316211113_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
