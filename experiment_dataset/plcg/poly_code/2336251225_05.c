...
void kernel_2336251225_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] + D[i_0] * E[i_0] * E[i_0-1] - A[i_0+1] + A[i_0-2] * 1;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 2; i_2 < min(min(i_1, -i_1+PB_L), PB_M-1); i_2++) {
            B[i_2+1][i_2] = F[i_2+i_1][0] - E[i_1+1] - G[i_1-1][i_2][i_1] + F[i_1][0] * F[i_2][i_1+1] - A[i_2-2] * 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_M-1, i_3); i_4++) {
            B[i_4+1][i_4] = D[0] * H[i_3][i_4+1] - H[i_4+1][i_3+1] + D[i_4] + F[i_4+1][i_3] - A[i_4] - 3;
        }
    }
#pragma endscop
...
kernel_2336251225_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
