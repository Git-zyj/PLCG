...
void kernel_3214112215_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L+1; i_1++) {
            B[i_1+i_0-1][i_0][0] = C[i_0][i_1] - 5;
        }
        for (int i_2 = -i_0+PB_L - 1; i_2 >= i_0; i_2--) {
            B[i_2+i_0][i_2][0] = B[i_2+i_0-1][i_0][0] * B[i_2+i_0-1][i_2-1][0] - 3;
        }
    }
#pragma endscop
...
kernel_3214112215_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
