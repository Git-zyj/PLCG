...
void kernel_4414332223_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M+1; i_1++) {
            A[i_1][i_0-2] = E[i_0][i_1] + F[i_0-2][i_1+i_0-1] + A[i_1-2][i_0-2] - A[i_1][i_0-3] - 5;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_N+1; i_2++) {
            for (int i_3 = 0; i_3 < i_2; i_3++) {
                for (int i_4 = 0; i_4 < -i_2+PB_M+1; i_4++) {
                    B[i_0][i_2][i_3][i_3] = G[i_4+i_2-1][i_3][i_0+2] + H[i_2+i_0-1][i_4][i_2+i_0-2][i_3+2] - B[i_0][i_2+1][i_3][i_3] + B[i_3+2][i_0][i_2][i_2] * 2;
                }
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
        C[i_5] = I[i_5-2] * J[i_5] * K[2] - 4;
        for (int i_6 = 0; i_6 < -i_5+PB_P; i_6++) {
            for (int i_7 = i_5; i_7 < PB_M; i_7++) {
                for (int i_8 = i_7; i_8 < PB_P-2; i_8++) {
                    D[i_5+2][i_6][i_8][i_8] = H[i_6+2][i_7][i_5-1][i_7] * L[i_7-2][i_6+i_5][i_8][i_5] * D[i_5+2][i_6][i_8+2][i_8+2] * 2;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4414332223_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L)));
