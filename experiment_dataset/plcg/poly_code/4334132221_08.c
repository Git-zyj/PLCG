...
void kernel_4334132221_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_Q; i_1++) {
            for (int i_2 = 2; i_2 < PB_Q-2; i_2++) {
                for (int i_3 = 0; i_3 < i_2; i_3++) {
                    A[i_1][i_0][i_3][i_2+2] = D[i_3+1][i_0][i_2][i_1-1] + D[i_1-1][i_3][i_1+i_0][i_2-2] * A[i_3][i_0-2][i_1][i_2+2] + A[i_3][i_2][i_0-1][i_1] * A[i_1-1][i_0][i_2-2][i_3+2] + 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        B[i_4] = E[i_4-1][i_4] + F[i_4] - F[i_4] * F[i_4] + 4;
    }
    for (int i_5 = 1; i_5 < PB_P; i_5++) {
        for (int i_6 = 1; i_6 < min(min(PB_Q-2, -i_5+PB_Q), i_5); i_6++) {
            for (int i_7 = 0; i_7 < PB_M; i_7++) {
                for (int i_8 = 1; i_8 < PB_P+1; i_8++) {
                    A[i_7][i_5-1][i_8-1][i_6+2] = D[i_6-1][i_5][i_7][i_7] + D[i_7][i_6][i_6+i_5][i_8] - 6;
                }
            }
        }
    }
...
kernel_4334132221_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
