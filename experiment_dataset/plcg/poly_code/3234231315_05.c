...
void kernel_3234231315_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1-1][i_0] = C[i_1][i_0] + A[i_1][i_0] * A[i_1-1][i_0-1] - A[i_0][i_1-1] * A[i_1-1][i_0+1] * A[i_1][i_0+1] * 3;
        }
    }
    for (int i_2 = PB_M - 1; i_2 >= 1; i_2--) {
        B[i_2] = D[i_2] - E[i_2-1] - E[i_2] * D[i_2] * B[i_2-1] - 5;
    }
#pragma endscop
...
kernel_3234231315_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
