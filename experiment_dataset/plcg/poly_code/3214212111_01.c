...
void kernel_3214212111_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_M, PB_L-1), -i_0+PB_P-1); i_2++) {
                A[i_1+i_0][i_2+1][i_2+i_0] = A[i_2+i_1][i_0+2][i_2+i_0+1] + 5;
            }
        }
        B[i_0] = C[i_0-2] + B[i_0-1] * 6;
    }
#pragma endscop
...
kernel_3214212111_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
