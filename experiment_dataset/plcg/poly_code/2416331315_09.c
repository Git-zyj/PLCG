...
void kernel_2416331315_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            A[i_0][0] = E[i_0][i_0] + A[i_0+1][0] * D[i_0] - 6;
        }
        A[i_0-1][0] = A[i_0-1][0] - A[i_0][0] * 5;
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            A[i_2][0] = F[i_3+1] * E[i_2+1][i_3+1] + E[i_3][i_2] * G[i_3][i_2+1] + E[i_2][i_3-1] + 1;
        }
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            D[i_4] = G[i_4+1][i_2] - H[i_2][i_2-1] - I[i_4][i_2][1] * D[i_2-1] - D[i_4+1] * 3;
        }
    }
#pragma endscop
...
kernel_2416331315_09(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
