...
void kernel_2416111125_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0][i_0] = E[i_0] + A[i_0-1][i_0-1] * A[i_0+1][i_0+1] + 1;
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_M+1); i_1++) {
            B[i_1+i_0-1][i_0-1] = F[i_1][i_0] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
        ...
        for (int i_4 = i_2; i_4 < min(-i_2+PB_P-2, PB_L); i_4++) {
            D[i_4][i_4+i_2] = E[i_4] * D[i_4][i_4+i_2+2] * D[i_4][i_4+i_2-2] - 2;
        }
    }
#pragma endscop
...
kernel_2416111125_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
