...
void kernel_4214211223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[i_1][i_0] = C[i_0] - A[i_1+1][i_0] * A[i_1+2][i_0] + 2;
        }
    }
        ...
        for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                for (int i_5 = 0; i_5 < PB_P; i_5++) {
                    B[i_3][i_4][i_5] = B[i_4][i_5][i_3+2] * B[i_3][i_5][i_2+2] + 5;
                }
            }
        }
    }
...
kernel_4214211223_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
