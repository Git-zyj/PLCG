...
void kernel_3312312313_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < PB_P+1; i_1++) {
            for (int i_2 = i_0; i_2 < PB_P+1; i_2++) {
                A[i_1+2][i_1-2] = A[i_2+3][i_2-1] - A[i_2+1][i_2-3] + A[i_1+1][i_1-3] * A[i_2+3][i_2-1] * A[i_1+3][i_1-1] * 5;
            }
        }
        B[i_0-2] = D[0] * B[i_0-3] * B[i_0-1] + 3;
        for (int i_3 = 0; i_3 < -i_0+PB_L; i_3++) {
            C[i_0-2][i_3+i_0] = C[i_0-1][i_3+i_0] * C[i_0-2][i_3+i_0-1] * 5;
        }
    }
#pragma endscop
...
kernel_3312312313_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
