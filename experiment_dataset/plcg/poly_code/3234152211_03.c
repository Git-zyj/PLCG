...
void kernel_3234152211_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < -i_0+PB_M+1; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_M+3, -i_1+PB_M+2); i_2++) {
                A[i_2+i_1-2][2] = C[i_0-1][i_1] - D[i_1-1][i_2-2][i_0-1] + D[i_2][i_1][i_0-1] - C[i_1][i_0] + D[i_2-1][i_0][i_1] * A[i_2+i_1-3][2] - A[i_1+i_0-1][2] - A[i_2+i_0-3][2] + 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < -i_3+PB_M; i_4++) {
            for (int i_5 = min(-i_3+PB_M, PB_P) - 1; i_5 >= 1; i_5--) {
                B[i_5+i_3][i_4][i_5] = D[i_4][i_5-1][i_3] - D[i_5][i_3-2][i_4] + D[i_4+i_3][i_5][i_4] * D[i_4-2][i_5][i_3] + C[i_4-1][i_5+1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3234152211_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
