...
void kernel_3424352121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] + F[i_0] * E[i_0] + E[i_0] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            for (int i_3 = 2; i_3 < PB_N-2; i_3++) {
                B[i_3+2][i_1+2][i_3+1] = G[i_2][i_3+1] + H[i_1+1][i_3-1][i_2] + I[i_3][i_2+i_1] + G[i_3+1][i_2-2] + I[i_3-2][i_2] * H[i_3+i_2][i_1][i_3] + H[i_3+i_2-1][i_2][i_2+i_1] - J[i_1][i_2][i_3] * H[i_2][i_1][i_3] + 6;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_N-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_N-1; i_5++) {
            C[i_5-1][i_4] = E[i_4-1] * K[i_5][i_4] - C[i_5+1][i_4] - 2;
            for (int i_6 = 0; i_6 < min(i_4, -i_4+PB_N-2); i_6++) {
                D[i_4][i_6][i_6] = K[i_6+i_4+2][i_5-2] - J[i_6][i_4][i_5] * L[i_5][i_4+1][i_4] * C[i_5-1][i_6] * D[i_4-2][i_6+2][i_6+2] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_3424352121_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
