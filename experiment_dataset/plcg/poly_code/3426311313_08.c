...
void kernel_3426311313_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_P-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_0][i_2][i_1+i_0] = A[i_0-1][i_2+1][i_1+i_0-1] - A[i_0+1][i_2+1][i_1+i_0+1] + 5;
            }
        }
        B[i_0+1] = B[i_0] - 5;
    }
    for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_Q-2; i_4++) {
            for (int i_5 = i_3; i_5 < min(-i_4+PB_Q-2, -i_3+PB_Q-2); i_5++) {
                C[i_3][i_4][i_5+i_4+1] = C[i_4][i_3+1][i_5+i_3+2] + C[i_3][i_4][i_5+i_4+2] * C[i_3+1][i_4][i_5+i_4+1] - 4;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
        D[i_6] = E[i_6] * D[i_6-1] - D[i_6+1] + D[i_6-1] * 5;
    }
#pragma endscop
...
kernel_3426311313_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
