...
void kernel_2224131115_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(min(-i_2+PB_L, i_2), PB_L-1); i_3++) {
            A[i_2] = C[i_3][i_3+i_2] - C[i_2][i_3+1] - C[i_2][i_3-1] * D[i_3] - 3;
        }
    }
#pragma endscop
...
kernel_2224131115_02(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
