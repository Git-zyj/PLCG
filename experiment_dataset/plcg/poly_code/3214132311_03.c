...
void kernel_3214132311_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-3; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
                A[i_0][i_1+2][i_2] = C[i_2][i_0][i_1] + C[i_2-2][i_0][i_1] * D[i_2+1][i_2] - A[i_0-1][i_1+2][i_2+1] - A[i_0][i_1+3][i_2] + A[i_2][i_1+3][i_0] - 2;
            }
        }
        for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
            B[i_0-2][i_3] = E[i_3-2] + C[i_0+1][i_3][i_0] * D[i_0][i_3-2] + B[i_0-2][i_3-1] * B[i_0-3][i_3] + B[i_0-2][i_3+1] * 1;
        }
    }
#pragma endscop
...
kernel_3214132311_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
