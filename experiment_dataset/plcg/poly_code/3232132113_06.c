...
void kernel_3232132113_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(PB_Q, -i_0+PB_N), -i_1+PB_L); i_2++) {
                A[i_0+1][i_2+2][i_0] = C[i_2+i_0][i_1+1][i_2+2][0] * D[i_1][i_2][i_0] + D[i_0+2][i_2+i_1][i_2] + A[i_0+1][i_2+3][i_0] * 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < min(-i_3+PB_P-2, PB_Q); i_4++) {
            for (int i_5 = 2; i_5 < PB_M; i_5++) {
                B[i_3][i_5][i_4] = E[i_4-2][i_5-2] - C[i_4][i_5-1][i_3+2][2] + D[i_5][i_3][i_4+i_3+2] + B[i_5-1][i_3+1][i_4-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3232132113_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
