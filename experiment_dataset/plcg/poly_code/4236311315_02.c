...
void kernel_4236311315_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            A[i_0+1][0] = C[i_0+1] + A[i_0+2][0] * A[i_0][0] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        A[i_2+1][0] = A[i_2+1][0] * 4;
    }
#pragma endscop
...
kernel_4236311315_02(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
