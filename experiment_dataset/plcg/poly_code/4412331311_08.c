...
static void print_array(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,int wl,DATA_TYPE POLYBENCH_5D(L,xL,yL,zL,vL,wL,xl,yl,zl,vl,wl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4412331311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,int wl,DATA_TYPE POLYBENCH_5D(L,xL,yL,zL,vL,wL,xl,yl,zl,vl,wl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0-1] + F[i_0-1] * A[i_0+1] - A[i_0-1] + A[i_0+1] - A[i_0+1] * 5;
        for (int i_1 = i_0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                for (int i_3 = 0; i_3 < min(PB_Q, -i_2+PB_L); i_3++) {
                    B[i_0][i_3][i_2][i_1] = G[i_1][i_2][i_3][i_1-1] - H[i_1-1][i_0-1][i_3+i_2][i_1+1] + B[i_0][i_3][i_2-1][i_1+1] - B[i_0+1][i_3][i_2][i_1-1] - 6;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_Q; i_4++) {
        for (int i_5 = 1; i_5 < min(-i_4+PB_M-1, PB_Q-1); i_5++) {
            for (int i_6 = 0; i_6 < min(-i_5+PB_M-1, -i_4+PB_L); i_6++) {
                C[i_5+i_4][i_5-1][i_5] = I[i_5][i_4+1][i_5+1] * J[i_6][i_5+i_4][i_6+i_4] + C[i_5+i_4+1][i_5][i_5+1] + C[i_5+i_4+1][i_5-1][i_5] * C[i_6+i_5+1][i_5-1][i_5] + 3;
            }
        }
        for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
            for (int i_8 = 1; i_8 < min(PB_Q, -i_4+PB_M); i_8++) {
                for (int i_9 = 1; i_9 < min(min(min(PB_Q, -i_4+PB_M-1), -i_8+PB_Q), -i_7+PB_P); i_9++) {
                    D[i_7][i_8+i_4][i_7-1] = K[i_7][i_8][i_9-1] * H[i_4][i_8][i_9][i_7] + L[i_4+1][i_8][i_9+i_7][i_4][i_9] + M[i_7+1][i_8][i_4][i_9+i_8] + B[i_7-1][i_8][i_4][i_9-1] + D[i_8][i_9+i_4+1][i_8-1] * 1;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,wl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4412331311_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,wl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M));
