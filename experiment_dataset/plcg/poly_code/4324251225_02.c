...
void kernel_4324251225_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_L-1); i_1++) {
            A[i_1+1][i_1+i_0] = D[i_1-1][0] + D[i_0][i_1] - E[i_1+1] * F[i_1][i_0][i_0] + A[i_1+1][i_1+i_0-1] - A[i_1+2][i_1+i_0+1] - 2;
        }
    }
        ...
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, PB_M-3); i_4++) {
                for (int i_5 = i_3; i_5 < -i_3+PB_Q; i_5++) {
                    B[i_5][i_5][i_4+1] = G[i_3][i_4+1][i_5][i_2] - G[i_3+1][i_2][i_4+1][i_5+i_3] - H[i_5][i_3][1][i_2] + G[i_4+i_3][i_5][i_3-1][i_4] + H[i_4+i_3][i_5-1][i_3][i_2+1] * G[i_2][i_5][i_3+1][i_4] - B[i_5][i_5][i_4+3] + 1;
                }
            }
        }
        for (int i_6 = i_2; i_6 < -i_2+PB_L; i_6++) {
            A[i_2+1][i_6+i_2] = D[i_6+1][1] - I[i_6][i_2] + I[i_6][i_2-1] * D[i_6+i_2][i_6+1] + 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4324251225_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
