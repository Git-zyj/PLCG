...
void kernel_3222311125_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_1 = 3; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < PB_N; i_2++) {
            for (int i_3 = 0; i_3 < -i_1+PB_M; i_3++) {
                B[i_2][i_3+i_1] = B[i_2-2][i_3+i_1-3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3222311125_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
