...
void kernel_4214112315_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_N-1, PB_L-1); i_1++) {
            A[i_0+2][i_1] = C[i_1+i_0+1][i_0+1] + A[i_0+2][i_1+1] - A[i_0+2][i_1-1] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            for (int i_4 = i_2; i_4 < PB_L; i_4++) {
                A[i_3+1][i_4] = A[i_3+2][i_4] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4214112315_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
