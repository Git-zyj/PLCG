...
void kernel_4314131323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_1+i_0-1] = D[i_1+i_0+1] + D[i_1] + A[i_1+i_0-1] - A[i_1+i_0+1] - 3;
        }
        B[i_0] = D[i_0+1] * D[i_0] * B[i_0-1] * B[i_0-2] + B[i_0+2] - 1;
    }
    for (int i_2 = PB_M-1 - 1; i_2 >= 0; i_2--) {
        B[i_2] = D[i_2] + D[i_2] + D[i_2+1] * 3;
    }
#pragma endscop
...
kernel_4314131323_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
