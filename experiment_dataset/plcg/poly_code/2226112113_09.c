...
void kernel_2226112113_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            B[i_1-1][i_0] = C[i_0][i_0] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_2-1][i_3] = D[i_3] + B[i_2-1][i_3+1] * 6;
        }
    }
#pragma endscop
...
kernel_2226112113_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
