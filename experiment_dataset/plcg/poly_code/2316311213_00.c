...
void kernel_2316311213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = i_0; i_2 < PB_P-1; i_2++) {
            B[i_0-1][i_2] = D[i_2][i_0+1] + B[i_0-1][i_2-1] - B[i_0-2][i_2] * B[i_0-2][i_2+1] * C[i_2-1] * 5;
        }
        C[i_0] = E[i_0][0] - A[i_0] + 1;
    }
#pragma endscop
...
kernel_2316311213_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
