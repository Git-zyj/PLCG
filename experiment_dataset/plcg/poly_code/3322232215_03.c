...
void kernel_3322232215_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
    ...
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_M, -i_2+PB_L); i_3++) {
                A[i_3] = F[i_3+i_2][i_1+1][i_1] + G[i_3+i_1][i_2-2] - 2;
            }
        }
        for (int i_4 = 2; i_4 < min(-i_1+PB_M, PB_P-1); i_4++) {
            A[i_4-1] = H[i_1+1][i_4] * H[i_4][i_1] * H[i_4+i_1][i_4] - H[i_4-2][i_4+1] - H[i_1][i_4] * A[i_4] * 3;
        }
    }
#pragma endscop
...
kernel_3322232215_03(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
