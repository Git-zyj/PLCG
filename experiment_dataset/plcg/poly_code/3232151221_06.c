...
void kernel_3232151221_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            A[i_1] = C[i_1-1] - D[i_0][i_1-1] - E[i_1-1][i_0-1][0] - D[i_1-1][i_0] + D[i_0-1][i_1-1] + C[i_1] + A[i_1-1] * A[i_1+1] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_P-1, -i_2+PB_M); i_3++) {
            B[i_2][i_3] = C[i_3+1] - D[i_2-1][i_3-1] + C[i_2-1] * D[i_3+i_2][0] + B[i_2][i_3+1] - 6;
        }
    }
#pragma endscop
...
kernel_3232151221_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
