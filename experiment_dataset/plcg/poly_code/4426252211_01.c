...
void kernel_4426252211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] - E[i_0+2] * E[i_0] - F[i_0] * A[i_0-1] * A[i_0+1] * A[i_0-1] + 4;
        for (int i_1 = 3; i_1 < min(-i_0+PB_L+2, i_0); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_Q-1; i_2++) {
                for (int i_3 = 0; i_3 < min(i_0, -i_1+PB_M+1); i_3++) {
                    B[i_0][i_1-2][i_3+2][i_0] = G[i_2][i_3][i_1+i_0-2][i_1+2] + H[i_1+2][i_0+1][i_2+1][i_3] - G[i_3+i_1-1][i_0-2][i_2][0] * H[i_3][i_0][i_3][i_2+i_0+1] * I[i_2][i_3][i_0][i_1-2][2] * H[i_1][i_2][i_3][i_0] * B[i_0-1][i_1-3][i_3+2][i_0-1] + 2;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = i_4; i_5 < min(PB_M, -i_4+PB_N); i_5++) {
            C[i_4+1][i_5] = J[i_5][2] * J[i_5+i_4][0] * J[i_5+1][i_4] - E[i_5] * C[i_4+2][i_5] - 1;
        }
    }
    for (int i_6 = 1; i_6 < PB_M; i_6++) {
        for (int i_7 = 2; i_7 < i_6; i_7++) {
            D[i_7-2][i_6] = F[i_7] - F[i_6] + E[i_7] - J[i_7][i_6] * K[i_7][i_6] + A[i_6-1] + 2;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4426252211_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
