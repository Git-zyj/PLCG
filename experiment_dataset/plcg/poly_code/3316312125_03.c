...
void kernel_3316312125_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[i_0-2] = D[i_1+2][i_0] - A[i_0] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_M+2; i_2++) {
        ...
        for (int i_5 = i_2; i_5 < PB_M; i_5++) {
            for (int i_6 = 0; i_6 < PB_M; i_6++) {
                C[i_5][i_2-2][i_6] = D[i_6][i_5] * B[i_5][i_2-1][i_6] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3316312125_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
