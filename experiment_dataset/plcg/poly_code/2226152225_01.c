...
void kernel_2226152225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L; i_1++) {
            A[i_0-2][i_1] = C[i_1+i_0][i_1] + C[i_1][i_1+i_0] - C[i_0][i_1] - D[i_0] * A[i_0][i_1] - A[i_0-3][i_1-2] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M+2; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_N, i_2); i_3++) {
            A[i_2-2][i_3] = C[i_3-1][i_2] + C[i_2+2][i_3] - C[i_3][i_2+2] + C[i_3-1][i_2] * C[i_3+i_2][i_2] - 2;
        }
    }
#pragma endscop
...
kernel_2226152225_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
