...
void kernel_4314131325_03(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            C[i_0+3][i_1] = D[i_0-1] * E[i_1+1][i_1] * E[i_1][i_0] * 5;
            for (int i_2 = 2; i_2 < min(-i_1+PB_M, -i_0+PB_L); i_2++) {
                B[i_0-1][i_2][i_1] = F[i_1-1][i_2+1][i_0+1] * F[i_2][i_2-1][i_1] - F[i_2+i_1][i_2+i_0][i_1] - B[i_0-2][i_2-2][i_1] * B[i_0-1][i_2-2][i_1+2] - 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_Q; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-3; i_4++) {
            C[i_4+1][i_3] = E[i_4][i_3] - C[i_4+2][i_3+2] * C[i_4+3][i_3] - C[i_4][i_3] + 4;
        }
    }
#pragma endscop
...
kernel_4314131325_03(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
