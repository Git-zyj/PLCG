...
void kernel_3212152113_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_N+2, -i_0+PB_M-2); i_1++) {
            for (int i_2 = 2; i_2 < PB_N+2; i_2++) {
                A[i_1-2][i_1-2][i_2-1] = C[i_0][i_2][i_1] + D[i_1+i_0+2][i_2+2] + D[i_1][i_2] * C[i_0+1][i_1][i_0] + 5;
                A[i_2-2][i_2-2][i_1-1] = C[i_2][i_1+i_0][0] + C[i_0][i_2-1][i_1-2] - C[i_0][i_0-1][i_2] * C[i_2][i_2-2][i_1+1] + C[i_2][i_0-2][i_1] * C[i_2][i_0][0] - A[i_1-3][i_1-3][i_2-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3212152113_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
