...
void kernel_3324151111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
    ...
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                B[i_2][i_3][i_2] = E[i_2+i_1][i_3+1][i_3] - E[i_3-1][i_1][i_1+1] + E[i_2-1][i_1][i_1] + B[i_1-1][i_3][i_1-1] - B[i_2-1][i_3+1][i_2-1] + 2;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        C[i_4] = D[i_4] + D[i_4] + D[i_4] * D[1] - D[i_4] - D[i_4] + D[i_4] - D[i_4-1] - D[i_4-1] + 1;
    }
#pragma endscop
...
kernel_3324151111_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
