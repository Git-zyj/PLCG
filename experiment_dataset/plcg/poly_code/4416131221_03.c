...
void kernel_4416131221_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_N-1); i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_M+1, PB_M-1); i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_M-1; i_3++) {
                    A[i_2+i_0-1][i_1-1][i_3-1] = E[i_1][i_3][i_0][i_2] * E[i_1+1][i_3][i_0][i_0+1] + E[i_1-1][i_0][i_3-1][i_2] - A[i_3+i_2+1][i_1-1][i_0-1] - A[i_1+i_0][i_2-2][i_3-1] * 5;
                }
            }
            B[i_1] = F[i_0][i_1+1] - G[i_1+1][i_0][i_1] - B[i_1+2] - B[i_1-2] * 6;
        }
        for (int i_4 = 1; i_4 < PB_P; i_4++) {
            for (int i_5 = i_4; i_5 < PB_P; i_5++) {
                for (int i_6 = 0; i_6 < min(PB_P, -i_5+PB_M+1); i_6++) {
                    A[i_6+i_5-1][i_4][i_0-1] = E[i_5][i_0+1][i_4][i_6] * E[i_5-1][i_0][i_4-1][i_5] - 3;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
        for (int i_8 = 0; i_8 < i_7; i_8++) {
            D[i_8][i_7+1] = G[i_7-1][i_8][0] - H[i_8] - B[i_7] * D[i_8][i_7-1] + 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4416131221_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
