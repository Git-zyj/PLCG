...
void kernel_3222311115_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
            B[i_2] = C[i_2+i_1][i_1] + D[0] * B[i_2-1] - 3;
        }
    }
#pragma endscop
...
kernel_3222311115_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
