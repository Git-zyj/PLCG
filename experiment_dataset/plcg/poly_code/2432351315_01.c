...
void kernel_2432351315_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            B[i_1-1] = E[i_1+1][i_0] * F[i_1-1] + F[i_0] + G[i_1-1][i_0] + E[i_1+1][i_0+1] + E[i_0][i_0] - E[i_1-1][i_0+1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        B[i_2] = H[i_2+1] * I[i_2+1] - I[1] - B[i_2-1] + 1;
        B[i_2-1] = I[i_2-1] - I[i_2] - J[i_2+1][i_2] + F[i_2] - E[i_2][i_2-1] * B[i_2] - 3;
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            D[i_3][i_4] = H[i_3] - E[i_4][i_3] * G[i_4][i_3] - G[i_4][i_3] + B[i_4-1] + D[i_3][i_4+1] * D[i_3-1][i_4] * D[i_3+1][i_4] - 3;
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2432351315_01(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
