...
void kernel_3222311325_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = i_0; i_2 < -i_1+PB_M-1; i_2++) {
                B[i_0-2][i_2-1][i_2+i_1+1] = 3;
            }
        }
        for (int i_3 = 3; i_3 < -i_0+PB_M-1; i_3++) {
            for (int i_4 = 2; i_4 < -i_0+PB_M; i_4++) {
                B[i_3-1][i_4][i_4+i_0] = C[i_3][i_4+i_0][i_3+1] - B[i_3-3][i_4-2][i_4+i_0-1] + B[i_4][i_3+1][i_3+i_0+1] * B[i_4][i_3][i_3+i_0] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3222311325_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
