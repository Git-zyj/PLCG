...
void kernel_3432152115_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_1+PB_P-2, -i_2+PB_M); i_3++) {
                B[i_1-1][i_2][i_3-1] = F[i_3+i_2][i_1][i_2] * F[i_3+2][i_3+i_1][i_2] * F[i_2][i_3+i_1+2][2] + B[i_1-1][i_2+1][i_3-1] * 3;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < PB_P-2; i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_P-2; i_6++) {
                C[i_5+2][i_5][0] = F[i_6-1][i_5-1][i_5] - F[i_5+2][i_6-1][i_4] * F[1][i_4-2][i_6+i_5+2] - F[i_5][i_6][i_5] - 2;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
        for (int i_8 = i_7; i_8 < PB_M-2; i_8++) {
            A[i_8] = G[i_8+2][i_7+1] + G[1][i_7] * G[i_8+2][i_7-1] - G[i_8+2][i_7] + G[i_7][i_8+2] * G[i_8+2][i_7] * G[i_8][i_7] + A[i_8-1] + 5;
        }
    }
#pragma endscop
...
kernel_3432152115_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
