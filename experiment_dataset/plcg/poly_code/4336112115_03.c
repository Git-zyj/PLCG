...
void kernel_4336112115_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0] - D[i_0-2] + A[i_0-1] + 4;
    }
        ...
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                for (int i_5 = 0; i_5 < -i_3+PB_M-1; i_5++) {
                    C[i_5+i_3][i_4][i_2-2][0] = C[i_5+i_3+1][i_4][i_2-3][0] + 5;
                }
            }
        }
    }
...
kernel_4336112115_03(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
