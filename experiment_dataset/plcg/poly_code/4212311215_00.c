...
void kernel_4212311215_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_N, -i_0+PB_L); i_2++) {
                A[i_1][i_0][i_2] = C[i_1][i_2+i_0][i_1] - D[i_1][i_1][i_0] * A[i_2][i_0][i_1+1] - B[i_1] - 3;
            }
        }
...
kernel_4212311215_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
