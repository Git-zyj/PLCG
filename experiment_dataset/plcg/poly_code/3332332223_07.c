...
void kernel_3332332223_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0-1] = D[i_0+1] - E[i_0] - D[i_0+1] + E[i_0] - 2;
        B[i_0] = D[i_0-2] * F[i_0][2] - E[i_0-2] * A[i_0] - A[i_0-1] - A[i_0-2] + 6;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
                C[i_2][i_2][i_3-1] = G[i_2+2][i_1][i_1] - H[i_1][i_3+2][i_2] * C[i_2][i_2][i_3] * C[i_2-1][i_2-1][i_3-1] - C[i_1-2][i_1-2][i_3-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3332332223_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
