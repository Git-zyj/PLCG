...
void kernel_2224211221_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < min(i_1, PB_M-1); i_2++) {
            B[i_2][i_1] = C[i_2][i_1][0] + B[i_2+1][i_1] * B[i_2-1][i_1] - B[i_2-2][i_1+1] * 5;
        }
    }
#pragma endscop
...
kernel_2224211221_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
