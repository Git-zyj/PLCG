...
void kernel_4424112111_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 2; i_3 < PB_P+2; i_3++) {
                    A[i_1][i_2][i_3-2] = D[i_1][i_0+2][i_2][i_2-1] + 6;
                }
            }
        }
        for (int i_4 = 1; i_4 < min(-i_0+PB_L, i_0); i_4++) {
            for (int i_5 = 2; i_5 < min(PB_N, -i_4+PB_L-1); i_5++) {
                for (int i_6 = 2; i_6 < -i_4+PB_L-1; i_6++) {
                    B[i_5+i_4+1][i_6-2][i_4-1] = E[i_5][i_6][i_4+i_0][i_0] * B[i_6+i_4+1][i_5-2][i_4] + 6;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
        for (int i_8 = 2; i_8 < PB_N; i_8++) {
            for (int i_9 = 1; i_9 < PB_N; i_9++) {
                C[i_8][i_7][i_9] = F[i_8][i_9][i_7+1] + G[i_9][i_8-2] - C[i_8-1][i_7-1][i_9-1] + 5;
            }
        }
    }
    for (int j_0 = 0; j_0 < PB_M-2; j_0++) {
        for (int j_1 = max(j_0, 1); j_1 < PB_L; j_1++) {
            for (int j_2 = 2; j_2 < PB_N; j_2++) {
                D[j_2][j_0+2][j_1][j_1-1] = F[j_0][j_2][j_2-2] - 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4424112111_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
