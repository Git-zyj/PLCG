...
void kernel_2236212321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_M-1; i_1++) {
            A[i_1-1] = A[i_1+1] + A[i_1-2] - A[i_1-3] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L; i_3++) {
            B[i_2][i_3] = C[i_2][i_3+i_2] - B[i_2-2][i_3] * B[i_2][i_3+1] + 6;
        }
    }
#pragma endscop
...
kernel_2236212321_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
