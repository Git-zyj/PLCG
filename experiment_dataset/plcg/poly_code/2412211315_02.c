...
void kernel_2412211315_02(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            C[i_0-1][i_1] = 6;
            C[i_0-2][i_1] = E[1][i_0] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            C[i_3-1][i_2] = F[i_3][i_2] + E[i_2][i_3+1] + C[i_3-1][i_2-1] + C[i_3][i_2] * C[i_3-1][i_2+1] * 6;
        }
...
kernel_2412211315_02(xc,yc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
