...
void kernel_3424332313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0-2] + F[i_0] * F[1] + A[i_0+1] + 1;
        A[i_0-1] = D[i_0] - 3;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
            D[i_2-1] = G[i_2][i_1] - H[i_2+1][i_1] + D[i_2-1] - D[i_2-2] - 3;
        }
        D[i_1-1] = E[i_1] - F[i_1-2] * F[i_1] * I[i_1+1] + F[i_1] * A[i_1] * D[i_1-2] * 4;
    }
#pragma endscop
...
kernel_3424332313_00(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
