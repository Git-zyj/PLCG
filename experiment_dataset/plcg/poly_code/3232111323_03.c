...
void kernel_3232111323_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            B[i_4] = C[i_4][i_3+1] + B[i_4-1] - B[i_4+1] + B[i_4+2] - 4;
        }
    }
#pragma endscop
...
kernel_3232111323_03(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
