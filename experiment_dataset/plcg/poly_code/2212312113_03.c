...
void kernel_2212312113_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 3; i_1 < PB_M+1; i_1++) {
            A[i_1-1][i_0-2] = A[i_1-1][i_0-3] - A[i_0-1][i_1-3] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            B[i_2+1][i_3] = C[i_3][i_2] + 6;
        }
    }
#pragma endscop
...
kernel_2212312113_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
