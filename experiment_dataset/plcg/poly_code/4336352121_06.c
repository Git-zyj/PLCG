...
void kernel_4336352121_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                    A[i_1+2][i_3][i_0][i_2] = D[i_1][i_0+2][i_3][0] - E[i_0][i_3+2][i_2][i_1] * 6;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_Q-2; i_4++) {
        for (int i_5 = 0; i_5 < min(PB_Q-3, -i_4+PB_M); i_5++) {
            for (int i_6 = 2; i_6 < min(i_4, -i_4+PB_Q-2); i_6++) {
                B[i_4][i_6+2][i_5+1][i_6-2] = F[i_5][i_4+1][i_6+i_4+2] + G[i_5][i_6+i_4] - H[i_5+1][i_4][i_6+2][i_5] * F[i_5][i_6][i_4] - G[i_6+2][i_5] - I[i_6-1][i_5+1][i_4] + I[i_6][i_4][i_5] + I[i_5+i_4][i_6+i_4][i_5] * B[i_4][i_6+2][i_5+3][i_6-2] + 3;
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_L; i_7++) {
        for (int i_8 = 1; i_8 < PB_Q; i_8++) {
            for (int i_9 = 0; i_9 < min(min(PB_N-2, -i_7+PB_L), i_7); i_9++) {
                C[i_9+2][i_8] = H[i_9][i_9][i_7-1][i_8] - J[i_8][i_9+i_7][0] * G[i_9+i_7-2][i_8] * C[i_9+2][i_8-1] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4336352121_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
