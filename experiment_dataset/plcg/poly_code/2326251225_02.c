...
void kernel_2326251225_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0][i_0] = D[i_0+1] * D[i_0+1] + E[0] * F[i_0][i_0] + A[i_0-1][i_0-1] * A[i_0+2][i_0+2] * 3;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M-1, i_1); i_2++) {
            A[i_1][i_1] = D[i_1] - F[i_1][i_2+1] * F[i_2+i_1][i_2+i_1+1] * G[i_2][i_1] - E[i_2+i_1] + 6;
        }
    }
    for (int i_3 = 2; i_3 < PB_Q; i_3++) {
        for (int i_4 = i_3; i_4 < PB_P; i_4++) {
            A[i_3-2][i_3-2] = H[i_3+1][i_4][1] * F[i_4+1][i_3] - E[i_4+1] + G[i_4][i_3] - E[i_4-1] - G[i_4-1][i_3] - 6;
        }
    }
#pragma endscop
...
kernel_2326251225_02(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
