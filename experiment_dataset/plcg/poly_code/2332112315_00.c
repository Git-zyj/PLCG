...
void kernel_2332112315_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_0-2][i_1] = D[i_1][i_0] + 4;
        }
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            B[i_0-1][i_2] = B[i_0-2][i_2] * B[i_0-1][i_2+1] * 1;
        }
    }
...
kernel_2332112315_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
