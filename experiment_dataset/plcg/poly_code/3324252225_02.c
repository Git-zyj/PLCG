...
void kernel_3324252225_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-2] = D[i_0] * D[i_0] - E[i_0] - A[i_0-1] - 1;
        for (int i_1 = i_0; i_1 < min(-i_0+PB_P, PB_L-2); i_1++) {
            B[i_0][i_1] = F[i_0-1][0][i_1] - F[i_1+1][i_1+i_0][i_1] + G[i_1][i_0+1] + B[i_0][i_1+1] + B[i_0-2][i_1] + B[i_0][i_1+2] - 6;
        }
        A[i_0-2] = D[0] - D[i_0-1] + G[i_0-2][i_0-1] * D[i_0-1] + D[i_0] * D[i_0] * D[i_0] + 4;
    }
#pragma endscop
...
kernel_3324252225_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
