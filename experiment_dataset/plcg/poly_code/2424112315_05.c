...
void kernel_2424112315_05(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            C[i_1-1] = E[i_2] * C[i_2] - 1;
        }
        C[i_1] = 2;
    }
    for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
        D[i_3+2] = D[i_3+1] * D[i_3+3] - D[i_3+3] + 6;
    }
#pragma endscop
...
kernel_2424112315_05(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
