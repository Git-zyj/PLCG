...
void kernel_4126212323_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = min(i_0, -i_0+PB_P) - 1; i_1 >= 2; i_1--) {
            for (int i_2 = 1; i_2 < min(PB_Q-3, -i_0+PB_Q-1); i_2++) {
                for (int i_3 = 2; i_3 < -i_2+PB_Q-3; i_3++) {
                    A[i_0][i_3][i_1][i_3+i_2+2] = B[i_3+2][i_2][i_1+i_0] * A[i_3][i_0][i_1][i_2+i_0+1] + A[i_0+2][i_2][i_1-2][i_3+i_2+3] * A[i_0][i_3-2][i_1][i_3+i_2-1] - 1;
                }
            }
        }
    }
...
kernel_4126212323_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
