...
void kernel_3216211215_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_0 = PB_M-2 - 1; i_0 >= 1; i_0--) {
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_0+1][0][i_1] = B[i_0+2][0][i_1-1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3216211215_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
