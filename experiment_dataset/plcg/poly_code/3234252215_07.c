...
void kernel_3234252215_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_Q-1, -i_0+PB_P-2); i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_Q; i_2++) {
                A[i_2+i_1-2][i_1+i_0-1][i_2] = C[i_1+1][i_0][i_2+i_1] * C[i_0][i_2][0] + D[i_2-1][i_0][i_1] - C[1][i_0-1][i_0] + C[i_1][i_2][i_0] + A[i_2+i_1-3][i_1+i_0-1][i_2-1] - A[i_2+i_1-2][i_1+i_0-2][i_2] - 4;
            }
            for (int i_3 = 1; i_3 < -i_1+PB_L+1; i_3++) {
                A[i_3+i_1-3][i_1+i_0-1][i_3-1] = C[i_3+2][i_0-1][i_1] - C[i_0][i_3][i_1+1] * D[i_3+i_1-1][i_1+i_0+2][i_0+1] * C[i_3-1][i_0][i_1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3234252215_07(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
