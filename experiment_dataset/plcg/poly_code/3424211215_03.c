...
void kernel_3424211215_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        ...
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            C[i_0][i_3] = C[i_0][i_3+1] - 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M+1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-1; i_5++) {
            D[i_5] = E[i_5][0] * D[i_5-1] * D[i_5+1] - D[i_4-1] * 6;
        }
    }
#pragma endscop
...
kernel_3424211215_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
