...
void kernel_3224212125_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_P+2, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_1+i_0-1][i_2][i_0+1] = C[i_2][i_0+1][i_1-2] * 1;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < PB_P+1; i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                B[i_4][i_3][i_4-1] = B[i_3-2][i_5][i_3-3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3224212125_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
