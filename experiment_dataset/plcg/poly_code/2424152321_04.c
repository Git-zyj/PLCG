...
void kernel_2424152321_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(PB_N, -i_0+PB_L); i_1++) {
            A[i_0][i_1-2] = E[2] + F[i_1][i_1-2][i_0+2] + G[i_1+i_0][i_0] + A[i_0+1][i_1-1] - D[0][i_1] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_N-1; i_3++) {
            B[i_3+i_2][i_2] = G[i_3+i_2+1][i_3] - G[i_3][i_2+1] + E[i_3] * E[i_2-2] * B[i_3+i_2+1][i_2] + 3;
        }
        for (int i_4 = 2; i_4 < min(-i_2+PB_L, PB_M); i_4++) {
            C[i_2+1][i_4+2] = G[i_4][i_2+2] + G[i_2][i_4] + G[i_4][i_2] * G[i_4][i_2+1] * A[i_2][i_4-2] + C[i_2][i_4+3] * D[0][i_2-1] * 5;
            D[0][i_2] = G[i_2][i_4+i_2] + G[i_4+1][i_2-2] + E[i_4] - E[i_4] - G[i_4+i_2][i_4-1] * G[i_4][0] * C[i_2-1][i_4+4] * D[0][i_2+2] * D[0][i_2-2] * 6;
        }
    }
#pragma endscop
...
kernel_2424152321_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
