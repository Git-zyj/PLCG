...
void kernel_3336312223_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L-2; i_2++) {
                A[i_2+i_0][i_1][i_2+i_0] = A[i_2+i_0][i_1+1][i_2+i_0] * A[i_2+i_0][i_1-1][i_2+i_0] + A[i_2+i_0+2][i_1][i_2+i_0+2] * 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_L-2; i_4++) {
            for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
                B[i_4][i_5] = B[i_4-2][i_5+1] * B[i_4+2][i_5-1] + 3;
            }
        }
        C[i_3-1] = D[i_3][1] + C[i_3] - 6;
    }
#pragma endscop
...
kernel_3336312223_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
