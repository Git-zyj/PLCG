...
void kernel_2314252125_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-4; i_0++) {
        A[i_0] = D[i_0-2] + D[i_0+2] - E[i_0-2] - E[i_0-2] + A[i_0-1] + 5;
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            B[i_0+2][i_1] = F[i_1][i_0][i_1] * E[i_1-2] + E[i_0] + G[i_1][i_0] - H[i_1][i_0-1][i_1+1] - B[i_0+4][i_1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_M-1; i_3++) {
            B[i_2+2][i_3] = I[i_2][0] - F[i_2-1][i_3-2][i_2] - I[i_3+1][i_2+1] + I[i_2][i_3] * 2;
        }
    }
#pragma endscop
...
kernel_2314252125_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
