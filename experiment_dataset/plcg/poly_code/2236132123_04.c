...
void kernel_2236132123_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-2] = C[i_0][i_0] + D[i_0] + A[i_0] - 5;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < min(PB_L, i_1); i_2++) {
            A[i_2-2] = D[i_2] - C[i_1+2][i_2] + C[i_1][i_2-1] * 4;
        }
    }
#pragma endscop
...
kernel_2236132123_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
