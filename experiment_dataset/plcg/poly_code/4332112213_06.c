...
void kernel_4332112213_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd)){
        ...
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            for (int i_3 = 0; i_3 < -i_1+PB_Q; i_3++) {
                for (int i_4 = 2; i_4 < -i_1+PB_M-1; i_4++) {
                    B[i_4][i_4+i_3][i_2] = D[i_2][2][i_4-2][i_3+i_1] + B[i_1+1][i_4+i_1+1][i_2] - 3;
                }
            }
        }
...
kernel_4332112213_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D));
