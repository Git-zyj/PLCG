...
void kernel_3312211323_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_0][i_1][i_1] = A[i_0][i_1-2][i_1-2] - A[i_0+2][i_1][i_1] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        ...
        for (int i_3 = 0; i_3 < min(PB_L, -i_2+PB_P); i_3++) {
            for (int i_4 = 0; i_4 < PB_N; i_4++) {
                C[i_4][i_3][i_2] = D[i_3][i_4][i_3+i_2] - A[i_2][i_3][i_3] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3312211323_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
