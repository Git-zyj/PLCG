...
void kernel_4414352211_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] * F[0] * E[i_0] - E[2] * B[i_0] - 2;
        B[i_0] = E[i_0-1] + E[1] + G[i_0] + E[i_0] + A[i_0] * 1;
    }
    for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 2; i_2 < min(PB_P-2, i_1); i_2++) {
            for (int i_3 = 2; i_3 < PB_P-2; i_3++) {
                C[i_2][i_3+1] = H[i_1-2][i_2+2][i_3+2][i_3] * I[i_2-2][i_3-1][i_1-1] + J[i_1-1][i_3+1][i_2+1] + J[i_2+1][i_3-2][i_1] * C[i_2][i_3+2] + 3;
            }
        }
        for (int i_4 = 2; i_4 < -i_1+PB_P-1; i_4++) {
            for (int i_5 = i_4; i_5 < min(-i_4+PB_M-1, -i_1+PB_P); i_5++) {
                for (int i_6 = 1; i_6 < -i_1+PB_M; i_6++) {
                    D[i_5-1][i_6][i_4+i_1][i_1] = H[i_1][i_6][i_4][i_4] - K[i_1][i_5+i_4][i_6][i_1] - L[i_5+i_4+1][i_1][0][i_6] - K[i_1][i_6+i_4][i_5][i_4-2] * L[i_6+2][i_1-2][i_5][i_4] * L[i_4][i_6+i_1][i_6][i_5] - D[i_6-1][i_5+1][i_4+i_1+1][i_1+1] - D[i_4-2][i_6][i_5+i_1][i_1] * 5;
                }
            }
        }
    }
