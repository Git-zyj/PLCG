...
void kernel_2226311321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-2; i_0++) {
        A[0][i_0] = C[i_0] + A[0][i_0+2] * 2;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            B[i_1+i_0-1][i_1] = D[i_1][i_0+1][i_0] * A[0][i_0] - B[i_1+i_0-2][i_1-1] - B[i_1+i_0+1][i_1+2] - B[i_1+i_0-3][i_0-2] + B[i_1+i_0][i_1+1] + 4;
        }
    }
#pragma endscop
...
kernel_2226311321_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
