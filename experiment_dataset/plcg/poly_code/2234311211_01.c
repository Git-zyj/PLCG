...
void kernel_2234311211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = C[i_0-1] * A[i_0+1] - 6;
        for (int i_1 = 0; i_1 < min(i_0, PB_P-1); i_1++) {
            B[i_0][i_1] = A[i_0] - B[i_0+1][i_1] * B[i_0][i_1+1] + 4;
        }
    }
#pragma endscop
...
kernel_2234311211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
