...
void kernel_2424252325_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            A[i_1][i_0] = E[i_1] - F[i_1+2][i_0-1] * F[i_1][i_0] + F[i_0][i_0+1] * G[i_0+1][i_0] - E[i_1-1] - A[i_0-1][i_1] + A[i_1+2][i_0] + B[i_1+1][i_0] * B[i_0-2][i_1] - 6;
            B[i_1][i_0] = F[i_1][i_0] + F[i_1+2][i_0] - F[i_0+2][i_1+1] * F[i_1-2][i_0] * H[i_1+2] - A[i_1][i_0] * 6;
        }
        for (int i_2 = 0; i_2 < PB_Q; i_2++) {
            C[i_0+1][i_2] = H[i_0+2] * E[i_2] - G[i_0][i_2] * A[i_2][i_0-1] + C[i_0+3][i_2] * C[i_2+1][i_0-2] - 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_Q-1; i_4++) {
            D[i_3][i_4] = E[i_4] + G[i_4][i_4+1] + H[i_3] - F[i_3][i_4-2] + D[i_3][i_4-2] * D[i_3-2][i_4] * D[i_3][i_4-1] + D[i_3+1][i_4] - 2;
        }
    }
#pragma endscop
...
kernel_2424252325_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
