...
void kernel_2334111223_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            A[i_1+i_0][1] = D[i_0][i_1-1] + A[i_1+i_0+1][1] * A[i_1+i_0-2][1] - A[i_1+i_0-2][1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_L+1; i_3++) {
            C[i_2+1][i_3-2] = D[i_2-1][i_3-1] + 1;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            C[i_4+1][i_5-1] = C[i_4+1][i_5] + 4;
        }
    }
#pragma endscop
...
kernel_2334111223_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
