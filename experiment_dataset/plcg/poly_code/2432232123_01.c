...
void kernel_2432232123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0-2] = E[i_0-2] - E[i_0] + 6;
        for (int i_1 = 3; i_1 < PB_L; i_1++) {
            A[i_1-3] = F[i_0][i_1] * A[i_1-2] * A[i_0-2] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 4; i_3 < -i_2+PB_M; i_3++) {
            C[i_3-2] = F[2][i_2] - F[i_3][i_3] * G[i_3][i_2] - F[i_3+i_2][1] - C[i_3-4] * 4;
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        D[i_4+1] = E[i_4] * H[i_4-2] * 2;
    }
#pragma endscop
...
kernel_2432232123_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
