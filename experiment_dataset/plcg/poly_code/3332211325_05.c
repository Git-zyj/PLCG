...
void kernel_3332211325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0+1] * A[i_0-2] + B[i_0] * 1;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        B[i_1] = 2;
        for (int i_2 = 0; i_2 < min(i_1, PB_M-1); i_2++) {
            B[i_2+1] = B[i_1-1] + B[i_1] + B[i_1-2] + 2;
        }
    }
#pragma endscop
...
kernel_3332211325_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
