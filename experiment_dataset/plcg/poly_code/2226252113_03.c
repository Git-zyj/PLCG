...
void kernel_2226252113_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            A[i_1+1] = C[i_0][1] * D[i_0][i_0] - C[i_0-2][i_1+1] - D[i_1][i_0] - E[i_1] + D[i_1][i_0+2] - A[i_1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_N-1, -i_2+PB_M); i_3++) {
            A[i_3+1] = D[i_3][i_3+i_2] + C[i_3][i_2] * D[i_2][i_3] * D[i_3+1][i_2] * 3;
        }
    }
#pragma endscop
...
kernel_2226252113_03(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
