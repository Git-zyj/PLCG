...
void kernel_3324112325_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+i_0][i_0+1] = D[i_1+2] + A[i_1+i_0+2][i_0+1] - A[i_1+i_0+1][i_0+2] - A[i_1+i_0][i_0+3] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
        for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
            for (int i_4 = 2; i_4 < min(-i_3+PB_M, i_2); i_4++) {
                A[i_4+i_3][i_3+1] = E[i_3-1][i_3] + C[i_4-2][i_3-1] - C[i_4][i_3-1] + 2;
            }
            C[i_3][i_2-1] = 4;
        }
    }
...
kernel_3324112325_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
