...
void kernel_2236112225_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0] = A[i_1+i_0-1] + B[i_1-2] * 3;
        }
    }
    for (int i_2 = 4; i_2 < PB_N; i_2++) {
        B[i_2-2] = C[i_2] + B[i_2-4] * 2;
    }
#pragma endscop
...
kernel_2236112225_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
