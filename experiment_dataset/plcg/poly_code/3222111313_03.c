...
void kernel_3222111313_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                B[i_2+1][i_1-2] = C[i_1][i_0-1][i_2] + 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_L, i_3); i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                B[i_4][i_5-1] = C[i_4][i_5-1][i_3] + B[i_5][i_4] * B[i_5-1][i_4-1] * B[i_4-1][i_5-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3222111313_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
