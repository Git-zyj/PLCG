...
void kernel_4412152325_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = E[0] - E[i_0] - E[i_0] * E[i_0] + E[i_0] * A[i_0-3] * A[i_0+1] + A[i_0] + 5;
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            B[i_0][i_1] = F[i_1][i_0+1] + E[i_1] - F[i_1+2][i_0] * F[i_1-2][i_0+1] * F[i_1+1][i_0] + A[i_1-1] + A[i_1+1] + B[i_0-1][i_1] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
            for (int i_4 = 1; i_4 < PB_M; i_4++) {
                B[i_3][i_4] = G[i_4][i_3][i_2] * G[i_2+1][i_2-1][i_3+1] - G[i_4][i_2][i_3] * G[i_4][i_2][i_4] * H[i_2][i_4-1][i_3][i_4] - G[i_4][i_3][i_2] * 3;
            }
            for (int i_5 = 0; i_5 < min(-i_3+PB_M, PB_Q-2); i_5++) {
                for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
                    D[i_5][i_3+2][i_6] = H[i_5+i_3][i_2+2][i_6][i_2] + H[i_6+2][i_3][i_5+2][i_2-2] + D[i_5+1][i_3+2][i_6] + 2;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4412152325_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
