...
void kernel_4436212221_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        ...
        B[i_0-2] = C[i_0+1] - C[i_0+2] * C[i_0-1] * 6;
        C[i_0+1] = 5;
    }
    for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_M-2; i_3++) {
            D[i_2][i_3] = E[i_2+1][i_3] - B[i_3-2] - D[i_2+2][i_3] + D[i_2][i_3+2] + 5;
        }
    }
#pragma endscop
...
kernel_4436212221_08(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
