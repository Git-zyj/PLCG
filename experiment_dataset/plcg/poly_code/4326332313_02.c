...
void kernel_4326332313_02(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
        ...
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_P; i_3++) {
                    B[i_2+i_0][i_2][i_1-2][2] = D[i_3][i_1][i_0][i_2] + E[i_2+1][i_3+i_0][i_1][i_3] * B[i_2+i_0-1][i_0-1][i_1-2][2] - 3;
                }
            }
        }
        for (int i_4 = 3; i_4 < min(-i_0+PB_M-1, i_0); i_4++) {
            for (int i_5 = 2; i_5 < min(-i_0+PB_M, i_0); i_5++) {
                B[i_5+i_0][i_0][i_4-2][2] = F[i_5-1][i_0][i_4] - G[i_5][i_4] - F[i_4-1][i_5][i_0] + B[i_5+i_0-2][i_0-1][i_4-3][2] + B[i_4+i_0+1][i_4][i_5-2][2] * 4;
            }
        }
    }
    for (int i_6 = 3; i_6 < PB_P-2; i_6++) {
        for (int i_7 = 1; i_7 < PB_P; i_7++) {
            for (int i_8 = i_6; i_8 < -i_7+PB_P+1; i_8++) {
                for (int i_9 = 0; i_9 < min(-i_7+PB_M, -i_6+PB_Q); i_9++) {
                    B[i_9+i_7][i_7][i_6-3][2] = H[i_8-2][i_8][i_9+i_6-1][i_7] + E[0][i_6+2][i_8+i_7-1][i_9+i_6] - H[i_9][i_6][i_7][i_8] + E[i_9][i_8][i_8][i_7] - 1;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4326332313_02(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
