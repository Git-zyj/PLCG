...
void kernel_2412132213_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M-2; i_1++) {
            A[i_1-2][i_0] = E[i_0+2] * F[i_1+2][i_0] + A[i_1-1][i_0] - A[i_1-2][i_0-1] * 4;
        }
        for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
            B[i_0][i_2][i_0] = F[i_2][i_0] * B[i_2+1][i_0][i_2+1] - B[i_0+1][i_2+1][i_0+1] - B[i_0][i_2-1][i_0] * B[i_2-1][i_0][i_2-1] * B[i_0][i_2+1][i_0] - 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_L, PB_P-1); i_4++) {
            C[i_3][i_4+1] = E[i_4+i_3] + F[i_4+i_3-1][i_3+1] * F[i_4-2][i_4+i_3] - F[i_4+2][i_3] - F[i_4][0] * 4;
        }
        for (int i_5 = 3; i_5 < PB_M; i_5++) {
            D[i_5-2][i_3] = F[i_5][i_3] - D[i_5-3][i_3+1] + 6;
        }
    }
#pragma endscop
...
kernel_2412132213_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
