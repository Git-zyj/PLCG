...
void kernel_4212112125_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-4; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P-2; i_1++) {
            A[i_0+2][i_1] = C[i_1][i_0-1] + C[i_1][i_0-2] * A[i_0+2][i_1+2] - 3;
            for (int i_2 = 0; i_2 < -i_0+PB_L-4; i_2++) {
                B[i_1+1][i_2+i_0+2][i_0+1] = B[i_1+1][i_2+i_0+4][i_0+3] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4212112125_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
