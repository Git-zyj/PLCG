...
void kernel_4416131315_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_M, i_1); i_2++) {
                B[i_1-1][i_0-2][i_2-2] = E[i_2+i_0][i_2+1][i_1] * E[i_1+i_0][i_2-1][i_1] * 1;
            }
            for (int i_3 = 2; i_3 < -i_1+PB_M; i_3++) {
                B[i_1-1][i_0-1][i_3-1] = E[i_0][i_3-1][i_1] * E[i_3+i_1][i_3][i_0] * B[i_1-1][i_0-2][i_3-2] - B[i_1-1][i_0-1][i_3] * 3;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 2; i_5 < -i_4+PB_L; i_5++) {
            for (int i_6 = i_5 - 1; i_6 >= 1; i_6--) {
                B[i_5-2][i_4-1][i_6-1] = F[i_6+1][i_5+i_4] + E[i_4+1][i_5+i_4][i_6] * F[i_4][i_5] + E[i_4][i_6][i_5] * E[i_6][i_4][i_4+1] * D[i_5-1][i_6][i_4][i_4] - D[i_6][i_4][i_5][i_5] - 5;
                for (int i_7 = 0; i_7 < -i_5+PB_L; i_7++) {
                    D[i_5][i_7][i_6][i_6] = G[i_6][i_7+i_5][i_4][i_5-1][i_6-1] - H[i_4][i_7][i_5+1][i_6] * D[i_7][i_4][i_6-1][i_6-1] + 6;
                }
            }
        }
    }
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4416131315_09(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
