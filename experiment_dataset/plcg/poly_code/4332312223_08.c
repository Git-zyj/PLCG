...
void kernel_4332312223_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_N; i_3++) {
        B[i_3] = D[i_3] - E[0] - E[i_3] + 6;
        for (int i_4 = 0; i_4 < PB_Q-2; i_4++) {
            for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
                C[i_3][i_5][i_4+1] = C[i_3][i_5+1][i_4+1] + C[i_3][i_5-1][i_4+2] - C[i_5][i_3-2][i_4+2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4332312223_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
