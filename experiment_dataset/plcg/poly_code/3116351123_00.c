...
void kernel_3116351123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            A[i_1+1] = B[i_0+1][i_0-1] + C[i_1][i_0] * B[i_1][i_0-1] * D[i_1][i_0-1] + B[i_1][i_0] * A[i_1] * 5;
        }
    }
#pragma endscop
...
kernel_3116351123_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
