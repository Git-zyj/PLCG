...
void kernel_3224311321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M-3, PB_L-2); i_2++) {
                A[i_2+i_1][i_2] = C[i_2+i_1][i_0][i_0+1] + A[i_2+i_1+2][i_2+2] * A[i_2+i_1+3][i_2+1] * A[i_2+i_1-1][i_1] * A[i_2+i_1+1][i_2+1] * 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_3+PB_M, PB_L-1); i_5++) {
                B[i_5+i_3][i_4+1][i_5] = D[i_4][i_5+1][i_3] - B[i_5+i_3-1][i_4][i_5-1] - B[i_5+i_3-1][i_4+1][i_3-1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3224311321_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
