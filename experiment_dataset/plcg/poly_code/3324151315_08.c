...
void kernel_3324151315_08(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M+1; i_1++) {
            C[i_1-2][i_1+i_0-1] = D[i_1] - E[i_0][i_1] + E[i_1-1][i_0-1] + D[i_0+1] + D[i_0] + D[0] * E[i_1][i_0+1] - E[i_1][i_0] * 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
                B[i_2][i_4][i_3] = F[i_2][i_4][i_3-1] - E[i_4+i_3][i_2] * E[i_4][i_2+1] * F[i_3][i_2-1][i_2+1] - F[i_2-1][i_4][i_3] + B[i_2-1][i_4-1][i_3+1] + B[i_2+1][i_4-1][i_3] + 2;
            }
        }
        for (int i_5 = 2; i_5 < -i_2+PB_M; i_5++) {
            C[i_2-1][i_5+i_2-1] = E[i_5+1][i_2] * E[i_5][i_2] * C[i_2-2][i_5+i_2-2] * C[i_2-2][i_5+i_2-1] * C[i_5-2][i_5+i_2-2] * C[i_2][i_5+i_2] - 4;
        }
    }
#pragma endscop
...
kernel_3324151315_08(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
