...
void kernel_4312132311_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_P; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_1+PB_L, -i_0+PB_P-1); i_3++) {
                    A[i_0][i_2][i_3-2][i_3+i_0+1] = D[i_3+i_1][i_1-2][i_0][0] * E[i_2+2][i_1+2][i_2+i_1] - D[i_2][i_3-1][i_1][i_0+2] * D[i_2+2][i_1][i_3+i_0][i_3+1] * A[i_2-1][i_0+1][i_1-2][i_2+i_1] - 2;
                }
            }
        }
        B[i_0] = F[i_0] * G[i_0-2][i_0] * F[i_0] * G[i_0-1][0] - F[i_0] * B[i_0-1] * B[i_0+1] + 2;
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 2; i_5 < PB_P-1; i_5++) {
            for (int i_6 = 3; i_6 < PB_Q; i_6++) {
                for (int i_7 = 0; i_7 < -i_5+PB_P-1; i_7++) {
                    C[i_4][i_5][i_6-2][i_6] = A[i_7][i_6][i_5-2][i_7+i_5+1] * C[i_6-1][i_7+1][i_5-2][i_5] + C[i_4+1][i_5][i_6-2][i_6] * C[i_4][i_5-1][i_6-3][i_6-1] * 3;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4312132311_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
