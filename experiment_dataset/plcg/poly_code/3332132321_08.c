...
void kernel_3332132321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 4; i_1 < PB_L-1; i_1++) {
            A[i_1-2][i_1] = D[i_1][i_0] * E[i_1-1] - A[i_1-4][i_1-2] - A[i_1-1][i_1+1] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_M-2; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_M, -i_2+PB_N); i_4++) {
                B[i_3][i_2][i_4] = F[i_4+i_3-1][i_2-1][i_2] + F[i_4+i_2][i_3+i_2+2][i_3+1] * D[i_4+i_3][i_2] - F[i_4][i_3-2][i_2-1] * B[i_3+2][i_2][i_4-1] * B[i_3+2][i_2-2][i_4] - 3;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 1; i_6 < -i_5+PB_M-1; i_6++) {
            C[i_6+i_5][i_6+1] = D[i_6][i_5] * E[i_6+1] * E[i_6] + C[i_6+i_5-2][i_6-1] - C[i_6+i_5-2][i_6+1] + C[i_6+i_5+1][i_6] - C[i_6+i_5-2][i_5+1] - 4;
        }
    }
#pragma endscop
...
kernel_3332132321_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
