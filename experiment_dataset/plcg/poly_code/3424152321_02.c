...
void kernel_3424152321_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] * E[i_0+2] - E[i_0+1] - B[i_0-1] * D[i_0] + D[i_0-2] * 2;
        B[i_0+1] = E[i_0] * E[i_0] + E[i_0] + E[i_0] * E[i_0] - E[i_0] + E[i_0] + E[i_0] + A[i_0] + B[i_0] - D[i_0] + 1;
    }
    for (int i_1 = 4; i_1 < PB_L+2; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_L, i_1); i_2++) {
            for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
                C[i_2][i_1-2][i_3] = F[i_2][i_1+1][i_3] + G[i_3][i_1][i_3-2][i_2-1] - F[i_3+1][i_1][i_2+1] + F[i_3-2][i_1][i_2+2] * C[i_2+2][i_1-2][i_3+2] + C[i_2+2][i_3-1][i_1-1] + C[i_2][i_1-4][i_3] * C[i_2][i_1-3][i_3] + 5;
            }
        }
        D[i_1] = E[i_1] - E[i_1+1] + E[i_1-2] - E[i_1-1] + E[i_1+2] + D[i_1-1] - 3;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3424152321_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
