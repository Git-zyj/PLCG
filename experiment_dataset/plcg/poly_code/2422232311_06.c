...
void kernel_2422232311_06(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        D[i_0-2] = E[i_0] + F[i_0+2] - 1;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        B[i_1] = F[i_1] + E[i_1] + B[i_1+1] + B[i_1-1] + B[i_1-1] + 3;
        for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_M-1); i_2++) {
            C[i_2+i_1] = E[i_1] * G[i_1][i_2] * H[i_2+i_1][i_1] - B[i_2] + C[i_2+i_1-1] * C[i_2+i_1+1] - C[i_2+i_1+1] + 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        D[i_3-2] = E[i_3] * E[i_3] * B[i_3] + 3;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_2422232311_06(xd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
