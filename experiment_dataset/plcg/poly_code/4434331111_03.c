...
void kernel_4434331111_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < min(min(min(-i_0+PB_L+1, -i_2+PB_Q), -i_1+PB_L), PB_M-1); i_3++) {
                    A[i_0][i_3][i_1][i_0-1] = E[i_2-1][i_0][i_1][i_3-1] + E[i_2][i_3+i_1][i_0][i_3+i_2] * F[i_2][i_3+i_0-1][i_3+i_1][i_0-1] + E[i_3-1][i_0][i_1][i_2] + A[i_0+1][i_3+1][i_1][i_0] - 4;
                }
            }
        }
    }
        ...
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            for (int i_6 = 1; i_6 < PB_M+1; i_6++) {
                for (int i_7 = 0; i_7 < PB_M-1; i_7++) {
                    A[i_4+1][i_7+1][i_5][i_4] = G[i_7][i_4+1][i_5-1][i_6+1] + F[i_6-1][i_7+1][i_4+1][i_5] * 6;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_N-1; i_8++) {
        C[i_8+1][i_8] = H[i_8] - H[i_8] * I[i_8] * C[i_8][i_8-1] * 4;
        for (int i_9 = 1; i_9 < i_8; i_9++) {
            for (int j_0 = 0; j_0 < min(PB_M, -i_9+PB_P); j_0++) {
                D[j_0][i_9-1][i_9][i_8] = J[i_8][j_0][j_0+i_9] - K[j_0][i_9] - L[j_0][i_8][i_9-1] + 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4434331111_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
