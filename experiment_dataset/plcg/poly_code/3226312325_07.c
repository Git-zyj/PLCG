...
void kernel_3226312325_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_0+1][i_1][i_1+1] = B[i_2][i_0-2][i_0-1] * B[i_1][i_0][i_0+1] + 5;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L; i_3++) {
            for (int i_4 = i_0; i_4 < PB_M-1; i_4++) {
                B[i_0][i_4][i_4+1] = C[i_4][i_0][i_3] + B[i_0-1][i_4][i_4+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3226312325_07(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
