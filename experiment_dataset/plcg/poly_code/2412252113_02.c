...
void kernel_2412252113_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_L-1); i_1++) {
            B[i_0][i_1+1] = E[i_1+i_0] + F[i_1][0] - F[i_0+2][i_1] - G[i_1][i_1] * H[i_0] - 1;
        }
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            B[i_0][i_2] = G[i_2][i_0] + I[i_2][i_0][2] * F[i_2][i_0] + G[i_0][i_2] * G[i_0+2][i_2] - F[i_2][i_0-1] + G[i_2+1][i_0] * F[i_0][i_2] - 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            B[i_3][i_4] = G[i_4][i_3-1] * D[i_4+2][0] - 2;
        }
        for (int i_5 = 0; i_5 < PB_P; i_5++) {
            D[i_5+2][0] = F[i_5][i_3] - G[i_5][0] * F[i_5][i_3] - E[i_5+2] + 6;
        }
    }
#pragma endscop
...
kernel_2412252113_02(xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
