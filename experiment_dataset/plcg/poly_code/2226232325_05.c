...
void kernel_2226232325_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(-i_0+PB_L, PB_M-2); i_1++) {
            A[i_1][0] = C[i_1+2][i_0+2] * C[i_0][i_1] - C[i_1][i_1+i_0] - D[i_1-1] + C[i_0][i_1] - A[i_1-1][0] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            A[i_3-1][0] = C[i_3][i_2-2] * A[i_2][0] - A[i_3][0] * 1;
        }
    }
#pragma endscop
...
kernel_2226232325_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
