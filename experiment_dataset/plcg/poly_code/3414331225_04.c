...
void kernel_3414331225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - A[i_0+1] * 6;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_1] = F[i_1][i_0+1] - G[i_0+1][i_1] * A[i_1] - 2;
        }
        A[i_0] = H[i_0] + G[i_0+1][i_0-1] - H[i_0] + H[i_0] * 5;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
                A[i_4+2] = I[i_4][i_2][i_3] * I[i_4][i_2][i_4] - I[i_4+1][i_2][i_2] * J[i_3][i_2][i_3] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3414331225_04(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
