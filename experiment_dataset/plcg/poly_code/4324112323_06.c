...
void kernel_4324112323_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            C[i_1+1][i_2-2] = A[i_2] + 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_N+2; i_4++) {
            C[i_3][i_4-2] = D[i_4-2] * C[i_3+1][i_4-2] - 5;
        }
    }
#pragma endscop
...
kernel_4324112323_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
