...
void kernel_2122252215_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_N-1, i_0); i_1++) {
            A[i_1] = B[i_1+2][i_0] - B[i_0][0] + B[i_0-2][i_1] + B[2][i_1-1] - C[i_1-2] * A[i_1-1] + A[i_1+1] * 1;
        }
    }
#pragma endscop
...
kernel_2122252215_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
