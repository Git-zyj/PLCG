...
void kernel_2236312325_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 4; i_1 < PB_M-2; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L; i_2++) {
            B[i_1][i_1-2] = D[i_2][i_1-1] * B[i_1-1][i_1-3] - B[i_1+2][i_1] * B[i_1-2][i_1-4] * 5;
        }
    }
#pragma endscop
...
kernel_2236312325_02(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
