...
void kernel_2334112315_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[i_2] = D[i_2-1][i_2+2][i_3] - B[i_3+1] * B[i_2-1] + B[i_3-1] + 3;
        }
    }
...
kernel_2334112315_01(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D));
