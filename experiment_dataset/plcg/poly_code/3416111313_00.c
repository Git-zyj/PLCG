...
void kernel_3416111313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = A[i_0+1] * A[i_0-1] - D[i_0] - 3;
        A[i_0] = A[i_0-1] * C[i_0+1] * 3;
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            C[i_0] = E[i_0][i_1] * E[i_1+1][i_0] - C[i_1+1] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            D[i_3] = E[i_3+1][i_2] * E[i_3-1][i_2] + D[i_3-1] * D[i_3+1] * 6;
        }
    }
#pragma endscop
...
kernel_3416111313_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
