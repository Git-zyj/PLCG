...
void kernel_2414351313_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        B[i_0] = E[0] + F[0] - G[i_0] * D[i_0] * 3;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        B[i_1] = F[i_1] * G[i_1] - F[i_1] - F[i_1] * E[i_1+1] * B[i_1+1] - B[i_1-1] * D[i_1-1] + 4;
        for (int i_2 = 1; i_2 < min(PB_M-1, -i_1+PB_L+1); i_2++) {
            C[i_1-1][i_2] = E[i_1] - H[i_2][i_1] - H[i_2+1][i_1] * I[i_1][i_2] * H[i_2+1][i_1] - I[i_2+i_1-1][i_1] - H[i_2][i_1] * B[i_1-1] + B[i_1] * 6;
            D[i_2] = I[i_2][i_1] * I[i_2-1][i_1] * J[i_2-1][i_1+1] + C[i_2-1][i_1] + C[i_1-2][i_2] + 4;
        }
    }
#pragma endscop
...
kernel_2414351313_07(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
