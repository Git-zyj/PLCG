...
void kernel_3324312311_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            A[i_1+2] = A[i_1+3] - B[i_0] * B[i_0-1] - 1;
        }
        B[i_0] = D[0] + B[i_0-1] - 4;
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            C[i_2][i_3] = B[i_2+1] - B[i_3] - C[i_2+1][i_3+1] * C[i_2][i_3-1] * 6;
        }
    }
#pragma endscop
...
kernel_3324312311_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
