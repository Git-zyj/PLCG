...
void kernel_2224112325_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < min(min(-i_2+PB_L, PB_P), i_2); i_3++) {
            B[i_3] = C[i_3+i_2][i_3] - B[i_3-1] + B[i_3+1] - B[i_3-2] * 3;
        }
    }
#pragma endscop
...
kernel_2224112325_07(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
