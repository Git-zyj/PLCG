...
void kernel_4334331113_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                for (int i_3 = i_2; i_3 < min(-i_2+PB_L+1, -i_1+PB_L+1); i_3++) {
                    A[i_1][i_1][i_3+i_2-1][i_0] = D[i_3][i_0][i_1+1][i_1] + E[i_2][i_3][i_0][i_1] - A[i_2+1][i_2+1][i_3+i_1-1][i_0] * 2;
                }
            }
        }
        for (int i_4 = 1; i_4 < -i_0+PB_N; i_4++) {
            for (int i_5 = 0; i_5 < min(min(-i_0+PB_M, -i_4+PB_M-1), PB_Q-1); i_5++) {
                B[i_4+i_0][i_5+i_0][i_5+i_4+1] = F[i_5][i_5+1][i_0] + G[i_5][i_4-1] + F[i_4-1][i_0][i_5] - 1;
            }
        }
    }
        ...
        for (int i_7 = 1; i_7 < PB_N; i_7++) {
            for (int i_8 = 1; i_8 < i_6; i_8++) {
                for (int i_9 = 0; i_9 < PB_P; i_9++) {
                    C[i_7][i_9][i_8+1][i_6] = H[i_8-1][i_7-1][i_6] * E[i_7][i_6+1][i_9][i_8] + 4;
                }
            }
        }
    }
...
kernel_4334331113_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
