...
void kernel_3234311315_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = 2; i_2 < i_0; i_2++) {
            for (int i_3 = 1; i_3 < i_2; i_3++) {
                B[i_0-1][i_3][i_2-1] = C[i_3][i_0-1][i_2] * A[i_2+1][i_3-1] * B[i_0-2][i_3][i_2-2] + B[i_0-1][i_3][i_2-2] + B[i_0-1][i_3+1][i_2-1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3234311315_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
