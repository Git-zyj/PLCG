...
void kernel_2422311313_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            B[i_2-1] = E[i_1][0] * F[i_1-1] + B[i_2-2] + 2;
        }
        D[i_1] = F[i_1-1] - B[i_1-1] * B[i_1] * B[i_1-2] - 3;
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        D[i_3] = F[i_3] + D[i_3+1] - D[i_3-1] * 5;
    }
#pragma endscop
...
kernel_2422311313_08(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
