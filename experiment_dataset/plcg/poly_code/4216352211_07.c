...
void kernel_4216352211_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(-i_0+PB_Q, PB_M-1); i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_Q, -i_1+PB_Q); i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_0+PB_P, -i_1+PB_Q-2), i_1); i_3++) {
                    A[i_0][i_3][i_3+i_1][i_2+i_0] = C[i_2-1][i_1-1][i_3][i_1+i_0] * D[i_2][i_0+2][i_1+2][i_2-2] + C[i_3+i_2][i_1+1][i_0-1][i_1] + D[i_2-2][i_1-2][i_3+i_0][i_3+1] * C[i_2-2][i_3][i_1][i_0] - A[i_0][i_3][i_3+i_2+1][i_1+i_0] + A[i_2][i_3+1][i_3+i_1+2][i_2+i_0-1] + A[i_3][i_1][i_2+i_1][i_3+i_0-1] * 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        B[i_4-1] = E[i_4+1] - F[i_4+2] * G[i_4+2] + G[i_4] - 6;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4216352211_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G)));
