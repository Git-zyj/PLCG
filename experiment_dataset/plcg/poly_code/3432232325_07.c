...
void kernel_3432232325_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[0] = E[i_0][i_0] * E[i_0-2][i_1] - E[i_0][i_1+2] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            B[0] = F[i_3+2][i_2-1] - G[i_3][i_2+2][0] - B[0] + B[0] - B[0] - 4;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        B[0] = H[i_4] - I[i_4] * H[i_4+1] - H[i_4] - 5;
        for (int i_5 = 0; i_5 < min(PB_M, -i_4+PB_L); i_5++) {
            B[0] = E[i_5+1][i_4] - E[i_4+1][i_5+i_4] - F[i_5][i_4] + 6;
        }
    }
#pragma endscop
...
kernel_3432232325_07(xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
