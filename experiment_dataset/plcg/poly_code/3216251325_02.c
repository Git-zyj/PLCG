...
void kernel_3216251325_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            A[i_0][i_1] = C[i_1][i_0] - D[i_1] - E[i_1-1][i_0] * E[i_1][i_0+1] * E[i_1][i_0] * D[i_1] + A[i_0+1][i_1] - A[i_0+2][i_1] * A[i_0][i_1-2] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_M; i_3++) {
            A[i_3][i_2+2] = E[i_3][i_2] - C[i_3+i_2][i_2] * C[i_3][i_2] * 3;
        }
    }
#pragma endscop
...
kernel_3216251325_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
