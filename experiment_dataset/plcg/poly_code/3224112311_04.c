...
void kernel_3224112311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                A[i_1-1][i_2+i_0] = A[i_1-1][i_2+i_0-1] - B[i_2+1] - 4;
            }
        }
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            B[i_3+1] = C[i_0][i_3] + B[i_3+2] + B[i_3] * B[i_3] * B[i_3] * 6;
        }
    }
#pragma endscop
...
kernel_3224112311_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
