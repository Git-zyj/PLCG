...
void kernel_3426352325_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_L+2, PB_P); i_1++) {
            A[i_1+2][i_0][i_1] = E[i_0][i_1+i_0-2] * F[i_1][i_0] * A[i_1+2][i_0+2][i_1] + A[i_0][i_1][i_0-2] * A[i_1+2][i_0-2][i_1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        B[i_2] = G[i_2] + H[i_2] - I[i_2] + H[i_2-1] - I[i_2] + B[i_2-1] * 1;
    }
    for (int i_3 = 2; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M+1, i_3); i_4++) {
            C[i_4+2][i_4] = J[i_4+1][i_3] + G[i_3] * F[i_3][i_4-2] * E[i_4+i_3-1][i_4] + K[i_4][i_3][i_4-1] - J[i_4][i_3] * B[i_4+1] + B[i_4-2] + C[i_4][i_4-2] - C[i_4+4][i_4+2] + 4;
        }
        for (int i_5 = 2; i_5 < i_3; i_5++) {
            for (int i_6 = 0; i_6 < min(-i_5+PB_M-3, i_5); i_6++) {
                D[i_6+i_5][i_6][i_3][i_6] = K[i_6][i_3][i_3] * K[i_6+1][i_3-2][i_5+2] * L[i_6][i_3-2][i_5] * K[i_3][i_5][i_3+2] - E[i_6][i_5] - C[i_6+2][i_6] + D[i_6+i_5+3][i_6+2][i_3-2][i_6+2] * D[i_6+i_5-2][i_6][i_3][i_6] * 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_3426352325_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
