...
void kernel_2434212213_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            A[i_0][i_1] = A[i_0][i_1+1] - A[i_0][i_1-1] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_L+1, PB_M); i_3++) {
            B[i_2][i_3] = E[i_2][i_3+i_2-1] - F[i_2] + 6;
        }
    }
    ...
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = i_5; i_6 < min(-i_5+PB_M, PB_M-2); i_6++) {
            D[i_6+1] = G[i_5][0] * G[i_5][i_6+i_5] * D[i_6] + D[i_6+2] + 3;
        }
    }
#pragma endscop
...
kernel_2434212213_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
