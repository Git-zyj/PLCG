...
void kernel_3232112311_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_L; i_3++) {
            for (int i_4 = 0; i_4 < -i_2+PB_P; i_4++) {
                B[i_3][i_4+i_2][0] = C[i_3+i_2][i_4][i_3+2] * B[i_4+1][i_3+i_2][0] * B[i_3+1][i_4+i_2-1][0] - B[i_3-1][i_4+i_2-2][0] - B[i_3][i_4+i_2-1][0] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3232112311_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
