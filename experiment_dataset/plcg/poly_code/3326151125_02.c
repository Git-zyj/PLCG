...
void kernel_3326151125_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            A[i_1] = D[i_1+1] + E[i_1+i_0-1][i_1] * E[i_1][0] * E[i_1][i_1+i_0] + D[i_1+i_0] + E[i_0+1][i_1-1] + A[i_1+2] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        B[i_2][i_2] = B[i_2+2][i_2+2] - 6;
        B[i_2-1][i_2-1] = D[i_2] * D[i_2] + D[i_2] + D[i_2] + D[i_2] + D[i_2] * D[i_2+1] + 3;
    }
#pragma endscop
...
kernel_3326151125_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
