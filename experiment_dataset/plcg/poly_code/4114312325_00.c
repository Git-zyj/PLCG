...
void kernel_4114312325_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-4; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = max(1, i_0); i_2 < PB_N-3; i_2++) {
                A[i_0+2][i_2][i_2] = B[i_0][i_1][i_0] + A[i_0+4][i_1][i_1] - A[i_2+3][i_0][i_0] + A[i_0+2][i_2-1][i_2-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4114312325_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
