...
void kernel_2334112323_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_1] = D[i_0][i_1] + A[i_0-1][i_1] * A[i_1][i_0-1] - A[i_0][i_1+1] + 2;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_2][i_0] = B[i_2-1][i_0+1] * B[i_2][i_0-1] - 3;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
            C[i_3+2][i_4+1] = D[i_3][i_4-2] * A[i_3][i_4] * C[i_3+2][i_4-1] * 1;
        }
    }
#pragma endscop
...
kernel_2334112323_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
