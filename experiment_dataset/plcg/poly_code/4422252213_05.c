...
void kernel_4422252213_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0] + F[i_0][0] + A[i_0+1] * 4;
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            B[i_0][i_1] = F[i_1+1][2] - F[i_1-1][i_0+2] - F[i_0+2][i_1] + E[i_0-1] * F[i_0+1][i_1] - G[i_1+2][i_0+1] + A[i_1] - B[i_0-1][i_1] * 2;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_L+1; i_2++) {
            A[i_0-1] = G[i_0][i_2] + F[i_0-1][i_0] - F[i_2-1][i_2+i_0-1] - F[i_2][i_0-1] + F[i_2+i_0-2][i_2] + 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_Q; i_3++) {
        for (int i_4 = 0; i_4 < PB_N; i_4++) {
            for (int i_5 = 1; i_5 < PB_N; i_5++) {
                B[i_5-1][i_3] = H[i_4][i_3][2] - F[i_5][i_4+1] * I[i_5][i_3][i_4] - G[i_5+2][i_4] - I[i_4][i_3-1][i_4] * 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4422252213_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
