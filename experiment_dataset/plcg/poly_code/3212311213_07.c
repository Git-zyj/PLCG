...
void kernel_3212311213_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_N, -i_0+PB_L); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_L-1, PB_M); i_2++) {
                A[i_1-1][i_2][i_1+i_0] = C[i_0][i_2] - A[i_1][i_0][i_2+i_1+1] * A[i_1-2][i_0][i_2+i_1-1] * 5;
            }
        }
    }
...
kernel_3212311213_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
