...
void kernel_3422152225_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[2] = E[i_1][i_0] * E[i_1][i_0] * A[2] * A[2] * 2;
        }
        B[i_0] = F[i_0] * F[i_0] * F[i_0+1] - F[i_0] - F[i_0] + A[2] + B[i_0+2] + B[i_0-2] * 6;
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        C[i_2] = F[i_2] + F[i_2+1] + F[i_2] * F[i_2-1] * F[i_2] * 2;
        D[i_2][i_2+1] = E[i_2][1] * F[i_2] * E[i_2][i_2] + F[i_2] * F[i_2] - F[i_2] - F[i_2+2] + A[2] * C[i_2+1] * D[i_2-2][i_2-1] + 4;
    }
#pragma endscop
...
kernel_3422152225_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
