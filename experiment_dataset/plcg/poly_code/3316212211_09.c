...
void kernel_3316212211_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_0-1][i_1] = D[i_1-2][i_0+2] + A[i_0][i_1] + 5;
            B[i_0][i_1] = A[i_0-1][i_1] * B[i_0-1][i_1] * B[i_0][i_1-1] - 3;
        }
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            C[i_0+1][i_2] = D[i_0][i_2] + C[i_0+2][i_2] + C[i_0+1][i_2-1] - 5;
        }
    }
#pragma endscop
...
kernel_3316212211_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
