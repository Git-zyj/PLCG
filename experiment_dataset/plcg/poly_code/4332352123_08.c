...
void kernel_4332352123_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 3; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_P+2, PB_Q-2); i_2++) {
                for (int i_3 = 2; i_3 < min(min(-i_0+PB_Q, -i_1+PB_L), -i_2+PB_Q-1); i_3++) {
                    A[i_0][i_0][i_2+i_1-2][i_3+i_2+1] = D[i_2-2][i_2+1][i_0][i_1] * E[i_3][i_0][i_2+2][i_3+2] + F[i_3+i_1][i_0][i_2][i_3+i_0] * F[i_2][i_1][i_3-1][i_0+1] - G[i_3][i_1+2][i_0-2][i_3][i_2] + F[i_1][i_3][i_2][i_0] * F[i_2][i_1+1][i_0+2][i_3-2] - A[i_0-2][i_0-2][i_2+i_1-3][i_3+i_1+1] * 1;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_P-3; i_4++) {
        for (int i_5 = i_4; i_5 < -i_4+PB_P-3; i_5++) {
            B[i_5-2][i_5+i_4+1][i_4] = H[i_5] - I[i_4][i_5] + I[i_5][i_4] * I[i_4+2][i_5] + B[i_4-3][i_5+i_4][i_5] - B[i_4-2][i_5+i_4+3][i_5+2] - 3;
        }
    }
    for (int i_6 = 0; i_6 < PB_P-2; i_6++) {
        for (int i_7 = 2; i_7 < min(-i_6+PB_M, PB_L-2); i_7++) {
            C[i_7+2][i_6+2] = J[i_6+1][i_7+i_6] - H[i_7] - I[i_7-2][i_6] + K[i_7+i_6][i_6] - 2;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4332352123_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
