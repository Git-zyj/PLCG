...
void kernel_4312311223_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
                A[i_0][i_1][i_0] = D[i_2-1][i_1] - A[i_0+1][i_1][i_0+1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-3; i_4++) {
            B[i_4+1][i_3] = B[i_4+3][i_3] * B[i_4][i_3] + B[i_4][i_3+2] - 3;
        }
        for (int i_5 = 0; i_5 < i_3; i_5++) {
            for (int i_6 = 0; i_6 < PB_N-1; i_6++) {
                C[i_3][i_5][i_6] = D[i_6][i_5] + C[i_5][i_6+1][i_3+2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4312311223_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
