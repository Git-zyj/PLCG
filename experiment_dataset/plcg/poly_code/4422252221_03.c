...
void kernel_4422252221_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = E[i_0-2] * E[i_0] + F[i_0+2] * E[i_0] - A[i_0+4] - D[i_0] + 2;
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            for (int i_2 = i_1; i_2 < -i_1+PB_M; i_2++) {
                for (int i_3 = 4; i_3 < PB_P-2; i_3++) {
                    B[i_0-1][i_1-2][i_2][i_0] = G[i_1][i_2+1][i_0-2][i_1-2] + H[i_1][i_0][i_3][1] * I[i_2][i_1+2][i_0+1][i_3+2][i_1-1] * H[i_0][i_2+i_1][i_3][i_0] * J[i_3][i_1][i_2-1] + G[i_0][i_2][i_2][i_1-1] - H[i_1+2][i_0][i_1][i_0+1] * B[i_0-3][i_3-4][i_2-2][i_0-2] * 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 0; i_6 < min(min(-i_5+PB_L, i_5), PB_L-2); i_6++) {
                C[i_5][i_6+2][i_4][i_4] = K[i_5+1][i_6][i_4-2] - J[i_6+2][i_4][i_5] - K[i_6][i_6+i_5][i_5] * C[i_5+1][i_6+2][i_4][i_4] - 2;
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_M-1; i_7++) {
        D[i_7] = E[i_7] + E[i_7-2] * F[i_7] - F[i_7-2] * D[i_7+1] - D[i_7-2] + 4;
    }
#pragma endscop
...
kernel_4422252221_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
