...
void kernel_3232252125_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P-2, -i_0+PB_P); i_1++) {
            A[i_0][i_1+2] = C[i_0][i_1+i_0] + C[i_0+2][i_1] - D[i_0+1][2] + C[i_1][i_0+2] + D[i_1][i_0] * D[i_1][i_0+1] - C[i_0][i_1] + C[i_1][i_0+1] - A[i_0+1][i_1+2] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            A[i_3][i_2+2] = C[i_3][i_2] + C[i_3][i_2-2] * 6;
        }
    }
#pragma endscop
...
kernel_3232252125_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
