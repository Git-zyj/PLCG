...
void kernel_2316111313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_N-1, i_1); i_2++) {
            B[i_1][i_2] = E[i_2-1][i_1] - B[i_1-1][i_2] * B[i_1][i_2-1] + B[i_1][i_2+1] + B[i_1-1][i_2-1] - 6;
        }
        C[i_1] = C[i_1+1] - 6;
    }
#pragma endscop
...
kernel_2316111313_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
