...
void kernel_2316312313_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_M; i_3++) {
            B[i_2][i_3-1] = B[i_2-1][i_3-2] * B[i_2][i_3] + B[i_2+1][i_3-1] * 1;
        }
        for (int i_4 = 3; i_4 < i_2; i_4++) {
            C[i_4-2][i_2+1] = B[i_2][i_4-1] + B[i_2-1][i_4-1] + C[i_4-3][i_2+1] * 3;
        }
    }
#pragma endscop
...
kernel_2316312313_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
