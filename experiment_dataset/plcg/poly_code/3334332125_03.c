...
void kernel_3334332125_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        B[0] = D[i_0-2] * D[i_0] * E[i_0] + 1;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        B[0] = 2;
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            B[0] = F[i_2-2][i_1][i_2] - E[i_2] - G[i_2][i_2] + E[i_2-2] + H[i_2+2][i_1] + B[0] * 6;
        }
    }
#pragma endscop
...
kernel_3334332125_03(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
