...
void kernel_2424351125_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = E[i_0] - F[i_0-1] - F[i_0+1] * F[i_0] + F[i_0] * E[i_0+1] * B[i_0] - B[i_0-1] + 3;
        B[i_0] = G[i_0][1] * H[i_0] * H[i_0] * E[i_0] * H[i_0] * E[i_0] * B[i_0-2] + 6;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        C[i_1] = G[i_1][i_1] - H[i_1] - H[i_1-1] * I[i_1-1][i_1-1] + E[i_1] * 4;
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N; i_3++) {
            C[i_3+1] = I[i_3+1][i_2] - G[i_3][i_2-1] + G[i_3+i_2][i_3] * 5;
        }
    }
#pragma endscop
...
kernel_2424351125_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
