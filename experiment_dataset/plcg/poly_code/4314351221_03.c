...
void kernel_4314351221_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-3; i_0++) {
        A[i_0+1] = D[i_0+1] + E[i_0] + F[i_0-1] * E[0] + D[i_0] * A[i_0+3] + A[i_0-1] - 6;
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                for (int i_3 = 0; i_3 < PB_Q-1; i_3++) {
                    B[i_0][i_1-1][i_3+1][i_0] = G[i_0-1][i_2][i_1][i_3][i_3] - H[i_2+1][i_0][i_3+1][i_0-1] * H[i_3][i_0][i_1][i_2] * B[i_0-1][i_1-1][i_3+1][i_0-1] + 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 3; i_5 < min(PB_Q-3, -i_4+PB_P); i_5++) {
            for (int i_6 = 1; i_6 < min(min(-i_4+PB_L+1, -i_5+PB_M), PB_L-1); i_6++) {
                C[i_4][i_6+i_5-1][i_4] = I[i_6+1][i_4] * J[i_4-1][i_6-1][i_5] - K[i_4-1][i_6][i_5+i_4] + J[i_5][i_6][i_4] + L[i_4][i_5][i_5] - L[i_5][i_6+i_4-1][i_5] - B[i_4][i_6-1][i_5+3][i_4] * C[i_4][i_6+i_5][i_4] - C[i_4][i_6+i_5-3][i_4] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4314351221_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
