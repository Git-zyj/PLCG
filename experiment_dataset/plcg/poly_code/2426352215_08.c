...
void kernel_2426352215_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_L-1, PB_M); i_2++) {
            B[i_1-2][i_2+i_1] = G[i_1+1][i_2-2] - H[i_2][i_1+2] - I[i_2][i_1+2] + B[i_1-2][i_2+i_1-1] * B[i_1-3][i_2+i_1-1] + B[i_1-2][i_2+i_1+1] * B[i_1-1][i_2+i_1+1] + 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        A[i_3] = E[i_3-1] * J[i_3] - E[i_3] + E[i_3] + J[0] - 2;
        for (int i_4 = i_3; i_4 < min(-i_3+PB_L, PB_M); i_4++) {
            A[i_4+1] = I[i_4][i_4+i_3] - E[2] - I[i_3][i_4] + E[i_3+2] - H[i_4][i_3] - I[i_4][0] - H[i_4][0] - 4;
        }
    }
#pragma endscop
...
kernel_2426352215_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
