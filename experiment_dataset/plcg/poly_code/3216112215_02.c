...
void kernel_3216112215_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_P-1, -i_0+PB_L+1); i_2++) {
                A[i_0+2][i_1-1][i_2] = C[i_1][i_0+1][i_1+2] * C[i_1][i_2+i_0-1][i_0] - A[i_0+1][i_1][i_2] * A[i_0+2][i_1][i_2+1] - 3;
            }
        }
    }
...
kernel_3216112215_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
