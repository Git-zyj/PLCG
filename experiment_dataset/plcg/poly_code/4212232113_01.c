...
void kernel_4212232113_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0+2] = C[i_0][i_1] + D[i_1][i_0] + C[i_0][i_1] - A[i_0+3] * 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            A[i_2+2] = E[i_3] + D[i_2-2][i_3+1] + C[i_2-1][i_3-1] * 6;
        }
    }
#pragma endscop
...
kernel_4212232113_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
