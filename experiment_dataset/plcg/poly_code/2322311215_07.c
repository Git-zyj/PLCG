...
void kernel_2322311215_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0+1][i_0] = D[0] + A[i_0][i_0-1] - 5;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        A[i_1][i_1-1] = 3;
        for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
            C[i_1-1][i_2] = C[i_1-1][i_2-1] + C[i_1][i_2-1] * C[i_1-1][i_2+1] * 6;
        }
    }
#pragma endscop
...
kernel_2322311215_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
