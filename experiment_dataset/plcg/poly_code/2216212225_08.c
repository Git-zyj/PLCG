...
void kernel_2216212225_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            A[i_1+1] = A[i_1+2] + A[i_1] - A[i_1+3] - 3;
        }
        for (int i_2 = i_0; i_2 < min(-i_0+PB_M, PB_M-2); i_2++) {
            B[i_2] = C[i_2+i_0] - B[i_2+2] - 6;
        }
    }
#pragma endscop
...
kernel_2216212225_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
