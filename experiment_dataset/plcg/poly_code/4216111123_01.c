...
void kernel_4216111123_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L; i_2++) {
                A[i_0][i_0-1][i_1] = A[i_0+2][i_0+1][i_1] * 2;
            }
        }
        for (int i_3 = 2; i_3 < -i_0+PB_N-1; i_3++) {
            for (int i_4 = 0; i_4 < min(min(-i_3+PB_L, PB_M), -i_0+PB_N+3); i_4++) {
                for (int i_5 = 0; i_5 < min(min(-i_3+PB_L, PB_M), -i_4+PB_N); i_5++) {
                    B[i_0+1][i_3+i_0+1][i_4][i_5+i_3-1] = C[i_5+i_4][i_0+1][i_4+i_3][0] + C[i_3][i_0][i_5+i_3][i_4] + B[i_0-1][i_4+i_0-3][i_5][i_4+i_3-2] + 4;
                }
            }
        }
    }
...
kernel_4216111123_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
