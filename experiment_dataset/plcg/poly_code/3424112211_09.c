...
void kernel_3424112211_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = B[i_0] + B[i_0+1] * B[i_0-1] * B[i_0+1] * 2;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_1] = E[i_0][i_1] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        C[i_2] = F[i_2+1] - F[i_2] - E[i_2][0] * B[i_2] + 3;
    }
    for (int i_3 = 3; i_3 < PB_L+2; i_3++) {
        for (int i_4 = 3; i_4 < PB_P+2; i_4++) {
            D[i_3-2][i_4-2] = D[i_4-3][i_3-3] * D[i_3-3][i_4-2] * 1;
        }
    }
#pragma endscop
...
kernel_3424112211_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
