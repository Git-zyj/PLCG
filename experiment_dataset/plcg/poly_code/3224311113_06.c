...
void kernel_3224311113_06(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = -i_0+PB_P - 1; i_1 >= 0; i_1--) {
            for (int i_2 = 0; i_2 < min(PB_Q, -i_0+PB_P-1); i_2++) {
                B[i_0][i_2][i_2][i_1+i_0] = C[i_2][i_1][i_0] + 5;
                B[i_2][i_1][i_1][i_2+i_0] = D[i_2][i_0+1][i_2] * B[i_2+1][i_1][i_1][i_2+i_0+1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3224311113_06(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
