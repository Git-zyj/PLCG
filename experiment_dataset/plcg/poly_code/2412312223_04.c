...
void kernel_2412312223_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_M; i_1++) {
        ...
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            C[i_2][i_1-1] = D[i_2+2][i_2+i_1] - D[i_2+1][i_2+i_1-1] + 4;
        }
        for (int i_3 = 0; i_3 < min(-i_1+PB_M, i_1); i_3++) {
            D[i_3+2][i_3+i_1] = C[i_3][i_1-3] - 5;
        }
    }
#pragma endscop
...
kernel_2412312223_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
