...
void kernel_4426212223_08(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
        ...
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
                    D[i_3-1][i_0-1][i_2][i_2] = E[i_1][i_3][i_2][i_0] - 1;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            B[i_0+2][i_0+2] = B[i_0+4][i_0+4] - B[i_0+3][i_0+3] * 6;
        }
    }
        ...
        for (int i_6 = i_5; i_6 < PB_L; i_6++) {
            for (int i_7 = i_5; i_7 < PB_Q-1; i_7++) {
                for (int i_8 = 2; i_8 < PB_P; i_8++) {
                    D[i_5][i_7-1][i_8][i_8] = F[i_5-2][i_6][i_7+1][i_8] * D[i_5+2][i_6-1][i_8-2][i_8-2] + 2;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4426212223_08(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F)));
