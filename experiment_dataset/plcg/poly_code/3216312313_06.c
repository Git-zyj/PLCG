...
void kernel_3216312313_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_0][i_2][0] = C[i_2][i_0][0] * 5;
            }
            A[i_0][i_1+1][0] = A[i_0+1][i_1][0] + A[i_1+1][i_0-1][0] + A[i_0][i_1-1][0] - 2;
        }
    }
#pragma endscop
...
kernel_3216312313_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
