...
void kernel_3214252313_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q; i_1++) {
            A[i_0][i_1] = C[i_1-2][i_0] - D[i_1][i_0] * C[i_0][i_1] * D[i_1][i_0] + A[i_0-1][i_1] + A[i_0+1][i_1] * 1;
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_P); i_2++) {
                B[i_2+2][i_1][i_0+1] = E[i_1][i_0][i_2] - D[i_2+i_1][i_0] * F[i_2][i_0][i_1-1] + D[i_2][i_1] - F[i_2][i_0][i_1] + A[i_0-1][i_1+1] - B[i_2+2][i_1][i_0] + B[i_2+2][i_1-1][i_0+1] * B[i_2+1][i_1][i_0+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3214252313_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
