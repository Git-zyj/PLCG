...
void kernel_4132231113_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(-i_0+PB_L, PB_P-1); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_P, -i_1+PB_L+1); i_2++) {
                A[i_2][i_1+i_0][i_1] = B[i_0][i_2-1][i_1+1] + B[i_1][i_0][i_2] * C[i_2][i_1] + A[i_0][i_2+i_1-1][i_1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4132231113_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
