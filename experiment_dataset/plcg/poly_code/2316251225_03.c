...
void kernel_2316251225_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0-1][i_1+1] = D[i_1] + D[i_1] * E[i_1+i_0] - B[i_0+1] * B[i_1+1] * 1;
        }
        B[i_0+1] = D[i_0-1] * E[i_0] - E[i_0] - E[i_0] - E[1] + B[i_0-1] + 4;
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[i_3+1] = F[i_2-1][i_3+1] - F[i_3][i_2] + G[i_3][i_2+1] + H[i_3][i_2+1][1] - G[i_3][0] + E[i_3-1] - E[i_2-1] + 3;
        }
    }
#pragma endscop
...
kernel_2316251225_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
