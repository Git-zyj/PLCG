...
void kernel_2234212115_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            B[i_1-1] = C[i_0+1][i_1] * 5;
        }
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            B[i_2] = C[i_2][i_0] * B[i_2+1] - 4;
        }
    }
#pragma endscop
...
kernel_2234212115_01(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
