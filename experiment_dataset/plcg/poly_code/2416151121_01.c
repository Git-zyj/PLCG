...
void kernel_2416151121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            A[i_0][i_1] = E[i_1] - F[i_1][i_0] * F[i_1][i_0] - G[i_1][i_1][i_0] - F[i_0][i_1+1] + A[i_0+2][i_1+2] + 1;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_L; i_2++) {
            B[i_0][i_2] = F[i_2][i_2+i_0] - F[i_2][i_0] + F[i_2+i_0][i_2] - F[i_2+i_0][1] + 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < -i_3+PB_L; i_4++) {
            C[i_3][i_4] = F[i_4][i_3+1] - F[i_4+1][i_3-1] * A[i_3][i_4+2] + D[i_3][i_4-1] * D[i_3][i_4-2] + 1;
            D[i_3][i_4-1] = F[i_4+i_3][1] + E[i_4] - F[i_3-1][i_4+i_3] - F[i_3][i_4] - F[i_3][i_4+1] * F[i_4+1][i_3] - 1;
        }
    }
#pragma endscop
...
kernel_2416151121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
