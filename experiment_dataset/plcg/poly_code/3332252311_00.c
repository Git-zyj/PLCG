...
void kernel_3332252311_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0-1] = D[i_0-2] * D[i_0] - D[i_0-2] * E[i_0+2] + E[i_0+2] + A[i_0-2] * A[i_0] + A[i_0-2] * 5;
    }
    for (int i_1 = 1; i_1 < PB_M-3; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_M-2; i_2++) {
            for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
                B[i_3+1][i_2-1][i_1] = F[i_1][i_3][i_3+2] * G[i_1][i_2][i_2] + H[i_3+2][1] - B[i_3+1][i_2-1][i_1-1] + B[i_3+1][i_2-2][i_1] + B[i_2+2][i_1-1][i_3-1] * 5;
            }
        }
        C[i_1+2] = D[i_1] * E[i_1+1] - E[i_1] - D[1] - D[i_1] * E[i_1] + C[i_1+1] * C[i_1+3] * 2;
    }
#pragma endscop
...
kernel_3332252311_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
