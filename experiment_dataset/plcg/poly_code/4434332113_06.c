...
void kernel_4434332113_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
    ...
    for (int i_0 = 2; i_0 < PB_Q-1; i_0++) {
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_Q-2; i_2++) {
                for (int i_3 = 2; i_3 < min(min(-i_1+PB_N, PB_L), -i_2+PB_P); i_3++) {
                    B[i_3+i_1-1][i_2][i_0+1] = G[i_3+i_1][i_2][i_1][i_0-2] * H[i_1][i_3+i_2][i_1-2][i_0+1] * G[i_2+1][i_3][i_1][0] + I[i_3-2][i_1+2][i_2+i_1+2] + 2;
                }
            }
            C[i_1][i_0-1] = J[i_1][i_0-1] - K[i_1][i_1+i_0] - K[i_1+2][i_0] * L[i_1][i_0] + A[i_0-1] - 2;
        }
        for (int i_4 = i_0; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < PB_N; i_5++) {
                D[i_4+1][i_5-1][i_0] = G[i_5][i_0][i_4+1][i_0] * C[i_4][i_5-1] + D[i_4][i_5-1][i_0] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4434332113_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
