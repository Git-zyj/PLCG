...
void kernel_4224151111_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0][i_0] = C[i_0] - D[i_0+1][i_0] * C[i_0] + C[i_0-1] * C[0] + A[i_0-1][i_0-1] - 1;
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                B[i_1][i_0+1][i_2+1] = E[i_2+1][i_1+1][i_0] + E[i_1+1][i_2][i_0] - D[i_2][i_0] + D[i_1-1][i_2] + D[1][i_0] - B[i_1+1][i_0+1][i_2+1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4224151111_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
