...
void kernel_3322312123_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        ...
        for (int i_2 = 2; i_2 < PB_P+1; i_2++) {
            B[i_0+2][i_2-1] = D[i_2][i_0] + B[i_0+2][i_2-2] + 3;
        }
    }
...
kernel_3322312123_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
