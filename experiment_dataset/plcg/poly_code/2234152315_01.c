...
void kernel_2234152315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = C[i_0] * C[i_0-1] * C[i_0] + C[i_0] + C[i_0] + C[i_0] + C[i_0-1] - A[i_0-1] + A[i_0+1] - A[i_0-1] * 4;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(i_1, PB_M-1); i_2++) {
            A[i_2+1] = D[i_2][0] * C[i_2+1] - C[i_2] + 1;
        }
    }
#pragma endscop
...
kernel_2234152315_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
