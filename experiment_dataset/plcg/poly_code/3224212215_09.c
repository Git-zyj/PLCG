...
void kernel_3224212215_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_L-1); i_1++) {
            for (int i_2 = 2; i_2 < PB_N; i_2++) {
                A[i_2][i_1][i_1] = C[i_1][i_1+i_0][i_2-2] * A[i_2][i_1+1][i_1+1] * 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                B[i_5][i_3][i_3-1] = B[i_5][i_4+1][i_4] + B[i_5-1][i_3][i_3-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3224212215_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
