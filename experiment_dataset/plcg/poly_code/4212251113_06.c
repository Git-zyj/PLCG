...
void kernel_4212251113_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                B[i_1-1][i_2] = C[i_2+1][i_0-1][i_0] + D[i_1][i_2][i_0] + C[i_0][i_2][i_1-1] + 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_4+PB_P-1, -i_3+PB_L-1); i_5++) {
                B[i_3-1][i_5] = D[i_3][i_4-1][i_5] * C[i_5][i_3+1][i_4] + D[i_5][i_3+1][i_4] * D[i_4+1][i_5+i_3+1][i_4+1] - D[i_5-1][i_3+1][i_5+i_4+1] - E[i_3][i_4] * B[i_4][i_5] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4212251113_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
