...
void kernel_3424232213_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
    ...
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = 2; i_3 < PB_Q; i_3++) {
                B[i_1][i_2-2][i_3] = G[i_2][i_3-2] - H[i_2][i_3-2][0] + I[i_1][i_2] - 3;
            }
        }
        for (int i_4 = max(3, i_1); i_4 < min(-i_1+PB_L-1, PB_P+2); i_4++) {
            for (int i_5 = 3; i_5 < min(-i_1+PB_L-1, PB_P+1); i_5++) {
                C[i_1][i_4+i_1+1][i_5-2] = J[i_5+1][i_4][i_1] + J[i_5-1][i_4+i_1+1][i_1-1] - C[i_1-1][i_5+i_1+1][i_4-2] - C[i_1][i_5+i_1+1][i_4-3] * C[i_1][i_4+i_1+1][i_5-3] - C[i_1][i_4+i_1+1][i_5-1] * 2;
            }
        }
    }
    for (int i_6 = 3; i_6 < PB_P+3; i_6++) {
        for (int i_7 = 0; i_7 < PB_P; i_7++) {
            for (int i_8 = i_6; i_8 < min(PB_P+1, -i_7+PB_L); i_8++) {
                C[i_8][i_8+i_7][i_6-3] = J[i_8][i_6][i_7] * H[i_6][i_7][i_8] * J[i_6][i_7][i_8-1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3424232213_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
