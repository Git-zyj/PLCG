...
void kernel_4314332325_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < -i_0+PB_L+1; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_L; i_2++) {
                for (int i_3 = 4; i_3 < -i_1+PB_L+1; i_3++) {
                    A[i_0][i_2+i_1][i_2][i_1-2] = D[i_3][i_2+1][i_2][i_0+2] * E[i_1][i_3][i_2] + A[i_2][i_1+i_0-1][i_0][i_1-3] + A[i_2][i_3+i_1-1][i_3-1][i_1-2] - A[i_0+2][i_3+i_2-2][i_2][i_3-4] + A[i_0][i_2+i_1-2][i_2-2][i_1-2] + 2;
                }
            }
        }
        B[i_0] = F[i_0] + G[i_0] + B[i_0-2] - B[i_0+2] + 4;
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < -i_0+PB_Q; i_5++) {
                for (int i_6 = 2; i_6 < -i_5+PB_L; i_6++) {
                    A[i_0+2][i_6+i_5][i_5][i_6-2] = H[i_6+i_5-1][i_4][i_5][i_6][i_0-2] * I[i_4][i_6+i_5][i_0][i_5+i_0] + D[i_5][0][i_6][i_4-1] + I[i_4+1][i_5][i_0-2][0] - I[i_4][i_4][i_6][i_0-2] - 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4314332325_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
