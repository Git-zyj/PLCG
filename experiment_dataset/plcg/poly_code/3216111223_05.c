...
void kernel_3216111223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 3; i_2 < PB_P+2; i_2++) {
        for (int i_3 = i_2; i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < min(min(-i_3+PB_P, i_2), -i_2+PB_P+2); i_4++) {
                B[i_2][i_3][i_4+i_3] = A[i_3+2][i_4+2] + A[i_3+2][i_4] - B[i_3][i_2-2][i_4+i_2-3] - B[i_3-1][i_4][i_4+i_2-2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3216111223_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
