...
void kernel_4334252123_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_N-2; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_M, PB_L); i_3++) {
                    B[i_1-2][i_3][i_2-1] = D[i_1+2][i_3+i_0][i_2][i_0-1] - D[i_3][i_0][i_2][i_1] - E[i_2+2][i_3+2][i_1][0] + D[i_2+1][i_3][i_2][0] - 1;
                }
            }
        }
        for (int i_4 = 2; i_4 < -i_0+PB_L; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_0+PB_L, PB_L-2); i_5++) {
                B[i_5-1][i_0][i_4] = F[i_0][i_5][i_4] - F[i_4][i_0][i_5+i_0] - G[i_5][i_4-2] * H[i_4][i_0][i_4+i_0] * F[i_0][i_5][i_4-2] * F[i_4][i_4][i_5+2] * F[0][i_0][i_4] - C[i_4+3][i_4+1] * 6;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
        for (int i_7 = i_6; i_7 < PB_M-2; i_7++) {
            C[i_7+2][i_7] = G[i_7][i_6+1] + I[i_7] + G[i_7][i_6] - 1;
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4334252123_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
