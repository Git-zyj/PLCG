...
void kernel_4434212113_03(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                C[i_0][i_1][i_1-2][i_2] = 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
            B[i_4+1][i_3-2] = B[i_4][i_3-2] - 6;
        }
        for (int i_5 = 0; i_5 < PB_Q-2; i_5++) {
            for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
                for (int i_7 = 0; i_7 < PB_L; i_7++) {
                    C[i_5][i_6][i_6-2][i_3] = E[i_7][i_3][i_6][0] * F[i_6+2][i_3-1][i_7][i_5+2] + 2;
                }
            }
        }
        for (int i_8 = 0; i_8 < min(PB_P-1, i_3); i_8++) {
            for (int i_9 = 2; i_9 < PB_M-1; i_9++) {
                for (int j_0 = 0; j_0 < min(PB_P, -i_9+PB_M+2); j_0++) {
                    D[i_9][i_3][i_8][j_0] = G[i_8][i_9][i_8] * F[j_0+i_9-2][i_3][i_8][0] * D[i_9+1][i_3+1][i_8+1][j_0] - 4;
                }
            }
        }
    }
...
init_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4434212113_03(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
