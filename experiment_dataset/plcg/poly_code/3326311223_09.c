...
void kernel_3326311223_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_N-1, i_0); i_1++) {
            A[i_1+1][i_1+1] = A[i_1][i_1] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        A[i_2-1][i_2-1] = D[i_2] - 5;
    }
...
kernel_3326311223_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
