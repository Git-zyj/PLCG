...
void kernel_4416132115_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < min(min(-i_1+PB_N, i_1), PB_M-2); i_2++) {
            for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                C[i_2+i_1-1][i_1][i_2+1][i_3] = F[i_3][i_2] + C[i_2+i_1][i_1][i_2+2][i_3+1] - 1;
            }
            for (int i_4 = 0; i_4 < -i_1+PB_L; i_4++) {
                for (int i_5 = i_1; i_5 < PB_M; i_5++) {
                    C[i_4+i_1-2][i_1-1][i_4+1][i_5] = G[i_4+2][i_5][i_5] * H[i_4][i_2][i_1-2][i_5+2] + H[i_1][i_5][i_2-2][i_4] * H[i_5][i_4+i_1][i_5][i_2+1] * 1;
                }
            }
        }
    }
...
kernel_4416132115_09(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
