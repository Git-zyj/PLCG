...
void kernel_3312132113_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_Q-1; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                A[i_2][i_1][0] = D[i_1][i_0][i_1-1] + D[i_1][i_2][i_0+1] * D[i_0][i_1][0] - E[i_2+1][i_1] * D[i_1][i_0][i_0] + 5;
            }
            for (int i_3 = 2; i_3 < min(PB_Q-1, -i_0+PB_M+1); i_3++) {
                B[i_1][i_3-2][i_3] = E[i_3+i_0-1][i_1] - D[i_1-2][i_3-1][i_1+i_0+1] - D[i_0][i_1][i_3+1] * 6;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 2; i_5 < i_4; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
                C[i_6][i_5-1][i_4+1] = D[i_6][i_5-2][i_4] + C[i_6+1][i_5-1][i_4+1] - C[i_4][i_5][i_6+1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3312132113_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
