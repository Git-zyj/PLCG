...
void kernel_3412211321_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            A[i_0][i_1+1] = E[i_1] * A[i_0][i_1-1] * C[i_0][i_1+2] + 1;
        }
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            B[i_0][i_2+1] = F[i_2][i_0] - B[i_0][i_2] + 2;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
            C[i_3][i_4] = E[1] * C[i_3+1][i_4+2] * C[i_3+1][i_4-2] - C[i_4][i_3-2] - C[i_3-2][i_4+1] * C[i_3-2][i_4] - C[i_3][i_4-2] - C[i_3+2][i_4] - 6;
        }
        for (int i_5 = 3; i_5 < PB_N; i_5++) {
            for (int i_6 = 3; i_6 < PB_M; i_6++) {
                D[i_6][i_5-1] = G[i_5][i_3][i_3] - D[i_6][i_5-3] - D[i_5+1][i_6-3] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3412211321_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
