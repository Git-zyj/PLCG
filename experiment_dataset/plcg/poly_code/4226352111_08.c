...
void kernel_4226352111_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = C[i_0] * C[i_0-1] + D[i_0] - D[i_0] * D[i_0] + A[i_0+1] * 2;
    }
    for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = 2; i_3 < min(min(-i_1+PB_M, -i_2+PB_L+1), i_1); i_3++) {
                for (int i_4 = 2; i_4 < min(min(-i_3+PB_P, PB_P-2), i_1); i_4++) {
                    B[i_1][i_3+i_2-1][i_4+2][0] = E[i_3][i_1][i_1+1][i_3-2] * F[i_2][i_1][i_4+i_3][i_4-2][0] * E[i_3+i_1][i_4][i_2-2][i_3] + G[i_3+1][i_4][i_2][2] * E[i_3+1][i_1-2][i_4][i_2+1] + 4;
                }
            }
        }
    }
...
kernel_4226352111_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
