...
void kernel_4214331311_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = PB_N - 1; i_1 >= 1; i_1--) {
            A[i_1][i_0][0] = C[i_1][0] + C[i_0+1][i_1] * A[i_1-1][i_0-1][0] * B[i_0][i_0] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_N; i_3++) {
            B[i_3][i_3] = C[i_3][i_2-1] - D[i_3] + E[i_3][i_2] - E[i_2][i_3] + B[i_3-1][i_3-1] - B[i_3+1][i_3+1] * B[i_2+1][i_2+1] - B[i_3-1][i_3-1] - 4;
        }
    }
#pragma endscop
...
kernel_4214331311_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
