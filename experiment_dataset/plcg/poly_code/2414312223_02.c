...
void kernel_2414312223_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L+2; i_1++) {
            A[i_0-1][i_1+i_0-2] = E[i_1+1][i_0] + F[i_0+2][0] * A[i_1-1][i_1+i_0-3] * A[i_0][i_1+i_0-3] * 4;
        }
        B[i_0] = 2;
        B[i_0+1] = B[i_0-1] + B[i_0+1] * 6;
    }
...
kernel_2414312223_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
