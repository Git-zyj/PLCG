...
void kernel_2336111323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_M-1); i_1++) {
            A[i_1][i_0+1] = A[i_1-2][i_0+1] - A[i_1-1][i_0+1] + A[i_1+1][i_0+1] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < PB_N-1; i_3++) {
            B[i_2][i_3-1] = D[i_2][i_3+1] - B[i_2-1][i_3-2] - B[i_3+1][i_2+1] * 6;
        }
        for (int i_4 = 2; i_4 < i_2; i_4++) {
            C[i_4][i_2-1] = C[i_4][i_2] + C[i_4][i_2+1] * C[i_4-2][i_2-1] + 3;
        }
    }
#pragma endscop
...
kernel_2336111323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
