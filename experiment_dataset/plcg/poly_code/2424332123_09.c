...
void kernel_2424332123_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0+1] = E[i_0] - F[i_0-1] * F[i_0-1] + 2;
        for (int i_1 = 0; i_1 < min(min(i_0, -i_0+PB_L+1), PB_M-2); i_1++) {
            A[i_1+1] = G[i_1+i_0-1][i_0-1] * H[i_1+2][i_0] - H[i_0+1][i_1] + 3;
        }
        for (int i_2 = 0; i_2 < min(i_0, PB_L-2); i_2++) {
            C[i_0][i_2+2] = I[i_2][i_0+1] + I[i_0-2][i_2] - C[i_0+1][i_2+2] - 5;
        }
        D[i_0] = J[i_0] - E[i_0-1] - J[i_0] * E[i_0] * 5;
    }
#pragma endscop
...
kernel_2424332123_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
