...
void kernel_2212112323_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L+2; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
            B[i_2] = D[i_2][i_1-2] * B[i_2-2] + B[i_2+1] - B[i_2-1] * 2;
        }
    }
#pragma endscop
...
kernel_2212112323_07(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
