...
void kernel_2224151311_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = C[i_0+1][i_0] + D[i_0-1] + D[i_0+1] - D[i_0] - D[0] + D[i_0] - D[i_0] * A[i_0+1] - A[i_0-1] * 6;
    }
    for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
        A[i_1-1] = D[i_1+1] + D[i_1] * D[i_1] * A[i_1] * 2;
    }
#pragma endscop
...
kernel_2224151311_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
