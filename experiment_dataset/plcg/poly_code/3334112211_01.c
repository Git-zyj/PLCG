...
void kernel_3334112211_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N-3; i_1++) {
            A[i_1+2][i_0][i_0] = A[i_1+1][i_0][i_0] * A[i_1+2][i_0+1][i_0+1] + A[i_1+3][i_0-1][i_0-1] + 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = i_3; i_4 < PB_L+1; i_4++) {
                B[i_4][i_3][i_2-1] = D[i_4-1][i_3][0] * B[i_4-1][i_3-1][i_2-2] + B[i_2-1][i_4-1][i_3-1] - 5;
            }
        }
        for (int i_5 = 0; i_5 < PB_M; i_5++) {
            for (int i_6 = 0; i_6 < i_2; i_6++) {
                C[i_5][i_2-1][i_6] = C[i_5][i_2-2][i_6+1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3334112211_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
