...
void kernel_2412351213_09(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        C[i_0-1] = E[i_0] + F[i_0] * F[i_0] * 4;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_Q; i_2++) {
            B[i_1][i_2-1] = G[i_1+1][i_2+1] + H[i_1][i_2] + G[i_2][i_1] - G[i_2-1][i_1] * D[i_2] - 2;
        }
        C[i_1] = E[i_1] + E[i_1-1] + F[i_1] - F[i_1] * E[i_1] * I[i_1+1] * I[i_1] * F[i_1-1] + E[i_1] + E[i_1] - E[i_1-1] * D[i_1] * D[i_1+1] - 4;
        D[i_1] = E[i_1] + F[i_1] * C[i_1-1] * D[i_1-1] * 5;
    }
#pragma endscop
...
kernel_2412351213_09(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
