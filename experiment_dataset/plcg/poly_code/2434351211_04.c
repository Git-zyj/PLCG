...
void kernel_2434351211_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_L, i_0), PB_M-1); i_1++) {
            A[i_1+i_0][i_0] = E[i_1] - F[i_1+1][i_0-1] * A[i_1+i_0-1][i_0-1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_L-1, PB_M-1); i_3++) {
            B[i_2][i_2] = E[i_3] + G[i_3+1][i_2] - G[i_2][i_3+1] * F[i_3][0] * G[i_2-1][i_3] + A[i_3+i_2+1][i_2] - B[i_2-1][i_2-1] * 5;
        }
        for (int i_4 = 0; i_4 < i_2; i_4++) {
            D[i_4][i_2+1] = H[i_2-1][i_4] - I[i_4+1] * H[i_2][0] - H[i_2][i_4] - 3;
        }
        for (int i_5 = 1; i_5 < min(-i_2+PB_L, PB_M-1); i_5++) {
            D[i_5][i_2] = G[i_2-1][i_5] + F[i_5-1][i_2] - F[i_5][i_2] - G[i_5][i_2+1] - I[i_5] - J[i_5][i_2+1][i_5] - A[i_5+i_2][i_2] * D[i_5+1][i_2] - 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2434351211_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
