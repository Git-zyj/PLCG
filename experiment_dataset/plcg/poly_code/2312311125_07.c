...
void kernel_2312311125_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M-1, PB_P-1); i_2++) {
            B[i_2+i_1+1][i_2+1] = B[i_2+i_1][i_2] + B[i_2+i_1-1][i_2+1] + 2;
        }
        for (int i_3 = 2; i_3 < min(-i_1+PB_M, i_1); i_3++) {
            C[i_3+i_1][i_3] = D[i_3][i_1] - C[i_3+i_1-2][i_3-2] + 2;
        }
    }
#pragma endscop
...
kernel_2312311125_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
