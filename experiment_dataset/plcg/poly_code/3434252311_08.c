...
void kernel_3434252311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0+1] + E[i_0+2] - E[i_0-1] + F[i_0] + E[i_0] * E[i_0] * F[i_0-2] * F[i_0-2] + B[i_0] - 6;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_1+1] = G[i_1][i_0] - A[i_0] + A[i_1] * A[i_0+1] - B[i_1] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_L, PB_M); i_3++) {
            C[i_3] = G[i_2][i_3] + G[i_3+i_2][i_2-1] + E[i_3] - C[i_3+1] - 2;
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-1; i_5++) {
            D[i_5] = F[i_4+2] - G[i_4][i_5] - F[i_5+1] * H[i_4][i_5] * I[i_4-2][i_5-2][i_4+2] + G[i_4][i_5-1] + H[i_5-2][i_4] * C[i_5] + C[i_5-1] * D[i_5-1] + 2;
        }
    }
#pragma endscop
...
kernel_3434252311_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
