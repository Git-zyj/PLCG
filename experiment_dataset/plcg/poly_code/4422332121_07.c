...
void kernel_4422332121_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-3; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                for (int i_3 = 0; i_3 < min(min(PB_P-1, -i_0+PB_M+1), -i_2+PB_M-2); i_3++) {
                    A[i_0+2][i_1+1][i_2][i_1-1] = E[i_2+1][i_0][i_3][i_1] * F[i_3+i_2+2][i_1][i_0] - G[i_3+i_0-1][i_2][i_3+1][i_1-1] + A[i_0+2][i_1+3][i_2][i_1+1] - 1;
                }
            }
        }
        for (int i_4 = 0; i_4 < -i_0+PB_L; i_4++) {
            for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
                B[i_5+2][i_0+2][i_5] = H[i_4][i_5-1][i_0-2] - I[i_5][1][i_4] - H[i_0+2][i_5-2][i_4+i_0] * F[i_5][i_4+i_0][0] + I[i_5][i_0][i_4] + 5;
            }
        }
        C[i_0+1] = J[i_0] * K[i_0] * 5;
    }
    for (int i_6 = 0; i_6 < PB_L; i_6++) {
        for (int i_7 = 0; i_7 < PB_L-2; i_7++) {
            for (int i_8 = 0; i_8 < i_6; i_8++) {
                D[i_6][i_6][i_8][i_7+2] = D[i_8+1][i_8+1][i_6][i_7+2] - 5;
            }
        }
    }
#pragma endscop
