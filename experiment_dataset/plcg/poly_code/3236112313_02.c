...
void kernel_3236112313_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_P+2; i_3++) {
            for (int i_4 = max(2, i_2); i_4 < PB_M-2; i_4++) {
                B[i_2][i_4-2][i_2+1] = C[i_4+2][2][i_3-2] - B[i_2-1][i_4-2][i_2] - B[i_2+1][i_4-2][i_2+2] + B[i_2][i_4-1][i_2+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3236112313_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
