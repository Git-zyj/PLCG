...
void kernel_4316352313_00(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,int wj,DATA_TYPE POLYBENCH_5D(J,xJ,yJ,zJ,vJ,wJ,xj,yj,zj,vj,wj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_Q-2, i_0); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M+2; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_1+PB_Q-2, -i_0+PB_N); i_3++) {
                    B[i_3+i_0][i_1][i_2][i_3] = D[i_3+i_1][i_0][i_2][i_3] + E[i_2+i_0-2][i_2][i_3][i_3+i_1+2] - F[i_2][2][0] * G[i_2][i_0][i_3+i_1][i_0-2] - E[i_3-1][i_2][i_1][i_0] + 2;
                }
            }
        }
        for (int i_4 = i_0; i_4 < min(-i_0+PB_N, PB_L-2); i_4++) {
            for (int i_5 = 2; i_5 < min(-i_4+PB_N+1, PB_M-1); i_5++) {
                for (int i_6 = 0; i_6 < min(-i_0+PB_M, -i_5+PB_M-1); i_6++) {
                    B[i_4+i_0][i_6][i_5][i_0] = H[i_5+1][i_6][1] * I[i_4][i_0+2][i_5-2] + H[i_6+i_5+1][i_4+2][i_0] + J[i_6+i_0][i_4-2][0][i_5+1][i_6+i_0] * B[i_5+i_4-1][i_0][i_6][i_5] + 5;
                }
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_M-1; i_7++) {
        C[i_7] = K[i_7] * L[i_7+1] + K[i_7-2] * L[i_7-1] * M[i_7+1] - M[i_7] + C[i_7+1] + C[i_7-1] * C[i_7+1] + C[i_7-1] * 5;
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4316352313_00(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M)));
