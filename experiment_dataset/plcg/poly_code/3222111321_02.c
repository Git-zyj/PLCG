...
void kernel_3222111321_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 3; i_2 < PB_Q; i_2++) {
                A[i_1-1][i_0-1][i_2] = A[i_1+1][i_2-3][i_0] - A[i_1-1][i_0+1][i_2-1] + 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N-2; i_3++) {
        B[i_3] = C[1] - C[i_3] * B[i_3+2] - B[i_3-2] - B[i_3+1] * B[i_3-1] - 6;
    }
#pragma endscop
...
kernel_3222111321_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
