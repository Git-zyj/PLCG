...
void kernel_2216312221_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        A[i_0+2] = C[i_0+2] - C[i_0] * A[i_0+3] - A[i_0+1] * 1;
        for (int i_1 = max(2, i_0); i_1 < PB_M; i_1++) {
            B[i_1][i_0+2] = A[i_1] + B[i_1-2][i_0+2] - 6;
        }
    }
#pragma endscop
...
kernel_2216312221_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
