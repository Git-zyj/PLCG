...
void kernel_2316112323_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            A[i_1][i_1+i_0] = D[i_0][i_1] * A[i_0+1][i_1+i_0] - 2;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_L); i_2++) {
            C[i_0-1][i_2] = D[i_2+i_0][0] - 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_L-3; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_L-3; i_4++) {
            C[i_3][i_4] = D[i_3-1][i_4] + A[i_4+1][i_4+i_3+3] - C[i_3+1][i_4-1] - C[i_3][i_4-2] - C[i_3-2][i_4] + C[i_3+1][i_4] * 1;
        }
    }
#pragma endscop
...
kernel_2316112323_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
