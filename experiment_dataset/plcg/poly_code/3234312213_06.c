...
void kernel_3234312213_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q+2; i_0++) {
        for (int i_1 = 3; i_1 < PB_N-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_1][i_2+1] = C[i_2][i_1] * A[i_1+1][i_2] * 4;
            }
            for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                B[i_1-2][i_3+1][i_0-2] = A[i_1+1][i_3+1] - B[i_1-3][i_3+1][i_0-2] * B[i_1-2][i_3+1][i_0-3] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3234312213_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
