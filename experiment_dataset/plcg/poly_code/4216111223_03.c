...
void kernel_4216111223_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            A[i_1] = C[i_1][i_0-1] + A[i_1+1] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            for (int i_4 = 2; i_4 < -i_2+PB_M-2; i_4++) {
                B[i_2][i_4][i_4+i_2] = B[i_4-2][i_2][i_4+i_2-2] + B[i_2-2][i_4][i_4+i_2-2] * B[i_2+2][i_4][i_4+i_2+2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4216111223_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
