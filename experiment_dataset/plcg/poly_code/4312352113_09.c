...
void kernel_4312352113_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        A[i_0] = D[i_0] - E[i_0] * A[i_0-1] * 4;
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            A[i_1+1] = F[i_1][0] * G[i_0][i_1] - H[i_1][i_0] * F[i_1][i_0] - H[i_0-1][i_1] * F[i_1+2][i_0+2] * 3;
        }
        for (int i_2 = 2; i_2 < PB_Q; i_2++) {
            for (int i_3 = 1; i_3 < PB_N; i_3++) {
                C[i_0][i_2][i_2-2] = I[i_3][i_2][i_0] + I[i_0+1][i_2][i_3-1] * J[i_3][i_3][i_0] * J[i_2][i_0][i_2] + K[i_3][i_2][i_0] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4312352113_09(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
