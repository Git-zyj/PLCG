...
void kernel_2416311313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            A[i_1] = A[i_0-1] + A[i_1+1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            B[i_2][i_3] = E[i_2+1][i_3] - B[i_3][i_2-1] - B[i_2-1][i_3+1] - 4;
        }
        for (int i_4 = 2; i_4 < i_2; i_4++) {
            D[i_4-1] = D[i_2] + D[i_4-2] + D[i_4] + 3;
        }
        D[i_2-1] = 4;
    }
#pragma endscop
...
kernel_2416311313_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
