...
void kernel_3334132213_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            B[i_1+1] = D[i_1+1][i_0] + D[i_1][i_0-2] + D[i_1-1][i_0+2] - D[i_0][i_1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-2; i_2++) {
        B[i_2] = E[i_2] + E[i_2] - 6;
        for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
            for (int i_4 = 0; i_4 < min(i_2, -i_3+PB_N-2); i_4++) {
                C[i_3][i_2][i_4] = D[i_2+2][i_3-1] * D[i_4+i_3+2][i_2] - C[i_3][i_2-1][i_4] * C[i_3-1][i_2][i_4] - C[i_3][i_2][i_4+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3334132213_03(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
