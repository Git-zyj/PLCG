...
void kernel_3234112325_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_N+1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_0-2][i_1] = C[i_1] + A[i_0-1][i_1-2] + A[i_0-3][i_1] + A[i_0-4][i_1+2] * 6;
        }
    }
    for (int i_2 = 3; i_2 < PB_L+1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_Q; i_3++) {
            for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
                B[i_4][i_2-1][i_3] = B[i_4+2][i_2-1][i_3] * B[i_4-2][i_2-3][i_3] * B[i_4+1][i_2-1][i_3] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3234112325_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
