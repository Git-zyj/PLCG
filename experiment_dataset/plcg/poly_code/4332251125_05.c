...
void kernel_4332251125_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][i_1+1] = D[i_0-1][i_1] + E[i_1] * F[i_1+1][i_0+1] * E[i_1] - F[i_1+1][i_0-1] - G[i_0+1][1][i_1] + 6;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_P; i_2++) {
            for (int i_3 = 1; i_3 < -i_2+PB_L; i_3++) {
                B[i_2-1][i_0][i_3] = G[i_3][i_0-1][i_2-1] * G[i_3+i_2][i_3][i_2+i_0] * F[i_3-1][i_2] * B[i_2-1][i_0-1][i_3] + 6;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < PB_P; i_5++) {
            for (int i_6 = 1; i_6 < i_5; i_6++) {
                C[i_5][i_6] = G[i_4][i_6][i_5] + G[i_6-1][i_5-1][i_4-1] + H[i_4][i_5-1][i_5] - H[i_4+1][i_6][0] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4332251125_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
