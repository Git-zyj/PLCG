...
void kernel_3332211115_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
    ...
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        ...
        for (int i_3 = 0; i_3 < min(PB_Q-1, i_2); i_3++) {
            for (int i_4 = 1; i_4 < PB_Q+1; i_4++) {
                C[i_3][i_3] = C[i_3+1][i_3+1] + C[i_4-1][i_4-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3332211115_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
