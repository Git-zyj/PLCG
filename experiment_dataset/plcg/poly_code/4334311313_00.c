...
void kernel_4334311313_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                A[i_0][i_1][i_1] = D[i_1-1][i_0-1][0] + A[i_1-1][i_0][i_0] - 2;
            }
        }
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_0][i_3][i_3] = B[i_0+1][i_3][i_3] - B[i_0][i_3+1][i_3+1] + B[i_0+1][i_4][i_4] - B[i_0+1][i_3-1][i_3-1] + 5;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_P; i_5++) {
        for (int i_6 = 2; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < PB_M-1; i_7++) {
                C[i_7][i_6-1] = C[i_7+1][i_6] * C[i_7+1][i_6-2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4334311313_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
