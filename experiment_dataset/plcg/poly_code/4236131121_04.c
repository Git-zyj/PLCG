...
void kernel_4236131121_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1-1][i_0] = C[i_0+1][i_1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_2+PB_P+1, PB_M-1); i_4++) {
                B[i_4][i_3][i_2] = C[i_3][i_4] * C[i_2-1][i_3-1] * D[i_4+1][i_3][i_2] + D[i_3][i_4+i_2-1][i_2] * D[i_4][i_3][0] * B[i_2][i_3-2][i_4+1] * B[i_2][i_4][i_3+1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4236131121_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
