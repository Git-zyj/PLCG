...
void kernel_2336351313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = D[i_0] + D[i_0+1] + D[i_0+1] * A[i_0-1] * 6;
    }
    for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
        A[i_1] = E[i_1] * D[i_1] + E[i_1+1] + F[i_1] - D[i_1] + E[i_1] * E[i_1] + 4;
        for (int i_2 = max(1, i_1); i_2 < PB_P; i_2++) {
            A[i_2] = G[i_2][i_1] - H[i_1+1][i_2-1] + H[i_1][i_2] * I[i_2][i_2][i_1+1] * A[i_2] + A[i_2+1] + 2;
        }
    }
#pragma endscop
...
kernel_2336351313_00(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
