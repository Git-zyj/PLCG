...
void kernel_2424132215_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-2; i_1++) {
            B[i_1-1] = E[i_1+i_0+2][i_0] + F[i_0] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        B[i_2-1] = F[i_2] - F[i_2] + F[i_2] + F[i_2-1] + F[i_2] * F[i_2] + F[i_2] * B[i_2-2] + 2;
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            B[i_2-1] = E[i_2][i_3] * B[i_3] + 5;
        }
        D[i_2] = D[i_2-1] + 4;
    }
#pragma endscop
...
kernel_2424132215_06(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
