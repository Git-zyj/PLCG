...
void kernel_3422251125_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        A[i_0] = E[i_0] + F[i_0] - F[i_0] + A[i_0+2] + 5;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        A[i_1] = E[i_1] + 5;
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_L-1); i_2++) {
            A[i_2] = G[i_2][i_1] + H[i_2][i_1-1] * I[i_2+1][i_1-1][0] - H[i_1-1][i_2+i_1] - H[i_2][i_1-1] * H[i_2+1][i_1] * G[i_1-1][i_2+1] - F[i_2] + 2;
        }
        A[i_1+1] = F[i_1-1] + F[i_1+1] * F[i_1+1] + F[i_1] * E[i_1] * 1;
    }
#pragma endscop
...
kernel_3422251125_09(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
