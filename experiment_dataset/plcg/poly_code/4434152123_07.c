...
void kernel_4434152123_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = E[i_0] * E[i_0] * E[i_0] - E[i_0] + E[2] * F[i_0][0] * 4;
        for (int i_1 = 2; i_1 < min(PB_N-2, i_0); i_1++) {
            A[i_0+1] = F[i_1][i_0] + F[i_0][i_1-2] + F[i_1+2][i_0] + F[i_0][i_1] * F[i_1+1][i_0] - A[i_0+1] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 2; i_4 < -i_3+PB_L; i_4++) {
                C[i_4-1][i_3][i_2] = G[i_4][i_3][i_2+2] - G[i_3-2][i_2][i_4] * H[i_4][i_2][i_3+1][i_4] + G[i_4-2][i_3][i_2] + G[i_4+i_3][i_2+1][i_3-2] + 5;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
        D[i_5] = F[i_5][i_5-1] * E[i_5] * E[i_5+1] * E[i_5] * D[i_5-1] - 6;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434152123_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
