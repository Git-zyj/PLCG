...
void kernel_3212152115_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M-1; i_1++) {
            B[i_0+2] = C[i_1+i_0][i_0] * C[i_0-1][i_1-2] + C[i_1+i_0+1][0] - C[i_0][i_1] + C[i_1+1][i_0+2] * C[i_1][i_0] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-3; i_3++) {
            B[i_3+2] = C[i_2][i_3] * C[i_3][0] + C[i_3+2][i_2] * C[i_3][i_2] * B[i_3+3] + 4;
        }
    }
#pragma endscop
...
kernel_3212152115_05(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
