...
void kernel_3214112325_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_M-3, -i_0+PB_P-1); i_1++) {
            A[i_1+2][i_1+i_0] = C[i_0][i_0] - A[i_1+3][i_1+i_0+1] + B[i_0] - 1;
        }
        B[i_0] = D[i_0] + B[i_0-2] + B[i_0+2] + B[i_0-1] * 2;
    }
#pragma endscop
...
kernel_3214112325_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
