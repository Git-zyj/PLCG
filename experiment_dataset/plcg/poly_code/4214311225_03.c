...
void kernel_4214311225_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(i_1, PB_M-2); i_2++) {
            for (int i_3 = max(2, i_1); i_3 < PB_L-2; i_3++) {
                B[i_2][i_3] = B[i_2+2][i_3-1] + B[i_2+1][i_3] - B[i_2][i_3+2] - B[i_2][i_3-2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4214311225_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
