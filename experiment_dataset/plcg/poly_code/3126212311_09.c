...
void kernel_3126212311_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L-3; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
                A[i_2][i_1+2][i_1] = B[i_2+i_1][2] + A[i_2-1][i_1+2][i_1] - A[i_0][i_1+1][i_1-1] * A[i_2][i_1+3][i_1+1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3126212311_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
