...
void kernel_4326111321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_L+3; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                A[i_1][i_2-1] = A[i_2][i_1-3] - A[i_1+1][i_2+1] - 1;
            }
        }
        B[i_0] = D[i_0] * D[i_0] * B[i_0+2] - B[i_0-1] * B[i_0+1] - 5;
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_L, i_3); i_4++) {
            C[i_3+1][i_4] = C[i_3][i_4] - C[i_3+1][i_4-1] - 2;
        }
    }
#pragma endscop
...
kernel_4326111321_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
