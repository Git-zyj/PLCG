...
void kernel_2224152225_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_N; i_1++) {
            B[i_0-1][i_1] = C[i_0+1][i_1] * D[i_1] * C[i_1][i_0] + D[i_1] * B[i_0-3][i_1-2] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_N+2; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_L; i_3++) {
            B[i_2-1][i_3] = C[i_2-1][i_3+i_2] * C[i_3+i_2][i_3] - C[i_2][i_3] - C[i_3][i_2] * D[i_2-2] + B[i_2-1][i_3-2] * 2;
        }
    }
#pragma endscop
...
kernel_2224152225_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
