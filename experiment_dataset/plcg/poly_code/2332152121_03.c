...
void kernel_2332152121_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_0+2][i_1-2] = D[i_1+i_0][i_0] - E[i_0+2] - D[i_0][0] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_3][i_2+2] = D[i_3][i_2] - E[0] * D[i_3][i_2] * D[i_3][i_2] + B[i_3][i_2] - 6;
        }
    }
    for (int i_4 = 0; i_4 < PB_N-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
            C[i_4][i_5-2] = D[i_5][i_4+1] + E[i_4] * E[i_5] - D[i_5][i_4] * D[i_4][i_5] - D[i_5+1][i_4] - C[i_4+2][i_5-2] - 4;
        }
    }
#pragma endscop
...
kernel_2332152121_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
