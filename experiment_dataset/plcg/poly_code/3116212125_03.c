...
void kernel_3116212125_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_Q+1, -i_0+PB_P+2); i_1++) {
            for (int i_2 = 3; i_2 < min(min(PB_Q, -i_0+PB_P+2), -i_1+PB_L); i_2++) {
                A[i_1-1][i_2+i_0-2][i_2] = B[i_0][i_2+i_1] * A[i_2-3][i_1+i_0-2][i_1-1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3116212125_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
