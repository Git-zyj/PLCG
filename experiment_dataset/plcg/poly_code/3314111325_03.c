...
void kernel_3314111325_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_1][i_0] = D[i_0][0] + D[i_0][i_1] * D[i_0][i_1+1] * A[i_1][i_0-1] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        B[i_2] = B[i_2-2] - B[i_2-1] * B[i_2+2] - B[i_2+1] * 1;
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
                C[i_2-1][i_4-1][i_3-1] = A[i_4][i_3] - C[i_2-1][i_4-2][i_3+1] + C[i_2-1][i_4+1][i_3-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3314111325_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
