...
void kernel_3326312321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
                A[i_0][i_0][i_2] = A[i_1-1][i_1-1][i_2+1] * A[i_0][i_0][i_2+2] - A[i_2-1][i_2-1][i_1+1] - 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_P-2; i_4++) {
            B[i_3][i_4+1] = D[i_3][i_4-2][i_4+2] + B[i_3+1][i_4+1] * 6;
        }
        for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_P+1; i_6++) {
                C[i_5+1][i_3-1][i_6] = C[i_5][i_3+1][i_6] * C[i_5-1][i_6-1][i_3-1] * C[i_6+1][i_3-2][i_5-1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3326312321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
