...
void kernel_3324232123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        B[i_1+2] = D[i_1] - D[i_1] - D[i_1+1] + A[i_1-1] * B[i_1+4] * 3;
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 2; i_3 < min(-i_2+PB_L, -i_1+PB_P); i_3++) {
                C[i_1][i_3+i_2][i_3-2] = F[i_2+1][i_3+i_1] + G[i_2][i_1+2][i_1] - H[i_2][i_1][i_1-2] + G[i_1][i_2+2][1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3324232123_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
