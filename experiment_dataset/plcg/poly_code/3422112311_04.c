...
void kernel_3422112311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 3; i_1 < PB_M+2; i_1++) {
            A[i_1-2][i_0] = A[i_1-3][i_0] * A[i_1-3][i_0+1] * 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
                B[i_2-2][i_2][i_4] = E[i_3][i_3-2][i_2] * E[i_3][i_2][i_4+2] - B[i_4-1][i_4+1][i_3+1] + B[i_2-1][i_2+1][i_4] - B[i_2-1][i_2+1][i_3] + 1;
            }
        }
        for (int i_5 = 2; i_5 < PB_L+1; i_5++) {
            C[i_2][i_5-2] = C[i_5+1][i_2-1] - C[i_2-1][i_5-2] - C[i_2-1][i_5-1] - 6;
        }
    }
    for (int i_6 = 2; i_6 < PB_P; i_6++) {
        for (int i_7 = 0; i_7 < i_6; i_7++) {
            D[i_6+2] = B[i_6-2][i_6][i_7+1] + D[i_6+1] - 2;
        }
    }
#pragma endscop
...
kernel_3422112311_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
