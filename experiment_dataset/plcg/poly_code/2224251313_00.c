...
void kernel_2224251313_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(-i_0+PB_N-1, PB_P-1); i_1++) {
            A[i_1-1][i_1+1] = C[i_1+i_0][0] * C[i_1+i_0+1][1] - C[i_1][i_1] - D[i_1+1] * A[i_0][i_0+2] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
            A[i_3-1][i_3+1] = E[i_3+1][i_2] + E[i_2][i_3-1] + C[i_3][0] - E[i_3+1][i_2] + D[i_2] * E[i_3][i_3-1] * A[i_3-1][i_3+1] * A[i_2-1][i_2+1] + 1;
        }
    }
#pragma endscop
...
kernel_2224251313_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
