...
void kernel_2322132215_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        C[i_3] = D[i_3] + D[i_3+2] * D[i_3] + E[i_3][1] * D[i_3] - D[i_3] - D[i_3+1] * C[i_3+1] + 1;
    }
#pragma endscop
...
kernel_2322132215_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
