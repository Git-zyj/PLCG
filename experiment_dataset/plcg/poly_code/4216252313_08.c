...
void kernel_4216252313_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_M-2); i_1++) {
            for (int i_2 = max(2, i_1); i_2 < min(-i_0+PB_L, -i_1+PB_M-2); i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_N-2; i_3++) {
                    A[i_1][i_3][i_2] = C[i_3+i_2+2][i_1][i_2+1] * D[i_2][i_0-1][i_3-1][i_1] + D[i_3+i_2][i_0][i_1-2][i_3+2] + D[i_2][i_0+1][i_3-1][i_1] * E[i_3-1][i_0][i_2-2][i_3-1] + E[i_1+1][i_3][i_2][i_0] - 3;
                }
                B[i_0-1][i_1][i_2] = C[i_0][i_2+i_1+2][i_0] * F[i_2+i_0][i_1+2] * C[i_1][i_0-1][i_0] * B[i_0-1][i_1-1][i_2-1] * B[i_2-1][i_1-1][i_0] * B[i_0][i_1+1][i_2] + B[i_0-1][i_1-1][i_2] * B[i_0-1][i_1+1][i_2] - 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4216252313_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F)));
