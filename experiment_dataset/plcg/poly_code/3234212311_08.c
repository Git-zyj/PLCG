...
void kernel_3234212311_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = -i_0+PB_M+2 - 1; i_1 >= 2; i_1--) {
            for (int i_2 = 2; i_2 < -i_0+PB_M+1; i_2++) {
                B[i_1-2][i_2+i_1-2] = B[i_2-2][i_2+i_0-2] - B[i_2-2][i_2+i_0-1] - B[i_0-2][i_1+i_0-2] + 1;
            }
            B[i_1-2][i_1+i_0-2] = C[i_1-1][i_0] - 6;
        }
    }
#pragma endscop
...
kernel_3234212311_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
