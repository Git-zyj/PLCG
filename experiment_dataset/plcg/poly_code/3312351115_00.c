...
void kernel_3312351115_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_N, PB_L-1); i_1++) {
            A[i_1][i_1] = D[i_0][i_1+1] * D[i_1+i_0][1] - E[i_0][i_1+1][0] + F[i_0+1][i_1+1] - A[i_1-1][i_1-1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M-1; i_3++) {
            A[i_3][i_3] = D[i_2][i_2] + G[i_3+i_2+1] - H[i_2][i_3+1] * H[i_2-1][i_3] - F[i_3][i_2] * D[i_2+1][i_3] * 3;
            for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
                A[i_4][i_4] = I[i_3+i_2+1][i_4][i_4+1] - D[i_2][i_4] * I[i_4+1][i_2+1][i_3+i_2+1] + I[i_2-1][i_4+1][i_3] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3312351115_00(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
