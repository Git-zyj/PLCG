...
void kernel_4312332121_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, i_0); i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_M; i_3++) {
                    A[i_3+i_1][i_1-2][i_2][i_3-1] = D[i_2+1][i_3][i_0][0] * E[i_1][i_2][i_2][i_0+2] * D[i_2+i_1][i_0-2][i_3][i_1] + D[i_2][i_0][i_1+2][i_1+1] * D[i_3][i_0][i_2+1][i_1] + 6;
                }
            }
        }
        for (int i_4 = 0; i_4 < -i_0+PB_L-1; i_4++) {
            B[i_0][i_4] = F[i_4][i_0+2] * G[i_0][i_4+i_0+1][0] - F[i_4+i_0+1][i_4] * B[i_0][i_4+1] * 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_Q; i_5++) {
        for (int i_6 = 2; i_6 < PB_M; i_6++) {
            for (int i_7 = 1; i_7 < -i_6+PB_M; i_7++) {
                A[i_7+i_6][i_6-2][i_5-1][i_7-1] = H[i_7][i_5][0] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4312332121_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
