...
void kernel_4222212321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0-1][i_1] = A[i_1][i_0+1] * A[i_0-3][i_1+1] + A[i_0-1][i_1-1] * 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        B[i_2] = C[i_2] + B[i_2-2] + B[i_2-1] + B[i_2+1] + 4;
    }
#pragma endscop
...
kernel_4222212321_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
