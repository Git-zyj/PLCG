...
void kernel_3234232213_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-1); i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_L+2; i_2++) {
                B[i_2][0] = C[i_2][i_1][i_1] * D[i_0][i_2-2][i_1+1] + D[i_0][i_1][i_2] * D[i_1][i_2+i_0-2][i_0] - 2;
            }
        }
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            B[i_3][0] = E[i_0][i_3] - B[i_3+1][0] - B[i_0-1][0] * 1;
        }
    }
#pragma endscop
...
kernel_3234232213_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
