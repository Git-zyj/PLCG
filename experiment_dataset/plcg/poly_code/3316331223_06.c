...
void kernel_3316331223_06(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        C[i_0-1] = D[i_0+1] + E[i_0-1] + 5;
    }
    for (int i_1 = 2; i_1 < PB_M-3; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 1; i_3 < min(i_1, -i_1+PB_M-3); i_3++) {
                B[i_3+i_1+1][i_2+1][i_1] = F[i_2][i_3][0] * G[i_2+1][i_3][i_2+1] - H[i_2-1][i_3-1] + B[i_3+i_1-1][i_2+1][i_1] * B[i_3+i_1+3][i_2+1][i_1+2] - 2;
            }
        }
        C[i_1-1] = E[i_1] + D[i_1] - E[i_1-1] + E[i_1] - C[i_1-2] + C[i_1+1] * 5;
    }
#pragma endscop
...
kernel_3316331223_06(xc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
