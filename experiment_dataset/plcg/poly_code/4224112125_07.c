...
void kernel_4224112125_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < min(i_0, PB_M-2); i_3++) {
                    A[i_2][i_3][i_0-2] = A[i_3+2][i_1-1][i_0-3] * 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
            B[i_5+2][i_4][i_5+1] = C[i_5+2][i_4-1] - 4;
        }
    }
#pragma endscop
...
kernel_4224112125_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
