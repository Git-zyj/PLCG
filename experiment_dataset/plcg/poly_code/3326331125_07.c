...
void kernel_3326331125_07(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, i_0); i_2++) {
                C[i_2] = D[i_0-1][i_2+i_1] * E[i_2+i_0][i_0][i_1] - E[i_2-1][i_1+i_0][i_2-1] + F[i_0][i_1] * G[i_1][i_2+1][i_0] * C[i_2-1] + 1;
            }
        }
        C[i_0-1] = H[i_0] - I[i_0] + 3;
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
            C[i_4-1] = F[i_4+i_3][i_3+1] - 4;
        }
    }
#pragma endscop
...
kernel_3326331125_07(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
