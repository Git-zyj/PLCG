...
void kernel_3212112323_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            B[i_1+1][i_0] = B[i_1+2][i_0] - 4;
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_1+1][i_0] = C[i_1][i_2][i_0] - B[i_1+2][i_0+2] + B[i_2+1][i_1+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3212112323_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
