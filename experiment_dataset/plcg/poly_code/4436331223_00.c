...
void kernel_4436331223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                for (int i_4 = i_2; i_4 < min(-i_3+PB_L, -i_1+PB_N); i_4++) {
                    D[i_4+i_1-2][i_4+i_1-2] = F[i_4+i_1][i_2][i_3] * G[i_4][i_1][i_3][i_2-1] - H[i_2][i_4+i_3][i_1][0] + H[i_4-1][i_1][i_2][i_3] * 4;
                }
            }
        }
        for (int i_5 = 0; i_5 < PB_N; i_5++) {
            for (int i_6 = 0; i_6 < i_1; i_6++) {
                for (int i_7 = 0; i_7 < -i_5+PB_M; i_7++) {
                    D[i_7+i_5][i_7+i_5] = F[i_5][i_5][i_6] + 3;
                }
            }
        }
    }
    for (int i_8 = 0; i_8 < PB_M-1; i_8++) {
        for (int i_9 = max(i_8, 4); i_9 < -i_8+PB_M-1; i_9++) {
            for (int j_0 = 1; j_0 < -i_9+PB_M; j_0++) {
                D[j_0+i_9][j_0+i_9] = I[i_9][i_8][i_9] + F[j_0+1][i_8][i_8] * J[j_0-1][i_9] - D[i_9+i_8+1][i_9+i_8+1] * D[j_0+i_9-4][j_0+i_9-4] - D[j_0+i_9-2][j_0+i_9-2] - 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4436331223_00(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
