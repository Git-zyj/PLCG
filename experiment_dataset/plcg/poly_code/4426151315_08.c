...
void kernel_4426151315_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = 1; i_2 < min(min(PB_N, -i_1+PB_L), -i_0+PB_P); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_L, PB_M); i_3++) {
                    A[i_3][i_0][i_2][i_1] = E[i_3][i_2+i_1][i_0][i_3] + E[i_3][i_1][i_0][i_2] - E[i_1+1][i_0][i_0][i_2-1] - E[i_3][i_2-1][i_1][i_2-1] + E[i_1+1][i_2][i_0-1][i_3+i_2] * E[i_1][i_3+i_2][i_0][i_3] - E[i_3][i_1+i_0][i_2+i_0][1] * A[i_0][i_1][i_2][i_3+1] - A[i_2][i_0+1][i_3][i_1-1] * A[i_3][i_0][i_2][i_1+1] + A[i_3][i_0][i_2+1][i_1] + 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            B[i_4][i_5] = F[i_4][i_5] * G[i_5+1] * F[i_4][i_5-1] - F[i_4+1][i_5-1] + C[i_4][i_5] - C[i_4-1][i_5-1] * 3;
            C[i_4][i_5] = F[i_5][i_4+1] + G[i_5] - 2;
        }
    }
    for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 1; i_7 < min(i_6, -i_6+PB_P+1); i_7++) {
            for (int i_8 = 0; i_8 < -i_6+PB_M-1; i_8++) {
                D[i_7+1][i_8] = H[i_8+i_6+1][i_7+i_6-1][1] + H[i_6+1][i_7+1][i_8] - H[i_7-1][i_8+1][i_6+1] * H[i_8][i_6-1][i_7] - H[i_7+1][i_7][i_6+1] - D[i_8+2][i_7-1] + D[i_7+1][i_8+1] + D[i_7][i_6] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4426151315_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
