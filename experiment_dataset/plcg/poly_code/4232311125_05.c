...
void kernel_4232311125_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M+1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L; i_2++) {
                B[i_1+i_0-1][i_1-2][i_2] = C[i_0][i_2+i_1][i_0] + 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M-1; i_4++) {
            for (int i_5 = 2; i_5 < PB_M; i_5++) {
                B[i_4+i_3+1][i_3][i_5] = C[i_5][i_4][i_4] - B[i_4+i_3+1][i_4][i_5-2] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4232311125_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
