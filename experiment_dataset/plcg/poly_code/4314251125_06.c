...
void kernel_4314251125_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
        ...
        for (int i_1 = max(1, i_0); i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_P); i_2++) {
                for (int i_3 = 1; i_3 < PB_L; i_3++) {
                    A[i_2+1][i_1-1][i_2-1][i_3] = D[i_0][i_3-1][i_1][i_2] * D[i_2][i_3][i_0][i_1] - E[i_2+i_1][i_3][i_0][i_2] - D[i_2][i_3][i_3][i_1+i_0] - 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
        B[1] = F[i_4] + G[i_4] - H[i_4][0] + F[i_4] - F[i_4+1] - F[i_4] - B[1] * B[1] + 5;
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_N); i_5++) {
            for (int i_6 = 3; i_6 < i_4; i_6++) {
                for (int i_7 = 3; i_7 < PB_P-1; i_7++) {
                    A[i_6-1][i_7-3][i_6-3][i_5] = D[i_7][i_4+1][i_6][i_5] * D[i_4][i_7][i_5][i_6] - D[i_6-1][i_4][i_5][i_4+1] * E[i_5][i_6][i_7+1][i_4] * D[i_5+i_4][i_6+1][i_6][i_7] + 1;
                }
            }
        }
    }
...
kernel_4314251125_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
