...
void kernel_2422211221_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_1+i_0] = A[i_1+i_0+1] * A[i_1+i_0-2] - A[i_1+i_0] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_M+1, -i_2+PB_L-2); i_3++) {
            B[i_3+i_2][i_2+1] = E[i_3-1][i_2] - B[i_3+i_2+1][i_2+2] + B[i_3+i_2+2][i_2+3] - B[i_3+i_2-1][i_2] - 6;
        }
        C[i_2] = 6;
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < PB_M; i_5++) {
            D[i_5][0] = D[i_5+2][0] * D[i_5-2][0] - 3;
        }
    }
#pragma endscop
...
kernel_2422211221_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
