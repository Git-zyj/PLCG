...
void kernel_4222111215_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_P-1, PB_M-1); i_2++) {
                B[i_2+1][i_1] = C[i_0+1][i_2+i_1+1][1] * B[i_0+1][i_1+1] - 1;
            }
            B[i_1+1][i_0+1] = B[i_1+1][i_0+2] * 4;
        }
    }
#pragma endscop
...
kernel_4222111215_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
