...
void kernel_4134111221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q; i_1++) {
            for (int i_2 = 2; i_2 < min(min(PB_Q, -i_0+PB_P+1), -i_1+PB_P+2); i_2++) {
                A[i_2-1][i_2+i_0-1] = B[i_2][i_0-1][1] + A[i_2-2][i_2+i_1-2] * A[i_2-1][i_2+i_0-3] + 3;
            }
        }
    }
#pragma endscop
...
kernel_4134111221_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
