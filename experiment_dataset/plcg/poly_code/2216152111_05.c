...
void kernel_2216152111_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_1-1][i_0] = C[i_1+i_0] + D[i_1][i_0-2] * D[i_1+i_0][2] - D[i_1][i_0] + B[i_0-2][i_1-1] * 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 3; i_3 < PB_P; i_3++) {
            B[i_2-2][i_3] = D[i_3][i_2-1] + C[i_2] + D[i_2][i_3] + D[i_3+2][i_2] * D[i_3][i_2+1] * C[i_2] - B[i_3-3][i_2-1] * 5;
        }
    }
#pragma endscop
...
kernel_2216152111_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
