...
void kernel_4224132223_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
            A[i_0-1][i_1] = A[i_0][i_1] * A[i_0-1][i_1+2] * 4;
        }
    }
    for (int i_2 = 3; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_L; i_3++) {
            A[i_2-3][i_3] = C[i_3][i_3+i_2] + C[i_2][i_3-1] - C[i_2-2][i_2-2] * C[i_3][i_2] + C[i_2+1][i_3-2] - 3;
        }
    }
#pragma endscop
...
kernel_4224132223_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
