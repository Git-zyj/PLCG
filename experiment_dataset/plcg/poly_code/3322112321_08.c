...
void kernel_3322112321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_1] = D[0] + A[i_1+1][i_0+1] + A[i_0+2][i_1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            B[i_2][i_3] = B[i_2-1][i_3] + B[i_2+1][i_3-2] * B[i_2][i_3-1] - 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < PB_M; i_5++) {
            for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
                C[i_4][i_6+1][i_5] = E[i_6][i_5] * A[i_5][i_6] * C[i_4][i_6+1][i_5-2] * C[i_4][i_6+1][i_5+2] + C[i_4-1][i_6+1][i_5-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3322112321_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
