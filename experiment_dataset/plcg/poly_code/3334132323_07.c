...
void kernel_3334132323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        A[i_0] = D[i_0] * A[i_0+1] * B[i_0][i_0] + 2;
        for (int i_1 = max(1, i_0); i_1 < PB_P-1; i_1++) {
            B[i_1][i_1] = E[i_0][i_1] * B[i_1+1][i_1+1] + B[i_1-1][i_1-1] * 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_2][i_2] = D[i_3-1] + E[i_2][i_3+2] - E[i_2][i_3] - E[i_3][i_3] + E[i_3][i_2] * A[i_3] * 4;
        }
    }
#pragma endscop
...
kernel_3334132323_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
