...
void kernel_4414352211_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_1+PB_L, PB_L-2); i_3++) {
                    A[i_1][i_2][i_3] = E[i_1][i_2][i_3][i_3] * F[i_3+1][i_2][i_0-2][i_1] * G[i_1][i_3][i_0][i_3] + H[i_2-2][i_3+2][i_3+i_1] * A[i_1][i_3+1][i_2] * 2;
                }
            }
        }
        B[i_0] = I[i_0+1] - I[i_0+1] + J[i_0-2] + K[i_0] - I[i_0] - J[i_0-2] - J[i_0-2] + B[i_0-1] * B[i_0+1] * 4;
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_4+PB_M, PB_M-2); i_5++) {
                C[i_5][i_0][i_4] = L[i_5+i_4][i_0-1][0] - L[i_4][i_5][i_0+1] + L[i_5+2][i_0][i_4] + M[i_0][i_4][i_5-1] - A[i_5][i_0-1][i_4] - C[i_5+1][i_0-1][i_4] + C[i_5][i_4+1][i_0] - 6;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M; i_6++) {
        for (int i_7 = 0; i_7 < min(i_6, PB_P-3); i_7++) {
            for (int i_8 = 2; i_8 < PB_P-2; i_8++) {
                for (int i_9 = 0; i_9 < min(-i_8+PB_L+2, -i_6+PB_L+1); i_9++) {
                    D[i_7][i_9+i_6-1][i_8+2][i_6] = F[i_8][i_9][i_6][i_7] + F[i_8+1][i_6][i_9][i_7] + D[i_6][i_9+i_8-2][i_7+3][i_9] + D[i_7][i_9+i_6-1][i_8+1][i_6] * 6;
                }
            }
        }
    }
