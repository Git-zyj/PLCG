...
void kernel_2332232125_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        B[i_0] = D[i_0+2] + E[i_0+1] - E[i_0-2] * 5;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
            B[i_2] = D[i_1] - B[i_2+2] + 1;
        }
        for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
            B[i_3] = F[i_3-2][i_1] + G[i_3][i_1] + H[i_1][i_3+1][i_1] * F[i_3][i_1] + 3;
        }
    }
#pragma endscop
...
kernel_2332232125_03(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
