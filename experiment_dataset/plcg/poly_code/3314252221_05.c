...
void kernel_3314252221_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M-1; i_0++) {
        A[i_0-2] = D[i_0+1] - E[i_0-1] + D[1] - D[i_0] + D[0] - A[i_0-4] * B[i_0] + 6;
        for (int i_1 = 4; i_1 < PB_L; i_1++) {
            B[i_1] = F[i_1][i_0] - G[i_1][i_0][i_0] + F[i_1][i_0] - H[i_0][i_1][0] - I[i_0][i_0] * A[i_1-4] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_2+PB_P-1, PB_L-1); i_4++) {
                C[i_4-1][i_3][i_3+2] = H[i_4][i_2][i_3-1] * G[i_2][i_4+1][i_3] - H[i_3-1][i_2+1][i_3] * H[i_3+i_2][i_4][i_4+i_2+1] - C[i_4-1][i_2+2][i_2+4] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3314252221_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
