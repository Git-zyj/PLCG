...
void kernel_3324252325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] * E[i_0-1] * D[i_0+1] - D[i_0-1] + A[i_0+2] * A[i_0+1] + 3;
    }
    for (int i_1 = 4; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_L-2; i_2++) {
            B[i_1-1][i_2][i_2+i_1] = F[i_1][i_2] * F[i_1][i_2-1] - D[i_2+2] + A[i_2-2] - A[i_2] - B[i_1-3][i_2-2][i_2+i_1-4] * B[i_1+1][i_2][i_2+i_1+2] - 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
            for (int i_5 = 2; i_5 < min(min(PB_P, -i_4+PB_M+1), -i_3+PB_L-1); i_5++) {
                B[i_3][i_5][i_5+i_3+1] = G[i_5][i_3-1][i_3+1] - H[i_5][i_4+1] - G[i_3][i_5][i_4] * I[i_5+i_4-1][i_3][i_4+1] - I[i_3][i_3-2][i_5-2] * G[i_4][i_5][i_3] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3324252325_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
