...
void kernel_4322112321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = i_0; i_2 < PB_N-1; i_2++) {
                A[i_2][i_1] = A[i_2][i_1-2] * A[i_1-1][i_2] - A[i_2+1][i_1] - 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        ...
        for (int i_4 = 0; i_4 < PB_M-4; i_4++) {
            C[i_3][i_4] = D[i_4] * B[i_3] - B[i_4+4] * C[i_3-2][i_4] + C[i_3][i_4+1] * C[i_3+1][i_4] - 1;
        }
    }
#pragma endscop
...
kernel_4322112321_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
