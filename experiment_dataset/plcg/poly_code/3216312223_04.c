...
void kernel_3216312223_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_L, i_0); i_1++) {
            A[i_0][i_1][i_1] = A[i_0-1][i_1][i_1] + A[i_0][i_1-2][i_1-2] + 4;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_L); i_2++) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                A[i_0+2][i_2][i_2] = C[i_3][i_2+i_0][i_3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3216312223_04(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
