...
void kernel_3212311325_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < PB_Q-1; i_2++) {
                B[i_0][i_2-2][i_1+1] = C[i_1][i_0-1][i_2+1] * B[i_0][i_2][i_1+1] * B[i_2][i_0-1][i_1+1] * B[i_0-1][i_2][i_1+1] * 5;
                B[i_0][i_2][i_1+1] = 3;
            }
        }
    }
...
kernel_3212311325_04(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
