...
void kernel_4324332315_05(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
...
#pragma scop
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_M-2; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_M-2; i_3++) {
                    C[i_3+i_1-2][0] = D[i_0][i_1][i_0-1][i_2] * E[i_0][i_3+i_1+2][i_1][i_2-1] + 3;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_L-1; i_5++) {
            for (int i_6 = 0; i_6 < min(PB_P-2, -i_5+PB_M); i_6++) {
                C[i_6+i_5][0] = F[i_4+1][i_5+1][i_6+2] - 3;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
        for (int i_8 = 1; i_8 < -i_7+PB_M-1; i_8++) {
            C[i_8+i_7][0] = G[i_7][i_8-1] + G[i_8+i_7-1][i_8] + H[i_8] - I[i_8+1] - C[i_8+i_7-1][0] * C[i_8+i_7+1][0] - C[i_8+i_7+1][0] - 1;
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4324332315_05(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
