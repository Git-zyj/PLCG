...
void kernel_4422231115_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = E[i_0] + F[i_0] - F[i_0-1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        A[i_1] = E[i_1-1] + E[i_1] - E[i_1] + E[1] + E[i_1-1] * F[i_1] * 4;
        ...
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, PB_L-1); i_4++) {
                for (int i_5 = 1; i_5 < min(-i_3+PB_L, -i_4+PB_M-1); i_5++) {
                    D[i_1+1][i_4+i_3][i_4][i_1+1] = G[i_1][i_5+1][i_5+i_3][0] - H[i_5-1][i_4][i_3][i_1] * D[i_3+2][i_5+i_4+1][i_4+1][i_3+2] * 6;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422231115_00(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
