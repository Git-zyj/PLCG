...
void kernel_4412252123_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0+2] + F[i_0] * E[i_0-2] * F[i_0] + F[i_0] + E[i_0] * F[i_0] - E[i_0+1] + F[i_0] + 6;
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-2; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_0+PB_L-2, -i_1+PB_M+2); i_3++) {
                    B[i_3][i_2][i_2][i_1] = G[i_1][i_3+i_0+2][i_0+1][i_2+2] - H[i_2][i_3+i_0-2][i_3+i_1-2][0] - I[i_3][i_1][1] - G[i_2][i_0][i_3][i_0] - I[i_2][i_1+1][i_1] - B[i_3-2][i_2][i_2][i_0] + B[i_0-2][i_2][i_2][i_3-2] - 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N-3; i_4++) {
        for (int i_5 = 2; i_5 < min(-i_4+PB_L-1, PB_M-2); i_5++) {
            C[i_4+1][i_5-2] = J[i_5+2][i_4+1] + E[i_4] + J[i_5][i_4] + K[i_4][i_5+i_4+1] * C[i_4+3][i_5-2] * 2;
        }
        A[i_4] = F[i_4-1] - E[i_4+2] * 2;
    }
#pragma endscop
...
kernel_4412252123_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
