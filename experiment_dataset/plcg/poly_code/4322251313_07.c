...
void kernel_4322251313_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M-1; i_2++) {
                for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
                    A[i_0][i_1+1][i_3][0] = D[i_1][i_0][i_0][i_3+i_2] - E[i_3][i_0][i_2+1] * F[i_2][i_2+i_0+1][i_1] - E[i_3][i_2-1][1] - G[i_1][i_0-1][i_3][i_2] * D[i_2][i_3-1][i_0][i_1] * D[i_0-1][i_2+1][i_3][i_1] * A[i_0][i_1+1][i_3+1][0] * A[i_1-1][i_3+1][i_0][0] - A[i_0][i_1+1][i_3-1][0] - A[i_0+1][i_1+1][i_3][0] + 6;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < PB_L-1; i_5++) {
            B[i_5] = H[i_4][i_5+1] - B[i_5-1] + 4;
        }
    }
    for (int i_6 = 0; i_6 < PB_N; i_6++) {
        for (int i_7 = 1; i_7 < PB_N; i_7++) {
            for (int i_8 = 1; i_8 < PB_N; i_8++) {
                B[i_6] = E[i_7][i_6][i_6+1] + E[i_6][i_8-1][i_7+1] + F[i_6][i_8-1][i_7] + F[i_8][i_6][0] * E[i_8][i_8-1][i_7-1] - E[i_7][i_6][i_8+1] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4322251313_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
