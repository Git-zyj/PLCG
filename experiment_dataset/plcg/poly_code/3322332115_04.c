...
void kernel_3322332115_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        A[i_0] = D[i_0-1] + E[i_0] - 2;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_L-2); i_1++) {
            B[i_1][i_1] = D[i_0+2] + D[i_1+i_0] + F[i_1+2][i_0+1] + E[i_0+1] - B[i_1-1][i_1-1] - 6;
        }
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_0+PB_M-2, i_0); i_3++) {
                B[i_3-1][i_3-1] = G[i_3+i_0+2][i_2+2][0] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3322332115_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
