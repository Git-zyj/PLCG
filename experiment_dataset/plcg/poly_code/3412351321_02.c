...
void kernel_3412351321_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0+1] + F[i_0+1] - G[i_0] - G[i_0+1] + E[i_0+1] + A[i_0-2] + A[i_0+1] + 4;
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, -i_0+PB_P-1); i_2++) {
                B[i_1][i_2+1][i_0] = H[i_1][i_0][i_2+i_0+1] * H[i_0][i_2][i_1] + I[i_2+i_1][i_0][i_0] + J[i_1][i_2+1][i_0+1] + K[i_2+i_1][0] - J[i_1+1][i_0][i_2-1] * J[i_2][i_1][i_0-1] - 1;
            }
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 3; i_4 < min(-i_3+PB_N, -i_0+PB_P); i_4++) {
                C[i_4][i_0-1][i_4] = H[i_3][i_0][i_3] + I[i_3][i_0][i_4] + L[i_4+i_3][i_0+1] - J[i_4][i_4+i_0][i_3] * C[i_0][i_4-3][i_0] * C[i_3][i_4-3][i_3] * C[i_4][i_0][i_4] * 2;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = 2; i_6 < min(-i_5+PB_L, PB_M-1); i_6++) {
            C[i_5][i_6-1][i_5] = K[i_6+i_5][i_5] - M[0][i_5] - M[i_5-1][i_6+1] * A[i_6] - A[i_6-2] + A[i_6+1] * A[i_6-1] * 5;
        }
    }
#pragma endscop
...
kernel_3412351321_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M));
