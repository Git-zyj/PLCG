...
void kernel_4136211113_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, -i_1+PB_M-1); i_2++) {
                A[i_1][i_2] = B[i_0][i_2+i_1+1][i_0][i_1] * A[i_1-1][i_2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4136211113_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B));
