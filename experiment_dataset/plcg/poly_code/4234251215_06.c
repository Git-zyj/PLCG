...
void kernel_4234251215_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_P-1 - 1; i_0 >= 1; i_0--) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_0-1][i_1][i_0+1] = C[i_0-1][i_1] * C[i_0][i_1+1] - D[i_1] * C[i_1-1][i_1+1] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2] = D[i_2] - E[i_2] * D[i_2] - D[i_2] + D[0] - E[i_2] + B[i_2-1] - B[i_2+1] * B[i_2+1] + B[i_2-1] + 2;
    }
#pragma endscop
...
kernel_4234251215_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
