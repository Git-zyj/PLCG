...
void kernel_2224132121_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        ...
        for (int i_2 = max(1, i_0); i_2 < min(PB_P, -i_0+PB_N); i_2++) {
            B[i_0][i_2] = C[i_2+2][i_0] * D[i_2] - C[i_0][i_2] + C[i_2+i_0][i_0] - C[i_0][i_2-1] + B[i_0+2][i_2] * 5;
        }
    }
#pragma endscop
...
kernel_2224132121_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
