...
void kernel_2214132313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
            A[i_2-1] = C[i_0] + D[i_0][i_2] + D[i_2+1][i_0] - D[i_2-2][i_0] * D[i_2+2][i_0] * A[i_2] * 2;
        }
    }
#pragma endscop
...
kernel_2214132313_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
