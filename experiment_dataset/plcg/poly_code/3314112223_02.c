...
void kernel_3314112223_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M-4; i_2++) {
        B[i_2+2] = E[i_2] + B[i_2+4] + B[i_2+3] + 1;
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < -i_2+PB_P-1; i_4++) {
                C[i_2][i_3][i_4+i_2] = C[i_2][i_3][i_4+i_2+1] + C[i_2-2][i_3][i_4+i_2-2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3314112223_02(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
