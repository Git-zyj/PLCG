...
void kernel_2232132111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N+2; i_0++) {
        A[i_0-2] = C[i_0] * C[i_0-2] * C[i_0+2] - C[i_0] + A[i_0-3] - 5;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        B[i_1-2] = C[i_1-2] - C[i_1] * 6;
    }
#pragma endscop
...
kernel_2232132111_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
