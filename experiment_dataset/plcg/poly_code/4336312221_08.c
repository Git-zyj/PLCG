...
void kernel_4336312221_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L-1; i_2++) {
                for (int i_3 = i_1; i_3 < -i_0+PB_M-2; i_3++) {
                    A[i_1-2][i_3+i_0][i_2+2][i_3] = D[i_3+2][i_1][i_2+i_0][i_2+i_0+1] - A[i_1-2][i_2+i_0-2][i_3+2][i_0] * A[i_1-2][i_3+i_0-1][i_2+2][i_3-2] * A[i_1-2][i_3+i_0+2][i_2+1][i_3+2] * 5;
                }
            }
        }
    }
    ...
    for (int i_8 = 0; i_8 < PB_Q; i_8++) {
        for (int i_9 = 1; i_9 < min(i_8, PB_N-2); i_9++) {
            for (int j_0 = 0; j_0 < PB_Q; j_0++) {
                for (int j_1 = 0; j_1 < PB_M; j_1++) {
                    B[j_1][i_8][1][i_9-1] = E[i_9+2][i_9][j_1][i_8] * 4;
                }
            }
        }
    }
...
kernel_4336312221_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
