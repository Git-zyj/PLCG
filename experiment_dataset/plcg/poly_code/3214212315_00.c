...
void kernel_3214212315_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_1-2][i_2] = C[i_1][i_2+2][i_0] - B[i_1-1][i_2] + 3;
            }
            B[i_0-1][i_1+1] = B[i_0][i_1+2] * B[i_0-1][i_1] - 5;
        }
    }
#pragma endscop
...
kernel_3214212315_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
