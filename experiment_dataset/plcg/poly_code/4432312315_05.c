...
void kernel_4432312315_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0] = 3;
        A[i_0] = E[i_0+2] - F[i_0] + A[i_0] + 5;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            C[i_2][i_1] = G[i_1+1][i_2] + C[i_2+1][i_1+1] * C[i_1][i_2-1] - C[i_2+1][i_1] * C[i_2-1][i_1] + 4;
        }
        for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                C[i_4][i_3] = H[i_4][i_1][i_3] + C[i_3-1][i_4] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4432312315_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
