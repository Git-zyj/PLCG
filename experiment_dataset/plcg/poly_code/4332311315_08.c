...
void kernel_4332311315_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0-1] = A[i_1][i_0-2] + A[i_1+1][i_0-1] + A[i_1-1][i_0-1] - 1;
        }
    }
    ...
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        C[i_3+1] = D[i_3+1] - D[i_3] * C[i_3] + C[i_3+2] - C[i_3+2] * 5;
    }
#pragma endscop
...
kernel_4332311315_08(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
