...
void kernel_3336112225_04(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_M, i_1); i_2++) {
                C[i_2][i_0-1] = D[i_1][i_0][i_2] - 6;
            }
        }
    }
    ...
    for (int i_5 = 2; i_5 < PB_L; i_5++) {
        for (int i_6 = 2; i_6 < i_5; i_6++) {
            C[i_6][i_5-2] = E[i_6][i_5] + E[i_6-2][i_5] - 5;
        }
    }
#pragma endscop
...
kernel_3336112225_04(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
