...
void kernel_3216132125_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = min(PB_M, -i_0+PB_N-2) - 1; i_1 >= 0; i_1--) {
            A[i_1+i_0][i_0] = C[i_1][i_0] + C[i_1][i_1+i_0] + C[i_0+2][i_1] * C[i_1][i_1+i_0] * A[i_1+i_0+2][i_0] - 3;
        }
    }
...
kernel_3216132125_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
