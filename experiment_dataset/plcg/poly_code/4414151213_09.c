...
void kernel_4414151213_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - E[i_0-1] * E[i_0+1] * E[i_0] - F[i_0][i_0-1] * E[i_0] + E[i_0-1] - A[i_0+1] * C[i_0-1] - C[i_0] * 1;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M-1, -i_1+PB_M-1); i_2++) {
                for (int i_3 = 1; i_3 < PB_P; i_3++) {
                    B[i_2+i_1][i_2+i_0+1][i_2][i_3-1] = G[i_2][i_1][i_3] + H[i_1-1][i_0][i_3-1][i_2-1] + G[i_2][i_3][i_1-1] * B[i_3+i_2][i_2+i_1+1][i_2][i_0-2] * 5;
                }
            }
        }
        C[i_0] = E[i_0+1] * E[0] - E[i_0] * E[i_0] + A[i_0] - 4;
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        C[i_4+1] = E[i_4] + E[i_4] - E[i_4-1] * E[1] + E[i_4] - 6;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414151213_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
