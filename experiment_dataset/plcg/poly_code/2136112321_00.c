...
void kernel_2136112321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_Q-2 - 1; i_0 >= 2; i_0--) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q-2, PB_L-2); i_1++) {
            A[i_1+2][i_1+i_0] = B[i_1][i_0] + A[i_1+2][i_1+i_0+1] * A[i_1+2][i_1+i_0+2] - A[i_1+2][i_1+i_0-2] - 4;
        }
    }
#pragma endscop
...
kernel_2136112321_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
