...
void kernel_4334352211_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
        ...
        for (int i_1 = 2; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M-2; i_2++) {
                for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
                    A[i_0][i_2][i_1+2][i_3+2] = D[i_0-2][i_3][i_2+1] + E[i_3][i_1-1][i_3][0] * F[i_3+1][i_1][i_2] + G[i_0][i_1][i_2][i_3][i_1-1] + E[i_2+i_0+2][i_1-2][i_3+2][0] + E[i_2][i_3-2][i_0][i_1-1] - A[i_0-1][i_3][i_1+2][i_2+2] - 1;
                }
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < -i_4+PB_L; i_5++) {
            for (int i_6 = i_5; i_6 < -i_4+PB_P; i_6++) {
                for (int i_7 = 2; i_7 < -i_4+PB_L-2; i_7++) {
                    B[i_4][i_5-2][i_6+i_4][i_7] = H[i_6][i_5+i_4][i_7+1][i_5][i_7+2] + I[i_6+i_5][i_4][i_5][i_4] * E[i_5][i_6][i_7-1][i_7+i_4] + E[i_6+i_5-1][i_7+i_4+2][i_7-2][i_6+2] + B[i_4][i_7-2][i_6+i_4][i_5-1] * 2;
                }
            }
        }
        for (int i_8 = 0; i_8 < min(i_4, -i_4+PB_L); i_8++) {
            C[i_8][i_4-2] = J[i_8+2][i_4] * K[i_8] - L[i_8+i_4][1] - C[i_8][i_4-1] - C[i_8][i_4-3] - 2;
        }
    }
#pragma endscop
