...
void kernel_3116131325_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(min(PB_P-2, -i_0+PB_M-1), i_0); i_1++) {
            A[i_1+i_0][i_1+1][0] = B[i_1-1][i_0] + C[i_1+i_0+1][i_0][i_1] - D[0] - A[i_1+i_0+1][i_1+1][0] + A[i_1+i_0][i_1+2][0] - A[i_1+i_0-2][i_1+1][0] - 3;
        }
    }
#pragma endscop
...
kernel_3116131325_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
