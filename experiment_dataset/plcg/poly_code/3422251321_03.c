...
void kernel_3422251321_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = E[i_0] + E[i_0-1] - F[i_0-1] + B[i_0+1] - B[i_0] * B[i_0-1] + 5;
        B[i_0] = F[1] * E[i_0] * B[i_0-1] - 5;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
            for (int i_3 = 0; i_3 < -i_2+PB_N; i_3++) {
                C[i_1][i_2+1][i_1] = G[i_2-1][i_3] - G[1][i_1+1] + H[i_3][i_3+i_2-1] + I[i_2+1][i_1][i_3+i_2+1] * I[i_3+i_2][i_1-1][i_1-1] - C[i_1+1][i_2+2][i_1+1] - C[i_1-2][i_2+2][i_1-2] - C[i_1-2][i_2+1][i_1-2] * C[i_1+2][i_2+1][i_1+2] - C[i_1+1][i_2-1][i_1+1] + 3;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        D[i_4] = F[i_4-1] * E[i_4] + H[i_4+1][0] - E[i_4+1] + F[i_4-1] * F[i_4-1] * E[1] + D[i_4-2] - 3;
    }
#pragma endscop
...
kernel_3422251321_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
