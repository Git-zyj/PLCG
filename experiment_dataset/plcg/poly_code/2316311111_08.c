...
void kernel_2316311111_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0][i_0+1] = B[i_0-1] - 4;
        B[i_0] = D[i_0] + E[i_0] + A[i_0][i_0+1] + A[i_0-1][i_0] + 5;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_N; i_2++) {
            C[i_2][i_2] = D[i_2+i_1] * 2;
        }
    }
#pragma endscop
...
kernel_2316311111_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
