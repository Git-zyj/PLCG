...
void kernel_3422112211_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_0][i_1+1][i_2] = A[i_0+1][i_1+1][i_2] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_P-2; i_4++) {
            for (int i_5 = i_3; i_5 < PB_L; i_5++) {
                B[i_3-1][i_5][2] = C[i_4-1][i_5] - D[i_3+1][i_4+2][i_5] + 6;
            }
            C[i_3][i_4+1] = B[i_3-1][i_4-1][2] + 4;
        }
    }
    for (int i_6 = 1; i_6 < PB_P-2; i_6++) {
        for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
            for (int i_8 = 0; i_8 < min(i_6, -i_6+PB_M); i_8++) {
                D[i_8][i_6+2][i_7] = E[i_7+1][i_6][i_7-1] - E[i_6-1][i_8][1] + E[i_8+i_6][i_7][i_7] + C[i_7][i_8+1] * C[i_7][i_8] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3422112211_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
