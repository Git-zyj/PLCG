...
void kernel_2434331125_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        B[i_0] = E[i_0] * E[i_0-1] * E[i_0] - 3;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        B[i_1] = E[i_1+1] * 4;
        for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
            B[i_1+2] = F[i_2] - G[i_2+i_1][i_2] - H[i_2+1][i_1+1] - D[i_2-1] * 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        D[i_3] = F[i_3] + F[i_3] + F[i_3+1] * I[i_3+1] - I[i_3] - D[i_3-1] * 4;
    }
#pragma endscop
...
kernel_2434331125_02(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
