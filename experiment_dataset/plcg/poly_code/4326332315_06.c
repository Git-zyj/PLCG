...
void kernel_4326332315_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
...
#pragma scop
        ...
        for (int i_1 = 3; i_1 < PB_P; i_1++) {
            for (int i_2 = i_1; i_2 < PB_Q-2; i_2++) {
                for (int i_3 = 0; i_3 < i_0; i_3++) {
                    A[i_1-2][i_2+2][i_3] = D[i_3][i_0][i_1][i_2+2] * A[i_1-3][i_3+2][i_0] * A[i_2-2][i_3+1][i_1] + A[i_1-1][i_2+2][i_3] * A[i_1-1][i_3+2][i_2] * 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_M; i_5++) {
            B[i_5] = E[i_5][i_4] - E[i_5-1][i_4] + E[i_4][i_5] + B[i_5+1] + C[i_5+1][i_4-2] + C[i_5-1][i_4-2] * C[i_5][i_4-2] * 6;
            C[i_5][i_4-2] = F[i_5+i_4][i_5][i_4] * E[i_5][i_4] - G[i_4][i_5] * E[i_5][i_4-1] - B[i_5] * 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4326332315_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
