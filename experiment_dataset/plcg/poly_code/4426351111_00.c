...
for (i_0 = 0; i_0 < xq; i_0++) {
    for (i_1 = 0; i_1 < yq; i_1++) {
        Q[i_0][i_1] = 0.9 + (rand() / (DATA_TYPE)RAND_MAX) * (1.1 - 0.9);
    }
}
}
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn),int xo,int yo,DATA_TYPE POLYBENCH_2D(O,xO,yO,xo,yo),int xp,int yp,DATA_TYPE POLYBENCH_2D(P,xP,yP,xp,yp),int xq,int yq,DATA_TYPE POLYBENCH_2D(Q,xQ,yQ,xq,yq))
{
int i_0, i_1, i_2, i_3, i_4;
POLYBENCH_DUMP_START;
    ...
    for (i_0 = 0; i_0 < xq; i_0++) {
        for (i_1 = 0; i_1 < yq; i_1++) {
            fprintf(POLYBENCH_DUMP_TARGET, "\n");
            fprintf (POLYBENCH_DUMP_TARGET, DATA_PRINTF_MODIFIER, Q[i_0][i_1]);
        }
    }
    ...
    fprintf(POLYBENCH_DUMP_TARGET, "\nsum: ");
    fprintf (POLYBENCH_DUMP_TARGET, DATA_PRINTF_MODIFIER, sum_Q);
    POLYBENCH_DUMP_END("Q");
#endif
POLYBENCH_DUMP_FINISH;
}
void kernel_4426351111_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn),int xo,int yo,DATA_TYPE POLYBENCH_2D(O,xO,yO,xo,yo),int xp,int yp,DATA_TYPE POLYBENCH_2D(P,xP,yP,xp,yp),int xq,int yq,DATA_TYPE POLYBENCH_2D(Q,xQ,yQ,xq,yq)){
        ...
        for (int i_1 = 1; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_Q-1); i_3++) {
                    A[i_0][i_3+i_2+1][i_1][0] = E[i_1][i_0][i_3][i_2] - F[i_1+1][i_2-1][i_3] * G[i_3][i_2+1][i_0][i_1] * H[i_3][i_2][i_0][i_1-1] + I[i_1][i_2][i_3][i_0][1] + 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        B[i_4-1] = J[i_4] * K[i_4] * J[i_4] - L[i_4+1] * 3;
    }
    for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 0; i_7 < min(min(i_6, -i_6+PB_M), -i_5+PB_L-1); i_7++) {
                C[i_6][i_5][i_7+1] = F[i_7][i_6+1][i_5+1] + M[i_7+i_6][i_5][i_6+1] - N[i_6+1][i_5][1] + M[i_7][i_5][i_6] + O[i_6][i_7+i_5+1] + 1;
            }
        }
        for (int i_8 = 1; i_8 < min(-i_5+PB_M-1, i_5); i_8++) {
            D[i_5+1][i_8] = O[i_8+1][i_8+1] - P[i_8+1][i_5] * O[i_8][i_5] - Q[i_5+1][i_8] * O[i_8+i_5+1][1] * D[i_5+2][i_8-1] + D[i_5+1][i_8+1] + D[i_8+1][i_5+1] * 2;
        }
    }
#pragma endscop
...
POLYBENCH_2D_ARRAY_DECL(Q, DATA_TYPE, xQ,yQ,xq,yq);
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N), xo,yo,POLYBENCH_ARRAY(O), xp,yp,POLYBENCH_ARRAY(P), xq,yq,POLYBENCH_ARRAY(Q), INIT_SEED);
kernel_4426351111_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N), xo,yo,POLYBENCH_ARRAY(O), xp,yp,POLYBENCH_ARRAY(P), xq,yq,POLYBENCH_ARRAY(Q));
