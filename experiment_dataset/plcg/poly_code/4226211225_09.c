...
void kernel_4226211225_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                A[i_2][i_1] = A[i_1+2][i_2] - 5;
            }
        }
        for (int i_3 = 0; i_3 < min(PB_N-1, i_0); i_3++) {
            for (int i_4 = 2; i_4 < PB_N; i_4++) {
                B[i_4][i_0-1][i_3+1] = C[i_4][i_0][i_3+1] - B[i_4-2][i_0-1][i_3+1] * B[i_4][i_0-1][i_3] + B[i_4][i_0-2][i_3+1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4226211225_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
