...
void kernel_3436232213_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        B[i_0+1] = E[i_0+2] - F[i_0] * 1;
    }
    ...
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_N; i_3++) {
            B[i_2] = G[i_3+i_2][0] * G[i_3][i_2] - H[i_3+1][i_2] + E[i_3] + E[i_3+i_2] * 6;
        }
        for (int i_4 = 1; i_4 < min(-i_2+PB_L, i_2); i_4++) {
            D[i_4][i_4+i_2] = G[i_4][i_2] * H[i_4][i_2] + I[i_4][i_2][2] * F[i_4-1] * D[i_4-1][i_4+i_2-1] + D[i_4+1][i_4+i_2] * 2;
        }
    }
#pragma endscop
...
kernel_3436232213_06(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
