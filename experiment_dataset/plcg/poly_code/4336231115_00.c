...
void kernel_4336231115_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
        ...
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_L-1); i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = 1; i_3 < i_1; i_3++) {
                    A[i_3-1][i_2][i_2-1][i_0-1] = D[i_3+i_2][i_1+i_0+1][i_2][i_3] * D[i_3][i_0][i_1][i_2-1] * E[i_2+i_0][i_1+1][i_3] - F[i_3][i_0][i_1+1][i_2-1] + F[i_0][i_3-1][i_1+1][i_2] * E[i_0][i_1][i_2] * 3;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        B[i_4+1] = G[i_4] + B[i_4+2] + 3;
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            B[i_4+1] = H[i_5-1][i_4+1] * B[i_5] * 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4336231115_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
