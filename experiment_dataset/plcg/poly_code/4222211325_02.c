...
void kernel_4222211325_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_N-1, PB_M-1); i_1++) {
            for (int i_2 = PB_L+1 - 1; i_2 >= 3; i_2--) {
                A[i_0+1][i_2-1][i_1] = A[i_0+1][i_2-2][i_1] - A[i_0+3][i_2-1][i_1] - A[i_0-1][i_2-3][i_1] * A[i_0][i_2-1][i_1] - 2;
            }
            B[i_1-1] = C[i_1+i_0+1][i_1] + C[i_1+i_0][i_0] - B[i_1+1] - B[i_1] * 4;
        }
    }
#pragma endscop
...
kernel_4222211325_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
