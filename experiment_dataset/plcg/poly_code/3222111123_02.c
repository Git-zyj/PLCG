...
void kernel_3222111123_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_N); i_2++) {
                B[i_2][i_2+i_1] = C[i_2][i_0-1][i_0] + C[i_1][i_0][i_2] - 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 2; i_4 < -i_3+PB_N; i_4++) {
            B[i_4][i_4+i_3] = B[i_4-2][i_4+i_3-1] + 1;
        }
    }
#pragma endscop
...
kernel_3222111123_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
