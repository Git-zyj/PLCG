...
void kernel_3312311315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M-1; i_2++) {
            C[i_2+i_1][i_2] = D[i_1][i_2] * E[i_2+1] - 6;
            C[i_2+i_1][i_2] = D[i_2+1][i_1+1] - C[i_2+i_1+1][i_2+1] + C[i_2+i_1][i_2-1] * 2;
        }
    }
#pragma endscop
...
kernel_3312311315_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
