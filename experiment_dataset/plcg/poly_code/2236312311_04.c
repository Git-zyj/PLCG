...
void kernel_2236312311_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        A[i_0-2] = C[i_0][0] + A[i_0-3] * 6;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = max(3, i_1); i_2 < PB_L+1; i_2++) {
            B[i_1][i_2-2] = B[i_1-1][i_2-2] - B[i_1][i_2-1] - B[i_1][i_2-3] + B[i_1-1][i_2-3] - B[i_1-1][i_2-1] - 2;
        }
    }
#pragma endscop
...
kernel_2236312311_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
