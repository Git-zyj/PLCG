...
void kernel_4216111123_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
        ...
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = max(1, i_2); i_3 < PB_Q-1; i_3++) {
                    A[i_2][i_1][i_3-1] = A[i_2][i_1][i_3+1] * 1;
                }
            }
        }
        for (int i_4 = 0; i_4 < i_0; i_4++) {
            for (int i_5 = 2; i_5 < i_4; i_5++) {
                A[i_5-2][i_4][i_0-1] = C[i_0+1][i_5][i_4] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4216111123_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
