...
void kernel_3336212325_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[0] = D[i_0] * A[0] - 6;
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            B[i_0-1][i_1-2] = E[i_1][i_0+2] * A[0] + B[i_0-1][i_1] + 2;
        }
    }
    for (int i_2 = 3; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            B[i_2-1][i_3-2] = E[i_3][i_2] * B[i_2-1][i_3] - B[i_2-3][i_3-2] * 6;
        }
    }
#pragma endscop
...
kernel_3336212325_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
