...
void kernel_3326252223_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        B[i_0] = D[i_0] + E[i_0-2] - 4;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        B[i_1] = D[i_1+2] - E[i_1] + D[i_1] - E[i_1] * B[i_1+2] * 6;
        for (int i_2 = max(1, i_1); i_2 < min(-i_1+PB_L-2, PB_M); i_2++) {
            for (int i_3 = 2; i_3 < min(min(-i_1+PB_M, PB_M-3), -i_2+PB_L-1); i_3++) {
                C[i_1+1][i_3+i_2][i_2] = F[i_2][i_1][i_3] + F[i_3+i_2+1][i_2][i_3] * G[i_3][i_2] + H[i_3][i_2] - I[i_2][i_3+i_1][0] + I[i_3+1][i_1][i_2+1] * G[i_3][i_1] - I[i_2-1][i_3-2][i_1] - C[i_3+3][i_2+i_1][i_2] - C[i_3-1][i_2+i_1+2][i_2+1] - 6;
            }
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3326252223_02(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
