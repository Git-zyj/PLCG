...
void kernel_4434251213_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = E[i_0] * F[i_0-1] + F[i_0+1] * E[i_0] + A[i_0] + A[i_0-2] - A[i_0-2] - 3;
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < -i_0+PB_L-1; i_3++) {
                    B[i_2][i_3][i_2+1][i_0+1] = G[i_0][i_3][i_2][i_1] + G[i_3+1][i_2][i_1+1][i_0-1] * G[i_3-1][i_0-1][i_1-1][i_0+1] * H[i_1+1][i_3+i_0+1][i_3][i_0] * H[i_1-1][i_0+1][i_2][0] + 2;
                }
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < min(min(i_4, -i_4+PB_P+1), PB_M-1); i_5++) {
            for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 1; i_7 < PB_L; i_7++) {
                    C[i_6][i_5+1][i_7] = H[i_6][i_7][i_4-1][0] * I[i_7-1][i_6-1][i_5] - H[i_6][i_4][i_5+1][i_7] + H[i_6][i_7][i_5+i_4-1][0] * G[i_5][i_4][i_7][i_6] * G[i_6][i_4][i_6+1][i_7] - C[i_7][i_6+1][i_4-1] * 3;
                }
            }
        }
    }
    for (int i_8 = 0; i_8 < PB_L-1; i_8++) {
        for (int i_9 = 1; i_9 < -i_8+PB_L-1; i_9++) {
            for (int j_0 = 1; j_0 < PB_L-2; j_0++) {
                B[j_0+1][i_8][j_0+2][i_9+1] = J[j_0+i_9][i_8][i_9-1] + J[i_9][j_0][i_8+1] * K[j_0+i_9][i_8+1] - L[j_0-1][i_9+i_8+1] + 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4434251213_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L)));
