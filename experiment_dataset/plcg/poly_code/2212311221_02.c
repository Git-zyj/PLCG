...
void kernel_2212311221_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_P, -i_0+PB_L-1); i_1++) {
            A[i_0+1][i_1-1] = C[i_1+i_0+1][i_1] + A[i_0+3][i_1-1] * 3;
            A[i_0+1][i_1-1] = D[i_0-1][i_1] + A[i_0+1][i_1-3] + 2;
        }
    }
#pragma endscop
...
kernel_2212311221_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
