...
void kernel_3334111321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < min(PB_M, -i_1+PB_N); i_2++) {
                A[i_1][i_2] = D[i_2+i_1][i_0] * A[i_2][i_1+1] - A[i_2][i_1-2] + A[i_1][i_2-2] - A[i_1][i_0+1] - 3;
            }
        }
        for (int i_3 = 3; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                B[i_3-1][i_3-1] = B[i_3][i_3] * B[i_0][i_0] - B[i_3-2][i_3-2] + B[i_3-3][i_3-3] - B[i_3+1][i_3+1] + 2;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M+1; i_5++) {
        for (int i_6 = 1; i_6 < i_5; i_6++) {
            C[i_6-1][i_5-1] = D[i_6][i_5+1] - 1;
        }
    }
#pragma endscop
...
kernel_3334111321_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
