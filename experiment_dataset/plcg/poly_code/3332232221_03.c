...
void kernel_3332232221_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-4; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_0+2][i_1] = D[i_1][i_1] - E[i_0-1] - A[i_0+1][i_1] + A[i_0+2][i_1+2] - A[i_0+4][i_1-1] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 3; i_3 < PB_M-2; i_3++) {
            B[i_2][i_3-1] = D[i_3][i_2+2] - F[i_2-2][i_3+2] + B[i_2][i_3-3] + 5;
        }
    }
    for (int i_4 = 2; i_4 < PB_P; i_4++) {
        for (int i_5 = 0; i_5 < PB_M; i_5++) {
            for (int i_6 = i_4; i_6 < min(PB_M, -i_4+PB_P); i_6++) {
                C[i_6][i_5] = G[i_6][i_4-2][i_5] * H[i_5][i_6+2][i_4] * G[i_5][i_6+i_4][i_4] * C[i_6-2][i_5] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3332232221_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
