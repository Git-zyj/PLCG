...
void kernel_2214231211_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        B[i_0+1] = C[i_0] * D[i_0][1] - B[i_0] * 6;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_L; i_2++) {
            B[i_2] = E[i_2+i_1][i_2-1] + C[i_2-1] * D[i_2][i_1] + B[i_1+1] + 6;
        }
    }
#pragma endscop
...
kernel_2214231211_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
