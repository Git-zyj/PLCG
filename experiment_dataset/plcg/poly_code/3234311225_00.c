...
void kernel_3234311225_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
            B[i_2+1] = C[i_2][i_1] + B[i_2-1] + B[i_2+2] + 5;
        }
    }
#pragma endscop
...
kernel_3234311225_00(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
