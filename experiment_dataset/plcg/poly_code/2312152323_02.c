...
void kernel_2312152323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            A[i_1][i_0] = D[i_0][i_1] * D[i_0][i_1] - D[i_1][i_0] - E[i_1] + D[i_1][i_0] * D[i_1][i_1] * D[i_1][i_0+2] + A[i_1-1][i_0] - A[i_1+1][i_0] + 1;
        }
        for (int i_2 = 2; i_2 < PB_N; i_2++) {
            B[i_0][i_2-1] = D[i_2][i_0] - B[i_0+1][i_2-1] + B[i_0+1][i_2-2] - B[i_0-1][i_2] + B[i_0][i_2+1] * 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_N, PB_N-2); i_4++) {
            C[i_3][i_4] = E[i_4+i_3] + D[i_4+i_3][i_4] * E[i_3+1] - D[i_4][i_3] - D[i_4+2][i_3] * D[i_3][i_4] * D[i_4][i_3] + C[i_3][i_4+1] * C[i_3-1][i_4] + 4;
        }
    }
#pragma endscop
...
kernel_2312152323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
