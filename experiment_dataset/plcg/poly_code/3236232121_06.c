...
void kernel_3236232121_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1] = C[i_1][i_0] + C[i_1][i_0] - C[i_1-1][i_1] * A[i_0-1] + A[i_1+2] - 4;
        }
        for (int i_2 = i_0; i_2 < min(-i_0+PB_M, PB_P-1); i_2++) {
            B[i_0][i_2+1] = C[i_2-1][i_0] + C[i_2+i_0][i_2+i_0] + 4;
        }
    }
#pragma endscop
...
kernel_3236232121_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
