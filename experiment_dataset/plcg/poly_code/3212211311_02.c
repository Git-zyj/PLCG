...
void kernel_3212211311_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < min(min(-i_1+PB_L-1, i_1), PB_M-1); i_2++) {
            for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                B[i_2+i_1+1][i_3] = C[i_2+1][i_1] - B[i_3+i_2][i_1] - B[i_2+i_1+1][i_3+1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3212211311_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
