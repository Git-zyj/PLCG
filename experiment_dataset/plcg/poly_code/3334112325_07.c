...
void kernel_3334112325_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            ...
            for (int i_2 = i_0; i_2 < PB_L-1; i_2++) {
                A[i_1+2][i_1+2] = D[i_2+1][i_1-1] + C[i_0][i_2-1] + C[i_1-1][i_2-1] + 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M+2; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L; i_4++) {
            C[i_3-2][i_4-1] = C[i_3-2][i_4] * 1;
        }
    }
#pragma endscop
...
kernel_3334112325_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
