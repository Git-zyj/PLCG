...
void kernel_4434351325_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,int vm,int wm,DATA_TYPE POLYBENCH_5D(M,xM,yM,zM,vM,wM,xm,ym,zm,vm,wm),int xn,int yn,int zn,int vn,DATA_TYPE POLYBENCH_4D(N,xN,yN,zN,vN,xn,yn,zn,vn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_0][i_1+1] = E[i_1+i_0+1][i_1] + F[i_0][i_1] * G[i_1+1] - F[i_0][i_1+1] + H[i_0-1][i_1] + I[i_1+1] - F[i_1][i_0] - A[i_0][i_1-1] + A[i_0-2][i_1+1] + C[i_1] * C[i_1+1] * C[i_1+2] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            for (int i_4 = i_3; i_4 < -i_3+PB_P; i_4++) {
                for (int i_5 = 0; i_5 < min(i_4, -i_2+PB_M); i_5++) {
                    D[i_5+i_2][i_4+i_3][i_2][i_5+1] = J[i_4][i_2][i_3][i_5] + J[i_3][i_2][i_4-1][i_5+i_2] * K[i_4+i_3+1][i_5+i_2][0][i_3-1] - 4;
                }
            }
        }
        C[i_2] = I[i_2] + L[i_2+1] - L[i_2] - G[i_2-1] + G[i_2] + C[i_2+2] - C[i_2+1] - 2;
    }
    for (int i_6 = 1; i_6 < PB_M; i_6++) {
        for (int i_7 = 2; i_7 < min(i_6, PB_M-1); i_7++) {
            for (int i_8 = max(0, -i_7+2); i_8 < -i_7+PB_P; i_8++) {
                for (int i_9 = 1; i_9 < min(-i_6+PB_M, -i_8+PB_P-1); i_9++) {
                    D[i_9+i_6][i_8+i_7][i_6][i_9+1] = J[i_8][i_6][i_7][1] + M[i_7+1][i_6-1][i_9+i_8+1][i_9][i_8] * J[i_6][i_7-1][i_9-1][i_8] - N[i_6][i_8][i_7][0] * J[i_9][i_7+1][i_6][i_8+1] * D[i_9+i_6-1][i_8+i_7][i_9][i_6] * D[i_9+i_6][i_8+i_7-2][i_9][i_6+1] + 1;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4434351325_08(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,POLYBENCH_ARRAY(N));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,POLYBENCH_ARRAY(N)));
