...
void kernel_4312232313_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
        ...
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 2; i_3 < -i_2+PB_M+1; i_3++) {
                    B[i_0][i_2+1][i_1] = D[i_3+i_2-1][i_3-2][i_0+1][i_0+2] + E[i_3][i_2+2][i_1][i_0+2] * 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_L-1; i_5++) {
            for (int i_6 = 1; i_6 < min(-i_5+PB_L, PB_M-1); i_6++) {
                B[i_6][i_5][i_4] = F[i_6-1][i_4][i_5] * F[i_6][i_5-1][i_4] * G[i_5-2][i_4] * H[i_4][i_6+i_5][i_4] * B[i_6][i_5-1][i_4] - B[i_6+1][i_5-1][i_4] - B[i_6][i_5+1][i_4] - B[i_6+1][i_5][i_4-1] + 2;
            }
        }
        C[i_4][i_4] = G[i_4][i_4+2] - I[i_4] * I[i_4] + C[i_4+1][i_4+1] + 4;
    }
#pragma endscop
...
kernel_4312232313_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
