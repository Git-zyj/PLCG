...
void kernel_2212211123_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_Q-3; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            A[i_3][i_2-1] = D[i_2] + A[i_3][i_2+3] - 5;
        }
    }
#pragma endscop
...
kernel_2212211123_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
