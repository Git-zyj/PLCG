...
void kernel_3212111321_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_N, -i_0+PB_P); i_1++) {
            for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
                A[i_1-1][i_2+1][i_1] = C[i_0][i_2][i_1+i_0] + A[i_1-1][i_2][i_1] + A[i_1-1][i_2-1][i_1] + 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
            B[i_3][i_4] = B[i_3][i_4-2] + B[i_3-1][i_4+2] + B[i_3-2][i_4-2] + 1;
        }
    }
#pragma endscop
...
kernel_3212111321_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
