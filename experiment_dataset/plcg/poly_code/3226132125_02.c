...
void kernel_3226132125_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_L-2; i_2++) {
                B[i_2-2] = C[i_2+i_0][i_1][i_1] * C[i_0+2][i_1][i_2] + C[i_2][i_1+i_0][i_2] * D[i_2+i_0+2][i_1] - 2;
            }
        }
        for (int i_3 = 3; i_3 < i_0; i_3++) {
            B[i_3-2] = E[i_3] + D[i_3][i_0] * B[i_3-3] - 5;
        }
    }
#pragma endscop
...
kernel_3226132125_02(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
