...
void kernel_4314111315_02(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
        ...
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, PB_L-2); i_2++) {
                for (int i_3 = max(1, i_2); i_3 < -i_2+PB_L-1; i_3++) {
                    B[i_1-1][0][i_0][i_3+i_2+1] = C[i_3-1][i_2+2][i_2+2][i_0] + 1;
                }
            }
        }
    }
        ...
        for (int i_5 = 1; i_5 < -i_4+PB_L; i_5++) {
            for (int i_6 = 0; i_6 < i_5; i_6++) {
                for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
                    B[i_4][0][i_6][i_7+i_5] = D[i_7+1][i_4][i_5][i_6+1] - B[i_4+1][0][i_6][i_7+i_5-1] - B[i_6][0][i_7][i_5+i_4] - 1;
                }
            }
            for (int i_8 = 0; i_8 < i_5; i_8++) {
                for (int i_9 = 0; i_9 < PB_L-1; i_9++) {
                    C[i_4][i_8+1][i_8+1][i_5] = E[i_9+1][i_5][i_8] - C[i_4][i_8+2][i_8+2][i_5] + C[i_4][i_8+1][i_8+1][i_9+1] * 6;
                }
            }
        }
    }
...
kernel_4314111315_02(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
