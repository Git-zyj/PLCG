...
void kernel_3414251215_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_L, i_1); i_2++) {
                A[i_2][i_0+1] = E[i_1][i_2+i_0][i_0] - F[i_1][i_2][i_0] + G[i_2][i_0] + E[i_1-1][i_0][i_1] + E[i_2][i_0-1][i_0] - A[i_2][i_1+2] + A[i_2+1][i_0+1] - 1;
            }
        }
        C[i_0] = H[i_0] - I[i_0] + I[i_0] - H[i_0-1] * G[i_0][i_0] * H[i_0] - I[i_0] * H[i_0] * C[i_0] + 3;
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        C[i_3] = H[i_3] - H[i_3] + C[i_3+1] - 5;
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                A[i_5][i_4+1] = F[i_5][i_3][i_4] * J[i_4][0] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3414251215_00(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
