...
void kernel_3216132123_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_M+2); i_1++) {
            B[i_1+i_0-2][i_0+1] = C[i_1-1] - D[i_1-2][i_0] - D[i_1][i_0] * D[i_1][i_0+1] + D[i_1][i_0] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M-1; i_3++) {
            B[i_3+i_2][i_2+1] = D[i_3+i_2+1][i_2-1] * B[i_3+i_2+1][i_2+2] * 2;
        }
    }
#pragma endscop
...
kernel_3216132123_06(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
