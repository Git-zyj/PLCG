...
void kernel_3234312315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_M+1, PB_P-1); i_3++) {
                B[i_1][i_2][i_2] = C[i_3+i_1-1][i_2+i_1][i_3] * D[i_2][i_3+1][i_1] + B[i_1+1][i_2][i_2] * B[i_1][i_2-1][i_2-1] + B[i_1-1][i_2+1][i_2+1] * B[i_1+1][i_2+1][i_2+1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3234312315_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
