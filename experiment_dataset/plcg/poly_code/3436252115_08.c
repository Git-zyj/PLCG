...
void kernel_3436252115_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + F[i_0+1][i_0+2] + E[i_0] * 2;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            D[i_2+1][i_1+1] = G[i_2][i_1] * E[i_1-1] + H[i_2] * D[i_1][i_2] * 6;
        }
        for (int i_3 = max(2, i_1); i_3 < min(-i_1+PB_L-2, PB_M-1); i_3++) {
            A[i_3-1] = I[i_1][i_3+1][i_3+i_1+2] + H[i_3+1] + F[i_1+2][0] * E[1] * G[i_1][i_3+1] + G[i_3-2][i_3+i_1] * G[i_3][i_1] + E[i_3] + 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = i_4; i_5 < PB_L; i_5++) {
            D[i_4][i_5] = G[i_4+2][i_5] - G[i_5][i_4] - F[i_4-1][i_4] + 5;
        }
    }
#pragma endscop
...
kernel_3436252115_08(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
