...
void kernel_4334332325_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_0+2] = D[i_1-1][i_0] * E[i_1] - F[i_1] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_N-1, -i_2+PB_P+2); i_3++) {
            B[i_3+1] = G[i_2][i_3+i_2-2] * B[i_3-1] * 2;
        }
    }
        ...
        for (int i_5 = i_4; i_5 < -i_4+PB_Q; i_5++) {
            for (int i_6 = 1; i_6 < PB_Q-3; i_6++) {
                for (int i_7 = 0; i_7 < min(PB_Q-2, -i_6+PB_P-2); i_7++) {
                    C[i_5][i_4][i_7+1][i_4-1] = H[i_5][i_7+1][i_7+i_6+2][i_4-1] + I[i_7][i_5][i_4][i_6] * H[i_5][i_6-1][i_7][i_5+i_4] - C[i_5-1][i_4][i_7+1][i_4-1] * C[i_5][i_7+1][i_6+3][i_7] + C[i_5-2][i_4-2][i_7+1][i_4-3] + C[i_5+2][i_4][i_7+2][i_4-1] * C[i_5][i_4][i_7+2][i_4-1] + 1;
                }
            }
        }
    }
...
init_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4334332325_01(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
