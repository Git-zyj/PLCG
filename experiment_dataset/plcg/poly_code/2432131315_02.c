...
void kernel_2432131315_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M+1; i_1++) {
            B[i_1-2] = E[i_1][i_0-1] + E[i_1][i_0-1] * E[i_1+i_0-1][i_1-1] * B[i_1-1] * 1;
        }
        B[i_0-1] = F[i_0-1] + F[i_0] * B[i_0-2] + B[i_0] * 3;
    }
    for (int i_2 = 2; i_2 < PB_N+1; i_2++) {
        B[i_2-1] = F[i_2] - F[i_2] * F[i_2] * D[i_2-2] + 5;
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        D[i_3-1] = F[i_3] + F[i_3] * E[i_3][i_3+1] - F[i_3] * D[i_3-2] + 5;
    }
#pragma endscop
...
kernel_2432131315_02(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
