...
void kernel_3324231215_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0] * E[i_0] - E[i_0] + E[i_0] - 6;
    }
    for (int i_1 = 1; i_1 < PB_N-2; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_N-2; i_2++) {
            B[i_2+i_1] = F[i_2][i_1+1] * G[i_1][i_2-1] - B[i_2+i_1+2] + B[i_2+i_1-1] * B[i_2+i_1+1] + B[i_2+i_1+1] + B[i_2+i_1+1] * B[i_2+i_1+2] - 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_Q); i_5++) {
                C[i_3+1][i_5][i_5+i_4] = H[i_5][i_3][i_4] + F[i_5][i_3] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3324231215_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
