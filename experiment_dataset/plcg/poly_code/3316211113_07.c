...
void kernel_3316211113_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P-1; i_1++) {
            A[i_0][i_1][i_1] = A[i_0][i_1+1][i_1+1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                C[i_4][i_3+1][i_4] = C[i_4][i_3][i_4] + 2;
            }
        }
        for (int i_5 = 0; i_5 < min(i_2, PB_M-1); i_5++) {
            for (int i_6 = 0; i_6 < PB_P; i_6++) {
                C[i_6][i_5+1][i_6] = D[i_5][i_6][i_2+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3316211113_07(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
