...
void kernel_3334352111_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = D[i_0] * E[i_0] + 5;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
            for (int i_3 = 2; i_3 < min(min(PB_Q, -i_1+PB_M), i_2); i_3++) {
                B[i_1][i_2][i_3] = F[i_2][i_1][i_3-2] * G[i_3+i_1][i_2-2][i_2+2] * H[i_3+i_1][i_2-2][0][i_2] + I[i_1][i_2][i_2-2] * B[i_1-1][i_2][i_3] - 6;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            C[i_4][i_5] = F[i_4][i_5][i_4] + J[i_5][i_5] - K[i_5][i_4] + L[i_5-1][i_4+1] * M[i_5] + J[i_5][1] * L[i_4][i_5] * C[i_4-1][i_5-1] + 3;
        }
    }
#pragma endscop
