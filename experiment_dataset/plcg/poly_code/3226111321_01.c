...
void kernel_3226111321_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = C[i_0] - A[i_0+1] - A[i_0] - A[i_0-2] - A[i_0-3] + 3;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_L; i_2++) {
            for (int i_3 = i_2; i_3 < min(-i_1+PB_M, -i_2+PB_L+1); i_3++) {
                B[i_1][i_3+i_2-1][i_2+i_1] = B[i_2-1][i_3+i_1][i_3+i_2-1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3226111321_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
