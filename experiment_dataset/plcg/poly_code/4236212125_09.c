...
void kernel_4236212125_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        ...
        for (int i_4 = 0; i_4 < min(PB_N-1, i_0); i_4++) {
            for (int i_5 = 1; i_5 < i_4; i_5++) {
                B[i_0][i_4][i_5-1] = C[i_4+1][i_5][i_0-1] - B[i_0-2][i_4+2][i_5-1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4236212125_09(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
