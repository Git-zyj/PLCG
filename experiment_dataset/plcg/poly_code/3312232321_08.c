...
void kernel_3312232321_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-4; i_0++) {
        A[i_0+2] = D[i_0] - E[i_0] + D[i_0-2] * A[i_0+4] + A[i_0+3] * 1;
        A[i_0] = D[i_0+1] + D[i_0] * D[i_0-1] - A[i_0+2] + 2;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 2; i_3 < -i_1+PB_M-1; i_3++) {
                C[i_3][i_3+i_1+1][i_2] = F[i_3+i_1][i_3][i_2] - G[i_1][i_2-1][i_3] * H[i_1][i_3] * C[i_3-2][i_3+i_1-1][i_2] + C[i_3][i_3+i_1-1][i_2-1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3312232321_08(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
