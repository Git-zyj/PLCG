...
void kernel_3224151121_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_L; i_1++) {
            A[i_1-1] = C[i_0][i_1][i_0] + D[i_1-1][0] - A[i_1+1] * A[i_1-3] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_P; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_N, PB_L); i_4++) {
                B[i_2][i_4][i_2] = C[i_3][i_4-1][i_2+1] * D[i_4][i_4] + C[i_4+i_3][i_2][i_2] - D[i_4][i_3+i_2] - D[i_4-1][i_3] + C[i_3][i_4][i_3] + C[i_3][i_4][i_2] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3224151121_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
