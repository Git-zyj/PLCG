...
void kernel_2336311111_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
            B[i_0+1][i_2+1] = D[i_0-1][i_2] - 3;
        }
    }
    for (int i_3 = PB_L-2 - 1; i_3 >= 0; i_3--) {
        for (int i_4 = 0; i_4 < -i_3+PB_L-2; i_4++) {
            C[i_4+i_3+1][i_4] = C[i_4+i_3][i_4] * C[i_4+i_3+2][i_4+1] - 3;
        }
    }
#pragma endscop
...
kernel_2336311111_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
