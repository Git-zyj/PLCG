...
void kernel_3214212221_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            A[i_1+1] = A[i_1+3] * B[i_1+3][i_0-1][2] - B[i_0+2][i_1][2] + 2;
            for (int i_2 = PB_M-4 - 1; i_2 >= 0; i_2--) {
                B[i_2+2][i_1][2] = C[i_2][i_0+1][0] * B[i_2+4][i_1][2] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3214212221_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
