...
void kernel_3412231113_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0-1] = E[i_0] - F[i_0+1] * A[i_0] + 3;
        for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
            B[i_0][i_1-1] = F[0] + G[i_1+1][i_0] + A[i_1-1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < min(min(i_2, PB_L-2), -i_2+PB_P); i_3++) {
            B[i_3][i_2-1] = H[i_3][i_2-1] * G[i_3][i_2] + H[i_3][i_2+1] + H[i_3+i_2-1][i_3] + 4;
            for (int i_4 = 1; i_4 < i_3; i_4++) {
                D[i_2+1][i_3][i_4] = G[i_4][i_3] * I[i_3][i_4][i_3+i_2] * D[i_3+2][i_2][i_4-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3412231113_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
