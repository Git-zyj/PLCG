...
void kernel_4434351323_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
        ...
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M+1, i_1); i_2++) {
                for (int i_3 = i_2; i_3 < PB_P; i_3++) {
                    A[i_2][i_1][i_3-1][0] = E[i_2][i_3][i_1-1][i_3] + F[i_2][i_2][i_3] - G[i_3][i_2][i_3][i_1][i_0] + E[i_3][i_2+i_1-1][i_0][i_2+1] + H[i_3][i_0][i_1-1][i_2] + A[i_3+1][i_1-1][i_2-1][0] + 3;
                }
            }
        }
        B[i_0] = I[i_0][1] + J[i_0][1] * K[i_0] * L[i_0] - J[i_0][i_0] + B[i_0-2] + C[i_0-1][i_0-1] + 1;
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_M-1; i_5++) {
            C[i_5][i_5] = I[i_5+1][i_4] + L[i_4] + J[i_5][i_4-1] * C[i_5-2][i_5-2] * C[i_5-1][i_5-1] * C[i_5+1][i_5+1] * 2;
        }
    }
    for (int i_6 = PB_M-2 - 1; i_6 >= 1; i_6--) {
        C[i_6][i_6] = K[i_6+1] * J[i_6-1][i_6] + L[i_6] + K[i_6] * J[i_6-1][i_6+1] + B[i_6+2] - C[i_6][i_6] * 3;
    }
#pragma endscop
