...
void kernel_3412311311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[i_0-1][i_1] = E[i_1] - A[i_0-2][i_1] - D[i_1+2][i_0-1] * 5;
        }
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            B[i_2][i_2] = B[i_2-1][i_2-1] + 3;
            C[i_0][i_2] = B[i_0][i_0] - B[i_0+1][i_0+1] + B[i_2][i_2] * 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                D[i_5+1][i_3] = C[i_4+1][i_5] + D[i_4+1][i_5-1] * D[i_5][i_4] + D[i_5][i_3-1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3412311311_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
