...
void kernel_4212312311_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_3 = 3; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 3; i_4 < min(PB_P-2, -i_3+PB_M+2); i_4++) {
            B[i_3-2][i_4+2] = C[i_4+i_3-2][0] + B[i_4-3][i_3+2] * B[i_3-2][i_4+1] - B[i_3-3][i_4+2] + 5;
        }
    }
#pragma endscop
...
kernel_4212312311_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
