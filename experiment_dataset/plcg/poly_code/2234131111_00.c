...
void kernel_2234131111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            A[i_1][i_1+i_0] = C[i_1][i_0] - C[i_0+1][i_1] - C[i_0+1][i_1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < min(-i_2+PB_L, PB_M); i_3++) {
            B[i_3-1] = C[i_3+i_2][0] * C[i_3+1][i_2] - B[i_3] + 5;
        }
    }
#pragma endscop
...
kernel_2234131111_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
