...
void kernel_4216112325_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_P; i_2++) {
                A[i_1][i_2-1][i_2+i_0] = A[i_1-1][i_0-1][i_2+i_0-1] - A[i_1-2][i_0-1][i_2+i_0] - 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-4; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_M-4, -i_3+PB_M-2); i_4++) {
            B[i_3+2][i_4] = C[i_4+i_3+2] * B[i_3+3][i_4] + B[i_3+2][i_4+1] * B[i_3+4][i_4+2] + B[i_4+4][i_3] + 6;
        }
    }
#pragma endscop
...
kernel_4216112325_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
