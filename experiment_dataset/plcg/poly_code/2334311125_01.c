...
void kernel_2334311125_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N-1; i_3++) {
            C[i_3+i_2-1][i_2] = C[i_3+i_2+1][i_2] * 4;
        }
    }
#pragma endscop
...
kernel_2334311125_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
