...
void kernel_2334251213_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0-1] + E[i_0] - D[1] + E[i_0] * E[i_0] - 1;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < min(-i_1+PB_M, PB_L-1); i_2++) {
            B[i_1][i_2-1] = F[i_2+1][i_1-1] * D[i_2] * D[i_2] + G[i_2][i_1][i_2+1] - H[i_1][i_2] + G[i_2][i_1+1][i_2] + I[i_2+i_1][i_2][i_1] - H[i_2+i_1][i_1+1] + B[i_1-1][i_2-2] * B[i_1][i_2-2] * 4;
        }
        for (int i_3 = 0; i_3 < i_1; i_3++) {
            A[i_3] = H[i_3+1][i_1+1] * B[i_3][i_1] + 1;
        }
    }
#pragma endscop
...
kernel_2334251213_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
