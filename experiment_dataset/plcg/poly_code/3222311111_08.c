...
void kernel_3222311111_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = i_3; i_4 < PB_N; i_4++) {
            for (int i_5 = 1; i_5 < min(PB_P, -i_4+PB_L); i_5++) {
                B[i_4][i_5] = C[i_4][i_5+i_4][1] * B[i_4][i_5+1] * B[i_5-1][i_3] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3222311111_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
