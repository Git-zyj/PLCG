...
void kernel_4426232213_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(min(PB_N, -i_0+PB_M-1), i_0); i_1++) {
            A[i_1+i_0][i_1] = E[i_1] - F[i_1+1][i_0] * A[i_1+i_0-1][i_1-1] * A[i_1+i_0+1][i_0+1] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
            for (int i_4 = i_3; i_4 < PB_Q-2; i_4++) {
                B[i_4][i_2][i_3][i_2-2] = G[i_4][i_3] + H[i_2][i_2][i_3] * H[i_4][i_2][i_4+2] - B[i_4-1][i_2][i_3+1][i_2-2] - B[i_4-1][i_2+1][i_3][i_2-1] - B[i_4][i_2][i_3-1][i_2-2] + 3;
            }
        }
        for (int i_5 = i_2; i_5 < -i_2+PB_P; i_5++) {
            for (int i_6 = 0; i_6 < -i_5+PB_P-1; i_6++) {
                D[i_5+3][i_6+i_5+1] = I[i_5][i_2][i_6+i_5+1] * 4;
            }
            D[i_2+2][i_5+i_2] = F[i_2-1][i_5] + F[i_2][i_5-2] * G[i_5][i_2] + 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4426232213_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
