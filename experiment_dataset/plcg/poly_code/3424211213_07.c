...
void kernel_3424211213_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = i_0; i_2 < PB_M; i_2++) {
                A[i_0][i_1+1][i_2] = E[i_1][i_0-1][i_0] * E[i_2][i_1+1][i_0-1] + A[i_0-1][i_1+2][i_2] + A[i_0][i_1+2][i_2] - A[i_0-1][i_1+1][i_2-1] * A[i_0][i_1][i_2+1] - 6;
            }
        }
    }
    ...
    for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
        for (int i_7 = i_6; i_7 < PB_L; i_7++) {
            C[i_6-1][i_7-1] = C[i_7][i_6-1] - C[i_6][i_7] * 2;
        }
        D[i_6+1] = 3;
    }
#pragma endscop
...
kernel_3424211213_07(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
