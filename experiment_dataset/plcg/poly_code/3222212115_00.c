...
void kernel_3222212115_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_Q, -i_0+PB_M-1); i_1++) {
            for (int i_2 = 3; i_2 < PB_L-2; i_2++) {
                A[i_1+i_0][i_2-2][i_0] = C[i_1][i_0][i_2+2] - A[i_1+i_0+1][i_2-3][i_1] - 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M+1, PB_L); i_4++) {
            for (int i_5 = 2; i_5 < PB_N-2; i_5++) {
                A[i_4+i_3-1][i_5-2][i_3] = D[i_5+2][i_4] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3222212115_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
