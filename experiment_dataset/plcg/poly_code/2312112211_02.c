...
void kernel_2312112211_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        ...
        for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
            B[i_2][i_0+2] = D[i_0][i_2] * E[i_2+i_0] * B[i_2][i_0+3] - 1;
        }
    }
...
kernel_2312112211_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
