...
void kernel_2336131115_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = D[i_0][i_0] + D[i_0-1][1] - 4;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            C[i_1+1][i_0-1] = D[i_1][i_0] * D[i_1][i_0] + D[i_1+i_0][i_1-1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_M-1, -i_2+PB_M); i_3++) {
            C[i_2+1][i_3] = E[i_2] + D[i_2][i_2] - D[i_3][i_2] - E[i_3+i_2] - A[i_3+1] * C[i_2+1][i_3+1] * 5;
        }
    }
#pragma endscop
...
kernel_2336131115_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
