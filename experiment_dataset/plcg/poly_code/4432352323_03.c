...
void kernel_4432352323_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 5; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 3; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_N+2, -i_1+PB_Q); i_2++) {
                for (int i_3 = 2; i_3 < -i_1+PB_N+1; i_3++) {
                    A[i_3+i_2-1][i_1][2][i_0-1] = E[i_3+1][i_2][i_1] * F[i_1][i_3][i_1-1][i_2] + G[i_3][i_2-2][i_0+1][i_2-2] + F[i_1-1][i_2][i_3][i_3] - G[i_3][i_0+2][i_0+2][0] * G[i_0][i_2][i_3-1][i_2+i_1] + H[i_2+i_1-2][i_3][i_0] + A[i_2+i_0-5][i_3-1][2][i_1-3] - A[i_3+i_1-1][i_2+1][2][i_0-1] * A[i_2+i_0-2][i_1-1][2][i_3-2] + A[i_3+i_2][i_1][2][i_0-1] + 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 2; i_5 < PB_M; i_5++) {
            for (int i_6 = 2; i_6 < min(PB_Q-3, -i_4+PB_M); i_6++) {
                B[i_4][i_4-1][i_6+1] = F[i_5][i_4][i_6][i_6-2] + H[i_6+i_4][i_5][i_5] * B[i_4+2][i_4+1][i_6+1] + B[i_4][i_4-1][i_6] - B[i_4][i_4-1][i_6+3] + B[i_5-1][i_5-2][i_6+1] + 2;
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_M-2; i_7++) {
        for (int i_8 = 2; i_8 < PB_M-1; i_8++) {
            B[i_7][i_7-1][i_8] = E[i_8][i_7+1][1] - I[i_8-2][0] - I[i_8+1][i_7] - J[i_8][i_7] * I[i_8][i_7] - K[i_8][i_7-2] + 1;
        }
        for (int i_9 = 2; i_9 < PB_L; i_9++) {
            D[i_7][i_9] = J[i_9][i_7] * J[i_9-1][i_7-1] + I[i_9-1][i_7+1] * L[i_9] - I[i_9-2][i_7+2] + D[i_7+1][i_9] - 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4432352323_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L)));
