...
void kernel_3326131115_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        A[i_0+1] = A[i_0+2] + A[i_0] * 1;
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            B[i_1+i_0] = D[i_0] + D[i_1+i_0] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        A[i_2+1] = D[i_2-1] + D[i_2-1] + D[i_2] * D[i_2] + D[i_2] + 3;
    }
#pragma endscop
...
kernel_3326131115_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
