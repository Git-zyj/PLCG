...
void kernel_3424112213_04(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        C[i_0+2] = E[i_0] + 5;
        for (int i_1 = i_0; i_1 < -i_0+PB_M-2; i_1++) {
            B[i_1+i_0][i_0] = B[i_1+i_0-1][i_0-1] + B[i_1+i_0+2][i_0+1] + 4;
        }
    }
    ...
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_M); i_5++) {
                B[i_5+i_4-1][i_5] = B[i_5+i_4][i_4-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3424112213_04(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E));
