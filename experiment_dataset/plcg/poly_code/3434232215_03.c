...
void kernel_3434232215_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        A[i_0-1] = E[i_0] - E[i_0] + E[i_0] - F[i_0] * B[i_0] - 4;
        B[i_0] = F[i_0] - E[i_0] + A[i_0-1] - 3;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1-1] = G[i_1][i_0] * A[i_1-1] - A[i_1] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L; i_3++) {
            for (int i_4 = 2; i_4 < i_2; i_4++) {
                A[i_4-2] = H[i_4-2][0] + I[i_4][i_3+i_2][i_2+1] - G[i_3][i_4] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3434232215_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
