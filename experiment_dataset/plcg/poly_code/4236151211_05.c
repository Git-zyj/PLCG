...
void kernel_4236151211_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
        ...
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_0+PB_M, i_1), PB_M-1); i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_M; i_3++) {
                    A[i_3][i_2+i_0][i_1] = C[i_1][i_2-1][i_3-1][i_3] - C[i_2][i_1][i_3][0] - C[i_3][i_2+i_0][i_1+1][0] * C[i_2][i_0][i_3][i_0] + D[i_0+1][i_3+1][i_2] * C[i_3][i_2][i_0+1][1] - D[i_3+i_1-1][i_2+1][i_1+1] * A[i_0+1][i_3+i_1][i_2-1] - 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
            for (int i_6 = max(1, i_5); i_6 < PB_M; i_6++) {
                B[i_4][i_5][i_4] = D[i_4][i_5][i_6-1] + D[i_6][i_4][i_5+1] * B[i_4-1][i_5][i_4-1] + B[i_4+1][i_5][i_4+1] - B[i_4][i_6-1][i_4] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4236151211_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
