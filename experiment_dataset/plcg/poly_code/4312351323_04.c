...
void kernel_4312351323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        A[i_0+1] = D[i_0] * A[i_0+3] * A[i_0+2] - A[i_0] * A[i_0-1] - 5;
        for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_N; i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_N; i_3++) {
                    B[i_1+1][i_0][i_2][i_3] = E[i_2][i_3-1][i_1+1] - F[i_2][i_0][i_3][i_1] * G[i_3-1][i_0+1][i_2][i_1][0] - H[i_1-1][i_0][i_3][i_0] - I[i_3+i_2][i_0][i_1][i_1] + B[i_1][i_0][i_2-1][i_3] + B[i_1+2][i_3-1][i_0-1][i_2] * 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            C[i_4][0] = J[i_5+1][i_4] + K[i_4+1][i_5] * J[i_5+1][i_4-1] * L[i_5-1] - M[i_5-1][i_4] * M[i_4][i_5] - J[i_5][i_4] - C[i_4-1][0] + C[i_4+1][0] - 3;
        }
    }
#pragma endscop
