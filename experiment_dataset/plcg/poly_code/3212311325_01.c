...
void kernel_3212311325_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_Q, -i_0+PB_P-2); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P; i_2++) {
                B[i_1+i_0][i_2][i_2+i_1] = C[i_0][i_0][i_1] * 3;
            }
            for (int i_3 = 2; i_3 < min(-i_1+PB_L-3, -i_0+PB_P-1); i_3++) {
                B[i_1+i_0][i_3][i_3+i_0] = B[i_3+i_1][i_0+2][i_3+i_0+1] + B[i_3+i_1+3][i_0+1][i_1+i_0+2] * B[i_1+i_0][i_3-2][i_3+i_0-2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3212311325_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
