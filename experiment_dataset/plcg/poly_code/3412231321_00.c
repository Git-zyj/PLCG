...
void kernel_3412231321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0] - E[i_0] * A[i_0+1] + A[i_0+2] - B[i_0] - 4;
        B[i_0] = E[i_0] + F[i_0+1] * F[i_0] - B[i_0+1] * 1;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
            C[i_1+1][i_2] = F[i_1] * G[i_2][i_1] * G[i_2][i_1] + C[i_1+1][i_2+1] - C[i_1+2][i_2-2] * C[i_2+1][i_1-1] + 3;
        }
        for (int i_3 = 0; i_3 < -i_1+PB_L-1; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                D[i_3][i_4] = H[i_4][i_3+i_1+1][i_1+1] * I[i_4][i_3+1][i_1-1] * D[i_4-1][i_3] - D[i_3][i_1-2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3412231321_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
