...
void kernel_2224131213_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        A[i_0+1] = C[i_0] * C[1] - C[i_0+1] * A[i_0+2] * A[i_0] - 1;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_P, PB_M-1); i_2++) {
            B[i_1+1][i_2-1] = D[i_2+1][i_1] - E[i_1][i_2+i_1][i_2] - B[i_1+2][i_2-1] + 1;
        }
    }
#pragma endscop
...
kernel_2224131213_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
