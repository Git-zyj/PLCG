...
void kernel_2334132321_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = D[i_0] - D[i_0-1] * D[i_0+2] + A[i_0+4] - 2;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        B[i_1] = D[i_1] * B[i_1+2] - B[i_1-1] * B[i_1+1] + B[i_1-2] * 5;
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        B[i_2+2] = D[i_2+1] - D[i_2+2] * D[i_2-2] - D[0] + D[i_2] + 4;
    }
#pragma endscop
...
kernel_2334132321_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
