...
void kernel_3336311115_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        B[i_0][i_0] = D[i_0] * D[0] * 5;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        B[i_1][i_1] = B[i_1-1][i_1-1] * 4;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = i_3; i_4 < PB_L; i_4++) {
                C[i_4][i_3] = C[i_3-1][i_4] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3336311115_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
