...
void kernel_3336112311_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        B[i_1-1] = D[0] - A[i_1] * B[i_1-2] + B[i_1] * 4;
        for (int i_2 = 0; i_2 < min(PB_P, i_1); i_2++) {
            for (int i_3 = i_1; i_3 < min(min(PB_P+1, -i_1+PB_M+1), -i_2+PB_M); i_3++) {
                C[i_3+i_2][i_2] = C[i_3+i_1-2][i_3-1] + C[i_3+i_1-1][i_3-1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3336112311_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
