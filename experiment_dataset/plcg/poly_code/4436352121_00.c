...
void kernel_4436352121_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_0+PB_L, -i_1+PB_P), PB_L-2); i_3++) {
                    A[i_0][i_1][i_2][i_3+i_0] = E[i_1][i_3+2][i_2][i_3+i_0-1] - E[i_2][i_0-1][i_3+i_1][i_1] + F[i_2+2][i_0][i_1] - G[i_2-1][i_3][i_0][i_1] * H[i_1][i_2-2][i_3][i_2] + A[i_0][i_1][i_2+1][i_3+i_0] + 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
            B[i_4][i_5][i_5] = I[i_4][i_4] - J[i_5-2][i_4-2][i_4] - K[1] - L[i_4] * M[i_5+1][i_4] * I[i_5][i_4] + B[i_5-1][i_4-2][i_4-2] - 5;
        }
    }
    for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
        for (int i_7 = 1; i_7 < PB_M; i_7++) {
            C[i_7][i_6] = M[i_7][i_6-2] * I[i_6-1][i_7-1] * N[i_7][i_6] * K[i_7] - N[i_7][i_6-1] - N[i_7][i_6-1] + 3;
        }
        for (int i_8 = 0; i_8 < PB_P-2; i_8++) {
            for (int i_9 = 1; i_9 < i_8; i_9++) {
                D[i_6+2][i_8][i_6] = J[i_8][i_8][i_6+1] * F[i_9][i_6][0] * J[i_8][i_6][i_9-1] - D[i_6+1][i_8+2][i_6-1] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,yn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4436352121_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,yn,POLYBENCH_ARRAY(N));
