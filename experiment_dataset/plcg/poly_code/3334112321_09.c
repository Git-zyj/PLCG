...
void kernel_3334112321_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            A[i_1][i_0-2] = A[i_1][i_0-4] - A[i_1-2][i_0-2] + A[i_1+2][i_0-2] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
            B[i_2][i_3] = B[i_2][i_3-2] - B[i_2][i_3+1] - B[i_2+2][i_3] - B[i_2-1][i_3+2] - B[i_2][i_3+2] + B[i_2+1][i_3] - 6;
        }
        for (int i_4 = 0; i_4 < -i_2+PB_M; i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_M-1; i_5++) {
                C[i_4][i_4+i_2] = D[i_5+i_4+1][i_2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3334112321_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
