...
void kernel_2422251325_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        A[i_0-1] = E[i_0] - E[i_0] - F[i_0] * F[i_0-1] * A[i_0-3] - A[i_0] + 5;
    }
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 3; i_2 < PB_P; i_2++) {
            A[i_2-3] = G[i_1+1][i_2] + G[i_1][i_2] * G[i_2-1][i_1] - G[i_1-1][i_2] * G[i_2][i_1] - G[i_2][i_1] + D[i_2-1] - 5;
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            A[i_3+1] = G[i_3+1][i_1+1] - H[i_3][i_1] * E[i_1] * D[i_3-1] * 6;
        }
    }
    for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            D[i_5-1] = E[i_5+1] - I[i_4][i_5][i_5] + G[i_4][i_5+1] - H[i_5][i_4-1] * G[i_5-1][i_4+1] - D[i_5] + 5;
        }
    }
#pragma endscop
...
kernel_2422251325_09(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
