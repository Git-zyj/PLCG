...
void kernel_2226111325_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        A[i_0][i_0] = A[i_0+2][i_0+2] * A[i_0+1][i_0+1] - 6;
    }
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = PB_L-1 - 1; i_2 >= max(1, i_1); i_2--) {
            B[i_1+1][i_2-1] = C[i_2][i_1+1] * A[i_1][i_1] - B[i_1+3][i_2-1] * B[i_1+2][i_2+1] * B[i_1][i_2-1] - 6;
        }
    }
#pragma endscop
...
kernel_2226111325_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
