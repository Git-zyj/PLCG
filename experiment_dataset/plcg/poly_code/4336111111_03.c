...
void kernel_4336111111_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < PB_N; i_4++) {
            for (int i_5 = 1; i_5 < min(PB_Q, i_3); i_5++) {
                B[i_5][i_3][i_5] = D[i_4][i_3][i_5-1] * 3;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 0; i_7 < PB_L; i_7++) {
            C[i_6+1][i_7] = C[i_6][i_7] * 6;
        }
    }
#pragma endscop
...
kernel_4336111111_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
