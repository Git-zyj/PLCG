...
void kernel_4434231225_01(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                D[i_2+2][i_1+i_0][i_1] = 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        B[i_3-1] = E[i_3] + F[i_3+1][i_3] - 4;
        B[i_3-1] = E[i_3] + B[i_3-1] * B[i_3-2] + 3;
        for (int i_4 = 1; i_4 < min(-i_3+PB_P, PB_L-1); i_4++) {
            for (int i_5 = 1; i_5 < min(PB_L, -i_3+PB_P-1); i_5++) {
                D[i_4][i_5+i_3][i_5-1] = G[i_5][i_3][i_4+i_3-1] * F[i_4][i_3] * G[i_4][i_5-1][i_3] + H[i_5][i_4+i_3][0] + G[i_5][i_3][i_4-1] - G[i_3-1][i_5][i_4-1] - D[i_4][i_5+i_3+1][i_5-1] - D[i_4+1][i_5+i_3+1][i_5] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434231225_01(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
