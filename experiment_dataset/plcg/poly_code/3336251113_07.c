...
void kernel_3336251113_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = D[i_0+1] * D[i_0] + E[i_0-1] + D[i_0] - A[i_0] - 3;
    }
    for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            for (int i_3 = 0; i_3 < min(PB_N, -i_1+PB_L-1); i_3++) {
                B[i_3][i_2-1] = F[i_3][i_2-1][i_1+1] * F[i_2][i_3+i_1+1][0] * F[i_2][i_2][i_1] * F[i_3][i_2+1][i_1+1] + 3;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        for (int i_5 = i_4; i_5 < min(PB_Q-1, -i_4+PB_L); i_5++) {
            for (int i_6 = 1; i_6 < min(-i_4+PB_N, -i_5+PB_M+1); i_6++) {
                B[i_6][i_5] = G[i_5+1][i_6-1][i_5+1][i_4] + F[i_6+i_5-1][i_6][i_4] - H[i_6-1][i_6-1][i_4] + F[i_6][i_5+i_4][i_6] - H[i_6+i_4][i_5-1][i_4] + H[i_6][i_4-1][i_4] * F[i_4][i_5+1][i_6+1] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3336251113_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
