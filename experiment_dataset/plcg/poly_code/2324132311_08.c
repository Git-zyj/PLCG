...
void kernel_2324132311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        A[i_0] = D[i_0-1][i_0+1] + E[i_0+2] * E[i_0] * A[i_0-1] + C[i_0+3][2] + C[i_0+2][2] + 6;
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_0] = D[i_0][i_1+2] + D[i_1-2][0] - A[i_1] - A[i_0] * 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < PB_P; i_3++) {
            C[i_3+2][2] = D[i_3][i_2] - D[i_3][i_2] + D[i_3][i_2] * C[i_3+3][2] - 6;
        }
    }
#pragma endscop
...
kernel_2324132311_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
