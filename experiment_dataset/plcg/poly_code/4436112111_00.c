...
void kernel_4436112111_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
                A[i_1][i_0-2][i_2-1] = A[i_1][i_0-2][i_2-2] + B[i_2+1][i_1][i_1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < min(min(-i_3+PB_P-2, PB_L-2), i_3); i_5++) {
                B[i_5+2][i_4][i_4] = E[i_4][i_5+i_3+2][i_3] - E[i_4][i_3][i_4] + 3;
            }
        }
        for (int i_6 = 1; i_6 < min(-i_3+PB_P-2, PB_L); i_6++) {
            for (int i_7 = i_6; i_7 < min(-i_3+PB_L-1, -i_6+PB_L); i_7++) {
                C[i_3][i_7+i_6] = E[i_7][i_6+i_3+2][i_6+1] - E[i_6][i_7+i_3][i_3] + C[i_6][i_7+i_3+1] * D[i_7-1][i_6-1][2] - 3;
            }
        }
    }
...
kernel_4436112111_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
