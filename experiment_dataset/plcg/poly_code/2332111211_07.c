...
void kernel_2332111211_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M-2, PB_L-1); i_2++) {
            B[i_2+i_1][i_2][i_2] = B[i_2+i_1+2][i_2+1][i_2+1] + B[i_2+i_1-1][i_2-1][i_2-1] + 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_Q; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            C[i_4][i_3][i_4] = D[i_4][i_3] * C[i_4+1][i_3][i_4+1] - 1;
        }
    }
#pragma endscop
...
kernel_2332111211_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
