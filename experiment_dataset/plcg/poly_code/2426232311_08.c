...
void kernel_2426232311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0+1] * F[i_0] - F[i_0] + E[i_0] * E[i_0] + F[i_0+1] * A[i_0-1] * A[i_0+1] * 2;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_N; i_2++) {
            C[i_2+1] = G[i_2][i_1] * A[i_2+1] * C[i_2] * 3;
        }
        C[i_1] = C[i_1+1] - C[i_1-1] - 4;
    }
    for (int i_3 = 1; i_3 < PB_N-1; i_3++) {
        D[i_3] = E[i_3] - E[i_3] - F[i_3+1] + H[i_3][i_3-1] - F[i_3] * D[i_3-1] + 1;
    }
#pragma endscop
...
kernel_2426232311_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
