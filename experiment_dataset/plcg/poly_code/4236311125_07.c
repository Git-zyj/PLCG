...
void kernel_4236311125_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_N-1; i_2++) {
                A[i_0+1][i_1][i_2-1] = C[i_0+1][i_2+i_0+1][i_1-1] - A[i_2+1][i_0][i_1-2] * 3;
            }
        }
        for (int i_3 = 0; i_3 < PB_P; i_3++) {
            for (int i_4 = max(3, i_3); i_4 < PB_Q+3; i_4++) {
                A[i_0+1][i_3][i_4-3] = 6;
            }
        }
    }
...
kernel_4236311125_07(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
