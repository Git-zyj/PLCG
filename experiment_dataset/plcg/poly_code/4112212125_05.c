...
void kernel_4112212125_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-5; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L-5; i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_M-3; i_3++) {
                    A[i_3+i_0][i_2+i_1+1][i_0-2][i_1-2] = B[i_1][i_3+2][0] + A[i_3+i_0+3][i_2+i_1+5][i_0-1][i_1] + 6;
                }
            }
        }
    }
...
kernel_4112212125_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
