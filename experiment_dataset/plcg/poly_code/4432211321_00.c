...
void kernel_4432211321_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < min(PB_P-2, i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
                    A[i_0+1][i_1][i_3][i_2] = E[i_3][i_2-1][i_0+1][i_1] - A[i_3+3][i_1][i_0][i_2] - A[i_0+1][i_3][i_1+2][i_2] + 4;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-3; i_5++) {
            B[i_5+1][i_4-1] = F[i_5+1][i_4+1] * F[i_5][i_4-1] + B[i_5+3][i_4-3] - B[i_5+1][i_4] + B[i_5+3][i_4-1] * 6;
        }
    }
    for (int i_6 = 1; i_6 < PB_M-2; i_6++) {
        for (int i_7 = 2; i_7 < PB_P-2; i_7++) {
            C[i_6][i_7] = C[i_6-1][i_7+2] - C[i_6+2][i_7-2] + C[i_6][i_7-2] + 4;
        }
    }
    for (int i_8 = 2; i_8 < PB_M-1; i_8++) {
        for (int i_9 = 0; i_9 < PB_L-2; i_9++) {
            for (int j_0 = 0; j_0 < PB_N-1; j_0++) {
                D[j_0][i_8-1][i_9] = D[j_0+1][i_8+1][i_9] + D[j_0][i_8-2][i_9+2] - D[j_0][i_8-2][i_9] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4432211321_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
