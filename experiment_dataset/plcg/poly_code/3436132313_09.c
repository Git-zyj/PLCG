...
void kernel_3436132313_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        A[i_0] = A[i_0+1] - B[i_0+3] * 5;
        B[i_0+2] = E[i_0+1] * E[i_0] + E[i_0] + A[i_0] * 3;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_P-2, i_1); i_2++) {
            for (int i_3 = 0; i_3 < -i_2+PB_P-2; i_3++) {
                C[i_1][i_3+i_2][i_3] = F[i_3][i_1][i_2] - F[i_1][i_3+i_2][i_1+2] * F[i_2][i_3][i_1] - C[i_1+1][i_3+i_2+2][i_3+1] * C[i_1-1][i_3+i_2][i_2-1] - C[i_1][i_3+i_2+1][i_3] + C[i_1+1][i_3+i_2][i_3] + 3;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        D[i_4] = E[i_4+2] * E[i_4] * E[i_4-2] - E[i_4+1] + D[i_4-1] * D[i_4+1] * 6;
    }
#pragma endscop
...
kernel_3436132313_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
