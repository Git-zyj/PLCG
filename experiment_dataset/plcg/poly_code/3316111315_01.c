...
void kernel_3316111315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = A[i_0+1] * B[i_0-2] * B[i_0-1] - B[i_0] - 1;
        B[i_0-1] = D[i_0][i_0] + A[i_0-1] - 4;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_L-1; i_2++) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                C[i_2][1] = C[i_2-1][1] - C[i_2+1][1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3316111315_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
