...
void kernel_4324151315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            A[i_1] = D[i_1+1][i_0+1] - D[i_1][i_0] + D[i_1+1][i_1+i_0+1] * D[i_1][i_0] * A[i_1+1] * A[i_1-1] + 4;
        }
        for (int i_2 = 1; i_2 < min(PB_N, -i_0+PB_L-1); i_2++) {
            A[i_2] = D[i_0][i_2-1] * D[i_2-1][i_0] + C[i_0][i_2+i_0+1] + 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_N+1, i_3); i_4++) {
            C[i_3-1][i_4+i_3] = D[i_4][i_4] * E[i_3] * D[i_3][i_4] - D[i_4+i_3-1][i_4] * D[i_3-1][i_4] - E[i_4+1] + D[i_4][i_4] * A[i_4-1] - C[i_3-2][i_4+i_3-1] - C[i_3][i_4+i_3+1] + 3;
        }
    }
#pragma endscop
...
kernel_4324151315_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
