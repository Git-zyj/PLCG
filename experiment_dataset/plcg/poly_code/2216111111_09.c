...
void kernel_2216111111_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = PB_M - 1; i_0 >= 1; i_0--) {
        ...
        for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
            B[i_2+i_0][i_2+i_0] = D[i_2] * B[i_2+i_0-1][i_2+i_0-1] + 1;
        }
    }
#pragma endscop
...
kernel_2216111111_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
