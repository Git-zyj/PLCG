...
void kernel_3232132311_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-3; i_1++) {
            A[i_0][i_1+2] = C[i_0-1][i_1-1] + C[i_0][0] - C[i_1][i_0+2] + C[i_1][i_0] + A[i_0][i_1+3] - A[i_0+1][i_1+2] - 1;
        }
        for (int i_2 = 0; i_2 < min(i_0, -i_0+PB_L-3); i_2++) {
            for (int i_3 = 0; i_3 < -i_0+PB_L-3; i_3++) {
                B[i_2+i_0+2][i_2+2][i_3+2] = D[i_2][i_3][i_0] + C[i_0+2][i_2] * B[i_3+i_0+3][i_0+3][i_2+3] + B[i_2+i_0+3][i_2+2][i_3+2] - B[i_2+i_0+1][i_2+2][i_3+2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3232132311_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
