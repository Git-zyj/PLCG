...
void kernel_2434251215_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M+1, PB_L-1); i_1++) {
            B[i_0+1][i_0+1] = E[i_0] + F[i_0][i_1+1] * G[i_1+i_0-1][0] - F[i_0+1][i_1] * F[i_0][i_1+1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_N, i_2); i_3++) {
            B[i_3][i_3] = F[i_3][i_2] + F[i_3+1][i_2] * G[i_3][i_3] - H[i_3+i_2] + F[i_2][0] * B[i_3-1][i_3-1] + B[i_3+1][i_3+1] + 1;
        }
        B[i_2][i_2] = E[i_2] * H[i_2] + H[i_2] * 2;
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-1; i_5++) {
            B[i_4-1][i_4-1] = G[i_5+1][i_4-1] + F[i_5+1][i_4-1] + F[i_5][i_4] * I[i_4+1][i_5+1][1] * G[i_5][i_4] + F[i_5][i_4-1] + F[0][i_5] - 6;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2434251215_05(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
