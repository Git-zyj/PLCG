...
void kernel_2314331113_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        A[i_0] = D[i_0+1] + E[i_0] - A[i_0+1] - 5;
        for (int i_1 = 1; i_1 < min(PB_Q, -i_0+PB_L); i_1++) {
            B[i_1-1][0] = F[i_1+i_0][i_1-1] + G[i_1-1][i_0] + H[i_1+i_0][i_0][0] * 5;
            A[i_1] = D[i_1-1] - G[i_0][i_1] + E[i_0-1] - 2;
        }
    }
#pragma endscop
...
kernel_2314331113_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
