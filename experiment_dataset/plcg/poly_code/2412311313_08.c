...
void kernel_2412311313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M+1, PB_M-1); i_1++) {
            A[i_1] = 2;
            A[i_1] = E[i_1+i_0] * A[i_0] - 1;
            A[i_1-1] = F[i_1][i_0] * A[i_1] + A[i_1-1] - 4;
            A[i_1+1] = G[i_1+i_0-1][i_1-1] * 1;
        }
    }
#pragma endscop
...
kernel_2412311313_08(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
