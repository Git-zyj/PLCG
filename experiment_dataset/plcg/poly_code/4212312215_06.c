...
void kernel_4212312215_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                A[i_1][i_2][0] = C[i_2][i_1-1][i_0+2] * A[i_1][i_2-1][0] * A[i_1][i_2+1][0] - A[i_1-1][i_2][0] * 4;
            }
        }
    }
...
kernel_4212312215_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
