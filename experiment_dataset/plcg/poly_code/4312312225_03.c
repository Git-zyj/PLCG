...
void kernel_4312312225_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_Q-2; i_2++) {
            for (int i_3 = 0; i_3 < min(min(min(PB_L, -i_1+PB_M-2), i_1), -i_2+PB_Q-2); i_3++) {
                B[i_1][i_3][i_3+i_2] = D[i_2-2][i_3+i_1+2][1] - D[i_3][i_2-1][i_1+2] + B[i_1][i_3][i_3+i_2-2] - B[i_1][i_3][i_3+i_2+2] - B[i_1][i_3][i_3+i_2+1] - 4;
            }
...
kernel_4312312225_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
