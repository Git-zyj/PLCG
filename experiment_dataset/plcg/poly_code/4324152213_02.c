...
void kernel_4324152213_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = i_0; i_1 < min(PB_L-1, -i_0+PB_M+2); i_1++) {
            for (int i_2 = 2; i_2 < PB_Q-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                    A[i_3+1][i_0][i_2][i_1] = D[i_3][i_2][i_1+i_0-2][i_3+1] - D[i_1-1][i_0][i_2][i_3] + D[i_3][i_2][i_3][i_0] * D[i_2-2][i_0][i_3][i_1] - A[i_0+1][i_1+1][i_3][i_2+1] + 4;
                }
            }
        }
        for (int i_4 = 2; i_4 < min(i_0, -i_0+PB_L+1); i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_L; i_5++) {
                C[i_4][i_0-2][i_5] = E[i_0][i_5+i_4] - F[i_0-1][0][i_4-2] + F[i_5+i_4][i_0][i_4+i_0-1] + F[i_5][i_0][i_4] + D[i_4+i_0-1][i_5-1][i_5][i_0+2] * 4;
            }
        }
    }
    for (int i_6 = 3; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 0; i_7 < -i_6+PB_L+2; i_7++) {
            for (int i_8 = 3; i_8 < -i_6+PB_M+1; i_8++) {
                C[i_8][i_6-2][i_7] = F[i_8][i_8][i_7+i_6-2] * F[i_8+i_6-1][i_7][i_7] * F[i_6+1][i_6][i_7] * E[i_8][i_7] - E[i_7][0] - C[i_8-1][i_6-2][i_7] - C[i_7+1][i_6-3][i_8] - C[i_6-1][i_8-3][i_7] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4324152213_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
