...
void kernel_4336252113_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
        ...
        for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 2; i_2 < min(i_1, -i_1+PB_Q); i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_P-2; i_3++) {
                    A[i_3-1][i_2-2][i_0][i_3-1] = D[i_3][i_2][i_0+2][i_1] - D[i_2-2][i_0][i_1-1][i_3+i_1+2] * D[i_2+i_1][i_0][i_3][i_1-1] - E[i_1][i_3+i_2][i_0][i_0] * E[i_3][i_0][i_0][i_2-1] + E[i_2][i_3+2][i_0-2][i_2+i_1] * D[i_2][i_3][i_1][i_0] + A[i_3-1][i_2-2][i_0-1][i_3-1] * 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 1; i_6 < min(-i_5+PB_L+1, -i_4+PB_L); i_6++) {
                B[i_4][i_4+1][i_6] = F[i_6+i_4][i_6+i_5-1][i_5-1] - G[i_5+2][i_6-1] + H[i_6+i_5-1][i_5-2] + F[i_4][i_5][1] + A[i_4-1][i_5-2][i_6-1][i_4-1] - 4;
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_P-2; i_7++) {
        for (int i_8 = 1; i_8 < PB_L-1; i_8++) {
            B[i_8][i_8+1][i_7+1] = I[i_8-1] + G[2][i_7] - G[i_7][i_8] + G[i_8][i_7+2] - 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4336252113_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
