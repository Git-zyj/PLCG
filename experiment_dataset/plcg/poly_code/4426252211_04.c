...
void kernel_4426252211_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        A[i_0-2] = E[i_0] * E[i_0-1] + E[i_0] - F[i_0] + F[i_0-2] * A[i_0-3] * A[i_0-1] - A[i_0-3] * A[i_0-3] + 1;
    }
    for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_P; i_2++) {
            for (int i_3 = i_1; i_3 < -i_2+PB_P-1; i_3++) {
                for (int i_4 = 2; i_4 < min(min(-i_3+PB_M+1, PB_N-2), -i_1+PB_Q); i_4++) {
                    B[i_1][i_2][i_4][i_3] = G[i_2][i_4+i_3-1][i_2+i_1][i_3] * G[i_4][i_1][0][i_3+i_2+1] * G[i_4+2][i_1][i_1][i_2+1] + H[i_3][i_4][i_1][i_4+i_1] + H[i_4-2][i_3][i_1-1][i_2] * G[i_2][i_1][i_1+2][i_3] - G[i_2][i_1+1][i_3][i_3] - B[i_3][i_2-1][i_4][i_1] - 3;
                }
            }
        }
        for (int i_5 = 2; i_5 < PB_L; i_5++) {
            for (int i_6 = i_1; i_6 < PB_N; i_6++) {
                C[i_6-1][i_5+1] = I[i_5][i_1][i_6] + I[i_6][i_5][i_1] + J[i_1][i_6][i_5-2] + I[i_1][i_5-1][i_6] * K[i_6][i_5] - D[i_1+1][i_1-1][i_5] - 2;
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_L-2; i_7++) {
        for (int i_8 = 0; i_8 < PB_L; i_8++) {
            for (int i_9 = 1; i_9 < i_7; i_9++) {
                D[i_7+2][i_7][i_8] = I[i_9][i_8][i_7] + J[i_7-2][i_9-1][1] * L[i_9][i_7+2] + 5;
            }
        }
    }
#pragma endscop
