...
void kernel_2432352115_00(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+2; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            C[i_0-1] = E[i_1] - F[i_1][i_0] + G[i_0-2][i_1] * H[i_1][i_0-1] + 1;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_P; i_2++) {
            C[i_0-2] = F[i_0-2][i_2+i_0] + F[i_2][i_0-2] + F[i_2+1][i_0] * 2;
        }
        C[i_0-1] = I[i_0-1] + E[i_0-2] + 3;
    }
    for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
        D[i_3+1] = I[i_3] * I[i_3] - E[i_3] * J[i_3] - E[i_3] * E[i_3] + I[i_3+2] + I[i_3] * D[i_3] - D[i_3+2] + 6;
    }
#pragma endscop
...
kernel_2432352115_00(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
