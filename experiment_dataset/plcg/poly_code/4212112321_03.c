...
void kernel_4212112321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            A[i_1][0] = C[i_1+1][i_1-2] * A[i_1+2][0] - A[i_1-2][0] * A[i_1+1][0] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q; i_2++) {
        for (int i_3 = 4; i_3 < PB_Q; i_3++) {
            for (int i_4 = 2; i_4 < PB_L; i_4++) {
                B[i_4][i_3-2][i_2] = B[i_2-2][i_4-2][i_3] + B[i_4-1][i_3-2][i_2-1] - B[i_4][i_3-4][i_2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4212112321_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
