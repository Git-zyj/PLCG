...
void kernel_4234311123_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < PB_N; i_2++) {
                A[i_2][i_1+i_0][i_0] = A[i_2-2][i_1+i_0][i_0] - 1;
            }
        }
        for (int i_3 = 0; i_3 < min(min(i_0, PB_M), -i_0+PB_N); i_3++) {
            for (int i_4 = 1; i_4 < PB_Q-1; i_4++) {
                A[i_4-1][i_3+i_0][i_0] = C[i_3+1][i_0][i_4-1] + D[i_3][i_0-1][i_4+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4234311123_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
