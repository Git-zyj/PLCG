...
void kernel_4416251125_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[1] + F[i_0] * F[i_0] + E[i_0] - E[i_0+1] * 3;
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, PB_M-1); i_1++) {
            A[i_0] = G[i_0+1][i_1] + G[i_1+i_0][i_1+1] * G[i_1+1][i_1] - A[i_1] * 1;
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                    C[i_0][i_2-1][i_2][i_3] = H[i_2][i_3+1][i_1+1][i_1-1] * I[i_0][i_1][i_0-1] * I[i_3][i_1+1][i_2] + C[i_1+1][i_2-1][i_2][i_3-1] + 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_M+1); i_5++) {
            A[i_5-1] = J[i_4][i_5] * J[i_5][i_4] * J[i_4][i_5] + J[i_5][1] + J[i_5][0] + F[i_4] * J[i_5+i_4-1][i_4-1] - 4;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4416251125_06(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
