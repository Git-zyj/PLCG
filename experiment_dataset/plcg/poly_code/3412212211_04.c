...
void kernel_3412212211_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            A[i_0][i_1] = E[i_1+1][i_0+2] + A[i_1+1][i_0] - 3;
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_0][i_1] = E[i_1][i_0-2] - 3;
            }
        }
        for (int i_3 = 0; i_3 < min(PB_P-3, -i_0+PB_L); i_3++) {
            C[i_0-1][i_3+2] = E[i_0][i_3+i_0] * C[i_0][i_3+2] * C[i_0-1][i_3+1] + C[i_0-1][i_3+3] * 1;
        }
        D[i_0] = D[i_0-1] * 3;
    }
#pragma endscop
...
kernel_3412212211_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
