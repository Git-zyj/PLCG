...
void kernel_3236211115_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            B[i_1][i_2] = B[i_2-1][i_1-1] * B[i_1+1][i_2-1] * 3;
        }
    }
#pragma endscop
...
kernel_3236211115_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
