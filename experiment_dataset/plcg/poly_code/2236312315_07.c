...
void kernel_2236312315_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            A[i_0][0] = A[i_0-1][0] - A[i_0+1][0] + A[i_0+1][0] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        A[i_2][0] = C[i_2] * D[i_2] + 4;
    }
#pragma endscop
...
kernel_2236312315_07(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
