...
void kernel_2426251123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(min(-i_1+PB_M-1, PB_L-1), i_1); i_2++) {
            C[i_2+1] = F[i_1][i_2][i_1] - G[i_2+1][i_1] + H[i_2+i_1+1][0] - D[i_2+2] * 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        C[i_3+1] = I[i_3+1] + I[i_3] * E[0] * I[i_3] + E[i_3] - E[i_3] + 6;
        D[i_3] = E[i_3] * I[i_3+1] * I[i_3-1] * I[i_3] + 5;
    }
#pragma endscop
...
kernel_2426251123_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
