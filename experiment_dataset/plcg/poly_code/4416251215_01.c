    ...
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = max(i_4, 1); i_5 < PB_L+1; i_5++) {
                C[i_3][i_4][i_5+1] = H[i_3-1][i_4][i_5] + C[i_5-1][i_3][i_4+2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4416251215_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
