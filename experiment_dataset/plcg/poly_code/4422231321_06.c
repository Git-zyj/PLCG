...
void kernel_4422231321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = E[i_0+1] - F[i_0+1] - F[i_0-1] - G[i_0][0] * F[i_0] - A[i_0-2] * C[i_0] * 2;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_L; i_2++) {
            B[i_2-1] = E[i_2] - H[i_1-1][0] * H[i_2+i_1-1][i_2+i_1] - G[i_2][i_1] * H[i_2+1][i_1-1] * D[i_1+1] + 4;
        }
        C[i_1] = B[i_1-1] - C[i_1+2] - C[i_1-1] + 3;
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            D[i_4+1] = F[i_4+1] - B[i_4-1] + D[i_4+3] + D[i_4] + D[i_4-1] + 3;
        }
    }
#pragma endscop
...
kernel_4422231321_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
