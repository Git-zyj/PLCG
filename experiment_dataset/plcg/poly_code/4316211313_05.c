...
void kernel_4316211313_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1][i_0] = D[i_1][i_0] - A[i_1][i_0+1] * A[i_1-1][i_0] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
                for (int i_5 = max(1, i_3); i_5 < min(PB_M+1, -i_4+PB_Q); i_5++) {
                    B[i_2][i_3+1][i_5][i_5+i_4] = B[i_2][i_3][i_5][i_5+i_4] + B[i_2][i_3+2][i_5][i_5+i_4] + B[i_5-1][i_4+2][i_2-1][i_3+i_2-2] * 6;
                }
            }
            for (int i_6 = 0; i_6 < min(-i_3+PB_L, -i_2+PB_L-2); i_6++) {
                C[i_6][i_3][i_6+i_2+1] = C[i_6][i_3][i_6+i_2+2] - C[i_6+1][i_3][i_6+i_2+2] - C[i_6][i_2][i_6+i_3] - C[i_6][i_3+1][i_6+i_2+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4316211313_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
