...
void kernel_4434331323_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(i_2, PB_M-1); i_3++) {
                    A[i_1][i_3+1][i_2][i_0+1] = E[i_1-1][i_0][i_3+1][i_2] + F[i_3][i_2][i_1][i_0+1] * A[i_1][i_3+1][i_2-2][i_0+1] * 5;
                }
            }
            for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
                for (int i_5 = 0; i_5 < -i_4+PB_L-1; i_5++) {
                    D[i_5+1][i_1-1] = F[i_4+1][i_5+i_4+1][i_1+1][i_0] - G[i_5+1][i_0][i_1-1][i_0] - F[i_0+1][i_1][i_5+1][i_4+1] * 6;
                }
            }
        }
    }
        ...
        for (int i_7 = i_6; i_7 < PB_M; i_7++) {
            for (int i_8 = 1; i_8 < -i_7+PB_M; i_8++) {
                for (int i_9 = 1; i_9 < min(i_8, -i_7+PB_M); i_9++) {
                    D[i_8][i_9-1] = F[i_9+i_7][i_6][i_8-1][i_7] * E[i_6-1][i_8][i_9][i_7-1] * F[i_9-1][i_6+1][i_9][i_7] + E[i_8+i_7][i_9][i_6][i_8] + 4;
                }
            }
        }
        for (int j_0 = 1; j_0 < PB_M; j_0++) {
            for (int j_1 = max(j_0, 3); j_1 < PB_M-1; j_1++) {
                D[i_6][j_0-1] = D[j_0][j_1-3] * D[j_0][j_1] + D[j_1+1][j_0-1] - D[j_1+1][i_6+1] + D[i_6-2][j_0-1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4434331323_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
