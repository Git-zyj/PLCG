...
void kernel_2324111321_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = PB_M-1 - 1; i_0 >= 3; i_0--) {
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_M-1, PB_L-2); i_1++) {
            C[i_1+i_0-1][i_1] = C[i_1+i_0-3][i_1] + C[i_1+i_0+1][i_1+2] + C[i_1+i_0-3][i_1-2] * 5;
        }
    }
#pragma endscop
...
kernel_2324111321_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
