...
void kernel_3236232221_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_3 = 3; i_3 < PB_L; i_3++) {
        B[i_3-1] = C[i_3] + D[i_3] * D[i_3] - C[i_3] + C[i_3] + B[i_3-3] * B[i_3+1] + 4;
    }
#pragma endscop
...
kernel_3236232221_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
