...
void kernel_3232111311_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            B[i_0+1][i_1-1][i_0+1] = C[i_1+i_0][i_0+1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            for (int i_4 = PB_P - 1; i_4 >= 0; i_4--) {
                B[i_2][i_3][i_2] = B[i_2][i_3+1][i_2] * B[i_2][i_3-1][i_2] - B[i_2-1][i_3][i_2-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3232111311_04(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
