...
void kernel_3424231123_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+i_0][i_0-1] = E[i_0][i_1] + A[i_1+i_0+2][i_0+1] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_2][i_3] = E[i_3][i_2] + F[i_2-1] - E[i_3][i_2+1] - G[i_2][i_3] - B[i_2+2][i_3-1] - 6;
        }
        D[i_2-1] = F[i_2-1] + F[i_2] - 1;
        D[i_2-1] = H[i_2] * H[i_2-1] + H[i_2+1] + 3;
    }
#pragma endscop
...
kernel_3424231123_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
