...
void kernel_4234211121_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = i_3; i_5 < min(PB_P-2, -i_4+PB_M); i_5++) {
                B[i_4][i_3][i_5+1] = C[i_5+i_4][i_3][i_4-1] - B[i_4][i_3][i_5+2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4234211121_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
