...
void kernel_2436252125_05(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M - 1; i_0 >= 0; i_0--) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            D[i_1] = E[i_1] + F[i_1+1][i_0] - E[i_1] * G[i_1][i_0] * G[0][i_1] * F[i_1+2][i_0] + F[i_1][2] * H[i_0][i_1-1][i_0] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        D[i_2] = E[i_2-1] * I[i_2] + I[i_2] + I[i_2] + 1;
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        D[i_3+1] = F[i_3][i_3+1] + I[i_3] * E[i_3] - 1;
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        D[i_4] = I[1] + F[i_4+1][1] * E[i_4] + E[i_4-2] * E[i_4] + D[i_4+2] * 5;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2436252125_05(xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
