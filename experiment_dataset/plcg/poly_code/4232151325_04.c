...
void kernel_4232151325_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            B[i_0+2][i_1] = C[i_1][i_0] + D[i_1] - C[i_1+1][i_0] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_N-1; i_3++) {
            B[i_2][i_3] = E[i_3+i_2+1][i_2][i_3] * C[i_3][i_2] - C[i_2][i_3] * C[0][i_3+1] * C[i_3][i_2] + C[i_3+1][i_2] - C[i_3+1][i_2] - B[i_3-1][i_2] - B[i_2+2][i_3] - B[i_2][i_3+1] * 6;
        }
    }
#pragma endscop
...
kernel_4232151325_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
