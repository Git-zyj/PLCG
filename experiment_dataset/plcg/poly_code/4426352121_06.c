...
void kernel_4426352121_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0+1] = E[i_0] * F[i_0+2] * G[i_0-1] + G[i_0] - 1;
        for (int i_1 = i_0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_L-2, -i_0+PB_Q+2); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_Q+1, -i_1+PB_P); i_3++) {
                    B[i_0][i_3][i_2+1][1] = H[i_3+i_2][i_0+1][i_0][i_1+2] * I[i_2+i_1+2][i_3][i_0-1][i_2] * I[i_1][i_2][i_3+i_0-1][i_1] - I[i_2][i_0][i_1+1][i_3+2] - J[i_1+2][i_2-2][i_3][i_0] + K[i_2][i_3][i_2-2] * H[i_3+1][i_3][i_2-2][i_1+2] + K[i_0][i_2-2][i_3+i_1] * I[i_3+i_1-1][i_2+i_0-2][i_1][i_1] + 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, PB_L-2); i_5++) {
            for (int i_6 = 1; i_6 < min(-i_4+PB_M, i_5); i_6++) {
                C[i_5+1][i_4-2][i_6+1][i_6-1] = L[i_4][i_5+2][i_6] - M[i_6+i_4][i_5+2] - N[i_6][i_5+1][i_4-2] * 6;
            }
        }
    }
        ...
        for (int i_8 = i_7; i_8 < PB_P-1; i_8++) {
            for (int i_9 = 1; i_9 < PB_M-1; i_9++) {
                for (int j_0 = 2; j_0 < PB_M-2; j_0++) {
                    D[i_7][j_0][i_9][i_7+1] = I[j_0][i_8][i_7+2][i_9+1] - J[j_0+2][i_9+1][i_8][i_7+1] * D[i_7+2][j_0-2][i_9][i_7+3] + D[i_8][i_9-1][j_0+1][i_8+1] * 6;
                }
            }
        }
    }
...
kernel_4426352121_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N));
