...
void kernel_4416332125_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        B[i_0] = E[i_0] * F[i_0] + F[0] + F[i_0-2] * G[i_0] + 6;
        B[i_0] = F[i_0+1] * G[i_0] + E[i_0-2] * G[i_0] - B[i_0-2] - 2;
    }
    for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L; i_2++) {
            for (int i_3 = 2; i_3 < PB_L+1; i_3++) {
                for (int i_4 = 0; i_4 < -i_3+PB_L+1; i_4++) {
                    D[i_1-1][i_4+i_3-1][i_4][i_2-1] = H[i_3-2][i_1][i_4][i_3-2] * 1;
                }
            }
            for (int i_5 = 0; i_5 < -i_2+PB_L; i_5++) {
                for (int i_6 = 3; i_6 < -i_1+PB_M; i_6++) {
                    D[i_1-1][i_5+i_2-1][i_2][i_6-1] = I[i_6-2][i_5][i_1+2][i_5+i_2] * H[i_5+i_2][i_6+i_1][i_5+i_2][2] + D[i_1-1][i_5+i_2][i_2][i_6-3] * 2;
                }
            }
        }
    }
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4416332125_06(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
