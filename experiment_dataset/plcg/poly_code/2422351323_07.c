...
void kernel_2422351323_07(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_N, -i_0+PB_L); i_1++) {
            C[i_1+1][i_1+i_0] = E[i_1] - F[i_1][i_0] + G[i_1][i_1] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_P; i_3++) {
            B[i_2][i_3] = H[i_2-1][i_3] * H[i_3+1][i_2] - B[i_2+1][i_3] * B[i_2][i_3-2] + B[i_2][i_3+2] * B[i_2-2][i_3] - B[i_2+2][i_3] - 5;
        }
    }
    for (int i_4 = 2; i_4 < PB_P+1; i_4++) {
        for (int i_5 = 1; i_5 < min(-i_4+PB_L-1, PB_N+1); i_5++) {
            C[i_5][i_5+i_4+1] = H[i_5][i_4-1] + E[i_5-1] + E[i_5-1] * H[i_4][i_5] + G[i_5-1][i_4-1] * I[i_5] * D[i_5][i_4] - D[i_5][i_4-2] + 4;
            D[i_5][i_4] = F[i_5+i_4-1][i_4] - H[i_4][i_5] - F[i_4+1][i_5+1] * F[i_5][i_4+1] + H[i_5][i_5] + J[i_5+i_4-1][i_4-1][i_5-1] * 6;
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2422351323_07(xc,yc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
