...
void kernel_4412132323_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            A[i_0+1][0] = E[i_0] + F[i_0][i_1+i_0] + F[i_1][i_0-1] - E[i_0] * F[i_0-2][i_1] - F[i_0][0] * A[i_0+2][0] * A[i_0+3][0] + A[i_0-1][0] * A[i_0][0] + A[i_0][0] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 2; i_4 < min(min(-i_3+PB_M+1, i_2), PB_L-1); i_4++) {
                C[i_2-1][i_2-1][i_4][i_3] = G[i_2][i_4+i_3-1][i_4] - A[i_3+1][0] - 4;
                for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
                    C[i_2][i_2][i_4][i_5] = H[i_4-2][0][i_5+1][i_2-2] + C[i_2+2][i_2+2][i_4+1][i_5] - 2;
                }
            }
        }
        for (int i_6 = 2; i_6 < PB_L; i_6++) {
            for (int i_7 = i_6; i_7 < -i_6+PB_P+2; i_7++) {
                for (int i_8 = i_6; i_8 < -i_2+PB_M-2; i_8++) {
                    D[i_2+1][i_2][i_7][i_8+1] = H[i_8+i_2+2][i_7][i_6][i_7+i_6-2] - H[i_6][i_7][i_2-1][i_8] + D[i_2+1][i_2][i_7-2][i_8+1] + 5;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4412132323_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
