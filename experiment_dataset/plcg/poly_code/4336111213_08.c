...
void kernel_4336111213_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_1-1][i_0-1] = A[i_1-1][i_0-2] - A[i_1][i_0] * B[i_1-1] - B[i_1] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        B[i_2] = C[i_2+1][i_2+1] * 1;
        for (int i_3 = i_2; i_3 < PB_N-1; i_3++) {
            C[i_3+1][i_3+1] = D[i_3] * B[i_3] + 6;
        }
    }
#pragma endscop
...
kernel_4336111213_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
