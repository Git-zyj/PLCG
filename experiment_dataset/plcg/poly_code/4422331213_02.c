...
void kernel_4422331213_02(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < min(min(PB_N, -i_0+PB_L), -i_1+PB_L); i_2++) {
                for (int i_3 = i_0; i_3 < min(-i_0+PB_M, -i_1+PB_P-1); i_3++) {
                    D[i_3][i_2][i_1] = E[i_2][i_0][i_3][i_1+1][i_2-1] + F[i_1][i_2+i_0][i_0][i_1] - G[i_2][i_1+1][0][i_0+1] + H[i_3][i_2][i_1] + E[i_2][i_3+i_1+1][i_0][i_2][i_1+1] + F[i_3+i_0][i_2+i_1][i_3+i_1][0] - F[i_1][i_3][i_0+1][i_2] + B[i_3][i_1][i_0][i_2] + 6;
                }
            }
        }
        for (int i_4 = i_0; i_4 < -i_0+PB_M-1; i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_N; i_5++) {
                for (int i_6 = 0; i_6 < PB_Q; i_6++) {
                    B[i_4][i_5][i_0+1][i_6] = I[i_6][i_4+1][i_5][i_0-1] * G[i_5+i_4][i_4+i_0+1][i_6][i_0] - B[i_6][i_5][i_4+1][i_0+1] * B[i_4-1][i_5][i_0+1][i_6] + 4;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
        for (int i_8 = 0; i_8 < PB_Q-1; i_8++) {
            for (int i_9 = 1; i_9 < PB_P; i_9++) {
                for (int j_0 = 0; j_0 < min(-i_8+PB_Q, -i_7+PB_M+1); j_0++) {
                    D[i_8+1][i_9][j_0] = F[j_0+i_7-1][i_9][j_0+i_8][i_9-1] - F[i_7-1][i_9][i_8+1][j_0] + I[i_9-1][j_0][i_7][i_9] + 2;
                }
            }
        }
        for (int j_1 = 1; j_1 < PB_P; j_1++) {
            for (int j_2 = 0; j_2 < PB_Q; j_2++) {
                D[i_7+1][j_1][j_2] = D[i_7+1][j_1-1][j_2] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4422331213_02(xd,yd,zd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
