...
void kernel_3212211111_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_L+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_2][i_1-1][i_0] = D[i_2][i_0][i_2] * B[i_2][i_1-2][i_0] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3212211111_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
