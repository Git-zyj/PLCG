...
void kernel_3322111323_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
    ...
    for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_M-3); i_2++) {
            for (int i_3 = 0; i_3 < min(-i_2+PB_L, -i_1+PB_M-1); i_3++) {
                C[i_3+i_2+1][i_2] = D[i_3+i_2][i_1-1] - C[i_3+i_1+1][i_1] + 6;
            }
            C[i_2+i_1+1][i_1] = E[i_2] - C[i_2+i_1+2][i_1] - C[i_2+i_1+3][i_1+2] + 3;
        }
    }
#pragma endscop
...
kernel_3322111323_02(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
