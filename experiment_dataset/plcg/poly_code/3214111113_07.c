...
void kernel_3214111113_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_Q, -i_1+PB_L); i_2++) {
            B[i_1][i_2+i_1][i_2] = C[i_2][i_1] * B[i_2-1][i_2+i_1][i_1+1] * 3;
        }
    }
#pragma endscop
...
kernel_3214111113_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
