...
void kernel_4326131211_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = max(1, i_0); i_1 < PB_P; i_1++) {
            for (int i_2 = i_0; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_P; i_3++) {
                    A[i_3+1][i_2+1][i_3] = D[i_3][i_2][i_3+i_1][i_0] - D[i_3+1][0][i_1-1][i_0] + A[i_3][i_2+2][i_3-1] * 5;
                }
            }
        }
    }
    ...
    for (int i_5 = 1; i_5 < PB_L+1; i_5++) {
        for (int i_6 = 1; i_6 < min(i_5, PB_L-1); i_6++) {
            for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
                C[i_6][i_7] = F[i_6-1][i_5-1][0] * F[i_5-1][i_6][i_7-1] + F[i_6+1][i_7+1][i_5] + C[i_6][i_7+1] - C[i_6][i_7-1] * 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4326131211_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
