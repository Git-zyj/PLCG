...
void kernel_3422331311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[i_0] * F[i_0] + A[i_0+2] + A[i_0] + 4;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            C[i_2+1] = G[i_2][i_1] + G[i_1][1] * G[i_2][i_1-1] - 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        C[i_3] = F[i_3] + C[i_3+1] * C[i_3-1] * C[i_3-1] + 5;
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = i_4; i_5 < min(-i_3+PB_Q-1, PB_M-1); i_5++) {
                D[i_5+1][i_3][i_4] = H[i_4][i_5][i_5+i_3+1] - I[i_5][i_3][i_4] * I[i_5][i_3+1][i_4+1] + D[i_5][i_3][i_4] * D[i_5+1][i_3-1][i_4-1] + D[i_5+1][i_3][i_4-1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3422331311_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
