...
void kernel_2214111311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            A[i_0][i_1] = C[0] * B[i_0][i_1][i_0] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_N; i_3++) {
            B[i_3][i_2][i_3] = B[i_2+1][i_3][i_2+1] - B[i_2-1][i_3][i_2-1] * B[i_3][i_2-1][i_3] + B[i_3][i_2+1][i_3] - 4;
        }
    }
#pragma endscop
...
kernel_2214111311_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
