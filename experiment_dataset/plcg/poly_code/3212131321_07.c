...
void kernel_3212131321_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_0][i_1+1][0] = A[i_0][i_1-1][0] + A[i_0-2][i_1-1][0] + 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M+1, PB_L); i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_L; i_5++) {
                B[0][i_4][i_5] = C[i_4][i_5][i_3] * C[i_3][i_5+i_4][i_4] - C[i_3][i_5-1][0] - C[i_4+i_3-1][i_5+1][i_5] + C[i_4][i_3][i_5] - B[0][i_4][i_5-1] - B[0][i_4-2][i_5+1] * B[0][i_4+2][i_5+2] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3212131321_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
