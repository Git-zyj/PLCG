...
void kernel_3216332115_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_N-2); i_1++) {
            A[i_0+2][i_1] = C[i_0][i_1-2] - D[i_1+i_0] - C[i_1+2][i_0] * E[i_1-1][i_0] + A[i_0+2][i_1-1] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_N-2, i_2); i_3++) {
            A[i_2+2][i_3] = C[i_3+2][i_2] * 6;
        }
    }
#pragma endscop
...
kernel_3216332115_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
