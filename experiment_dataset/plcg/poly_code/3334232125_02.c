...
void kernel_3334232125_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0] - D[i_0+2] * E[i_0] - A[i_0-2] * 3;
    }
    for (int i_1 = 2; i_1 < PB_N+1; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_N, i_1); i_2++) {
            for (int i_3 = 4; i_3 < -i_1+PB_N+1; i_3++) {
                B[i_1-1][i_2][i_3-2] = F[i_2][i_3+i_1-1][i_1-1] * G[i_2][i_1][i_3-2] + B[i_1-2][i_2][i_3-4] * 5;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N; i_4++) {
        A[i_4] = H[i_4][1] * E[i_4] * E[i_4+1] - 5;
    }
#pragma endscop
...
kernel_3334232125_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
