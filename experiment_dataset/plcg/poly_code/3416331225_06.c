...
void kernel_3416331225_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            B[i_1][i_0-1][i_1] = E[i_1+1][i_0+1] + F[i_1][i_0] * G[i_0+1][i_1] * F[i_1][i_0] * F[i_1-1][i_0] - 4;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            for (int i_3 = 1; i_3 < PB_M; i_3++) {
                B[i_3][i_2-1][i_3] = F[i_2][i_0+1] - H[i_3-1][i_0][i_2] - B[i_3-1][i_2-1][i_3-1] + B[i_3][i_2+1][i_3] + 2;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            for (int i_6 = 0; i_6 < i_4; i_6++) {
                B[i_5][i_4-2][i_5] = H[i_5][i_6][i_4] * 4;
            }
            for (int i_7 = 0; i_7 < PB_Q; i_7++) {
                B[i_4][i_5-1][i_4] = E[i_5-1][i_4] - 3;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3416331225_06(xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
