...
void kernel_4212211223_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < PB_L+2; i_1++) {
            for (int i_2 = i_1; i_2 < min(PB_Q+1, -i_1+PB_M+2); i_2++) {
                B[i_2+i_1-2][i_1-2][i_2-1] = 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_Q; i_3++) {
        for (int i_4 = 3; i_4 < min(PB_Q+3, -i_3+PB_M-1); i_4++) {
            B[i_4+i_3][i_4][i_3-1] = C[i_4-1][i_3-1] - B[i_4+i_3-2][i_3][i_4-3] + B[i_4+i_3+1][i_4][i_3] * 5;
        }
    }
#pragma endscop
...
kernel_4212211223_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
