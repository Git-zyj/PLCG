...
void kernel_2416352215_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            B[i_1] = E[i_1+i_0][i_1] + F[i_1-1][i_0+1] * F[i_1][i_0+2] * F[i_0][0] + 3;
        }
        for (int i_2 = i_0; i_2 < PB_L; i_2++) {
            B[i_2] = F[i_0][i_2] - G[i_2+2][i_0] * E[i_2][i_0] - F[i_2+1][i_0-1] * E[i_0][0] + B[i_2+1] + B[i_2-1] + 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_L; i_4++) {
            B[i_4+1] = G[i_3][i_4] * E[i_4+1][i_3] - G[i_3][i_4-1] * H[i_3][i_4-2][i_3-1] * G[i_3][i_4-2] * G[i_4][i_3] + 4;
            B[i_4-1] = F[i_4+1][i_3] - G[i_3][i_4] + G[i_4-2][i_3] * F[i_4-1][i_3] + I[i_3] - 1;
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2416352215_05(xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
