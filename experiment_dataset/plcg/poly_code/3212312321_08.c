...
void kernel_3212312321_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            B[i_3][i_2] = C[i_2][0] + B[i_3][i_2+2] + B[i_3+2][i_2] - B[i_3-1][i_2+2] + B[i_3-1][i_2] - 2;
        }
    }
#pragma endscop
...
kernel_3212312321_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
