...
void kernel_4332251311_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] + D[0] + E[i_0] - E[i_0] * E[i_0-1] * A[i_0+1] + A[i_0-1] + A[i_0+1] + 2;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            B[i_2-1][i_1] = F[i_1-1][i_1] * G[i_2][i_1] - F[i_2-1][i_1] * D[i_2] - G[i_1-1][i_2] + G[i_2-1][i_1] - 2;
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
                C[i_4][i_3] = H[i_3-1][i_1][i_3] + G[i_3][i_1-1] - I[i_3][i_4][i_1] - C[i_4-1][i_3-1] + C[i_4][i_3-1] * C[i_4+1][i_3] + C[i_3+1][i_4+1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4332251311_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
