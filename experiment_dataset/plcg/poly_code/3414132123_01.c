...
void kernel_3414132123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        A[i_0+1] = E[i_0] * F[i_0][i_0] * E[0] + 6;
    }
    for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
        A[i_1+1] = E[i_1] * E[i_1-1] + E[i_1] - 5;
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = 2; i_3 < min(-i_2+PB_L-2, i_2); i_3++) {
                D[i_2+2][i_3-2] = G[i_1][i_1][i_2] * G[i_1+2][i_3][0] * G[i_3][i_1][0] + H[i_3+i_2+2][i_1][0][i_1+1] - 6;
            }
        }
        for (int i_4 = 2; i_4 < min(-i_1+PB_P+1, i_1); i_4++) {
            D[i_1+2][i_4-2] = F[i_4][i_4+i_1-1] * A[i_4+2] * 2;
        }
    }
#pragma endscop
...
kernel_3414132123_01(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
