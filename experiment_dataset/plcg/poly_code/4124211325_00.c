...
void kernel_4124211325_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_P-2, i_1); i_2++) {
                A[i_1+1][i_0+1][i_2+1] = B[i_2][i_1] + A[i_1+1][i_0+2][i_2+1] * A[i_1+1][i_0+2][i_2+2] - A[i_1+1][i_0-1][i_2+1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4124211325_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
