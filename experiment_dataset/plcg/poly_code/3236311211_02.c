...
void kernel_3236311211_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_P+2, -i_0+PB_P+1); i_2++) {
                A[i_1][i_2+i_0-1][i_2][i_0+1] = C[0][i_1][i_0] * A[i_0-1][i_2+i_1-2][i_2-1][i_1+1] * 6;
            }
        }
        B[i_0][i_0] = B[i_0+1][i_0+1] - B[i_0-1][i_0-1] + B[i_0-1][i_0-1] + 1;
    }
#pragma endscop
...
kernel_3236311211_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
