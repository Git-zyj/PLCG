...
void kernel_4232212213_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                A[i_0][i_1-1][i_2] = C[i_0+1][i_2][i_1] + A[i_0][i_1-2][i_2] + A[i_0-1][i_1][i_2-1] + 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            B[i_4][i_3] = D[i_4][i_3] + B[i_4][i_3-1] - 3;
        }
    }
#pragma endscop
...
kernel_4232212213_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
