...
void kernel_3412132125_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        A[i_0+1] = E[i_0] + E[i_0-1] + E[i_0] - E[i_0+2] - A[i_0+3] - 1;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        ...
        for (int i_3 = 0; i_3 < PB_N; i_3++) {
            for (int i_4 = max(2, i_3); i_4 < PB_N; i_4++) {
                D[i_4] = F[i_4][i_1+1] * G[i_1][i_4-2][i_3] * G[i_1+2][i_3][i_4] * F[i_3][i_4] + 2;
            }
            D[i_3] = 6;
        }
    }
...
kernel_3412132125_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
