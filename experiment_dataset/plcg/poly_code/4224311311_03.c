...
void kernel_4224311311_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L, i_0); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_P-1, i_1); i_2++) {
                A[i_0][i_1][i_2] = A[i_0][i_1][i_2+1] + A[i_0][i_1][i_2-1] - A[i_0+1][i_1][i_2-1] - 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_Q+1, -i_3+PB_M-1); i_4++) {
            B[i_3+1][i_4-1] = C[i_4+i_3+1][0] * B[i_3][i_4-1] - B[i_3+2][i_4-1] + 4;
        }
    }
#pragma endscop
...
kernel_4224311311_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
