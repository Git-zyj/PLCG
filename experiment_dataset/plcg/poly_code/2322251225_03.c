...
void kernel_2322251225_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        B[i_0-1] = D[i_0+1] + E[i_0+1] - E[i_0] + E[i_0] * D[i_0] - D[i_0] + B[i_0-1] - 5;
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            B[i_0-1] = E[0] * F[i_1][i_0] * F[i_0+1][i_0] * B[i_1+1] - 5;
        }
    }
    for (int i_2 = 3; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            C[i_2][i_3-1] = G[i_3][i_2] + H[i_3][i_2+1][i_3] + G[i_3][i_2] - E[i_3] * G[i_2][i_3] + G[i_2][i_3+1] + C[i_3][i_2-3] * 5;
        }
    }
#pragma endscop
...
kernel_2322251225_03(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
