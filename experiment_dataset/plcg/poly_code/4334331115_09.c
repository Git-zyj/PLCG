...
void kernel_4334331115_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = PB_Q - 1; i_1 >= i_0; i_1--) {
            for (int i_2 = 0; i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_L, PB_L-1); i_3++) {
                    A[i_0][i_3][i_2][i_1] = D[1][i_3+i_0][i_1][i_0] + E[i_3][i_2+i_0][i_3+1][i_1] - A[i_0+1][i_3][i_2][i_1] * 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        B[i_4-1] = F[i_4] * G[i_4] * 1;
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        B[i_5-1] = H[i_5-1] * H[0] * H[i_5] - 5;
    }
#pragma endscop
...
kernel_4334331115_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
