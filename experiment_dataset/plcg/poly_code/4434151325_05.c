...
void kernel_4434151325_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
                for (int i_3 = i_2; i_3 < -i_0+PB_M; i_3++) {
                    A[i_2][i_0][i_1][i_0] = E[i_2][i_3+i_0][i_1][i_0] * E[i_2+1][i_3][i_1+1][i_0+1] - E[i_3][i_0][i_1][i_2+1] - E[i_1-1][i_3][i_0][i_2] * E[i_0+1][i_1][i_0][i_2] * A[i_3][i_0+1][i_1+2][i_0+1] * A[i_2-1][i_0][i_1+1][i_0] * A[i_2][i_0][i_1+2][i_0] - 3;
                }
            }
        }
        B[i_0-1] = F[i_0][0] * G[i_0-1] - G[i_0+1] - B[i_0+1] * B[i_0-3] - 1;
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            B[i_5-1] = F[i_5][i_4-1] * F[i_5][i_4] - G[i_5] + F[i_5][1] * G[i_5] * 1;
        }
    }
    for (int i_6 = PB_L - 1; i_6 >= 2; i_6--) {
        for (int i_7 = 0; i_7 < -i_6+PB_L; i_7++) {
            for (int i_8 = 0; i_8 < PB_L; i_8++) {
                B[i_6-2] = F[i_8+i_7][0] - H[i_7+1][i_6][i_8+i_7] * H[i_7+i_6][i_8][i_7] - H[i_8][i_7+i_6][i_6] + 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434151325_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
