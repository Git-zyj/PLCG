...
void kernel_2424251313_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0+1] = E[i_0+1] * E[i_0+1] * A[i_0] - A[i_0+2] * A[i_0] * 3;
        for (int i_1 = 1; i_1 < min(i_0, PB_P-1); i_1++) {
            B[i_0][i_1] = F[i_1-1][i_0] + G[i_0][i_1-1] * H[i_0-1][i_1][i_0-1] * A[i_0+1] + B[i_0][i_1+1] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        C[i_2] = I[i_2] + E[i_2] - I[i_2] - I[i_2] - E[i_2+1] + E[i_2-1] - C[i_2+1] + C[i_2-1] - 2;
        D[i_2] = F[i_2][1] + E[i_2] - E[i_2+1] - E[i_2] + I[i_2] * E[i_2] * I[0] * E[i_2] - E[i_2] * D[i_2+1] - D[i_2-1] - 1;
    }
#pragma endscop
...
kernel_2424251313_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
