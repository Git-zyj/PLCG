...
void kernel_3334112325_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-4; i_1++) {
            A[i_1+2][0] = D[i_0][i_1-1] * A[i_1+1][0] + A[i_1+3][0] - A[i_1+4][0] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            B[i_2][i_3] = B[i_2+1][i_3+2] * B[i_2-2][i_3] * 2;
        }
    }
    for (int i_4 = 0; i_4 < PB_N-2; i_4++) {
        C[i_4][i_4+2] = C[i_4+1][i_4+3] + C[i_4+2][i_4+4] - 4;
    }
#pragma endscop
...
kernel_3334112325_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
