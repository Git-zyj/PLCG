...
void kernel_2316352113_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0] - E[i_0] * D[i_0] * C[i_0-1] * 3;
        for (int i_1 = max(2, i_0); i_1 < PB_M; i_1++) {
            A[i_1] = F[i_0][i_0] * F[i_1][i_1] - G[i_1][i_0-1] * G[i_1-2][i_0] * H[1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        C[i_2] = H[i_2+1] * E[i_2] + D[i_2] + D[i_2] - H[0] * E[i_2-1] + D[i_2] * A[i_2+1] * 3;
    }
#pragma endscop
...
kernel_2316352113_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
