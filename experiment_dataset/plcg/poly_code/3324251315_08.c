...
void kernel_3324251315_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = D[i_0+1] - D[i_0] + D[i_0+1] - E[i_0] - D[i_0] - E[i_0] - A[i_0] * A[i_0-2] - A[i_0-2] - 2;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L; i_2++) {
                B[i_2+1][i_1] = F[i_2][i_0+1][i_1-1] - F[i_0][i_2+i_1][1] + B[i_2+1][i_1-1] + B[i_1][i_2] + 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P; i_3++) {
        A[i_3-2] = E[i_3] * E[i_3] * G[i_3][0] * D[i_3+1] - E[i_3] * E[i_3-1] + 5;
    }
#pragma endscop
...
kernel_3324251315_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
