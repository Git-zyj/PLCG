...
void kernel_3236311215_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                B[i_2][i_2-1] = C[i_1][i_2][i_0-1] + C[i_1][i_0][i_2] * 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_M-1, i_3); i_4++) {
            for (int i_5 = i_4; i_5 < PB_P; i_5++) {
                B[i_4][i_4-1] = B[i_4+1][i_4] + B[i_4-1][i_4-2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3236311215_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
