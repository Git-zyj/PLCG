...
void kernel_4414231215_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_N-1; i_1++) {
            B[i_1+1][i_0] = E[i_1+i_0+1][i_1+1][1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
                B[i_3][i_4-1] = F[i_4][i_3-1] + B[i_3][i_4-2] + B[i_3][i_4] - B[i_4+1][i_3-1] * 5;
            }
        }
        for (int i_5 = 1; i_5 < -i_2+PB_Q; i_5++) {
            for (int i_6 = 1; i_6 < PB_L-2; i_6++) {
                for (int i_7 = i_2; i_7 < PB_Q; i_7++) {
                    C[i_2][i_5][i_6+1][i_2] = G[i_5-1][i_6][i_7][i_6] + H[i_5][i_2][i_6-1][i_7] - H[i_7][i_2][i_5][i_6] + G[i_7+1][i_6+1][i_5+i_2][0] - G[i_7+1][i_2][i_5+1][i_7-1] + G[i_7-1][i_2][i_6+i_5-1][i_2-1] * C[i_2-1][i_7][i_6+2][i_2-1] * 4;
                }
            }
            B[i_5][i_2-1] = F[i_2-1][i_5] * F[i_2][i_5+i_2] + I[i_2+1][i_5-1] - 1;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4414231215_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
