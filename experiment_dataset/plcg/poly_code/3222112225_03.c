...
void kernel_3222112225_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_0+2][i_2][i_1] = C[i_2+2][i_0][0] * A[i_0+2][i_2+2][i_1] + B[i_1][i_2] * 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M+1, PB_L-2); i_4++) {
            B[i_4][i_3] = D[i_4+i_3-1][i_3] + B[i_4+2][i_3-1] + B[i_4][i_3+1] - 5;
        }
    }
#pragma endscop
...
kernel_3222112225_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
