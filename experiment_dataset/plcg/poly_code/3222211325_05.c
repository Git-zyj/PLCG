...
void kernel_3222211325_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            A[i_0][i_1][0] = A[i_0][i_1-1][0] - A[i_0+2][i_1][0] - A[i_1+1][i_0][0] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                A[i_3-1][i_2][0] = C[i_3][i_2][i_3] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3222211325_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
