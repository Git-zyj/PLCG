...
void kernel_2222212115_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_0] = A[i_1+1] - A[i_1-1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        B[i_2-1] = C[i_2] - C[i_2+2] - 1;
    }
#pragma endscop
...
kernel_2222212115_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
