...
void kernel_3234212211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0][i_1] = A[i_0-1][i_1] - A[i_0][i_1-1] + A[i_0+1][i_1] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M+1; i_2++) {
        B[i_2-1] = C[i_2-2] * B[i_2-2] * 6;
    }
#pragma endscop
...
kernel_3234212211_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
