...
void kernel_4232132223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+2][i_0-1] = C[i_1+i_0][i_1] * C[i_1-2][i_0] - C[i_1+i_0-1][1] + A[i_1][i_0-2] - A[i_1][i_0] * 4;
        }
        for (int i_2 = 0; i_2 < PB_P-4; i_2++) {
            A[i_2+4][i_0-1] = C[i_2][i_0] + C[i_2][i_0-2] * 5;
        }
    }
#pragma endscop
...
kernel_4232132223_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
