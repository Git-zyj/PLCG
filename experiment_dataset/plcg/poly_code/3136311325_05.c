...
void kernel_3136311325_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 3; i_1 < min(-i_0+PB_P+1, PB_M); i_1++) {
            for (int i_2 = 3; i_2 < min(min(-i_1+PB_M, -i_0+PB_P), PB_L+1); i_2++) {
                A[i_2+i_1][i_1-1][i_2+i_0] = B[i_1][i_2-1] - A[i_2+i_1-2][i_1-3][i_2+i_0] + A[i_2+i_1-3][i_2-3][i_1+i_0-1] - A[i_2+i_1][i_1-1][i_2+i_0-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3136311325_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
