...
void kernel_2224112211_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            A[i_1] = C[i_1+1] + A[i_1-1] + A[i_1+1] * 2;
        }
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            B[i_0-1] = D[i_0][i_0] + B[i_0] * 2;
        }
    }
#pragma endscop
...
kernel_2224112211_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
