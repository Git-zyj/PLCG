...
void kernel_2424132121_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N; i_1++) {
            A[i_1+i_0-2][i_1] = E[i_1] * E[i_1] - F[i_1+i_0][i_0] - F[i_0][i_0-1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_N; i_3++) {
            B[i_2][i_3] = F[i_3][i_2] - F[i_3][i_2] * 5;
        }
    }
    for (int i_4 = 2; i_4 < PB_N; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_N); i_5++) {
            C[i_5+i_4-1][i_5+2] = F[i_4-2][i_5] - F[i_5+i_4][i_4] + C[i_5+i_4-2][i_5+1] - 4;
        }
        for (int i_6 = 0; i_6 < PB_M; i_6++) {
            D[i_4][i_4-1] = F[i_4-1][0] - D[i_4+2][i_4+1] + 2;
        }
    }
#pragma endscop
...
kernel_2424132121_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
