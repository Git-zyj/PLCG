...
void kernel_4226311315_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            B[i_0+1][i_1] = C[i_1-1][i_1] - B[i_0+1][i_1] * 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_P; i_3++) {
            B[i_2+1][i_3] = B[i_2][i_3] + B[i_2+2][i_3] + 6;
        }
    }
#pragma endscop
...
kernel_4226311315_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
