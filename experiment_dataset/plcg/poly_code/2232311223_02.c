...
void kernel_2232311223_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0+1] = C[0] + C[i_0-1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_N; i_2++) {
            B[i_2][i_1+1] = B[i_2][i_1] - B[i_2][i_1-1] - B[i_2-2][i_1+1] * 3;
        }
    }
#pragma endscop
...
kernel_2232311223_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
