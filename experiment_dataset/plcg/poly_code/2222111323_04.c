...
void kernel_2222111323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+i_0] = C[i_1+i_0] + A[i_1+i_0-2] * A[i_1+i_0-2] * A[i_1+i_0+2] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_M-2); i_3++) {
            A[i_3+i_2+2] = D[i_3+1][i_2] + 2;
        }
    }
#pragma endscop
...
kernel_2222111323_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
