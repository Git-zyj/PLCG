...
void kernel_2434111323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0][i_0] = E[i_0] - A[i_0-1][i_0-1] + A[i_0+2][i_0+2] * D[i_0-1][i_0] * 2;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
            B[i_2][i_2] = B[i_2+1][i_2+1] * B[i_2+2][i_2+2] * B[i_2-1][i_2-1] * B[i_2-2][i_2-2] + 2;
        }
    }
    ...
    for (int i_4 = 3; i_4 < PB_L-2; i_4++) {
        D[i_4-1][i_4] = E[i_4] * E[i_4] + D[i_4+1][i_4+2] - D[i_4-2][i_4-1] * D[i_4][i_4+1] * D[i_4-3][i_4-2] - 6;
    }
#pragma endscop
...
kernel_2434111323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
