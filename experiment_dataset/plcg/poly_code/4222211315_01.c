...
void kernel_4222211315_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
                B[i_4][i_3-1] = C[1][i_4+1] - B[i_4-1][i_3-1] * B[i_4][i_3] + B[i_4+1][i_3-1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4222211315_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
