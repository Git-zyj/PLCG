...
void kernel_2334131115_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_M, PB_M-1); i_2++) {
            B[i_1][i_2-1] = E[i_2+i_1][i_1] - E[i_2+1][i_1] - B[i_1][i_2-2] - 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        A[i_3] = D[i_3] * D[i_3-1] * D[i_3] * 3;
    }
#pragma endscop
...
kernel_2334131115_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
