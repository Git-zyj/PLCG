...
void kernel_2134211325_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M-1, -i_0+PB_L-1); i_1++) {
            A[i_1][i_0] = B[i_1+i_0+1][i_1][i_0] + A[i_1+1][i_0] * A[i_1][i_0-2] + A[i_1-1][i_0] + 4;
        }
    }
#pragma endscop
...
kernel_2134211325_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
