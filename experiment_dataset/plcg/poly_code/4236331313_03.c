...
void kernel_4236331313_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < min(PB_P-2, -i_0+PB_M+1); i_2++) {
                A[i_1][i_2+1][i_2-1] = C[i_1][i_2+i_0-1][i_0] * A[i_1][i_2][i_2-2] - A[i_1][i_2+2][i_2] - 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_Q, i_3); i_4++) {
            for (int i_5 = 1; i_5 < min(min(-i_3+PB_M, -i_4+PB_Q-2), i_3); i_5++) {
                for (int i_6 = 1; i_6 < min(min(min(PB_N-1, -i_4+PB_N), -i_5+PB_M), -i_3+PB_L-1); i_6++) {
                    B[i_5+1][i_6][i_4+i_3] = C[i_3][i_5+i_3][i_6] - D[i_6+i_4][i_3][i_5-1][0] * D[i_6+1][i_6+i_5][i_6+i_3+1][i_4] - E[i_5-1][i_3+1][i_4+1][i_6-1] - B[i_4][i_6][i_5+i_3-1] - B[i_6+1][i_4-1][i_5+i_3] - B[i_6][i_3][i_5+i_4+2] * B[i_6+1][i_3][i_5+i_4-1] * 6;
                }
            }
        }
    }
...
kernel_4236331313_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
