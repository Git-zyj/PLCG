...
void kernel_3336331223_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0-1] * D[i_0] + E[i_0] - F[i_0] - A[i_0-1] - 3;
    }
    for (int i_1 = 1; i_1 < PB_Q; i_1++) {
        for (int i_2 = i_1; i_2 < PB_N-1; i_2++) {
            A[i_2] = G[i_2+1][i_1] * G[i_2][i_1] * 2;
        }
        for (int i_3 = 0; i_3 < min(PB_Q, -i_1+PB_L); i_3++) {
            for (int i_4 = max(1, i_3); i_4 < PB_Q; i_4++) {
                C[i_1][i_4-1][i_3+1] = H[i_4][i_3+i_1][1] + H[i_4-1][i_1][i_3] * I[i_4][i_1-1][i_4+1] * C[i_1+1][i_4-1][i_3+3] - C[i_1+1][i_4-1][i_3] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3336331223_09(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
