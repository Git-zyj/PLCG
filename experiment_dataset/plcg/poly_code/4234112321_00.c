...
void kernel_4234112321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+2; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            A[i_1] = A[i_0-2] + A[i_1+1] * A[i_1-1] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        B[i_2][0] = C[i_2+1] + A[i_2] - B[i_2-1][0] * B[i_2-2][0] - 1;
    }
#pragma endscop
...
kernel_4234112321_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
