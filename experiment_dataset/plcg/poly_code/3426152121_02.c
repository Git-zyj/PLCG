...
void kernel_3426152121_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        A[i_0-1][1] = E[i_0] - E[i_0] - E[i_0] + 3;
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M-1; i_2++) {
                B[i_2+i_0][i_1][i_2] = F[i_1+1][i_2][i_0+2] - F[i_2+1][i_2][i_0] * G[i_0][i_2] - F[i_2-1][i_1][0] - B[i_2+i_0+1][i_1][i_2+1] + 5;
            }
        }
        for (int i_3 = 2; i_3 < i_0; i_3++) {
            C[i_0-1][i_3] = G[i_3][i_0] * G[i_3][i_0] + E[i_3-2] - G[i_3][i_0+1] * G[i_3][0] * E[i_3] * A[i_0+1][1] * 1;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_L; i_5++) {
            D[i_4+2][i_5] = G[i_5-2][i_4] - E[i_5] * E[i_4] + G[i_5][i_4] * A[i_5][1] - 3;
        }
    }
#pragma endscop
...
kernel_3426152121_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
