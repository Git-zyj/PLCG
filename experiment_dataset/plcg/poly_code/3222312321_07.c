...
void kernel_3222312321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_M, -i_0+PB_P-1); i_1++) {
            A[i_0-1][i_1] = C[i_1][i_1+i_0+1] + D[i_1][i_0] * A[i_0][i_1] + B[i_1-1][1] * B[i_0-2][1] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = PB_L-1 - 1; i_4 >= 2; i_4--) {
                B[i_4-1][1] = B[i_4][1] - B[i_4-2][1] + B[i_4+1][1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3222312321_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
