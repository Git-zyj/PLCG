...
void kernel_4212212113_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            B[i_1][i_1][i_0] = B[i_0-1][i_0-1][i_1] * 4;
        }
        for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
            for (int i_3 = 0; i_3 < -i_0+PB_P; i_3++) {
                B[i_2][i_2][i_3] = C[i_0+1][i_3+i_0][i_2+2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4212212113_09(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
