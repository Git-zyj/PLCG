...
void kernel_2412331323_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        B[i_0+2][0] = E[i_0][i_0-1] - F[i_0] * G[i_0][0] + 6;
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            B[i_0+1][0] = H[i_1][i_0-1] + E[i_1][i_0] * G[i_1+i_0][i_1-1] * B[i_0][0] * 3;
        }
        D[i_0] = I[i_0] + J[i_0] * B[i_0+1][0] * B[i_0][0] - 5;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        D[i_2] = J[0] + D[i_2+1] - D[i_2+2] + D[i_2-2] - 4;
    }
#pragma endscop
...
kernel_2412331323_03(xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
