...
void kernel_4214212315_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < min(PB_L, -i_1+PB_N+1); i_2++) {
                A[i_2+i_1-1][i_0][i_2] = C[i_2-2][i_0][i_1] + A[i_2+i_1-2][i_0+1][i_2-1] * A[i_2+i_1-2][i_0][i_2] - A[i_2+i_1-1][i_0-1][i_2] * 6;
            }
        }
        for (int i_3 = 1; i_3 < PB_N+1; i_3++) {
            for (int i_4 = 1; i_4 < min(min(-i_3+PB_N+1, i_3), PB_L+1); i_4++) {
                A[i_4+i_3-1][i_0][i_4-1] = 2;
            }
        }
    }
#pragma endscop
...
kernel_4214212315_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
