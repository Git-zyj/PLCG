...
void kernel_2426252325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] * F[0] + A[i_0+2] - A[i_0-1] * 5;
        B[i_0] = G[i_0][i_0] + H[i_0][i_0] + G[i_0][i_0] + F[i_0+2] - B[i_0-1] * B[i_0+1] * 3;
    }
    for (int i_1 = 4; i_1 < PB_M-2; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_M; i_2++) {
            A[i_1] = H[i_2+2][i_1-1] + G[i_2][i_1] - E[i_1-1] - F[i_2+2] - F[i_2+i_1] + H[i_2-2][i_1] * 3;
        }
        for (int i_3 = 2; i_3 < min(-i_1+PB_M-2, i_1); i_3++) {
            D[i_3][i_1-2] = E[i_3] * G[i_3][i_1+2] * G[i_1+2][i_3] - H[i_3][i_1] - G[i_3][i_1] * I[i_3+i_1+2][i_3][i_1] * H[i_3-1][i_1] + G[i_3-1][i_1-1] + D[i_1-1][i_3-2] + D[i_3][i_1-1] * D[i_3][i_1] + D[i_3-2][i_1-4] * 1;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2426252325_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
