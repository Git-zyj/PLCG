...
void kernel_3324252223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_L, i_0), PB_M-2); i_1++) {
            A[i_1+2][i_0] = D[i_1][i_0] - D[i_1+i_0][i_0] - E[i_0][i_1][0] * F[i_1][i_0] - D[i_1][i_0] * G[i_0+1] * A[i_1+2][i_0-1] - A[i_1+1][i_0] - 4;
        }
        for (int i_2 = i_0; i_2 < PB_M-3; i_2++) {
            A[i_2+3][i_0] = F[i_2-1][i_0-2] + D[i_2][i_0] - D[i_0][i_2] * D[i_2][i_0] - E[i_2][i_2][i_0] * 2;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            for (int i_5 = i_3; i_5 < PB_M; i_5++) {
                C[i_4][i_3][i_5] = H[i_4-1][i_3][i_5] + H[i_5][i_4-1][i_3-2] - E[i_5][i_3][i_4] + H[i_4+2][i_3][i_3] - C[i_4-1][i_3][i_5] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3324252223_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
