...
void kernel_3412252315_01(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L; i_1++) {
            C[i_1+2] = E[i_1] * F[i_1+2][i_0] - F[i_0][i_1-2] * 3;
            B[i_1][i_0] = G[i_1][i_0] + G[i_1][i_1+i_0] * G[i_1][i_0-2] + G[i_1+1][i_0] - G[i_1][i_0] - F[i_1][i_0] + G[i_0][i_1-2] - B[i_1+1][i_0] * B[i_1][i_0+1] * B[i_1-1][i_0+1] * 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            C[i_3+2] = H[i_2][i_3][i_2] * I[i_3-2] + G[i_3+1][i_2+1] - G[i_3+i_2][0] - F[i_3][i_2+1] + C[i_3+1] * 5;
        }
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = max(i_4, 2); i_5 < PB_L; i_5++) {
                C[i_5+2] = J[i_2][i_5-2][i_4] - H[i_5+2][i_4][i_2] - J[i_5][i_2][i_4+1] - H[i_5][i_2][0] - F[i_5][i_4+1] - B[i_5][i_4] * C[i_2+2] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3412252315_01(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
