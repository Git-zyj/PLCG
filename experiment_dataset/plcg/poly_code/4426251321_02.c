...
void kernel_4426251321_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_0+PB_Q, -i_1+PB_Q), -i_2+PB_P); i_3++) {
                    A[i_0][i_1][i_0][i_3+1] = E[i_2][0][i_3+i_0][i_1] + E[i_2][i_1][i_2][i_3] * E[i_2-1][i_3][i_0][i_1+1] + E[i_2-1][i_0+1][i_3][i_1-1] + E[i_1+1][i_0][i_3+i_1][i_2] + E[i_0][i_3+i_2][i_1][i_1] * E[i_2][i_3][i_0+1][i_1] + 6;
                }
            }
        }
        for (int i_4 = 2; i_4 < -i_0+PB_M; i_4++) {
            B[i_0][i_4] = F[i_0][i_4] + F[i_0+1][i_4] - F[i_4][i_0] + G[i_0-1][i_4+1] - F[i_4+i_0][i_0] * B[i_0+1][i_4+2] - B[i_0+2][i_4] + B[i_0-1][i_4-2] * B[i_0-1][i_4] * 4;
        }
        for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
            C[i_0][i_5] = G[i_5+1][i_0] - F[i_0+1][i_5-1] - C[i_0+2][i_5] - C[i_0-2][i_5] - C[i_0-2][i_5-2] - C[i_5][i_0+1] * 5;
        }
    }
    for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
        for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
            for (int i_8 = i_6; i_8 < PB_L-1; i_8++) {
                D[i_7+1][i_7-1][i_6] = H[i_6+1][i_8][i_7] * H[i_6][i_8][i_7] * F[i_6][i_7-1] + H[i_8+1][i_6-1][1] + D[i_7+1][i_7-1][i_6-2] + D[i_6+1][i_6-1][i_7-1] + 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4426251321_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
