...
void kernel_2222131113_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_N, -i_0+PB_M-1); i_1++) {
            A[i_1+i_0][0] = C[i_1][i_0-1] + C[i_0-1][0] + A[i_1+i_0+1][0] + 5;
            B[i_1-1] = C[i_1][i_0] + C[i_1][i_0] * C[i_0][0] * C[i_1-1][i_0+1] * B[i_1] - 6;
        }
    }
#pragma endscop
...
kernel_2222131113_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
