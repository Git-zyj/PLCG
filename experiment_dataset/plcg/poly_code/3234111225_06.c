...
void kernel_3234111225_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_0+1][i_1-1] = C[i_0][i_1] - A[i_0+2][i_1-1] - A[i_0-1][i_1-1] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_Q; i_3++) {
            for (int i_4 = 1; i_4 < PB_Q+1; i_4++) {
                B[i_3][i_3] = B[i_3-1][i_3-1] * B[i_4-1][i_4-1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3234111225_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
