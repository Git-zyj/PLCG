...
void kernel_3226252225_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 3; i_2 < min(min(-i_1+PB_N, -i_0+PB_N), PB_L-2); i_2++) {
                A[i_0][i_2-1][i_2] = C[i_1][i_2-2][i_0] * D[i_1][i_2+i_0][0] * C[i_1][i_2+2][i_0] + C[i_1-2][0][i_0] + D[i_2+i_1][i_2+i_0][i_1+2] - C[i_0][i_2][i_1] + A[i_0-2][i_1-1][i_1] * A[i_0+1][i_2-3][i_2-2] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_P; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_L; i_5++) {
                A[i_4-1][i_5-1][i_5] = D[i_4+1][i_5][i_4] - D[i_4][i_5-1][i_3+2] * C[i_3][i_5][i_4] * C[i_5+i_4][i_3][i_3] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3226252225_09(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
