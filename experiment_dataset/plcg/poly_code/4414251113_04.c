...
void kernel_4414251113_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            for (int i_2 = i_0; i_2 < -i_0+PB_Q; i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_Q; i_3++) {
                    A[i_1+i_0][i_3][i_3+i_2][i_2] = E[i_0+1][i_1][i_2][i_3-1] + F[i_2][i_0][i_3-1][i_2+i_0] - G[i_3][i_1+1][i_2+i_0] * H[i_2][i_3][i_0][i_1][i_0] - 2;
                }
                B[i_0][i_2-1][i_0] = G[i_1][i_2][i_0-1] + G[i_2][i_0][i_1+1] - G[i_1][i_2][i_0-1] + I[i_2][i_2][i_0] * B[i_0][i_2][i_0] * 1;
            }
        }
        C[i_0] = J[i_0+1] * J[i_0] + K[i_0] * J[i_0] + J[i_0+1] * 1;
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 2; i_5 < min(PB_L, -i_4+PB_M); i_5++) {
            D[i_4-1][i_5-1] = J[0] - L[i_5][i_4+1] * L[i_5-1][0] - L[i_5+i_4][i_4] * D[i_4-1][i_5-2] - 3;
        }
    }
#pragma endscop
...
kernel_4414251113_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
