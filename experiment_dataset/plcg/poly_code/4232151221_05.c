...
void kernel_4232151221_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = C[i_0+1][i_0] * D[i_0] + D[i_0-1] * A[i_0-2] * A[i_0+2] * 1;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1-1] = D[i_1] + D[i_1] * D[i_1] + D[i_1-1] * D[i_1] * D[i_1-1] + D[i_1+1] - B[i_1+1] - 4;
    }
#pragma endscop
...
kernel_4232151221_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
