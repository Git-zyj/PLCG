...
void kernel_3116312325_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[i_1+2][i_2][i_0][0] = B[0][i_0][i_1] * A[i_1+3][i_2][i_0][0] + A[i_2+4][i_1+2][i_0+1][0] * A[i_1+2][i_2-2][i_0+1][0] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3116312325_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
