...
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = PB_L - 1; i_1 >= 1; i_1--) {
            A[i_0][i_1] = C[i_1][0] + D[i_0-2][i_1-1] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
                B[i_2][i_3][i_4] = E[i_4+i_3][i_2][i_3] + F[i_3][i_2][i_4-1] * E[i_2+2][i_4-1][i_3] + B[i_2+2][i_3+2][i_4] * B[i_2][i_3+1][i_4+2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3222232123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
