...
void kernel_3422352325_07(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            D[i_0+2][i_1-2] = E[i_1+2][i_0] - F[i_1][i_0] * G[i_0][i_1] + G[i_1+2][i_0] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 2; i_4 < PB_L; i_4++) {
                B[i_3][i_4+1][i_2][i_4-1] = G[i_2+1][i_4] - H[i_3][i_4][i_2] * B[i_3+1][i_4+1][i_2][i_4-1] + B[i_4-2][i_3+3][i_2][i_3+1] * 4;
            }
        }
        for (int i_5 = 3; i_5 < PB_L; i_5++) {
            for (int i_6 = 1; i_6 < min(-i_2+PB_Q, PB_L-1); i_6++) {
                B[i_5][i_6+1][i_2+1][i_6-1] = I[0][i_6+1][i_2] - I[i_6+1][i_6][i_2] * I[i_5][i_6+i_2][i_2+2] * F[i_6+i_2][i_5] + J[i_2][0][i_5] - J[i_2-2][i_6+2][i_5+2] * K[i_2-1][i_6][i_5][i_6] + D[i_2+1][i_5-2] - 3;
            }
            D[i_2][i_5-2] = L[i_5] + E[i_5-2][i_2] - F[i_5+2][i_2] - F[i_5][i_2-1] + D[i_2][i_5-3] - D[i_2-1][i_5-3] * 6;
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_3422352325_07(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L)));
