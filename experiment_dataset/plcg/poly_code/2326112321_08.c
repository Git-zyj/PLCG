...
void kernel_2326112321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[0][i_0] = A[0][i_0-2] - A[0][i_0+1] * 1;
        }
        A[0][i_0] = 3;
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        C[i_2] = D[i_2] - C[i_2-2] + C[i_2-1] * C[i_2+1] * 5;
    }
#pragma endscop
...
kernel_2326112321_08(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
