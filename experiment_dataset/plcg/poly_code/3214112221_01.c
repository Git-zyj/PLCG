...
void kernel_3214112221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L+3; i_0++) {
        for (int i_1 = max(i_0, 4); i_1 < PB_L+2; i_1++) {
            A[i_0][i_1-2] = A[i_0+1][i_1-2] - A[i_1][i_0-3] * A[i_0][i_1-4] - 3;
        }
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            B[i_2-1][i_2] = C[i_0][i_2-1] + D[1] - 2;
        }
    }
#pragma endscop
...
kernel_3214112221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
