...
void kernel_3412351323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] * F[i_0-1] * E[i_0] - G[i_0] - A[i_0+1] - A[i_0-1] * 3;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        A[i_1] = F[i_1+1] * H[i_1][i_1] * I[i_1][i_1+1] * E[i_1-1] * E[i_1] - F[i_1] + E[1] * G[i_1-1] * 3;
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            C[i_2-1][i_1] = H[i_2+1][i_1+1] + J[i_1][i_2] * J[i_1+1][i_2-1] + A[i_1] - C[i_2][i_1+1] * 3;
        }
        for (int i_3 = 1; i_3 < i_1; i_3++) {
            for (int i_4 = 3; i_4 < min(-i_3+PB_L, PB_M-2); i_4++) {
                D[i_1][i_4-1][i_3] = K[i_3-1][i_1+1][i_4] + K[i_1][i_4+i_3][0] * L[i_4][i_3][i_1][i_3+1] - M[i_4+1][i_1][i_1-1] + C[i_4-1][i_1-2] + D[i_1+2][i_4-3][i_3] + D[i_4+2][i_1-1][i_3] - 3;
            }
        }
    }
#pragma endscop
