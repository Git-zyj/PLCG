...
void kernel_4212312211_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
        ...
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = i_0; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < -i_2+PB_P; i_3++) {
                    A[i_1][i_2][i_3+i_2][i_1-1] = 4;
                }
            }
        }
        for (int i_4 = 1; i_4 < -i_0+PB_N; i_4++) {
            B[i_0][i_0] = C[i_4+i_0][i_4+i_0][0] + B[i_4+1][i_4+1] * B[i_4-1][i_4-1] - B[i_0-1][i_0-1] * 5;
        }
    }
#pragma endscop
...
kernel_4212312211_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
