...
void kernel_3236212111_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_N-1, i_0); i_1++) {
            A[i_1] = C[i_1-1] * A[i_1+1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = PB_P-1 - 1; i_3 >= i_2; i_3--) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                B[i_4][i_2+2][i_3+1] = B[i_2-1][i_4+1][i_3+1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3236212111_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
