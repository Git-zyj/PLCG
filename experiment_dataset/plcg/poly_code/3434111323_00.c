...
void kernel_3434111323_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
    ...
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            for (int i_3 = 2; i_3 < i_2; i_3++) {
                B[i_2][i_2][i_3] = E[i_0+1][i_3-1][i_2] * B[i_2][i_2][i_3-2] + B[i_2][i_2][i_3+1] * 2;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = min(i_4, -i_4+PB_L) - 1; i_5 >= 1; i_5--) {
            C[i_4-1][i_5] = A[i_5-1][i_5+i_4] - C[i_4-1][i_5-1] * C[i_4-1][i_5+1] + C[i_4+1][i_5+1] + C[i_4-1][i_5+2] + 6;
        }
        D[i_4] = D[i_4-2] + D[i_4+2] + D[i_4-1] * D[i_4+1] + 5;
    }
#pragma endscop
...
kernel_3434111323_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
