...
void kernel_2226112213_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L-2, i_0); i_1++) {
            A[i_1+i_0+1] = A[i_1+i_0+2] * A[i_1+i_0+2] * 5;
        }
        for (int i_2 = 3; i_2 < i_0; i_2++) {
            B[i_2-2][1] = C[i_0][i_0] - B[i_2-3][1] - B[i_2-1][1] * 6;
        }
    }
#pragma endscop
...
kernel_2226112213_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
