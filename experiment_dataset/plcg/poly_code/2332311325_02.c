...
void kernel_2332311325_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            C[i_3][i_4-1] = C[i_3-2][i_4-1] * C[i_4+1][i_3-2] * C[i_3-1][i_4-1] - C[i_3+2][i_4-1] * 6;
        }
    }
#pragma endscop
...
kernel_2332311325_02(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
