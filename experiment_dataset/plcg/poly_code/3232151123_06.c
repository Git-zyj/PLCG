...
void kernel_3232151123_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        B[i_0+1] = C[i_0+1] * C[i_0-1] - C[i_0-1] - 1;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M-1; i_2++) {
            B[i_2] = D[i_2][i_1] * D[i_2][i_1] * D[i_1][i_2] + D[i_1][i_2] * D[i_1-1][i_2] + D[i_2+i_1+1][i_1+1] + D[1][i_2-1] + B[i_2+1] * 2;
        }
    }
#pragma endscop
...
kernel_3232151123_06(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
