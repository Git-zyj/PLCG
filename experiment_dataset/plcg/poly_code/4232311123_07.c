...
void kernel_4232311123_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, -i_1+PB_P); i_2++) {
                for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                    A[i_1][i_2][i_3+i_2] = A[i_3+1][i_1][i_2+i_1] + 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        B[i_4+1] = C[i_4+1] + B[i_4-1] - 6;
    }
#pragma endscop
...
kernel_4232311123_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
