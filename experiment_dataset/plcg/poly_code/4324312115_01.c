...
void kernel_4324312115_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[i_0][i_0] = D[i_0][0] - 4;
        A[i_0][i_0] = A[i_0][i_0] * 2;
    }
    for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
        for (int i_2 = 1; i_2 < PB_N; i_2++) {
            C[i_2][i_1+2] = C[i_2-1][i_1+3] - 1;
        }
    }
#pragma endscop
...
kernel_4324312115_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
