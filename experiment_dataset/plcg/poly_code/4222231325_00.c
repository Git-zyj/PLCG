...
void kernel_4222231325_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
        ...
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_P+1; i_3++) {
                    A[i_0][i_2][i_1+1][i_3] = C[i_2-1][i_0][i_3+i_1-1][1] * D[i_1][i_0-1][i_3][i_3] + D[i_3-1][i_0][i_2][i_1] + C[i_2+1][i_3][1][i_3] * A[i_0+2][i_2][i_1+1][i_3-1] - A[i_0][i_2][i_1][i_3] * 1;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        B[i_4-1] = E[i_4-1] + B[i_4+1] * B[i_4-2] - B[i_4] * 6;
    }
#pragma endscop
...
kernel_4222231325_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
