...
void kernel_4332111125_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < min(PB_N, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    A[i_2+1][i_3+i_1][0][i_1+1] = D[i_1][i_2][i_3][i_0] - 6;
                }
            }
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                for (int i_5 = 0; i_5 < PB_N-2; i_5++) {
                    A[i_5+2][i_1+i_0-1][0][i_1] = E[i_1+i_0-1][i_5+1][i_4][i_0-1][i_4] + 5;
                }
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M; i_6++) {
        for (int i_7 = 2; i_7 < PB_M; i_7++) {
            C[i_7] = F[i_6][i_7-1][i_7] + C[i_7-1] - C[i_7-2] * 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4332111125_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
