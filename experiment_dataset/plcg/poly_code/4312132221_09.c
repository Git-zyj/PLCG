...
void kernel_4312132221_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                A[i_1][i_0-2][i_1+2] = D[i_1+2][i_0][i_2] * D[i_2][i_0][i_2-1] * A[i_1][i_0-3][i_1+2] * B[i_0-2][i_2-2][i_0][i_1+2] - 6;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                for (int i_5 = 2; i_5 < -i_0+PB_Q; i_5++) {
                    B[i_5-2][i_3][i_5][i_4+2] = E[i_3][i_4+2][i_0-2][i_5-1] - D[i_4+1][i_3][i_5+i_0] + B[i_0-2][i_3][i_0][i_5+3] - 3;
                }
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_L; i_6++) {
        for (int i_7 = 2; i_7 < min(min(i_6, -i_6+PB_M+2), PB_L-2); i_7++) {
            C[i_6][i_7] = F[i_7+i_6-2][i_6] - F[i_7][i_6-1] - G[i_7] * F[i_7-2][i_6] - C[i_6][i_7+2] - C[i_7-1][i_6-1] + 6;
        }
    }
#pragma endscop
...
kernel_4312132221_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
