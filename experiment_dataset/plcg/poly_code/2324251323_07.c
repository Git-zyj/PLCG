...
void kernel_2324251323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-1] = D[i_0] * E[i_0] + F[i_0][1] - E[i_0+1] - E[i_0] + E[i_0+1] * D[i_0] * A[i_0-2] * 6;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_N; i_2++) {
            B[i_2+1][i_2] = G[i_2][i_2][i_1+1] - H[i_1][i_2] + H[i_2+i_1][i_2] + F[i_2][0] - F[i_2][i_1] - B[i_2][i_2-1] + 5;
        }
        A[i_1-1] = D[0] * E[i_1-1] * D[i_1] + A[i_1-1] - A[i_1] - B[i_1+1][i_1] * 2;
    }
#pragma endscop
...
kernel_2324251323_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
