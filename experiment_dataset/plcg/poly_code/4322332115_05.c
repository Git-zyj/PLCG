...
void kernel_4322332115_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, i_0); i_2++) {
                A[i_2+i_1][i_0][i_1] = A[i_2+i_1-1][i_0-1][i_1] + 1;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                for (int i_6 = 1; i_6 < -i_3+PB_M-1; i_6++) {
                    A[i_6+i_3+1][i_4][i_6] = D[i_4][i_3][i_6][i_5] * E[i_6][i_4][i_3-1][i_3] * D[i_5][i_4][i_6][i_3] + E[i_5+1][i_3][i_3][i_4] - E[i_6-1][i_5][i_3][i_6] - 2;
                }
            }
        }
        for (int i_7 = 2; i_7 < -i_3+PB_M; i_7++) {
            for (int i_8 = 2; i_8 < PB_L; i_8++) {
                for (int i_9 = 0; i_9 < PB_M; i_9++) {
                    A[i_7+i_3][i_8][i_3] = F[i_9][i_7-2][i_3][i_8] + F[i_8][i_7][i_3][i_8-2] - 4;
                }
            }
        }
    }
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4322332115_05(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F)));
