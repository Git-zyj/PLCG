...
void kernel_3324112211_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_2][i_1-1][2] = D[i_2+2][i_0+1][i_1+2] * 1;
            }
        }
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < i_0; i_4++) {
                B[i_0][i_3][i_4] = D[i_0+1][i_4][i_3] * D[i_0][i_4][0] * B[i_0+1][i_3][i_4] * B[i_0+1][i_3-1][i_4+1] - B[i_0+1][i_3-1][i_4] * 4;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 1; i_6 < -i_5+PB_L-1; i_6++) {
            for (int i_7 = max(1, i_5); i_7 < PB_L; i_7++) {
                C[i_7][i_6+i_5][0] = A[i_7][i_6-1][2] - C[i_7-1][i_6+i_5+1][0] - C[i_7][i_6+i_5+1][0] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3324112211_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
