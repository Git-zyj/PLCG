...
void kernel_2212132325_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-3; i_1++) {
            B[i_1+3][i_1+2] = C[i_1][i_0] + D[0] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-3; i_2++) {
        B[i_2+1][i_2] = D[i_2] * D[i_2] - D[i_2] - B[i_2+3][i_2+2] * B[i_2-1][i_2-2] * B[i_2][i_2-1] - 2;
    }
#pragma endscop
...
kernel_2212132325_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
