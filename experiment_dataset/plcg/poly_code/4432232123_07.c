...
void kernel_4432232123_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] + F[i_0] - E[i_0-2] + 4;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        B[i_1] = F[i_1+1] + E[i_1+1] - E[i_1] * 6;
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            C[i_1+1][i_2] = G[i_2+1][i_1] - H[i_1][i_2][i_1] + G[i_2-1][i_1-2] * G[i_1][i_2] - 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q; i_5++) {
                D[i_3+2][i_5][i_4+2] = I[i_5][i_5][i_4] - D[i_3][i_5][i_4+1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4432232123_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
