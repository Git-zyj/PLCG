...
void kernel_4334332115_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_M, i_0), -i_1+PB_Q); i_2++) {
                for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
                    A[i_0][i_0][i_2][i_2] = D[i_2][i_0][i_3][i_1] + D[i_2+i_0][i_1+1][i_3][0] - E[i_2][i_0][i_2+i_1][i_0] + A[i_3-1][i_3-1][i_2][i_2] - A[i_1+1][i_1+1][i_2][i_2] + 5;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        B[i_4+1] = F[i_4+1] * G[i_4] * H[i_4] * H[i_4] + 5;
    }
    for (int i_5 = 0; i_5 < PB_Q-1; i_5++) {
        for (int i_6 = 0; i_6 < PB_L; i_6++) {
            for (int i_7 = i_5; i_7 < PB_L; i_7++) {
                for (int i_8 = i_7; i_8 < -i_5+PB_Q-1; i_8++) {
                    A[i_6][i_6][i_5][i_5] = E[i_7][i_6][i_5+1][i_8+i_5+1] * 3;
                }
            }
        }
    }
...
kernel_4334332115_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
