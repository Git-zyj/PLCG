...
void kernel_3124111325_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M-2; i_2++) {
                A[i_2][i_1][i_0-1] = B[i_2][i_0][i_1] + A[i_2+2][i_1][i_0-1] + A[i_1][i_2][i_0+1] * A[i_2][i_1-2][i_0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3124111325_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
