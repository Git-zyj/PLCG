...
void kernel_2412252323_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L-1; i_1++) {
            A[i_1+i_0][i_1+i_0] = E[i_1][i_0] * A[i_1+i_0-1][i_1+i_0-1] - A[i_1+i_0+1][i_1+i_0+1] * A[i_1+i_0-2][i_1+i_0-2] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_L+1; i_3++) {
            B[i_3+i_2] = E[i_3-2][i_3-2] - E[i_3+i_2][i_3] - F[i_3][i_2] * G[i_3+i_2-1] + F[i_3][i_2] * B[i_3+i_2-1] + B[i_3+i_2] + 6;
            B[i_3+i_2-1] = E[i_3][i_2] - G[i_3] * F[i_3+i_2][i_3] * F[i_3-1][i_2] - F[i_3][0] + G[0] * F[i_3][i_3] + F[i_3+1][0] + A[i_3+i_2-1][i_3+i_2-1] * 1;
        }
        for (int i_4 = 2; i_4 < -i_2+PB_L; i_4++) {
            A[i_4+i_2][i_4+i_2] = F[i_4+2][i_2] + E[i_2][i_4-2] * F[i_4][i_4+1] + F[i_2][i_2-1] - 1;
        }
    }
#pragma endscop
...
kernel_2412252323_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
