...
void kernel_2336211221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-2; i_1++) {
            A[i_0][i_1] = A[i_0][i_1-2] + A[i_0][i_1+2] + 4;
        }
        for (int i_2 = 0; i_2 < min(PB_P, -i_0+PB_L); i_2++) {
            B[i_2+i_0][i_2] = D[i_0][i_2+1] - A[i_0+2][i_2] + 2;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
        C[i_3+1] = E[0] * C[i_3] - C[i_3+2] * C[i_3+3] - 2;
    }
#pragma endscop
...
kernel_2336211221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
