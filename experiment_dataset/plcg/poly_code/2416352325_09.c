...
void kernel_2416352325_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            A[i_0-2][i_1] = E[i_1][i_1+i_0] * F[i_1] - G[i_1][i_0] + G[i_0][i_1-2] - G[i_0][i_1] - 4;
            A[i_0-2][i_1] = H[i_1-1] + I[i_1] * G[i_1][0] * A[i_0-2][i_1-1] + A[i_0-2][i_1] - 6;
        }
        for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
            A[i_0-4][i_2] = E[i_2][i_0-1] + E[i_2+2][i_0] - 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M; i_4++) {
            A[i_4][i_3] = I[i_4+i_3] + G[i_4][i_3-2] * J[i_4+i_3+2][1] * E[i_4][i_3] + F[i_4+i_3+1] - I[i_4] - J[i_4+i_3][i_3] + E[i_4+i_3][i_4+2] - E[i_4+2][i_3] + A[i_3-2][i_4] * 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2416352325_09(xa,ya,POLYBENCH_ARRAY(A), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
