...
void kernel_4222252325_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M+2, PB_M-2); i_1++) {
            B[i_1+2] = C[i_0] - D[i_1][i_0] * D[i_1][i_0] + C[i_1+i_0-2] + D[i_1][i_0] - B[i_1+2] + 2;
        }
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            B[i_2+2] = E[i_2+1][0] + D[i_2][i_0] - D[i_2][i_0] + E[i_2-1][i_0-2] - D[i_2][1] * B[i_2+1] * B[i_2+3] + 5;
        }
    }
#pragma endscop
...
kernel_4222252325_04(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
