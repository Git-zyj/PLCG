...
void kernel_2336312211_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_P-3; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            C[i_4+2][i_3+2] = D[i_4] - C[i_4+1][i_3+1] - C[i_4+2][i_3+1] * C[i_4+3][i_3+2] * C[i_4+1][i_3+3] * 6;
        }
    }
#pragma endscop
...
kernel_2336312211_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
