...
void kernel_3236212113_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0 - 1; i_1 >= 2; i_1--) {
            for (int i_2 = 2; i_2 < min(PB_M+1, -i_0+PB_L+2); i_2++) {
                A[i_2-2][i_1-2] = C[i_1][i_2+i_0-2] - A[i_2-1][i_1-2] * 2;
            }
        }
...
kernel_3236212113_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
