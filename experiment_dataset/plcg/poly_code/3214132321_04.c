...
void kernel_3214132321_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(min(i_0, PB_M), -i_0+PB_P-2); i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, PB_M-2); i_2++) {
                A[i_2][i_0][0] = C[i_2+2][i_0][i_1+i_0+2] - A[i_1][i_0-1][0] - A[i_2+1][i_0+2][0] + A[i_2+2][i_0+1][0] - 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < -i_3+PB_P-2; i_4++) {
            A[i_3][i_4+2][0] = D[i_4][i_3] - D[i_3+1][i_4] + D[i_4][i_3] * D[i_4-1][i_4+i_3+2] - 6;
        }
    }
#pragma endscop
...
kernel_3214132321_04(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
