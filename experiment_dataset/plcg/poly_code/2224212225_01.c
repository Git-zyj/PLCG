...
void kernel_2224212225_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_N; i_1++) {
            A[i_1-2] = A[i_1] + A[i_1-1] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M+2; i_2++) {
        A[i_2-2] = C[i_2] - D[i_2-2] - 4;
    }
#pragma endscop
...
kernel_2224212225_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
