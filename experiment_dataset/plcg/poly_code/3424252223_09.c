...
void kernel_3424252223_09(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        C[i_0] = E[1] + E[i_0+2] + F[i_0] + E[i_0] - 6;
    }
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        C[i_1-1] = F[i_1] + G[i_1][0] * G[i_1-1][0] * E[i_1+2] + F[i_1] + C[i_1-2] + 1;
    }
    for (int i_2 = 2; i_2 < PB_Q-1; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_Q, -i_2+PB_N); i_3++) {
            C[i_3] = G[i_3+i_2][1] * G[i_3][i_2-2] - 4;
            for (int i_4 = 0; i_4 < min(PB_Q, -i_2+PB_M); i_4++) {
                C[i_3] = H[i_2-2][i_4][i_3-2] + H[i_2][i_4][0] + H[i_4+i_2][0][i_3] * H[i_4][i_2][i_3-1] * I[i_4][i_3][i_2+1] - H[i_4][i_3][i_2+1] - C[i_3] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3424252223_09(xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
