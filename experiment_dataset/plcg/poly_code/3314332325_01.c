...
void kernel_3314332325_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_0] = D[i_0+2][i_0] + E[i_1+i_0][0] * E[i_1][i_0+1] + D[i_1-1][i_0+2] * A[i_0-1][i_0-1] + A[i_0-2][i_0-2] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                A[i_4][i_4] = F[i_4][i_3+1] - G[i_3][i_4][i_2] - H[i_4][i_2][i_3] * 5;
            }
        }
        for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 0; i_6 < PB_N; i_6++) {
                A[i_5+2][i_5+2] = A[i_2][i_2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3314332325_01(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
