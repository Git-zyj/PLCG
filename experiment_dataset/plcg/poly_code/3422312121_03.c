...
void kernel_3422312121_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_5 = 2; i_5 < PB_L; i_5++) {
        for (int i_6 = 0; i_6 < i_5; i_6++) {
            D[i_6][i_5] = D[i_6][i_5-2] - D[i_6+2][i_5] - 4;
        }
    }
#pragma endscop
...
kernel_3422312121_03(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
