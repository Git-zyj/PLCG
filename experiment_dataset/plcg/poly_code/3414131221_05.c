...
void kernel_3414131221_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_0-1][i_1] = E[i_1-1] * E[i_1+1] - E[i_1] * A[i_0+1][i_1] - A[i_0][i_1] - 1;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_L-1); i_2++) {
            B[i_0][i_0] = F[i_2][i_2+1] * F[i_2+i_0][i_2] * F[i_2+1][i_0] + B[i_0-2][i_0-2] - B[i_0+1][i_0+1] + 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_L-1; i_4++) {
            A[i_4+1][i_3] = E[i_4+1] - F[i_3+1][i_3-1] - F[i_4][i_3] * F[i_4+i_3][i_3+1] * B[i_4][i_4] + 5;
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                D[i_4][i_5][i_3] = G[i_5-1][i_4+i_3+1][0] - F[i_3][i_5] * B[i_4+1][i_4+1] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3414131221_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
