...
void kernel_4424312221_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < PB_L; i_1++) {
            for (int i_2 = i_1; i_2 < -i_1+PB_L; i_2++) {
                A[i_2][i_2+i_1][i_0] = A[i_2-1][i_2+i_1-3][i_0-1] - 5;
            }
        }
        B[i_0] = D[1] * D[1] * 3;
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 4; i_4 < i_0; i_4++) {
                C[i_3][i_3][i_4-2] = C[i_3][i_3][i_4-4] + C[i_4][i_4][i_0-4] * 5;
            }
        }
        D[1] = E[i_0] + B[i_0] - D[1] - 3;
    }
#pragma endscop
...
kernel_4424312221_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
