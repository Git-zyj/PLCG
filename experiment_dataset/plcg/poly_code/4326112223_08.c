...
void kernel_4326112223_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            ...
            for (int i_3 = 0; i_3 < i_1; i_3++) {
                B[i_0][i_1][i_3] = D[i_0][i_1-2][i_1-2] + B[i_1+1][i_0][i_3] * B[i_0][i_1-2][i_3] - 4;
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_P+2; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M; i_5++) {
            C[i_5][i_4-2] = C[i_5-1][i_4-2] * C[i_5][i_4-3] + 5;
        }
    }
#pragma endscop
...
kernel_4326112223_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
