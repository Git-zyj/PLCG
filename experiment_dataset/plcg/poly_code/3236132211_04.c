...
void kernel_3236132211_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_1][i_0] = C[i_2][i_1+2] - D[i_2+1][i_0][i_1+1] + A[i_2][i_1-1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_P, PB_L-3); i_4++) {
            for (int i_5 = 1; i_5 < min(-i_3+PB_P, PB_L); i_5++) {
                B[i_3][i_4+2][i_5] = D[i_4][i_5+i_3][i_5] - D[i_5][i_4+i_3][i_3] * D[i_4][i_5][i_5+i_3] + D[i_3][i_4+2][i_5] * B[i_3][i_4+3][i_5] + B[i_3+1][i_4+3][i_5-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3236132211_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
