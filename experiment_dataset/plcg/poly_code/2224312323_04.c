...
void kernel_2224312323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 4; i_1 < PB_P+1; i_1++) {
        for (int i_2 = 4; i_2 < min(PB_P+2, -i_1+PB_L+1); i_2++) {
            B[i_1][i_2-1][i_1-2] = C[i_2+i_1-1][i_2-1] * C[i_2-1][i_1] * B[i_2-2][i_1-1][i_2-4] * B[i_2-1][i_1-1][i_2-3] - B[i_1][i_2-3][i_1-2] + B[i_1+1][i_2-1][i_1-1] + B[i_2][i_1+1][i_2-2] * B[i_1-2][i_2-1][i_1-4] * 5;
        }
    }
#pragma endscop
...
kernel_2224312323_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
