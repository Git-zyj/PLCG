...
void kernel_4324132223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_0] = D[0][i_0] + E[i_0] - D[i_1+2][i_0] - D[i_0][i_1] + A[i_0-2][i_0-2] - A[i_0+1][i_0+1] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < i_2; i_4++) {
                A[i_2][i_2] = F[i_3][i_4][i_2] + 1;
            }
        }
    }
        ...
        for (int i_6 = 0; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < PB_Q; i_7++) {
                for (int i_8 = 2; i_8 < PB_Q-1; i_8++) {
                    A[i_8][i_8] = G[i_6][i_8-1][i_8+1][i_5] * G[i_6][i_8-2][i_7][i_5] - 5;
                }
            }
        }
    }
...
kernel_4324132223_01(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
