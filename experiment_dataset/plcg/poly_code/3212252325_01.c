...
void kernel_3212252325_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_N, -i_0+PB_L); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_Q, -i_0+PB_L); i_2++) {
                A[i_0][i_1][i_2] = C[i_0+2][i_2+i_1-2][i_0-2] - C[i_2][i_1][1] * C[i_2][i_0+1][i_1+i_0] * C[i_0][i_1][0] + D[i_1][i_2][i_0] - A[i_0][i_1+1][i_2] - A[i_0+1][i_1+2][i_2-1] + B[i_2+i_0][i_0][i_2] * 4;
            }
        }
        for (int i_3 = 1; i_3 < min(PB_P-2, -i_0+PB_L-3); i_3++) {
            B[i_3+i_0][i_0][i_3] = D[i_0-1][i_3-1][i_0] + E[i_3][2] - E[i_3+2][i_0+2] + D[i_3][i_0][2] * E[i_3+2][i_0+1] - B[i_3+i_0+3][i_3+2][i_0+1] - B[i_3+i_0+1][i_0+1][i_3] + 1;
        }
    }
#pragma endscop
...
kernel_3212252325_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
