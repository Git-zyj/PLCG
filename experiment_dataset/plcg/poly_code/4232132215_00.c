...
void kernel_4232132215_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = i_0; i_2 < PB_L; i_2++) {
                B[i_1][i_2] = C[i_1+2][i_0-1][i_0-2] - 1;
            }
        }
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            B[i_0][i_3-1] = D[i_3][i_0] + E[i_0] * D[i_3][i_0] + E[i_3-1] + B[i_0-1][i_3] + B[i_0][i_3-2] + 3;
        }
    }
#pragma endscop
...
kernel_4232132215_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
