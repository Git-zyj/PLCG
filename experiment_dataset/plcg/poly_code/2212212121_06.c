...
void kernel_2212212121_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M+1, -i_0+PB_Q-1); i_1++) {
            A[i_1-1][i_0-2][i_1+i_0+1] = C[i_1][i_0] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_M+1, -i_2+PB_Q-2); i_3++) {
            B[i_2+2][i_3+i_2] = A[i_3-1][i_2-2][i_3+i_2+1] - B[i_2+2][i_3+i_2+2] - 2;
        }
    }
#pragma endscop
...
kernel_2212212121_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
