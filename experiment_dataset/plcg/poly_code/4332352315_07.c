...
void kernel_4332352315_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 3; i_2 < PB_L+1; i_2++) {
                for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                    A[i_0-2][i_1][i_3][i_2] = D[i_3][i_1][i_3+1][i_0+2] * E[i_2-2][i_3][i_0] * D[i_2-1][i_0-1][i_0][i_1] * F[i_0][i_3+2][i_3][i_2] + A[i_2-3][i_1][i_0+1][i_3+1] * A[i_0-3][i_1][i_3+1][i_2] * A[i_2-2][i_1][i_3][i_0+1] + A[i_0-2][i_1][i_3-1][i_2] - A[i_0-1][i_1][i_3][i_2] * A[i_3-1][i_0][i_1][i_2] + 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        B[i_4] = G[i_4] * H[i_4] + H[i_4+1] - I[i_4] * I[i_4-1] + G[i_4] * I[i_4+1] + G[i_4] * 1;
        for (int i_5 = 0; i_5 < PB_Q-2; i_5++) {
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                for (int i_7 = 2; i_7 < -i_6+PB_L; i_7++) {
                    A[i_7-2][i_5][i_4][i_6] = D[i_7+i_6][i_4-1][i_4][i_5+2] + F[i_6][i_7][i_4][i_5] * 4;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4332352315_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
