...
void kernel_2334152115_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_1+2] = D[i_0][i_1] - D[i_0-2][i_1] * D[i_1][i_0] - E[i_1-2] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        C[i_2-2][2] = E[1] * E[i_2] - E[i_2] - D[i_2][0] + E[i_2] - C[i_2-1][2] + 1;
        for (int i_3 = PB_L - 1; i_3 >= 0; i_3--) {
            C[i_2-1][2] = D[i_3+2][i_2] + E[i_3] - D[i_3][i_2] - D[i_3+2][i_2] - 5;
        }
    }
#pragma endscop
...
kernel_2334152115_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
