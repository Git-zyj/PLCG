...
void kernel_4222112313_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-3; i_1++) {
            A[i_0-1][i_1+2] = A[i_0-2][i_1+2] + A[i_0-1][i_1+3] * 6;
        }
        for (int i_2 = i_0; i_2 < PB_P; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                B[i_0+2][i_2+2][i_3] = C[i_3][i_2][i_0] - B[i_0+3][i_3+2][i_2] + B[i_0+3][i_2+2][i_3] - B[i_3+2][i_2+3][i_0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4222112313_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
