...
void kernel_4322151115_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_P-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
                    A[i_2+1][i_1][i_3+1][1] = D[i_0][i_2][i_1+1][i_3-1] + D[i_2][i_1][i_0+1][i_2] * D[i_3][i_1+i_0][i_2][i_3] * D[i_1][i_0+1][i_3+1][i_1] - 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_M; i_5++) {
            B[i_5-1] = E[i_5+1][i_5-1] * E[i_5][i_4] * E[i_5][i_4] * E[i_4][0] + E[i_4+1][i_5+1] * E[i_5][i_4] + E[i_5+i_4-1][0] * F[i_5+i_4][i_5+1][i_4] - E[i_5+1][i_5] * B[i_5] * 5;
        }
        for (int i_6 = 1; i_6 < PB_M; i_6++) {
            for (int i_7 = i_6; i_7 < -i_4+PB_P-1; i_7++) {
                A[i_6][i_4][i_7+1][1] = F[i_6][i_7+i_4+1][i_6-1] + F[i_7][i_6][i_4+1] - A[i_7+1][i_4][i_6+1][1] - 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4322151115_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
