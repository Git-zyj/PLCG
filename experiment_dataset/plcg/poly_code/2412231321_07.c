...
void kernel_2412231321_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0+1] + F[i_0] - A[i_0+2] * A[i_0-1] * A[i_0+1] * A[i_0-2] * 1;
    }
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < PB_N; i_2++) {
            B[i_1][i_1+1] = E[i_2] * G[i_1+1][i_2] - G[i_1][i_2] + G[i_1-1][i_2] * A[i_2+1] * B[i_1-1][i_1] - 3;
        }
        D[i_1-2] = F[i_1] - E[i_1] * E[i_1-1] * E[i_1] - 1;
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            D[i_1] = H[i_1][i_3][i_3] - I[i_1][i_3] - D[i_1+2] - D[i_3-1] - D[i_3-2] - 2;
        }
    }
#pragma endscop
...
kernel_2412231321_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
