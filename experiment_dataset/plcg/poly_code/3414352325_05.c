...
void kernel_3414352325_05(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 2; i_2 < min(min(-i_0+PB_P+1, PB_L), -i_1+PB_M-2); i_2++) {
                C[i_2+i_1+2][0] = E[i_0][i_2][i_1] - F[i_1+2][i_0][i_2+i_0-1] + G[i_2][i_1] + H[i_1+1][i_2-2][i_0-2] + 3;
            }
            for (int i_3 = 0; i_3 < min(-i_0+PB_M, PB_L-2); i_3++) {
                C[i_3+i_0][0] = H[i_0][0][i_3] + H[i_3][i_1][i_1+2] * I[i_1-1][i_3][i_0][0] - E[i_3+1][i_0][i_1] * J[i_0][i_3+2] - 5;
            }
        }
        for (int i_4 = 1; i_4 < min(i_0, -i_0+PB_M-5); i_4++) {
            C[i_4+i_0+2][0] = K[i_0+1][i_4] - G[i_4+i_0][i_0+2] + J[i_4-1][i_0+1] - C[i_4+i_0+4][0] * C[i_4+i_0][0] * C[i_4+i_0+3][0] - C[i_4+i_0+5][0] * 5;
        }
    }
    for (int i_5 = 4; i_5 < PB_M-2; i_5++) {
        D[i_5-2] = L[i_5+2] * M[i_5-1] + N[i_5-1] - M[i_5] * N[i_5] + L[i_5] + N[0] * L[i_5+1] * D[i_5-1] * D[i_5-4] + 1;
    }
#pragma endscop
