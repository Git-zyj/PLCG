...
void kernel_3236312313_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            A[0][i_0][i_1] = A[0][i_1][i_0-1] - A[0][i_1+1][i_0-1] - A[0][i_0][i_1-1] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            for (int i_4 = 1; i_4 < PB_P+1; i_4++) {
                A[0][i_3-1][i_2] = C[i_3-2][i_2][i_4-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3236312313_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
