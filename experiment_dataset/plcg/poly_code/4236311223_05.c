...
void kernel_4236311223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        A[i_0+1][i_0-1] = A[i_0][i_0-2] - A[i_0+3][i_0+1] + 5;
    }
    for (int i_1 = 3; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            for (int i_3 = 1; i_3 < PB_Q; i_3++) {
                B[i_2-1][i_2+i_1-1][i_3+1] = C[i_2+i_1][i_3-1][0] - B[i_2-1][i_2+i_1-3][i_3+1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4236311223_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
