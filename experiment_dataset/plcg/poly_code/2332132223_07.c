...
void kernel_2332132223_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-2; i_1++) {
            A[i_0+1][i_1+1] = D[i_0][i_1] + D[i_1][i_0-1] - D[i_1][0] * A[i_0+1][i_1+2] * A[i_0-1][i_1+1] + B[i_0-1][i_1] - 4;
        }
        for (int i_2 = 0; i_2 < min(min(PB_N, -i_0+PB_M), i_0); i_2++) {
            B[i_0-1][i_2] = D[i_2][i_0] + D[i_2][i_0] + D[i_2+i_0][0] * E[i_2] - B[i_2][i_0] * 6;
        }
    }
    for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 2; i_4 < -i_3+PB_L-2; i_4++) {
            C[i_3][i_4+i_3] = D[i_3+1][i_4] - C[i_3+1][i_4+i_3+2] * C[i_4-2][i_4+i_3-2] + 1;
        }
    }
#pragma endscop
...
kernel_2332132223_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
