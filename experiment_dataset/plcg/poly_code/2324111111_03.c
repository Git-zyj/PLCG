...
void kernel_2324111111_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        B[i_1-1] = D[i_1] - B[i_1] - B[i_1-2] - 6;
    }
    for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            C[i_3-1][i_2-1] = C[i_3-1][i_2-2] + 2;
        }
    }
#pragma endscop
...
kernel_2324111111_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
