...
void kernel_2334112321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0+1] = D[i_0][i_0] + A[i_0-1] - A[i_0+2] * 2;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_L, i_1); i_2++) {
            B[i_1][i_2] = B[i_1+1][i_2] * B[i_1-2][i_2] - 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_N, i_3); i_4++) {
            C[i_3-2][i_4] = D[i_3+1][i_4] + D[i_4+i_3][0] + C[i_3-2][i_4-2] + C[i_3-2][i_4+1] + C[i_3-2][i_4+2] + 4;
        }
    }
#pragma endscop
...
kernel_2334112321_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
