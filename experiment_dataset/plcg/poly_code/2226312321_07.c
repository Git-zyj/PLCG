...
void kernel_2226312321_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_L-2; i_1++) {
            A[i_0][i_1-1][i_1] = A[i_0][i_1+1][i_1+2] * A[i_0][i_1-3][i_1-2] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-3; i_2++) {
        B[i_2+2] = C[i_2+2] * B[i_2+3] + B[i_2] + B[i_2+1] - 3;
    }
#pragma endscop
...
kernel_2226312321_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
