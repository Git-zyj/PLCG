...
void kernel_4416331325_03(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        D[i_0] = E[1] * F[i_0] + E[i_0+1] * 1;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_Q, PB_P); i_3++) {
                    C[i_0][i_2+2][i_3][i_1+1] = G[i_1][i_2-1][i_3+i_0][i_0+1] - H[i_1][i_2][i_0][i_3] * 3;
                }
            }
        }
    }
        ...
        for (int i_5 = i_4; i_5 < PB_P-2; i_5++) {
            for (int i_6 = max(2, i_5); i_6 < PB_M-2; i_6++) {
                for (int i_7 = 1; i_7 < -i_6+PB_Q; i_7++) {
                    C[i_6][i_5][i_4][i_7-1] = G[i_6][i_5][i_7][i_4] * G[i_4+1][i_5-1][i_7][i_6-1] + G[i_4][i_5][i_7+i_6][i_7] - C[i_4][i_7+1][i_5+2][i_6-1] - C[i_6+2][i_5][i_4][i_7] - C[i_7][i_5+1][i_6-2][i_4-1] * 4;
                }
            }
        }
        D[i_4] = E[i_4-1] - F[i_4] - D[i_4+1] + D[i_4+2] + 2;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4416331325_03(xd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
