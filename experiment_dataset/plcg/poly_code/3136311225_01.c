...
void kernel_3136311225_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_N - 1; i_0 >= 0; i_0--) {
        for (int i_1 = max(1, i_0); i_1 < PB_N-2; i_1++) {
            A[i_1] = B[i_1] + A[i_1-1] * A[i_1+2] + 5;
        }
    }
#pragma endscop
...
kernel_3136311225_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
