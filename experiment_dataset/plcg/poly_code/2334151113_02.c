...
void kernel_2334151113_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0-1] = D[i_0] + D[i_0] - D[i_0] * A[i_0] + 5;
    }
    for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
        A[i_1-2] = D[i_1-1] + D[i_1] * D[i_1+1] * 6;
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        C[i_2] = D[i_2] * D[i_2+1] + D[i_2-1] + D[i_2] - D[i_2] * D[i_2] + D[i_2] * C[i_2+1] - 3;
    }
#pragma endscop
...
kernel_2334151113_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
