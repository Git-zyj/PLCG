...
void kernel_2436151115_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            B[i_0+1] = E[i_1][i_0] - E[i_1][i_0] * F[i_0-1] * E[i_1][i_0+1] - F[i_1] * E[i_0][0] + E[i_0-1][i_1] + E[i_1+i_0][i_1-1] + F[i_1-1] * 3;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M-1; i_2++) {
            B[i_2] = E[i_0][i_2-1] + E[i_2+i_0+1][i_0-1] + E[i_2-1][i_0] - 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            D[i_4-1] = E[i_4][i_3] + E[i_4-1][i_3] + E[i_4][i_3-1] * F[i_4+1] * E[i_3][i_4] + E[i_4+1][i_3] * E[i_3][i_3] * 1;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
        D[i_5] = F[i_5+1] * D[i_5-1] * 2;
    }
#pragma endscop
...
kernel_2436151115_01(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
