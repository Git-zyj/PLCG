...
void kernel_4214312315_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-3; i_2++) {
                A[i_1][i_0+1][i_2+2] = A[i_1][i_0+1][i_2+3] - 2;
            }
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[i_3][i_0] = C[i_3][i_0+1] + B[i_0+1][i_3] * B[i_3][i_0+1] + B[i_3+1][i_0] + B[i_3-1][i_0] + 5;
        }
    }
#pragma endscop
...
kernel_4214312315_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
