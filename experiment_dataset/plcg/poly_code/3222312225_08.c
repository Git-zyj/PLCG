...
void kernel_3222312225_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_1][i_2+1] = A[i_1-1][i_2+2] + A[i_1][i_2] + 3;
            }
        }
    }
    for (int i_3 = PB_M - 1; i_3 >= 1; i_3--) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M+1, PB_L-1); i_4++) {
            A[i_3][i_4+1] = C[i_4+i_3-1][i_4] - 4;
        }
    }
#pragma endscop
...
kernel_3222312225_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
