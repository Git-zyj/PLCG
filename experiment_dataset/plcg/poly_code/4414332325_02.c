...
void kernel_4414332325_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
        ...
        for (int i_1 = i_0; i_1 < PB_P+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = i_0; i_3 < -i_1+PB_P+1; i_3++) {
                    A[i_2][i_3+i_1-1][i_3][i_0+2] = 4;
                }
            }
        }
        for (int i_4 = 0; i_4 < min(-i_0+PB_M, PB_P-2); i_4++) {
            for (int i_5 = i_0; i_5 < PB_L-2; i_5++) {
                B[i_0+1][i_5][i_4] = E[i_4][i_5+2][i_0] - E[i_4+i_0][i_5][0] * B[i_0-1][i_5][i_4] + B[i_5+3][i_0+1][i_4+2] * B[i_0][i_5][i_4] - B[i_0+1][i_5][i_4+1] - 1;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_L; i_6++) {
        for (int i_7 = 0; i_7 < PB_L; i_7++) {
            for (int i_8 = 2; i_8 < PB_M; i_8++) {
                for (int i_9 = 2; i_9 < min(-i_8+PB_P+2, -i_6+PB_P); i_9++) {
                    A[i_6-2][i_9+i_8-2][i_9-1][i_7+1] = F[i_6-2][i_6][i_9-1][i_7] + G[i_8][i_7][i_9-2][i_9+i_6] - 6;
                }
            }
        }
        for (int j_0 = 2; j_0 < -i_6+PB_L; j_0++) {
            D[j_0][i_6+1] = H[j_0][i_6+1] - I[j_0][i_6] * H[j_0+i_6+1][j_0+1] - I[j_0+i_6][2] + H[i_6][j_0] - J[j_0][i_6] * D[j_0+1][i_6+1] + D[j_0-1][i_6+1] * D[j_0-2][i_6+1] - D[j_0][i_6+2] + D[j_0+1][i_6] * 2;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4414332325_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
