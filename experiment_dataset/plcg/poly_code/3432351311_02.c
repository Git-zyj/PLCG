...
void kernel_3432351311_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_M, -i_0+PB_P-1); i_2++) {
                A[i_1][i_2] = E[i_2+1][i_2+1][i_1] * E[i_1][i_2+i_0+1][i_1+1] + F[i_1][i_2] - G[i_2][i_1] + H[i_1-1][i_2][i_0] - A[i_1+1][i_2-1] + A[i_2][i_1-1] * A[i_1-1][i_2-1] + 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_M-1; i_5++) {
                B[i_3][i_4][i_5] = H[i_3][i_4][i_5] + H[i_5][i_4+1][i_3] - E[i_3][i_5][i_4] - I[i_5+i_4+1][0][i_3] - B[i_3][i_4-1][i_5] + B[i_5][i_4][i_3+1] + B[i_3][i_5-1][i_4] * 1;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M+1; i_6++) {
        for (int i_7 = 0; i_7 < min(i_6, PB_Q-1); i_7++) {
            for (int i_8 = 1; i_8 < PB_M; i_8++) {
                C[i_6][i_8][i_7+1] = J[i_8][i_7] + I[i_6-1][i_7][i_8-1] + C[i_6-1][i_8][i_7+1] * 1;
            }
        }
    }
    for (int i_9 = 1; i_9 < PB_N; i_9++) {
        for (int j_0 = 1; j_0 < min(PB_N, -i_9+PB_M+1); j_0++) {
            A[j_0][i_9-1] = J[j_0+i_9-1][0] * K[j_0] * K[i_9] * L[0] + F[j_0-1][i_9] - G[i_9][j_0-1] - 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_3432351311_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L)));
