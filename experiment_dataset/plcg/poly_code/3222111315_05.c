...
void kernel_3222111315_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < PB_M-1; i_2++) {
                B[i_1+1][i_2-1] = B[i_1][i_2+1] - 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
            B[i_4][i_3] = C[i_4+1][i_3] + B[i_4+1][i_3] * B[i_4+1][i_3-1] + 5;
        }
    }
#pragma endscop
...
kernel_3222111315_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
