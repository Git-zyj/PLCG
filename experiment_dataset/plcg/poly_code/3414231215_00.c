...
void kernel_3414231215_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0+1] * F[i_0-1] + F[i_0] * F[i_0] * F[i_0] * F[i_0] - A[i_0+1] - 3;
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            A[i_0+1] = G[i_1-1][i_0-1] + H[i_1][i_0] - A[i_1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            for (int i_4 = 1; i_4 < PB_M; i_4++) {
                C[i_3+1][i_4-1][i_2+1] = C[i_3+1][i_4][i_2+1] * C[i_3][i_4-1][i_2+1] * 1;
            }
        }
        A[i_2] = F[i_2-1] + F[i_2] * 4;
    }
#pragma endscop
...
kernel_3414231215_00(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
