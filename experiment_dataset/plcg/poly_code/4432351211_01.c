...
void kernel_4432351211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1][i_0] = E[i_1][i_0] + F[i_1][i_0] - F[1][i_1] + G[i_0][i_1-1][i_1] - E[i_1][0] - 4;
        }
        for (int i_2 = 1; i_2 < min(i_0, PB_P-1); i_2++) {
            for (int i_3 = 1; i_3 < min(-i_0+PB_Q, -i_2+PB_L); i_3++) {
                for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
                    B[i_3][i_2][i_4+1][i_0] = H[i_4][i_3][i_2][i_2-1] * I[i_4][i_3+i_2][i_0] + J[i_4+i_3-1][i_4+1][i_0-1][i_2] * K[i_3+i_2][i_4+1][i_0][i_3+i_0][i_2] + B[i_3][i_2][i_0][i_4-1] - B[i_4+1][i_0][i_2+1][i_3] * B[i_3-1][i_2+1][i_4+2][i_0] - 1;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        for (int i_6 = 1; i_6 < min(i_5, PB_M-1); i_6++) {
            for (int i_7 = 0; i_7 < min(min(min(-i_6+PB_M, i_5), PB_M-1), -i_5+PB_M+1); i_7++) {
                C[i_6][i_7] = F[i_7+i_5][i_6] - G[i_6+1][i_7+1][1] + G[i_6-1][i_7+i_5-1][i_5] * L[i_7][i_5][i_7+i_6][i_7] * 6;
            }
        }
    }
        ...
        for (int i_9 = 0; i_9 < min(-i_8+PB_L, i_8); i_9++) {
            for (int j_0 = 0; j_0 < -i_8+PB_L; j_0++) {
                for (int j_1 = 0; j_1 < PB_M-2; j_1++) {
                    D[j_1+1][i_9][j_0][j_0] = H[j_1][j_0+i_8][i_9+i_8][i_9] + H[i_9][i_8][j_1+1][j_0] + H[j_1+1][j_0][i_9][i_8] * J[j_0+i_8][i_9][j_1][i_9] + D[j_1+1][i_9+1][j_0+1][j_0+1] - D[j_1+1][i_8-1][j_0][j_0] - D[j_1+2][i_9][j_0][j_0] - 6;
                }
            }
        }
    }
