...
void kernel_3314132111_07(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
                C[i_1][i_2-2] = D[i_1+2][i_2][i_0+1] * D[i_1][i_2][i_0-1] * D[i_0][i_2][i_1] + D[i_0+2][i_2-1][i_2+2] + 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M-2, i_3); i_4++) {
            B[i_4+1] = E[i_4+i_3+2][i_4] * E[i_4][i_3] * 4;
            for (int i_5 = 2; i_5 < PB_N; i_5++) {
                C[i_4][i_5-2] = D[i_3][i_5-1][i_3+2] - D[i_5][i_4+i_3][0] + C[i_4][i_5-1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3314132111_07(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
