...
void kernel_2236212221_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_N-1, i_0), PB_M-2); i_1++) {
            B[i_1+2] = C[i_1+i_0+1][i_0-2] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_M; i_3++) {
            B[i_3] = C[i_3][i_2+2] + B[i_3-2] - B[i_3-1] + 6;
        }
    }
#pragma endscop
...
kernel_2236212221_04(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
