...
void kernel_4326251123_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                    B[i_3+1][i_2+2][i_1+1] = D[i_2-1][i_3][i_1][i_0] - D[i_1][i_3+1][i_3-1][i_1+1] + 5;
                }
            }
        }
    }
    ...
    for (int i_8 = 1; i_8 < PB_M-1; i_8++) {
        C[i_8] = E[i_8] - F[i_8] - F[i_8-1] - E[i_8] + F[i_8] + E[i_8+1] * E[i_8] + F[i_8+1] + E[i_8-1] * E[i_8+1] * E[i_8] * C[i_8-1] + 6;
    }
#pragma endscop
...
kernel_4326251123_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
