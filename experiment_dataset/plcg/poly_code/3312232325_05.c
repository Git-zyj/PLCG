...
void kernel_3312232325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] - E[i_0] * A[i_0-1] - A[i_0+1] * A[i_0-2] + A[i_0+2] + 3;
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < PB_M-3; i_2++) {
                B[i_2+1][i_0] = F[i_2][i_0][i_1] - B[i_0+1][i_2-2] - B[i_2+3][i_1] + 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < min(PB_M-3, -i_4+PB_M); i_5++) {
                B[i_5+3][i_4] = G[i_3][i_4][i_5] + H[i_3+1][i_4-2][i_5][1] * G[i_5+i_4][i_3][i_5] - G[i_3][i_5+i_4][0] - F[i_5][i_3][i_4] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3312232325_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
