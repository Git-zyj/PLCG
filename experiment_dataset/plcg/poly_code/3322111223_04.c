...
void kernel_3322111223_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            C[i_2][i_3] = D[i_3] + C[i_2+1][i_3] - C[i_2][i_3-1] * C[i_3][i_2+1] + 1;
        }
    }
#pragma endscop
...
kernel_3322111223_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
