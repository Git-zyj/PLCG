...
void kernel_3216212225_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_1][i_2-1] = A[i_1-2][i_2-1] + A[i_1+2][i_2-1] + A[i_2][i_1+1] - 4;
            }
        }
        for (int i_3 = max(2, i_0); i_3 < min(-i_0+PB_P, PB_M-2); i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                B[i_3][i_0][i_3-2] = C[i_4][i_3+i_0] + B[i_3+2][i_0][i_3] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3216212225_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
