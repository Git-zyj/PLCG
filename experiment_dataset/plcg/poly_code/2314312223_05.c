...
void kernel_2314312223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            B[i_0+1][i_2] = D[i_2][i_0+1] * B[i_0+1][i_2+2] + 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_N-2; i_3++) {
        C[i_3] = C[i_3-2] - C[i_3+1] - C[i_3-1] * C[i_3+2] + C[i_3-1] * 5;
    }
#pragma endscop
...
kernel_2314312223_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
