...
void kernel_3432352211_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] + E[i_0] * F[i_0] * E[i_0+1] * G[i_0] + A[i_0-1] + A[i_0+1] * 3;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_1+PB_M, PB_N); i_3++) {
                B[i_3][i_2][i_1] = H[i_3+i_1][i_2-1][i_1] - I[i_1-1][i_2][i_3] * J[i_3][i_2-1] + J[i_3-1][i_1] - H[i_3][i_1][i_1] - 1;
            }
        }
        for (int i_4 = 0; i_4 < min(-i_1+PB_M, PB_P-1); i_4++) {
            for (int i_5 = 2; i_5 < min(-i_4+PB_M, -i_1+PB_M-1); i_5++) {
                C[i_5+i_4][i_1][i_1] = K[i_5+1][i_1-1][i_4] - I[i_5-2][i_4+i_1][i_5] + B[i_5][i_4+1][i_1+1] + C[i_5+i_1+1][i_4][i_4] - C[i_5+i_1][i_4+1][i_4+1] - 4;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_N; i_6++) {
        A[i_6-1] = E[i_6] - G[i_6+1] * F[i_6-2] + E[i_6] + J[i_6][0] - G[i_6] + 5;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3432352211_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
