...
void kernel_4336132321_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q; i_1++) {
            for (int i_2 = 2; i_2 < min(PB_Q, -i_0+PB_P); i_2++) {
                A[i_0-2][i_2][i_1] = D[i_1][i_0][i_0] * D[i_2+2][i_0][i_1] + D[i_1+2][i_2+i_0][i_2] * A[i_1-2][i_0][i_2-1] + A[i_0-2][i_2-2][i_1] * A[i_1-1][i_2][i_0] + 3;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_0][i_3][i_3] = A[i_3-2][i_4+1][i_0] + A[i_0-3][i_4][i_3] - B[i_0+1][i_4][i_4] - 5;
            }
        }
        for (int i_5 = 1; i_5 < min(min(-i_0+PB_P, i_0), PB_L-2); i_5++) {
            for (int i_6 = 4; i_6 < PB_L-2; i_6++) {
                C[i_0-1][i_6][i_5+2][i_6-2] = D[i_6][i_0-1][i_5] * E[i_5][0] + D[i_6][i_0+1][i_5] * E[i_5+i_0][i_0] + D[i_6+1][i_5+i_0][i_6] * B[i_0][i_5][i_5] - C[i_5-1][i_0+1][i_6+2][i_0-1] - C[i_0-1][i_6-2][i_5+2][i_6-4] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4336132321_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
