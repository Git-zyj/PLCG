...
void kernel_2332211313_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_M+1; i_2++) {
            B[i_1][i_1-1] = B[i_1+1][i_1] + B[i_2-1][i_2-2] + 1;
        }
        for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
            C[i_1][i_3] = C[i_1][i_3-1] - C[i_1-1][i_3] + C[i_1][i_3+1] * 5;
        }
    }
#pragma endscop
...
kernel_2332211313_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
