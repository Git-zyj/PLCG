...
void kernel_2236151115_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M+1, -i_0+PB_L); i_1++) {
            A[i_1][i_0] = C[i_1][i_0+1] * A[i_0+1][i_1-1] * 2;
            A[i_1-1][i_0] = C[i_1-1][i_0] * C[i_0][0] - C[i_1+i_0-1][0] * D[i_1+1] * C[i_0][0] * D[0] * C[i_1+i_0][0] - D[i_1] + 1;
        }
    }
#pragma endscop
...
kernel_2236151115_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
