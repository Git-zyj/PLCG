...
void kernel_4322352121_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N; i_1++) {
            A[i_0][i_1] = D[i_1+i_0][i_1] * D[i_1][i_0] + E[i_0] * F[i_1][i_0] * 4;
        }
    }
        ...
        for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
            for (int i_4 = 2; i_4 < i_2; i_4++) {
                for (int i_5 = 0; i_5 < PB_M; i_5++) {
                    B[i_4][i_3][i_5+1][1] = G[i_2+2][i_3][i_4][i_5] * H[i_5][i_4+2][i_3+2][i_2] + H[i_4][i_5][i_2][i_2+1] - I[i_4][i_4+1][i_3] - B[i_4-2][i_3][i_5+1][1] - 2;
                }
            }
        }
        for (int i_6 = 2; i_6 < PB_L-2; i_6++) {
            for (int i_7 = 0; i_7 < min(-i_2+PB_L+1, PB_M-1); i_7++) {
                C[i_2][i_6+1][0] = J[i_7+1][i_6] - K[i_7][i_2][i_6] * G[i_7+i_2-1][i_6][i_7][0] - K[i_7][i_2][i_6-2] - I[i_6-1][i_2-2][i_7] - K[i_2][i_6+2][i_7] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4322352121_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
