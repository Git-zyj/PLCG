...
void kernel_3332111225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-3; i_2++) {
            C[i_1-1][i_2+1] = A[i_2+2] - C[i_1-1][i_2+3] - C[i_1][i_2+1] + 4;
        }
    }
#pragma endscop
...
kernel_3332111225_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
