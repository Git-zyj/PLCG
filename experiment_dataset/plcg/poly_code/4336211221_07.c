...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-3; i_1++) {
        B[i_1+1] = E[i_1] + B[i_1+3] + B[i_1-1] + 6;
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            for (int i_3 = i_1; i_3 < PB_M-2; i_3++) {
                C[i_1][i_2+1][i_3] = F[i_2][i_1][0] * C[i_1+1][i_3+2][i_2+1] * C[i_3+2][i_1+2][i_2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4336211221_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
