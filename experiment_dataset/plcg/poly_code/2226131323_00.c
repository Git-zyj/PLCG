...
void kernel_2226131323_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            B[i_1-1] = C[i_1-1][i_0][0] * D[i_1][i_1] * D[i_1][i_0+1] - D[i_1][i_0] - 6;
        }
        B[i_0+1] = E[i_0] + E[i_0-1] + B[i_0+2] + B[i_0] * B[i_0-1] + 6;
    }
#pragma endscop
...
kernel_2226131323_00(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
