...
void kernel_3236131325_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_Q, -i_0+PB_P); i_1++) {
            for (int i_2 = 2; i_2 < min(PB_Q, -i_0+PB_P); i_2++) {
                A[i_2][i_2+i_0-1][i_1] = C[i_2][i_0][i_1] * C[i_2-1][i_0][i_1] + C[i_2][i_1+i_0][0] - A[i_1][i_1+i_0][i_2] + A[i_2-2][i_2+i_0-2][i_1-2] + A[i_2+2][i_2+i_0][i_1] - 5;
            }
        }
        for (int i_3 = i_0; i_3 < PB_M-2; i_3++) {
            B[i_3][i_0] = D[i_3+1][i_0] - C[i_0][i_3][i_0+1] + D[i_3][i_0] - B[i_3+2][i_0] * B[i_3-2][i_0-1] * 2;
        }
    }
#pragma endscop
...
kernel_3236131325_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
