...
void kernel_3124232215_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < min(PB_M, -i_0+PB_P); i_2++) {
                A[i_1+1][i_0+2][i_2] = B[i_2][i_1+1] - C[i_2-2][i_2-2] - C[i_2+1][i_2+i_0] - A[i_1][i_0+1][i_2+1] + A[i_1+1][i_0+2][i_2+1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3124232215_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
