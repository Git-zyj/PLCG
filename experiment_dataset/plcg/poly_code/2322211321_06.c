...
void kernel_2322211321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_1] = D[i_1][i_0] * A[i_1-2] * A[i_1-1] * 5;
        }
    }
    ...
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_M; i_5++) {
            C[i_4][i_5] = C[i_4+1][i_5-2] - C[i_4][i_5-2] + C[i_4+2][i_5] + C[i_4+2][i_5+1] - C[i_5][i_4+1] - 5;
        }
    }
#pragma endscop
...
kernel_2322211321_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
