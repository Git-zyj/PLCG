...
void kernel_2324112315_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_1+i_0][0] = 4;
            A[i_1+i_0+1][0] = D[i_0][i_1] + A[i_1+i_0-1][0] - A[i_1+i_0][0] * 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_M+2, PB_M-1); i_3++) {
            C[i_2][i_3] = A[i_3+i_2-2][0] + C[i_2+1][i_3-1] - C[i_2+1][i_3] * C[i_2][i_3+1] - 3;
        }
    }
#pragma endscop
...
kernel_2324112315_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
