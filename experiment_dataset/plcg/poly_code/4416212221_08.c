...
void kernel_4416212221_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L-3; i_1++) {
            A[i_1][i_1+2] = A[i_1+1][i_1+3] - A[i_1-1][i_1+1] + 5;
        }
        for (int i_2 = 3; i_2 < PB_L+2; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_2+PB_L+2, i_2); i_3++) {
                B[i_2-1][i_3+i_2-2][i_0] = B[i_2][i_3+i_2-3][i_0] * 5;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L+2; i_4++) {
        ...
        for (int i_5 = 2; i_5 < i_4; i_5++) {
            for (int i_6 = 0; i_6 < PB_P-2; i_6++) {
                D[i_6][i_5] = E[i_6][i_5][i_4-2] - D[i_6+2][i_5] * D[i_5][i_6+1] + D[i_5][i_6+2] * D[i_6][i_5-2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4416212221_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
