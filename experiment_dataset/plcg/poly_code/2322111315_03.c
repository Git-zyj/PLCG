...
void kernel_2322111315_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            B[i_1] = B[i_2-1] - B[i_2+1] - B[i_2+1] + B[i_2+1] - 3;
        }
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            A[i_3+1] = D[i_3] + A[i_3] - 2;
        }
    }
#pragma endscop
...
kernel_2322111315_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
