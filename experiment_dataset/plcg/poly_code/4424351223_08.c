...
void kernel_4424351223_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,int wj,DATA_TYPE POLYBENCH_5D(J,xJ,yJ,zJ,vJ,wJ,xj,yj,zj,vj,wj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0+1] + F[i_0] + A[i_0+2] + A[i_0-1] * A[i_0-2] + 2;
        for (int i_1 = 1; i_1 < PB_L-3; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_Q-1; i_2++) {
                for (int i_3 = 2; i_3 < min(i_1, -i_1+PB_Q); i_3++) {
                    B[i_1+1][i_3][i_0+1][i_2-1] = G[i_3-1][i_0+1][i_2][i_1] + H[i_3][i_0][i_1][i_2+i_0+1] - G[i_1][i_2][i_0][i_1] * I[i_2][i_1][i_0][i_3+i_2] - H[i_0][i_2][i_3][i_3+i_1] - G[i_1][i_0][i_3+1][i_2-1] - J[i_3][i_2][i_0][i_1-1][i_3] + H[i_3][i_2][i_1][i_0] - B[i_0+1][i_3-2][i_1+3][i_2-1] + 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        A[i_4+2] = F[i_4] + K[i_4] * K[i_4-1] - E[i_4] * F[i_4] * F[i_4-1] + 4;
        for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
            A[i_5] = L[i_5-1][i_4] * M[i_4-1][0] + M[i_4][i_5] + M[i_4][i_5+1] * 1;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4424351223_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M));
