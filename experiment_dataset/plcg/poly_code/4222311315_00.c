...
void kernel_4222311315_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_L+1); i_1++) {
            B[i_1-1][i_0+1] = C[i_1+i_0][i_1-1][0] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_L+1, PB_L-1); i_3++) {
            for (int i_4 = i_2; i_4 < PB_L-1; i_4++) {
                B[i_3][i_4+1] = D[i_4][i_3+i_2-1][0] + B[i_4+1][i_3+1] - B[i_3][i_4] - B[i_4][i_3] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4222311315_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
