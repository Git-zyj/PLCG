...
void kernel_4314352311_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_M-3); i_1++) {
            A[i_1+2] = D[i_0+1][i_1-1] * E[i_1+1] * F[i_1+2][i_0-1] - F[i_0][i_1+i_0] - D[i_0+1][i_1-1] + D[i_1][i_0+1] + D[i_1+i_0][i_1] - A[i_0+1] + A[i_1+1] - A[i_1+3] + C[i_1][i_0] + 6;
            for (int i_2 = 2; i_2 < -i_0+PB_P-2; i_2++) {
                for (int i_3 = 0; i_3 < PB_Q-1; i_3++) {
                    B[i_2+1][i_0][i_1-2][i_3] = G[i_3][i_2-2][i_0][i_1+2] * H[i_3+i_2][i_1][i_0-2][i_0] + I[i_1][i_3][i_1-2][0] - I[i_2][i_3][i_3+1][i_1-2] * J[i_3+i_1][i_2][i_2+i_0+2] * B[i_0][i_2-1][i_1-2][i_3+1] + B[i_0][i_2][i_1-2][i_3+1] - 3;
                }
            }
            C[i_1][i_0] = D[i_1][i_0] + K[i_1][i_0+1] * L[i_1] * A[i_1+3] - C[i_1][i_0-1] + 3;
        }
    }
#pragma endscop
