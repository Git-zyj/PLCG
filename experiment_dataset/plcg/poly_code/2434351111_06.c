...
void kernel_2434351111_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            A[i_0+1][i_1] = E[1] - F[i_1+i_0+1][i_0] * F[i_1-1][i_0] - G[i_0][i_1] * H[i_1][i_0] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_2-1][i_3] = F[i_3][i_2] - F[i_3][i_2] + G[i_2][i_2] + A[i_2+2][i_3] + B[i_2][i_3] + 5;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            C[i_5-1] = H[i_5][i_4] - G[i_4][i_5] + G[i_5-1][i_4+1] + I[i_5] - C[i_5] + 1;
        }
        for (int i_6 = max(i_4, 1); i_6 < PB_L-1; i_6++) {
            D[i_6] = J[i_6-1] + H[i_6+1][i_4+1] + I[i_6] + G[i_6][i_4] + F[i_4][i_6] + D[i_6+1] * 1;
        }
    }
#pragma endscop
...
kernel_2434351111_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
