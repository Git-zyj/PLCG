...
void kernel_4432151115_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[i_0] + E[i_0+1] + E[i_0] * E[i_0] - A[i_0] - D[i_0+2] - 6;
        for (int i_1 = i_0; i_1 < -i_0+PB_N-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_L, PB_M-1); i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_M-1; i_3++) {
                    C[i_2+1][i_1][i_3] = F[i_3+1][i_2+i_1][i_1] + G[i_2][i_0][i_1][i_0] - F[i_3+i_0+1][i_1-1][i_2-1] + G[i_1+i_0+1][i_3][i_3][i_0+1] * G[i_0-1][i_3][i_0][i_1] + G[i_3+i_2][i_0+1][i_1][0] * 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_M; i_6++) {
                C[i_4+1][i_5][i_6] = F[i_5][i_6-1][i_4] - F[i_4][i_6+i_5+1][i_6] * F[i_6+i_5][i_4-1][i_6] + H[i_5][i_4] * 6;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
        D[i_7+1] = E[i_7] + E[i_7] - E[i_7-1] - E[i_7+1] * E[i_7] + D[i_7] * 5;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4432151115_08(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
