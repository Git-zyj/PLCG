...
void kernel_4416312325_02(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                for (int i_3 = 0; i_3 < PB_Q; i_3++) {
                    B[i_1+1][i_2][i_0-2][i_3] = 2;
                }
                for (int i_4 = i_0; i_4 < PB_Q; i_4++) {
                    B[i_1][i_2][i_0-2][i_4] = E[i_0][i_0][i_4][i_1-2][i_2] + B[i_1+2][i_2][i_4-2][i_0] - B[i_1+2][i_2][i_0-3][i_4] + B[i_1][i_2-1][i_0-3][i_4] * B[i_1][i_2-1][i_0][i_4] * 5;
                }
            }
        }
        for (int i_5 = 0; i_5 < i_0; i_5++) {
            for (int i_6 = 0; i_6 < PB_L-2; i_6++) {
                for (int i_7 = 0; i_7 < PB_P; i_7++) {
                    B[i_6+2][i_7][i_0-2][i_5] = 1;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_M-1; i_8++) {
        D[i_8-1] = F[i_8] + G[i_8][i_8] * D[i_8] + D[i_8+1] + 6;
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4416312325_02(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
