...
void kernel_4336312223_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
            for (int i_2 = i_0; i_2 < PB_N; i_2++) {
                A[i_2][i_1-2][i_0-2] = A[i_2][i_1-2][i_0] + A[i_2][i_1-2][i_0-3] + A[i_2][i_1-2][i_0-1] * 2;
            }
        }
        for (int i_3 = 2; i_3 < min(-i_0+PB_M, i_0); i_3++) {
            B[i_3+i_0][i_0-1] = D[i_0][i_3-2] - 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_M); i_5++) {
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                C[i_5+i_4-1][i_5][0] = C[i_5+i_4][i_5][0] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4336312223_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
