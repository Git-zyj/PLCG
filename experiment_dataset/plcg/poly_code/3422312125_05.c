...
void kernel_3422312125_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M+1, -i_0+PB_L-2); i_1++) {
            A[i_1-1][i_1+i_0] = A[i_1-1][i_1+i_0+1] + A[i_1-1][i_1+i_0+2] * 5;
        }
        for (int i_2 = i_0; i_2 < PB_M-2; i_2++) {
            for (int i_3 = 0; i_3 < -i_2+PB_M-2; i_3++) {
                B[i_3+i_2+1][i_0][i_2] = E[i_0][i_3][0] + B[i_3+i_2+2][i_0+2][i_2+1] + 1;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, PB_M-2); i_5++) {
            C[i_5+1][i_4] = C[i_5+2][i_4] - 5;
        }
        D[i_4] = 6;
    }
#pragma endscop
...
kernel_3422312125_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
