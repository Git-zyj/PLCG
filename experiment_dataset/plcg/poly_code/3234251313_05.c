...
void kernel_3234251313_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            A[i_1+i_0][i_0][i_1-1] = C[i_1-1][i_0] - C[i_1][i_0] * C[i_1][i_0+1] + A[i_1+i_0-1][i_0-1][i_1-1] - A[i_1+i_0+1][i_0][i_1] * 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_L, -i_3+PB_Q); i_4++) {
                B[i_2-1][i_4][i_3] = D[i_3][i_4-1][i_3+1] * E[i_4][i_2][i_3] + D[i_3][i_2+1][i_4] * E[i_4+1][i_2+1][0] - E[i_3][i_2-1][i_2] + E[i_3+1][i_2][i_4] * C[i_3+1][i_4+i_3] + B[i_2-1][i_3][i_4+1] * B[i_2-2][i_4][i_3] + B[i_2-1][i_4][i_3-1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3234251313_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
