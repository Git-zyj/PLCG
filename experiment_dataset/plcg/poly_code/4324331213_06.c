...
void kernel_4324331213_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_M-1; i_3++) {
                    A[i_1+i_0+1][i_0][i_2][i_3] = D[i_0+1][i_2][i_1][i_3] + E[i_3][i_0-1][i_1+1][i_2+i_1] - A[i_3+i_1+1][i_1-1][i_2][i_0+1] + 5;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            B[i_4+1][i_5] = F[i_5][i_4] - G[i_5][i_4] - B[i_4+2][i_5] + B[i_4][i_5] + B[i_4+1][i_5+1] + B[i_4+1][i_5-1] - 3;
        }
    }
    for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
        for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
            for (int i_8 = 0; i_8 < i_7; i_8++) {
                C[i_7][i_7] = H[i_8+1][i_6-1][i_8] + I[i_7][i_6-1][1] + I[i_7+1][i_8+1][i_6+1] - C[i_7-1][i_7-1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4324331213_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
