...
void kernel_2232111211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
            B[i_1-1][i_2+1] = C[i_2] + B[i_2][i_1+1] - 1;
        }
    }
#pragma endscop
...
kernel_2232111211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
