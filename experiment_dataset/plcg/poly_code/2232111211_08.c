...
void kernel_2232111211_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_L, -i_0+PB_P); i_1++) {
            A[i_0+1][i_1+i_0][i_1+1] = 4;
            B[i_1-1] = C[i_1+1][0] * A[i_0+1][i_1+i_0][i_1+1] + B[i_1-2] + B[i_1] - 3;
        }
    }
#pragma endscop
...
kernel_2232111211_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
