...
void kernel_4436332213_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_L-1; i_2++) {
                A[i_1][i_2][i_2] = E[i_2][i_0+2][i_1][i_1] - F[i_1+2][i_0][i_2] + G[i_0][i_1] - F[i_2+i_1+1][i_0+1][i_1+1] + A[i_1+1][i_2-1][i_2-1] * A[i_1+1][i_2+1][i_2+1] * 2;
            }
        }
        for (int i_3 = 2; i_3 < min(-i_0+PB_Q-1, i_0); i_3++) {
            for (int i_4 = 1; i_4 < -i_0+PB_Q-2; i_4++) {
                for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
                    B[i_3][i_4+1][i_5][i_3+i_0+1] = H[i_5+2][i_3-2][i_0+1] * F[i_4][i_5][i_3-2] - I[i_4+i_0][i_3][i_4][i_3] * B[i_0][i_5+2][i_3][i_4+i_0+2] + B[i_3][i_0][i_4-1][i_5+i_3+1] - 3;
                }
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_N-2; i_6++) {
        C[i_6+1] = J[i_6-1] + J[i_6] + C[i_6+2] + C[i_6] + 4;
    }
    for (int i_7 = 1; i_7 < PB_L; i_7++) {
        for (int i_8 = 0; i_8 < min(PB_Q-2, i_7); i_8++) {
            for (int i_9 = 1; i_9 < min(-i_8+PB_L, PB_L-2); i_9++) {
                A[i_8][i_7-1][i_7-1] = I[i_7][i_8][i_9+2][i_9-1] * K[i_9+i_8][i_7][i_8+2] + K[i_9+2][i_9][i_7] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4436332213_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
