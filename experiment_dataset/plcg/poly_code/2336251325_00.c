...
void kernel_2336251325_00(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        C[i_0-1][0] = D[i_0] * E[i_0] * E[i_0-1] - C[i_0+1][0] + 6;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_L, PB_M-1); i_2++) {
            C[i_2-1][0] = F[i_2+1][i_2+i_1] * F[i_2+1][i_1-1] + G[i_1][i_2] + F[i_2][i_1] - H[i_2][i_2][i_1] + D[i_1] - G[i_2][i_1] - G[i_1-1][0] - G[i_2][i_1] + F[i_1][i_2] - G[i_2-1][i_1] + 2;
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 3; i_4 < min(PB_M-1, i_3); i_4++) {
            C[i_4-1][0] = F[i_4+1][i_3] + C[i_3-2][0] - C[i_4-3][0] * 5;
        }
    }
#pragma endscop
...
kernel_2336251325_00(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
