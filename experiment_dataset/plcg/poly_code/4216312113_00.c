...
void kernel_4216312113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        ...
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M-1; i_2++) {
                B[i_2+1][i_1+2][i_0] = B[i_2][i_1+2][i_0] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4216312113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
