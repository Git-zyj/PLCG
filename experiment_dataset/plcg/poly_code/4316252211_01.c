...
void kernel_4316252211_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M, -i_0+PB_P); i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_L-2; i_2++) {
                for (int i_3 = 0; i_3 < min(min(PB_Q-2, -i_2+PB_M+1), -i_0+PB_L); i_3++) {
                    A[i_1][i_2+i_0][i_3][0] = D[i_2][i_3][i_1-1][0] + D[i_2][i_1+i_0][i_3+2][i_0] * E[i_3][i_2+1][i_0-1][i_1-2][0] - F[i_3+i_2-1][i_0][1] * G[i_2][i_3+i_0][i_1+1][1] + G[i_0+1][i_3][i_2-2][0] * D[i_0+1][i_2][i_3][i_1-1] - A[i_1][i_2+i_0+2][i_3][0] * A[i_2][i_3+i_0][i_1+1][0] - 5;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        B[0] = H[i_4] + H[1] + H[i_4] * B[0] - 1;
        for (int i_5 = max(i_4, 2); i_5 < PB_M; i_5++) {
            for (int i_6 = 2; i_6 < PB_M; i_6++) {
                for (int i_7 = i_4; i_7 < -i_5+PB_Q; i_7++) {
                    C[i_4][i_5][i_6][i_7+i_5] = D[i_5][i_7+1][i_4][0] + G[i_6][i_5][i_7][i_6] * D[i_6-2][i_7][i_4][i_4] + D[i_6][i_7][i_4][i_5] - C[i_4][i_5-1][i_6][i_7+i_5-2] + 4;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4316252211_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
