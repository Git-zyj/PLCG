...
void kernel_4422231323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0-1] = E[i_0] - F[i_0] * A[i_0] - A[i_0-2] - A[i_0+1] + 1;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_P; i_2++) {
            B[i_1][i_2] = G[i_1-1][i_2] + G[i_1][i_2+1] - G[i_1][i_2+i_1] - H[i_2][i_1] * B[i_1+1][i_2] * 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
        for (int i_4 = min(PB_N, -i_3+PB_L+1) - 1; i_4 >= 1; i_4--) {
            D[i_4] = H[i_4][i_3-1] - H[i_3][i_4+1] * H[1][i_4+i_3-1] * F[1] * H[i_4][i_3] * B[i_3][i_4] + D[i_4-1] + D[i_4] * 3;
        }
        D[i_3] = D[i_3+2] + 6;
    }
#pragma endscop
...
kernel_4422231323_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
