...
void kernel_2234212215_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_L, i_1); i_2++) {
            B[i_1][i_2] = C[i_2] + B[i_1-1][i_2] + B[i_1-1][i_2-1] - B[i_1+1][i_2] * 6;
        }
    }
#pragma endscop
...
kernel_2234212215_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
