...
void kernel_4224111125_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            B[i_4][i_3-1] = C[0] + B[i_4+1][i_3-1] - B[i_4][i_3+1] + 3;
        }
    }
#pragma endscop
...
kernel_4224111125_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
