...
void kernel_4312151321_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 0; i_1 < PB_P-3; i_1++) {
            for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
                for (int i_3 = 2; i_3 < -i_2+PB_P; i_3++) {
                    A[i_2][i_1+1][i_3][i_0-1] = D[i_0+1][i_3][i_0][i_1] + D[i_1][i_0][i_3+i_2][i_1] - E[i_1][i_3][i_2+1] + F[i_3][i_2][i_1][i_3][i_0-1] + A[i_2][i_1][i_3-1][i_0] + A[i_0][i_3+1][i_1+1][i_2-1] * A[i_2][i_1+1][i_3-2][i_0-1] * A[i_2][i_1+3][i_3+1][i_0-1] + A[i_3][i_2+1][i_0][i_1+1] * 1;
                }
            }
        }
        for (int i_4 = 1; i_4 < min(-i_0+PB_M, PB_L-1); i_4++) {
            B[i_4] = G[i_4][i_0] + G[i_4][i_0-1] - G[i_4+i_0][1] * B[i_0-1] + 3;
            for (int i_5 = i_4; i_5 < -i_4+PB_L-1; i_5++) {
                C[i_4-1][i_0][i_5-1][i_0+1] = E[i_0][i_4][i_5] * E[i_5][i_4+1][i_0] + E[i_5+i_4+1][i_0][0] - E[i_5-1][i_0][i_4+i_0] * D[i_0][i_4-1][i_5+i_4][0] + E[i_4][i_0][i_5] + B[i_4] * 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4312151321_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
