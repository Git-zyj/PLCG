...
void kernel_3234112223_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 3; i_1 < PB_L+3; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_0][i_2][i_0-2] = A[i_1-1][i_2][i_1-3] + A[i_0-2][i_2+1][i_0-4] + 3;
            }
        }
        for (int i_3 = 1; i_3 < min(i_0, -i_0+PB_L-1); i_3++) {
            for (int i_4 = i_3; i_4 < PB_N; i_4++) {
                B[0][i_3+i_0][i_4] = C[i_3][i_0][i_3-1] * B[0][i_3+i_0+1][i_4] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3234112223_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
