...
void kernel_2436231311_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0+1] + F[i_0] - F[i_0] - E[i_0] + F[i_0-1] - A[i_0-1] - A[i_0+1] * A[i_0+1] * 4;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M+1, PB_P-1); i_2++) {
            B[0][i_2] = G[i_1-1][i_2+1] + H[i_1][i_2-1] * H[i_2+i_1-1][1] * F[i_2] + A[i_2] - B[0][i_2+1] + 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_N+1; i_3++) {
        C[i_3] = E[i_3-1] * F[i_3] + B[0][i_3] * C[i_3+1] - 5;
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < min(-i_4+PB_M-1, PB_L); i_5++) {
            B[0][i_5] = I[i_5+i_4+1][i_5+i_4][0] * C[i_4] + 1;
        }
    }
#pragma endscop
...
kernel_2436231311_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
