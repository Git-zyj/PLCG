...
void kernel_3222111225_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        ...
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < PB_Q-1; i_2++) {
                B[i_1][i_0] = C[i_1-1][i_0][i_2+1] * B[i_2-2][i_0] + B[i_2-1][i_1] * B[i_1+1][i_0] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3222111225_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
