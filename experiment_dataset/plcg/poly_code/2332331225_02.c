...
void kernel_2332331225_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        A[i_0][i_0-1] = D[i_0][i_0] - E[i_0+1] + A[i_0-2][i_0-3] - 5;
    }
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        B[i_1] = F[i_1] + B[i_1-1] - 3;
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            B[i_1] = E[i_1] * D[i_2+1][i_1+1] * D[i_1-1][0] + F[i_2+1] * G[i_2][i_1+1] + H[i_2-1][i_1] - B[i_1] * 2;
        }
    }
#pragma endscop
...
kernel_2332331225_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
