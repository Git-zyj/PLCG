...
void kernel_4212131121_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_2][i_1] = C[i_2][i_1] * D[i_0][i_1+1][i_2+1] + D[i_0][i_2][i_0] * A[i_1][i_2+1] * 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        B[i_3][i_3] = E[i_3] * E[i_3] * E[i_3-1] + B[i_3-1][i_3-1] + 2;
    }
#pragma endscop
...
kernel_4212131121_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
