...
static void print_array(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4312352225_04(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_Q-2, i_0); i_2++) {
                for (int i_3 = 1; i_3 < min(min(-i_2+PB_M, i_0), -i_1+PB_Q); i_3++) {
                    C[i_1-2][i_3-1][i_2+2][i_0] = D[i_2][i_1][i_3+1] - E[i_2][i_1][i_3+i_1][i_0+1] - F[i_3+i_2][i_0+2][i_1][i_1+1] + G[i_3][i_2][i_1][i_0] * F[i_1-2][i_3+2][0][i_2] - 1;
                }
            }
            B[i_0][i_0] = H[1][i_1+1] * I[i_1+2] - J[i_1-2][i_0] + B[i_0+2][i_0+2] * B[i_0+1][i_0+1] - 5;
        }
    }
    for (int i_4 = 2; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_Q; i_5++) {
            for (int i_6 = i_5; i_6 < min(-i_5+PB_M, PB_L-2); i_6++) {
                for (int i_7 = 0; i_7 < min(-i_6+PB_L-2, -i_4+PB_Q); i_7++) {
                    C[i_4][i_7][i_6][i_5] = K[i_6+2][i_7][i_5] - E[i_4][i_7+i_6][i_5-1][1] + E[i_5][i_7][i_7+i_4][0] - L[i_6][i_7+2][i_5] + E[i_6+i_5][i_4][i_7+i_4][1] * G[i_5][i_7+i_6+2][i_4+1][i_6] + C[i_4-2][i_7][i_6][i_5] + 4;
                }
            }
        }
    }
...
init_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4312352225_04(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L)));
