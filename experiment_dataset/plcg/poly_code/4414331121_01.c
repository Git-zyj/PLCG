...
void kernel_4414331121_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    B[i_0+1][i_2][i_1][i_2] = F[i_2][i_3][i_0] - G[i_1][i_2][i_3][0] + G[i_1][i_0-1][i_0][i_3] * 3;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_N-3; i_5++) {
            for (int i_6 = max(i_4, 1); i_6 < min(PB_L-3, -i_5+PB_Q-1); i_6++) {
                for (int i_7 = 1; i_7 < min(-i_5+PB_Q, PB_L-1); i_7++) {
                    C[i_5-1][i_4+1][i_7+1] = H[i_7-1][i_6][i_5][1] + F[i_6-1][i_7+1][i_5-1] * G[i_6][i_4][i_7+i_5][i_5] + I[i_7+i_5-1][i_4+1][i_7][i_6+i_5] - I[i_7][i_5-1][i_4][i_6] + I[i_6][i_7][i_4][i_6+i_5+1] + C[i_5-1][i_7+1][i_6+3] - D[i_5+3][i_4+2][i_6][0] * 4;
                }
            }
        }
        for (int i_8 = 0; i_8 < PB_L-1; i_8++) {
            for (int i_9 = 0; i_9 < PB_L-3; i_9++) {
                for (int j_0 = max(i_8, 1); j_0 < PB_P; j_0++) {
                    D[i_8+1][i_4][i_9][0] = F[i_8][i_8][i_4] + C[j_0-1][i_8+1][i_9+3] + 2;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4414331121_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
