...
void kernel_3432352223_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0] + E[i_0] * F[i_0] + G[i_0] * F[i_0+1] + A[i_0-1] + D[i_0] * D[i_0-2] + 3;
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_M, -i_1+PB_N); i_2++) {
                B[i_0][i_1][i_2] = H[i_1][i_2+i_1-1] - H[i_2+i_0][i_1] - I[i_0+2][i_2][0] - J[i_2+i_1][i_0-2][0] - B[i_0][i_1][i_2-1] + B[i_0][i_1][i_2-2] * C[i_2+1][i_1] + 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N-2; i_3++) {
        for (int i_4 = 2; i_4 < PB_N; i_4++) {
            for (int i_5 = 2; i_5 < PB_N-2; i_5++) {
                C[i_4][i_5-1] = K[i_4][i_5][i_3-2][i_4-2] * I[i_4][i_5-2][i_5-1] - J[i_5][i_4][i_3] * L[i_5+2][i_4+1][i_3][i_3-2] + M[i_5+1][i_4][i_3+1] + 4;
            }
        }
        D[i_3] = F[i_3] * F[i_3+2] - N[i_3-2][i_3+2] + F[i_3+2] * G[i_3] + F[i_3] - D[i_3+1] * D[i_3-2] - 3;
    }
#pragma endscop
