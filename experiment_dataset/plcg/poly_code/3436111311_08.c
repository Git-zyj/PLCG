...
void kernel_3436111311_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_L, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_1+i_0-1][i_1][i_0+1] = C[i_2][i_1] - 4;
            }
        }
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < min(min(min(i_0, PB_L), -i_3+PB_P-1), -i_0+PB_P); i_4++) {
                B[i_0][i_3][i_4+i_3+1] = B[i_0+1][i_3-1][i_4+i_3] - B[i_3+1][i_4][i_4+i_0] + 4;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L+1; i_5++) {
        for (int i_6 = i_5; i_6 < PB_L-1; i_6++) {
            C[i_6][i_5] = C[i_6][i_5-1] - C[i_5-1][i_6-1] - C[i_6+1][i_5] - 3;
        }
    }
    for (int i_7 = 0; i_7 < PB_L; i_7++) {
        for (int i_8 = max(1, i_7); i_8 < PB_L; i_8++) {
            for (int i_9 = 1; i_9 < PB_L; i_9++) {
                D[i_9][i_8][0] = E[i_8+1][i_9][0] - D[i_9-1][i_8][0] + D[i_9+1][i_8+1][0] + D[i_9+1][i_8-1][0] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3436111311_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
