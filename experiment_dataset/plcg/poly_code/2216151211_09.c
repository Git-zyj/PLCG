...
void kernel_2216151211_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            A[i_0][i_1] = C[i_1][i_0] - C[i_0-1][i_1] - C[i_0+1][0] + D[i_1][i_1][i_0] * E[0] + A[i_0][i_1-1] - A[i_0][i_1+1] * A[i_0+1][i_1+1] + 4;
        }
        for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
            B[i_2][i_2] = C[i_0+1][i_2+1] + C[i_2][i_0] * C[i_2][i_2] * C[i_0][i_2] * B[i_2-1][i_2-1] * 3;
        }
    }
#pragma endscop
...
kernel_2216151211_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
