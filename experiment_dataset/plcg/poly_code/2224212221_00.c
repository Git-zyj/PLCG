...
void kernel_2224212221_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        ...
        for (int i_2 = max(1, i_0); i_2 < PB_N-2; i_2++) {
            B[i_2] = D[i_2][i_0] - B[i_2+2] - B[i_2-1] * 2;
        }
    }
#pragma endscop
...
kernel_2224212221_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
