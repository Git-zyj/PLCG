...
void kernel_4326231325_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_0][i_0-1] = D[i_1][i_0] + E[i_1] * F[i_1+1] + D[i_0+1][i_1] * A[i_0+1][i_0] + A[i_0-1][i_0-2] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            A[i_2+1][i_2] = D[i_3][i_2] * 4;
        }
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 0; i_5 < min(i_4, -i_2+PB_M-1); i_5++) {
                for (int i_6 = 1; i_6 < PB_Q-1; i_6++) {
                    C[i_4][i_2][i_5][i_6] = G[i_2+1][i_4][0][i_6-1] + H[i_4][i_5+i_2+1][i_6][i_5] + H[i_6][i_2][i_6][i_5] + C[i_4][i_2+1][i_5][i_6+1] - C[i_4+1][i_2+1][i_5][i_6] * C[i_4+1][i_2][i_6][i_5] + C[i_4][i_2][i_5][i_6+1] + 3;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4326231325_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
