...
void kernel_4324112125_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_N; i_2++) {
                A[i_1][i_1] = D[i_2-1][0] * A[i_2-1][i_2-1] + 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_N, -i_4+PB_L); i_5++) {
                A[i_5][i_5] = E[i_3][i_3][i_5+i_4] - E[i_5+i_3][i_4-2][i_4] * 4;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M; i_6++) {
        for (int i_7 = max(i_6, 1); i_7 < PB_P; i_7++) {
            for (int i_8 = 0; i_8 < -i_6+PB_M; i_8++) {
                C[i_8+i_6][i_7][i_6] = C[i_8+i_6][i_7-1][i_6] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4324112125_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
