...
void kernel_4232311313_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_2][i_0] = A[i_2+1][i_1] + A[i_1-1][i_2] + A[i_2][i_1-1] - 2;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_L+1; i_4++) {
            for (int i_5 = 2; i_5 < PB_L; i_5++) {
                for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
                    B[i_5+1][i_4-1][i_6-1][i_3-1] = C[i_4-1][i_5][i_3-1][1] * B[i_6+1][i_4-1][i_3-2][i_5-2] * B[i_6+1][i_4-1][i_5-1][i_3-2] * B[i_6+1][i_4-1][i_5][i_3-1] * 5;
                }
            }
        }
    }
...
kernel_4232311313_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
