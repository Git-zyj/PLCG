...
void kernel_4332112311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                A[i_2+i_1-1][i_1] = A[i_2+i_1-1][i_1-1] - A[i_2+i_1][i_1+1] + A[i_2+i_1][i_2] - A[i_2+i_1-3][i_1-1] * A[i_2+i_1][i_2+1] + 6;
            }
        }
        B[i_0-2] = D[2] - B[i_0-3] * B[i_0-1] - 6;
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < i_3; i_5++) {
                C[i_4][i_3] = C[i_5][i_3-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4332112311_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
