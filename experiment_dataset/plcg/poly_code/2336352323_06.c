...
void kernel_2336352323_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] * E[0] + F[i_0+2] - E[i_0-2] - D[i_0] - G[i_0][i_0] + A[i_0-1] - 5;
    }
    for (int i_1 = 2; i_1 < PB_M-3; i_1++) {
        for (int i_2 = 2; i_2 < min(PB_L, i_1); i_2++) {
            B[i_2-1][i_1+1] = H[i_1][i_1-1] + H[i_2][i_1+2] * H[i_2][i_1] + G[i_1-1][i_2] - I[i_2][i_2][i_1] * A[i_2] - B[i_2-2][i_1+3] + B[i_2-1][i_1+3] - B[i_2][i_1+1] + 6;
        }
        for (int i_3 = i_1; i_3 < PB_L; i_3++) {
            A[i_3] = H[i_1][i_3] + J[i_1][i_3] + G[i_3-2][i_1] + H[i_1-2][1] - 3;
        }
    }
#pragma endscop
...
kernel_2336352323_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
