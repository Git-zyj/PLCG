...
void kernel_3324352221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0-2] + D[i_0] * D[i_0+2] - E[i_0] - E[i_0] + A[i_0-2] * 6;
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            C[i_0-4][i_1+2] = E[i_0] * F[i_1][i_0] + F[i_1+i_0][i_0] + G[i_1][i_0+1] * H[i_1][i_0] + A[i_1-1] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            C[i_2-2][i_3] = I[i_2] - G[i_3][i_2] * E[2] - G[i_2][i_3] - F[i_2-1][i_3+1] - A[i_3] * C[i_2-1][i_3] - 3;
        }
    }
#pragma endscop
...
kernel_3324352221_00(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
