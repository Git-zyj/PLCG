...
void kernel_2336311111_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = 6;
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_N); i_1++) {
            B[i_0][i_0] = D[i_1+i_0][i_1] * A[i_0] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, PB_M-1); i_3++) {
            C[i_2] = E[i_3+1][i_2] - E[i_3+1][i_2] + C[i_2-1] * 5;
        }
    }
#pragma endscop
...
kernel_2336311111_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
