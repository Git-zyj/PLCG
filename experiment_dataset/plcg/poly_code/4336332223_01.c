...
void kernel_4336332223_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        B[i_0] = D[i_0] + D[i_0] * E[i_0+1] + B[i_0] * B[i_0+2] + B[i_0+1] - 6;
        B[i_0] = F[i_0] * D[i_0] + F[i_0] * F[i_0] - 5;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L-2; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                for (int i_4 = PB_M-1 - 1; i_4 >= 2; i_4--) {
                    C[i_3+2][i_2][i_1][i_4] = G[i_4-2][i_1][i_2][i_3] + H[i_4+1][i_4][i_2+2][0] + 2;
                }
            }
        }
    }
...
kernel_4336332223_01(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
