...
void kernel_4426352223_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-2] * F[0] - E[i_0] * G[i_0] + E[i_0] - A[i_0+1] * 1;
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_1+i_0][i_2][2] = H[i_1-1][i_2][i_0-2] - I[i_0][i_2] - 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-4; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_N, i_3); i_4++) {
            C[i_4+2][i_3] = F[i_4] - J[i_4+1][i_3+1] - K[i_4+1][i_3-1][0] - K[i_4][i_3][i_3+2] - L[i_4+2][0] + F[i_4+i_3] + L[i_4][0] - I[i_4+1][i_4] - I[i_3+2][i_4] + J[i_4][i_3] * I[i_4][i_3] + C[i_3+4][i_4] + C[i_4+4][i_3] - C[i_4+2][i_3-2] + C[i_4+2][i_3+1] + 5;
        }
        D[i_3] = G[i_3+2] - F[i_3-2] + D[i_3+1] * D[i_3-2] * 6;
    }
#pragma endscop
