...
void kernel_3226131123_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_N; i_1++) {
            B[i_0][i_1-2] = C[i_1][i_0+1] - C[i_1][i_0-1] - C[i_0-1][i_1+1] + C[i_1+i_0][1] * C[i_0][i_0+1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_N; i_3++) {
            B[i_3][i_2] = C[i_3][i_2] - B[i_3][i_2+2] - 6;
        }
    }
#pragma endscop
...
kernel_3226131123_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
