...
void kernel_4436132223_09(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        C[i_0] = E[i_0] * E[i_0] + 2;
        for (int i_1 = max(1, i_0); i_1 < PB_L-1; i_1++) {
            C[i_1-1] = F[i_1+1][i_1][i_0] * G[i_1][i_0+1] + C[i_1+1] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        C[i_2] = E[0] * E[1] + E[i_2] - E[i_2] - E[i_2-2] * E[i_2] - C[i_2-2] - C[i_2+1] + 3;
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_P; i_4++) {
            for (int i_5 = 2; i_5 < -i_4+PB_L; i_5++) {
                for (int i_6 = 2; i_6 < PB_M; i_6++) {
                    D[i_5+i_4][i_4][i_5][0] = H[i_4][i_5-2][i_6-2][i_3] - H[i_6][0][i_5-2][i_4-2] + 5;
                }
            }
        }
    }
...
init_array(xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436132223_09(xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
