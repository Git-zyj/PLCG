...
void kernel_4336132213_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < min(PB_Q-1, -i_1+PB_L); i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_L-1; i_3++) {
                    A[i_0][i_3+i_1+1][i_2][i_2+1] = D[i_1+1][i_2-1][i_0][i_0] * D[i_0][i_2+i_1][i_3][0] * D[i_3+i_1][i_3][i_0][i_2-2] + 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        B[i_4+1] = E[i_4] + E[i_4] + E[i_4] * B[i_4+2] - B[i_4] * 1;
    }
    for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 2; i_6 < min(-i_5+PB_M, i_5); i_6++) {
            B[i_6] = F[i_6][i_5] + F[i_6+i_5][i_6-2] + F[i_6][i_5] + B[i_6+1] - B[i_5+2] + 4;
        }
    }
#pragma endscop
...
kernel_4336132213_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
