...
void kernel_3232132315_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_N, -i_0+PB_L); i_1++) {
            A[i_0][i_1-2] = C[i_1][i_0+1] + C[i_0][i_1+1] * C[i_0-2][i_1+i_0] + A[i_0][i_1-1] + A[i_0+1][i_1-2] + A[i_0-1][i_1-2] + 1;
        }
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            A[i_0-1][i_2-2] = D[i_2] - C[i_0+2][2] + 4;
        }
    }
#pragma endscop
...
kernel_3232132315_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
