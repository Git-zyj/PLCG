...
void kernel_3232111123_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M+1; i_1++) {
            for (int i_2 = i_0; i_2 < PB_N; i_2++) {
                A[i_1-1][i_0][i_2] = C[i_2][i_0][i_1-1] - A[i_1-1][i_0+2][i_2+1] - 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M+2; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_N, -i_3+PB_L-1); i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_N; i_5++) {
                A[i_3-2][i_4+2][i_5] = C[i_5+i_4][i_4+i_3+1][i_4+i_3] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3232111123_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
