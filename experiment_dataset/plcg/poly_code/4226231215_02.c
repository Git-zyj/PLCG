...
void kernel_4226231215_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0-1][i_1] = C[i_1+i_0][i_1+i_0] + D[i_1+1][i_1] * E[i_1-1] - D[i_0+1][i_1] + A[i_0-2][i_1] * A[i_1-1][i_0-1] - 6;
        }
        for (int i_2 = 1; i_2 < min(-i_0+PB_L, i_0); i_2++) {
            for (int i_3 = 0; i_3 < PB_M; i_3++) {
                for (int i_4 = 1; i_4 < -i_2+PB_L-1; i_4++) {
                    B[i_3][i_2+i_0][i_4-1] = F[i_2][i_3][i_0][i_2] + B[i_4-1][i_3+i_2-1][i_0-2] - B[i_3][i_4+i_2+1][i_0-1] * 4;
                }
            }
        }
    }
...
kernel_4226231215_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
