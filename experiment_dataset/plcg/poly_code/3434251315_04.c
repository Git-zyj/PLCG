...
void kernel_3434251315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = E[i_0] * E[i_0] - F[i_0] * F[i_0-1] + E[i_0] - E[i_0-1] * F[i_0] * B[i_0] - B[i_0+1] * 2;
    }
    for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
        B[i_1] = F[i_1+1] + F[i_1+1] - E[i_1] + E[i_1+1] * B[i_1+1] + 2;
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            B[i_1] = F[i_1] - E[i_1] + B[i_2] - B[i_1+1] - 3;
        }
        for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_P, i_1); i_4++) {
                D[i_1][i_1][i_3] = G[i_3][i_4] - G[i_4][i_3] - H[i_4][i_1-1][i_1] * H[i_3][i_4][1] - I[i_4+1][i_1+1][0] + I[i_3][i_1][i_4-1] * D[i_1-1][i_1-1][i_3+1] - D[i_1+1][i_1+1][i_3-1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3434251315_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
