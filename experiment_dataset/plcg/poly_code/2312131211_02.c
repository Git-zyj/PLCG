...
void kernel_2312131211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            A[i_1+1] = D[i_1][i_0] * D[i_1][i_0] - B[i_0+1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_M-2, -i_2+PB_P); i_3++) {
            B[i_3+1] = D[i_3][i_3+i_2] + D[i_2][i_3-1] * B[i_3+2] * B[i_3] - C[i_2][i_3+1] - 5;
            C[i_2][i_3+1] = D[i_2+1][i_3-1] * D[i_3][i_2] - D[i_2][i_3] - D[i_3][i_2+1] * A[i_3+1] - 2;
        }
    }
#pragma endscop
...
kernel_2312131211_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
