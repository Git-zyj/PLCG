...
void kernel_4422152125_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
    ...
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        ...
        for (int i_1 = 4; i_1 < -i_0+PB_P-1; i_1++) {
            for (int i_2 = 2; i_2 < min(i_0, PB_M-2); i_2++) {
                C[i_2+1][i_1-4][i_0] = F[i_0-2][0][i_1] + F[i_2+2][i_1][i_0+2] * F[i_1][i_0][i_2] * F[i_2+2][i_1+i_0+1][i_2] - F[i_1-1][i_2][i_0+1] * G[i_2-2][i_0-1] * F[i_2][i_0][i_1] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_Q, PB_M-2); i_4++) {
            for (int i_5 = 1; i_5 < i_3; i_5++) {
                C[i_3+1][i_4-2][i_5] = F[i_5][i_3][i_3] + G[i_5][i_4] - G[i_5][2] * F[i_4][i_5][i_3] * G[i_5][i_4] - F[i_4][i_5][i_4] - F[i_4+2][i_5][i_4+i_3] - C[i_3+1][i_5-1][i_4] - C[i_3+1][i_5][i_4] * 4;
            }
        }
        for (int i_6 = 2; i_6 < PB_M; i_6++) {
            A[i_6] = E[i_6-2] - G[i_6][i_3] * G[i_3+1][i_6] * E[i_6-1] * 1;
        }
    }
#pragma endscop
...
kernel_4422152125_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
