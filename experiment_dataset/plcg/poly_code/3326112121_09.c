...
void kernel_3326112121_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-4; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < min(i_1, PB_L-2); i_2++) {
                A[i_0+1][i_1][i_2+2] = D[i_2][i_1+1][i_0] - A[i_1][i_2+1][i_0+4] * 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = i_3; i_5 < PB_L-1; i_5++) {
                A[i_3+1][i_4][i_5+1] = D[i_5][i_3][i_4+1] - 2;
            }
        }
...
kernel_3326112121_09(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
