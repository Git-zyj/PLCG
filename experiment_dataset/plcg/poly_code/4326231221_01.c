...
void kernel_4326231221_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < min(min(-i_0+PB_P+1, i_0), PB_L-1); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_L-1, -i_0+PB_M-1); i_2++) {
                for (int i_3 = 1; i_3 < PB_L; i_3++) {
                    A[i_3][i_2+i_1][i_0][i_0-1] = D[i_2-1][i_0+1][i_3][i_1+i_0-1] * E[i_1][i_2+i_0+1][i_1-1][i_3] - D[i_0][i_3][i_1][i_2] + D[i_0][i_3-1][i_2+i_1+1][i_0] - A[i_1-1][i_3+i_2][i_0][i_0-1] + A[i_1+2][i_3+i_2][i_0][i_0-1] - 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P; i_4++) {
        ...
        for (int i_5 = 1; i_5 < -i_4+PB_M+1; i_5++) {
            for (int i_6 = 0; i_6 < min(PB_P, -i_4+PB_Q+2); i_6++) {
                for (int i_7 = 3; i_7 < min(PB_L-1, -i_6+PB_Q); i_7++) {
                    C[i_5-1][i_7-1][i_4][i_7+i_6] = D[i_5][i_6][i_7+1][0] * E[i_6][i_4-1][i_7-1][i_5] * E[i_6][i_5+i_4-1][i_7][i_6] - C[i_7-3][i_4-1][i_5][i_6+i_4-2] * 6;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), INIT_SEED);
kernel_4326231221_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E)));
