...
void kernel_2222212123_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M-3, -i_0+PB_M+2); i_1++) {
            B[i_0][i_1+3] = C[i_1+i_0-2][2] - 1;
        }
        for (int i_2 = 1; i_2 < PB_M-3; i_2++) {
            B[i_0][i_2+2] = C[i_2-1][i_0+1] + B[i_0][i_2+3] + 5;
        }
    }
#pragma endscop
...
kernel_2222212123_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
