...
void kernel_3322111311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_Q-1, -i_1+PB_Q); i_2++) {
            for (int i_3 = 1; i_3 < i_2; i_3++) {
                B[i_3-1][i_1][i_2] = D[i_2][i_2][i_3] + D[i_1][i_3][i_2+i_1] + B[i_3][i_1][i_2] * B[i_3-1][i_1][i_2-1] * B[i_3-1][i_1][i_2+1] - 6;
            }
        }
        for (int i_4 = 1; i_4 < -i_1+PB_L-2; i_4++) {
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                C[i_1][i_4+i_1][i_4] = C[i_1+1][i_4+i_1][i_4-1] - C[i_1][i_4+i_1+1][i_4+1] * C[i_1+1][i_4+i_1+2][i_4+1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3322111311_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
