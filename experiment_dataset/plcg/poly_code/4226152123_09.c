...
void kernel_4226152123_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_L-2); i_1++) {
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                A[i_0][i_1][i_2][i_1] = C[i_2][i_1][i_0] - D[i_2+1][i_1] * C[i_2][i_1][2] + C[i_2+i_1][i_1+i_0][i_2-1] - C[i_2][i_0][i_1] - C[i_2+i_1][i_0+1][i_1] * A[i_1+2][i_2][i_0][i_2] + 4;
            }
        }
        for (int i_3 = max(1, i_0); i_3 < PB_L; i_3++) {
            for (int i_4 = 2; i_4 < min(PB_L+2, -i_0+PB_M-2); i_4++) {
                B[i_3-1][i_3] = C[i_4+i_0+2][i_3-1][i_4] + C[i_0][i_4-2][i_3] + D[i_4+1][0] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4226152123_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
