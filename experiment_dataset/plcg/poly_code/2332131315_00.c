...
void kernel_2332131315_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
            C[i_1+1] = E[i_2+1][i_1] + D[1] + C[i_2+1] - C[i_2+2] * 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M+1, PB_M-2); i_4++) {
            C[i_4+1] = E[i_4+1][i_3+1] + D[i_3-1] * E[i_3][i_4-1] - E[i_3][i_4] * E[i_4+i_3-1][1] - E[i_3][i_4+1] + C[i_4+2] * C[i_4] * 3;
        }
    }
#pragma endscop
...
kernel_2332131315_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
