...
void kernel_2136252321_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+2; i_0++) {
        for (int i_1 = 4; i_1 < min(i_0, -i_0+PB_N+2); i_1++) {
            A[i_1-2] = B[i_1] + C[i_1+i_0-2][i_1+i_0] + D[i_1] * B[i_0-2] - C[i_1][i_1] + A[i_1-1] - A[i_1-4] * A[i_1-3] - 1;
        }
    }
#pragma endscop
...
kernel_2136252321_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
