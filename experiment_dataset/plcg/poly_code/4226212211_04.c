...
void kernel_4226212211_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_2][i_1-1][0] = C[i_2][i_1-2][i_0-1][i_2] + A[i_2-1][i_1][0] * 5;
            }
        }
        for (int i_3 = 2; i_3 < PB_P+2; i_3++) {
            for (int i_4 = i_0; i_4 < PB_Q-1; i_4++) {
                for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
                    B[i_5+1][i_3-2][i_4][i_0] = B[i_5+2][i_3-2][i_4][i_0] + B[i_5+1][i_3-2][i_0+1][i_4] + B[i_5+1][i_3-2][i_4+1][i_0-1] * 4;
                }
            }
        }
    }
...
kernel_4226212211_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
