...
void kernel_3316251115_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        B[i_0+1] = D[i_0+1] - D[0] + E[i_0+1][0] - D[i_0] * F[i_0] * F[i_0-1] + 2;
        B[i_0+1] = D[i_0] + D[i_0] - 6;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        B[i_1] = D[i_1-1] * D[i_1] + F[i_1] - F[i_1] * D[i_1] + B[i_1+2] - 3;
    }
#pragma endscop
...
kernel_3316251115_07(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
