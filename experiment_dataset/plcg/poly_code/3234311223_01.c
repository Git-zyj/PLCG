...
void kernel_3234311223_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
                B[i_2-1][i_1-1] = B[i_1-1][i_2] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 3; i_4 < PB_M+1; i_4++) {
            for (int i_5 = 1; i_5 < min(PB_M-1, i_3); i_5++) {
                B[i_4-1][i_5] = C[i_5+1][i_4][i_5] * B[i_4-3][i_5-1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3234311223_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
