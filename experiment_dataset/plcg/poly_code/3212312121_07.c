...
void kernel_3212312121_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_P-3; i_1++) {
        for (int i_2 = min(PB_N-2, -i_1+PB_M-4) - 1; i_2 >= 2; i_2--) {
            for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                B[i_2+i_1+2][i_3][i_1+2] = C[i_2-2][i_1][i_3+1] + D[i_2+2][i_1][i_3] - B[i_2+i_1+4][i_3][i_1+3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3212312121_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
