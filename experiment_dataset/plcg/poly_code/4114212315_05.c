...
void kernel_4114212315_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M-3, i_0); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_Q-2, PB_M-3); i_2++) {
                for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                    A[i_1+2][i_3][i_2+i_1] = B[i_1][i_3+1][i_2] * A[i_2+3][i_3-1][i_2+i_1+1] - A[i_1+3][i_3][i_2+i_1+2] + A[i_1+3][i_3+1][i_2+i_1+2] + 4;
                }
            }
        }
    }
...
kernel_4114212315_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
