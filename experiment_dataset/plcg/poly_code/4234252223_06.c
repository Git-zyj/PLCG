...
void kernel_4234252223_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
                A[i_1][i_1+i_0+2][i_2] = C[i_1][i_0+1][i_2-1] - D[i_1-2][i_0][i_2] * D[i_0+1][i_2][i_1] + C[i_0][i_1-2][i_2] + C[i_2][i_1+i_0+1][0] + D[i_1-2][i_2][i_0] - D[i_2][i_0][i_1] - C[i_2+1][i_2-2][i_1] + A[i_1][i_1+i_0+1][i_2] + A[i_1][i_1+i_0][i_2] + 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M-1; i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                B[i_3+1][i_4] = C[i_5][i_3][i_4] - A[i_3][i_4+i_3+1][i_5] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4234252223_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
