...
void kernel_4236312111_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_N+1; i_3++) {
            for (int i_4 = max(2, i_2); i_4 < PB_M; i_4++) {
                B[i_3][i_4] = C[i_3][i_2][i_4] - C[i_3+i_2-1][i_4-2][1] - B[i_3][i_4+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4236312111_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
