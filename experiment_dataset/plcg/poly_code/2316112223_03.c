...
void kernel_2316112223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_M+1, -i_2+PB_L); i_3++) {
            B[i_2] = D[i_2][i_3+i_2] + B[i_3-1] * B[i_2+2] + 3;
        }
        for (int i_4 = 2; i_4 < PB_M+2; i_4++) {
            C[i_2] = D[i_4-2][i_2] + C[i_4+1] - C[i_4-2] - 4;
        }
    }
#pragma endscop
...
kernel_2316112223_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
