...
void kernel_3232211215_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_M-1); i_1++) {
            B[i_1+1][1][i_0-1] = C[i_0][i_1] * B[i_1][1][i_0-1] - 6;
            for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
                B[i_1][1][i_2-1] = B[i_1][1][i_0-2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3232211215_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
