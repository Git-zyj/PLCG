...
void kernel_4116232121_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
        ...
        for (int i_1 = 1; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                for (int i_3 = 3; i_3 < PB_L-1; i_3++) {
                    A[i_1][i_2][i_0-2] = B[i_2+2][i_3][i_1][i_0] * C[i_1-1][i_1][i_0][i_2] - C[i_1][i_2+i_0][i_3+1][i_1+2] * A[i_1][i_2][i_3-3] * 1;
                }
            }
        }
    }
...
kernel_4116232121_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
