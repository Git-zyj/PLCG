...
void kernel_4234152111_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_L, -i_1+PB_N); i_3++) {
                    A[i_3+i_1][i_1-2][i_2] = C[i_1+2][0][i_2][i_0] - D[i_0][i_2][i_1] - C[i_1+2][i_3+i_0][i_0][i_2] - D[i_3][i_2+1][i_1] * C[i_2+i_1][i_0+2][i_3+2][0] - C[i_0-2][i_2][i_1][0] - C[i_2][i_1][i_3][i_0] - A[i_3+i_1][i_1-1][i_2] - 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_P-2; i_5++) {
            for (int i_6 = max(3, i_5); i_6 < -i_4+PB_M-2; i_6++) {
                for (int i_7 = 0; i_7 < -i_6+PB_N+1; i_7++) {
                    B[i_5][i_6+1][i_6] = C[i_7][i_5-2][i_7][i_6] + C[i_6][i_7][i_5+2][0] - C[i_6+i_4+2][i_7][i_5][i_6-2] + A[i_7+i_6-1][i_6-3][i_5] - 4;
                }
            }
        }
    }
...
kernel_4234152111_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
