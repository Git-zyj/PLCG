...
void kernel_2334132225_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M+2; i_0++) {
        A[i_0-2] = D[i_0-2] + A[i_0-4] * A[i_0-3] * 4;
    }
    ...
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, PB_M+2); i_3++) {
            C[i_3+2] = D[i_2] + E[i_3][i_2] - E[i_3][i_2] - E[i_3][i_2+1] - E[i_3][i_2] + A[i_3-2] - 6;
        }
    }
#pragma endscop
...
kernel_2334132225_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
