...
void kernel_2332132323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = D[i_0] - D[0] + D[i_0+1] - A[i_0+4] + A[i_0+1] * 4;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        A[i_1+2] = D[i_1] + D[i_1] - D[i_1+2] * D[i_1] + 3;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L-3, i_2); i_3++) {
            C[i_3+2] = E[i_3-2][i_2] - D[i_3] * C[i_3+3] - C[i_3+1] - C[i_3] + C[i_2] * 6;
        }
    }
#pragma endscop
...
kernel_2332132323_05(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
