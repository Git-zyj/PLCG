...
void kernel_4224131213_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_M, -i_1+PB_L); i_2++) {
            for (int i_3 = 1; i_3 < i_1; i_3++) {
                B[0][i_2+i_1][i_3] = C[i_3][i_1-1][i_3] * D[i_3-1][i_3+1] * C[i_1+1][i_3][i_2] - C[i_3][i_1][0] - C[i_3+1][i_1][i_2] + B[0][i_2+i_1][i_3+1] - B[0][i_2+i_1-1][i_3-1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4224131213_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
