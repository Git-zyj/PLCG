...
void kernel_2434252223_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_N); i_1++) {
            A[i_1][i_1] = E[i_1+i_0] * F[i_1] + G[i_1][i_0] - H[i_1][i_0+1] - G[i_1][i_0] * H[i_1-1][i_0-2] + A[i_1+1][i_1+1] + A[i_1+2][i_1+2] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        A[i_2][i_2] = F[i_2-1] - F[i_2-2] - F[i_2] + F[i_2] * F[i_2] - F[i_2] * E[i_2] + 3;
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            A[i_3][i_3] = H[i_2][i_3-1] - G[i_3][i_2+1] - G[i_3][i_2] + G[i_2+2][i_3] * 3;
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < PB_L; i_5++) {
            A[i_5-2][i_5-2] = I[i_5][i_4-1][1] * H[i_5+1][i_4] + 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2434252223_02(xa,ya,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
