...
void kernel_3432131221_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_1+i_0][i_2+1][0] = A[i_1+i_0][i_2][0] * 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            B[1] = E[i_3][i_4] * F[i_4] * E[i_3][i_4] + E[i_4+1][i_3-1] - E[i_4][i_3-1] * E[i_4][i_3] - B[1] * B[1] - 6;
        }
        for (int i_5 = i_3; i_5 < PB_M-2; i_5++) {
            C[i_3][i_5] = F[i_5] * E[i_5+1][i_3] * F[1] - C[i_3][i_5+2] + C[i_3+2][i_5] + 1;
        }
        for (int i_6 = 1; i_6 < PB_M-2; i_6++) {
            for (int i_7 = 1; i_7 < PB_M; i_7++) {
                D[i_7][i_3][i_6-1][i_6] = G[i_3][i_6][i_6+1][i_3] - D[i_6+2][i_3][i_7-1][i_7] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3432131221_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
