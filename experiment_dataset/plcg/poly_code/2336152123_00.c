...
void kernel_2336152123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < min(-i_0+PB_M, PB_P-2); i_1++) {
            A[0][i_1-2] = D[i_1+i_0][i_1+2] - D[i_1][i_0] + A[0][i_1-3] * 1;
            B[i_0][i_1] = D[i_1][i_0] * D[i_1+i_0][0] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
        C[i_2] = E[i_2+2] * E[i_2+2] - E[i_2] - E[i_2] * E[i_2] - E[i_2+1] * E[i_2] - E[i_2] - E[i_2] + 5;
    }
#pragma endscop
...
kernel_2336152123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
