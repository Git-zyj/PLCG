...
void kernel_3226312113_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        ...
        for (int i_3 = 0; i_3 < min(-i_0+PB_N-1, i_0); i_3++) {
            B[i_3+i_0][i_0+1] = C[i_3][i_3] + B[i_3+i_0+1][i_0+2] - 5;
        }
    }
#pragma endscop
...
kernel_3226312113_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
