...
void kernel_4432352325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M+1; i_1++) {
            A[i_0][i_1] = E[i_1-1][i_0-1] + F[i_0+2][i_1] + E[i_0][i_1+i_0-2] - E[i_1+i_0-1][i_0-2] - E[i_0][0] + A[i_0][i_1+2] + A[i_0+2][i_1+1] * 1;
        }
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            for (int i_3 = i_0; i_3 < min(min(-i_2+PB_L+1, -i_0+PB_L), PB_M-2); i_3++) {
                for (int i_4 = 2; i_4 < min(min(-i_3+PB_P, -i_0+PB_L), -i_2+PB_M); i_4++) {
                    B[i_2][i_3+i_2-1][i_4] = G[i_3][i_4+i_3][i_0+1][i_2] + H[i_3][i_0][i_2-2][i_4-2] - I[i_3+2][i_4+i_2][i_0] - H[i_2-2][i_0][i_4][i_2] - I[i_0-2][i_4][i_3] * H[i_2-1][i_4+i_0][i_3][i_2] + B[i_2][i_3+i_2-1][i_4+2] * B[i_0][i_3+i_0][i_4] + B[i_3][i_3+i_0-1][i_4+2] - 4;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = 2; i_6 < PB_M; i_6++) {
            C[i_5][i_6] = F[i_6-2][i_6] + J[i_5][i_6] + E[i_6-2][i_5] * J[i_6][i_5] + K[i_6] * 4;
            for (int i_7 = 1; i_7 < PB_P-1; i_7++) {
                C[i_5][i_7+1] = L[i_5-1][i_7][i_6] * C[i_6][i_7-1] * C[i_5][i_7] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4432352325_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
