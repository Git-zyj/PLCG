...
void kernel_2234231121_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        for (int i_3 = min(PB_M, -i_2+PB_N) - 1; i_3 >= i_2; i_3--) {
            B[i_3][i_2-1] = C[i_3][i_2+1] * D[i_3+1][i_2] * C[i_3][i_2] + C[i_2][i_3] * E[i_3+i_2] - B[i_3+2][i_2-3] * 3;
        }
    }
#pragma endscop
...
kernel_2234231121_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
