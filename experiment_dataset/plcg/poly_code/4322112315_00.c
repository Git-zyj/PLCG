...
void kernel_4322112315_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_M+2); i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
                A[i_2+i_1-1][i_2] = D[i_2][i_1] + A[i_1+i_0-2][i_0-1] - A[i_2+i_1][i_1+1] * A[i_2+i_0][i_0+1] * A[i_2+i_1-2][i_2-1] + 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_N; i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_N; i_5++) {
                B[i_5][i_5+i_4] = B[i_5][i_5+i_4-1] - 4;
            }
        }
...
kernel_4322112315_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
