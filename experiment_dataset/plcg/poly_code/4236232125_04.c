...
void kernel_4236232125_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 3; i_1 < PB_P; i_1++) {
            for (int i_2 = i_1; i_2 < min(PB_P-1, -i_1+PB_M+2); i_2++) {
                A[i_0][i_1][i_2+i_1-2] = C[i_0+2][i_2+1][i_0][i_1] - D[i_1+1][i_0][i_1] + A[i_0][i_1-1][i_2+i_1-3] * 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = PB_M+2 - 1; i_4 >= max(2, i_3); i_4--) {
            B[i_3][i_3] = E[i_3] * F[i_3][i_4] + F[i_4-2][i_3+2] + B[i_3+1][i_3+1] * 1;
        }
    }
#pragma endscop
...
kernel_4236232125_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
