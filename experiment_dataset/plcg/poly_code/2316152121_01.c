...
void kernel_2316152121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1] = D[i_1+i_0][0] * D[i_1][i_0+1] * D[i_0][2] + D[i_1][i_0-1] + E[i_1+i_0] + D[i_1+i_0-2][i_0] - 3;
        }
        for (int i_2 = min(-i_0+PB_M, i_0) - 1; i_2 >= 0; i_2--) {
            B[i_0][i_2] = D[i_2+i_0][i_2] * D[i_2+1][0] + E[i_2+i_0] + C[i_2] - 3;
        }
        C[i_0] = E[i_0] + E[i_0] - E[i_0+2] * E[i_0] + E[i_0-2] + E[i_0] - 3;
    }
#pragma endscop
...
kernel_2316152121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
