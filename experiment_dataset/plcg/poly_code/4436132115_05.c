...
void kernel_4436132115_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < min(-i_1+PB_L, -i_0+PB_P); i_2++) {
                for (int i_3 = 0; i_3 < min(min(min(-i_2+PB_P+1, PB_N-2), -i_0+PB_P+2), -i_1+PB_L+1); i_3++) {
                    A[i_0][i_2+i_1][i_3+i_2-1][i_2+i_0] = E[i_3+2][i_1-1][i_2-1][i_0+2] + A[i_2-1][i_3+i_1-1][i_3+i_0-2][i_3+i_2-2] * 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M; i_5++) {
            B[i_4][i_5] = F[i_4] * F[i_5] - B[i_4-1][i_5-1] * 6;
        }
    }
    for (int i_6 = 2; i_6 < PB_P-1; i_6++) {
        for (int i_7 = 0; i_7 < min(i_6, -i_6+PB_Q); i_7++) {
            for (int i_8 = 2; i_8 < min(min(PB_N-2, -i_7+PB_L), -i_6+PB_P+2); i_8++) {
                for (int i_9 = 0; i_9 < -i_8+PB_P; i_9++) {
                    A[i_9][i_8+i_7][i_8+i_6-2][i_9+i_8] = E[i_8-2][i_9][i_6][i_7+i_6] - E[i_8+2][i_6][i_7+1][i_7] * 6;
                }
            }
        }
        for (int j_0 = 2; j_0 < PB_M; j_0++) {
            B[i_6][j_0] = G[j_0-2][i_6+1] + F[i_6+1] - H[j_0-1][i_6][j_0-2] * G[j_0-1][i_6] + 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436132115_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
