...
void kernel_3432351323_03(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P-1, -i_0+PB_L-3); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_N, PB_M-1); i_2++) {
                C[i_2-1][i_1+i_0+3][i_1+1] = E[i_2+1][i_1+1] + F[i_2+i_0][i_1][i_1+1] - G[i_2+1][i_1-1] * G[i_1][i_2] * 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P; i_3++) {
        B[i_3+1] = H[i_3] + I[i_3] + J[0] * J[i_3] * J[i_3] - I[i_3] - J[i_3] * B[i_3+2] - B[i_3-1] - B[i_3+3] - 6;
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < min(PB_P-2, -i_4+PB_M+1); i_5++) {
                C[i_3-1][i_5+i_4][i_5] = K[i_4][i_3-1][1] * L[i_3][i_4][i_5] - K[i_5+1][i_3+1][i_4] - E[i_5+i_4-1][i_3] - C[i_3-1][i_5+i_4+1][i_5+2] + C[i_3-1][i_5+i_4+1][i_5] + C[i_3-2][i_5+i_4][i_5] - 5;
            }
        }
        for (int i_6 = 0; i_6 < PB_M; i_6++) {
            for (int i_7 = 1; i_7 < min(PB_P, -i_6+PB_L); i_7++) {
                C[i_3-2][i_7+i_6][i_7] = F[i_7][i_3][i_6] * L[i_7-1][i_6][i_3] + 5;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,zc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_3432351323_03(xc,yc,zc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xc,yc,zc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L)));
