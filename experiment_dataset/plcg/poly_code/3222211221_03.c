...
void kernel_3222211221_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_L); i_2++) {
            B[i_2+1][i_0-1] = C[i_0][1] - C[i_2+i_0][0] * B[i_2][i_0-1] * B[i_2+1][i_0-2] + 2;
        }
    }
#pragma endscop
...
kernel_3222211221_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
