...
void kernel_2216132113_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_L+2; i_1++) {
            A[i_0][i_1+2] = C[i_0][i_1-2] * D[i_1] * A[i_0-1][i_1+2] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        B[i_2] = D[i_2] - D[i_2] * D[i_2+1] + D[i_2] + B[i_2+1] + 2;
    }
#pragma endscop
...
kernel_2216132113_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
