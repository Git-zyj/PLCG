...
void kernel_2432232311_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L-2; i_1++) {
            A[i_1][i_0+1] = E[i_0] + F[i_1+i_0+2][i_0] + F[i_1+i_0][i_0] + A[i_1-1][i_0+1] * B[i_1+i_0][i_1+2] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, -i_2+PB_N-1); i_3++) {
            B[i_3+i_2][i_3+2] = F[i_3][i_2] * C[i_3+i_2][0] - C[i_3+i_2-1][0] + C[i_3+i_2+1][0] + C[i_3+i_2+1][0] * 2;
            C[i_3+i_2][0] = G[i_3-1][i_2] * F[i_3][i_2] - F[i_3+i_2-2][i_3-2] - B[i_3+i_2][i_3+2] + 5;
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < min(PB_L-1, -i_4+PB_L); i_5++) {
            D[i_4+2][i_5] = G[i_5+i_4][i_5+i_4] * G[i_5][i_5+i_4] + E[i_5] + G[i_4][i_5+1] + F[i_5][i_4] + D[i_4+3][i_5] * D[i_5+1][i_4] - D[i_4+1][i_5] * 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_2432232311_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
