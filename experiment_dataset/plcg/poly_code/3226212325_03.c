...
void kernel_3226212325_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-3; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P-3; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
                B[i_2+1][i_1+i_0+3][i_2-1] = B[i_2+3][i_1+i_0+2][i_2+1] - 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-4; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_P-2, i_3); i_4++) {
            for (int i_5 = 0; i_5 < min(i_4, -i_3+PB_P-4); i_5++) {
                B[i_5+2][i_4+i_3+2][i_5] = C[i_5][i_4+i_3+2][0] * B[i_4+2][i_5+i_3+4][i_4] + B[i_5+2][i_4+i_3+1][i_5] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3226212325_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
