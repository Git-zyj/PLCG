...
void kernel_3224311313_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-2; i_2++) {
                A[i_1-1][i_0][i_2+1] = A[i_1-1][i_0][i_2+2] * A[i_1-1][i_0][i_2] + A[i_1-1][i_0-1][i_2+1] + A[i_1-1][i_0+1][i_2+1] * A[i_1-2][i_0][i_2+1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        B[i_3+1] = C[i_3] * D[0] - B[i_3+2] - 5;
    }
#pragma endscop
...
kernel_3224311313_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
