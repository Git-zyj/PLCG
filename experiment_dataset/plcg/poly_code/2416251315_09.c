...
void kernel_2416251315_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L+1; i_1++) {
            B[i_1] = E[i_0] * F[i_1-1][i_1+i_0-1] - B[i_1-1] * C[i_1+i_0][i_0-1] - 4;
        }
        B[i_0] = F[i_0+1][i_0] * E[i_0] - E[i_0] - G[i_0-1] * E[i_0-1] * G[i_0] * G[i_0] - 2;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, -i_2+PB_M-1); i_3++) {
            C[i_3+i_2][i_2-1] = F[i_3][i_2-1] - H[i_2-1][i_2][i_3-1] * F[i_3-1][i_2] * F[i_3+i_2][1] - C[i_3+i_2-1][i_2-2] + C[i_3+i_2-1][i_2-1] * C[i_3+i_2+1][i_2] * C[i_3+i_2][i_2] + C[i_3+i_2-2][i_3-2] * C[i_3+i_2+1][i_2-1] + 1;
        }
        D[i_2-1] = E[i_2] * G[i_2-1] * G[i_2-1] * G[i_2] * G[i_2] * E[i_2] * E[i_2] - 4;
    }
#pragma endscop
...
kernel_2416251315_09(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
