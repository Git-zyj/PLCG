...
void kernel_4236131123_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = i_1; i_2 < PB_N-1; i_2++) {
                A[i_0][i_2][1] = C[i_2+1][i_1] - C[i_1][i_2] + D[i_1][i_2][i_0+1] - 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q-3; i_3++) {
        for (int i_4 = i_3; i_4 < PB_P-1; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_M, PB_L); i_5++) {
                B[i_5][i_4+1][i_3+1] = D[i_4+1][i_5+i_3][i_5] + D[i_4+1][i_5][i_3-1] + B[i_5][i_4+1][i_3+3] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4236131123_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
