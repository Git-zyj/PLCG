...
void kernel_4412352323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_N, PB_L-2); i_1++) {
            A[i_1+1] = E[i_0][i_1+2] - F[i_0][i_1] * E[i_1][i_0+2] + F[i_0-2][i_1+1] + G[i_1+i_0][i_0] + A[i_1+2] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
            for (int i_4 = i_3; i_4 < min(-i_3+PB_L-2, -i_2+PB_M); i_4++) {
                for (int i_5 = i_3; i_5 < PB_P; i_5++) {
                    D[i_3-2][i_3][i_2-1][i_4] = H[i_5][i_2][i_4+1][i_2+1] + I[i_4+i_3+2][i_5][i_2][i_2] * C[i_4+i_2][i_3][i_2] + 3;
                }
            }
        }
        for (int i_6 = 2; i_6 < -i_2+PB_L-1; i_6++) {
            for (int i_7 = 2; i_7 < PB_L; i_7++) {
                C[i_6+i_2][i_7][i_2] = J[i_7-2][i_2-2][i_6] + K[i_6][i_7][i_2] + K[i_6][i_7][i_6+i_2+1] * A[i_6+1] * A[i_7-1] + A[i_7+1] * C[i_6+i_2][i_7-1][i_2] + C[i_7+i_6-2][i_2][i_7-2] - 2;
            }
            for (int i_8 = 2; i_8 < -i_6+PB_L; i_8++) {
                for (int i_9 = 0; i_9 < -i_2+PB_L; i_9++) {
                    D[i_8-2][i_8][i_2][i_9] = L[i_8-1][i_6+2][i_9+i_2][i_8] * L[i_9+i_6][i_2+2][0][i_8] - L[i_9][i_8][i_2][i_8+i_6] + L[i_9][i_8][i_6][i_2] * K[i_8][i_9+i_2-1][i_8+i_6] - I[i_8+i_6-2][i_9+i_2][i_6-1][i_8] - I[i_2][i_9][i_2][i_8+i_6] + 1;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4412352323_04(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L)));
