...
void kernel_3226212313_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, PB_M-2); i_3++) {
            for (int i_4 = 2; i_4 < min(PB_L-2, i_3); i_4++) {
                B[i_3+1][i_4-2] = C[i_3][i_4+2][2] * B[i_3][i_4-2] - B[i_3+2][i_4-2] + B[i_3+2][i_4-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3226212313_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
