...
void kernel_2436332313_08(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        D[i_0] = E[i_0+1] * E[i_0] * F[1] + 2;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            B[i_2][i_1] = G[i_2-1][0] + B[i_2-1][i_1+1] + B[i_2+1][i_1] * 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 2; i_4 < PB_N; i_4++) {
            C[i_3][i_4] = H[i_3][i_4-2] * I[i_3+1][0] + C[i_4][i_3-1] + C[i_3-1][i_4] - C[i_3][i_4+1] - 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
        D[i_5] = E[i_5+2] - J[i_5] * J[i_5-1] * D[i_5+1] * D[i_5-1] + 3;
    }
#pragma endscop
...
kernel_2436332313_08(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
