...
void kernel_4424131215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        A[i_1+1] = E[i_1] * E[i_1-1] * E[i_1] * 6;
        for (int i_2 = max(2, i_1); i_2 < PB_M-2; i_2++) {
            A[i_2+1] = E[i_1] + F[i_1-1][i_2] + F[i_1][1] - F[i_2][i_1] + 3;
            for (int i_3 = 2; i_3 < PB_L; i_3++) {
                for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
                    D[i_2+1][0][i_1+1][i_4-1] = G[i_1-1][i_2][i_3+1][i_4] * G[i_4+1][i_3][i_2+1][i_1+1] + G[i_2][i_3-1][i_4][i_1-1] * D[i_2][0][i_1+1][i_4-1] + D[i_4+1][0][i_1+1][i_2-2] - D[i_2+2][0][i_1+1][i_3-2] * D[i_1][0][i_2+1][i_4] * 1;
                }
            }
        }
    }
...
kernel_4424131215_02(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
