...
void kernel_2322211215_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P+2; i_0++) {
        for (int i_1 = 2; i_1 < PB_P+2; i_1++) {
            B[i_1-1][i_0-2] = B[i_0-2][i_1-2] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_2-1][i_3-1] = D[i_2+1][i_3-1] - B[i_2][i_3-1] - B[i_3-1][i_2] * 6;
        }
        C[i_2] = C[i_2+1] + 1;
    }
#pragma endscop
...
kernel_2322211215_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
