...
void kernel_3416352323_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_N+1; i_1++) {
            B[i_1-1] = E[i_0-2][i_1-1] + F[i_1-1] + G[i_1][i_0] + G[i_1][i_0] + B[i_1-1] - 6;
        }
        B[i_0] = F[i_0] - F[0] + B[i_0-1] + 4;
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < PB_P-2; i_3++) {
            D[i_3-1] = H[i_3][i_2] + I[i_3-1][i_3][i_2] * J[i_3] - G[i_3][i_2] + G[i_2][i_3+2] * H[i_3][i_2] * H[i_3][i_2] + D[i_3] * D[i_3-2] - 1;
        }
        D[i_2] = K[i_2-2] - J[i_2] * K[i_2-1] * K[i_2] - F[i_2] + D[i_2+1] - 1;
    }
#pragma endscop
...
kernel_3416352323_06(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
