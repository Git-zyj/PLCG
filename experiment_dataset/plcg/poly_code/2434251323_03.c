...
void kernel_2434251323_03(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        C[i_0-1] = E[i_0-1] * E[i_0] - 6;
        C[i_0+1] = F[i_0] + E[i_0] - E[i_0] + F[i_0] * E[i_0] + F[i_0] - F[i_0] + 4;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            C[i_2-1] = G[i_1][i_2+1] + F[0] * H[i_1][i_2+i_1] - E[i_1] + H[i_1-1][i_2] - I[i_1][1][i_2+1] + C[i_1-2] + C[i_2] - D[i_2-1] * 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
        for (int i_4 = 2; i_4 < PB_N-1; i_4++) {
            D[i_4-1] = H[i_4+1][i_3-1] - G[i_4+1][i_3-1] + D[i_4+1] * D[i_4-2] - 5;
        }
    }
#pragma endscop
...
kernel_2434251323_03(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
