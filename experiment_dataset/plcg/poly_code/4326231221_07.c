...
void kernel_4326231221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] * D[i_0] + E[i_0] + E[i_0] + E[i_0] * A[i_0+2] * 2;
        for (int i_1 = 1; i_1 < min(i_0, PB_M-1); i_1++) {
            B[i_1+1] = E[i_0] + F[i_1+1][i_0] + G[i_1][i_0-1] - A[i_1] + A[i_1-1] * 6;
        }
        for (int i_2 = 0; i_2 < min(PB_P-2, i_0); i_2++) {
            for (int i_3 = 0; i_3 < min(-i_2+PB_P-2, i_2); i_3++) {
                C[i_3][i_0-1][i_3+i_2] = C[i_3][i_0-1][i_3+i_2+2] * C[i_3+2][i_0-1][i_3+i_2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4326231221_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
