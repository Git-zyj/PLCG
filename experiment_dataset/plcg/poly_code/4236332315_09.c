...
void kernel_4236332315_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(min(-i_0+PB_L, i_0), PB_M-1); i_1++) {
            B[i_1] = C[i_1][i_0] * C[i_1-1][i_0] - D[i_0][0] + C[i_1][i_1+i_0] * C[i_0][i_1+1] * B[i_1+1] - 4;
        }
        for (int i_2 = 2; i_2 < min(-i_0+PB_P, PB_M-1); i_2++) {
            B[i_2] = E[i_2-2][i_2+i_0][i_0] * B[i_2+1] - B[i_2-1] + 1;
        }
    }
#pragma endscop
...
kernel_4236332315_09(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
