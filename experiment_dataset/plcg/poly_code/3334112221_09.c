...
void kernel_3334112221_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_L, -i_0+PB_P-1); i_1++) {
            B[i_1+1][i_1+i_0+1] = D[i_0-2][i_1] - E[i_1-1] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-3; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_P-3; i_3++) {
            B[i_2][i_3+i_2] = B[i_2][i_3+i_2-1] + B[i_3+2][i_3+i_2+3] + 5;
        }
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 2; i_5 < PB_P-2; i_5++) {
                C[i_2][i_4][i_5] = C[i_5-2][i_4][i_2] - C[i_2][i_4][i_5+2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3334112221_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
