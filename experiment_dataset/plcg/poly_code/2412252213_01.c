...
void kernel_2412252213_01(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        C[i_0-1] = E[i_0] * E[i_0] * F[i_0+2] + F[i_0] - F[i_0] - F[0] + C[i_0] - 3;
        for (int i_1 = 2; i_1 < min(PB_P+1, -i_0+PB_L); i_1++) {
            B[i_1][i_1+i_0] = G[i_1-1][0] * E[i_0] - G[i_1][i_1] * G[i_1][i_0+1] + G[i_1][i_0] + H[i_0+2][i_1-2] * G[i_1-1][i_1+2] + I[i_0][i_1-1][i_1+1] * C[i_1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        C[i_2] = E[i_2] * F[i_2+1] + F[i_2] - C[i_2+1] * 2;
        for (int i_3 = 2; i_3 < -i_2+PB_M-2; i_3++) {
            C[i_3] = E[i_3-2] * H[i_3+i_2+2][2] - H[i_3][1] + C[i_2-1] - 6;
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2412252213_01(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
