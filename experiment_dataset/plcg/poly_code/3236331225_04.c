...
void kernel_3236331225_04(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_Q-2, -i_1+PB_N); i_2++) {
                B[i_1+i_0+1][i_1-1][i_0][i_2+2] = C[i_0][i_1][i_2] * D[i_1][i_0-1][i_2+1] * D[i_2+i_1][i_0][i_1+i_0] - 3;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_L+3; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_L, -i_3+PB_M-1); i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                B[i_4+i_3+1][i_4-1][i_3][i_5] = C[i_5-1][i_4+i_3][i_4] + D[i_4][i_5][i_5] * D[i_4][i_5][i_4] - B[i_4+i_3-1][i_3-3][i_4][i_5] + B[i_4+i_3+1][i_4-1][i_3][i_5-1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3236331225_04(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
