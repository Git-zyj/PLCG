...
void kernel_2316112325_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            B[i_1][i_0+1] = D[i_0+2][i_1] - B[i_1][i_0] * B[i_1][i_0+1] + B[i_1+2][i_0+1] - B[i_1][i_0+3] + 2;
            B[i_1][i_0+1] = D[i_1][i_0] * D[i_0][i_1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < PB_N-2; i_3++) {
            C[i_2] = C[i_3-2] + C[i_3+2] + 4;
        }
    }
#pragma endscop
...
kernel_2316112325_09(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
