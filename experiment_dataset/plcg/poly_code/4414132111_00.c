...
void kernel_4414132111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_0+1][i_1] = E[i_1+1] - F[i_1][i_0] * F[i_1][i_0] + F[i_1][i_0] * A[i_0+1][i_1-1] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_2+PB_L, PB_M); i_4++) {
                A[i_3+2][i_4] = G[i_4][i_2][i_2] - G[i_3+2][i_4+i_2][0] - 3;
            }
        }
        for (int i_5 = i_2; i_5 < PB_L-1; i_5++) {
            for (int i_6 = 2; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 0; i_7 < min(min(-i_2+PB_L-1, -i_6+PB_M+2), -i_5+PB_L-1); i_7++) {
                    A[i_6+1][i_7+1] = G[i_7+i_6-2][i_7+i_5+1][i_2+1] - G[i_6-1][i_7][i_2] - H[i_6][i_7+i_2+1][i_5][0] + 1;
                }
            }
            for (int i_8 = max(1, i_2); i_8 < PB_L; i_8++) {
                A[i_2+1][i_8-1] = G[i_5][i_8][i_5] + 3;
            }
        }
    }
#pragma endscop
...
kernel_4414132111_00(xa,ya,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
