...
void kernel_4316111213_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1] = D[i_1][i_0-1][i_0] - A[i_0+1] - A[i_1-1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_P-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
                B[i_4][i_3][0] = B[i_4][i_3-1][0] + B[i_4+1][i_3+1][0] * 2;
            }
        }
        C[i_2+1] = C[i_2] - 6;
    }
...
kernel_4316111213_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
