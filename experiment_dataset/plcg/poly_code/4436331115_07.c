...
void kernel_4436331115_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
            B[i_0-1][i_1+1] = E[i_1][i_0-1] + F[i_0][i_0+1] + 6;
        }
        for (int i_2 = 2; i_2 < min(-i_0+PB_M, i_0); i_2++) {
            B[i_0-1][i_2+1] = G[i_2+i_0+1][i_0] - E[i_2][i_0] - F[i_2][i_0] * E[i_2+i_0][1] + G[i_2][i_0+1] + H[i_2] - B[i_2-2][i_0+1] + 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_Q; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M-1; i_4++) {
            B[i_4-1][i_3] = E[i_4+1][i_3-1] - 5;
        }
    }
    for (int i_5 = 0; i_5 < PB_P; i_5++) {
        for (int i_6 = 1; i_6 < PB_M; i_6++) {
            for (int i_7 = 0; i_7 < PB_Q-2; i_7++) {
                B[i_6-1][i_7+2] = I[i_6][i_7][i_5] - 6;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4436331115_07(xb,yb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
