...
void kernel_4416211125_08(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = 0; i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = i_0; i_3 < PB_P; i_3++) {
                    C[i_1+i_0+2][i_2][i_0+1] = E[i_2][i_0][i_1+1][1] - F[i_2][i_3][i_1][i_0-1] * 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_P; i_5++) {
            for (int i_6 = 0; i_6 < min(PB_Q, -i_5+PB_M); i_6++) {
                C[i_6+i_5][i_4+1][i_6] = G[i_4+1][i_6][i_5] - 1;
            }
        }
        for (int i_7 = 1; i_7 < PB_M; i_7++) {
            for (int i_8 = 0; i_8 < min(PB_Q, -i_7+PB_M); i_8++) {
                C[i_8+i_7][i_4][i_8] = C[i_8+i_7][i_4+1][i_8] * 4;
            }
            for (int i_9 = 0; i_9 < min(PB_Q, -i_7+PB_M+1); i_9++) {
                C[i_9+i_7-1][i_4+1][i_9] = 2;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4416211125_08(xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
