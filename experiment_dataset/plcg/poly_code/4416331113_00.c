...
void kernel_4416331113_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
        ...
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M+1, i_1); i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    A[i_2+i_1-1][i_2-1][i_3] = E[i_3][i_1+1][i_2][i_0] + 2;
                }
            }
            for (int i_4 = 0; i_4 < min(min(-i_0+PB_P, -i_1+PB_L), i_1); i_4++) {
                for (int i_5 = 1; i_5 < min(PB_Q-1, -i_1+PB_M+1); i_5++) {
                    A[i_5+i_1-1][i_5-1][i_0] = F[i_1][i_4+i_0][i_5+1][1] - G[i_4][i_0][i_5][i_1] + H[i_5-1][i_4+i_1][i_0-1][i_0][1] + 3;
                }
            }
        }
        C[i_0+1] = I[0] + J[i_0-1] + K[i_0] - C[i_0+2] * C[i_0] * 1;
    }
    for (int i_6 = 1; i_6 < PB_P; i_6++) {
        for (int i_7 = 1; i_7 < min(i_6, -i_6+PB_P); i_7++) {
            for (int i_8 = 1; i_8 < min(PB_P-1, -i_6+PB_Q+1); i_8++) {
                for (int i_9 = 1; i_9 < PB_M-1; i_9++) {
                    A[i_9+i_8][i_8-1][i_6] = L[i_9][i_8-1][i_7+i_6] - M[i_9][i_8+1][i_7-1] * F[i_7+1][i_9-1][i_7][i_8+i_6-1] * E[i_9+1][i_8][i_7][i_6] + 1;
                }
            }
        }
    }
