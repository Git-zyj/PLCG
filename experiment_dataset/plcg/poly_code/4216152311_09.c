...
void kernel_4216152311_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0] = C[i_0] + C[i_0] - C[i_0-1] - C[i_0-1] - C[i_0] * C[i_0] - A[i_0-1] - A[i_0+1] * A[i_0+1] + 1;
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
                B[i_2][i_0][i_2][i_1] = D[i_1-1][i_0+2] * E[i_1-1][i_2+1][i_1] - E[i_1][i_0][0] - E[i_2-2][i_1+i_0][0] + B[i_2-1][i_1][i_2-1][i_0+1] * B[i_2][i_1+1][i_2][i_0] - B[i_2-1][i_0][i_2-1][i_1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4216152311_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
