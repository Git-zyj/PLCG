...
void kernel_3412251311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = E[i_0] + F[i_0] * F[i_0] * F[i_0-1] + G[i_0+1][i_0-1] * G[i_0+1][0] + F[i_0] + H[i_0][0] * A[i_0] * 2;
        B[i_0] = F[i_0] - E[i_0] + E[i_0] * F[i_0-1] - E[i_0+1] * B[i_0-1] * B[i_0+1] + 6;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_P); i_2++) {
            D[i_1] = H[i_2+i_1][i_1] + I[i_2][i_1][0] - H[i_2][i_1] - D[i_1] * 3;
        }
        D[i_1] = E[i_1-1] - E[i_1] - F[i_1] - E[i_1+1] * A[i_1-1] - D[i_1+1] - D[i_1-1] + 3;
    }
#pragma endscop
...
kernel_3412251311_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
