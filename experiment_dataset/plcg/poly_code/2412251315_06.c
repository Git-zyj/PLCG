...
void kernel_2412251315_06(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_N-1, -i_0+PB_L+1); i_1++) {
            D[i_1+1] = E[i_0][1] * E[i_0][i_1] * F[i_1+1] * E[i_1+i_0-1][0] - E[i_1][i_0] + 6;
        }
        for (int i_2 = i_0; i_2 < PB_N; i_2++) {
            B[i_0][i_2] = G[i_2-1][0] - H[i_2-1][i_2-1][i_0] * G[i_2][i_0] - G[i_2-1][i_0] * G[i_2][i_0] - F[i_2] + G[i_0+1][i_0] * I[i_2+1] + G[i_0-1][i_0+1] + J[i_0][i_2][i_2] + B[i_0+1][i_2] * B[i_0+1][i_2-1] * B[i_0-1][i_2+1] + 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_N; i_3++) {
        D[i_3+2] = I[i_3] + I[i_3] - F[i_3] * D[i_3+2] + 1;
        D[i_3+1] = I[i_3] - F[i_3] * D[i_3] - 2;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2412251315_06(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
