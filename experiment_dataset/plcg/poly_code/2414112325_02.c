...
void kernel_2414112325_02(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_L; i_1++) {
        B[i_1-1] = C[i_1-1][i_1-1] - C[i_1][i_1] + C[i_1-3][i_1-3] - 1;
        for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
            C[i_2-1][i_2-1] = C[i_2+1][i_2+1] + C[i_2-2][i_2-2] - 5;
        }
        for (int i_3 = 1; i_3 < min(-i_1+PB_M-1, PB_L-1); i_3++) {
            D[i_3+i_1][0][i_3] = E[i_3+1][0] - B[i_1-1] * B[i_3-1] * D[i_3+i_1+1][0][i_3+1] - D[i_3+i_1-3][0][i_1-2] + 2;
        }
    }
#pragma endscop
...
kernel_2414112325_02(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
