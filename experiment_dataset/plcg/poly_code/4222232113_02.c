...
void kernel_4222232113_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                A[i_1][i_1-2][i_0] = C[i_1][i_0][i_2] * C[i_2][i_1-2][i_0] * C[i_0][i_2][i_1+1] + C[i_0][i_2][i_1] * 1;
            }
        }
        for (int i_3 = 2; i_3 < min(-i_0+PB_L, PB_M-1); i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                A[i_3+1][i_3-1][i_0] = D[i_4][i_4][i_0] + C[i_3+i_0][i_4][i_0] - A[i_3][i_3-2][i_0] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4222232113_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
