...
void kernel_3322151315_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = PB_P-2 - 1; i_1 >= 0; i_1--) {
        for (int i_2 = 1; i_2 < min(PB_P, -i_1+PB_L); i_2++) {
            B[i_2][i_1+1] = E[i_2+i_1][i_2] * E[i_2][i_1] - E[i_2-1][i_1+1] + E[i_2][i_1] - E[i_2][i_1+1] - B[i_2][i_1] + B[i_2+1][i_1+2] + B[i_2+1][i_1+1] - B[i_2-1][i_1+1] * 2;
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
            B[i_3][i_4+1] = D[i_3] - D[i_4-1] * D[0] - E[i_4+1][1] - A[i_4] + 3;
        }
    }
#pragma endscop
...
kernel_3322151315_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
