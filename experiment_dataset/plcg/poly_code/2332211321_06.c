...
void kernel_2332211321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N-1; i_1++) {
            A[i_1+i_0-1][i_0] = D[i_0][i_1] - A[i_1+i_0-2][i_0-2] + A[i_1+i_0-3][i_1] - A[i_1+i_0+1][i_0+2] + A[i_1+i_0][i_0+1] + 5;
        }
    }
    for (int i_2 = 3; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 2; i_3 < PB_N-2; i_3++) {
            B[i_3][i_2-1] = B[i_3-2][i_2-3] * B[i_3][i_2-3] + B[i_3+2][i_2-2] - 1;
        }
    }
    for (int i_4 = 3; i_4 < PB_M; i_4++) {
        C[i_4-1] = C[i_4-3] * C[i_4] - 4;
    }
#pragma endscop
...
kernel_2332211321_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
