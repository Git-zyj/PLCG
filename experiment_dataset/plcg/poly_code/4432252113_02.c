...
static void print_array(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4432252113_02(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q, PB_L); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = max(1, i_2); i_3 < PB_P-2; i_3++) {
                    D[i_1][i_0][i_3+2][i_2] = E[i_0][i_1-2][i_3][i_2] * F[i_3][i_2][i_0][i_1-2] + F[i_0][i_3+i_1-1][i_2][i_1+1] - G[i_2][i_3][i_0][i_1][i_1-2] * H[i_3-1][i_1][i_2] + F[i_3][i_2][i_1][i_1+i_0] * F[i_0][i_2][i_3][i_2] + 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            for (int i_6 = 0; i_6 < min(min(-i_5+PB_M, PB_L-2), -i_4+PB_L); i_6++) {
                B[i_6+i_5][i_4][i_5] = H[i_6+i_4][i_5][1] - I[i_4][i_6+2][2] + J[i_5][i_6+i_4] - I[i_6+1][i_4][i_5] * 6;
            }
        }
        for (int i_7 = i_4; i_7 < PB_L-2; i_7++) {
            for (int i_8 = 1; i_8 < min(-i_7+PB_M-1, PB_L-1); i_8++) {
                B[i_8+i_7+1][i_4][i_7] = J[i_8][i_7+2] + I[i_8][i_4][i_7] - I[i_8][i_4-1][i_8+1] + H[i_7][i_4+1][0] + H[i_7+2][i_4][i_8-1] + 2;
            }
        }
    }
    for (int i_9 = 2; i_9 < PB_L-1; i_9++) {
        for (int j_0 = 0; j_0 < PB_L; j_0++) {
            for (int j_1 = 1; j_1 < PB_P-2; j_1++) {
                for (int j_2 = 2; j_2 < -j_1+PB_P; j_2++) {
                    D[i_9][j_0][j_1+2][j_2+1] = I[j_1][j_2-2][j_0] - H[j_1-1][j_1][j_0] + F[j_0][i_9+1][j_2+j_1][i_9-2] * F[j_0][j_2-2][j_1][i_9] * D[i_9-1][j_0][j_1+2][j_2+1] * D[i_9][j_0][j_1+2][j_2+2] * 3;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4432252113_02(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
