...
void kernel_2214232121_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(i_1, -i_1+PB_N-2); i_2++) {
            B[i_2+i_1] = E[i_1-2][i_2+i_1] - F[i_2][i_1] * E[i_2][i_1+1] + B[i_2+i_1+2] - 2;
        }
    }
#pragma endscop
...
kernel_2214232121_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
