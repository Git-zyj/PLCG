...
void kernel_2432251221_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] + E[i_0-1] + E[i_0] + E[i_0-1] * E[i_0+1] * E[i_0] * 3;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 2; i_2 < min(min(-i_1+PB_M, PB_N-2), i_1); i_2++) {
            B[i_2] = F[i_1] * B[i_2-1] - B[i_2-2] - B[i_2+2] * B[i_2+1] + 1;
            C[i_2+1][i_1][i_2-1] = G[i_2][i_2-1] - G[i_1][1] - E[i_2-1] + H[i_1-1][i_2] * I[i_1][i_2][i_2] + F[i_2-1] + G[i_2][i_1] + G[i_2][i_1] - G[i_2+i_1][0] * B[i_2] + C[i_2+1][i_1+1][i_2-1] - C[i_1+1][i_2-2][i_1-1] - 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        D[i_3] = E[i_3+1] + F[i_3-1] * 2;
    }
#pragma endscop
...
kernel_2432251221_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
