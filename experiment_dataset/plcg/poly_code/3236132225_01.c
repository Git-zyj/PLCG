...
void kernel_3236132225_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            B[i_0-2][i_1-1][0] = C[i_1+i_0][i_0] * C[i_1][i_0+1] + C[i_1+i_0][i_0+1] + C[i_0][i_1+1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_M; i_3++) {
            for (int i_4 = 2; i_4 < i_2; i_4++) {
                B[i_3-2][i_4-1][0] = C[i_3][i_4] * C[i_3][i_2] - B[i_3][i_4-1][0] - B[i_2-1][i_4-2][0] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3236132225_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
