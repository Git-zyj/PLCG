...
void kernel_4434112311_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_P; i_2++) {
                A[i_1+2][i_2][i_2+i_0-1] = A[i_1+2][i_2-1][i_2+i_0-2] - A[i_1+2][i_2+1][i_2+i_0] + A[i_1+3][i_2][i_2+i_0-1] * 3;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            for (int i_4 = i_0; i_4 < -i_0+PB_Q-1; i_4++) {
                B[i_3][i_4+1][i_4+i_0] = E[i_0+2][i_4][i_3] * B[i_3][i_4][i_4+i_0-1] + B[i_3+1][i_4+2][i_4+i_0+1] - B[i_3+1][i_4][i_4+i_0-1] * 6;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_N-1; i_5++) {
        for (int i_6 = i_5; i_6 < PB_L; i_6++) {
            C[i_5][0] = C[i_5+1][0] - C[i_5-1][0] - C[i_5+1][0] + 6;
        }
        for (int i_7 = 0; i_7 < PB_M-1; i_7++) {
            for (int i_8 = 0; i_8 < PB_M; i_8++) {
                D[i_5][i_7][i_8+2] = E[i_8][i_5][i_7] * D[i_7+1][i_5+1][i_8+2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4434112311_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
