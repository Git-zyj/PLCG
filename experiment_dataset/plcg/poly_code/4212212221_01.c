...
void kernel_4212212221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_1 = PB_L-1 - 1; i_1 >= 3; i_1--) {
        for (int i_2 = 2; i_2 < min(PB_L, -i_1+PB_M+2); i_2++) {
            for (int i_3 = 0; i_3 < min(min(PB_L, -i_1+PB_M+1), -i_2+PB_M+2); i_3++) {
                B[i_3+i_1-2][i_2][i_2] = C[i_3+i_2-2][i_1+1][i_1] + C[i_2-2][i_1][0] - B[i_2+i_1-3][i_3][i_3] + B[i_3+i_1-1][i_2][i_2] * B[i_3+i_1-2][i_2-2][i_2-2] - B[i_2+i_1-2][i_3][i_3] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4212212221_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
