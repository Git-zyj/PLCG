...
void kernel_3332212325_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        C[i_1] = D[i_1-2] * C[i_1] + C[i_1+2] + C[i_1+1] * 3;
        C[i_1] = D[i_1] - C[i_1-1] + 2;
    }
#pragma endscop
...
kernel_3332212325_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
