...
void kernel_2414332211_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - F[i_0] * A[i_0-1] + A[i_0+1] + 6;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_0+1] = F[0] * G[i_0][i_1-1] - H[i_0-1][i_1] * G[i_1+i_0][i_0+2] + E[i_1-1] + A[i_1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        C[i_2] = I[i_2] + F[i_2] - D[i_2] * 3;
        D[i_2-1] = E[i_2-1] * E[i_2] - C[i_2] - C[i_2-1] * 6;
    }
#pragma endscop
...
kernel_2414332211_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
