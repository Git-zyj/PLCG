...
void kernel_3416251123_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = E[i_0] * F[i_0-1][i_0] - 4;
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_M+1); i_1++) {
            B[i_1][i_0] = F[i_0][i_1+1] - G[i_1+i_0-1][i_0] + F[i_0][i_1+1] * E[i_1] + F[0][i_0] * H[i_0+1][i_1-1][i_1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < -i_2+PB_M; i_3++) {
            for (int i_4 = 2; i_4 < min(-i_3+PB_L+1, -i_2+PB_M); i_4++) {
                B[i_4-2][i_3-2] = F[i_4+i_2][i_4+i_3-1] * F[i_2+1][i_3-1] + H[i_4-1][i_3+i_2][i_2] + I[i_3+i_2][i_4+1][1] + I[i_4][i_2][i_3] + I[i_3][i_4][i_2] - 6;
            }
        }
        for (int i_5 = 1; i_5 < min(i_2, -i_2+PB_M); i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_M; i_6++) {
                D[i_2-1][i_5][i_6+1] = H[i_5+i_2][i_6][i_6] * F[i_6][0] * I[i_6+1][i_5+i_2][i_5-1] * G[i_6+i_5][i_5] + I[i_2-1][i_5][i_2] * G[i_6-1][i_5] + D[i_6-1][i_5][i_2+2] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3416251123_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
