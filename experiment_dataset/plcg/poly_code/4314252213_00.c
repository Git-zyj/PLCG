...
void kernel_4314252213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            A[i_0][i_1-1] = D[i_0][i_1+i_0] + E[i_1+i_0-1][i_1][i_0] + A[i_1+1][i_0-2] - A[i_0+1][i_1-1] * 4;
        }
        for (int i_2 = i_0; i_2 < PB_M-2; i_2++) {
            for (int i_3 = 2; i_3 < -i_0+PB_L; i_3++) {
                A[i_2][i_3] = F[i_3+1][i_0] * G[i_3-2][i_2+2][i_0] * G[i_0-1][i_2+2][i_3] + G[i_3-2][i_0][i_2-1] * F[i_3][i_2-1] - E[i_0][i_3][i_2+2] + G[i_2][i_3+i_0][i_3] - E[i_3][i_0][i_2] + 2;
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < PB_M; i_5++) {
            for (int i_6 = 1; i_6 < min(-i_5+PB_M, i_5); i_6++) {
                for (int i_7 = 0; i_7 < PB_N-2; i_7++) {
                    A[i_5][i_6-1] = H[i_7+2][i_6][i_4][i_5] - H[i_7][i_4][i_6+i_5][i_7] - G[i_6][i_7][i_5-2] - 4;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4314252213_00(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
