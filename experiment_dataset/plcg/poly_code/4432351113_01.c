...
void kernel_4432351113_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_M, -i_0+PB_L); i_2++) {
                for (int i_3 = 0; i_3 < -i_2+PB_N; i_3++) {
                    A[i_0][i_3][i_2+i_0][i_3] = E[i_3][i_0][i_2-1][i_1-1] * F[i_3+i_2][i_1+i_0][i_0][i_2] * E[i_2][i_1][i_0+1][i_3] + F[i_3][i_2][i_1+i_0][i_2] + 1;
                }
            }
        }
        for (int i_4 = 0; i_4 < min(-i_0+PB_M+1, PB_M-1); i_4++) {
            for (int i_5 = i_0; i_5 < -i_0+PB_M; i_5++) {
                for (int i_6 = 2; i_6 < -i_5+PB_Q; i_6++) {
                    B[0][i_6-1][i_5][i_0] = G[i_5-1][i_5+i_0-1][i_6+i_4] + H[i_0+1][i_6+i_5][i_4][i_5] + F[i_6][i_5+i_0][i_6][1] * B[0][i_6-1][i_5+1][i_0] - B[0][i_6][i_5][i_0+1] * B[0][i_6-2][i_5][i_0] * 5;
                }
            }
            D[i_4+1] = I[i_4+i_0-1][1] - I[i_0-1][i_4] * I[i_4][i_0] + I[i_4+1][i_0] + I[i_4][i_0+1] + J[i_4][i_0+1] - 5;
        }
    }
    for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
        D[i_7+1] = K[i_7+1] + K[i_7-1] + L[i_7] + K[1] + M[i_7] - 2;
    }
#pragma endscop
...
kernel_4432351113_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M));
