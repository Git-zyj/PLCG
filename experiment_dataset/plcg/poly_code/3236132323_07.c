...
void kernel_3236132323_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_L-1); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_N; i_2++) {
                A[i_0][i_2+2][i_0] = C[1][i_0][i_1] - C[i_2][i_0][i_1+i_0+1] - C[i_2+i_0][i_2+1][i_1] * C[i_0][i_2+1][i_1-2] - A[i_0+1][i_2+3][i_0+1] - 1;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_P-1, i_3); i_4++) {
            B[i_3-1][i_4] = D[i_4][i_3] * D[i_3][i_3+1] * B[i_3-1][i_4+1] * B[i_3][i_4] * B[i_3-3][i_4] - B[i_3][i_4-2] - 2;
        }
    }
#pragma endscop
...
kernel_3236132323_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
