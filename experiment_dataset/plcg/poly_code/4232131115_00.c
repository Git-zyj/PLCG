...
void kernel_4232131115_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        B[i_0+1] = C[i_0-1] + D[i_0][1] - C[i_0+1] * C[i_0] * 4;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        B[i_1+1] = C[i_1] + C[i_1] + B[i_1+2] * 2;
    }
#pragma endscop
...
kernel_4232131115_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
