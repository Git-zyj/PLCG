...
void kernel_2236152125_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L-1; i_1++) {
            B[i_1][i_0] = C[i_1][i_1+i_0+1] * D[i_1] + D[i_0-2] + C[i_1][i_0-2] * C[i_1][i_1] + C[i_1-2][i_0] - C[i_0][i_1-2] + D[0] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_2][i_3+1] = C[i_3+1][i_2+1] * B[i_3+1][i_2+1] + 5;
        }
    }
#pragma endscop
...
kernel_2236152125_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
