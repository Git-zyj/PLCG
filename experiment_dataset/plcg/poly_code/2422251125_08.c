...
void kernel_2422251125_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = min(i_0, -i_0+PB_P) - 1; i_1 >= 1; i_1--) {
            A[i_0][i_1+i_0] = E[i_1][0] - 4;
            B[i_1] = E[i_0+1][i_0] * E[i_0+1][i_1+i_0] + F[i_1][i_0] + E[i_1][0] - F[i_1+i_0-1][0] - G[i_0] - E[i_0][i_1-1] * E[i_0][i_1] + A[i_0][i_1+i_0] * A[i_0+2][i_1+i_0+2] * 6;
        }
        for (int i_2 = 0; i_2 < min(PB_M, -i_0+PB_L); i_2++) {
            B[i_0+1] = E[i_2+i_0][i_2] + G[i_2] + E[i_2][i_0] + E[i_2][i_0] - E[i_0][i_2] - E[i_2][i_0+1] + 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_L, PB_P); i_4++) {
            A[i_4][i_4+i_3-1] = G[i_3-1] * E[i_3][i_4] + E[i_4][i_3] * F[i_4+i_3][1] + 5;
        }
    }
#pragma endscop
...
kernel_2422251125_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
