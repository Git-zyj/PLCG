...
void kernel_2334112225_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][i_1] = D[2] + A[i_0-1][i_1] * 2;
        }
    }
    for (int i_2 = 4; i_2 < PB_M-1; i_2++) {
        ...
        C[i_2-2] = C[i_2-1] * C[i_2] + C[i_2-4] + 4;
    }
#pragma endscop
...
kernel_2334112225_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
