...
void kernel_2322112215_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        ...
        for (int i_2 = max(1, i_1); i_2 < PB_L-1; i_2++) {
            C[i_1][i_2] = D[i_1][0] + E[i_2-1] + C[i_1][i_2+1] + C[i_1+1][i_2+1] * 6;
        }
    }
#pragma endscop
...
kernel_2322112215_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
