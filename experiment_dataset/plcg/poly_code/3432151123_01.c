...
void kernel_3432151123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
    ...
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            for (int i_3 = 1; i_3 < min(min(-i_1+PB_L+1, PB_P-1), -i_2+PB_M); i_3++) {
                B[i_2][i_3][i_1] = F[i_3-1][i_2][i_1] * F[i_3+1][i_1][1] * F[i_2][i_3][i_1] + F[i_2][i_1][i_3+1] + F[i_3][i_1][i_2] - G[i_3+1][i_3+i_2-1] - G[i_3+i_2][i_3+i_1-1] * F[i_2+1][i_3+1][i_1] - 3;
            }
        }
        for (int i_4 = 0; i_4 < -i_1+PB_P-1; i_4++) {
            for (int i_5 = 0; i_5 < PB_P; i_5++) {
                C[i_5][i_1-1][1] = H[i_1-1][i_4][i_5][i_4+i_1+1] - 3;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_P; i_6++) {
        for (int i_7 = 1; i_7 < PB_P; i_7++) {
            C[i_7][i_6-1][1] = G[i_7][i_6+1] * G[i_7][i_6-1] * G[i_7-1][i_6-1] - G[i_7+1][i_6] - E[i_7] - G[i_6][i_7] - 6;
        }
    }
#pragma endscop
...
kernel_3432151123_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
