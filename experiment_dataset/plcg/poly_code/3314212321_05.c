...
void kernel_3314212321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_P-2); i_2++) {
                A[i_0][i_2][i_1] = D[i_2+i_1][i_1+2][i_0] * A[i_0][i_2+2][i_1] - A[i_0+2][i_1-2][i_2+2] - A[i_0][i_2+2][i_1-1] + 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_M+2; i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                B[i_5][i_3-1] = B[i_5-1][i_4-1] * B[i_4-2][i_5-1] + B[i_5-1][i_3-1] + 4;
            }
        }
        for (int i_6 = 1; i_6 < PB_P; i_6++) {
            for (int i_7 = 0; i_7 < min(-i_3+PB_L, PB_Q); i_7++) {
                C[i_3+1][i_6][i_7] = D[i_3-1][i_7+i_3][i_6] + C[i_3-1][i_6][i_7] + C[i_3+1][i_6-1][i_7] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3314212321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
