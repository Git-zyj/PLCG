...
void kernel_4334352315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] * D[i_0-2] - E[i_0-1] + D[0] + A[i_0+1] * A[i_0-1] - A[i_0-1] - A[i_0-1] + 3;
    }
    for (int i_1 = 2; i_1 < PB_Q-2; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_Q); i_2++) {
            B[i_1+2][i_2] = F[i_2][i_1+1] * F[i_2][i_1+1] + G[i_2+i_1][i_1-1] - H[i_2][i_1] - H[i_1+2][i_2] * B[i_1+3][i_2] * B[i_1+1][i_2] * 4;
        }
        for (int i_3 = 1; i_3 < min(-i_1+PB_M, i_1); i_3++) {
            for (int i_4 = 0; i_4 < PB_Q-2; i_4++) {
                B[i_1+2][i_3-1] = I[i_4][i_3][i_1+2] - J[i_3+i_1][i_1][i_3+1] * I[i_4+2][i_3+1][i_1-2] - I[i_3+i_1][i_1][i_4] * J[i_1][i_4][i_3] + J[i_1][i_4][i_4+2] + 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4334352315_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
