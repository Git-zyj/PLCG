...
void kernel_3234132213_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_P+2; i_1++) {
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            A[i_2] = D[i_1-1][i_2] + D[i_2][i_1-2] - 4;
        }
    }
#pragma endscop
...
kernel_3234132213_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
