...
void kernel_3214111213_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P-1; i_2++) {
                B[i_0][i_2+i_1][i_2] = C[i_0][i_1] * B[i_0+1][i_2+i_1+1][i_2] * 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_P-1, i_3); i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_P-1; i_5++) {
                B[i_3][i_5+i_4+1][i_5] = B[i_3][i_5+i_4][i_5-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3214111213_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
