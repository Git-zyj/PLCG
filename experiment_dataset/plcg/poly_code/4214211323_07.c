...
void kernel_4214211323_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, PB_M-2); i_3++) {
            B[i_2+1] = B[i_2] - B[i_2-1] * B[i_3+2] - 4;
        }
    }
#pragma endscop
...
kernel_4214211323_07(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
