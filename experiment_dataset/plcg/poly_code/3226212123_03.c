...
void kernel_3226212123_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_M+2; i_3++) {
        for (int i_4 = 3; i_4 < PB_L+1; i_4++) {
            B[i_3-2][i_4-1] = C[i_4] + B[i_3-2][i_4-3] + 3;
        }
    }
#pragma endscop
...
kernel_3226212123_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
