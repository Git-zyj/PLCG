...
void kernel_4334332215_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = i_0; i_2 < min(-i_0+PB_Q, PB_L-1); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_Q, PB_P-2); i_3++) {
                    A[i_1+2][i_3][i_3] = D[i_1][i_2][i_3][i_3+i_0] * E[i_2+1][i_3+2][i_1][i_0+1] + E[i_1+2][i_3+2][i_0][i_2+i_0] + A[i_1+2][i_3+1][i_3+1] + 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        B[i_4] = F[i_4] + F[i_4-2] * G[i_4-2] - B[i_4+1] * B[i_4-1] + B[i_4+1] - 3;
    }
    for (int i_5 = 0; i_5 < PB_Q; i_5++) {
        for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
            for (int i_7 = 0; i_7 < PB_M; i_7++) {
                C[i_6][i_7] = H[i_7][i_7][i_6+1] - C[i_6+1][i_7] * C[i_7][i_6-1] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4334332215_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
