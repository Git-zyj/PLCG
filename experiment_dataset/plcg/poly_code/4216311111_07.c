...
void kernel_4216311111_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        ...
        for (int i_3 = 0; i_3 < min(i_0, PB_M-1); i_3++) {
            B[i_0-1][i_3] = C[i_3+1][i_0] + B[i_0][i_3] + B[i_0-2][i_3] - 3;
        }
    }
#pragma endscop
...
kernel_4216311111_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
