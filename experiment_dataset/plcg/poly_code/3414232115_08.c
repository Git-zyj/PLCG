...
void kernel_3414232115_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[0] * E[i_0] * F[i_0] + 4;
        B[i_0] = E[i_0-1] * E[i_0] * F[i_0] + E[i_0] + E[i_0] * 4;
        for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
            C[i_1][i_0+1][i_1+1] = G[i_1][i_0] - A[i_1] + C[i_1+1][i_0+1][i_1+2] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, PB_L); i_4++) {
                D[i_2][i_3][i_4] = H[i_4+i_3][i_2][i_3] - H[i_3+1][i_4][i_2] + I[i_2][i_4][i_3] + D[i_2+1][i_3][i_4] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3414232115_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
