...
void kernel_4312331115_08(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                    C[i_3][i_2-1] = D[i_2][i_0][i_3+1][i_1] * E[i_1+1][i_2+1][i_0+1][i_3] - 3;
                }
            }
            for (int i_4 = 1; i_4 < min(i_1, -i_1+PB_Q-1); i_4++) {
                for (int i_5 = 1; i_5 < min(PB_P-1, -i_1+PB_L); i_5++) {
                    B[i_4-1][i_4][i_5+1] = D[i_5][i_5+i_1][i_0+1][i_4] * F[i_4][i_0-1][i_5][i_1] - D[i_1][i_0][i_5-1][i_4-1] - G[i_5][i_4][i_4+i_1+1] * E[i_1][i_4][i_5][i_0] * 1;
                }
            }
        }
        for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 1; i_7 < -i_0+PB_M; i_7++) {
                C[i_6][i_7] = D[i_7+i_0][i_6+1][i_7-1][i_6] - C[i_6+1][i_7] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4312331115_08(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
