...
void kernel_3114211321_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 5; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L+1; i_1++) {
            A[i_1+i_0-1] = B[i_1+i_0+1][i_1] + A[i_1+i_0-5] * A[i_1+i_0-3] + A[i_1+i_0-4] - 2;
        }
    }
#pragma endscop
...
kernel_3114211321_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
