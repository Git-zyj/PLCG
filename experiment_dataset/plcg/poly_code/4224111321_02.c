...
void kernel_4224111321_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_0] = A[i_1+2] + A[i_0-2] + A[i_1-2] + A[i_1-1] * A[i_0+2] - 5;
        }
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                B[i_0+1][i_2+1][i_3] = C[i_2+1][i_3][1] + B[i_2+1][i_0+1][i_3+1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4224111321_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
