...
void kernel_3334312213_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L+2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 3; i_2 < PB_L+3; i_2++) {
                A[i_0-2][i_0+1][i_2] = A[i_0-3][i_0][i_2] + A[i_2-3][i_2][i_1+1] * 3;
            }
            for (int i_3 = 0; i_3 < PB_Q; i_3++) {
                A[i_0-3][i_0][i_3] = D[i_1-1][i_0-1][i_3] + 4;
            }
        }
...
kernel_3334312213_03(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
