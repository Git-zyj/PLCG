...
void kernel_3216312111_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_Q-3, i_0); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_Q-3, PB_P+1); i_2++) {
                A[i_0][i_2-1][i_2+i_1+2] = A[i_0-1][i_2-1][i_2+i_1+1] + A[i_0][i_2-1][i_2+i_1+3] + 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P+2; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            B[i_4][i_3-2] = C[i_4][0] * 6;
        }
    }
#pragma endscop
...
kernel_3216312111_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
