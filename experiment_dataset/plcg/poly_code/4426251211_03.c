...
void kernel_4426251211_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0][i_0] = E[i_0] + F[i_0] * E[i_0] * A[i_0+1][i_0+1] + 6;
        for (int i_1 = 0; i_1 < min(PB_L-1, -i_0+PB_L+1); i_1++) {
            B[i_1+i_0][i_0] = G[i_1][i_0-1] + H[i_1+1][i_0] - G[i_1][i_0+1] * H[i_0][i_1+i_0-1] + H[i_0][i_1+1] - H[i_1][i_0] - G[i_1+1][i_0] - G[i_1][i_0] * A[i_0][i_0] - A[i_0-1][i_0-1] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            A[i_3][i_3] = G[i_3][i_2] - H[i_3][i_2] * G[i_3][0] * G[i_2][i_3+i_2] - A[i_3][i_3] + 5;
        }
        for (int i_4 = 0; i_4 < -i_2+PB_L-1; i_4++) {
            D[i_2][i_4+i_2][i_4] = H[i_4][i_2] + H[i_4+i_2-1][i_4] + G[i_4+1][0] * G[i_4][i_2] - D[i_2+1][i_4+i_2+1][i_4] - 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4426251211_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
