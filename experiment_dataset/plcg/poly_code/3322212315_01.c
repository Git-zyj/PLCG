...
void kernel_3322212315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-1] = D[i_0] * A[i_0-2] * A[i_0] - A[i_0] + A[i_0] * 3;
    }
    for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
        ...
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
                C[i_4+2][i_3+2][i_3+1][i_1] = C[i_4+1][i_3+2][i_3+1][i_1] * C[i_1+3][i_3+2][i_3+1][i_4] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3322212315_01(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
