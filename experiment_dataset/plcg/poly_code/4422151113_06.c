...
void kernel_4422151113_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_M; i_3++) {
                    B[i_3][i_2] = E[i_1][i_0-1][0][i_2+1] * E[i_3][i_2+1][i_0+1][i_1] * E[i_0][i_3][i_1][0] - E[i_2][i_3+i_0][i_3][i_1] * 1;
                }
            }
        }
        for (int i_4 = 1; i_4 < -i_0+PB_M-1; i_4++) {
            B[i_0][i_4] = F[i_4][i_0] - G[i_0-1][i_4][i_0] * F[i_4+i_0+1][0] * B[i_0-1][i_4-1] - 6;
        }
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_N; i_6++) {
                B[i_0][i_6] = G[i_5][i_0+1][i_6-1] - G[i_6+i_5][i_0+1][i_5] * G[i_0][i_6][0] + F[i_6-1][i_5] + 2;
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
        for (int i_8 = 0; i_8 < min(-i_7+PB_M, i_7); i_8++) {
            B[i_7][i_8+1] = F[i_8+i_7][i_7] - F[i_8][i_7] - F[i_8][i_7+1] - F[i_8+i_7][i_7] * H[i_7] * H[i_8] * 3;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422151113_06(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
