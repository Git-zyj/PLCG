...
void kernel_4312352125_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0-2] + D[i_0] - D[i_0] - D[i_0] - E[i_0] - 6;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        A[i_1] = D[i_1+2] + D[i_1] + D[i_1-2] + E[i_1+1] - D[i_1] - D[i_1] * A[i_1-1] * 3;
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_M-2, -i_2+PB_M); i_3++) {
                for (int i_4 = 1; i_4 < PB_L-2; i_4++) {
                    C[i_3][i_4][i_4+2] = F[i_1-1][i_3][i_2-1][i_4-1] - G[i_3][i_4+1][i_2] + F[i_4+1][i_3+i_2][i_1][i_3] * F[i_3+i_1+2][i_2][i_4][i_2+2] - 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4312352125_02(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
