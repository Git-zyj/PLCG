...
void kernel_2212131115_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-1; i_1++) {
            B[i_1+1][i_1+1] = C[i_1+i_0+1][i_1] * C[i_1-1][i_0] - C[i_0-1][i_1] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_L-1, -i_2+PB_M); i_3++) {
            B[i_3][i_3] = C[i_3+i_2][i_2] * C[i_3-1][i_2] + C[i_3][i_2] * B[i_3+1][i_3+1] * 5;
        }
    }
#pragma endscop
...
kernel_2212131115_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
