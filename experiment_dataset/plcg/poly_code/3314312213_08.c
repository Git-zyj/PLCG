...
void kernel_3314312213_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, -i_1+PB_L+2); i_2++) {
                A[i_2][i_1] = D[i_0][i_2+i_1-2][i_1] * B[i_0][i_0+2] * 3;
            }
            B[i_0][i_0+2] = A[i_0][i_1] + A[i_0][i_1-1] - A[i_0-1][i_1] * 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        C[i_3] = B[i_3][i_3+2] * B[i_3-1][i_3+1] - 2;
    }
#pragma endscop
...
kernel_3314312213_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
