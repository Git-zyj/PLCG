...
void kernel_2234152221_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            A[i_0][i_1+2] = C[i_0+2] + D[i_1][i_0-1] * C[i_0] + D[i_1+i_0][0] * C[i_0] + A[i_0-1][i_1+2] + A[i_0+2][i_1+1] + A[i_0+2][i_1+4] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
            B[i_3-1] = D[i_2-2][i_3-2] - D[0][i_3+2] - D[i_3+1][i_2-1] * D[i_3-1][i_2] - C[i_3] * 5;
        }
    }
#pragma endscop
...
kernel_2234152221_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
