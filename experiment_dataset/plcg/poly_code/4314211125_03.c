...
void kernel_4314211125_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_P, -i_1+PB_P-1); i_2++) {
                A[i_0][i_1+1][i_2+i_1] = D[i_2-1][i_1][i_0-1] * 4;
                A[i_0-2][i_1][i_2+i_1+1] = D[i_2-1][i_1][i_0+1] - A[i_0-1][i_1+1][i_2+i_1-1] + 2;
                for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
                    A[i_3-1][i_0+1][i_2+i_0] = 2;
                }
            }
        }
    }
...
kernel_4314211125_03(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
