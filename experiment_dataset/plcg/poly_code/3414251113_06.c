...
void kernel_3414251113_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
    ...
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_P-1, i_1); i_2++) {
            for (int i_3 = 1; i_3 < min(-i_2+PB_L, PB_P); i_3++) {
                B[i_2-1][i_1][i_1+1] = G[i_3][i_2] + H[i_2][i_1][0] * I[i_2][i_3-1][i_1] + H[i_1][i_3][i_2+1] * I[i_2][i_1][i_3+i_2] - I[i_1+1][i_3][i_2] + I[i_3-1][i_1+1][i_2+1] - I[i_2][i_3][i_2] + B[i_2-1][i_1-1][i_1] + 1;
            }
        }
        D[i_1-2][0] = E[i_1] + A[i_1-1] * 2;
        D[i_1-1][0] = F[i_1] + E[i_1+1] + F[i_1] * F[i_1] + F[i_1+1] * E[i_1+1] + 2;
    }
#pragma endscop
...
kernel_3414251113_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
