...
void kernel_2324251111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = D[i_1+1][i_0+1] + D[i_1][i_0+1] * E[i_1] + E[i_1] - D[i_1][i_0] - D[i_1-1][i_0] - D[i_0-1][i_1] * D[i_1][i_0] + D[i_0][i_1] - F[i_1+1][i_0] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_2-1][i_3+1] = D[i_3][i_2] + E[i_3] + D[i_3][i_2+1] * D[i_2+1][0] + B[i_2-1][i_3+2] - B[i_2-2][i_3] - 1;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            C[i_5-1][i_4] = E[i_5] + C[i_5-1][i_4+1] - 3;
        }
    }
#pragma endscop
...
kernel_2324251111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
