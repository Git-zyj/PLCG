...
void kernel_4226112111_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 3; i_1 < PB_L; i_1++) {
            B[i_1-3][i_0-2][i_0-1] = C[i_0][i_1] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
            B[i_2-2][i_3-1][i_3] = C[i_2][i_3+2] * B[i_2-1][i_3-2][i_3-1] * 3;
        }
    }
#pragma endscop
...
kernel_4226112111_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
