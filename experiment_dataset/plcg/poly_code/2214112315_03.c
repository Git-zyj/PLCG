...
void kernel_2214112315_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M+1; i_1++) {
            A[i_1-1][i_0] = C[i_1][i_1+i_0-1] + A[i_1-1][i_0+1] - A[i_1][i_0] + 5;
        }
    }
    for (int i_2 = 3; i_2 < PB_L+1; i_2++) {
        B[i_2-2] = B[i_2-1] - B[i_2-3] + B[i_2-1] - B[i_2-3] * 6;
    }
#pragma endscop
...
kernel_2214112315_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
