...
void kernel_3222151211_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(min(i_0, PB_P-1), -i_0+PB_L+1); i_2++) {
                A[i_1+1][i_2] = C[i_1][i_2] + D[i_1][i_2-1][i_0] - D[i_2+i_0-1][i_1][i_1-1] - A[i_1+1][i_2+1] + 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M-1, i_3); i_4++) {
            B[i_3+1][0] = C[0][i_4] * E[i_4] * C[i_4][i_3] * C[i_4][i_3] + C[i_3+1][i_4] * C[i_3][i_4-1] + C[i_4+i_3+1][i_4] - B[i_4+2][0] * B[i_3+2][0] * 6;
        }
    }
#pragma endscop
...
kernel_3222151211_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
