...
void kernel_4326252315_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        A[i_0] = D[i_0] * D[i_0+1] + E[i_0] + D[i_0] + D[i_0] * D[i_0] + D[i_0] * E[i_0] + B[i_0+2][i_0+1] + B[i_0+1][i_0] - 1;
        for (int i_1 = max(2, i_0); i_1 < PB_M-1; i_1++) {
            B[i_0+1][i_0] = F[i_1-2][i_0] + G[i_0][i_1] - A[i_1+1] + A[i_1] + A[i_1-1] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q+1; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_P, i_2); i_3++) {
            for (int i_4 = max(2, i_3); i_4 < PB_Q; i_4++) {
                B[i_3+1][i_3] = H[i_4][i_2][i_3] * G[i_4-2][i_3-2] + H[i_3][i_4][i_2-1] - H[i_3][i_2][i_4] * B[i_3][i_3-1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4326252315_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
