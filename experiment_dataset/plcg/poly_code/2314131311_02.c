...
void kernel_2314131311_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        C[i_1] = D[i_1] + D[i_1] * E[i_1][0] - D[i_1] + C[i_1] + 4;
        C[i_1] = A[i_1] + A[i_1+1] * C[i_1-1] * C[i_1+1] * 6;
    }
#pragma endscop
...
kernel_2314131311_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
