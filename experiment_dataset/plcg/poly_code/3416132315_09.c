...
void kernel_3416132315_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-1] - A[i_0+1] + A[i_0-1] - A[i_0-1] + D[i_0-1] + 4;
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            B[i_0][i_1] = F[i_1][i_0-1] + F[i_1][i_0] - F[i_1][i_0] + A[i_1-1] - B[i_0][i_1+1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_P, i_2); i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_4][i_3] = G[i_4][i_3-2][i_2] - 4;
            }
        }
        D[i_2-1] = E[i_2] - E[i_2] + E[i_2] * E[i_2] * A[i_2-1] - D[i_2] * 5;
    }
#pragma endscop
...
kernel_3416132315_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
