...
void kernel_3436331215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_P-1, -i_0+PB_M+1); i_1++) {
            A[i_0][i_1+1] = E[i_1+i_0-1][i_1] - A[i_0-1][i_1+1] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < min(min(-i_3+PB_M, PB_P-2), -i_2+PB_M); i_4++) {
                B[i_3][i_4+i_2][i_2+1] = F[i_4-1][i_2] - G[i_4][i_3][i_2+1] - H[i_4+i_3][i_4+1][0] - F[i_4+i_2][i_3] - I[i_4][i_3+1][i_2-1] + J[i_4][i_2] * B[i_3][i_4+i_2+1][i_2+2] + B[i_3-1][i_4+i_2+1][i_4+2] * 1;
            }
        }
        for (int i_5 = i_2; i_5 < PB_P-2; i_5++) {
            for (int i_6 = 0; i_6 < -i_2+PB_L; i_6++) {
                B[i_5-1][i_6+i_2][i_2+1] = J[i_5][i_6+1] + I[i_5][i_6+i_2][i_2-1] - 6;
            }
            for (int i_7 = 0; i_7 < PB_M; i_7++) {
                A[i_7][i_5+2] = F[i_7][i_2] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3436331215_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
