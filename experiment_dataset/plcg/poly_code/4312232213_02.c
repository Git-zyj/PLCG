...
void kernel_4312232213_02(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
        ...
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_M-1, -i_1+PB_L+1); i_2++) {
                for (int i_3 = 2; i_3 < -i_0+PB_L; i_3++) {
                    C[i_2][i_2+i_1-1][i_2] = D[i_3-2][i_2+1][i_1+2][i_0-2] + E[i_0][i_2][i_1-2][i_3] * E[i_1][i_3+i_0][i_2][i_0+2] - 4;
                }
            }
            B[i_0+2][i_1] = F[i_1-2][i_0] * F[i_1][i_0] + G[i_0][i_0-1] - 3;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < -i_4+PB_L; i_5++) {
            for (int i_6 = 0; i_6 < -i_4+PB_L-1; i_6++) {
                C[i_4][i_5+i_4][i_4] = H[i_4][i_6][0] * I[i_5][i_4][i_6] + H[i_4][i_6][i_5] * C[i_4+1][i_6+i_4+1][i_4+1] + C[i_4-1][i_5+i_4-1][i_4-1] * C[i_4+1][i_5+i_4][i_4+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4312232213_02(xc,yc,zc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
