...
void kernel_4214212215_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_1+i_0][i_1][i_2] = A[i_1+i_0+1][i_1][i_2] * A[i_1+i_0][i_1-1][i_2] - 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M+2; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_P, i_3); i_4++) {
            for (int i_5 = 2; i_5 < -i_3+PB_M+2; i_5++) {
                A[i_5+i_3-2][i_5-1][i_4] = C[i_5][i_5-2][i_4] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4214212215_07(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
