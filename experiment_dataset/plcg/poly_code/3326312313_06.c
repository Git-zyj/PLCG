...
void kernel_3326312313_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = 3; i_2 < PB_L-2; i_2++) {
                A[i_2][i_1][i_2-2] = D[i_2+2][i_0][i_1] - A[i_2-1][i_1-1][i_2-3] + A[i_1][i_2+1][i_1-2] * A[i_2+1][i_1][i_2-1] * 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L; i_4++) {
            ...
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                A[i_3][i_6][i_3-2] = D[i_6][i_4][i_3] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3326312313_06(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
