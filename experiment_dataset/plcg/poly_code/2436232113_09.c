...
void kernel_2436232113_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0] + F[i_0] * D[i_0-1][i_0-1] - 5;
        B[i_0] = E[i_0] - F[i_0] + 3;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        B[i_1] = F[i_1+2] * E[i_1+1] + F[i_1+2] - F[i_1] * E[i_1] + F[i_1] * 4;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            D[i_2][i_2] = G[i_3-1][i_2] + H[i_3+1][i_2] + B[i_3] - D[i_3+1][i_3+1] * 1;
        }
    }
#pragma endscop
...
kernel_2436232113_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
