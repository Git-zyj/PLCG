...
void kernel_2232331225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_L-1); i_1++) {
            A[i_1+i_0][i_0] = C[i_1][i_0-1] * C[i_1][i_0] * D[i_1+i_0][1] + D[i_0][i_1+1] + A[i_1+i_0][i_0-1] - A[i_1+i_0][i_0+2] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_L+1, -i_2+PB_M); i_3++) {
            A[i_3+i_2][i_2] = D[i_2][i_3-1] + C[i_2-1][i_3+1] * 3;
        }
    }
#pragma endscop
...
kernel_2232331225_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
