...
void kernel_3312232211_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_P; i_2++) {
            A[i_2] = F[i_2-1][i_1+2] * G[i_2][i_1] * F[i_2][i_1] + 4;
            for (int i_3 = 0; i_3 < min(-i_1+PB_P, -i_2+PB_M); i_3++) {
                C[i_1+1][i_2][i_3] = H[i_3+i_2][i_3+i_1][0] - H[i_3][i_2+i_1][i_2+i_1] * C[i_1+2][i_2+1][i_3] - C[i_1+1][i_2-1][i_3+1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3312232211_05(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
