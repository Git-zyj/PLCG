...
void kernel_4312111323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = A[i_0+1] - A[i_0+2] + A[i_0-2] + A[i_0-1] - A[i_0+2] * A[i_0-2] - 4;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_N-1, i_1); i_2++) {
                B[i_1][i_2+i_0][i_1] = D[i_1+1][i_2+i_0+1][i_0] - 2;
            }
        }
    }
        ...
        for (int i_4 = max(1, i_3); i_4 < PB_M-1; i_4++) {
            for (int i_5 = 2; i_5 < PB_M; i_5++) {
                for (int i_6 = 0; i_6 < min(-i_4+PB_L+1, PB_L-1); i_6++) {
                    C[i_4][i_5][i_6+1] = D[i_5][i_4][i_4+1] * B[i_5-2][i_6+i_4-1][i_5-2] * C[i_3+1][i_5][i_6+1] - C[i_4+1][i_6][i_3+1] - 2;
                }
            }
        }
    }
...
kernel_4312111323_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
