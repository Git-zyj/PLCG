...
void kernel_3422152125_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            A[i_1+2][i_1] = E[i_1+1][i_0] - E[i_1-2][0] + E[i_0][i_1-1] + E[i_0][i_1-1] * E[i_1+i_0+1][i_1-1] * E[i_0][i_1] * F[i_0+1][i_1][i_0] - E[i_1-1][i_0-1] * G[i_1-2] * E[i_1][i_0] * G[1] + 5;
        }
        A[i_0+2][i_0] = G[i_0-1] - G[1] * G[i_0] - A[i_0+2][i_0] - 3;
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < min(min(PB_L, -i_3+PB_Q), -i_2+PB_M); i_4++) {
                C[i_3][i_3][i_4] = F[i_4][i_2][i_4+i_3] + F[i_4+i_2][i_4][i_3] + 4;
            }
        }
        for (int i_5 = 1; i_5 < min(PB_L, -i_2+PB_M); i_5++) {
            A[i_2+3][i_2+1] = E[i_5][i_2] + E[i_5+i_2][1] - E[i_5-1][i_2] - E[i_2+1][i_5] + 6;
        }
    }
#pragma endscop
...
kernel_3422152125_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
