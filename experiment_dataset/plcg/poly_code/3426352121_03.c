...
void kernel_3426352121_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0+2] * F[i_0-1] - G[i_0] * 5;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1; i_2 < min(-i_1+PB_M, PB_L); i_2++) {
            for (int i_3 = i_2; i_3 < min(-i_2+PB_L, -i_1+PB_L); i_3++) {
                B[i_1][i_3+i_2][i_3] = H[i_2-1][i_3][i_1] - I[i_1][i_3][i_2-2] - H[i_2-2][i_3][i_2] * J[i_2+i_1][i_3][i_2-1] + I[i_2+i_1-2][i_3][i_3+i_1] + I[i_2][i_3][i_1] * H[i_2+i_1][i_3][i_2-1] - 2;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 2; i_5 < min(PB_L+2, -i_4+PB_M); i_5++) {
            C[i_5+i_4][i_4-1] = K[i_5][i_4-2] - G[i_4] - E[i_5] - L[i_4][i_5-2] + 6;
        }
        for (int i_6 = 2; i_6 < min(i_4, PB_L-4); i_6++) {
            D[i_6][i_4+2] = M[i_4][i_6] * K[i_6-2][i_4] * M[i_6-2][i_4-2] - L[i_4-2][0] + D[i_4][i_6+4] + 4;
        }
    }
#pragma endscop
