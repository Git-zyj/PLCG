...
void kernel_4232111123_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = i_0; i_2 < PB_P; i_2++) {
                A[i_1-1][0] = A[i_1+1][0] * 6;
            }
        }
        for (int i_3 = 0; i_3 < min(PB_L, -i_0+PB_P); i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                for (int i_5 = 0; i_5 < PB_L; i_5++) {
                    B[i_0][i_3][i_5][i_0] = C[i_3][i_4][i_0+1][i_5+1] * C[i_4][i_5][i_3+i_0][i_3] - 5;
                }
            }
        }
    }
...
kernel_4232111123_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
