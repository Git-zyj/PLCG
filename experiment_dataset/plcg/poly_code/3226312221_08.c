...
void kernel_3226312221_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                A[i_0][i_2-2] = C[i_1][i_2][i_0] - A[i_2-2][i_1-2] * A[i_2][i_0-3] - 1;
            }
        }
        for (int i_3 = 0; i_3 < min(i_0, PB_L-2); i_3++) {
            B[i_0] = D[i_0][i_3+2] - B[i_3+2] * B[i_3+1] - 6;
        }
    }
#pragma endscop
...
kernel_3226312221_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
