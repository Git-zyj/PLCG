...
void kernel_3236132221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 4; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
                B[i_2][i_3-2][i_4] = C[i_3][i_2-1] * D[i_3-2][i_4][i_2-1] - D[i_4+2][i_2][i_3+2] + D[i_4][i_3][i_2] - C[i_4][i_3] - B[i_2][i_3][i_4] + B[i_2][i_3-4][i_4] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3236132221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
