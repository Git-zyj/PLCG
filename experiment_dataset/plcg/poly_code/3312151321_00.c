...
void kernel_3312151321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_L; i_2++) {
                A[i_1][i_2] = D[i_0-1][i_2+i_1][i_0] - E[i_2][0] * D[i_2+i_1-1][i_0-1][i_0-1] - D[i_1][i_0][i_2-1] + D[i_0+1][i_2][i_1+1] * E[i_0][i_1] + D[i_2][i_1-1][i_0] + 3;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            B[i_3][i_4] = E[i_4][i_3] * E[i_4][0] * E[i_4][i_3] - F[i_4] * F[i_3] + E[i_4][i_3] * B[i_4+1][i_3+1] * B[i_3-1][i_4-1] * B[i_3][i_4-2] + 4;
        }
        for (int i_5 = 0; i_5 < min(-i_3+PB_N-1, i_3); i_5++) {
            C[i_5+i_3-1][i_5] = E[i_5+1][i_3] + E[i_5][i_3] * C[i_5+i_3+1][i_5+2] - C[i_5+i_3-3][i_5] - C[i_5+i_3-3][i_3] - C[i_5+i_3][i_5] - C[i_5+i_3+1][i_5] + 3;
        }
    }
#pragma endscop
...
kernel_3312151321_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
