...
void kernel_2234112323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            A[i_0-1][i_1] = A[i_0-2][i_1] + A[i_0-3][i_1] * A[i_1-1][i_0-2] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            B[i_2+2][1] = C[i_2][i_3] + C[i_3-2][i_2] + B[i_2+1][1] * B[i_2+3][1] + 5;
        }
    }
#pragma endscop
...
kernel_2234112323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
