...
void kernel_2332331325_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        B[i_0] = D[i_0] * E[i_0] * D[i_0] * B[i_0] - 3;
        B[i_0] = D[i_0+1] * D[i_0] - 5;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < min(min(PB_L-1, -i_1+PB_P), i_1); i_2++) {
            C[i_1][i_2-1] = F[i_2+1][i_1] * G[i_2+1][i_2+i_1] - G[i_2][i_1] + H[i_1][i_2+i_1-1] * C[i_1+1][i_2+1] * C[i_1][i_2-2] * C[i_1+1][i_2-1] * C[i_1-2][i_2-2] - C[i_1][i_2] * 6;
        }
    }
#pragma endscop
...
kernel_2332331325_02(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
