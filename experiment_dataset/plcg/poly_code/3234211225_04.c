...
void kernel_3234211225_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_1][i_0][i_2] = C[i_2+1][i_0+1][i_1+1] * A[i_1+1][i_0][i_2] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_M-2, i_3); i_4++) {
            B[i_4+1] = B[i_4+2] + B[i_4-1] * 4;
        }
    }
#pragma endscop
...
kernel_3234211225_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
