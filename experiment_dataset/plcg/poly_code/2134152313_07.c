...
void kernel_2134152313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
            A[i_1] = B[i_0][i_0+1] + B[i_1][i_0-1] - C[i_1-2] + B[i_1-1][i_0] + C[0] + A[i_1+1] - A[i_1-1] + A[i_1+1] + 4;
        }
    }
#pragma endscop
...
kernel_2134152313_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
