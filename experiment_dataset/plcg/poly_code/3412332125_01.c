...
void kernel_3412332125_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1+2] = E[i_1+1][i_0-1] * F[i_0-2][i_1+i_0] * E[i_1][i_0] - F[i_0][i_1] + F[i_1][i_0] - A[i_0+2][i_1+3] - 1;
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_0][i_2][i_1] = B[i_0-1][i_2][i_1] + 2;
            }
            for (int i_3 = i_0; i_3 < PB_M+2; i_3++) {
                A[i_0+2][i_3+2] = G[i_3-2][i_1+i_0][i_1] * F[i_3-2][i_0-1] * H[i_0][i_3+2][i_1] + 1;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 1; i_6 < PB_M; i_6++) {
                A[i_6][i_5+2] = H[i_6-1][i_4-1][i_5+2] - F[i_4+1][i_6] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3412332125_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
