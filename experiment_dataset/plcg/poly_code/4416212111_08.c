...
void kernel_4416212111_08(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = max(0, -i_1+2); i_2 < min(-i_1+PB_P+2, PB_P); i_2++) {
                for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
                    D[i_0+1][i_1][i_2][2] = E[i_0][i_2+i_1-2][i_3+2][i_2] * 1;
                }
            }
    ...
    for (int i_6 = 1; i_6 < PB_L; i_6++) {
        for (int i_7 = 0; i_7 < i_6; i_7++) {
            for (int i_8 = 2; i_8 < i_6; i_8++) {
                C[i_8-1][0][i_7] = F[i_6][i_8] * C[i_8-2][0][i_7] + 4;
            }
        }
        for (int i_9 = 0; i_9 < i_6; i_9++) {
            for (int j_0 = 0; j_0 < PB_L; j_0++) {
                for (int j_1 = 2; j_1 < PB_M; j_1++) {
                    D[i_6][j_0][i_9+1][2] = E[j_1-2][j_1][i_6-1][i_9] + E[j_1][j_0][i_6][i_9+1] * 4;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4416212111_08(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F)));
