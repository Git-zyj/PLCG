...
void kernel_3426112325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M; i_4++) {
            C[i_4+1][i_3] = E[i_4] * C[i_4+2][i_3+2] - C[i_4-1][i_3] - C[i_4+3][i_3] - C[i_4+2][i_3] - C[i_4][i_3] - 5;
        }
        for (int i_5 = max(3, i_3); i_5 < PB_M; i_5++) {
            D[i_5-1][0] = D[i_5-3][0] + D[i_5][0] + D[i_3-2][0] - 4;
        }
    }
#pragma endscop
...
kernel_3426112325_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
