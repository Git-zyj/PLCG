...
void kernel_4434252223_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_P-2, -i_0+PB_L-1); i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_0+PB_L, -i_1+PB_P-2); i_3++) {
                    A[i_0-2][i_3+i_1][i_2-1][i_0] = E[i_3-2][i_1][i_2+1][i_0-1] + E[i_2+i_1-2][i_0][i_3][i_3+i_0-2] * F[i_2][i_1+i_0+1][i_3][i_3] - G[i_1+1][i_3][0] + H[i_1][i_2][i_3+i_0][i_1-1][0] * A[i_0-2][i_3+i_1+2][i_2-1][i_0] * 4;
                }
            }
        }
        for (int i_4 = 2; i_4 < min(-i_0+PB_M, PB_P-1); i_4++) {
            C[i_4+i_0-2][i_4-1] = I[i_0][i_0] * I[i_0][i_4] * I[i_4][i_0] + D[i_4] * D[i_0] + 6;
            C[i_4+i_0][i_4+1] = J[i_4+i_0][0] + J[i_4][i_0-2] * J[i_4][i_0] - J[i_4-2][i_0] - J[i_4][i_0] + C[i_4+i_0-2][i_4-1] - C[i_4+i_0-1][i_4-1] - 1;
        }
        D[i_0] = K[i_0-1] - K[i_0] + J[i_0][i_0] + L[i_0] * 3;
    }
#pragma endscop
