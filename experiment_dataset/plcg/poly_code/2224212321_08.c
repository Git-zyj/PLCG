...
void kernel_2224212321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            A[i_1][0] = C[i_1][0] * A[i_0-2][0] - B[i_1+4] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        B[i_2+2] = A[i_2][0] * B[i_2+4] - B[i_2+3] + 3;
    }
#pragma endscop
...
kernel_2224212321_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
