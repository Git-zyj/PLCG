...
void kernel_4426251113_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = E[i_0-1] * F[i_0-1] - F[i_0] - F[i_0+1] * F[i_0] + E[i_0] + E[i_0] + A[i_0+1] - 1;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_P; i_2++) {
            for (int i_3 = max(1, i_1); i_3 < PB_N; i_3++) {
                for (int i_4 = 1; i_4 < PB_N; i_4++) {
                    D[i_1][i_3+1][i_3+i_2][i_4-1] = G[i_3][i_1][1][i_4+1] - H[i_4][i_3][i_2+i_1][0] * H[i_4+1][i_4][i_3][0] + H[i_3-1][i_2][i_4][i_1] + I[i_3][i_2-1][i_3-1] * G[i_4][i_1][i_2-1][i_3] + 2;
                }
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_N-1; i_5++) {
        A[i_5] = E[i_5] * E[i_5+1] + E[i_5] * 6;
        for (int i_6 = i_5; i_6 < PB_P; i_6++) {
            for (int i_7 = i_6; i_7 < min(-i_6+PB_P, PB_P-1); i_7++) {
                for (int i_8 = 0; i_8 < PB_Q; i_8++) {
                    D[i_5][i_7+1][i_7+i_6][i_8] = J[i_8+1][i_6][i_5+1] - 3;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4426251113_06(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
