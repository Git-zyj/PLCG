...
void kernel_3322332223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[i_0][i_0] = D[i_0] * E[i_0] + D[i_0+2] - E[i_0] * F[i_0][i_0] + A[i_0+1][i_0+1] * 1;
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            A[i_0][i_0] = F[i_0][i_1] + G[i_1-1][i_0][i_0] - A[i_0][i_0] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_P-1; i_4++) {
                A[i_4][i_4] = G[i_3][i_4+i_3+1][i_2] * H[i_3+1][i_2][i_4] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3322332223_01(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
