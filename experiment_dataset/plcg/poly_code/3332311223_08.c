...
void kernel_3332311223_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            B[i_1-2][i_0] = B[i_1-1][i_0] - 4;
            B[i_1-1][i_0] = B[i_1][i_0] + 1;
        }
        for (int i_2 = 1; i_2 < PB_Q-2; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_2+PB_P+1, PB_L); i_3++) {
                C[i_0][i_3][i_2] = D[i_3][i_3+i_2-1][i_0] + C[i_0][i_3][i_2+2] * C[i_2][i_3-1][i_0+1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3332311223_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
