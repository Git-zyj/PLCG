...
void kernel_3214211321_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M+1; i_1++) {
            A[i_1] = C[i_0-1][i_1+i_0-1][i_1] * D[i_1-1][i_0-1] + 1;
        }
        for (int i_2 = i_0; i_2 < PB_P; i_2++) {
            for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
                B[i_3+1][i_0][i_2-1] = B[i_3+2][i_0][i_2] - B[i_3+1][i_0-2][i_2-1] - B[i_3][i_0][i_2-1] + B[i_0+1][i_3-2][i_2-1] - B[i_0+1][i_2-1][i_3-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3214211321_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
