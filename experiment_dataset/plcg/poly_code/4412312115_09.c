...
void kernel_4412312115_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
        ...
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_M, i_0), PB_L-1); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    B[i_1] = E[i_3][i_0][i_2+i_1+1][i_1] * 5;
                }
            }
            B[i_1] = F[i_1][i_1+i_0] - 1;
    ...
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        for (int i_6 = 2; i_6 < PB_L; i_6++) {
            for (int i_7 = 2; i_7 < -i_5+PB_M; i_7++) {
                for (int i_8 = 3; i_8 < -i_6+PB_M; i_8++) {
                    D[i_8+i_6][i_6][i_7-2][i_5-1] = G[i_6-2][i_5][i_6-2][i_8] + G[i_8-2][i_7][i_6][i_5] * D[i_7+i_5][i_7][i_8-3][i_6-1] - 1;
                }
            }
        }
    }
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4412312115_09(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
