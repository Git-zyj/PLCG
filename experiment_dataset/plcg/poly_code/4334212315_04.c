...
void kernel_4334212315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_N-3; i_4++) {
            C[i_3+2] = E[i_4][i_3+1] * C[i_4+3] + C[i_4+1] + C[i_4+3] + 2;
        }
    }
#pragma endscop
...
kernel_4334212315_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
