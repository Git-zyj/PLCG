...
void kernel_3436332325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_M-1; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_M-1; i_2++) {
            B[i_2+i_1][i_1-1] = F[i_1+2] * G[i_2-1] + H[i_2-1][i_1-1] * I[i_1][i_2] * B[i_2+i_1-1][i_1-1] + B[i_2+i_1-2][i_1-3] - B[i_2+i_1-2][i_1-1] * B[i_2+i_1-1][i_2-2] * B[i_2+i_1+1][i_2] - 4;
        }
        for (int i_3 = 0; i_3 < -i_1+PB_M-1; i_3++) {
            B[i_3+i_1+1][i_1-1] = H[i_3][i_1] - 5;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
                D[i_4+2][i_5][i_6] = J[i_6][i_5] - K[i_6][i_4][i_5] + D[i_6+2][i_5-2][i_4] - D[i_6+2][i_5+2][i_4] - D[i_4+1][i_6+2][i_5] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3436332325_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
