...
void kernel_3236112215_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < min(min(-i_3+PB_M, i_3), PB_M-2); i_4++) {
            B[i_4][i_3+1] = C[i_4+i_3][0] * C[i_4+2][i_3] - B[i_4][i_3] * B[i_4-1][i_3] * 5;
        }
    }
#pragma endscop
...
kernel_3236112215_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
