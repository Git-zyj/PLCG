...
void kernel_2326351111_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = D[i_0-1] * D[i_0] * E[1] + E[i_0-1] + 4;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_1+1][i_1] = E[i_1] + F[i_1][i_0+1] - B[i_1][i_1-1] - 2;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M-1, i_0); i_2++) {
            B[i_2+2][i_2+1] = D[i_2] * F[i_0][i_2] + D[i_2+i_0+1] * F[i_2][i_0+1] * G[i_2+1][i_0] * H[1] - F[i_2][i_0] * A[i_0-1] + 2;
        }
    }
#pragma endscop
...
kernel_2326351111_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
