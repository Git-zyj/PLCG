...
void kernel_2334112311_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1][i_0] = A[i_1-1][i_0] - A[i_1-1][i_0-1] + A[i_1][i_0+1] * A[i_1][i_0-1] - 1;
        }
        B[i_0] = B[i_0-1] * 3;
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 3; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            C[i_3-2][i_2] = D[i_3+i_2] * C[i_3-3][i_2] * C[i_3-2][i_2-1] - C[i_3-1][i_2] + 3;
        }
    }
#pragma endscop
...
kernel_2334112311_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
