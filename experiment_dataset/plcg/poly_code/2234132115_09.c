...
void kernel_2234132115_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 3; i_1 < i_0; i_1++) {
            A[i_0-2][0] = C[i_1][i_0] * C[i_1][i_0] + C[i_0][i_1+2] * A[i_1-3][0] - 4;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        A[i_2-3][0] = D[i_2] - D[i_2-1] - 3;
    }
#pragma endscop
...
kernel_2234132115_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
