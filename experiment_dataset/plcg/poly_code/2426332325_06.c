...
void kernel_2426332325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 5; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+i_0+1][i_1+i_0-1] = E[i_1+1] - E[i_1] * F[i_1][i_0-1] * A[i_1+i_0-3][i_1+i_0-5] + A[i_1+i_0+2][i_1+i_0] - A[i_1+i_0][i_1+i_0-2] * 6;
        }
        for (int i_2 = i_0; i_2 < PB_L-4; i_2++) {
            C[i_2+2] = G[i_2] + C[i_2+4] + C[i_2+2] - 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        C[i_3+2] = G[i_3] + G[i_3] + E[i_3] + H[i_3] - E[i_3] - C[i_3] + 4;
    }
    for (int i_4 = 3; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_M+1); i_5++) {
            A[i_5+i_4-1][i_5+i_4-3] = G[i_4-1] + I[i_5][i_4] - 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2426332325_06(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
