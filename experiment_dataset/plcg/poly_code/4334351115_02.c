...
void kernel_4334351115_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] + D[i_0-1] + E[i_0] + D[i_0] - E[i_0] + F[i_0] + A[i_0-1] * A[i_0+1] * 5;
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < PB_M; i_3++) {
                    B[i_1][i_0-1][i_2-1][i_1] = G[i_3][i_1-1][i_2][i_0-1][i_2] + H[i_0][i_3-1][i_1][i_1] + H[i_1][i_2][i_0][i_3] * H[i_0][i_3][i_1][i_1+1] * I[i_1-1][i_0][i_2][i_2] - 2;
                }
                for (int i_4 = 1; i_4 < PB_P+1; i_4++) {
                    B[i_1-1][i_2-1][i_4-1][i_1-1] = I[i_2][i_2][i_1+1][0] * J[i_1-1][i_1-1][i_4-1] * I[i_0][i_1][i_4][i_2] * 4;
                }
            }
        }
    }
...
kernel_4334351115_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
