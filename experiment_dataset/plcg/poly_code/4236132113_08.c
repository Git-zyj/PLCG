...
void kernel_4236132113_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_0+PB_M-2, PB_P-1), i_1); i_2++) {
                A[i_0-1][i_1][i_2] = C[i_0][i_2][i_1-2] * C[i_1][i_2+i_0+2][i_1-2] * A[i_2-1][i_1-1][i_0-1] + A[i_0-1][i_1][i_2+1] * 6;
            }
        }
        for (int i_3 = i_0; i_3 < min(PB_P, -i_0+PB_N); i_3++) {
            B[i_3+i_0][i_3] = D[i_3][i_0] - D[i_3+i_0][i_3] * D[i_0][i_3] + 3;
        }
    }
#pragma endscop
...
kernel_4236132113_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
