...
void kernel_4226131125_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_2+1][i_0][1] = C[i_1][i_0][i_2+1] - C[i_2][i_1][i_0-1] + C[i_1][i_0][i_2+1] + C[i_0][i_2+1][i_1] * A[i_2+2][i_0+1][1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_L; i_5++) {
                A[i_5][i_3][1] = C[i_3+1][i_5+i_4][i_3+1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4226131125_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
