...
void kernel_3236352313_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_Q, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, PB_L-2); i_2++) {
                A[i_1][i_2+2] = C[i_1-1][i_0][i_2] + D[i_2+2][i_1+i_0] * E[i_2][i_0+1][i_1] + A[i_1+1][i_2+2] - A[i_0+1][i_2+2] - A[i_2+1][i_1+1] * 6;
            }
        }
        for (int i_3 = i_0; i_3 < -i_0+PB_L; i_3++) {
            A[i_0-1][i_3+1] = F[i_3][i_0+1] - D[i_3+1][0] * F[i_3][i_0] * F[i_3][i_0] + D[i_3+1][i_3+1] * F[i_0][i_3+i_0] - 4;
        }
    }
#pragma endscop
...
kernel_3236352313_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
