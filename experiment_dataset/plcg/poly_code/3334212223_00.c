...
void kernel_3334212223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-4; i_1++) {
        for (int i_2 = 0; i_2 < min(min(i_1, PB_L), -i_1+PB_M-4); i_2++) {
            for (int i_3 = 0; i_3 < min(min(-i_2+PB_L, PB_L), -i_1+PB_M-3); i_3++) {
                B[i_3+i_1+2][i_2][0] = D[i_1+2][i_3+i_2][i_2+i_1] - B[i_3+i_1+3][i_2][0] * B[i_2+i_1+4][i_3][0] * 6;
            }
        }
    }
...
kernel_3334212223_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
