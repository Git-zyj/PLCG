...
void kernel_2222112323_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-4; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            A[i_0+2][i_1] = A[i_1][i_0] * A[i_0+4][i_1+1] * 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-3; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N-3; i_3++) {
            B[i_3+i_2][i_2] = C[i_3][i_2+1] - B[i_3+i_2+1][i_2] + B[i_3+i_2+2][i_2+1] * B[i_3+i_2+2][i_2+2] - B[i_3+i_2+3][i_2+1] * 4;
        }
    }
#pragma endscop
...
kernel_2222112323_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
