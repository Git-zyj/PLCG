...
void kernel_4436251323_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_L-1); i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                for (int i_3 = 1; i_3 < PB_L; i_3++) {
                    A[i_1][i_2-1][i_0][i_3] = E[i_3-1][i_2][i_1+i_0][i_1+i_0+1] - E[i_2-1][i_3][0][i_0] + F[i_3+1][i_0-1][i_2][i_1] * G[i_1][i_2+1][i_3][i_0+1][i_2] * E[i_2+i_1][i_0][i_3][i_2] + A[i_1][i_3][i_0+1][i_2+2] * A[i_1+2][i_2-1][i_0+2][i_3] + A[i_1][i_2][i_0][i_3] * 3;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_M-3; i_4++) {
        for (int i_5 = i_4; i_5 < -i_4+PB_M-3; i_5++) {
            B[i_4][i_5+i_4+1] = H[i_4][i_5] * I[i_4-1] - J[i_4][i_5] * H[i_5+1][i_4-1] - B[i_5-2][i_5+i_4-3] - B[i_4+2][i_5+i_4+1] + B[i_4-1][i_5+i_4+1] + B[i_4+2][i_5+i_4+3] - B[i_4-1][i_5+i_4-1] - 1;
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                for (int i_7 = i_5; i_7 < PB_L; i_7++) {
                    C[i_4][i_5][i_6][i_5] = K[i_6][i_7][i_5] - F[i_7][i_4-1][i_5+1][i_6] - E[i_4][i_6][i_5][0] - K[i_7][i_4+1][i_5] - C[i_6][i_5][i_4-2][i_5] * C[i_4-1][i_5][i_6][i_5] - 4;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_L-1; i_8++) {
        D[i_8-1] = H[i_8+1][i_8+1] * I[i_8+1] * I[0] + L[i_8] * I[i_8+1] - D[i_8+1] - 3;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4436251323_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
