...
void kernel_2234132223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_P-3); i_1++) {
            A[i_0][i_1+1] = C[i_0][i_1] * C[i_1+i_0][i_1] * C[i_1][i_0] + C[i_1-2][i_0] + A[i_0+1][i_1] - A[i_0][i_1+3] + 5;
        }
        for (int i_2 = i_0; i_2 < min(-i_0+PB_M, PB_P-1); i_2++) {
            A[i_0][i_2+1] = C[i_2][i_0] - C[i_2+i_0][0] * 3;
        }
    }
#pragma endscop
...
kernel_2234132223_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
