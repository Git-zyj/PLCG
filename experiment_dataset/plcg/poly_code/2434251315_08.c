...
void kernel_2434251315_08(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        D[i_0-1] = E[i_0] - E[i_0] - F[i_0] + F[i_0+1] - 1;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        D[i_1] = F[i_1] * E[i_1] * E[i_1] + F[i_1+1] * F[i_1+1] + E[i_1] + F[i_1] + C[i_1-2] * 4;
        for (int i_2 = 2; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            C[i_2-1] = G[i_2][i_1+1] + G[i_1][i_2+i_1] * H[i_1][i_2] - H[i_2+i_1][i_1] * I[i_2-1][i_1][i_2] * C[i_2-2] + 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
            D[i_4-1] = H[i_4+1][0] * H[i_4+1][i_3] * G[i_4+1][i_3-1] + H[i_4][i_3] * C[i_3-1] + D[i_4] + D[i_4-2] - 2;
        }
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2434251315_08(xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
