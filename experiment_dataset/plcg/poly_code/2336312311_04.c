...
void kernel_2336312311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 3; i_3 < min(-i_2+PB_L, PB_M+2); i_3++) {
            B[i_3-2][i_2] = D[i_3-1][i_3+i_2] + D[i_2][i_3+1] * B[i_3-2][i_2-1] * B[i_2-2][i_3-1] + B[i_2-1][i_3] - B[i_3-3][i_2] - B[i_3-3][i_2+1] - 4;
        }
    }
...
kernel_2336312311_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
