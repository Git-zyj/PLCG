...
void kernel_2224312211_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = PB_L-1 - 1; i_1 >= i_0; i_1--) {
            A[i_1] = A[i_0-1] + A[i_1+1] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
        B[i_2+1] = C[i_2] * C[i_2-2] - B[i_2+2] - B[i_2] + 2;
    }
#pragma endscop
...
kernel_2224312211_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
