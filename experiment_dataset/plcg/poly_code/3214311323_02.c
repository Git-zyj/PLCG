...
void kernel_3214311323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_0-1][i_1] = C[i_0][i_1] * A[i_0-1][i_1-2] - A[i_1-1][i_0+1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            for (int i_4 = 3; i_4 < PB_Q-1; i_4++) {
                B[i_2-1][i_3+1][i_4-1] = D[i_4][i_3-1][i_2] * B[i_3-2][i_2][i_4-1] - B[i_2-1][i_3+3][i_4+1] * B[i_2-1][i_3+1][i_4-3] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3214311323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
