...
void kernel_3312132121_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_M-2); i_1++) {
            A[i_1][i_0] = D[i_1][i_0] - D[i_1+2][i_0] * D[i_1-2][0] - D[i_0+2][i_1] + A[i_1][i_0+1] * 4;
            B[i_1][i_0] = E[i_1+i_0] * D[i_1][i_0] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_P, -i_2+PB_N-1); i_4++) {
                C[i_4+i_2+1][i_4] = F[i_2+1][i_4+i_3][0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3312132121_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
