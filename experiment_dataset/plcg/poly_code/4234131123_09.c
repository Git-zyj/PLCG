...
void kernel_4234131123_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(min(i_0, PB_P-1), -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                for (int i_3 = max(-i_1+1, i_0); i_3 < PB_Q-1; i_3++) {
                    A[i_1+1][i_2][i_3+i_1-1] = C[i_3][i_2+1][i_1+i_0][1] - C[i_2][i_0][i_1+1][i_3+1] * 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P; i_4++) {
        for (int i_5 = 2; i_5 < min(i_4, -i_4+PB_M); i_5++) {
            B[i_5+i_4][i_5-1] = D[i_5][i_4] - D[i_5+i_4][1] + D[i_4][i_4-1] + B[i_5+i_4-1][i_5-2] * 4;
        }
    }
#pragma endscop
...
kernel_4234131123_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
