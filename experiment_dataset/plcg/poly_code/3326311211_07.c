...
void kernel_3326311211_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            C[i_3+1] = D[i_3+1][i_2] * 2;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        C[i_4+1] = C[i_4] + C[i_4+2] - 4;
    }
#pragma endscop
...
kernel_3326311211_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
