...
void kernel_2322112213_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_M+2, PB_M-1); i_3++) {
            B[i_3+i_2+1][i_3+1] = D[i_2][i_3+2] + B[i_3+i_2-1][i_2] + B[i_3+i_2][i_3+1] - C[i_3+i_2-2] - 6;
            C[i_3+i_2-2] = 2;
        }
    }
#pragma endscop
...
kernel_2322112213_03(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
