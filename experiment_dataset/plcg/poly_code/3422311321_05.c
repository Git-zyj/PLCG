...
#endif
static void init_array(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int seed)
{
srand(seed);
int i_0, i_1, i_2, i_3;
...
void kernel_3422311321_05(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P+2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                D[i_1+1][i_0-3][i_0-2][i_2] = D[i_1-1][i_2-1][i_2][i_0-2] * 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
        B[i_3+1] = E[i_3] * B[i_3+2] + B[i_3-1] * 2;
    }
    for (int i_4 = 3; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            for (int i_6 = 0; i_6 < -i_5+PB_L; i_6++) {
                C[i_4][i_6+i_5][i_4-1] = F[i_5][i_4][i_4] + C[i_4][i_6+i_5-1][i_4-1] + C[i_4][i_6+i_5][i_4-1] * C[i_4-2][i_6+i_5][i_4-3] + C[i_4+2][i_6+i_5][i_4+1] + 4;
            }
        }
        for (int i_7 = 1; i_7 < PB_P; i_7++) {
            for (int i_8 = 1; i_8 < PB_M+1; i_8++) {
                D[i_8-1][i_7-1][i_7][i_4] = 4;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_3422311321_05(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
