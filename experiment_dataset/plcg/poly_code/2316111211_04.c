...
void kernel_2316111211_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_L; i_2++) {
            B[i_1-1][i_2] = D[i_2+i_1-1][0] * D[i_2][i_2+i_1] + B[i_1-1][i_2-1] - B[i_1][i_2] * 3;
        }
        for (int i_3 = i_1; i_3 < PB_L-1; i_3++) {
            C[i_1][i_3+1] = C[i_1-1][i_3+1] * 4;
        }
    }
#pragma endscop
...
kernel_2316111211_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
