...
void kernel_3422332325_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_1+2] = E[i_1][i_0] * E[i_0+2][2] * F[i_1][i_0] + G[i_1+1][i_0] + A[i_1+3] - A[i_1+4] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_L-1; i_3++) {
            for (int i_4 = i_3; i_4 < PB_L; i_4++) {
                B[i_3][i_2][i_4] = H[i_3+2][i_2][i_4-1] * I[i_4][i_3+i_2+1][i_4] * B[i_3+1][i_2][i_4] + B[i_3][i_2-1][i_4-1] - B[i_3][i_2+1][i_4] - B[i_3-2][i_2-2][i_4-1] + 5;
            }
        }
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            A[i_2+2] = F[i_2][i_2+1] + G[i_5][i_2] * E[i_2][i_5] * 5;
        }
    }
    for (int i_6 = 0; i_6 < PB_P; i_6++) {
        for (int i_7 = max(i_6, 1); i_7 < PB_P+1; i_7++) {
            A[i_7+2] = F[i_6+2][i_7-1] + F[i_7][i_6] * 1;
        }
    }
#pragma endscop
...
kernel_3422332325_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
