...
void kernel_2316351113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = D[i_0] + D[i_0+1] * E[i_0] - 6;
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            A[i_1] = F[i_0-1][i_1+1] * D[i_0+1] * G[i_1-1][i_0] - G[i_1][i_0] * F[i_1+1][i_1-1] - H[i_0] + A[i_0-1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_N+1, PB_M); i_3++) {
            A[i_3-1] = F[i_3+1][i_2] - G[i_3][i_2] + I[i_3+i_2-1][i_3] - I[i_3][i_2] * 2;
        }
    }
#pragma endscop
...
kernel_2316351113_00(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
