...
void kernel_3224151213_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N-1; i_1++) {
            A[i_1+1][i_0] = C[1] * D[i_1][i_0] * D[i_1][i_0] + D[i_1][i_0] - D[i_0+1][i_1+1] - D[i_0][i_1] + A[i_0+2][i_1] * A[i_1][i_0] * A[i_1+1][i_0+1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            for (int i_4 = i_3; i_4 < PB_P; i_4++) {
                B[i_3][i_2][i_4] = E[i_2][i_4-1][i_3-1] + E[i_2][i_4-1][i_3] * D[i_4][i_3] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3224151213_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
