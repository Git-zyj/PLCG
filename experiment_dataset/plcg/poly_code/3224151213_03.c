...
void kernel_3224151213_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            A[i_1-1][i_0] = C[i_1+i_0+1][i_1] * D[i_1] * C[i_1][i_0] + C[i_0-1][i_0] + C[i_1][i_0] + C[i_1+i_0-1][i_1-1] + D[0] * C[i_0][i_1] * A[i_1-2][i_0+1] + A[i_1-1][i_0-1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L; i_3++) {
            A[i_3][i_2-1] = D[i_2] * C[i_2][i_3+i_2] * 3;
        }
    }
#pragma endscop
...
kernel_3224151213_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
