...
void kernel_4212211323_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_3 = 2; i_3 < min(PB_P-1, -i_0+PB_L+1); i_3++) {
            for (int i_4 = i_0; i_4 < PB_P; i_4++) {
                B[i_0][i_3][i_3-1] = C[i_0][i_3+i_0-1][i_4] - B[i_0][i_3+2][i_3+1] * B[i_0+1][i_3-1][i_3-2] + B[i_0][i_3-1][i_3-2] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4212211323_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
