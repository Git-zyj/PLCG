...
void kernel_4224112115_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_P-1; i_2++) {
                B[i_0][i_1][i_2+i_1+1] = 1;
                B[i_0][i_1][i_2+i_1] = C[i_2][i_1+i_0-2][i_2-1] * C[i_1+i_0][i_2][i_1+2] * B[i_2][i_1][i_1+i_0+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4224112115_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
