...
void kernel_2222232215_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        A[i_0] = C[i_0] + C[i_0+2] * C[i_0] - A[i_0-1] * 1;
        for (int i_1 = 1; i_1 < -i_0+PB_P; i_1++) {
            B[i_1][i_1+i_0] = D[i_1+2][i_0] + E[i_0][i_1] * D[i_1+1][i_0] * A[i_1] - B[i_1-1][i_1+i_0-1] - B[i_1][i_1+i_0-1] * 6;
        }
    }
#pragma endscop
...
kernel_2222232215_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
