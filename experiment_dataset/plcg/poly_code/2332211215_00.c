...
void kernel_2332211215_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
            B[i_1+1] = D[i_0][i_1] * D[i_1+1][i_0] - B[i_0+1] - B[i_1+1] - 6;
        }
        B[i_0+1] = 4;
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            C[i_3] = D[i_2][i_3] * C[i_3+1] + C[i_3-1] - 3;
        }
    }
#pragma endscop
...
kernel_2332211215_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
