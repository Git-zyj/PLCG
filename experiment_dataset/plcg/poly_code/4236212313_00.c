...
void kernel_4236212313_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_L-1); i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                A[i_1][i_1][i_2] = C[i_2][i_1-2] * A[i_1+1][i_1+1][i_2-1] * 2;
            }
        }
        for (int i_3 = 2; i_3 < i_0; i_3++) {
            B[i_3-1] = C[i_3+1][i_0-2] + B[i_3] - B[i_3-2] * B[i_3-2] + B[i_3] - 3;
        }
    }
#pragma endscop
...
kernel_4236212313_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
