...
void kernel_3424232223_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            A[i_0][i_1] = E[i_1+i_0][0] - E[i_1+1][i_0-1] + E[i_0+2][i_1-1] + A[i_0+2][i_1] * 2;
        }
        for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
            B[i_2][i_0] = F[i_0-2] + G[i_0][i_2+1] * E[i_2+2][i_0] - E[i_2+1][i_0] + B[i_2-2][i_0] - 6;
        }
    }
    ...
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < PB_L; i_5++) {
            D[i_5-1] = G[i_4+2][i_5-2] * H[i_4] - E[i_5][i_4] + E[i_5][i_4+2] * D[i_5-2] + D[i_5] - 3;
        }
    }
#pragma endscop
...
kernel_3424232223_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
