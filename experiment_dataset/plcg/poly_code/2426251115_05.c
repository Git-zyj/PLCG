...
void kernel_2426251115_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1-1][i_0] = E[i_0] + F[i_1][0] + E[0] - F[i_1][1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            A[i_2-1][i_3] = F[i_3+i_2][i_3] - G[i_3][i_2+1] * F[i_3][i_2] + A[i_3-1][i_2] * 1;
        }
        C[i_2+1] = E[i_2] - H[i_2+1] * H[i_2] * H[i_2] - E[i_2] * 5;
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < min(-i_4+PB_M-1, PB_L-1); i_5++) {
            D[i_5+1][i_4] = H[i_5-1] * E[i_5] * F[i_5][i_4] + E[i_5+1] + G[i_4+1][i_5+1] * G[i_5+i_4+1][i_4] + 2;
        }
    }
#pragma endscop
...
kernel_2426251115_05(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
