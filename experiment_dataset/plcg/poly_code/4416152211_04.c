...
void kernel_4416152211_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        A[i_0+1] = E[i_0] - E[i_0+2] * E[i_0+1] * E[i_0+1] * E[i_0] * E[i_0] - A[i_0] - 2;
    }
    for (int i_1 = 1; i_1 < PB_N-2; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = 2; i_3 < i_2; i_3++) {
                for (int i_4 = 3; i_4 < min(-i_2+PB_P-1, PB_Q-2); i_4++) {
                    B[i_1][i_3][i_4-2][i_1] = F[i_3+2][i_4][i_2] * F[i_3][i_4+i_2+1][i_1] + G[i_1][i_3][i_4][i_2+2] - F[i_3-2][i_2+1][i_4+2] + B[i_1+1][i_3-1][i_4-2][i_1+1] * B[i_1][i_2+1][i_4-2][i_1] - B[i_1][i_3][i_4-3][i_1] + 6;
                }
            }
        }
        for (int i_5 = max(2, i_1); i_5 < -i_1+PB_Q; i_5++) {
            for (int i_6 = max(2, i_1); i_6 < min(min(PB_N-2, -i_1+PB_P), -i_5+PB_M+2); i_6++) {
                C[i_6+2][i_5+i_1][i_5] = F[i_6][i_6][i_5+i_1] + F[i_6][i_5][i_1-1] * F[i_5][i_6+i_1][i_1] * F[i_6][i_1][i_5] + H[i_6-2][i_5] - C[i_1+2][i_6+i_5-2][i_5-1] + 1;
            }
            for (int i_7 = 0; i_7 < PB_P-1; i_7++) {
                D[i_5][i_7][i_5] = F[i_5][i_7][0] * F[i_5][i_7+1][i_5-1] - H[i_7][i_5-1] * D[i_5+1][i_7][i_5+1] + 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4416152211_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
