...
void kernel_2136252211_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_1][i_0+2] = B[i_0-2][i_1] * B[i_1][i_0] + B[i_0][i_1] * C[i_1+1][i_0-1] + C[i_0][i_1] + A[i_0+1][i_1+1][i_0+3] + A[i_0-1][i_1][i_0+1] + 4;
        }
    }
#pragma endscop
...
kernel_2136252211_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
