...
void kernel_3316312221_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_1][i_2] = A[i_2+2][i_1] + A[i_1+1][i_2] + 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_L+2, -i_3+PB_M-2); i_4++) {
            for (int i_5 = 0; i_5 < i_3; i_5++) {
                C[i_5+i_4][i_4-2] = 3;
            }
            C[i_4+i_3][i_3-2] = D[i_4+1][i_3] - C[i_4+i_3+2][i_3] + 6;
        }
    }
#pragma endscop
...
kernel_3316312221_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
