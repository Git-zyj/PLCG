...
void kernel_2334112223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 4; i_1 < i_0; i_1++) {
            A[i_0][i_1-2] = A[i_0-1][i_1-4] * A[i_0+2][i_1-1] + A[i_0-1][i_1-2] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        ...
        for (int i_4 = 2; i_4 < -i_2+PB_M; i_4++) {
            C[i_2][i_4-2] = D[i_4+i_2][0] + D[i_2][0] * C[i_2+2][i_4-2] + 2;
        }
    }
#pragma endscop
...
kernel_2334112223_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
