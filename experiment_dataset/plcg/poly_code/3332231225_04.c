...
void kernel_3332231225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        B[i_1] = E[i_1+1] + E[i_1] + B[i_1+2] * 3;
    }
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-3; i_3++) {
            for (int i_4 = 3; i_4 < min(PB_L-3, -i_3+PB_M+1); i_4++) {
                C[i_2][i_3+1][i_4-1] = F[i_4-1][i_3-1][1] - G[i_4+i_3-1][1] - H[i_3-1][i_2-1][i_2-1] * F[i_4][i_3][i_2+1] - C[i_2][i_4+3][i_3] * C[i_2][i_3+3][i_4-3] - C[i_3][i_4-1][i_2-1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3332231225_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
