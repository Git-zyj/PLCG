...
void kernel_4216112213_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(i_0, -i_1+PB_L), PB_L-2); i_2++) {
                B[i_1-1][i_2+i_1] = C[i_2+2][i_1+i_0][0] - 6;
            }
            B[i_1-2][i_1+i_0] = B[i_1-3][i_1+i_0] * B[i_1-2][i_1+i_0+1] + 6;
        }
    }
#pragma endscop
...
kernel_4216112213_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
