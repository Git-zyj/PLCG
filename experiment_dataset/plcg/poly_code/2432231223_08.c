...
void kernel_2432231223_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            A[i_0][i_1] = E[i_1][i_0] - F[i_1+1][i_0-1] * E[i_1][i_0] * A[i_0+2][i_1+2] + A[i_1-1][i_0-1] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2+1] = E[i_2-1][i_2] * B[i_2-1] * 2;
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            B[i_3+1] = F[i_4][i_4] + G[i_4][i_4+1][i_3] * E[i_3][i_4] - H[i_4] - E[i_4][i_3+1] - 3;
        }
    }
    for (int i_5 = 0; i_5 < PB_N; i_5++) {
        for (int i_6 = 1; i_6 < min(PB_M, -i_5+PB_N); i_6++) {
            A[i_5][i_6] = I[i_6-1] + F[i_6+i_5][0] + F[i_5][i_6] - A[i_5][i_6] + 2;
        }
    }
#pragma endscop
...
kernel_2432231223_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
