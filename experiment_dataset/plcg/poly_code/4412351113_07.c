...
void kernel_4412351113_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-1 - 1; i_0 >= 2; i_0--) {
        A[i_0-1] = A[i_0-2] + 3;
        for (int i_1 = 1; i_1 < -i_0+PB_M+1; i_1++) {
            B[i_0+1][i_1+1] = E[i_1+1] * F[i_1][i_0-1] * G[i_1-1][i_0] - G[i_1][i_0] - H[i_1+i_0-1][i_1] * E[i_1] + I[i_1] + 3;
            C[i_0-1][i_1+1] = H[i_1][i_0] - H[i_0][0] + J[0] - F[i_1][i_0] * G[i_1][i_0] - B[i_0+1][i_1+1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        A[i_2-1] = I[1] * I[i_2] - J[i_2-1] - E[i_2+1] * G[i_2][0] + E[i_2+1] + 4;
    }
#pragma endscop
...
kernel_4412351113_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
