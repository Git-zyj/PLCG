...
void kernel_4314132321_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_L+1); i_2++) {
                A[i_0][i_1][i_1] = D[i_1][i_0][i_2] + D[i_2+i_1][i_0][1] * A[i_1-1][i_0][i_0] + A[i_0+2][i_1][i_1] + A[i_1][i_2-1][i_2-1] * 3;
            }
        }
    ...
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            for (int i_6 = 2; i_6 < min(min(-i_5+PB_M, PB_L-2), -i_4+PB_L); i_6++) {
                C[i_4+1][i_6][i_5+1] = D[i_6+i_5][i_6+i_4][i_5+1] + D[i_6][i_4][i_5] - D[i_4-1][i_4][i_6] + D[i_5+2][i_6-2][i_4] - D[i_5][i_6+2][i_5] + D[i_6][i_5][2] - C[i_4+1][i_6+2][i_5+1] + C[i_4+1][i_6][i_5+3] * C[i_4+1][i_6][i_5+2] - C[i_4+1][i_6-1][i_5] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4314132321_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
