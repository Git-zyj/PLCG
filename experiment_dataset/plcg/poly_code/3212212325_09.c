...
void kernel_3212212325_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
                A[i_1][i_2][i_0] = C[i_0][i_2+1][i_0-1] - A[i_1][i_2-2][i_0-2] - A[i_1][i_2-2][i_0] * A[i_1][i_2-1][i_0] + 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_N, i_3); i_4++) {
            for (int i_5 = 0; i_5 < min(i_4, PB_N-2); i_5++) {
                B[i_4][i_5] = B[i_5+1][i_4-2] + B[i_4-2][i_5] - B[i_5+2][i_4] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3212212325_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
