...
void kernel_4222212125_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_1-1][i_0][i_0] = C[i_0][i_1+2] - B[i_0-1][i_1][i_1] * 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_2][i_3][i_3] = D[i_3-1][i_4][0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4222212125_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
