...
void kernel_2314212313_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        B[i_1+2] = C[i_1] - C[i_1+1] + C[i_1-1] + C[i_1] + 4;
        C[i_1] = D[i_1-2][0] - B[i_1+2] * B[i_1+1] * 4;
    }
#pragma endscop
...
kernel_2314212313_02(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
