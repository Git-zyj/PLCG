...
void kernel_3214311121_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                A[i_0+1][i_2][i_0] = A[i_0+1][i_1-2][i_0] * 4;
            }
            for (int i_3 = i_0; i_3 < PB_P; i_3++) {
                A[i_0+1][i_3][i_0] = C[i_3][i_0][i_1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3214311121_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
