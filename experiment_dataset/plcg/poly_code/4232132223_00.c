...
void kernel_4232132223_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_N; i_1++) {
            for (int i_2 = 1; i_2 < PB_N; i_2++) {
                A[i_0][i_2][i_2] = C[i_1][i_2-1][i_0] - C[i_1+i_0][i_2][0] * C[i_2][i_0-2][i_1-2] + A[i_0-2][i_2][i_2] * A[i_0+1][i_2][i_2] * 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M, i_3); i_4++) {
            for (int i_5 = 2; i_5 < PB_L; i_5++) {
                B[i_4+2][i_3][i_5] = C[i_5-1][i_5][i_4+i_3] * C[i_4][i_3-2][i_5] - B[i_4+1][i_3][i_5] - B[i_4+2][i_3][i_5-2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4232132223_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
