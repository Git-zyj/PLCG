...
void kernel_3216112221_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_0 = PB_M-2 - 1; i_0 >= 2; i_0--) {
        ...
        for (int i_1 = 0; i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                B[i_1+i_0][i_2+i_1][i_0] = B[i_1+i_0+2][i_2+i_1+3][i_0] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3216112221_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
