...
void kernel_4222331221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 3; i_1 < PB_M; i_1++) {
            A[i_1-1] = C[i_0][i_1] * A[i_1-3] * A[i_1-2] * 5;
        }
        for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
                for (int i_4 = 1; i_4 < min(-i_0+PB_M, -i_2+PB_M); i_4++) {
                    B[i_3][i_2][i_4] = D[i_4-1][i_4][i_2+1][i_0] * E[i_4+i_2][i_0][i_2][i_3] - D[i_2][i_4+i_0][i_3-1][1] + D[i_2][i_4][i_0][i_3] + D[i_4-1][i_3][i_0][i_0+1] - B[i_2-2][i_3+2][i_4] * 5;
                }
            }
        }
    }
...
kernel_4222331221_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
