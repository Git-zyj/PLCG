...
void kernel_2324351313_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-1; i_1++) {
            B[i_0][i_1-1] = D[i_1+i_0][0] - E[0] - D[i_0+1][i_1+i_0+1] * F[i_1][i_0][1] - G[i_1+i_0][i_0-1] - D[i_1-1][i_0-1] + G[i_0][i_1-1] - G[i_1+i_0+1][i_1] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            B[i_2][i_3-1] = G[i_3][i_2-1] - H[i_3][i_2] + D[i_3][i_2] - B[i_2][i_3-2] * B[i_2-1][i_3-2] + 5;
        }
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            B[i_2][i_4-2] = I[i_4] + H[i_4][i_2-1] * D[i_2][i_4] * D[i_4-1][i_2+1] - B[i_2-1][i_4-2] + 5;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2324351313_00(xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
