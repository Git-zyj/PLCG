...
void kernel_3334251325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0+1] * E[i_0] - D[i_0] * E[i_0-1] - D[i_0] + A[i_0-1] - 4;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_P-1); i_2++) {
            for (int i_3 = i_2; i_3 < -i_1+PB_P; i_3++) {
                B[i_2+1][i_3-1] = F[i_1][i_2][i_3+i_1] + G[i_3][i_1][i_1+1][i_2] + F[i_3][i_1][i_2-1] * H[i_2+i_1][i_3][1] + F[i_2][i_3][i_1] * B[i_3+1][i_2+1] * B[i_2-1][i_3-1] - B[i_3-1][i_2-1] + 6;
            }
        }
        A[i_1] = D[i_1] + E[i_1] * D[i_1+1] - E[i_1-1] * A[i_1] - A[i_1+1] - 3;
    }
#pragma endscop
...
kernel_3334251325_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
