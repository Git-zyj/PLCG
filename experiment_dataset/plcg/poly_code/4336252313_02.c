...
void kernel_4336252313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0-1] + D[i_0] + E[i_0] - D[i_0+1] + E[i_0] * A[i_0+1] - B[i_0] + 3;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1] = D[i_1] * E[i_1-1] * D[i_1] * A[i_1-1] + B[i_1+1] - B[i_1-1] * 3;
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, -i_2+PB_P); i_3++) {
            for (int i_4 = 2; i_4 < min(-i_3+PB_L, PB_M); i_4++) {
                C[i_2][i_3+1][i_4+i_3-1] = F[1][i_2-1][i_4+i_3] - G[i_3+2][i_2+1] + F[i_2][i_4-2][i_3+1] + F[i_2][i_4-2][i_3-2] * H[i_4][i_3-2] * I[i_4-1][i_3+i_2][i_4] - C[i_2+1][i_3+1][i_4+i_3-1] - C[i_4][i_3+1][i_3+i_2-2] + C[i_2][i_3+1][i_4+i_3] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4336252313_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
