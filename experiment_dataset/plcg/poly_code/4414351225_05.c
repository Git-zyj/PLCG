...
void kernel_4414351225_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
            for (int i_2 = max(3, i_1); i_2 < -i_0+PB_N-1; i_2++) {
                for (int i_3 = i_0; i_3 < -i_2+PB_N+3; i_3++) {
                    A[i_2+i_0+1][i_3][i_3][i_0] = E[i_1+1][i_2-1][i_3-1][1] * F[i_2][i_0][i_3-1][i_1+1] + A[i_3+i_2-3][i_1][i_1][i_3-2] + A[i_2+i_0-1][i_3][i_3][i_0-2] * 1;
                }
            }
            for (int i_4 = 3; i_4 < PB_P; i_4++) {
                B[i_0][i_0][i_1-1] = G[i_0][i_1][i_4][i_0+1] * H[0][i_0][i_4-1] + I[i_1][i_4][i_0] + J[i_1][i_1-1] + F[i_1][i_4][i_0+1][i_0] + K[i_0-1][i_1] - B[i_1-2][i_1-2][i_4-3] - B[i_0][i_0][i_4-2] + 6;
            }
        }
        C[i_0+1] = L[i_0+1] + L[i_0] + L[i_0+1] + L[i_0] + L[i_0] - L[i_0] - M[i_0] + C[i_0-1] + C[i_0+2] - 4;
    }
    for (int i_5 = 1; i_5 < PB_N-1; i_5++) {
        C[i_5+1] = M[i_5] - M[i_5-1] + N[i_5] + L[i_5] * 1;
    }
#pragma endscop
...
kernel_4414351225_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N));
