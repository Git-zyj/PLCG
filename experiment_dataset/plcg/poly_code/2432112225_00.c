...
void kernel_2432112225_00(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_1][i_2-1] = 5;
            C[i_2] = B[i_1][i_2-1] * B[i_1-1][i_2-1] * 4;
        }
        C[i_1+1] = E[i_1-1] - C[i_1] - 6;
    }
#pragma endscop
...
kernel_2432112225_00(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E));
