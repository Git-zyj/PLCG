...
void kernel_2214152213_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_0][i_0+2] = C[i_1] - D[i_1][i_0] - C[i_1] * D[i_0][i_1] * D[0][i_0] * C[i_0-2] * A[i_0+1][i_0+3] - A[i_1-1][i_1+1] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        B[i_2] = C[i_2] * C[i_2] - C[i_2] - C[0] - A[i_2][i_2+2] + B[i_2+1] + 3;
    }
#pragma endscop
...
kernel_2214152213_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
