...
void kernel_2336111115_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_L-1; i_2++) {
            B[i_2][i_1] = D[i_2] - D[i_2+1] * C[i_2][i_2+i_1+1] - 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_L-3; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_L-3, i_3); i_4++) {
            C[i_4][i_4+i_3+1] = C[i_4+1][i_4+i_3+2] * C[i_4+1][i_4+i_3+3] * 4;
        }
    }
#pragma endscop
...
kernel_2336111115_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
