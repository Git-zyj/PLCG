...
void kernel_3326312315_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_L-2, -i_0+PB_L); i_2++) {
                A[i_1][i_2+i_0] = D[i_0][i_2+i_1+2][i_2+i_0-1] - 4;
            }
        }
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            for (int i_4 = 2; i_4 < min(-i_0+PB_L, i_0); i_4++) {
                B[i_4-1][i_3-1][i_0-1] = D[i_3][i_4][i_0] - A[i_3][i_4+i_0] + B[i_4-1][i_3-2][i_0-1] + B[i_0][i_3][i_4-2] * B[i_3-1][i_0-1][i_4] * 5;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 1; i_6 < min(-i_5+PB_M-2, i_5); i_6++) {
            for (int i_7 = 3; i_7 < min(-i_5+PB_Q-2, PB_L+2); i_7++) {
                C[i_6+i_5+1][i_7-2][i_7+i_5+1] = B[i_5][i_6-1][i_7-1] * C[i_6+i_5][i_7-3][i_7+i_5-1] + C[i_6+i_5+2][i_7-2][i_7+i_5+1] - C[i_6+i_5+1][i_7-2][i_7+i_5+2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3326312315_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
