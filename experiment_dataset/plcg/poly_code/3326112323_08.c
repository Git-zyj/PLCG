...
void kernel_3326112323_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_N; i_1++) {
            for (int i_2 = 2; i_2 < PB_N; i_2++) {
                A[i_0][i_1][i_2] = A[i_0+1][i_1-2][i_2] - A[i_0][i_2][i_1+2] * A[i_0][i_2-2][i_1] + A[i_0-1][i_1-2][i_2] * 3;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L-1; i_3++) {
            B[i_0][i_3] = D[i_0][i_3+1] + B[i_0][i_3-1] - B[i_0][i_3+1] * 4;
        }
    }
    for (int i_4 = 3; i_4 < PB_L-1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_P-2; i_5++) {
            for (int i_6 = i_4; i_6 < PB_Q-1; i_6++) {
                C[i_4-1][i_5+1][i_6+1] = C[i_4-3][i_5][i_6] * C[i_4+1][i_5+2][i_6-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3326112323_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
