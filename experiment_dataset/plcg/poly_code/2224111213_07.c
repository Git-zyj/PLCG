...
void kernel_2224111213_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-1; i_1++) {
            B[i_1+i_0+1][i_1] = C[i_1][i_0-1] + B[i_1+i_0][i_1-1] - 4;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
            B[i_2+i_0-1][i_2-1] = C[i_2][i_0] + B[i_2+i_0][i_2-1] + 2;
        }
    }
#pragma endscop
...
kernel_2224111213_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
