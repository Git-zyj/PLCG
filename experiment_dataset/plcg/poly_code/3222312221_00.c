...
void kernel_3222312221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0-2][i_1][i_0+1] = A[i_0-1][i_1][i_0+2] * A[i_0][i_1][i_0+3] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            for (int i_4 = 2; i_4 < min(-i_3+PB_M, i_3); i_4++) {
                A[i_4-2][i_3-2][i_4+1] = C[i_4+i_3][i_2][i_4] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3222312221_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
