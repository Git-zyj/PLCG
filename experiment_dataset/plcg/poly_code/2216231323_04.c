...
void kernel_2216231323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_1] = C[i_0] * D[i_0][i_1] + A[i_1-1] + A[i_1-2] - A[i_0-2] * 6;
        }
        for (int i_2 = i_0; i_2 < -i_0+PB_M; i_2++) {
            A[i_2-1] = D[i_0][i_2+1] * C[0] + C[i_2+i_0] + 4;
        }
    }
#pragma endscop
...
kernel_2216231323_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
