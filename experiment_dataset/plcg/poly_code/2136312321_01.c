...
void kernel_2136312321_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_P-1, -i_0+PB_L-1); i_1++) {
            A[i_1+i_0-2][i_1-1] = B[i_0-1][0] + A[i_1+i_0+1][i_1+1] * A[i_1+i_0-4][i_1-3] * A[i_1+i_0-3][i_1-1] + 6;
        }
    }
#pragma endscop
...
kernel_2136312321_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
