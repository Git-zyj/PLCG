...
void kernel_3312211221_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        ...
        B[i_0+1] = A[i_0-1] - B[i_0] + B[i_0-1] + 2;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = min(PB_M, i_2) - 1; i_3 >= 1; i_3--) {
            C[i_3][i_3+1] = D[i_3][i_2] * C[i_3-1][i_3] + 6;
        }
    }
#pragma endscop
...
kernel_3312211221_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
