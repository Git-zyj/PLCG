...
void kernel_2334131223_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
        C[i_1+1] = D[i_1-1] - C[i_1-1] - C[i_1] + 5;
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        C[i_2] = D[i_2] + D[i_2] + D[i_2] * D[i_2] - D[i_2] * C[i_2-2] + 3;
    }
#pragma endscop
...
kernel_2334131223_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
