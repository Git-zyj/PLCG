...
void kernel_3236311215_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = max(max(0, -i_3), -i_4+1); i_5 < min(min(-i_3+PB_P, -i_4+PB_P), PB_L-1); i_5++) {
                A[i_3][i_5+i_4][i_5] = C[i_5+1][i_3][i_4+1] * A[i_4-1][i_5+i_3][i_3] - A[i_3][i_5+i_4-1][i_4-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3236311215_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
