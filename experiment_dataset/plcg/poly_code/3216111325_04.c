...
void kernel_3216111325_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < PB_N; i_2++) {
                A[i_1][i_2] = A[i_1-1][i_2-2] + A[i_0][i_2-1] * A[i_2][i_1+1] + 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_N, i_3); i_4++) {
            A[i_4][i_3+1] = C[i_3-1][i_4] - 1;
        }
    }
#pragma endscop
...
kernel_3216111325_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
