...
void kernel_3236312225_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0][i_0-1] = A[i_0-2][i_0-3] - A[i_0+1][i_0] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        A[i_2-1][i_2-2] = C[i_2] + 5;
    }
#pragma endscop
...
kernel_3236312225_05(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
