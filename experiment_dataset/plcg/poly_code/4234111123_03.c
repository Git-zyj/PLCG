...
void kernel_4234111123_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < i_0; i_3++) {
                    A[i_3][i_1][i_1][i_0-1] = 6;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_L+1; i_4++) {
        B[i_4-1][i_4-1] = C[i_4+1] + C[i_4-1] - B[i_4-3][i_4-3] - 5;
    }
#pragma endscop
...
kernel_4234111123_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
