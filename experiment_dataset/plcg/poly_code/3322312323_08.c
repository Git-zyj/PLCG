...
void kernel_3322312323_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_N; i_0++) {
        A[i_0-2] = 4;
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            B[i_0][i_1] = A[i_1-2] * B[i_0-2][i_1-1] + 6;
        }
        for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
            C[i_0-2][i_2] = D[i_2+2][i_0] + B[i_0][i_2-2] - B[i_0][i_2] * C[i_0-4][i_2-2] - C[i_0][i_2-1] * C[i_0][i_2] + C[i_0-2][i_2-1] - C[i_0-2][i_2+1] - 2;
        }
    }
#pragma endscop
...
kernel_3322312323_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
