...
void kernel_4324332115_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                for (int i_3 = max(1, i_0); i_3 < PB_L-1; i_3++) {
                    A[i_1][i_2-2][i_2][i_0] = D[i_3][i_0][i_2][i_1] + E[i_2][i_3+1][i_2] + A[i_3-1][i_2-2][i_2][i_1-1] - 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < min(-i_4+PB_N, PB_L); i_5++) {
            B[i_5] = F[i_5+i_4][i_4] + F[i_5+1][i_4] - F[i_5][i_4] * G[i_5-2][i_4] * 4;
        }
        for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
            B[i_6+1] = F[i_6+2][i_4] * H[i_6] - F[i_4][i_6] * 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4324332115_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
