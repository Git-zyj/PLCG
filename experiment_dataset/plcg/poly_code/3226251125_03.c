...
void kernel_3226251125_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = C[i_0+1] * D[i_0-1][i_0] * C[i_0] * E[i_0][i_0] + F[i_0] - F[i_0] * F[i_0] * A[i_0+2] - 2;
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            B[i_0] = D[i_1][i_0+1] - C[i_0+1] + E[i_1][i_0] - B[i_0-1] * 6;
        }
    }
#pragma endscop
...
kernel_3226251125_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
