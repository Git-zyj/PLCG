...
void kernel_3216211325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_0 = 3; i_0 < PB_N-2; i_0++) {
        ...
        for (int i_1 = i_0; i_1 < PB_L+2; i_1++) {
            for (int i_2 = 0; i_2 < min(i_1, PB_L-2); i_2++) {
                B[i_1][i_0-1][i_2] = B[i_1][i_0-1][i_2+2] * B[i_1][i_0-3][i_2] - B[i_2][i_0-1][i_1-2] * B[i_1-2][i_0-1][i_2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3216211325_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
