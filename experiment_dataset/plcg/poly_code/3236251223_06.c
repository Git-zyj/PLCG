...
void kernel_3236251223_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_L); i_2++) {
                A[i_2-1][i_1] = C[i_1][i_0+1][i_2] + D[i_2+1][i_1-1][i_0] - E[i_2][i_1] - C[i_1-1][i_0][i_2] - D[i_2+i_1][i_0][i_0] + C[i_2][i_0][i_1] + A[i_1][i_2-1] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-3; i_4++) {
            B[i_4+1] = F[i_4][i_3] - F[i_4][i_3] - E[0][i_3-1] - B[i_4+2] * B[i_4+3] + B[i_4-1] + 6;
        }
    }
#pragma endscop
...
kernel_3236251223_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
