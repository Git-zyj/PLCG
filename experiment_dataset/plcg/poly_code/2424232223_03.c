...
void kernel_2424232223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_L-2); i_1++) {
            A[i_0][i_1] = E[i_1+1][i_0] - F[i_0] - E[i_1-1][i_0] + A[i_1+1][i_0] * 3;
            B[i_1+i_0][0] = F[i_0+2] * G[i_1-2][i_0-2] * E[i_1][i_0] * H[0] * A[i_0][i_1] * 1;
            A[i_0+1][i_1+1] = G[i_1-2][i_0+2] + A[i_0][i_1+2] + B[i_1+i_0][0] * B[i_1+i_0-1][0] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        D[i_2] = F[i_2] - D[i_2+1] + 2;
    }
#pragma endscop
...
kernel_2424232223_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
