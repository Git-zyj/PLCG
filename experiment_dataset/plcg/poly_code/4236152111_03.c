...
void kernel_4236152111_03(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
                B[i_1][i_1][i_2-1][i_0-1] = C[i_2][1][i_1+2] * C[i_2+2][i_1][i_0] - C[i_2-1][i_0][i_1-2] + C[i_1-1][i_2][i_1-1] * B[i_0][i_0][i_1-1][i_2-1] - 1;
            }
        }
        for (int i_3 = i_0; i_3 < -i_0+PB_Q; i_3++) {
            for (int i_4 = 1; i_4 < min(min(min(-i_0+PB_M, -i_3+PB_M+1), i_0), PB_P-2); i_4++) {
                for (int i_5 = 0; i_5 < min(-i_3+PB_M, -i_0+PB_L-2); i_5++) {
                    B[i_3][i_3][i_4-1][i_5] = C[i_5][i_3-2][i_4] + D[i_4+i_3-1][i_5+i_0+2][i_0][i_4+2] + D[i_3+2][i_5][i_4+2][i_0-2] + D[i_4+i_0][i_3][i_5][i_4-1] - D[i_0+1][i_5+1][i_4][i_3] + C[i_5+i_3][i_4][i_3+i_0] + 5;
                }
            }
        }
    }
...
kernel_4236152111_03(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D));
