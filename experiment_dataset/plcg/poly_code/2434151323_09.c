...
void kernel_2434151323_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M+1, PB_M-1); i_1++) {
            B[i_1-1] = E[i_1][i_0+1] + E[i_1+1][i_0] * E[i_1+i_0-1][i_0] - B[i_0-1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2] = F[i_2-1] - F[i_2] + F[i_2] - F[i_2] * F[i_2] * B[i_2+1] - D[i_2+1] + 4;
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            B[i_3] = E[i_3-1][i_2+1] + E[i_3-1][i_2] - F[i_3] * F[i_3] - E[i_3][i_2-1] * B[i_2] * 1;
        }
        D[i_2] = F[i_2+1] - F[i_2+1] - F[i_2-1] - F[1] - E[i_2+1][i_2] * B[i_2] - 3;
    }
#pragma endscop
...
kernel_2434151323_09(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
