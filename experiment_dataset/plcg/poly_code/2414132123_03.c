...
void kernel_2414132123_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        B[i_0] = E[i_0][i_0+2] + F[i_0] * 4;
        B[i_0] = F[i_0-1] - F[i_0+2] + F[i_0] + F[i_0] - 4;
        for (int i_1 = i_0; i_1 < PB_N+2; i_1++) {
            B[i_0-2] = F[i_1-2] + E[i_0-1][i_1] * 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        D[i_2] = F[i_2+1] * F[i_2-2] * F[i_2] - F[i_2-2] * D[i_2+1] * 6;
    }
#pragma endscop
...
kernel_2414132123_03(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
