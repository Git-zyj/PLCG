...
void kernel_2336311321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M+1; i_1++) {
            A[i_0+1][i_1] = A[i_1-1][i_0-2] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_L-2; i_3++) {
            B[i_3][1] = B[i_3+2][1] + B[i_3+1][1] * B[i_3-1][1] + B[i_3-2][1] * B[i_3-1][1] * B[i_3-1][1] - 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        C[i_4][0] = D[i_4+1] * C[i_4-1][0] - C[i_4+1][0] - 4;
    }
#pragma endscop
...
kernel_2336311321_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
