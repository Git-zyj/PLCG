...
void kernel_2326252323_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            A[i_0][i_1] = D[i_1+2][i_0+2] + D[i_1][i_0-1] + E[0] * E[i_1] - F[i_1][i_0-1] - D[i_1+1][i_0+2] - F[i_1][i_0+1] + 2;
        }
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            B[i_2][i_0+1] = F[i_0+1][i_2] * F[i_2][i_0-2] * F[i_0][i_2] - D[i_2][i_0+2] * B[i_2][i_0] + 6;
        }
    }
    for (int i_3 = 3; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M, PB_P-2); i_4++) {
            C[i_3-1][i_4] = D[i_4+i_3][0] * D[i_4][i_3-1] * C[i_3-3][i_4] - C[i_3-2][i_4+2] + C[i_3][i_4] - C[i_3-2][i_4] - C[i_3-1][i_4+2] + C[i_3-2][i_4-2] + 1;
        }
    }
#pragma endscop
...
kernel_2326252323_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
