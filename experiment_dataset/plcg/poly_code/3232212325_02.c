...
void kernel_3232212325_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, PB_M-2); i_2++) {
                A[i_1][i_2] = A[i_2+1][i_1] * A[i_0][i_2+2] + A[i_1-2][i_2] - 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M+2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
                A[i_5][i_4] = C[i_5+2][i_3] + C[i_3-2][i_4] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3232212325_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
