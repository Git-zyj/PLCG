...
void kernel_4236311215_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_L); i_5++) {
                A[i_3][i_5+i_4][i_3] = C[i_5+i_4][i_3+1][i_4+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4236311215_09(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
