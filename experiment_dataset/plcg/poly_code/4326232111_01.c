...
void kernel_4326232111_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 1; i_1 < min(PB_Q-2, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < i_0; i_3++) {
                    A[i_2][i_1+2][i_3] = D[i_1][i_2][i_0+1][i_1+2] * E[i_1][i_3-1][i_2][i_1-1] - 6;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 1; i_6 < min(i_5, -i_4+PB_L); i_6++) {
                B[i_5][i_4][i_5] = F[i_5-2][i_6+i_4][0] * G[i_6][i_4-1] + F[i_5-2][i_4-2][i_6] - H[i_6][i_5+2][0] * A[i_6-1][i_4+2][i_5] + B[i_5][i_4-1][i_5] * B[i_5-1][i_4+1][i_5-1] + 5;
            }
        }
        for (int i_7 = 0; i_7 < i_4; i_7++) {
            for (int i_8 = 1; i_8 < i_7; i_8++) {
                for (int i_9 = 0; i_9 < PB_L; i_9++) {
                    C[i_9][i_9][i_8-1] = F[i_9][i_8][i_4-2] - 6;
                }
            }
        }
    }
...
kernel_4326232111_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
