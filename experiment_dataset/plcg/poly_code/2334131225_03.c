...
void kernel_2334131225_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = D[i_0+1] - D[i_0+1] * D[i_0] - A[i_0-2] * A[i_0+1] * 1;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        C[i_1] = D[i_1] + E[i_1-1][i_1] - 5;
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        C[i_2] = E[i_2][1] * D[i_2-1] * D[i_2] - C[i_2-2] + 4;
    }
#pragma endscop
...
kernel_2334131225_03(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
