...
void kernel_3214111223_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_L-1); i_2++) {
                B[i_1-2][i_1-1] = C[i_2+i_1][i_0][i_0-1] * B[i_2][i_2+1] * 6;
            }
            B[i_0][i_0+1] = B[i_1+1][i_1+2] - 4;
        }
    }
#pragma endscop
...
kernel_3214111223_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
