...
void kernel_4422132215_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P+1; i_2++) {
                for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                    A[i_0][i_3][i_0][i_2+i_1-2] = E[i_3+1][i_2][i_0][i_1+1] - A[i_0][i_3+1][i_0][i_2+i_1-1] * 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_L; i_5++) {
            B[i_5][i_4-1] = F[2] * G[i_4][i_5] - G[i_5-2][i_4] - B[i_5-1][i_4-2] * B[i_4][i_5] - B[i_5+1][i_4-1] - 2;
        }
        for (int i_6 = 0; i_6 < min(PB_Q-2, -i_4+PB_P+2); i_6++) {
            B[i_6][i_4-1] = G[i_6][i_4] + F[i_6] + G[i_6+2][i_4] * G[i_6+2][i_4] + 1;
            for (int i_7 = 0; i_7 < -i_4+PB_L-2; i_7++) {
                for (int i_8 = 0; i_8 < -i_6+PB_Q; i_8++) {
                    A[i_7][i_8][i_7][i_6+i_4-2] = E[i_4][i_7][i_8+i_6][i_6+2] - H[i_8][i_7+i_4+2][i_8+i_6] * 1;
                }
            }
        }
    }
...
kernel_4422132215_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
