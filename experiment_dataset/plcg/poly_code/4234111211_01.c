...
void kernel_4234111211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                A[i_2-1][i_2] = C[i_0-1][i_2][i_1+1] - A[i_2-2][i_2-1] * 6;
            }
        }
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            B[i_0+1] = B[i_3+2] - B[i_3] + 5;
        }
    }
#pragma endscop
...
kernel_4234111211_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
