...
void kernel_2224132121_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_M-2); i_1++) {
            A[i_0+1][i_1] = C[i_1][i_0][i_1] + D[i_1-2][i_0] + A[i_0-1][i_1-2] + 1;
            B[i_1+i_0] = D[i_1][i_0] * D[i_1][i_0+2] * D[i_0][i_1+1] - D[i_1+2][i_0] - 2;
        }
    }
#pragma endscop
...
kernel_2224132121_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
