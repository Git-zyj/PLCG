...
void kernel_4314232313_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L; i_1++) {
            A[i_0][i_1+i_0-2] = D[i_0][i_1+1] - D[i_1-2][i_0-2] + A[i_0-1][i_1+i_0-4] - A[i_0][i_1+i_0-1] + A[i_0+1][i_1+i_0] * 6;
        }
        for (int i_2 = 1; i_2 < min(PB_P+1, -i_0+PB_L); i_2++) {
            B[i_2+1][i_0][i_2-1] = E[i_0+2] * D[i_2+i_0][i_0] * D[i_2][i_0] + B[i_2+1][i_0-1][i_2-1] + C[i_0+1][i_2+2] - C[i_2+1][i_0+2] + C[i_0+2][i_2+2] * 6;
            C[i_0+1][i_2+2] = D[i_2][i_0-1] * F[i_2] + G[i_0][i_2] * 4;
        }
    }
#pragma endscop
...
kernel_4314232313_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
