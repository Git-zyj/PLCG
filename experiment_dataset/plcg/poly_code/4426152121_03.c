...
void kernel_4426152121_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-2, PB_P-2); i_1++) {
            A[i_0+2][i_1] = E[i_1][i_1+i_0+2] - E[i_1+1][i_0] * E[i_1][i_0] - F[i_0+2] + E[i_1+i_0][i_1] - E[i_0][0] - A[i_0+2][i_1+2] + 2;
        }
    }
        ...
        for (int i_3 = i_2; i_3 < -i_2+PB_P; i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
                for (int i_5 = 2; i_5 < PB_P-2; i_5++) {
                    B[i_4][i_2][i_3][i_5+2] = G[i_3][i_2][i_4+1][i_3+i_2] * G[i_3][i_2+1][i_5-2][i_4+1] - H[i_4+i_3][i_5+1][i_3] - G[i_2][i_5][i_3][0] * 4;
                }
                for (int i_6 = 0; i_6 < PB_P-2; i_6++) {
                    C[i_2][i_3][i_6][i_4] = G[i_6][i_4][i_3][i_2] + G[i_6][i_3+1][i_2][i_4] + G[i_6+i_4][i_2+1][i_2+1][i_3] - B[i_4-1][i_6][i_2][i_3+1] * C[i_2-2][i_3][i_6+2][i_4+1] + 5;
                }
            }
        }
        for (int i_7 = i_2; i_7 < min(-i_2+PB_M+2, PB_L-2); i_7++) {
            for (int i_8 = 2; i_8 < min(-i_7+PB_M, i_7); i_8++) {
                D[i_7][i_8+2] = H[i_2+1][i_8-1][i_7] * H[i_7+1][i_2][i_7] * H[i_8+i_7][i_2-2][i_2] + H[i_8][i_8][i_2-1] * H[i_7+i_2-2][2][i_2] * H[i_2][i_8-2][i_7-1] - D[i_8-2][i_7+2] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4426152121_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
