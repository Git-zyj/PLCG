...
void kernel_2334232223_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M+1; i_1++) {
            A[i_0][i_1-1] = D[i_1][i_0] * E[i_1-2] + D[i_0+2][i_1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
            B[i_3][i_3-1] = D[i_2][i_3] * B[i_3-1][i_3-2] - B[i_3+1][i_3] * B[i_3+2][i_3+1] + 3;
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = max(i_4, 4); i_5 < PB_M; i_5++) {
            C[i_5-2][i_5] = F[i_5][i_4] + D[i_5][i_4-2] - D[i_4-2][i_5] + C[i_5-4][i_5-2] + 2;
        }
    }
#pragma endscop
...
kernel_2334232223_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
