...
static void print_array(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4436252223_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_N-1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M-2, i_0); i_2++) {
                A[i_1][i_2][i_0] = E[i_2+i_0+1][i_1][2] * F[i_2][i_0][i_1] - E[0][i_0-1][i_2] + F[i_0][i_1][0] * E[i_2+i_0+2][i_1-2][i_1] * A[i_1][i_2][i_0+2] * A[i_1+1][i_2][i_0] - 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < -i_3+PB_P; i_5++) {
                for (int i_6 = i_4; i_6 < min(-i_3+PB_N, -i_4+PB_Q-1); i_6++) {
                    A[i_5][i_4-2][i_6-1] = G[i_4][i_6][i_5+1][i_5+i_3] * H[i_4][i_5][i_3-2][i_6+i_4+1][i_5] + I[i_4][i_6][i_3][i_4][1] + J[i_6+i_3][i_5][i_4][i_3] - J[i_5][i_3][i_6][i_4+1] * F[i_6+2][i_4-2][i_5] - 3;
                }
            }
        }
        for (int i_7 = 0; i_7 < i_3; i_7++) {
            C[i_7][i_3] = K[i_7] - L[i_3][i_7] * M[i_3][0] * K[i_7+1] + C[i_7][i_3-2] - C[i_7][i_3+1] + 4;
        }
        D[i_3-2] = N[i_3] * N[i_3+1] + 5;
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4436252223_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N));
