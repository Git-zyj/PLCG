...
void kernel_3412252213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_L, PB_M-2); i_2++) {
            B[i_2+2][i_1] = G[i_2][i_1] + E[i_2-1] - H[i_1][i_2] - G[i_2][i_1] - H[i_1][i_2+i_1] - H[i_2][i_1] - G[i_2-2][i_1] - B[i_2+2][i_1+1] + 3;
        }
        A[i_1+1] = E[i_1] + F[i_1] - F[i_1] * 6;
        for (int i_3 = 2; i_3 < PB_P-2; i_3++) {
            for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
                D[i_1][i_4][i_3+2] = I[i_1-2][i_3][2] * J[i_3][i_4-2][i_1] * I[i_4][i_3][i_1+1] + H[i_4][i_3-2] + I[i_4][i_3][i_1] * B[i_4+2][i_3+1] * D[i_4-1][i_1][i_3+2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3412252213_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
