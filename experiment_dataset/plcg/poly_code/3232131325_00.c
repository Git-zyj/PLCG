...
void kernel_3232131325_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_Q, -i_0+PB_L-1); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M, -i_1+PB_Q); i_2++) {
                A[i_0][i_2+i_1][i_1][i_2+i_1] = C[i_2+i_0][i_1][i_2+i_1] + C[i_1][i_2][0] * A[i_2][i_1+i_0+1][i_0-1][i_1+i_0+1] - 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_P-3, -i_3+PB_M); i_4++) {
            B[i_3-1][i_4+1] = D[i_4+i_3][i_4] * D[i_4+1][i_3] + D[i_4+1][i_3-1] * D[i_3-1][i_4] * B[i_3-1][i_4-1] + B[i_3-1][i_4+3] + B[i_3][i_4+1] + B[i_3+1][i_4-1] - 4;
        }
    }
#pragma endscop
...
kernel_3232131325_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
