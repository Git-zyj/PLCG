...
void kernel_3236111225_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_0][i_1-1][i_2] = A[i_0][i_1-2][i_2] + A[i_0][i_1-1][i_2+2] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = i_4; i_5 < -i_4+PB_M+1; i_5++) {
                A[i_3][i_4-1][i_5] = C[i_5+i_4-1][i_3][0] - C[i_5][i_4][i_3] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3236111225_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
