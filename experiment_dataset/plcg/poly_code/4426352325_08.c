...
void kernel_4426352325_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_M, PB_Q), -i_0+PB_L+1); i_2++) {
                B[i_1+1][i_2] = E[i_2][i_0][i_1] + F[0][i_2+i_0-1] + G[i_2+i_1][i_0+1][i_1] - E[i_1][i_0][i_2] + E[2][i_0+1][i_1] * 4;
            }
        }
        for (int i_3 = max(2, i_0); i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_M, i_3); i_4++) {
                B[i_4][i_3] = H[i_4+i_3][i_0-1][i_4][2] + B[i_4+2][i_3-2] + B[i_3][i_4-1] * B[i_4+1][i_3-2] * 2;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_Q; i_5++) {
        for (int i_6 = 2; i_6 < min(-i_5+PB_L, i_5); i_6++) {
            for (int i_7 = 2; i_7 < PB_Q; i_7++) {
                B[i_5][i_7+2] = G[i_5][i_7+1][i_6+i_5] + I[i_7][i_5][i_6] - G[i_5][i_6][i_7-2] * E[i_7+1][i_6-2][i_7] - I[i_7][i_5+2][i_6] - G[i_5][i_7-1][i_6+1] - I[i_7][i_7][i_5] * E[i_6][i_7][i_5] - 4;
            }
        }
    }
    for (int i_8 = 2; i_8 < PB_L-1; i_8++) {
        for (int i_9 = i_8; i_9 < min(-i_8+PB_L, PB_M); i_9++) {
            for (int j_0 = 1; j_0 < min(-i_9+PB_M, -i_8+PB_L+2); j_0++) {
                B[j_0][i_8-2] = I[j_0][i_8-1][i_9] * G[i_9-2][j_0][i_8+1] + G[j_0-1][i_9-1][j_0+i_8-2] - G[j_0-1][i_8-1][i_9] + J[j_0+i_9][i_9+i_8] * 5;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4426352325_08(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
