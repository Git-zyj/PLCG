...
void kernel_3332231225_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[0] = D[i_0] + E[i_0-1] - E[i_0] - F[i_0][i_0+1] - A[0] - A[0] + 2;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                C[i_3][i_2][i_1-1] = G[i_2][i_1][i_3] * G[i_1][i_3][i_2] - H[i_1][i_3][i_2-1] * 4;
            }
            for (int i_4 = 1; i_4 < min(-i_1+PB_M, -i_2+PB_L); i_4++) {
                C[i_1][i_2][i_4] = F[i_4+i_2][i_1] - H[i_2][i_4+i_1][i_1] - C[i_1+2][i_2-2][i_4-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3332231225_00(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
