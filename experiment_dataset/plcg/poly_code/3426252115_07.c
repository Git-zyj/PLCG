...
void kernel_3426252115_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            A[i_1] = E[i_1] * F[i_1][i_0] + G[i_0-1][i_1] - G[i_1][i_0+2] - G[i_0][i_1+1] - F[i_1+i_0][0] + G[i_1][i_0] - G[i_1][i_0] - F[i_1+i_0][i_1] * A[i_1-1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        A[i_2] = H[i_2] - E[i_2+1] * H[i_2] - E[i_2-1] + F[i_2][i_2] + E[i_2-1] - H[i_2] * 2;
    }
    for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            A[i_4] = G[i_4][i_3-2] + F[i_4][i_3] - 2;
        }
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            for (int i_6 = 2; i_6 < min(-i_5+PB_L, i_3); i_6++) {
                A[i_3+1] = I[i_6-2][i_3][i_5+1] - G[i_6][i_6+i_5] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3426252115_07(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
