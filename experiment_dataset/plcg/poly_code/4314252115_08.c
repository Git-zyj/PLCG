...
void kernel_4314252115_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_Q-1; i_1++) {
            for (int i_2 = 3; i_2 < min(min(-i_1+PB_M, i_0), PB_L-2); i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_0+PB_Q, -i_2+PB_L-2), -i_1+PB_Q); i_3++) {
                    A[i_2-2][i_1][i_0][i_3+i_1] = D[i_3+i_2+2][i_1][i_0][0] + E[i_3][i_3][i_2] - F[i_3][i_1][i_2] + G[i_1][i_3][i_0][i_2] + D[i_1-2][i_0+1][i_3][i_2] * H[i_2+i_1][i_3][i_3][i_0-2][i_1] * A[i_2-2][i_1][i_3][i_1+i_0+1] + A[i_2-3][i_0][i_1][i_3+i_0] * 5;
                }
            }
            B[i_1] = I[i_1] * I[0] - J[i_1][i_0+2] - K[i_0][i_1] + J[i_1+1][i_0-2] + K[i_1+1][i_0] * 4;
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < PB_N; i_5++) {
            C[i_4][i_4] = J[i_5][i_4-2] - 5;
        }
    }
#pragma endscop
