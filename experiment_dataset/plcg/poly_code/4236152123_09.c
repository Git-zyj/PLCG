...
void kernel_4236152123_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        B[i_0+3][i_0+4] = C[i_0+2] - C[i_0+2] + C[i_0+1] - D[i_0][0] - C[i_0] * C[i_0+1] * D[i_0-2][i_0] - 5;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        B[i_1+1][i_1+2] = C[i_1] + C[i_1-1] + B[i_1+2][i_1+3] * 4;
    }
#pragma endscop
...
kernel_4236152123_09(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
