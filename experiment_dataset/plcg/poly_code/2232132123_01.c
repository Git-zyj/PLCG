...
void kernel_2232132123_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            B[i_0+2][i_1+2] = C[i_0][i_1] + D[i_1] * C[i_0][i_1+1] + C[i_1-1][i_0+2] * C[i_1+1][i_0] + B[i_0+1][i_1+2] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, PB_L-1); i_3++) {
            B[i_2][i_3+2] = D[i_3+1] + 6;
        }
    }
#pragma endscop
...
kernel_2232132123_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
