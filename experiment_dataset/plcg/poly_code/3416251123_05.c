...
void kernel_3416251123_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = E[i_0] + F[i_0] * E[i_0+1] * F[i_0] - B[i_0] * 6;
        B[i_0] = E[i_0] * E[i_0] - F[i_0] * F[i_0-1] - F[i_0-1] - 3;
        C[i_0] = F[i_0] - F[i_0+1] * F[i_0] * E[i_0] * E[i_0] + E[i_0] - G[i_0-1][i_0] + E[i_0] + 5;
    }
    for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_M+1, -i_1+PB_P); i_2++) {
            B[i_2-1] = H[i_1+1][i_2+i_1][i_2] + F[i_1-1] * 5;
        }
    }
#pragma endscop
...
kernel_3416251123_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
