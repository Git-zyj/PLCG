...
void kernel_4432252115_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0][i_0-1] = E[i_0] + F[i_0+1] + E[i_0-1] * G[i_0][2] - E[i_0-2] * E[i_0-2] - F[i_0] * E[i_0] * A[i_0-1][i_0-2] - 3;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_P-2, -i_1+PB_L); i_2++) {
            for (int i_3 = 0; i_3 < PB_P; i_3++) {
                for (int i_4 = 0; i_4 < min(PB_Q-1, -i_1+PB_L); i_4++) {
                    D[i_1][i_2+2][i_4+1][i_3] = H[1][i_4][i_2+i_1][i_3] + I[i_3][i_4+i_1][i_2][i_2] + 4;
                }
            }
        }
    }
        ...
        for (int i_6 = 2; i_6 < PB_P-3; i_6++) {
            for (int i_7 = 1; i_7 < PB_P; i_7++) {
                for (int i_8 = 1; i_8 < PB_M-1; i_8++) {
                    D[i_8][i_6+3][i_7][i_5-1] = I[i_8+1][i_7][i_5][i_6] + I[i_8-1][i_5][i_6][i_7] + H[i_7][i_6-2][i_5][i_7-1] + 5;
                }
            }
        }
        for (int i_9 = 2; i_9 < PB_P-2; i_9++) {
            for (int j_0 = 2; j_0 < -i_5+PB_L; j_0++) {
                for (int j_1 = 0; j_1 < min(PB_P-3, -i_9+PB_L-1); j_1++) {
                    D[i_5][i_9+2][j_1+1][j_0] = H[i_9][j_0+i_5][i_9][j_0-2] * J[j_1+i_9-2][i_5-2][j_0] * H[j_0][j_1][i_5][i_9] + H[j_1+i_9+1][i_5][j_1+i_9+1][j_0-2] * K[j_1][j_0][i_9] + I[i_9+2][j_0][i_5+1][j_1] * H[i_9][j_1][j_0][0] * D[i_9+1][j_1+3][i_5+2][j_0] + 2;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4432252115_02(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
