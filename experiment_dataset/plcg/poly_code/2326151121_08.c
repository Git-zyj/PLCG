...
void kernel_2326151121_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
            B[i_2+i_1+1][i_1] = D[i_2+1] * E[i_2-1][i_1-1] * E[i_2+1][0] + E[i_2][i_1] + E[i_2][i_1] * E[i_1][i_2] + E[i_2+i_1][i_1] * E[i_2][i_1-1] - B[i_2+i_1+1][i_1+2] - B[i_2+i_1-1][i_1] - 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        C[i_3] = D[i_3] * D[i_3-1] - D[i_3-1] + D[i_3] - 5;
    }
#pragma endscop
...
kernel_2326151121_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
