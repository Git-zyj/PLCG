...
void kernel_3216112111_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-1; i_1++) {
            ...
            for (int i_3 = 1; i_3 < i_0; i_3++) {
                B[i_1][i_3] = C[i_3-1][i_1][i_3-1] * A[i_1+1][i_0+1][i_1+1] * B[i_1-1][i_3+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3216112111_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
