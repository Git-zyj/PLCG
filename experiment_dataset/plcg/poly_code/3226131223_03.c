...
void kernel_3226131223_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            A[i_1] = C[i_1] + C[i_1] - D[i_1+i_0+1][i_1] - A[i_1-2] - A[i_1+2] - A[i_1+1] * 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(min(i_2, PB_L-1), -i_2+PB_M); i_3++) {
            B[i_3] = D[i_3+i_2][1] + D[i_3][i_3+1] + C[i_3-1] + B[i_3+2] + 1;
        }
    }
#pragma endscop
...
kernel_3226131223_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
