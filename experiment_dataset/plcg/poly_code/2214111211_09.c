...
void kernel_2214111211_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            A[i_0+1][i_0+1][i_1-1] = C[i_0][i_1] + A[i_1+1][i_1+1][i_0] - A[i_0][i_0][i_1] * 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < PB_N; i_3++) {
            B[i_2][i_3+1] = B[i_2][i_3+2] + B[i_3][i_2] - 2;
        }
    }
#pragma endscop
...
kernel_2214111211_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
