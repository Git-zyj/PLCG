...
void kernel_4234112221_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        ...
        for (int i_1 = 3; i_1 < min(i_0, PB_L-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_1][i_1-1][0] = C[i_2][i_0][0] + B[i_1+1][i_1][0] * B[i_1-2][i_1-3][0] + B[i_1+2][i_1+1][0] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4234112221_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
