...
void kernel_2216111325_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        A[i_0-1] = A[i_0-3] + A[i_0-2] + 4;
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            B[i_0-1][i_1+i_0] = C[i_1-1][i_1] - C[i_1][i_0] * A[i_1-1] + A[i_0-1] * B[i_0-1][i_1+i_0+1] + B[i_0-1][i_1+i_0-1] - 6;
        }
    }
#pragma endscop
...
kernel_2216111325_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
