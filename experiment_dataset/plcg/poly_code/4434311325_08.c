...
void kernel_4434311325_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_1+i_0-1] = E[i_1+i_0-1][i_0] * A[i_1+i_0+1] - A[i_1+i_0-3] * A[i_1+i_0-3] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_P-3, -i_2+PB_P); i_3++) {
            for (int i_4 = 0; i_4 < min(-i_2+PB_M+1, PB_L-1); i_4++) {
                C[i_4][i_3+3] = F[i_2][i_4][i_3] - G[i_2+1][i_4+1][i_3+i_2][1] - A[i_4+i_2-1] + 6;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_Q; i_5++) {
        for (int i_6 = 0; i_6 < min(i_5, PB_L-2); i_6++) {
            C[i_6][i_5+1] = C[i_6+2][i_5+1] * 1;
            for (int i_7 = 0; i_7 < PB_Q-2; i_7++) {
                for (int i_8 = 1; i_8 < min(-i_5+PB_P, PB_Q); i_8++) {
                    D[i_8-1][i_6][i_7][i_7] = H[i_6][i_7][i_8+i_5][0] + D[i_5-1][i_6][i_7+2][i_7+2] + D[i_8-1][i_6][i_7+2][i_7+2] - 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434311325_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
