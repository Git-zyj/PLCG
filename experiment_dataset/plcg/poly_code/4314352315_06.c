...
void kernel_4314352315_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,int wd,DATA_TYPE POLYBENCH_5D(D,xD,yD,zD,vD,wD,xd,yd,zd,vd,wd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
        ...
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L-1, -i_0+PB_P); i_2++) {
                for (int i_3 = 2; i_3 < min(PB_L, -i_1+PB_Q); i_3++) {
                    B[i_2][i_3-1][i_1-1] = D[i_2][i_3+i_1][i_0][i_0][0] * E[i_0][i_2+1][i_3][i_1-1] + F[i_1][i_3][i_0][i_2+i_0] * E[i_0][i_2][i_2][i_3-2] + 3;
                }
            }
        }
    }
        ...
        for (int i_5 = 1; i_5 < min(-i_4+PB_P, PB_M-1); i_5++) {
            for (int i_6 = i_4; i_6 < PB_L+1; i_6++) {
                for (int i_7 = 0; i_7 < min(-i_4+PB_N, i_5); i_7++) {
                    B[i_7][i_6-2][i_4] = G[i_7][i_5+1][i_4][i_7][i_6+1] - H[i_4][i_6-2][i_5+i_4][i_7+2] * H[i_7+i_4][i_4][i_5-1][i_6] + B[i_4+1][i_6-1][i_7] + 4;
                }
            }
        }
        for (int i_8 = 1; i_8 < -i_4+PB_M-1; i_8++) {
            C[i_8+i_4][2] = I[i_8] * J[i_4][i_8] * J[i_8-1][i_4-2] - K[i_8] - L[i_8][i_4-2] * L[i_8-1][i_4] - C[i_8+i_4+1][2] - C[i_8+i_4+1][2] - C[i_8+i_4-2][2] + C[i_8+i_4-1][2] + 4;
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,wd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4314352315_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,wd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
