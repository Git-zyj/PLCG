...
void kernel_4212251325_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        A[i_0] = C[i_0] * C[i_0] - C[1] * C[i_0] * 5;
        for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
            A[i_0] = D[i_0+1] * E[i_0][i_1-1][i_1] - C[i_1] * F[i_1+1][i_0] * E[i_0-1][i_1][i_0] + F[i_1][i_0-1] - A[i_1] * A[i_1-2] * A[i_0+2] + 3;
        }
    }
#pragma endscop
...
kernel_4212251325_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
