...
void kernel_3316312225_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        ...
        for (int i_3 = 1; i_3 < PB_Q; i_3++) {
            for (int i_4 = max(1, i_0); i_4 < PB_N-1; i_4++) {
                B[i_4+1][i_3-1][i_0] = D[i_4-1][i_0][0] * B[i_4+1][i_0+1][i_3] + 5;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 0; i_6 < i_5; i_6++) {
            C[i_5+1][i_5-1] = C[i_5+2][i_5] - C[i_5][i_5-2] + 3;
        }
    }
#pragma endscop
...
kernel_3316312225_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
