...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            B[i_1][i_2+1] = F[i_2+i_1][i_1] + G[i_2][i_1] * G[i_1+1][i_2+1] + H[i_1][i_2+i_1] - B[i_1][i_2-1] - 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        A[i_3] = I[i_3+1] - I[i_3] * 1;
        A[i_3-2] = E[i_3-1] * 2;
    }
#pragma endscop
...
kernel_4422331123_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
