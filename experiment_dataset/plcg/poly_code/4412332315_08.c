...
void kernel_4412332315_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M; i_1++) {
            B[i_0-1][i_1] = E[i_1][1] + F[i_1][i_0] - 1;
            B[i_0][i_1] = G[i_0][i_1] + G[i_1][i_0+1] - E[0][i_0+2] + E[i_1-2][i_0] + B[i_0][i_1-1] + B[i_0+1][i_1-1] - B[i_0+1][i_1] * 2;
        }
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                B[i_0][i_3+1] = H[i_2][i_3][i_0+1] + G[i_2][0] * 2;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_Q; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
            for (int i_6 = 0; i_6 < PB_Q-1; i_6++) {
                B[i_6][i_5] = H[i_5+1][i_6+2][0] - I[i_5][i_4-2][i_6] + I[i_4-2][i_6][i_5] * H[i_5][i_4-1][i_6+1] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4412332315_08(xb,yb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
