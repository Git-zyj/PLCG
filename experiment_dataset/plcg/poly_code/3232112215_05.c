...
void kernel_3232112215_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        B[i_0+1][i_0+1] = C[i_0-2] + 4;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            B[i_1][i_1] = D[i_1-2][1] * B[i_1+1][i_1+1] * B[i_1-1][i_1-1] - 1;
        }
    }
#pragma endscop
...
kernel_3232112215_05(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
