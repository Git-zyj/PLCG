...
void kernel_3214112221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_N; i_2++) {
                A[i_2][i_0] = A[i_0][i_2-2] + 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_N-2; i_4++) {
            B[i_4][i_3] = C[i_4][i_3] - C[i_3][i_4+1] - A[i_4+2][i_3] * B[i_4-1][i_3] * 6;
        }
    }
#pragma endscop
...
kernel_3214112221_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
