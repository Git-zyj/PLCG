...
void kernel_3122312123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 4; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_M+1, -i_1+PB_L); i_2++) {
                A[i_1-2][i_2+1] = B[i_2-1][i_2+i_1] - A[i_1-4][i_2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3122312123_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
