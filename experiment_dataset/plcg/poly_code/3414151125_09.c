...
void kernel_3414151125_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        A[i_1-1] = E[i_1] + E[0] * F[i_1][0] + E[i_1] - E[i_1] + 2;
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_L-1); i_2++) {
            for (int i_3 = 2; i_3 < PB_M; i_3++) {
                C[i_3][i_2][i_2] = G[i_2][i_1][i_2] * F[i_2+i_1][i_3-1] + G[i_2][i_1+1][0] - C[i_3-2][i_2+1][i_2+1] + 2;
            }
            D[i_2] = F[i_2+1][i_1] - F[i_1+1][i_1] + F[i_2][i_2+i_1-1] * E[i_2] * F[i_2+i_1][i_1] * F[i_2][i_1] + G[i_2][i_1-1][i_2+i_1] * 4;
        }
    }
#pragma endscop
...
kernel_3414151125_09(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
