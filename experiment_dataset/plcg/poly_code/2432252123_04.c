...
void kernel_2432252123_04(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
            D[i_1-1] = E[i_1][i_0] - E[i_0][i_1+2] + F[i_1+1][i_0] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_P; i_3++) {
            D[i_3-2] = G[i_3-1][i_2-1][1] - E[i_2][i_3] + 2;
        }
    }
    for (int i_4 = 3; i_4 < PB_N-2; i_4++) {
        C[i_4] = H[i_4-1] - I[i_4+2] + H[i_4+2] * H[i_4-1] + I[i_4] * E[i_4][i_4] * D[i_4-2] + 1;
        D[i_4-2] = F[i_4+1][0] * I[i_4] * I[i_4-2] + H[i_4-1] - I[i_4] + I[i_4] - D[i_4-3] + 4;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2432252123_04(xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
