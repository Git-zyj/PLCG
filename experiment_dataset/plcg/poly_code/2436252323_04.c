...
void kernel_2436252323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] * E[i_0] + F[i_0] - F[i_0] * F[i_0+2] - E[i_0] * F[i_0] * F[i_0] - F[i_0] + A[i_0+2] * A[i_0+1] + A[i_0-2] - 5;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            B[i_1][i_2] = G[i_2][i_1-1] + H[i_2-1][i_1-1] * H[i_1+1][i_2] - H[i_1][1] - B[i_1-2][i_2] - B[i_1][i_2+2] + 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            A[i_4] = G[i_3-2][i_4] - G[i_3+2][i_4] - H[i_3][i_4-2] + H[i_3-2][i_4-1] - 5;
        }
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        for (int i_6 = 2; i_6 < i_5; i_6++) {
            A[i_6+2] = G[i_6][i_5-1] - F[i_6-2] * G[i_6][i_5] + A[i_6] * 1;
        }
    }
#pragma endscop
...
kernel_2436252323_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
