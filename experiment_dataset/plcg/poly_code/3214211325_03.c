...
void kernel_3214211325_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            B[i_0+1][i_0-1] = C[i_1+1][i_0] * 4;
        }
    }
    for (int i_2 = 3; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_2+1][i_2-1] = B[i_2+2][i_2] * B[i_2-1][i_2-3] + B[i_2][i_2-2] + 1;
        }
    }
#pragma endscop
...
kernel_3214211325_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
