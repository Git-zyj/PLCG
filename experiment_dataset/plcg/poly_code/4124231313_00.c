...
void kernel_4124231313_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_M, -i_0+PB_N); i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_N, -i_0+PB_N+1), i_0); i_2++) {
                A[i_2+i_0-1][i_1][i_0+1] = B[i_2][i_1+1][i_0] - B[i_2][i_0][i_1] * C[i_2][i_1][i_0] - A[i_2+i_1][i_0][i_2+2] - A[i_1+i_0][i_2][i_0+1] * A[i_2+i_0-1][i_1][i_0] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4124231313_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
