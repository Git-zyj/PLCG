...
void kernel_4232131215_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_0+1][i_1][i_0] = C[i_1][i_0][0] * D[i_0-1][i_1] + 5;
        }
    }
    for (int i_2 = PB_M-1 - 1; i_2 >= 1; i_2--) {
        for (int i_3 = 1; i_3 < -i_2+PB_L; i_3++) {
            for (int i_4 = 1; i_4 < -i_2+PB_M-1; i_4++) {
                B[i_3+1][i_4][i_3] = C[i_4][i_3][i_2-1] - C[i_4-1][i_3+i_2][0] - C[i_4+i_2+1][i_4][i_3] - E[i_3-1][i_4][i_2][i_4] * B[i_3][i_4+1][i_3-1] * B[i_3+1][i_4+1][i_3] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4232131215_09(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
