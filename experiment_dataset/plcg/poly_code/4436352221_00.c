...
void kernel_4436352221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_Q; i_2++) {
                for (int i_3 = i_0; i_3 < -i_0+PB_L; i_3++) {
                    A[i_1][i_2-1][i_3+2] = E[i_3][i_0][i_1-1][i_2] - F[i_1][i_2+2][i_0][i_3] * G[i_3][i_3+1][i_1][i_2+i_1] * F[i_2-1][i_1][i_0][i_3] - F[i_1][i_3+i_0][i_2][2] * A[i_1][i_3-1][i_2+1] * 1;
                }
            }
        }
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            B[i_0-1][i_4-2] = H[i_4-2][i_0-2] - I[i_4][i_0+1] + H[i_4][i_0+2] * J[i_0] * B[i_0][i_4-2] + 1;
        }
    }
    for (int i_5 = 2; i_5 < PB_M; i_5++) {
        C[i_5] = J[i_5] * K[i_5+2] * H[i_5][i_5-1] * J[i_5] - K[i_5] + K[i_5] * K[i_5+1] + C[i_5-2] - 3;
    }
    for (int i_6 = 2; i_6 < PB_L; i_6++) {
        for (int i_7 = 2; i_7 < PB_Q-2; i_7++) {
            for (int i_8 = 0; i_8 < PB_Q-1; i_8++) {
                for (int i_9 = 2; i_9 < min(PB_M, -i_8+PB_P-1); i_9++) {
                    D[i_6-2][i_9][i_8][i_7+1] = E[i_9][i_6][i_9+i_8+1][i_7+2] + F[i_7][i_6][i_8][i_9] + L[i_7-2][i_9-1][0] * D[i_6][i_8][i_9][i_7+1] + D[i_9-2][i_6][i_7+2][i_8+1] + 3;
                }
            }
        }
    }
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4436352221_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
