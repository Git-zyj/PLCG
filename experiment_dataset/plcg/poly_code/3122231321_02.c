...
void kernel_3122231321_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_L, PB_P-1); i_2++) {
                A[i_1][i_2+1][i_2] = B[i_2+i_0][i_1] * C[1][i_2-1] * C[i_1][i_2+1] + A[i_2][i_0+3][i_0+2] - A[i_1+2][i_0+1][i_0] + A[i_1][i_2][i_2-1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3122231321_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
