...
void kernel_4314231323_04(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            C[i_1] = D[i_0] - C[i_1] + C[i_0+1] * 1;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_L; i_2++) {
            for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
                for (int i_4 = 1; i_4 < -i_3+PB_Q-1; i_4++) {
                    B[i_0][i_4][i_3][i_0] = E[i_3-1][i_4-1][i_2+i_0][i_2] + F[i_0][1][i_4+i_3+1][i_2][i_2] * G[i_4][i_3][i_2][i_0-1] * E[i_3][i_2][i_0][i_0] + B[i_0-1][i_4][i_3][i_0-1] + B[i_0][i_4-1][i_3+1][i_0] - 5;
                }
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
        C[i_5] = H[i_5] * D[i_5+1] * D[i_5+1] * H[i_5] + C[i_5+2] - C[i_5+1] + 5;
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4314231323_04(xc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
