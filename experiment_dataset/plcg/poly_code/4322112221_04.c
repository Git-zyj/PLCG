...
void kernel_4322112221_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 2; i_2 < i_0; i_2++) {
                for (int i_3 = max(2, i_1); i_3 < PB_Q-3; i_3++) {
                    A[0][i_3][i_3+2] = A[0][i_3-2][i_3] * A[0][i_3+1][i_3+3] * A[0][i_3-1][i_3+1] * A[0][i_1-1][i_1+1] + A[0][i_2-2][i_2] * 2;
                }
            }
        }
    }
...
kernel_4322112221_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
