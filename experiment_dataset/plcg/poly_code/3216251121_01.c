...
void kernel_3216251121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1+1] = C[i_1-1][i_0] * D[i_1] + C[i_0][i_1-1] + E[i_1][i_0-1] * E[i_1+i_0][i_0+1] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_N-1, i_2); i_3++) {
            for (int i_4 = 1; i_4 < min(min(-i_3+PB_L-1, i_2), PB_M-1); i_4++) {
                B[i_4][i_3][i_2][i_4+i_3] = F[i_4+1][i_2][i_3] * F[i_4][i_2][0] - G[i_2][i_3+1][i_4-1] * G[i_4][i_4][i_3] * B[i_4][i_3+1][i_2-2][i_4+i_3+1] + B[i_4][i_3+1][i_2][i_4+i_3+1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3216251121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
