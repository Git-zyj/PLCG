...
void kernel_3414251115_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L; i_1++) {
            B[i_0+1][i_0+1] = E[i_1-1] - F[i_0][i_1][i_1] + G[i_1][i_0] * E[i_1] - G[i_1][1] - F[i_0][i_1][i_0] - B[i_0][i_0] * 2;
        }
        B[i_0][i_0] = H[i_0] + H[i_0+1] * E[i_0+1] + E[i_0] + E[i_0] - 1;
        for (int i_2 = i_0; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_2+PB_L, i_2); i_3++) {
                B[i_0][i_0] = I[i_3+i_2][i_0][i_3] * F[i_3+1][i_0][i_2+1] - F[i_2+1][i_3+1][i_0] * J[i_2][i_3][i_2][i_0] - 4;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            B[i_4-1][i_4-1] = G[i_5][0] - E[i_5] - 6;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3414251115_09(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J)));
