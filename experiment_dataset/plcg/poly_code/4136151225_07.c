...
void kernel_4136151225_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = i_1; i_2 < min(-i_1+PB_M-1, -i_0+PB_N); i_2++) {
                A[i_1+1][i_2+1] = B[i_2+i_0][i_2+i_1+1][i_1-1][i_0] - C[i_2][i_0][i_1+1] * C[i_2][i_0][i_1-1] + D[i_2-1][i_1+1] * C[i_2][i_2+i_0][i_1+1] - A[i_2+1][i_1] * A[i_1-1][i_0+1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4136151225_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
