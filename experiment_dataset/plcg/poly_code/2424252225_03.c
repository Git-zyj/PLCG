...
void kernel_2424252225_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        A[i_0-2][i_0-2] = E[i_0] + E[i_0] - E[i_0-1] * F[i_0] - F[i_0] + A[i_0-4][i_0-4] + 2;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0-2][i_0-2] = G[i_1][i_0+2] + H[i_0][i_0-2] - H[i_1][i_0+1] * G[i_0][i_1] - A[i_0-2][i_0-2] + 5;
        }
    }
    for (int i_2 = 3; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M-2; i_3++) {
            C[i_3][i_2-2] = G[i_3+i_2-2][0] * G[i_3+i_2][i_2+2] * E[i_3] + I[i_3+i_2+2][i_3][i_2] - H[i_2][i_2] - C[i_3+2][i_2-2] - C[i_3][i_2-3] - 3;
        }
    }
    for (int i_4 = 3; i_4 < PB_M-1; i_4++) {
        A[i_4-3][i_4-3] = F[i_4+1] - H[i_4][i_4] - E[0] * H[i_4][0] * 4;
    }
#pragma endscop
...
kernel_2424252225_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
