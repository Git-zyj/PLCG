...
void kernel_3224112311_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_N-1; i_3++) {
            B[i_2][i_3] = C[i_3] + D[i_3-2][0] * B[i_3+1][i_2] - B[i_2-1][i_3+1] + B[i_2+1][i_3] * 3;
        }
    }
#pragma endscop
...
kernel_3224112311_05(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
