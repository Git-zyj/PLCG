...
void kernel_3232212225_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 3; i_2 < PB_N-1; i_2++) {
            B[i_2-1] = D[i_2][i_1] * B[i_2+1] - B[i_2-3] - 5;
        }
    }
#pragma endscop
...
kernel_3232212225_05(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
