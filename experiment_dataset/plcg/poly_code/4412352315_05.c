...
void kernel_4412352315_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
        ...
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_0+PB_M-1, -i_1+PB_M-2); i_3++) {
                    A[i_3+i_0][i_2+1][i_2+2][i_3+i_0-2] = E[i_3-1][i_1][i_2] * F[i_3+1][i_1+1][i_2][i_0] * G[i_2][i_3][i_0+2][i_1] - H[i_1-2][i_2][i_0][i_1-1] - G[i_1+1][i_0-2][i_3][i_2+1] * A[i_3+i_0+1][i_2+1][i_2+2][i_3+i_0-1] - A[i_3+i_1+2][i_2+1][i_2+2][i_3+i_1] - 2;
                }
            }
        }
        B[i_0] = I[i_0] - J[i_0] + I[i_0] - J[i_0-1] + B[i_0-1] - B[i_0+1] + 3;
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 2; i_5 < min(i_4, -i_4+PB_L); i_5++) {
            C[i_4-2][i_5] = K[i_4][2] * L[i_5+i_4][i_5] - K[i_4+1][0] - J[i_5-1] * E[i_5][i_4-1][i_4] - E[i_5-1][i_4][0] + B[i_4] + C[i_5-2][i_4+1] + C[i_4-1][i_5] * 1;
        }
        for (int i_6 = 2; i_6 < PB_L; i_6++) {
            B[i_6] = M[i_6][i_4] * I[i_4-1] + L[i_6-2][i_4+2] + B[i_6] + C[i_4-1][i_6] * 2;
        }
    }
#pragma endscop
