...
void kernel_3432231125_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            B[i_1] = F[i_2][i_1] + F[i_1][i_2] - B[i_2-2] + 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_M+1; i_4++) {
            for (int i_5 = 1; i_5 < min(PB_M, -i_4+PB_Q); i_5++) {
                C[i_5][i_4][i_5+i_4] = G[i_4-1][i_5][i_5] + H[i_4][i_3][i_5] + G[i_3+1][i_5+1][i_4-1] + H[i_5+1][i_3-1][i_4] * C[i_5-1][i_4][i_5+i_4-1] - 2;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M+1; i_6++) {
        B[i_6-1] = E[i_6] - E[i_6+1] - E[i_6] - E[i_6] + 2;
    }
#pragma endscop
...
kernel_3432231125_05(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
