...
void kernel_4212212121_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_M, -i_2+PB_N-1); i_3++) {
            B[i_3+i_2][i_3][i_2-1] = C[i_2+1][i_3-2] + C[i_3][i_2] * B[i_3+i_2+1][i_2+1][i_3-1] * 5;
        }
    }
#pragma endscop
...
kernel_4212212121_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
