...
void kernel_4436332115_00(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            C[i_1+2][i_0-2] = E[i_0] * 6;
        }
        for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
            for (int i_3 = max(2, i_2); i_3 < min(-i_0+PB_L, PB_P-1); i_3++) {
                for (int i_4 = 2; i_4 < PB_P; i_4++) {
                    B[i_3-2][i_2+2][i_4] = F[i_4+1][i_0][i_3] * G[i_4-2][i_3+i_0][i_2][i_3+1] - H[i_0-2][i_4][i_2+2][i_3] * B[i_4-2][i_3+1][i_2] * 2;
                }
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_P; i_5++) {
        for (int i_6 = 0; i_6 < min(-i_5+PB_P, i_5); i_6++) {
            for (int i_7 = 1; i_7 < PB_N; i_7++) {
                C[i_6+2][i_7-1] = F[i_7-1][i_5][i_6] * F[i_7][i_6+i_5][i_5+2] * I[i_7][i_5][i_6] * H[i_6+1][i_7][i_5][0] - 4;
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_P; i_8++) {
        for (int i_9 = 2; i_9 < i_8; i_9++) {
            for (int j_0 = 1; j_0 < min(PB_P-3, -i_8+PB_Q); j_0++) {
                for (int j_1 = 2; j_1 < j_0; j_1++) {
                    D[j_1][i_8][j_0+2][i_9] = J[j_1][i_8][i_9-1][j_0-1] * H[j_0][j_1+1][i_9][i_8] - J[i_9-2][j_0][i_8][j_0+i_8] - G[j_1-2][i_8][0][j_0] * D[j_1][i_8-1][j_0+3][i_9] + 5;
                }
            }
        }
    }
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4436332115_00(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J)));
