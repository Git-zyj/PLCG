...
void kernel_4224211315_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, i_1); i_2++) {
                A[i_0][i_0][i_2] = C[i_2+i_1][i_1+i_0][i_2+1] - A[i_0][i_0][i_2-1] * A[i_0-1][i_0-1][i_2+1] * A[i_1][i_1][i_2-1] * 6;
            }
        }
        for (int i_3 = 0; i_3 < PB_N; i_3++) {
            A[i_0-1][i_0-1][i_3] = D[i_3][i_0] - 1;
        }
    }
#pragma endscop
...
kernel_4224211315_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
