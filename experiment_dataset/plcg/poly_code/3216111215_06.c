...
void kernel_3216111215_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_1+i_0][i_2][i_0] = A[i_1+i_0][i_2-1][i_0] - 2;
            }
        }
        for (int i_3 = i_0; i_3 < PB_M; i_3++) {
            for (int i_4 = i_0; i_4 < -i_0+PB_N-1; i_4++) {
                A[i_4+i_0+1][i_3][i_0] = C[i_0][i_4][i_3] * A[i_4+i_0][i_3][i_4] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3216111215_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
