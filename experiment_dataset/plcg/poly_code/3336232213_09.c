...
void kernel_3336232213_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        A[i_0+2] = D[i_0] * E[i_0+2] - E[i_0] - A[i_0+3] + 1;
    }
    for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_L-2); i_2++) {
            A[i_2+2] = F[i_2][i_1-1] - E[i_2+i_1] - G[i_2][i_1-1] - A[i_2+2] + 2;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_P-3, i_3); i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_P-3; i_5++) {
                C[i_3][i_4][i_5+i_4+1] = H[i_5+2][i_3][i_4] - C[i_3+1][i_5+1][i_5+i_4+3] + C[i_3][i_4][i_5+i_4+2] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3336232213_09(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
