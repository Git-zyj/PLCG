...
void kernel_3232212221_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_M+1; i_2++) {
                A[i_1][i_2-2][i_2+2] = C[i_1][i_2][i_0] + A[i_1+2][i_2-1][i_2+3] - A[i_1][i_2-1][i_2+3] + 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < PB_N; i_4++) {
            for (int i_5 = max(3, i_3); i_5 < PB_M+3; i_5++) {
                A[i_3][i_5-3][i_5+1] = 5;
            }
        }
    }
...
kernel_3232212221_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
