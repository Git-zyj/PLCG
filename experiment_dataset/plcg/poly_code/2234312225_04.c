...
void kernel_2234312225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0-1] = A[i_0-3] * A[i_0+1] + A[i_0-2] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        B[i_2] = C[i_2] * C[i_2] - A[i_2+1] + 5;
    }
#pragma endscop
...
kernel_2234312225_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
