...
void kernel_3334112225_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_M, i_1); i_2++) {
                B[i_2][i_0-2] = D[i_2][i_1][1] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = max(4, i_3); i_4 < PB_M-2; i_4++) {
            B[i_3][i_4-2] = E[i_4+2][i_3] * B[i_3-1][i_4-2] * B[i_3][i_4-4] * 1;
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = max(2, i_5); i_6 < PB_M; i_6++) {
            for (int i_7 = 2; i_7 < PB_M+2; i_7++) {
                C[i_5][i_6][i_7] = C[i_7-2][i_6-2][i_5] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3334112225_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
