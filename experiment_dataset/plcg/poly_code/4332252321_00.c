...
void kernel_4332252321_00(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        C[i_0+1] = D[i_0] + D[i_0] - E[i_0] + D[i_0+1] - E[i_0-1] + E[i_0] + D[i_0] - D[i_0+1] * D[i_0+2] + 2;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                C[i_0+2] = F[i_1-1][i_2-1][0] + F[i_2][i_1+i_0][0] - G[i_2][i_1] + F[i_1][i_2+2][i_0] - 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M+2; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            C[i_4+1] = G[i_3-2][i_4] - H[i_4+1][i_3-2] * C[i_4-1] + C[i_4+2] + C[i_4] - 1;
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4332252321_00(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
