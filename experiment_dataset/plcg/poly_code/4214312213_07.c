...
void kernel_4214312213_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M+2; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M+2; i_2++) {
                A[i_1][i_2+i_0-2] = C[i_0][i_2][i_1+2] - A[i_2][i_1+i_0-2] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                B[i_3][i_5][i_4] = D[i_3][i_5-1][i_4] - B[i_3][i_5-1][i_4-1] - B[i_4][i_3][i_5-1] * B[i_5][i_4][i_3-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4214312213_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
