...
void kernel_4222252223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L+1; i_1++) {
            A[i_1][i_0-2] = C[i_1][i_1+i_0-1] - D[i_0][i_1+1] - D[i_0-1][i_1] + E[i_1] - C[i_1-2][i_1] * F[i_1] + C[i_1][i_0+2] - A[i_1-2][i_0-3] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        B[i_2] = D[i_2][i_2] - C[i_2][0] - F[i_2] * B[i_2+2] * B[i_2+1] + 6;
    }
#pragma endscop
...
kernel_4222252223_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
