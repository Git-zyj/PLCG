...
void kernel_4426132225_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_Q; i_2++) {
                for (int i_3 = 2; i_3 < PB_Q-2; i_3++) {
                    A[i_0-1][i_2][i_1][i_3] = E[i_1][i_2-2][i_1][i_3-2] - E[i_3+2][i_2][i_0-1][i_2+i_1] * A[i_0-1][i_2][i_1][i_3+1] * A[i_0-1][i_2][i_1][i_3+2] - A[i_1-1][i_2][i_3-2][i_0] - 6;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_Q; i_4++) {
        for (int i_5 = i_4; i_5 < PB_L; i_5++) {
            D[i_4-2][i_5] = F[i_5+1][i_5-2][i_4] - G[i_5-1][i_4] + 2;
        }
    }
    for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
        for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
            D[i_6][i_7] = G[i_6-2][i_7] - G[i_6+2][i_7+1] - G[i_7][i_6] - H[0] * G[i_7][i_6] - 4;
        }
        for (int i_8 = 0; i_8 < min(-i_6+PB_M, i_6); i_8++) {
            D[i_6][i_8] = G[i_8+i_6][i_8] * G[i_8][i_6-2] - 2;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4426132225_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
