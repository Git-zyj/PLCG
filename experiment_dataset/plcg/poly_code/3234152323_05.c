...
void kernel_3234152323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        A[i_0] = C[i_0-1][i_0] + D[i_0] * D[i_0] - D[0] - D[i_0-1] - D[i_0] + D[i_0-2] + A[i_0-1] - A[i_0+2] * 3;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            B[i_2-2] = C[i_2+2][i_1] + C[i_1][1] * C[i_1-2][i_2] - A[i_2] * A[i_2+1] + B[i_2] * 6;
        }
    }
#pragma endscop
...
kernel_3234152323_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
