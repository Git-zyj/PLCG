...
void kernel_2422352311_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0] - E[i_0] * F[2] + 3;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            B[i_1][2] = G[i_0+2][i_1] - H[i_0][i_1] + H[i_0][i_1] * I[i_1-1][i_0] - A[i_1] + A[i_0-1] - B[i_0+1][2] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_N, PB_L-2); i_3++) {
            C[i_2][i_3+2] = G[i_2][1] * I[i_3][i_2-2] * H[i_3+i_2][i_3] * G[i_3+1][i_2+1] * E[i_2] * C[i_2-1][i_3+2] + C[i_2+1][i_3+1] - C[i_2][i_3+1] - C[i_2+1][i_3+2] - 2;
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        D[i_4][i_4] = J[i_4-2] + J[i_4-2] - J[i_4] + F[i_4] * F[i_4] - B[i_4][2] - D[i_4+1][i_4+1] - 4;
    }
#pragma endscop
...
kernel_2422352311_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
