...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl))
{
int i_0, i_1, i_2, i_3, i_4;
        ...
        for (int i_1 = 1; i_1 < min(-i_0+PB_L-2, i_0); i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
                for (int i_3 = 1; i_3 < -i_0+PB_L; i_3++) {
                    A[i_1][i_3+i_0][0][i_2] = E[i_3][i_0-1][i_1][i_2] - F[i_3-1][i_1-1][i_2+i_0][i_1][i_2] * E[i_0-1][1][i_3+1][i_2] * G[i_2][i_3][i_2][i_1+1][i_0] - E[i_1-1][i_3][i_0+1][i_3-1] * A[i_1+1][i_3+i_0-2][0][i_2-1] * A[i_3][i_1+i_0+2][0][i_2] + 5;
                }
            }
        }
        for (int i_4 = i_0; i_4 < -i_0+PB_P; i_4++) {
            for (int i_5 = 0; i_5 < min(min(-i_0+PB_M, i_4), -i_4+PB_M); i_5++) {
                B[i_0+1][i_5][i_4] = H[i_5][i_0][i_4+1] - I[i_0][i_4][i_4+i_0] + J[i_5+i_0][i_5+i_4][0] - B[i_4][i_0][i_5] * B[i_4+1][i_0+2][i_5] * 1;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M; i_6++) {
        for (int i_7 = 1; i_7 < PB_M+1; i_7++) {
            for (int i_8 = 1; i_8 < PB_M; i_8++) {
                C[i_8][i_7-1] = C[i_7-1][i_8-1] * 1;
            }
        }
    }
    for (int i_9 = 1; i_9 < PB_Q; i_9++) {
        for (int j_0 = max(i_9, 2); j_0 < PB_M-1; j_0++) {
            D[j_0] = K[j_0][i_9] - L[j_0][i_9-1] * D[j_0-2] + D[j_0+1] - D[j_0-1] * 4;
        }
    }
#pragma endscop
