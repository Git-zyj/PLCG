...
void kernel_2426111321_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q; i_0++) {
        A[i_0-1] = E[i_0][1] - F[i_0] + A[i_0-2] * A[i_0] + A[i_0-3] + 3;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        B[i_1-1] = F[i_1] + C[i_1] - C[i_1-2] * C[i_1-1] * 1;
        C[i_1] = F[0] - 5;
        for (int i_2 = i_1; i_2 < PB_N-1; i_2++) {
            D[i_1][i_2] = B[i_2-1] - B[i_1-1] * B[i_2-2] - B[i_2+1] + 6;
        }
    }
#pragma endscop
...
kernel_2426111321_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
