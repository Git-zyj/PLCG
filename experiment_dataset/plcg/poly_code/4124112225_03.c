...
void kernel_4124112225_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = PB_N-1 - 1; i_1 >= max(2, i_0); i_1--) {
            A[i_1] = B[i_0][i_1+1] - A[i_1+1] * A[i_1-2] + 3;
        }
    }
#pragma endscop
...
kernel_4124112225_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
