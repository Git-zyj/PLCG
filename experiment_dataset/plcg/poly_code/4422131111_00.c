...
#endif
static void init_array(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int seed)
{
srand(seed);
...
void kernel_4422131111_00(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                for (int i_3 = 1; i_3 < -i_0+PB_Q-1; i_3++) {
                    D[i_2][i_0+1][i_3+i_1][i_3] = E[i_3-1][i_2][i_1+i_0] * F[i_1+1][i_2-1][i_3+i_0+1][i_1][i_0] - D[i_3][i_2][i_1+i_0-1][i_1-1] + 5;
                }
                C[i_2-1][i_1] = E[i_1][i_2][i_0] + G[i_0][i_1] * E[i_1][i_0][i_1] - 4;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        for (int i_5 = 1; i_5 < PB_N; i_5++) {
            for (int i_6 = 1; i_6 < PB_L; i_6++) {
                C[i_5-1][i_6] = E[i_5][i_6-1][i_5] + E[i_6][i_5][i_4-1] - E[i_6][i_4-1][i_5-1] * E[i_6][i_4][1] * G[i_6][i_4] + C[i_5][i_6] * 4;
            }
        }
        for (int i_7 = 0; i_7 < PB_P-1; i_7++) {
            for (int i_8 = i_4; i_8 < PB_L; i_8++) {
                for (int i_9 = 0; i_9 < -i_8+PB_M; i_9++) {
                    D[i_7][i_4][i_9+i_8][i_8] = H[i_4][i_8][i_7+1][i_8] * 3;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422131111_00(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
