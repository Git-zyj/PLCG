...
void kernel_3224211325_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_N+1, PB_M); i_1++) {
            B[i_1+i_0-1][i_1+1] = C[i_1][i_0-1][i_0] + B[i_1+i_0-2][i_1+1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-3; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N-3; i_3++) {
            B[i_3+i_2-1][i_3+1] = B[i_3+i_2+3][i_3+3] * B[i_3+i_2+1][i_3+3] + 4;
        }
    }
#pragma endscop
...
kernel_3224211325_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
