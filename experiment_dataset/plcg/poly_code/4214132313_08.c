...
void kernel_4214132313_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M-2, PB_L); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_L-1, -i_1+PB_M-4); i_2++) {
                B[i_2+i_1+1][i_2-1] = C[i_2][i_1] - B[i_2+i_1+3][i_2+1] * B[i_2+i_1+4][i_2+1] - B[i_2+i_1+3][i_2] + 4;
            }
            B[i_1+i_0+2][i_1] = C[i_1-1][i_0] * D[i_0] * E[i_1-2][i_1][i_0] * C[i_0][1] * D[i_1] + 1;
        }
    }
#pragma endscop
...
kernel_4214132313_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
