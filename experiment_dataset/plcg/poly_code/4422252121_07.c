...
void kernel_4422252121_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
        ...
        for (int i_1 = 0; i_1 < -i_0+PB_P-2; i_1++) {
            for (int i_2 = 2; i_2 < i_0; i_2++) {
                for (int i_3 = 2; i_3 < min(PB_M-2, -i_2+PB_M); i_3++) {
                    A[i_1+i_0][i_3+2][i_0][i_3] = E[i_3+1][i_0][i_2+i_1+2][i_1+1] - F[i_0][i_2-2][i_1][i_3][i_3-2] - G[i_2][i_0][i_1] * G[i_2][i_3+i_1][i_0] + E[i_0][i_2][i_3][i_0] - H[i_1+2][i_2-2][i_0][i_3+2] * I[i_3][i_3][i_1+i_0+2][i_2][i_2] + G[i_3+i_2][i_0][i_1] * 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            for (int i_6 = i_5; i_6 < PB_M; i_6++) {
                B[i_4+1][i_5][i_6] = G[i_4][i_6][i_4+2] * J[i_6][i_5-1] * G[i_6][i_5][i_4-1] + G[i_6][i_4-1][i_5] - B[i_4-1][i_5][i_6] - 6;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-3; i_7++) {
        C[i_7+2] = K[i_7-1] + J[i_7+1][i_7] + L[i_7] * K[i_7] * C[i_7] + C[i_7+3] + 1;
        for (int i_8 = 0; i_8 < PB_M; i_8++) {
            for (int i_9 = 0; i_9 < i_8; i_9++) {
                D[i_9][i_7+2][0] = G[i_8][i_7][1] - M[i_8][i_7+1][i_9] * G[i_8][i_7][i_9] - 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4422252121_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
