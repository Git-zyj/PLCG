...
void kernel_4436131313_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_2+PB_Q, PB_L-1); i_3++) {
                    A[i_2][i_1][i_1][i_3] = E[i_2-1][i_3][i_1][i_0] - F[i_1][i_2][i_3+1] - E[i_0][i_1][i_3+i_2][i_2-1] + E[i_3-1][i_0+1][i_2][i_0] - A[i_2][i_1][i_1][i_3+1] - 6;
                }
            }
        }
        for (int i_4 = 0; i_4 < min(i_0, PB_L-2); i_4++) {
            B[i_4+1] = G[i_4+1][i_0] + B[i_4] * B[i_4+2] * 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_L-2; i_5++) {
        B[i_5+2] = H[i_5-1] * H[i_5] + H[i_5] - 4;
        for (int i_6 = 1; i_6 < -i_5+PB_L; i_6++) {
            for (int i_7 = 1; i_7 < min(min(-i_5+PB_L+1, -i_6+PB_L-1), i_5); i_7++) {
                D[i_6+i_5][i_7][i_7][i_6] = F[i_7-1][i_5][i_6-1] + F[i_7-1][i_5][i_6] * G[i_6][0] + F[i_7+i_6+1][i_5+1][1] + B[i_5+1] - D[i_7+i_6+1][i_5+1][i_5+1][i_6+1] + D[i_6+i_5-1][i_7-1][i_7-1][i_6] * D[i_7+i_5-1][i_6][i_6][i_7-1] - 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436131313_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
