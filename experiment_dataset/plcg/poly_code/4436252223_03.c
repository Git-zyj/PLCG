...
void kernel_4436252223_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 3; i_1 < PB_M+1; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < min(-i_0+PB_L, PB_M-2); i_2++) {
                for (int i_3 = 2; i_3 < PB_M; i_3++) {
                    A[i_2-2][i_3-1][i_0] = E[i_3][i_2+i_0][i_3-1][0] - E[i_2+2][i_3][i_1][i_0] * F[i_1-1][i_3][i_2] * A[i_2-2][i_3-2][i_1] + A[i_3-2][i_1-3][i_2] - 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-2; i_5++) {
            B[i_4-2][i_4] = G[i_4][i_5+2] * H[i_5][i_4] + I[i_4+2] * H[i_5][i_4+2] * B[i_4-1][i_4+1] * 3;
        }
    }
    for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
        for (int i_7 = 3; i_7 < i_6; i_7++) {
            B[i_7-2][i_7] = G[i_7+1][i_6+2] + H[i_7][i_6] - H[i_7+1][i_6] - 6;
            for (int i_8 = 2; i_8 < PB_M-2; i_8++) {
                B[i_7-3][i_7-1] = F[i_8+2][i_7][i_6] + F[i_7][i_8][i_6] - J[i_7-1][i_8][i_6] - J[i_7-1][i_6][i_8] * F[i_6][i_8][i_7] * E[i_8-1][i_7][i_8][i_6] + F[i_8][i_7][i_8] + A[i_8-2][i_7-1][i_6+1] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4436252223_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
