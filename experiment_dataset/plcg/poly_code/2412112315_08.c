...
void kernel_2412112315_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_1] = E[i_0-2][i_1] - B[i_0] - B[i_1-1] - 3;
        }
        B[i_0] = B[i_0+1] * B[i_0-1] + 1;
        B[i_0-1] = 1;
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        D[i_2] = F[i_2-1] - E[i_2][1] + D[i_2-1] + D[i_2+1] + 1;
    }
#pragma endscop
...
kernel_2412112315_08(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
