...
void kernel_3422311325_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_L+1); i_1++) {
            C[i_1] = E[i_1+i_0-1] * F[i_1][i_0-1] * 6;
        }
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            for (int i_3 = 3; i_3 < PB_N; i_3++) {
                B[i_2][i_2][i_3-1] = G[i_3][i_2] - B[i_3-1][i_3-1][i_2-1] - B[i_2-2][i_2-2][i_3-1] * B[i_2+1][i_2+1][i_3-1] + B[i_2-2][i_2-2][i_3] * B[i_2][i_2][i_3-3] + 4;
            }
        }
        C[i_0] = H[i_0] - 2;
        C[i_0] = 3;
...
kernel_3422311325_02(xc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
