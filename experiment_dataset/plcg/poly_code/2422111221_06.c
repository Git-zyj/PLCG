...
void kernel_2422111221_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            A[0] = A[0] * A[0] * B[i_1+2][i_0] - 2;
        }
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            B[i_2][i_0] = E[i_2][i_0+1] + 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            C[i_3] = E[i_4+1][i_3-1] - E[i_4-1][i_3-1] - D[i_4] - 1;
            D[i_4] = E[i_4][i_3] + C[i_4] + 2;
        }
    }
#pragma endscop
...
kernel_2422111221_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
