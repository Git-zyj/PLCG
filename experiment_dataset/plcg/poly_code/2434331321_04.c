...
void kernel_2434331321_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        B[i_0] = E[i_0] + D[i_0-2] * 2;
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            B[i_1] = F[i_1][i_0+1] + G[i_0-1][i_1] * H[i_1] + E[i_0] * I[i_1][i_1] + B[i_1-2] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_P; i_3++) {
            C[i_2][0] = F[i_3-1][i_2-1] + I[i_3][i_2] + F[i_2][i_3+i_2] - C[i_3+1][0] + D[i_3+1] - 5;
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        D[i_4] = H[i_4-1] * D[i_4+1] + D[i_4-2] + D[i_4+2] + D[i_4-1] - D[i_4+2] * 2;
    }
#pragma endscop
...
kernel_2434331321_04(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
