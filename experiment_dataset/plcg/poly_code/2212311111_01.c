...
void kernel_2212311111_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = 0; i_2 < min(PB_M-1, -i_0+PB_P-1); i_2++) {
            B[i_2] = C[i_2][i_2+i_0+1] - B[i_2+1] * 6;
        }
    }
#pragma endscop
...
kernel_2212311111_01(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
