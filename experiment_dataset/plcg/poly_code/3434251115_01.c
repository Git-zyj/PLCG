...
void kernel_3434251115_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        B[i_0-1] = E[i_0+1] + F[i_0] * E[i_0] - E[i_0+1] - E[i_0] - 5;
        B[i_0-1] = F[i_0+1] - E[i_0+1] - F[i_0] * E[i_0-1] + E[i_0] * F[i_0] * F[i_0] * 4;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_P); i_2++) {
            C[i_2+1] = G[i_1][i_2] + H[i_2][i_1] - H[i_2][i_2+i_1] * C[i_2] * 3;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        D[i_3] = F[i_3] + F[i_3+1] - E[i_3] - F[0] * 4;
    }
#pragma endscop
...
kernel_3434251115_01(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
