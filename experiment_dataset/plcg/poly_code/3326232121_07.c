...
void kernel_3326232121_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_N, -i_1+PB_L-1); i_2++) {
            B[i_2][i_1-2] = F[i_1][i_2][i_1] + G[i_2][i_2+i_1+1] + E[i_1] - B[i_2][i_1-1] + 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        A[i_3] = D[i_3+1] - E[i_3+2] + D[i_3] + D[i_3+1] + 2;
    }
#pragma endscop
...
kernel_3326232121_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
