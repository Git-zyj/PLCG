...
void kernel_3316212211_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_1+1][i_0][i_0] = A[i_1+2][i_0-1][i_0-1] * A[i_1+1][i_0+1][i_0+1] - 1;
            B[i_0][i_1] = A[i_1][i_0][i_0] + B[i_0-1][i_1] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            C[i_3] = D[i_3+1][i_2+2] + C[i_3+1] * 3;
        }
    }
#pragma endscop
...
kernel_3316212211_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
