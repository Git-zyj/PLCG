...
void kernel_3422111221_00(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 1; i_3 < i_2; i_3++) {
                B[i_1][i_2][i_3] = B[i_1+2][i_3-1][i_2+1] + 3;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        C[i_4] = D[1] - D[1] + D[1] + 5;
        D[1] = E[i_4] * C[i_4] - D[1] * 5;
    }
#pragma endscop
...
kernel_3422111221_00(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E));
