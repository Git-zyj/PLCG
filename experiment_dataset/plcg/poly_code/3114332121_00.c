...
void kernel_3114332121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_N, -i_0+PB_L+1); i_1++) {
            A[i_1+i_0-1][2] = B[i_1] + C[0] - D[i_1-1][i_0] * A[i_1+i_0-3][2] - 1;
        }
    }
#pragma endscop
...
kernel_3114332121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
