...
void kernel_4216152313_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        B[i_0] = C[i_0] * C[i_0] * C[i_0] - C[i_0] + C[i_0] - C[i_0] - 5;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            B[i_1] = C[i_2+i_1] * C[i_2-2] - D[i_1+1][i_2-1] - D[i_2][i_1-1] - B[i_2-1] + B[i_2+1] + B[i_1+1] * 5;
        }
    }
#pragma endscop
...
kernel_4216152313_02(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
