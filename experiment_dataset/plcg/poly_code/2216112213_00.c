...
void kernel_2216112213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            A[i_1+i_0][i_0] = A[i_1+i_0-2][i_0-1] - 1;
        }
        for (int i_2 = 2; i_2 < min(min(-i_0+PB_L-1, i_0), PB_M-2); i_2++) {
            B[i_0+1] = C[i_2-2][i_0+1] + A[i_2+i_0+1][i_0] * B[i_2+2] - B[i_2] - 2;
        }
    }
#pragma endscop
...
kernel_2216112213_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
