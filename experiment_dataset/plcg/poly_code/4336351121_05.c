...
void kernel_4336351121_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
        ...
        for (int i_1 = 0; i_1 < min(i_0, PB_M-2); i_1++) {
            for (int i_2 = 0; i_2 < min(PB_Q, i_0); i_2++) {
                for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
                    A[i_1][i_0][i_3+1][0] = D[i_3-1][i_0][i_3][i_1] + E[i_1][i_0-1][i_3-1][i_2][i_2] * A[i_1+2][i_2][i_3+2][0] * 3;
                }
            }
        }
    }
        ...
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
                for (int i_7 = 0; i_7 < -i_4+PB_Q-1; i_7++) {
                    B[i_6+1][0][i_5][i_7+i_4+1] = F[i_7][i_5][i_4+1][1] * F[i_4][i_7][i_6+1][i_5] * F[i_4+1][i_7][i_6+1][i_5] * G[i_5][i_6-1][i_4][1] - G[i_7][i_4-1][i_5-1][i_6+1] * 4;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_Q; i_8++) {
        for (int i_9 = 1; i_9 < PB_Q; i_9++) {
            for (int j_0 = 2; j_0 < min(PB_Q, -i_8+PB_M); j_0++) {
                C[j_0-1][i_9-1] = H[i_8+1][j_0][i_9] + I[j_0][i_8-1][1] - J[i_9][i_8][i_9] * K[i_9][j_0-1] - J[j_0+1][i_8][i_9-1] - J[j_0+i_8][i_9][i_8] * C[j_0-2][i_9-1] - 2;
            }
        }
    }
#pragma endscop
