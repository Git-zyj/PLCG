...
void kernel_3216132223_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_N; i_1++) {
            for (int i_2 = i_1; i_2 < PB_P-2; i_2++) {
                B[i_0-2][i_1+2] = B[i_2][i_1] - B[i_1][i_2+2] + 5;
            }
            B[i_0][i_1] = C[i_1+1] * C[i_1+i_0] - D[i_1-2][i_0] - C[i_1] - D[i_1][i_0] * 6;
        }
    }
#pragma endscop
...
kernel_3216132223_07(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
