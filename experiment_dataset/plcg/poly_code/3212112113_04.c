...
void kernel_3212112113_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-2; i_0++) {
        ...
        for (int i_2 = 1; i_2 < min(-i_0+PB_P+1, PB_L-1); i_2++) {
            B[i_2][i_2+i_0-2] = C[i_0][i_2+1] * B[i_2-1][i_2+i_0-3] - B[i_2+1][i_2+i_0-1] * 1;
        }
    }
#pragma endscop
...
kernel_3212112113_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
