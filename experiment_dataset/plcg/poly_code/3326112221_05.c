...
void kernel_3326112221_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            A[i_1][i_0] = D[i_1+1][i_1] - A[i_1+1][i_0+2] * A[i_1][i_0-1] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < -i_2+PB_P; i_4++) {
                B[i_2][i_3+2][i_4+i_2][i_4] = 1;
            }
        }
        for (int i_5 = 4; i_5 < i_2; i_5++) {
            for (int i_6 = max(-i_5+4, i_5); i_6 < min(PB_L, -i_5+PB_M); i_6++) {
                C[i_6+i_5-2][1][i_5] = C[i_6+i_5-4][1][i_5] * C[i_6+i_5][1][i_5] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3326112221_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
