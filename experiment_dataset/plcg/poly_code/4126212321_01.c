...
void kernel_4126212321_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q, i_0); i_1++) {
            for (int i_2 = 0; i_2 < min(PB_P-3, i_0); i_2++) {
                for (int i_3 = max(4, i_2); i_3 < PB_Q+2; i_3++) {
                    A[i_0][i_2+1][i_0][i_3-2] = B[i_2][i_0+1][i_1+i_0] + A[i_0][i_2+1][i_0][i_3-4] + A[i_0][i_2+3][i_0][i_3-2] * A[i_0-2][i_2+1][i_0-2][i_3-2] + 4;
                }
            }
        }
    }
...
kernel_4126212321_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
