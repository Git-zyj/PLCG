...
void kernel_2436352125_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L-2, i_0); i_1++) {
            A[i_1+i_0+1][i_0] = E[i_1][i_0] + F[0] + G[i_1][i_0-2] - E[i_1][i_0] - F[i_1+i_0+2] - H[i_1][i_0-2] - A[i_1+i_0+3][i_0] - 4;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_M+1; i_2++) {
            B[i_2+i_0-1][i_0] = I[i_2+1] + H[i_2][i_0-2] - G[i_0-2][i_2] + G[i_0][1] * B[i_2+i_0-1][i_0-1] + 1;
        }
        for (int i_3 = 2; i_3 < min(-i_0+PB_N-1, PB_L); i_3++) {
            A[i_3+i_0+1][i_0] = H[i_3-2][i_0-2] * G[i_0-2][i_3] * F[1] + I[i_3] * 2;
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        D[i_4-1] = F[i_4] - F[i_4] + J[i_4-2] + I[i_4] + F[i_4] + J[i_4] - 4;
    }
#pragma endscop
...
kernel_2436352125_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
