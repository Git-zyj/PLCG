...
void kernel_4236232123_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(min(PB_Q, -i_1+PB_L), -i_0+PB_L+1); i_2++) {
                A[i_2][i_1][i_2+i_1] = C[i_1][i_2-1][i_0+2] * D[i_2][i_1][i_0] + D[i_2][i_1][i_0] + D[i_1][i_1][i_0] * C[i_1][i_2][i_0] * A[i_0-1][i_2][i_2+i_0-1] * 1;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_L-1; i_3++) {
            A[i_3+1][i_0][i_3+i_0+1] = 2;
        }
    }
#pragma endscop
...
kernel_4236232123_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
