...
void kernel_4422152125_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0-2] * E[i_0+2] + E[i_0] + E[i_0+1] + 6;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                B[i_1-1][i_0-1][i_0] = F[i_0-1][i_2][i_1] + F[i_0][i_2+1][i_1] - F[i_1-1][i_2][0] * 5;
            }
            for (int i_3 = 2; i_3 < -i_1+PB_L; i_3++) {
                C[i_3+1][i_0][i_0] = G[i_1+2][i_0][i_3][i_0] + G[i_3][i_3+i_1][i_0][1] + F[i_3-2][i_1][i_0] * F[i_1+1][i_0+1][i_3+1] + F[i_1+1][i_0][i_0+2] + C[i_3][i_0-2][i_0-2] + 2;
            }
        }
    }
        ...
        for (int i_5 = 1; i_5 < min(i_4, PB_L-2); i_5++) {
            for (int i_6 = 2; i_6 < -i_5+PB_L-2; i_6++) {
                for (int i_7 = 1; i_7 < PB_L-2; i_7++) {
                    C[i_7+2][i_6][i_6] = F[i_7][i_6+i_5+2][i_4] - G[2][i_7+2][i_6][i_4] + G[i_4][i_7+2][i_5][i_6] - F[i_6-1][i_7-1][i_4] + F[i_7][i_4][i_6] - H[i_4][i_7][i_6-2][i_6+i_5-1][1] + 3;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422152125_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H)));
