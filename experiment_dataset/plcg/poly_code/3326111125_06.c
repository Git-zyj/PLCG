...
void kernel_3326111125_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 3; i_2 < PB_M-1; i_2++) {
                A[i_1][i_2][i_2-1] = D[i_2+1][i_1][0] * A[i_1-2][i_2-2][i_2-3] - 3;
            }
        }
        for (int i_3 = max(2, i_0); i_3 < -i_0+PB_M+1; i_3++) {
            A[i_0][i_3-1][i_3-2] = D[i_3+i_0-1][i_3][1] + 2;
        }
    }
...
kernel_3326111125_06(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
