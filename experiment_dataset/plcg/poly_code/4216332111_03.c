...
void kernel_4216332111_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M, i_0); i_2++) {
                for (int i_3 = 2; i_3 < min(-i_0+PB_N+1, i_1); i_3++) {
                    A[i_3+i_0-1][i_3-1][i_1+i_0-1][i_2] = C[i_2][i_3][i_1] * D[i_2+i_0][i_3-1][i_1][1] + E[i_0][i_3][i_2][i_1] - F[i_2][i_3+1][i_1+i_0][i_2] * F[i_3+1][i_2][i_0][i_1-1] + A[i_3+i_0-2][i_3-2][i_1+i_0][i_2] - 1;
                }
            }
        }
    }
        ...
        for (int i_5 = max(i_4, 2); i_5 < -i_4+PB_M; i_5++) {
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                for (int i_7 = 2; i_7 < -i_5+PB_N+2; i_7++) {
                    A[i_7+i_5-2][i_7-2][i_5+i_4][i_6] = E[i_5][i_7][i_6][0] + 3;
                }
            }
        }
    }
...
kernel_4216332111_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
