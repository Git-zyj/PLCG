...
void kernel_4416151221_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 2; i_1 < -i_0+PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(i_0, -i_1+PB_L); i_2++) {
                for (int i_3 = 1; i_3 < min(-i_2+PB_L+1, i_2); i_3++) {
                    A[i_1][i_0][i_3+i_2-1][i_2] = E[i_1][i_3][i_2][0] + E[i_3-1][i_0+1][i_2+i_1][i_1] - E[i_0][i_3][i_1][i_2] + E[i_1][i_3+1][i_0][i_2] * E[i_0][i_2+i_1][i_3][i_2] * A[i_1-2][i_0][i_3+i_2-1][i_2] * A[i_1][i_0-1][i_3+i_2-1][i_2+1] + 1;
                }
            }
            B[i_0+1][0] = F[i_1+i_0] - F[i_1] + G[i_1-1][i_0-1] - G[i_1+i_0+1][i_1] * G[i_1+i_0][0] * 6;
        }
        for (int i_4 = 1; i_4 < min(i_0, -i_0+PB_M+1); i_4++) {
            C[i_0-1][i_4+1] = F[i_4+i_0-1] * F[i_4+i_0-1] - G[i_4+1][0] - F[i_4] + C[i_0-1][i_4] + C[i_4-1][i_0] - 3;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-3; i_5++) {
        for (int i_6 = 2; i_6 < PB_L; i_6++) {
            for (int i_7 = i_5; i_7 < PB_L-2; i_7++) {
                D[i_5+1][i_7][i_6+1] = G[i_5][i_6] - H[i_5][i_6][i_7+1] + H[i_5-1][i_6-1][i_7] * H[i_6][i_7-1][i_5-1] * H[i_7-1][i_5][i_6-1] - D[i_5+1][i_7+2][i_6-1] + D[i_5+3][i_7+2][i_6+1] + D[i_7-1][i_6-2][i_5+1] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4416151221_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
