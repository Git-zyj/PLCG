...
void kernel_4232211225_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        ...
        for (int i_4 = max(1, i_0); i_4 < PB_L-1; i_4++) {
            for (int i_5 = 2; i_5 < -i_4+PB_L+1; i_5++) {
                B[i_4+1][i_4] = C[i_0][i_5+i_4-1][i_4] * B[i_5-1][i_5-2] + B[i_4][i_4-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4232211225_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
