...
void kernel_4334312113_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
                B[i_2][i_1] = D[i_2-1][i_1] * 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            B[i_4][i_3] = B[i_4+1][i_3+1] + 6;
        }
    }
    for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 0; i_6 < -i_5+PB_M-1; i_6++) {
            C[i_6+i_5] = C[i_6+i_5+1] - 1;
        }
    }
#pragma endscop
...
kernel_4334312113_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
