...
void kernel_2416332215_01(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        D[i_0+1] = E[0] - E[i_0] - F[i_0+2] * 3;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            B[i_2][i_1] = F[0] * G[i_1][i_2] - H[i_2+i_1][i_2] + I[1] * F[i_2+i_1] - D[i_1+3] + 3;
            D[i_2+1] = H[i_2+i_1][0] - H[i_1-2][i_2] - B[i_2][i_1] + B[i_2][i_1-1] + 2;
        }
        D[i_1+2] = I[i_1] + D[i_1+1] - 4;
    }
#pragma endscop
...
kernel_2416332215_01(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
