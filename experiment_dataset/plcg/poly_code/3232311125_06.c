...
void kernel_3232311125_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_P+2; i_2++) {
                B[i_1-1][i_2-2] = 6;
                B[i_0-1][i_1] = C[i_1-1][i_2+1][i_0] * B[i_2-1][i_1+2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3232311125_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
