...
void kernel_3212212323_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_1][i_2][i_0] = A[i_2][i_1-2][i_0] + A[i_1-2][i_2][i_0] * A[i_1][i_2][i_0-1] * 1;
            }
        }
        B[i_0-1] = C[i_0] * D[i_0] * B[i_0+1] - B[i_0-2] - B[i_0-3] - 2;
    }
#pragma endscop
...
kernel_3212212323_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
