...
void kernel_3334212323_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            A[i_0][i_1+1] = D[i_1][i_0-2] * A[i_0][i_1-1] + A[i_0][i_1+2] + B[i_1-1] - 5;
        }
    }
    ...
    for (int i_3 = 3; i_3 < PB_N-2; i_3++) {
        C[i_3-1] = B[i_3+2] - C[i_3+1] + C[i_3-3] * C[i_3] * 4;
    }
#pragma endscop
...
kernel_3334212323_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
