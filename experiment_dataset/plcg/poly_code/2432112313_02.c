...
void kernel_2432112313_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            B[i_0][i_2] = B[i_0+1][i_2+1] * B[i_0][i_2-1] * B[i_2-1][i_0] - 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            C[i_4] = C[i_4+1] - C[i_3-1] * C[i_4-1] + 5;
        }
    }
...
kernel_2432112313_02(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E));
