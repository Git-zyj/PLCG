...
void kernel_2232212315_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
            B[i_2] = B[i_1+1] * B[i_2-1] + B[i_2+1] + 2;
        }
    }
#pragma endscop
...
kernel_2232212315_04(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
