...
void kernel_3224312115_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_P; i_1++) {
            A[i_1-1][i_0+2][i_1] = A[i_1-1][i_0+3][i_1] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_M+1, PB_P); i_3++) {
            for (int i_4 = 2; i_4 < PB_P; i_4++) {
                A[i_4-1][i_2+1][i_4] = C[i_3+i_2-1][i_4][1] - D[i_2+2][i_4-2][i_3] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3224312115_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
