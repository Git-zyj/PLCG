...
void kernel_3232212115_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 0; i_4 < min(min(-i_3+PB_M, -i_2+PB_M+1), i_2); i_4++) {
                B[i_3-2][i_2][i_4+1] = C[i_4+i_2-1][i_3+1][i_2] - D[i_4+i_3][i_2+1][i_3-2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3232212115_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
