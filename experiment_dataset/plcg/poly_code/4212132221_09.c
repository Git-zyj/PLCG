...
void kernel_4212132221_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
                A[i_1][i_2] = C[i_2][i_1][i_0+1] - C[i_2][i_0][i_1+i_0+1] - C[i_1+i_0][i_2][i_0] - C[i_1][i_0][i_1] - A[i_1][i_2+2] * A[i_1+1][i_2-2] * 5;
            }
        }
        for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 2; i_4 < PB_M; i_4++) {
                for (int i_5 = 0; i_5 < min(PB_M, -i_3+PB_L+1); i_5++) {
                    B[i_3+1][i_4][i_5+i_3-1] = C[i_5][i_4+1][i_3+1] - B[i_3+1][i_4-2][i_5+i_3-1] + B[i_3+1][i_4][i_5+i_3-2] * 5;
                }
            }
        }
    }
...
kernel_4212132221_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
