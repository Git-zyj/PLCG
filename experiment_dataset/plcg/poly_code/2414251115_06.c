...
void kernel_2414251115_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        B[i_0-1] = E[1] + E[i_0] - F[i_0+1] * F[i_0+1] - F[0] * B[i_0-1] * 5;
        B[i_0-1] = E[i_0] + E[i_0] * F[i_0] * G[i_0][1] * 1;
    }
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            C[i_2][i_1] = H[i_2][i_2-1] * H[i_2][i_1] - E[i_2] + 5;
        }
        D[i_1+1] = F[i_1] * F[i_1] * E[i_1-1] * G[i_1][i_1+1] + E[i_1] - 4;
    }
#pragma endscop
...
kernel_2414251115_06(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
