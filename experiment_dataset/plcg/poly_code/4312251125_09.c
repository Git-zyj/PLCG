...
void kernel_4312251125_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = D[i_0] + D[i_0-1] - D[i_0] - D[i_0] + A[i_0-1] * 3;
        A[i_0] = D[i_0] + D[i_0] * E[i_0-1] + E[i_0+1] + D[0] - E[i_0] + 6;
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                    C[i_3][i_2+1][i_3] = F[i_1][i_3][i_2][i_0] + F[i_2-1][i_0+1][i_3][i_1] * G[i_2+1][i_3][i_1] + C[i_3+2][i_2-1][i_3+2] + 5;
                }
            }
        }
    }
...
kernel_4312251125_09(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
