...
void kernel_3322112325_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = i_1; i_2 < -i_1+PB_M; i_2++) {
                A[i_0][i_2+i_1][i_1] = A[i_0-2][i_2+i_1][i_1] * 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            B[i_4+2][i_3] = D[i_4][i_3+1] + B[i_4][i_3] - B[i_4+2][i_3-1] - 1;
        }
    }
    for (int i_5 = 2; i_5 < PB_N-2; i_5++) {
        C[i_5] = C[i_5+2] - C[i_5-1] + C[i_5-2] - C[i_5+1] - 2;
    }
#pragma endscop
...
kernel_3322112325_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
