...
void kernel_3236152321_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L, -i_0+PB_M+2); i_1++) {
            A[i_1+i_0-2][2] = C[i_0][i_0] + C[i_1][i_0] + C[i_1][i_0] * C[i_1][i_0] + C[i_1][i_0+1] - A[i_1+i_0-3][2] + A[i_1+i_0-3][2] + A[i_1+i_0-4][2] * 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 2; i_4 < min(-i_2+PB_L, i_2); i_4++) {
                B[i_2][i_3][i_4-1] = D[i_3][i_4+i_2][i_3] + D[i_4][i_2+2][i_3] - D[i_2+1][i_4][i_3+1] + C[i_3][i_2] - B[i_2+1][i_3][i_4-1] + B[i_2][i_3+1][i_4-2] - B[i_2][i_3][i_4+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3236152321_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
