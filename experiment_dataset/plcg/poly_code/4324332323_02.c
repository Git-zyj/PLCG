...
void kernel_4324332323_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] + E[i_0+2] * A[i_0+2] + A[i_0-2] * 2;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_M+2; i_2++) {
            B[i_1][i_2] = F[i_2-2][i_1] + G[i_1+1][0] + B[i_1-1][i_2] + B[i_1-1][i_2-2] * 4;
        }
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, -i_1+PB_N); i_4++) {
                B[i_1][i_4+1] = H[i_4+1][i_4+i_1][i_3] + G[i_4+i_3][i_1] - I[i_3+2][i_4][i_1] + J[i_3+2][i_4+i_1][i_4+2] - B[i_3][i_1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4324332323_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
