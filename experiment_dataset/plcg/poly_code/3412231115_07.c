...
void kernel_3412231115_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0+1] = E[0] * F[i_0] * 3;
        for (int i_1 = 0; i_1 < -i_0+PB_L-1; i_1++) {
            B[i_1+1][i_0+1] = G[i_1+1][i_0] * G[i_1+i_0][i_0+1] - H[i_1+i_0+1][i_0] * B[i_1+1][i_0] + 1;
        }
        for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
            A[i_2+2] = G[i_0][i_2] + H[i_2][i_0] + G[i_2][i_0+1] * A[i_2] + 2;
            A[i_0+1] = G[i_2][i_0] * H[i_0][i_0] * H[i_2][i_0] * 2;
        }
    }
#pragma endscop
...
kernel_3412231115_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
