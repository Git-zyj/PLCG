...
void kernel_4416212213_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-3; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_Q-2; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M+2; i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_Q-3; i_3++) {
                    A[i_2+i_0-2][i_1][i_3][i_1+i_0+2] = A[i_2+i_0-2][i_3+1][i_1+1][i_3+i_0+3] * 1;
                }
            }
        }
        for (int i_4 = i_0; i_4 < PB_M; i_4++) {
            for (int i_5 = 2; i_5 < PB_Q-3; i_5++) {
                for (int i_6 = 0; i_6 < -i_5+PB_P-2; i_6++) {
                    B[i_4][i_0+2][i_5+2][i_6] = E[i_4][i_0][i_6+i_5+2][i_4][i_5-2] - B[i_4][i_0+2][i_5+3][i_6] + B[i_4][i_5+2][i_0+1][i_6] * 6;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), INIT_SEED);
kernel_4416212213_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E)));
