...
static void print_array(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,int vl,int wl,DATA_TYPE POLYBENCH_5D(L,xL,yL,zL,vL,wL,xl,yl,zl,vl,wl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4432251113_06(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,int vl,int wl,DATA_TYPE POLYBENCH_5D(L,xL,yL,zL,vL,wL,xl,yl,zl,vl,wl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        C[i_0][i_0] = E[0] * F[i_0] - 3;
    }
        ...
        for (int i_2 = 1; i_2 < min(-i_1+PB_P+1, i_1); i_2++) {
            for (int i_3 = 0; i_3 < -i_1+PB_L; i_3++) {
                for (int i_4 = 1; i_4 < min(-i_3+PB_M, PB_L-1); i_4++) {
                    B[i_2-1][i_3][i_4+1] = G[i_4][i_1][i_2-1][i_3+1] * G[i_4][i_2][i_3+i_1][0] + H[i_3][i_4+1][i_2+i_1-1][i_4] * H[i_2+1][i_4-1][i_1][i_1] + I[i_4+i_3][i_2][i_2+1] - G[i_4][i_1][i_3][i_2-1] + B[i_2][i_4][i_1+2] * 5;
                }
            }
        }
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            C[i_1][i_1] = F[i_5] * J[i_5-1][i_1] + E[i_1-1] - K[i_1+1][i_5-1] * J[i_5-1][i_1] + C[i_1-1][i_1-1] + 3;
        }
    }
        ...
        for (int i_7 = 1; i_7 < -i_6+PB_M; i_7++) {
            for (int i_8 = 1; i_8 < PB_N; i_8++) {
                for (int i_9 = 1; i_9 < min(-i_8+PB_P, PB_M-1); i_9++) {
                    B[i_8-1][i_9-1][i_6+1] = L[i_6+1][i_9+i_7][i_8][i_7][1] * H[i_7][i_6-1][i_9-1][1] - M[i_9-1][i_9-1][i_7] - G[i_9][i_6][i_9][i_8-1] - L[i_7+i_6][i_9+i_8][i_8+1][i_6+1][1] - G[i_8][i_9+1][i_7][i_6] - H[i_8+i_7-1][i_6][i_7+1][i_9-1] + 5;
                }
            }
        }
    }
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,wl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4432251113_06(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,wl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
