...
void kernel_4322152223_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = i_0; i_2 < PB_L; i_2++) {
                A[i_0-1][i_1][i_2] = D[i_0][i_1][i_2] + D[i_2][i_1][i_2-2] * A[i_0-1][i_2][i_1+1] + A[i_0-2][i_1][i_2] + 4;
            }
        }
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = 1; i_4 < PB_P-2; i_4++) {
                B[i_0][i_3][i_0] = D[i_4-1][i_4+2][i_3] * D[i_3][i_4][i_0] - B[i_0][i_3-1][i_0] + B[i_4+1][i_0][i_4+1] + 2;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
        for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 2; i_7 < min(-i_5+PB_P, -i_6+PB_P); i_7++) {
                C[i_6][i_5+2][i_7+1][i_7] = D[i_7][i_5][i_6-2] + E[i_6][i_6] * E[i_5][i_6+1] - D[i_6][i_7+i_5][i_6] + D[i_6+1][i_7+i_6][i_5] + D[i_7+2][i_7-2][2] + E[i_7+1][i_6] - E[i_7][i_5-2] * E[i_6-1][i_5+2] + C[i_7+2][i_5+2][i_6+1][i_6] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4322152223_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
