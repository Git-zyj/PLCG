...
void kernel_2426331121_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0+1] + F[i_0-1][0] + E[i_0+1] * 4;
        for (int i_1 = PB_M - 1; i_1 >= 0; i_1--) {
            A[i_0-1] = F[i_1+1][i_0-1] + G[i_1][i_0] + G[i_1][i_0-1] * E[i_1+1] * H[i_1][i_0] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_N, PB_M-1); i_3++) {
            C[i_3][i_3] = G[i_3+i_2][i_3+1] * H[i_3][i_2] * D[i_3-1] + 1;
        }
        D[i_2-1] = I[i_2] * 1;
    }
#pragma endscop
...
kernel_2426331121_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
