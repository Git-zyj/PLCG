...
void kernel_3222111115_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            A[i_0-1][i_1-1] = A[i_1-1][i_0] * 5;
        }
        for (int i_2 = min(-i_0+PB_M, PB_L) - 1; i_2 >= 1; i_2--) {
            A[i_0-2][i_2-1] = C[i_2+i_0][i_2] - 3;
        }
    }
#pragma endscop
...
kernel_3222111115_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
