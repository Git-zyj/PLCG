...
void kernel_3222232313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = C[i_0-2][i_0] + A[i_0+1] + A[i_0-1] * 5;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        for (int i_2 = min(i_1, -i_1+PB_M+2) - 1; i_2 >= 2; i_2--) {
            B[i_2][i_1] = C[i_2][i_2-2] * D[i_2+i_1-2][i_2] - C[i_2][i_1-2] + C[i_2+1][i_1] + B[i_2][i_1+1] + B[i_1-1][i_2] - B[i_2+1][i_1] * 6;
        }
    }
#pragma endscop
...
kernel_3222232313_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
