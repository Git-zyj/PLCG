...
void kernel_3322252115_00(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_N-1, -i_0+PB_M+2); i_1++) {
            C[i_1-1] = D[i_1+2][i_0] + E[i_1][i_0][i_0] + D[i_1][i_0] - F[i_1+i_0-2][i_1-2] - 1;
            C[i_1+1] = F[i_1-2][i_0-2] + F[i_1-1][i_0] + F[i_1][i_0+1] + G[i_1+1][i_0][i_1+2] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        C[i_2] = H[i_2-1] - I[i_2] * H[i_2+1] + H[i_2] + I[i_2] * C[i_2+1] + 2;
    }
#pragma endscop
...
kernel_3322252115_00(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
