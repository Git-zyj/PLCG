...
void kernel_3214131315_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            A[i_0][i_0] = C[i_1-1] + A[i_0-1][i_0-1] * A[i_0+1][i_0+1] - A[i_0-1][i_0-1] - A[i_0-1][i_0-1] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_N, -i_2+PB_M); i_3++) {
            B[i_3+i_2] = C[i_3] - D[i_3][i_2] + D[i_3][i_2] * D[i_3+i_2][i_3-1] * D[i_3+i_2][1] * A[i_3][i_3] - 3;
        }
    }
#pragma endscop
...
kernel_3214131315_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
