...
void kernel_4216132223_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < PB_P+2; i_2++) {
                B[i_0-1][i_2][i_2-2] = C[i_0][i_2][i_1] * C[i_1+1][i_2][i_0] + 5;
            }
        }
        for (int i_3 = max(2, i_0); i_3 < -i_0+PB_M; i_3++) {
            for (int i_4 = i_3; i_4 < PB_M; i_4++) {
                B[i_0][i_4][i_4-2] = C[i_3-2][i_4-1][i_0] + C[i_4][i_3][i_0-1] + C[i_3+i_0][i_4][i_4] * C[i_4][i_0][i_3+1] + B[i_4-1][i_3][i_3-2] - B[i_0+2][i_4][i_4-2] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4216132223_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
