...
void kernel_4316332125_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-4; i_0++) {
        A[i_0+2] = D[i_0-2] * D[i_0] + A[i_0+1] - 6;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
                for (int i_3 = 0; i_3 < PB_Q-1; i_3++) {
                    C[i_0+4][i_2+1][i_3+1][i_1] = E[i_2-1][i_0-1][i_3+1][i_1] * 2;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < min(PB_L, -i_4+PB_Q); i_5++) {
            for (int i_6 = 0; i_6 < min(min(PB_L, -i_4+PB_P), -i_5+PB_Q+2); i_6++) {
                for (int i_7 = 2; i_7 < -i_5+PB_L; i_7++) {
                    C[i_4+2][i_6][i_7][i_5] = E[i_7+i_5][i_6+i_4][i_7][0] - F[i_7][i_4][i_4+1][i_7] - F[i_5+1][i_6][i_7][i_5+i_4] + F[i_7][i_4][i_5][i_6] - E[i_7-2][i_4][i_6+i_5-2][i_7+i_5] - G[i_5][i_6][i_7] + 4;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4316332125_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
