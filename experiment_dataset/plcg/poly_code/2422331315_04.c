...
void kernel_2422331315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = E[i_0] - F[i_0] * E[i_0] + A[i_0] - 2;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_L, -i_1+PB_M-1); i_2++) {
            A[i_2] = G[i_1+1][i_2] * H[i_2][i_1+1] - H[i_2][i_2-1] * A[i_2+2] * 6;
            A[i_2+1] = I[i_2][i_2] - G[i_2+i_1+1][0] - A[i_2+1] + 2;
        }
        A[i_1+1] = F[i_1-1] * I[i_1][i_1] * E[i_1+1] - J[i_1+1] + 6;
    }
#pragma endscop
...
kernel_2422331315_04(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
