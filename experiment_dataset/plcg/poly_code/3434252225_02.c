...
void kernel_3434252225_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-1] - E[i_0] + F[i_0] - F[i_0] + E[i_0] * 1;
        A[i_0+1] = E[i_0] * E[i_0] - F[i_0] + G[i_0][0] * F[i_0] - A[i_0] - 2;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
            for (int i_3 = 2; i_3 < min(PB_Q-2, -i_2+PB_L); i_3++) {
                C[i_2+2][i_3][i_1] = H[i_1][i_3+i_2][i_1-1] * I[i_1][i_3][i_2] * H[i_3][i_1+1][2] * I[i_3][i_1][i_2] * I[i_2][i_1][i_3+2] * G[i_1][i_2] - C[i_2+2][i_3][i_1+2] + C[i_2][i_3-2][i_1] + 2;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        A[i_4] = F[i_4+1] + F[i_4-2] + 4;
    }
#pragma endscop
...
kernel_3434252225_02(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
