...
void kernel_3336332215_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] - E[i_0+1] + D[i_0] * F[i_0+2] * A[i_0-1] + 2;
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            B[i_0][i_1] = G[i_1][i_1+i_0+1] - G[i_0][i_1-1] * H[i_1][i_0+2] * G[i_0+1][i_1] * G[i_1+i_0][i_1+i_0+2] - B[i_0-1][i_1+1] - B[i_0+1][i_1+1] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                B[i_4][i_3] = B[i_3][i_4+1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3336332215_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
