...
void kernel_3226111225_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                A[i_2-1][i_1+1][i_0-1] = 5;
                A[i_2-1][i_1+1][i_0-1] = C[i_2][i_1-1][i_0] * A[i_2][i_1+1][i_0+1] * A[i_2][i_1+1][i_0-1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3226111225_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
