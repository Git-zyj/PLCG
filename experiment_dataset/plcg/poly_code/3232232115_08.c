...
void kernel_3232232115_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < PB_M-2; i_2++) {
                A[i_2-2][i_0][i_1+1] = C[i_1][i_2][i_0] * D[i_0+2][i_2][i_1] + C[i_2][i_2+2][i_1] - 2;
            }
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 2; i_4 < PB_M; i_4++) {
                B[i_3][i_0-1][i_4+2] = D[i_4][i_3+1][i_0] + D[i_4][i_4-2][i_0] * D[i_0+1][i_4][i_3] - B[i_3-1][i_4-1][i_0+2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3232232115_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
