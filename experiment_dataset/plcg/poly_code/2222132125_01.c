...
void kernel_2222132125_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_P; i_1++) {
            B[i_1+i_0+2][i_1+1] = C[0] - D[i_1-1][i_0-1] * D[i_1-1][i_1+i_0] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 2; i_3 < min(-i_2+PB_M+1, PB_P); i_3++) {
            B[i_3+i_2-1][i_3] = D[i_3-2][i_2+1] - D[i_3][i_2] - D[i_3][0] - B[i_3+i_2-2][i_3-1] - 3;
        }
    }
#pragma endscop
...
kernel_2222132125_01(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
