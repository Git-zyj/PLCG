...
void kernel_4436112325_05(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        C[i_0] = E[i_0] * E[i_0] * C[i_0] + C[i_0-1] - 3;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = 1; i_2 < PB_N-2; i_2++) {
            B[i_2][i_1] = B[i_2+1][i_1] * B[i_2-1][i_1] + B[i_2+2][i_1-1] + 3;
        }
        C[i_1] = E[i_1] * 4;
...
kernel_4436112325_05(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E));
