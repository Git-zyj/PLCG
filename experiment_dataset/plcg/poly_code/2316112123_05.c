...
void kernel_2316112123_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        A[i_0-1] = 6;
        for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
            A[i_1-1] = D[i_1-2][i_0-1] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            C[i_3] = D[i_2][i_3] - C[i_3-2] - 1;
        }
    }
#pragma endscop
...
kernel_2316112123_05(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
