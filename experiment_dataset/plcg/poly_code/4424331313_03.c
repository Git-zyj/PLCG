...
void kernel_4424331313_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L-1, i_1); i_2++) {
                A[i_0][i_1][i_2+1] = E[i_1][i_2+1][i_0+1] * E[i_1][i_2][i_0] * F[i_2][i_1] + A[i_0][i_1-1][i_2+1] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_L-1, i_3); i_4++) {
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                A[i_5][i_3+1][i_4+1] = G[i_5][i_3][i_4-1] - A[i_5][i_3][i_4+1] * 6;
            }
            for (int i_6 = 0; i_6 < min(PB_L-2, -i_4+PB_L); i_6++) {
                A[i_3][i_4-1][i_6+2] = E[i_3][i_6][i_6+i_4] * E[i_4][i_6][i_3] + G[i_6+i_4][i_3][i_4-1] + E[i_6][i_3-1][i_4+1] * G[i_4][i_3][i_6] - G[i_3][i_6+1][i_4] - 4;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
        D[i_7] = F[i_7-1][1] * H[0] - D[i_7+1] + D[i_7-1] - D[i_7+1] - 2;
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4424331313_03(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
