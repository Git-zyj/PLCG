...
void kernel_4212212321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
                A[i_1][i_0+1][i_2+1] = A[i_1-1][i_0+1][i_2+1] - A[i_1-2][i_0+3][i_2+1] + A[i_1+2][i_0+3][i_2+2] - 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            B[i_4] = C[i_4][i_3] * B[i_3-1] - B[i_4-1] - B[i_4+1] + 3;
        }
    }
#pragma endscop
...
kernel_4212212321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
