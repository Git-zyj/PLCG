...
void kernel_4336352215_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,int wd,DATA_TYPE POLYBENCH_5D(D,xD,yD,zD,vD,wD,xd,yd,zd,vd,wd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < -i_0+PB_Q-1; i_2++) {
                for (int i_3 = i_2; i_3 < min(PB_Q-2, -i_1+PB_P); i_3++) {
                    A[i_2-2][i_3][i_1+1][i_0] = D[i_2][i_1-1][i_0-1][i_3][0] * E[i_1][i_1+1][i_2+i_0+1][i_3+2] - F[i_3-1][i_0-1][i_1-2][i_2+2] * E[i_3][i_0+2][i_1-2][i_2+1] * G[i_1][i_3][i_2] + H[i_2-1][i_3][i_3+i_1][2] - A[i_2-2][i_0-1][i_1][i_3] + 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        B[i_4] = I[i_4] + J[2] * J[i_4+2] + K[i_4] * B[i_4+1] + B[i_4-1] - 5;
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        for (int i_6 = 1; i_6 < -i_5+PB_M; i_6++) {
            C[i_5-1][i_6-1] = L[i_6-1][i_5] * L[i_6+i_5][0] * M[i_6][1] + N[i_5][i_6] - C[i_5][i_6-1] * C[i_5-1][i_6] + 3;
        }
    }
#pragma endscop
