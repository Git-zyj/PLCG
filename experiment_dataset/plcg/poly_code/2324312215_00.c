...
void kernel_2324312215_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        B[i_2+1] = B[i_2+2] - C[i_2] + 2;
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            C[i_3+1] = D[i_3-1][0] * D[i_2][i_2] - B[i_2+2] + B[i_2+1] + 4;
        }
    }
#pragma endscop
...
kernel_2324312215_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
