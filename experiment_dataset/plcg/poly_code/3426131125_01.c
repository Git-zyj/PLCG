...
void kernel_3426131125_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] * A[i_0-2] - 2;
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            B[i_0][i_1] = F[i_1][i_0] + F[i_1][i_0+1] * F[i_0][i_1+1] - F[i_0][i_1] * F[i_0+1][i_1] - 4;
        }
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            for (int i_3 = i_0; i_3 < PB_L-2; i_3++) {
                B[i_2][i_3+2] = G[i_2][i_3][i_0] - 1;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_L); i_5++) {
            for (int i_6 = 2; i_6 < PB_L+1; i_6++) {
                D[i_6-1][i_4+1][i_5+i_4] = G[i_5+1][i_4][i_6-1] - G[i_5][i_6-1][i_4] - D[i_6-2][i_4+1][i_5+i_4] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3426131125_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
