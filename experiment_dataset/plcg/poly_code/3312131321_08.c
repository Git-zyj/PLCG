...
void kernel_3312131321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-3, PB_L-1); i_1++) {
            A[i_1][i_1] = D[i_0-1][i_1] * D[i_1+1][i_0-1] * D[i_0][i_1] - B[i_1+i_0+3][i_1] - B[i_1+i_0-1][i_0] + 5;
            B[i_1+i_0+1][i_0] = D[i_1-1][i_0] + E[i_0] * D[i_1-1][i_0] * C[i_0+1][i_1+2] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_P-3, -i_2+PB_M-1); i_3++) {
            C[i_2+1][i_3+1] = D[i_2][i_3] - E[i_2] - B[i_3+i_2+1][i_2] * C[i_2+1][i_3+2] * C[i_2-1][i_3] - C[i_2+1][i_3+3] * C[i_2][i_3-1] - C[i_2+1][i_3-1] - 1;
        }
    }
#pragma endscop
...
kernel_3312131321_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
