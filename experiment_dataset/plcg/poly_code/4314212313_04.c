...
void kernel_4314212313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        ...
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            C[i_0][i_1] = D[i_1] * A[i_1] + C[i_0][i_1+1] + C[i_0][i_1-1] * C[i_1][i_0+1] * 1;
        }
    }
#pragma endscop
...
kernel_4314212313_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
