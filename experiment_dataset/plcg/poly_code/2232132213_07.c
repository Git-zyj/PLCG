...
void kernel_2232132213_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            A[i_1] = C[i_1-1][i_1-1] * D[i_0-1] + D[i_1] - A[i_1-1] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
        B[i_2] = D[i_2-2] + D[i_2+2] * B[i_2-1] + B[i_2+1] - 3;
    }
#pragma endscop
...
kernel_2232132213_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
