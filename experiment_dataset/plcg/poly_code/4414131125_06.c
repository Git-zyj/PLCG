...
void kernel_4414131125_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = 2; i_1 < min(PB_N-1, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 1; i_2 < PB_Q+1; i_2++) {
                for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
                    A[i_1+i_0-1][i_0][i_2-1][i_3+1] = E[i_3][i_1][i_0-1][i_3-1] - 4;
                }
            }
            B[i_1][i_0] = F[i_1+i_0-1][i_0] - G[i_0-1][i_1+1][0] - F[i_1][i_0] * B[i_1-2][i_0+2] + 6;
        }
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        for (int i_5 = 1; i_5 < PB_N; i_5++) {
            for (int i_6 = i_5; i_6 < PB_M-1; i_6++) {
                B[i_6][i_5-1] = F[i_6][i_5] + G[i_5][i_4][0] - G[i_6][i_4-1][i_5] - B[i_6+1][i_5] - 5;
            }
        }
        for (int i_7 = 0; i_7 < min(PB_Q, -i_4+PB_M+1); i_7++) {
            for (int i_8 = i_4; i_8 < PB_M-1; i_8++) {
                for (int i_9 = 0; i_9 < PB_Q-3; i_9++) {
                    A[i_7+i_4-1][i_7][i_8][i_9+3] = E[i_9][i_8+1][i_4-1][i_7] * E[i_4][i_7][0][i_8-1] + 3;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4414131125_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
