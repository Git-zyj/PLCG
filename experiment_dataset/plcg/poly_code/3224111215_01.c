...
void kernel_3224111215_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            B[i_0+1][i_1+2][i_1+1] = B[i_1+1][i_0+2][i_0+1] - 3;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_P, PB_M-1); i_2++) {
            for (int i_3 = 0; i_3 < PB_N; i_3++) {
                B[i_2+1][i_0+1][i_0] = C[i_3][i_2][i_2+i_0] - B[i_2+1][i_0+2][i_0+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3224111215_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
