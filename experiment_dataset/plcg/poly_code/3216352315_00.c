...
void kernel_3216352315_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L+1; i_1++) {
            A[i_1][i_0] = C[i_1-2][i_0] - D[i_1-2][i_0] * D[i_1][i_0] + E[i_1+i_0][i_1+i_0-1][0] + D[i_0-2][i_1+1] - A[i_0-1][i_1+1] - A[i_1][i_0-1] - A[i_1-1][i_0] + A[i_1+1][i_0-1] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_L; i_3++) {
            for (int i_4 = 2; i_4 < -i_3+PB_P-2; i_4++) {
                B[i_2][i_4+i_3][i_3+2] = F[i_3-1][i_4-2][i_2+2] - F[i_4][i_3][i_2-2] * F[i_4+i_3][i_2][i_2] - D[i_3+i_2][i_3] + D[i_2][i_3] - B[i_2][i_4+i_3][i_3+1] + B[i_2][i_4+i_3+2][i_4+3] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3216352315_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
