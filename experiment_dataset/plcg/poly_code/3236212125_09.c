...
void kernel_3236212125_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P; i_2++) {
                A[i_1][i_2+i_1] = C[i_2+i_1+1][i_0] - A[i_1][i_2+i_1-2] * 5;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_P; i_3++) {
            A[i_3][i_3+i_0] = C[i_3+1][i_0] - 5;
        }
    }
#pragma endscop
...
kernel_3236212125_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
