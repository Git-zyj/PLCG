...
void kernel_4216152223_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P+2; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
                A[i_2+2][i_2][i_0] = C[i_2][i_1-2][i_0+2] - C[i_1][i_2][i_0] + C[i_1][i_0+1][i_1] + D[i_2+2][i_1] + C[i_1][i_2-2][i_1+1] * A[i_2+2][i_2][i_0+1] * A[i_2+1][i_2-1][i_0] * 1;
            }
        }
        for (int i_3 = i_0; i_3 < min(-i_0+PB_Q, PB_L); i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
                B[i_3][i_4+i_3][i_0+1] = C[i_3][i_0+1][i_4+i_3] - C[i_3][i_4][i_0] + C[i_4][i_0][i_3+1] - C[i_4][i_0-2][i_3+i_0] + B[i_3][i_4+i_3][i_0-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4216152223_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
