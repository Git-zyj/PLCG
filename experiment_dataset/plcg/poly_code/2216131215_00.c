...
void kernel_2216131215_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(-i_0+PB_M, PB_L+1); i_1++) {
            B[i_0][i_1-1] = C[i_0] + D[i_1+i_0][i_1-1] * D[i_1][i_0] + 2;
        }
        for (int i_2 = 2; i_2 < min(PB_L, i_0); i_2++) {
            B[i_0][i_2-1] = C[i_2] - D[i_2][i_2] + B[i_0][i_2-2] - B[i_0][i_2] * 5;
        }
    }
#pragma endscop
...
kernel_2216131215_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
