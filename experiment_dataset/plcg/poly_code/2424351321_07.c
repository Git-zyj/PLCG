...
void kernel_2424351321_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0+1] = E[i_0] + F[i_0] * F[i_0] + G[i_0-1][0] * F[i_0+1] * E[i_0] - D[i_0] * 6;
        for (int i_1 = 1; i_1 < -i_0+PB_P-1; i_1++) {
            B[i_1][i_0-1][i_1+i_0+1] = G[i_0][i_1-1] * H[i_0][i_1] * I[i_1][i_0-1] * I[i_0][i_1] * H[i_1][i_0-1] - A[i_1+2] + B[i_1][i_0-2][i_1+i_0] - D[i_0+2] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_P-2, i_2); i_3++) {
            C[i_2][i_3+1] = H[i_3][i_2] - I[i_2+1][i_3] - B[i_2][i_3-1][i_3+i_2+1] - B[i_2+1][i_3-1][i_3+i_2+2] * B[i_3+1][i_2-1][i_3+i_2+2] - C[i_2-1][i_3+1] * C[i_2+1][i_3+1] - 4;
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        D[i_4] = J[i_4+1] + F[i_4+1] + E[i_4] + J[i_4] - E[i_4] + A[i_4+1] * D[i_4-2] + D[i_4+1] + 4;
    }
#pragma endscop
...
kernel_2424351321_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
