...
void kernel_2432232225_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            A[i_1] = E[i_0] - F[i_0][i_1+1] * E[i_1] - A[i_1+2] + A[i_1-1] - 5;
        }
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            B[i_0-1][i_2+1] = G[i_2][i_0] - F[i_2][i_0-2] * F[i_2][i_0+1] - B[i_0-1][i_2] + 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        A[i_3] = H[i_3] + E[i_3] - H[i_3] + 1;
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            A[i_4] = I[i_3][i_4][i_4] * F[i_3][i_3] + 5;
        }
    }
#pragma endscop
...
kernel_2432232225_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
