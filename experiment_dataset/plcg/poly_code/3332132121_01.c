...
void kernel_3332132121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1-1][i_0-2] = D[i_1][i_0] + E[i_0+1] + D[i_0][i_1] + D[i_0][i_1] - 5;
        }
    }
    for (int i_2 = 3; i_2 < PB_L; i_2++) {
        for (int i_3 = 3; i_3 < PB_L-1; i_3++) {
            B[i_3+2][i_2-2] = D[i_3][i_2] * D[i_2][i_3+1] - A[i_3-3][i_2-3] + 2;
        }
        C[i_2-1] = E[i_2-2] + E[i_2-2] + E[i_2] - C[i_2+1] + C[i_2] * 5;
    }
#pragma endscop
...
kernel_3332132121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
