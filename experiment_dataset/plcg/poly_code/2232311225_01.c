...
void kernel_2232311225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            A[i_0][i_1] = A[i_0-1][i_1-2] - A[i_1+2][i_0-2] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_P; i_3++) {
            A[i_3-2][i_2] = C[i_2][i_3] * 1;
        }
    }
#pragma endscop
...
kernel_2232311225_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
