...
void kernel_2414232223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1][i_0] = E[i_1][i_0] * F[i_1] + G[i_0][i_1-1] * H[i_1] * A[i_1][i_0+2] + D[0] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            B[i_3][i_3] = G[i_3-1][i_2] * G[i_3+2][i_3] - F[i_2] * G[i_3][i_2] - G[i_3-1][i_2-2] - B[i_3+1][i_3+1] + 6;
        }
        for (int i_4 = i_2; i_4 < min(-i_2+PB_L+1, PB_M-1); i_4++) {
            D[0] = E[i_4+1][i_4+i_2-1] + D[0] * D[0] + 5;
        }
        D[0] = F[i_2] - D[0] * 5;
    }
#pragma endscop
...
kernel_2414232223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
