...
void kernel_4316312221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-4; i_0++) {
        A[i_0+2][i_0] = D[i_0] + A[i_0][i_0-2] + A[i_0+4][i_0+2] * 4;
    }
    for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_M-3; i_2++) {
            for (int i_3 = 0; i_3 < i_1; i_3++) {
                B[i_2+i_1][i_2][i_1] = B[i_2+i_1+3][i_2+1][i_1+2] * B[i_2+i_1+2][i_2+2][i_1] * 6;
            }
        }
        for (int i_4 = 0; i_4 < i_1; i_4++) {
            for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                C[i_4+1][i_5+1] = E[i_4+2][i_5-1][i_1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4316312221_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
