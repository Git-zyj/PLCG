...
void kernel_3212311323_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-3; i_0++) {
        A[i_0+1] = A[i_0-1] * A[i_0] - A[i_0+3] + A[i_0+2] * 6;
    }
    for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_M-1; i_2++) {
            for (int i_3 = i_2; i_3 < min(min(-i_2+PB_N-3, PB_M-1), -i_1+PB_N-2); i_3++) {
                B[i_3+i_2+1][i_2-1] = C[i_3+1][i_2+1] * B[i_3+i_1+2][i_3] - B[i_3+i_2+3][i_2+1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3212311323_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
