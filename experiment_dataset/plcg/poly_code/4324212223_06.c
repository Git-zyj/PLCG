...
void kernel_4324212223_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M-3, i_0); i_1++) {
            A[i_1+1][i_1] = A[i_1-1][i_1-2] - A[i_1+3][i_1+2] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                B[i_2][i_4][i_3] = D[i_2][i_3+1][i_4] * B[i_4][i_2][i_3+2] - 5;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-4; i_5++) {
        for (int i_6 = 4; i_6 < PB_P+2; i_6++) {
            for (int i_7 = 0; i_7 < i_5; i_7++) {
                C[i_5+2][i_7][i_6-2] = C[i_5+4][i_7+1][i_6-2] - C[i_5+2][i_7][i_6-4] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4324212223_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
