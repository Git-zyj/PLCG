...
void kernel_2214211325_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-3; i_0++) {
        A[i_0+1][i_0+1] = A[i_0+2][i_0+2] - A[i_0-1][i_0-1] * A[i_0+3][i_0+3] - 5;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            A[i_1+2][i_1+2] = C[i_1][i_2] - D[i_2] * 2;
        }
    }
#pragma endscop
...
kernel_2214211325_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
