...
void kernel_3216312321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            ...
            for (int i_2 = 0; i_2 < min(-i_1+PB_L, i_1); i_2++) {
                B[i_2+2][i_1+1][i_0+1] = C[i_0-1][i_2+i_1][i_2+2] + A[i_0+2] * A[i_1+2] - B[i_1][i_2+1][i_0+1] + B[i_2+3][i_1-1][i_0] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3216312321_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
