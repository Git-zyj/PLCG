...
void kernel_3424111311_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                A[i_1][i_2][i_0] = A[i_1-1][i_2-1][i_0+1] - 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        ...
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            C[i_3][i_4] = E[i_3+1] * B[i_3] + B[i_3+1] * B[i_4-1] * C[i_4+1][i_3] + 5;
        }
    }
    for (int i_5 = 2; i_5 < PB_M; i_5++) {
        D[i_5-1] = E[i_5] + E[i_5-1] + D[i_5] + D[i_5-2] + D[i_5-2] + 2;
    }
#pragma endscop
...
kernel_3424111311_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
