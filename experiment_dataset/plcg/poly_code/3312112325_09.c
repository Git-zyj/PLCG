...
void kernel_3312112325_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-3; i_1++) {
            B[i_1+1] = B[i_0] * B[i_1+3] * B[i_1+1] + 5;
        }
        B[i_0+1] = 1;
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        B[i_2+1] = D[i_2] * D[i_2] - 3;
    }
#pragma endscop
...
kernel_3312112325_09(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
