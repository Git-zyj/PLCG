...
void kernel_4424232325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] + F[i_0] - G[i_0][i_0] * F[i_0] + A[i_0-1] + A[i_0-2] + 1;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < min(PB_P+2, -i_1+PB_M-3); i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_M-4, -i_2+PB_M); i_3++) {
                B[i_3+i_1+2][i_2+2][i_3] = H[i_3+i_2][i_1][i_3] * H[i_2][i_2-2][i_1] - B[i_3+i_1+2][i_2+4][i_1] * B[i_3+i_1+4][i_2+2][i_3+2] * B[i_2+i_1+3][i_3+2][i_1] * 1;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_P; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_M-1; i_5++) {
            C[i_4][i_5][i_4] = G[i_5-2][i_4] * I[i_4][i_5] * I[i_4][i_5] - G[i_4][i_5-1] + A[i_5] + C[i_4+2][i_5][i_4+2] * C[i_4+2][i_5+1][i_4+2] + C[i_4][i_5-1][i_4] - 6;
        }
        for (int i_6 = 0; i_6 < min(PB_P, -i_4+PB_M-2); i_6++) {
            for (int i_7 = 0; i_7 < PB_P; i_7++) {
                B[i_6+i_4+2][i_7+2][i_6] = J[i_6][i_4][i_7] - J[i_7][i_4][0] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4424232325_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
