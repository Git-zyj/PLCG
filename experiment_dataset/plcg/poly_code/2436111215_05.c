...
void kernel_2436111215_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M; i_2++) {
            C[i_1][i_1] = C[i_1+1][i_1+1] + C[i_1+2][i_1+2] * 2;
        }
        C[i_1+1][i_1+1] = E[i_1] + C[i_1+2][i_1+2] + 1;
        for (int i_3 = 1; i_3 < min(-i_1+PB_N, i_1); i_3++) {
            D[i_3+i_1][i_3] = C[i_1][i_1] + D[i_3+i_1-1][i_3-1] - D[i_3+i_1][i_3-1] * 1;
        }
    }
#pragma endscop
...
kernel_2436111215_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
