...
void kernel_3224211123_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_0][i_1][i_0] = C[i_1-1][i_2][i_1+i_0] + A[i_0+2][i_1][i_0+2] * 2;
            }
        }
        for (int i_3 = 1; i_3 < -i_0+PB_M; i_3++) {
            A[i_0-2][i_3][i_0-2] = C[i_3+i_0][i_3-1][1] - 6;
        }
    }
#pragma endscop
...
kernel_3224211123_09(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
