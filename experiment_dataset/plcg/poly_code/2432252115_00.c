...
void kernel_2432252115_00(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        C[i_0-1] = E[i_0+1] - F[i_0] - E[i_0] + E[i_0] * F[i_0+1] * E[i_0] * E[i_0-2] * F[i_0-1] * E[i_0] - B[i_0][i_0] + C[i_0-1] - 3;
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            B[i_0][i_0] = G[i_1][i_0] + H[i_0-1][i_0] * G[i_1][i_0] - I[i_1+i_0][i_1][i_0-2] + E[i_1] + 5;
        }
        C[i_0] = E[i_0] + F[i_0] * E[i_0] * E[i_0+1] - 1;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_M+1, -i_2+PB_L); i_3++) {
            C[i_3-1] = G[i_3+i_2][0] + H[i_3-1][i_2-2] * 3;
        }
    }
#pragma endscop
...
kernel_2432252115_00(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
