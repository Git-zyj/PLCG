...
void kernel_4216211325_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_L-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_0+1][i_1][i_0] = C[i_1-1][i_1+i_0+1][i_1] + 5;
            }
            A[i_0-1][i_1][i_0-2] = D[i_1+i_0][i_0] * A[i_1+1][i_0][i_1] + A[i_0+1][i_1-1][i_0] - A[i_0+1][i_1][i_0] + 3;
        }
    }
#pragma endscop
...
kernel_4216211325_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
