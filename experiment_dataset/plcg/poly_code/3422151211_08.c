...
void kernel_3422151211_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-1] - E[i_0-1] + E[i_0+1] - E[i_0-1] + E[i_0-1] + A[i_0-1] + B[i_0-2] - 6;
        B[i_0-1] = F[i_0][0] - E[i_0] + E[i_0] + 2;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_P-1; i_2++) {
            for (int i_3 = 2; i_3 < PB_P+1; i_3++) {
                C[i_1+1][i_2][i_3-1] = G[i_2+1][i_1][i_3-1] - G[i_1][i_2-1][i_3-1] + G[i_1][i_3][i_2+1] * C[i_1+1][i_2-1][i_3-1] + C[i_1+1][i_2+1][i_3-1] * C[i_1+1][i_2+1][i_3-2] - 6;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_M; i_5++) {
            D[i_4] = E[i_5-1] + F[i_5][i_4-1] * F[i_5+i_4][0] + F[i_5][i_4+1] - F[i_4][i_5] * F[i_5][1] * E[i_4+1] + D[i_5-1] - D[i_5+1] - D[i_4+1] + 1;
        }
    }
#pragma endscop
...
kernel_3422151211_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
