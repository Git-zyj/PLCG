...
void kernel_4436351123_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn)){
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_Q; i_2++) {
                for (int i_3 = 1; i_3 < min(PB_P-1, -i_1+PB_P); i_3++) {
                    A[i_2][i_1][i_3+1][i_2] = E[i_2][i_0][i_3][i_0+1] * F[i_3-1][i_2-1][i_1] * G[i_0+1][i_2][i_1][i_3-1][i_3] * E[i_2+1][i_0][i_3+i_1][i_1+1] * H[i_2-1][i_1][i_0][i_2+i_0] - 2;
                }
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 1; i_6 < min(PB_Q-1, -i_5+PB_M+1); i_6++) {
                for (int i_7 = 0; i_7 < min(i_4, -i_6+PB_M); i_7++) {
                    C[i_4][i_5-2][i_6] = I[i_6+i_5-1][i_4-1][i_7+1][i_6+1] + E[i_7+i_6][i_4][i_5][i_5] * E[i_6-1][i_4+1][i_7][i_5] + F[i_6][i_5][i_5+1] * H[i_7][i_5-1][i_4][i_6-1] - C[i_7+1][i_5-2][i_4] - 2;
                }
                C[i_4+1][i_5][i_6] = F[i_6][i_5][i_4] * J[i_6][i_5][i_6-1] + F[i_4][i_6][i_5] - 2;
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_L; i_8++) {
        D[i_8] = K[i_8][i_8] * L[i_8] - M[0] - L[i_8+1] - L[i_8] - L[i_8-1] * N[i_8+1] + D[i_8+1] + 2;
    }
#pragma endscop
