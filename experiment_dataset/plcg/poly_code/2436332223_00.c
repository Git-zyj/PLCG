...
void kernel_2436332223_00(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        D[i_0] = E[i_0-1] - E[i_0] - E[0] * F[i_0] * F[i_0] + 3;
    }
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            B[i_2][i_1] = G[0][i_2+i_1-2] * H[i_2+i_1][i_2+2] * B[i_2][i_1+2] - B[i_2][i_1-2] * B[i_2-2][i_1] + B[i_2-1][i_1] - B[i_2-2][i_1+2] * 5;
        }
        C[i_1] = F[i_1] * C[i_1+1] * 5;
    }
    for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
        D[i_3] = H[i_3-2][2] * E[i_3] + I[i_3+2] + 3;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2436332223_00(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
