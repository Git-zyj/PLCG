...
void kernel_3312112215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-2; i_1++) {
            A[i_0][i_1+2] = D[i_1+2][i_0] + A[i_0+1][i_1+2] - 5;
            A[i_0][i_1+2] = 6;
        }
        for (int i_2 = 0; i_2 < min(PB_P-1, -i_0+PB_L-3); i_2++) {
            for (int i_3 = 0; i_3 < min(PB_M, -i_0+PB_L-1); i_3++) {
                C[i_2][i_2+i_0+2][i_3] = D[i_2][i_3] * E[i_2][i_3][i_0] + C[i_2][i_2+i_0+3][i_3] + C[i_0-1][i_3+i_0+1][i_2+1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3312112215_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
