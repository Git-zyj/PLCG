...
void kernel_3426231321_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] - F[i_0-1] - F[i_0+1] * E[i_0] + F[i_0] * 6;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            B[i_1+i_0] = A[i_1-1] + A[i_1+2] * B[i_1+i_0+1] - 6;
        }
    }
    for (int i_2 = 4; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L-1, -i_2+PB_M-2); i_3++) {
            C[i_3+i_2][i_3][0] = G[i_2][i_3] * H[i_3][i_2+1] - G[i_2][0] - H[i_3+i_2][i_2] + B[i_3+i_2-3] - B[i_3+i_2] + C[i_3+i_2-2][i_2-2][0] + C[i_3+i_2+2][i_3][0] + C[i_3+i_2+2][i_2+2][0] + C[i_3+i_2+1][i_3+1][0] - C[i_3+i_2-3][i_3-2][0] + C[i_3+i_2-4][i_3-2][0] - 4;
        }
        D[i_2] = F[i_2] - F[i_2+1] + F[i_2] + D[i_2-2] * 4;
    }
#pragma endscop
...
kernel_3426231321_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
