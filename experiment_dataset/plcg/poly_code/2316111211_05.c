...
void kernel_2316111211_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[0] = A[0] * 1;
        for (int i_1 = 0; i_1 < min(i_0, PB_M-2); i_1++) {
            B[i_0+1] = A[0] * B[i_1+2] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            C[i_2+1][i_3] = D[i_3-1][i_2] * C[i_3+1][i_2+1] * C[i_2][i_3] * C[i_2+1][i_3-1] + 1;
        }
    }
#pragma endscop
...
kernel_2316111211_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
