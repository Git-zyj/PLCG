...
void kernel_3416232325_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0-2] = E[i_0+2] + E[i_0-1] * 5;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        B[i_1] = E[0] * E[i_1] * E[i_1] + B[i_1-2] - B[i_1+1] + B[i_1+2] + 6;
        C[i_1] = E[i_1-1] - F[i_1] + F[i_1-1] + C[i_1+1] - C[i_1-1] - 3;
        for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_M-1); i_2++) {
            B[i_1-2] = E[i_2+i_1+1] + G[i_1-2][i_1] + H[i_2+i_1][i_1+1] + G[i_2+i_1][0] + B[i_1+1] * B[i_2] * 6;
        }
    }
#pragma endscop
...
kernel_3416232325_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
