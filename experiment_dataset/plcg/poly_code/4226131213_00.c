...
void kernel_4226131213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < min(i_1, PB_M-1); i_2++) {
                A[i_2][i_1] = C[i_2][i_1] * C[i_2+1][i_1] - A[i_1-1][i_2-1] * 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                B[i_3][i_5-1][0] = D[i_3+1][i_4+1][i_3] - D[i_4][0][i_5] + C[i_4][i_5] - B[i_3+1][i_5-1][0] + B[i_3][i_5][0] - B[i_3-1][i_5-1][0] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4226131213_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
