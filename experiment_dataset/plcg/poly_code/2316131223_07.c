...
void kernel_2316131223_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        A[i_0] = D[i_0] - D[i_0-1] * D[i_0] * D[i_0] - D[i_0+1] + 3;
        for (int i_1 = min(PB_P, i_0) - 1; i_1 >= 1; i_1--) {
            B[i_1][i_0-1] = E[i_1-1][i_0] - E[i_0+1][i_1] - E[i_0][0] + A[i_0] - B[i_1-1][i_0-1] + B[i_0+1][i_1] * 4;
        }
        A[i_0-1] = D[i_0-1] + 3;
    }
#pragma endscop
...
kernel_2316131223_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
