...
void kernel_4324351111_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = D[i_0] - E[i_0+1] - D[i_0+1] - 3;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            B[i_2] = F[i_1][i_2+1] + G[i_2+1][i_1] + G[i_2][i_1] - H[i_2+1][i_1] - G[i_2][i_1] * F[i_2+1][i_1] * F[i_2][i_1] + B[i_2-1] + 1;
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            A[i_1+1] = F[i_3][i_1] + D[i_3] - D[i_1] - H[i_3][0] * 2;
        }
    }
#pragma endscop
...
kernel_4324351111_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
