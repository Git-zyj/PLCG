...
void kernel_4326312121_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        ...
        for (int i_2 = max(3, i_0); i_2 < -i_0+PB_M; i_2++) {
            for (int i_3 = 2; i_3 < min(PB_P-1, -i_0+PB_P); i_3++) {
                B[i_2+i_0][i_3+i_0][i_3] = D[i_2][i_0][i_3-2] + D[i_3+1][i_0][i_3+1] + 4;
            }
            for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
                C[i_2-2][i_4+1] = C[i_2-3][i_4+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4326312121_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
