...
void kernel_2334312323_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_M-2; i_2++) {
            B[i_2] = D[i_2+1][i_1] + B[i_2-2] - B[i_2+2] * 6;
        }
    }
    for (int i_3 = 3; i_3 < PB_M; i_3++) {
        C[i_3-2] = C[i_3-1] - C[i_3-3] - C[i_3] + 2;
    }
#pragma endscop
...
kernel_2334312323_03(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
