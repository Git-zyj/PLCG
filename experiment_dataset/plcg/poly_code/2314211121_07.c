...
void kernel_2314211121_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-1); i_1++) {
            A[i_0-1][i_1] = A[i_0-1][i_1+1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
        B[i_2-1] = D[i_2+1] * D[i_2-1] - E[i_2] * 4;
        for (int i_3 = max(2, i_2); i_3 < PB_M-2; i_3++) {
            C[i_3][i_2] = C[i_3-2][i_2] - C[i_3+2][i_2-1] * 2;
        }
    }
#pragma endscop
...
kernel_2314211121_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
