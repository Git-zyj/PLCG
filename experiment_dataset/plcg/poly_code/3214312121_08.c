...
void kernel_3214312121_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        ...
        for (int i_2 = 0; i_2 < PB_L-4; i_2++) {
            B[i_2+1][i_2+2] = C[i_2][i_0] + D[i_2][i_0+1] - B[i_2+3][i_2+4] * 2;
        }
    }
#pragma endscop
...
kernel_3214312121_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
