...
void kernel_3234212323_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-4; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_1][i_0+2] = A[i_1][i_0] + A[i_1][i_0+4] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            B[i_3] = C[i_2][i_3] - C[i_3][i_2] * B[i_3-2] - B[i_3-1] - B[i_3+2] * B[i_3+1] + 4;
        }
    }
#pragma endscop
...
kernel_3234212323_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
