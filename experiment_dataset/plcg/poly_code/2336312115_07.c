...
void kernel_2336312115_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L+1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_P-3; i_2++) {
            B[i_1-1][i_2+2] = D[i_2+2][i_1-2] + B[i_1-1][i_2+1] - 1;
            C[i_1-1][i_2] = B[i_1-2][i_2+3] + 2;
        }
    }
#pragma endscop
...
kernel_2336312115_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
