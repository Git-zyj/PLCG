...
void kernel_3236152221_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q-2, PB_P-2); i_1++) {
            for (int i_2 = i_0; i_2 < min(-i_0+PB_M, PB_N); i_2++) {
                A[i_0+2][i_2][i_1+i_0] = C[i_1][i_0-2] * D[i_1][i_2][i_0+1] - D[i_2+2][i_1-2][i_0] * D[i_2][i_0-1][i_1] - D[i_0][i_2-1][i_1+2] - C[i_2][i_2+i_0] + A[i_0+4][i_2][i_1+i_0+2] * 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_3+PB_M, PB_P); i_5++) {
                B[i_3][i_4][i_5-2] = C[i_4][i_5+i_3] + D[i_4][i_3][i_5] * D[i_5][i_4][i_3+2] - D[i_3][i_4][i_5] - B[i_4][i_3+2][i_5-2] - B[i_4][i_5-1][i_3-2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3236152221_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
