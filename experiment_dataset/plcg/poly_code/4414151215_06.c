...
void kernel_4414151215_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 1; i_1 < min(PB_P, -i_0+PB_L); i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_P; i_2++) {
                for (int i_3 = 1; i_3 < -i_0+PB_P; i_3++) {
                    A[i_0][i_3+i_1][i_2][i_1] = E[i_2-1][i_3][i_1][i_0-1] * E[i_1-1][i_3][i_0-1][i_2] + E[i_2][i_0][i_1][0] - E[i_1][i_3-1][i_3+i_0][i_2-1] - E[i_2+1][i_1+i_0][i_1][i_0+1] + E[i_1][i_1-1][i_2+i_0][i_3] * A[i_0-1][i_3+i_1+1][i_2][i_1+1] - A[i_3][i_2+i_1+1][i_0][i_1] + A[i_0][i_3+i_1-1][i_2][i_1] + A[i_0][i_3+i_1][i_2-1][i_1] + 1;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                B[i_4-1][i_5] = F[i_5+1][i_4][i_0] * 1;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_P; i_6++) {
        for (int i_7 = 1; i_7 < -i_6+PB_M; i_7++) {
            for (int i_8 = 1; i_8 < i_7; i_8++) {
                B[i_8-1][i_7-1] = F[i_7-1][i_6][i_6+1] - E[i_7][i_8][i_6][i_7+1] - G[i_7+1][i_8] * F[i_8][i_6][i_7] + F[i_8][i_6+1][i_7+1] - F[i_6-1][i_8+i_7+1][i_8-1] * F[i_7][i_7-1][i_6] - 6;
            }
            for (int i_9 = 0; i_9 < PB_M; i_9++) {
                B[i_7-1][i_9] = F[i_9][i_6-1][0] + F[i_7+i_6][i_9][i_6] + F[i_9][i_6-1][i_7] * G[i_9][i_6] - F[i_7][i_9][i_6] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4414151215_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
