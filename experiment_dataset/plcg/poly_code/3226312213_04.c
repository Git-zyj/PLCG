...
void kernel_3226312213_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < min(PB_M, -i_0+PB_L-2); i_1++) {
            A[i_1] = C[i_1][i_1][i_0] + 3;
            for (int i_2 = 2; i_2 < min(-i_0+PB_L-2, PB_M+2); i_2++) {
                B[i_2-2][i_2][i_1+i_0+2] = A[i_1] * B[i_1-3][i_1-1][i_2+i_0+1] - B[i_2-2][i_2][i_1+i_0] - B[i_1-2][i_1][i_2+i_0+2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3226312213_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
