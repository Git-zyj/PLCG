...
void kernel_3434152115_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0+2] * E[i_0] - E[i_0+2] * E[i_0] + E[i_0] - E[i_0] + 3;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < min(-i_1+PB_L, PB_L-2); i_2++) {
            for (int i_3 = 1; i_3 < min(-i_1+PB_M-2, -i_2+PB_P); i_3++) {
                B[i_2+i_1][i_3+1][i_2-1] = F[i_3-1][i_2][i_1] * G[i_2][i_3][i_2+i_1-1][0] + H[i_3][i_1-1] * F[i_2-2][i_1][i_3] + G[i_3][i_2][i_1][0] + F[i_3][i_1][i_1] * F[i_3+1][i_1][i_3+i_2] + F[i_3+i_1+2][i_2+2][i_2-2] - 3;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < min(PB_P, -i_4+PB_M); i_5++) {
            for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
                C[i_5][i_6][i_4-1] = F[i_5+i_4-2][i_6][i_5] + F[i_4][i_6][i_4] + H[i_5+i_4][i_6-2] - C[i_4][i_6+1][i_5-1] * 6;
            }
        }
    }
...
kernel_3434152115_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
