...
void kernel_4322251315_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_N-1; i_2++) {
                B[i_0+1] = D[i_2][i_1][i_0] - E[i_2+i_1+1][i_0] + D[i_2][i_0][i_1+1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = i_3; i_4 < min(-i_3+PB_M-1, PB_M-2); i_4++) {
            B[i_4+1] = F[i_3][1] - E[i_4+i_3][i_3] + E[i_4][i_3] + G[i_4][i_3][0] - E[i_3][i_4] + F[i_4+i_3+1][0] * E[i_3][i_4] * H[i_3] - I[i_4+i_3] + B[i_4+2] + B[i_4] - B[i_4] + 4;
        }
        for (int i_5 = 0; i_5 < min(-i_3+PB_M-1, PB_M-2); i_5++) {
            B[i_5+2] = F[i_3][i_5] + F[i_5+i_3+1][1] + 5;
        }
    }
#pragma endscop
...
kernel_4322251315_03(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
