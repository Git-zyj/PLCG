...
void kernel_3432211221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0][i_0] = E[i_0-1] + A[i_0-1][i_0-1] * A[i_0+2][i_0+2] * A[i_0-2][i_0-2] - 6;
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L; i_2++) {
                B[i_1][i_0][i_2+i_1] = F[i_0][i_1] * B[i_1][i_0-1][i_2+i_1] - 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            C[i_3+1][i_4] = F[i_4-1][i_3+1] - D[i_3][i_3+1] + 6;
        }
        D[i_3][i_3+1] = D[i_3-2][i_3-1] - 3;
    }
#pragma endscop
...
kernel_3432211221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
