...
void kernel_4436332323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] + A[i_0-2] * A[i_0-1] * A[i_0+1] + 3;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_M-2; i_2++) {
            for (int i_3 = 2; i_3 < min(-i_2+PB_Q-1, -i_1+PB_Q-1); i_3++) {
                for (int i_4 = 2; i_4 < min(i_1, PB_L-2); i_4++) {
                    B[i_4][i_2][i_3+i_1][0] = F[i_2][i_4][i_3-2] - G[i_4+2][i_1][i_1][i_1] - H[i_2+i_1+2][i_4-2][i_3][i_4-1] * I[i_1][i_2-1][i_3][1] - H[i_4+i_3][i_1][i_1][i_2] * B[i_1+2][i_4][i_3+i_2+1][0] - B[i_4+2][i_2][i_3+i_1+1][0] * B[i_4-1][i_2-2][i_3+i_1][0] - 1;
                }
            }
        }
        C[i_1] = C[i_1-1] + C[i_1+1] - 2;
    }
    for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
        D[i_5] = E[i_5] * J[i_5-2] + K[i_5-1] * D[i_5+2] - D[i_5+1] * D[i_5-2] * 1;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4436332323_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
