...
void kernel_4332151315_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    B[i_2][i_3] = D[i_1][i_2][i_3][i_2+i_0] + D[i_1][i_3][i_1][i_2-1] - D[i_1][i_3][i_0][i_1] * 2;
                }
            }
            B[i_1][i_0] = E[i_0][i_1][i_0] + F[i_1+i_0] - G[i_1-1][i_0] + G[i_1+1][i_0] + G[i_1][i_0+1] * B[i_0+1][i_1] * B[i_1][i_0-1] + B[i_0][i_1+1] + 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
                B[i_6][i_5-1] = E[i_6+1][i_5][i_6] - E[i_6+1][i_5][1] * E[i_6-1][i_4][i_5] - E[i_5-1][i_4+1][i_5] - E[i_6+1][i_4][i_5] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4332151315_00(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
