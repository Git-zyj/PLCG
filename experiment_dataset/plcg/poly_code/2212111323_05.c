...
void kernel_2212111323_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][i_1] = A[i_0+2][i_1] + A[i_0+1][i_1] * 5;
        }
        for (int i_2 = 2; i_2 < PB_Q; i_2++) {
            B[i_0-1][i_2][i_2] = C[i_0][i_2-1] + A[i_0][i_2] * B[i_0-3][i_2][i_2] + B[i_0][i_2-2][i_2-2] + B[i_0+1][i_2][i_2] + 5;
        }
    }
#pragma endscop
...
kernel_2212111323_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
