...
void kernel_3312112221_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < min(min(PB_L+2, i_0), -i_1+PB_P-1); i_2++) {
                A[i_0-2][i_1][i_2-2] = C[i_1+1][i_2+i_1+1] + C[i_1][i_2+i_1] * 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        B[i_3][i_3] = D[i_3] * B[i_3+2][i_3+2] - 1;
        for (int i_4 = 0; i_4 < min(-i_3+PB_P, PB_L-2); i_4++) {
            C[i_3][i_4+i_3] = E[i_3+1][i_4] - E[i_4+2][i_3+1] + B[i_4+2][i_4+2] + B[i_3-1][i_3-1] - 2;
        }
    }
#pragma endscop
...
kernel_3312112221_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
