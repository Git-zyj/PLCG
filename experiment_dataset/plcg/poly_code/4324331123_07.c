...
void kernel_4324331123_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0][i_0] = D[i_0] - E[i_0] - E[i_0] + E[i_0-1] - A[i_0-1][i_0-1] + 1;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_N; i_2++) {
            for (int i_3 = 1; i_3 < -i_1+PB_M; i_3++) {
                B[i_2+i_1][i_3-1][i_3] = F[i_2][i_3-1] + G[i_1][i_2][i_3+1] + G[i_3+i_1][i_3][i_2] * 1;
            }
            for (int i_4 = 2; i_4 < PB_P; i_4++) {
                for (int i_5 = 0; i_5 < -i_4+PB_N+2; i_5++) {
                    B[i_5+i_4-2][i_2-1][i_2] = H[i_1][i_4][i_2+1][i_2] + I[i_2][i_1-1][i_4][0] - 1;
                }
            }
        }
    }
...
kernel_4324331123_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
