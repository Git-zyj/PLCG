...
void kernel_4222112323_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
                A[i_2][i_0-1][i_1-1] = C[i_2+1][i_0+1][i_1] * C[i_2][i_0-1][i_1+2] + A[i_0+1][i_2-1][i_1-1] - A[i_2][i_0-3][i_1-1] - 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P-3; i_3++) {
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            B[i_3][i_3+1] = B[i_3-1][i_3] * B[i_3+1][i_3+2] + B[i_3+2][i_3+3] - 1;
        }
    }
#pragma endscop
...
kernel_4222112323_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
