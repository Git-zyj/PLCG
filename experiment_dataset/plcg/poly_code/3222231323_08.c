...
void kernel_3222231323_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_P-1, -i_1+PB_Q); i_2++) {
                A[i_1][i_2][0] = C[i_2][i_2+1][i_0] - C[i_1][i_0][i_2+i_1] * C[i_1+1][i_2-1][i_0] + A[i_1+2][i_2][0] * A[i_1+1][i_0][0] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_M-1, i_3); i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                A[i_3-1][i_5-1][0] = C[i_5][i_3][i_5-1] - C[i_4+1][i_5][i_3] * A[i_5][i_4][0] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3222231323_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
