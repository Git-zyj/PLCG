...
void kernel_3226311311_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M+1, i_0); i_1++) {
            for (int i_2 = i_0; i_2 < PB_Q-1; i_2++) {
                A[i_1][i_2] = C[i_1-1][0] - A[i_1][i_2-1] - A[i_2][i_1+1] + A[i_1-1][i_2+1] - A[i_1+1][i_0] + A[i_1+1][i_2-1] * 3;
            }
        }
        B[i_0] = B[i_0-1] + 5;
    }
...
kernel_3226311311_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
