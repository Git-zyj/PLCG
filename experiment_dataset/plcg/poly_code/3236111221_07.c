...
void kernel_3236111221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
            B[i_1+1][i_1] = C[i_2-1] + B[i_1+2][i_1+1] * 5;
        }
    }
#pragma endscop
...
kernel_3236111221_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
