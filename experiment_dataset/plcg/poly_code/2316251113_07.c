...
void kernel_2316251113_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-1; i_1++) {
            A[i_0][i_1+1] = D[i_1+1][i_0+1] - D[i_1][i_1] * D[i_0][i_1] + E[i_0][i_1][i_0] + D[i_0][i_1] - F[i_1][i_0] - A[i_1+1][i_0+1] - 3;
            A[i_0][i_1+1] = D[i_1][i_0+1] - D[i_0][i_1+1] * G[1] + H[i_0] * F[i_1-1][i_0] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            A[i_3][i_2+1] = F[i_3][i_2] - D[i_2][i_3] + 3;
        }
    }
#pragma endscop
...
kernel_2316251113_07(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
