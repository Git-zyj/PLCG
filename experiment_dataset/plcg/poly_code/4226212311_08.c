...
void kernel_4226212311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
            for (int i_3 = i_2; i_3 < min(PB_N, -i_2+PB_P-1); i_3++) {
                B[i_2+1][i_3+i_2-1] = C[i_3][i_2][i_1] * B[i_2+1][i_3+i_2] + B[i_2+2][i_3+i_2+1] + B[i_2+2][i_3+i_2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4226212311_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
