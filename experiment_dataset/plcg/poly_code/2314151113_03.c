...
void kernel_2314151113_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        A[i_0-1] = D[i_0] + D[1] * D[i_0] + D[i_0-1] - D[i_0-1] * D[1] + D[i_0] + D[i_0-1] + 4;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_L-1, i_1); i_2++) {
            B[i_1][i_1+1] = E[i_2-1][i_1] * D[i_2+1] - E[i_2-1][i_1] + D[i_1] - B[i_2+1][i_2+2] - 6;
            B[i_1][i_1+1] = E[i_2][i_1] * E[i_2][i_1] - D[0] * B[i_1][i_1+1] - 6;
        }
    }
#pragma endscop
...
kernel_2314151113_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
