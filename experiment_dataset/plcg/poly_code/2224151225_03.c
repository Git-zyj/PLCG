...
void kernel_2224151225_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_1][i_0-1] = C[1] + D[i_1-1][i_0] * D[i_1-1][i_0] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 3; i_3 < -i_2+PB_M; i_3++) {
            B[i_2-1][i_3-1] = D[i_3-1][i_2] * C[i_2] - D[i_3][i_2] + D[i_3+1][i_3+i_2] - D[i_3+1][i_2] + D[i_3+i_2][i_3] * B[i_2-1][i_3-3] + B[i_2][i_3+1] - 3;
        }
    }
#pragma endscop
...
kernel_2224151225_03(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
