...
void kernel_3326212311_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_0][i_1] = A[i_0-1][i_1] * A[i_0][i_1-1] - A[i_0+1][i_1-1] + A[i_0+1][i_1] * A[i_0][i_1+1] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 3; i_3 < -i_2+PB_M+2; i_3++) {
            for (int i_4 = 1; i_4 < i_2; i_4++) {
                B[i_3-2][i_4][i_2] = D[i_4+2][i_3+i_2-2][i_4] * B[i_3-3][i_4-1][i_2] + 6;
            }
        }
        for (int i_5 = i_2; i_5 < PB_P-1; i_5++) {
            for (int i_6 = 0; i_6 < min(-i_5+PB_M, PB_L-2); i_6++) {
                C[i_2+2][i_5][i_6+1] = D[i_2][i_6+i_5][2] - C[i_2+2][i_5+1][i_6] - C[i_6+2][i_5][i_2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3326212311_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
