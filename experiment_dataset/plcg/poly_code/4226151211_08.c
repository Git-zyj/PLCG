...
void kernel_4226151211_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(-i_0+PB_N, PB_L-1); i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_0+PB_M, PB_L), -i_1+PB_M+1); i_2++) {
                A[i_0][i_2-1][i_1-1] = C[i_2][i_1][i_0] - D[i_2-1][i_1+1] - C[i_1+i_0][i_2-1][0] + C[i_0][i_2+i_1-1][i_0] + C[i_2][i_0][i_1] * C[i_0][i_2+i_0][i_1] - A[i_0][i_2-1][i_1-2] * 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_L+2, -i_4+PB_L-1); i_5++) {
                B[i_3+1][i_5+1][i_5+i_4][i_4] = C[i_4+i_3][i_5+1][0] + C[i_5][i_3][i_4+i_3] + C[i_5][i_5][i_3] * C[i_4][i_3][i_3] + B[i_4+2][i_5][i_5+i_3-2][i_3-1] + B[i_3+1][i_4+1][i_5+i_4+1][i_5+1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4226151211_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
