...
void kernel_2212131323_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P, -i_0+PB_M-2); i_1++) {
            A[i_1+i_0] = C[i_1+i_0][i_0] - C[i_0][i_0-1] * C[i_1][0] + A[i_1+i_0+2] + A[i_1+i_0+2] + A[i_1+i_0-2] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(min(PB_P, i_2), -i_2+PB_M+1); i_3++) {
            A[i_3+i_2-1] = D[i_2+1] * C[i_2-1][i_3] * 6;
        }
    }
#pragma endscop
...
kernel_2212131323_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
