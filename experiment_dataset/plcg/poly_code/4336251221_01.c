...
void kernel_4336251221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_N-1; i_1++) {
            A[i_0][i_1] = D[i_1][i_0-1] * E[i_1+1] * D[i_1+i_0+1][i_1+1] + D[i_1+i_0][0] + 5;
            for (int i_2 = i_1; i_2 < min(-i_0+PB_M, PB_L); i_2++) {
                B[i_0][i_1][i_0-1] = F[i_1+1][i_2+i_0][i_2] * F[i_1][i_2][i_0] + A[i_2][i_0] * A[i_0][i_2] - B[i_0+1][i_1][i_0] + 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < min(min(-i_3+PB_L, PB_L-2), i_3); i_5++) {
                C[i_4-1][i_5-1] = G[i_3][i_3-1][i_4+1] - H[i_4][i_5+i_3] * G[i_3-1][0][i_4] + F[i_5-1][i_4][i_3] * F[i_5-1][i_3][i_4-1] - G[i_5][i_4][i_3] + D[i_4][i_5] * A[i_4][i_5+2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4336251221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
