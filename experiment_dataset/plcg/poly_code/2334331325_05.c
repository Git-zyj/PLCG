...
void kernel_2334331325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        A[i_0] = D[i_0] + E[i_0] - D[1] + A[i_0+2] + 2;
    }
    for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_L-2; i_2++) {
            A[i_2+2] = F[i_2-1][i_1+1] - F[i_2+1][i_1] * A[i_1] - 3;
        }
    }
    for (int i_3 = 0; i_3 < PB_Q; i_3++) {
        for (int i_4 = 2; i_4 < i_3; i_4++) {
            A[i_4-2] = G[i_4][i_3] + H[i_4][i_3][i_4-1] + G[i_4][i_3] + E[0] + A[i_4+1] * 5;
        }
    }
#pragma endscop
...
kernel_2334331325_05(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
