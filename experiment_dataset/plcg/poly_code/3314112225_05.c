...
void kernel_3314112225_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        A[i_0][i_0-2] = A[i_0+1][i_0-1] * A[i_0-1][i_0-3] * 2;
    }
    for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 2; i_3 < min(PB_L, -i_1+PB_Q); i_3++) {
                B[i_3][i_2][i_3+i_1] = D[i_2][i_3-2][i_2] - D[i_3-2][i_1][i_2-1] * 1;
            }
        }
        for (int i_4 = i_1; i_4 < PB_P; i_4++) {
            for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
                C[i_1-1][i_4][i_5] = D[i_5][i_1][i_4-2] * C[i_4-1][i_1+1][i_5-1] * C[i_1][i_4][i_5] - C[i_1-2][i_5+1][i_4] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3314112225_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
