...
void kernel_2236212325_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_P-1; i_2++) {
            B[i_2][i_2] = C[i_1][i_2+1] - B[i_2-1][i_2-1] - B[i_2+1][i_2+1] * 5;
        }
    }
#pragma endscop
...
kernel_2236212325_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
