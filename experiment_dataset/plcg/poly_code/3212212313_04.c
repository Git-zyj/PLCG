...
void kernel_3212212313_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 3; i_0 < PB_P; i_0++) {
        ...
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            for (int i_3 = 3; i_3 < PB_P; i_3++) {
                B[i_0-2][i_2][0] = B[i_0-2][i_2-1][0] - B[i_3-3][i_2][0] + B[i_0-2][i_2+1][0] - B[i_0-3][i_2][0] * B[i_0-3][i_3][0] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3212212313_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
