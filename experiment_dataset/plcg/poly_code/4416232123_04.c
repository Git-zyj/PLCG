...
void kernel_4416232123_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] - 1;
        for (int i_1 = 2; i_1 < -i_0+PB_M-2; i_1++) {
            B[i_1+i_0][i_1][i_0] = E[i_1] * F[i_1-1] * G[i_1][i_0] + G[i_1][i_0+1] + B[i_1+i_0-1][i_0-1][i_1] * B[i_1+i_0+2][i_1][i_0+2] + 2;
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = i_0; i_3 < min(PB_Q-2, -i_0+PB_P-2); i_3++) {
                    C[i_1-2][i_0][i_2][i_3+1] = H[i_2][i_3+i_0+2][i_3+2][i_1] * I[i_1-1][i_3+1][i_3][0] * 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_Q; i_4++) {
        for (int i_5 = 2; i_5 < min(i_4, -i_4+PB_M+1); i_5++) {
            for (int i_6 = 2; i_6 < min(min(i_4, PB_L-2), -i_4+PB_L); i_6++) {
                for (int i_7 = 0; i_7 < PB_Q; i_7++) {
                    C[i_5-2][i_7][i_6+2][i_4+1] = I[i_7][i_5+i_4-1][i_6-2][i_6+i_4] * H[i_7][i_5-1][i_4-1][0] * 2;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4416232123_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
