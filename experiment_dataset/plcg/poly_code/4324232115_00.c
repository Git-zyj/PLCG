...
void kernel_4324232115_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L; i_2++) {
                A[i_1][i_2][i_0] = D[i_2][i_0][i_1+1] * D[i_2][i_0][i_1] + A[i_2][i_1+1][i_0-1] - 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_M-2, i_3); i_4++) {
            for (int i_5 = 0; i_5 < -i_3+PB_P; i_5++) {
                for (int i_6 = 1; i_6 < min(-i_3+PB_P+2, -i_4+PB_M-2); i_6++) {
                    B[i_4][i_5+i_3][i_6-1][i_6] = E[i_6+i_5][i_3-2][i_6+i_4+1][i_4] - F[i_6+2][i_3][i_5+2][i_4] * G[i_3][i_4][i_6] + E[i_6+i_4+2][i_3][i_4][i_3-2] - B[i_5+1][i_6+i_3-2][i_4-1][i_4] + 1;
                }
            }
        }
        for (int i_7 = 2; i_7 < i_3; i_7++) {
            C[i_7][i_3] = H[i_7-2][i_7] - 1;
        }
    }
#pragma endscop
...
kernel_4324232115_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
