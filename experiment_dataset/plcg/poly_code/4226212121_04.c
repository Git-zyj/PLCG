...
void kernel_4226212121_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_M, PB_L), i_1); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_M, PB_L); i_3++) {
                    A[i_3+i_0][i_2][i_1-2][i_1] = A[i_2+i_0][i_3][i_1][i_1+2] + 1;
                }
            }
        }
    }
...
kernel_4226212121_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
