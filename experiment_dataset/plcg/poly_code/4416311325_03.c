...
void kernel_4416311325_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                    A[i_2+1][i_3][i_0-1][i_1] = E[i_1][i_2-1][i_0][i_3] - A[i_2+1][i_0+2][i_3+1][i_1] + A[i_3][i_0][i_1][i_2] * A[i_2+1][i_1][i_0-1][i_3+1] * A[i_2+1][i_3][i_0+1][i_1] * A[i_1+2][i_3][i_2-3][i_0] * 2;
                }
            }
            B[i_1][i_0-1] = 5;
        }
        for (int i_4 = 0; i_4 < i_0; i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                for (int i_6 = 0; i_6 < PB_L; i_6++) {
                    A[i_6+1][i_4][i_0-3][i_5] = F[i_5+1][i_0-1][i_4][i_6+1] * F[i_4+1][i_6][i_4][i_0] + 1;
                }
            }
        }
    }
        ...
        for (int i_8 = i_7; i_8 < PB_L; i_8++) {
            for (int i_9 = 1; i_9 < PB_M-1; i_9++) {
                for (int j_0 = 0; j_0 < PB_L; j_0++) {
                    A[i_9+1][i_8][i_7-3][j_0] = F[i_7][j_0][i_8][i_9-1] * 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4416311325_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F)));
