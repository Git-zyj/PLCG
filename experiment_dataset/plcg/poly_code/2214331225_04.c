...
void kernel_2214331225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            A[i_2-1] = D[i_1][i_2] + C[i_2+1] * D[i_1+1][i_2] * D[i_1-1][i_2+1] - A[i_2-1] - 4;
        }
    }
#pragma endscop
...
kernel_2214331225_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
