...
void kernel_4334211113_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            A[i_1][i_1+i_0-1] = D[i_0][i_1-1] - D[i_1-1][i_0-1] * A[i_1][i_1+i_0] - 4;
        }
        ...
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                for (int i_6 = 1; i_6 < -i_5+PB_M-1; i_6++) {
                    C[i_6+i_5][i_4-1][i_6-1] = C[i_6+i_5+1][i_4-1][i_5] * 2;
                }
            }
        }
    }
...
kernel_4334211113_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
