...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4432352311_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_2+PB_N, PB_P-2); i_3++) {
                    A[i_0+1][i_1][i_3+2][i_2] = E[i_3][i_0][i_2][i_0] - E[i_1][i_3+i_2][i_2][i_0] + F[i_2][i_3-1][i_1][i_0] * E[i_3][i_2+1][i_1][i_1] * A[i_0][i_1+1][i_3+2][i_2-1] + 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_N-1; i_5++) {
            for (int i_6 = 0; i_6 < PB_Q-2; i_6++) {
                for (int i_7 = 0; i_7 < PB_M-2; i_7++) {
                    B[i_7+1][i_6][i_5] = G[i_4][i_6][i_4+1][i_6] * F[i_4-2][i_7][i_6+2][i_5+1] * E[i_7][i_5+i_4+1][i_7][0] + B[i_7][i_6+1][i_5] - B[i_7+1][i_5-1][i_6] - B[i_7+2][i_5-1][i_6+1] + B[i_6+1][i_7][i_5+1] + B[i_4+1][i_6+1][i_5-1] * B[i_5][i_6][i_7] * 1;
                }
            }
        }
    }
    for (int i_8 = 2; i_8 < PB_P-1; i_8++) {
        C[i_8] = H[i_8] * I[i_8-1] - I[i_8-2] - I[i_8] - J[i_8-1] + H[i_8] - C[i_8+1] - C[i_8-1] - 2;
        for (int i_9 = 0; i_9 < PB_L-1; i_9++) {
            for (int j_0 = 1; j_0 < min(-i_8+PB_L, -i_9+PB_P); j_0++) {
                for (int j_1 = 2; j_1 < min(-i_9+PB_L, PB_L-1); j_1++) {
                    D[j_0][i_9+1][i_8][j_1] = K[i_9][j_0+2][i_8-2] - L[1][j_1][j_0+i_9] + G[j_1+1][j_1+i_9][j_0+i_8][j_0] - F[j_0][i_8][i_9][j_1-2] - L[j_0][j_1+1][i_9] * D[j_0-1][i_9][i_8][j_1+1] * D[j_1][i_9][i_8+1][j_0] * 4;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4432352311_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
