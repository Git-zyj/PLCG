...
void kernel_3332211113_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
            A[i_0][i_0-1][i_1-1] = A[i_0-1][i_0-2][i_1-1] - 5;
        }
    }
    ...
    for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q; i_5++) {
                C[i_5][i_3+1][i_5] = C[i_5][i_3+2][i_5] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3332211113_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
