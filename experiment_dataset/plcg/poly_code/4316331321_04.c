...
void kernel_4316331321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0+1] = D[i_0-1] - E[i_0] + F[i_0] * D[i_0+1] - E[i_0] - E[i_0] + A[i_0-1] * A[i_0+3] + A[i_0] - 2;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = i_2; i_3 < PB_M; i_3++) {
                B[i_2][i_1][i_1] = G[i_3][i_2] + B[i_2][i_1-2][i_1-2] - B[i_2-2][i_1][i_1] - 3;
            }
        }
        for (int i_4 = 0; i_4 < i_1; i_4++) {
            B[i_4][i_1][i_1] = H[i_1][i_4] + 5;
        }
    }
#pragma endscop
...
kernel_4316331321_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
