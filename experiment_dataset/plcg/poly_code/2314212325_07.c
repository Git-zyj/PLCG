...
void kernel_2314212325_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1][i_0] = D[i_1] * A[i_1-1][i_0] * A[i_1+2][i_0+2] * A[i_1+2][i_0] - 3;
        }
    }
    for (int i_2 = 3; i_2 < PB_L-1; i_2++) {
        ...
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            C[i_2-1][i_4] = C[i_2][i_4] - C[i_2-1][i_4-2] * C[i_2-3][i_4] - 5;
        }
    }
#pragma endscop
...
kernel_2314212325_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
