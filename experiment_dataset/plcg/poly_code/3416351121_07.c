...
void kernel_3416351121_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < min(min(i_1, PB_P-1), -i_0+PB_P); i_2++) {
                A[i_0][i_0][i_2+1] = E[i_2][i_1][i_1+i_0+1] * F[0][i_1+1] - E[i_1][i_2-1][i_0] - G[i_2][i_1+i_0][i_2] * H[i_0][i_1][i_2][0] + I[i_1-1][i_0][i_2+i_0] * 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_P+1, PB_P-2); i_4++) {
            B[i_4] = J[i_3][i_4+1] + J[i_4][i_4+i_3-1] - K[i_3][i_4-1] + A[i_3-2][i_3-2][i_4+2] - 2;
        }
        C[i_3] = L[i_3-1] - M[i_3] * N[i_3] - L[i_3+1] - L[i_3] + L[i_3] - B[i_3+1] * C[i_3+1] - 5;
        for (int i_5 = 1; i_5 < min(-i_3+PB_M+1, i_3); i_5++) {
            for (int i_6 = 0; i_6 < min(min(-i_3+PB_M, PB_P-1), -i_5+PB_M+1); i_6++) {
                D[i_6+i_3][i_3][i_5] = I[i_3][i_6][i_5] * E[i_6+i_3][i_5][i_6+1] * E[i_6+i_5-1][i_3-1][0] - G[i_5+i_3-1][i_6+1][i_5-1] - 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_3416351121_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N));
