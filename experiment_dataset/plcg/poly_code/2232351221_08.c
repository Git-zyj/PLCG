...
void kernel_2232351221_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-3; i_1++) {
            A[i_0-1][i_1+1] = C[i_1][i_0-1] * D[i_1][i_0] * D[i_1+1][i_0] + E[i_0] + D[i_1][i_0] - A[i_0-1][i_1] * A[i_0+1][i_1-1] + A[i_0-3][i_1+1] + A[i_0][i_1+3] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_P-1, -i_2+PB_M); i_3++) {
            B[i_3][i_2] = F[i_2+1] + G[i_2][i_2-1] - G[i_2][i_3+1] - G[i_3+i_2][i_3] + G[i_3][i_2] - 2;
        }
    }
#pragma endscop
...
kernel_2232351221_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
