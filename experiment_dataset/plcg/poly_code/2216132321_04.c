...
void kernel_2216132321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 1; i_1 < min(min(PB_P, i_0), -i_0+PB_M+2); i_1++) {
            A[i_1-1] = C[i_0-2][i_1] + C[i_1+i_0-2][i_1-1] - A[i_1+1] + A[i_1] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_P; i_3++) {
            B[i_2][i_3] = C[i_3+2][i_2] - C[i_3+1][i_2-2] * C[i_3][i_3+i_2] - C[i_2][i_3] - B[i_2+2][i_3] - B[i_2-2][i_3+2] * B[i_2][i_3+1] * B[i_2][i_3+2] - 3;
        }
    }
#pragma endscop
...
kernel_2216132321_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
