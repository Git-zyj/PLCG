...
void kernel_4326152123_07(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        C[i_0][i_0-2] = D[i_0+1] - D[i_0-2] * D[i_0] + D[i_0] * D[i_0+2] * D[i_0+1] - D[0] + 6;
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_N-2); i_1++) {
            for (int i_2 = 1; i_2 < PB_N; i_2++) {
                B[i_0][i_0+2][i_2] = E[i_2-1][i_1+1] * E[i_2][i_0] + F[i_1+i_0][i_2][2] - F[i_2][i_0][i_1-1] - C[i_1+2][i_1] - 1;
            }
            C[i_0+2][i_0] = D[i_0-1] + E[i_1-1][i_0] + B[i_0][i_0+2][i_1+1] + 4;
        }
    }
#pragma endscop
...
kernel_4326152123_07(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
