...
void kernel_2236132223_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_L+1); i_1++) {
            B[i_1+i_0] = C[i_1] - D[i_1][i_0] + D[i_1][i_0-1] - D[i_1+i_0-1][i_1-1] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_M; i_3++) {
            B[i_3+i_2] = C[i_3] - D[i_2+1][i_2] * B[i_3+i_2] * B[i_3+i_2-1] * 4;
        }
    }
#pragma endscop
...
kernel_2236132223_08(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
