...
void kernel_2434331313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = E[0] - F[i_0] + E[i_0+1] - G[i_0][i_0] - A[i_0+1] * B[i_0-2] - 4;
        B[i_0-1] = E[i_0] * A[i_0] + B[i_0] + B[i_0-2] + 5;
    }
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M; i_2++) {
            A[i_2] = G[i_2][i_1] - H[i_2][i_1+1] * 4;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < min(PB_L, -i_3+PB_M-1); i_4++) {
            A[i_4+1] = H[i_4+i_3+1][1] + I[i_3][i_4] * J[i_4+i_3] * B[i_4-2] + 4;
        }
    }
#pragma endscop
...
kernel_2434331313_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
