...
void kernel_4336231213_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg)){
        ...
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(min(i_0, PB_L-1), -i_1+PB_Q); i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    A[i_1][i_2+1][i_1-1][i_0] = D[i_3][i_0][i_0][i_2+i_1] + D[i_1][i_2][i_0][0] - 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        B[i_4] = E[i_4] - E[i_4] - B[i_4+1] * B[i_4-1] + B[i_4+1] - B[i_4+1] + 1;
        for (int i_5 = 0; i_5 < min(i_4, PB_L-1); i_5++) {
            for (int i_6 = 1; i_6 < min(PB_L, -i_4+PB_Q); i_6++) {
                for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
                    A[i_6][i_5+1][i_6-1][i_4] = D[i_7][i_5][i_4][i_6+1] - F[i_5][i_7][i_6][i_6+i_4] + G[i_7+1][i_5][i_6+1][i_6][i_4+1] * D[i_5][i_6][i_4][i_7] - 5;
                }
            }
        }
    }
...
kernel_4336231213_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G));
