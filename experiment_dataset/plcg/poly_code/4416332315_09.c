...
void kernel_4416332315_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
        ...
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_M, PB_P-1); i_2++) {
                for (int i_3 = 0; i_3 < PB_P; i_3++) {
                    B[i_3][i_1+1][i_2+1] = E[i_1+1][i_2][i_1][i_3] * F[i_1-2][i_2+2][0] * G[i_2][i_3][i_1][i_2] + 3;
                }
                B[i_0][i_1][i_2] = H[i_2+i_0][i_1-1][i_2-2][0] * B[i_0][i_1+1][i_2] - 5;
            }
        }
        for (int i_4 = i_0; i_4 < PB_N-3; i_4++) {
            C[i_0+2][i_0] = I[i_0+1][i_4-1] * J[i_4][i_0] * K[i_4+2][i_0-2] - C[i_4+3][i_4+1] + C[i_0+1][i_0-1] - 4;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
        for (int i_6 = i_5; i_6 < PB_N-3; i_6++) {
            for (int i_7 = i_5; i_7 < -i_5+PB_M-2; i_7++) {
                B[i_7][i_6][i_5-1] = L[i_7][i_5][i_6+1] - F[i_5+2][i_7+i_5+2][i_6-1] + M[i_6][i_7][i_7+i_5] * I[i_7+i_5][0] - C[i_6+3][i_6+1] + C[i_6+2][i_6] + 4;
            }
        }
    }
#pragma endscop
