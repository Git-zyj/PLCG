...
void kernel_4312312115_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            A[i_0+2][i_1+i_0][i_0] = 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        ...
        for (int i_5 = 1; i_5 < PB_N-1; i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_N-1; i_6++) {
                C[i_2][i_6][i_6] = D[i_2][i_6][i_5-1] + D[i_6+i_5+1][i_2+1][i_2] * C[i_2][i_6+1][i_6+1] - C[i_2+1][i_6-1][i_6-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4312312115_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
