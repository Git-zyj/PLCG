...
void kernel_2214132123_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_M-2); i_1++) {
            A[i_0][i_1] = C[i_1+i_0][i_1] * C[i_1+2][i_0+1] - A[i_0+2][i_1] - 3;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M-1; i_2++) {
            B[i_2-1][i_0-1][i_0] = C[i_2][0] * C[i_2+i_0+1][i_0] + C[i_0][i_2] * C[i_2][0] - 5;
        }
    }
#pragma endscop
...
kernel_2214132123_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
