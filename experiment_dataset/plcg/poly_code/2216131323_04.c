...
void kernel_2216131323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_L-1); i_1++) {
            A[i_1] = C[i_0][i_1] - C[i_1+1][i_0+1] + C[0][i_0] * C[i_0][i_1] * B[i_1-2] + B[i_1] + 2;
        }
        B[i_0] = D[i_0-1] * A[i_0] + B[i_0+1] - B[i_0+2] + 3;
    }
#pragma endscop
...
kernel_2216131323_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
