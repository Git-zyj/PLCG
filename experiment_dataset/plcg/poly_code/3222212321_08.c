...
void kernel_3222212321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-4; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_L-3); i_1++) {
            A[i_0-1][i_1+i_0+2] = C[i_1-1][i_0] - A[i_0-1][i_1+i_0+1] + A[i_0-1][i_1+i_0+3] - 2;
        }
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                B[i_0+2][i_3][i_2] = B[i_0+2][i_3-1][i_2-1] - B[i_0+3][i_3][i_2] * B[i_0+4][i_3-1][i_2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3222212321_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
