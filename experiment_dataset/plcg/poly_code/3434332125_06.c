...
void kernel_3434332125_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = E[0] - F[i_0] - A[i_0+2] * 5;
    }
    for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_M, i_1); i_2++) {
            A[i_1] = G[i_2][i_1-2] + H[i_1-1] * I[i_1-2][i_2] - 6;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        A[i_3-2] = E[i_3] - E[i_3] * E[i_3-1] - F[i_3] + 2;
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            D[i_5][i_4] = F[i_5] - E[i_5] - D[i_5-1][i_4] * 4;
        }
    }
#pragma endscop
...
kernel_3434332125_06(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
