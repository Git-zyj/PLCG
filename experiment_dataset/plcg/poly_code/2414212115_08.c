...
void kernel_2414212115_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        ...
        for (int i_5 = 1; i_5 < min(PB_M-1, i_3); i_5++) {
            D[i_5+1] = E[i_5][i_3-1] + E[i_3][i_5-1] - 4;
        }
    }
#pragma endscop
...
kernel_2414212115_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
