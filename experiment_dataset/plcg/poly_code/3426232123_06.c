...
void kernel_3426232123_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        ...
        B[i_0-2] = E[0] * A[i_0-1] - 1;
        B[i_0-2] = F[i_0] * F[i_0] + E[i_0-1] * F[i_0] - F[i_0] + F[0] * F[i_0-2] * 4;
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < min(PB_M-2, -i_2+PB_M); i_3++) {
            D[i_3+i_2][0] = G[i_3-2][i_2] - H[i_3][i_2] - F[i_3+2] + H[i_3-1][i_2] + 3;
        }
    }
#pragma endscop
...
kernel_3426232123_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
