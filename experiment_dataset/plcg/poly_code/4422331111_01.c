...
void kernel_4422331111_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,int wb,DATA_TYPE POLYBENCH_5D(B,xB,yB,zB,vB,wB,xb,yb,zb,vb,wb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(PB_Q, -i_2+PB_M+1); i_3++) {
                    A[i_0][i_2][i_1][i_0] = E[i_2][i_0][i_3][i_1-1] - E[i_3+i_2-1][i_0][i_1][0] - E[i_2-1][i_0-1][i_0][i_1-1] * 6;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                for (int i_6 = 0; i_6 < -i_5+PB_L; i_6++) {
                    B[i_5+1][i_0][i_4][i_6][i_4-1] = F[i_4+1][i_6+i_5][i_0][i_4][0] + B[i_5][i_0-1][i_4][i_6][i_4-1] - 5;
                }
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_L; i_7++) {
        for (int i_8 = 1; i_8 < i_7; i_8++) {
            C[i_7][i_8] = G[i_7] - H[i_8-1][i_7] + H[i_7][i_8] - H[i_7][i_8] + 6;
        }
        for (int i_9 = 1; i_9 < -i_7+PB_L; i_9++) {
            for (int j_0 = 1; j_0 < -i_9+PB_L; j_0++) {
                D[i_7][j_0+i_9-1][j_0-1] = I[i_9][i_9+i_7][j_0] + I[i_9][j_0][i_7] + J[j_0][i_7][i_9-1] * K[i_7][j_0+i_9-1][j_0] - D[i_7+1][j_0+i_9][j_0-1] * 2;
            }
        }
    }
#pragma endscop
