...
void kernel_3334331225_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-1; i_1++) {
            A[i_1][i_1] = A[i_1-1][i_1-1] + A[i_1+1][i_1+1] * 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_M, -i_2+PB_L-1); i_3++) {
            for (int i_4 = 1; i_4 < PB_M; i_4++) {
                A[i_3-2][i_3-2] = D[i_4][i_3] * E[i_4][i_2][i_3] * F[i_2][i_3-1] * E[i_4-1][i_3+i_2+1][0] * 2;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = 0; i_6 < min(i_5, PB_M-1); i_6++) {
            for (int i_7 = 1; i_7 < -i_6+PB_M; i_7++) {
                A[i_7][i_7] = D[i_7+i_6][i_6+1] * D[i_7+i_6][i_5] * E[i_7-1][i_5][1] - G[i_5][i_7][i_7+1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3334331225_06(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
