...
void kernel_3212131325_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[i_0-1][i_1-1][i_2] = C[i_2][i_2][0] * D[i_2][i_1+1] * D[i_2][i_1] + A[i_0-3][i_1][i_2-2] + A[i_0-2][i_2-2][i_1+2] - A[i_0-1][i_1][i_2] - A[i_2-1][i_1-2][i_0] + 1;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            B[i_0][i_3] = E[i_3] - D[i_3][i_0-1] - D[i_3][i_0] * B[i_0+1][i_3] - 3;
        }
    }
#pragma endscop
...
kernel_3212131325_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
