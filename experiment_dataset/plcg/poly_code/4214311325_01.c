...
void kernel_4214311325_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
                    A[i_2][i_0][i_3][i_1] = A[i_2+1][i_0-2][i_3][i_1+1] - A[i_1+2][i_0][i_3][i_2] - A[i_2][i_3+2][i_0+1][i_1] - 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
            B[i_4][i_5] = C[i_4][i_5+1][0] * D[i_4][i_5] - B[i_4][i_5-1] + B[i_5-2][i_4-1] + 5;
        }
    }
#pragma endscop
...
kernel_4214311325_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
