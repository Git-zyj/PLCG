...
void kernel_2126132323_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        for (int i_1 = min(-i_0+PB_M, PB_M-2) - 1; i_1 >= i_0; i_1--) {
            A[i_0+2][i_1+1] = B[i_0][i_1] + B[i_1+2][i_0+1] * B[i_1+1][i_1+i_0] + A[i_0+3][i_1+1] - A[i_0+4][i_1+2] - A[i_0+1][i_1+1] + 6;
        }
    }
#pragma endscop
...
kernel_2126132323_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
