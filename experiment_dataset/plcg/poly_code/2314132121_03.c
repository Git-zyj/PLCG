...
void kernel_2314132121_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_1-2] = D[i_1] + E[2][i_0] + E[i_1-1][i_0] * A[i_1] * 1;
        }
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            B[i_0][i_2] = E[i_2][i_0] + E[i_2+1][i_0-2] + B[i_0][i_2-2] - 4;
        }
    }
    for (int i_3 = 3; i_3 < PB_M; i_3++) {
        A[i_3-3] = D[i_3] * D[i_3-2] * 6;
    }
#pragma endscop
...
kernel_2314132121_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
