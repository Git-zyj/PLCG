...
void kernel_4216211111_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_N-1, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                A[i_2][i_0][i_1-1] = A[i_2][i_0][i_1-2] - 1;
            }
            B[i_1+i_0-1][i_1-1] = C[i_1+1][i_0+1] - 5;
        }
    }
#pragma endscop
...
kernel_4216211111_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
