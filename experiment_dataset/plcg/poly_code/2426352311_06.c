...
void kernel_2426352311_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0][i_0] = E[i_0] * F[i_0] - F[i_0] * E[i_0-1] * 5;
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-2, i_0); i_1++) {
            D[i_0][i_1] = G[i_1+1][i_0-2] + H[i_1][i_0] + H[i_1+i_0+2][0] * E[i_1] - G[i_1][i_0+2] + A[i_0][i_0] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < min(-i_2+PB_L, i_2); i_3++) {
            D[i_3][i_2] = I[i_3][i_2] - G[i_3-2][i_3+i_2] * H[i_3+1][i_2] - G[i_3][i_2] + 6;
        }
        for (int i_4 = 2; i_4 < i_2; i_4++) {
            D[i_2][i_4] = H[i_4-2][i_2+1] * H[i_2][1] + J[i_2] * H[i_4][i_2+2] * A[i_2][i_2] - D[i_4-1][i_2] * D[i_2+1][i_4-1] + D[i_2][i_4-1] + D[i_2][i_4+1] - 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2426352311_06(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
