...
void kernel_4316252221_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0+2] - E[i_0] * D[i_0] + E[i_0] + D[i_0] * 1;
        for (int i_1 = i_0; i_1 < PB_N-1; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_N, PB_M-2); i_2++) {
                for (int i_3 = 1; i_3 < min(i_0, -i_0+PB_M+1); i_3++) {
                    B[i_0][i_3][i_2][1] = F[i_3][i_2+2][i_0-1][i_1] - G[i_1+1][i_3+i_0-1][i_2][i_3-1] + G[i_2+i_1][i_3-1][i_0-1][i_1] - B[i_1+1][i_3][i_2][1] - B[i_0-1][i_3][i_2][1] * B[i_1][i_3][i_2-2][1] * B[i_0][i_3+1][i_2][1] + 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
            for (int i_6 = 2; i_6 < min(-i_5+PB_M, -i_4+PB_L); i_6++) {
                C[i_5][i_6+1] = H[i_5][i_6][i_6+i_4] * H[i_5][i_4][i_6+2] - I[i_6-2][i_5][i_4] * I[i_6+i_5][i_4][0] * J[i_6][i_5] - I[i_5+2][i_4][i_6-1] * H[i_6+i_5-2][i_4+2][i_5] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4316252221_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
