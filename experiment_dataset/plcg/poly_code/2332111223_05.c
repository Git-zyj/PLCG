...
void kernel_2332111223_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-3; i_1++) {
        ...
        for (int i_2 = 0; i_2 < PB_N; i_2++) {
            C[1][i_1+1] = C[1][i_1+3] - C[1][i_1-1] * 2;
        }
    }
#pragma endscop
...
kernel_2332111223_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
