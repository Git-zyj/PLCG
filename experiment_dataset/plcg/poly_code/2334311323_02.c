...
void kernel_2334311323_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = B[i_0+1] - 3;
        B[i_0+1] = D[i_0][i_0-1] - A[i_0] + A[i_0-1] * A[i_0-2] - A[i_0+2] - 5;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        C[i_1] = A[i_1] * C[i_1-2] + 2;
    }
#pragma endscop
...
kernel_2334311323_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
