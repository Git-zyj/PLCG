...
void kernel_3316311323_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_P-1); i_1++) {
            for (int i_2 = i_1; i_2 < PB_Q; i_2++) {
                A[i_2][i_1][i_2] = A[i_2][i_1-2][i_2] * A[i_2][i_1+1][i_2] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
        B[i_3] = B[i_3+2] - C[i_3] * C[i_3+1] * 1;
        C[i_3] = D[i_3] + B[i_3] * C[i_3+2] - 6;
    }
#pragma endscop
...
kernel_3316311323_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
