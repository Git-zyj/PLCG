...
void kernel_3314311123_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        ...
        for (int i_2 = i_0; i_2 < min(PB_M, -i_0+PB_L); i_2++) {
            for (int i_3 = 0; i_3 < -i_0+PB_M+1; i_3++) {
                B[i_3+i_0-1][i_2+i_0][1] = D[i_3+1][i_2][0] - 2;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_L+2; i_5++) {
            C[i_4][i_5][i_5] = C[i_5-2][i_4][i_4] * 4;
        }
    }
#pragma endscop
...
kernel_3314311123_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
