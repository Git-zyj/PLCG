...
void kernel_2116231221_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(PB_Q, -i_0+PB_L); i_1++) {
            A[i_0][i_1] = B[i_1][i_0] * B[i_1+i_0][0] * B[i_1][i_0+1] - A[i_0][i_1-1] * A[i_0][i_1+1] * 4;
        }
    }
#pragma endscop
...
kernel_2116231221_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
