...
void kernel_4224232225_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = C[i_0+2] * D[i_0] * A[i_0-1] + 1;
        for (int i_1 = max(2, i_0); i_1 < min(PB_M, -i_0+PB_L-1); i_1++) {
            A[i_0+1] = E[i_1][i_0+2] - E[i_0][i_1+i_0+1] + C[i_1-2] - A[i_0] * 6;
        }
    }
#pragma endscop
...
kernel_4224232225_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
