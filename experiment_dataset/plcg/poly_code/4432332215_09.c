...
void kernel_4432332215_09(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
                    B[i_3][i_0][i_0][i_1] = E[i_3][i_1-1][i_0][i_1] + F[i_0-2][i_3+2][i_1][i_2] * G[i_2-1][i_3][i_1][i_0-1] * G[i_3+1][i_2+2][i_0-2][i_1] + 4;
                }
            }
        }
        for (int i_4 = 1; i_4 < -i_0+PB_Q; i_4++) {
            for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
                for (int i_6 = 1; i_6 < PB_L; i_6++) {
                    B[i_5][i_6][i_6][i_4] = F[i_0][i_5][0][i_4+2] - F[i_0-2][i_5+1][i_6+2][i_4] + F[i_5][0][i_4+i_0][i_6-1] - B[i_5+1][i_6][i_6][i_4-1] * B[i_5-1][i_0-1][i_0-1][i_4] + B[i_5][i_6][i_6][i_4-1] + 5;
                }
            }
        }
        for (int i_7 = 1; i_7 < PB_Q+1; i_7++) {
            for (int i_8 = 1; i_8 < min(PB_P, i_7); i_8++) {
                for (int i_9 = 0; i_9 < PB_M; i_9++) {
                    B[i_9][i_0][i_0][i_7-1] = E[i_9][i_8-1][i_0][i_8] * 2;
                }
            }
        }
    }
    for (int j_0 = 0; j_0 < PB_P-2; j_0++) {
        for (int j_1 = 1; j_1 < j_0; j_1++) {
            D[j_1+1] = H[0][j_0+1] * I[j_1+2] + H[j_1-1][j_0+2] * D[j_1+2] * 4;
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4432332215_09(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
