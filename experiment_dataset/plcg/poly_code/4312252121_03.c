...
static void print_array(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4312252121_03(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        C[i_0-2][i_0-2] = D[i_0+2] * D[i_0+2] + D[i_0+1] * D[i_0] + D[i_0] + C[i_0][i_0] - 1;
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                for (int i_3 = 1; i_3 < min(PB_P-2, -i_1+PB_L); i_3++) {
                    B[i_3+i_1][i_2][i_0][i_1] = E[i_0+2][i_1+1][i_3-1][0] + F[i_2+2][i_3+2][i_1] + E[i_2+i_0][i_3+i_1][i_0+2][i_3] + 3;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N-2; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            C[i_4][i_4] = G[i_5][1] * G[i_5][i_4] + H[i_5+2][i_4] + G[i_5][i_4] * I[i_5] + H[i_4][i_5] + C[i_4+2][i_4+2] + 4;
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4312252121_03(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
