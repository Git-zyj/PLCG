...
void kernel_3224132113_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M+1, PB_L-1); i_1++) {
            B[i_1+i_0-1][i_1+1] = C[i_0+2][i_0+1] + D[i_1] + C[i_0][i_1-2] * B[i_1+i_0-2][i_1] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            for (int i_4 = max(-i_3+2, 0); i_4 < min(PB_P, -i_3+PB_Q-2); i_4++) {
                B[i_4+i_3-2][i_3] = E[i_4+i_3][i_4][i_2] * E[i_4][i_3][i_2] + E[i_4][i_2][i_4+i_3+2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3224132113_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
