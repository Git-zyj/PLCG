...
void kernel_2316351123_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M+1; i_1++) {
            A[i_1+i_0-1][i_1][i_1] = D[i_1][i_0] * B[i_1-2][i_1+i_0-1] - 1;
        }
    }
    for (int i_2 = 4; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 3; i_3 < -i_2+PB_M; i_3++) {
            B[i_3-1][i_3+i_2] = E[i_3+1] * D[i_3+1][i_2] * F[i_2+1][i_3] * D[i_3-1][i_2] - F[i_2][i_3] + D[i_2][i_3] + 5;
            C[i_3+i_2] = D[i_3-1][i_2] * F[i_2][i_3-1] + D[i_3][i_2-1] - F[i_3][i_2] * D[i_3+i_2][i_2] + D[i_3-1][i_2+1] - B[i_3-3][i_3+i_2-4] * 4;
        }
    }
#pragma endscop
...
kernel_2316351123_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
