...
void kernel_4422251115_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-1] * F[i_0-1] + E[i_0] + F[i_0] * F[i_0-1] + F[i_0+1] + F[i_0] - A[i_0-1] * 1;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                for (int i_4 = 1; i_4 < PB_L; i_4++) {
                    B[i_1-1][i_3][i_2-1][0] = G[i_3-1][i_4][i_2+1][i_1] + G[i_1][i_3][i_2+1][i_4] * G[i_2][i_3][i_1+1][i_4] * H[i_4][i_2][i_1-1][i_3] * B[i_4-1][i_3][i_2-2][0] + 3;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
            A[i_6+1] = I[i_6][i_5+1] * I[i_6][i_5] * I[i_6][i_5] + 4;
        }
        A[i_5-1] = F[i_5-1] * E[i_5] + F[i_5+1] - E[i_5-1] - F[i_5] * 1;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4422251115_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
