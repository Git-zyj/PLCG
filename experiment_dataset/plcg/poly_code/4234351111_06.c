...
void kernel_4234351111_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = C[i_0] * C[i_0-1] + D[i_0] + C[i_0] * A[i_0-1] + 4;
    }
        ...
        for (int i_2 = 0; i_2 < min(-i_1+PB_L, PB_M-2); i_2++) {
            for (int i_3 = 0; i_3 < i_2; i_3++) {
                for (int i_4 = 1; i_4 < -i_3+PB_P; i_4++) {
                    B[i_2+1][i_1-1][i_3+1][i_3] = E[i_4+1][i_3+1][i_2+1][i_2] + F[i_4][i_2+i_1][i_3][i_1] + G[i_4][i_3][i_2][0] * E[i_4+1][i_1][i_2][i_3+1] + E[i_4-1][i_1][i_2][i_3+1] + E[i_2][i_1][i_4+i_3][i_4] * B[i_2+2][i_1-1][i_3+1][i_3] * 4;
                }
            }
        }
    }
...
kernel_4234351111_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
