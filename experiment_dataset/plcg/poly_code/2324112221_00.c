...
void kernel_2324112221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        ...
        for (int i_2 = 3; i_2 < PB_M-2; i_2++) {
            B[i_2-1] = B[i_2-3] + B[i_2] + 6;
            C[i_0][i_2] = D[i_0][i_2] * B[i_2-1] + C[i_0+1][i_2+2] * 4;
        }
    }
#pragma endscop
...
kernel_2324112221_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
