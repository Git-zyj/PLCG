...
void kernel_2414111225_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            B[i_1+1] = E[i_1+1][i_0-1] * 5;
        }
        B[i_0+1] = B[i_0] - B[i_0-1] * 1;
    }
    for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
        B[i_2+1] = F[1] * 5;
        D[i_2] = E[i_2][i_2-1] * B[i_2+1] + D[i_2-2] - 1;
    }
#pragma endscop
...
kernel_2414111225_04(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
