...
void kernel_4232311323_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-3; i_0++) {
        A[i_0+1] = A[i_0+3] + A[i_0+2] + A[i_0] - 4;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_N-1, -i_1+PB_N); i_2++) {
            A[i_2+1] = C[i_2+i_1][i_2] + D[i_2+i_1] * 4;
        }
    }
#pragma endscop
...
kernel_4232311323_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
