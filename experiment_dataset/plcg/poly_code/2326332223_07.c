...
void kernel_2326332223_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-4; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[i_0+2][i_0+2] = D[i_1] + E[i_1+2][i_0] - F[i_0+2] * A[i_0+3][i_0+3] - A[i_0+4][i_0+4] + A[i_0+1][i_0+1] * 6;
        }
        for (int i_2 = 2; i_2 < -i_0+PB_M-1; i_2++) {
            B[i_0-2][i_2+2] = G[i_2+2][i_0] - H[i_2][i_0] * E[i_2+i_0+1][i_2-2] - 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            A[i_3+1][i_3+1] = H[i_3][i_4] + D[0] + 3;
        }
    }
#pragma endscop
...
kernel_2326332223_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
