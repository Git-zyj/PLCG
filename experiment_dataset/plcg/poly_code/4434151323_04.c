...
void kernel_4434151323_04(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
        ...
        for (int i_1 = i_0; i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    B[i_2-1][i_0][i_0][i_1] = E[i_2+i_1][i_3][i_0][0] - F[i_2-1][i_0][i_2-1][i_1+i_0][i_3] * 4;
                }
            }
        }
        for (int i_4 = 0; i_4 < i_0; i_4++) {
            for (int i_5 = 2; i_5 < -i_0+PB_L; i_5++) {
                for (int i_6 = 1; i_6 < -i_5+PB_M-1; i_6++) {
                    B[i_0][i_4][i_4][i_5] = E[i_5-1][i_4+1][i_6-1][i_0] * G[i_5][i_4][i_5+i_0] - E[i_6+i_5+1][i_0+1][i_0][i_4+1] + E[i_4][i_0+1][i_6+1][1] - G[i_0][i_6+i_5-1][i_5] - B[i_0+2][i_4][i_4][i_5] - B[i_0-2][i_4][i_4][i_5+1] * B[i_0][i_4][i_4][i_5-2] - 2;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-2; i_7++) {
        C[i_7] = H[i_7] - H[i_7] + H[i_7+1] - H[i_7-1] * C[i_7+2] + C[i_7+1] * 2;
        for (int i_8 = 0; i_8 < PB_M-1; i_8++) {
            for (int i_9 = max(i_8, 1); i_9 < -i_7+PB_M; i_9++) {
                B[i_9][i_8][i_8][i_7+1] = G[i_8][i_7+1][i_9] - E[i_8+1][i_9][i_7-1][i_7+1] - G[i_8][i_9][i_7] + G[i_8+1][i_9-1][i_9] - G[i_8+1][i_7][i_9+1] * I[i_8+1][i_9+1] * G[i_9+i_7][i_8+1][1] + G[i_8][i_7-1][i_9] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4434151323_04(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
