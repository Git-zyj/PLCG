...
void kernel_4434251325_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < min(min(-i_2+PB_L, -i_0+PB_L), -i_1+PB_L); i_3++) {
                    A[i_0+1][i_2][i_1][i_3] = E[i_2-1][i_0+1][i_3+i_1][0] - E[i_0][i_3-1][i_1+1][i_2] - E[i_3+1][i_1][i_0][i_2+1] - F[i_1+1][i_3+i_2][i_0][i_3] - G[i_2][i_3+i_0][i_1+1][i_0][i_1] - H[i_1][i_3+i_0][1] - A[i_0+1][i_2-2][i_1][i_3] - A[i_3+1][i_2-2][i_0][i_1] + A[i_0-1][i_2][i_1-1][i_3+1] * 6;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        B[i_4] = I[i_4+1] * I[i_4] * I[i_4-1] - J[i_4] + B[i_4+1] + B[i_4-2] * 5;
    }
    for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 0; i_6 < -i_5+PB_L; i_6++) {
            B[i_6] = K[i_6+i_5][i_6+1] + K[i_6+i_5+1][0] + L[i_6][i_6] - K[i_6][i_5] * L[i_6+i_5][i_6] * 1;
        }
        for (int i_7 = 0; i_7 < -i_5+PB_L; i_7++) {
            for (int i_8 = 0; i_8 < PB_L-1; i_8++) {
                B[i_8+2] = M[i_7][i_5+1][i_8+1] - M[i_8][i_7+i_5][i_8] - H[i_8+i_7][i_5][0] * M[i_8][i_7][i_5] + 6;
            }
        }
    }
#pragma endscop
