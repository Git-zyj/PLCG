...
void kernel_4222132215_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_M, -i_0+PB_N-2); i_2++) {
                A[i_2+i_0+2][i_1-2][i_1-1] = C[i_1+1][i_2][i_1] * D[i_2][i_0-2] * A[i_2+i_0+2][i_1-1][i_1] * 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = 1; i_5 < min(PB_M+1, -i_4+PB_N-2); i_5++) {
                A[i_5+i_4+2][i_3-2][i_3-1] = D[i_5-1][i_4-1] + C[i_5][i_4][i_3] * C[i_5][i_3][i_4] + A[i_5+i_4+2][i_3-1][i_3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4222132215_09(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
