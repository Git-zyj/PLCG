...
void kernel_3226111221_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            B[i_1][i_1][0] = C[i_1-1][i_0][0] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
            for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
                B[i_4][i_4][0] = C[i_4-1][i_3-1][i_2] + B[i_4+1][i_4+1][0] + B[i_4-1][i_4-1][0] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3226111221_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
