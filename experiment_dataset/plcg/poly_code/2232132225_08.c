...
void kernel_2232132225_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = C[i_0] * C[i_0] - D[i_0][1] - A[i_0-1] * A[i_0+1] * 5;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            A[i_2] = D[i_2-1][i_1] - D[i_2-1][i_1+2] + D[i_2][i_2] + 1;
        }
    }
#pragma endscop
...
kernel_2232132225_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
