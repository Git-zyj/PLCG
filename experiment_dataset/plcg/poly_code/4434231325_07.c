...
void kernel_4434231325_07(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < i_0; i_2++) {
                for (int i_3 = 3; i_3 < min(PB_L, -i_1+PB_P); i_3++) {
                    C[i_0+1][i_2-2][i_1][i_3-3] = E[i_2][i_0][i_3][i_1-1] - F[i_0][i_2][0][i_3+i_1] + 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        B[i_4] = G[i_4] * B[i_4-1] + B[i_4-2] * 1;
        for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
            for (int i_6 = 0; i_6 < -i_5+PB_Q-1; i_6++) {
                for (int i_7 = 1; i_7 < PB_L; i_7++) {
                    C[i_5][i_4][i_6][i_7-1] = H[i_6][0][i_7] + F[i_7-1][i_5][i_4-1][i_6+1] + E[i_4][i_5-1][i_7][i_7] + E[i_7][i_4][i_5+1][i_6+i_5+1] - C[i_5][i_4][i_6][i_7] * C[i_5][i_4+1][i_6][i_7-1] * C[i_5+2][i_4][i_6+1][i_7] - 1;
                }
            }
        }
        for (int i_8 = 0; i_8 < PB_M; i_8++) {
            for (int i_9 = 0; i_9 < PB_Q-1; i_9++) {
                for (int j_0 = 1; j_0 < min(i_8, PB_Q-1); j_0++) {
                    C[i_8][i_4+2][i_9+1][j_0+1] = F[i_8][i_4-1][j_0][i_9+1] * H[j_0][i_9][j_0-1] + 2;
                }
            }
        }
    }
...
init_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434231325_07(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
