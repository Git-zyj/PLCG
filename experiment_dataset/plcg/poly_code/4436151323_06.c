...
void kernel_4436151323_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        A[i_0+1] = E[i_0] + E[i_0-1] - E[i_0] + A[i_0-1] + A[i_0+3] + 6;
        A[i_0+1] = E[i_0] - E[i_0] * E[i_0] * E[i_0] + E[i_0] - E[i_0+1] * E[i_0] - E[i_0+1] * A[i_0+1] - 6;
    }
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_Q-3, -i_1+PB_Q); i_2++) {
            for (int i_3 = 0; i_3 < -i_2+PB_Q-3; i_3++) {
                C[i_1][i_1-1][i_3+i_2+1] = F[i_3][i_2][i_2-1] * F[i_3][i_1-1][i_2+i_1] * F[i_3][i_1+1][i_2] * C[i_1+2][i_1+1][i_3+i_2-1] + C[i_1-1][i_1-2][i_3+i_2+3] * C[i_1][i_1-1][i_3+i_2+1] - 6;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = i_4; i_5 < PB_L-1; i_5++) {
            for (int i_6 = i_4; i_6 < min(-i_4+PB_M, -i_5+PB_Q+1); i_6++) {
                C[i_4-1][i_4-2][i_6+i_5-1] = F[i_5+1][i_6][i_4] - G[i_6+i_4][1] * F[i_6][i_5][i_4+1] + F[i_5+1][i_6][i_4] * F[i_4+1][i_6-1][0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4436151323_06(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
