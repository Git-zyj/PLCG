...
void kernel_4222311225_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_N, -i_0+PB_P+3); i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_P+1; i_3++) {
                    A[i_2][i_1][i_3+i_0-1][i_1] = A[i_3][i_1][i_2+i_0-3][i_1] - 6;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        B[i_4] = C[i_4] + C[i_4] * B[i_4-2] + B[i_4+2] - B[i_4-1] * 3;
    }
#pragma endscop
...
kernel_4222311225_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
