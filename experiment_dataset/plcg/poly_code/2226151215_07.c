...
void kernel_2226151215_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1-1][i_0+1][i_1+1] = C[i_0][i_1] * C[i_0][i_0] * D[i_0+1] + D[i_0] + C[i_0-1][i_1] - A[i_1-1][i_0+2][i_1+1] * A[i_0-1][i_1][i_0+1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_N; i_3++) {
            A[i_2-1][i_3+1][i_2+1] = C[i_3+i_2][i_3] - C[i_3+i_2][i_3+1] * E[i_3+i_2][i_3][0] * C[i_3+i_2+1][0] - 6;
        }
    }
#pragma endscop
...
kernel_2226151215_07(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
