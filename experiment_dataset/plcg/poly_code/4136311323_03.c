...
void kernel_4136311323_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = PB_L - 1; i_1 >= 2; i_1--) {
            for (int i_2 = max(2, i_0); i_2 < min(-i_0+PB_M, -i_1+PB_P); i_2++) {
                for (int i_3 = 1; i_3 < min(-i_0+PB_L, i_0); i_3++) {
                    A[i_0][i_1][i_3][i_2+i_1] = B[i_2+i_0][i_3+i_0][i_1-1] * A[i_0][i_1-1][i_3][i_2+i_1-1] + A[i_2-2][i_3][i_1-2][i_3+i_0] * A[i_0][i_1][i_3-1][i_2+i_1-2] + 6;
                }
            }
        }
    }
...
kernel_4136311323_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
