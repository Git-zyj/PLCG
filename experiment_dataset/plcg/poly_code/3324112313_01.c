...
void kernel_3324112313_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            B[i_3][i_2+2] = E[i_3+i_2][i_3+1] + B[i_2-1][i_3+1] - B[i_3][i_2+3] - B[i_3-1][i_2+1] * B[i_3][i_2+1] - B[i_3-1][i_2+2] - 6;
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < PB_M; i_5++) {
            B[i_4][i_5+1] = E[i_5][i_4] * 3;
        }
    }
#pragma endscop
...
kernel_3324112313_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
