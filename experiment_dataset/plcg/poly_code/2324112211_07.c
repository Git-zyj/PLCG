...
void kernel_2324112211_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_N-2; i_1++) {
            A[i_1+2][i_0-2] = D[i_1-1][i_0] * D[i_1-2][i_0] - 5;
            B[i_0][i_1] = A[i_1+2][i_0-3] - A[i_1+2][i_0-2] + B[i_0][i_1+1] * B[i_0-1][i_1] * 5;
        }
    }
...
kernel_2324112211_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
