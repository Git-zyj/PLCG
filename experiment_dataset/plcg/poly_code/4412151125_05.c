...
void kernel_4412151125_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    A[i_3][i_1][i_0-1][0] = E[i_2+i_0+1][i_1][i_3][i_3] + E[i_3][i_2][i_3][i_0] - 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_M-2; i_5++) {
            B[i_5+i_4][1] = F[i_5][i_4][i_5] + G[i_4][i_5] - G[i_5-1][i_4-1] - G[i_5][i_4] * G[i_4][i_5] - B[i_5+i_4+2][1] + 4;
            for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 1; i_7 < min(-i_6+PB_P+1, PB_M-1); i_7++) {
                    A[i_5][i_6+1][i_7-1][0] = E[i_5][i_7+i_6-1][i_5][i_4-1] * F[i_6][i_7+1][i_5] - E[i_5-1][i_4][i_6+i_5][i_5] + E[i_4-1][i_6][i_5][i_7] * 1;
                }
            }
        }
        for (int i_8 = 0; i_8 < min(-i_4+PB_Q-1, PB_M-2); i_8++) {
            for (int i_9 = 1; i_9 < PB_M; i_9++) {
                for (int j_0 = 1; j_0 < min(i_9, -i_9+PB_M); j_0++) {
                    A[i_4][i_8+2][j_0-1][0] = F[j_0-1][i_4-1][i_9] * E[j_0+i_9][i_8][i_4][i_9] * E[j_0-1][i_9-1][i_8+i_4+1][1] - E[i_8][i_9][j_0][i_4] - E[i_8][i_4][i_9][i_8] * E[j_0+1][i_4][i_8][i_9+1] + 2;
                }
            }
        }
    }
...
kernel_4412151125_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
