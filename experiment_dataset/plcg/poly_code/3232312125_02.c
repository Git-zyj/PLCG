...
void kernel_3232312125_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+2][i_0] = C[i_0+1] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            for (int i_3 = 4; i_3 < PB_L; i_3++) {
                B[i_1][i_3-2][1] = B[i_1+1][i_3-4][1] - B[i_1][i_3-4][1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3232312125_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
