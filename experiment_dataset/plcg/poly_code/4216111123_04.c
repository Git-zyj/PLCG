...
void kernel_4216111123_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-3; i_1++) {
            for (int i_2 = PB_P - 1; i_2 >= 1; i_2--) {
                A[i_0][i_1+1][i_2] = C[i_2-1][i_1][i_0] + 6;
            }
            for (int i_3 = 0; i_3 < PB_Q-1; i_3++) {
                B[i_0+1][i_3][i_1+1] = C[i_0][i_3+1][i_1+1] * A[i_0][i_1+1][i_3] * B[i_0+3][i_3+1][i_1+3] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4216111123_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
