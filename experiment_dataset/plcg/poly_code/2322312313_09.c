...
void kernel_2322312313_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 3; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 3; i_3 < PB_L+1; i_3++) {
            C[i_2][i_3-2] = C[i_2+1][i_3-3] - C[i_2][i_3-1] - C[i_3][i_2-1] * C[i_3][i_2-3] - C[i_2-1][i_3-3] + 3;
        }
    }
#pragma endscop
...
kernel_2322312313_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
