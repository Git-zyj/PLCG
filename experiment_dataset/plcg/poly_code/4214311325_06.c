...
void kernel_4214311325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M+1; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M-1; i_2++) {
                A[i_2+i_1-1][i_1+i_0] = A[i_2+i_0+1][i_2+i_1+2] - A[i_1+i_0-3][i_2+i_1] + A[i_1+i_0-1][i_2+i_1-2] - A[i_2+i_0-1][i_2+i_1-2] + A[i_2+i_0-1][i_2+i_1+2] * A[i_1+i_0-2][i_2+i_1] + 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M+1, i_3); i_4++) {
            B[i_4+i_3-1][i_4] = C[i_4][i_4] - 2;
        }
    }
#pragma endscop
...
kernel_4214311325_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
