...
void kernel_2216152113_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-1); i_1++) {
            A[i_1+2][i_1] = C[i_1][i_0] + D[i_1] + C[i_1+1][i_0] * C[i_0][i_1+2] + A[i_1+3][i_1+1] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L+2; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[i_3][i_2-2] = C[i_3][i_2+2] - D[i_3] * C[i_2][i_3] - C[i_3+1][i_2] - D[i_3-1] - D[i_3-1] - 4;
        }
    }
#pragma endscop
...
kernel_2216152113_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
