...
void kernel_2224152213_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_N+1, PB_L); i_1++) {
            A[i_0-1][i_1] = C[i_1][i_0] - C[i_1][i_0] + D[i_1] * D[i_1] + C[i_1+i_0-1][i_1] * A[i_1-1][i_0-1] * A[i_0][i_1] + 6;
        }
        for (int i_2 = 0; i_2 < min(PB_M, -i_0+PB_N+2); i_2++) {
            B[i_0][i_2] = C[i_2][i_0] - D[i_0] * C[i_2][i_0] * C[i_2+2][i_0+1] - C[i_2+1][i_2+i_0-2] - B[i_0+1][i_2] * 5;
        }
    }
#pragma endscop
...
kernel_2224152213_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
