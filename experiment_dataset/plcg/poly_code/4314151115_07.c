...
void kernel_4314151115_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_N; i_2++) {
                for (int i_3 = 2; i_3 < min(PB_N-1, -i_2+PB_N); i_3++) {
                    A[i_3-1][i_1][i_0] = D[i_1][i_0][i_3][i_2] * D[i_3+i_2][i_1+i_0][i_3+1][i_0-1] * D[i_3+1][i_0-1][i_1][i_2] - A[i_3-2][i_1][i_0] * 5;
                }
            }
        }
        B[i_0+1] = E[i_0-1] * E[i_0] * E[i_0] * E[i_0] - E[i_0] * B[i_0+2] * 6;
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < min(PB_L, -i_4+PB_M); i_5++) {
            C[i_4][i_5] = E[i_4] * F[i_5][i_4] + E[i_4] * F[i_5-1][i_4+1] - F[i_5][i_4] + F[i_5+i_4][1] * C[i_4][i_5+1] * 4;
        }
    }
#pragma endscop
...
kernel_4314151115_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
