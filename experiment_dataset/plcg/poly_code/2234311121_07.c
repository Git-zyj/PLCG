...
void kernel_2234311121_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            B[i_2][i_2] = B[i_0-1][i_0-1] - B[i_2+1][i_2+1] + 4;
        }
    }
#pragma endscop
...
kernel_2234311121_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
