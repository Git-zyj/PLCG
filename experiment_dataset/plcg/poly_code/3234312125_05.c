...
void kernel_3234312125_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[i_0-1][i_1+2] = C[i_1][i_0] - A[i_0][i_1+2] - B[i_1+1][i_0] * 4;
        }
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            for (int i_3 = i_0; i_3 < min(-i_0+PB_N, -i_2+PB_M); i_3++) {
                B[i_2][i_3] = D[i_3+i_0][i_3+i_2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3234312125_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
