...
void kernel_4216112313_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
                B[i_1][i_2+1] = C[i_1][i_0] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
            B[i_4][i_3] = B[i_4][i_3-1] - B[i_4+1][i_3-1] * B[i_4-1][i_3-1] - 2;
        }
    }
#pragma endscop
...
kernel_4216112313_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
