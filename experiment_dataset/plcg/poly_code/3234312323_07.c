...
void kernel_3234312323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 3; i_1 < PB_N-1; i_1++) {
            A[i_1-1] = A[i_1-3] - A[i_1-2] + A[i_1+1] + A[i_1] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_L, -i_2+PB_M-2); i_3++) {
            B[i_3+i_2][i_3] = C[i_2][i_3][1] + B[i_3+i_2-1][i_3-1] - B[i_3+i_2+2][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_3234312323_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
