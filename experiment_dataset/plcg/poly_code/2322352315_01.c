...
void kernel_2322352315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = D[i_0+1] * E[i_0+1] + D[0] * E[i_0+1] - A[i_0-1] - A[i_0+1] - 6;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
            A[i_2] = F[i_2+2][i_1+2] + G[i_1][0] - H[i_2][i_1] * I[i_2] * G[i_2][i_1-2] - G[i_2-2][i_1] + H[i_2+1][0] + 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        A[i_3] = D[i_3+1] + I[i_3] + E[2] + E[i_3] * A[i_3] - 4;
    }
#pragma endscop
...
kernel_2322352315_01(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
