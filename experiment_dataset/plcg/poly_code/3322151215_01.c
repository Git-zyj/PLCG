...
void kernel_3322151215_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0] + D[i_0] * D[i_0] + D[i_0] * D[i_0-1] * 4;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1] = D[i_1] * D[i_1] - D[i_1] * A[i_1] + B[i_1-1] - 2;
        B[i_1+1] = D[i_1] * D[i_1] * D[i_1] * D[0] + D[i_1] * B[i_1] + 1;
    }
#pragma endscop
...
kernel_3322151215_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
