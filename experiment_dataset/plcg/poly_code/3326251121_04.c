...
void kernel_3326251121_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] - D[i_0] - D[i_0] + D[i_0+1] * E[i_0+1] * 5;
        B[i_0] = D[i_0-1] + D[i_0-1] - E[i_0+1] * F[i_0-1][i_0] + D[i_0] - B[i_0+2] * 4;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_L, PB_L-1); i_2++) {
            for (int i_3 = 1; i_3 < i_2; i_3++) {
                C[i_1][i_3-1][i_3+1] = G[i_3][i_3+i_1][0] * G[i_3][i_1][i_2+i_1] + G[i_2][i_1+1][i_2] - G[i_3][i_1][i_2+1] * H[i_1][i_3][i_3] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3326251121_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
