...
void kernel_4334152313_01(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            for (int i_2 = i_0; i_2 < PB_N-1; i_2++) {
                C[i_1][i_2+1] = D[i_2][i_0][i_1] - 4;
            }
        }
        for (int i_3 = 2; i_3 < -i_0+PB_P-2; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_0+PB_L+2, PB_M); i_4++) {
                for (int i_5 = 0; i_5 < min(-i_4+PB_M, -i_3+PB_M-2); i_5++) {
                    C[i_5+1][i_4] = E[i_4][i_5+2][i_0][i_3] * F[i_3][i_3-2][0][i_4+i_0-2][i_5] * E[i_3][i_0][i_5][i_4] * E[i_5+i_3+2][i_0+1][i_5][i_4] * E[i_0][i_4][i_5+i_3][i_5] + E[i_3][i_5+i_4][i_3+i_0+2][i_4-1] * E[i_5+i_4][i_0][i_3+2][i_0] - 4;
                }
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_N-1; i_6++) {
        for (int i_7 = 1; i_7 < -i_6+PB_M; i_7++) {
            C[i_6][i_7] = G[i_7][i_6] + G[i_7+2][i_6] * G[i_6][0] * G[i_7][i_6-2] * G[i_7][i_6] * G[i_7][i_6] - G[i_7+i_6][i_7] - C[i_6-1][i_7-1] * C[i_6][i_7-1] * C[i_7][i_6+1] - 6;
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4334152313_01(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
