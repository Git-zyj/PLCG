...
void kernel_2436231121_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            B[i_2+1] = F[i_2][i_1] - G[i_1][i_2] * 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
            C[i_3][i_4-1] = F[i_4][i_3-1] * F[i_4][i_3+1] - G[i_4+i_3][1] + F[i_4][i_3+1] - 5;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
        D[i_5+1] = H[i_5-1] - H[i_5] - H[i_5+1] - D[i_5+2] * 2;
    }
#pragma endscop
...
kernel_2436231121_07(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
