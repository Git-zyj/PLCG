...
void kernel_4132111213_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < PB_M-1; i_2++) {
                for (int i_3 = 2; i_3 < PB_P-1; i_3++) {
                    A[i_2-1][i_1][i_3+1] = B[i_2+1][i_1-1][i_1] + A[i_3-2][i_1][i_0+1] + A[i_2-2][i_3][i_1+1] * 1;
                }
            }
        }
    }
...
kernel_4132111213_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
