...
void kernel_3432352223_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = A[i_0] - A[i_0+4] - A[i_0+3] + 6;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        A[i_1+2] = E[i_1-1] * F[i_1] - G[i_1] + G[i_1+1] + 2;
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            C[i_2][i_3] = H[i_2][i_3] * E[i_3] - G[i_2] - C[i_2-1][i_3] - 5;
        }
        for (int i_4 = 2; i_4 < -i_2+PB_L+1; i_4++) {
            for (int i_5 = 2; i_5 < min(PB_L-2, -i_2+PB_P); i_5++) {
                D[i_5][i_4][i_2] = I[i_5][i_4] + J[i_4-2][i_5-2][i_4] - I[i_5+1][i_2+1] + J[i_4-1][i_5+2][i_5] + K[i_4][i_5+i_2] * L[i_5][i_4-2][i_2+2] * M[i_5+i_4-2][i_2][i_2] - J[i_5][i_4+i_2-1][i_4] * H[i_5][1] + M[i_5][i_5+2][i_2-1] - 3;
            }
        }
    }
#pragma endscop
