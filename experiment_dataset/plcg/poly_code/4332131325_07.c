...
void kernel_4332131325_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_L-1; i_2++) {
                A[i_2][i_1][i_0] = D[i_1][i_0][i_2] * D[i_0][i_2][i_2+i_1+1] * D[i_2][i_2+1][i_1+1] + A[i_2][i_1+1][i_0+1] + A[i_2-1][i_1][i_0] - A[i_2][i_1][i_0+1] * 2;
            }
        }
    }
        ...
        for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 2; i_5 < PB_L; i_5++) {
                for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
                    B[i_6][i_4+1][i_4] = E[i_4+1][i_6][i_5][i_3-1] + E[i_3][i_4][i_6][0] * E[i_4][i_5-1][i_3+1][i_3] - B[i_4-2][i_6+1][i_6] + B[i_5-2][i_4+1][i_4] + 3;
                }
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_M-1; i_7++) {
        for (int i_8 = i_7; i_8 < PB_N-2; i_8++) {
            for (int i_9 = 0; i_9 < -i_7+PB_P-2; i_9++) {
                C[i_8+1][i_9][i_9+i_7+1] = D[i_9][i_7][i_8] + D[i_7+1][i_9+1][1] * C[i_8+1][i_9][i_9+i_7+2] * C[i_8+2][i_9][i_9+i_7+2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4332131325_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
