...
void kernel_4212212221_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-4; i_2++) {
                A[i_2+2][i_0][i_1] = A[i_2+3][i_0][i_1] * A[i_2][i_0][i_1-1] * A[i_2+4][i_0-1][i_1] + 5;
            }
        }
        for (int i_3 = 2; i_3 < min(PB_M, -i_0+PB_P); i_3++) {
            B[i_3][i_0][i_3+i_0] = C[i_0][i_3] + B[i_3-2][i_0][i_3+i_0-2] + 2;
        }
    }
#pragma endscop
...
kernel_4212212221_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
