...
void kernel_3216212313_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L-3; i_2++) {
        for (int i_3 = i_2; i_3 < PB_P-3; i_3++) {
            B[i_2+2][i_3+2] = C[i_2][i_3] + B[i_2+3][i_3+3] - B[i_2+2][i_3+1] * B[i_2+1][i_3+3] + 4;
        }
    }
#pragma endscop
...
kernel_3216212313_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
