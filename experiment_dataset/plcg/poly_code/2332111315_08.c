...
void kernel_2332111315_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 2; i_2 < min(min(-i_1+PB_M+1, i_1), PB_N+1); i_2++) {
            C[i_2+i_1-1][i_2-2] = A[i_2-1] - 5;
        }
        for (int i_3 = 2; i_3 < -i_1+PB_M-1; i_3++) {
            C[i_3+i_1][i_3-1] = D[i_1-1] * C[i_3+i_1-1][i_3-2] * C[i_3+i_1+1][i_3-1] * C[i_3+i_1][i_1] * C[i_3+i_1-1][i_1-1] + 6;
        }
    }
#pragma endscop
...
kernel_2332111315_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
