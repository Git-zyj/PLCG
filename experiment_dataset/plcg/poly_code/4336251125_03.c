...
void kernel_4336251125_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                for (int i_3 = 0; i_3 < i_1; i_3++) {
                    B[i_0][i_2+i_1][i_1-1] = D[i_3][i_2][i_1-1][i_0] * D[i_3][i_2][i_1+i_0][i_2] * E[i_3][i_1+i_0][i_3+1][1] + 4;
                }
            }
        }
        for (int i_4 = 1; i_4 < -i_0+PB_M; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_0+PB_N+1, PB_M); i_5++) {
                B[i_5][i_4+i_0][i_0-1] = F[i_5][i_5-1][i_0-1] - G[i_5][i_0] - H[i_4-1][i_0][i_5-1] - F[i_4+1][i_4+i_0][i_5] - F[i_5+i_0-1][i_4][i_5] + F[i_5+1][i_4][0] + G[i_5][i_5-1] - B[i_4][i_5+i_0-1][i_0-1] - 4;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M+1; i_6++) {
        for (int i_7 = i_6; i_7 < PB_P-2; i_7++) {
            for (int i_8 = 0; i_8 < min(-i_7+PB_P-2, -i_6+PB_L); i_8++) {
                B[i_6][i_8+i_7+2][i_7-1] = F[i_7-1][i_6-1][i_6] - G[i_7][i_8+i_6] - H[i_8+1][i_6-1][i_7] + 4;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4336251125_03(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
