...
void kernel_2234112321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            A[i_0-2] = A[i_1-1] - B[i_0-2][i_0] + 4;
        }
    }
    for (int i_2 = 4; i_2 < PB_M; i_2++) {
        B[i_2-2][i_2] = C[i_2] * C[i_2-2] - B[i_2-4][i_2-2] - B[i_2][i_2+2] * B[i_2-1][i_2+1] - B[i_2-3][i_2-1] + 5;
    }
#pragma endscop
...
kernel_2234112321_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
