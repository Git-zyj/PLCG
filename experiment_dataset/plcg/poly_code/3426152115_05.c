...
void kernel_3426152115_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - E[i_0] * E[i_0+1] - E[i_0] - E[i_0-2] * E[0] * A[i_0+1] * 5;
    }
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        for (int i_2 = i_1; i_2 < min(-i_1+PB_M, PB_Q+1); i_2++) {
            A[i_1+1] = E[i_2+i_1] + E[i_2] * E[i_2] - 4;
            for (int i_3 = 0; i_3 < min(min(PB_Q, -i_1+PB_P), i_1); i_3++) {
                C[i_2][i_1][i_3] = F[i_1][i_3][i_3] * F[i_2][i_3+i_1][i_2-1] + G[i_2][2] * G[i_3+2][0] - C[i_2-1][i_1-1][i_3+1] * 3;
            }
        }
        for (int i_4 = 1; i_4 < -i_1+PB_L-2; i_4++) {
            A[i_4-1] = G[i_1][i_4+i_1+2] * G[i_4+i_1][0] * E[i_4] * G[i_1-1][i_4+2] + 5;
        }
    }
#pragma endscop
...
kernel_3426152115_05(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
