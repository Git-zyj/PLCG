...
void kernel_4332112221_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_1][i_0+1] = D[i_1][i_0] * 4;
        }
    }
    ...
    for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 2; i_4 < -i_3+PB_L-1; i_4++) {
            for (int i_5 = 2; i_5 < PB_P-2; i_5++) {
                C[i_3][i_5+i_4-1][i_5] = C[i_5-2][i_4+i_3+1][i_3+2] + C[i_3][i_5+i_4+1][i_5+2] + C[i_3][i_5+i_4][i_5] - C[i_3][i_5+i_4-2][i_5] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4332112221_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
