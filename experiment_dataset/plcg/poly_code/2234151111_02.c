...
void kernel_2234151111_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M-1; i_1++) {
            A[i_1-1][i_0] = C[i_1][i_0] + C[i_0][i_1] - C[i_1][i_0] * C[i_1+1][i_0+1] - C[i_1-1][i_0+1] * A[i_1-1][i_0+1] - A[i_1-2][i_0] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_2][i_3-1] = C[i_2][i_3] - C[i_2+1][i_3] * C[i_2][i_3] + D[i_2-1] * C[i_3-1][i_3-1] + 5;
        }
    }
#pragma endscop
...
kernel_2234151111_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
