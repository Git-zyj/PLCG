...
void kernel_4426332315_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
                for (int i_3 = PB_Q - 1; i_3 >= 1; i_3--) {
                    A[i_0][i_2][i_1-2][i_3-1] = E[i_2+2][i_1][i_3][i_0] * F[i_2][i_3][i_1+2] + G[i_0][i_3][i_1][i_1] * E[i_2][i_3][i_1][i_2] - H[i_2-2][i_3][i_1+1][i_0] - A[i_0][i_1][i_2-2][i_3] + A[i_0][i_3-1][i_1-2][i_2] + 5;
                }
            }
        }
        for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
            B[i_0+2][i_4] = I[i_4][i_4-2] * B[i_4+2][i_0-1] + B[i_0+2][i_4-1] - C[i_4+1] + 1;
        }
        C[i_0+1] = J[i_0+2] * K[i_0] - C[i_0+2] * C[i_0] * C[i_0] - 6;
    }
    for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 2; i_6 < PB_M; i_6++) {
            D[i_6] = I[i_5+1][i_6] * L[i_6-2][i_5] - J[i_6-2] - D[i_6+1] + D[i_6-1] - 3;
        }
    }
#pragma endscop
...
kernel_4426332315_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
