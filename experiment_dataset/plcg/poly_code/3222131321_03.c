...
void kernel_3222131321_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_Q, -i_0+PB_P-2); i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_2][i_1-1][i_1+i_0+1] = C[i_2+1][i_0-1][i_1] - A[i_2+2][i_1-1][i_1+i_0+2] + A[i_2][i_1-3][i_1+i_0-1] - 3;
            }
        }
        for (int i_3 = 3; i_3 < min(PB_Q, -i_0+PB_P); i_3++) {
            for (int i_4 = 1; i_4 < PB_Q; i_4++) {
                A[i_0][i_3-3][i_4+i_3-1] = C[i_0][i_3-1][0] * D[i_4][i_3+1] + C[i_4][i_0-1][i_3] - C[i_4-1][i_3][i_0] - A[i_4][i_3-2][i_3+i_0] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3222131321_03(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
