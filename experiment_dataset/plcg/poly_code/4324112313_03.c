...
void kernel_4324112313_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_0][i_2][i_1] = D[i_0-2][i_2][i_1] * A[i_0+1][i_1][i_2] - A[i_0][i_2][i_1-1] * A[i_2][i_0][i_1+1] + A[i_0][i_2-1][i_1] + 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 3; i_4 < PB_M+1; i_4++) {
            B[i_3][i_4-2] = B[i_3][i_4-3] + B[i_4-1][i_3-2] - 1;
        }
        for (int i_5 = 0; i_5 < PB_M; i_5++) {
            for (int i_6 = i_3; i_6 < PB_P; i_6++) {
                C[i_3][i_5+2][i_6] = D[i_5][i_3][i_3-1] + C[i_3-1][i_5+3][i_6-1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4324112313_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
