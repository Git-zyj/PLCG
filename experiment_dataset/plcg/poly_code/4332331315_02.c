...
void kernel_4332331315_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = D[i_0] - D[i_0+1] + A[i_0] - B[i_0] - 2;
        B[i_0] = E[i_0-1] * A[i_0+1] * A[i_0+2] + A[i_0] + 6;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_L-1, PB_M); i_2++) {
            C[i_1][i_2+1][i_2+1] = F[i_2][i_1] + G[i_2+i_1+1][1] + F[i_2-1][i_1] + H[i_2][i_1-1] - F[i_1+1][i_2-1] * C[i_1][i_2+2][i_2+2] - C[i_2][i_1+2][i_1+2] - C[i_1-1][i_2][i_2] - C[i_1+1][i_2+1][i_2+1] * 5;
        }
    }
#pragma endscop
...
kernel_4332331315_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
