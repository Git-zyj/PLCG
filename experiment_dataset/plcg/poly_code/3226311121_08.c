...
void kernel_3226311121_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = min(PB_N-1, -i_0+PB_L) - 1; i_2 >= i_0; i_2--) {
            for (int i_3 = 0; i_3 < -i_2+PB_N-1; i_3++) {
                B[i_3+i_2+1][i_0+1][i_2+i_0] = 2;
            }
        }
    }
#pragma endscop
...
kernel_3226311121_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
