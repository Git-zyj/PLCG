...
void kernel_2326152215_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0-1] - D[2] - D[i_0] + D[i_0] + D[i_0] * D[i_0-2] + A[i_0-1] - 2;
    }
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            B[i_1][i_2] = E[i_1][i_2] - E[i_2][i_1-1] + E[i_1][i_2-1] + E[i_2-1][i_1] + E[i_2][i_1+1] + B[i_1][i_2-1] * B[i_1+1][i_2-1] - 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_P, -i_3+PB_L+1); i_4++) {
            A[i_4] = D[i_3] + D[i_3] + E[i_4][i_3] * E[i_4+i_3-1][0] * 3;
        }
    }
#pragma endscop
...
kernel_2326152215_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
