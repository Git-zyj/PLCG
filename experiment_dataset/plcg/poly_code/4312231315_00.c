...
void kernel_4312231315_00(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                B[i_1][i_0-1][i_0][i_2] = B[i_0][i_1][i_1+1][i_2] * 1;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_3+PB_P, PB_L-1); i_5++) {
                for (int i_6 = 1; i_6 < PB_Q-1; i_6++) {
                    B[i_3][i_5][i_5+1][i_6] = D[i_6][i_5][i_4][i_5+i_3] - E[i_6-1][i_5][i_4+1][i_5] + B[i_3][i_5+1][i_5+2][i_6] - B[i_3][i_4+1][i_4+2][i_6] + B[i_3][i_6][i_6+1][i_5-1] * B[i_3-1][i_5][i_5+1][i_6] + B[i_3+1][i_5][i_5+1][i_6+1] + 1;
                }
            }
        }
        for (int i_7 = 1; i_7 < -i_3+PB_L-1; i_7++) {
            C[i_7-1][i_3-1] = F[i_7][i_7+i_3+1][i_3-1] + G[i_7][i_7+1] + G[i_7][i_3] - H[i_7][i_3] - G[i_7+i_3][0] - 5;
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4312231315_00(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
