...
void kernel_4334131223_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0+1] = D[i_0-1] - D[1] + D[i_0] - 4;
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q, PB_M); i_1++) {
            A[i_0+1] = E[i_0+1][i_1][i_1+i_0] - F[i_0][i_1] + D[i_1] * A[i_1+1] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_M-2; i_3++) {
            C[i_3] = F[i_3-1][i_2+1] + C[i_3+1] - C[i_3+2] - 4;
        }
    }
#pragma endscop
...
kernel_4334131223_05(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
