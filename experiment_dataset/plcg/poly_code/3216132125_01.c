...
void kernel_3216132125_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(min(PB_P-2, -i_0+PB_N), i_1); i_2++) {
                B[i_1][i_2] = C[i_0+2][i_1][i_2+2] - D[i_2+i_0][i_1] + C[i_2+1][i_1+i_0][1] - 4;
            }
            B[i_0][i_1] = E[i_1] + E[i_0] - E[i_0-1] * B[i_0-1][i_1] - 5;
        }
    }
#pragma endscop
...
kernel_3216132125_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
