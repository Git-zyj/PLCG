...
void kernel_3426151115_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        B[i_0] = E[i_0][0] * F[i_0] * F[0] * F[i_0+1] * 5;
        B[i_0-1] = F[i_0] - F[1] * F[i_0] - F[i_0] + F[i_0] - 3;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        D[i_1-1] = F[0] * F[i_1] + F[i_1-1] - F[i_1] - F[i_1] - 5;
        D[i_1] = F[i_1] * F[i_1] * F[i_1-1] + F[i_1] * D[i_1-1] * 4;
    }
#pragma endscop
...
kernel_3426151115_06(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
