...
void kernel_2426112311_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1+2][i_1-1] = A[i_1+3][i_1] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        B[i_2+2] = E[i_2+1] - B[i_2+3] * B[i_2+1] + B[i_2+1] - 4;
        for (int i_3 = i_2; i_3 < -i_2+PB_L-1; i_3++) {
            C[i_2+2][i_3+i_2] = C[i_2+1][i_3+i_2-1] - C[i_2+2][i_3+i_2-1] * C[i_2+3][i_3+i_2+1] + 5;
        }
    }
...
kernel_2426112311_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
