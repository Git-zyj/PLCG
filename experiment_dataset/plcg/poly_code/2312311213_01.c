...
void kernel_2312311213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0+1] = A[i_0] - C[i_0][i_0] * 2;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        C[i_1][i_1] = A[i_1+1] * 1;
        C[i_1][i_1] = D[i_1] + A[i_1+1] * 5;
    }
#pragma endscop
...
kernel_2312311213_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
