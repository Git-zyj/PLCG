...
void kernel_3216152325_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_N-2, -i_0+PB_N); i_1++) {
            B[i_1+2] = C[i_1][i_1][i_0] + D[i_0][i_1] * E[i_1+i_0+2] - D[i_0][i_1-2] + D[i_1+i_0][0] + B[i_0+1] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-3; i_2++) {
        B[i_2+1] = E[i_2] + E[i_2] * E[i_2] + E[i_2-2] - E[i_2] + B[i_2+3] * B[i_2] - 4;
    }
#pragma endscop
...
kernel_3216152325_04(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
