...
void kernel_4422232225_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < min(-i_0+PB_Q, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_M, -i_0+PB_L); i_2++) {
                A[i_1-1][i_2+1] = E[i_0][i_2][i_1] * F[i_1][i_2][i_1+i_0] * G[i_2+i_0][i_1] * F[i_0][i_2][2] - A[i_2][i_0+1] + A[i_1+1][i_2-1] + A[i_1-3][i_2+1] * 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            A[i_3-1][i_4+1] = H[i_3][i_4] - I[i_4] + 3;
        }
    }
    for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
        C[i_5] = J[i_5] + I[i_5] + J[i_5+1] * 5;
        C[i_5] = I[i_5] + C[i_5] - 4;
    }
#pragma endscop
...
kernel_4422232225_03(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
