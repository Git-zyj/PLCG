...
void kernel_2316131223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_L-1, -i_0+PB_L+1); i_1++) {
            A[i_1+1][i_1] = D[i_1+i_0-1][i_1+1] - D[i_0][i_1] * A[i_1][i_1-1] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            B[i_2][i_3] = D[i_2][i_3] - D[i_2][1] - D[i_3][i_2] - B[i_2-2][i_3] * B[i_2+1][i_3-1] * 3;
        }
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            C[i_2][i_4] = D[i_2-1][i_4-1] * D[i_4][i_2+1] * C[i_2+2][i_4] + 1;
        }
    }
#pragma endscop
...
kernel_2316131223_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
