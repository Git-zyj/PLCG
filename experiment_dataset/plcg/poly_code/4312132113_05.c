...
void kernel_4312132113_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_1+i_0][i_1] = D[i_1-2][i_0-1] - A[i_1+i_0-1][i_1] * 1;
        }
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                C[i_0][i_3][i_2-1] = E[i_3][i_0][i_2-2] - D[i_3][0] + 4;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_L-2); i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_L-2; i_6++) {
                C[i_4][i_5][i_6] = D[i_5+1][i_5] + E[i_5][i_4-2][i_6] * E[i_6+i_5+2][i_4][i_6-1] + E[i_6][i_4][i_5-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4312132113_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
