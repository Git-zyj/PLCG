...
void kernel_2336111321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            A[i_1+i_0][i_0+1] = A[i_1+i_0-1][i_0] * A[i_1+i_0+1][i_0+2] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            C[i_3-1] = C[i_3-1] + C[i_3] + C[i_2-1] + 5;
        }
        C[i_2-1] = D[i_2] - 4;
    }
#pragma endscop
...
kernel_2336111321_06(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
