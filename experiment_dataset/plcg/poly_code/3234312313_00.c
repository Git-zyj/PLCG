...
void kernel_3234312313_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M+2; i_2++) {
                B[i_0-2][i_2-2] = C[i_1][i_0][i_1] - 3;
            }
        }
        for (int i_3 = 3; i_3 < i_0; i_3++) {
            B[i_0-2][i_3-2] = B[i_0-1][i_3-2] * B[i_0-1][i_3-3] - B[i_3-1][i_0-1] - 6;
        }
    }
#pragma endscop
...
kernel_3234312313_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
