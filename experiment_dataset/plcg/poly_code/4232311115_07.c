...
void kernel_4232311115_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L-1; i_1++) {
            A[i_1][i_1+i_0+1] = C[i_1-1] - A[i_1-1][i_1+i_0] - 1;
        }
    }
        ...
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_2+PB_L, PB_M); i_4++) {
                for (int i_5 = 0; i_5 < min(-i_3+PB_L, PB_P-2); i_5++) {
                    B[i_4][i_5+i_3][i_2+1] = B[i_3-1][i_4+i_2][i_5+2] + 5;
                }
            }
        }
    }
...
kernel_4232311115_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
