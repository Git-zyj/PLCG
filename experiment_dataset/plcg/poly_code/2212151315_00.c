...
void kernel_2212151315_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_N-1, -i_0+PB_P); i_1++) {
            A[i_1] = C[i_0] + D[i_0-1][i_1] + D[i_1+i_0][i_0] * D[i_1+i_0+1][1] + D[i_1-1][i_0] + D[i_0][i_1+i_0] * A[i_1+1] - A[i_1-1] * A[i_1+1] + 2;
        }
        for (int i_2 = 0; i_2 < min(PB_M, -i_0+PB_L); i_2++) {
            A[i_0-1] = C[i_2] + D[i_2+i_0][i_0] * D[i_0][i_2] + 4;
        }
    }
#pragma endscop
...
kernel_2212151315_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
