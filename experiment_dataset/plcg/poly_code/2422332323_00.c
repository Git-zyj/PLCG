...
void kernel_2422332323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = E[2] + F[i_0] - D[i_0] * D[i_0-2] * 1;
    }
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
            B[i_1-1][i_1-1] = G[i_2+2][i_1+1] - H[i_2+2][i_1][i_2+1] - B[i_1][i_1] + B[i_1+1][i_1+1] + 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_Q; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_L, PB_P); i_4++) {
            A[i_3+2] = I[i_4+i_3][i_3] - J[i_3+2][i_4] * B[i_3-1][i_3-1] + D[i_4] * 3;
        }
        for (int i_5 = 1; i_5 < min(PB_P, -i_3+PB_M-1); i_5++) {
            D[i_5] = J[i_5][i_3] * J[i_5+i_3][i_3] - F[i_5] - E[i_5+i_3+1] * G[i_3][i_5] + D[i_5-1] + 4;
        }
    }
#pragma endscop
...
kernel_2422332323_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
