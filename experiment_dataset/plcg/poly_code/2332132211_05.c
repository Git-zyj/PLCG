...
void kernel_2332132211_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L+2; i_1++) {
            A[i_1] = D[i_0][i_1] * D[i_1][i_1+i_0-2] - A[i_1-1] - A[i_0+1] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            B[i_3+1] = D[i_3][i_2+2] - D[i_2-2][i_3] - B[i_3+2] + 2;
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_L); i_5++) {
            C[i_5][i_4-2] = D[i_5][i_4+2] * D[i_5][i_5+i_4] + D[i_5][i_4] - D[i_5-1][i_4] * B[i_5+1] - C[i_4+1][i_5-1] * 1;
        }
    }
#pragma endscop
...
kernel_2332132211_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
