...
void kernel_4426352215_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0-1] + F[i_0] + E[1] * G[i_0+1][0] + H[0] + H[i_0] - F[i_0+2] - A[i_0+1] - D[i_0-1] + 2;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-2; i_2++) {
            for (int i_3 = i_2; i_3 < -i_1+PB_L; i_3++) {
                B[i_2+2][i_3-1] = I[i_2][i_3+i_1][i_3] - J[i_3][i_2] * K[i_1][i_2][i_3] * L[i_1-2][i_3][i_2+1] + B[i_2+1][i_3-2] - B[i_2+1][i_3-1] * 5;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_L; i_5++) {
            A[i_5] = J[i_5-2][i_4] - G[i_5][i_4-2] * G[i_5-1][i_4] * M[i_5][i_4+1] * 4;
        }
        for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
            D[i_6-1] = G[i_4+1][i_6-1] * H[i_6+2] * E[i_6] - G[i_6-2][i_4] + M[i_6][i_4] * D[i_6-2] + D[i_6] * 4;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4426352215_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M));
