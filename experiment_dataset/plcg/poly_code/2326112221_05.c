...
void kernel_2326112221_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        A[i_1] = 2;
        C[i_1][i_1] = D[i_1] + C[i_1+2][i_1+2] + C[i_1+1][i_1+1] - C[i_1-1][i_1-1] - 1;
    }
#pragma endscop
...
kernel_2326112221_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
