...
void kernel_3212112213_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = A[i_0+1] * 6;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = i_1; i_2 < min(min(PB_M+1, -i_1+PB_N), -i_0+PB_N+1); i_2++) {
                B[i_2+i_1][i_0] = C[i_2-1][i_0][i_1] + B[i_2+i_1][i_0-1] + B[i_2+i_0-1][i_1-1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3212112213_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
