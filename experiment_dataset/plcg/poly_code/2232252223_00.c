...
void kernel_2232252223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N+2; i_0++) {
        for (int i_1 = PB_N - 1; i_1 >= 2; i_1--) {
            A[i_0-2][i_1-2] = C[i_0-2][i_0] * D[i_1] + C[i_1][i_0] * D[i_0+2] + C[i_1][i_0] + E[i_1][i_0] + C[i_1][i_0+1] - E[i_1-2][i_0-1] + A[i_0-1][i_1-2] * A[i_1-2][i_0-3] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < PB_M; i_3++) {
            A[i_3-3][i_2-2] = E[i_3][i_2] * 1;
        }
    }
#pragma endscop
...
kernel_2232252223_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
