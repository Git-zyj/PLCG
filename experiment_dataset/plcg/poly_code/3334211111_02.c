...
void kernel_3334211111_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_Q, -i_3+PB_M-1); i_4++) {
            B[i_4+i_3+1][i_4+i_3] = D[i_4][i_3] + A[i_3][i_4][i_4] - 2;
            for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
                C[i_3][i_5][i_4] = C[i_3][i_5+1][i_4-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3334211111_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
