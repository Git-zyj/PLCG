...
void kernel_3236131325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_0][1] = C[i_1][i_1] - C[i_0][i_1] * C[i_0][i_1] + C[i_0][i_1] * A[i_0+1][1] * A[i_0+2][1] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, i_2); i_4++) {
                A[i_3][1] = C[i_2][i_4+i_3] * C[i_4][i_3+1] - A[i_3][1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3236131325_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
