...
void kernel_3336111123_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_P; i_1++) {
            A[i_0][i_0+1] = A[i_1-1][i_1] * 4;
        }
        for (int i_2 = i_0; i_2 < PB_N; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_2+PB_M-3, -i_0+PB_N); i_3++) {
                B[i_3+i_2+1][i_0+1][0] = D[i_2][i_3+i_0][i_2] + B[i_3+i_2+3][i_0+1][0] + 3;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < PB_N+1; i_6++) {
                C[i_4][i_5+1][i_6-1] = D[i_4][i_6-1][i_5] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3336111123_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
