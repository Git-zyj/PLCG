...
void kernel_3226252225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            A[i_1] = C[i_1+1][i_1] * C[i_0-2][i_1-1] + C[i_1][i_0] * C[i_1][i_0] - C[i_1+1][i_0-2] + D[i_1][i_1+1] * C[i_1][0] - A[i_1+2] + 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        A[i_2-2] = E[i_2-2] - E[i_2] + A[i_2] * 4;
    }
#pragma endscop
...
kernel_3226252225_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
