...
void kernel_2214311113_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            A[i_1][i_0] = C[i_0+1][i_1] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_L, i_2); i_3++) {
            A[i_3+1][i_2-1] = A[i_2][i_3] + 5;
        }
    }
#pragma endscop
...
kernel_2214311113_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
