...
void kernel_3334112315_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        A[i_0+1] = A[i_0+2] + A[i_0] - A[i_0] * A[i_0] - 3;
        for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                B[i_2][i_0][i_1-1] = D[i_1][i_2][i_0+1] + B[i_2+1][i_0][i_1-1] + B[i_2-1][i_0][i_1-1] * 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L-2; i_4++) {
            for (int i_5 = 0; i_5 < -i_3+PB_P; i_5++) {
                C[i_3+1][i_4+2][i_5+i_3] = C[i_3][i_4+1][i_5+i_3-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3334112315_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
