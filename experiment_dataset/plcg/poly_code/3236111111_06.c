...
void kernel_3236111111_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_Q); i_1++) {
            for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
                B[i_2-1][i_0][i_1] = C[i_1+i_0][i_2-1] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_N-1, PB_P-1); i_5++) {
                B[i_5][i_3][i_4] = D[i_5+i_3+1][i_4][i_5+1] * B[i_5][i_3+1][i_4-1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3236111111_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
