...
void kernel_3316312223_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = -i_1+PB_P+2 - 1; i_2 >= 0; i_2--) {
                B[i_2+i_1-2][i_2+i_1-2] = 5;
            }
            B[i_1+i_0][i_1+i_0] = 4;
        }
    }
    for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 2; i_4 < -i_3+PB_P-2; i_4++) {
            C[i_4] = D[i_4][i_3+2] * B[i_4+i_3+2][i_4+i_3+2] * C[i_4+1] * C[i_4-2] * 2;
        }
    }
#pragma endscop
...
kernel_3316312223_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
