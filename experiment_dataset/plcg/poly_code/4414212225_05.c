...
void kernel_4414212225_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[i_0] - F[i_0+2][0] * A[i_0-1] - A[i_0+2] * 5;
        for (int i_1 = 3; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_L; i_3++) {
                    D[i_2-1][i_3+i_1][i_1][i_0-3] = 2;
                }
            }
            for (int i_4 = -i_0+PB_L - 1; i_4 >= i_1; i_4--) {
                for (int i_5 = 0; i_5 < -i_1+PB_L-2; i_5++) {
                    D[i_5+1][i_4+i_0][i_4][i_1-3] = 5;
                    D[i_4][i_5+i_1][i_1][i_0-1] = G[i_4][i_1][i_1-2] * H[i_5][i_1][i_4][i_0] + D[i_0][i_5+i_1+2][i_1+2][i_4+1] + D[i_4-1][i_5+i_1-1][i_1][i_0-1] - 4;
                }
            }
        }
    }
...
kernel_4414212225_05(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
