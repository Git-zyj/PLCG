...
void kernel_2226232311_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_1-1] = A[i_1] * A[i_1-2] * A[i_1] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            B[i_3][i_2] = C[i_3][0] * C[i_2][i_3] - C[i_3][i_2] + C[i_3][i_2] - D[i_2-1][0] * B[i_3+1][i_2] - B[i_3][i_2+1] * 3;
        }
    }
#pragma endscop
...
kernel_2226232311_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
