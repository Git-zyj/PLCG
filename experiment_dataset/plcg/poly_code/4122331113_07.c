...
void kernel_4122331113_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_L, -i_0+PB_P); i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                A[i_1-1][i_2] = B[i_1-1][i_0-1][i_2] - C[i_2-1][i_1+i_0] + D[i_2-1][i_1][i_0] * A[i_1][i_2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4122331113_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
