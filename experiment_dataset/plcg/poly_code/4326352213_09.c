...
void kernel_4326352213_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,int wj,DATA_TYPE POLYBENCH_5D(J,xJ,yJ,zJ,vJ,wJ,xj,yj,zj,vj,wj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(min(PB_L-3, i_0), -i_0+PB_L-1); i_1++) {
            A[i_0][i_1+2] = D[i_1+i_0+1][0] - D[i_1][i_0] - A[i_0-1][i_1+2] * A[i_0][i_1+3] + 1;
        }
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            for (int i_3 = i_0; i_3 < -i_2+PB_P; i_3++) {
                A[i_3-1][i_2+2] = E[i_2-1][i_0][i_3+i_2] - E[i_3][i_0][i_2] * F[i_2+1][i_0-1] * 5;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < i_4; i_5++) {
            for (int i_6 = 2; i_6 < -i_4+PB_P; i_6++) {
                for (int i_7 = 0; i_7 < min(min(-i_4+PB_M, i_5), -i_5+PB_M+1); i_7++) {
                    A[i_7][i_6+1] = G[i_6-2][i_6][i_4][i_5] * H[i_7+i_5-1][i_4][i_6][0] + I[i_5][i_4-2][i_5-2][i_6] * E[i_6][i_5][i_5] * H[i_7][i_6+i_4][i_5][i_5-2] + H[i_7][i_5][i_7+i_6][i_4] + J[i_7+i_4][i_5][i_6][i_5-2][i_6] * I[i_5][i_4][i_7][1] - 1;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4326352213_09(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J)));
