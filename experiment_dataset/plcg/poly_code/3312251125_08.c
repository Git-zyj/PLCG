...
void kernel_3312251125_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0][i_0+1] = D[i_0+1] - D[i_0] - D[i_0-1] * D[i_0] + D[i_0] + E[i_0+1] - A[i_0-2][i_0-1] + 4;
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[i_0-2][i_0-1] = F[i_1][i_0] + G[0][i_1] * G[i_1][i_0+1] - D[i_1+i_0] * F[i_1][i_0] + F[i_1][i_1+i_0] * D[i_0] + 3;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                A[i_0][i_0+1] = G[i_2-1][i_0] - H[i_2][i_0+1][i_3] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3312251125_08(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
