...
void kernel_2122112311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M - 1; i_0 >= 0; i_0--) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_M-3); i_1++) {
            A[i_1+2] = B[i_1+i_0][0] * A[i_1+3] * A[i_1+1] + A[i_1+1] - 6;
        }
    }
#pragma endscop
...
kernel_2122112311_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
