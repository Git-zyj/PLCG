...
void kernel_3332132323_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_0][i_2][i_0] = D[i_0][i_2][i_0] - D[i_2][i_0][i_1+1] + E[i_1][i_2+2] + A[i_0-1][i_2][i_0-1] * A[i_0-2][i_2][i_0-2] * A[i_0-2][i_2+2][i_0-2] + A[i_0+1][i_1][i_0+1] * A[i_0+1][i_1-2][i_0+1] + 4;
            }
        }
        for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_M; i_4++) {
                B[i_3+2][i_0-2][0] = D[i_3-2][i_0-1][i_4+2] + D[i_4+i_3][i_0][i_3] * B[i_3][i_0-1][0] + B[i_3+2][i_4][0] + 1;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_L+2; i_5++) {
        for (int i_6 = 1; i_6 < -i_5+PB_L+2; i_6++) {
            for (int i_7 = 0; i_7 < PB_L; i_7++) {
                C[i_5][i_7][i_6-1] = D[i_7][i_6+i_5-2][0] * D[i_6-1][i_7][i_5-2] + C[i_5+2][i_7][i_6+1] - C[i_5][i_7][i_6] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3332132323_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
