...
void kernel_4232112225_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q+2; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q+2; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M; i_2++) {
                A[i_2][i_1][i_0-2] = A[i_2][i_1][i_0-3] - A[i_2-1][i_0][i_1-2] - 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
        for (int i_4 = 1; i_4 < PB_P+1; i_4++) {
            for (int i_5 = 3; i_5 < PB_L-1; i_5++) {
                A[i_3][i_4-1][i_5-3] = C[i_5+1][i_3-2][i_3] - C[i_4][i_5+1][i_3] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4232112225_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
