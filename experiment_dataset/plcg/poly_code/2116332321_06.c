...
void kernel_2116332321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(i_0, 4); i_1 < PB_M-2; i_1++) {
            A[i_1][i_1-2] = B[i_1+2][i_0-2] - B[i_1+1][i_0+1] * B[i_1+1][i_0+1] * A[i_1+1][i_1-1] * A[i_1-2][i_1-4] - A[i_1+2][i_1] + 6;
        }
    }
#pragma endscop
...
kernel_2116332321_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
