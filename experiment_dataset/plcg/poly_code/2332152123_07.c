...
void kernel_2332152123_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0+2] = D[i_0] + D[i_0-2] + E[1][i_0+1] * D[i_0] * E[i_0][i_0] - D[0] - 2;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        C[i_1+1] = D[i_1] * D[i_1] * D[i_1] * D[0] - 6;
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            C[i_2+1] = D[i_3] * D[i_3] + D[i_3] - A[i_3+2] * 2;
        }
    }
#pragma endscop
...
kernel_2332152123_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
