...
void kernel_2314211311_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            B[i_0+1][i_0] = B[i_1+1][i_1] - B[i_0+1][i_0] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        B[i_2+1][i_2] = D[i_2] + 3;
        for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
            C[i_2][i_3] = C[i_2-1][i_3+1] * C[i_2][i_3+1] + C[i_2+1][i_3-1] + 6;
        }
    }
#pragma endscop
...
kernel_2314211311_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
