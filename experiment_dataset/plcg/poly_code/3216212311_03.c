...
void kernel_3216212311_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        ...
        for (int i_2 = 0; i_2 < min(PB_M, i_0); i_2++) {
            B[i_0][i_2] = C[i_0-2][i_2+1] + C[i_2][i_0] - B[i_0][i_2+1] - 4;
        }
    }
#pragma endscop
...
kernel_3216212311_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
