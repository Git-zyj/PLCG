...
void kernel_4226311215_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        ...
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            for (int i_3 = i_0; i_3 < PB_L; i_3++) {
                B[i_0][i_3][i_2] = C[i_3-1][i_0][i_2-1] - D[i_2][i_3][i_0] + A[i_3-1][i_2] - B[i_3][i_0+1][i_2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4226311215_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
