...
void kernel_3312111325_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        C[i_3] = D[1] + D[i_3] * C[i_3-2] - C[i_3+1] - C[i_3-1] - C[i_3+2] * 3;
    }
#pragma endscop
...
kernel_3312111325_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
