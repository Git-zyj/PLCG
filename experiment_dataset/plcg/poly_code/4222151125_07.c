...
void kernel_4222151125_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_Q; i_2++) {
                for (int i_3 = 0; i_3 < min(min(PB_P-1, -i_1+PB_P), -i_0+PB_P); i_3++) {
                    A[i_0][i_2][i_3+i_1] = C[i_2+1][i_0+1][i_3+1][i_0] - C[i_1][i_2-1][i_3+i_0][i_2+i_1] + 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < PB_P; i_5++) {
            for (int i_6 = 1; i_6 < PB_M-2; i_6++) {
                for (int i_7 = 1; i_7 < min(min(-i_5+PB_P, -i_4+PB_P), PB_M-2); i_7++) {
                    B[i_7][i_6][i_5] = C[i_6][i_4][i_5][i_7-1] - D[i_4][i_5-1][i_6] * D[i_6-1][i_7+i_5][i_5] + D[i_6+1][i_7-1][i_5] + C[i_5][i_7+i_4][i_5][i_6+1] * C[i_4+1][i_7][i_7][i_5] * C[i_5+1][i_4][i_6][i_7] - B[i_7+2][i_6+2][i_5] - B[i_6+2][i_7][i_5] * 6;
                }
            }
        }
    }
...
kernel_4222151125_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
