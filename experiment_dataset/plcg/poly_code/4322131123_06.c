...
void kernel_4322131123_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_L; i_3++) {
                    A[i_0+1][i_1+1][i_2][i_3] = D[i_2+1][i_0][i_0][i_1] + E[i_2][i_1-1][i_3+i_1] * D[i_3][i_0][i_1][i_2] - 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            B[i_5-1] = F[i_4][i_5] - F[i_5][i_4-1] + G[i_5] * 5;
        }
        for (int i_6 = 0; i_6 < PB_M; i_6++) {
            for (int i_7 = 3; i_7 < PB_P-1; i_7++) {
                for (int i_8 = 1; i_8 < PB_L; i_8++) {
                    C[i_8-1][i_4-1][i_6][i_7-1] = D[i_6][i_8-1][0][i_7+1] - C[i_8-1][i_4+1][i_6][i_7-1] * C[i_4-1][i_8-1][i_6][i_7-3] + 5;
                }
            }
        }
    }
...
kernel_4322131123_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
