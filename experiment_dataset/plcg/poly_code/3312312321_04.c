...
void kernel_3312312321_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_0+3][i_1] = D[i_0][i_1][i_1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_Q-2; i_3++) {
            B[i_2+1][i_3] = E[i_2][i_2] + 4;
            for (int i_4 = 0; i_4 < PB_Q-2; i_4++) {
                C[i_3][i_2][i_4] = C[i_3][i_2+2][i_4+2] - C[i_3-2][i_2-1][i_4] - C[i_3-1][i_2][i_4] - C[i_4][i_2][i_3+2] + C[i_3][i_2][i_4+1] * C[i_3][i_2][i_4+2] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3312312321_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
