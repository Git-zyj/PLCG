...
void kernel_3316111221_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_Q, PB_L-2); i_1++) {
            for (int i_2 = 3; i_2 < PB_P+1; i_2++) {
                A[i_0][i_1][i_2-1] = D[i_2][i_2-1][i_1+i_0] * D[i_0-1][i_2][i_1+1] - A[i_0+2][i_1][i_2-3] - A[i_0+1][i_1+2][i_2-1] - B[i_1] + 1;
            }
            B[i_1-1] = 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < min(-i_3+PB_M, PB_P+2); i_4++) {
            for (int i_5 = i_4; i_5 < min(-i_3+PB_M, PB_P-1); i_5++) {
                C[i_4+i_3][i_5][i_5] = C[i_4+i_3-1][i_5+1][i_5+1] - C[i_5+i_3][i_4-2][i_4-2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3316111221_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
