...
void kernel_4232112313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
            B[i_2] = C[i_2-2] * B[i_2+1] * B[i_2-1] + B[i_2+1] * 2;
        }
    }
#pragma endscop
...
kernel_4232112313_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
