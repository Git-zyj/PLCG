...
void kernel_4416211321_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
        ...
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = max(2, i_2); i_3 < PB_L; i_3++) {
                    A[i_0][i_1][i_3][i_2] = E[i_1][i_2][i_3][i_0] + A[i_3-2][i_1][i_0][i_2] - 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        B[i_4] = F[i_4][i_4] + B[i_4+2] - 5;
        B[i_4+2] = G[i_4] * B[i_4+1] - 4;
        for (int i_5 = max(i_4, 2); i_5 < min(PB_L, -i_4+PB_P); i_5++) {
            for (int i_6 = 2; i_6 < -i_4+PB_L; i_6++) {
                for (int i_7 = max(i_4, 2); i_7 < PB_L-1; i_7++) {
                    D[i_5][i_4][i_6-1][i_7+1] = H[i_7][i_6+i_4][i_5+i_4][0] * D[i_6-2][i_4][i_5-1][i_7+1] + D[i_7+1][i_4+2][i_6-1][i_5+1] - D[i_5-2][i_4][i_6-1][i_7+2] * D[i_5][i_4-1][i_6][i_7] + D[i_6][i_4+1][i_7-2][i_5-1] * 2;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4416211321_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
