...
void kernel_3216212223_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
            B[i_3][i_4] = C[i_4][i_3] * B[i_3+2][i_4] - B[i_3][i_4+1] * B[i_3-2][i_4-2] - 2;
        }
    }
#pragma endscop
...
kernel_3216212223_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
