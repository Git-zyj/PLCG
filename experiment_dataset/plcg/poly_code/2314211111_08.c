...
void kernel_2314211111_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_N+1); i_1++) {
            A[i_1+i_0] = D[i_1-1][i_0] * A[i_1+i_0-1] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N-1; i_3++) {
            B[i_2][i_3+i_2+1] = E[i_2][i_3] - C[i_2-1] + 2;
        }
        C[i_2] = 6;
    }
#pragma endscop
...
kernel_2314211111_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
