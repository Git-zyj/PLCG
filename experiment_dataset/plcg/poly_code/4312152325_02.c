...
void kernel_4312152325_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_Q; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                for (int i_3 = 2; i_3 < min(min(-i_1+PB_P, PB_L-2), -i_2+PB_P); i_3++) {
                    A[i_1][i_0][i_3][0] = D[i_0][i_3][i_2][i_1] * D[i_1-2][i_3][i_0][i_2] + D[i_2-1][i_3+i_1][i_1+i_0][i_3] * D[i_1-2][i_3+i_2][i_0][i_0] + D[i_2][i_0][i_3+1][2] - D[i_3-2][i_1][i_0-1][i_2-2] - A[i_1][i_0+2][i_2][0] - A[i_3][i_1][i_0+2][0] + A[i_1][i_0+2][i_3+2][0] + 2;
                }
            }
        }
        B[i_0+1] = E[i_0+2] * E[i_0-2] * E[i_0] + E[i_0+2] - B[i_0+3] + B[i_0] * B[i_0+2] + 6;
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_P); i_5++) {
            for (int i_6 = 2; i_6 < PB_L; i_6++) {
                A[i_5][i_4][i_6][0] = F[i_4][i_5+2][i_4+1] - F[i_5][i_5+i_4][i_6] - F[i_6-2][i_4-2][i_5] * F[i_6-2][i_4][i_4-1] * F[i_6][i_4][i_5-1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4312152325_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
