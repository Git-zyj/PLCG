...
void kernel_4316112311_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = max(1, i_0); i_2 < PB_P; i_2++) {
                A[i_1][i_2] = A[i_1+1][i_2-1] - A[i_1-1][i_0] * 2;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_Q; i_3++) {
        ...
        for (int i_4 = 2; i_4 < i_3; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_L-2, -i_4+PB_P); i_5++) {
                C[i_5+i_3+2][i_4-1][i_3] = D[i_5][i_5+i_4] - C[i_5+i_3+1][i_4-1][i_3-1] * C[i_5+i_3+2][i_4-2][i_3] + 3;
            }
        }
    }
#pragma endscop
...
kernel_4316112311_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
