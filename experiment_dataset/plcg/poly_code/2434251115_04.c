...
void kernel_2434251115_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        B[0] = E[i_1] * F[i_1] * E[i_1-1] + F[i_1] - F[i_1] + G[i_1+1][0] - H[i_1][i_1] - F[i_1] + E[i_1+1] + F[i_1+1] - F[i_1] * E[i_1-1] - E[i_1] * B[0] + 6;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_M, -i_2+PB_P); i_3++) {
            B[0] = G[i_3][i_2] + I[i_2][i_3+i_2][1] - 6;
        }
        B[0] = E[i_2] + F[i_2] - 1;
    }
#pragma endscop
...
kernel_2434251115_04(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
