...
void kernel_2312351311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        A[i_0] = D[i_0] + E[i_0-1] + F[i_0-1] + F[i_0-1] + A[i_0+1] + 1;
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            A[i_1] = G[i_1][i_0] + H[i_1][i_0] * G[i_1][i_0-1] * I[i_1-1][i_0+1][i_1] - A[i_1-1] + C[i_1] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        C[i_2] = F[i_2-1] + E[i_2] * E[i_2] - F[i_2] * E[i_2+1] + C[i_2+1] - C[i_2-1] * C[i_2+1] * 6;
    }
#pragma endscop
...
kernel_2312351311_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
