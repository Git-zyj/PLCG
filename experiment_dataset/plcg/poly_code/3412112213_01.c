...
void kernel_3412112213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_0][i_1] = A[i_0][i_1+1] * B[i_0+1][i_1] * 4;
            B[i_0+2][i_1] = A[i_1+1][i_0] + B[i_0+3][i_1-1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        C[i_2+2] = E[i_2] * 1;
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            D[i_2][i_3] = C[i_3+2] - D[i_2][i_3+1] + D[i_2-1][i_3] * 2;
        }
    }
#pragma endscop
...
kernel_3412112213_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
