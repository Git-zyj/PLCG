...
void kernel_3212312221_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_1][i_0+1][i_1+2] = B[i_0-1][i_1+1][i_0+1] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_4][i_2+1][i_4+2] = C[i_2+2][i_4][i_3] + B[i_4][i_2-1][i_4+2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3212312221_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
