...
void kernel_4232212315_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_0+PB_M, PB_N-3), -i_2+PB_M-1); i_3++) {
                    B[i_0+1][i_3+1][i_3+i_0] = B[i_2][i_3+3][i_3+i_2+1] + 6;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_Q-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
            for (int i_6 = 0; i_6 < min(PB_Q-2, -i_5+PB_M-1); i_6++) {
                B[i_6][i_5+2][i_6+i_5] = C[i_5+1][i_6+2][i_4+2] - B[i_6+1][i_5+2][i_6+i_5+1] - B[i_6+1][i_5+1][i_6+i_5] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4232212315_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
