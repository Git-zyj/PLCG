...
void kernel_2334112323_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
            A[i_0][i_1] = D[i_1+1][i_0] + A[i_0-1][i_1] + A[i_0-2][i_1-2] + A[i_0+2][i_1-1] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        B[i_2+2] = D[i_2][i_2+2] * B[i_2] - 4;
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_N, i_3); i_4++) {
            C[i_4] = D[i_4][i_3] - C[i_4-1] + C[i_4+1] + C[i_4-2] + 3;
        }
    }
#pragma endscop
...
kernel_2334112323_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
