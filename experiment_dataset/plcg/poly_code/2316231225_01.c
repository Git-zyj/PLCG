...
void kernel_2316231225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[0][i_0] = D[i_0-1][i_1] + E[i_1+i_0] - F[i_1+1][i_0+1] - A[0][i_0+2] * A[0][i_0+1] + 1;
        }
        for (int i_2 = i_0; i_2 < -i_0+PB_L; i_2++) {
            A[0][i_0] = F[i_2-1][i_0] * G[i_2][i_2][i_0-1] * F[i_2+i_0][i_0+1] * E[i_2+i_0] - D[i_0-1][i_0] - 2;
        }
    }
...
kernel_2316231225_01(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
