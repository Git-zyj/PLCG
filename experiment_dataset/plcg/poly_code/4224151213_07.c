...
void kernel_4224151213_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            B[i_1-1] = C[i_1] + C[i_0] + D[i_0-1][i_1-1] * D[i_1][0] * D[i_1+1][i_0] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2] = C[i_2-1] * C[1] + C[i_2] - C[i_2] + C[i_2] * B[i_2+1] * B[i_2-1] - 5;
    }
#pragma endscop
...
kernel_4224151213_07(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
