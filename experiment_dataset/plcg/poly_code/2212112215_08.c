...
void kernel_2212112215_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            B[i_0][i_1][i_0] = C[i_1][0] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, PB_L-1); i_3++) {
            B[i_3][i_2][i_3] = C[i_3+1][i_2] - B[i_3-1][i_2][i_3-1] + B[i_3+1][i_2][i_3+1] + 5;
        }
    }
#pragma endscop
...
kernel_2212112215_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
