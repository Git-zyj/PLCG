...
void kernel_4236232311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M-2; i_1++) {
            A[i_0][i_1] = C[i_1+2][i_0] + D[i_0][i_1-2] * E[i_1-1] - 4;
        }
    }
        ...
        for (int i_3 = max(2, i_2); i_3 < PB_P; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_P-1, i_2); i_4++) {
                for (int i_5 = 1; i_5 < PB_P; i_5++) {
                    B[i_3][i_4][i_5] = F[i_4][i_4][i_3-2][i_5] + G[i_2][i_5][i_3] * B[i_3][i_5][i_4+1] * B[i_3][i_4+1][i_5] + B[i_3+1][i_5][i_4] + B[i_3+1][i_4][i_5-1] + B[i_3][i_4-1][i_5] * 6;
                }
            }
        }
    }
...
kernel_4236232311_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
