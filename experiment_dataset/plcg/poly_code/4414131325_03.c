...
void kernel_4414131325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[0] + F[i_0+1][0] + F[i_0][i_0] * 5;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = i_2; i_3 < PB_L; i_3++) {
                    B[i_0+1][i_1][i_1][0] = G[i_2+1][i_3+1][i_1] + G[i_2-1][i_1-1][i_0] - H[i_2][i_1-1][i_3][i_1] + H[i_1+1][1][i_3][i_0+1] + B[i_3][i_0][i_0][0] - B[i_0][i_1][i_1][0] * B[i_0+2][i_2][i_2][0] * 5;
                }
            }
        }
        for (int i_4 = 1; i_4 < min(-i_0+PB_L, i_0); i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                B[i_0+1][i_5][i_5][0] = F[i_5][i_4+i_0] * G[i_4+i_0][i_5+1][1] - G[i_4-1][i_5][i_0] + 1;
            }
        }
    }
        ...
        for (int i_7 = 0; i_7 < PB_M-3; i_7++) {
            for (int i_8 = 0; i_8 < PB_P-1; i_8++) {
                for (int i_9 = 1; i_9 < min(i_6, -i_6+PB_Q); i_9++) {
                    D[i_7+1][i_9][i_6][1] = G[i_7][i_8][i_9+i_6] - D[i_7+3][i_9][i_6][1] * D[i_9-1][i_7+1][i_6][1] - D[i_7+1][i_8+1][i_9][1] + D[i_7+2][i_9][i_6][1] * D[i_8][i_7][i_6][1] + 5;
                }
            }
        }
    }
...
kernel_4414131325_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
