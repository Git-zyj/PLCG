...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn),int xo,DATA_TYPE POLYBENCH_1D(O,xO,xo))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4422351211_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn),int xo,DATA_TYPE POLYBENCH_1D(O,xO,xo)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(min(PB_Q-1, -i_1+PB_N), -i_0+PB_P); i_2++) {
                for (int i_3 = 0; i_3 < -i_2+PB_L; i_3++) {
                    A[i_3+i_2-1][i_1-1][i_0][i_2+1] = E[i_2-1][i_0][i_1][i_1] + E[i_2+i_1][i_3][i_0][i_2+i_0] * F[i_1][i_2-1][i_3+i_2][i_0] + G[i_3][i_2][0][i_1][i_0] * H[i_1-1][i_0][i_0][i_2] - H[i_3][i_1][i_0-1][i_0] - H[i_3][i_0-1][i_1][0] + 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_L-1); i_5++) {
            B[i_4][i_5] = I[i_5][i_4-1] * J[i_5-1][i_4] + I[i_4+1][i_5] * J[i_4][i_5] - K[i_5+1] + 4;
        }
        for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 1; i_7 < -i_6+PB_Q; i_7++) {
                for (int i_8 = 2; i_8 < min(-i_4+PB_M-1, -i_6+PB_Q); i_8++) {
                    C[i_8][i_7-1][i_7+i_6] = E[i_6-1][i_8+i_4+1][i_7][i_4] - H[i_6][i_8][i_6][i_4] + L[i_6][i_8][i_4] - C[i_7-1][i_8-1][i_8+i_6] + C[i_6+1][i_8-2][i_8+i_7-1] + 1;
                }
            }
        }
    }
    for (int i_9 = 0; i_9 < PB_L; i_9++) {
        for (int j_0 = max(i_9, 1); j_0 < -i_9+PB_L; j_0++) {
            D[i_9+1][j_0] = I[i_9][j_0] + M[j_0][j_0-1] - N[j_0+i_9][i_9+1][1] + M[j_0+1][i_9] + O[j_0+i_9] - D[i_9+2][j_0-1] + D[j_0+2][i_9] + D[i_9+1][j_0-1] + D[i_9][j_0] * 2;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N), xo,POLYBENCH_ARRAY(O), INIT_SEED);
kernel_4422351211_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N), xo,POLYBENCH_ARRAY(O));
