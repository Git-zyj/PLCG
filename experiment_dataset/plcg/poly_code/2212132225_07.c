...
void kernel_2212132225_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            B[i_0][i_1-2] = C[i_1] + D[0][i_1] - 6;
        }
        for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
            B[i_0][i_2-2] = D[i_2-2][i_0] + D[i_2][i_0] * D[i_2][i_0-1] - B[i_0+2][i_2-2] + B[i_2+2][i_0-2] * 3;
        }
    }
#pragma endscop
...
kernel_2212132225_07(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
