...
void kernel_2126211113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q+1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q+1, PB_L); i_1++) {
            A[i_1+1] = B[i_1][i_1+i_0-1] - A[i_1] + 3;
        }
    }
#pragma endscop
...
kernel_2126211113_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
