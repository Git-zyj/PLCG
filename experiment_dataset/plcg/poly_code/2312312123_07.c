...
void kernel_2312312123_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_N+1; i_1++) {
        for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
            C[i_1][i_2-1] = D[i_1-1][i_2-2] + E[i_2-1] + 5;
        }
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            C[i_1][i_3] = C[i_1+1][i_3+2] * 6;
        }
    }
#pragma endscop
...
kernel_2312312123_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
