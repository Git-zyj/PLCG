...
void kernel_2432252115_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        B[i_0] = E[i_0] - F[0] + B[i_0] * 1;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        B[i_1] = F[i_1+1] * E[i_1] - F[i_1-2] + F[i_1] - E[i_1] - F[i_1] + E[i_1+1] - 5;
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            B[i_3-1] = G[i_3][i_2] * G[i_3][i_2] * H[i_3][i_3] * 6;
        }
        for (int i_4 = 2; i_4 < min(PB_P, -i_2+PB_M); i_4++) {
            B[i_4] = I[i_4-2][i_2+2][i_2] - H[i_2][i_4] - H[i_4+i_2][0] + G[i_4][i_2+2] * G[i_4][i_2] * 4;
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2432252115_06(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
