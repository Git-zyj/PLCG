...
void kernel_2426311225_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            A[i_0][i_1+i_0] = E[i_1][i_1] * E[0][i_1+1] - F[i_0][i_1] - A[i_0][i_1+i_0-1] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        B[i_2] = G[i_2] + B[i_2+2] * B[i_2-2] * 2;
        for (int i_3 = i_2; i_3 < min(-i_2+PB_L, PB_M); i_3++) {
            A[i_2][i_3+i_2] = B[i_3] - 4;
            B[i_3] = 5;
        }
    }
#pragma endscop
...
kernel_2426311225_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
