...
void kernel_4236311315_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-1); i_1++) {
            for (int i_2 = min(i_0, PB_L-1) - 1; i_2 >= 0; i_2--) {
                A[i_0][i_2][i_1+1] = A[i_0][i_2+1][i_1+1] + 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
            B[i_4+i_3+1] = C[i_3][i_4][i_4+i_3] - B[i_4+i_3+2] - B[i_4+i_3] - B[i_4+i_3+1] - B[i_4+i_3+2] - 6;
        }
    }
#pragma endscop
...
kernel_4236311315_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
