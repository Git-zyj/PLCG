...
void kernel_4412312115_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
    ...
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                B[i_1+i_0][i_0][i_2] = F[i_0+2][i_2][i_1-2] * B[i_1+i_0-1][i_0][i_2+1] - 5;
            }
            A[i_0+1][i_0+1] = 3;
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = i_4; i_5 < -i_4+PB_M; i_5++) {
                for (int i_6 = 0; i_6 < -i_3+PB_M-1; i_6++) {
                    B[i_5+i_4][i_4][i_6+1] = G[i_4-1][i_6+i_3+1][i_4][i_5] - 4;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4412312115_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
