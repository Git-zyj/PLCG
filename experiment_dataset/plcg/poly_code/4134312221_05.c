...
void kernel_4134312221_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 4; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                    A[i_1][i_0-2][i_3+1] = B[i_3][i_1][i_2] * A[i_2][i_1-4][i_3+1] - A[i_0][i_1-4][i_3] * 3;
                }
            }
        }
    }
...
kernel_4134312221_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
