...
void kernel_2322231221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        A[0] = D[i_0+1] - E[i_0] * D[i_0] * A[0] * 5;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_N; i_2++) {
            B[i_1][0] = D[i_2+i_1] * F[i_1+1][i_2][i_2] + G[i_2][i_1] + E[i_2+i_1] * A[0] * B[i_1+1][0] + 6;
        }
        B[i_1+2][0] = E[i_1] - B[i_1][0] + 1;
    }
#pragma endscop
...
kernel_2322231221_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
