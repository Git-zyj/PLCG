...
void kernel_3414351315_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        B[i_0-1] = E[i_0-1] + F[i_0+1] + B[i_0] - 6;
        B[i_0] = G[0] + G[i_0+1] + E[i_0] - G[0] + E[i_0] - E[i_0] - F[i_0] - B[i_0-1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M-1; i_2++) {
            B[i_2] = G[i_2-1] * H[i_2][i_1] - F[0] - E[i_2] + I[i_2-1][i_1] + J[i_1][1] - J[i_2+i_1+1][i_2] + I[i_2-1][i_1] * B[i_2] + 2;
            for (int i_3 = 0; i_3 < PB_M; i_3++) {
                B[i_3] = K[i_2+1][i_3+1][i_1] - L[i_1][i_2][i_3] + M[i_3][i_1-1][i_2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3414351315_00(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
