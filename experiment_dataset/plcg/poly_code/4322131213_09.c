...
void kernel_4322131213_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_M-1); i_1++) {
            for (int i_2 = i_1; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                    A[i_0][i_2][i_3][i_1] = D[i_3+1][i_2+i_1][i_0][0] * A[i_2][i_3][i_0-1][i_1] * 4;
                }
            }
            B[i_1] = E[i_1][i_1+i_0] * E[i_1+1][i_0] - F[i_1+1] * B[i_1+1] * 1;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, PB_N-1); i_5++) {
            B[i_5+1] = E[i_4][i_5] - E[i_4][i_5+1] - E[i_5][i_4+1] - E[i_5][i_4] + B[i_5] + B[i_5+1] - 6;
        }
    }
#pragma endscop
...
kernel_4322131213_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
