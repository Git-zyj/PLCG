...
void kernel_3326212123_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L+1; i_1++) {
            A[i_0][i_1-2] = A[i_0-2][i_1-1] - 3;
        }
    }
    for (int i_2 = 4; i_2 < PB_M; i_2++) {
        B[i_2-2] = B[i_2-1] * B[i_2-4] + 2;
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < min(i_2, PB_M-2); i_4++) {
                C[i_3][i_2][i_4+1] = D[i_4+2][i_4] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3326212123_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
