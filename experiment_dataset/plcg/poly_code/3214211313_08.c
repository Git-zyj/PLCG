...
void kernel_3214211313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                B[i_0+1][i_1-1][i_2-1] = C[i_1][i_2][i_0-1] * B[i_0+1][i_1][i_2-1] + B[i_0+1][i_1-2][i_2-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3214211313_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
