...
void kernel_2316311313_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0+1] = A[i_0] * A[i_0+2] + A[i_0+2] - A[i_0] + 2;
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_L-1); i_1++) {
            B[i_0-1][i_1+i_0+1] = B[i_0-2][i_1+i_0] * 4;
        }
        for (int i_2 = i_0; i_2 < PB_N+1; i_2++) {
            C[i_0][i_2+1] = D[i_2][i_0] - C[i_0+1][i_2+1] + C[i_2-1][i_0] - 6;
        }
    }
#pragma endscop
...
kernel_2316311313_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
