...
void kernel_3432231215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
    ...
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_Q-1, -i_1+PB_M-1); i_2++) {
            for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                D[i_3+1][i_2-1] = F[i_3][i_1][i_2+1] - F[i_3][i_2][i_1] - F[i_3+1][i_2-1][i_1] - 5;
            }
            D[i_1+2][i_2] = G[i_2+i_1+1][i_1-1] * H[i_1][i_2-1] + H[i_2][i_1] + 2;
        }
        for (int i_4 = 1; i_4 < min(-i_1+PB_M+1, PB_L-1); i_4++) {
            D[i_1+1][i_4] = H[i_1-1][i_4+1] + I[i_4+i_1-1][i_4-1][0] + G[i_1+1][i_4] - H[i_4][i_1] + D[i_1+2][i_4] - D[i_1][i_4] + 5;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3432231215_02(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
