...
void kernel_2322331111_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1-1][1] = D[i_0][i_1] - E[i_1] * C[i_0-1][i_1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_L-1); i_3++) {
            B[i_2][i_3+i_2] = D[i_3-1][0] * F[i_3][i_2] - G[i_2][i_3-1] - B[i_2+1][i_3+i_2+1] + 4;
        }
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            C[i_4][i_2] = G[i_2][i_4] * F[i_4][i_2+1] * G[i_2][i_4-1] * C[i_4-1][i_2] * 5;
        }
    }
#pragma endscop
...
kernel_2322331111_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
