...
void kernel_2426211323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_0][i_1] = A[i_0-2][i_1-2] - A[i_0][i_1-2] - A[i_0+2][i_1] + 2;
        }
    }
    ...
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_L; i_5++) {
            C[i_4+1][i_5] = E[0][i_4] - C[i_4][i_5-1] * C[i_4][i_5] * D[i_4-1] + 4;
        }
        D[i_4-1] = D[i_4+1] * 6;
    }
#pragma endscop
...
kernel_2426211323_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
