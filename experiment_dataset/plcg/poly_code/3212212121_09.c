...
void kernel_3212212121_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        ...
        for (int i_2 = PB_M-2 - 1; i_2 >= 3; i_2--) {
            for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                B[i_3][i_2-2][i_0] = C[i_2+2][i_3][i_0] - B[i_3+2][i_2-3][i_0-1] * B[i_3+2][i_2-2][i_0] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3212212121_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
