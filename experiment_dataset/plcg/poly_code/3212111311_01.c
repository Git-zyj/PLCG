...
void kernel_3212111311_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_N+1; i_1++) {
            A[i_0][i_1-1][0] = C[i_1+i_0-1][i_1][i_0] - A[i_0-1][i_1-1][0] * A[i_0+1][i_1-2][0] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            B[i_2][i_3] = A[i_3][i_2-2][0] * A[i_2][i_3][0] + B[i_2+1][i_3-1] * B[i_2][i_3-1] * 4;
        }
    }
#pragma endscop
...
kernel_3212111311_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
