...
void kernel_4332231223_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_Q-2; i_4++) {
            for (int i_5 = 1; i_5 < min(min(PB_Q, -i_3+PB_M+1), -i_4+PB_L); i_5++) {
                B[i_3][i_5][i_4+i_3] = D[i_5-1][i_5+i_4][i_3-1][0] * E[i_5][i_3][i_3+1] - E[i_5][i_3][i_4-1] - E[i_5][i_5][i_4-1] * E[i_5+i_3-1][i_4-1][0] * F[i_4][i_5-1][i_3] + B[i_3][i_5][i_4+i_3+2] - B[i_3][i_5][i_4+i_3+1] * 3;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_Q-1; i_6++) {
        for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
            for (int i_8 = 0; i_8 < -i_7+PB_Q+1; i_8++) {
                B[i_7-1][i_6+1][i_8+i_7-1] = G[i_7+1][i_8] * E[i_6][i_7+1][i_8] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4332231223_00(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
