...
void kernel_3316112323_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_0][i_1-1][i_1+2] = A[i_0-2][i_1-1][i_1+2] - A[i_0][i_1][i_1+3] + A[i_0][i_1-2][i_1+1] + A[i_1-1][i_0-1][i_0+2] * 3;
        }
        for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
            A[i_0+1][i_2-1][i_2+2] = 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_Q; i_3++) {
        for (int i_4 = 3; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_4+PB_P, i_3); i_5++) {
                C[i_5][i_4-2][i_3] = D[i_4][i_3][i_5+i_4] + D[i_5][i_3-2][i_4+2] * C[i_5][i_4-3][i_3] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3316112323_09(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
