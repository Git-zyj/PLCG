...
void kernel_4436112211_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                A[i_0][i_1][i_2] = A[i_0][i_1-1][i_2-1] - B[i_1+i_0] - 2;
            }
            B[i_1+i_0] = 6;
        }
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            C[i_0][i_3+2] = C[i_0][i_3+1] + 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        D[i_4] = E[i_4+2] + D[i_4-1] * D[i_4+1] - 1;
    }
#pragma endscop
...
kernel_4436112211_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
