...
void kernel_4316151225_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
                    A[i_3][i_1][i_2+1][i_2] = D[i_3-1][i_0+1][i_1+1][i_2] - D[i_1+1][i_3][i_0+1][i_2] - D[i_0+1][i_1][i_3][i_2] + A[i_0+1][i_1-1][i_3+1][i_3] + A[i_3][i_1-1][i_2+1][i_2] * A[i_3+2][i_1-2][i_2+1][i_2] * 4;
                }
            }
        }
    }
        ...
        for (int i_5 = i_4; i_5 < min(PB_M, -i_4+PB_P); i_5++) {
            for (int i_6 = 0; i_6 < PB_M; i_6++) {
                for (int i_7 = 1; i_7 < -i_5+PB_Q; i_7++) {
                    A[i_4+1][i_6][i_5+1][i_5] = D[i_4-1][i_7-1][i_7+i_5][i_6] + E[i_6][i_5][i_7] - D[i_5-1][i_7][i_7][i_4-1] * D[i_7][i_5+i_4][i_6][0] * D[i_6][i_7+1][i_4][i_5] + 1;
                }
            }
        }
        for (int i_8 = 1; i_8 < min(i_4, -i_4+PB_L); i_8++) {
            C[i_8][i_4][i_4-1] = F[1] + G[i_4-1][0] + G[i_8+i_4][i_4] * G[i_8+i_4][0] * F[i_4] - E[i_4][i_8+1][i_4-1] * G[i_8-1][i_4] * C[i_8][i_4+1][i_4] * 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4316151225_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
