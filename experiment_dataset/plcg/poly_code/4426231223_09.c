...
void kernel_4426231223_09(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        C[i_0-1] = E[i_0] + 5;
        for (int i_1 = 2; i_1 < min(i_0, PB_M-1); i_1++) {
            B[i_0][i_1] = F[i_1+1][i_0-1] + F[i_1-1][i_0-1] - B[i_0][i_1-2] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        C[i_2-1] = G[i_2] * E[i_2] - G[i_2] - C[i_2+1] * C[i_2-2] * 6;
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_L; i_4++) {
            for (int i_5 = PB_L-1 - 1; i_5 >= 0; i_5--) {
                for (int i_6 = i_4; i_6 < PB_M-1; i_6++) {
                    D[i_3][i_6][i_5][i_6] = H[i_5][i_3][i_4][i_6] + I[i_6][i_4+i_3][i_5][i_3+1] + I[i_6+1][i_3-1][i_4+1][i_3] - I[i_5][i_4][i_3][i_5+1] * H[i_6][i_3][i_5][i_4] + D[i_3+1][i_6][i_5+1][i_6] + D[i_3-1][i_6][i_5][i_6] * 5;
                }
            }
        }
    }
...
init_array(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4426231223_09(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
