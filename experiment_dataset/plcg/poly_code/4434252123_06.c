...
void kernel_4434252123_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        A[i_0] = E[2] * E[i_0+2] * F[i_0+1] - E[i_0] * F[i_0] * F[0] - E[0] + E[i_0] + 4;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
            for (int i_3 = i_1; i_3 < min(-i_1+PB_Q+2, PB_M-2); i_3++) {
                for (int i_4 = 1; i_4 < -i_1+PB_L; i_4++) {
                    B[i_4+1][i_3][i_2][i_1] = G[i_2][i_4-1][i_2+2][i_3+i_1-2] + G[i_2][i_4+1][i_3-2][i_1] - H[i_2+1][i_4+i_1][i_3-1][i_3-1] * I[i_2][1][i_1] - B[i_3+2][i_4][i_2-2][i_1] * 2;
                }
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
        A[i_5] = F[i_5+1] - E[i_5] + J[i_5][i_5] - F[i_5-2] + E[i_5] + 3;
    }
    for (int i_6 = 0; i_6 < PB_M; i_6++) {
        for (int i_7 = i_6; i_7 < PB_M; i_7++) {
            D[i_6][i_7] = E[i_7] * K[i_7][1] + 5;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4434252123_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K)));
