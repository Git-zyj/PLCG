...
void kernel_2432211311_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        ...
        for (int i_3 = 0; i_3 < min(PB_P, -i_2+PB_L-1); i_3++) {
            C[i_3+i_2][i_2] = C[i_3+i_2-1][i_2] * C[i_3+i_2+1][i_2+1] - C[i_3+i_2][i_2+1] * C[i_3+i_2+1][i_3] - C[i_3+i_2+1][i_2] + 6;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        D[i_4+1] = E[i_4] * D[i_4+2] * 2;
    }
#pragma endscop
...
kernel_2432211311_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
