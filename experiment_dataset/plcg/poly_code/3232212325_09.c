...
void kernel_3232212325_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < PB_P; i_2++) {
                A[i_0+1][i_1][i_2] = A[i_0+2][i_1][i_2] + A[i_0+1][i_1][i_2-1] - A[i_0+1][i_1][i_2-2] + 6;
            }
            for (int i_3 = 2; i_3 < PB_P; i_3++) {
                A[i_0+2][i_1][i_3] = C[i_3-2][i_0][i_1-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3232212325_09(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
