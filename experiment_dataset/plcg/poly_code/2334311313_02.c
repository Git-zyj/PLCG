...
void kernel_2334311313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L-1; i_3++) {
            B[i_2-1][i_3] = D[i_3+1] * E[i_2][1] * A[i_3+1] + B[i_2][i_3] * 6;
            B[i_2-1][i_3+1] = E[i_2][i_3] * B[i_2-1][i_3] - B[i_3-1][i_2] - 5;
        }
    }
#pragma endscop
...
kernel_2334311313_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
