...
void kernel_4316112315_01(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < min(min(-i_1+PB_M, i_1), PB_M-1); i_2++) {
            C[i_1] = D[i_2+i_1] + C[i_2-1] - C[i_2+1] - C[i_2+1] * 4;
        }
    }
#pragma endscop
...
kernel_4316112315_01(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
