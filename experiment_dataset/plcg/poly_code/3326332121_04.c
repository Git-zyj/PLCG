...
void kernel_3326332121_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0-2] * D[i_0] - B[i_0-2] * 1;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 3; i_2 < min(i_1, PB_L-2); i_2++) {
            B[i_2-2] = E[i_2] * F[i_2+1][i_1+1] + E[i_2+2] * G[i_1-1][i_2-1] - B[i_2-3] - 6;
        }
        for (int i_3 = 1; i_3 < min(i_1, -i_1+PB_M+2); i_3++) {
            for (int i_4 = i_1; i_4 < -i_1+PB_L; i_4++) {
                C[i_3+i_1-2][i_4][i_1] = H[i_3-1][i_1][0] + H[i_3][i_4+i_1][i_1] * I[i_4][i_1][i_3] * C[i_3+i_1-2][i_4+1][i_1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3326332121_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
