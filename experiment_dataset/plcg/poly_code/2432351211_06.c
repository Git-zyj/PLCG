...
void kernel_2432351211_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-1] + F[i_0] + E[i_0+1] - A[i_0-1] * 6;
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_0][i_1+1] = G[i_0][i_1] - H[i_1][i_0] * I[i_1][i_0] * H[i_0][i_0] + I[i_1][i_0-1] + A[i_1-1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_M-2; i_3++) {
            C[i_3+i_2+1] = H[i_3+i_2+1][0] + H[i_2][i_3] - I[i_3][i_2-1] - I[i_3][i_2] - F[i_3] * H[i_3][i_2] - A[i_3] - C[i_3+i_2+2] - 1;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        D[i_4] = E[i_4] * F[i_4] + J[i_4] * F[i_4] + F[i_4] - J[i_4] * A[i_4-1] * D[i_4-1] - 3;
    }
#pragma endscop
...
kernel_2432351211_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
