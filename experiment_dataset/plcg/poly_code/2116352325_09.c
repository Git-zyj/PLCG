...
void kernel_2116352325_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = min(-i_0+PB_M-2, PB_P-2) - 1; i_1 >= i_0; i_1--) {
            A[i_0][i_1+2][i_1] = B[i_1][i_0] + C[i_1] * B[i_1+i_0+2][0] + B[i_0+2][i_1+2] * B[i_1][i_0] * A[i_0+2][i_1+2][i_1] - A[i_0-1][i_1+2][i_1] * A[i_0-1][i_1+4][i_1+2] - 6;
        }
    }
#pragma endscop
...
kernel_2116352325_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
