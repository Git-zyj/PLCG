...
void kernel_2312211315_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        A[i_0-1] = A[i_0-2] - A[i_0] - 3;
        for (int i_1 = 1; i_1 < min(min(PB_M, i_0), -i_0+PB_L-1); i_1++) {
            B[i_1][i_0] = D[i_1+i_0+1] + A[i_1-1] + A[i_1] + B[i_1][i_0+1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            A[i_3-1] = E[i_3+1][i_2] * 5;
        }
    }
#pragma endscop
...
kernel_2312211315_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
