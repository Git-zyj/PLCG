...
void kernel_4314251121_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = D[i_0] - E[i_0+1] - D[0] - 1;
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, -i_1+PB_L); i_2++) {
                B[i_1+1][i_2] = F[i_0][i_2+i_1][0] - G[i_0-1][i_1][i_2] * G[i_0][i_2-1][i_1-1] * F[i_2+i_0][i_2-1][i_1] - F[i_2+1][i_0][i_1] * H[i_2][i_1] - 6;
            }
        }
        C[i_0] = E[i_0+1] * D[0] - D[i_0+1] + E[i_0] * D[i_0] - D[i_0] + C[i_0-2] * 6;
    }
#pragma endscop
...
kernel_4314251121_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
