...
void kernel_4236211313_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_N-1, i_2); i_3++) {
            for (int i_4 = 1; i_4 < min(PB_P, i_2); i_4++) {
                B[i_3][i_2] = C[i_3-1][i_4][i_2+1] * B[i_3+1][i_4] + B[i_4-1][i_3] + B[i_4+1][i_3] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4236211313_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
