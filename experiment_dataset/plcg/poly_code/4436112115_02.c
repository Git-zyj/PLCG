...
void kernel_4436112115_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        B[i_0+1][i_0-1] = B[i_0][i_0-2] + 3;
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            B[i_1][i_1-2] = 3;
        }
    }
    for (int i_2 = 3; i_2 < PB_P-2; i_2++) {
        B[i_2-1][i_2-3] = E[i_2+2] * E[i_2] + 3;
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_Q+1, PB_M-2); i_4++) {
                for (int i_5 = 0; i_5 < min(-i_3+PB_P, -i_2+PB_P-2); i_5++) {
                    D[i_3][i_4][i_2][i_5+i_4] = F[i_2][i_4+2][i_4][i_5+i_3][0] - G[i_4-1][i_5+i_2+2][i_4][0] * D[i_5+1][i_4][i_2+1][i_4+i_3-1] + 1;
                }
            }
        }
    }
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4436112115_02(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
