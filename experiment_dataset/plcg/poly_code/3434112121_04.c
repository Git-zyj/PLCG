...
void kernel_3434112121_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 4; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[i_1-2] = E[i_1][i_1+i_0] * A[i_1-4] + 3;
        }
    ...
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            D[i_4][i_5] = E[i_4][i_5] * D[i_4+2][i_5] * 2;
        }
    }
#pragma endscop
...
kernel_3434112121_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
