...
void kernel_2336111125_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        ...
        for (int i_3 = max(1, i_2); i_3 < -i_2+PB_M; i_3++) {
            C[i_3+i_2][i_2] = E[i_3][i_3][0] * D[i_3-1][i_2] * 4;
        }
    }
#pragma endscop
...
kernel_2336111125_00(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
