...
void kernel_2224131223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0+1] = C[i_1+1][i_0] - C[i_1+1][i_0] + A[i_1][i_0+2] - A[i_1-2][i_0] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            B[i_3-1][i_2] = C[i_2+1][i_2-1] - C[i_2][i_3] + D[i_2-1] + B[i_3-1][i_2-1] * B[i_3-1][i_2+2] * 2;
        }
    }
#pragma endscop
...
kernel_2224131223_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
