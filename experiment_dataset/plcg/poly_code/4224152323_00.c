...
void kernel_4224152323_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_P-1); i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_M; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_0+PB_P+3, PB_M-1); i_3++) {
                    A[i_1][i_3+i_2-2][i_0] = C[i_1-1][i_2][i_3][i_0] * D[i_2+i_0][i_1][i_3-1] + C[i_2][i_3][i_1][i_0-2] - C[i_3][i_0][i_2+i_0-2][i_1] * C[i_0][i_2+2][i_1+2][i_3] + C[i_2][i_0][i_3][i_1] * C[i_3][i_0-1][i_2][0] + C[i_2][i_3][i_3-1][i_0] - A[i_3+1][i_1+i_0+1][i_2+1] - A[i_2][i_3+i_0-3][i_1-1] * A[i_3][i_1+i_0-2][i_2-1] * 1;
                }
            }
        }
    }
        ...
        for (int i_5 = max(i_4, 2); i_5 < -i_4+PB_M; i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_P; i_6++) {
                for (int i_7 = 1; i_7 < -i_5+PB_M; i_7++) {
                    B[i_5+i_4][i_6+i_5][i_7-1] = C[i_7][i_6][i_5][i_4] * B[i_7+i_5][i_7+i_6-1][i_4] * B[i_7+i_5-2][i_6+i_5-2][i_4-1] - 4;
                }
            }
        }
    }
...
kernel_4224152323_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
