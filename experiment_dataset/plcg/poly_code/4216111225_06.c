...
void kernel_4216111225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 3; i_1 < i_0; i_1++) {
            A[i_1-1] = A[i_1-3] + A[i_1-2] * 1;
        }
        for (int i_2 = i_0; i_2 < PB_L-3; i_2++) {
            for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
                B[i_0+1][i_2+1][i_3+1] = C[i_3-1][i_2][i_0] + B[i_0+1][i_2+3][i_3+1] * B[i_0+2][i_2+1][i_3+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4216111225_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
