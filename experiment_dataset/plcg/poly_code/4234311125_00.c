...
void kernel_4234311125_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                A[i_0][i_1][i_2+i_1] = A[i_0+2][i_1][i_2+i_1] * 3;
            }
        }
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            for (int i_4 = max(1, i_3); i_4 < PB_L; i_4++) {
                B[i_3+1][i_4][i_0] = C[i_4-1][i_0][i_3] + B[i_3+1][i_4][i_0+2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4234311125_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
