...
void kernel_4316312225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[i_0][i_0+2] = 2;
        for (int i_1 = 4; i_1 < i_0; i_1++) {
            B[i_1-2][i_0] = A[i_0][i_0+2] - B[i_1-4][i_0] * B[i_1-3][i_0] + B[i_1][i_0] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            C[i_3][i_2] = D[i_3][i_2] * B[i_3-2][i_2] * C[i_3][i_2+2] * 6;
        }
    }
#pragma endscop
...
kernel_4316312225_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
