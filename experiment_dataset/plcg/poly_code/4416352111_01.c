...
void kernel_4416352111_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_P-2; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, -i_1+PB_P-2); i_2++) {
                for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
                    A[i_3][i_1][i_1-1][i_0] = E[i_2+i_0][i_3+2][i_3][i_2] - E[i_2+i_0][i_3][i_1-2][i_1-1] - E[i_3][i_0][i_1][i_2] * F[i_2-1][0][i_3] + G[i_3][i_3][i_0][i_2+i_1] - E[i_3+i_2][i_0+1][i_1][1] * H[i_0-1][i_0][i_2+i_1+2][i_3] * A[i_3+1][i_1][i_1-1][i_0] * 5;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = i_4; i_5 < min(PB_L, -i_4+PB_M); i_5++) {
            for (int i_6 = 2; i_6 < PB_P; i_6++) {
                for (int i_7 = max(i_4, 2); i_7 < -i_5+PB_L; i_7++) {
                    B[i_5][i_7-1][i_4][i_6-2] = E[i_5+i_4][i_7-2][i_6][i_6-2] + H[i_4][i_7+i_5][i_6][i_5] * 3;
                }
            }
        }
        C[i_4+2] = I[i_4] * I[i_4] + J[i_4][0] * K[1] - K[i_4] * K[i_4] + 4;
        for (int i_8 = i_4; i_8 < PB_L; i_8++) {
            for (int i_9 = 2; i_9 < -i_8+PB_L; i_9++) {
                D[i_8][i_9-2] = F[i_9][i_4+2][i_8] - L[i_4+2][i_9][i_9] + L[i_8][i_9][0] - L[i_9-1][i_9+i_8][i_4+2] - F[i_4][i_8][i_8] + 2;
            }
        }
    }
#pragma endscop
