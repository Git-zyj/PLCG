...
void kernel_3232211111_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
            B[i_2+1] = C[i_2-1][i_0] * B[i_2+2] * B[i_2] - 2;
        }
    }
#pragma endscop
...
kernel_3232211111_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
