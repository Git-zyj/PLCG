...
void kernel_4222132225_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_1][i_0] = C[i_0][i_1] + D[i_0+1] - C[i_0][i_1] * A[i_1-2][i_0] * A[i_0-1][i_1] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
            for (int i_4 = 2; i_4 < PB_M; i_4++) {
                A[i_4][i_3-2] = C[i_4][i_2] * C[i_4-2][i_3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4222132225_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
