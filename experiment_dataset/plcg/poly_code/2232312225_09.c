...
void kernel_2232312225_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = 2; i_2 < i_0; i_2++) {
            B[i_0][i_2-2] = D[i_2][i_0] * B[i_0][i_2] + B[i_0+1][i_2-2] * 1;
        }
    }
#pragma endscop
...
kernel_2232312225_09(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
