...
void kernel_4422132223_09(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    C[i_2-1][i_0][i_2-1] = E[i_1+1][i_3][i_2][i_0] * 5;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            C[i_4][i_0-2][i_4] = F[i_4+1][i_0] * 2;
        }
        for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
            C[i_5][i_0][i_5] = F[i_5+2][i_0-2] - F[i_5+2][i_0-2] + G[i_5] + C[i_0][i_5+2][i_0] - C[i_5-1][i_0][i_5-1] + 2;
        }
    }
    for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
        for (int i_7 = 2; i_7 < -i_6+PB_M-2; i_7++) {
            for (int i_8 = 1; i_8 < -i_6+PB_M-2; i_8++) {
                C[i_7][i_6+2][i_7] = H[i_7+1][i_8][0] + H[i_7][i_8+i_6+2][i_6] + H[i_8][i_7+i_6+2][i_6+1] - E[i_7][i_6][i_7-2][i_8-1] + 5;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422132223_09(xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
