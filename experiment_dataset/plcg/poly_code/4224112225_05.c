...
void kernel_4224112225_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                B[i_2][i_0-2][i_2+1] = C[i_1+1][i_2-1][i_0+2] - B[i_2-1][i_1-3][i_2] - 5;
            }
        }
        for (int i_3 = 1; i_3 < i_0; i_3++) {
            for (int i_4 = 3; i_4 < PB_L+2; i_4++) {
                for (int i_5 = 2; i_5 < PB_L+2; i_5++) {
                    B[i_3-1][i_4-2][i_3] = B[i_4-3][i_5-2][i_4-2] * 2;
                }
            }
        }
    }
...
kernel_4224112225_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
