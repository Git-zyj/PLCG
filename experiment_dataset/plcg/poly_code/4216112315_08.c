...
void kernel_4216112315_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 0; i_1 < min(PB_Q, i_0); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = 1; i_3 < -i_0+PB_L-1; i_3++) {
                    A[i_1][i_2+i_0][i_1][i_3] = A[i_2][i_3+i_0+1][i_2][i_1] - A[i_1][i_2+i_0-1][i_1][i_3-1] - 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        B[i_4] = C[i_4-1] + B[i_4+1] + B[i_4-1] * B[i_4+1] * 6;
    }
#pragma endscop
...
kernel_4216112315_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
