...
void kernel_3326251115_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0][i_1-1] = D[i_0][1] - D[i_1][i_0] * E[i_1] + F[i_1+1][i_0] * D[i_1][i_0+1] - A[i_0+1][i_1-1] * 1;
        }
        for (int i_2 = 1; i_2 < i_0; i_2++) {
            for (int i_3 = 1; i_3 < min(PB_L, -i_2+PB_Q-1); i_3++) {
                A[i_2][i_3-1] = G[i_2+1][i_0+1][i_3+i_2+1] + H[i_0][i_2][i_0-1] - H[i_2][1][i_3] - G[i_3][i_2+1][i_0] + F[i_3][i_2-1] * H[i_3][i_2+1][i_0] + 6;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
            for (int i_6 = 1; i_6 < min(min(-i_4+PB_P, i_5), -i_5+PB_M+1); i_6++) {
                A[i_5-1][i_6-1] = G[i_5+1][i_6+i_4][i_5+1] + G[i_6][0][i_5] + D[i_6][i_4] * H[i_5-1][i_4+1][i_6+i_5-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3326251115_07(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
