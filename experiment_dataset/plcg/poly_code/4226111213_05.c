...
void kernel_4226111213_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_Q-1, -i_0+PB_M-1); i_2++) {
                A[i_0-1][i_1-1][i_2] = C[i_2][i_2+i_0+1][i_1+1] * A[i_0-1][i_1-1][i_2-1] + A[i_0-1][i_1][i_2+1] + 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            B[i_3+1][i_4] = B[i_4+2][i_3-1] * B[i_4+1][i_3+1] * 2;
        }
    }
#pragma endscop
...
kernel_4226111213_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
