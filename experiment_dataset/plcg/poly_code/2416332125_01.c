...
void kernel_2416332125_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-2 - 1; i_0 >= 2; i_0--) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_N, PB_M-1); i_1++) {
            A[i_0-2][i_1] = E[i_0+2] - F[i_1+1][i_0] * G[i_0+2][i_0] * F[i_1+i_0][i_1-2] * G[i_1+1][i_1+2] * H[i_1][i_0] - A[i_0][i_1] + 4;
            B[i_1][i_0-2] = H[i_1+2][i_0] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        D[i_2+1] = I[0] * 2;
        D[i_2+2] = H[i_2][0] + I[i_2-2] * 2;
    }
#pragma endscop
...
kernel_2416332125_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
