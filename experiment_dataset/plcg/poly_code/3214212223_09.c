...
void kernel_3214212223_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0] = C[i_0] + C[0] * A[i_0+1] + 4;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = max(3, i_1); i_2 < PB_M+2; i_2++) {
            for (int i_3 = i_1; i_3 < PB_L-4; i_3++) {
                B[i_2-2][i_3+2] = B[i_2-2][i_3+4] * B[i_2-3][i_3+2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3214212223_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
