...
void kernel_4324252311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            A[i_0][i_1] = D[i_1+i_0][i_0+2] + D[i_1][i_0] - E[i_0][0] + F[i_1] * F[i_0+1] * E[i_1][i_0] - D[i_0+2][0] - A[i_0][i_1-1] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            for (int i_4 = 1; i_4 < min(min(PB_Q-1, -i_2+PB_M+2), i_3); i_4++) {
                for (int i_5 = 1; i_5 < -i_3+PB_M; i_5++) {
                    B[i_2-1][i_3][i_4][i_5] = G[i_3][i_2][i_5][i_3] - H[i_4][i_2-1][i_3-2][i_5+1] * G[i_3-1][i_4+i_2-2][i_5-1][0] + I[i_5+i_3][i_2+2][0] - B[i_2-1][i_4][i_3][i_5+1] - B[i_2][i_3][i_4-1][i_5] - B[i_2][i_4][i_3][i_5] - B[i_3-1][i_4+1][i_5][i_2] + 2;
                }
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
        for (int i_7 = i_6; i_7 < PB_L-1; i_7++) {
            for (int i_8 = 2; i_8 < -i_6+PB_L-1; i_8++) {
                for (int i_9 = 0; i_9 < min(min(min(-i_7+PB_M, PB_L-1), -i_8+PB_L-1), -i_6+PB_L+1); i_9++) {
                    C[i_7][i_6][i_9+i_8][i_7+1] = G[i_7][i_8-2][i_6-1][i_6-1] - G[i_8][i_9+i_7][i_6+1][i_8] - B[i_7-1][i_8][i_9][i_6] - C[i_6-1][i_7][i_9+i_8+1][i_6] - C[i_9][i_7+1][i_8+i_6+1][i_9+1] * C[i_7][i_8][i_9+i_6-1][i_7+1] * 6;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4324252311_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
