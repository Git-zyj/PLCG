...
void kernel_4216132123_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_0-1][i_1] = C[i_0][i_1] + D[i_1] + 1;
        }
        for (int i_2 = i_0; i_2 < PB_L; i_2++) {
            B[i_0-1][i_2] = C[i_0][1] * C[i_2][0] + D[i_0+2] - B[i_0][i_2] - 4;
        }
    }
#pragma endscop
...
kernel_4216132123_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
