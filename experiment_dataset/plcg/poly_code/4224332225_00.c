...
void kernel_4224332225_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_L, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                A[i_1+2][i_1] = C[i_1][i_1+i_0][i_2] * D[i_1][i_2+i_1] * E[i_1][i_0][i_2] * C[i_1][i_0][i_0] - C[i_2+i_1][i_0+2][0] * C[i_2][i_0][i_1-2] + A[i_1+3][i_1+1] + 2;
            }
        }
        for (int i_3 = 0; i_3 < PB_N; i_3++) {
            for (int i_4 = 0; i_4 < PB_N; i_4++) {
                for (int i_5 = i_3; i_5 < PB_M; i_5++) {
                    A[i_5+2][i_5] = A[i_5+2][i_5] + 1;
                }
            }
        }
    }
...
kernel_4224332225_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
