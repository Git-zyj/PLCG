...
void kernel_3332212225_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        A[i_0] = 2;
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            B[i_1][i_0-1] = D[i_1] + E[i_0+1][i_1-2] - A[i_1] - B[i_1][i_0-2] - B[i_1][i_0-3] + 3;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            A[i_2] = E[i_0][i_2+1] - B[i_2-1][i_0-3] + 4;
        }
    }
#pragma endscop
...
kernel_3332212225_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
