...
void kernel_3334311311_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            A[i_0][i_1+1] = A[i_0][i_1] + A[i_0+1][i_1+1] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, PB_L-1); i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                B[i_3][i_4-1] = A[i_4][i_3+1] - B[i_3-1][i_4-1] - B[i_3][i_4] + 1;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 0; i_6 < PB_Q; i_6++) {
            C[i_5+1][i_6] = D[i_5][i_6] * C[i_6+1][i_5-1] * C[i_5][i_6+1] - C[i_5+1][i_6+1] + C[i_5+2][i_6+1] - 5;
        }
    }
#pragma endscop
...
kernel_3334311311_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
