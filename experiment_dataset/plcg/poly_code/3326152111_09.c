...
void kernel_3326152111_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            A[i_0][i_1] = D[i_1][i_0] * E[i_1] - D[i_1][i_0] * D[i_0][i_0] - D[i_1][i_0] + A[i_0][i_1+1] - 4;
        }
        for (int i_2 = 1; i_2 < min(PB_Q+1, -i_0+PB_P); i_2++) {
            for (int i_3 = i_2; i_3 < PB_L-2; i_3++) {
                B[i_2+2][i_3+2][i_0] = F[i_3][i_2+i_0][i_0-1] + F[i_0][i_3][i_2-1] * F[i_0][i_2][i_3-1] - B[i_2+1][i_3+2][i_0] + 1;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < min(min(i_4, -i_4+PB_M+2), PB_L-1); i_5++) {
            C[i_4][i_5] = D[i_4+1][i_5-2] - D[i_5+2][i_5] + D[i_5-1][i_5] - D[i_5][i_4] * D[i_5+i_4-2][0] + C[i_4][i_5+1] + 1;
        }
    }
#pragma endscop
...
kernel_3326152111_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
