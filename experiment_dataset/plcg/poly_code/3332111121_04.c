...
void kernel_3332111121_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_L, i_0); i_2++) {
                A[i_2][i_1][i_2+i_0] = D[i_2][i_0][i_2] + 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_L+1; i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                A[i_3][i_5][i_4+i_3-1] = D[i_5][i_3-1][i_4-1] - 3;
            }
        }
        for (int i_6 = 1; i_6 < -i_3+PB_L; i_6++) {
            for (int i_7 = 0; i_7 < PB_L; i_7++) {
                C[i_6-1][i_6][i_3] = A[i_6][i_7][i_6+i_3] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3332111121_04(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
