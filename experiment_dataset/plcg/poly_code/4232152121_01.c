...
void kernel_4232152121_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_0][i_2+3][i_1] = C[i_1][i_0-2][i_2] - D[i_2][i_1-1] + 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_N-2; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_3+PB_M, -i_4+PB_N); i_5++) {
                B[i_5][i_3+1][i_4] = C[i_5][i_3][i_5] + E[i_5][i_3][i_4+1][i_3] * C[i_4+1][i_5-1][i_5] + C[i_5+i_4][i_5+i_3][i_3] + C[i_4-1][i_5][i_3] - C[i_5][i_3][i_4] - C[i_4+2][i_3-2][i_5] - C[i_3][i_5-2][i_4] + B[i_5][i_3+2][i_4-1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4232152121_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
