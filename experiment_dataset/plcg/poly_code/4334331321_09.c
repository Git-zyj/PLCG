...
void kernel_4334331321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0-1] + E[i_0] + D[i_0-1] + F[i_0-1][i_0] - A[i_0-2] * A[i_0+2] * C[i_0-1] - C[i_0+1] - 5;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < min(i_1, PB_L+1); i_2++) {
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                B[i_2][i_3][i_1] = G[i_3-1][i_2-1][i_1-1] + 2;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        C[i_4-1] = H[i_4] * H[i_4] + A[i_4] * C[i_4] + C[i_4+1] - C[i_4-2] * 1;
    }
#pragma endscop
...
kernel_4334331321_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
