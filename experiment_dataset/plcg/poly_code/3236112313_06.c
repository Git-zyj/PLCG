...
void kernel_3236112313_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        ...
        for (int i_3 = 1; i_3 < i_0; i_3++) {
            B[i_0-1][i_3] = C[i_0][i_3-1] - D[i_0] * B[i_0-2][i_3-1] + B[i_0-1][i_3-1] * B[i_0-2][i_3] + 1;
        }
    }
#pragma endscop
...
kernel_3236112313_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
