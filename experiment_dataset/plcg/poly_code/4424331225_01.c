...
void kernel_4424331225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_P; i_1++) {
            A[i_0][i_1] = E[i_0+1] - F[i_0][i_1] * F[i_1-1][i_0] - A[i_0+2][i_1] * 3;
        }
        for (int i_2 = 1; i_2 < i_0; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                C[i_2][i_3+1][i_0+1][0] = G[i_3][i_2][0] - G[i_2-1][i_3][i_0] + 5;
            }
        }
    }
        ...
        for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_P-1; i_6++) {
                for (int i_7 = 0; i_7 < PB_N-2; i_7++) {
                    C[i_7][i_6-1][i_5+1][0] = H[i_5][i_7][i_6][i_4+1] + I[i_7][i_4][i_6+1][i_5] - I[i_7][i_6+1][i_4][i_5] - C[i_7][i_6][i_5-1][0] - C[i_7+2][i_6-1][i_5+1][0] * C[i_6][i_4-3][i_5+1][0] + 4;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_P-1; i_8++) {
        for (int i_9 = 1; i_9 < PB_M; i_9++) {
            for (int j_0 = 2; j_0 < PB_P-1; j_0++) {
                C[j_0-2][i_9-1][i_8+1][0] = G[i_9][j_0+1][i_8-1] + I[i_9][i_8][j_0+1][j_0+1] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4424331225_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
