...
void kernel_4324131113_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M-1; i_2++) {
                for (int i_3 = 1; i_3 < min(PB_N, -i_0+PB_L); i_3++) {
                    A[i_1+i_0][i_2][i_3][i_2] = D[i_0][i_3][i_1][0] - D[i_2-1][i_3+i_0][i_1][i_2] - E[i_2+i_1][i_3][i_0] - A[i_2+i_0+1][i_1][i_3-1][i_1] + 4;
                }
            }
        }
        B[i_0] = F[i_0] - F[i_0] * F[i_0] * 1;
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            B[i_0] = G[i_4][i_0] + F[i_4+1] * B[i_4-1] - 2;
        }
    }
#pragma endscop
...
kernel_4324131113_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
