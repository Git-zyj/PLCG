...
void kernel_4216212321_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P, i_0); i_1++) {
            for (int i_2 = 2; i_2 < min(min(-i_0+PB_Q, PB_L), -i_1+PB_L); i_2++) {
                A[i_2][i_1+1][i_2+i_0] = C[i_1][i_0][i_0] * D[i_0][i_2+i_1][i_1] + A[i_2-1][i_1+1][i_2+i_0-1] + A[i_2-2][i_1+1][i_2+i_0-2] - 4;
            }
        }
        for (int i_3 = 2; i_3 < PB_P; i_3++) {
            for (int i_4 = max(3, i_0); i_4 < PB_P; i_4++) {
                B[i_0][i_4-2][i_4] = B[i_0+2][i_4-2][i_4] * B[i_0+1][i_3-2][i_3] - B[i_0-2][i_4-3][i_4-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4216212321_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
