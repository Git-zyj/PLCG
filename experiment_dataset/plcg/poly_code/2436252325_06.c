...
void kernel_2436252325_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0+2] = E[i_0] + F[i_0] - F[i_0] - E[i_0-1] + A[i_0+4] * A[i_0] - 3;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        A[i_1+3] = E[i_1] - G[i_1+2][0] + F[i_1+1] + F[i_1-2] + E[i_1] + E[i_1] * E[i_1] * 3;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        A[i_2+2] = E[i_2] - E[i_2-1] - F[i_2+1] + F[i_2] * F[i_2] - A[i_2+3] + 3;
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            D[i_3][i_2-1][i_3] = G[i_3][i_2] + H[i_2][i_3] * H[i_2-2][i_3] + G[i_3][i_2] + A[i_3+2] - D[i_3][i_2][i_3] * 6;
        }
    }
#pragma endscop
...
kernel_2436252325_06(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
