...
void kernel_2436231125_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_1+1] = E[i_1] + F[i_0][i_1-1] - G[i_1][i_0+1] - 6;
        }
        B[i_0+1] = E[i_0-1] - H[i_0-1] - H[i_0-1] + E[i_0+1] + H[i_0] - B[i_0+2] * 3;
        C[i_0+1] = E[i_0] + E[i_0] - H[i_0] + H[i_0] + 2;
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 3; i_3 < min(PB_L, i_2); i_3++) {
            D[i_2][i_3-1] = D[i_2][i_3-3] - D[i_2][i_3] - 6;
        }
    }
#pragma endscop
...
kernel_2436231125_05(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
