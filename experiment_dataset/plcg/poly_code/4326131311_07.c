...
void kernel_4326131311_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = i_1; i_2 < min(-i_0+PB_Q+1, -i_1+PB_Q); i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_Q-1; i_3++) {
                    A[i_3][i_1][i_0][i_2+i_1-1] = D[i_0][i_3][i_1+1][i_2+1] + D[i_0+1][i_2][i_3+i_1+1][0] - A[i_3-1][i_1][i_0][i_2+i_1] * A[i_3+1][i_0][i_1][i_2+i_0-1] * 6;
                }
            }
        }
        for (int i_4 = 1; i_4 < min(PB_N-1, i_0); i_4++) {
            B[i_4] = E[i_4][i_0] - E[i_0][i_4] * B[i_4+1] - B[i_4-1] + B[i_4+1] + B[i_4-1] - 3;
        }
    }
    for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 1; i_6 < PB_P; i_6++) {
            for (int i_7 = 0; i_7 < min(i_6, -i_5+PB_Q+2); i_7++) {
                for (int i_8 = i_6; i_8 < -i_6+PB_Q; i_8++) {
                    A[i_8][i_5-1][i_6-1][i_7+i_5-2] = F[i_8][i_7][i_6] - F[i_7][i_6][i_8+1] + D[i_5+1][i_7][i_8+i_6][i_7] * 5;
                }
            }
        }
    }
...
kernel_4326131311_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
