...
void kernel_4214312323_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_1][i_0+1][0] = C[i_1][i_0] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_2][i_3][0] = B[i_2-2][i_3][0] + B[i_2][i_3+2][0] + B[i_4][i_3-1][0] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4214312323_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
