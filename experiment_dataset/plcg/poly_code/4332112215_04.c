...
void kernel_4332112215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            A[i_0][i_1] = D[i_0][2] * D[i_1+1][i_1] - D[i_0][i_1+1] * B[i_1] * 1;
        }
        B[i_0] = B[i_0-1] - B[i_0+1] - 1;
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            C[i_2][i_3+1][i_3] = C[i_2][i_3+2][i_3+1] + 2;
        }
    }
#pragma endscop
...
kernel_4332112215_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
