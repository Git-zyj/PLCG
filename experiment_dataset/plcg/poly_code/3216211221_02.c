...
void kernel_3216211221_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M, i_0); i_2++) {
                A[i_0][i_2][i_1] = C[i_0-1][i_2+i_0][i_2][i_1] - A[i_0-1][i_1][i_2] * A[i_0][i_2][i_1-1] - 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_Q; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q; i_5++) {
                B[i_3+1][i_4-1][0] = D[i_4-1][i_5][i_4] * B[i_3+1][i_4][0] * B[i_5+2][i_4-1][0] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3216211221_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
