...
void kernel_3316311225_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_P; i_2++) {
                A[i_0][i_2+i_1][i_1] = D[i_1-1][i_2][i_0+1] + A[i_0][i_2+i_1-1][i_2-2] - 6;
            }
            for (int i_3 = 0; i_3 < PB_P; i_3++) {
                B[i_0][i_0+1][0] = B[i_0+2][i_0+3][0] - 3;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P; i_4++) {
        for (int i_5 = i_4; i_5 < PB_P-1; i_5++) {
            for (int i_6 = 3; i_6 < PB_P+1; i_6++) {
                C[i_4][i_5-1][i_6-1] = C[i_4-2][i_6-3][i_5+1] + C[i_6][i_5-2][i_4] * C[i_4][i_5+1][i_6-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3316311225_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
