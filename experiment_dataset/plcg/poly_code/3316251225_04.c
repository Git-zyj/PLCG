...
void kernel_3316251225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] - D[i_0] * D[i_0] + D[i_0] * E[i_0-1] - E[i_0] * E[i_0-1] - A[i_0+2] + 1;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_M; i_2++) {
            A[i_2] = E[i_2+1] + F[i_2+i_1][0] - G[i_2][i_1+1] - F[i_1+1][i_1] * 5;
        }
        for (int i_3 = 1; i_3 < PB_P-3; i_3++) {
            for (int i_4 = 0; i_4 < -i_1+PB_N; i_4++) {
                C[i_3+1][i_1+1][i_4] = H[i_4][i_1-1][i_3+1] - H[i_4+i_1][i_3-1][i_3] * H[i_4][i_1][i_3-1] - C[i_3+1][i_1-1][i_4] * C[i_1][i_4+1][i_3+1] + C[i_1][i_3+3][i_4] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3316251225_04(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
