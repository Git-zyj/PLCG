...
void kernel_3214132321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_1][i_0-1] = C[i_1+i_0][0] * C[i_1][i_0] * A[i_1][i_0+1] - 6;
        }
        for (int i_2 = 0; i_2 < PB_P-4; i_2++) {
            for (int i_3 = max(3, i_2); i_3 < min(PB_P, -i_2+PB_L-2); i_3++) {
                B[i_3+2][i_2+2][i_3-2] = C[i_3][i_2] + D[i_0-1][i_2][i_3] + C[i_3+i_2][i_3] - D[i_3+i_2+2][i_0][i_3] - A[i_0][i_2+1] + B[i_3+2][i_2+4][i_3-2] + B[i_3+2][i_2][i_3-2] + B[i_3+1][i_2+3][i_3-3] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3214132321_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
