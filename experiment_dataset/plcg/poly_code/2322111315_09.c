...
void kernel_2322111315_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_0][i_1] = D[i_1-1] - A[i_1+1][i_0+1] + A[i_1-1][i_0] + 6;
        }
    }
    ...
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        C[i_4-1] = D[i_4] + D[i_4] * C[i_4] + C[i_4-2] - C[i_4-2] + 2;
    }
#pragma endscop
...
kernel_2322111315_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
