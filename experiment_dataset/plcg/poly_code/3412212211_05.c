...
void kernel_3412212211_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    for (int i_0 = 3; i_0 < PB_N+2; i_0++) {
        ...
        C[i_0] = E[i_0-1] - B[i_0-2] - C[i_0-1] + 5;
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = max(3, i_3); i_4 < PB_N+3; i_4++) {
            B[i_4-3] = 1;
        }
    }
#pragma endscop
...
kernel_3412212211_05(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E));
