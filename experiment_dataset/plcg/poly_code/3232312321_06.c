...
void kernel_3232312321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M+3, PB_L); i_1++) {
            for (int i_2 = 0; i_2 < min(min(PB_L, -i_0+PB_M+1), -i_1+PB_M+2); i_2++) {
                A[i_2+i_1-2][i_0] = A[i_2+i_0-1][i_1] * A[i_2+i_0-3][i_1] + A[i_1+i_0-3][i_2] - 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
        B[i_3] = C[i_3][i_3] + D[i_3] - B[i_3-2] - B[i_3+2] * 5;
    }
#pragma endscop
...
kernel_3232312321_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
