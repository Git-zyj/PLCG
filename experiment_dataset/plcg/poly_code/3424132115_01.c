...
void kernel_3424132115_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] * E[i_0-1] + E[i_0] + 5;
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            B[i_1][i_0+1] = F[i_0][i_1+1] - E[i_1+1] - A[i_1+1] * A[i_1-1] - 1;
        }
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_L, -i_2+PB_M); i_4++) {
                D[i_4+i_2][i_3][i_3] = G[i_2+2][i_3][i_4] * G[i_4][i_2][i_3] * G[i_2][i_4][i_3] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3424132115_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
