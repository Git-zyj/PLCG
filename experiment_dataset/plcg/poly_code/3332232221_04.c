...
void kernel_3332232221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                A[i_1+1][2] = D[i_2][i_0][i_1+2] * D[i_1][i_2][i_0] - A[i_1-1][2] * 3;
            }
        }
        for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
            B[i_0+1] = E[i_3][i_0] * E[i_3][i_0+1] - F[i_3][i_0] - F[i_3+2][i_0] - F[i_3-2][i_0+1] + E[i_3][i_0] - B[i_3] - 5;
        }
    }
    for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_P-2; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
                C[i_5][i_4][i_4] = C[i_6][i_5+2][i_5+2] - C[i_6+2][i_5][i_5] * C[i_5][i_4+1][i_4+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3332232221_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
