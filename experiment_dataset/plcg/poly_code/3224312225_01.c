...
void kernel_3224312225_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < min(PB_P, -i_1+PB_N); i_2++) {
                B[i_1][i_2-2] = C[i_2+i_1][i_0][0] - B[i_1-2][i_2] - 2;
            }
            for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                B[i_1-2][i_3] = C[i_0-2][i_3+1][i_1-1] + B[i_1][i_3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3224312225_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
