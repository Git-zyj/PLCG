...
void kernel_2224232323_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_L, -i_0+PB_N-2); i_1++) {
            A[i_1+i_0][i_1+i_0] = C[i_1][i_0] * D[i_1-1][0][i_0] - E[i_0][i_1] * C[i_0-2][1] * A[i_1+i_0+2][i_1+i_0+2] * A[i_1+i_0-1][i_1+i_0-1] - A[i_1+i_0-1][i_1+i_0-1] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < PB_P+1; i_3++) {
            B[i_2][i_3-2] = F[i_3] + C[i_3][i_2] * B[i_2-2][i_3-1] + B[i_2+2][i_3-2] * 6;
        }
    }
#pragma endscop
...
kernel_2224232323_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
