...
void kernel_4114312125_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = PB_M - 1; i_1 >= 2; i_1--) {
            for (int i_2 = 2; i_2 < PB_N; i_2++) {
                for (int i_3 = 2; i_3 < PB_Q-2; i_3++) {
                    A[i_2][i_2][i_3-2][i_3+2] = B[i_2][i_1-2][0] + A[i_2-2][i_2-2][i_3-2][i_3+2] * 3;
                }
            }
        }
    }
...
kernel_4114312125_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
