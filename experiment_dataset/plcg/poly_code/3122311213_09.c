...
void kernel_3122311213_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_M, PB_Q-1), i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_1+1][i_2] = B[i_1+i_0][i_2][i_2+1][i_1+1] + A[i_1+1][i_2+1] * A[i_2][i_1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3122311213_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B));
