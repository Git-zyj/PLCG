...
void kernel_4412152313_02(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 0; i_1 < min(PB_P, -i_0+PB_L-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
                    C[i_3][i_2] = E[i_2][i_0][i_3+i_1][i_3] * E[i_1+i_0][i_3+2][i_2+1][i_3+i_1] * F[i_0][i_3][i_2] * E[i_0][i_3-2][i_2][i_1] * E[i_1][i_1+i_0+1][i_3-1][i_2] + 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < min(-i_4+PB_L+1, PB_L-2); i_5++) {
            for (int i_6 = 1; i_6 < PB_L; i_6++) {
                C[i_6-1][i_5+1] = F[i_5+2][i_4][i_4] - F[i_5][i_6][i_5+2] + F[i_6][i_5+i_4-1][i_4] - F[i_4+1][i_6][i_5] * F[i_4-1][i_5][i_6] * 2;
            }
        }
        for (int i_7 = 1; i_7 < -i_4+PB_M; i_7++) {
            C[i_4][i_7] = G[i_7+1][i_4] + G[i_7+i_4][i_7] - G[i_4][i_7] * G[i_7][i_7] - C[i_4+1][i_7-1] * C[i_7+1][i_4] - 4;
            C[i_4][i_7] = G[i_7-1][i_7] + G[i_7][i_4] - H[i_7] * C[i_4][i_7] + 4;
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4412152313_02(xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
