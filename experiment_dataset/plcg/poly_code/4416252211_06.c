...
void kernel_4416252211_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        A[i_0+2] = E[i_0] * F[0] - F[i_0+2] - F[i_0] + A[i_0+1] * A[i_0+3] * A[i_0+1] * 5;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_N; i_2++) {
            for (int i_3 = 2; i_3 < min(min(-i_1+PB_M, -i_2+PB_L+1), i_2); i_3++) {
                for (int i_4 = i_1; i_4 < min(-i_1+PB_Q-2, -i_2+PB_L); i_4++) {
                    B[i_4][i_3+i_2-2][i_1-2][i_4+i_1] = G[i_2][i_4][i_1+2][0] * H[i_3][i_3+i_1][i_4+i_2][i_1] + H[i_2][i_3][i_1][i_3-2] * I[i_3+i_1-1][i_4+1][i_2] * B[i_4+1][i_3+i_2-1][i_1-1][i_4+i_1+2] - B[i_1-1][i_4+i_2-2][i_3-1][i_3+i_1] + 6;
                }
            }
        }
        for (int i_5 = i_1; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 0; i_6 < -i_5+PB_L; i_6++) {
                C[i_1-2][i_5+2][i_6] = J[i_6][i_5-2] + K[i_6][i_5] + L[i_6][i_1-2][i_5] - G[i_5][i_1-2][i_5][i_6] - L[i_1-2][i_6+i_5][i_1] - L[i_6][i_5][i_1] + 5;
            }
            for (int i_7 = i_1; i_7 < min(PB_L-2, -i_1+PB_L+1); i_7++) {
                D[i_1][i_7+1][i_5] = I[i_7][i_1][i_5+2] * I[i_5][i_1+2][i_7] + L[i_7][i_1][i_5-2] * L[i_1][i_5+2][0] + L[i_5][i_7+i_1-1][i_5+1] + I[i_5][i_7+i_1][i_5+1] + C[i_1-1][i_5+3][i_7] - D[i_1+1][i_5+1][i_7] * D[i_1][i_7+2][i_5] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4416252211_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
