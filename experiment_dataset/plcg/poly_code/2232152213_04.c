...
void kernel_2232152213_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_0][0] = C[i_0][1] * D[i_0] - C[i_1][i_0+2] + C[i_0][i_1] + 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        B[i_2][0] = D[i_2+2] + D[i_2-2] * D[i_2] * D[i_2] * D[i_2] - B[i_2+1][0] + B[i_2-1][0] + 4;
    }
#pragma endscop
...
kernel_2232152213_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
