...
void kernel_3414332215_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_1] = E[i_0+2] * F[i_0][i_1] + B[i_1][i_0] - 5;
        }
        for (int i_2 = 1; i_2 < min(-i_0+PB_L+2, PB_M); i_2++) {
            B[i_0][i_2] = F[i_2+i_0-2][i_0] * G[i_2][i_0] - H[i_2][i_0+1] - H[i_2][i_0-2] - H[i_0][i_2] + B[i_0][i_2+1] * B[i_0+1][i_2] - B[i_0+1][i_2-1] + D[i_0] * 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            C[i_3+1] = C[i_4+2] - D[i_3+1] - 4;
        }
        D[i_3] = I[1] * J[i_3] + I[i_3] - D[i_3-1] - 6;
    }
#pragma endscop
...
kernel_3414332215_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
