...
void kernel_2316251323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] - D[i_0] + E[i_0] - A[i_0+1] + A[i_0-2] - 5;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            C[i_2][i_1] = D[i_2-1] * F[i_2][i_1] + F[i_2][i_1] + F[i_2-1][i_1-1] + F[i_2][i_1-1] + G[i_1+1][i_2] * F[i_2+1][i_1-1] * H[i_2][i_1-1][0] * 5;
            C[i_2][i_1] = E[i_2] - E[i_2-1] + G[i_1][i_2] + E[i_2-1] - C[i_1+2][i_2] - C[i_2][i_1-1] * C[i_2-1][i_1-2] * 3;
        }
    }
#pragma endscop
...
kernel_2316251323_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
