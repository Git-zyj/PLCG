...
void kernel_2434251313_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] * F[i_0+1] * E[i_0] * E[i_0] * A[i_0+1] * A[i_0-1] + A[i_0+1] * 4;
    }
    for (int i_1 = 1; i_1 < PB_Q; i_1++) {
        C[i_1] = E[i_1] * F[i_1+1] - F[i_1] - C[i_1+1] - 1;
        for (int i_2 = i_1; i_2 < min(PB_Q, -i_1+PB_M-1); i_2++) {
            C[i_2] = G[i_2][i_1] - F[i_2] - H[i_2][i_1] - H[i_2][i_1-1] + H[i_2+i_1+1][i_2+1] * E[i_2-1] + I[i_1][i_2+i_1-1][0] + E[i_2+i_1-1] * C[i_2-1] * 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            C[i_4] = G[i_3][i_4+1] + E[i_4] - H[i_4][i_3] + G[i_4-1][i_3-1] - G[i_3-1][1] - 4;
        }
    }
#pragma endscop
...
kernel_2434251313_05(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
