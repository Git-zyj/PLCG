...
void kernel_2314152121_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L-2 - 1; i_0 >= 2; i_0--) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_N+1, i_0); i_1++) {
            A[i_0-1][i_1] = D[i_0][i_1] * D[i_1-1][i_0] * D[i_1][i_0-1] * D[i_1][i_0] - D[i_0][i_1] + 4;
            B[i_0][i_1] = D[i_1+i_0-1][0] * D[i_1][i_0+1] + D[i_1][i_0] - D[i_1][i_1+i_0-2] + D[i_1][i_0+2] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_N-1); i_3++) {
            C[i_3+2][0] = D[i_3][i_2] + E[i_2-1] + D[i_3+i_2+1][i_3-1] - C[i_2+1][0] - 5;
        }
    }
#pragma endscop
...
kernel_2314152121_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
