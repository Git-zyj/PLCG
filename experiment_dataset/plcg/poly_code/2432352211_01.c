...
void kernel_2432352211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = E[i_0] - F[i_0-1] + F[i_0+2] + E[i_0] * B[i_0][i_0] * 3;
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            B[i_0][i_0] = F[i_1+i_0] + G[0][i_1+1] - H[i_0-2][i_1+1] * E[i_1+i_0] + H[i_0-2][2] + I[i_1][i_0] * J[i_1] * A[i_1] - A[i_1-1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            C[i_3] = G[i_3][2] + I[i_3][i_2] + I[i_3+1][0] * C[i_3-1] * C[i_3+1] - 1;
        }
        for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
            C[i_2] = J[i_4+1] + G[i_4][i_2] + H[i_4-2][i_2+2] + C[i_4] + 4;
        }
    }
#pragma endscop
...
kernel_2432352211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
