...
void kernel_4326112123_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
                B[i_0][i_2+1][i_1-1] = D[i_1+i_0][i_2-2][i_1] - 3;
                B[i_1][i_2-1][i_0] = D[i_2][i_1-2][i_0] - B[i_2-2][i_0+1][i_1+1] * 2;
            }
            for (int i_3 = 0; i_3 < i_1; i_3++) {
                C[i_1][0] = D[i_0-1][i_0-2][i_3] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4326112123_04(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
