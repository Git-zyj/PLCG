...
void kernel_3426331315_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            A[i_0][i_1] = E[i_1] - F[i_1][i_0] * A[i_0][i_1-1] - A[i_1+1][i_0] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            A[i_2+1][i_3] = F[i_3][i_2] - F[i_3-1][i_2-1] + G[i_3-1][i_3] + 3;
        }
        for (int i_4 = 1; i_4 < -i_2+PB_M+1; i_4++) {
            A[i_2][i_4] = H[i_4+i_2-1][i_2+1] * F[1][i_4-1] - A[i_2][i_4] * 6;
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = 0; i_6 < -i_5+PB_M; i_6++) {
            for (int i_7 = 1; i_7 < i_5; i_7++) {
                A[i_5][i_7-1] = I[i_6+i_5][i_7][i_7] + I[i_6][i_7+1][i_7] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3426331315_02(xa,ya,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
