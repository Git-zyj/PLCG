...
void kernel_4212231215_07(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                for (int i_3 = i_0; i_3 < min(-i_1+PB_L-1, PB_M-1); i_3++) {
                    B[i_3][i_0+1][i_0+1][i_1] = C[i_0+1][i_3+1][i_1][i_3] * D[i_3][i_2][i_1-1][i_0-1] - D[i_0][i_3+i_1+1][i_0-1][i_2] * 4;
                }
            }
            for (int i_4 = 0; i_4 < PB_Q; i_4++) {
                for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                    B[i_5+1][i_0][i_0][i_1+1] = E[i_5][i_4][i_1] - C[i_5][i_1][i_5][i_4+1] * B[i_5+1][i_0-1][i_0-1][i_1+1] * B[i_5+1][i_4+1][i_4+1][i_1+2] * 4;
                }
            }
        }
    }
...
kernel_4212231215_07(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
