...
void kernel_3436312223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
                A[i_1-1][i_2] = A[i_1-1][i_2+1] + 5;
            }
        }
        B[i_0] = E[0] * F[i_0] + B[i_0+2] + B[i_0+1] + B[i_0-2] * 5;
    }
    for (int i_3 = 2; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M-2, i_3); i_4++) {
            C[i_4+i_3][i_3] = C[i_4+i_3+2][i_3+1] - C[i_4+i_3-2][i_4] * 4;
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = 2; i_6 < min(i_5, PB_L-1); i_6++) {
            D[1][i_6] = D[1][i_6-2] + D[1][i_6+1] + 6;
        }
    }
#pragma endscop
...
kernel_3436312223_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
