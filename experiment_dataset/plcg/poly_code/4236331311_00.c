...
void kernel_4236331311_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                A[i_2+1][i_1-1] = C[i_2][i_0][i_1] - D[i_2][i_0][1] * E[i_2][i_0] * F[i_1][i_2] + A[i_1][i_2-1] - A[i_1+1][i_2] - 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M-1, i_3); i_4++) {
            B[i_4+i_3] = E[i_3][i_3-1] * B[i_4+i_3+1] + B[i_4+i_3+1] - B[i_4+i_3-1] + 6;
        }
    }
#pragma endscop
...
kernel_4236331311_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
