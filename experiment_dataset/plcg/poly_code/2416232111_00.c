...
void kernel_2416232111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = A[i_0+1] - 3;
        B[i_0] = E[i_0] + F[i_0] + G[i_0][i_0] + F[i_0-1] - B[i_0+1] + 6;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < min(-i_1+PB_P, PB_L+1); i_2++) {
            A[i_2] = G[i_2+2][i_1] * H[i_2-1][i_1] + H[i_1][i_2-1] + I[i_1+2][i_2-2][i_2+i_1] - 6;
        }
        for (int i_3 = max(2, i_1); i_3 < PB_L; i_3++) {
            D[i_3-2] = G[i_3][i_1+2] - G[i_3+1][i_1+1] - F[i_3] + G[i_1][i_3] - 6;
        }
    }
#pragma endscop
...
kernel_2416232111_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
