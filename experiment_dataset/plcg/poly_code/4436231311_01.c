...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4436231311_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, PB_L-1); i_2++) {
                for (int i_3 = 0; i_3 < i_2; i_3++) {
                    A[i_2+1][i_1-1][i_0-1][i_3] = E[i_2][i_3][i_0+1][i_2] - A[i_3+1][i_1-2][i_0][i_2] - 1;
                }
            }
            for (int i_4 = 1; i_4 < min(i_0, PB_L-1); i_4++) {
                for (int i_5 = 1; i_5 < PB_P; i_5++) {
                    B[i_0-1][i_1-1][i_4][1] = E[i_5-1][i_0-1][i_0-1][i_4] - F[i_4][i_0][i_5-1][i_1][0] - G[i_1][i_4][i_0] + B[i_0-1][i_1][i_4][1] - B[i_0-2][i_5-1][i_4-1][1] - B[i_0-1][i_1-1][i_4+1][1] * 2;
                }
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_L-2; i_6++) {
        for (int i_7 = 0; i_7 < PB_L-2; i_7++) {
            C[i_6+1][i_7+1] = H[i_7] + I[i_6+1][i_7] + J[i_7+1][i_6] + H[i_6] + C[i_6][i_7] + C[i_6][i_7+2] * C[i_7+1][i_6+2] - C[i_6+1][i_7] * C[i_6+1][i_7+2] - 5;
        }
    }
    for (int i_8 = 1; i_8 < PB_L-1; i_8++) {
        for (int i_9 = 1; i_9 < i_8; i_9++) {
            for (int j_0 = i_8; j_0 < -i_8+PB_L-1; j_0++) {
                for (int j_1 = 0; j_1 < min(min(min(-i_9+PB_L, -i_8+PB_P-1), -j_0+PB_M+1), PB_L-2); j_1++) {
                    D[i_8][j_0][i_9][j_0+1] = K[j_1][j_0+i_8][i_9][i_8+1] - K[j_1][j_0+i_8+1][i_9-1][j_0] * K[j_1+j_0-1][i_9+1][i_8-1][j_1+i_8+1] - L[j_0][j_1+i_9][j_0-1] * A[j_1+2][i_8-1][i_9-1][j_0-1] + D[i_8-1][j_0+1][i_9][j_0+2] * D[i_8+1][j_0][i_9][j_0+1] + 1;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4436231311_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
