...
void kernel_3226131121_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M+1, PB_M-1); i_2++) {
                A[i_1][i_1][i_2-1] = C[i_2+1][i_0][i_1-1] - C[i_2][i_0][i_1] + D[i_2+i_1-1][i_0] + A[i_1+1][i_1+1][i_0] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = i_3; i_4 < min(-i_3+PB_M, PB_M-1); i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                B[i_5][i_4+i_3][i_5+1] = C[i_4][i_3+1][i_5] + D[i_4+1][i_5-1] * C[i_5][i_3+1][i_4] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3226131121_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
