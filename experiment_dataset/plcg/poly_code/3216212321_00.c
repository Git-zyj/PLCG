...
void kernel_3216212321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0-2][i_1] = A[i_1+i_0][i_1+2] - A[i_1+i_0-1][i_1] * A[i_1+i_0][i_0] - A[i_1+i_0][i_1] + 4;
        }
        for (int i_2 = 0; i_2 < min(i_0, PB_M-2); i_2++) {
            B[i_2][i_2] = C[i_2][i_0] * B[i_0+1][i_0+1] + B[i_2+2][i_2+2] * 6;
        }
    }
#pragma endscop
...
kernel_3216212321_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
