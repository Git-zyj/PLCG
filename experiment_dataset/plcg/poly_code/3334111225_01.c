...
void kernel_3334111225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            A[i_0][i_1] = D[1][i_1] - A[i_0+2][i_1+2] + 5;
        }
    ...
    for (int i_4 = 3; i_4 < PB_M; i_4++) {
        C[i_4][i_4-1] = C[i_4-1][i_4-2] * C[i_4-2][i_4-3] * 1;
    }
#pragma endscop
...
kernel_3334111225_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
