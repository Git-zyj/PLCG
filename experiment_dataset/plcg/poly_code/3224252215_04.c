...
void kernel_3224252215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1-1] = C[i_1+1][i_0] + D[i_1][i_0+2] - D[i_1+i_0][i_1] * C[i_1][i_0-1] + D[i_0][i_1-2] + C[i_1-1][i_1] + A[i_0][i_1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
            A[i_2][i_3-1] = C[i_3][i_2] + D[i_3+1][i_2] + C[i_2][i_3-2] + A[i_2][i_3-2] * 2;
        }
    }
#pragma endscop
...
kernel_3224252215_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
