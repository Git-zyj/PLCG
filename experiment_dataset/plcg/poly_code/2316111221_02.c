...
void kernel_2316111221_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_N; i_1++) {
            A[i_0][i_0+1] = D[i_1+i_0][0] * A[i_0+2][i_0+3] - A[i_0-2][i_0-1] - 5;
        }
    ...
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_L-1, i_3); i_4++) {
            C[i_4][i_3] = D[i_4][0] - D[i_4][i_3] * C[i_3+1][i_4+1] - C[i_4-2][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_2316111221_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
