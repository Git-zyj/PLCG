...
void kernel_2232311315_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_N, i_2); i_3++) {
            A[i_2-1][i_3-1] = C[i_3+1][i_2+1] * A[i_3][i_2+1] + A[i_2-1][i_3] + 3;
        }
    }
#pragma endscop
...
kernel_2232311315_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
