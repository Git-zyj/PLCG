...
void kernel_3434131115_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_M, -i_1+PB_M-2), PB_L-1); i_2++) {
                A[i_0][i_2+i_1+1][i_2] = E[i_1][i_2][i_0] + E[i_2+i_0][0][i_2] * E[i_2+i_1][i_0+1][i_1] + A[i_0][i_2+i_1+2][i_2+1] + 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L-1; i_4++) {
            B[i_3][1] = F[i_4] - G[i_4][i_3] * 3;
            for (int i_5 = 0; i_5 < min(-i_4+PB_M+1, -i_3+PB_L+1); i_5++) {
                C[i_3][i_5][i_5+i_4-1] = E[i_4][i_5+i_3-1][i_3] * E[i_5][i_4+1][0] - E[i_3-1][i_5][i_4] * 5;
            }
        }
...
kernel_3434131115_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
