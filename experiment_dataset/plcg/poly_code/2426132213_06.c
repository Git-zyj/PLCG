...
void kernel_2426132213_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0][i_0] * A[i_0-1] + 3;
        B[i_0] = F[i_0] * F[i_0] + F[i_0+1] - B[i_0-1] + 5;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        C[i_1] = F[i_1] + F[i_1+2] * F[i_1+2] - F[i_1+2] - C[i_1+1] * 3;
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_M-2); i_2++) {
            D[i_2+i_1][i_2+1] = E[i_2][i_1] + E[i_2][i_1] * E[i_2][i_1] - C[i_2-1] + D[i_2+i_1][i_2+2] * 6;
        }
    }
#pragma endscop
...
kernel_2426132213_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
