...
void kernel_2236111215_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q+1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_Q, i_0); i_1++) {
            A[i_0+1][i_1] = A[i_1+1][i_0-1] * A[i_0+1][i_1-1] * 1;
        }
        for (int i_2 = 0; i_2 < min(PB_M, i_0); i_2++) {
            B[i_2+1] = C[i_2][i_0] + B[i_2+2] + B[i_2] * 1;
        }
    }
#pragma endscop
...
kernel_2236111215_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
