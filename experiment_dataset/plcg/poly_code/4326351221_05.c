...
void kernel_4326351221_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_Q-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
                for (int i_3 = 1; i_3 < i_1; i_3++) {
                    A[i_1][i_2-1][i_3][i_0] = D[i_1+1][i_2][i_0] * E[i_1][i_2-1][i_3][i_2-1] * E[i_3][i_2-1][i_3-1][i_2+i_1] - F[i_3+1][i_3+1][i_1+i_0+1][0] - G[i_3][0][i_2+1] + H[1][i_2][i_3+i_1+1][i_2+i_0] + E[i_1][i_3][i_0][i_2] + F[i_0][i_2][i_1][i_3] * A[i_1-1][i_2-1][i_3][i_0] * 4;
                }
            }
        }
        B[i_0] = I[i_0+1] - J[i_0+1] * I[i_0] * B[i_0-1] * 1;
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_M; i_5++) {
            C[i_4] = J[i_5-1] * K[i_5][i_4] * C[i_5-2] - C[i_4+2] + 5;
        }
    }
#pragma endscop
...
kernel_4326351221_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
