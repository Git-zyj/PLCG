...
void kernel_2434351315_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            C[i_0-1] = E[i_1+1][i_0] * 4;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
            C[i_2-1] = E[i_2+i_0][1] * F[i_2+1][i_0] + G[i_2][i_2+i_0] * H[i_0] - E[i_2][i_0] - 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        C[i_3-1] = I[i_3-1] + J[i_3] + I[1] * I[i_3] - I[i_3+1] * C[i_3] * C[i_3-2] + C[i_3-2] - 5;
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < -i_4+PB_M; i_5++) {
            C[i_5-2] = E[i_4][i_5] + F[i_5][1] * E[i_5+i_4][1] - G[i_5][i_4] - G[i_5][0] * K[i_5][i_4-1][i_4+1] * H[i_5] * 1;
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_2434351315_02(xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
