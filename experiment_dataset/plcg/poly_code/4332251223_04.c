...
void kernel_4332251223_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M+1; i_1++) {
            for (int i_2 = i_0; i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_0+PB_M+3, PB_P-1); i_3++) {
                    A[i_1+i_0-1][i_3][i_2][0] = D[i_0][i_3][i_2][i_1] + E[i_2][i_0][i_3][i_1] - E[i_0+1][i_2+1][i_3][i_1+1] * D[i_3][i_0+1][i_2][i_1-1] - D[i_1][i_2+i_0][i_3+1][i_0-1] + D[i_1+1][i_0][i_3][i_2] + E[i_2+i_0-1][i_3-1][1][i_1] - A[i_2+i_1-2][i_3][i_0][0] * A[i_3+i_0-3][i_1-1][i_2][0] + 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P; i_4++) {
        for (int i_5 = 0; i_5 < min(PB_M-1, -i_4+PB_L); i_5++) {
            C[i_5+1][i_4][i_5] = F[i_5+1][i_5+i_4] - G[i_5] - H[i_5+1][i_5+i_4-1] + F[i_5][i_4] + F[i_5][i_4] * 6;
        }
        for (int i_6 = 1; i_6 < min(min(i_4, -i_4+PB_M+1), PB_M-1); i_6++) {
            C[i_6+1][i_4][i_6] = I[i_6+i_4-1] + H[i_6][i_4] + C[i_6][i_4-1][i_6-1] * 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4332251223_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
