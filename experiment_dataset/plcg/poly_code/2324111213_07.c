...
void kernel_2324111213_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[1] = 2;
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_0+1][i_1] = D[i_1+1][i_1] * A[1] + A[1] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_M+1, i_2); i_3++) {
            C[i_2+1][i_3+i_2-1] = D[i_3][i_2] * B[i_3+1][i_2-1] + B[i_2+1][i_3] - C[i_2+1][i_3+i_2-2] + 6;
        }
    }
#pragma endscop
...
kernel_2324111213_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
