...
void kernel_4212351215_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = C[i_0] * C[i_0+1] - D[i_0+1] + D[i_0] + C[i_0] + A[i_0+1] + A[i_0-1] + 6;
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_0+PB_Q, i_0), -i_2+PB_L-1); i_3++) {
                    B[i_2][i_1][i_3+i_0][i_3] = E[i_3+1][i_2][i_1][i_1+1] - F[i_0][i_1+1][i_2][i_2+i_1+1] * F[i_3][i_2+i_1][i_0][i_2] + F[i_3+i_2+1][i_1][i_0-1][i_1+1] + G[i_2-1][i_1+i_0][i_3][0] + B[i_2][i_1][i_3+i_0-1][i_3] + 2;
                }
            }
        }
    }
...
kernel_4212351215_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
