...
void kernel_4226231123_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_M, i_0), PB_M-1); i_2++) {
                B[i_1+1][i_2] = C[i_2+1][i_1][i_1+1] * C[i_2+i_0][i_1][i_0] * 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M+1, i_3); i_4++) {
            B[i_3][i_4] = D[i_4][i_3-1] * E[i_4+i_3-1] - D[i_4][i_3] - B[i_3+1][i_4] + 1;
        }
    }
#pragma endscop
...
kernel_4226231123_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
