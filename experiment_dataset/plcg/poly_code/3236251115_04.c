...
void kernel_3236251115_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = C[i_0] * D[i_0] * C[i_0] * D[i_0-1] - A[i_0-1] * 2;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        A[i_1] = D[i_1+1] - D[i_1+1] * D[i_1] + D[i_1] * D[i_1] + D[0] + 4;
    }
#pragma endscop
...
kernel_3236251115_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
