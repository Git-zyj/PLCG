...
void kernel_2336211311_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-2); i_1++) {
            A[i_1+1][i_1+1] = A[i_1+2][i_1+2] + A[i_1][i_1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        ...
        for (int i_4 = i_2; i_4 < PB_L-2; i_4++) {
            C[i_4+1] = D[i_2] - C[i_4] * C[i_4+2] * C[i_4] * C[i_4] * 4;
        }
    }
#pragma endscop
...
kernel_2336211311_01(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
