        ...
        for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
            for (int i_6 = 0; i_6 < -i_4+PB_L; i_6++) {
                for (int i_7 = 0; i_7 < -i_6+PB_M-1; i_7++) {
                    B[i_7+i_6][i_5][i_4-1][i_7] = F[i_7][i_6+i_4][i_4][i_5+1] - F[i_7][i_5+1][i_4+1][0] - F[i_5-1][i_6][i_7][i_4-1] * G[i_4+1][i_5-1][1][i_6][i_7+1] - F[0][i_4][i_6][i_7+1] - B[i_7+i_6+1][i_5][i_4-1][i_7+1] - 3;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_L-1; i_8++) {
        for (int i_9 = 1; i_9 < min(-i_8+PB_Q, PB_L-1); i_9++) {
            for (int j_0 = 1; j_0 < -i_9+PB_L-1; j_0++) {
                for (int j_1 = 0; j_1 < -i_8+PB_L-1; j_1++) {
                    C[i_8+1][j_0+1][i_9][j_1] = G[j_1][i_9-1][i_8-1][j_0-1][i_9+i_8] - E[i_9][j_1][i_9][i_8] * H[j_0+i_9+1][j_1+i_8+1][j_0] * I[j_1][i_9][i_9] - F[j_0+1][j_1+1][i_8][i_9] + 5;
                }
            }
        }
    }
    for (int j_2 = 1; j_2 < PB_N-1; j_2++) {
        D[j_2+1] = J[j_2+1] - K[j_2-1] + J[j_2-1] + L[j_2+1][j_2] + J[j_2] * J[j_2] - 3;
    }
#pragma endscop
...
kernel_4432251125_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
