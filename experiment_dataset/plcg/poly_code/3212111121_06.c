...
void kernel_3212111121_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_L-1); i_2++) {
                A[i_2+i_1-1][i_1] = C[i_2][i_1] - A[i_2+i_1][i_2+1] + A[i_2+i_1-2][i_2-1] * 4;
            }
        }
...
kernel_3212111121_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
