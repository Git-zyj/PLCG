...
void kernel_2324152321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1] = D[i_0+1] * E[i_1][i_1+i_0] * E[i_1][i_0] * E[i_1+i_0+2][i_1] + E[i_1][i_1+i_0-1] + E[i_1-2][i_0] - E[i_1][i_0] - A[i_1+2] + A[i_1-2] * A[i_1-1] + 2;
        }
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            C[i_2+3] = E[i_2][i_0] + D[i_0] * C[i_0+2] - 2;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_N+2, -i_3+PB_M-2); i_4++) {
            C[i_4+2] = E[i_4+i_3+2][i_3-2] - E[i_4-1][i_3] * D[i_4-2] - E[i_4+i_3-1][i_3-2] + E[i_4][i_3] + A[i_4] - C[i_4+3] - 3;
        }
    }
#pragma endscop
...
kernel_2324152321_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
