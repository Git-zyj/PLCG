...
void kernel_2334131211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = D[i_0] * D[i_0+1] - D[i_0+1] * A[i_0] * B[i_0-1] + B[i_0] + 4;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        B[i_1] = D[0] + D[i_1] * D[i_1] - B[i_1+1] + 6;
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        C[i_2][i_2-1] = D[i_2] * D[i_2] - D[i_2] + C[i_2-1][i_2-2] + 4;
    }
#pragma endscop
...
kernel_2334131211_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
