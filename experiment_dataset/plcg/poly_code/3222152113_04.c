...
void kernel_3222152113_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M; i_1++) {
            A[i_1] = C[i_1] * C[i_1] - C[i_0] - D[i_0][i_1-2] + D[i_1][i_0] + D[i_0][i_1+2] * A[i_1+1] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        A[i_2] = C[i_2-2] * C[i_2] * C[i_2] + 1;
    }
#pragma endscop
...
kernel_3222152113_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
