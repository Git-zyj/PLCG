...
void kernel_2212151215_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_0][i_1] = C[i_1][i_0-1] * D[i_1] + D[i_1-1] - C[i_1][i_0+1] * C[i_0][0] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            B[i_2][i_3] = E[i_3][i_2+1][i_2] - C[i_2][i_3-1] * D[i_2+1] + C[i_3][i_2] - C[i_3][i_2-1] - B[i_2-1][i_3] * B[i_2][i_3+1] * 1;
        }
    }
#pragma endscop
...
kernel_2212151215_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
