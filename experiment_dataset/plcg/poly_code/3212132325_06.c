...
void kernel_3212132325_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < -i_0+PB_Q+1; i_2++) {
                B[i_2-2][i_0][i_0+1] = C[i_2+1][i_1][i_2+i_0-1] * C[i_0][i_2][i_0-2] * C[i_2][i_0][i_1] * C[i_2-1][i_0][i_1] - 2;
            }
        }
        for (int i_3 = 0; i_3 < PB_Q; i_3++) {
            for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
                B[i_4][i_0][i_0+1] = C[i_0+1][i_0-2][i_3] + B[i_4][i_0+2][i_0+3] * B[i_4+2][i_0][i_0+1] + B[i_3][i_4+1][i_4+2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3212132325_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
