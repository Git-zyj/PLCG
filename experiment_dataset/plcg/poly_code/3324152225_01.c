...
void kernel_3324152225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0+2][0] = D[i_0][i_0-2] + E[i_0] - E[i_0+1] + E[i_0+1] * E[i_0] + A[i_0+3][0] - 5;
    }
    for (int i_1 = 3; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = max(1, i_2); i_3 < min(min(PB_N-2, -i_2+PB_N), -i_1+PB_M-2); i_3++) {
                B[i_3-1][i_1] = F[i_2][i_2][i_3+i_1+2] - F[i_3+i_2][i_1-1][i_1] + F[i_3+2][i_1][i_1] + F[i_2][i_3][i_1-2] - F[i_2][i_1][i_3] * B[i_1-3][i_2] + 2;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P-2; i_4++) {
        for (int i_5 = i_4; i_5 < -i_4+PB_P-2; i_5++) {
            C[i_4][i_5+i_4] = E[i_5+2] - D[i_5-2][i_4] - D[i_5][i_4] - C[i_4+1][i_5+i_4+2] * C[i_4][i_5+i_4-2] - C[i_5][i_5+i_4+2] - 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_3324152225_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
