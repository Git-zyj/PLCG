...
void kernel_4326132225_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-4; i_2++) {
                for (int i_3 = max(2, i_2); i_3 < min(PB_Q-2, -i_0+PB_L+1); i_3++) {
                    A[i_1][i_2+2][i_3][i_3] = D[i_2][i_3][i_3+i_0-1][0] + A[i_1][i_2+4][i_3-2][i_3-2] * A[i_1][i_2+2][i_3+2][i_3+2] - 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_N-2; i_4++) {
        B[i_4] = E[i_4] * E[i_4] * E[i_4+2] * B[i_4-2] + 3;
        for (int i_5 = i_4; i_5 < -i_4+PB_N; i_5++) {
            for (int i_6 = 1; i_6 < PB_L-2; i_6++) {
                for (int i_7 = 2; i_7 < PB_N-2; i_7++) {
                    A[i_5][i_6+2][i_7-2][i_7-2] = D[i_5+i_4][i_7][i_5][i_6-1] * D[i_7+2][i_6][i_4+1][i_5] * D[i_5-2][i_7][i_4][i_4] + F[i_7-2][i_6][i_5] - 5;
                }
            }
        }
    }
...
kernel_4326132225_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
