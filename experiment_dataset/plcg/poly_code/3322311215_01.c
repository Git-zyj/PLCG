...
void kernel_3322311215_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0-1][i_0-1] = D[i_0] + 1;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0-1][i_0-1] = A[i_0-1][i_0-1] + A[i_0][i_0] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_N+1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            C[i_2-1][i_3] = C[i_2-1][i_3+1] - C[i_2-2][i_3-1] * 5;
        }
    }
#pragma endscop
...
kernel_3322311215_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
