...
void kernel_3232112223_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_N; i_2++) {
            B[i_2+1] = D[i_2][0] + B[i_2+2] - B[i_2-1] * 5;
        }
    }
#pragma endscop
...
kernel_3232112223_02(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
