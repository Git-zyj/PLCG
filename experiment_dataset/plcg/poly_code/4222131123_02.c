...
void kernel_4222131123_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
                for (int i_3 = i_1; i_3 < min(PB_M, -i_0+PB_L); i_3++) {
                    A[i_2][i_1-1][i_3+i_0][i_1+1] = C[i_3][i_2-1][i_1][i_0+1] - D[i_2+i_1][i_3][i_1-1] + C[i_3+1][i_2][i_3][i_1] - C[i_2][i_3][i_1][i_0] * A[i_2][i_1-1][i_3+i_0-1][i_1+1] + 3;
                }
            }
        }
    }
        ...
        for (int i_5 = 1; i_5 < min(i_4, PB_P-1); i_5++) {
            for (int i_6 = 0; i_6 < PB_M; i_6++) {
                for (int i_7 = 0; i_7 < min(PB_P, -i_6+PB_L); i_7++) {
                    A[i_4-2][i_5-1][i_7+i_6][i_5+1] = C[i_4][i_5-1][i_6][i_7] - 2;
                }
            }
        }
    }
...
kernel_4222131123_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
