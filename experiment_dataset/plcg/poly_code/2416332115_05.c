...
void kernel_2416332115_05(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        C[i_0-1] = E[i_0+1] - F[i_0-2] - E[i_0-2] * 4;
    }
    for (int i_1 = 3; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            C[i_1] = G[i_2][0] * 2;
        }
        C[i_1-1] = F[i_1] - E[i_1] + H[i_1-1] * F[i_1+1] + I[i_1][2] - 2;
        for (int i_3 = 2; i_3 < PB_L+1; i_3++) {
            D[i_1-2][i_3-1] = D[i_1-1][i_3-1] - D[i_1-3][i_3-2] + 6;
        }
    }
#pragma endscop
...
kernel_2416332115_05(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
