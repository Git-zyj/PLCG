...
void kernel_4432231221_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
        ...
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 2; i_3 < i_1; i_3++) {
                    A[i_1][i_3][i_2][i_0] = E[i_1][i_2+1][i_0][1] * F[i_3+1][i_2][i_0][i_1+1] + A[i_2][i_3-2][i_0][i_1] - A[i_2][i_3][i_0+1][i_1] + A[i_1][i_0+1][i_2][i_3] + 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
        B[i_4] = G[i_4][i_4+1] + G[i_4-1][i_4] * 6;
        for (int i_5 = 1; i_5 < PB_P; i_5++) {
            for (int i_6 = 3; i_6 < -i_4+PB_L; i_6++) {
                for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
                    D[i_5][i_6-3][i_7+1] = E[i_7][i_6][i_5+1][i_4+1] + E[i_7-1][i_6][i_7][i_4-1] + E[i_5][i_5-1][i_6][i_4] * F[i_7][i_6+i_4][i_5][i_6-1] + 1;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_L-1; i_8++) {
        for (int i_9 = 1; i_9 < PB_M-1; i_9++) {
            for (int j_0 = 3; j_0 < PB_L-1; j_0++) {
                D[i_8][i_9-1][j_0+1] = H[i_9][j_0][i_8] + H[i_8][i_9][0] * I[j_0-1][i_9+1][i_8] + I[j_0][i_8+1][i_9-1] + D[j_0-1][i_9-1][i_8-1] + D[i_9-1][j_0-3][i_8+1] + D[j_0][i_9-1][i_8] * 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4432231221_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
