...
void kernel_2226351315_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            A[i_1] = C[i_0+1][i_1-1] * C[i_1][i_0] + D[i_1+1][i_0+1] + A[i_1+1] + A[i_1-1] - 6;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
            A[i_2] = E[i_0] * F[i_0] + C[i_2][i_0] * G[i_2][0] - H[i_2+i_0][i_2][i_0] * G[i_2][i_0-1] - C[i_2][i_0] * A[i_2] + 4;
        }
    }
#pragma endscop
...
kernel_2226351315_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
