...
void kernel_3234212313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
        for (int i_2 = 3; i_2 < PB_M+2; i_2++) {
            for (int i_3 = 2; i_3 < PB_M+2; i_3++) {
                B[i_1-2][i_3] = B[i_2-2][i_3-1] - B[i_2-3][i_3] + B[i_3-2][i_2+1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3234212313_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
