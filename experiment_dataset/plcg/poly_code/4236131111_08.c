...
void kernel_4236131111_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M; i_2++) {
                A[i_1][i_2][i_0-1] = C[i_2][i_2][i_1] + C[i_2][i_0][i_1] - D[i_1][i_2] + 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_L, PB_M-1); i_5++) {
                B[i_4][i_5] = C[i_5+1][i_4+1][i_3-1] + C[i_4][i_5+i_3][i_4+1] - C[i_4+1][i_5+1][i_3] - B[i_5+1][i_4] * B[i_5][i_4+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4236131111_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
