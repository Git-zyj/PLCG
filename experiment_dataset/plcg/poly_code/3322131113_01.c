...
void kernel_3322131113_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_N-1); i_2++) {
                A[i_2+1][i_1+1][0] = D[i_2+i_1][0][i_0] - D[i_2][i_1][i_2] - D[i_1][i_0][i_1] * A[i_2+1][i_1][0] * 2;
            }
        }
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                B[i_3][i_4][i_0] = D[i_3-1][i_4][1] + D[i_0][i_3][i_4] * D[i_3][i_4-1][i_0] * D[i_3][i_0][0] - 4;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_Q; i_5++) {
        for (int i_6 = 0; i_6 < min(-i_5+PB_M, PB_L-1); i_6++) {
            for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
                C[i_6+1][i_7] = D[i_6+i_5][i_7][0] - D[i_6+1][i_7+1][i_5] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3322131113_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
