...
void kernel_3316312221_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        B[i_0] = 5;
        B[i_0] = D[i_0-2][i_0] - E[i_0] + B[i_0-2] + 5;
        for (int i_1 = PB_L - 1; i_1 >= 0; i_1--) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                C[i_2-1][i_1][i_0] = C[i_0-1][i_1][i_2-1] + C[i_1][i_2][i_0] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3316312221_03(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
