...
void kernel_2432252225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[0] * E[i_0] - A[i_0-1] - A[i_0+1] * 2;
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            A[i_1] = F[i_1][0] * G[i_0-1][i_1] - F[i_1][i_0] * A[i_1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_L+1, PB_P); i_3++) {
            D[i_3] = E[i_3+i_2-1] + G[i_2][i_3] + F[i_3+i_2][i_2] * E[i_3] * 1;
            D[i_3] = G[i_2][i_3] - G[i_3][i_2] * G[i_2][i_3-1] * E[i_3-1] - G[i_2][i_3-1] + F[i_2-1][i_3] * H[i_3][i_3+i_2+1][1] * F[i_3+i_2+1][i_3+1] * G[i_2-1][i_3] + 5;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_2432252225_06(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
