...
void kernel_2424251225_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            B[i_1+i_0][i_1-1] = E[i_0] * F[i_1+i_0][i_0+1] * G[i_1][i_0] * 6;
            B[i_1+i_0][i_1-1] = H[i_0][i_1][0] - G[i_1+i_0][i_1-1] + F[i_1][i_0] + G[i_1][1] + B[i_1+i_0-2][i_1-3] * 6;
            B[i_1+i_0+1][i_1] = F[i_1-1][i_0] * E[i_1] - F[i_1-1][i_0] * F[i_1][i_0] + G[i_1+1][i_0] - F[i_0][0] + F[i_0][1] - G[i_1+i_0][0] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 3; i_3 < min(PB_P-1, -i_2+PB_M-1); i_3++) {
            B[i_3+i_2+1][i_2-1] = E[i_3] * I[i_2][i_3][i_3+1] * B[i_3+i_2-2][i_3-3] - 6;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2424251225_02(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
