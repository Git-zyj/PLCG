...
void kernel_2412252323_00(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            C[i_1+2] = E[i_1] - F[i_1+2][i_0+1] - F[i_0-2][i_0] + F[i_0][i_1] + F[i_1][i_0+1] + G[i_1][i_0] - C[i_1+2] - 4;
        }
        for (int i_2 = 2; i_2 < -i_0+PB_M; i_2++) {
            C[i_2+1] = E[i_2] + H[i_2] * I[i_0][i_2-2][0] * F[i_2+i_0-2][i_0] - G[i_2+i_0+1][i_0] - F[i_2+i_0][i_0] + 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        C[i_3+2] = H[2] + E[i_3-1] * H[i_3] - E[i_3-2] * E[i_3] * C[i_3+3] * 1;
        for (int i_4 = 0; i_4 < PB_P-2; i_4++) {
            D[i_3][i_4] = F[i_4][i_3] - C[i_4+2] + D[i_3][i_4+1] * D[i_3+1][i_4] - D[i_3+1][i_4+2] * 1;
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2412252323_00(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
