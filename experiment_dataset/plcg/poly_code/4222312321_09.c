...
void kernel_4222312321_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-3; i_1++) {
            B[i_1+i_0+3][i_0+2] = C[i_1-1][i_0] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_M-2; i_4++) {
                B[i_4+i_3][i_3] = B[i_4+i_3-1][i_3] - B[i_4+i_3+2][i_3] * B[i_4+i_3+2][i_4+2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4222312321_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
