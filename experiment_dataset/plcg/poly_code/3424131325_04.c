...
void kernel_3424131325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0] + E[i_0-1] * E[i_0] - E[i_0] - A[i_0+1] * A[i_0+2] * A[i_0-2] - A[i_0-1] - A[i_0+2] - A[i_0+2] * 4;
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                C[i_2][i_1][i_1] = F[i_0][i_1][i_2] * F[i_0-1][i_2][i_1] - F[i_0][i_2+i_1][i_1+i_0] * G[i_2][i_1] * 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            ...
            for (int i_6 = 2; i_6 < PB_P+1; i_6++) {
                C[i_6-2][i_3][i_3] = G[i_4][i_6-1] + G[i_4][i_6-1] * F[i_3+1][i_6][i_4] + F[i_6][i_4][i_3-1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3424131325_04(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
