...
void kernel_2426251325_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[1] - E[i_0] - E[i_0] + A[i_0+2] * A[i_0+1] * A[i_0-1] + 2;
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_1][i_0+1] = E[i_1] + F[i_1][i_0-1] + G[i_1-1][i_0] * H[i_0+1][i_1][i_0] * F[i_1][i_0+1] - A[i_0] - B[i_1-1][i_0+1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        A[i_2] = I[i_2] + E[i_2] + I[0] + I[i_2] + E[i_2] - I[i_2] - 1;
    }
    for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M-1; i_4++) {
            A[i_4] = J[i_4][i_3-1][i_4+1] + G[i_4][i_3] + F[i_4+1][i_3] - F[i_3-1][i_3] * B[i_4][i_3+1] - 5;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2426251325_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
