...
void kernel_4332251113_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0] * D[i_0] + E[i_0] - D[i_0] + D[i_0-1] - D[i_0] - D[i_0+1] + D[i_0-1] - 1;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            for (int i_3 = 2; i_3 < -i_1+PB_L; i_3++) {
                B[i_2-1][i_2] = F[i_2][i_3+i_1][i_3] * F[i_2][i_3][i_1] * B[i_3-2][i_3-1] * B[i_3][i_3+1] + 4;
            }
        }
        for (int i_4 = 1; i_4 < PB_M+1; i_4++) {
            A[i_4-1] = G[i_4-1][i_1] - D[i_4] - G[i_1-1][0] * F[i_1][i_4][i_1] - 2;
        }
    }
#pragma endscop
...
kernel_4332251113_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
