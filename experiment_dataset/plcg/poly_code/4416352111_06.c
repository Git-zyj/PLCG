...
void kernel_4416352111_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        A[i_0+2] = E[i_0+2] - F[i_0] * G[i_0-2] * A[i_0+3] + 2;
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_Q-2, -i_0+PB_M+1); i_2++) {
                for (int i_3 = 0; i_3 < min(i_2, -i_1+PB_P); i_3++) {
                    B[i_0][i_2][i_3+i_1][i_1] = H[i_3][i_2-2][i_1-1][i_0] + I[i_2+i_0-1][i_3+2][i_2+i_1+2] * J[i_1][i_0][i_1][i_2] - J[i_0][i_3][i_1][i_1] + 3;
                }
            }
        }
        for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_L-2; i_5++) {
                for (int i_6 = 2; i_6 < min(-i_0+PB_M, i_0); i_6++) {
                    C[i_0][i_4+2][i_4][i_6] = H[i_4-2][i_6][i_0][i_5-1] - K[i_5][i_0][i_6][i_6] + J[i_6][i_5-1][i_4][i_0] * L[i_6-1][i_5+i_4+2][i_4] + J[i_4][i_6+i_0][i_6-2][i_4] + 5;
                }
            }
        }
    }
        ...
        for (int i_8 = 0; i_8 < min(PB_M, i_7); i_8++) {
            for (int i_9 = 0; i_9 < min(i_8, PB_P-2); i_9++) {
                for (int j_0 = 1; j_0 < min(-i_9+PB_L, PB_M-1); j_0++) {
                    D[i_9][i_8][i_7][i_8] = L[i_8][j_0-1][i_9+2] - H[j_0+1][i_7-1][i_8][j_0+i_9] + M[i_7][i_8][j_0+1] + J[i_8][j_0][i_7][i_9] * K[i_7][j_0][i_8][i_7] * 6;
                }
            }
        }
    }
...
kernel_4416352111_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
