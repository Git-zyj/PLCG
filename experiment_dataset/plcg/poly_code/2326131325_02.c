...
void kernel_2326131325_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] - D[i_0] * D[i_0] - A[i_0+1] * A[i_0-2] - A[i_0-1] + 4;
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1] = E[i_0][i_1][i_1] * F[i_1][0] - F[i_1][i_0-1] - F[i_1-1][i_0] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
        C[i_2+1] = D[i_2] * D[i_2] + C[i_2] + C[i_2+2] + 2;
    }
#pragma endscop
...
kernel_2326131325_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
