...
void kernel_3226111121_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P, i_0); i_1++) {
            A[i_0+1] = C[i_1][i_1][i_0-1] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-3; i_2++) {
        for (int i_3 = 2; i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
                B[i_3][i_4+1][i_2] = B[i_4][i_2+3][i_3] + B[i_3-2][i_4+1][i_2] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3226111121_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
