...
void kernel_4222212323_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_0][i_2+1][i_1] = C[i_1][i_0][i_2+1] * A[i_0-2][i_2+1][i_1] + A[i_0][i_2+1][i_1-2] + A[i_0][i_2][i_1-2] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_L+1; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q; i_5++) {
                A[i_3][i_5+3][i_4] = D[i_4-1][i_3][i_5] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4222212323_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
