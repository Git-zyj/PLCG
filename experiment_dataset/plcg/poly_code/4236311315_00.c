...
void kernel_4236311315_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_L+1); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L-1; i_2++) {
                A[i_1][i_2+i_0][i_0+1] = A[i_1][i_2+i_0+1][i_0+2] - A[i_2][i_1+i_0-1][i_0+1] * A[i_1][i_2+i_0-1][i_0+1] + 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < PB_N-1; i_5++) {
                for (int i_6 = 0; i_6 < min(-i_3+PB_L, PB_P-1); i_6++) {
                    A[i_4-1][i_6+i_3][i_6+1] = C[i_5+1][i_3+1][1] + 5;
                }
            }
        }
    }
...
kernel_4236311315_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
