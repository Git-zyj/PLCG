...
void kernel_3216111323_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_P-2 - 1; i_0 >= 2; i_0--) {
        for (int i_1 = 2; i_1 < min(PB_L, i_0); i_1++) {
            A[i_1][i_0] = B[i_1][i_1] + B[i_1-2][i_1-2] - B[i_1-1][i_1-1] + 4;
            B[i_1][i_1] = C[i_1+1][i_0+1] + A[i_1][i_0] * A[i_1][i_0-2] + A[i_1-1][i_0+2] + 5;
        }
    }
#pragma endscop
...
kernel_3216111323_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
