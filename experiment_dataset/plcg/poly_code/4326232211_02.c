...
void kernel_4326232211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_1 = 2; i_1 < PB_Q-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_P+2; i_2++) {
            for (int i_3 = 2; i_3 < PB_N; i_3++) {
                B[i_1-1][i_3+2][i_2-2] = D[i_3][i_2-1][i_1] * D[i_3-2][i_1][i_2] * B[i_3-1][i_2+1][i_1-2] - B[i_2-1][i_3+1][i_1-2] + B[i_1][i_3+3][i_2-2] - 2;
            }
        }
        for (int i_4 = 2; i_4 < min(min(-i_1+PB_M, PB_N-2), i_1); i_4++) {
            for (int i_5 = 0; i_5 < -i_1+PB_N; i_5++) {
                C[i_1][i_5][i_4+1] = D[i_4+2][i_5+i_1][i_5] - D[i_5][i_4+i_1][i_1] * D[i_5+i_1][i_4+1][1] - D[i_1-1][i_5][i_4-2] * D[i_5][i_4][i_5+i_1] - C[i_5+1][i_1+1][i_4+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4326232211_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
