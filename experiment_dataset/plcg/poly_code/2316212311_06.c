...
void kernel_2316212311_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_L-3, i_1); i_2++) {
            B[i_2+2] = D[i_2][i_1] - B[i_2+3] + 5;
        }
        for (int i_3 = 0; i_3 < i_1; i_3++) {
            C[i_3][i_1] = C[i_3+1][i_1-1] - C[i_3+1][i_1] - C[i_3][i_1+1] + 2;
        }
    }
#pragma endscop
...
kernel_2316212311_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
