...
void kernel_3314312115_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
                B[i_2][i_2][i_3] = D[i_3][i_4][i_2] - B[i_3][i_3][i_4+1] - 5;
            }
        }
        for (int i_5 = i_2; i_5 < min(PB_L, -i_2+PB_P); i_5++) {
            C[i_5][i_5+i_2][0] = 4;
        }
    }
#pragma endscop
...
kernel_3314312115_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
