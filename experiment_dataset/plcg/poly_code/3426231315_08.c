...
void kernel_3426231315_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M-1; i_2++) {
                B[i_2] = E[i_2][i_1][i_0] * E[i_2][i_0][i_1] - F[i_2+i_1+1][i_0][i_2] * F[i_2+1][i_0][i_1+1] + 5;
            }
        }
        for (int i_3 = 1; i_3 < i_0; i_3++) {
            B[i_3] = B[i_3+1] * 3;
            for (int i_4 = 1; i_4 < -i_3+PB_Q; i_4++) {
                B[i_3] = E[i_3][i_0][i_4+1] + F[i_4-1][i_3+1][i_0+1] * G[i_3+1][i_0-1][i_4][i_4+i_3] + F[i_3][i_0][i_4] * E[i_3-1][i_4][i_0] * E[i_3-1][i_0][i_0-1] - B[i_4+1] - 2;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = max(1, i_5); i_6 < PB_M; i_6++) {
            for (int i_7 = i_5; i_7 < PB_M; i_7++) {
                B[i_6-1] = H[i_7][0] + I[i_7][i_6-1] * B[i_5] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3426231315_08(xb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
