...
void kernel_4234151315_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_N, PB_L-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_1+i_0][i_2][0] = C[i_1+1][i_2][i_0+1] * D[i_2][i_1+1] - C[i_1][i_0][i_0] - D[i_0][i_2] * A[i_1+i_0-1][i_2][0] + A[i_1+i_0-2][i_2][0] - 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        B[i_3+1][i_3-1] = E[i_3] * E[i_3] + E[0] - E[i_3] + E[i_3] * B[i_3+2][i_3] - B[i_3][i_3-2] - B[i_3+2][i_3] - 6;
    }
#pragma endscop
...
kernel_4234151315_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
