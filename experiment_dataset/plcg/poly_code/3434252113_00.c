...
void kernel_3434252113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1] = E[i_0] * F[i_1-2][i_0] + F[i_0][i_1] + G[i_0][i_1][i_0] * F[i_1+i_0][i_0] - H[i_1][i_0-1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        B[i_2] = E[i_2+1] - E[i_2] - B[i_2+1] - 5;
    }
    for (int i_3 = 2; i_3 < PB_M-3; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_M-2; i_4++) {
            for (int i_5 = 2; i_5 < -i_3+PB_M; i_5++) {
                D[i_4+1][i_5] = G[i_5-2][i_3-2][i_4] + I[i_4+1][i_3-1][i_5] + G[i_4][i_5+i_3][0] * H[i_4+i_3+1][i_5+i_4] - I[i_5+i_4][i_4-1][i_3-1] + D[i_4+2][i_5] + 4;
            }
            D[i_3+2][i_4] = F[i_4+i_3][i_4-1] * F[i_4][i_3] - H[i_4+i_3-1][i_4-1] + I[i_4+i_3+2][i_4][1] + H[i_4+i_3][0] * F[i_4][i_3] + D[i_3+3][i_4+1] - 3;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3434252113_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
