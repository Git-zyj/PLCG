...
void kernel_2436132115_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        B[i_2] = F[i_2] - F[i_2] - F[i_2] - F[i_2] - B[i_2+1] * B[i_2-1] * 3;
    }
    ...
    for (int i_5 = 0; i_5 < PB_L; i_5++) {
        for (int i_6 = max(1, i_5); i_6 < PB_M-1; i_6++) {
            D[i_6+1][i_6] = E[i_6+1][i_5] + E[i_6-1][i_5] + E[i_6][1] - 3;
        }
    }
#pragma endscop
...
kernel_2436132115_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
