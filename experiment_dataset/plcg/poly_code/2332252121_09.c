...
void kernel_2332252121_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M-1; i_0++) {
        A[i_0-2] = D[i_0-2] - E[i_0] - D[i_0+1] * E[i_0+2] - B[i_0-2] + B[i_0-3] + 5;
        B[i_0-2] = E[i_0] + E[0] - 4;
        for (int i_1 = 1; i_1 < -i_0+PB_M+1; i_1++) {
            A[i_0-4] = F[i_0][i_1] * G[i_1][i_0] * G[i_1+i_0-1][2] + H[i_1][i_1-1][i_0+2] * F[i_1][i_0] - F[i_0][i_1] * E[i_0-1] + D[0] + 4;
        }
    }
#pragma endscop
...
kernel_2332252121_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
