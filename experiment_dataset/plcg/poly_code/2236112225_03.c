...
void kernel_2236112225_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 3; i_1 < PB_N+2; i_1++) {
            A[i_0][i_1-2] = C[i_1][i_0+1] + A[i_0][i_1-3] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_M-1); i_3++) {
            B[i_2][i_3+i_2] = B[i_2+1][i_3+i_2+1] + B[i_2][i_3+i_2-1] * B[i_2][i_3+i_2-2] - 1;
        }
    }
#pragma endscop
...
kernel_2236112225_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
