...
void kernel_4336352213_07(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
        ...
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_L-2, -i_1+PB_N+1); i_2++) {
                for (int i_3 = 1; i_3 < i_2; i_3++) {
                    C[i_2][i_0-1] = D[i_2+i_1-1][i_0][i_3][i_2-1] + E[i_2][i_1+2][i_1] - F[i_1+2][i_2+2][i_0][i_2] - E[i_2][i_1+1][i_3-1] * E[i_3+1][i_1+2][i_2] * D[i_3][i_1][i_2][0] - G[i_2][i_1-2][i_0+2][i_1][i_3] + 1;
                }
            }
        }
        B[i_0] = H[i_0] * I[i_0-1] - J[i_0][i_0] * H[i_0+1] - B[i_0+1] + B[i_0-1] * 2;
        for (int i_4 = 0; i_4 < min(-i_0+PB_L, i_0); i_4++) {
            for (int i_5 = max(i_4, 1); i_5 < PB_M-1; i_5++) {
                C[i_4][i_5] = K[i_5][i_4+i_0] * L[i_5][i_0][i_4+2] * E[i_0-2][i_5][i_4] * C[i_4][i_5-1] * C[i_4][i_5+1] * 4;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4336352213_07(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
