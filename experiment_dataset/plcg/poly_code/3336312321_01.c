...
void kernel_3336312321_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_4 = 2; i_4 < PB_N+2; i_4++) {
        for (int i_5 = 4; i_5 < min(i_4, PB_N); i_5++) {
            C[i_4-2] = C[i_5-1] * C[i_5] * C[i_5-4] + C[i_5-3] - 6;
        }
    }
#pragma endscop
...
kernel_3336312321_01(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
