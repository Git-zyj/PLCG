...
void kernel_3334311213_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
    ...
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_M+1; i_2++) {
            for (int i_3 = max(1, i_1); i_3 < PB_M; i_3++) {
                C[i_3] = E[i_2-1][i_3-1][i_1] - 5;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M+1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            C[i_5] = F[i_4-1][i_5-1] - C[i_5+1] * C[i_5-1] + 4;
        }
    }
#pragma endscop
...
kernel_3334311213_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
