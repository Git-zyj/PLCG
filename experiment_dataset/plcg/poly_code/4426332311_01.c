...
void kernel_4426332311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] + A[i_0+1] - 5;
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = min(PB_L-1, -i_1+PB_Q) - 1; i_2 >= i_0; i_2--) {
                B[i_0+1][i_2][i_1] = F[i_2+1][i_0][i_1] * F[i_0-2][i_1+2][2] + G[i_0][i_2][0][i_2+i_1] - B[i_0][i_2][i_1] * B[i_0][i_2][i_1+1] - B[i_1+1][i_0-1][i_2-1] - 1;
            }
            for (int i_3 = 0; i_3 < i_1; i_3++) {
                for (int i_4 = 1; i_4 < min(-i_0+PB_Q+2, i_0); i_4++) {
                    D[i_0][i_1+1][i_3+1][i_3] = H[i_4-1][0][i_0][i_1] - H[i_1+1][i_3][i_4+i_0-2][1] + B[i_0+1][i_1+1][i_3] * 3;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 0; i_6 < PB_L-2; i_6++) {
            for (int i_7 = 0; i_7 < min(-i_6+PB_L, PB_M-2); i_7++) {
                D[i_5][i_6+1][i_7+1][i_7] = F[i_6][i_7][i_5] + I[i_6][i_7+1] + F[i_7+i_6][i_5][i_6] * D[i_7][i_6+1][i_5][i_5-1] + D[i_7][i_5+1][i_6+2][i_6+1] - D[i_5][i_7+2][i_6+2][i_6+1] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4426332311_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
