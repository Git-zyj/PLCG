...
void kernel_4134312125_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_Q-1, -i_1+PB_Q); i_2++) {
                for (int i_3 = i_0; i_3 < -i_2+PB_Q-1; i_3++) {
                    A[i_0][i_3][i_0][i_2+i_1] = B[i_0-2][i_3][i_2] - A[i_0][i_1][i_0][i_3+i_2+1] * 6;
                }
            }
        }
    }
...
kernel_4134312125_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
