...
void kernel_4336251221_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
        ...
        for (int i_1 = i_0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                    A[i_2][i_0-1][i_3][i_3] = D[i_2][i_3+1][i_1+1][i_2+1] * D[i_2][i_3][i_0][i_1] * E[i_3][i_2+i_1][i_3][i_0] * 3;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 1; i_5 < -i_0+PB_M; i_5++) {
                for (int i_6 = 0; i_6 < min(min(min(-i_5+PB_Q-1, -i_0+PB_L), i_5), -i_4+PB_Q); i_6++) {
                    B[i_4][i_6+i_0][i_5+1][i_6+i_5] = F[i_6][i_5-1][i_4-1] + E[i_6][i_5+i_0][i_6+i_4][i_5] + D[i_6][i_6][i_4][i_5] - D[i_0-1][i_5][i_4][i_6+i_4] - F[1][i_4][i_6+i_5-1] * E[i_6+i_0-1][i_5-1][i_6][i_4] * D[i_5][i_6][i_4-1][i_0] + B[i_4][i_6+i_0][i_5+2][i_6+i_5+1] - B[i_4+2][i_6+i_0-2][i_5+1][i_6+i_5] + 4;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
        for (int i_8 = i_7; i_8 < min(PB_M-1, -i_7+PB_M+1); i_8++) {
            C[i_7-1][i_8-1] = G[i_8+i_7-1][i_8] - G[i_8][0] + H[i_8] - G[i_7][0] * I[i_8][i_7] * C[i_7+1][i_8-1] * C[i_8-1][i_7] - C[i_8-1][i_7+1] - C[i_8+1][i_7+1] * 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4336251221_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
