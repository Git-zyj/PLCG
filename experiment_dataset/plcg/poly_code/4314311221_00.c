...
void kernel_4314311221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                A[i_0-1][i_2][i_1+i_0+1] = A[i_0-3][i_2-1][i_1+i_0-1] + A[i_0-3][i_2][i_1+i_0-1] + 5;
            }
        }
        for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
            for (int i_4 = i_0; i_4 < min(-i_3+PB_P-1, -i_0+PB_P-1); i_4++) {
                A[i_4-1][i_0][i_4+i_3+1] = D[i_0-1][i_3][i_4+i_0+1] + 5;
            }
        }
    }
...
kernel_4314311221_00(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
