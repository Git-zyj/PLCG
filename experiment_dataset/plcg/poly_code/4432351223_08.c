...
void kernel_4432351223_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
        ...
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
                for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
                    A[0][i_1][i_3+1] = E[i_3-1][i_2][i_3][i_0] - E[i_1-1][i_0-1][i_1][i_2+1] * E[i_3-1][i_2][i_1][i_2] * F[i_2-1][i_0-1][i_2] * A[0][i_1][i_3+2] + 3;
                }
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < min(i_4, PB_P-3); i_5++) {
            for (int i_6 = 1; i_6 < min(-i_5+PB_L, -i_4+PB_M); i_6++) {
                for (int i_7 = 1; i_7 < PB_L-2; i_7++) {
                    A[0][i_7+2][i_5+3] = G[i_7][i_6][i_4][i_5-1] - H[i_7][i_7][i_6][i_5] - E[i_6+i_4][i_7+1][i_6][i_5-1] * G[i_7+i_6][i_4+1][i_5+1][i_6+i_5] * H[i_5][i_7+i_6-1][i_4][i_6] * G[i_5][i_7][i_6][i_4] + I[i_6][i_5][i_7] - A[0][i_5-2][i_7-1] - 5;
                }
            }
        }
        for (int i_8 = 0; i_8 < PB_P; i_8++) {
            for (int i_9 = 0; i_9 < PB_M-1; i_9++) {
                C[i_4][i_4][i_8] = J[i_9+1][i_4][i_4] + K[i_9][i_8] + F[i_9][i_4][i_8] + J[i_8][i_4-1][i_9] - 4;
            }
        }
    }
    for (int j_0 = 1; j_0 < PB_M-3; j_0++) {
        D[j_0+1] = L[0] + M[j_0] - L[j_0] * D[j_0+3] * D[j_0] + D[j_0+2] * D[j_0-1] + 5;
    }
#pragma endscop
...
kernel_4432351223_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M));
