...
void kernel_3234112215_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            for (int i_4 = max(max(0, -i_3+1), -i_2-2); i_4 < min(-i_3+PB_L, -i_2+PB_Q-2); i_4++) {
                B[i_2][i_2][i_4+i_3] = C[i_3][i_2][i_4+i_2+2] * B[i_2][i_2][i_4+i_3-1] - B[i_2+1][i_2+1][i_4+i_3] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3234112215_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
