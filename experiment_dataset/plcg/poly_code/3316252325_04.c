...
void kernel_3316252325_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        B[i_0-2] = D[i_0+1] - D[i_0] - D[i_0] * E[i_0] + B[i_0] - 5;
        B[i_0] = D[i_0] * F[i_0][0] * E[i_0] - E[i_0] * G[i_0][i_0-2] + 5;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < min(PB_N-2, i_1); i_2++) {
            for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
                C[i_2][i_3][i_1] = H[i_1][i_1][i_2-1] * F[i_2+2][i_1-1] - I[i_2-2][i_3-1][i_1+2] + G[i_3][i_1] + H[i_3][0][i_1] * F[i_2][2] * C[i_2-1][i_3][i_1] + C[i_2][i_3+2][i_1] - C[i_2+1][i_3-1][i_1+2] + C[i_2][i_3-2][i_1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3316252325_04(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
