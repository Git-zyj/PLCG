...
void kernel_2412112221_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+2][i_0+2] = E[i_0] - A[i_0][i_0] * A[i_0+3][i_0+3] - 4;
        for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
            B[i_0][i_1+2] = 4;
        }
        C[i_0] = C[i_0+2] * 1;
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            D[i_0][i_2+2] = A[i_2+2][i_2+2] * B[i_0+2][i_2+2] - D[i_0+2][i_2+2] * 2;
        }
    }
#pragma endscop
...
kernel_2412112221_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
