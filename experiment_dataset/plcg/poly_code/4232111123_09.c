...
void kernel_4232111123_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_0][i_1][i_0+1] = C[i_1][i_2][i_0] * A[i_0+1][i_1][i_0+2] * 4;
            }
        }
        for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                A[i_4][i_0][i_4+1] = D[i_3+1][i_3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4232111123_09(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
