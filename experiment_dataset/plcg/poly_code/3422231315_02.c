...
void kernel_3422231315_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
        ...
        for (int i_2 = 1; i_2 < -i_1+PB_P-2; i_2++) {
            C[i_2-1][i_1][i_2+i_1+1] = G[i_2+1][i_1] + A[i_2+1] * C[i_2-1][i_1+1][i_2+i_1+2] * C[i_1-1][i_2+1][i_2+i_1+2] * C[i_2-1][i_1-1][i_2+i_1] + 6;
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 2; i_4 < -i_1+PB_M; i_4++) {
                C[i_4-2][i_1-1][i_4+i_1-1] = H[i_4-1][i_1][i_4] - I[i_3][i_4+i_1][i_3] + H[i_3][i_1][i_3] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3422231315_02(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
