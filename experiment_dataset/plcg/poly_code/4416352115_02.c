...
void kernel_4416352115_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm),int xn,int yn,int zn,int vn,int wn,DATA_TYPE POLYBENCH_5D(N,xN,yN,zN,vN,wN,xn,yn,zn,vn,wn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0+2][i_1+1] = E[i_1][i_0+1][i_1] * F[i_1][i_0+2] + 2;
        }
    }
        ...
        for (int i_3 = 2; i_3 < min(-i_2+PB_L, PB_M); i_3++) {
            for (int i_4 = i_3; i_4 < PB_Q-2; i_4++) {
                for (int i_5 = 1; i_5 < -i_4+PB_N+2; i_5++) {
                    B[i_3][i_2+2][i_3-2][i_5-1] = E[i_5+i_4-2][i_3+i_2][i_4+2] * G[i_5][i_3][i_3] - H[i_3][i_5][i_2][i_4-1] * 5;
                }
            }
        }
        C[i_2][i_2] = I[i_2] + I[i_2] * I[i_2] * J[i_2] * I[i_2] + K[i_2] + K[i_2] * K[i_2] + J[i_2] + C[i_2-1][i_2-1] * 4;
        for (int i_6 = 2; i_6 < PB_M; i_6++) {
            for (int i_7 = 1; i_7 < min(min(-i_6+PB_M, i_2), -i_2+PB_P); i_7++) {
                for (int i_8 = 1; i_8 < PB_N; i_8++) {
                    B[i_6][i_2+2][i_6-2][i_7] = L[i_7+2][i_2+1][i_8-1][i_6] + M[i_8][i_2][i_6-2][i_7-1] - M[i_6][i_7+1][i_6][i_7+i_2] * G[i_7][i_6][i_2] * N[i_2][i_6+1][i_8][i_7][i_7] - H[i_7+i_6][i_8][i_2][0] + 4;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,wn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4416352115_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,wn,POLYBENCH_ARRAY(N));
