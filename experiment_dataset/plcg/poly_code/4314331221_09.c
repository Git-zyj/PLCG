...
void kernel_4314331221_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
        ...
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < i_2; i_3++) {
                    A[i_0+1][i_1][i_2][i_3-1] = D[i_2][i_1-1][i_0][i_3-1] - E[i_1][i_2+1][i_0+1][i_2-1] - 1;
                }
            }
        }
    }
    for (int i_4 = PB_L-3 - 1; i_4 >= 0; i_4--) {
        B[i_4+1] = F[i_4] - F[i_4] * G[i_4] + B[i_4+3] + 2;
        for (int i_5 = 2; i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_Q-1; i_6++) {
                for (int i_7 = i_6; i_7 < min(PB_M, -i_4+PB_P); i_7++) {
                    C[i_5][i_6][i_6+1][i_7] = H[i_5][i_7][i_5][i_7] + E[i_6][i_6][i_5][i_7+i_4] + E[i_4][i_6][i_7][i_5] - D[i_5][i_6][i_4][i_5+1] * A[i_4+1][i_7][i_6-1][i_5-2] * C[i_5][i_6-1][i_6][i_4] + C[i_5-2][i_6][i_6+1][i_7] + 5;
                }
            }
        }
    }
...
kernel_4314331221_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
