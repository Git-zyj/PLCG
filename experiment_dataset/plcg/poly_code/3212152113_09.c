...
void kernel_3212152113_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            B[i_0+1][i_0-1] = C[i_1][i_0] * C[i_1][i_0+2] - C[i_1][i_0] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 3; i_3 < min(PB_L-1, -i_2+PB_M); i_3++) {
            B[i_2][i_2-2] = C[i_3+i_2][0] - D[i_3+1] * C[i_3+2][i_2] + C[i_3][i_2] - C[i_2][i_3] * C[i_3][i_2] + C[i_2][i_3] + B[i_3-1][i_3-3] - 2;
        }
    }
#pragma endscop
...
kernel_3212152113_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
