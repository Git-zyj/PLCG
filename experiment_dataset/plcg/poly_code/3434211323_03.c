...
void kernel_3434211323_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_0+1][i_1] = E[i_0] - A[i_0-1][i_1] - A[i_0+1][i_1-2] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
            C[i_3-1] = F[i_2][i_3-1] * 5;
        }
    }
    for (int i_4 = 3; i_4 < PB_L; i_4++) {
        C[i_4-1] = G[i_4] - G[i_4+1] * C[i_4-2] * C[i_4-3] * C[i_4] - 6;
    }
    for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
        for (int i_6 = 1; i_6 < PB_M-2; i_6++) {
            for (int i_7 = i_5; i_7 < PB_L-2; i_7++) {
                D[i_6][i_7][i_5] = D[i_6][i_7+2][i_5] + D[i_6][i_7+1][i_5] + D[i_6+2][i_5+2][i_7] - D[i_6-1][i_7][i_5-2] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3434211323_03(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
