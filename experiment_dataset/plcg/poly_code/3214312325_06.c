...
void kernel_3214312325_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 3; i_2 < min(PB_P, -i_0+PB_L); i_2++) {
                A[i_1][i_2][i_0-2] = C[i_2+i_0][i_1-2][0] - A[i_1-2][i_0][i_2-3] + A[i_1][i_2][i_0] + A[i_1][i_2][i_0-3] * 2;
            }
        }
        for (int i_3 = i_0; i_3 < PB_M+2; i_3++) {
            for (int i_4 = 2; i_4 < PB_L; i_4++) {
                A[i_3-2][i_4][i_0] = D[i_0][i_0][i_4-2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3214312325_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
