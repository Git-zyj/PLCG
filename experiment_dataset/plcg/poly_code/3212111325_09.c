...
void kernel_3212111325_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            for (int i_2 = i_1; i_2 < min(min(PB_M-3, -i_0+PB_M-2), -i_1+PB_M-2); i_2++) {
                A[i_1+1][i_2+i_1+1] = C[i_1][1] + A[i_1+2][i_2+i_1+2] * A[i_2+3][i_2+i_0+2] + A[i_2+1][i_2+i_0] * A[i_1][i_2+i_1+1] - 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        B[i_3-1] = D[i_3] + B[i_3+1] + 6;
    }
#pragma endscop
...
kernel_3212111325_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
