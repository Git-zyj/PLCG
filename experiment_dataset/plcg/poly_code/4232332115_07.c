...
void kernel_4232332115_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,int wc,DATA_TYPE POLYBENCH_5D(C,xC,yC,zC,vC,wC,xc,yc,zc,vc,wc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_L; i_2++) {
                for (int i_3 = 2; i_3 < PB_Q-2; i_3++) {
                    A[i_0][i_3][i_3+i_2-1][i_1] = C[i_0][i_3-2][i_2+i_1][i_3+2][i_0+2] - D[i_0+2][i_2][i_2][i_1-1] + A[i_0-1][i_1][i_2+i_1][i_3] - 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_Q-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
            for (int i_6 = i_4; i_6 < min(min(PB_Q-1, -i_5+PB_M), -i_4+PB_L); i_6++) {
                for (int i_7 = 0; i_7 < PB_Q; i_7++) {
                    B[i_4][i_6+1][2][i_5] = E[i_6+i_5][i_4][i_5+1][i_4+1] - F[i_7][i_6+i_4][i_5-1][i_6+1] - G[i_7][i_5-1][i_6] + B[i_4-1][i_6+1][2][i_5] * 3;
                }
            }
        }
    }
...
kernel_4232332115_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,wc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
