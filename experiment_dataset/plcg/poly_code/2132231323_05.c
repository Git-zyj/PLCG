...
void kernel_2132231323_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(PB_P+1, -i_0+PB_M-1); i_1++) {
            A[i_1][i_0-1] = B[i_1+i_0][i_0+1] + C[i_1+i_0+1] - D[i_1][i_0-1][i_1-1] * A[i_1-2][i_0+1] - A[i_1+2][i_0+1] * A[i_1][i_0+1] - 5;
        }
    }
#pragma endscop
...
kernel_2132231323_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
