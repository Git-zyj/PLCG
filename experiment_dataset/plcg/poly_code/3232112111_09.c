...
void kernel_3232112111_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 3; i_4 < min(PB_L+2, i_3); i_4++) {
            for (int i_5 = 2; i_5 < min(PB_P+1, -i_4+PB_M); i_5++) {
                B[i_4-2][i_5-1][i_3] = C[i_5+i_4][i_3][i_3+1] + B[i_4-3][i_5-2][i_3] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3232112111_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
