...
void kernel_2434252225_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        B[i_0+1] = E[i_0+2] * E[i_0-2] - E[i_0] * 4;
    }
    for (int i_1 = 2; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_M; i_2++) {
            B[i_2+1] = F[i_2-2][i_2] - G[i_2+i_1][0] * H[0] * I[i_2][i_2+2][i_1+2] - G[i_1][0] * B[i_1-1] - 1;
        }
        for (int i_3 = 0; i_3 < min(PB_M-3, -i_1+PB_N); i_3++) {
            B[i_3+3] = H[i_1-2] - F[i_1][i_3] - F[i_3][i_1] - H[i_3+i_1] - B[i_1] - 2;
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        B[i_4+2] = H[i_4-2] - H[i_4+2] - H[i_4] + E[2] * H[i_4] * 5;
    }
#pragma endscop
...
kernel_2434252225_07(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
