...
void kernel_4212111321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_P-1 - 1; i_0 >= 1; i_0--) {
        A[i_0+1] = A[i_0] - A[i_0+2] * A[i_0-1] + A[i_0+3] + A[i_0+3] + 2;
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_0][i_2][i_0-1] = C[i_2][i_0][i_1] - B[i_0+2][i_2][i_0+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4212111321_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
