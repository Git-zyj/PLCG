...
void kernel_2216111311_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_N; i_3++) {
            B[i_2-1][i_3] = C[i_3] - D[i_2][i_3] - B[i_2-1][i_3-1] + B[i_2-1][i_3+1] + B[i_2-2][i_3-1] + B[i_2][i_3-1] * 2;
        }
    }
#pragma endscop
...
kernel_2216111311_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
