...
void kernel_2334232121_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1] = D[i_1+1] * E[i_1][i_1] + D[i_1] * D[i_1] - F[i_1+1] + 6;
        B[i_1] = D[i_1-1] + F[i_1+1] + D[i_1] * D[i_1+1] * 6;
    }
#pragma endscop
...
kernel_2334232121_04(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
