...
void kernel_4326151125_07(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 2; i_3 < i_1; i_3++) {
                    C[i_2][i_3-2] = D[i_2-1][i_1][i_3][i_0] + D[i_2][i_0][i_3+i_1][i_3-1] - D[i_3][i_1+i_0+1][i_0][i_2] + D[i_0][i_2][i_3+i_1-1][0] - E[i_3-1][i_1][i_2] + 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N-2; i_4++) {
        B[i_4] = F[i_4-1] * F[i_4+1] - F[i_4+1] - F[i_4-1] + B[i_4+2] * B[i_4-1] + 1;
        for (int i_5 = 1; i_5 < -i_4+PB_L-1; i_5++) {
            for (int i_6 = 1; i_6 < i_5; i_6++) {
                C[i_6][i_4] = D[i_5-1][i_5][i_4][i_6] * G[i_5+1][1] * G[i_4-1][i_5] - E[i_6-1][i_5-1][i_4] * E[i_6][i_5+i_4+1][i_6-1] - G[i_6][0] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4326151125_07(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
