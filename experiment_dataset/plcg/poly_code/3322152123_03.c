...
void kernel_3322152123_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L-3; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L-2; i_2++) {
                A[i_2][i_2+i_1+2] = D[i_2][i_1][i_0] + D[i_2][i_2][i_1] + A[i_0-1][i_1+i_0+3] + A[i_1-2][i_2+i_1] - A[i_2][i_2+i_1+1] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            B[i_3][i_4] = E[i_4+2][0] - E[i_3][i_4] + E[i_3][i_4] * E[i_4-1][i_3-1] - F[i_3] + 3;
        }
    }
    for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
        for (int i_6 = 0; i_6 < -i_5+PB_L-2; i_6++) {
            C[i_6][i_6+i_5] = E[i_5][i_6+i_5+2] * F[i_6] + E[i_5][i_6] + E[i_6+i_5-2][i_5] * E[i_6][i_5] - F[i_6] - 1;
        }
    }
#pragma endscop
...
kernel_3322152123_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
