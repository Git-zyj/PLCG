...
void kernel_4324211321_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_M-2); i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_M; i_2++) {
                A[i_2][i_1+1][i_2+i_0] = A[i_2-2][i_1+1][i_2+i_0-1] + A[i_2][i_1+2][i_2+i_0-1] * A[i_0][i_1+2][i_2+i_0] + 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_M-1, i_3); i_4++) {
            B[i_3][i_4] = D[i_3][i_4] - B[i_3][i_4+1] - B[i_3-1][i_4-2] - 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = 2; i_6 < min(i_5, PB_M-1); i_6++) {
            for (int i_7 = 1; i_7 < -i_5+PB_M; i_7++) {
                C[i_5-1][i_6][i_5] = A[i_5-1][i_6+1][i_7+i_5] - C[i_7][i_6][i_7+1] * C[i_7-1][i_6-2][i_7] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4324211321_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
