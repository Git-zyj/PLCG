...
void kernel_3332251125_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0][1] + E[i_0] - E[i_0] * F[i_0] + F[0] * 1;
    }
    for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
            C[i_2+1] = D[i_2-1][i_1] + G[i_2-1][i_1] * H[i_1][i_2-1][i_1+1] - G[i_2][1] - C[i_2+1] * 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        C[i_3+1] = E[i_3] * E[i_3] * E[0] - F[i_3-1] - F[i_3-1] * F[i_3] * C[i_3-1] + 1;
    }
#pragma endscop
...
kernel_3332251125_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
