...
void kernel_4426351325_07(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_L-1; i_2++) {
                for (int i_3 = max(2, i_1); i_3 < min(-i_0+PB_M, -i_1+PB_M); i_3++) {
                    B[i_1+1][i_2+1][i_0+1][i_3-2] = E[i_3][i_2+i_1+1][i_0] - F[i_3+i_0][i_1+1][i_2][1] * G[i_3+i_1][i_0][i_2-1][1] + F[i_2][i_1-1][i_0+1][i_3] * 5;
                }
                for (int i_4 = 1; i_4 < min(-i_0+PB_L, i_0); i_4++) {
                    B[i_4+1][i_2][i_0-1][i_1] = H[i_2][i_4+i_0][i_1][i_1] - G[i_1][i_4][i_0-1][i_2] * F[i_2][i_0][i_1][0] - I[i_1][i_2][i_0][i_4][i_4-1] * B[i_4+1][i_2][i_0][i_1] - B[i_4+1][i_1-1][i_0-3][i_2-2] - B[i_4+1][i_2+2][i_0-1][i_1-1] + B[i_4+1][i_2+1][i_0][i_1] + B[i_4+1][i_2][i_0-3][i_1+2] - 6;
                }
            }
        }
        C[i_0] = J[i_0] + K[i_0+1] - L[i_0] + K[i_0] * J[i_0+1] - K[i_0] + J[i_0-1] * C[i_0+1] + 4;
    }
    for (int i_5 = 0; i_5 < PB_Q; i_5++) {
        for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
            for (int i_7 = 0; i_7 < PB_L; i_7++) {
                for (int i_8 = 1; i_8 < -i_6+PB_P; i_8++) {
                    B[i_6+1][i_7][i_8-1][i_5] = M[i_8+1][i_7][i_6-1] - G[i_7][i_8+i_6][i_5][i_7] + M[i_6][i_8][i_8] + 4;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4426351325_07(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M)));
