...
void kernel_4416251121_02(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        D[i_0+1] = E[i_0+1] * E[i_0] - E[0] * F[i_0+1] - E[i_0+1] + E[i_0] * F[i_0-1] * 6;
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_M; i_3++) {
                    B[i_3+i_1][i_2][i_0-1][i_3] = G[i_1][i_0][i_3][i_2] - H[i_2][i_0][i_1] + I[i_0][i_3][i_1][i_2] * G[i_2][i_1+1][i_0][1] * 6;
                }
            }
        }
        for (int i_4 = 0; i_4 < -i_0+PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < i_4; i_5++) {
                for (int i_6 = 0; i_6 < i_5; i_6++) {
                    C[i_6+1][i_5+i_4+1][i_4+i_0][i_6+1] = I[i_4][i_6][i_0][i_5+1] + I[i_4][i_4+1][i_0][i_5] * 1;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
        D[i_7+1] = F[i_7] * F[i_7] - F[i_7] * F[i_7] * E[i_7-1] + D[i_7-1] + 2;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4416251121_02(xd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
