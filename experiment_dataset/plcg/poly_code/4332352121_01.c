...
void kernel_4332352121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0+1][i_0] = D[i_0-2] - E[i_0-2] * F[i_0][0] + E[i_0+2] - F[i_0][i_0] - A[i_0-1][i_0-2] * 3;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            A[i_1+1][i_1] = G[i_1+2][i_2] * F[i_2][i_1] + H[i_2] + F[i_2][i_1] + 2;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_M; i_4++) {
            A[i_4][i_4-1] = I[i_3][i_4] - D[i_4-2] + I[i_4+i_3][2] + F[i_4][i_3-2] + I[i_4-1][i_3] - 1;
        }
    }
#pragma endscop
...
kernel_4332352121_01(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
