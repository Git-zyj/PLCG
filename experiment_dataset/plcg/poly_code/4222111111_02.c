...
void kernel_4222111111_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_M+1, PB_M-1); i_3++) {
            B[i_3][i_2] = C[i_3+i_2-1][i_3] - C[i_3-1][i_2] - B[i_3+1][i_2] * B[i_3][i_2+1] * 1;
        }
    }
#pragma endscop
...
kernel_4222111111_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
