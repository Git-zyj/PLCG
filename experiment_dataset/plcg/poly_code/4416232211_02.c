...
void kernel_4416232211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[i_0] - E[i_0] + F[i_0+2] * F[i_0] - A[i_0] + 6;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            B[i_2][i_1] = G[i_1][i_2] - G[i_1][i_2] * H[i_2][i_1] + A[i_2+1] * 2;
            for (int i_3 = i_2; i_3 < PB_P-1; i_3++) {
                B[i_3][i_2+1] = I[i_2+2][i_1][i_3+1] - I[i_3][i_1-1][i_2] - 4;
            }
        }
        D[i_1+1] = E[i_1] + D[i_1+2] * D[i_1] + 3;
    }
#pragma endscop
...
kernel_4416232211_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
