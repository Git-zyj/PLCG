...
void kernel_4424332221_02(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_M+2); i_1++) {
            for (int i_2 = 2; i_2 < min(min(-i_1+PB_M, PB_Q-1), -i_0+PB_P); i_2++) {
                for (int i_3 = 2; i_3 < -i_0+PB_M; i_3++) {
                    D[i_1+i_0-2][i_3+2][i_1-2][0] = E[i_3-2][i_3+i_1+1][i_2+1][i_0] - E[i_3][i_0-1][i_1][i_0] + F[i_1][i_3][i_0][i_2] - F[i_2+i_1][i_0][i_1][i_3+i_2-2] - E[i_3][i_2+i_0][i_3][i_1] * G[i_2][i_3+i_0][i_3][i_1] + 3;
                }
            }
        }
        B[i_0] = H[i_0-2] + H[i_0] + 1;
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        C[i_4+1] = I[i_4+1] + H[2] + J[i_4][i_4] - H[i_4-1] * C[i_4] - 3;
    }
    for (int i_5 = 2; i_5 < PB_M-3; i_5++) {
        for (int i_6 = 5; i_6 < -i_5+PB_M; i_6++) {
            for (int i_7 = 3; i_7 < PB_M-4; i_7++) {
                D[i_6+i_5-2][i_7+2][i_6-2][0] = D[i_7+i_6-5][i_5+3][i_7-3][0] * D[i_6+i_5-2][i_7+4][i_6-2][0] * D[i_7+i_6-2][i_5+1][i_6-2][0] + D[i_6+i_5][i_7+4][i_6][0] * D[i_6+i_5-1][i_7+2][i_6-1][0] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4424332221_02(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
