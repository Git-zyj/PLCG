...
void kernel_2336312113_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_N-1; i_2++) {
            C[i_2+i_1+1][i_2] = A[i_2] * A[i_2-1] * 4;
        }
...
kernel_2336312113_02(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
