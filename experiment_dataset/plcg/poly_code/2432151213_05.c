...
void kernel_2432151213_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_P; i_1++) {
            B[i_1] = E[i_1][i_0+1] + F[i_1-1] + E[i_1][i_0] - E[i_1][i_1+i_0] * B[i_0-1] * 3;
        }
        B[i_0-1] = F[i_0] * F[i_0+1] + B[i_0] - 5;
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_L+1, i_2); i_3++) {
            D[i_3] = E[i_3-1][i_2] * E[i_2][i_3+1] * F[i_3+1] - E[i_2+1][i_3] - F[i_2+1] - E[i_3+i_2-1][i_3] * 6;
        }
    }
    for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_L; i_5++) {
            D[i_5] = E[i_5][i_4] * E[i_5][i_4+1] - F[i_4+1] - E[i_5+i_4][i_5] + E[i_5][i_4-1] - D[i_5+1] - D[i_5-1] + 4;
        }
    }
#pragma endscop
...
kernel_2432151213_05(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
