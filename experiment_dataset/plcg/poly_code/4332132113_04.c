...
void kernel_4332132113_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_L, PB_P-1); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_P-1; i_3++) {
                    A[i_2+i_0][i_3][i_0][i_1] = D[i_2+1][i_1+i_0][i_0][i_1-2] + D[i_2][i_0][i_3+i_1+1][0] + E[i_3][i_1][i_2] - 6;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
            C[i_4+1][i_5+1][i_5+1] = F[i_5][i_4-2] * 2;
        }
    }
    for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
        for (int i_7 = 0; i_7 < PB_L-2; i_7++) {
            for (int i_8 = 0; i_8 < min(-i_6+PB_M, PB_N-2); i_8++) {
                C[i_6][i_8][i_8] = F[i_8+2][i_7+1] - E[i_8][i_7][i_6+1] * E[i_8+i_6][i_7+2][i_8+1] * E[i_7][i_8][i_6] * F[i_6][i_7+1] - C[i_6][i_8+1][i_8+1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4332132113_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
