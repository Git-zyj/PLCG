...
void kernel_4336212323_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_1] = D[i_0][i_1] + C[i_0-3] + 6;
        }
        B[i_0] = C[i_0-3] - 1;
        for (int i_2 = max(i_0, 4); i_2 < PB_N+1; i_2++) {
            C[i_2-2] = C[i_2-3] - C[i_2-1] + C[i_2-4] * 5;
        }
    }
#pragma endscop
...
kernel_4336212323_09(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
