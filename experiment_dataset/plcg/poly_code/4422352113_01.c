...
#endif
static void init_array(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,int zm,int vm,int wm,DATA_TYPE POLYBENCH_5D(M,xM,yM,zM,vM,wM,xm,ym,zm,vm,wm),int xn,int yn,int zn,int vn,int wn,DATA_TYPE POLYBENCH_5D(N,xN,yN,zN,vN,wN,xn,yn,zn,vn,wn),int seed)
{
srand(seed);
...
void kernel_4422352113_01(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,int zm,int vm,int wm,DATA_TYPE POLYBENCH_5D(M,xM,yM,zM,vM,wM,xm,ym,zm,vm,wm),int xn,int yn,int zn,int vn,int wn,DATA_TYPE POLYBENCH_5D(N,xN,yN,zN,vN,wN,xn,yn,zn,vn,wn)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P+2; i_2++) {
                for (int i_3 = 2; i_3 < -i_0+PB_L-1; i_3++) {
                    D[i_3][i_2][i_0+1][i_1] = E[i_3+i_0+1][i_2+i_1-2][i_3][1] * E[i_3-2][i_0][i_1][i_2+2] + F[i_1][i_0][i_3-1][i_2] * G[i_3][i_1][i_3-1] - 2;
                }
            }
            for (int i_4 = 2; i_4 < PB_P; i_4++) {
                for (int i_5 = 1; i_5 < -i_4+PB_P; i_5++) {
                    B[i_1][i_4][i_5] = H[i_5+i_4][i_1+2][2] + E[i_0-1][i_4][i_5][i_5] * E[i_1][i_5][i_5-1][i_0-1] + E[i_5][i_1][i_4-2][i_0] + I[i_1][i_0][i_5+i_4][i_4+1][i_1-1] + 3;
                }
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_P-2; i_6++) {
        for (int i_7 = 2; i_7 < -i_6+PB_L+2; i_7++) {
            C[i_7] = J[i_7-1][i_6] * K[i_7+i_6-2][i_6] + L[i_7-2][i_6+2] * L[i_7][i_6] * J[i_7][0] * 6;
        }
    }
    for (int i_8 = 2; i_8 < PB_P-1; i_8++) {
        for (int i_9 = 2; i_9 < PB_Q-2; i_9++) {
            for (int j_0 = 2; j_0 < -i_9+PB_L; j_0++) {
                for (int j_1 = j_0; j_1 < -j_0+PB_N+1; j_1++) {
                    D[i_8][j_1][j_0][i_9] = I[j_0+i_9][i_8-2][j_0][j_1][0] - M[i_9][j_0+1][j_1][i_8-1][0] - F[j_0-2][j_1-1][i_8][i_9] + N[i_9][j_1][j_0][i_8][i_9+2] + F[j_1+j_0-1][i_9-2][i_8+1][j_1] * D[i_8][j_1-1][j_0][i_9] + D[i_8][j_1][j_0+1][i_9] - 3;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,wn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4422352113_01(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,wn,POLYBENCH_ARRAY(N));
