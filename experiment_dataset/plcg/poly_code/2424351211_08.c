...
void kernel_2424351211_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0] + F[i_0] + F[i_0+1] * G[i_0] + A[i_0+1] + 1;
        A[i_0] = E[i_0-1] + E[i_0] + E[i_0+1] * F[i_0] + F[i_0] + A[i_0+1] + D[i_0+1] + 1;
    }
    for (int i_1 = 1; i_1 < PB_Q; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            C[i_2][i_1][i_1-1] = H[i_1+1][i_2+1] + F[i_1+1] + G[i_2] - G[i_2] + I[i_1][i_2] - C[i_2][i_1+1][i_1] + C[i_1][i_2+1][i_2] + 2;
        }
        D[i_1+1] = G[i_1] - G[i_1] * E[i_1] * G[i_1] * E[i_1] * 1;
    }
#pragma endscop
...
kernel_2424351211_08(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
