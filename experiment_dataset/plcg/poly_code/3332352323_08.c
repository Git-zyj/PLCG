...
void kernel_3332352323_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0] - E[i_0] - D[i_0-2] * E[i_0] * E[i_0-2] * E[i_0] * F[i_0] - D[i_0] * A[i_0-1] * A[i_0-2] - 2;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        A[i_1-1] = E[i_1+1] * D[i_1] + G[i_1][i_1] - A[i_1] - 6;
        for (int i_2 = 0; i_2 < -i_1+PB_N; i_2++) {
            for (int i_3 = 0; i_3 < min(min(-i_2+PB_N, -i_1+PB_M-2), PB_L-1); i_3++) {
                C[i_3+i_2][i_1][i_3] = H[i_3][i_3][i_1] - H[i_3+i_1+2][i_3+1][i_2] + G[i_2][0] - H[i_2+i_1][i_3][i_1] - C[i_2+i_1][i_3+1][i_1] - C[i_3+i_2][i_1-2][i_3] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3332352323_08(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
