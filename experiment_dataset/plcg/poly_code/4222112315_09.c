...
void kernel_4222112315_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                for (int i_3 = 1; i_3 < PB_L; i_3++) {
                    A[i_1-1][i_3-1][i_2] = A[i_2-1][i_3][i_0] + A[i_2-1][i_3][i_1-1] - A[i_1-2][i_3][i_2] - 1;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_L-2); i_5++) {
            B[i_5+1][i_4-2][i_5] = C[i_5][i_4] - B[i_5+2][i_4-2][i_5+1] + B[i_5+1][i_4-3][i_5] * B[i_5][i_4-2][i_5-1] - 2;
        }
    }
#pragma endscop
...
kernel_4222112315_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
