...
void kernel_3236112123_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_N, -i_0+PB_L+1); i_1++) {
            for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
                B[i_1][i_2+2] = C[i_2][i_1+i_0-1] + 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            B[i_3][i_4+2] = B[i_3-1][i_4+2] * 1;
        }
    }
#pragma endscop
...
kernel_3236112123_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
