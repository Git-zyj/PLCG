...
void kernel_4436331121_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_1] = E[i_0][i_1] + F[i_1][i_1] * F[i_1][i_0-1] * A[i_1-2] - 1;
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_L+1; i_3++) {
                    B[i_0+1][i_1][i_2][i_2] = G[i_2][i_3][i_1-1] + H[i_3+i_0-1][i_1][i_2][0] * 1;
                }
            }
        }
        for (int i_4 = max(2, i_0); i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < PB_P+1; i_5++) {
                C[i_4][i_5] = I[i_5][i_5-1][i_0+1][i_0-1] - J[i_5-1][i_0][i_4] - C[i_4-2][i_5] * 1;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
        for (int i_7 = 1; i_7 < i_6; i_7++) {
            for (int i_8 = i_7; i_8 < -i_6+PB_L-1; i_8++) {
                D[i_6][i_7][i_8] = J[i_6][i_7][i_8+i_6+1] * K[i_8+1][i_7-1][i_6+1] - D[i_7][i_6-2][i_8+1] + 3;
            }
        }
    }
#pragma endscop
