...
void kernel_3236112115_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_Q-2, -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                A[0][i_1][i_0][i_2+1] = C[i_1][i_2][i_1+i_0] + A[0][i_2][i_0][i_1+2] - 2;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            B[i_0][i_3] = D[i_0][i_3] * 1;
        }
    }
#pragma endscop
...
kernel_3236112115_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
