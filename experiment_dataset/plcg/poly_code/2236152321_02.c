...
void kernel_2236152321_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M - 1; i_0 >= 1; i_0--) {
        for (int i_1 = max(i_0, 4); i_1 < -i_0+PB_M; i_1++) {
            A[i_1-2][0] = C[i_1][i_0-1] * C[i_1+i_0][i_0] * D[i_1] * D[i_1+1] - A[i_1-3][0] - A[i_1-1][0] + 1;
            A[i_1-2][0] = C[i_1][i_0-1] - C[i_1][i_0] - C[i_1][i_0-1] * D[i_1] * C[i_1][2] * D[i_1] * A[i_1-4][0] * 5;
        }
    }
#pragma endscop
...
kernel_2236152321_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
