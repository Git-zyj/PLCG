...
void kernel_4436111325_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
        ...
        for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = i_0; i_2 < PB_N; i_2++) {
                for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
                    A[i_0-1][i_3][i_2][i_2] = E[i_2][i_3-1][i_0-1][i_1+1] + A[i_0-1][i_3][i_2+1][i_2+1] - A[i_0-2][i_3][i_2][i_2] * 5;
                }
            }
        }
        B[i_0] = F[i_0] + F[0] + B[i_0+2] + B[i_0-1] - B[i_0-2] - B[i_0+1] + B[i_0+1] * 5;
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < PB_M+1; i_5++) {
            for (int i_6 = 0; i_6 < min(min(-i_4+PB_N, i_5), PB_M-1); i_6++) {
                for (int i_7 = 0; i_7 < -i_6+PB_L; i_7++) {
                    D[i_4+1][i_5][i_6][i_6+1] = E[i_6+i_4][i_5-1][i_7+i_6][i_5] * D[i_6+1][i_4][i_5][i_5+1] - 4;
                }
            }
        }
        for (int i_8 = 0; i_8 < PB_L-1; i_8++) {
            for (int i_9 = i_8; i_9 < PB_P; i_9++) {
                for (int j_0 = 0; j_0 < PB_P; j_0++) {
                    D[i_4+1][i_8][i_9][i_9+1] = D[i_4+1][i_8+1][j_0][j_0+1] - 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4436111325_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F)));
