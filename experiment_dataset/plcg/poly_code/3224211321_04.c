...
void kernel_3224211321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_Q; i_1++) {
            A[i_1] = C[i_1+i_0][i_1+i_0][1] * A[i_1-2] * A[i_1+2] + A[i_1+1] + 2;
        }
        for (int i_2 = i_0; i_2 < PB_M; i_2++) {
            for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                B[i_2-1][i_3+1] = B[i_2-1][i_0-1] * B[i_2][i_3+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3224211321_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
