...
void kernel_3232131113_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_L-1, -i_2+PB_M); i_3++) {
            B[i_3-1][i_3] = C[i_3] * D[i_2][i_3-1] - D[i_2-1][i_3+1] - D[i_3+i_2][i_2-1] * D[i_2][i_3] - B[i_3][i_3+1] * 4;
        }
    }
#pragma endscop
...
kernel_3232131113_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
