...
void kernel_2226231311_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_N, -i_0+PB_L+1); i_1++) {
            A[i_0][i_1-1] = C[i_1] * D[i_1+i_0-1][1] * D[i_1][i_0+1] * D[i_1][i_0] * A[i_0+1][i_1-1] - A[i_0-1][i_1-1] * A[i_0][i_1-2] + A[i_0+1][i_1-2] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_3] = C[i_2] + B[i_2-1] * B[i_3-1] * 2;
        }
    }
#pragma endscop
...
kernel_2226231311_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
