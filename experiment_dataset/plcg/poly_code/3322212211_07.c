...
void kernel_3322212211_07(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_N, PB_M-2); i_2++) {
                C[i_2+i_0][i_0] = D[i_2+2][i_1-1][i_2] - 5;
            }
        }
        B[i_0] = E[i_0] * F[i_0] - B[i_0+1] - B[i_0-1] - 5;
        for (int i_3 = i_0; i_3 < -i_0+PB_N-1; i_3++) {
            C[i_3+i_0][i_0] = C[i_3+i_0+1][i_0] * C[i_3+i_0-1][i_0] + 4;
        }
    }
#pragma endscop
...
kernel_3322212211_07(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
