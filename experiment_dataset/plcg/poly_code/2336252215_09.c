...
void kernel_2336252215_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            A[i_0][i_1] = D[i_0][i_1][i_1] - E[i_1] - F[i_1][i_0] * E[1] * A[i_1+1][i_0] - A[i_0-1][i_1] + 1;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_P); i_2++) {
            A[i_0][i_2] = G[i_0-2][0] - G[i_2+i_0][0] * G[i_2][i_0+2] * F[i_2][i_0-1] * F[i_2][i_0] - E[i_2] - 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_N; i_3++) {
        C[i_3+2] = H[i_3] + E[0] * H[i_3] + E[i_3] * H[i_3] - C[i_3+3] - 1;
    }
#pragma endscop
...
kernel_2336252215_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
