...
void kernel_4332131115_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_0+1][i_1+1][i_2] = D[i_2-1][i_0+1][i_1+1] + D[i_2][i_2][i_0] + D[i_2][i_1][i_0+1] * 1;
            }
        }
        for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_L, PB_P-1); i_4++) {
                for (int i_5 = 0; i_5 < PB_N; i_5++) {
                    B[i_0][i_3][i_4+1][i_0] = E[i_5][i_0][i_3][i_4+1] * E[i_5][i_0][i_4+i_3][1] + E[i_3][i_0-1][i_3][i_4] * E[i_3+1][i_5][i_4][i_0-1] + 4;
                }
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_P-2; i_6++) {
        for (int i_7 = max(i_6, 1); i_7 < PB_M-1; i_7++) {
            C[i_7][i_6+1] = F[i_7][i_6] * C[i_7+1][i_6] + C[i_7-1][i_6+1] * C[i_7][i_6+2] - 4;
        }
    }
#pragma endscop
...
kernel_4332131115_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
