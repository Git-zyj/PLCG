...
void kernel_2236131221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            A[i_0][i_1-1] = C[i_0][i_1-1] * C[i_1][i_0-1] - C[i_0-1][i_1+i_0] * C[i_1+1][i_0] + C[i_1+i_0+1][i_0] + A[i_0][i_1] + A[i_0][i_1+1] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, PB_M-1); i_3++) {
            A[i_2][i_3-1] = C[i_3+1][i_2-1] + 4;
        }
    }
#pragma endscop
...
kernel_2236131221_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
