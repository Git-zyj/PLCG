...
void kernel_4416251115_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] * E[i_0] * F[i_0] * E[i_0] + F[i_0] * E[i_0] - G[i_0][i_0-1] - F[0] * F[i_0] + A[i_0+1] + A[i_0-1] * 3;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            A[i_2] = H[i_2+1][i_1] * E[i_2] * 6;
        }
        for (int i_3 = max(1, i_1); i_3 < min(-i_1+PB_M, PB_M-1); i_3++) {
            C[i_1][i_3+1] = G[i_3+i_1][i_3-1] - 4;
            D[i_3-1] = G[i_3+1][i_1] * I[i_3-1][i_1+1][i_3] - E[i_3] - H[i_1][i_3] * G[i_3][i_1] - 1;
        }
    }
#pragma endscop
...
kernel_4416251115_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
