...
void kernel_2424331213_02(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_L+1, PB_M-1); i_1++) {
            D[i_1+1] = E[i_1] * F[i_1][i_0+1] + F[i_1][1] - F[i_0][i_1+i_0-1] + G[i_1][i_0] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_2][i_3-1] = H[i_3][i_2] + F[i_3][i_2+1] * B[i_2+1][i_3-1] - B[i_2][i_3] + 4;
        }
        C[i_2-1] = I[i_2-1] * C[i_2] + C[i_2-2] + 3;
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        D[i_4+1] = J[i_4] * D[i_4+2] + 4;
    }
#pragma endscop
...
kernel_2424331213_02(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
