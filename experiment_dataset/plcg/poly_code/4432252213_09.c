...
void kernel_4432252213_09(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                D[i_0][i_1][i_2+1][i_2+1] = E[i_2][i_0][i_1-2] * F[i_1][i_0+1][i_2] + G[i_2][i_0] - 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_N-2; i_4++) {
            for (int i_5 = 1; i_5 < PB_N; i_5++) {
                C[i_5] = E[i_4+1][i_5][i_3] * G[i_5][i_4] * F[i_4][i_3][i_4+1] * E[i_5][i_3][i_4-1] + F[i_5-1][i_3][i_3+1] * F[i_5][i_3][i_4+1] - F[i_5][i_3-1][i_4] * E[i_4+2][i_5][i_3] - C[i_5] * 4;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_L; i_6++) {
        for (int i_7 = max(i_6, 1); i_7 < PB_M-1; i_7++) {
            C[i_7] = H[i_6][i_7] - G[i_7][i_6] + G[i_7-1][i_6] * H[i_7][i_6] * C[i_7-1] + C[i_7+1] * 3;
        }
    }
    for (int i_8 = 2; i_8 < PB_L-2; i_8++) {
        for (int i_9 = 1; i_9 < PB_M-1; i_9++) {
            for (int j_0 = 1; j_0 < PB_N; j_0++) {
                for (int j_1 = 0; j_1 < PB_P-2; j_1++) {
                    D[i_9][i_8][j_1][j_1] = E[j_0][i_8-2][i_9-1] + I[i_8][j_0-1][j_1][i_9][i_8] * J[i_8-2][j_1+2][j_0][i_9] + K[i_9][i_8+2][j_1][0] + D[i_9+1][i_8][j_1][j_1] * 6;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4432252213_09(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K)));
