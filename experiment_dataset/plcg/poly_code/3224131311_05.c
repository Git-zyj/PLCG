...
void kernel_3224131311_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            A[i_0][i_1+1] = C[i_1] * D[i_1+i_0][i_0] * A[i_0][i_1] * A[i_0+1][i_1+1] - A[i_0-1][i_1] * A[i_0+1][i_1+2] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_L; i_3++) {
            for (int i_4 = i_2; i_4 < PB_M; i_4++) {
                B[i_2][i_4][i_3] = E[i_2][i_4+i_3-1][i_3+i_2] - D[i_4][i_3+1] + E[i_4][i_2+1][i_3] - A[i_3+1][i_4] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3224131311_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
