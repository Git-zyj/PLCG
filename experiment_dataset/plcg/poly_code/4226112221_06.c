...
void kernel_4226112221_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-4; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_1][i_2] = A[i_1-2][i_2] * 6;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L; i_3++) {
            for (int i_4 = i_3; i_4 < PB_Q-2; i_4++) {
                for (int i_5 = 0; i_5 < PB_M; i_5++) {
                    B[i_5][i_3][i_4][i_0+2] = C[i_0][i_3][i_5][i_4+2] * B[i_5][i_3][i_4][i_0+4] + B[i_5][i_3][i_4+1][i_0+2] - 5;
                }
            }
        }
    }
...
kernel_4226112221_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
