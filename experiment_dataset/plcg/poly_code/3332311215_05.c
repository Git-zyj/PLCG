...
void kernel_3332311215_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
        B[i_1-1] = A[i_1+1][i_1+1] * 6;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_Q+1; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_P-1, PB_M-1); i_4++) {
                C[i_4][i_2][i_4+i_3] = D[i_4][i_3-1] - C[i_4+1][i_2][i_4+i_3+1] - C[i_4-1][i_2][i_4+i_3-1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3332311215_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
