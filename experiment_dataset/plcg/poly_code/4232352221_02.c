...
void kernel_4232352221_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < min(i_0, PB_L-2); i_2++) {
                A[i_2+1][i_1][i_2] = C[i_2][i_1][i_0] + C[i_2][i_0][i_1] * D[i_0+1][i_2+2][i_2-2] - A[i_2+1][i_1-2][i_2] * A[i_2-1][i_1][i_2-2] + A[i_0+1][i_1+1][i_0] - 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M+1, PB_P-2); i_4++) {
            for (int i_5 = 2; i_5 < -i_3+PB_L-1; i_5++) {
                B[i_3][i_4+2][2] = D[i_4+i_3-1][i_5+1][0] - E[i_4][i_5][i_3][i_5] * F[i_3+2][i_4] * D[i_4][i_5-2][i_3-2] * G[i_4][i_5][i_3] * D[i_3+2][i_5][i_4-1] - C[i_4][i_5+i_3+1][i_3] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4232352221_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
