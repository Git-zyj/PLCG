...
void kernel_2312152321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_N+1, PB_M); i_1++) {
            A[i_1-1] = D[0] * D[i_1+i_0-1] - E[i_1][i_1] * D[i_1+i_0-1] + E[i_1][0] - E[i_1][i_0] * A[i_1] + C[i_1-1] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        C[i_2-1] = D[i_2] * D[i_2+1] * D[i_2] - C[i_2-1] + 2;
        for (int i_3 = 3; i_3 < PB_M; i_3++) {
            C[i_3-1] = E[i_3][i_2] * D[i_2] - E[i_3][i_2+2] - E[i_2][i_3] - C[i_3-3] + C[i_3] - 6;
        }
    }
#pragma endscop
...
kernel_2312152321_06(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
