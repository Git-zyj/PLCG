...
void kernel_3412112225_03(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L-1, -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                D[i_2][i_1+1][i_0+1] = E[i_1][i_0+2][i_1+i_0] - 2;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L; i_3++) {
            for (int i_4 = 2; i_4 < min(-i_3+PB_P-2, PB_M); i_4++) {
                D[i_4][i_0][i_3+1] = E[i_3][i_0][0] * E[i_3][i_0][i_4+i_3+2] - 2;
                D[i_4-2][i_3][i_0+3] = E[i_3][i_0][i_0] + D[i_0][i_3][i_4-1] - 2;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
        for (int i_6 = 0; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < PB_M; i_7++) {
                D[i_7][i_6][i_5+1] = D[i_7][i_6][i_5-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3412112225_03(xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
