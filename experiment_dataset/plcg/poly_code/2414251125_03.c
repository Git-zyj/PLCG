...
void kernel_2414251125_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1] = E[i_1+1][i_0-1] + F[i_1][i_0-1] * G[i_1+i_0][i_1][i_0] + F[i_1-1][i_0] - A[i_1-2] + 2;
        }
        B[i_0] = H[i_0] + I[0] + H[i_0] - I[0] - H[i_0] * A[i_0] * 2;
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        B[i_2] = I[i_2+1] * H[i_2-1] - H[i_2] + I[i_2] + H[0] - 6;
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            B[i_3-2] = H[i_2-1] * F[i_2][i_3-1] * G[i_3][i_2][i_2-1] * 5;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2414251125_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
