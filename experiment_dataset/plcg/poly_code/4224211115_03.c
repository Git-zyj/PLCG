...
void kernel_4224211115_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_M+1, -i_0+PB_Q-1); i_2++) {
                B[i_0-1][i_2+i_0+1] = C[i_2-1][i_1-1] * 2;
            }
        }
        for (int i_3 = 2; i_3 < min(i_0, -i_0+PB_N); i_3++) {
            B[i_3-1][i_3+i_0] = D[i_3+i_0] - B[i_3-2][i_3+i_0-1] + 3;
        }
    }
#pragma endscop
...
kernel_4224211115_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
