...
void kernel_3236311223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            A[i_1] = C[i_1][i_0] * A[i_1-1] * A[i_1-2] + A[i_1+2] - 1;
        }
        for (int i_2 = 0; i_2 < min(PB_L, i_0); i_2++) {
            for (int i_3 = 0; i_3 < PB_Q; i_3++) {
                B[i_0][i_2][i_3] = B[i_0+1][i_2][i_3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3236311223_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
