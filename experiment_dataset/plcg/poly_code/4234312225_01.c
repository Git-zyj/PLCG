...
void kernel_4234312225_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
        ...
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M-2; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_1+PB_M, -i_2+PB_M-2); i_3++) {
                    B[i_3+i_2+2][i_1][i_2] = B[i_3+i_1][i_2+1][i_3] - 4;
                }
            }
        }
        for (int i_4 = max(2, i_0); i_4 < -i_0+PB_M; i_4++) {
            for (int i_5 = 2; i_5 < min(i_4, -i_4+PB_M+2); i_5++) {
                B[i_4+i_0][i_5][i_0] = C[i_4][i_0][0] * B[i_5+i_4-2][i_0+2][i_5-2] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4234312225_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
