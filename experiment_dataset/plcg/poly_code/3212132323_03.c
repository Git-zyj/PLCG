...
void kernel_3212132323_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            B[i_1+i_0][i_1][i_0-2] = C[i_1][i_0] + C[i_1][i_0+2] + 1;
        }
    }
    for (int i_2 = 4; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_L; i_3++) {
            for (int i_4 = 2; i_4 < min(PB_L, -i_2+PB_M); i_4++) {
                B[i_3+i_2][i_3][i_2-2] = D[i_4][i_3+1][i_2-2] * D[i_2][i_4][i_3+i_2] * D[i_3][i_4+i_2][i_3] - D[i_2][i_4+i_3-1][0] + B[i_3+i_2-2][i_3][i_2-4] * B[i_3+i_2+2][i_3+2][i_2-2] - B[i_4+i_3-2][i_4-2][i_3-2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3212132323_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
