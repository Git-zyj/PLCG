...
void kernel_4316351211_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_M+1, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_M; i_3++) {
                    A[i_3][i_2][i_1-1][0] = D[i_2][i_3+i_1][0] + E[i_3][i_2-1][i_0][i_1+1] * F[i_3+i_1][i_0-1][i_0][i_2+1] - E[i_0-1][i_3+1][i_1][i_2] * F[i_0][i_3][i_1+1][i_1+1] + G[i_3][i_0+1][i_1][i_0-1] - A[i_3][i_2][i_1-2][0] - 4;
                }
            }
            B[i_0][i_1] = H[i_0][i_1+i_0-1][0] + I[i_1+1][i_0] + J[i_1+1][i_0-1] + B[i_0][i_1+1] + B[i_0+1][i_1-1] - 5;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        C[i_4][i_4] = K[i_4] - L[i_4-1] + L[i_4] - L[i_4] * C[i_4-1][i_4-1] - 3;
    }
#pragma endscop
