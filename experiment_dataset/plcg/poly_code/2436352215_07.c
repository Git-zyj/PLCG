...
void kernel_2436352215_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        B[i_0-1][i_0-1] = E[i_0-1] * F[i_0+2][i_0] + G[i_0-1] - B[i_0+1][i_0+1] * 3;
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            B[i_1][i_1] = H[i_1+1][i_0-2] - H[i_0][i_1] - H[i_0][i_1] * I[i_0][i_1] + F[i_1][i_0-1] + H[i_0][i_1] + B[i_1-1][i_1-1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_3][i_3] = F[i_3][i_2] + I[i_3][i_2+2] + F[i_2][i_2] + G[i_3+1] + G[i_3+1] * H[i_2][i_3-1] * 2;
        }
        B[i_2][i_2] = J[i_2+2] * F[i_2][i_2] * 6;
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2436352215_07(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
