...
void kernel_4232112325_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
        ...
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_L, -i_0+PB_L+1); i_3++) {
                    A[i_1][i_3+i_2][i_1] = A[i_1+2][i_3+i_2][i_1+2] - A[i_1][i_3+i_0-1][i_1] * 6;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
            B[i_0+1][i_4-1] = C[i_0-1][i_4] * B[i_0+3][i_4] + B[i_4+1][i_0+1] - B[i_0+2][i_4-1] * B[i_0+1][i_4] + 6;
        }
    }
#pragma endscop
...
kernel_4232112325_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
