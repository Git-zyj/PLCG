...
void kernel_4234111215_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = min(i_0, -i_0+PB_M+1) - 1; i_1 >= 0; i_1--) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                B[i_1+i_0-1][i_2][i_2] = 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M-1, PB_N+1); i_4++) {
            for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
                B[i_4+i_3][i_5][i_5] = C[i_5+i_4-1][i_3][1] - B[i_4+i_3][i_5+1][i_5+1] - B[i_4+i_3+1][i_5][i_5] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4234111215_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
