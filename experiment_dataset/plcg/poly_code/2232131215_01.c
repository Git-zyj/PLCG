...
void kernel_2232131215_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_N-1, PB_M-1); i_1++) {
            A[i_1+i_0-1][i_1] = A[i_1+i_0][i_1] * A[i_1+i_0+1][i_1+1] - 6;
        }
    }
    for (int i_2 = 3; i_2 < PB_N+3; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_N+3, PB_M+1); i_3++) {
            A[i_3+i_2-3][i_3-1] = C[i_3-1][i_2] * C[i_3][0] * C[i_3+i_2][i_3+1] * C[i_2][i_3] + C[i_3-1][i_2] + 1;
        }
    }
#pragma endscop
...
kernel_2232131215_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
