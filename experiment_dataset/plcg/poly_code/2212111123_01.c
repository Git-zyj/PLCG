...
void kernel_2212111123_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M+1; i_1++) {
            A[i_1+i_0-1][i_0+1] = C[i_1] - 4;
        }
    }
    for (int i_2 = 3; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M+1; i_3++) {
            A[i_3+i_2-1][i_2+1] = A[i_3+i_2-3][i_2-1] - 3;
        }
    }
#pragma endscop
...
kernel_2212111123_01(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
