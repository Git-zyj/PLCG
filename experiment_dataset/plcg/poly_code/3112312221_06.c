...
void kernel_3112312221_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-4; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_Q-4; i_2++) {
                A[i_2][i_1+2][i_0][i_2+i_0+2] = B[i_1-2][i_0][i_2+1] - A[i_0+1][i_1+2][i_2+1][i_2+i_0+4] * A[i_0][i_1+2][i_2-2][i_2+i_0] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3112312221_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
