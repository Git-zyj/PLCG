...
void kernel_4224331115_05(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we)){
        ...
        for (int i_1 = 0; i_1 < -i_0+PB_L+2; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    B[i_2+1][i_3][i_1+i_0-2][i_2] = C[i_2-1][i_3][i_0][i_0+1] + C[i_2+1][i_3][i_0][i_1] * 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_P-1; i_5++) {
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                for (int i_7 = 0; i_7 < min(min(PB_L-1, -i_4+PB_P), -i_6+PB_L); i_7++) {
                    B[i_6+1][i_7][i_5+i_4-1][i_6] = D[i_5+1][i_7+1][i_6][i_6] + D[i_5][i_7][i_4][i_7] * C[i_6][i_4+1][i_7][i_5] - E[i_4][i_7+i_6][i_5][i_7+i_4][i_5+i_4+1] * B[i_4+2][i_7][i_6+i_5-1][i_4+1] - 5;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), INIT_SEED);
kernel_4224331115_05(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E)));
