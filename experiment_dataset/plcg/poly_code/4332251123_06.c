...
void kernel_4332251123_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0] - E[i_0] + E[i_0] + D[0] + E[i_0] - A[i_0-1] + 6;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_P, -i_1+PB_L); i_2++) {
            B[i_2+1] = F[i_2][i_2+i_1] + F[i_1][i_2+i_1] + F[i_2][i_1] + D[i_2] + D[1] * G[i_2+i_1][0] + B[i_2] + 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_P+1, i_3); i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_L, PB_P); i_5++) {
                B[i_4+1] = H[i_5][i_3][i_4-1] * H[i_4][i_3][i_5] + H[i_5][i_5+i_3][i_4-1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4332251123_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
