...
void kernel_4224212115_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
        ...
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                for (int i_3 = PB_M-1 - 1; i_3 >= 1; i_3--) {
                    A[i_3+1][i_1][i_2][i_3] = C[i_0][i_3][i_1] - A[i_3][i_1][i_2+1][i_3-1] * 6;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            B[i_0-2][i_4] = D[i_4][i_0] - 3;
        }
    }
#pragma endscop
...
kernel_4224212115_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
