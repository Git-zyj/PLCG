...
void kernel_2322312223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < PB_N-3; i_1++) {
            A[i_1+2][i_1-1] = A[i_1][i_1-3] - A[i_1+3][i_1] + 3;
        }
        B[i_0] = 5;
    }
    for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            C[i_2][i_3] = D[i_3] * C[i_2-1][i_3] * C[i_2+2][i_3] + C[i_2+1][i_3] * C[i_2-2][i_3] - 4;
        }
    }
#pragma endscop
...
kernel_2322312223_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
