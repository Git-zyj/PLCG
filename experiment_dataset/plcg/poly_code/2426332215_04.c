...
void kernel_2426332215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            A[i_1][i_0] = E[i_1] + E[i_1] - A[i_1][i_0-1] + 3;
        }
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            C[i_0-2][i_2] = F[i_0-1][i_2+1] - F[i_2][i_0+2] + 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        for (int i_4 = i_3; i_4 < PB_N; i_4++) {
            C[i_3-1][i_4] = G[i_4][1] * H[i_4][i_3] - G[i_4][i_3+2] + A[i_4][i_3+1] - C[i_3][i_4+1] * 4;
        }
    }
    for (int i_5 = 2; i_5 < PB_M-3; i_5++) {
        D[i_5+2] = E[i_5] * E[i_5] + I[i_5-2] + D[i_5+3] - 3;
    }
#pragma endscop
...
kernel_2426332215_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
