...
void kernel_4216212315_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+1; i_0++) {
        A[i_0-2] = C[i_0] + A[i_0-1] - A[i_0-3] + A[i_0-3] - A[i_0-3] + A[i_0-1] + 3;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = max(1, i_1); i_3 < PB_M; i_3++) {
                B[i_3][i_2][i_1] = B[i_2][i_3-1][i_1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4216212315_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
