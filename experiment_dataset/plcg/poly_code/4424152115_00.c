...
void kernel_4424152115_00(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            D[i_1+1] = E[i_1] * E[i_0] * F[i_0][1] + F[i_1-2][i_0-2] - F[i_1][i_0] + F[i_1][i_0] * 3;
            for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
                B[i_0][i_2+1][i_1-2][i_2] = G[i_0][i_2][i_1+i_0] - F[i_2-2][i_1] + H[i_0][i_2][i_1][0] * G[i_2+2][i_1+i_0][i_1-1] * 2;
            }
        }
        D[i_0] = E[i_0-2] - E[i_0] - E[i_0] - E[i_0] + 5;
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        D[i_3] = E[i_3] - E[i_3+1] + E[i_3] - E[0] + E[i_3] - E[i_3] + D[i_3+1] - 4;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4424152115_00(xd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
