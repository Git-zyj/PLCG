...
void kernel_4334212223_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 3; i_1 < min(-i_0+PB_L, PB_M-1); i_1++) {
            for (int i_2 = i_1; i_2 < PB_M; i_2++) {
                A[i_1][i_1-2][i_0] = D[i_2][i_1+i_0][i_1+i_0][0] - E[i_1+1][i_2] * A[i_1-1][i_1-3][i_0] * 2;
            }
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < i_3; i_4++) {
                B[i_3][i_0+1][0] = B[i_3][i_0+2][0] * 6;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_Q-1; i_5++) {
        for (int i_6 = 1; i_6 < PB_Q; i_6++) {
            for (int i_7 = 0; i_7 < min(-i_5+PB_Q-1, -i_6+PB_Q); i_7++) {
                C[i_5-1][i_6+1][i_7+i_5] = C[i_5-1][i_6+1][i_7+i_5+1] + C[i_6-1][i_5-1][i_7+i_6] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4334212223_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
