...
void kernel_3212112115_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_N; i_1++) {
            B[i_0][i_1-2][i_1] = C[i_1][i_0+1] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_Q; i_3++) {
            for (int i_4 = max(3, i_2); i_4 < PB_Q+1; i_4++) {
                B[i_2][i_3-2][i_3] = B[i_3][i_4-3][i_4-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3212112115_04(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
