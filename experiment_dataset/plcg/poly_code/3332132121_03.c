...
void kernel_3332132121_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        B[i_1] = D[i_1] * D[i_1] + D[i_1+2] + A[i_1] + B[i_1+2] * 4;
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        A[i_2] = D[i_2-1] - D[i_2+1] + 6;
    }
#pragma endscop
...
kernel_3332132121_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
