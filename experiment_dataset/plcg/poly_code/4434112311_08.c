...
void kernel_4434112311_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = i_0; i_2 < PB_Q-1; i_2++) {
                for (int i_3 = i_0; i_3 < -i_0+PB_L; i_3++) {
                    A[i_3][i_1][i_2][i_0-2] = E[i_1][i_2][i_3+i_0][i_0] + E[i_1][i_2][i_3][i_0] + A[i_2+1][i_1][i_3][i_0-3] + A[i_0][i_1-1][i_3-1][i_2-1] + A[i_3-1][i_1][i_2][i_0-2] + A[i_3][i_1][i_2+1][i_0-2] * A[i_3+1][i_1][i_0+1][i_2-2] + 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, PB_M-2); i_5++) {
            B[1] = F[i_4][i_5+2] + F[i_4][i_5] * B[1] + 5;
        }
    }
        ...
        for (int i_7 = i_6; i_7 < PB_P; i_7++) {
            for (int i_8 = 0; i_8 < PB_M-1; i_8++) {
                for (int i_9 = 1; i_9 < PB_Q-1; i_9++) {
                    C[i_8][i_7][i_9][i_7-2] = C[i_8][i_6][i_9+1][i_6-2] + C[i_8+1][i_7][i_9-1][i_7-2] + 5;
                }
            }
        }
    }
    for (int j_0 = 2; j_0 < PB_L-1; j_0++) {
        for (int j_1 = 2; j_1 < PB_Q; j_1++) {
            for (int j_2 = 0; j_2 < -j_0+PB_Q-1; j_2++) {
                for (int j_3 = 2; j_3 < PB_P; j_3++) {
                    D[j_0][j_3][j_1-2][j_2+j_0] = A[j_3-1][j_2][j_1][j_0-2] * D[j_0][j_3][j_1-1][j_2+j_0-1] - D[j_0+1][j_1][j_3-2][j_2+j_0+1] + 2;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4434112311_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F)));
