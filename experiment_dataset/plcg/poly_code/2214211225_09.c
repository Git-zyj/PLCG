...
void kernel_2214211225_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = PB_M - 1; i_2 >= 0; i_2--) {
        for (int i_3 = max(3, i_2); i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            B[i_3-1] = C[i_3+i_2][i_3] * B[i_3-3] - B[i_3] + 1;
        }
    }
#pragma endscop
...
kernel_2214211225_09(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
