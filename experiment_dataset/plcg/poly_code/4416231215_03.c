...
void kernel_4416231215_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[i_0-1] * E[i_0-1] + A[i_0+2] * 3;
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
                for (int i_3 = i_2; i_3 < -i_1+PB_M; i_3++) {
                    D[i_3+i_1][i_0][i_2+1][i_2+1] = F[i_3][i_2][i_1] + G[i_2-1][i_1+i_0][i_3][i_0] - 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_N-1; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_M); i_5++) {
            A[i_5+2] = H[i_5][i_4] - I[i_4+1][i_5+1] + J[i_5+1][i_4-1][i_5] + H[i_5][i_5+i_4-1] - 3;
            for (int i_6 = 1; i_6 < min(PB_P, -i_5+PB_M); i_6++) {
                for (int i_7 = 1; i_7 < min(-i_6+PB_M, PB_P-1); i_7++) {
                    D[i_5+i_4][i_6-1][i_7][i_7] = K[i_5+1][i_4][i_5][i_6] * G[i_4+1][i_7][i_7+i_6][1] * D[i_6+i_5][i_4-2][i_7][i_7] + D[i_5+i_4-2][i_7-1][i_6][i_6] + D[i_5+i_4-1][i_6][i_7+1][i_7+1] * 1;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4416231215_03(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K)));
