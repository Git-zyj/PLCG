...
void kernel_2434232311_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0+1] + F[i_0] + F[i_0] * E[i_0+1] * E[i_0-1] * A[i_0-1] + D[i_0] + 6;
        B[i_0] = B[i_0+1] + B[i_0-1] - 2;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_L, -i_1+PB_M-2); i_2++) {
            C[i_2+i_1][i_1][2] = G[i_2][i_1+1] + H[i_2+2][i_1+2] - C[i_2+i_1+2][i_1+1][2] * C[i_2+i_1-1][i_1][2] - D[i_2] * 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
        D[i_3] = E[i_3+2] + F[i_3+1] * E[i_3+1] - D[i_3-1] + D[i_3+1] * 4;
    }
#pragma endscop
...
kernel_2434232311_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
