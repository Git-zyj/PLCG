...
static void print_array(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4416351225_09(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < min(PB_L-3, -i_1+PB_P); i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_1+PB_M, -i_2+PB_M+1), PB_L-1); i_3++) {
                    C[i_3+i_1][i_2+3][i_2+3] = E[i_0][i_3+i_2-1][i_1][1] * F[i_3][i_2+i_1][i_1+1] * G[i_0][i_3+1][i_1][i_2+1] + H[i_2-1][i_2-1][i_1][i_0+1] - H[i_1-1][i_0+1][i_2][i_1] - 6;
                }
            }
        }
        for (int i_4 = 1; i_4 < -i_0+PB_M; i_4++) {
            B[1][i_4+i_0] = I[i_4+1][1] + J[i_0][i_0-1] * K[i_4][i_0] * K[i_0][i_4-1] * L[i_4] + I[i_0][i_4] + 3;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
        for (int i_6 = i_5; i_6 < -i_5+PB_M-2; i_6++) {
            for (int i_7 = 1; i_7 < min(PB_L-3, -i_5+PB_M-1); i_7++) {
                C[i_7+i_5][i_6+1][i_6+1] = J[i_7+1][i_6] * M[i_7][i_5][i_6+i_5] * N[i_6][i_7+1][i_5-1] - F[i_7-1][i_7][i_5] + K[i_6][i_6] - C[i_7+i_5+1][i_6+1][i_6+1] * C[i_6+i_5+2][i_7+1][i_7+1] - C[i_6+i_5][i_7+3][i_7+3] + 2;
            }
            for (int i_8 = 1; i_8 < min(PB_Q-1, -i_6+PB_M+1); i_8++) {
                for (int i_9 = 0; i_9 < i_6; i_9++) {
                    C[i_8+i_6-1][i_9+1][i_9+1] = G[i_9][i_8-1][i_5][i_6+i_5+1] + E[i_8][i_9+1][i_6][i_5] - H[i_8-1][i_5][i_9][i_6+1] * G[i_9][i_5-1][i_6][i_8+1] * 4;
                }
            }
        }
    }
...
init_array(xc,yc,zc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4416351225_09(xc,yc,zc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N));
polybench_prevent_dce(print_array(xc,yc,zc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N)));
