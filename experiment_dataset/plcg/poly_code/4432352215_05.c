...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4432352215_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_1+PB_L, PB_M-2); i_3++) {
                    A[i_0][i_1][2][i_3] = E[i_0][i_2+2][i_3][0] + E[i_3+2][i_0-1][i_1][i_2-1] + F[i_1][i_2][i_3-1] - G[i_2-2][i_3][i_3+i_1] - H[i_2-1][i_0][i_1][i_3+1] * A[i_3-1][i_1-1][2][i_2] - 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 2; i_5 < -i_4+PB_L-2; i_5++) {
            B[i_4][i_5] = I[i_5-2][i_4-1] - J[i_5] * I[i_5][i_5+i_4+2] * I[i_5][0] - K[i_5][i_4] + L[i_5][i_4-2] * B[i_4-1][i_5] + 6;
        }
        for (int i_6 = 0; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < PB_Q; i_7++) {
                for (int i_8 = 0; i_8 < i_4; i_8++) {
                    A[i_4][i_8][2][i_6] = H[i_4][i_6][0][i_7] + 2;
                }
            }
        }
    }
        ...
        for (int j_0 = 2; j_0 < PB_Q-2; j_0++) {
            for (int j_1 = 2; j_1 < -i_9+PB_P-2; j_1++) {
                for (int j_2 = 1; j_2 < PB_M-1; j_2++) {
                    A[j_2+1][j_0][2][j_1+1] = H[j_2][j_0+1][j_1+i_9+2][j_0] + H[j_2][j_0-2][i_9][j_1] * G[j_1-2][j_2-1][0] - M[j_2-1][i_9][j_0+2][i_9] - H[j_1][j_2][i_9-2][j_0] * G[j_2][j_0+2][i_9] * E[j_0+1][j_2+j_1][i_9-2][i_9] * A[j_0][j_1][2][j_2-1] - 3;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4432352215_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M));
