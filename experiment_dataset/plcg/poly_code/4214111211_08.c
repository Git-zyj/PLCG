...
void kernel_4214111211_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L; i_2++) {
                A[i_1+1][i_2+i_0-1] = C[i_0-1][i_2+1][i_1-1] + A[i_1+1][i_2+i_0] + A[i_1][i_2+i_0-1] - 5;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
                    B[i_5+1][i_4][i_3] = B[i_6+1][i_5][i_4+1] + 2;
                }
            }
        }
    }
...
kernel_4214111211_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
