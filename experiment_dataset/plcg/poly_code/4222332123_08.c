...
void kernel_4222332123_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,int wb,DATA_TYPE POLYBENCH_5D(B,xB,yB,zB,vB,wB,xb,yb,zb,vb,wb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = C[i_0-1] + D[i_0+2] * D[1] * D[i_0] + 1;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                for (int i_4 = 1; i_4 < i_3; i_4++) {
                    B[i_2+1][i_1][i_4][i_1-1][i_3] = E[i_3][i_4][i_2-2][i_1] - F[i_4][i_3][i_2][i_1] + B[i_3+1][i_1][i_2+1][i_1-1][i_4-1] + 1;
                }
            }
        }
    }
...
kernel_4222332123_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,wb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
