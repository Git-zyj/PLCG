...
void kernel_4416312225_00(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < PB_P-2; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_N+1, i_0); i_3++) {
                    D[i_1-2][i_2+2][i_3] = E[i_3+i_2-1][i_0][i_1][i_0] * 5;
                }
            }
        }
        B[i_0-2] = 1;
        ...
        for (int i_5 = 3; i_5 < PB_P; i_5++) {
            ...
            for (int i_6 = 0; i_6 < min(i_5, PB_L-2); i_6++) {
                for (int i_7 = 1; i_7 < PB_Q-1; i_7++) {
                    D[i_6][i_5][i_7] = G[i_7][i_4+2][i_5-1][i_5] * D[i_4-1][i_5-2][i_7] * D[i_4+2][i_6][i_7+1] - D[i_6+2][i_5-2][i_7-1] * 6;
                }
            }
        }
    }
...
init_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4416312225_00(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
