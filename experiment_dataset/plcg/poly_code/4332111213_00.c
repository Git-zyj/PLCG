...
void kernel_4332111213_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 2; i_4 < PB_M+1; i_4++) {
                B[i_4-1][i_3] = B[i_4-2][i_3] * 3;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
        C[i_5][i_5] = D[i_5+1] - C[i_5+1][i_5+1] - 6;
    }
#pragma endscop
...
kernel_4332111213_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
