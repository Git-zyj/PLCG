...
void kernel_3216311323_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_P-4, i_0), PB_M-2); i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_1+PB_P-1, PB_M-1), -i_0+PB_P-1); i_2++) {
                A[i_0+1][i_1][i_1+1][i_2+i_0+1] = A[i_0+1][i_1+1][i_1+2][i_2+i_0+1] - A[i_2+1][i_0][i_0+1][i_2+i_1] + A[i_1+2][i_2-1][i_2][i_1+i_0+4] - A[i_1+1][i_0+1][i_0+2][i_2+i_1+1] + 1;
            }
        }
        B[i_0-1] = C[i_0] + B[i_0-3] - B[i_0] + 6;
    }
#pragma endscop
...
kernel_3216311323_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
