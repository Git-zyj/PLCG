...
void kernel_4322111323_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_2][i_0] = B[i_1][i_2] - 3;
            }
            B[i_1][i_0] = D[i_0][i_1+1] + B[i_1+2][i_0] - B[i_1-2][i_0] - 4;
        }
    }
    for (int i_3 = 3; i_3 < PB_M; i_3++) {
        C[i_3-1] = C[i_3] - C[i_3-3] * C[i_3-2] * 1;
    }
#pragma endscop
...
kernel_4322111323_02(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
