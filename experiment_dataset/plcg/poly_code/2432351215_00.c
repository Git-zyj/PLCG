...
void kernel_2432351215_00(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        C[i_0-1] = E[i_0] + F[i_0] - D[i_0][i_0-1] - D[i_0-1][i_0-2] * 1;
    }
    for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_P; i_2++) {
            C[i_2-1] = G[i_2][i_1] - H[i_2][i_1] + E[i_2] * H[i_2-1][0] - H[i_2][i_1+1] * 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        C[i_3-1] = E[i_3] - F[0] - I[i_3+1][0] * F[i_3] + J[i_3] * J[i_3] - 1;
    }
    for (int i_4 = 1; i_4 < PB_P; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_P; i_5++) {
            D[i_5][i_5-1] = G[i_5-1][i_4-1] - H[i_5+1][i_4] + H[i_5][i_4] - J[i_5+1] + J[i_5] + H[i_4][i_5] + I[i_5-1][i_4-1] - D[i_5-1][i_5-2] + 6;
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2432351215_00(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
