...
void kernel_2232132321_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 2; i_1 < min(min(PB_P-3, i_0), -i_0+PB_P); i_1++) {
            A[i_1+i_0][i_0+2][i_1] = C[i_1-2][i_0] + C[i_0][i_1+i_0] * C[i_1+2][i_0] * C[i_1+1][i_0+1] * A[i_1+i_0+1][i_1+3][i_0] + A[i_1+i_0-2][i_0][i_1] + A[i_1+i_0-1][i_1+2][i_0-1] + 1;
        }
    }
    for (int i_2 = PB_P - 1; i_2 >= 2; i_2--) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            B[i_3][i_2-2] = C[i_3-1][i_2] - B[i_3][i_2] * B[i_2-2][i_3-2] + B[i_3][i_2-1] - 6;
        }
    }
#pragma endscop
...
kernel_2232132321_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
