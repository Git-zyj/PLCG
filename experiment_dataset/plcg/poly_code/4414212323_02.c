...
void kernel_4414212323_02(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 4; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
                B[i_2+i_0][i_0][i_1][i_2] = 6;
                for (int i_3 = 0; i_3 < min(-i_2+PB_M+1, PB_L); i_3++) {
                    B[i_2+i_0][i_0][i_3][i_2] = B[i_1+i_0][i_0+2][i_3][i_1-2] * B[i_3+i_2-1][i_3][i_0+2][i_2-1] + 4;
                }
                B[i_2+i_0][i_0][i_1-2][i_2] = D[i_1+1][i_2+1][i_0-1][i_2-1] - 1;
            }
            for (int i_4 = i_0; i_4 < -i_1+PB_P; i_4++) {
                D[i_1][i_0][i_4-2][i_0-2] = E[i_1][i_0][i_4+i_1] - D[i_4][i_1-2][i_0-1][i_1-4] * D[i_1+2][i_0-2][i_4-2][i_0-4] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4414212323_02(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
