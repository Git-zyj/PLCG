...
void kernel_4216311123_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = min(-i_2+PB_N, PB_L) - 1; i_3 >= i_1; i_3--) {
                    A[i_3+i_2][i_1+1][0] = 3;
                }
            }
        }
    ...
    for (int i_4 = 2; i_4 < PB_L+1; i_4++) {
        B[i_4-1] = C[i_4] + C[i_4-1] * B[i_4-2] + 5;
    }
#pragma endscop
...
kernel_4216311123_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
