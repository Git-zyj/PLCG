...
void kernel_4416351323_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[i_0+1] * E[i_0] + E[i_0-1] - E[i_0+1] * A[i_0+2] * A[i_0] * A[i_0-1] - 5;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                B[i_2][i_1-1][i_0] = F[i_2+i_0][i_1][i_2] - F[i_0][i_2+1][i_0] + G[i_2][i_0-1] * H[i_2+1][i_1] * I[i_2][i_0] * J[i_2][i_0][i_1] * K[i_2+1][i_0][i_1] * J[i_0][i_1][i_2] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 3; i_4 < min(-i_3+PB_M, i_3); i_4++) {
            C[i_3][i_4] = L[i_3] + I[0][i_3-1] * E[0] - H[i_4][0] * G[i_4][i_3] * A[i_4+1] + D[i_3][i_4-1] * D[i_3+1][i_4-3] + D[i_3][i_4+1] - D[i_3][i_4] * 5;
            for (int i_5 = 1; i_5 < min(-i_3+PB_L, PB_M); i_5++) {
                D[i_3][i_5-1] = I[i_5-1][i_4] + J[i_4-1][i_5+i_3][i_3] * F[i_5][i_4+i_3][0] + C[i_4-1][i_5] + C[i_3][i_4-2] - C[i_5][i_4] * C[i_4-2][i_5] - 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4416351323_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
