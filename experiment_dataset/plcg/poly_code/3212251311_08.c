...
void kernel_3212251311_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0][i_1] = C[i_0][1] + D[i_1][i_0] - A[i_0-1][i_1] - B[i_0][i_1][i_0-1] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_M-1, -i_2+PB_M); i_4++) {
                B[i_2][i_3][i_2-1] = E[i_4][i_2-1][i_3] - D[i_3+1][i_4-1] * F[i_4][i_3+1][i_2] - E[i_4][i_3][i_2-1] * D[i_4+i_3+1][i_2-1] * F[i_4][i_4+i_2][1] + E[i_2][i_4+1][0] * B[i_2+1][i_3][i_2] + B[i_2-1][i_4][i_2-2] + B[i_2+1][i_3-1][i_2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3212251311_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
