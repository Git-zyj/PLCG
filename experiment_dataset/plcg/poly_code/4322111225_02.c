...
void kernel_4322111225_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0][i_0+1] = A[i_0+1][i_0+2] + A[i_0-1][i_0] - 6;
        for (int i_1 = i_0; i_1 < PB_N-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_N-1, -i_0+PB_N-1), i_0); i_2++) {
                B[i_2+i_1][i_0][i_2] = B[i_2+i_0+1][i_1][i_2] - B[i_2+i_1+1][i_0-2][i_2+1] - 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < min(PB_L, -i_4+PB_M); i_5++) {
                C[i_4-1][i_5+i_4-1] = D[i_5][i_3][i_4] * C[i_4-1][i_5+i_4] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4322111225_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
