...
void kernel_4426251313_03(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            C[i_1-1][i_0] = E[i_1+i_0] * E[1] * F[i_1][i_0+1] - G[i_0] * H[i_0][i_1-1] - F[i_0][0] + F[i_1][i_0] * 6;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_2+PB_L, -i_0+PB_P); i_3++) {
                for (int i_4 = i_0; i_4 < -i_0+PB_M; i_4++) {
                    B[i_3-1][i_2][i_4+1][i_0-1] = I[i_2][i_4][i_3-1][i_0] - J[i_3][i_4][0] * K[i_0][i_3][i_4][i_2-1][i_0] - L[i_0+1][i_4][i_3+i_2][i_3-1] + L[i_2][i_4+i_0][i_2-1][i_3+i_0] + J[i_4+1][i_2-1][i_3-1] + I[i_2][i_0-1][i_3-1][i_2] - 6;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = 0; i_6 < min(-i_5+PB_M, i_5); i_6++) {
            C[i_6][i_5] = H[i_6+i_5][i_6] - C[i_6][i_5-1] * C[i_6+1][i_5] * 1;
        }
    }
    for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
        for (int i_8 = 2; i_8 < i_7; i_8++) {
            for (int i_9 = i_8; i_9 < PB_L-2; i_9++) {
                for (int j_0 = 1; j_0 < min(-i_9+PB_P-1, -i_8+PB_M+1); j_0++) {
                    D[i_8-1][i_8][i_9+1][j_0] = I[j_0+i_8-1][i_7][i_9+1][i_9-1] + I[i_9][i_7][j_0+1][j_0] * I[j_0+i_9][i_7+1][i_8][0] * M[i_9][i_8-1][j_0+i_9+1] * B[j_0-1][i_8][i_9+1][i_7-1] + B[i_7-1][j_0][i_9+1][i_8-2] * D[i_8-1][i_8][i_9][j_0+1] * D[j_0-1][j_0][i_9+2][i_7] * D[i_8-1][i_8][i_9+2][j_0] - 6;
                }
            }
        }
    }
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4426251313_03(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
