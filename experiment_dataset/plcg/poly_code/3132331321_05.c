...
void kernel_3132331321_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            A[i_1+1] = B[i_1][i_0-1] * C[i_0] * B[i_1][i_0+1] + A[i_1+2] * A[i_1] * A[i_1+3] + 5;
        }
    }
#pragma endscop
...
kernel_3132331321_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
