...
void kernel_4224111211_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = 1; i_2 < min(i_0, PB_L-1); i_2++) {
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                B[i_2][i_3][i_0-1] = C[i_3-1][i_2] * D[i_3][i_2][i_0] * B[i_2-1][i_3-1][i_0-1] * B[i_3][i_2+1][i_0-1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4224111211_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
