...
void kernel_3214211221_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
            for (int i_3 = i_2; i_3 < -i_2+PB_N-1; i_3++) {
                B[i_1][i_2][1] = C[i_3+i_2+1][i_1][1] + B[i_1][i_2+1][1] + B[i_1+2][i_3][1] + B[i_1-1][i_3][1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3214211221_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
