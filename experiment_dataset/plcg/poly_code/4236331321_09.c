...
void kernel_4236331321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0] = C[0] * C[i_0] * D[i_0] * C[i_0+1] - D[i_0] - A[i_0-2] * A[i_0+2] - A[i_0+1] + A[i_0-1] + 6;
    }
    for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            for (int i_3 = i_2; i_3 < PB_M-3; i_3++) {
                B[i_3+1][i_1+1][i_2] = E[i_2][i_1][i_1] + B[i_3+1][i_2+3][i_1] - B[i_3+3][i_2+1][i_1+2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4236331321_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
