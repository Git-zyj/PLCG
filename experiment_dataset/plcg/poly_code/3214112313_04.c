...
void kernel_3214112313_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < min(min(-i_1+PB_L-3, PB_M), -i_0+PB_L-2); i_2++) {
                A[i_0][i_1][i_2+i_1+1] = A[i_0][i_2-1][i_2+i_1] + A[i_0-1][i_1+1][i_2+i_1+2] * A[i_0-1][i_1+1][i_2+i_1+3] + A[i_1][i_2][i_2+i_0+2] + A[i_0][i_1][i_2+i_1+2] * 1;
            }
            for (int i_3 = 2; i_3 < -i_1+PB_M; i_3++) {
                B[i_1][i_3-2][i_0] = C[i_3][i_1+2] * D[i_3+i_1][i_3][i_0-1] * B[i_1+1][i_3-2][i_0] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3214112313_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
