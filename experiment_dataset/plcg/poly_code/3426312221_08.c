...
void kernel_3426312221_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_L; i_2++) {
                A[i_0][i_2-1] = E[i_0][i_2+i_1][i_0] - A[i_1-2][i_2-1] * A[i_1+2][i_2-2] * A[i_0-2][i_2-1] + 6;
            }
        }
    }
    ...
    for (int i_6 = 0; i_6 < PB_P; i_6++) {
        for (int i_7 = 0; i_7 < PB_P; i_7++) {
            D[i_6][0] = C[i_7] - D[i_6+1][0] * D[i_6+2][0] * 4;
        }
    }
#pragma endscop
...
kernel_3426312221_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
