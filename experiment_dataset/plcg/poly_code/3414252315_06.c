...
void kernel_3414252315_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_P-3, -i_0+PB_Q-1); i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_Q, -i_1+PB_M), PB_P-2); i_2++) {
                A[i_0+1][i_1+2][i_2+i_0] = E[i_2][i_0-2][i_1-1] + F[i_2+i_1][i_0][i_1+i_0] + A[i_0+1][i_2+2][i_1+i_0+1] * A[i_0+1][i_1+3][i_2+i_0] * 2;
            }
        }
        B[i_0] = G[i_0] + G[i_0] - H[i_0-1] * G[i_0] - G[i_0] + B[i_0+1] + B[i_0-1] * 1;
        for (int i_3 = i_0; i_3 < min(-i_0+PB_M, PB_L); i_3++) {
            B[i_3] = H[i_3+i_0] + I[i_3][i_0+2] * J[i_3][i_0] + B[i_3] + 1;
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < min(i_4, PB_P-1); i_5++) {
            B[i_5+1] = J[i_5][i_5] - J[i_4-2][i_5] - J[i_4][i_5-2] + J[i_5+1][i_4] - G[i_4-2] + J[i_5-1][i_4] * I[i_4][i_5+1] - 6;
        }
    }
#pragma endscop
...
kernel_3414252315_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
