...
void kernel_2124212321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_N - 1; i_0 >= 0; i_0--) {
        for (int i_1 = i_0; i_1 < min(PB_N-3, -i_0+PB_N); i_1++) {
            A[i_1+2] = B[i_0][i_1+i_0] * A[i_1+3] + A[i_1+1] + A[i_1] + 6;
        }
    }
#pragma endscop
...
kernel_2124212321_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
