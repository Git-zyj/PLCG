...
void kernel_2322352211_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] - E[i_0+2] * E[i_0-1] + F[i_0] - E[i_0] * A[i_0-1] - 2;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        B[i_1-1] = F[i_1] + D[i_1] * E[i_1] - E[1] + E[i_1] * G[i_1][0] * B[i_1] - B[i_1-2] * 1;
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L+2, i_2); i_3++) {
            B[i_3-1] = G[i_3][2] + H[i_3-2][i_2] + G[i_2-1][i_3] + G[i_2-2][i_3] - 1;
        }
    }
#pragma endscop
...
kernel_2322352211_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
