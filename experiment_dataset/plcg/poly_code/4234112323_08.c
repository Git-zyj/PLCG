...
void kernel_4234112323_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_P, PB_L-2); i_2++) {
                A[i_1][i_2] = C[i_0][i_2+i_1][i_0+2] - C[i_2+2][i_0][i_1+2] * A[i_2+1][i_0] + 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = i_3; i_4 < PB_P; i_4++) {
            B[i_4] = A[i_3][i_4] + A[i_4][i_3] * A[i_4+1][i_3] + A[i_4+2][i_3] - 2;
        }
    }
#pragma endscop
...
kernel_4234112323_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
