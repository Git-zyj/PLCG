...
void kernel_2216132221_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_0][i_1-1] = C[i_1][0] * C[i_1+i_0+1][0] + C[i_1-1][i_0] + A[i_1][i_0-3] + A[i_0][i_1+1] - 5;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            A[i_0+1][i_2-1] = C[i_2][i_0] + C[i_2][i_0+2] + C[i_0][i_2] - 4;
        }
    }
#pragma endscop
...
kernel_2216132221_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
