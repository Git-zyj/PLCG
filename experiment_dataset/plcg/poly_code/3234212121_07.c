...
void kernel_3234212121_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_N; i_1++) {
            for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
                A[i_0-2][i_1][i_2-1] = C[i_0][i_1-1] + A[i_1-2][i_2+1][i_0-2] + 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M-1; i_4++) {
            for (int i_5 = 2; i_5 < PB_L; i_5++) {
                B[i_4+i_3][i_3+1][i_5] = B[i_4+i_3+1][i_3+2][i_5-2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3234212121_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
