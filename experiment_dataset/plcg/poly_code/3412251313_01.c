...
void kernel_3412251313_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_N; i_2++) {
                A[i_0][i_0][i_2] = E[i_1][i_2+1][i_0] * F[i_2+1][i_1-1][i_1] - F[i_2][i_0-1][i_1-1] - F[i_2+i_1][i_0][i_2] - A[i_0][i_0][i_2-1] + A[i_0+1][i_0+1][i_2] - A[i_0-1][i_0-1][i_2] * 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_N-1, PB_M); i_4++) {
            for (int i_5 = 1; i_5 < min(-i_4+PB_M, -i_3+PB_M-1); i_5++) {
                B[i_4][i_5] = E[i_4-1][i_5+i_3][i_4] * F[i_4-1][i_3][0] + E[i_5][i_3][i_4] - F[i_4+i_3+1][i_5+1][i_3] * B[i_4][i_5-1] - B[i_5+1][i_3] * 6;
                A[i_4][i_4][i_5] = G[i_3][i_4+1] + G[i_5][i_4] + F[i_4][i_5][i_3+1] + F[i_3][i_4][0] * E[i_5][i_3+1][i_4-1] - F[i_5-1][i_5+i_3+1][i_4+1] + F[i_5+i_3][i_5+i_4][i_4+1] - 5;
            }
            for (int i_6 = 1; i_6 < min(-i_3+PB_L, PB_M); i_6++) {
                B[i_4][i_6-1] = E[i_4][i_6][i_3] - F[i_4][i_6][i_3] * H[i_4-1][i_6] + F[i_4-1][i_3-1][i_6+i_3] * E[i_6][i_4+i_3][i_3] - 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3412251313_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
