...
void kernel_2424131215_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, -i_0+PB_P); i_1++) {
            B[i_1] = E[i_1][i_1+i_0] - F[i_0+1] + E[i_1][i_0] * B[i_1-1] * 5;
        }
        B[i_0] = F[i_0] + E[i_0][0] - 6;
        for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_M-1); i_2++) {
            B[i_2+1] = F[i_0] + F[i_2+i_0] - E[i_0][i_2+i_0] - F[i_2] + F[i_2] + B[i_2+1] * 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        D[i_3] = E[i_3][i_3] + D[i_3+1] - D[i_3-1] - 4;
    }
#pragma endscop
...
kernel_2424131215_08(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
