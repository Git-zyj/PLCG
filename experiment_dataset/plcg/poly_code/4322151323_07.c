...
void kernel_4322151323_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-3; i_2++) {
                A[i_1+1][i_2+1][i_0-1] = D[i_2][i_1-1][i_0] * D[i_0][i_1+1][i_2-1] * D[1][i_0][i_2-1] * D[i_2][i_0+1][i_0] + A[i_1][i_2][i_0-1] - A[i_1+1][i_2+3][i_0-1] + A[i_1-1][i_0+1][i_2-1] * C[i_0][i_1] + 3;
            }
            for (int i_3 = 1; i_3 < -i_1+PB_P-1; i_3++) {
                for (int i_4 = i_3; i_4 < min(-i_0+PB_P+1, PB_L-1); i_4++) {
                    B[i_0][i_1][i_3+1][i_4] = E[i_3][i_4][i_0][i_1] - E[i_3+1][i_4+1][i_1-1][i_0] * E[i_4][i_3][i_0][i_1+1] - F[i_4][i_3+1][i_0][i_0][i_4+i_0-1] * E[i_4+1][i_1+1][i_3][i_0-1] - E[i_1][i_0-1][i_4][i_3] + D[i_3][i_1][i_4] + E[i_4][i_0-1][i_3+i_1+1][i_4] - E[i_3-1][i_4][i_1][i_0+1] + A[i_3+1][i_4+1][i_1-1] * B[i_4+1][i_1][i_3+1][i_0] + B[i_1][i_3][i_0+2][i_4] + 1;
                }
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 0; i_6 < i_5; i_6++) {
            C[i_5][i_6] = G[i_6+1][i_5] - G[i_6+1][i_5+1] - C[i_5+2][i_6+1] + C[i_5-2][i_6+1] * 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4322151323_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
