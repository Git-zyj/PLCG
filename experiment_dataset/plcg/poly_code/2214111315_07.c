...
void kernel_2214111315_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            B[i_1-2][i_0-1] = C[i_1][0] - 1;
        }
    }
    for (int i_2 = PB_L - 1; i_2 >= 2; i_2--) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            B[i_3-1][i_2-1] = B[i_3-1][i_2-2] - B[i_3][i_2-2] + B[i_3-1][i_2] * 4;
        }
    }
#pragma endscop
...
kernel_2214111315_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
