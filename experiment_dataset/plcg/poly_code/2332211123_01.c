...
void kernel_2332211123_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            A[i_0+1][i_1+i_0] = D[i_0+1][i_1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-3; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_N, -i_2+PB_M-3); i_3++) {
            B[0] = 1;
            C[i_3] = A[i_2+3][i_3+i_2+3] * B[0] + 6;
        }
    }
#pragma endscop
...
kernel_2332211123_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
