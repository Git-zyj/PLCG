...
void kernel_3322151113_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L-1 - 1; i_0 >= 1; i_0--) {
        A[i_0-1] = D[i_0][i_0] - E[i_0] - E[i_0+1] * D[i_0][i_0] - 4;
        A[i_0-1] = E[1] * E[i_0] - E[i_0-1] + E[i_0] - E[i_0] + E[i_0] + 4;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            C[i_1][i_2] = D[i_2][i_1] + D[i_2][i_1] * D[i_1][i_2+1] * C[i_1+1][i_2] + 1;
        }
    }
#pragma endscop
...
kernel_3322151113_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
