...
void kernel_4412211221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0+1] = A[i_0-1] - A[i_0+3] - 5;
        for (int i_1 = 2; i_1 < PB_L+2; i_1++) {
            B[i_1-1][i_0] = B[i_0-1][i_1-2] + 2;
        }
        C[i_0] = A[i_0] * A[i_0-1] - C[i_0+2] * 3;
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            D[0] = E[i_3][i_2] - E[i_3][i_2+1] - D[0] * 6;
        }
    }
#pragma endscop
...
kernel_4412211221_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
