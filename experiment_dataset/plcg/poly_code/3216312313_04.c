...
void kernel_3216312313_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_L-3; i_2++) {
                B[i_0+1][i_1+i_0+1][i_2+3] = C[i_0][i_2][i_0-2] - 3;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M, i_3); i_4++) {
            for (int i_5 = i_4; i_5 < PB_L-3; i_5++) {
                B[i_3][i_4+i_3-1][i_5+2] = B[i_3+1][i_4+i_3][i_5+2] + B[i_3-1][i_4+i_3-3][i_5+2] + B[i_3][i_4+i_3-1][i_5+3] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3216312313_04(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
