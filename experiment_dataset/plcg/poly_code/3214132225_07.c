...
void kernel_3214132225_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_2] = C[i_1][i_2][i_0] - C[i_1][i_2][i_0+2] + 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
            B[i_4] = D[i_4-1][i_3] - D[i_4+2][i_3] * D[i_4-1][0] * B[i_4+1] - B[i_4-2] - 4;
        }
    }
#pragma endscop
...
kernel_3214132225_07(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
