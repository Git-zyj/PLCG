...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4424352223_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 3; i_1 < min(-i_0+PB_P, PB_L-2); i_1++) {
            for (int i_2 = 3; i_2 < min(-i_0+PB_P+1, PB_L-1); i_2++) {
                for (int i_3 = 3; i_3 < min(i_0, -i_0+PB_P); i_3++) {
                    A[i_3][i_1][i_2+i_0-1][i_2-1] = E[i_3][i_0][i_1][i_0] - F[i_2][i_1][i_3][i_1][i_0] - G[i_2+1][i_3][i_1-2] * H[i_1][i_2][i_1+i_0][i_3+i_0] * A[i_3+2][i_1+1][i_2+i_0-3][i_2-3] * A[i_3+2][i_1+2][i_2+i_0-1][i_2-1] - A[i_0][i_1][i_3+i_2-3][i_3-3] + 1;
                }
            }
            B[i_1-1] = I[i_1+i_0-1][0] - J[i_1+1][i_0-2] - K[i_0][i_1] + L[i_1+1] * K[i_1][i_0+1] + K[i_1][i_0] + B[i_0-2] * B[i_1-3] - 4;
        }
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 0; i_5 < i_4; i_5++) {
                for (int i_6 = 0; i_6 < -i_4+PB_L-1; i_6++) {
                    C[i_4][i_6][i_0-1][i_6+i_4-1] = M[i_6+2][i_0][i_4][i_5] - H[i_4][i_6][i_6+i_4][i_5+1] - M[i_5][i_6][i_6+i_4+1][i_5] + H[i_6][i_5][i_4][i_0] - 6;
                }
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_N; i_7++) {
        for (int i_8 = i_7; i_8 < -i_7+PB_P-1; i_8++) {
            for (int i_9 = 2; i_9 < PB_L; i_9++) {
                for (int j_0 = 1; j_0 < min(-i_8+PB_P, -i_7+PB_P+1); j_0++) {
                    C[i_7][j_0][i_8-1][j_0+i_7-1] = H[i_7-2][j_0][i_9][i_8] - H[i_8][i_9][i_8+i_7+1][j_0-1] - N[j_0][i_7][i_8] + H[i_9][i_9-2][i_7-2][j_0+i_8] * 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4424352223_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N));
