...
#pragma scop
    ...
    for (int i_1 = PB_Q+1 - 1; i_1 >= 1; i_1--) {
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            C[i_2+1][i_1][i_1-1] = F[i_1][i_2] - F[i_2][i_1] + G[i_2][i_2+1] - G[i_1][i_1] + 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_P; i_4++) {
            for (int i_5 = 2; i_5 < PB_Q+1; i_5++) {
                C[i_3+1][i_5][i_5-1] = H[i_5][i_4-1][i_4+i_3] + H[i_4][i_3+1][i_5+1] + C[i_3+1][i_5-1][i_5-2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3332231125_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
