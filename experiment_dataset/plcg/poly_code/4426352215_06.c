...
void kernel_4426352215_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(min(min(-i_0+PB_M+2, PB_P-2), i_1), -i_1+PB_Q); i_2++) {
                for (int i_3 = 2; i_3 < min(-i_1+PB_Q, -i_0+PB_P-1); i_3++) {
                    A[i_0][i_3][i_2][i_2+i_1] = E[i_2+i_0-2][i_1][i_2][i_3] + F[i_2][i_0][i_1][i_3] - F[i_3-2][i_2+2][i_0-2][i_1-2] + G[i_2+1][i_3-1][i_1][i_0] + E[i_3][i_3+i_0+1][i_2][i_1] + E[i_3][i_2][i_0][i_1-2] * E[i_2][i_3][i_0][i_1+1] - A[i_0][i_2-1][i_3][i_3+i_1] - A[i_1+1][i_3-1][i_2-1][i_2+i_0-2] * 1;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < min(PB_L, -i_4+PB_Q+2); i_5++) {
            for (int i_6 = 2; i_6 < min(-i_5+PB_L, -i_4+PB_P+1); i_6++) {
                for (int i_7 = 2; i_7 < PB_L-1; i_7++) {
                    B[i_6][i_5][i_6][i_7] = G[i_7][i_6+i_5][i_7-2][i_6-2] + H[i_5][i_7+1][i_6+i_4-1][i_4+2][i_5+2] * F[i_4][i_6][i_7][i_5+i_4-2] - I[i_4][i_7][i_6+i_5][0][i_6] + B[i_6-1][i_5][i_6-1][i_7+1] + 5;
                }
            }
        }
        C[i_4-2] = J[i_4-2] - K[i_4+2] * K[i_4] * K[i_4-1] * C[i_4-3] - C[i_4-1] * 5;
    }
    for (int i_8 = 0; i_8 < PB_L; i_8++) {
        for (int i_9 = 0; i_9 < PB_L-1; i_9++) {
            for (int j_0 = max(i_8, 1); j_0 < PB_M; j_0++) {
                B[i_9+1][i_8][i_9+1][j_0-1] = L[i_9][i_8] + L[j_0][i_9] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4426352215_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
