...
void kernel_2326132211_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_L, -i_0+PB_M-3); i_1++) {
            A[i_1+i_0+2][i_0] = D[i_0][i_0] + E[i_1+i_0-1] * D[i_1+i_0+2][0] * D[i_1+i_0+2][2] * D[i_0+2][i_1-1] + A[i_1+i_0+3][i_0] + 6;
        }
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            B[i_0+2][0] = D[i_2][i_0] - F[i_2][i_0][i_0+2] - B[i_0+1][0] - B[i_0+3][0] + 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            C[i_3+2][i_4] = E[i_3] - E[i_4+2] * C[i_3+3][i_4] - C[i_3+2][i_4-1] * 2;
        }
    }
#pragma endscop
...
kernel_2326132211_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
