...
void kernel_3336252215_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        B[i_0] = D[i_0] + E[2] + D[i_0] - E[i_0] * D[i_0+2] + E[i_0] * 3;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        B[i_1] = E[i_1] - E[0] + D[i_1] * F[i_1][i_1] - B[i_1-1] * 3;
        for (int i_2 = min(PB_M, -i_1+PB_L) - 1; i_2 >= 2; i_2--) {
            B[i_2] = G[i_2+2][i_1-2] - H[i_1][i_2+i_1][i_2] * F[i_2][i_1] - F[i_2-2][i_1] - B[i_2] + 4;
        }
    }
#pragma endscop
...
kernel_3336252215_03(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
