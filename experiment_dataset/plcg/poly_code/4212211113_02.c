...
void kernel_4212211113_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = 2; i_5 < i_4; i_5++) {
                B[i_5-1][i_4][i_4-1] = C[i_5][i_3-1][i_4] * B[i_5-2][i_4][i_4-1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4212211113_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
