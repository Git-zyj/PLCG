...
void kernel_4234312223_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
        ...
        for (int i_1 = PB_L - 1; i_1 >= 0; i_1--) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = 2; i_3 < PB_Q; i_3++) {
                    A[i_2][i_3-2][i_1+1] = C[i_1][i_0-2][i_3] - A[i_2][i_3-2][i_1] + 1;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_M-2; i_4++) {
        B[i_4-1] = D[i_4+2] - B[i_4-2] * B[i_4+1] * B[i_4-3] - 6;
    }
#pragma endscop
...
kernel_4234312223_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
