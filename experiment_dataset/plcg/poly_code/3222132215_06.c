...
void kernel_3222132215_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-2; i_1++) {
            for (int i_2 = i_0; i_2 < min(-i_1+PB_N+2, -i_0+PB_P); i_2++) {
                A[i_1][i_1][i_2+i_0] = C[i_2][i_0][i_1+2] - D[i_2+i_1-2][i_0] * C[i_2+1][i_1-1][i_0] * D[i_2][i_1] * A[i_1-1][i_1-1][i_2+i_0-1] * A[i_1+1][i_1+1][i_2+i_0-1] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_P; i_4++) {
            for (int i_5 = 2; i_5 < PB_N+2; i_5++) {
                A[i_5][i_5][i_4+i_3] = D[i_5-2][i_4] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3222132215_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
