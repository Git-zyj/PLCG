...
void kernel_4336152211_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = 2; i_1 < min(PB_Q-2, i_0); i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_Q, i_0); i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_Q; i_3++) {
                    A[i_0][i_3][i_1][i_2+i_0] = D[i_1][i_0][i_2+1][i_3+i_2-2] * D[i_3-1][i_2+i_0][i_3+i_1][i_0-1] + D[i_2+1][i_3+2][i_0][i_1+2] * D[i_1-2][i_0+2][i_2][i_2] + D[i_3][i_1][i_0][1] + D[i_3][i_3][i_0][i_1] - 2;
                }
            }
        }
        B[i_0] = E[i_0] - E[i_0] + E[i_0-2] + E[i_0-1] + E[i_0] * B[i_0+1] * B[i_0-1] - B[i_0-1] - B[i_0-1] + 2;
    }
    for (int i_4 = 0; i_4 < PB_P; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_M-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_M; i_6++) {
                for (int i_7 = 0; i_7 < min(min(PB_M, -i_6+PB_L), -i_4+PB_Q); i_7++) {
                    A[i_7][i_6][i_5+1][i_7+i_4] = D[i_5+1][i_6-1][i_4][i_7] * D[i_7][i_5-2][i_4][i_6] * F[i_7+i_6][i_5-1][i_6] - F[i_5][i_5][i_4] + 3;
                }
            }
        }
    }
...
kernel_4336152211_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
