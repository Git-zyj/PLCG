...
void kernel_2326352323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0] = D[i_0-1] + E[i_0] - F[i_0] + F[i_0+1] + E[i_0] + D[i_0] + D[i_0-2] - A[i_0+2] - A[i_0-2] * 2;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_N-1; i_2++) {
            B[i_1][i_2] = G[i_2][i_1-2] + G[i_2][i_1-1] - H[i_2+1][i_1] - H[i_2+1][i_1] * I[i_1][i_2][i_1+1] * B[i_1-2][i_2] - B[i_1+2][i_2] * 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        A[i_3] = F[i_3-1] + E[0] * D[i_3] - A[i_3] + 5;
    }
#pragma endscop
...
kernel_2326352323_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
