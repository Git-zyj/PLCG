...
void kernel_2426351325_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            B[i_1-1] = E[1] + F[i_1+1][i_0+1] - G[i_1][i_0] - G[i_0][i_1-1] + H[0] + I[i_1][i_0] + J[i_1] * B[i_1-1] * 6;
        }
        B[i_0-1] = E[i_0+1] + J[0] - B[i_0-2] + B[i_0-3] - 2;
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        B[i_2-1] = J[i_2] * H[i_2+1] + H[i_2] * E[i_2] + E[i_2] * H[i_2] * 5;
        for (int i_3 = 0; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            B[i_3+1] = F[i_2][i_3] * F[i_3+i_2][1] - E[i_2+1] + G[i_3][i_2] * 5;
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2426351325_01(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
