...
void kernel_2222131321_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_1+i_0][i_0] = C[i_1][i_0-1] + D[i_1+i_0] * C[i_0][i_1] + A[i_1+i_0-3][i_0-1] + 2;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_M-1; i_2++) {
            B[i_2][i_0] = C[i_0+1][i_2+1] * D[i_0-1] + D[0] + A[i_2+i_0+1][i_2+1] - B[i_0-2][i_2] + B[i_2][i_0-2] + B[i_2][i_0+1] * B[i_2+1][i_0] + 4;
        }
    }
#pragma endscop
...
kernel_2222131321_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
