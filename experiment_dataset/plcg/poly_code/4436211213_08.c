...
void kernel_4436211213_08(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_0][i_2][i_2][i_1-1] = 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_M; i_4++) {
            for (int i_5 = max(1, i_3); i_5 < -i_4+PB_Q; i_5++) {
                for (int i_6 = 0; i_6 < min(PB_M-1, -i_5+PB_M+1); i_6++) {
                    B[i_4][i_5][i_5][i_6] = E[i_6+i_5-1][i_3][i_4][i_5+i_4] + B[i_6+1][i_4][i_4][i_5] * B[i_4-1][i_3][i_3][i_5] - 2;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
        C[i_7] = C[i_7-1] + C[i_7+1] - 5;
        for (int i_8 = i_7 - 1; i_8 >= 1; i_8--) {
            for (int i_9 = 0; i_9 < min(i_7, -i_8+PB_M+1); i_9++) {
                B[i_7][i_8][i_8][i_9+1] = F[i_7-1][i_9+i_8-1][i_9] * G[i_9+1][i_7] * 4;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4436211213_08(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
