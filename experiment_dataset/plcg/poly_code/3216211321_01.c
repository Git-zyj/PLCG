...
void kernel_3216211321_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_L-2); i_1++) {
            A[i_0] = A[i_1+1] - A[i_1-1] * A[i_1+2] + A[i_1-2] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                B[i_4+1][i_3+1][i_4] = C[i_2+1][i_4+1][i_3] + B[i_4+1][i_3+2][i_4] * B[i_4+1][i_3-1][i_4] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3216211321_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
