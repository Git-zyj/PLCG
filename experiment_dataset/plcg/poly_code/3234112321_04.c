...
void kernel_3234112321_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            A[i_0-1][i_1] = A[i_0+1][i_1+1] * A[i_0-1][i_1-1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 3; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 3; i_4 < PB_M-1; i_4++) {
                B[i_2+2][i_3-1][i_4-1] = C[i_4][i_2][i_3] * B[i_2+2][i_3-3][i_4-1] * B[i_2+2][i_3+1][i_4-2] - B[i_2+2][i_3-1][i_4+1] - B[i_2+2][i_3-1][i_4-3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3234112321_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
