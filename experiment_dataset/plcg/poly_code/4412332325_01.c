...
void kernel_4412332325_01(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_L, PB_M-2); i_1++) {
            C[i_1+2] = E[i_0][i_1+2] - E[i_1+2][i_1+i_0] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-4; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            C[i_3-1] = E[i_3+2][i_2] - F[i_2] - C[i_3] - C[i_2-2] * C[i_2] - 5;
        }
        C[i_2] = G[i_2] * F[i_2] + 6;
        for (int i_4 = 3; i_4 < PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_Q+2; i_5++) {
                for (int i_6 = 2; i_6 < PB_P-2; i_6++) {
                    D[i_4-1][i_6][i_5+2][i_2+2] = H[i_4][i_6][i_5][i_2-2] + I[i_6][i_5][i_2+2][i_5+i_4-2] + H[i_6][i_5][i_2-1][i_4] - D[i_4-3][i_6-2][i_5+1][i_2+4] + D[i_4-3][i_5][i_6+2][i_2+2] * 5;
                }
            }
        }
    }
...
kernel_4412332325_01(xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
