...
void kernel_2436231125_03(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            C[i_1+i_0][i_0] = E[i_1][i_0] - F[i_1][i_0] * E[i_1][i_0+1] - 2;
        }
        for (int i_2 = i_0; i_2 < min(-i_0+PB_M+2, PB_M-1); i_2++) {
            C[i_2+i_0-2][i_0] = E[i_2+1][i_0] - 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M, i_3); i_4++) {
            C[i_4+i_3][i_3] = F[i_4+i_3][0] + C[i_4+i_3-1][i_4-1] + 4;
        }
    }
    for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
        for (int i_6 = i_5; i_6 < min(PB_L, -i_5+PB_M); i_6++) {
            C[i_6+i_5-2][i_5-2] = F[i_6][i_6] - F[i_6+i_5][0] - E[i_6][i_5+1] + E[i_5+1][i_6-1] - E[i_6][i_6] + 5;
        }
    }
#pragma endscop
...
kernel_2436231125_03(xc,yc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
