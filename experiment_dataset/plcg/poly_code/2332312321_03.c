...
void kernel_2332312321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q-1; i_1++) {
            A[i_1][i_1] = A[i_1-2][i_1-2] * A[i_1+1][i_1+1] - 4;
        }
        B[i_0] = D[i_0] + D[i_0] * A[i_0][i_0] * B[i_0-1] + B[i_0-2] * B[i_0+1] * 5;
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_L, PB_M-2); i_3++) {
            C[i_2+2][i_3+i_2] = E[i_2] * C[i_3+2][i_3+i_2-2] - C[i_2+1][i_3+i_2-1] * 1;
        }
    }
#pragma endscop
...
kernel_2332312321_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
