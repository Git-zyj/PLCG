...
void kernel_2234152211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0][i_0+2] = C[i_0] * D[i_0][i_0] + A[i_0+1][i_0+3] * A[i_0-1][i_0+1] - 6;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_M, PB_N-1); i_2++) {
            B[i_2][i_1] = D[i_1+2][i_2] - D[i_1][i_2] * D[i_2+i_1][i_2] + C[i_2+2] * C[i_2-2] * C[i_1+2] + D[i_1][i_2] + B[i_2+1][i_1+1] * B[i_2][i_1-1] + 3;
        }
    }
#pragma endscop
...
kernel_2234152211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
