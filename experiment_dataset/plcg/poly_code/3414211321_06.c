...
void kernel_3414211321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
            B[i_3] = C[1][i_2+1] * C[1][i_2-1] + 2;
            C[1][i_2-1] = B[i_3] + B[i_3-2] - B[i_3+1] + B[i_2] - D[i_2-2] + 4;
        }
...
kernel_3414211321_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
