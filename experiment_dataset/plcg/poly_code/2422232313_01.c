...
void kernel_2422232313_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_1] = E[i_1][i_0] + F[i_0] * G[i_0][i_0] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2] = H[0] * F[i_2] - B[i_2+1] - B[i_2-1] + B[i_2-1] - 3;
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M, PB_L); i_4++) {
            B[i_4+1] = F[i_4-2] * G[i_3][i_4-2] - G[i_4+i_3][i_3] + G[i_4+i_3][i_4-1] * I[i_3][i_4+2][i_3+2] + E[i_4+2][i_4] * 2;
        }
        B[i_3] = H[i_3] - 5;
    }
#pragma endscop
...
kernel_2422232313_01(xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
