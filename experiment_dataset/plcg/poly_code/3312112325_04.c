...
void kernel_3312112325_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+1; i_0++) {
        ...
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            B[i_0-1][i_3] = B[i_0-1][i_3-1] * B[i_0-3][i_3] * B[i_0-2][i_3-2] * 2;
        }
    }
    for (int i_4 = 2; i_4 < PB_L+2; i_4++) {
        for (int i_5 = 2; i_5 < min(PB_L, -i_4+PB_P); i_5++) {
            for (int i_6 = 0; i_6 < min(min(-i_5+PB_L+2, PB_L-1), -i_4+PB_P+2); i_6++) {
                C[i_4-1][i_5][i_6+2] = D[i_6][i_4-2][i_5+i_4] + D[i_4][i_6+i_5-2][i_6+i_4-2] * C[i_4+1][i_5][i_6+4] + C[i_4-1][i_5][i_6+3] * C[i_5-1][i_6+1][i_4+3] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3312112325_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
