...
void kernel_4436132225_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            B[i_1+i_0][i_0] = E[i_1+i_0] + F[i_1][i_0] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q+2; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, PB_M-2); i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_M-2; i_4++) {
                B[i_4+i_3][i_4] = G[i_4][i_4+1][i_2-2] + B[i_4+i_3-1][i_4-1] - B[i_4+i_3+2][i_4] * 2;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_P-2; i_5++) {
        for (int i_6 = 0; i_6 < PB_M-4; i_6++) {
            for (int i_7 = 0; i_7 < min(-i_5+PB_M, -i_6+PB_M-4); i_7++) {
                B[i_7+i_6+4][i_6+2] = F[i_7+i_5][i_6] + G[i_6][i_5+2][i_5] + 4;
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_P; i_8++) {
        for (int i_9 = i_8; i_9 < min(-i_8+PB_P, PB_M-2); i_9++) {
            for (int j_0 = 1; j_0 < -i_9+PB_M-2; j_0++) {
                B[j_0+i_9+2][j_0] = G[j_0][i_9+i_8][i_8] + G[i_8+2][j_0-1][i_9] - G[i_8][j_0][i_9+1] * H[i_8-1][i_9+1][j_0][i_8] - 5;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436132225_02(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
