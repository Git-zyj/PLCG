...
void kernel_4426331325_08(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_2+PB_L, PB_L-1); i_3++) {
                    D[i_0][i_2][i_3][i_1] = E[i_2][i_2][i_3-1][i_0] + F[i_1+1][i_3+i_2][i_1] - G[i_3+1][i_2][i_1][i_0-1] * H[i_1][i_2][i_1-1][i_2+1] * 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_M; i_5++) {
            B[i_4][i_5] = I[i_4][i_5+1] - J[i_5][i_4-1] + J[i_5][i_4+1] * K[i_5] + I[i_5][i_4] - B[i_4-2][i_5] + B[i_4+1][i_5] * B[i_4-1][i_5] + 4;
            for (int i_6 = 1; i_6 < i_4; i_6++) {
                for (int i_7 = 1; i_7 < PB_Q+1; i_7++) {
                    D[i_5][i_6-1][i_4-2][i_7-1] = 1;
                }
            }
        }
        for (int i_8 = 0; i_8 < i_4; i_8++) {
            for (int i_9 = i_8; i_9 < PB_L-2; i_9++) {
                for (int j_0 = 0; j_0 < PB_Q-1; j_0++) {
                    D[i_8][i_9][i_4][j_0] = G[i_8][j_0][i_9][i_4-1] + H[i_8][i_4+1][j_0][i_9] - D[i_4][i_9+2][i_8][j_0] * D[i_8][i_9+2][i_4][j_0+1] - 2;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4426331325_08(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K)));
