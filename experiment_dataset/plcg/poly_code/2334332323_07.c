...
void kernel_2334332323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0-1] = D[i_0] - E[i_0] * D[i_0-2] * A[i_0-2] + 2;
    }
    for (int i_1 = 4; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            B[i_1-2][i_2+1] = F[i_1][i_2] + B[i_1-2][i_2+2] + B[i_1-4][i_2+1] + 6;
            B[i_1-2][i_2+1] = G[i_1][i_1] + H[i_2+1][i_1+1][i_2+i_1-2] + I[i_2][i_1+1] - F[0][i_2+i_1] * A[i_2-1] - A[i_2] - 1;
        }
    }
#pragma endscop
...
kernel_2334332323_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
