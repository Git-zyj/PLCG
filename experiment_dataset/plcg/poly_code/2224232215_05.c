...
void kernel_2224232215_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_L-1, -i_0+PB_P); i_1++) {
            A[i_1][i_1] = C[i_1-2][i_1+i_0] - C[i_1-1][i_0] - C[i_1-2][i_0] + A[i_1+1][i_1+1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        A[i_2][i_2] = D[i_2] - D[i_2] - A[i_2][i_2] * 1;
    }
#pragma endscop
...
kernel_2224232215_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
