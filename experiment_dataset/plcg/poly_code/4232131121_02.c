...
void kernel_4232131121_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        B[0][i_0+1] = C[i_0][i_0] * D[i_0-1] * B[0][i_0+1] * 1;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_L-1); i_2++) {
            B[0][i_2+1] = C[i_2+i_1][0] + C[i_1][i_2] + C[i_1-1][i_2+1] - 4;
        }
    }
#pragma endscop
...
kernel_4232131121_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
