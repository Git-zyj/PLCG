...
void kernel_2434111323_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        A[i_0-1][i_0+1] = E[i_0-1] * A[i_0][i_0+2] * 1;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < min(PB_N-1, i_1); i_2++) {
            B[i_1-1][i_2] = E[i_2+1] + B[i_1-1][i_2-2] - B[i_2][i_1] + 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
        for (int i_4 = i_3; i_4 < min(-i_3+PB_M, PB_M-2); i_4++) {
            C[i_3][i_4] = F[i_4+i_3][i_4] + C[i_3+2][i_4] + C[i_3][i_4-1] + C[i_3][i_4+2] - 4;
        }
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            D[i_3][i_5] = F[i_5][i_3] - D[i_3-1][i_5] + D[i_3+2][i_5] + D[i_3][i_5-1] + 3;
        }
    }
#pragma endscop
...
kernel_2434111323_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
