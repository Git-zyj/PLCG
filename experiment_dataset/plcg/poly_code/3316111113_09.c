...
void kernel_3316111113_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        ...
        for (int i_5 = 0; i_5 < min(PB_N-1, i_3); i_5++) {
            C[i_3] = D[i_3][i_5] - C[i_5+1] * C[i_3-1] + 4;
        }
    }
#pragma endscop
...
kernel_3316111113_09(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
