...
void kernel_2336111223_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1] = D[i_1+i_0][0] + B[i_0-2][i_1+1] - 1;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            B[i_2-1][i_3] = A[i_2][i_3] - B[i_2-3][i_3] + 1;
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = 3; i_5 < PB_M+1; i_5++) {
            C[i_5-1][i_4] = C[i_5-1][i_4-1] * C[i_5-1][i_4-2] + C[i_5-3][i_4] - 1;
        }
    }
#pragma endscop
...
kernel_2336111223_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
