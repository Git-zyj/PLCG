...
#endif
static void init_array(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl),int seed)
{
srand(seed);
...
void kernel_4434352211_01(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                for (int i_3 = 2; i_3 < min(min(-i_2+PB_L+2, PB_L-2), -i_1+PB_Q); i_3++) {
                    D[i_2][i_1+2][i_2][i_3+i_1] = E[i_3+i_2][i_1][0][i_0] + E[i_2+i_1][i_0][i_3+2][0] + F[i_2+1][i_3-2][i_3+1][i_1+2] * E[i_1][i_3][i_3+i_2-2][0] + E[i_3][i_2][i_1+i_0][i_0+2] + 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 2; i_5 < min(i_4, -i_4+PB_P); i_5++) {
            B[i_4][i_5+i_4] = G[i_5+1][i_4+1] - H[i_5+1][i_4] * I[i_5-2][i_4] + I[i_5][2] - G[i_5-2][i_5-1] * B[i_4-1][i_5+i_4-1] - 6;
        }
    }
    for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
        for (int i_7 = 2; i_7 < i_6; i_7++) {
            for (int i_8 = i_6; i_8 < min(-i_7+PB_Q, -i_6+PB_L); i_8++) {
                for (int i_9 = 2; i_9 < -i_8+PB_L-2; i_9++) {
                    C[i_7-2][i_6+1][i_8][i_9+i_8] = E[i_9][i_9-2][i_8+1][i_7] + J[i_9+i_8][0][i_7] + K[i_7][i_9+i_8+2][i_6][i_9-2][i_6+1] - F[i_7-2][i_8+i_6][0][i_9] - C[i_7-2][i_6+1][i_8-1][i_9+i_8-2] - D[i_6][i_8+2][i_6][i_8+i_7] + 6;
                }
            }
        }
        for (int j_0 = 2; j_0 < i_6; j_0++) {
            for (int j_1 = 1; j_1 < min(-i_6+PB_L+2, -j_0+PB_Q-1); j_1++) {
                for (int j_2 = 2; j_2 < -j_0+PB_Q; j_2++) {
                    D[i_6][j_2+2][i_6][j_2+j_0] = E[j_1][j_0+1][i_6][j_2-1] * F[j_2-2][j_1+i_6-2][j_0][i_6] - E[j_1-1][j_2][j_0][i_6] + L[j_0+1][j_2-1][i_6][j_1] + D[i_6][j_2+1][i_6][j_2+j_0-2] * D[j_2-1][j_1+3][j_2-1][j_1+j_0+1] + 1;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4434352211_01(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L)));
