...
void kernel_4314152325_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
                for (int i_3 = i_1; i_3 < min(-i_2+PB_Q, -i_0+PB_P); i_3++) {
                    A[i_0][i_1][i_0+2][i_2] = D[i_0][i_2-2][1][i_0+1] * D[i_0][i_2][i_1][i_3+i_2] + E[i_2][i_3+i_0][i_1+2][i_3][i_0-1] - A[i_0-2][i_1][i_0][i_2] - A[i_0][i_3][i_0+2][i_2+2] - 1;
                }
            }
        }
        B[i_0] = F[i_0+2] * F[i_0] + F[0] - F[i_0] * B[i_0+1] * B[i_0-1] * B[i_0+2] + B[i_0-2] - 5;
    }
        ...
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_Q; i_6++) {
                for (int i_7 = i_6; i_7 < min(-i_5+PB_P, -i_6+PB_P-1); i_7++) {
                    A[i_4][i_5][i_4+2][i_7] = E[i_6-1][i_7+i_5][i_5-1][i_4][i_4] - D[i_7][i_6][i_5][i_4] * D[i_7][i_4-2][i_5+2][i_6+1] * D[i_5-1][i_6][i_5][i_4] * D[i_4][0][i_4][i_6+i_5] + G[i_5][i_7][i_6] + D[i_5-1][i_4][i_7+i_6+1][i_4+1] + 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4314152325_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
