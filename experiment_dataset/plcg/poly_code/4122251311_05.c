...
void kernel_4122251311_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-1 - 1; i_0 >= 1; i_0--) {
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_P-1); i_1++) {
            A[i_0][i_1+i_0][0] = B[i_1][i_0+1] * C[1] + B[i_1+1][i_0] * B[i_1][i_0-1] - B[i_0][i_1] * A[i_0][i_1+i_0+1][0] * A[i_0+1][i_1+i_0+1][0] * A[i_0][i_1+i_0-1][0] - 3;
        }
    }
#pragma endscop
...
kernel_4122251311_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
