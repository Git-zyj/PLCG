...
void kernel_3426111215_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
            C[i_0][i_2-1] = C[i_0][i_2-2] + C[i_2-1][i_0-2] * 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_L, i_3); i_4++) {
            D[i_4][i_3-1] = D[i_4-1][i_3-1] - 1;
        }
    }
#pragma endscop
...
kernel_3426111215_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
