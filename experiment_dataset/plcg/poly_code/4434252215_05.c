...
void kernel_4434252215_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_0][i_0] = E[i_1+2][i_0] + E[i_0][i_0] * F[i_1+i_0][i_0] * E[i_1][i_0] + F[i_1][i_0] * A[i_0+1][i_0+1] * A[i_0-1][i_0-1] * 2;
        }
        for (int i_2 = 2; i_2 < PB_P; i_2++) {
            for (int i_3 = i_0; i_3 < -i_2+PB_P; i_3++) {
                B[i_3-1][i_2][i_0-1] = G[i_0-2][0][i_3+i_2] * G[i_2][i_3][i_0] * E[i_3][i_2+1] - H[i_3][i_0+1][i_2-2] - B[i_3-1][i_2-1][i_0-2] - 4;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_P; i_5++) {
            for (int i_6 = 2; i_6 < min(PB_Q, -i_4+PB_L+2); i_6++) {
                A[i_5-1][i_5-1] = H[i_5][i_6][i_4] * H[i_6][i_6+i_4-2][i_5+1] + G[i_4][i_6][i_5] + I[i_4][i_6+2][i_5][2] - G[i_6-2][i_5][i_4] - 4;
            }
        }
        for (int i_7 = 0; i_7 < PB_L; i_7++) {
            A[i_4+1][i_4+1] = E[i_4+2][i_7] * E[i_4][i_7] + J[i_4+1] * 2;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4434252215_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
