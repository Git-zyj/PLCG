...
void kernel_3232132213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, -i_0+PB_P-1); i_1++) {
            A[i_0][i_1+i_0] = C[i_1] * A[i_0][i_1+i_0+1] + A[i_1-1][i_1+i_0-1] * A[i_0-1][i_1+i_0-1] + 1;
        }
        B[i_0] = C[i_0] * C[i_0] + C[i_0+1] * C[i_0] * B[i_0-1] + 2;
    }
#pragma endscop
...
kernel_3232132213_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
