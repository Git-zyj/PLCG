...
void kernel_2312111311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_1+1] = D[i_1][i_0] * A[i_0-1][i_1+1] - A[i_0+1][i_1+1] - A[i_0+1][i_1+2] + 3;
        }
    ...
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_M-1, i_3); i_4++) {
            C[i_3-1][i_4] = C[i_3-2][i_4+1] - C[i_3][i_4] * C[i_3-1][i_4-1] * C[i_3][i_4+1] - C[i_3][i_4-1] - 6;
        }
    }
#pragma endscop
...
kernel_2312111311_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
