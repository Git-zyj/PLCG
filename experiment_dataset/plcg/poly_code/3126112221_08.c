...
void kernel_3126112221_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_L, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_L-4; i_2++) {
                A[i_2+2][i_1-2] = B[i_1][i_2][i_0-1] * A[i_2+4][i_1-2] + A[i_2+3][i_1-3] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3126112221_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
