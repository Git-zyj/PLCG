...
void kernel_3226212123_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, i_0); i_1++) {
            A[i_1-1] = C[i_1][i_0] - C[i_1][i_0+1] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = i_3; i_4 < PB_M; i_4++) {
                B[i_2][i_4][i_3] = B[i_2+1][i_4][i_3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3226212123_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
