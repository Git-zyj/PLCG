...
void kernel_4426351125_04(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_L); i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_N; i_3++) {
                    B[i_1-1][i_2][i_3][i_1] = E[0][i_2-1][i_0][i_1+1] - F[i_3+i_1][i_2+1][i_3][i_0+1] * G[i_2+i_1][i_3][i_0][i_2] * G[i_3][i_1+1][i_2][i_0+1] - F[i_2+1][i_3][i_1-1][i_2] + 5;
                }
            }
        }
        for (int i_4 = 1; i_4 < min(PB_P-2, i_0); i_4++) {
            for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
                for (int i_6 = 1; i_6 < -i_4+PB_P-1; i_6++) {
                    B[i_6-1][i_4][i_5][i_6] = G[i_5][i_0][i_6+i_4+1][i_4-1] + G[i_5][i_0+1][i_6+i_4][i_5] * G[i_6][i_4+1][i_0][i_4] + G[i_6][i_4+1][i_0+1][i_5+1] + F[i_5][i_0][i_4+1][i_6] * B[i_0+1][i_4][i_6][i_0+2] + 4;
                }
            }
            for (int i_7 = 1; i_7 < min(-i_0+PB_M, PB_L); i_7++) {
                B[i_7-1][i_4+2][i_0-1][i_7] = H[i_7][i_0][i_4+1] + I[i_7+i_0][i_4-1] - J[i_7][i_4+1][i_0] - J[i_4-1][i_0+1][i_4] - K[i_4+1][i_7][0] + J[i_4-1][i_7][i_0] * 6;
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_L; i_8++) {
        for (int i_9 = 1; i_9 < min(i_8, PB_P-1); i_9++) {
            for (int j_0 = 0; j_0 < min(PB_Q-2, -i_8+PB_M+1); j_0++) {
                B[i_8-1][i_9+1][j_0+2][i_8] = H[j_0][i_8][i_9-1] * L[j_0+i_8-1][i_9-1] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4426351125_04(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L)));
