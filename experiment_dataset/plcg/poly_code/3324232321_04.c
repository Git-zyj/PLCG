...
void kernel_3324232321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[0] - E[i_0] + A[i_0+2] + A[i_0-1] - 6;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_M-2); i_1++) {
            B[i_0] = F[i_1][i_1] + G[i_0][0] - F[1][i_1+i_0] + B[i_1-1] * B[i_1+2] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-3; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_L; i_3++) {
            for (int i_4 = max(2, i_3); i_4 < min(PB_L, -i_2+PB_Q-2); i_4++) {
                C[i_2+2][i_4+i_3][i_2] = H[i_4][i_3+i_2][i_2][1] + I[i_3][i_4][i_2] * G[i_4][0] + I[i_4-2][i_2][i_3+2] - C[i_2+2][i_4+i_3+1][i_2] + C[i_2+3][i_4+i_3-1][i_2+1] + C[i_2][i_4+i_3][i_2-2] * C[i_3+2][i_4+i_2-1][i_3] * C[i_3+2][i_4+i_2+2][i_3] - 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3324232321_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
