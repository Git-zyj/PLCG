...
void kernel_4426151215_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[i_0-1] - E[i_0] - E[i_0-1] + A[i_0+2] - A[i_0] * 4;
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                B[i_1][i_2-1] = F[i_1-1][i_2] + F[i_2+1][i_0] + G[i_2][i_0][i_1] * G[i_1][i_2][i_0+1] + 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            A[i_4+1] = E[i_4+1] * F[i_3-1][i_4-1] + F[i_3][i_4] * F[i_4][i_3+1] + F[i_4+1][i_3-1] * F[i_4-1][i_3] + A[i_4] - 6;
        }
    }
        ...
        for (int i_6 = i_5; i_6 < -i_5+PB_L; i_6++) {
            for (int i_7 = 1; i_7 < PB_Q; i_7++) {
                for (int i_8 = 1; i_8 < PB_M-1; i_8++) {
                    B[i_8][i_7] = H[i_8-1][i_5][i_8][i_7] * G[i_5-1][i_7][i_8] - H[i_8][i_7][i_6+i_5][i_8] - H[i_8+1][i_5-1][i_6][i_7] * G[i_8-1][i_6+i_5][i_7-1] + 3;
                }
            }
        }
    }
...
kernel_4426151215_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
