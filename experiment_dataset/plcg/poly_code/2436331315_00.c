...
void kernel_2436331315_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_N+1; i_2++) {
            B[i_2+i_1-1][i_1-1] = H[i_2+1][i_1] * E[i_1-1] + H[i_2][i_1-1] - I[i_2][i_1-1] + A[i_2] * B[i_2+i_1-2][i_1-1] + 6;
        }
        for (int i_3 = i_1; i_3 < PB_M; i_3++) {
            A[i_3] = H[i_3][i_1-1] * J[i_3][i_1-1] - A[i_1] * A[i_3] * 5;
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_L-1); i_5++) {
            D[i_4] = E[i_4] + I[i_4][i_5] - I[i_5-1][i_4] * A[i_5] * D[i_5+1] + D[i_5-1] + 5;
        }
    }
#pragma endscop
...
kernel_2436331315_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
