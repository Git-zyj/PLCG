...
void kernel_4436351111_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_1][i_0] = E[i_1][i_0][0] - F[i_1+i_0][0] - G[i_1][i_0-1] - A[i_1+1][i_0] - 5;
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = i_2; i_3 < PB_L; i_3++) {
                    B[i_0+1][i_1-1][i_2][i_3] = H[i_3][i_0][i_1][i_2] + I[i_2+1][i_0+1][i_3][i_1-1] * J[i_2+1][i_0][i_3][i_1] - H[i_3][i_2][i_0][i_1] * 1;
                }
            }
        }
        for (int i_4 = i_0; i_4 < min(-i_0+PB_M+1, PB_M-1); i_4++) {
            C[i_4][i_0-1] = F[i_4+1][i_0] + K[i_4][i_0-1] + G[i_4+1][i_0] - F[i_4-1][i_0] * F[i_4+i_0-1][i_4-1] * K[i_4-1][i_0] - 6;
        }
    }
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 1; i_6 < min(i_5, -i_5+PB_M+1); i_6++) {
            D[i_6][i_5] = G[i_6+i_5-1][0] + F[i_6][i_5+1] + L[1] - F[i_6][i_5] + F[i_6-1][i_5] * 5;
        }
    }
#pragma endscop
...
kernel_4436351111_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
