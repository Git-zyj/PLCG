...
void kernel_3434331111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0][i_0] = E[i_0] + E[i_0+1] + 4;
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = min(min(-i_1+PB_M, -i_0+PB_M), i_1) - 1; i_2 >= 0; i_2--) {
                B[i_0][i_1][i_2] = F[i_1-1][i_2+i_0][i_1] + G[i_2+i_1][i_0-1][i_1+1] - F[i_2][i_0][i_1] * G[i_2][i_2][i_0-1] + B[i_0][i_1][i_2+1] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            C[i_3-1][i_4] = E[i_3-1] + 3;
        }
    }
    for (int i_5 = 1; i_5 < PB_Q; i_5++) {
        for (int i_6 = 1; i_6 < PB_Q; i_6++) {
            D[i_5][i_6-1] = H[i_5][0] + H[i_6][i_5] - H[i_6][i_5] * I[i_6-1][i_5-1] - D[i_5-1][i_6-1] + D[i_5+1][i_6] + 2;
        }
    }
#pragma endscop
...
kernel_3434331111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
