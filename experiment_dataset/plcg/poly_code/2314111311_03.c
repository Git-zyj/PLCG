...
void kernel_2314111311_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            B[i_1][i_2+1] = E[i_2+i_1][i_2] + E[i_1][i_1] - B[i_1][i_2+2] - 2;
        }
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            B[i_1][i_3+2] = B[i_1][i_3+1] + 4;
        }
    }
#pragma endscop
...
kernel_2314111311_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
