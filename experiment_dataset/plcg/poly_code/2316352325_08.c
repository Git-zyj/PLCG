...
void kernel_2316352325_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        B[i_0] = D[0] + E[i_0-1] + F[i_0][i_0] + G[i_0+2] - G[i_0] * G[i_0+1] + 2;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        B[i_1] = G[i_1] + D[i_1+1] + G[i_1] * H[i_1][0] - B[i_1+1] + B[i_1+2] + 5;
        for (int i_2 = 2; i_2 < min(PB_N+2, -i_1+PB_L); i_2++) {
            C[i_1][i_2+2] = D[i_2+1] + I[i_2-2][i_1] + G[i_2-2] - H[i_1][i_2+i_1] * F[i_2][i_1] * C[i_1-2][i_2+2] * C[i_2-1][i_1+4] - C[i_1][i_2+4] * C[i_1-1][i_2+1] + 2;
        }
    }
#pragma endscop
...
kernel_2316352325_08(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
