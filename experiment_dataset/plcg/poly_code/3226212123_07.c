...
void kernel_3226212123_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            B[i_0][i_1-2][i_0-1] = C[i_1][i_0] * 5;
        }
        for (int i_2 = i_0; i_2 < PB_L+2; i_2++) {
            for (int i_3 = i_0; i_3 < PB_M; i_3++) {
                B[i_0][i_2-2][i_0-1] = B[i_0-1][i_2-2][i_0-2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3226212123_07(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
