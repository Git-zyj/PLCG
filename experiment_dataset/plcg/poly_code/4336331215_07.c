...
#pragma scop
    ...
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-1; i_5++) {
            B[i_5-1][i_5] = F[i_5][i_4] - G[i_5][i_4-1] * G[i_5+1][i_4+1] - F[i_5-1][i_4+1] - B[i_5][i_5+1] * 5;
        }
    }
    for (int i_6 = 0; i_6 < PB_M; i_6++) {
        for (int i_7 = max(i_6, 1); i_7 < PB_L; i_7++) {
            B[i_7-1][i_7] = H[i_6][i_7] * G[i_6][i_7] * 6;
        }
    }
#pragma endscop
...
kernel_4336331215_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
