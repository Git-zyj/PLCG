...
void kernel_2236232211_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_N+1, PB_L); i_1++) {
            A[i_0-2][i_1] = C[i_1+i_0-1][i_1] * C[i_1+1][i_0] + C[i_1][i_0] + C[i_1-1][i_0] + A[i_0-2][i_1-1] - A[i_1-2][i_0+1] - 3;
        }
    }
    for (int i_2 = PB_M - 1; i_2 >= 2; i_2--) {
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            A[i_2-2][i_3] = D[i_3-1][0] * D[i_2][i_3] - 1;
        }
    }
#pragma endscop
...
kernel_2236232211_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
