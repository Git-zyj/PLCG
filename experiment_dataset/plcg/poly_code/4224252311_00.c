...
void kernel_4224252311_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-3; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < min(-i_0+PB_L, -i_1+PB_Q); i_2++) {
                A[i_0][i_1+2][i_2] = C[i_0][i_2-2] * D[i_1][i_2][i_2+i_1] + D[i_1][i_2+i_0][i_2] + A[i_0+1][i_1+2][i_2] * A[i_0][i_1+1][i_2] - A[i_0-1][i_1+2][i_2-1] - A[i_0+1][i_1+3][i_2] * A[i_0-1][i_1+1][i_2+1] - 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L; i_4++) {
            for (int i_5 = 2; i_5 < -i_3+PB_L-1; i_5++) {
                for (int i_6 = 2; i_6 < min(-i_3+PB_M-2, -i_4+PB_L); i_6++) {
                    B[i_3-1][i_6+i_4][i_5-1][i_5] = E[i_4][i_5+2][i_3][0] * E[i_6-2][i_3-2][i_4][i_5-2] * E[i_4+1][i_3][i_6+1][i_5] - F[i_5][i_6+i_3+2][i_4][i_5+2] * G[i_5][i_4][i_4+2] * F[i_5][i_3][i_6][i_4] + B[i_4-1][i_5+i_3+1][i_6-1][i_6] * 2;
                }
            }
        }
    }
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4224252311_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
