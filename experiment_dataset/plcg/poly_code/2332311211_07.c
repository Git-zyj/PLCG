...
void kernel_2332311211_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = 3;
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            B[i_0][i_1+i_0] = A[i_0] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_M, -i_2+PB_N); i_3++) {
            C[i_3+i_2][i_3] = D[i_2+1][i_3] - B[i_2][i_3+i_2] - B[i_3][i_3+i_2] * C[i_3+i_2-1][i_3-1] + 2;
        }
    }
#pragma endscop
...
kernel_2332311211_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
