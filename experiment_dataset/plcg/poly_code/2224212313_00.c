...
void kernel_2224212313_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            A[i_1][i_1-1] = A[i_1-1][i_1-2] + A[i_1+1][i_1] + A[i_1+1][i_1] - 6;
        }
        for (int i_2 = 1; i_2 < min(min(i_0, PB_L-1), -i_0+PB_P); i_2++) {
            A[i_2][i_2-1] = C[i_2][i_2+i_0] - C[i_2+1][i_0] - 1;
        }
    }
#pragma endscop
...
kernel_2224212313_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
