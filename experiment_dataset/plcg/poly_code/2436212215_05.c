...
void kernel_2436212215_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M+2, PB_M-1); i_1++) {
            B[i_1+1] = E[i_1+i_0-2][0] + 5;
        }
    }
    ...
    for (int i_5 = 0; i_5 < PB_N-2; i_5++) {
        for (int i_6 = 0; i_6 < min(PB_N-3, i_5); i_6++) {
            D[i_5+2] = D[i_6+3] - D[i_5+1] + D[i_6+1] * 3;
        }
    }
#pragma endscop
...
kernel_2436212215_05(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
