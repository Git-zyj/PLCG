...
void kernel_2436252121_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L+2; i_0++) {
        A[i_0-2] = E[i_0+1] + E[i_0] - F[i_0] - F[0] - F[i_0] - E[i_0] - E[i_0] * A[i_0-3] * 3;
    }
    for (int i_1 = 2; i_1 < PB_L+1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L+1; i_2++) {
            B[i_1-2][i_2+1] = G[i_1][i_2-1] * G[i_2-1][i_1-1] * 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_M-1; i_4++) {
            D[i_4+1] = G[i_4][i_3] - G[i_4][i_3] + H[i_4-1][i_3+1] - H[i_4][i_3] - H[i_4-2][i_3-1] + 6;
        }
        for (int i_5 = 0; i_5 < min(PB_L, -i_3+PB_M-1); i_5++) {
            D[i_5] = H[i_3][i_5] * E[i_5] + H[i_5][i_3] * H[i_5+i_3][i_5] + G[i_5+i_3+1][2] - 4;
        }
    }
#pragma endscop
...
kernel_2436252121_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
