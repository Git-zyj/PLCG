...
void kernel_3216111111_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < min(min(PB_M, -i_0+PB_L), i_0); i_1++) {
            B[i_1+i_0][i_1-1][i_0] = C[i_1][i_0-1] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_Q-1, -i_2+PB_L-1); i_3++) {
            B[i_3+i_2+1][i_3][i_2] = B[i_3+i_2+1][i_3+1][i_2-1] - 4;
        }
    }
#pragma endscop
...
kernel_3216111111_09(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
