...
void kernel_2436252121_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] - E[i_0] * E[i_0] - F[i_0] + A[i_0-2] * A[i_0+2] - 3;
    }
    for (int i_1 = 4; i_1 < PB_M; i_1++) {
        C[i_1-4] = F[i_1] * G[i_1][i_1] * H[i_1-2][i_1-1] + E[i_1] + 2;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            C[i_3-2] = G[i_3+1][1] - H[i_2][2] * G[i_3][i_2] - G[i_3][i_2] + E[i_2] - F[2] * 2;
        }
        D[i_2-1] = E[i_2] + E[i_2] * F[i_2] - C[i_2-2] - 6;
    }
#pragma endscop
...
kernel_2436252121_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
