...
void kernel_3424352125_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_N, -i_0+PB_L); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_N, -i_0+PB_N); i_2++) {
                B[i_2+2][i_1] = E[i_2+i_1][i_0][i_1-1] * E[i_1-1][i_0-2][i_2] * F[i_2][i_1] * G[i_2+i_0][i_1][i_0][i_1+1] * E[i_2][i_1+i_0][i_1] - H[i_1][i_0][i_0] + E[i_0][i_2][i_1+2] * 2;
            }
        }
        for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
            for (int i_4 = 2; i_4 < i_0; i_4++) {
                B[i_3+2][i_4] = E[i_3+2][i_4][0] - I[i_0][i_4+1][i_3] - E[i_0][i_4][i_3+2] - E[i_0+2][i_4][i_3] - B[i_3+2][i_4-2] + 4;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_N; i_5++) {
        for (int i_6 = 0; i_6 < -i_5+PB_N; i_6++) {
            B[i_5+2][i_6] = F[i_6+1][i_5] * F[i_6+i_5][0] + F[i_6+i_5][i_6] - 3;
            for (int i_7 = i_5; i_7 < min(PB_N, -i_5+PB_L+2); i_7++) {
                B[i_7][i_5] = H[i_6][i_5][2] + I[i_7][i_5-1][i_6] * J[i_7][i_6] - E[i_7][i_5][i_6] + H[i_6][i_7+i_5-2][i_5] - H[i_7+2][i_5][i_6] + 5;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3424352125_09(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
