...
void kernel_2414332211_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] * F[i_0] * F[2] + E[i_0] - G[i_0+2][i_0] * A[i_0+1] * B[i_0] * 5;
        B[i_0] = H[i_0] * E[i_0] - E[i_0] + 6;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        C[i_1] = I[i_1+1][i_1] + H[i_1-2] + C[i_1-1] * C[i_1+1] - 1;
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            D[i_2][i_1-1] = D[i_2][i_1] - D[i_2+1][i_1-1] * 5;
        }
    }
#pragma endscop
...
kernel_2414332211_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
