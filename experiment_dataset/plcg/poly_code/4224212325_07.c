...
void kernel_4224212325_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-5; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_Q-5; i_1++) {
            for (int i_2 = -i_1+PB_Q-1 - 1; i_2 >= 0; i_2--) {
                for (int i_3 = 0; i_3 < PB_Q; i_3++) {
                    A[i_2][i_1][i_2+i_1+1][i_3] = A[i_0+2][i_1+2][i_1+i_0+5][i_3] * A[i_0+2][i_1-1][i_1+i_0+2][i_3] + A[i_2][i_1-2][i_2+i_1-1][i_3] + 6;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_N-1; i_4++) {
        B[i_4+1] = C[i_4-2] * B[i_4] + B[i_4-1] * 3;
    }
#pragma endscop
...
kernel_4224212325_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
