...
void kernel_3312132111_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q, PB_M-2); i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_1][i_0][i_2+2] = D[i_2][i_0][i_1+1] * D[i_2][i_0][i_1+i_0] - D[i_1-2][i_0][i_2] + D[i_1+2][i_0][i_0] - A[i_1+1][i_0-1][i_2+2] - 6;
            }
        }
        B[i_0] = E[i_0-2] - E[i_0-2] - 3;
    }
    for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                C[i_3-1][i_4-1][i_5+1] = D[i_5+1][i_3-1][i_4] - D[i_4][i_3][i_4] - C[i_3-1][i_4-1][i_5+2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3312132111_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
