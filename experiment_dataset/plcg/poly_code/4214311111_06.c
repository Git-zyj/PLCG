...
void kernel_4214311111_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = max(max(0, -i_1+1), -i_0); i_2 < min(-i_0+PB_L, -i_1+PB_L+1); i_2++) {
                for (int i_3 = 2; i_3 < PB_P; i_3++) {
                    A[i_1-1][i_2+i_0][i_3] = A[i_3-2][i_2+i_1-1][i_0+1] + 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < PB_N; i_5++) {
            B[i_4][i_4][i_5-1] = C[i_5] + B[i_4][i_4][i_5-2] + 1;
        }
    }
#pragma endscop
...
kernel_4214311111_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
