...
void kernel_4216312225_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                A[i_1][i_0] = A[i_1+2][i_0] * A[i_1][i_2+1] - 5;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            A[i_3+1][i_0-2] = C[i_0][0] + C[i_0+1][i_3] + 3;
        }
    }
#pragma endscop
...
kernel_4216312225_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
