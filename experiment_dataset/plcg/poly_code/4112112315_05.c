...
void kernel_4112112315_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-3; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_N-3, -i_1+PB_L); i_2++) {
                A[i_2+2][i_1] = B[i_0][i_2+i_1][i_2-1] * A[i_2+3][i_1+1] + A[i_1+3][i_2] - A[i_2+3][i_1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4112112315_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
