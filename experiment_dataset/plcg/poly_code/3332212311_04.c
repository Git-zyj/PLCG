...
void kernel_3332212311_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_1][i_0][i_0+1] = A[i_1][i_0+1][i_0+2] + 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
                B[i_3][i_4][i_5] = A[i_5-1][i_3-1][i_3] + B[i_3][i_4-1][i_5] * B[i_3+1][i_4+1][i_5] * B[i_3+1][i_5][i_4+1] + B[i_3][i_4+1][i_5+1] + 4;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_L; i_6++) {
        for (int i_7 = 0; i_7 < PB_L; i_7++) {
            for (int i_8 = 0; i_8 < PB_L; i_8++) {
                C[i_8][i_7][i_6] = D[i_7][0] + A[i_8][i_6][i_6+1] * C[i_8+1][i_7][i_6] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3332212311_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
