...
void kernel_3314252211_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0+1][0] * E[i_0+2] + F[i_0] + B[i_0] * 3;
        B[i_0] = D[i_0][0] - F[i_0] * G[i_0][0] - E[i_0+1] + A[i_0] - B[i_0-1] * 4;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            for (int i_3 = 0; i_3 < min(min(PB_M, -i_2+PB_N), -i_1+PB_L); i_3++) {
                C[i_2][i_3+i_1] = H[i_3][i_1][0] + D[i_3][i_2+1] - H[i_2][i_3][i_1] - I[i_2+i_1][i_3][i_2] + H[i_3+i_2][i_1-2][i_1] * D[i_3][i_2+1] + C[i_3][i_2+i_1+1] - C[i_2][i_3+i_1-2] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3314252211_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
