...
void kernel_2414111113_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            A[i_1+i_0][i_0-1] = E[i_0] - 5;
        }
        for (int i_2 = i_0; i_2 < PB_M; i_2++) {
            B[i_0][i_2] = F[i_2][i_0] + B[i_0+1][i_2-1] - 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        ...
        for (int i_5 = 1; i_5 < i_3; i_5++) {
            D[i_3][i_5+1] = F[i_5+1][i_3-1] + D[i_5-1][i_3] - 1;
        }
    }
#pragma endscop
...
kernel_2414111113_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
