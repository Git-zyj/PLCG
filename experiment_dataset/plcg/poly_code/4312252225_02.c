...
void kernel_4312252225_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L-1; i_1++) {
            A[i_1] = D[i_1+1][i_0-1] + D[i_1+i_0][i_1] - E[i_0+1] * D[i_1][i_0-1] * A[i_1+1] + A[i_1+2] * 1;
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                A[i_1-2] = F[i_1][i_0] * G[i_2][i_1+i_0+1][i_2+1] + H[i_1][i_0][i_2][i_1] + D[i_2][i_1] * I[i_1+i_0][i_2][i_2] - G[i_0+2][i_2+i_0][i_1] + 4;
            }
        }
    }
        ...
        for (int i_4 = max(2, i_3); i_4 < -i_3+PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_M; i_5++) {
                for (int i_6 = 2; i_6 < -i_3+PB_M; i_6++) {
                    A[i_6-2] = H[i_5+i_4][i_3+2][i_6-1][i_4+i_3] - G[i_5][i_6-2][i_4-2] * G[i_6][i_5][i_4] * H[i_6+i_3][i_4-1][i_5][i_3] * 2;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4312252225_02(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
