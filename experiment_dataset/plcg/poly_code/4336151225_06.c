...
void kernel_4336151225_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            A[i_0][i_1] = D[0] * E[i_1][i_0-1] - E[i_0][i_0] - E[i_1][i_0] - E[i_0][i_1] + A[i_0+1][i_1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        C[i_2-1][i_2] = D[i_2] * D[i_2] * D[i_2] - D[i_2+1] - D[i_2] + D[i_2-1] + 4;
    }
    for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_L, i_3); i_4++) {
            C[i_4-1][i_4] = E[i_4][i_3] - E[i_4][i_3-1] - E[i_3][i_4+i_3] * E[i_4][i_3] + C[i_3+1][i_3+2] * C[i_4+1][i_4+2] + 1;
        }
    }
#pragma endscop
...
kernel_4336151225_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
