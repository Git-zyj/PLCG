...
void kernel_2232112321_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 4; i_1 < PB_M+1; i_1++) {
            A[i_1-2] = C[i_1-1] + A[i_1-4] * A[i_1-3] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        B[i_2] = C[i_2+1] * B[i_2-1] + B[i_2-2] - B[i_2+2] * 6;
    }
#pragma endscop
...
kernel_2232112321_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
