...
void kernel_2236251321_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        A[i_0] = C[i_0+1] - D[i_0-1] + C[i_0-1] + D[i_0] * D[i_0] - D[i_0] * D[i_0-1] - A[i_0+2] * A[i_0-1] + A[i_0-2] + 3;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            B[i_1] = E[i_2][i_1] - D[i_2] * F[i_1][0] + A[i_1] + B[i_2-2] + 1;
        }
    }
#pragma endscop
...
kernel_2236251321_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
