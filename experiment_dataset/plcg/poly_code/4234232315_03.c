...
void kernel_4234232315_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = C[i_0-2] + D[i_0] - D[i_0] - D[i_0-1] + A[i_0+1] + 2;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
            for (int i_3 = 2; i_3 < i_2; i_3++) {
                B[i_2+2][i_3-2][i_1] = E[i_2][i_3+1][i_1] * B[i_2+1][i_3-2][i_1+1] * B[i_3+2][i_2-1][i_1] + B[i_3+1][i_2-2][i_1-1] + B[i_2+2][i_3-2][i_1-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4234232315_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
