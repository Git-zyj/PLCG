...
void kernel_2234231313_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            A[i_1-1] = C[i_0][i_1+1] + A[i_1-2] - A[i_1] * A[i_1] + 2;
        }
    }
    for (int i_2 = PB_L - 1; i_2 >= 0; i_2--) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            A[i_3-1] = D[i_2][i_3] + E[0] * C[i_3-1][i_2] - C[i_2+1][0] - E[i_3+1] * 6;
        }
    }
#pragma endscop
...
kernel_2234231313_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
