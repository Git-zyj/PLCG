...
void kernel_2334312321_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] + D[i_0+2] + A[i_0+2] * 1;
    }
    for (int i_1 = 4; i_1 < PB_M; i_1++) {
        B[i_1-2] = B[i_1-1] * B[i_1-3] * B[i_1] - B[i_1-4] * 4;
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            C[i_2][i_3] = C[i_3][i_2-2] - C[i_2][i_3+1] * C[i_2][i_3+2] * C[i_2][i_3-2] + 3;
        }
    }
#pragma endscop
...
kernel_2334312321_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
