...
void kernel_3324311325_05(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_N; i_2++) {
                C[i_2][i_1] = D[i_2][i_0][i_1] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            C[i_4][i_3+2] = E[i_3-1][i_4-1] * F[i_3+1][i_4] * 3;
        }
    }
    for (int i_5 = 1; i_5 < PB_N-2; i_5++) {
        for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
            C[i_5][i_6] = C[i_5+2][i_6+1] + C[i_5-1][i_6] + C[i_5+2][i_6] - 1;
        }
    }
#pragma endscop
...
kernel_3324311325_05(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
