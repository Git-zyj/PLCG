...
void kernel_4432351215_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_L-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = i_0; i_3 < min(min(-i_0+PB_M, PB_L), -i_2+PB_M); i_3++) {
                    A[i_3][i_2][i_1][i_0] = E[i_3][i_2][i_0] * F[i_1][i_0-1][i_3][i_2] * G[i_3+i_2][i_3+i_0][i_1+1][i_1] + H[i_1+i_0][i_3][i_2][i_1] + A[i_3][i_2+1][i_1-1][i_0] + 5;
                }
            }
        }
        B[i_0] = I[i_0+1] + J[i_0] - J[i_0] - I[i_0] + K[i_0] + I[i_0-1] * B[i_0+1] - 1;
        for (int i_4 = 1; i_4 < PB_Q; i_4++) {
            for (int i_5 = min(-i_4+PB_P-1, PB_L-1) - 1; i_5 >= 0; i_5--) {
                for (int i_6 = 0; i_6 < PB_Q; i_6++) {
                    A[i_6][i_4-1][i_5+1][i_0-1] = F[i_5][i_6][i_4][i_5] * G[i_6][i_5+1][i_0][i_5+i_4+1] * F[i_4][i_5][i_6][i_4-1] - 2;
                }
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
        for (int i_8 = 1; i_8 < -i_7+PB_L-1; i_8++) {
            for (int i_9 = 1; i_9 < PB_L; i_9++) {
                D[i_7][i_8][i_9] = L[i_9+i_8][1] * M[i_9-1][i_8+i_7+1][0] + N[i_7+1][i_8][0] - M[i_8][i_9-1][i_7] + D[i_7][i_8][i_9-1] + D[i_7][i_8-1][i_9] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4432351215_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N));
