...
void kernel_2424252125_06(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_M; i_1++) {
            C[i_0] = E[i_1+i_0] - E[i_1+1] * F[i_0][i_1][i_0] + G[i_1+2][i_0] + H[i_1-2][i_0+1] * H[i_1][i_0] + H[i_1][i_0] + 5;
        }
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            C[i_2] = I[i_2+2] - H[i_2][i_0] * H[i_2][0] * C[i_2] - 4;
        }
        C[i_0] = E[i_0-1] + 4;
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        C[i_3-1] = E[i_3] + I[i_3+2] + H[i_3][0] + I[i_3] - E[i_3-1] + E[i_3] * 3;
    }
#pragma endscop
...
kernel_2424252125_06(xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
