...
void kernel_3324151213_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0+1] = D[i_0-1] * D[i_0+1] * D[i_0-1] * D[i_0+1] - D[i_0] + D[i_0] + A[i_0] + A[i_0+2] - 5;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        C[i_1-1] = D[i_1] * D[i_1+1] + D[i_1] - D[i_1-1] + D[i_1-1] - D[i_1+1] - C[i_1] * 1;
        C[i_1] = D[i_1] * D[i_1] * D[i_1] - A[i_1] * 6;
    }
#pragma endscop
...
kernel_3324151213_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
