...
void kernel_4226111321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            A[i_0][i_1] = C[i_0+1][i_1+1] + A[i_0][i_1-1] + A[i_0+2][i_1] * A[i_0-1][i_1+2] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q-4; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            for (int i_4 = i_2; i_4 < -i_2+PB_Q-4; i_4++) {
                B[i_4][i_2][i_4+i_2] = C[i_3][i_3-1] - B[i_4][i_2+2][i_4+i_2+2] * B[i_2+2][i_4+2][i_4+i_2+4] * B[i_4+1][i_2-2][i_4+i_2-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4226111321_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
