...
void kernel_2336112213_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N-3; i_1++) {
            B[i_1+3] = D[i_0][i_1+1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-3; i_2++) {
        B[i_2+2] = B[i_2+3] + B[i_2+1] * 6;
    }
...
kernel_2336112213_01(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
