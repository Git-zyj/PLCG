...
void kernel_4216311311_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_N-2, i_0); i_1++) {
            for (int i_2 = 0; i_2 < min(PB_N-2, i_0); i_2++) {
                A[i_1+1][i_1-1] = A[i_1+2][i_1] - A[i_1][i_1-2] + A[i_2+2][i_2] * 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
            A[i_4+1][i_4-1] = C[i_4+1][i_3] - 4;
        }
    }
#pragma endscop
...
kernel_4216311311_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
