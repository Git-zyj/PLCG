...
void kernel_3322112113_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_0][i_1] = D[i_1][i_0-2] + A[i_0-1][i_1-1] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_Q; i_3++) {
            for (int i_4 = 0; i_4 < PB_Q-1; i_4++) {
                C[i_2-2][i_4+1][i_4+1] = 5;
            }
        }
        for (int i_5 = 1; i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 0; i_6 < -i_5+PB_Q+1; i_6++) {
                C[i_2-2][i_5+1][i_5+1] = E[i_2][i_6][i_6+i_5-1] + C[i_2-2][i_5][i_5] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3322112113_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
