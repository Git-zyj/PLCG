...
void kernel_3226212111_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            ...
            for (int i_2 = 3; i_2 < PB_L+2; i_2++) {
                B[i_0][i_1][i_2-2] = B[i_0][i_1][i_2-3] * B[i_0][i_1+1][i_2-2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3226212111_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
