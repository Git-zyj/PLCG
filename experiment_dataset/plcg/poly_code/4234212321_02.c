...
void kernel_4234212321_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M-1, i_1); i_2++) {
                A[i_2+i_0][i_1+2][i_0-1] = C[i_1+2][i_2][i_0] - A[i_2+i_0+1][i_1][i_0] * 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_M-2, i_3); i_4++) {
            for (int i_5 = max(2, i_3); i_5 < min(-i_3+PB_M, PB_L-2); i_5++) {
                B[i_4][i_5] = A[i_5+i_3][i_4+2][i_3-1] + B[i_4][i_5+2] + B[i_5][i_3+1] - B[i_5+2][i_4+2] - B[i_4][i_5-2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4234212321_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
