...
void kernel_4224232215_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q, PB_P+2); i_1++) {
            for (int i_2 = max(3, i_0); i_2 < -i_1+PB_P+2; i_2++) {
                A[i_0-2][i_2][i_1-2] = C[i_2+i_1+2][i_0][i_0] - D[i_2][i_0][i_1+i_0] - D[i_2-2][i_2+i_1-2][i_1+i_0] - C[i_0][i_2][i_0] - D[i_0][i_2-1][i_1] + A[i_1-2][i_2-1][i_0-1] * A[i_2-3][i_0][i_1-2] - 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L+2; i_4++) {
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                A[i_3-2][i_5][i_4-2] = D[i_5][i_3][i_4] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4224232215_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
