...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q, PB_P-1); i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_Q-1; i_3++) {
                    A[i_3][i_2][i_1+i_0][0] = E[i_1][i_3][i_0-1][i_0] * F[i_2][i_3][i_1+1][i_0] - E[i_3][i_0][i_1+1][i_2] + F[i_1+1][i_0][i_0][i_3] - G[i_3][i_2][i_1+i_0-1][i_0] * H[i_1+1][i_0][i_3][i_2][i_1] + A[i_2][i_3-1][i_1+i_0-1][0] * A[i_2][i_0][i_3+i_1+1][0] - A[i_3][i_2][i_1+i_0-1][0] - 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_Q; i_4++) {
        for (int i_5 = 0; i_5 < min(-i_4+PB_M+1, PB_M-1); i_5++) {
            C[i_5+1] = I[i_5+i_4-1][i_4-1][0] * 5;
        }
    }
    for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
        C[i_6] = J[i_6] + J[i_6] - K[i_6] * L[i_6][i_6+1] * K[i_6-1] + J[i_6] - 6;
    }
    for (int i_7 = 1; i_7 < PB_P; i_7++) {
        for (int i_8 = 0; i_8 < -i_7+PB_P+1; i_8++) {
            for (int i_9 = 0; i_9 < min(-i_7+PB_Q+1, PB_P); i_9++) {
                for (int j_0 = 1; j_0 < min(min(-i_8+PB_L, PB_P), -i_7+PB_L+1); j_0++) {
                    D[i_7-1][i_8][j_0-1][i_9+i_7-1] = G[i_9][i_7+1][j_0+i_8][j_0] - F[i_9][j_0][i_8+i_7-1][1] * G[i_9][j_0+i_7-1][i_8][j_0] - F[i_7][j_0+1][i_8][i_7] + F[i_8+1][i_9+1][j_0][i_7-1] - 5;
                }
            }
        }
    }
...
kernel_4436351113_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
