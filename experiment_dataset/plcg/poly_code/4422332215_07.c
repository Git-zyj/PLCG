...
void kernel_4422332215_07(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
        ...
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_L-1); i_2++) {
                for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
                    D[i_2+1][i_1][i_3][i_3+2] = E[i_2+1][i_2+i_0][i_0][i_1-2] - 1;
                }
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 2; i_6 < i_5; i_6++) {
                for (int i_7 = 0; i_7 < PB_L; i_7++) {
                    D[i_7][i_6+1][i_4][i_4+2] = F[i_7][i_4][i_5+2][i_6-2] + F[i_6-2][i_4][i_4][i_5] - 1;
                }
            }
        }
    }
    for (int i_8 = 2; i_8 < PB_L-1; i_8++) {
        for (int i_9 = 1; i_9 < PB_L; i_9++) {
            for (int j_0 = 1; j_0 < min(i_9, PB_L-1); j_0++) {
                D[i_8+1][i_9][j_0-1][j_0+1] = G[j_0+1][i_8][i_9-1] * 6;
            }
        }
        for (int j_1 = 0; j_1 < -i_8+PB_L; j_1++) {
            for (int j_2 = 0; j_2 < -i_8+PB_Q; j_2++) {
                for (int j_3 = 1; j_3 < PB_Q-3; j_3++) {
                    D[j_1][j_2][j_3][j_3+2] = E[j_1][j_3][i_8][j_2] + H[j_3][j_2][j_1][j_1+i_8] + E[j_2+j_1][i_8-1][j_1][j_2] * H[i_8-2][j_3][j_1][j_2+2] - H[j_1+i_8][j_3][j_2+i_8][0] * D[j_1][i_8][j_3+1][j_3+3] - D[j_1][j_2][j_3-1][j_3+1] * 4;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422332215_07(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
