...
void kernel_3324132125_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = 1; i_3 < -i_2+PB_N-1; i_3++) {
                B[i_2][i_1] = E[i_3+i_2+1][i_1] - B[i_3-1][i_2] + 4;
            }
        }
        for (int i_4 = 1; i_4 < min(-i_1+PB_N, i_1); i_4++) {
            B[i_4][i_1] = D[i_4] + E[i_4][i_1] + E[i_4-1][i_1-2] - E[i_1][i_4] * E[i_4+i_1][i_1] + 2;
        }
    }
#pragma endscop
...
kernel_3324132125_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
