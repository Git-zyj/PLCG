...
void kernel_4436332311_05(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = i_1; i_2 < -i_0+PB_L; i_2++) {
                D[i_0-2][i_2+1][i_1-2][i_1-1] = E[i_2][i_0-2][i_1-1][0] - F[i_1-2][i_2+i_0][i_2] + 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-3; i_4++) {
            for (int i_5 = 2; i_5 < i_4; i_5++) {
                B[i_3][i_5-2][i_4+2] = G[i_5][i_4] * H[i_3][i_4] + F[i_3-2][i_5][i_4-1] * I[i_3+1][i_4-1][i_5] - B[i_4+1][i_5-2][i_3+1] * B[i_3][i_5-2][i_4+3] + B[i_3][i_5-2][i_4+1] - 3;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
        for (int i_7 = i_6; i_7 < -i_6+PB_L-2; i_7++) {
            C[i_7] = H[i_7+i_6][i_7+i_6+2] - 4;
        }
        for (int i_8 = 2; i_8 < PB_P-2; i_8++) {
            for (int i_9 = 2; i_9 < i_6; i_9++) {
                for (int j_0 = 1; j_0 < -i_6+PB_M-2; j_0++) {
                    D[i_6-2][i_8+2][i_9-1][i_9] = J[j_0+1][i_6+2][i_6][i_9] - F[j_0][i_6][i_9-2] * K[j_0+2][i_9][i_6][i_8-1] + I[j_0][i_8][i_9] * K[j_0+i_6+2][i_9+1][j_0][i_8-2] - D[i_6-2][i_8+2][i_9][i_9+1] - D[i_6-2][i_8+1][j_0-1][j_0] + D[i_8-2][i_6+2][i_9-2][i_9-1] - D[i_6-2][j_0+1][i_9-1][i_9] - 4;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4436332311_05(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K));
