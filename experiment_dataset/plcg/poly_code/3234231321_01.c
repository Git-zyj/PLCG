...
void kernel_3234231321_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P, -i_0+PB_M-2); i_1++) {
            for (int i_2 = i_0; i_2 < min(min(PB_L-3, -i_1+PB_M), -i_0+PB_M-3); i_2++) {
                A[i_1+i_0][i_2+i_0+1][i_2+1] = C[i_1][i_2][i_0] * D[i_1][i_2-1][i_0] + C[i_2+i_1][i_0+1][i_1] - A[i_1+i_0+1][i_2+i_1][i_2] * A[i_1+i_0+2][i_2+i_0+3][i_2+1] * A[i_1+i_0][i_2+i_0+3][i_2+3] * A[i_1+i_0+2][i_2+i_0-1][i_2-1] + 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_P; i_4++) {
            for (int i_5 = 0; i_5 < PB_P; i_5++) {
                B[i_4+1][i_3][i_5] = D[i_3][i_4-1][i_5] * C[i_5][i_3][i_4] * B[i_4][i_3][i_5] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3234231321_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
