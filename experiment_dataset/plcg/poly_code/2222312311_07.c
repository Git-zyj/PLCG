...
void kernel_2222312311_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1+2][i_0][i_1] = A[i_0+3][i_1-1][i_0+1] - A[i_0+1][i_1-1][i_0-1] + A[i_1+2][i_0+1][i_1] - 5;
        }
    }
    for (int i_2 = 3; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
            B[i_2-2][i_3] = C[i_3][i_2] * B[i_2-3][i_3] - B[i_2-2][i_3+1] * 6;
        }
    }
#pragma endscop
...
kernel_2222312311_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
