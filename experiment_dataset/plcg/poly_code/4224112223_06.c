...
void kernel_4224112223_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_0][i_1-1] = C[i_1-1][i_0] + A[i_0-1][i_1-1] * A[i_0+2][i_1+1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_2+1] = B[i_3-1] + B[i_3] - 3;
        }
    }
#pragma endscop
...
kernel_4224112223_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
