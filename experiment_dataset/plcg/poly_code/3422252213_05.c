...
void kernel_3422252213_05(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_N; i_1++) {
            C[i_0][i_1] = E[i_1+i_0] * F[i_0-2][i_0+2] - G[i_1+1][i_0+2] * 2;
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                B[i_2][i_0][i_0] = H[i_0-2][i_2][i_1] * F[i_0][i_1+2] - H[i_0][i_2+i_0][i_1] * B[i_2][i_0+1][i_0+1] - B[i_1-1][i_0][i_0] - B[i_2+1][i_0][i_0] + 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
            C[i_3][i_4] = I[i_4+i_3] + F[i_4][i_3] - H[i_3][i_4][i_3] + F[i_4-1][i_3] * F[i_4+i_3-1][1] * G[i_4][i_3+2] - F[i_4][1] + C[i_3+1][i_4] - 5;
            C[i_3+1][i_4] = F[i_4-1][i_3-1] * I[i_4+i_3-2] * F[i_3][i_4] - J[i_4][0][i_3-1] - G[i_4+i_3][i_4+i_3-1] * G[i_4+i_3-2][i_4+i_3] + 4;
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3422252213_05(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
