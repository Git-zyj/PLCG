...
void kernel_4236111321_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < min(i_0, PB_M-1); i_1++) {
            A[i_1-1] = A[i_1+1] + A[i_1-2] - A[i_1-3] * A[i_1] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        B[i_2+1] = C[i_2+1] - C[i_2+1] - B[i_2] + 6;
    }
#pragma endscop
...
kernel_4236111321_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
