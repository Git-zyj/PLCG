...
void kernel_3214111315_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = PB_L-1 - 1; i_1 >= i_0; i_1--) {
            B[i_0+1][i_1+1] = C[i_1-1][i_0-1][i_1] - C[i_1][i_0-1][i_0] * B[i_1+1][i_0+1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < min(i_2, PB_L-2); i_4++) {
                B[i_3+1][i_4+1] = B[i_4][i_2+1] * B[i_3][i_4+2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3214111315_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
