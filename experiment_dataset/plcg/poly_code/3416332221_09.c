...
void kernel_3416332221_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0] = E[i_0] - F[i_0+2] * G[i_0] - H[i_0][i_0] + A[i_0+1] - A[i_0+2] + 5;
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M-2; i_2++) {
                B[i_2][i_1+1][i_1] = I[i_1][i_2+2][i_0] * J[i_0][i_1-1][i_1] * I[i_1][i_0+1][0] + K[i_2][i_0][i_1+2] + B[i_2+2][i_1+1][i_1] - B[i_2+1][i_1+1][i_1] * 5;
            }
        }
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 2; i_4 < PB_L; i_4++) {
                C[i_4][i_3] = J[i_3][i_0][i_4] + L[i_4][i_0] - K[i_0][i_4][i_3] - C[i_4][i_3+2] + C[i_3+1][i_4] + 3;
                D[i_3][0] = I[i_3+1][i_4-2][i_0-1] * D[i_3+2][0] - D[i_4-1][0] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3416332221_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
