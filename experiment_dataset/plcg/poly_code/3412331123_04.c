...
void kernel_3412331123_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[i_0] + E[i_0] - F[i_0] + E[i_0-1] - A[i_0+2] - 2;
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            B[i_1+i_0+1][i_0] = G[i_1][i_1] + E[i_0] + H[i_1][i_0] - I[i_0][i_1] + 4;
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                C[i_1+1][i_2][i_1+i_0] = C[i_1][i_2][i_1+i_0-1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        D[i_3] = E[i_3] - J[i_3+1] * 4;
    }
#pragma endscop
...
kernel_3412331123_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
