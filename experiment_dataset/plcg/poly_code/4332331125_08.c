...
void kernel_4332331125_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 1; i_3 < min(min(i_0, -i_0+PB_M+1), -i_2+PB_M-1); i_3++) {
                    A[i_0+1][i_2][i_1+1][i_3+1] = D[i_3+i_2+1][i_1+1][i_2+1] + E[i_1][i_3+i_2][i_0+1][i_0] * F[i_2][i_2][i_1][i_0] - G[i_3-1][i_0][i_0+1][i_2] * E[i_3+i_0-1][i_2-1][i_1][1] + A[i_0+1][i_3+2][i_1][i_2+1] * 5;
                }
            }
        }
    }
        ...
        for (int i_6 = 1; i_6 < PB_M; i_6++) {
            for (int i_7 = 0; i_7 < -i_6+PB_M; i_7++) {
                for (int i_8 = 1; i_8 < PB_P; i_8++) {
                    A[i_7+1][i_8][i_6][i_5+1] = F[i_8-1][i_7][i_5+1][i_6-1] * D[i_5][i_7+i_6][i_8] + E[i_7+1][i_5+1][i_8][i_6] * 1;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4332331125_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
