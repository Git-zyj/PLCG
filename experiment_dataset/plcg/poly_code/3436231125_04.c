...
void kernel_3436231125_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L-1 - 1; i_0 >= 0; i_0--) {
        A[i_0+1] = E[i_0] * E[i_0+1] * E[i_0] * A[i_0+3] + 6;
    }
    for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
        for (int i_2 = 0; i_2 < min(i_1, -i_1+PB_L+1); i_2++) {
            A[i_2] = F[i_2][i_1-1] + F[i_2+i_1-1][i_1-1] * 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            A[i_4+1] = F[i_4-1][i_3] + F[i_4+1][i_3-1] * G[i_4][i_3] + F[i_4][0] + E[i_3] + 6;
        }
    }
...
kernel_3436231125_04(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
