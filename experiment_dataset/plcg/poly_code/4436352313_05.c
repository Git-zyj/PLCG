...
void kernel_4436352313_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < min(min(min(-i_0+PB_M, -i_1+PB_L-1), i_1), PB_M-2); i_2++) {
                for (int i_3 = i_2; i_3 < -i_2+PB_N-2; i_3++) {
                    B[i_2+i_0][i_0+1][i_3] = E[i_1-2][i_0][i_3+1][0] + F[i_3][i_1][i_2-2] - G[i_3-1][i_0][i_2+i_1+1][i_3] - G[i_2+2][i_0][i_3][1] * H[i_3+i_2+2][i_1-2][i_3] - 6;
                }
            }
        }
        for (int i_4 = 1; i_4 < min(-i_0+PB_M, i_0); i_4++) {
            for (int i_5 = 2; i_5 < i_0; i_5++) {
                B[i_4+i_0][i_0][i_5] = F[i_4+i_0][i_5][i_5] * I[i_5][i_4-1] * J[i_5+i_4][i_0][i_0+2] + F[i_4-1][i_5][i_0+2] * J[i_0][i_5-2][i_4] - K[i_4+i_0-2][i_5+2] * B[i_4+i_0][i_0][i_5-1] + 1;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_M-1; i_6++) {
        C[i_6] = L[i_6] * M[i_6] - N[i_6] - N[2] * N[i_6] * C[i_6-1] + C[i_6+1] + C[i_6-1] - 2;
        for (int i_7 = i_6; i_7 < PB_M-1; i_7++) {
            for (int i_8 = 0; i_8 < min(-i_6+PB_M-1, -i_7+PB_M-1); i_8++) {
                B[i_8+i_7+1][i_7][i_6] = H[i_6-2][i_8][i_7] - J[i_8+i_7][i_6+1][i_6] - B[i_8+i_6+1][i_6][i_7] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4436352313_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N));
