...
void kernel_3334112321_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 4; i_1 < PB_N; i_1++) {
            A[i_1-2] = A[i_1-3] + A[i_1] - A[i_1-4] * A[i_1-1] + 3;
        }
    }
    for (int i_2 = 4; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L-1; i_3++) {
            B[i_2-2][i_3] = D[i_2+2][0] * B[i_2-4][i_3] + B[i_2-2][i_3+1] - 4;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-5; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_M-5); i_5++) {
            C[i_5][i_5+i_4+2] = C[i_5+2][i_5+i_4+5] - C[i_5-1][i_5+i_4] * 1;
        }
    }
#pragma endscop
...
kernel_3334112321_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
