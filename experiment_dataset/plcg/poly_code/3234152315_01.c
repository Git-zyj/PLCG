...
void kernel_3234152315_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L; i_1++) {
            B[i_0][i_1-2] = C[i_1+2][i_0] + C[i_0][i_1] + C[i_0][i_1] - C[i_0-1][i_1] + C[i_0][i_1] * D[i_0] + D[i_1+i_0] + B[i_0][i_1-2] + 5;
        }
        for (int i_2 = max(2, i_0); i_2 < -i_0+PB_L; i_2++) {
            B[i_0][i_2-2] = C[i_2][i_2+i_0] + C[i_0][i_2] - C[i_2-1][i_2] * B[i_0+1][i_2-2] * B[i_0-1][i_2-2] + 1;
        }
    }
#pragma endscop
...
kernel_3234152315_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
