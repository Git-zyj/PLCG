...
void kernel_3212312121_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                A[i_1+1][i_0+2] = C[i_2+1][i_0] + A[i_2-1][i_1+2] * 1;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_M; i_3++) {
            B[i_3+i_0] = B[i_3+i_0-4] + 2;
        }
    }
#pragma endscop
...
kernel_3212312121_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
