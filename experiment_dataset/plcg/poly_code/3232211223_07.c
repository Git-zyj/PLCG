...
void kernel_3232211223_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q+1; i_2++) {
                B[i_2+3][i_1-2] = C[i_0+1][i_1][i_2-1] - D[i_1][i_0-1][i_1] + 6;
            }
        }
        for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
            B[i_3+1][i_0] = B[i_3][i_0] - B[i_3+3][i_0] - 3;
        }
    }
#pragma endscop
...
kernel_3232211223_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
