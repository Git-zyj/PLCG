...
void kernel_3432232213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_Q; i_1++) {
            A[i_0+2][i_0-1] = E[i_0][i_1][i_1+i_0] * F[i_0][i_1] * A[i_0+3][i_0] + A[i_0+1][i_0-2] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            A[i_2+1][i_2-2] = F[i_2+1][i_3] * F[i_3][i_2] + F[i_3][i_2+1] * F[i_2][i_3] * 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 0; i_6 < -i_5+PB_Q-1; i_6++) {
                C[i_4][i_6][i_6+i_5] = G[i_6][i_4][i_5][i_4] * E[i_5][i_6][i_4] + F[i_6][i_5-2] - F[i_6+2][i_5] + C[i_4][i_6+1][i_6+i_5+1] * C[i_4][i_5-1][i_6+i_5] + 2;
            }
        }
        for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
            D[i_7][i_4+1] = F[i_7+1][i_4] + D[i_7-1][i_4+1] - D[i_7][i_4+2] - 1;
        }
    }
#pragma endscop
...
kernel_3432232213_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
