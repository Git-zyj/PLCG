...
void kernel_2322332323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0+1] - E[i_0] - A[i_0-1] + A[i_0-2] + A[i_0+2] + 1;
    }
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_P-2, PB_M-1); i_2++) {
            B[i_2][i_1] = F[i_2+1][i_1+2] - G[i_2] * H[i_2][i_1] - H[i_1][i_2+i_1+2] + B[i_2][i_1-1] + B[i_2+1][i_1] + B[i_2-1][i_1+2] * 6;
        }
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            B[i_3][i_1] = F[i_3-1][i_1] - 5;
        }
    }
#pragma endscop
...
kernel_2322332323_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
