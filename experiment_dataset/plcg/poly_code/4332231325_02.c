...
void kernel_4332231325_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0-1] - E[i_0-1] * D[i_0] * A[i_0+1] - A[i_0+2] + A[i_0-2] - 1;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
            for (int i_3 = 1; i_3 < PB_M; i_3++) {
                for (int i_4 = i_3; i_4 < PB_M-2; i_4++) {
                    B[i_1][i_3][i_4] = F[i_4][i_2][i_3] - G[i_4+1][i_3][i_2+1] - B[i_4+2][i_3-1][i_1] * B[i_3][i_4+1][i_1] * 6;
                }
            }
        }
    }
        ...
        for (int i_6 = 0; i_6 < min(PB_Q-2, i_5); i_6++) {
            for (int i_7 = 1; i_7 < PB_M+1; i_7++) {
                for (int i_8 = 1; i_8 < PB_Q-1; i_8++) {
                    B[i_5-1][i_8][i_6+2] = H[i_5][i_6][i_8-1][i_7] - I[i_7-1][i_5][i_6][i_8+1] + 2;
                }
            }
        }
    }
...
kernel_4332231325_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
