...
void kernel_4336352113_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0] * E[i_0-2] - F[0] + E[i_0] + A[i_0+1] * 6;
    }
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        for (int i_2 = i_1; i_2 < PB_N; i_2++) {
            for (int i_3 = 0; i_3 < PB_Q; i_3++) {
                B[i_2][i_3][i_1] = G[i_3+2][i_2-2] * H[1][i_1-2][i_2] * H[i_3+1][i_2][i_1-2] - I[i_2+2][i_1][i_3] - 6;
            }
        }
        A[i_1] = D[0] + E[i_1] + E[i_1] * E[i_1] * D[i_1] * E[i_1] - 5;
    }
#pragma endscop
...
kernel_4336352113_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
