...
void kernel_2326112311_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 3; i_4 < PB_M; i_4++) {
            C[i_4-2][i_3] = D[i_4] * C[i_4-1][i_3] * C[i_4-3][i_3-1] * C[i_4-2][i_3-1] * 2;
        }
    }
#pragma endscop
...
kernel_2326112311_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
