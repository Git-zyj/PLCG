...
void kernel_4316252115_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = D[i_0+2] * E[i_0] - D[i_0] * D[i_0-1] - E[i_0] + E[i_0] - D[i_0+2] * E[i_0+1] + 4;
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_L-2; i_2++) {
                for (int i_3 = 2; i_3 < PB_M; i_3++) {
                    B[i_2][i_3-2][i_1][i_0] = F[i_3][i_1+2][i_2][i_0] + F[i_3][i_2+i_1+2][i_0-2][0] * G[i_1+2][i_2+1][i_3][i_0+1] - F[i_2+1][i_3][i_1][i_0] - G[i_2][i_1][i_3+2][i_0+1] * B[i_2][i_1-2][i_0][i_3+1] * B[i_1-1][i_3-1][i_2-1][i_0-1] + 6;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            A[i_5-1] = H[i_4][i_5] * I[i_5+1][i_4] + 4;
        }
    }
#pragma endscop
...
kernel_4316252115_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
