...
void kernel_3326331325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] - E[i_0-1] * F[i_0] * A[i_0+1] - A[i_0+2] + 4;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        A[i_1] = F[i_1+1] * D[i_1-1] * A[i_1] + 2;
    }
    for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_Q; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_L, i_3); i_4++) {
                C[i_4-1][i_2-1][i_3] = G[i_4+1][i_4] + H[i_4][i_4][i_3] - H[i_2-1][i_4][i_3] * C[i_4-1][i_2-1][i_3-2] + C[i_4][i_2-2][i_3] - C[i_4-1][i_2+1][i_3] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3326331325_03(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
