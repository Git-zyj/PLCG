...
void kernel_4326251111_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_P-1; i_2++) {
                A[i_2-1][i_0][i_1] = D[i_0][i_2+i_1+1][i_2][i_1] - E[i_1][i_0][i_1-1] + A[i_2][i_0+1][i_1] + 1;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_P; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q-1; i_5++) {
                for (int i_6 = 1; i_6 < PB_P-1; i_6++) {
                    A[i_6-1][i_4][i_5] = D[i_3+1][i_6+1][i_5][0] * D[i_3][i_5][i_6-1][i_4] * D[i_4-1][i_3][i_6+1][i_5+1] - F[i_4-1][i_5][i_3][i_6][i_3] + G[i_4-1][i_5][i_3+1][i_6] + H[i_5][i_4][i_6-1] + 1;
                }
            }
        }
        for (int i_7 = i_3; i_7 < PB_M-1; i_7++) {
            C[i_3][i_7] = I[i_7+1] + J[i_7+1] + I[i_3] * K[i_7][i_3] * L[i_7][i_3+1] - 3;
        }
    }
#pragma endscop
