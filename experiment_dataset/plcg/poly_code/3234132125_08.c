...
void kernel_3234132125_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
            A[i_2-2] = C[i_1][i_2-2] * C[i_1+2][i_2+1] + C[i_2+2][i_1] - D[i_1+1] - C[i_1-2][i_2] * 2;
        }
    }
#pragma endscop
...
kernel_3234132125_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
