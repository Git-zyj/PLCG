...
void kernel_3324112311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        ...
        A[i_0] = D[i_0+2] - D[i_0] - A[i_0] + 5;
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                C[i_3][i_4] = C[i_3+1][i_4+1] * C[i_4][i_3-1] * C[i_4][i_3+1] - C[i_4+1][i_3] + C[i_3+1][i_4] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3324112311_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
