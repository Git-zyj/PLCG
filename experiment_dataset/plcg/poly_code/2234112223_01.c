...
void kernel_2234112223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M, -i_0+PB_P-1); i_1++) {
            A[i_1-2][i_1+i_0] = C[i_1][i_0] - A[i_1-1][i_1+i_0+1] + 5;
        }
        for (int i_2 = 0; i_2 < PB_L-4; i_2++) {
            B[i_0-1][i_2+2] = B[i_0-3][i_2+2] + B[i_0-3][i_2+4] + B[i_0+1][i_2+2] + 3;
        }
    }
#pragma endscop
...
kernel_2234112223_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
