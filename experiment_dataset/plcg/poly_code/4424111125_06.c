...
void kernel_4424111125_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg)){
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        ...
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, PB_M-1); i_4++) {
                A[i_4+1] = F[0][i_4+i_3] - 3;
            }
        }
        for (int i_5 = 2; i_5 < min(i_1, PB_M-1); i_5++) {
            for (int i_6 = max(0, -i_5+2); i_6 < min(-i_5+PB_P, PB_M-1); i_6++) {
                for (int i_7 = 0; i_7 < PB_P-1; i_7++) {
                    D[i_5+1][i_1][i_7][i_6+i_5] = G[i_6][i_1][i_7+1][i_5][i_5] - D[i_6+1][i_1][i_7][i_6+i_5-2] * 4;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4424111125_06(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G)));
