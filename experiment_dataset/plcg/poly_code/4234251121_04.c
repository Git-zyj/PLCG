...
void kernel_4234251121_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = i_1; i_2 < min(min(PB_L, -i_1+PB_P), -i_0+PB_P); i_2++) {
                for (int i_3 = 1; i_3 < min(-i_1+PB_L-1, -i_2+PB_M-1); i_3++) {
                    A[i_0][i_1+1][i_2-1][i_3] = C[i_2+1][i_0+1][i_2][i_3+i_1+1] + D[i_3+i_2+1][i_0][i_1][i_2+i_1] * C[i_1][i_3-1][i_2+1][i_1-1] - E[i_3+1][i_1][i_2+i_0] - C[i_1][i_3][i_2][i_0] * C[i_2+1][i_0][i_3+i_1][i_2] + 2;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_N; i_5++) {
            B[i_5+1] = F[i_5-1][i_5+i_4] * G[i_4][i_5] - H[i_4] - B[i_5-1] + B[i_5+2] + 4;
        }
    }
#pragma endscop
...
kernel_4234251121_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
