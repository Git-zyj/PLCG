...
void kernel_3222132123_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
    ...
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_N, -i_0+PB_M-2); i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_M, PB_P), -i_1+PB_M+1); i_2++) {
                A[i_2+i_0][i_0][i_1+2] = C[i_2][i_1+i_0+2][i_0+1] - C[i_2][i_1][i_0] + D[i_2][0] + D[i_2+1][i_1] - C[i_2+i_1-1][i_0][i_1-2] - A[i_2+i_0-1][i_0][i_1+2] + 4;
            }
            B[i_1][i_0-2] = E[i_0] * B[i_1][i_0-1] + 2;
        }
    }
#pragma endscop
...
kernel_3222132123_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
