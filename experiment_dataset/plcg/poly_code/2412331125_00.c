...
void kernel_2412331125_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = E[0] - F[i_0+1] + E[i_0] + G[i_0][i_0-1] + H[i_0] + 1;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        B[i_1+1] = E[i_1] - H[i_1] * B[i_1] - 3;
        for (int i_2 = i_1; i_2 < min(-i_1+PB_M, PB_P-1); i_2++) {
            B[i_1+1] = G[i_2+1][i_1] * F[i_2+i_1] * I[i_1][i_2+1] - 5;
        }
        D[i_1] = E[i_1] * E[i_1] + 5;
    }
#pragma endscop
...
kernel_2412331125_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
