...
void kernel_2314312215_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < PB_M+1; i_1++) {
            A[i_1-1][i_0-2] = A[i_0][i_1-3] - A[i_0][i_1-2] + A[i_1-2][i_0-2] * 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
            B[i_2][i_3+2] = C[i_3+2] + 6;
            C[i_3+2] = D[i_3][i_2-2] * C[i_3+1] + C[i_3+3] - 6;
        }
    }
#pragma endscop
...
kernel_2314312215_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
