...
void kernel_3334111313_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_0][i_1-1] = D[i_1+1] + A[i_0-1][i_1-1] * A[i_0+1][i_1-1] - A[i_0-1][i_1] + 1;
            for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
                A[i_0-1][i_1-2] = A[i_1][i_2-1] * 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            C[i_4][i_3] = E[i_4][i_3] - C[i_4-1][i_3] - C[i_4+1][i_3-1] + 1;
        }
    }
#pragma endscop
...
kernel_3334111313_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
