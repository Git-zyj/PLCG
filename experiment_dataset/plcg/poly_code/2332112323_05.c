...
void kernel_2332112323_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_0+1][i_1] = A[i_0][i_1+1] * A[i_0+1][i_1-1] + 3;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
            A[i_0+1][i_2] = D[i_2+i_0] - D[i_0] - 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        C[i_3] = D[i_3] + C[i_3+2] * C[i_3+1] - C[i_3-2] * C[i_3-1] - 1;
    }
#pragma endscop
...
kernel_2332112323_05(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
