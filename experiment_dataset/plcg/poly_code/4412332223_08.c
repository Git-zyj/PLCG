...
void kernel_4412332223_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_2+PB_N-2, i_1); i_3++) {
                    A[i_3-2][i_2][i_1] = E[i_0+1][i_1][i_2][i_3] * F[i_3+i_2+2][i_1-2][i_2] + G[i_1-1][i_3-2][i_0][i_2][i_1-1] - 5;
                }
            }
        }
        for (int i_4 = 2; i_4 < PB_Q-2; i_4++) {
            for (int i_5 = 0; i_5 < min(PB_P-1, -i_4+PB_L+2); i_5++) {
                for (int i_6 = 2; i_6 < PB_L-2; i_6++) {
                    A[i_6-2][i_4][i_5] = H[i_4][i_5+i_4-2][i_0][i_5] * I[i_0][i_4-2][i_6+2][i_5+1] * H[i_6+2][i_5][i_0][i_4+2] - H[i_0][i_6][i_5][0] + 6;
                }
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_Q-2; i_7++) {
        for (int i_8 = 2; i_8 < PB_P-3; i_8++) {
            C[i_7][i_8+2] = J[i_8][i_7] - K[i_7-1][i_8] + L[i_8-2][i_7-1][i_7] + C[i_7-2][i_8+3] + 3;
        }
        for (int i_9 = 2; i_9 < PB_L-1; i_9++) {
            D[i_7][i_9] = D[i_9][i_7+2] - D[i_9+1][i_7-1] + D[i_7-1][i_9-2] + 2;
        }
    }
#pragma endscop
