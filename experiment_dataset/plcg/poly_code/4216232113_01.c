...
void kernel_4216232113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_P-2); i_1++) {
            for (int i_2 = 2; i_2 < PB_P; i_2++) {
                A[i_1][i_2] = C[i_2-2][i_2][i_1+i_0+2] + C[i_1-2][i_0][i_2] + A[i_2+1][i_1] + 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            for (int i_5 = 0; i_5 < -i_3+PB_P; i_5++) {
                B[i_5][i_4][i_5+i_3] = D[i_5+2][i_3][i_4] - C[i_4][i_3][i_5] * C[i_5][i_3][i_4] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4216232113_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
