...
void kernel_3224332115_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-2 - 1; i_0 >= 3; i_0--) {
        A[i_0-2] = C[i_0+2] * D[0] + E[i_0][0] - A[i_0-3] * 1;
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            B[i_1][i_0+2] = E[i_1+2][i_0] * D[i_1] - 5;
        }
    }
#pragma endscop
...
kernel_3224332115_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
