...
void kernel_4212211315_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_1 = 1; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_Q-2, -i_0+PB_Q-1); i_2++) {
                B[i_0][i_1+1][i_2+i_0+1] = C[i_2+i_1][i_0] * B[i_1][i_0+1][i_2+i_1] + B[i_1-1][i_0][i_2+i_1] + B[i_1][i_0+1][i_2+i_1+2] + B[i_0][i_1+2][i_2+i_0] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4212211315_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
