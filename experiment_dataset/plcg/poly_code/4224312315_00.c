...
void kernel_4224312315_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 3; i_2 < PB_M+1; i_2++) {
                A[i_2-2][i_0] = A[i_2-3][i_1] + A[i_1-2][i_2-1] * A[i_2-2][i_1-1] * 4;
            }
        }
        B[i_0] = C[i_0] * C[i_0+2] - B[i_0+1] * B[i_0-1] * 2;
    }
#pragma endscop
...
kernel_4224312315_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
