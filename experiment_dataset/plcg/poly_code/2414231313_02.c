...
void kernel_2414231313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = E[i_0-1] + F[i_0] + E[i_0] * E[i_0] - B[i_0] + B[i_0-1] - 4;
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_1] = G[i_1-1][i_0] + H[i_0+1][i_1] - A[i_0] - B[i_1-1] * 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M-1; i_3++) {
            C[i_3] = F[i_3+i_2+1] + C[i_3+1] * C[i_3-1] * C[i_3-1] + 3;
        }
        for (int i_4 = 0; i_4 < i_2; i_4++) {
            A[i_4+1] = H[i_4][i_2] + G[i_4][i_2] + G[i_4][i_2] - B[i_4] - 5;
        }
    }
#pragma endscop
...
kernel_2414231313_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
