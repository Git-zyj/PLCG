...
void kernel_2412351213_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-1] + E[i_0+1] + F[i_0+1] + F[i_0] * A[i_0+1] - 5;
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            A[i_1] = G[i_1-1][i_0] + H[i_0][i_1-1][i_0+1] - E[i_1] * A[i_1] - 6;
        }
        A[i_0] = I[i_0] - I[i_0-1] - F[i_0-1] * I[i_0] - J[i_0+1][i_0] - 1;
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        A[i_2-1] = F[i_2] * F[i_2] * E[i_2] - E[0] - F[i_2] + E[i_2] + F[i_2] - 2;
    }
#pragma endscop
...
kernel_2412351213_03(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
