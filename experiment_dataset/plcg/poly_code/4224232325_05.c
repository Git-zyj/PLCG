...
void kernel_4224232325_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_N; i_1++) {
            A[i_0][i_1] = C[i_0][i_1] * D[i_1][i_0+2] * D[i_0][0] * E[i_0-2] * E[i_1] * A[i_0][i_1+2] + A[i_0-2][i_1] * A[i_0][i_1-2] + 1;
        }
        B[i_0] = F[i_0] * B[i_0-2] - B[i_0+2] + 3;
    }
#pragma endscop
...
kernel_4224232325_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
