...
void kernel_3434252123_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[i_1-2][i_0+1] = E[i_1] + F[i_1][i_1+i_0] - F[i_1][0] + F[i_1][i_0] + G[i_1+1][i_0] * G[i_1][i_0] * A[i_1-2][i_0+2] + 1;
        }
        B[i_0+1] = E[i_0+2] * E[i_0] - E[i_0] + H[i_0-1] * H[0] * 6;
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_N; i_3++) {
            A[i_3-2][i_2+1] = F[i_2][i_3] + G[i_2][i_3] - G[i_2][i_3] + I[i_3+i_2][i_3+i_2+2][i_3] + G[i_3][i_2] - E[i_2] + B[i_3+2] - 2;
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        B[i_4+1] = E[i_4] * E[i_4] - H[i_4] * 6;
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3434252123_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
