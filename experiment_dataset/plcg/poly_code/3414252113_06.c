...
void kernel_3414252113_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        B[i_0] = E[i_0] * E[i_0] * F[i_0] + G[i_0][i_0+2] + E[i_0-2] - F[i_0-2] - 1;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            B[i_2] = H[i_2][i_1] - F[i_1] + G[i_2][2] - 5;
        }
        for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
            for (int i_4 = max(2, i_1); i_4 < PB_L; i_4++) {
                C[i_3-1][i_4] = I[i_4][i_1][i_3+2] * I[i_1-1][i_4-2][i_3] - 4;
            }
            D[i_1][i_3] = H[i_3][i_1] * G[i_3][i_1] - H[i_1][i_3] * G[i_3][i_1] * H[i_1][i_3] + H[i_3][i_1+1] * E[i_1] * G[i_3-1][i_3-1] * F[i_3] + D[i_3-1][i_1] - 4;
        }
    }
#pragma endscop
...
kernel_3414252113_06(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
