...
void kernel_4132312325_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 3; i_2 < PB_Q+1; i_2++) {
                for (int i_3 = 2; i_3 < min(PB_Q, i_1); i_3++) {
                    A[i_0][i_1][i_2-2][i_3] = B[i_1-2][i_2][i_3-2] + A[i_0][i_1][i_2-3][i_3] * A[i_1-1][i_2][i_3-2][i_0] + A[i_1][i_3][i_2-1][i_0] * 5;
                }
            }
        }
    }
...
kernel_4132312325_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
