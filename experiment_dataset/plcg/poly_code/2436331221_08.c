...
void kernel_2436331221_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0-1] * F[i_0] - E[i_0-1] * A[i_0-2] * 5;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_P, i_1); i_2++) {
            A[i_2] = G[i_2][i_2+i_1] * H[i_2][i_1-1] + 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        C[i_3][i_3] = F[i_3+1] - E[i_3-1] + E[i_3] * I[i_3-1] * I[i_3] - C[i_3-1][i_3-1] * 1;
        for (int i_4 = 2; i_4 < i_3; i_4++) {
            D[i_4+1][i_3] = C[i_3-2][i_3-2] + D[i_4+1][i_3-2] * D[i_3+1][i_4-2] - D[i_4+1][i_3+2] * 1;
        }
    }
#pragma endscop
...
kernel_2436331221_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
