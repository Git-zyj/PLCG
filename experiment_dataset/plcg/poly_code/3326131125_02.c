...
void kernel_3326131125_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[i_1][i_0+1] = D[i_0] + E[i_1+i_0][i_1+i_0] * A[i_1-1][i_0+1] + 4;
        }
        for (int i_2 = i_0; i_2 < -i_0+PB_L-1; i_2++) {
            C[i_0-1][1] = E[i_0][i_2+i_0+1] - D[i_2] + 3;
        }
    }
    for (int i_3 = 3; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_L-1, i_3); i_4++) {
            C[i_3-1][1] = E[i_3][i_4+i_3+1] + E[i_3+1][i_4] + E[i_4+i_3][i_4] - E[i_3-1][i_4-1] * D[i_4] + C[i_3-3][1] - 5;
        }
    }
#pragma endscop
...
kernel_3326131125_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
