...
void kernel_3222211323_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N-1; i_1++) {
            A[i_1+i_0][i_1+1] = A[i_1+i_0][i_1] * A[i_1+i_0+1][i_1+2] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_N-1; i_4++) {
                A[i_4+i_3+1][i_4+2] = C[i_3][i_2+1] + A[i_4+i_3][i_4+1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3222211323_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
