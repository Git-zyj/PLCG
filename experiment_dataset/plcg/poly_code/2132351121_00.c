...
void kernel_2132351121_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_L-1); i_1++) {
            A[i_0][i_0-1][i_1+1] = B[i_1+i_0+1][0] - B[i_1][i_1+i_0] + C[i_1][i_0][0] + B[i_0][i_1] + B[i_1+i_0][i_0] * A[i_0][i_0-1][i_1+3] * 1;
        }
    }
#pragma endscop
...
kernel_2132351121_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
