...
void kernel_3236111315_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = A[i_1][i_0-1] + A[i_1+1][i_0-1] * A[i_1-1][i_0] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
            for (int i_4 = 0; i_4 < i_3; i_4++) {
                A[i_4+1][i_3-1] = C[i_4][i_3] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3236111315_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
