...
void kernel_3126112123_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = -i_0+PB_P - 1; i_1 >= max(i_0, 4); i_1--) {
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_Q+2, i_0), -i_0+PB_P); i_2++) {
                A[i_1+i_0][i_2][i_2+i_1-2] = B[i_1][i_0] - A[i_2+i_0][i_1-2][i_2+i_1-4] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3126112123_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
