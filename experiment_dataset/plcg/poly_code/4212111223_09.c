...
void kernel_4212111223_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < PB_N-1; i_1++) {
            A[i_1-1] = C[i_1][i_0] - C[i_0-1][i_1] - A[i_1-3] - A[i_1] * A[i_1+1] + 4;
        }
    }
        ...
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            for (int i_4 = 2; i_4 < PB_P; i_4++) {
                for (int i_5 = 0; i_5 < PB_P; i_5++) {
                    B[i_3][i_4][i_5] = B[i_4-2][i_5][i_3] - 1;
                }
            }
        }
    }
...
kernel_4212111223_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
