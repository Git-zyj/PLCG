...
void kernel_4216352121_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(min(i_0, -i_0+PB_L+1), PB_M-1); i_1++) {
            A[i_1-1][i_0] = C[i_1+i_0-1][0] - D[i_1-1][i_0] + C[0][i_1] * E[i_1] + E[i_1] * F[i_1+1][i_0] * 5;
        }
        for (int i_2 = 2; i_2 < i_0; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                B[i_3][i_2][i_0] = G[i_3][i_0][i_2] * C[i_2][i_3] + H[0][i_2][i_2] + B[i_3+2][i_2-2][i_0-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4216352121_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
