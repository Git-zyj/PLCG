...
void kernel_3322312223_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = D[i_0] * A[i_0+4] + A[i_0] * B[i_0-2] + 6;
    }
    for (int i_1 = 3; i_1 < PB_M+2; i_1++) {
        B[i_1-2] = D[i_1-2] - B[i_1-3] - 1;
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M-2; i_3++) {
            C[i_3+i_2+1][i_3+i_2] = C[i_3+i_2+2][i_3+i_2+1] * C[i_3+i_2+3][i_3+i_2+2] * 5;
        }
    }
#pragma endscop
...
kernel_3322312223_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
