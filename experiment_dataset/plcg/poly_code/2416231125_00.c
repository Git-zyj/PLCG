...
void kernel_2416231125_00(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        C[i_0] = E[i_0] + F[i_0] - F[i_0] * F[i_0] + 5;
    }
    for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_N; i_2++) {
            B[i_2][i_1] = G[i_2][i_2+i_1] * F[i_2+i_1] * H[i_2-1][i_1] * 5;
            C[i_2] = F[i_2+i_1] * I[i_2-1][i_1][i_1] + 6;
        }
        D[i_1] = F[i_1+1] + F[i_1] + D[i_1+1] - 5;
    }
#pragma endscop
...
kernel_2416231125_00(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
