...
void kernel_2436311325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
            B[i_1][i_2] = B[i_1+2][i_2-2] + B[i_1][i_2-2] * B[i_1+1][i_2+1] - 2;
        }
        D[i_1] = 2;
    }
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        D[i_3] = F[i_3-1][i_3-1] - G[i_3+1] + H[i_3+1] + D[i_3+2] * D[i_3-1] + D[i_3+1] + 3;
    }
#pragma endscop
...
kernel_2436311325_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
