...
void kernel_4432152123_09(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        C[i_0+1][i_0+1] = E[i_0] - E[i_0] + E[i_0-2] - E[i_0] + E[i_0+1] * E[i_0] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
            for (int i_3 = 2; i_3 < -i_1+PB_Q; i_3++) {
                for (int i_4 = i_2; i_4 < -i_2+PB_M+2; i_4++) {
                    B[i_2][i_3][i_1+2] = F[i_4][i_3+2][i_2] - F[i_2][i_4-1][i_3] - G[i_4+i_2-2][0][i_1+1][i_3] - G[i_2][i_3][i_1][i_3] + G[i_2][i_4][i_2][i_1] + G[i_3-2][i_2][i_4-1][i_3+i_1] - B[i_1][i_3-1][i_2+2] + 1;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = i_5; i_6 < PB_M; i_6++) {
            C[i_5][i_5] = H[i_6+1][i_5] + E[i_6] - H[i_6][i_5] + H[i_5][i_6] - H[i_5-1][i_6-1] * 1;
            C[i_5][i_5] = H[i_6+1][0] - E[i_6] + F[i_5][i_6-1][i_5] + 1;
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4432152123_09(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
