...
void kernel_4214132225_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_P; i_1++) {
            A[i_0+1][i_1] = C[i_1-2][i_0] * C[i_1][i_1] + C[i_1+i_0-2][i_1+i_0] * D[i_0] - C[i_0-2][i_1] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-4; i_2++) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            for (int i_4 = 2; i_4 < -i_2+PB_M-4; i_4++) {
                B[i_3-1][i_4+i_2+2][i_4+2] = E[i_3-2][i_2][i_4-2] * B[i_3-1][i_4+i_2+3][i_2+2] * B[i_3-1][i_4+i_2][i_4+2] * B[i_3][i_4+i_2+4][i_4+4] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4214132225_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
