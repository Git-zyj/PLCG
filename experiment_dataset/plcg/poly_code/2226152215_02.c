...
void kernel_2226152215_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            A[i_0][i_1+1] = C[i_1][i_0-1] - D[0] - C[i_1][i_0] + C[i_0][i_1] - A[i_1][i_0+2] + A[i_0-1][i_1+1] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(min(i_2, PB_L-1), -i_2+PB_M); i_3++) {
            A[i_2][i_3+1] = C[i_3][i_3+i_2] * C[i_3][i_2] - C[i_2][0] + C[i_2][0] + D[i_3+2] * C[i_2][i_3] - 6;
        }
    }
#pragma endscop
...
kernel_2226152215_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
