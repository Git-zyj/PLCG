...
void kernel_4116211325_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-3; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = PB_M - 1; i_2 >= 1; i_2--) {
                A[i_2][i_0+1][i_1-1][i_2] = B[i_2][i_0] + A[i_2][i_0-1][i_1][i_2] - A[i_2][i_0+3][i_1+1][i_2] + A[i_1+1][i_0+2][i_2-1][i_1+1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4116211325_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
