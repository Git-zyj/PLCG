...
void kernel_2236111323_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_N); i_1++) {
            B[i_1][i_1] = C[i_1+i_0][0] * B[i_0+2][i_0+2] * 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_2][i_2] = B[i_2-2][i_2-2] + B[i_2+2][i_2+2] - 5;
        }
    }
#pragma endscop
...
kernel_2236111323_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
