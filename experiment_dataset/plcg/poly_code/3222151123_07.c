...
void kernel_3222151123_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][i_1] = C[i_1] - D[i_0][0] - D[i_1][i_0] - A[i_0+2][i_1] - A[i_0+1][i_1] * 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_M-1, -i_2+PB_P); i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_L; i_4++) {
                B[i_3][i_4] = E[i_3][i_4][i_2] - F[i_4][i_3][i_2][i_3+1] * E[i_4-1][i_2][i_3] - E[i_2][i_3][i_2] + E[i_3+1][i_4-1][i_3+i_2] - E[i_2][i_4+i_3][0] + D[i_4+i_3][i_3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3222151123_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
