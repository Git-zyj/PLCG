...
void kernel_3226332325_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_P+2; i_1++) {
            for (int i_2 = 0; i_2 < min(min(i_0, -i_1+PB_L), PB_M-1); i_2++) {
                A[i_1][i_2] = C[i_1][i_0][i_1-2] + D[i_2+1][i_1] * C[i_1][i_2][i_0] * C[i_2+i_1][i_0][i_0] * A[i_0-1][i_2] + A[i_1][i_0+2] - A[i_2+1][i_1] + 2;
            }
        }
    }
...
kernel_3226332325_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
