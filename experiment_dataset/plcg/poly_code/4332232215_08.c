...
void kernel_4332232215_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M-2, PB_L); i_1++) {
            A[i_1-2][i_1] = D[i_1][i_0] * D[i_1][i_1] + E[i_1+i_0+2] - A[i_0-1][i_0+1] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = 2; i_4 < PB_L; i_4++) {
                B[i_3-1][i_3+2] = F[i_2][i_4][i_3] * G[i_2][i_3+2][1] + A[i_2-2][i_2] + B[i_4][i_4+3] - B[i_4-2][i_4+1] + 1;
            }
        }
        for (int i_5 = i_2; i_5 < -i_2+PB_L; i_5++) {
            A[i_2-2][i_2] = H[i_5+i_2][0] - H[i_5][i_5+i_2] - 1;
        }
    }
#pragma endscop
...
kernel_4332232215_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
