...
void kernel_4234312221_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < PB_M; i_2++) {
                B[i_1+3][i_2-1] = C[i_2][i_1] * 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-3; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            B[i_3+2][i_4-1] = D[i_4+2] + B[i_4+1][i_3-1] - B[i_3+3][i_4-1] - 4;
        }
    }
#pragma endscop
...
kernel_4234312221_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
