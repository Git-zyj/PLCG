...
void kernel_4414152115_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_M, i_1); i_2++) {
                for (int i_3 = -i_2+2; i_3 < -i_2+PB_M; i_3++) {
                    A[i_0-2][0][i_3+i_2-2][i_3] = E[i_1-1][i_0-1][i_3+i_2][i_2] - E[i_1][i_0-1][i_3][i_2] - E[i_3+i_2][i_0-2][i_1][i_3+1] + F[i_2+i_1][i_3][i_0-2] * E[i_2][i_3][i_0-2][i_0+2] * F[i_1][i_2][i_3] + 2;
                }
            }
        }
        for (int i_4 = i_0; i_4 < min(-i_0+PB_P+2, PB_M-1); i_4++) {
            C[i_0-2] = G[i_0][1] - G[i_4+1][i_4+i_0-2] - G[i_0][i_4-2] - G[i_4][i_0] + 4;
        }
    }
    for (int i_5 = 3; i_5 < PB_P-3; i_5++) {
        C[i_5-2] = H[i_5-2] - H[i_5] - H[i_5-1] - H[i_5-2] + H[i_5] - C[i_5-1] * C[i_5-3] * 5;
        for (int i_6 = 0; i_6 < PB_P-2; i_6++) {
            for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
                D[i_7][i_6][i_5+2][i_7+1] = F[i_6][i_7][i_6+2] * F[i_5][i_7][i_6] * F[i_7+1][i_5][i_6] * F[i_5][i_7-1][i_6+1] + D[i_6][i_7][i_5+3][i_6+1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4414152115_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
