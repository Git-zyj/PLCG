...
void kernel_3216232315_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1] = C[i_0-2][i_1+1] - D[i_1][i_0-1] + E[0] + E[i_1+i_0+2] - A[i_1+1] + A[i_1-1] + A[i_1+1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_M+1; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                A[i_4] = D[i_3+i_2-1][i_4+1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3216232315_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
