...
void kernel_4414132215_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 3; i_1 < i_0; i_1++) {
            A[i_1-2][i_0] = E[i_0] - F[i_1][i_0] + F[i_1][1] - A[i_1-1][i_0-1] * A[i_1-1][i_0] * A[i_1-3][i_0+1] - 1;
        }
        B[i_0+2] = E[i_0] - E[i_0-1] * B[i_0+3] + 4;
    }
    for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
        B[i_2+2] = E[i_2-1] + E[i_2-2] + 1;
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
                A[i_4-2][i_3] = F[i_4][i_3] * G[i_4-2][i_2-1][2] * G[i_3][i_4-2][i_2] + F[i_2+1][i_4+2] * H[i_4+2][i_3][i_2+1][0] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414132215_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
