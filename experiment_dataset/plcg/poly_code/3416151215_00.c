...
void kernel_3416151215_00(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                D[i_2-1] = E[i_2-1][i_1][i_0+1][i_0] + F[i_2+1][i_0-1] + F[i_1][i_2] * F[i_2][i_1] + F[i_2+1][0] + 3;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            D[i_3] = F[i_3][0] - F[0][i_0+1] - F[i_0][i_0-1] - F[i_3][i_0] - G[0] - 3;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = i_4; i_5 < PB_L-1; i_5++) {
            for (int i_6 = i_4; i_6 < PB_M-1; i_6++) {
                C[i_6][i_5+1][i_5][i_4] = H[i_5+1][i_6][i_4] + F[i_6][i_5] * H[i_5][i_6][i_5] * H[i_4-1][i_4][i_5] + C[i_6+1][i_5+1][i_5][i_4+1] + 3;
            }
        }
        for (int i_7 = 1; i_7 < -i_4+PB_M; i_7++) {
            D[i_7] = F[i_7+1][i_4] + F[i_7][i_4] + F[i_7][i_4] - F[i_7+i_4][1] - D[i_7+1] - D[i_7-1] - 1;
        }
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3416151215_00(xd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
