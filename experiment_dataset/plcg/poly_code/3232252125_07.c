...
void kernel_3232252125_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        A[i_0] = C[i_0] + D[i_0] - C[i_0] * E[i_0][i_0+2] * D[i_0] * C[i_0] + C[i_0] - C[i_0] + 4;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            for (int i_3 = 2; i_3 < PB_P; i_3++) {
                B[i_3-1][i_1-1] = E[i_3-1][0] * B[i_2][i_3-2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3232252125_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
