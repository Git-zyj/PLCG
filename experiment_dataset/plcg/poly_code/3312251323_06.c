...
void kernel_3312251323_06(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        C[i_0] = D[i_0] + D[i_0+1] * E[0] * D[i_0-1] - E[i_0] * D[i_0+1] - E[i_0] - 2;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_P, -i_1+PB_L+1); i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_M, -i_2+PB_P); i_3++) {
                B[i_1][i_3][i_2-1][i_3] = F[i_3][i_1+1] * G[i_2+1][i_3+i_1][i_3+i_2] * G[i_3+i_1][i_1+1][i_2] - H[i_2-1][i_3+1][i_2] + G[i_2+1][i_3][0] - H[i_3+1][i_2+i_1-1][i_2] * B[i_1-1][i_3][i_2-1][i_3] - B[i_2-1][i_3][i_1-1][i_3] - 6;
            }
        }
        C[i_1] = F[i_1][i_1] * D[i_1] - C[i_1-1] * C[i_1-2] * C[i_1+2] * 3;
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3312251323_06(xc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
