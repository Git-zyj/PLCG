...
void kernel_3112232311_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_1][i_0] = B[i_1] * C[i_1][i_0+1][0] * D[i_1][2] - A[i_1][i_0-1] * A[i_0][i_1+1] * A[i_1+1][i_0+1] * 1;
        }
    }
#pragma endscop
...
kernel_3112232311_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
