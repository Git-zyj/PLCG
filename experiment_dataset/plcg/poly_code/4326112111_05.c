...
void kernel_4326112111_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_1+i_0][i_2][i_2] = D[i_2+2][i_0+1] + 2;
            }
        }
        for (int i_3 = 2; i_3 < PB_L+1; i_3++) {
            for (int i_4 = i_3; i_4 < PB_L; i_4++) {
                B[i_3-2][i_4] = B[i_4-2][i_3-1] + 2;
            }
        }
    }
...
kernel_4326112111_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
