...
void kernel_2326311323_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_L, i_0); i_1++) {
            A[i_1-1][i_0][i_1-1] = A[i_1][i_0][i_1] + A[i_1-1][i_0-1][i_1-1] * A[i_0-1][i_1-1][i_0-1] - 3;
        }
    }
    for (int i_2 = PB_L - 1; i_2 >= 2; i_2--) {
        B[i_2+1] = B[i_2+2] - B[i_2+3] - 2;
        C[i_2] = D[i_2] - B[i_2+1] * C[i_2-2] - C[i_2-1] * 6;
    }
#pragma endscop
...
kernel_2326311323_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
