...
void kernel_3412212213_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = E[i_1-2] * A[i_1+1][i_0] - A[i_1][i_0+1] - A[i_1][i_0-1] * 1;
        }
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            B[i_2-1][i_2] = B[i_0-2][i_0-1] * 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = min(-i_3+PB_L+2, PB_L-1) - 1; i_4 >= 3; i_4--) {
            C[i_4-2][1] = C[i_4-3][1] * D[i_4][i_4+i_3-2] * D[i_3][i_4+i_3-2] - 5;
            D[i_3][i_4+i_3-2] = F[i_4+1] * G[i_3][i_4-1] - D[i_4+1][i_4+i_3-2] * 3;
        }
    }
#pragma endscop
...
kernel_3412212213_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
