...
void kernel_2126311225_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-2; i_1++) {
            A[i_1] = B[i_1+1][i_0] * A[i_1+2] * A[i_1-1] * 4;
        }
    }
#pragma endscop
...
kernel_2126311225_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
