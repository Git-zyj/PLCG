...
void kernel_3432351313_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_Q-1, PB_P-1); i_1++) {
            for (int i_2 = i_1; i_2 < min(PB_P, -i_0+PB_L); i_2++) {
                A[i_0][i_1][i_1+i_0] = E[i_1][i_2][i_0] * E[i_2+1][i_1][i_2+i_0-1] + F[i_1-1][1] - E[i_0][i_1][i_2-1] * F[i_0-1][i_1] * G[i_2+1][i_1] + E[i_2+i_0][i_1-1][i_2] - E[i_2-1][i_1][i_0] * A[i_0][i_1+1][i_1+i_0+1] + A[i_0][i_1-1][i_1+i_0-1] - 3;
            }
        }
        D[i_0+2] = H[i_0+1] + I[i_0] + I[i_0-1] - D[i_0] + 4;
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_P, -i_3+PB_Q); i_4++) {
            for (int i_5 = 1; i_5 < i_4; i_5++) {
                A[i_4][i_5][i_5+i_4] = J[i_3+1][i_5] * E[i_4][i_3][i_5-1] + F[i_3][0] - E[i_4][i_5-1][i_3+1] + E[i_5][i_3-1][i_4+1] - A[i_3][i_4][i_4+i_3] * A[i_3-1][i_5][i_5+i_3-1] - 3;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_L+1; i_6++) {
        D[i_6+1] = K[i_6-1] + G[i_6+1][i_6-1] - K[i_6] * D[i_6+2] * 1;
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3432351313_00(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K)));
