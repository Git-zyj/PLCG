...
void kernel_4216151321_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_L-1; i_2++) {
                A[i_1-1][i_0] = C[i_2][i_1] * D[i_1][i_0-1][i_2+i_0+1] - C[i_2][1] - D[i_0][i_2][i_1-1] * A[i_2-1][i_1+2] * A[i_1-2][i_0-1] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            B[i_4][i_3] = C[i_4][i_3] * C[0][i_3+1] + C[i_3][i_3+1] + E[i_3-1] * C[1][i_4] + B[i_4-2][i_3] - B[i_4][i_3-1] - B[i_4-1][i_3] - 6;
        }
    }
#pragma endscop
...
kernel_4216151321_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
