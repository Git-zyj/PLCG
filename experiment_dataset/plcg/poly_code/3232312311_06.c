...
void kernel_3232312311_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        B[i_1] = C[i_1] * B[i_1-1] + B[i_1+1] + B[i_1+1] * 3;
    }
#pragma endscop
...
kernel_3232312311_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
