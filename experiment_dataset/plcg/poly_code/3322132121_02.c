...
void kernel_3322132121_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            A[i_0+1][i_1] = D[i_1][i_0] + D[i_1][i_0] - E[0] - D[i_1][i_0] * B[i_0-2][0] * 1;
        }
    }
    ...
    for (int i_4 = 1; i_4 < PB_N-2; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_N-2; i_5++) {
            C[i_4-1] = D[i_5+i_4][i_5+i_4+2] - E[i_5-1] - C[i_5+1] * 1;
        }
    }
#pragma endscop
...
kernel_3322132121_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
