...
void kernel_2434351113_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            A[i_1+1] = E[i_1+1] * E[i_0] * F[i_0+1][i_1+1] * G[i_0][i_1] + F[i_1+1][i_0] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_N, i_2); i_3++) {
            B[i_2][i_3] = F[i_3+1][i_2+1] - G[i_3+i_2][0] * H[i_3-1][i_2] + H[i_3-1][i_2] + I[i_2-1] - B[i_2-1][i_3+1] * 5;
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        C[i_4+1] = F[i_4][1] - I[i_4+1] + J[i_4] + E[i_4] * I[i_4] * C[i_4+2] - 3;
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_N-1); i_5++) {
            D[i_5][i_4+1] = G[i_4][i_5] - F[i_5][i_4] * G[i_5+i_4+1][i_5] + 3;
        }
    }
#pragma endscop
...
kernel_2434351113_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
