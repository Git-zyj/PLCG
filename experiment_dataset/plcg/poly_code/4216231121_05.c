...
void kernel_4216231121_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
        ...
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                for (int i_3 = 0; i_3 < min(PB_P-2, i_1); i_3++) {
                    A[i_1][i_2][i_3] = C[i_0][i_1][i_1] * D[i_2][i_0][i_1][i_3+1] + E[i_1][i_2+1][i_0-1][i_1+1] + D[i_1+1][i_3][i_0][i_2-1] + A[i_1][i_3+2][i_2] * 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < PB_M+1; i_5++) {
            for (int i_6 = i_5; i_6 < PB_P-2; i_6++) {
                B[i_4-1][i_4][i_5] = C[i_5-1][i_4][1] * A[i_5][i_4][i_6+2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4216231121_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
