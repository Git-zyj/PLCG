...
void kernel_4232132325_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            B[i_4][i_3] = C[i_4+1][i_3+2] * D[i_4] + D[i_3+2] * C[i_4+1][i_3] - D[0] * B[i_4+1][i_3] + B[i_4+2][i_3+1] - B[i_4][i_3-1] * 3;
        }
    }
#pragma endscop
...
kernel_4232132325_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
