...
void kernel_3334212311_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        B[i_2-1] = D[i_2-2] - B[i_2-2] * B[i_2] - B[i_2] + 5;
    }
    for (int i_3 = 3; i_3 < PB_M; i_3++) {
        C[i_3-2][i_3-1] = C[i_3-3][i_3-2] - C[i_3-1][i_3] * 4;
    }
#pragma endscop
...
kernel_3334212311_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
