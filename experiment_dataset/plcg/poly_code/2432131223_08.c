...
void kernel_2432131223_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M-1; i_1++) {
            B[i_1-2] = E[i_1][i_0-1] * E[i_1+i_0+1][1] - D[i_1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_M, -i_2+PB_P); i_3++) {
            B[i_3] = E[i_2-1][i_3] + E[i_3][i_3+i_2] * E[i_3][i_2] - B[i_3-1] - 6;
        }
        B[i_2] = E[i_2-1][i_2] - B[i_2] + 2;
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        D[i_4] = F[i_4] - F[i_4] - F[i_4] * F[0] * F[i_4-1] * D[i_4+1] * 2;
    }
#pragma endscop
...
kernel_2432131223_08(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
