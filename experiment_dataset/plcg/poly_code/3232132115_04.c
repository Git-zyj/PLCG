...
void kernel_3232132115_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_Q - 1; i_0 >= 0; i_0--) {
        for (int i_1 = i_0; i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            A[i_1] = C[i_1+i_0][i_1] - C[i_1][i_0] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        A[i_2] = D[i_2] + D[i_2+2] - D[0] - D[i_2] - A[i_2] * 2;
    }
#pragma endscop
...
kernel_3232132115_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
