...
void kernel_4314232115_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = D[i_0+1] - E[0] - D[i_0] - A[i_0] + 2;
    }
    for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            B[i_1][i_2][i_2+2] = F[i_2][i_1+1] - G[i_2-2][i_1-2] + G[i_2][i_2+i_1] + B[i_1][i_2-1][i_2+1] - 5;
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            A[i_1] = H[i_3+2][i_1][0] - G[i_3][i_1+1] * 3;
        }
    }
#pragma endscop
...
kernel_4314232115_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
