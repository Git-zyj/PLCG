...
void kernel_3224312223_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0] = C[i_0+2] + C[i_0] + 5;
        for (int i_1 = 0; i_1 < min(PB_L-4, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                B[i_1+2][i_2+2] = B[i_1+4][i_2] * B[i_1+1][i_2+1] * B[i_2+2][i_1+4] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3224312223_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
