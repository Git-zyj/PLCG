...
void kernel_3222231325_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                B[i_0+1][i_1] = C[i_2+i_1][i_1][i_0+1] + D[i_1][i_0+1][i_1] - D[i_1+i_0][i_2][i_1] + E[i_2][i_1-1] * 2;
            }
            for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                B[i_3+1][i_1] = D[i_1][i_3][i_0-1] - B[i_3+2][i_1-2] + B[i_3+1][i_1-2] * B[i_1+1][i_3+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3222231325_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
