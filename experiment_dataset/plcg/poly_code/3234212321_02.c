...
void kernel_3234212321_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L-2; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                A[i_2][i_1+i_0][i_1] = C[i_2+2][i_0][i_2][0] + A[i_2][i_1+i_0+2][i_1+2] + A[i_2-1][i_1+i_0-2][i_1] + 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 3; i_4 < -i_3+PB_M; i_4++) {
            B[i_4-2][i_3] = D[i_4+i_3][0] * B[i_4-2][i_3+1] - B[i_4-2][i_3-2] + B[i_4-3][i_3+1] - 3;
        }
    }
#pragma endscop
...
kernel_3234212321_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
