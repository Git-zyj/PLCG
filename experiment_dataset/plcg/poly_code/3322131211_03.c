...
void kernel_3322131211_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            A[i_0][i_1-1] = D[i_0][i_1+1] + D[i_0+1][i_0+1] * D[i_1-1][i_0] - D[i_1][i_0] + C[i_1+1] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_L-2, -i_2+PB_P); i_3++) {
            B[i_2][i_3+1] = E[i_2] - D[i_3][i_3+i_2] * B[i_2+1][i_3+1] - B[i_2][i_3+2] - 1;
        }
    }
    for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
        C[i_4] = E[i_4+1] * E[i_4] * E[i_4+1] + C[i_4+1] * 4;
    }
#pragma endscop
...
kernel_3322131211_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
