...
void kernel_3126131223_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_L, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < min(min(PB_M, i_0), -i_0+PB_P); i_2++) {
                A[i_2][i_0] = B[i_1+1][i_0+1][i_2] + B[i_1-1][i_2][i_0-1] - C[i_1+i_0][i_2+i_0] * A[i_2][i_0+1] * A[i_2][i_1+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3126131223_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
