...
void kernel_3332112121_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_1][i_1-1][i_0] = A[i_1][i_1-1][i_0+1] * 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        B[i_3] = D[0] + D[i_3] - B[i_3+1] * 3;
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = PB_L - 1; i_5 >= 0; i_5--) {
            C[i_4][i_5][i_4] = C[i_5][i_4-1][i_5] - 6;
        }
    }
#pragma endscop
...
kernel_3332112121_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
