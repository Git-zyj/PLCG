...
void kernel_2416352125_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + E[i_0] * E[i_0] - E[i_0+2] - F[i_0] - F[i_0-1] * E[i_0+1] - E[0] + E[i_0] + A[i_0+2] - A[i_0-1] + 2;
    }
    for (int i_1 = 2; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
            B[i_2][i_2+i_1-1] = G[i_2+i_1][i_2] + H[i_2-1][i_1] + 5;
        }
        for (int i_3 = 1; i_3 < min(-i_1+PB_M-2, i_1); i_3++) {
            B[i_3-1][i_3+i_1-2] = I[i_1][i_3] - F[i_1] * I[i_3+i_1+2][i_1+2] - 4;
        }
        A[i_1+2] = E[i_1] - F[i_1] * F[i_1-2] + H[i_1+1][i_1-2] * J[i_1] - E[i_1] - 3;
    }
#pragma endscop
...
kernel_2416352125_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
