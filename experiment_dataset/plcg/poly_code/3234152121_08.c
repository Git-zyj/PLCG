...
void kernel_3234152121_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0][i_0] = C[i_0][i_0] + D[i_0] - D[i_0] * 6;
    }
    for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            B[i_2][i_1] = C[i_2+1][i_1] * C[i_2][i_1] + C[i_2+1][i_1] - C[i_2][i_1] - C[i_2][i_1+1] + C[i_2][i_1+1] - C[i_2][i_1+2] * B[i_2-1][i_1] - 2;
        }
    }
#pragma endscop
...
kernel_3234152121_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
