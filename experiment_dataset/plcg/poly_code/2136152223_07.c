...
void kernel_2136152223_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 4; i_1 < PB_N; i_1++) {
            A[i_0][i_1-2] = B[i_1+1] + C[i_1][i_0][i_0+2] * D[i_1][i_0-1] * D[i_0+2][i_1+1] * D[i_1][i_1] - A[i_0][i_1] + A[i_0][i_1-4] * 2;
        }
    }
#pragma endscop
...
kernel_2136152223_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
