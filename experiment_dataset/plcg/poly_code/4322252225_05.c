...
void kernel_4322252225_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        B[i_0+2] = D[i_0] * E[i_0] - D[i_0] * D[i_0] * E[i_0-1] + 6;
    }
    for (int i_1 = 2; i_1 < PB_M-3; i_1++) {
        B[i_1+2] = E[i_1] - D[i_1-2] + D[i_1] * D[i_1] * B[i_1] - B[i_1+3] + 5;
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 4; i_4 < PB_M; i_4++) {
                C[i_3-1][i_4-2][i_2] = F[i_2-1][i_4][i_3] + F[i_4-1][i_2-2][i_3] - G[i_4][i_2][i_3] * F[i_2+1][i_3][i_4] * H[i_2-2][i_3+1] * F[i_3][i_4][i_2-2] + C[i_3-1][i_4-4][i_2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4322252225_05(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
