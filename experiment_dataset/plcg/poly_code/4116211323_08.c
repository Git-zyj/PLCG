...
void kernel_4116211323_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                for (int i_3 = 1; i_3 < PB_N-3; i_3++) {
                    A[i_0+1][i_3][i_2][i_0] = B[i_2-1][i_3][0] * A[i_0-1][i_3][i_2][i_0-2] * A[i_3+3][i_0][i_2][i_3+2] * A[i_0+3][i_3-1][i_2-1][i_0+2] - 2;
                }
            }
        }
    }
...
kernel_4116211323_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
