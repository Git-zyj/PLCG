...
void kernel_2212312321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(min(i_0, -i_0+PB_L-3), PB_M-2); i_1++) {
            A[i_1][i_1+i_0] = C[i_1][i_0] * A[i_1][i_1+i_0-1] + A[i_1+2][i_1+i_0+3] - A[i_0-2][i_1+i_0-2] - A[i_1][i_1+i_0+2] * A[i_1-2][i_1+i_0-2] + 4;
        }
    }
...
kernel_2212312321_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
