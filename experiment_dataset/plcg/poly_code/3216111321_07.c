...
void kernel_3216111321_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[i_1-1][i_0+1][i_2][i_0] = C[i_0][i_2][i_1] - A[i_2-2][i_0+1][i_1-1][i_0] + A[i_2-1][i_0+1][i_1-1][i_0] - 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_N-2, i_3); i_4++) {
            for (int i_5 = 0; i_5 < PB_N-2; i_5++) {
                B[i_3][i_4] = C[i_4+1][0][i_3] * A[i_5][i_3+1][i_4][i_3] + B[i_4+2][i_5+1] * B[i_3+1][i_4] + B[i_5+2][i_4+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3216111321_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
