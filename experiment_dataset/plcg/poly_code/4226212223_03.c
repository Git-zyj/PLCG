...
void kernel_4226212223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_Q-3, i_0); i_1++) {
            for (int i_2 = 2; i_2 < min(i_0, PB_M-2); i_2++) {
                A[i_2+2][i_1] = C[i_1][i_2-2][0] + D[i_2][i_1] * 6;
            }
            for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
                for (int i_4 = 0; i_4 < PB_P; i_4++) {
                    B[i_1-1][i_0-2][i_1+2][i_3] = A[i_3+2][i_1] * B[i_1][i_0-2][i_1+3][i_3] - B[i_1-1][i_0][i_1+2][i_3] + B[i_3-1][i_1-3][i_3+2][i_0] + 5;
                }
            }
        }
    }
...
kernel_4226212223_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
