...
void kernel_4324231125_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                for (int i_3 = 0; i_3 < -i_2+PB_M; i_3++) {
                    A[i_1+1][i_0][i_2][i_0] = D[i_3+i_2][i_0+1][i_1][i_2+1] + D[i_3][i_1-1][i_2][i_0] + 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N-3; i_4++) {
        B[i_4] = E[i_4] - E[i_4] * E[i_4-1] * E[i_4] + B[i_4-1] - 3;
        for (int i_5 = 0; i_5 < PB_P; i_5++) {
            for (int i_6 = 0; i_6 < i_5; i_6++) {
                A[i_4+3][i_6][i_5][i_6] = F[i_5][i_4][i_6] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4324231125_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
