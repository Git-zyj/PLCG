...
void kernel_4316251121_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                for (int i_3 = 0; i_3 < i_0; i_3++) {
                    A[i_3][i_1][i_0-1][i_2] = D[i_0][i_3][i_1][i_2] * E[i_2][i_3][i_0] - D[i_3+i_1][i_0][i_2-1][i_1+1] - F[i_3][i_2+i_1][i_2-1] * D[i_2][i_1+1][i_0+1][i_2] * 3;
                }
            }
            C[i_1+i_0+2][i_0+2][i_1] = G[i_0][i_1] * H[i_1-1][i_1] - H[i_1][i_0] + G[i_0][i_1] + H[i_0][i_1] * 6;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = i_4; i_5 < min(PB_P-3, -i_4+PB_M-2); i_5++) {
            for (int i_6 = 1; i_6 < -i_4+PB_Q; i_6++) {
                C[i_5+i_4+1][i_5+1][i_4] = F[i_5][i_4][i_4+1] - G[i_6-1][i_5+1] + E[i_5+i_4-1][i_6][i_6+i_4] + F[i_5][i_6+i_4][i_5+1] + F[i_4][i_6][i_5] + C[i_5+i_4+2][i_5+3][i_4-1] + C[i_5+i_4+1][i_5+2][i_4-1] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4316251121_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
