...
void kernel_3414112123_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_0-2][i_1+2][i_0-1] = 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L; i_4++) {
            B[i_4][i_4] = E[i_4][i_3-1] + E[i_4][i_3] * B[i_4-1][i_4-1] + 3;
        }
        for (int i_5 = 2; i_5 < PB_P; i_5++) {
            C[i_3][i_5] = E[i_5][i_3] - C[i_5+2][i_3] + 1;
            D[i_5] = C[i_3][i_5] + D[i_5-2] * 5;
        }
    }
#pragma endscop
...
kernel_3414112123_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
