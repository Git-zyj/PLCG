...
static void print_array(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4432231321_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] * E[i_0+1] * F[i_0] * A[i_0+1] * A[i_0+2] + 2;
    }
        ...
        for (int i_2 = 1; i_2 < min(PB_M-2, -i_1+PB_Q); i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_M, -i_2+PB_M-2); i_3++) {
                for (int i_4 = 0; i_4 < PB_M; i_4++) {
                    B[i_1][i_4][i_3+i_2][i_3] = G[i_3+i_1][i_4][i_2+i_1][i_2] * B[i_1+1][i_4][i_3+i_2-1][i_3+1] + B[i_1+1][i_4][i_3+i_2+2][i_3] * 2;
                }
            }
        }
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            C[i_1+1][i_5+1] = H[i_1][i_5] + I[i_5][i_5] + F[i_1] * C[i_1+3][i_5] - C[i_5+1][i_1-1] + C[i_1-1][i_5+2] - C[i_1-1][i_5+1] * C[i_1+1][i_5-1] + 4;
        }
    }
    for (int i_6 = 2; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 1; i_7 < -i_6+PB_M-1; i_7++) {
            for (int i_8 = 3; i_8 < min(i_6, -i_6+PB_L-2); i_8++) {
                D[i_8+i_6-1][i_7+i_6][i_7-1][i_8] = J[i_7][i_8-1][i_6-1][i_6] * K[i_7-1][i_6][i_7+1] - K[i_6][i_7][i_8] * D[i_7+i_6-2][i_8+i_6-2][i_8-3][i_7-1] + D[i_7+i_6+1][i_8+i_6+2][i_8-1][i_7] * D[i_7+i_6+1][i_8+i_7+2][i_8-1][i_6] - 3;
            }
        }
    }
#pragma endscop
