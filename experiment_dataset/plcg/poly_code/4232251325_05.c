...
void kernel_4232251325_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_1][i_0+1][i_0] = C[i_1][i_0] * A[i_1][i_0+2][i_0+1] * A[i_1][i_0+3][i_0+2] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q-1; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_P+1, PB_M-1); i_3++) {
            for (int i_4 = 3; i_4 < i_2; i_4++) {
                for (int i_5 = 1; i_5 < min(min(-i_3+PB_P+1, PB_Q-1), -i_4+PB_P+3); i_5++) {
                    B[i_2-1][i_5+i_3-1][i_3][i_4] = D[i_4-1][i_5][i_3] - E[i_3][i_2][i_5-1][i_4] - E[i_5][i_4][i_3-1][1] * D[i_5][i_3][i_4+1] - F[i_5][i_5][i_3] * G[i_3][i_4][i_2][i_5+1] + G[i_4][i_3-1][i_5][i_2+1] + E[i_3+1][i_2+1][i_5][i_4] - B[i_2-1][i_5+i_3-1][i_3][i_4-1] * B[i_2-1][i_5+i_4-3][i_4][i_3] + B[i_5-1][i_3+i_2-1][i_3][i_4-1] + B[i_4-3][i_5+i_3-1][i_5][i_2-2] - 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4232251325_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
