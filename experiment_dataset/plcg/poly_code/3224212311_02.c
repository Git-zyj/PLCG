...
void kernel_3224212311_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_2][i_0][i_1] = C[i_2][i_0][i_1] - 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_M-1, i_3); i_4++) {
            for (int i_5 = max(i_4, 1); i_5 < PB_M-1; i_5++) {
                B[i_4][i_3][i_5] = B[i_4][i_3-1][i_5-1] + B[i_4+1][i_5][i_3] * B[i_5+1][i_3][i_4] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3224212311_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
