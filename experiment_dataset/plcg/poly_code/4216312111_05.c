...
void kernel_4216312111_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < min(i_0, PB_L-1); i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
                    A[i_2][i_0][i_3+2] = A[i_2][i_3][i_1+1] - 6;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            B[i_4-2][i_4] = C[i_4-2][i_5] + B[i_4-3][i_4-1] - 5;
        }
    }
#pragma endscop
...
kernel_4216312111_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
