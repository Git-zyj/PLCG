...
void kernel_3224112225_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L, i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
                A[i_0+1][i_1][i_2-1] = C[i_1][i_0][i_0] + A[i_0-1][i_1][i_2-1] - A[i_0][i_1][i_2+1] + 5;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_M-4; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_M-4; i_4++) {
            B[i_3-2][i_4+i_3+2] = B[i_3][i_4+i_3+4] + B[i_3-3][i_4+i_3+1] + 6;
        }
    }
#pragma endscop
...
kernel_3224112225_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
