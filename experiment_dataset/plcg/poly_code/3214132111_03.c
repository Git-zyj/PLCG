...
void kernel_3214132111_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_0+2][i_1] = C[i_1+2][i_0] * D[i_1] - A[i_0+1][i_1-1] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < min(min(i_2, PB_L-2), -i_2+PB_M); i_3++) {
            B[i_3+1] = C[i_2-2][i_3+2] * C[i_3][i_2] * D[i_2-2] + D[i_3+i_2] * B[i_3] * 5;
        }
    }
#pragma endscop
...
kernel_3214132111_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
