...
void kernel_2332131121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_L, -i_0+PB_N); i_1++) {
            A[i_1+i_0][1] = D[i_1] * E[i_1-1][i_0+1] * E[i_0][0] + A[i_1+i_0-1][1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_M-1); i_3++) {
            A[i_3+i_2][1] = E[i_3+i_2+1][i_3] + E[i_2][i_3+1] + E[i_3][i_2+1] * E[i_2-1][i_3+1] * E[i_2][i_3+1] * 6;
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < PB_Q; i_5++) {
            C[i_4][i_5] = E[i_5][i_4-1] - C[i_4-2][i_5] + 5;
        }
    }
#pragma endscop
...
kernel_2332131121_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
