...
void kernel_2332232125_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        B[i_1] = E[i_1][i_1] * D[i_1+2] - B[i_1-1] * 2;
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        B[i_2+2] = D[i_2] + F[i_2] - D[i_2] + D[i_2+2] - 2;
    }
#pragma endscop
...
kernel_2332232125_05(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
