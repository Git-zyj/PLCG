...
void kernel_2216212213_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
            B[i_2+1] = C[i_2][i_0-1] - D[i_2+1] + B[i_2+2] - B[i_2] + 2;
        }
    }
#pragma endscop
...
kernel_2216212213_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
