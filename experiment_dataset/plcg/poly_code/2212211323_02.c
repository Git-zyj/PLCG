...
void kernel_2212211323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 3; i_1 < PB_M-2; i_1++) {
            A[i_1-1][i_1] = A[i_0+1][i_0+2] * A[i_1+1][i_1+2] + A[i_1-3][i_1-2] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 3; i_3 < -i_2+PB_M; i_3++) {
            A[i_3-3][i_3-2] = C[i_3][i_2] * C[i_3+i_2][i_2] + 2;
        }
    }
#pragma endscop
...
kernel_2212211323_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
