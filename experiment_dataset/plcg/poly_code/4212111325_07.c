...
void kernel_4212111325_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_M-2); i_1++) {
            A[i_1][i_0] = C[i_1-1][i_0] * A[i_1+2][i_0] * A[i_1][i_0+1] + 4;
        }
    }
        ...
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
                for (int i_5 = 1; i_5 < PB_M; i_5++) {
                    B[i_3+1][i_4-1][i_5] = B[i_4+2][i_5-1][i_3] + B[i_4+2][i_3-1][i_5] - B[i_3+1][i_4-2][i_5] - B[i_5-1][i_4-2][i_2] * 3;
                }
            }
        }
    }
...
kernel_4212111325_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
