...
void kernel_3234311221_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 3; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_L, i_3); i_4++) {
                B[i_2][i_3-1][i_2] = C[i_2][i_4+i_3] * B[i_2][i_3-3][i_2] * B[i_2-1][i_3-1][i_2-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3234311221_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
