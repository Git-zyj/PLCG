...
void kernel_2214312313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0-2] = C[i_0+1] + B[i_0+1] + 3;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        B[i_1] = A[i_1-2] - B[i_1-1] * B[i_1+1] + B[i_1+1] * 5;
    }
#pragma endscop
...
kernel_2214312313_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
