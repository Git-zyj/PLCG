...
void kernel_3212311215_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1] = C[i_1+i_0][0] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_Q, -i_2+PB_L-3); i_3++) {
            for (int i_4 = 0; i_4 < PB_Q; i_4++) {
                B[i_3+i_2+1][i_4][0] = B[i_3+i_2+1][i_4+1][0] + B[i_3+i_2][i_4][0] * B[i_3+i_2+3][i_4][0] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3212311215_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
