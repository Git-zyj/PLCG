...
void kernel_2432232313_07(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        D[i_0+2][i_0] = E[i_0+1] - F[i_0-1] + E[i_0] + D[i_0+1][i_0-1] - 1;
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            B[i_0][i_1-2] = G[i_0][0] + H[i_1+i_0][0] - H[i_0-2][i_1+2] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        D[i_2+3][i_2+1] = F[i_2] * 6;
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M, PB_P-1); i_4++) {
            D[i_3+2][i_3] = I[i_3][i_4][0] * H[i_4+2][i_3] - G[i_4+i_3][i_4] - H[i_4+i_3][i_4+2] + G[i_4][i_3] - B[i_3][i_4-2] * B[i_3][i_4-1] * B[i_3-1][i_4-2] + D[i_4+3][i_4+1] - D[i_4+1][i_4-1] * 4;
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2432232313_07(xd,yd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
