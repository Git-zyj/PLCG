...
void kernel_3312332221_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] - E[i_0] * F[i_0+1] + E[i_0-1] * E[i_0] * D[i_0] * A[i_0+2] - 1;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            B[i_1-1] = G[i_0][i_1] - A[i_0-2] + 3;
        }
    }
    for (int i_2 = 4; i_2 < PB_Q-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
                C[i_2-2][i_4][i_2] = H[i_4+1][i_2][i_4+2] + H[i_4][i_3][i_2] - C[i_2-4][i_4-2][i_2-2] + C[i_2-1][i_4][i_2+1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3312332221_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
