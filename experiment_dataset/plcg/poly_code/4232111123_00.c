...
void kernel_4232111123_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L+2; i_1++) {
            for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
                B[i_2-1][i_0+1][i_1-2] = 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L+1; i_4++) {
            for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
                B[i_5+1][i_3+1][i_4-1] = C[i_5+1][i_4] * B[i_5+1][i_3][i_4-1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4232111123_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
