...
void kernel_2224112323_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_Q, i_2); i_3++) {
            B[i_3+1] = C[i_2+2][i_3] - B[i_3-1] + B[i_3+2] * 3;
        }
    }
#pragma endscop
...
kernel_2224112323_09(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
