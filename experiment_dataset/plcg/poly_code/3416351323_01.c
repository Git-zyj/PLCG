...
void kernel_3416351323_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + F[i_0] + F[i_0] * E[i_0] + E[i_0-1] * F[i_0+1] + E[i_0+1] * E[i_0] - E[i_0] * A[i_0-2] * A[i_0+2] - 5;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_N, PB_L+1); i_2++) {
            B[i_2+i_1][i_1+1] = G[i_2][i_1] + G[i_2][i_1-1] - H[i_1-1][i_2-1] * B[i_2+i_1-2][i_1] + B[i_2+i_1-1][i_2+1] * B[i_2+i_1-1][i_1+1] - B[i_2+i_1-2][i_1-1] * B[i_2+i_1][i_1] * 6;
        }
        for (int i_3 = i_1; i_3 < min(PB_L, -i_1+PB_N+1); i_3++) {
            B[i_3+i_1-1][i_1+1] = I[i_1][i_3] - I[i_3][i_1+1] - H[i_1][0] * H[i_1-1][i_3] + I[i_3][i_1] + 3;
            for (int i_4 = 0; i_4 < i_3; i_4++) {
                D[i_4+1][i_3+i_1][i_3-1] = J[i_3][i_1][i_4] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3416351323_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
