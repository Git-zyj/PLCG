...
void kernel_2222152223_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            B[i_0][i_1] = C[i_0] + D[i_1][i_0] + D[i_0+2][i_1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
            B[i_3][i_2] = D[i_2][i_3-2] - D[i_3][i_2+2] + D[i_3][i_2+2] * C[i_3-2] - D[0][i_2] + C[i_3] + D[i_3][i_2] - B[i_3][i_2+1] + B[i_3+1][i_2] - 4;
        }
    }
#pragma endscop
...
kernel_2222152223_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
