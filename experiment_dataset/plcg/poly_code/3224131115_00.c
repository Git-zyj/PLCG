...
void kernel_3224131115_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[1] = C[i_1-1][i_0-1][i_0+1] * C[i_2][i_0+1][i_1] * C[i_0][i_2+1][i_1] - C[i_1+1][0][i_0] + B[1] - 1;
            }
            B[1] = D[i_1+1][i_0] * D[i_1][i_0-1] * 3;
        }
    }
#pragma endscop
...
kernel_3224131115_00(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
