...
void kernel_3414251221_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = PB_M - 1; i_1 >= 0; i_1--) {
            B[i_0-1][0] = E[i_0][i_1] - 3;
        }
        for (int i_2 = 1; i_2 < min(-i_0+PB_L, i_0); i_2++) {
            B[i_0][0] = F[i_2][i_0+1] - E[i_2][i_0] - E[i_0+1][i_0-1] * F[i_2+i_0][0] - B[i_2-1][0] * 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_Q-2; i_4++) {
            for (int i_5 = 2; i_5 < PB_M; i_5++) {
                C[i_3][i_5-1][i_4+i_3] = G[i_4-1][i_3-1][i_3] + G[i_4][i_3][i_5] * F[i_3][i_4-1] - G[i_5][i_4][i_3] - G[i_4][i_3-1][i_5] * G[i_5][i_4][i_3] + G[i_4+1][i_3][i_3] + C[i_3][i_5-1][i_4+i_3+2] - C[i_3][i_5-2][i_4+i_3] * 4;
            }
        }
        B[i_3][0] = H[i_3] - H[i_3-1] - I[i_3] + I[i_3] * I[i_3] + I[i_3+1] - 4;
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3414251221_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
