...
void kernel_3114232315_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            A[i_1] = B[i_0-1][i_1] * C[i_0-2][i_1+2] * B[i_1+2][i_0] * A[i_1+1] + A[i_1-1] - A[i_1+1] + 5;
        }
    }
#pragma endscop
...
kernel_3114232315_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
