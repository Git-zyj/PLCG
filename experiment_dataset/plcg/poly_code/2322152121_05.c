...
void kernel_2322152121_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1-2] = D[i_1+2][i_0] - D[i_1][i_0] - D[i_1-1][i_0] + E[i_1] + E[i_1+2] - D[i_1+1][i_0] - D[i_1][i_0-2] + E[i_1+i_0] - E[i_0] - D[i_0][i_1] - A[i_0+2][i_1-2] * A[i_0][i_1-1] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_2][i_3-1] = E[i_2] + D[i_2-1][i_3+2] + B[i_2-2][i_3-1] - 6;
        }
    }
    for (int i_4 = 2; i_4 < PB_Q; i_4++) {
        for (int i_5 = 0; i_5 < PB_M; i_5++) {
            C[i_5] = D[i_5+1][i_4] * D[i_4-2][i_5] - D[i_4-1][1] - 4;
        }
    }
#pragma endscop
...
kernel_2322152121_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
