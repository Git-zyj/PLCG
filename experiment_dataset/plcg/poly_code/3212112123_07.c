...
void kernel_3212112123_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_M; i_4++) {
            B[i_3][i_4] = C[i_3+1] - D[i_4][i_3] - B[i_3][i_4-2] + 1;
        }
    }
#pragma endscop
...
kernel_3212112123_07(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
