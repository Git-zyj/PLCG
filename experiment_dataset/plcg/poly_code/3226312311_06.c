...
void kernel_3226312311_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P, i_0); i_1++) {
            A[i_0+1][i_1-1] = A[i_0+2][i_1] + A[i_0+2][i_1-1] - 1;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        B[i_2-2] = C[i_2] + B[i_2-1] * B[i_2-3] + B[i_2-1] + 5;
    }
#pragma endscop
...
kernel_3226312311_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
