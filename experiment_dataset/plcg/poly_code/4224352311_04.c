...
void kernel_4224352311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,int wb,DATA_TYPE POLYBENCH_5D(B,xB,yB,zB,vB,wB,xb,yb,zb,vb,wb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-3; i_1++) {
            A[i_1+2][i_1] = C[i_1+2][i_0-1] + C[i_1-2][i_0] * C[i_0+2][i_0] - A[i_1+3][i_1+1] + A[i_1+1][i_1-1] * A[i_0+1][i_0-1] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_L; i_3++) {
            for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
                for (int i_5 = 2; i_5 < min(min(-i_4+PB_M-1, i_2), -i_2+PB_P); i_5++) {
                    B[i_4][i_3][i_3][i_4][i_2+2] = D[i_2-1][i_3][i_4][i_5] * E[i_5][i_4][i_2][i_3+1][0] - F[i_4+2][i_5][i_2-2][i_5+i_3-1] - D[i_5+i_4+1][i_3+i_2][i_3][i_5+2] + E[i_5+i_4][i_5-2][i_2+1][i_3][i_4] * G[i_3][i_4+1][i_2-2][i_5][0] + F[i_4][i_5+i_2][i_5-2][i_3+2] * B[i_4+1][i_3-1][i_3-1][i_4+1][i_2+1] + B[i_2-1][i_3-1][i_3-1][i_2-1][i_4+2] * 1;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,wb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4224352311_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,wb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,wb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G)));
