...
void kernel_3116311313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+1; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M+2, PB_M-1); i_1++) {
            for (int i_2 = i_1; i_2 < -i_0+PB_M+1; i_2++) {
                A[i_1][i_2+i_0-1] = B[i_1+1][1] * A[i_1-1][i_2+i_0-1] - A[i_2][i_1+i_0-2] * A[i_1-1][i_2+i_0-2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3116311313_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
