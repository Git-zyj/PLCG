...
void kernel_4316232223_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            A[i_0][i_1] = B[i_0][i_0] * 5;
        }
        for (int i_2 = 1; i_2 < i_0; i_2++) {
            B[i_0][i_0] = D[i_2][i_0-1] + E[i_2] * F[i_2][i_0] - F[i_2][i_0] - B[i_0-2][i_0-2] + B[i_2-1][i_2-1] * 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
            C[i_4+1][i_3] = D[i_3][i_4-2] - G[i_4] * F[i_4-1][i_3] + D[i_3-2][i_4] - C[i_4+2][i_3] - C[i_4+1][i_3-2] + C[i_4+1][i_3+1] - 6;
        }
    }
#pragma endscop
...
kernel_4316232223_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
