...
void kernel_3222152325_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M+4, PB_L); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M+2, PB_L+1); i_2++) {
                A[i_2+i_0-2][i_1][i_1] = C[i_1][i_0][i_1] * A[i_2+i_0-3][i_1-2][i_1-2] + A[i_1+i_0-4][i_2-1][i_2-1] - 2;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_L; i_3++) {
        for (int i_4 = 3; i_4 < min(-i_3+PB_M, PB_L); i_4++) {
            B[i_4-1][i_3-1] = D[i_4-1][i_3] + D[i_4][i_3] - D[i_3][i_3-1] * D[i_4][i_4-1] + E[i_4+i_3] * D[i_3-2][i_4] + D[i_4][i_3] - D[i_3+2][i_4] + D[i_4][i_3-2] + B[i_4-2][i_3-1] + B[i_4-1][i_3-3] + B[i_4-3][i_3-1] * 4;
        }
    }
#pragma endscop
...
kernel_3222152325_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
