        ...
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
                    B[i_3-1] = D[i_3][i_2][0][i_1-1] + E[i_3+1][i_1-1][i_2+1] - 2;
                }
            }
        }
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            B[i_4-1] = F[i_0][i_4] * G[i_4][i_0-1] - B[i_4-2] * B[i_4] - 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 1; i_6 < -i_5+PB_L-1; i_6++) {
            for (int i_7 = 0; i_7 < i_5; i_7++) {
                B[i_5-1] = F[i_7+i_6-1][i_6+i_5+1] + E[i_7+i_6][i_5+1][i_5] - H[i_6][i_7+1][i_5] * B[i_7] * 4;
            }
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4332231313_00(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
