...
void kernel_4224251315_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M+1, PB_L-2); i_1++) {
            A[i_0][i_1+1] = C[i_0+1][i_0] - D[i_0][i_0] - C[i_1][i_0+1] + E[i_1] + C[i_1+i_0-1][i_0] * A[i_0-1][i_1+1] - A[i_0][i_1+2] * A[i_0-1][i_1+2] + 1;
        }
    }
        ...
        for (int i_3 = i_2; i_3 < -i_2+PB_P; i_3++) {
            for (int i_4 = 1; i_4 < PB_Q-1; i_4++) {
                for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                    B[i_3][i_4][i_5] = F[i_5][i_3][i_2][i_4] + G[i_3][i_2+1][i_4+1][i_5-1] + G[i_4-1][i_5][i_4-1][i_3] * F[i_5][i_5][i_3+i_2][i_4] - F[i_4-1][i_5+1][i_3][i_2] + B[i_3][i_4+1][i_5] - B[i_3][i_5][i_4-1] + B[i_4][i_5+1][i_3] + 5;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4224251315_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
