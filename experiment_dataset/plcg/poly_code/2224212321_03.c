...
void kernel_2224212321_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_M-3, i_1); i_2++) {
            A[i_2+3] = C[i_2] - A[i_2+1] - A[i_2+3] * 3;
        }
    }
#pragma endscop
...
kernel_2224212321_03(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
