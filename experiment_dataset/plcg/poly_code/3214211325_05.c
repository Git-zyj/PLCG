...
void kernel_3214211325_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            for (int i_5 = 3; i_5 < PB_L-2; i_5++) {
                B[i_5-1][i_5] = C[i_5][i_3+1][i_4] + B[i_5-2][i_5-1] - B[i_5+1][i_5+2] - B[i_5-3][i_5-2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3214211325_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
