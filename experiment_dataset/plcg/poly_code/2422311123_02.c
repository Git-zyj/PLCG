...
void kernel_2422311123_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = 5;
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            A[i_1-1] = E[i_1-1][i_0-1] + A[i_0] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_N; i_3++) {
            C[i_3] = C[i_3-2] * 1;
            D[i_3+i_2][i_3] = C[i_3] + 2;
        }
    }
#pragma endscop
...
kernel_2422311123_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
