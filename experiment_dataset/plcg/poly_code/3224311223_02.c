...
void kernel_3224311223_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_N-1, i_1); i_2++) {
            for (int i_3 = 1; i_3 < PB_N+1; i_3++) {
                B[i_2][i_1][i_1] = C[i_3-1][i_1][i_1] + B[i_2+1][i_1][i_1] - B[i_2-1][i_1][i_1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3224311223_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
