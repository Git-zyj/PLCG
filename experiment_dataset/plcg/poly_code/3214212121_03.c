...
void kernel_3214212121_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_P+4; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 4; i_2 < PB_P; i_2++) {
                A[i_1-1][i_2-2] = C[i_1-2][i_2] + A[i_1-1][i_0-4] - A[i_1-1][i_2-4] * 1;
            }
        }
        B[i_0-2] = 4;
    }
...
kernel_3214212121_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
