...
void kernel_3324212225_09(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                C[i_2+1][i_2+1] = D[i_2][i_1+2][i_0+1] * B[i_2] - 6;
            }
            B[i_1] = 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
            C[i_4][i_4] = E[i_3][i_4] * C[i_4-2][i_4-2] - C[i_4+2][i_4+2] + 1;
        }
    }
#pragma endscop
...
kernel_3324212225_09(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
