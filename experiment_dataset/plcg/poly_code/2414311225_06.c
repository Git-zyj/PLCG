...
void kernel_2414311225_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    for (int i_0 = PB_L-1 - 1; i_0 >= 2; i_0--) {
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_L-1; i_1++) {
            B[i_1-1] = C[i_1+i_0] * 4;
            C[i_1+i_0] = B[i_1-1] + C[i_1+i_0+1] * 4;
        }
        B[i_0-2] = E[i_0+1] * B[i_0-2] - 5;
    }
#pragma endscop
...
kernel_2414311225_06(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E));
