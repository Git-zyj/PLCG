...
void kernel_4424312313_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L+1; i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                A[i_1+i_0-1][i_0][i_2+1] = A[i_2+i_1-1][i_2][i_0] + A[i_2+i_1-2][i_2-1][i_0+1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        B[i_3+1] = E[i_3] - B[i_3+2] - 3;
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 1; i_6 < min(-i_5+PB_L, PB_L-2); i_6++) {
                C[i_6][i_4+2] = A[i_6+i_5][i_5+1][i_4+1] + C[i_6-1][i_5+2] + C[i_6-1][i_5+1] * C[i_5+1][i_6+2] - 2;
            }
        }
        D[i_4+2] = E[1] - D[i_4+1] + D[i_4+3] + D[i_4+1] - 1;
    }
#pragma endscop
...
kernel_4424312313_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
