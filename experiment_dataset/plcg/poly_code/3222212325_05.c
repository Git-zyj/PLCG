...
void kernel_3222212325_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 2; i_2 < PB_N+2; i_2++) {
                B[i_2-2][i_1] = C[i_1-1][i_0][i_2] - 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N+1; i_3++) {
        for (int i_4 = 2; i_4 < PB_N+2; i_4++) {
            B[i_3-1][i_4] = B[i_4-2][i_3] + B[i_3-2][i_4] - B[i_3-1][i_4+2] * 3;
        }
    }
#pragma endscop
...
kernel_3222212325_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
