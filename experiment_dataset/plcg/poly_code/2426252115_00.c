...
void kernel_2426252115_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0-2] + E[i_0] + F[i_0+1] * E[i_0] - E[i_0-2] + E[0] - A[i_0+1] * 1;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < min(min(-i_1+PB_M, PB_M-3), i_1); i_2++) {
            B[i_2+2][i_2][i_1] = E[i_2] - G[i_2+i_1][0] + H[i_1+1][i_2] * H[i_1][i_2] * G[i_2][i_1] * H[i_2+1][i_1] * B[i_2+3][i_2+1][i_1] * 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        A[i_3-1] = E[i_3] + F[i_3+2] + E[i_3] + 4;
        for (int i_4 = 2; i_4 < i_3; i_4++) {
            A[i_3+1] = H[i_4][i_3+1] - H[i_4][i_4] - H[i_4-2][i_3] * G[i_4][i_3+1] * 5;
        }
    }
#pragma endscop
...
kernel_2426252115_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
