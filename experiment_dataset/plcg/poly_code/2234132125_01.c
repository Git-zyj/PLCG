...
void kernel_2234132125_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_1] = C[i_1] + D[i_1][i_0+2] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_P, -i_2+PB_M-2); i_3++) {
            B[i_2+1] = D[i_3+i_2+2][0] + D[i_3-1][i_2-2] * D[i_3][i_2] + B[i_2+2] + 6;
        }
    }
#pragma endscop
...
kernel_2234132125_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
