...
void kernel_4424131313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        A[i_0+1] = E[i_0+1] * A[i_0] - A[i_0+2] * A[i_0+2] * A[i_0] - 3;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_N-1); i_1++) {
            A[i_1+1] = E[i_1] + F[i_1+i_0][i_1-1] * F[i_1+1][i_0+1] * A[i_1+1] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_M, PB_M-2); i_4++) {
                for (int i_5 = i_3; i_5 < min(-i_3+PB_M, -i_2+PB_Q); i_5++) {
                    C[i_3+1][i_4][i_5-1] = G[i_4+i_3][i_5][i_3][i_2] + G[i_4-1][i_3+1][i_5+i_2][i_4] - G[i_5+i_3][i_4][i_2][i_3] - G[i_3][i_5-1][i_2][i_4] * C[i_4+2][i_5+1][i_3-1] + C[i_4+2][i_5][i_3-1] * 2;
                }
            }
        }
        D[i_2] = E[i_2] * E[i_2+1] * E[i_2-1] * D[i_2+1] - 6;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4424131313_02(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
