...
void kernel_3226132221_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                B[i_1-2] = C[i_1][i_1][i_2-1] - C[i_0][i_2-2][i_1+1] - D[i_2][i_2] * C[i_1][i_0][i_2] * 1;
            }
        }
        for (int i_3 = max(i_0, 4); i_3 < PB_M; i_3++) {
            B[i_3-2] = D[i_3][i_0] * B[i_3] + B[i_3-4] * 5;
        }
    }
#pragma endscop
...
kernel_3226132221_06(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
