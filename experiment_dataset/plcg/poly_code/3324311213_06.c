...
void kernel_3324311213_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
                A[i_1][i_2-1] = A[i_2][i_1] * A[i_1][i_2-2] + 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q; i_5++) {
                B[i_4][i_3][i_5] = D[i_3+1][i_4][i_5] * B[i_4][i_3][i_5+1] - 4;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M+1; i_6++) {
        for (int i_7 = 1; i_7 < PB_L+1; i_7++) {
            for (int i_8 = 0; i_8 < i_7; i_8++) {
                C[i_7][i_8+1] = A[i_6-1][i_7-1] + C[i_8+1][i_7] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3324311213_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
