...
void kernel_3312312213_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                A[i_1+2][i_0-1][i_0][i_2] = D[i_2-1][i_2][i_0] + A[i_0+2][i_1-1][i_1][i_2+1] * A[i_1+1][i_0-2][i_0-1][i_2+1] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                B[i_4][i_5] = E[i_4-2][i_5][i_3-1][i_3] - B[i_5-1][i_4] * B[i_4+1][i_5] * 4;
            }
            C[i_4][i_3] = C[i_4+1][i_3] + C[i_4-1][i_3-1] + 6;
        }
    }
#pragma endscop
...
kernel_3312312213_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
