...
void kernel_4214111125_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            B[i_0][i_1+1] = B[i_1-1][i_0] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_M, i_2); i_3++) {
            B[i_2-1][i_3+1] = C[i_3][i_2-1] - 6;
        }
    }
#pragma endscop
...
kernel_4214111125_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
