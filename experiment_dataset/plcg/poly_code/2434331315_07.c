...
void kernel_2434331315_07(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        C[i_0] = E[i_0] + C[i_0+1] - C[i_0] - 2;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        C[i_1+1] = E[i_1] - 1;
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        C[i_2] = F[i_2+1] + G[i_2][i_2] * H[i_2-1] * C[i_2+1] * 1;
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            D[i_3][i_4] = I[i_4][i_3+1] - G[i_4][0] + J[i_4][i_3] * G[i_4][i_3] - C[i_4] * D[i_3-1][i_4] * 1;
        }
    }
#pragma endscop
...
kernel_2434331315_07(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
