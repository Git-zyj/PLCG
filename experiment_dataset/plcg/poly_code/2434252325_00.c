...
void kernel_2434252325_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + F[i_0][i_0+1] - G[i_0][i_0-2] - E[i_0] + F[i_0-1][i_0] - H[0] * A[i_0-1] - A[i_0-2] - A[i_0+2] + 4;
        for (int i_1 = 2; i_1 < -i_0+PB_M-1; i_1++) {
            D[i_1][i_0+1] = G[i_1+i_0+1][0] + F[i_1+1][i_0] + F[i_1-2][i_0] * H[i_0+1] - G[i_1-1][0] - A[i_1] - D[i_1][i_0-1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            A[i_3] = I[i_2][i_3][i_2] + F[i_3][i_2] - G[i_3][i_2] - 2;
        }
        for (int i_4 = max(2, i_2); i_4 < min(-i_2+PB_L, PB_M-2); i_4++) {
            D[i_2][i_4+1] = F[i_4+i_2-1][1] + F[i_2+2][i_4+2] - G[i_2-1][i_4-2] + F[i_4+i_2][0] + G[i_2][i_4+2] * G[i_4-1][2] * D[i_2][i_4] - 6;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2434252325_00(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
