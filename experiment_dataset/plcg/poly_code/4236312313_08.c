...
void kernel_4236312313_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L-1; i_1++) {
            A[i_0][i_1+i_0] = A[i_0+1][i_1+i_0+1] * A[i_0][i_1+i_0-1] + A[i_0-1][i_1+i_0-1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            for (int i_4 = i_3; i_4 < PB_M; i_4++) {
                B[i_2][i_3][i_4-1] = C[i_2-1][i_3][0] * B[i_4][i_3-1][i_2-1] - B[i_2][i_3+1][i_4-1] - B[i_2+1][i_3+1][i_4-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4236312313_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
