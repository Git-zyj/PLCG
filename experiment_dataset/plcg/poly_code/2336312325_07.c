...
void kernel_2336312325_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_1][i_0] = D[i_1] + D[i_1] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_P-2, i_2); i_3++) {
            B[i_2][i_3] = B[i_2-2][i_3+2] - B[i_2-1][i_3] - B[i_2][i_3+1] * 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_P-2; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            C[i_5][i_4] = C[i_5][i_4+2] + C[i_5+2][i_4] - C[i_5-1][i_4] - 1;
        }
    }
#pragma endscop
...
kernel_2336312325_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
