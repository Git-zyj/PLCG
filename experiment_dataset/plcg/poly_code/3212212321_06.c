...
void kernel_3212212321_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_0+2][i_1-2][i_1-1] = C[i_2][i_0-2] * 5;
            }
        }
        for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_0+2][i_3][i_3+1] = D[i_0][i_4][i_4] * B[i_0+1][i_3][i_3+1] - B[i_0+2][i_3-2][i_3-1] + B[i_0+4][i_3][i_3+1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3212212321_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
