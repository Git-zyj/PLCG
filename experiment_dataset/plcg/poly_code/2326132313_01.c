...
void kernel_2326132313_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0+2][i_1-1] = D[i_1][i_0] * D[i_1-1][i_0] * A[i_0+1][i_1-1] * A[i_0+3][i_1-1] - A[i_0+2][i_1] + 5;
        }
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            B[i_0-2][i_2] = D[i_2][i_2] - D[i_2][i_0-1] + D[i_2-1][0] + B[i_0-1][i_2+1] + B[i_0-1][i_2-1] - 3;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(min(-i_3+PB_M, PB_L-1), i_3); i_4++) {
            C[i_3] = D[i_3][i_4+i_3] + D[i_4][i_3+2] + D[i_4-1][0] - C[i_4-1] + C[i_4+1] - 6;
        }
    }
#pragma endscop
...
kernel_2326132313_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
