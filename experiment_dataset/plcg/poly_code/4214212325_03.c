...
void kernel_4214212325_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_N+2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_N+2, i_0); i_2++) {
                A[i_0-2][i_2][i_1+2] = C[i_2+i_1-2][i_0+2][0] * D[i_2][i_0][i_0] * A[i_0-2][i_2][i_1] - A[i_0-2][i_1][i_2+1] - 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        B[i_3] = B[i_3-1] - B[i_3+2] - B[i_3+1] * B[i_3-2] + 5;
    }
#pragma endscop
...
kernel_4214212325_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
