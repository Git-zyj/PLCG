...
void kernel_3212312321_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_1][i_2] = C[i_2][i_1] * A[i_1+1][i_2] + A[i_1-2][i_2] - A[i_0+1][i_2] - A[i_1][i_2+1] - 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < min(min(-i_4+PB_L, -i_3+PB_M), i_3); i_5++) {
                B[i_5+i_3][i_5+i_4][i_4] = B[i_5+i_3-1][i_5+i_4][i_4] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3212312321_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
