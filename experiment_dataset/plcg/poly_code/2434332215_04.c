...
void kernel_2434332215_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            A[i_1] = E[i_0] - F[i_1] - G[i_1+1][i_0] - A[i_1+1] - 2;
        }
        for (int i_2 = 1; i_2 < i_0; i_2++) {
            A[i_2-1] = H[i_2][i_0+1] - I[i_2][i_0] - E[i_2] + A[i_0] + 5;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            C[i_4] = E[i_3] * F[i_4+2] + J[i_3] - C[i_4-1] * 2;
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        D[i_5+1] = E[i_5] - A[i_5] + D[i_5] * D[i_5+2] - 5;
    }
#pragma endscop
...
kernel_2434332215_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
