...
static void print_array(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,int zl,int vl,int wl,DATA_TYPE POLYBENCH_5D(L,xL,yL,zL,vL,wL,xl,yl,zl,vl,wl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn),int xo,DATA_TYPE POLYBENCH_1D(O,xO,xo))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4432351125_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,int zl,int vl,int wl,DATA_TYPE POLYBENCH_5D(L,xL,yL,zL,vL,wL,xl,yl,zl,vl,wl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn),int xo,DATA_TYPE POLYBENCH_1D(O,xO,xo)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - E[i_0] - D[i_0+1] + 6;
    }
    for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_N, -i_1+PB_L); i_2++) {
            for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                B[i_3+1][i_2-1] = F[i_2][i_2-1][i_1] - G[i_3][i_1][i_2] - H[i_2+i_1][i_2+i_1] + 3;
            }
        }
        for (int i_4 = 1; i_4 < i_1; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_L; i_5++) {
                for (int i_6 = 0; i_6 < min(min(i_1, -i_4+PB_P), -i_1+PB_P-1); i_6++) {
                    B[i_6+1][i_5-1] = I[i_4][i_5-1][i_6+i_1+1][i_5] * J[i_4][i_6][i_1][i_5] + K[i_6][i_6][i_4][i_6][i_1] - I[i_5-1][i_4-1][i_6][i_4] + G[i_6][i_5+i_4][i_1] - L[i_5][i_6+i_4][i_1][i_1][i_5] * 3;
                }
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_L-1; i_7++) {
        D[i_7] = M[i_7+1][i_7+1] + N[i_7-1] * M[i_7-1][0] + E[i_7+1] + N[i_7] * O[i_7] - O[i_7] + D[i_7-2] * 3;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,wl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N), xo,POLYBENCH_ARRAY(O), INIT_SEED);
kernel_4432351125_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,wl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N), xo,POLYBENCH_ARRAY(O));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,wl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N), xo,POLYBENCH_ARRAY(O)));
