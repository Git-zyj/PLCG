...
void kernel_4324331311_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        B[i_0][i_0] = D[i_0-1] * E[i_0+1] - E[i_0] + D[i_0-1] * B[i_0][i_0] * 5;
    }
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_N; i_2++) {
            B[i_1][i_1] = F[i_2][i_1] + G[i_2][i_1] + F[i_2][i_1] + B[i_1+1][i_1+1] + 5;
        }
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            for (int i_4 = 2; i_4 < PB_P-1; i_4++) {
                C[i_1-1][i_3][i_4] = H[i_4][i_3][i_4] - I[i_3][i_1][i_3] * B[i_3][i_3] - C[i_1-1][i_3-1][i_4] * C[i_4-2][i_1-1][i_3-1] * C[i_1][i_3-1][i_4+1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4324331311_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
