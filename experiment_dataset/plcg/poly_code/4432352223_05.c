...
void kernel_4432352223_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0] - F[i_0+1][2] * G[i_0-1] * A[i_0+1] - A[i_0-2] + A[i_0-1] * 1;
    }
    for (int i_1 = 0; i_1 < PB_Q-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
            for (int i_3 = 2; i_3 < PB_L; i_3++) {
                for (int i_4 = 2; i_4 < PB_L; i_4++) {
                    B[i_4][i_2-2][i_1] = H[i_1][1][i_4][i_2] + I[i_4][i_2+1][i_1+2][i_3] - B[i_2+1][i_3-2][i_1] - B[i_4-2][i_3][i_1] * 4;
                }
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 2; i_6 < PB_L; i_6++) {
            C[i_5][i_6-2] = F[i_6-1][0] * J[i_5][i_6+1][i_5] * G[i_5] + K[i_6][i_5] + K[i_5-2][i_6] + K[i_5][i_6] + E[i_6-2] - C[i_5+1][i_6-1] * D[i_6] + 2;
        }
    }
    for (int i_7 = 1; i_7 < PB_L; i_7++) {
        D[i_7] = G[i_7-1] * G[i_7] - L[i_7] * E[i_7] - G[i_7] + D[i_7-1] * 4;
    }
#pragma endscop
