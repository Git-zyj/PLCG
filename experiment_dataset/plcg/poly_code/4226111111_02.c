...
void kernel_4226111111_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M-1; i_2++) {
                A[i_0][i_1][i_2+1] = C[i_2][i_1] + C[i_2+1][i_1] * 3;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            for (int i_5 = i_3; i_5 < PB_Q-1; i_5++) {
                for (int i_6 = 0; i_6 < PB_M; i_6++) {
                    B[i_4+1][i_6][i_4+1] = A[i_4][i_6][i_5+1] * 5;
                }
            }
        }
    }
...
kernel_4226111111_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
