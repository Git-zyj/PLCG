...
void kernel_3412132125_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[i_0-2] * E[i_0] - E[i_0] * E[i_0+2] * E[i_0] + A[i_0] + 1;
    }
    for (int i_1 = 2; i_1 < PB_Q-1; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_Q+2, PB_Q-2); i_2++) {
            D[i_1-1][i_1] = F[i_1][i_2-2] + A[i_2+1] - 5;
            for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                D[i_3][i_3+1] = G[i_2+2][i_3+1][i_1] + G[i_3][i_1-1][i_2+i_1-2] * 6;
            }
            for (int i_4 = i_2; i_4 < PB_P-1; i_4++) {
                D[i_4][i_4+1] = G[i_4][i_1][i_1] + H[i_1][i_4][i_2+2][i_1+1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3412132125_06(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
