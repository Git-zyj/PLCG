...
void kernel_3324312325_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            A[i_1-1][i_0] = D[i_0-2][i_1+1] * A[i_1][i_0] - A[i_1-1][i_0+1] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        B[i_2] = B[i_2-1] - B[i_2-2] * B[i_2+2] * 2;
    }
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_L-3, i_3); i_4++) {
            for (int i_5 = 1; i_5 < min(-i_3+PB_M, PB_L-2); i_5++) {
                C[i_3][i_4+1][i_5+1] = D[i_5+i_3][i_4-2] - D[i_5+1][i_4-1] * C[i_3+1][i_5+2][i_4+2] * C[i_3][i_4+3][i_5+1] * C[i_5-1][i_4+3][i_3+1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3324312325_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
