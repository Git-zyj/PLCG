...
void kernel_3422311223_07(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L+1; i_1++) {
            for (int i_2 = PB_N-1 - 1; i_2 >= 0; i_2--) {
                D[i_2+1][i_2][i_1+i_0-1] = 4;
            }
            C[i_0] = C[i_1] - 3;
        }
        ...
        for (int i_3 = 3; i_3 < -i_0+PB_L+1; i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_L+1; i_4++) {
                D[i_0+1][i_0][i_4+i_3-1] = D[i_4][i_4-1][i_3+i_0-1] + D[i_0-1][i_0-2][i_4+i_3-3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3422311223_07(xd,yd,zd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E));
