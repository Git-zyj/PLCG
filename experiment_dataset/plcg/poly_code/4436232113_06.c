...
void kernel_4436232113_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
        ...
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_N-2); i_2++) {
                for (int i_3 = 0; i_3 < i_2; i_3++) {
                    B[i_0][i_2] = E[i_3][i_3][i_0][i_1] - E[i_2+i_1][i_3][i_1][i_0] + F[i_2+2][i_3+2][i_1] - E[i_3+i_2][i_3][i_1][i_0] + 6;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < PB_L; i_6++) {
                B[i_4][i_6] = F[i_4][i_5][i_6] + G[i_6-1][i_5] + F[i_5][i_4+2][i_4] - B[i_4+1][i_5-1] * 1;
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_L; i_7++) {
        for (int i_8 = 0; i_8 < i_7; i_8++) {
            B[i_8][i_7] = G[i_7-2][i_8] + H[i_8] - 2;
        }
    }
    for (int i_9 = 2; i_9 < PB_M-2; i_9++) {
        for (int j_0 = i_9; j_0 < PB_M; j_0++) {
            for (int j_1 = i_9; j_1 < -j_0+PB_M; j_1++) {
                B[j_0][j_1] = F[j_0-1][i_9+2][j_1-2] + I[j_1+j_0][j_1][i_9-1] * F[j_0][i_9-2][j_1] + 5;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4436232113_06(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
