...
void kernel_2424252123_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0+1] = E[i_0-1] - E[i_0] * 6;
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            B[i_0][i_1] = E[1] - F[i_0+2][i_1] * F[i_1][i_0] * G[i_0][0] * F[i_0+1][0] * F[i_0][i_1-1] - B[i_1-2][i_0] * B[i_0-2][i_1] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
        A[i_2] = H[i_2] * E[i_2+2] * E[i_2-2] * H[i_2-1] + E[i_2-1] + H[i_2-2] - H[i_2-2] + E[i_2] + 5;
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        D[i_3] = E[i_3] + F[i_3][i_3] + D[i_3-1] - 6;
    }
#pragma endscop
...
kernel_2424252123_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
