...
void kernel_3416251223_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N-2; i_0++) {
        A[i_0-1][i_0+1] = E[i_0] * F[i_0] - E[i_0-1] * G[i_0][0] * A[i_0-2][i_0] - A[i_0-3][i_0-1] - C[i_0+2] + 2;
        B[i_0+1] = F[i_0] * F[i_0] - F[i_0] + E[i_0] + A[i_0-1][i_0+1] + B[i_0] + 6;
        C[i_0] = E[i_0+1] * F[0] * E[i_0] * F[i_0] - E[i_0] + E[i_0+1] * E[i_0+1] + F[1] + A[i_0][i_0+2] * C[i_0+1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
                D[i_3][1][i_1-1] = H[i_2][i_1-1][i_3] * H[i_2][i_3][i_2] + I[i_2][i_3][i_1-1] * I[i_2][i_3+1][i_1] + D[i_3-2][1][i_1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3416251223_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
