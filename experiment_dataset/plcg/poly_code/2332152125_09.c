...
void kernel_2332152125_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = D[i_0] + D[i_0-2] - D[i_0] + D[i_0+1] * D[i_0] * D[i_0] + D[i_0] * D[i_0-2] + 4;
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            B[i_0-2][i_1] = D[0] * E[i_0+2][i_1] - E[i_1+i_0][i_0] * E[i_1+2][i_0] + E[i_1][i_0] - B[i_0-1][i_1+2] + 3;
        }
    }
...
kernel_2332152125_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
