...
void kernel_2236211323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_N-1, i_0); i_1++) {
            A[i_1-1] = C[0] * A[i_1-3] * A[i_1+1] - 3;
        }
    }
    for (int i_2 = 3; i_2 < PB_N+1; i_2++) {
        A[i_2-1] = C[i_2] + A[i_2-3] * 2;
    }
#pragma endscop
...
kernel_2236211323_05(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
