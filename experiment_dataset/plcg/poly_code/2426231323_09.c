...
void kernel_2426231323_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        A[i_0+1] = E[i_0-1] - A[i_0-1] + A[i_0+3] + A[i_0] * 3;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        C[i_1] = E[i_1+1] - F[i_1] - E[i_1] * C[i_1+2] * C[i_1] - 4;
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            C[i_2] = G[i_2+i_1][i_2+1] + F[i_1] + H[i_2][i_1] - 2;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L+1; i_4++) {
            D[i_3][i_4] = H[i_3][i_4-1] * H[i_4][i_3] - D[i_3+2][i_4] * D[i_3][i_4+1] - 6;
        }
    }
#pragma endscop
...
kernel_2426231323_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
