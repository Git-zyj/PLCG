...
void kernel_4236132325_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N+1; i_1++) {
            B[i_1+i_0-1] = C[i_1+i_0-1][1] - C[i_1][0] - 3;
        }
    }
    for (int i_2 = 3; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N-1; i_3++) {
            B[i_3+i_2-1] = C[i_3][i_2] + C[i_3][i_2-1] + C[i_3+i_2][i_2] + B[i_3+i_2-2] - B[i_3+i_2-3] + B[i_3+i_2+1] - 3;
        }
    }
#pragma endscop
...
kernel_4236132325_07(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
