...
void kernel_4316332115_04(int xc,int yc,int zc,int vc,int wc,DATA_TYPE POLYBENCH_5D(C,xC,yC,zC,vC,wC,xc,yc,zc,vc,wc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M, i_0); i_2++) {
                C[i_0][i_2][i_1+1][i_1][i_2+i_0] = D[i_1][i_2+i_0][i_2][i_0] - 1;
            }
        }
        for (int i_3 = 2; i_3 < i_0; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
                for (int i_5 = 0; i_5 < min(-i_0+PB_P, PB_M-1); i_5++) {
                    C[i_0][i_5][i_4+2][i_4+1][i_5+i_0] = E[i_0][i_5][i_5+1][i_4] + F[i_0][i_4][i_3-2] * D[i_4][i_0][i_5][i_3] + 5;
                }
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_L; i_6++) {
        for (int i_7 = 1; i_7 < PB_L-2; i_7++) {
            for (int i_8 = 0; i_8 < min(min(i_6, -i_7+PB_P+1), PB_M-1); i_8++) {
                for (int i_9 = 1; i_9 < min(-i_7+PB_L-2, -i_6+PB_P); i_9++) {
                    C[i_6][i_9][i_8+1][i_8][i_9+i_6] = G[i_9][i_8][i_6][i_7] * H[i_9-1][i_7][i_9+1][i_6+2][i_6] - H[i_6-2][i_8][i_9+i_7+2][i_9][i_8] + D[i_8][i_9][i_6][i_7-1] + E[i_9-1][i_8+1][i_7+2][i_6] - C[i_7-1][i_8][i_9][i_9-1][i_8+i_7-1] + 1;
                }
            }
        }
    }
...
init_array(xc,yc,zc,vc,wc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4316332115_04(xc,yc,zc,vc,wc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,yc,zc,vc,wc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H)));
