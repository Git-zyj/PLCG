...
void kernel_3426212321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            A[i_1] = E[i_0][i_1] + A[i_1-1] + 2;
            B[i_0][i_1-1] = A[i_1] * A[i_1-1] * B[i_0-2][i_1-1] + 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
        C[i_2] = A[i_2] + D[i_2-1][i_2] * D[i_2+1][i_2+2] - 3;
        for (int i_3 = i_2; i_3 < min(-i_2+PB_M+2, PB_L+1); i_3++) {
            D[i_2-1][i_2] = E[i_3+i_2-2][i_2+1] + B[i_2][i_3-1] + C[i_3+2] * C[i_3+1] - C[i_3] * 5;
        }
    }
#pragma endscop
...
kernel_3426212321_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
