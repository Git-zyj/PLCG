...
void kernel_3334112323_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 3; i_1 < min(-i_0+PB_M-2, PB_L); i_1++) {
            A[i_1+i_0+1][i_1-1] = A[i_1+i_0-1][i_1-3] - A[i_1+i_0+2][i_1] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            A[i_3+i_2][i_3-1] = D[i_2][0] * 5;
        }
        for (int i_4 = i_2; i_4 < PB_L-2; i_4++) {
            C[i_4][i_4] = D[i_4+1][i_2-2] - D[i_4][i_2+1] * C[i_4-1][i_4-1] * C[i_4-2][i_4-2] * C[i_4+1][i_4+1] - C[i_4+2][i_4+2] + 1;
        }
    }
#pragma endscop
...
kernel_3334112323_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
