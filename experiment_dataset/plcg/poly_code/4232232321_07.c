...
void kernel_4232232321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0][i_1] = C[i_0][i_1] - C[i_1][i_0+1] + C[i_1][i_0-1] * A[i_1+i_0-2][i_1-2] + B[i_1+2] - 1;
        }
        for (int i_2 = 0; i_2 < min(PB_M-3, -i_0+PB_M+1); i_2++) {
            B[i_2+2] = C[i_2][i_0] - C[i_0][i_2] * C[i_2+2][i_0] + A[i_2+i_0-1][i_2] * B[i_2+3] + B[i_2+1] * 6;
        }
    }
#pragma endscop
...
kernel_4232232321_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
