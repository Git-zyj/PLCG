...
void kernel_4236312311_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd)){
        ...
        for (int i_2 = 3; i_2 < -i_1+PB_L; i_2++) {
            for (int i_3 = 2; i_3 < PB_M; i_3++) {
                for (int i_4 = 2; i_4 < min(PB_P-1, i_1); i_4++) {
                    B[i_2-2][i_4-2][i_4] = D[i_3][i_2+i_1][i_4-2][i_3-2] + B[i_2-3][i_4-1][i_4+1] - B[i_2-1][i_1-2][i_1] * B[i_1-2][i_3-1][i_3+1] + 5;
                }
            }
        }
    }
...
kernel_4236312311_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D));
