...
void kernel_2314331215_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            A[i_0][i_1] = D[i_1] * A[i_0+1][i_1] * B[i_1][i_1] - 3;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_0][i_0] = E[i_0+1][i_2] - F[i_2-1] + G[i_2][i_0+1] - B[i_0-1][i_0-1] * 3;
        }
    }
    for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
        B[i_3][i_3] = D[0] + F[i_3] - D[i_3] - D[i_3+1] - D[i_3+1] - 5;
    }
#pragma endscop
...
kernel_2314331215_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
