...
void kernel_4112132111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+2; i_0++) {
        for (int i_1 = 1; i_1 < min(min(-i_0+PB_M, i_0), PB_N+1); i_1++) {
            A[i_1-1][i_0+1] = B[0][i_1+i_0-1] + B[i_1+i_0][i_1] + B[i_1][i_0] + A[i_0-2][i_1+1] * 1;
        }
    }
#pragma endscop
...
kernel_4112132111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
