...
static void print_array(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4426252225_03(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        C[i_0-2][i_0] = E[i_0] + F[i_0+2] + F[i_0+1] + C[i_0-2][i_0] - 2;
        for (int i_1 = i_0; i_1 < -i_0+PB_P-2; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_P, -i_0+PB_P); i_2++) {
                for (int i_3 = 0; i_3 < i_2; i_3++) {
                    D[i_0][i_1-1][i_2-1][i_3] = G[i_0][i_3][i_2+i_1][0] - H[i_0-1][i_2][i_2][i_1][i_1] - I[i_1-1][i_3+2][i_2+i_0] - J[i_2][i_3][i_1+i_0+2] * G[i_1][i_3][i_1+1][i_2] - K[i_2-1][i_0][i_1+1][i_1] * 3;
                }
            }
        }
    }
    for (int i_4 = 4; i_4 < PB_Q-1; i_4++) {
        C[i_4-2][i_4] = E[i_4] - E[i_4] - F[i_4+1] - E[i_4] - C[i_4-4][i_4-2] - C[i_4-1][i_4+1] + 5;
    }
    for (int i_5 = 0; i_5 < PB_P; i_5++) {
        for (int i_6 = max(1, i_5); i_6 < PB_L; i_6++) {
            for (int i_7 = 2; i_7 < PB_L; i_7++) {
                for (int i_8 = i_7; i_8 < min(-i_6+PB_M, PB_P-2); i_8++) {
                    D[i_5][i_7][i_6-1][i_8] = G[i_8+i_6][i_7][i_5][i_7-2] * J[i_6][i_7][i_8] + K[i_5][i_7+1][i_8+2][i_6] + J[i_7][i_6][i_8] + 3;
                }
            }
        }
    }
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4426252225_03(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K)));
