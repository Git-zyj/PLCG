...
void kernel_3234211221_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][i_1] = C[i_0-1] - B[i_0][i_0-1][i_1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < PB_P+1; i_3++) {
            for (int i_4 = 0; i_4 < PB_Q; i_4++) {
                B[i_3][i_3-1][i_4] = B[i_2+1][i_2][i_4] + B[i_3-2][i_3-3][i_4] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3234211221_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
