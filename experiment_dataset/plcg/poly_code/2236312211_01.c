...
void kernel_2236312211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N-2; i_1++) {
            A[i_1+1] = A[i_1+2] - A[i_1] + A[i_1+2] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_P; i_3++) {
            B[i_2][i_3] = C[i_2][i_3-2] - B[i_2-1][i_3] - 6;
        }
    }
#pragma endscop
...
kernel_2236312211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
