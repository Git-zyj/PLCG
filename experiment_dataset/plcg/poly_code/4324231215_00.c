...
void kernel_4324231215_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
        ...
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
                    B[i_2][i_0][i_2] = D[i_2][i_0][i_3-1][i_1-1] * E[i_2][i_1+1][i_3] + 1;
                }
            }
            for (int i_4 = 0; i_4 < PB_Q-1; i_4++) {
                for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
                    B[i_1][i_5][i_1] = F[i_1+i_0][i_4][i_5][i_4+1] * G[i_5][i_4][i_4][i_1][i_0] * B[i_4][i_5+1][i_4] - B[i_5+1][i_1][i_5+1] * 1;
                }
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
        for (int i_7 = 0; i_7 < i_6; i_7++) {
            for (int i_8 = 0; i_8 < -i_7+PB_L; i_8++) {
                B[i_7+1][i_6+1][i_7+1] = H[i_8][i_6+1] - E[i_7+1][i_6][i_7] * I[0][i_7] + H[i_8+i_7][i_6+1] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4324231215_00(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
