...
void kernel_2222112323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-3; i_1++) {
            A[i_1+i_0+1] = A[i_1+i_0+3] + A[i_1+i_0+2] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-4; i_2++) {
        B[i_2+2] = C[i_2+1] * B[i_2+4] + B[i_2+1] + B[i_2+3] * 5;
    }
#pragma endscop
...
kernel_2222112323_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
