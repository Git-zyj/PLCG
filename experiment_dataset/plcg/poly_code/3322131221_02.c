...
void kernel_3322131221_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_N; i_1++) {
            A[i_0-1][i_1] = D[i_1][i_0] - E[i_1][i_0][i_1] + D[i_1][i_0] * D[i_1+i_0][0] + A[i_0-1][i_1-1] + A[i_0][i_1] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = i_2; i_3 < PB_Q-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_N; i_4++) {
                B[i_2-1][i_2][i_3] = D[i_4][i_2] + B[i_2-1][i_2][i_3+2] + B[i_2-2][i_2-1][i_3+1] + B[i_2-1][i_2][i_3-1] - 3;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_N; i_5++) {
        for (int i_6 = 0; i_6 < -i_5+PB_N; i_6++) {
            C[i_6+i_5-1] = D[i_6+1][i_5+1] - D[i_5][i_6] - D[i_6+i_5][i_6] + 6;
        }
    }
#pragma endscop
...
kernel_3322131221_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
