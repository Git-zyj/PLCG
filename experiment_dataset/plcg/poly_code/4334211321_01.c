...
void kernel_4334211321_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
        ...
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_Q-1); i_1++) {
            for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 2; i_3 < -i_0+PB_Q-2; i_3++) {
                    A[i_2+1][i_1-1][i_3+i_0] = D[i_2][i_1][i_3] + A[i_2+1][i_1-1][i_3+i_0+2] - A[i_3+1][i_2-1][i_1+i_0+1] + B[i_1-1][i_3-2][i_2-2] * 1;
                }
            }
        }
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            for (int i_5 = i_4; i_5 < PB_M-2; i_5++) {
                B[i_0][i_4][i_5] = B[i_0][i_4][i_5-2] - B[i_0][i_4-2][i_5] - B[i_5+2][i_4-1][i_0] + B[i_0-1][i_4][i_5] - 3;
            }
        }
        for (int i_6 = 2; i_6 < i_0; i_6++) {
            C[0][i_6] = C[0][i_6-2] + C[0][i_6+1] - 2;
        }
    }
#pragma endscop
...
kernel_4334211321_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
