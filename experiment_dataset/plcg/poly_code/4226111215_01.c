...
void kernel_4226111215_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_P-1, i_1); i_2++) {
                A[i_0-1][i_1][i_2] = A[i_0-2][i_1][i_2+1] - A[i_0][i_1][i_2] + 4;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < min(PB_L-1, i_3); i_4++) {
            for (int i_5 = 1; i_5 < PB_L+1; i_5++) {
                for (int i_6 = i_5; i_6 < PB_P; i_6++) {
                    B[i_4][i_5-1][i_6] = C[i_3][i_4][i_5-1] + B[i_4][i_5-1][i_6-1] + B[i_4+1][i_6-1][i_5-1] + 5;
                }
            }
        }
    }
...
kernel_4226111215_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
