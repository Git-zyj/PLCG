...
void kernel_3416232325_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-3; i_0++) {
        A[i_0+1] = E[i_0] - F[i_0] * A[i_0+2] + A[i_0+3] - 4;
        A[i_0+1] = F[i_0] * G[i_0][1] - F[i_0] * 5;
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 4; i_2 < PB_N; i_2++) {
                D[i_1-1][i_0-2][i_0-1] = H[i_1][i_0][i_2] + I[i_2][i_1-1][i_0] - D[i_0][i_2-4][i_2-3] - D[i_1-1][i_0-3][i_0-2] - 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 2; i_4 < i_3; i_4++) {
            for (int i_5 = 3; i_5 < min(PB_M, -i_3+PB_L-1); i_5++) {
                D[i_3][i_5-2][i_5-1] = H[i_5-2][i_5+i_3+1][0] * G[i_5-1][i_5] * D[i_4-2][i_5-2][i_5-1] * D[i_3][i_5-3][i_5-2] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3416232325_01(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
