...
void kernel_3312311225_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M+1; i_3++) {
            A[i_2][i_3-2] = D[i_2][i_3-1] * 6;
        }
        C[i_2][i_2] = C[i_2-2][i_2-2] + C[i_2-1][i_2-1] * 2;
    }
#pragma endscop
...
kernel_3312311225_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
