...
void kernel_3334332113_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = D[i_0] + D[i_0-2] - E[i_0] * D[i_0] * A[i_0+2] + 3;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        A[i_1+1] = E[i_1] * F[i_1] + A[i_1+1] + 2;
        for (int i_2 = 0; i_2 < min(-i_1+PB_Q-2, PB_P); i_2++) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                C[i_1][i_2][i_3+2] = G[i_3][i_3][i_2+i_1+2] - H[i_3][i_1][i_2+2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3334332113_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
