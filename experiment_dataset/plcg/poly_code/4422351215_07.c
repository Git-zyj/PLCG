...
void kernel_4422351215_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn),int xo,DATA_TYPE POLYBENCH_1D(O,xO,xo)){
        ...
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            for (int i_2 = max(1, i_0); i_2 < PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_M-1; i_3++) {
                    B[i_3+1][i_3+1][i_0] = E[i_3][i_1+1][i_2] - F[i_2][i_1+i_0][i_3+i_1][0] * G[i_3+i_1+1][i_0][i_1][i_2+1] * H[i_2+1][i_3][i_0][0] * F[i_1-1][i_3][i_0][i_0] - H[i_0+1][i_3+i_1][i_2-1][1] * G[i_2][i_3][i_1][i_0] + H[i_1+i_0][i_2][i_3][1] + 5;
                }
            }
        }
        for (int i_4 = 0; i_4 < -i_0+PB_N-1; i_4++) {
            for (int i_5 = max(1, i_0); i_5 < min(-i_0+PB_M, PB_M-1); i_5++) {
                B[i_5][i_5][i_0] = I[i_0][i_4][i_5] + J[i_4+1][i_5+1][i_0] - E[i_5+1][i_4+i_0+1][i_5] * I[i_4][i_5+i_0][i_5] + I[i_0][i_5+1][i_4] * B[i_5-1][i_5-1][i_0] * 1;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
            B[i_6+1][i_6+1][i_7] = K[i_7-1][i_6] + L[i_7+1][i_6] + K[i_7][i_6] - 1;
        }
    }
    for (int i_8 = 1; i_8 < PB_M-1; i_8++) {
        D[i_8] = M[i_8] * N[1] * N[i_8] * O[i_8] + D[i_8+1] * D[i_8-1] - D[i_8+1] - 5;
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N), xo,POLYBENCH_ARRAY(O), INIT_SEED);
kernel_4422351215_07(xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N), xo,POLYBENCH_ARRAY(O));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N), xo,POLYBENCH_ARRAY(O)));
