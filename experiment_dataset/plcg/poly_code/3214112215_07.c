...
void kernel_3214112215_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_L+1; i_2++) {
            for (int i_3 = 1; i_3 < min(PB_L+1, -i_2+PB_P); i_3++) {
                B[i_1+1][i_2-1][i_3+i_2] = B[i_1+1][i_2-1][i_3+i_2-1] * B[i_1+2][i_3-1][i_3+i_2-1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3214112215_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
