...
void kernel_2424211115_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = 5;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1] = E[i_1] - A[i_1-1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
        A[i_2] = F[i_2][i_2] + 1;
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            A[i_3+1] = G[i_3-1][i_2-1] + 2;
        }
    }
#pragma endscop
...
kernel_2424211115_01(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
