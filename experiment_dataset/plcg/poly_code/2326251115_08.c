...
void kernel_2326251115_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = D[i_0] + E[i_0] - E[i_0-1] - E[i_0] - D[0] + 4;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            B[i_1][i_0+1] = F[i_1][i_0] * F[i_1][i_0] - F[i_1-1][i_0] + B[i_1-1][i_0+1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_L-1; i_3++) {
            A[i_3-1] = F[i_2+1][i_2] - F[i_3][i_3] + D[i_3] - F[i_2][i_3] + G[i_3+1][i_3][i_2] + 6;
        }
    }
#pragma endscop
...
kernel_2326251115_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
