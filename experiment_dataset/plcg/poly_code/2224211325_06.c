...
void kernel_2224211325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1][i_1+i_0-1] = A[i_1][i_1+i_0] * A[i_1][i_1+i_0-3] + A[i_1-2][i_1+i_0-4] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M+1; i_3++) {
            A[i_3][i_3+i_2-1] = C[i_3-1][i_2+1] - 2;
        }
    }
#pragma endscop
...
kernel_2224211325_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
