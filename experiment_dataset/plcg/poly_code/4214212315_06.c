...
void kernel_4214212315_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_1+i_0][i_1] = C[i_1][i_0] * C[i_1+i_0][i_1] + A[i_1+i_0+1][i_0+1] + A[i_1+i_0-2][i_1-1] + 3;
        }
    }
        ...
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
                for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
                    B[i_4][i_5][2] = B[i_4][i_5+1][2] * B[i_4+1][i_5+1][2] + B[i_3][i_4+1][2] * B[i_5-1][i_4+1][2] - 2;
                }
            }
        }
    }
...
kernel_4214212315_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
