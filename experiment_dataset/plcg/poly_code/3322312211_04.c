...
void kernel_3322312211_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_P-1; i_2++) {
                A[i_2][i_0-2][i_2+i_1] = A[i_2-1][i_0-2][i_2+i_1] - A[i_2][i_0-3][i_2+i_1+1] - 5;
            }
        }
        B[i_0] = D[i_0-2] - E[i_0] - B[i_0+1] + B[i_0-1] - B[i_0+1] - 6;
    }
    for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
            for (int i_5 = i_4; i_5 < -i_3+PB_P+1; i_5++) {
                C[i_3+2][i_4+2][i_5+i_3-1] = 2;
            }
        }
    }
#pragma endscop
...
kernel_3322312211_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
