...
void kernel_4416352221_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < min(min(PB_M-4, i_0), -i_0+PB_N); i_1++) {
            for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
                for (int i_3 = 2; i_3 < PB_M; i_3++) {
                    A[i_1][i_2-2][i_3-1][i_0-2] = E[i_2+1][i_0][i_3][i_1+1] * F[i_3][i_2][i_0][i_1] + G[i_3][i_1][0] + H[i_2+i_1-2][i_2+2][i_3][1] * H[i_0][i_3][i_1][i_2] - H[i_1-1][i_3][i_0][i_2] * B[i_2+2][i_3-2][i_1] + B[i_1+4][i_2][i_0+1] + 3;
                }
                B[i_0+2][i_2][i_1] = I[i_1+i_0][i_2][i_1] - J[i_1][i_2-1][i_2] * B[i_0+2][i_2][i_1+2] + 3;
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < i_4; i_5++) {
            for (int i_6 = 0; i_6 < -i_5+PB_M-2; i_6++) {
                for (int i_7 = 2; i_7 < i_4; i_7++) {
                    C[i_4][i_5][i_6][i_7+1] = F[i_6][i_7][i_5][i_4+1] - H[i_5][i_7-2][i_6][i_4] + J[i_6][i_5-2][i_5] * J[i_6+2][i_7][0] - E[i_6+i_5+2][i_6][i_4-2][i_4] * K[i_6][i_7-1][i_4][i_5][i_5-1] + 1;
                }
            }
        }
        D[i_4+1] = L[i_4][i_4] * M[i_4+1] + M[i_4] - D[i_4+3] * D[i_4-1] * D[i_4] - 2;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4416352221_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M));
