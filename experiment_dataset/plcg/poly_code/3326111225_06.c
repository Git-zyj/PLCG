...
void kernel_3326111225_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 3; i_1 < i_0; i_1++) {
            A[i_0-1][i_1-1] = A[i_0-1][i_1+1] - A[i_0-1][i_1-3] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < min(PB_P-1, -i_2+PB_P); i_3++) {
            for (int i_4 = 0; i_4 < min(PB_M-1, i_3); i_4++) {
                B[i_4][i_3+1] = D[i_4][i_3][i_3+i_2] - B[i_3-1][i_4+1] + B[i_3][i_4] + B[i_4+1][i_3+1] + 3;
            }
        }
    }
...
kernel_3326111225_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
