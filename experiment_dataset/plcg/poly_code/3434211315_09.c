...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M-1, PB_L-1); i_2++) {
            B[i_2+i_1][i_1+1] = F[i_1][i_2+1] * B[i_2+i_1+1][i_1+2] * B[i_2+i_1-2][i_1] + B[i_2+i_1-1][i_1+1] * 4;
            for (int i_3 = 0; i_3 < -i_1+PB_M; i_3++) {
                B[i_3+i_1][i_3+1] = G[i_3][i_2-1] * 3;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 1; i_5 < min(-i_4+PB_M-2, PB_L); i_5++) {
            for (int i_6 = 2; i_6 < min(-i_4+PB_M-2, PB_L-1); i_6++) {
                D[i_6+i_4+1][i_5][i_5-1] = G[i_5][1] - D[i_5+i_4+2][i_6-1][i_6-2] + D[i_6+i_4][i_5][i_5-1] * D[i_5+i_4+1][i_6+1][i_6] + D[i_6+i_4+2][i_5][i_5-1] - 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3434211315_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
