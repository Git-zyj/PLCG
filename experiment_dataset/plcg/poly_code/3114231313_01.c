...
void kernel_3114231313_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_N+1, PB_L-2); i_1++) {
            A[i_1][i_1+1] = B[i_1+1] * C[i_1][i_0+1] - C[i_1+i_0-1][i_1] + A[i_1+1][i_1+2] + A[i_1-1][i_1] * A[i_0+1][i_0+2] + 3;
        }
    }
#pragma endscop
...
kernel_3114231313_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
