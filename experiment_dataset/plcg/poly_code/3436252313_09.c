...
void kernel_3436252313_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_Q, PB_P-1); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, PB_Q-2); i_2++) {
                A[i_1][i_2+i_0][2] = E[i_1][i_1+i_0][i_2+2] * F[i_2][i_0][i_1] + G[i_2-1][i_1] + F[i_0][i_2][i_1+1] + A[i_1][i_2+i_0-1][2] * 1;
            }
        }
        B[i_0][i_0] = H[i_0] - H[i_0] * I[i_0] - B[i_0+1][i_0+1] - B[i_0-1][i_0-1] + 4;
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < -i_3+PB_P; i_4++) {
            for (int i_5 = max(2, i_3); i_5 < -i_3+PB_Q; i_5++) {
                A[i_5][i_4+i_3-1][2] = J[i_5-2][i_3] + E[i_4+1][i_3][i_4-2] - E[i_4][i_5+i_3][i_4] - F[i_5][i_3][i_4+i_3] + 2;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_P; i_6++) {
        for (int i_7 = 0; i_7 < min(-i_6+PB_L+2, PB_P); i_7++) {
            B[i_7][i_7] = J[2][i_6] * G[i_7+i_6-2][0] + H[i_6] + G[i_7+2][i_6] - J[i_6][i_7] - J[i_7][i_6-2] * B[i_6+1][i_6+1] + B[i_6][i_6] - B[i_6-1][i_6-1] - 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3436252313_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
