...
void kernel_3312111325_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q; i_0++) {
        for (int i_1 = 3; i_1 < min(i_0, -i_0+PB_L+1); i_1++) {
            A[i_0-1][i_1-1][i_0] = D[i_1][i_1+i_0-1] - A[i_1-3][i_0-2][i_1-2] * A[i_0-3][i_1-1][i_0-2] + 4;
            A[i_0-1][i_1-1][i_0] = 4;
        }
        for (int i_2 = 0; i_2 < PB_M-3; i_2++) {
            for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
                C[i_3][i_2+1][i_0] = C[i_2][i_3+1][i_0-2] - C[i_3+2][i_2+3][i_0+2] - C[i_3+1][i_2][i_0] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3312111325_04(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
