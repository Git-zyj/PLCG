...
void kernel_3326211325_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[i_2][i_0][i_1+1] = A[i_0+2][i_2][i_1+1] + A[i_2-2][i_0][i_1+1] + A[i_2+1][i_0][i_1+2] * A[i_2+1][i_0][i_1+1] * A[i_2+1][i_0+1][i_1+1] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < PB_Q-1; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                B[i_5+1][i_4][i_4] = B[i_5+1][i_4+1][i_4+1] - 1;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M; i_6++) {
        for (int i_7 = i_6; i_7 < PB_P; i_7++) {
            for (int i_8 = max(i_6, 2); i_8 < PB_L; i_8++) {
                C[i_6][i_6-1][i_8] = D[i_6][i_8][i_7] - C[i_6][i_6-1][i_8+2] - C[i_6][i_6-1][i_8+1] * C[i_6][i_6-1][i_8-2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3326211325_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
