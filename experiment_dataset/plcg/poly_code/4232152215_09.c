...
void kernel_4232152215_09(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                B[i_0-1][0][i_2][i_1+2] = C[i_1][i_0][i_2] - D[i_0+1][i_1][i_0][i_2] * 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_P+2, PB_P-2); i_4++) {
            for (int i_5 = 1; i_5 < min(-i_3+PB_L, PB_Q-3); i_5++) {
                for (int i_6 = 1; i_6 < PB_Q-1; i_6++) {
                    B[i_3][0][i_4][i_5+2] = D[i_6][i_5+i_3][i_4][0] - D[i_5][i_3-2][i_4+2][i_6] + D[i_6][i_5-1][i_4+i_3-2][i_5] + D[i_5][i_3][i_4][0] * D[i_4][i_3][i_5-1][i_6] - D[i_4][i_6][i_5][i_3+2] - D[i_5-1][i_3][i_6-1][i_4] - D[i_3][i_4][i_5][i_6+1] * B[i_3+1][0][i_4+1][i_5+2] - B[i_3][0][i_4][i_5+3] * 1;
                }
            }
        }
    }
...
kernel_4232152215_09(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D));
