...
void kernel_2426231115_07(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            C[i_1-1] = E[0] + F[i_0][i_1-1] * G[i_1][1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        B[i_2-1] = H[i_2] - E[i_2] + H[i_2] + 5;
        C[i_2-1] = G[i_2][0] * E[i_2] * H[i_2] + 3;
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_M; i_4++) {
            D[i_4+i_3][i_3+1] = F[i_3][i_4] - F[i_4][i_3+1] + C[i_4-1] + D[i_4+i_3-1][i_3+1] + 6;
        }
    }
#pragma endscop
...
kernel_2426231115_07(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
