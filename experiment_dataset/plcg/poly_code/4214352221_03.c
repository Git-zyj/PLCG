...
void kernel_4214352221_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_L-1, -i_0+PB_Q-1); i_2++) {
                for (int i_3 = -i_1+PB_Q-2 - 1; i_3 >= 0; i_3--) {
                    A[i_1][i_0][i_2+i_0][i_3] = C[i_3+i_2][i_0][i_1][i_3] * D[i_0-2][i_3][i_1][1] - E[i_3][i_0+2][i_2+i_1+1][0] + A[i_2][i_1][i_3+i_1+2][i_0+1] * A[i_1][i_0+1][i_2+i_0+1][i_3] + 1;
                }
            }
        }
        B[i_0] = F[1] + F[i_0] + F[i_0+1] - G[i_0] + G[i_0-2] - H[i_0] - G[i_0] * B[i_0+2] * 4;
    }
#pragma endscop
...
kernel_4214352221_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
