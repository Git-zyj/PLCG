...
void kernel_3232212215_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            A[i_1-1][i_0-1] = C[i_1+2][i_0-1] + A[i_1-2][i_0-1] * A[i_1-1][i_0] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 3; i_3 < -i_2+PB_L+2; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                B[i_4][i_3+i_2-2] = B[i_2][i_4+i_3-3] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3232212215_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
