...
void kernel_4234312223_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        B[i_0+2][i_0+2] = C[i_0-1] + 5;
    }
    for (int i_1 = 0; i_1 < PB_P-4; i_1++) {
        for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
            B[i_2+2][i_2+2] = D[i_1][i_2+1] * B[i_1+3][i_1+3] - B[i_1+4][i_1+4] * 4;
        }
    }
#pragma endscop
...
kernel_4234312223_05(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
