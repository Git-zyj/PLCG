...
void kernel_3216312111_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            ...
            for (int i_3 = 2; i_3 < PB_L; i_3++) {
                B[i_0][i_1-1][i_3] = C[i_3-2][i_1-1][i_0] * B[i_0+1][i_1-2][i_3] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3216312111_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
