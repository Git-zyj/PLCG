...
void kernel_2334151323_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = D[i_0-1] + D[i_0] + E[1][i_0] + D[i_0] * D[i_0] * D[i_0-1] - A[i_0+1] + 4;
        for (int i_1 = max(2, i_0); i_1 < min(-i_0+PB_M, PB_L-2); i_1++) {
            B[i_0][i_1] = E[i_1+i_0][i_0] + E[i_1][i_0] + E[i_1][i_0-1] * E[i_1-1][i_0] * E[i_1][i_0-1] * E[i_1-1][i_0+1] - B[i_0][i_1+2] * B[i_0-1][i_1-1] + B[i_0][i_1-2] - B[i_0][i_1-1] * 6;
        }
        A[i_0] = E[i_0][i_0] - D[i_0+1] * D[0] * A[i_0] - 5;
    }
#pragma endscop
...
kernel_2334151323_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
