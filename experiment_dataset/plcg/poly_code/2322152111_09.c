...
void kernel_2322152111_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = PB_L-1 - 1; i_1 >= 0; i_1--) {
            A[i_1][i_0] = D[i_0][i_1+1] + D[i_1+1][i_0] + D[i_1][i_0] + D[i_1+2][i_0] + 6;
        }
        for (int i_2 = 1; i_2 < PB_Q; i_2++) {
            B[i_0][i_0] = E[0] - D[i_2-1][i_0] + D[i_0][i_2] - D[i_2][i_0+1] + D[i_0][i_2-1] + F[i_2][i_0-2][0] * E[i_0] - 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-3; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_L; i_4++) {
            C[i_3][i_3+2] = D[i_3-1][i_4] * E[i_4] + D[i_3][i_4-2] * E[i_3] - C[i_3+1][i_3+3] + 4;
        }
    }
#pragma endscop
...
kernel_2322152111_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
