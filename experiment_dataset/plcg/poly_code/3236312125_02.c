...
void kernel_3236312125_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M+2; i_1++) {
            A[i_0][i_1+2] = C[i_1-2][i_0] - A[i_0][i_1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = PB_M - 1; i_3 >= i_2; i_3--) {
            A[i_2+2][i_3+2] = D[i_3][i_2+1] + 6;
        }
    }
#pragma endscop
...
kernel_3236312125_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
