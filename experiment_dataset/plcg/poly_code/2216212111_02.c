...
void kernel_2216212111_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            A[i_2+2][i_3] = C[i_2+1][i_2-1] * A[i_2+2][i_3+2] - 3;
        }
    }
#pragma endscop
...
kernel_2216212111_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
