...
void kernel_3422252315_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_1] = E[i_1-2] - F[i_1][i_0] + G[i_1][i_0] + A[i_1-1] - A[i_1+1] * 5;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_P, PB_M-2); i_2++) {
            D[i_2+1][i_0+1] = G[i_0][i_2] * G[i_2][i_0-2] + G[i_2][i_0+1] - F[i_2][i_0+2] - F[i_2][i_0] * 4;
            for (int i_3 = 0; i_3 < min(PB_M-2, -i_2+PB_M); i_3++) {
                D[i_3+2][i_2+2] = G[i_3][i_0] + H[i_3][i_0][i_2+1] + I[i_3+i_2][i_0][i_2+i_0] - H[i_0][i_2][i_3] * H[i_3+i_2][i_0-1][i_2] * D[i_2+2][i_0+1] + 1;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-3; i_4++) {
        for (int i_5 = 1; i_5 < min(-i_4+PB_M, PB_M-2); i_5++) {
            D[i_5+2][i_4+1] = G[i_5][i_5] + G[i_5+i_4][i_4] * F[i_5+1][i_5] - F[i_4][i_5] - G[i_5][i_4] + A[i_5-1] - D[i_5+1][i_4] + D[i_4+3][i_5+1] - 2;
        }
    }
#pragma endscop
...
kernel_3422252315_06(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
