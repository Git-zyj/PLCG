...
void kernel_2132132225_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(PB_P, -i_0+PB_M+1); i_1++) {
            A[i_0][i_1] = B[i_1+2][i_0] * B[i_1+i_0-1][1] + B[i_1][i_0-1] * A[i_0][i_1-1] + A[i_0+2][i_1-2] + 2;
        }
    }
#pragma endscop
...
kernel_2132132225_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
