...
void kernel_3214311315_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_Q-1, i_0); i_1++) {
            B[i_1][i_1-1][i_0] = B[i_0][i_0-1][i_1+1] - 3;
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                B[i_1][i_1-1][i_2] = C[i_1+1][i_2][i_0+1] - C[i_1][i_0][i_1] - B[i_1-1][i_1-2][i_2] - B[i_1][i_1-1][i_2-1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3214311315_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
