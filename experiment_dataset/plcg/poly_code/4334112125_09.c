...
void kernel_4334112125_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf)){
        ...
        for (int i_1 = 2; i_1 < PB_Q; i_1++) {
            for (int i_2 = i_0; i_2 < PB_Q; i_2++) {
                for (int i_3 = 2; i_3 < -i_1+PB_Q; i_3++) {
                    A[i_0-1][i_1-2][i_3-2][i_2] = D[i_3][i_0][i_3+i_1][i_2] * 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        B[i_4+1] = E[i_4][i_4-1] * B[i_4+2] + 5;
    }
    for (int i_5 = 2; i_5 < PB_M+1; i_5++) {
        for (int i_6 = 1; i_6 < min(PB_Q-2, i_5); i_6++) {
            for (int i_7 = 0; i_7 < PB_L; i_7++) {
                for (int i_8 = 2; i_8 < PB_M; i_8++) {
                    A[i_5-1][i_6-1][i_8-2][i_7] = F[i_8][i_7][i_8][i_5-2][i_6+2] + 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4334112125_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F)));
