...
void kernel_3216111213_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0-1] = A[i_0-2] + A[i_0] * A[i_0] - A[i_0-2] * 2;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_P; i_2++) {
            for (int i_3 = 1; i_3 < PB_Q; i_3++) {
                B[i_1+1][i_1-1][i_3] = C[i_3-1][i_2] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3216111213_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
