...
void kernel_3414211311_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
                A[i_0][i_2-1][i_1] = A[i_1+1][i_2-2][i_0] + A[i_0][i_2-1][i_1-1] * A[i_0+1][i_2-1][i_1] + A[i_0][i_2-2][i_1] * 1;
            }
            B[i_0][i_1][i_0] = 6;
        }
        for (int i_3 = 1; i_3 < PB_Q; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_Q, -i_3+PB_L-1); i_4++) {
                C[i_4+i_3][i_3] = C[i_4+i_3-1][i_4-1] + C[i_4+i_3+1][i_4] - 1;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 0; i_6 < min(PB_Q, -i_5+PB_L); i_6++) {
            for (int i_7 = 1; i_7 < PB_M+1; i_7++) {
                D[i_6][i_6][i_7] = E[i_6+i_5][i_7-1][i_6] + D[i_5-1][i_5-1][i_7] + D[i_6][i_6][i_7+1] - D[i_5+1][i_5+1][i_7] * D[i_7-1][i_7-1][i_6] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3414211311_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
