...
void kernel_3224212123_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_M, -i_2+PB_N); i_3++) {
                B[i_3+i_1][i_2][i_1] = C[i_3+i_2][i_1-2][i_2-2] * B[i_3+i_1-2][i_2][i_1-2] - B[i_3+i_1-1][i_2][i_1+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3224212123_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
