...
void kernel_2424351121_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        B[i_0] = E[0] + F[i_0+1] + F[i_0] - G[i_0] + 2;
        B[i_0] = H[i_0][i_0] - G[i_0+1] + F[i_0] * F[i_0+1] - F[i_0] - G[i_0+1] + 4;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            C[i_2-1][i_1] = I[i_2+i_1][i_1] + J[i_1][i_2] + H[i_2][i_1] * 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        D[i_3-1] = F[i_3+1] - J[i_3+1][0] - E[i_3] * E[i_3+1] - D[i_3] * 3;
    }
#pragma endscop
...
kernel_2424351121_00(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
