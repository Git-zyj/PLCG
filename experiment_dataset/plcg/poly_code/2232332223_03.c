...
void kernel_2232332223_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = C[0] * D[i_0+1] - C[0] - A[i_0-2] + A[i_0+1] + A[i_0+2] - 6;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_N); i_2++) {
            B[i_2+i_1][0] = E[i_1][i_2] + C[i_1] + C[i_2] + B[i_2+i_1-1][0] - 3;
        }
    }
#pragma endscop
...
kernel_2232332223_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
