...
void kernel_2324111315_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_N+2; i_1++) {
            A[i_0][i_1-1] = A[i_0+1][i_1-1] + B[i_1-2] + 2;
        }
        B[i_0-1] = B[i_0] + B[i_0-2] * 5;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            C[i_3+1] = D[i_3-1][i_2] - C[i_3] - C[i_3+2] - C[i_3+2] - 5;
        }
    }
#pragma endscop
...
kernel_2324111315_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
