...
void kernel_3214311321_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        A[i_0] = A[i_0+2] + 1;
        for (int i_1 = 1; i_1 < min(min(PB_N-3, -i_0+PB_M), i_0); i_1++) {
            B[i_1][i_0+1] = C[i_1+i_0][i_1-1] * A[i_0-1] * B[i_1][i_0-1] + B[i_0][i_1+3] + B[i_0][i_1-1] * B[i_1-1][i_0+1] * 4;
        }
    }
#pragma endscop
...
kernel_3214311321_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
