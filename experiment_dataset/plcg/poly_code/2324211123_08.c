...
void kernel_2324211123_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        ...
        for (int i_2 = max(3, i_0); i_2 < PB_L-1; i_2++) {
            B[i_2-1][i_0-1] = D[i_2+1][1] * E[i_2] + B[i_2][i_0-1] + B[i_2-3][i_0-1] - 3;
        }
    }
...
kernel_2324211123_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
