...
void kernel_2226152315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[0] = C[i_0+1] * C[i_0] * C[i_0] + C[i_0] * C[i_0+2] * D[i_0][i_0] * A[0] + A[0] * A[0] * 6;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(i_1, PB_M-1); i_2++) {
            A[0] = D[i_1][i_2+2] + D[i_2+1][i_1+2] * D[i_1-2][2] - D[i_2][i_1] + 2;
        }
    }
#pragma endscop
...
kernel_2226152315_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
