...
void kernel_4236132321_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_1] = C[i_1][i_0+2] - C[i_1-2][i_0] + C[i_1][i_0-2] + A[i_1+2] - A[i_1-2] - A[i_1-1] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = min(PB_M-2, -i_2+PB_M) - 1; i_3 >= i_2; i_3--) {
            B[i_3] = C[i_3+i_2][i_2-1] + D[0] - C[i_3+2][i_2] + A[i_3] - B[i_3+1] - B[i_3-1] * 6;
        }
    }
#pragma endscop
...
kernel_4236132321_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
