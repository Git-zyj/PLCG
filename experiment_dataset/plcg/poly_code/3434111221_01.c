...
void kernel_3434111221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0][i_1] = E[i_1-1] - A[i_0-1][i_1] + A[i_1][i_0-1] * 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_M-2; i_3++) {
            B[i_2][i_3+i_2] = B[i_3][i_3+i_2+2] - B[i_3-2][i_3+i_2-2] + 2;
        }
        for (int i_4 = i_2; i_4 < PB_L; i_4++) {
            C[i_4][i_2] = C[i_4-1][i_2+2] + 6;
        }
    }
    for (int i_5 = 2; i_5 < PB_P; i_5++) {
        for (int i_6 = i_5; i_6 < PB_M; i_6++) {
            D[i_5][i_5] = F[i_6][i_5] - F[i_6-1][i_5-1] + D[i_5-2][i_5-2] * 4;
        }
    }
#pragma endscop
...
kernel_3434111221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
