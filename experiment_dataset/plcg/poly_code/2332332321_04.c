...
void kernel_2332332321_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, PB_M-2); i_1++) {
            A[i_0][i_1] = D[i_1][i_0] - E[i_1+i_0] - F[i_1-1][i_0-1] * F[i_1][i_0] - A[i_0+2][i_1+1] * B[i_0+2][i_1] - 1;
            B[i_0+2][i_1] = F[i_1][i_0+2] - D[i_0][i_1+2] * G[i_1][i_0-1] + B[i_0][i_1] + B[i_0+3][i_1] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        C[i_2][i_2] = H[0] * C[i_2+2][i_2+2] - C[i_2+1][i_2+1] * C[i_2-2][i_2-2] * C[i_2-1][i_2-1] - C[i_2+1][i_2+1] - 3;
    }
#pragma endscop
...
kernel_2332332321_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
