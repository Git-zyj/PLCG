...
void kernel_4236212123_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 3; i_2 < PB_P+1; i_2++) {
                A[i_0+1][i_1-2][i_2-1] = C[i_2][i_1][i_0] * A[i_0+1][i_1-2][i_2-3] + 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_M-2; i_4++) {
            B[i_4+i_3] = B[i_4+i_3+2] - 3;
        }
    }
#pragma endscop
...
kernel_4236212123_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
