...
void kernel_2316212311_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = i_0; i_2 < PB_L-1; i_2++) {
            B[i_0-2][i_2+1] = B[i_0-2][i_2] + B[i_0-3][i_2] - 2;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 3; i_4 < PB_M-1; i_4++) {
            C[i_4][i_4-2] = D[i_3] * C[i_4-1][i_4-3] * C[i_4+1][i_4-1] + C[i_4+1][i_4-1] * 2;
        }
    }
#pragma endscop
...
kernel_2316212311_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
