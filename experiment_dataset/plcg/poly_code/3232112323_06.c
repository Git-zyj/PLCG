...
void kernel_3232112323_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = i_0; i_2 < min(min(PB_M-3, -i_1+PB_L-4), -i_0+PB_L-4); i_2++) {
                A[i_0+2][i_2+1][i_2+i_1+2][i_1] = C[i_1][i_0][i_2] - A[i_1+2][i_0+3][i_2+i_0+4][i_2] - A[i_0+2][i_2+3][i_2+i_1+4][i_1] * A[i_0+4][i_2+1][i_2+i_1+2][i_1] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
        for (int i_4 = 3; i_4 < PB_L+2; i_4++) {
            B[i_3+2][i_4-2] = B[i_3+1][i_4-2] * B[i_3+2][i_4-3] * B[i_3+3][i_4-2] + 2;
        }
    }
#pragma endscop
...
kernel_3232112323_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
