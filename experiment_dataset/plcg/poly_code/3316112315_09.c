...
void kernel_3316112315_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            A[i_1][i_0+2] = A[i_1-1][i_0+2] * A[i_1-1][i_0+3] * A[i_1+1][i_0+2] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < PB_P-2; i_3++) {
            C[i_2][i_3-2] = D[i_2-2][i_3] - C[i_2+1][i_3-2] - C[i_2-1][i_3-3] - 1;
            C[i_2][i_3-2] = D[i_3-2][0] * D[i_2][i_3+2] * 3;
        }
    }
#pragma endscop
...
kernel_3316112315_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
