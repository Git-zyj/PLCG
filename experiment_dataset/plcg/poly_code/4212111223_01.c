...
void kernel_4212111223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_L, i_1); i_2++) {
                A[i_2][i_1-1] = A[i_1][i_2] - A[i_2-1][i_1-1] * 6;
            }
        }
        B[i_0-1] = C[i_0] * C[i_0] * B[i_0+1] + B[i_0-2] - 2;
    }
#pragma endscop
...
kernel_4212111223_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
