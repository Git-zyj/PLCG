...
void kernel_2324111221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L-1; i_3++) {
            B[i_2][i_3] = D[i_3+i_2+1][i_2] - D[0][i_2+1] - B[i_3][i_2-1] - 2;
        }
    }
    for (int i_4 = 0; i_4 < PB_N-2; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_M); i_5++) {
            C[i_4][i_5] = C[i_4+2][i_5] - C[i_4][i_5-1] + 5;
        }
    }
#pragma endscop
...
kernel_2324111221_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
