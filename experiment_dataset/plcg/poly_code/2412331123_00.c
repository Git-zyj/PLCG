...
void kernel_2412331123_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0][i_1+1] = E[i_0] - F[i_1][i_0] * A[i_0-1][i_1+1] * 5;
        }
        D[i_0+2] = G[i_0][1] + E[i_0] - 6;
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            C[i_2-1][i_3] = H[i_3][i_2+1] * E[i_3] - H[i_3][i_2-1] - G[i_3][i_2] + E[i_2+1] - F[i_2][i_3+1] - C[i_2+1][i_3] * 3;
        }
        D[i_2] = I[1] - D[i_2+1] * 6;
    }
#pragma endscop
...
kernel_2412331123_00(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
