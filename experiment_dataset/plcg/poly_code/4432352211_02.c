...
void kernel_4432352211_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_2+PB_Q+1, PB_L-2); i_3++) {
                    A[i_0-2][i_3+2][i_0-1][i_2] = E[i_1][i_0][i_3+i_2-1][i_2-1] + E[0][i_2][i_0+2][i_3-2] + F[i_2+2][i_1][i_2][i_3-1] - G[i_2][i_1+1][i_3] * F[i_0-2][i_2-1][i_2][i_1] - H[i_1-1][i_0+1][i_2-2][i_3] + E[i_2][i_3][i_1][i_0+2] * A[i_0-1][i_1+2][i_0][i_2] * 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
            for (int i_6 = 0; i_6 < -i_4+PB_P; i_6++) {
                B[i_5+2][i_6][i_4] = H[i_5][i_6][i_4+1][i_6+i_4] * I[i_5][i_6+2] - J[i_5][i_6][i_4] * A[i_5-1][i_6+2][i_5][i_4-1] * B[i_5+1][i_6][i_4] * 2;
            }
        }
        C[i_4] = K[i_4-1] - K[i_4] - L[i_4-2] - K[i_4] - C[i_4+1] * C[i_4-1] - C[i_4+1] - C[i_4-1] + 1;
    }
    for (int i_7 = 2; i_7 < PB_P; i_7++) {
        for (int i_8 = 0; i_8 < PB_L; i_8++) {
            for (int i_9 = 0; i_9 < -i_7+PB_M; i_9++) {
                for (int j_0 = 2; j_0 < PB_L; j_0++) {
                    D[i_9][i_8][i_7][j_0] = H[i_9+i_7][j_0][i_7][i_8] * H[j_0-2][i_8][i_9][i_8] - E[j_0][i_7-2][i_9][i_8] + 2;
                }
            }
        }
    }
