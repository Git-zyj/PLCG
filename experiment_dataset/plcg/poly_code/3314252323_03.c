...
void kernel_3314252323_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0] * E[i_0] + E[i_0] + D[i_0] * D[i_0-2] + A[i_0-1] - 6;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_M, -i_1+PB_P); i_2++) {
            B[i_2+2][i_2+i_1] = F[i_2][i_1] - F[i_2][i_1] + F[i_1-1][i_2+1] + G[i_2][i_1+2] + G[i_2][i_1] - A[i_1] + B[i_2][i_2+i_1-2] + B[i_2+2][i_2+i_1-1] - B[i_2+3][i_2+i_1] + 6;
        }
        for (int i_3 = 2; i_3 < PB_P; i_3++) {
            for (int i_4 = 2; i_4 < min(-i_1+PB_P, PB_M-2); i_4++) {
                B[i_4+2][i_4+i_1] = H[i_1+2][i_4][i_3] * I[i_4-2][i_3-2][i_1] * H[i_4+2][i_1][i_4] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3314252323_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
