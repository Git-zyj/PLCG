...
void kernel_3426312313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
    ...
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        ...
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < min(PB_M-3, i_0); i_2++) {
                B[i_2+2][i_1][i_2-2] = B[i_2+3][i_1][i_2-1] - B[i_2+2][i_1-1][i_2-2] - B[i_0+2][i_1+1][i_0-2] + 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_Q-1; i_3++) {
        for (int i_4 = 3; i_4 < PB_L-2; i_4++) {
            C[i_4] = F[i_3-2][i_3+2] + G[i_3+2][i_4+2] + B[i_4+1][i_3][i_4-3] + 6;
            for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                C[i_4] = H[i_4][i_5][i_3+1] - C[i_5+1] + C[i_5] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3426312313_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
