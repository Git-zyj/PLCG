...
void kernel_3236131115_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            A[i_1][i_1] = C[i_1][i_0+1] - C[i_1-1][i_1+i_0] + C[i_1+1][i_0] - A[i_1-1][i_1-1] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_N, -i_2+PB_L); i_3++) {
            A[i_2][i_2] = D[i_3] + C[i_3][i_2] - C[i_3+i_2][i_3+1] - 5;
        }
    }
#pragma endscop
...
kernel_3236131115_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
