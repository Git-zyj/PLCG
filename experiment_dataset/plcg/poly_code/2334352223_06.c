...
void kernel_2334352223_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-1] = D[i_0] - E[i_0] * D[i_0] + D[i_0] + E[i_0] * A[i_0-2] + 3;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 4; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            B[i_2-2] = F[i_2][i_1] - G[i_2+i_1][i_1+2] * G[i_2-2][i_1-2] * B[i_2-4] - B[i_2] - B[i_2-3] * 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        B[i_3-2] = H[i_3-1] - E[i_3] - H[0] * D[i_3] * D[i_3+1] * E[i_3] + H[i_3] + 6;
    }
#pragma endscop
...
kernel_2334352223_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
