...
void kernel_4232111321_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            B[i_2+1][i_3] = C[i_3][i_2-1] * B[i_2+1][i_3-2] * B[i_2][i_3] + B[i_2+1][i_3+1] + B[i_2+1][i_3-1] * B[i_2+1][i_3+2] + B[i_2-1][i_3] * 6;
        }
    }
#pragma endscop
...
kernel_4232111321_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
