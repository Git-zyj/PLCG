...
void kernel_3216312223_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_N-1, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_N); i_2++) {
                A[i_0][i_2+i_1][i_1] = C[i_1+1][i_2] + A[i_2][i_1+i_0+1][i_1+1] - 6;
            }
        }
        for (int i_3 = 2; i_3 < min(-i_0+PB_M, PB_L-2); i_3++) {
            for (int i_4 = i_3; i_4 < PB_L; i_4++) {
                B[i_4][i_3] = C[i_3+i_0][i_4] - B[i_4+1][i_3] - B[i_4][i_3+2] + B[i_4][i_3-2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3216312223_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
