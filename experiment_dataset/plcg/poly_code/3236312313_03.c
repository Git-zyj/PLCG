...
void kernel_3236312313_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 3; i_2 < min(PB_M+1, -i_0+PB_L); i_2++) {
                A[i_0][i_2-2][i_2+i_0] = A[i_0-1][i_2-1][i_2+i_0] * A[i_0][i_2-3][i_2+i_0-1] * A[i_0-1][i_2-2][i_2+i_0-1] * 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
        B[i_3+2][i_3+1] = C[i_3] - B[i_3+1][i_3] * B[i_3+3][i_3+2] * 1;
    }
#pragma endscop
...
kernel_3236312313_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
