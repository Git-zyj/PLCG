...
void kernel_4312111225_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
        ...
        for (int i_1 = 3; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_P; i_3++) {
                    A[i_0-1][i_1][i_2-1][i_3] = D[i_1][i_2-1][i_3+i_0][i_0] * 6;
                }
            }
            B[i_1-1] = B[i_1-3] * B[i_1-2] * B[i_1] - 1;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        B[i_4-1] = E[i_4+1] + F[i_4][1] - 6;
    }
#pragma endscop
...
kernel_4312111225_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
