...
void kernel_2224252221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M-3, -i_0+PB_L); i_1++) {
            A[i_0][i_1] = C[i_1+i_0-1][i_0] * C[i_1][i_0-1] + C[i_0][0] * D[i_0][i_1+i_0] - D[i_1+i_0][i_1-2] - A[i_0][i_1-2] * B[i_1+2] - B[i_1+3] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-3; i_2++) {
        B[i_2+2] = E[i_2] - E[i_2] * E[i_2+2] * F[i_2-2] * E[i_2] - B[i_2+3] - 3;
    }
#pragma endscop
...
kernel_2224252221_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
