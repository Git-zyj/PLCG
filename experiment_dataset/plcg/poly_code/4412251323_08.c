...
void kernel_4412251323_08(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        D[i_0+2] = E[i_0] * E[i_0+1] - F[i_0+1] * E[i_0-1] + F[i_0] + E[i_0+1] - F[i_0-1] + D[i_0+2] * 1;
    }
    for (int i_1 = 2; i_1 < PB_M-3; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M-2; i_2++) {
            for (int i_3 = 0; i_3 < min(i_2, -i_1+PB_M-3); i_3++) {
                B[i_2+i_1+1][i_2+1][i_3+1] = G[i_3][i_1][i_2] - G[i_1+1][i_2][i_3+i_1-1] * B[i_3+i_1][i_3+1][i_2+1] + B[i_2+i_1+2][i_2+2][i_3+3] + B[i_3+i_1+3][i_3+3][i_2+1] + 6;
            }
        }
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_4+PB_M-1, -i_1+PB_P); i_5++) {
                for (int i_6 = 0; i_6 < min(PB_P-1, -i_4+PB_Q); i_6++) {
                    B[i_5+i_4+1][i_5+1][i_6+1] = H[i_5+1][i_1][i_4][i_1] * H[i_5-1][i_6][i_4][i_1+1] * I[i_4+1][1][i_1][i_5+i_1] - J[i_4-1][i_6][i_4+1] + H[i_5+i_4+1][i_1][i_6+i_4][i_5] + 4;
                }
            }
        }
        D[i_1] = F[i_1+1] * F[0] * E[i_1] + E[0] - F[i_1] * D[i_1-2] + 2;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4412251323_08(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
