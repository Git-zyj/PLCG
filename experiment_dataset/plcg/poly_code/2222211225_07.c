...
void kernel_2222211225_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            B[i_0][i_1][i_0+1] = B[i_1][i_0][i_1+1] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_L, PB_L-2); i_3++) {
            B[i_3][i_2][i_3+1] = C[i_3+1][i_3+i_2] - B[i_3+1][i_2][i_3+2] - 6;
        }
    }
#pragma endscop
...
kernel_2222211225_07(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
