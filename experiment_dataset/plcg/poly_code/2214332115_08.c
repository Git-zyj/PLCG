...
void kernel_2214332115_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_Q, -i_0+PB_L-1); i_1++) {
            A[i_1-2][i_1+i_0][0] = C[i_0][i_1] + A[i_1-2][i_1+i_0-1][0] - A[i_0-1][i_1+i_0+1][0] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
            B[i_2][i_3+2] = D[i_3] - C[i_3][i_2] + E[i_3][i_2+1] - C[i_2][i_3] + E[i_3][i_2] + 4;
        }
    }
#pragma endscop
...
kernel_2214332115_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
