...
void kernel_4214211323_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P-2, i_0); i_1++) {
            A[i_0][i_1] = C[i_1] * A[i_0][i_1+2] - A[i_0+1][i_1] - A[i_0][i_1-1] + A[i_0][i_1+1] - 4;
        }
    }
        ...
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
                for (int i_5 = 0; i_5 < PB_M; i_5++) {
                    B[i_3][i_4-1][i_4] = B[i_3][i_4][i_4+1] + 2;
                }
            }
        }
    }
...
kernel_4214211323_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
