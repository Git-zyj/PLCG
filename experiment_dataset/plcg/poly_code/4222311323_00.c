...
void kernel_4222311323_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                B[i_2+2][i_0-1] = C[i_1+1][i_0][i_2] - D[i_2][i_0][i_1+1] + B[i_2+2][i_0-3] + B[i_2][i_1] + 4;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            for (int i_4 = 3; i_4 < PB_M+1; i_4++) {
                B[i_3][i_4-1] = B[i_3][i_4-3] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4222311323_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
