...
void kernel_4214332321_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_P; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_P-2; i_3++) {
                    A[i_1+1][i_2-1][i_3+i_1] = C[1][i_1][i_2+i_1-2][2] * D[i_2+2][i_0-2][i_3] + A[i_3+3][i_2-1][i_3+i_1+2] + A[i_1+1][i_2-2][i_1+i_0] - A[i_2+1][i_3+1][i_2+i_1] + 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_M; i_5++) {
            B[i_4-1][i_5+1] = E[i_5] + F[i_4-1][i_5] * F[i_5+i_4][i_5] + B[i_4-1][i_5] * B[i_4-1][i_5-1] + B[i_5-1][i_4] * 5;
        }
    }
#pragma endscop
...
kernel_4214332321_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
