...
void kernel_4316131223_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-3; i_2++) {
                for (int i_3 = 1; i_3 < PB_M; i_3++) {
                    A[i_1][i_3][i_2+1][i_0] = D[i_3-1][i_1][i_0][i_0] - A[i_1][i_3][i_2+3][i_0] * 5;
                }
            }
        }
        B[i_0] = E[i_0] * E[i_0] + E[i_0-1] - E[i_0] + B[i_0-1] * 4;
    }
    for (int i_4 = 3; i_4 < PB_N-2; i_4++) {
        for (int i_5 = i_4; i_5 < min(-i_4+PB_L+3, PB_L-1); i_5++) {
            for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 1; i_7 < min(i_6, -i_5+PB_M+1); i_7++) {
                    C[i_4][i_7+i_5-1][i_6][0] = D[i_5][i_6][i_4][i_7] - F[i_6][i_5][i_5+1] + D[i_6+1][i_4-1][i_7][i_5-1] * F[i_6+1][i_7+i_5-1][i_6] + C[i_4+2][i_7+i_5-1][i_6][0] - C[i_6][i_5+i_4-3][i_7-1][0] * 3;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4316131223_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
