...
void kernel_3324331315_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        A[0] = D[i_0+1] + E[i_0] - A[0] + 5;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_Q; i_2++) {
                C[i_2][i_2+i_0] = F[i_0][i_2-1][i_1+1] + F[i_1][1][i_0] - G[i_0][i_1][i_2] * H[i_2+1][i_0] * 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_Q-1; i_4++) {
            C[i_4][i_4+i_3] = H[0][i_3+1] - A[0] - A[0] + C[i_4+1][i_4+i_3+1] - C[i_4][i_4+i_3-1] - C[i_4+1][i_4+i_3] + 6;
        }
    }
#pragma endscop
...
kernel_3324331315_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
