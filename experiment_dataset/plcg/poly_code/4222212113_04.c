...
void kernel_4222212113_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            A[i_0-2][i_1] = A[i_0-2][i_1+1] - A[i_0-3][i_1-1] * 2;
        }
    }
        ...
        for (int i_3 = max(2, i_2); i_3 < PB_Q; i_3++) {
            for (int i_4 = 0; i_4 < PB_Q; i_4++) {
                for (int i_5 = 1; i_5 < PB_Q; i_5++) {
                    B[i_4][i_3][i_2][0] = C[i_5-1][i_3-2][i_2][i_5-1] - 3;
                }
            }
        }
    }
...
kernel_4222212113_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
