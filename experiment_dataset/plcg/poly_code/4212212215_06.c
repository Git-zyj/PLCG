...
void kernel_4212212215_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_P, -i_0+PB_L); i_1++) {
            for (int i_2 = 2; i_2 < PB_Q; i_2++) {
                A[i_1-2][i_1-1][i_2] = C[i_2][i_2-2][i_1+i_0] - C[i_0][i_2][i_1] * A[i_1-1][i_1][i_2] * 6;
                for (int i_3 = 0; i_3 < PB_Q-1; i_3++) {
                    A[i_2-2][i_2-1][i_3+1] = A[i_2-2][i_2-1][i_1-1] * 4;
                }
            }
        }
    }
...
kernel_4212212215_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
