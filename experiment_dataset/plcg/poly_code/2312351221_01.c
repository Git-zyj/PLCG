...
void kernel_2312351221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_0][i_1-1] = D[i_0+1][i_1] + E[i_0] + F[i_1][i_1+1] + D[i_0][i_1] + D[i_1-1][i_0] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 2; i_3 < min(-i_2+PB_L, PB_M); i_3++) {
            B[i_2][i_3] = G[i_3][i_2+1] - D[i_3+i_2][0] * D[i_2+1][i_3-1] + D[i_2][i_3+1] - B[i_2-2][i_3] + B[i_2+1][i_3] + B[i_2-1][i_3-2] * B[i_2+2][i_3] * 6;
        }
        for (int i_4 = i_2; i_4 < -i_2+PB_M; i_4++) {
            C[i_4+i_2][i_4] = G[i_4+1][i_2] + F[i_4][i_2] * F[i_4][i_2] + F[i_4][i_2] * 5;
        }
    }
#pragma endscop
...
kernel_2312351221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
