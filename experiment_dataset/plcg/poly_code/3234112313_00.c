...
void kernel_3234112313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                B[i_1][i_0-1] = C[i_2][i_2-2] * B[i_1][i_0-2] - B[i_1-1][i_0-2] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3234112313_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
