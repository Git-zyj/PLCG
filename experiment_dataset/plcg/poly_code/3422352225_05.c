...
void kernel_3422352225_05(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            C[i_0][i_1+2] = E[i_1] * F[i_1-1][i_0+1] - G[i_1][i_0] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = PB_M - 1; i_3 >= 0; i_3--) {
            C[i_2][i_3] = E[i_3] - H[i_2+1][i_3] * F[i_3+2][i_2] + 1;
        }
        for (int i_4 = 0; i_4 < min(-i_2+PB_M+2, PB_M-2); i_4++) {
            C[i_2][i_4+2] = G[i_2+2][2] * H[i_2][i_4] * E[i_4] * G[i_2][i_4] * F[i_4][i_2] + E[i_4] - F[i_4][i_2] * G[i_4+i_2-2][i_2] - C[i_2-1][i_4+1] * C[i_4][i_2+1] - 2;
        }
    }
    for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
        D[i_5-2] = I[i_5] * G[i_5][i_5] * E[i_5] - I[i_5+1] + D[i_5] * 4;
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3422352225_05(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
