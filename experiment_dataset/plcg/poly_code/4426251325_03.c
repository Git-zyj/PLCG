...
void kernel_4426251325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] - E[i_0+1] + E[i_0-1] * F[i_0-1] - A[i_0+2] + A[i_0+1] * 6;
    }
        ...
        for (int i_2 = 1; i_2 < PB_P-3; i_2++) {
            for (int i_3 = 1; i_3 < min(i_1, -i_2+PB_M); i_3++) {
                for (int i_4 = 1; i_4 < PB_L-2; i_4++) {
                    B[i_1-1][i_4+1][i_2+1][i_3] = G[i_3][i_4-1][i_1][0] - G[i_2][i_1][i_1][i_3] * H[i_3][i_1][i_4-1][i_2] + I[i_3+i_2][i_2+1][0] - B[i_1-1][i_4+1][i_2+1][i_3+2] + B[i_1+1][i_2+1][i_4+1][i_3+2] + B[i_1-1][i_4-1][i_2+3][i_3-1] - B[i_1][i_4+2][i_2-1][i_3+2] * 1;
                }
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
        for (int i_6 = i_5; i_6 < -i_5+PB_M; i_6++) {
            C[i_5][i_5+1] = J[i_5+1][i_6] + K[i_6+i_5][0] + J[i_6][i_5+1] * L[i_5][i_6][i_5] + J[i_5][i_5] * J[i_5][i_6] - J[i_5][i_6] - J[i_5][i_6+1] - C[i_5-1][i_5] - D[1] + D[1] + 2;
        }
        D[1] = K[i_5][0] + F[i_5-1] * F[i_5] * F[i_5] - C[i_5+1][i_5+2] - C[i_5-2][i_5-1] - D[1] * 6;
    }
#pragma endscop
...
kernel_4426251325_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
