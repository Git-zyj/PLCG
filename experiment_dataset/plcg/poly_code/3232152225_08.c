...
void kernel_3232152225_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
            A[i_1] = C[i_0] + D[i_1][i_0] - D[i_1][i_0+1] * D[i_0][i_1] - A[i_1+1] + A[i_1-2] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        B[i_2] = C[i_2] - C[i_2-2] * C[i_2] * C[i_2+2] - C[0] * C[i_2] - A[i_2] * B[i_2+1] * 3;
    }
#pragma endscop
...
kernel_3232152225_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
