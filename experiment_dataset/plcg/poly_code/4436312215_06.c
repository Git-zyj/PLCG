...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_M-1; i_2++) {
            B[i_2] = B[i_1+1] * B[i_2-1] + B[i_2+1] * B[i_2-1] + 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_L+2; i_4++) {
            for (int i_5 = 2; i_5 < PB_M; i_5++) {
                B[i_3+1] = F[i_3][i_5] * G[i_5-2][i_5][i_3][i_4-2] * 4;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M+1; i_6++) {
        for (int i_7 = 0; i_7 < PB_M; i_7++) {
            for (int i_8 = 0; i_8 < min(PB_Q-1, -i_6+PB_M+1); i_8++) {
                for (int i_9 = 2; i_9 < i_8; i_9++) {
                    D[i_8+i_6-1][i_8][i_7][i_9-2] = H[i_7][i_9][i_8+1] * 6;
                }
            }
        }
    }
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436312215_06(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
