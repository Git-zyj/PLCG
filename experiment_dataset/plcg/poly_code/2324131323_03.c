...
void kernel_2324131323_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0+1] = D[i_0] + D[i_0] + D[i_0] + A[i_0] * A[i_0+2] + A[i_0-1] - 3;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            B[i_2][i_1] = E[i_2][i_1] - E[i_1][i_2] * B[i_2+2][i_1] - B[i_2][i_1-2] + 3;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_N; i_4++) {
            A[i_4+1] = E[i_4+i_3][i_4] - E[i_4-1][i_3] + 4;
        }
    }
#pragma endscop
...
kernel_2324131323_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
