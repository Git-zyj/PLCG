...
void kernel_4426332221_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
        ...
        for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, PB_M-2); i_2++) {
                for (int i_3 = 1; i_3 < PB_M; i_3++) {
                    A[i_0][i_1-1][i_3-1][i_0] = E[i_3][i_1][i_0][i_2] + E[i_2+2][i_3-1][i_1+1][i_0] * 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_Q; i_4++) {
        for (int i_5 = i_4; i_5 < min(PB_Q-1, -i_4+PB_M); i_5++) {
            for (int i_6 = 2; i_6 < -i_5+PB_P; i_6++) {
                for (int i_7 = 2; i_7 < min(min(min(i_4, -i_4+PB_P), -i_6+PB_L), PB_M-2); i_7++) {
                    B[i_5][i_4-1][i_7-2][i_6+i_5] = E[i_7+2][i_4][i_5+1][i_6-2] - E[i_5][i_7+i_4][i_7-1][i_6] + F[i_5+i_4][i_7+i_6][i_6][i_7+i_4][1] - 6;
                }
            }
        }
    }
        ...
        for (int i_9 = i_8; i_9 < PB_Q; i_9++) {
            for (int j_0 = 2; j_0 < PB_M-1; j_0++) {
                for (int j_1 = 0; j_1 < -j_0+PB_Q; j_1++) {
                    C[i_9][i_8][j_1][j_0] = G[j_0][i_8-1][j_1][i_9] * H[i_9][j_0][j_1][i_8] + E[i_8][j_1+j_0-2][i_9][j_1+j_0] + C[j_1+1][i_8+2][j_0][i_9] - C[i_9][j_1][i_8+1][j_0] * C[j_0+1][j_1][i_9][i_8] * 3;
                }
            }
        }
        D[i_8] = I[0] * I[i_8-1] * J[i_8] - D[i_8+2] - D[i_8+1] - 1;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4426332221_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
