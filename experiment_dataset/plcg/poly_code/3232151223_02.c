...
void kernel_3232151223_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1] = C[i_1+i_0] - D[i_1][i_0] - D[i_1-1][i_0] - A[i_1-2] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-3; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
            for (int i_4 = 0; i_4 < i_3; i_4++) {
                B[i_2][i_4][i_3+1][i_3] = E[i_2][i_4][i_3] - E[i_3-1][i_2][1] * E[i_3][i_4][i_3] - D[i_4+i_2][i_3] * E[i_3][i_4][i_2-1] + E[i_3+i_2][i_4+1][i_3-1] + E[i_4+i_3+1][i_4+1][i_2] + B[i_4][i_2+1][i_3+1][i_3] * B[i_3][i_4][i_2+3][i_2+2] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3232151223_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
