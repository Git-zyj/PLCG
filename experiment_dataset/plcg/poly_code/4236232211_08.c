...
void kernel_4236232211_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                A[i_0-2][i_2+1][i_1+i_0] = C[i_1+1][i_2][i_0] * C[i_2+i_1][i_0-2][i_0] + C[i_2][i_0][i_1] - D[i_2][i_1-2] - A[i_1-2][i_2][i_1+i_0] * 4;
            }
        }
    }
        ...
        for (int i_4 = i_3; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
                for (int i_6 = 0; i_6 < PB_N; i_6++) {
                    B[i_3-2][i_3][i_6][i_5] = C[i_6][i_4+1][i_5] * B[i_5-1][i_5+1][i_4][i_6] * B[i_3-2][i_3][i_6][i_4-1] + 5;
                }
            }
        }
    }
...
kernel_4236232211_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
