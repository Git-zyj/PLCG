        ...
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_M-1; i_3++) {
                    A[i_1+i_0-1][i_1][i_3][i_2] = D[i_3][i_3][i_1-1][i_0] + E[i_3-1][i_2][0] - D[i_1+i_0][i_2][i_1][i_3] * A[i_3+i_1+1][i_3+2][i_0-2][i_2] - 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        B[i_4+1] = F[i_4+1] - F[i_4] - F[i_4] * F[i_4+1] * B[i_4+2] + B[i_4-1] - 2;
    }
    for (int i_5 = 0; i_5 < PB_M-3; i_5++) {
        B[i_5+1] = F[i_5] * F[i_5] - B[i_5+3] + B[i_5+1] * 6;
    }
#pragma endscop
...
kernel_4336131325_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
