...
void kernel_4236131311_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_L-1); i_1++) {
            B[i_0-1][i_1+1] = C[i_0][i_1] * C[i_0][0] * D[i_0-1] - C[i_1-1][i_0] + 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L, i_2); i_3++) {
            for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
                B[i_3-1][i_4] = C[i_2-1][i_3] - E[i_3][i_2-1][i_4] * B[i_3-2][i_4-1] - B[i_2-2][i_4-1] * B[i_3-1][i_4+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4236131311_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
