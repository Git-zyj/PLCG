...
void kernel_4212211125_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_2 = 3; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_Q-1, i_2); i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                B[i_2-1][i_4][i_3] = B[i_2-3][i_4][i_3+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4212211125_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
