...
void kernel_3232112123_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_Q+1, -i_3+PB_L-1); i_4++) {
                B[i_2][i_3][i_4-1] = C[i_2][i_4+i_3+1][i_4] * B[i_2][i_3+1][i_4-1] * B[i_2+1][i_3+1][i_4-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3232112123_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
