...
void kernel_3324131123_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0-1] + D[i_0-1] - D[i_0] + D[i_0-1] - 1;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L; i_2++) {
            B[i_1+1][i_2] = E[i_1][i_2] * E[i_1][i_2] * E[i_2][i_1] + E[i_1+1][i_2] * B[i_1+2][i_2] - 2;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M; i_4++) {
            A[i_4] = E[i_4+i_3][i_4] + 2;
        }
    }
#pragma endscop
...
kernel_3324131123_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
