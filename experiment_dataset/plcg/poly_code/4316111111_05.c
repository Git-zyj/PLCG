...
void kernel_4316111111_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
                A[i_1][i_0][i_2-1][i_2] = A[i_0+1][i_2+1][i_1-1][i_1] + 5;
            }
            B[i_0+1][i_1] = D[i_1][i_0] - 4;
            for (int i_3 = 0; i_3 < i_1; i_3++) {
                C[i_1+i_0][i_3][i_3+1] = 5;
            }
        }
    }
#pragma endscop
...
kernel_4316111111_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
