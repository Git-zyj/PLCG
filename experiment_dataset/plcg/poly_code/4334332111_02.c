...
void kernel_4334332111_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_Q-1; i_3++) {
                    A[i_0][i_1][i_2][i_3+i_0+1] = D[i_3][i_2+1][i_0][i_1] + E[i_2][i_0-2][i_3][i_1] + A[i_0-1][i_2][i_1][i_3+i_0-1] * 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
            B[i_5-1][i_5+2] = F[i_5][i_5] + G[i_4+2][i_5-2] - G[i_5][i_4] * 2;
        }
        for (int i_6 = 0; i_6 < min(PB_L, -i_4+PB_Q+1); i_6++) {
            for (int i_7 = 0; i_7 < PB_L; i_7++) {
                C[i_6][i_6] = F[i_4][i_6+i_4-1] + H[i_4][i_7][i_6] - H[i_7][i_4][i_6] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4334332111_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
