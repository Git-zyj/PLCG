...
void kernel_4222212321_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            for (int i_2 = 3; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < PB_L; i_3++) {
                    A[i_3][i_2-1][i_1-1] = A[i_1][i_3-1][i_2] - A[i_1][i_3-1][i_2-3] + A[i_1][i_3][i_2-1] * A[i_3][i_0-1][i_2+1] + 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        B[i_4] = C[i_4] * B[i_4+2] + B[i_4-1] + 3;
    }
#pragma endscop
...
kernel_4222212321_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
