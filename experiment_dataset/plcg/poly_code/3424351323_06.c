...
void kernel_3424351323_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        A[i_0] = E[i_0-1] - A[i_0+2] - 6;
        for (int i_1 = 1; i_1 < min(-i_0+PB_N-1, i_0); i_1++) {
            B[i_1][i_0+1] = F[i_1][i_0] * G[i_1][i_0-1] * G[i_1+1][i_0+1] - G[i_1+i_0][i_1] * F[i_1+i_0+1][0] * F[i_0][i_1] + H[i_0][i_1-1] * A[i_1] + B[i_1+1][i_0+1] - B[i_1][i_0+2] * 4;
        }
        for (int i_2 = 1; i_2 < min(-i_0+PB_M, PB_N); i_2++) {
            C[i_0+1][i_2] = F[i_2][i_2+i_0] * H[i_0][i_2] - I[i_2+1] - H[i_0+1][i_2+1] - J[i_2+1][i_0][i_0+1] + F[i_0-1][i_2-1] * C[i_0+1][i_2-1] + C[i_2+3][i_0] - C[i_0-1][i_2] + 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = i_3; i_4 < PB_N; i_4++) {
            C[i_3][i_4+2] = F[i_4][i_3] - F[i_3][i_4] + H[i_4-1][i_3-1] + H[i_4][i_4] - 4;
        }
    }
#pragma endscop
...
kernel_3424351323_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
