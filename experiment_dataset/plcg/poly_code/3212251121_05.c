...
void kernel_3212251121_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_M, -i_1+PB_M-1), PB_N); i_2++) {
                A[i_2][i_1-1] = C[i_0-1][0][i_1] - D[i_2+i_1+1][i_1+i_0+1] + C[i_2][i_0][i_1+i_0] + E[i_2+i_0][i_1][i_1] - A[i_2][i_1] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_L; i_4++) {
            A[i_3][i_4-1] = D[i_4-1][0] + F[i_4+i_3][i_4-1] - F[i_4][i_3-1] - D[i_4][i_3+1] - D[i_4+1][1] - 6;
        }
    }
#pragma endscop
...
kernel_3212251121_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
