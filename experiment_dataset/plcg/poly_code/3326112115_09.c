...
void kernel_3326112115_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L+2; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_L+2, PB_M); i_1++) {
            A[i_1][i_1+i_0-2] = A[i_1-1][i_1+i_0-3] + 6;
        }
        for (int i_2 = i_0; i_2 < min(PB_M+1, -i_0+PB_L+3); i_2++) {
            A[i_2-1][i_2+i_0-3] = D[i_0+1] * 3;
        }
    }
...
kernel_3326112115_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
