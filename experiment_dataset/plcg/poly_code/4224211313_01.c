...
void kernel_4224211313_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P+1; i_2++) {
                for (int i_3 = 1; i_3 < min(PB_M+1, i_1); i_3++) {
                    A[i_3-1][i_0-1][i_2+i_1-1] = 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        B[i_4] = C[i_4] - B[i_4+1] * B[i_4-1] - B[i_4-1] + B[i_4+1] - B[i_4+1] * 4;
    }
#pragma endscop
...
kernel_4224211313_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
