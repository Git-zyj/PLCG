...
void kernel_3226112323_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
                A[i_2+2][i_1] = A[i_1+1][i_2] + A[i_2+2][i_1+2] - A[i_0+2][i_2-1] * 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-4; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            A[i_3+4][i_4] = C[i_4][i_3] - 4;
        }
    }
#pragma endscop
...
kernel_3226112323_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
