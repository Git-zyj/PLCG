...
void kernel_2232331313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M-2; i_3++) {
            B[i_3+i_2][0] = C[i_3] - C[i_3] + D[i_3+1][i_2] + D[i_3-1][i_2+1] * E[i_3][i_2] - B[i_3+i_2+1][0] + B[i_3+i_2+1][0] - B[i_3+i_2+2][0] - 4;
        }
    }
#pragma endscop
...
kernel_2232331313_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
