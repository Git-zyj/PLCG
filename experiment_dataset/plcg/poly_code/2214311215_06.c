...
void kernel_2214311215_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
            A[i_2+1][i_3-1] = C[i_3][i_3+i_2] * A[i_2+2][i_3-1] * 4;
        }
    }
#pragma endscop
...
kernel_2214311215_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
