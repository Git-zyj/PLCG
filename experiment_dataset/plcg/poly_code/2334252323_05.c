...
void kernel_2334252323_05(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+1; i_0++) {
        C[i_0-1] = D[i_0+2] * D[i_0] + D[i_0+2] + D[i_0] * E[i_0-2] + D[i_0] + C[i_0-1] - 1;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        C[i_1+1] = E[i_1] * D[0] + E[i_1] + D[i_1] - D[i_1] * 5;
    }
    for (int i_2 = PB_M+1 - 1; i_2 >= 3; i_2--) {
        C[i_2-1] = D[i_2] * E[i_2] + D[i_2-2] * E[i_2] + C[i_2-2] - C[i_2-3] - 1;
    }
#pragma endscop
...
kernel_2334252323_05(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
