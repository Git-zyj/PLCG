...
void kernel_3422312311_07(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        D[i_0] = E[i_0] + D[i_0] * 4;
        for (int i_1 = 3; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(PB_P-1, -i_0+PB_L+1), -i_1+PB_L+2); i_2++) {
                B[i_0][i_2+i_1-2][i_2+1] = B[i_0][i_2+i_1-3][i_2+1] + B[i_1+1][i_2+i_0-1][i_2+1] * 3;
            }
        }
    }
...
kernel_3422312311_07(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E));
