...
void kernel_4422151325_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_N, PB_M-2); i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_L, PB_L-1); i_3++) {
                    B[i_1][i_1] = E[i_3+i_2][i_0][i_1][i_0-1] + E[i_3+1][i_2][i_0][0] - 5;
                }
            }
            B[i_1][i_1] = F[i_1+i_0][i_0-1] * G[i_0+1] + F[i_1][i_0+1] - B[i_1+1][i_1+1] - B[i_1-2][i_1-2] + B[i_1+2][i_1+2] - 1;
        }
    }
    for (int i_4 = 1; i_4 < PB_Q; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 0; i_6 < min(PB_Q-1, i_5); i_6++) {
                B[i_6][i_6] = H[i_5][i_6][0] - F[i_5][0] - H[i_6+1][0][i_4] - H[i_5+1][i_4][i_6+1] * 2;
            }
        }
        for (int i_7 = 0; i_7 < -i_4+PB_Q+1; i_7++) {
            for (int i_8 = 0; i_8 < PB_M-2; i_8++) {
                B[i_8+2][i_8+2] = H[i_8][i_4][i_7] - H[i_7+1][i_8][i_7] - H[i_8][i_4-1][i_7] + H[i_8][0][i_7] + F[i_8][i_7] - H[i_7][i_8][i_4] + H[i_8+i_7][i_4][i_7+i_4-1] - H[i_4-1][i_7][i_8] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422151325_08(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
