...
void kernel_3234212223_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            B[i_1-2][i_0+2] = C[i_0-1][i_1+1] * 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
            B[i_3-2][i_2] = C[i_2][i_3+2] * B[i_3-2][i_2-2] - B[i_2-1][i_3] - 1;
        }
    }
#pragma endscop
...
kernel_3234212223_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
