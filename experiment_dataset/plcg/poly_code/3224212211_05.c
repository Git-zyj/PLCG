...
void kernel_3224212211_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd)){
    ...
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        ...
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_1][i_0][i_2+2] = D[i_2][i_1-2][i_0+2][i_0] + B[i_1][i_0+1][i_2+2] + B[i_1-1][i_0][i_2+3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3224212211_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D));
