...
void kernel_2432231323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        B[i_1-1] = F[i_1+1] - E[i_1] - F[i_1+1] + E[i_1] + G[i_1][0] + E[i_1] - B[i_1] - 5;
        B[i_1-1] = E[i_1] + 6;
    }
    for (int i_2 = PB_L - 1; i_2 >= 0; i_2--) {
        for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
            D[i_3] = H[i_3][i_2] * B[i_3-1] * D[i_3-2] + D[i_3+1] + D[i_3+2] - 5;
        }
    }
#pragma endscop
...
kernel_2432231323_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
