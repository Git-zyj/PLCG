...
void kernel_2234211213_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = 1; i_2 < min(min(i_0, PB_L-1), -i_0+PB_N); i_2++) {
            B[i_2][i_2] = C[i_2][i_0-1] - D[i_2+i_0][i_2+1] + B[i_2+1][i_2+1] * B[i_2-1][i_2-1] * 3;
        }
    }
#pragma endscop
...
kernel_2234211213_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
