...
void kernel_3224212313_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        ...
        for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
            B[i_0][i_3] = C[i_3-1][i_3-2] - B[i_0-1][i_3] * B[i_0][i_3+1] + 5;
        }
    }
#pragma endscop
...
kernel_3224212313_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
