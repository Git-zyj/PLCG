...
void kernel_4236311115_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                for (int i_3 = 2; i_3 < i_1; i_3++) {
                    A[i_3][1][i_2][i_3-1] = A[i_3-1][1][i_2-1][i_3-2] * 4;
                }
            }
        }
        B[i_0+1] = C[i_0] + 3;
...
kernel_4236311115_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
