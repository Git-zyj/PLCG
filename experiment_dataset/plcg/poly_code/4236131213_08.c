...
void kernel_4236131213_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1] = C[i_0][i_1] * C[i_0][i_1] * A[i_1-1] - A[i_1+1] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            A[i_3] = D[i_2] * D[i_3+1] * D[0] + C[i_3][i_2+1] + 3;
        }
    }
#pragma endscop
...
kernel_4236131213_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
