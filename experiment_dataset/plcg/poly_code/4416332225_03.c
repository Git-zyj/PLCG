...
void kernel_4416332225_03(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-2; i_1++) {
            D[i_0-3][i_1] = E[i_1][i_0-1] * F[i_0+1][i_1] - 1;
            for (int i_2 = i_0; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_M-1, -i_1+PB_P); i_3++) {
                    B[i_3+2][i_1][i_1-2][1] = G[i_2][i_3][i_0][i_1] * G[i_2][i_0][i_1+2][i_3] * H[i_1-2][i_0+1][i_3+1][i_1-1] - H[i_2-1][i_0][i_1+2][i_3+1] + G[i_3+i_0+1][i_2][i_3+i_1][i_2] + B[i_3+1][i_0][i_0-2][1] - B[i_3+3][i_1][i_1-2][1] - 1;
                }
            }
        }
    }
        ...
        for (int i_5 = i_4; i_5 < PB_Q; i_5++) {
            for (int i_6 = 0; i_6 < -i_4+PB_Q-2; i_6++) {
                for (int i_7 = 0; i_7 < PB_L-2; i_7++) {
                    B[i_7+2][i_5][i_5-2][1] = G[i_5][i_7][i_4][i_6+i_4+2] * 4;
                }
            }
        }
        for (int i_8 = i_4; i_8 < PB_L-2; i_8++) {
            D[i_4-1][i_8] = F[i_8][i_4+2] - D[i_4-1][i_8-2] - D[i_4-2][i_8+2] - 2;
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4416332225_03(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
