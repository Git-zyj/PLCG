...
void kernel_2326332325_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = D[i_0+1] * D[i_0+2] - E[i_0] * A[i_0+3] + A[i_0] - A[i_0+4] * 1;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_Q-1; i_2++) {
            B[i_1-2][i_2-1] = B[i_1-2][i_2+1] * B[i_1-2][i_2-2] + 6;
        }
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            B[i_1-2][i_3+1] = F[i_3-1][i_1+1] + G[i_3][i_1][i_1] * H[i_3][i_1] - H[i_3][i_1] - 3;
        }
    }
#pragma endscop
...
kernel_2326332325_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
