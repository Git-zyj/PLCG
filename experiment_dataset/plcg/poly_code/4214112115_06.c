...
void kernel_4214112115_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        ...
        for (int i_3 = 0; i_3 < min(i_0, PB_M-2); i_3++) {
            for (int i_4 = max(2, i_3); i_4 < min(-i_0+PB_L, PB_M-2); i_4++) {
                B[i_3+2][i_4-2] = C[i_4][i_4+i_0][i_3+1] - B[i_4+2][i_0-3] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4214112115_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
