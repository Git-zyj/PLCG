...
void kernel_2336111311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0] = D[i_0][i_1] + A[i_0+1] - A[i_0-1] + A[i_1+1] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_M-1); i_3++) {
            B[i_3+i_2][i_3+1] = D[i_3-1][i_2] - B[i_3+i_2-1][i_3+1] - B[i_3+i_2+1][i_3+2] + 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = i_4; i_5 < min(PB_M, -i_4+PB_P-2); i_5++) {
            C[i_4][i_5+i_4] = D[i_5][i_4] - C[i_4-1][i_5+i_4-1] + C[i_4][i_5+i_4+1] - C[i_4+1][i_5+i_4+2] * C[i_4][i_5+i_4-1] + 3;
        }
    }
#pragma endscop
...
kernel_2336111311_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
