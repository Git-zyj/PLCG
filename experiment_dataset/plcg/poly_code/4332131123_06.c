...
void kernel_4332131123_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_N; i_2++) {
            C[i_1-1] = E[i_2][i_1] * D[i_2+i_1] + C[i_2+1] - 4;
        }
        C[i_1] = D[i_1] * D[i_1] - 2;
    }
#pragma endscop
...
kernel_4332131123_06(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
