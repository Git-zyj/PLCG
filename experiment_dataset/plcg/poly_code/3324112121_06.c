...
void kernel_3324112121_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        ...
        B[i_0] = D[i_0] + D[i_0] - D[0] - 3;
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_P+1; i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                C[i_5-1][i_3-1][i_4-1] = C[i_5-1][i_3+1][i_4-1] * C[i_5][i_3-1][i_4-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3324112121_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
