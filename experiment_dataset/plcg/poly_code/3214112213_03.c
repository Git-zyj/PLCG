...
void kernel_3214112213_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        ...
        for (int i_1 = 0; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_0][i_1][i_2] = C[i_0][i_1] + B[i_0-1][i_1+1][i_2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3214112213_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
