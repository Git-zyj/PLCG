...
void kernel_3322211111_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        ...
        for (int i_2 = 0; i_2 < -i_0+PB_P; i_2++) {
            for (int i_3 = 0; i_3 < min(PB_N, -i_0+PB_M-1); i_3++) {
                B[i_3][i_2][i_2+i_0] = D[i_2][i_3+i_0+1][1] - 1;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L+1; i_4++) {
        for (int i_5 = 2; i_5 < PB_P; i_5++) {
            C[i_4-1][i_5-1] = C[i_4-2][i_5-2] + C[i_4-1][i_5] * 5;
        }
    }
#pragma endscop
...
kernel_3322211111_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
