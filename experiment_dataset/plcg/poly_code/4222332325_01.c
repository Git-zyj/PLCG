...
void kernel_4222332325_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1+1] = C[i_1][i_0] * D[i_1][i_0] - A[i_1-1] + A[i_1] + A[i_1+3] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < min(min(PB_M-3, -i_2+PB_N), i_2); i_3++) {
            A[i_3+3] = E[i_2] - C[i_3+i_2][1] - C[i_3][i_2-2] - 6;
        }
    }
#pragma endscop
...
kernel_4222332325_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
