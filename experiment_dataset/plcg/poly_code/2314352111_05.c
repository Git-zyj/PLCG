...
void kernel_2314352111_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1] = D[i_1] * E[i_1] - F[i_1][i_1] - A[i_1-1] * 1;
        }
        B[i_0-1] = D[i_0-2] + E[i_0+2] + E[i_0] * D[i_0] - D[i_0-1] - G[0][i_0] + H[0] * 4;
    }
    for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            B[i_3-1] = F[i_3][i_2] * I[i_2-1][0] + G[i_3][i_2+2] - B[i_3-1] - 6;
        }
    }
#pragma endscop
...
kernel_2314352111_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
