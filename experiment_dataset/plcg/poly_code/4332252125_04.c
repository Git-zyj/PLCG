...
void kernel_4332252125_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            A[i_0][i_1] = D[i_0-1][i_1-2] - D[i_1+i_0+1][1] * D[i_1][i_0] + E[i_0] * D[i_0+2][i_1] - F[i_1+2][i_0+1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L-2, -i_2+PB_M); i_3++) {
            for (int i_4 = 2; i_4 < PB_P-2; i_4++) {
                B[i_3+i_2][i_4][i_4+2] = G[i_3-2][i_4-2][i_3+2] - G[i_4][i_2][i_3+2] * G[0][i_2-1][i_3] - B[i_3+i_2][i_4+2][i_4+4] - 3;
            }
        }
        for (int i_5 = i_2; i_5 < PB_Q-2; i_5++) {
            C[i_5][i_2] = E[i_5+1] * F[i_5][i_2-1] + F[i_5+1][i_2+2] * H[i_5+2][i_2][2] - C[i_5+2][i_2] - C[i_2][i_5+2] - 3;
        }
    }
#pragma endscop
...
kernel_4332252125_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
