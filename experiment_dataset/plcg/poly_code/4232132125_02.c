...
void kernel_4232132125_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            B[i_0][i_0] = C[i_1][i_0][i_1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_Q; i_4++) {
                B[i_3][i_3] = C[i_3][i_2][i_4] + C[i_4][i_2][i_4+i_3] + C[i_3][i_4+1][i_2-1] * C[i_3][i_4][i_2] * B[i_3-2][i_3-2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_4232132125_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
