...
void kernel_2234132225_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_0][i_1] = C[i_1][i_0] + C[i_0+2][i_1] * C[i_0-2][i_1] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            B[i_2][i_3] = C[i_2-2][i_3] - C[i_3][i_2] * B[i_2][i_3-1] + B[i_2][i_3-2] + 4;
        }
    }
#pragma endscop
...
kernel_2234132225_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
