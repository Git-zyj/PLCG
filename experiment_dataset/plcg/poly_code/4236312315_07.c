...
void kernel_4236312315_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_M+2; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    A[i_1-2][i_2+1][i_3] = A[i_1-2][i_3][i_2] - 3;
                }
            }
        }
        B[i_0+1] = C[i_0] - D[i_0] * B[i_0+2] - B[i_0] * B[i_0] * B[i_0+2] + B[i_0] + 1;
    }
#pragma endscop
...
kernel_4236312315_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
