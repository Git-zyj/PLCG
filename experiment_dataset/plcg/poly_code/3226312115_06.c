...
void kernel_3226312115_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            A[i_1][i_1+i_0] = C[i_1][i_1-1] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < min(-i_2+PB_L, i_2); i_3++) {
            for (int i_4 = 1; i_4 < min(min(PB_P, -i_3+PB_L+2), -i_2+PB_M); i_4++) {
                B[i_4+i_2][i_3+i_2][0] = A[i_4-1][i_4+i_3-2] - B[i_4+i_2-2][i_3+i_2][0] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3226312115_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
