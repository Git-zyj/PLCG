...
void kernel_4216212215_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_P-1); i_1++) {
            for (int i_2 = min(-i_0+PB_L, PB_P-2) - 1; i_2 >= 0; i_2--) {
                B[1][i_2][i_1] = C[i_1][i_2+i_0][i_2+2] - 1;
                B[1][i_2][i_1] = B[1][i_2][i_1+1] * B[1][i_2+1][i_1+1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4216212215_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
