...
void kernel_4414232325_08(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_Q; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_M; i_2++) {
                C[i_2] = E[i_1-2][i_2][i_1+i_0] - F[i_1][i_2] * 3;
                C[i_2-2] = F[i_0+2][i_2+i_1] + E[i_0][i_2+i_1+2][i_0] * F[i_2][0] * 1;
            }
        }
        for (int i_3 = 2; i_3 < i_0; i_3++) {
            C[i_3] = G[i_0][i_3] * H[i_3] * H[i_3] - C[i_0+1] + C[i_3-2] + C[i_3+1] - 3;
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                C[i_3-1] = I[i_4+1][i_3][i_4] * I[i_4+1][i_3][i_0] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4414232325_08(xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
