...
void kernel_2226152125_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            B[i_0+1][i_1+1] = C[i_1][i_0] + C[i_1-1][i_0-1] * C[i_1][i_0+2] * D[i_1] - C[i_1][i_0] + D[i_1] + E[i_0][i_1-2][i_0] + D[i_1+2] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_2+1][i_3] = C[i_3-1][i_2] * C[i_3][i_2+1] - B[i_2][i_3] + 4;
        }
    }
#pragma endscop
...
kernel_2226152125_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
