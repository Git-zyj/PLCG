...
void kernel_4234111213_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            A[i_1] = C[i_1+1][i_0-1] + A[i_1+1] * A[i_1-1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                B[i_4+1][i_3][i_2-1] = B[i_4+1][i_2+1][i_3-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4234111213_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
