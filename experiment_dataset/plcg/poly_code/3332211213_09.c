...
void kernel_3332211213_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_L, -i_0+PB_L-1); i_2++) {
                A[i_2-1][i_2+i_1] = D[i_2][i_1] - A[i_0-1][i_2+i_0+1] + 5;
            }
        }
    ...
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = PB_P - 1; i_4 >= 0; i_4--) {
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                C[i_5][i_3][i_3+1] = C[i_5][i_3-1][i_3] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3332211213_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
