...
void kernel_4324352325_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_Q-1, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                for (int i_3 = 2; i_3 < min(min(-i_0+PB_Q, i_1), -i_2+PB_M); i_3++) {
                    B[i_2][i_3-2][i_1-3] = D[i_2][i_1][i_3][2] + E[i_0][i_2][i_1-1][i_1+1] - F[i_2][i_1][i_0-2][i_3+i_0] + D[i_2][i_3][i_0][i_1] - D[i_3+i_2][i_0+2][i_3+2][i_1] + 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P; i_4++) {
        for (int i_5 = 2; i_5 < min(PB_P-2, -i_4+PB_L); i_5++) {
            for (int i_6 = 2; i_6 < PB_M-1; i_6++) {
                B[i_5][i_6-2][i_4-2] = G[i_6][i_5+i_4][i_5+2] + H[i_6+i_5-2][i_4][i_4] * B[i_6+1][i_5][i_4-2] * B[i_5+1][i_6-2][i_4-2] - 4;
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_P; i_7++) {
        for (int i_8 = 2; i_8 < -i_7+PB_P; i_8++) {
            for (int i_9 = 2; i_9 < min(-i_7+PB_L, PB_P-1); i_9++) {
                B[i_7][i_8-2][i_9-2] = I[i_9+1][i_8] + J[i_8+1][i_9+i_7] * H[i_8-1][i_7][i_9+1] - G[i_8+i_7][i_7][i_9] * G[i_7][i_9+2][i_7] - H[i_7][i_9][i_8+i_7] - B[i_7][i_9-2][i_8-2] * 5;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4324352325_04(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
