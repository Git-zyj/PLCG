...
void kernel_3424111311_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[0][i_1-1] = E[i_1] - A[0][i_1] + A[0][i_0] + A[0][i_0-2] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        B[i_2] = E[i_2-1] * E[i_2+1] * A[0][i_2-1] + 2;
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            C[i_3][i_2-1] = C[i_3][i_2-2] - C[i_3][i_2] * C[i_3+1][i_2-1] - 6;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        D[i_4] = D[i_4-1] + D[i_4+1] - D[i_4+1] + D[i_4+1] - D[i_4+1] + 5;
    }
#pragma endscop
...
kernel_3424111311_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
