...
void kernel_2324211321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_N; i_2++) {
            B[i_1][i_2] = D[i_2][i_1+1] + B[i_1-1][i_2] * B[i_1+1][i_2] - B[i_1][i_2-2] * 1;
        }
    }
    for (int i_3 = 3; i_3 < PB_L-3; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_L-3, PB_M-2); i_4++) {
            C[i_4][i_4+i_3+1] = C[i_4+2][i_4+i_3+3] * C[i_4][i_4+i_3+2] * C[i_4-2][i_4+i_3-3] * 4;
        }
    }
#pragma endscop
...
kernel_2324211321_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
