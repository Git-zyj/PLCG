...
void kernel_2434252125_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        B[i_0] = E[i_0-1] * E[i_0] + F[1] + B[i_0] + 3;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        B[i_1] = E[i_1+2] - E[i_1] * 1;
    }
    for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            C[i_3][i_2-2] = G[i_3][i_2] * H[i_2+2][i_3][i_2] * G[i_2][i_3-2] + 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        D[i_4-1] = F[i_4] + F[i_4] - G[i_4][i_4] + F[i_4] - F[i_4] - E[i_4] * E[i_4] - E[i_4-1] + E[i_4+1] - F[i_4] * 3;
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_2434252125_08(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
