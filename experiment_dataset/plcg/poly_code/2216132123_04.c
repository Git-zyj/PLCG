...
void kernel_2216132123_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 4; i_1 < PB_M-2; i_1++) {
            A[i_1-2] = C[i_1+2][i_0-2] - C[i_1][i_0+2] + C[i_0+2][i_0] - C[i_1][i_0-2] + C[i_1][i_0] - A[i_1-4] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        B[i_2] = D[i_2+2] + A[i_2-2] - 4;
    }
#pragma endscop
...
kernel_2216132123_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
