...
void kernel_3434351313_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P-1; i_1++) {
            A[i_0][i_1] = E[i_1][i_0] + F[i_0+1][0] - A[i_0-1][i_1] + A[i_0][i_1-1] * A[i_0][i_1+1] + A[i_0+1][i_1-1] - 2;
        }
        for (int i_2 = i_0; i_2 < PB_N-1; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_0+PB_L-1, -i_2+PB_P); i_3++) {
                B[i_3+i_0+1][i_2][i_3+i_2] = G[i_0+1][i_2+1][i_3] * E[i_3][i_2+1] - E[i_3-1][i_2] - H[i_3][i_2][i_0] + G[i_3][i_0][i_0] - I[i_0+1][i_3+i_2] * J[i_2][i_0][0] - B[i_3+i_0][i_2][i_3+i_2-1] - 1;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N; i_4++) {
        for (int i_5 = 0; i_5 < min(PB_N, -i_4+PB_L-2); i_5++) {
            for (int i_6 = 1; i_6 < min(-i_5+PB_P, -i_4+PB_M); i_6++) {
                B[i_5+i_4+2][i_6][i_6+i_5] = J[i_4][i_6][i_5] - G[i_4][i_5][i_6] + J[i_5][i_6-1][i_4] + H[i_6+i_4][i_6+i_5][0] + G[i_6][i_5][i_4] - 2;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_N; i_7++) {
        for (int i_8 = 1; i_8 < PB_P-1; i_8++) {
            A[i_7][i_8+1] = F[i_8][i_7] * G[i_7][i_8][i_7] * K[i_8-1] + E[i_8][i_7-1] - I[i_8][i_7-1] + E[i_7][i_8] - 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3434351313_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K)));
