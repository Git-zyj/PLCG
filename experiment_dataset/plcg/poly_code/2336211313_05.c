...
void kernel_2336211313_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_N-1; i_1++) {
            A[i_0][i_1+i_0-1] = D[i_1+i_0+1][i_0-1] - A[i_0][i_1+i_0-2] * A[i_0-1][i_1+i_0-3] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < min(min(-i_2+PB_P-1, i_2), PB_L-2); i_3++) {
            B[i_3+1][i_3+i_2] = B[i_3+1][i_3+i_2-1] + B[i_3+2][i_3+i_2+1] + B[i_2+1][i_3+i_2-1] * B[i_3][i_3+i_2-2] + B[i_2][i_3+i_2-1] + 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_L+1); i_5++) {
            C[i_4+1][i_5-1] = D[i_5][i_4] - A[i_4][i_5+i_4-1] * C[i_4+2][i_5-1] + 2;
        }
    }
#pragma endscop
...
kernel_2336211313_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
