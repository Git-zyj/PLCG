...
void kernel_4222112215_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_N, -i_2+PB_M-1); i_3++) {
            B[i_3+i_2] = C[i_3][i_2] - B[i_3+i_2+1] + B[i_3+i_2-1] + 5;
        }
    }
#pragma endscop
...
kernel_4222112215_08(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
