...
void kernel_2436251315_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] + E[i_0] - F[i_0] * F[i_0] * E[i_0+1] * F[i_0] * A[i_0+1] * A[i_0-1] * 3;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_0-1][i_1] = G[i_1][i_0-1] * H[i_0][i_1-1] + G[i_1][i_0] + A[i_0] * B[i_0][i_1-1] - B[i_0-2][i_1] * 4;
        }
        for (int i_2 = i_0; i_2 < min(-i_0+PB_M, PB_L-1); i_2++) {
            A[i_2-1] = G[i_0][i_2-1] + H[i_2-1][i_0] + H[i_2+i_0][0] * G[i_0+1][i_2+1] + A[i_2+1] * 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_L-1; i_4++) {
            D[i_3+1][i_4][i_4] = G[i_4-1][i_3-1] - H[i_3][i_4-1] * G[i_4+i_3][i_4] - H[i_3][i_4-1] - G[i_4][1] + H[i_4+i_3][1] + G[i_4+i_3+1][i_3-1] - D[i_3+2][i_4][i_4] + D[i_3+1][i_4+1][i_4+1] - 4;
        }
    }
#pragma endscop
...
kernel_2436251315_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
