...
void kernel_4222152315_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_Q, PB_M); i_1++) {
            for (int i_2 = 2; i_2 < min(PB_M, -i_0+PB_P); i_2++) {
                B[i_0-2][i_1][i_2] = C[i_2][i_1] + D[i_2-1][i_2+i_0][i_1+i_0] * D[i_2][i_0][i_1] + C[i_2][i_0+2] * D[i_2][i_2-2][i_1-1] + D[i_1][i_2][i_1+1] - C[i_2][i_1] + D[i_1][i_0-2][2] + 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_P, PB_M-1); i_4++) {
            for (int i_5 = 3; i_5 < PB_L; i_5++) {
                B[i_5-2][i_4][i_3] = C[i_4][i_5] - D[i_5][i_4+i_3][i_5] * B[i_5-2][i_4-1][i_3] * B[i_5-2][i_4+1][i_3] * B[i_5-3][i_4][i_3] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4222152315_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
