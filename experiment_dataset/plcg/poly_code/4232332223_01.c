...
void kernel_4232332223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M-3, i_0); i_1++) {
            A[i_0][i_1+2] = C[i_1-2][i_0] + D[i_1][i_0] - E[i_1+2] * A[i_0][i_1+3] + A[i_0][i_1] * 5;
        }
        for (int i_2 = 2; i_2 < PB_Q; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_2+PB_L+2, PB_Q-4); i_3++) {
                B[i_0][i_2][i_3+2] = C[i_3][i_2] + F[i_0][i_3+i_2-2][0] + B[i_0+2][i_2][i_3+4] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4232332223_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
