...
void kernel_3324352221_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            A[i_0][i_1] = D[i_0][i_1][i_0+2] * E[i_1][i_0-2] * F[i_0][i_1+i_0] * A[i_0+1][i_1-1] * 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            B[0] = F[i_2+1][i_3] + F[i_3][i_2-2] * F[i_3][i_2] - F[i_3+1][i_2] + E[i_2][i_3] + B[0] - 5;
            for (int i_4 = 1; i_4 < min(min(-i_3+PB_M, i_3), -i_2+PB_M); i_4++) {
                C[i_4+i_2][i_3][i_4+i_3] = G[i_3][i_4-1][i_2] - G[i_2][i_4+i_3][0] + D[i_4][i_3][i_2] + F[i_4][i_3] * H[i_4][i_4-1][i_2] + G[i_4+1][i_2-1][i_3] * B[0] + B[0] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3324352221_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
