...
void kernel_3426312311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_M+1; i_1++) {
            A[i_0-2] = A[i_1-1] - A[i_1-3] * A[i_1-3] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[i_2][i_3+1] = B[i_2-1][i_3+1] * B[i_3-1][i_2+2] * 1;
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
            D[i_4+1] = E[i_5][i_4-2] * A[i_5-2] * D[i_4] * D[i_5+1] + 1;
        }
        D[i_4] = D[i_4-1] - 5;
    }
#pragma endscop
...
kernel_3426312311_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
