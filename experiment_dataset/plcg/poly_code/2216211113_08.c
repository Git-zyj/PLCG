...
void kernel_2216211113_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = 1; i_2 < min(PB_N-1, i_0); i_2++) {
            B[i_2] = C[i_2][i_0+1] - B[i_2+1] + B[i_2-1] - 4;
        }
    }
#pragma endscop
...
kernel_2216211113_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
