...
void kernel_2232312311_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        ...
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            B[i_0][i_2] = C[i_2][i_0][i_0] + C[i_0+1][i_2][0] - B[i_0][i_2-1] + B[i_0-1][i_2+1] + 5;
        }
    }
#pragma endscop
...
kernel_2232312311_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
