...
void kernel_4416151111_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_L, -i_1+PB_N), PB_P-1); i_2++) {
                A[i_0-1][i_1][i_2] = E[i_1][i_0][i_2+1] - F[i_2+i_1][i_0] + E[i_2][i_2+i_0][i_1] - E[i_1][i_0][i_0] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        D[i_3] = G[i_3] - G[1] * G[i_3+1] - G[i_3] * G[i_3+1] - D[i_3+1] - 4;
        for (int i_4 = i_3; i_4 < min(-i_3+PB_M, PB_P-1); i_4++) {
            for (int i_5 = 1; i_5 < min(-i_3+PB_L, -i_4+PB_P+1); i_5++) {
                for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
                    A[i_4-1][i_3][i_6] = H[i_5][i_4+i_3][i_3][i_6+1] * H[i_5][i_6][i_5][i_4+1] * H[i_3][i_6+1][i_4][i_5+i_4-1] - H[i_4][i_6][i_5+i_3][i_6+1] * H[i_5-1][i_6+1][i_4][i_3-1] - 4;
                }
            }
        }
        D[i_3] = G[i_3+1] - G[i_3+1] - G[i_3] - G[i_3] + 5;
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4416151111_07(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
