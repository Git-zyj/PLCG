...
void kernel_3432251315_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M+1; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_L, PB_P-1); i_2++) {
                A[i_0][i_2][i_0-1] = E[i_2-1][i_0][0] * F[i_2+i_0][i_1][i_2] + G[i_1-1][0] * F[i_1][i_2+1][i_1-1] + E[i_1+i_0-1][1][i_2] - A[i_1][i_2-1][i_1-1] + A[i_0+1][i_2+1][i_0] + A[i_0-1][i_2-1][i_0-2] + A[i_2-1][i_0][i_2-2] * 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = 1; i_5 < i_4; i_5++) {
                B[i_4][i_5] = F[i_5+1][i_3][i_4] + E[i_4+1][i_3][i_5-1] + F[i_4][i_3][i_3-1] * H[i_4][i_5] * E[i_5][i_3+1][i_4-1] + F[i_4-1][i_5][i_3+1] + F[i_5][i_4][i_3+1] + B[i_5-1][i_4] - B[i_4+1][i_5] + 2;
            }
        }
        for (int i_6 = 1; i_6 < PB_P+1; i_6++) {
            A[i_3+1][i_6+1][i_3] = G[i_3+1][i_6] - H[i_6][i_3] + H[i_3+1][i_6-1] + 3;
        }
        for (int i_7 = 1; i_7 < PB_M; i_7++) {
            A[i_7][i_3+1][i_7-1] = H[i_7+1][i_3-1] * H[i_3-1][i_7] - G[i_3-1][i_7] - I[i_3+1] - 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3432251315_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
