...
void kernel_2226151325_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_M+1); i_1++) {
            B[i_1][i_0] = C[i_1-1][i_0] * C[i_1+1][i_0] * C[i_1][1] * C[i_1][i_0] * C[i_1][i_0+1] + C[i_1+i_0-1][1] * C[i_0+1][i_1-1] - 4;
        }
        for (int i_2 = 2; i_2 < i_0; i_2++) {
            B[i_0][i_2] = C[i_2+1][0] * C[i_2-1][i_0+1] - D[i_2] - B[i_0+1][i_2] + B[i_0-2][i_2-1] + B[i_0][i_2-2] * 2;
        }
    }
#pragma endscop
...
kernel_2226151325_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
