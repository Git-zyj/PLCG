...
void kernel_4216112313_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_L, i_3); i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                B[i_5][i_4-2][i_3] = C[i_4][i_3][i_4+i_3] + B[i_5][i_4-1][i_3-1] + B[i_5][i_4-2][i_3-1] - B[i_5-1][i_4-2][i_3+1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4216112313_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
