...
void kernel_3214112215_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        ...
        for (int i_3 = 1; i_3 < min(i_0, PB_L-1); i_3++) {
            B[i_0][i_3] = C[i_3] - D[i_3][i_0] - B[i_0][i_3-1] * B[i_0-1][i_3] * B[i_0][i_3+1] * 6;
        }
    }
#pragma endscop
...
kernel_3214112215_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
