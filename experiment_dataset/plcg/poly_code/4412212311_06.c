...
void kernel_4412212311_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < PB_N-2; i_1++) {
            for (int i_2 = i_1; i_2 < min(-i_1+PB_P+2, PB_Q-1); i_2++) {
                for (int i_3 = 2; i_3 < -i_2+PB_M; i_3++) {
                    A[i_0-1][i_3][i_1-1] = E[i_1][i_3][i_1+1][i_2] * F[i_0][i_3][i_2][i_1] - E[i_1+2][i_3][i_0][i_2+i_1-2] * F[i_3+i_2][i_1][i_0][i_2+1] - A[i_3][i_1-1][i_2-2] - A[i_0][i_1+1][i_2-1] + A[i_0-2][i_1][i_2-1] + A[i_0-2][i_3][i_2-2] * A[i_3-2][i_0+1][i_1-2] - 5;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
            for (int i_6 = 0; i_6 < PB_P-2; i_6++) {
                B[i_5][i_4-2][i_6+2] = D[i_4-2][i_5][i_5+1] * 3;
            }
        }
        for (int i_7 = max(i_4, 4); i_7 < PB_M-1; i_7++) {
            for (int i_8 = 1; i_8 < min(-i_4+PB_P+3, -i_7+PB_P+1); i_8++) {
                C[i_7][i_4][i_8+i_7-2] = C[i_8-1][i_4+1][i_8+i_7-4] + C[i_7+1][i_4-1][i_8+i_7-1] * C[i_4][i_7][i_8+i_4-3] + D[i_4-1][i_8][i_8+1] * 6;
                D[i_4-2][i_8][i_8+1] = B[i_7][i_4-2][i_8+2] * D[i_4-3][i_8+1][i_8+2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4412212311_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
