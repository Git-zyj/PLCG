...
void kernel_4336211215_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_P+1, PB_L); i_1++) {
            for (int i_2 = 2; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_0+PB_P+1, PB_L-1), -i_2+PB_P); i_3++) {
                    A[i_0][i_3+i_2][i_1-1] = A[i_1][i_3+i_2-2][i_0-1] + A[i_3+1][i_1+i_0-1][i_2-1] - A[i_1][i_3+i_0-1][i_2-1] + 2;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            B[i_0][i_4+1] = D[i_4+1][i_0] - D[i_0][i_4+1] + B[i_0][i_4+2] - B[i_0-1][i_4+1] - 6;
        }
    }
    for (int i_5 = 1; i_5 < PB_M+1; i_5++) {
        C[i_5] = D[i_5][i_5-1] * C[i_5-1] * 3;
    }
#pragma endscop
...
kernel_4336211215_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
