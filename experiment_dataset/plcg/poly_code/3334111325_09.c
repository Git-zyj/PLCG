...
void kernel_3334111325_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        B[i_1] = D[i_1+1][i_1] * B[i_1+2] - B[i_1+1] + B[i_1-2] * 6;
    }
    for (int i_2 = 0; i_2 < PB_M-3; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M-3; i_3++) {
            C[i_3][i_3+i_2] = C[i_3+2][i_3+i_2+2] * C[i_3+2][i_3+i_2+3] - C[i_3][i_3+i_2+1] * C[i_3+1][i_3+i_2+1] - 3;
        }
    }
#pragma endscop
...
kernel_3334111325_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
