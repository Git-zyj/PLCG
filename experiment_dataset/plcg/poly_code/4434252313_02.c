...
void kernel_4434252313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0+2] + F[i_0] - G[i_0][0] + E[i_0] - A[i_0-1] * A[i_0+1] * B[i_0] * B[i_0+1] + 5;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1] = F[i_1] * E[i_1-1] - E[0] - B[i_1+1] * B[i_1-1] + B[i_1-1] + 2;
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_M-3, -i_2+PB_P); i_3++) {
            for (int i_4 = i_2; i_4 < PB_P-1; i_4++) {
                D[i_2][i_3+3][i_2+2] = H[i_3+2][i_2][i_4-1] - I[i_4][i_3+i_2][i_4] + I[i_4-2][i_3-2][i_2] - I[i_2][i_4+1][2] * G[0][i_3+1] + I[i_4][i_2][i_3] + 2;
            }
        }
        for (int i_5 = i_2; i_5 < PB_M-2; i_5++) {
            for (int i_6 = 2; i_6 < min(PB_N-1, -i_2+PB_M); i_6++) {
                D[i_6][i_5+2][i_6+2] = H[i_6+2][i_2-2][i_5] + H[i_6+i_2][i_5+1][i_5+2] * H[i_6][i_2+1][i_5] + G[i_6][i_2+2] * I[i_6-2][i_6][i_5] * D[i_6-1][i_2+2][i_6+1] - D[i_6+1][i_5+2][i_6+3] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4434252313_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
