...
void kernel_3234211311_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < PB_P; i_2++) {
                A[i_0][i_2][i_2][i_0] = C[i_1+1][i_2-1][i_0-1] * A[i_0-1][i_2][i_2][i_0-1] + 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_P; i_4++) {
            for (int i_5 = 2; i_5 < PB_N+1; i_5++) {
                B[i_5-1][i_4-1] = A[i_3][i_4][i_4][i_3] * B[i_5-1][i_4-2] + B[i_5-2][i_4-1] + B[i_5-2][i_3-2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3234211311_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
