...
void kernel_3334252211_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, i_1); i_2++) {
                B[i_1-1][i_2] = D[i_2+i_1][i_0][i_1-2] * E[i_2][i_0][i_1+1][0] + D[i_2+2][i_0][i_1-1] + F[i_1][i_2][i_0] + G[i_2][i_1] * 6;
            }
            B[i_0][i_1] = G[i_0-1][i_1-2] - H[i_0][i_1] - H[i_1-2][i_0] * C[i_1+2] - 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_N; i_3++) {
        C[i_3+2] = I[i_3] + I[i_3] * G[i_3][i_3-1] - J[i_3+1] * I[i_3-2] - C[i_3+3] + C[i_3+1] + 4;
    }
#pragma endscop
...
kernel_3334252211_06(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
