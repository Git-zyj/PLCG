...
void kernel_2316112221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        ...
        B[i_0+2] = D[0] * B[i_0+3] * B[i_0+1] + 2;
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            C[i_2+2][i_3-2] = C[i_2+2][i_3-1] + C[i_2+1][i_3-1] + C[i_2+2][i_3] + 1;
        }
    }
#pragma endscop
...
kernel_2316112221_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
