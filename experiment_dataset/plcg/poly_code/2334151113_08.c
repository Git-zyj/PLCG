...
void kernel_2334151113_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            B[i_2+1][i_1] = E[i_1][i_2] + E[i_2][i_1+1] - E[i_2+1][i_1] - E[i_2-1][i_2] + B[i_2][i_1-1] * 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        A[i_3] = D[i_3] * D[i_3+1] * D[i_3-1] - D[i_3] * E[i_3][i_3+1] * D[i_3] * D[i_3] * D[i_3-1] + 3;
    }
#pragma endscop
...
kernel_2334151113_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
