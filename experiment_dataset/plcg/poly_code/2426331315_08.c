...
void kernel_2426331315_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - F[i_0+1][i_0+1] - G[i_0] + A[i_0+1] + B[i_0][i_0] * B[i_0-1][i_0-1] * 5;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            B[i_1][i_1] = H[i_1][i_2+1] - E[i_2-1] - E[i_2] + B[i_1+1][i_1+1] + 3;
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_1+1][i_1+1] = G[i_3] - H[i_3][i_1] + F[0][i_1] * A[i_3] - 4;
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        D[i_4-1] = G[i_4] - G[1] * G[i_4] - B[i_4][i_4] - D[i_4-2] + D[i_4] + 2;
    }
#pragma endscop
...
kernel_2426331315_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
