...
void kernel_2332112211_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
        A[i_1+3] = D[i_1+2] * D[i_1] + D[i_1+2] * C[i_1+1] + 6;
        C[i_1] = C[i_1+1] * 4;
    }
#pragma endscop
...
kernel_2332112211_03(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
