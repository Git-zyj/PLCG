...
void kernel_2336352121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] + E[i_0+2] - F[i_0-1][i_0] - G[i_0][i_0] * H[i_0-1] + H[i_0] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        B[i_1] = H[i_1] * E[i_1] + H[i_1] * E[i_1] * B[i_1+2] - 1;
    }
    for (int i_2 = PB_L - 1; i_2 >= 2; i_2--) {
        for (int i_3 = i_2; i_3 < PB_M-2; i_3++) {
            C[i_3+2][i_3] = F[i_3][i_2] * G[i_3-1][i_2-2] + I[i_3][i_2-1] - F[i_3][i_3-2] * 6;
        }
    }
#pragma endscop
...
kernel_2336352121_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
