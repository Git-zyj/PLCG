...
void kernel_3224312223_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_2][i_1][1] = C[i_2][i_0][i_0+1] + 3;
            }
            for (int i_3 = 2; i_3 < i_1; i_3++) {
                B[i_3][i_1][1] = C[i_3][i_0][i_3-2] * B[i_3][i_0+2][1] - B[i_3][i_1+2][1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3224312223_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
