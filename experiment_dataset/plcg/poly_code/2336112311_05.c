...
void kernel_2336112311_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        A[i_0+2] = D[1] + A[i_0+3] - 1;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1] = D[i_1] * D[i_1] - B[i_1+1] - B[i_1-1] - 6;
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_M-3, -i_2+PB_L-2); i_3++) {
            C[i_2-1][i_3+i_2+1] = A[i_3+3] - C[i_2][i_3+i_2+2] - C[i_2][i_3+i_2+1] * C[i_2-1][i_3+i_2] - C[i_2-2][i_3+i_2] - C[i_2-1][i_3+i_2+2] - 6;
        }
    }
#pragma endscop
...
kernel_2336112311_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
