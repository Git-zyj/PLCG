...
void kernel_4226252215_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_Q-1); i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_P; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_Q; i_3++) {
                    A[i_2][i_0][i_3][i_1+i_0] = C[i_0-2][i_2-2][i_3+2][i_1-2] * D[i_2][i_0-1][i_3][i_1] - D[i_3][i_2+i_0][i_0][i_1] + D[i_3+i_2+1][i_1+2][i_2][i_0] + E[i_1][i_3][i_0] - C[i_0][1][i_2][i_3+i_1] + C[i_3][i_0][i_2+i_0][i_1] + A[i_2][i_0-1][i_3+1][i_1+i_0-1] - A[i_2][i_0+1][i_3][i_1+i_0+1] + 3;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-3; i_4++) {
        B[i_4+2] = F[i_4+1][i_4+2] - G[i_4][2] * H[i_4] - B[i_4+3] * B[i_4+1] * 4;
    }
#pragma endscop
...
kernel_4226252215_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
