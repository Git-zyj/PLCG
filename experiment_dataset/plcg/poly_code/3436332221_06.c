...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
        B[i_1+1] = E[i_1+1] - E[i_1+2] + E[i_1] + E[i_1] - B[i_1+2] * B[i_1] * 4;
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_M; i_3++) {
            C[i_3][i_2-2] = F[i_3+i_2][0] * F[i_2][i_3] * G[0][i_2-2] + H[i_3][i_3+i_2] - C[i_3][i_2-1] + C[i_3+1][i_2-2] + 3;
        }
        D[i_2-1] = I[i_2] * E[i_2-1] + D[i_2-2] * D[i_2-3] + 3;
    }
#pragma endscop
...
kernel_3436332221_06(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
