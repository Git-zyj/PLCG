...
void kernel_3322111223_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            B[i_3-1] = B[i_3] * C[i_2][i_3+2] * C[i_3][i_2+1] * 5;
            C[i_3][i_2+1] = B[i_3-1] + B[i_3+1] * 1;
        }
    }
#pragma endscop
...
kernel_3322111223_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
