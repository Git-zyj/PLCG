...
void kernel_4222312321_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 3; i_1 < i_0; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M+1; i_2++) {
                A[i_0-2][i_1-2][i_0] = C[i_2-1][i_0+2][i_1-1] + A[i_0-4][i_1-2][i_0-2] + A[i_0-2][i_1-3][i_0] - 5;
            }
        }
    }
        ...
        for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 2; i_5 < PB_M; i_5++) {
                for (int i_6 = max(2, i_3); i_6 < PB_N; i_6++) {
                    B[i_4][i_6][1] = D[i_4][i_6][i_5] - A[i_6-2][i_5-2][i_6] + B[i_4][i_6+1][1] + B[i_4+1][i_3][1] - B[i_4-2][i_6][1] * 6;
                }
            }
        }
    }
...
kernel_4222312321_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
