...
void kernel_2316252123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0-2] = D[0] * E[0] - D[i_0] - D[i_0] * D[i_0-1] + B[i_0][i_0] * 2;
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            B[i_1][i_1] = F[i_1+i_0][0] * F[i_0-1][i_1] - E[i_1] + F[i_1-2][i_0] + G[i_0][i_1][i_0+2] * F[i_1-2][i_0] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        B[i_2+1][i_2+1] = D[i_2-2] + E[i_2] + D[2] - D[i_2] + A[i_2-1] - 6;
    }
#pragma endscop
...
kernel_2316252123_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
