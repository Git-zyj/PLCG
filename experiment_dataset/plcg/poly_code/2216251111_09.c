...
void kernel_2216251111_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
            A[i_0-1][i_1-1] = C[i_1+1][i_0-1] * C[i_1][i_0] - D[i_1][i_0] - 4;
        }
        for (int i_2 = i_0; i_2 < min(-i_0+PB_M, PB_L-1); i_2++) {
            B[i_0][i_2] = D[i_2-1][i_2] + D[i_2+i_0][i_2+1] + D[i_2][i_0-1] * C[i_2][i_2] - D[i_2][i_0] - D[i_2][i_0] - B[i_0][i_2+1] - 3;
        }
    }
#pragma endscop
...
kernel_2216251111_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
