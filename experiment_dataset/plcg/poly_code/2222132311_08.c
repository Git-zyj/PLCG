...
void kernel_2222132311_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0-2][i_1-1] = C[i_0-2][i_1] * A[i_0-3][i_1-1] + A[i_0-1][i_1-1] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = i_2; i_3 < PB_P; i_3++) {
            B[i_2][i_3] = C[i_3][i_2+1] - C[i_2][i_3] + D[i_3+1][i_2][i_3] - E[i_2] + C[i_3+1][i_2] - B[i_2-1][i_3] - B[i_2][i_3+1] * B[i_2+1][i_3] * B[i_2-1][i_3-1] - 3;
        }
    }
#pragma endscop
...
kernel_2222132311_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
