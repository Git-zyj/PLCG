...
void kernel_4134312313_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_N-2; i_1++) {
            A[i_1] = B[i_1+2][i_0] + A[i_1-1] + A[i_1+1] + A[i_1+1] - 1;
        }
    }
#pragma endscop
...
kernel_4134312313_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
