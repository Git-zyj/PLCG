...
void kernel_3434152213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[0] * E[i_0] * E[i_0] - E[i_0-1] - A[i_0-1] - A[i_0+1] * A[i_0-1] + 4;
    }
    for (int i_1 = 3; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < min(min(-i_1+PB_M-1, i_1), PB_L-2); i_2++) {
            for (int i_3 = 2; i_3 < min(PB_P+2, -i_1+PB_L); i_3++) {
                B[i_1][i_2+2][i_3-2] = F[i_3+i_1-2][i_2][i_2] - F[i_3][i_2+i_1+1][i_3-2] * F[i_3][i_2][i_1-2] - F[i_3][i_2][i_3+i_1] * F[i_2-1][i_3+i_1][i_2-2] + 2;
            }
        }
        for (int i_4 = 1; i_4 < PB_P+1; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_1+PB_L, -i_4+PB_L); i_5++) {
                C[i_1-2][i_4][i_5] = F[i_1][i_4+2][i_5+i_4] + F[i_4][i_5][i_1] * F[i_5][i_1+2][i_4] + F[i_4][i_5-2][i_1-2] - G[i_4][i_5+i_1] * F[i_4][i_5-1][i_4] + B[i_1][i_5+3][i_4-1] + C[i_1-3][i_4+1][i_5-1] * 1;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
        A[i_6+1] = E[i_6] - E[i_6] + E[i_6] + E[i_6] + A[i_6] - 5;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3434152213_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
