...
void kernel_2332212325_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
            C[i_2] = C[i_2+2] + C[i_2-2] * C[i_2+1] - 1;
        }
    }
#pragma endscop
...
kernel_2332212325_02(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
