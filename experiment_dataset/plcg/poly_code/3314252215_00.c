...
void kernel_3314252215_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0+2] = D[i_0+2] - D[i_0-1] - D[i_0] + E[i_0+2] + A[i_0+1] * 3;
    }
    for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
        for (int i_2 = min(PB_N, -i_1+PB_P) - 1; i_2 >= max(2, i_1); i_2--) {
            for (int i_3 = 1; i_3 < min(PB_N-2, -i_1+PB_N+1); i_3++) {
                C[i_1-1][i_3-1][i_2-2] = F[i_1][i_3][i_2-1] * G[i_3+2][i_1+2][2] * F[i_1][i_3][i_2+i_1] * G[i_2][i_3][i_2] + G[i_2][i_3][i_2] - G[i_3+i_1-1][i_1][i_2] - 5;
            }
        }
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            for (int i_5 = i_4; i_5 < min(PB_N, -i_4+PB_P+2); i_5++) {
                C[i_1][i_4-1][i_5-2] = F[i_1][i_5][i_5+i_4-2] + H[i_5][i_4-1] * G[i_5-1][i_1][i_4] * C[i_1-1][i_4][i_5-2] * C[i_1+1][i_4-1][i_5-2] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3314252215_00(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
