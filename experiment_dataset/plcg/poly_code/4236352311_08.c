...
void kernel_4236352311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = C[i_0-2] + D[i_0-2] + E[i_0+2] - E[i_0] + C[i_0] - A[i_0-1] + A[i_0+1] - A[i_0-1] + A[i_0-1] * A[i_0+1] - 5;
    }
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            for (int i_3 = 0; i_3 < i_1; i_3++) {
                for (int i_4 = 2; i_4 < min(min(-i_1+PB_L-2, -i_3+PB_P), i_3); i_4++) {
                    B[i_1-1][i_2-1][i_4+i_3][i_4+i_1+2] = F[i_3+2][i_2][i_4+i_1] - G[i_1][i_4][i_4-2][i_2] - H[i_4][i_1][i_3][i_2] + I[i_3][i_2-2][i_1][i_4+1] * H[i_4][i_4+i_1+2][i_2][i_2] + 1;
                }
            }
        }
    }
...
kernel_4236352311_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
