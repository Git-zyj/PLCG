...
void kernel_4422331213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0] = E[i_1][i_0+1] + F[0][i_0] * E[i_0][i_1] - A[i_0-1] + 6;
        }
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_P); i_3++) {
                for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                    B[i_4][i_2-1][i_0+1] = G[i_4+1][i_0][i_2+1][i_3+i_2] - B[i_3+1][i_2][i_4+1] * D[i_4][i_2-1][i_0][i_3-1] * 1;
                }
            }
        }
        for (int i_5 = 0; i_5 < i_0; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
                A[i_0] = A[i_6] * A[i_6+1] * 2;
            }
        }
    }
        ...
        for (int i_8 = 1; i_8 < PB_Q-1; i_8++) {
            for (int i_9 = 0; i_9 < PB_M-1; i_9++) {
                for (int j_0 = 1; j_0 < i_9; j_0++) {
                    D[i_7-1][i_8-1][j_0][i_9] = G[j_0][i_7][i_8+1][i_8-1] * G[j_0-1][i_7][i_9][i_8] - H[i_9+1][j_0-1][i_7][i_8+1] * I[i_7][j_0-1][i_9][i_8] + H[j_0][i_7-1][i_9][i_8] * D[i_7-1][i_8-1][j_0][i_9+1] - 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4422331213_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
