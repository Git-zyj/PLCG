...
void kernel_2332311315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = D[i_0+1] * A[i_0-1] - 4;
        for (int i_1 = 0; i_1 < -i_0+PB_N-1; i_1++) {
            B[i_1+i_0] = B[i_1+i_0+1] - B[i_1+i_0] * 5;
        }
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            A[i_2] = A[i_2] * A[i_0] - A[i_2+1] - 6;
        }
    }
#pragma endscop
...
kernel_2332311315_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
