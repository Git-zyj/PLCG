...
void kernel_3322151313_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] - D[i_0] + D[i_0] + D[i_0] * A[i_0+1] + A[i_0-1] - A[i_0+1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1] = D[0] + D[i_1] - D[i_1] + D[i_1] + D[i_1] - 5;
        B[i_1+1] = D[i_1+1] * D[i_1] + D[0] - D[i_1] + D[i_1] * D[0] + B[i_1] + B[i_1-1] + B[i_1+1] * 5;
    }
#pragma endscop
...
kernel_3322151313_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
