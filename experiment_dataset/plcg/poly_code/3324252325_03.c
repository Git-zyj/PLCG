...
void kernel_3324252325_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        B[i_0-2] = D[i_0-1] - E[i_0] - E[i_0] - D[i_0+1] * E[i_0] + E[i_0] + D[i_0] - E[i_0] - 4;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        B[i_1] = D[i_1-1] + B[i_1-1] - B[i_1+1] * 4;
        for (int i_2 = 0; i_2 < PB_Q-2; i_2++) {
            for (int i_3 = 2; i_3 < min(min(-i_1+PB_L, i_2), -i_2+PB_L-1); i_3++) {
                C[i_3][i_3] = F[i_3+i_2+1][i_3+i_1][2] * F[i_3][i_1][i_2+2] - G[i_3+i_2][i_1+1][i_1] - H[i_2][i_3+1] + C[i_3-2][i_3-2] - C[i_3-1][i_3-1] * C[i_3+2][i_3+2] - C[i_2+2][i_2+2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3324252325_03(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
