...
void kernel_2236311123_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1][i_0+1] = A[i_0+1][i_0+3] + 1;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        A[i_1-1][i_1+1] = C[i_1] - D[i_1] * 4;
    }
#pragma endscop
...
kernel_2236311123_07(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
