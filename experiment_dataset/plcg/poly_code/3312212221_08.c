...
void kernel_3312212221_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            B[i_2+1][i_3-2] = E[i_3][i_2] - B[i_2-1][i_3-2] * B[i_2+1][i_3] * 6;
        }
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            C[i_4-2] = D[i_4] + B[i_2+1][i_4-2] + C[i_4-1] + 3;
        }
    }
#pragma endscop
...
kernel_3312212221_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
