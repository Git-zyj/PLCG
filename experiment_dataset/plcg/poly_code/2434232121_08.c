...
void kernel_2434232121_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        B[i_0+1] = E[i_0-2] * F[i_0] + C[i_0] + 6;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        B[i_1] = E[i_1] * F[i_1] * E[i_1-1] * 2;
    }
    for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
        C[i_2] = E[i_2] - F[i_2+2] * 4;
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            D[i_2+1][i_3] = G[i_2][i_2-1] - H[i_3][i_2] - D[i_2+3][i_3] * D[i_2-1][i_3-1] + 5;
        }
    }
#pragma endscop
...
kernel_2434232121_08(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
