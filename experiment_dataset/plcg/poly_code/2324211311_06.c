...
void kernel_2324211311_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        ...
        for (int i_3 = 0; i_3 < min(i_1, PB_M-2); i_3++) {
            C[i_3+1][i_1] = C[i_3+1][i_1-1] * C[i_3][i_1] * C[i_3+1][i_1+1] * C[i_3+2][i_1-1] - 3;
        }
    }
#pragma endscop
...
kernel_2324211311_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
