...
void kernel_3424251115_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_N); i_1++) {
            A[i_1] = E[i_0] * F[i_1] + G[i_1][i_0] * E[i_1+i_0] - H[i_1-1][i_0+1] * A[i_1+1] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_2][i_3] = G[i_3][i_2] - G[i_2-1][i_3] * H[i_2][i_3-1] + 5;
        }
        A[i_2] = E[i_2-1] - F[i_2] * E[i_2] - E[i_2] * F[i_2] - 3;
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = i_2; i_5 < -i_4+PB_L; i_5++) {
                A[i_5-1] = H[i_4][i_2] + I[i_5+1][i_2-1][i_4] * I[i_5][i_5+i_4][i_2-1] + I[i_5+i_4-1][i_5-1][i_4] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3424251115_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
