...
void kernel_3434152125_08(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        D[i_0] = E[i_0] + E[i_0] + E[i_0-2] + 4;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                B[i_2][i_3][i_3+1] = F[i_3][i_1][i_1] + G[i_3][i_2+1][i_1][i_2+1] - H[i_3][i_2] - F[i_2][i_3][0] + H[i_2][i_3] + 1;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 2; i_5 < -i_4+PB_M; i_5++) {
            D[i_5] = H[i_5][i_4] - H[i_5+i_4][i_4] * H[i_5][i_4] - H[i_4][i_5+i_4] - B[i_4-1][i_5-2][i_5-1] + 4;
        }
        D[i_4] = E[i_4+2] + E[i_4+1] - E[i_4] - E[i_4-2] * E[i_4+1] * E[i_4] - D[i_4-2] - 4;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3434152125_08(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
