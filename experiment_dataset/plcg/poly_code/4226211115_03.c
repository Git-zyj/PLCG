...
void kernel_4226211115_03(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                B[i_2-1][i_0+1][i_2+1][i_1] = C[i_2][i_1][i_0+1] + 5;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
                for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
                    B[i_5-1][i_6][i_5+1][i_3] = D[i_5][i_3+1][i_4][i_6+1] - B[i_3-1][i_6+1][i_3+1][i_4+1] + 6;
                }
            }
        }
    }
...
kernel_4226211115_03(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D));
