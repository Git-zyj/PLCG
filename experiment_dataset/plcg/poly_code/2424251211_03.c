...
void kernel_2424251211_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] * F[1] - A[i_0+1] * A[i_0-1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_L-1, -i_1+PB_L+1); i_2++) {
            B[i_2+i_1-1] = G[i_2][i_2-1] * H[i_1][i_2] - H[i_2+1][i_1] + F[i_1] * G[i_1][i_2] - G[i_1+1][i_2-1] - A[i_2-1] - B[i_2+i_1-1] * 4;
        }
        C[i_1] = E[i_1] - F[i_1] * F[i_1+1] * E[i_1] * A[i_1] * D[i_1+1][i_1+1] + 5;
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            D[i_3][i_3] = G[i_3][i_1] + G[i_1][i_3] - G[i_1+1][i_3] - F[i_1-1] - H[i_3][i_1] - I[i_3+1][i_1-1][i_1] - D[i_3+1][i_3+1] * 5;
        }
    }
#pragma endscop
...
kernel_2424251211_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
