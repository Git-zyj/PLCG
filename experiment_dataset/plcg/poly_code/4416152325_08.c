...
void kernel_4416152325_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 3; i_1 < min(-i_0+PB_P+2, i_0); i_1++) {
            for (int i_2 = 2; i_2 < i_1; i_2++) {
                A[i_1-1][i_0][i_2] = E[i_0-2][i_1] * E[i_1][0] - F[i_1+1][i_1-2][i_2-2] - F[i_2][i_0][i_0-2] - E[i_1+2][i_2] - E[i_2][i_0] * A[i_1-1][i_0-1][i_2] * A[i_1-3][i_0][i_2] * A[i_1-1][i_0+1][i_2-2] * 4;
            }
            for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
                A[i_1-1][i_0][i_3] = G[i_0][i_3+2][i_1+2][0] + E[i_3][i_0] - G[i_1-1][i_0][i_3+2][i_3] + F[i_3][i_1+i_0-2][0] * F[i_0][i_1][i_3] * 3;
            }
        }
        for (int i_4 = 0; i_4 < i_0; i_4++) {
            for (int i_5 = i_0; i_5 < PB_P; i_5++) {
                for (int i_6 = 1; i_6 < PB_P-1; i_6++) {
                    A[i_5-2][i_4+1][i_6] = F[i_6-1][i_5][i_4] - G[i_4][i_6+1][i_0][i_5+1] + 2;
                }
            }
        }
        D[i_0] = H[i_0] * H[i_0] * H[i_0] * H[i_0] - H[i_0-2] - E[i_0-2][i_0] - D[i_0+2] + D[i_0-2] * D[i_0-1] - 4;
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4416152325_08(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
