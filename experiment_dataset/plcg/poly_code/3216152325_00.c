...
void kernel_3216152325_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M-2; i_1++) {
            B[i_1+2][i_0] = C[i_1][i_0+2] - C[i_1][i_0] - C[i_1+2][i_0] * C[i_1-2][i_0] + C[i_1][1] - 5;
        }
        for (int i_2 = 0; i_2 < min(PB_M, -i_0+PB_L+1); i_2++) {
            B[i_0][i_2] = C[i_2][i_2+i_0-1] * C[i_0+1][i_2] - D[0] * C[i_0][i_2] - B[i_0-1][i_2] * B[i_0-1][i_2+2] - B[i_0][i_2+2] - 2;
        }
    }
#pragma endscop
...
kernel_3216152325_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
