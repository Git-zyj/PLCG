...
void kernel_4434231115_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            for (int i_2 = i_0; i_2 < min(-i_0+PB_M, PB_P); i_2++) {
                A[i_0][i_1][i_2] = E[i_2+i_0][i_1][1][i_0+1] - F[i_0+1][i_1-1][i_2] - A[i_0][i_1+1][i_2] - 1;
            }
        }
    }
    ...
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 0; i_5 < min(PB_P, -i_4+PB_M+1); i_5++) {
            B[i_5] = H[i_5+i_4-1][i_5] - G[i_5][i_4] * H[i_5][i_4] * G[i_4-1][i_4+1] * G[i_4-1][i_5+1] - 5;
        }
        for (int i_6 = 1; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < min(i_6, -i_4+PB_P); i_7++) {
                B[i_6-1] = I[i_7][i_4][0] - F[i_6][i_7][i_4] - I[i_4+1][i_7][i_7+1] * F[i_6-1][i_7+i_4][i_6-1] - 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4434231115_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
