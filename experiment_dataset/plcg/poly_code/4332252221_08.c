...
void kernel_4332252221_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_L; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_2+PB_L, -i_1+PB_Q); i_3++) {
                    B[i_3+2][i_2+3] = D[i_1+i_0][i_3+i_2][i_3+2][i_1] - D[i_2][i_3][i_0][i_1+2] * E[i_1][i_2][i_0][0] * D[i_3+2][i_2+i_1][i_0][i_2] + E[i_2][i_0+1][i_3+i_1][0] - E[i_2-2][i_3-2][i_0][i_1+1] - 1;
                }
            }
        }
        for (int i_4 = i_0; i_4 < -i_0+PB_L-2; i_4++) {
            B[i_4+1][i_0+2] = F[i_0][i_4+1] * F[i_0][i_0+2] + F[i_4+i_0+2][0] + G[i_4+i_0-2][0] * F[i_4+2][i_0] + F[i_4][i_4] * G[i_4+2][i_0] * B[i_4+2][i_0+2] * B[i_4+1][i_0+1] + 4;
        }
    }
    for (int i_5 = 0; i_5 < PB_P; i_5++) {
        for (int i_6 = 2; i_6 < PB_N-3; i_6++) {
            for (int i_7 = 1; i_7 < PB_L-2; i_7++) {
                B[i_6+3][i_7+2] = H[i_6-2][i_7+2][i_5] + D[i_6+2][i_7-1][i_5][i_5] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4332252221_08(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
