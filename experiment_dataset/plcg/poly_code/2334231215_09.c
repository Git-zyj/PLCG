...
void kernel_2334231215_09(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        C[i_0] = D[i_0] + D[i_0-1] + D[i_0+1] - E[i_0] * C[i_0] - 4;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            C[i_2] = 4;
        }
        C[i_1] = E[i_1] * D[i_1] - E[i_1] - D[i_1] * C[i_1-1] + 3;
    }
#pragma endscop
...
kernel_2334231215_09(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
