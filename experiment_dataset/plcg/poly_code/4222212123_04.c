...
void kernel_4222212123_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                for (int i_3 = 2; i_3 < PB_Q+2; i_3++) {
                    A[i_1][i_0-2][i_2][i_3-2] = 2;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            B[i_0][i_4+1] = C[2] - B[i_0+2][i_4+1] * 2;
        }
    }
#pragma endscop
...
kernel_4222212123_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
