...
void kernel_2236152223_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            B[i_1][i_0] = C[i_0+1][i_1] + C[i_0][i_1] * C[i_1][i_0] * C[i_1+i_0][i_1+i_0] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            B[i_3][i_2] = C[i_3][i_2-1] + C[i_3+2][i_2] * D[i_3+i_2] + D[i_3+i_2] - D[i_2+2] * B[i_3-2][i_2] * B[i_3-1][i_2] - 2;
        }
    }
#pragma endscop
...
kernel_2236152223_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
