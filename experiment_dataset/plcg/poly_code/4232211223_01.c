...
void kernel_4232211223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        B[i_2+1] = C[i_2+1] + B[i_2+3] - B[i_2+2] - 3;
    }
#pragma endscop
...
kernel_4232211223_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
