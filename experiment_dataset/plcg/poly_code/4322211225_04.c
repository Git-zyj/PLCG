...
void kernel_4322211225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
    ...
    for (int i_0 = 1; i_0 < PB_Q-2; i_0++) {
        ...
        for (int i_1 = min(-i_0+PB_L-2, i_0) - 1; i_1 >= 0; i_1--) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                C[i_2][i_1+i_0][i_2][i_0] = C[i_2-2][i_1+i_0+2][i_2-2][i_0+2] * C[i_2][i_1+i_0-1][i_2][i_0-1] + C[i_2-1][i_1+i_0][i_2-1][i_0] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4322211225_04(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
