...
void kernel_2232211221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_M, i_0), PB_N-1); i_1++) {
            A[i_1+1] = C[i_1+1][i_1+i_0] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_P-1; i_3++) {
            B[i_3][i_3+i_2] = D[i_2-1][i_3] - B[i_3][i_3+i_2+1] - B[i_3-2][i_3+i_2-2] * B[i_3+1][i_3+i_2+1] * B[i_3+2][i_3+i_2] - 6;
        }
    }
#pragma endscop
...
kernel_2232211221_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
