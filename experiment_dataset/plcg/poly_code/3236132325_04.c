...
void kernel_3236132325_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
                A[i_2+1][i_1][i_1+i_0] = C[1][i_1+2][i_0+1] * D[i_2+1][i_1+1] * D[i_2][0] * A[i_2][i_1+1][i_1+i_0] * A[i_2+2][i_1-2][i_1+i_0-2] * 3;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_N; i_3++) {
            B[i_0-2][i_3+2] = D[i_3+1][i_0] * D[i_3+1][0] - D[i_3+i_0][i_0] + B[i_0-2][i_3+1] + B[i_0-2][i_3+4] - B[i_0-1][i_3+1] - B[i_0][i_3+2] + 6;
        }
    }
#pragma endscop
...
kernel_3236132325_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
