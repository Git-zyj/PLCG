...
void kernel_3314331121_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
    ...
    for (int i_1 = PB_N-3 - 1; i_1 >= 0; i_1--) {
        for (int i_2 = 1; i_2 < PB_N; i_2++) {
            A[i_2] = D[i_2] - E[i_1] * D[i_2] - F[i_2][i_1+1] * F[i_1][i_2-1] - 4;
            for (int i_3 = max(1, i_1); i_3 < PB_L+1; i_3++) {
                C[i_1+1][i_2-1][i_3] = G[i_2][i_3-1] * H[i_2-1][i_3+1][i_1+1] * I[i_2][i_3][i_2+1] + C[i_1+3][i_2-1][i_3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3314331121_04(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
