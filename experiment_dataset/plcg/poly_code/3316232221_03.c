...
void kernel_3316232221_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M-1; i_0++) {
        A[i_0-2] = D[i_0] - D[i_0-2] * E[i_0] * E[i_0] * A[i_0-3] + A[i_0-4] * 2;
        B[i_0] = D[i_0+1] + A[i_0-2] * A[i_0-3] * B[i_0+1] + 1;
    }
    for (int i_1 = 2; i_1 < PB_Q-2; i_1++) {
        for (int i_2 = 0; i_2 < min(min(-i_1+PB_Q+2, PB_P), i_1); i_2++) {
            for (int i_3 = 1; i_3 < PB_P; i_3++) {
                C[i_1][i_2][i_1] = F[i_3][i_2][i_2] * G[i_3-1][i_3] * F[i_2][i_1][i_3] * H[i_3+2][i_1+2][i_2+i_1-2] * C[i_2+2][i_3+1][i_2+2] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3316232221_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
