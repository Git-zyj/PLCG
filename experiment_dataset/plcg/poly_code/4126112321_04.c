...
void kernel_4126112321_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L, -i_0+PB_Q-1); i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_Q, -i_1+PB_Q-2), PB_L); i_2++) {
                A[i_1][i_2+i_0] = B[i_2][i_1+2] - A[i_2][i_1+i_0+1] - A[i_0][i_2+i_1+2] + A[i_0-1][i_2+i_1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_4126112321_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
