...
void kernel_3336351323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] + D[i_0] + D[1] * E[i_0] - F[1] * A[i_0-2] * C[i_0-1] * C[i_0-3] - 1;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_Q, i_1); i_2++) {
            for (int i_3 = 1; i_3 < min(PB_Q-1, -i_2+PB_M); i_3++) {
                B[i_1+1][i_1][i_2] = G[i_3+i_2][i_1-1] * H[i_1+1][i_3][i_2] - H[i_1][i_3-1][i_2-1] - I[i_3+1][i_1][i_2] * B[i_1+1][i_1][i_3+1] - 1;
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_L-1; i_4++) {
        C[i_4-1] = G[i_4+1][i_4+1] + F[i_4-1] - D[i_4] * E[i_4] + D[i_4] + F[i_4] + A[i_4+1] - C[i_4+1] * C[i_4-3] + 1;
    }
#pragma endscop
...
kernel_3336351323_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
