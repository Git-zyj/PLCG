...
void kernel_3422251113_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = E[i_0-1] + E[i_0] * E[i_0+1] - E[i_0-1] - E[i_0] + E[i_0+1] + F[i_0-1] - B[i_0] * 3;
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            B[i_1] = G[i_1][i_0-1] + G[i_0][i_1+i_0] * H[i_1][i_0] + C[i_1][i_0+1][i_1-1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_M-1; i_4++) {
                C[i_2][i_4+1][i_2-1] = H[i_4][i_2-1] * H[i_4][i_3] + I[i_4][i_3][i_2] - J[i_4+i_3+1][i_2][i_4] + 5;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        for (int i_6 = i_5; i_6 < -i_5+PB_L-1; i_6++) {
            D[i_6] = G[1][i_6+i_5+1] - H[i_6+i_5][i_5-1] - H[i_5][i_6] + H[i_6][i_6] - D[i_6+1] - D[i_6-1] + 5;
        }
    }
#pragma endscop
...
kernel_3422251113_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
