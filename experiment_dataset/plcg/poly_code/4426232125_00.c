...
void kernel_4426232125_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_L+2, i_0); i_3++) {
                    B[i_2][i_1][i_3+i_2-2] = 1;
                }
            }
        }
        for (int i_4 = 3; i_4 < min(PB_M, i_0); i_4++) {
            for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
                for (int i_6 = 2; i_6 < min(-i_5+PB_L+1, -i_4+PB_L); i_6++) {
                    B[i_6][i_4][i_6+i_5-1] = E[i_5][i_6+i_4][i_0][i_4] * B[i_6][i_5+2][i_6+i_4-3] - D[i_5][i_6-2][i_4+1][i_0-2] + 2;
                }
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_M; i_7++) {
        for (int i_8 = 2; i_8 < PB_M; i_8++) {
            for (int i_9 = i_7; i_9 < -i_7+PB_L-1; i_9++) {
                B[i_7][i_8-2][i_9+i_7+1] = F[i_9][i_8][i_9] + G[i_7][i_8][i_9-2] - F[i_7][i_9-2][i_8] + H[i_9+2][i_8-1] + H[i_9][i_9] * F[i_8][i_9][i_8] * 4;
                for (int j_0 = 2; j_0 < PB_M; j_0++) {
                    D[i_8][i_9][i_7][j_0-2] = I[j_0][i_9][i_7-2][i_8] * G[i_9][j_0][i_8-1] * 2;
                }
            }
        }
    }
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4426232125_00(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
