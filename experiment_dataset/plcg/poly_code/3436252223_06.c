...
void kernel_3436252223_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        A[i_0+1] = E[i_0] + F[i_0] * F[0] - F[i_0] * A[i_0] * 6;
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_P-2, -i_1+PB_L); i_2++) {
                B[i_1][i_0][i_2] = G[i_1][0] + H[i_1][i_2+i_0+2][0] * I[i_2+i_1][i_0][0] - I[i_2-1][i_1][i_0] * H[i_1-2][i_0][i_0] + B[i_1+2][i_0][i_2] * 5;
            }
        }
        for (int i_3 = i_0; i_3 < min(PB_N, -i_0+PB_Q-1); i_3++) {
            for (int i_4 = 1; i_4 < PB_N; i_4++) {
                C[i_4+1][i_0+1] = I[i_0][i_4+1][i_3+i_0+1] - H[i_4+1][i_0][i_3] * I[i_0-2][i_3+2][i_4] + J[i_4][i_3][i_0][i_3] - I[i_4][i_0][i_3-1] * H[i_4-1][i_3-1][i_0-1] + H[i_3+i_0][i_4][0] - C[i_4+1][i_3] - C[i_0+1][i_4-1] * 6;
            }
        }
    }
    for (int i_5 = 3; i_5 < PB_P; i_5++) {
        for (int i_6 = 0; i_6 < i_5; i_6++) {
            for (int i_7 = max(i_6, 2); i_7 < min(PB_P, -i_6+PB_L); i_7++) {
                D[i_7][i_5-1][i_6][i_5+2] = H[i_6][i_5][i_7-2] * G[i_7+i_6][i_5] * K[i_7][i_5] * G[i_7][2] - D[i_6+2][i_5-2][i_7][i_5+1] * D[i_7][i_5-3][i_6][i_5] + D[i_7+2][i_5-1][i_6][i_5+2] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3436252223_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
