...
void kernel_3416332323_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < -i_0+PB_L; i_2++) {
                A[1][i_2] = E[i_0][i_2][i_1] * F[i_2][i_2+i_0][i_1+1] * A[1][i_2-1] + A[1][i_1+1] - A[1][i_0+2] * 2;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            A[1][i_3] = G[i_3][i_0] - H[i_3] - G[i_3][0] + 1;
            for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
                A[1][i_4+1] = E[i_3][i_4-1][i_0] * I[i_3+1][i_4-2][i_0+1] * J[i_0][i_3] * 1;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
            for (int i_7 = 0; i_7 < min(-i_5+PB_L+1, PB_L-2); i_7++) {
                A[1][i_5+1] = I[i_6+1][i_7+i_5-1][i_6] + F[i_6+2][i_7+2][i_5] - I[i_5+1][i_7+1][i_7] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3416332323_05(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
