...
void kernel_3126151311_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < min(min(-i_0+PB_N-1, i_0), PB_M-1); i_1++) {
            A[i_1][i_0] = B[i_1+1][i_0][i_0] - C[i_1+i_0+1] * B[i_0-1][i_1][i_0] + C[i_0] + C[i_1] + A[i_1+1][i_0] - A[i_1+1][i_0+1] * A[i_1-1][i_0+1] - 1;
        }
    }
#pragma endscop
...
kernel_3126151311_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
