...
void kernel_2336152211_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_1][i_0+1] = D[i_1+i_0][i_0] - E[i_1+i_0] * D[i_1-1][i_0] * A[i_1][i_0] * 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_L, i_2); i_3++) {
            B[i_2][i_3] = D[i_2][i_3+1] * D[i_2][i_3] * D[i_3+2][i_2-1] + D[i_2-2][0] + D[i_3][i_2-2] - B[i_2-1][i_3] + B[i_2+1][i_3] * 5;
        }
    }
    for (int i_4 = 2; i_4 < PB_N; i_4++) {
        for (int i_5 = i_4; i_5 < PB_P; i_5++) {
            C[i_4][i_5] = E[i_4-2] * D[i_5][i_4] + D[i_5][2] + D[i_5][i_4] + D[i_4-1][i_5] * E[i_4-1] - D[i_4][i_4-1] + C[i_4-1][i_5+1] + 2;
        }
    }
#pragma endscop
...
kernel_2336152211_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
