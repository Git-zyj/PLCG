...
void kernel_2332132115_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_M, PB_L); i_2++) {
            A[i_2-2] = D[i_2+i_1][0] * E[i_1+1] - D[i_2+1][i_1] - D[i_1][i_2-1] - D[i_1][i_2] + 4;
        }
        for (int i_3 = 2; i_3 < min(-i_1+PB_M, PB_L-1); i_3++) {
            C[i_3][i_1] = D[i_3-2][i_1] - D[i_1][i_3+1] * E[i_3+i_1] - C[i_3-1][i_1] + 6;
        }
    }
#pragma endscop
...
kernel_2332132115_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
