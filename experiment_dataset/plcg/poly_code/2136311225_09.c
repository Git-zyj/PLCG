...
void kernel_2136311225_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_M-1); i_1++) {
            A[1] = B[i_1+1][i_0] + A[1] - A[1] * 1;
        }
    }
#pragma endscop
...
kernel_2136311225_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
