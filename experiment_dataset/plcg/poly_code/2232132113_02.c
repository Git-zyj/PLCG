...
void kernel_2232132113_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            A[i_0+1][i_1+i_0] = C[i_1][i_0] - C[i_0][i_1-2] * C[i_1][i_0-2] - C[i_0][i_1+1] - A[i_0+2][i_1+i_0] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_P-1; i_3++) {
            A[i_2+2][i_3+i_2+1] = C[i_3+i_2][0] + C[i_3][i_2+1] * 1;
        }
    }
#pragma endscop
...
kernel_2232132113_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
