...
void kernel_2226112311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_1+2] = A[i_1+3] - A[i_1+1] * 5;
        }
    }
    for (int i_2 = 3; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 3; i_3 < min(i_2, PB_M-2); i_3++) {
            B[i_2-2][i_3+1] = C[i_3-1][i_3] * B[i_2-3][i_3] + B[i_2-2][i_3+2] * B[i_3-3][i_2+1] - 4;
        }
    }
#pragma endscop
...
kernel_2226112311_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
