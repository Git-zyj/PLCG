...
void kernel_4212211315_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L-1 - 1; i_0 >= 1; i_0--) {
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            A[i_1+1][i_0] = C[i_1+1][i_0+1][i_0] + A[i_1][i_0] - A[i_1+2][i_0-1] + 2;
        }
        B[i_0+1] = B[i_0+2] + B[i_0] + B[i_0+2] + B[i_0+2] - 5;
    }
#pragma endscop
...
kernel_4212211315_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
