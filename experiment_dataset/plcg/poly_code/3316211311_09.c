...
void kernel_3316211311_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_1+1][i_1] = A[i_1][i_1-1] - A[i_2][i_2-1] + 2;
                A[i_1][i_1-1] = 6;
            }
        }
        C[i_0] = D[i_0] - C[i_0-1] + C[i_0+1] + C[i_0-1] - 1;
    }
#pragma endscop
...
kernel_3316211311_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
