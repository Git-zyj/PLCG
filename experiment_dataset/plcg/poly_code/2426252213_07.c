...
void kernel_2426252213_07(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            D[i_1][i_0-3] = E[i_1] - F[i_1+i_0][i_0] - F[i_1][i_0-2] + G[i_0][i_0] - H[i_1][i_1+i_0-1][i_1+i_0] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        B[i_2-1] = E[i_2] + E[i_2] - I[i_2-1] - I[i_2] * I[i_2] + B[i_2] - B[i_2-2] * 6;
    }
    for (int i_3 = 2; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            C[i_3][i_4] = F[i_3][i_4-1] * G[i_4-1][i_3+2] + H[i_3][i_4+2][i_3-2] + C[i_3][i_4-1] * 5;
        }
        for (int i_5 = 2; i_5 < PB_P-1; i_5++) {
            D[i_5][i_3-2] = F[i_3][i_5] + F[i_5+2][i_3] * J[i_3][i_5+1][0] * F[i_5-2][i_3+1] * H[i_5+1][i_3][1] - G[i_5-2][i_3+2] * D[i_5][i_3-1] - 6;
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2426252213_07(xd,yd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
