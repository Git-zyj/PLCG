...
void kernel_3334111221_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_P, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < min(PB_M, -i_0+PB_P); i_2++) {
                A[i_1+1][i_2+i_0] = A[i_2][i_1+i_0] * A[i_1+1][i_2+i_0-2] + 4;
            }
        }
        for (int i_3 = 1; i_3 < -i_0+PB_M; i_3++) {
            B[i_3+i_0][i_0-1] = D[i_3+1][i_0] * D[i_3][i_0-1] - D[i_3-1][i_0] - B[i_3+i_0-1][i_0-2] - 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < PB_M; i_5++) {
            C[i_5][i_4] = C[i_5][i_4-1] * C[i_5-2][i_4] + 5;
        }
    }
#pragma endscop
...
kernel_3334111221_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
