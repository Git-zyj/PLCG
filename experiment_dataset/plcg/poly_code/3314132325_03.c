...
void kernel_3314132325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        ...
        A[i_0] = E[1] + E[i_0] * E[i_0+2] - E[0] + A[i_0] * A[i_0+2] * 5;
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            C[i_0][i_2] = D[i_2][i_0+2] * D[i_0][i_0] + C[i_0][i_2+1] * C[i_0+2][i_2] - C[i_0-1][i_2] - 2;
        }
    }
#pragma endscop
...
kernel_3314132325_03(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
