...
void kernel_2336152115_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_0][0] = D[i_0-2][i_1+1] - D[i_0-2][i_1] * D[i_1][i_0+2] + D[i_0+2][i_1] + D[i_0][i_0+2] - D[i_0+1][i_1] + D[i_1+i_0+2][i_0+2] - A[i_0-1][0] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_L, PB_P-2); i_3++) {
            B[i_2][i_3+2] = E[i_2] * D[i_2][i_3] - D[i_3+i_2][i_3+2] + D[i_2][i_3+i_2] - B[i_2+1][i_3+2] * 5;
        }
    }
    for (int i_4 = 2; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < PB_P; i_5++) {
            C[i_4][i_4] = D[i_4][i_5] + E[i_5] * F[i_4-2][i_5-1][2] + C[i_4-1][i_4-1] * 4;
        }
    }
#pragma endscop
...
kernel_2336152115_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
