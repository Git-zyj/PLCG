...
void kernel_3212152323_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-3; i_0++) {
        A[i_0+1] = C[i_0] - C[i_0+2] + C[i_0] + C[i_0] - C[i_0] + C[i_0] - C[i_0+2] * A[i_0] + A[i_0-1] + A[i_0+2] + A[i_0+3] * 4;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        B[i_1] = C[i_1-1] - C[i_1+1] + C[i_1] - A[i_1+1] - B[i_1+1] * 4;
    }
#pragma endscop
...
kernel_3212152323_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
