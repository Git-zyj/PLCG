...
void kernel_2414212221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-3; i_0++) {
        A[i_0+1][i_0+1] = E[i_0] * A[i_0-1][i_0-1] - A[i_0+2][i_0+2] - A[i_0+3][i_0+3] - 6;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_1][i_0] = B[i_0-1][i_1] * B[i_1+2][i_0] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_L-1; i_3++) {
            C[i_3][i_3] = F[i_3][i_2-1] + F[i_2][i_3-2] * C[i_3+2][i_3+2] + C[i_3+1][i_3+1] + C[i_3-1][i_3-1] - 6;
            D[i_3+i_2][i_3+i_2+1] = 4;
        }
    }
#pragma endscop
...
kernel_2414212221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
