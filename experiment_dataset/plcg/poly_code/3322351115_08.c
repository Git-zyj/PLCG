...
void kernel_3322351115_08(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P, -i_0+PB_L); i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                C[i_2][i_1-1][i_0][i_2-1] = D[i_1][i_2][i_0] * D[i_2-1][i_1+i_0][i_0] - E[i_2+1][i_0][i_1][i_2-1] + D[i_2-1][i_0][i_1] + D[i_1-1][i_2+1][0] * F[i_0][i_2][i_1] * G[i_2][i_2][i_1-1][i_0+1] + C[i_2][i_0-1][i_1][i_2-1] * 3;
            }
        }
        B[i_0] = H[i_0-1] + H[i_0] + I[i_0] - J[i_0][i_0] * 4;
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = i_3; i_4 < PB_P; i_4++) {
            for (int i_5 = 1; i_5 < PB_Q; i_5++) {
                C[i_5+1][i_3-1][i_4][i_5] = K[i_5-1][i_4] + L[i_5][i_3][i_4] - F[i_5+1][i_3-1][i_4] + D[i_4-1][i_5][i_3] - 4;
            }
        }
    }
#pragma endscop
