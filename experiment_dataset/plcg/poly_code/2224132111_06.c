...
void kernel_2224132111_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+i_0+1][i_0-1] = C[i_1][i_0-2] + C[i_0+2][i_1] * D[i_1+i_0] + C[i_1+i_0+2][i_1] - A[i_1+i_0+1][i_0-2] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q+1; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < -i_2+PB_L-1; i_3++) {
            A[i_3+i_2+1][i_2-1] = D[i_3-2] - 5;
        }
    }
#pragma endscop
...
kernel_2224132111_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
