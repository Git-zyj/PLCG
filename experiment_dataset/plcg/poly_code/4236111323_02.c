...
void kernel_4236111323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            A[i_0][i_1] = A[i_0+2][i_1-2] + A[i_0-2][i_1] * A[i_0+2][i_1] * A[i_0][i_1+2] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                for (int i_5 = 2; i_5 < min(-i_2+PB_M, i_3); i_5++) {
                    B[i_2][i_5][i_4][i_2+1] = C[i_4][i_5+i_2][i_3-1][i_4] - C[i_5][i_3+1][i_4][i_2] - B[i_2+1][i_5-2][i_4+1][i_2+2] - 5;
                }
            }
        }
    }
...
kernel_4236111323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
