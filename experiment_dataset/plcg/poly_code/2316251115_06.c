...
void kernel_2316251115_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] * E[i_0] + E[i_0-1] - A[i_0-1] * 6;
        for (int i_1 = i_0; i_1 < -i_0+PB_M-1; i_1++) {
            B[i_1][i_0-1] = F[i_1+i_0-1][i_1] * F[i_1+i_0+1][i_1] * G[i_1+1][i_0][0] * 2;
        }
        A[i_0] = D[i_0+1] * E[i_0-1] - D[i_0+1] - D[i_0+1] + E[i_0] + E[i_0] * E[i_0] + D[i_0] * 5;
    }
#pragma endscop
...
kernel_2316251115_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
