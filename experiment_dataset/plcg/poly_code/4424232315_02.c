...
void kernel_4424232315_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_Q+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q-2; i_2++) {
                for (int i_3 = 1; i_3 < min(PB_Q, i_1); i_3++) {
                    B[i_2-1][i_3-1] = E[i_2][i_3][i_2+2][i_1-1] * 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 2; i_5 < i_4; i_5++) {
            B[i_4][i_5-1] = F[i_5][0] + B[i_4][i_5-2] - B[i_4-1][i_5-1] * B[i_4+1][i_5] - 5;
        }
    }
    for (int i_6 = 2; i_6 < PB_L-2; i_6++) {
        for (int i_7 = i_6; i_7 < PB_M; i_7++) {
            for (int i_8 = i_6; i_8 < -i_7+PB_M; i_8++) {
                B[i_7-1][i_8-1] = G[i_7][i_8+i_7-1][i_6] - H[i_7][i_8] + H[i_8+i_7][i_6] + 6;
            }
        }
        for (int i_9 = 0; i_9 < PB_P-2; i_9++) {
            for (int j_0 = i_6; j_0 < PB_N-2; j_0++) {
                B[i_9][j_0-1] = I[i_6][j_0][i_9] - I[j_0][i_9+2][i_6-2] - G[i_6][j_0][j_0] - G[j_0+2][i_6-1][i_6+2] - 4;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4424232315_02(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
