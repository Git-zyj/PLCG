...
void kernel_4232312315_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-1, PB_L-2); i_1++) {
            A[i_1+i_0][i_1+2] = C[i_1+i_0-2][i_1+i_0] * A[i_1+i_0-1][i_1+2] * A[i_1+i_0+1][i_1+2] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, PB_L-1); i_4++) {
                A[i_4+i_3][i_3+2] = A[i_4+i_3-1][i_4+1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4232312315_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
