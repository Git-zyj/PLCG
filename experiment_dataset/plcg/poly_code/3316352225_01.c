...
void kernel_3316352225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_N-2, i_0); i_1++) {
            A[i_0-1][i_1] = D[i_1+2][i_0] - E[i_0] * F[2] * 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = PB_M - 1; i_3 >= 2; i_3--) {
            B[i_3] = G[i_2][i_3] + D[i_3][i_2] - H[i_3][i_2] - D[i_3-2][i_2+1] * H[i_3][0] + H[i_3][i_2-2] * B[i_3-1] + C[i_2+1] * 5;
        }
        C[i_2+1] = F[i_2+2] * I[i_2+1] * E[i_2+2] - F[i_2+1] + I[i_2] - B[i_2] + C[i_2+2] * 5;
    }
#pragma endscop
...
kernel_3316352225_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
