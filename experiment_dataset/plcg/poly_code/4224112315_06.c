...
void kernel_4224112315_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = PB_M-1 - 1; i_0 >= 0; i_0--) {
        for (int i_1 = max(2, i_0); i_1 < PB_M; i_1++) {
            ...
            for (int i_4 = 2; i_4 < PB_L; i_4++) {
                B[i_1-1][i_4] = C[i_1][i_0+1][0] + B[i_1-2][i_4] - B[i_1-1][i_4-1] - B[i_4-2][i_1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4224112315_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
