...
void kernel_2232251325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            A[i_0-1][1] = C[i_1][i_0] * D[i_1][i_0] + D[i_1][i_0+1] + C[i_1+i_0][i_1] - D[i_1-1][i_0] * D[i_1][i_0] * A[i_0-2][1] - A[i_0][1] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_P; i_3++) {
            B[i_2][i_3] = D[i_3+i_2][i_3] * D[i_3+i_2][i_3] - D[i_3][i_2-1] - C[i_3+i_2][i_3+i_2] - A[i_2-1][1] + B[i_2-1][i_3+1] * B[i_2][i_3+1] - B[i_2+1][i_3] - 2;
        }
    }
#pragma endscop
...
kernel_2232251325_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
