...
void kernel_4212251125_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = C[i_1-1][i_0+1] - D[i_1-1][i_0] * E[i_1+1] + D[i_0][i_1] * D[i_0-1][i_1] + A[i_1-2][i_0+2] - 6;
        }
        for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
            for (int i_3 = 0; i_3 < -i_2+PB_P-1; i_3++) {
                B[i_0+1][i_2][i_3] = F[i_3+i_2-1][i_0][i_3+1] * F[i_0+1][i_3+1][i_2-1] + G[i_3][i_2-1][i_0] * G[i_3][i_0][i_3+i_2+1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4212251125_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
