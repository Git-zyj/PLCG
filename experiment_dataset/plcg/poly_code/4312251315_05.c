...
void kernel_4312251315_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = D[i_0] + E[1] * D[i_0] * A[i_0+2] + 2;
        A[i_0+1] = E[i_0] + D[i_0] * F[i_0+1][i_0] * G[i_0][1] + A[i_0+1] + A[i_0+2] + A[i_0] * 1;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 1; i_3 < min(PB_M-2, -i_1+PB_Q); i_3++) {
                for (int i_4 = 0; i_4 < min(min(-i_3+PB_L, i_2), -i_1+PB_Q); i_4++) {
                    C[i_4+1][i_3+1][i_1+1][i_2] = H[i_1][i_3][i_4][i_2-1] * I[i_3][i_1][i_2][i_4] * H[i_3][i_2][i_4][i_3+i_1] * H[i_4][i_3+i_1-1][i_2-1][i_1+1] - H[i_2][i_4][i_4+i_1][i_1] - H[i_2][i_3][i_3][i_1] * I[i_4][i_3-1][i_2+1][i_1-1] + J[i_2-1][i_4+i_3][1] * C[i_3+2][i_4+1][i_1+1][i_2+1] + 3;
                }
            }
        }
    }
...
kernel_4312251315_05(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
