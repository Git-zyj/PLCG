...
static void print_array(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4424331321_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L-2; i_1++) {
            A[i_1+i_0+1][i_0] = A[i_1+i_0+2][i_0] - A[i_1+i_0-1][i_0-2] + A[i_1+i_0][i_0+1] + C[i_0][i_1+1] * 6;
        }
        for (int i_2 = 2; i_2 < -i_0+PB_L; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                for (int i_4 = 0; i_4 < min(-i_3+PB_M, -i_2+PB_L-1); i_4++) {
                    B[i_3][i_4+i_2-1][i_0+1][i_0] = E[i_3][i_4+i_2+1][i_0-1][i_2] * F[i_4+i_3][i_2][i_0] * E[i_4][i_3][i_0][1] * B[i_3+2][i_4+i_2-1][i_0+1][i_0] * B[i_3][i_4+i_2-2][i_0+2][i_0+1] + B[i_4][i_2+i_0][i_3+1][i_3] - 6;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 0; i_6 < min(i_5, -i_5+PB_M+1); i_6++) {
            C[i_5][i_6+1] = G[i_6][i_5] + H[0][i_5-1] + I[i_6+i_5-1][i_6+1] * C[i_5+1][i_6+1] * 6;
        }
    }
    for (int i_7 = 1; i_7 < PB_L; i_7++) {
        for (int i_8 = 1; i_8 < PB_L; i_8++) {
            for (int i_9 = 1; i_9 < min(PB_M-1, -i_7+PB_L+1); i_9++) {
                for (int j_0 = 0; j_0 < PB_M-2; j_0++) {
                    D[j_0+1][i_8][i_9][j_0] = J[i_8][i_9-1][i_7][j_0] * E[j_0+1][i_9+i_7-1][j_0][i_7] - K[i_9+1][j_0+1][i_7-1] + L[i_8-1][i_7][j_0][i_9] + D[j_0+2][i_8-1][i_9][j_0+1] - D[i_9+1][j_0][i_7+1][i_9] + 1;
                }
            }
        }
    }
...
kernel_4424331321_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L));
