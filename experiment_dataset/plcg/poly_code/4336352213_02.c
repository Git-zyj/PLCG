...
void kernel_4336352213_02(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        C[i_0-1][i_0-1] = D[i_0+1] - E[i_0-1] - D[i_0] - 6;
    }
        ...
        for (int i_2 = 1; i_2 < min(PB_N-3, i_1); i_2++) {
            for (int i_3 = 2; i_3 < i_1; i_3++) {
                for (int i_4 = 0; i_4 < -i_3+PB_M+2; i_4++) {
                    B[i_1+2][i_4][i_3-2][i_2] = F[i_3][i_4][i_2] - G[i_4][i_2-1][i_1][i_3] * H[i_4+i_3-2][i_1][0][i_2] - I[i_3][i_2][i_4+1][i_1-2] + H[i_2][i_1-1][i_3][i_4] + B[i_2+3][i_1][i_3-2][i_4+1] + 3;
                }
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M; i_5++) {
        for (int i_6 = i_5; i_6 < -i_5+PB_M; i_6++) {
            C[i_6-1][i_6-1] = J[i_6-2][i_5] * K[i_5] - L[i_6][i_6] * K[i_5] - J[i_6][i_6+i_5] + L[i_6][i_5-2] + J[i_6][i_5-2] * C[i_6][i_6] * C[i_6-2][i_6-2] - 6;
        }
    }
#pragma endscop
