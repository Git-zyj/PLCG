...
void kernel_2324311123_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        B[i_1-1] = D[i_1+1] * D[i_1] + B[i_1-2] * 4;
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            C[i_1-1][i_2] = B[i_1-1] + 1;
        }
    }
#pragma endscop
...
kernel_2324311123_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
