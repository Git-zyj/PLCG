    ...
    for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_Q; i_2++) {
            for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
                C[i_3+2][i_2][i_3] = F[i_1-1][i_3-1][i_2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3336312113_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
