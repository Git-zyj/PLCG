...
void kernel_3226211325_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                A[i_1][i_0+1][i_2] = C[i_0-1][i_1-1][i_2-1] * A[i_1][i_0+2][i_2] - A[i_1+2][i_0+1][i_2] - A[i_1][i_0+1][i_2-1] - A[i_1+1][i_0+1][i_2] + 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                B[i_4][i_5+1] = B[i_4+2][i_5+1] - B[i_4][i_5] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3226211325_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
