...
void kernel_3236332225_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M - 1; i_0 >= 0; i_0--) {
        for (int i_1 = 2; i_1 < PB_N-2; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L; i_2++) {
                A[i_2][i_1][i_0] = C[i_1+2][0] * A[i_2][i_1-2][i_0] - 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        B[i_3] = C[i_3+1][0] * D[i_3] + E[i_3] * E[i_3-2] * B[i_3-1] * B[i_3+2] - 1;
    }
#pragma endscop
...
kernel_3236332225_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
