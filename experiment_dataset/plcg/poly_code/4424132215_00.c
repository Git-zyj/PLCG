...
void kernel_4424132215_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_M-2; i_2++) {
                for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
                    B[i_3+2][i_2+1] = E[i_2+i_1+2][i_3-2][i_3] + F[i_3][i_2-1][i_0+2][i_2-1] + E[i_2][i_1][i_2] * 5;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_M-3; i_4++) {
            B[i_4+2][i_0] = G[i_4-1][i_0+1] + H[i_4] + B[i_4+2][i_0-1] * B[i_4+3][i_0+1] * 5;
        }
    }
    for (int i_5 = 0; i_5 < PB_L; i_5++) {
        for (int i_6 = 2; i_6 < min(i_5, PB_M-2); i_6++) {
            for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
                B[i_6+2][i_7+1] = E[i_7+1][i_5][i_6-2] + E[i_5][i_7][i_6] * 6;
            }
        }
    }
        ...
        for (int i_9 = i_8; i_9 < PB_N; i_9++) {
            for (int j_0 = 1; j_0 < -i_8+PB_M; j_0++) {
                for (int j_1 = 1; j_1 < PB_M-1; j_1++) {
                    D[i_8][j_0][i_8][j_1-1] = F[i_9][j_0+i_8][j_1][i_9] - F[j_0][j_0][i_9][i_8-2] - E[j_0+2][j_1][i_9] * D[i_8][j_1+1][i_8][j_0-1] + D[i_8-1][j_1][i_8-1][j_0-1] - 3;
                }
            }
        }
    }
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4424132215_00(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
