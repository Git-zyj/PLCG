...
void kernel_3314112311_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M-1; i_1++) {
            B[i_1+1][i_1] = D[i_1+i_0+1][i_1] * D[i_0+2][i_1-2] * E[i_0] + 3;
        }
        for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
            for (int i_3 = 1; i_3 < PB_N-1; i_3++) {
                B[i_3+1][i_3] = B[i_2+2][i_2+1] - B[i_3][i_3-1] - 3;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
                C[i_6][i_6][i_5] = C[i_6+1][i_6+1][i_5+1] + C[i_6][i_6][i_5-1] + C[i_6-1][i_6-1][i_5-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3314112311_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
