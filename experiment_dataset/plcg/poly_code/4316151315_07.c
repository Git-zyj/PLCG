...
void kernel_4316151315_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
        ...
        for (int i_1 = 1; i_1 < min(i_0, PB_P-1); i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_Q-1; i_2++) {
                for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
                    A[i_1+1][i_3+1][i_2+i_0][i_2] = D[i_2][i_1+1][i_3+1][i_0+1] - D[i_1][i_2+1][i_0][i_3-1] - D[i_3][i_1+1][i_0-1][i_2] - D[i_0][i_2][i_1-1][i_2] - D[i_0-1][0][i_2][i_3] * E[i_1+1][i_0+1][i_2] + D[i_1][i_2+1][i_3+1][i_0] - A[i_1+1][i_3+2][i_2+i_0-1][i_2-1] - A[i_1+1][i_3][i_2+i_0+1][i_2] - 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        B[i_4] = F[i_4-1] + F[i_4] + F[i_4-1] + B[i_4+1] * 4;
        B[i_4] = F[i_4] + F[i_4+1] + F[i_4+1] + F[i_4] + F[i_4+1] * B[i_4] + B[i_4-1] - B[i_4+1] * 4;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4316151315_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F)));
