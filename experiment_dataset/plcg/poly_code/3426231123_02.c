...
void kernel_3426231123_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                A[i_1][i_0][i_2] = E[i_2][i_0][i_0+1] + F[i_2][i_0-1][i_1] * E[i_2][i_0][i_1] - E[i_2-1][i_1+i_0][i_2] * A[i_1][i_0][i_2-1] - 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_L, PB_N+1); i_4++) {
            B[i_3][i_4] = G[i_3-1][i_4+i_3] + H[i_4-1][i_3] * I[i_3+1] - 2;
        }
        for (int i_5 = 0; i_5 < i_3; i_5++) {
            B[i_3-2][i_5] = G[i_5+1][i_3] - G[i_5+1][i_3-1] - G[i_5+1][i_3-1] - 1;
        }
    }
    for (int i_6 = 0; i_6 < PB_P; i_6++) {
        for (int i_7 = 0; i_7 < PB_P-1; i_7++) {
            for (int i_8 = i_6; i_8 < PB_L-2; i_8++) {
                B[i_7][i_8+2] = F[i_7][i_6][i_7+1] * E[i_7][1][i_6] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3426231123_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
