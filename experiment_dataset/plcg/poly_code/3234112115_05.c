...
void kernel_3234112115_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_N+2); i_1++) {
            A[i_1+i_0-2][i_0+2] = C[i_1][i_0] * A[i_1+i_0-3][i_0+2] * 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            for (int i_4 = max(-i_3+2, 0); i_4 < min(-i_3+PB_N+2, PB_N-2); i_4++) {
                A[i_4+i_3-2][i_4+2] = 1;
            }
        }
    }
...
kernel_3234112115_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
