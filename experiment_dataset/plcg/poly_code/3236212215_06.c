...
void kernel_3236212215_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = -i_0+PB_P - 1; i_2 >= 2; i_2--) {
                A[i_1][i_2-2] = C[i_2][i_2+i_0][i_0] + A[i_2-1][i_0-1] - 2;
            }
        }
    }
...
kernel_3236212215_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
