...
void kernel_3424252223_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        B[i_0] = E[i_0] * E[i_0+1] - F[i_0-2] + F[i_0] + E[i_0] - 6;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        B[i_1] = F[i_1] + F[i_1-1] + F[i_1-1] + F[i_1] + E[i_1] + F[i_1] - F[i_1] + B[i_1-1] - 3;
        for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_M-2); i_2++) {
            B[i_2+1] = G[i_1][i_2] + H[i_2+i_1][i_2] + F[i_2+i_1+2] + E[i_1] * D[i_1][i_2+2][i_1+2] * 2;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < min(i_4, PB_L-2); i_5++) {
                D[i_3][i_5][i_3+2] = H[i_4][i_4-1] * G[i_4-1][i_5+2] - I[i_4][i_3][i_4] * I[i_3][i_5][i_4] - D[i_5+2][i_3][i_5+4] * D[i_4][i_5+2][i_4+2] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3424252223_01(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
