...
void kernel_4426332325_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
                for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                    A[i_2][i_3+1][i_0-2][i_1+2] = E[i_0][i_2][i_1-1][i_3-1] - F[i_1][i_3][i_2] - G[i_3][i_0+1][i_1][i_2+2] + H[i_1][i_3+1][i_0-1][i_2] * A[i_2-1][i_0+1][i_3-1][i_1+2] - A[i_0+1][i_3][i_2-2][i_1+2] - 1;
                }
            }
        }
        for (int i_4 = i_0; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                B[i_4-1][i_5+1][i_0+2] = I[i_4][i_0][0] - B[i_4+1][i_5+1][i_0+2] + 3;
            }
        }
    }
    for (int i_6 = 4; i_6 < PB_M; i_6++) {
        D[i_6-4][i_6-2] = J[i_6] * D[i_6-2][i_6] + D[i_6-3][i_6-1] * 1;
    }
    for (int i_7 = 0; i_7 < PB_L; i_7++) {
        for (int i_8 = max(4, i_7); i_8 < PB_M; i_8++) {
            D[i_8-2][i_8] = K[i_8-1][i_7] * K[i_8][i_7] + K[i_8][0] * L[i_8-1] * D[i_8-3][i_8-1] * D[i_8-1][i_8+1] + D[i_8-4][i_8-2] + 4;
        }
    }
#pragma endscop
...
kernel_4426332325_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
