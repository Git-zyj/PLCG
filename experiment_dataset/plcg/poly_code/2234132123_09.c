...
void kernel_2234132123_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M-4, -i_0+PB_N); i_1++) {
            A[i_1+2][i_0] = C[i_1+i_0][i_0] - C[i_0][i_0+2] * D[i_1] + D[i_1] - A[i_1+4][i_0] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_N, PB_L); i_3++) {
            B[i_2][i_3] = C[0][i_3] - C[i_3+i_2][i_2] + B[i_2+1][i_3] + 4;
        }
    }
#pragma endscop
...
kernel_2234132123_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
