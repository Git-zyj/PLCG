...
void kernel_2236151215_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = -i_0+PB_M-1 - 1; i_1 >= 1; i_1--) {
            B[i_0][i_1] = C[i_1][i_0][i_1] * D[i_0][1] - D[i_1][i_0] * D[i_0][i_1+i_0+1] + E[i_1-1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            B[i_2][i_3] = D[i_3-1][i_2+1] + D[i_3][i_2] - D[i_2+1][i_3] - D[i_3-1][0] + D[i_3+1][i_2] - B[i_2-1][i_3] * B[i_3-1][i_2] + 4;
        }
    }
#pragma endscop
...
kernel_2236151215_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
