...
void kernel_3416331211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_N-1 - 1; i_0 >= 0; i_0--) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1+1][i_0+1] = E[i_1] - F[i_1+1][i_0] * G[i_0][i_1][i_0] - H[i_1-1][i_0] + F[i_1][i_1-1] - D[i_1] * 4;
            for (int i_2 = 0; i_2 < -i_0+PB_P; i_2++) {
                A[i_0+1][i_1+1] = G[i_1][i_0][i_2+i_0] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < i_3; i_5++) {
                C[i_3][i_4][i_3+1] = G[i_4][i_3-1][i_4] - F[i_5+1][1] - I[i_5][i_3][i_4+i_3] * I[i_5][i_3+1][i_4] - I[i_5][0][i_4+i_3] + C[i_3+1][i_4][i_3+2] * C[i_3][i_4+1][i_3+1] - 2;
            }
        }
        D[i_3] = E[i_3] * D[i_3-1] + D[i_3+1] + 5;
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3416331211_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
