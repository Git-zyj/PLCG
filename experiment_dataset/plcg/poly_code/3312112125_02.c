...
void kernel_3312112125_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_P+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                A[i_0+1][i_1-1][i_2] = A[i_0+3][i_1-2][i_2] + 3;
            }
        }
        for (int i_3 = 0; i_3 < min(-i_0+PB_M, PB_M-1); i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                A[i_3+1][i_0+1][i_4+2] = D[i_3+i_0][i_4][i_4] - 1;
            }
        }
    }
...
kernel_3312112125_02(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
