...
void kernel_4226131115_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_1][i_2] = C[i_0-1][i_1-1] - 5;
            }
            A[i_1][i_0] = C[i_1][i_0+1] + C[i_1][i_1] + C[i_1][i_0] * C[i_0+1][i_1-1] * C[i_1+i_0-1][0] + A[i_1][i_0] + 2;
        }
    }
#pragma endscop
...
kernel_4226131115_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
