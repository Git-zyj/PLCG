...
void kernel_3422312215_01(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            C[i_0-1][i_0-1] = E[i_1+2][i_0] * F[i_1][i_0] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        C[i_2][i_2] = 4;
        for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
            C[i_2][i_2] = E[i_3+i_2][i_3-1] + C[i_2-1][i_2-1] * 4;
        }
        for (int i_4 = 0; i_4 < i_2; i_4++) {
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                C[i_2+1][i_2+1] = C[i_4+1][i_4+1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3422312215_01(xc,yc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
