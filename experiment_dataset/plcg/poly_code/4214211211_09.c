...
void kernel_4214211211_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
        ...
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    A[i_1][i_2][i_3] = A[i_1+1][i_3+1][i_0] * A[i_3][i_2-1][i_1+1] + A[i_1+1][i_2][i_3] * 2;
                }
            }
            B[i_0][i_0] = C[i_1][i_0] * B[i_1-1][i_1-1] + 4;
        }
    }
#pragma endscop
...
kernel_4214211211_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
