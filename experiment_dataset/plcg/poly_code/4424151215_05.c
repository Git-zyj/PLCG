...
void kernel_4424151215_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[i_0-1] - E[i_0+1] * E[i_0+1] * E[i_0+1] * E[i_0] + E[i_0] + A[i_0] * A[i_0+2] + 1;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_L, -i_2+PB_M); i_3++) {
                D[i_2][i_1-2][0] = F[i_3][i_3][i_2] * G[i_3+i_2][i_3+i_1] - F[i_3][i_2][i_1-1] - H[i_1][i_2][i_3][i_2] - 3;
            }
        }
        A[i_1+1] = E[i_1] - E[i_1] + E[i_1+1] + E[i_1] + E[i_1] - E[i_1-1] * E[i_1] - A[i_1+2] - 6;
    }
    for (int i_4 = 0; i_4 < PB_N; i_4++) {
        for (int i_5 = i_4; i_5 < PB_P; i_5++) {
            for (int i_6 = 1; i_6 < PB_L; i_6++) {
                for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
                    D[i_5][i_7-1][0] = F[i_6][i_7][i_5] + F[i_7][i_5][i_6] - F[i_4][i_6][i_7+1] - D[i_6-1][i_5][0] + 5;
                }
            }
        }
    }
...
kernel_4424151215_05(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
