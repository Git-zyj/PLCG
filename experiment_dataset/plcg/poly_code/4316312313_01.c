...
void kernel_4316312313_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M-1; i_1++) {
            for (int i_2 = 3; i_2 < PB_L+1; i_2++) {
                A[i_1][i_2-2] = A[i_1+1][i_2-1] + A[i_0][i_2-3] + A[i_1-1][i_2-3] + A[i_2-1][i_1-2] + A[i_2][i_1-1] + 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        ...
        for (int i_6 = max(2, i_3); i_6 < PB_N; i_6++) {
            for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
                for (int i_8 = 1; i_8 < PB_P-1; i_8++) {
                    C[i_7-1][i_6-2][i_8] = D[i_6][i_8][i_7+1] * C[i_7-1][i_6-1][i_8+1] - C[i_7][i_6-2][i_8] + C[i_8-1][i_6-2][i_7+1] + 5;
                }
            }
        }
    }
...
kernel_4316312313_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
