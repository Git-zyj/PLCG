...
void kernel_2232132313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L-3, -i_0+PB_M+2); i_1++) {
            A[i_0][i_1+2] = C[i_1][i_0] * C[i_1+2][i_0] * C[i_1+i_0-2][i_0] * C[i_1][i_1+1] - C[i_1][i_0] + A[i_0+1][i_1+2] + A[i_0][i_1+1] + A[i_0][i_1+3] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-3; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            B[i_2+2][i_3+2] = C[i_2+1][i_3+2] + B[i_2+1][i_3+2] * B[i_2+3][i_3+2] * B[i_2+2][i_3+3] * 5;
        }
    }
#pragma endscop
...
kernel_2232132313_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
