...
void kernel_2432211323_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][0] = A[i_0+2][0] - A[i_0-1][0] + A[i_1+2][0] + A[i_0-2][0] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            B[i_3][i_2+1] = B[i_3][i_2+2] * B[i_3][i_2] - B[i_3][i_2-1] + B[i_2+1][i_3+2] - 3;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        C[i_4+1] = E[i_4-1] - E[i_4] * 1;
        for (int i_5 = 0; i_5 < -i_4+PB_M; i_5++) {
            D[i_5+i_4][i_4-1] = C[i_5+1] + 4;
        }
    }
#pragma endscop
...
kernel_2432211323_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
