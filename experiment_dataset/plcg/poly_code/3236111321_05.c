...
void kernel_3236111321_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = A[i_1-2][i_0] + A[i_1][i_0+2] - A[i_1][i_0-2] - A[i_1+1][i_0] * A[i_1-1][i_0] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_2+1] = C[i_2][i_3][i_2] - B[i_2-1] * 2;
        }
    }
#pragma endscop
...
kernel_3236111321_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
