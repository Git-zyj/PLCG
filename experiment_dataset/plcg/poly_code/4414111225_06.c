...
void kernel_4414111225_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < min(i_1, PB_L-2); i_2++) {
                A[i_2][i_1][i_0] = A[i_2-1][i_1][i_0] * A[i_2+2][i_0][i_1] * A[i_2][i_1][i_0-1] - 2;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_Q; i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                for (int i_5 = 1; i_5 < PB_M; i_5++) {
                    A[i_3][i_4][i_5] = E[i_5][i_5-1][i_0][i_3] * 2;
                }
                for (int i_6 = 1; i_6 < PB_L+1; i_6++) {
                    C[i_0][i_6-1][i_4][i_3+i_0] = C[i_0-1][i_6-1][i_4][i_3+i_0-1] - 6;
                }
            }
        }
    }
...
kernel_4414111225_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
