...
void kernel_2112332215_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = PB_L+2 - 1; i_1 >= 2; i_1--) {
            A[i_1][i_0-1] = B[i_1-2][i_0] - C[i_1] * D[i_1][i_0+2] - A[i_1][i_0] - A[i_1-1][i_0-1] * 2;
        }
    }
#pragma endscop
...
kernel_2112332215_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
