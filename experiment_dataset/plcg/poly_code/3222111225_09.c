...
void kernel_3222111225_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_P, i_1); i_2++) {
                B[i_0+1][i_2+i_1][i_2] = 1;
            }
        }
        for (int i_3 = i_0; i_3 < PB_P-2; i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_P-2; i_4++) {
                B[i_0-1][i_4+i_3][i_3] = C[i_4][i_0] * B[i_0+1][i_4+i_3+1][i_3+1] - B[i_0+1][i_4+i_3+2][i_3+2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3222111225_09(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
