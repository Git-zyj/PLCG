...
void kernel_2434311211_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_N-1, i_0); i_1++) {
            A[i_0][i_1] = E[i_1][i_0] * F[i_1+i_0+1] - A[i_0+1][i_1] * 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_3-1][i_3] = G[i_3][i_2+1] + 6;
        }
        for (int i_4 = i_2; i_4 < PB_M; i_4++) {
            D[i_2-1] = 3;
        }
        D[i_2-1] = E[i_2+1][i_2] * B[i_2-1][i_2] - B[i_2-2][i_2-1] * B[i_2][i_2+1] - 2;
    }
#pragma endscop
...
kernel_2434311211_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
