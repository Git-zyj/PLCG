...
void kernel_2422251221_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0] - E[i_0] + E[i_0] * F[i_0] - E[i_0-1] - F[i_0+1] + E[i_0] - E[i_0+1] + A[i_0+2] + A[i_0-1] + A[i_0+1] - 6;
    }
    for (int i_1 = 2; i_1 < PB_M+1; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_M, -i_1+PB_L); i_2++) {
            B[i_2][i_1] = G[i_1-1][i_2+i_1] + G[i_2][i_1-1] + B[i_1+1][i_2] * B[i_2][i_1-2] * 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M; i_4++) {
            A[i_4] = H[i_4+i_3][i_4] * G[i_4][i_4+i_3] + F[i_4+i_3] - H[i_4+i_3-1][0] + 2;
        }
        for (int i_5 = 1; i_5 < PB_N+1; i_5++) {
            D[i_5] = H[i_5-1][1] - E[i_5-1] * G[i_5][i_3] + H[i_5-1][i_3] * H[i_3][i_5-1] + D[i_5+2] * 5;
        }
    }
#pragma endscop
...
kernel_2422251221_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
