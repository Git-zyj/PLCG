...
void kernel_3434252225_08(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        D[i_0-2] = E[i_0] - E[i_0] + F[i_0+2] + F[i_0] + E[i_0-1] * F[i_0-1] + E[i_0+2] - D[i_0-1] - 5;
    }
    for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            D[i_2-2] = G[i_2-1][i_1+1] * G[i_1][i_2-1] * 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_N; i_3++) {
        for (int i_4 = PB_P-1 - 1; i_4 >= 2; i_4--) {
            for (int i_5 = 2; i_5 < min(-i_4+PB_M, -i_3+PB_Q); i_5++) {
                D[i_5] = H[i_5-2][i_3+2][i_4+1] + I[i_4][i_3][i_5+i_3] * I[i_5+i_4][i_5+i_3+1][i_4-2] * I[i_4][i_3][i_5] + 4;
            }
        }
        D[i_3] = F[i_3] * F[i_3+2] * F[i_3-2] - E[i_3] - E[i_3] + D[i_3+1] * 6;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3434252225_08(xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
