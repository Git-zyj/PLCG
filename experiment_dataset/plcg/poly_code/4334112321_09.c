...
void kernel_4334112321_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-3; i_0++) {
        A[i_0+1][i_0] = A[i_0-1][i_0-2] + A[i_0+3][i_0+2] - A[i_0][i_0-1] + 3;
    }
    for (int i_1 = 4; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 4; i_2 < min(-i_1+PB_M, PB_L+2); i_2++) {
            B[i_2+i_1][i_2-2] = D[i_1] - B[i_2+i_1-4][i_2-4] - B[i_2+i_1-2][i_2-4] * 4;
        }
        for (int i_3 = 2; i_3 < min(-i_1+PB_M, PB_L+2); i_3++) {
            C[i_1][i_1] = B[i_3+i_1][i_3-2] * C[i_1+1][i_1+1] - C[i_1-2][i_1-2] - 3;
        }
    }
#pragma endscop
...
kernel_4334112321_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
