...
void kernel_4224111215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                A[i_1][i_2] = A[i_1-1][i_2-1] + A[i_1-1][i_2+1] - 2;
            }
        }
        for (int i_3 = i_0; i_3 < PB_N; i_3++) {
            A[i_3][i_0] = C[i_3][i_0-1] * 5;
        }
    }
#pragma endscop
...
kernel_4224111215_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
