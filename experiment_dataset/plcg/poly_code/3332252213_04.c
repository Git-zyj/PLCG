...
void kernel_3332252213_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        A[i_0] = D[i_0] + E[i_0] - E[i_0] - D[i_0] * A[i_0+1] * A[i_0-1] + 3;
        for (int i_1 = PB_Q-2 - 1; i_1 >= 2; i_1--) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                B[i_1][i_2+1] = F[i_1-2][i_2+1][i_0] + F[i_1][i_0][i_1+2] * G[i_2][i_1] - F[i_1][i_2][i_0] - B[i_0+1][i_2+1] + 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        A[i_3] = E[2] + D[i_3+2] - D[i_3] + D[i_3] - E[0] - E[i_3+2] - A[i_3] + 3;
    }
#pragma endscop
...
kernel_3332252213_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
