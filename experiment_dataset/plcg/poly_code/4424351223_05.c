...
void kernel_4424351223_05(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            C[i_1-1][i_1+i_0] = E[0] + E[i_1-1] - F[i_0][i_0] - 1;
        }
        for (int i_2 = max(1, i_0); i_2 < min(PB_M-1, -i_0+PB_L-1); i_2++) {
            C[i_2+1][i_2+i_0+1] = G[i_2-1][i_0] + G[i_2][i_0] * H[i_0][i_2] - G[i_0][i_2] + 1;
        }
    }
    for (int i_3 = 3; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_L-2, PB_M-1); i_4++) {
            C[i_4][i_4+i_3] = F[i_4-1][i_3] * F[i_3+1][i_4] - G[i_4][i_3-1] + H[i_3][i_3+1] * C[i_4-1][i_4+i_3-1] - C[i_4+1][i_4+i_3+2] * C[i_4-2][i_4+i_3-3] + 3;
        }
    }
    for (int i_5 = 1; i_5 < PB_N; i_5++) {
        D[i_5] = I[i_5-1] * I[i_5+1] * J[i_5] + J[i_5-1] - J[i_5] - E[i_5] + D[i_5+1] - 5;
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4424351223_05(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
