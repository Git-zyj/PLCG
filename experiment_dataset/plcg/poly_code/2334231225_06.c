...
void kernel_2334231225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = D[i_0-1] - E[i_0] + E[i_0-1] + 6;
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M-2, PB_L); i_1++) {
            A[i_0-2] = F[i_1-1][i_0] - C[i_1+i_0+2][i_0+1][0] * C[i_1+i_0+2][i_1][0] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_M-3, PB_L-1); i_3++) {
            C[i_3+i_2][i_3-1][0] = F[i_3][i_2-1] * G[i_3][i_2] - G[i_3][i_2] * A[i_3] - C[i_3+i_2+3][i_3+1][0] - 6;
        }
    }
#pragma endscop
...
kernel_2334231225_06(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
