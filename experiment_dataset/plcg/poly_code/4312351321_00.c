...
void kernel_4312351321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            A[i_0][i_1] = D[1] + E[0] * E[i_1-1] * F[i_1][i_1+1] + E[i_1] + A[i_0-2][i_1] * C[i_1+1] + C[i_1+2] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            C[i_2-1] = G[i_2-1][i_2] * D[i_3] - F[i_2][i_3] + G[i_3][i_3] + H[i_2-1][i_3] - C[i_3+1] - 6;
        }
        for (int i_4 = 1; i_4 < min(PB_M-3, -i_2+PB_M); i_4++) {
            C[i_4+1] = H[i_2][i_4] - H[i_2][i_2] + H[i_4+i_2][i_4+i_2-1] + F[i_4-1][i_2+1] + D[i_4+1] + C[i_4+3] + 3;
        }
    }
#pragma endscop
...
kernel_4312351321_00(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
