...
void kernel_2332151223_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        B[i_0] = D[i_0-1] - D[i_0] * D[i_0] + D[i_0] + D[i_0] * 1;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1] = D[i_1] * D[i_1-1] + D[i_1+1] * D[i_1-1] * B[i_1-1] + B[i_1+1] + 1;
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        B[i_2+1] = D[i_2] - D[0] + D[i_2] + D[i_2] * D[i_2] - D[i_2] * 5;
    }
#pragma endscop
...
kernel_2332151223_04(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
