...
void kernel_4432352221_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,int vj,int wj,DATA_TYPE POLYBENCH_5D(J,xJ,yJ,zJ,vJ,wJ,xj,yj,zj,vj,wj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q, PB_Q-2); i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < i_1; i_3++) {
                    A[i_0+1][i_3][i_2][i_0] = E[0][i_2][i_0][i_1+i_0] * F[i_2-1][i_0][i_3][i_1+2] * G[i_3][i_2][i_1][i_0] - F[i_3][i_0][i_2][i_3] - 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 2; i_5 < -i_4+PB_M+1; i_5++) {
            B[i_5] = H[i_5][2] * I[i_5+i_4-1][i_4+1] - H[i_4+1][i_5] + B[i_5-1] + B[i_5-2] - 6;
        }
        for (int i_6 = 0; i_6 < PB_P-2; i_6++) {
            for (int i_7 = 0; i_7 < PB_Q-1; i_7++) {
                for (int i_8 = 2; i_8 < PB_L-2; i_8++) {
                    C[i_7][i_8][i_4][i_4-1] = J[i_8+2][i_6+2][i_4][i_8-1][i_7] + F[i_7][i_8+1][i_6][i_4-1] - F[i_6][i_4-1][i_8][i_7+1] + F[i_8-2][i_7][i_6+2][i_7] - C[i_7][i_8][i_4+1][i_4] + 4;
                }
            }
        }
    }
    for (int i_9 = 1; i_9 < PB_M; i_9++) {
        for (int j_0 = 1; j_0 < PB_M; j_0++) {
            for (int j_1 = 0; j_1 < min(min(-j_0+PB_M, -i_9+PB_M), PB_Q-4); j_1++) {
                D[i_9][j_1][j_1+2] = K[j_0][j_0-1][i_9-1] - L[j_1+i_9][2][j_0-1] + M[j_1][i_9][j_0] + M[j_1][j_1][i_9] + L[j_1][i_9][i_9] - M[j_1+j_0][i_9][j_0] * D[i_9][j_1+2][j_1+4] * D[i_9-1][j_1][j_1+2] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4432352221_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
