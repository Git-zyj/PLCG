...
void kernel_2316312211_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1][i_0][i_0-1] = D[i_1] * B[i_0+3][i_1] + B[i_0+1][i_1-1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            B[i_2+2][i_3] = E[i_3][i_2] - A[i_2][i_3-1][i_3-2] * 5;
            C[i_2][i_3] = A[i_3][i_2][i_2-1] + B[i_2+1][i_3] - C[i_2+1][i_3] + 6;
        }
    }
#pragma endscop
...
kernel_2316312211_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
