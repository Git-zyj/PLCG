...
void kernel_3334212115_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_N-2; i_2++) {
            for (int i_3 = 0; i_3 < i_2; i_3++) {
                B[i_2+2][i_3] = B[i_3+2][i_2-1] - 2;
            }
        }
        for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
            B[i_4+1][i_1] = E[i_1][i_4-1] * 6;
        }
    }
#pragma endscop
...
kernel_3334212115_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
