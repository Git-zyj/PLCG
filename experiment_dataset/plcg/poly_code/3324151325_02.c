...
void kernel_3324151325_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_1] = D[i_1] * E[i_0][i_1] * E[i_1+1][i_0] + E[i_1][i_0-1] - D[i_1] * E[i_1+1][i_0+1] * A[i_0+2] + A[i_1+1] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        C[i_2-2] = E[i_2][1] * D[i_2-1] + D[i_2] - D[i_2-1] + 3;
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        C[i_3-1] = D[i_3-1] - D[i_3] * D[i_3-1] + C[i_3-2] - C[i_3+1] * C[i_3] - 4;
    }
#pragma endscop
...
kernel_3324151325_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
