...
void kernel_3232112225_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[i_1-1][i_2-2] = A[i_1-1][i_2] * A[i_2-2][i_1-2] + 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        B[i_3] = C[i_3-1] - C[i_3] - B[i_3-1] * B[i_3-2] * 4;
    }
#pragma endscop
...
kernel_3232112225_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
