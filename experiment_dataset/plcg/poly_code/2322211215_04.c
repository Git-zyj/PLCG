...
void kernel_2322211215_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_N+2; i_1++) {
            B[i_1-2] = D[i_1+1][i_0] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        B[i_2-1] = B[i_2-2] - B[i_2] + B[i_2] + 6;
    }
...
kernel_2322211215_04(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
