...
void kernel_4432331111_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
        ...
        for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    A[i_2+1][i_3][i_1][i_0] = E[i_3][i_0+1][i_2][i_1] * E[i_3][i_0-1][i_0][i_1+1] + C[i_1][i_2][i_3] * 2;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                B[i_4+1][i_5-1][0] = F[i_5+1][i_4] + 2;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
            for (int i_8 = 1; i_8 < i_7; i_8++) {
                C[i_6+1][i_7][i_8] = G[i_6][i_8-1][i_7+1] - H[i_7][i_6-1][0] * I[i_7][i_7][i_6-1] * C[i_6+1][i_7+1][i_8] - 1;
            }
        }
    }
    for (int i_9 = 0; i_9 < PB_M-1; i_9++) {
        D[i_9] = J[0] - K[i_9] * L[i_9+1] + J[i_9] + K[i_9] + D[i_9+1] + 3;
    }
#pragma endscop
...
kernel_4432331111_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
