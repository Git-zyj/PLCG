...
void kernel_2426351223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_0][i_1-1] = E[i_1] - F[i_0][i_1] + G[i_0-1][i_1+1] - G[i_1][i_0+1] - H[i_0][i_1] + A[i_0-1][i_1-1] + A[i_0-2][i_1-1] + A[i_0][i_1+1] + A[i_0][i_1] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        B[i_2] = E[0] - I[0] * J[i_2-1] + E[i_2+1] * E[i_2+1] + I[i_2] * B[i_2+1] * 1;
        C[i_2] = I[i_2-1] - I[i_2] - G[i_2][i_2] + J[i_2+1] * C[i_2-2] * 1;
        for (int i_3 = i_2; i_3 < min(-i_2+PB_N+1, PB_L-1); i_3++) {
            D[i_3+i_2-1] = G[i_3][i_2] - H[i_3][i_2] * G[i_2][i_3-1] * F[i_3][i_2-1] + A[i_2][i_3+1] + B[i_2] + 5;
        }
    }
#pragma endscop
...
kernel_2426351223_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
