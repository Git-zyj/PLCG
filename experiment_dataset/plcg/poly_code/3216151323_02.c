...
void kernel_3216151323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N; i_1++) {
            A[i_0][i_0] = C[i_0+1][i_1] + C[i_1+i_0][0] + A[i_0-2][i_0-2] + A[i_0-1][i_0-1] - A[i_0+2][i_0+2] + 6;
        }
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            for (int i_3 = i_0; i_3 < -i_2+PB_N+1; i_3++) {
                B[i_0][i_2][i_3] = C[i_3+i_2-1][0] - C[i_3][i_0+1] * D[i_2][i_2][i_0-1] * D[i_2+1][i_0][i_3] + D[i_0][i_3][i_2+1] + D[i_3][i_2][i_0+1] * C[i_2-1][i_3] + B[i_2][i_3][i_0+2] + B[i_3][i_2-1][i_0-2] + B[i_2][i_0+1][i_3] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3216151323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
