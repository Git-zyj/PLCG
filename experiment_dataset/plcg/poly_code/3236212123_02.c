...
void kernel_3236212123_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_N-3; i_2++) {
                B[i_0+2][i_2][i_1] = 4;
                B[i_0+2][i_1][i_2] = C[i_0-2][i_2] + B[i_2+3][i_1][i_0] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3236212123_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
