...
void kernel_2436352115_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            A[i_0][i_1] = E[i_0] + E[i_0] + F[i_1][0] * F[i_1][i_1-2] + G[i_1-1][i_0] + A[i_0-1][i_1] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_N+2; i_2++) {
        B[i_2+1] = E[i_2] * H[i_2] + I[i_2-2] + H[i_2-1] - J[i_2][0] * 4;
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M, i_3); i_4++) {
            A[i_3][i_4] = G[i_4-1][i_3] + F[i_4][i_3] + J[i_4+i_3][i_3+2] * 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        for (int i_6 = i_5; i_6 < PB_N; i_6++) {
            A[i_5-1][i_6+1] = J[i_5][0] - F[i_6+2][i_5] + G[i_6][i_5] + F[i_6][i_5] - J[i_6][i_5] + I[i_6] - H[i_5+1] * 2;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2436352115_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
