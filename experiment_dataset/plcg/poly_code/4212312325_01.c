...
void kernel_4212312325_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 3; i_3 < min(-i_2+PB_Q, PB_L); i_3++) {
            for (int i_4 = 1; i_4 < -i_2+PB_M; i_4++) {
                B[i_3][i_4-1] = C[i_3][i_2+1][i_4+i_2] - D[i_2][i_4][i_3+i_2] + B[i_3-1][i_4-1] * B[i_2][i_3-3] - B[i_4-1][i_3-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4212312325_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
