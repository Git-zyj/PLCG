...
void kernel_2314232225_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        B[i_0-3] = D[i_0] * E[i_0] + E[i_0+1] - 1;
    }
    for (int i_1 = 2; i_1 < PB_L+1; i_1++) {
        B[i_1-2] = D[i_1] + E[i_1] + 4;
        for (int i_2 = 2; i_2 < min(PB_L, i_1); i_2++) {
            C[i_1][i_2] = F[i_1][i_2][i_1-1] * G[i_2+1][i_1] * G[i_2][i_1] - G[i_2-2][i_1+2] - C[i_1+1][i_2-2] - C[i_1-1][i_2-1] + C[i_1-2][i_2] * 4;
        }
    }
#pragma endscop
...
kernel_2314232225_00(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
