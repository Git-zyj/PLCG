...
void kernel_3332111123_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_1-1][i_0] = A[i_1-3][i_2] + A[i_1][i_2] * 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
            C[i_3][0] = D[i_3+1][i_4+1] * 2;
        }
    }
...
kernel_3332111123_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
