...
void kernel_4226312325_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_Q; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
                A[i_1][i_2+2][i_1+i_0] = C[i_2-2][i_1][i_0+1] - D[i_0-2][i_1+2][i_2-1] - A[i_1][i_2+2][i_1+i_0-1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            B[i_4+1] = B[i_4+3] - B[i_4-1] * B[i_4] * B[i_4+2] + 5;
        }
    }
#pragma endscop
...
kernel_4226312325_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
