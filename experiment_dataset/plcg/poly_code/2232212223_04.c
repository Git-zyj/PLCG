...
void kernel_2232212223_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-4; i_1++) {
            A[i_1+2] = C[i_1-2] - A[i_1+4] + A[i_1+3] * 1;
        }
        for (int i_2 = 0; i_2 < PB_P-3; i_2++) {
            B[i_0+2][i_2+1] = B[i_0+1][i_2+1] + B[i_0+3][i_2+3] - 4;
        }
    }
#pragma endscop
...
kernel_2232212223_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
