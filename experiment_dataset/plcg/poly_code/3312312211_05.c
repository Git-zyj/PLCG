...
void kernel_3312312211_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
...
#pragma scop
    ...
    for (int i_1 = PB_N-2 - 1; i_1 >= 1; i_1--) {
        B[i_1] = D[i_1] * E[i_1+2] * B[i_1+1] - B[i_1-1] + B[i_1+1] * B[i_1-1] - 1;
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                C[i_1][i_2+i_1][i_3] = F[i_2][i_3][i_1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3312312211_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
