...
void kernel_2212152223_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = C[i_0] - D[i_0][i_0] * C[i_0] * C[i_0-2] * C[i_0] - C[i_0] - A[i_0+2] - 3;
    }
    for (int i_1 = 4; i_1 < PB_N; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_M+2; i_2++) {
            B[i_1-2][i_2] = E[i_2+i_1-2][i_2-1][i_1] * D[i_2][i_2] * D[i_1][i_2] * C[i_2] * B[i_1-4][i_2-2] + B[i_1-2][i_2+2] + B[i_1][i_2] * 3;
        }
    }
#pragma endscop
...
kernel_2212152223_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
