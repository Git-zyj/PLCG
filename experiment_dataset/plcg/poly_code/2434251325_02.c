...
void kernel_2434251325_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = PB_P - 1; i_1 >= max(1, i_0); i_1--) {
            A[i_1] = E[i_0][i_1+1] * E[i_1][i_0] - F[i_1-1][i_0] + G[i_1][i_0][1] + E[i_0][i_1-1] + A[i_1+1] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        C[i_2+1] = H[i_2] - I[i_2+1] + I[i_2] + I[i_2] * C[i_2+1] + 1;
        C[i_2+1] = I[i_2] + A[i_2] * A[i_2-1] + 6;
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        C[i_3-1] = I[i_3+1] * I[i_3-1] + I[i_3] - I[i_3] - H[i_3-1] - I[i_3] - H[i_3+1] - I[i_3] * A[i_3] - 6;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2434251325_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
