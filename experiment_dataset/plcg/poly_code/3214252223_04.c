...
void kernel_3214252223_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < min(min(min(-i_1+PB_L-1, i_0), PB_P-2), -i_0+PB_P); i_2++) {
                A[i_2][i_1] = C[i_1+1][i_0-1][i_2+2] - D[i_1][i_2+i_0][i_2] * D[i_2+i_1][i_0][0] - C[i_2][i_1][i_0] - E[i_2-2][i_0] - F[i_2+i_1+1][i_0] - C[i_1+2][i_0][i_2+1] + A[i_2][i_1-1] - A[i_2+1][i_1] * 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        B[i_3] = E[i_3+2][0] - G[i_3] + H[i_3+1] - B[i_3+1] + B[i_3-2] * 3;
    }
#pragma endscop
...
kernel_3214252223_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
