...
void kernel_3136252323_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(min(PB_L, -i_0+PB_N-3), i_0); i_1++) {
            A[i_1+i_0] = B[i_1+2][i_0] - B[i_1-1][i_0] + B[i_1][i_0] - B[i_1+i_0+1][0] * C[i_0-1][i_1] + A[i_1+i_0-2] - A[i_1+i_0+3] * A[i_1+i_0-1] * 2;
        }
    }
#pragma endscop
...
kernel_3136252323_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
