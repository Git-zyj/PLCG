...
void kernel_3314351215_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1-1] = D[i_1+i_0][0] * D[i_1+i_0][i_1+1] * E[i_0] + D[i_1][i_0-1] * D[i_1][i_0] * A[i_0][i_1] * A[i_1][i_0] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_N; i_3++) {
            A[i_2-1][i_3-1] = F[i_3+1][i_2] + G[i_3][i_2+1] - D[i_3][1] + D[i_2+1][i_2+1] + F[i_3][i_2+1] - 3;
            A[i_2][i_3] = D[i_2][i_3] * D[i_3][i_2] - H[i_3][i_2-1][0] - 3;
        }
    }
#pragma endscop
...
kernel_3314351215_00(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
