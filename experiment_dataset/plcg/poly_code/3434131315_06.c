...
void kernel_3434131315_06(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < min(PB_P, -i_0+PB_L); i_2++) {
                D[i_2+i_0] = E[i_1][i_2][i_0][0] - F[i_2+i_0][i_1+i_0][0] - F[i_2][i_0-1][i_1] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        B[i_3] = B[i_3-1] - B[i_3+1] * B[i_3-1] * 1;
        B[i_3] = G[i_3][i_3] * H[i_3] - H[i_3] - B[i_3] - 1;
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        for (int i_5 = 0; i_5 < min(PB_N, -i_4+PB_L); i_5++) {
            D[i_5+i_4] = G[i_5][i_4] - G[i_4][i_5+i_4] * H[i_5] + G[i_5][i_4] - D[i_5+i_4-1] - 4;
        }
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3434131315_06(xd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
