...
void kernel_3216111315_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L-1, i_1); i_2++) {
                B[i_2+1][i_1-1][i_0] = C[i_0][i_2][i_1] + 3;
            }
        }
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_L-2, i_3); i_4++) {
                B[i_4+1][i_3][i_0] = B[i_4+1][i_3-1][i_0] + B[i_0+1][i_3][i_4-1] * B[i_4+2][i_3-1][i_0-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3216111315_07(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
