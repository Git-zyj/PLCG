...
void kernel_4412132115_08(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_Q+1, PB_Q-2); i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_M; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_2+PB_P, -i_1+PB_Q); i_3++) {
                    B[i_0][i_2][i_3+i_1][i_3] = E[i_1][i_2][i_3][i_0] + E[i_2-2][i_1][i_0][i_3+i_1] * E[i_3][i_0-2][i_2][i_1+2] * 1;
                    B[i_0][i_1-1][i_3+i_2][i_3] = F[i_2][i_0][i_3+i_1] - G[i_0][i_3][i_3-2][i_1+i_0-1][i_2] + E[i_2+i_1][i_2][i_3][i_0] * 4;
                }
            }
        }
        C[i_0] = H[0] + H[i_0] + H[i_0] - H[i_0+2] * C[i_0-1] + 4;
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4412132115_08(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
