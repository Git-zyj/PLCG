...
void kernel_3236251215_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = max(1, i_0); i_2 < PB_Q; i_2++) {
                A[i_2][i_2] = C[i_1+1][i_0][i_2] + C[i_2][i_0][i_1+1] + D[i_1+1][i_1] + A[i_2-1][i_2-1] * 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_Q, -i_3+PB_M); i_4++) {
            for (int i_5 = 1; i_5 < min(PB_Q, -i_4+PB_P); i_5++) {
                A[i_5+1][i_5+1] = E[i_5][i_4+i_3][i_4-1] * C[i_4][i_3][0] - D[i_5-1][i_4] - E[i_5][i_4+i_3-1][0] * C[i_3][i_5+i_4][i_3+1] - D[i_3-1][i_4+1] - C[i_4+i_3][i_5][i_3] * A[i_4][i_4] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3236251215_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
