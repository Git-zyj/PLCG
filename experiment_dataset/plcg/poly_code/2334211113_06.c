...
void kernel_2334211113_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = i_0; i_2 < -i_0+PB_M+1; i_2++) {
            A[i_2+i_0-1][i_2-1] = D[i_2+i_0-1][i_2+1] - 5;
        }
        for (int i_3 = i_0; i_3 < PB_M-1; i_3++) {
            C[i_0][0] = D[i_0][0] + C[i_3+1][0] - 6;
        }
    }
#pragma endscop
...
kernel_2334211113_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
