...
void kernel_3236152215_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = C[i_0-2] - C[i_0] + D[i_0][1] * C[i_0-2] + C[i_0-2] * A[i_0-1] - B[i_0+1] * B[i_0] + 5;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        B[i_1] = C[i_1-1] * C[i_1] + C[i_1] * C[i_1] - C[i_1] - 1;
    }
#pragma endscop
...
kernel_3236152215_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
