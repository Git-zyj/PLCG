...
void kernel_3224151221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-4; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_L-4, i_1); i_2++) {
                A[i_1+1][i_2+i_1+1] = A[i_1+1][i_2+i_1+2] + A[i_1+1][i_2+i_1+3] + A[i_2+3][i_2+i_1+4] * A[i_1+2][i_2+i_1+2] + 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        B[i_3-1] = C[i_3] + C[i_3] * C[i_3+1] + C[i_3] - D[i_3][i_3] * C[i_3] * C[i_3] - C[i_3] + C[i_3] + 4;
    }
#pragma endscop
...
kernel_3224151221_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
