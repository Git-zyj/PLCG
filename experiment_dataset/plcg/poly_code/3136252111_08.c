...
void kernel_3136252111_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(-i_0+PB_Q, PB_P-2); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_P-2, -i_1+PB_L+1); i_2++) {
                A[i_0][i_2][i_1+2] = B[i_2-1][i_2+i_1-1] - C[i_1][i_0][i_2] * D[i_2][i_0+1][i_1] * D[i_2][i_0][i_1+i_0] + D[i_2][i_0][i_1] - A[i_0][i_1-1][i_2+2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3136252111_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
