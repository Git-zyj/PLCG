...
void kernel_3426311323_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q+2, i_0); i_1++) {
            A[i_0][i_1+1] = E[i_1] + A[i_0-2][i_1+1] - D[i_1-2][i_1+i_0-2] * 2;
        }
        for (int i_2 = i_0; i_2 < -i_0+PB_Q; i_2++) {
            D[i_2-1][i_2+i_0-1] = D[i_2][i_2+i_0] * D[i_0][i_2+i_0] + 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_Q; i_3++) {
        C[i_3] = F[i_3-1] + F[i_3] - C[i_3-2] * C[i_3+1] - 1;
        for (int i_4 = i_3; i_4 < -i_3+PB_Q; i_4++) {
            D[i_4][i_4+i_3] = G[i_4+i_3][i_4][0] * A[i_4][i_3+1] - C[i_4] - C[i_4-2] + 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3426311323_03(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
