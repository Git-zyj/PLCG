...
void kernel_4234112221_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[i_0][i_2][i_1-1] = C[i_1][i_2][i_0] - A[i_2-2][i_0][i_1-2] - 1;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
                for (int i_6 = 1; i_6 < PB_Q+1; i_6++) {
                    A[i_5][i_4][i_6-1] = A[i_5+2][i_4][i_6-1] + 2;
                }
            }
        }
    }
...
kernel_4234112221_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
