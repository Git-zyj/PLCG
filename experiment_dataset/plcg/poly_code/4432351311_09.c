...
void kernel_4432351311_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_1+1][i_1] = E[i_1][i_0] + E[i_1+1][0] + F[1] - E[i_0][i_1-1] + A[i_1][i_1-1] + A[i_1+2][i_1+1] + 1;
        }
    }
        ...
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
                for (int i_5 = 0; i_5 < min(PB_L, -i_4+PB_Q); i_5++) {
                    B[i_4][i_2][i_5][i_5+1] = G[i_3][i_2][i_5+i_4][0] + H[i_3][i_5][i_4][i_2] + H[i_3+1][i_2][i_5][i_4-1] * I[i_2][i_4+1][i_3-1][i_5] + B[i_4-1][i_2][i_5][i_5+1] * B[i_4+1][i_2+1][i_5+1][i_5+2] + B[i_4][i_2+1][i_5+1][i_5+2] * 2;
                }
            }
        }
        for (int i_6 = 1; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < PB_M-2; i_7++) {
                C[i_7+1][0] = J[i_2+1][i_6] * J[i_7][i_6] - K[i_6][i_6-1][i_7] - A[i_7+1][i_7] * B[i_6][i_2+1][i_7][i_7+1] - C[i_7+2][0] - C[i_7][0] * C[i_7][0] + 4;
            }
        }
        D[i_2] = F[i_2] - F[i_2] - L[i_2] - L[i_2] + M[i_2] - F[i_2] + 2;
    }
#pragma endscop
