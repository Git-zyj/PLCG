...
void kernel_3332312321_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
            A[i_0-2][i_1] = A[i_0-2][i_1+2] + 1;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            B[i_2][i_3-1] = A[i_2-2][i_3] - C[i_2][i_3] - 6;
            C[i_2][i_3] = D[i_2][i_2] - B[i_3-2][i_2-3] * B[i_2][i_3-1] - B[i_2-2][i_3-2] - B[i_2][i_3-3] + B[i_3][i_2-1] + B[i_2][i_3+1] - 3;
        }
    }
#pragma endscop
...
kernel_3332312321_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
