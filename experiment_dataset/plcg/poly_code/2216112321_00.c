...
void kernel_2216112321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L-3; i_1++) {
        for (int i_2 = max(4, i_1); i_2 < PB_L-4; i_2++) {
            B[i_2-2][i_1+2] = C[i_2][i_1] + B[i_2][i_1+2] * B[i_1-2][i_2+4] - B[i_2-4][i_1+3] - B[i_2-3][i_1+2] - 1;
        }
    }
#pragma endscop
...
kernel_2216112321_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
