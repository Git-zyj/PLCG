...
void kernel_4412252323_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
        ...
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_1+PB_M, i_0); i_3++) {
                    A[i_0][i_1][i_2][i_0] = E[i_3+1][i_1+2][i_0][i_2] + F[i_3+i_1][i_0][i_3+i_2][i_0][i_2] * G[i_2+i_1][i_3][i_0-1][i_2] * E[i_2][i_1+2][i_3+i_0-2][i_3] * E[i_1][i_0+1][i_3][i_2] + A[i_0+1][i_1+1][i_2-1][i_0+1] * A[i_0+2][i_1][i_2-2][i_0+2] + 6;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 2; i_5 < PB_M-4; i_5++) {
                B[i_4+2][i_4] = H[i_4][i_5] - I[i_4][i_0+2] + J[i_4][i_5][i_0] + B[i_5+4][i_5+2] * B[i_5][i_5-2] * B[i_5+1][i_5-1] + 2;
            }
        }
        C[i_0] = K[i_0] + K[i_0] - K[i_0] * L[i_0+2] - K[i_0] + I[i_0][0] + L[i_0] - C[i_0-2] * C[i_0+1] * 2;
    }
    for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
        D[i_6] = H[i_6][i_6+2] + K[i_6] * L[i_6] + K[i_6] * C[i_6] - C[i_6+2] * 3;
    }
#pragma endscop
