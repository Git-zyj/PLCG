...
void kernel_2216212225_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 5; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M+2; i_1++) {
            A[i_1+i_0-2][i_1][i_0-2] = A[i_1+i_0-5][i_1-2][i_0-3] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-2; i_2++) {
        B[i_2] = C[i_2] * B[i_2+2] - B[i_2+1] - B[i_2-1] + 6;
    }
#pragma endscop
...
kernel_2216212225_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
