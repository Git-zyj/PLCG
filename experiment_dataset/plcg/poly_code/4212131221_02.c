...
void kernel_4212131221_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_P-1 - 1; i_0 >= 1; i_0--) {
        for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, -i_0+PB_P-1); i_2++) {
                for (int i_3 = 2; i_3 < min(min(-i_0+PB_P, i_0), -i_1+PB_Q-1); i_3++) {
                    A[i_1][i_0-1][i_3+i_2] = C[i_2][i_1][i_3][1] * C[i_3][i_2][i_3+i_1+1][i_0] - D[i_2+i_1][i_3][i_3+i_0] + C[i_0][i_3+1][i_3][i_2] * A[i_1][i_3-2][i_2+i_0+1] - 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_P-1; i_5++) {
            for (int i_6 = 2; i_6 < -i_4+PB_P; i_6++) {
                B[0][i_5][i_6+i_4] = E[i_6+i_5][1] * B[0][i_6-2][i_5+i_4+1] + B[0][i_5-1][i_6+i_4-2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4212131221_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
