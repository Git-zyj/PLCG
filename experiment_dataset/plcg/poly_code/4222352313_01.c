...
void kernel_4222352313_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_P+1, PB_M-2); i_2++) {
                for (int i_3 = 2; i_3 < min(-i_1+PB_L-1, -i_0+PB_M+1); i_3++) {
                    A[i_1][i_2][i_3] = C[i_2+1][i_3-2][i_1-1] + D[i_0][i_3][i_1] * E[i_0][i_2][i_1+2][i_2] * F[i_3+i_0-1][i_2-1][i_2+i_1-1][i_3][i_1] + G[i_2+2][i_3+i_1+1][i_1][i_0-1] * D[i_2][i_1-2][i_1] * A[i_3][i_1][i_2-1] * A[i_1][i_3+1][i_2] + A[i_1+1][i_2][i_3] - 5;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_P-3; i_4++) {
            for (int i_5 = 2; i_5 < min(min(PB_M, -i_4+PB_N+1), -i_0+PB_N); i_5++) {
                B[i_5-1][i_4+2][i_5-1] = C[i_4+1][i_5][i_0] - H[i_5+i_0][i_4][i_0] + H[i_5+i_4-1][i_0][0] * D[i_5][i_0+2][i_4-1] - B[i_5-2][i_4+1][i_5-2] - B[i_5-1][i_4+3][i_5-1] + B[i_0][i_4+1][i_0] - 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4222352313_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
