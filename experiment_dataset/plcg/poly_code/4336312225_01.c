...
void kernel_4336312225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 4; i_1 < PB_L-1; i_1++) {
            A[i_1-2][i_1] = D[i_1] * A[i_1-3][i_1-1] * A[i_1-4][i_1-2] * A[i_1-1][i_1+1] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        A[i_2-2][i_2] = 2;
        for (int i_3 = 1; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            C[i_3+i_2][i_2] = D[i_3-1] * E[i_3][i_2] + C[i_3+i_2-1][i_2] + 5;
        }
    }
#pragma endscop
...
kernel_4336312225_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
