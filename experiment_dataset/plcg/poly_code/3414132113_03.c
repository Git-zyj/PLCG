...
void kernel_3414132113_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_P; i_2++) {
                A[i_2][i_1][i_0+2] = E[i_0-1][i_1+i_0][0][i_2+i_1+1] - F[i_1][i_2+i_1][0] * F[i_1][i_2+i_0][i_1+2] + 5;
            }
            B[i_0-2][i_1][i_0] = G[i_1][i_0] + G[i_0][i_1] * G[i_1][i_0] * B[i_0-2][i_1+1][i_0] - 3;
        }
    }
    for (int i_3 = 3; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = i_3; i_5 < PB_N-1; i_5++) {
                C[i_3+1][i_5-2][i_4] = F[i_4][i_3][i_5+1] - C[i_5+1][i_3-3][i_4] * 5;
            }
            for (int i_6 = i_3; i_6 < PB_L; i_6++) {
                D[i_3][i_6-2][i_6] = F[i_4][i_3][i_6] - G[i_4+2][i_3+1] + G[i_6][i_4] + F[i_3][i_6][i_4] + A[i_4][i_6][i_3+2] * D[i_3+1][i_6-2][i_6] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3414132113_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
