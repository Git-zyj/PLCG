...
void kernel_2424252223_01(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        D[i_0-2] = E[i_0] * F[i_0+2][1] - 2;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_L; i_2++) {
            B[i_1][i_2] = G[i_2+1][i_1] - G[i_1][i_2+i_1] * G[i_2-1][i_1] - H[i_2][i_1-1][0] - F[i_2-2][i_1] * F[i_2-1][i_1] - F[i_2][i_1] * B[i_1][i_2-2] + B[i_1-2][i_2] * 2;
        }
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            D[i_3-2] = G[i_3][i_1] + I[i_1] * J[i_3-1][i_3+1][i_1] * J[i_1-1][i_3][i_1] - G[i_1-1][i_3] + 2;
        }
        D[i_1] = E[i_1-1] + I[i_1] * E[i_1-1] * E[i_1] * I[i_1] + D[i_1+2] + 4;
    }
#pragma endscop
...
kernel_2424252223_01(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
