...
void kernel_2426252115_06(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        C[i_0][i_0] = E[i_0] - E[i_0-2] + E[i_0] + E[i_0] * 2;
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            C[i_0+1][i_0+1] = E[i_1] - F[i_1][i_0] * G[i_1+1][i_0] - C[i_1+1][i_1+1] + 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        C[i_2][i_2] = H[i_2] - H[i_2+2] - E[i_2-2] * E[i_2-1] + F[i_2][0] + E[i_2] - 6;
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_L-1, i_3); i_4++) {
            C[i_4+1][i_4+1] = F[i_4+1][i_3] * G[i_4][i_3] + H[i_4] * I[i_4][i_3-2][0] + G[i_3][i_4] * 2;
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2426252115_06(xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
