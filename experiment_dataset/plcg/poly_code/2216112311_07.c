...
void kernel_2216112311_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            A[i_0+2][i_1][i_1] = C[i_1] + A[i_1+2][i_0-1][i_0-1] - A[i_0+2][i_1+1][i_1+1] + A[i_1+3][i_0+1][i_0+1] * 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        B[i_2+1] = C[i_2-2] * B[i_2+2] - B[i_2] * B[i_2+2] + 4;
    }
#pragma endscop
...
kernel_2216112311_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
