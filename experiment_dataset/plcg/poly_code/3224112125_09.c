...
void kernel_3224112125_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-4; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_1][i_0] = C[i_0+1][i_1] + 1;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_L, PB_P-1); i_2++) {
            B[i_0+2][i_2] = C[i_2+i_0][i_2] + B[i_0+2][i_2+1] - B[i_0+4][i_2] + 3;
        }
    }
#pragma endscop
...
kernel_3224112125_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
