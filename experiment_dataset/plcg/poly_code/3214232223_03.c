...
void kernel_3214232223_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_M, -i_0+PB_L+1); i_2++) {
                A[i_0][i_1][i_2] = C[i_1][i_2][i_0] + D[i_0][i_1][i_2+i_0-1] * C[i_2][i_1+1][i_0] * A[i_0][i_1][i_2+2] * A[i_0][i_1+1][i_2] - 2;
            }
        }
        for (int i_3 = i_0; i_3 < PB_M; i_3++) {
            for (int i_4 = 2; i_4 < min(-i_3+PB_N, PB_P); i_4++) {
                A[i_0][i_3][i_4] = C[i_4-2][i_3][i_0] + C[i_4+i_3][i_0+1][i_4] + C[i_4][i_0+1][i_3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3214232223_03(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
