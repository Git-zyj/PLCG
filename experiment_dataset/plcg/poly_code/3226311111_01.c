...
void kernel_3226311111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_P-1); i_1++) {
            for (int i_2 = 2; i_2 < i_1; i_2++) {
                A[i_1-1][i_1] = A[i_2-2][i_2-1] + B[i_1][i_0] - 2;
            }
            B[i_0][i_1+1] = C[i_1][i_0+1] - 2;
        }
    }
#pragma endscop
...
kernel_3226311111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
