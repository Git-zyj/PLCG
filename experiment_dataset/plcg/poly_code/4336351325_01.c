...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4336351325_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
        ...
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = 2; i_3 < -i_1+PB_P; i_3++) {
                    A[i_3-1][i_0+1][i_1][i_2-1] = D[i_3-1][i_3][i_0+1][i_1] * D[i_1][i_0][i_3+1][i_2] + E[i_2][i_3][i_3+i_1][i_0] - E[i_3+i_2][i_1-1][i_3-1][i_0] + E[i_3][i_2+i_0][i_1][i_3] * A[i_0+1][i_1+1][i_3][i_2-1] * A[i_3+1][i_1+1][i_2][i_0-3] * A[i_2-1][i_0+1][i_1][i_3-2] * A[i_3-2][i_1+1][i_2][i_0-2] * A[i_3-1][i_0+1][i_1+1][i_2] + 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        B[i_4] = F[i_4] + G[i_4+1] * G[i_4] + H[i_4] + B[i_4-2] * 2;
    }
    for (int i_5 = 0; i_5 < PB_L; i_5++) {
        B[i_5] = F[i_5] * F[i_5] - G[i_5] * I[i_5][i_5] * 6;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4336351325_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
