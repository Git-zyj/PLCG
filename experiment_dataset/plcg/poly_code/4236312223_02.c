...
void kernel_4236312223_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
        ...
        for (int i_1 = i_0 - 1; i_1 >= 0; i_1--) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = i_1; i_3 < min(-i_1+PB_L-2, -i_2+PB_L-1); i_3++) {
                    B[i_2][i_3+i_2][i_3] = B[i_1][i_3+i_1+2][i_3+2] * B[i_3][i_3+i_2+1][i_2+1] + 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < -i_4+PB_L; i_5++) {
            for (int i_6 = i_5; i_6 < PB_P; i_6++) {
                B[i_4][i_5+i_4][i_5] = C[i_4][i_5][i_6] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4236312223_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
