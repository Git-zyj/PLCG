...
void kernel_3334212321_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_N, -i_2+PB_P-4); i_3++) {
            for (int i_4 = 3; i_4 < -i_2+PB_P-3; i_4++) {
                B[i_4-1][i_3][i_3+i_2+2] = E[i_3][i_2][i_2] * B[i_4-3][i_2][i_3+i_2+1] * B[i_3-1][i_4+1][i_4+i_2+3] * B[i_4-3][i_3][i_3+i_2+4] + 3;
            }
        }
        for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
            for (int i_6 = i_5; i_6 < PB_L; i_6++) {
                C[i_6][i_5-1] = F[i_6][i_2][i_6] - C[i_6][i_2-2] - C[i_5-2][i_6] + C[i_6][i_5-2] * C[i_6-2][i_5+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3334212321_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
