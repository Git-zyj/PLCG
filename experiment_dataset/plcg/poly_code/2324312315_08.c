...
void kernel_2324312315_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1] = D[0] + D[i_1] * B[i_1+1] * B[i_1-1] - B[i_1-1] + 5;
        for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
            B[i_2+1] = A[i_2+2] * A[i_2+1] + B[i_1] - 1;
        }
    }
#pragma endscop
...
kernel_2324312315_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
