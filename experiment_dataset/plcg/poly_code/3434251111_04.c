...
void kernel_3434251111_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-1] - F[i_0] * F[i_0] * F[i_0+1] + E[i_0] - A[i_0+1] * A[i_0-1] - 2;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_0-1][i_1-1] = F[i_1] - G[i_1][i_0-1] + H[i_0][i_1] * F[i_1] - H[i_1][i_0] * 2;
        }
        A[i_0] = F[i_0-1] * F[i_0] + H[i_0+1][0] + F[i_0] * F[i_0] + 2;
        for (int i_2 = i_0; i_2 < -i_0+PB_M-1; i_2++) {
            for (int i_3 = 1; i_3 < PB_P; i_3++) {
                D[i_2+i_0+1][i_0][i_3] = I[i_2][i_3-1][i_0-1] + I[i_2+1][i_3][i_0-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3434251111_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
