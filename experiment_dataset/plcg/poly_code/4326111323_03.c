...
void kernel_4326111323_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            B[i_1+1] = D[i_0][i_1] - D[i_1+i_0][0] + B[i_0-1] + B[i_0] + 4;
        }
        B[i_0-1] = B[i_0] + 1;
    }
    for (int i_2 = 2; i_2 < PB_M+1; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < -i_2+PB_M+1; i_3++) {
            for (int i_4 = max(3, i_2); i_4 < min(-i_3+PB_P, -i_2+PB_M+1); i_4++) {
                C[i_4+i_2-1][i_3-1][i_4+i_3] = C[i_3+i_2-1][i_4-3][i_4+i_3-2] * C[i_4+i_2-2][i_3-3][i_4+i_3-3] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4326111323_03(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
