...
void kernel_2434251123_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0-1] = E[i_0-1] + F[i_0+1] + F[i_0] + E[i_0] + F[i_0] - E[i_0] * F[i_0] + F[i_0] - A[i_0] + 6;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            B[i_1][i_2-1] = F[i_2] + G[i_2][i_1] + H[i_1][i_2] - B[i_1-1][i_2-2] + B[i_1][i_2] + 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            D[i_3+1][0] = G[i_3+1][i_4] + G[i_4][i_3] * G[i_3][i_4] + I[i_4][i_3][i_4] - 3;
        }
        D[i_3+1][0] = F[i_3] - E[i_3+1] * F[i_3] - F[i_3] + F[i_3] * 4;
    }
#pragma endscop
...
kernel_2434251123_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
