...
void kernel_4336111213_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[1][i_1][i_2-1] = A[1][i_0-1][i_2-1] + A[1][i_0][i_2-2] * A[1][i_1-1][i_2-1] + A[1][i_2][i_0] + 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M; i_4++) {
            B[i_3][i_4] = D[i_3+1][0] * D[i_4][i_3] * 2;
        }
        for (int i_5 = 0; i_5 < i_3; i_5++) {
            for (int i_6 = 2; i_6 < PB_L+1; i_6++) {
                for (int i_7 = 0; i_7 < PB_M; i_7++) {
                    C[i_3][i_7][i_6-1][i_3] = C[i_3+1][i_7][i_6-2][i_3+1] + 1;
                }
            }
        }
    }
...
kernel_4336111213_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
