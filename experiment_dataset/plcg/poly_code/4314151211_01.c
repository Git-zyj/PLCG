...
void kernel_4314151211_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_Q, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_Q; i_2++) {
                for (int i_3 = min(min(-i_0+PB_Q, PB_L-1), -i_2+PB_P) - 1; i_3 >= 1; i_3--) {
                    A[i_0][i_2][i_3][i_1] = D[i_0][i_2][i_1][i_3] + D[i_1][i_3+i_2][i_0][i_2] - D[i_2][i_1][i_3-1][i_1+i_0] + D[i_3+1][i_3][i_0][i_1-1] * D[i_0-1][i_1-1][i_2+i_1][i_3+1] + D[i_1][i_0][i_3+i_0][i_2] + A[i_0+1][i_2-1][i_3][i_1] * A[i_1+1][i_0-1][i_3][i_2] + 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        B[i_4+1] = E[i_4] * E[i_4+1] - E[i_4-1] * E[i_4-1] + E[i_4] + B[i_4] + B[i_4+2] + 4;
        for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_L; i_6++) {
                for (int i_7 = 1; i_7 < min(i_5, -i_4+PB_Q+1); i_7++) {
                    C[i_7][i_5][i_6][i_4] = D[i_7-1][i_5][i_4][i_5-1] + D[i_5+1][i_6+1][i_7+i_4-1][i_5] - D[i_6][i_7][i_4][i_5] + C[i_7][i_5][i_6-1][i_4+1] - 2;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), INIT_SEED);
kernel_4314151211_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E)));
