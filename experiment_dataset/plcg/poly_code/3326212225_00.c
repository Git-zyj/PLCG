...
void kernel_3326212225_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            ...
            for (int i_2 = 3; i_2 < min(-i_0+PB_M, PB_P-2); i_2++) {
                C[i_2-2][i_2-3] = D[i_1][i_2+i_0] + A[i_1-2][i_2] * A[i_1][i_2+2] * 3;
            }
        }
        for (int i_3 = 3; i_3 < i_0; i_3++) {
            C[i_3][i_3-1] = C[i_3-2][i_3-3] - 2;
        }
    }
#pragma endscop
...
kernel_3326212225_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
