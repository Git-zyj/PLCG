...
void kernel_4414231313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0+1] * E[i_0] - E[i_0+1] * F[i_0] * A[i_0-1] + A[i_0+1] + 6;
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < min(PB_N-1, -i_1+PB_L); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_N-1, -i_0+PB_M+1); i_3++) {
                    B[i_0][i_1+1][i_2-1][i_2] = G[i_2+1][i_0][i_2][i_1][i_3] + H[i_3+i_2+1][i_3+i_0-1][i_2+i_1][0] - B[i_2][i_1][i_0][i_0+1] * B[i_0][i_1+2][i_2-2][i_2-1] - 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
        A[i_4] = A[i_4] + A[i_4-1] * 3;
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_P+1; i_6++) {
                for (int i_7 = 1; i_7 < -i_4+PB_M; i_7++) {
                    B[i_4][i_5+1][i_6][i_6+1] = H[i_6-1][i_5][i_4][i_7] * H[i_5-1][i_7+i_4][i_6][0] + I[i_7][i_5][i_4][i_6] * I[i_6][i_5][i_4+1][i_7-1] * H[i_4+1][i_7][1][i_6+i_5-1] - 1;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4414231313_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
