...
void kernel_4436231313_04(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_M, PB_Q), -i_1+PB_N); i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    D[i_0][i_3][i_1-1][i_2] = E[i_1-1][i_3][i_0][i_2] + F[i_2+i_1][i_0][i_3][i_2+i_0] + 4;
                }
            }
        }
        for (int i_4 = 1; i_4 < i_0; i_4++) {
            for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
                for (int i_6 = max(i_4, 1); i_6 < -i_5+PB_Q; i_6++) {
                    D[i_6+1][i_4-1][i_0][i_5] = G[i_4][i_6][i_0][i_0+1][0] + E[i_4+1][i_4][i_6-1][i_0] + E[i_4-1][i_5+1][i_4][0] + E[i_4][i_0][i_6+i_5][i_0] + D[i_0+1][i_6][i_5][i_4] + 4;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_P; i_7++) {
        for (int i_8 = i_7; i_8 < PB_L-1; i_8++) {
            for (int i_9 = 0; i_9 < PB_L; i_9++) {
                C[i_7][i_8][0] = H[i_9][i_8] * I[i_7-1][i_9][i_8] - C[i_7+1][i_8][0] - C[i_7-1][i_8][0] + C[i_7][i_8+1][0] - 4;
            }
            for (int j_0 = 1; j_0 < -i_8+PB_L; j_0++) {
                for (int j_1 = 1; j_1 < min(-i_8+PB_M, PB_P-1); j_1++) {
                    D[j_1+1][i_8][i_7][j_0] = F[j_1][i_7+1][j_0][i_8] - E[j_0][j_1+1][j_0-1][i_8] - I[j_1+i_8][j_0+i_8][i_7] * D[i_8+1][j_1-1][i_7][j_0] + 6;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4436231313_04(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
