...
void kernel_4436352111_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        A[i_0-1] = E[i_0] * E[i_0] + F[i_0][i_0] * E[i_0+1] - G[i_0+2][i_0+1] - H[i_0-1] * I[i_0] * H[i_0] - A[i_0] + 1;
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_L+2, PB_P); i_2++) {
                B[i_1-2][i_0][i_2+i_0-2] = J[i_1+2][i_0][0] - G[i_2-2][0] + K[i_1][i_0][i_1] + 5;
            }
        }
        for (int i_3 = 1; i_3 < -i_0+PB_L+1; i_3++) {
            for (int i_4 = 2; i_4 < -i_0+PB_P+1; i_4++) {
                C[i_3+1][i_3] = L[i_4][i_0-1][i_3] - K[i_3+i_0-1][i_4-2][i_3] - J[i_3][i_0][i_4+i_0-1] * G[i_4][i_3-1] + L[i_4][i_0][i_3] + 4;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_P; i_5++) {
        for (int i_6 = 1; i_6 < min(-i_5+PB_M, i_5); i_6++) {
            D[i_5+1][i_6] = H[i_6] + G[i_6+i_5][i_5] * J[i_6][i_5][0] + G[i_6][i_5-2] * D[i_5+1][i_6-1] * 1;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4436352111_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
