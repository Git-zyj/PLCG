...
void kernel_4432332125_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            B[i_1-2][i_1+i_0-1] = E[i_1+i_0][i_1+2] - F[i_1+i_0-1][i_0] * 1;
        }
        for (int i_2 = max(2, i_0); i_2 < -i_0+PB_M; i_2++) {
            for (int i_3 = 2; i_3 < min(min(PB_M, -i_0+PB_L), -i_2+PB_L); i_3++) {
                B[i_2][i_3+i_2] = G[i_2+i_0][i_3][i_2] + G[i_2][i_3+i_0][1] + H[i_3][i_2-2][i_0][i_2] * I[i_0+2][i_3-2][i_2-1] + 6;
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 1; i_6 < PB_P-1; i_6++) {
                for (int i_7 = 2; i_7 < -i_5+PB_L-1; i_7++) {
                    C[i_7][i_5-2][i_6+1] = J[i_4][i_7+2][i_5+2][i_4+1] + J[i_6-1][i_7+i_5+1][i_4][i_6] + K[i_7][i_6][i_5][i_5][i_4] + C[i_5-2][i_7-2][i_6+1] + 6;
                }
            }
        }
        for (int i_8 = max(i_4, 1); i_8 < min(PB_M, -i_4+PB_L); i_8++) {
            B[i_4][i_8+i_4] = L[i_8][i_4+1] - L[i_8-1][i_4] + E[i_8][i_4] + 2;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4432332125_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L)));
