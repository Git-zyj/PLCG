...
void kernel_2416212125_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            A[i_1] = E[i_1][i_1+i_0] * A[i_1-1] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        B[i_2-1] = E[i_2-1][i_2-2] * 4;
        C[i_2-2] = F[i_2] - B[i_2-2] + 4;
        D[i_2][i_2] = 6;
    }
#pragma endscop
...
kernel_2416212125_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
