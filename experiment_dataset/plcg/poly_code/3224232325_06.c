...
void kernel_3224232325_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-4; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_Q-2, -i_0+PB_L-1); i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                A[i_2][i_1][i_0+2] = C[i_2][i_1+i_0+1][2] + D[i_2+1][i_1+i_0][i_0] - C[i_2+1][i_1][i_0] + C[i_2-1][i_0][i_1+2] * C[i_2][i_0+1][i_2+i_1] - A[i_0][i_1-1][i_2+4] + A[i_2+2][i_1-2][i_0] + A[i_2][i_1][i_0+4] + 1;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L; i_3++) {
            for (int i_4 = i_0; i_4 < PB_M; i_4++) {
                A[i_4][i_3][i_0+2] = D[i_3][i_0][i_3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3224232325_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
