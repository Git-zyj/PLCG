...
void kernel_3234111223_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            B[i_0][i_1-2][i_1-1] = C[i_1][i_0] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = i_2; i_4 < PB_N+1; i_4++) {
                B[i_2][i_4-1][i_4] = B[i_2-1][i_4-1][i_4] + B[i_2+2][i_4-1][i_4] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3234111223_09(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
