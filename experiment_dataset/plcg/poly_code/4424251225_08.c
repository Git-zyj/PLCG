...
void kernel_4424251225_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 3; i_3 < PB_P-1; i_3++) {
                    A[i_2][i_3][i_3-1] = E[i_2][i_1+1][i_3+1][0] * A[i_1-2][i_3][i_3-1] * A[i_2-1][i_3][i_3-1] - A[i_2][i_3-2][i_3-3] * 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-3; i_4++) {
        for (int i_5 = 1; i_5 < PB_L-3; i_5++) {
            for (int i_6 = 1; i_6 < min(min(-i_4+PB_M, -i_5+PB_L-3), i_5); i_6++) {
                for (int i_7 = 1; i_7 < min(min(-i_4+PB_M, -i_5+PB_L-1), PB_M-1); i_7++) {
                    B[i_6+i_4][i_5+1][i_7+i_5+1][i_7] = E[i_5-1][i_7+1][i_6+i_4-1][i_7] * F[i_7+1][i_6][i_5-1] + G[i_6][i_6][i_5] * H[i_7-1][i_4][i_5][i_6] + H[i_6+1][i_4][i_5][i_6-1] - H[i_7+i_6-1][i_5+1][i_4-1][i_7] + B[i_7+i_4][i_6+3][i_6+i_5+3][i_5] - 3;
                }
            }
        }
        C[i_4+1] = I[0] * J[i_4] + J[i_4] * I[i_4-1] + I[i_4+1] - I[i_4+1] * J[i_4] - C[i_4+3] * 5;
        for (int i_8 = 0; i_8 < PB_M; i_8++) {
            for (int i_9 = 0; i_9 < PB_M-2; i_9++) {
                A[i_9+2][i_4+1][i_4] = K[i_9][i_8+1] * L[i_8][i_9] - G[i_9+1][1][i_8] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4424251225_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
