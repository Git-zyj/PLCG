...
void kernel_2222151113_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        B[i_0][0] = C[i_0] + C[0] * C[i_0+1] + D[i_0-1][i_0] - C[i_0-1] * B[i_0][0] + 1;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_1][0] = D[i_1+1][i_0] + D[i_1][i_0-1] * C[0] * D[i_0][0] + D[0][i_1+1] + 4;
        }
    }
#pragma endscop
...
kernel_2222151113_02(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
