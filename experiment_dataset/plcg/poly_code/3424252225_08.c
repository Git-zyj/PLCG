...
void kernel_3424252225_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_1][i_0+1] = E[i_1+1][i_0] - F[i_1-1][i_0-1] * F[i_1][i_0] - A[i_1][i_0] + 6;
            for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
                B[i_0][i_1][i_2-1] = G[i_2+1][i_1-1][i_0] * H[i_1-1][i_2][i_0-1] + F[i_2][i_1] * H[i_0-2][i_2][i_1] * H[i_2+i_0][i_1][i_0+1] * G[i_2][i_2][i_1-2] - A[i_2][i_1+2] - B[i_1][i_0+2][i_2] - B[i_0+2][i_1][i_2-1] - 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-3; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M, i_3); i_4++) {
            A[i_4+2][i_3+3] = F[i_3][i_4] - I[i_4+i_3] + I[i_4+i_3] + E[i_3][i_4-2] * 5;
        }
    }
    for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
        for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
            A[i_6-2][i_5+1] = F[i_6][i_6-1] + J[i_6] * E[i_6][i_5] + F[i_6+2][i_5+1] - F[i_5][i_6-1] * 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3424252225_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
