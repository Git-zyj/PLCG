...
void kernel_4226311313_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_0][i_1] = A[i_0][i_1+1] - A[i_0][i_1-1] * B[1] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        B[1] = C[i_2+1][1] * B[1] + B[1] - B[1] - 1;
    }
#pragma endscop
...
kernel_4226311313_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
