...
void kernel_2214231315_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][i_0] = C[i_1+1][i_0+1] * D[i_1] + C[i_1][i_0+1] * A[i_0-1][i_0-1] * 6;
        }
        for (int i_2 = i_0; i_2 < PB_M; i_2++) {
            A[i_0+1][i_0+1] = E[i_2-1][i_0] - C[i_2][i_0] * A[i_0][i_0] + A[i_2][i_2] * 2;
        }
    }
#pragma endscop
...
kernel_2214231315_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
