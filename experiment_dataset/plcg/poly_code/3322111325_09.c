...
void kernel_3322111325_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L-3, i_0); i_2++) {
                A[i_0][i_2+1][i_2] = A[i_0-1][i_2+3][i_2+2] * A[i_0+2][i_2+1][i_2] - A[i_0][i_2+3][i_2+2] + 5;
            }
        }
    ...
    for (int i_3 = 3; i_3 < PB_Q; i_3++) {
        for (int i_4 = 1; i_4 < PB_Q+1; i_4++) {
            for (int i_5 = 2; i_5 < PB_L+1; i_5++) {
                C[i_5-1][i_4+1][i_4-1] = C[i_5-1][i_3+2][i_3] - C[i_5-1][i_3-1][i_3-3] * C[i_5-2][i_3+2][i_3] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3322111325_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
