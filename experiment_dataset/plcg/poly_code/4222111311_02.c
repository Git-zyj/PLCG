...
void kernel_4222111311_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0][i_1-1] = C[i_1][i_0][i_0+1] - D[i_0-1][i_1] + A[i_0][i_1] * A[i_0+1][i_1] * 4;
        }
    }
        ...
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            for (int i_4 = i_3; i_4 < PB_L-1; i_4++) {
                for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                    B[i_4][i_5+1][i_3+1][i_2] = B[i_4][i_5+1][i_3+2][i_2] * B[i_4][i_5][i_3][i_2] * B[i_5-1][i_3+1][i_4+1][i_2] - B[i_4+1][i_5+1][i_3+1][i_2-1] - 1;
                }
            }
        }
    }
...
kernel_4222111311_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
