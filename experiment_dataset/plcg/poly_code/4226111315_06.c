...
void kernel_4226111315_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_N+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
                B[i_2-1][i_2] = C[i_2+1][i_1+i_0-1][i_2] + 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 2; i_4 < i_3; i_4++) {
            B[i_4-1][i_4] = B[i_4-2][i_4-1] * B[i_4][i_4+1] * B[i_3][i_3+1] - 3;
        }
    }
#pragma endscop
...
kernel_4226111315_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
