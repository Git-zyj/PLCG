...
void kernel_4424332225_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0-2] + F[i_0] - E[i_0] + 5;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1] = G[i_1-1][i_0] - H[i_0+2][i_1] - A[i_0] + A[i_0-1] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, PB_L-2); i_3++) {
            C[i_3+2] = G[i_2][i_3+1] + E[i_3] - G[i_3][0] + I[i_3][i_2] + C[i_2] * 5;
        }
        for (int i_4 = 0; i_4 < min(i_2, PB_L-1); i_4++) {
            A[i_4+1] = J[i_4] - F[i_4] * G[i_2][i_4] * 5;
        }
    }
#pragma endscop
...
kernel_4424332225_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
