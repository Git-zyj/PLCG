...
void kernel_4226212315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N; i_0++) {
        A[i_0-2] = C[i_0] - A[i_0-3] - A[i_0-1] + A[i_0-1] - 3;
    }
    for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_M+2; i_2++) {
            A[i_2-2] = D[i_2+i_1-2] * 5;
        }
    }
#pragma endscop
...
kernel_4226212315_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
