...
void kernel_4212152123_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_Q, -i_0+PB_P); i_1++) {
            B[i_1][i_0][1] = C[i_1][i_1+i_0] * 2;
        }
        for (int i_2 = max(2, i_0); i_2 < PB_Q; i_2++) {
            for (int i_3 = 2; i_3 < min(min(-i_2+PB_P+1, PB_Q), -i_0+PB_L); i_3++) {
                B[i_3][i_0+1][1] = C[i_3+2][i_0] + C[i_0+2][i_3+i_2-1] - D[i_2+1][i_3][i_0] * D[i_2][i_3+i_0][i_3-1] - D[i_3+i_0][i_2+1][i_2-2] + D[i_3+i_2+2][i_0-1][i_0-1] * C[i_3+i_0][i_2-2] + C[i_3+1][i_0] + B[i_3-2][i_0+1][1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4212152123_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
