...
void kernel_4314231215_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(min(i_0, -i_0+PB_L+1), PB_M-1); i_1++) {
            A[i_1][i_0] = D[i_0] - E[i_1+1] - F[i_1+i_0-1][i_1] + G[i_0][1] - 1;
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                A[i_1][i_0] = G[i_2][i_1+1] - H[i_1][1][i_2-1] + G[i_2+1][i_1-1] * A[i_2+1][i_0] + A[i_1][i_0] + 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                A[i_4+1][i_3] = I[i_4][i_3][i_5] - I[i_4+1][i_5+1][i_3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4314231215_01(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
