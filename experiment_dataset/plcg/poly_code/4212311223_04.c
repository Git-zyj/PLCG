...
void kernel_4212311223_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 3; i_2 < PB_M; i_2++) {
                A[i_2-1][i_1][i_0] = A[i_2-2][i_1][i_0] * A[i_2-3][i_1][i_0] - A[i_1+1][i_2][i_0] + 3;
            }
            B[i_1+1][i_0] = C[i_1] - D[i_0][i_1-1] * B[i_1+2][i_0-2] * 1;
        }
    }
#pragma endscop
...
kernel_4212311223_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
