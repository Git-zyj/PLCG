...
void kernel_4132111215_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_M, PB_Q-1), -i_1+PB_P); i_2++) {
                A[i_1][i_2+i_0-1][i_1-1][i_0] = B[i_2+i_0][i_1-1] - A[i_1+1][i_2+i_0-1][i_1][i_0] + A[i_0][i_2+i_1][i_0-1][i_2+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4132111215_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
