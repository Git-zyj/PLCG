...
void kernel_4212111211_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, PB_P-1); i_2++) {
                A[i_0][i_2+1][i_1] = C[i_2+i_0][i_1+1][i_2-1][0] * A[i_0][i_2+1][i_1+1] * 3;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
                    B[i_3][i_4][i_6][i_5] = C[i_3][i_6][i_5][i_4] + B[i_3+1][i_4][i_6][i_5-1] - B[i_3][i_4+1][i_6+1][i_5] - B[i_5-1][i_4-1][i_6][i_3] * 1;
                }
            }
        }
    }
...
kernel_4212111211_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
