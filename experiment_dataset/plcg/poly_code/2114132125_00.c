...
void kernel_2114132125_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < -i_0+PB_L; i_1++) {
            A[i_1][i_0] = B[i_0][i_0+2][i_1] + C[i_1] - C[i_1+i_0] + A[i_1-1][i_0] - 6;
        }
    }
#pragma endscop
...
kernel_2114132125_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
