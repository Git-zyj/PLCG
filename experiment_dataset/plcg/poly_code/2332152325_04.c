...
void kernel_2332152325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        A[0] = D[i_0] * E[i_0+2][i_0] * D[i_0] * D[i_0+2] - D[i_0] * A[0] * A[0] + A[0] - 6;
    }
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_M, PB_N); i_2++) {
            B[i_2][i_1] = D[i_1-2] - D[i_2] + E[i_1-1][i_2] + E[i_2][i_1] - E[i_2+1][i_1] + E[i_2+i_1][2] + B[i_2-2][i_1+1] * B[i_2][i_1+1] * B[i_2-2][i_1] + 4;
        }
        A[0] = E[i_1][i_1+1] - D[i_1] - D[i_1] - 2;
    }
#pragma endscop
...
kernel_2332152325_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
