...
void kernel_3214212215_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 2; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            ...
            for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                B[i_0-1][i_3][i_0] = C[i_1][i_0][i_1+1] + B[i_0-2][i_3][i_0-1] - B[i_1-2][i_3+1][i_1-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3214212215_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
