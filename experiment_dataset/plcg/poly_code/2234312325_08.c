...
void kernel_2234312325_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            B[i_0+1][i_1+1] = C[i_0-1][i_1] * 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
            B[i_2][i_3] = B[i_2+2][i_3+2] + B[i_2][i_3-2] + B[i_2+1][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_2234312325_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
