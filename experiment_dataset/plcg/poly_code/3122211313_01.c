...
void kernel_3122211313_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_L-2); i_1++) {
            for (int i_2 = i_1; i_2 < min(min(-i_0+PB_M, -i_1+PB_M), PB_L-1); i_2++) {
                A[i_1+i_0][i_2+1] = B[i_2+1][i_1-1][i_0][i_0-1] - A[i_2+i_1][i_0+1] + A[i_2+i_1-1][i_0+1] - A[i_2+i_0][i_1+2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3122211313_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B));
