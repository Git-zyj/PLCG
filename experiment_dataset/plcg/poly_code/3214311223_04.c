...
void kernel_3214311223_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_P-2, PB_L+1); i_4++) {
                B[i_2+1][i_4+i_3][i_3] = C[i_4-1][i_3] + B[i_2][i_4+i_3+2][i_3+2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3214311223_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
