...
void kernel_4236231325_04(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < PB_M; i_2++) {
                B[i_1-1][i_0+1][i_2-2][i_2-2] = C[i_2][i_1] + 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = i_3; i_5 < -i_3+PB_L; i_5++) {
                for (int i_6 = 3; i_6 < min(PB_L-1, -i_5+PB_M+1); i_6++) {
                    B[i_3-1][i_4+1][i_6-1][i_6-1] = D[i_3-1][i_4-1][i_5][i_6-1] - D[i_5][i_6+i_5-1][i_6+1][i_4] + E[i_6][i_5+i_3][i_6-1][i_4] - F[i_4][i_6][i_3] - B[i_3-1][i_5+2][i_6-1][i_6-1] - B[i_3-2][i_4+1][i_6-1][i_6-1] - B[i_3][i_4+1][i_6-3][i_6-3] - 3;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4236231325_04(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
