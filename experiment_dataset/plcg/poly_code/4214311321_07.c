...
void kernel_4214311321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_P-1 - 1; i_0 >= 0; i_0--) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_0][i_1] = A[i_1][i_0+1] * A[i_0][i_1-2] - A[i_0][i_1+1] - 6;
        }
    }
        ...
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 1; i_4 < -i_2+PB_M; i_4++) {
                for (int i_5 = i_2; i_5 < PB_P-2; i_5++) {
                    B[i_3+1][i_5+1][i_4][i_2] = C[i_4+1][i_2-1][i_5][i_3] - C[i_4+i_2][0][i_3][0] + B[i_3+1][i_5+2][i_2][i_4] * B[i_3+1][i_5-1][i_4][i_2] * B[i_5-1][i_2+3][i_4-1][i_3] - 6;
                }
            }
        }
    }
...
kernel_4214311321_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
