...
void kernel_3422231221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0+1] = E[i_0] + F[i_0][i_0-1] - E[i_0] + 5;
        for (int i_1 = 2; i_1 < min(PB_N+1, -i_0+PB_L-1); i_1++) {
            B[i_1][i_1+i_0] = G[i_1][i_0] * F[i_1][i_0+1] * E[i_1-1] + B[i_1-2][i_1+i_0-2] - B[i_0][i_1+i_0+1] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_L, PB_M); i_3++) {
            B[i_2][i_3+i_2] = B[i_3][i_3+i_2] + D[i_2-1] * 1;
        }
        D[i_2] = H[0] + H[i_2] + E[i_2] * 6;
    }
#pragma endscop
...
kernel_3422231221_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
