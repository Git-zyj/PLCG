...
void kernel_4416212313_05(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
...
#pragma scop
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_P; i_3++) {
                    D[i_1+1][i_1][i_2-1] = E[i_1-2][i_2+1][i_1] * 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        ...
        for (int i_7 = 0; i_7 < min(i_4, -i_4+PB_L-1); i_7++) {
            C[i_4][i_7+i_4] = C[i_4-1][i_7+i_4-1] - C[i_4][i_7+i_4-1] * C[i_4+1][i_7+i_4+1] - C[i_4-1][i_7+i_4] + 2;
            for (int i_8 = max(1, i_7); i_8 < PB_P+1; i_8++) {
                D[i_4+1][i_4][i_7] = D[i_4+2][i_4+1][i_7] * D[i_4+1][i_4][i_8-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4416212313_05(xd,yd,zd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E));
