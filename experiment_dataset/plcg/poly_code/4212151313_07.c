...
void kernel_4212151313_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < PB_N; i_2++) {
                A[i_0][i_1+1][i_2] = C[i_0-1][i_2][i_1] + C[i_1][i_0][i_2+1] + A[i_0+1][i_1+1][i_2] + A[i_0-1][i_1+1][i_2] * A[i_0+1][i_1+1][i_2+1] * 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 1; i_4 < min(min(-i_3+PB_N, PB_P-2), i_3); i_4++) {
            for (int i_5 = 1; i_5 < PB_N-1; i_5++) {
                B[i_3+1][i_4][i_4+1] = D[i_5-1][i_4] - C[i_5-1][i_3][i_4+1] + D[i_5][i_4+i_3+1] * D[i_5][i_4] + C[i_3+1][i_5][i_5+1] + C[i_5+1][i_3+1][i_4] + C[i_5-1][i_4+i_3][1] + A[i_5][i_4+1][i_3] + B[i_3+2][i_4+1][i_4+2] - B[i_3+1][i_4-1][i_4] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4212151313_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
