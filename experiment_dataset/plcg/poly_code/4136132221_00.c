...
void kernel_4136132221_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_N; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_N; i_2++) {
                A[i_0+1][i_1][i_2][0] = B[i_1][i_2] + C[i_1+i_0][i_2][i_2] * C[i_2+i_1][i_0-1][i_1-2] * A[i_0+1][i_1-2][i_2+1][0] - A[i_0+1][i_1+1][i_2][0] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4136132221_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
