...
void kernel_2416132321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-3; i_0++) {
        A[i_0][i_0] = E[i_0+1] + E[i_0] * F[i_0][i_0] * E[i_0] + B[i_0][i_0] + B[i_0+2][i_0+2] + 1;
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            B[i_1][i_1] = F[i_0-2][i_1+2] * E[0] * A[i_0][i_0] - 1;
        }
        for (int i_2 = i_0; i_2 < min(PB_M-4, -i_0+PB_P); i_2++) {
            C[i_2+2][i_0+1] = F[i_0+1][i_0] * F[i_0+2][i_2+i_0] * A[i_0][i_0] + C[i_2+4][i_0+1] + C[i_2+2][i_0+3] + 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M, PB_L-1); i_4++) {
            D[i_3-1][i_4-2] = F[i_4+i_3][i_4] * E[i_4] - F[i_4][i_3-1] - B[i_4+1][i_4+1] * B[i_3][i_3] - D[i_4-1][i_3] + 1;
        }
    }
#pragma endscop
...
kernel_2416132321_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
