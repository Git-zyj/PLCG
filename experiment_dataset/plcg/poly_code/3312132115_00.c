...
void kernel_3312132115_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_L-2); i_1++) {
            A[i_0][i_1+2] = D[i_1][i_0] * D[i_0][i_1] * D[i_0][i_0] * 2;
            A[i_0+1][i_1+2] = D[i_1+i_0][i_1] * D[i_1][i_0-1] + 6;
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                C[i_0][i_2][i_1+2] = E[i_2][i_0][i_2] * E[i_2][i_1+1][i_2] - C[i_2-1][i_0][i_1+2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3312132115_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
