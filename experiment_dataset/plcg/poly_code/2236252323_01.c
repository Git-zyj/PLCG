...
void kernel_2236252323_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_1-2][i_0] = C[i_1+2][1] - D[0] + C[i_0+1][i_1] * C[i_1][i_0-2] + A[i_1-2][i_0+1] * A[i_0-3][i_1] + A[i_1-1][i_0] + 4;
        }
    }
    for (int i_2 = 4; i_2 < PB_P-2; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_P, -i_2+PB_M); i_3++) {
            A[i_2-4][i_3] = C[i_2-1][i_3] - C[i_3][i_3] - E[i_3+i_2][0] + C[i_3+2][i_2+2] * E[i_3+i_2][0] * E[i_3][i_2] * 4;
        }
    }
#pragma endscop
...
kernel_2236252323_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
