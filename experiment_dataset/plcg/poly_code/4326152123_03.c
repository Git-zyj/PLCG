...
void kernel_4326152123_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q+2, PB_L); i_1++) {
            for (int i_2 = i_1; i_2 < min(PB_L-4, -i_1+PB_L); i_2++) {
                A[i_1][i_2+2][i_0-2] = D[i_2+i_1][i_2][i_0] + D[i_1][i_2-1][i_0-2] + D[i_1][i_0-1][i_2] * D[i_1-2][i_2][i_1+i_0-2] - A[i_1][i_2+4][i_0-2] + 2;
            }
        }
        B[i_0-1] = E[i_0] * E[i_0] + E[i_0-1] * E[i_0-2] + E[i_0] + 6;
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_P, PB_L-2); i_4++) {
            for (int i_5 = 2; i_5 < i_4; i_5++) {
                C[i_4][i_5-2][i_3] = D[i_3][i_5-2][i_4] - D[i_5][i_4+1][i_3] - D[i_4][i_4+i_3][i_5-2] + D[i_4][i_5][i_5+i_3] * D[i_4-1][i_3][i_5] - C[i_4][i_5-2][i_3-1] - C[i_4+2][i_5][i_3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4326152123_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
