...
void kernel_3232112221_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            B[i_0+1][i_1+2][0] = C[i_1][i_0] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 1; i_3 < PB_Q; i_3++) {
            for (int i_4 = 2; i_4 < PB_Q; i_4++) {
                B[i_2][i_3][0] = D[i_3-1][i_4][i_2] - B[i_3-1][i_4-2][0] - B[i_2+2][i_3-1][0] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3232112221_09(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
