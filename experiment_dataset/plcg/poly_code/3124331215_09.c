...
void kernel_3124331215_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = min(PB_Q, -i_0+PB_L+1) - 1; i_2 >= 1; i_2--) {
                A[i_1+1][i_2] = B[i_2][i_0][i_1] * C[i_2][i_2+i_0-1][i_1+1] + C[i_1][i_0][i_2] - A[i_1+1][i_2-1] * A[i_1][i_2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3124331215_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
