...
void kernel_3234132113_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_0][i_1+2][0] = C[i_2][i_1+i_0][i_1] - 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < PB_P; i_4++) {
            for (int i_5 = 3; i_5 < min(min(-i_3+PB_P+2, i_4), PB_P-1); i_5++) {
                B[i_3+1][i_4][i_4-2] = C[i_3][i_5+i_3-2][i_4-2] + C[i_5+i_3][0][i_4] + D[i_3][i_5][i_4-1][i_3] * E[i_4][i_5] * C[i_4][i_5+1][i_3] - B[i_4+1][i_5-1][i_5-3] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3234132113_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
