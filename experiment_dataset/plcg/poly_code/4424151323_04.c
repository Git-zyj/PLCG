...
void kernel_4424151323_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_1+PB_L-1, i_0), -i_0+PB_P); i_2++) {
                for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
                    A[i_3+1][i_2+i_1][i_3][i_0] = E[i_1+1][i_3][i_0+1][i_2+i_0] * E[i_0][i_3+i_2-1][i_1-1][i_2] + E[i_1][i_2+1][i_1][i_0] - E[i_2+i_1-1][i_3][i_0][i_1] * E[i_1][i_3][i_0][i_2] + A[i_3+2][i_2+i_1+1][i_3+1][i_0] + A[i_3-1][i_2+i_1][i_3-2][i_0+1] * 6;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 2; i_6 < -i_4+PB_Q; i_6++) {
                for (int i_7 = 1; i_7 < min(min(-i_6+PB_P, i_5), -i_4+PB_P+2); i_7++) {
                    B[i_5][i_7+i_6][i_6][i_4-1] = E[i_7][i_4][i_5-1][i_4-1] + F[i_5+1][i_7][i_4-1][i_6+i_4][i_5-1] * B[i_5][i_7+i_6-2][i_6-2][i_4-1] + B[i_5-1][i_6+i_4-2][i_6][i_7-1] * B[i_5+1][i_7+i_4-2][i_4][i_6-2] + 2;
                }
            }
        }
        for (int i_8 = 2; i_8 < PB_L; i_8++) {
            D[i_8] = G[i_8][i_8] + H[i_4+1] + G[i_4][i_8] - G[i_8-1][i_4] * D[i_8-2] - 5;
        }
    }
    for (int i_9 = 1; i_9 < PB_L-1; i_9++) {
        D[i_9] = H[i_9] - H[i_9] + H[i_9+1] + H[i_9+1] * H[i_9] * H[i_9+1] + D[i_9+1] * D[i_9+2] - D[i_9-1] - 5;
    }
#pragma endscop
...
kernel_4424151323_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
