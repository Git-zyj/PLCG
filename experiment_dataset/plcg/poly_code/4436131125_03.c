...
void kernel_4436131125_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = PB_Q-1 - 1; i_2 >= i_0; i_2--) {
                for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
                    A[i_1-1][i_3-1][i_0][i_2-1] = E[i_1][i_1][i_2][i_0] + F[i_3][i_1][i_2] - E[i_1+1][i_3-1][i_0][i_2+1] - A[i_1-1][i_0-1][i_2][i_3+1] - 5;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
            C[i_5] = G[i_4][i_5] - G[i_4][0] + H[i_5+1] * H[i_5] + 2;
        }
        C[i_4] = H[i_4] * 4;
    }
    for (int i_6 = 1; i_6 < PB_L; i_6++) {
        for (int i_7 = 0; i_7 < min(i_6, PB_Q); i_7++) {
            for (int i_8 = 0; i_8 < PB_P; i_8++) {
                for (int i_9 = 0; i_9 < min(-i_7+PB_Q, i_7); i_9++) {
                    D[i_6-1][i_7][i_8][i_8] = E[i_6-1][i_6][i_8][i_9+i_7] - 1;
                }
            }
        }
    }
...
kernel_4436131125_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
