...
void kernel_3224312311_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                A[i_2][i_1][0] = C[i_1-1][i_2][i_0] + C[i_1][i_0][i_2] * A[i_2+1][i_1][0] * A[i_2-1][i_1][0] + B[i_2][i_1+2] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L-2; i_4++) {
            for (int i_5 = 0; i_5 < min(PB_L-2, i_3); i_5++) {
                B[i_4][i_5+2] = B[i_5+1][i_4+2] + B[i_4][i_5+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3224312311_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
