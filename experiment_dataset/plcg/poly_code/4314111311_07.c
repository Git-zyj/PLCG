...
void kernel_4314111311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1] = D[i_1][i_0] * D[i_1][i_0] + D[i_0][i_0] + A[i_1+1] + A[i_1-1] + 5;
        }
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            B[i_0-1][i_2] = B[i_0-1][i_2-1] * B[i_0-1][i_2+1] - B[i_0][i_2] + C[i_0-1][i_2-1] + 1;
            for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
                C[i_3][i_2] = C[i_3-1][i_0-1] + C[i_0][i_2-1] * C[i_2][i_3+1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4314111311_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
