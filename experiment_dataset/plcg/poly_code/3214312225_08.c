...
void kernel_3214312225_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < min(min(PB_P, -i_1+PB_M+1), -i_0+PB_M+2); i_2++) {
                A[i_2+i_0-2][i_1][i_2-2] = C[i_0-1][i_2][1] - A[i_2+i_1-1][i_0+2][i_1-1] - 2;
            }
        }
        for (int i_3 = i_0; i_3 < PB_N-2; i_3++) {
            B[i_3][i_0] = D[0] * B[i_3+2][i_0] + B[i_3+2][i_0+1] * 4;
        }
    }
#pragma endscop
...
kernel_3214312225_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
