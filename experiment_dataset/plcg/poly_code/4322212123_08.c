...
void kernel_4322212123_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                A[i_2][i_1+i_0][i_0-2] = C[i_1+i_0-1][i_0+1][i_2-1] - 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            for (int i_5 = 3; i_5 < -i_3+PB_P-1; i_5++) {
                A[i_4][i_5+i_3+1][i_5-3] = D[i_4-2][i_3-1][i_5] + 3;
            }
        }
        for (int i_6 = 0; i_6 < -i_3+PB_M; i_6++) {
            for (int i_7 = 2; i_7 < PB_L+2; i_7++) {
                C[i_6+i_3][i_3+2][i_7-2] = C[i_6+i_3-1][i_3+1][i_7-2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4322212123_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
