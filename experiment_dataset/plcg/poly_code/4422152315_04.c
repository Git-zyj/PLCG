...
void kernel_4422152315_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
        ...
        for (int i_1 = i_0; i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_Q-1; i_2++) {
                for (int i_3 = 2; i_3 < -i_2+PB_M-1; i_3++) {
                    A[i_1][i_2][i_3-2][i_2+i_0] = E[i_3+i_2+1][i_1][i_1+2] - F[i_2][i_3][i_0][1] - F[i_2][i_0-1][i_1+i_0-2][i_3-2] + G[i_3+1][i_1][i_0][i_2][i_3] * F[i_1][i_3][i_2][i_0-2] * F[i_1-2][i_3][i_0+1][i_2] * F[i_1][i_3+i_2][i_0][i_2] + F[i_1][i_0+2][i_3][i_2-2] - A[i_1][i_2][i_0-3][i_3+i_2] - A[i_1-1][i_2+1][i_3-2][i_2+i_0+1] - 3;
                }
            }
            for (int i_4 = 1; i_4 < min(-i_1+PB_M-1, -i_0+PB_M-3); i_4++) {
                B[i_1+i_0+2][i_4][i_0] = E[i_1+i_0-1][i_4][i_1] - E[i_1][i_1+2][i_0-1] * B[i_4+i_1+1][i_0+1][i_4] * B[i_1+i_0+1][i_4-1][i_0-1] - B[i_4+i_0+3][i_1][i_0] - 3;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 2; i_6 < -i_5+PB_M-2; i_6++) {
            for (int i_7 = 2; i_7 < PB_L-2; i_7++) {
                B[i_6+i_5+2][i_7-1][i_5] = E[i_6-2][i_5][i_7+2] - E[i_7-1][i_6+i_5-2][0] + H[i_6-2][i_7-2] * E[i_6][i_5][i_5] - H[i_7][i_5-1] * E[i_7][i_7][i_6] - 1;
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_M-2; i_8++) {
        for (int i_9 = i_8; i_9 < PB_M-2; i_9++) {
            D[i_8+1][i_9+1] = H[i_8-1][i_9-1] * H[i_9][i_8] * H[i_9][i_8] - H[i_9+2][i_8] + D[i_8+1][i_9+2] - D[i_8+1][i_9] * D[i_8+2][i_9] * D[i_8+2][i_9+1] - 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422152315_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
