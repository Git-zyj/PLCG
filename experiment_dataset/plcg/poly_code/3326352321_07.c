...
void kernel_3326352321_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] * D[i_0] * E[i_0] + E[0] + F[i_0-1] * D[i_0+1] - D[i_0-2] - G[i_0-2][0] + D[i_0+1] - A[i_0-2] - A[i_0-1] * A[i_0+2] - A[i_0+1] - A[i_0+1] + 6;
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                C[i_2][0][i_1+1] = H[i_0][i_2+2][i_1] - 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_P, PB_L); i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                C[i_3][0][i_5] = I[i_4][i_5][i_3-2] - H[i_5][i_5-1][i_3] + J[i_4-1][i_5][i_4+i_3] * J[i_5-1][i_5][i_4] - H[i_3][i_5][i_4+2] + C[i_4-1][0][i_5] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3326352321_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
