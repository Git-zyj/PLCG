...
void kernel_4312312223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M-1; i_2++) {
                A[i_2+i_1][i_1-1] = A[i_2+i_1+1][i_1-1] * 2;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_M-1; i_3++) {
        B[i_3][i_3-1] = D[i_3] + B[i_3-2][i_3-3] * B[i_3+1][i_3] + C[i_3][i_3+1] + 2;
        for (int i_4 = 0; i_4 < -i_3+PB_M; i_4++) {
            C[i_3][i_3+1] = A[i_4+i_3][i_3-1] + C[i_3-2][i_3-1] + 3;
        }
    }
#pragma endscop
...
kernel_4312312223_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
