...
void kernel_3326331125_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            B[0][i_0-1][i_1-1] = D[i_1][i_0][i_0] - E[i_1][i_0-1] - F[i_1][i_0-1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_Q+1; i_4++) {
                B[0][i_3-1][i_4-1] = G[i_3][i_2][i_4+i_3-1] * D[i_3-1][i_2+1][i_4] * 4;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
        C[i_5] = H[i_5] * I[i_5-1] * C[i_5+2] * 4;
    }
#pragma endscop
...
kernel_3326331125_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
