...
void kernel_3424231315_01(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_M, -i_0+PB_L-1); i_1++) {
            D[i_1] = E[i_1] - F[i_1] * G[i_1][i_1+i_0+1] + D[i_1] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            D[i_3-1] = H[i_3+1][i_2+1] - E[i_3-1] * 2;
        }
        for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 2; i_5 < min(i_4, -i_2+PB_M+1); i_5++) {
                C[i_2-1][i_5][i_4+1] = G[i_5+i_2-1][0] * I[i_5][i_2][0] * C[i_4-2][i_5+1][i_2+2] * C[i_5-2][i_4+1][i_2+1] + 6;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
        D[i_6] = E[i_6] * F[i_6-1] * D[i_6-1] - D[i_6+1] - 5;
    }
#pragma endscop
...
kernel_3424231315_01(xd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
