...
void kernel_4232112121_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_N+2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_L-2, PB_M); i_2++) {
                A[i_0][i_2+i_1+2][i_0] = C[i_1-2][i_2][i_0] + A[i_0+2][i_2+i_1+2][i_0+2] * 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-3; i_4++) {
            for (int i_5 = 0; i_5 < min(i_3, -i_4+PB_L-3); i_5++) {
                A[i_3-1][i_5+i_4+3][i_3-1] = 5;
            }
        }
    }
#pragma endscop
...
kernel_4232112121_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
