...
void kernel_4236151123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[i_0] = C[i_0][i_0] * D[i_0] - D[i_0] - D[i_0] * C[1][i_0] - D[0] + D[i_0+1] + 5;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            A[i_1] = C[i_2][i_1-1] - C[i_2][i_1] * D[i_2] + A[i_2] - 5;
        }
    }
#pragma endscop
...
kernel_4236151123_00(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
