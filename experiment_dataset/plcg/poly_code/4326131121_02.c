...
void kernel_4326131121_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_L; i_2++) {
            B[i_1+1][i_2] = E[i_2-1][i_1] - E[i_2][i_1] + E[i_2][0] * E[i_2+1][i_1+1] + D[i_1] + B[i_1][i_2] + 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            B[i_3][i_4] = E[i_3][i_4] + 6;
        }
    }
#pragma endscop
...
kernel_4326131121_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
