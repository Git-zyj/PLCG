...
void kernel_2212151125_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-2 - 1; i_0 >= 2; i_0--) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            B[i_0+2][i_0+1] = C[i_0][0] - C[1][i_0] + C[i_1][i_0-1] + C[i_1-1][i_0] * C[i_0-1][i_1] + 1;
        }
        B[i_0+1][i_0] = D[i_0] + D[i_0] * D[i_0-1] + C[i_0][0] + D[i_0-1] * B[i_0-1][i_0-2] + 6;
    }
#pragma endscop
...
kernel_2212151125_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
