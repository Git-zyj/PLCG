...
void kernel_2424352125_08(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        D[i_0] = E[0] + F[i_0+2] - E[i_0+2] * E[i_0+2] * 5;
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            D[i_1] = F[i_0+1] - G[i_1][i_0] - H[i_0][i_1] - I[i_1+2][i_0] * G[i_0][1] + I[i_1+2][i_0] * 4;
        }
        for (int i_2 = 1; i_2 < i_0; i_2++) {
            D[i_2-1] = H[i_2][0] - I[i_2][i_0] - 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        D[i_3] = J[i_3] * F[i_3+1] + H[i_3][2] + F[i_3+1] * J[i_3] * J[i_3-2] + E[i_3] + D[i_3+1] * 2;
    }
#pragma endscop
...
kernel_2424352125_08(xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
