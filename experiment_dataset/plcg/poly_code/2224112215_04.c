...
void kernel_2224112215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            A[i_0][i_1-1] = A[i_0+1][i_1-1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
            A[i_2-1][i_3-1] = C[i_2+1][i_3+2] - A[i_2][i_3-1] + 5;
        }
    }
#pragma endscop
...
kernel_2224112215_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
