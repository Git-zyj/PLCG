...
void kernel_3336212115_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_0+2][i_1-2] = D[i_1][i_1] * A[i_0+3][i_1-1] + 6;
            A[i_0+2][i_1-2] = 3;
        }
        for (int i_2 = 2; i_2 < i_0; i_2++) {
            for (int i_3 = 2; i_3 < PB_M; i_3++) {
                C[i_2-2][i_3-2][i_0+2] = E[i_3][i_2][i_0-2] + C[i_2-2][i_3-2][i_0+1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3336212115_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
