...
void kernel_2224311313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_1+1][i_0] = C[i_1-1] * A[i_1][i_0] * A[i_1+1][i_0+1] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        B[0] = B[0] * B[0] * B[0] * B[0] + 6;
    }
#pragma endscop
...
kernel_2224311313_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
