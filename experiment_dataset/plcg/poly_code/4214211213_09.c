...
void kernel_4214211213_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
        ...
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                    B[i_2][i_3+1] = C[i_1][i_3+1][i_0] - 5;
                }
            }
        }
        for (int i_4 = 1; i_4 < min(-i_0+PB_M, PB_L-2); i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_L-1; i_5++) {
                B[i_0][i_5+1] = C[i_4+i_0][i_5+i_4+1][i_0] + B[i_4-1][i_0] - B[i_0][i_4+2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4214211213_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
