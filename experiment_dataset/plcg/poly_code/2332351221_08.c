...
void kernel_2332351221_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-1 - 1; i_0 >= 2; i_0--) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_1][i_0] = D[i_1+i_0][i_0-1] * E[i_1] * E[i_1] * F[i_1][i_1-1] - 1;
        }
        for (int i_2 = 2; i_2 < -i_0+PB_M; i_2++) {
            B[i_2][i_0-1] = D[i_2+1][i_0] - F[i_2+i_0][i_2] + G[i_2+i_0][i_2-1] - G[i_0][i_2] - D[i_0-1][i_2] * B[i_2-2][i_0+1] * B[i_2][i_0+1] * B[i_2+2][i_0-2] - 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
            C[i_4+i_3][i_4+1] = G[i_4][i_3] * G[i_4-1][i_3] + E[i_4+1] * F[i_4+1][i_3] + D[i_4-1][i_3+1] + G[i_4-1][1] + A[i_4][i_3] + C[i_4+i_3-1][i_4+1] - 5;
        }
    }
#pragma endscop
...
kernel_2332351221_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
