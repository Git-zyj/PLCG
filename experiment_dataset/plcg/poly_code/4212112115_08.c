...
void kernel_4212112115_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_P+1 - 1; i_0 >= 2; i_0--) {
        for (int i_1 = 2; i_1 < PB_P+2; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_1][i_1-2][i_0-1] = C[0][i_0][i_2+1] + A[i_1][i_1-2][i_0-2] + 6;
            }
        }
        for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
            A[i_3+1][i_3-1][i_0-1] = 4;
        }
    }
#pragma endscop
...
kernel_4212112115_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
