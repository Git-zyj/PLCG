...
void kernel_3326212311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        ...
        for (int i_3 = 0; i_3 < min(PB_P-1, i_1); i_3++) {
            for (int i_4 = i_3; i_4 < PB_P; i_4++) {
                C[i_3][i_4] = B[i_3] * B[i_4] - C[i_4][i_3+1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3326212311_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
