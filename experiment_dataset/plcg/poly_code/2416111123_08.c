...
void kernel_2416111123_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_P, i_3); i_4++) {
            D[i_4-2] = E[i_3][i_4+i_3] - F[i_3-1] - 1;
            D[i_4] = E[i_3][0] - E[i_3][i_4] + D[i_4+2] + D[i_3-2] * 3;
        }
    }
#pragma endscop
...
kernel_2416111123_08(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
