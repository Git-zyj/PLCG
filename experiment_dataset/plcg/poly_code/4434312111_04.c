...
void kernel_4434312111_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
        ...
        for (int i_2 = 0; i_2 < -i_1+PB_M+1; i_2++) {
            for (int i_3 = 1; i_3 < i_2; i_3++) {
                for (int i_4 = 0; i_4 < PB_Q; i_4++) {
                    B[i_2+i_1-1][i_1-1][i_3-1][i_4] = 6;
                }
            }
        }
        A[i_1+3] = F[i_1] - F[i_1+2] + 4;
    }
    for (int i_5 = 0; i_5 < PB_P; i_5++) {
        for (int i_6 = 2; i_6 < PB_P-2; i_6++) {
            for (int i_7 = 0; i_7 < PB_M; i_7++) {
                for (int i_8 = 2; i_8 < min(min(i_6, PB_P-1), -i_7+PB_M); i_8++) {
                    D[i_6-2][i_7][i_8+1][i_6-1] = G[i_8+i_7][i_5][i_6-2][i_5] * D[i_8-2][i_7][i_6+2][i_8-1] - 1;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4434312111_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
