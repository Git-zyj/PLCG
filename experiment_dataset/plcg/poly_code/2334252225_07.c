...
void kernel_2334252225_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        B[i_0+1] = D[i_0+2] * E[i_0] * D[i_0] - E[i_0] + E[i_0] * 5;
    }
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_N, i_1); i_2++) {
            B[i_2+1] = D[i_2+i_1] + F[i_1][i_2][i_1-2] * D[i_1] * G[i_2][i_1] - H[i_2+i_1][i_1] * G[i_2][i_2+1] * B[i_2+3] * B[i_2+2] - 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_P; i_4++) {
            B[i_3-1] = G[i_3][i_4] - G[i_4][i_3+2] - G[i_4-2][i_3] - H[i_4-2][0] + 6;
        }
    }
#pragma endscop
...
kernel_2334252225_07(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
