...
void kernel_3422251213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[0] * E[i_0] + A[i_0] + A[i_0+2] - 2;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        A[i_1+2] = F[i_1] + G[i_1][0] - E[i_1] * E[i_1-1] - E[i_1] + 1;
        A[i_1+1] = F[i_1] * F[i_1] + F[i_1+1] + F[i_1-1] * E[i_1+1] - F[i_1] * 1;
        for (int i_2 = i_1; i_2 < min(-i_1+PB_L-1, PB_M); i_2++) {
            for (int i_3 = 0; i_3 < min(-i_2+PB_N, PB_M-1); i_3++) {
                D[i_3][i_2][i_3] = H[i_3][i_2][i_1] + I[i_3][1][i_2] * G[i_3+1][i_2+i_1+1] - H[i_3][i_1-1][i_2] - I[i_1+1][i_3][i_2] * H[i_3+i_2][i_1][i_2-1] + I[i_3][i_1][i_2] * D[i_3][i_2-1][i_3] * D[i_3][i_2+1][i_3] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3422251213_01(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
