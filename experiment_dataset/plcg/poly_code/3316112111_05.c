...
void kernel_3316112111_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0][i_1] = D[i_0][i_1+2] - D[i_1][i_0] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                B[i_4][i_2][i_3][i_2] = B[i_4][i_2][i_3-1][i_2] - 4;
            }
        }
        for (int i_5 = i_2; i_5 < PB_M; i_5++) {
            for (int i_6 = i_5; i_6 < PB_M; i_6++) {
                C[i_5][i_2][i_6] = C[i_5-1][i_2][i_6-1] + C[i_6][i_5-1][i_2-1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3316112111_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
