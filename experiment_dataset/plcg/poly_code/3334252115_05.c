...
void kernel_3334252115_05(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        C[i_0] = D[i_0][i_0+2] + E[i_0] - 6;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_L, PB_M-2); i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_L+1, -i_2+PB_M-2); i_3++) {
                C[i_3] = D[i_3][i_2-1] - F[i_3+i_2-2][i_1-1][0] + F[i_3+i_2+2][i_1][i_3+2] + F[i_3][i_2][i_1] + G[i_3][i_2] + F[i_2][i_2+i_1][i_3] * F[i_2][i_3][i_1] * F[i_1][i_3+i_1-1][i_3] * F[i_3][i_1][i_1] * H[i_3+2][i_1][0] + G[i_3][i_2-2] * H[i_1-1][i_3][i_2] - 1;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, PB_M-1); i_5++) {
            C[i_5] = D[i_5][i_4] + C[i_5+1] - 6;
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3334252115_05(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
