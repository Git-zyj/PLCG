...
void kernel_3426212321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_0][i_1] = E[i_0+2] * A[i_0-1][i_1] - A[i_0][i_1-1] * A[i_0-2][i_1-2] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        B[i_2-2] = F[i_2+1] - B[i_2-1] + 6;
    }
    for (int i_3 = 3; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_M-2; i_4++) {
            C[i_4+2][i_3-1] = C[i_4+2][i_3-3] + C[i_4+4][i_3-1] - D[i_4+i_3][i_4] * D[i_4+i_3+2][i_4+2] - 6;
            for (int i_5 = i_3; i_5 < PB_M-2; i_5++) {
                D[i_5+i_4][i_4] = C[i_5+2][i_4-1] - D[i_5+i_4+1][i_5+1] - D[i_5+i_4+1][i_4] + D[i_5+i_4-1][i_4] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3426212321_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
