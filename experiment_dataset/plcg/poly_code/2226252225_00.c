...
void kernel_2226252225_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < min(min(-i_0+PB_M, i_0), PB_M-1); i_1++) {
            A[i_1-1] = C[i_1][i_0] - D[i_1+i_0][0] + C[i_1][i_0] * D[i_1][i_0] - D[i_0][i_1] + D[i_1][i_0+2] * A[i_1-3] - A[i_1+1] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_M+2, PB_M-1); i_3++) {
            B[i_2] = D[i_3][i_2] + D[i_2][i_3] * C[i_3+i_2-2][i_3+i_2] * B[i_3+1] - 6;
        }
    }
#pragma endscop
...
kernel_2226252225_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
