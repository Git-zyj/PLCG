...
void kernel_3326231125_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0] = D[i_0] * E[i_0+1] + A[i_0-2] - 2;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_P, i_1); i_2++) {
            for (int i_3 = i_1; i_3 < PB_M; i_3++) {
                B[i_3][i_2+i_1][i_3] = F[i_3][i_1][i_2] - F[i_2][i_1][i_3] * F[i_3-1][i_1-1][i_2] + 6;
            }
        }
        for (int i_4 = i_1; i_4 < -i_1+PB_M; i_4++) {
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                B[i_5][i_4+i_1+1][i_5] = G[i_5][i_4] - H[i_4+i_1][i_5][i_4+1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3326231125_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
