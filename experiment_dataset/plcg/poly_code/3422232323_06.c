...
void kernel_3422232323_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-1] = E[i_0-2] + E[i_0] - E[i_0-1] * F[i_0+2] * E[i_0-2] - F[i_0+1] * A[i_0] + B[i_0] - B[i_0+1] + 4;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            B[i_1] = G[i_1+i_0][i_1] * A[i_1-1] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
            for (int i_4 = 2; i_4 < PB_P-2; i_4++) {
                C[i_3][i_2-1][i_4] = H[i_2+2][i_4][i_3] + I[i_3][i_2][i_2] + C[i_4-2][i_2-1][i_3-1] - C[i_3][i_2+1][i_4+2] - C[i_3-1][i_2-1][i_4] + 4;
            }
            B[i_3] = F[i_2] + 3;
        }
    }
...
kernel_3422232323_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
