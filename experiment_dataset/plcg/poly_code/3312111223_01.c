...
void kernel_3312111223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1+1] = D[i_0][i_1-1] + A[i_1] - 2;
        }
        B[i_0] = B[i_0+2] * B[i_0-1] * 3;
    }
    for (int i_2 = 3; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            C[i_2-1][i_3-1] = C[i_2-3][i_3-1] * C[i_3+1][i_2-2] - 6;
        }
    }
#pragma endscop
...
kernel_3312111223_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
