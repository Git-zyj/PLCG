...
void kernel_3236311213_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_P-1); i_1++) {
            for (int i_2 = -i_1+1; i_2 < -i_1+PB_M-1; i_2++) {
                B[i_2+i_1+1][i_1+1] = C[i_1][i_2][i_0] * C[i_1][i_2][i_1+i_0+1] * B[i_2+i_1-1][i_2] - 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
            B[i_4+i_3][i_4] = B[i_4+i_3][i_4-1] + 4;
        }
    }
#pragma endscop
...
kernel_3236311213_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
