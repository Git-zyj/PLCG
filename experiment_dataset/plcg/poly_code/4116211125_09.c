...
void kernel_4116211125_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-1; i_1++) {
            for (int i_2 = 3; i_2 < i_1; i_2++) {
                for (int i_3 = max(max(1, -i_1+1), -i_2+3); i_3 < min(-i_1+PB_P+1, -i_2+PB_P+3); i_3++) {
                    A[i_0+1][i_3][i_3+i_1-1] = B[i_3-1][i_0][i_2] - A[i_1+1][i_2-2][i_3+i_2-3] + 6;
                }
            }
        }
    }
...
kernel_4116211125_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
