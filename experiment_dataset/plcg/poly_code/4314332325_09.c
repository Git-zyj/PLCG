...
void kernel_4314332325_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
        ...
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(PB_L-3, -i_0+PB_P); i_3++) {
                    A[i_0][i_1][i_3][i_3+2] = D[i_1+i_0][i_3][i_2][i_3+i_0] + D[i_2][i_0][i_1][i_0-2] + A[i_0-2][i_1-2][i_3+1][i_3+3] * 5;
                }
            }
            for (int i_4 = 1; i_4 < PB_Q; i_4++) {
                for (int i_5 = 3; i_5 < PB_L-1; i_5++) {
                    B[i_4][i_5-1][i_4][0] = E[i_0][i_1+1][i_5-2][0] - F[i_4][i_0][i_5+1][i_1-1] + D[i_5+1][i_5+1][i_4][i_1+2] * B[i_4][i_5][i_4][0] - B[i_0-2][i_5-1][i_0-2][0] - B[i_4-1][i_5-3][i_4-1][0] + B[i_0][i_5+1][i_0][0] + 6;
                }
            }
        }
        for (int i_6 = i_0; i_6 < PB_L; i_6++) {
            for (int i_7 = 2; i_7 < PB_Q; i_7++) {
                for (int i_8 = 0; i_8 < min(PB_L-4, -i_6+PB_L); i_8++) {
                    A[i_0][i_6+2][i_8+2][i_8+4] = D[i_8+i_6][i_7-2][i_0+2][0] + E[i_6][i_8][i_0][i_7] - 6;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4314332325_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F)));
