...
void kernel_3232112325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 4; i_1 < PB_L-3; i_1++) {
            A[i_1-2][i_1+2] = A[i_1-3][i_1+1] - A[i_1-1][i_1+3] + A[i_1-4][i_1] * 6;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
                B[i_2][i_0][i_3] = C[i_2][i_0][i_0] + C[i_2][i_3][i_0] - B[i_0][i_2-1][i_3] * B[i_2-1][i_0][i_3+2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3232112325_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
