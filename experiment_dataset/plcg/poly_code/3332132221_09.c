...
void kernel_3332132221_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
                A[i_0+2][i_1][i_0] = D[i_0][i_2+i_1][i_1+1] * E[i_2][i_1] * D[i_2+1][i_0][i_1+2] * A[i_0+1][i_1][i_0-1] - 4;
            }
            for (int i_3 = 0; i_3 < i_1; i_3++) {
                B[i_1+2][i_3][i_0] = E[i_3][i_1+i_0] * D[i_3][i_0][i_1+2] + 6;
            }
        }
    }
    for (int i_4 = 4; i_4 < PB_P-2; i_4++) {
        for (int i_5 = 2; i_5 < -i_4+PB_L+1; i_5++) {
            for (int i_6 = 0; i_6 < min(i_4, -i_4+PB_L-1); i_6++) {
                C[i_6+2][i_5+i_4-1][i_4][0] = D[i_6][i_4+1][i_5-1] - D[i_5][i_6][i_5] - B[i_4+2][i_6][i_5-2] + C[i_5+2][i_6+i_4+1][i_4+2][0] - C[i_6][i_5+i_4-4][i_4-2][0] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3332132221_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
