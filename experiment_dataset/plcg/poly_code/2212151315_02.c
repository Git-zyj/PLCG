...
void kernel_2212151315_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1-1] = C[i_0][i_1] + C[i_1+i_0][i_1+1] * C[i_0-1][i_1] + C[i_1+1][i_0+1] + C[i_1][i_0] + A[i_1+1][i_0-1] - A[i_0-1][i_1-2] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_L, PB_M-1); i_3++) {
            A[i_3-1][i_2-1] = C[i_3][i_3+i_2] * C[i_3][i_2+1] * C[i_2][i_2-1] - C[i_3+1][i_2-1] * C[i_3][i_2-1] - A[i_3][i_2-1] + 1;
        }
    }
#pragma endscop
...
kernel_2212151315_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
