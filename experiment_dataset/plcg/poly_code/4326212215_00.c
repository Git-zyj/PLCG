...
void kernel_4326212215_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-1; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < PB_L-1; i_2++) {
                A[i_1+i_0][i_2][i_0+1] = D[i_2-2][i_0][i_1+2] + A[i_1+i_0+1][i_2][i_0+1] * A[i_1+i_0][i_2+1][i_0+1] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = i_3; i_5 < PB_L-1; i_5++) {
                B[i_3][i_4+1][i_3] = B[i_3+1][i_4][i_3+1] * B[i_4-1][i_5+1][i_4-1] * B[i_3][i_5][i_3] * 4;
            }
        }
    }
        ...
        for (int i_7 = 0; i_7 < -i_6+PB_M-1; i_7++) {
            for (int i_8 = 1; i_8 < PB_L; i_8++) {
                for (int i_9 = max(i_6, 1); i_9 < PB_P; i_9++) {
                    C[i_7+i_6][i_8-1][i_8][i_9] = C[i_7+i_6+1][i_8-1][i_8][i_9-1] * 1;
                }
            }
        }
    }
...
kernel_4326212215_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
