...
void kernel_4232112315_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            A[i_0][i_0] = A[i_0-1][i_0-1] * A[i_1-1][i_1-1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < PB_P+2; i_3++) {
            A[i_2][i_2] = C[i_2][i_3-2] * C[i_3][i_2] - A[i_2][i_2] - 4;
        }
    }
#pragma endscop
...
kernel_4232112315_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
