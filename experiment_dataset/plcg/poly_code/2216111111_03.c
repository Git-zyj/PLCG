...
void kernel_2216111111_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            A[i_0-1][i_1] = A[i_0][i_1-1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_L+1, PB_P); i_3++) {
            A[i_3-1][i_2-1] = C[i_3+i_2-1][0] * 6;
        }
    }
#pragma endscop
...
kernel_2216111111_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
