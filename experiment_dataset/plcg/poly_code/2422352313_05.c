...
void kernel_2422352313_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0-1] = E[0] - F[i_0] * E[i_0] * G[i_0+2][0] + E[i_0] - H[i_0+1] + A[i_0] * 5;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        B[i_1] = E[i_1] * H[i_1] + F[i_1-1] - B[i_1+1] * B[i_1-1] * 1;
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
            A[i_3-1] = G[i_3][i_2] - G[i_3][i_2] * I[i_3][i_2] + E[0] * J[i_3-2][i_3-2] - I[i_3+1][i_2-2] + B[i_3] - 3;
        }
        for (int i_4 = i_2; i_4 < PB_M; i_4++) {
            A[i_4] = J[i_4-2][i_2] - G[i_4-1][i_2-1] + J[i_4][i_2+1] + B[i_4] * 2;
        }
    }
#pragma endscop
...
kernel_2422352313_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
