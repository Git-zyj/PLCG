...
void kernel_4334132311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_P+2, i_1); i_2++) {
            C[i_2] = E[i_2][i_2+i_1-2][i_1-2] - D[i_1] + F[i_2-2][i_1] * F[i_1][i_2] + C[i_2] - 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        C[i_3] = D[i_3] * D[i_3+2] + D[i_3] + C[i_3+1] * C[i_3-1] * C[i_3-1] * 2;
    }
#pragma endscop
...
kernel_4334132311_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
