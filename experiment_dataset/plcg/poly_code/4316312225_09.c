...
void kernel_4316312225_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_P, -i_1+PB_P-2), i_1); i_2++) {
                A[i_1][i_0-2][i_2+i_0] = A[i_0][i_1][i_2+i_1+2] + 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_P; i_4++) {
            for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                A[i_5][i_3][i_4+i_3] = D[i_4-2][i_3-1][0] * 4;
                A[i_5-1][i_4-2][i_4+i_3] = D[i_3][i_5][i_4] * A[i_5+1][i_3-2][i_4+i_3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4316312225_09(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
