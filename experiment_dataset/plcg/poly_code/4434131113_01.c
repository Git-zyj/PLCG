...
void kernel_4434131113_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_Q, PB_P-1); i_2++) {
                for (int i_3 = 0; i_3 < min(min(PB_Q-1, -i_0+PB_M-1), i_0); i_3++) {
                    A[i_2+i_0][i_1+1][i_3][i_3+1] = E[i_0][i_1-1][i_3][i_2+i_0][i_2-1] + F[i_1][i_2][i_0][1] - A[i_3+i_0+1][i_2+1][i_1][i_1+1] - 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                B[i_5][i_6] = G[i_5-1][i_6] - H[i_5][i_6][i_4-1] - B[i_5+1][i_6] - 1;
            }
        }
        for (int i_7 = i_4; i_7 < PB_L; i_7++) {
            B[i_7][i_4] = G[i_7+1][i_4] + I[i_4-1] + G[i_4][0] * G[i_7][i_4-1] * 1;
        }
    }
        ...
        for (int i_9 = 0; i_9 < PB_L-1; i_9++) {
            for (int j_0 = 1; j_0 < PB_L-1; j_0++) {
                for (int j_1 = 0; j_1 < PB_P; j_1++) {
                    D[i_8][i_9][j_1][j_0] = F[j_0+1][j_1][i_9+1][i_8] + H[j_0-1][i_9+1][i_8] * F[i_9][i_8][j_0][0] - 2;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4434131113_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
