...
void kernel_4212332315_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 3; i_1 < PB_M+2; i_1++) {
            for (int i_2 = 3; i_2 < -i_1+PB_M+2; i_2++) {
                A[i_0][i_2-2][i_1] = C[i_2][i_1] + C[i_2+i_1-2][0] + A[i_0+1][i_2-3][i_1+1] * A[i_0][i_1-3][i_2-1] * 4;
            }
        }
        B[i_0+1] = D[i_0] + E[i_0] - E[i_0] - B[i_0] + B[i_0+2] * B[i_0] + 2;
    }
#pragma endscop
...
kernel_4212332315_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
