...
void kernel_3212212223_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            B[i_1][i_1+2][i_0] = B[i_1][i_1+2][i_0] + 1;
        }
        for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
                B[i_3][i_3+2][i_0] = C[i_2][i_3+1][i_2+2] - B[i_3+1][i_3+3][i_0] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3212212223_07(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
