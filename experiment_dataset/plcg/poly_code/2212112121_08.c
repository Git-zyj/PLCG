...
void kernel_2212112121_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = PB_L-1 - 1; i_1 >= max(2, i_0); i_1--) {
            A[i_0-1][i_1] = A[i_0-1][i_1-2] * A[i_0][i_1+1] * 2;
        }
        B[i_0] = C[i_0] * 4;
    }
#pragma endscop
...
kernel_2212112121_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
