...
void kernel_4332231213_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            for (int i_2 = PB_M-1 - 1; i_2 >= 0; i_2--) {
                for (int i_3 = i_2; i_3 < PB_Q; i_3++) {
                    A[i_2][i_3][i_2+1][i_0] = D[i_0][i_2][i_1][i_2] * D[i_1][i_2][i_0-1][i_3] - A[i_1-1][i_2][i_1][i_0] * A[i_1+1][i_3][i_1+2][i_2] + 4;
                }
            }
        }
    }
        ...
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_Q; i_6++) {
                for (int i_7 = 1; i_7 < PB_L; i_7++) {
                    A[i_4][i_6][i_4+1][i_7] = D[i_7-1][i_4-1][i_5-1][i_6] + E[i_7-1][i_5+1][i_4+1][i_6-1] - 6;
                }
            }
        }
        for (int i_8 = 1; i_8 < -i_4+PB_L; i_8++) {
            for (int i_9 = 1; i_9 < PB_L; i_9++) {
                A[i_9-1][i_8][i_9][i_4] = F[i_8+i_4][i_9][i_9+i_8] + G[i_9][i_8] - H[i_8+i_4][i_9][0] + H[i_8-1][i_4-1][i_8] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4332231213_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
