...
void kernel_3426332221_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_N-1; i_1++) {
            A[i_1][i_1-1] = E[i_1] + A[i_1+1][i_1] * 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_3] = F[i_3][i_2] + B[i_3+2] + B[i_2+1] - 5;
        }
        for (int i_4 = max(3, i_2); i_4 < PB_N-2; i_4++) {
            A[i_4-2][i_4-3] = G[i_4][i_2] - F[i_4][i_2] - G[i_4+2][i_2] * G[i_4+1][i_2] - G[i_2][2] * 2;
        }
        for (int i_5 = i_2; i_5 < PB_L; i_5++) {
            for (int i_6 = 2; i_6 < PB_L; i_6++) {
                A[i_6-1][i_6-2] = H[i_5][0][i_2+2] - G[i_6][2] + G[i_6][0] + 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3426332221_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
