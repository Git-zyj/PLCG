...
void kernel_2212212213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            A[i_0][i_1] = C[i_0][i_1] * D[i_0+1][i_1-2] - A[i_0][i_1-1] + 3;
        }
        for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
            A[i_0][i_2] = A[i_0][i_2+1] - 2;
        }
    }
#pragma endscop
...
kernel_2212212213_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
