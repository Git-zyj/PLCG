...
void kernel_2216111221_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            A[i_0] = C[i_1][i_0] + A[i_0+1] - A[i_0-2] - 4;
        }
        B[i_0+1] = D[i_0+1] - B[i_0+3] + B[i_0-1] * 1;
    }
#pragma endscop
...
kernel_2216111221_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
