...
void kernel_3326211321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
    ...
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M; i_2++) {
                C[i_1+1][i_2] = C[i_1+1][i_2] + 5;
            }
            C[i_0+1][i_1] = A[i_1-1] * A[i_0] - A[i_0-2] * A[i_1] - A[i_0+1] * 6;
        }
    }
#pragma endscop
...
kernel_3326211321_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
