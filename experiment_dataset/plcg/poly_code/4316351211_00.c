...
void kernel_4316351211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] * D[i_0-1] + E[i_0-1] + E[i_0] - A[i_0-1] - 4;
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = i_0; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < PB_M; i_3++) {
                    B[i_3][i_1-1][i_2][i_0+1] = F[i_1][i_2-1][i_0] - G[i_3][i_1][i_2][i_3-1] - H[i_1][i_1+i_0][i_2][i_3] * I[i_0+1][i_1][i_3+1][i_2+1][i_2] + B[i_1-1][i_0-1][i_2+1][i_3+1] + 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = i_4; i_5 < -i_4+PB_M; i_5++) {
            C[i_4][i_5] = J[i_5+i_4][i_4-1] - K[i_4+1][i_5] - K[i_5-1][i_4] + J[i_5][i_4] + K[i_5][i_5+i_4-1] * K[i_4][i_5] - C[i_4+1][i_5] - C[i_4+1][i_5-1] + C[i_4-1][i_5-1] + 5;
        }
    }
#pragma endscop
