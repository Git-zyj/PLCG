...
void kernel_2414112323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + 1;
        A[i_0] = A[i_0] - A[i_0-2] + A[i_0+1] + A[i_0-1] * A[i_0+2] + 3;
    }
    for (int i_1 = 0; i_1 < PB_N-4; i_1++) {
        ...
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            D[i_1+2][i_3] = D[i_1+3][i_3+1] + D[i_1+4][i_3] - D[i_1+2][i_3-1] - D[i_1][i_3] - 3;
        }
    }
#pragma endscop
...
kernel_2414112323_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
