...
void kernel_4232312225_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_1][i_0-1][i_2] = A[i_1-1][i_0-1][i_2] * A[i_1][i_0-1][i_2+1] - 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_P, PB_N-2); i_4++) {
            for (int i_5 = 2; i_5 < min(PB_M-2, i_3); i_5++) {
                A[i_4+2][i_3-1][i_5] = C[i_5-2][i_3][i_4+i_3] * D[i_5+2][i_3][i_4+2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4232312225_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
