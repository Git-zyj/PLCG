...
void kernel_2332152323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_0-1][i_1] = D[i_1+1][i_0-1] - E[i_1] * D[i_1][i_0] + D[i_0+1][i_0] * D[i_1][i_0] * E[i_1] - D[i_1+1][i_0] - A[i_0+1][i_1-2] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-4; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-3; i_3++) {
            B[i_3][i_2+2] = D[i_3+1][i_2] * D[i_3][i_2] + B[i_3][i_2+4] - B[i_3-1][i_2+2] * B[i_2+2][i_3+3] - C[i_2] * 1;
        }
        C[i_2] = E[i_2] * E[i_2] - E[i_2] - E[i_2-1] - C[i_2-1] + C[i_2+2] + 4;
    }
#pragma endscop
...
kernel_2332152323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
