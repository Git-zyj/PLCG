...
void kernel_3416332115_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_L-1, PB_P); i_2++) {
                A[i_1][i_2-1] = E[i_2+i_1+1][1][i_0][i_1] * A[i_0+1][i_1] * 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_Q; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_L; i_4++) {
            A[i_4][i_3-1] = F[i_3][0] * F[i_4+i_3][i_4+1] - F[i_4][i_3] * 3;
            A[i_4+1][i_3-2] = G[i_4][i_4+i_3] - G[i_3-2][i_4] * H[i_4+i_3][i_3] * 5;
        }
        for (int i_5 = 1; i_5 < -i_3+PB_M; i_5++) {
            A[i_5][i_3] = H[i_5+i_3][i_3-1] + H[i_5-1][i_3] * G[i_5+i_3][i_3] * 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3416332115_00(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
