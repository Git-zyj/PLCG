...
void kernel_2214151313_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P+1, -i_0+PB_L); i_1++) {
            A[i_1] = C[i_1+i_0][i_1-1] + C[i_1][i_0] - D[i_1] * D[i_1] * C[i_1][i_0] + A[i_1+1] * 6;
        }
        A[i_0-1] = D[i_0-1] - D[i_0] * D[i_0+1] * D[i_0] - C[i_0+1][0] * A[i_0] + A[i_0+1] - 6;
    }
#pragma endscop
...
kernel_2214151313_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
