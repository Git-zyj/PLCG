...
void kernel_3216111125_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < min(PB_M, -i_2+PB_N); i_3++) {
            B[i_3-1][i_2] = C[i_3+i_2][i_3] + B[i_3-3][i_2+1] * 5;
        }
    }
#pragma endscop
...
kernel_3216111125_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
