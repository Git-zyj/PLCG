...
void kernel_2236352125_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-4; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[i_0][i_1+2] = C[i_0][0] + C[i_1+2][i_0] - C[i_1][i_1+i_0] * C[i_1-2][i_0-1] - D[i_0+1][i_1-2] - C[i_1][i_0] + 5;
        }
        for (int i_2 = i_0; i_2 < PB_P; i_2++) {
            A[i_2-1][i_0+4] = C[i_2+1][i_0-2] + D[i_0][i_2] * D[i_2-1][i_0] + D[i_2+1][0] - A[i_0+2][i_2] - 3;
        }
    }
#pragma endscop
...
kernel_2236352125_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
