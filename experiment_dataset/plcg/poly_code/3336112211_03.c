...
void kernel_3336112211_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+1; i_0++) {
        A[i_0-1][i_0] = D[i_0-2] * A[i_0-2][i_0-1] + 5;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        ...
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            for (int i_4 = i_1; i_4 < PB_L-1; i_4++) {
                C[i_4][i_3] = E[i_3][i_4+1] + C[i_4][i_3+1] * C[i_4+1][i_3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3336112211_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
