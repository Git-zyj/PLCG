...
void kernel_3234252323_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M-2; i_0++) {
        A[i_0-2] = C[i_0+1] + C[i_0-1] * C[i_0+2] * C[0] - D[i_0] - A[i_0-4] - A[i_0-3] - 1;
    }
    for (int i_1 = 2; i_1 < PB_Q; i_1++) {
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            A[i_2-2] = D[i_2] - E[i_2+2][i_2][i_1] * F[i_1-2][i_2+2] * F[i_1][i_2+2] + D[i_2-1] - A[i_2-2] * 2;
        }
    }
#pragma endscop
...
kernel_3234252323_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
