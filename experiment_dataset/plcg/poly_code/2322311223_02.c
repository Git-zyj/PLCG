...
void kernel_2322311223_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][i_1+1] = D[i_1][i_0] * A[i_0+1][i_1+1] + 2;
        }
    }
    ...
    for (int i_4 = 1; i_4 < PB_L-3; i_4++) {
        C[i_4+1] = D[i_4][i_4-1] - C[i_4+3] - C[i_4+2] * 4;
    }
#pragma endscop
...
kernel_2322311223_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
