...
void kernel_4414111121_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                A[i_1][i_0-1][i_2+1] = A[i_1][i_0+1][i_2+1] * 3;
            }
        }
        B[i_0] = E[i_0] * B[i_0-1] - 6;
        for (int i_3 = i_0; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
                C[i_0][i_3][i_4] = C[i_0-1][i_3][i_4+2] * 1;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M; i_5++) {
        D[i_5] = E[i_5-1] * D[i_5-2] * 1;
    }
#pragma endscop
...
kernel_4414111121_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
