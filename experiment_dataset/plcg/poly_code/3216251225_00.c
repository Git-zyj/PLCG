...
void kernel_3216251225_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_Q-1, -i_0+PB_L); i_1++) {
            for (int i_2 = i_1; i_2 < PB_P; i_2++) {
                B[i_2][i_1-2] = C[i_2][i_0+1][i_1+1] - C[i_1][i_2][i_0] + C[i_1-1][i_0+1][i_0] * D[i_2-1][i_2][i_1+i_0] + 6;
            }
        }
        for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
            for (int i_4 = 2; i_4 < -i_0+PB_P-1; i_4++) {
                B[i_3][i_4] = E[i_4-1][i_3-1] + C[i_4+1][i_4+i_0+1][i_3] * D[i_0][i_4+1][i_3-1] + C[i_0][i_4][i_3] - D[i_3][i_0][i_3+1] + C[i_3][i_4][i_4+i_0-1] + B[i_3][i_4-2] + B[i_3][i_4-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3216251225_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
