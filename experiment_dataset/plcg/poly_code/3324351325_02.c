...
void kernel_3324351325_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_N, i_0); i_1++) {
            C[i_1+1] = D[i_1] - E[i_0] - F[i_0][i_0] + G[i_1][i_0] * H[i_1-1][i_0-1] * F[i_1][i_0-1] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-3; i_2++) {
        C[i_2+3] = D[i_2] - I[1] * I[0] + C[i_2+2] + 1;
        C[i_2+1] = I[i_2-1] + D[i_2-1] - I[i_2] * E[i_2] * C[i_2+2] * C[i_2+3] - 4;
    }
#pragma endscop
...
kernel_3324351325_02(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
