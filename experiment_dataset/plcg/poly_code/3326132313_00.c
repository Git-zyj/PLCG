...
void kernel_3326132313_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = max(0, -i_1+2); i_2 < min(PB_N-2, -i_1+PB_M+1); i_2++) {
                B[i_1][i_2+2] = D[i_2+i_1-1][i_0][i_2+i_1-2] + 4;
            }
            for (int i_3 = 2; i_3 < PB_L; i_3++) {
                B[i_1][i_3+2] = D[i_3][i_1][2] + D[i_0][i_3][i_1] * D[i_1-2][i_3][0] * D[i_1][i_3][0] - E[i_3-2][i_1-2] - B[i_1][i_0+3] + 2;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < i_4; i_5++) {
            for (int i_6 = 3; i_6 < min(i_4, -i_4+PB_M); i_6++) {
                C[i_4][i_6-2][2] = D[i_6+i_4][i_5-1][i_4] + D[i_6][i_4][i_5] * D[i_5][i_4][i_5-1] + C[i_6+1][i_5-2][2] - C[i_4-1][i_6-2][2] - C[i_4][i_6-3][2] - C[i_4][i_6-1][2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3326132313_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
