...
void kernel_3232132315_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0][i_0] = C[i_0] * C[i_0-1] * A[i_0-1][i_0-1] + A[i_0+1][i_0+1] - A[i_0+1][i_0+1] * 4;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_L, -i_1+PB_M-2); i_2++) {
            for (int i_3 = 0; i_3 < min(PB_L-2, -i_2+PB_M-1); i_3++) {
                B[i_3+1][i_2+i_1+1][i_2] = D[i_2][i_3] + E[i_2][i_1][1] - E[i_2][i_1][i_3+2] + B[i_3][i_2+i_1+2][i_2] - B[i_3+2][i_2+i_1+1][i_2] - B[i_1][i_3+i_2+1][i_2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3232132315_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
