...
void kernel_3332112113_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            B[i_3+1][i_4-1] = D[i_4][i_3-1] - B[i_3+1][i_4] * 4;
        }
        C[i_3] = C[i_3-1] + 5;
    }
#pragma endscop
...
kernel_3332112113_08(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
