...
void kernel_4222351213_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = C[i_0][i_1] + D[i_0-1][i_1+1] + E[i_1-1] * C[i_1][i_0] + D[i_1][i_0] * F[i_0][i_1] + F[i_1+1][i_0-1] * A[i_1+1][i_0] * A[i_1][i_0+1] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            B[i_3][i_2][i_3] = F[i_3][i_2] * D[i_3][i_2] * G[i_3][i_2+1][i_2+1] * B[i_3][i_2-1][i_3] - B[i_3+1][i_2][i_3+1] - 5;
        }
    }
#pragma endscop
...
kernel_4222351213_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
