...
void kernel_4216132321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_0+2][i_1] = C[i_1-1] - D[i_1+i_0+1][0] + D[i_1][i_0] - D[i_1][i_0-1] + A[i_0+4][i_1+2] + A[i_0+3][i_1] * A[i_0+2][i_1+1] - A[i_0+2][i_1-1] * 2;
        }
        for (int i_2 = 2; i_2 < i_0; i_2++) {
            for (int i_3 = 4; i_3 < min(PB_M-4, -i_0+PB_L); i_3++) {
                B[i_2][i_3+2][i_3-2] = E[i_2][i_0+1][i_0] + D[i_2][i_3+i_0] + B[i_2-2][i_3+4][i_3] - B[i_2-2][i_3][i_3-4] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4216132321_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
