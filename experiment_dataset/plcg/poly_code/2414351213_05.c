...
void kernel_2414351213_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-1] - F[i_0-1] + F[i_0] * G[i_0][i_0+1] * E[i_0] * H[i_0-1] - F[i_0] - F[0] + C[i_0+1] - 1;
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            B[i_0][i_1] = I[i_1][i_0] - F[i_1] + I[i_1][i_1+i_0] + F[i_1] - G[i_1][i_0] + C[i_1] + C[i_0] + 1;
        }
        C[i_0] = H[i_0] * F[i_0+1] - F[i_0+1] - H[i_0] * A[i_0] - C[i_0+1] + 5;
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            C[i_3] = G[i_3-1][i_2] * G[i_2][i_3] - H[i_3] * B[i_3][i_2] - 5;
        }
    }
#pragma endscop
...
kernel_2414351213_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
