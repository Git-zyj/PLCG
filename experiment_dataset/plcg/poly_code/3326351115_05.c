...
void kernel_3326351115_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0-1] + E[i_0] - F[i_0] * D[i_0+1] + F[i_0] * 6;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = i_1; i_2 < PB_P-1; i_2++) {
            for (int i_3 = 0; i_3 < PB_N; i_3++) {
                B[i_3][i_2+1] = G[i_2-1][i_3][0] * H[i_3][i_2][i_1-1] + G[i_3][i_1+1][i_2-1] * B[i_3][i_2] * 1;
            }
        }
        A[i_1+1] = F[i_1] * E[i_1] - F[i_1] + F[i_1] - D[i_1] - 6;
    }
#pragma endscop
...
kernel_3326351115_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
