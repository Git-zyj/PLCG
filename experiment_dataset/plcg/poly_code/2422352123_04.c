...
void kernel_2422352123_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_L-4); i_1++) {
            A[i_0][i_1+2] = E[i_0][i_0-1] * E[i_1+i_0][i_0] + F[i_1] - G[i_1][i_0-1] * H[i_1][i_0+2] + A[i_0-1][i_1+4] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        B[i_2] = I[1] - I[i_2] + F[i_2] * J[0] - F[i_2] * F[i_2-1] + 2;
        C[i_2-2] = J[i_2] * F[i_2-1] * I[1] - J[i_2] + 6;
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        B[i_3] = F[i_3] - J[i_3] * J[i_3] - J[i_3-1] + J[0] + 5;
    }
#pragma endscop
...
kernel_2422352123_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
