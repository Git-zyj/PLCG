...
void kernel_3224311111_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            for (int i_3 = PB_L - 1; i_3 >= 2; i_3--) {
                B[i_2+1][i_3-1][i_0] = C[i_2+1][i_3] + B[i_0+1][i_3-2][i_2+1] * B[i_2+1][i_3][i_0+1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3224311111_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
