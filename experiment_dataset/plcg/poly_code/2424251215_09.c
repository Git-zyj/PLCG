...
void kernel_2424251215_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0-1] = E[i_0-1] + F[i_0][i_0] * E[i_0-1] + E[i_0] - A[i_0] * 3;
        for (int i_1 = 0; i_1 < min(PB_N, -i_0+PB_M-2); i_1++) {
            D[i_1+i_0+2][i_1+1] = G[i_1][i_0+1] - G[0][i_0] - 1;
        }
        for (int i_2 = max(2, i_0); i_2 < PB_N; i_2++) {
            A[i_2-2] = F[i_2-1][0] * E[i_2-1] + F[i_2][i_0] * F[i_2][i_0+1] - G[i_2-1][i_2] * F[i_0][i_2] - G[i_0-1][i_2] * 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_N-1, -i_3+PB_M-1); i_4++) {
            D[i_4+i_3][i_4] = G[i_4+1][i_3-1] * F[i_4][i_3+1] - H[i_4] - F[i_4][i_3+1] * I[i_3][i_4][i_3+1] + E[i_4] - E[i_4+i_3+1] - D[i_4+i_3-1][i_4] - D[i_4+i_3-2][i_4-1] * 1;
        }
    }
#pragma endscop
...
kernel_2424251215_09(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
