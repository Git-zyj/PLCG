...
void kernel_3212212115_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_N+2, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < min(PB_P-1, -i_1+PB_N+1); i_2++) {
                A[i_1+i_0-2][i_0][i_2] = A[i_2+i_1-1][i_1+1][i_0+1] - A[i_1+i_0-2][i_0][i_2+1] + 3;
            }
        }
    }
...
kernel_3212212115_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
