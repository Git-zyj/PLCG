...
void kernel_3416351125_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                B[i_0][i_2][i_0] = E[i_1][i_2+1][i_0] - F[i_0][i_2][i_1-1] + E[i_2][i_2+1][i_0] + G[i_1-1][i_0+1][i_2][i_0+1] + H[i_1][i_2+1][i_1-1][i_0] * F[i_0][i_2][i_0] + 1;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            for (int i_4 = 1; i_4 < PB_N; i_4++) {
                B[i_4][i_3][i_4] = G[i_4][i_4][i_0][i_3+1] * I[i_4+1][i_3][i_0][0] * J[i_3][i_4+1] - B[i_0][i_4-1][i_0] - 5;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_N-2; i_5++) {
        for (int i_6 = 2; i_6 < min(-i_5+PB_L+1, i_5); i_6++) {
            for (int i_7 = i_6; i_7 < min(PB_M, -i_5+PB_L); i_7++) {
                B[i_5+2][i_6-2][i_5+2] = F[i_7][i_5][i_6+1] * K[i_6][0] * E[i_6][i_7+i_5][i_5] * E[i_7][i_5+1][i_6+1] - F[i_5-1][i_7][i_5] + F[i_6][i_6+i_5-1][i_5] - 6;
            }
        }
        D[i_5] = L[i_5] - K[i_5][i_5+1] * M[i_5+1] * N[i_5] + M[i_5] + D[i_5-1] - 1;
    }
#pragma endscop
...
kernel_3416351125_09(xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N));
