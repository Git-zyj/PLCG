...
void kernel_3216312225_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, PB_L-2); i_2++) {
                A[i_0+1][i_2][i_2] = C[i_2+1][i_0-2][i_1] * D[i_2][i_1] * A[i_0+1][i_2+2][i_2+2] - A[i_0-1][i_2][i_2] * 3;
            }
        }
        for (int i_3 = i_0; i_3 < PB_M-3; i_3++) {
            for (int i_4 = 2; i_4 < min(PB_L+2, i_3); i_4++) {
                A[i_3+3][i_4-2][i_4-2] = 5;
            }
        }
    }
#pragma endscop
...
kernel_3216312225_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
