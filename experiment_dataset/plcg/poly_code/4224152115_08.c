...
void kernel_4224152115_08(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd)){
        ...
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_Q+1, -i_0+PB_P); i_2++) {
                for (int i_3 = 2; i_3 < min(-i_1+PB_Q+2, PB_M-2); i_3++) {
                    B[i_3+2][i_1][i_2][i_2+i_1-1] = C[i_2-1][i_1][i_3] - D[i_0+2][i_3-2][i_1-1][i_2-1] * D[i_0-2][i_2+2][i_1][0] - D[i_1][i_0][i_2][i_3+i_1-2] - D[i_1-1][i_2+i_0][i_3][i_1-1] - D[i_1][i_2][i_0][i_3] - 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_Q; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_M-2); i_5++) {
            for (int i_6 = 2; i_6 < min(PB_Q, -i_5+PB_L+1); i_6++) {
                for (int i_7 = 2; i_7 < min(-i_6+PB_Q+1, -i_4+PB_Q); i_7++) {
                    B[i_5+2][i_6][i_7][i_7+i_6-1] = C[i_7-2][i_5][i_6-2] + D[0][i_6][i_4-1][i_5] + D[i_6+i_5-1][i_7+i_4-1][0][i_4-2] - B[i_5+2][i_4+1][i_7][i_7+i_4] - 1;
                }
            }
        }
    }
...
kernel_4224152115_08(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D));
