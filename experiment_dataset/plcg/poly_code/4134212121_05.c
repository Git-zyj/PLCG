...
void kernel_4134212121_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 1; i_1 < min(i_0, PB_M-2); i_1++) {
            for (int i_2 = max(1, i_1); i_2 < PB_M-4; i_2++) {
                for (int i_3 = 0; i_3 < PB_P; i_3++) {
                    A[i_1+2][i_2-1][i_1][i_3] = B[i_2-1][i_3][i_1+1] - A[i_2+4][i_1-1][i_2+2][i_3] * 4;
                }
            }
        }
    }
...
kernel_4134212121_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
