...
void kernel_4426312111_05(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
        ...
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_P-2, i_1); i_2++) {
                for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
                    B[i_1][i_2+i_0-2][i_3][i_3] = E[i_1+1][i_0+2][i_3+2][i_2+i_0+2] * F[i_2][i_3][i_0][i_1] * 4;
                }
            }
        }
    }
        ...
        for (int i_6 = 0; i_6 < PB_P; i_6++) {
            for (int i_7 = 2; i_7 < PB_Q; i_7++) {
                for (int i_8 = 2; i_8 < i_7; i_8++) {
                    D[i_7+1][i_8-2][i_6][i_7-1] = F[i_6][i_7][i_7-2][i_5] + 5;
                }
            }
        }
    }
...
init_array(xc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4426312111_05(xc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F)));
