...
void kernel_3224152323_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = PB_L-2 - 1; i_1 >= max(2, i_0); i_1--) {
            A[i_1][0] = C[i_1][i_0] * C[i_0+1][i_1+2] * A[i_1+2][0] + A[i_1+1][0] * A[i_1-2][0] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-4; i_2++) {
        B[i_2+2][i_2] = D[i_2] + D[i_2+2] * D[i_2] + D[i_2-2] + D[i_2-1] * D[i_2-2] * D[0] + D[0] * B[i_2+3][i_2+1] + B[i_2+4][i_2+2] + 2;
    }
#pragma endscop
...
kernel_3224152323_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
