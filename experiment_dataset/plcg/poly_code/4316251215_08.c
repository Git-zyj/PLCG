...
void kernel_4316251215_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 0; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = i_0; i_2 < PB_P-1; i_2++) {
                for (int i_3 = 1; i_3 < PB_L; i_3++) {
                    A[i_0][i_2][i_1+1][i_3-1] = D[i_1][i_3][i_2-1][i_0-1] * A[i_0][i_2+1][i_1+1][i_3-1] * A[i_0][i_2][i_1+2][i_3-1] + 1;
                }
            }
        }
        B[i_0] = E[i_0] - F[i_0] + E[i_0-1] * F[i_0] * G[i_0+1][i_0] - F[i_0] * F[i_0-1] + F[i_0] * E[i_0] + F[1] - B[i_0-1] * B[i_0+1] * 6;
    }
    for (int i_4 = 0; i_4 < PB_Q-1; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < PB_M; i_5++) {
            B[i_5] = G[i_5][i_4+1] * G[i_5-1][i_4+1] - G[i_5][i_4] - G[i_5][i_4+1] + 6;
        }
    }
#pragma endscop
...
kernel_4316251215_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
