...
void kernel_4226311215_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_L-1); i_1++) {
            for (int i_2 = -i_1+1; i_2 < -i_0+PB_L-1; i_2++) {
                A[i_2+i_1][i_2+i_1] = A[i_2+i_1-1][i_2+i_1-1] - A[i_1+i_0+1][i_1+i_0+1] - A[i_2+i_0+1][i_2+i_0+1] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N+1; i_3++) {
        B[i_3+1] = C[i_3-1][i_3] * D[i_3-1] * 3;
    }
#pragma endscop
...
kernel_4226311215_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
