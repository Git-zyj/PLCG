...
void kernel_3236112323_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_L+4; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                B[i_0-4][i_2+2][i_1+1] = 2;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_L-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M; i_4++) {
            for (int i_5 = 2; i_5 < -i_4+PB_M; i_5++) {
                B[i_3-2][i_4][i_5+1] = C[i_5+i_4][i_3+1][2] * C[i_5+i_4][i_3][i_4] - B[i_5-2][i_4+1][i_3+1] - B[i_3-2][i_4+1][i_5+1] * B[i_3-3][i_5][i_4] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3236112323_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
