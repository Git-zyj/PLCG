...
void kernel_4422131215_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 1; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_Q+1; i_3++) {
                    A[i_1+1][i_3][i_2][i_0] = E[i_2-1][i_0][i_3][0] * E[i_3][i_0+1][i_1-1][i_2] + E[0][i_3-1][i_1-1][i_3+i_2-1] + 3;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            D[i_0] = F[i_4+1][i_0] - F[i_0-1][1] - 3;
            D[i_4+2] = G[i_4-1][i_4+1][i_0] - F[i_0][i_4] * D[i_4+2] - 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
        D[i_5+1] = H[i_5-1] * H[i_5-1] + H[i_5] + D[i_5] - D[i_5+2] - 6;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422131215_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
