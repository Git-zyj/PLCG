...
void kernel_4316131223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-3; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-2, i_0); i_1++) {
            A[i_1+i_0][i_0+1] = D[i_1][i_0] + D[i_1][i_0] * D[i_0-1][i_0] + D[i_0-1][i_1+1] * A[i_1+i_0+2][i_0+3] - A[i_1+i_0+1][i_0+2] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            B[i_3] = D[i_3][i_2+1] * D[i_3][i_2-1] + B[i_3-2] - 3;
        }
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            C[i_4][i_2][1] = D[i_4][i_2-1] - D[i_2][i_4] * C[i_4][i_2+2][1] + 6;
        }
    }
#pragma endscop
...
kernel_4316131223_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
