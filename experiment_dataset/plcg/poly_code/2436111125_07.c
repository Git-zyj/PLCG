...
void kernel_2436111125_07(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        C[i_0-1] = E[i_0] + E[i_0] + 3;
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            B[i_1+i_0-1][i_1+i_0] = 4;
        }
    }
    for (int i_2 = PB_L - 1; i_2 >= 1; i_2--) {
        for (int i_3 = 0; i_3 < -i_2+PB_L; i_3++) {
            C[i_3] = B[i_3+i_2-1][i_3+i_2] * 6;
        }
        C[i_2] = E[i_2+1] * 1;
    }
#pragma endscop
...
kernel_2436111125_07(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E));
