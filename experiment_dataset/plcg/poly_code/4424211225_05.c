...
void kernel_4424211225_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
    ...
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        ...
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                C[i_3+1][i_3+3] = A[i_3][i_0][i_4] - 1;
            }
            C[i_0-1][i_0+1] = E[i_3][i_0][i_3] * C[i_0-3][i_0-1] + 5;
        }
    }
    for (int i_5 = 3; i_5 < PB_M-1; i_5++) {
        D[i_5-1] = D[i_5-3] + D[i_5] * D[i_5+1] - 1;
    }
#pragma endscop
...
kernel_4424211225_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
