...
void kernel_4434152225_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 2; i_2 < i_1; i_2++) {
                for (int i_3 = 4; i_3 < -i_2+PB_L-2; i_3++) {
                    A[i_1+i_0][i_3-2][i_2-2][i_3] = E[i_3][i_0+1][i_2+i_1+1][1] * E[i_2][i_3-2][i_1][i_0] + E[i_1][i_3+i_2+2][i_0][i_0] * E[i_3+i_2][i_2][i_0+2][i_1] + A[i_1+i_0][i_3-4][i_2-2][i_3-2] * 5;
                }
            }
        }
        for (int i_4 = max(3, i_0); i_4 < -i_0+PB_M; i_4++) {
            B[i_4-2][i_0] = F[i_0][i_4-1] * F[i_4+i_0][i_0-2] + B[i_4-3][i_0] + B[i_4-1][i_0] + B[i_4-2][i_0-2] * 4;
        }
    }
    for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 2; i_6 < PB_L-2; i_6++) {
            for (int i_7 = 2; i_7 < PB_L; i_7++) {
                for (int i_8 = 1; i_8 < min(min(-i_5+PB_M-2, -i_7+PB_L), i_5); i_8++) {
                    A[i_8+i_5+1][i_7-2][i_6-2][i_7] = G[i_8-1][i_7-1][i_6] - E[i_7][i_8][i_6-2][i_5] - G[i_6-1][i_8+1][i_7] * E[i_7][i_5][i_6][i_8] + G[i_8][i_7][i_6+2] + E[i_7][i_8+2][i_5][i_6] * E[i_8+i_5+2][i_8-1][i_6+2][i_7] - E[i_6][i_8+i_7][i_5][i_8] + 1;
                }
            }
        }
    }
    for (int i_9 = 1; i_9 < PB_L; i_9++) {
        for (int j_0 = 2; j_0 < i_9; j_0++) {
            B[j_0-1][i_9-1] = F[j_0][i_9] - H[i_9] + F[j_0-2][i_9] * 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434152225_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
