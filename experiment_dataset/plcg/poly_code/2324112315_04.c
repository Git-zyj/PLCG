...
void kernel_2324112315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        A[i_1] = D[i_1+1] * D[i_1] - A[i_1] * 4;
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-3; i_3++) {
            C[i_2][i_3+2] = C[i_2][i_3+1] - C[i_2-1][i_3+3] - C[i_2+1][i_3+2] - C[i_2-1][i_3+1] - 5;
        }
    }
#pragma endscop
...
kernel_2324112315_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
