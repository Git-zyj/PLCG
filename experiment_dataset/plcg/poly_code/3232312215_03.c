...
void kernel_3232312215_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = PB_Q-1 - 1; i_1 >= 0; i_1--) {
            for (int i_2 = 0; i_2 < PB_Q-2; i_2++) {
                B[i_0][i_1][i_2+2] = C[i_2][i_1+1] - 3;
            }
        }
        for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_Q-3; i_4++) {
                B[i_0][i_3][i_4+2] = B[i_0-1][i_4][i_3+2] * B[i_0+1][i_3][i_4+3] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3232312215_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
