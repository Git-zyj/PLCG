...
void kernel_3326212315_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            B[i_0] = D[i_1][i_0] + B[i_1+1] - B[i_0] * 4;
        }
    ...
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
                C[i_4][i_3+i_2][i_3-2] = C[i_4][i_3+i_2+1][i_3-1] * C[i_4+1][i_3+i_2][i_3-2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3326212315_09(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
