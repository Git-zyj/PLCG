...
void kernel_2434231313_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            A[i_1][i_0][i_1] = E[i_1] - F[i_0][0] - G[i_1][i_0] * E[i_1] + G[i_1+1][i_0] * A[i_1+1][i_0][i_1+1] + A[i_1-1][i_0][i_1-1] - A[i_1][i_0-1][i_1] * A[i_1][i_0+1][i_1] - A[i_1-1][i_0+1][i_1-1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[i_3][i_2-1] = G[i_3][i_2] + E[i_2] * F[i_3+1][i_2-1] + B[i_3-1][i_2-1] - 1;
        }
        for (int i_4 = i_2; i_4 < PB_N-1; i_4++) {
            C[i_4] = E[i_4] - C[i_4-1] + C[i_4+1] - 5;
        }
        for (int i_5 = 1; i_5 < PB_N; i_5++) {
            D[i_2][i_5] = H[i_5] * G[i_5][i_5] * C[i_5-1] * D[i_5][i_2-1] * D[i_2][i_5+1] * 5;
        }
    }
#pragma endscop
...
kernel_2434231313_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
