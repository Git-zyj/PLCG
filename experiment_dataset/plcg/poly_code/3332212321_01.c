...
void kernel_3332212321_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            A[i_0][i_0] = C[i_1] * C[i_1-2] * C[i_1+1] + C[i_1-1] - C[i_1+2] * C[i_1-2] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_M+2; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M+2; i_3++) {
            for (int i_4 = 1; i_4 < min(min(-i_2+PB_L+1, PB_M+1), -i_3+PB_M); i_4++) {
                B[i_4+i_3][i_3+i_2] = D[i_3+i_2-2][i_4-1][i_4] - B[i_4+i_3-1][i_4+i_2-1] + 1;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
        C[i_5] = A[i_5][i_5] - A[i_5+1][i_5+1] * 4;
    }
#pragma endscop
...
kernel_3332212321_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
