...
void kernel_3336251325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0+1] = D[i_0] * D[i_0] - D[i_0] - E[i_0] * D[i_0-1] * A[i_0+3] + 6;
    }
    for (int i_1 = 1; i_1 < PB_Q; i_1++) {
        for (int i_2 = 1; i_2 < min(i_1, PB_M-1); i_2++) {
            for (int i_3 = 1; i_3 < PB_M; i_3++) {
                A[i_3+1] = F[i_3][i_1-1][i_2] - G[i_2][i_1-1] + H[i_2+1][i_3][i_1-1] + F[i_3][i_3-1][i_1] * H[i_2-1][i_3][i_1] * H[i_3][i_2+1][i_1] * 1;
            }
            A[i_2+3] = D[i_2] * G[i_2][i_1] + A[i_2+1] - A[i_2+3] - 6;
        }
    }
#pragma endscop
...
kernel_3336251325_04(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
