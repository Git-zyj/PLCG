...
void kernel_2124251223_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_L; i_1++) {
            A[i_1][i_1+1] = B[i_1][i_0-1] - C[i_1][i_0][1] + D[i_1] + E[i_0][i_1+i_0] - E[i_1][i_0] * A[i_1-2][i_1-1] + A[i_1+2][i_1+3] - 1;
        }
    }
#pragma endscop
...
kernel_2124251223_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
