...
void kernel_4414311325_05(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            D[i_1-2][0][i_0+1] = C[i_0+1] - 1;
            D[i_0-1][0][i_1] = 3;
        }
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 1; i_4 < PB_P-3; i_4++) {
                D[i_3-1][0][i_4+1] = D[i_3-1][0][i_4+3] * D[i_3][0][i_4] - D[i_3+1][0][i_4-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4414311325_05(xd,yd,zd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E));
