...
void kernel_3332211223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
            C[i_4][i_5] = D[i_4] - C[i_4][i_5+2] - C[i_4-1][i_5-2] + C[i_4][i_5-1] * 3;
        }
    }
#pragma endscop
...
kernel_3332211223_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
