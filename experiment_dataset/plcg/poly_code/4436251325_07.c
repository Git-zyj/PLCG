...
void kernel_4436251325_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M-1; i_2++) {
                A[i_1+i_0][i_2][i_2] = E[i_1-1][i_0-1][0] - E[i_2+1][i_2+i_0][i_1] - F[i_2+i_0+1][i_1-1][i_1] - E[i_0][i_1-1][i_1+1] + F[i_1][i_2][i_0] + F[i_1][i_0-1][0] + F[i_0][i_2][i_1] + E[i_1][i_2][i_0] - A[i_1+i_0][i_2+1][i_2+1] * A[i_1+i_0][i_2-1][i_2-1] + A[i_1+i_0-2][i_2+2][i_2+2] - 5;
            }
        }
        B[i_0] = G[i_0] + H[i_0] + H[i_0+1] * G[i_0-1] * B[i_0-1] + B[i_0-2] * 6;
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            B[i_3+2] = I[i_3][i_4] * J[i_4][i_3] + I[i_3][i_4+1] + B[i_4] * 5;
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = i_5; i_6 < -i_5+PB_M; i_6++) {
            for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
                B[i_7+1] = E[i_6+i_5][i_7][i_6] * I[i_7-1][i_6+i_5+1] * E[i_6][i_7][i_5] - F[i_7-1][i_5+1][i_5] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4436251325_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
