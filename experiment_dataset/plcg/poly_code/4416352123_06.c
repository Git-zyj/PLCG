...
void kernel_4416352123_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0+1] = E[i_0+1] - E[i_0] - F[i_0-1] * F[i_0] - E[i_0] + E[i_0] * F[i_0+2] * A[i_0] + 6;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        B[i_1] = 1;
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            D[i_2] = F[i_1] * G[i_1][0] + G[i_1+1][i_2] + G[i_1][i_2+2] + 1;
        }
        D[i_1] = H[i_1] - H[i_1+2] + F[0] * H[i_1-2] - F[i_1] + F[i_1+2] * H[i_1] - H[i_1+2] - 4;
    }
#pragma endscop
...
kernel_4416352123_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
