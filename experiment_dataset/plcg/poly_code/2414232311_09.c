...
void kernel_2414232311_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        A[i_0-2] = E[i_0] * F[i_0-1] * F[i_0] * A[i_0-3] + 2;
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            B[i_0][i_1] = G[i_0][i_1] - A[i_1-2] + A[i_0-2] * A[i_0-3] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            D[i_2+2] = H[i_2][i_3] * H[i_3+1][i_3] * H[i_3-1][i_2+2] * G[i_3][i_2] - H[i_3-2][i_2] * B[i_3][i_2] + B[i_3][i_2+1] - 1;
        }
...
kernel_2414232311_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
