...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            A[i_2+2] = E[i_1] - F[i_1][i_2] * 5;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M, PB_L-1); i_4++) {
            A[i_4+1] = F[i_3][i_4-2] - G[i_4+i_3][i_3] - H[i_4][i_4][0] + 6;
        }
    }
#pragma endscop
...
kernel_2324232115_07(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
