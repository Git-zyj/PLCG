...
void kernel_3436152125_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+2] = E[i_0] * E[i_0] - 5;
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = min(PB_M, -i_1+PB_L+1) - 1; i_2 >= 2; i_2--) {
                B[i_0][i_2][i_2] = F[i_0][i_1][0] + F[i_1][i_2-2][i_0+2] - F[i_1-2][i_0][i_2-1] + F[i_1+i_0+2][i_0+1][i_2+i_1-1] - 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 2; i_5 < -i_4+PB_M; i_5++) {
                C[i_4-1][i_3][i_5+1] = F[i_3][i_5+i_4][0] - F[i_4+1][i_3][i_5] - F[i_4][i_5][i_3] * G[i_5][i_4] * F[i_4][i_3+1][i_3] + F[i_4][i_3][i_5-2] * F[i_4+2][i_3-2][i_5] * F[i_5][i_3][i_4] - F[i_5][2][i_3] * C[i_4+1][i_3-1][i_5+1] - 2;
            }
        }
        for (int i_6 = i_3; i_6 < PB_M; i_6++) {
            for (int i_7 = 0; i_7 < min(i_6, PB_P-1); i_7++) {
                C[i_3-1][i_6+1][i_7+1] = F[i_7][1][i_7] * H[i_7][i_3+1][i_6][0] - G[i_7][i_6] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3436152125_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
