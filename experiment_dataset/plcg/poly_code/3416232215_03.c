...
void kernel_3416232215_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            B[i_0-1][i_0+1] = E[i_1] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
        B[i_2-1][i_2+1] = E[i_2] + F[i_2] * G[i_2-1][i_2-2] + B[i_2][i_2+2] * 1;
        for (int i_3 = i_2; i_3 < min(-i_2+PB_L, PB_P); i_3++) {
            for (int i_4 = i_2; i_4 < PB_L; i_4++) {
                B[i_3-1][i_3+1] = H[i_4][i_2-2][i_3-2] - H[i_4][i_2][i_2+2] * H[i_4][i_3][i_2] + D[i_4-1][i_4][i_3][i_2] - 4;
            }
            for (int i_5 = 1; i_5 < PB_P; i_5++) {
                D[i_2-1][i_2][i_3][i_5] = H[i_5-1][i_2+2][i_3] + I[i_3-1][i_2-2][i_3] * I[i_5+1][i_3+i_2][i_5] * D[i_3-1][i_3][i_2+1][i_5] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3416232215_03(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
