...
void kernel_2436332315_07(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < -i_0+PB_M-2; i_1++) {
            C[i_1+i_0+2][i_1-1] = E[0] * F[i_1][i_0] * 5;
        }
    }
    ...
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M-2, i_3); i_4++) {
            C[i_4+i_3+1][i_3-1] = G[i_3][i_4+2] * G[i_4][i_3] + F[i_4+i_3][i_4-1] * C[i_4+i_3+2][i_3-1] + C[i_4+i_3][i_3-1] - C[i_4+i_3+2][i_3] * 6;
        }
        for (int i_5 = 0; i_5 < min(-i_3+PB_M, i_3); i_5++) {
            B[i_5] = H[i_3] + I[i_5+i_3][i_3+2] + F[i_5][i_3] * G[i_5+i_3][0] + G[i_5][i_3] + 4;
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2436332315_07(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
