...
void kernel_4124232115_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+2; i_0++) {
        for (int i_1 = 2; i_1 < min(min(PB_N, -i_0+PB_L), i_0); i_1++) {
            A[i_1+i_0] = B[i_1+2] * C[i_1][i_0-2] - D[i_1-2] - A[i_1+i_0-1] - 6;
        }
    }
#pragma endscop
...
kernel_4124232115_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
