...
void kernel_2326112323_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_L+2, -i_0+PB_M-2); i_1++) {
            A[i_1+i_0][i_1-2] = A[i_1+i_0+1][i_1-2] * A[i_1+i_0-2][i_1-2] * A[i_1+i_0+2][i_1-2] - 6;
        }
        for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
            B[i_2] = B[i_2-2] * B[i_2+2] - 2;
        }
        for (int i_3 = 4; i_3 < min(min(-i_0+PB_M+2, i_0), PB_L-1); i_3++) {
            C[i_0-1][i_3] = D[i_0] + A[i_3+i_0-2][i_3-4] - C[i_0-1][i_3+1] + 1;
        }
    }
#pragma endscop
...
kernel_2326112323_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
