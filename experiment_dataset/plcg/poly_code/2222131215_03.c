...
void kernel_2222131215_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0-1][i_1-1] = C[i_1][0] + C[i_1+1][i_0] - D[i_0+1] + C[i_1+i_0][i_1] - B[i_1-1] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[i_3] = C[i_2][i_3] * D[i_3] + B[i_3+1] + B[i_3-1] - B[i_3+1] + 1;
        }
    }
#pragma endscop
...
kernel_2222131215_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
