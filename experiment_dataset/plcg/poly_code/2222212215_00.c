...
void kernel_2222212215_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_M-1; i_2++) {
            B[i_2+i_1][i_2+1] = C[i_1][i_1] * B[i_2+i_1+1][i_1+1] + B[i_2+i_1-1][i_2+1] * B[i_2+i_1-1][i_1+1] + 6;
        }
    }
#pragma endscop
...
kernel_2222212215_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
