...
static void print_array(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh))
{
int i_0, i_1, i_2, i_3;
...
void kernel_3414152215_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_M-2, i_0), -i_1+PB_L); i_2++) {
                A[i_1][0] = E[i_2+i_0+2][i_2+i_1][i_0-2][i_1] + F[i_1][i_0][i_2+2] - F[i_1][i_2+i_0][i_0] * F[i_1][i_0][i_0] - F[i_0][i_1][i_2] - F[i_0][i_2][0] * A[i_1+1][0] * 2;
            }
        }
        for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                A[i_3+1][0] = G[i_3+2][i_0-1] + G[i_4-1][i_3] * A[i_4][0] - 4;
            }
            A[i_3][0] = G[i_3][i_0-2] + G[i_3-1][i_0+1] - G[i_0+1][i_3-2] - G[i_0-1][i_3+2] - G[i_3][i_0] * H[i_3] - 3;
        }
    }
    for (int i_5 = 2; i_5 < PB_L; i_5++) {
        for (int i_6 = 0; i_6 < -i_5+PB_M; i_6++) {
            for (int i_7 = i_5; i_7 < PB_L; i_7++) {
                A[i_7][0] = G[i_7][i_6] * F[i_6+i_5][i_7][0] - F[i_7+i_6][i_6+i_5][i_5-1] + F[i_6][i_5-2][i_7-2] * 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3414152215_07(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
