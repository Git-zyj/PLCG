...
void kernel_4432152313_07(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = i_0; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_M; i_3++) {
                    B[i_1][i_3][i_2][i_0+1] = E[i_2][i_3][i_2+2][i_3][i_0] * F[i_2][i_3+1][i_1][i_0] * G[i_3][i_1][i_2] * F[i_3+i_1][i_0][i_0+2][i_2] - F[i_3][i_1][i_0][i_0+2] - 1;
                }
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_P-1; i_6++) {
                for (int i_7 = i_4; i_7 < PB_M-1; i_7++) {
                    B[i_7][i_5][i_6][i_4+1] = B[i_7+1][i_5][i_6][i_4+1] + B[i_7][i_5][i_6+1][i_4+1] - B[i_7][i_4-1][i_6][i_5+1] * B[i_7][i_5+1][i_6-1][i_4+1] + 1;
                }
            }
        }
    }
    for (int i_8 = 2; i_8 < PB_M-1; i_8++) {
        for (int i_9 = 2; i_9 < PB_M; i_9++) {
            D[i_8] = H[i_9] * I[i_9][i_8-1] * I[i_9][i_8] + I[i_9][i_8-1] + I[i_9+1][i_9-2] * I[i_8][i_9] - 1;
        }
        D[i_8] = I[i_8+1][0] - H[i_8] - H[i_8] - H[i_8] + H[i_8] * H[i_8-2] * H[i_8] * H[i_8] * D[i_8+1] * 1;
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4432152313_07(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
