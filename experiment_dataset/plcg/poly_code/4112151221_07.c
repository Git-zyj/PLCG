...
void kernel_4112151221_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-1 - 1; i_0 >= 1; i_0--) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M+1, PB_M-1); i_1++) {
            A[i_1][0] = B[i_1+i_0] * C[i_0+1][i_1] + C[i_0+1][i_1] * C[i_1+1][i_0] * C[i_1+i_0-1][i_0-1] * A[i_1-2][0] - A[i_1-1][0] * 1;
        }
    }
#pragma endscop
...
kernel_4112151221_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
