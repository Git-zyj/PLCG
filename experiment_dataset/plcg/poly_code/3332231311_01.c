...
void kernel_3332231311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        B[i_1+1] = E[i_1] * D[i_1] + D[i_1+1] * D[i_1] * B[i_1] * 6;
    }
    for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_2+PB_M+1, -i_3+PB_M-1); i_4++) {
                C[i_4+i_3][i_3][i_2] = F[i_4][i_3] * G[i_2][i_4][i_3+1] - C[i_4+i_3-1][i_3][i_2] - C[i_4+i_3+1][i_3][i_2] * C[i_4+i_3+1][i_4][i_2-1] + C[i_4+i_3-1][i_3][i_2+1] + C[i_4+i_3+1][i_3+1][i_2] + C[i_4+i_2-1][i_2-1][i_3] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3332231311_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
