...
void kernel_3224212125_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L-3, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_2][i_2+2] = A[i_1+1][i_1+3] - 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        B[i_3+1] = C[i_3+2] + B[i_3] + 4;
    }
#pragma endscop
...
kernel_3224212125_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
