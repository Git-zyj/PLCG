...
void kernel_2216312325_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_N+2; i_1++) {
            A[i_1] = C[i_1+2] * B[i_1-2] - 2;
        }
    }
    for (int i_2 = 4; i_2 < PB_N; i_2++) {
        B[i_2-2] = B[i_2-3] * B[i_2] + B[i_2-4] * B[i_2-1] - 5;
    }
#pragma endscop
...
kernel_2216312325_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
