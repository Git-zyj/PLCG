...
void kernel_2322111323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] - D[i_0] + A[i_0+1] - A[i_0-1] * A[i_0+2] * 5;
    }
    ...
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            C[i_3][i_2] = C[i_3][i_2+1] * C[i_3-2][i_2] + C[i_2+1][i_3] * 3;
        }
    }
#pragma endscop
...
kernel_2322111323_00(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
