...
void kernel_3222351311_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1][i_0-1] = C[i_0+1][i_1] + D[i_1][i_0+1] * E[1] * D[i_1+1][i_0+1] + F[i_1][i_0+1][0] - F[i_1][i_1][i_0] * A[i_1-1][i_0-1] * 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
            for (int i_4 = i_2; i_4 < PB_L; i_4++) {
                B[i_2-1][i_3][i_4] = F[i_4][i_3-1][i_2] - G[i_2-1][i_4][i_3+1] + F[i_2+1][i_3-1][i_2] * B[i_2-2][i_3][i_4] + B[i_2][i_4][i_3+1] - B[i_2-1][i_3+1][i_4] * B[i_2][i_3][i_4] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3222351311_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
