...
void kernel_2234332111_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < min(min(PB_M, i_0), -i_0+PB_N-2); i_1++) {
            B[i_1] = C[i_0-2][i_1] * D[i_1][i_0+1] * C[i_1+i_0+2][i_1] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
            B[i_3] = C[i_2+1][i_3] * E[i_3+i_2][i_2+1][0] + C[i_3-1][i_2-2] - B[i_3+1] + 4;
        }
    }
#pragma endscop
...
kernel_2234332111_07(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
