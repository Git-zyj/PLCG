...
void kernel_2236312321_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        ...
        for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
            B[i_0] = C[i_2][i_0-1] * B[i_2+2] * B[i_2-2] + B[i_0+1] - B[i_2-1] + B[i_2+1] - 6;
        }
    }
#pragma endscop
...
kernel_2236312321_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
