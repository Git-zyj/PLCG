...
void kernel_3212111221_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = PB_Q+2 - 1; i_2 >= 2; i_2--) {
                A[i_0][i_2][i_1] = A[i_0-2][i_2][i_1] - A[i_1][i_0+1][i_2-2] + 1;
            }
            B[i_1-1] = C[i_0][i_1] - C[i_1][i_0] * B[i_1] * 5;
        }
    }
#pragma endscop
...
kernel_3212111221_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
