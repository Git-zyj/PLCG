...
void kernel_4416351121_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0] * E[i_0-1] * F[i_0-1] - 5;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_P); i_2++) {
            for (int i_3 = i_1; i_3 < min(PB_P, -i_1+PB_Q+1); i_3++) {
                for (int i_4 = 0; i_4 < i_3; i_4++) {
                    B[i_3+1][i_2][i_2][i_4] = G[i_2-1][i_1+1][i_3][i_4] - H[i_3-1][i_1+1][i_4+i_2][i_2] - G[i_2][i_1][i_3][i_3+i_1-1] - I[i_2][i_4][i_2][i_4] + H[i_4+i_2][i_1][i_3][0] - 4;
                }
            }
            C[i_2+i_1][0] = F[i_1] + J[i_1+1][i_2+i_1-1] * K[i_2][i_1] + L[i_2][i_1] - J[i_1][i_2+1] + K[i_1][i_2] * D[i_1+1] * 2;
        }
        D[i_1+1] = E[i_1] - M[i_1-1] + M[i_1] + 3;
    }
#pragma endscop
