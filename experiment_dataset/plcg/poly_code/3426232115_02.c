...
void kernel_3426232115_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q+2; i_1++) {
            for (int i_2 = 2; i_2 < min(min(PB_N, -i_0+PB_M), i_1); i_2++) {
                A[i_2][i_1][i_0+2] = E[i_0-2][i_2+2][i_1-2] + F[i_2-1][i_2+i_0] - G[i_2][i_1][i_0] - E[i_2][i_1][i_0+1] + H[i_2-2][i_1-1][i_2][i_0] * 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_Q+2; i_4++) {
            for (int i_5 = 2; i_5 < min(min(i_4, PB_Q), -i_4+PB_M-1); i_5++) {
                A[i_5-1][i_4][i_3+2] = G[i_5-1][i_4+2][i_3] * E[i_3][i_3][i_4-2] + G[i_5-2][i_3+2][0] * E[i_5+i_4+1][i_3-1][i_5] - A[i_4][i_5][i_3+2] * 5;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_N; i_6++) {
        for (int i_7 = 0; i_7 < PB_Q-2; i_7++) {
            for (int i_8 = 0; i_8 < min(i_7, -i_6+PB_N); i_8++) {
                A[i_6][i_8][i_7+2] = G[i_8+i_6][i_7][i_6] + I[i_8][i_7] * 4;
            }
        }
        D[i_6] = 5;
    }
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3426232115_02(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
