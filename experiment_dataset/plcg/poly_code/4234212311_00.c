...
void kernel_4234212311_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_1+i_0-2][1] = A[i_1+i_0-3][1] + A[i_1+i_0][1] * A[i_1+i_0-3][1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_P-1; i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
                B[i_2][i_4+i_3-1][i_3] = C[i_4][i_2+2][i_3] * B[i_2][i_4+i_3-2][i_3-1] - B[i_2-1][i_4+i_3][i_3+1] - B[i_2-1][i_4+i_3][i_3] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4234212311_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
