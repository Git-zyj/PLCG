...
void kernel_2426351211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_0-1][i_0] = E[i_1] * F[i_1][i_0+1] + F[i_1][1] * F[i_1][i_0] * A[i_0][i_0+1] * A[i_0-2][i_0-1] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-2; i_3++) {
            B[i_3+1] = G[i_3-1][i_2] * E[i_3] + H[i_2][i_3-1] * F[i_2-1][i_3-1] - B[i_3+2] * 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        D[i_4] = E[i_4+1] - E[i_4] - E[i_4] * I[i_4] * H[i_4][1] - B[i_4] - 6;
        D[i_4] = J[i_4+1] * E[i_4] + I[i_4] + J[i_4+1] * J[i_4] + I[i_4] * D[i_4-1] - D[i_4+1] - 3;
    }
#pragma endscop
...
kernel_2426351211_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
