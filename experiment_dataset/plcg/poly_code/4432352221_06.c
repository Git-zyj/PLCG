...
void kernel_4432352221_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + E[0] + F[i_0+2] - A[i_0-1] - A[i_0+1] * A[i_0+2] * 4;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
            B[i_2] = E[1] - F[i_2] + G[i_2-1][i_1-2] - H[i_1+1][i_2] + H[i_1][i_2+2] - I[i_2-2][i_1+1] * I[i_2][i_2] * 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        C[i_3] = F[i_3-2] * J[i_3-2] * F[i_3] - A[i_3] * C[i_3+2] + 2;
        for (int i_4 = 0; i_4 < -i_3+PB_M; i_4++) {
            D[0][i_3] = H[i_4+i_3][i_3] * I[i_3][i_4] * I[i_4][i_3] * J[i_3] - G[i_3+2][i_4] + I[i_4][i_3] - G[i_4][i_3-1] + A[i_4] + C[i_4] - D[0][i_4+2] + 1;
        }
    }
#pragma endscop
...
kernel_4432352221_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
