...
void kernel_4436212325_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_Q-2, -i_2+PB_M-2); i_3++) {
                    A[i_1+2][i_3+i_2-2][i_0][i_3+i_0-2] = E[i_1][i_3+i_2-1][i_0][i_3+i_0+2] - D[i_3+i_2+2][i_2+i_0][i_1][0] + 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-4; i_5++) {
            for (int i_6 = 3; i_6 < min(i_4, -i_5+PB_P-2); i_6++) {
                for (int i_7 = 0; i_7 < min(PB_L-2, -i_4+PB_L+2); i_7++) {
                    B[i_7][i_6-1][i_6+i_5][i_4] = A[i_5+4][i_7+i_4-2][i_6][i_7+i_6-2] * B[i_7][i_6-3][i_6+i_5-2][i_4+1] * B[i_4][i_6][i_6+i_5+1][i_7] * B[i_7+2][i_5-1][i_6+i_5+2][i_4] - 6;
                }
            }
        }
    }
        ...
        for (int i_9 = 1; i_9 < PB_L-1; i_9++) {
            for (int j_0 = 0; j_0 < -i_9+PB_L-1; j_0++) {
                for (int j_1 = 1; j_1 < -j_0+PB_M-1; j_1++) {
                    D[j_1+j_0+1][j_0+i_9+1][i_8][0] = F[j_1][i_8][j_0+i_9-1] * B[i_9][j_1-1][j_1+j_0+1][i_8] * 6;
                }
            }
        }
    }
    for (int j_2 = 0; j_2 < PB_L-2; j_2++) {
        for (int j_3 = 2; j_3 < j_2; j_3++) {
            for (int j_4 = max(max(0, -j_3+2), -j_2); j_4 < min(min(-j_3+PB_L, PB_L-4), -j_2+PB_L-2); j_4++) {
                for (int j_5 = 2; j_5 < min(-j_2+PB_M-2, -j_4+PB_L-4); j_5++) {
                    D[j_5+j_4][j_4+j_2][j_3][0] = E[j_5][j_2][j_5-2][j_4] + G[j_5][j_4][j_2][j_3-1] - D[j_4+j_3-2][j_4+j_2+2][j_5][0] - D[j_5+j_2+2][j_5+j_4+4][j_3][0] * D[j_5+j_4+1][j_4+j_3][j_2][0] - 1;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4436212325_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
