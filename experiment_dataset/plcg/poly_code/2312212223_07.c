...
void kernel_2312212223_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
            A[i_0][i_1] = A[i_0+2][i_1+1] * A[i_0][i_1-2] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        C[i_2-2] = D[i_2] * C[i_2] - C[i_2-2] - 3;
        C[i_2] = 5;
    }
#pragma endscop
...
kernel_2312212223_07(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
