...
void kernel_3332312113_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-3; i_2++) {
                A[i_0][i_2+2] = A[i_1+1][i_2+3] - 5;
            }
        }
        for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
            A[i_0-1][i_3+3] = D[i_3] * 5;
        }
    }
...
kernel_3332312113_02(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
