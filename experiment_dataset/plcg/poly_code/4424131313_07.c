...
void kernel_4424131313_07(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            C[i_0+1][i_1] = E[i_0][i_1] - F[i_0] - E[1][i_0] * B[i_0-1] - 3;
        }
        B[i_0] = F[i_0] - F[i_0] + F[i_0-1] - E[i_0-1][i_0] - 3;
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            C[i_0][i_2] = E[i_0+1][i_2] * C[i_0-1][i_2] - C[i_0+1][i_2] - 5;
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_M; i_5++) {
                for (int i_6 = 1; i_6 < i_3; i_6++) {
                    D[i_5][i_3][i_4][i_6] = G[i_5+i_4][i_6+i_5+1][i_3][i_3] - G[i_5+1][i_3][i_4+1][i_6-1] * D[i_6+1][i_3+1][i_5][i_4] - D[i_5+1][i_3][i_4-1][i_6+1] - D[i_3-1][i_5-1][i_4+1][i_6] + D[i_5][i_3][i_4][i_6-1] + 3;
                }
            }
        }
    }
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4424131313_07(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
