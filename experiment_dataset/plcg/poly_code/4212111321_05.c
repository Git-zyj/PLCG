...
void kernel_4212111321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(-i_0+PB_M, PB_N); i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_0+PB_P, -i_1+PB_N), i_1); i_2++) {
                A[i_2+i_1][i_0][1] = B[i_2+i_1+1][i_0-1] * B[i_2+i_0+2][i_1] * B[i_2+i_1-2][i_0-1] * B[i_2+i_0-1][i_1-1] - 4;
                B[i_1+i_0][i_2-1] = C[i_1][i_0][i_2+i_0] + C[i_1][i_2][i_0] + A[i_2+i_1][i_0+1][1] - A[i_2+i_1][i_0][1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4212111321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
