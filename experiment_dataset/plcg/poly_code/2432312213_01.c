...
void kernel_2432312213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            A[i_1+2] = A[i_1+3] * A[i_1+1] * A[i_1+1] * 3;
        }
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            B[i_2][0] = B[i_0-1][0] - B[i_2+1][0] * B[i_2-1][0] - 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
            C[i_3] = E[i_4+2][i_3] * D[i_3] - 4;
        }
        D[i_3] = D[i_3+1] - 2;
    }
#pragma endscop
...
kernel_2432312213_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
