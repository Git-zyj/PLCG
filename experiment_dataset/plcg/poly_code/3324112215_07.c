...
void kernel_3324112215_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0+1][i_1] = A[i_0+1][i_1-1] * A[i_1+2][i_0] + A[i_0+2][i_1] + A[i_0][i_1] * 5;
        }
        for (int i_2 = 0; i_2 < min(PB_Q-2, -i_0+PB_L-1); i_2++) {
            for (int i_3 = 1; i_3 < PB_N; i_3++) {
                B[i_3][i_0][i_2+2] = D[i_2+i_0+1][i_3][i_2] + B[i_3-1][i_0][i_2+2] - B[i_3][i_0+1][i_2+2] - 6;
            }
        }
        C[i_0+2][i_0] = 2;
    }
...
kernel_3324112215_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
