...
void kernel_3112331111_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, -i_0+PB_N); i_1++) {
            A[i_1-1] = B[i_1][i_0+1] + C[i_1+i_0][i_1] * C[i_1+i_0][i_1] * A[i_1] + 1;
        }
    }
#pragma endscop
...
kernel_3112331111_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
