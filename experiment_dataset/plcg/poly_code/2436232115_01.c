...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        A[i_1+1] = E[i_1] - F[i_1] - F[i_1] - 5;
        A[i_1] = E[i_1] * F[i_1+1] + G[i_1-1][i_1] - F[i_1] + 2;
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < min(-i_2+PB_L, PB_M-2); i_3++) {
            D[i_2+1][i_3-1] = H[i_3+2][i_2] * I[i_2+1][i_3+i_2][i_2] - 6;
        }
    }
#pragma endscop
...
kernel_2436232115_01(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
