...
void kernel_2234152221_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0][i_0] = C[i_0] + C[i_0+2] - C[i_0] + C[i_0] * C[i_0] + D[i_0][i_0+1] + A[i_0+2][i_0+2] * 2;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            B[i_1] = D[i_2+i_1][0] * D[i_1][i_2+2] + D[i_2+i_1][1] * B[i_2+2] + B[i_2+1] - B[i_1-2] + 6;
        }
    }
#pragma endscop
...
kernel_2234152221_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
