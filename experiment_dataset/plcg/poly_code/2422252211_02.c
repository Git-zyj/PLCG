...
void kernel_2422252211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + F[i_0] + E[0] - 6;
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_1] = F[i_1-1] - G[i_1][i_0] * A[i_1] + A[i_1-1] * 6;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
            A[i_0] = E[i_2+i_0] + H[i_2-1][i_0-1] * H[i_2+i_0][i_0] * H[i_2][i_0+2] + H[i_2][i_0+2] - E[i_2+i_0] + H[i_0+2][i_2+1] * 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_L-1, PB_M-2); i_4++) {
            D[i_3][i_4+i_3] = G[i_4+2][i_3-1] - G[i_3][i_4] + H[i_3][i_4] + G[i_4+2][i_3-2] - H[i_3][i_4] * E[i_4] * I[i_3][i_4+2][i_4-1] + D[i_4+1][i_4+i_3+1] - 6;
        }
    }
#pragma endscop
...
kernel_2422252211_02(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
