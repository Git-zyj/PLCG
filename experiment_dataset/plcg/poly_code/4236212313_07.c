...
void kernel_4236212313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[0] = C[i_1][i_0] + A[0] - A[0] - A[0] * 5;
            for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 0; i_3 < PB_P-3; i_3++) {
                    B[i_1][i_2+2][i_3+2] = B[i_2-1][i_1+2][i_3+3] * B[i_3+1][i_0+2][i_1+1] - 5;
                }
            }
        }
    }
...
kernel_4236212313_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
