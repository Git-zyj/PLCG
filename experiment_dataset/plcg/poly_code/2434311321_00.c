...
void kernel_2434311321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        B[i_1] = E[i_1+1] - F[i_1] + B[i_1+1] - B[i_1+2] - B[i_1-2] * B[i_1-1] * 1;
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_P-2, -i_2+PB_M-2); i_3++) {
            C[i_3+i_2][i_2][i_3] = G[i_3][0] + C[i_3+i_2-2][i_3][i_2-2] * C[i_3+i_2+2][i_3+2][i_2] - C[i_3+i_2-1][i_2-1][i_3] - C[i_3+i_2-2][i_2-2][i_3] * 5;
        }
        D[i_2] = H[i_2-1] * D[i_2+1] + 2;
    }
#pragma endscop
...
kernel_2434311321_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
