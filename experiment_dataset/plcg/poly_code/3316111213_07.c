...
void kernel_3316111213_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        A[i_1] = D[i_1] * D[i_1+1] - 5;
        for (int i_2 = 0; i_2 < -i_1+PB_N; i_2++) {
            C[i_2+i_1][0] = C[i_2+i_1-1][0] * 6;
        }
    }
#pragma endscop
...
kernel_3316111213_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
