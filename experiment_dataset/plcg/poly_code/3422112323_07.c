...
void kernel_3422112323_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0-1][i_1] = A[i_1-1][i_0+1][i_1-1] * 6;
            for (int i_2 = 2; i_2 < PB_P; i_2++) {
                A[i_2-2][i_1-1][i_2-2] = E[i_2][i_1-1][i_2][i_0] * F[i_2][i_1] * A[i_1+1][i_0-1][i_1+1] * A[i_0][i_1-2][i_0] + 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-4; i_3++) {
        for (int i_4 = 2; i_4 < PB_L+2; i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                A[i_3][i_4-2][i_3] = G[i_5][i_4-2][i_3-2] * A[i_4-2][i_5-1][i_4-2] - 4;
            }
        }
        D[i_3+2] = H[i_3+1] - D[i_3+3] + D[i_3+4] + 5;
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3422112323_07(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
