...
void kernel_3436251215_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            B[i_1] = E[i_1] * F[i_0+1][i_1][i_0+1] * G[i_0][i_1] + G[i_0+1][i_1] * B[i_1] * 4;
        }
        for (int i_2 = i_0; i_2 < PB_N-1; i_2++) {
            B[i_2] = H[i_2][i_0-1] + B[i_2+1] - C[i_2+1][i_0] + 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_N, -i_3+PB_M-1); i_4++) {
            C[i_3+1][i_4] = G[i_4][i_3+1] + H[i_3][i_4+1] - G[i_4-1][i_3] + H[i_4][i_3-1] + G[i_4][i_3] + E[1] - E[i_4+i_3+1] - H[i_4][i_3+1] + H[i_4][i_3+1] * C[i_3+2][i_4-1] - 5;
        }
        for (int i_5 = 0; i_5 < PB_N-1; i_5++) {
            B[i_5+1] = I[i_3] - H[i_3][i_5] * H[i_5][i_3-1] - 1;
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3436251215_08(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
