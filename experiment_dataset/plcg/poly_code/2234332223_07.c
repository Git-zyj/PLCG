...
void kernel_2234332223_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
            B[i_2] = D[i_2][i_0] - E[1][i_0][i_2-2] + D[i_2-1][i_0-2] - B[i_2+2] + B[i_2-1] + 4;
        }
    }
#pragma endscop
...
kernel_2234332223_07(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
