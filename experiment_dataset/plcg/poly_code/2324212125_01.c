...
void kernel_2324212125_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L-3; i_2++) {
        for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
            C[i_2+1][i_3+1] = 2;
            C[i_2+1][i_3-1] = D[2][i_3+2] - C[i_2+3][i_3-1] + 6;
        }
    }
#pragma endscop
...
kernel_2324212125_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
