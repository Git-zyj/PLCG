...
void kernel_3422351325_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_N, -i_0+PB_M+1); i_1++) {
            A[i_0-1][i_1] = E[1] + E[i_0+1] + F[i_1] + G[i_1+i_0-1][i_0] - H[i_0][i_1] - A[i_0][i_1] + A[i_1-3][i_0] + A[i_0-2][i_1-1] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        B[i_2] = I[0] + E[i_2] * E[i_2] - F[i_2] - B[i_2-1] * B[i_2-2] + 4;
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        B[i_3] = I[i_3-1] * E[i_3-1] + F[i_3] * B[i_3] * 3;
        B[i_3+2] = E[i_3+1] * F[i_3] - F[i_3-1] * I[i_3] - H[i_3][0] - 4;
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3422351325_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
