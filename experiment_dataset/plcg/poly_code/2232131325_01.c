...
void kernel_2232131325_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            B[i_0+1][i_1+i_0] = C[i_0][i_1+i_0] - C[i_1][i_1-1] * C[i_1][i_0-1] - 4;
        }
        for (int i_2 = 0; i_2 < min(PB_M, -i_0+PB_L-2); i_2++) {
            B[i_0+1][i_2+i_0] = C[i_0][i_2] + C[i_2][i_0] * B[i_0+1][i_2+i_0+2] * B[i_0][i_2+i_0] * B[i_0-1][i_2+i_0-2] * 5;
        }
    }
#pragma endscop
...
kernel_2232131325_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
