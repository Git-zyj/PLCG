...
void kernel_3236112313_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, i_0); i_1++) {
            A[i_0][i_1][0] = A[i_0][i_1-1][0] + A[i_0-1][i_1-1][0] + A[i_0+1][i_1][0] - 6;
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_0][i_1][0] = C[i_0+1][i_2][i_1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3236112313_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
