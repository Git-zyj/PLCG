...
void kernel_4212212115_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                B[i_1][i_1-1][i_2-1] = C[i_0][i_2+1][i_1] - 6;
            }
            for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
                for (int i_4 = 0; i_4 < -i_1+PB_M; i_4++) {
                    B[i_3+1][i_3][i_4] = C[i_4+i_1][i_3][i_0] + B[i_3][i_3-1][i_1] + 4;
                }
            }
        }
    }
...
kernel_4212212115_09(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
