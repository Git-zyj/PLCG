...
void kernel_4416311225_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[i_0][i_1] = E[i_0][i_1] - A[i_0+1][i_1+1] * A[i_0+1][i_1] * B[i_1+2] - B[i_0+2] + 4;
        }
        B[i_0] = B[i_0-2] * 1;
    }
        ...
        for (int i_3 = 1; i_3 < PB_Q; i_3++) {
            for (int i_4 = 0; i_4 < -i_2+PB_P-1; i_4++) {
                for (int i_5 = 1; i_5 < i_2; i_5++) {
                    C[i_5-1][i_3-1][i_4+i_2+1][i_5] = F[i_5-1][i_4][0] + 6;
                }
            }
        }
        for (int i_6 = i_2; i_6 < PB_P; i_6++) {
            for (int i_7 = 0; i_7 < PB_M-1; i_7++) {
                for (int i_8 = 0; i_8 < PB_Q-1; i_8++) {
                    D[i_2+1][i_7][i_2][i_8] = D[i_7+1][i_2][i_7][i_8+1] + 6;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4416311225_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
