...
void kernel_3234132315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = C[i_0-2] - C[i_0] * C[i_0] + C[i_0] - A[i_0-1] + A[i_0+1] * A[i_0-1] - A[i_0+1] - A[i_0-1] * 4;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_M-2; i_2++) {
            B[i_2-2] = D[i_2][i_1] + C[i_2+i_1+2] + 3;
        }
    }
#pragma endscop
...
kernel_3234132315_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
