...
void kernel_3332332111_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_1][i_1+i_0] = D[i_1][i_0] * E[i_1+i_0] - F[i_0-1][i_1] * G[i_1][i_0+2] - D[i_1][i_0+1] * A[i_1-1][i_1+i_0-1] - 5;
        }
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            for (int i_3 = 0; i_3 < -i_2+PB_M; i_3++) {
                A[i_2][i_3+i_2] = H[i_2][i_3][i_0] + 2;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < -i_4+PB_M; i_5++) {
            A[i_4][i_5+i_4] = F[i_5+i_4][i_5-2] - D[i_5][i_4] - E[i_5+i_4-2] + 3;
        }
    }
#pragma endscop
...
kernel_3332332111_02(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
