...
void kernel_2234231221_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            A[i_1][i_1+1] = C[i_1] + C[i_1+1] * A[i_1-1][i_1] + A[i_1+1][i_1+2] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        A[i_2][i_2+1] = C[i_2-1] * C[i_2+1] + C[1] * 1;
    }
#pragma endscop
...
kernel_2234231221_07(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
