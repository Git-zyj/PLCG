...
void kernel_2226132225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_0-2][i_1] = C[i_1][i_0] * C[i_1-2][i_0] * A[i_0-1][i_1-2] * B[i_0-1][i_1] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_N; i_3++) {
            B[i_3-1][i_2] = D[i_3] - C[i_2][i_3-2] * C[i_3-1][i_2] + A[i_2-2][i_3-2] * B[i_3-2][i_2] + 2;
        }
    }
#pragma endscop
...
kernel_2226132225_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
