...
void kernel_3422112215_06(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                D[i_0-1][i_2-1][i_1] = E[i_1][i_0][i_2] * D[i_2][i_0][i_1] + 2;
            }
        }
    ...
    for (int i_8 = 0; i_8 < PB_M; i_8++) {
        for (int i_9 = 0; i_9 < PB_M; i_9++) {
            for (int j_0 = 1; j_0 < PB_M; j_0++) {
                D[j_0][i_9][i_8] = E[j_0-1][i_8][i_9+1] - D[j_0-1][i_9][i_8] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3422112215_06(xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
