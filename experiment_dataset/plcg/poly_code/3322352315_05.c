...
void kernel_3322352315_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        A[i_0-2][i_0] = D[i_0] * D[i_0] + E[i_0] - A[i_0-3][i_0-1] - 6;
    }
    for (int i_1 = 3; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_L; i_2++) {
            B[i_1-2][i_1-1] = F[i_2-2][1] + G[i_2][i_1] * F[i_1][i_2+i_1] - D[i_1] * A[i_1-2][i_1] * B[i_1-3][i_1-2] - B[i_1-1][i_1] * B[i_1-3][i_1-2] + 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < PB_P; i_4++) {
            A[i_4-2][i_4] = F[i_4][i_3+2] - H[i_4] - I[i_4][i_3] * H[i_4] * I[i_3][2] + I[i_4-1][i_3] * A[i_3-2][i_3] + 6;
        }
    }
#pragma endscop
...
kernel_3322352315_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
