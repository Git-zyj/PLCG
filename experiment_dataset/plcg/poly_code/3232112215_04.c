...
void kernel_3232112215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q+1; i_1++) {
            for (int i_2 = 2; i_2 < PB_Q; i_2++) {
                A[i_1-2][i_2] = C[i_1-1][i_2][i_1] - A[i_2-2][i_1-1] + 6;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_L; i_3++) {
            for (int i_4 = 0; i_4 < PB_P-3; i_4++) {
                B[i_0][i_3+i_0-1][i_4+2] = C[i_4+i_3][i_0][0] * B[i_0][i_3+i_0-1][i_4+1] - B[i_0+1][i_3+i_0][i_4+2] - B[i_0][i_3+i_0-1][i_4+3] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3232112215_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
