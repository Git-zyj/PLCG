...
void kernel_4426132115_05(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
        ...
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_M, i_0); i_3++) {
                    C[i_3+i_0][i_3+i_0-2][i_2][1] = E[i_3][i_1+2][i_0][i_2] + 5;
                }
            }
        }
        B[i_0] = F[i_0] - F[i_0] - F[i_0+1] + F[i_0-2] - 5;
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = i_4; i_5 < PB_Q-1; i_5++) {
            for (int i_6 = i_4; i_6 < PB_Q; i_6++) {
                for (int i_7 = 0; i_7 < -i_4+PB_M; i_7++) {
                    C[i_7+i_4][i_7+i_4-2][i_5][1] = G[i_4-1][i_6][i_5][i_7][2] * C[i_7+i_4][i_7+i_4-2][i_5+1][1] * 6;
                }
            }
        }
        for (int i_8 = 1; i_8 < min(i_4, -i_4+PB_L); i_8++) {
            for (int i_9 = 1; i_9 < -i_8+PB_M-1; i_9++) {
                C[i_9+i_8+1][i_9+i_8-1][i_4+1][1] = H[i_4+2][i_8-1][i_9-1] * I[i_9-1][i_8] - H[i_4][i_4][i_8] + H[i_8+i_4][i_9][i_9] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4426132115_05(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
