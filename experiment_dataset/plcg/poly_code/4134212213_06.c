...
void kernel_4134212213_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
                A[i_2+1][i_1-2] = B[i_2][i_1] * A[i_0+2][i_1-1] + A[i_2+2][i_1-2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4134212213_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
