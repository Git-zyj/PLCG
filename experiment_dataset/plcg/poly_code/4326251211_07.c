...
void kernel_4326251211_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] + E[i_0+1] * E[i_0+1] - E[i_0] + F[i_0+1][1] + A[i_0+1] - A[i_0-1] - 2;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_P; i_3++) {
                    B[i_1][i_2][i_0][i_3] = G[i_1][i_3][i_2-1] * H[i_2+1][i_1][i_3+i_1] - B[i_1][i_2][i_0-1][i_3] + B[i_1][i_2][i_0][i_3+1] - 2;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        C[i_4] = E[i_4] - D[i_4] - E[i_4] - D[i_4+1] + E[i_4+1] * D[i_4] + A[i_4] + 5;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4326251211_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
