...
void kernel_4414151115_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_N; i_1++) {
            A[i_1-1] = E[i_1] + F[i_1+i_0][i_1-1] + F[i_1][i_0] * F[i_0][i_1-1] * A[i_1-2] * 6;
        }
        for (int i_2 = 1; i_2 < i_0; i_2++) {
            A[i_2-1] = F[i_2][i_0-1] + F[i_2+1][i_0-1] * F[i_2][0] + F[i_2][1] * 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_Q-1; i_3++) {
        A[i_3] = E[i_3+1] - E[i_3] * E[i_3-1] + E[i_3] + E[i_3] + 5;
        for (int i_4 = 1; i_4 < -i_3+PB_L; i_4++) {
            for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
                for (int i_6 = 1; i_6 < -i_3+PB_Q; i_6++) {
                    D[i_4-1][i_5][i_6][i_3-1] = G[i_5][i_4][i_6][i_3] - G[i_5-1][i_4+i_3][i_4][i_3+1] - G[i_3][i_6][i_5+1][i_4+i_3-1] + H[i_5+1][i_4][i_6+i_3][i_4][i_5] - D[i_6-1][i_5][i_4][i_3-2] * 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414151115_04(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H)));
