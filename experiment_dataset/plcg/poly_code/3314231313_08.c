...
void kernel_3314231313_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_N+1); i_1++) {
            A[i_1][i_0+1] = D[i_1][i_0] + E[i_1-1] - F[i_1+i_0][i_0] * A[i_1][i_0+2] * A[i_1-1][i_0+1] * 5;
        }
        for (int i_2 = 1; i_2 < PB_N; i_2++) {
            for (int i_3 = 1; i_3 < i_2; i_3++) {
                B[i_0][i_3] = G[i_3][i_0][i_2] - H[i_2+1][i_0][i_3] + B[i_2][i_3-1] * B[i_2-1][i_3] - 5;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
                C[i_5][i_6+1] = H[i_6-1][i_4][i_5] + D[i_4+1][i_5] + C[i_5-1][i_6+1] * C[i_5+1][i_6+1] + C[i_6+1][i_4] + C[i_6][i_4+2] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3314231313_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
