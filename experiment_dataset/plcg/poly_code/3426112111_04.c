...
void kernel_3426112111_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L, i_1); i_2++) {
                A[i_0][i_2][0] = A[i_0-1][i_2][0] - A[i_0+1][i_2][0] * 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = max(3, i_3); i_4 < PB_M+2; i_4++) {
            B[i_4-2] = E[i_3][i_3] + B[i_4-3] - 5;
        }
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            for (int i_6 = i_3; i_6 < PB_P; i_6++) {
                C[i_5+2][i_3+2][i_6] = 4;
            }
            D[i_3][i_5] = D[i_3+1][i_5] * 2;
        }
    }
#pragma endscop
...
kernel_3426112111_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
