...
void kernel_3226132323_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            A[i_1+1][i_0] = C[i_1+1][i_0] * C[i_1+i_0][1] + C[i_1+i_0][0] - A[i_1+1][i_0-2] * A[i_1+1][i_0+1] - A[i_1+1][i_0+2] * 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, PB_L-2); i_3++) {
            A[i_3-1][i_2-2] = C[i_3+1][i_2] - C[i_3][i_2] * C[i_2-2][i_3+2] * 6;
        }
    }
#pragma endscop
...
kernel_3226132323_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
