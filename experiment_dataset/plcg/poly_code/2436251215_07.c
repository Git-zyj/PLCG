...
void kernel_2436251215_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        A[i_0] = E[i_0-1] + F[i_0] * E[i_0-1] - F[i_0+1] * A[i_0-1] + 5;
        B[0] = E[i_0] - F[i_0-1] * F[i_0+1] * E[i_0] * E[i_0] * A[i_0] + 1;
        for (int i_1 = 1; i_1 < min(PB_P, -i_0+PB_M+1); i_1++) {
            A[i_0] = G[i_0][1] + H[i_1-1][i_0] * H[i_0+1][i_1] * I[i_1+i_0-1][i_0+1][i_1+1] + G[i_1][0] - B[0] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            A[i_3-1] = F[i_3] * H[i_3][0] - H[i_2-1][i_3] + 4;
        }
    }
#pragma endscop
...
kernel_2436251215_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
