...
void kernel_4232212215_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = PB_N-1 - 1; i_1 >= 2; i_1--) {
            A[i_1][i_1-1] = A[i_1+1][i_1] + A[i_1-1][i_1-2] - 3;
        }
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 1; i_3 < PB_N+1; i_3++) {
                A[i_2][i_2-1] = C[i_3-1][i_2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4232212215_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
