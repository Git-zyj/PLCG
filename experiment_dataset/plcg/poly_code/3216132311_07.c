...
void kernel_3216132311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1] = C[i_1] + A[i_1+1] - A[i_1-1] - A[i_1-1] - A[i_1-1] + A[i_1+1] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_Q, i_2); i_3++) {
            for (int i_4 = 2; i_4 < -i_2+PB_P; i_4++) {
                B[i_4][i_3][i_2] = D[i_2][i_4-2][i_4] + D[i_3][i_4+i_2][i_2-2] + D[i_4][i_2][i_2] + D[i_4][i_3+i_2-1][i_3+i_2] * B[i_4-1][i_3-1][i_2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3216132311_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
