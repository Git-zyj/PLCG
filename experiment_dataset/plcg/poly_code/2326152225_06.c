...
void kernel_2326152225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_P+1, i_1); i_2++) {
            B[i_1][i_2] = E[i_2][i_1+1] * E[i_2+1][i_1] - E[i_2][i_1] * E[i_2][i_1] * E[i_2+2][i_2+i_1-1] + A[i_2-2] - B[i_1+1][i_2] + 3;
        }
        A[i_1-3] = D[i_1] - D[i_1] + D[i_1] * D[i_1] - D[i_1+1] * A[i_1] - 5;
    }
#pragma endscop
...
kernel_2326152225_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
