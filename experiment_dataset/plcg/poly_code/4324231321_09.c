...
void kernel_4324231321_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, PB_Q-1); i_2++) {
                for (int i_3 = 0; i_3 < i_0; i_3++) {
                    A[i_2][i_1][i_0][i_3+1] = D[i_2+i_0][i_1-1][i_3][0] - E[i_1+1][i_2-1][i_3+1][i_0+1] * A[i_3][i_1-2][i_0][i_2+1] - 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 2; i_5 < min(-i_4+PB_M+1, PB_M-2); i_5++) {
            B[i_5] = F[0] * G[i_5+i_4-1][i_5+1] + B[i_5+2] * B[i_4+2] + B[i_5+1] - B[i_5-2] + 3;
            B[i_5+2] = H[i_4][i_5][i_5+i_4] + G[i_5][i_4-1] + F[i_5+1] - G[i_5][i_4] * F[i_5-1] - B[i_4] - 4;
        }
    }
#pragma endscop
...
kernel_4324231321_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
