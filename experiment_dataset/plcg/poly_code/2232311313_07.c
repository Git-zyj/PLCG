...
void kernel_2232311313_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_M-1; i_3++) {
            B[i_3] = C[i_3][i_2+1] - C[i_2][i_3] + B[i_3+1] + B[i_3-1] + 3;
        }
    }
#pragma endscop
...
kernel_2232311313_07(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
