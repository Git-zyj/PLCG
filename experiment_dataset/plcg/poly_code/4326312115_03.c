...
void kernel_4326312115_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 3; i_2 < PB_L; i_2++) {
                A[i_1+1][i_1-2] = D[i_2][i_0+1] * A[i_2][i_2-3] - 6;
            }
        }
        B[i_0+2] = 6;
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_M+1; i_4++) {
            for (int i_5 = 3; i_5 < -i_4+PB_M; i_5++) {
                C[i_5+i_4][i_3][i_3-2] = A[i_4+1][i_4-2] * C[i_4+i_3-1][i_5-1][i_5-3] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4326312115_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
