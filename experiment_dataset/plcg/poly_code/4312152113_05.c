...
void kernel_4312152113_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < min(PB_M, -i_1+PB_N); i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_0+PB_M, -i_2+PB_M), PB_M-2); i_3++) {
                    A[i_0][i_1][i_2-2][i_3] = D[i_2][i_3+2][i_1][0] + D[i_1+2][i_3+i_0][i_2][0] - D[i_2+i_1][i_3][i_1+2][i_0+2] * D[i_1][i_3+i_2][i_1][1] + D[i_0-1][i_2][i_1+1][i_1] - 5;
                }
            }
        }
        B[i_0] = E[i_0] * E[i_0] * E[i_0] - E[i_0] * B[i_0-1] + B[i_0+1] - 6;
    }
        ...
        for (int i_5 = 0; i_5 < min(-i_4+PB_N, PB_M-1); i_5++) {
            for (int i_6 = 0; i_6 < -i_4+PB_M; i_6++) {
                for (int i_7 = 1; i_7 < min(-i_5+PB_P, -i_6+PB_M); i_7++) {
                    A[i_5][i_6][i_7-1][i_4+1] = D[i_6+1][i_7][i_4+2][i_5] * D[i_6][i_5][i_6][i_4] - D[i_7+1][i_6+i_4][i_4][i_7+i_5] + D[i_5+i_4][i_7+i_6][i_7][i_4] - F[i_5+1][i_7+i_6][2] - 1;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4312152113_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
