...
void kernel_2326351225_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < min(min(PB_L, -i_0+PB_M), i_0); i_1++) {
            B[i_1] = D[i_1+i_0][i_1-1] + D[i_1][i_0-1] * E[i_0-1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            B[i_3] = F[1][0] - G[i_3-1][i_2+1][i_2+1] + F[i_3-1][i_2] * F[i_2-1][i_3] - H[i_2] - B[i_3-2] - 1;
        }
        B[i_2] = I[i_2] - E[i_2] * I[i_2] - H[i_2] + H[i_2+1] + B[i_2+1] * 4;
    }
#pragma endscop
...
kernel_2326351225_07(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
