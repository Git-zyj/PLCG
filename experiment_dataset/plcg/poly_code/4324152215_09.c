...
void kernel_4324152215_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_M; i_2++) {
                for (int i_3 = i_2; i_3 < min(min(-i_1+PB_M, -i_2+PB_Q), PB_L-2); i_3++) {
                    A[i_2][i_3+i_1][i_0][i_1] = D[i_2][i_3+2][i_3][0] * D[i_1+1][i_0][i_2-2][i_1+2] + D[i_1+1][i_0+1][i_0+2][i_3+i_2] + A[i_3+1][i_2+i_1][i_0][i_2] + 6;
                }
            }
        }
        B[i_0] = E[i_0][i_0] + F[i_0-2] * F[i_0] * B[i_0-1] * B[i_0+1] - 4;
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_P-2; i_5++) {
            B[i_4] = E[i_4+1][i_5] * E[i_5][i_4-1] * G[i_4+2][i_5+2][i_4-2] * E[i_5][i_4] * F[0] - E[i_5][i_4] * E[i_5+1][i_4] + B[i_5] + 2;
        }
    }
#pragma endscop
...
kernel_4324152215_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
