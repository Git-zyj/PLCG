...
void kernel_2212151125_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        B[i_0-2][i_0-1] = C[i_0] * C[i_0] * C[i_0] * C[i_0] * C[i_0] * 6;
    }
    for (int i_1 = 1; i_1 < PB_Q; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_P-1, -i_1+PB_L+1); i_2++) {
            B[i_2-1][i_2] = D[i_2][i_1] + D[i_2+i_1-1][0] + D[i_2][i_1] - D[i_2+i_1-1][0] - B[i_2][i_2+1] + 3;
        }
    }
#pragma endscop
...
kernel_2212151125_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
