...
void kernel_3212212225_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_0][i_1-2] = C[2][i_1][i_0] * B[i_2][i_0] * 4;
            }
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_0][i_3] = B[i_0-2][i_3] + 5;
        }
    }
#pragma endscop
...
kernel_3212212225_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
