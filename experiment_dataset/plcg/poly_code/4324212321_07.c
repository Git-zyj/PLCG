...
void kernel_4324212321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_2][i_1] = A[i_1][i_2+1] - A[i_2][i_1+1] - A[i_2+1][i_1] + 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_M-3, i_3); i_4++) {
            B[i_3+2][i_3] = B[i_3][i_3-2] - B[i_4+3][i_4+1] * B[i_3+1][i_3-1] * 5;
        }
        for (int i_5 = 1; i_5 < i_3; i_5++) {
            C[i_5] = D[i_5][i_3+1] - D[i_5+1][i_3] * C[i_5+1] * C[i_5+2] * C[i_5-1] - 2;
        }
    }
#pragma endscop
...
kernel_4324212321_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
