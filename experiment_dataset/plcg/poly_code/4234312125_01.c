...
void kernel_4234312125_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_Q; i_2++) {
                for (int i_3 = i_2; i_3 < -i_1+PB_Q-2; i_3++) {
                    A[i_0][i_1-2][i_3+i_1+2][i_2] = A[i_0-1][i_3-2][i_3+i_1][i_2-2] + 5;
                }
            }
        }
    }
...
kernel_4234312125_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
