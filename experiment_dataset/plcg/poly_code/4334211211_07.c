...
void kernel_4334211211_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_M+1, -i_0+PB_L); i_2++) {
                A[i_2+i_0][i_0][i_1] = D[i_2-1][i_0][i_1+i_0+1] + A[i_2+i_0-1][i_0-1][i_1] - 6;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            for (int i_4 = 2; i_4 < PB_L; i_4++) {
                B[i_0][i_4-1][i_0-1] = D[i_3][i_4][i_0] + B[i_0][i_4-2][i_0-1] * B[i_0][i_4][i_0-1] * B[i_0+1][i_4-1][i_0] + 6;
            }
        }
        for (int i_5 = i_0; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 0; i_6 < min(i_0, PB_M-2); i_6++) {
                C[i_5+1][i_0-1][i_6] = D[i_6][i_0][i_5] - C[i_6+2][i_5-1][i_0] - C[i_6+1][i_0][i_5] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4334211211_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
