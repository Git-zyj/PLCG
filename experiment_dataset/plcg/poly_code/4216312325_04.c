...
void kernel_4216312325_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M-2; i_2++) {
                B[i_0][i_2+1][i_1] = C[i_2][i_2-1] + B[i_1][i_2+2][i_0-1] + 6;
            }
        }
    }
        ...
        for (int i_4 = max(1, i_3); i_4 < PB_L-2; i_4++) {
            for (int i_5 = i_3; i_5 < PB_M-2; i_5++) {
                for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
                    B[i_4][i_5+1][i_3] = B[i_4-1][i_6+2][i_5+2] * B[i_6][i_5+2][i_4+2] * 6;
                }
            }
        }
    }
...
kernel_4216312325_04(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
