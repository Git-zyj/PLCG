...
void kernel_4322311313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_2+1][i_3] = D[i_3][i_2-1] - B[i_2][i_3+1] * B[i_2+1][i_3-1] - B[i_2+2][i_3] + 3;
        }
        for (int i_4 = 0; i_4 < PB_N; i_4++) {
            C[i_2][i_4] = C[i_4][i_2+1] * C[i_2-1][i_4] + C[i_2+1][i_4] * 2;
        }
    }
#pragma endscop
...
kernel_4322311313_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
