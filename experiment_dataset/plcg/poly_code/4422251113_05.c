...
void kernel_4422251113_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-1] = E[i_0] + F[i_0] * F[i_0-1] + F[i_0] * A[i_0-2] + 3;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        B[i_1-1] = F[i_1] * F[i_1] + F[i_1] + G[i_1][0] - 3;
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = 0; i_3 < PB_M; i_3++) {
                C[i_1+1][i_2+1][i_3] = H[i_2][i_3][i_1] + 4;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            D[i_5] = G[i_4][i_4] * G[i_5+1][i_4-1] + G[i_5-1][i_4+1] - I[i_4-1][i_5-1] - G[i_5][i_4] - I[i_5-1][i_4] * G[i_5+1][i_4-1] - F[i_5] * 4;
        }
    }
#pragma endscop
...
kernel_4422251113_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
