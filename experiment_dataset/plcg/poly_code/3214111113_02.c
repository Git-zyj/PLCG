...
void kernel_3214111113_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = min(i_1, PB_M-1) - 1; i_2 >= 1; i_2--) {
            B[i_2+1][0] = C[i_2-1][i_1] * A[i_2-1] + 6;
        }
    }
#pragma endscop
...
kernel_3214111113_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
