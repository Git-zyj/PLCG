...
void kernel_3426131313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
    ...
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            for (int i_3 = max(1, i_2); i_3 < -i_2+PB_L; i_3++) {
                B[i_1][i_3][i_1+1] = F[i_1-1][i_3][i_3+i_2] * F[i_3+i_2][i_1][i_2] * B[i_1][i_3-1][i_1+1] - B[i_3-1][i_2][i_3] + B[i_1-1][i_2][i_1] - B[i_1][i_3+1][i_1+1] - 6;
            }
        }
        for (int i_4 = i_1; i_4 < PB_L; i_4++) {
            for (int i_5 = i_4; i_5 < PB_L-1; i_5++) {
                C[i_1-1][i_5][i_4] = F[i_4][i_1][i_5-1] - F[i_1][i_5][i_4] * F[i_4][i_5-1][i_1+1] + C[i_1-1][i_5+1][i_4] + 4;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_L+1; i_6++) {
        for (int i_7 = 0; i_7 < min(i_6, -i_6+PB_L+1); i_7++) {
            for (int i_8 = 1; i_8 < PB_L; i_8++) {
                C[i_6][i_8][i_7] = G[i_8][i_7] + F[i_8+i_7][i_6][i_8-1] * F[i_8+i_7+1][i_6-1][i_7+i_6-1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3426131313_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
