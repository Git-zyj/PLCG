...
void kernel_3216212221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = PB_P-1 - 1; i_0 >= 0; i_0--) {
        ...
        for (int i_2 = i_0; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                B[i_3][i_3][i_2] = C[i_2][i_0+1][i_3] * A[i_2] + B[i_3+2][i_3+2][i_2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3216212221_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
