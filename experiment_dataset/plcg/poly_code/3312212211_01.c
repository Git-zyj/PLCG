...
void kernel_3312212211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        ...
        B[i_0] = D[i_0] + E[i_0+2] + A[i_0] + A[i_0+1] * 4;
    }
    for (int i_2 = 0; i_2 < PB_Q-3; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_Q, -i_2+PB_M); i_4++) {
                C[i_3][i_4][i_2+2] = F[0][i_4+i_2][i_3] * C[i_3-1][i_4][i_2+3] + C[i_3][i_4][i_2+1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3312212211_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
