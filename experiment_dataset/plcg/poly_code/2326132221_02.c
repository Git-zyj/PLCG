...
void kernel_2326132221_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_P, i_0); i_1++) {
            A[i_0][i_1] = D[i_1-2][i_0+1] + D[i_0][i_1] + A[i_0][i_1-2] * A[i_0-2][i_1-1] * 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        B[i_2][2] = E[i_2] + E[i_2] + E[i_2] * D[i_2][i_2] - B[i_2+2][2] - 6;
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            C[i_4-2] = D[i_4][i_4] + C[i_4-1] + C[i_4] + 3;
        }
    }
#pragma endscop
...
kernel_2326132221_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
