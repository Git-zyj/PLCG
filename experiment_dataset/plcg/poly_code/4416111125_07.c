...
void kernel_4416111125_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                B[i_1][i_2-1][i_0] = E[i_0][i_2][i_1][i_1] + B[i_1-1][i_2+1][i_0] + 1;
                for (int i_3 = 1; i_3 < PB_Q; i_3++) {
                    B[i_1][i_3-1][i_0] = E[i_2][i_1][i_3][i_0] * 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < PB_P-1; i_5++) {
            for (int i_6 = 0; i_6 < PB_M; i_6++) {
                B[i_6][i_4-1][i_5] = F[i_6][i_4][i_5+1] * 1;
            }
        }
        for (int i_7 = 1; i_7 < -i_4+PB_M; i_7++) {
            D[i_4][i_7+i_4] = G[i_7-1][i_4] + 4;
        }
    }
#pragma endscop
...
kernel_4416111125_07(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
