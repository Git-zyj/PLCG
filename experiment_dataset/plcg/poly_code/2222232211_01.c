...
void kernel_2222232211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
            A[i_0+1][i_1] = A[i_0+1][i_1+1] * A[i_0+2][i_1+1] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L; i_3++) {
            B[i_2+1][i_3+i_2-2] = C[i_2-2] - D[i_3][i_2+2] + D[i_3][i_2] * C[1] * E[i_2-1] * B[i_2+2][i_3+i_2] + 6;
        }
    }
#pragma endscop
...
kernel_2222232211_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
