...
void kernel_2432111315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            C[i_2-1][i_3] = C[i_2-2][i_3-1] * C[i_2-1][i_3-1] * C[i_2][i_3] * C[i_2-2][i_3+1] * 6;
        }
        for (int i_4 = i_2; i_4 < PB_L-1; i_4++) {
            D[i_2][i_4+1] = D[i_2-1][i_4+1] - D[i_4+1][i_2+2] * 5;
        }
    }
#pragma endscop
...
kernel_2432111315_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
