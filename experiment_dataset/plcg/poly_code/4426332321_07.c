...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4426332321_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
        ...
        for (int i_1 = i_0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-3; i_2++) {
                for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
                    A[i_0][i_3-1][i_1][i_2+1] = A[i_0-2][i_3-1][i_1][i_2+1] * A[i_0+2][i_3-1][i_1+1][i_2+2] + A[i_1][i_3-1][i_0][i_2] + A[i_0][i_3-1][i_1][i_2+3] * A[i_0-1][i_3-1][i_1][i_2+1] * 5;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_Q-1; i_4++) {
            for (int i_5 = 0; i_5 < min(min(-i_4+PB_M, i_0), -i_0+PB_P); i_5++) {
                for (int i_6 = 2; i_6 < PB_N-2; i_6++) {
                    D[i_6-2][i_4][i_0][i_0] = E[i_5+i_4][i_0][i_6][i_4] * E[0][i_5][i_6][i_4] * F[i_0+1][i_5][i_4][i_4] * G[i_6+2][i_4][i_0][i_5+i_0][i_4+1] * E[i_4][i_0-2][i_5][i_6] - 3;
                }
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_M-1; i_7++) {
        C[i_7] = H[i_7] * H[i_7] - H[i_7] + C[i_7-1] * 2;
        for (int i_8 = i_7; i_8 < PB_Q; i_8++) {
            for (int i_9 = max(3, i_7); i_9 < -i_8+PB_M; i_9++) {
                D[i_9-2][i_8][i_7][i_7] = I[i_8-2][i_7][i_9] - J[i_9+i_8][i_7][0] * D[i_9-3][i_7+1][i_8][i_8] - D[i_8][i_9][i_7-2][i_7-2] * D[i_9-2][i_8][i_7-2][i_7-2] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4426332321_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
