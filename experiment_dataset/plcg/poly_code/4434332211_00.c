...
void kernel_4434332211_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 3; i_2 < PB_M-2; i_2++) {
                for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
                    A[i_0][i_2][i_3][i_2-2] = E[i_3][i_0][i_1][0] + E[i_2+2][i_3][i_0+2][i_1] + A[i_2][i_0+1][i_3-1][i_0-1] * A[i_0][i_2][i_1-1][i_2-2] + A[i_2][i_1][i_3-1][i_1-2] - A[i_0-1][i_2-1][i_3+1][i_2-3] * 6;
                }
            }
        }
        for (int i_4 = 2; i_4 < -i_0+PB_M; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_0+PB_L, PB_M-2); i_5++) {
                for (int i_6 = 1; i_6 < PB_M-2; i_6++) {
                    B[i_5+2][i_0][i_5][i_6] = F[i_4][i_5+i_0][i_6][i_5] - G[i_6][i_4+i_0][i_6+i_5-2][i_5][0] * H[i_6+2][i_4-2][i_5][i_0] + E[i_5-1][i_0-1][i_6-1][i_4] + E[i_5-1][i_4][i_6][i_0] * 4;
                }
            }
        }
    }
    ...
    for (int i_8 = 2; i_8 < PB_L-2; i_8++) {
        for (int i_9 = 0; i_9 < min(-i_8+PB_L, PB_M-2); i_9++) {
            for (int j_0 = 1; j_0 < min(min(-i_8+PB_Q+2, PB_P-3), -i_9+PB_M-2); j_0++) {
                D[i_8-1][2][j_0+2] = J[j_0+i_9+2][i_8+2][j_0+i_8-2] * K[j_0][i_8][i_9] + L[j_0-1][i_9+i_8] * D[i_8][2][j_0+2] + D[i_8-1][2][j_0+3] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4434332211_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
