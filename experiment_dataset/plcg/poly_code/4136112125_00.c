...
void kernel_4136112125_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(i_1, PB_L-2); i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    A[i_1][i_2][i_3] = B[i_1][i_2+2][i_0-2] + A[i_2+1][i_0-2][i_3] - 4;
                }
            }
        }
    }
...
kernel_4136112125_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
