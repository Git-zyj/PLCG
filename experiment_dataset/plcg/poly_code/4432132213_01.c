...
void kernel_4432132213_01(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_M-2); i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_Q-2; i_3++) {
                    B[i_2][i_0+1][i_1+1][i_3+i_0+2] = E[i_2+i_1][i_0][i_3][0] - F[i_2][i_1][2] * E[i_0+2][i_2][i_1+1][i_3] + E[i_2+2][i_3+1][i_1][i_0+1] - 6;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_Q-3; i_4++) {
        for (int i_5 = 0; i_5 < -i_4+PB_Q-3; i_5++) {
            for (int i_6 = 0; i_6 < -i_5+PB_Q-3; i_6++) {
                for (int i_7 = 1; i_7 < PB_L; i_7++) {
                    B[i_6][i_5][i_7][i_5+i_4+2] = E[i_7][i_6+i_5+1][i_4][i_6+i_5+2] * E[i_5][i_7][i_7-1][2] * B[i_6][i_5][i_7+1][i_5+i_4+3] * B[i_4][i_6+1][i_7][i_6+i_5+3] * 5;
                }
            }
        }
        for (int i_8 = 1; i_8 < -i_4+PB_M; i_8++) {
            C[i_4+1][i_8] = G[i_4][i_8] * G[i_8+1][i_4-2] + G[i_8+i_4][i_4] * G[i_4][i_8-1] + C[i_4+2][i_8] * 4;
        }
    }
        ...
        for (int j_0 = 0; j_0 < PB_Q; j_0++) {
            for (int j_1 = 1; j_1 < PB_L; j_1++) {
                for (int j_2 = 0; j_2 < PB_M; j_2++) {
                    D[j_2][i_9][j_1][j_0] = F[j_2][j_1][j_0] - D[j_2][i_9-1][j_1-1][j_0] + 6;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4432132213_01(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
