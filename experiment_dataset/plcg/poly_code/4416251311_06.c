...
void kernel_4416251311_06(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        D[i_0][i_0] = E[1] + F[i_0] - G[i_0+1][i_0] * F[i_0] * F[i_0] - F[i_0] + E[i_0] - D[i_0][i_0] - 3;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
            for (int i_3 = 2; i_3 < i_2; i_3++) {
                for (int i_4 = 1; i_4 < PB_Q-1; i_4++) {
                    B[i_1][i_3-1][i_2][i_4] = H[i_3+1][i_4][i_1][i_2-1] - H[i_3][i_4][i_1+1][i_2] * B[i_3][i_1-1][i_2-1][i_4] - B[i_1+1][i_3-2][i_2+1][i_4-1] * B[i_1][i_3-1][i_2][i_4+1] + 6;
                }
            }
        }
        C[i_1] = E[i_1] + E[i_1] - E[i_1] + C[i_1-1] + D[i_1][i_1] - D[i_1+1][i_1+1] * 4;
        for (int i_5 = i_1; i_5 < PB_L; i_5++) {
            D[i_1][i_1] = I[i_5][0] + I[i_5+1][i_1] * I[i_5-1][i_1] * I[i_5][i_1-1] - I[i_5][i_1] * I[i_5-1][i_1] + E[0] + C[i_5-1] + D[i_1+1][i_1+1] * 2;
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4416251311_06(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
