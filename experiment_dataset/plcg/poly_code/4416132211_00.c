...
void kernel_4416132211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[2] + E[i_0] - E[i_0-2] - E[i_0] - A[i_0+1] - A[i_0-1] * A[i_0-1] - 1;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_Q; i_2++) {
            for (int i_3 = 0; i_3 < i_1; i_3++) {
                B[i_1][i_3][1] = F[i_2][i_3] * G[i_1+2][i_3][i_2] * B[i_1+1][i_3][1] * C[i_1][i_3][0] * 1;
            }
        }
        for (int i_4 = i_1; i_4 < PB_Q-1; i_4++) {
            for (int i_5 = 1; i_5 < min(PB_Q, -i_4+PB_L); i_5++) {
                C[i_1][i_4][0] = G[i_5][i_1][0] * G[i_5][i_1][i_4] - G[i_5-1][i_1][i_4+1] + F[i_1][i_5+i_4] - 2;
            }
            for (int i_6 = i_4; i_6 < PB_Q; i_6++) {
                for (int i_7 = 0; i_7 < PB_Q; i_7++) {
                    B[i_4][i_7][1] = H[i_7][i_6][i_7][i_4] + 1;
                }
            }
        }
    }
...
kernel_4416132211_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
