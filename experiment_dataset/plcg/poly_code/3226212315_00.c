...
void kernel_3226212315_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_Q+2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_Q+2, PB_P-1); i_2++) {
                B[i_1][i_0+1][i_2+i_1-2] = B[i_1][i_2+1][i_1+i_0-2] - 2;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_Q-2, -i_3+PB_Q+2); i_4++) {
            for (int i_5 = i_4; i_5 < -i_3+PB_Q-2; i_5++) {
                B[i_3][i_5+1][i_4+i_3-2] = C[i_3-1][i_5+i_3+2][i_4+2] + C[i_3][i_5][i_4] * B[i_3][i_5][i_4+i_3-3] + B[i_3-1][i_5+1][i_4+i_3-3] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3226212315_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
