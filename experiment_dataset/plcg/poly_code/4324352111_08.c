...
void kernel_4324352111_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] + E[i_0] * F[i_0] - E[i_0] - A[i_0+1] * A[i_0-1] - A[i_0+1] + 5;
        for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_P; i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_M-1; i_3++) {
                    B[i_2-1][i_1+1][i_0-1][i_3+i_0] = G[i_1-2][i_2][0][i_3+i_0-1] * H[i_3+i_0+1][i_2][i_1][2] * H[i_3][i_2+i_0][i_1][i_0+1] + I[i_3][i_1][i_2] - H[i_2][i_1][i_0-2][i_3+2] * J[i_2][i_3][i_0][i_1] - 1;
                }
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_M; i_6++) {
                for (int i_7 = 1; i_7 < min(PB_M-2, -i_4+PB_Q); i_7++) {
                    C[i_7][i_5][i_6-1] = J[i_4][i_7][i_6][i_5+1] - H[i_6][i_4][i_5+1][i_4+2] - K[i_5+1][i_6][i_7+i_4][i_7][i_5] + H[i_7-1][i_6][i_5-2][2] + H[i_7+2][i_5][i_4-1][i_5] * 2;
                }
            }
        }
    }
...
kernel_4324352111_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K));
