...
void kernel_4236252113_08(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_P, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_Q; i_3++) {
                    B[i_2-1][i_1+1][i_3+1][0] = C[i_1][i_3+2][i_1-2][i_0+2] * D[i_3][i_1+i_0][i_3-1][1] * E[i_2][i_3+i_1][i_0+2][i_0][i_1-1] - 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P-2; i_4++) {
        for (int i_5 = i_4; i_5 < -i_4+PB_Q; i_5++) {
            for (int i_6 = 1; i_6 < PB_Q-1; i_6++) {
                for (int i_7 = 0; i_7 < min(min(min(i_6, -i_5+PB_Q), PB_M-2), -i_4+PB_Q); i_7++) {
                    B[i_4-1][i_5+1][i_7][0] = F[i_5][i_7][i_6] + D[i_6][i_4-1][i_7][i_7] + D[i_6][i_7][i_5+i_4][i_7] - D[i_6-1][i_4+2][i_6][i_7+i_5] * C[i_4][i_5][i_7][i_6+1] + C[i_5][i_6][i_7+i_4][i_6] - C[i_4][i_6][i_7+i_5][i_5] * B[i_6-1][i_7+2][i_4][0] + 1;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4236252113_08(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
