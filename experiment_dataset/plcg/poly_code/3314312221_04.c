...
void kernel_3314312221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, -i_1+PB_L); i_2++) {
                A[i_2+i_1][i_1] = D[i_0-2][i_2+i_1] + A[i_2+i_1-2][i_1] - 4;
            }
        }
        B[i_0] = B[i_0+1] + B[i_0+2] + B[i_0-2] - C[i_0] - 1;
        C[i_0] = C[i_0+1] * 5;
    }
#pragma endscop
...
kernel_3314312221_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
