...
void kernel_3226231115_06(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                B[i_2][i_0][i_1+1][i_1+2] = C[i_2][i_1+1][i_0+1] - C[i_2-1][i_0][i_1] + 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_L, -i_3+PB_P-1); i_4++) {
            for (int i_5 = 0; i_5 < min(PB_Q-2, i_3); i_5++) {
                B[i_4][i_3][i_5][i_5+1] = D[i_4][i_5][i_3] - D[i_5][i_3+1][i_4+i_3+1] + C[i_5][i_3+1][i_4-1] - B[i_4][i_3][i_5+1][i_5+2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3226231115_06(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
