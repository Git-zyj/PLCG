...
void kernel_2324211311_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        B[i_1][i_1] = D[i_1-1] + D[i_1] - 1;
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_Q-1; i_3++) {
            C[i_2+1][i_3] = C[i_2+1][i_3+1] + C[i_2][i_3+1] + C[i_2][i_3] * C[i_2+2][i_3-1] * 5;
        }
    }
#pragma endscop
...
kernel_2324211311_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
