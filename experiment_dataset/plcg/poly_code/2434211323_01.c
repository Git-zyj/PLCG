...
void kernel_2434211323_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P+1; i_0++) {
        A[i_0-1] = C[i_0] * C[i_0-2] + C[i_0+2] * 4;
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            B[i_1][i_0-1] = E[i_1-1][i_0] + A[i_1+1] - B[i_1][i_0-3] * B[i_1-1][i_0-1] + 2;
        }
        for (int i_2 = 3; i_2 < -i_0+PB_M+1; i_2++) {
            C[i_2] = E[i_2+i_0-1][0] * A[i_2-3] - 3;
        }
    }
    for (int i_3 = 0; i_3 < PB_N; i_3++) {
        for (int i_4 = 3; i_4 < PB_N-1; i_4++) {
            D[i_4-1][i_4-1] = D[i_4-2][i_4-2] + D[i_4-3][i_4-3] + D[i_4+1][i_4+1] + D[i_4][i_4] * D[i_4+1][i_4+1] - 6;
        }
    }
#pragma endscop
...
kernel_2434211323_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
