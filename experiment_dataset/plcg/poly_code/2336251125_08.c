...
void kernel_2336251125_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < min(min(-i_1+PB_M, i_1), PB_M-1); i_2++) {
            A[i_2-1] = E[i_2-1] - F[i_1][0] + F[i_1][i_2] - G[i_2+i_1][i_1] + G[i_1][i_2-1] - F[i_2][i_1] + F[i_2][i_1] + G[i_2+1][i_1] * G[i_2][i_1] * 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_M+2; i_4++) {
            A[i_4-2] = H[i_3][i_4+1][i_3-1] - D[0] * 5;
        }
    }
#pragma endscop
...
kernel_2336251125_08(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
