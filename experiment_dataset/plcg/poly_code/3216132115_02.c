...
void kernel_3216132115_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
    ...
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_L-1, i_1); i_2++) {
            for (int i_3 = i_2; i_3 < min(PB_P, -i_1+PB_L); i_3++) {
                B[i_2][i_1][i_3+i_1][i_3] = D[i_1][i_1] - E[i_1+1][i_2][i_3] * B[i_3+1][i_2+1][i_2+i_1+1][i_1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3216132115_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
