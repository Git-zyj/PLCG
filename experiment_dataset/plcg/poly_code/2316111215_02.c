...
void kernel_2316111215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        A[i_0] = A[i_0-1] * 6;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            C[i_0-1][i_1-1] = D[i_1+1] - A[i_0] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_N+1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            C[i_2-1][i_3] = D[i_3-1] * C[i_2-2][i_3] - 3;
        }
    }
#pragma endscop
...
kernel_2316111215_02(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
