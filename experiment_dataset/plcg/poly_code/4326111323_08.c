...
void kernel_4326111323_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                    A[i_2][i_1][i_3-1] = A[i_1+2][i_2][i_3-1] + A[i_2][i_3+1][i_1] - A[i_2-2][i_3][i_1-2] - 5;
                }
                B[i_2][i_2][i_0] = D[i_1-1][i_0][i_2] + B[i_2+2][i_2+2][i_0] + B[i_2][i_2][i_0+1] + B[i_1][i_1][i_0+2] + 4;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N-2; i_4++) {
        for (int i_5 = 1; i_5 < PB_P; i_5++) {
            for (int i_6 = 1; i_6 < i_4; i_6++) {
                C[i_4][i_6-1][i_5] = C[i_4+1][i_6-1][i_5-1] - C[i_4+2][i_6-1][i_5] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4326111323_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
