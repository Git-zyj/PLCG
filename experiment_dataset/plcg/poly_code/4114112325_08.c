...
void kernel_4114112325_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        for (int i_1 = max(i_0, 4); i_1 < PB_P+2; i_1++) {
            for (int i_2 = 2; i_2 < PB_N; i_2++) {
                A[i_0+1][i_1-2][i_2] = B[i_2][i_1] - A[i_0+1][i_1-4][i_2] + A[i_0+3][i_1-2][i_2] * A[i_0+1][i_1-2][i_2-2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4114112325_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
