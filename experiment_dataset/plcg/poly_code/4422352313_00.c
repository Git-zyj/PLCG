...
void kernel_4422352313_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn)){
        ...
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 2; i_3 < i_2; i_3++) {
                    A[i_1][i_3-2][i_2-1][i_0+2] = E[i_1+2][i_0][i_2][i_3][0] + F[i_1-1][i_0][i_3+1][i_2] * G[i_3][i_0-1][i_2][i_1] * H[i_1][i_2][i_0-1][0] * F[i_1][i_2-2][i_0][i_3] * H[i_1+1][i_2+2][i_0][i_1] + A[i_1][i_3-2][i_2][i_0+2] + A[i_1-1][i_0-2][i_3-2][i_2+1] + A[i_1][i_3-1][i_2-1][i_0+2] * 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P-2; i_4++) {
        B[i_4] = I[1] - J[i_4+2] + K[i_4+1] + I[i_4] + I[i_4] * J[i_4] * B[i_4+1] + 4;
        for (int i_5 = i_4; i_5 < -i_4+PB_N; i_5++) {
            C[i_5+1][i_4] = L[i_5+i_4][i_4][0] + M[i_5+2][i_4] + N[i_4+2][i_5] + M[i_5+i_4][i_5+1] + B[i_5] + C[i_5+1][i_4+1] * 2;
            for (int i_6 = 1; i_6 < -i_4+PB_P-1; i_6++) {
                for (int i_7 = 0; i_7 < -i_4+PB_P-2; i_7++) {
                    D[i_5][i_6][i_7+i_4+1][2] = L[i_7][i_7][i_6+2] - A[i_6][i_4-2][i_5-1][i_7+3] + D[i_5][i_7+1][i_6+i_4+1][2] - D[i_5-1][i_6+1][i_7+i_4+1][2] * D[i_5][i_6-1][i_7+i_4+1][2] + D[i_5][i_6][i_7+i_4+2][2] + D[i_5+1][i_7][i_6+i_4+1][2] - 1;
                }
            }
        }
    }
