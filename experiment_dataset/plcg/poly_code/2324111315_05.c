...
void kernel_2324111315_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, i_0); i_1++) {
            B[i_1-1][i_0-1] = D[i_1][i_0+1] - 5;
        }
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            B[i_2-1][i_0] = B[i_2][i_0+1] * B[i_2-2][i_0] - B[i_2-1][i_0+1] - 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_N; i_3++) {
        C[i_3-1] = C[i_3-2] * C[i_3] * C[i_3-2] - 2;
    }
#pragma endscop
...
kernel_2324111315_05(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
