...
void kernel_4216311315_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < min(min(i_0, PB_L-1), -i_0+PB_P); i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                A[i_1][i_2][i_2+i_0-1] = C[i_1][i_2][i_0-1] + A[i_2-1][i_1+1][i_1+i_0] - 5;
            }
        }
    }
        ...
        for (int i_4 = 2; i_4 < PB_L+1; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_P+2; i_5++) {
                for (int i_6 = 0; i_6 < -i_4+PB_P+1; i_6++) {
                    A[i_4][i_6][i_6+i_5-1] = C[i_4-1][i_4-1][i_3] + A[i_6][i_4-1][i_5+i_4-2] - A[i_5][i_6][i_6+i_4-1] * 6;
                }
            }
        }
    }
...
kernel_4216311315_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
