...
void kernel_2436251223_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_0] = E[i_1-1] + F[i_1][i_0-1] - A[i_0+2] * A[i_1+1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            B[i_2+1][i_3] = F[i_3][i_2] - F[i_2+1][i_3] + G[i_3+i_2][0] * F[i_2][i_3-1] - F[i_3][i_2+1] + G[i_3][i_2-1] + E[i_3] + B[i_2+1][i_3-2] * 6;
        }
        for (int i_4 = 2; i_4 < min(-i_2+PB_L, i_2); i_4++) {
            A[i_4-2] = G[i_4][i_2] * G[i_4-1][i_4+i_2] + H[i_2] - H[i_2] - G[i_4][i_2] * H[i_4] * 3;
        }
    }
    for (int i_5 = 0; i_5 < PB_L; i_5++) {
        for (int i_6 = i_5; i_6 < PB_M-3; i_6++) {
            D[i_6+1] = F[i_6][i_5] * F[i_6][i_5] + D[i_6+3] + 2;
        }
    }
#pragma endscop
...
kernel_2436251223_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
