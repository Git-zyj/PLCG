...
void kernel_2222111213_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0][i_0] = C[i_0][i_0-1] + A[i_0-1][i_0-1] + A[i_0+1][i_0+1] - 4;
    }
    for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L; i_2++) {
            A[i_1-1][i_1-1] = C[i_2][i_1-1] * 1;
        }
    }
#pragma endscop
...
kernel_2222111213_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
