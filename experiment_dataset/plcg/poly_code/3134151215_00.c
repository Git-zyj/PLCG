...
void kernel_3134151215_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(min(i_0, PB_M-1), -i_0+PB_L-1); i_1++) {
            A[i_1] = B[i_1][i_0-1] - B[i_1][i_0] + B[i_1][i_0] + B[i_1][i_0] + B[i_1+i_0+1][i_0] * A[i_1+1] + A[i_1-1] - 3;
        }
    }
#pragma endscop
...
kernel_3134151215_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
