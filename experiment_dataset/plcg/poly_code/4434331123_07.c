...
void kernel_4434331123_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
                for (int i_3 = 1; i_3 < -i_0+PB_M-1; i_3++) {
                    A[i_2-1][i_1+i_0][i_2][i_3] = E[i_2+i_0][i_3+1][i_1][i_0-1] * F[i_1][i_3+i_0+1][i_0][i_2] * G[i_2][i_0][i_3+i_1][i_1] + H[i_3-1][i_1-1][i_0][i_1][i_2-1] + I[i_2][i_3][i_2] - 3;
                }
            }
        }
    }
        ...
        for (int i_6 = 0; i_6 < -i_4+PB_M; i_6++) {
            for (int i_7 = 1; i_7 < PB_M; i_7++) {
                for (int i_8 = 1; i_8 < min(-i_7+PB_N+1, PB_M-1); i_8++) {
                    C[i_6][i_7][i_8-1] = F[i_8+i_7-1][i_4][i_6+i_4-1][1] * F[i_6][i_8+1][i_4-1][i_4] + G[i_8-1][i_7][i_4][i_4] * I[i_8][i_6+i_4][i_7] * A[i_7][i_6+i_4-1][i_7+1][i_8] - 4;
                }
            }
            for (int i_9 = 0; i_9 < PB_M; i_9++) {
                D[i_6][0] = K[i_6+1][i_9] * L[i_4][i_6] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4434331123_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
