...
void kernel_4332112321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_N+1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0-2][i_0] = A[i_0-3][i_0-1] * A[i_0-1][i_0+1] + A[i_0-4][i_0-2] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 2; i_3 < PB_Q+2; i_3++) {
            for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
                B[i_2][i_4][i_4] = D[i_4][i_3-2][i_2] + B[i_2+1][i_4+1][i_4+1] - 6;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_Q-1; i_5++) {
        for (int i_6 = 0; i_6 < i_5; i_6++) {
            for (int i_7 = i_5; i_7 < PB_M-1; i_7++) {
                C[i_7][i_6][i_5] = D[i_7-2][i_6][i_5+1] - C[i_7][i_6+2][i_5-2] * C[i_7+1][i_6][i_5] + C[i_7][i_6][i_5+1] * C[i_7-1][i_6][i_5] * C[i_7][i_6+2][i_5] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4332112321_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
