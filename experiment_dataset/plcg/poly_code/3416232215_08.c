...
void kernel_3416232215_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_M, i_0), PB_M-2); i_1++) {
            A[i_0-1][i_1] = E[i_1+1][i_0] * F[i_1+2][i_1+i_0] - E[i_1+i_0-1][i_1] + 3;
        }
        for (int i_2 = max(2, i_0); i_2 < PB_M; i_2++) {
            for (int i_3 = 2; i_3 < min(PB_M, -i_2+PB_P); i_3++) {
                B[i_2][i_2-1][i_0] = G[i_3-2][i_0+1][i_3+i_2] - E[i_2][i_3] + H[i_3][i_2-1][i_0] - B[i_2-1][i_2-2][i_0] - B[i_3][i_3-1][i_2-1] + B[i_2][i_2-1][i_0-1] - 1;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 2; i_5 < min(i_4, -i_4+PB_L+2); i_5++) {
            for (int i_6 = 2; i_6 < PB_M-1; i_6++) {
                B[i_5][i_5-1][i_4+1] = G[i_6+1][i_4][i_5-1] + H[i_6][i_5+i_4-2][i_6-2] * 5;
            }
            for (int i_7 = i_5; i_7 < -i_5+PB_L+2; i_7++) {
                B[i_5][i_5-1][i_4] = G[i_4-1][i_7+i_5-2][i_5] - G[i_7][i_5-1][i_4] + G[i_5][i_4][i_5-1] * F[i_5][i_7] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3416232215_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
