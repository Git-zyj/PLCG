...
void kernel_3334151121_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            A[i_1+i_0] = D[i_0-1] - E[i_1][i_0] * E[i_1+i_0-1][i_0] - E[i_1][i_0-1] - E[i_0][0] + E[i_0][i_1+i_0-1] * E[i_1][i_0+1] - D[i_1] * A[i_1+i_0-1] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
            B[i_2-1][i_3+1] = D[i_3] - E[i_3+i_2][i_3-1] - E[i_2-1][i_3] * D[i_3] * 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 2; i_5 < min(min(PB_N-1, i_4), -i_4+PB_M); i_5++) {
            C[i_5] = E[i_5+i_4][i_4] + E[i_5][i_4+1] * E[i_5+1][i_4-1] * C[i_5+1] + C[i_5-2] * 6;
        }
    }
#pragma endscop
...
kernel_3334151121_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
