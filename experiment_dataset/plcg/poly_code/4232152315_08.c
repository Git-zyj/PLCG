...
void kernel_4232152315_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-3; i_2++) {
                A[i_0-2][i_0][i_2+2] = C[i_1][i_1][i_2] - C[i_1+2][i_0][1] - D[i_1][i_0-1] + C[i_2+2][i_0+2][i_1+1] + C[i_2][i_0+1][i_1-1] - C[i_1+1][i_2][i_0+1] * A[i_0-1][i_0+1][i_1+2] + A[i_0-1][i_0+1][i_2+1] * A[i_0-2][i_0][i_2+3] + 5;
            }
        }
        for (int i_3 = 2; i_3 < PB_Q-1; i_3++) {
            for (int i_4 = 2; i_4 < min(PB_M, -i_0+PB_L); i_4++) {
                A[i_0-1][i_0+1][i_3+1] = D[i_4-2][i_3] * C[i_3-2][i_4+2][i_0+1] * D[i_4][i_3] * C[i_3][i_4+i_0][0] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4232152315_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
