...
void kernel_3332251313_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P-1, -i_0+PB_L+1); i_1++) {
            B[i_1-1][i_1+i_0-1] = D[i_1][i_0+1] + E[i_0+1][i_1+1] - D[i_0][i_1] + F[i_1] * G[1] + E[i_1+1][i_1+1] - B[i_1][i_1+i_0-1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 1; i_4 < min(min(-i_2+PB_L, -i_3+PB_L-1), PB_M-1); i_4++) {
                B[i_4][i_4+i_2] = H[i_4][i_3+1][i_2-1] * I[i_2][i_3][i_3] - I[i_3][i_4][i_2] + I[i_3][i_4+1][i_2+1] * B[i_4+1][i_4+i_3+1] * B[i_4-1][i_4+i_3-2] - B[i_4][i_4+i_3+1] + 2;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_N-1; i_5++) {
        C[i_5][i_5] = G[i_5+1] + F[i_5] + F[i_5] - G[0] * C[i_5-1][i_5-1] - 5;
    }
#pragma endscop
...
kernel_3332251313_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
