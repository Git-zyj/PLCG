...
void kernel_2426111321_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = A[i_0+2] * 2;
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            B[i_0-1][i_0] = A[i_0] * A[i_1] - 4;
        }
        C[i_0] = B[i_0][i_0+1] * 6;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L-1, -i_2+PB_M); i_3++) {
            D[i_2-1][i_3] = E[i_3+i_2][1] - E[i_3][i_2] + D[i_2][i_3] + D[i_2-1][i_3-2] + D[i_2-2][i_3+1] + D[i_2-2][i_3-2] + D[i_2-2][i_3] * 6;
        }
    }
#pragma endscop
...
kernel_2426111321_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
