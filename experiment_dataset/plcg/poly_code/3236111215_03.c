...
void kernel_3236111215_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_M-1); i_3++) {
            B[i_3] = C[i_3+i_2+1][0] - B[i_3-1] - B[i_3+1] + 4;
        }
    }
#pragma endscop
...
kernel_3236111215_03(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
