...
void kernel_3324112221_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_1+i_0][i_2][i_0] = A[i_1+i_0-1][i_2][i_0] - 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_P-2; i_4++) {
            for (int i_5 = 1; i_5 < PB_Q+1; i_5++) {
                C[i_3+1][i_4-1][i_5-1] = 4;
            }
            for (int i_6 = 3; i_6 < PB_Q+1; i_6++) {
                C[i_3-1][i_4-1][i_6-1] = D[i_6][i_4+1][i_3+1] - D[i_3][i_6-2][i_4+2] + C[i_3-2][i_4-1][i_6-1] + C[i_3-1][i_4-1][i_6-3] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3324112221_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
