...
void kernel_3422131113_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        A[i_0] = E[i_0] - E[i_0] - E[i_0] + E[i_0] * A[i_0-1] * 1;
        for (int i_1 = 0; i_1 < min(PB_N, -i_0+PB_L-1); i_1++) {
            B[i_1][i_1+1] = F[i_1][i_1+i_0+1][0] + 1;
        }
    ...
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            for (int i_4 = i_3; i_4 < min(-i_3+PB_L, PB_P-1); i_4++) {
                D[i_2][i_4+i_3-1][i_4] = G[i_4][i_3] + F[i_4-1][i_2-1][i_3] + F[i_4][i_3][i_2] - F[i_4][i_2-1][i_3] + G[i_4+1][i_2] * D[i_2-1][i_4+i_3][i_4+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3422131113_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
