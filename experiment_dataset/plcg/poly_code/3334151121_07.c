...
void kernel_3334151121_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_N+1, PB_P-1); i_1++) {
            for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
                A[i_0][i_1][i_0] = D[i_1-1][i_0][i_2] - D[i_2][i_1+i_0-1][i_0] + D[i_0][i_0+1][i_1-1] * E[i_2-1][i_0][i_1][i_2+1] + D[i_2+1][i_1][i_1-1] * A[i_0+2][i_1+1][i_0+2] + 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
        for (int i_4 = i_3; i_4 < min(-i_3+PB_N, PB_P); i_4++) {
            for (int i_5 = 0; i_5 < min(PB_P, -i_4+PB_N-1); i_5++) {
                B[i_4][i_3][i_5] = E[i_3][i_4][i_4][i_5] - D[i_5][i_3][i_4+i_3] - D[i_4-1][i_3][i_4] * D[i_5][i_3+1][i_4] + D[i_5+i_4][i_3-1][i_3] - D[i_5+i_4+1][i_3][i_3+1] * D[i_3][i_5][i_4] * F[i_5][1] * 2;
            }
        }
        for (int i_6 = 0; i_6 < PB_N-1; i_6++) {
            for (int i_7 = 0; i_7 < min(-i_3+PB_P+1, -i_6+PB_L); i_7++) {
                C[i_7+i_3][i_7+i_6][i_7+i_3-1] = D[i_7][i_6+1][i_3+1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3334151121_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
