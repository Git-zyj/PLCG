...
void kernel_4224111211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_Q, -i_0+PB_L); i_1++) {
            ...
            for (int i_2 = 1; i_2 < min(min(PB_Q, i_0), -i_1+PB_L); i_2++) {
                B[i_2-1][i_1][i_0] = C[i_0][i_2][i_1] + A[i_2+i_1] + B[i_2-1][i_1-1][i_0] * B[i_2][i_1][i_0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4224111211_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
