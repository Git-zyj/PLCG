...
static void print_array(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,int yn,int zn,DATA_TYPE POLYBENCH_3D(N,xN,yN,zN,xn,yn,zn))
{
int i_0, i_1, i_2, i_3, i_4;
        ...
        for (int i_1 = 1; i_1 < PB_M-4; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M-3; i_2++) {
                for (int i_3 = 2; i_3 < -i_0+PB_N; i_3++) {
                    C[i_1+4][i_2+i_1+3][i_3+2] = E[i_0][i_3][i_1+1][0] - F[i_1][i_3][i_2] - G[i_1-1][i_0][i_3][i_2][i_2] * H[i_3+i_0][i_1+1][i_3-2][i_2] + I[i_3][i_0][i_1][i_2+2] * I[i_2][i_0+1][i_3][i_1] - 1;
                }
            }
        }
        B[i_0-1] = J[i_0] - K[i_0-2] - L[i_0][i_0] - B[i_0-3] - B[i_0-2] - 1;
    }
    for (int i_4 = 1; i_4 < PB_M-4; i_4++) {
        for (int i_5 = 0; i_5 < -i_4+PB_M-2; i_5++) {
            for (int i_6 = i_4; i_6 < min(PB_L-2, -i_4+PB_L); i_6++) {
                C[i_4+2][i_5+i_4][i_6+2] = M[i_5][i_6+i_4][i_6-1] * M[i_5][i_6+2][i_4] - N[i_5][i_6][i_4+1] - M[i_6][i_5][i_4] - C[i_4+4][i_5+i_4+2][i_6+1] * 2;
            }
        }
        for (int i_7 = i_4; i_7 < -i_4+PB_Q-1; i_7++) {
            for (int i_8 = 0; i_8 < min(-i_7+PB_M, PB_L-2); i_8++) {
                C[i_7+2][i_8+i_7][i_4+2] = F[i_7+i_4][i_8][i_7] * N[i_7+2][i_8+2][i_4-1] - F[i_8+1][i_7+i_4][i_7+i_4+1] + M[i_7][i_8][i_7] - N[i_7+1][i_4-1][i_7] - 6;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,zc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4422352225_01(xc,yc,zc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,yn,zn,POLYBENCH_ARRAY(N));
