...
void kernel_3424332325_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 3; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                B[i_1-3][i_1] = E[i_0][i_2][i_1] * 5;
            }
        }
        for (int i_3 = 3; i_3 < PB_L-1; i_3++) {
            B[i_0-1][i_0+2] = F[i_3-2][i_0] - F[i_0+1][i_0+1] - G[i_3+1][i_0] * B[i_0-2][i_0+1] * B[i_0][i_0+3] + B[i_3-3][i_3] - 3;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-4; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            B[i_4+1][i_4+4] = G[i_5][i_4] + H[i_4] - F[i_5+2][i_4] * 6;
        }
        for (int i_6 = 0; i_6 < i_4; i_6++) {
            B[i_4-1][i_4+2] = H[i_4-1] + I[i_6+2][i_4] * 2;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3424332325_07(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
