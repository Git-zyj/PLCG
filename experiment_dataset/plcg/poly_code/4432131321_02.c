...
void kernel_4432131321_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] * E[i_0] - E[i_0] + E[i_0] + E[i_0] * A[i_0+2] + 4;
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_1][i_2][i_1+1] = F[i_2][i_1][i_0-1] * F[i_0][i_1][i_2] * B[i_1+2][i_2][i_1+3] * B[i_2][i_1-1][i_2+1] + 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            C[i_3-1][i_4] = G[i_3][i_4] + G[i_4][0] + G[i_4][i_3+1] * G[i_3-1][i_4] - C[i_4][i_3] + C[i_3-2][i_4+1] + 1;
        }
    }
    for (int i_5 = 2; i_5 < PB_Q-2; i_5++) {
        for (int i_6 = 3; i_6 < PB_Q-2; i_6++) {
            for (int i_7 = 2; i_7 < PB_P-1; i_7++) {
                for (int i_8 = 1; i_8 < min(PB_P-3, i_7); i_8++) {
                    D[i_8-1][i_6-1][i_7+1][i_5] = H[i_6+1][i_8][i_7+1][i_6] - D[i_8-1][i_6-3][i_7+1][i_5-2] + D[i_6-2][i_8-1][i_7-1][i_5+2] * D[i_6-1][i_8+1][i_7+1][i_5-2] - D[i_5-1][i_7][i_8+3][i_6] * D[i_8-1][i_7-2][i_5-1][i_6+2] * D[i_5-2][i_6-1][i_7+1][i_8] * 3;
                }
            }
        }
    }
...
kernel_4432131321_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
