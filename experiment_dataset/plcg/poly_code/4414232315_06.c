...
void kernel_4414232315_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(min(i_0, -i_0+PB_P), PB_M-2); i_2++) {
                A[i_1-1][i_2] = E[i_2][i_1][i_2+i_0] * F[i_0][i_1] + G[i_2+2][i_2] * E[i_1][i_2-1][i_0] + A[i_2-1][i_1-1] + A[i_2-1][i_0-1] + A[i_2][i_1] + 2;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = 2; i_5 < PB_Q; i_5++) {
                for (int i_6 = 1; i_6 < PB_Q; i_6++) {
                    A[i_5][i_6] = H[i_3+1][i_3][i_6-1][i_5-2] + H[i_6][i_5][i_4-1][i_3+2] * I[i_6][i_4][i_5][1][i_3-2] + J[i_6][i_4-1][i_4][i_3] * 5;
                }
            }
        }
        for (int i_7 = 2; i_7 < PB_L-2; i_7++) {
            for (int i_8 = 1; i_8 < min(-i_3+PB_P, -i_7+PB_L); i_8++) {
                for (int i_9 = 0; i_9 < PB_M; i_9++) {
                    A[i_8-1][i_3-1] = I[i_9][i_8+i_7][i_8+i_3][i_7-2][i_3-1] - 4;
                }
            }
            for (int j_0 = 1; j_0 < i_3; j_0++) {
                A[j_0-1][i_7] = E[j_0][i_7][i_3] - K[i_7+2][i_3][j_0] - 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4414232315_06(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
