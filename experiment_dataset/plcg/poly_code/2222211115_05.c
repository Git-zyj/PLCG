...
void kernel_2222211115_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_L+1; i_2++) {
            B[i_1][i_2+i_1-1] = C[i_1][i_2] - A[i_2+1] - 3;
        }
    }
#pragma endscop
...
kernel_2222211115_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
