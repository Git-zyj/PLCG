...
void kernel_3422352311_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_M+1); i_1++) {
            A[i_1+2] = E[i_1][i_0] + F[i_1][i_0-2] - F[i_0][i_1] + A[i_1+3] + D[0] + 6;
            for (int i_2 = 0; i_2 < min(-i_1+PB_M-1, -i_0+PB_M-3); i_2++) {
                B[i_1+i_0-1][i_2+i_0+2][i_1] = G[i_2][i_0][i_1] + H[i_2][i_1][i_0][i_2+i_1] + I[i_0][i_2][i_1] * G[i_2+2][i_0-1][i_2+1] - G[i_1][i_2][i_0] - A[i_1+3] - A[i_2+2] + A[i_2+3] - B[i_2+i_1][i_2+i_0+3][i_1] + B[i_1+i_0-2][i_2+i_1+1][i_0] + B[i_1+i_0-1][i_2+i_0+3][i_1] + B[i_1+i_0-2][i_2+i_0+1][i_1] - 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M, PB_L); i_4++) {
            C[i_4-2] = E[i_3][i_4+1] + E[i_3][2] * J[i_3] * K[i_3+1][0] * F[i_4+i_3][0] * F[i_4][i_3+1] + E[i_4][i_3+2] + A[i_4+2] + 3;
        }
        D[0] = L[i_3] - M[i_3] + L[i_3] + L[i_3+1] + D[0] - D[0] + 3;
    }
#pragma endscop
...
kernel_3422352311_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M));
