...
void kernel_3114252223_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_1+i_0-1] = B[0] * C[i_1][i_0+1] - D[i_1-2][i_0+1] * C[i_1][i_0+2] + B[i_1+i_0+1] + A[i_1+i_0-3] - A[i_1+i_0-2] * 1;
        }
    }
#pragma endscop
...
kernel_3114252223_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
