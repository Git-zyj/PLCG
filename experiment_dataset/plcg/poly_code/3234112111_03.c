...
void kernel_3234112111_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q+1; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q+1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_Q+1, PB_L); i_2++) {
                A[i_2][i_2+i_0-1][i_1-2] = A[i_2][i_2+i_0-1][i_1-1] * 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < min(PB_L, -i_3+PB_Q+2); i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_L, -i_4+PB_L); i_5++) {
                B[i_5+i_3][i_4][i_3] = C[i_4-1][i_5][i_4+i_3-2] - B[i_5+i_4][i_3-1][i_4] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3234112111_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
