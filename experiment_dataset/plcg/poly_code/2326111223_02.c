...
void kernel_2326111223_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 3; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            C[i_2-1][i_2] = D[i_2-1] + C[i_2+1][i_2+2] - C[i_2-2][i_2-1] * C[i_2-3][i_2-2] + 1;
        }
    }
#pragma endscop
...
kernel_2326111223_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
