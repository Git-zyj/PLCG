...
void kernel_3236211225_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_2+1][i_1][i_0] = A[i_2+1][i_1][i_0-2] + A[i_2+1][i_1-1][i_0-2] - 4;
            }
        }
        for (int i_3 = 1; i_3 < min(i_0, PB_M-1); i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                A[i_3+1][i_4][i_0] = C[i_3-1][i_4][i_0-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3236211225_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
