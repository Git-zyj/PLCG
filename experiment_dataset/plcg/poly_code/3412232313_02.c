...
void kernel_3412232313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0] = E[i_0] + F[i_0+2] - A[i_0+1] - A[i_0-1] - A[i_0-1] - 6;
    }
    for (int i_1 = 3; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 3; i_2 < -i_1+PB_M; i_2++) {
            for (int i_3 = 2; i_3 < -i_2+PB_Q-2; i_3++) {
                B[i_1][i_3][i_2-2][i_3+i_2+1] = G[i_3][i_2] - H[i_2-1][i_3-2][i_1][i_3] + I[i_2+i_1][i_3-2][i_3] - B[i_1][i_3+1][i_2-2][i_3+i_2+2] * B[i_1][i_3-1][i_2-3][i_3+i_2-1] * B[i_1][i_3][i_2-1][i_3+i_2+2] * B[i_1+1][i_3+1][i_2-2][i_3+i_2+2] * B[i_1-1][i_3][i_2-2][i_3+i_2+1] - 6;
            }
        }
        C[i_1-2] = E[0] + C[i_1-1] - C[i_1-3] + C[i_1-1] + 4;
        for (int i_4 = 2; i_4 < -i_1+PB_L+1; i_4++) {
            for (int i_5 = 0; i_5 < -i_1+PB_L; i_5++) {
                D[i_4][i_5][i_5+i_1] = J[i_5][i_4-2] + I[i_1][i_4-1][0] + K[i_5+2][i_1][0] - K[i_5][i_4][i_1] + L[i_1-1][i_5][i_4][i_4] * D[i_5][i_4][i_4+i_1-1] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_3412232313_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L));
