...
void kernel_4332152225_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_M+1, PB_M-1); i_3++) {
                    A[i_1][i_3+2][i_2][i_0+1] = D[i_3][i_1][i_2][i_0-1] + D[i_1-2][i_2][i_0-2][i_3] * D[i_2][i_1+2][i_3][i_0] + E[i_0][i_1][i_3] + D[i_3+1][i_2-1][i_1+i_0][i_1-2] + D[i_1][i_0][i_3+i_2-1][i_0] + A[i_1][i_3+2][i_2+2][i_0+1] * A[i_1+1][i_0+2][i_3+1][i_2+1] - 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        B[i_4] = F[i_4+1] + F[i_4] + B[i_4+1] + B[i_4-1] * 5;
    }
    for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
        B[i_5] = F[i_5-1] - F[i_5-2] * F[i_5+2] * G[i_5-1][i_5+2] - F[i_5+2] + 5;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4332152225_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
