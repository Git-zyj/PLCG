...
void kernel_4214132321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-2); i_1++) {
            A[i_1+1] = C[i_1][i_0] + C[i_1][i_0] * C[i_0][0] + A[i_1+2] - A[i_1] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-3; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_M-3, PB_L-1); i_3++) {
            B[i_3+i_2] = C[i_2][i_3+1] - D[0] + B[i_3+i_2-2] + B[i_3+i_2+3] - B[i_3+i_2-1] + 1;
        }
    }
#pragma endscop
...
kernel_4214132321_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
