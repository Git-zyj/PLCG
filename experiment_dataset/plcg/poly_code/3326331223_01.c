...
void kernel_3326331223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M; i_2++) {
                A[i_2][i_1] = D[i_2][i_0+1][i_1+1] * 5;
            }
        }
        B[i_0-1][i_0-1] = E[i_0-1] + F[i_0] + E[i_0+1] - E[1] * B[i_0][i_0] - B[i_0+1][i_0+1] - B[i_0-2][i_0-2] + 3;
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_P; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q-2; i_5++) {
                A[i_4][i_5+2] = D[i_5][i_4][i_4] + G[i_5][i_5] + H[i_3][i_4-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3326331223_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
