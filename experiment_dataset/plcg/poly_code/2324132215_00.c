...
void kernel_2324132215_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_P+1; i_1++) {
            A[i_0][i_1-1] = D[i_0] - A[i_0+1][i_1-1] - A[i_0][i_1-2] - 1;
        }
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            C[i_0+2][i_2+1] = E[i_2][i_0+2] + E[i_2+2][i_0] * E[i_2][0] * 4;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_L, i_3); i_4++) {
            C[i_3+2][i_4] = E[i_4][i_3] * E[i_4][i_3+1] - E[i_4+i_3][1] + C[i_3+3][i_4-1] - C[i_3+2][i_4-1] * 4;
        }
    }
#pragma endscop
...
kernel_2324132215_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
