...
void kernel_2432311223_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
            B[i_2] = E[i_1][i_2] * B[i_2+2] + B[i_2-1] + 5;
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            C[i_3] = F[i_3] + A[i_1-1] * C[i_3+1] * C[i_3-1] - 1;
        }
        for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
            D[i_1-1][i_4] = F[i_4+1] * E[i_4][i_1+1] + D[i_1-1][i_4-2] + 4;
        }
    }
#pragma endscop
...
kernel_2432311223_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
