...
void kernel_4234211123_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < PB_L+2; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = 2; i_3 < PB_M; i_3++) {
                    A[i_3][i_2][i_0] = A[i_3-2][i_0-2][i_1-2] * 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 0; i_5 < min(PB_P, -i_4+PB_M); i_5++) {
            B[i_5+1][0] = C[i_5+i_4][i_4-1] * B[i_4+2][0] * 6;
        }
    }
#pragma endscop
...
kernel_4234211123_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
