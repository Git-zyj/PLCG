...
void kernel_2224212311_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_M; i_3++) {
            B[i_3+i_2-2][i_3] = C[i_3][i_2] + B[i_3+i_2-3][i_3] * B[i_3+i_2][i_3+1] * B[i_3+i_2-3][i_3-1] + 2;
        }
    }
#pragma endscop
...
kernel_2224212311_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
