...
void kernel_4436352121_05(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            for (int i_2 = i_0; i_2 < -i_0+PB_M-1; i_2++) {
                for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
                    C[i_1][i_2+i_0+1][i_2+1][i_0-1] = E[i_1+2][i_3+2][i_0-1][i_2] + F[i_1][i_2][i_0][i_0+1] + F[i_0][i_3][i_2][i_1] * G[i_3-1][i_1][i_2] - F[i_3][i_0][i_1-2][i_2+1] - 2;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-3; i_5++) {
            for (int i_6 = 0; i_6 < min(min(i_4, PB_M-2), -i_4+PB_Q); i_6++) {
                for (int i_7 = 2; i_7 < -i_5+PB_M-1; i_7++) {
                    B[i_4][i_6+2][i_4][i_7] = F[i_7][i_4+1][i_6][i_5] - G[i_4+1][i_6][i_5] + H[i_5][i_7][i_6][i_6+i_4] * I[i_7+i_5+1][i_4][i_6] * B[i_4][i_5+3][i_4][i_7-2] * 2;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_Q-2; i_8++) {
        for (int i_9 = i_8; i_9 < min(-i_8+PB_P+1, PB_Q-1); i_9++) {
            for (int j_0 = max(i_9, 2); j_0 < min(-i_9+PB_M, PB_M-2); j_0++) {
                for (int j_1 = j_0; j_1 < PB_M; j_1++) {
                    C[j_1+1][j_0+i_9][i_9-1][j_0] = F[j_0+2][j_1-1][i_9+i_8-1][j_1] * H[j_0-2][j_1][i_8+2][i_9+1] + J[j_1][i_9][i_8] * H[i_9][i_8-1][j_0][j_1] + 4;
                }
            }
        }
    }
    for (int j_2 = 0; j_2 < PB_M-2; j_2++) {
        D[j_2+1] = K[j_2+2] + L[j_2] * M[j_2] - K[j_2] - 5;
    }
#pragma endscop
...
kernel_4436352121_05(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M));
