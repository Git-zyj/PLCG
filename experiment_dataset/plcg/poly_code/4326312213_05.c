...
void kernel_4326312213_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_L+1); i_1++) {
            A[i_0+2][i_1] = C[i_0][i_1+i_0-1] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        ...
        for (int i_3 = 2; i_3 < min(-i_2+PB_L+1, PB_M); i_3++) {
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                C[i_3][i_3+i_2-1] = C[i_4][i_4+i_3-2] + C[i_4+1][i_4+i_3] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4326312213_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
