...
void kernel_2232252215_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_M, -i_0+PB_N); i_1++) {
            A[i_0][i_1] = C[i_0-2][i_1] - C[i_1+i_0][i_0-2] + D[i_0] - C[i_0][i_1] * E[i_1-1][i_0+2] * A[i_1-1][i_0] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            A[i_3][i_2] = D[i_2] + E[i_3][i_2+1] * C[i_3-2][i_2+1] * E[i_3][i_2-2] - A[i_2+1][i_3] - 3;
        }
    }
#pragma endscop
...
kernel_2232252215_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
