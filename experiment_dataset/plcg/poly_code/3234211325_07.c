...
void kernel_3234211325_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_N; i_2++) {
                A[i_0][i_2][i_0] = C[i_2][i_1+i_0] * A[i_0-2][i_2][i_0-2] - A[i_0][i_2+2][i_0] * A[i_0-1][i_2][i_0-1] * 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        B[i_3] = B[i_3+1] * B[i_3-2] * B[i_3-1] * 2;
    }
#pragma endscop
...
kernel_3234211325_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
