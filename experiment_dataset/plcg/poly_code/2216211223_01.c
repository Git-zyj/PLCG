...
void kernel_2216211223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[i_0-1][i_1] = A[i_0-3][i_1+2] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M+1; i_3++) {
            B[i_2][i_3] = C[i_3-1][i_2+1] * B[i_2][i_3-2] - B[i_2-2][i_3+2] - 6;
        }
    }
#pragma endscop
...
kernel_2216211223_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
