...
void kernel_3232311125_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_N+1; i_1++) {
            ...
            for (int i_3 = 1; i_3 < -i_1+PB_N+1; i_3++) {
                B[i_0-1][i_1] = C[i_3+i_1-1][i_3-1][0] * B[i_3-1][i_1-1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3232311125_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
