...
void kernel_4234111125_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_2 = i_1; i_2 < PB_L-1; i_2++) {
            for (int i_3 = i_1 - 1; i_3 >= 0; i_3--) {
                for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
                    B[i_4+1][i_3][i_2] = B[i_3][i_4-1][i_2+1] - 4;
                }
            }
        }
    }
...
kernel_4234111125_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
