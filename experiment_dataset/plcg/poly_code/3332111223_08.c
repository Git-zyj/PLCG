...
void kernel_3332111223_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_0+1][i_1][i_1] = A[i_0+1][i_1-2][i_1-2] * A[i_0+2][i_1][i_1] + 2;
            }
        }
        for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_4][i_3+1][i_0+1] = B[i_4][i_3][i_0+1] * B[i_4][i_3+2][i_0] + 1;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_Q; i_5++) {
        for (int i_6 = i_5; i_6 < PB_Q+1; i_6++) {
            for (int i_7 = 0; i_7 < PB_P-3; i_7++) {
                C[i_6][i_7+1][i_5] = D[i_5-1][1][i_6-1] - C[i_6-1][i_7+3][i_5] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3332111223_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
