...
void kernel_2212152215_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            B[i_1] = C[i_1][i_0][i_0] + C[i_0][i_1][0] - D[i_1][0] + D[i_0][i_1-1] + E[i_1-2] + D[i_1][i_0-2] + B[i_1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        B[i_2] = E[i_2-1] - E[i_2] - E[i_2] - E[i_2] + B[i_2-1] - 2;
    }
#pragma endscop
...
kernel_2212152215_04(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
