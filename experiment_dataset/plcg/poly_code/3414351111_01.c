...
void kernel_3414351111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0-1] = E[i_0] + F[i_0] - G[i_0+1] + G[i_0+1] + G[0] + 4;
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            B[i_1][i_0-1][0] = H[i_1+1][i_0] * A[i_1-1] + 2;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M-1, PB_L-1); i_2++) {
            for (int i_3 = 1; i_3 < min(-i_2+PB_L-1, PB_L-2); i_3++) {
                C[i_3][i_2] = H[i_2][i_3] - I[i_0][i_3][i_2] + J[i_2+1][i_0+1][1] * J[i_0][i_3][i_0] + I[i_2+i_0+1][i_3][i_3-1] - 1;
                D[i_2][i_3+1] = H[i_2][i_3] * J[i_3+1][1][i_2] * K[i_0-1][i_2] - L[i_0][i_3+i_2+1][i_3][0] + H[i_2][i_3] + K[i_3][i_2] * J[i_3+i_2][i_0][i_2] - D[i_2][i_3+2] * 2;
            }
        }
    }
#pragma endscop
