...
void kernel_2216112113_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_M, -i_0+PB_N-2); i_1++) {
            B[i_1+i_0+2][i_0-1] = C[i_1][i_0] * 3;
        }
        for (int i_2 = 0; i_2 < min(min(-i_0+PB_N-1, i_0), PB_M-1); i_2++) {
            B[i_2+i_0][i_0-2] = D[i_2+1] * B[i_2+i_0+1][i_0-1] + 1;
        }
    }
#pragma endscop
...
kernel_2216112113_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
