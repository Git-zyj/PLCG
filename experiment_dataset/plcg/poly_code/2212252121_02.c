...
void kernel_2212252121_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1][i_0] = C[i_1+i_0][i_1] * C[i_1+2][i_0] * C[i_1][i_1] * C[i_1+i_0][i_1] + A[i_1-2][i_0+1] * 5;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_N+1, PB_M-1); i_2++) {
            A[i_2+1][i_0] = C[i_2][i_0+1] + C[i_2][i_0] - D[i_2][i_0+1] + D[i_0][i_2] * D[i_2+i_0-1][0] - D[i_2][i_0-2] - 6;
        }
    }
#pragma endscop
...
kernel_2212252121_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
