...
void kernel_3312212121_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_0][i_2][i_1] = A[i_0-2][i_2+1][i_1] * A[i_0-1][i_2][i_1] - 5;
            }
        }
        for (int i_3 = 1; i_3 < min(-i_0+PB_M, PB_L-2); i_3++) {
            B[i_0-2][i_3-1] = D[i_0-2][i_3+i_0][i_3+2] * 1;
            for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
                C[i_4][i_3+2][i_0] = C[i_3+2][i_4+2][i_0+1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3312212121_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
