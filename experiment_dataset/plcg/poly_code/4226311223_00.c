...
void kernel_4226311223_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L+2; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_L, -i_1+PB_L+2); i_3++) {
                    A[i_1][i_3+i_2][i_1] = A[i_2][i_3+i_1-2][i_2] + A[i_1-1][i_3+i_2-2][i_1-1] - 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-3; i_4++) {
        B[i_4+1] = C[i_4] - B[i_4-1] + B[i_4+3] + 1;
    }
#pragma endscop
...
kernel_4226311223_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
