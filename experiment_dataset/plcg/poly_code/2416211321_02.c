...
void kernel_2416211321_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = C[i_0+1] + C[i_0-1] - 3;
        for (int i_1 = max(3, i_0); i_1 < PB_L+1; i_1++) {
            B[i_1-1][i_0] = E[i_1-1][i_0] * B[i_1-3][i_0] * 6;
        }
        C[i_0-1] = A[i_0] + 1;
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 3; i_3 < PB_L; i_3++) {
            D[i_2][i_3-1] = E[i_3][i_2] + A[i_3] - C[i_3-3] + D[i_2][i_3] - D[i_2+2][i_3-1] + D[i_2-1][i_3-1] + D[i_2][i_3-3] + D[i_2][i_3-2] * 5;
        }
    }
#pragma endscop
...
kernel_2416211321_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
