...
void kernel_3412111221_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            A[i_1][i_0-1] = B[i_0][i_1+1] - B[i_0+2][i_1+1] + B[i_1][i_0] * 3;
            B[i_0][i_1+1] = A[i_1][i_0-1] * A[i_1-1][i_0-1] + B[i_1][i_0-1] + 6;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_L+1; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_0+PB_M, i_2); i_3++) {
                C[i_3+i_0][i_2+i_0-1][i_2-1] = E[i_3][i_0+1] * 1;
            }
        }
    }
...
kernel_3412111221_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
