...
void kernel_3222111311_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        B[i_1] = C[0] + A[i_1] - B[i_1+1] - B[i_1-1] * B[i_1+1] - 1;
    }
#pragma endscop
...
kernel_3222111311_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
