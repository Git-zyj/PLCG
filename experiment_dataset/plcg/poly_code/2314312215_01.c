...
void kernel_2314312215_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            C[i_1+1][i_0] = C[i_1+1][i_0+1] + C[i_1][i_0-1] - 3;
        }
    }
#pragma endscop
...
kernel_2314312215_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
