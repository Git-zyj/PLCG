...
#endif
static void init_array(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,int wj,DATA_TYPE POLYBENCH_5D(J,xJ,yJ,zJ,vJ,wJ,xj,yj,zj,vj,wj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int seed)
{
srand(seed);
...
static void print_array(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,int wj,DATA_TYPE POLYBENCH_5D(J,xJ,yJ,zJ,vJ,wJ,xj,yj,zj,vj,wj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4416352115_08(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,int wj,DATA_TYPE POLYBENCH_5D(J,xJ,yJ,zJ,vJ,wJ,xj,yj,zj,vj,wj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = i_1; i_2 < -i_0+PB_L; i_2++) {
                C[i_1][i_2+1] = E[i_2+i_0][i_1] + F[i_2][i_0][i_1] - 5;
            }
        }
        for (int i_3 = i_0; i_3 < PB_Q-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_Q-2; i_4++) {
                for (int i_5 = 1; i_5 < min(min(-i_3+PB_M, -i_4+PB_Q-2), -i_0+PB_P); i_5++) {
                    C[i_5][i_3] = F[i_4][i_5+i_3][i_0] * G[i_5+i_4][i_3+2][i_0][i_5-1] + H[i_5][i_0-2][i_5+i_4+2][i_0] - I[i_4][i_3-1][i_5] * I[i_4+1][i_3-1][i_5] * J[i_4][i_3][i_5+i_0][i_0-2][i_5+i_3-2] * 5;
                }
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
        for (int i_7 = i_6; i_7 < -i_6+PB_L-1; i_7++) {
            C[i_7][i_6] = K[i_7][i_6] - L[i_6-2][i_7+i_6] - L[i_7+i_6][i_7+i_6+1] * E[i_7][0] * L[i_7][i_6] * C[i_7][i_6-1] - 4;
        }
        for (int i_8 = 2; i_8 < min(i_6, -i_6+PB_L); i_8++) {
            D[i_8-1] = L[i_8-1][i_6] - E[i_8][i_6] + E[i_8+i_6][i_8] - L[i_8][i_6] + K[i_8][i_6-1] - E[i_8+i_6-1][i_8] * M[i_8-2] - 3;
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4416352115_08(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,wj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M));
