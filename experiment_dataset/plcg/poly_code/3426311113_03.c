...
void kernel_3426311113_03(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        C[i_3] = E[i_3] + F[i_3] * F[i_3-1] * C[i_3-1] - 5;
        for (int i_4 = 1; i_4 < min(-i_3+PB_M+1, PB_L); i_4++) {
            D[i_3][i_4] = G[i_4+i_3-1][i_4][0] + D[i_3][i_4-1] - 6;
        }
    }
#pragma endscop
...
kernel_3426311113_03(xd,yd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
