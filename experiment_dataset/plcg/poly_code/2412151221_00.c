...
void kernel_2412151221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_N-1, PB_M-1); i_1++) {
            A[i_1+i_0] = E[1][i_0] * E[i_1+i_0][i_1] + E[i_1][i_0] - E[i_1+1][i_0] + E[i_0][i_1+1] * E[i_1-1][i_0+1] + E[i_0-1][i_1] + A[i_1+i_0+1] * B[i_1] * C[i_0] - 6;
            B[i_1] = E[i_1+i_0][1] * E[i_0][i_0-1] * F[i_0] * E[i_1][i_0] + A[i_1+i_0] - B[i_1-2] - 2;
        }
        C[i_0] = E[i_0-1][1] - F[i_0+1] + 2;
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_M; i_3++) {
            D[i_2][i_3] = E[i_3+i_2][i_3-1] + F[i_3] - E[i_2][i_3+1] + F[i_3+i_2] + E[i_3-1][i_2+1] - A[i_3+i_2-2] + D[i_3][i_2+2] * 6;
        }
    }
#pragma endscop
...
kernel_2412151221_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
