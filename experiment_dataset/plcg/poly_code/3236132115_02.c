...
void kernel_3236132115_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            A[i_2+1][i_3+1] = C[i_3-1][i_3+i_2] - C[i_2-2][i_3] + D[i_3+i_2] * C[i_2-2][i_3] - C[i_3-2][i_2-2] + 1;
        }
    }
#pragma endscop
...
kernel_3236132115_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
