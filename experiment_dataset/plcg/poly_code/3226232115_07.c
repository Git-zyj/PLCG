...
void kernel_3226232115_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
    ...
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_P-1, i_1); i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_M, PB_P); i_3++) {
                B[i_3+i_1][i_2+1][i_3] = E[i_3][i_2-1][i_3] + E[i_2][i_1][0] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3226232115_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
