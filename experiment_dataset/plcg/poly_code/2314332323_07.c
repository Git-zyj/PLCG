...
void kernel_2314332323_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 3; i_1 < PB_N; i_1++) {
            A[i_0][i_1-2] = D[i_0][i_1] * E[i_1] - F[i_0][i_0] * D[i_0][i_1+1] * A[i_0][i_1] * A[i_0+2][i_1-3] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            B[i_2][i_3] = B[i_2+1][i_3] * B[i_2][i_3-2] - C[i_2][i_2+1] * C[i_2-2][i_2-1] + 6;
        }
        for (int i_4 = 2; i_4 < PB_N; i_4++) {
            C[i_2][i_2+1] = F[i_4][i_2-2] + G[i_4][i_2-2] + F[i_2][i_4-2] + C[i_2-2][i_2-1] * C[i_2+1][i_2+2] - C[i_2-1][i_2] + 3;
        }
    }
#pragma endscop
...
kernel_2314332323_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
