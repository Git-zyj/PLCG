...
void kernel_2212152315_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-2; i_1++) {
            B[i_1-1][i_0+1] = C[i_1+2][i_0+1] - D[i_1-1] - C[i_1][i_0] * C[i_1-1][i_0] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_M+1, i_2); i_3++) {
            B[i_3][i_2] = C[i_2][i_3] - C[i_3][i_2-2] - C[i_3+i_2-1][0] - C[i_3][0] - C[i_3][i_2-2] - B[i_3][i_2-1] * B[i_3][i_2+1] + B[i_3-1][i_2] - 5;
        }
    }
#pragma endscop
...
kernel_2212152315_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
