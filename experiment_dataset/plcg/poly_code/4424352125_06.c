...
void kernel_4424352125_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < min(min(-i_1+PB_L-1, i_2), -i_0+PB_P); i_3++) {
                    A[i_3-1][i_1][i_2-2][i_3+i_0] = E[i_3+i_1+1][i_2-2][i_0-1] * F[i_0][i_1][i_2][i_3] - G[i_1][i_0][i_2][i_1] * 5;
                }
            }
        }
        for (int i_4 = 2; i_4 < i_0; i_4++) {
            for (int i_5 = 2; i_5 < -i_0+PB_N; i_5++) {
                for (int i_6 = 1; i_6 < -i_4+PB_M; i_6++) {
                    A[i_5-1][i_6][i_4-2][i_5+i_0] = H[i_4][i_6][i_5-2][i_0] - H[i_5-2][i_4][i_0][0] * G[i_0][i_5][i_6][i_4] + G[0][i_4+1][i_6-1][i_5] - F[i_5+i_0][i_6+i_4][i_4][2] + G[i_4][i_0][i_5][i_6] * I[i_6][i_4][i_5] * 4;
                }
            }
            for (int i_7 = 2; i_7 < min(-i_0+PB_L, -i_4+PB_Q); i_7++) {
                C[i_7-2][i_4] = J[i_4-1][0] + K[i_7+i_0][i_4+2][0] - J[i_7-1][i_4-1] - L[i_7+i_4][i_0] + L[i_4][i_0+2] + I[i_7][i_0][i_4-2] + K[i_0][i_7+i_4][i_7] + C[i_7-2][i_4+1] * C[i_0-3][i_4] + 2;
            }
        }
    }
    for (int i_8 = 2; i_8 < PB_L; i_8++) {
        for (int i_9 = 0; i_9 < -i_8+PB_L; i_9++) {
            C[i_8-2][i_9] = L[i_9+i_8][i_9] * M[i_8][i_9] - J[i_9+i_8][1] * 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4424352125_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M));
