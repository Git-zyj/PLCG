...
void kernel_2216332321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-2; i_0++) {
        A[i_0-2] = C[i_0] * C[i_0] * A[i_0] - A[i_0-3] - A[i_0-1] - B[i_0+2] - 6;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < min(-i_1+PB_M, PB_L+2); i_2++) {
            B[i_2+2] = D[i_2][i_1] + E[i_2+i_1][i_2-2] - D[i_2-2][i_1] - F[i_2-2] * B[i_2+3] * B[i_2+4] - 4;
        }
    }
#pragma endscop
...
kernel_2216332321_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
