...
void kernel_3322312125_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        ...
        for (int i_3 = 2; i_3 < PB_Q; i_3++) {
            for (int i_4 = 0; i_4 < PB_P-3; i_4++) {
                C[i_0][i_4+2][i_3] = D[i_4][i_3-2] - C[i_0][i_4+3][i_3] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3322312125_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
