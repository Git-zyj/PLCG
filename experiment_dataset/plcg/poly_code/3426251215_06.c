...
void kernel_3426251215_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(i_1, -i_1+PB_Q); i_2++) {
                A[i_2][i_1][0] = E[i_0][i_2][i_1-1] - F[i_0][i_0] - E[i_0][i_2+i_1][0] * E[i_0+1][i_2+i_1-1][i_1-1] + A[i_2][i_1-1][0] * A[i_2+1][i_1][0] - 5;
            }
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = i_3; i_4 < PB_M; i_4++) {
                A[i_0-1][i_3][0] = F[0][i_0+1] - 4;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_N-1; i_5++) {
        for (int i_6 = 0; i_6 < i_5; i_6++) {
            for (int i_7 = 1; i_7 < min(i_6, -i_5+PB_M); i_7++) {
                A[i_5-1][i_7-1][0] = G[i_6+1][i_7][i_5] * G[i_6][i_7+i_5][i_7] * 6;
            }
        }
        D[i_5] = H[i_5] + I[i_5+1] * H[i_5] * H[i_5] + H[i_5-1] - H[i_5] - I[i_5-1] - H[i_5] + I[i_5] + I[i_5] + D[i_5+1] + 3;
    }
#pragma endscop
...
kernel_3426251215_06(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
