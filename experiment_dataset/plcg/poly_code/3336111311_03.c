...
void kernel_3336111311_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            B[i_3] = D[i_2] * B[i_3-1] + B[i_3+1] * B[i_3-1] * B[i_3-1] * 4;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < min(min(i_4, -i_4+PB_M-1), PB_L-1); i_5++) {
            C[i_5+i_4][i_5] = B[i_5] + C[i_5+i_4+1][i_5+1] + C[i_5+i_4+1][i_5] + 3;
        }
    }
#pragma endscop
...
kernel_3336111311_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
