...
void kernel_3424232121_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < PB_L; i_2++) {
                A[i_2-1][i_1][i_1] = A[i_2][i_1][i_1] * 6;
            }
        }
        for (int i_3 = i_0; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < min(i_0, PB_L-1); i_4++) {
                B[i_0][i_4] = B[i_0][i_4+1] * 4;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_P; i_5++) {
        C[i_5-2] = E[i_5] - F[i_5-2][0] * E[i_5-2] - E[1] * 3;
        for (int i_6 = i_5; i_6 < min(-i_5+PB_M-2, PB_P); i_6++) {
            for (int i_7 = 2; i_7 < min(-i_5+PB_P, -i_6+PB_M); i_7++) {
                D[i_7-2][i_6+i_5+2] = G[i_6+i_5+2][i_7][0] + G[i_6][i_5][i_5-1] - H[i_6+2][i_7+i_5][i_5-2] * G[i_7][i_6][i_5] + H[i_6][i_5][i_7] + G[i_7+i_6][i_7+i_5][0] * A[i_6-1][i_5][i_5] * 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3424232121_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
