...
void kernel_2434351111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_L+1); i_1++) {
            A[i_0][i_1] = E[i_1] * F[i_0][i_1-1] * G[i_1+i_0][i_0] - G[i_0-1][i_0] + A[i_0-1][i_1] + A[i_0][i_1+1] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        B[i_2] = H[i_2+1] - I[i_2] - E[i_2] - I[i_2] + I[i_2] * E[i_2] * B[i_2+1] + 6;
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M-1, PB_L-1); i_4++) {
            C[i_3][i_4-1] = E[i_4+i_3+1] - F[i_3][i_4] - J[i_4][i_3] + G[i_3][i_4+1] * F[i_3][i_4+1] - 4;
        }
        B[i_3] = H[i_3] - H[i_3+1] + F[i_3+1][0] - E[i_3] * 5;
    }
#pragma endscop
...
kernel_2434351111_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
