...
void kernel_4326132311_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < -i_0+PB_M-2; i_2++) {
                A[i_1][i_1][i_2+i_0+2] = D[i_2][i_1][i_0] * E[i_0][i_2-2] - D[i_1][i_0][i_2+1] * A[i_2+1][i_2+1][i_1+i_0+2] - 4;
            }
        }
        for (int i_3 = 1; i_3 < i_0; i_3++) {
            for (int i_4 = i_0; i_4 < -i_0+PB_M; i_4++) {
                B[i_3][i_3][i_4+2] = D[i_4][i_0][i_3] + E[i_4+i_0][i_3-1] + B[i_3-1][i_3-1][i_0+2] * B[i_3+1][i_3+1][i_4+1] - 6;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = max(2, i_5); i_6 < -i_5+PB_N-1; i_6++) {
            for (int i_7 = 1; i_7 < min(-i_6+PB_N-1, PB_M-2); i_7++) {
                C[i_6+i_5][i_7][i_7+2] = D[i_6][i_7+2][i_5] + D[i_5][i_7][i_6-2] - C[i_6+i_5][i_7+1][i_7+3] * C[i_6+i_5-1][i_7][i_7+2] * C[i_7+i_6+1][i_5][i_5+2] + C[i_6+i_5-1][i_7-1][i_7+1] * C[i_6+i_5+1][i_7][i_7+2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4326132311_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
