...
void kernel_4214212111_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = i_1; i_2 < min(min(-i_1+PB_M, -i_0+PB_L), PB_L-2); i_2++) {
                A[i_0+1][i_2+2][i_1] = C[i_2+i_1][i_2+i_0][i_1+1] + A[i_2+2][i_0+2][i_1] * 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < PB_P; i_5++) {
                B[i_3][i_4][i_5] = B[i_3][i_4-1][i_5] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4214212111_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
