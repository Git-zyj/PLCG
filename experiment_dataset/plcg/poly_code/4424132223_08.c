...
void kernel_4424132223_08(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_M; i_2++) {
                for (int i_3 = 0; i_3 < min(i_1, -i_0+PB_P); i_3++) {
                    D[i_0-2][i_0-2][i_1][i_2-1] = E[i_1][i_0][i_3][i_2] - E[i_3][i_1][i_3+i_0][i_2] * E[i_2+i_1][i_3+i_0][i_0][0] + E[i_3][i_0][i_2-2][i_2] + 5;
                }
            }
        }
    }
    for (int i_4 = 4; i_4 < PB_L-2; i_4++) {
        B[i_4-2] = F[i_4][0] * B[i_4-4] - 5;
        for (int i_5 = i_4; i_5 < -i_4+PB_L-2; i_5++) {
            C[i_4][i_5+i_4] = F[i_5][i_5+i_4+1] + F[i_5][i_4] - C[i_4+2][i_5+i_4+2] * C[i_4+1][i_5+i_4+1] * C[i_4][i_5+i_4+2] * 4;
        }
    }
    for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
        for (int i_7 = 2; i_7 < PB_M-2; i_7++) {
            for (int i_8 = 2; i_8 < min(-i_7+PB_M, PB_L-2); i_8++) {
                for (int i_9 = 2; i_9 < min(-i_7+PB_M-2, i_7); i_9++) {
                    D[i_6][i_6][i_8][i_7-1] = E[i_9][i_8+i_7][i_6+1][i_6] - E[i_9+i_7+2][i_8-2][i_6][i_6] - E[i_8][i_9-2][i_6][i_7-2] + G[i_9][i_7][i_8+2] + D[i_9+1][i_9+1][i_8][i_7-1] - 1;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4424132223_08(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
