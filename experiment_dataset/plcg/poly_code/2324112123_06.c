...
void kernel_2324112123_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            C[i_3][i_4] = C[i_3+1][i_4-1] * C[i_3-1][i_4] - 4;
        }
    }
#pragma endscop
...
kernel_2324112123_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
