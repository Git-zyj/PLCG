...
void kernel_4226332125_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(PB_M+1, -i_0+PB_P-1); i_1++) {
            for (int i_2 = 1; i_2 < min(min(PB_M+1, -i_0+PB_P-3), -i_1+PB_L); i_2++) {
                A[i_1-1][i_1+i_0+1][i_2-1] = C[i_1][i_0+1][i_2+1] + C[i_1][i_0][0] + D[i_2+i_1][i_0+1][i_1-2] * C[i_0][i_1+1][i_2] + A[i_2-1][i_2+i_0+3][i_1-1] * A[i_1-2][i_1+i_0][i_2-1] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            B[i_3+2][i_4][0] = E[i_4][i_3+1] + 4;
        }
    }
#pragma endscop
...
kernel_4226332125_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
