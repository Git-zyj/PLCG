...
void kernel_4212132123_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_0-2][i_2][i_0] = C[i_1-1][i_0][i_1-2] * A[i_0][i_2][i_0+2] - 1;
            }
        }
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            A[i_0][i_3][i_0+2] = D[i_0][0] - C[i_0][i_3][i_0] * D[i_0+2][i_3] * D[i_0][i_3] + 1;
        }
    }
#pragma endscop
...
kernel_4212132123_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
