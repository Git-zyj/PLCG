...
void kernel_3414231321_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        B[i_0] = E[1] + F[i_0-1] * F[i_0] * F[i_0+1] + E[i_0] + B[i_0] + 2;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        B[i_1] = F[i_1] * E[i_1] - E[i_1] * B[i_1+2] * 5;
        for (int i_2 = 2; i_2 < min(-i_1+PB_M, PB_M-2); i_2++) {
            D[i_1+1][i_2][i_2+1] = G[i_2+i_1][i_2] - H[i_2+1][i_1+1] * B[i_2+2] + B[i_2] * B[i_2-2] + 1;
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                D[i_2+1][i_3][i_3+1] = D[i_2][i_1][i_1+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3414231321_02(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
