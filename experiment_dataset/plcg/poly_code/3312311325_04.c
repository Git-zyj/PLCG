...
void kernel_3312311325_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        ...
        for (int i_3 = 2; i_3 < min(PB_N-1, -i_0+PB_L-1); i_3++) {
            for (int i_4 = i_3; i_4 < -i_3+PB_N-1; i_4++) {
                B[i_0][i_4][i_3] = D[i_4+i_3+1][i_0][i_3+i_0+1] - B[i_3-2][i_0-2][i_4] + B[i_0][i_4+1][i_3-2] * 2;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_N-2; i_5++) {
        for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
            C[i_5][i_6] = C[i_5-1][i_6+2] - C[i_5+2][i_6] - C[i_5][i_6-2] - 1;
        }
    }
#pragma endscop
...
kernel_3312311325_04(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
