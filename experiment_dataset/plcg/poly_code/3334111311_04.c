...
void kernel_3334111311_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                A[i_1-1][i_2][i_0] = A[i_1-1][i_2-1][i_0-1] * A[i_2][i_1][i_0-1] * 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N-1; i_3++) {
        B[i_3] = D[i_3+1] + D[i_3] * B[i_3+1] - B[i_3-1] * B[i_3+1] - B[i_3-1] * 2;
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < PB_P; i_5++) {
            for (int i_6 = 0; i_6 < min(PB_Q-1, i_5); i_6++) {
                C[i_4][i_5][i_6+1] = C[i_6][i_5][i_4] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3334111311_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
