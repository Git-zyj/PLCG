...
void kernel_3422212325_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            B[i_1-1] = B[i_1+1] - B[i_0-1] * 5;
        }
        B[i_0-1] = B[i_0] * B[i_0-2] * 3;
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        C[i_2] = E[i_2] * C[i_2+1] + C[i_2-2] * 4;
        C[i_2] = 2;
    }
#pragma endscop
...
kernel_3422212325_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E));
