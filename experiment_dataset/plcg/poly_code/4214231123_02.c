...
void kernel_4214231123_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_P+1, PB_P-1); i_1++) {
            for (int i_2 = i_0; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_1+PB_N, PB_L-1); i_3++) {
                    A[i_2][i_3-1][i_0+1][i_2+1] = C[i_2][i_3+1][i_1][i_0-1] * D[i_3+i_1][i_2][i_0][i_1+i_0-1] * A[i_2][i_3][i_1+1][i_2+1] * 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        B[i_4-1] = E[i_4] + F[i_4+1] + E[i_4+1] * G[i_4][i_4] - 5;
    }
#pragma endscop
...
kernel_4214231123_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
