...
void kernel_4234211321_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_L-2); i_1++) {
            A[i_1+i_0+1] = C[i_1][i_0] + A[i_1+i_0-1] - A[i_1+i_0+2] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-3; i_2++) {
        for (int i_3 = 3; i_3 < min(PB_Q-1, -i_2+PB_L-1); i_3++) {
            B[i_2+1][i_3-1][i_3] = A[i_3+i_2+1] + B[i_2+1][i_3][i_3+1] + B[i_2+3][i_3-3][i_3-2] + 1;
        }
    }
#pragma endscop
...
kernel_4234211321_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
