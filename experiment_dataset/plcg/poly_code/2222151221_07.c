...
void kernel_2222151221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_1+i_0] = C[i_1] + D[i_1][i_0] + D[i_1][i_0+1] + D[i_1][i_0] * D[i_1][i_0] * A[i_1+i_0-1] - A[i_1+i_0+1] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
        B[i_2] = C[i_2] + C[i_2] * C[i_2+1] + C[i_2+1] * C[1] * B[i_2+2] - 1;
    }
#pragma endscop
...
kernel_2222151221_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
