...
void kernel_2226312125_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M - 1; i_0 >= 0; i_0--) {
        for (int i_1 = 2; i_1 < min(min(PB_L+2, i_0), -i_0+PB_P); i_1++) {
            A[i_0][i_1-2][i_1+i_0] = 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < min(PB_L+2, -i_2+PB_P); i_3++) {
            B[i_2][i_3-1] = C[i_2] - A[i_2][i_3-2][i_3+i_2] * 5;
        }
    }
#pragma endscop
...
kernel_2226312125_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
