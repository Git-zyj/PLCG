...
void kernel_3232111311_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_N; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_2+PB_Q, PB_L-1); i_4++) {
                B[i_4][i_2+1][i_2] = C[i_4+1][i_3+1][i_4+i_2] + B[i_4+1][i_2+1][i_2] + B[i_3][i_2+2][i_2+1] - B[i_4][i_3][i_3-1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3232111311_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
