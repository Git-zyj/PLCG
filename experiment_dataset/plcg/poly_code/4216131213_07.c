...
void kernel_4216131213_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-1); i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_M-1; i_2++) {
                for (int i_3 = 1; i_3 < min(min(PB_M, i_2), -i_0+PB_P); i_3++) {
                    A[i_1][i_2][i_3][i_3] = C[i_2+i_1+1][i_3-1][0][i_0+1] - D[i_2-1][i_1+1][i_2+1] - C[i_1+1][i_2][i_3+i_0][1] - A[i_1+1][i_2][i_3-1][i_3-1] + A[i_3][i_2+1][i_1][i_1] * 4;
                }
            }
        }
    }
        ...
        for (int i_5 = i_4; i_5 < -i_4+PB_P; i_5++) {
            for (int i_6 = 1; i_6 < PB_L; i_6++) {
                for (int i_7 = 0; i_7 < PB_L; i_7++) {
                    A[i_6][i_5-1][i_7][i_7] = C[i_7][i_6][i_5][i_4-1] + C[i_6-1][i_7][i_5+i_4][i_5] * 6;
                }
            }
        }
    }
...
kernel_4216131213_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
