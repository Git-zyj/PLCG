...
void kernel_2314112225_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0-1][i_1] = D[i_0] + A[i_0-3][i_1] * A[i_0][i_1] + A[i_0-1][i_1-1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        B[i_2] = D[i_2] + 5;
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            C[i_2][i_3+2] = B[i_2] + B[i_3] + B[i_2+2] + 2;
        }
    }
#pragma endscop
...
kernel_2314112225_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
