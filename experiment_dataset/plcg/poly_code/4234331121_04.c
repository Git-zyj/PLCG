...
void kernel_4234331121_04(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-3; i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_1+PB_L-1, -i_0+PB_L), PB_M-1); i_2++) {
                for (int i_3 = i_0; i_3 < min(-i_1+PB_L-1, PB_M-2); i_3++) {
                    B[i_3+2][i_0-1][i_2][i_1+3] = C[i_0][i_2][i_2][i_3] * D[i_3][i_0][i_1][i_2+i_1+1] - C[i_3+1][i_2+1][i_3+i_1+1][i_2+i_0] - E[i_1][i_2-1][i_3][i_0] * 4;
                }
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < PB_N; i_5++) {
            for (int i_6 = 2; i_6 < PB_Q; i_6++) {
                for (int i_7 = 1; i_7 < PB_Q-1; i_7++) {
                    B[i_4+1][i_5][i_6][i_7+1] = F[i_7][i_6][i_5][i_4][i_6-1] - C[i_5][i_4][i_6][0] * B[i_4+1][i_5][i_6-2][i_7-1] * 2;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4234331121_04(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F)));
