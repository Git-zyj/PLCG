...
void kernel_3236232125_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                B[i_2][i_2+2] = C[i_1][i_2][i_0] + C[i_2-2][i_0][i_1] * 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_Q-1; i_4++) {
            for (int i_5 = i_4; i_5 < PB_L-2; i_5++) {
                B[i_4][i_4+2] = D[i_5+2][i_4][i_4+1] * D[i_5][i_4][i_3+2] * D[i_4][i_5][i_3] * B[i_4+2][i_4+4] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3236232125_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
