...
void kernel_2432211323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-2; i_3++) {
            C[i_3][i_2] = A[i_3+1] - C[i_3][i_2-2] - C[i_3+2][i_2-2] + 3;
        }
        for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
            D[i_4][i_2] = D[i_4-2][i_2] + D[i_4][i_2-2] - D[i_4][i_2+1] + D[i_4+2][i_2] + 6;
        }
    }
#pragma endscop
...
kernel_2432211323_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
