...
void kernel_2426311313_04(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            C[i_0+1][i_1] = E[i_0-1][i_1+1] * 4;
        }
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            C[i_0][i_2] = C[i_2+1][i_0+1] + 2;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_L-1; i_4++) {
            C[i_3][i_4] = F[i_4][i_3] + C[i_3][i_4+1] + C[i_3][i_4-1] + 2;
        }
    }
...
kernel_2426311313_04(xc,yc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
