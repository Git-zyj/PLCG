...
void kernel_4216211215_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = i_0; i_2 < min(-i_1+PB_M, PB_M-1); i_2++) {
                B[i_2][i_1+1] = C[i_2+i_1][i_0][i_0] - B[i_2][i_1+2] + 1;
                B[i_2+1][i_0+1] = B[i_1][i_2+2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4216211215_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
