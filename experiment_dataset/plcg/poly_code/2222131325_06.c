...
void kernel_2222131325_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_L-1); i_1++) {
            B[i_1+i_0][i_1] = C[i_0][i_1] + C[i_1+1][i_0+1] - C[i_1+i_0][1] + C[i_0][i_1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L-2; i_3++) {
            B[i_3+i_2][i_2] = C[i_3+i_2][i_2] + B[i_3+i_2+2][i_2] * B[i_3+i_2-1][i_2] - B[i_3+i_2-1][i_2-1] - 4;
        }
    }
#pragma endscop
...
kernel_2222131325_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
