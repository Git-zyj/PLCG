...
void kernel_3216232225_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N; i_1++) {
            A[i_1+1] = C[i_1][i_0] * D[i_1+i_0][2] * C[i_0][i_0] + B[i_0][i_1] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            B[i_2][i_3] = D[i_2][i_3] - C[i_3+i_2][0] + C[i_2][i_3] + B[i_2-1][i_3+2] * B[i_2][i_3-2] * B[i_2][i_3+1] - 2;
        }
    }
#pragma endscop
...
kernel_3216232225_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
