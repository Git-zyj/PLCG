...
void kernel_4332332113_09(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
        ...
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 1; i_3 < min(PB_Q-2, i_2); i_3++) {
                    C[i_0-1][i_0-1][i_1][i_3+2] = D[i_0+1][i_2][i_3-1][i_1] * 6;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_Q-1; i_4++) {
        B[i_4+1] = E[i_4+2] + F[i_4] - F[i_4] + B[i_4+2] - 5;
        for (int i_5 = 2; i_5 < min(PB_P-2, -i_4+PB_L); i_5++) {
            for (int i_6 = 0; i_6 < min(min(-i_4+PB_Q-1, -i_5+PB_P-2), PB_L-2); i_6++) {
                for (int i_7 = 1; i_7 < -i_4+PB_Q; i_7++) {
                    C[i_4][i_4][i_7][i_6+2] = D[i_6+2][i_5][i_7+i_4][i_6+i_4+1] * G[i_7-1][i_5+i_4][i_6+i_5+2][0] - H[i_5][i_6][i_4][i_7+i_5-2] + G[i_4][i_5][i_7][i_6] - 4;
                }
            }
        }
    }
...
init_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4332332113_09(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
