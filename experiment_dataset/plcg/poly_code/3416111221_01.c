...
void kernel_3416111221_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L, i_1); i_2++) {
                A[i_0][i_2][i_0] = A[i_2][i_0-1][i_2] * 2;
            }
        }
        B[i_0] = B[i_0-2] * B[i_0+2] - B[i_0-1] + B[i_0+1] + 2;
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
            C[i_4+i_3][i_3-1][i_4+1] = E[i_4][i_3] + C[i_4+i_3-1][i_3-2][i_4+1] * 5;
        }
        for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
            D[i_3][i_5] = E[i_5][i_3] * D[i_3+1][i_5+1] * 1;
        }
    }
#pragma endscop
...
kernel_3416111221_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
