...
void kernel_3336251225_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] * D[i_0] - D[i_0+1] - D[i_0] + D[i_0] * E[i_0] - E[i_0] + A[i_0+1] + A[i_0-1] * 1;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            A[i_2] = F[i_2][i_1] + D[i_2+i_1] + G[i_2][i_1] * 5;
        }
        for (int i_3 = 1; i_3 < min(i_1, PB_M-2); i_3++) {
            A[i_3+2] = E[i_3] - G[i_3][i_1] * H[0][i_3-1][i_1] * F[i_3-1][0] - 2;
        }
    }
#pragma endscop
...
kernel_3336251225_01(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
