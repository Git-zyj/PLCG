...
void kernel_3212152211_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0+1][i_0] = C[i_0-1][i_0] - D[i_0] * D[i_0] - 6;
    }
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M-2; i_2++) {
            B[i_2+i_1][i_1] = C[i_2-1][i_1] + C[i_2][i_1] - C[i_2][i_1] - C[i_1+1][i_2] + C[i_2][i_1+2] + C[i_2][i_1] + B[i_2+i_1-1][i_1-1] + B[i_2+i_1-1][i_2] + B[i_2+i_1+2][i_1+1] + 4;
        }
    }
#pragma endscop
...
kernel_3212152211_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
