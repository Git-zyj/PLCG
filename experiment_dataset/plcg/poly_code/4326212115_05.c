...
void kernel_4326212115_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1][i_0+2] = D[i_0+1][i_1-1] - 6;
        }
    }
        ...
        for (int i_3 = 0; i_3 < min(PB_M, i_2); i_3++) {
            ...
            for (int i_5 = 0; i_5 < PB_P-2; i_5++) {
                for (int i_6 = 2; i_6 < PB_L+1; i_6++) {
                    C[i_6-2][i_5][i_5+1] = E[i_3][i_6-1][i_2-1] * C[i_6-1][i_5+1][i_5+2] + 3;
                }
            }
        }
    }
...
kernel_4326212115_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
