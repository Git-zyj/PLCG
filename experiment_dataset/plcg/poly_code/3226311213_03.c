...
void kernel_3226311213_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < min(min(PB_P, -i_2+PB_L-2), i_2); i_4++) {
                B[i_3][i_2][i_4+i_2] = C[i_3+i_2+1][i_4][i_4] * B[i_3][i_2+1][i_4+i_2+2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3226311213_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
