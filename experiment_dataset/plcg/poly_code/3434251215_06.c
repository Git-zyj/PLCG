...
void kernel_3434251215_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_N; i_1++) {
            A[i_1] = E[i_1+i_0] - E[i_0] + F[i_1-1][i_0] + G[i_1][i_0] + G[i_1][i_0] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_P; i_3++) {
            for (int i_4 = 1; i_4 < PB_P; i_4++) {
                B[i_2][i_4][i_4] = H[i_3][i_2][i_4] * I[i_3][i_4][i_2] - G[i_4][i_3] + F[i_4][i_2] + H[i_2-1][i_4][i_3] - B[i_4+1][i_2][i_2] * B[i_4-1][i_2+1][i_2+1] - B[i_2+1][i_4][i_4] - B[i_2-1][i_4][i_4] * 5;
            }
        }
        for (int i_5 = 0; i_5 < -i_2+PB_P; i_5++) {
            B[i_2][i_5+1][i_5+1] = G[i_2][i_5] + I[i_2+1][i_5+1][0] - F[i_2][0] + G[i_5+i_2][i_5+i_2] + 3;
            for (int i_6 = 0; i_6 < -i_2+PB_L-1; i_6++) {
                B[i_2][i_5][i_5] = F[i_5+1][i_6] + H[i_2][i_5][i_6] + I[i_6][i_2][i_2+1] * I[i_5+i_2][i_6+i_2+1][i_5+1] - I[i_6+1][i_2][i_5+1] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3434251215_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
