...
void kernel_2322311211_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N-1; i_1++) {
            A[i_1+i_0][i_0-1] = A[i_1+i_0+1][i_0-1] * A[i_1+i_0-1][i_0-1] + A[i_1+i_0+1][i_0] * 1;
        }
    }
    ...
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_P); i_5++) {
            C[i_4][i_5] = D[i_4+1] - C[i_4][i_5-1] - C[i_4+1][i_5-1] + 6;
        }
    }
#pragma endscop
...
kernel_2322311211_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
