...
void kernel_4322111223_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
            C[i_2-1] = C[i_3+1] - C[i_3+2] * C[i_2] + 4;
        }
        C[i_2+1] = D[i_2] - D[i_2] + 2;
    }
#pragma endscop
...
kernel_4322111223_04(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
