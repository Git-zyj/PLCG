...
void kernel_3232211323_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = 2; i_2 < PB_P; i_2++) {
            for (int i_3 = 3; i_3 < min(-i_2+PB_L, PB_P-1); i_3++) {
                B[i_2][i_3-1][i_3] = C[i_3+i_2][i_1][0] * B[i_2][i_3][i_3+1] - B[i_2-2][i_1-2][i_1-1] + B[i_2-2][i_3-3][i_3-2] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3232211323_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
