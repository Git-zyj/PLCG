...
void kernel_4236211323_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            B[i_0-2][i_1+1][0] = C[i_1+i_0][i_0+1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            for (int i_4 = i_3; i_4 < PB_P; i_4++) {
                B[i_3-1][i_4+1][0] = B[i_3-1][i_4][0] - B[i_3-1][i_4+3][0] * B[i_3][i_4+1][0] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4236211323_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
