...
void kernel_4232231323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0+1] = C[i_0] + D[i_0+1] + D[i_0+1] - D[i_0] * A[i_0+3] * A[i_0-1] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
            A[i_2+1] = E[i_2-1][i_1+1] - E[i_2-1][i_1] * A[i_1+1] * 3;
        }
    }
#pragma endscop
...
kernel_4232231323_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
