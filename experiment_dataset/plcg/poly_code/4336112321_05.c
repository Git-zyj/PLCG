...
void kernel_4336112321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_0][i_2+1][i_1] = D[i_2][i_0][i_0] * A[i_0][i_2+1][i_1-1] * A[i_0][i_2+2][i_1] - A[i_0][i_2+1][i_1+2] * A[i_0-1][i_2+1][i_1] * 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-4; i_3++) {
        for (int i_4 = 2; i_4 < -i_3+PB_M-4; i_4++) {
            for (int i_5 = i_3; i_5 < min(min(-i_3+PB_N, -i_4+PB_N), PB_L-2); i_5++) {
                B[i_4+i_3][i_3-1][i_5+2] = E[i_3][i_4-2] + B[i_5+i_3][i_3-1][i_4+3] * B[i_5+i_4][i_4-1][i_3] * C[i_4+i_3+4][i_5+2][i_4] - 1;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_L; i_6++) {
        for (int i_7 = 0; i_7 < PB_M-3; i_7++) {
            for (int i_8 = 0; i_8 < -i_7+PB_M-3; i_8++) {
                C[i_8+i_7+2][i_6][i_8] = D[i_8][i_6][i_7+1] - C[i_8+i_7+3][i_6][i_8] - C[i_8+i_7+2][i_6-2][i_8] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4336112321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
