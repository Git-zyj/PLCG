...
void kernel_4414112311_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-3; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-3; i_1++) {
            A[i_0][i_1-1] = B[i_0-2][i_1] + B[i_1-2][i_0] + C[i_1+2] * C[i_1+3] + 5;
            B[i_0-2][i_1] = B[i_0-1][i_1+1] + B[i_0-3][i_1] - C[i_1+2] * 5;
        }
        C[i_0+2] = E[i_0] * C[i_0+3] + C[i_0+1] - C[i_0+1] * 5;
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
                D[i_3+2][i_4] = D[i_4+2][i_2+1] - D[i_3+3][i_4+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4414112311_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
