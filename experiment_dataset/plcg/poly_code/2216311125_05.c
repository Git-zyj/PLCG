...
void kernel_2216311125_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = PB_Q - 1; i_1 >= max(2, i_0); i_1--) {
            A[i_0-1][i_1] = A[i_0-1][i_1-2] * 2;
        }
        for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
            A[i_0-1][i_2+1] = C[i_0-1][i_2] + 6;
        }
    }
#pragma endscop
...
kernel_2216311125_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
