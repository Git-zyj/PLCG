...
void kernel_4214112223_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M-3, -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_L-2, PB_M-3), -i_0+PB_L-4); i_2++) {
                A[i_2+1][i_2+i_0][i_1] = C[i_2][i_1+i_0][i_0-2] - A[i_2+3][i_2+i_0+2][i_1] * A[i_2+3][i_2+i_0+4][i_1] - A[i_1+3][i_2+i_1+2][i_0] * 3;
            }
            B[i_1-2][i_0] = D[i_1+2][i_0] + 6;
        }
    }
#pragma endscop
...
kernel_4214112223_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
