...
void kernel_4226212215_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_P; i_2++) {
                A[i_0+1][i_2-2][i_1] = A[i_0+1][i_1-1][i_2] + 3;
            }
        }
        B[i_0+1] = C[i_0+1] * B[i_0+2] + B[i_0] + B[i_0] * 5;
    }
#pragma endscop
...
kernel_4226212215_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
