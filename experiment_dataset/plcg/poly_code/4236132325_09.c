...
void kernel_4236132325_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[i_0-2][i_1+2] = C[i_1+2][i_0] + C[i_1+i_0][i_0+2] * C[i_0][i_0+2] + C[i_0][i_1] * A[i_0-2][i_1+4] * A[i_0-1][i_1] + A[i_0-1][i_1+2] - 3;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_L, i_0); i_2++) {
            A[i_0-4][i_2+2] = C[i_2][i_0+2] + C[i_2+i_0][0] + 6;
        }
    }
#pragma endscop
...
kernel_4236132325_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
