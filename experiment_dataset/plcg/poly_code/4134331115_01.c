...
void kernel_4134331115_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_P+1, PB_L-1); i_1++) {
            for (int i_2 = -i_0+PB_Q - 1; i_2 >= 0; i_2--) {
                A[i_2+i_0-1][i_1][i_1+1][1] = B[i_2][i_1+i_0-1] + C[i_1][i_2+i_0][0] - D[i_0][i_1+1] - A[i_2+i_0-1][i_1-1][i_1][1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4134331115_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
