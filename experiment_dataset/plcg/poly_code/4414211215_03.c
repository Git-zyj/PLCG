...
void kernel_4414211215_03(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                for (int i_3 = 1; i_3 < PB_P; i_3++) {
                    D[i_0-1][i_2][i_3-1][i_1] = E[i_2][i_3-1][i_1][i_0][1] + B[i_1][i_3-1][i_3] + 2;
                }
            }
        }
        for (int i_4 = 1; i_4 < i_0; i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                B[i_4][i_5-1][i_5] = F[i_4][i_0][i_5] * F[i_5-1][i_0-1][i_4-1] + B[i_5][i_0-2][i_0-1] * B[i_0-1][i_4-1][i_4] * 1;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 0; i_7 < PB_M-1; i_7++) {
            for (int i_8 = i_7; i_8 < PB_L-1; i_8++) {
                for (int i_9 = 1; i_9 < -i_6+PB_M-1; i_9++) {
                    D[i_6][i_8+1][i_9-1][i_7] = G[i_7+1][i_9+i_6+1][i_7][i_8] * 6;
                }
            }
            for (int j_0 = 1; j_0 < PB_L+1; j_0++) {
                for (int j_1 = 0; j_1 < PB_L; j_1++) {
                    D[i_6][j_1][j_0-1][i_7] = D[j_0-1][i_6-1][j_1][i_7+1] * 6;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4414211215_03(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
