...
void kernel_4316351225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0] * D[i_0] * E[i_0] - F[i_0][i_0] - D[0] + E[i_0] - E[i_0] - A[i_0+2] - A[i_0+1] + 1;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_P; i_2++) {
            B[i_2][i_2+i_1] = E[i_2] + G[i_2][i_1] + H[i_2][i_1] + G[i_1][i_2] + H[i_2+i_1][0] + B[i_2][i_2+i_1-2] * B[i_2][i_2+i_1-1] - 5;
        }
        A[i_1] = D[i_1] - E[i_1] * D[i_1] + 3;
    }
#pragma endscop
...
kernel_4316351225_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
