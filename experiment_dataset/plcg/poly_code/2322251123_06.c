...
void kernel_2322251123_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_1][i_0+1] = D[i_1][i_0] + E[i_1] * A[i_1-2][i_0-1] + A[i_0+1][i_1+2] + 5;
        }
        for (int i_2 = i_0; i_2 < PB_M; i_2++) {
            B[i_0][i_2] = F[i_0][i_2+1] * F[i_0-1][i_0+1] - F[i_0][i_2+1] + G[i_0+1][i_2][i_0] * E[1] + F[i_2][i_0] - D[i_2-1][i_0] * D[i_2][i_0] - 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            B[i_3][i_4] = D[i_4][i_3+1] - E[i_3] - H[i_3-1][i_4][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_2322251123_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
