...
void kernel_2232111325_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_0][i_1] = C[i_0][i_1][i_1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
            B[i_2][i_3] = B[i_2][i_3+2] - B[i_2+2][i_3] + B[i_2-1][i_3+1] * 1;
        }
    }
#pragma endscop
...
kernel_2232111325_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
