...
void kernel_3224131111_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0][i_0] = C[i_0] + C[i_0] * C[i_0] * C[i_0] + A[i_0-1][i_0-1] - 6;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_1] = D[i_1][i_0-1] + D[i_0+1][i_1] * B[i_1+1] - 2;
        }
    }
#pragma endscop
...
kernel_3224131111_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
