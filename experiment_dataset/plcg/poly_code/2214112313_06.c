...
void kernel_2214112313_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 3; i_1 < PB_P; i_1++) {
            B[i_0-2][i_1+1] = C[i_0+2][i_1] * B[i_1-3][i_0+2] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < PB_M; i_3++) {
            B[i_3-2][i_2+1] = C[i_3][i_2] * B[i_3-1][i_2+1] * B[i_3-3][i_2+2] + 1;
        }
    }
#pragma endscop
...
kernel_2214112313_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
