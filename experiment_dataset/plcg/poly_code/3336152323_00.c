...
void kernel_3336152323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0-1] = D[i_0] + D[i_0+1] * E[i_0-2][i_0] + D[i_0-1] + D[i_0-1] - D[i_0] * A[i_0] * 1;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_M; i_2++) {
            B[i_1][i_2] = E[i_2][i_1] - E[i_2][i_1] + E[i_2-2][i_1] - E[i_1][i_2+i_1] * B[i_1+1][i_2] - B[i_2][i_1+2] * B[i_1-2][i_2] - B[i_1+1][i_2-2] + 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M-1, i_3); i_4++) {
            C[i_4+i_3] = E[i_4+1][i_3] + E[i_4][i_3] - E[i_4][i_3+2] * E[i_4][i_3] * C[i_4+i_3+1] - C[i_4+i_3] - C[i_4+i_3-1] + 2;
        }
    }
#pragma endscop
...
kernel_3336152323_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
