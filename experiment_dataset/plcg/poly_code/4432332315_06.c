...
void kernel_4432332315_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                B[i_1][i_2] = E[i_1-2][i_0][i_2] - F[i_1][i_2-2][i_0] + 4;
            }
        }
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            B[i_3][i_0-1] = G[i_0-2][i_3-2] - H[i_3][i_0] * H[i_0+1][i_3] - B[i_0][i_3-2] * B[i_3][i_0] + 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < min(-i_4+PB_M+1, PB_L-1); i_5++) {
            B[i_5][i_4-1] = G[i_5+i_4][i_5+i_4-1] - H[i_4+1][i_5+1] + D[i_5] + 2;
        }
    }
    for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
        D[i_6] = I[i_6+1] + J[i_6-2] + D[i_6-1] - D[i_6+1] + 6;
    }
#pragma endscop
...
kernel_4432332315_06(xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
