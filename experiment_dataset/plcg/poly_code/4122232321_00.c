...
void kernel_4122232321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_Q, PB_M); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_M, -i_0+PB_P); i_2++) {
                A[i_1][i_2] = B[i_2][i_0][i_1+i_0] - C[i_1][i_0+1][i_2+i_0] * C[i_0][i_2][i_1] * A[i_1][i_2-1] * A[i_1-1][i_2] - A[i_0-1][i_2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4122232321_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
