...
void kernel_4112311325_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-3; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_P-2, i_0); i_2++) {
                A[i_1+1][i_0-1][i_2+1] = B[i_2-1][i_1-1] * A[i_1+1][i_0-1][i_2] - A[i_1+1][i_0-1][i_2+2] - A[i_1+3][i_0-1][i_2+1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4112311325_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
