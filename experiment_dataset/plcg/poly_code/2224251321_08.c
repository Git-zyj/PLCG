...
void kernel_2224251321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            A[i_1+1][i_1-1] = C[i_1][i_0] + D[i_0][i_1] - D[i_1+i_0][i_0] * C[i_1][i_0] - A[i_1][i_1-2] * A[i_1+2][i_1] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_Q+1; i_3++) {
            B[i_2][i_3-1] = D[i_2][1] * E[i_3-1] - F[i_2][i_3-1][i_2+1] + D[i_3][i_2] + D[i_3+1][0] + C[i_3+1][i_2] * A[i_3+1][i_3-1] - B[i_2][i_3-2] + B[i_2-2][i_3-1] - B[i_2-1][i_3+1] - 1;
        }
    }
#pragma endscop
...
kernel_2224251321_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
