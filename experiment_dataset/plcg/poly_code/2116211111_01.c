...
void kernel_2116211111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(PB_N, -i_0+PB_P); i_1++) {
            A[i_1] = B[i_0][i_1+i_0] * A[i_1-1] * 1;
        }
    }
#pragma endscop
...
kernel_2116211111_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
