...
#endif
static void init_array(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,int zn,int vn,int wn,DATA_TYPE POLYBENCH_5D(N,xN,yN,zN,vN,wN,xn,yn,zn,vn,wn),int seed)
{
srand(seed);
...
void kernel_4414351225_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,int zn,int vn,int wn,DATA_TYPE POLYBENCH_5D(N,xN,yN,zN,vN,wN,xn,yn,zn,vn,wn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0-1] = E[i_0] * F[i_0-1] * G[1] - F[i_0] * F[i_0] * F[i_0] + F[i_0] - A[i_0] + A[i_0+1] - 4;
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 3; i_2 < -i_1+PB_Q; i_2++) {
                for (int i_3 = 1; i_3 < -i_0+PB_L; i_3++) {
                    B[i_2+i_1][i_3][i_0] = H[i_3+1][i_2][i_0+1] * I[i_1+1][i_2+1][i_3+i_0][i_2] + J[i_3-1][i_0][i_2+1] + K[i_2-1][i_1+1][i_0][i_3] - I[i_2+1][i_3-1][i_1+i_0][i_2+i_1] + J[i_3][i_0][i_2] - B[i_3+i_2-3][i_0][i_1-2] * 6;
                }
            }
            C[i_1][i_0] = L[i_0+1][i_1] - M[i_1][i_0] - C[i_1-2][i_0] + 4;
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < min(PB_Q, -i_4+PB_P); i_5++) {
            for (int i_6 = i_5; i_6 < PB_Q; i_6++) {
                for (int i_7 = 1; i_7 < min(PB_Q, -i_6+PB_P); i_7++) {
                    D[i_6][i_4][i_5-1][i_7] = N[i_7+1][1][i_5+i_4][i_7+i_6][i_5] * I[i_5][i_7][i_4][i_6] - D[i_4][i_6-1][i_5-1][i_7-1] + 1;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,yn,zn,vn,wn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4414351225_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,yn,zn,vn,wn,POLYBENCH_ARRAY(N));
