...
void kernel_2336212225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(-i_0+PB_N+1, PB_P); i_1++) {
            A[i_0][i_1] = D[i_1][i_0] + D[i_1+i_0-1][i_1] * D[i_1][i_0-1] + A[i_0][i_1-2] * 2;
        }
    }
    for (int i_2 = 3; i_2 < PB_P-2; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_L, -i_2+PB_M-2); i_3++) {
            B[i_3+i_2-1][1] = A[i_2][i_3] * B[i_3+i_2+2][1] + B[i_3+i_2-3][1] + B[i_3+i_2][1] * 6;
            C[i_3] = A[i_3][i_2+2] * 6;
        }
    }
#pragma endscop
...
kernel_2336212225_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
