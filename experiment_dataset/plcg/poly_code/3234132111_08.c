...
void kernel_3234132111_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_Q, -i_0+PB_P-2); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_P-4, i_1); i_2++) {
                A[i_0-1][i_2+i_1+2][i_2+1] = C[i_1][i_2+i_1][i_0+1] * A[i_1][i_2+i_0+4][i_2+2] + A[i_2-1][i_1+i_0+2][i_0] * 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_4+PB_P+1, -i_3+PB_Q); i_5++) {
                B[i_4][i_3-1][i_4][i_5-2] = D[i_4+1][i_5] * D[i_4][i_5] * C[i_4][i_5+i_3][i_5] * D[i_3][i_5+i_4-1] + D[i_3+2][0] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3234132111_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
