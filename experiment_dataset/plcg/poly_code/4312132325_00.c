...
void kernel_4312132325_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_L+2, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_Q+1; i_2++) {
                for (int i_3 = 1; i_3 < -i_0+PB_L; i_3++) {
                    B[i_1-2] = D[i_1][i_0][i_3-1][2] - E[i_2-1][i_3][i_1+i_0-2][i_3+2][i_3] + D[i_3][i_2][i_3+i_0][i_2+i_1-1] - 2;
                }
            }
            B[i_1-1] = F[i_0+2][2] + F[i_1][i_0] - G[i_0-1] - G[i_0] + B[i_0-2] + B[i_1+1] * 6;
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_L-2); i_5++) {
            for (int i_6 = 1; i_6 < PB_P; i_6++) {
                B[i_6-1] = H[i_5+2][i_4][i_6] - B[i_5-1] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4312132325_00(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
