...
void kernel_4314111325_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < min(min(PB_M, -i_0+PB_L), i_1); i_2++) {
                A[i_1-1][i_2+i_0] = A[i_2][i_1+i_0+1] * A[i_2-1][i_1+i_0+1] - 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M-1; i_4++) {
            ...
            C[i_4][i_3-1] = D[i_4][i_3] - C[i_4+1][i_3] * C[i_4][i_3+1] * C[i_3-1][i_4-1] - 3;
        }
    }
#pragma endscop
...
kernel_4314111325_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
