...
void kernel_3314111111_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M, i_0); i_1++) {
            A[i_1-1][0] = A[i_1-2][0] * 6;
            A[i_1][0] = 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            C[i_3+i_2][i_3+i_2] = D[i_2][i_2] * D[i_3+i_2][i_2] + D[i_3+i_2][0] - C[i_3+i_2-1][i_3+i_2-1] * 6;
        }
    }
#pragma endscop
...
kernel_3314111111_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
