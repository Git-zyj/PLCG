...
void kernel_2222251121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        A[i_0][i_0-1] = C[i_0][0] * D[i_0] - D[i_0] + D[i_0-1] - 6;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        B[i_1] = E[i_1] - E[0] * D[i_1-1] - D[i_1] + E[i_1] + B[i_1+2] * 4;
    }
#pragma endscop
...
kernel_2222251121_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
