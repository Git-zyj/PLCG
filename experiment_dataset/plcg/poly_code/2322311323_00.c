...
void kernel_2322311323_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_0+1][i_0+2] = B[i_0+1][i_0+2] + B[i_0-1][i_0] + B[i_0-2][i_0-1] + 5;
        }
        B[i_0-1][i_0] = 5;
    }
    for (int i_2 = 0; i_2 < PB_N-3; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < PB_M+1; i_3++) {
            C[i_2+1][i_3-1] = D[i_2+1] + C[i_2+3][i_3-2] + C[i_2+1][i_3-3] + 6;
        }
    }
#pragma endscop
...
kernel_2322311323_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
