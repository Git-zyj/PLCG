...
void kernel_4426131121_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
        ...
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < PB_Q; i_3++) {
                    A[i_1+i_0-1][i_2][i_0][i_3] = E[i_2][i_3][i_1][i_0] - E[i_3][i_3][i_0-1][i_1-1] + F[i_1-1][i_3][i_0][i_2-1][i_3] - 4;
                }
            }
        }
        for (int i_4 = i_0; i_4 < -i_0+PB_L; i_4++) {
            B[i_0][i_4+1] = G[i_4-1][i_0] - G[i_0+1][i_4+i_0] + 1;
        }
    }
        ...
        for (int i_6 = 0; i_6 < min(i_5, -i_5+PB_M+2); i_6++) {
            for (int i_7 = 1; i_7 < -i_5+PB_M; i_7++) {
                for (int i_8 = 0; i_8 < PB_Q; i_8++) {
                    A[i_7+i_5][i_6][i_5][i_8] = H[i_8][i_6][i_7-1] - A[i_6+i_5-2][i_7-1][i_5-1][i_8] - 4;
                }
            }
        }
        D[i_5-1] = I[i_5] * I[i_5+1] - I[i_5-1] * I[i_5+1] - I[i_5] + 6;
    }
#pragma endscop
...
kernel_4426131121_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
