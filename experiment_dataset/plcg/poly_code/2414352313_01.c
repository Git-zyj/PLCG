...
void kernel_2414352313_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_P-3); i_1++) {
            A[i_0][i_1+2] = E[i_1] + F[i_0+1][i_1] - G[i_1][i_0] - H[i_0-2][i_1] + G[i_1+2][i_0] * A[i_0+1][i_1+2] - A[i_0][i_1+1] - A[i_0][i_1+3] * A[i_0+1][i_1+1] + A[i_0-1][i_1+2] + 1;
            B[i_0-2][i_1+1] = G[i_1+i_0][i_0+2] - E[i_1-1] + G[i_0][i_1] - G[i_0][i_0] * F[i_0-1][i_1] - I[i_0+2] * J[i_1] * F[i_0+2][i_1+i_0-1] * A[i_0][i_1+1] - C[i_0] + 3;
        }
        C[i_0] = E[i_0+2] + G[i_0+1][i_0] + E[i_0] - E[i_0] + C[i_0+1] - C[i_0-1] - 2;
    }
    for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_L, PB_M); i_3++) {
            A[i_3][i_2+2] = F[i_3][i_3+i_2] - 5;
        }
    }
#pragma endscop
...
kernel_2414352313_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
