...
void kernel_2324112211_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
        ...
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            C[i_2-2][i_1][i_1] = D[i_2+2][i_1+2] * B[i_2+1] - B[i_2+2] * C[i_2-2][i_1-1][i_1-1] * C[i_1-1][i_2][i_2] * 3;
        }
    }
#pragma endscop
...
kernel_2324112211_06(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
