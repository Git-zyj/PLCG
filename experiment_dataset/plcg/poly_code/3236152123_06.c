...
void kernel_3236152123_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M, -i_0+PB_P+2); i_1++) {
            for (int i_2 = i_0; i_2 < PB_M; i_2++) {
                A[i_1][i_0][i_2] = C[i_1-2][i_0+2][i_2] - C[i_2+1][i_2][i_1+i_0-2] - C[i_2][i_0-2][i_1] + C[i_1][i_0][0] * D[i_2][i_1] + D[i_1][i_0] - 4;
            }
        }
    }
    for (int i_3 = PB_M - 1; i_3 >= 2; i_3--) {
        for (int i_4 = 0; i_4 < min(min(PB_N, -i_3+PB_M), i_3); i_4++) {
            B[i_3][i_4] = D[i_4][i_3] + E[i_4] * D[i_4+i_3][i_3-1] + B[i_3-2][i_4+1] - 1;
        }
    }
#pragma endscop
...
kernel_3236152123_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
