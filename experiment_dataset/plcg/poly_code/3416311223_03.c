...
void kernel_3416311223_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_1+1] = B[i_0] + C[0][i_0-2] + 2;
        }
        B[i_0] = E[i_0-1] + 2;
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            for (int i_3 = 3; i_3 < i_0; i_3++) {
                C[0][i_3-1] = F[i_3][0] - C[0][i_3-3] * 4;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
            D[i_5-1] = G[i_5][i_4][i_4] - D[i_5-2] + D[i_5+1] * D[i_5] + 6;
        }
    }
#pragma endscop
...
kernel_3416311223_03(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
