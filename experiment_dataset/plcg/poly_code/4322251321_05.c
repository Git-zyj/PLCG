...
void kernel_4322251321_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = i_1; i_2 < PB_P-1; i_2++) {
                for (int i_3 = min(-i_2+PB_L, -i_1+PB_Q) - 1; i_3 >= 0; i_3--) {
                    A[i_0-1][i_3+1][i_0][i_1+1] = D[i_2][i_1+1][i_3][i_0] * E[i_1-1][i_0+1][i_2][i_3+i_1] * D[i_0+1][i_3+i_2][i_1][i_1] - E[i_1][i_3+1][i_0+1][1] - D[i_1][i_2][i_3][i_3] + E[i_1-1][i_2-1][i_3][i_0] * A[i_0-1][i_3+1][i_0][i_1-1] + A[i_0+1][i_3+1][i_0+2][i_1] * A[i_0][i_2+1][i_0+1][i_1+1] - A[i_0][i_3+1][i_0+1][i_1+1] - 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        ...
        for (int i_5 = 0; i_5 < -i_4+PB_L-1; i_5++) {
            B[i_4] = G[i_5+i_4][i_5+i_4+1] - H[i_5] + H[i_4] - H[i_5] - G[i_4][i_5] * G[i_4][0] - 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4322251321_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
