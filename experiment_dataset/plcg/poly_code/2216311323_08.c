...
void kernel_2216311323_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[0] = C[i_1] + A[0] + B[i_0][i_1] + B[i_0-2][i_1-2] * 4;
            B[i_0][i_1] = A[0] - A[0] - A[0] - 5;
        }
    }
#pragma endscop
...
kernel_2216311323_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
