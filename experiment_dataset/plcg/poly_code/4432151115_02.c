...
void kernel_4432151115_02(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            D[i_0+1][i_0+1][i_1] = E[i_1] - F[i_0][i_1] * F[i_0-1][i_1] * F[i_1-1][i_0] - D[i_0][i_0][i_1] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2+1] = E[1] + E[i_2] - E[i_2+1] * E[i_2+1] + E[i_2-1] * 4;
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_P); i_3++) {
            for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
                for (int i_5 = 0; i_5 < -i_4+PB_L; i_5++) {
                    D[i_3-1][i_3-1][i_2-1] = G[i_5+i_4][i_3][i_4] - H[i_3][i_2-1][i_4][i_2] - G[i_4][i_3+i_2][i_5] * H[i_5][i_4-1][i_3+1][i_5] + H[i_3+1][i_4+1][0][i_4] + H[i_3-1][i_2-1][i_5+1][0] - 4;
                }
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 0; i_7 < PB_M; i_7++) {
            for (int i_8 = 0; i_8 < PB_Q-1; i_8++) {
                D[i_7][i_7][i_8] = F[i_8][i_7] * H[i_6][i_7][i_8][i_8+1] * D[i_6+1][i_6+1][i_8] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4432151115_02(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
