...
void kernel_2212311321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_1][i_0+1] = A[i_1][i_0-1] + A[i_1][i_0+2] * A[i_1][i_0] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            A[i_2][i_3-1] = C[i_3+1] + 2;
        }
    }
#pragma endscop
...
kernel_2212311321_03(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
