...
void kernel_3314351321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] + E[i_0-1] - F[i_0] - D[i_0] - A[i_0+1] * A[i_0-2] + 4;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        B[i_1] = F[i_1-1] * E[0] + D[i_1-1] * E[i_1] * B[i_1+2] + B[i_1-1] * 5;
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_2+PB_L, -i_1+PB_Q); i_3++) {
                C[i_1][i_2][i_3] = G[i_3+i_2][i_1][i_1] - H[i_3+1][i_1-1] - G[i_1+1][i_2][i_3] + I[i_2-1][i_3+i_1][i_1] - I[i_1][i_3][i_1] + C[i_1-1][i_2][i_3] * C[i_3-1][i_2][i_1] + C[i_1][i_2-2][i_3] * C[i_2+1][i_3][i_1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3314351321_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
