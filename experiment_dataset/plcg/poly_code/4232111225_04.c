...
void kernel_4232111225_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
        B[i_3] = C[i_3] - B[i_3+2] + B[i_3+1] * B[i_3-1] + B[i_3-2] - 1;
    }
#pragma endscop
...
kernel_4232111225_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
