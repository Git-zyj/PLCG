...
void kernel_4226312111_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        ...
        for (int i_2 = 0; i_2 < min(PB_P, -i_0+PB_L-2); i_2++) {
            for (int i_3 = 0; i_3 < i_2; i_3++) {
                B[i_2][i_3+i_0+1][0] = C[i_3][i_0][i_2] + B[i_3][i_2+i_0+2][0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4226312111_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
