...
void kernel_3326312111_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < PB_L; i_5++) {
            for (int i_6 = 0; i_6 < min(PB_M, -i_4+PB_P); i_6++) {
                C[i_5][i_4][i_6] = D[i_6][i_5][i_6+i_4] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3326312111_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
