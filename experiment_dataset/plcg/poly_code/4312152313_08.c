        ...
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_N; i_2++) {
                for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
                    B[i_1-1][i_0+1][i_1] = D[0][i_1][i_3][i_0+1] - D[i_1][i_0+1][i_0][i_3+2] + D[i_2][i_0][i_1-1][i_1] * D[i_2+i_0][i_3][i_1][i_1-1] + D[i_0][i_3][i_1][i_0] * E[i_0+1][i_3][i_2][i_1][i_0-1] - 1;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_Q-2; i_4++) {
            for (int i_5 = max(2, i_0); i_5 < PB_N-1; i_5++) {
                B[i_0-1][i_5][i_0] = F[i_0+2][i_5][i_4+2] * F[i_4][i_0][i_4] * B[i_0-1][i_5+1][i_0] - B[i_5-2][i_0][i_5-1] - B[i_0][i_5][i_0+1] * 3;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_L; i_6++) {
        for (int i_7 = 2; i_7 < PB_L-1; i_7++) {
            B[i_7-1][i_6][i_7] = G[i_7+1][i_6-1] - H[i_7] + G[i_6][i_7-2] + G[i_7][i_6+2] * H[i_7] * 1;
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4312152313_08(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
