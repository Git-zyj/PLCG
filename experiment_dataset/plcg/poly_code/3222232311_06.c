...
void kernel_3222232311_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_P-2; i_2++) {
                A[i_0][i_2+i_0-2][i_1] = C[i_2+2][i_1+1] + D[i_1][i_2+i_0+2][i_0] * A[i_2-1][i_2+i_0-3][i_1] + A[i_0-1][i_2+i_0-2][i_1-1] - 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < PB_P; i_4++) {
            for (int i_5 = 2; i_5 < min(min(PB_P, -i_4+PB_M+1), -i_3+PB_M-1); i_5++) {
                B[i_5+i_3][i_3][i_4+2] = E[i_5-2][i_3][1] - D[i_5+i_3][i_4][i_4] * C[i_4][i_5] * D[i_4+1][i_5][i_3] - A[i_4][i_5+i_4-1][i_3] - B[i_5+i_3+1][i_3][i_4+1] * B[i_5+i_4-1][i_4][i_3+3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3222232311_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
