...
void kernel_4332131225_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_1+1] = D[i_0][i_1] + D[i_1][i_0] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_L, -i_2+PB_M-1); i_3++) {
            B[i_3+1] = D[i_2][i_3] + E[i_3] - D[i_2][1] * E[i_3+i_2+1] - E[i_3] - D[i_3][0] - B[i_3-1] * 3;
        }
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        for (int i_5 = 2; i_5 < PB_Q; i_5++) {
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                C[i_4][i_5][i_6] = D[i_6][i_5] - B[i_6+1] - C[i_4-1][i_6][i_5-2] + C[i_4-1][i_5][i_6] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4332131225_07(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
