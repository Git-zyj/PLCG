...
void kernel_2234111221_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        ...
        for (int i_2 = i_0; i_2 < -i_0+PB_M-3; i_2++) {
            B[i_2+i_0+1][i_2] = A[i_2][i_0] * B[i_2+i_0-2][i_2-1] + B[i_2+i_0+3][i_2+2] + 2;
        }
    }
#pragma endscop
...
kernel_2234111221_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
