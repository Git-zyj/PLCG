...
void kernel_3432111315_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_5 = 0; i_5 < PB_P; i_5++) {
        for (int i_6 = 1; i_6 < min(PB_P, -i_5+PB_M); i_6++) {
            B[i_6-1] = E[i_6+i_5][0] + 5;
        }
    }
    for (int i_7 = 1; i_7 < PB_M+1; i_7++) {
        D[i_7] = F[i_7] - B[i_7-1] * D[i_7+1] * D[i_7-1] - 3;
    }
#pragma endscop
...
kernel_3432111315_04(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
