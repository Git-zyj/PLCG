...
void kernel_4426151123_00(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_L, PB_M-2); i_2++) {
                D[i_2+1][i_2+2] = E[i_2][i_1-1][i_0][i_1] - F[i_2-1][i_0-1] + G[i_2][i_0][i_0] * G[i_2][i_0-1][0] - G[i_2+1][i_0-1][i_1+1] * G[i_0+1][i_2+i_1][i_1-1] - D[i_1-1][i_1] * 4;
            }
        }
        for (int i_3 = max(2, i_0); i_3 < PB_M; i_3++) {
            D[i_3-2][i_3-1] = F[i_3][i_0] + F[i_3-1][i_0+1] * H[i_0] * F[i_3][i_0] - 5;
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < PB_M; i_5++) {
            for (int i_6 = 1; i_6 < min(min(i_4, -i_5+PB_P), PB_M-1); i_6++) {
                for (int i_7 = 0; i_7 < min(min(i_6, -i_5+PB_N), PB_M-1); i_7++) {
                    D[i_7][i_7+1] = E[i_4][i_6][i_7+i_5][1] - E[i_6][i_7][i_7][i_5] + G[0][i_6-1][i_7] + E[i_7+i_5][i_6+1][i_4][i_6+i_5] + 5;
                }
            }
        }
    }
    for (int i_8 = 0; i_8 < PB_N; i_8++) {
        for (int i_9 = max(i_8, 1); i_9 < min(-i_8+PB_N, PB_M-1); i_9++) {
            D[i_9][i_9+1] = F[i_9][i_8] * H[i_9+i_8] * F[i_9-1][i_8] - F[i_9+1][i_8] * H[i_9] + 6;
        }
    }
#pragma endscop
...
kernel_4426151123_00(xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
