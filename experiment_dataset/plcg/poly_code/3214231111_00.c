...
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            A[i_0][i_1] = C[i_1+i_0][i_1+1] * C[i_0][i_1+1] + D[i_0][0] * 3;
        }
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            for (int i_3 = 1; i_3 < PB_M; i_3++) {
                B[i_2][i_3-1][i_0-1] = E[i_0][i_2][i_3] - F[i_3][i_0][i_2] - B[i_2][i_3][i_0-1] + B[i_2-1][i_3-1][i_0-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3214231111_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
