...
void kernel_4414132313_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        A[i_0-2] = E[i_0] + E[i_0] * 5;
        A[i_0-2] = E[i_0] + A[i_0-3] * A[i_0-2] - A[i_0-1] * 1;
    }
    for (int i_1 = 2; i_1 < PB_M+1; i_1++) {
        for (int i_2 = i_1; i_2 < min(-i_1+PB_L, PB_M-2); i_2++) {
            for (int i_3 = 0; i_3 < -i_2+PB_M-1; i_3++) {
                C[i_2+2][i_3] = F[i_3][i_2+2][i_1] * G[i_3+i_2+1][0] * F[i_1-1][i_2][i_3] * F[i_2][i_2+i_1][i_3+2] + C[i_2+1][i_3] * D[i_3][i_2][i_1+1][i_1-1] + 5;
            }
        }
        for (int i_4 = 2; i_4 < PB_P; i_4++) {
            for (int i_5 = 0; i_5 < PB_N; i_5++) {
                for (int i_6 = 2; i_6 < min(PB_P, i_1); i_6++) {
                    D[i_6-1][i_4][i_1][i_1-2] = H[i_4][i_1][i_6][i_5] + H[i_5][i_4][i_4-2][i_1] * H[i_6-1][i_1][i_4][i_5] - D[i_1-2][i_4][i_6][i_6-2] - D[i_6-2][i_4][i_1+1][i_1-1] - 2;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414132313_03(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
