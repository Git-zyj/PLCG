...
void kernel_3236212321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 3; i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_P; i_2++) {
                A[i_2-2][i_1+i_0][i_1] = C[i_0+2][i_2][i_2] + A[i_0-2][i_2+i_1-3][i_1-2] - A[i_1-1][i_2+i_0][i_2] * 3;
            }
        }
    }
    for (int i_3 = 4; i_3 < PB_M; i_3++) {
        B[i_3-2] = B[i_3-1] * B[i_3] * B[i_3-4] - B[i_3-3] - 5;
    }
#pragma endscop
...
kernel_3236212321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
