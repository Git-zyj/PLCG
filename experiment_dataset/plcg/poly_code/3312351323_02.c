...
void kernel_3312351323_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0-1] - E[i_0] - F[i_0] * E[i_0] - A[i_0-2] + A[i_0+2] + 2;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
                C[i_3+2][i_2-1] = G[i_2][i_1][i_3] * H[i_3+1][i_3][i_2][i_1] + I[i_3-1][i_2-1][i_1+1] * J[i_2+1][i_1][i_3] - I[i_2][i_3][i_1-1] * G[i_2][i_3][0] + C[i_2][i_3] - 5;
            }
            C[i_1][i_2] = K[i_2][i_1] - D[i_2] + L[i_2][i_1] * D[i_2+1] + M[i_2][i_1+1] - A[i_2+2] - C[i_1+1][i_2] * C[i_1+1][i_2-2] - 3;
        }
    }
#pragma endscop
