...
void kernel_2234312321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
            A[i_0+2][i_1] = C[i_1] - A[i_0+2][i_1+2] * A[i_0+4][i_1] - A[i_0+1][i_1] * 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 3; i_3 < PB_L; i_3++) {
            B[i_2][i_3-1] = D[i_2+1][i_3] * B[i_2-2][i_3-1] * B[i_2][i_3-2] - B[i_2-1][i_3-3] + 5;
        }
    }
#pragma endscop
...
kernel_2234312321_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
