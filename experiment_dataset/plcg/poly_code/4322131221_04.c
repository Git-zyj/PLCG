...
void kernel_4322131221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L-1; i_1++) {
            A[i_0+1][i_1+i_0] = D[i_1-1][i_0-1] - D[i_1][i_0] - A[i_0+1][i_1+i_0+1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_Q, -i_2+PB_L); i_3++) {
            A[i_2+1][i_3+i_2] = E[i_2][i_3+i_2][i_3] - D[i_2][i_3+1] + D[i_3][i_2+1] + D[i_2][i_3] * 4;
        }
    }
    for (int i_4 = 4; i_4 < PB_Q; i_4++) {
        for (int i_5 = 1; i_5 < min(PB_Q, -i_4+PB_M+4); i_5++) {
            for (int i_6 = 0; i_6 < -i_4+PB_M-2; i_6++) {
                C[i_6+i_4][i_5][i_6+i_4] = E[i_6+i_4][i_5][i_5] + E[i_6][i_5-1][i_4] * D[i_5+i_4+1][i_6] + C[i_6+i_4+2][i_5-1][i_6+i_4+2] * C[i_5+i_4-4][i_6][i_5+i_4-4] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4322131221_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
