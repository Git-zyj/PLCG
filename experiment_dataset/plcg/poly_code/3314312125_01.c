...
void kernel_3314312125_01(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
            for (int i_2 = i_0; i_2 < -i_1+PB_L; i_2++) {
                C[i_0-2][i_1+1] = D[i_0+2][i_2+i_1][2] * D[i_1-2][i_2][i_0+2] - 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M+2; i_3++) {
        ...
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            C[i_3-2][i_4+1] = E[i_4][i_3-2] + C[i_3-2][i_4-1] - C[i_3-2][i_4+3] - 6;
        }
    }
#pragma endscop
...
kernel_3314312125_01(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
