...
void kernel_4216312121_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < min(min(PB_P-2, i_0), -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P-2; i_2++) {
                A[i_0][i_2][i_1+i_0] = A[i_2+1][i_0+2][i_2+i_1+2] * 6;
            }
            for (int i_3 = 0; i_3 < min(PB_M-4, -i_0+PB_L-2); i_3++) {
                for (int i_4 = 2; i_4 < i_1; i_4++) {
                    B[i_0+2][i_4+i_1][i_3+2][i_4-2] = C[i_1-2][i_3+i_0+2][i_4][i_4+i_1+1] - B[i_0+2][i_4+i_1][i_3+4][i_1-2] * 4;
                }
            }
        }
    }
...
kernel_4216312121_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
