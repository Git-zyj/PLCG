...
void kernel_2426251323_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        B[1] = E[i_0] + E[1] + D[i_0-2] - D[i_0] + D[i_0+2] - 3;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        B[1] = F[i_1] * E[i_1+1] - E[i_1+1] + E[i_1] - E[i_1] * B[1] + B[1] * D[i_1-2] - 4;
        for (int i_2 = 1; i_2 < min(PB_L-1, -i_1+PB_P-1); i_2++) {
            C[i_2] = G[i_2][i_2+i_1+1] - H[i_1][i_2-1] * I[i_2][i_2+1][i_1] + B[1] + 6;
        }
        D[i_1] = E[i_1-1] * F[i_1] + F[i_1+1] + F[i_1+1] - E[i_1-1] - F[1] + E[i_1] * B[1] * 1;
    }
#pragma endscop
...
kernel_2426251323_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
