...
void kernel_4434132123_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1-1] = E[0] * F[i_0][i_1][i_0] + G[i_1][i_0] + B[i_1+1] * 6;
        }
        B[i_0+1] = G[i_0][0] * E[i_0] + E[0] - B[i_0-1] - 3;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L; i_3++) {
            for (int i_4 = 1; i_4 < -i_2+PB_M; i_4++) {
                A[i_4-1] = F[i_3][i_4][i_2] * F[i_3][i_4+i_2][i_3+i_2] + 6;
            }
        }
    }
        ...
        for (int i_6 = 0; i_6 < i_5; i_6++) {
            for (int i_7 = 2; i_7 < PB_M; i_7++) {
                for (int i_8 = 2; i_8 < PB_N-1; i_8++) {
                    A[i_8-1] = H[i_8-2][i_5+2][i_8][i_6] + F[i_8][i_7-1][i_6] + H[i_6][i_8][i_7-2][i_5] * H[i_8+1][i_7][i_6][i_5] + 1;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434132123_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
