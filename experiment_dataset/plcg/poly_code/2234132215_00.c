...
void kernel_2234132215_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        B[i_0+2] = C[i_0-2] * C[i_0-1] + B[i_0+2] - 5;
    }
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_N; i_2++) {
            B[i_2+2] = D[i_2-2][i_1-2] + D[i_2+i_1][2] + D[i_2][i_1] + B[i_2+3] - 2;
        }
    }
#pragma endscop
...
kernel_2234132215_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
