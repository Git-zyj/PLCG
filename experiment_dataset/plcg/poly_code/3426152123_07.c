...
void kernel_3426152123_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0+1] = E[i_0+1] - E[i_0] * A[i_0+2] * 1;
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                C[i_1-1][i_0-2][i_2] = F[i_1][i_2][i_1] + F[i_2][i_1+i_0][i_2] + F[i_0][i_2][i_1] - F[i_2][i_0][i_1-2] * F[i_1-2][i_0][i_2] + F[i_2][i_1+i_0][i_0] + G[i_2][i_1] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q; i_5++) {
                C[i_4-1][i_3][i_5] = G[i_4][i_5] + G[i_5+2][i_4] - 4;
            }
        }
        A[i_3+1] = E[i_3+2] * G[i_3][i_3+2] * E[i_3-1] * E[i_3] * G[i_3-1][2] * E[i_3] + E[i_3+2] - A[i_3+3] * 1;
    }
#pragma endscop
...
kernel_3426152123_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
