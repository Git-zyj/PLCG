...
void kernel_2234311315_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][i_1][i_1] = A[i_1][i_0-1][i_0-1] * B[i_1+1] + B[i_1+2] * 2;
        }
    }
    for (int i_2 = PB_L-1 - 1; i_2 >= 0; i_2--) {
        B[i_2+1] = C[i_2+1] * C[i_2] + B[i_2] - B[i_2+2] - 3;
    }
#pragma endscop
...
kernel_2234311315_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
