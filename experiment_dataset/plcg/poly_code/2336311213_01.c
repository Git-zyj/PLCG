...
void kernel_2336311213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1-1][i_0] = D[i_0][i_1] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M-1; i_3++) {
            B[i_2][i_3+i_2-1] = B[i_2+1][i_3+i_2+1] - B[i_2+1][i_3+i_2] * B[i_2-1][i_3+i_2-2] - 6;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < PB_P-1; i_5++) {
            C[i_4+1][i_5] = C[i_4+2][i_5-1] + C[i_4+1][i_5+1] - 6;
        }
    }
#pragma endscop
...
kernel_2336311213_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
