...
void kernel_4324251211_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-1] = D[i_0] - E[i_0] * E[i_0+1] - D[i_0-1] + A[i_0] * A[i_0-2] * 1;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            B[i_2][i_1-1][i_1] = F[i_1-1][i_2][i_1] + D[i_1] + G[i_1][i_2-1] + G[i_2][i_1] * E[i_1+1] - H[i_2][i_1-1] - D[i_2+1] * A[i_1] - B[i_2-1][i_1-1][i_1] - 2;
        }
        for (int i_3 = 1; i_3 < -i_1+PB_L; i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_Q-1; i_4++) {
                B[i_3][i_1-1][i_1] = F[i_4-1][i_3+i_1][i_3+i_1] - F[i_4][i_1+1][i_3] - I[i_1][i_3-1][i_4] * I[i_1][i_4+i_3+1][i_3] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4324251211_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
