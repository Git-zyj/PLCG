...
void kernel_4314332113_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        A[i_0+1] = D[i_0] + E[i_0-1] - A[i_0+2] - A[i_0] * 2;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
                B[i_2+i_1][i_3-1][i_3+1] = F[i_1][i_3+1][i_2] * G[i_2-1][i_1] * 5;
            }
            for (int i_4 = 0; i_4 < min(PB_P-2, -i_1+PB_M-1); i_4++) {
                for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
                    C[i_2][i_4][i_4] = F[i_5][i_4][i_2] * H[i_2+1][i_5-2][i_1-1][i_5+1] * I[i_2+1][i_1][i_5][i_4+2] - H[i_2][i_4+i_1+1][i_5+2][i_1] + C[i_2+1][i_4][i_4] - 4;
                }
            }
        }
    }
...
kernel_4314332113_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
