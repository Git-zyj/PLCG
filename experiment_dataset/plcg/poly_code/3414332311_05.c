...
void kernel_3414332311_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        A[i_0-2] = E[i_0] - F[i_0+2] + A[i_0-3] * A[i_0-1] - A[i_0-1] + 4;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            B[i_1][i_2] = G[i_2][i_2] + G[i_2][i_1] + H[i_2+2][i_2] + B[i_1][i_2-1] + B[i_1-1][i_2] - B[i_1+1][i_2] - B[i_1][i_2+1] - 2;
        }
        C[i_1] = F[i_1] * E[i_1] + F[0] - F[i_1] * A[i_1-2] - C[i_1-1] + 5;
        for (int i_3 = 0; i_3 < i_1; i_3++) {
            for (int i_4 = 1; i_4 < PB_M; i_4++) {
                B[i_1+1][i_3+1] = H[i_4][i_1] * I[i_3+2][i_1+1][i_4-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3414332311_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
