...
void kernel_3312311211_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_Q, -i_0+PB_P); i_2++) {
                A[i_0-1][i_1][i_2] = D[i_1-1][i_2+i_0][i_1] + A[i_0-1][i_1][i_2-1] * A[i_1-1][i_2-1][i_0] * 4;
            }
            B[i_1] = E[i_1-1] * B[i_0-1] - B[i_1-1] - B[i_1+1] + 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_M; i_5++) {
                C[i_3][i_5+i_4][i_3+1] = 3;
            }
        }
    }
#pragma endscop
...
kernel_3312311211_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
