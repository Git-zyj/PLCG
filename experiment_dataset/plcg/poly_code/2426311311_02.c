...
void kernel_2426311311_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_L-1); i_1++) {
            A[i_1-1][i_1] = A[i_1-2][i_1-1] - A[i_1][i_1+1] - 3;
            B[i_0][i_1] = A[i_1-1][i_1] - 1;
        }
        for (int i_2 = max(1, i_0); i_2 < PB_L; i_2++) {
            C[i_0+1][i_2] = E[i_0] - C[i_0+1][i_2-1] + C[i_0][i_2-1] + C[i_0][i_2] - 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_L-1, i_3); i_4++) {
            D[i_4][i_4+i_3] = D[i_4+1][i_4+i_3+1] - D[i_4-1][i_4+i_3-1] + D[i_4][i_4+i_3+1] * D[i_4-1][i_4+i_3-2] + 6;
        }
    }
#pragma endscop
...
kernel_2426311311_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
