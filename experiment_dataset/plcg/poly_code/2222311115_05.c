...
void kernel_2222311115_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            B[i_1-1][i_0-2][i_1] = C[i_0][i_1] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < PB_Q; i_3++) {
            B[i_3-1][i_2-1][i_3] = B[i_3-2][i_2][i_3-1] - 6;
        }
    }
#pragma endscop
...
kernel_2222311115_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
