...
void kernel_3214311125_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            A[i_1-1] = C[i_1+1][i_0] + D[i_0-1][i_1] - A[i_1+1] * 4;
        }
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            B[i_2][i_0-1] = B[i_2+1][i_0-1] * 2;
        }
    }
#pragma endscop
...
kernel_3214311125_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
