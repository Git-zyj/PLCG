...
void kernel_3326132311_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = D[i_1-2] - E[i_1][i_0] - A[i_1+1][i_0] + A[i_1][i_0-1] * A[i_1][i_0+1] + A[i_1+1][i_0+1] - A[i_0][i_1-1] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            C[i_3+1][i_2] = E[i_3][i_2+1] * E[i_3][i_2] + 6;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_M); i_5++) {
            C[i_5+1][i_4] = E[i_5-1][i_4] - E[i_5+i_4][i_4+1] - D[i_4] - C[i_5][i_4] + 2;
        }
    }
#pragma endscop
...
kernel_3326132311_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
