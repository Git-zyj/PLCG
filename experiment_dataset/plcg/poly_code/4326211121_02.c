...
void kernel_4326211121_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < PB_N; i_1++) {
            for (int i_2 = i_1; i_2 < PB_N; i_2++) {
                A[i_2][i_1-1] = A[i_2][i_1-3] + A[i_1][i_2+1] - 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
            A[i_4+1][i_3-1] = D[i_4][0] * 6;
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = 1; i_6 < min(PB_N, i_5); i_6++) {
            C[i_6-1][i_6] = D[i_6][1] - 5;
        }
    }
#pragma endscop
...
kernel_4326211121_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
