...
void kernel_2426352121_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] + F[i_0] + F[i_0] + G[i_0] * 3;
    }
    for (int i_1 = 3; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            B[i_1][i_2] = H[i_2][i_1] * I[i_2][i_1-2] + I[i_2+i_1][i_2] * J[i_2][i_1] * E[i_2+i_1] - 5;
        }
        D[i_1-2] = G[i_1-2] * G[i_1] * E[i_1] - H[i_1][i_1-2] - G[i_1] + E[i_1] + F[i_1] + I[i_1+2][i_1] + E[i_1] * D[i_1-3] + 6;
        D[i_1-2] = F[i_1+2] + G[i_1] - 1;
    }
#pragma endscop
...
kernel_2426352121_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
