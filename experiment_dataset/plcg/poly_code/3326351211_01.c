...
void kernel_3326351211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0+1] = D[i_0+1] - E[i_0+1] * F[i_0][i_0] + E[i_0] + A[i_0] + 1;
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            B[i_0-1][i_1+1] = G[i_1][i_0] * H[i_0][i_1] + I[i_1+1][i_0][i_1+1] - F[i_0][i_1+1] * G[i_1][i_0-1] + A[i_1] * B[i_0][i_1+1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < min(min(PB_N-1, i_2), -i_2+PB_L-1); i_3++) {
            A[i_3+1] = H[i_2][i_3] * H[i_3][i_2-1] - H[i_2][i_3] + H[i_3+1][i_3+i_2+1] - F[i_3+1][i_2] + 2;
        }
    }
#pragma endscop
...
kernel_3326351211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
