...
void kernel_2322112321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            A[i_1] = D[i_1-1][i_0] * D[i_0][i_1] + D[i_0][i_1] - A[i_1+1] - A[i_1+2] + 1;
        }
        for (int i_2 = 2; i_2 < min(i_0, PB_L-2); i_2++) {
            B[i_2][i_2] = A[i_2] - B[i_2-2][i_2-2] * B[i_2+2][i_2+2] - 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_P+1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
            C[i_4+2][i_3-1] = C[i_4+1][i_3-1] * C[i_4+1][i_3-2] + C[i_4+2][i_3-2] * C[i_4][i_3-1] * 3;
        }
    }
#pragma endscop
...
kernel_2322112321_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
