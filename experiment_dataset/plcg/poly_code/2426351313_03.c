...
void kernel_2426351313_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
            B[i_1] = E[i_0] - F[i_0][i_1] - G[i_0+1][i_1+1] * F[i_0-1][1] * F[i_0][i_1+1] - B[i_1] * C[i_0+2][i_1] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        B[i_2] = H[i_2] + I[i_2] + H[i_2] * E[i_2+1] * E[i_2-1] + I[i_2] - E[i_2] * H[i_2] - I[i_2-1] * B[i_2+1] - 2;
        for (int i_3 = i_2; i_3 < PB_Q; i_3++) {
            C[i_2+1][i_3] = J[i_2][i_3] + G[i_2+1][i_3] - E[i_2] * B[i_3] + C[i_3+2][i_2] * C[i_2+1][i_3-1] + 4;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        D[i_4+1] = H[i_4] * H[i_4] - D[i_4+2] - 2;
    }
#pragma endscop
...
kernel_2426351313_03(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
