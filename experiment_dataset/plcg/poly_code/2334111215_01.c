...
void kernel_2334111215_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        ...
        for (int i_3 = i_1; i_3 < PB_L-1; i_3++) {
            C[i_1][i_3] = C[i_1][i_3-1] * C[i_1-1][i_3] * C[i_1-1][i_3+1] * C[i_1][i_3+1] - 5;
        }
    }
#pragma endscop
...
kernel_2334111215_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
