...
void kernel_2214131221_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            A[i_0][i_1+i_0-1][i_1] = C[i_1-1][i_0] - C[i_1-1][i_0-1] - C[i_1+1][i_0+1] * C[i_0][i_1] - A[i_0][i_1+i_0][i_1+1] + B[i_0][i_1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_L-1); i_3++) {
            B[i_2][i_3] = C[1][i_3+i_2+1] + C[i_3][i_2-1] + B[i_3][i_2+2] * B[i_2-1][i_3] + 4;
        }
    }
#pragma endscop
...
kernel_2214131221_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
