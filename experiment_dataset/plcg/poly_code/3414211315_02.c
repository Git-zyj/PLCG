...
void kernel_3414211315_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_0][i_1] = E[i_1] + E[i_1-1] * A[i_0-1][i_1] + 1;
        }
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            for (int i_3 = i_0; i_3 < PB_M; i_3++) {
                B[i_2][i_0][i_3] = F[i_2][i_3][i_0] + B[i_3][i_0-1][i_2+1] * B[i_3][i_2+1][i_0+1] + B[i_2][i_3-1][i_0] - B[i_2][i_0-1][i_3] + 1;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_Q-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
            A[i_5][i_4-1] = A[i_5+1][i_4+1] - 5;
        }
        for (int i_6 = 1; i_6 < i_4; i_6++) {
            for (int i_7 = 0; i_7 < PB_P-1; i_7++) {
                D[i_4][i_6+1][i_7] = D[i_4-1][i_7][i_6-1] * D[i_4-1][i_6][i_7] + D[i_4][i_6+1][i_7+1] * 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_3414211315_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
