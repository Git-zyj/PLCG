...
void kernel_4316132315_04(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                for (int i_3 = 1; i_3 < min(PB_P-3, i_0); i_3++) {
                    B[i_0][i_2][i_3+3][i_3-1] = D[i_2][i_2][i_0][i_1+1] * D[i_3][i_0][i_2][i_1] + 6;
                }
            }
            for (int i_4 = i_1; i_4 < -i_1+PB_M-1; i_4++) {
                for (int i_5 = 3; i_5 < PB_P-2; i_5++) {
                    B[i_0][i_4][i_5+2][i_5-2] = D[i_5][i_1-1][i_0+2][i_4] * E[i_4+i_1+1][i_5][i_1+2] * D[i_1][i_0][i_5-2][2] - B[i_5-1][i_4][i_0+2][i_0-2] - B[i_0][i_4][i_5+1][i_5-3] + 5;
                }
            }
        }
        for (int i_6 = i_0; i_6 < PB_P-3; i_6++) {
            for (int i_7 = 0; i_7 < min(i_6, -i_0+PB_L); i_7++) {
                for (int i_8 = 2; i_8 < PB_M-1; i_8++) {
                    B[i_0-1][i_7][i_6+3][i_6-1] = D[i_6-1][i_8][i_7+1][2] + D[i_6][i_7][i_0][0] - D[i_8+i_7][i_8-2][i_6][i_0+1] * D[i_6][i_7+i_0][i_8][i_0+2] - B[i_8+1][i_7][i_0+1][i_0-3] - 3;
                }
            }
        }
    }
...
kernel_4316132315_04(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
