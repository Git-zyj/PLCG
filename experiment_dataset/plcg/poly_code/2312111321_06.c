...
void kernel_2312111321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        ...
        for (int i_3 = i_1; i_3 < PB_M-1; i_3++) {
            C[i_3][i_1] = D[i_1] - C[i_3-1][i_1] * C[i_3][i_1-2] - C[i_3+1][i_1] + 1;
        }
    }
#pragma endscop
...
kernel_2312111321_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
