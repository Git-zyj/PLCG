...
void kernel_4424251225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = E[0] + F[i_0] * F[i_0] - E[i_0] - A[i_0-2] + A[i_0-1] * 2;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        A[i_1] = E[i_1] - E[i_1] - E[i_1-1] * F[i_1] + F[i_1] + 1;
        for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_1+PB_P+1, PB_Q-1); i_3++) {
                for (int i_4 = 1; i_4 < PB_Q-1; i_4++) {
                    C[i_1+1][i_3][i_4][i_2] = G[i_3+1][i_2][i_4+1][i_4] * G[i_2][i_3+i_1-1][i_4][i_1] - H[i_3][i_2][i_4+1][i_4] + G[i_1-1][i_4][i_3+1][i_2] * G[i_3][i_1+1][i_2-1][i_4] - G[i_2-1][i_3][i_1][i_2] * C[i_2][i_3][i_4-1][i_1+1] + 3;
                }
                for (int i_5 = 1; i_5 < min(i_2, -i_1+PB_L+1); i_5++) {
                    C[i_1+1][i_3-1][i_5][i_2] = G[i_2][i_5][i_1-1][i_2] * G[i_3][i_5-1][i_2+1][0] * G[i_5+i_1-1][i_2][0][i_3] - H[i_1][i_5][i_3][i_5] + 6;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4424251225_06(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
