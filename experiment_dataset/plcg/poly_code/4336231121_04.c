...
void kernel_4336231121_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 1; i_3 < PB_P; i_3++) {
                    A[i_2][i_3+i_1][i_3][i_0] = D[i_3-1][i_0][i_2][0] - E[i_2][i_3][i_0][i_1+1] * D[i_3][i_1+i_0][i_2][0] * D[i_3][i_1][i_0][i_2] * A[i_3][i_2+i_1][i_1][i_0-1] + 1;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_P; i_4++) {
            for (int i_5 = 1; i_5 < i_4; i_5++) {
                B[i_0][i_5][i_4][1] = F[i_4][i_5-1][1] * F[i_4-1][i_0-1][i_5] + C[i_4+1][i_5] * 3;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
        for (int i_7 = 1; i_7 < PB_L; i_7++) {
            C[i_6+1][i_7] = G[i_7-1][i_6] * H[i_6+1] * 6;
        }
    }
#pragma endscop
...
kernel_4336231121_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
