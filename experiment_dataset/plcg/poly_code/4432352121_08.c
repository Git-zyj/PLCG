...
void kernel_4432352121_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            A[i_1][i_0] = E[i_1][i_0-2] * E[i_0][0] * F[i_1][i_0] - G[i_1][i_0] * E[i_0][0] - G[i_1+i_0][1] * 6;
        }
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            B[i_2] = F[i_0-2][i_2] + H[i_0+2] - G[i_2][i_0-1] - F[i_2][i_0] + B[i_2-2] + 4;
        }
        for (int i_3 = 0; i_3 < min(-i_0+PB_L-2, i_0); i_3++) {
            C[i_3+i_0][i_0] = I[i_0] * G[i_0][i_3] - F[i_3+1][i_0] + G[i_3][i_0] - E[i_3+2][i_0] - C[i_3+i_0+2][i_0+2] * 2;
        }
    }
        ...
        for (int i_5 = max(i_4, 2); i_5 < PB_P; i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_Q-1; i_6++) {
                for (int i_7 = 0; i_7 < PB_M; i_7++) {
                    D[i_7][i_4+1][i_5][i_6] = J[i_7][i_4][i_5][i_6+i_5+1] + K[i_5-1][i_7][i_4][i_6-1] + K[i_7][i_5-1][i_6][i_4] * K[i_6][i_5-2][i_7][i_4] + L[i_6][i_4][i_7][i_5] + 6;
                }
            }
        }
    }
