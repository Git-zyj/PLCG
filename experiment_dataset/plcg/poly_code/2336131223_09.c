...
void kernel_2336131223_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            A[i_1] = D[i_0] + E[i_0-1][i_0] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_N; i_3++) {
            B[i_2][i_3] = D[i_3] - E[i_2][i_3] - E[i_2][1] * E[i_2][i_3-1] + B[i_2-2][i_3] * B[i_2+1][i_3] + 4;
        }
    }
    for (int i_4 = 2; i_4 < PB_N-2; i_4++) {
        C[i_4] = D[1] * D[i_4] * E[i_4][i_4] * C[i_4-2] + C[i_4+2] - 2;
    }
#pragma endscop
...
kernel_2336131223_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
