...
void kernel_4234151211_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
        ...
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_M; i_3++) {
                    A[i_2][i_1-1][i_1-1] = C[i_3-1][i_3+i_1][i_1][i_2+1] + C[i_1][i_2][i_0][i_3-1] * C[i_2][i_3+i_1-1][i_0][i_1+1] * C[i_3][i_1+1][i_2][i_0] - C[i_1-1][i_0+1][i_0+1][i_2-1] + 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_L-1); i_5++) {
            for (int i_6 = 2; i_6 < i_5; i_6++) {
                B[i_6][i_6-1][i_5] = D[i_6][i_5+i_4+1][i_4-1] - D[i_6+i_4-1][i_5][i_4] - D[i_6+1][i_4][i_5] * D[i_4+1][i_6][i_5] - B[i_6][i_6-1][i_5-1] + B[i_6-1][i_6-2][i_5-1] - B[i_6][i_6-1][i_4+1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4234151211_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
