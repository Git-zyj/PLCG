...
void kernel_3214211125_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_1+1][i_2] = A[i_1+1][i_2+2] - 4;
            }
        }
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = i_3; i_4 < PB_L; i_4++) {
                A[i_0-1][i_4] = C[i_4][i_3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3214211125_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
