...
void kernel_3322232215_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0+1] = D[i_0] - D[i_0] * E[i_0] - D[i_0-1] + A[i_0] * 3;
    }
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_Q; i_2++) {
            for (int i_3 = 0; i_3 < min(PB_L, -i_1+PB_P); i_3++) {
                C[i_1][i_3+i_1][0] = F[i_3][i_2][i_2+i_1] + G[i_3][i_1+1] * 5;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 0; i_6 < min(min(PB_Q, -i_5+PB_M), -i_4+PB_P-2); i_6++) {
                C[i_4][i_6+i_4][0] = H[i_6+i_5][i_4][i_5] * H[i_5][i_5+2][i_4] - C[i_4][i_6+i_4-1][0] * C[i_4+1][i_6+i_4+1][0] + C[i_4+1][i_6+i_4+2][0] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3322232215_06(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
