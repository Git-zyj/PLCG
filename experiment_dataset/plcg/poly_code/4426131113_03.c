...
void kernel_4426131113_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1-1] = E[i_1+1] + F[i_1][1] * 5;
    }
    for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                C[i_3][i_2+1][i_4] = G[i_4][i_3][i_2+1] - 1;
            }
        }
        for (int i_5 = 1; i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 0; i_6 < PB_P-2; i_6++) {
                for (int i_7 = 1; i_7 < min(PB_Q, -i_2+PB_L); i_7++) {
                    D[i_2][i_7+1][i_6+1][i_5-1] = G[i_7][i_5+1][i_6] - H[i_6][i_2-1][i_7][i_7-1] + H[i_6][i_7+i_2][i_5][0] + G[i_6][i_7-1][i_5+1] + D[i_2-1][i_7][i_6+2][i_5-1] + D[i_2+1][i_7+1][i_6+2][i_5-1] + D[i_2][i_7+1][i_6][i_5-1] * 2;
                }
            }
        }
    }
...
kernel_4426131113_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
