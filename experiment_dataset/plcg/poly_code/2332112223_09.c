...
void kernel_2332112223_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_1] = D[i_1][i_0-2] + A[i_1-1] * A[i_1-2] + 5;
        }
    }
    ...
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_M-1, i_3); i_4++) {
            C[i_3-2][i_4+1] = D[i_4+1][i_3] + B[i_4-1] - C[i_3-1][i_4+3] + C[i_3][i_4+1] * 4;
        }
    }
#pragma endscop
...
kernel_2332112223_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
