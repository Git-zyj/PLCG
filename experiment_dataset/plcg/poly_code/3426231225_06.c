...
void kernel_3426231225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - F[i_0] - E[i_0+1] - F[i_0] + 3;
        A[i_0+1] = E[i_0-1] * F[i_0] + A[i_0] - A[i_0-2] + 4;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
            A[i_2+2] = G[i_2][i_1] + 2;
            for (int i_3 = 1; i_3 < i_1; i_3++) {
                A[i_3] = H[i_1][i_3][i_2-1] + I[i_2+1][i_1][i_3] + I[i_3-1][i_1][i_2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3426231225_06(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
