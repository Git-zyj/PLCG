...
void kernel_2332112325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0][i_0] = D[i_0] * D[i_0] - A[i_0+1][i_0+1] * A[i_0+2][i_0+2] + A[i_0-1][i_0-1] * A[i_0-2][i_0-2] - 1;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        A[i_1-2][i_1-2] = D[i_1] - 1;
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            C[i_2][i_3] = C[i_2+2][i_3-1] - C[i_2-2][i_3-2] - 3;
        }
    }
#pragma endscop
...
kernel_2332112325_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
