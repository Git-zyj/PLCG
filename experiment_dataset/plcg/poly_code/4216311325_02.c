...
void kernel_4216311325_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        A[i_0-1] = C[i_0] - A[i_0-3] * A[i_0+1] * A[i_0-2] + 2;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_L; i_2++) {
            for (int i_3 = i_2; i_3 < min(-i_2+PB_L, -i_1+PB_M-2); i_3++) {
                B[i_3+i_2][i_2+i_1] = B[i_3+i_1-2][i_3+i_2-2] + B[i_3+i_1+2][i_3+i_2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4216311325_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
