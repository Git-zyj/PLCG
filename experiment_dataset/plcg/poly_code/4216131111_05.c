...
void kernel_4216131111_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_N; i_1++) {
            A[i_1+i_0][i_1] = C[i_1+i_0][0] - C[i_1][i_0+1] + C[i_1-1][i_0] - A[i_1+i_0-1][i_1] + A[i_1+i_0-2][i_1-1] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < PB_N; i_3++) {
            B[1] = C[i_3][i_2-1] - C[i_2][i_3+1] - 4;
        }
    }
#pragma endscop
...
kernel_4216131111_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
