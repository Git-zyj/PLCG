...
void kernel_2422252215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] - F[i_0+2] - F[0] * F[i_0+1] - F[i_0] + A[i_0-1] * A[i_0+1] + A[i_0+1] * 5;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        A[i_1-1] = F[i_1] - F[i_1-2] + E[i_1] + E[i_1] * F[i_1] - E[i_1] - E[i_1] + 5;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_L-1, -i_2+PB_M); i_3++) {
            C[i_2][i_3] = G[i_3+1][i_2] + H[i_3+i_2][i_2] + H[i_3][i_2] - C[i_2][i_3+1] + 6;
        }
        for (int i_4 = 0; i_4 < -i_2+PB_L; i_4++) {
            D[i_4+i_2-2] = G[i_2][i_4+2] + G[i_4][i_2-1] - I[i_4+1][i_4+i_2][0] * G[i_4+i_2][i_2+2] + 1;
        }
    }
#pragma endscop
...
kernel_2422252215_02(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
