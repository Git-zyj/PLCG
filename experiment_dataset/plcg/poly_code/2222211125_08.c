...
void kernel_2222211125_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_N; i_2++) {
            A[i_1-1][i_1-2] = C[i_2][i_1] * D[i_2-1] * A[i_1+1][i_1] - 5;
        }
    }
#pragma endscop
...
kernel_2222211125_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
