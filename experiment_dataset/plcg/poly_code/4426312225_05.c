...
void kernel_4426312225_05(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_Q-2; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_L; i_3++) {
                    D[i_1-1][i_3][i_2-2][i_0] = E[i_3][i_0-2][i_1][i_1] * F[i_3+i_1][i_0][i_0+2][i_2] * 5;
                }
                for (int i_4 = i_2; i_4 < PB_N-2; i_4++) {
                    C[i_4+2][i_2+1] = F[i_1][i_4+1][i_0][i_2+2] + 1;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_P; i_5++) {
        for (int i_6 = i_5; i_6 < PB_M-2; i_6++) {
            for (int i_7 = 0; i_7 < PB_M-3; i_7++) {
                C[i_6][i_7+1] = C[i_6][i_7+3] * C[i_6+1][i_7+1] + C[i_7][i_6+2] - C[i_6][i_7+2] + 1;
            }
        }
        for (int i_8 = 0; i_8 < min(i_5, PB_L-2); i_8++) {
            for (int i_9 = 0; i_9 < PB_M-1; i_9++) {
                for (int j_0 = 0; j_0 < -i_9+PB_M-1; j_0++) {
                    D[i_5-1][i_8][i_9][j_0] = G[j_0+i_9+1][i_8+2][0] + 2;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4426312225_05(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
