...
void kernel_3226312115_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        ...
        for (int i_3 = 0; i_3 < min(-i_0+PB_M, i_0); i_3++) {
            B[i_0][i_3+i_0+1] = C[i_0+1][i_3+i_0] * B[i_0-1][i_3+i_0+1] + 1;
        }
    }
#pragma endscop
...
kernel_3226312115_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
