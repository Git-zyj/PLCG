...
void kernel_2236111325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            A[0][i_0-1] = C[i_0-1][i_1] + A[0][i_0-2] - A[0][i_0-3] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        A[0][i_2-1] = A[0][i_2-1] - 3;
    }
#pragma endscop
...
kernel_2236111325_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
