...
void kernel_2214252313_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M-3, -i_0+PB_N); i_1++) {
            A[i_1+2] = C[i_1][i_0+2] - D[i_1][i_1] + D[i_1][i_0+2] + C[i_1][i_0+2] + D[i_1][i_0] + D[i_0][i_1-1] + C[i_1+i_0][i_0] - D[i_1][i_0] - A[i_1+1] * A[i_1+3] * B[i_0] - 3;
        }
        for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
            B[i_2] = C[i_2-1][i_0-2] + D[i_2][i_0] * A[i_2+2] - B[i_2+1] * 5;
        }
    }
#pragma endscop
...
kernel_2214252313_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
