...
void kernel_2236212315_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L+1; i_1++) {
            B[i_1+2] = C[i_0][i_1-1] + 5;
        }
    }
    for (int i_2 = PB_N-3 - 1; i_2 >= 0; i_2--) {
        B[i_2+2] = B[i_2+3] * B[i_2+1] - B[i_2+3] - 4;
    }
#pragma endscop
...
kernel_2236212315_07(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
