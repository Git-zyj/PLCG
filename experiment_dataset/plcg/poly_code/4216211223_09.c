...
void kernel_4216211223_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][i_1] = A[i_1][i_0+1] + A[i_0+1][i_1] - A[i_0][i_1+1] + 1;
        }
        for (int i_2 = max(1, i_0); i_2 < PB_L-3; i_2++) {
            for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
                B[i_2-1][i_3+1][i_0] = C[i_0][i_2+1][i_3] - B[i_3-2][i_2+3][i_0+2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4216211223_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
