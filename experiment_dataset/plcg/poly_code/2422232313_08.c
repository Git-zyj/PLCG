...
void kernel_2422232313_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0][i_0] = E[i_0] + F[i_0-2] + F[i_0+1] + A[i_0+1][i_0+1] * A[i_0-1][i_0-1] * 3;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        A[i_1][i_1] = F[i_1] - A[i_1-1][i_1-1] * 4;
        A[i_1+1][i_1+1] = G[i_1][0] * E[i_1+1] - F[i_1] * 4;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            D[i_2][i_3+2] = H[i_2][i_3] + H[i_3-1][i_3] + A[i_2][i_2] + D[i_3][i_2+3] - D[i_2][i_3+3] - 4;
        }
    }
#pragma endscop
...
kernel_2422232313_08(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
