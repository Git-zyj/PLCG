...
void kernel_2222311215_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
        for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
            B[i_2-1][i_1][i_1] = B[i_1-2][i_2][i_2] * 2;
        }
    }
#pragma endscop
...
kernel_2222311215_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
