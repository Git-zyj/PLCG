...
void kernel_4312352315_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0-1] * E[i_0] - E[i_0-1] + A[i_0-1] - A[i_0+1] - 3;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_Q; i_2++) {
            for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
                for (int i_4 = 2; i_4 < min(-i_3+PB_P, -i_1+PB_N-2); i_4++) {
                    B[i_3-1][i_1+1][i_4][i_2-1] = F[i_4+1][i_3][i_1][i_2+i_1] + G[i_4+2][i_2][i_3] - H[i_2+1][i_1][i_3+2][i_4-2] + I[i_3][i_2][i_4+i_3] - H[i_3-1][i_4+i_1+2][i_3+i_2][i_1] - B[i_3-1][i_4][i_1][i_2-1] * B[i_3-1][i_2+1][i_4-1][i_1-2] * B[i_2][i_4+2][i_3][i_1-1] - B[i_4][i_3+1][i_1][i_2-2] - 2;
                }
            }
        }
        for (int i_5 = 2; i_5 < PB_N-2; i_5++) {
            C[i_1][i_5] = J[i_1][i_5+2] * K[i_5][i_1] + J[i_5-2][i_1+1] + L[i_5+2][i_1] - L[i_5][i_1+2] + E[i_5+2] + J[i_5-1][i_1+1] - A[i_5+1] - C[i_1+1][i_5] * C[i_1-1][i_5] * 5;
        }
    }
#pragma endscop
...
kernel_4312352315_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
