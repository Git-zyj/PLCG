...
void kernel_3414352315_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        A[i_0-2] = E[i_0] - F[0] * A[i_0-1] * A[i_0-3] + 5;
        A[i_0-2] = G[i_0] - E[i_0] + G[i_0-1] - 1;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < min(PB_M, -i_1+PB_L+2); i_2++) {
            for (int i_3 = i_2; i_3 < PB_M; i_3++) {
                A[i_3-2] = H[i_2][i_3] * I[i_2][i_3-2][2] * H[i_2-1][i_3] * J[i_3][i_1-2][i_2] - K[i_3+i_2-2][i_2] * 4;
            }
            A[i_2-2] = L[i_2][i_1+2] - G[i_1] * H[i_2][i_1] + K[i_2+1][i_1] * K[i_2+i_1-2][i_1] - L[i_1][i_1] * K[i_2][0] - A[i_2-2] * 1;
        }
    }
#pragma endscop
...
kernel_3414352315_02(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
