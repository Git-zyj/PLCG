...
void kernel_4314332115_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0+1] * E[i_0] + 3;
    }
    for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_L, PB_L-1); i_2++) {
            for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                for (int i_4 = 0; i_4 < min(min(-i_1+PB_Q, -i_2+PB_Q-1), i_3); i_4++) {
                    B[i_3][i_3][i_4+i_2+1][i_1+1] = F[i_4][i_2][i_3+2] - G[i_3][i_4][i_2+i_1][i_2] * B[i_3+1][i_3+1][i_4+i_2+1][i_1+1] - B[i_3][i_3][i_4+i_1][i_2+1] + 6;
                }
            }
            for (int i_5 = 0; i_5 < min(PB_P-1, -i_1+PB_L); i_5++) {
                for (int i_6 = 0; i_6 < -i_2+PB_Q-1; i_6++) {
                    B[i_5+1][i_5+1][i_6+i_2+1][i_1+1] = F[i_5][i_2][i_6+1] + H[i_5][i_5+i_1][i_6+i_2-2][i_2+1] + G[i_6][i_5+i_1][i_2-2][i_2] * I[i_6][i_2][i_5] * 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4314332115_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
