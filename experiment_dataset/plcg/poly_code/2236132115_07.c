...
void kernel_2236132115_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(min(i_0, PB_M-2), -i_0+PB_L-1); i_1++) {
            A[i_1+1] = C[i_1+2][i_1+i_0+1] - C[i_1][i_0] * A[i_1] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L-2; i_3++) {
            A[i_3+1] = C[i_3][i_2] - C[i_2][i_3+2] * D[i_2+2] - C[i_3][i_2] - 1;
        }
    }
#pragma endscop
...
kernel_2236132115_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
