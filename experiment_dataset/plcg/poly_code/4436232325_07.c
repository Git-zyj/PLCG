...
void kernel_4436232325_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] - F[i_0] - F[i_0] * G[i_0][i_0] - A[i_0-2] - A[i_0+2] + A[i_0+1] * A[i_0-1] * 6;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M; i_2++) {
            for (int i_3 = i_1; i_3 < -i_2+PB_Q; i_3++) {
                for (int i_4 = 2; i_4 < min(-i_3+PB_L, -i_1+PB_Q); i_4++) {
                    C[i_2-1][i_3][i_4-2][i_1-2] = H[i_2][i_3][i_4+i_1][i_3+i_2-1] + I[i_4+i_3][i_1][i_1-2][i_3+i_2] + 1;
                }
            }
        }
    }
        ...
        for (int i_6 = 0; i_6 < i_5; i_6++) {
            for (int i_7 = 2; i_7 < PB_Q; i_7++) {
                for (int i_8 = 2; i_8 < PB_M; i_8++) {
                    C[i_6][i_8][i_7-2][i_5] = H[i_8][i_8][i_6][i_7] - H[i_5][i_8][i_6][i_7-1] - C[i_6][i_8-2][i_7-2][i_5+1] * 3;
                }
            }
        }
        for (int i_9 = 2; i_9 < i_5; i_9++) {
            for (int j_0 = 2; j_0 < PB_M; j_0++) {
                for (int j_1 = 0; j_1 < min(-i_5+PB_Q, -i_9+PB_P); j_1++) {
                    C[j_0-2][j_1][i_9-2][i_5+2] = I[j_0][j_1][i_9-1][i_5-1] + H[i_5-1][j_1+i_9][j_0][j_1+i_5] + H[j_0][i_5][j_1][i_5-1] - 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4436232325_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
