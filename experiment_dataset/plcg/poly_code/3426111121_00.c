...
void kernel_3426111121_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
        A[i_1+3] = E[i_1] - E[i_1] * 4;
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        C[i_2-1] = C[i_2] + 5;
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            D[i_2][i_3] = F[i_2-1][i_3+1][i_2] * G[i_2][i_3] * 1;
        }
    }
#pragma endscop
...
kernel_3426111121_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
