...
void kernel_2314112225_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = D[i_0-2][i_0+2] + A[i_0+1] + 2;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        A[i_1] = E[i_1] * 2;
        for (int i_2 = i_1; i_2 < PB_L-1; i_2++) {
            C[i_1][i_2] = D[i_2-1][i_2+1] - C[i_1-1][i_2] * C[i_1][i_2-1] * 6;
        }
    }
#pragma endscop
...
kernel_2314112225_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
