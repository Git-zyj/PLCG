...
void kernel_3234211313_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_2] = C[i_1+i_0-1][1] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_P; i_4++) {
            B[i_4] = C[i_4+i_3-1][i_4+i_3] + B[i_4-1] - B[i_4+1] * B[i_4-1] + 4;
        }
    }
#pragma endscop
...
kernel_3234211313_03(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
