...
void kernel_2232132321_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+1] = C[i_1][i_1+i_0] + D[i_1] + C[i_1-2][i_0] * C[i_1][i_0] - C[i_1+i_0][i_0] * A[i_1] * B[i_1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            B[i_3] = C[i_3+2][i_3] + A[i_3-1] - A[i_2+1] + A[i_3+3] - 3;
        }
    }
#pragma endscop
...
kernel_2232132321_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
