...
void kernel_2224112313_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-1; i_1++) {
            B[i_0][i_1+1] = C[i_1+i_0+1][0] + 6;
        }
        for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
            B[i_0][i_2+1] = D[i_0] + B[i_2-1][i_0+1] + B[i_0][i_2] - B[i_0-1][i_2+1] + 5;
        }
    }
#pragma endscop
...
kernel_2224112313_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
