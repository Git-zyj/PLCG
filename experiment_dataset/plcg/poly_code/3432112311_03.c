...
void kernel_3432112311_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            A[i_1] = E[i_1][i_0] * B[i_1-1][i_1-1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2][i_2] = C[i_2] * C[i_2+1] - 3;
        C[i_2] = B[i_2][i_2] * B[i_2-1][i_2-1] - B[i_2+1][i_2+1] * B[i_2][i_2] + 5;
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
                D[i_3][i_3] = A[i_4+1] * D[i_4-1][i_4-1] * D[i_3-1][i_3-1] * D[i_4+1][i_4+1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3432112311_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
