...
void kernel_2336112321_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L, i_0); i_1++) {
            B[i_1] = D[i_0+2][i_1] * D[i_0][i_1] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        B[i_2] = B[i_2-1] - 6;
        for (int i_3 = 2; i_3 < min(i_2, PB_L-1); i_3++) {
            C[i_3] = D[i_3-2][i_2] * B[i_2] + B[i_3+1] + B[i_2-2] + C[i_3-1] - 3;
        }
    }
#pragma endscop
...
kernel_2336112321_05(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
