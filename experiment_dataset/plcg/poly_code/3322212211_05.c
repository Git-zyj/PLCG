...
void kernel_3322212211_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0][i_1] = D[i_1][i_0] * A[i_1+i_0-1][i_1] - 4;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_2][i_0] = B[i_2-1][i_0] - B[i_2-1][i_0+1] * B[i_2][i_0+1] * 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        C[i_3-1] = E[i_3] - C[i_3] * 6;
    }
#pragma endscop
...
kernel_3322212211_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
