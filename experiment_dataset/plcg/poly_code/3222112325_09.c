...
void kernel_3222112325_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_M, PB_Q-1), -i_0+PB_L-3); i_2++) {
                A[i_2+i_0+1][i_1][i_2] = C[i_2+i_1][i_0+2][i_0] * A[i_2+i_0+3][i_1][i_2+1] + A[i_2+i_0+1][i_1+1][i_2] * 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_Q; i_3++) {
        for (int i_4 = max(3, i_3); i_4 < PB_Q; i_4++) {
            B[i_4-1][0] = D[i_4+2] * B[i_4+1][0] - B[i_4][0] - B[i_4-3][0] - 4;
        }
    }
#pragma endscop
...
kernel_3222112325_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
