...
void kernel_2326152311_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0+1] = D[i_0+2] * D[i_0] + D[i_0] - D[i_0] * D[i_0+1] - A[i_0] - 1;
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M-2, PB_L-1); i_1++) {
            B[i_0][i_1] = E[i_1][i_0] - E[i_1+2][i_1+i_0+2] * E[i_1][i_1] - E[i_0][i_1] * A[i_1+1] + B[i_0+1][i_1] * B[i_0-1][i_1+1] * B[i_0-1][i_1-1] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        A[i_2+2] = D[i_2] + D[i_2+1] + E[i_2][i_2] - D[i_2] + D[i_2-1] - D[i_2] + 5;
    }
#pragma endscop
...
kernel_2326152311_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
