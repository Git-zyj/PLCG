...
void kernel_4212312215_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_M-1; i_2++) {
            for (int i_3 = 3; i_3 < PB_M; i_3++) {
                B[i_3-2][i_1] = C[i_3][i_2+2] - B[i_3-1][i_2] * B[i_3-2][i_2+1] - B[i_3-3][i_2-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4212312215_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
