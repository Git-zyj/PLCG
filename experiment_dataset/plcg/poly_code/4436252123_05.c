...
void kernel_4436252123_05(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
                for (int i_3 = 1; i_3 < -i_0+PB_P; i_3++) {
                    D[i_3][0][i_1][i_2+2] = E[i_1][i_0][i_1][i_0+1] * F[i_3-1][i_3+i_0][i_1][i_2] + F[i_0-2][i_3][i_2][i_1] * G[i_3-1][i_0][i_2][i_1][i_2] - E[i_1][i_2][i_0][i_3] - 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P-2; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_P); i_5++) {
            for (int i_6 = 2; i_6 < PB_M-1; i_6++) {
                D[i_6-2][0][i_5+1][i_4+2] = H[i_6+1][i_5-1][0] * H[i_6][i_5+i_4][i_5+i_4+2] - I[i_6][i_5] - H[i_6][i_5][i_4] - 1;
            }
        }
        for (int i_7 = 0; i_7 < PB_M; i_7++) {
            for (int i_8 = 2; i_8 < -i_7+PB_M; i_8++) {
                for (int i_9 = 2; i_9 < min(-i_8+PB_M, -i_4+PB_L-1); i_9++) {
                    D[i_9+2][0][i_7][i_4+2] = E[i_9+2][i_8+i_7][i_9-2][i_4] - F[i_9+i_4+1][i_8][i_7][i_4-1] * E[i_8-2][i_4][i_9][i_7] + H[i_9+i_8][i_4][0] * F[i_8+1][i_9][i_4+2][i_7] - E[i_9-2][i_7][i_8-2][i_9+i_4] + 3;
                }
            }
        }
    }
    for (int j_0 = 0; j_0 < PB_M-1; j_0++) {
        for (int j_1 = 2; j_1 < PB_L-2; j_1++) {
            for (int j_2 = 0; j_2 < min(j_1, PB_P-2); j_2++) {
                for (int j_3 = 2; j_3 < PB_M; j_3++) {
                    D[j_3][0][j_1][j_2+2] = F[j_2][j_2][j_1][j_1-2] + E[j_3+2][j_2][j_1][j_0] - J[j_2][j_3-2][j_1] - F[j_3][j_2+2][j_1-2][j_0] - E[j_3][j_0+1][j_2][1] * D[j_3][0][j_1+2][j_2+2] + 2;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4436252123_05(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
