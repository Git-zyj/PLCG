...
void kernel_3114251321_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < min(-i_0+PB_L+1, PB_L-2); i_2++) {
                A[i_2][i_1-1][i_0] = B[i_2+1][i_1][i_0] + C[i_1][i_2+i_0-1][i_0] + B[i_2-1][i_0][i_1-1] * D[i_1][i_2][i_0-1][i_2] - E[i_2+i_0+1][i_1] - A[i_2-2][i_1-1][i_0+2] + A[i_2][i_1+1][i_0] - A[i_2+2][i_1-1][i_0] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3114251321_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
