...
void kernel_4216311221_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_P-2, i_0); i_1++) {
            ...
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                B[i_3-1][i_0][i_1] = C[i_3][i_0-1] * B[i_0-2][i_3][i_1-2] + B[i_0-2][i_3-1][i_1] * B[i_3-1][i_0+1][i_1] - B[i_3-1][i_0-1][i_1+2] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4216311221_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
