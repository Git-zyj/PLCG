...
void kernel_3422251125_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_1+1][i_2] = E[i_1-1][i_0][1] + E[i_1][i_2][i_1] + E[i_0][i_1][i_2] - F[i_1+1][i_0][i_2] - A[i_1+1][i_2+1] + 4;
            }
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 1; i_4 < -i_0+PB_M-1; i_4++) {
                C[i_4+1] = F[i_0-1][i_4-1][i_3+1] - E[i_4+1][i_4+i_0][i_3-1] + E[i_4+i_0+1][i_4][i_3-1] + E[i_0][i_4-1][i_3] * G[i_3][i_4] * F[i_4][i_3][i_0-1] - 3;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        C[i_5+1] = H[i_5] - I[i_5] + I[i_5-1] - H[i_5] - H[i_5] + 4;
        for (int i_6 = 0; i_6 < PB_N-1; i_6++) {
            C[i_6+2] = G[i_6+1][i_5] * G[i_6+1][1] * I[i_5] - J[i_6][0] * 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3422251125_01(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
