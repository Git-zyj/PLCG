...
void kernel_4216132315_01(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_P+1; i_1++) {
            for (int i_2 = 3; i_2 < min(i_0, -i_1+PB_P-1); i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_M-2; i_3++) {
                    B[i_0-2][i_2][i_3+i_2][0] = C[i_0][i_3+i_2+2][i_3-1] + B[i_2-3][i_0][i_1+i_0-1][0] + B[i_0-2][i_1+1][i_2+i_1+1][0] + 5;
                }
            }
        }
        for (int i_4 = i_0; i_4 < -i_0+PB_P+1; i_4++) {
            for (int i_5 = 2; i_5 < -i_4+PB_P; i_5++) {
                for (int i_6 = 0; i_6 < min(i_0, -i_4+PB_M+2); i_6++) {
                    B[i_0-2][i_4][i_5+i_4][0] = D[i_5][i_6][i_4][i_5] - D[i_6][i_5][i_0][i_4-2] - D[i_6+i_4-2][i_0+2][i_0+1][i_5+1] + D[i_6+1][i_5-2][i_4][i_0] + B[i_5-2][i_4][i_4+i_0-1][0] - 3;
                }
            }
        }
    }
...
kernel_4216132315_01(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D));
