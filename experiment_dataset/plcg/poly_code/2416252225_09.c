...
void kernel_2416252225_09(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        C[i_0-2][0] = E[i_0] - E[i_0-2] - E[i_0] - F[i_0] + E[i_0+2] - 2;
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            C[i_1-2][0] = G[i_1+2][i_0] * F[i_1] * H[i_1+1][i_0-1] * H[i_1][i_0] + H[i_0][i_1] - F[i_1] - 6;
        }
        for (int i_2 = 4; i_2 < PB_M+2; i_2++) {
            C[i_2-2][0] = C[i_2-4][0] + 6;
        }
    }
    for (int i_3 = 2; i_3 < PB_Q; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_P, PB_Q); i_4++) {
            C[i_4-1][0] = H[i_3][i_4+1] * H[i_3][i_4+i_3] - I[i_4-2][i_3+2][0] * G[i_4][i_3-1] * H[i_4][i_3] + H[i_3-2][i_3-2] + C[i_3-2][0] - 1;
        }
    }
#pragma endscop
...
kernel_2416252225_09(xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
