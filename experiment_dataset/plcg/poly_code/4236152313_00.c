...
void kernel_4236152313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+2; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_N+2); i_1++) {
            A[i_0] = C[i_1][i_1+i_0] - C[i_1+i_0-2][i_1+1] - C[i_1][i_0-2] * A[i_1-1] + 2;
        }
    }
    for (int i_2 = PB_N - 1; i_2 >= 1; i_2--) {
        B[i_2] = D[i_2+1] - D[0] - C[i_2][0] - D[i_2-1] + D[i_2-1] + D[i_2] * B[i_2+1] - B[i_2-1] - B[i_2-1] + B[i_2+1] + 5;
    }
#pragma endscop
...
kernel_4236152313_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
