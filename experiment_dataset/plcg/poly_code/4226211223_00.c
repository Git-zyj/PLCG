...
void kernel_4226211223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = C[0] + A[i_0-2] + A[i_0+1] + 6;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 3; i_2 < i_1; i_2++) {
            for (int i_3 = max(0, -i_2+2); i_3 < min(PB_M, -i_2+PB_L+2); i_3++) {
                for (int i_4 = 1; i_4 < -i_2+PB_L; i_4++) {
                    B[i_3][i_4-1][i_4+i_2] = B[i_4][i_2-3][i_3+i_2-2] - 5;
                }
            }
        }
    }
...
kernel_4226211223_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
