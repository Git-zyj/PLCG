...
void kernel_3314112321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_L, i_0); i_1++) {
            A[i_0][i_1] = A[i_0-2][i_1-2] + A[i_0+1][i_1-1] + 2;
        }
        C[i_0-1] = D[i_0+1] - 4;
    }
    for (int i_2 = 3; i_2 < PB_M-1; i_2++) {
        C[i_2-1] = C[i_2+1] * C[i_2-3] + C[i_2-2] - C[i_2] + 5;
    }
#pragma endscop
...
kernel_3314112321_00(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
