...
void kernel_4332151321_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_Q-3; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_L; i_3++) {
                    A[i_0][i_1][i_2][i_2+1] = D[i_3-1][i_2][i_1][i_0-1] * D[i_3+1][i_2][i_0][i_0] * D[i_1-1][i_0][i_2-1][1] * D[i_0+1][i_2+1][i_3-1][i_1] + E[i_2][i_3+i_1][i_1] + E[i_2][i_3-1][0] * A[i_0+1][i_2][i_1][i_1+1] * A[i_0][i_2+2][i_1+2][i_1+3] + A[i_0+2][i_1-1][i_3-1][i_3] * 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 2; i_5 < PB_L; i_5++) {
            B[i_5] = F[i_5-1][i_4] - F[i_5-1][i_4+1] * F[i_4][i_5] - E[i_5][i_4][i_5] + F[i_5][i_5] * B[i_5-2] + 4;
        }
    }
    for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
        for (int i_7 = i_6; i_7 < PB_Q-3; i_7++) {
            for (int i_8 = 2; i_8 < PB_Q-1; i_8++) {
                A[i_8+1][i_6-2][i_7+2][i_7+3] = E[i_8][i_6+1][i_7] * E[i_8-1][i_6][i_7+1] * A[i_7-2][i_6][i_8][i_8+1] + A[i_6+2][i_8-2][i_7][i_7+1] * 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4332151321_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F)));
