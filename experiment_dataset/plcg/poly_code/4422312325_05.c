...
void kernel_4422312325_05(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_N-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
            B[i_1][i_2] = B[i_1-1][i_2-1] + B[i_1][i_2+1] * B[i_1+2][i_2+2] * 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        C[i_3] = E[i_3-2] - C[i_3+1] * 1;
        for (int i_4 = 3; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
                for (int i_6 = i_4; i_6 < PB_L; i_6++) {
                    D[i_5][0][i_6-2][i_6] = F[i_6][i_4][0] + G[i_5+2][i_3][i_6-1][i_4-2] * D[i_3-1][0][i_4-1][i_4+1] * D[i_5][0][i_4-3][i_4-1] * D[i_5][0][i_4-1][i_4+1] * 2;
                }
            }
        }
    }
...
init_array(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4422312325_05(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
