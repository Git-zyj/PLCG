...
void kernel_3212311111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_N-1; i_2++) {
            B[i_2] = C[i_2-1][i_1] * D[i_2][i_1] * B[i_2+1] * 6;
        }
    }
#pragma endscop
...
kernel_3212311111_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
