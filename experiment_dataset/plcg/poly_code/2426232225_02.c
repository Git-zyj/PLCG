...
void kernel_2426232225_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            B[i_2] = G[i_1+2][i_2-2] * G[i_2-2][i_2] - H[i_1][i_1] - B[i_2-1] + B[i_2-2] + 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_N; i_3++) {
        C[i_3] = F[i_3-1] + G[i_3+1][i_3-1] * F[i_3-1] * F[i_3] + E[i_3-2] - C[i_3-2] - 3;
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            D[i_3][i_4] = B[i_4] - B[i_4+1] + 5;
        }
    }
#pragma endscop
...
kernel_2426232225_02(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
