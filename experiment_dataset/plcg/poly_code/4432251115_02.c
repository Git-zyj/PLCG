...
void kernel_4432251115_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0][i_0+1] = E[i_0] + F[i_0] + 1;
        A[i_0][i_0+1] = F[i_0+1] * E[i_0+1] + F[i_0+1] * F[1] * F[i_0] + E[i_0] * F[i_0] - E[i_0-1] - E[i_0] + A[i_0-1][i_0] - 3;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 1; i_3 < PB_N; i_3++) {
                for (int i_4 = 0; i_4 < min(min(PB_N-1, -i_3+PB_M), -i_2+PB_L); i_4++) {
                    C[i_4][i_2][i_3-1][i_3] = G[i_4+i_3][i_4][i_2] + H[i_3][i_1][i_4+i_2][i_1] + I[i_4+i_3-1][i_3][i_2][i_1] - I[i_4+1][i_1][i_2-1][i_3] + 4;
                }
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
        A[i_5+1][i_5+2] = E[i_5+1] * F[i_5+1] * A[i_5][i_5+1] + 1;
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4432251115_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
