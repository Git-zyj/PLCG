...
void kernel_4312332213_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = D[i_0][i_0] + D[i_0][i_0] - E[i_0] + A[i_0-1] + A[i_0+1] - 1;
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                B[i_0][i_2-2][i_0] = F[i_0-2][i_2][i_1] + G[i_2][i_0][i_1] * 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-3; i_3++) {
        C[i_3+2] = H[i_3][i_3-1] + E[i_3] * C[i_3+3] + C[i_3+1] + 1;
    }
#pragma endscop
...
kernel_4312332213_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
