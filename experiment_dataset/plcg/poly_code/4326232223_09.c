...
void kernel_4326232223_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
        ...
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
                for (int i_3 = i_0; i_3 < PB_L; i_3++) {
                    A[2][i_1][i_3][i_0-1] = D[i_0][i_2+2][i_1][i_3] * D[i_3][i_2][i_0][i_1-2] * A[2][i_1+2][i_0][i_3-1] * A[2][i_1][i_2][i_0-3] + A[2][i_0][i_3][i_2-2] + 5;
                }
            }
        }
        for (int i_4 = 0; i_4 < min(PB_N-1, i_0); i_4++) {
            B[i_4+1] = E[i_4] - F[i_0][i_4] - 3;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
        B[i_5] = E[i_5] - E[i_5+1] - E[i_5-1] + E[i_5] - 1;
    }
#pragma endscop
...
kernel_4326232223_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
