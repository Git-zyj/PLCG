...
void kernel_3332312123_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                A[i_1-2][i_2][i_2] = D[i_2-1][i_0][i_1+i_0] + A[i_1][i_2][i_2] - 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-3; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M-3; i_4++) {
            for (int i_5 = 2; i_5 < -i_3+PB_L; i_5++) {
                B[i_4+i_3+1][i_5+i_3][i_5-1] = D[i_4][i_3][i_5-2] - B[i_4+i_3+3][i_5+i_3-1][i_5-2] + 3;
            }
        }
        for (int i_6 = 0; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < -i_3+PB_M; i_7++) {
                C[i_7+i_3][i_6][i_6+2] = 3;
            }
        }
    }
#pragma endscop
...
kernel_3332312123_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
