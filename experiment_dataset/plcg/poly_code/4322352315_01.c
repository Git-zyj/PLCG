...
void kernel_4322352315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_L-2); i_1++) {
            A[i_1+2] = D[i_0][i_1+2][0] + E[i_0][i_0] + F[i_1][i_1+i_0] * E[i_1][i_0+1] * G[i_1][i_0] - F[i_1][i_0] * A[i_1+3] - A[i_1+1] - A[i_1+3] * 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            A[i_3+1] = F[i_2][i_3] * F[i_3+2][i_2] + G[i_2][2] + F[i_2][i_3] - 1;
            A[i_3+2] = F[i_2+1][i_3] + E[i_3-1][2] + H[i_3] - I[i_3][i_2][i_3+1] * G[i_2][i_3] - 2;
        }
    }
#pragma endscop
...
kernel_4322352315_01(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
