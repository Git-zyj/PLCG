...
void kernel_3324111311_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < PB_N; i_2++) {
                A[i_2-1][i_1-1][i_0-1] = D[i_0][i_1] + A[i_2-2][i_1-1][i_0-1] - A[i_2][i_1-1][i_0-1] * 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
                B[i_3][i_4-1][i_5] = B[i_3][i_4-2][i_5] - B[i_3][i_4][i_5] + B[i_3+1][i_4-1][i_5+1] + B[i_3][i_4][i_5-1] + 6;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M+1; i_6++) {
        for (int i_7 = 1; i_7 < min(PB_M+1, -i_6+PB_N-1); i_7++) {
            for (int i_8 = 1; i_8 < min(-i_6+PB_N, -i_7+PB_M+1); i_8++) {
                C[i_7+i_6+1][i_8+i_7-1][i_8] = A[i_8-1][i_7-1][i_6-1] + C[i_8+i_6][i_8+i_7-1][i_7] - C[i_7+i_6+1][i_8+i_7-1][i_8-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3324111311_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
