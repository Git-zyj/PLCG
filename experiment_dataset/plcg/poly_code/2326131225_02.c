...
void kernel_2326131225_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        C[i_0] = D[i_0] + D[i_0-1] * 6;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            C[i_1-1] = C[i_1-1] * 6;
        }
        C[i_1] = D[i_1-1] * D[i_1] * D[i_1+1] * D[i_1] - D[i_1] + D[i_1+1] - C[i_1+2] - 4;
    }
#pragma endscop
...
kernel_2326131225_02(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
