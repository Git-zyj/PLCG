...
void kernel_2412332323_01(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        D[i_0-1] = E[i_0] * F[i_0+2] + 1;
        for (int i_1 = 3; i_1 < -i_0+PB_M+1; i_1++) {
            B[i_0][i_1-1] = G[i_0][i_1] + H[i_1+i_0-1][1] + F[0] * I[i_1] + B[i_0][i_1-3] - B[i_0-2][i_1-1] + B[i_0+1][i_1-1] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M-1; i_3++) {
            D[i_2] = H[i_3+i_2+1][i_3-1] - G[i_3+1][i_2] * D[i_2+1] + D[i_3] - 1;
        }
        D[i_2] = F[i_2] * I[i_2+2] + I[i_2] + F[i_2] - D[i_2+1] * 1;
    }
#pragma endscop
...
kernel_2412332323_01(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
