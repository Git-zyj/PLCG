...
void kernel_3316251325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        A[i_0+1] = D[i_0] - E[i_0-1] * F[i_0][i_0] * D[0] * E[i_0] + A[i_0+3] - 1;
        for (int i_1 = 3; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < i_0; i_2++) {
                B[i_1-1][i_2+1][i_0] = G[i_2][i_0+1] * H[i_1][i_0][i_2] * F[i_0][1] + I[i_2][i_0][i_1] + B[i_1-1][i_2+1][i_0+2] + B[i_1-3][i_2+1][i_0] - B[i_2-2][i_1+1][i_0] * B[i_1-1][i_2+1][i_0-1] * 5;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_P+1, i_3); i_4++) {
            for (int i_5 = 1; i_5 < PB_P; i_5++) {
                B[i_3-3][i_5+2][i_4] = I[i_3][i_5][i_4+i_3-1] + G[i_5-1][i_4+i_3] * I[i_4][i_3][0] + F[i_5-1][i_4] * 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3316251325_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
