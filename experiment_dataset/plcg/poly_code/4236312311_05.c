...
void kernel_4236312311_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            A[i_0][i_1] = C[i_1+2][i_0] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-3; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 0; i_4 < -i_2+PB_M-3; i_4++) {
                B[i_3+1][i_4+i_2+1][i_2] = A[i_3][i_2] + A[i_3][i_4] - A[i_3+1][i_4] - B[i_3+2][i_4+i_2+1][i_2] * B[i_3+1][i_4+i_2+3][i_2+1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4236312311_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
