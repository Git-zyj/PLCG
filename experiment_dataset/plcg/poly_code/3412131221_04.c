...
void kernel_3412131221_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        A[i_0-1] = E[i_0] * E[i_0] - E[i_0-1] * E[i_0-1] - B[i_0] + 2;
        B[i_0] = F[i_0][i_0-1] - B[i_0-1] + D[i_0+1] + D[i_0-1] * 4;
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                C[i_1][i_1][i_2] = G[i_2][i_2][1] - G[i_2-1][i_1][i_0-1] + G[i_1][i_2-1][i_0-1] - G[i_1][i_0][i_2] + G[i_1+1][i_2][i_0+1] + 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-3; i_3++) {
        D[i_3+1] = E[i_3-1] - E[i_3] + A[i_3-1] + D[i_3+2] + D[i_3] + D[i_3+3] * 4;
    }
#pragma endscop
...
kernel_3412131221_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
