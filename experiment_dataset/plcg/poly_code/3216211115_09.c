...
void kernel_3216211115_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_P+1, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_1-1][i_2][i_1+i_0-1] = 4;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_P; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_0+PB_P, i_3); i_4++) {
                B[i_0][i_3][i_4+i_0] = C[i_4+1][i_3] + B[i_0][i_4-1][i_3+i_0] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3216211115_09(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
