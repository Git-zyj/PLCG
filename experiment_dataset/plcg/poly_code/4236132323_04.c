...
void kernel_4236132323_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_N-2); i_1++) {
            B[i_1-1][i_0+2] = C[i_1+i_0+2][i_1-2] * C[i_1][i_0-1] + C[i_1][i_1+i_0] + C[i_1-1][i_0] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < PB_N; i_3++) {
            B[i_3][i_2] = C[i_2][i_3] * C[i_3][i_2] * B[i_3-2][i_2] + B[i_3][i_2-2] - B[i_3][i_2+2] * 2;
        }
    }
#pragma endscop
...
kernel_4236132323_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
