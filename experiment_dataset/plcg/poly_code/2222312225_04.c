...
void kernel_2222312225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-2; i_0++) {
        ...
        for (int i_2 = 0; i_2 < min(-i_0+PB_M-4, PB_L-2); i_2++) {
            B[i_2+i_0][i_0] = C[i_0][i_2+2] + B[i_2+i_0-3][i_0-1] * B[i_2+i_0+4][i_0+2] - 3;
        }
    }
#pragma endscop
...
kernel_2222312225_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
