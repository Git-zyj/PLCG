...
void kernel_2324252125_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        C[i_0-1] = D[i_0] + E[i_0+2] * E[i_0] - 4;
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            C[i_1] = F[i_1][i_0+2] * G[i_0+2][i_0] + F[i_0][i_1] - G[i_1][i_0+1] - H[i_1-2][i_0][i_0+1] * 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        C[i_2] = E[i_2] + D[i_2-2] + D[i_2] - D[i_2] + D[i_2] + C[i_2+1] - 4;
    }
#pragma endscop
...
kernel_2324252125_02(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
