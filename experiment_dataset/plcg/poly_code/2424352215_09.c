...
void kernel_2424352215_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_N, PB_M-2); i_1++) {
            A[i_1-2][i_1] = E[i_1+i_0][i_1] * F[i_1+2] * G[i_0-1][i_1] + A[i_0-3][i_0-1] - 2;
        }
        for (int i_2 = 2; i_2 < min(PB_M-3, -i_0+PB_L); i_2++) {
            D[i_2+3] = G[i_2-1][i_0-2] * G[i_2+i_0][0] - H[i_0][i_2-2] - H[i_0][1] - H[i_0][i_2] - E[i_2][i_0] * 4;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 3; i_4 < min(-i_3+PB_M, PB_M-1); i_4++) {
            C[i_4+1][i_3] = I[i_4+i_3][i_4+i_3+2][0] + J[i_4] + G[i_3][2] + H[i_4-2][i_3] + E[i_4][0] - A[i_4-3][i_4-1] - C[i_4][i_3] - C[i_4+1][i_3+1] + 3;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
        D[i_5+2] = K[i_5] + K[i_5] - K[i_5] + F[i_5] - A[i_5-1][i_5+1] + 5;
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_2424352215_09(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K)));
