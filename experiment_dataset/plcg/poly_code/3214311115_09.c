...
void kernel_3214311115_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                B[i_2+1][i_1] = C[i_1][i_0][i_1] * D[i_1][i_2-1][i_2] + 3;
            }
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                B[i_1][i_3] = B[i_1][i_3-1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3214311115_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
