...
void kernel_3224211125_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        ...
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 0; i_3 < min(PB_M-3, -i_0+PB_L); i_3++) {
                B[i_0][i_2+1][i_0+1] = C[i_2][i_3+i_0][i_0] - B[i_2][i_3+3][i_2+1] + B[i_0][i_2-1][i_0+1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3224211125_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
