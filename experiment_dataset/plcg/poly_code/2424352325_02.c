...
void kernel_2424352325_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[0] - E[i_0-2] + F[i_0] + F[i_0] * A[i_0-1] * A[i_0+2] + 5;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            D[i_1+i_0][i_1-1] = E[i_1] * G[i_1+1][i_0+2] - A[i_1+1] * 2;
        }
        A[i_0] = H[i_0][0] + I[i_0] + E[i_0] - E[i_0-1] * I[i_0] + F[i_0+1] + F[i_0-2] * 5;
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < -i_2+PB_M; i_3++) {
            D[i_3+i_2][i_2-1] = G[i_3+i_2][i_3] * F[i_2] - I[i_3] - G[i_3+1][i_2] - H[i_3][i_2-2] + H[i_2][i_3+1] * D[i_3+i_2-2][i_3-3] - D[i_3+i_2-2][i_2-1] * 4;
        }
    }
#pragma endscop
...
kernel_2424352325_02(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
