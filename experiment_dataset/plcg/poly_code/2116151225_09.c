...
void kernel_2116151225_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, -i_0+PB_N-1); i_1++) {
            A[0][i_1] = B[i_1] - C[i_1+i_0+1][i_0] * B[i_1-1] - C[i_1][i_0] + B[i_0] - A[0][i_1+1] * A[0][i_1+2] - 4;
        }
    }
#pragma endscop
...
kernel_2116151225_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
