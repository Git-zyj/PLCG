...
void kernel_2336212215_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
            C[i_0][i_2-2] = C[i_0-1][i_2-2] - C[i_0][i_2-1] - 6;
        }
    }
#pragma endscop
...
kernel_2336212215_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
