...
void kernel_3322212211_04(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        B[i_3+1] = B[i_3] - B[i_3+2] + B[i_3+2] - 5;
        for (int i_4 = 0; i_4 < PB_P-2; i_4++) {
            for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
                C[i_5+2][i_4] = D[i_4][i_3][i_5+1] + E[i_3-1][i_4] * D[i_5][i_3+2][i_4+2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3322212211_04(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
