...
void kernel_3326211323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        A[i_0+1] = D[i_0] * A[i_0] + A[i_0+3] - A[i_0+2] - A[i_0-1] + A[i_0] + 4;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 0; i_3 < min(PB_M-3, i_1); i_3++) {
                B[i_2][i_3+1] = B[i_2][i_3+3] - B[i_2+1][i_3+1] - C[0] * 2;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < min(PB_L, -i_4+PB_M); i_5++) {
            C[0] = E[i_5+i_4][0] + A[i_5+1] * 5;
        }
    }
#pragma endscop
...
kernel_3326211323_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
