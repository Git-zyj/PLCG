...
void kernel_4216212115_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_L, i_0), PB_L-2); i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                B[i_2][i_1+1][i_1+i_0] = C[i_0+1][i_1+2][0] - C[i_2+1][i_1][i_0] - 5;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = max(2, i_3); i_5 < PB_L-1; i_5++) {
                for (int i_6 = 0; i_6 < min(-i_5+PB_L, -i_4+PB_L); i_6++) {
                    B[i_4-1][i_5+1][i_6+i_5] = B[i_5-2][i_4+1][i_6+i_4] * 6;
                }
            }
        }
    }
...
kernel_4216212115_09(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
