...
void kernel_2232132115_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            B[i_0][i_1] = C[i_1][i_0] + C[0][i_1+i_0] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L-2; i_3++) {
            B[i_2][i_3] = C[i_2+2][i_3+2] * C[i_2][i_3] - C[2][i_2] + C[i_2-1][i_3] + B[i_2-1][i_3] + 4;
        }
    }
#pragma endscop
...
kernel_2232132115_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
