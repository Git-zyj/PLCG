...
void kernel_2216112315_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L+2; i_1++) {
            B[i_0-1][i_1-2] = C[i_0][i_0] - 1;
        }
        for (int i_2 = max(3, i_0); i_2 < PB_L+2; i_2++) {
            B[i_0][i_2-2] = B[i_0-1][i_2-3] + B[i_0-1][i_2-2] * B[i_0+1][i_2-3] * 4;
        }
    }
#pragma endscop
...
kernel_2216112315_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
