...
void kernel_3232132123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        B[i_2-1] = C[i_2] - D[i_2+2][i_2] * D[i_2][1] - C[i_2-2] - C[i_2] * C[i_2+1] + B[i_2+1] - 5;
    }
#pragma endscop
...
kernel_3232132123_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
