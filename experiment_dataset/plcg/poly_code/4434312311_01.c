...
void kernel_4434312311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-2; i_0++) {
        A[i_0-1] = E[i_0] * A[i_0] * A[i_0-2] + A[i_0] + C[i_0][i_0] - 3;
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-2, i_0); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                B[i_1+i_0][i_2][i_0-2] = F[i_1][i_2+i_0][i_1] + B[i_1+i_0-1][i_2][i_0-3] + B[i_1+i_0][i_2+1][i_0-2] + B[i_1+i_0+2][i_2][i_0-1] * 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_P; i_4++) {
            C[i_3][i_3] = G[i_3-2][i_4-1] - H[i_4+2][i_4+i_3] * A[i_4-1] * C[i_3-1][i_3-1] + 6;
        }
    }
        ...
        for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
            for (int i_7 = 0; i_7 < PB_P; i_7++) {
                for (int i_8 = 0; i_8 < -i_5+PB_L; i_8++) {
                    D[i_7][i_8+i_5][i_6][i_7] = D[i_7][i_8+i_5][i_6+1][i_7] + 6;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434312311_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
