...
void kernel_4422352115_06(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                D[i_1+1][i_2][i_0+1][i_2] = E[i_1+2][i_2][i_0-2] * F[i_2][i_0][i_1+2] - G[i_2][i_0][i_1] - F[i_1+1][i_2][i_2-2] * F[i_2][i_0-1][i_1] + G[i_2][i_0+1][i_1] + E[i_1][i_2-2][i_0] + 4;
            }
        }
        for (int i_3 = 1; i_3 < -i_0+PB_P; i_3++) {
            for (int i_4 = 0; i_4 < PB_P-2; i_4++) {
                for (int i_5 = 0; i_5 < -i_0+PB_L-1; i_5++) {
                    D[i_0+1][i_5][i_3+1][i_5] = E[i_4+i_3][i_5+i_0+1][i_4] - H[i_4][i_5][i_3][i_0+2] - I[i_4][i_0][i_3+i_0][i_5] - I[i_5][i_3][i_4+2][i_0-2] - G[i_5][i_4][i_3-1] * 2;
                }
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
        for (int i_7 = 2; i_7 < PB_L; i_7++) {
            for (int i_8 = 0; i_8 < -i_7+PB_M; i_8++) {
                D[i_8][i_7][i_6][i_7] = F[i_8+i_7][i_6][i_6-2] - E[i_7][i_6+1][i_8+1] * F[i_7-1][i_8][i_6] + G[i_7][i_6][i_7] - J[i_8][i_7-1] - G[i_7-2][i_6-2][i_8] * 4;
            }
        }
        for (int i_9 = 1; i_9 < PB_L+1; i_9++) {
            for (int j_0 = 0; j_0 < PB_P-1; j_0++) {
                for (int j_1 = 2; j_1 < i_9; j_1++) {
                    D[i_6][j_1][j_0+1][j_1] = G[j_1-2][i_6][i_9-1] - D[i_6][j_0][j_1][j_0] * 1;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4422352115_06(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
