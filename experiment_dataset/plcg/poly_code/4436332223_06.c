...
void kernel_4436332223_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
        ...
        for (int i_1 = 0; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < min(PB_P-1, -i_0+PB_L-1); i_2++) {
                for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
                    A[i_0+2][i_1+2][i_2][i_3] = E[i_3][i_2+i_0+1][i_0-1][i_1+2] + A[i_0+4][i_1][i_2+1][i_3+2] - A[i_1][i_0+2][i_2][i_3] * A[i_0+2][i_1+3][i_2-2][i_3+1] - 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 2; i_6 < PB_L; i_6++) {
                B[i_4+2][i_6][i_6-2] = F[i_4][i_5][i_4] + F[i_6-1][i_5][i_4] * G[i_5][i_4] * H[i_4][i_6-1][i_5][i_5] * 1;
            }
        }
        for (int i_7 = i_4; i_7 < PB_M-1; i_7++) {
            for (int i_8 = i_7; i_8 < -i_4+PB_M; i_8++) {
                B[i_4+2][i_8][i_8-2] = I[i_7][i_8-2][i_4-2] * I[i_7][i_8+i_4][i_7] * J[i_7+1][i_8-1][i_4] - K[i_7][0] * 1;
            }
        }
        for (int i_9 = 2; i_9 < min(i_4, -i_4+PB_M); i_9++) {
            for (int j_0 = 2; j_0 < PB_L-2; j_0++) {
                for (int j_1 = 1; j_1 < -i_4+PB_M; j_1++) {
                    D[j_1+i_4][j_0][j_0][i_9] = H[j_1-1][j_0+1][i_4][i_9] - I[i_9-2][i_9][j_1+1] + E[j_1+i_4][j_0+2][i_9-1][j_1] * D[j_1+i_4][j_0-2][j_0-2][i_9-1] - D[i_9+i_4][j_0][j_0][j_1-1] * D[j_1+i_4-2][j_0][j_0][i_9] * 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4436332223_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
