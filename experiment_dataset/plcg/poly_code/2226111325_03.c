...
void kernel_2226111325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = A[i_0-2] + A[i_0] - A[i_0-3] * A[i_0+1] + 1;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            B[i_2+i_1+1][i_2+i_1][0] = C[i_1][0] * B[i_2+i_1][i_2+i_1-1][0] - B[i_2+i_1-1][i_2+i_1-2][0] + 4;
        }
    }
#pragma endscop
...
kernel_2226111325_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
