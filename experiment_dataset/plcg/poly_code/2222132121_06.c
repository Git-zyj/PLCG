...
void kernel_2222132121_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P-1; i_0++) {
        A[i_0-1] = C[i_0-1] + D[i_0][i_0-1] - C[i_0+1] - A[i_0-3] * 5;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_Q); i_1++) {
            B[i_1][i_0-2] = E[i_1-1][i_0+1][i_1] + D[i_1+i_0][i_0] + 4;
        }
    }
#pragma endscop
...
kernel_2222132121_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
