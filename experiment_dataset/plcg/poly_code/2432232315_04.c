...
void kernel_2432232315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            D[i_2] = G[i_2-1][0] * G[i_2][i_1+1] * 6;
            D[i_2+1] = H[i_2][i_1] + H[i_1-1][i_2] - 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_M; i_4++) {
            D[i_4] = G[i_3][i_3+2] + I[i_3][i_4-2][i_3-2] + G[i_4+i_3][i_3-1] - G[i_4][i_3-1] * E[i_3+2] * A[i_4] - D[i_4+1] + D[i_4-1] - 5;
        }
    }
#pragma endscop
...
kernel_2432232315_04(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
