...
void kernel_2422312325_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            A[i_4-1] = F[i_4][i_3-2] + 1;
        }
        for (int i_5 = 0; i_5 < PB_P; i_5++) {
            A[i_5+2] = G[i_3-2][i_5] - 5;
        }
    }
#pragma endscop
...
kernel_2422312325_01(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
