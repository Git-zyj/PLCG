...
void kernel_2336351321_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        A[i_0-1] = D[i_0] * E[i_0] - F[i_0] - E[0] * A[i_0-3] * A[i_0-2] - 4;
    }
    for (int i_1 = 1; i_1 < PB_M-3; i_1++) {
        for (int i_2 = max(3, i_1); i_2 < PB_M; i_2++) {
            B[i_1+1][i_2-1] = G[i_2][i_1] * G[i_2][i_1] * G[i_1][0] - A[i_2-3] * B[i_1-1][i_2-1] - B[i_1+3][i_2-1] * B[i_1-1][i_2+1] * 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_N-3; i_4++) {
            C[i_4+1] = G[i_4][i_3+1] - H[1][i_3] + I[i_4][i_3-1][0] * G[i_4][i_3] * G[i_4][i_3+1] * G[i_4][i_3] - A[i_4-1] + A[i_4] * C[i_4+3] + 1;
        }
    }
#pragma endscop
...
kernel_2336351321_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
