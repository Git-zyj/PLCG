...
void kernel_4216111123_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < PB_P; i_5++) {
            B[i_4][i_5] = C[i_5][i_4] + B[i_4-2][i_5] * B[i_4-1][i_5] - 4;
        }
    }
#pragma endscop
...
kernel_4216111123_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
