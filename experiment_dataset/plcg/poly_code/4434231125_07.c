...
void kernel_4434231125_07(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xb,int yb,int zb,int vb,int wb,DATA_TYPE POLYBENCH_5D(B,xB,yB,zB,vB,wB,xb,yb,zb,vb,wb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        D[i_0][0] = E[i_0] + E[i_0] - F[i_0-1] - F[i_0] - 1;
    }
    for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_L, PB_L-1); i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_L, PB_M-1); i_3++) {
                for (int i_4 = i_2; i_4 < min(-i_1+PB_N-1, -i_3+PB_L-1); i_4++) {
                    B[i_2][i_4+1][i_3+i_1][i_2][i_1] = G[i_4+i_1+1][i_3+1][i_2+i_1][i_4+1] + B[i_1][i_2+1][i_4+i_3+1][i_1][i_4+1] - 4;
                }
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 0; i_6 < -i_5+PB_N; i_6++) {
            D[i_5+1][0] = H[i_6+i_5][i_6] * E[i_5] - 1;
        }
        for (int i_7 = 2; i_7 < -i_5+PB_M; i_7++) {
            D[i_7][0] = E[i_7-1] * I[i_7+i_5][0] + D[i_7-2][0] * 1;
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,wb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4434231125_07(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,wb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,wb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
