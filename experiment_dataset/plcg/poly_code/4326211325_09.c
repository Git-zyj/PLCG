...
void kernel_4326211325_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            ...
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                B[i_1][i_0][i_0-1] = D[i_1+i_0][i_2][i_2-1] * A[i_0+1][i_2] + A[i_1+1][i_0] + A[i_0][i_2] - A[i_2][i_1] + A[i_2+1][i_1] + A[i_2+2][i_1] * 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < i_3; i_5++) {
                C[i_3-1][i_5][i_4] = C[i_3+1][i_5-1][i_4+1] - C[i_3][i_5-1][i_4] * C[i_4-1][i_3][i_5+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4326211325_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
