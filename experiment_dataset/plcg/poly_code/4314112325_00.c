...
void kernel_4314112325_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-2; i_1++) {
            A[i_1] = A[i_1+2] + A[i_1+1] + A[i_1-1] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-3; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_M, -i_2+PB_L-3); i_3++) {
            A[i_3] = D[i_3][i_2] - C[i_3+i_2+3][i_3-1] + 6;
        }
        for (int i_4 = 2; i_4 < -i_2+PB_L-1; i_4++) {
            C[i_4+i_2+1][i_4-1] = E[i_4-1] * C[i_4+i_2][i_4-2] - 5;
        }
    }
#pragma endscop
...
kernel_4314112325_00(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
