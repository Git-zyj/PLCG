...
void kernel_3222312125_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L, i_0); i_1++) {
            A[i_0][i_1] = A[i_0-1][i_1] * A[i_0-2][i_1] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_N; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_P+2, PB_L); i_4++) {
                B[i_3][i_2][i_4+i_3-2] = C[i_4][i_2][i_4] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3222312125_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
