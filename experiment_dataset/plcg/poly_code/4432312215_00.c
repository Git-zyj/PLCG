...
void kernel_4432312215_00(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
        ...
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                    D[i_2+1][i_1-1][i_0-1][i_3-1] = D[i_3+1][i_0][i_1][i_2] * D[i_2][i_0+1][i_1][i_3] - 2;
                }
            }
        }
    }
        ...
        for (int i_7 = 0; i_7 < PB_L-2; i_7++) {
            for (int i_8 = 1; i_8 < PB_N-2; i_8++) {
                for (int i_9 = 0; i_9 < -i_7+PB_L-2; i_9++) {
                    D[i_8][i_7][i_6-1][i_9] = F[i_6+1][i_9][i_8-1][i_7][i_6] - G[i_8+2][i_9+i_7+2][i_6+2][i_9+2] * 5;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4432312215_00(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
