...
void kernel_3314112113_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_3 = 3; i_3 < PB_L+3; i_3++) {
        for (int i_4 = i_3; i_4 < PB_N-1; i_4++) {
            for (int i_5 = 2; i_5 < PB_L; i_5++) {
                C[i_5-2][i_3][i_3] = D[i_4+1][i_5][i_3-2] + D[i_4-1][i_5][0] + C[i_3-3][i_4+1][i_4+1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3314112113_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
