...
void kernel_3226212313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(min(PB_M, i_0), -i_0+PB_L-1); i_1++) {
            A[i_1][i_1+i_0] = C[i_1][i_0+1] - A[i_1-1][i_1+i_0-1] * A[i_1][i_1+i_0+1] * 2;
            for (int i_2 = 0; i_2 < min(min(PB_M, -i_0+PB_L), i_0); i_2++) {
                A[i_1][i_2+i_1] = A[i_2][i_2+i_0] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3226212313_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
