...
void kernel_4334351311_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
                    A[i_1-1][i_2][i_0][i_3] = D[i_3+1][i_2][i_1][i_0] * E[i_2-1][i_0][i_3][i_1] - E[i_2][i_0][i_1][i_3+1] - F[i_3][i_1][i_0-1] - G[i_3-1][i_1-1][i_0][i_2] - A[i_1-2][i_2][i_0][i_3+1] + A[i_1-1][i_0-1][i_3+1][i_2] * 6;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                B[i_4][i_5][i_0+1] = F[i_4][i_5][i_0] * H[i_5][i_0-1][i_4-1] + I[i_0-1][i_5] + H[i_4][i_5][i_0] + J[i_5+1][i_0][i_4] * F[0][i_4+1][i_0-1] + B[i_4-1][i_5+1][i_0+2] * B[i_4][i_5-1][i_0+1] + B[i_4+1][i_5+1][i_0+1] * B[i_4+1][i_5][i_0+2] - B[i_0+1][i_5][i_4] * B[i_5][i_4][i_0+2] * 5;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
        C[i_6] = K[i_6-1] - L[i_6] + L[i_6] - K[i_6-1] + C[i_6+1] + 5;
    }
#pragma endscop
