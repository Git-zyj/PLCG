...
void kernel_3212212321_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_0-1][i_2][i_0] = C[i_2][0] - 3;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_P; i_4++) {
            for (int i_5 = 0; i_5 < PB_P; i_5++) {
                B[i_3-1][i_4][i_3] = B[i_3-3][i_5][i_3-2] * B[i_3+1][i_4][i_3+2] * B[i_3-1][i_4-2][i_3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3212212321_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
