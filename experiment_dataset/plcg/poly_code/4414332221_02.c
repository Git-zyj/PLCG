...
void kernel_4414332221_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
        ...
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
                for (int i_3 = i_2; i_3 < min(PB_P, -i_1+PB_M+1); i_3++) {
                    B[i_0][i_2+1][i_2][i_3] = G[i_2][i_0+2][i_1][i_2+1] + H[i_2+1][i_1][i_0][i_2] + I[i_3+i_1-1][i_2-2][i_1] * G[i_0][i_2-2][i_1][i_3] + B[i_1][i_2+1][i_2][i_0+2] - B[i_0+2][i_2+2][i_2+1][i_3] - B[i_0+1][i_1+1][i_1][i_3] + 6;
                }
            }
            A[i_1] = J[i_0][i_0] * K[i_1][i_0+1] - 1;
        }
    }
        ...
        for (int i_5 = 0; i_5 < PB_P; i_5++) {
            for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
                for (int i_7 = 0; i_7 < i_5; i_7++) {
                    D[i_5][i_7+1][i_6+1][i_5] = L[i_5][i_7][i_6] - 4;
                }
            }
        }
    }
...
kernel_4414332221_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
