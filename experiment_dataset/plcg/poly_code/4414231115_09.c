...
void kernel_4414231115_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
        ...
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_Q-1, -i_0+PB_P); i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_P; i_3++) {
                    A[i_0+1][i_1][i_0+1][i_3] = E[i_3][i_1+i_0][i_0][i_3-1] * E[i_1][i_3+i_2][i_0][i_2-1] - F[i_3][i_1+1][i_2+i_0] - A[i_0+1][i_2+1][i_0+1][i_3-1] + 6;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            for (int i_5 = max(i_4, 2); i_5 < PB_P; i_5++) {
                for (int i_6 = 0; i_6 < -i_5+PB_P; i_6++) {
                    C[i_4+2][i_5-2][i_0] = G[i_6][i_4-1][i_6+i_5] * F[i_6+1][i_4+1][i_5-1] * 1;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_P; i_7++) {
        for (int i_8 = i_7; i_8 < PB_M-1; i_8++) {
            for (int i_9 = 0; i_9 < -i_8+PB_P-1; i_9++) {
                C[i_9+1][i_8-1][i_7] = F[i_8+1][i_7][i_9+i_8+1] * F[i_9][i_7+1][i_8] * F[i_8][i_7][i_9] + C[i_9][i_8-1][i_7] + 4;
            }
        }
        for (int j_0 = 1; j_0 < min(PB_P-1, i_7); j_0++) {
            for (int j_1 = 1; j_1 < PB_P; j_1++) {
                for (int j_2 = 0; j_2 < PB_M; j_2++) {
                    D[j_0-1][i_7-1][j_1-1][j_2] = F[j_2][j_1][j_0+1] * H[j_2][j_0-1][j_1][0] + G[j_2][j_1][0] + 4;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414231115_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
