...
void kernel_3222351221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[0] = C[i_0-1] - C[i_0] * D[i_0] * C[i_0] * A[0] - A[0] * 2;
    }
    for (int i_1 = 2; i_1 < PB_Q-1; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_L; i_2++) {
            for (int i_3 = 1; i_3 < PB_P; i_3++) {
                B[i_1][i_2][i_1+1] = E[i_2+i_1][i_3][i_2-1] * F[i_3+1][i_1][i_2] + G[i_1][i_2][i_2] - F[i_1][i_3-1][i_2-1] * G[i_2+1][i_3-1][i_1] + B[i_1-2][i_2][i_1-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3222351221_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
