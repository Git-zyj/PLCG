...
void kernel_3322132313_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0][i_1] = D[i_0][i_0-2] - D[i_0][i_1] - A[i_0][i_1-1] + 6;
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_0+1][i_1-1] = E[i_2][i_1][i_0] * E[i_1][i_0][i_0] - E[i_2][i_0][i_1] * A[i_1+1][i_2] + A[i_0][i_1] - A[i_1][i_2] + A[i_1-1][i_2] * 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 3; i_4 < -i_3+PB_L-2; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_3+PB_P+2, PB_L+1); i_5++) {
                C[i_4-2][i_5][i_3] = E[i_3][i_4][i_5-1] * E[i_3][i_5-1][i_5-2] - E[i_5+1][i_4+i_3+2][i_5+i_3-2] * C[i_4-3][i_5][i_3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3322132313_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
