...
void kernel_2436352225_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 4; i_1 < min(min(PB_L, i_0), -i_0+PB_P-2); i_1++) {
            B[i_1-4] = E[i_1] + F[i_1][i_0] * F[i_1+1][i_1+i_0+2] - B[i_1-3] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = max(4, i_2); i_3 < -i_2+PB_L; i_3++) {
            B[i_3-2] = G[i_3][i_2-1] - H[i_2][i_3] + E[i_3+i_2] * I[i_3] * G[i_3][i_2-1] * E[i_3] + B[i_3-4] - 6;
        }
        B[i_2] = J[i_2] + J[i_2] - J[i_2] - I[i_2] - I[i_2] + I[i_2] * I[i_2] + 1;
        for (int i_4 = 2; i_4 < i_2; i_4++) {
            B[i_4-1] = H[i_4][i_2-2] * J[i_4-2] * F[i_4-1][i_2-2] - 3;
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2436352225_05(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
