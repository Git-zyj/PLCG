...
void kernel_4322252225_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        B[i_0][1] = D[i_0] + E[i_0] * D[i_0-1] - E[i_0] - 1;
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            B[i_1][1] = D[i_1] - F[i_1+1][i_0+2] * D[i_1] - G[i_0][i_1-1] + G[i_1][i_0+2] * G[i_0][i_1] * B[i_1+1][1] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_P); i_3++) {
            for (int i_4 = 2; i_4 < PB_P; i_4++) {
                B[i_3][1] = H[i_3][i_2][i_2] * H[i_4-1][i_2][i_3] + F[i_2-1][i_4] - G[i_4+1][i_3] * I[i_4-2][i_3+i_2][0] + B[i_4+2][1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4322252225_07(xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
