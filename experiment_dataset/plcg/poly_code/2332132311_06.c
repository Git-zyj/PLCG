...
void kernel_2332132311_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_P, -i_0+PB_L); i_1++) {
            A[i_1-1] = D[i_1][i_1] + D[i_1+i_0][i_1] - D[i_1][i_0-1] + D[i_0][i_1] - A[i_1-2] - A[i_1] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
        B[i_2] = A[i_2-1] - 4;
        for (int i_3 = 2; i_3 < -i_2+PB_P-2; i_3++) {
            C[i_3-2][i_2+2] = D[i_2][i_2-2] * D[i_3][i_3+i_2+2] + D[i_3][i_2] - A[i_3-1] - C[i_3-2][i_2+3] + C[i_3-1][i_2+2] - C[i_3-2][i_2+1] + 2;
        }
    }
#pragma endscop
...
kernel_2332132311_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
