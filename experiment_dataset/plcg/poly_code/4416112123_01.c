...
void kernel_4416112123_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M+2, PB_M-2); i_1++) {
            for (int i_2 = i_1; i_2 < PB_L; i_2++) {
                for (int i_3 = 2; i_3 < min(PB_M-1, -i_1+PB_Q); i_3++) {
                    A[i_1+i_0-2][i_3-2][i_2][i_0] = E[i_1+2][i_2][i_3-2][i_0] * E[i_3+1][i_1][i_0][i_2] - E[i_2][i_0][i_3+i_1][i_0+1] + 4;
                }
            }
        }
    ...
    for (int i_5 = 1; i_5 < PB_L-2; i_5++) {
        ...
        for (int i_7 = 0; i_7 < min(-i_5+PB_L, i_5); i_7++) {
            D[i_7+i_5][i_7+i_5] = C[i_5+2][i_7] - D[i_7+i_5-1][i_7+i_5-1] * 3;
        }
    }
#pragma endscop
...
kernel_4416112123_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
