...
void kernel_4324351323_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_L, -i_1+PB_Q); i_2++) {
                for (int i_3 = 0; i_3 < min(min(min(-i_2+PB_L, i_2), -i_1+PB_L), -i_0+PB_P); i_3++) {
                    A[i_2][0][i_3+i_0][i_3+1] = D[i_0][i_1-1][i_3+i_2][0] * E[i_3+i_1][i_0][i_2][i_2+i_1] * F[i_0-1][i_0][i_3] + G[i_3][i_3][i_2-1][i_0] + E[i_2][i_3][i_0][i_1] + A[i_2+2][0][i_3+i_0][i_3+1] - A[i_2+1][0][i_3+i_1][i_1+1] + A[i_1][0][i_3+i_0-2][i_3+1] - 6;
                }
            }
        }
        B[i_0] = H[i_0-1] * I[i_0] - H[i_0] * H[0] - B[i_0+2] - 6;
    }
    for (int i_4 = 3; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < PB_L; i_5++) {
            C[i_5-1][i_4-1] = J[i_5-1][i_5] - K[i_5][i_4] * J[i_5-1][i_4] - L[i_4][i_5] * C[i_5][i_4-1] - C[i_5-1][i_4-2] * C[i_4][i_5-1] * C[i_5-3][i_4-3] * C[i_5-1][i_4] + 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4324351323_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
