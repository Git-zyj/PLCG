...
void kernel_3212111323_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_M; i_2++) {
                A[i_2+i_0][i_1][i_2] = A[i_2+i_0-1][i_1-2][i_2] - A[i_1+i_0][i_2-2][i_0] - 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        B[i_3][i_3] = C[i_3] * B[i_3-2][i_3-2] - B[i_3-1][i_3-1] * B[i_3+2][i_3+2] * B[i_3+1][i_3+1] * 2;
    }
#pragma endscop
...
kernel_3212111323_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
