...
void kernel_3224112311_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_N-1, PB_L); i_1++) {
            for (int i_2 = i_1; i_2 < min(-i_0+PB_N+1, PB_L); i_2++) {
                A[i_1+i_0][i_2-2][i_0] = C[i_1-2][0] + A[i_2+i_0-1][i_1-2][i_2-1] - A[i_1+i_0+1][i_2-2][i_0] * 3;
            }
        }
        for (int i_3 = max(2, i_0); i_3 < PB_P; i_3++) {
            for (int i_4 = 1; i_4 < PB_M; i_4++) {
                B[i_4-1][i_3-1] = B[i_4-1][i_3] - B[i_4-1][i_3-2] + B[i_3-2][i_4-1] - B[i_4][i_3-1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3224112311_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
