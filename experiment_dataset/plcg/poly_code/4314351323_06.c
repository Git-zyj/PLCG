...
void kernel_4314351323_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            B[i_1] = D[i_1+i_0-1][i_1+1] + D[i_1+i_0][i_0] * D[i_1][i_0-1] + D[i_1-1][i_0-1] * E[i_1][i_0] - E[i_1+i_0][i_1] - B[i_1] + 1;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
            B[i_2] = F[i_0][i_2+1] * E[i_2+i_0][0] + D[1][i_0] - E[i_2][i_0-1] * E[i_2+1][i_0] + B[i_2+1] + 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_N-1; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_N-1; i_5++) {
                B[i_5-1] = G[i_5][i_5+i_4+1][i_3-1] * F[i_5+i_4][i_3+1] * H[i_3-1][i_5+i_4-1][i_3] + F[i_5+i_4][i_4] - B[i_5] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4314351323_06(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
