...
void kernel_4214252115_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            for (int i_2 = 2; i_2 < min(i_0, -i_1+PB_P); i_2++) {
                for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                    A[i_1+i_0][i_3][i_0+2][i_2+i_1] = C[i_3+2][i_2-1][i_1-2][i_0] * C[i_3+2][i_0][i_1-2][i_2-2] * D[i_1][i_2][i_3][i_1-1] - E[i_1][i_3+1][i_2] + 4;
                }
            }
        }
        for (int i_4 = max(2, i_0); i_4 < PB_L-3; i_4++) {
            B[i_4+2] = F[i_4-2][i_0] + G[i_4+1] * H[i_0+1][i_4-1] * F[i_0][i_4] + H[i_4-2][i_0-1] - B[i_4+1] * B[i_4+3] * 2;
        }
    }
#pragma endscop
...
kernel_4214252115_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
