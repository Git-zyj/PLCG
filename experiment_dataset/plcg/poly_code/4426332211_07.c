...
void kernel_4426332211_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
        ...
        for (int i_1 = 0; i_1 < min(PB_P-2, i_0); i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_M, i_1); i_2++) {
                for (int i_3 = 0; i_3 < min(PB_P-1, -i_1+PB_P); i_3++) {
                    A[i_2+1][i_0-2][i_3+1][i_0-2] = E[i_2+i_0][i_3][i_1+2][i_2] - F[i_0][i_2-2][i_3][i_1] + E[i_2][i_0][i_3+i_1][i_2] + G[i_2-2][i_0][i_1+2][i_2] - H[i_1+2][i_2-1][i_0][i_3][i_3] - A[i_1+1][i_0-1][i_3+1][i_0-1] - 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 0; i_6 < -i_5+PB_M; i_6++) {
                B[i_4][i_6][i_4] = I[i_4-2][i_6+i_5] - A[i_5+1][i_4-1][i_6+1][i_4-1] + 3;
            }
            for (int i_7 = 0; i_7 < i_4; i_7++) {
                for (int i_8 = i_4; i_8 < -i_4+PB_P; i_8++) {
                    C[i_4+2][i_7][i_7][i_8+1] = F[i_5][i_8+i_4][i_7][0] + A[i_8+2][i_4-2][i_7+1][i_4-2] - C[i_4+2][i_5][i_5][i_8+2] * 1;
                }
            }
        }
    }
    for (int i_9 = PB_M-2 - 1; i_9 >= 3; i_9--) {
        D[i_9-2] = J[i_9+2] + K[i_9-1] - J[i_9] + K[i_9] * K[i_9] + D[i_9-1] * D[i_9-3] - D[i_9-1] * 5;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4426332211_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
