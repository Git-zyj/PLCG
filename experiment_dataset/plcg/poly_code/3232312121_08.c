...
void kernel_3232312121_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
        for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
            for (int i_3 = max(1, i_2); i_3 < -i_2+PB_N-1; i_3++) {
                B[i_3+i_2][i_3+2] = C[i_2][i_1-1][i_3-1] * B[i_3+i_2+1][i_3+2] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3232312121_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
