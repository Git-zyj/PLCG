...
void kernel_3422231223_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_1][i_0+2] = E[i_1][i_0] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = i_2; i_3 < PB_P; i_3++) {
            B[i_2][i_3] = E[i_3+1][i_2] - F[i_2][i_3] - 5;
        }
        for (int i_4 = 2; i_4 < PB_P; i_4++) {
            for (int i_5 = 2; i_5 < min(PB_P-1, -i_4+PB_M+1); i_5++) {
                C[i_2][i_4][0] = G[i_5+i_4-1][i_2-1][i_4-1] * F[i_4-1][i_5] - G[i_4][i_2+1][i_5+1] - C[i_2][i_5-2][0] * C[i_4][i_5+1][0] * C[i_2][i_4-2][0] - 6;
            }
            for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
                B[i_2][i_6] = H[i_6][i_2][i_4-1] - H[i_4-1][i_6+1][i_4] + H[i_2-1][i_4][1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3422231223_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
