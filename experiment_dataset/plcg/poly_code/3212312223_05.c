...
void kernel_3212312223_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        ...
        for (int i_1 = 2; i_1 < PB_L+1; i_1++) {
            for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
                B[i_1][i_2][i_2-2] = C[i_2-2][i_0][i_0] - D[i_0-2][i_1-1] + B[i_1-2][i_2+1][i_2-1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3212312223_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
