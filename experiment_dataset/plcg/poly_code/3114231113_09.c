...
void kernel_3114231113_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_L, -i_0+PB_P); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_L, i_1); i_2++) {
                A[i_1][i_2][i_1-1] = B[i_2+i_1][i_1][i_0] * C[i_1][i_2+i_0][i_1+i_0] + C[i_0][i_1][0] + A[i_1-1][i_2-1][i_1-2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3114231113_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
