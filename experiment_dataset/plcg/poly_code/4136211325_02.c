...
void kernel_4136211325_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < min(-i_0+PB_Q, PB_P); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_Q, -i_1+PB_P); i_2++) {
                A[i_0][i_2+i_1-1][i_1+i_0] = B[i_2+1][i_1] + A[i_0][i_2+i_1-2][i_1+i_0-1] * A[i_0][i_2+i_1-3][i_2+i_0-2] + A[i_0][i_2+i_1][i_2+i_0] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4136211325_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
