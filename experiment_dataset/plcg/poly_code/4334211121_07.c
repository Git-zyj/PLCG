...
void kernel_4334211121_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_1][i_2] = D[i_1][i_0-1][i_2] + 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            B[i_3][i_4] = E[i_4-1][i_3] + B[i_3-2][i_4] + 1;
        }
    }
    for (int i_5 = 2; i_5 < PB_L+2; i_5++) {
        for (int i_6 = i_5; i_6 < -i_5+PB_P-1; i_6++) {
            for (int i_7 = 1; i_7 < min(PB_L, -i_6+PB_P+2); i_7++) {
                C[i_5][i_7][i_6+i_5+1] = D[i_5][i_7][i_6-1] + C[i_7-1][i_5-2][i_7+i_6-2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4334211121_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
