...
void kernel_2232231313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = C[i_0] + D[i_0+1] - D[0] + D[i_0-1] + A[i_0+1] * A[i_0-1] - 2;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            B[i_1][i_2] = D[i_2] + E[i_1][i_2] - B[i_1][i_2+1] * B[i_1+1][i_2+1] * B[i_1-1][i_2] - 1;
        }
    }
#pragma endscop
...
kernel_2232231313_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
