...
void kernel_4236312121_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
            for (int i_4 = 0; i_4 < min(i_2, PB_L-1); i_4++) {
                B[i_2][i_3-1][i_4+1] = B[i_2+2][i_3-1][i_4+1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4236312121_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
