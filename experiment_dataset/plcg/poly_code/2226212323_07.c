...
void kernel_2226212323_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][i_1] = A[i_0+1][i_1] - A[i_0-1][i_1] - A[i_1][i_0+1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            A[i_2+1][i_3] = C[i_3+2] - 6;
        }
    }
#pragma endscop
...
kernel_2226212323_07(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
