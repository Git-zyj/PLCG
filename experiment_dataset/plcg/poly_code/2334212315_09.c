...
void kernel_2334212315_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_0+1][i_1] = A[i_0+2][i_1-1] + A[i_1+2][i_0+1] - A[i_0][i_1] * 6;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        C[i_2-2] = D[i_2] - 6;
        C[i_2-2] = D[i_2-2] - C[i_2-3] - C[i_2-1] - 2;
    }
#pragma endscop
...
kernel_2334212315_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
