...
void kernel_4332132313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0-1] - A[i_0-1] * A[i_0+1] * A[i_0-1] + A[i_0-1] + 4;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < -i_1+PB_M-1; i_3++) {
                C[i_1] = E[i_2][i_3][i_1-2] + F[i_3+i_1+1][i_2] - E[i_1][i_3][i_2] * 3;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
            C[i_5-1] = F[i_5][i_4] + D[i_4] - F[i_5+2][i_4] + F[i_4][1] * C[i_5-2] - 5;
        }
    }
#pragma endscop
...
kernel_4332132313_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
