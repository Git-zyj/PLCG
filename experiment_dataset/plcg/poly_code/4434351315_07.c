...
void kernel_4434351315_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    A[i_0][i_1][i_2][i_3] = E[i_0][i_3][i_1][i_2] * E[i_3][i_0-1][i_2+i_1+1][i_0] * F[i_1][i_3+i_2][0] + G[i_2+1][i_3][i_1] + F[i_3][i_1][i_2] * H[i_0+1][i_2+i_1+1][i_3][0] * I[i_1][i_0-1][i_2][i_0+1] * A[i_0][i_1+1][i_2+1][i_3+1] - 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_M; i_5++) {
            B[i_5-1][i_4+1] = J[i_5][i_4+1] * B[i_5-2][i_4+1] + B[i_5-1][i_4+2] - B[i_5-1][i_4] + B[i_5][i_4+1] - 1;
        }
        for (int i_6 = 0; i_6 < PB_N; i_6++) {
            B[i_4-2][i_6+1] = K[i_4-1][i_6] * J[i_6][i_4] - L[i_4][i_6] * 6;
        }
    }
    for (int i_7 = 1; i_7 < PB_M; i_7++) {
        for (int i_8 = i_7; i_8 < -i_7+PB_Q; i_8++) {
            for (int i_9 = 1; i_9 < min(-i_7+PB_P, PB_M); i_9++) {
                B[i_9-1][i_8+1] = M[i_9][i_8+1][i_7-1] * L[i_7+1][i_8] + M[i_8][i_9][i_8+i_7] * M[i_8][i_9+i_7][i_8+1] * G[i_8-1][i_9][i_7+1] * M[i_7][i_9][i_8+1] * 1;
            }
        }
    }
#pragma endscop
