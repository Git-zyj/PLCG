...
void kernel_3326112213_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-3; i_1++) {
            for (int i_2 = 2; i_2 < i_1; i_2++) {
                A[i_0][i_1+2][i_2-1] = A[i_0-1][i_1+2][i_2-2] + A[i_0-1][i_1+3][i_2-1] * B[i_1][i_1] * 4;
            }
            B[i_1][i_1] = C[i_1-1][i_1-1][i_0-1][i_0-3] + 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < i_3; i_4++) {
            for (int i_5 = 2; i_5 < min(PB_L-1, i_3); i_5++) {
                C[i_3][i_3][i_5][i_5-2] = D[i_4-1][i_5-2] * C[i_5+1][i_5+1][i_4][i_4-2] + C[i_3-1][i_3-1][i_5][i_5-2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3326112213_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
