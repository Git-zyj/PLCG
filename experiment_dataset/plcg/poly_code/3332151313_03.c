...
void kernel_3332151313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_1-1][i_0] = D[i_1-1][i_0] * D[i_1][0] * D[i_0-1][i_1] * D[i_1][i_0] * D[i_0][i_1-1] - D[i_1][i_0] - A[i_1-2][i_0] * A[i_1-1][i_0+1] - A[i_1-1][i_0-1] - A[i_1][i_0] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_Q, -i_2+PB_M-1); i_3++) {
            for (int i_4 = i_2; i_4 < min(-i_3+PB_P, PB_L+1); i_4++) {
                C[i_2+1][i_4-1][i_4+i_3] = E[i_3+1][i_4][i_4-1] + E[i_4][i_3+1][i_2] * E[i_4+1][i_2][i_3] + E[i_2][i_4][i_3] - E[i_3+i_2+1][i_4][i_2-1] * 4;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_Q+1; i_5++) {
        for (int i_6 = 0; i_6 < -i_5+PB_Q+1; i_6++) {
            for (int i_7 = 1; i_7 < i_6; i_7++) {
                C[i_5][i_7-1][i_7+i_6+1] = D[i_5][i_7] + E[i_5-1][i_7+1][i_6+i_5-1] - C[i_5-1][i_7-1][i_7+i_6+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3332151313_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
