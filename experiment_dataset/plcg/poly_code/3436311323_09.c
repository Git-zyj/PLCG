...
void kernel_3436311323_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            A[i_0+1][i_1] = A[i_0+2][i_1] - A[i_0-1][i_1] + A[i_0][i_1] - 2;
        }
    }
    ...
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        C[i_3] = F[i_3+1] * C[i_3+2] * C[i_3-2] * 4;
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = max(i_4, 4); i_5 < -i_4+PB_M+1; i_5++) {
            for (int i_6 = 2; i_6 < min(-i_5+PB_L, PB_M+2); i_6++) {
                D[i_4][i_6+i_5][i_6+1] = G[i_5+i_4-1][i_4][i_6+i_5] * D[i_4][i_6+i_5-4][i_6-1] * D[i_4][i_6+i_5-1][i_6] * D[i_6-2][i_5+i_4+2][i_5+3] - D[i_4+2][i_6+i_5][i_6+1] * 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3436311323_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
