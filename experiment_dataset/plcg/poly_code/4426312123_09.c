...
void kernel_4426312123_09(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < PB_P-1; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_L, PB_P-2); i_3++) {
                    B[i_1-1][i_3+2][i_2+1][i_3+2] = E[i_1][i_0+1][i_2-2][0] * F[i_1+2][i_3+i_0][i_2+1][i_0] + 6;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            for (int i_5 = i_0; i_5 < PB_L; i_5++) {
                for (int i_6 = i_5; i_6 < PB_Q-1; i_6++) {
                    B[i_0-1][i_4][i_6+1][i_4] = 3;
                }
            }
        }
        for (int i_7 = 2; i_7 < PB_N+2; i_7++) {
            C[i_0] = G[i_7-1][i_0] * C[i_7-2] + 6;
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4426312123_09(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
