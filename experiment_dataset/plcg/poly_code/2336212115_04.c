...
void kernel_2336212115_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_N, PB_M-2); i_3++) {
            C[i_2+1][i_3][i_3] = D[i_3-1][i_2] - D[i_3+i_2][0] - C[i_3+1][i_2-1][i_2-1] + C[i_3+2][i_2][i_2] + 5;
        }
    }
#pragma endscop
...
kernel_2336212115_04(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
