...
void kernel_4424232223_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
        ...
        for (int i_1 = 2; i_1 < min(PB_Q-2, i_0); i_1++) {
            for (int i_2 = i_0; i_2 < -i_1+PB_Q; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_2+PB_Q, -i_0+PB_P+2); i_3++) {
                    A[i_1][i_3+1][i_2+2][i_1] = E[i_1][i_0][i_3-1][i_2+i_1] - E[i_0][i_3+i_0-2][i_1][i_2-2] * F[i_0][i_3+i_2][1][i_1][i_1+2] * G[i_1-2][i_3][i_0-1][i_2] * G[i_2-2][i_0][i_1][i_3] * A[i_2][i_3+2][i_1][i_2] + 6;
                }
            }
        }
        for (int i_4 = 1; i_4 < min(-i_0+PB_L, PB_P-2); i_4++) {
            for (int i_5 = 1; i_5 < PB_P-2; i_5++) {
                B[i_4+i_0][i_5+1][i_5-1] = H[i_4-1][i_0][i_5] + H[i_4][i_5][i_4+2] - I[i_5][i_0-1][i_4+2] * B[i_4+i_0][i_5+2][i_5] * 4;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_L-2; i_6++) {
        for (int i_7 = 0; i_7 < PB_M-2; i_7++) {
            for (int i_8 = max(1, i_7); i_8 < -i_6+PB_P; i_8++) {
                for (int i_9 = 2; i_9 < -i_6+PB_L-2; i_9++) {
                    C[i_8-1][i_7+2][i_9][i_6] = G[i_9-1][i_7][i_8+i_6][i_6] - G[i_7][i_9+i_6+2][i_8][0] * C[i_8-1][i_7+2][i_9-2][i_6-2] - 1;
                }
            }
        }
        for (int j_0 = i_6; j_0 < PB_L-1; j_0++) {
            D[i_6][j_0] = H[i_6][j_0][2] - J[j_0][i_6] - D[i_6][j_0+1] + D[j_0][i_6-1] + D[i_6][j_0-2] * 6;
        }
    }
#pragma endscop
...
kernel_4424232223_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
