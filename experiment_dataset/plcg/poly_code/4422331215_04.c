...
void kernel_4422331215_04(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M+1, PB_M-1); i_2++) {
                D[i_1-1][i_0+2][i_2][i_1-1] = E[i_1-1][i_2-1][i_0+1] - F[i_2+1][i_1] + G[i_2+i_1-1][i_0] * H[i_1-1][i_2][i_0-1] - 5;
            }
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_M, -i_0+PB_L); i_4++) {
                B[i_0][i_3][i_4] = I[i_3][i_4][i_0] * J[i_4+1][i_4-1] - E[i_4][i_4+i_0][i_3] * B[i_0][i_3][i_4-1] + 6;
            }
        }
        C[i_0] = K[i_0] + L[i_0+1] - L[i_0] + L[i_0] * C[i_0+1] - 1;
    }
    for (int i_5 = 0; i_5 < PB_P-2; i_5++) {
        for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 0; i_7 < PB_Q-1; i_7++) {
                D[i_6][i_5+1][i_7][i_6] = D[i_6+1][i_5+2][i_7][i_6+1] + D[i_6][i_5+1][i_7+1][i_6] + 3;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4422331215_04(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L)));
