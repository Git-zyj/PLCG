...
void kernel_2434332315_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(min(-i_0+PB_M, i_0), PB_P-1); i_1++) {
            A[i_1-1][i_0] = E[i_0] + F[i_1+i_0][i_0] + G[i_1+i_0][i_0] - A[i_1-1][i_0-1] + B[i_0-2][i_1] - B[i_0-2][i_1+1] + B[i_0-1][i_1] - B[i_0-2][i_1-1] + B[i_0-3][i_1-1] + 2;
            B[i_0-2][i_1] = G[i_1][i_0] * H[i_1+i_0][i_1+1] - F[i_0][i_1] - G[i_1+i_0][i_1] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L+2; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_L, -i_2+PB_M-2); i_3++) {
            B[i_2-2][i_3] = E[i_2] + I[i_3+i_2+2] - 4;
        }
    }
    for (int i_4 = PB_L-2 - 1; i_4 >= 1; i_4--) {
        D[i_4] = J[i_4+2] - D[i_4+1] - D[i_4-1] * D[i_4-1] - 5;
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2434332315_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
