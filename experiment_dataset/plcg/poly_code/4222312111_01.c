...
void kernel_4222312111_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = PB_L - 1; i_1 >= 0; i_1--) {
            for (int i_2 = 0; i_2 < -i_0+PB_Q-1; i_2++) {
                for (int i_3 = max(1, i_0); i_3 < -i_0+PB_M; i_3++) {
                    A[i_2][i_3+i_0][i_1] = C[i_3-1][i_0][i_1][i_2+i_0+1] * 1;
                }
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < PB_M+2; i_5++) {
            for (int i_6 = 1; i_6 < PB_M+1; i_6++) {
                for (int i_7 = i_6; i_7 < PB_L-2; i_7++) {
                    B[i_6][i_4][i_5-2][i_7+1] = B[i_7-1][i_6-1][i_5-2][i_4+1] - B[i_6][i_4+1][i_5-2][i_7+2] + 3;
                }
            }
        }
    }
...
kernel_4222312111_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
