...
void kernel_2436332221_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = E[i_0] + F[i_0] * A[i_0+2] - A[i_0-1] * A[i_0+1] * 1;
    }
    for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
        A[i_1] = 5;
        A[i_1-1] = G[i_1][i_1+1] * H[i_1] + H[i_1] * F[i_1] + 3;
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            D[i_3][i_3] = G[i_3][i_2] - I[i_2][i_3] + G[i_3][2] + I[i_3][i_2+2] - 1;
        }
    }
#pragma endscop
...
kernel_2436332221_04(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
