...
void kernel_4432232223_00(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 3; i_2 < PB_M-1; i_2++) {
                for (int i_3 = i_0; i_3 < PB_M; i_3++) {
                    D[i_2-3][i_3+2][i_0+2][i_1+2] = E[i_2+1][i_0][i_1][i_3-2] - F[i_3][i_1+1][i_0+1][i_2] * F[i_1][i_3][i_0][i_2+2] * D[i_3-2][i_1+1][i_0+3][i_2] - 5;
                }
            }
        }
        for (int i_4 = 0; i_4 < min(-i_0+PB_M+2, PB_M-2); i_4++) {
            for (int i_5 = max(i_4, 2); i_5 < min(PB_P-2, -i_4+PB_P); i_5++) {
                for (int i_6 = 1; i_6 < PB_M; i_6++) {
                    B[i_4][i_0][i_5][i_6+1] = E[i_4][i_0+1][i_6-1][i_4] - F[i_6][i_4+2][i_0][i_5-2] + E[i_6][i_0][i_4][i_5+i_4] * B[i_4+2][i_0][i_5+2][i_6+1] * B[i_6][i_4][i_5+1][i_0+1] * B[i_4][i_0][i_5-1][i_6+1] * 2;
                }
            }
            for (int i_7 = 2; i_7 < i_0; i_7++) {
                C[i_0][i_4][i_4+i_0] = G[i_4+i_0-2][i_7-2][i_4] + G[i_0][i_7][i_4] - G[i_7][i_7-2][i_4] * 2;
            }
        }
    }
    for (int i_8 = 0; i_8 < PB_Q-2; i_8++) {
        for (int i_9 = 1; i_9 < PB_M; i_9++) {
            for (int j_0 = 0; j_0 < min(PB_L-1, -i_9+PB_Q); j_0++) {
                for (int j_1 = max(i_8, 2); j_1 < PB_M; j_1++) {
                    D[j_1-2][i_9+1][i_8+2][j_0] = F[i_9-1][j_1][j_0+1][i_8] * C[i_9][j_0][j_0+i_9] * 6;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4432232223_00(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
