...
void kernel_3434252315_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0+1] - F[i_0] * G[i_0-2][0] * F[i_0] + A[i_0+1] - A[i_0-1] - A[i_0+1] - 2;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            D[i_2+2] = H[i_2][i_1] * G[i_2][i_1] + 1;
        }
        C[i_1] = E[i_1] - E[i_1] + E[i_1] + F[i_1] + F[0] - D[i_1+2] + 1;
    }
    for (int i_3 = 2; i_3 < PB_L-3; i_3++) {
        D[i_3+2] = E[i_3] * E[i_3] + F[i_3-2] * E[i_3] + E[i_3-2] - E[0] - F[i_3-1] + F[i_3] - D[i_3+3] - D[i_3+1] - D[i_3+1] * 4;
    }
#pragma endscop
...
kernel_3434252315_09(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
