...
void kernel_2212211321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = C[i_0] + A[i_0-1] - A[i_0+2] * 1;
        for (int i_1 = 0; i_1 < PB_N-3; i_1++) {
            B[i_1+1] = B[i_1+2] - B[i_1+3] + B[i_1] + 2;
        }
    }
#pragma endscop
...
kernel_2212211321_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
