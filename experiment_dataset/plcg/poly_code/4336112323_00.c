...
void kernel_4336112323_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-4; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M-2; i_2++) {
                A[i_1][i_1+2][i_0+2] = D[i_2+2][i_0-1][i_1] * A[i_1][i_1+2][i_0+3] + A[i_1-2][i_1][i_0+4] * A[i_1+2][i_1+4][i_0+3] - 2;
            }
        }
        for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                B[i_0][i_4][i_3+1] = B[i_0][i_4-1][i_3] - 1;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
        for (int i_6 = 1; i_6 < min(PB_Q-2, i_5); i_6++) {
            for (int i_7 = 2; i_7 < PB_P-2; i_7++) {
                C[i_5+2][i_7][i_6] = C[i_5+2][i_7][i_6+2] * C[i_5+2][i_7+2][i_6] + C[i_6+2][i_5+1][i_7] + C[i_5][i_7-2][i_6+2] * C[i_5][i_7][i_6-1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4336112323_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
