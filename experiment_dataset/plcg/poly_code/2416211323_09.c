...
void kernel_2416211323_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        C[i_1+2] = C[i_1] + C[i_1-1] - 4;
        for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
            C[i_2] = A[i_2] - A[i_2+2] * A[i_2+1] - 5;
        }
        for (int i_3 = 1; i_3 < min(i_1, PB_M-2); i_3++) {
            D[i_1] = E[i_3][i_1] + D[i_3+2] + D[i_1+1] * D[i_3+1] + D[i_3-1] - 5;
        }
    }
#pragma endscop
...
kernel_2416211323_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
