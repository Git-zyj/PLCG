...
void kernel_4414131215_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-1] = E[i_0] + E[i_0] * A[i_0-2] * 6;
        A[i_0-1] = F[i_0-1][i_0-1] - E[i_0] - E[i_0] + A[i_0] - 5;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
            for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
                C[i_2][i_3-1] = G[i_2+i_1][i_3][i_2] - C[i_2][i_3-2] + C[i_3+1][i_2-1] - 5;
            }
        }
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = max(i_4, 1); i_5 < PB_P; i_5++) {
                for (int i_6 = 0; i_6 < min(-i_5+PB_P, -i_4+PB_L-1); i_6++) {
                    C[i_6][i_1-1] = H[i_1][i_6+i_5][i_4][1] + H[i_1][i_6][i_4][1] * H[i_6+i_4+1][i_5-1][i_5][0] - 2;
                }
            }
        }
    }
...
kernel_4414131215_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
