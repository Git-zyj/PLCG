...
void kernel_4436231111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0+1] * F[i_0-1] - E[i_0] - E[0] - A[i_0-1] + 5;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[0] = F[i_1] + F[i_1+1] * E[i_1] - B[0] * 3;
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                C[i_3+1][i_1][i_1-1] = G[i_3][i_2][i_1] - G[i_3][i_1+1][i_2] * G[i_2][i_1-1][i_3] - C[i_3+2][i_1][i_1-1] * 5;
            }
        }
        D[i_1+1] = F[i_1-1] * E[i_1] + 5;
    }
#pragma endscop
...
kernel_4436231111_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
