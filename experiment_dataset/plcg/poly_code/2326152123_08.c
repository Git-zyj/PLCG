...
void kernel_2326152123_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_Q, -i_0+PB_N); i_1++) {
            A[i_1+i_0][i_0] = D[i_0][i_1][0] * E[i_1+i_0][0] - E[i_1][i_0-1] * F[i_0-1] + F[0] + F[i_1-1] * 4;
        }
    }
    for (int i_2 = 4; i_2 < PB_Q; i_2++) {
        B[i_2][i_2-2] = F[i_2] - F[i_2] + F[i_2-2] - F[0] - B[i_2-1][i_2-3] * B[i_2-2][i_2-4] * 5;
        for (int i_3 = 2; i_3 < min(-i_2+PB_L-2, i_2); i_3++) {
            C[i_3][i_2-2] = E[i_3+2][i_2] + E[i_3+i_2+2][i_3-1] + E[i_3][i_2] * F[i_2] * E[i_2-2][i_3-2] * C[i_3-2][i_2-2] - 5;
        }
    }
#pragma endscop
...
kernel_2326152123_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
