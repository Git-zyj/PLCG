...
void kernel_3216311223_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_Q; i_2++) {
                B[i_0+1] = C[i_1+1][i_0][i_2+i_1] - B[i_0] * 6;
            }
        }
        for (int i_3 = 1; i_3 < i_0; i_3++) {
            B[i_0] = D[i_3+1][i_0] - B[i_3-1] - 2;
        }
    }
#pragma endscop
...
kernel_3216311223_06(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
