...
void kernel_3214211121_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
                A[i_1-1][i_2] = A[i_1-1][i_2+1] * 4;
            }
        }
        for (int i_3 = i_0; i_3 < min(PB_N-1, -i_0+PB_M+1); i_3++) {
            B[i_3][i_0+1] = C[i_0][i_3] * C[i_3+i_0-1][0] + B[i_3+1][i_0+1] * 4;
        }
    }
#pragma endscop
...
kernel_3214211121_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
