...
void kernel_3326111323_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L; i_2++) {
                A[i_1][i_2] = D[i_0][i_2+i_1] - A[i_1-2][i_2] - A[i_0+1][i_2] * 1;
            }
        }
        for (int i_3 = i_0; i_3 < -i_0+PB_L+1; i_3++) {
            C[i_3][i_3+i_0-1] = A[i_3-2][i_0-2] * 6;
        }
    }
    for (int i_4 = 2; i_4 < PB_L+1; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_L+1; i_5++) {
            C[i_5][i_5+i_4-1] = C[i_5-1][i_5+i_4-2] + C[i_5][i_5+i_4-2] - 1;
        }
    }
#pragma endscop
...
kernel_3326111323_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
