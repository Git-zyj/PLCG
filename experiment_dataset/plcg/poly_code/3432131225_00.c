...
void kernel_3432131225_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_0+1][i_1-1] = E[i_0][i_1][i_0] + F[i_0][1] - G[i_0] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                B[i_2+1][i_4-1] = H[i_4][i_3-1][i_2][i_2] + B[i_2-1][i_4-1] + 2;
            }
        }
        for (int i_5 = i_2; i_5 < PB_N-1; i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_M; i_6++) {
                B[i_5+1][i_6-1] = E[i_5][i_2][i_5] - F[i_6][i_2] - E[i_6+i_5][i_2][i_5] * 3;
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
        for (int i_8 = 1; i_8 < PB_N-1; i_8++) {
            for (int i_9 = 1; i_9 < -i_7+PB_M-1; i_9++) {
                B[i_8-1][i_9-1] = E[i_9-1][i_7][i_7] - H[i_7+1][i_9-1][i_9][i_8-1] * E[i_9-1][i_8][i_7] + E[i_9+i_7+1][i_8][0] * B[i_8+1][i_9-1] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3432131225_00(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
