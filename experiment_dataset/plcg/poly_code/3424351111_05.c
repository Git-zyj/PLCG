...
void kernel_3424351111_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        A[i_0] = E[i_0] * F[i_0] + G[i_0] * E[i_0+1] * 1;
        for (int i_1 = i_0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, PB_P); i_2++) {
                B[i_0][i_1][i_2] = H[i_1][i_2+1][i_1] + I[i_2+1][i_0][1] * J[i_2+i_0][0] + K[i_2][i_0][i_1] - K[i_2+i_0-1][i_2+1][i_1] * I[i_0][i_2-1][i_1+1] * I[i_1][i_0][i_2] + B[i_0][i_1+1][i_2] * 6;
            }
        }
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            C[i_0][0] = F[i_0-1] + L[i_3+1][i_0] * J[i_0][i_0] - C[i_0-1][0] * 6;
        }
    }
    for (int i_4 = 0; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < min(PB_P, -i_4+PB_L-1); i_5++) {
            D[i_5-1][i_5+i_4+1] = K[i_5][i_5+1][0] + J[i_4][i_5+i_4] * L[i_5][i_4] - M[i_5][0] + D[i_5-1][i_5+i_4] - 1;
        }
    }
#pragma endscop
