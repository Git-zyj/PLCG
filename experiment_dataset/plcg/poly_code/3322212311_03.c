...
void kernel_3322212311_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 3; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_1-2][i_2][i_2] = A[i_1-3][i_2][i_2] + A[i_1-1][i_0][i_0] - A[i_1-2][i_2+1][i_2+1] - A[i_1-1][i_2][i_2] - 5;
            }
        }
    ...
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            C[i_3][i_4] = E[i_4] + E[i_4-1] + B[i_3] - C[i_4][i_3-1] - C[i_4+1][i_3-1] + 5;
        }
    }
#pragma endscop
...
kernel_3322212311_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
