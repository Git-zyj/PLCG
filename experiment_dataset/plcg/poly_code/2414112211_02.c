...
void kernel_2414112211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            A[i_1] = E[i_1+2][i_0-1] + A[i_1-1] - A[i_1+1] - 5;
        }
        for (int i_2 = 0; i_2 < min(i_0, -i_0+PB_L-1); i_2++) {
            B[i_0-2][i_2+i_0] = B[i_0-2][i_2+i_0+1] - B[i_0-1][i_2+i_0+1] + B[i_0-2][i_2+i_0-1] + 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            C[i_4] = E[i_4][i_3] + E[i_4+1][i_3] + 6;
        }
        for (int i_5 = 0; i_5 < PB_M-3; i_5++) {
            D[i_5+2][i_3] = D[i_5+3][i_3] - D[i_5+1][i_3-1] + D[i_5+1][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_2414112211_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
