...
void kernel_4336211225_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_L; i_1++) {
        B[i_1][i_1-1] = D[i_1] * B[i_1-2][i_1-3] * 6;
    }
    for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 0; i_3 < min(min(-i_2+PB_L+1, PB_M), i_2); i_3++) {
            A[i_3] = E[i_3][i_3+i_2-1] * 1;
        }
    }
#pragma endscop
...
kernel_4336211225_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
