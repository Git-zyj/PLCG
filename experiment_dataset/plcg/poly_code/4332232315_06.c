...
void kernel_4332232315_06(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                C[i_2-1][i_1+1] = D[i_1+i_0-2][i_2][i_1+i_0] * 1;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < -i_3+PB_P; i_4++) {
            for (int i_5 = 2; i_5 < PB_P-2; i_5++) {
                for (int i_6 = 1; i_6 < i_4; i_6++) {
                    C[i_6][i_5] = E[i_5][i_3][i_6][i_4+i_3] - F[i_5+2][i_5+2][i_6-1][i_4+1] * G[i_4][i_5-1][i_6][i_3][i_5-2] * 3;
                }
            }
        }
        for (int i_7 = 1; i_7 < PB_M-2; i_7++) {
            for (int i_8 = 0; i_8 < -i_3+PB_L-2; i_8++) {
                C[i_7][i_8] = H[i_7+2][i_8+i_3+2][i_8] + D[i_7][i_3][i_8] + D[i_8+2][i_3-2][i_7] + H[i_8][i_3+2][i_8] * C[i_8+1][i_3] * C[i_7+1][i_8+1] * C[i_7-1][i_8] * 5;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4332232315_06(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
