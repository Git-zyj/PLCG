...
void kernel_3122152121_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < PB_N; i_1++) {
            for (int i_2 = min(min(-i_0+PB_L, PB_P-2), -i_1+PB_Q) - 1; i_2 >= 0; i_2--) {
                A[i_1][i_0][i_2+2] = B[i_1][i_2+i_0][i_1] + B[i_1][i_0+1][i_2+i_1] * B[i_2][i_1+2][i_0] + B[i_1][i_1][i_0] * C[2][i_1-2] - A[i_1][i_0+2][i_2+2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3122152121_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
