...
void kernel_4414252225_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M-1; i_1++) {
            A[i_0-1][i_1] = E[i_1+2] - F[i_0][i_1] - F[i_1+2][i_0] + E[i_1] - G[i_1][i_0] * G[i_0][i_1] + G[i_1][i_0] * F[i_1+i_0+1][0] + H[i_0] - F[i_0][i_0] + F[i_1][i_0] + A[i_0-1][i_1-2] * 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            A[i_3-1][i_2] = F[i_3][0] + F[i_3][i_2] + H[i_2] - 2;
            A[i_2-1][i_3] = F[i_3-1][i_2-2] - G[i_3][i_2] - F[i_2-1][i_3] + 3;
        }
        for (int i_4 = 1; i_4 < i_2; i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                D[i_2][i_2-1][i_5] = G[i_5][i_2] * I[i_5-1][i_2][i_4] * D[i_2][i_2-1][i_5-1] - D[i_4][i_4-1][i_5+1] + D[i_2-1][i_2-2][i_5] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4414252225_00(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
