...
void kernel_2236232215_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            B[i_3] = D[i_3][i_3+2] + D[i_2][i_3] + E[i_3-2][i_2] - E[i_3][i_2+1] * E[i_2-1][i_3-2] + B[i_3+1] + B[i_3-1] + 4;
        }
    }
#pragma endscop
...
kernel_2236232215_06(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
