...
void kernel_2334131225_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-2, i_0); i_1++) {
            A[i_1+i_0][i_0] = D[i_1][i_0-1] * D[i_1][i_1+i_0+1] + D[i_1][i_0] - E[i_0+1] - A[i_1+i_0+2][i_0+2] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_N-1; i_3++) {
            B[i_2][i_3] = D[i_3+1][i_2] + D[i_3+1][i_2] + B[i_3-1][i_2-2] * B[i_2-2][i_3] - B[i_2][i_3-2] + 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        C[0] = E[i_4] * E[i_4] * E[i_4] * C[0] - C[0] + 6;
    }
#pragma endscop
...
kernel_2334131225_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
