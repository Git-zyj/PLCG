...
void kernel_2426331325_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            B[i_0][i_1] = E[i_1+1] + F[i_1][i_0] + G[i_1][i_0] + B[i_0][i_1+2] + B[i_1][i_0+2] - 4;
            B[i_0][i_1] = F[i_1+i_0][0] + G[i_0][i_1] * F[i_1+i_0-1][0] - G[i_1][i_0+1] * B[i_0+2][i_1] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_M-1); i_3++) {
            C[i_3+i_2-1] = H[i_3][i_2+1] - F[i_3][i_2+1] + C[i_3+i_2-1] + C[i_3+i_2+1] + D[i_2-2][i_2-2] + 1;
        }
        D[i_2][i_2] = I[i_2] + I[i_2] * E[i_2] + D[i_2+1][i_2+1] + 1;
    }
#pragma endscop
...
kernel_2426331325_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
