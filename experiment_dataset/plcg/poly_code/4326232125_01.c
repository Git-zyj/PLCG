...
void kernel_4326232125_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        ...
        A[i_0-1] = D[i_0] + D[i_0-1] - D[i_0-1] + E[i_0] + 1;
    }
    for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            for (int i_4 = 2; i_4 < -i_3+PB_M; i_4++) {
                for (int i_5 = 0; i_5 < PB_P-2; i_5++) {
                    C[i_5][i_3][i_2] = F[i_4][i_5][i_3][i_2] * G[i_4+i_3][i_2][i_5][i_2+2] + H[i_2][i_3+2][i_4] + C[i_4-2][i_3][i_5+2] * 4;
                }
            }
        }
    }
...
kernel_4326232125_01(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
