...
void kernel_4324312211_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
        ...
        for (int i_3 = 0; i_3 < min(PB_L, i_2); i_3++) {
            for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
                for (int i_5 = 0; i_5 < PB_L; i_5++) {
                    C[i_2-1][i_4+1][i_3][i_5] = C[i_2-2][i_4+1][i_5][i_3] * 1;
                }
            }
        }
    }
...
kernel_4324312211_08(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
