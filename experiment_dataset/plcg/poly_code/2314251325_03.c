...
void kernel_2314251325_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        B[i_0] = D[i_0] * E[i_0] + D[i_0+1] + D[i_0] * F[i_0+1][i_0] - E[i_0-1] + C[i_0+1] + 5;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < min(min(-i_1+PB_N, PB_P-1), i_1); i_2++) {
            B[i_2] = E[i_2] * D[i_2+i_1] * G[i_2][i_1] - G[i_2][i_2+1] - H[i_1-1][i_2][i_1] - C[i_2] * 3;
        }
        C[i_1] = D[i_1] * D[i_1] + B[i_1-2] * B[i_1] + B[i_1-1] + C[i_1-2] - 1;
    }
#pragma endscop
...
kernel_2314251325_03(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
