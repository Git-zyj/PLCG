...
void kernel_2426251225_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        A[i_0] = E[i_0] + E[i_0] + E[i_0] - F[i_0+1] - E[i_0] * E[i_0] + A[i_0+2] - 5;
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_L-1); i_1++) {
            B[i_0][i_1] = G[i_1][i_0] - H[i_0][i_1] + G[i_1+i_0+1][i_1-1] - G[i_1][i_0] * F[i_1] + H[i_1+1][i_0] - B[i_1][i_0+2] + B[i_0-1][i_1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
            A[i_3-2] = H[i_3][i_2-1] - H[i_3+1][i_2] + G[i_3][i_2] * H[i_3][i_2-1] * 4;
        }
        A[i_2] = F[i_2-1] + F[i_2] * F[i_2] - 3;
    }
#pragma endscop
...
kernel_2426251225_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
