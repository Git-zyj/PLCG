...
void kernel_4314312315_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_0-2][i_2][i_1] = A[i_0-2][i_2-1][i_1] * B[i_1+1][i_0-1] + 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            B[i_4][i_3] = D[i_4][0] - B[i_4][i_3+1] + B[i_4][i_3-1] * B[i_4-1][i_3] * 3;
        }
        for (int i_5 = 0; i_5 < min(PB_Q-2, i_3); i_5++) {
            for (int i_6 = 1; i_6 < PB_Q-1; i_6++) {
                for (int i_7 = 0; i_7 < -i_6+PB_Q-1; i_7++) {
                    C[i_3+2][i_5+1][i_3][i_7+i_6] = C[i_3+1][i_5+2][i_3-1][i_7+i_6+1] * C[i_3+2][i_5+2][i_3][i_7+i_6-1] + C[i_3+2][i_5+2][i_3][i_7+i_6] + 5;
                }
            }
        }
    }
...
kernel_4314312315_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
