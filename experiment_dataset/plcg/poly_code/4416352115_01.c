...
void kernel_4416352115_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_L-2, i_1); i_3++) {
                    A[i_1+2][i_3][i_2][i_3+1] = E[i_1][i_2+1][i_2-2][i_0-2] * F[i_1+i_0+2][i_2][i_3+i_0+2][i_1+2] + G[i_1][i_2][i_0][i_0] - G[i_3+2][i_0][i_2+2][i_1-1] - G[i_2][i_0][i_1][i_3] - H[i_2][i_1][i_3] + 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        D[i_4] = I[i_4] + J[i_4][1] + K[i_4] * K[i_4] * I[i_4+2] - D[i_4+1] + 2;
        for (int i_5 = 2; i_5 < min(i_4, -i_4+PB_M-2); i_5++) {
            C[i_4][i_5+i_4-1] = L[i_5-2][i_4-2] * I[i_4] - L[i_5+i_4+2][i_5] + J[i_5][i_4-1] - M[i_4][i_5-2][i_5-2] - 1;
        }
        D[i_4] = N[i_4] + N[i_4-2] * N[i_4] + N[i_4] * 4;
    }
#pragma endscop
