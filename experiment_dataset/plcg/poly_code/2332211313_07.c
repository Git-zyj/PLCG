...
void kernel_2332211313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            A[i_2] = C[i_2+1] - C[i_2] - 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        C[i_3+1] = D[i_3] * E[i_3][i_3] - C[i_3] - C[i_3+2] + C[i_3+2] * 6;
    }
#pragma endscop
...
kernel_2332211313_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
