...
void kernel_4232111215_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = min(-i_0+PB_Q, PB_Q-1) - 1; i_2 >= 2; i_2--) {
            for (int i_3 = 1; i_3 < min(min(PB_Q, -i_2+PB_Q-1), -i_0+PB_P); i_3++) {
                for (int i_4 = 0; i_4 < PB_L; i_4++) {
                    B[i_0][i_4+1][i_2-1][i_3] = C[i_4][i_3+i_0][i_2+i_0][i_3+i_2+1] + B[i_4][i_0][i_2-2][i_3-1] - B[i_0][i_2+1][i_4][i_3] + 3;
                }
            }
        }
    }
...
kernel_4232111215_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
