...
void kernel_3426252315_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_0][i_1] = E[i_1+i_0] - F[i_1][i_1] - G[i_1+i_0][i_0+2] + A[i_0][i_1-1] - A[i_0-1][i_1] + 2;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_L-2); i_2++) {
            C[i_2] = G[i_2+2][i_0] + H[i_2+i_0][i_0][0] * I[i_0-2][i_2+2][i_2] * G[i_0][i_2] - G[i_2+2][i_0] - E[1] + 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        C[i_3] = E[i_3] + E[i_3] + E[i_3] + J[i_3] + J[i_3+1] - E[i_3] + E[i_3+1] + C[i_3-1] + C[i_3+1] - C[i_3+1] - 2;
    }
    for (int i_4 = 1; i_4 < PB_M+1; i_4++) {
        C[i_4-1] = E[i_4+2] + E[i_4] * 4;
    }
#pragma endscop
...
kernel_3426252315_03(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
