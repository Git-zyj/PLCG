...
void kernel_3216111325_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 3; i_1 < PB_M-1; i_1++) {
            ...
            for (int i_3 = 1; i_3 < PB_M-3; i_3++) {
                B[i_3+1][i_1-1] = C[i_1][i_0-1][i_3-1] - B[i_1+1][i_3+1] - B[i_3-1][i_1-3] - B[i_3+3][i_1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3216111325_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
