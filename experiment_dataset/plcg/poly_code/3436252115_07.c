...
void kernel_3436252115_07(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        D[i_0] = E[i_0] + F[i_0+1] - F[i_0] * E[i_0] + E[0] - F[i_0-2] - F[i_0] + F[i_0-2] + 3;
        for (int i_1 = i_0; i_1 < -i_0+PB_N; i_1++) {
            for (int i_2 = 2; i_2 < PB_N; i_2++) {
                B[i_0][i_1][i_2-2] = G[i_2+2][i_1+1] * H[i_2][i_1+i_0][i_2] * I[i_1+i_0][i_1+1][i_2][i_2] + B[i_0][i_1-1][i_2-2] - 3;
            }
        }
        for (int i_3 = i_0; i_3 < -i_0+PB_M+1; i_3++) {
            D[i_3-1] = G[i_3+i_0-1][i_0] + J[i_3][i_0] * F[i_3] + D[i_3-1] + 1;
        }
        D[i_0] = F[i_0] * E[i_0] * E[i_0] + E[i_0-2] - E[i_0] * 5;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3436252115_07(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
