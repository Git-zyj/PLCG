...
void kernel_3412111223_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
    ...
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                B[i_1-1][i_2][0] = B[i_1+1][i_2+2][0] * 5;
            }
        }
        for (int i_3 = 3; i_3 < PB_Q-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                C[i_0-1][i_0][i_3-1] = F[i_0][i_4][i_3+1] + B[i_3-3][i_4][0] - C[i_0-2][i_0-1][i_3-1] - 1;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = 3; i_6 < PB_L; i_6++) {
            B[i_5][i_6][0] = G[i_6][i_6] * C[i_6-3][i_6-2][i_5-1] - 3;
        }
    }
#pragma endscop
...
kernel_3412111223_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
