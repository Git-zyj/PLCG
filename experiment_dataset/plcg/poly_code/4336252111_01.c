...
void kernel_4336252111_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M-2, i_0); i_1++) {
            for (int i_2 = i_0; i_2 < PB_M; i_2++) {
                B[i_1+i_0+2][i_0-2] = D[i_0][i_1-2][i_2] + E[i_2][i_2][i_1+i_0][0] * D[i_2][i_1][0] + F[i_2][i_0-2][i_1-2] * F[i_2][i_0][i_1] + 1;
            }
        }
        for (int i_3 = i_0; i_3 < -i_0+PB_M-3; i_3++) {
            B[i_3+i_0+2][i_3-2] = G[i_3][i_0] - H[i_3][i_0-2] + G[i_3+i_0][i_0-2] + B[i_3+i_0+3][i_3-1] * 4;
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        C[i_4] = I[i_4] * I[i_4] - I[i_4] * J[i_4] * I[i_4] * I[i_4] * J[i_4+1] * 3;
    }
#pragma endscop
...
kernel_4336252111_01(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
