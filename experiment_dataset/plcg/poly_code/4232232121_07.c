...
void kernel_4232232121_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_Q-1, -i_0+PB_L); i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                A[i_2][i_1][i_2+i_1+1] = C[i_2][i_0][i_1+i_0] + D[i_1][i_2][i_0-2] + A[i_2+1][i_1-1][i_2+i_1+1] + 3;
            }
        }
        for (int i_3 = 2; i_3 < min(PB_M, -i_0+PB_L+1); i_3++) {
            for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
                B[i_3-2][i_0][i_4+2] = C[i_3][0][i_0+1] + D[i_4][i_0][i_3] + C[i_0][i_4][i_3] - D[i_0][i_4][i_3+i_0-1] * B[i_3][i_4+2][i_0+2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4232232121_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
