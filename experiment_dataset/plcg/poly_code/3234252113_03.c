...
void kernel_3234252113_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_N; i_1++) {
            A[i_0][i_1-2] = C[i_0+1][i_1] * C[i_1+i_0][0] * C[i_0][i_1] - C[i_1+2][i_0] - D[i_1][i_0] * 1;
        }
        for (int i_2 = 2; i_2 < PB_N; i_2++) {
            B[i_2-2][i_0] = D[i_2+1][i_0] * D[i_2][i_0] - D[i_2+1][i_0+2] + C[i_2][i_0-2] - B[i_2-2][i_0+1] * B[i_2-2][i_0-1] + 4;
        }
    }
#pragma endscop
...
kernel_3234252113_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
