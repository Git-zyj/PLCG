...
void kernel_4424232215_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
            B[i_1] = E[i_0][i_0] - E[i_0+2][i_1+2] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        ...
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            for (int i_4 = 2; i_4 < -i_2+PB_L; i_4++) {
                for (int i_5 = max(3, i_2); i_5 < PB_L-1; i_5++) {
                    C[i_2][i_4-2][i_5-2][i_3] = G[i_3-2][i_4+i_2][i_5][0] + H[i_4][i_5-2][i_3] + I[i_2][i_5+2][i_2][i_3] - G[i_3][i_2-1][i_4][i_5] * G[i_4][i_5+1][i_5][i_2+2] * C[i_2+1][i_4-2][i_3-2][i_5] * C[i_4+1][i_2-2][i_5-3][i_3] * 4;
                }
            }
        }
    }
        ...
        for (int i_7 = 2; i_7 < PB_Q-1; i_7++) {
            for (int i_8 = 1; i_8 < PB_Q-2; i_8++) {
                for (int i_9 = 2; i_9 < PB_L-1; i_9++) {
                    C[i_6+1][i_9-2][i_8-1][i_7+1] = I[i_7-2][i_6][i_9+1][i_8+2] - 3;
                }
            }
        }
    }
...
init_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4424232215_08(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
