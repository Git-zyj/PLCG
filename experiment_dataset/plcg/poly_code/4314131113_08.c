...
void kernel_4314131113_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_1+PB_P, -i_2+PB_L-1); i_3++) {
                    A[i_3][i_1-1][i_2-1][i_2] = D[i_0][i_2][i_3+i_1] - E[i_0-1][i_3+i_2+1][i_1][i_2-1] - D[i_2+1][i_3][i_0] - E[i_3][i_2][i_3][i_1] * 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_M-2); i_5++) {
            B[i_5+i_4+1][i_5-1] = F[i_4][i_5+i_4-1] + F[i_5][i_4-1] * C[i_5-1][i_5] * 3;
            C[i_5-1][i_5] = F[i_4+1][i_5] + F[i_5+1][i_4-1] + B[i_5+i_4+2][i_5] - 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4314131113_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F)));
