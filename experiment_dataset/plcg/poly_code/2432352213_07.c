...
void kernel_2432352213_07(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            D[i_1-2] = E[i_0][i_1] - F[i_0] + G[i_0][i_1] + H[i_0-2][i_1-2] * E[i_1][i_0] + 1;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            D[i_2-2] = G[i_2][i_3] + D[i_3-1] - 3;
        }
        C[i_2-1] = G[i_2-2][i_2-1] - I[i_2+1] * I[i_2] - F[i_2] * F[i_2] - 1;
        D[i_2-2] = I[i_2] * I[i_2] - F[i_2] + J[i_2] - J[0] * J[i_2] * I[i_2] - C[i_2-2] + C[i_2] * D[i_2-3] + 1;
    }
#pragma endscop
...
kernel_2432352213_07(xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
