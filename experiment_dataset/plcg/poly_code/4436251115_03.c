...
void kernel_4436251115_03(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_M-1; i_3++) {
                    D[i_1+i_0-1][i_3][i_0-1][i_2+1] = E[i_2][i_3][i_1][i_0] + F[i_1+1][i_3][i_0][i_2-1] + E[i_1][i_0][i_3][i_1] * G[i_3+i_2+1][i_1+i_0][i_0][i_3-1][1] * H[i_3][i_1-1][i_2] * G[i_0][i_2][i_1][i_1][1] * 2;
                }
            }
        }
        for (int i_4 = i_0; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                B[i_0-1][i_4][i_5-1] = H[i_0+1][i_4][i_5+1] * I[i_5][i_4] * H[i_0][i_5-1][i_4] * H[i_4+1][i_0+1][i_0+1] + J[i_4][i_5][i_0] + 2;
            }
        }
        for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
            for (int i_7 = i_0; i_7 < PB_M; i_7++) {
                for (int i_8 = 1; i_8 < min(min(i_6, -i_6+PB_M-1), -i_0+PB_Q+1); i_8++) {
                    B[i_7-1][i_8-1][i_6-1] = H[i_7][i_6][i_7] + H[i_7-1][i_8][i_6] + F[i_8+i_6+1][i_0-1][i_7+1][i_0] + F[i_7-1][i_0][i_6][i_8+i_0-1] * 6;
                }
            }
        }
    }
    for (int i_9 = 1; i_9 < PB_Q-1; i_9++) {
        for (int j_0 = 0; j_0 < PB_L-1; j_0++) {
            for (int j_1 = max(j_0, 1); j_1 < PB_P-1; j_1++) {
                for (int j_2 = i_9; j_2 < -j_0+PB_L; j_2++) {
                    D[j_2+j_0][j_1][j_2][i_9+1] = F[j_2][j_0+1][j_1+1][i_9] - G[i_9-1][j_0][j_1-1][j_2][1] - K[j_2][j_2][j_1+1][i_9-1][0] + D[j_2+j_0][j_1+1][j_2][i_9+1] - 3;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4436251115_03(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,wk,POLYBENCH_ARRAY(K)));
