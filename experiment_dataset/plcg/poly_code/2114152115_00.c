...
void kernel_2114152115_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-2); i_1++) {
            A[i_1][i_0-1][i_0-1] = B[i_1] - B[i_1+2] + C[i_0+1][i_1] * C[i_0][i_1] - C[i_0-2][i_1+2] - A[i_0-1][i_1][i_1] + 4;
        }
    }
#pragma endscop
...
kernel_2114152115_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
