...
void kernel_4224112111_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1][i_0] = A[i_1-1][i_0-1] + 6;
        }
    }
        ...
        for (int i_3 = max(1, i_2); i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_M-2, i_3); i_4++) {
                for (int i_5 = 1; i_5 < PB_Q-2; i_5++) {
                    B[i_2+2][i_4][i_5+2][i_3] = C[i_3][i_4][i_5][i_2] + C[i_5-1][i_2][i_3][i_4] - B[i_4+2][i_5][i_2+2][i_3-1] * 3;
                }
            }
        }
    }
...
kernel_4224112111_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
