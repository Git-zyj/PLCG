...
void kernel_3132231225_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                A[i_2][i_1+1][i_0] = B[i_0+1][i_2][i_1+1] - C[i_1+i_0][i_2][i_1][i_0] + D[i_2+1][i_0][i_1+i_0-1] - A[i_0+2][i_2+1][i_1-1] - A[i_0-2][i_2+2][i_1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3132231225_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
