...
void kernel_2424252225_05(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            D[i_1] = E[i_0+1] - F[i_1][i_0+1] + G[i_1][i_0] * F[i_1][i_0-2] - F[i_0][i_1] + H[1] + F[i_1][i_0] * G[i_0][i_1-1] + 5;
        }
        B[i_0] = H[i_0+2] + 5;
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(min(PB_P-1, i_2), -i_2+PB_M); i_3++) {
            D[i_3+2] = G[i_2][2] + G[i_3+i_2][0] - I[i_3][i_2][i_3+1] * E[i_3] + G[i_3][i_2] + D[i_3+2] + 3;
        }
    }
    for (int i_4 = 1; i_4 < PB_P; i_4++) {
        for (int i_5 = 2; i_5 < PB_M-4; i_5++) {
            D[i_5+2] = G[i_5+2][i_4] * F[i_5-2][i_5] - F[i_5][i_4] - F[i_5-2][i_4-1] * D[i_5+4] - D[i_5] * 3;
        }
    }
#pragma endscop
...
kernel_2424252225_05(xd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
