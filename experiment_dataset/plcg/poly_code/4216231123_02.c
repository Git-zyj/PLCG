...
void kernel_4216231123_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(min(i_0, -i_0+PB_P), PB_M-2); i_1++) {
            for (int i_2 = max(1, i_0); i_2 < -i_1+PB_M-1; i_2++) {
                A[i_2][i_2] = C[i_2+1][i_0+1][0] * C[i_0][i_1+1][i_2] - D[i_2+i_1+1][i_0] - C[i_2][i_1+i_0][i_2+1] * E[i_1][i_2-1][i_0] - A[i_1+2][i_1+2] - 1;
            }
        }
    }
...
kernel_4216231123_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
