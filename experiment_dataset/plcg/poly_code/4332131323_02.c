...
void kernel_4332131323_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = 0; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_0+PB_L, -i_2+PB_P); i_3++) {
                    A[i_3][i_1][0][i_0-1] = D[i_1+1][i_3+i_0][i_3+i_2][i_2+1] + D[i_2][i_0][i_1+1][i_3-1] * A[i_3+1][i_1][0][i_0-1] - 6;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        B[i_4] = E[i_4-1] - B[i_4-1] * B[i_4+2] - B[i_4+1] + B[i_4-2] + B[i_4+1] * 4;
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            for (int i_6 = 2; i_6 < -i_4+PB_P; i_6++) {
                for (int i_7 = 1; i_7 < min(-i_5+PB_P, i_5); i_7++) {
                    A[i_4][i_6-2][0][i_7-1] = D[i_7-1][i_6+1][i_7-1][i_4] * F[i_6][i_7][i_7+i_5] * D[i_7][i_5][i_6+i_4][i_6] - D[i_4+1][i_6+1][i_7+i_5][i_6] - D[i_5][i_7+1][i_5-1][i_4-1] * 4;
                }
            }
        }
    }
...
kernel_4332131323_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
