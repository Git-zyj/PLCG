...
void kernel_3222211215_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            for (int i_3 = 1; i_3 < PB_N-1; i_3++) {
                B[i_1][i_2][i_3] = C[i_3+1][i_2-1] + B[i_1][i_2][i_3-1] + B[i_1][i_2-1][i_3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3222211215_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
