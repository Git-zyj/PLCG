...
void kernel_3222132115_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q+1, PB_P+2); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_Q, PB_P); i_2++) {
                A[i_2][i_1][i_2+i_0] = C[i_1][i_0][i_2] + C[i_2][i_0+1][i_1-2] - C[i_1][i_2][i_0] - A[i_1][i_2][i_1+i_0-1] * 5;
            }
        }
        for (int i_3 = 2; i_3 < i_0; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_P, -i_3+PB_Q); i_4++) {
                A[i_4][i_0][i_4+i_3] = C[i_3][i_0+2][i_4] + C[i_4][i_0][i_3-2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3222132115_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
