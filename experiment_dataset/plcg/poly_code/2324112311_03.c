...
void kernel_2324112311_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            B[i_2] = D[i_1+2][1] + B[i_2+1] - B[i_2-1] * C[i_2][i_2] + 3;
            C[i_1][i_1] = B[i_2] + B[i_2+1] - 5;
        }
    }
#pragma endscop
...
kernel_2324112311_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
