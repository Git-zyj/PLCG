...
void kernel_3216112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        ...
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            for (int i_3 = -i_2+PB_N - 1; i_3 >= 0; i_3--) {
                B[i_3+i_2][i_0] = D[i_2][i_3][i_0-1] - B[i_3+i_2][i_0+2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3216112121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
