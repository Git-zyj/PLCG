...
void kernel_2316152113_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0] - D[i_0] - D[2] + D[i_0] - D[0] + D[i_0] + A[i_0-1] - 3;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M; i_2++) {
            B[i_1][i_2] = E[i_2][i_1] * D[i_2] * E[i_2][i_1] * E[i_2][i_1] * 4;
        }
        for (int i_3 = 2; i_3 < min(min(-i_1+PB_M, i_1), PB_L-2); i_3++) {
            A[i_3] = E[i_3][i_1] + E[i_1][i_3-2] + E[i_3-2][i_1] - E[i_3+i_1][i_3+2] - 5;
        }
    }
#pragma endscop
...
kernel_2316152113_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
