...
void kernel_4212312325_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_M, PB_P); i_2++) {
                B[i_2-2][i_0] = C[i_2+i_0][i_1][i_2] * 4;
            }
        }
    }
    for (int i_3 = 4; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            B[i_3-2][i_4] = B[i_3-4][i_4] + B[i_3][i_4-1] - B[i_3][i_4] + 1;
        }
    }
#pragma endscop
...
kernel_4212312325_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
