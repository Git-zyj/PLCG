...
void kernel_2422351215_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P, -i_0+PB_N-1); i_1++) {
            A[i_0][i_1+i_0] = E[i_1+i_0] - F[i_1-1][i_1-1] * G[i_1][i_0+1] + F[i_1][i_0+1] - E[i_0] - G[i_1+i_0-1][i_0] - F[i_0][i_1] - A[i_0+1][i_1+i_0+1] * A[i_0][i_1+i_0+1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_P-1; i_3++) {
            C[i_3] = H[i_2-1][i_3+1] + G[i_3][i_2+1] + 1;
        }
    }
    for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
        C[i_4] = I[i_4] + 6;
        for (int i_5 = 0; i_5 < -i_4+PB_P; i_5++) {
            C[i_4] = H[i_5+i_4][i_5+i_4] * G[i_5+i_4][0] + F[i_4-1][1] * G[i_5+1][i_4+1] - F[i_4][i_5+i_4] - H[i_4][0] - H[i_5][i_4] - C[i_5] * 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2422351215_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
