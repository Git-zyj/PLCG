...
void kernel_4426132213_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(min(PB_P-3, -i_0+PB_Q), i_0); i_1++) {
            for (int i_2 = 0; i_2 < min(min(PB_L-1, -i_1+PB_L), -i_0+PB_P); i_2++) {
                for (int i_3 = 1; i_3 < PB_Q-2; i_3++) {
                    A[i_2][i_1+2][i_3+1][i_1+i_0] = E[i_3-1][i_2+i_1][i_2+i_0][2] - E[i_2][i_1][i_3][i_0] - A[i_2][i_1+2][i_3+2][i_1+i_0] * A[i_2+1][i_1+3][i_0+1][i_3+i_1+1] * 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        B[i_4+2] = F[i_4+1] - F[i_4-2] - F[i_4] + B[i_4+1] + 5;
        for (int i_5 = i_4; i_5 < -i_4+PB_N-1; i_5++) {
            B[i_4+2] = G[i_5+1][i_4] + G[i_5][i_4-1] * H[i_5+i_4+1][i_5][i_4-2] + B[i_5+2] - 6;
            for (int i_6 = 2; i_6 < PB_L; i_6++) {
                for (int i_7 = 2; i_7 < -i_4+PB_N-2; i_7++) {
                    D[i_5][i_6][i_7+i_4][i_6] = H[i_6-2][i_7+i_4+2][2] * E[i_6][i_7+i_4][i_5][0] + E[i_6][i_4+2][i_7][i_5-2] - E[i_6-2][i_4][i_7-2][i_5] - A[i_6][i_4+2][i_5+1][i_7+i_4] - 4;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4426132213_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
