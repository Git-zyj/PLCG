...
void kernel_4424132111_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_1] = E[i_1] + F[i_1][i_0] * 4;
        }
        for (int i_2 = max(1, i_0); i_2 < PB_L; i_2++) {
            for (int i_3 = max(2, i_0); i_3 < min(-i_0+PB_M, PB_L-2); i_3++) {
                C[i_2-1][i_3] = G[i_2][i_3][0] + G[i_3+i_0][i_3+2][i_2] - F[i_3-2][i_0] * G[i_2][i_3][i_0] - G[i_3][i_0][0] + C[i_2][i_3+1] + 3;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            for (int i_6 = 2; i_6 < i_5; i_6++) {
                C[i_6-1][i_5] = F[i_6][0] + F[i_6][i_5] * G[i_6-2][i_4][i_6] * F[i_6][i_5] * G[i_4][i_6][i_5-1] + C[i_5][i_6-1] + C[i_5-1][i_6-1] + 3;
            }
        }
        for (int i_7 = 0; i_7 < min(i_4, PB_Q); i_7++) {
            for (int i_8 = 0; i_8 < PB_L; i_8++) {
                for (int i_9 = max(2, i_7); i_9 < PB_M+2; i_9++) {
                    D[i_4-1][i_9-2][i_8][i_7] = 6;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4424132111_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
