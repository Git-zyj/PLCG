...
void kernel_2322111323_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
            B[i_2][i_2+1] = D[i_2][i_1] * D[i_2][i_1+1] + A[i_1][i_1] + B[i_2+2][i_2+3] * B[i_2-2][i_2-1] * 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        C[i_3] = B[i_3][i_3+1] - C[i_3-2] - C[i_3+1] - 2;
    }
#pragma endscop
...
kernel_2322111323_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
