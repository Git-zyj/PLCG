...
void kernel_3432111323_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_1][0] = A[i_2+2][0] + A[i_1+2][0] + 3;
            }
        }
        B[i_0+1] = B[i_0] + 5;
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M-1; i_4++) {
            C[i_3][i_4+1] = A[i_3][0] * B[i_4+1] - C[i_3][i_4] * D[i_3] + 3;
        }
        D[i_3] = E[i_3-1] - E[i_3] * E[i_3-1] + E[i_3] * D[i_3-2] + D[i_3+1] + 6;
    }
#pragma endscop
...
kernel_3432111323_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
