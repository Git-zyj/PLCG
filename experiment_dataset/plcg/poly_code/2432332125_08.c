...
void kernel_2432332125_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[2] * F[i_0-1] * 2;
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            B[i_0-1][i_1-2] = G[i_1][i_0] - G[i_0+1][i_1] + G[i_1][i_0] * H[i_0][0] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        A[i_2] = I[i_2] + E[i_2-1] + E[i_2] + I[i_2] - 2;
    }
    for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
        D[i_3] = I[i_3] * E[i_3-2] - A[i_3] * 1;
    }
#pragma endscop
...
kernel_2432332125_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
