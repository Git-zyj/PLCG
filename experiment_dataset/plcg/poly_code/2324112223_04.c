...
void kernel_2324112223_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-3; i_1++) {
            A[i_0-2][i_1+1] = A[i_0-1][i_1+1] + A[i_0-3][i_1-1] * A[i_0-2][i_1+3] * 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
        B[i_2] = D[i_2] - B[i_2+2] + B[i_2-2] - 1;
    }
    for (int i_3 = 2; i_3 < PB_N+1; i_3++) {
        C[i_3-2][0] = C[i_3-1][0] + 2;
    }
#pragma endscop
...
kernel_2324112223_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
