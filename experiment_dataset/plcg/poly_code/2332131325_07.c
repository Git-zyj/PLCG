...
void kernel_2332131325_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0] = D[0] * D[i_0] + D[i_0] * D[i_0] - A[i_0-2] * A[i_0+1] - 6;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            B[i_1+1][i_2] = E[i_1+1][i_2] + E[i_2-1][i_1] - B[i_1][i_2] + B[i_1+1][i_2+2] + B[i_1-1][i_2+2] + 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_N; i_4++) {
            A[i_4] = E[i_4][i_3-1] + E[i_4][i_3-1] + E[i_4+1][i_3] - A[i_4] - 4;
        }
    }
#pragma endscop
...
kernel_2332131325_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
