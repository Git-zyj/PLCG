...
void kernel_2434151313_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P, -i_0+PB_L-1); i_1++) {
            A[i_1] = E[i_1+i_0+1][1] - E[i_1][i_0] * E[0][i_1-1] * A[i_1+1] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[i_3] = F[i_2+1] * E[i_3+1][i_2] - E[i_3-1][i_2] * E[i_3-1][i_2+1] - E[i_3][i_2-1] + A[i_3] * B[i_3+1] * B[i_3-1] - 1;
        }
        C[i_2] = F[i_2] * F[i_2] * F[i_2] + F[i_2-1] - F[i_2] - B[i_2] * C[i_2-1] + C[i_2+1] + 2;
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        for (int i_5 = 1; i_5 < min(PB_P-1, -i_4+PB_L); i_5++) {
            D[i_4][i_5+i_4] = E[i_5][i_4] * E[i_5][i_4+1] * E[i_5][i_4-1] * E[i_4][i_5+1] - A[i_5-1] * D[i_4-1][i_5+i_4-1] + 1;
        }
    }
#pragma endscop
...
kernel_2434151313_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
