...
void kernel_2316352323_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            A[i_1] = D[i_1] * E[i_1+2] + A[i_1+2] - A[i_1-1] + A[i_1-2] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_M+1, i_2); i_3++) {
            A[i_3] = F[i_2-2][0] - F[i_3][i_2] + F[i_3][i_2] * G[i_2][i_3] - H[i_3+i_2-1][i_2-1] + D[i_3] - 5;
        }
        A[i_2] = I[i_2+1] + E[i_2] * E[i_2] + D[i_2] * D[i_2-1] * D[i_2] - 4;
    }
#pragma endscop
...
kernel_2316352323_09(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
