...
void kernel_4434252311_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - E[i_0+1] - F[i_0-1] * A[i_0+1] + A[i_0-1] - A[i_0-1] + 3;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
                for (int i_4 = 2; i_4 < min(min(-i_3+PB_P+2, -i_2+PB_Q), PB_L-2); i_4++) {
                    B[i_2][i_3+2][i_4+1][i_1] = G[i_3+2][i_1][i_4+i_2][i_1] + H[i_2][i_4-2][i_1][i_3] - H[i_3][i_4+2][i_1][i_2] + I[i_4][i_2][i_3+1] * J[i_1][i_3][i_2] + G[i_1][i_4+i_3-2][i_2][i_2] + H[i_3-1][i_2][i_1][0] - H[i_3-1][i_4][i_2][0] * H[i_1][i_2][i_4+i_3-2][2] - B[i_2-1][i_4+2][i_3][i_1] * B[i_2-1][i_3+1][i_4][i_1-1] + 5;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_N-2; i_5++) {
        for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
            for (int i_7 = i_5; i_7 < min(-i_5+PB_P, PB_M-1); i_7++) {
                C[i_7][i_7] = K[0][i_6] * J[i_5+2][i_6][i_6] * I[i_6+2][i_7][i_7+i_5] + K[i_6][i_7] + C[i_7-1][i_7-1] * C[i_7+1][i_7+1] - 5;
            }
        }
        for (int i_8 = 2; i_8 < i_5; i_8++) {
            for (int i_9 = 2; i_9 < min(-i_5+PB_L, PB_L-2); i_9++) {
                for (int j_0 = 0; j_0 < PB_Q-3; j_0++) {
                    D[i_5][i_8+2][i_9][j_0+2] = G[i_9+2][j_0][i_8+1][i_5] * H[j_0+i_9-2][i_9+i_5][i_8-2][i_8] + H[j_0][i_5-1][i_8][i_9] * D[i_5][i_8+3][i_9-1][j_0+2] + D[i_5-1][i_8+2][i_9][j_0+3] + 2;
                }
            }
        }
    }
...
kernel_4434252311_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
