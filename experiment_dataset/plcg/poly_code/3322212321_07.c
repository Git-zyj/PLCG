...
void kernel_3322212321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-4; i_1++) {
            for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
                A[i_1+2][i_2-1] = A[i_1+4][i_2-1] * 4;
            }
        }
    ...
    for (int i_3 = 4; i_3 < PB_Q; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 3; i_5 < PB_Q+1; i_5++) {
                C[i_3-2][i_4][i_3] = D[i_5-2][i_4] * C[i_5-3][i_4][i_5-1] * C[i_3-3][i_4][i_3-1] - C[i_3-4][i_5+2][i_3-2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3322212321_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
