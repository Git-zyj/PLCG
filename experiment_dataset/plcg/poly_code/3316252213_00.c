...
void kernel_3316252213_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            A[i_0][i_1] = D[i_1][i_0] - E[0] * D[i_1][i_0] + A[i_0-1][i_1] * A[i_1-1][i_0] + A[i_0+1][i_1+1] + 3;
        }
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_M-2); i_3++) {
                B[i_3+i_2][i_0][i_2] = F[i_3+i_2+2][i_0][0] * G[i_0+1][i_2] - H[i_2][i_0-1][i_2-2] + F[i_3-1][i_0][i_2] * B[i_3+i_2-1][i_0][i_2-1] - B[i_3+i_2][i_0-1][i_2] - 1;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
        C[i_4+1] = I[i_4] + E[0] - I[i_4+1] - E[i_4] + I[i_4] - I[i_4] - I[0] * I[i_4] * C[i_4] * 1;
    }
#pragma endscop
...
kernel_3316252213_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
