...
void kernel_4234312211_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_P, i_0); i_2++) {
                for (int i_3 = 1; i_3 < min(-i_2+PB_Q, -i_1+PB_P-1); i_3++) {
                    A[i_2][i_3-1][i_1] = C[i_0][i_2][i_3+i_1+1][i_3+i_2] * A[i_1-1][i_3-1][i_2] - 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_P; i_5++) {
            for (int i_6 = 1; i_6 < min(i_4, -i_4+PB_P+1); i_6++) {
                B[i_5][i_4+1][i_6][i_5+i_4] = A[i_4][i_6-1][i_5] - B[i_4-1][i_6+1][i_5-1][i_6+i_4-1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4234312211_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
