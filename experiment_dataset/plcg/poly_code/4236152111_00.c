...
void kernel_4236152111_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < min(i_0, -i_1+PB_L); i_2++) {
                A[i_0-2][i_2+i_1][i_0-1] = C[i_2-1][i_0][i_1] + C[i_1+1][i_0][i_0+2] - C[i_0][i_2][i_1] * C[i_2-2][i_1][i_0] - C[i_0+1][i_2+i_1][i_1] + C[i_2][i_1][i_0] * A[i_0-2][i_2+i_1-2][i_0-1] + 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            B[i_3][i_4] = D[i_3+2][i_4] + D[i_4+1][i_3] * D[i_4][i_3+2] - B[i_3][i_4-1] - 3;
        }
    }
#pragma endscop
...
kernel_4236152111_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
