...
void kernel_4412111311_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L-1; i_2++) {
                A[i_2+i_0][i_1][i_1+i_0] = A[i_2+i_0-1][i_1][i_1+i_0] + A[i_1+i_0-2][i_2][i_2+i_0-1] + A[i_2+i_0+1][i_1-1][i_1+i_0] * A[i_2+i_0+1][i_1][i_1+i_0] + 4;
            }
        }
        B[i_0+1] = E[i_0-1] * B[i_0+2] - B[i_0] - 6;
        for (int i_3 = 0; i_3 < min(i_0, -i_0+PB_P); i_3++) {
            C[i_0][i_3+i_0] = F[i_3][i_0] * B[i_3+2] - B[i_3+1] - B[i_3] * C[i_0-1][i_3+i_0-1] * 5;
        }
    }
    for (int i_4 = 0; i_4 < PB_P; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 0; i_6 < min(-i_5+PB_Q, PB_L-1); i_6++) {
                for (int i_7 = 0; i_7 < -i_6+PB_Q-1; i_7++) {
                    D[i_7][i_4][i_6][i_6+i_5] = G[i_6+1][i_7][i_4][i_5+1] * D[i_5-1][i_4][i_7][i_7+i_6+1] + 1;
                }
            }
        }
    }
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4412111311_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
