...
void kernel_4226212323_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L+1; i_1++) {
            B[i_1+2] = C[i_1-1][i_0] + C[i_1-1][i_0] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-4; i_2++) {
        B[i_2+2] = B[i_2+3] * B[i_2+1] - B[i_2+4] + 6;
    }
#pragma endscop
...
kernel_4226212323_01(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
