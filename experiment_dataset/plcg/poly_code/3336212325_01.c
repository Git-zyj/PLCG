...
void kernel_3336212325_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L-2; i_1++) {
            A[i_0][i_1] = D[i_0][i_1+2] * A[i_0][i_1-2] + A[i_0-1][i_1] * A[i_1-2][i_0] * 4;
        }
    ...
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = i_4; i_5 < min(-i_3+PB_M, PB_L); i_5++) {
                C[i_3][i_4][i_3] = E[i_4][i_5+i_3][i_4] * D[i_5][i_4] * C[i_3][i_4+1][i_3] * C[i_3][i_4+2][i_3] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3336212325_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
