...
void kernel_2322112121_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_P+1; i_1++) {
            A[i_0-1][i_1-2] = D[i_0][i_0] * A[i_0-1][i_1-1] * 1;
        }
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            B[i_0][i_2] = D[i_2+2][i_0] - B[i_0][i_2-1] - 5;
        }
    }
...
kernel_2322112121_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
