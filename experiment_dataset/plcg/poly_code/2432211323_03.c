...
void kernel_2432211323_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            A[i_0-1][i_0] = A[i_0+1][i_0+2] * A[i_0-2][i_0-1] * B[i_0] - 6;
        }
        B[i_0] = E[i_0] - B[i_0-1] + B[i_0+1] * 3;
    }
    for (int i_2 = 3; i_2 < PB_L; i_2++) {
        C[i_2-1][i_2] = F[i_2-1] + F[i_2] - C[i_2-3][i_2-2] * 4;
    }
    for (int i_3 = 1; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 3; i_4 < i_3; i_4++) {
            D[i_4][i_4-1] = A[i_3-1][i_3] - A[i_3+1][i_3+2] + D[i_4-2][i_4-3] * D[i_4+1][i_4] - D[i_3+2][i_3+1] + 6;
        }
    }
#pragma endscop
...
kernel_2432211323_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
