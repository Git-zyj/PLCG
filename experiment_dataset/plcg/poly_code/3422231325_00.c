...
void kernel_3422231325_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0+1] + E[i_0] - F[0] * A[i_0+1] + 4;
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_1][i_0] = G[i_0][i_1-1] + G[i_0+1][i_1] * A[i_1] + A[i_0] * 4;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                B[i_3-1][i_0] = H[i_3][i_2][0] - I[i_2-1][i_0+1][i_2] * 3;
            }
            for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
                B[i_4][i_0] = I[i_4][i_0][i_2+1] * H[i_2+i_0][i_0+1][0] + H[i_0][i_4-1][i_2] - B[i_4+1][i_0] * B[i_4+2][i_2] - B[i_4-1][i_2-1] * 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3422231325_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
