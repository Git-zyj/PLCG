...
void kernel_4422151111_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-1] * E[i_0+1] * E[i_0-1] + E[i_0] * E[i_0] - A[i_0-1] * 1;
        B[i_0] = E[i_0+1] + E[i_0] * E[i_0+1] * E[i_0-1] - E[i_0] - E[i_0] * E[i_0] * 2;
    }
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
                D[i_1+2][i_2][i_2+1][i_3+1] = F[i_1][i_3-1] - 2;
            }
            for (int i_4 = 0; i_4 < min(-i_2+PB_L, -i_1+PB_P); i_4++) {
                for (int i_5 = 0; i_5 < min(-i_2+PB_L, PB_M); i_5++) {
                    D[i_1+1][i_2][i_2+1][i_5] = G[i_2][i_5+1][i_1] - H[i_1-1][i_5+i_2][i_4][i_1-1] - H[i_5][i_4+i_2][i_4+i_1][i_1+1] + H[i_2-1][i_5][i_1][i_2-1] * H[i_1][i_4][i_4][i_1] + 1;
                }
            }
        }
    }
...
kernel_4422151111_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
