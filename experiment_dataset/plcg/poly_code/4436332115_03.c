...
void kernel_4436332115_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_P-2, i_0); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_P+2, i_0); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_P+2, PB_L-2); i_3++) {
                    A[i_3][i_2+i_1-2][i_0-1][i_3] = E[i_3][i_2][i_1+2][i_0] + E[i_1][i_3+2][i_2][i_3] * A[i_2+1][i_3+i_0-2][i_1-1][i_2+1] + 2;
                }
            }
        }
        for (int i_4 = 3; i_4 < i_0; i_4++) {
            D[i_0+1][i_4-3] = F[i_4+1][i_0-1] * G[i_4][i_0+1] * 4;
        }
    }
        ...
        for (int i_6 = max(2, i_5); i_6 < -i_5+PB_M; i_6++) {
            for (int i_7 = 1; i_7 < i_6; i_7++) {
                for (int i_8 = 0; i_8 < -i_7+PB_P; i_8++) {
                    A[i_5][i_8+i_7][i_6-2][i_5] = H[i_6+i_5][i_7-1][i_6][i_8] * I[i_7][i_8][i_6-1][i_5] + E[i_5][i_8+1][i_6][i_5] + H[i_8+1][i_5][i_7][i_6] + I[i_8][i_6][i_5+2][i_5] * 4;
                }
            }
        }
        for (int i_9 = 2; i_9 < PB_M; i_9++) {
            for (int j_0 = i_5; j_0 < PB_L-2; j_0++) {
                D[j_0][i_9-2] = J[i_9-1][j_0+2][i_5-1] + K[i_9][j_0][i_5-1] - 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4436332115_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
