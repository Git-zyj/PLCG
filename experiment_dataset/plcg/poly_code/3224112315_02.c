...
void kernel_3224112315_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_0][i_1+i_0+1] = C[i_1+i_0+2] - A[i_0+1][i_1+i_0+2] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            for (int i_4 = i_3; i_4 < min(min(-i_2+PB_P-2, -i_3+PB_P-3), PB_L-1); i_4++) {
                A[i_3+1][i_4+i_3+3] = A[i_2][i_4+i_2+2] * A[i_4+1][i_4+i_3+2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3224112315_02(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
