...
void kernel_2236331225_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = C[i_0+1] - C[i_0] * C[i_0-1] + 5;
    }
    for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-3; i_2++) {
            A[i_2+1] = D[i_1][0] + D[i_1][i_2] - A[i_2+3] + A[i_1+3] * 4;
        }
    }
#pragma endscop
...
kernel_2236331225_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
