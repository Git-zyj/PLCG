...
void kernel_3316312123_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            C[i_4][i_5] = C[i_5+1][i_4] - C[i_4][i_5-1] * 1;
        }
    }
#pragma endscop
...
kernel_3316312123_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
