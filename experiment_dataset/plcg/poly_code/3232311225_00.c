...
void kernel_3232311225_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N+1; i_1++) {
            for (int i_2 = 2; i_2 < PB_N+2; i_2++) {
                A[i_1-1][0] = A[i_2-2][0] * 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N+1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 1; i_5 < min(i_4, PB_N); i_5++) {
                A[i_5-1][0] = C[i_5][i_3] + A[i_3-1][0] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3232311225_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
