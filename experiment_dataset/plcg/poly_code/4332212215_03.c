...
void kernel_4332212215_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                A[i_1][i_2+i_0-2][i_1] = D[i_1][i_2+i_0][0] - A[i_1-1][i_2+i_0-2][i_1-1] * 3;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
            for (int i_5 = i_4; i_5 < PB_P; i_5++) {
                for (int i_6 = 1; i_6 < PB_P-2; i_6++) {
                    B[i_3][i_4-1][i_5][i_6+2] = E[i_6][i_3][i_4][i_5] * B[i_6-1][i_4-1][i_5-1][i_3+2] * B[i_3][i_5-1][i_4+1][i_6+2] * 1;
                }
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_L-2; i_7++) {
        for (int i_8 = 0; i_8 < -i_7+PB_L-2; i_8++) {
            C[i_8+i_7+2][i_7-1] = C[i_8+i_7][i_7-2] - C[i_8+i_7+1][i_7-1] + C[i_8+i_7+1][i_7-2] - 5;
        }
    }
#pragma endscop
...
kernel_4332212215_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
