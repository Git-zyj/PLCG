...
void kernel_2226152123_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[i_0][i_1] = C[i_1][i_0] + D[i_1+i_0] + C[i_0][i_1] * C[i_0-2][i_1] - A[i_0][i_1-1] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_3][i_2][2] = C[i_2-1][i_3] * C[i_3][i_2-1] - D[i_2-2] * C[i_3][i_2] - D[i_2] - E[i_2-2][i_3][i_3] + B[i_3+1][i_2-2][2] - 3;
        }
    }
#pragma endscop
...
kernel_2226152123_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
