...
void kernel_2214331113_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_N; i_1++) {
            A[i_1-1] = C[i_1+i_0] + D[0] + A[i_1-2] * A[i_1] + 1;
        }
        for (int i_2 = 1; i_2 < PB_N; i_2++) {
            B[i_2] = E[i_2][i_2] * F[i_2-1][i_0+1] * E[i_2][i_0] + C[i_0] - 3;
        }
    }
#pragma endscop
...
kernel_2214331113_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
