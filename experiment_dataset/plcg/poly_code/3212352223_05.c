...
void kernel_3212352223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_N, PB_M-2); i_1++) {
            A[i_1+i_0-1][i_0+2] = C[i_1+1][i_0] - C[i_1+2][i_0-1] + D[i_0-2][i_1] - E[i_1] - C[i_1][i_0] * A[i_1+i_0-3][i_0+2] + A[i_1+i_0][i_0+2] * A[i_1+i_0-2][i_0+2] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_P; i_3++) {
            for (int i_4 = 3; i_4 < -i_3+PB_M; i_4++) {
                B[i_4-1][i_2][i_3+1] = F[i_4+i_3][i_2][i_2-1] + F[i_4+i_3][i_2][i_3+i_2] * G[i_4-1][i_3] * F[i_4][i_3+i_2][i_4] - D[i_3][i_4] * B[i_4-3][i_2][i_3+1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3212352223_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
