...
void kernel_2334352115_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0-1] - D[i_0] + D[i_0+1] + E[i_0] * A[i_0+1] * 5;
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            A[i_1] = F[i_1][i_0][i_1] * G[i_1][i_1+2] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_L-1); i_3++) {
            A[i_3-1] = H[i_3][i_2] + G[i_3][i_2] * H[i_3][i_2] - H[i_3][i_2+1] * H[i_2-1][i_3] * D[i_3+2] - H[i_2][i_3] - G[i_3+i_2+1][i_3] * 6;
        }
    }
#pragma endscop
...
kernel_2334352115_05(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
