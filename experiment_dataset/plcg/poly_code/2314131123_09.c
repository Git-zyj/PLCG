...
void kernel_2314131123_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        B[i_0+1][i_0+1] = D[i_0] * D[i_0-1] + C[i_0][i_0] * 6;
        B[i_0][i_0] = D[i_0] + D[i_0] - 4;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        C[i_1][i_1] = D[i_1] * D[i_1] * D[i_1] * C[i_1-2][i_1-2] - 2;
    }
#pragma endscop
...
kernel_2314131123_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
