...
void kernel_2326112221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 3; i_3 < i_2; i_3++) {
            B[i_2][i_3-2] = D[i_3][i_2+2] + B[i_2-1][i_3-2] * B[i_2+1][i_3-3] - B[i_2][i_3-1] * 5;
        }
    }
    for (int i_4 = 0; i_4 < PB_N-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-3; i_5++) {
            C[i_4][i_5+2] = C[i_4][i_5+1] - C[i_4+2][i_5+2] + C[i_4+1][i_5+3] * 1;
        }
    }
#pragma endscop
...
kernel_2326112221_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
