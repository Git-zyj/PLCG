...
void kernel_2234311323_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0][0] = A[i_1][i_0+2][0] - A[i_1+1][i_0][0] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-3; i_2++) {
        B[i_2+1] = C[i_2-1] * B[i_2-1] - B[i_2+2] + B[i_2+3] * 3;
    }
#pragma endscop
...
kernel_2234311323_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
