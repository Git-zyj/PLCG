...
void kernel_4326212223_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_L, -i_0+PB_L-1); i_2++) {
                B[i_0-3][i_2+i_1][i_1] = B[i_1-1][i_2+i_0+1][i_0+2] * 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_L+2; i_4++) {
            for (int i_5 = 0; i_5 < -i_3+PB_L-1; i_5++) {
                B[i_4-2][i_5+i_3-1][i_5] = D[i_5][i_3-1][i_4+i_3-2] + B[i_4][i_5+i_3+1][i_5+2] + 3;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_N; i_6++) {
        for (int i_7 = 3; i_7 < PB_L+1; i_7++) {
            C[i_6][i_7-1] = C[i_6-1][i_7-3] + 1;
        }
    }
#pragma endscop
...
kernel_4326212223_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
