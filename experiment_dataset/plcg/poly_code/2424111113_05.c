...
void kernel_2424111113_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_0] = E[i_1-1][i_0] + A[i_1+1] - C[i_1] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        ...
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            D[i_3+1][i_2] = E[i_2][i_3+1] - D[i_3+1][i_2-1] * 3;
        }
    }
#pragma endscop
...
kernel_2424111113_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
