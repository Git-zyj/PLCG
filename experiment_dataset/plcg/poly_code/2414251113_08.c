...
void kernel_2414251113_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            A[i_0-1][i_1] = E[i_1] - F[i_1][i_0] - F[i_1][i_0-1] - F[i_0][i_0] - F[i_1][i_0] + F[i_1-1][i_0] * 6;
        }
        C[i_0+1] = E[i_0] - G[i_0-1] + G[i_0+1] - C[i_0] - 6;
        for (int i_2 = 1; i_2 < min(min(PB_N-1, -i_0+PB_M-1), i_0); i_2++) {
            C[i_2+1] = G[i_2+i_0] * E[i_2+1] + H[i_2+i_0+1][i_2+i_0+1] + H[i_0][i_2] - F[i_2-1][i_0+1] + I[i_0][i_2][i_0] + C[i_2+2] + 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_N; i_3++) {
        for (int i_4 = 1; i_4 < PB_Q+1; i_4++) {
            D[i_3][i_4][i_4-1] = E[i_4] * E[i_3] + H[i_4-1][0] + H[i_4+1][i_3] - 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2414251113_08(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
