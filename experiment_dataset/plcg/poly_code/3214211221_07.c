...
void kernel_3214211221_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_Q, -i_0+PB_L); i_1++) {
            A[i_0][i_1+i_0-1] = 3;
            for (int i_2 = 2; i_2 < min(min(PB_Q, -i_0+PB_M+3), i_0); i_2++) {
                B[i_2][i_1][i_0] = C[i_2][i_1+i_0][i_0] + D[i_0+1][i_1][i_2] + A[i_1][i_2+i_1-3] + A[i_2-2][i_2+i_1-3] * A[i_2-2][i_2+i_0-3] * B[i_2][i_1-2][i_0] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3214211221_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
