...
void kernel_2316212123_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            B[i_1+1] = D[i_1+2][i_0-2] - B[i_1-1] + 3;
        }
        B[i_0-1] = 1;
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            C[i_3] = E[i_3-2][i_2+2] * D[i_3][i_2] * 5;
        }
    }
#pragma endscop
...
kernel_2316212123_05(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
