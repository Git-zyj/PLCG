...
void kernel_2432252215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        A[i_0] = E[i_0-2] * F[i_0+2] * F[i_0] * F[i_0] * A[i_0+1] + 1;
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            A[i_0+1] = E[i_1] * G[i_1+1][i_0] * 1;
            A[i_0] = G[i_1][i_0] - G[i_1+i_0][i_1] * H[i_1+i_0][i_0] - G[i_1][i_0] * G[i_1+1][i_0] * H[i_1][i_0] - H[i_0][2] * I[i_0][i_1][0] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_P, -i_2+PB_M); i_3++) {
            D[i_3][i_2] = G[i_3][i_2] * H[i_3][i_2] * E[i_3+i_2] - H[i_2][i_3] - F[i_3] * D[i_3+1][i_2-1] - D[i_3][i_2+1] * 1;
        }
    }
#pragma endscop
...
kernel_2432252215_02(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
