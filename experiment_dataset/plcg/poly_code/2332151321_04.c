...
void kernel_2332151321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = D[0] * D[0] - D[1] - D[i_0] - E[i_0+1][i_0+1] * D[i_0-1] * A[i_0+1] * A[i_0-3] + A[i_0-2] + A[i_0] * 2;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            B[i_1][i_2+1] = E[i_2][i_1] + B[i_1][i_2+2] - 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
            B[i_3][i_4+1] = E[i_4+i_3][i_3] * E[i_4][i_3] - E[i_3][i_4] + D[i_4+i_3-1] + E[i_3+1][i_4] + E[i_3][i_3] - 3;
        }
    }
#pragma endscop
...
kernel_2332151321_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
