...
void kernel_2214251325_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            A[i_1] = C[i_1-1][i_0] * D[i_1][i_0] - D[i_1+1][i_0] + C[i_1][i_0] * D[i_0-1][i_1] + C[i_0+1][i_1+1] - A[i_1-1] * A[i_1+1] + A[i_1+2] * 2;
            A[i_1] = C[i_0+1][i_1] + E[i_0] + C[i_1+1][i_0] + D[i_1-1][i_0] * 2;
        }
    }
#pragma endscop
...
kernel_2214251325_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
