...
void kernel_3234311325_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            A[i_1+1] = C[i_1+1] + A[i_1-1] + A[i_1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        A[i_2+1] = C[i_2] * A[i_2+1] + 4;
    }
#pragma endscop
...
kernel_3234311325_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
