...
void kernel_3224311321_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_Q; i_3++) {
            for (int i_4 = 1; i_4 < min(min(PB_Q-2, -i_3+PB_M), i_3); i_4++) {
                B[i_2][i_3][i_4] = C[i_4+i_3][i_2][i_3] * C[i_3][i_3][i_4] * B[i_4-1][i_2][i_3] - B[i_2][i_3][i_4+2] - B[i_2+2][i_3][i_4] + B[i_2+2][i_4][i_3] - B[i_2-2][i_3-1][i_4-1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3224311321_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
