...
void kernel_4434251125_05(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-3; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, -i_0+PB_L-1); i_2++) {
                for (int i_3 = 2; i_3 < min(-i_1+PB_M, -i_0+PB_M+1); i_3++) {
                    D[i_1][i_0+1][i_2+1][i_3-2] = E[i_3][i_2][i_0][i_1-1] * E[i_3+i_1][i_2+i_0+1][i_3+i_1][0] * F[i_3][i_1][i_2] + E[i_3+i_0-1][i_2][i_1][i_3] + E[i_0][i_3][i_2+1][i_1] - D[i_0][i_2+1][i_1+3][i_3+1] * 2;
                }
            }
        }
        for (int i_4 = 0; i_4 < -i_0+PB_L-3; i_4++) {
            B[i_4+i_0+1][i_0] = G[i_4][i_0] - H[i_4][i_0-1] * G[i_0][i_4] * G[i_4][i_0] + G[i_4+1][i_0-1] + H[i_4+1][i_0] * B[i_4+i_0+3][i_4+1] + 1;
        }
    }
    for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 0; i_6 < min(i_5, -i_5+PB_L-1); i_6++) {
            B[i_6+i_5+1][i_5] = G[i_5][i_6] - I[0] * 3;
        }
    }
    for (int i_7 = 1; i_7 < PB_L; i_7++) {
        for (int i_8 = 0; i_8 < min(-i_7+PB_P+1, PB_L-1); i_8++) {
            for (int i_9 = 0; i_9 < min(PB_L-1, -i_7+PB_M+1); i_9++) {
                for (int j_0 = 0; j_0 < i_7; j_0++) {
                    D[i_7][i_8+1][i_9+1][j_0] = J[i_9][j_0][i_8][i_8] + K[j_0][i_8+i_7-1][i_9] + J[j_0][i_9+i_7-1][i_8][i_8] + J[i_9][j_0][i_8+1][i_7] * 2;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4434251125_05(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
