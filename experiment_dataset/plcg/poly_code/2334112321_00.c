...
void kernel_2334112321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-4; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_L+2, -i_0+PB_M-4); i_1++) {
            A[i_1+i_0][i_1] = D[i_1+2][i_1-2] * A[i_1+i_0+4][i_1+2] * A[i_1+i_0-3][i_1-1] - 3;
        }
        for (int i_2 = 2; i_2 < -i_0+PB_M-1; i_2++) {
            B[i_0][i_2] = A[i_2+i_0+1][i_2+1] - B[i_0+2][i_2+1] + B[i_2-2][i_0] + 5;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            C[i_4-2] = B[i_3][i_4] - C[i_4] * C[i_4-1] - 2;
        }
    }
#pragma endscop
...
kernel_2334112321_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
