...
void kernel_3232311225_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        ...
        for (int i_3 = 3; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_N; i_4++) {
                B[i_3][i_0][i_3-1] = C[i_4][i_3][i_0] - B[i_3-2][i_0][i_3-3] + B[i_3+2][i_0-1][i_3+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3232311225_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
