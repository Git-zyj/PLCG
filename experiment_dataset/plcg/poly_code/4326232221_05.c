...
void kernel_4326232221_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
        ...
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_P; i_2++) {
                for (int i_3 = 2; i_3 < -i_0+PB_M; i_3++) {
                    A[i_1][i_3+i_0][i_3-1][i_2] = D[i_1][i_0][i_2-1][0] - E[i_3-2][i_2-2][i_0][i_3-2] + 4;
                }
            }
        }
        B[i_0] = F[1] * G[0] * G[i_0] - B[i_0-1] - B[i_0-2] * B[i_0+2] - 4;
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-1; i_5++) {
            B[i_5+1] = H[i_5][i_4-2] + H[0][i_4-1] * 1;
        }
    }
#pragma endscop
...
kernel_4326232221_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
