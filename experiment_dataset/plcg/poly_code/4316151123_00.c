...
void kernel_4316151123_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(min(PB_Q-1, -i_0+PB_L), i_0); i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                    A[i_0][i_2][i_1+i_0][i_3+1] = D[i_2-1][i_0][i_0][i_1] * D[i_0][i_3][i_2-1][i_1] * D[i_1][i_2+i_0+1][i_3+1][i_2] - D[i_2][i_3][i_0][i_1+1] + A[i_0][i_2-2][i_1+i_0][i_3+1] * 4;
                }
            }
        }
        B[i_0] = E[i_0] - E[i_0] - F[i_0][i_0] - E[i_0-1] + E[i_0-1] - 1;
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 0; i_6 < min(PB_P-2, -i_5+PB_Q); i_6++) {
                for (int i_7 = 2; i_7 < PB_P; i_7++) {
                    C[i_4][i_6+1][i_6][i_5+1] = G[i_5][i_7][i_6] - D[i_7-1][i_6][i_5][i_5] * D[i_4][i_6+1][i_7-1][i_6+i_5] * D[i_7-1][i_5][i_6][i_4] - D[i_6+i_5][i_4+1][i_7][0] - C[i_4][i_6+2][i_6+1][i_5+1] * C[i_6][i_7-1][i_7-2][i_5+1] + 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4316151123_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
