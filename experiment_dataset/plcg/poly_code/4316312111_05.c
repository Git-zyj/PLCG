...
void kernel_4316312111_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_L+1; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            B[i_3][i_4-1] = D[i_3+2][i_4+2] + E[i_3] * B[i_4-1][i_3-1] - B[i_4][i_3-2] + 4;
        }
        C[i_3-1] = F[1] - 6;
    }
#pragma endscop
...
kernel_4316312111_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
