...
void kernel_3224312221_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_P-3, PB_L-2); i_2++) {
                A[i_0][i_2][i_2+i_1+1] = C[i_2+i_1][i_0-2][i_2] + D[i_2+2][i_1+i_0][1] + A[i_0+1][i_2+2][i_2+i_1+3] - A[i_0][i_2-1][i_2+i_1] * A[i_0+1][i_2][i_2+i_1] + 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 2; i_5 < PB_P; i_5++) {
                B[i_4+1][i_5][i_4] = B[i_4+2][i_5-2][i_4+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3224312221_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
