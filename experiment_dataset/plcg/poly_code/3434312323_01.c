...
void kernel_3434312323_01(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        C[i_0] = 4;
        C[i_0-1] = E[i_0+1] * C[i_0] * C[i_0+1] * C[i_0-1] * 4;
        C[i_0] = F[i_0] * 1;
    }
    for (int i_1 = PB_M-2 - 1; i_1 >= 0; i_1--) {
        for (int i_2 = 2; i_2 < PB_M+1; i_2++) {
            for (int i_3 = 2; i_3 < i_1; i_3++) {
                D[i_1][i_2+2][i_3] = G[i_2-2][i_3] * H[i_2-1][i_1][i_3+2] + D[i_1+2][i_2+2][i_3-2] + D[i_3-2][i_2+2][i_1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3434312323_01(xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
