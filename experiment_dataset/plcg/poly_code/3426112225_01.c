...
void kernel_3426112225_01(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_L-2; i_2++) {
            B[i_2-2][i_2+i_1+2] = E[i_2+1] + B[i_2-2][i_2+i_1+1] + B[i_1-2][i_2+i_1+1] - 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        C[i_3] = 6;
        C[i_3-1] = E[i_3] + E[i_3] * C[i_3] * C[i_3+1] + 2;
    }
#pragma endscop
...
kernel_3426112225_01(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E));
