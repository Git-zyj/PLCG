...
void kernel_3326112325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_N-4; i_2++) {
        for (int i_3 = 0; i_3 < PB_N; i_3++) {
            for (int i_4 = i_2; i_4 < PB_N-4; i_4++) {
                C[i_2][i_4+2] = C[i_4][i_2+4] * C[i_3][i_4+4] * C[i_2][i_4+4] + C[i_2][i_4+3] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3326112325_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
