...
#endif
static void init_array(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int seed)
{
srand(seed);
int i_0, i_1, i_2, i_3;
...
static void print_array(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4424351323_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < min(PB_L-3, -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L-3; i_2++) {
                for (int i_3 = max(1, i_2); i_3 < PB_L; i_3++) {
                    B[i_1+2][i_0][i_2+i_1+3] = E[i_1][i_3][i_1+i_0][i_0] + F[i_0][i_1-1][i_3][i_2] + G[i_2][i_3][i_0][i_1+1] * G[i_3-1][i_3][i_1][i_0-1] * F[0][i_1][i_3][i_2] * 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N-3; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_L-2); i_5++) {
            for (int i_6 = 0; i_6 < -i_4+PB_N-1; i_6++) {
                B[i_4+1][i_6][i_5+i_4+1] = H[i_5-1][i_4-1][i_5-1] + I[i_6+i_4+1][i_5][i_6] + H[i_4][i_6+i_5][i_6+i_4-1] + J[i_5][i_4+1][i_4-1] * G[i_6][i_5+1][i_4][i_4] * B[i_5+1][i_4+1][i_6+i_5+1] * B[i_4+2][i_6+2][i_5+i_4+2] + 6;
            }
        }
        C[i_4+1] = K[i_4+1] - L[i_4] - M[i_4-1] * M[i_4] - L[i_4] + K[i_4-1] + C[i_4-1] + C[i_4+3] - C[i_4+2] * C[i_4] * 3;
    }
    for (int i_7 = 0; i_7 < PB_P; i_7++) {
        for (int i_8 = i_7; i_8 < PB_L; i_8++) {
            for (int i_9 = 0; i_9 < min(-i_8+PB_L, PB_L-1); i_9++) {
                for (int j_0 = max(i_8, 1); j_0 < PB_L; j_0++) {
                    B[i_8][j_0][i_9+i_8] = G[j_0-1][i_7][i_8][i_9] + J[j_0][i_8][i_9+1] + 6;
                }
            }
        }
    }
