...
void kernel_3436152321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] * E[i_0] - E[i_0] + E[i_0] * E[0] * E[i_0] * A[i_0+2] - 3;
    }
    for (int i_1 = 3; i_1 < PB_M; i_1++) {
        for (int i_2 = i_1; i_2 < min(-i_1+PB_M, PB_Q); i_2++) {
            for (int i_3 = 0; i_3 < min(PB_Q, -i_1+PB_M+1); i_3++) {
                B[i_3+i_1-1][i_2][i_1] = F[i_3][i_2-1] - G[i_2+i_1][i_3][1] * G[i_3][i_2][i_1-1] * G[i_3][i_2+i_1+1][i_2-2] + G[i_3][i_2][i_1] * H[i_2][i_1][i_3][i_2] - B[i_3+i_1-1][i_2][i_1-2] - B[i_3+i_1-2][i_2][i_1] - B[i_3+i_1-3][i_2-2][i_1-2] + 4;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < min(-i_4+PB_L-1, PB_M-1); i_5++) {
            for (int i_6 = 4; i_6 < -i_4+PB_M; i_6++) {
                C[i_4][i_5+i_4][i_6-2] = G[i_5][i_4][i_5+i_4] + G[i_6][i_4][i_5] * G[i_6+i_4][i_5][i_4-1] + C[i_4][i_5+i_4+1][i_6-4] + C[i_5+1][i_6+i_5+1][i_4-2] + C[i_4+1][i_6+i_4+1][i_5] + 5;
            }
        }
        for (int i_7 = 0; i_7 < -i_4+PB_M+1; i_7++) {
            for (int i_8 = 2; i_8 < PB_M; i_8++) {
                C[i_4][i_7+i_4-2][i_8-2] = G[i_8][i_4][i_7] + F[i_7][i_4] * G[i_7][i_4+2][i_7] - G[i_7][i_8][i_4+2] + B[i_7+i_4-1][i_8][i_4] - 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3436152321_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
