...
void kernel_3236212323_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L, i_0); i_1++) {
            B[i_1] = C[i_1][i_0] + B[i_1] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            B[i_3] = C[i_2][i_3+1] - B[i_3-1] * B[i_3+2] * 6;
        }
    }
#pragma endscop
...
kernel_3236212323_01(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
