...
void kernel_2326331215_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = D[1] * A[i_0+1] + A[i_0-1] - 3;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        A[i_1] = E[i_1-1] + E[i_1] + F[i_1] + D[i_1] - 6;
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            C[i_3+1][i_2] = G[i_2+1][i_3] * H[i_3][i_2-1] * G[i_2][i_3] - C[i_3][i_2] + 6;
        }
    }
#pragma endscop
...
kernel_2326331215_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
