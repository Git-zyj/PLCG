...
void kernel_2322111215_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = A[i_1+1][i_0] - A[i_1][i_0-1] * A[i_1+1][i_0+1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        B[i_2-1] = D[i_2] * D[0] * D[i_2] * 6;
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            C[i_3+1][i_4] = C[i_3+2][i_4+1] * C[i_3+2][i_4-1] + C[i_3][i_4] * 1;
        }
    }
#pragma endscop
...
kernel_2322111215_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
