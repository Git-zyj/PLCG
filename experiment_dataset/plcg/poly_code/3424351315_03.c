...
void kernel_3424351315_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            A[i_1+1][i_0] = E[i_1-1][i_0-1] * E[i_1][i_0+1] + F[i_1-1][i_0] - G[i_1][i_0] - A[i_1+2][i_0+1] * A[i_1+1][i_0+1] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q+1; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_L, PB_M-1); i_3++) {
            for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
                B[i_4-1][i_2][i_3] = H[i_3+1][i_2][i_2-1] * G[i_4][i_3+i_2] + H[i_4][i_2][i_2] - H[i_4-1][i_3-1][i_2] * H[i_3+1][i_4-1][i_2] + I[i_4][i_4-1][i_2-1] * J[i_3][i_2+1][i_4+1] - B[i_4][i_2][i_3] * B[i_4][i_2-1][i_3+1] * B[i_2-2][i_3][i_4-1] + 4;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 1; i_6 < i_5; i_6++) {
            A[i_5+2][i_6+1] = G[i_6][i_5-1] * K[i_6] + F[i_6][i_5] + 4;
            for (int i_7 = 0; i_7 < min(-i_6+PB_L+1, PB_M-1); i_7++) {
                A[i_7+1][i_6+1] = G[i_5+1][i_6-1] + J[i_6][i_5][i_5] * E[i_6-1][i_7] * H[i_6+1][i_7+1][i_5] - J[i_5+1][i_7+i_6-1][i_5] - 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3424351315_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K)));
