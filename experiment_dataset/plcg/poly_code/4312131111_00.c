...
void kernel_4312131111_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = i_1; i_2 < -i_1+PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_1+PB_L, -i_2+PB_Q+1); i_3++) {
                    A[i_3][i_3+i_1][i_2-1][i_1] = D[i_1][i_0-1][i_3][i_2+i_1] + D[i_3+1][i_0-1][i_1-1][i_2-1] - D[i_2][i_3+1][i_0][i_1] * D[i_2-1][i_0][i_1+1][i_3+i_2-1] + A[i_2+1][i_2+i_1+1][i_3-1][i_1] * 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            B[i_5-1] = E[i_5][i_4] * F[i_4+1] + B[i_5] * 2;
            for (int i_6 = i_5; i_6 < PB_L; i_6++) {
                for (int i_7 = 0; i_7 < -i_6+PB_L; i_7++) {
                    C[i_5][i_4][i_7+i_6][0] = D[i_6][i_7+1][i_5][i_5-1] - D[i_6][i_4-1][i_5][0] * D[i_6][i_5][i_7][1] + 3;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4312131111_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F)));
