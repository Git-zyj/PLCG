...
void kernel_3316211211_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = PB_N-1 - 1; i_1 >= 1; i_1--) {
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            B[i_1][i_2] = E[i_1][i_2] - B[i_1-1][i_2] - B[i_1+1][i_2] * B[i_1+1][i_2+1] * 2;
        }
        A[i_1-1] = 5;
    }
#pragma endscop
...
kernel_3316211211_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
