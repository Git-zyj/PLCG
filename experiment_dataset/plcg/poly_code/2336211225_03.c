...
void kernel_2336211225_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_1-1][i_0-1] = A[i_1-2][i_0-1] * A[i_1-1][i_0] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q+1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L+1; i_3++) {
            A[i_3-1][i_2-1] = C[i_2-1] * 2;
        }
        C[i_2-1] = D[i_2] * 6;
    }
#pragma endscop
...
kernel_2336211225_03(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
