...
void kernel_4324111125_04(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < -i_0+PB_M; i_2++) {
                B[i_1][i_0+1][i_2+i_0][i_1] = 3;
            }
        }
        for (int i_3 = max(2, i_0); i_3 < PB_N-1; i_3++) {
            for (int i_4 = 0; i_4 < min(i_0, -i_3+PB_M-2); i_4++) {
                B[i_0][i_3+1][i_4+i_3][i_0] = B[i_0][i_4+1][i_4+i_3+2][i_0] + 6;
            }
            for (int i_5 = 0; i_5 < min(min(PB_Q, i_3), -i_0+PB_L-1); i_5++) {
                C[i_3][i_5+i_0][i_5] = D[i_5][i_0-1][0] - C[i_3-2][i_5+i_0+1][i_5] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4324111125_04(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
