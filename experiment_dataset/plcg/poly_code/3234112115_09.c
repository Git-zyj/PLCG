...
void kernel_3234112115_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            B[i_1-1][i_2-1] = C[i_1][i_2][i_1] - B[i_1-2][i_2-2] * B[i_1-1][i_2] - 2;
        }
    }
#pragma endscop
...
kernel_3234112115_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
