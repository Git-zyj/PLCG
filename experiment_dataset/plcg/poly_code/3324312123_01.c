...
void kernel_3324312123_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            A[i_4+1][i_3+1][0] = A[i_3+1][i_4+1][0] + 1;
        }
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
                C[i_3][i_5][i_6] = D[i_3][i_6+2][i_5-1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3324312123_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
