...
void kernel_2232231123_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_0][i_1] = C[i_1][i_1] * C[i_1][0] - A[i_0-2][i_1-2] - 5;
            A[i_0][i_1] = D[i_1][i_0-1] + D[i_1+1][i_0] * D[i_0][i_1] + D[i_0][i_1] + 5;
        }
    }
#pragma endscop
...
kernel_2232231123_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
