...
void kernel_2336211321_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = B[i_0-1][i_0-1] - B[i_0-2][i_0-2] + 3;
        for (int i_1 = 2; i_1 < PB_N-2; i_1++) {
            B[i_0][i_0] = A[i_1] + A[i_1+2] - A[i_1-2] + A[i_1+1] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            C[i_2][0] = D[i_3][i_2] + D[i_2+1][i_3-1] * C[i_2-1][0] - C[i_2+2][0] + C[i_2+1][0] + 5;
        }
    }
#pragma endscop
...
kernel_2336211321_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
