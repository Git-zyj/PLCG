...
void kernel_3326312321_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M, i_0); i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[i_2-2][i_1][i_2] = A[i_2-2][i_1-2][i_2] * B[i_1][i_2] - 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 2; i_4 < i_3; i_4++) {
            B[i_3][i_4] = D[i_4][i_3] - B[i_3][i_4+1] - B[i_3][i_4-2] - B[i_3+2][i_4] - B[i_3+1][i_4] + 5;
        }
...
kernel_3326312321_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
