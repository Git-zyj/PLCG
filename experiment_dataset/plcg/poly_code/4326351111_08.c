...
void kernel_4326351111_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
                for (int i_3 = 1; i_3 < i_2; i_3++) {
                    A[i_3-1][i_1][i_2+1][i_2] = D[i_0][i_3][i_2-1] * E[i_1][i_3][i_3][i_1+1] * F[i_3][i_1][i_2][i_2] + G[i_2-1][i_1][i_3][i_2][i_0] * 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < i_4; i_6++) {
                A[i_5][i_4][i_6+1][i_6] = H[i_6][i_5] * H[i_6-1][i_4] + I[i_6+1][i_6-1][i_4] + 1;
            }
        }
        C[i_4] = J[i_4] - K[i_4-1] - J[i_4] * L[i_4-1] + J[i_4+1] - K[i_4] + K[i_4] * C[i_4+1] * C[i_4-1] + 1;
    }
#pragma endscop
...
kernel_4326351111_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
