...
void kernel_4336352311_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        A[i_0] = D[i_0+1] + D[1] + E[i_0-2] - F[i_0][0] + G[i_0] * G[i_0] + A[i_0-1] - A[i_0+1] - A[i_0-1] - A[i_0-1] - A[i_0-1] + 2;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
                B[i_1][i_2][i_3+2] = H[i_1-2][i_2+1] - I[i_1][i_3][i_2] + I[i_3][i_1-1][i_2-2] * J[i_2+2][i_3-1][i_1][i_2] * K[i_3+2][i_1][i_2] + 3;
            }
        }
        for (int i_4 = 1; i_4 < i_1; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_4+PB_M-1, i_1); i_5++) {
                C[i_5+i_4][i_4+2] = K[i_5][i_4][i_1-1] - L[i_4][i_5-2] - C[i_5+i_4-1][i_4+1] - C[i_5+i_4+1][i_4+3] * C[i_5+i_4-1][i_5+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4336352311_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
