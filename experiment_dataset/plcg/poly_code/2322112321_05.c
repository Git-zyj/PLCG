...
void kernel_2322112321_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        ...
        for (int i_3 = i_2; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            C[i_3+2] = D[i_3+i_2][i_3] + B[i_2-2] * B[i_3] + B[i_3-1] - B[i_3-2] + C[i_3] + 1;
        }
    }
#pragma endscop
...
kernel_2322112321_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
