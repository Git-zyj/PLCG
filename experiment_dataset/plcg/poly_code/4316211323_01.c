...
void kernel_4316211323_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = D[i_0-1] * A[i_0-2] * A[i_0+1] - A[i_0] - A[i_0-3] - 5;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_M; i_2++) {
            A[i_2-2] = 6;
            for (int i_3 = 3; i_3 < i_1; i_3++) {
                for (int i_4 = max(max(0, -i_2), -i_3+3); i_4 < min(-i_3+PB_P+1, -i_2+PB_M); i_4++) {
                    C[i_4+i_2][i_1+1][i_4+i_3-1][i_1] = C[i_4+i_2][i_1+1][i_4+i_3-2][i_1] * C[i_4+i_2][i_1][i_4+i_3-3][i_1-1] - 6;
                }
            }
        }
    }
...
kernel_4316211323_01(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
