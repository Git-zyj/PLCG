...
void kernel_4236211325_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < PB_P+2; i_2++) {
                A[i_1+1][i_0-1][i_1][i_0] = A[i_2-1][i_0-1][i_2-2][i_0] + A[i_1+1][i_0+1][i_1][i_0+2] * 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        B[1] = C[i_3][1] + B[1] * B[1] + B[1] + B[1] * 3;
    }
#pragma endscop
...
kernel_4236211325_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
