...
void kernel_3232311321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                A[i_1][i_2-1][i_0] = C[i_0-1][i_1+1] - A[i_1][i_2][i_0] + A[i_1][i_2][i_0+1] * A[i_1+1][i_2-1][i_0] + A[i_1+2][i_2-1][i_0] - 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N-3; i_3++) {
        B[i_3+1] = B[i_3+3] - B[i_3-1] + 2;
    }
#pragma endscop
...
kernel_3232311321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
