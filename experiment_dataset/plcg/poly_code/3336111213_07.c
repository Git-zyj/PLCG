...
void kernel_3336111213_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_P); i_3++) {
            B[i_2][i_3] = D[i_3+1][i_2] - D[i_3][i_2-1] + D[i_3][i_3+i_2] * B[i_2][i_3+1] - B[i_2][i_3-1] * 1;
        }
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            C[i_2][i_4] = C[i_4-1][i_2+1] + 4;
        }
    }
#pragma endscop
...
kernel_3336111213_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
