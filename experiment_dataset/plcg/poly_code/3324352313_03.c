...
void kernel_3324352313_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0-2] * D[i_0] * D[i_0] + E[i_0] + E[i_0] + A[i_0-1] - A[i_0+1] - A[i_0+1] + 3;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            for (int i_3 = 0; i_3 < -i_2+PB_L-1; i_3++) {
                B[i_2][i_1+2][0] = F[i_2-1][i_3][i_1] - G[i_3][i_2] - F[i_3+i_2+1][i_1][i_3+i_2] * H[i_3][i_1][i_2] + B[i_2+1][i_1+2][0] + B[i_3+1][i_1+2][0] - 6;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        A[i_4] = I[0] * E[i_4] + D[i_4+2] - I[i_4] + I[i_4-1] + A[i_4-1] - 3;
    }
#pragma endscop
...
kernel_3324352313_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
