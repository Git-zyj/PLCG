...
void kernel_4236111215_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_0+1][i_1+1] = C[i_1][i_0] * 6;
        }
        for (int i_2 = 1; i_2 < min(i_0, PB_P-1); i_2++) {
            B[i_0][i_2] = B[i_0][i_2-1] + B[i_0][i_2+1] - 1;
        }
    }
#pragma endscop
...
kernel_4236111215_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
