...
void kernel_4422312323_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
                for (int i_3 = 0; i_3 < PB_Q; i_3++) {
                    A[i_2][i_1][i_0][i_3] = A[i_2+2][i_3][i_1][i_0-1] - A[i_1][i_3][i_2][i_0+1] + 3;
                }
            }
        }
    }
        ...
        for (int i_5 = 1; i_5 < PB_N; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 0; i_7 < -i_5+PB_N; i_7++) {
                    A[i_7][i_5][i_6][i_4-2] = E[i_4][i_7+i_5][i_4+1][i_6] - F[i_7+i_5][i_6+1][i_4][1] * D[i_6][i_5-1][i_4-2][i_4-3] * 5;
                }
            }
        }
    }
    for (int i_8 = 2; i_8 < PB_L-1; i_8++) {
        for (int i_9 = 0; i_9 < PB_M-1; i_9++) {
            for (int j_0 = 2; j_0 < min(i_8, PB_M-1); j_0++) {
                for (int j_1 = 4; j_1 < -i_8+PB_L-1; j_1++) {
                    D[i_8-1][i_9][j_0-1][j_0-2] = E[j_0][j_1][i_9][i_8] - E[j_0][j_1+i_8+1][i_9][0] + D[j_0][i_9+1][i_8+1][i_8] - D[i_8-2][i_9][j_0][j_0-1] + D[i_8][i_9][j_0-1][j_0-2] - D[i_8][j_0+1][j_1-3][j_1-4] - D[j_1-1][i_9][j_0+1][j_0] + 2;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4422312323_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F)));
