...
void kernel_2316132215_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = D[i_0] + D[i_0] + D[i_0+2] - A[i_0] * 5;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            B[i_1][i_2+2] = E[i_1][i_2] - E[i_2-2][i_1+2] + E[i_2][i_1-1] + B[i_1][i_2+1] * B[i_1][i_2+3] * B[i_1+1][i_2+2] * 1;
        }
        A[i_1+1] = D[2] + D[i_1] * D[i_1] * 1;
    }
#pragma endscop
...
kernel_2316132215_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
