...
void kernel_4324131213_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_1+PB_N, -i_0+PB_N-1), i_0); i_2++) {
                for (int i_3 = 1; i_3 < i_0; i_3++) {
                    A[i_2+i_0+1][i_1][i_2][0] = D[i_2][i_3-1][i_0+1][i_1] - E[i_1-1][i_1+1][i_0][i_3][i_2+1] - A[i_3+i_2][i_1][i_3][0] - A[i_2+i_1][i_0][i_2-1][0] - 1;
                }
            }
        }
        B[i_0] = F[i_0] + F[i_0+1] + F[i_0] * F[i_0] + F[i_0] * B[i_0-1] * 3;
    }
    for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
        for (int i_5 = 0; i_5 < -i_4+PB_N-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_L; i_6++) {
                A[i_5+i_4+1][i_6-1][i_5][0] = 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4324131213_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F)));
