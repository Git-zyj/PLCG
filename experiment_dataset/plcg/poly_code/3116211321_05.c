...
void kernel_3116211321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < min(-i_0+PB_M, PB_N); i_2++) {
                A[i_1+1][i_0][i_2] = B[0][i_2+i_0] + A[i_1+2][i_2][i_0] - A[i_1+3][i_0][i_2-2] - A[i_1-1][i_0+2][i_2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3116211321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
