...
void kernel_3336111115_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < min(min(i_0, -i_0+PB_N), PB_L-1); i_1++) {
            B[i_1+1][i_1-1] = 6;
            B[i_1+1][i_1-1] = D[i_1+i_0] - 5;
        }
        for (int i_2 = max(1, i_0); i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < min(min(i_0, -i_0+PB_P), PB_M-1); i_3++) {
                C[i_3][i_2] = E[i_2-1][i_3+i_0][i_2] * C[i_3][i_2-1] * C[i_3+1][i_2] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3336111115_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
