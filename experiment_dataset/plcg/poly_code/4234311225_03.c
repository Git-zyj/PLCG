...
void kernel_4234311225_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
            for (int i_2 = i_1; i_2 < -i_1+PB_P-2; i_2++) {
                A[i_1][i_2+i_1-1][i_2] = C[i_2-1][i_1] + C[i_2][i_1] - A[i_1+1][i_2+i_1+2][i_2+2] + A[i_1][i_2+i_1][i_2+1] * 3;
            }
        }
        for (int i_3 = i_0; i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
                for (int i_5 = 2; i_5 < min(-i_0+PB_P+3, PB_M-2); i_5++) {
                    B[i_5][i_4][i_3-1] = A[i_0][i_5+i_0-3][i_5-2] - B[i_5+2][i_4+2][i_3] + 1;
                }
            }
        }
    }
...
kernel_4234311225_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
