...
void kernel_4322331211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = D[i_0] - D[i_0] + E[i_0] + A[i_0+2] * 2;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        B[i_1] = D[i_1] * D[i_1] - E[i_1-1] + B[i_1-1] + 2;
        for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
            for (int i_3 = 1; i_3 < -i_1+PB_L; i_3++) {
                for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
                    C[i_4][i_2-1][i_1+1] = F[i_2][i_1][i_4+1][i_3+1] * G[i_3+i_1][i_4][i_2] + H[i_4][i_3][i_2] + C[i_3+1][i_4-2][i_2+1] + C[i_4-1][i_3-1][i_2+2] - 2;
                }
            }
        }
    }
...
kernel_4322331211_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
