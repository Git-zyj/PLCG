...
void kernel_2226232111_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_N-1); i_1++) {
            A[i_1+1][i_1-2] = C[i_1+i_0] - D[i_1][i_1] - D[i_1-2][0] * D[i_0+1][i_1] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_P+1; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            B[i_3+1][i_2-1] = D[i_3][i_2] + A[i_3+1][i_3-2] + B[i_3+1][i_2-2] - 5;
        }
    }
#pragma endscop
...
kernel_2226232111_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
