...
void kernel_4436212115_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_0+1][i_1] = E[i_0] * F[i_0][i_0][i_1] + A[i_0+2][i_1] * A[i_0][i_1] - 3;
        }
    }
        ...
        for (int i_3 = 0; i_3 < min(PB_M, -i_2+PB_Q); i_3++) {
            for (int i_4 = i_3; i_4 < PB_P; i_4++) {
                for (int i_5 = 0; i_5 < PB_P; i_5++) {
                    D[i_3][i_2+1][i_4][i_4] = G[i_4][i_5][i_3+i_2][i_3] + 5;
                }
            }
            A[i_2+2][i_3] = 4;
        }
    ...
    for (int i_6 = 0; i_6 < PB_M; i_6++) {
        for (int i_7 = 0; i_7 < i_6; i_7++) {
            for (int i_8 = 2; i_8 < PB_P; i_8++) {
                for (int i_9 = 0; i_9 < min(i_8, PB_P-1); i_9++) {
                    D[i_6][i_7][i_8][i_8] = H[i_6][i_9+1][i_8-2] - 6;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436212115_00(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
