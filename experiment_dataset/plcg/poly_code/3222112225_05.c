...
void kernel_3222112225_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-3; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_Q-3); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_Q-2; i_2++) {
                A[i_1][i_2][i_2+i_0] = C[i_0+2][i_1-1][i_2+1] - A[i_2][i_0+1][i_1+i_0+3] + A[i_1][i_0+2][i_2+i_0+2] * 2;
            }
            for (int i_3 = 0; i_3 < -i_0+PB_Q; i_3++) {
                A[i_1-1][i_3][i_3+i_0] = 1;
            }
        }
    }
#pragma endscop
...
kernel_3222112225_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
