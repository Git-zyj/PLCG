...
void kernel_3316111321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        B[i_1] = D[i_1] + B[i_1-1] + B[i_1-2] - C[i_1-2][i_1-2] + C[i_1][i_1] + C[i_1+2][i_1+2] * 1;
        C[i_1][i_1] = D[i_1] + D[i_1] - B[i_1+1] + C[i_1-2][i_1-2] * 5;
    }
#pragma endscop
...
kernel_3316111321_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
