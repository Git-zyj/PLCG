...
void kernel_4312211315_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0-1] + 1;
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            A[i_1] = D[i_0] + A[i_1+1] + A[i_0-1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        C[i_2] = C[i_2-1] * C[i_2+1] * C[i_2+1] + C[i_2+1] - 5;
    }
#pragma endscop
...
kernel_4312211315_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
