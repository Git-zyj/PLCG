...
void kernel_3312111313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0][i_0] = D[i_1+i_0][i_1-1] - A[i_1+i_0-1][i_0-1] * A[i_1+i_0-1][i_0] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M-2; i_3++) {
            for (int i_4 = 0; i_4 < -i_2+PB_M+1; i_4++) {
                B[i_3+i_2][0][i_2-1] = E[i_4+i_3][i_2][i_3] + B[i_4+i_2-1][0][i_2-2] * B[i_3+i_2+2][0][i_2] * B[i_3+i_2-1][0][i_2-1] + 5;
            }
        }
...
kernel_3312111313_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
