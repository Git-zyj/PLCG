...
void kernel_4124132121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < min(PB_P+3, -i_0+PB_L-1); i_1++) {
            A[i_0][i_0-1] = B[i_0+2][i_1-1] + B[i_0-1][i_1+i_0+1] * B[i_0-1][1] * A[i_1-2][i_1-3] * 4;
        }
    }
#pragma endscop
...
kernel_4124132121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
