...
void kernel_2334252113_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        A[i_1] = D[i_1] - D[0] * D[i_1] - E[i_1] - E[i_1] + F[i_1][i_1] * 3;
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = PB_L-2 - 1; i_3 >= 1; i_3--) {
            A[i_3] = F[i_2+2][i_3+2] * G[i_3][i_2-1] + D[i_3] - D[i_2+1] - H[i_2][i_3-1][i_3+2] + E[i_2] + 3;
        }
    }
#pragma endscop
...
kernel_2334252113_00(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
