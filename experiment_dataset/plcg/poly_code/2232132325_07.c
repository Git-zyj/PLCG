...
void kernel_2232132325_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        A[i_0-1] = C[i_0] + D[i_0+1][1] * A[i_0-2] - B[i_0-1] - 3;
    }
    for (int i_1 = 3; i_1 < PB_M-2; i_1++) {
        B[i_1-1] = C[i_1] * C[i_1] - C[i_1+2] - B[i_1-3] * B[i_1] + B[i_1+1] + 6;
    }
#pragma endscop
...
kernel_2232132325_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
