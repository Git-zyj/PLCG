...
void kernel_4234311215_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < min(PB_Q-1, i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                for (int i_3 = 1; i_3 < PB_M; i_3++) {
                    A[i_3][i_0-1][i_2-1][i_1] = A[i_3-1][i_0-2][i_2][i_1+1] - 3;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            B[i_4+1] = C[i_5] * B[i_4+2] + B[i_5+2] - B[i_5] + 5;
        }
    }
#pragma endscop
...
kernel_4234311215_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
