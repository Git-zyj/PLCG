...
void kernel_4214312221_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = i_0; i_1 < PB_Q-4; i_1++) {
            for (int i_2 = 2; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < PB_Q-4; i_3++) {
                    A[i_2][i_2][i_3+2][i_1+2] = A[i_2][i_2][i_3+1][i_1+4] + A[i_2-2][i_2-2][i_3+4][i_1+2] + 3;
                }
            }
        }
        B[i_0-1] = C[1] + B[i_0+1] * 1;
    }
#pragma endscop
...
kernel_4214312221_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
