...
void kernel_2214132225_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-4; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            A[i_1+2][i_1] = C[i_1][i_0+2] + D[0] * C[i_0][0] * C[i_1][i_0+2] * C[i_0][i_1] - A[i_1+1][i_1-1] - 6;
            A[i_1+1][i_1-1] = A[i_0+4][i_0+2] + 1;
        }
    }
#pragma endscop
...
kernel_2214132225_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
