...
void kernel_4232132113_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = C[i_0] * C[i_0-2] - C[i_0] - C[i_0] + C[i_0] + 2;
    }
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_L-1; i_2++) {
            for (int i_3 = 0; i_3 < i_1; i_3++) {
                B[i_1][i_2+i_1+1][i_3] = D[i_2][i_3][i_2-1] - B[i_1][i_2+i_1][i_3] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4232132113_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
