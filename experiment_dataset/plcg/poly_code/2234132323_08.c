...
void kernel_2234132323_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0+1][i_1+2] = C[i_0-1][0] + A[i_0-1][i_1+3] * A[i_0-1][i_1+2] * A[i_0+3][i_1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            A[i_2+3][i_3+2] = C[i_3-1][i_2] * C[i_3][i_2] - C[i_2-1][i_3] + C[i_2+2][i_3] - 3;
        }
    }
#pragma endscop
...
kernel_2234132323_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
