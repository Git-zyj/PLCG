...
void kernel_3326312225_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        B[i_0][0] = 3;
        B[i_0][0] = D[i_0] - C[i_0+2][0] - 1;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            C[i_1][0] = C[i_1+2][0] - C[i_1-1][0] - 6;
        }
    }
#pragma endscop
...
kernel_3326312225_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
