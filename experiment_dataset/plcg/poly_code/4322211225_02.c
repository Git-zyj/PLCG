...
void kernel_4322211225_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0-1] = A[i_0+1] * 2;
        A[i_0-1] = D[0] + A[i_0-1] * 3;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-3; i_2++) {
            for (int i_3 = 0; i_3 < -i_2+PB_L-3; i_3++) {
                for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
                    C[i_1-1][i_3+i_2][i_1][i_4] = C[i_1-2][i_4+i_3][i_1-1][i_2-1] - C[i_1-1][i_3+i_2+3][i_1][i_4] + 3;
                }
            }
        }
    }
...
kernel_4322211225_02(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
