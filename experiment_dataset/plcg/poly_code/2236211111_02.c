...
void kernel_2236211111_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_L-1, i_1); i_2++) {
            B[i_1][i_2][i_2+i_1] = C[i_2] + B[i_1+1][i_2][i_2+i_1+1] * 4;
        }
    }
#pragma endscop
...
kernel_2236211111_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
