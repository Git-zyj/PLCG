...
void kernel_4414112123_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            C[i_1][i_1-1] = F[i_2+2][i_1] + C[i_1+1][i_1] + 5;
            for (int i_3 = i_2; i_3 < PB_M-2; i_3++) {
                for (int i_4 = 0; i_4 < -i_3+PB_M-2; i_4++) {
                    D[i_1][i_4][i_1][i_2+1] = G[i_4+i_3+2][i_1][i_2][i_2] + 5;
                }
            }
        }
    }
...
kernel_4414112123_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
