...
void kernel_2422251113_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1] = E[i_0+1] * F[i_1][0] * E[i_1] * G[i_0][i_1] * H[i_1-1] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        B[i_2] = H[0] + E[i_2+1] - E[i_2] - E[i_2+1] - 3;
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        C[i_3] = E[i_3] * H[i_3+1] - H[i_3] * E[i_3] * C[i_3+1] + 6;
        C[i_3] = E[i_3] + H[i_3] + E[i_3] + E[i_3-1] - C[i_3] - 1;
    }
#pragma endscop
...
kernel_2422251113_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
