...
void kernel_3426132325_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_N+2; i_1++) {
            B[i_1-2] = E[i_0][i_1] * E[i_0][i_1+2] * 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            B[i_2] = E[i_3+1][i_2] * F[i_2-2] - E[i_3][i_2] - 2;
        }
        B[i_2+1] = F[i_2] + F[i_2] + B[i_2] - B[i_2+1] - 1;
    }
    for (int i_4 = 2; i_4 < PB_N-1; i_4++) {
        D[i_4] = F[i_4+1] - F[i_4+1] - F[i_4-1] + B[i_4] + D[i_4-2] * D[i_4-1] + 3;
    }
#pragma endscop
...
kernel_3426132325_00(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
