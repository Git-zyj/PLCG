...
void kernel_3226131111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[i_1][i_1+i_0] = C[i_1] * C[i_1] + D[i_1][i_1-1] + A[i_1-1][i_1+i_0-1] * 1;
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                B[i_2][i_1][i_2] = E[i_2][i_0][i_1+1] - E[i_1][i_0-1][i_2] - E[i_2][i_1][i_0] + B[i_2][i_1+1][i_2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3226131111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
