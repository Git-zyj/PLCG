...
void kernel_2216312321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 4; i_3 < PB_M; i_3++) {
            B[i_3-2] = C[i_2] - B[i_3] - B[i_3-1] - B[i_3-3] * B[i_3-4] - 6;
        }
    }
#pragma endscop
...
kernel_2216312321_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
