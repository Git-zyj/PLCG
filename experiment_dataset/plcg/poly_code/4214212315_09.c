...
void kernel_4214212315_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(min(-i_0+PB_L, i_0), PB_M-2); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M-2, -i_0+PB_L-1); i_2++) {
                A[i_2+i_1][i_1+i_0][i_2] = C[i_2][i_1+1] + A[i_2+i_1+2][i_2+i_0+1][i_1+1] + 2;
            }
            B[i_1][i_1] = C[i_1][i_0] + B[i_1-1][i_1-1] * B[i_1+1][i_1+1] - B[i_1-1][i_1-1] + B[i_1-1][i_1-1] - 1;
        }
    }
#pragma endscop
...
kernel_4214212315_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
