...
void kernel_3312111325_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            B[i_1] = D[i_1+i_0][0] * B[i_1+1] + B[i_1-1] - B[i_1+2] + 2;
        }
        B[i_0+1] = E[i_0] - 1;
        for (int i_2 = i_0; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 1; i_3 < PB_M; i_3++) {
                C[i_3][i_0][i_2-1] = D[i_3-1][i_3-1] + C[i_3][i_0-2][i_2-1] * C[i_0][i_2+1][i_3-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3312111325_05(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
