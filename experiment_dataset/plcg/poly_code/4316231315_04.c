...
void kernel_4316231315_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < min(i_1, PB_L-2); i_2++) {
                for (int i_3 = max(0, -i_1+2); i_3 < -i_1+PB_N+1; i_3++) {
                    A[i_3+i_1-1][i_1+1][i_2+1][i_0] = D[i_2-1][i_1-1][i_0][i_0] - A[i_3+i_1-1][i_1+1][i_2+2][i_0] * A[i_3+i_1-2][i_1+1][i_2+2][i_0] + A[i_3+i_1-1][i_1+1][i_2+2][i_0-1] * 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        B[i_4-1] = E[i_4] * B[i_4-2] + B[i_4] * B[i_4] * 1;
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_N+1); i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_L; i_6++) {
                for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
                    A[i_5+i_4-1][i_4+1][i_7][i_6-1] = F[i_5+1][i_7-1][i_6+1][i_4] * D[i_6][1][i_7][i_5+i_4-1] + D[i_5][i_4+1][i_7+1][i_6] + D[i_6+i_5][i_7-1][i_7+1][0] + G[i_5][i_6][i_7][i_4][i_7+i_5] * F[i_5+1][i_6][i_4][i_6] * D[i_5+1][i_7+1][i_5][i_6] - 6;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4316231315_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G)));
