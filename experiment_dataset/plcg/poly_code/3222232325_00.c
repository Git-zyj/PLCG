...
void kernel_3222232325_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = PB_M - 1; i_1 >= max(1, i_0); i_1--) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_M, i_1); i_2++) {
                B[i_2-2] = C[i_0][i_2-1][i_0] - D[i_0][i_1-1][i_2-2] * D[i_2+i_1][i_0+2][i_1] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            B[i_4] = E[i_4][i_3-1] + F[i_3][i_4] * E[i_4][i_3] * B[i_4-1] - B[i_4+2] * B[i_4-2] - 4;
        }
    }
#pragma endscop
...
kernel_3222232325_00(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
