...
void kernel_3332231213_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_1][i_0][i_0-1] = D[i_1][i_0] - D[i_0][i_0-1] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            B[i_2][i_3] = E[i_3] * D[i_3+1][i_2] * F[i_3+1][i_2] - B[i_2+1][i_3] + 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M+1; i_4++) {
        for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
            C[i_5-1] = F[i_5+1][i_4] - D[i_4-1][i_5+1] + F[i_5+1][i_4] * G[i_5+1] + C[i_5-2] + C[i_5] * C[i_5] - 2;
        }
    }
#pragma endscop
...
kernel_3332231213_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
