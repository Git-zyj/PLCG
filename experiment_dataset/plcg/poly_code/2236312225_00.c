...
void kernel_2236312225_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_0][i_1+1] = C[i_1-2][i_0+1] + A[i_0+2][i_1+1] + 3;
        }
    }
    for (int i_2 = 4; i_2 < PB_L+2; i_2++) {
        B[i_2-2] = B[i_2-4] * B[i_2-3] + 2;
    }
#pragma endscop
...
kernel_2236312225_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
