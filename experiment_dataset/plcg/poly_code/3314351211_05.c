...
void kernel_3314351211_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        B[i_0-2] = D[i_0-1] + E[i_0+1] * E[i_0-1] - E[i_0] + D[i_0] * 4;
        B[i_0-1] = E[i_0-1] - E[i_0-1] * F[i_0-1][0] - G[i_0] * D[i_0] + G[i_0+1] * 2;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            C[i_2+i_1-1][0] = H[i_1][i_2-1] - F[i_2][i_1] + F[i_1+1][i_2] * C[i_2+i_1-2][0] * C[i_2+i_1][0] * C[i_2+i_1][0] + 2;
        }
    }
#pragma endscop
...
kernel_3314351211_05(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
