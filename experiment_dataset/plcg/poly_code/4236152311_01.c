...
void kernel_4236152311_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            B[i_1] = C[i_0] * D[i_0][i_1+1] * D[i_1][i_0] + D[i_1-1][i_0-2] - D[i_0][i_1] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        B[i_2] = C[i_2] - D[i_2][0] - C[i_2] + C[i_2-2] + C[i_2] + B[i_2-1] - B[i_2+1] - B[i_2+1] + 3;
    }
#pragma endscop
...
kernel_4236152311_01(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
