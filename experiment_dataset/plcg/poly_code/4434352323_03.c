...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4434352323_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_Q-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_0+PB_Q, PB_M-1); i_3++) {
                    A[i_0][i_1][i_2][i_3] = E[i_3+1][i_2-1][i_1] + F[i_2-2][i_1-2][i_3+i_0][i_1] + G[i_2-1][i_1][0] + H[i_3+2][i_1+1][i_1+i_0+2][i_2] * H[i_1][1][i_3][i_0] + A[i_0][i_3][i_2+1][i_1] * A[i_0][i_1+2][i_2-2][i_3-2] - A[i_0][i_3][i_2-2][i_1] * 5;
                }
            }
        }
    }
    for (int i_4 = 4; i_4 < PB_P; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-2; i_5++) {
            B[i_5] = E[i_5+1][i_4-2][i_5] + I[i_5][i_5] + J[i_5] + K[i_5+1][i_4] - B[i_5+1] * B[i_5+2] - B[i_5-2] * B[i_5-1] * 4;
        }
        for (int i_6 = 0; i_6 < -i_4+PB_P; i_6++) {
            for (int i_7 = 2; i_7 < PB_P-2; i_7++) {
                for (int i_8 = 2; i_8 < -i_7+PB_M; i_8++) {
                    C[i_4-2][i_6][i_8][i_8-2] = L[i_8][i_7+2][i_6+2][i_4] * L[i_8+i_7][i_6+i_4][i_8][i_7] - L[i_6][i_8][i_7][i_4] - L[i_6+2][i_8][i_4][i_7] - C[i_4-4][i_6][i_8][i_8-2] * C[i_4-1][i_6][i_8][i_8-2] - C[i_7-2][i_6][i_8+2][i_8] - 6;
                }
            }
        }
    }
    for (int i_9 = 0; i_9 < PB_P-2; i_9++) {
        for (int j_0 = 1; j_0 < PB_L-2; j_0++) {
            for (int j_1 = 3; j_1 < j_0; j_1++) {
                for (int j_2 = max(max(max(1, -j_1+3), -j_0+1), -i_9); j_2 < min(min(min(-j_1+PB_M+3, PB_Q-2), -i_9+PB_Q), -j_0+PB_M+1); j_2++) {
                    D[j_2+j_0-1][i_9][j_1][j_2+2] = L[j_1][j_0][j_2+i_9][j_0] - E[j_2-1][j_0+2][i_9+2] * F[j_2][j_1][j_0][i_9] - G[j_1][j_2][j_1] - M[j_2+2][j_0][j_1] + A[j_1-2][j_0][i_9][j_2] + D[j_2+j_1-3][i_9][j_0][j_2+1] - 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4434352323_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M)));
