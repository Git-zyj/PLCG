...
void kernel_3232211321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, PB_M-2); i_1++) {
            for (int i_2 = i_1; i_2 < min(-i_0+PB_L, PB_M-2); i_2++) {
                A[i_1][i_2+i_0][i_0] = A[i_2+2][i_1+i_0][i_0] + A[i_1-1][i_2+i_0][i_0] - A[i_1+2][i_2+i_0][i_0] - 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 2; i_4 < PB_M+1; i_4++) {
            for (int i_5 = 1; i_5 < PB_M+1; i_5++) {
                B[i_4-1][i_5] = C[i_5-1][i_3] * B[i_5-1][i_3+2] + B[i_4-2][i_5] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3232211321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
