...
void kernel_2334112223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-1; i_1++) {
            A[i_0][i_1] = D[i_1+1][i_0] * A[i_0+2][i_1-1] - A[i_0+1][i_1] - A[i_0+2][i_1] + 1;
        }
        for (int i_2 = max(2, i_0); i_2 < PB_P; i_2++) {
            B[i_0][i_2] = B[i_0+2][i_2] + B[i_0][i_2-2] + 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            C[i_3][i_4] = D[i_4][i_3] + 3;
        }
    }
#pragma endscop
...
kernel_2334112223_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
