...
void kernel_2322131313_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        A[i_0+1] = D[i_0+1] * A[i_0] * 6;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        B[i_1-1] = D[i_1] - D[i_1-1] * D[i_1] - D[i_1] * B[i_1-2] * B[i_1] - 2;
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_N, i_2); i_3++) {
            C[i_2][i_3] = D[i_3] * D[i_3] + B[i_3-1] - B[i_3-2] * C[i_3+1][i_2+1] * C[i_2-1][i_3] - 5;
        }
    }
#pragma endscop
...
kernel_2322131313_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
