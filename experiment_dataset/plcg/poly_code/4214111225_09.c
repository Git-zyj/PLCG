...
void kernel_4214111225_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-4; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_P-4; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
                A[i_2-1][i_1+i_0+1][i_0] = C[i_2+i_1][i_0+1][i_1+1] - A[i_2-1][i_1+i_0+4][i_0+2] - A[i_2-1][i_1+i_0+2][i_0+1] - 3;
            }
        }
        for (int i_3 = 1; i_3 < -i_0+PB_M+1; i_3++) {
            for (int i_4 = 0; i_4 < -i_0+PB_P-1; i_4++) {
                A[i_3-1][i_4+i_0+1][i_0] = C[i_3+i_0-1][i_4][0] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4214111225_09(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
