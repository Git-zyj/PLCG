...
void kernel_2332212213_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_1-1][i_0-2] = D[i_1] - 2;
        }
    }
    for (int i_2 = 3; i_2 < PB_L+3; i_2++) {
        for (int i_3 = 2; i_3 < PB_L+1; i_3++) {
            B[i_2][i_3-2] = B[i_2][i_3-1] + B[i_3+1][i_2-3] * B[i_2-1][i_3-2] - B[i_2+1][i_3-1] * 3;
        }
    }
...
kernel_2332212213_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
