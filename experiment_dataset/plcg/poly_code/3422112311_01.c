...
void kernel_3422112311_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_1-1][i_0][i_0] = E[i_1-1][i_0] * A[i_1][i_0+1][i_0+1] - A[i_1][i_0][i_0] * A[i_0-2][i_1][i_1] + A[i_1-2][i_0-1][i_0-1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_3] = F[i_2+1][i_3][i_2+1] - B[i_3+1] - 5;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-3; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            C[i_4+2][i_4] = C[i_4+1][i_4-1] - C[i_4+3][i_4+1] * 4;
        }
        for (int i_6 = 1; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < PB_M-1; i_7++) {
                D[i_6][i_4][i_6][i_7] = D[i_6-1][i_4][i_6-1][i_7] - D[i_6][i_7][i_6][i_4-1] - D[i_7+1][i_4-1][i_7+1][i_6] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_3422112311_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
