...
void kernel_3216212321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_M+1); i_1++) {
            A[i_1+i_0-1][i_1+i_0+1] = C[i_1][i_0] - A[i_1+i_0-3][i_1+i_0-1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < min(min(-i_2+PB_M+1, i_2), PB_M-2); i_3++) {
            B[i_3] = C[i_3-2][i_2] + A[i_3+i_2-1][i_3+i_2+1] * B[i_3+2] * B[i_3-1] - B[i_3+1] - 3;
        }
    }
#pragma endscop
...
kernel_3216212321_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
