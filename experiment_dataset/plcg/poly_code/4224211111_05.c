...
void kernel_4224211111_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_Q, PB_P+1); i_1++) {
            for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
                A[i_2+1][i_0][i_1+i_0] = B[i_2-1][i_1-1][i_0] * 2;
            }
        }
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < PB_N; i_4++) {
                B[i_0][i_3][i_4] = C[i_4][i_0][i_3-1] * D[i_0][i_3][i_4+1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4224211111_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
