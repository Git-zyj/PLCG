...
void kernel_2414331311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0+1] * F[i_0-1][i_0] * E[i_0] - A[i_0-1] - A[i_0+1] * 2;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_L; i_2++) {
            B[i_2] = G[i_1][i_2+i_1] + G[i_2][i_1] + H[i_2+i_1] - B[i_2+1] + D[i_1][i_2] * 2;
        }
        C[i_1] = H[i_1] - E[i_1+1] - H[0] + B[i_1] - C[i_1+1] * C[i_1-1] + 4;
        for (int i_3 = 0; i_3 < i_1; i_3++) {
            D[i_1][i_3] = G[i_3][i_1] - I[i_3][i_1+1] * A[i_3] * C[i_1+1] + C[i_3] - 3;
        }
    }
#pragma endscop
...
kernel_2414331311_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
