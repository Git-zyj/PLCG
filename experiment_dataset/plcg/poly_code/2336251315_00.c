...
void kernel_2336251315_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = D[i_0] * D[i_0] - A[i_0] + A[i_0+2] + B[i_0] + 2;
        B[i_0] = D[i_0] * E[i_0+1] + E[i_0] - D[0] * D[i_0+1] * E[i_0] - E[i_0] * A[i_0+1] - B[i_0-1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_Q; i_1++) {
        for (int i_2 = i_1; i_2 < PB_N; i_2++) {
            A[i_2] = F[i_2][i_1] - G[i_2][i_1-1] + G[i_2-1][i_2] + H[i_1][i_2][0] + A[i_1+1] - 2;
        }
    }
#pragma endscop
...
kernel_2336251315_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
