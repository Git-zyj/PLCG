...
void kernel_4426232225_03(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
                C[i_1-1] = E[i_2][i_2][0] + F[i_1][i_0+1][i_2+1] * G[i_1][i_2-2] - G[i_2][i_2-1] * 4;
            }
        }
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            C[i_3] = G[i_3][i_0] * C[i_3] + 4;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        C[i_4] = H[i_4] + I[i_4] * I[i_4] * H[i_4] * C[i_4+1] - 3;
        for (int i_5 = 0; i_5 < PB_P-2; i_5++) {
            for (int i_6 = 2; i_6 < i_5; i_6++) {
                C[i_6-1] = G[i_4][i_5] * F[i_4][i_6-2][i_5] + J[i_5][i_5+2] - 4;
            }
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4426232225_03(xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
