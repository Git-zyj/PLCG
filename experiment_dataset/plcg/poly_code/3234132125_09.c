...
void kernel_3234132125_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_L, i_0); i_1++) {
            A[i_1] = C[i_0][i_1] * D[i_1] + A[i_1-1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        A[i_2+2] = D[2] + D[i_2] + D[i_2] * C[i_2][0] - 4;
    }
#pragma endscop
...
kernel_3234132125_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
