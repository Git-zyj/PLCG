...
void kernel_3316112115_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_N; i_3++) {
            for (int i_4 = 1; i_4 < i_2; i_4++) {
                B[i_2][i_4] = C[i_3][i_4-1] + 5;
            }
        }
        for (int i_5 = 2; i_5 < i_2; i_5++) {
            C[i_2][i_5] = D[i_5-2][i_2] * 1;
        }
    }
#pragma endscop
...
kernel_3316112115_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
