...
void kernel_3226112111_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_M; i_1++) {
        B[i_1-2] = C[i_1-2] + C[i_1] + B[i_1-3] - 5;
    }
#pragma endscop
...
kernel_3226112111_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
