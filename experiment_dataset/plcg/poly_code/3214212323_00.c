...
void kernel_3214212323_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < PB_M-1; i_1++) {
            A[i_0][i_0+1][i_1-1] = C[i_1][i_0] - A[i_1+1][i_1+2][i_0-3] * A[i_0-1][i_0][i_1-1] - A[i_0][i_0+1][i_1-2] - A[i_0][i_0+1][i_1-3] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 3; i_3 < PB_Q+2; i_3++) {
            for (int i_4 = 0; i_4 < PB_Q-3; i_4++) {
                B[i_2+1][i_3-2][i_4+1] = D[i_4][i_2][i_3] - B[i_2+1][i_3-3][i_4+2] * B[i_2-1][i_3-3][i_4+3] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3214212323_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
