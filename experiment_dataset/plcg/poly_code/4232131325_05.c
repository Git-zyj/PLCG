...
void kernel_4232131325_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_M, -i_1+PB_L); i_2++) {
                B[i_2+1] = C[i_2][i_0][i_1] - C[i_2-1][i_0][i_1] * C[i_1][i_0][i_2+i_1-1] * C[i_0-1][i_2+i_1][1] - B[i_1] - B[i_2] + 2;
            }
            B[i_1] = D[i_0][i_1+1] - D[i_1+1][i_0+1] - B[i_1+2] * 2;
        }
    }
#pragma endscop
...
kernel_4232131325_05(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
