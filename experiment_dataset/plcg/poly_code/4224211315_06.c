...
void kernel_4224211315_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = i_1; i_2 < -i_1+PB_L; i_2++) {
                A[i_0][i_2+i_1-1][i_0-1][i_2] = A[i_0-1][i_2+i_1][i_0-2][i_2] + A[i_0][i_2+i_1-1][i_0-1][i_2-1] - 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
            B[i_3][i_4+1] = C[i_4] - B[i_3][i_4+2] * B[i_3][i_4] - B[i_3-1][i_4] - 5;
        }
    }
#pragma endscop
...
kernel_4224211315_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
