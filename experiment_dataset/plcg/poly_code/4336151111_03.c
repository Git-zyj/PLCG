...
void kernel_4336151111_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-2 - 1; i_0 >= 1; i_0--) {
        for (int i_1 = i_0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = i_0; i_2 < min(-i_0+PB_M, -i_1+PB_M); i_2++) {
                A[i_0+1][i_2][i_1+1] = D[i_1+1][i_0-1][i_2+i_0] + D[i_2-1][i_0-1][0] * D[i_2+i_1][i_2+i_0][i_0] + A[i_0+2][i_2][i_1] * 4;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L; i_3++) {
            B[i_0][i_3-1] = E[i_0+1] - E[i_0] - F[i_0][i_3] - F[i_0][i_0] * F[i_3][i_0] * F[i_0][i_3] - 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_P; i_4++) {
        for (int i_5 = 2; i_5 < min(PB_P, -i_4+PB_Q+1); i_5++) {
            for (int i_6 = i_5; i_6 < -i_5+PB_M; i_6++) {
                for (int i_7 = 1; i_7 < min(-i_6+PB_M-1, -i_4+PB_P); i_7++) {
                    C[i_6+i_5][i_7-1][i_7][i_5+i_4-1] = D[i_5][i_6+1][0] - G[i_5][i_7][i_4][i_5] - G[i_5][i_7-1][i_4][i_6] * G[i_5][i_7][i_7+i_4][i_6] - D[i_5][i_7][i_6] + G[i_7][i_4][i_5][i_4] - C[i_7+i_6+1][i_5-2][i_5-1][i_7+i_4] + C[i_7+i_6-1][i_4][i_4+1][i_7+i_5-1] * 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4336151111_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
