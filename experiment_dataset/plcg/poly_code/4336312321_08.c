...
void kernel_4336312321_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = A[i_0-1] + A[i_0-2] - A[i_0+2] + A[i_0+1] * 2;
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            B[i_0-2][i_1] = D[i_0][i_1] - B[i_0-1][i_1] * B[i_0-2][i_1-2] + B[i_0-2][i_1+1] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_P-1, i_2); i_3++) {
            for (int i_4 = i_2; i_4 < -i_3+PB_P-1; i_4++) {
                C[i_4+i_3][1] = C[i_4+i_3+1][1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4336312321_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
