...
void kernel_4426131323_05(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
    ...
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        ...
        for (int i_1 = 3; i_1 < -i_0+PB_P+1; i_1++) {
            for (int i_2 = i_0; i_2 < PB_Q-1; i_2++) {
                B[i_1][i_0-1][i_2+1] = F[i_2-1][i_0][i_1+1] * F[i_2][i_1+i_0-1][i_2] * B[i_1+1][i_0-1][i_2+1] - B[i_1][i_0][i_2+1] - B[i_1][i_0-3][i_2] - B[i_0+2][i_1-3][i_2+1] + 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 1; i_4 < min(min(-i_3+PB_P-1, PB_M-1), i_3); i_4++) {
            for (int i_5 = 2; i_5 < -i_4+PB_P; i_5++) {
                B[i_4][i_5-2][i_3] = F[i_5-1][1][i_4+1] - F[i_5][i_4+i_3+1][0] + F[i_5][i_3][0] * G[i_4+1][i_5+i_4][i_3][i_4-1] * F[i_3-1][i_5+i_4][i_3] * 4;
            }
        }
        for (int i_6 = 1; i_6 < PB_M-2; i_6++) {
            D[i_6] = E[i_6-1] + D[i_6+1] - D[i_6+2] - 5;
        }
    }
#pragma endscop
...
kernel_4426131323_05(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
