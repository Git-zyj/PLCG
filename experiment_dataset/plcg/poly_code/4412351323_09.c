...
void kernel_4412351323_09(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn),int xo,DATA_TYPE POLYBENCH_1D(O,xO,xo)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, -i_0+PB_M-3); i_2++) {
                C[i_2+i_0+3][i_2+1] = E[i_2+i_1][i_0][1] * F[i_2-1][i_0][i_2] + G[i_1][i_2] + 1;
            }
            for (int i_3 = 1; i_3 < -i_0+PB_M-1; i_3++) {
                for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                    C[i_3+i_0+1][i_3] = H[i_3-1][i_3][i_4][i_1] * I[i_4+1][i_3][i_1][i_0] + I[i_4][i_3][i_0][i_1] - H[i_3+1][i_0+1][i_4+1][i_1-1] - J[i_3][i_4][i_0-1][i_1] * 1;
                }
            }
        }
        for (int i_5 = 3; i_5 < -i_0+PB_M-2; i_5++) {
            C[i_5+i_0][i_5-1] = K[i_0][0] - L[i_5+1][i_0] + K[i_5][i_0-1] + M[i_5+1] * N[i_5-1] + L[i_5][i_0+1] * C[i_5+i_0+1][i_5] * C[i_5+i_0+2][i_5] + C[i_5+i_0][i_5] - C[i_5+i_0-2][i_5-3] - 3;
        }
    }
    for (int i_6 = 2; i_6 < PB_L; i_6++) {
        D[i_6-1] = O[i_6] * M[i_6] * G[i_6][1] * D[i_6-2] + D[i_6+1] + 3;
    }
#pragma endscop
