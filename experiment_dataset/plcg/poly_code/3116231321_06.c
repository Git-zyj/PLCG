...
void kernel_3116231321_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_M+1); i_2++) {
                A[i_1][i_0+1][i_2] = B[i_0][i_2][i_1+1] + B[i_2+i_1-1][i_0][i_0] + C[i_0][i_2][i_1+1] - A[i_1][i_0-1][i_2] + A[i_1-2][i_0+3][i_2] + A[i_1+1][i_0+2][i_2] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3116231321_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
