...
void kernel_2412251213_08(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        C[i_0+1][i_0] = E[i_0] + E[i_0-1] + F[i_0] - F[i_0] + F[1] + F[i_0] - E[i_0] - 5;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            B[i_0-1][i_1-1] = G[i_1+1][i_0] + H[i_1][i_0+1] * G[i_1][i_0] + I[i_0][i_1][i_0] - G[i_1][i_0] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            C[i_2+1][i_2] = G[i_2][i_3] - H[i_3-1][i_2] + B[i_2][i_3] + 4;
        }
        D[i_2] = E[i_2] + F[i_2-1] + F[i_2-1] + F[i_2] * E[i_2] + D[i_2-1] * D[i_2+1] + D[i_2-1] + 4;
    }
#pragma endscop
...
kernel_2412251213_08(xc,yc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
