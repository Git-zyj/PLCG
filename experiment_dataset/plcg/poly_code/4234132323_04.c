...
void kernel_4234132323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(i_0, 4); i_1 < PB_M; i_1++) {
            A[i_1-2] = C[i_0] + D[i_1][i_1] * A[i_1-4] + A[i_1-1] - A[i_1-3] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            A[i_3-1] = D[i_3+2][i_2] * D[i_3][i_2-1] + C[i_3] + D[i_2][i_3] + 5;
        }
    }
#pragma endscop
...
kernel_4234132323_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
