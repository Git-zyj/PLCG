...
void kernel_3226312311_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_Q; i_2++) {
            for (int i_3 = 2; i_3 < PB_N; i_3++) {
                B[i_1+2][i_2-2][i_3] = C[i_3][i_2-2] - B[i_1+1][i_3-2][i_2] * B[i_1+1][i_2-2][i_3] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3226312311_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
