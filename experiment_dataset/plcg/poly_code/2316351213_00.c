...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            B[i_2+1] = F[i_2][i_1-1] * D[i_2] * G[i_2-1][i_2-1] - D[i_2+i_1] * G[i_1+1][i_2] + H[i_2][i_1+1] - B[i_2] - 2;
        }
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_3] = I[0] - F[i_3][i_1] * H[i_3+1][i_1] * H[i_3][i_1-1] - A[i_3-1] * 3;
        }
    }
#pragma endscop
...
kernel_2316351213_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
