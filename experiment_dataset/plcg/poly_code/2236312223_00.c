...
void kernel_2236312223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            A[i_0][i_0+1] = C[i_1][i_0-1] - A[i_1-1][i_1] + A[i_0-1][i_0] * 3;
        }
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            B[i_0][i_2] = B[i_0-2][i_2] - B[i_2][i_0+2] - 3;
        }
    }
#pragma endscop
...
kernel_2236312223_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
