...
void kernel_3234151221_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_1] = A[i_0-2] + A[i_1-2] * A[i_1-1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_P+1, PB_L); i_3++) {
            for (int i_4 = 1; i_4 < min(-i_2+PB_P-1, i_3); i_4++) {
                B[i_4][i_3+i_2-1][i_2] = C[i_2][i_4][i_3] - D[i_3][0] - C[i_3][i_4][i_3] * C[i_4+i_3+1][i_4][1] * D[i_4][i_3] + C[i_2][i_4][0] * D[i_4][i_3-1] * C[i_4-1][i_2+1][i_3] + C[i_4][i_2][i_3] * B[i_3][i_4+i_2+1][i_4+2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3234151221_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
