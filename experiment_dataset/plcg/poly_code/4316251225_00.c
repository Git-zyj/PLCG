...
void kernel_4316251225_00(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < -i_0+PB_P; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_2+PB_L+2, PB_L-1); i_3++) {
                    C[i_0+1][i_1][i_3+i_2-2][i_2+i_0] = D[i_1+1][i_0-1][i_2][i_2] * D[i_1][i_2-1][i_0][i_3] * E[i_0][i_2][i_2][i_1] - E[i_1][i_0][i_3-1][i_2+1] * D[i_2-1][i_0][i_3+1][i_0] + F[i_0][i_1][i_2][i_3][i_3] + 2;
                }
            }
        }
        for (int i_4 = i_0; i_4 < PB_L-1; i_4++) {
            B[i_4-1][i_4-1] = G[i_0+1][i_4] + G[i_0][i_4-1] + H[i_4][i_0] + H[i_4][i_0] + G[i_4+1][i_0-1] * I[i_4] + B[i_0-2][i_0-2] * 6;
            for (int i_5 = 4; i_5 < -i_0+PB_L-2; i_5++) {
                for (int i_6 = 0; i_6 < min(-i_5+PB_P-2, -i_0+PB_L-1); i_6++) {
                    C[i_0+1][i_4][i_6+i_5][i_6+i_0] = D[i_5][i_6+i_0+1][i_4+1][1] + E[i_4][i_5][i_6+i_0][i_4-1] + C[i_6+1][i_4][i_5+i_0+2][i_6+i_5+2] * C[i_0+1][i_4][i_6+i_5-4][i_6+i_0-2] * 5;
                }
            }
        }
    }
...
init_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4316251225_00(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
