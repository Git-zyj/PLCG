...
void kernel_3326311221_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        A[i_0-1] = 6;
        for (int i_1 = 3; i_1 < i_0; i_1++) {
            B[i_0+1][i_1-1] = D[i_1-1][i_0] - B[i_0+2][i_1-1] - B[i_0+1][i_1-3] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < PB_M-1; i_3++) {
            C[i_3-1] = E[i_3][i_2] - E[i_2-1][0] - C[i_3] + C[i_3-3] * C[i_3+1] - 1;
        }
    }
#pragma endscop
...
kernel_3326311221_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
