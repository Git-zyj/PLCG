...
void kernel_4316252225_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_1][i_0] = D[i_0+1][0] * D[i_1][i_0] - E[i_1][i_0] + E[i_1+i_0][0] * D[i_0-1][i_1] * A[i_1][i_0+2] * A[i_1-2][i_0-2] + 1;
            A[i_1][i_0] = E[i_1][i_0+2] - D[i_1+2][i_0-1] * E[i_1-2][i_0] + F[i_1] - E[i_1][i_0+1] * A[i_1][i_0+2] * 4;
        }
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 0; i_3 < i_2; i_3++) {
                for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                    C[i_2][i_0][i_3][i_4] = G[i_4+1][i_3][i_2][1] - H[i_2][i_0][i_3] + G[i_2+1][i_4][0][i_3] - I[i_4][i_3][i_0-2][i_3] + 6;
                }
            }
        }
    }
...
kernel_4316252225_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
