...
void kernel_2436352215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L-2 - 1; i_0 >= 2; i_0--) {
        A[i_0] = E[i_0+2] * F[i_0+1] * A[i_0-1] + B[i_0] * 2;
        B[i_0] = E[i_0-2] + F[i_0] * B[i_0+1] + 5;
    }
    for (int i_1 = 2; i_1 < PB_Q; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L; i_2++) {
            C[i_1][i_2] = G[i_1-1][i_2] + H[i_2][i_1-2] * H[i_1][i_1] + I[i_2][i_1] - I[i_2+2][i_1-2] + C[i_1+1][i_2] + C[i_2][i_1+1] * C[i_1][i_2-1] + 5;
        }
    }
    for (int i_3 = 0; i_3 < PB_Q; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_Q; i_4++) {
            D[i_4] = H[i_3][i_3] + I[i_3][i_4] + G[i_4][i_3] + E[i_3] + G[i_4][i_3] - H[i_4+1][i_3] + I[i_3][i_4-2] * H[i_3+2][i_4] + I[i_4][i_4] + J[i_4] * D[i_4-1] - 3;
        }
    }
#pragma endscop
...
kernel_2436352215_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
