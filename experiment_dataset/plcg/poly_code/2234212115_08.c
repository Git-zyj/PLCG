...
void kernel_2234212115_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = max(2, i_0); i_2 < PB_L; i_2++) {
            A[i_0-1][i_0-1] = C[i_2][0] + D[i_0][i_2-2] - 1;
        }
    }
#pragma endscop
...
kernel_2234212115_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
