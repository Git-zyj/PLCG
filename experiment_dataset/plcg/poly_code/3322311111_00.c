...
void kernel_3322311111_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
            B[i_4+i_3][i_4] = D[i_3][i_4-1] * B[i_4+i_3-1][i_4-1] + 1;
        }
    }
...
kernel_3322311111_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
