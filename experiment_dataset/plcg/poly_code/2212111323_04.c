...
void kernel_2212111323_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(PB_P, -i_0+PB_L+1); i_1++) {
            A[i_0+1][i_1-1] = C[i_1+1][i_1+i_0-1] - A[i_0+1][i_1] + A[i_0+3][i_1-2] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_P+1, i_2); i_3++) {
            B[0] = A[i_2+1][i_3-1] + B[0] - B[0] * B[0] + 4;
        }
    }
#pragma endscop
...
kernel_2212111323_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
