...
void kernel_3332251315_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0+1] - D[i_0+1] * D[i_0] - D[i_0+1] * E[i_0-1] * A[i_0+1] + A[i_0-1] - 2;
        A[i_0] = E[i_0+1] - F[i_0][1] - E[i_0+1] * E[i_0+1] + D[i_0] * E[i_0-1] + A[i_0-1] * A[i_0] + 2;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            for (int i_3 = 1; i_3 < PB_P; i_3++) {
                C[i_1][i_3][i_1] = F[i_2+i_1][i_3-1] * G[i_3][1] * C[i_1+1][i_3][i_1+1] + C[i_1+1][i_3-1][i_1+1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3332251315_09(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
