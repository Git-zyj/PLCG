...
void kernel_2224111311_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
            B[i_2] = C[0] * B[i_2+1] + B[i_2-1] + B[i_2-1] * B[i_2+1] + 6;
        }
    }
#pragma endscop
...
kernel_2224111311_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
