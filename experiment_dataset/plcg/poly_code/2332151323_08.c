...
void kernel_2332151323_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
            A[i_1-1] = D[i_1][i_0] * D[i_0-1][i_0-1] * D[i_1][i_0] + D[i_0][i_1+1] + A[i_0] + A[i_1-2] * 4;
        }
        for (int i_2 = 0; i_2 < min(PB_N, -i_0+PB_P+1); i_2++) {
            C[i_2][i_2+i_0-1] = E[i_0] * D[i_2][i_0] * D[i_2+i_0][i_2] * E[i_0] - E[i_2] + D[i_0][i_2] - D[i_0][i_0] + A[i_0-2] - 2;
        }
    }
    for (int i_3 = 3; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_P, PB_N+1); i_4++) {
            C[i_3][i_4+i_3-1] = D[i_3+1][0] + D[i_4][i_3] + E[i_4-1] * C[i_3-1][i_4+i_3-2] - C[i_3][i_4+i_3-3] + C[i_4+1][i_4+i_3] + 1;
        }
    }
#pragma endscop
...
kernel_2332151323_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
