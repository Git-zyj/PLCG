...
void kernel_3136111111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(min(PB_Q-1, i_0), -i_0+PB_P-1); i_1++) {
            A[i_1-1][i_1+i_0+1] = B[i_0][i_1][i_1+1] + A[i_0-2][i_1+i_0] + 1;
        }
    }
#pragma endscop
...
kernel_3136111111_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
