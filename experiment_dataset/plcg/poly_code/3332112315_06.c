...
void kernel_3332112315_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            A[i_1] = A[i_1+1] * A[i_0-1] - C[i_1][i_0+1] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, PB_L-2); i_3++) {
            A[i_3-1] = C[i_2-1][i_3+2] - 4;
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
            C[i_4][i_5+2] = D[i_5] + C[i_4][i_5+1] * 3;
        }
    }
#pragma endscop
...
kernel_3332112315_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
