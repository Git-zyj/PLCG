...
void kernel_4126112321_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-4; i_1++) {
            for (int i_2 = i_0; i_2 < -i_0+PB_L+2; i_2++) {
                A[i_0][i_2][i_1+2] = B[i_2+i_0-2][i_1] * A[i_0][i_2-1][i_1+2] + A[i_0-1][i_2-2][i_1+2] - A[i_0-1][i_2-1][i_1+4] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4126112321_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
