...
void kernel_2314252211_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 3; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_0+1][i_1-2] = D[i_1][i_0] - E[i_0+1] + D[i_1][i_0] * D[i_1][i_0] + F[i_1+i_0][i_1+1] * A[i_0+1][i_1-3] * C[i_0+1][i_0+1] * 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = PB_M-2 - 1; i_3 >= 1; i_3--) {
            B[i_3][i_3] = E[i_3] - G[0] + G[i_3] + B[i_3-1][i_3-1] + C[i_2+1][i_2+1] * 5;
            C[i_2][i_2] = D[i_3+2][i_2+2] + F[i_3+2][i_2] - F[i_3][i_2+2] - H[i_2][i_3][i_2] * E[i_2] - B[i_3][i_3] - B[i_3+1][i_3+1] + 3;
        }
    }
#pragma endscop
...
kernel_2314252211_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
