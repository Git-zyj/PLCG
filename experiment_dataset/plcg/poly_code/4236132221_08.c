...
void kernel_4236132221_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 4; i_1 < PB_M; i_1++) {
            A[i_1-2] = C[i_1][i_0] * A[i_1-4] * 1;
        }
    }
        ...
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            for (int i_4 = i_3; i_4 < PB_L; i_4++) {
                for (int i_5 = 2; i_5 < -i_3+PB_M-1; i_5++) {
                    B[i_3][i_5][i_2] = D[i_3][i_5][i_2][i_4+1] - E[i_3][i_3-2][i_4][i_5-2][i_2-2] * D[i_3-2][i_4-1][i_2+1][i_5] * D[i_5+i_3+1][i_2][i_4][i_5+2] - D[i_2][i_5][i_5-2][i_4-2] + B[i_3+1][i_5][i_4] - B[i_5-2][i_4][i_3] * 1;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), INIT_SEED);
kernel_4236132221_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E)));
