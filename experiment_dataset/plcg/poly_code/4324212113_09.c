...
void kernel_4324212113_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < PB_Q; i_2++) {
                A[i_0][i_1-1][i_2] = A[i_0+1][i_2-1][i_1-1] + 1;
            }
        }
    }
    ...
    for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
        for (int i_5 = 0; i_5 < -i_4+PB_L; i_5++) {
            for (int i_6 = 0; i_6 < min(min(PB_N, -i_5+PB_M), -i_4+PB_P); i_6++) {
                C[i_6+i_5][i_5+i_4][i_6+i_4] = D[i_4+1][i_6][i_5] + C[i_6+i_5][i_5+i_4-1][i_6+i_4-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4324212113_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
