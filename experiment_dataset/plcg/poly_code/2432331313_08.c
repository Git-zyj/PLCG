...
void kernel_2432331313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] * F[i_0] - F[1] + A[i_0-1] * D[i_0+1] + 2;
    }
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            A[i_2] = G[i_2+1][i_1+1] - A[i_2-1] + 5;
        }
        for (int i_3 = 1; i_3 < min(-i_1+PB_M, PB_P); i_3++) {
            C[i_1-1][i_3][i_1] = H[i_3][i_1] + G[i_1-1][i_3-1] - I[i_3+i_1][i_3-1] + J[i_3][i_1-1][i_3-1] - A[i_1] * C[i_3-1][i_1+1][i_3] + 1;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        D[i_4] = F[i_4-1] * F[1] * A[i_4] + A[i_4-1] - D[i_4+1] + 3;
    }
#pragma endscop
...
kernel_2432331313_08(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
