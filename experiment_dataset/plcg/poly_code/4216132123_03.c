...
void kernel_4216132123_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                B[i_1-2][i_2+2][i_0+2] = 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                B[i_5][i_4][i_3] = C[i_5][i_4+2] * C[i_5][i_5] * D[i_4][i_3][0] + D[i_4][i_5-1][i_3+2] * D[i_4][i_3-2][i_3] + B[i_4-1][i_5][i_3+1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4216132123_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
