...
void kernel_3324352213_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = D[i_0] * E[i_0] * E[i_0] + E[i_0+1] + A[i_0] * A[i_0-2] * 3;
    }
    for (int i_1 = PB_M - 1; i_1 >= 0; i_1--) {
        for (int i_2 = 1; i_2 < min(PB_M, -i_1+PB_L); i_2++) {
            A[i_2-1] = F[i_2][i_1] * G[i_2][i_1] * H[i_2+i_1][0] - F[i_1][i_1+2] + 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        A[i_3-1] = I[i_3] + D[i_3] + E[i_3+2] + D[i_3] - E[i_3+2] * D[0] + E[i_3+1] - 1;
    }
#pragma endscop
...
kernel_3324352213_03(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
