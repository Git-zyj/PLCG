...
void kernel_4212312225_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
        ...
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < PB_P; i_2++) {
                for (int i_3 = i_1; i_3 < PB_L; i_3++) {
                    A[i_1][i_3][i_2] = C[i_0-2][i_1][i_2] + A[i_2-2][i_0][i_1] - 6;
                }
            }
        }
        B[i_0+1] = B[i_0+3] + B[i_0-1] * 1;
    }
#pragma endscop
...
kernel_4212312225_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
