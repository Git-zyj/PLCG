...
void kernel_4412251125_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
                for (int i_3 = 2; i_3 < min(PB_P-1, -i_1+PB_P); i_3++) {
                    A[i_3-1][i_1][i_2][i_3+1] = E[i_3][i_0-1][i_2][i_2] + F[i_3][i_0+1][i_3+i_1][i_2-1] + E[i_2+1][i_3][i_1+1][i_0] + A[i_3-2][i_1][i_2+1][i_3] * 4;
                }
            }
        }
        for (int i_4 = 1; i_4 < -i_0+PB_M; i_4++) {
            B[i_4] = G[i_4-1][i_0] * H[i_4+i_0][0] * I[i_4] * B[i_4+1] * 3;
        }
    }
    for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
        B[i_5] = J[i_5-1] + I[i_5-1] + J[i_5] * I[i_5] - J[i_5] * 1;
        B[i_5-2] = G[i_5][i_5] + I[i_5+1] + I[0] - I[i_5-1] * I[i_5] - J[i_5+1] - I[i_5] - J[i_5-1] * J[i_5] * 5;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4412251125_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
