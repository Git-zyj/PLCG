...
void kernel_4432111211_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P-1; i_1++) {
            A[i_1+1][i_1+i_0] = E[i_1+i_0][0] - A[i_1+2][i_1+i_0+1] - A[i_1][i_1+i_0] - A[i_1+1][i_1+i_0+1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_N-1, i_2); i_3++) {
            for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
                B[i_3][i_4] = B[i_3-1][i_4] + B[i_3+1][i_4] * B[i_4+1][i_3-1] - B[i_4-1][i_3+1] + 4;
            }
        }
        for (int i_5 = 0; i_5 < PB_M; i_5++) {
            for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
                C[i_5][i_2][i_6-1] = C[i_6+1][i_5][i_2-1] - 6;
            }
        }
...
kernel_4432111211_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
