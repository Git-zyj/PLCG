...
void kernel_2336152123_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0+1] - D[i_0+1] - D[i_0] * D[i_0-1] * 4;
        B[i_0-2] = D[i_0] + D[i_0] * D[i_0-2] - D[i_0+2] - A[i_0] * A[i_0+2] + 2;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        A[i_1] = D[i_1-2] - D[i_1] * D[i_1] - D[i_1] * D[i_1-2] * 3;
    }
#pragma endscop
...
kernel_2336152123_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
