...
void kernel_4434352225_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_L, PB_P-2); i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_Q, PB_P-2); i_2++) {
                for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                    A[i_0][i_1][i_2+2][i_3] = E[i_3][i_2-2][i_1][i_2-1] + F[i_3+1][i_1][i_2] * E[i_1][i_3][i_0-2][i_2] + G[i_1+i_0][i_3][i_1][i_2+i_0][i_2+2] + A[i_1+1][i_0][i_2+2][i_3] - A[i_0-1][i_2-1][i_1+2][i_3] * A[i_3][i_1+1][i_2+2][i_0+2] - A[i_0][i_1+1][i_2+2][i_3] + A[i_0-1][i_1-2][i_2+2][i_3] + 2;
                }
            }
        }
    }
    for (int i_4 = PB_M-1 - 1; i_4 >= 2; i_4--) {
        C[i_4] = H[i_4] + I[i_4+1] - I[i_4] * J[i_4+1] + I[i_4] * I[i_4+1] + I[i_4] - C[i_4+1] - 4;
        C[i_4] = H[i_4] * J[i_4+1] * I[0] + I[i_4] - J[i_4-2] - 4;
    }
    for (int i_5 = 0; i_5 < PB_P; i_5++) {
        for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
            for (int i_7 = 2; i_7 < min(PB_P-2, i_5); i_7++) {
                for (int i_8 = 2; i_8 < PB_P; i_8++) {
                    D[i_7][i_6][i_8][i_5] = K[i_7-2][i_6+1][i_8][i_7] * K[i_7][i_6][i_8-2][0] * L[i_6-1][i_7+2][i_5] * 4;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4434352225_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
