...
void kernel_2416212211_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        A[i_0+2] = A[i_0+3] + A[i_0+1] * 1;
        for (int i_1 = max(1, i_0); i_1 < min(-i_0+PB_Q, PB_M-1); i_1++) {
            B[i_1][i_1] = E[i_0][i_1][i_1+i_0] - B[i_1+1][i_1+1] - B[i_1-1][i_1-1] - B[i_1-1][i_1-1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        C[i_2] = C[i_2-1] + 2;
        for (int i_3 = i_2; i_3 < PB_P-1; i_3++) {
            D[i_2-1][i_3+1] = C[i_2] + D[i_2-1][i_3] + 5;
        }
    }
#pragma endscop
...
kernel_2416212211_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
