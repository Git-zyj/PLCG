...
void kernel_3326111123_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M+1, PB_L-1); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                B[i_1+i_0-1][i_2][i_0] = D[i_1-1][i_0+1][i_0] + B[i_2+i_0][i_1+1][i_2] * 5;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L; i_3++) {
            for (int i_4 = max(3, i_3); i_4 < -i_0+PB_M; i_4++) {
                B[i_4+i_0][i_3][i_4] = C[i_0][i_4-3] * 5;
            }
            C[i_0][i_3-1] = 2;
        }
    }
...
kernel_3326111123_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
