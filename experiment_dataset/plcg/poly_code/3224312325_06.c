...
void kernel_3224312325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_L-2); i_1++) {
            A[i_0][i_1] = C[i_0-2] * A[i_0][i_1+2] - A[i_0][i_1-1] - A[i_0+2][i_1] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            for (int i_4 = 2; i_4 < PB_P-1; i_4++) {
                B[i_3][i_2][i_4] = B[i_3-1][i_2][i_4] * B[i_3][i_2][i_4-2] + B[i_3][i_2][i_4+1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3224312325_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
