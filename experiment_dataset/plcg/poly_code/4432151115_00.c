...
void kernel_4432151115_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
        ...
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_Q; i_3++) {
                    A[i_3+1][i_1][i_0-1] = E[i_3][i_1][i_0+1][i_3+i_2] * E[i_0][i_2][i_1+1][i_1-1] + F[i_3][i_1][i_0] - A[i_3+1][i_2+1][i_1-1] + A[i_1+1][i_2-1][i_3-1] * 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        B[i_4] = G[1] - G[i_4] * G[i_4] + G[i_4] - B[i_4-1] + 2;
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        B[i_5] = G[i_5] - G[i_5] * G[i_5] - G[i_5-1] * 1;
    }
    for (int i_6 = 1; i_6 < PB_L; i_6++) {
        for (int i_7 = 1; i_7 < -i_6+PB_N; i_7++) {
            D[i_7+i_6][i_6+1] = H[i_6-1][i_7+1] * G[i_7+i_6] - H[i_7-1][i_6] + H[i_6][i_7] + H[i_7][0] - H[i_7][i_6+1] + G[i_7] * 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4432151115_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
