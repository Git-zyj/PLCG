...
void kernel_3222232221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = C[0] * D[i_0] * D[i_0] * A[i_0-1] + 2;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_1][i_0] = E[i_0][i_0] + C[i_1] - B[i_1-1][i_0] * B[i_1][i_0-1] * 1;
        }
    }
#pragma endscop
...
kernel_3222232221_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
