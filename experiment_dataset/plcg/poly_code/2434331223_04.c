...
void kernel_2434331223_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        A[i_0][i_0] = E[i_0+1] * F[i_0-1] * A[i_0-2][i_0-2] + A[i_0+2][i_0+2] * 1;
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            B[i_1+1] = G[i_1][i_0+1] * E[i_1] - H[i_1-1] - B[i_1+2] - B[i_1] - B[i_1-1] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
            C[i_3] = F[i_3+1] + I[i_3][i_3] + I[i_3][i_2] - C[i_3+1] * C[i_3+2] + 3;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        D[0] = E[i_4] * E[i_4-1] - E[0] + D[0] - 5;
    }
#pragma endscop
...
kernel_2434331223_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
