...
void kernel_3234112311_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L+1; i_0++) {
        A[i_0-2] = A[i_0-1] * A[i_0-3] * A[i_0-1] + 6;
    }
    for (int i_1 = 1; i_1 < PB_Q; i_1++) {
        for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
            for (int i_3 = 0; i_3 < min(min(PB_L-1, i_1), -i_2+PB_P); i_3++) {
                B[i_3][i_2][0] = C[i_1][i_3+i_2][i_2] + C[i_3+2][i_1-1][i_2+1] - B[i_3+1][i_1][0] * B[i_3][i_2+1][0] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3234112311_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
