...
void kernel_3212211225_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_P - 1; i_0 >= 1; i_0--) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L-2; i_2++) {
                A[i_1][i_0] = A[i_2+2][i_1] - 5;
            }
        }
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            A[i_3+2][i_0-1] = C[i_0][i_3+1] + A[i_3+2][i_0] * 5;
        }
    }
#pragma endscop
...
kernel_3212211225_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
