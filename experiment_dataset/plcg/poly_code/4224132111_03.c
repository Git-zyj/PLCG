...
void kernel_4224132111_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_N; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                A[i_1][i_0+1][i_1-2] = C[i_1-2][i_0][0] + D[i_2-2][i_1] * C[i_2][i_0][i_1] + C[i_2-1][i_0+2][0] * C[i_2][i_0][i_1] * A[i_1-1][i_0+1][i_1-3] - 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_M-1; i_4++) {
            for (int i_5 = 2; i_5 < min(PB_N, -i_4+PB_P+1); i_5++) {
                A[i_5][i_4+1][i_5-2] = C[i_3-1][i_5+i_4-1][i_5+i_4-2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4224132111_03(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
