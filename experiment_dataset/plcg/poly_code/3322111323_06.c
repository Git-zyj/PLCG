...
void kernel_3322111323_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            A[i_1][i_0] = A[i_0][i_1+1] + A[i_1+1][i_0-2] * A[i_0+2][i_1-2] * 2;
        }
    }
    ...
    for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
        C[i_5] = D[i_5] * C[i_5+1] + C[i_5-1] + C[i_5-2] - 4;
    }
#pragma endscop
...
kernel_3322111323_06(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
