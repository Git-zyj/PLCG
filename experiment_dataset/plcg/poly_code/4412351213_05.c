...
static void print_array(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4412351213_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_M+1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_L, i_0); i_2++) {
                for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                    A[i_2][i_1-1][i_1] = E[i_1+i_0-1][i_3][i_2][i_2] * F[i_2][i_1][i_0][i_3] * E[i_2][i_3-1][i_1+1][1] + G[i_3+1][i_2+i_0][i_1-1][0] + A[i_3+1][i_2-1][i_2] - A[i_3][i_2][i_2+1] - 6;
                }
            }
        }
        for (int i_4 = 1; i_4 < min(i_0, PB_M-1); i_4++) {
            for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                for (int i_6 = i_0; i_6 < min(-i_5+PB_Q, -i_0+PB_P-1); i_6++) {
                    A[i_6][i_0-2][i_0-1] = F[i_0][i_5][i_6][i_4] + E[i_6+1][i_5][i_6][i_5] * H[i_4-1][i_6+i_0+1][i_6+i_5][i_0][i_0] + G[i_4+1][i_0][i_4][i_0+1] * F[i_4][i_5-1][i_6][i_0] + F[i_5+1][i_0][i_4][i_5+1] - G[i_5][i_6][i_4-1][i_0] * I[i_6][i_5][i_4] - 1;
                }
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_M-1; i_7++) {
        for (int i_8 = 1; i_8 < -i_7+PB_L; i_8++) {
            C[i_8][i_7-1] = J[i_8+i_7][i_8+i_7-1] + K[i_7][i_8] * L[i_8][i_7] + C[i_7+1][i_8-1] * 4;
        }
        D[i_7-1] = M[i_7] + N[i_7] * D[i_7-2] + D[i_7] + 3;
    }
#pragma endscop
