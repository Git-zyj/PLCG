...
void kernel_4236112221_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 3; i_1 < min(i_0, PB_L+1); i_1++) {
            A[i_0-2][i_1-1] = A[i_0-2][i_1-3] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            B[i_3] = C[i_3][i_2] + C[i_2][i_3-2] - A[i_3-2][i_2-1] * A[i_2-2][i_3-2] + B[i_3+1] + 5;
        }
    }
#pragma endscop
...
kernel_4236112221_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
