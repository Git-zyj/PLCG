...
void kernel_3234312123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1+1] = C[i_1] * A[i_1-1] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 3; i_3 < PB_Q-2; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_L+1, -i_2+PB_M); i_4++) {
                B[i_2][i_3][i_4+i_3-1] = D[i_4+i_2][i_3-1][i_3+2] + B[i_2][i_3][i_4+i_3-3] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3234312123_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
