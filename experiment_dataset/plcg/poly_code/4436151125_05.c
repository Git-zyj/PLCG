...
void kernel_4436151125_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = E[i_0] + E[i_0+1] * E[i_0] - E[i_0-1] + E[i_0] * 6;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        A[i_1-1] = E[0] + E[i_1-1] * E[i_1-1] - E[i_1] - E[i_1+1] * E[i_1+1] + 5;
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            A[i_3] = F[i_3+1][i_2] * G[i_2][i_3][i_3] - F[i_3][i_2] * F[i_3+1][i_2-1] * F[0][i_3+1] - 1;
            for (int i_4 = i_2; i_4 < PB_P; i_4++) {
                for (int i_5 = 2; i_5 < PB_M; i_5++) {
                    D[i_5][i_2][i_3][i_5] = H[i_5][i_4][i_2-1][i_3] + G[i_3+1][i_5][i_2] + D[i_4-2][i_2-2][i_5-2][i_4-2] * D[i_4-2][i_2][i_5][i_4-2] * 6;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436151125_05(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
