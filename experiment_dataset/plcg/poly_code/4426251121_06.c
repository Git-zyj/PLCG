...
void kernel_4426251121_06(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        C[i_0+1] = E[i_0] * E[i_0] * E[i_0+1] + E[i_0] * F[i_0-1] + E[i_0-1] + 3;
        B[i_0+1] = F[i_0+1] + E[i_0] - F[i_0] + 1;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        C[i_1+1] = E[i_1] - E[1] * E[i_1] * E[i_1] + F[i_1] + F[i_1] * E[i_1+1] + C[i_1+2] * 2;
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                for (int i_5 = 0; i_5 < i_2; i_5++) {
                    D[i_2+1][i_3][i_4][i_5] = G[i_3-1][i_4][i_5][i_2+1] + 4;
                }
            }
        }
    }
...
init_array(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4426251121_06(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
