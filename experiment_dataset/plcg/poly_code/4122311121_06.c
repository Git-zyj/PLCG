...
void kernel_4122311121_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_M+1, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
                    A[i_1+1][0][i_2] = B[i_3+i_2][i_1+i_0-1][0] - A[i_1+1][0][i_3-1] * 3;
                }
            }
        }
    }
...
kernel_4122311121_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
