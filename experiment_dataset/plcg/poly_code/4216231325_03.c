...
void kernel_4216231325_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_0][i_0][i_1+1] = C[i_2][i_0][i_1+i_0] - C[i_2][i_0][i_2] + D[i_2][i_0][i_1] + D[i_1][i_2][i_0] + A[i_0-2][i_0-2][i_1+1] + A[i_0][i_0][i_1-1] + A[i_0][i_0][i_1+2] * A[i_0-2][i_0-2][i_1+2] * A[i_0][i_0][i_1] - 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        B[i_3-1] = E[i_3] - E[i_3-1] - B[i_3-2] + 4;
    }
#pragma endscop
...
kernel_4216231325_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
