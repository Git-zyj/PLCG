...
void kernel_4412311215_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P; i_1++) {
            A[i_0-1][i_1+1][i_1+i_0-1] = A[i_1][i_0+1][i_1+i_0] * A[i_0][i_1+1][i_1+i_0] - 1;
            for (int i_2 = 0; i_2 < PB_N; i_2++) {
                A[i_0][i_1+1][i_1+i_0] = 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        C[i_3][0] = E[i_3+1] * C[i_3+1][0] * 1;
        C[i_3][0] = 3;
    }
#pragma endscop
...
kernel_4412311215_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E));
