...
void kernel_3216211315_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L-1; i_1++) {
            A[i_0+1][i_1] = A[i_0+1][i_1-1] * A[i_0+1][i_1+1] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_L-1; i_3++) {
            for (int i_4 = 1; i_4 < i_2; i_4++) {
                B[i_3][i_4][i_2] = C[i_3][i_4+1] * B[i_3][i_4-1][i_2] + B[i_3+1][i_4+1][i_2] + B[i_3-1][i_4-1][i_2+1] * B[i_4-1][i_3+1][i_2+1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3216211315_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
