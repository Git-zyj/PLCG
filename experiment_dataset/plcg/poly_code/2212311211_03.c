...
void kernel_2212311211_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
        B[i_2+1] = C[i_2] + B[i_2] - B[i_2+2] * B[i_2+2] - 3;
    }
#pragma endscop
...
kernel_2212311211_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
