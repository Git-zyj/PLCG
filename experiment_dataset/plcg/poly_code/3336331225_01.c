...
void kernel_3336331225_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        B[i_0][i_0-1] = D[i_0+1] - E[i_0] - D[i_0] * 1;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = 3; i_2 < PB_M-1; i_2++) {
            B[i_2][i_2-1] = F[i_1][i_2+1][0] - G[i_2][i_1] * H[i_2][0] * G[i_2][i_1] - G[i_1+1][i_2] + B[i_2-2][i_2-3] * 5;
        }
        for (int i_3 = 1; i_3 < i_1; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_1+PB_M, i_3); i_4++) {
                B[i_3][i_3-1] = F[i_3][i_4+i_1][i_4] + B[i_1][i_1-1] + 5;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3336331225_01(xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
