...
void kernel_4214112323_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-3; i_1++) {
            A[i_0][i_1+2] = A[i_0][i_1+3] - A[i_0][i_1] + A[i_0-1][i_1+1] * 6;
        }
    }
        ...
        for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
            for (int i_4 = i_2; i_4 < PB_M; i_4++) {
                for (int i_5 = 1; i_5 < PB_L; i_5++) {
                    B[i_2][i_3+1][i_5][i_2+1] = C[i_5-1][i_3-2][i_4][i_2-1] - B[i_2][i_3-1][i_5][i_2+1] * B[i_4][i_2+3][i_5][i_4+1] * 3;
                }
            }
        }
    }
...
kernel_4214112323_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
