...
void kernel_3226211223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_L+1); i_1++) {
            for (int i_2 = i_1; i_2 < min(-i_1+PB_L-3, PB_Q-1); i_2++) {
                A[i_2+i_1][i_2] = A[i_1+i_0-1][i_0] * A[i_2+i_1+3][i_2+1] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_L, PB_P); i_4++) {
            A[i_4+i_3][i_3] = C[i_4+i_3][0] + C[i_3+1][i_4-1] - 2;
        }
    }
#pragma endscop
...
kernel_3226211223_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
