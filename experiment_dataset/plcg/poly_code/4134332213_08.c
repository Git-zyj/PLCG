...
void kernel_4134332213_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-2; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, PB_M-3); i_2++) {
                A[i_0+2][i_1][i_2-1] = B[i_2][i_1-1] + C[i_1][i_2+i_0] - B[i_1+2][i_2] + A[i_1+1][i_2][i_0-1] * A[i_2+3][i_1+1][i_0-1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4134332213_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
