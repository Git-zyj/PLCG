...
void kernel_2336351221_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = D[i_0+1] - D[i_0-1] * D[i_0] + D[i_0] - A[i_0+1] - A[i_0-1] * 2;
    }
    for (int i_1 = PB_L - 1; i_1 >= 2; i_1--) {
        B[i_1] = E[i_1+1] + D[i_1] - E[i_1] * E[i_1-1] + B[i_1+2] * B[i_1-2] * 6;
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_N+1, PB_L); i_3++) {
            B[i_3] = F[i_3+1][i_2] - D[i_3] + G[i_2][0] - G[i_3-1][i_3-1] + H[i_3+i_2-1][i_3-1] - 5;
        }
    }
#pragma endscop
...
kernel_2336351221_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
