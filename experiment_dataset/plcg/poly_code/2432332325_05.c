...
void kernel_2432332325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+2; i_0++) {
        A[i_0] = E[i_0-2] * F[2] * F[i_0+1] + A[i_0+1] + A[i_0-1] * A[i_0-2] - 5;
    }
    for (int i_1 = 2; i_1 < PB_N-2; i_1++) {
        B[i_1] = E[i_1-2] * E[i_1+2] - E[i_1] + B[i_1+2] - B[i_1-2] * B[i_1+1] * 4;
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_L-2; i_3++) {
            A[i_3-2] = G[i_3+2][i_2+2] - H[i_3+i_2+2][i_2] * 3;
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        B[i_4-2] = I[i_4-2] * I[i_4] - 6;
    }
#pragma endscop
...
kernel_2432332325_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
