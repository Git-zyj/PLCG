...
void kernel_2332112321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < PB_N; i_3++) {
            B[0] = D[i_3-1][i_2-2] + B[0] - B[0] * B[0] + 2;
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-3; i_5++) {
            C[i_4][i_5+2] = C[i_4+2][i_5+3] * C[i_4-2][i_5+2] + C[i_4+1][i_5+2] + 4;
        }
    }
#pragma endscop
...
kernel_2332112321_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
