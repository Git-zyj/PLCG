...
void kernel_3212152123_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            for (int i_2 = max(2, i_0); i_2 < min(PB_P-1, -i_0+PB_P); i_2++) {
                A[i_0][i_1][i_2] = C[i_2-2][i_0][i_1] * C[i_0][i_2][i_1] * C[i_1][i_0][i_2+i_0] + C[i_1+i_0][i_2+1][i_1] - D[i_1+i_0][i_2] + A[i_0][i_1][i_2-1] - 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_Q; i_4++) {
            B[1][i_4+i_3] = E[i_3] + D[i_4][i_3] - E[i_4] * D[i_4+1][i_3] + B[1][i_4+i_3-1] * 1;
        }
    }
#pragma endscop
...
kernel_3212152123_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
