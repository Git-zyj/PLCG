...
void kernel_4332152115_01(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 3; i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_2+PB_L+2, i_1); i_3++) {
                    B[i_0][i_1+1][i_2-3][i_0] = D[i_2][i_3-1][i_1][i_2] + D[i_3][i_0+2][i_1-2][i_1-2] * D[i_3][i_2-2][i_0][i_1] - D[i_1][i_3+i_2-2][i_0][i_1] - E[i_3][i_2][i_0-2][i_1][i_2] + 5;
                }
            }
        }
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_4+PB_M-1, -i_0+PB_M+1); i_5++) {
                for (int i_6 = 0; i_6 < PB_L; i_6++) {
                    B[i_0][i_4][i_5-2][i_0] = D[i_5+i_0-1][i_6][i_4][0] * F[i_0][i_5+i_4-2][i_6] + F[i_5+i_4+1][i_6][i_5+2] * D[i_0][i_4][i_5][i_6] - D[i_4+1][i_5-2][i_6][i_0] + D[i_5][i_0][i_6][i_4] - D[i_6][i_5][i_0+2][i_4] + B[i_0-1][i_4][i_5-2][i_0-1] + 3;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
        for (int i_8 = 2; i_8 < -i_7+PB_P; i_8++) {
            for (int i_9 = 1; i_9 < -i_7+PB_L-1; i_9++) {
                B[i_7-1][i_9-1][i_8-1][i_7-1] = F[i_9+2][i_7][i_8+i_7] - F[i_8-2][i_9+i_7+1][i_9] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4332152115_01(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
