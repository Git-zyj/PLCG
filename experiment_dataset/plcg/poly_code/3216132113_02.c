...
void kernel_3216132113_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, i_1); i_2++) {
                A[i_1][i_2+i_0][0] = A[i_2-1][i_1+i_0][0] - 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_M, -i_3+PB_L+1); i_4++) {
            for (int i_5 = 2; i_5 < min(min(-i_4+PB_M, PB_M-2), i_3); i_5++) {
                A[i_3][i_5+i_4][0] = C[i_4-1][i_5][i_4+i_3-1] * D[i_5][i_3] * D[i_4-1][i_5-2] - C[i_4+i_3-1][i_5+2][i_4-2] - D[i_5+1][i_4] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3216132113_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
