...
void kernel_3212152223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 4; i_1 < i_0; i_1++) {
            A[i_0-1][i_1-2] = C[i_1][i_0] * C[i_1][i_0] * C[i_1-1][i_0] * C[i_0+1][i_1] - C[i_1+1][i_0+1] * A[i_0-3][i_1-4] * A[i_0+1][i_1-2] + 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_L, i_2); i_3++) {
            B[i_2] = C[i_2][i_3] + C[i_3][i_2-2] - C[i_2+2][i_3] * C[i_2][i_3+2] - C[i_3-1][i_3+i_2] + B[i_3+1] * 6;
        }
    }
#pragma endscop
...
kernel_3212152223_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
