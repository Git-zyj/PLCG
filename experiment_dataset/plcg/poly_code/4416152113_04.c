...
void kernel_4416152113_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0-2] = E[i_0] - E[i_0] + E[i_0] * A[i_0-1] * 3;
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            B[i_1][i_0] = F[i_1][i_0] * F[i_1-2][i_0+1] * E[i_1] * 1;
        }
    }
        ...
        for (int i_3 = i_2; i_3 < min(-i_2+PB_L, PB_M-2); i_3++) {
            for (int i_4 = 3; i_4 < PB_L; i_4++) {
                for (int i_5 = 0; i_5 < PB_M; i_5++) {
                    D[i_4-3][i_5][i_5] = G[i_2][i_3+1][i_4] * H[i_3][i_4][i_5][i_2] * H[i_2][i_4-1][i_3-1][i_5] - G[i_5][i_3][i_4] - H[i_3][i_5][i_3+i_2][i_2] - 2;
                }
            }
            for (int i_6 = 0; i_6 < -i_3+PB_M-2; i_6++) {
                for (int i_7 = 2; i_7 < PB_M; i_7++) {
                    D[i_7-2][i_6][i_6] = H[i_6][i_2][i_3+1][i_7+1] * H[i_6][i_3][i_7][i_2] * H[i_6+i_3][i_2-2][i_7-2][i_2] + H[i_6+i_3+2][i_2][i_7+2][i_2] + H[i_7+i_6][i_3+2][i_2+1][i_7-1] * I[i_7][i_3+i_2][i_6+2][i_3][i_6] * 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4416152113_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I)));
