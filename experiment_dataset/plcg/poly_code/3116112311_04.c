...
void kernel_3116112311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(min(PB_N-1, i_0), -i_0+PB_M+2); i_1++) {
            for (int i_2 = i_1; i_2 < PB_L; i_2++) {
                A[i_1][0] = B[i_1+i_0-2][i_2-1] - A[i_1+1][0] * A[i_1-1][0] - A[i_1+1][0] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3116112311_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
