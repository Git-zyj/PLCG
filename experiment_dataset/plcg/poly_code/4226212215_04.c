...
void kernel_4226212215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = 0; i_2 < PB_L-3; i_2++) {
            for (int i_3 = i_0; i_3 < PB_L; i_3++) {
                B[i_0][i_2+2][i_3] = B[i_0][i_2+3][i_3-1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4226212215_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
