...
void kernel_3312331313_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0-1] = D[i_0] - D[1] - A[i_0-2] + A[i_0] - A[i_0-2] + A[i_0-2] + 6;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        B[i_1] = E[i_1+1] - F[i_1] + E[i_1] + A[i_1-1] + 5;
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            for (int i_3 = i_1; i_3 < -i_1+PB_Q; i_3++) {
                C[i_2][i_3][i_2] = G[i_2][i_3+i_1][i_1] - G[i_2-1][i_1][i_3] + H[i_2][i_3+1][i_1-1] * C[i_2][i_3+1][i_2] - C[i_2+1][i_3-1][i_2+1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3312331313_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
