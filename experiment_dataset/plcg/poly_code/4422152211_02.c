...
void kernel_4422152211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        A[i_0] = E[i_0] * E[i_0+2] * E[i_0] - A[i_0-1] + 5;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
                C[i_3-1][i_2] = F[i_3][i_1-1][i_2+2] - G[i_3][i_3-1] - F[i_1-2][i_3][i_2-2] + F[i_3+2][i_2][i_1] + F[i_3][i_2+i_1][i_1] + F[i_3-2][i_1+1][i_2] + F[i_3][i_2][i_1] * H[i_3-1][i_2+i_1-1][0][i_2] * F[i_1][i_3][i_2] * 5;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            C[i_4][i_5] = G[i_5][i_4-1] + E[i_4] + E[i_4-2] + C[i_4+1][i_5] + C[i_4+1][i_5-1] * C[i_4-1][i_5] + 5;
        }
        for (int i_6 = 1; i_6 < -i_4+PB_Q; i_6++) {
            for (int i_7 = 1; i_7 < PB_M; i_7++) {
                for (int i_8 = 1; i_8 < PB_M-1; i_8++) {
                    D[i_7][i_4][i_6][i_8+2] = H[i_6][i_7][i_4][i_8+1] - I[i_7][i_6][i_8-1][i_4-2][i_6+i_4] + H[i_8+1][i_7][2][i_4] + H[i_8][i_6][i_4][i_4+1] - H[i_8+1][i_4][i_6][i_7] + D[i_6-1][i_4][i_7-1][i_8+3] + D[i_7][i_8][i_6-1][i_4+2] * 3;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4422152211_02(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I)));
