...
void kernel_3434331221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        B[i_1] = E[i_1] + F[i_1+1][i_1-1] + E[i_1] + G[i_1] - E[i_1-1] * B[i_1-2] - 3;
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_L; i_3++) {
            C[i_2][i_2][0] = H[i_3-1][i_2] - G[i_2] + C[i_3-2][i_3-2][0] - C[i_2+1][i_2+1][0] + 6;
            D[i_3] = B[i_3] * B[i_3-1] - 1;
        }
    }
#pragma endscop
...
kernel_3434331221_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
