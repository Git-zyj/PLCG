...
#endif
static void init_array(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn),int xo,int yo,int zo,int vo,DATA_TYPE POLYBENCH_4D(O,xO,yO,zO,vO,xo,yo,zo,vo),int xp,int yp,int zp,int vp,int wp,DATA_TYPE POLYBENCH_5D(P,xP,yP,zP,vP,wP,xp,yp,zp,vp,wp),int xq,int yq,int zq,int vq,DATA_TYPE POLYBENCH_4D(Q,xQ,yQ,zQ,vQ,xq,yq,zq,vq),int seed)
{
srand(seed);
    ...
    for (i_0 = 0; i_0 < xq; i_0++) {
        for (i_1 = 0; i_1 < yq; i_1++) {
            for (i_2 = 0; i_2 < zq; i_2++) {
                for (i_3 = 0; i_3 < vq; i_3++) {
                    fprintf(POLYBENCH_DUMP_TARGET, "\n");
                    fprintf (POLYBENCH_DUMP_TARGET, DATA_PRINTF_MODIFIER, Q[i_0][i_1][i_2][i_3]);
                }
            }
        }
    }
...
void kernel_4412351113_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn),int xo,int yo,int zo,int vo,DATA_TYPE POLYBENCH_4D(O,xO,yO,zO,vO,xo,yo,zo,vo),int xp,int yp,int zp,int vp,int wp,DATA_TYPE POLYBENCH_5D(P,xP,yP,zP,vP,wP,xp,yp,zp,vp,wp),int xq,int yq,int zq,int vq,DATA_TYPE POLYBENCH_4D(Q,xQ,yQ,zQ,vQ,xq,yq,zq,vq)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        C[i_0-1] = E[i_0] - E[i_0-1] * F[i_0-1] * G[i_0] - E[i_0] + G[i_0] + 6;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 2; i_3 < -i_2+PB_L+1; i_3++) {
                C[i_1-2] = H[i_1][i_3+i_2-1][i_3-1][i_1-1] + I[i_1][i_3+i_2][i_1] * J[i_2][i_1][i_3] + K[i_2-1][i_3-1][i_3-1] + K[i_2][i_1][i_2] * C[i_3-2] + 1;
            }
            C[i_2-1] = L[i_2-1][i_1-1] + L[i_2][i_1-1] + M[i_1+1][i_2] * L[i_2-1][i_1] * N[i_2+1][i_1+1] + 6;
        }
        for (int i_4 = i_1; i_4 < min(-i_1+PB_Q+1, PB_L); i_4++) {
            for (int i_5 = 0; i_5 < -i_1+PB_L; i_5++) {
                for (int i_6 = 0; i_6 < PB_P; i_6++) {
                    D[i_5+i_1][i_4][i_6] = O[i_5][i_4][i_1][0] - P[i_4][i_5+i_1][i_5][i_6][i_4+1] - Q[i_5][i_6][i_1][i_4+i_1-1] * 3;
                }
            }
        }
    }
...
POLYBENCH_4D_ARRAY_DECL(Q, DATA_TYPE, xQ,yQ,zQ,vQ,xq,yq,zq,vq);
init_array(xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,yn,POLYBENCH_ARRAY(N), xo,yo,zo,vo,POLYBENCH_ARRAY(O), xp,yp,zp,vp,wp,POLYBENCH_ARRAY(P), xq,yq,zq,vq,POLYBENCH_ARRAY(Q), INIT_SEED);
kernel_4412351113_02(xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,yn,POLYBENCH_ARRAY(N), xo,yo,zo,vo,POLYBENCH_ARRAY(O), xp,yp,zp,vp,wp,POLYBENCH_ARRAY(P), xq,yq,zq,vq,POLYBENCH_ARRAY(Q));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M), xn,yn,POLYBENCH_ARRAY(N), xo,yo,zo,vo,POLYBENCH_ARRAY(O), xp,yp,zp,vp,wp,POLYBENCH_ARRAY(P), xq,yq,zq,vq,POLYBENCH_ARRAY(Q)));
