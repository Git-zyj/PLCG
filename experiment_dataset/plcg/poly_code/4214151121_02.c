...
void kernel_4214151121_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_1][i_2][i_0] = C[i_0+1][i_1-1][i_2] + D[i_0][i_1][i_2][i_1] + C[i_1][i_0][i_1] * A[i_1+2][i_2][i_0] - 2;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_N; i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                for (int i_6 = 0; i_6 < min(-i_3+PB_M, PB_L); i_6++) {
                    B[i_4][i_5][i_5] = D[i_4][i_3-1][i_6][i_4+1] - D[i_4][1][i_5][i_3] + D[i_4][i_6+i_3][i_5][i_6+1] * E[i_3][i_6][i_4][i_5][i_6] + D[i_3][i_6][i_6][i_4-1] * C[i_6][i_4-1][i_3] - 4;
                }
            }
        }
    }
...
kernel_4214151121_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E));
