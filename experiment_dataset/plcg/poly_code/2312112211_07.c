...
void kernel_2312112211_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M; i_1++) {
            A[i_0][i_1-2] = D[i_0][i_0] * A[i_0+1][i_1-2] - A[i_0-1][i_1-1] - 5;
            B[i_1][i_0] = D[i_0][i_1-1] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            C[i_3] = B[i_3][i_2-1] * C[i_3-1] * C[i_3+1] + 1;
        }
    }
#pragma endscop
...
kernel_2312112211_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
