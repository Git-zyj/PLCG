...
void kernel_2322151113_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0-1] + D[i_0] * D[i_0] * D[0] * D[i_0-1] + D[i_0-1] - 3;
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_N); i_1++) {
            A[i_1-1] = E[i_1-1][i_0] - E[i_1][0] + E[i_1+i_0][i_1-1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M+1; i_3++) {
            C[i_3] = E[i_2+1][i_3] + E[i_2][i_3-1] + E[i_2-1][i_3] * D[i_3+i_2-1] + E[i_2+1][i_3+1] - C[i_3-1] - 1;
        }
    }
#pragma endscop
...
kernel_2322151113_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
