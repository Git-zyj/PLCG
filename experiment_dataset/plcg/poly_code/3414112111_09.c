...
void kernel_3414112111_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_1][i_2-1] = E[i_2-1][i_2-1] + B[i_2][i_1-2] + C[i_2] + 3;
        }
        C[i_1] = C[i_1-1] * 1;
        for (int i_3 = i_1; i_3 < -i_1+PB_M; i_3++) {
            D[i_1+2][i_3] = E[i_1-2][0] * E[i_3][i_3] + E[i_3+i_1][i_3] * 2;
        }
    }
#pragma endscop
...
kernel_3414112111_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
