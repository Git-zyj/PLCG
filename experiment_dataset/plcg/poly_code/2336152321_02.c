...
void kernel_2336152321_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-3; i_0++) {
        A[i_0] = D[i_0-1] - D[i_0+1] * D[i_0-2] * D[i_0-1] - D[i_0] + 6;
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_L-1); i_1++) {
            B[i_0+2][i_1] = E[i_1+i_0+1][i_1] + A[i_0] * A[i_1] + A[i_0+1] * B[i_0+3][i_1] + B[i_0+1][i_1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        A[i_2] = D[i_2] - D[i_2+2] * D[i_2] + D[i_2-1] + D[i_2+2] + D[i_2-1] - D[i_2] * D[i_2+2] + 6;
    }
#pragma endscop
...
kernel_2336152321_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
