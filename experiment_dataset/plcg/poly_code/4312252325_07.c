...
void kernel_4312252325_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0+1] = D[i_0] * D[i_0] - A[i_0] + A[i_0+2] * A[i_0-1] - 2;
    }
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_M, PB_M-2); i_2++) {
            A[i_2+1] = E[i_1][i_2+1] + F[i_1+2][i_2] + E[i_2+i_1][i_2] - F[i_2][i_1-1] - G[i_2][i_1][i_1] * E[i_1][i_1] * 3;
            for (int i_3 = 1; i_3 < -i_1+PB_M-2; i_3++) {
                for (int i_4 = 1; i_4 < PB_L; i_4++) {
                    C[i_3+i_1+2][i_2][0][i_4] = H[i_2][i_1][i_4][i_1] + G[i_4][i_1-1][i_2-1] - I[i_2][i_4-1][i_3+2][i_1] + H[i_1][i_4][i_3][i_2] * I[i_4][i_1+2][i_3+i_2+1][i_4] + G[i_4-1][i_4][i_2-2] * C[i_4+i_3-1][i_2][0][i_1+2] - C[i_3+i_2][i_4][0][i_1] + C[i_3+i_1+2][i_2+2][0][i_4] + 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4312252325_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
