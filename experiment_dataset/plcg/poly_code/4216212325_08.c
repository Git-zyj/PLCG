...
void kernel_4216212325_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = C[i_0] - A[i_0+3] * A[i_0] * A[i_0+4] + 4;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = max(3, i_1); i_2 < PB_N; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                B[i_2-1][i_3] = B[i_2-3][i_3+1] - B[i_2][i_3+2] + B[i_2-1][i_3+1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4216212325_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
