...
void kernel_3234111111_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q+1; i_1++) {
            A[i_0][i_1+1][i_1-1] = C[i_0-1][i_1-1] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q+2; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                B[i_4][i_2][i_2] = C[i_4][i_2] + A[i_4][i_2][i_2-2] + B[i_4-1][i_2][i_2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3234111111_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
