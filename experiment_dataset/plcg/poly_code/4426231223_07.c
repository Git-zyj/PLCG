...
void kernel_4426231223_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_Q+1, -i_1+PB_P); i_3++) {
                    A[i_2-1][i_1-1][i_3+i_0-1][i_0] = E[i_2][i_3][i_0][i_1-1] + F[i_3][i_1+1][i_1] - E[i_0][i_2][i_3+i_1][i_1-1] * 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N-3; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            B[i_4][i_5+1][i_5] = G[i_5][i_4+1] - G[i_5][0] + G[i_4][i_5] + H[i_5][i_4] + B[i_4][i_5+3][i_5+2] * B[i_4][i_5+2][i_5+1] - 4;
        }
        C[i_4+1] = I[i_4-1] + I[i_4+1] - I[i_4] * C[i_4+3] - 1;
    }
    for (int i_6 = 1; i_6 < PB_M+1; i_6++) {
        for (int i_7 = 0; i_7 < PB_P; i_7++) {
            for (int i_8 = 0; i_8 < min(i_6, PB_P); i_8++) {
                B[i_8][i_7+1][i_7] = B[i_6-1][i_8+1][i_8] - 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4426231223_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
