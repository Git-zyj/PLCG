...
void kernel_4314352115_00(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < -i_0+PB_Q-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_Q; i_2++) {
                for (int i_3 = 0; i_3 < i_0; i_3++) {
                    C[i_3][i_1+2][i_2][i_0-2] = D[i_3][i_0][i_1][i_2-2] + 5;
                }
            }
            for (int i_4 = 2; i_4 < PB_M; i_4++) {
                for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                    C[i_5+1][i_1+1][i_4][i_0-3] = E[i_4+2][i_0][i_1][0] + D[i_0][i_4][i_5+i_1+2][i_5] * F[i_5][i_4-2][i_1+i_0+2][i_0+1] * E[i_4][i_5][i_1][2] - C[i_4][i_0+1][i_1][i_5-1] * 6;
                }
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_Q-2; i_6++) {
        for (int i_7 = 2; i_7 < PB_M-2; i_7++) {
            for (int i_8 = 0; i_8 < min(PB_Q, -i_6+PB_P+2); i_8++) {
                for (int i_9 = 2; i_9 < -i_7+PB_M; i_9++) {
                    C[i_7][i_6+2][i_8][i_9-2] = D[i_9][i_6+2][i_7][i_8] * D[i_9+2][i_6][i_8][i_7-2] - D[i_9+i_7][i_6+2][i_8][0] * F[i_7+2][i_8][i_6+2][i_6] + D[i_7-1][i_9][i_6][i_8] + F[i_7][i_8+i_6-2][i_9+1][0] + G[i_9][i_9][0] * F[i_9][i_6][i_7][i_8] + 6;
                }
            }
        }
    }
...
init_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4314352115_00(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
