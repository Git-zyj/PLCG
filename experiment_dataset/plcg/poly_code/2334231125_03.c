...
void kernel_2334231125_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        B[i_0-1] = D[1] + D[i_0] + D[i_0] + B[i_0-1] - 4;
    }
    for (int i_1 = 3; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            B[i_1-1] = E[i_2][i_1] - E[i_1][0] * 5;
        }
        B[i_1-3] = E[i_1][0] * D[i_1-1] * F[i_1-1] + D[i_1] - 6;
    }
#pragma endscop
...
kernel_2334231125_03(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
