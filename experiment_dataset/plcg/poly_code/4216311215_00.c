...
void kernel_4216311215_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            A[i_0-1][i_1+1] = C[i_1][i_0] * A[i_0][i_1+1] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, PB_L-2); i_3++) {
            for (int i_4 = 1; i_4 < min(PB_N-1, -i_3+PB_N); i_4++) {
                for (int i_5 = max(max(0, -i_3+1), -i_4+1); i_5 < min(min(-i_3+PB_N+1, -i_4+PB_N-1), PB_L-1); i_5++) {
                    B[i_4+i_3][i_5][i_5+1] = B[i_5+i_4+1][i_3][i_3+1] * B[i_5+i_4-1][i_3+1][i_3+2] * B[i_5+i_3-1][i_4-1][i_4] + 4;
                }
            }
        }
    }
...
kernel_4216311215_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
