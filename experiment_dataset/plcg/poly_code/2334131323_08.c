...
void kernel_2334131323_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_0-1][i_1] = D[i_0-1][i_1+1] + D[i_1][i_0+1] - D[i_1-1][i_0] - A[i_0-3][i_1] * A[i_0-1][i_1-2] - A[i_0+1][i_1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            A[i_2][i_3] = D[i_2+1][i_3] * D[i_3][i_2] * D[i_3+1][i_2-1] + 3;
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
            C[i_4][i_5-1] = D[i_4][i_5+1] + C[i_4-2][i_5-2] - C[i_4-1][i_5-1] + 5;
        }
    }
#pragma endscop
...
kernel_2334131323_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
