...
void kernel_2316332225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = D[i_0] - E[2] * 1;
        for (int i_1 = 4; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            B[i_0-2][i_1-2] = F[i_1][0] * G[i_1-2][i_0+2] - 4;
            B[i_0-2][i_1-4] = G[i_1-2][i_0-2] * F[i_1+i_0][i_0] + H[i_1+1][i_0][0] * A[i_0] - A[i_1+1] * B[i_1-2][i_0-2] * 6;
        }
    }
#pragma endscop
...
kernel_2316332225_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
