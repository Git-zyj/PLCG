...
void kernel_3314132321_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_P-2, -i_0+PB_L-1); i_1++) {
            for (int i_2 = i_1; i_2 < -i_0+PB_M; i_2++) {
                A[i_2+i_0][i_1-1][i_2+1] = D[i_1][i_0][i_2] - D[i_2][i_1][i_0+2] + D[i_1][i_0][i_2] - D[i_2][i_0+1][i_1] + D[i_1][i_2][i_0] - A[i_2+i_0][i_1][i_2+1] + A[i_2+i_0][i_1-3][i_2+1] * A[i_2+i_0-2][i_1-3][i_2+1] - 1;
            }
            for (int i_3 = 2; i_3 < min(PB_M, -i_1+PB_L); i_3++) {
                B[i_0-1][i_1][i_3] = D[i_3][i_1+i_0+1][i_1+2] + D[i_0][i_3][0] * D[i_0+1][i_3+i_1][i_1] + B[i_0][i_1][i_3-1] + B[i_0+1][i_1][i_3] + C[i_3-2][i_1-1] - 6;
            }
        }
    }
    for (int i_4 = 4; i_4 < PB_M+3; i_4++) {
        for (int i_5 = 2; i_5 < min(i_4, PB_M+2); i_5++) {
            C[i_5-2][i_4] = C[i_4-4][i_5] - C[i_5-2][i_4+1] + C[i_4-3][i_5+2] * 2;
        }
    }
#pragma endscop
...
kernel_3314132321_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
