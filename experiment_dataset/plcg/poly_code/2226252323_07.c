...
void kernel_2226252323_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L-2 - 1; i_0 >= 2; i_0--) {
        for (int i_1 = 0; i_1 < min(PB_P-2, -i_0+PB_M+1); i_1++) {
            A[i_0][i_1] = C[i_0][i_1] + D[i_1][i_0+2] + D[i_1][i_0-2] - C[i_1+i_0-1][i_1] - A[i_0][i_1+2] + 4;
        }
        for (int i_2 = 1; i_2 < PB_L-3; i_2++) {
            B[i_0+1][i_2+1] = D[i_2+2][i_0] * D[i_2][i_2] + D[i_2][i_0-1] * C[i_2][i_0] - C[i_0+2][2] * A[i_2+2][i_0] + B[i_2+1][i_0-1] + B[i_0+1][i_2+3] + B[i_0+1][i_2-1] * 5;
        }
    }
#pragma endscop
...
kernel_2226252323_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
