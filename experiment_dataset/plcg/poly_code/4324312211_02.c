...
void kernel_4324312211_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                A[i_0][i_1][i_2] = D[i_0+1][i_2][i_1] * A[i_0][i_1][i_2+1] - A[i_0-1][i_1+1][i_2-1] * 3;
            }
        }
        ...
        for (int i_5 = 0; i_5 < PB_M; i_5++) {
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                for (int i_7 = 1; i_7 < PB_M+1; i_7++) {
                    C[i_5][i_6][i_5] = C[i_7-1][i_5][i_7-1] + 3;
                }
            }
        }
    }
...
kernel_4324312211_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
