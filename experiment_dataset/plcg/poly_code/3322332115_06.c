...
void kernel_3322332115_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_1] = D[i_0][i_1-2] - D[i_1-1][i_0] + E[i_0] + E[i_0] * D[i_1-1][i_0] * A[i_1+1] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        A[i_2] = F[i_2+1] * 2;
        for (int i_3 = 0; i_3 < min(PB_L-2, -i_2+PB_M); i_3++) {
            A[i_3] = G[i_3+i_2][0] * F[i_3] * H[i_2-1][i_3+2][2] * 6;
        }
    }
#pragma endscop
...
kernel_3322332115_06(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
