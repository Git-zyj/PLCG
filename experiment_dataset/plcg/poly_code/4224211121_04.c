...
void kernel_4224211121_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_Q-2; i_3++) {
            B[i_2][i_3+i_2] = C[i_2][i_2-1] * B[i_2+2][i_3+i_2+2] - 2;
        }
    }
#pragma endscop
...
kernel_4224211121_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
