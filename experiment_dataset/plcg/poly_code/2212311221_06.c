...
void kernel_2212311221_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        ...
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_2][i_0] = B[i_2][i_0+1] - B[i_2][i_0-1] + B[i_2-1][i_0] * 4;
        }
    }
#pragma endscop
...
kernel_2212311221_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
