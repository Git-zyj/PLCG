...
void kernel_4432252125_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_Q; i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_0+PB_L, -i_2+PB_M), PB_M-2); i_3++) {
                    A[i_2-1][i_1+2][i_0][i_2+1] = E[i_3+2][i_0][i_1][i_2-2] + F[i_1][i_2][i_3+i_0][2][i_3] - G[i_3+i_2][i_1+1][i_0][i_2+i_0] + 1;
                }
            }
        }
        B[i_0] = H[i_0] + I[i_0] + H[i_0-1] * H[i_0] + I[i_0] * B[i_0-2] * 3;
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        B[i_4+1] = I[i_4-1] + I[i_4+1] - I[i_4+1] + H[i_4] * I[0] + I[i_4] - 4;
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            for (int i_6 = 1; i_6 < min(-i_4+PB_M+1, -i_5+PB_L-1); i_6++) {
                D[i_6+1][i_5+1][i_4] = J[i_6+i_4][i_5-1][i_4] + K[i_4][i_6+2][i_5] + K[i_6+i_4-1][i_6+i_5+1][0] * K[i_5][i_4][i_6-1] - K[i_4-1][i_5][i_6] * L[i_5][i_6] + 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4432252125_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
