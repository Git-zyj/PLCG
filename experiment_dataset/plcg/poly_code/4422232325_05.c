...
void kernel_4422232325_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[2][i_1] = E[0] * E[i_0-1] - A[2][i_1-1] - D[i_1][i_0+3] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-3; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M-3; i_3++) {
            for (int i_4 = i_2; i_4 < -i_2+PB_M; i_4++) {
                for (int i_5 = 2; i_5 < -i_4+PB_L; i_5++) {
                    B[i_4+i_2][i_3][i_5+i_3][i_4] = F[i_4+2][i_2][i_5][0] * G[i_2][i_5+1][i_3][i_4+2] + H[i_2][i_5-2][i_3][i_4][i_2] + F[i_5+i_4-1][i_2][i_3][i_3+2] * F[i_4][i_2+2][i_5][i_3] - I[i_2+1][i_4][i_3] - G[i_4][i_2+2][i_5+i_3][0] * G[i_3+1][i_2][i_5+1][i_4] + B[i_3+i_2+3][i_5][i_5+i_4][i_3+1] - B[i_4+i_2][i_3][i_5+i_3-1][i_4] * 2;
                }
            }
        }
        for (int i_6 = 2; i_6 < PB_M; i_6++) {
            C[i_6][i_2] = J[i_2-1][i_6] - C[i_6-2][i_2] - C[i_6][i_2+1] - C[i_6-1][i_2] - D[i_6][i_2+2] - D[i_6][i_2+1] * D[i_6][i_2+3] * 6;
            D[i_6][i_2+2] = K[i_6][i_2-2] + C[i_6][i_2-2] - 5;
        }
    }
#pragma endscop
