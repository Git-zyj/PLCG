...
void kernel_4424152213_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - E[0] * E[i_0] + E[i_0-1] * E[i_0] + E[i_0] - A[i_0+1] + A[i_0-1] + A[i_0-1] + 1;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        B[i_1-1] = E[i_1-2] + E[i_1-2] * E[i_1-1] * E[i_1] + E[2] + E[i_1] * B[i_1] + B[i_1-2] * 2;
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                C[i_2][i_3][i_2+1] = F[i_4][i_3][i_2] + 5;
            }
        }
        D[i_2] = E[i_2+2] * E[i_2] * G[i_2][i_2] - E[i_2] + E[i_2+1] * G[i_2][i_2] + D[i_2-1] * 5;
    }
#pragma endscop
...
kernel_4424152213_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
