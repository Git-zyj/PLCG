...
void kernel_3414211311_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                A[i_2][0] = A[i_2+1][0] * A[i_2-1][0] * A[i_2-1][0] * A[i_2-1][0] * 5;
            }
        }
    ...
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        ...
        for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
            D[i_3][i_4+1] = E[i_4] + C[i_4+1] - D[i_3+1][i_4+1] - D[i_3+1][i_4] + D[i_3][i_4+2] - D[i_3-1][i_4+1] + D[i_3+1][i_4+2] * D[i_4][i_3+2] + 2;
        }
    }
#pragma endscop
...
kernel_3414211311_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
