...
void kernel_2414251115_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[0] * F[i_0-1] - A[i_0-1] + 6;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        A[i_1] = F[i_1-1] - F[i_1] - G[i_1-1][i_1-1] * E[i_1] + E[i_1] - 6;
        for (int i_2 = i_1; i_2 < -i_1+PB_M; i_2++) {
            C[i_1-1][i_2-1] = F[0] + H[i_1][i_2+1] * G[i_1+1][i_2] * I[i_2+i_1][i_2-1][i_1] + G[i_2][i_1] + C[i_1-2][i_2-2] * 3;
        }
        for (int i_3 = 1; i_3 < -i_1+PB_M+1; i_3++) {
            A[i_1+1] = H[i_1][i_3] - F[i_3] + G[i_3][i_3+i_1-1] - H[i_3][i_1+1] + H[i_3+1][i_1-1] * H[i_3][0] * G[i_3][i_1+1] + H[i_1][i_3-1] * 2;
        }
    }
#pragma endscop
...
kernel_2414251115_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
