...
void kernel_3222132313_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-3; i_1++) {
            A[i_0][i_1+2] = C[i_1-1][i_0-2] + C[i_0][i_1] + C[i_1-1][i_0-1] + C[i_1+1][i_0] * A[i_0+1][i_1+2] + A[i_0][i_1+3] * 3;
        }
        for (int i_2 = i_0; i_2 < PB_P-2; i_2++) {
            A[i_0][i_2+2] = C[i_0+1][i_2+1] * C[i_0][i_2] * A[i_2][i_0+2] - 5;
        }
    }
#pragma endscop
...
kernel_3222132313_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
