...
void kernel_4234312225_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = max(0, -i_1+1); i_2 < min(PB_P-3, -i_1+PB_M+1); i_2++) {
                A[i_1][i_0+2][i_2+1] = C[i_0+1][i_2][i_2+i_1-1] - A[i_1+1][i_0+2][i_2+3] * A[i_1-2][i_2+2][i_0+1] + 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = i_3; i_5 < min(PB_P-1, -i_3+PB_L-1); i_5++) {
                A[i_4][i_3][i_5+1] = D[i_4][i_5+i_3+1][i_5] + 3;
            }
        }
    }
#pragma endscop
...
kernel_4234312225_04(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
