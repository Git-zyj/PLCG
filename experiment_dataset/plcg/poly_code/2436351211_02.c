...
void kernel_2436351211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - F[i_0-1] + E[i_0-1] + G[i_0] + E[i_0] + E[i_0+1] + A[i_0+1] - A[i_0-1] + A[i_0+1] * 5;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M+1, PB_M-1); i_2++) {
            B[i_2][i_1] = H[i_2+1][i_1][i_2] * F[i_2] + I[i_2+1][i_1] * J[i_2][i_1] - D[i_1-1] * 2;
            B[i_2-1][i_1] = K[i_2][i_1] - I[i_2+i_1-1][1] + J[i_2][i_1] + J[i_2+1][i_1] - D[i_2-1] * 6;
        }
        D[i_1] = E[i_1+1] - F[i_1+1] + F[0] + G[1] * 3;
    }
#pragma endscop
...
kernel_2436351211_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
