...
void kernel_2436332321_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] - A[i_0-2] * A[i_0+2] * D[i_0+1] + 6;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_M-1; i_2++) {
            B[i_2+i_1][i_2] = F[i_2-1][i_1-1] + F[i_2][i_1] + F[i_2+1][i_1] + F[i_2][i_1] - B[i_2+i_1-1][i_2-1] - B[i_2+i_1+1][i_2] - 5;
        }
        for (int i_3 = i_1; i_3 < PB_N; i_3++) {
            D[i_3-1] = E[i_3] * G[i_3][i_1-1] * 6;
        }
    }
    for (int i_4 = 3; i_4 < PB_M-1; i_4++) {
        D[i_4-1] = E[i_4] + H[i_4-1] * D[i_4-3] + D[i_4-2] - D[i_4] * D[i_4+1] - 2;
    }
#pragma endscop
...
kernel_2436332321_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
