...
void kernel_3426351113_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < -i_0+PB_N; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_2] = E[i_2][i_0+1][i_0] * E[i_1][i_2+1][i_0+1] + E[i_1][i_0][0] + B[i_0] * 4;
            }
            B[i_1] = F[i_1+i_0][i_1] * G[i_0+1][i_1-1] - H[i_0][i_1] + F[i_0][i_1] - I[i_1][i_0][1] - 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                C[i_4+1][i_5-1] = E[i_4][i_3][1] - I[i_4-1][i_3][i_5] * J[i_4][i_5][i_3] * E[i_4][i_5][0] * 3;
            }
        }
        D[i_3] = K[i_3+1] * L[i_3] * M[i_3+1] + G[i_3-1][i_3+1] + M[i_3-1] - K[i_3] - 6;
    }
#pragma endscop
