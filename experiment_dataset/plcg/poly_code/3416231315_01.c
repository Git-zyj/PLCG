...
void kernel_3416231315_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                A[i_1][i_0+1][i_2+1] = E[i_2][i_1][i_1+1] - F[i_2][i_1+1] * E[i_1][i_0][i_2] - A[i_2-1][i_0+2][i_1+1] * A[i_1][i_0+2][i_2] + A[i_2-1][i_0+1][i_1+1] * A[i_1][i_2][i_0] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        D[i_3-1] = G[i_3] - D[i_3] * 1;
        for (int i_4 = i_3; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                A[i_5][i_4+2][i_3] = E[i_3+1][i_5-1][i_4] + F[i_3][i_5] - H[i_3][i_5][i_5] * 2;
            }
        }
        D[i_3] = I[i_3] - G[i_3] - D[i_3-1] - 4;
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3416231315_01(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
