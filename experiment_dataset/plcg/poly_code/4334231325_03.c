...
void kernel_4334231325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        A[i_0+1] = D[i_0] * E[0] - D[i_0] * D[i_0] - A[i_0] * A[i_0+2] + 3;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_N-2, i_1); i_2++) {
            A[i_2+1] = F[i_2-1][i_1] - G[i_2][i_1] + A[i_2+2] - A[i_2] - 1;
        }
        for (int i_3 = i_1; i_3 < PB_P+1; i_3++) {
            for (int i_4 = 2; i_4 < min(PB_M, -i_1+PB_P-1); i_4++) {
                C[i_1+1][i_4+i_1][i_4] = H[i_4][i_1][i_3-1] + C[i_1+1][i_4+i_1+1][i_4+1] + C[i_1+1][i_4+i_1-2][i_4-2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4334231325_03(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
