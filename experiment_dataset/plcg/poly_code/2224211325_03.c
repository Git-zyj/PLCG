...
void kernel_2224211325_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_M-2; i_3++) {
            B[i_3] = C[i_3-1] * B[i_3+1] - B[i_3+2] + B[i_3-2] * 6;
        }
    }
#pragma endscop
...
kernel_2224211325_03(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
