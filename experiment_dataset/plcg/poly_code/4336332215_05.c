...
void kernel_4336332215_05(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < min(-i_0+PB_Q+2, PB_Q-1); i_1++) {
            for (int i_2 = PB_L - 1; i_2 >= 1; i_2--) {
                for (int i_3 = max(2, i_2); i_3 < PB_Q; i_3++) {
                    C[i_3-2][i_2][i_1] = D[i_0][i_3][i_1-1][i_2-1] * E[i_2][i_3-2][i_0][i_1+1] - F[i_3-1][i_2][i_1+i_0-2][i_3] * 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-3; i_4++) {
        for (int i_5 = 0; i_5 < min(-i_4+PB_M, PB_P-1); i_5++) {
            B[i_4+2] = G[i_5][0] - G[i_5+i_4][i_5+1] + B[i_4+3] * 6;
        }
    }
    for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
        for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
            for (int i_8 = 3; i_8 < PB_P; i_8++) {
                C[i_8-2][i_7][i_6] = H[i_7+2][i_8][i_6] * I[i_6][i_7][i_8] * C[i_8-3][i_7+1][i_6] + C[i_8-3][i_7][i_6] + C[i_8-3][i_7][i_6+1] + 3;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,zc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4336332215_05(xc,yc,zc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,yc,zc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
