...
void kernel_4234151321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_1][i_0] = C[i_1+i_0][1] - C[i_0-1][0] - D[i_1-1][i_0][i_1+1] - C[i_0][i_1] * C[i_1+1][i_0] * A[i_1+2][i_0] - A[i_1-1][i_0] * A[i_1][i_0-2] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-3; i_2++) {
        for (int i_3 = 3; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            B[i_2+1][i_3-1] = E[i_3+i_2] - C[i_3+i_2][i_3+1] + C[i_3+1][i_2-1] + C[i_3][i_2] * C[i_2][i_3] * B[i_2+1][i_3-3] - B[i_2+3][i_3-1] - 1;
        }
    }
#pragma endscop
...
kernel_4234151321_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
