...
void kernel_3234131325_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_Q, -i_1+PB_L); i_2++) {
                B[i_0-3][i_2][i_1-1] = C[i_0][i_0][0] - C[i_2+i_1][i_0][0] * C[i_0][i_1][i_2] - C[i_2][i_0][i_1] + 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L; i_4++) {
            for (int i_5 = 3; i_5 < PB_Q; i_5++) {
                B[i_5-1][i_4][i_3-1] = C[0][i_3][i_5] + B[i_5-3][i_4][i_3+1] - B[i_5+1][i_4][i_3-1] - B[i_5-1][i_4-1][i_3] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3234131325_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
