...
void kernel_4436212211_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
        ...
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = 0; i_3 < PB_Q; i_3++) {
                for (int i_4 = 1; i_4 < PB_M; i_4++) {
                    B[i_4][i_4][i_3][i_2] = B[i_1][i_1][i_3][i_4+1] - B[i_4-1][i_4-1][i_3][i_2] + 4;
                }
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_Q; i_5++) {
        for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
            for (int i_7 = i_6; i_7 < PB_P-1; i_7++) {
                C[i_6][i_6][i_7] = C[i_6+1][i_6+1][i_7+1] * 2;
            }
        }
    }
    for (int i_8 = 3; i_8 < PB_L+1; i_8++) {
        for (int i_9 = 1; i_9 < min(i_8, -i_8+PB_L+1); i_9++) {
            for (int j_0 = 0; j_0 < PB_L; j_0++) {
                D[j_0][i_9+i_8-2][i_9] = D[j_0][i_9+i_8-3][i_9-1] + D[j_0][i_9+i_8-1][i_9+1] - D[j_0][i_9+i_8-3][i_9] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), INIT_SEED);
kernel_4436212211_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E)));
