...
void kernel_2312312123_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        A[i_0] = 6;
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            B[i_0][i_1] = D[i_0][i_0+2] + B[i_0][i_1-1] * B[i_0][i_1-2] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-2; i_3++) {
            C[i_2][i_3+2] = C[i_2][i_3] - 3;
        }
    }
#pragma endscop
...
kernel_2312312123_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
