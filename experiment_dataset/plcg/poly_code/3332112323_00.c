...
void kernel_3332112323_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        B[i_0+1] = B[i_0-1] * B[i_0+1] - B[i_0+2] + 3;
        B[i_0+1] = 2;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
            for (int i_3 = 1; i_3 < i_2; i_3++) {
                C[i_1][i_2][i_3] = D[i_2][i_1] - C[i_3][i_2+2][i_1] - C[i_2][i_3-1][i_1] * C[i_1][i_2-1][i_3] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3332112323_00(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
