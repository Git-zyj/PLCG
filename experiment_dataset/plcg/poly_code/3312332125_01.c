...
void kernel_3312332125_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            B[i_0-3][i_0-1] = D[i_1][i_0-1] - E[i_1+i_0] - 1;
        }
        for (int i_2 = 2; i_2 < -i_0+PB_P; i_2++) {
            B[i_0-1][i_0+1] = D[i_2][i_0] * E[i_0+1] + F[i_2-2] + G[i_0+1][i_2+i_0] * B[i_0][i_0+2] + 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M, PB_P); i_4++) {
            for (int i_5 = 1; i_5 < min(PB_P, i_3); i_5++) {
                B[i_5-1][i_5+1] = H[i_4+i_3][i_5][i_5] + I[i_4][i_3][i_3-1] - G[i_5][i_4] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3312332125_01(xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
