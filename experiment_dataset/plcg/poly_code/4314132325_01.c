...
void kernel_4314132325_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < min(PB_P-1, i_1); i_2++) {
                for (int i_3 = 2; i_3 < min(PB_P-2, -i_2+PB_Q+1); i_3++) {
                    A[i_3][i_1][i_2+1][i_3-2] = D[i_0][i_1][0][i_3+i_2-1] + A[i_1][i_2][i_3+2][i_1-2] + A[i_3][i_1-2][i_2+1][i_3-2] + A[i_2][i_1][i_3+2][i_2-2] - 4;
                }
            }
        }
        B[i_0] = E[i_0] - E[i_0] - E[i_0+1] - E[i_0] + B[i_0+1] - B[i_0-2] - B[i_0+2] + 3;
    }
    for (int i_4 = 2; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_M; i_5++) {
            for (int i_6 = i_5; i_6 < PB_P; i_6++) {
                for (int i_7 = max(i_4, 4); i_7 < min(-i_5+PB_Q, PB_M-2); i_7++) {
                    A[i_7-2][i_5][i_4+1][i_7-4] = D[i_7+2][i_6][i_7-1][i_5] * D[i_5][i_4][i_6][i_7+i_5] * F[i_6][i_5][i_4-2] + 6;
                }
            }
        }
    }
...
kernel_4314132325_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
