...
void kernel_4426212125_05(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                for (int i_3 = max(2, i_1); i_3 < PB_P; i_3++) {
                    D[i_1][i_3-2][i_0-2][i_2+1] = E[i_0][i_2][i_3][i_1] + 2;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_P-3; i_5++) {
            for (int i_6 = 1; i_6 < min(i_4, PB_Q-3); i_6++) {
                B[i_5+1][i_5+2][i_6+1] = B[i_5+1][i_5+2][i_6-1] + B[i_5+2][i_5+3][i_6+3] * 3;
            }
        }
        for (int i_7 = 0; i_7 < i_4; i_7++) {
            B[i_4+1][i_4+2][i_7+1] = 4;
            for (int i_8 = 0; i_8 < -i_4+PB_L; i_8++) {
                for (int i_9 = 0; i_9 < i_7; i_9++) {
                    D[i_8][i_4-2][i_7][i_9+1] = E[i_4-1][i_7][i_9][i_8] + F[i_4-1][i_7][i_9+1][i_4] * F[i_8][i_8+i_4][i_9][i_4] + 4;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4426212125_05(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F)));
