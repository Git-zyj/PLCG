...
void kernel_2414352323_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        C[i_0+2] = E[i_0] + F[i_0] * F[i_0] * E[i_0] + F[i_0+1] + F[i_0+2] * E[i_0+1] * 6;
        for (int i_1 = 2; i_1 < min(PB_L+2, -i_0+PB_N-2); i_1++) {
            B[i_1+i_0][i_1+2] = E[i_1] + G[i_0+2][i_1-2] - F[i_1] - B[i_1+i_0-1][i_1+1] * B[i_1+i_0-2][i_1] - B[i_1+i_0+2][i_1+4] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        C[i_2] = H[i_2-1] + H[i_2+1] * H[i_2] - E[i_2] - I[i_2][1] * C[i_2-1] - 3;
        C[i_2] = F[i_2] + H[i_2+2] + H[i_2] + C[i_2-1] + 2;
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2414352323_02(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
