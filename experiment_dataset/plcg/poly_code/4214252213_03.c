...
void kernel_4214252213_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q, PB_M-1); i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_M-2; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_M-1; i_3++) {
                    A[i_1][i_0+2][i_2+2][i_3] = C[i_2+1][i_3][i_1+i_0][i_2-2] + C[i_0-2][i_2][i_1+2][i_3+2] + C[i_2+i_0][i_3][i_1][i_3] * D[i_3+i_1+1][i_2+2][i_1][i_0] - D[i_1+1][i_2+i_0+2][i_3+1][i_2] * D[i_3][i_0][i_1+1][i_0+2] + 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < min(-i_4+PB_L-1, PB_M-1); i_5++) {
            B[i_5+i_4] = E[i_5+i_4][2] * E[i_5-2][i_4] - F[i_5+1][i_4] - B[i_5+i_4-1] + B[i_5+i_4] + B[i_5+i_4+1] * 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4214252213_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F)));
