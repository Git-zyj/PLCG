...
void kernel_3226211111_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        ...
        for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
            for (int i_4 = i_3; i_4 < PB_M; i_4++) {
                B[i_4][i_0][i_4] = C[i_4][i_0] - B[i_4-1][i_3-1][i_4-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3226211111_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
