...
void kernel_4236311215_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = C[i_0] + A[i_0] - A[i_0+2] - A[i_0+2] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_1+PB_M-1, i_2); i_3++) {
                B[i_3+i_1][i_2][i_3-1] = D[i_2][i_3+i_1][i_3+1][i_1] + B[i_3+i_1+1][i_2][i_3-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4236311215_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D));
