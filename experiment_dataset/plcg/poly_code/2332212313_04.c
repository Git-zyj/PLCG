...
void kernel_2332212313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_N, PB_M-1); i_1++) {
            A[i_1] = D[i_1][i_0] - A[i_1-1] * A[i_1+1] + A[i_1+1] - 2;
            B[i_1+i_0] = A[i_1] * A[i_1+1] + A[i_1-1] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_N+1; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_M+1, -i_2+PB_N+1); i_3++) {
            C[i_3-2] = B[i_3+i_2-2] - B[i_3+i_2-1] - C[i_3-1] * 6;
        }
    }
#pragma endscop
...
kernel_2332212313_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
