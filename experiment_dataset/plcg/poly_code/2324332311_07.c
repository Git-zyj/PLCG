...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_Q; i_1++) {
        for (int i_2 = i_1; i_2 < min(-i_1+PB_M, PB_N-1); i_2++) {
            B[i_1+2][i_2] = D[i_2+i_1] - F[i_2+1] - B[i_1+3][i_2] * B[i_1+1][i_2-1] - B[i_2+3][i_1] * B[i_1+1][i_2] + B[i_1+2][i_2+1] + 6;
        }
        for (int i_3 = i_1; i_3 < min(PB_N, -i_1+PB_L+2); i_3++) {
            A[i_3] = G[i_3-2][i_1] - H[i_3][i_3+i_1-2][i_1] - I[i_1][i_3+1] + 1;
        }
    }
#pragma endscop
...
kernel_2324332311_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
