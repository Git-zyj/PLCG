...
void kernel_3224211311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_N+1; i_2++) {
        B[i_2-1] = D[i_2] - A[i_2-1] + B[i_2-2] + B[i_2] + B[i_2-2] - 1;
    }
#pragma endscop
...
kernel_3224211311_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
