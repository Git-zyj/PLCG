...
void kernel_4332131321_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
                A[i_0][i_0][i_2] = D[i_0][i_2-1][i_1] * D[i_2+1][i_0][i_1] - A[i_0][i_0][i_2-2] * A[i_0+2][i_0+2][i_2+2] * A[i_0+1][i_0+1][i_2] - A[i_0-1][i_0-1][i_2] - 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
            for (int i_5 = i_3; i_5 < PB_M-2; i_5++) {
                B[i_5][i_4][i_3-1] = D[i_4][i_3][i_5+1] * E[i_4][i_5-1] * D[i_5-1][i_3+1][i_4+1] * D[i_4][i_5][0] + D[i_4][i_5][i_3] - B[i_5+2][i_4][i_3+1] - B[i_4][i_5][i_3+1] * B[i_3-2][i_4][i_5+1] * 5;
            }
        }
    }
        ...
        for (int i_7 = 1; i_7 < PB_Q; i_7++) {
            for (int i_8 = 1; i_8 < PB_P-1; i_8++) {
                for (int i_9 = 1; i_9 < PB_Q; i_9++) {
                    C[i_9-1][i_7][i_8][i_7] = A[i_7][i_7][i_8+1] + C[i_7-1][i_9][i_8-1][i_9] * 1;
                }
            }
        }
    }
...
kernel_4332131321_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
