...
void kernel_4326352315_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M-1; i_1++) {
            A[i_0][i_1] = D[i_0-1][i_1+1] + E[i_1] + E[i_0] + D[i_1-2][i_0] + F[i_0][i_1] - G[i_0][i_1+2] * G[i_1][i_1+2] + D[i_1][i_0] + A[i_0+1][i_1+1] + A[i_0-1][i_1] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            for (int i_4 = max(2, i_3); i_4 < PB_N-2; i_4++) {
                B[i_4+2][i_2] = H[i_4+1][i_3][i_2+2] + F[i_2][i_3] - G[i_4+1][i_3] + I[i_4-2][i_2][i_3] * B[i_4+1][i_3-1] - B[i_4+1][i_3] + B[i_4+2][i_3-1] * 4;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
        for (int i_6 = 0; i_6 < PB_M; i_6++) {
            for (int i_7 = 0; i_7 < min(i_6, PB_N-2); i_7++) {
                B[i_7+2][i_6] = I[i_6][i_7][i_5+1] - F[i_7][i_6] * D[i_6][1] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4326352315_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
