...
void kernel_2412232115_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        A[i_0] = E[i_0] + E[i_0] - F[i_0+2] - 3;
        for (int i_1 = i_0; i_1 < min(PB_L-1, -i_0+PB_P); i_1++) {
            A[i_0] = G[i_1+1][i_0-1] - G[i_1][i_1+i_0] - G[i_0+2][i_1] * H[i_1+1][i_0] - A[i_0+1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q; i_2++) {
        A[i_2-1] = F[i_2+2] * F[i_2] + 4;
        for (int i_3 = 0; i_3 < PB_Q; i_3++) {
            A[i_3] = I[i_2][i_3][0] - H[i_3][i_2] + 6;
        }
    }
#pragma endscop
...
kernel_2412232115_00(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
