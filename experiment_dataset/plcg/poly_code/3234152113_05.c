...
void kernel_3234152113_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        B[i_0] = C[i_0] + C[i_0] * C[i_0] - B[i_0] * 3;
        for (int i_1 = 2; i_1 < min(-i_0+PB_M-2, i_0); i_1++) {
            B[i_0] = D[i_1+1][i_0+1] * D[i_0][i_1] + D[i_1+2][i_0] + D[i_1-2][i_0-1] - C[i_1+i_0+2] + D[i_1-2][i_0] * 6;
        }
    }
#pragma endscop
...
kernel_3234152113_05(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
