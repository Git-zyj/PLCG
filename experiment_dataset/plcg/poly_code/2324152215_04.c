...
void kernel_2324152215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0][i_0+1] = D[i_0] + D[i_0+1] * D[i_0] * A[i_0-1][i_0] + 3;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_N; i_2++) {
            A[i_2-1][i_2] = E[i_2+1][i_1] - E[i_1][i_2+1] * E[i_1+1][i_2-2] + E[i_2+i_1][i_1] * D[i_1+1] * E[i_2][i_1+2] * C[i_1][i_2-1] - 1;
        }
        for (int i_3 = 2; i_3 < -i_1+PB_M; i_3++) {
            C[i_1][i_3] = D[i_3] - E[i_1][i_3-1] * E[i_1-2][i_3+i_1] + D[i_3-2] * E[0][i_1] + A[i_1][i_1+1] * 6;
        }
    }
#pragma endscop
...
kernel_2324152215_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
