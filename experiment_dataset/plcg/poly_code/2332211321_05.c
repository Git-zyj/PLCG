...
void kernel_2332211321_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-3; i_0++) {
        A[i_0][i_0+1] = B[i_0+3] + B[i_0+1] - 4;
        B[i_0+1] = D[1] - A[i_0][i_0+1] - A[i_0+2][i_0+3] + A[i_0-2][i_0-1] * 2;
    }
    for (int i_1 = 3; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            C[i_2-1][i_1+1] = D[i_2] - B[i_1+1] - C[i_2-1][i_1-1] * C[i_1-3][i_2+1] + C[i_2-1][i_1+2] * 5;
        }
    }
#pragma endscop
...
kernel_2332211321_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
