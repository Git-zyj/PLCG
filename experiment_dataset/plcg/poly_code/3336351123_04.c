...
void kernel_3336351123_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        B[i_0] = D[i_0] * D[i_0+1] + E[i_0-1] + F[i_0-1] - E[i_0] + B[i_0] * 1;
        B[i_0] = D[i_0] * D[i_0] - D[i_0-1] * F[i_0] * 3;
    }
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_P, -i_1+PB_L); i_2++) {
            C[i_1][i_2] = G[i_2][i_2+i_1] - G[i_1][i_2] - G[i_1][i_2-1] * G[i_2][i_1] + H[i_1][i_2] * H[i_2][i_1+1] * C[i_1-1][i_2+2] - 6;
        }
    }
#pragma endscop
...
kernel_3336351123_04(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
