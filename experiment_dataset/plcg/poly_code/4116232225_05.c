...
void kernel_4116232225_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = max(0, -i_1+1); i_2 < min(-i_1+PB_M+1, i_1); i_2++) {
                for (int i_3 = 2; i_3 < PB_P; i_3++) {
                    A[i_1][0][i_3] = B[i_3-2][i_2+i_1-1][i_1][i_0] * B[i_1+2][i_0][i_2][i_0+2] - C[i_2][i_0][i_3-1][i_2] + A[i_2+1][0][i_3-2] * A[i_2][0][i_3+2] - 2;
                }
            }
        }
    }
...
kernel_4116232225_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
