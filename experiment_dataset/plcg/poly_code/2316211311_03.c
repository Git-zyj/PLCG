...
void kernel_2316211311_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_M+1; i_2++) {
            B[i_2-1] = A[i_2-1] + B[i_2-2] - B[i_1] - 4;
        }
        for (int i_3 = 1; i_3 < i_1; i_3++) {
            C[i_1+1][i_3] = D[i_3-1] - A[i_3-1] + C[i_1+1][i_3+1] * C[i_1+2][i_3+1] * 3;
        }
    }
#pragma endscop
...
kernel_2316211311_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
