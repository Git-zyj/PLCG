...
void kernel_2432112113_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_Q; i_1++) {
        for (int i_2 = 0; i_2 < PB_Q; i_2++) {
            B[i_1][i_2] = E[i_2][0] - E[i_2][i_1-1] - 3;
        }
        A[i_1-1] = 2;
    }
    for (int i_3 = 0; i_3 < PB_Q; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            D[i_4] = E[i_4][i_3] * B[i_4][i_3] + 4;
        }
    }
#pragma endscop
...
kernel_2432112113_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
