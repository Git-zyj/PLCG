...
void kernel_3316112321_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        ...
        for (int i_2 = i_0; i_2 < PB_L-2; i_2++) {
            for (int i_3 = 2; i_3 < PB_L; i_3++) {
                B[i_3][i_2] = D[i_3-2][i_2+2] + B[i_2+1][i_3] - B[i_3-1][i_2+2] + 4;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        C[i_4] = C[i_4+1] * C[i_4-1] - C[i_4-2] + C[i_4+2] * 6;
    }
#pragma endscop
...
kernel_3316112321_05(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
