...
void kernel_4216111323_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L-2; i_1++) {
            for (int i_2 = i_1; i_2 < -i_0+PB_L-2; i_2++) {
                A[i_2+i_1-1][i_2+i_0] = A[i_2+i_1-1][i_1+i_0+2] - A[i_1+i_0+1][i_2+i_1+1] + A[i_1+i_0+1][i_2+i_0+2] * 6;
            }
        }
        for (int i_3 = 0; i_3 < min(i_0, -i_0+PB_L-3); i_3++) {
            for (int i_4 = i_0; i_4 < PB_L; i_4++) {
                A[i_4+i_3][i_3+i_0+3] = C[i_4][0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4216111323_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
