...
void kernel_2314312213_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < min(i_1, -i_1+PB_M+2); i_2++) {
            B[i_2+i_1-2][i_1+2] = D[i_2][i_1] - B[i_2+i_1-3][i_1+2] * B[i_2+i_1-3][i_1+1] - B[i_2+i_1-3][i_2+2] * 4;
        }
        C[i_1-2] = E[i_1-1] + C[i_1-3] + C[i_1-1] + 4;
    }
#pragma endscop
...
kernel_2314312213_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
