...
void kernel_3332352213_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[2] = D[i_0] - E[i_0] - F[i_0] + D[i_0] + D[i_0-1] * D[i_0-1] * A[2] * 6;
    }
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        A[2] = F[i_1] * E[i_1] * D[i_1-2] - F[i_1-2] - E[i_1] - A[2] + A[2] + 6;
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                C[i_2][i_3][i_3+2] = G[i_2][1] + H[i_4][i_3][i_2] - H[i_3][i_4][i_2] + I[i_4][i_2][i_3] + C[i_2+1][i_3][i_3+2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3332352213_06(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
