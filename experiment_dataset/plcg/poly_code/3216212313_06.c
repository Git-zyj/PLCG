...
void kernel_3216212313_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1] = A[i_1-1] + A[i_1+1] - B[i_0+1][i_0+1] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_M-1; i_3++) {
            for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
                B[i_3][i_3] = C[i_4-2][i_2][i_3] * B[i_4+1][i_4+1] * B[i_3-1][i_3-1] + B[i_3+1][i_3+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3216212313_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
