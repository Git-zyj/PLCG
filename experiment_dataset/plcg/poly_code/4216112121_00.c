...
void kernel_4216112121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_0+1][i_2] = C[i_2][i_1] - C[i_2-1][i_1] + 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_L-1; i_4++) {
            for (int i_5 = i_4; i_5 < PB_M+1; i_5++) {
                B[i_5-1][i_4] = B[i_5-2][i_4+1] - B[i_5-1][i_4-2] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4216112121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
