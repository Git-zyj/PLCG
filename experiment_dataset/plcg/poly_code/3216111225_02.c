...
void kernel_3216111225_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                A[i_2][i_0][i_1+1] = A[i_2][i_0-1][i_1+1] + A[i_2+2][i_0][i_1+1] - A[i_2][i_0][i_1+3] + 3;
            }
        }
        B[i_0] = C[i_0-1] * 5;
    }
...
kernel_3216111225_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
