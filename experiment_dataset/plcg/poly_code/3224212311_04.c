...
void kernel_3224212311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            A[i_1+2][i_0] = A[i_1+3][i_0] + A[i_1+3][i_0+1] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 3; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            B[i_3-2] = C[i_3+i_2][i_3-1] * B[i_3-1] * B[i_3-3] * B[i_3-3] - 4;
        }
    }
#pragma endscop
...
kernel_3224212311_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
