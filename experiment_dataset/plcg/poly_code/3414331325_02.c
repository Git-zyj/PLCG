...
void kernel_3414331325_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_N); i_1++) {
            C[i_0] = E[i_1-1] * F[i_0-1][i_1] - F[i_1+i_0][i_1] * G[i_1][i_0] + D[i_1] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            C[i_2-2] = H[i_3+1][0] * C[i_3] - C[i_3+1] - 4;
        }
        C[i_2] = E[i_2] * I[i_2-1] * E[i_2] * C[i_2+1] + C[i_2+2] + 4;
        D[i_2] = J[i_2+1] * F[i_2][i_2+1] + 2;
    }
#pragma endscop
...
kernel_3414331325_02(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
