...
void kernel_4222312313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M-2; i_2++) {
                A[i_0+2][i_1+2] = A[i_0+3][i_1+2] + A[i_1+2][i_2+1] * A[i_2+2][i_0+3] + 6;
            }
        }
    }
        ...
        for (int i_4 = 2; i_4 < PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                for (int i_6 = 2; i_6 < PB_Q-1; i_6++) {
                    B[i_5][i_4-1][i_3][i_6] = C[i_6-2][i_5][i_4][i_3+2] * B[i_3][i_4-1][i_5][i_6+1] * B[i_5][i_4-2][i_3][i_6] - B[i_5+1][i_6-1][i_3][i_4] * 5;
                }
            }
        }
    }
...
kernel_4222312313_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
