...
void kernel_2224212325_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-4; i_2++) {
            B[i_2+2] = B[i_2] + B[i_1+3] * B[i_2+4] - 6;
        }
    }
#pragma endscop
...
kernel_2224212325_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
