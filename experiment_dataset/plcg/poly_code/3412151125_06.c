...
void kernel_3412151125_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0] - E[i_0-1] * E[i_0] * A[i_0-1] + A[i_0+2] + 2;
        for (int i_1 = 0; i_1 < -i_0+PB_N; i_1++) {
            for (int i_2 = PB_N - 1; i_2 >= 1; i_2--) {
                B[i_1][i_2+1][i_1] = F[i_1+i_0][1][i_2] + F[i_0+1][i_2][i_1] + F[i_2-1][i_0][i_1] + 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
            C[i_4+i_3][i_4+1][i_3] = G[i_4][i_3] * E[i_4] - E[i_3+1] - 4;
        }
        for (int i_5 = 1; i_5 < min(-i_3+PB_L+1, PB_L-2); i_5++) {
            A[i_5] = G[i_3-1][i_5] + G[i_5+1][i_3] - G[i_5][1] * G[i_5+i_3+1][0] - E[i_5] - G[i_5][i_3] * G[i_3-1][i_5+1] * G[i_5][i_3] * G[i_5][0] * G[i_5][i_3] * G[i_5-1][i_5+i_3-1] + B[i_3][i_5+2][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_3412151125_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
