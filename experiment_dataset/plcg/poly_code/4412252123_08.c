...
static void print_array(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4412252123_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0+1] = E[i_0-1] * F[i_0] - F[i_0] + E[i_0] * F[0] - F[i_0] * 4;
        for (int i_1 = 1; i_1 < PB_M-3; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_P-2, i_1); i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_M; i_3++) {
                    B[i_3+i_1][i_2][i_0][1] = G[i_2+2][i_3+1][i_0][i_1] * G[i_0+2][i_1][i_3][i_2+1] - H[i_2-2][i_3][i_0+2][i_1][i_2+1] - 4;
                }
                for (int i_4 = 0; i_4 < -i_1+PB_M-3; i_4++) {
                    B[i_4+i_1+3][i_2][i_0][1] = H[i_1-1][i_0][i_4+i_2][i_4][i_2+i_1+2] + I[i_2][i_0-1][i_1] - J[i_1][i_4][i_2-2][i_0+2] - 5;
                }
            }
        }
    }
    for (int i_5 = 3; i_5 < PB_L-2; i_5++) {
        for (int i_6 = 2; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < PB_P-2; i_7++) {
                D[i_6][i_5-1][i_7+2] = K[i_7][i_6] * I[i_7][i_5+2][i_6] + K[i_6][i_5+1] + L[i_6-2][i_7][i_5] + I[i_6][i_7][i_5] - K[i_6][i_7] * D[i_6][i_5-3][i_7+2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4412252123_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
