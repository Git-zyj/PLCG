...
void kernel_4216312123_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        ...
        for (int i_3 = i_0; i_3 < min(PB_L-3, -i_0+PB_M-1); i_3++) {
            for (int i_4 = i_0; i_4 < PB_M; i_4++) {
                B[i_4][i_3+2][i_0+2] = C[i_4][i_3+i_0+1][i_0] * D[i_4][i_0][i_3] + B[i_4][i_3+1][i_0+2] - B[i_4][i_3+3][i_0+2] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4216312123_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
