...
void kernel_3212112225_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_P-2; i_2++) {
                A[i_1+i_0][i_2][i_2+i_1+2] = A[i_1+i_0-2][i_2][i_2+i_1] * A[i_1+i_0+2][i_2][i_2+i_1+2] * A[i_1+i_0][i_2-1][i_2+i_1+1] - 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M; i_4++) {
            B[i_4] = C[i_4][i_4+i_3] * B[i_4+2] - 4;
        }
    }
#pragma endscop
...
kernel_3212112225_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
