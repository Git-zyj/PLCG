...
void kernel_4422212221_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
                    A[i_1-1][i_2][i_3] = A[i_2-1][i_1][i_3+2] - 5;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_Q; i_4++) {
        for (int i_5 = 2; i_5 < PB_N-1; i_5++) {
            B[i_4-2][i_5] = E[i_5-1] * B[i_4][i_5] + B[i_4-2][i_5+1] + B[i_4-3][i_5-2] - B[i_4-3][i_5+1] + B[i_4-2][i_5-2] - 3;
        }
        for (int i_6 = 0; i_6 < PB_Q-1; i_6++) {
            for (int i_7 = 1; i_7 < -i_4+PB_L-1; i_7++) {
                C[i_4][i_7+1][i_6][i_4] = C[i_4][i_7-1][i_6+1][i_4] + D[i_6+2][i_7+i_4+1][i_4-1] - 2;
            }
            for (int i_8 = i_4; i_8 < PB_L; i_8++) {
                for (int i_9 = i_8; i_9 < -i_8+PB_L; i_9++) {
                    D[i_6][i_9+i_8][i_9-2] = 5;
                }
            }
        }
    }
...
kernel_4422212221_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
