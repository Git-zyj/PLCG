...
static void print_array(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4432352115_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        A[i_0] = E[i_0] * F[i_0][i_0] - E[i_0+2] * G[i_0+2] * E[i_0] - 5;
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < min(i_1, -i_2+PB_M); i_3++) {
                    D[i_3+i_2][i_2-2][i_0] = H[i_1][i_3][i_3+2][i_2+2] - I[i_1-2][i_3][i_2] * H[i_0-2][i_2][i_1][i_2] - 6;
                }
            }
        }
        for (int i_4 = 2; i_4 < i_0; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_4+PB_M, -i_0+PB_P-2); i_5++) {
                D[i_5+i_4-1][i_4-2][i_0] = I[i_5+2][i_4][i_5+i_0+2] - J[i_5][i_0+2] - K[i_5+i_4][i_0] + 2;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_P-1; i_6++) {
        for (int i_7 = 2; i_7 < -i_6+PB_M-1; i_7++) {
            for (int i_8 = 0; i_8 < PB_L-2; i_8++) {
                D[i_7+i_6][i_6-2][i_8] = F[i_8][i_7-2] - I[i_7-2][i_8+i_7][0] + L[i_8+2][i_7-2][i_6+1] * M[i_8][i_6+1][i_7] - F[i_8][i_6] + I[i_7][i_8][i_8] - D[i_7+i_6+1][i_6-2][i_8] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4432352115_02(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M)));
