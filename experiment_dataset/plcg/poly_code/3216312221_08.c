...
void kernel_3216312221_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_2+1][i_3+1] = D[i_3][i_2-2] * A[i_2][i_3+1] + B[i_2+2][i_3+1] * B[i_3][i_2+1] + B[i_2][i_3] + 1;
        }
    }
#pragma endscop
...
kernel_3216312221_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
