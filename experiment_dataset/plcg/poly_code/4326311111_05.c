...
void kernel_4326311111_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_2 = 1; i_2 < PB_Q; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_P-1, i_2); i_3++) {
            for (int i_4 = i_2; i_4 < min(-i_3+PB_M, PB_L); i_4++) {
                B[i_4][i_4+1][i_2] = D[i_4+i_3][i_4][i_2-1] * B[i_3][i_3+1][i_2-1] + 2;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_L; i_5++) {
        for (int i_6 = 1; i_6 < i_5; i_6++) {
            C[i_6][i_5] = C[i_6-1][i_5] - 3;
        }
    }
#pragma endscop
...
kernel_4326311111_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
