...
void kernel_3336212321_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_N-3 - 1; i_0 >= 1; i_0--) {
        A[i_0+1][2] = D[i_0] - A[i_0][2] + A[i_0-1][2] - A[i_0+3][2] + 2;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_Q+1; i_2++) {
            for (int i_3 = 0; i_3 < PB_Q; i_3++) {
                B[i_1-1][i_2][i_3] = E[i_1][i_3][i_3] + B[i_2+1][i_3+2][i_1] + B[i_1+1][i_3][i_2-1] * B[i_1+1][i_2-2][i_3] - 5;
            }
        }
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            C[i_1][i_4] = D[0] + C[i_1][i_4-2] - 1;
        }
    }
#pragma endscop
...
kernel_3336212321_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
