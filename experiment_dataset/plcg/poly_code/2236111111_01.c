...
void kernel_2236111111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0-1][i_0] = C[i_0-1] * A[i_0][i_0+1] + A[i_0-2][i_0-1] * 2;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_N, -i_1+PB_L+1); i_2++) {
            B[i_2] = D[i_1][i_2+i_1-1] * 2;
        }
    }
#pragma endscop
...
kernel_2236111111_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
