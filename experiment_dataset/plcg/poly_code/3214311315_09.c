...
void kernel_3214311315_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M+1, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M-1; i_2++) {
                A[i_0+1][i_2+i_1][i_2] = A[i_0+1][i_2+i_1+1][i_2] - A[i_2][i_1+i_0-1][i_1-1] * 5;
            }
        }
    }
    for (int i_3 = PB_N-1 - 1; i_3 >= 0; i_3--) {
        for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_3+PB_N-1, -i_4+PB_N-1); i_5++) {
                B[i_5+i_3][i_4][i_5] = C[i_5-1][i_4-1] * B[i_5+i_3+1][i_4][i_3] * B[i_5+i_4+1][i_3][i_5] * B[i_5+i_3][i_4-1][i_5] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3214311315_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
