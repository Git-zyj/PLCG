...
void kernel_4432211321_01(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
        ...
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_Q, PB_L-1); i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    B[i_0][i_2][i_3][0] = E[i_2-1][i_3][i_1-1][i_2+i_0] - B[i_3][i_2+1][i_1+1][0] - 5;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < PB_L-2; i_5++) {
                for (int i_6 = 1; i_6 < PB_L; i_6++) {
                    B[i_0][i_5][i_6][0] = E[i_5][i_0][i_6][i_4] * F[i_5-1][i_0][i_6+1][i_4] + B[i_4][i_5+2][i_6][0] + B[i_5][i_6-1][i_4][0] * 3;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4432211321_01(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G)));
