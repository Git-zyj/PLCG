...
void kernel_4414352323_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-2; i_0++) {
        A[i_0-1] = E[i_0] - F[i_0] * E[i_0] - F[i_0] * E[i_0+2] * F[i_0] + A[i_0-3] + A[i_0] + 2;
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_N); i_1++) {
            A[i_0-1] = G[i_1][i_1+1] - H[i_1+i_0][0] * I[i_1][i_0] + F[i_1] * H[i_1-2][i_1+1] * A[i_0-1] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 3; i_3 < -i_2+PB_M; i_3++) {
            A[i_3-3] = H[i_3+i_2][0] - E[i_3+i_2] + J[i_3] + H[i_3][i_2] - 1;
        }
        for (int i_4 = i_2; i_4 < -i_2+PB_M-1; i_4++) {
            A[i_4-1] = F[i_4] - H[i_2][i_4+2] + H[i_4+i_2+1][i_2] + I[i_2+1][0] - H[i_2][i_4+i_2-2] - 3;
        }
    }
#pragma endscop
...
kernel_4414352323_03(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
