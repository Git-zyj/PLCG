...
void kernel_3132331221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_Q - 1; i_0 >= 1; i_0--) {
        for (int i_1 = 3; i_1 < -i_0+PB_Q+1; i_1++) {
            A[i_0][i_1-1] = B[i_1] * C[i_1][i_1+i_0-1] * C[i_1][i_0] + A[i_0][i_1-3] * A[i_0][i_1-2] - 5;
        }
    }
#pragma endscop
...
kernel_3132331221_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
