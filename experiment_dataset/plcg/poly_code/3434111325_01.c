...
void kernel_3434111325_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_P-1); i_2++) {
                A[i_2+i_0][i_1][i_1] = E[i_1-1][i_2+i_0-1] + A[i_1+i_0][i_2+1][i_2+1] - 4;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L; i_4++) {
            B[i_4-1][i_3] = B[i_4-3][i_3] + B[i_4-2][i_3] + B[i_3-3][i_4] * B[i_4-2][i_3-1] * 1;
        }
    }
    for (int i_5 = 2; i_5 < PB_P-1; i_5++) {
        for (int i_6 = 1; i_6 < -i_5+PB_P-1; i_6++) {
            C[i_5][i_6+i_5] = C[i_5+1][i_6+i_5+1] * C[i_6-1][i_6+i_5-1] + 6;
        }
        D[i_5] = D[i_5-2] + D[i_5+2] - 6;
    }
#pragma endscop
...
kernel_3434111325_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
