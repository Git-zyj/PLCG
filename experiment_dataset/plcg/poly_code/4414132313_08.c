...
void kernel_4414132313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0+1] - E[i_0+1] - E[i_0] + E[i_0-2] - E[i_0] * A[i_0-1] * A[i_0+1] + 5;
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < i_2; i_3++) {
                    D[i_1-2][i_2][i_3] = F[i_0][i_2][i_3][i_1][i_1] - 4;
                }
            }
            for (int i_4 = 0; i_4 < PB_P-2; i_4++) {
                for (int i_5 = 0; i_5 < PB_L; i_5++) {
                    D[i_1][i_4][i_5] = G[i_5][i_5][i_1][i_0] - G[i_5][i_0][i_4+2][i_0] * 2;
                }
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
        for (int i_7 = i_6; i_7 < PB_M; i_7++) {
            for (int i_8 = 0; i_8 < PB_L-2; i_8++) {
                D[i_6-1][i_7][i_8] = G[i_6+2][i_8+2][i_7][0] * G[i_7][i_8+1][i_6][i_6+1] - D[i_6][i_7][i_8] * D[i_6-2][i_7][i_8] + D[i_6-1][i_7-1][i_8] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4414132313_08(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
