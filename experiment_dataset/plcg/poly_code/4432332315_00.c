...
void kernel_4432332315_00(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
        ...
        for (int i_1 = 0; i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
                for (int i_3 = 0; i_3 < -i_2+PB_L; i_3++) {
                    B[i_3+1][i_1+i_0+1][i_2+2][i_3] = E[i_2+2][i_3+i_2][i_1][i_0-1] * F[i_2-2][i_0][i_1+1][i_3] - 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            for (int i_6 = 1; i_6 < PB_P-2; i_6++) {
                for (int i_7 = 0; i_7 < min(min(i_6, -i_4+PB_L-1), -i_6+PB_L); i_7++) {
                    B[i_5][i_7+i_4+1][i_6+2][i_5-1] = G[i_7][i_6][i_5][i_4-1][i_5] - H[i_7+i_6][i_6-1][i_5] - 5;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_M-2; i_8++) {
        C[i_8] = I[i_8+1] * I[i_8+1] + I[i_8+2] - J[i_8] - I[i_8+1] - C[i_8-1] - C[i_8+1] * C[i_8-1] - C[i_8+1] * C[i_8-1] + C[i_8+1] + 1;
    }
    for (int i_9 = 0; i_9 < PB_L; i_9++) {
        for (int j_0 = i_9; j_0 < -i_9+PB_L; j_0++) {
            for (int j_1 = 1; j_1 < min(-j_0+PB_L, PB_M); j_1++) {
                for (int j_2 = 0; j_2 < min(PB_P-2, -j_0+PB_P); j_2++) {
                    B[j_1][j_0+i_9][j_2+2][j_1-1] = F[j_1+j_0][j_2+j_0][i_9][j_1] * F[j_1][i_9][j_2][j_0] - 6;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4432332315_00(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
