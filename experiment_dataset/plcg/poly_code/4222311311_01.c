...
void kernel_4222311311_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
                A[i_1+1][i_2-1][i_0-1] = A[i_1+1][i_2-2][i_0] * A[i_1][i_2-2][i_0-2] - A[i_1+1][i_0-1][i_2] * 4;
            }
        }
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 1; i_4 < PB_L+1; i_4++) {
                A[i_3+1][i_4-1][i_0] = C[i_4+1][i_3][i_0] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4222311311_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
