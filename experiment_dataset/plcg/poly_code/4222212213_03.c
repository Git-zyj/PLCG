...
void kernel_4222212213_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            A[i_1][i_1][i_1] = A[i_0-1][i_0-1][i_0-1] * A[i_1+1][i_1+1][i_1+1] * 4;
        }
    }
        ...
        for (int i_3 = 1; i_3 < PB_Q; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_L, PB_Q); i_4++) {
                for (int i_5 = 1; i_5 < PB_P-2; i_5++) {
                    B[i_4+i_3-1][i_5+2][i_2][0] = C[i_4+1][i_5][i_3][i_2-1] - B[i_4+i_3][i_2+2][i_5-1][0] * B[i_4+i_3][i_2+2][i_5][0] + 2;
                }
            }
        }
    }
...
kernel_4222212213_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
