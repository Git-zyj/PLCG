...
void kernel_3426352115_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0+2] = E[i_0] - E[i_0] + F[i_0] + 1;
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            B[i_1+i_0][i_0+2] = G[i_1+i_0][i_1] + H[i_1+i_0][1] * 6;
        }
        A[i_0+2] = E[i_0] + E[i_0] * F[i_0] * F[i_0+2] + E[i_0] + E[i_0-1] - A[i_0+2] * 3;
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        A[i_2+3] = E[i_2-1] + I[i_2][2] * I[i_2][i_2+2] + F[i_2] * I[i_2][2] + E[i_2] + J[i_2-1] + F[i_2] * F[i_2] * 1;
    }
#pragma endscop
...
kernel_3426352115_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
