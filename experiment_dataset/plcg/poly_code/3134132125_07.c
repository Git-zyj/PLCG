...
void kernel_3134132125_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_1+1] = B[i_1-1][i_0+1] - B[i_1-1][i_0] * C[i_0][i_1+1][i_0-1] + A[i_1+2] + 2;
        }
    }
#pragma endscop
...
kernel_3134132125_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
