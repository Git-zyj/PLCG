...
void kernel_4316111121_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            for (int i_3 = i_2; i_3 < -i_1+PB_M; i_3++) {
                C[i_3+i_1][i_2][i_3] = D[i_3+i_1][i_2-1] + C[i_3+i_1+1][i_2-1][i_3] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4316111121_05(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
