...
void kernel_2214112121_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        ...
        for (int i_2 = 0; i_2 < PB_N; i_2++) {
            B[i_0][i_2] = C[i_2][i_0+2] - B[i_0][i_2+2] * 5;
        }
    }
#pragma endscop
...
kernel_2214112121_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
