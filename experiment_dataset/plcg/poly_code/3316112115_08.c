...
void kernel_3316112115_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_1][i_1][i_2] = D[i_1][i_0][i_1] + D[i_1+i_0+2][i_2][i_1] + A[i_1-1][i_1-1][i_2] - 3;
                A[i_1][i_1][i_2+1] = 6;
            }
            for (int i_3 = 0; i_3 < PB_M; i_3++) {
                A[i_1][i_1][i_3] = D[i_0-1][i_3][i_1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3316112115_08(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
