...
void kernel_3326351313_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0] = D[i_0] + E[i_0-1] * F[i_0] + A[i_0+1] * A[i_0-1] - A[i_0+1] * 5;
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                B[i_1][i_2+1][i_0+1] = G[i_1][i_0+1][i_2] * H[i_2][i_0][i_1] + G[i_1-1][i_0][i_1+1] - H[i_1][i_2][i_0] * I[i_2][i_0][i_1] + J[i_2+1][i_1][i_0-1][0] - I[i_1][i_0][1] * B[i_1-1][i_2+1][i_0+2] + B[i_2][i_1+1][i_0] - 4;
            }
        }
        for (int i_3 = 0; i_3 < min(-i_0+PB_M-1, i_0); i_3++) {
            for (int i_4 = 1; i_4 < i_0; i_4++) {
                C[i_4][i_3] = G[i_3+i_0+1][i_4-1][i_4] * H[i_3+1][i_3][i_4] - K[i_4][i_0] + L[i_4+1][i_4] * C[i_4+1][i_3] - C[i_4+1][i_3+1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3326351313_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
