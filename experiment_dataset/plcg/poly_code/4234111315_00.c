...
void kernel_4234111315_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            for (int i_2 = i_1; i_2 < PB_N-2; i_2++) {
                A[i_2+1][i_0][i_2-1] = A[i_1+1][i_0-1][i_1-1] + A[i_2+2][i_0-1][i_2] - A[i_2+1][i_0-1][i_2-1] * 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                A[i_3+2][i_5][i_3] = C[i_5][i_3-1][i_4+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4234111315_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
