...
void kernel_2412352111_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            A[i_0] = E[i_1-2] - F[i_0] - F[i_1+2] + G[i_0+1][i_1-2] * 5;
            B[i_1] = H[i_0][i_1-1] * H[i_1][i_0] - A[i_1-1] * 5;
            C[i_0][0] = F[i_1+i_0] * F[i_1] - I[i_1-2][i_0] * E[i_1] - J[i_1+1] * I[i_1+1][i_0] * 6;
        }
        for (int i_2 = 1; i_2 < min(PB_M, -i_0+PB_L); i_2++) {
            D[i_0][i_2] = H[i_2][i_0-2] - I[i_2-1][i_0] + J[i_2+i_0] - G[i_0][i_2] + E[i_2-1] + F[i_2] - D[i_0][i_2-1] + D[i_0][i_2+1] - D[i_2-1][i_0] - 1;
        }
    }
#pragma endscop
...
kernel_2412352111_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
