...
void kernel_3316111215_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        B[i_2-1][i_2] = D[i_2] * B[i_2-2][i_2-1] * 1;
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            B[i_2-2][i_2-1] = E[i_2][i_2-1] - D[i_3] - B[i_2-1][i_2] * 3;
        }
    }
#pragma endscop
...
kernel_3316111215_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
