...
void kernel_4424131215_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_2+PB_M+1, i_2), -i_1+PB_P); i_3++) {
                    A[i_0][i_2][i_3+i_1][i_0] = E[i_3+i_2-1][i_1][i_0][i_2] - E[i_2+1][i_3][i_0][i_1-1] * A[i_0][i_2-1][i_3+i_1][i_0] + 3;
                }
                for (int i_4 = 0; i_4 < PB_P; i_4++) {
                    B[i_0][i_1-1][i_2+1][1] = F[i_4+1][i_1][i_2] * B[i_0+1][i_1-1][i_2+1][1] * B[i_0-1][i_1-1][i_2+2][1] + 1;
                }
            }
            for (int i_5 = 0; i_5 < PB_P; i_5++) {
                B[i_0][i_1-1][i_5+1][1] = F[i_1-1][i_0][i_1] - F[i_0-1][i_1-1][i_5] + 6;
            }
        }
        for (int i_6 = 1; i_6 < PB_P+1; i_6++) {
            for (int i_7 = 0; i_7 < min(-i_0+PB_L, -i_6+PB_L); i_7++) {
                for (int i_8 = 1; i_8 < PB_Q-1; i_8++) {
                    D[i_0][i_7][i_6-1][i_8+1] = F[i_7][i_8+1][i_6] + E[i_7][i_6][i_0+1][i_8-1] * F[i_7][i_8-1][i_6-1] * F[i_8][i_6-1][i_7+i_0] - E[i_8][i_8-1][i_7+i_6][i_0] + E[i_7][i_6-1][i_8][i_0-1] + D[i_0+1][i_7+1][i_6-1][i_8] * 5;
                }
            }
        }
    }
...
kernel_4424131215_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
