...
void kernel_2216212315_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 3; i_1 < PB_M+2; i_1++) {
            A[i_1-2][i_0] = A[i_1-3][i_0-1] + A[i_1-2][i_0+1] - A[i_1-3][i_0] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_M-1; i_3++) {
            A[i_3-1][i_2] = C[i_3+1][i_2] + 1;
        }
    }
#pragma endscop
...
kernel_2216212315_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
