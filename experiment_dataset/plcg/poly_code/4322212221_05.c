...
void kernel_4322212221_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            ...
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                B[i_2][i_1][1] = D[i_0][i_2][i_1] + A[i_0][i_1-2][1] + B[i_2-1][i_1-2][1] * B[i_0-2][i_2][1] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
                C[i_4][i_5][i_5] = B[i_5+2][i_4-1][1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4322212221_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
