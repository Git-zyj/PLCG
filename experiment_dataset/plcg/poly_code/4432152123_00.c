...
void kernel_4432152123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] - E[i_0] - E[i_0-2] * E[i_0] + E[i_0] + E[i_0] - A[i_0+1] * 3;
    }
    for (int i_1 = 3; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_L; i_2++) {
            B[i_1-2][i_2-2] = F[i_2+i_1-1][i_1+2] - F[i_2][i_1-1] - E[i_2-2] + F[i_2][i_2+i_1] * G[i_1-1][i_2][i_1] + F[i_2][i_1+2] * B[i_1-3][i_2-2] + 6;
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 2; i_5 < -i_3+PB_N; i_5++) {
                for (int i_6 = 1; i_6 < PB_M; i_6++) {
                    D[i_4+2][i_5-2][i_6-1] = H[i_5+i_3][i_6][i_4][i_5] + H[i_6][i_5][i_3-2][i_3] - G[i_6][i_3][i_5+1] * 4;
                }
            }
        }
    }
        ...
        for (int i_8 = 2; i_8 < PB_M; i_8++) {
            for (int i_9 = 2; i_9 < PB_M; i_9++) {
                for (int j_0 = 2; j_0 < PB_P; j_0++) {
                    D[i_8][i_9][j_0-2] = H[i_9][i_8-1][j_0][i_7] - H[j_0-2][i_9][i_7][j_0] + G[i_9+2][i_8-2][i_7] * D[i_8-1][j_0][i_9-2] * 4;
                }
            }
        }
    }
...
kernel_4432152123_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
