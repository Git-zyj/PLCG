...
void kernel_3232311225_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            ...
            for (int i_2 = 2; i_2 < PB_M+1; i_2++) {
                B[i_1][i_2-1][i_0-1] = B[i_1+2][i_0-1][i_2-1] - B[i_1-2][i_2-1][i_0] * B[i_1][i_2-2][i_0-2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3232311225_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
