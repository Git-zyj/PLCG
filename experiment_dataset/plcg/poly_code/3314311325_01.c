...
void kernel_3314311325_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = i_0; i_2 < min(PB_M, -i_1+PB_N); i_2++) {
                B[i_2][i_2+1] = D[i_2+i_1][i_0+1] * B[i_1][i_1+1] - B[i_1-1][i_1] - B[i_0+2][i_0+3] - 3;
            }
        }
...
kernel_3314311325_01(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
