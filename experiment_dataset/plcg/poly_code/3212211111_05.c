...
void kernel_3212211111_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1 - 1; i_2 >= 1; i_2--) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                B[i_1-1][i_2][0] = C[i_1][i_2] * B[i_1][i_2-1][0] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3212211111_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
