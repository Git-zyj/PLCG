...
void kernel_2312211225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = PB_M-1 - 1; i_2 >= 1; i_2--) {
            B[i_2+1] = D[i_2+1] - 3;
            B[i_2+1] = B[i_0+1] - B[i_2+1] + B[i_2-1] - 1;
        }
    }
#pragma endscop
...
kernel_2312211225_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
