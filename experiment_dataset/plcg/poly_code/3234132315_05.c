...
void kernel_3234132315_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < i_0; i_2++) {
                B[i_0+1][i_1+1][i_2+1] = C[i_0-1][i_2][i_2-2] * C[i_1-1][i_0][i_1] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_M-2; i_4++) {
            for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
                B[i_5][i_4+1][i_3] = C[i_4-2][i_5][i_3] * C[i_5-2][i_3][i_4] * C[i_4+2][i_3][0] + C[i_5][i_4+2][i_5] + B[i_5][i_4+1][i_3-1] - B[i_5+1][i_4+1][i_3] - B[i_5][i_4][i_3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3234132315_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
