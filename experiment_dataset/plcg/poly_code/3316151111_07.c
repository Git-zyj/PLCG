...
void kernel_3316151111_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0-1][i_0] = D[i_1][i_0] * D[i_1+i_0][i_1+1] * E[i_1-1] * D[i_0][i_1] * D[i_1-1][i_0] - D[i_0-1][i_1] + D[i_1][i_1+i_0] + A[i_1+i_0-2][i_0] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_P, -i_2+PB_M); i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_M-1, i_2); i_4++) {
                B[i_3][i_2][i_4] = F[i_4][i_3+i_2][i_4-1] + D[i_4+i_3+1][i_2] * F[i_2][i_4+1][i_3] + C[i_3][0] - 2;
            }
            C[i_3][0] = D[i_2-1][i_2] * D[i_3+1][i_3+i_2] * D[i_2][i_3] * D[i_3][i_2] + 5;
        }
    }
#pragma endscop
...
kernel_3316151111_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
