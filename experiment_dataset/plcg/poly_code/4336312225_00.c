...
void kernel_4336312225_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-4; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L-4; i_1++) {
            A[i_0-2][i_1+i_0+2] = A[i_0-1][i_1+i_0+3] + A[i_0][i_1+i_0+4] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_2+2][i_3+2] = B[i_2+2][i_3+3] + B[i_2+2][i_3+4] - 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            C[i_4-1][i_5] = D[i_4][i_5] * C[i_4-1][i_5-1] * C[i_4+1][i_5] * 6;
        }
    }
#pragma endscop
...
kernel_4336312225_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
