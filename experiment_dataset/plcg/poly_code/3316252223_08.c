...
void kernel_3316252223_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0+1] + E[i_0] * E[i_0] + E[i_0-2] * 3;
    }
    for (int i_1 = 3; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M+2; i_2++) {
            C[i_2+i_1-2][i_1-3][i_1-2] = F[i_1][i_2] * G[0][i_1] + A[i_2] + 6;
        }
        for (int i_3 = 1; i_3 < -i_1+PB_M-2; i_3++) {
            C[i_3+i_1][i_1-1][i_1] = F[i_1][i_3+1] + F[i_1][i_3+2] - H[i_1][i_3+2][i_1] + F[i_1][i_3] * G[i_3][i_1] - F[i_3][i_1-2] * G[i_3][i_1-2] * G[i_3][i_3+i_1] - C[i_3+i_1-1][i_3-1][i_3] - C[i_3+i_1+1][i_1][i_1+1] + C[i_3+i_1+2][i_3-1][i_3] * 6;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3316252223_08(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
