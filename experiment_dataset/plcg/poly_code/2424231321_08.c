...
void kernel_2424231321_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        A[i_0] = E[i_0+1] + F[i_0-1] - A[i_0+2] - A[i_0-1] + D[i_0] * 4;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M+1, PB_L-1); i_2++) {
            B[i_2+i_1-1][i_2] = G[i_2+1][i_2] + G[i_1][i_2] * E[i_2] + H[i_1][i_2] + A[i_2-1] - A[i_2] - B[i_2+i_1-2][i_2-1] + D[i_2] + 6;
        }
        D[i_1] = E[i_1] - F[i_1] + F[i_1] + D[i_1+2] - 3;
        D[i_1] = F[i_1] * F[i_1] - E[i_1] + D[i_1-2] * 4;
    }
#pragma endscop
...
kernel_2424231321_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
