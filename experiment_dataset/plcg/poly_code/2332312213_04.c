...
void kernel_2332312213_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L-3; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_L-2); i_2++) {
            B[i_1+1][i_2+2] = D[i_2+i_1][0] + B[i_2+1][i_1+3] + 4;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M-2; i_4++) {
            C[i_4+i_3+1][i_4] = C[i_4+i_3+2][i_4+1] + 1;
        }
    }
#pragma endscop
...
kernel_2332312213_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
