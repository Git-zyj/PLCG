...
void kernel_3236311315_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            A[i_0][i_1] = C[i_1][i_0] * A[i_0][i_1+1] - A[i_0+1][i_1-1] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        B[i_2+1][i_2] = B[i_2][i_2-1] + B[i_2+2][i_2+1] - B[i_2][i_2-1] - B[i_2+2][i_2+1] - 4;
    }
#pragma endscop
...
kernel_3236311315_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
