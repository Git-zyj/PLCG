...
void kernel_3432211321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0][i_1] = E[i_1] + A[i_1][i_0+1] - 4;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M-3; i_2++) {
            for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
                B[i_2][i_2+i_0+1][i_3+1] = B[i_2-1][i_2+i_0][i_3+1] + B[i_2-1][i_2+i_0+2][i_3-1] * B[i_2][i_3+i_2-1][i_0+1] + B[i_3-2][i_3+i_2-1][i_0+3] * B[i_2+2][i_2+i_0+3][i_3+1] + 2;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 4; i_5 < -i_4+PB_M-2; i_5++) {
            for (int i_6 = i_5; i_6 < PB_M-1; i_6++) {
                C[i_5][i_6+i_5] = F[i_6+1][i_5+1][i_4] + B[i_5][i_5+i_4+2][i_6+1] + C[i_6-1][i_6+i_5-1] + C[i_5-2][i_6+i_5-4] - C[i_6+1][i_6+i_5+1] * 6;
            }
        }
        for (int i_7 = 1; i_7 < -i_4+PB_M; i_7++) {
            D[i_7+i_4][i_7+1] = G[i_7-1][i_4-1] + D[i_7+i_4-1][i_7] * D[i_7+i_4-1][i_7+1] * 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3432211321_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
