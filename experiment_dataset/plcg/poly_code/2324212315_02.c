...
void kernel_2324212315_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = 1; i_2 < min(-i_0+PB_M-2, PB_L); i_2++) {
            B[i_0][i_2] = D[i_2][i_0-2] * D[i_2+i_0+2][1] - A[i_2][0] - B[i_0+1][i_2-1] * B[i_2+1][i_0] - C[i_2-1] * 6;
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        C[i_3-1] = C[i_3-2] * C[i_3] - C[i_3] + C[i_3] - C[i_3] - 2;
    }
#pragma endscop
...
kernel_2324212315_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
