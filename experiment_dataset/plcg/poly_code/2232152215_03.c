...
void kernel_2232152215_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L; i_1++) {
            A[i_1] = C[i_1] * D[i_1][i_0] - D[i_1][i_0+2] - D[i_0][i_1] - E[i_1][i_1-1][i_0] * D[i_1][i_0+2] + A[i_1+1] - A[i_1-1] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_N; i_3++) {
            B[i_3][i_2] = D[i_3-2][i_2-2] - D[i_3][i_2-1] * D[i_3-1][i_2-2] - D[i_3][i_2] * B[i_3][i_2-1] - 6;
        }
    }
#pragma endscop
...
kernel_2232152215_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
