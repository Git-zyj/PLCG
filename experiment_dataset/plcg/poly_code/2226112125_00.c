...
void kernel_2226112125_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, PB_M-1); i_3++) {
            A[i_3+1][i_3-2] = C[i_2][i_3-1] + C[i_3][i_2] + A[i_3+1][i_3-2] + 3;
        }
    }
#pragma endscop
...
kernel_2226112125_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
