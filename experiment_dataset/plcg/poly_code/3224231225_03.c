...
void kernel_3224231225_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[i_0][i_1-1][i_2] = C[i_2][i_0][i_1] + C[i_2][i_0][i_1] - C[i_2+1][i_0+1][i_1] + A[i_0][i_1+1][i_2-2] - A[i_0+2][i_1+1][i_2] + 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
            for (int i_5 = 1; i_5 < PB_P; i_5++) {
                A[i_5+1][i_3-1][i_4] = D[i_4][i_5-1][i_3] - D[i_5-1][i_4+i_3][i_5] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3224231225_03(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
