...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            A[i_2] = F[i_2][i_1] + F[i_1][i_1-1] - F[i_1][i_2+1] - G[i_2][i_1] * 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        C[i_3] = H[i_3+1] + D[i_3] * F[i_3][1] * D[i_3] - E[i_3+1] + D[i_3] + E[i_3-1] * C[i_3+1] * C[i_3-1] - 2;
    }
#pragma endscop
...
kernel_3332351111_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
