...
void kernel_2414352211_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + E[0] * F[i_0+2] + F[i_0] * E[0] + E[i_0+1] + A[i_0-1] + 5;
    }
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        B[i_1] = E[i_1] - B[i_1-1] - 3;
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            C[i_2] = G[i_2+1][i_1] - H[0][i_2] - H[i_1+2][2] + F[i_2] * I[i_1+1][i_2-2] - G[i_2+2][i_1] - B[i_2] * D[i_1][i_2] + 1;
            D[i_1+1][i_2] = J[i_2] + H[i_2][i_1+2] * H[i_2+2][i_1] + G[i_2-1][i_1+2] - H[i_2][2] + I[i_1][i_2] - A[i_2] - C[i_2+1] * C[i_2] - D[i_2][i_1+1] - 4;
        }
    }
#pragma endscop
...
kernel_2414352211_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
