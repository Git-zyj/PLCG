...
void kernel_4316152315_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_2][i_1][i_0] = D[i_2][i_0][i_1] * D[i_1+2][i_0][i_2] * D[i_1][i_0+1][i_0] * D[i_2+1][i_1+i_0][i_1] * D[i_1][i_0][i_2] * 1;
            }
            for (int i_3 = 2; i_3 < -i_1+PB_L; i_3++) {
                for (int i_4 = 2; i_4 < -i_1+PB_L-1; i_4++) {
                    B[i_0+1][i_4+2][i_3][i_1] = E[i_0+1][0][i_4-1][i_3-2] * D[i_3+2][i_4+i_1+1][i_4] - E[i_3+i_1][i_1+1][i_0][i_4] * E[i_4-2][i_3+i_1][i_0][i_3] * A[i_4-1][i_1][i_0] * A[i_3][i_0-1][i_1] - B[i_0+1][i_4+1][i_3][i_1] * B[i_0+1][i_4+2][i_3-1][i_1] + B[i_0+2][i_3+1][i_4][i_1-1] - 4;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L-2; i_5++) {
        for (int i_6 = max(2, i_5); i_6 < min(-i_5+PB_L, PB_L-2); i_6++) {
            C[i_5][i_6] = F[i_6+2][i_5] + G[i_6-2] + G[i_6] - G[i_6-1] + F[i_5+2][i_6] + F[i_6+i_5][i_6] * C[i_5-1][i_6] * C[i_5][i_6+1] - C[i_5][i_6-1] + C[i_5+1][i_6] * 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4316152315_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G)));
