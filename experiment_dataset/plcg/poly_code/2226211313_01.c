...
void kernel_2226211313_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q-1; i_1++) {
            A[i_1] = B[i_0+1][i_1+1][0] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_3][i_2][0] = C[i_3+1][i_3] - A[i_3] - B[i_3][i_2-1][0] * B[i_2][i_3-1][0] - B[i_3-1][i_2][0] + 2;
        }
    }
#pragma endscop
...
kernel_2226211313_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
