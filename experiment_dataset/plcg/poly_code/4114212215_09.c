...
void kernel_4114212215_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q-1, PB_M-2); i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_2+PB_Q, i_2); i_3++) {
                    A[i_2][i_3-2][i_1][i_1+i_0+1] = B[i_1+2][i_1][0] + A[i_2-1][i_3-1][i_1][i_1+i_0+1] * A[i_0][i_1-2][i_2][i_3+i_2] + 1;
                }
            }
        }
    }
...
kernel_4114212215_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
