...
void kernel_2336151221_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_N; i_1++) {
            A[i_0][i_1-1] = D[i_1][i_0] - E[i_1+1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_N, -i_2+PB_M); i_3++) {
            B[i_3][i_2] = D[i_3-1][i_3+i_2] + D[i_3][i_2] - E[i_3] * D[i_3][i_2] - D[i_3][i_2-1] + F[i_3][i_2-1][i_2-1] + A[i_2][i_3] + B[i_2-1][i_3] - B[i_3][i_2-1] * 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_N); i_5++) {
            C[i_5][i_4] = D[i_4][1] - D[i_5+1][i_4] * D[i_4][i_5-1] - D[i_5+i_4][i_5] * D[i_5][i_4-1] * D[i_5][i_4] + A[i_5][i_4-1] + 2;
        }
    }
#pragma endscop
...
kernel_2336151221_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
