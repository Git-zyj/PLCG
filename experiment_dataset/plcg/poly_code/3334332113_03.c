...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < min(min(-i_1+PB_M, PB_L), i_1); i_2++) {
            A[i_2-1] = F[i_2+1] - G[i_2+i_1][i_2] + 2;
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_L, -i_3+PB_M+2); i_4++) {
            C[i_4-2][i_3][0] = G[i_4+i_3-2][0] - G[i_4+1][i_3] + H[i_3-2][i_3][i_4] * C[i_4-2][i_3+1][0] + 1;
        }
    }
#pragma endscop
...
kernel_3334332113_03(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
