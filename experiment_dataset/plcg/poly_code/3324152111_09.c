...
void kernel_3324152111_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_N-1, -i_0+PB_N+2); i_1++) {
            for (int i_2 = 0; i_2 < PB_N; i_2++) {
                A[i_1][i_2][i_0] = D[i_1][i_2][i_0] * D[i_2][i_0+1][i_1] + D[i_1+1][i_0][0] - D[i_2][i_1][i_0+1] - D[i_0+2][i_1][i_1-2] - 2;
            }
            for (int i_3 = 2; i_3 < min(PB_N-1, -i_0+PB_P-2); i_3++) {
                B[i_0][i_1][i_3] = D[i_3+1][i_0-1][i_1] * D[i_3-2][i_0][i_1] * D[i_1+i_0-2][i_3][i_1] + D[i_3-1][i_1][i_0] * D[i_1][i_3][i_0+2] + E[i_1][i_3+i_0+2] + B[i_0-1][i_1][i_3+1] + B[i_0+1][i_1][i_3] - B[i_0-1][i_3+1][i_1+1] - 6;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < min(i_5, -i_5+PB_N); i_6++) {
                C[i_4][i_5][i_6] = D[i_6+i_5][i_4][i_6-1] + E[i_4][i_6] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3324152111_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
