...
void kernel_3436311223_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_2 = 3; i_2 < PB_N+1; i_2++) {
        B[i_2-1] = E[i_2][1] * B[i_2-2] * B[i_2-3] - C[i_2] * 5;
        C[i_2] = 5;
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            D[i_3][i_4] = B[i_4-1] + D[i_3+2][i_4] * D[i_3-2][i_4] - 6;
        }
    }
#pragma endscop
...
kernel_3436311223_08(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
