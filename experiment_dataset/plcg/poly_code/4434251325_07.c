...
void kernel_4434251325_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_L); i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
                    B[i_2][i_2] = E[i_3-1][i_0-1][i_1][0] + F[i_3+i_2-1][i_0][i_0] + E[i_3+i_2][i_0][i_0+1][i_1+1] - E[i_3][i_0][i_1][i_2] + F[i_2+i_1][i_3][i_0] - 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = i_4; i_5 < PB_L; i_5++) {
            B[i_4][i_4] = G[i_4+1][1] + H[i_5][i_4+1] + H[i_4-1][i_5] - B[i_4-2][i_4-2] * B[i_4+1][i_4+1] * B[i_4+2][i_4+2] - 4;
        }
    }
    for (int i_6 = 1; i_6 < PB_L; i_6++) {
        for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
            B[i_7][i_7] = I[i_6][i_7+1][i_7] - F[i_6-1][i_7][i_6-1] + H[i_7][i_7] + H[i_6][0] - 2;
        }
        for (int i_8 = 1; i_8 < PB_L-1; i_8++) {
            for (int i_9 = 0; i_9 < min(i_6, -i_8+PB_M-1); i_9++) {
                B[i_8+1][i_8+1] = F[i_9+i_8+1][0][i_6] + I[i_8][i_9+1][i_6] * F[i_8][i_6][i_9+1] - I[i_9][i_8][i_6] + F[i_9+1][i_9][i_8] * G[i_9+1][i_8-1] + I[i_6+1][i_8][i_6-1] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4434251325_07(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
