...
void kernel_2226151321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < min(-i_0+PB_L, PB_L-1); i_1++) {
            A[i_0][i_1-1] = C[i_1+1][i_0] + D[i_1] + C[i_1+i_0][0] + C[i_1+i_0][i_0] - D[i_1] + C[i_1+1][i_0] + A[i_1][i_0+1] - A[i_0][i_1-2] + A[i_0][i_1-3] + 3;
        }
    }
    for (int i_2 = 4; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_P-1; i_3++) {
            B[i_3][i_3+i_2-1] = C[i_2][i_3-1] * C[i_2][i_3] - C[i_3][1] * D[i_2] * B[i_3+2][i_3+i_2+1] * B[i_3-1][i_3+i_2-3] + B[i_3-2][i_3+i_2-4] - 5;
        }
    }
#pragma endscop
...
kernel_2226151321_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
