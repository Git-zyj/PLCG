...
void kernel_2324212321_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_L-4, -i_0+PB_M-1); i_1++) {
            A[i_0][i_1+2] = A[i_0][i_1+4] + A[i_0][i_1+1] - B[i_1+i_0][i_1-1] * B[i_1+i_0][i_1] * B[i_1+i_0+1][i_1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        ...
        for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
            C[i_4] = D[i_2][i_4-2] + C[i_4-2] * C[i_4+1] * C[i_4+2] - 1;
        }
    }
#pragma endscop
...
kernel_2324212321_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
