...
void kernel_2424112211_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M+1; i_1++) {
        for (int i_2 = 3; i_2 < PB_M; i_2++) {
            C[i_2][i_1-2] = A[i_2-1] * C[i_2][i_1-1] * C[i_2-1][i_1-2] * D[i_1-2][i_2-3] * 5;
            D[i_1-1][i_2-2] = 6;
        }
    }
#pragma endscop
...
kernel_2424112211_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
