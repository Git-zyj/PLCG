...
void kernel_3432331315_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] - F[i_0+1] + F[i_0] - A[i_0-1] - 5;
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_1] = G[i_1+i_0][1] + H[i_1][i_0] - I[i_0][i_0] * A[i_1+1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            D[i_3][i_2] = I[i_2+1][i_3] - J[i_3-1] - I[i_3+1][i_2-1] - D[i_2+1][i_3-1] * D[i_3-1][i_2-1] + 4;
            D[i_3][i_2] = E[i_3] * H[i_2+1][0] * A[i_3] + A[i_3-1] * 2;
        }
    }
#pragma endscop
...
kernel_3432331315_03(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
