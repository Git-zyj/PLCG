...
void kernel_3326312323_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        A[i_0-2] = D[i_0] + A[i_0-4] * A[i_0-3] + A[i_0] * 1;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-3; i_2++) {
            for (int i_3 = 0; i_3 < PB_N-3; i_3++) {
                C[i_3+3][i_2+1] = C[i_3+2][i_2+3] - 3;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, PB_N-2); i_5++) {
            C[i_5+2][i_4+1] = C[i_5+2][i_4] - 3;
        }
    }
#pragma endscop
...
kernel_3326312323_03(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
