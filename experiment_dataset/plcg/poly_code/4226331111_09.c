...
void kernel_4226331111_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_P-1, -i_1+PB_L); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_P-1, i_0); i_3++) {
                    A[i_1][i_2][i_0] = C[i_3][i_2+i_1][i_2] + D[i_1][i_3][i_3+i_2+1][i_0+1] * C[i_2][i_1-1][i_3] * 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, PB_L-2); i_5++) {
            for (int i_6 = 1; i_6 < min(min(min(-i_5+PB_P, PB_P), -i_4+PB_M), i_5); i_6++) {
                B[i_4-1][i_5+1][i_6-1] = C[i_5][i_4-1][i_6+i_5] * E[i_5][i_6+i_4] - B[i_4-1][i_5+2][i_6] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4226331111_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
