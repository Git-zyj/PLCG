...
void kernel_4216212313_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q+2, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_2][i_1][i_1-1] = C[i_2][i_0][i_1] + C[i_2][i_1+i_0-2][0] * A[i_2-1][i_1+1][i_1] + A[i_2-1][i_1][i_1-1] * A[i_2][i_1-1][i_1-2] + 6;
            }
        }
    }
...
kernel_4216212313_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
