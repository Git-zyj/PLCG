...
void kernel_3436232215_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L; i_2++) {
                B[i_1][i_2] = E[i_2][i_0][i_1][0] * F[i_0][i_2][i_1] - 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_L+1; i_4++) {
            B[i_3][i_4-1] = G[i_4][i_3+2] * H[i_3][i_4] * H[i_4][i_3-1] - B[i_3-1][i_4-1] * 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        for (int i_6 = 2; i_6 < min(i_5, PB_L-2); i_6++) {
            B[i_6][i_5-1] = G[i_5][i_6+2] * G[i_6-2][i_5] + G[i_6][i_5-1] + 3;
        }
    }
    for (int i_7 = 1; i_7 < PB_L+1; i_7++) {
        for (int i_8 = max(2, i_7); i_8 < PB_L+2; i_8++) {
            B[i_8-1][i_7-1] = H[i_7][i_8] - B[i_7][i_8-2] * 4;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3436232215_01(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
