...
void kernel_3324111313_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                A[i_2][i_1] = D[i_1][i_1][i_2] + A[i_2+1][i_1] - A[i_2-1][i_1] * A[i_2][i_1-1] + A[i_2+1][i_1-1] - A[i_2][i_1+1] - A[i_0][i_2+1] + 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                B[i_4][i_3+1][i_5] = D[i_4-1][i_3][i_3] + B[i_4][i_3+2][i_5] - B[i_4][i_5+1][i_3+1] - 1;
            }
        }
        for (int i_6 = 0; i_6 < PB_M; i_6++) {
            for (int i_7 = 1; i_7 < PB_N; i_7++) {
                C[i_7][i_7] = C[i_7-1][i_7-1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3324111313_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
