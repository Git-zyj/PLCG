...
void kernel_2416251225_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0-1] = E[i_0] * F[i_0] + E[i_0-1] + E[i_0+1] + E[i_0] - A[i_0-2] - 1;
        for (int i_1 = i_0; i_1 < -i_0+PB_N; i_1++) {
            B[i_1+i_0][i_1+1] = G[i_0][i_1] * F[i_0] + A[i_0-1] - B[i_1+i_0-1][i_0] * 4;
        }
    }
    for (int i_2 = 3; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 3; i_3 < min(PB_N, i_2); i_3++) {
            A[i_3-3] = G[i_3][i_2] - H[i_3][i_3] - E[i_2] * 5;
        }
        A[i_2-3] = E[i_2+1] + E[i_2] * E[i_2-1] * E[i_2] + F[i_2+1] + E[i_2] * E[i_2] + E[i_2] * F[i_2] * E[i_2] - 5;
    }
#pragma endscop
...
kernel_2416251225_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
