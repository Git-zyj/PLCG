...
void kernel_4216112325_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = 1; i_2 < min(i_0, PB_L-2); i_2++) {
            for (int i_3 = max(0, -i_2+1); i_3 < min(i_2, -i_2+PB_M-2); i_3++) {
                B[i_3+i_2][i_2] = C[i_2][i_3][i_2] - B[i_3+i_2+2][i_2] * B[i_3+i_2-1][i_2-1] - B[i_3+i_2+2][i_2+2] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4216112325_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
