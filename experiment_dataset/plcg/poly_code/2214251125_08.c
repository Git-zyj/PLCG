...
void kernel_2214251125_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M - 1; i_0 >= 0; i_0--) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_M-1); i_1++) {
            A[i_0][i_1] = C[i_0][i_1-1] + D[i_1+i_0] + E[i_0][0] * C[i_1+1][0] + E[i_1+1][i_0] + A[i_0][i_1-2] - 6;
        }
        for (int i_2 = 1; i_2 < min(PB_M, -i_0+PB_L); i_2++) {
            A[i_0][i_2-1] = E[i_2+i_0][i_2+1] * D[i_2-1] * C[i_0][i_2] * C[i_2][0] - 2;
        }
    }
#pragma endscop
...
kernel_2214251125_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
