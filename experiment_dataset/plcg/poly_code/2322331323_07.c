...
void kernel_2322331323_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            A[i_0+1][i_0] = D[i_0+1][i_1-1] + D[i_1][i_0-1] - D[i_1][i_0+1] - A[i_0-1][i_0-2] - A[i_0+2][i_0+1] - A[i_0][i_0-1] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < PB_M; i_3++) {
            B[i_2][i_2-1] = D[i_2][i_3] * E[i_3] * D[i_3][i_2+1] - B[i_3-2][i_3-3] * B[i_2-1][i_2-2] - B[i_2+1][i_2] + 3;
        }
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            C[i_2][i_4] = F[i_2] * E[i_2] * G[i_4][i_2] - C[i_2+1][i_4-1] * C[i_4][i_2+1] + 1;
        }
    }
#pragma endscop
...
kernel_2322331323_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
