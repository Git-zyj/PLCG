...
void kernel_2426251313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] + E[i_0] * F[i_0] * F[i_0] + E[i_0] - F[i_0+1] * G[i_0][i_0] + A[i_0-1] - A[i_0+1] * A[i_0+1] - 1;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1] = E[i_1+1] - F[i_1] * F[i_1] + F[i_1] + E[i_1] * C[i_1] + D[i_1] * 2;
        for (int i_2 = 0; i_2 < min(-i_1+PB_M-1, i_1); i_2++) {
            C[i_2] = G[i_1][i_2+1] + H[i_2+i_1+1][i_2+1] + G[i_2+i_1][i_1+1] - I[i_1][i_2][i_2+1] + B[i_2] - D[i_2+1] + 4;
        }
        D[i_1+1] = E[i_1-1] - E[i_1] + E[i_1] * F[i_1] - B[i_1] * C[i_1] + 1;
    }
#pragma endscop
...
kernel_2426251313_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
