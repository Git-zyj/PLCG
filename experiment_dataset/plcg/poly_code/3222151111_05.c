...
void kernel_3222151111_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = C[i_0+1] + C[i_0] + C[i_0] + C[i_0] + D[i_0][i_0] + A[i_0-1] - 4;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M+1; i_2++) {
                B[i_2+i_1-1][i_1] = E[i_0+1][i_2][i_1] + E[i_2][i_1][i_1] - E[i_1][i_2][i_1] + E[i_2][i_0][i_1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3222151111_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
