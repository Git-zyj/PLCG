...
void kernel_3326132213_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_P, PB_L+1); i_2++) {
                A[i_1][i_2][i_0] = D[i_0-1][i_2+i_1][i_2-2] + D[i_2-1][i_0][i_1] - A[i_1][i_2][i_0-1] + A[i_1][i_2+1][i_0-1] * 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        ...
        for (int i_5 = i_3; i_5 < -i_3+PB_L; i_5++) {
            for (int i_6 = 2; i_6 < -i_3+PB_L+2; i_6++) {
                C[i_6-1][i_5][i_3] = D[i_5+i_3][i_6+i_3+2][0] - D[i_6+i_3-2][i_5+2][0] + E[i_6][0] + D[i_6][i_6][i_3] - D[i_6][i_6][i_5] + D[i_6][i_3-2][i_5] * A[i_6-1][i_3][i_5] * C[i_6-2][i_5+1][i_3+1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3326132213_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
