...
void kernel_3224252321_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, -i_0+PB_L); i_2++) {
                B[i_2][i_1] = C[i_1-1][i_2+i_0][0] * D[i_2+i_1][i_2+i_0][i_0+1] * B[i_2][i_1] * 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < min(-i_3+PB_M, PB_L); i_4++) {
            for (int i_5 = 2; i_5 < min(min(-i_3+PB_L, i_4), -i_4+PB_P); i_5++) {
                B[i_4][i_5] = D[i_5][i_4+i_3][i_3-2] * D[i_4][i_3][i_5] + D[i_4][i_3+1][i_5+i_4] + D[i_5-2][i_3][i_3+1] - D[i_5][i_4][i_3] * C[i_4+2][i_5][i_3] + E[i_5][i_4] * C[i_4][i_5+i_3][i_4+1] * B[i_4][i_5+1] + B[i_4][i_5-2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3224252321_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
