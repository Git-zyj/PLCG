...
void kernel_3316112313_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        A[i_0+2] = D[i_0+2] - C[i_0] + C[i_0-1] + 4;
        for (int i_1 = 0; i_1 < PB_N-3; i_1++) {
            B[i_0][i_1] = E[i_0+2][i_1+2] * A[i_1+2] * A[i_1+1] * A[i_1+3] * B[i_0+1][i_1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            C[i_3] = E[i_2][i_3] + B[i_2][i_3] * C[i_3+1] * C[i_3-1] + 5;
        }
    }
#pragma endscop
...
kernel_3316112313_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
