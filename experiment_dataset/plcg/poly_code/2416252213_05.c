...
void kernel_2416252213_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        B[0] = E[i_0] - E[i_0] * E[i_0+2] + B[0] + 2;
        B[0] = F[i_0][i_0-2] - E[i_0] * E[i_0] - G[i_0+1] * E[i_0] * B[0] - B[0] + 5;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < min(i_1, PB_M-2); i_2++) {
            C[i_1][i_2+1] = F[i_2+2][0] + F[i_2][i_1] * F[i_2][i_1] + G[i_1+1] + H[i_2+2][i_1+2] - H[i_2][i_1+1] - G[i_2] + 2;
        }
        for (int i_3 = 1; i_3 < i_1; i_3++) {
            D[i_3-1] = F[i_1][i_3+2] - F[i_1][i_3] * G[i_3] + G[i_3] - D[i_3] * 4;
        }
    }
#pragma endscop
...
kernel_2416252213_05(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
