...
void kernel_4234311321_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_L+1, -i_1+PB_L+1); i_2++) {
                A[i_2+i_1-1][i_0-1] = C[i_2][i_0][i_1+i_0] + A[i_2+i_0-1][i_1-2] - 1;
            }
        }
        B[i_0-1] = B[i_0-3] * B[i_0-2] + B[i_0+1] - B[i_0] + B[i_0-3] - 2;
    }
#pragma endscop
...
kernel_4234311321_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
