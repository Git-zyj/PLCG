...
void kernel_3216212325_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            B[i_1-2][i_0+2][i_0+2] = C[i_1][i_0+2] + B[i_0-2][i_1][i_1] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_N+2; i_3++) {
            for (int i_4 = 3; i_4 < min(-i_3+PB_P, PB_N+2); i_4++) {
                B[i_4-2][i_2][i_2] = D[i_4][i_2][i_4+i_3] + B[i_4-3][i_2][i_2] + B[i_3-2][i_4-2][i_4-2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3216212325_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
