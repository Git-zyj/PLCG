...
void kernel_2322252325_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            B[i_1+1] = D[i_1][i_0] * D[i_1-1][i_0] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        B[i_2] = E[i_2+1] - F[i_2-1] - E[i_2] - F[i_2] + F[i_2-1] * E[i_2] - E[i_2+1] - B[i_2-1] * B[i_2+2] + B[i_2+1] * 1;
    }
    for (int i_3 = 2; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_P, PB_L+2); i_4++) {
            B[i_3+1] = G[i_4+i_3-2][0] * H[i_4+i_3][i_4][i_4-2] + F[i_4+i_3-2] * G[i_4+2][i_4+i_3] - 6;
        }
    }
#pragma endscop
...
kernel_2322252325_01(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
