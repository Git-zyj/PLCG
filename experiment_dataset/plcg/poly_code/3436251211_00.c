...
void kernel_3436251211_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            B[i_1] = E[i_1+1] + F[i_1][i_0] * G[i_0-1][i_1+1] - G[i_1][i_0-1] * G[i_1][i_1] * F[i_1][i_0-1] * 5;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_2] = E[i_0] - G[i_2-1][i_0] + G[i_0][i_2] - F[i_0+1][i_2] + B[i_2-1] + B[i_2+1] + 2;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = i_3; i_4 < min(PB_M+1, -i_3+PB_L+1); i_4++) {
            C[i_4-1] = F[i_3][i_4+i_3-1] * F[i_3+1][i_4+1] + H[i_3-1][i_4-1][i_3] + 1;
        }
        C[i_3-1] = E[i_3] * E[i_3] * I[0] * I[i_3-1] * I[i_3+1] - E[i_3] * C[i_3-2] + 3;
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3436251211_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
