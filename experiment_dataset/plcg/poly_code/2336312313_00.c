...
void kernel_2336312313_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        ...
        for (int i_3 = 0; i_3 < -i_1+PB_M-1; i_3++) {
            C[i_3+i_1][i_1] = D[i_1][i_3+2] * C[i_3+i_1+1][i_1+1] - C[i_3+i_1-2][i_1-1] * C[i_3+i_1-1][i_1-1] * C[i_3+i_1-1][i_1] + C[i_3+i_1][i_1-1] * 6;
        }
    }
#pragma endscop
...
kernel_2336312313_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
