...
void kernel_3412311223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        A[i_0+1] = E[i_0] * 4;
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            B[i_1][0] = B[i_1+2][0] + B[i_1+1][0] * B[i_1-2][0] * 6;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_L, i_0); i_2++) {
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                B[i_2][0] = F[i_3-1][i_0][i_2+i_0] * G[i_3][i_0+1][i_2] - 2;
            }
        }
    }
...
kernel_3412311223_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
