...
void kernel_2436351211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = E[i_0-1] * E[i_0] + F[i_0-1] - F[i_0] - F[i_0] * A[i_0-1] + A[i_0+1] + 6;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
            B[i_2] = G[i_1-1][i_2] + E[i_2] * H[i_2][i_1+1] - H[i_1-1][i_2] - B[i_2-1] * B[i_2+1] * 1;
        }
        for (int i_3 = 1; i_3 < min(-i_1+PB_M-1, i_1); i_3++) {
            C[i_3+i_1][i_1] = G[i_1+1][i_3+1] + I[i_3+1][0] * E[i_3-1] * E[i_1] - G[i_3+i_1+1][i_3-1] - C[i_3+i_1-1][i_1-1] - C[i_3+i_1+1][i_1+1] + D[i_3-1] - 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_N; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_N; i_5++) {
            D[i_5-1] = J[i_5+i_4] + H[i_5+i_4][i_5] + H[i_5][i_4] + D[i_5] - 3;
        }
    }
#pragma endscop
...
kernel_2436351211_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
