...
void kernel_2324112323_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        A[i_0-2] = D[i_0] - A[i_0] * A[i_0-4] * 5;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        A[i_1] = D[i_1] * D[i_1] * 4;
        for (int i_2 = 3; i_2 < PB_L; i_2++) {
            C[i_1+1][i_2-1] = C[i_1+1][i_2-3] - C[i_1+2][i_2] - C[i_1+1][i_2-2] * 3;
        }
    }
#pragma endscop
...
kernel_2324112323_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
