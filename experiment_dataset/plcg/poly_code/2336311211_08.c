...
void kernel_2336311211_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        ...
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            C[i_2][i_1][i_2] = D[i_2][i_1] + E[i_1] * B[i_2] + C[i_2+1][i_1][i_2+1] - C[i_2-1][i_1+1][i_2-1] - C[i_1][i_2+1][i_1] - 1;
        }
    }
#pragma endscop
...
kernel_2336311211_08(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
