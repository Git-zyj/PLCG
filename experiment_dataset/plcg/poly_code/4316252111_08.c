...
void kernel_4316252111_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(min(-i_0+PB_M, i_0), PB_M-2); i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_0+PB_Q, -i_1+PB_M-2), -i_2+PB_M); i_3++) {
                    A[i_0][i_1][i_3+i_2][i_3+i_0] = D[i_3][i_0][i_0][i_1] - D[i_3+1][i_2][i_1][2] + E[i_3+i_1+2][i_2][i_1][i_0] + E[i_3+i_2-1][i_0][i_0][i_1] * E[i_3+i_2][i_3+i_1][i_2+1][i_0] * E[i_2][i_1][i_0][i_3] * A[i_0-1][i_1-1][i_3+i_2+1][i_3+i_0] * 4;
                }
            }
            for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
                C[i_4+1][i_1-1] = F[i_1+i_0][i_4+2][i_4] - G[i_4+2][i_1] + F[i_4][i_1][i_0] * H[i_1][i_0][i_4-1] * H[i_4+i_1][i_1+i_0][i_1] * 3;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M; i_5++) {
        for (int i_6 = i_5; i_6 < PB_M-1; i_6++) {
            C[i_6+1][i_5] = H[i_5][i_6][i_5-2] + I[i_6] + G[i_5+1][i_6] * 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4316252111_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
