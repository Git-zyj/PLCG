...
void kernel_4312351311_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < -i_0+PB_Q; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_P; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_M; i_3++) {
                    A[i_3+i_2-1][i_1+1][i_3] = D[i_2][i_3][i_1] * E[i_2+i_1][i_3][i_0-1] - F[i_3+i_1][i_2][i_0][i_0+1][i_3] - G[i_3-1][i_2][i_3+1][i_1+i_0][i_1+1] * A[i_3+i_2-2][i_1+1][i_3] - A[i_3+i_2-2][i_1+2][i_3-1] - A[i_2+i_1-2][i_3+1][i_1] * A[i_2+i_1-1][i_3][i_1+1] - A[i_1+i_0-2][i_2+1][i_1-1] + 2;
                }
                for (int i_4 = 1; i_4 < min(-i_1+PB_M, -i_0+PB_P); i_4++) {
                    B[i_1+1][i_0][i_2][i_4+1] = H[i_2][i_4+i_1][i_0+1][i_0] + I[i_2+1][i_1][i_4+i_0][i_2] - H[i_2][0][i_1+i_0][i_2] + H[i_2][i_0][i_4+i_1][i_1] + J[i_1+1][i_4-1][i_0][i_4] + K[i_1-1][i_4+i_2][i_0] - B[i_1+1][i_4-1][i_2][i_0+1] - 3;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = 1; i_6 < min(i_5, PB_M-1); i_6++) {
            C[i_5][i_6] = L[i_6][i_5] + L[i_6+1][i_5] * M[i_6] * N[i_6][i_5] - C[i_5][i_6-1] - C[i_5-1][i_6] + 4;
        }
    }
#pragma endscop
...
kernel_4312351311_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), xn,yn,POLYBENCH_ARRAY(N));
