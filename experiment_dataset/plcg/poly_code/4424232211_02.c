...
void kernel_4424232211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    B[i_2-2][i_1-1][i_1][i_0-2] = G[i_1+i_0][i_3][i_2-2][i_2] - H[i_1][i_0][i_3][i_2] + B[i_2-1][i_1-1][i_1][i_0-3] * 4;
                }
            }
        }
        for (int i_4 = 2; i_4 < i_0; i_4++) {
            for (int i_5 = 2; i_5 < -i_0+PB_L; i_5++) {
                C[i_0][i_5][i_4-2] = I[i_4][i_5+i_0][i_0] * J[i_4][i_5] * C[i_4-1][i_0][i_5-2] * 5;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_L; i_6++) {
        for (int i_7 = 3; i_7 < PB_P-1; i_7++) {
            for (int i_8 = 2; i_8 < -i_7+PB_P; i_8++) {
                D[i_8][i_7][i_7-2] = I[i_8][i_6][i_8+2] + K[i_8-1][i_7+1][2] + K[i_6][i_8][i_8+i_7] * B[i_8-1][i_6-1][i_6][i_7-2] * C[i_8-1][i_6-1][i_7-3] * D[i_8+1][i_7][i_7-2] * D[i_7-1][i_8][i_8-2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4424232211_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
