...
void kernel_2224132215_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        B[i_0] = C[i_0] - C[i_0] + C[i_0] - 3;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < min(i_1, PB_M-1); i_2++) {
            B[i_2] = C[i_2] + C[i_2-1] - B[i_2-1] - B[i_2+1] * 3;
        }
    }
#pragma endscop
...
kernel_2224132215_09(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
