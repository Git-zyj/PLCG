...
void kernel_3432351313_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P, -i_0+PB_N); i_1++) {
            A[i_0+1][i_1-1] = E[i_1] - F[i_1][i_0+1] * F[i_0][i_0] + G[i_1+i_0][1] + F[i_0][0] + H[i_1][i_0] + A[i_1][i_0-1] + A[i_0][i_1-1] - A[i_0+1][i_1] * 6;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_2] = E[i_0] + F[i_0][i_2] - H[i_0+1][i_2] * B[i_2-1] - 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < PB_N; i_4++) {
            B[i_3] = G[i_4][i_3-1] - F[i_3+1][1] + G[i_3][i_4] - I[i_4] + F[0][i_3] * H[i_4][i_3-1] + B[i_3] * 5;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
        D[i_5+1] = E[i_5] * J[i_5] - B[i_5-1] * B[i_5] + D[i_5+2] - 4;
    }
#pragma endscop
...
kernel_3432351313_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
