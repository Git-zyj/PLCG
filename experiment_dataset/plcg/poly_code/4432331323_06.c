...
void kernel_4432331323_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_0] = E[i_1+i_0] * A[i_0-1][i_0-1] + A[i_0-2][i_0-2] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-3; i_2++) {
        B[i_2+1] = E[i_2] - F[i_2] * F[i_2] + B[i_2+3] * B[i_2-1] + B[i_2+2] + B[i_2] + 4;
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            C[i_3][i_4] = G[i_3][i_4] - H[i_3+1][i_4][i_4-1] - H[i_4][i_3][i_4+1] * B[i_4+1] * C[i_3+2][i_4] - C[i_3-1][i_4] - 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
        for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 0; i_7 < -i_6+PB_L; i_7++) {
                A[i_6][i_6] = H[i_7+1][i_6+1][i_5+1] - H[i_5][i_7+i_6][i_7+1] - I[i_5-1][i_7+i_6][i_6-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4432331323_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
