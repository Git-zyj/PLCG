...
void kernel_2426331215_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] + F[i_0] * A[i_0-1] * 4;
        for (int i_1 = 1; i_1 < min(i_0, PB_L-1); i_1++) {
            B[i_0][i_1] = G[i_1+1][i_0] - G[i_0][i_1] * H[i_1] * A[i_1] + B[i_0-1][i_1+1] - B[i_0+1][i_1-1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            A[i_3] = I[i_3][i_2] + E[i_3] - G[i_3][i_2] + G[i_3][0] - D[i_3+1] - 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        D[i_4] = F[i_4] + H[i_4+1] + H[i_4+1] + D[i_4+1] + 6;
    }
#pragma endscop
...
kernel_2426331215_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
