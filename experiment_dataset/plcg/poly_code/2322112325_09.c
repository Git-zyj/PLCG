...
void kernel_2322112325_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            A[i_0][i_1] = A[i_0-1][i_1] + A[i_0][i_1+2] - C[i_1-1] * 3;
        }
    }
    for (int i_2 = PB_L-2 - 1; i_2 >= 3; i_2--) {
        for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
            A[i_2-1][i_3-2] = C[i_3-1] + 5;
        }
        C[i_2-1] = D[i_2+2] - D[i_2] * C[i_2-3] + 5;
    }
#pragma endscop
...
kernel_2322112325_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
