...
void kernel_3212112313_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_1+i_0-2][i_2][i_0+2] = C[i_0][i_2] + A[i_1+i_0][i_2][i_0+3] - A[i_1+i_0-4][i_2][i_0+1] * A[i_1+i_0-3][i_2][i_0+2] + 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                B[i_3][i_5] = B[i_4-1][i_5] * B[i_5-1][i_4] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3212112313_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
