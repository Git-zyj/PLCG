...
void kernel_4316232123_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_P+1; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M; i_2++) {
                B[i_1-3][i_2] = D[i_2][i_0][i_1-1] - 5;
            }
        }
        for (int i_3 = 3; i_3 < min(-i_0+PB_M, i_0); i_3++) {
            B[i_3-2][i_0] = E[i_0+1] - F[i_3+2][i_0-1] + F[i_3+i_0][i_3-2] * F[i_0][i_3] - G[i_0-2][i_3+1] + G[i_0-2][i_3+i_0] * B[i_3-3][i_0] - 2;
        }
    }
        ...
        for (int i_5 = i_4; i_5 < PB_L; i_5++) {
            for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
                for (int i_7 = 1; i_7 < -i_5+PB_Q-2; i_7++) {
                    C[i_4+2][i_5][i_6][i_7+i_5] = H[i_4][i_6-1][i_7-1][i_5] * I[i_4][i_6+1][i_6+1][i_7+i_5+2] * 2;
                }
            }
        }
    }
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4316232123_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
