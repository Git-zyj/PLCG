...
void kernel_3322312321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = A[i_0-1] - A[i_0+2] * 4;
        for (int i_1 = 2; i_1 < PB_L-4; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < min(-i_0+PB_L, PB_L-2); i_2++) {
                B[i_2][i_1+2][i_2-2] = D[i_2][i_0][i_1] * D[i_1-2][i_2+i_0][i_2+2] * B[i_2][i_1+4][i_2-2] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-4; i_3++) {
        C[i_3+2] = C[i_3] - C[i_3+3] * C[i_3+4] + C[i_3+1] + C[i_3+3] - C[i_3+4] + 2;
    }
#pragma endscop
...
kernel_3322312321_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
