...
void kernel_2334112221_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
            A[i_0-2][i_1+2] = D[i_1-2][i_0] * A[i_1-2][i_0+4] + 6;
        }
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            B[i_0+1][i_2] = D[i_2+1][i_0] + B[i_0+2][i_2] - 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_N-4; i_3++) {
        C[i_3][i_3+2] = C[i_3-1][i_3+1] * C[i_3+2][i_3+4] + C[i_3+1][i_3+3] + 3;
    }
#pragma endscop
...
kernel_2334112221_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
