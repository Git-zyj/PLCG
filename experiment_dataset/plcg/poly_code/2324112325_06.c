...
void kernel_2324112325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L-2, PB_M-1); i_1++) {
            A[i_0][i_1+1] = D[i_1][i_0] + A[i_0][i_1-1] - C[i_1+1][i_1+i_0+2] - C[i_0][i_1+i_0] * 2;
        }
    }
    ...
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 0; i_5 < min(-i_4+PB_L-2, PB_M-1); i_5++) {
            C[i_5][i_5+i_4] = C[i_5][i_5+i_4+2] * C[i_5][i_5+i_4+1] * C[i_5+1][i_5+i_4+1] * 5;
        }
    }
#pragma endscop
...
kernel_2324112325_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
