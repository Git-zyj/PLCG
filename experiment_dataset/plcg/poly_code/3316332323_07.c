...
void kernel_3316332323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] - E[i_0] - A[i_0-1] - A[i_0+1] * A[i_0+2] - 5;
    }
    for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_L+2; i_2++) {
            B[i_2][i_2] = F[i_2][i_1-1] - G[i_1][i_2][1] - H[i_1+1][i_2-2] - E[1] + A[i_2] - B[i_2+2][i_2+2] * 2;
        }
        for (int i_3 = 2; i_3 < i_1; i_3++) {
            for (int i_4 = 0; i_4 < PB_P-2; i_4++) {
                B[i_4][i_4] = G[i_3-2][i_1][i_4+2] + B[i_3-2][i_3-2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3316332323_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
