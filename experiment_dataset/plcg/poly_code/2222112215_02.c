...
void kernel_2222112215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N-1; i_1++) {
            A[i_1+i_0] = C[i_0+2] + A[i_1+i_0-1] * A[i_1+i_0+1] + A[i_1+i_0+1] * 1;
        }
    }
    for (int i_2 = PB_P - 1; i_2 >= 1; i_2--) {
        for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
            B[i_3+2][i_2-1] = B[i_3+3][i_2] - 5;
        }
    }
#pragma endscop
...
kernel_2222112215_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
