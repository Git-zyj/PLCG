...
void kernel_4214111125_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = 3; i_2 < min(PB_M, -i_1+PB_N); i_2++) {
            B[i_2-1][i_2] = D[i_2+i_1][i_2-1] - B[i_2-3][i_2-2] + 1;
        }
    }
#pragma endscop
...
kernel_4214111125_06(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
