...
void kernel_4112152121_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+2; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            for (int i_2 = PB_N - 1; i_2 >= 0; i_2--) {
                A[i_2][i_1][i_0] = B[i_0-2][i_0][i_2] + B[i_1][i_0+1][i_2+1] + B[i_1-1][i_2][i_1-1] - B[i_2][i_1][i_0+1] + B[i_0-2][i_2][i_1] * A[i_2][i_1][i_0+1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4112152121_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
