...
void kernel_4226111215_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
        ...
        for (int i_1 = max(1, i_0); i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q+1; i_2++) {
                for (int i_3 = 1; i_3 < PB_Q; i_3++) {
                    A[i_2][i_2-1][i_3] = A[i_1][i_1-1][i_3-1] * A[i_3][i_3-1][i_1+1] * 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < -i_4+PB_M; i_5++) {
            for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
                A[i_4+1][i_4][i_5-1] = C[i_5+i_4][i_6+1][i_5] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4226111215_09(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
