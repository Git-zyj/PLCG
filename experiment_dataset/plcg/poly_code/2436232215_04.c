...
void kernel_2436232215_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        B[i_1-2] = E[1] - E[i_1-2] + F[i_1-2] - B[i_1-1] + 3;
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_N); i_2++) {
            A[i_2] = G[i_2][i_1] + E[i_2] * H[i_2+i_1][i_1-2] + E[i_1] * H[i_2][i_1] * 4;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        D[i_3+2] = E[i_3] - F[i_3] - D[i_3+1] - 3;
    }
#pragma endscop
...
kernel_2436232215_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
