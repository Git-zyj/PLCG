...
void kernel_2326352121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-4; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L-2; i_1++) {
            A[i_1-2][i_1+2] = D[i_1][i_1] * E[i_1] + D[i_1][i_0] - F[i_1][i_0] * G[i_1][i_1-1] - A[i_0][i_0+4] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        B[i_2] = H[i_2-2] - D[i_2][0] - E[i_2] - E[i_2] + E[i_2] - 6;
        for (int i_3 = 2; i_3 < min(-i_2+PB_L, i_2); i_3++) {
            B[i_3] = G[i_3][i_2+2] - F[i_3][i_3+1] * F[i_3+1][i_2] - I[i_3+i_2-1][i_3+i_2][1] + G[i_3-2][i_2] - B[i_3] - 2;
        }
    }
#pragma endscop
...
kernel_2326352121_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
