...
void kernel_4426311325_07(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                B[i_0][i_2+1][i_1][i_1+1] = B[i_1][i_0+1][i_2][i_2+1] - 2;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
            for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
                for (int i_6 = 0; i_6 < PB_Q-1; i_6++) {
                    B[i_6][i_4+1][i_5][i_5+1] = E[i_6+1][i_3+1][i_4][i_5] - B[i_3-1][i_5+1][i_6][i_6+1] * B[i_6+2][i_4+1][i_5][i_5+1] - 4;
                }
            }
        }
        C[i_3+1] = F[i_3+1] - C[i_3+3] + C[i_3] + 1;
    }
    for (int i_7 = 1; i_7 < PB_L; i_7++) {
        for (int i_8 = 0; i_8 < min(i_7, PB_M-1); i_8++) {
            C[i_8+1] = G[i_8][i_7-1] - G[i_8][i_7] + 4;
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4426311325_07(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
