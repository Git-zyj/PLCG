...
void kernel_3336352113_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0+1] = D[i_0] + E[i_0] + D[i_0+1] + D[i_0-1] * F[i_0] * E[i_0-1] * E[i_0] * 2;
    }
    for (int i_1 = 2; i_1 < PB_Q; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            for (int i_3 = i_1; i_3 < min(-i_2+PB_L, PB_M-1); i_3++) {
                B[i_3][i_2][i_1] = G[i_3+1][i_1+1][i_2+1] * H[i_1-2][i_2][i_3] + I[i_2+2][i_1+2][i_1] - J[i_3+i_2][0] - B[i_3][i_2-1][i_1] - 1;
            }
        }
        for (int i_4 = 0; i_4 < -i_1+PB_M; i_4++) {
            C[i_1-1][i_4+2] = K[i_1+2][i_4] + K[i_1][i_4] * L[i_4+i_1][2] + J[i_4][i_1] * 1;
        }
    }
#pragma endscop
