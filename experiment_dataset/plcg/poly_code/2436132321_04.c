...
void kernel_2436132321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0-1] = E[i_0+1][1] - F[i_0] - F[i_0] * A[i_0-2] * A[i_0] * C[i_0] + 3;
        B[i_0] = F[i_0-2] + F[i_0] + A[i_0-1] - C[i_0] + C[i_0+1] + 3;
        C[i_0] = F[i_0] * F[i_0] * F[i_0] - F[i_0+2] - F[i_0+1] - C[i_0-2] + 4;
    }
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        for (int i_2 = 1; i_2 < min(min(-i_1+PB_M, PB_N-2), i_1); i_2++) {
            D[i_1][i_2-1] = F[i_2+i_1] * E[i_2][i_1+1] * B[i_2+2] + D[i_1-2][i_2-1] * 1;
        }
    }
#pragma endscop
...
kernel_2436132321_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
