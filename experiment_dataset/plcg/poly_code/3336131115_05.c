...
void kernel_3336131115_05(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_Q, -i_0+PB_L); i_1++) {
            for (int i_2 = 1; i_2 < min(i_0, -i_0+PB_M+1); i_2++) {
                C[i_2][i_1] = D[i_2][i_1+i_0][1] - D[i_1+i_0][i_2][i_2] + E[i_1+i_0][i_2-1][i_0+1][i_1-1] * D[i_2+i_0-1][i_1][0] * 3;
            }
        }
        for (int i_3 = 1; i_3 < i_0; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                C[i_0-1][i_4] = F[i_3-1][i_4+1] * 2;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_L; i_5++) {
        for (int i_6 = 1; i_6 < PB_M; i_6++) {
            C[i_6][i_5] = F[i_5][i_6] + G[i_5] * C[i_6-1][i_5] - 1;
        }
    }
#pragma endscop
...
kernel_3336131115_05(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
