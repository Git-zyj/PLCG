...
void kernel_3312132225_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L+1; i_1++) {
            B[i_1+i_0-1][i_0] = D[i_0][i_1] * D[i_1-1][i_0] + E[i_0] + D[i_0-2][i_1-2] - 2;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_L-1; i_2++) {
            B[i_2+i_0-1][i_2] = D[i_2][i_0] * D[i_2][i_0-1] + D[i_0][i_2+2] + B[i_2+i_0+1][i_2+2] - 6;
        }
    }
    for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 3; i_5 < min(i_4, PB_L-1); i_5++) {
                C[i_3-2][i_4][i_5] = C[i_5-3][i_4-1][i_3-2] + C[i_3-2][i_4+1][i_5-1] - C[i_3-2][i_4][i_5+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3312132225_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
