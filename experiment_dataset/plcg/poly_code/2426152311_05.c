...
void kernel_2426152311_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            B[i_0] = E[i_1][i_0] + E[i_1][i_0+2] + E[i_0][i_1] + E[i_1+i_0][i_0] + B[i_0-1] + 4;
        }
        B[i_0] = F[i_0] * F[i_0] - F[i_0-1] + F[i_0+2] * F[i_0] * B[i_0-1] - B[i_0+1] + 4;
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            D[i_2-2] = E[i_3][i_2-1] * E[i_3][i_2-2] - F[i_2] + 1;
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        D[i_4-2] = F[i_4-2] * F[i_4] * F[i_4] * F[i_4] * F[i_4] * F[i_4] * B[i_4] - D[i_4-1] * 4;
    }
#pragma endscop
...
kernel_2426152311_05(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
