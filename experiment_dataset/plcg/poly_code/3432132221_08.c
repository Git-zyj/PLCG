...
void kernel_3432132221_08(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
...
#pragma scop
    ...
    for (int i_2 = 4; i_2 < PB_N; i_2++) {
        B[i_2-2] = E[i_2-1] - F[i_2][i_2-2] - E[0] * E[i_2] * B[i_2-3] - B[i_2-4] + 1;
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_L-3, i_3); i_4++) {
            C[i_3][i_4+1] = F[i_3-2][i_4] - G[i_3][i_4][i_3-2] * F[i_4-1][i_3] - F[i_4][i_3] + C[i_3][i_4+3] * C[i_3][i_4+2] - C[i_3-1][i_4+1] * 1;
        }
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                D[i_3][i_5][i_6][i_6] = G[i_5][i_3][0] + D[i_5][i_6][i_3-2][i_3-2] - 4;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3432132221_08(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
