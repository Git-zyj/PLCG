...
void kernel_3412112221_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_0+2][i_1] = E[0] * A[i_0+4][i_1] - A[i_0+2][i_1-2] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = max(3, i_2); i_3 < min(PB_L-1, -i_2+PB_M); i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_2-2][i_4] = B[i_3-3][i_4] * B[i_3][i_4] * 3;
            }
            C[i_3+i_2-2][i_3-1] = C[i_3+i_2-1][i_2] * C[i_3+i_2][i_3+1] + 4;
            for (int i_5 = 0; i_5 < -i_3+PB_M+2; i_5++) {
                D[i_3+i_2-2][i_5][i_2] = C[i_5+i_3-2][i_3-1] + D[i_3+i_2-1][i_5][i_2] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3412112221_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
