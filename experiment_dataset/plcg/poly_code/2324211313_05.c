...
void kernel_2324211313_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_M-1); i_1++) {
            A[i_1+1] = A[i_1] + B[i_1-1] - 1;
        }
    }
    ...
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_M+1; i_4++) {
            C[i_4-1][i_3] = C[i_4-2][i_3] + C[i_4-1][i_3-1] * C[i_4-1][i_3+1] + C[i_4-2][i_3-1] * 5;
        }
    }
#pragma endscop
...
kernel_2324211313_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
