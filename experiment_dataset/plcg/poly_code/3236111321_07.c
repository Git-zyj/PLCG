...
void kernel_3236111321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0][i_0+1] = C[i_0-1] - C[i_0-1] * A[i_0+1][i_0+2] + A[i_0-2][i_0-1] * A[i_0-1][i_0] - 6;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = 0; i_3 < min(i_1, -i_2+PB_Q-2); i_3++) {
                B[i_1][i_3+i_2][i_3+i_2] = B[i_1+2][i_3+i_2+2][i_3+i_2+2] + B[i_1+1][i_3+i_2][i_3+i_2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3236111321_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
