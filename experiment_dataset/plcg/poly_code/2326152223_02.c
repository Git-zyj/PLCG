...
void kernel_2326152223_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_L-1); i_1++) {
            A[i_0+2][i_1+1] = D[i_1+i_0-1][0] * E[i_1] - D[i_1+i_0+1][i_1] * F[i_0][i_1-2][i_0] + D[i_1][i_0] + E[i_0] + D[i_0][i_1] + D[i_1+1][i_0-2] - D[i_0+2][i_1] * A[i_0+1][i_1-1] - C[2][i_1] + C[2][i_0+1] * 5;
        }
        for (int i_2 = i_0; i_2 < PB_L; i_2++) {
            B[i_0][i_0] = E[i_2] - B[i_0+1][i_0+1] - 6;
        }
        for (int i_3 = 2; i_3 < min(PB_P, -i_0+PB_L); i_3++) {
            C[2][i_3] = D[i_3+i_0][i_0] + D[i_3][i_0] - D[i_3][i_0] + E[i_0-1] + D[i_3][0] + C[2][i_3-2] - 5;
        }
    }
#pragma endscop
...
kernel_2326152223_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
