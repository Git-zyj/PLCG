...
void kernel_4322211325_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_1][i_0][i_2] = A[i_1][i_0-2][i_2+2] * A[i_1+1][i_0+2][i_2] - B[i_2+1][i_1] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
            B[i_4][i_3] = B[i_4+2][i_3] + B[i_4-2][i_3] + 5;
        }
        for (int i_5 = 1; i_5 < i_3; i_5++) {
            for (int i_6 = 0; i_6 < PB_Q-2; i_6++) {
                C[i_3-1][i_5-1][i_6] = D[i_5-1][i_3][0] + C[i_3][i_5-1][i_6] - C[i_3+1][i_5][i_6] + C[i_3-1][i_5-1][i_6+2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4322211325_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
