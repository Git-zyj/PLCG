...
void kernel_4122151323_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_Q-1); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_N, -i_0+PB_L); i_2++) {
                A[i_0-1][i_1][i_2+1] = B[i_1][i_2][i_2+i_0][i_1+1] - C[i_1+i_0][i_2-1][0] - D[i_2][1] + C[i_1-1][i_0][0] - C[i_1][i_0][i_0] - A[i_0-1][i_1+1][i_2+1] * A[i_0-2][i_1][i_2+1] + A[i_0-1][i_1-2][i_2+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4122151323_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
