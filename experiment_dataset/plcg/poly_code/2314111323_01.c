...
void kernel_2314111323_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            B[i_1-2] = B[i_1+1] - B[i_1] - B[i_1+2] - 6;
        }
        B[i_0] = B[i_0+2] * 4;
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            C[i_3-1][i_2][i_3-1] = D[i_2][i_3+1] * D[i_3+1][i_2] * C[i_3-1][i_2+2][i_3-1] - C[i_3][i_2][i_3] * 5;
        }
    }
#pragma endscop
...
kernel_2314111323_01(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
