...
void kernel_4312112215_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = 1;
        A[i_0-1] = D[i_0] - D[i_0] * A[i_0] + A[i_0-1] + 4;
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
                    C[i_3+2][i_2][i_0][i_3] = C[i_3+1][i_2-1][i_0][i_3-1] * 6;
                }
            }
        }
    }
...
kernel_4312112215_02(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
