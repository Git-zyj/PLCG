...
void kernel_2334112123_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_1][i_0-2] = C[i_1][i_0-2] * 2;
        }
    }
    ...
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
            C[i_4][i_5-1] = D[0][i_4-1] - D[i_4][i_5] - A[i_5+2][i_4-2] * 1;
        }
    }
#pragma endscop
...
kernel_2334112123_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
