...
void kernel_3222152323_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_0][0] = C[i_1][i_0] - A[i_0+1][0] * A[i_0+2][0] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        A[i_2][0] = D[i_2] + D[i_2] + D[i_2] - D[i_2-1] + D[i_2] * D[i_2] - D[i_2] + D[i_2-1] + D[i_2] - A[i_2][0] * 6;
    }
#pragma endscop
...
kernel_3222152323_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
