...
void kernel_2332312313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            C[i_2][i_2] = D[i_2] * A[i_3] - 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        C[i_4][i_4] = D[i_4] * C[i_4-1][i_4-1] * 5;
    }
#pragma endscop
...
kernel_2332312313_00(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
