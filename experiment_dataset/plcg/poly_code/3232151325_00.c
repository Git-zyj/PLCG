...
void kernel_3232151325_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_1] = C[i_1][i_0] - A[i_1+2] + A[i_1-1] * A[i_1-2] * 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        A[i_2-2] = D[1] * D[i_2-1] + D[i_2-1] - D[i_2] * C[i_2][1] * D[i_2] - D[i_2] + D[i_2] * 4;
    }
#pragma endscop
...
kernel_3232151325_00(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
