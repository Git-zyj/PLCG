...
void kernel_4234212211_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_L-1); i_1++) {
            A[i_1+i_0-2][i_1][i_0] = C[i_1][i_0] - A[i_1+i_0][i_1+1][i_0+1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
            for (int i_4 = 1; i_4 < PB_P; i_4++) {
                B[i_2][i_3-1][i_4] = D[i_2][i_4][i_3] + B[i_4-1][i_3-1][i_2] - B[i_2+1][i_4-1][i_3] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4234212211_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
