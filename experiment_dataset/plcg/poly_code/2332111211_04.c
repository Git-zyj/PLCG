...
void kernel_2332111211_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1+1] = D[i_1] - A[i_0+1][i_1+1] + C[i_1][i_1+i_0] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            C[i_2][i_3+i_2] = E[i_3+i_2][i_3] - 2;
        }
        for (int i_4 = 0; i_4 < min(i_2, -i_2+PB_M); i_4++) {
            C[i_2][i_4+i_2] = A[i_4+1][i_2+1] - C[i_2-1][i_4+i_2-2] + 1;
        }
    }
#pragma endscop
...
kernel_2332111211_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
