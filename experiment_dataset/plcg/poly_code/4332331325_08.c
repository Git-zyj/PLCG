...
void kernel_4332331325_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = D[i_0+1] * E[i_0] - A[i_0-2] + A[i_0-3] - A[i_0] + 5;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = i_1; i_2 < -i_0+PB_P-1; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_L; i_3++) {
                    C[i_0+2][i_2][i_3+1] = F[i_1-1][i_2+i_0+1][i_3][i_2] - G[i_2][i_3+i_1][i_3-1][i_0+1][i_1-1] + H[i_1][i_2+1][i_0][i_2] * H[i_0][i_3-1][i_1][i_0] + 5;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 2; i_5 < -i_0+PB_M-1; i_5++) {
                C[i_5+1][i_0][i_4] = I[i_4][i_5+i_0+1] - C[i_5+2][i_0-1][i_4] * C[i_5+2][i_0][i_4] * C[i_0+3][i_5-2][i_4+1] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4332331325_08(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
