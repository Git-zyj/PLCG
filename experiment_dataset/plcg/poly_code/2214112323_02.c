...
void kernel_2214112323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-4; i_1++) {
            A[i_0][i_1+2] = A[i_0+1][i_1] - A[i_0-1][i_1+2] * A[i_0-2][i_1] + A[i_0][i_1+1] * A[i_0][i_1+4] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 3; i_3 < PB_M-2; i_3++) {
            B[i_3-2][i_2-2] = C[i_3-2][i_2] - C[i_3+2][i_2] - B[i_3-3][i_2-2] * 1;
        }
    }
#pragma endscop
...
kernel_2214112323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
