...
void kernel_4234212325_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_L+2; i_1++) {
            B[i_0][i_1-2] = C[i_1+2] - 1;
        }
        for (int i_2 = 2; i_2 < -i_0+PB_N; i_2++) {
            B[i_0-2][i_2] = D[i_2+i_0][i_2+i_0] * B[i_0-3][i_2] - B[i_0-2][i_2-2] - B[i_0-2][i_2+1] * 4;
        }
    }
#pragma endscop
...
kernel_4234212325_09(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
