...
void kernel_2416351321_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = E[i_0] - F[i_0] - E[i_0] + F[i_0] - A[i_0+1] * B[i_0] + 4;
        for (int i_1 = 1; i_1 < PB_N-2; i_1++) {
            B[i_1] = G[i_0-1][i_1] + G[i_0][i_1-1] * H[i_1][i_0-1] * G[i_1][i_0] + G[i_1][i_0] - G[i_1][i_0] * I[i_1+1][i_0-1] - A[i_1] + B[i_1+2] - B[i_1+1] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < min(PB_M-2, -i_2+PB_M); i_3++) {
            C[i_2][i_3] = G[i_3+i_2][0] + J[i_3-1] + C[i_2][i_3+2] + C[i_2+1][i_3] + C[i_2][i_3+1] * C[i_2][i_3-2] + 4;
        }
        for (int i_4 = max(2, i_2); i_4 < -i_2+PB_M-1; i_4++) {
            D[i_4+i_2][i_4-1] = H[i_4][i_2] + H[i_2+1][i_4] * G[i_4-1][0] * I[i_4][i_2] - H[i_4][i_4] * D[i_4+i_2-1][i_4-2] - D[i_4+i_2+1][i_4] + 2;
        }
    }
#pragma endscop
...
kernel_2416351321_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
