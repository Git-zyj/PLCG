...
void kernel_2334212323_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0] - 6;
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            B[i_0][i_1-1] = E[i_1][0] + D[i_0] + A[i_1+2] * A[i_1] * A[i_1-2] - A[i_0] - B[i_0][i_1] * B[i_0][i_1+1] - 2;
        }
    }
...
kernel_2334212323_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
