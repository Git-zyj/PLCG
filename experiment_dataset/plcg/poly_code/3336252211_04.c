...
void kernel_3336252211_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0+1] * D[i_0] * E[0] * E[i_0] - A[i_0+1] - 4;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_M, PB_P); i_2++) {
            A[i_2-1] = F[i_2+2][i_1][i_1] + G[i_2+1][1] * F[i_2][i_1+1][i_2] - H[i_2-2][i_1] * H[i_2+i_1][i_2] * 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < min(-i_3+PB_M, PB_M-2); i_4++) {
            A[i_4+1] = G[i_4][i_3-1] + H[i_4][i_3] + H[i_4-1][i_3-1] * H[i_4+2][i_3] - H[i_4+1][0] + G[i_4+i_3][i_4] + A[i_4] + 3;
        }
    }
#pragma endscop
...
kernel_3336252211_04(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
