...
void kernel_2332112223_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            A[i_0+2][i_1+2] = D[i_0][i_1] * 2;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        B[i_2-2] = B[i_2-3] + B[i_2] * 1;
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            C[i_4] = A[i_3+2][i_4+3] + C[i_4-2] + 1;
        }
    }
#pragma endscop
...
kernel_2332112223_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
