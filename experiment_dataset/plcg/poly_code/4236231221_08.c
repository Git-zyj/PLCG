...
void kernel_4236231221_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 2; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = i_0; i_2 < PB_L; i_2++) {
                for (int i_3 = 2; i_3 < PB_L; i_3++) {
                    A[i_1][i_0][i_2][i_3] = A[i_1-2][i_0-1][i_3][i_2] + A[i_1+1][i_0][i_2][i_3-2] + 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        B[i_4] = C[i_4+1] - C[i_4+1] + C[i_4] - C[i_4] * C[i_4] - B[i_4+2] * B[i_4-2] + 1;
    }
#pragma endscop
...
kernel_4236231221_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
