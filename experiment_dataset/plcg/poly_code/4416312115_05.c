...
void kernel_4416312115_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 3; i_2 < i_0; i_2++) {
                for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                    A[i_1-2][i_0][i_1-1] = E[i_3+1][i_1][i_3][i_0] + F[i_0][i_1][i_3][i_2] + A[i_2-3][i_1][i_2-2] - 1;
                }
            }
            for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
                B[i_1][i_4][i_0] = B[i_1][i_4+1][i_0-1] * 3;
            }
        }
        for (int i_5 = 0; i_5 < min(i_0, PB_M-1); i_5++) {
            for (int i_6 = 0; i_6 < -i_0+PB_M+2; i_6++) {
                for (int i_7 = 1; i_7 < PB_L; i_7++) {
                    B[i_6][i_0][i_7-1] = E[i_5][i_6+i_0-2][i_7+2][i_7] * G[i_5+1][i_7][i_6] - 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4416312115_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
