...
void kernel_4234111321_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
            for (int i_2 = i_0; i_2 < PB_Q; i_2++) {
                A[i_2-1][i_0+1][i_1-1] = B[i_0][i_0-1][i_2][i_1] + 4;
            }
        }
    }
        ...
        for (int i_4 = max(3, i_3); i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_Q-2; i_5++) {
                for (int i_6 = 2; i_6 < min(-i_3+PB_M, PB_L); i_6++) {
                    B[i_4][i_4-1][i_5][i_6] = C[i_4+1][i_6+i_3][i_6][i_5] * A[i_5-1][i_4+1][i_6-2] - B[i_4][i_4-1][i_5-1][i_6] + B[i_4+1][i_4][i_5+2][i_6] * B[i_3][i_3-1][i_5+2][i_4-2] - B[i_4-2][i_4-3][i_5][i_6] + 3;
                }
            }
        }
    }
...
kernel_4234111321_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
