...
void kernel_3222232323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = C[i_0+1] * D[i_0+1] - A[i_0-1] + A[i_0-2] * A[i_0+2] * A[i_0+1] - 1;
    }
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_L-2; i_2++) {
            B[i_2][i_1] = E[i_2][i_1] + E[i_1-2][0] - E[i_2+i_1+2][0] * E[i_2][i_1] - A[i_2] + 3;
        }
    }
#pragma endscop
...
kernel_3222232323_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
