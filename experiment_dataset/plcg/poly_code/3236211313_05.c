...
void kernel_3236211313_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L+1; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
                A[i_1-1][i_0][i_0][i_2+1] = A[i_2-1][i_1-1][i_1-1][i_0+2] * A[i_1][i_0][i_0][i_2+1] + 5;
            }
        }
    }
...
kernel_3236211313_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
