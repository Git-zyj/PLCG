...
void kernel_4212211125_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        ...
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, -i_0+PB_M+1); i_4++) {
                B[i_4+i_0-1][i_0+1] = C[i_3][i_4][i_0+1] + B[i_4+i_3][i_3+1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_4212211125_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
