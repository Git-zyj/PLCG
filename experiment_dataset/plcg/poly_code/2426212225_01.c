...
void kernel_2426212225_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L-3; i_3++) {
            D[i_2][i_3+3] = E[i_3][i_2+1] + 6;
        }
        for (int i_4 = 1; i_4 < min(min(i_2, PB_L-2), -i_2+PB_M); i_4++) {
            D[i_2][i_4+1] = E[i_4][i_2] - F[i_2+1][i_4+i_2][i_4] * D[i_2][i_4-1] + D[i_2][i_4+2] * 5;
        }
    }
#pragma endscop
...
kernel_2426212225_01(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
