...
void kernel_4224212125_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 2; i_1 < PB_L-3; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_L+2, -i_2+PB_L); i_3++) {
                    A[i_3+i_2][i_1-2][i_0][i_1] = A[i_3+i_0-2][i_1][i_2+2][i_1+2] - 3;
                }
            }
            B[i_0][i_1+1] = C[i_1] * B[i_0][i_1+3] * 4;
        }
    }
#pragma endscop
...
kernel_4224212125_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
