...
void kernel_3432251115_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0-1] = E[i_0] + F[i_0] + F[i_0] * F[0] + F[i_0-1] - E[i_0] * E[i_0-1] * E[i_0] * F[i_0] * A[i_0-2] - 4;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            A[i_1-1] = G[i_2+1][i_1] + G[i_2+i_1][0] * 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        A[i_3-2] = F[i_3] * E[i_3] * E[i_3] + E[i_3] + F[i_3] - 6;
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                D[i_3][i_5][i_4] = H[i_4][i_3][i_5] * I[i_4+1][i_3][i_4-1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3432251115_06(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
