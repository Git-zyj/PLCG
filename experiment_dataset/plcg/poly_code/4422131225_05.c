...
void kernel_4422131225_05(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                D[i_1-3][i_0][i_2][i_2] = E[i_0][i_2+1][i_1] - F[i_2][i_1-1] - E[i_1+1][i_2][i_0] + 3;
            }
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < -i_0+PB_L; i_4++) {
                for (int i_5 = i_0; i_5 < PB_M; i_5++) {
                    D[i_0-1][i_3][i_4][i_4] = G[i_5][i_4+i_0][1][i_3] * 6;
                }
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
        for (int i_7 = max(i_6, 1); i_7 < min(-i_6+PB_P, PB_M); i_7++) {
            for (int i_8 = 1; i_8 < PB_L; i_8++) {
                D[i_8-1][i_7][i_6+1][i_6+1] = E[i_7-1][i_8][i_6+1] * E[i_8][i_8-1][i_7+i_6] - 1;
            }
        }
    }
    for (int i_9 = 2; i_9 < PB_Q-1; i_9++) {
        for (int j_0 = 1; j_0 < PB_L; j_0++) {
            for (int j_1 = 1; j_1 < min(i_9, -j_0+PB_Q); j_1++) {
                for (int j_2 = 2; j_2 < min(-j_0+PB_L, j_1); j_2++) {
                    D[j_2-1][i_9][j_0][j_0] = G[i_9][j_1][j_0][j_2] + E[i_9][j_1][j_0-1] - H[j_2+j_0][i_9][j_2][j_1+j_0][i_9-1] - G[j_0-1][j_1-1][j_2-1][i_9] + G[j_0][j_2+1][0][i_9+1] + D[j_2-1][i_9-2][j_0][j_0] * D[j_2-2][j_1][j_0][j_0] - 1;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422131225_05(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H)));
