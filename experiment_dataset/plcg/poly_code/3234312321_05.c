...
void kernel_3234312321_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            for (int i_2 = 3; i_2 < min(PB_Q-1, -i_0+PB_M-3); i_2++) {
                A[i_2+i_0+1][i_1-1][i_0][i_2-1] = A[i_2+i_0+3][i_1-1][i_0][i_2+1] + A[i_2+i_0-3][i_1-1][i_0-2][i_2-3] - B[i_2+2][i_0-4] * 2;
            }
        }
    }
    for (int i_3 = 4; i_3 < PB_P+2; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            B[i_4][i_3-2] = C[i_4] - B[i_4-1][i_3-2] + B[i_4][i_3-4] + 3;
        }
    }
#pragma endscop
...
kernel_3234312321_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
