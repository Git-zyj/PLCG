...
void kernel_2336232313_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_N; i_2++) {
            C[0] = E[i_2-2][i_1] + D[2] - C[0] - C[0] + 6;
        }
        C[0] = D[i_1] + D[0] + D[i_1+2] + C[0] * C[0] - 2;
    }
#pragma endscop
...
kernel_2336232313_03(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
