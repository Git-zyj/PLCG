...
void kernel_4234132123_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        B[i_0-1] = C[i_0-1] * C[i_0] + C[i_0] * 4;
    }
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 3; i_2 < min(-i_1+PB_L-2, PB_M+1); i_2++) {
            B[i_2-1] = D[i_2+i_1][0] - D[i_2+2][i_1-1] * D[i_2+i_1+2][i_1-2] + B[i_2-3] - 5;
        }
    }
#pragma endscop
...
kernel_4234132123_01(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
