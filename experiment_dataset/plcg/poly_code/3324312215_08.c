...
void kernel_3324312215_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 3; i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < i_2; i_4++) {
                B[i_3-2][i_4] = B[i_3-3][i_4] * B[i_3-2][i_4-1] * B[i_4-1][i_3] - 5;
            }
        }
        for (int i_5 = 1; i_5 < PB_M+1; i_5++) {
            C[i_2][i_5] = C[i_5-1][i_2] - 1;
        }
    }
#pragma endscop
...
kernel_3324312215_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
