...
void kernel_2422351321_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, -i_0+PB_N-1); i_1++) {
            B[i_1+1] = E[i_0] * F[i_1][i_0+1] * G[i_0][i_1] - E[i_1-1] * H[i_1+i_0+1][0] - B[i_1] + 6;
        }
        B[i_0] = I[i_0] - J[i_0+1] + I[i_0] + J[i_0] + E[i_0] + B[i_0+1] + 4;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        B[i_2-2] = J[i_2+1] + J[i_2] + E[i_2] * I[i_2] - J[i_2] - I[i_2-1] * 6;
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M-1, i_3); i_4++) {
            D[i_4+i_3][i_3+1][i_4] = G[i_4][i_3] - F[i_4+i_3][i_3+1] - G[i_4][i_4+i_3+1] * D[i_4+i_3-2][i_3-1][i_4] - D[i_4+i_3+1][i_3+2][i_4] * D[i_4+i_3-2][i_3+1][i_4-2] - D[i_4+i_3+1][i_4+2][i_3] + 2;
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2422351321_08(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
