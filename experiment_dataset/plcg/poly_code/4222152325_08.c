...
void kernel_4222152325_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M+3; i_2++) {
                B[i_2+i_0-3][i_1+2][i_0-2] = C[i_2-1][i_1][i_0-1] - C[i_1][i_0][i_1] - D[i_2][i_1] * C[i_1-2][i_0][i_0] + C[i_1][i_0][i_2] * D[i_1][i_2] * 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 2; i_5 < min(PB_M-4, -i_3+PB_M+2); i_5++) {
                for (int i_6 = i_5; i_6 < min(-i_4+PB_M-2, -i_5+PB_M); i_6++) {
                    B[i_6+i_5][i_4+2][i_6] = E[i_5-2][i_3+1][i_6+i_4][0] - C[i_6][i_5+1][1] - E[i_3-1][i_5][i_6-2][0] + B[i_5+i_3-2][i_6+2][i_3] * B[i_6+i_4+2][i_5+4][i_6+2] + B[i_6+i_5-1][i_4+2][i_5] * 6;
                }
            }
        }
    }
...
kernel_4222152325_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
