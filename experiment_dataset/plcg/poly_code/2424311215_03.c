...
void kernel_2424311215_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        ...
        B[i_0+1] = A[i_0-1][0] + C[i_0] * 3;
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        C[i_2] = E[i_2] + F[i_2] * C[i_2+1] * C[i_2-1] - D[i_2+1][i_2] * 3;
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            D[i_2+1][i_2] = G[i_2][i_3] + H[i_3][i_2] + C[i_2] * 2;
        }
    }
#pragma endscop
...
kernel_2424311215_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
