...
void kernel_4336132323_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < min(PB_P-2, -i_1+PB_Q); i_2++) {
                for (int i_3 = max(2, i_1); i_3 < -i_2+PB_L; i_3++) {
                    A[i_0+1][i_1][i_0+2][i_3] = D[i_1][i_3+i_2][i_0][i_2+i_1] * D[i_1][i_0][i_3+i_2-2][0] - A[i_0-1][i_1][i_0][i_3-2] - A[i_0+1][i_1+2][i_0+2][i_2] - A[i_0+2][i_1][i_0+3][i_3+1] + A[i_0][i_1+1][i_0+1][i_2+2] * 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-4; i_4++) {
        for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
            B[i_5] = E[i_5][i_4] * E[i_5+2][i_4] - F[i_5-2][i_4][i_4] - B[i_5-1] + B[i_4-2] + 3;
        }
        for (int i_6 = 2; i_6 < i_4; i_6++) {
            for (int i_7 = 2; i_7 < PB_P; i_7++) {
                for (int i_8 = 2; i_8 < -i_4+PB_Q; i_8++) {
                    C[i_4+2][i_8][i_7-2][i_6-2] = D[i_8][i_8+1][i_4][i_7] - D[2][i_8-1][i_6][i_8+i_4] * C[i_4+4][i_6][i_7][i_8-2] + C[i_4+2][i_8][i_7-2][i_6] + C[i_4+3][i_8][i_7-1][i_6-2] - 4;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4336132323_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
