...
void kernel_3222111113_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M-1, -i_0+PB_L-1); i_1++) {
            A[i_0] = B[i_1+1][i_1+i_0+1][i_0] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_L, PB_P); i_3++) {
            B[i_3][i_3+i_2][i_2] = C[i_2][i_3] - C[i_2][i_2] - B[i_3-1][i_3+i_2][i_2+1] + 1;
        }
    }
#pragma endscop
...
kernel_3222111113_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
