...
void kernel_2414131223_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            A[i_0][i_1] = E[i_0] + A[i_0][i_1+2] - A[i_0][i_1+1] - A[i_0+1][i_1] * 6;
            B[i_0][i_1+i_0] = F[i_1][i_0+1] + F[i_1-1][i_0] + F[i_1+1][i_0+1] * F[i_1][i_0] + F[i_1][i_0] + F[i_1][i_0] - F[i_1][i_0] + B[i_0-1][i_1+i_0+1] * 6;
        }
        for (int i_2 = i_0; i_2 < PB_P+1; i_2++) {
            C[i_0-1][i_2-1] = C[i_0-1][i_2-2] + C[i_0-3][i_2-1] * 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            D[i_3][i_4] = F[i_3][i_4+1] * F[i_4][i_3-1] + 6;
        }
    }
#pragma endscop
...
kernel_2414131223_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
