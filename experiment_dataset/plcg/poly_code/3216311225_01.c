...
void kernel_3216311225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            A[i_1][i_1+i_0] = C[i_1+i_0][0] - C[i_1][i_0] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_L; i_3++) {
            for (int i_4 = 2; i_4 < -i_3+PB_L; i_4++) {
                B[i_4][i_3] = A[i_3][i_4+i_3] * B[i_3][i_4-2] * B[i_4][i_3-1] + B[i_4+2][i_3-2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3216311225_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
