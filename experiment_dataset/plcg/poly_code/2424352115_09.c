...
void kernel_2424352115_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0-1] - F[i_0] + G[i_0][1] + E[i_0-1] - 2;
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-2, i_0); i_1++) {
            A[i_1+1] = E[i_1+i_0+2] - H[i_1][0] + G[i_0-2][i_0] * I[i_1][i_0] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        C[i_2] = J[i_2+2] * F[i_2] + J[i_2] * F[0] * J[i_2-1] + C[i_2-1] + C[i_2+1] * 5;
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        A[i_3] = E[i_3] + F[i_3] * F[i_3] - F[i_3] - E[i_3+2] * 5;
    }
#pragma endscop
...
kernel_2424352115_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
