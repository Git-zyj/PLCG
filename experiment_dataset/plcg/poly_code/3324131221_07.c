...
void kernel_3324131221_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1][i_0+1] = D[i_1][i_0+1] * D[i_1][i_0+1] + E[i_0][i_1][i_1] * D[i_0-1][i_1] - D[i_1-1][i_0-1] + A[i_0+1][i_1+1] * A[i_1+1][i_0+1] - A[i_1-1][i_0+1] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            A[i_2+1][i_3+1] = D[i_2+1][i_3] + 4;
        }
        for (int i_4 = max(1, i_2); i_4 < PB_P-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_P+1; i_5++) {
                C[i_2][i_4+1][i_5-1] = D[i_4+1][i_4-1] + C[i_4][i_2][i_5-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3324131221_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
