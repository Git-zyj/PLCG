...
void kernel_3226312123_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
                B[i_4][i_3] = C[i_2][i_3] * B[i_4+1][i_3+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3226312123_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
