...
void kernel_3226112121_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        for (int i_1 = max(i_0, 4); i_1 < PB_L+2; i_1++) {
            A[i_0+2][i_1-2] = A[i_0+4][i_1-4] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < min(-i_2+PB_L, PB_P); i_3++) {
            for (int i_4 = 2; i_4 < min(PB_P, -i_2+PB_M); i_4++) {
                B[i_2][i_4][i_3] = C[i_4+i_2][i_3+i_2][1] - C[i_4-2][i_3-1][i_2] - B[i_3][i_2+2][i_4-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3226112121_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
