...
void kernel_2432311221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        B[i_1-1] = F[i_1] - B[i_1-2] + B[i_1] * 4;
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        C[i_2] = D[i_2-1] * 5;
        for (int i_3 = 1; i_3 < -i_2+PB_M-1; i_3++) {
            D[i_3-1] = G[i_3+i_2+1][i_3] + H[i_3-1][i_2-1] * C[i_3] - C[i_3+1] + 5;
        }
    }
#pragma endscop
...
kernel_2432311221_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
