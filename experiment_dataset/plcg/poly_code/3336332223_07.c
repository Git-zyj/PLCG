...
void kernel_3336332223_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        A[i_0] = D[i_0+2] + D[i_0] + 6;
        A[i_0] = E[i_0] + D[i_0] * E[i_0+2] * F[i_0+1][i_0] * 1;
    }
    for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
        for (int i_2 = max(4, i_1); i_2 < PB_P; i_2++) {
            for (int i_3 = 3; i_3 < -i_2+PB_M; i_3++) {
                C[i_3-2][i_1][i_2-2] = G[i_3][i_2] + H[i_3+i_2][i_1][i_2] + I[i_1][i_2][i_2] - C[i_3-3][i_1+2][i_2] + C[i_3-2][i_1+1][i_2-4] * C[i_3-2][i_1+1][i_2-3] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3336332223_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
