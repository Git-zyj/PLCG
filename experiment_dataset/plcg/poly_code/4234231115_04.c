...
void kernel_4234231115_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < PB_P; i_1++) {
            for (int i_2 = i_0; i_2 < -i_1+PB_P+1; i_2++) {
                A[i_1-1][i_0-1][i_2+i_1-1] = C[i_0][i_2][i_1] + D[i_2-1][i_0][i_1-1] * A[i_1-2][i_0-1][i_2+i_1-3] - 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            for (int i_5 = max(i_4, 1); i_5 < PB_L; i_5++) {
                B[i_4+1][i_4][i_5] = D[i_4][i_3][i_5] - E[i_3][i_5][i_5-1][i_4+1] + E[i_3][i_4+1][i_5][i_3] + B[i_4+2][i_4+1][i_5] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4234231115_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
