...
void kernel_4322332221_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] * E[i_0-2] + F[i_0] - A[i_0-2] + A[i_0+1] - A[i_0-1] - 2;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        B[i_1] = E[i_1] + E[i_1+2] * A[i_1] - 5;
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_Q; i_3++) {
            for (int i_4 = 1; i_4 < min(min(PB_P+1, -i_3+PB_Q), -i_2+PB_M-1); i_4++) {
                C[i_3][i_2][i_4+i_3] = G[i_3][i_2][i_4] - H[i_4+i_2+1][i_3+1] - C[i_3][i_4-1][i_3+i_2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4322332221_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
