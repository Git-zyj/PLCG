...
void kernel_4226151115_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M-1; i_2++) {
            B[i_2] = D[i_2][i_1] * D[i_2][i_1] - D[i_1][i_2] * C[i_2] * D[i_2+i_1+1][i_2] + D[i_2][i_1-1] + C[i_1] - D[i_2-1][i_1] - B[i_2+1] + 5;
        }
    }
#pragma endscop
...
kernel_4226151115_04(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
