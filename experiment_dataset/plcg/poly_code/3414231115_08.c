...
void kernel_3414231115_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M-1, i_1); i_2++) {
            for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
                C[i_2+1][i_3+1] = F[i_2+1][i_1][1] - F[i_2][i_1][i_3] * G[i_2+i_1+1][0] + 1;
                C[i_2+1][i_3+1] = H[i_2][i_3] + F[i_3][i_2][i_1-1] + F[i_2][i_3][i_1-1] * F[i_1][i_3+1][0] * C[i_2][i_3+1] - 6;
            }
        }
        A[i_1+1] = E[i_1-1] * E[i_1] - I[0] - 2;
    }
#pragma endscop
...
kernel_3414231115_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
