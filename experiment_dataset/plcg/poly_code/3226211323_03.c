...
void kernel_3226211323_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_0][i_1+2][i_2+1] = C[i_2][i_2] - 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_P+2; i_4++) {
            for (int i_5 = 2; i_5 < PB_M+1; i_5++) {
                B[i_3][i_4][i_5] = C[i_5-1][i_4-1] - B[i_3-1][i_4-2][i_5] - B[i_3-1][i_5][i_4-2] * B[i_3][i_4+1][i_5-2] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3226211323_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
