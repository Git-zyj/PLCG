...
void kernel_4322252213_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        B[0] = D[i_0] * E[i_0] + E[i_0+2] * D[i_0] * E[i_0] * 1;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
            B[0] = F[i_2][0] * G[i_1][i_2+1][0] - F[i_1][i_2+1] - H[i_1][i_2] * B[0] * B[0] * 4;
        }
        for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
            for (int i_4 = 2; i_4 < min(-i_3+PB_N, i_3); i_4++) {
                B[0] = G[i_4-2][i_1][0] + I[i_1][i_4-2][i_3] + H[i_4-1][i_3+2] + H[i_4][i_3] * I[i_4-1][i_1][i_3+2] * G[i_4+i_3][i_1-2][i_3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4322252213_08(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
