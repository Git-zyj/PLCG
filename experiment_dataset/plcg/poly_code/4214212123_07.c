...
void kernel_4214212123_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_1][i_0] = C[i_0-2][i_1] * A[i_1][i_0-1] + 3;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            A[i_2][i_0] = D[i_2-1][i_0-2] * 4;
        }
    }
#pragma endscop
...
kernel_4214212123_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
