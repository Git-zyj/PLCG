...
void kernel_3232312321_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_P+1, PB_P-4); i_2++) {
                A[i_1][i_2+2][i_0] = C[i_0-2][i_2][i_2+i_1-1] * A[i_1-2][i_2][i_0] * A[i_2-1][i_1+2][i_0+1] * A[i_1][i_2+2][i_0+2] + A[i_1][i_2+4][i_0] - A[i_1+2][i_2+1][i_0] - 1;
            }
        }
    }
...
kernel_3232312321_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
