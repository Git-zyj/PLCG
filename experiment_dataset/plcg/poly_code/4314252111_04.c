...
void kernel_4314252111_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = D[i_0+1] - D[i_0] * D[i_0-1] + E[i_0+2] * E[i_0] * E[i_0-2] - D[i_0] * D[i_0-2] + D[i_0-1] - A[i_0+1] + 3;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
            for (int i_3 = 2; i_3 < i_1; i_3++) {
                for (int i_4 = 0; i_4 < min(PB_P, -i_2+PB_M); i_4++) {
                    C[i_1][i_3-2][i_4] = F[i_4+1][i_3][i_1][i_2-1] * G[i_4+i_2][i_3][i_1][i_1] * G[i_2][i_4][i_1][i_2+1] - F[i_3][i_1][i_2][0] * 3;
                }
            }
            for (int i_5 = 2; i_5 < PB_L; i_5++) {
                C[i_1][i_5-2][i_2] = H[i_5][i_1][1] + H[i_5-2][i_2][i_1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4314252111_04(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
