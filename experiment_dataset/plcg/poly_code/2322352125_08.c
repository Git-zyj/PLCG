...
void kernel_2322352125_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        B[i_0] = D[i_0] + D[i_0] * E[i_0] - E[i_0] + D[i_0] + D[i_0+2] + 4;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        B[i_1] = F[i_1] + D[i_1] * D[i_1] - E[i_1] * F[i_1] + B[i_1-1] + 2;
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            C[i_2+2][i_3] = G[i_3][i_2] * H[i_3+i_2][i_2] + I[i_3][i_2-2] - C[i_3+2][i_2+1] - 6;
        }
    }
#pragma endscop
...
kernel_2322352125_08(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
