...
void kernel_3312251215_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        A[i_0][i_0] = D[i_0] - E[i_0][i_0] - F[i_0] + A[i_0+1][i_0+1] + 6;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_L, -i_1+PB_Q); i_2++) {
            B[i_1+1][i_2-1] = F[i_2] + G[i_1][i_2+i_1] * E[i_2][i_1+1] + G[i_2][i_1] * B[i_1][i_2] - 6;
        }
        A[i_1][i_1] = D[i_1+1] * F[i_1] + E[i_1][i_1+1] * D[i_1] + F[i_1] - F[i_1-1] * F[i_1-1] + A[i_1+1][i_1+1] + 5;
    }
#pragma endscop
...
kernel_3312251215_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
