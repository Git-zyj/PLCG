...
void kernel_2426232323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_M-2); i_1++) {
            A[i_1+1] = E[i_0] * F[i_0-1] + A[i_1] * A[i_1-1] + A[i_1+2] - 3;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M; i_2++) {
            D[i_2] = G[i_2+i_0][0] + G[i_0][i_2-1] - G[i_2][i_2+i_0-1] + 1;
        }
        for (int i_3 = 0; i_3 < min(-i_0+PB_P, PB_M-2); i_3++) {
            C[i_0][i_0] = F[1] + H[i_3+2][i_3+i_0] + H[i_3][i_0] + I[i_0][i_3][i_0] * D[i_3] + D[i_3+2] * 5;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        D[i_4] = F[i_4] * E[i_4] - E[i_4] + A[i_4+1] - D[i_4+2] - D[i_4-1] + 6;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2426232323_04(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
