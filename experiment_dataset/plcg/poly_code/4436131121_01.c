...
void kernel_4436131121_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
                    A[i_1][i_1][i_2] = E[i_1][i_0][i_3+1][i_2-1] * E[i_3][i_0][i_2][i_1] - E[i_3-1][i_0][i_1][i_2] + 3;
                }
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                for (int i_7 = 0; i_7 < -i_4+PB_L; i_7++) {
                    B[i_5][i_6][i_5+1][i_7+1] = E[i_5+1][i_7+i_4][i_7][i_6] * 4;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_L; i_8++) {
        for (int i_9 = 1; i_9 < i_8; i_9++) {
            C[i_8][i_9] = F[i_9][i_8] * F[i_9-1][i_8-1] - G[i_9] + F[i_9][i_8] - G[i_9] + C[i_9-1][i_8+1] * 5;
        }
    }
    for (int j_0 = 1; j_0 < PB_L; j_0++) {
        for (int j_1 = 1; j_1 < j_0; j_1++) {
            D[j_1+1][j_0-1][j_0] = F[j_1+1][j_0] * F[j_0][j_1-1] * 5;
        }
    }
#pragma endscop
...
kernel_4436131121_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
