...
void kernel_4222152123_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_M, i_0), PB_M-2); i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_M, PB_M-4); i_2++) {
                for (int i_3 = 2; i_3 < PB_L; i_3++) {
                    A[i_1+2][i_3][i_3] = C[i_1+2][i_2+i_0][i_3][0] * C[i_1][i_0][i_2][i_3-2] * C[i_2-2][i_1+i_0][i_3][i_1] * A[i_2+4][i_3+1][i_3+1] * 3;
                }
            }
        }
    }
    for (int i_4 = 4; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 2; i_5 < min(-i_4+PB_M+2, PB_L-1); i_5++) {
            B[i_5+i_4-2][i_5] = D[i_5+2][i_4-1] + D[i_5][i_4+2] + D[i_4][i_5+1] - D[i_5][i_4] + D[i_5-2][i_4-2] - D[i_4][i_5] - B[i_5+i_4-4][i_5] + 5;
        }
    }
#pragma endscop
...
kernel_4222152123_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
