...
void kernel_3332211221_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, PB_M-1); i_5++) {
            for (int i_6 = 1; i_6 < -i_4+PB_M; i_6++) {
                C[i_5][i_4][i_6-1] = D[i_6+i_4][i_5] * C[i_5+1][i_4][i_6-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3332211221_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
