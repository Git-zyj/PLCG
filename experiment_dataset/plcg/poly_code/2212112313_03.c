...
void kernel_2212112313_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[0][i_1-1] = C[i_1][i_0+2] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[0][i_3] = C[i_2][i_3] - B[0][i_3-1] + B[0][i_3+1] + B[0][i_3+1] - 3;
        }
    }
#pragma endscop
...
kernel_2212112313_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
