...
void kernel_4424232123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
    ...
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = i_1; i_2 < -i_0+PB_N-1; i_2++) {
                C[i_1][i_2+i_0+1][i_1+2] = F[i_2][i_0-2] * 2;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < min(-i_3+PB_P, PB_Q-2); i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_L-1; i_5++) {
                for (int i_6 = 1; i_6 < i_4; i_6++) {
                    C[i_6][i_5+i_4][i_6+2] = G[i_6][i_5+i_4+1][i_3] + G[i_6][i_4][i_5] * H[i_5+1][i_3-2][i_6][i_4+2] - G[i_6-1][i_4][i_3+1] + G[i_5][i_4][i_4+i_3] + 4;
                }
            }
        }
        for (int i_7 = 0; i_7 < PB_P-2; i_7++) {
            for (int i_8 = 0; i_8 < -i_7+PB_N; i_8++) {
                C[i_3][i_8+i_7][i_3+2] = I[i_8][i_7+2] + 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4424232123_00(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
