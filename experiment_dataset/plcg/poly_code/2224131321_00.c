...
void kernel_2224131321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0][i_1-1] = C[i_1][i_0] + C[i_0+1][i_1+1] - D[i_1-1] * A[i_1+i_0-1][i_1-3] * A[i_1+i_0-2][i_1-1] * A[i_1+i_0-2][i_1-3] - 6;
        }
        for (int i_2 = i_0; i_2 < -i_0+PB_M; i_2++) {
            A[i_2+i_0][i_0-1] = D[i_0] + C[i_0][i_2-1] + 5;
        }
    }
#pragma endscop
...
kernel_2224131321_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
