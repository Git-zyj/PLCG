...
void kernel_4226111123_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-1); i_1++) {
            B[i_0-1][i_1+1] = C[i_1][i_0] + C[i_1][i_0-1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, PB_L+1); i_3++) {
            B[i_2-1][i_3-1] = B[i_2-1][i_3-2] * 2;
        }
    }
#pragma endscop
...
kernel_4226111123_03(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
