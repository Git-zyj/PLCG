...
void kernel_4234152311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            A[i_1][i_0] = C[i_0-2][i_1] * C[i_1+2][i_0] * C[i_1][i_0] + A[i_1+1][i_0+1] + B[i_1+3] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_P; i_3++) {
            B[i_3+2] = C[i_2+2][i_2] * C[i_2][i_3+i_2] * C[i_3][i_2+2] + C[i_3][i_2-2] + C[i_2][i_3] + C[i_3][i_2+1] * B[i_3+3] * B[i_3+1] + B[i_3+3] - 2;
        }
    }
#pragma endscop
...
kernel_4234152311_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
