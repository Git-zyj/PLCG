...
void kernel_3312112125_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, i_0); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                B[i_2+i_1][i_2] = D[i_2+i_1][i_2] - 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_M, -i_3+PB_N-2); i_4++) {
            B[i_4+i_3][i_3] = D[i_4][i_3-1] * B[i_4+i_3+2][i_3] + C[i_3] + 3;
        }
        C[i_3] = 1;
    }
#pragma endscop
...
kernel_3312112125_06(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
