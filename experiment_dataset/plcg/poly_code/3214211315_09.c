...
void kernel_3214211315_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_1][i_1] = C[i_2][i_1+i_0] + B[i_2][i_2] + 5;
            }
        }
        for (int i_3 = i_0; i_3 < min(PB_L-1, -i_0+PB_P); i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                B[i_3][i_3] = C[i_4][i_3+i_0] + B[i_3+1][i_3+1] - B[i_4-1][i_4-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3214211315_09(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
