...
void kernel_3436311225_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_1-1][i_1] = A[i_1-2][i_1-1] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M-3; i_3++) {
            for (int i_4 = i_3; i_4 < PB_P; i_4++) {
                C[i_4][i_3+i_2+3][i_4] = 3;
            }
        }
        for (int i_5 = 1; i_5 < min(i_2, -i_2+PB_M-1); i_5++) {
            for (int i_6 = 0; i_6 < min(PB_P, -i_5+PB_M); i_6++) {
                C[i_6][i_5+i_2+1][i_6] = E[i_6+i_5][i_2] + F[i_6][i_2-1][i_5-1] - C[i_6][i_5+i_2-1][i_6] * 2;
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_L-1; i_7++) {
        A[i_7-2][i_7-1] = G[i_7] - A[i_7][i_7+1] + 6;
    }
#pragma endscop
...
kernel_3436311225_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
