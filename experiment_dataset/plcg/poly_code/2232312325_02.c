...
void kernel_2232312325_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-4; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_0+2] = A[i_0-1][i_0+1] * A[i_0-2][i_0] * A[i_0+2][i_0+4] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-3; i_2++) {
        for (int i_3 = 0; i_3 < PB_N; i_3++) {
            A[i_2+1][i_2+3] = C[i_3][i_2] - 4;
        }
    }
#pragma endscop
...
kernel_2232312325_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
