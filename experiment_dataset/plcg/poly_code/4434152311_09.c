...
void kernel_4434152311_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
        ...
        for (int i_1 = 0; i_1 < min(PB_L-1, -i_0+PB_P); i_1++) {
            for (int i_2 = 2; i_2 < i_1; i_2++) {
                for (int i_3 = 2; i_3 < PB_L; i_3++) {
                    A[i_3][i_0][i_2][i_1] = E[i_3][i_1+i_0][i_2-2][i_0] + F[i_2+i_1][i_3][0] - F[i_3][i_2-2][i_1] * E[i_2-2][i_1][i_2][i_3-2] * F[i_2][i_3][i_1+1] + E[i_2+1][i_0][0][i_1] + G[i_1][i_0][i_2+2][2][i_3-2] + E[i_3][i_2][i_0-1][i_3] - F[i_3][i_0-1][i_2] + A[i_3-1][i_0][i_2][i_1] + A[i_1][i_0+1][i_2][i_3] * 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 0; i_6 < PB_M; i_6++) {
                B[i_6+1][i_5] = H[i_6][i_5] - F[i_5][i_6][i_4-1] + B[i_6][i_5] * B[i_6+1][i_5+1] + B[i_6+2][i_5] + 2;
            }
        }
        for (int i_7 = 0; i_7 < -i_4+PB_L-2; i_7++) {
            for (int i_8 = 1; i_8 < -i_4+PB_M; i_8++) {
                B[i_7+1][i_4] = F[i_7][i_8][i_4] + F[i_8][i_7][i_4] * F[i_8][i_4-2][i_7+i_4+2] - H[i_4][i_7] - F[i_8][i_4][i_7+i_4] - 2;
                B[i_7+1][i_8] = F[i_7][i_8+i_4][i_4] - H[i_8][i_7] + F[i_8-1][i_4][i_7] - F[i_7][i_8+1][i_4] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434152311_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
