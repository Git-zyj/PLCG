...
void kernel_4332212321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        A[i_0] = D[i_0-1] * A[i_0-1] + A[i_0+1] + A[i_0+2] - A[i_0-2] * A[i_0-1] - 3;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        A[i_1] = 3;
        for (int i_2 = 2; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            for (int i_3 = 1; i_3 < min(i_2, -i_1+PB_L+1); i_3++) {
                for (int i_4 = 2; i_4 < PB_M+2; i_4++) {
                    C[i_4-2][i_2+i_1][i_3][i_2-1] = C[i_2-2][i_3+i_1-1][i_4-2][i_3-1] * 6;
                }
            }
        }
    }
...
kernel_4332212321_00(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
