...
void kernel_2234312321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 3; i_1 < PB_L+1; i_1++) {
            A[i_0][i_0-2] = A[i_0+2][i_0] + A[i_1-1][i_1-3] + A[i_0-1][i_0-3] - 2;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        A[i_2-1][i_2-3] = C[i_2] - 6;
    }
#pragma endscop
...
kernel_2234312321_07(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
