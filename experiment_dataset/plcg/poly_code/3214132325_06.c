...
void kernel_3214132325_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-4; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L; i_2++) {
                A[i_0+2][i_1-1][i_0] = C[i_1][i_2+1][i_2] * C[i_1+1][i_2+i_0][i_1] - A[i_0][i_1+1][i_0-2] + A[i_1+4][i_0-1][i_1+2] * A[i_0+3][i_1-1][i_0+1] + 2;
            }
        }
        for (int i_3 = 1; i_3 < i_0; i_3++) {
            for (int i_4 = 3; i_4 < PB_M; i_4++) {
                A[i_0+2][i_4-3][i_0] = D[i_4][0] + D[i_4][i_3] * C[i_0][i_4+1][i_3] - C[i_4][i_0+2][i_3-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3214132325_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
