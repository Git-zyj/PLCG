...
void kernel_4436251221_08(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,int vm,int wm,DATA_TYPE POLYBENCH_5D(M,xM,yM,zM,vM,wM,xm,ym,zm,vm,wm)){
        ...
        for (int i_1 = max(1, i_0); i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
                for (int i_3 = 1; i_3 < PB_M; i_3++) {
                    C[i_3][i_3-1] = E[i_2+i_1][i_0][i_3-1][i_1+1] + F[i_0][i_1][i_3+i_2-1] + G[i_2][i_3][i_0][i_1-1] + F[i_3][i_1-1][i_1-1] * C[i_2][i_2-1] - 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        B[i_4+1] = H[i_4] + I[i_4] - H[i_4] - H[i_4] - H[i_4+1] + I[i_4] - 6;
        for (int i_5 = i_4; i_5 < PB_P-1; i_5++) {
            for (int i_6 = 3; i_6 < min(PB_M-2, -i_5+PB_N-1); i_6++) {
                C[i_6][i_6-1] = J[i_6+i_5+1][i_5] - F[i_6][i_4][i_5+1] - K[i_6-1][i_5] + F[i_4][i_6][i_5] - C[i_6-2][i_6-3] - C[i_6+2][i_6+1] - 3;
            }
        }
        for (int i_7 = 1; i_7 < min(i_4, PB_P-3); i_7++) {
            for (int i_8 = 1; i_8 < PB_M; i_8++) {
                for (int i_9 = 1; i_9 < min(-i_7+PB_L-1, -i_8+PB_M); i_9++) {
                    D[i_7][i_8][i_7+1] = L[i_7+1][i_9+i_8][0] + G[i_9+i_7+1][i_4][i_9+i_8-1][i_8] * G[i_7][i_7+1][i_9+i_8][i_4] * G[i_7][i_4-1][i_9][i_9-1] + E[i_7][i_4+1][i_8-1][i_9+i_7-1] * M[i_8][i_4][i_7][i_9][i_8-1] - D[i_7+2][i_4][i_7+3] + D[i_4][i_9+2][i_4+1] - D[i_7-1][i_8][i_7] * 2;
                }
            }
        }
    }
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4436251221_08(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M));
