...
void kernel_2322112125_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L-3; i_1++) {
            A[i_1] = 1;
            B[i_1+i_0+1] = D[i_1+2][i_0] - B[i_1+i_0+3] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_L+2; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            C[i_2][i_3] = C[i_3][i_2-2] - 2;
        }
    }
#pragma endscop
...
kernel_2322112125_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
