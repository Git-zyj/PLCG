...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4424352111_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < -i_1+PB_L-2; i_2++) {
                for (int i_3 = 1; i_3 < PB_P; i_3++) {
                    A[i_2-1][i_1][i_3][i_3] = E[i_0-2][i_1][i_3][i_2] + F[i_2+i_1+2][i_3-1][i_0][i_0] * 2;
                }
            }
        }
        for (int i_4 = i_0; i_4 < -i_0+PB_M; i_4++) {
            for (int i_5 = 2; i_5 < -i_0+PB_Q; i_5++) {
                for (int i_6 = i_5; i_6 < PB_P-1; i_6++) {
                    B[i_6][i_5-2][i_5+i_0][0] = E[i_6][i_5][i_4][i_5] + G[i_6+i_5-2][i_4][i_4+i_0][i_0] - H[i_6][i_0][i_5] - E[i_0][i_5][i_6+1][i_4] + F[i_6][i_4][i_5-1][i_0] * 5;
                }
            }
            for (int i_7 = 1; i_7 < i_0; i_7++) {
                for (int i_8 = 1; i_8 < -i_0+PB_L; i_8++) {
                    C[i_0][i_7][i_7-1][i_8] = G[i_7][i_0][i_8-1][i_4] - E[i_0][i_7+1][i_4+1][i_8] + H[i_7+1][i_4][2] + E[i_7][i_8+i_0][i_8][i_4-1] * E[0][i_7][i_4][i_0] * F[i_7-1][i_4][i_0][i_8] * 3;
                }
            }
        }
    }
    for (int i_9 = 2; i_9 < PB_P-1; i_9++) {
        for (int j_0 = 1; j_0 < PB_P-2; j_0++) {
            for (int j_1 = 1; j_1 < min(min(j_0, -i_9+PB_P-1), -j_0+PB_M+1); j_1++) {
                D[j_1][j_0] = I[j_1][j_0][i_9] - J[j_0][j_1-1][i_9-2] * K[j_1+i_9][j_0+2] * L[j_0][j_1+i_9+1] * I[i_9-1][j_0][j_1+i_9] + M[j_1+j_0-1][i_9+1] * H[j_0][j_0][i_9-2] + D[j_1-1][j_0] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4424352111_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,POLYBENCH_ARRAY(M));
