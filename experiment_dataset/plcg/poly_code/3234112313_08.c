...
void kernel_3234112313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+1; i_0++) {
        A[i_0-2] = A[i_0-3] - A[i_0-1] * A[i_0-1] + A[i_0-1] - 4;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_L-4; i_2++) {
            B[i_2][i_2+i_1+2] = C[i_1] - B[i_2-1][i_2+i_1+1] + B[i_2+1][i_2+i_1+4] - 6;
        }
    }
#pragma endscop
...
kernel_3234112313_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
