...
void kernel_2226212321_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = C[i_0] * A[i_0+4] - A[i_0+1] * 1;
    }
    for (int i_1 = 4; i_1 < PB_N-1; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_N, -i_1+PB_M+1); i_2++) {
            B[i_1][i_2-1][i_2+i_1-2] = B[i_1-2][i_2-1][i_2+i_1-4] * B[i_1][i_2][i_2+i_1-1] * B[i_1+1][i_2-2][i_2+i_1-2] - 4;
        }
    }
#pragma endscop
...
kernel_2226212321_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
