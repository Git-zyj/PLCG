...
void kernel_3234312315_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = C[2] + A[i_0+1] * A[i_0-1] * A[i_0-1] + A[i_0+1] * 2;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_L, i_1); i_2++) {
            for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                B[i_2][i_3] = B[i_3+1][i_2+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3234312315_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
