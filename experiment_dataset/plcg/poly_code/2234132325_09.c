...
void kernel_2234132325_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1][i_1] = C[i_1] * A[i_1-1][i_1-1] + A[i_1+1][i_1+1] + 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        A[i_2][i_2] = C[i_2-2] + C[i_2-2] - C[i_2] - C[i_2+1] - C[i_2] * A[i_2-1][i_2-1] + 3;
    }
#pragma endscop
...
kernel_2234132325_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
