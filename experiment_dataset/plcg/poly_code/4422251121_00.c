...
void kernel_4422251121_00(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,int wl,DATA_TYPE POLYBENCH_5D(L,xL,yL,zL,vL,wL,xl,yl,zl,vl,wl),int xm,int ym,int zm,int vm,int wm,DATA_TYPE POLYBENCH_5D(M,xM,yM,zM,vM,wM,xm,ym,zm,vm,wm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        D[i_0+1] = E[i_0] - F[i_0] - G[i_0][0] + 4;
    }
        ...
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
                for (int i_4 = 0; i_4 < min(-i_2+PB_M+1, PB_L-1); i_4++) {
                    B[i_4+i_2-1][i_3+1][i_2][i_1] = H[i_4][i_1][i_3][i_2+1] - H[i_4+1][i_1+1][i_2-1][i_3] * I[i_4][i_3][i_2] * B[i_4+i_2-1][i_3+2][i_2][i_1] + B[i_4+i_2-1][i_3+3][i_2][i_1+1] - 6;
                }
            }
        }
        for (int i_5 = 0; i_5 < min(-i_1+PB_P, PB_L-1); i_5++) {
            for (int i_6 = 1; i_6 < min(PB_L, -i_1+PB_P); i_6++) {
                for (int i_7 = 0; i_7 < PB_L-1; i_7++) {
                    C[i_7][i_5][i_6+i_1] = J[i_1+1][i_7][i_6][i_1] * J[i_5][i_7+1][i_5+i_1][i_6-1] - J[i_6][i_7+1][i_5][i_1] - K[i_6][i_5+1][i_1-1] + L[i_6-1][i_7][i_1][i_5][i_5] * M[i_5][i_7][i_1][i_6][i_1] + 2;
                }
            }
        }
        D[i_1] = E[i_1-1] * E[i_1] - E[i_1-1] * E[i_1] * F[i_1] * F[i_1-1] - E[i_1] - 3;
    }
#pragma endscop
...
kernel_4422251121_00(xd,POLYBENCH_ARRAY(D), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,wl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M));
