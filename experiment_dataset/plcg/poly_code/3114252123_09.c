...
void kernel_3114252123_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_N; i_1++) {
            A[i_0-1][i_1+i_0] = B[i_1+i_0][i_0-2] + C[i_1] - D[i_1][i_0] + D[i_1][i_1-1] + D[i_0][i_1] - A[i_0-1][i_1+i_0-1] * 4;
        }
    }
#pragma endscop
...
kernel_3114252123_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
