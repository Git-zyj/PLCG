...
void kernel_2224312221_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-3; i_1++) {
            A[i_0][i_1+2] = A[i_0][i_1+3] * 4;
        }
        for (int i_2 = 2; i_2 < i_0; i_2++) {
            B[i_2-1][i_0] = C[i_0+1][i_0-2] - B[i_2-1][i_0+1] * B[i_2-2][i_0+2] - 4;
        }
    }
#pragma endscop
...
kernel_2224312221_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
