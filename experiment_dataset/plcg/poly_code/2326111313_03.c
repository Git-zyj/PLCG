...
void kernel_2326111313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P-1; i_1++) {
            A[i_0][i_1+i_0] = A[i_0+1][i_1+i_0+1] * A[i_0+1][i_1+i_0] * A[i_0-1][i_1+i_0-1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            B[i_3+1][i_2] = B[i_3][i_2] + B[i_3+1][i_2+1] - 6;
        }
        for (int i_4 = 0; i_4 < min(PB_M, i_2); i_4++) {
            C[i_2][i_4+1] = D[i_4][i_2] + D[i_4][i_2] * C[i_2][i_4+2] * C[i_2][i_4] - C[i_2+1][i_4+2] - 6;
        }
    }
#pragma endscop
...
kernel_2326111313_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
