...
void kernel_3324132225_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_0][i_1] = D[i_0][1] - E[i_0] + A[i_0][i_1-1] + A[i_0][i_1-2] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                A[i_3][i_4] = D[i_3-1][i_3] * 6;
            }
            for (int i_5 = 1; i_5 < min(-i_3+PB_N, PB_L); i_5++) {
                A[i_5-1][i_3+1] = F[i_3+1][i_5][i_2+1] - G[i_2-1][i_5][i_3][i_3] + F[i_3][i_2][i_2] - G[i_2][i_5+i_3][i_2][0] * D[i_3][i_2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3324132225_06(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
