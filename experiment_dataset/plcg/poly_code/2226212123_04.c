...
void kernel_2226212123_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_P-2; i_2++) {
            B[i_2-1][i_2] = C[i_2][i_1] - C[i_2-1][i_1+2] + B[i_2+1][i_2+2] + 2;
        }
    }
#pragma endscop
...
kernel_2226212123_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
