...
void kernel_2436111221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L-2; i_3++) {
            B[i_3+i_2][i_3+i_2] = E[i_3] - F[i_3][i_2][i_2] - B[i_3+i_2+2][i_3+i_2+2] - 6;
        }
    }
    for (int i_4 = 3; i_4 < PB_L; i_4++) {
        D[i_4-3] = G[i_4][i_4] + E[i_4] * 1;
    }
    for (int i_5 = PB_M-1 - 1; i_5 >= 1; i_5--) {
        D[i_5-1] = D[i_5] + D[i_5+1] + 5;
    }
#pragma endscop
...
kernel_2436111221_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
