...
void kernel_2226211323_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            A[i_0+1][i_1] = A[i_0-1][i_1] - A[i_0+1][i_1-2] + A[i_0+2][i_1-2] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_L, i_2); i_3++) {
            A[i_2+1][i_3] = C[i_2][i_3-1] + 6;
        }
    }
#pragma endscop
...
kernel_2226211323_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
