...
void kernel_3236211321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0-1] = A[i_1+1][i_0+1] + A[i_1-2][i_0-1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_N; i_4++) {
                B[i_3][i_3][i_2] = C[i_4+i_3][i_2-1][i_2] - C[i_4+1][i_2+1][1] + B[i_4-1][i_4-1][i_2] * B[i_4][i_4][i_2+1] * B[i_3+2][i_3+2][i_2+1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3236211321_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
