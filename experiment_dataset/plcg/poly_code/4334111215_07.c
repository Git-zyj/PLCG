...
void kernel_4334111215_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        ...
        for (int i_3 = 1; i_3 < min(PB_P+1, i_2); i_3++) {
            for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
                C[i_2+1][i_3-1][i_4] = C[i_2+1][i_4-1][i_3-1] + C[i_2+1][i_3-1][i_4+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4334111215_07(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
