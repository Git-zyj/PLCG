...
void kernel_4226312125_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_2 = 1; i_2 < -i_1+PB_P+1; i_2++) {
            for (int i_3 = 0; i_3 < min(i_1, PB_L-2); i_3++) {
                for (int i_4 = 1; i_4 < -i_1+PB_P; i_4++) {
                    B[i_3+2][i_4+i_1][i_2-1] = B[i_3+2][i_2+i_1-1][i_4-1] * 2;
                }
            }
        }
    }
...
kernel_4226312125_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
