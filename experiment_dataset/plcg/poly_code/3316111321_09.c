...
void kernel_3316111321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < PB_N-2; i_3++) {
            C[i_3] = C[i_3-2] - C[i_3+1] + C[i_3-1] * C[i_3+2] + 4;
        }
    }
#pragma endscop
...
kernel_3316111321_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
