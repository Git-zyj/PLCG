...
void kernel_4216352225_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                    B[i_0+2][i_3][i_3] = C[i_2][i_3][1][i_0-2] * D[i_3+2][i_1-1][i_2-2] - E[i_1][i_0][i_2+1][i_3+1] * E[i_2][i_0][i_3][0] * F[i_2][i_3][i_0+1][i_1] * 2;
                }
            }
        }
        for (int i_4 = 2; i_4 < i_0; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_4+PB_M, PB_L-2); i_5++) {
                B[i_0][i_5][i_5] = D[i_0][i_4-2][i_5] - G[i_5+i_4-2][i_0][i_4] * G[i_5+i_4][i_0-1][0] - G[i_5+2][i_0-1][i_4+2] + B[i_0-2][i_5][i_5] + B[i_0+1][i_5+2][i_5+2] - 3;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4216352225_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
