...
void kernel_4336131125_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = max(1, i_0); i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_P; i_3++) {
                    A[i_1][i_0][i_2-1][i_3] = D[i_1][i_2][i_3][i_0] + D[i_0][i_3][i_1][i_2] * D[i_3][i_0+1][i_1-1][i_1] - D[i_3][i_3+i_1][i_2][i_0] - 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
        B[i_4] = E[i_4] + F[i_4][i_4] + E[i_4] + B[i_4+1] * 6;
    }
    for (int i_5 = 0; i_5 < PB_Q; i_5++) {
        for (int i_6 = 1; i_6 < i_5; i_6++) {
            for (int i_7 = 1; i_7 < min(i_6, -i_6+PB_N); i_7++) {
                for (int i_8 = 0; i_8 < PB_L; i_8++) {
                    A[i_8][i_6-1][i_7-1][i_5] = G[i_7+i_6][i_7+1][i_8] + 2;
                }
            }
        }
    }
...
kernel_4336131125_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
