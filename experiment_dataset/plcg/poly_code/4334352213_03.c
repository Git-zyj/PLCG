...
void kernel_4334352213_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        A[i_0] = D[i_0] * E[i_0] * F[i_0][i_0] + E[i_0] - G[i_0] * G[i_0] - 4;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            B[i_1][i_2] = H[i_2][i_1] - I[i_2+2][0] + F[i_2+2][i_2] - I[i_2+2][i_2] + F[i_2][i_1] + B[i_1+1][i_2-1] * B[i_1][i_2-1] + B[i_1][i_2+1] - 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        A[i_3] = D[i_3] * F[i_3][2] * G[i_3] + E[i_3] - 2;
    }
#pragma endscop
...
kernel_4334352213_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
