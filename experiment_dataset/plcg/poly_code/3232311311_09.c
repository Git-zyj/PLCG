...
void kernel_3232311311_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < PB_L+2; i_2++) {
                A[i_1][i_1-1] = A[i_1+1][i_1] * A[i_1-1][i_1-2] + A[i_2-1][i_2-2] + 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-2; i_4++) {
            B[i_4][i_3+1] = C[i_4+1][i_3] + B[i_3][i_4+2] + B[i_4-1][i_3+1] * B[i_4][i_3] - 1;
        }
    }
#pragma endscop
...
kernel_3232311311_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
