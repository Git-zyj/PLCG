...
void kernel_4234352115_03(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
        ...
        for (int i_1 = 1; i_1 < min(PB_P-2, i_0); i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_Q; i_2++) {
                for (int i_3 = i_1; i_3 < -i_1+PB_M; i_3++) {
                    B[i_3-1][i_3+i_1][i_1+1][i_2-2] = C[i_0][i_2][i_1+2][0] - D[i_3][i_2][i_3][i_0+1] + C[i_1][i_2-2][i_3][i_0] - C[i_0-1][i_3][i_1][i_2+i_1] + C[i_2][i_1-1][i_3][i_1] - E[i_3][i_0][i_2][i_1+2][i_3] * F[i_2-2][i_2][i_0][i_1] - 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 1; i_6 < -i_5+PB_P; i_6++) {
                for (int i_7 = 0; i_7 < min(-i_5+PB_L, -i_4+PB_M); i_7++) {
                    B[i_4-1][i_7+i_4-1][i_7][i_5-2] = E[i_4+2][i_7+i_5][i_6+i_5][i_4][i_6-1] + G[i_6+1][i_5+2][i_6+2] + B[i_7][i_7+i_4][i_4][i_5-2] + 1;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4234352115_03(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
