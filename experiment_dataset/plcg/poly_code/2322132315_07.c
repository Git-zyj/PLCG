...
void kernel_2322132315_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            A[i_1] = D[i_0] - A[i_1+1] + A[i_1-1] - 6;
        }
        A[i_0] = D[i_0-2] + A[i_0] * C[i_0] * 3;
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        C[i_2] = D[i_2] - D[i_2] * D[i_2] - D[i_2+1] + D[i_2-2] - D[i_2] + C[i_2+1] - C[i_2-1] - 4;
    }
#pragma endscop
...
kernel_2322132315_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
