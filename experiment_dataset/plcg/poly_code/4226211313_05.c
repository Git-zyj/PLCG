...
void kernel_4226211313_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_P, -i_0+PB_M+1); i_1++) {
            B[i_0-1][i_1+i_0-1][0] = C[i_1][i_0] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_P, -i_2+PB_M-2); i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                B[i_2-1][i_3+i_2][0] = B[i_2-2][i_3+i_2-1][0] - B[i_2][i_3+i_2+2][0] - B[i_2-1][i_3+i_2+1][0] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4226211313_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
