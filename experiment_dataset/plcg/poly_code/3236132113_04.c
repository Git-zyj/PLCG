...
void kernel_3236132113_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                A[i_0][i_1-1][i_0-2] = C[i_2][i_1+i_0][i_0] * C[i_2-2][i_1+1][i_0] * D[i_1+1][i_0] + A[i_0+1][i_1-1][i_0-1] * 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 3; i_4 < PB_M+1; i_4++) {
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                A[i_4-1][i_5][i_4-3] = C[i_4-2][i_3][i_5] + C[i_5][i_3+1][i_4] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3236132113_04(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
