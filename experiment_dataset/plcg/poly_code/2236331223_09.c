...
void kernel_2236331223_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            A[i_1+i_0][i_1] = C[i_1][i_0] + D[i_1+i_0][1] * A[i_1+i_0-3][i_0-1] - A[i_1+i_0-1][i_1-1] - 4;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M-1, i_0); i_2++) {
            A[i_2+i_0+1][i_2] = C[i_2][i_0+1] * C[i_2][i_0] + D[i_2][i_0+1] * D[i_2][i_0] + 4;
        }
    }
#pragma endscop
...
kernel_2236331223_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
