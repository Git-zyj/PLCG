...
void kernel_2336112115_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            A[i_0-1][i_0-1] = D[i_0][i_0+1] * 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_L-3; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_P; i_4++) {
            C[i_3+2][i_4] = D[i_3][i_4+i_3] + C[i_3+3][i_4-1] - 2;
        }
    }
#pragma endscop
...
kernel_2336112115_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
