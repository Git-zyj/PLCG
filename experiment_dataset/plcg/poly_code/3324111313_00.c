...
void kernel_3324111313_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P, -i_0+PB_N); i_1++) {
            A[i_1+i_0-1][i_1] = D[i_0][1] + A[i_1+i_0][i_1] * 3;
            for (int i_2 = i_0; i_2 < PB_P; i_2++) {
                A[i_2+i_1-1][i_1] = E[i_1][i_2+1][i_0] * C[i_0-1][i_1-1][0] + C[i_1][i_0][0] * C[i_0][i_1][0] + 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = max(0, -i_4+1); i_5 < min(PB_P, -i_4+PB_N+1); i_5++) {
                C[i_5][i_4][0] = A[i_5+i_4-1][i_5] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3324111313_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
