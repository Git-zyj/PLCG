...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        ...
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_N-3); i_2++) {
            B[i_2+1] = G[1][i_1] - H[i_2+i_1][0] - B[i_2+3] - 3;
        }
    }
#pragma endscop
...
kernel_2434211125_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
