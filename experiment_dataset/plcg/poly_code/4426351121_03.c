...
void kernel_4426351121_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
        ...
        for (int i_2 = max(2, i_1); i_2 < -i_1+PB_L; i_2++) {
            for (int i_3 = 0; i_3 < -i_1+PB_P; i_3++) {
                for (int i_4 = i_3; i_4 < PB_M; i_4++) {
                    B[i_2][i_1][i_3][i_4] = G[i_4][i_2-1][i_3] * H[i_4][i_3][i_1-1][i_2-1] + I[i_3][i_3+i_1][i_2] - G[i_3+i_1][i_4+1][i_2] * 6;
                }
            }
            C[i_2][i_2] = J[i_2][i_1+1] * J[i_2+i_1][i_2] * F[i_2] + K[i_1+1][i_2] - F[i_2] * K[i_2-1][i_2+i_1] * F[i_2] + A[i_2+1] * C[i_2-2][i_2-2] * 6;
        }
    }
    for (int i_5 = PB_P-1 - 1; i_5 >= 1; i_5--) {
        for (int i_6 = 1; i_6 < min(-i_5+PB_M, i_5); i_6++) {
            D[i_6][i_5-1] = F[i_6-1] - L[i_6] - K[i_6][i_6+1] * L[i_6+i_5] + J[i_6][i_5+1] + K[i_6][i_5] * D[i_6+1][i_5-1] - 6;
        }
    }
#pragma endscop
