...
void kernel_2226311315_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = C[i_0-1] * A[i_0] + A[i_0+2] * A[i_0+2] + 2;
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            A[i_1+1] = D[i_0-1][i_1+1] * 1;
        }
    }
#pragma endscop
...
kernel_2226311315_05(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
