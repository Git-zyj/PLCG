...
void kernel_2322312221_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-4; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-4, i_0); i_1++) {
            A[i_1+i_0][i_1] = A[i_1+i_0+4][i_1+2] - A[i_1+i_0-1][i_0-1] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        ...
        for (int i_4 = 1; i_4 < PB_N-3; i_4++) {
            C[i_4+2][i_2][i_4] = D[i_4+2][i_2] - C[i_4+3][i_2-1][i_4+1] * C[i_2][i_4-1][i_2-2] + C[i_4+2][i_2+2][i_4] * 6;
        }
    }
#pragma endscop
...
kernel_2322312221_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
