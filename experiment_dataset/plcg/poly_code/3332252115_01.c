...
void kernel_3332252115_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_0+1][i_1-2] = D[i_1][i_0] - E[i_1][0][i_0+1] * F[i_1-2][i_0] + F[i_0+1][i_1] - A[i_1][i_0-2] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_M, -i_2+PB_N); i_3++) {
            A[i_2+2][i_3-1] = D[i_2+1][i_3] + G[i_3+i_2] + F[i_2][i_2] * D[i_2][i_3-2] + F[i_2-1][i_3] - D[i_3-2][0] + D[i_3][i_2] * F[i_3+i_2][i_3-2] + 5;
        }
    }
    for (int i_4 = 0; i_4 < PB_P; i_4++) {
        for (int i_5 = 2; i_5 < PB_M+2; i_5++) {
            for (int i_6 = 2; i_6 < PB_M+2; i_6++) {
                A[i_5+2][i_6-2] = H[i_5-2][i_6][i_4] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3332252115_01(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
