...
void kernel_2322312211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        ...
        for (int i_3 = 0; i_3 < PB_N-3; i_3++) {
            C[i_1+2] = E[i_3][i_1+2] * E[i_1][i_3] - C[i_3+1] - C[i_3+3] + C[i_1+1] * 3;
        }
    }
#pragma endscop
...
kernel_2322312211_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
