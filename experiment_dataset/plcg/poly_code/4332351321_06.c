...
void kernel_4332351321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0][i_0] * E[i_0] - F[i_0] - G[i_0] + F[i_0-1] - G[i_0] * A[i_0+1] - C[i_0] + 3;
    }
    for (int i_1 = 4; i_1 < PB_Q; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_Q-3, i_1); i_2++) {
            for (int i_3 = 2; i_3 < -i_1+PB_Q+4; i_3++) {
                for (int i_4 = max(1, i_2); i_4 < min(-i_1+PB_Q+2, -i_2+PB_Q-3); i_4++) {
                    B[i_1][i_2][i_3][i_4+i_2] = H[i_4][i_3][i_2][i_1] * I[i_4+i_3][i_2][i_3] - J[i_2][i_1-1][i_4+1][i_3-1] + H[i_4][i_1][0][i_2] * B[i_2][i_1-2][i_3+1][i_4+i_1-2] * B[i_1+1][i_2][i_3-1][i_4+i_2+1] * B[i_1-1][i_2+1][i_3-1][i_4+i_2+3] - B[i_2+2][i_3-2][i_4-1][i_3+i_1-4] - 4;
                }
            }
        }
        for (int i_5 = i_1; i_5 < PB_N; i_5++) {
            C[i_5] = D[i_5-1][i_1] - K[i_5][i_1+1] + L[i_5][i_1+1] - D[i_1][i_1+1] + M[i_5][i_1][i_5] + C[i_5-1] * 1;
        }
    }
#pragma endscop
...
kernel_4332351321_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
