...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = max(1, i_0); i_2 < PB_N; i_2++) {
            B[i_2-1] = F[i_2][i_0] + G[i_2][i_0] + F[i_0][i_2] * F[i_2-1][i_0] - 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        C[i_3] = E[i_3] - D[i_3] + H[i_3-1] - C[i_3+1] + 4;
    }
#pragma endscop
...
kernel_2316331111_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
