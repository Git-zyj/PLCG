...
void kernel_3236232225_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                B[i_0-2][i_2][i_1+1] = C[i_2][i_2-2] * D[i_2][i_0+1][i_1] - 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_M, i_3); i_4++) {
            for (int i_5 = 2; i_5 < min(PB_M, -i_4+PB_P); i_5++) {
                B[i_3][i_5][i_4+1] = D[i_3-1][i_5][i_5+i_4] - D[i_4][i_3][i_5] - D[i_5][i_4][i_3] * B[i_3-1][i_5][i_4+2] * B[i_3+2][i_5-2][i_4+1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3236232225_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
