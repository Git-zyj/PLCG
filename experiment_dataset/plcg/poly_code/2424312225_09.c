...
void kernel_2424312225_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 3; i_3 < PB_M-2; i_3++) {
            C[i_3-1][i_3-3] = D[i_3+1][i_3+2] - D[i_2][i_2+1] + D[i_2+1][i_2+2] - 1;
        }
        C[i_2][i_2-2] = E[i_2] - E[i_2-1] + C[i_2+2][i_2] - 3;
        D[i_2][i_2+1] = 6;
    }
#pragma endscop
...
kernel_2424312225_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
