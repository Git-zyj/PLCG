...
void kernel_4232311215_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
        ...
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_L-1, i_1); i_2++) {
                for (int i_3 = 1; i_3 < PB_P; i_3++) {
                    A[i_1][i_2][i_3] = C[i_2-1][i_3-1][i_1] + A[i_2+1][i_1+1][i_3] - A[i_1][i_3][i_2+1] + 1;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        B[i_4][i_4] = B[i_4+1][i_4+1] + 4;
    }
#pragma endscop
...
kernel_4232311215_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
