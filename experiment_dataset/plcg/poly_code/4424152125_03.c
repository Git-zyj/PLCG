...
void kernel_4424152125_03(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_L-2; i_3++) {
                    B[i_0-2][i_3+2][i_1-2][0] = E[i_3+2][i_0][i_1+1][i_0] + F[i_2+2][i_1][i_3] + E[i_2][i_3+1][i_0][i_1-2] + E[i_0][i_1][i_3][0] + E[i_2][i_3+i_1+2][i_0][i_3] * E[i_1-2][i_3][i_3][i_2+2] - 5;
                }
            }
        }
    }
        ...
        for (int i_5 = max(i_4, 1); i_5 < PB_L; i_5++) {
            for (int i_6 = 2; i_6 < min(PB_L-2, -i_4+PB_L); i_6++) {
                for (int i_7 = 1; i_7 < -i_5+PB_L; i_7++) {
                    B[i_4][i_6+2][i_7][0] = F[i_7+i_5][i_7-1][i_6-2] - E[i_4+2][i_5][i_6][i_7+1] * E[i_4][0][i_7][i_6+2] + E[i_7][i_6+i_4][i_6][i_5-1] - E[i_5-1][i_7-1][i_6][i_4] + E[i_7][i_4][i_7][i_6] - 6;
                }
            }
        }
    }
    for (int i_8 = 2; i_8 < PB_L-2; i_8++) {
        for (int i_9 = i_8; i_9 < PB_L; i_9++) {
            for (int j_0 = 0; j_0 < -i_8+PB_L; j_0++) {
                for (int j_1 = 2; j_1 < -i_8+PB_P-1; j_1++) {
                    B[i_9+2][i_8+2][j_1+2][0] = F[j_1-2][i_9][i_9] * F[i_9-2][j_0][j_1+i_8+1] - G[i_9][j_0+1][j_1-2][j_1][j_0] * E[j_1+1][j_0+i_8][i_9][i_8-2] - F[j_0][j_1][i_8] + 6;
                }
            }
        }
        D[i_8] = H[i_8] * D[i_8+2] - 3;
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4424152125_03(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
