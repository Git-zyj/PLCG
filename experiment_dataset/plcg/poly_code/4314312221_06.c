...
void kernel_4314312221_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 2; i_3 < PB_P; i_3++) {
                    A[i_2][i_0][i_3][0] = D[i_1-2][i_2][i_0][i_1+1] + A[i_1][i_0+2][i_3][0] + A[i_2][i_0][i_3-2][0] + A[i_3-2][i_2+1][i_1][0] + 4;
                }
            }
        }
        B[i_0] = E[i_0] - 1;
    ...
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, PB_N); i_5++) {
            for (int i_6 = i_4; i_6 < min(PB_M, -i_4+PB_L); i_6++) {
                for (int i_7 = 2; i_7 < PB_M; i_7++) {
                    A[i_5][i_6][i_7-2][0] = F[i_7][i_6+i_4][i_7-2][i_5] + 3;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4314312221_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F)));
