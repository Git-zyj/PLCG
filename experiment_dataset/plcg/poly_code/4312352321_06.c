...
void kernel_4312352321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M-1; i_0++) {
        A[i_0-2] = D[i_0] * E[i_0] - D[i_0-2] + E[i_0] * E[i_0+1] * E[1] + D[i_0] - A[i_0-1] - A[i_0] * A[i_0-3] + A[i_0-4] * 3;
        for (int i_1 = 1; i_1 < -i_0+PB_P+2; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_1-1][i_0][i_2] = F[i_2][i_1+i_0-2] + G[i_1+2][i_0][i_2] * G[i_1+2][i_0-1][i_0+1] * B[i_0-3][i_2][i_1] - 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
                B[i_5-1][i_3][i_4] = H[i_5+2][i_3][i_4] + G[i_5][i_4-2][i_3-2] + F[i_3][i_4] - B[i_4+1][i_3][i_5-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4312352321_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
