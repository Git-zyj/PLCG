...
void kernel_4322351325_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_P+1, PB_M-2); i_1++) {
            for (int i_2 = i_1; i_2 < min(PB_M-1, -i_0+PB_P-1); i_2++) {
                A[i_2+1][i_1+i_0-1][i_1] = D[i_2][i_1-1][i_0] * A[i_1+1][i_2+i_0+1][i_2] - A[i_1+2][i_2+i_0+1][i_2+2] + A[i_2-1][i_1+i_0-1][i_1] * 1;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < -i_3+PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < -i_3+PB_L-2; i_5++) {
                for (int i_6 = 2; i_6 < PB_M-1; i_6++) {
                    B[i_5][i_4+i_3][i_6+i_5+1][i_6-1] = E[i_5][i_4][i_6-1][1] - F[i_6][i_4+1][i_3][i_5+i_3+1] - G[i_5][i_6][i_4][i_3][i_6] + E[i_6-1][i_5][i_4-1][i_3] * B[i_6+1][i_4+i_3+1][i_6+i_5+2][i_5-1] - B[i_6+1][i_5+i_3+2][i_6+i_4+2][i_4-1] * B[i_5][i_4+i_3][i_6+i_5][i_6-2] * 4;
                }
            }
        }
        for (int i_7 = 1; i_7 < -i_3+PB_M-1; i_7++) {
            C[i_3][i_7+1] = H[i_7][i_3] + I[i_7+i_3][i_3] * H[i_7+1][i_3] + I[i_7-1][i_3+1] - J[i_3][0] + J[i_3][i_7] + K[i_3+1][i_7][i_3-1] - J[i_7+i_3+1][0] - H[i_7][i_3] * C[i_7+2][i_3+1] - C[i_3][i_7-1] * 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4322351325_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
