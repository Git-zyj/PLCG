...
void kernel_4332352111_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,int wb,DATA_TYPE POLYBENCH_5D(B,xB,yB,zB,vB,wB,xb,yb,zb,vb,wb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0-1] + E[i_0+2] * F[i_0][i_0] + G[i_0] * E[i_0] * G[i_0-2] * 4;
        for (int i_1 = 1; i_1 < -i_0+PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                for (int i_3 = i_0; i_3 < -i_0+PB_M; i_3++) {
                    B[i_2][i_3+i_0][i_2+2][i_1-1][i_3] = H[i_3-2][i_2][i_0-1][i_1+1] + I[i_0+2][i_2][i_1][i_3] - J[i_2-2][i_3-2][i_1] * B[i_3-1][i_1+i_0+1][i_3+1][i_2-1][i_1] + 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < -i_4+PB_Q+2; i_5++) {
            for (int i_6 = 1; i_6 < PB_L; i_6++) {
                for (int i_7 = 0; i_7 < min(-i_4+PB_M-2, -i_6+PB_M-2); i_7++) {
                    B[i_5][i_7+i_4][i_5+2][i_6-1][i_7] = I[i_7+i_6+2][i_4][i_7][i_5+2] + J[i_7+i_5][i_6][0] - K[i_6+2][i_7][i_5+i_4-2][i_5-2][0] - L[i_7][i_4][i_5][i_6] * L[i_4][i_6-1][i_5][i_4] + J[i_7+i_4+2][i_6][i_5] - 3;
                }
            }
        }
    }
