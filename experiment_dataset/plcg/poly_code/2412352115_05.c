...
void kernel_2412352115_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0-1] = E[i_0] + F[i_0-1] + B[i_0+1][i_0-2] + 5;
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_0+2][i_0-1] = G[i_1][i_0] + H[i_1+1][i_0] + I[i_1][i_1] - J[i_1+1] * A[i_1] - 5;
        }
        for (int i_2 = 1; i_2 < min(PB_L, -i_0+PB_N); i_2++) {
            A[i_2-1] = H[i_0][i_2] * I[i_0][i_2] - G[i_2+i_0][i_0+1] * G[i_2][i_0+2] + I[i_2][i_0] + 4;
        }
    }
    for (int i_3 = 0; i_3 < PB_L-3; i_3++) {
        for (int i_4 = i_3; i_4 < min(-i_3+PB_N, PB_L); i_4++) {
            B[i_3+3][i_3] = I[i_4][i_3] * H[i_4][i_4] * H[i_3][i_4] - I[i_4][i_3+1] + G[i_4+i_3][0] + I[i_3][i_4] - 3;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2412352115_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
