...
void kernel_3322312223_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 3; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < PB_P; i_2++) {
                A[i_1-2][1] = D[i_2-2][i_1-2] - A[i_0-1][1] - A[i_1-3][1] + 2;
            }
        }
        B[i_0+2] = B[i_0+3] * 1;
        for (int i_3 = 0; i_3 < PB_P; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_M-2, i_3); i_4++) {
                C[i_0][i_4][i_4] = C[i_0-2][i_4-1][i_4-1] * C[i_4+2][i_0][i_0] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3322312223_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
