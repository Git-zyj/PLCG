...
void kernel_3234211323_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                A[i_0-1][i_1][i_2-1] = A[i_0-3][i_1][i_2-1] + 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_3+PB_L, PB_M); i_5++) {
                A[i_3-1][i_4][i_5-1] = C[i_5][i_5+i_3][i_4] * D[i_5][i_5+i_3][i_4] - A[i_3+1][i_4][i_5-1] + A[i_5-1][i_3][i_4-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3234211323_03(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
