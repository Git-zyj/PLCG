...
void kernel_2222112215_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_N+1; i_1++) {
            A[i_0][i_1-1] = C[i_1][i_0] + A[i_0-1][i_1-2] - A[i_0][i_1-2] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = i_2; i_3 < PB_N-1; i_3++) {
            A[i_3+1][i_2] = C[i_3][i_2] * 6;
        }
    }
#pragma endscop
...
kernel_2222112215_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
