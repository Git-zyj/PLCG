...
void kernel_2236151323_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M-3; i_1++) {
            A[i_1] = C[i_1] * C[i_1-1] * D[i_1][i_0] - B[i_1+3] * B[i_1-1] * B[i_1+1] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        B[i_2+1] = C[i_2] - C[i_2+1] + D[i_2+1][i_2] * C[i_2+1] - C[i_2] * D[i_2][i_2] - C[0] + A[i_2] + B[i_2+3] - B[i_2+2] + 6;
    }
#pragma endscop
...
kernel_2236151323_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
