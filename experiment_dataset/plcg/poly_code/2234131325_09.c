...
void kernel_2234131325_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_N-2 - 1; i_0 >= 2; i_0--) {
        A[i_0] = C[i_0][1] * A[i_0+2] + A[i_0-2] - A[i_0+1] * 2;
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            A[i_1-2] = D[i_1+1] + C[i_0-1][i_1+1] + C[i_1][i_0-1] - C[i_1+1][i_0-1] + 6;
        }
    }
#pragma endscop
...
kernel_2234131325_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
