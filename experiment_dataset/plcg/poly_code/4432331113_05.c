...
void kernel_4432331113_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - F[i_0] - G[i_0] - E[i_0+1] * 5;
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                    B[i_0+1][i_3][i_3][1] = H[i_1][i_3+1][i_3+1][i_0] - H[i_1][i_3][i_2][0] + H[i_2][i_0-1][i_1][0] - 2;
                }
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < min(-i_5+PB_P, i_4); i_6++) {
                for (int i_7 = 0; i_7 < -i_5+PB_M; i_7++) {
                    C[i_6-1][i_4][i_4][i_7] = I[i_7+i_5][i_7][i_4][i_6+i_5] * I[i_7][i_4+1][i_6][i_4] + J[i_6][i_7][i_5][i_4] + 2;
                }
            }
            for (int i_8 = 0; i_8 < PB_M; i_8++) {
                for (int i_9 = 0; i_9 < -i_8+PB_M; i_9++) {
                    D[i_8][i_4][i_5][0] = K[i_9+i_8][i_9][i_8] * D[i_8][i_4-1][i_5][0] * 5;
                }
            }
        }
    }
...
kernel_4432331113_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
