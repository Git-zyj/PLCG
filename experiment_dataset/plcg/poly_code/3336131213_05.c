...
void kernel_3336131213_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_N, i_0); i_1++) {
            for (int i_2 = 0; i_2 < min(PB_N, -i_0+PB_L-1); i_2++) {
                A[i_1][i_0+1][i_2] = D[i_2][i_0-1][i_0] - E[i_2+i_0+1][i_0+1] + D[i_1][i_2][i_0-1] - E[i_2][i_1] - C[i_1-1][i_2+1][i_2] * 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_P+1, PB_N); i_4++) {
            for (int i_5 = max(i_4, 1); i_5 < PB_N; i_5++) {
                B[i_4][i_4+1][i_5] = D[i_4][i_5][i_3+1] + D[0][i_4+1][i_3+1] - E[i_5+1][i_4+i_3-1] - B[i_4][i_4+1][i_5-1] - B[i_4+1][i_4+2][i_5] + 3;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M; i_6++) {
        for (int i_7 = 2; i_7 < min(i_6, PB_N); i_7++) {
            for (int i_8 = 0; i_8 < min(i_6, PB_N); i_8++) {
                C[i_7-1][i_8+1][i_8] = D[i_8][i_6][i_7+1] - D[i_7][i_8][i_7-1] - A[i_7][i_6+1][i_8] - C[i_7-2][i_8+1][i_8] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3336131213_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
