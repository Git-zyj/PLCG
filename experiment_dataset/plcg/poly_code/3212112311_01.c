...
void kernel_3212112311_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-3; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                B[i_0+1][i_1+1][i_2] = C[i_0][i_2][i_1-2] * 4;
            }
            for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                B[i_0][i_1+2][i_3] = B[i_0-1][i_1+2][i_3] + B[i_3][i_1+3][i_0] + B[i_0][i_3+1][i_1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3212112311_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
