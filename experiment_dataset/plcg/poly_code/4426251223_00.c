...
static void print_array(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4426251223_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = max(1, i_0); i_2 < PB_M-1; i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_Q; i_3++) {
                    B[i_2][i_3+1][i_1] = E[i_2][i_0+1][i_1][i_3+i_2] * E[i_2][i_0+1][i_3][i_1-1] - F[i_1-1][i_3][i_2+1] - E[i_2-1][i_0][i_3][i_3+1] - G[i_2][i_0][i_3][i_1] - E[i_3-1][i_0+1][i_2+1][i_1+1] * H[i_1][i_0][i_2][i_3+1][i_3] + E[i_1][i_2-1][i_0][i_3-1] - 2;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            for (int i_5 = max(1, i_0); i_5 < PB_M-1; i_5++) {
                for (int i_6 = 0; i_6 < PB_M; i_6++) {
                    B[i_5][i_4+1][i_6] = E[i_5][i_0][i_4-1][0] + F[i_5-1][i_6][i_5+1] * I[i_0+1][i_5][i_6][i_6][i_4-1] + B[i_6][i_5-1][i_4] - 1;
                }
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_M-1; i_7++) {
        for (int i_8 = 0; i_8 < PB_M-2; i_8++) {
            C[i_8] = J[i_7+1][i_8] - K[i_7][i_8+1][1] - J[i_8][i_8+1] + F[i_7][i_8][i_7+1] + L[i_7] * C[i_8+2] + 2;
        }
        for (int i_9 = max(1, i_7); i_9 < PB_M; i_9++) {
            for (int j_0 = 1; j_0 < min(i_9, -i_9+PB_M); j_0++) {
                B[i_9][j_0+1][i_7] = K[i_7][j_0-1][i_9-1] * F[i_7][j_0+i_9][i_9] * F[i_9][i_7+1][i_9] + J[j_0][0] * C[j_0] + C[j_0-1] - 3;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4426251223_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L)));
