...
void kernel_3314312225_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_L+2; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            A[i_2-2][i_3] = D[i_3] * C[i_2+3] * C[i_2] - 3;
        }
        C[i_2+2] = 5;
    }
#pragma endscop
...
kernel_3314312225_03(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
