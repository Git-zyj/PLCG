...
void kernel_4234111121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                A[i_2][i_2] = C[i_2][i_1] - 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_Q-1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_Q-1; i_4++) {
            for (int i_5 = 0; i_5 < min(min(i_4, -i_3+PB_L), -i_4+PB_L+2); i_5++) {
                B[i_5+i_3][i_4][i_4+1] = A[i_4][i_4] * B[i_5+i_4-2][i_3][i_3+1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4234111121_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
