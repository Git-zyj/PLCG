...
void kernel_4414252323_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q+2, PB_P); i_1++) {
            for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 0; i_3 < min(PB_P-2, -i_1+PB_P); i_3++) {
                    A[i_0+2][i_3][i_2][i_0] = E[i_3][i_2][i_1][i_0-2] - F[i_0][i_1][i_3][i_1] * G[i_2+2][i_3+i_1][i_1] * E[i_3][i_2][i_1+i_0-2][i_0] * A[i_2+2][i_3+2][i_1][i_2] * A[i_2+2][i_1][i_3+1][i_2] + A[i_0+2][i_3][i_2-2][i_0] * 3;
                }
            }
        }
        for (int i_4 = max(3, i_0); i_4 < -i_0+PB_M-1; i_4++) {
            B[i_4+i_0+1][i_4-1] = H[i_4][0] - I[i_4][i_0] + I[i_0][i_0+1] - B[i_4+i_0-1][i_4-3] + B[i_4+i_0+1][i_4] * 3;
        }
    }
    for (int i_5 = 4; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 2; i_6 < -i_5+PB_M-1; i_6++) {
            C[i_6][i_5] = H[i_6-2][i_5] - I[i_5][i_6+i_5] + I[i_6][i_5-2] - I[i_6+2][i_5] - B[i_6+i_5+1][i_5-1] * C[i_6+2][i_5] - 4;
        }
        for (int i_7 = 3; i_7 < -i_5+PB_M+1; i_7++) {
            D[i_7+i_5-2][i_7-1] = H[i_5][i_7] * J[i_5-1] * J[i_7+2] * H[i_7+1][i_5-1] - H[0][i_5] * K[i_5] + C[i_5+1][i_7] - D[i_7+i_5-1][i_7] - D[i_7+i_5-1][i_7-1] - D[i_7+i_5-4][i_7-3] + 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4414252323_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
