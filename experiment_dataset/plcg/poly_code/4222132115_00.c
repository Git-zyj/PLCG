...
void kernel_4222132115_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_M, -i_1+PB_M), PB_L); i_2++) {
                A[i_2][i_2+2][i_0-1] = C[i_2+i_1][i_0-1][i_0] - C[i_1][i_2][i_0] * C[i_2+i_0][i_1][0] + A[i_2+1][i_2+3][i_0-2] * 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = PB_L - 1; i_4 >= max(2, i_3); i_4--) {
            for (int i_5 = 0; i_5 < -i_4+PB_L; i_5++) {
                A[i_5][i_5+2][i_3-1] = C[i_5][i_3][i_4-2] + C[0][i_3][i_5+i_4] + C[i_3][i_5][0] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4222132115_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
