...
void kernel_2426112225_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0][i_0+1] + C[i_0-1] - 4;
        A[i_0] = 6;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        C[i_1-1] = C[i_1-2] - C[i_1+1] + 3;
    }
    for (int i_2 = 4; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_L-1; i_3++) {
            D[i_3+i_2-1][i_2-2] = D[i_3+i_2-3][i_2-4] - D[i_3+i_2+1][i_2] * D[i_3+i_2+1][i_3-2] * 6;
        }
    }
#pragma endscop
...
kernel_2426112225_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
