...
void kernel_4212252111_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-2; i_1++) {
            A[i_1+1][i_0] = C[i_1][i_0] * D[i_1-2][i_0] + D[i_0][i_1] - C[i_0][i_1] + C[i_1+2][i_0-2] * E[i_1] - A[i_1+2][i_0] - 4;
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                B[i_0][i_1][i_2] = F[i_2][i_2+2][i_1-1] * G[i_1][i_0+1][0] * D[i_2+1][i_1+2] * B[i_0][i_1][i_2-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4212252111_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
