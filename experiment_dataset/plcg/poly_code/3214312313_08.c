...
void kernel_3214312313_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 3; i_1 < min(min(i_0, -i_0+PB_P), PB_L-2); i_1++) {
            A[i_0-2][i_1+i_0] = C[i_1+2][i_0] + A[i_1-3][i_1+i_0] * B[i_0-1][i_1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L-1, -i_2+PB_P); i_3++) {
            B[i_2][i_3] = D[i_3+1][0] + A[i_3-2][i_3+i_2] * B[i_3-1][i_2] - B[i_2-1][i_3+1] - B[i_2][i_3-1] * 2;
        }
    }
#pragma endscop
...
kernel_3214312313_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
