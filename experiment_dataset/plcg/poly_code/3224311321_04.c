...
void kernel_3224311321_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_N; i_1++) {
            B[i_1-1] = C[i_1+i_0][i_0] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            B[i_2+1] = C[i_2][i_3] * B[i_3] + B[i_3+2] * B[i_3+3] - 2;
        }
    }
#pragma endscop
...
kernel_3224311321_04(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
