...
void kernel_2434331221_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        B[i_0-1] = E[i_0] * F[i_0][i_0+1] + 3;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            B[i_2-1] = G[i_2][i_1] - H[i_2+1][i_1-1] * B[i_2+1] + 6;
        }
        for (int i_3 = i_1; i_3 < -i_1+PB_L; i_3++) {
            C[i_3][i_1-1] = I[i_3-1] * H[i_3][i_3+i_1] * I[i_3+i_1] - C[i_3+1][i_1-1] - 1;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_M; i_5++) {
            D[i_5-1][i_4] = H[i_5+1][i_4+1] * E[i_5] - D[i_5-2][i_4+1] - D[i_5-1][i_4+2] - 4;
        }
    }
#pragma endscop
...
kernel_2434331221_02(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
