...
void kernel_2236112313_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_N+1; i_1++) {
            B[i_1-2] = C[i_1-1][i_0] * 6;
        }
    }
    for (int i_2 = 3; i_2 < PB_N+1; i_2++) {
        B[i_2-2] = B[i_2-1] - B[i_2-3] - B[i_2-3] - 6;
    }
#pragma endscop
...
kernel_2236112313_01(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
