...
void kernel_2326131211_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_1+1][i_0+1] = D[i_1] + E[i_1][i_0] + A[i_0+1][i_1+1][i_0+2] - A[i_0][i_1+2][i_0+1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_2+1][i_3] = E[i_3][i_3] + B[i_2][i_3] - 6;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M; i_5++) {
            C[i_4][i_5] = E[i_5][i_4+1] - E[i_4][i_5] + E[i_5][i_4-1] * E[i_5-1][i_4] * C[i_4][i_5-1] * C[i_4+1][i_5+1] - 6;
        }
    }
#pragma endscop
...
kernel_2326131211_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
