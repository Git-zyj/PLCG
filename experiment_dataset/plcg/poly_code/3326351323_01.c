...
void kernel_3326351323_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] + D[i_0] - E[i_0] - F[i_0] * E[i_0] + E[1] + F[i_0] + F[i_0] * G[i_0-1][0] - F[i_0+1] * A[i_0+2] - A[i_0+1] * 1;
        for (int i_1 = 2; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P-1; i_2++) {
                B[i_0][i_1][i_2+i_1+1] = H[i_1][i_0][i_2] - I[i_2][i_0][i_1] * I[i_2][i_0][0] - H[i_1][i_0][i_2] * B[i_0][i_1-2][i_2+i_1-1] - B[i_0+1][i_1+2][i_2+i_1+1] + 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M, PB_L); i_4++) {
            A[i_4+2] = I[i_4+i_3][i_4][0] - A[i_4-2] - 4;
        }
    }
#pragma endscop
...
kernel_3326351323_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
