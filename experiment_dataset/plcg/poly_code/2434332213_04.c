...
void kernel_2434332213_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0] + F[i_0][0] + E[i_0-2] + G[i_0] - 3;
    }
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_P-2; i_2++) {
            B[i_1][i_1] = H[i_2][i_2+2] * B[i_1-1][i_1-1] * B[i_1+1][i_1+1] * 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
        D[i_3+1] = I[i_3] * I[i_3] - A[i_3+1] * D[i_3+1] * 4;
        D[i_3+1] = I[i_3] * I[i_3+2] - I[i_3] - I[i_3] + D[i_3] + 6;
    }
#pragma endscop
...
kernel_2434332213_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
