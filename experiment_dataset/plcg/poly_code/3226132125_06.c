...
void kernel_3226132125_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_M, -i_0+PB_N); i_2++) {
                B[i_2-1] = C[i_0+2][i_1][0] * C[i_2+i_0][i_1-1][i_0] + 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M, i_3); i_4++) {
            B[i_4-1] = D[i_4+i_3][0] * D[i_3][i_4+i_3-2] + D[i_3+1][i_4] * B[i_4-2] * 1;
        }
    }
#pragma endscop
...
kernel_3226132125_06(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
