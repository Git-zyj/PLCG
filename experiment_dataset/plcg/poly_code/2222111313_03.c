...
void kernel_2222111313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_Q, i_0); i_1++) {
            A[i_1][i_1+i_0-1] = A[i_1-1][i_1+i_0-3] - A[i_1][i_1+i_0] * A[i_1+1][i_1+i_0] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q+2; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_Q+2; i_3++) {
            A[i_3-1][i_3+i_2-2] = C[i_3+i_2][0] + 1;
        }
    }
#pragma endscop
...
kernel_2222111313_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
