...
void kernel_2426252215_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_1] = E[i_1][i_1] * F[i_1][i_0] + E[i_1][i_0-2] + E[i_1-2][i_0+2] * A[i_1-1] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2] = G[0] - B[i_2+1] - C[i_2] + C[i_2-1] * 1;
        C[i_2] = G[i_2] + H[i_2] * G[i_2] * G[i_2] - H[i_2] - A[i_2-1] * B[i_2] - 1;
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M; i_4++) {
            B[i_4] = F[i_4][i_3-2] - H[0] - F[i_4][i_3] - H[i_4] * F[i_4+i_3][0] + F[i_4][i_3] + F[i_3+2][i_4+1] + F[i_3+2][i_4] * E[i_4+i_3][0] - 3;
        }
    }
#pragma endscop
...
kernel_2426252215_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
