...
void kernel_2236132313_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            B[i_0-1] = C[i_1] + D[i_1+2][i_0-1] * B[i_1] + B[i_1-1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(min(PB_N, i_2), -i_2+PB_M); i_3++) {
            B[i_3] = D[i_2][i_3] - D[i_3+i_2][2] * D[i_2][i_3] * B[i_3-1] - 6;
        }
    }
#pragma endscop
...
kernel_2236132313_08(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
