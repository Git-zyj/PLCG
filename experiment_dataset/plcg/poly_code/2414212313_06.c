...
void kernel_2414212313_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M+2, PB_L+1); i_1++) {
            A[i_0+2][i_1-2] = E[i_1+i_0-2][i_1-2] * A[i_0+3][i_1-2] * A[i_0+2][i_1-1] * C[i_0][i_1+1] + 1;
        }
        for (int i_2 = 1; i_2 < i_0; i_2++) {
            C[i_0+1][i_2] = E[i_0][i_0] + C[i_2+1][i_0] + 4;
            C[i_0+1][i_2] = C[i_0+1][i_2-1] - 4;
        }
        D[i_0-2] = D[i_0-1] * D[i_0-3] + 3;
    }
#pragma endscop
...
kernel_2414212313_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
