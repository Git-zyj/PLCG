...
void kernel_2224152321_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = C[i_0] + C[i_0] * C[i_0+1] * C[i_0-2] * D[i_0+1][i_0] * C[i_0-2] - A[i_0+1] + A[i_0-1] * A[i_0+2] + 4;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        B[i_1] = C[i_1+2] - C[i_1] * C[i_1-1] * A[i_1-2] + B[i_1-1] - 6;
    }
#pragma endscop
...
kernel_2224152321_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
