...
void kernel_2316232121_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_1+1] = D[i_1] * D[i_1] - E[i_0][i_1+1] + A[i_0+2] * 3;
            B[i_1][i_0+2] = E[i_1][0] * B[i_1][i_0] * 4;
        }
        A[i_0] = D[i_0] * D[i_0+2] + D[i_0-1] + 1;
    }
#pragma endscop
...
kernel_2316232121_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
