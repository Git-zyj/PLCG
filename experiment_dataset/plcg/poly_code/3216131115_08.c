...
void kernel_3216131115_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_Q-1 - 1; i_0 >= 1; i_0--) {
        for (int i_1 = i_0; i_1 < min(PB_L, -i_0+PB_N); i_1++) {
            A[i_0][i_1] = C[i_1][i_0] + D[i_1+i_0] - C[i_1][i_0+1] * D[1] * A[i_0-1][i_1] + 4;
        }
        for (int i_2 = i_0; i_2 < PB_L; i_2++) {
            A[i_0][i_2] = C[i_2][i_0] * 1;
        }
    }
#pragma endscop
...
kernel_3216131115_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
