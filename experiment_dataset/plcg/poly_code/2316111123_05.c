...
void kernel_2316111123_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = PB_L-3 - 1; i_1 >= 0; i_1--) {
        for (int i_2 = 2; i_2 < min(i_1, -i_1+PB_M-4); i_2++) {
            A[i_2] = A[i_2-2] - 1;
            C[i_2+i_1][i_1+1] = C[i_2+i_1+4][i_1+3] + 2;
        }
    }
#pragma endscop
...
kernel_2316111123_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
