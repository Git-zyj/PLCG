...
void kernel_3412211213_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        B[i_0-1] = B[i_0] + 3;
        B[i_0-1] = C[i_0+1] * 2;
        C[i_0] = E[i_0+1][1] * B[i_0] * 3;
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            D[i_1-1][i_0] = E[i_1-1][i_0] - C[i_1] - 1;
        }
    }
#pragma endscop
...
kernel_3412211213_03(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
