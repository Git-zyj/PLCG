...
void kernel_4434112211_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 4; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_L+2; i_2++) {
                A[i_1][i_2+i_1-2] = A[i_1-1][i_2+i_1-4] - A[i_2-1][i_2+i_1-3] - 1;
            }
        }
    }
    ...
    for (int i_8 = 0; i_8 < PB_L; i_8++) {
        for (int i_9 = 1; i_9 < min(PB_N-1, -i_8+PB_N); i_9++) {
            D[i_9] = E[i_9+i_8][i_8] + D[i_9+1] + D[i_9-1] + 6;
        }
    }
#pragma endscop
...
kernel_4434112211_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
