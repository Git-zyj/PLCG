...
void kernel_4324312321_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 3; i_2 < min(PB_L, -i_1+PB_M-2); i_2++) {
            B[i_2-2][i_2+i_1] = D[i_2+i_1][i_1+2][0] * B[i_2-2][i_2+i_1+2] - B[i_2-3][i_2+i_1+1] + B[i_2-1][i_2+i_1+2] - B[i_2][i_2+i_1+2] * 6;
        }
        C[i_1+1] = C[i_1+3] + C[i_1+2] + C[i_1-1] + C[i_1] + 2;
    }
#pragma endscop
...
kernel_4324312321_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
