...
void kernel_3412131325_05(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
    ...
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        ...
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q, PB_M); i_1++) {
            for (int i_2 = PB_M - 1; i_2 >= 0; i_2--) {
                B[i_0][i_1][i_2+1] = F[i_2][i_0-1][0] + F[i_1-1][i_0][i_2] * G[i_1][i_2][i_0+1][i_1+i_0] - B[i_0][i_1-1][i_2+1] - B[i_0-1][i_1-2][i_2+1] - B[i_0+2][i_1][i_2+1] + B[i_0+1][i_1-2][i_2+1] * 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        D[i_3+1] = E[i_3] * E[i_3] * E[i_3] - D[i_3+1] * 6;
        D[i_3] = E[i_3] + E[i_3] * E[i_3+1] + E[i_3] * E[i_3] - D[i_3-2] + 5;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3412131325_05(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
