...
void kernel_4436252213_06(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_M-2); i_1++) {
            C[i_1] = E[i_1] + F[i_1][i_0] * E[i_1] + F[i_1+2][i_0] * D[i_1-2][i_0+1] - D[i_0-1][i_1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2] = G[i_2] - E[i_2] * G[i_2] - E[i_2+1] - E[i_2] + G[i_2] * E[i_2] * C[i_2] * C[i_2+1] - 5;
        C[i_2] = G[i_2] * G[i_2] * G[i_2] + G[i_2+1] * E[i_2-1] * B[i_2] - B[i_2+1] + 4;
    }
    for (int i_3 = 2; i_3 < PB_M+2; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_M+1, i_3); i_4++) {
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                D[i_4-1][i_5] = H[i_3-2][i_3][i_5] - H[i_5][i_3][i_3] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436252213_06(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
