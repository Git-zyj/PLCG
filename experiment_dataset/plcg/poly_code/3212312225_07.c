...
void kernel_3212312225_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L-3, PB_M-1); i_1++) {
            A[i_1][i_1+i_0] = A[i_1+1][i_1+i_0+1] * A[i_1+1][i_1+i_0+3] - 4;
        }
    }
    for (int i_2 = 4; i_2 < PB_P; i_2++) {
        B[i_2-2][i_2] = C[i_2+1][i_2] - B[i_2-4][i_2-2] + 2;
    }
#pragma endscop
...
kernel_3212312225_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
