...
void kernel_4222112315_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = C[i_0] + A[i_0+1] * A[i_0-1] - A[i_0-1] * A[i_0+1] + A[i_0+1] - 5;
        for (int i_1 = 0; i_1 < -i_0+PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q-2; i_2++) {
                for (int i_3 = 1; i_3 < -i_2+PB_Q-2; i_3++) {
                    B[i_2][i_2][i_3] = D[i_1][i_2][i_3+i_2+2][i_1+i_0] * B[i_2-1][i_2-1][i_3-1] - 4;
                }
            }
        }
    }
...
kernel_4222112315_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D));
