...
void kernel_4226212113_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < PB_L; i_5++) {
            B[i_4][i_5] = C[i_5-1] + B[i_4][i_5-1] - 5;
        }
    }
#pragma endscop
...
kernel_4226212113_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
