...
void kernel_2212151123_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P, -i_0+PB_M+1); i_1++) {
            B[i_1][i_0+1] = C[i_1+i_0-1][i_1] * C[i_1+i_0-1][i_0+1] * D[i_0+1] + D[i_1-1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_P-1, -i_2+PB_M); i_3++) {
            B[i_2][i_3+1] = C[i_2][i_3+1] * E[i_3][i_3][1] - D[i_2] + C[i_2][i_3+1] * C[i_3+i_2][1] - D[i_3+i_2] + B[i_2-1][i_3+1] * 4;
        }
    }
#pragma endscop
...
kernel_2212151123_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
