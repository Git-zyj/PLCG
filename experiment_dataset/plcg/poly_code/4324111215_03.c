...
void kernel_4324111215_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0][i_0] - A[i_0+1] * A[i_0-1] - A[i_0+1] - 5;
    }
    for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
                B[i_3-1][i_2][i_1+1] = B[i_3-1][i_2-1][i_1+2] * B[i_3-1][i_2+1][i_1] * 2;
            }
        }
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            for (int i_5 = i_1; i_5 < -i_4+PB_P; i_5++) {
                C[i_1][i_5+i_4][i_5+i_4] = 3;
            }
        }
    }
#pragma endscop
...
kernel_4324111215_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
