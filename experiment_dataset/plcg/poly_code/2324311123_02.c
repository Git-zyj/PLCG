...
void kernel_2324311123_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_L, -i_2+PB_M-2); i_3++) {
            C[i_3+i_2+1][i_3-1] = D[i_3-1] - D[i_2] + C[i_3+i_2+2][i_3] + C[i_3+i_2-1][i_3-1] - 1;
        }
    }
#pragma endscop
...
kernel_2324311123_02(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
