...
void kernel_2312112315_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M; i_2++) {
            B[i_1][i_1-2] = D[i_2][i_1-1] + A[i_2] * C[i_1][i_2] * C[i_1][i_2+1] * C[i_1][i_2-1] - C[i_1+1][i_2-1] * 2;
            C[i_1][i_2] = D[i_1][i_1] - B[i_1][i_1-2] - C[i_1][i_2-1] - C[i_2][i_1-1] + 4;
        }
    }
#pragma endscop
...
kernel_2312112315_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
