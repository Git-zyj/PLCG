...
void kernel_4224212225_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M; i_2++) {
                A[i_1][i_2-1] = A[i_0-1][i_2-1] + A[i_2][i_1-2] + 4;
            }
        }
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            A[i_3+2][i_0-1] = C[i_3][i_0] + 3;
        }
    }
#pragma endscop
...
kernel_4224212225_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
