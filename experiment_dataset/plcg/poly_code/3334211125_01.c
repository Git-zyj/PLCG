...
void kernel_3334211125_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L; i_2++) {
                A[i_1][i_2][i_2] = A[i_1-1][i_2][i_2] + 3;
            }
        }
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                C[i_4][i_3+2][i_0] = D[i_3+1][i_4][i_0] * 4;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_Q-1; i_5++) {
        for (int i_6 = 1; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < i_5; i_7++) {
                C[i_7][i_6][i_5+1] = C[i_7][i_6-1][i_5+1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3334211125_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
