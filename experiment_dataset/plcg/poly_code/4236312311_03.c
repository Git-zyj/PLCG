...
void kernel_4236312311_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = max(2, i_1); i_2 < min(PB_P+1, -i_0+PB_N); i_2++) {
                A[i_0+1][i_1-1][i_2-2] = C[i_2+i_0][i_1][i_1] + A[i_0+1][i_1-1][i_2-1] * A[i_2+1][i_1-1][i_0-1] - B[i_1+1][i_0] + 1;
            }
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[i_0][i_3] = D[i_3] + B[i_0][i_3-1] - B[i_0-1][i_3] + B[i_0][i_3+1] + 4;
        }
    }
#pragma endscop
...
kernel_4236312311_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
