...
void kernel_3234312313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+2] = C[i_0] + B[i_0] * B[i_0-1] - B[i_0+1] + 4;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_1] = A[i_1+1] * A[i_1+2] * B[i_1+1] - 3;
        }
    }
#pragma endscop
...
kernel_3234312313_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
