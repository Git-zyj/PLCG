...
void kernel_3224311223_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_Q-1; i_2++) {
                A[i_1][i_0][i_2] = A[i_1][i_0][i_2-2] + A[i_1+1][i_0-1][i_2+1] + 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
                A[i_5][i_3+2][i_4] = C[i_5+1][i_3][i_4+1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3224311223_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
