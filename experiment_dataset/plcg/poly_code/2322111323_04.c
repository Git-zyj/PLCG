...
void kernel_2322111323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = A[i_0-3] + A[i_0+1] + A[i_0] * 2;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_M; i_2++) {
            B[i_1-1][i_2] = D[i_2+i_1][0] - B[i_1][i_2-2] - B[i_1][i_2] + C[i_1][i_2] * 5;
            C[i_1][i_2] = D[i_1][i_2+1] * D[i_2][i_1-1] * C[i_1][i_2+2] + C[i_1-2][i_2+1] + C[i_2+2][i_1] * 4;
        }
    }
#pragma endscop
...
kernel_2322111323_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
