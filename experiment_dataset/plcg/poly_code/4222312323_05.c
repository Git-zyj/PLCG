...
void kernel_4222312323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        A[i_0] = C[i_0-2] * A[i_0+2] * A[i_0-1] * 2;
    }
    for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = i_1; i_3 < PB_N-2; i_3++) {
                B[i_3][i_2][i_3] = B[i_1+2][i_3][i_1+2] - B[i_3+2][i_2-2][i_3+2] * B[i_3][i_2-1][i_3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4222312323_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
