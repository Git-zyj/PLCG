...
void kernel_2334212311_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < i_0; i_1++) {
            A[i_1+1][i_0] = A[i_1+1][i_0-1] * B[i_1-3] - 3;
        }
    }
    ...
    for (int i_3 = 2; i_3 < PB_M+1; i_3++) {
        for (int i_4 = 3; i_4 < PB_L; i_4++) {
            C[i_3-2][i_4-2] = D[i_4] * C[i_3-2][i_4-3] * C[i_3-2][i_4-1] - C[i_3-1][i_4-2] * 5;
        }
    }
#pragma endscop
...
kernel_2334212311_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
