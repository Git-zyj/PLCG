...
void kernel_4212111123_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P, -i_0+PB_L); i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_2+1][i_1+i_0][i_0][i_0+1] = C[i_2][i_0][i_1] - 6;
            }
        }
        for (int i_3 = 3; i_3 < PB_M; i_3++) {
            B[i_3][i_3-1] = B[i_3-2][i_3-3] * 6;
        }
    }
#pragma endscop
...
kernel_4212111123_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
