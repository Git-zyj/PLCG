...
void kernel_4424232325_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0] = E[1] * F[i_0] * F[i_0-2] * A[i_0+2] * A[i_0+1] + 5;
        for (int i_1 = 4; i_1 < min(min(PB_N-1, -i_0+PB_M-1), i_0); i_1++) {
            B[i_0][i_1-2] = G[i_1+1][i_0] * H[i_0+2][i_0-2] - F[i_1+i_0+1] + B[i_0][i_1-4] - B[i_0][i_1] + B[i_0-2][i_1-2] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
        A[i_2] = E[i_2-2] + E[i_2+2] * 2;
        A[i_2+2] = E[i_2] * 4;
    }
#pragma endscop
...
kernel_4424232325_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
