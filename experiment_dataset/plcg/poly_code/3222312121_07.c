...
void kernel_3222312121_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_L, PB_P+2); i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_1][i_2] = C[i_2][i_1+i_0][i_0] + A[i_2][i_1-2] * 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N; i_3++) {
        B[i_3-2] = D[i_3][i_3] - B[i_3] + 5;
    }
#pragma endscop
...
kernel_3222312121_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
