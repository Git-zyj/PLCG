...
void kernel_4434252125_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        A[i_0] = E[i_0] + E[i_0+1] + E[i_0] + E[i_0] - F[i_0-1] * E[i_0] - G[i_0+2][0] * A[i_0+1] * A[i_0-2] + 3;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_P); i_2++) {
            B[i_2][i_1] = H[i_1][i_2+i_1] - F[i_2] - G[i_2][i_1] * G[i_2+i_1][1] - 1;
        }
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            A[i_3+1] = E[i_3] + H[i_1+1][i_3] * H[i_3][0] * 2;
        }
    }
    for (int i_4 = 0; i_4 < PB_P; i_4++) {
        for (int i_5 = i_4; i_5 < min(PB_L, -i_4+PB_M); i_5++) {
            A[i_5] = I[i_5+i_4][i_4][0] - E[i_5] - G[i_5][i_4] - 4;
        }
    }
#pragma endscop
...
kernel_4434252125_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
