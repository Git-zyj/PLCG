...
void kernel_2336112323_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_0][i_1+1] = D[i_1] - A[i_0][i_1] + 4;
            B[i_1][i_0] = D[i_0+1] + B[i_1-1][i_0] - 1;
        }
    }
    for (int i_2 = 3; i_2 < PB_L-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-3; i_3++) {
            C[i_3+2][i_2-1][i_2] = E[i_3][i_2+1] - B[i_3][i_2] * C[i_3+3][i_2][i_2+1] + C[i_3+2][i_2-3][i_2-2] * C[i_3+2][i_2][i_2+1] * C[i_3+1][i_2+1][i_2+2] - 5;
        }
    }
#pragma endscop
...
kernel_2336112323_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
