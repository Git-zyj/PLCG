...
void kernel_3216312311_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0][i_1+2] = C[i_1] - A[i_0+1][i_1+2] - A[i_0][i_1+3] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, PB_M-1); i_3++) {
            A[i_3+1][i_2+2] = C[i_3+1] + A[i_3][i_2+1] * 5;
        }
    }
#pragma endscop
...
kernel_3216312311_05(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
