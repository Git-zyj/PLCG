...
void kernel_4426231325_06(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_Q-1, i_0); i_2++) {
                for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
                    C[i_3-1][i_1][i_3] = E[i_3-1][i_1][i_0][i_2+1] * 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = i_4; i_5 < -i_4+PB_M-1; i_5++) {
            D[i_5+i_4+1][i_4-1] = F[i_5][i_4+1] + G[i_5][i_4+1] - G[i_4][i_5] - G[i_4-1][i_5] * C[i_5-1][i_4-1][i_5] * 6;
        }
    }
    for (int i_6 = 3; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 1; i_7 < i_6; i_7++) {
            for (int i_8 = 2; i_8 < PB_M; i_8++) {
                C[i_8-1][i_7][i_8] = H[i_8][i_6][i_7-1] + H[i_6][i_8][i_7] * I[i_6][i_7][i_8] * C[i_8][i_7][i_8+1] - C[i_8-2][i_7][i_8-1] * 3;
            }
        }
        for (int i_9 = i_6; i_9 < -i_6+PB_M; i_9++) {
            D[i_9+i_6][i_9-1] = F[i_9+i_6+1][i_6] * F[i_9][i_6] - F[i_6][i_9] - F[i_9][i_6+1] * D[i_9+i_6][i_6-3] + D[i_9+i_6-1][i_9] - D[i_9+i_6-2][i_9-1] + 5;
        }
    }
#pragma endscop
...
init_array(xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4426231325_06(xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
