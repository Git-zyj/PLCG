...
void kernel_4236111115_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_M-1); i_5++) {
            B[i_5+i_4+1][0] = C[i_5][0] * B[i_5+i_4][0] - 3;
        }
    }
#pragma endscop
...
kernel_4236111115_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
