...
void kernel_2214132111_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0][i_0] = C[i_0] + C[i_0-1] * C[i_0+1] + C[1] * C[i_0] * A[i_0+1][i_0+1] * 4;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            A[i_1+1][i_1+1] = C[i_2+i_1] * 3;
        }
    }
#pragma endscop
...
kernel_2214132111_00(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
