...
void kernel_4412352321_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl),int xm,int ym,int zm,int vm,int wm,DATA_TYPE POLYBENCH_5D(M,xM,yM,zM,vM,wM,xm,ym,zm,vm,wm),int xn,int yn,int zn,int vn,DATA_TYPE POLYBENCH_4D(N,xN,yN,zN,vN,xn,yn,zn,vn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = E[0] + F[i_0] * G[i_0][2] + A[i_0-1] * A[i_0-2] + 1;
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, -i_0+PB_L-1); i_2++) {
                B[i_2+2][i_0-2][i_1] = H[i_1][i_2][i_0] * I[i_2+i_1][i_0] * J[i_2][i_0+2][i_1] * H[i_0][i_2][i_2] + H[i_1-2][i_2+i_0+1][i_2+2] + H[i_2+i_1][i_0+2][i_1] + B[i_2+4][i_0-2][i_1] * 6;
            }
        }
        for (int i_3 = 4; i_3 < PB_P-1; i_3++) {
            for (int i_4 = 4; i_4 < PB_M-2; i_4++) {
                C[i_4-2][i_3-2][i_0] = G[i_4+2][i_3] - B[i_4+2][i_0-2][i_3+1] - B[i_4][i_0-2][i_3+1] - C[i_4-4][i_3-2][i_0] * C[i_4-2][i_3-3][i_0] + C[i_4-2][i_3-2][i_0+2] + C[i_4-2][i_3-4][i_0-1] * 6;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_P-2; i_5++) {
        for (int i_6 = 2; i_6 < PB_L; i_6++) {
            for (int i_7 = 2; i_7 < PB_L-2; i_7++) {
                for (int i_8 = max(1, i_5); i_8 < min(min(min(-i_7+PB_M, -i_5+PB_L), -i_6+PB_P+2), PB_L-1); i_8++) {
                    D[i_5+2][i_7+2][i_8][i_7] = K[i_8][i_5][i_8-1][i_7+2] * L[i_8][i_5+2][i_7-2][i_6+2] + K[i_5][i_6-1][i_8][i_7] * M[i_5][i_6][i_8+i_5][i_7][i_6] + K[i_7][i_5][i_8-1][i_6] * L[i_7][i_8][i_8+i_6-2][i_5] + N[i_8+i_7][i_7][i_5+2][i_6] + K[i_7-1][i_8+1][i_5+1][0] + K[i_6+1][i_7+1][i_8+i_5][i_5] + C[i_7-2][i_6-1][i_8] + D[i_5+1][i_7+2][i_8][i_7] - 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4412352321_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,POLYBENCH_ARRAY(N));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,POLYBENCH_ARRAY(N)));
