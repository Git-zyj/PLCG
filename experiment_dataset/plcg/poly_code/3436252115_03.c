...
void kernel_3436252115_03(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                D[i_2] = E[i_1+i_0][i_2][0] + F[i_0][i_1+i_0+2][i_2] * 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M+2, i_3); i_4++) {
            D[i_4+1] = G[i_3][i_4] + H[i_4+1] * I[i_3-2][i_4-2] * F[i_3+1][i_4+2][0] - G[i_3][i_4-2] + G[i_4+i_3-2][i_4] - G[i_4][i_3-1] * 3;
        }
    }
    for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 0; i_6 < PB_P; i_6++) {
            for (int i_7 = i_5; i_7 < min(min(-i_6+PB_L-2, PB_L-2), -i_5+PB_M+1); i_7++) {
                D[i_6] = I[i_7][i_5] + G[i_7][i_5+1] + E[i_7+i_6+2][i_6][i_5] - I[i_7+i_5-1][i_6+1] * J[i_5][i_7+2][i_7][i_6] - 6;
            }
        }
        D[i_5] = H[i_5+1] + H[i_5+1] * H[i_5] * H[i_5] + D[i_5-1] - 5;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3436252115_03(xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J)));
