...
void kernel_3332231313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_N; i_2++) {
            A[i_1] = F[i_1][i_1] - A[i_2] * 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_M; i_5++) {
                C[i_5+1][i_3][i_4] = G[i_5][i_3][i_4+1] + G[i_5+i_4][i_3+1][1] + H[i_5][i_3-1][i_4] * G[i_5-1][i_4][i_3-1] * C[i_3][i_4][i_5] * C[i_5][i_4][i_3] * C[i_5+2][i_3][i_4+1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3332231313_00(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
