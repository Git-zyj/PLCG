...
void kernel_3232311211_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_1][i_0][i_1+1] = A[i_1][i_0+1][i_1+1] + A[i_0][i_1+1][i_0+1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 0; i_3 < min(min(PB_Q-1, i_2), -i_2+PB_L-1); i_3++) {
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                B[i_2][i_3+i_2][i_3] = C[i_2+1][i_3][0] - B[i_2][i_3+i_2+1][i_3+1] + B[i_3+1][i_3+i_2+1][i_2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3232311211_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
