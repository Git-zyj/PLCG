...
void kernel_3412252113_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        A[i_0-2] = E[i_0-1] - F[0] * G[i_0+1][0] + E[i_0] + E[i_0] * E[i_0+1] * E[0] * A[i_0-3] + 1;
        for (int i_1 = 1; i_1 < min(PB_N-2, i_0); i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_M, PB_N-1); i_2++) {
                B[i_2+i_0][i_0][i_1] = H[i_2][i_1-1][0] * I[i_2+2][i_0][0] + I[i_2][i_0][i_1-1] - H[i_0][i_0][i_1] * H[i_2][i_2-2][i_0] + H[i_2][i_1+2][i_0+1] + H[i_2-2][i_2+1][i_1-1] * I[i_1][i_2+2][i_0] - H[i_2][i_1][i_0] - B[i_2+i_0-1][i_0][i_1] - 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N+2; i_3++) {
        for (int i_4 = 0; i_4 < PB_P-2; i_4++) {
            for (int i_5 = 0; i_5 < min(PB_L, -i_4+PB_M); i_5++) {
                C[i_5][i_4+2][i_5+2] = J[i_5+i_4][i_3] + 1;
            }
        }
        A[i_3-1] = F[i_3-2] + 5;
    }
...
kernel_3412252113_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
