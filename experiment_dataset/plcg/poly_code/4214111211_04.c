...
void kernel_4214111211_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        ...
        for (int i_2 = 1; i_2 < PB_Q; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_2+PB_Q, i_2); i_3++) {
                B[i_2][i_0][i_3+i_2] = C[i_2][i_0-1][i_3] * B[i_2][i_0][i_3+i_2-1] + B[i_2][i_0-1][i_3+i_2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4214111211_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
