...
void kernel_4312252325_03(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q, PB_L-2); i_1++) {
            for (int i_2 = max(2, i_0); i_2 < -i_0+PB_M-2; i_2++) {
                for (int i_3 = 0; i_3 < -i_2+PB_M-2; i_3++) {
                    C[i_1][i_3+i_2+2][i_0][1] = D[i_0][i_2+2][i_1-2][i_2-2] + E[i_3+2][i_0+2][i_2][i_0] * F[i_2-1][i_3+i_1][i_1] * E[i_1][i_2+i_0][i_3][i_1-1] + C[i_0][i_3+i_2+1][i_1-1][1] + 5;
                }
                B[i_0][i_1+1][i_2] = G[i_2+i_0+2][i_1+2] * H[i_2+2][i_1+1] + I[i_2][i_1+1][i_0+1] * E[i_0][i_2+2][i_2][i_1+i_0] * I[i_2+i_0][i_1][i_2] * I[i_1+2][i_2][i_0] + F[i_0+1][i_2-1][i_1] * F[i_2+i_1][i_0][i_0-1] + 6;
                for (int i_4 = i_0; i_4 < -i_1+PB_M-1; i_4++) {
                    C[i_0][i_4+i_1+1][i_2][1] = E[i_2][i_1][i_0-1][i_4+2] * I[i_0+2][i_4][i_1] * B[i_1][i_2+1][i_4] * B[i_1+1][i_2+1][i_4] + B[i_1][i_0+1][i_2+1] + B[i_1-2][i_2+3][i_4+2] + B[i_4][i_1+1][i_2+1] - 6;
                }
            }
        }
    }
...
init_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4312252325_03(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
