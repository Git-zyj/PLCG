...
void kernel_4336132121_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
        ...
        for (int i_1 = i_0; i_1 < PB_P-2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_Q, -i_1+PB_L-1); i_2++) {
                for (int i_3 = i_1; i_3 < -i_0+PB_M; i_3++) {
                    A[i_3][i_1][i_2][i_1-2] = D[i_3][i_2+i_1+1][i_1+2] * E[i_2][i_3+i_0][i_1][i_2+i_0] * E[i_3+1][i_2+2][i_0-2][i_1] - 3;
                }
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < i_4; i_5++) {
            for (int i_6 = 2; i_6 < min(i_4, -i_5+PB_L); i_6++) {
                B[i_6-2][i_5] = D[i_6+1][i_5-2][i_4] + D[i_4][i_6+i_5][i_4] + D[i_5][i_4-1][i_6] - D[i_5][i_4-2][i_6-2] + 3;
            }
        }
        for (int i_7 = 0; i_7 < min(i_4, PB_P-1); i_7++) {
            C[i_4-2][i_7] = C[i_4][i_7+1] - C[i_4-3][i_7] - 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), INIT_SEED);
kernel_4336132121_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E)));
