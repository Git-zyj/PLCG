...
void kernel_4322231213_01(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_M+1); i_1++) {
            C[i_1-1] = D[i_1+i_0-1][i_0-1] * E[i_1] + F[i_1+i_0][i_0][1] - G[i_1][i_0] * 4;
        }
        for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
            C[i_2] = D[i_2+1][i_0] - C[i_2-2] - 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        C[i_3-1] = H[i_3] * H[i_3] - E[i_3] - C[i_3-2] - 3;
    }
#pragma endscop
...
kernel_4322231213_01(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
