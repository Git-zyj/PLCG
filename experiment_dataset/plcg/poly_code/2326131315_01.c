...
void kernel_2326131315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[0] - D[i_0] * D[i_0] - D[i_0] + D[i_0] - D[i_0] + D[i_0] * A[i_0+1] - A[i_0-1] + 3;
        A[i_0-1] = D[i_0] * D[i_0] + 3;
    }
    for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            C[i_1+1][i_2] = C[i_1][i_2] - C[i_1+2][i_2] + C[i_1+2][i_2-1] + 3;
        }
    }
#pragma endscop
...
kernel_2326131315_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
