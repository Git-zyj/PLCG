...
void kernel_2214132221_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            B[i_1-1][i_0-2] = C[i_0] + D[i_0][i_1] - 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            B[i_3-1][i_2-2] = C[i_3] * D[i_3][i_3-2] + D[i_3+i_2][i_3] * B[i_2+1][i_3-2] + B[i_3+1][i_2-2] + 5;
        }
    }
#pragma endscop
...
kernel_2214132221_04(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
