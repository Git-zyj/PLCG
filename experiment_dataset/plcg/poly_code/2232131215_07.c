...
void kernel_2232131215_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, PB_M-1); i_1++) {
            B[i_0+1][i_1+i_0] = C[i_1-1][i_0-1] * C[i_1][i_0] - C[i_1+1][i_0] * C[i_1][i_1-1] - 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_L+1, PB_M); i_3++) {
            B[i_2+1][i_3+i_2-1] = C[i_3][i_2] + C[i_2][i_3-1] + B[i_2+1][i_3+i_2-2] + B[i_2][i_3+i_2-2] * 6;
        }
    }
#pragma endscop
...
kernel_2232131215_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
