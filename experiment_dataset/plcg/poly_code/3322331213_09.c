...
void kernel_3322331213_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0-1] * E[i_0] * D[i_0] - A[i_0+1] * C[i_0] + 5;
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1] = F[i_0+1][i_1-1] + E[0] + G[i_1][i_0+1] + A[i_1-1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        C[i_2] = E[i_2] + H[i_2-1] * H[0] - C[i_2+1] - 5;
    }
#pragma endscop
...
kernel_3322331213_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
