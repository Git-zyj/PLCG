...
void kernel_4424232323_08(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            D[i_1] = E[i_1][i_0-1] - E[i_1][i_0-2] + D[i_1+1] + 6;
        }
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
                D[i_3+1] = F[i_3][i_0+1] * E[i_3][i_0] * G[i_3-1][i_0][i_2] - F[i_2][i_0] * 5;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-3; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-3; i_6++) {
                D[i_6+3] = G[i_6+2][i_6+1][i_4] - 6;
            }
        }
        D[i_4+1] = H[i_4-2] - I[i_4-2] - H[i_4] * I[i_4] - D[i_4+3] - D[i_4] * 6;
    }
#pragma endscop
...
kernel_4424232323_08(xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
