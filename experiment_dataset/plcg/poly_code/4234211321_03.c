...
void kernel_4234211321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-3; i_1++) {
            A[i_1][i_1+1] = A[i_1-2][i_1-1] + A[i_1+1][i_1+2] + A[i_1-1][i_1] - A[i_1+2][i_1+3] + A[i_1+2][i_1+3] + A[i_1-1][i_1] * 2;
        }
    }
        ...
        for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_M-1, i_3); i_4++) {
                for (int i_5 = 1; i_5 < PB_Q+1; i_5++) {
                    B[i_3][i_5-1][i_2][i_4+1] = C[i_3-1][i_4][i_5-1][i_2] - 6;
                }
            }
        }
    }
...
kernel_4234211321_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
