...
void kernel_2324252323_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_P; i_1++) {
            A[i_0][i_1] = D[i_1+1][i_0] - E[i_0] + F[i_0-1][i_1-1] + D[i_1][i_0] + D[i_0][i_1+i_0] + D[i_1][i_0] + D[i_1-2][i_0] * D[i_1][i_0] + A[i_1-2][i_0-2] + A[i_0-2][i_1] * A[i_1-2][i_0+2] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            B[i_3][i_2] = D[i_3][0] * D[i_2][i_3] * D[i_3+i_2][i_2] - G[i_3][i_3][i_2-1] - B[i_2][i_3+2] - B[i_3][i_2+1] + 1;
        }
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            C[i_2][i_4] = D[i_2][i_4] * C[i_2+1][i_4] - C[i_2][i_4+1] + 5;
        }
    }
#pragma endscop
...
kernel_2324252323_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
