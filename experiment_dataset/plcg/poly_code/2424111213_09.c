...
void kernel_2424111213_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < min(min(-i_3+PB_M, PB_M-1), i_3); i_4++) {
            B[i_4+1] = E[i_4+i_3][0] * 1;
        }
    }
    for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
        D[i_5-1] = F[i_5] - F[i_5+1] - D[i_5-2] * 2;
    }
#pragma endscop
...
kernel_2424111213_09(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
