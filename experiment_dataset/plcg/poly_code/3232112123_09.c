...
void kernel_3232112123_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_Q; i_2++) {
                A[i_1+1][i_0][i_2+i_0] = A[i_1][i_0][i_2+i_0] - 5;
            }
        }
        B[i_0] = C[i_0-2] * C[i_0] + B[i_0+1] + 4;
    }
#pragma endscop
...
kernel_3232112123_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
