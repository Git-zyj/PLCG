...
void kernel_3414211221_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        ...
        for (int i_2 = 3; i_2 < PB_L; i_2++) {
            C[i_2] = E[i_1-1][i_2+1] - D[i_2][i_1+1] + D[i_1][i_2-3] + 3;
            D[i_1][i_2-1] = E[i_2][i_1-1] - D[i_1-2][i_2-2] - 2;
        }
    }
#pragma endscop
...
kernel_3414211221_03(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
