...
void kernel_3212152321_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = min(i_0, -i_0+PB_N) - 1; i_1 >= 2; i_1--) {
            A[i_0][i_1][1] = C[i_1+i_0][i_1-2] - D[i_1] * C[i_1-2][2] + D[i_1] * D[i_1] + C[i_0][0] + A[i_0+2][i_1][1] * 2;
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                A[i_0+1][i_1][1] = E[i_0][i_2][i_1] * E[i_1][i_2][i_0] * E[i_2][i_0][i_1-2] + A[i_0][i_1-2][1] * A[i_0][i_1][1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3212152321_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
