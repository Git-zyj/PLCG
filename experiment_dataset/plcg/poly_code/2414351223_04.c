...
void kernel_2414351223_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[1] = E[0] - F[i_0] - 4;
        for (int i_1 = i_0; i_1 < -i_0+PB_M-1; i_1++) {
            B[i_0-1][i_1] = G[i_1][i_0-1] - H[i_1+i_0+1][i_0-1] - I[i_1+1][i_0] + G[i_1][i_1] + I[i_1][i_0] + A[1] - A[1] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-3; i_2++) {
        C[i_2][i_2+1] = J[i_2+1] + J[i_2+1] - E[i_2] * C[i_2+2][i_2+3] - D[i_2+2] + 6;
        D[i_2] = F[i_2] * F[i_2] + E[i_2] - J[i_2] + F[i_2+1] + F[i_2-1] * F[i_2-1] + D[i_2-1] - 2;
    }
#pragma endscop
...
kernel_2414351223_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
