...
void kernel_2334252125_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M+1, PB_L-1); i_1++) {
            B[i_1+1] = D[i_0][i_1+1][i_0-1] - E[i_1][i_0-2] - F[i_1][i_0] + F[i_0][i_1] * G[i_1][i_0][i_1] * F[i_1+i_0-1][0] * E[i_1-1][i_0] * B[i_1+1] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        B[i_2+1] = H[i_2-1] + H[i_2] - I[i_2] + I[i_2+1] * 3;
        for (int i_3 = 0; i_3 < -i_2+PB_M; i_3++) {
            B[i_2+1] = I[i_2] * F[i_3+i_2][1] * E[i_3][i_2] + 2;
        }
    }
#pragma endscop
...
kernel_2334252125_09(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
