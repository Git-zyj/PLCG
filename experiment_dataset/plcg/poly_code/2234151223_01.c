...
void kernel_2234151223_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_0][i_1] = C[i_1][i_0] - D[i_1] + C[i_1][i_0-1] - D[1] - C[i_1+1][i_0] * B[i_0][i_1+1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            B[i_3][i_2] = C[i_3][i_2] * C[i_3-1][i_2+1] + C[i_3][i_2] + C[i_3][i_2+1] - C[i_3][i_2-1] + B[i_3-2][i_2] + 6;
        }
    }
#pragma endscop
...
kernel_2234151223_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
