...
void kernel_2312112225_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_M-2); i_1++) {
            A[i_1-2] = D[i_1+2][i_0+2] - A[i_1-1] - A[i_1] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
            B[i_2][i_3] = B[i_2][i_3+1] * B[i_2-2][i_3] * B[i_2-1][i_3] - 1;
        }
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            C[i_2-2][i_4] = C[i_2-1][i_4+2] + 1;
        }
    }
#pragma endscop
...
kernel_2312112225_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
