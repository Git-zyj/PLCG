...
void kernel_4414131323_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0] = E[i_0] + E[i_0+1] * E[i_0] - D[0] + D[0] - 4;
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            B[i_1][i_0] = F[i_1+1][i_0] * F[i_1][i_0-1] - A[i_1+2] * B[i_1][i_0+2] + D[0] * 5;
            for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
                B[i_1+1][i_2-1] = G[i_2-1][i_1+i_0][i_0-1] * G[i_0][i_2+i_1-1][i_1+1] * H[i_1][i_0][i_2+1][i_0] * B[i_1+2][i_2] * 3;
            }
        }
        D[0] = E[i_0] + E[i_0+1] + E[i_0-1] + A[i_0] * D[0] - 6;
    }
#pragma endscop
...
kernel_4414131323_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
