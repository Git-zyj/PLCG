...
void kernel_3222211225_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-3; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L-3; i_2++) {
                A[i_0][i_2+i_1+1][i_0] = A[i_0-2][i_2+i_1+3][i_0-2] + A[i_0][i_2+i_1+1][i_0] * A[i_0][i_2+i_1-1][i_0] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            B[i_4] = C[i_3][i_4][i_3+1] + B[i_4-1] * 3;
        }
    }
#pragma endscop
...
kernel_3222211225_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
