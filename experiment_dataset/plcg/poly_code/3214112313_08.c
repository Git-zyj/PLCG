...
void kernel_3214112313_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = PB_M+1 - 1; i_1 >= 1; i_1--) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                B[i_2-1][i_1][i_2-1] = C[i_2-1][i_0][i_1-1] * 6;
            }
            for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                B[i_3][i_1][i_3] = C[i_1-1][i_0][i_0] - B[i_3][i_1-1][i_3] - B[i_3+1][i_1][i_3+1] * B[i_1+1][i_3][i_1+1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3214112313_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
