...
void kernel_4122231223_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
        ...
        for (int i_1 = 1; i_1 < min(-i_0+PB_Q, PB_M-2); i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_Q-1, PB_M-1); i_3++) {
                    A[i_3][i_2][1] = B[i_3][i_0][i_1][i_1-1] - C[i_3+i_1-1][i_0][i_1+1][i_1+1] * C[i_0-1][i_1][i_3+i_2+1][i_1+i_0] * A[i_3+1][i_2][1] - A[i_1+2][i_3][1] + 1;
                }
            }
        }
    }
...
kernel_4122231223_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
