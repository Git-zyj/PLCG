...
static void print_array(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4432351221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,DATA_TYPE POLYBENCH_1D(N,xN,xn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_Q-1 - 1; i_0 >= 1; i_0--) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_1][i_0] = E[i_1-1] + F[i_1][i_0] * E[i_1+i_0] * G[i_1-1][i_0] - 6;
        }
        for (int i_2 = 3; i_2 < PB_Q; i_2++) {
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                for (int i_4 = 2; i_4 < min(PB_Q-2, -i_0+PB_L); i_4++) {
                    B[i_0][i_3][i_2-1][i_4] = H[i_4+1][i_3][i_2][i_0+1] - H[i_2][i_0-1][i_4][i_3] - H[i_3-1][i_2][i_0][i_4] * I[i_4][i_0][i_3][i_2] - J[i_4][i_2][i_3][i_0] - I[i_3][i_4+i_0][i_2][i_0+1] + J[i_3][i_3][i_2+1][i_0+1] - I[i_4][0][i_0][i_2-1] + B[i_0][i_3-1][i_2-1][i_4+2] * B[i_0][i_3][i_2][i_4] - B[i_0][i_3][i_2-3][i_4] * B[i_0][i_3][i_4-2][i_2-2] + B[i_4][i_3][i_2-2][i_0] - 6;
                }
            }
        }
        for (int i_5 = 1; i_5 < -i_0+PB_L; i_5++) {
            for (int i_6 = 0; i_6 < -i_0+PB_Q-1; i_6++) {
                C[i_6+i_5][i_5+i_0][i_0][i_6] = F[i_5-1][0] - K[i_6][i_5] - L[i_5-1][i_5][i_6+i_0+1] * M[i_6][i_5][i_0] - 6;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_N-1; i_7++) {
        D[i_7+1] = N[i_7] - E[i_7] - E[i_7] * D[i_7-1] - D[i_7] * 3;
    }
#pragma endscop
...
kernel_4432351221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,POLYBENCH_ARRAY(N));
