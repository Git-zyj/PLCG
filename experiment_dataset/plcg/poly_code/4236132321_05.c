...
void kernel_4236132321_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 3; i_1 < PB_M-2; i_1++) {
            A[i_1-2] = C[i_1] - D[i_1-2][i_1-1] + C[i_0+1] - E[i_0][i_1+2][0] - A[i_1-3] + A[i_1] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 4; i_4 < PB_M-2; i_4++) {
                B[i_3][i_4+1][i_2][i_2] = D[i_4][0] * D[i_4+2][i_3+2] + A[i_4-4] - A[i_4-2] - B[i_4+1][i_2][i_3-1][i_3-1] - B[i_3-1][i_4+1][i_2-2][i_2-2] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4236132321_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
