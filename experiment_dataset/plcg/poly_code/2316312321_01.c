...
void kernel_2316312321_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-3; i_3++) {
            B[i_2][i_3+1] = D[i_2][i_3] * D[i_2+1][2] + A[i_3-2] * B[i_2][i_3] - B[i_2+2][i_3+3] - 4;
        }
        for (int i_4 = max(3, i_2); i_4 < PB_M; i_4++) {
            C[i_4][i_2-1] = C[i_4-1][i_2-1] * C[i_2][i_4-3] - 2;
        }
    }
#pragma endscop
...
kernel_2316312321_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
