...
void kernel_3226231125_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P; i_1++) {
            A[i_1+1][i_1+i_0] = C[i_1] - D[i_0][1] * C[i_0] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M; i_3++) {
            for (int i_4 = i_2; i_4 < -i_2+PB_P; i_4++) {
                B[i_2+1][i_3][i_4+i_2] = E[i_2][i_4][i_3+i_2] - E[i_4][i_3+i_2][i_3+i_2+1] - B[i_2][i_3+1][i_4+i_2-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3226231125_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
