...
void kernel_3426112321_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_P+2; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_Q; i_2++) {
                A[i_1][i_0-2][i_2-2] = A[i_1][i_0-4][i_2] * A[i_1][i_0-3][i_2-2] + A[i_1-1][i_0-3][i_2-2] - 4;
            }
        }
    ...
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 1; i_5 < i_3; i_5++) {
                C[i_4+1][i_3+1] = C[i_5+1][i_4+2] * C[i_5][i_4+1] + C[i_4][i_5-1] + 6;
            }
        }
        for (int i_6 = i_3; i_6 < PB_L-1; i_6++) {
            D[i_6] = C[i_6+1][i_3+1] + D[i_6+1] - 1;
        }
    }
#pragma endscop
...
kernel_3426112321_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
