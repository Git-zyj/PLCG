...
void kernel_3416351311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0-1] = E[i_0] * E[i_0] * F[i_0] + F[i_0] * A[i_0] * A[i_0-2] + 4;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, PB_N); i_2++) {
                B[i_1][i_0+1][i_1] = G[i_0][i_1][i_2] - H[i_2][i_0-1][i_1+1] - H[i_1][i_2][i_0] - I[i_2+i_0][i_1+1][1] + B[i_1-1][i_0+2][i_1-1] * B[i_1+1][i_0+1][i_1+1] - C[i_2-1][i_1] + 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M+1, i_3); i_4++) {
            C[i_4][i_3] = F[i_3] - J[i_3][i_4] * K[1] + F[i_4] + L[i_3-1][i_4] - J[i_3][i_4] + M[i_3][i_4] * J[i_4+i_3-1][0] - A[i_3-1] + C[i_4][i_3+1] - D[i_3] + 6;
        }
        D[i_3] = F[i_3] - K[i_3] + E[i_3] + A[i_3-2] * D[i_3-1] - D[i_3+1] - 4;
    }
#pragma endscop
