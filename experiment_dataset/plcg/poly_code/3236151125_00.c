...
void kernel_3236151125_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = PB_P - 1; i_1 >= i_0; i_1--) {
            A[i_1-1][i_0] = C[i_0][i_1] + C[i_1][i_0+1] - C[i_1][i_0-1] - C[i_1][i_0] + A[i_1-2][i_0-2] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_P-1; i_3++) {
            for (int i_4 = i_2; i_4 < min(PB_Q, -i_2+PB_L-1); i_4++) {
                A[i_4-1][i_2-1] = C[i_4][i_3-1] - D[i_3][i_3+i_2-1][i_4+i_2+1] + D[i_3][i_4][0] - E[i_3+1][i_4][i_3+i_2+1][i_4] * D[i_3-1][i_4][i_2] + C[i_4+i_3][i_2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3236151125_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
