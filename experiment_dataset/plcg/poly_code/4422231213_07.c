...
void kernel_4422231213_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0] * E[i_0] * E[i_0] + 2;
        A[i_0] = E[i_0+1] + F[i_0] * F[i_0] + 2;
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            C[i_0-1][i_1+1][i_1-1] = G[i_0][i_1] * E[i_0] - G[i_1-1][i_0] + G[i_1][i_0] + H[i_0][i_0] + A[i_1-1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
            D[i_3+1] = D[i_3+2] + D[i_3] * D[i_3+2] * 6;
        }
    }
#pragma endscop
...
kernel_4422231213_07(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
