...
void kernel_4436132115_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            A[i_0+2][i_1-2] = E[i_0] - A[i_1+2][i_0-3] + 5;
        }
        for (int i_2 = i_0; i_2 < PB_M; i_2++) {
            for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
                A[i_3+1][i_2-1] = F[i_3][i_0][i_2] * F[i_2][i_3-1][i_0] + G[i_2][i_3-2] + 3;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_P-2; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
                A[i_5+2][i_6-2] = F[i_5-1][i_6-2][i_4+2] - G[i_5][i_6+1] + F[i_6][i_4][i_5] - G[i_6][i_5] + 3;
            }
        }
        for (int i_7 = max(i_4, 2); i_7 < PB_P; i_7++) {
            for (int i_8 = 1; i_8 < PB_M-2; i_8++) {
                for (int i_9 = i_8; i_9 < -i_8+PB_Q+1; i_9++) {
                    A[i_8+2][i_7-2] = H[i_4][i_7][i_9+i_8-1][i_8+1] - 4;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436132115_00(xa,ya,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
