...
void kernel_4236212311_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_2][i_0] = C[i_1+1][i_0] * A[i_2][i_1+1] - A[i_1+1][i_2] - 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            for (int i_5 = i_4; i_5 < -i_3+PB_L-1; i_5++) {
                B[i_4][i_5+i_3][i_3-1] = C[i_5][i_4-2] - B[i_4][i_5+i_3-1][i_3-1] * B[i_4-1][i_5+i_3+1][i_3-1] + B[i_4][i_5+i_3+1][i_3-1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4236212311_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
