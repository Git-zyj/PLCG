...
void kernel_3214151325_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_P - 1; i_0 >= 1; i_0--) {
        A[i_0+1] = C[i_0-1][i_0] + D[i_0] + D[i_0] * D[i_0] * D[i_0-1] * A[i_0-1] * A[i_0+2] - 3;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_P-1, i_1); i_2++) {
            A[i_2+1] = D[i_2] * C[i_2][i_1] - D[i_1] * C[i_1][i_2+1] + D[i_2+1] - A[i_2+1] + 6;
        }
    }
#pragma endscop
...
kernel_3214151325_00(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
