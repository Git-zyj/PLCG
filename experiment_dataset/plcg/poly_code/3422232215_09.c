...
void kernel_3422232215_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + E[i_0+2] - E[i_0] + 1;
        for (int i_1 = 1; i_1 < min(PB_M-1, -i_0+PB_L-1); i_1++) {
            B[i_1][i_1+i_0-1] = E[i_1+1] - F[i_1+1][i_0] + B[i_1+1][i_1+i_0+1] * B[i_1-1][i_1+i_0-2] - B[i_1][i_1+i_0-2] + 5;
        }
        A[i_0] = G[i_0+1] * H[i_0][i_0] + 2;
    }
    for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 3; i_3 < PB_P; i_3++) {
            for (int i_4 = 1; i_4 < i_2; i_4++) {
                D[i_4][i_3-2][i_2] = I[i_4-1][i_3+1][i_2+2] - J[i_4][i_3][i_3] + D[i_4+1][i_3-1][i_2] * D[i_4+1][i_3-2][i_2-1] - D[i_4][i_3-3][i_2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3422232215_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
