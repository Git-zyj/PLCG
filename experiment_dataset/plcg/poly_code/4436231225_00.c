...
void kernel_4436231225_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_L+1); i_1++) {
            A[i_0][i_1] = E[i_1][i_0] + F[i_1+i_0-1][i_0] * E[i_1][i_0] + A[i_0+1][i_1] - A[i_0+2][i_1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            for (int i_4 = i_3; i_4 < min(PB_M, -i_3+PB_L+1); i_4++) {
                A[i_4][i_2] = F[i_3][i_4] - G[i_2+1][i_3][i_4+i_3-1] + G[i_3][i_4][1] * 3;
            }
        }
        for (int i_5 = 0; i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 0; i_6 < i_5; i_6++) {
                for (int i_7 = i_2; i_7 < PB_M; i_7++) {
                    A[i_6][i_7] = H[i_6][i_7][i_2-1][i_5] * G[i_7][i_7][i_5] + H[i_6][i_2-1][i_5+1][i_7] * 1;
                }
            }
        }
    }
        ...
        for (int i_9 = 1; i_9 < min(i_8, -i_8+PB_Q+1); i_9++) {
            for (int j_0 = 0; j_0 < PB_M-1; j_0++) {
                for (int j_1 = 0; j_1 < PB_M; j_1++) {
                    A[j_0+1][j_1] = I[i_9-1][j_1][i_8][0] + H[j_0][i_9][i_8][j_1] - H[j_0+1][j_1][i_9+i_8-1][i_9-1] + 6;
                }
            }
        }
    }
...
kernel_4436231225_00(xa,ya,POLYBENCH_ARRAY(A), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
