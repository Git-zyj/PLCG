...
void kernel_3236131123_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            for (int i_2 = i_1; i_2 < PB_Q; i_2++) {
                A[i_1+i_0+1][i_0][i_2] = C[i_1][i_1][i_2] - 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M-2, i_3); i_4++) {
            for (int i_5 = 2; i_5 < min(min(-i_4+PB_M-1, PB_L), -i_3+PB_Q-1); i_5++) {
                A[i_5+i_4+1][i_5][i_3+2] = C[i_5-1][i_3][i_4] - C[i_5][i_4+i_3][i_4-1] * C[i_3][i_5][i_4] * C[i_3][i_5+i_4+1][0] + C[i_4][i_5+i_3][i_5+i_3+1] * A[i_4+i_3+2][i_3][i_5-2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3236131123_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
