...
void kernel_3434311321_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_P-2; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P-2; i_2++) {
                A[i_0][i_2+i_1][i_1] = A[i_0+2][i_2+i_1+2][i_2] * A[i_0-1][i_2+i_1-2][i_1-1] * 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_N; i_3++) {
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            B[i_3][i_4] = B[i_3][i_4-2] * 1;
        }
    }
    for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
        for (int i_6 = i_5; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 3; i_7 < PB_L-1; i_7++) {
                C[i_5][i_6-1][i_7-1] = C[i_5-2][i_7+1][i_6-1] * C[i_5][i_6+1][i_7-3] - C[i_5-2][i_6+1][i_7] - 3;
            }
        }
        for (int i_8 = 2; i_8 < PB_M-2; i_8++) {
            D[i_8][i_5] = E[i_5] * B[i_8-2][i_5] + D[i_8][i_5+2] - D[i_8+1][i_5] - D[i_8+2][i_5] * D[i_8+2][i_5-1] - D[i_8-2][i_5] * 2;
        }
    }
#pragma endscop
...
kernel_3434311321_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
