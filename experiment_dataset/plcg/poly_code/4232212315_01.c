...
void kernel_4232212315_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_0+1][i_1-1] = A[i_0][i_1-1] - A[i_0+1][i_1] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        B[2] = C[i_2] * C[i_2-2] * B[2] * B[2] - B[2] + 4;
    }
#pragma endscop
...
kernel_4232212315_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
