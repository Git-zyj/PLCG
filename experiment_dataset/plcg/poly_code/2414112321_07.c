...
void kernel_2414112321_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            C[i_2] = E[i_2][i_1] - D[i_1][i_2] * 5;
        }
        C[i_1] = C[i_1-2] - 2;
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            D[i_1][i_3] = C[i_3] + C[i_3-2] - D[i_1-2][i_3] - 4;
        }
    }
#pragma endscop
...
kernel_2414112321_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
