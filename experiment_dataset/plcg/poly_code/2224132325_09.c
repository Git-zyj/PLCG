...
void kernel_2224132325_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            A[i_1][i_0] = C[i_1][i_0] - C[i_1][i_0] - C[i_1-2][i_0] + A[i_1][i_0-1] - A[i_0+1][i_1] + A[i_1][i_0+2] + B[i_1-2] - 2;
        }
    }
    for (int i_2 = 4; i_2 < PB_N-1; i_2++) {
        B[i_2-2] = C[i_2][0] + C[i_2+1][i_2-2] - B[i_2-4] + 6;
    }
#pragma endscop
...
kernel_2224132325_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
