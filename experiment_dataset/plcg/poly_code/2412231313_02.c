...
void kernel_2412231313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[1] - B[i_0] - B[i_0-1] + B[i_0+1] - 4;
        B[i_0] = F[i_0] + E[i_0] + E[i_0] + B[i_0+1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        A[i_1-1] = E[i_1-1] * E[i_1-1] * F[i_1] * E[i_1+1] - 3;
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            D[i_1][i_2] = G[i_2+1][i_1-1] * H[i_1][i_2] * H[i_2][i_2+1] * B[i_2] + D[i_1+1][i_2-1] - D[i_1-1][i_2-1] - D[i_1][i_2-1] - 1;
        }
    }
#pragma endscop
...
kernel_2412231313_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
