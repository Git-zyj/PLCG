...
void kernel_3234311313_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_1] = C[i_1+1] + A[i_1+1] - A[i_1-1] - A[i_1-1] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-2; i_3++) {
            B[i_2+1][i_3] = B[i_2+1][i_3-1] - B[i_3+2][i_2-1] - 2;
        }
    }
#pragma endscop
...
kernel_3234311313_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
