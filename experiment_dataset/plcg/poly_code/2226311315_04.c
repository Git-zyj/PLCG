...
void kernel_2226311315_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L+1; i_1++) {
            B[i_0-1][i_1+i_0-1] = C[i_0][i_1] * 3;
        }
        for (int i_2 = 0; i_2 < min(i_0, -i_0+PB_L-1); i_2++) {
            B[i_0][i_2+i_0] = C[i_2][i_0+1] - B[i_0-1][i_2+i_0-1] + B[i_0][i_2+i_0+1] + B[i_0+1][i_2+i_0+1] + 4;
        }
    }
#pragma endscop
...
kernel_2226311315_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
