...
void kernel_4436152213_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = i_0; i_2 < -i_0+PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_1+PB_M, -i_0+PB_L); i_3++) {
                    A[i_0-2][i_2][0][i_0] = E[i_3+i_0][i_2][1][i_1] + E[i_2+1][i_1][i_3][i_0] + E[i_1][i_3+i_0][i_2][i_1] * F[i_3][i_1][i_2] + F[i_2][i_1][0] + E[i_3+i_1][i_2+i_0+1][i_1+1][i_0] * A[i_0-1][i_2][0][i_0+1] - 6;
                }
            }
        }
        for (int i_4 = i_0; i_4 < PB_M-2; i_4++) {
            B[i_0][i_4+1] = G[i_0][0] * G[i_4+2][i_0] - G[i_4][i_0] + B[i_0-1][i_4] - 2;
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = 0; i_6 < min(PB_P-3, i_5); i_6++) {
            for (int i_7 = 0; i_7 < min(PB_M-3, -i_6+PB_M-2); i_7++) {
                C[i_7+2][i_6+2] = G[i_5][i_7+i_6+2] + C[i_7+3][i_6+3] * C[i_7+3][i_6+2] - 3;
            }
        }
    }
    for (int i_8 = 2; i_8 < PB_L-1; i_8++) {
        for (int i_9 = i_8; i_9 < min(-i_8+PB_L, PB_M-3); i_9++) {
            for (int j_0 = 1; j_0 < min(i_8, -i_9+PB_P); j_0++) {
                D[j_0+1][i_8+1][j_0] = F[j_0][i_8-2][i_8] * F[i_9][i_8+1][0] - G[j_0][i_9] + G[j_0][i_9] - F[i_8][i_9][j_0+i_9] - F[j_0][i_9+i_8][0] + F[j_0+1][i_9][1] - C[i_9+3][j_0+2] + D[j_0][i_8+1][j_0-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4436152213_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
