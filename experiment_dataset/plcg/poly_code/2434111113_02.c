...
void kernel_2434111113_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        A[i_0] = E[i_0][i_0] - 6;
        for (int i_1 = 1; i_1 < PB_N+1; i_1++) {
            B[i_1-1][i_0+1] = E[i_1][i_0] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            B[i_2-1][i_3+1] = E[i_3][0] * 2;
        }
        D[i_2-1] = D[i_2] * 4;
    }
#pragma endscop
...
kernel_2434111113_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
