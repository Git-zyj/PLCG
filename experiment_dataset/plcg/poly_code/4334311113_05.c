...
void kernel_4334311113_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L-1; i_1++) {
            A[i_0][i_1+i_0] = D[i_1+i_0+1] + A[i_0][i_1+i_0+1] + 2;
        }
    }
    ...
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M-1; i_4++) {
            C[i_4+i_3][i_3] = C[i_4+i_3+1][i_3] - 5;
        }
    }
#pragma endscop
...
kernel_4334311113_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
