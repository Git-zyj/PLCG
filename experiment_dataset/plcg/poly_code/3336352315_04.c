...
void kernel_3336352315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        A[i_0+2] = D[i_0][i_0] * E[0] * F[i_0+2] * A[i_0+3] - A[i_0+1] - A[i_0+3] + 2;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_M; i_2++) {
            A[i_2+2] = G[i_2-1][i_1-2] - D[i_2+i_1][i_1] - D[i_2+1][i_1+2] * G[i_2][i_1] * G[i_2][i_1+2] - H[i_2][i_2+i_1] * D[i_1+2][i_2+1] + A[i_2+2] * A[i_2+1] + 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_N; i_4++) {
            for (int i_5 = 1; i_5 < i_3; i_5++) {
                C[i_5-1][i_3][i_4] = I[i_4][i_3][i_5] + I[i_5+1][i_3][i_4-1] + D[i_3+1][i_4] - C[i_5][i_3-1][i_4+1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3336352315_04(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
