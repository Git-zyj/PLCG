...
void kernel_4212212213_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_2][i_1] = C[i_1-1][i_0][i_2] - A[i_2][i_1+1] * 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < PB_L; i_5++) {
                B[i_3][i_4][i_5] = A[i_4][i_5] + B[i_3+1][i_4][i_5] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4212212213_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
