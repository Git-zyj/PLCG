...
void kernel_3324232215_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] - E[i_0] + E[i_0-2] - A[i_0+1] * 3;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            B[i_1][i_2] = F[i_2][1] * A[i_2-1] * A[i_1] * B[i_1][i_2-1] - 5;
        }
        for (int i_3 = i_1; i_3 < min(PB_L, -i_1+PB_P); i_3++) {
            for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
                B[i_4][i_1+1] = G[i_3][i_1-1][i_1] * G[i_4][i_3+i_1][i_3] + G[i_4+2][i_3][i_1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3324232215_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
