...
void kernel_3324251325_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0-1] = D[i_0] + D[i_0+1] - E[i_0] + E[i_0] - D[i_0+1] - D[i_0] * A[i_0-2] + 2;
        A[i_0+1] = E[i_0] - F[i_0][i_0] * E[i_0-1] + D[i_0] - A[i_0+1] + 1;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_2+PB_L, PB_M); i_3++) {
                C[i_2][i_3][i_1+1] = G[i_3-1][0][i_2] * H[i_2+1][i_1][1] - G[i_3-1][i_1][i_2] + I[i_3][i_3+i_2][i_1][i_2] * C[i_1][i_2][i_3+3] * C[i_2][i_3+2][i_1+3] + C[i_2-2][i_1][i_3+1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3324251325_08(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
