...
void kernel_4334331215_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
        ...
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P-1; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_P+1, i_0); i_3++) {
                    B[i_3][i_1] = D[i_2][i_3+i_0-1][0][i_0] + D[i_0][i_2+i_1+1][i_3][i_3] + 2;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                B[i_4][i_0] = E[i_0-1][i_4][i_5] + F[i_4+1][i_5][1] + F[i_0][i_5+1][i_4] - B[i_5][i_4+1] - B[i_5-1][i_4-1] + 2;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 0; i_7 < -i_6+PB_M; i_7++) {
            B[i_7][i_6+1] = G[i_7+i_6][0] - G[i_6+1][i_7] - H[i_7] * 1;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4334331215_06(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
