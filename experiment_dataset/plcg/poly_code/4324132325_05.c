...
void kernel_4324132325_05(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_Q+1, PB_Q-2); i_2++) {
                for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
                    B[i_3][i_1][i_2+2][i_0+2] = D[i_1][i_0][i_3+2][i_2+1] * D[i_1][i_2-2][i_3+2][i_2+i_0-1] + D[i_3][i_2-2][i_1+2][i_0] * 1;
                }
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
            for (int i_6 = 2; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 1; i_7 < -i_4+PB_P+2; i_7++) {
                    B[i_4][i_5][i_6][i_7] = D[i_6-1][i_7+i_4-2][i_5][i_5] + E[i_5][i_7][i_4+2] - E[i_5][i_7][i_6] + E[i_6][i_7-1][i_5] * B[i_5+2][i_7][i_6-2][i_4-1] + B[i_4][i_5-2][i_6][i_7] * B[i_4+2][i_5][i_6-1][i_7] + 3;
                }
                for (int i_8 = i_6; i_8 < PB_P-2; i_8++) {
                    B[i_6+1][i_5][i_4][i_8+1] = D[i_6][i_8+2][i_4][i_5] + D[i_5][i_6-2][i_6][i_4+1] - 3;
                }
            }
        }
    }
...
kernel_4324132325_05(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
