...
void kernel_3412252315_05(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L - 1; i_0 >= 2; i_0--) {
        for (int i_1 = 1; i_1 < min(PB_L, -i_0+PB_N+2); i_1++) {
            D[i_1] = E[i_1][i_0] + F[i_1][i_0] - G[i_1+i_0-2][i_0][i_1] * E[i_0+2][i_1-1] + F[i_1][i_0] - F[i_1][i_0] + F[i_0][1] - 4;
        }
        for (int i_2 = 2; i_2 < PB_N+2; i_2++) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                D[i_3] = G[i_2-2][i_0-2][i_0] * 6;
            }
        }
        D[i_0] = H[i_0] + H[i_0-2] - I[i_0] * H[i_0] - 6;
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        D[i_4] = H[i_4] * I[i_4] + H[i_4] - I[i_4] - I[i_4+1] + I[i_4+1] + D[i_4-1] * D[i_4+1] * D[i_4-1] * 3;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3412252315_05(xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
