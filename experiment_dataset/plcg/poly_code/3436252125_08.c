...
void kernel_3436252125_08(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        D[i_0] = E[i_0] - F[i_0] - F[i_0+1] * F[i_0] - F[i_0] + F[i_0] - 2;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_M-4, i_1); i_2++) {
            for (int i_3 = 2; i_3 < -i_1+PB_L-2; i_3++) {
                B[i_2+2][i_1] = G[i_3][i_2] + H[i_2+2][i_1+1][i_3-2] * G[i_1][i_1] - I[i_2][i_3][i_1] * H[i_2+1][i_3+i_1+2][i_1] * B[i_2+4][i_3] * 1;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        C[i_4+2] = F[i_4] - E[i_4+2] + F[0] + F[i_4] * 4;
        D[i_4] = E[i_4] * E[i_4] - F[i_4] - E[i_4] * F[i_4] - C[i_4+2] + 2;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3436252125_08(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
