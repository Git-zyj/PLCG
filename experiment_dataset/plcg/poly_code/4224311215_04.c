...
void kernel_4224311215_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        B[i_4] = C[i_4+1] + C[i_4] * B[i_4+1] - B[i_4-1] + B[i_4+1] + B[i_4+1] * 2;
    }
#pragma endscop
...
kernel_4224311215_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
