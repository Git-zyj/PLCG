...
void kernel_2322212225_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = PB_M-1 - 1; i_2 >= 0; i_2--) {
        for (int i_3 = 0; i_3 < -i_2+PB_M-1; i_3++) {
            C[i_3+i_2] = D[i_3+i_2][0] - C[i_3+i_2+1] - 5;
        }
    }
#pragma endscop
...
kernel_2322212225_04(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
