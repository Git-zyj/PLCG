...
void kernel_3414252225_09(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                B[i_1-1][i_0][i_1-1] = E[i_2][i_1][2] + F[i_2-1][0][i_1] * E[i_2][i_0][i_1] - F[i_1][i_0-2][i_2] + E[i_0][0][i_1] - 1;
            }
            B[i_1][i_0][i_1] = G[i_1+i_0][i_0] - H[i_0][i_1] * G[i_1][i_0] + B[i_1][i_0+1][i_1] - 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            B[i_3-2][i_4][i_3-2] = I[i_4] - G[i_4][i_3] * G[i_3][0] + H[i_4+1][i_3] + 1;
        }
        for (int i_5 = i_3; i_5 < PB_P-2; i_5++) {
            for (int i_6 = 3; i_6 < min(-i_3+PB_M+1, -i_5+PB_L); i_6++) {
                D[i_5-1][i_6-1][i_3+2][i_5] = H[i_6+2][i_3] + E[i_6+i_3-1][i_6+i_5][i_5] * F[i_6+i_5][i_3][i_5] - G[i_6][i_5] * E[i_5][i_6][i_3] * F[i_3][i_5-1][i_5] + D[i_6][i_5-1][i_3+2][i_6+1] * D[i_6-3][i_3-1][i_5+2][i_6-2] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3414252225_09(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
