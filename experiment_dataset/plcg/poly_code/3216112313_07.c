...
void kernel_3216112313_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, i_0); i_1++) {
            A[i_1][i_0-2] = C[i_1][i_0] + A[i_1-1][i_0-2] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-3; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                B[i_2][i_3+2][i_2] = C[i_2][i_3] * A[i_4+1][i_3-2] * B[i_2+1][i_3+2][i_2+1] - B[i_2+1][i_3+3][i_2+1] - B[i_4+1][i_3+2][i_4+1] * B[i_2-1][i_3+2][i_2-1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3216112313_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
