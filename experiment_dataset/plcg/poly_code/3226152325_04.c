...
void kernel_3226152325_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < i_1; i_2++) {
                A[i_0+2][i_2][2] = C[i_0][i_2][i_1] + C[i_2-2][i_1][i_0] * C[i_2][i_0-1][i_1+1] * C[i_2][i_1+i_0+1][0] - A[i_0+2][i_2-1][2] - A[i_0+2][i_1+1][2] + A[i_0][i_2][2] * A[i_0+2][i_1-2][2] - 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_L, PB_M-1); i_4++) {
            for (int i_5 = max(i_4, 2); i_5 < min(min(-i_3+PB_L, PB_M), -i_4+PB_L); i_5++) {
                B[i_5+i_3][i_3][i_5][i_4-1] = D[i_5-2][i_4+1] * C[i_3][i_5][i_4+i_3] * C[i_3][i_5+i_4][i_3-1] + C[i_3][i_4][i_5-2] + D[i_5][0] - B[i_5+i_4][i_5][i_4][i_3] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3226152325_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
