...
void kernel_2212311215_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < min(-i_0+PB_M-1, PB_L-1); i_2++) {
            B[i_0][i_0] = C[i_2+i_0+1][i_2+1] + B[i_2+1][i_2+1] + 6;
        }
    }
#pragma endscop
...
kernel_2212311215_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
