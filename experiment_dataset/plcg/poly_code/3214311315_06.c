...
void kernel_3214311315_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                A[i_1][1][i_2+1] = C[i_2][i_0] * A[i_1+1][1][i_0+2] * A[i_2][1][i_1+2] - 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
                B[i_4][i_3][i_5] = B[i_4][i_3][i_5+1] * B[i_4][i_3][i_5-1] * B[i_4-1][i_3][i_5] * B[i_4+1][i_3+1][i_5] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3214311315_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
