...
void kernel_3336212213_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        ...
        for (int i_3 = 0; i_3 < min(PB_M-3, i_0); i_3++) {
            B[i_3+2][i_3+2] = D[i_3][i_0] * B[i_3+1][i_3+1] * B[i_3+3][i_3+3] * B[i_3+1][i_3+1] * 4;
        }
    }
...
kernel_3336212213_09(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
