...
void kernel_3116251311_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_L-2); i_1++) {
            A[i_1+1][i_1+1] = B[i_1][i_0-1] - C[i_1+i_0][0] * B[i_0-1][i_0] * D[i_0] - C[i_0][i_1+1] + A[i_1][i_1] - A[i_1+2][i_1+2] * A[i_1][i_1] * 1;
        }
    }
#pragma endscop
...
kernel_3116251311_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
