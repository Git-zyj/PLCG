...
void kernel_2432251121_01(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        C[i_0-1] = E[i_0] + F[i_0] * E[i_0] + E[i_0] + F[i_0] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        B[i_1] = E[i_1] * E[i_1+1] * F[i_1] * F[i_1+1] + 5;
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        C[i_2-1] = G[i_2][i_2] * E[i_2-1] * E[i_2] * E[i_2] * C[i_2] + C[i_2-2] - 2;
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            D[i_2][i_3] = G[i_3][i_2] + G[i_3][i_2] * G[i_2][i_3] * H[i_3][i_2] + D[i_2-2][i_3] * 3;
        }
    }
#pragma endscop
...
kernel_2432251121_01(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
