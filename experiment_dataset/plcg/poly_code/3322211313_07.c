...
void kernel_3322211313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_1] = B[i_1+1][i_0] - C[i_1-1] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                B[i_3+1][i_4] = B[i_3+2][i_4-1] + B[i_3+1][i_4+1] + B[i_4][i_3-1] * B[i_3][i_4] * 4;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        C[i_5] = D[i_5] + E[i_5][0] + C[i_5+1] * C[i_5-1] - C[i_5+1] * 2;
    }
#pragma endscop
...
kernel_3322211313_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
