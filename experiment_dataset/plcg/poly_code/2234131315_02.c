...
void kernel_2234131315_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1] = C[i_0] * D[i_0][i_1] * D[i_0][i_0+1] + A[i_1-1] + A[i_1+1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        A[i_2-1] = C[i_2] - C[i_2] - C[i_2] - A[i_2+1] * 3;
    }
#pragma endscop
...
kernel_2234131315_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
