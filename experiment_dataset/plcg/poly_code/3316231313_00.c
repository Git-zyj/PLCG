...
void kernel_3316231313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0-1] = D[i_1] - A[i_1+i_0-2] * A[i_1+i_0-2] + A[i_1+i_0] - B[i_0] * B[i_1-1] * 4;
        }
        B[i_0] = E[i_0] + E[i_0] - B[i_0-1] + B[i_0+1] + B[i_0-1] + 5;
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = max(2, i_2); i_3 < min(-i_0+PB_M, PB_P); i_3++) {
                C[i_2][i_3+i_0][i_3-1] = F[i_2][i_3][i_2-1] * G[i_2][i_3][i_0] - H[i_2][i_0] - G[i_0][i_2][i_3] + C[i_2][i_3+i_0-1][i_3-2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3316231313_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
