...
void kernel_4436251225_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_N-1, PB_L); i_2++) {
                for (int i_3 = 1; i_3 < PB_L; i_3++) {
                    A[i_2+i_1+1][i_1][i_2][i_3] = E[i_3+1][i_3][i_2+1] - F[i_2][i_1][i_1][i_0] - G[i_2][i_1][i_3-1][i_0+1][1] * 5;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < min(-i_5+PB_M, PB_L-1); i_6++) {
                for (int i_7 = 1; i_7 < min(min(PB_L, -i_6+PB_L-1), -i_5+PB_M); i_7++) {
                    A[i_7+i_6+1][i_6][i_7][i_5-2] = H[i_4][i_7][i_5][i_6-1] * F[i_4][i_6+1][i_7+i_5][i_5] - G[i_6+i_5][i_7-1][i_4][i_7+i_6+1][i_5] - F[i_7][i_4][i_5+1][i_6+1] * 4;
                }
            }
        }
    }
    for (int i_8 = 2; i_8 < PB_L-1; i_8++) {
        for (int i_9 = i_8; i_9 < -i_8+PB_N-2; i_9++) {
            C[i_9][i_8] = I[i_9+1][i_8+1] + I[i_9-1][i_8-1] - J[i_9+1] - I[i_9][i_8] * K[i_9] + L[i_9][i_8] + I[i_9][i_8] - C[i_9-1][i_8] + C[i_9][i_8+1] * C[i_9+2][i_8-2] + 2;
            for (int j_0 = 0; j_0 < PB_Q; j_0++) {
                for (int j_1 = 0; j_1 < PB_Q-1; j_1++) {
                    A[i_9+i_8+2][i_9+1][i_8][j_1+1] = F[j_0][j_0+1][j_1][i_9-1] * H[j_1][i_8][i_9+1][j_0] - M[j_0][i_8][i_9-1] + 1;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4436251225_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
