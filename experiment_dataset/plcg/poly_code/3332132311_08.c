...
void kernel_3332132311_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                A[i_1-1][i_2][i_0] = D[i_1][i_2-1][i_0] - A[i_1][i_2][i_0] * C[i_2-1][i_1][i_0+2] * 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_3+PB_M-2, -i_4+PB_L); i_5++) {
                B[i_3][i_4][i_5+2] = D[i_5-2][i_4][i_5] - D[i_5+1][i_3][i_4] * D[i_5+i_3+2][i_5+i_4][0] - D[i_4][i_5+i_3][i_3] * B[i_3][i_4+1][i_5+2] - B[i_3-1][i_4][i_5+2] * B[i_3-1][i_4][i_5+1] * 1;
            }
        }
        for (int i_6 = 0; i_6 < PB_P-2; i_6++) {
            for (int i_7 = 0; i_7 < PB_P-3; i_7++) {
                C[i_3-1][i_6+1][i_7+1] = D[i_7+2][i_3+2][i_6] - E[0][i_6] - B[i_6][i_3][i_7+3] - C[i_3][i_6+1][i_7] * C[i_3-1][i_6+2][i_7+1] - C[i_3-1][i_6+1][i_7] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3332132311_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
