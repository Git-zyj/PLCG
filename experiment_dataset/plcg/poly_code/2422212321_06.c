...
void kernel_2422212321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-3; i_2++) {
            B[i_2+1][i_1] = A[i_2+1] - B[i_2+3][i_1] - D[i_2] * 2;
        }
        D[i_1+1] = D[i_1+2] - 1;
        for (int i_3 = 2; i_3 < PB_N+1; i_3++) {
            D[i_3] = E[i_3-1][i_1] * B[i_3+1][i_1] * B[i_3][i_1] * B[i_3+1][i_1-1] - D[i_3-2] * 1;
        }
    }
#pragma endscop
...
kernel_2422212321_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
