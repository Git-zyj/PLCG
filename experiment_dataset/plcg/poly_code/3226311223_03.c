...
void kernel_3226311223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            for (int i_4 = 2; i_4 < min(PB_L, -i_3+PB_Q); i_4++) {
                B[i_3-1][i_2][i_4] = C[i_4-1][i_4+1][i_2] * D[i_3][i_2-1][i_4+i_3] - B[i_3-1][i_2][i_4-2] * B[i_3-1][i_2+1][i_4] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3226311223_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
