...
void kernel_3124152121_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_L-1); i_1++) {
            for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
                A[i_2][i_2+i_1] = B[i_2-2][i_1+i_0][i_0] - C[i_2][i_1] * C[i_2][i_1] - C[i_2][i_1] - C[i_2+1][i_1+1] - A[i_1][i_2+i_1+2] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3124152121_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
