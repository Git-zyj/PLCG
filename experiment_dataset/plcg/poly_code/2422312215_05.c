...
void kernel_2422312215_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0-1] = E[i_0] + D[i_0][i_0+1] * 6;
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            A[i_1-1] = 6;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            C[i_2][i_0+2] = C[i_2-1][i_0+1] + 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M-2; i_4++) {
            D[i_4+1][i_4+2] = D[i_4+2][i_4+3] - D[i_4][i_4+1] * D[i_3+2][i_3+3] - 5;
        }
    }
#pragma endscop
...
kernel_2422312215_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
