...
void kernel_4212132123_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, PB_L-1); i_2++) {
                A[i_1-2][i_0-2] = C[i_2+1][i_1+1] + C[i_2][i_1] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-3; i_3++) {
        B[i_3+1] = C[i_3+1][i_3+1] + D[i_3] * D[i_3] + D[i_3] + B[i_3+3] - 4;
    }
#pragma endscop
...
kernel_4212132123_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
