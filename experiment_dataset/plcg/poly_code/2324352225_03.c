...
void kernel_2324352225_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            A[i_1-1] = D[i_1][0] * E[i_1+1] - F[i_1+2] * G[i_1+2][i_0-1] + A[i_1] * 3;
        }
        B[i_0] = F[2] - E[i_0] + D[i_0][i_0] + E[i_0] + H[i_0] + A[i_0-1] * B[i_0-2] * 4;
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        B[i_2] = E[i_2] - H[i_2+1] * F[i_2] - E[i_2+2] * A[i_2-1] + 4;
    }
#pragma endscop
...
kernel_2324352225_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
