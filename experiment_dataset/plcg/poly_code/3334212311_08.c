...
void kernel_3334212311_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(min(-i_0+PB_M, i_0), PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M-1; i_2++) {
                A[i_1+i_0][i_2] = A[i_2+i_1+1][i_0] * A[i_2+i_1+1][i_0] + 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        B[i_3+1] = D[i_3-2][i_3] - B[i_3+2] * B[i_3] + C[i_3] - 4;
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        C[i_4] = B[i_4+1] - C[i_4-1] + C[i_4+1] + C[i_4+1] - 1;
    }
#pragma endscop
...
kernel_3334212311_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
