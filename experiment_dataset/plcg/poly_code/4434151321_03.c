...
void kernel_4434151321_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] * E[i_0] * E[i_0+1] + F[i_0][1] * 4;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_1+PB_N, i_2); i_3++) {
                B[i_1][i_3][i_2] = G[i_3-1][i_2][i_1] * G[i_3+1][i_1][i_1] - H[i_3-1][i_2][i_1][i_2-1] + F[i_3+i_1][i_2+1] + G[i_3+i_1][i_3][i_2+1] * G[i_2][i_1-1][i_2] + B[i_1-1][i_3][i_2-2] * B[i_1-1][i_3][i_2] * 2;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-3; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            C[i_4][i_5+1] = F[i_4][i_5] * F[i_4][0] - F[i_4+1][i_4+1] - F[i_5][i_4] + F[i_5+1][i_4+1] + C[i_4][i_5+3] - C[i_4+1][i_5+1] - C[i_5][i_4+3] - C[i_4][i_5+2] * C[i_4][i_5-1] + 4;
        }
        for (int i_6 = 1; i_6 < i_4; i_6++) {
            for (int i_7 = 1; i_7 < -i_4+PB_L; i_7++) {
                for (int i_8 = 2; i_8 < min(-i_6+PB_P, PB_M-2); i_8++) {
                    D[i_8][i_4][i_6][i_7+1] = H[i_4+1][i_7-1][i_8+i_6][i_7] * H[i_6+1][i_7-1][i_8-1][i_4] + H[i_7][i_7][i_8][i_4] * H[i_8-1][i_7+i_4][i_6][i_8] + D[i_7+2][i_8-2][i_6][i_4+1] - D[i_8-1][i_4+1][i_6][i_7+1] - D[i_8-1][i_4-2][i_6][i_7-1] + D[i_8+2][i_4][i_6-1][i_7-1] - 4;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434151321_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
