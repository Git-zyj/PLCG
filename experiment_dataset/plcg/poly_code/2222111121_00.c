...
void kernel_2222111121_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        B[i_1] = C[i_1] - C[i_1] * A[i_1+2][i_1+2] + B[i_1+2] + 4;
    }
#pragma endscop
...
kernel_2222111121_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
