...
void kernel_2114151325_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_1][i_0] = B[i_0] * C[i_1-1][i_0-1] * C[i_1][i_0] * C[i_1][i_0] * C[i_1+i_0][0] * A[i_1-1][i_0] * A[i_1-1][i_0-1] + A[i_1][i_0+2] - 4;
        }
    }
#pragma endscop
...
kernel_2114151325_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
