...
void kernel_3336112221_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = i_0; i_2 < PB_N; i_2++) {
                A[i_2][i_0][i_1] = D[i_2][i_0][i_1] * A[i_2][i_0-1][i_1] + 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < PB_P-2; i_5++) {
                B[i_3+1][i_4][i_5+2] = B[i_3+2][i_4][i_5+2] * B[i_3+1][i_4-1][i_5+2] - 6;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
        for (int i_7 = 2; i_7 < min(i_6, PB_N); i_7++) {
            for (int i_8 = 2; i_8 < PB_N; i_8++) {
                C[i_7][i_6][i_8-1] = D[i_8][i_7][i_7] + A[i_8][i_6+2][i_7] * C[i_7-2][i_6][i_8-2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3336112221_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
