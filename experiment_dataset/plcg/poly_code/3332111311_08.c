...
void kernel_3332111311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            A[i_1+1] = A[i_1+2] * A[i_1] * C[i_1-1] + 3;
        }
        for (int i_2 = 1; i_2 < i_0; i_2++) {
            B[i_0+1][i_2-1] = D[i_2] + E[i_2+1][i_0] - B[i_0][i_2-1] - B[i_0+1][i_2] * B[i_0+2][i_2-1] + 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            C[i_4-1] = A[i_4] + C[i_4-2] + 1;
        }
    }
#pragma endscop
...
kernel_3332111311_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
