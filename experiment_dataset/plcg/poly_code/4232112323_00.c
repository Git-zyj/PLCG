...
void kernel_4232112323_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_Q+1; i_2++) {
                A[i_2][i_1][i_2+i_1-2] = C[0][i_1][i_0] + A[i_2-1][i_1][i_2+i_1-3] - A[i_2+2][i_1-1][i_2+i_1-1] * A[i_2][i_1+1][i_2+i_1-1] - 1;
            }
        }
        for (int i_3 = 2; i_3 < -i_0+PB_Q+4; i_3++) {
            A[i_3-2][i_0][i_3+i_0-4] = 5;
        }
    }
#pragma endscop
...
kernel_4232112323_00(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
