...
void kernel_4316151315_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M - 1; i_0 >= 1; i_0--) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_M-1; i_2++) {
                for (int i_3 = 1; i_3 < min(min(PB_M, -i_0+PB_P), -i_2+PB_M); i_3++) {
                    A[i_1][i_3][i_2][i_0] = D[i_3][i_2-1][i_0][i_1+1] - D[i_3+i_2][i_1+1][i_0-1][i_3-1] - D[i_1][i_3+i_2][i_0][i_3] + D[i_3][i_2+1][i_0][0] - D[i_2+i_1+1][i_3][i_0][0] * D[i_3][i_2][i_3+i_0][i_0+1] - D[i_3][i_0][i_1][i_2] + A[i_1][i_0][i_2+1][i_3] - A[i_1-1][i_3][i_2-1][i_0+1] - A[i_1-1][i_2-1][i_0+1][i_3] + A[i_1-1][i_2][i_0+1][i_3] + 6;
                }
            }
            B[i_1] = E[i_0][0] - F[i_0][i_1][i_1] - E[i_1-1][i_0] + B[i_1+1] - 2;
        }
        for (int i_4 = i_0; i_4 < PB_M; i_4++) {
            for (int i_5 = i_4; i_5 < PB_Q; i_5++) {
                for (int i_6 = 1; i_6 < PB_M; i_6++) {
                    A[i_0][i_5][i_6][i_4] = D[i_4][i_0][i_6][i_5] - F[i_6-1][i_4][i_5] * G[i_6][i_5-1][i_0][i_4][i_0] - 4;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4316151315_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G)));
