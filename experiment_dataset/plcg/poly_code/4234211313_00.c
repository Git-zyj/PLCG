...
void kernel_4234211313_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_N, i_0); i_1++) {
            A[i_1-1][i_0+1] = A[i_1][i_0+1] * A[i_1-1][i_0] + B[i_1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_N-1; i_3++) {
            B[i_3] = C[i_2][i_3] * B[i_3+1] + B[i_3-1] - 4;
        }
    }
#pragma endscop
...
kernel_4234211313_00(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
