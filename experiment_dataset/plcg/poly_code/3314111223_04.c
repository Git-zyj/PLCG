...
void kernel_3314111223_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                B[i_2][i_0] = D[i_2][i_1][i_0] + C[i_0][i_2][i_1] + 1;
            }
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_0][i_3] = E[i_3][i_0] * 6;
        }
    }
    for (int i_4 = 1; i_4 < PB_L+1; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
            for (int i_6 = 2; i_6 < min(i_4, PB_L); i_6++) {
                C[i_4][i_5+1][i_6] = D[i_5][i_6-1][i_4-1] - C[i_5][i_4+2][i_6] - C[i_4][i_5+1][i_6-2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3314111223_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
