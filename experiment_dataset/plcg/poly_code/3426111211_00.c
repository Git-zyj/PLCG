...
void kernel_3426111211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            C[i_3] = E[i_3][i_2-1] - E[i_3][i_2] - C[i_3] * C[i_3+1] * 3;
        }
        C[i_2] = 4;
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            D[i_4][i_5] = D[i_4+1][i_5] - D[i_4][i_5-1] + 1;
        }
    }
#pragma endscop
...
kernel_3426111211_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
