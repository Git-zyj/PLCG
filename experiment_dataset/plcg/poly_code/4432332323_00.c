...
void kernel_4432332323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = E[i_0] - F[i_0][1] * G[i_0-2] * E[i_0] + A[i_0] + A[i_0+1] + 6;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_M, -i_1+PB_N); i_2++) {
            for (int i_3 = i_2; i_3 < PB_M-2; i_3++) {
                B[i_2+i_1][i_3][i_1] = H[i_3+i_2][i_1-1][i_1] + I[i_1][i_2] * J[i_2+i_1][i_3+1][2] * B[i_2+i_1-2][i_3][i_1] * B[i_2+i_1][i_3+2][i_1] + B[i_2+i_1-2][i_3+1][i_1-2] * 5;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_L); i_5++) {
            C[i_4][i_5-1][i_5] = F[i_5-1][i_4] + E[i_5+2] - A[i_5-1] * C[i_5-1][i_4-1][i_4] + 2;
            for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
                B[i_5+i_4+3][i_6][i_5+2] = J[i_5+i_4-2][i_6][i_5] * K[i_6+2][i_5+i_4][i_6+1] + J[i_6][i_4][i_5] + C[i_4][i_5-1][i_5] * C[i_4-1][i_5+1][i_5+2] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4432332323_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
