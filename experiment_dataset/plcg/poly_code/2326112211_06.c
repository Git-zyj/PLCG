...
void kernel_2326112211_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        A[i_0] = B[i_0-3] * B[i_0-2] - B[i_0-1] * 2;
        B[i_0-2] = A[i_0] * A[i_0-1] + 3;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            C[i_2+i_1][i_2+i_1] = D[i_2][i_2] * D[i_2][i_1+2] + 4;
        }
    }
#pragma endscop
...
kernel_2326112211_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
