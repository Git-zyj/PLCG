...
void kernel_3426251125_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            for (int i_2 = i_0; i_2 < -i_0+PB_P+1; i_2++) {
                A[i_0][i_1-1][i_2] = E[i_2][i_0][0] + F[i_1][i_2][i_0] + F[i_1][i_2+i_0-1][i_0] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        B[1] = G[i_3] + G[i_3] * H[i_3] + 2;
    }
    for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
        for (int i_5 = i_4; i_5 < -i_4+PB_P-1; i_5++) {
            for (int i_6 = 1; i_6 < min(PB_P-2, -i_4+PB_M); i_6++) {
                A[i_4-1][i_5+1][i_6+2] = E[i_5-1][i_6-1][i_5+i_4+1] * F[i_5][i_6][i_4] - F[i_6+i_4][i_5][1] - I[i_4-1][i_4] * J[i_6-1][i_6] + I[i_6+1][i_5] - E[i_6][i_5-1][i_4] + 1;
            }
        }
        B[1] = G[i_4-1] - G[i_4] * G[i_4+1] - H[i_4] - B[1] * 2;
    }
#pragma endscop
...
kernel_3426251125_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
