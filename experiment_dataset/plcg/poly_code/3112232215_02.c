...
void kernel_3112232215_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_0-1][i_1+2][1] = B[i_1][i_1] * B[i_1+i_0][0] * C[i_1-2][i_0-1] + A[i_0-1][i_1+3][1] - A[i_0-1][i_1+1][1] * 2;
        }
    }
#pragma endscop
...
kernel_3112232215_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
