...
void kernel_4432132323_01(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                C[i_0][i_0-1][i_1] = E[i_0+1][i_2][i_2] + 4;
            }
        }
        for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
            for (int i_4 = 2; i_4 < PB_P; i_4++) {
                C[i_0][i_0-1][i_4] = E[i_4-2][i_0-2][i_3] * E[i_3][i_4][i_0-1] - E[i_3][i_0+1][i_3] - C[i_4-1][i_4-2][i_3] - 3;
            }
            C[i_0][i_0-1][i_3] = F[i_3][i_3] + E[i_3][i_0][i_0] - G[i_0+2] - F[i_3][i_0] + C[i_0][i_0-1][i_3+1] * C[i_0][i_0-1][i_3-1] - 6;
        }
    }
        ...
        for (int i_6 = 1; i_6 < PB_M-2; i_6++) {
            for (int i_7 = 0; i_7 < i_6; i_7++) {
                for (int i_8 = 2; i_8 < PB_L-1; i_8++) {
                    C[i_7+2][i_7+1][i_5] = H[i_7+2][i_8][i_6-1][i_5-1] + H[i_8-2][i_6+2][i_8+1][i_7+2] + H[i_7][i_8][i_6-1][i_5] + H[i_6+2][i_7][i_5][i_8] + 6;
                }
            }
        }
    }
...
init_array(xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4432132323_01(xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
