...
void kernel_3432131323_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_M+1); i_2++) {
                B[i_2+i_1-1][i_1][0] = E[i_1+1][i_2][i_0][i_0-1] - 4;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_L, -i_3+PB_M-1); i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_M+1, PB_L); i_5++) {
                B[i_5+i_3-1][i_3][0] = F[i_3][i_5][i_5] * F[i_4-1][i_3+1][1] * 3;
            }
            B[i_4+i_3-3][i_3-2][0] = G[i_4-1][i_4+i_3] * H[i_4] + G[i_4][i_4+i_3-1] * B[i_4+i_3-1][i_3][0] - B[i_4+i_3][i_3][0] + B[i_4+i_3+1][i_4+2][0] + 5;
        }
    }
    for (int i_6 = 0; i_6 < PB_L; i_6++) {
        for (int i_7 = 1; i_7 < PB_L; i_7++) {
            for (int i_8 = 0; i_8 < min(-i_7+PB_M-2, PB_L); i_8++) {
                B[i_8+i_7+2][i_8+2][0] = G[i_7-1][i_7+1] - F[i_7-1][i_6][i_8] * G[i_7][i_8] * 1;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3432131323_05(xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
