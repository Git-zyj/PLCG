...
void kernel_3314151213_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] + D[i_0] + D[i_0] - D[i_0] + D[0] * D[i_0] * D[i_0-1] + A[i_0-1] * A[i_0+1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M+1, PB_M-1); i_2++) {
            B[i_1][i_2] = D[i_2] + E[i_1-1][0] + E[i_2+1][i_1-1] + E[i_2][i_1+1] - E[i_2+1][i_2+i_1-1] + B[i_1+1][i_2] - 6;
            B[i_1-1][i_2] = D[i_1-1] + E[i_2][i_1-1] * E[i_2][i_1+1] - 6;
        }
    }
#pragma endscop
...
kernel_3314151213_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
