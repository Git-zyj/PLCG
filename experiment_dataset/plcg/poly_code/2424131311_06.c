...
void kernel_2424131311_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M+1, PB_L-2); i_2++) {
            B[i_2+1] = E[i_2+1][i_1] - E[i_2-1][i_1] * E[i_1-1][0] * F[i_1] - E[i_2+i_1-1][0] - B[i_2+2] * D[i_1+1][i_1+1] - 5;
        }
        C[i_1] = B[i_1+1] + B[i_1] - B[i_1+2] * 4;
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            D[i_1][i_1] = F[1] + E[i_3][i_1] * C[i_3] * D[i_1+1][i_1+1] + 3;
        }
    }
#pragma endscop
...
kernel_2424131311_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
