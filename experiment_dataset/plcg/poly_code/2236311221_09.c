...
void kernel_2236311221_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            A[i_3+1] = C[i_3] * D[i_3-1][i_2+1] * A[i_3+1] - A[i_2+3] - 6;
        }
    }
#pragma endscop
...
kernel_2236311221_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
