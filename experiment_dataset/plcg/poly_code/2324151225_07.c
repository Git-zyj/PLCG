...
void kernel_2324151225_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[0] * D[i_0] - D[0] + D[i_0] + A[i_0-2] * A[i_0+2] + 4;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
            B[i_1+1][i_2-1] = E[i_2][i_1-1] + E[i_1][i_2] + E[i_1][i_2+1] + E[i_2-1][i_1] + E[i_2][i_1] - E[i_1][i_2] * E[i_1][i_2] + B[i_1-1][i_2-1] - 5;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        A[i_3] = D[0] - D[i_3] * D[i_3] + 4;
    }
#pragma endscop
...
kernel_2324151225_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
