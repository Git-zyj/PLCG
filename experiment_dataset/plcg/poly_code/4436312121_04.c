...
void kernel_4436312121_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_0][i_2][0] = A[i_0][i_2+1][0] + 3;
            }
        }
        for (int i_3 = 0; i_3 < min(-i_0+PB_M, i_0); i_3++) {
            for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
                B[i_3+i_0][i_4+2][i_4-2] = 5;
            }
        }
    }
        ...
        for (int i_6 = 0; i_6 < -i_5+PB_M+2; i_6++) {
            for (int i_7 = 2; i_7 < PB_M; i_7++) {
                for (int i_8 = 0; i_8 < -i_7+PB_M; i_8++) {
                    C[i_8+i_7][i_6+i_5-2][i_7-1][i_7] = C[i_8+i_7-2][i_6+i_5-2][i_7-1][i_7] + 5;
                }
            }
        }
    }
    for (int i_9 = 0; i_9 < PB_M; i_9++) {
        for (int j_0 = i_9; j_0 < PB_M-2; j_0++) {
            for (int j_1 = 0; j_1 < PB_N; j_1++) {
                D[j_1][i_9][j_0] = E[i_9][j_0] - D[j_1][i_9][j_0+2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4436312121_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
