...
void kernel_3324211321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < i_0; i_2++) {
                A[i_1][i_2-1][i_0] = A[i_2][i_1-2][i_0] + A[i_1-2][i_2-2][i_0] * 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            B[i_4][i_3] = D[i_3-1] * B[i_3][i_4-1] + B[i_4][i_3-1] - B[i_4][i_3+2] + B[i_4-1][i_3] * 5;
        }
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        for (int i_6 = 3; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 0; i_7 < PB_P-2; i_7++) {
                C[i_6][i_6-1][i_7] = C[i_5][i_5-1][i_7+2] * C[i_6-2][i_6-3][i_7] + C[i_6+1][i_6][i_7+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3324211321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
