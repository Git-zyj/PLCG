...
void kernel_2334112211_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L+2; i_0++) {
        A[i_0] = B[i_0-3] + 6;
        B[i_0-2] = D[i_0] - A[i_0] - A[i_0-1] + 2;
    }
    for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
        for (int i_2 = PB_P - 1; i_2 >= 0; i_2--) {
            C[i_1+2][i_1] = C[i_1+3][i_1+1] + 5;
        }
    }
#pragma endscop
...
kernel_2334112211_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
