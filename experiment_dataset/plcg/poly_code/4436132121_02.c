...
void kernel_4436132121_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                    A[i_0][i_2][i_1][i_3] = E[i_3][i_1+1][i_0][2] - A[i_2][i_3+1][i_1][i_0] * 1;
                }
            }
        }
    ...
    for (int i_4 = 2; i_4 < PB_M+2; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
            B[i_5+1] = G[i_5][i_4-2] + G[i_4-2][i_5+1] * 3;
        }
    }
    for (int i_6 = 1; i_6 < PB_Q-2; i_6++) {
        for (int i_7 = 2; i_7 < i_6; i_7++) {
            for (int i_8 = max(0, -i_7+2); i_8 < min(i_6, -i_7+PB_M+2); i_8++) {
                for (int i_9 = 0; i_9 < PB_M; i_9++) {
                    D[i_7][i_8+1][i_9][i_6+2] = E[i_7+2][i_6][i_9][i_8+2] * E[i_7-2][i_9][i_6][i_8+i_7-2] + E[i_8][i_7+2][i_9][i_6] - E[i_7][i_9][i_6-1][i_8] - 1;
                }
            }
        }
    }
...
kernel_4436132121_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
