...
void kernel_4234212311_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 0; i_1 < PB_N; i_1++) {
            A[i_1] = C[i_1][i_0-2] + A[i_1+1] + 1;
        }
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            B[i_2][0] = B[i_2+1][0] + B[i_2-1][0] - B[i_2+1][0] + B[i_2+1][0] * 6;
        }
    }
#pragma endscop
...
kernel_4234212311_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
