...
void kernel_2432311211_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            A[i_0][i_1] = B[i_0-1][i_0-1] - 4;
        }
    ...
    for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            C[i_3+1][i_4+1] = C[i_3][i_4+1] * 4;
        }
    }
    for (int i_5 = 1; i_5 < PB_P-2; i_5++) {
        D[i_5-1][i_5+1] = E[i_5+1] + D[i_5][i_5+2] + 5;
    }
#pragma endscop
...
kernel_2432311211_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
