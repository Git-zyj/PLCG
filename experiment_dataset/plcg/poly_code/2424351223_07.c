...
void kernel_2424351223_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_M-1); i_1++) {
            B[i_1-1] = E[i_1+i_0] * F[i_1+1][i_0] + G[i_1][i_0] + F[i_1][i_0] - H[i_1+i_0][i_0] * F[i_1][i_0] + 6;
            B[i_1-1] = H[i_0][i_0+1] - H[i_1][i_0+1] * B[i_1+1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
        B[i_2-1] = I[i_2] - I[i_2] + J[i_2] * H[i_2][i_2-1] - I[i_2] - I[i_2] + 1;
    }
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < -i_3+PB_P+1; i_4++) {
            D[i_4][i_3] = H[i_4][i_4+i_3-1] - H[i_4-1][i_3+1] + H[i_3][i_4] - D[i_4+1][i_3] * D[i_4-2][i_3+2] - 3;
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2424351223_07(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
