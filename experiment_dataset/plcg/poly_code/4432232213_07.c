...
void kernel_4432232213_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-2; i_0++) {
        A[i_0-2] = E[i_0+2] * F[i_0+1] + F[i_0] + E[i_0-2] + A[i_0-1] * A[i_0-3] + 6;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_P-2, -i_1+PB_M+2); i_2++) {
            for (int i_3 = 1; i_3 < PB_M; i_3++) {
                for (int i_4 = 0; i_4 < i_2; i_4++) {
                    D[i_2+i_1-2][i_3][i_4][i_2] = G[i_2+1][i_4][i_1][i_2+2] - H[i_4][i_2][i_3-1] * 4;
                }
            }
        }
    }
    for (int i_5 = 3; i_5 < PB_L; i_5++) {
        C[i_5] = F[i_5-2] + A[i_5-2] - 6;
        for (int i_6 = 1; i_6 < min(min(PB_P-2, i_5), -i_5+PB_M+2); i_6++) {
            for (int i_7 = 0; i_7 < PB_M-2; i_7++) {
                for (int i_8 = 1; i_8 < -i_5+PB_L; i_8++) {
                    D[i_6+i_5-2][i_7][i_8][i_6] = G[i_8+1][i_7+2][i_8+i_5][i_6+2] - G[i_6+2][i_5][i_8][i_7] - I[i_6][i_5][i_8][i_7] + D[i_7+i_6-1][i_8][i_5][i_6] + D[i_6+i_5-2][i_7][i_8-1][i_6] - D[i_6+i_5-3][i_7+1][i_8][i_6] * 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4432232213_07(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
