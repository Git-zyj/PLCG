...
void kernel_3332352123_07(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        C[i_0-1] = D[0] - E[i_0] - F[i_0][0] * G[i_0][0] - D[i_0+2] - 4;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_N-2, -i_1+PB_N); i_2++) {
            B[i_1][i_2] = H[i_2][i_1] * H[i_2+i_1][i_2] * I[i_1] - H[i_2][i_1] - H[i_2+2][i_1] + B[i_1-1][i_2] - 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_N-1; i_3++) {
        C[i_3-1] = E[i_3] - E[i_3+1] + I[i_3] - I[i_3] + 1;
    }
#pragma endscop
...
kernel_3332352123_07(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
