...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            D[i_1-2] = G[i_1+1][i_2] * G[i_2][i_1] + H[i_2][i_1] - 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_L+1; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_L-2, i_3); i_4++) {
            D[i_4] = H[i_4-1][i_4] + G[i_3][i_4] * I[i_3-2][i_4-2][i_4] - F[i_4-1] * G[i_4][i_3-1] * G[i_3][i_4+2] - D[i_4-1] - D[i_4-2] * 1;
        }
    }
#pragma endscop
...
kernel_2434232223_06(xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
