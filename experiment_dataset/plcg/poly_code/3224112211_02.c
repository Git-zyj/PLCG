...
void kernel_3224112211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = C[i_0] - A[i_0+1] * 5;
        for (int i_1 = PB_L-1 - 1; i_1 >= 1; i_1--) {
            B[i_1+1][i_1] = A[i_0+1] + B[i_1][i_1-1] - 4;
        }
    }
#pragma endscop
...
kernel_3224112211_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
