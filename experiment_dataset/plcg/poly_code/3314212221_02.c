...
void kernel_3314212221_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = i_1; i_2 < PB_Q; i_2++) {
                A[i_0][i_2+2][i_2] = A[i_0-1][i_2+2][i_2] - A[i_0+1][i_2+2][i_2] + 2;
            }
        }
        for (int i_3 = 0; i_3 < PB_Q; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                B[i_0-1][i_0+1][i_4+1] = D[i_0][i_4][i_3] * D[i_4][i_0-1][i_3] + 5;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_Q; i_5++) {
        for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
            for (int i_7 = 0; i_7 < min(i_5, -i_6+PB_L); i_7++) {
                C[i_6][i_7+i_6-2] = C[i_6][i_7+i_6-1] * C[i_6+2][i_7+i_6] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3314212221_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
