...
void kernel_3232311223_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
                A[i_0][i_2][i_1] = A[i_0+1][i_1][i_2+1] - A[i_0][i_2+1][i_1] + A[i_0][i_2-1][i_1+2] * 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L+1; i_3++) {
        B[i_3][i_3] = C[i_3-1] * B[i_3-2][i_3-2] - 4;
    }
#pragma endscop
...
kernel_3232311223_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
