...
void kernel_4424251225_02(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < min(i_0, -i_0+PB_L-1); i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_Q; i_3++) {
                    C[i_3+2][i_0][i_2+2][i_1-1] = E[i_1+1][i_2-1][i_0-1][i_3+i_2-1] * F[i_2][i_2+1][i_1-1][i_0] * F[i_1-1][i_2][i_3+i_0][i_1-1] + E[i_1][i_2+i_0+1][i_2-1][i_3] - F[i_2][i_0-1][i_1][i_2] * F[i_3+1][i_0+1][i_1][i_2] + F[i_2-1][i_1][i_0][i_3] - 2;
                }
            }
        }
        B[i_0+1] = G[i_0][i_0] + H[i_0] + G[i_0][i_0+1] + I[i_0] * I[i_0-1] - I[i_0-1] * I[i_0] + G[i_0][i_0] - H[i_0+1] * B[i_0] * 2;
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_Q; i_5++) {
            for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
                for (int i_7 = 1; i_7 < PB_L; i_7++) {
                    C[i_4][i_6][i_7][i_5] = J[i_6][i_7][i_5+1] - C[i_4+2][i_6-2][i_7-1][i_5] * C[i_4][i_6+2][i_7][i_5] * 5;
                }
            }
        }
    }
    for (int i_8 = 0; i_8 < PB_L-1; i_8++) {
        for (int i_9 = 1; i_9 < PB_L+1; i_9++) {
            B[i_9-1] = G[i_8][i_9-1] + K[i_9][i_8+1] + 3;
        }
    }
#pragma endscop
...
init_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4424251225_02(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K)));
