...
void kernel_2432252325_07(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < PB_N; i_1++) {
            D[i_1-3] = E[i_1] * F[i_1+2] - F[i_1] + 4;
        }
    }
    for (int i_2 = 3; i_2 < PB_L; i_2++) {
        D[i_2-1] = E[i_2] - F[i_2] * E[i_2] * G[i_2][i_2] * D[i_2-1] + 3;
        D[i_2-3] = F[2] - E[i_2] - F[i_2-2] + H[i_2+1][i_2] - F[i_2] + 3;
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_N, PB_P); i_4++) {
            D[i_4-1] = G[i_4+i_3][i_3] - G[i_3][i_4-2] * G[i_4-1][i_4+1] + G[i_4-1][i_3] + G[i_4][i_3] - G[i_3-1][i_3] - I[i_3+2][i_4][i_4] * D[i_4-2] * D[i_4+1] * 4;
        }
    }
#pragma endscop
...
kernel_2432252325_07(xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
