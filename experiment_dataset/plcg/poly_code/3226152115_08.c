...
void kernel_3226152115_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 3; i_2 < min(PB_N+2, i_0); i_2++) {
                A[i_2-2][i_1][i_2] = C[i_2+1][i_0][i_1+2] - C[i_1][i_0][i_2] * C[i_2][i_1][i_2] * C[i_2][i_0][i_1] - A[i_2-3][i_1][i_2-1] - 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_N+2, -i_3+PB_M); i_4++) {
            A[i_4-2][i_3][i_4] = D[i_4][i_3] - C[i_3][i_4+i_3][0] * D[i_3][0] * D[i_4][i_3] + D[i_3][1] + D[i_4-1][i_3] - 6;
        }
    }
#pragma endscop
...
kernel_3226152115_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
