...
void kernel_2222132321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P+2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            A[i_0] = C[i_1+2] + D[i_1][i_0-2] * A[i_1-2] * A[i_0+2] - A[i_1+2] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        A[i_2] = C[i_2] * C[i_2] + C[i_2] - C[i_2-2] + 5;
    }
#pragma endscop
...
kernel_2222132321_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
