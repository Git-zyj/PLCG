...
void kernel_4436232121_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
        ...
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                for (int i_3 = 0; i_3 < PB_P; i_3++) {
                    A[i_0+2][i_2][i_1][i_2] = E[i_3][i_2+1][i_0][i_1-1] + E[i_2][i_3][i_2][i_1] * E[i_0][i_2][i_3][i_1] - 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P-2; i_4++) {
        B[i_4] = F[i_4+2] - G[i_4] * G[i_4] + 1;
        for (int i_5 = max(i_4, 2); i_5 < PB_L-2; i_5++) {
            for (int i_6 = 0; i_6 < PB_P; i_6++) {
                for (int i_7 = 0; i_7 < PB_P-1; i_7++) {
                    C[i_4-1][i_7+1][i_6][0] = H[i_7][i_4][i_5][i_6] - I[i_7][i_5+2][i_6] * J[i_7+1][i_5-2][i_7] * 4;
                }
            }
        }
        for (int i_8 = 0; i_8 < i_4; i_8++) {
            for (int i_9 = 1; i_9 < -i_4+PB_P+1; i_9++) {
                for (int j_0 = 1; j_0 < min(min(-i_8+PB_P, PB_P-2), -i_9+PB_P); j_0++) {
                    D[i_4][i_9][j_0+i_9][j_0+2] = H[i_8][j_0-1][i_9][i_4+1] * E[i_9][i_4+1][j_0+i_8][i_9] + D[j_0][i_9-1][i_9+i_4-1][i_4+2] * 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4436232121_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
