...
void kernel_2322212121_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_L+1; i_4++) {
            C[i_3][i_4-1] = D[i_4-1][i_3] * C[i_3+2][i_4-1] - 1;
        }
    }
#pragma endscop
...
kernel_2322212121_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
