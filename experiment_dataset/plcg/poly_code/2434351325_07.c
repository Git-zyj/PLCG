...
void kernel_2434351325_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_1-1] = E[i_1] - F[i_1][i_0] + G[i_1][i_0-1] + G[i_1][i_0] * G[i_1-1][i_0] - A[i_1+1] - 1;
        }
    }
    for (int i_2 = 3; i_2 < PB_N; i_2++) {
        B[i_2+1] = H[i_2-1] * E[i_2+1] + E[i_2-1] - I[i_2] * A[i_2-3] * B[i_2] + 3;
        for (int i_3 = -i_2+PB_N - 1; i_3 >= i_2; i_3--) {
            B[i_3+1] = F[i_2-1][i_2] + J[i_3+i_2][i_3+1] * J[i_3+i_2][1] * F[i_2][0] - A[i_3-1] + 3;
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        A[i_4-1] = I[i_4] + E[i_4-1] + E[i_4] + I[i_4] * E[i_4] - A[i_4-1] * B[i_4+1] - 6;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2434351325_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
