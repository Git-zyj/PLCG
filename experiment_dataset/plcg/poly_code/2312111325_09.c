...
void kernel_2312111325_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_M; i_1++) {
        ...
        for (int i_2 = i_1; i_2 < PB_L-2; i_2++) {
            C[i_1][i_2] = B[i_2-1] * B[i_2-3] + C[i_1][i_2-1] * C[i_1][i_2+2] * 5;
        }
    }
#pragma endscop
...
kernel_2312111325_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
