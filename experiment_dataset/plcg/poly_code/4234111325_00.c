...
void kernel_4234111325_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_P-3; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            B[i_3][i_2+1] = C[i_3+i_2][i_3-1] * B[i_3+1][i_2+1] * B[i_3-1][i_2+1] * B[i_3][i_2+3] * 6;
        }
    }
#pragma endscop
...
kernel_4234111325_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
