...
void kernel_3216212223_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_M-1; i_2++) {
            B[i_2] = C[i_2][i_1] - B[i_2-2] + B[i_2+1] + 6;
        }
    }
#pragma endscop
...
kernel_3216212223_03(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
