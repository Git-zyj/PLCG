...
void kernel_4216212121_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_4 = 0; i_4 < PB_N-2; i_4++) {
        for (int i_5 = i_4; i_5 < min(-i_4+PB_M-2, PB_N-4); i_5++) {
            B[i_4+2][i_5+i_4+2] = C[i_5] * B[i_5+4][i_5+i_4+2] - 2;
        }
    }
#pragma endscop
...
kernel_4216212121_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
