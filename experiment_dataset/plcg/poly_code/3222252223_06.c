...
void kernel_3222252223_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, i_1); i_2++) {
                A[i_0-2][i_1][i_2+1] = C[i_2+i_0][i_2-1][i_1] - C[i_0][i_2][i_2] * D[i_1][i_0+2][i_2] + D[i_1][i_0+1][i_2-1] - D[i_1+i_0][i_2][i_1-1] * E[i_2][i_1] * 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_L, PB_M-4); i_4++) {
            for (int i_5 = 0; i_5 < min(-i_4+PB_M+1, PB_M-2); i_5++) {
                B[i_3+2][i_5+2][i_4] = D[i_5+i_4-1][i_3-1][i_4] * C[i_5][i_4+i_3][i_5] + D[i_5+2][i_3-1][i_4-1] - D[i_5][i_3-1][i_4] * B[i_5+2][i_3+2][i_4+1] + B[i_3+2][i_5+2][i_4-1] - B[i_4+4][i_3+2][i_5] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3222252223_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
