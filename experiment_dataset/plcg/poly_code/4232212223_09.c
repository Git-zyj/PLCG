...
void kernel_4232212223_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-3; i_1++) {
            for (int i_2 = 2; i_2 < min(i_0, -i_1+PB_P-2); i_2++) {
                for (int i_3 = max(-i_2+2, 1); i_3 < PB_L; i_3++) {
                    A[i_3][i_2+i_1+1][i_1+2] = A[i_3-1][i_2+i_1+2][i_1+3] - A[i_3][i_2+i_1+2][i_1+1] * A[i_1-2][i_3+i_2-2][i_2] * 5;
                }
            }
        }
        B[i_0] = C[i_0+2] + D[i_0] - B[i_0+1] + 3;
    }
#pragma endscop
...
kernel_4232212223_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
