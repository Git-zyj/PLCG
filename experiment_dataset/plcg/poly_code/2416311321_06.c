...
void kernel_2416311321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0+1] = E[i_0] * A[i_0+2] + C[i_0] - 5;
        for (int i_1 = max(2, i_0); i_1 < PB_N-1; i_1++) {
            A[i_1-1] = F[i_0][i_1-1] + G[i_1+1][i_0] - A[i_0+1] * C[i_1] + C[i_1-2] + C[i_0] + 2;
        }
        C[i_0] = C[i_0-1] - 6;
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_L; i_3++) {
            D[i_2][i_3] = H[i_3+1] - D[i_2][i_3-2] + 6;
        }
    }
#pragma endscop
...
kernel_2416311321_06(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
