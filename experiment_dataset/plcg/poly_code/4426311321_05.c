...
void kernel_4426311321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_P-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_Q-1; i_2++) {
                A[i_0-1][i_1][i_2] = A[i_0-2][i_1][i_2] + A[i_0+1][i_1][i_2] - A[i_0-1][i_1][i_2+1] + A[i_0-1][i_1+1][i_2-2] - A[i_1-3][i_0][i_2] * 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        B[i_3-1] = E[i_3] + B[i_3+1] - 3;
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 1; i_5 < min(PB_L-3, -i_4+PB_M); i_5++) {
            C[i_4][i_5+1] = F[i_4][i_5] - C[i_4][i_5-1] + C[i_4+1][i_5+1] - C[i_4+2][i_5+1] + C[i_4-2][i_5+3] * 1;
            for (int i_6 = 0; i_6 < PB_P-2; i_6++) {
                for (int i_7 = i_6; i_7 < -i_4+PB_M-1; i_7++) {
                    D[i_5+i_4][i_7][i_6][i_4] = D[i_7+i_4+1][i_5][i_6][i_4-1] - D[i_7+i_5][i_4+1][i_6+2][i_7] * 6;
                }
            }
        }
    }
...
kernel_4426311321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
