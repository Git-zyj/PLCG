...
void kernel_2212111123_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L - 1; i_0 >= 0; i_0--) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_0][i_1+3] = C[i_1][i_0] + 4;
        }
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            B[i_0][i_2+1] = C[i_0][i_2+1] + B[i_0][i_2] - 1;
        }
    }
#pragma endscop
...
kernel_2212111123_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
