...
void kernel_3226352311_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_N); i_1++) {
            A[i_0][i_1] = C[i_1+i_0][i_1] - D[i_0][0] + E[i_0-1] * C[i_1][i_0] + C[i_1][i_0-2] - A[i_0+1][i_1] - A[i_0+1][i_1+1] * A[i_0-1][i_1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
            B[i_2-1][i_3+1][i_3] = D[i_3+2][i_2+1] * F[i_3-2][i_2-1] * D[i_3][i_3] + C[i_2+1][i_3] * C[i_2-1][i_3+2] * A[i_2][i_3] * B[i_2][i_3+1][i_3] + B[i_2-1][i_3][i_3-1] - 3;
        }
    }
#pragma endscop
...
kernel_3226352311_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
