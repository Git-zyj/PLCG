...
void kernel_3232111213_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
            for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
                B[i_2-1][i_0+1] = C[i_0][i_2][i_3+1] * C[i_0][i_3][i_2+1] * A[i_0-1] - A[i_2] + B[i_3-1][i_2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3232111213_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
