...
void kernel_4236331225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_N; i_1++) {
            A[i_0][i_0] = C[i_1+i_0] * C[i_1+i_0] - A[i_0+2][i_0+2] * A[i_0-2][i_0-2] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        A[i_2][i_2] = D[i_2-1] + C[i_2-1] + D[i_2] - 2;
    }
#pragma endscop
...
kernel_4236331225_01(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
