...
void kernel_4424331213_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] * E[i_0] + F[i_0+1] - A[i_0-1] * B[i_0] * 6;
    }
    for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
        B[i_1] = E[i_1+1] - E[i_1+1] - G[i_1] + B[i_1-1] - 2;
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_1+PB_M, PB_L-1); i_3++) {
                C[i_3+i_1][i_2][i_1] = H[i_3+1][i_1][i_2] - I[i_2][i_1][i_3-1] * J[i_1][i_3][i_2] - C[i_3+i_1-1][i_2][i_1] - 1;
            }
        }
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < min(PB_N-1, -i_4+PB_M); i_5++) {
                for (int i_6 = 1; i_6 < min(-i_4+PB_P+1, -i_1+PB_P); i_6++) {
                    D[i_5+1][i_4-1][i_6+i_1][i_5] = K[i_4][i_1][i_5][i_6-1] + L[i_5+1][i_1+1][i_4][i_5-1] - K[i_6-1][i_1][i_6][i_5+1] - C[i_5+i_4][i_1+1][i_5] * D[i_5+1][i_1-2][i_6+i_4-1][i_5] * 2;
                }
            }
        }
    }
...
kernel_4424331213_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L));
