...
void kernel_4312352325_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        A[i_0+2] = D[i_0+1] * E[i_0] - E[i_0] * F[i_0+1] - A[i_0+3] + A[i_0] - A[i_0+1] + 2;
    }
    for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 4; i_3 < PB_L+1; i_3++) {
                for (int i_4 = 1; i_4 < min(-i_3+PB_P, -i_1+PB_P-2); i_4++) {
                    B[i_2][i_3-2][i_4][i_1+2] = G[i_4-1][i_1][i_2][i_1+2] - H[i_3][i_4-1][i_1][i_2+2] + I[i_2][i_4+i_3][i_1+1][i_2][i_1] * J[i_2][i_3][i_4] - K[i_4][i_1][i_2][i_4+i_1+2] * B[i_1][i_3-1][i_4][i_2+2] + B[i_2][i_3-2][i_4][i_1+1] - B[i_2-1][i_3-4][i_4][i_1+2] - 1;
                }
            }
        }
        C[i_1] = F[i_1+2] * E[i_1] - F[i_1] + E[i_1] * F[i_1] - A[i_1+2] + C[i_1+1] - C[i_1+2] + 1;
    }
#pragma endscop
...
kernel_4312352325_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K));
