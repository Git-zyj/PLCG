...
void kernel_4432131115_06(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_Q; i_3++) {
                    D[i_3][i_2][i_2+1] = E[i_2][i_0][i_3+i_1][i_0+1] - F[i_0-1][i_3][i_0][i_2][i_3] - 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
            B[i_5-1] = G[i_5][i_4] + G[i_5+1][i_4] - H[i_5] - H[i_5] + 5;
        }
        B[i_4] = H[i_4] - B[i_4-2] - 6;
    }
    for (int i_6 = 0; i_6 < PB_Q; i_6++) {
        for (int i_7 = 2; i_7 < PB_M-1; i_7++) {
            for (int i_8 = 1; i_8 < PB_L; i_8++) {
                D[i_7-1][i_8-1][i_8] = I[i_7+1][i_8+1][i_6] * G[i_8-1][i_7] * G[i_8][i_7] * D[i_8-1][i_7-2][i_7-1] + 4;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4432131115_06(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,yd,zd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
