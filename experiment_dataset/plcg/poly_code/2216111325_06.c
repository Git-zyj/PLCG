...
void kernel_2216111325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_L+1); i_1++) {
            A[i_0][i_1+i_0-1] = A[i_0+1][i_1+i_0-2] * A[i_0][i_1+i_0-3] + A[i_0-2][i_1+i_0-3] + 3;
        }
    }
    for (int i_2 = 2; i_2 < PB_L+2; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_M, -i_2+PB_L+2); i_3++) {
            A[i_3][i_3+i_2-2] = C[i_3-1] + 4;
        }
    }
#pragma endscop
...
kernel_2216111325_06(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
