...
void kernel_2432351121_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            A[i_0+1][i_1] = E[i_0+1] * F[i_1-1][i_0][i_0+1] * G[i_1][i_0+1] * H[i_1+1] * G[i_1][i_0] - A[i_0+1][i_1+2] * 5;
        }
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            B[i_2][i_0-1] = G[i_0][i_2+1] * I[i_2][i_0+1] + I[i_2][0] + I[i_2][i_0] * G[i_2-1][i_0] * I[i_2][i_0] - B[i_2][i_0-3] + 3;
        }
        for (int i_3 = 0; i_3 < min(-i_0+PB_M, PB_Q); i_3++) {
            C[i_0+1][i_3] = G[i_3+i_0][i_3] * E[i_3] - G[i_3][0] - J[i_3][i_0] + E[i_3] + F[i_3][i_0-1][i_3] * 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            D[i_4][i_5] = D[i_5][i_4+1] - 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2432351121_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
