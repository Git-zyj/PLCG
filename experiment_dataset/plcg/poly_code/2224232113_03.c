...
void kernel_2224232113_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+i_0][i_0-2] = C[i_1+i_0+2][2] + C[i_1+2][i_0] + C[i_1][i_0+2] * A[i_1+i_0+1][i_0-2] * 2;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_0][i_2+2] = D[i_2-1][i_0] + D[i_2][i_0] * B[i_0][i_2+3] - 2;
        }
    }
#pragma endscop
...
kernel_2224232113_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
