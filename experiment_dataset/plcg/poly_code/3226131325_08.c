...
void kernel_3226131325_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < -i_0+PB_M+1; i_1++) {
            A[i_1-1] = C[i_1+i_0-1][i_0] + D[i_1] - C[i_1][i_0] * D[i_1] + C[i_1][i_0] + A[i_1-3] - 5;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
            A[i_2-3] = C[i_3+i_2][i_3+1] * A[i_3-1] - A[i_2-1] * 1;
        }
    }
#pragma endscop
...
kernel_3226131325_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
