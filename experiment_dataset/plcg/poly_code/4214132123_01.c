...
void kernel_4214132123_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N; i_1++) {
            A[i_1+i_0][i_0] = C[i_1][i_0] + D[i_0] + A[i_1+i_0-1][i_0-1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < min(-i_2+PB_N+1, i_2); i_3++) {
            A[i_3+i_2-1][i_3] = C[i_3][i_2] + D[i_3] * C[i_3][i_3-2] + 4;
        }
    }
#pragma endscop
...
kernel_4214132123_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
