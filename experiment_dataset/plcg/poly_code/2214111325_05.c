...
void kernel_2214111325_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-3; i_1++) {
            B[i_1+i_0+3][i_0] = C[i_1][i_0-1] * 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_L, -i_2+PB_M-2); i_3++) {
            B[i_3+i_2+1][i_3] = B[i_3+i_2+2][i_3] - B[i_3+i_2-1][i_3] - B[i_3+i_2][i_3-1] * 5;
        }
    }
#pragma endscop
...
kernel_2214111325_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
