...
void kernel_2316352315_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            B[i_0] = D[i_1+i_0] + E[i_1] - F[i_1][i_0] * B[i_0] + 4;
        }
        B[i_0] = G[i_0] * F[i_0-1][i_0+2] + G[i_0] - D[i_0-2] * E[i_0-1] + G[i_0] - E[i_0-1] * B[i_0-1] - B[i_0+1] + B[i_0-1] * 2;
    }
    for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < min(PB_P, -i_2+PB_L-1); i_3++) {
            C[i_3-1][i_2] = H[i_2][1] - F[i_3][i_2+2] + F[i_2][i_3] - F[i_3+i_2+1][2] - C[i_3-2][i_2-1] - C[i_3-2][i_2] + 1;
        }
    }
#pragma endscop
...
kernel_2316352315_07(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
