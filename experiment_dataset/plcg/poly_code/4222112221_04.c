...
void kernel_4222112221_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_N+1; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                A[i_1-1][i_0][i_2] = A[i_1-1][i_0+2][i_2] + 5;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_Q; i_4++) {
            for (int i_5 = 2; i_5 < PB_M; i_5++) {
                for (int i_6 = 2; i_6 < PB_M-1; i_6++) {
                    B[i_4-1][i_5][i_6+1] = C[i_6-2][i_4][0] - A[i_4-1][i_6][i_5] + B[i_6-1][i_5-2][i_3+1] * 2;
                }
            }
        }
    }
...
kernel_4222112221_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
