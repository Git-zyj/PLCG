...
void kernel_2216151313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < min(-i_0+PB_M, PB_L-1); i_1++) {
            A[i_1] = C[i_1][i_0] * C[i_1+i_0][0] * C[i_0][i_1+1] * C[i_0][i_1] + D[i_1] * C[i_1-1][i_0] * A[i_1+1] - A[i_0+1] - A[i_1-1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            A[i_2+1] = D[i_2+1] * D[i_3] + C[i_3][1] * C[i_3+1][i_2] - 2;
        }
    }
#pragma endscop
...
kernel_2216151313_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
