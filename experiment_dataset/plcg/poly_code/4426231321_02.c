...
void kernel_4426231321_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_Q, -i_1+PB_Q-1); i_2++) {
                for (int i_3 = i_0; i_3 < min(min(-i_1+PB_Q+1, PB_M-1), -i_2+PB_Q-2); i_3++) {
                    A[i_0+1][i_0][i_3+i_2][0] = E[i_1][i_2+1][i_0+1][i_3+1] * F[i_0][i_3][i_1][i_3] + A[i_3+1][i_3][i_2+i_1+1][0] * A[i_1+1][i_1][i_3+i_2+2][0] - A[i_1+3][i_1+2][i_2+i_0][0] + A[i_0+2][i_0+1][i_3+i_1-1][0] - A[i_0][i_0-1][i_3+i_2+1][0] + A[i_0+3][i_0+2][i_3+i_2][0] - 2;
                }
            }
        }
        for (int i_4 = 0; i_4 < min(i_0, -i_0+PB_L-1); i_4++) {
            C[i_0+1][i_4+1][0] = G[i_4][i_0+1] - H[i_0][i_4+i_0+1] - H[i_4][i_0+1] + G[i_4+1][i_0] - 1;
        }
    }
    for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
        for (int i_6 = max(2, i_5); i_6 < PB_L-2; i_6++) {
            for (int i_7 = 2; i_7 < PB_M; i_7++) {
                C[i_6][i_5+1][0] = I[i_7-1][i_6][i_5] * G[i_7-1][i_6] * C[i_6-2][i_5+2][0] + C[i_6+2][i_5+1][0] * 5;
                for (int i_8 = 0; i_8 < min(min(-i_5+PB_P, i_5), -i_7+PB_M); i_8++) {
                    D[i_8+1][i_5][i_8][i_6] = F[i_8+i_7][i_8][i_6][i_5] + F[i_7][i_5][i_7][i_6] + F[i_8+i_7][i_5][i_6][i_5] * F[i_7][i_6][i_8+i_5][0] - D[i_5+2][i_8][i_5+1][i_7-2] + 2;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4426231321_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
