...
void kernel_2214212313_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = 2;
        for (int i_1 = 1; i_1 < min(PB_N-3, -i_0+PB_L-1); i_1++) {
            B[i_1+2][i_0][i_1+i_0] = C[0] + A[i_1] * A[i_0] - A[i_1-1] + B[i_1+3][i_0][i_1+i_0+1] - B[i_1+2][i_0-1][i_1+i_0-1] - B[i_1+3][i_0-1][i_1+i_0] * 4;
        }
    }
#pragma endscop
...
kernel_2214212313_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
