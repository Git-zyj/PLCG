...
void kernel_4426111215_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
    ...
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_Q; i_2++) {
            for (int i_3 = 1; i_3 < i_2; i_3++) {
                D[i_1][i_1-1][i_3-1] = F[i_3][i_1][i_2] * G[i_2][i_3] + 3;
            }
            for (int i_4 = 1; i_4 < PB_M; i_4++) {
                for (int i_5 = i_1; i_5 < PB_L; i_5++) {
                    D[i_4][i_4-1][i_2-1] = H[i_2][i_5][i_1][i_4-1] - 6;
                }
            }
        }
        for (int i_6 = i_1; i_6 < PB_L+1; i_6++) {
            for (int i_7 = 0; i_7 < i_6; i_7++) {
                D[i_1][i_1-1][i_6-1] = D[i_1-1][i_1-2][i_6-1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4426111215_03(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
