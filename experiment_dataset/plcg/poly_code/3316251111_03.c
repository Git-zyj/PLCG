...
void kernel_3316251111_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = D[i_0] - E[1] + D[i_0] + E[i_0] - D[i_0] * 5;
    }
    for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            B[i_1+1][i_2] = F[i_1][i_2-1] + G[i_2][i_1] * F[i_1+1][i_1] - A[i_2+1] + 6;
        }
        for (int i_3 = 0; i_3 < min(min(PB_Q, i_1), -i_1+PB_L+1); i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_Q; i_4++) {
                B[i_3+1][i_4] = H[i_4+1][i_3][i_1-1] + H[i_4][i_3+i_1-1][i_4+1] + G[i_4+1][i_3] - I[i_4][i_1-1][i_4+i_3] - H[i_4+i_3][i_1-1][i_1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3316251111_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
