...
void kernel_3326232111_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-2] = D[i_0-2] + D[1] - E[i_0+2] * D[i_0] - D[i_0] - 2;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        B[i_1] = D[i_1] * B[i_1-1] + 6;
        for (int i_2 = i_1; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                C[i_1][i_1][i_2] = F[i_3+2][i_1][i_2] * C[i_2+1][i_2+1][i_1] - C[i_1][i_1][i_2+1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3326232111_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
