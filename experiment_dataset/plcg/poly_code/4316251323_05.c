...
void kernel_4316251323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        A[i_0] = D[i_0] * D[i_0] * E[i_0] - E[i_0-1] - A[i_0-1] - A[i_0+2] - B[i_0] * B[i_0+1] + B[i_0-1] * B[i_0+2] * 4;
    }
    for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
        B[i_1] = E[1] + D[i_1-1] + D[i_1+1] + E[i_1] + A[i_1+1] - B[i_1+1] * 5;
        for (int i_2 = 1; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            for (int i_3 = i_2; i_3 < PB_Q; i_3++) {
                for (int i_4 = 1; i_4 < -i_3+PB_L; i_4++) {
                    C[i_2][i_3][i_1] = F[i_2-1][i_1][i_3][i_4] * G[i_3+1][i_2][i_4][i_1] - F[i_3][i_1-1][i_1][i_4] + F[i_4-1][i_1][i_2][i_3] * F[i_4+i_3][i_2][i_1][i_1] * G[i_4][i_2+i_1][i_3][i_1] * C[i_2][i_3+1][i_1] * 6;
                }
            }
        }
    }
...
kernel_4316251323_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
