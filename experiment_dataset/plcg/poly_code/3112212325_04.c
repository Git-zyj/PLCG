...
void kernel_3112212325_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_Q, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_L); i_2++) {
                A[i_1][i_2+i_0] = B[i_1-2][i_2][0] - A[i_1-2][i_2+i_0] - A[i_2][i_1+i_0-1] * A[i_2][i_1+i_0-2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3112212325_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
