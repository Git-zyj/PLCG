...
void kernel_3224211323_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            A[i_1+1] = A[i_1] - A[i_1+3] - A[i_1+2] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        A[i_2+1] = C[i_2] * D[i_2] + 3;
    }
#pragma endscop
...
kernel_3224211323_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
