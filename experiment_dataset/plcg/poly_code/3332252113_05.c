...
void kernel_3332252113_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_1][i_0+3] = D[i_1][i_0+2] + E[i_1] - F[i_1] + D[i_1][i_0] + G[i_1+1][i_0+1] * 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_N-1, PB_M-2); i_3++) {
            for (int i_4 = -i_3+PB_M-2 - 1; i_4 >= 0; i_4--) {
                B[i_4][i_3+2] = H[i_2][i_4][i_3] + I[i_3][i_2][i_4] - H[i_3+2][i_2][i_2] + I[i_4+i_3+2][i_2][i_4+i_3+1] * H[i_3+i_2+1][i_4][i_4+2] - B[i_4][i_3+1] + 6;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 0; i_6 < PB_L; i_6++) {
            B[i_6][i_5+2] = D[i_6][i_5] - I[0][i_6+1][i_5] + F[0] - G[i_6][i_5] + D[i_6][1] * 1;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3332252113_05(xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
