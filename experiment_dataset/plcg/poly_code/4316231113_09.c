...
void kernel_4316231113_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = i_0; i_1 < -i_0+PB_P-1; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_L, -i_0+PB_L); i_3++) {
                    A[i_0][i_1-1][i_3+1][0] = D[i_1][i_3+i_2][i_1+i_0+1][i_0] + D[i_2][i_0][i_3][i_1] + E[i_2][i_3+i_0][i_1+1][i_3] + 6;
                }
            }
        }
        B[i_0] = B[i_0+1] + B[i_0-1] * 4;
        for (int i_4 = max(2, i_0); i_4 < -i_0+PB_P; i_4++) {
            for (int i_5 = 0; i_5 < -i_0+PB_Q; i_5++) {
                for (int i_6 = 0; i_6 < -i_5+PB_M; i_6++) {
                    A[i_0][i_4-2][i_6][0] = F[i_6][i_4][i_5+i_0] - D[i_5][i_6][i_4][i_0+1] - F[i_6+i_5][i_4+i_0][i_4-1] - E[i_5][i_0][0][i_6] + 1;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4316231113_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
