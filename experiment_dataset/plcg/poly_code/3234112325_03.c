...
void kernel_3234112325_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0-2][i_0] = C[i_0] * A[i_0-1][i_0+1] + A[i_0][i_0+2] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_P-2; i_2++) {
            for (int i_3 = 0; i_3 < min(PB_Q, -i_2+PB_L-1); i_3++) {
                B[i_2][i_3+i_2] = D[i_1][i_2+2][i_3] * B[i_2-1][i_3+i_2+1] + B[i_3][i_3+i_2-1] + B[i_3][i_3+i_2+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3234112325_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
