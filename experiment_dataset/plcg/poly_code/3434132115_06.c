...
void kernel_3434132115_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[0] * E[i_0] + E[i_0+2] * A[i_0+1] * 3;
        A[i_0] = E[i_0] * E[i_0] + E[i_0] * A[i_0] + 4;
    }
    for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = 1; i_3 < -i_1+PB_P-2; i_3++) {
                C[i_2][i_2+2][i_3-1] = F[i_3-1][i_1][i_2] - G[i_2][i_3+i_1+2] * 5;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        A[i_4+1] = E[i_4] * E[i_4] * E[i_4+1] + 5;
    }
#pragma endscop
...
kernel_3434132115_06(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
