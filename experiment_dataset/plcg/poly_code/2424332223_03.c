...
void kernel_2424332223_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = E[i_0] + E[i_0] * E[i_0] * A[i_0+1] + A[i_0+4] - 3;
    }
    for (int i_1 = 4; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
            B[i_2-2][0] = F[i_1] * G[i_2+2][i_1] - B[i_2-1][0] * B[i_1-4][0] - 2;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        D[i_3-1] = H[i_3] + F[i_3] + H[i_3-2] * E[i_3] + I[i_3+2][i_3+1] - 6;
        D[i_3] = G[i_3][i_3] - E[i_3] * 5;
    }
#pragma endscop
...
kernel_2424332223_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
