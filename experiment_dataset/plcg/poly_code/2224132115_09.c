...
void kernel_2224132115_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_M-1); i_1++) {
            A[i_0][i_0+2] = C[i_1-1][i_0] + C[i_1+1][i_1+i_0] * D[i_1] - A[i_0+1][i_0+3] * 2;
        }
    }
    for (int i_2 = PB_L - 1; i_2 >= 0; i_2--) {
        A[i_2][i_2+2] = D[i_2] * D[i_2] + 4;
    }
#pragma endscop
...
kernel_2224132115_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
