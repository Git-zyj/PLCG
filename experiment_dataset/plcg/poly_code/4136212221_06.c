...
void kernel_4136212221_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P-2, i_0); i_1++) {
            for (int i_2 = 2; i_2 < min(i_0, -i_1+PB_P-2); i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_P; i_3++) {
                    A[i_0][i_3+i_1][i_3-1] = B[i_2-2][i_3][i_1+1] * A[i_3][i_2+i_1+2][i_1+1] * A[i_3][i_2+i_1+1][i_2] + 3;
                }
            }
        }
    }
...
kernel_4136212221_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
