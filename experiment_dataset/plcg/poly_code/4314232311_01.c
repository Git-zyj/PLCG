...
void kernel_4314232311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0] * E[i_0-1] + D[i_0] * A[i_0-1] + B[i_0+2] - 2;
        B[i_0+2] = E[i_0-2] + E[i_0] - F[i_0][i_0] - A[i_0] - A[i_0+1] * A[i_0-1] - 2;
    }
    for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            for (int i_3 = max(1, i_1); i_3 < PB_P-2; i_3++) {
                for (int i_4 = 0; i_4 < PB_P; i_4++) {
                    C[i_2][i_3+1][i_3+1] = G[i_3][i_1+1][i_3-1] + C[i_4][i_3+2][i_3+2] - C[i_2+1][i_3+1][i_3+1] * C[i_3][i_4][i_4] + 3;
                }
            }
        }
    }
...
kernel_4314232311_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
