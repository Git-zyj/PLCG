...
void kernel_3424351313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] + E[i_0+1] - F[i_0] - E[i_0] * F[i_0-1] - G[i_0] - F[i_0] - G[i_0] * A[i_0-1] - A[i_0+1] - A[i_0+1] - 5;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
            for (int i_3 = 1; i_3 < min(min(-i_1+PB_P, PB_M-1), -i_2+PB_M); i_3++) {
                B[i_3][i_2][i_2] = H[i_2][i_1][i_2] + I[i_1][i_3+i_2][i_3+i_1] + J[i_3][i_1][i_2] + J[i_2][i_3+1][i_1] + J[i_1][i_1-1][i_2-1] - H[i_2+i_1][i_3+i_1][i_2-1] - B[i_2][i_3-1][i_3-1] + B[i_3-1][i_2+1][i_2+1] * 3;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_M; i_5++) {
            C[i_5-1][0] = K[i_5-1][i_4] + L[i_5+i_4][0] * C[i_5][0] - 5;
        }
        for (int i_6 = 0; i_6 < -i_4+PB_M; i_6++) {
            D[i_4][i_6+i_4] = K[i_6][i_4] + M[i_4][i_6] - E[0] - M[i_6+i_4][i_4] * D[i_4-1][i_6+i_4-2] + D[i_4-1][i_6+i_4-1] + D[i_4+1][i_6+i_4+2] * D[i_4][i_6+i_4-1] - 2;
        }
    }
#pragma endscop
