...
void kernel_4116311215_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_Q-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_2+1][i_1-1][i_1+i_0] = B[i_1][i_0+1] * A[i_2+1][i_1-1][i_1+i_0+1] - A[i_2+1][i_1-1][i_1+i_0-1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4116311215_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
