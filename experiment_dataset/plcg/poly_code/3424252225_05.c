...
void kernel_3424252225_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_P; i_2++) {
                A[i_0+2][i_1-2][i_2] = E[i_1][i_2][i_0-1] - E[i_2-2][i_0][i_1+1] * E[i_2+1][i_1-2][i_0] - F[i_1][i_2+i_0][i_0] - F[i_0][i_1][i_2+1] + A[i_2+1][i_1-2][i_0-2] * A[i_2+2][i_0-2][i_1+2] - 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_N, PB_M-3); i_4++) {
            B[i_3+1][i_4+i_3] = G[i_4+2][i_3] * H[i_4][i_3] + G[i_3][i_4+2] + F[i_4-1][i_3+2][0] * G[i_4][i_3-2] * I[i_4+i_3] + G[i_3][i_4] - B[i_3+1][i_4+i_3+2] - 2;
            for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
                B[i_4+3][i_5+i_4+4] = E[i_4][i_5][i_3-1] + F[i_3][i_5+1][i_4] + E[1][i_5][i_3] - 3;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M; i_6++) {
        for (int i_7 = 0; i_7 < min(min(i_6, -i_6+PB_L), PB_M-1); i_7++) {
            B[i_7+1][i_7+i_6] = G[i_7][i_6] + J[i_6] * 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3424252225_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
