...
void kernel_4432332123_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_N-2; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_M+1, -i_0+PB_L-2); i_3++) {
                    A[i_2+2][i_1][i_3+i_0][i_1] = E[i_3+i_2-1][i_0][i_1][i_1] + E[i_1][i_0+1][i_3+i_0][i_2+1] - E[i_1][i_3][i_2][i_3] + F[i_3][i_1-2][i_1] * A[i_2+2][i_1+1][i_3+i_0+2][i_1+1] - 5;
                }
            }
        }
    }
    ...
    for (int i_7 = 0; i_7 < PB_Q-2; i_7++) {
        for (int i_8 = 2; i_8 < PB_P-2; i_8++) {
            for (int i_9 = 0; i_9 < i_8; i_9++) {
                D[i_8][i_9+2][i_7][i_8] = G[i_8-2][i_7] + H[i_9][i_9][i_7] + H[i_7][i_9][i_8+2] - 4;
            }
        }
        for (int j_0 = 0; j_0 < PB_L; j_0++) {
            for (int j_1 = 0; j_1 < PB_Q-2; j_1++) {
                D[j_0][j_1][i_7+2][j_0] = F[j_0][j_1][i_7] - F[i_7][j_0][j_1] - D[j_0][j_1+2][i_7+1][j_0] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4432332123_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
