...
void kernel_3224332111_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M; i_1++) {
            A[i_1] = C[i_0][i_1] - D[i_1][i_1] * A[i_1-1] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < -i_2+PB_M-2; i_3++) {
            B[i_3-2] = C[i_3][i_2] + D[i_3][i_2+2] - C[i_3+i_2][i_3-1] + D[i_3+i_2+2][i_2] + 3;
        }
    }
#pragma endscop
...
kernel_3224332111_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
