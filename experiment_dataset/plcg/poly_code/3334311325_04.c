...
void kernel_3334311325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = A[i_0+1] * A[i_0] * A[i_0-3] - 1;
    }
    ...
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_L-1); i_5++) {
            C[i_4-1][i_5-1] = D[i_5][i_4] + E[i_5] * C[i_4-1][i_5+1] - C[i_4][i_5-1] - 3;
        }
    }
#pragma endscop
...
kernel_3334311325_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
