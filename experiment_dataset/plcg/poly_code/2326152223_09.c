...
void kernel_2326152223_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        A[i_0] = D[i_0+2] * D[i_0] - E[i_0][0] * D[i_0-1] * D[i_0-2] - D[i_0] * D[i_0] + A[i_0+2] * 4;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        C[i_1-1] = E[i_1-1][i_1] - D[i_1] + D[i_1+2] + D[i_1] * D[i_1-2] - C[i_1-1] + 6;
        C[i_1-1] = D[i_1-2] - D[i_1] * D[i_1+1] * C[i_1] - 1;
    }
#pragma endscop
...
kernel_2326152223_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
