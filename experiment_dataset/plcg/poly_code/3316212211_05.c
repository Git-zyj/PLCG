...
void kernel_3316212211_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(min(i_0, PB_P), -i_0+PB_Q-1); i_1++) {
            for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
                A[i_1-1][i_0][i_2-1] = C[i_0-3][i_1][i_1+i_0-1] - C[i_0-1][i_1][i_1+i_0+1] - 3;
            }
        }
        B[i_0][i_0] = D[i_0+1] - B[i_0+1][i_0+1] * 4;
        for (int i_3 = 0; i_3 < min(-i_0+PB_Q, PB_P-1); i_3++) {
            C[i_0-2][i_3][i_3+i_0] = C[i_0-3][i_3+1][i_3+i_0] * 2;
        }
    }
#pragma endscop
...
kernel_3316212211_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
