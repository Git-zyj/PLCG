...
void kernel_3216112225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0] = A[i_0-1] - A[i_0+1] * A[i_0+2] + 2;
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            for (int i_2 = -i_0+PB_N - 1; i_2 >= i_1; i_2--) {
                B[i_2+i_0][i_1][1] = C[i_0][i_2+2][i_1-1] - B[i_2+i_0-2][i_1][1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3216112225_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
