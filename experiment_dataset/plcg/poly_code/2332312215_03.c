...
void kernel_2332312215_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_N; i_3++) {
            C[i_3-1][i_2] = A[i_3-2] + C[i_3-2][i_2] - C[i_3][i_2] - C[i_3-1][i_2+1] * 6;
        }
    }
#pragma endscop
...
kernel_2332312215_03(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
