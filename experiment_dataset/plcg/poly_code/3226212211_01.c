...
void kernel_3226212211_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-3; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_1][i_0+2] = C[i_1+2][i_0+1] * C[i_1+1][i_0] - A[i_1][i_0+1] + A[i_1+1][i_0+3] * 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, PB_M-2); i_3++) {
            for (int i_4 = 2; i_4 < PB_M; i_4++) {
                B[i_3+2][i_4-2] = A[i_4][i_3+1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3226212211_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
