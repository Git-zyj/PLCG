...
void kernel_3212311223_03(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_Q); i_2++) {
                B[i_1+1][i_0][i_2] = C[i_2+i_1][i_0][i_2] * C[i_0+1][i_2-1][i_2] - 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q; i_5++) {
                B[i_4+1][i_3][i_5] = B[i_4-1][i_5][i_3-1] + B[i_4+1][i_3+1][i_5] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3212311223_03(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
