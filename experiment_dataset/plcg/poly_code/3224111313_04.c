...
void kernel_3224111313_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0-1] = C[i_1+1][i_0][0] - A[i_1][i_0] + A[i_1-1][i_0] + A[i_1+1][i_0-1] * A[i_0][i_1] - A[i_0][i_1-2] - 4;
        }
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            for (int i_3 = i_2; i_3 < PB_Q; i_3++) {
                B[i_0][i_2][i_3] = B[i_3+1][i_2][i_0] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3224111313_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
