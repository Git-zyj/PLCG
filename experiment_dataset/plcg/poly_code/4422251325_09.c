...
void kernel_4422251325_09(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            D[i_1+2][i_1+2] = E[i_1-1][i_0] * E[i_1-1][i_0] + E[i_0+1][i_1] * E[i_0][i_1+1] + F[i_0][i_1] + F[i_1][i_0] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
        D[i_2][i_2] = G[i_2] - H[i_2+1] * G[i_2-1] + H[i_2] - G[i_2] + H[i_2] + G[i_2] * G[i_2] * 5;
        for (int i_3 = i_2; i_3 < PB_N-2; i_3++) {
            for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
                for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                    C[i_3][i_4][i_5-1][i_2] = I[i_5][i_4][0][i_2] * I[i_5+1][i_2][i_3][i_4] * C[i_3+2][i_4-1][i_5+1][i_2] + C[i_3][i_4+1][i_5-1][i_2] + C[i_3+1][i_4][i_5-1][i_2+1] + C[i_3][i_4][i_5][i_2+1] + C[i_3][i_4-2][i_5+1][i_2+2] * 1;
                }
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 1; i_7 < min(PB_L-1, -i_6+PB_P+1); i_7++) {
            for (int i_8 = i_7; i_8 < -i_6+PB_N; i_8++) {
                D[i_7][i_7] = J[i_8][i_7+i_6-1][i_8] * K[i_8+i_6][i_8][i_7+1] * K[i_7][i_8][i_8] * J[i_6+1][i_7-1][i_8+1] + 4;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4422251325_09(xd,yd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
