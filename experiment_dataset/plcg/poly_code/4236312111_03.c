...
void kernel_4236312111_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
        ...
        for (int i_1 = 0; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 0; i_2 < PB_N; i_2++) {
                for (int i_3 = 0; i_3 < -i_2+PB_Q; i_3++) {
                    A[i_1][i_2][i_3+i_2] = C[i_2][i_1+2][i_1] * 2;
                }
            }
        }
        for (int i_4 = 2; i_4 < min(PB_L, i_0); i_4++) {
            B[i_0][i_4] = D[i_0+1][i_4-2] * B[i_0][i_4-1] + 1;
        }
    }
#pragma endscop
...
kernel_4236312111_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
