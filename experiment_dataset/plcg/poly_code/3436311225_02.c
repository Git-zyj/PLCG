...
void kernel_3436311225_02(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_N-1; i_3++) {
        B[i_3] = E[i_3+1] + F[i_3-1] - B[i_3-2] - B[i_3-1] - B[i_3+1] * 4;
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            C[i_5][i_4] = F[0] * C[i_5][i_4+1] * 6;
        }
    }
    for (int i_6 = 2; i_6 < PB_L; i_6++) {
        for (int i_7 = 0; i_7 < PB_M-2; i_7++) {
            for (int i_8 = 0; i_8 < i_6; i_8++) {
                D[i_6][i_7][i_8] = D[i_6-2][i_8][i_7] - D[i_7+2][i_6][i_8] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3436311225_02(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
