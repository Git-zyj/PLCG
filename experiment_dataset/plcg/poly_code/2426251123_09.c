...
void kernel_2426251123_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        B[i_0] = E[i_0-1] + E[i_0] + D[i_0] * 4;
        B[i_0] = E[0] * E[i_0] * F[i_0] + F[i_0] + 1;
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            C[i_0][i_1-1] = F[i_0] + G[i_1][0] * H[i_1+1][i_0] + H[i_0-1][0] * G[i_0+1][i_1+i_0] - G[i_1][i_0] * G[i_1-1][i_0+1] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        D[i_2] = E[i_2-1] * F[i_2] - E[i_2] + F[i_2-1] - E[i_2] + G[i_2-1][i_2-1] + E[i_2] * D[i_2+2] * 3;
    }
#pragma endscop
...
kernel_2426251123_09(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
