...
void kernel_4336152123_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_P-2; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 2; i_3 < -i_2+PB_L-2; i_3++) {
                    A[i_1-2][i_2][i_3] = D[i_0][i_2][i_1][i_1] * D[i_1-2][i_3][i_0][i_2+1] * D[i_1][i_3+i_2+2][i_0][2] * D[i_3-2][i_2][i_1][i_0+1] * D[i_2][i_3][i_1+2][i_0] + E[i_2][i_3][i_0-1][i_1-2][0] + D[i_2][i_1][i_3-1][i_0] - 4;
                }
            }
        }
        B[i_0-1] = F[i_0] - F[i_0+1] * F[i_0] + B[i_0+1] * 1;
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_L-2); i_5++) {
            C[i_5][i_4] = G[i_4][i_5] - G[i_4][i_5] + G[i_5][i_4] * G[i_4][i_5+2] - G[i_5][i_4] - B[i_5-1] + C[i_5-1][i_4] * 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4336152123_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
