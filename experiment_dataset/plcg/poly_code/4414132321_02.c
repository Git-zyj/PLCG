...
void kernel_4414132321_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_L; i_2++) {
                for (int i_3 = 2; i_3 < min(PB_L-2, -i_1+PB_M+2); i_3++) {
                    A[i_2][i_1][i_2-2][i_3] = E[i_3+i_1-2][i_2+i_1][i_2-1][i_0] * E[i_1][i_3+2][i_3][i_0] * E[i_2][i_0+1][i_1+2][i_3] - E[i_2+i_1][i_0][i_3][0] - A[i_2][i_1][i_2-2][i_3-1] + A[i_2][i_1-1][i_2-2][i_3-2] - A[i_2][i_1+2][i_2-2][i_3] + 3;
                }
            }
        }
        for (int i_4 = max(2, i_0); i_4 < min(-i_0+PB_M, PB_L); i_4++) {
            B[i_4][0] = F[i_4+i_0][i_0] * B[i_4-2][0] * B[i_4-1][0] + 4;
        }
    }
    for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
        for (int i_6 = i_5; i_6 < -i_5+PB_M-1; i_6++) {
            C[i_6+i_5-2][i_5+1] = F[i_6+2][i_5-1] + B[i_6-1][0] - C[i_6+i_5-1][i_5+2] + C[i_6+i_5+1][i_5+2] + 6;
        }
        for (int i_7 = 1; i_7 < min(-i_5+PB_M-1, i_5); i_7++) {
            D[i_5][i_7] = F[i_5][i_7] - G[i_7] - F[i_7][i_5+1] - F[i_7+i_5+1][i_7] + F[i_5][i_5] + H[i_7-1][i_5-1][1] * D[i_5-1][i_7] + 6;
        }
    }
#pragma endscop
...
kernel_4414132321_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
