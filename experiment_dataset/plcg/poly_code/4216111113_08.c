...
void kernel_4216111113_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
        ...
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            ...
            for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
                for (int i_3 = i_1; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
                    B[i_2][i_1][i_0+1][i_3+1] = C[i_3+i_2][i_0][i_3][i_1] + B[i_3][i_1][i_0+1][i_2+2] * 4;
                }
            }
        }
    }
...
kernel_4216111113_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
