...
void kernel_2434351313_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_1] = E[i_1] - F[i_0-1][i_1] * A[i_0+1][i_1] - A[i_0][i_1+1] - A[i_0-1][i_1] - 3;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_0][i_2] = G[i_2][i_0-1] * H[i_2] * B[i_0-1][i_2+1] - B[i_0][i_2-1] * B[i_0+1][i_2] + 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        C[i_3] = E[i_3] + I[0] * E[1] - E[i_3] - E[i_3+1] - J[i_3][i_3] * C[i_3-1] + C[i_3+1] + C[i_3-1] - C[i_3-1] - 5;
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_M); i_5++) {
            D[i_4-1][i_4] = G[i_5+1][i_5+1] + J[i_5][i_4+1] - J[i_5+i_4][1] - H[i_5-1] + H[i_5] * E[i_5+i_4] + H[i_5] + C[i_5] + 1;
        }
    }
#pragma endscop
...
kernel_2434351313_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
