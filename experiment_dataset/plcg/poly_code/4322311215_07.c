...
void kernel_4322311215_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        C[i_3] = D[i_3-1] + D[i_3] * C[i_3-1] + C[i_3+1] + 5;
    }
#pragma endscop
...
kernel_4322311215_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
