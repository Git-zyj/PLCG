...
void kernel_3212251211_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P-1; i_2++) {
                A[i_1][i_0+1][i_2] = C[i_1][i_2+1][i_0] - D[i_1][i_0][i_2+i_1+1] * D[i_1+1][i_2+1][i_0] * A[i_1-1][i_0][i_2] * 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_Q; i_4++) {
            for (int i_5 = 1; i_5 < PB_P; i_5++) {
                B[i_3-1][i_4+1][i_5-1] = C[i_4-1][i_5-1][i_3-1] * E[i_5+1][i_4][i_4+i_3][i_5] - F[i_4][i_4] - D[i_4][i_3+1][i_5] + D[i_4][i_5][i_3] * C[i_4-1][i_5][i_3] + A[i_4+1][i_3+1][i_5] + B[i_3-2][i_4+1][i_5-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3212251211_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
