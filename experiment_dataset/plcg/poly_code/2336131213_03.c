...
void kernel_2336131213_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            A[i_1][i_0-1] = D[i_1][i_0] + D[i_1+1][i_1+i_0] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
            B[i_2][i_2] = D[i_3][i_2] + D[i_2][i_3+1] - D[i_2][i_3+1] + B[i_2-1][i_2-1] + 1;
        }
    }
    for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_M); i_5++) {
            C[i_5][i_4] = D[i_4][i_5] + D[i_5+i_4][i_4+1] - D[i_5][i_5+1] * D[i_5][1] * C[i_5-1][i_4-1] * C[i_5-1][i_4] - C[i_5+1][i_4-1] + C[i_5][i_4-1] - 3;
        }
    }
#pragma endscop
...
kernel_2336131213_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
