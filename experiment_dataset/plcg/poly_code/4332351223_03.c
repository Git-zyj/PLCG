...
void kernel_4332351223_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] - E[i_0+1] + E[i_0] * E[i_0] - E[i_0] - D[i_0] - F[i_0+1] + B[i_0] * 5;
        B[i_0] = E[i_0-1] - G[i_0][i_0] + A[i_0] * 2;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_L, -i_1+PB_M+1); i_2++) {
            for (int i_3 = i_1; i_3 < min(-i_2+PB_M-3, PB_L); i_3++) {
                C[i_3+i_2+1][i_1+1][0] = H[i_3+1][i_1][i_1+1] * I[i_3][0][i_2] - H[i_2][i_1][0] * J[i_3][i_2] + I[i_1-1][i_3][i_2] + C[i_2+i_1-1][i_3+1][0] - C[i_3+i_2][i_1+1][0] + C[i_3+i_2+3][i_1-1][0] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4332351223_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
