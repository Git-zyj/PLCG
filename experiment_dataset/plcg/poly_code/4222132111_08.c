...
void kernel_4222132111_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                B[i_1][i_0+1][i_1] = C[i_0][i_2][i_1-1] - C[i_2-2][i_0][i_1] - C[i_2+2][i_0][i_1] - 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_3+PB_L, PB_P); i_5++) {
                B[i_5][i_3][i_5] = C[i_5-1][i_4][i_3+2] * C[i_4][i_5][i_5+i_3] - C[i_3][i_5][i_4] * B[i_5-1][i_3][i_5-1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4222132111_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
