...
void kernel_4234132313_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_M+1; i_1++) {
            A[i_1-2][i_0] = C[i_1][i_0] - C[i_1][i_0] + C[i_1][i_0] - D[i_1] - A[i_1-1][i_0+1] * A[i_1-3][i_0] * A[i_1-1][i_0] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            A[i_3-2][i_2] = C[i_3+2][i_2] + C[i_3+1][i_2+2] + 3;
        }
    }
#pragma endscop
...
kernel_4234132313_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
