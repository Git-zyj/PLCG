...
void kernel_3224112223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_1][i_0+1] = C[i_1][i_0] - C[i_0][i_0-1] + A[i_1-2][i_0+2] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 3; i_3 < PB_N-1; i_3++) {
            for (int i_4 = 0; i_4 < min(min(-i_3+PB_N-1, -i_2+PB_N), i_3); i_4++) {
                B[i_4+i_2][i_3-2][0] = B[i_4+i_3+1][i_2-2][0] * B[i_4+i_2-1][i_3-3][0] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3224112223_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
