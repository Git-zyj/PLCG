...
void kernel_4334311111_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L+2; i_1++) {
            for (int i_2 = i_1; i_2 < PB_P-1; i_2++) {
                A[i_1][i_0-1][i_2+1] = A[i_2][i_1-2][i_0+1] + 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_Q+1; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_Q+1; i_4++) {
            for (int i_5 = 0; i_5 < i_3; i_5++) {
                B[i_3][i_4][i_5] = D[i_5][i_3-1][i_4-1] * E[i_5][i_4][i_4+i_3-1] - C[i_4][i_5] + 4;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_Q; i_6++) {
        for (int i_7 = i_6 - 1; i_7 >= 0; i_7--) {
            for (int i_8 = 1; i_8 < i_6; i_8++) {
                C[i_6][i_8] = E[i_7][i_8-1][i_6] - C[i_8+1][i_7] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4334311111_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
