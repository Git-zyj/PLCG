...
void kernel_2424312321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] - 4;
        for (int i_1 = i_0; i_1 < min(PB_M, -i_0+PB_L-2); i_1++) {
            B[i_0][i_1+i_0] = F[i_1][i_0-2] + A[i_1-2] * B[i_0][i_1+i_0+1] * B[i_1][i_1+i_0+2] * B[i_0+2][i_1+i_0+2] * 4;
        }
    }
    ...
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        D[i_3] = H[0] - D[i_3+2] + D[i_3-2] * D[i_3-1] + D[i_3+1] * D[i_3+2] * 1;
    }
#pragma endscop
...
kernel_2424312321_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
