...
void kernel_4236211113_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = i_1; i_2 < PB_L+1; i_2++) {
                A[i_2][i_1][i_0] = C[i_1+1][i_2-1][i_0-1][i_1-1] * 4;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < min(i_4, PB_Q-1); i_5++) {
                for (int i_6 = 0; i_6 < PB_Q-2; i_6++) {
                    B[i_3][i_4][i_6+1][i_5] = B[i_4][i_3][i_6+2][i_5+1] * 3;
                }
            }
        }
    }
...
kernel_4236211113_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
