...
void kernel_4422311123_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + E[i_0-1] + A[i_0+2] - 6;
        for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                B[i_2][i_1-1][i_1] = F[i_2][i_1] - B[i_2-2][i_1-1][i_1] + 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            C[i_3][i_4] = F[i_3][i_4] + 3;
        }
        for (int i_5 = 0; i_5 < -i_3+PB_P-2; i_5++) {
            for (int i_6 = 0; i_6 < -i_5+PB_Q-3; i_6++) {
                for (int i_7 = 1; i_7 < PB_L; i_7++) {
                    D[i_3][i_5+i_3][i_6+i_5+1][i_7-1] = B[i_7][i_5+1][i_5+2] + D[i_3][i_5+i_3+2][i_6+i_5+3][i_7-1] - 2;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4422311123_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F)));
