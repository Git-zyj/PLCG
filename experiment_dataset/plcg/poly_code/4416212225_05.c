...
void kernel_4416212225_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_L+2; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < min(i_1, PB_L-2); i_3++) {
                    A[i_3+1][i_1-2][i_2-2][i_2-1] = B[i_3+2][i_2+1][i_1-1][i_0] + 1;
                }
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 2; i_6 < PB_L-2; i_6++) {
                for (int i_7 = 0; i_7 < PB_L-2; i_7++) {
                    B[i_6+2][i_5][i_7][i_4] = E[i_6-2][i_5+1][i_7][i_4-2] - E[i_6][i_7][i_4][0] - B[i_7+2][i_5+1][i_6][i_4] * B[i_6+1][i_5][i_7][i_4] + 3;
                }
            }
        }
        for (int i_8 = 2; i_8 < PB_L-2; i_8++) {
            for (int i_9 = 2; i_9 < i_4; i_9++) {
                for (int j_0 = 0; j_0 < -i_4+PB_L; j_0++) {
                    B[j_0+1][i_8-2][i_4+2][i_9] = F[i_8+1][j_0+i_4][i_4+2][i_9] + B[i_8+2][i_9-2][j_0][i_4-1] - 6;
                }
            }
        }
        D[i_4+1] = D[i_4+2] - D[i_4+3] * 1;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4416212225_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F)));
