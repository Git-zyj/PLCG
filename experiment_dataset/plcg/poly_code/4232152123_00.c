...
void kernel_4232152123_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_L, -i_1+PB_M-2); i_2++) {
                B[i_2-1] = C[i_1-1][i_0+1][i_2] + D[i_0-1][i_1] * C[i_2][i_1][i_0] * C[i_2+i_1+2][i_0][i_2] - C[i_2][i_1+1][i_1] * C[i_0][i_2][i_0+1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            B[i_4] = E[i_4-2] + E[i_4] + E[i_4] * B[i_4-1] - 5;
        }
    }
#pragma endscop
...
kernel_4232152123_00(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
