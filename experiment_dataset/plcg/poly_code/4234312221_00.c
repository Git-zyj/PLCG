...
void kernel_4234312221_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_M; i_3++) {
            B[i_3+i_2-1] = B[i_3+i_2-2] - B[i_3+i_2] * 3;
        }
    }
#pragma endscop
...
kernel_4234312221_00(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
