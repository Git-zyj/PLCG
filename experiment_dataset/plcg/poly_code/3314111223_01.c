...
void kernel_3314111223_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_1+1][i_0+1][i_0] = D[i_1][i_2][0] + E[i_0][i_1] * A[i_1+3][i_0+1][i_0] * A[i_0-1][i_2+1][i_2] - A[i_1+1][i_0+2][i_0+1] * 1;
            }
            for (int i_3 = 0; i_3 < min(i_0, -i_1+PB_P-3); i_3++) {
                B[i_0][i_3+i_1][i_0] = E[i_0][i_1+1] * B[i_0+1][i_3+i_1+3][i_0+1] + 1;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P+1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M+1; i_5++) {
            for (int i_6 = 0; i_6 < min(-i_5+PB_Q, -i_4+PB_P+1); i_6++) {
                C[i_5-1][i_6+i_4-1][i_6+i_5] = 2;
            }
        }
    }
#pragma endscop
...
kernel_3314111223_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
