...
void kernel_2316212323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_L, -i_0+PB_M-3); i_1++) {
            A[i_1] = D[i_0][i_1-2] * A[i_1+1] + A[i_1-1] + A[i_1+2] + B[i_1+i_0+1][i_1] + 5;
            B[i_1+i_0+1][i_1] = D[i_1][0] - B[i_1+i_0][i_1-2] - B[i_1+i_0+3][i_1+2] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
            C[i_2][i_3] = C[i_3-2][i_2] * C[i_2-2][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_2316212323_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
