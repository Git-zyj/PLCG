...
void kernel_3424331225_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] - F[i_0] * E[i_0+1] * A[i_0+2] * 4;
    }
    for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
        A[i_1] = G[i_1-1][i_1] * 3;
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        C[i_2] = H[i_2] * I[i_2-1][1] * A[i_2] * 3;
        C[i_2-1] = F[0] - E[i_2] * H[i_2] - H[i_2-1] - C[i_2] - 2;
    }
#pragma endscop
...
kernel_3424331225_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
