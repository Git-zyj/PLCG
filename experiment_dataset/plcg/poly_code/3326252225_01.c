...
void kernel_3326252225_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0+2] + E[i_0-2] + E[i_0] - E[i_0] * D[i_0] + B[i_0] * 6;
        B[i_0] = E[i_0-1] + E[i_0-2] - F[i_0][i_0-2] - E[i_0-2] * D[i_0] + A[i_0] + B[i_0+1] - 4;
    }
    for (int i_1 = 1; i_1 < PB_Q; i_1++) {
        for (int i_2 = 2; i_2 < min(PB_L, i_1); i_2++) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                C[i_2][i_1][i_2] = G[i_1-1][i_2-1][i_3] + F[i_2][i_3] * H[i_2][i_1][i_1] * H[i_1][i_2][i_2-2] + C[i_2][i_1+2][i_2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3326252225_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
