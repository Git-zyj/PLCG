...
void kernel_3326111321_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_2][i_0][i_2] = D[i_2][i_0][i_1-1] - A[i_2][i_0+1][i_2] + 1;
            }
        }
        for (int i_3 = max(3, i_0); i_3 < PB_M; i_3++) {
            B[i_3-1] = B[i_3] * B[i_3-3] - 2;
        }
    }
    for (int i_4 = 2; i_4 < PB_M+1; i_4++) {
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            for (int i_6 = 2; i_6 < PB_L+1; i_6++) {
                C[i_5-1][i_6-1][i_4] = A[i_5][i_4-2][i_5] + C[i_5][i_6-1][i_4+1] * C[i_5-1][i_6-1][i_4+2] - C[i_4-1][i_6-2][i_5] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3326111321_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
