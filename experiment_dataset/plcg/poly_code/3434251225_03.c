...
void kernel_3434251225_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M+1; i_1++) {
            A[i_1-1][i_0][i_0] = E[1] * F[i_1-1][i_0] + A[i_0-1][i_1+1][i_1+1] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 3; i_3 < PB_M-1; i_3++) {
            A[i_3-3][i_2][i_2] = F[i_3+1][i_2] * F[i_3][i_2+1] * G[i_3][i_2] - G[i_3+1][i_2+1] - F[i_3-1][i_2] * G[i_3][i_2] + 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_L); i_5++) {
            for (int i_6 = 1; i_6 < PB_P; i_6++) {
                A[i_5][i_4][i_4] = H[i_4][i_6-1][i_5-1] + I[i_6][i_5+i_4][1] - H[i_6+1][i_5][i_4] + 6;
            }
        }
        for (int i_7 = i_4; i_7 < PB_P; i_7++) {
            for (int i_8 = 0; i_8 < min(min(PB_P, -i_4+PB_M), -i_7+PB_M); i_8++) {
                A[i_4-1][i_7][i_7] = G[i_7+1][i_8] * I[i_8+i_4][i_7][i_7] - F[i_7][i_7] + H[i_7-1][i_4][i_8] + H[i_4][i_8][i_7] * I[i_8+i_7][i_8][i_4] * I[i_7][i_4][i_4] + A[i_7-1][i_4][i_4] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3434251225_03(xa,ya,za,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
