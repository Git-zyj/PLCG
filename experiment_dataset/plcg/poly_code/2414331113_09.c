...
void kernel_2414331113_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            C[i_2] = E[i_2] + F[i_0-1][i_2] + 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        C[i_3] = E[i_3] - G[i_3][i_3] * E[i_3] - H[i_3] + C[i_3+1] + 5;
        for (int i_4 = 0; i_4 < -i_3+PB_M-1; i_4++) {
            D[i_3][i_4+1] = G[i_3][i_4] - I[i_3+1][i_4][i_3-1] - F[i_4][i_3] * F[i_4+i_3+1][1] * 1;
        }
    }
#pragma endscop
...
kernel_2414331113_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
