...
void kernel_4226132111_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(i_1, PB_M-2); i_2++) {
                B[i_1] = C[i_2+2][i_2][i_0+1] + C[i_0][i_2-1][i_1] * C[i_1][i_2][i_2] + C[i_2][i_0][i_0] - B[i_1+1] - 6;
            }
            B[i_1] = D[i_1-2][i_0-2] * D[i_1+1][i_0] + 1;
        }
    }
#pragma endscop
...
kernel_4226132111_05(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
