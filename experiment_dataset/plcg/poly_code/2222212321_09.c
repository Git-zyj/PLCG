...
void kernel_2222212321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < min(min(PB_P-2, -i_1+PB_M-2), i_1); i_2++) {
            B[i_2+i_1][i_2] = B[i_2+i_1+2][i_1] + B[i_2+i_1-1][i_2-1] * B[i_2+i_1+2][i_2+2] * 4;
        }
    }
#pragma endscop
...
kernel_2222212321_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
