...
void kernel_2312152111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = D[i_0+2] - D[i_0] * 3;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        B[i_1] = D[i_1+2] + D[i_1] - D[2] + D[i_1] - D[i_1] - D[i_1] - D[i_1] + D[i_1] - C[i_1+1][i_1] * 3;
        C[i_1+1][i_1] = D[i_1-2] + D[i_1] * D[i_1] + D[i_1] - D[i_1] + B[i_1] * C[i_1+2][i_1+1] + 2;
    }
#pragma endscop
...
kernel_2312152111_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
