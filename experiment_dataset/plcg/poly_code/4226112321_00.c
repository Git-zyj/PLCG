...
void kernel_4226112321_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 3; i_1 < PB_L+3; i_1++) {
            for (int i_2 = 3; i_2 < min(PB_P, i_0); i_2++) {
                for (int i_3 = 0; i_3 < min(PB_P, i_1); i_3++) {
                    A[i_2][i_0-1][i_2] = A[i_3][i_2-3][i_3] + A[i_2][i_1-3][i_2] * 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        B[i_4] = C[0] - B[i_4-1] - B[i_4+2] + B[i_4+1] + 3;
    }
#pragma endscop
...
kernel_4226112321_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
