...
void kernel_4422151115_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        B[i_0] = E[i_0] * E[i_0] + F[i_0][i_0-1] - E[i_0+1] * E[i_0] + 1;
        B[i_0] = E[i_0+1] - E[i_0] * F[i_0][1] - E[i_0] - E[i_0] + E[i_0+1] + 6;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_1+PB_M, -i_2+PB_M); i_3++) {
                D[i_2+1][i_1+1][i_3-1][i_3] = G[i_2][i_1][i_3] + G[i_3+i_1][i_2-1][i_1-1] * G[i_1][i_2][i_3] - G[i_2][i_1-1][i_1] + F[i_3+i_2][i_1+1] * D[i_1][i_3+1][i_2-1][i_2] * 6;
            }
        }
    }
        ...
        for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
            for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 0; i_7 < PB_M-1; i_7++) {
                    D[i_6+1][i_7+1][i_5-1][i_5] = H[i_6-1][i_4][i_7][i_5-1] - H[i_5-1][i_7][i_6-1][i_4] + H[i_6][i_7][i_5][i_4+1] * D[i_7+1][i_5+2][i_6-1][i_6] * 4;
                }
            }
        }
    }
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422151115_01(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
