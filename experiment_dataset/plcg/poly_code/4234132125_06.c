...
void kernel_4234132125_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < min(PB_P-2, -i_1+PB_Q); i_2++) {
                A[i_1][i_2][i_0+1] = C[i_1][i_2+2][i_0] - C[i_2][i_0][i_2+i_1] - A[i_1][i_2-1][i_0] * 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < PB_Q; i_4++) {
            for (int i_5 = i_3; i_5 < PB_P-2; i_5++) {
                A[i_4][i_3-2][i_5] = C[i_4-2][i_5+2][i_3] - C[i_4-2][i_5+1][i_3] + C[i_4][i_5][i_3] * C[i_5][i_3][i_4] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4234132125_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
