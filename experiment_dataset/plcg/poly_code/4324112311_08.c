...
void kernel_4324112311_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        B[i_4] = B[i_4-1] * B[i_4+1] + B[i_4-1] * B[i_4+1] + B[i_4+1] * B[i_4-1] * 6;
        for (int i_5 = 0; i_5 < min(PB_Q, -i_4+PB_M); i_5++) {
            for (int i_6 = 2; i_6 < i_5; i_6++) {
                C[i_6-2][i_5+i_4][i_6+1] = D[i_4][i_6+1][i_5] + C[i_4-1][i_6+i_5][i_4+2] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4324112311_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
