...
void kernel_2336331215_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[0][i_1] = D[i_1-1] * E[i_0-1][i_1-1] - A[0][i_1-1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            A[0][i_3] = F[i_3][i_2-1] * G[i_3][i_2+1] + A[0][i_3] + 5;
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        A[0][i_4] = H[i_4-1] - D[i_4+1] + D[i_4] + 3;
    }
#pragma endscop
...
kernel_2336331215_02(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
