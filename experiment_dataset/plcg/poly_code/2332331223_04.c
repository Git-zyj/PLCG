...
void kernel_2332331223_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            A[i_1][i_1+i_0-1] = D[i_0] + D[i_0] - E[i_1-1][i_0] - A[i_1][i_1+i_0-2] - A[i_1][i_1+i_0] + A[i_1-1][i_1+i_0-4] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        B[i_2][i_2] = F[i_2] * F[i_2] - 2;
        for (int i_3 = 0; i_3 < min(-i_2+PB_N+1, i_2); i_3++) {
            B[i_2][i_2] = G[i_2][0] + E[i_3+i_2-1][1] - B[i_2+2][i_2+2] - 4;
        }
    }
#pragma endscop
...
kernel_2332331223_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
