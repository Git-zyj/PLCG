...
void kernel_4314132125_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_Q, i_0); i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_P-2; i_2++) {
                for (int i_3 = 0; i_3 < min(i_0, -i_2+PB_M-2); i_3++) {
                    A[i_0][i_1-2][i_2][1] = D[i_2][i_3][i_1-1] * D[i_2+2][i_0-1][i_3] * E[i_3][i_2-1][i_1-1][i_0] - F[i_1][i_3][i_1+i_0][i_0][i_2] + E[i_1+2][i_3][i_2+i_0+2][i_3] - E[i_2][i_3+i_2+2][i_0-2][i_1] * A[i_1-1][i_2-2][i_0-1][1] + 2;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-4; i_4++) {
        B[i_4-1][i_4+2] = B[i_4+1][i_4+4] * 2;
        for (int i_5 = i_4; i_5 < -i_4+PB_N-2; i_5++) {
            C[i_5][i_5+i_4] = D[i_5-1][i_4][i_4] - G[i_5+i_4+2][0] + G[i_5][i_4] * 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4314132125_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
