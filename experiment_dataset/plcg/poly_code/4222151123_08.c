...
void kernel_4222151123_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
        ...
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < PB_L; i_2++) {
                for (int i_3 = 3; i_3 < -i_0+PB_L-1; i_3++) {
                    A[i_0-1][i_1+1][i_3] = C[i_1][i_2][i_3+i_0+1][i_0] * C[i_1+1][i_3][i_0][i_1] + C[i_0][i_3][i_2][i_2+1] + C[i_1+1][i_2][i_3][i_0-1] + C[i_1][i_3][i_3-1][i_2-1] - D[i_1][i_3][i_2+1] * A[i_3-3][i_2+2][i_0] * A[i_0-2][i_2][i_1] - 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < PB_P; i_5++) {
            for (int i_6 = 0; i_6 < PB_P; i_6++) {
                for (int i_7 = i_6; i_7 < min(min(PB_L, -i_5+PB_M), -i_6+PB_Q); i_7++) {
                    B[i_6][i_7][i_6] = D[i_5][i_6][i_7+i_6] - C[i_7+i_5][0][i_4][i_6] - D[1][i_5][0] - 6;
                }
            }
        }
    }
...
kernel_4222151123_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
