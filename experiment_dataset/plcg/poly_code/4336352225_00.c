...
void kernel_4336352225_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+2] = D[i_0] * D[i_0+2] + E[i_0] + D[0] * D[i_0+2] - E[i_0-1] * E[i_0+2] - A[i_0] + A[i_0+1] + 4;
    }
    for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
        for (int i_2 = 2; i_2 < min(i_1, PB_M-2); i_2++) {
            A[i_2+2] = D[i_1] + F[i_2+1][i_1] + G[i_1-2] - D[i_2-2] * 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < min(PB_P, -i_3+PB_M-1); i_4++) {
            for (int i_5 = 2; i_5 < min(PB_P-1, -i_3+PB_L-1); i_5++) {
                C[i_4+i_3][i_4+i_3][i_5] = H[i_5+i_3+1][i_4][i_4-2] + I[i_5][i_3][i_4] - I[i_3+1][i_5-2][i_5+1] * C[i_4+i_3+1][i_4+i_3+1][i_5] * C[i_4+i_3-1][i_4+i_3-1][i_5-2] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4336352225_00(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
