...
void kernel_4212211315_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        ...
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = i_1; i_2 < PB_M+1; i_2++) {
                B[i_0-1][i_1][i_2] = D[i_1][i_0][i_2] + B[i_0-2][i_1][i_2-1] + B[i_0-1][i_1-1][i_2] * B[i_2-1][i_1][i_0-1] * B[i_0-1][i_1+1][i_2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4212211315_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
