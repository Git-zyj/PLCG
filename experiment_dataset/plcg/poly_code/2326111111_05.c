...
void kernel_2326111111_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = max(1, i_0); i_2 < PB_M-1; i_2++) {
            C[i_2-1][i_0] = D[i_2+1][i_0+1] + D[i_0][i_2+1] + 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            C[i_4][i_3] = D[i_4][i_3] + C[i_3+1][i_4+1] - 4;
        }
    }
#pragma endscop
...
kernel_2326111111_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
