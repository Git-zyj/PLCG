...
void kernel_4324211315_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_0][i_1][0] = A[i_0+1][i_1][0] - A[i_0-1][i_1][0] - A[i_0+1][i_1-1][0] + A[i_0][i_1+1][0] - 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M; i_4++) {
            A[i_3][i_4+1][0] = D[i_4][i_3] - 3;
        }
        for (int i_5 = 0; i_5 < -i_3+PB_P; i_5++) {
            for (int i_6 = 0; i_6 < min(-i_3+PB_P+1, i_3); i_6++) {
                C[i_6][i_5+i_3][i_5] = C[i_5][i_6+i_3-1][i_6] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4324211315_03(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
