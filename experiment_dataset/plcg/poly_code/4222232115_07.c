...
void kernel_4222232115_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_P; i_2++) {
                A[i_1][i_2][i_2+i_0] = C[i_1+i_0][i_2+1][i_2-1] * D[i_1][i_0][i_2] * A[i_1][i_2-1][i_2+i_0-2] - 6;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_M; i_3++) {
            for (int i_4 = 0; i_4 < -i_0+PB_P+1; i_4++) {
                A[i_3+1][i_4][i_4+i_0-1] = C[i_3+2][i_0+1][i_4+1] - D[i_3+i_0][i_4][2] * D[i_0][i_4+1][i_0-2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4222232115_07(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
