...
void kernel_2324351221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] * D[i_0] * E[i_0+1] + A[i_0-1] - A[i_0+1] - 3;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_M+1; i_2++) {
            B[i_2][i_1-1] = E[i_2+i_1-1] * F[i_2+i_1][i_1] * F[i_2+i_1][i_1] - F[i_1+1][0] * F[i_2-1][i_1] * F[i_1][i_2-1] - G[i_2+i_1][i_2] * A[i_2] * B[i_1][i_2+1] * 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            A[i_4] = H[i_3+1] + I[i_4+1][i_3] * F[i_4][i_3] + G[i_3][i_4+1] * 3;
        }
    }
#pragma endscop
...
kernel_2324351221_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
