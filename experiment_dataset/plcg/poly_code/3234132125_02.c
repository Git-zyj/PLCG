...
void kernel_3234132125_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            B[i_1] = C[i_0] * D[i_1+1][i_0] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        B[i_2+1] = C[i_2-1] - C[i_2] * C[i_2] * C[i_2] + B[i_2+2] - 2;
    }
#pragma endscop
...
kernel_3234132125_02(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
