...
void kernel_4234311321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_0-1][i_1] = A[i_0][i_1-2] - A[i_0-3][i_1] + 5;
        }
    }
        ...
        for (int i_3 = max(1, i_2); i_3 < PB_P-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
                    B[i_5][i_2][i_3] = C[i_5][i_3][i_4] * B[i_5+2][i_3][i_4] + B[i_5-2][i_4][i_3-1] * B[i_5][i_3+2][i_4] * 1;
                }
            }
        }
    }
...
kernel_4234311321_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
