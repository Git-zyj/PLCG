...
void kernel_3224111321_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-3; i_0++) {
        for (int i_1 = 2; i_1 < PB_P+1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_0][i_1+1][i_0+1] = C[i_1+1][i_0][i_2] + A[i_1-2][i_0+1][i_1-1] * A[i_0][i_1-1][i_0+1] * A[i_0+2][i_1-1][i_0+3] * 1;
            }
        }
    }
...
kernel_3224111321_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
