...
void kernel_3222112213_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-3; i_1++) {
            A[0][i_1+2] = C[i_1+1][i_0] + A[0][i_1+3] - A[0][i_1+1] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            A[0][i_3+2] = C[i_3+1][i_2-1] * 4;
        }
    }
#pragma endscop
...
kernel_3222112213_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
