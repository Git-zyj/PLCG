...
void kernel_3422151123_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] * E[i_0] + E[i_0] + E[i_0] * E[1] + E[i_0] - E[i_0] * E[i_0] + E[i_0] - 4;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            for (int i_3 = 0; i_3 < -i_2+PB_M; i_3++) {
                B[i_1-1][i_2+1][i_3] = F[0][i_3][0] + G[i_3+i_2][i_3] * 5;
            }
            for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
                C[i_1][i_2][i_4+1] = G[i_4][i_2] - G[i_4][i_2] + F[i_2][i_4][i_1] * C[i_1][i_2-1][i_4-1] + 6;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        A[i_5] = G[1][i_5] + E[i_5] - G[i_5][i_5] * E[i_5] + E[i_5] * 5;
    }
#pragma endscop
...
kernel_3422151123_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
