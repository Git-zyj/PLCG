...
void kernel_2432311221_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < PB_M; i_1++) {
            A[i_1-1] = E[i_1][i_0] * A[i_1] * A[i_0] + A[i_1-2] + A[i_1-3] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        ...
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            D[i_2-1][i_3] = E[i_2][i_3+1] + D[i_2-2][i_3] + 3;
        }
    }
#pragma endscop
...
kernel_2432311221_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
