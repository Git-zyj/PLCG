...
void kernel_2312112221_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_1][i_0+2][i_1] = B[i_1] + 1;
            B[i_1] = D[0] - B[i_0+1] + B[i_0-2] * 6;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        C[i_2-1] = B[i_2] * C[i_2-3] * C[i_2] + 6;
    }
#pragma endscop
...
kernel_2312112221_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
