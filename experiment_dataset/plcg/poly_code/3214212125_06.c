...
void kernel_3214212125_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            A[i_1-1][i_0-1] = A[i_0-1][i_1] + 3;
        }
        for (int i_2 = max(3, i_0); i_2 < PB_M; i_2++) {
            A[i_2-3][i_0-1] = C[i_2][i_0] + 4;
        }
    }
#pragma endscop
...
kernel_3214212125_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
