...
void kernel_4422111221_07(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        ...
        for (int i_3 = 2; i_3 < min(min(-i_2+PB_L, i_2), PB_M-2); i_3++) {
            C[i_2][i_3] = E[i_2][i_3+i_2] + B[i_3+2] - C[i_2][i_3-2] - 2;
        }
        for (int i_4 = 1; i_4 < i_2; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_2+PB_L, PB_M); i_5++) {
                D[i_4-1][i_5][i_5+i_2] = D[i_4+1][i_5][i_5+i_2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4422111221_07(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
