...
void kernel_3334212321_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_0-2][i_0] = A[i_0-3][i_0-1] + A[i_1][i_1+2] * A[i_0-1][i_0+1] + 1;
        }
    }
    for (int i_2 = 4; i_2 < PB_M+2; i_2++) {
        ...
        for (int i_5 = 0; i_5 < min(-i_2+PB_L, i_2); i_5++) {
            C[i_2-2][i_5+i_2] = D[i_2][0] + C[i_2-4][i_5+i_2-1] - C[i_2-2][i_5+i_2-2] * 3;
        }
    }
#pragma endscop
...
kernel_3334212321_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
