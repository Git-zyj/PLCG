...
void kernel_3412152225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - E[i_0-2] + 3;
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0] = F[i_1][i_0] + E[i_1+i_0] + F[i_1][i_0+1] + F[i_1][i_0-1] + F[i_1+i_0][i_1] * 5;
        }
        for (int i_2 = 1; i_2 < min(PB_M, -i_0+PB_N); i_2++) {
            A[i_2] = E[i_2] * F[i_2+i_0][0] + E[i_0] * A[i_2-1] + 2;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_L-1, PB_M-2); i_4++) {
            for (int i_5 = 1; i_5 < -i_3+PB_M; i_5++) {
                D[i_4][i_5+2] = G[i_3][i_5][i_4] + G[i_5][i_3-2][i_4-2] - G[i_4][i_3][0] - G[i_5-1][i_4+i_3+1][i_4] + H[i_3-1][i_4][i_5+1][i_3] * G[i_5+i_3][i_3][i_4] + F[i_4][i_5] - D[i_4-2][i_5+2] * D[i_3-2][i_4+2] - 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3412152225_04(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
