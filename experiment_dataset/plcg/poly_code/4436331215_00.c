...
void kernel_4436331215_00(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
        ...
        for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    D[i_1+1][i_3][i_2-1][0] = 2;
                }
            }
        }
        B[i_0] = E[i_0] + F[i_0+1] - E[i_0] * F[i_0] + B[i_0-1] - 2;
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        C[i_4] = G[i_4-1] + E[i_4] * E[i_4+1] - G[i_4] - B[i_4] * C[i_4+1] + C[i_4-1] + 3;
    }
    for (int i_5 = 1; i_5 < PB_Q; i_5++) {
        for (int i_6 = 0; i_6 < PB_L; i_6++) {
            for (int i_7 = 2; i_7 < -i_5+PB_Q+1; i_7++) {
                for (int i_8 = 0; i_8 < min(min(i_6, -i_6+PB_P), PB_L-1); i_8++) {
                    D[i_6+1][i_8][i_7-1][0] = H[i_6][i_8+1][i_8+i_6][i_7+i_5-1] + D[i_6+1][i_8+1][i_7-2][0] * D[i_8][i_6][i_7-1][0] * 1;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436331215_00(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
