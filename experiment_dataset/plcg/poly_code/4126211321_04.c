...
void kernel_4126211321_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 2; i_1 < min(min(i_0, -i_0+PB_M+1), PB_L-2); i_1++) {
            A[i_1][i_1+1] = B[i_1+i_0-1][i_1] - A[i_1-1][i_1] * A[i_1-2][i_1-1] + A[i_1+1][i_1+2] + 1;
        }
    }
#pragma endscop
...
kernel_4126211321_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
