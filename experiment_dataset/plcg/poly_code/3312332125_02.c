...
void kernel_3312332125_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-3; i_0++) {
        A[i_0+1] = D[i_0-1] - D[i_0] - D[i_0+2] - A[i_0+3] - 5;
    }
    for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_N-1, -i_1+PB_M+1); i_2++) {
            A[i_2+1] = E[i_2+i_1-1] - F[i_2][i_1-1] + 3;
        }
        for (int i_3 = 0; i_3 < min(-i_1+PB_L, i_1); i_3++) {
            for (int i_4 = 1; i_4 < -i_1+PB_P+1; i_4++) {
                C[i_1][i_3][i_4-1] = G[i_4][i_3][i_1+2][0] + H[i_1][i_3][i_4+i_1-1] + H[i_3+i_1][i_4][i_3] - H[i_3][i_4][i_1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3312332125_02(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
