...
void kernel_3434311325_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L-3, i_0); i_1++) {
            A[i_0-1][i_1+1] = A[i_0-1][i_1+3] + A[i_0-3][i_1+1] - A[i_0+1][i_1+1] * A[i_0-2][i_1] + 4;
        }
    }
    ...
    for (int i_3 = 3; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
            C[i_3-1][i_4-1] = C[i_3+1][i_4] - C[i_3][i_4-1] + C[i_3-1][i_4-2] * C[i_3-3][i_4-1] + C[i_3-1][i_4+1] - 1;
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = 3; i_6 < PB_M-1; i_6++) {
            D[i_6-1] = D[i_6-3] * D[i_6+1] * 5;
        }
    }
#pragma endscop
...
kernel_3434311325_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
