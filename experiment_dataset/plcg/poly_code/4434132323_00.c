...
void kernel_4434132323_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_N, PB_M-2); i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    A[i_1][i_2-1][i_1][i_0-1] = E[i_3][i_1][i_0-2][i_2] - A[i_1][i_2-1][i_1][i_0] * A[i_1][i_3][i_1][i_0+1] - A[i_3][i_2][i_3][i_0-1] + 1;
                }
            }
            B[i_1][i_1] = F[i_0+1][i_1] * G[i_1+i_0] - B[i_1+1][i_1+1] - B[i_1+2][i_1+2] * B[i_0+2][i_0+2] - 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            B[i_5+1][i_5+1] = F[i_5][0] * F[i_5][i_4] * 2;
        }
        for (int i_6 = 1; i_6 < PB_L; i_6++) {
            for (int i_7 = 2; i_7 < PB_L-1; i_7++) {
                A[i_4+2][i_7-1][i_4+2][i_6-1] = H[i_7][i_6][i_6] + F[i_7-2][i_4] - H[i_6+1][i_7][i_6] - H[i_4][i_7+1][i_6+1] + 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434132323_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
