...
void kernel_4136311115_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_P-1); i_1++) {
            for (int i_2 = i_1; i_2 < PB_M; i_2++) {
                A[i_1][i_1+1] = B[i_2][i_1+1] + A[i_1+1][i_1+2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4136311115_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
