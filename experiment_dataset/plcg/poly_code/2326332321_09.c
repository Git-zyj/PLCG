...
void kernel_2326332321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L+2; i_0++) {
        A[i_0-2] = D[i_0][i_0-2] + E[i_0] - E[i_0-1] * F[i_0] + A[i_0-3] - 5;
    }
    for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L; i_2++) {
            B[i_1][i_2] = G[i_2][i_1-2] - D[i_2][i_1] + H[i_2][2] * A[i_2-2] * B[i_1-1][i_2] + B[i_2][i_1+2] + B[i_1][i_2-1] * B[i_1+1][i_2] + 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        A[i_3-2] = F[i_3] * I[i_3+1] - 6;
    }
#pragma endscop
...
kernel_2326332321_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
