...
void kernel_4424132323_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0-2] = E[i_0] - F[i_0][i_0] - A[i_0-1] + 3;
        for (int i_1 = 3; i_1 < PB_P-1; i_1++) {
            for (int i_2 = i_0; i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = i_0; i_3 < -i_1+PB_M; i_3++) {
                    B[i_0][i_1][i_3-2][i_2] = G[i_0+2][i_3][i_2][i_1] - G[i_3+i_1][i_2+i_0][i_3][i_1-2] * G[i_3][i_2+i_0][i_1+1][i_1] + B[i_0-1][i_1-2][i_3-2][i_2] - B[i_0][i_1+1][i_3-2][i_2] * B[i_0][i_3][i_1-3][i_2-1] + B[i_0+2][i_3+1][i_1-2][i_2] - B[i_0-1][i_1+2][i_3-2][i_2] + 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 4; i_5 < PB_M; i_5++) {
            for (int i_6 = 1; i_6 < min(i_5, PB_M-1); i_6++) {
                C[i_5-2][i_4][i_6+1] = H[i_5][i_4+1][i_5-2] - H[i_6][i_4+2][i_5] - C[i_4-2][i_6+1][i_5] - C[i_5-2][i_4-2][i_6-1] * C[i_5-4][i_4][i_6+1] * 5;
            }
        }
        for (int i_7 = 0; i_7 < -i_4+PB_L-2; i_7++) {
            for (int i_8 = i_4; i_8 < PB_P-1; i_8++) {
                D[i_4][i_7][i_8] = F[i_8][i_7] * H[i_8][i_7][i_4+1] - H[i_8][i_7+i_4+2][i_8+1] - D[i_8][i_7][i_4+1] * D[i_8][i_7][i_4-2] * 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4424132323_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
