...
void kernel_4216112121_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
        ...
        for (int i_1 = i_0; i_1 < PB_M+2; i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    A[i_0+1][i_3+i_2-2][i_3][i_0] = C[i_3][i_2+i_0][i_1] + 3;
                }
            }
            for (int i_4 = 0; i_4 < min(i_0, PB_M-2); i_4++) {
                B[i_4][i_1-2] = B[i_4+2][i_0-3] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4216112121_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
