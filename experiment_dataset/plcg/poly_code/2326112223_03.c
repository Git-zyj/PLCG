...
void kernel_2326112223_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0-1] - A[i_0-1] - 1;
        for (int i_1 = 2; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            B[i_0][i_1+i_0] = A[i_1-2] - A[i_1] * B[i_0][i_1+i_0-2] + 4;
        }
    }
    for (int i_2 = 3; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_P+3, PB_L+1); i_3++) {
            B[i_3-1][i_3+i_2-3] = E[i_3][i_2] - 4;
        }
    }
#pragma endscop
...
kernel_2326112223_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
