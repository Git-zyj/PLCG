...
void kernel_3334332225_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M-1, i_1); i_2++) {
            A[i_2] = D[0][i_2] - D[i_2][i_1] * E[i_1-2] * F[i_2+i_1+1][0] + D[i_2][i_1] - A[i_2] * 5;
        }
        for (int i_3 = 2; i_3 < PB_M+2; i_3++) {
            for (int i_4 = 2; i_4 < min(min(i_1, -i_3+PB_M+2), PB_M-2); i_4++) {
                A[i_4+2] = G[i_4][i_3][i_1] - H[i_4+i_3-2][i_4-2][i_1-1] + G[i_3-2][i_1][i_4] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3334332225_07(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
