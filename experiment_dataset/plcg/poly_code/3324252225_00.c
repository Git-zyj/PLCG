...
void kernel_3324252225_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        B[i_0] = D[i_0+2] * E[0] + E[i_0-1] + E[i_0] * E[i_0] * D[i_0-2] * B[i_0] - 6;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        B[i_1] = D[i_1] - E[i_1-2] * E[i_1-2] * B[i_1+1] - 1;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_P; i_3++) {
            for (int i_4 = 2; i_4 < i_3; i_4++) {
                C[i_3-1][i_4][i_2-1] = F[i_2][i_4-2] * F[i_3-1][1] - G[i_4][i_2][i_3] * H[i_4+1][i_2-1][i_3+i_2] + G[i_3][i_2][i_4] + C[i_4-1][i_2-2][i_3+1] - C[i_3-1][i_4][i_2+1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3324252225_00(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
