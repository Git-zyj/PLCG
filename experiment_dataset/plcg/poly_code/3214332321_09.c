...
void kernel_3214332321_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+1][i_1+i_0] = C[i_0][i_1] * D[i_1][i_0-2] + A[i_1+3][i_1+i_0+2] * A[i_1-1][i_1+i_0] * A[i_1+1][i_1+i_0+2] * 4;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M-2; i_2++) {
            B[i_0-2][i_2+1] = E[i_2] + E[i_2] * F[i_2+1][i_0] + A[i_2-1][i_2+i_0-2] + A[i_2+3][i_2+i_0+2] - 5;
        }
    }
#pragma endscop
...
kernel_3214332321_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
