...
void kernel_3314212215_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_N-1; i_1++) {
            A[i_1-1] = A[i_1-2] * C[i_1+1] + 3;
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                A[i_1-1] = 6;
            }
        }
        C[i_0+1] = D[i_0] - D[i_0] * E[i_0][1] + C[i_0+2] + 6;
    }
#pragma endscop
...
kernel_3314212215_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
