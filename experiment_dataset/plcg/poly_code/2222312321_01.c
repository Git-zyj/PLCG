...
void kernel_2222312321_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0-2][i_1] = A[i_0][i_1] * A[i_0-1][i_1] - A[i_0-3][i_1] * A[i_0-3][i_1-1] + A[i_0-4][i_1] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
            B[i_2][i_3+2] = C[i_2][i_3-1] * 5;
        }
    }
#pragma endscop
...
kernel_2222312321_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
