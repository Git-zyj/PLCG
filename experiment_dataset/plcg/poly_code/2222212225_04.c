...
void kernel_2222212225_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_Q+1; i_3++) {
            B[i_2+1][i_2] = C[i_2][i_3][i_3+i_2-1] * B[i_2+2][i_2+1] * B[i_2][i_2-1] + 3;
        }
    }
#pragma endscop
...
kernel_2222212225_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
