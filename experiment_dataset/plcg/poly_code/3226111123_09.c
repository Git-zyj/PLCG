...
void kernel_3226111123_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M, i_0); i_2++) {
                A[i_2+i_0][i_1+1][i_0] = A[i_2+i_0][i_1-1][i_0] - 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_P; i_4++) {
            for (int i_5 = max(1, i_3); i_5 < min(-i_3+PB_M, PB_P); i_5++) {
                A[i_5+i_3][i_4+2][i_5] = C[i_3][i_5][i_4] - C[i_3][i_5-1][i_4-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3226111123_09(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
