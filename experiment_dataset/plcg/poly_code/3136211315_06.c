...
void kernel_3136211315_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(i_0, -i_1+PB_L); i_2++) {
                A[i_1-1][i_2] = B[i_0][i_2+i_1][i_1] + A[i_1-1][i_2-1] * A[i_1][i_2] * A[i_1-1][i_2+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3136211315_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
