...
void kernel_4416251325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,int wb,DATA_TYPE POLYBENCH_5D(B,xB,yB,zB,vB,wB,xb,yb,zb,vb,wb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,int wl,DATA_TYPE POLYBENCH_5D(L,xL,yL,zL,vL,wL,xl,yl,zl,vl,wl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] * F[i_0][i_0] - G[i_0] + G[i_0] + A[i_0-1] - D[i_0+1][i_0] - 4;
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_P; i_3++) {
                    B[i_0][i_3+i_1][i_1][i_2][i_0] = H[i_3][i_2+1][i_1][i_0+1] + I[i_3+1][i_1][1] + J[i_1][i_2+1][i_0][i_0-1] + K[i_3-1][i_2][i_1] + J[i_0][i_3][i_1][i_2+1] * L[i_2-1][i_3][i_0][i_1][i_1] + B[i_0][i_3+i_1-2][i_3][i_2][i_0] - B[i_0+1][i_3+i_1][i_3][i_2][i_0+1] + 1;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-3; i_4++) {
        C[i_4+1] = E[i_4] * E[i_4] * C[i_4+3] * C[i_4-1] + C[i_4+2] + D[i_4+1][i_4] + 2;
        for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_M); i_5++) {
            D[i_4+1][i_4] = E[i_5+i_4] - F[i_5][i_4] - F[i_5+1][i_4-1] * F[i_4][i_5] * F[i_5][i_4] - D[i_4-1][i_4-2] - D[i_4+2][i_4+1] + 2;
        }
    }
#pragma endscop
