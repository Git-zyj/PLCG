...
void kernel_3326151323_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-3; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_1][i_0-1] = D[i_1][i_0] + D[i_1+1][i_0] - E[i_0] - D[i_1][i_0] - D[i_1][i_0+1] - E[i_0+1] + A[i_1][i_0-2] + C[i_0+2][i_1+1] - 5;
        }
        for (int i_2 = 1; i_2 < min(i_0, -i_0+PB_P-3); i_2++) {
            for (int i_3 = 0; i_3 < -i_0+PB_P-1; i_3++) {
                B[i_3][i_2][i_2+i_0] = F[i_2-1][i_3][i_0] * F[i_0-1][i_3][i_2-1] * D[i_3][i_2] - F[i_0][i_2-1][i_2-1] * F[i_3+i_2][i_0-1][0] + B[i_3][i_2+2][i_2+i_0+3] * B[i_3][i_2][i_2+i_0-2] - B[i_3][i_2+2][i_2+i_0+2] + B[i_0][i_3][i_3+i_2+1] * B[i_2][i_3+1][i_3+i_0+1] + 4;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_P; i_5++) {
            C[i_4][i_5-1] = D[i_5][i_5] * E[i_5-1] * D[i_4][i_5] - C[i_4][i_5-2] * 6;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_3326151323_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
