...
void kernel_3234152111_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M, i_0); i_1++) {
            for (int i_2 = 2; i_2 < min(PB_M, -i_0+PB_L-1); i_2++) {
                B[i_2+i_0+1][i_1][2] = C[i_1][i_2+1][i_1] - C[i_2][i_2][i_0+1] + D[i_0][i_1-2] * C[i_1][i_2+i_0][i_2-2] - 1;
            }
        }
        for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 2; i_4 < min(PB_M, -i_0+PB_L); i_4++) {
                B[i_4+i_0][i_3][2] = D[i_3][i_4+i_0] * C[i_3+1][i_4][i_0] * E[i_0-1][i_4][i_3][i_0] - C[i_4-2][i_3][i_0] - C[i_3-2][i_0][i_4] - B[i_4+i_0-1][i_3][2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3234152111_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
