...
void kernel_2236212115_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N+1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_N+1, i_0); i_1++) {
            A[i_1+i_0-2][i_1+i_0-2] = A[i_1+i_0-1][i_1+i_0-1] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N+2; i_3++) {
            A[i_3+i_2-2][i_3+i_2-2] = C[i_2] + 2;
        }
    }
#pragma endscop
...
kernel_2236212115_06(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
