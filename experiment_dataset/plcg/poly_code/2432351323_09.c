...
void kernel_2432351323_09(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M+1, PB_M-1); i_1++) {
            C[i_1-1] = E[i_1+1] - F[i_1+i_0-1][i_1+1] + G[i_1][i_0-1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            C[i_3] = H[i_2-1] * G[i_3][i_2] * G[i_3-1][i_2] - F[i_3][i_2] + F[i_3][i_2] + H[0] - F[i_2-1][i_3+1] + 1;
        }
    }
    for (int i_4 = 3; i_4 < PB_M-1; i_4++) {
        C[i_4-1] = I[i_4-1] + H[i_4] - H[i_4] + C[i_4+1] * C[i_4-3] + 1;
        for (int i_5 = 2; i_5 < PB_M; i_5++) {
            C[i_5-2] = F[i_5-1][i_4] * F[i_4+1][i_5] - J[i_5-1][i_4] + F[i_5][i_4-1] - E[i_5] - E[i_5] * F[i_4][i_5-1] + C[i_4-1] + 6;
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2432351323_09(xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
