...
void kernel_4334152325_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M+2; i_1++) {
            A[i_0][i_0] = D[i_0][i_1] + D[i_0][i_1+i_0+2] * D[i_0-1][i_1] - D[i_0+1][i_1-1] * D[i_1+i_0-2][i_1] + D[i_1][i_0] + D[i_0][i_1] + D[i_1][i_0] - A[i_0-2][i_0-2] * 3;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            A[i_0][i_0] = D[i_0][i_2-1] * D[i_2][i_0-2] * E[i_2][i_0][0] - A[i_0][i_0] * 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            for (int i_5 = i_3; i_5 < min(PB_M, -i_4+PB_L-3); i_5++) {
                C[i_3][i_5+i_4+1][i_5] = E[i_4][i_3-1][i_5] - E[i_5][i_3][i_4] - A[i_4+1][i_4+1] + A[i_4][i_4] * C[i_3+2][i_5+i_4+3][i_4+2] * C[i_3][i_5+i_4+3][i_5+2] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4334152325_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
