...
void kernel_3316111223_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
            B[i_3+1] = D[i_3][i_4] * B[i_4+2] * B[i_3-1] - C[i_4-2] * 6;
        }
        C[i_3] = C[i_3+2] - 2;
    }
#pragma endscop
...
kernel_3316111223_04(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
