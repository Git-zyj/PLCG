...
void kernel_3424251213_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0+1] - E[i_0] * E[i_0] + E[i_0+1] * F[i_0][i_0] - E[i_0] + G[i_0] + A[i_0+1] + 1;
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            B[i_0][i_1-1] = G[i_0] - H[i_1-1][i_0-1] + F[i_1][i_1+i_0] + H[i_1+i_0][i_0-1] + A[i_1] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_2][i_3] = H[i_3-1][i_2+1] + H[i_2][i_3] + B[i_2][i_3-1] + 5;
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < min(i_4, PB_Q); i_5++) {
            for (int i_6 = 1; i_6 < min(min(-i_5+PB_P, PB_L-2), -i_4+PB_L); i_6++) {
                D[i_4][i_5+1][i_6] = F[i_5-1][i_6-1] - I[i_4-1][i_6+i_4][1] + J[i_4][i_6+i_5][i_5] - I[i_6+1][i_4][0] + D[i_4][i_6+2][i_5] - D[i_4][i_5+2][i_6+1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3424251213_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
