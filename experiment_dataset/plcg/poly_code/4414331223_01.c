...
void kernel_4414331223_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,int wb,DATA_TYPE POLYBENCH_5D(B,xB,yB,zB,vB,wB,xb,yb,zb,vb,wb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
        ...
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
                for (int i_3 = 2; i_3 < -i_1+PB_L-1; i_3++) {
                    A[i_1][i_3][i_2][i_0-1] = E[i_3][i_2-1][i_0+1][i_1] + A[i_1][i_3+2][i_0][i_2-2] + A[i_1][i_3+2][i_2][i_0-1] - A[i_1][i_3-2][i_2+2][i_0-1] + 5;
                    B[i_2][i_3+i_1+1][i_0][0][i_1+1] = F[i_1][i_3-1][i_2] + G[i_0][i_1][i_3][i_2] - H[i_1-1][i_2][i_0-1][i_1][i_2-1] * 3;
                }
            }
        }
        for (int i_4 = 0; i_4 < min(i_0, PB_L-2); i_4++) {
            for (int i_5 = 3; i_5 < min(i_4, -i_4+PB_L-2); i_5++) {
                for (int i_6 = max(-i_5+3, 2); i_6 < min(-i_0+PB_M, -i_4+PB_M-1); i_6++) {
                    C[i_6][i_5+i_4][i_5] = I[i_6+i_4+1][i_4][i_0-1][i_6] - I[i_6+i_0][i_4][i_5][i_4] * C[i_0][i_6+i_5-3][i_6-2] - C[i_6][i_5+i_4+2][i_5] * 1;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
        D[i_7] = J[i_7-1] * J[i_7] - K[i_7+1] - D[i_7+1] + 3;
    }
#pragma endscop
