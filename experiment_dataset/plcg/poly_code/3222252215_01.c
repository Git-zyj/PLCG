...
void kernel_3222252215_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < min(min(i_0, -i_0+PB_L+1), PB_L-2); i_1++) {
            A[i_0] = C[i_1+i_0-1][0] + D[i_1] - C[i_1-2][i_0+1] * C[i_1+2][i_0] + A[i_0-1] * A[i_0+1] + 1;
        }
        for (int i_2 = PB_M-1 - 1; i_2 >= 0; i_2--) {
            A[i_2+1] = C[i_2][i_0] * D[i_2] * C[i_2][i_0] - C[i_2][i_0+2] * C[i_2][i_0] * 5;
        }
    }
#pragma endscop
...
kernel_3222252215_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
