...
void kernel_4314212223_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_1] = D[i_0+2][i_1] + C[i_0-1] + 2;
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                A[i_2] = E[i_0][i_1+2] * 2;
            }
        }
        C[i_0-1] = A[i_0+1] - A[i_0] - A[i_0-1] + 4;
    }
#pragma endscop
...
kernel_4314212223_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
