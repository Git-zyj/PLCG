...
void kernel_3334211221_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            for (int i_4 = 1; i_4 < min(min(-i_2+PB_N+1, -i_3+PB_N+2), PB_L+1); i_4++) {
                B[i_3][i_4-1][i_2] = A[i_4+i_3-2][i_3] - A[i_4+i_2-1][i_2-1] + 5;
            }
        }
        for (int i_5 = 1; i_5 < i_2; i_5++) {
            for (int i_6 = 1; i_6 < PB_M-2; i_6++) {
                C[i_2][i_5][i_6-1] = D[i_5-1][i_6][i_2+1] + D[i_5][i_2+1][i_6] * C[i_6+2][i_5][i_2-1] - C[i_2-2][i_5][i_6-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3334211221_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
