...
void kernel_3424232125_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
    ...
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            for (int i_3 = 1; i_3 < i_1; i_3++) {
                C[i_1-2][i_3][i_1-2] = F[i_3][i_2][i_1] * G[i_1][i_3] + H[i_3-1][i_1] * 1;
            }
        }
        for (int i_4 = i_1; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_M; i_5++) {
                C[i_1][i_5][i_1] = F[i_5+i_4][i_1][i_5] - F[i_4-2][i_1][i_5] * H[i_5+2][i_1+1] * G[i_5+1][i_5] + I[i_1-2][i_4][i_5] + C[i_1+2][i_5][i_1+2] - C[i_4-1][i_1][i_4-1] - 6;
            }
        }
        for (int i_6 = 0; i_6 < PB_M; i_6++) {
            for (int i_7 = 0; i_7 < PB_M; i_7++) {
                C[i_1][i_7][i_1] = H[i_1][i_6] + H[i_1][i_7] - F[i_6][i_7][i_1-2] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3424232125_01(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
