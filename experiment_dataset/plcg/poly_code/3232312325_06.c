...
void kernel_3232312325_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        A[i_0] = C[i_0+1] * A[i_0+2] - A[i_0-2] + A[i_0+1] * A[i_0-1] + 3;
    }
    for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
        for (int i_2 = i_1; i_2 < PB_N; i_2++) {
            for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                B[i_1+2][i_1][i_3] = B[i_1+2][i_1][i_3+1] - B[i_1+3][i_1+1][i_3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3232312325_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
