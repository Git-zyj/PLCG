...
void kernel_3222251113_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                A[i_2][i_1+i_0][0] = C[i_2+1][1] + D[i_1][i_0][i_2] * E[i_0][i_0][i_2+1] + C[i_2][i_1+1] - 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-3; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < min(min(-i_3+PB_M-3, PB_P), i_4); i_5++) {
                B[i_3-1][i_5+i_3+1][i_4] = E[i_5][i_3-1][i_5] - D[i_5][i_3+1][i_4+1] + E[i_4][i_3][i_5-1] - D[0][i_3][i_4-1] * D[i_5][i_3][i_4] - B[i_3-1][i_5+i_3][i_4+1] - B[i_3][i_5+i_3+3][i_4] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3222251113_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
