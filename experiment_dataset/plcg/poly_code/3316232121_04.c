...
void kernel_3316232121_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] + D[i_0+1] + E[i_0] * D[i_0] * D[i_0] + A[i_0-1] + 6;
        B[i_0] = D[i_0-1] - E[i_0] - D[i_0+1] * A[i_0] * 6;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                C[i_2-1][i_0][i_2-2] = F[i_2][i_0+2][i_1+1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3316232121_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
