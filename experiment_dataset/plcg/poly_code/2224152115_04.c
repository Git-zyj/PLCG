...
void kernel_2224152115_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        B[i_0-3] = C[i_0+2] + D[i_0+2][i_0] + C[i_0] + 1;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = max(3, i_1); i_2 < min(PB_L, -i_1+PB_M-2); i_2++) {
            B[i_2-2] = D[i_2+i_1+2][i_2] - D[i_2][i_1] - D[i_2][i_1] + D[i_1][i_2] - D[i_2-2][i_1-2] * D[i_1][i_2] - D[i_2-1][i_1] - B[i_2-3] + 1;
        }
    }
#pragma endscop
...
kernel_2224152115_04(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
