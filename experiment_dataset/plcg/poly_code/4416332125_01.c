...
void kernel_4416332125_01(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(min(i_0, -i_0+PB_P-2), PB_L-1); i_1++) {
            D[i_1+1] = E[i_1] - F[i_0] + F[1] - G[i_1-2][i_0+1] + 2;
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                for (int i_3 = 0; i_3 < min(PB_Q, -i_2+PB_P-1); i_3++) {
                    C[i_1-1][i_3+i_2+1][i_3] = 4;
                }
            }
            for (int i_4 = 0; i_4 < PB_M; i_4++) {
                C[i_4][i_1+i_0][i_1] = H[i_1-1][0] * C[i_4][i_1+i_0][i_1-1] + C[i_4][i_1+i_0+2][i_1] * 3;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
        D[i_5+1] = I[i_5] - F[i_5-2] - I[i_5] - F[i_5-1] + F[i_5] - F[i_5] - 5;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4416332125_01(xd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
