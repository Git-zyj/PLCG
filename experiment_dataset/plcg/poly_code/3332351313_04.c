...
void kernel_3332351313_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L; i_1++) {
            A[i_1][i_1-1] = D[i_0+1][i_1] * E[0] * F[i_0-1][i_1] * G[i_1+i_0][0] * E[i_1-1] + G[i_1][i_0-1] - A[i_1-1][i_1-2] + A[i_1+1][i_1] * A[i_1-1][i_1-2] + A[i_1-1][i_1-2] + 1;
        }
        for (int i_2 = 0; i_2 < PB_N; i_2++) {
            A[i_0][i_0-1] = D[i_2][i_0] - D[i_2+1][i_0+1] + D[i_0+1][i_2+1] * F[0][i_0+1] + 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_N+1; i_3++) {
        for (int i_4 = 2; i_4 < PB_N; i_4++) {
            for (int i_5 = i_3; i_5 < -i_4+PB_N; i_5++) {
                C[i_3-1][i_5][i_4-1] = G[i_5][i_4-1] + H[i_4][1][i_5+i_4] * I[i_3-1][i_4][0] + F[i_5+i_4][1] + C[i_3][i_5][i_4-2] - C[i_3][i_5+1][i_4-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3332351313_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
