...
void kernel_3236111125_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            B[i_1-1] = C[i_0+1][i_1] - 6;
        }
        for (int i_2 = 1; i_2 < i_0; i_2++) {
            B[i_2] = C[i_2-1][i_0] + B[i_0+2] * 4;
        }
    }
#pragma endscop
...
kernel_3236111125_06(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
