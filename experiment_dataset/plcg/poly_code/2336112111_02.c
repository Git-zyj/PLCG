...
void kernel_2336112111_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = 2; i_2 < min(-i_0+PB_M, PB_L-2); i_2++) {
            B[i_2+2][i_0] = D[i_2-2][i_0-1] + D[i_0][i_2+2] + D[i_2+i_0][i_2+1] + B[i_2+1][i_0] * 4;
        }
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            C[i_3] = C[i_3-1] + C[i_0+1] * 6;
        }
    }
#pragma endscop
...
kernel_2336112111_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
