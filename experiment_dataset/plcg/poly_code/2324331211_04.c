...
void kernel_2324331211_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L+1, PB_M-1); i_1++) {
            A[i_0][i_0] = D[i_1+i_0-1][i_1-1] + D[i_1+1][i_1] + D[0][i_1] - E[i_1+i_0-1] - F[i_1][i_0] - A[i_1-1][i_1-1] + B[i_1-1][i_0] * 1;
            B[i_1][i_0] = B[i_1+1][i_0] - B[i_1][i_0+1] - B[i_1][i_0-1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_N-1; i_3++) {
            C[i_3][i_2] = D[i_2][i_3] + D[i_2-1][0] + G[i_3] * D[i_3-1][i_2] + C[i_3+1][i_2] - 1;
        }
    }
#pragma endscop
...
kernel_2324331211_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
