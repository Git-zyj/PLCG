...
void kernel_3432152113_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            for (int i_2 = i_0; i_2 < -i_0+PB_Q-2; i_2++) {
                A[i_1][i_2+2] = E[i_0-2][i_2-1][i_1][i_0] * F[i_1][i_2+2][i_0] * G[i_2+2][i_1] + G[i_1][i_2] - E[i_1-1][0][i_0+2][i_2+i_0+2] - F[i_2][i_2][i_0] * F[i_0-1][i_1][i_2] - A[i_1+1][i_2+2] - 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 2; i_4 < -i_3+PB_M; i_4++) {
            C[i_4-2] = H[i_3] * F[i_4+i_3][i_4][2] - 4;
        }
        C[i_3-1] = H[i_3] + H[i_3] - H[i_3] * H[i_3] + H[i_3+2] + H[i_3-2] * H[i_3-1] - 4;
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
            for (int i_7 = 0; i_7 < PB_L-2; i_7++) {
                A[i_7][i_6+3] = F[i_6][i_7][0] * F[i_6][i_5-1][i_5] * F[i_6+2][i_5][i_7+2] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3432152113_06(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
