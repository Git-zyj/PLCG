...
void kernel_3414251325_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        B[i_0-1] = E[i_0-1] - E[i_0] * F[i_0-1] + F[i_0+1] + E[i_0+1] + E[0] - B[i_0] + 6;
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            B[i_1-1] = F[0] * F[i_1] - G[i_1][i_0] + B[i_1+1] - B[i_1-2] * D[i_0-1] + 2;
        }
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 1; i_3 < -i_2+PB_L+1; i_3++) {
                B[i_2-1] = H[i_0+1][i_3+i_2-1][i_2-1] * H[i_3-1][i_2][i_0] * I[i_3][i_0-1][i_2+1] - H[i_2][i_0][i_0+1] + 5;
            }
        }
    }
    for (int i_4 = 3; i_4 < PB_M-1; i_4++) {
        D[i_4-1] = F[i_4+1] + E[i_4] + F[i_4] * E[i_4] + D[i_4+1] - D[i_4-3] - 6;
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3414251325_03(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
