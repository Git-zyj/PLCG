...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_P+1, i_1); i_2++) {
            for (int i_3 = 1; i_3 < min(PB_M+1, -i_1+PB_P+3); i_3++) {
                D[i_3-1][i_3+i_1-3] = D[i_1][i_2+i_1-1] - 1;
            }
            D[i_1][i_2+i_1-2] = G[i_1][i_2] + H[i_2-1][i_1+2][i_1] * 1;
        }
    }
#pragma endscop
...
kernel_4416312113_04(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
