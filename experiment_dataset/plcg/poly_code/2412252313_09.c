...
void kernel_2412252313_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            B[i_1-1] = E[0] + F[i_1][i_0] + G[i_1][i_0+1] * H[i_1] - G[i_0+1][i_1] * G[i_1+2][i_0] * B[i_0] - 5;
        }
        B[i_0-1] = E[2] * E[i_0] * H[i_0] * E[i_0] * B[i_0-2] + B[i_0] + 2;
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = PB_M - 1; i_3 >= 0; i_3--) {
            B[i_3] = F[i_3][i_2] * I[i_3][i_2-1][i_2-2] * G[i_3][i_2] - G[0][i_2] + 2;
        }
        B[i_2-1] = H[i_2] - E[i_2] - E[i_2-1] - H[i_2] - G[i_2][i_2] + 6;
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2412252313_09(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
