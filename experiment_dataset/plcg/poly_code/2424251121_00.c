...
void kernel_2424251121_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M+1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_L, i_0); i_1++) {
            B[i_1-2] = E[i_0-1][i_1] * F[i_0][i_1] - F[i_0][i_1] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2] = G[i_2-1] + F[i_2+1][i_2] + H[i_2] + G[i_2+1] * G[i_2] + 3;
    }
    for (int i_3 = 3; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            C[i_3][i_4][i_4-1] = E[i_4][i_3] + F[i_4-1][i_3] + I[i_4-1][i_3-1][i_4+1] * E[i_4][i_3+1] - C[i_4-1][i_3-2][i_3-3] + 5;
        }
        for (int i_5 = 2; i_5 < min(-i_3+PB_M, PB_L-1); i_5++) {
            D[i_5-1] = G[0] + H[i_3] + E[i_5][i_3] - E[i_3][i_5] - E[i_5+i_3][i_5+1] - E[i_5][i_3+1] - D[i_5-2] + 4;
        }
    }
#pragma endscop
...
kernel_2424251121_00(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
