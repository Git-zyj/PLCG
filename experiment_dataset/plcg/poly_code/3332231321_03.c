...
void kernel_3332231321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 3; i_1 < PB_M-1; i_1++) {
            A[i_0+1][i_1-1] = D[i_0][i_1] + E[i_0] + D[i_0][i_1] + A[i_0+1][i_1-3] + A[i_1+1][i_0-3] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_L, -i_2+PB_P); i_4++) {
                B[i_2][i_4][i_2] = D[i_2][i_3] - F[i_3-1][i_4+i_2][i_3] * B[i_2+1][i_3][i_2+1] - C[i_3][i_3] - C[i_2][i_2] + 4;
            }
        }
        for (int i_5 = 0; i_5 < min(-i_2+PB_N, PB_M-1); i_5++) {
            C[i_2][i_2] = G[i_5][i_2] * G[i_5+i_2][i_5] - D[i_5+1][i_2-1] - C[i_2-1][i_2-1] - C[i_5+1][i_5+1] + C[i_2+2][i_2+2] * 6;
        }
    }
#pragma endscop
...
kernel_3332231321_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
