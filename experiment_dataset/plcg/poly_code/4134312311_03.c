...
void kernel_4134312311_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_N-2, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
                for (int i_3 = 0; i_3 < min(min(min(-i_0+PB_N-1, PB_Q-3), -i_2+PB_N-1), -i_1+PB_N-2); i_3++) {
                    A[i_3+i_0][1][i_2][i_3+2] = B[i_1][i_1][i_3] + A[i_3+i_0+1][1][i_2+1][i_3+2] + A[i_3+i_1+2][1][i_0][i_3+3] + A[i_3+i_2+1][1][i_1][i_3+3] * 1;
                }
            }
        }
    }
...
kernel_4134312311_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
