...
void kernel_4334311225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(min(i_0, -i_0+PB_M+1), PB_M-2); i_1++) {
            A[i_1] = D[i_1+i_0-1][i_0] * E[i_1][i_0] * A[i_1+2] + A[i_1-2] * 6;
        }
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            for (int i_3 = 0; i_3 < i_0; i_3++) {
                A[i_3] = F[i_3][i_0-1][i_2] + 6;
            }
        }
    }
...
kernel_4334311225_06(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
