...
void kernel_3432232315_07(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
            for (int i_3 = 1; i_3 < min(PB_M, -i_2+PB_Q); i_3++) {
                C[i_3][i_1][i_2+2] = F[i_3][i_1][i_2-1] * F[i_3-1][i_1][i_3+i_2] - 5;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
            for (int i_6 = i_4; i_6 < min(-i_4+PB_P+1, PB_M-1); i_6++) {
                C[i_4][i_6][i_5+2] = G[i_6][i_5+1] - F[i_4][i_6][i_5] * H[i_5][i_4+2][i_6] + H[i_5+1][i_4][i_6+i_4-1] + F[0][i_5][i_6+i_4] + C[i_4][i_6+1][i_5+2] + C[i_4+1][i_6][i_5+2] * 1;
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_L; i_7++) {
        for (int i_8 = 1; i_8 < PB_L-1; i_8++) {
            D[i_8] = I[i_7] - D[i_8+1] + D[i_8-1] * D[i_8-1] * 6;
        }
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3432232315_07(xd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
