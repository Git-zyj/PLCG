...
void kernel_4216312321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0+2] = C[i_0-1][i_0] * A[i_0+1] - A[i_0+4] - A[i_0+3] - 6;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 3; i_2 < PB_P+2; i_2++) {
            for (int i_3 = i_2; i_3 < PB_P; i_3++) {
                B[i_1][i_2-2][i_3] = B[i_2+1][i_3][i_1] + B[i_1-1][i_2-3][i_3-1] - B[i_1][i_2-2][i_3-1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4216312321_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
