...
void kernel_4436132223_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
...
#pragma scop
        ...
        for (int i_1 = max(2, i_0); i_1 < min(-i_0+PB_M, PB_Q-2); i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_Q-2; i_3++) {
                    A[i_3][i_1][i_2-1][i_0] = E[i_1+i_0][i_2][i_3+i_1+2][i_3] - F[i_2][i_3][0] * F[i_0][i_3][i_2] - A[i_3][i_1-2][i_2-1][i_0+2] * 1;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        for (int i_5 = i_4; i_5 < PB_L-2; i_5++) {
            C[i_4] = G[i_4][i_5+2] * C[i_4-1] - 5;
        }
        C[i_4] = H[i_4-2] * H[i_4] + H[i_4] + C[i_4+1] * C[i_4-2] * 1;
    }
    for (int i_6 = 0; i_6 < PB_L; i_6++) {
        for (int i_7 = 0; i_7 < min(min(-i_6+PB_M, i_6), PB_L-2); i_7++) {
            C[i_7] = G[i_7+i_6][i_7+2] + G[i_7][i_6] - 2;
        }
    }
#pragma endscop
...
kernel_4436132223_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
