...
void kernel_4216312215_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 3; i_2 < PB_L+1; i_2++) {
                A[i_1-2][i_2-1] = A[i_2-3][i_1-1] - A[i_2-1][i_1-1] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_N-1, i_3); i_4++) {
            B[i_4+i_3][i_4] = C[i_4][i_3] * D[i_3-1] - B[i_4+i_3-1][i_4] - B[i_4+i_3+1][i_4] * 2;
        }
    }
#pragma endscop
...
kernel_4216312215_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
