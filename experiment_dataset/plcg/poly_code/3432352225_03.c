...
void kernel_3432352225_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + F[i_0] * F[i_0] * F[i_0] + E[i_0+2] - E[i_0] * E[i_0] + E[i_0] - 2;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_M, i_1); i_2++) {
            B[i_2+2][i_1] = G[i_2][i_1] - G[i_2+i_1][i_1-2] + G[i_2-1][i_1-1] * H[i_2][i_2-2][i_1] - B[i_2+1][i_1+2] * B[i_2+2][i_1-2] - B[i_2+2][i_1+1] + 5;
        }
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
                D[i_4+1][i_3][i_1-2] = G[i_3][i_1] - I[i_4+1][i_1][i_3-2] - J[i_3-2][i_1][i_4+1] - 2;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = 1; i_6 < -i_5+PB_M; i_6++) {
            for (int i_7 = 0; i_7 < PB_L; i_7++) {
                D[i_7+1][i_6][i_5] = J[i_6][i_5][i_7] + I[i_7][i_6+i_5][i_5] * I[i_6][i_5-1][i_5-1] - D[i_7+1][i_6-1][i_5] + 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3432352225_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
