...
void kernel_3226131123_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            for (int i_2 = i_0; i_2 < -i_1+PB_P-1; i_2++) {
                A[i_0][i_1+i_0-1][i_2-1][1] = C[i_0-1][i_2+1] + D[i_2][i_0+1][i_1] - D[i_2+i_1][i_0][i_0] + E[i_0][i_2][i_1-1][i_2-1] + A[i_0][i_1+i_0][i_2-1][1] - A[i_2+2][i_2+i_1+1][i_0][1] + 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M+1, PB_L+1); i_4++) {
            B[i_3][i_4-1] = C[i_4+i_3-1][i_3-1] * 4;
        }
    }
#pragma endscop
...
kernel_3226131123_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
