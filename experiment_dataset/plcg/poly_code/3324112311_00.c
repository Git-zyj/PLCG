...
void kernel_3324112311_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M; i_1++) {
            A[i_1][i_0] = D[i_1][i_1] * A[i_1-1][i_0] * 2;
        }
    }
    ...
    for (int i_4 = 1; i_4 < PB_N-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_N-1; i_5++) {
            C[i_4][i_5] = D[i_4][i_5-1] + C[i_4][i_5-1] + C[i_4+1][i_5] - C[i_5+1][i_4] * C[i_4-1][i_5] * 4;
        }
    }
#pragma endscop
...
kernel_3324112311_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
