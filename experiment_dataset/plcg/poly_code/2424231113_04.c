...
void kernel_2424231113_04(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_P, -i_1+PB_M-1); i_2++) {
            B[i_1][i_2] = F[i_2] - G[i_1-1][i_1] * H[i_2-1][i_1] * B[i_1][i_2+1] - 2;
            B[i_1+1][i_2] = I[i_2][i_1][0] - G[i_2][i_1+1] - H[i_2-1][i_1] * G[i_2][i_1] * G[i_2+i_1+1][i_2+1] * H[i_2+i_1][i_2] - B[i_1][i_2+1] + 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        D[i_3] = E[i_3-1] - F[i_3] - 1;
    }
#pragma endscop
...
kernel_2424231113_04(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
