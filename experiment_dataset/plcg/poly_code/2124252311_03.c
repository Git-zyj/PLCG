...
void kernel_2124252311_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < min(PB_N, -i_0+PB_L); i_1++) {
            A[i_1-2] = B[i_1+2] * C[i_1][i_0] - D[i_1][i_0-1] * C[i_1][i_1+i_0] + D[i_0][i_0] - A[i_1-3] + A[i_1-1] - A[i_1-1] + 2;
        }
    }
#pragma endscop
...
kernel_2124252311_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
