...
void kernel_2426351315_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0+1] - F[i_0] * G[i_0+1][i_0] - A[i_0+1] - A[i_0-1] * 4;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        A[i_1] = H[i_1][0] * E[i_1] - E[i_1] + I[i_1] + F[i_1-1] * H[i_1-1][i_1] * F[i_1+1] + F[i_1] - A[i_1-1] * 1;
        A[i_1+1] = E[0] - F[i_1] - E[i_1] - I[i_1-1] - 2;
        for (int i_2 = 1; i_2 < min(-i_1+PB_P, i_1); i_2++) {
            A[i_2+1] = G[i_2+1][i_1] + F[i_2-1] - J[i_2][i_2+i_1][i_1-1] - G[i_1][i_2] - G[i_2][i_1+1] * 2;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2426351315_07(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
