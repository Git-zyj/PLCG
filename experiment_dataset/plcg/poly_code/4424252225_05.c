...
void kernel_4424252225_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_L, -i_0+PB_P-1); i_2++) {
                for (int i_3 = 4; i_3 < -i_0+PB_L-1; i_3++) {
                    A[i_0-2][i_3][i_0-2][i_2] = E[i_2+i_1][i_3+i_0-2][i_3][0] + F[i_1][i_2-2][i_3+i_0+1][i_0] - F[i_2+i_1][i_3-2][i_0-1][i_2+i_0+1] * A[i_3-4][i_0][i_3-4][i_2+2] + A[i_0-2][i_1][i_0-2][i_2+2] * 2;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 2; i_5 < PB_L+2; i_5++) {
                A[i_0-2][i_5-2][i_0-2][i_4] = G[i_0-2][i_4] - 4;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_L-4; i_6++) {
        for (int i_7 = 0; i_7 < -i_6+PB_L-4; i_7++) {
            C[i_7+i_6][i_7+1] = H[i_6-1][i_7] * G[i_7+1][i_6] + I[i_7+1] - G[i_7+1][i_6] + H[i_6][i_7] + H[i_7][i_6] * H[i_7][i_6-2] + G[i_7][i_6-1] * G[i_7][i_6-2] - I[i_6] - J[i_7][i_6][i_6] - C[i_7+i_6+4][i_6+3] * C[i_7+i_6+1][i_7+2] + 3;
        }
    }
    for (int i_8 = 0; i_8 < PB_L; i_8++) {
        for (int i_9 = 0; i_9 < min(min(-i_8+PB_L, i_8), PB_M-2); i_9++) {
            C[i_9+i_8][i_9+1] = H[i_9+2][0] + H[i_9][i_8] * 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4424252225_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
