...
void kernel_2212112323_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 4; i_1 < PB_N; i_1++) {
            A[i_0-1][i_1-2] = B[i_1-1] * B[i_1-3] - 2;
            B[i_1-1] = C[i_0][i_1][i_0-2] - A[i_0-3][i_1-4] * A[i_0-1][i_1-2] + A[i_0-2][i_1-2] + B[i_1] * 1;
        }
    }
#pragma endscop
...
kernel_2212112323_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
