...
void kernel_4236231313_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_M-2); i_1++) {
            A[i_1+1][i_1] = C[i_1+i_0][i_1+1][i_0-1] - D[i_0][i_1] - A[i_1][i_1-1] - A[i_1+2][i_1+1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, PB_M-1); i_4++) {
                A[i_4+1][i_4] = D[i_4][i_3] - D[i_3][i_4] * E[i_2][i_4+i_3][i_3] * A[i_4+1][i_4] + 3;
            }
        }
    }
#pragma endscop
...
kernel_4236231313_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
