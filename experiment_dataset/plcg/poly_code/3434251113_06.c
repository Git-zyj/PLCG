...
void kernel_3434251113_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = E[i_0-1] * E[i_0] + 4;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            A[i_0] = F[i_0][i_1] + F[i_1+i_0+1][1] - F[i_1][0] * F[i_1+i_0][0] + F[i_1-1][i_0] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_M, -i_2+PB_M-1); i_4++) {
                C[i_3+1][i_4] = F[i_3+1][i_4] + G[i_4][i_3] - H[i_4+i_3][i_2-1][i_3+1] + H[i_3][i_3][i_2] * H[i_4+i_2+1][i_3][i_3-1] + H[i_4-1][i_2][i_3] + C[i_3+1][i_4-1] + 2;
            }
        }
        for (int i_5 = 1; i_5 < i_2; i_5++) {
            for (int i_6 = 0; i_6 < PB_L-1; i_6++) {
                C[i_5+1][i_6] = I[i_2][i_6][i_5-1] - H[i_5][i_6][i_2-1] + H[i_5][i_2][i_6] + I[i_5][i_6+1][i_2-1] + I[i_6+1][i_2+1][i_5] - H[i_5][i_6][i_2] - I[i_2-1][i_6][i_5] - 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3434251113_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
