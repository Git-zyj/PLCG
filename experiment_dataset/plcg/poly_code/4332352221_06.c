...
void kernel_4332352221_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            A[i_1-1][i_0-1] = D[i_1+i_0] * E[i_0-1] - F[i_0][i_1] - D[i_1] * G[i_1][i_0-2] - H[i_1][i_0-1] * A[i_1][i_0-1] + A[i_1-2][i_0-1] * 3;
        }
    }
        ...
        for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 2; i_4 < PB_M-3; i_4++) {
                for (int i_5 = 2; i_5 < min(-i_3+PB_P, -i_4+PB_L); i_5++) {
                    B[i_3+2][i_4-2][i_2+2][i_5] = I[i_2][i_5+1][i_3] * J[i_2-2][i_3][i_4][i_5+i_3] * K[i_5][i_5][i_2-1][i_3] - J[i_2][i_5+i_4][i_3][i_3] + B[i_4+3][i_3-2][i_2+3][i_5-2] - B[i_3+2][i_4-2][i_2+3][i_5-1] + 2;
                }
            }
        }
        for (int i_6 = 1; i_6 < PB_M; i_6++) {
            C[i_6][i_2+1] = F[i_6][i_2-1] - L[0] + L[i_2-1] - F[i_6][i_2] - F[i_6+1][i_2-1] * C[i_6-1][i_2+1] - 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4332352221_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
