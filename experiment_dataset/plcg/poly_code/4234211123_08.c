...
void kernel_4234211123_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = i_0; i_2 < PB_L; i_2++) {
                A[i_1][i_2][i_0-1] = A[i_1-1][i_2-1][i_0-1] + A[i_1][i_2][i_0-2] * 5;
            }
        }
        B[i_0] = C[i_0] * D[i_0] * 3;
    }
...
kernel_4234211123_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
