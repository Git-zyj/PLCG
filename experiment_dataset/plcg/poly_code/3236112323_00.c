...
void kernel_3236112323_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(min(-i_0+PB_L+2, PB_L-4), i_0); i_1++) {
            for (int i_2 = 0; i_2 < min(min(i_0, PB_M-1), -i_1+PB_L-4); i_2++) {
                A[i_1][i_2+i_1] = A[i_1][i_2+i_1-2] + A[i_2+1][i_2+i_1-1] - A[i_1+2][i_2+i_1+4] * 3;
            }
            A[i_1-2][i_1+i_0-2] = C[i_0][i_1] - 4;
        }
    }
#pragma endscop
...
kernel_3236112323_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
