...
void kernel_4434132123_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0-1] * E[i_0-1] * E[i_0+1] * A[i_0+1] * 3;
        B[i_0+2] = F[i_0][i_0] - E[0] * 5;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        C[i_1-1] = E[i_1] * E[i_1] - 3;
        for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
            for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
                for (int i_4 = 0; i_4 < PB_Q-2; i_4++) {
                    D[i_1][i_3-1][i_2-1][i_1] = G[i_3][i_4][i_3+2] * H[i_3][i_1][i_2][i_4+2] - 1;
                }
            }
        }
    }
...
kernel_4434132123_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
