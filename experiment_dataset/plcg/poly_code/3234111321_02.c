...
void kernel_3234111321_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 3; i_1 < PB_P+1; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < PB_P; i_2++) {
                A[i_0+1][i_1-1][i_2] = A[i_0+1][i_1-3][i_2-2] - A[i_1-1][i_2+1][i_0] + A[i_0+1][i_1-3][i_2-1] + A[i_0+1][i_2-1][i_1-1] * 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < i_3; i_4++) {
            B[i_4-1][0] = C[i_4][i_3] * B[i_3][0] + B[i_4-2][0] + 4;
        }
    }
#pragma endscop
...
kernel_3234111321_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
