...
void kernel_3216111325_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        A[i_0+1] = A[i_0+3] - A[i_0] - A[i_0+2] - A[i_0-1] * 6;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
                B[i_2+i_1][i_3+1][0] = C[i_3+1][i_2][i_1] * B[i_2+i_1-2][i_3+1][0] + B[i_3+i_2+1][i_1+1][0] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3216111325_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
