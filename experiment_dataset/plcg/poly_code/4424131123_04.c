...
void kernel_4424131123_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0+1] = E[i_0] * E[i_0+1] + E[i_0-1] + E[i_0-1] * 5;
        for (int i_1 = i_0; i_1 < min(PB_P, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_N-2); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_L, -i_2+PB_P); i_3++) {
                    B[i_2][i_1+i_0-1][i_3] = F[i_2+i_1][i_3+i_0][i_3+i_2][i_1][i_0] * G[i_3][i_1][i_0] * G[i_1-1][i_3+i_0][i_2-1] - B[i_2+2][i_1+i_0-1][i_3+1] + 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < PB_P-1; i_5++) {
            for (int i_6 = 1; i_6 < min(i_4, -i_5+PB_M); i_6++) {
                C[i_4][i_6][i_5-1] = H[i_6-1][i_5-1] + H[i_6+i_5][i_5+1] * G[i_6][i_4-1][i_5] + 6;
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4424131123_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
