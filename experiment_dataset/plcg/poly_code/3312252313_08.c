...
void kernel_3312252313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] - D[i_0+1] * E[i_0] * E[i_0-2] + E[i_0] * E[i_0] - D[i_0-1] + D[i_0] - 3;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M+2, PB_M-2); i_1++) {
            A[i_0-1] = F[i_0-2][i_1] - G[i_0][i_1] * E[i_1+i_0-2] + F[i_0+1][i_1+1] + E[i_1+2] + G[i_1+1][i_0] * A[i_1+1] + A[i_1] - A[i_1-1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            for (int i_4 = 0; i_4 < min(min(-i_3+PB_N, PB_P), -i_2+PB_L-1); i_4++) {
                C[i_3][i_4+i_2][i_4] = G[i_2-1][i_4+i_3] - C[i_3+1][i_4+i_2+1][i_4] + C[i_3][i_4+i_2-1][i_4] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3312252313_08(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
