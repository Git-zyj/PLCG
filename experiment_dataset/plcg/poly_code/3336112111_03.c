...
void kernel_3336112111_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_N; i_1++) {
            A[i_1][i_0+2][i_1-1] = D[i_1-2][i_0-1] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_N+1; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_M, i_2); i_3++) {
            for (int i_4 = 0; i_4 < min(i_2, PB_M-2); i_4++) {
                B[i_2-1][i_3][i_3-1] = E[i_4+2][i_2-2][i_3] + 1;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        for (int i_6 = i_5; i_6 < -i_5+PB_L; i_6++) {
            for (int i_7 = 2; i_7 < PB_M; i_7++) {
                C[i_5-1][i_6][i_7-1] = E[i_7][i_6+i_5][i_7-2] + C[i_7-2][i_6-1][i_5-1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3336112111_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
