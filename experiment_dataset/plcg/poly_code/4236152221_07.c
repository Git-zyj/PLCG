...
void kernel_4236152221_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
                B[i_0-3][i_1-2][i_2] = C[i_2-1][i_1] - D[i_2][i_2][i_1] + C[i_2][i_1+2] * C[i_1][i_0-1] - D[i_2+i_1][i_0][0] * 2;
            }
        }
        for (int i_3 = 2; i_3 < i_0; i_3++) {
            for (int i_4 = 3; i_4 < min(-i_3+PB_M, -i_0+PB_L); i_4++) {
                B[i_0-1][i_3-2][i_4] = C[i_0-1][i_3-1] * C[i_3+1][i_0] * D[i_3][i_4+i_0][i_3] + D[i_3-1][i_0+1][i_3] + D[i_4+i_3][i_4+i_0][i_0-2] - B[i_0-1][i_4-3][i_3] * B[i_0-1][i_3][i_4] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4236152221_07(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
