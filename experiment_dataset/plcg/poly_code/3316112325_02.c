...
void kernel_3316112325_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+i_0+2][0] = D[i_0][i_1-1] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
            B[i_3] = D[i_3-1][i_2] + B[i_3-2] - B[i_3+2] * B[i_3+1] * 2;
        }
        for (int i_4 = max(2, i_2); i_4 < PB_P; i_4++) {
            for (int i_5 = 0; i_5 < PB_L-4; i_5++) {
                C[i_5+2][i_4-1][i_2] = C[i_2+2][i_4-2][i_5+1] * C[i_5+2][i_4-2][i_2] * C[i_2+1][i_4-1][i_5] - C[i_5+4][i_4-1][i_2+1] + C[i_2+1][i_4][i_5] * C[i_5][i_4-1][i_2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3316112325_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
