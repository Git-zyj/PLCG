...
void kernel_4222111315_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[0] = C[i_0] * C[i_0] * A[0] - A[0] + A[0] + 1;
    }
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
            B[i_2][i_1+1] = A[0] + B[i_2+1][i_1+1] + 6;
        }
    }
#pragma endscop
...
kernel_4222111315_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
