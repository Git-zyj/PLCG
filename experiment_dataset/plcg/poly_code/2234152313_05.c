...
void kernel_2234152313_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            A[i_1+1] = C[0] + D[i_1][i_0-1] + D[i_1-2][i_0] + D[i_1][i_0] * C[i_0+1] + A[i_1+2] * A[i_1] - A[i_1+2] + A[i_1+2] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        B[i_2] = C[i_2+2] - D[i_2][i_2] - C[i_2] + C[i_2] + A[i_2+2] + B[i_2-1] + 3;
    }
#pragma endscop
...
kernel_2234152313_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
