...
void kernel_3412111115_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[0][i_1] = E[i_1][i_0] - A[0][i_1-1] - 5;
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[0][i_1] = F[i_1-1][i_2][i_1+1] - F[i_0+1][i_1][i_2+1] - 1;
            }
        }
        for (int i_3 = max(1, i_0); i_3 < PB_M+1; i_3++) {
            for (int i_4 = 1; i_4 < -i_0+PB_L-1; i_4++) {
                C[i_3-1][i_4+i_0][i_4-1] = C[i_3-1][i_4+i_0+1][i_4] + 6;
            }
        }
        A[0][i_0] = G[i_0+1] * 5;
    }
...
kernel_3412111115_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
