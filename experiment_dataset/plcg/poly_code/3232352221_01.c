...
void kernel_3232352221_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_L, i_1); i_2++) {
                A[i_1][i_1][i_0] = C[i_1+2][i_2] - D[i_1][i_2+i_0][i_0][1] - E[i_1][i_2+i_0-2][i_1-1] - A[i_1+2][i_1+2][i_0] + A[i_1][i_1][i_0+1] * 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_N; i_4++) {
            B[i_3] = C[i_3-2][1] * C[i_3][i_4] - F[i_4][i_3+2] + F[i_4+i_3][0] * C[i_4][i_3] + G[i_4+2][i_3] - H[i_4] * B[i_3-1] * B[i_3-2] + 4;
        }
    }
#pragma endscop
...
kernel_3232352221_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
