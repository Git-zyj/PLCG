...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4414351325_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_P; i_2++) {
                for (int i_3 = max(2, i_1); i_3 < -i_0+PB_P; i_3++) {
                    A[i_1][i_3][i_2+i_0][i_3+1] = E[i_1][i_0][i_3+i_2+1][i_0] * F[i_3][i_2][i_1+1] - E[i_3][i_2][i_0][i_1-1] - G[i_3][i_2+1][i_0-1][i_1] + G[i_3+1][i_3][i_2+i_0][i_1-1] * A[i_1][i_3-2][i_2+i_0][i_3-1] - A[i_2][i_1-1][i_3+i_0][i_1] + 4;
                }
            }
        }
        for (int i_4 = 0; i_4 < min(PB_Q-1, i_0); i_4++) {
            for (int i_5 = 3; i_5 < -i_0+PB_P; i_5++) {
                for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
                    B[i_0][i_5-1][i_4][i_0] = H[i_4][i_5+1][i_4+1][i_0] - I[i_6][i_4][i_0] * E[i_5][i_6+1][i_0-1][i_6] + A[i_4][i_6][i_5+i_0][i_6+1] + B[i_0][i_5-3][i_4][i_0] + 3;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_P; i_7++) {
        for (int i_8 = 1; i_8 < min(i_7, PB_L-2); i_8++) {
            C[i_8-1][i_8] = J[i_8-1][i_7] - K[i_8][i_7] - K[i_7][i_8] + J[0][i_7] * L[i_8] - C[i_8+1][i_8+2] * 5;
            for (int i_9 = 1; i_9 < min(PB_L-2, -i_7+PB_M+1); i_9++) {
                C[i_9+1][i_9+2] = M[i_9][i_7-1][i_9-1] - J[i_8+1][i_9+1] + I[i_9][i_7][i_8] + F[i_9][i_7][i_8] + N[i_9][i_8] + M[i_8][i_9+i_7-1][i_9] + F[i_7][1][i_9] * C[i_9-1][i_9] + C[i_9][i_9+1] * 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,yn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4414351325_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), xn,yn,POLYBENCH_ARRAY(N));
