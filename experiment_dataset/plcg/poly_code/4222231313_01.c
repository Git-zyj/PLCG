...
void kernel_4222231313_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_L); i_2++) {
            A[i_2] = D[i_2+i_1][0] + D[i_2][i_1] - E[i_2+1][i_1] + E[i_2][0] - D[i_2-1][i_1+1] * 5;
        }
    }
#pragma endscop
...
kernel_4222231313_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
