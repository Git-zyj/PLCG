...
void kernel_4314332323_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
        ...
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
                for (int i_3 = max(1, i_2); i_3 < PB_N-2; i_3++) {
                    B[i_3+2][i_3-1][i_2+1] = D[i_2][i_2+2][i_1][i_0] + 4;
                }
            }
        }
        for (int i_4 = i_0; i_4 < -i_0+PB_L; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_0+PB_L, PB_M-2); i_5++) {
                B[i_0+2][i_0-1][i_5+1] = E[i_4+i_0][i_5][0] - F[i_5+2][i_0][i_4] * F[i_5+2][i_0][i_4] * F[i_4-1][i_5+i_0][i_4] - B[i_0+2][i_0-1][i_5-1] * B[i_5+3][i_5][i_4-1] * B[i_0+1][i_0-2][i_5+2] + 2;
            }
            for (int i_6 = 0; i_6 < PB_P; i_6++) {
                for (int i_7 = 0; i_7 < -i_4+PB_Q; i_7++) {
                    B[i_4+3][i_4][i_0-1] = G[i_7][i_4-1][i_6][i_0] * F[i_7][i_4][i_6] * G[i_0][i_6][i_4][i_7+i_4] + 6;
                }
            }
        }
    }
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4314332323_02(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
