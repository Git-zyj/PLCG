...
void kernel_4226111315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
        for (int i_2 = min(PB_N-1, i_1) - 1; i_2 >= 0; i_2--) {
            A[i_2+1] = C[i_2+1] * A[i_1-1] * 5;
        }
    }
#pragma endscop
...
kernel_4226111315_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
