...
void kernel_3214111323_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_1+1][i_0] = C[i_1][i_0] * C[i_1+1][i_0+1] + A[i_1+1][i_0+1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_P+2; i_3++) {
            for (int i_4 = 1; i_4 < PB_P; i_4++) {
                B[i_3][i_2][i_4-1] = A[i_4+1][i_3-2] - A[i_3+3][i_4] - B[i_3][i_2][i_4+1] * B[i_3][i_2-1][i_4-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3214111323_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
