...
void kernel_2214132113_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-2; i_1++) {
            B[i_1][i_0] = C[i_0] - D[i_0][i_1] - D[i_1][i_1+i_0+2] - D[i_1][i_0-1] + 5;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_2][i_0] = D[i_2][i_0-1] + D[i_0+2][i_2-1] - B[i_2][i_0+1] - 5;
        }
    }
#pragma endscop
...
kernel_2214132113_08(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
