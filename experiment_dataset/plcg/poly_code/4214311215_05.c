...
void kernel_4214311215_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_4 = 0; i_4 < PB_N; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
            B[i_5+1] = C[i_5] - B[i_5] - B[i_5+2] + B[i_5+2] * 5;
        }
    }
#pragma endscop
...
kernel_4214311215_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
