...
void kernel_4224112225_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P; i_2++) {
                A[i_0][i_1][i_2+i_1] = A[i_0][i_1-2][i_2+i_1-1] * A[i_0][i_1-1][i_2+i_1-1] * 4;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            B[i_0+1][2] = C[i_3] + B[i_0+3][2] - B[i_3+2][2] * 2;
        }
    }
#pragma endscop
...
kernel_4224112225_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
