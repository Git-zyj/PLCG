...
void kernel_2426331325_07(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        D[i_0] = E[i_0+1] + F[i_0] + F[i_0-1] + G[i_0-1][i_0] * H[i_0+1] + 1;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L; i_2++) {
            D[i_2] = I[i_1+1][i_2] * D[i_1+1] + 5;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        D[i_3+2] = F[i_3+1] + D[i_3] + D[i_3+1] * 6;
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            D[i_4] = J[i_4][i_3] + E[i_4] - 3;
        }
    }
#pragma endscop
...
kernel_2426331325_07(xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
