...
void kernel_3222111325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q+1; i_1++) {
            A[i_0][i_1-1] = C[i_0] - A[i_0][i_1-2] * A[i_0-2][i_1-1] + A[i_0+2][i_1-1] + A[i_0-1][i_1-1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2+1] = C[i_2] + B[i_2-1] * B[i_2] - 1;
    }
#pragma endscop
...
kernel_3222111325_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
