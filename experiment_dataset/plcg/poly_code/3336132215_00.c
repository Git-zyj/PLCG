...
void kernel_3336132215_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        A[i_0+2] = D[i_0] + D[i_0] * D[i_0] - D[i_0] * D[i_0] + A[i_0+3] * 4;
        for (int i_1 = max(3, i_0); i_1 < -i_0+PB_L-1; i_1++) {
            B[i_0-2][i_1+i_0] = E[i_1][i_0] - E[i_1-1][i_0+1] * B[i_1-1][i_1+i_0+1] + B[i_1-3][i_1+i_0-1] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
                B[i_3-2][i_4+i_3] = E[i_3][i_3+2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3336132215_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
