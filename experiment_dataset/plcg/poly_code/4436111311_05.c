...
void kernel_4436111311_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_N-1; i_1++) {
            A[i_1][i_0] = A[i_1+1][i_0] - A[i_1][i_0+1] + A[i_1-1][i_0] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_P-2, PB_M-1); i_3++) {
            B[i_3][i_3+i_2] = B[i_3+1][i_3+i_2+1] + B[i_3][i_3+i_2+1] * B[i_3-1][i_3+i_2-1] + B[i_3+1][i_3+i_2+2] - 1;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        C[i_4+1] = E[i_4+1] - C[i_4] * 1;
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            D[i_5][i_5] = C[i_4+1] + D[i_5+1][i_5+1] + 5;
        }
    }
#pragma endscop
...
kernel_4436111311_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
