...
void kernel_4312332311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        A[i_0+2] = D[i_0] * D[i_0] + D[i_0+1] - E[i_0] + F[i_0+1] + A[i_0+1] + B[i_0] + 5;
        B[i_0] = A[i_0+2] + A[i_0+3] - B[i_0+1] - B[i_0-1] * 6;
    }
    for (int i_1 = 1; i_1 < PB_Q+1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            for (int i_3 = max(3, i_1); i_3 < PB_L; i_3++) {
                C[i_1+1][i_3-2][i_3] = G[i_3][i_1+1] * H[i_3][i_2][i_1-1] - C[i_1][i_2-1][i_2+1] - C[i_1][i_3-2][i_3] * C[i_1+1][i_3-3][i_3-1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4312332311_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
