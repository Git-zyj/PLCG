...
void kernel_4432151215_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] - E[i_0] + E[i_0] * E[i_0] * E[i_0-1] * 5;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_N-1; i_2++) {
            for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                B[i_3][i_1][i_2] = F[i_1+1][i_2] - G[i_3][i_3][i_1] - H[i_1][i_3+i_2][i_2][i_1] * G[i_3][i_1][i_2] * H[i_2+i_1+1][i_3][i_3][i_2] + H[i_1+1][i_2][i_3-1][i_2] + G[i_3][i_2-1][i_1] - B[i_3-1][i_1][i_2] * B[i_1-1][i_3+1][i_2-1] - B[i_3+1][i_1][i_2] * 1;
            }
        }
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 1; i_5 < PB_L; i_5++) {
                for (int i_6 = 1; i_6 < PB_P-1; i_6++) {
                    B[i_1][i_5][i_6+1] = H[i_5][i_4][i_1][i_5-1] + H[i_5][i_6][i_5][i_1] - H[i_6][i_5][i_4][i_6-1] - G[i_4][i_5][0] + 2;
                }
            }
        }
        for (int i_7 = 1; i_7 < PB_L; i_7++) {
            for (int i_8 = 0; i_8 < PB_P-1; i_8++) {
                for (int i_9 = 1; i_9 < -i_1+PB_L; i_9++) {
                    B[i_9-1][i_7][i_8+1] = G[i_7-1][i_8][i_9] * H[i_8][i_1][i_9][i_7] + H[i_1][i_8][i_9+i_1][i_9] + 5;
                }
            }
        }
    }
...
kernel_4432151215_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
