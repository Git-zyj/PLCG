...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,int wc,DATA_TYPE POLYBENCH_5D(C,xC,yC,zC,vC,wC,xc,yc,zc,vc,wc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4436331321_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,int wc,DATA_TYPE POLYBENCH_5D(C,xC,yC,zC,vC,wC,xc,yc,zc,vc,wc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_L-2; i_3++) {
                    A[i_1][i_2+i_0][i_3+1][0] = E[i_2][i_3+1][i_0][i_1-1] + E[i_2][i_0][i_1][i_1] + A[i_2][i_3+i_0+2][i_1-1][0] * 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_Q; i_4++) {
        for (int i_5 = i_4; i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 0; i_6 < min(PB_Q, -i_4+PB_M-1); i_6++) {
                for (int i_7 = 3; i_7 < i_6; i_7++) {
                    B[i_6+i_4][i_7+i_5][i_6][i_7-1] = F[i_7][i_6][i_4][i_5] * F[i_7][i_4][i_7][i_5+1] - B[i_6+i_4][i_7+i_5][i_4][i_7-3] + B[i_6+i_4+1][i_7+i_5+1][i_6][i_7-1] * 5;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_Q-3; i_8++) {
        for (int i_9 = max(i_8, 2); i_9 < PB_Q; i_9++) {
            for (int j_0 = 1; j_0 < PB_Q-1; j_0++) {
                for (int j_1 = 2; j_1 < PB_Q-2; j_1++) {
                    C[i_8][j_1][j_0][i_9][j_1] = F[i_9-1][j_0][i_8][j_0-1] + F[j_1][j_0][j_1-1][i_9] - G[j_1-1][i_8-1][j_0][i_9] - H[i_9][j_1][0] + C[i_8][j_1][j_0-1][i_9][j_1] + C[j_1-2][i_8][i_9-2][j_0][i_8] - C[i_8][j_1+2][j_0+1][i_9][j_1+2] + 3;
                }
            }
        }
        for (int j_2 = 2; j_2 < min(-i_8+PB_L, PB_Q-1); j_2++) {
            for (int j_3 = 2; j_3 < i_8; j_3++) {
                D[j_3][j_2][j_2+1] = I[j_2][i_8][j_3] + H[j_3-1][j_2+i_8][j_3+i_8] + J[j_2+1][i_8+1][j_3-1] * H[j_2][j_3][i_8] - D[j_2-2][j_3][j_3+1] * D[j_3-2][i_8+2][i_8+3] * D[j_3+1][j_2][j_2+1] + 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,wc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4436331321_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,wc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,wc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
