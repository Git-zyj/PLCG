...
void kernel_4224112223_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L-1; i_2++) {
                B[i_0-1][i_1][i_2] = C[i_2][i_2+i_0+1][i_1] - 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_P-1; i_4++) {
            for (int i_5 = i_3; i_5 < PB_P; i_5++) {
                B[i_3][i_5][i_4] = C[i_4][i_5-1][i_4+1] * B[i_3-2][i_5][i_4-1] - B[i_3][i_5-2][i_4-1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4224112223_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
