...
void kernel_4232112323_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, -i_1+PB_M-1); i_2++) {
                B[i_2+i_1+1][1] = C[2][i_0] * 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M-1; i_4++) {
            B[i_4+i_3][1] = D[i_3+1] + B[i_4+i_3+1][1] - B[i_4+i_3-1][1] - B[i_4+i_3+1][1] + 3;
        }
    }
#pragma endscop
...
kernel_4232112323_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
