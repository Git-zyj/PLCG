...
void kernel_3414312325_01(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 3; i_1 < PB_M+1; i_1++) {
            ...
            C[i_1-3] = C[i_1-3] - C[i_1-1] * C[i_1-2] + 6;
            C[i_1-1] = 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        D[i_3+1] = E[i_3] + D[i_3+2] - D[i_3-1] - 6;
    }
#pragma endscop
...
kernel_3414312325_01(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
