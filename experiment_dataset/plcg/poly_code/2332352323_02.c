...
void kernel_2332352323_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        A[i_0-2] = D[i_0] - E[i_0] * F[i_0] * E[i_0] + E[i_0] * D[i_0] + D[i_0-2] + A[i_0-3] * A[i_0] + A[i_0-1] + 5;
    }
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            B[i_2][i_1] = G[i_2][i_1+2] - H[i_1-2][i_2] + A[i_2-2] + 2;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_L; i_4++) {
            B[i_4][i_3] = D[i_3] * H[i_3][i_4+i_3] - H[i_4][i_4] - F[i_4] + B[i_3][i_4] + 1;
        }
    }
#pragma endscop
...
kernel_2332352323_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
