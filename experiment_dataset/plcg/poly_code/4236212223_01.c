...
void kernel_4236212223_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
        ...
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < min(PB_Q-2, i_0); i_3++) {
                    A[i_2][i_3][i_3+2] = C[i_3][i_2][i_1] + 2;
                }
            }
            B[i_0-2][i_0] = D[i_1][i_0+1] * B[i_0][i_0+2] - B[i_0-4][i_0-2] - B[i_0-1][i_0+1] * 3;
        }
    }
#pragma endscop
...
kernel_4236212223_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
