...
void kernel_2222131111_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        ...
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_2] = C[i_2-1][i_0] - D[i_0-1] + C[i_2+1][i_0] + 1;
        }
    }
#pragma endscop
...
kernel_2222131111_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
