...
void kernel_3214252325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-4; i_1++) {
            A[i_1+2] = C[i_1][i_0] * D[i_0+1][i_1] + D[i_1][i_0] + C[i_1-2][i_0] + A[i_1+4] - A[i_1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 1; i_3 < min(min(i_2, PB_L-2), -i_2+PB_P); i_3++) {
            A[i_3+1] = C[i_3-1][i_2-1] - E[i_3+i_2] - C[i_2-1][i_3+i_2] - D[0][i_3] * C[i_3+i_2][i_3] - A[i_3+2] - 2;
        }
    }
#pragma endscop
...
kernel_3214252325_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
