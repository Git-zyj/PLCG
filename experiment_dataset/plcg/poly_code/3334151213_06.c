...
void kernel_3334151213_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            A[i_0-1][i_1-1] = D[i_1+1][i_0] * E[i_1-1] - D[i_0-1][i_0+1] * D[i_0][i_1] * D[i_0][i_1] - D[i_1][i_0+1] * A[i_0-1][i_1-2] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_L-1, -i_2+PB_M-1); i_3++) {
            B[i_3+1][i_3+i_2] = D[i_2][i_3+1] * E[0] + D[i_3][i_2] + D[i_2+1][i_3] * D[i_3][i_2] * B[i_3+2][i_3+i_2+1] + B[i_3+1][i_3+i_2-1] + 1;
        }
        for (int i_4 = i_2; i_4 < PB_L; i_4++) {
            C[i_2][i_4] = D[i_4][i_4-1] + D[i_4-1][i_2] + C[i_2-1][i_4-1] - 4;
        }
    }
#pragma endscop
...
kernel_3334151213_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
