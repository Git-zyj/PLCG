...
void kernel_4316212321_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                A[i_2+i_1][i_0][i_0] = D[0][i_1] + A[i_2+i_1-1][i_0][i_0] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_Q; i_4++) {
            B[i_3-1][i_4] = D[i_4][i_3] * B[i_3][i_4] + B[i_3-1][i_4-2] - 6;
        }
        for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
            for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
                for (int i_7 = 2; i_7 < PB_P; i_7++) {
                    C[i_5][0][i_6] = C[i_5][0][i_7-2] + C[i_5][0][i_6+1] - C[i_6][0][i_7-2] * C[i_5+2][0][i_7] * 5;
                }
            }
        }
    }
...
kernel_4316212321_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
