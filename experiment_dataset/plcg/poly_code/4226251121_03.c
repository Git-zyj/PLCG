...
void kernel_4226251121_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
        ...
        for (int i_1 = 1; i_1 < min(PB_N-1, i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_0+PB_Q, PB_N-1); i_3++) {
                    A[i_3+1][i_1][i_2][i_1] = C[i_0-1][i_2][i_1][i_1] + D[i_1+1][i_2][i_3+i_0][i_2] * E[i_3][i_1+1][i_3-1] + D[i_0-1][i_3][i_2][i_1-1] + A[i_1+1][i_2-1][i_3][i_2-1] + 3;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_N; i_4++) {
        B[i_4] = F[i_4] * G[i_4+1] + H[i_4][1] * F[0] * G[i_4] * 5;
    }
#pragma endscop
...
kernel_4226251121_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
