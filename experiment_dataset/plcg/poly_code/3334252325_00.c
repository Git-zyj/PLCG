...
void kernel_3334252325_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0][i_0-1] = D[i_0] * E[i_0] + D[i_0+2] * E[i_0] * A[i_0-1][i_0-2] - A[i_0+2][i_0+1] - 2;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < -i_1+PB_L-2; i_2++) {
            for (int i_3 = 2; i_3 < PB_M; i_3++) {
                A[i_3][i_3-1] = F[i_3][i_2+i_1+2] + G[i_3][i_2] * H[i_2-2][i_3-2][0] + 5;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 3; i_5 < -i_4+PB_M; i_5++) {
            A[i_5-2][i_5-3] = F[i_4][i_5] + G[i_4][i_5] * F[i_5+1][i_4-2] + G[i_5+i_4][0] * F[i_5][i_4] * F[i_5][i_4] - G[i_5+i_4][i_5-2] + A[i_4+2][i_4+1] + 1;
        }
    }
#pragma endscop
...
kernel_3334252325_00(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
