...
void kernel_3436251313_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = E[i_0-1] - F[i_0-1][i_0+1] - G[i_0+1] + G[i_0] * G[i_0+1] * A[i_0+2] * A[i_0] - 3;
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_0+1] = F[i_1][i_0] * F[i_1-1][i_0+1] - G[i_1] + A[i_0+1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
            for (int i_4 = i_3; i_4 < -i_2+PB_M-1; i_4++) {
                C[i_4+i_2][i_3-1][i_2] = H[i_3-1][i_4][i_2+1] - H[i_3-1][i_4][i_2] - H[i_4+1][i_2][i_2-1] + C[i_4+i_2+1][i_3-1][i_2] * C[i_4+i_2-1][i_3-1][i_2-1] - 1;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
        A[i_5+2] = E[i_5] - E[i_5] * E[i_5] - G[i_5] + G[i_5] + E[0] - G[i_5] + 2;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3436251313_06(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
