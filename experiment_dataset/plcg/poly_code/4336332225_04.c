...
void kernel_4336332225_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_L-2; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_M+1, PB_L-2); i_2++) {
                for (int i_3 = 1; i_3 < min(-i_0+PB_M, PB_Q-1); i_3++) {
                    A[i_3+i_0][i_2+2][i_1-1][1] = D[i_3][i_2-2][i_0-1][i_1] * E[i_0][i_3][i_1][i_0-1] * E[i_2-2][i_1-2][0][i_3+1] * A[i_2+i_0-1][i_1+2][i_3-1][1] * A[i_3+i_0-1][i_2+2][i_1-2][1] * 1;
                }
            }
        }
        for (int i_4 = 1; i_4 < i_0; i_4++) {
            for (int i_5 = 0; i_5 < min(-i_0+PB_M, PB_Q-1); i_5++) {
                for (int i_6 = 2; i_6 < i_4; i_6++) {
                    A[i_5+i_0][i_4+2][i_6-2][1] = F[i_5][i_4][i_6] + G[i_0][i_4-1][i_6-2][i_5+1] * A[i_5+i_0][i_6+2][i_4-1][1] + 1;
                }
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_Q; i_7++) {
        for (int i_8 = 0; i_8 < min(min(PB_M-3, -i_7+PB_M), i_7); i_8++) {
            for (int i_9 = 0; i_9 < i_8; i_9++) {
                C[i_8+1][i_9][1] = H[i_8+i_7][i_9][0] + I[i_7-2][i_8] * C[i_8+3][i_9][1] - 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4336332225_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
