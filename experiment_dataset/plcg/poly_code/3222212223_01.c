...
void kernel_3222212223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        ...
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                B[i_2][i_0] = B[i_1+1][i_2-1] * B[i_2-2][i_1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3222212223_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
