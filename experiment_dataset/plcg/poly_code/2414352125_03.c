...
void kernel_2414352125_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        B[i_0] = E[i_0+2] * E[0] + F[i_0] + F[i_0] - 4;
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            B[i_0] = G[i_1-2][i_0+1] * H[i_1+i_0][1] * G[i_0-1][i_1] + B[i_1-1] - B[i_1+2] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        C[i_2] = I[i_2] * I[i_2] - F[i_2] + E[i_2] * H[i_2][1] * F[i_2+1] - F[i_2] + 5;
        D[i_2-1] = E[i_2-1] - E[i_2] - F[i_2] * 6;
    }
#pragma endscop
...
kernel_2414352125_03(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
