...
#endif
static void init_array(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,int vm,int wm,DATA_TYPE POLYBENCH_5D(M,xM,yM,zM,vM,wM,xm,ym,zm,vm,wm),int seed)
{
srand(seed);
...
void kernel_4422352311_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,int vm,int wm,DATA_TYPE POLYBENCH_5D(M,xM,yM,zM,vM,wM,xm,ym,zm,vm,wm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1] = E[i_1][i_0+2] + F[i_1][i_0+1] * E[i_1+i_0][i_0] - G[i_0][2] + E[i_1-1][i_0] - A[i_0+1][i_1] * A[i_0-1][i_1-1] * 2;
        }
        for (int i_2 = i_0; i_2 < min(PB_M, -i_0+PB_P+2); i_2++) {
            for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
                B[i_0+2][i_3][i_0-1] = F[i_2][i_0] * F[i_3-2][i_2] + H[i_2][i_3][i_0] * H[i_3][i_0][i_2] - I[i_3][i_2+i_0-2][i_0] + F[i_3][i_0+1] * B[i_0+1][i_3+1][i_0-2] + B[i_3+2][i_0-1][i_3-1] * 2;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_M+1; i_5++) {
            for (int i_6 = 0; i_6 < -i_4+PB_Q+1; i_6++) {
                for (int i_7 = 1; i_7 < -i_4+PB_Q-2; i_7++) {
                    B[i_7+2][i_6][i_7-1] = J[i_7][i_6+i_4-1][i_5-1][i_5] + J[i_5][i_7+i_4+2][i_7][i_6] + 4;
                }
            }
            for (int i_8 = 2; i_8 < PB_M-1; i_8++) {
                for (int i_9 = 2; i_9 < min(-i_8+PB_L+1, -i_5+PB_Q+1); i_9++) {
                    D[i_8][i_4-1][i_9+i_5-1][i_9] = K[i_9][i_4-1][i_8][i_5] - L[i_8][i_9+1][i_4] * M[i_8][i_5-1][i_9][i_4][i_8-1] + J[i_9+i_8-1][i_4+2][i_5][i_5-1] - H[i_9-2][i_5][0] * D[i_8-1][i_4][i_9+i_5-1][i_9] + D[i_8+1][i_4][i_9+i_5-2][i_9-1] + D[i_4][i_5][i_9+i_8-2][i_9] * D[i_4+1][i_8-2][i_9+i_5-1][i_9] * 6;
                }
            }
        }
    }
...
kernel_4422352311_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,wm,POLYBENCH_ARRAY(M));
