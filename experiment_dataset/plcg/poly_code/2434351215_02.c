...
void kernel_2434351215_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        C[i_0+1] = E[i_0+1] - F[i_0] + F[i_0] - E[i_0] * G[i_0-1][i_0+1] * H[i_0] + E[i_0] + C[i_0] + 4;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        B[i_1] = H[i_1] - B[i_1+1] * 6;
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        C[i_2] = H[i_2] * H[i_2] - H[i_2] - F[i_2+1] * H[i_2-1] * F[i_2+1] * C[i_2+1] * D[i_2] * 5;
        D[i_2] = F[i_2-1] - H[i_2-1] + F[i_2+1] + E[i_2-1] - D[i_2-1] * 6;
    }
#pragma endscop
...
kernel_2434351215_02(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
