...
void kernel_4214312113_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
        ...
        for (int i_1 = 1; i_1 < min(PB_Q, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                for (int i_3 = i_2; i_3 < PB_Q; i_3++) {
                    A[i_2+1][i_2][i_1-1][0] = A[i_0+2][i_0+1][i_1-1][0] - 4;
                }
            }
            B[i_1+i_0-1] = C[i_1][i_1] + B[i_1+i_0-2] * 6;
        }
    }
#pragma endscop
...
kernel_4214312113_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
