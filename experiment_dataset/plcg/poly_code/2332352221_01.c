...
void kernel_2332352221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-4; i_0++) {
        A[i_0+2] = D[i_0+2] - E[i_0] - E[i_0-1] + A[i_0+4] + 3;
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            B[i_0][i_1] = F[i_0+2][i_1] + G[i_1][i_0] * G[i_1][i_0] * H[i_0][i_1+2] + H[i_1][i_0] + G[i_1+1][i_0] * F[i_1][i_0] - F[i_0][i_0+2] * A[i_1+2] + 2;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        C[i_2] = E[i_2+1] * I[i_2] - E[i_2] + D[i_2] - C[i_2-2] + C[i_2+1] + 2;
    }
#pragma endscop
...
kernel_2332352221_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
