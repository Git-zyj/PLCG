...
void kernel_3316112321_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        ...
        for (int i_3 = 0; i_3 < min(PB_L, i_1); i_3++) {
            C[i_3][i_1] = D[i_3][i_1-1] + C[i_3][i_1-1] * C[i_3][i_1+1] * C[i_3][i_1+2] * C[i_3+1][i_1] - C[i_3+2][i_1] + 3;
        }
    }
#pragma endscop
...
kernel_3316112321_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
