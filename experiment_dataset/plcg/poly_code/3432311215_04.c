...
void kernel_3432311215_04(int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = i_0; i_2 < min(PB_Q+1, -i_0+PB_P); i_2++) {
            D[i_2-1][i_0][i_2-1] = E[i_0-1][i_2+i_0][i_2-1] - 5;
        }
    ...
    for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 0; i_6 < PB_M; i_6++) {
            for (int i_7 = 2; i_7 < PB_M; i_7++) {
                D[i_7-1][i_5][i_7-1] = D[i_7][i_5][i_7] * D[i_7-2][i_5+1][i_7-2] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3432311215_04(xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
