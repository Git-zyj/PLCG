...
void kernel_3314312325_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            ...
            B[i_1] = D[i_0][i_1] * B[i_1+2] + C[i_1+2] * 1;
        }
        C[i_0+2] = B[i_0+2] * B[i_0] + B[i_0+1] * B[i_0-2] * 3;
    }
#pragma endscop
...
kernel_3314312325_08(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
