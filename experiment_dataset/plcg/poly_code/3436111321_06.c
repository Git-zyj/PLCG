...
void kernel_3436111321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0+1] * B[i_0] * B[i_0+2] - B[i_0+1] + B[i_0-2] - 5;
        B[i_0] = D[i_0] - 6;
    }
    for (int i_1 = 3; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_L-1, i_1); i_2++) {
            for (int i_3 = 2; i_3 < -i_1+PB_L; i_3++) {
                C[i_2][i_3+i_1-1][i_2] = F[i_2][i_3-1][i_1] * C[i_2][i_3+i_1][i_2] * C[i_2][i_3+i_1-3][i_2] + C[i_3-2][i_2+i_1+1][i_3-2] * 5;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        D[i_4] = A[i_4-2] * D[i_4-1] * D[i_4+2] - D[i_4+1] + 1;
    }
#pragma endscop
...
kernel_3436111321_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
