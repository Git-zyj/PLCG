...
void kernel_4432111325_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
    ...
    for (int i_1 = 2; i_1 < PB_P-4; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            for (int i_3 = i_1; i_3 < -i_2+PB_P; i_3++) {
                C[i_3][i_3+i_2] = E[i_3-1][i_1+1][i_2] * 6;
            }
        }
        for (int i_4 = 2; i_4 < -i_1+PB_P-4; i_4++) {
            C[i_4][i_4+i_1] = F[i_4][i_1+1] * C[i_4+2][i_4+i_1+3] * C[i_4-2][i_4+i_1-2] * C[i_4+2][i_4+i_1+4] - C[i_1+1][i_4+i_1+1] * 1;
        }
    }
        ...
        for (int i_6 = i_5; i_6 < PB_Q; i_6++) {
            for (int i_7 = 1; i_7 < PB_P-2; i_7++) {
                for (int i_8 = i_7; i_8 < PB_Q; i_8++) {
                    D[i_5-1][i_8][i_7+1][0] = D[i_5-1][i_8][i_7-1][0] - D[i_5-1][i_8][i_7+2][0] + 2;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4432111325_02(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F)));
