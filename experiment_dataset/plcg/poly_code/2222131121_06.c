...
void kernel_2222131121_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_N-1; i_1++) {
            B[i_0-1][i_1] = C[i_0+1][i_1-1] * C[i_0][i_1+1] + C[i_1+i_0+1][0] - C[i_0+1][i_1-1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q-2; i_2++) {
        for (int i_3 = 3; i_3 < min(i_2, PB_L+1); i_3++) {
            B[i_3-1][i_2] = D[i_3] * E[i_2][i_3][i_3-1] - B[i_3-3][i_2+2] + 5;
        }
    }
#pragma endscop
...
kernel_2222131121_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
