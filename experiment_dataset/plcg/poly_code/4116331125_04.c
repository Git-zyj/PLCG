...
void kernel_4116331125_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_P, i_0); i_1++) {
            for (int i_2 = max(2, i_1); i_2 < PB_L-1; i_2++) {
                for (int i_3 = max(max(max(1, -i_1+3), -i_2+1), -i_0+1); i_3 < min(min(-i_1+PB_Q+3, -i_2+PB_Q+1), -i_0+PB_L+1); i_3++) {
                    A[i_3][i_1][i_3+i_2-1] = B[0][i_3][i_2][i_1] * C[i_2+1][i_3-1][i_0][i_2] * B[i_1][i_3+i_0-1][i_2][1] + A[i_3][i_2-2][i_3+i_1-3] - 4;
                }
            }
        }
    }
...
kernel_4116331125_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
