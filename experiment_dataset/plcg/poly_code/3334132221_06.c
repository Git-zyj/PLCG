...
void kernel_3334132221_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        A[i_0+2][i_0] = A[i_0+3][i_0+1] - 6;
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q; i_2++) {
                B[i_2][i_0] = D[i_1-2][i_0-2][i_2] - D[i_2+2][i_0+2][i_1] + E[i_2][i_1] + D[i_2][i_1][0] + B[i_2][i_1+1] - B[i_2-1][i_1-2] + 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < PB_N-1; i_4++) {
            C[i_3][i_4] = F[i_3] + E[i_3][i_4+2] + E[i_4][i_3-1] * E[i_4+1][i_3] * C[i_3+1][i_4] + 1;
        }
    }
#pragma endscop
...
kernel_3334132221_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
