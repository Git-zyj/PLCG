...
void kernel_3322132313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0-1] = D[i_0] - E[i_0][i_0] - D[i_0-1] * A[i_0] * 6;
    }
    for (int i_1 = 3; i_1 < PB_M+1; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_1-2][i_2] = E[i_2][i_1] * A[i_2-1] - B[i_1-1][i_2] * B[i_1-2][i_2-1] + B[i_1-3][i_2] - 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_M+2; i_4++) {
            A[i_4-1] = E[i_3][i_4] + E[i_3][i_4+2] - E[i_4+i_3-2][i_4] * E[i_3][0] * E[i_3][i_4+2] - 1;
        }
    }
#pragma endscop
...
kernel_3322132313_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
