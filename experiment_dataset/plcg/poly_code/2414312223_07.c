...
void kernel_2414312223_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-4; i_3++) {
            B[i_3+2] = B[i_3+4] * B[i_3+3] + 3;
        }
        for (int i_4 = max(3, i_2); i_4 < PB_N; i_4++) {
            C[i_4][i_4] = D[i_4-3] * 1;
        }
        D[i_2-1] = E[i_2-1] + C[i_2+2][i_2+2] * C[i_2][i_2] + 2;
    }
#pragma endscop
...
kernel_2414312223_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
