...
void kernel_3226112323_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L-1; i_2++) {
                B[i_0-2][i_1][i_2+i_0+1] = C[2][i_0][i_1] - 6;
            }
        }
        for (int i_3 = i_0; i_3 < min(PB_Q-2, -i_0+PB_L); i_3++) {
            for (int i_4 = 1; i_4 < PB_P+1; i_4++) {
                B[i_0-1][i_4][i_3+i_0] = C[i_0][i_4-1][i_3+2] - B[i_0-1][i_4+2][i_3+i_0-2] + B[i_0-1][i_4][i_3+i_0-2] + B[i_0-1][i_4+2][i_3+i_0] - 4;
            }
        }
    }
#pragma endscop
...
kernel_3226112323_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
