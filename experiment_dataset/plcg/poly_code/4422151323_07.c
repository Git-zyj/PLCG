...
void kernel_4422151323_07(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                C[i_2+1][i_1-1][i_2-1] = E[i_0][i_2][i_1+1] * E[i_2][i_2][i_1+1] + E[i_1][i_0][i_2-1] + E[i_1][i_2][i_0] * 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < -i_3+PB_Q-1; i_4++) {
            for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
                for (int i_6 = 1; i_6 < min(-i_5+PB_Q-1, PB_L-1); i_6++) {
                    C[i_5+1][i_6+2][i_5-1] = F[i_5-1][i_6+i_4][i_3-1][0] * F[i_4][i_6-1][i_3][i_5] + F[i_4][i_5][i_3][i_6] + F[i_3+1][i_6][i_4][i_5] - G[i_6+1][i_4][i_6+i_5+1][i_3][i_4+i_3+1] - E[i_6][i_5][i_4-1] * 4;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
        for (int i_8 = 2; i_8 < PB_L-2; i_8++) {
            for (int i_9 = 1; i_9 < PB_P-1; i_9++) {
                for (int j_0 = 0; j_0 < min(min(PB_L-3, i_8), -i_7+PB_Q-1); j_0++) {
                    C[i_8+1][i_9][i_8-1] = E[j_0][i_8][i_9] - F[i_8][i_7-1][i_7][j_0] - F[i_8][i_9-1][j_0][i_7] + F[i_9][i_7][i_8][0] - F[i_8][i_9+1][j_0+i_7+1][i_9] * C[i_8+2][i_9][i_8] - C[j_0+3][i_9][j_0+1] - C[i_8][j_0+1][i_8-2] + 1;
                }
            }
        }
        D[i_7+1] = H[i_7+1] - H[i_7] + H[0] * H[i_7-1] + D[i_7-1] - D[i_7+2] + 1;
    }
#pragma endscop
...
init_array(xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422151323_07(xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
