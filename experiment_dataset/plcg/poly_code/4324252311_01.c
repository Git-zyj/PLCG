...
void kernel_4324252311_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-3; i_2++) {
                for (int i_3 = 1; i_3 < min(min(-i_2+PB_L, -i_1+PB_L+2), PB_M-2); i_3++) {
                    A[i_0][i_1][i_3+i_2-1][i_2+2] = D[i_0][i_2][i_3-1][i_1] * E[i_2][i_1][i_0][i_3] + D[i_3+1][i_0][i_2][i_1+1] + E[i_3][i_2][i_1][i_0] - D[i_3+2][i_1][i_0][i_2] + A[i_0][i_2][i_3+i_1-2][i_3+1] * A[i_0][i_1][i_3+i_2][i_2+3] + A[i_0][i_1+1][i_3+i_2-2][i_2+1] - A[i_0][i_1-1][i_3+i_2][i_2+3] + 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_N-1; i_4++) {
        B[i_4] = F[i_4] + G[i_4][0] * H[i_4] * F[i_4] * B[i_4-1] * B[i_4+1] + 6;
        for (int i_5 = 1; i_5 < -i_4+PB_M; i_5++) {
            for (int i_6 = 2; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 0; i_7 < min(-i_6+PB_P, PB_M-2); i_7++) {
                    C[i_5][i_7][i_4][i_6-2] = D[i_4][i_7][i_5][0] * I[i_6+1][i_7][i_4] * D[i_7+2][i_5][i_4][i_6] + D[i_5+i_4][i_7+i_6][i_5][i_6] * E[i_5][i_6][i_7][i_4] * E[i_5][i_4-1][i_7+2][i_4-2] + A[i_4+1][i_5][i_7+i_6-2][i_6+1] - C[i_5-1][i_7][i_6][i_4-2] * 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4324252311_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
