...
void kernel_2426231325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_L-2); i_1++) {
            A[i_1][i_0] = E[i_1][i_0] * F[i_0] * E[i_1][i_1+i_0] + A[i_0][i_1+2] - D[i_0] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_N-2, i_2); i_3++) {
            B[i_3][i_2] = E[i_3-1][i_3] * B[i_3][i_2+1] * D[i_3] * D[i_3+2] * 2;
        }
        for (int i_4 = 1; i_4 < i_2; i_4++) {
            D[i_4] = G[i_4+1][i_2-1] - G[i_2-1][i_4-1] - H[i_2][i_4][i_2+1] - A[i_4][i_2] + D[i_4] * D[i_2] + 4;
        }
        D[i_2] = I[i_2+1] + I[i_2+1] - F[i_2-1] - F[i_2] - I[1] * D[i_2+1] - 4;
    }
#pragma endscop
...
kernel_2426231325_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
