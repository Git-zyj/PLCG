...
void kernel_2226151223_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            B[i_0+1][i_1] = C[i_1][i_0] + C[i_0][i_1] * C[i_1][i_0] + B[i_0-1][i_1] - 6;
        }
        for (int i_2 = max(2, i_0); i_2 < -i_0+PB_M; i_2++) {
            B[i_0+1][i_2] = C[i_2-1][i_0] + C[i_2][i_0] + C[i_2][i_0-1] + C[i_2][i_0+1] - C[i_2-1][i_0] - C[i_2+i_0][0] + B[i_0+2][i_2-2] + 2;
        }
    }
#pragma endscop
...
kernel_2226151223_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
