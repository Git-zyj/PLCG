...
void kernel_4322351323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0+1][0] - E[i_0] * E[i_0] * E[i_0+1] - A[i_0+2] * A[i_0-1] * 6;
    }
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_L; i_2++) {
            for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                for (int i_4 = 2; i_4 < -i_3+PB_L; i_4++) {
                    B[i_1][i_4][i_3][i_2-1] = F[i_2][i_4][i_1+1][i_2-1] - G[i_1+1][i_4][i_2][i_3] - H[i_2-1][i_4+i_3][i_1+1][0] + I[i_3][i_1-1][i_2-1] - H[i_4][i_3][i_2+i_1][i_3] * G[i_3][i_2+i_1][i_4][1] + B[i_1][i_4][i_3-1][i_2] - B[i_4][i_2][i_1-1][i_3-1] * B[i_3+1][i_4-2][i_1][i_2-1] * 2;
                }
            }
        }
        for (int i_5 = 0; i_5 < -i_1+PB_L; i_5++) {
            A[i_5+2] = D[i_5+i_1][0] - J[i_1][i_5+i_1][0] * K[i_5][i_1] + L[i_1] + 5;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4322351323_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L)));
