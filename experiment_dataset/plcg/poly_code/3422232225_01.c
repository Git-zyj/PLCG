...
void kernel_3422232225_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_M, PB_P-2), -i_0+PB_P); i_2++) {
                A[i_1-1][i_1][i_2] = E[i_2+2][i_1][i_0+1] + E[i_1][i_2+i_0][0] * E[i_1][i_2][0] - E[i_0][i_2][i_1] * E[i_2+i_1][i_0][i_1] * A[i_0-1][i_0][i_1-2] + A[i_1-1][i_1][i_2+2] - A[i_1-1][i_1][i_0+1] * 3;
            }
        }
        for (int i_3 = i_0; i_3 < PB_P; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                A[i_4-1][i_4][i_3] = F[i_3][i_4] * 5;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_N-2; i_5++) {
        for (int i_6 = 2; i_6 < PB_L; i_6++) {
            A[i_6-1][i_6][i_5] = G[i_6-2][i_5+2][i_6+2] * 6;
        }
        D[i_5] = H[i_5] * H[0] * I[i_5] - D[i_5+2] * 1;
    }
#pragma endscop
...
kernel_3422232225_01(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
