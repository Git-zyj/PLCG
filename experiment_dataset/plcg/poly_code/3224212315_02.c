...
void kernel_3224212315_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                A[i_2][i_1][i_0] = A[i_2+1][i_1+1][i_0] + A[i_2-1][i_1+1][i_0+1] + A[i_1][i_2][i_0-1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_Q; i_5++) {
                B[i_3][i_5][i_3] = C[i_4][i_5-1] + D[i_3+2][i_5-1][i_5+i_4] + B[i_3][i_5-1][i_3] + B[i_3][i_5+1][i_3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3224212315_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
