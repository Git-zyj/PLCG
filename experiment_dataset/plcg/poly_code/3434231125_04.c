...
void kernel_3434231125_04(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        C[i_1] = E[i_1-1] * E[i_1] + F[i_1] - F[i_1] + E[i_1] - 1;
    }
    for (int i_2 = 3; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < PB_L-1; i_3++) {
            C[i_3-1] = G[i_3][i_2] * G[i_3][i_2] + G[i_3+1][i_2-1] * 1;
            for (int i_4 = 1; i_4 < -i_3+PB_M-1; i_4++) {
                D[i_4+1][i_3+1][i_2-1] = H[i_4][i_2-1][i_4-1] - H[i_4+i_3+1][i_2-1][1] - I[i_3+1][i_2][i_2] * D[i_4][i_3+1][i_2-3] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3434231125_04(xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
