...
void kernel_4434331215_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < PB_L+1; i_2++) {
                B[i_2-1][i_1] = E[i_2][i_0][i_1+i_0] - 6;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L; i_3++) {
            B[i_3][i_0] = F[i_3][i_0-1] * G[i_0][0] + B[i_3][i_0+1] - B[i_3-1][i_0-1] * 5;
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            B[i_4-1][i_5+1] = F[i_5][i_4] - H[i_5+1] - G[i_4][i_5] - I[i_4-1][i_5] * F[i_5][i_5-1] + 5;
        }
        for (int i_6 = i_4; i_6 < -i_4+PB_L-1; i_6++) {
            for (int i_7 = 1; i_7 < PB_L; i_7++) {
                B[i_7][i_6-1] = I[i_7][i_6+1] + J[i_7+1][i_6+i_4+1][i_4+1] * E[i_7-1][i_4][i_4+1] + 1;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4434331215_09(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
