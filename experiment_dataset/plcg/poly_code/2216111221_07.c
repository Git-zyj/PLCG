...
void kernel_2216111221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1-1] = A[i_1+1] + A[i_1] + 1;
        }
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_0][i_2] = C[i_2-1][i_0] - C[i_2][i_0+1] * A[i_2-1] - 2;
        }
    }
#pragma endscop
...
kernel_2216111221_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
