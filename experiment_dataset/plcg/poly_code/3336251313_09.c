...
void kernel_3336251313_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = D[i_0+1] * D[i_0+1] - E[i_0] + A[i_0-1] - 3;
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            A[i_1] = E[i_0+1] - F[i_1][i_0] * G[i_1][i_0] + G[i_0-1][i_1] + G[i_1+i_0][i_1+1] + F[i_1-1][i_0] * F[i_0][i_0+1] - A[i_0+1] - A[i_1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_L+1, PB_P); i_3++) {
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                A[i_4+1] = H[i_3][i_4][i_2] * H[i_4][i_2][0] + G[i_3][i_3+i_2-1] * I[i_4][i_2][i_3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3336251313_09(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
