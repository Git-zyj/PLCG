...
void kernel_2216211321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = A[i_0+1] - B[i_0+1][i_0] - 4;
    }
    for (int i_1 = 1; i_1 < PB_L-3; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            B[i_1+1][i_1] = C[i_1][i_2] + A[i_1-1] + B[i_1][i_1-1] + B[i_1+3][i_1+2] + 6;
        }
    }
#pragma endscop
...
kernel_2216211321_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
