...
void kernel_3412252225_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        B[i_0] = E[i_0+1] + E[i_0-2] * F[i_0] - E[i_0] + B[i_0+2] - 5;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        B[i_1] = F[i_1] * F[i_1] + 3;
        B[i_1+2] = G[i_1+2][i_1] + F[i_1-2] * F[i_1] * E[0] - F[i_1+2] * E[i_1] - F[i_1] - F[i_1-2] + E[i_1-2] + B[i_1] * 1;
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
                D[i_2-2][i_1-1][i_3] = H[i_3-2][i_2][i_1] + H[i_1-2][i_3-1][i_2] * I[i_3+2][i_2][i_1] - D[i_1-2][i_2-1][i_3+1] * D[i_2-2][i_1][i_3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3412252225_03(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
