...
void kernel_2222312315_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = PB_L - 1; i_1 >= 3; i_1--) {
            A[i_0][i_1][i_1-2] = C[0] + A[i_0+1][i_1-1][i_1-3] - A[i_1+1][i_0][i_0-2] - A[i_0][i_1-1][i_1-3] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
            B[i_2-1][i_3] = B[i_2-1][i_3+1] - B[i_2][i_3] + 4;
        }
    }
#pragma endscop
...
kernel_2222312315_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
