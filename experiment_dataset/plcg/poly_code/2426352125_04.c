...
void kernel_2426352125_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0-1] = E[i_0] - F[i_0-1] * E[i_0-1] * G[i_0] + A[i_0-2] * 1;
    }
    for (int i_1 = 1; i_1 < PB_N-2; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            B[i_2+i_1][i_2] = H[i_1+1][i_2+i_1] - 5;
        }
        A[i_1-1] = G[i_1+2] * G[i_1] - E[i_1] + G[i_1+1] * E[i_1] - G[i_1] * G[i_1] - 6;
    }
    for (int i_3 = 2; i_3 < PB_N-2; i_3++) {
        D[i_3] = G[0] - E[i_3+2] * I[i_3-2][i_3-2] - I[i_3][0] - F[i_3] + G[i_3] - D[i_3+1] * 3;
    }
#pragma endscop
...
kernel_2426352125_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
