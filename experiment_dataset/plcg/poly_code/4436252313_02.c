...
void kernel_4436252313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] - E[i_0-1] + F[i_0] - A[i_0-1] + 4;
        A[i_0] = G[i_0-2][i_0+1] + E[i_0-1] - F[i_0-1] - F[i_0] - E[i_0] * G[i_0][i_0] - A[i_0] - A[i_0-1] - 1;
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 3; i_2 < min(i_0, -i_1+PB_P); i_2++) {
                C[i_2-2][i_1+2] = H[i_1][i_2][i_0+1] + I[i_2][i_0][i_1-2] - I[i_2][i_0][i_1+2] + H[i_1][i_0][i_2+i_1] * C[i_2-3][i_1+2] - C[i_2-1][i_1+2] * C[i_2-2][i_1+1] - 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
        for (int i_4 = 2; i_4 < min(min(PB_M-2, -i_3+PB_Q-1), i_3); i_4++) {
            for (int i_5 = 2; i_5 < PB_Q-2; i_5++) {
                for (int i_6 = 3; i_6 < min(-i_5+PB_L+1, PB_M); i_6++) {
                    D[i_4][i_6][i_6-2] = J[i_4+2][i_5][i_6-1][i_5-1] * K[i_6][i_4+2][i_3+2][i_6][i_5] * J[i_6][i_3][i_4][i_5] + H[i_5-2][i_4+1][0] + L[i_4-2][i_6+i_5-1][i_4+i_3+1][i_5+2] + D[i_4-1][i_6][i_6-2] - D[i_4+1][i_6-1][i_6-3] - D[i_6][i_5+1][i_5-1] * 1;
                }
            }
        }
    }
