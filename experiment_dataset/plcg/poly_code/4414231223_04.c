...
void kernel_4414231223_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
        ...
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = max(1, i_0); i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < PB_L; i_3++) {
                    A[i_1][i_2][i_3][i_0] = E[i_1][i_0][i_1][i_2] - A[i_1-1][i_0][i_2-1][i_3+1] * A[i_1][i_2][i_3-1][i_0] * 6;
                }
            }
        }
        for (int i_4 = 0; i_4 < min(-i_0+PB_M, i_0); i_4++) {
            B[i_4][i_0+1] = F[i_0][i_4] * F[i_4+i_0][i_0] * B[i_4+1][i_0+1] - 5;
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = 1; i_6 < min(i_5, PB_L-1); i_6++) {
            for (int i_7 = 1; i_7 < -i_6+PB_N-1; i_7++) {
                B[i_7][i_6+1] = G[i_5][i_7][i_6] + G[i_7+i_6+1][i_5-1][i_6-1] * G[i_6-1][i_7+i_6][i_5] + F[i_7-1][i_6-1] - G[i_7][i_5+1][i_6] * 5;
            }
        }
        for (int i_8 = 0; i_8 < PB_L; i_8++) {
            for (int i_9 = i_8; i_9 < PB_M; i_9++) {
                B[i_9][i_8] = H[i_8][i_9][i_5] - H[i_8][i_5][i_9+1] * G[i_5][i_9][i_8] + 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414231223_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
