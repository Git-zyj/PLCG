...
void kernel_3434332313_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] + F[1] - B[i_0] * B[i_0+1] * 4;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1] = G[i_1] + G[i_1-1] * E[i_1+1] * B[i_1-1] - 3;
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
                C[i_1][i_2][i_3] = H[i_3-2][i_2] - I[i_3][i_1-1][i_2-2] * I[i_3][i_1-1][i_2] + C[i_1][i_2+1][i_3] * C[i_3+1][i_2][i_1] - C[i_2][i_1][i_3-1] + C[i_1+1][i_2+1][i_3] - C[i_2-1][i_1][i_3+1] + 3;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        D[i_4] = G[i_4] + F[i_4+2] + F[i_4] - E[i_4-1] * B[i_4] + B[i_4-1] - D[i_4+1] - 3;
    }
#pragma endscop
...
kernel_3434332313_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
