...
void kernel_3234212213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_L-3); i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_N+1, -i_1+PB_L); i_2++) {
                A[i_1+i_0][i_2+i_0-1] = A[i_1+i_0-1][i_2+i_0-1] * A[i_2+i_1][i_2+i_0-2] * B[i_1+i_0+3][i_2+i_1][i_0+1] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < min(PB_P+2, -i_3+PB_L-2); i_4++) {
            for (int i_5 = i_3; i_5 < min(-i_3+PB_P-1, -i_4+PB_L); i_5++) {
                B[i_4+i_3+2][i_5+i_4][i_3] = C[i_5][i_3][i_4-2] + C[i_5][i_4+1][i_5+i_3+1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3234212213_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
