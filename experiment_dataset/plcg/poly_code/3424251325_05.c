...
void kernel_3424251325_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P-1, -i_0+PB_M+1); i_1++) {
            A[i_0+1][0] = E[i_1][i_0-1] * E[i_1+i_0-1][i_1+1] + F[i_1][i_0-1] * G[i_1-1] + F[i_1-1][i_0+1] - A[i_1+2][0] + 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < PB_P-1; i_3++) {
            for (int i_4 = 3; i_4 < min(-i_2+PB_L-1, i_3); i_4++) {
                B[i_2-1][i_3][i_4] = E[i_4][i_3-1] * H[i_2][i_3][i_4] + I[i_2][i_4][i_3-1] - I[i_4][i_4+i_2][i_3] + E[i_3-1][i_2] * H[i_3][i_2+1][i_4-1] * I[i_4][i_2-1][i_4+i_2+1] - B[i_2-1][i_3+1][i_4] + B[i_2-2][i_3+1][i_4] + B[i_2][i_3][i_4] * B[i_4-3][i_3-2][i_2] + 2;
            }
        }
        A[i_2+2][0] = J[i_2] - J[i_2] - J[i_2] * J[i_2] * A[i_2+1][0] + 3;
        A[i_2][0] = J[i_2] + G[i_2+1] - G[i_2] * 6;
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3424251325_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
