...
void kernel_4422132125_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
                B[i_2-2][i_1] = E[i_1][i_0-1][1] * F[i_2][i_1] * F[i_1][i_2] * F[i_2+1][i_1-2] * B[i_2-2][i_0] - 1;
            }
            B[i_1-2][i_0] = F[i_1+1][i_0] * 3;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            B[i_3-1][i_4] = F[i_4][i_3] - F[i_4-1][i_3] + G[i_4] * F[i_4][i_3+2] + 6;
            for (int i_5 = 0; i_5 < PB_M; i_5++) {
                for (int i_6 = i_3; i_6 < PB_M; i_6++) {
                    B[i_6-2][i_5] = H[i_6][i_3][i_4-1][i_3-1] * H[i_6-1][i_5][i_4+i_3-2][i_3-2] + E[i_4][i_5][i_3] - 1;
                }
            }
        }
    }
...
kernel_4422132125_03(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
