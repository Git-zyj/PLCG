...
void kernel_3236211215_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            A[i_1+i_0][i_1] = B[i_1+1][i_1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < -i_2+PB_L; i_3++) {
            B[i_3+1][i_3] = C[i_3+i_2][0] - C[i_2][i_3] + B[i_3][i_3-1] * B[i_3+2][i_3+1] - 1;
        }
    }
#pragma endscop
...
kernel_3236211215_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
