...
void kernel_2434211323_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L-2; i_1++) {
            A[i_1][i_0+1] = A[i_1+2][i_0+1] * A[i_1-1][i_0+1] + 6;
        }
    }
    for (int i_2 = 3; i_2 < PB_N-1; i_2++) {
        B[i_2-1] = E[i_2] + E[i_2+1] * 3;
        for (int i_3 = 3; i_3 < PB_L+3; i_3++) {
            C[i_3][i_2-1] = B[i_2-1] - B[i_3-1] * C[i_2][i_3-3] + C[i_3+2][i_2-3] - 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-3; i_4++) {
        D[i_4+1] = D[i_4-1] * D[i_4+2] * D[i_4] + D[i_4+3] + 5;
    }
#pragma endscop
...
kernel_2434211323_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
