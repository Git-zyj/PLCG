...
void kernel_4322212123_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
            A[i_3-2][i_3+1] = D[i_3] + A[i_3-2][i_3+1] * 4;
        }
    }
    for (int i_4 = 3; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            C[i_4-1][i_5-1] = A[i_4-3][i_4] * 2;
        }
    }
#pragma endscop
...
kernel_4322212123_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
