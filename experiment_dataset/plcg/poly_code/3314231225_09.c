...
void kernel_3314231225_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0-1][0] * E[i_0] + F[i_0] - D[i_0][i_0] * A[i_0-1] - 3;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_1] = G[i_0][0] - A[i_1] + 1;
        }
        for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
            for (int i_3 = 0; i_3 < min(PB_M, i_2); i_3++) {
                A[i_3] = H[i_2-1][i_0-1][i_3] - H[i_3][i_0][i_2-1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3314231225_09(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
