...
void kernel_4236211321_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-3; i_0++) {
        A[i_0+1] = C[i_0] * C[i_0] - A[i_0+3] * A[i_0] * A[i_0+2] * 5;
    }
    for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 3; i_2 < i_1; i_2++) {
            for (int i_3 = i_1 - 1; i_3 >= 1; i_3--) {
                for (int i_4 = 1; i_4 < PB_N-2; i_4++) {
                    B[i_4+1][i_3-1][i_2-1] = B[i_1+1][i_4-1][i_2-3] - B[i_4+1][i_3-1][i_2-3] + B[i_4+2][i_3-1][i_2-1] + 1;
                }
            }
        }
    }
...
kernel_4236211321_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
