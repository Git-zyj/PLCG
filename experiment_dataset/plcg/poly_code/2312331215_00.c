...
void kernel_2312331215_00(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        C[i_0] = D[i_0] + D[0] * E[i_0+1] * 2;
        B[i_0] = F[i_0] - E[i_0+1] * B[i_0+1] - B[i_0-1] * 4;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_L-1, i_1); i_2++) {
            C[i_2] = G[i_1][i_1] * H[i_2-1][i_1] * C[i_2+1] + C[i_2-1] * 4;
        }
    }
#pragma endscop
...
kernel_2312331215_00(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
