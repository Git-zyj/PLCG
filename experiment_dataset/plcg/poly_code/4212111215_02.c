...
void kernel_4212111215_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_L, -i_1+PB_L); i_2++) {
                A[i_0-1][i_2][i_2+i_0] = C[i_2-1][i_0][i_2] + A[i_0-2][i_2][i_2+i_0-1] + A[i_2-2][i_1+1][i_2+i_1] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_Q; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_3+PB_L+1, PB_L-1); i_5++) {
                A[i_5-2][i_3][i_5+i_3-1] = C[i_5+1][i_3][i_4] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4212111215_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
