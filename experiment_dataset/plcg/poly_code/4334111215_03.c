...
void kernel_4334111215_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = min(PB_P-2, i_0) - 1; i_1 >= 0; i_1--) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_2][i_0][i_1+1] = D[i_0-1][i_1][i_2] * A[i_2][i_0][i_1+2] * A[i_2+1][i_0][i_1+1] + 5;
            }
        }
        for (int i_3 = i_0; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                for (int i_5 = 1; i_5 < PB_L+1; i_5++) {
                    A[i_3][i_5-1][i_0+1] = D[i_4+1][i_3-1][i_0] + 2;
                }
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_L; i_6++) {
        for (int i_7 = i_6; i_7 < PB_P-2; i_7++) {
            for (int i_8 = 0; i_8 < min(i_6, -i_6+PB_L); i_8++) {
                A[i_6][i_8][i_7+2] = D[i_7][i_8+i_6][i_7+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4334111215_03(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
