...
void kernel_4326131325_00(int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                for (int i_3 = max(3, i_1); i_3 < min(-i_0+PB_Q+2, -i_2+PB_L); i_3++) {
                    C[i_3-3][i_1-1][i_2-2][i_3+i_0-2] = D[i_1][i_3+i_2][i_3][i_0] * E[i_3-1][0][i_2][i_0+1][i_1] * D[i_1][i_0-1][i_1+1][0] - 3;
                }
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < -i_4+PB_M; i_5++) {
            for (int i_6 = 1; i_6 < i_5; i_6++) {
                for (int i_7 = 1; i_7 < -i_6+PB_Q; i_7++) {
                    C[i_7-1][i_5][i_4-1][i_7+i_6] = D[i_5+i_4][i_7+i_6-1][i_6][i_5] + D[i_5][i_7-1][i_6-1][i_4-1] * 5;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_Q-2; i_8++) {
        for (int i_9 = 0; i_9 < PB_P-2; i_9++) {
            for (int j_0 = i_9; j_0 < -i_8+PB_Q-2; j_0++) {
                for (int j_1 = 0; j_1 < PB_L-1; j_1++) {
                    C[i_8-1][i_9][j_1][j_0+i_8] = D[i_9+1][i_8-1][j_1][i_9] * F[i_9][j_0][i_8] - D[j_0][i_9][j_1][i_8] - C[i_8-1][j_1+1][i_9][j_0+i_8] * C[i_8][i_9][j_1][j_0+i_8-1] + C[i_8+1][j_1][i_9+2][j_0+i_8+2] + 6;
                }
            }
        }
    }
...
init_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4326131325_00(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
