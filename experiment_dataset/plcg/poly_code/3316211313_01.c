...
void kernel_3316211313_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        ...
        for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
            A[i_2-1] = B[i_2-1] + B[i_1] - B[i_2+1] - 1;
        }
    }
#pragma endscop
...
kernel_3316211313_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
