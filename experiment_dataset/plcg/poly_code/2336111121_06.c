...
void kernel_2336111121_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_N, -i_0+PB_L); i_1++) {
            A[i_1+i_0-1][i_0] = D[i_1][i_1+i_0] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-2; i_3++) {
            B[i_2-1][i_3] = B[i_2-1][i_3+2] + B[i_2-1][i_3+1] + 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-1; i_5++) {
            C[i_5] = D[i_4][i_5] + D[i_4-1][1] - C[i_5+1] - 1;
        }
    }
#pragma endscop
...
kernel_2336111121_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
