...
void kernel_4434312225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-3; i_0++) {
        A[i_0+1] = E[i_0-2] + E[i_0-1] - A[i_0] * A[i_0+3] + 5;
    }
    for (int i_1 = 3; i_1 < PB_Q; i_1++) {
        for (int i_2 = i_1; i_2 < PB_P-2; i_2++) {
            for (int i_3 = 0; i_3 < -i_1+PB_M+1; i_3++) {
                for (int i_4 = 0; i_4 < PB_Q; i_4++) {
                    B[i_3+i_1-2][i_4][i_2][1] = B[i_3+i_1-3][i_4][i_2+2][1] * B[i_3+i_1-1][i_4][i_2+2][1] * 2;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_P; i_5++) {
        ...
        for (int i_8 = 3; i_8 < min(min(-i_5+PB_L, i_5), PB_M-2); i_8++) {
            for (int i_9 = 1; i_9 < PB_M-2; i_9++) {
                D[i_9+2][i_5][i_8-1] = F[i_9][i_8+i_5][i_5] * D[i_8+2][i_5-1][i_9-1] * D[i_9+2][i_5][i_8-2] * D[i_9+2][i_5][i_8-3] + D[i_9+2][i_5-1][i_8-1] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4434312225_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F)));
