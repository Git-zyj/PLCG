...
void kernel_3132351325_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_L+1, PB_P), -i_0+PB_N-2); i_2++) {
                A[i_2+i_0][i_1][i_1] = B[i_1-1][i_2+i_0][i_1] * B[i_0-1][i_1][i_2] * C[i_2][i_2] * D[i_2][i_1][i_0] - B[i_2+i_1-1][i_0][i_0] * A[i_2+i_0+2][i_1+1][i_1+1] + A[i_2+i_0-1][i_1-1][i_1-1] * A[i_2+i_0+1][i_1-2][i_1-2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3132351325_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
