...
void kernel_2416251223_03(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        D[i_0-1][1] = E[i_0] + E[i_0] + F[i_0] * F[i_0] + F[i_0] * E[0] * D[i_0-1][1] * D[i_0+1][1] * 4;
        D[i_0][1] = G[i_0][i_0] + F[i_0+1] - H[i_0][0] * 2;
    }
    for (int i_1 = 1; i_1 < PB_Q-1; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_Q, -i_1+PB_M-1); i_2++) {
            D[i_1+1][1] = I[i_2-1][i_2][i_1] + H[i_1][i_1] * E[i_2-1] * G[i_1-1][i_2] * G[i_2+1][i_1+1] * H[i_2+1][i_1] + H[i_2+i_1][i_2+1] * 4;
            D[i_2-1][1] = H[i_2+i_1+1][i_1] * 4;
        }
    }
#pragma endscop
...
kernel_2416251223_03(xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
