...
void kernel_2322312325_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        ...
        for (int i_2 = 1; i_2 < -i_0+PB_L-2; i_2++) {
            B[i_0][i_2+i_0+1] = D[i_2-1][i_0+1] * B[i_0-1][i_2+i_0] * B[i_0-1][i_2+i_0+2] + 6;
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        C[i_3] = C[i_3-2] * C[i_3+2] - C[i_3-1] * C[i_3+1] + 2;
    }
#pragma endscop
...
kernel_2322312325_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
