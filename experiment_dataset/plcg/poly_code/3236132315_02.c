...
void kernel_3236132315_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
                B[i_2+1][i_1+1] = C[i_0][i_2-1][i_1] + C[i_1+2][i_0][i_1+1] * 5;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L-1; i_4++) {
            B[i_3][i_4+1] = D[i_3-2][i_4+1] + D[i_3][0] - E[i_3] + D[i_3][i_4+1] + B[i_4][i_3] * B[i_3+1][i_4+1] * B[i_3][i_4+2] + 6;
        }
    }
#pragma endscop
...
kernel_3236132315_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
