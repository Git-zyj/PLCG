...
void kernel_2224151325_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_N-1 - 1; i_0 >= 3; i_0--) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            A[i_0-1][0] = C[i_1][i_0-1] + C[i_1+1][i_0+1] + C[i_0-1][i_0] + C[i_1][i_0] - C[i_1][i_0+1] + C[i_1][i_0] + A[i_0-3][0] * A[i_0+1][0] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        A[i_2+1][0] = D[i_2] * D[i_2] * D[i_2] - D[i_2] * A[i_2-2][0] + 1;
    }
#pragma endscop
...
kernel_2224151325_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
