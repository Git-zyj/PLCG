...
void kernel_2336131123_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0] = D[1] + D[i_0] * D[i_0] - E[i_0-1][0] * A[i_0-2] + 5;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < min(-i_1+PB_M, PB_N+1); i_2++) {
            A[i_2-2] = D[i_2-1] * E[i_2+i_1][0] - E[i_2][i_1-1] + 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        C[i_3+1] = D[i_3-1] - D[i_3] - 5;
    }
#pragma endscop
...
kernel_2336131123_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
