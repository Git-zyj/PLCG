...
void kernel_2326211311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        C[i_2+1] = C[i_2] * 6;
        C[i_2] = D[i_2] - A[i_2] - A[i_2-1] + C[i_2+1] + 3;
    }
#pragma endscop
...
kernel_2326211311_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
