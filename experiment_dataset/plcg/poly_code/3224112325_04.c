...
void kernel_3224112325_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_N-3; i_2++) {
            B[i_2+2] = B[i_2] - B[i_2+1] * B[i_2+3] - 3;
        }
    }
#pragma endscop
...
kernel_3224112325_04(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
