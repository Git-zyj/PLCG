...
void kernel_2426351225_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < -i_0+PB_L; i_1++) {
            A[i_1] = E[i_1][i_1+i_0] - A[i_1+2] + A[i_1-1] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L-1; i_3++) {
            A[i_3] = E[i_3+1][i_3+i_2+1] - E[i_3+1][i_2] * 2;
        }
        for (int i_4 = i_2; i_4 < PB_L-1; i_4++) {
            A[i_4-1] = E[i_4][i_2] - F[i_2] * G[i_2][i_4+1] * H[i_4][0] - G[i_4-1][i_2+1] - G[i_4][i_2+1] - G[i_2][i_4+1] + E[i_2-1][i_4] * 2;
        }
    }
    for (int i_5 = 1; i_5 < PB_L; i_5++) {
        A[i_5] = I[i_5-1] - F[i_5] + J[i_5] - I[i_5] - F[i_5] * F[i_5] + E[i_5-1][i_5] * 4;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2426351225_07(xa,POLYBENCH_ARRAY(A), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
