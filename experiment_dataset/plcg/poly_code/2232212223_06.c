...
void kernel_2232212223_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_M, i_2); i_3++) {
            B[i_3][i_2+1] = C[i_3][i_2] * D[i_3] + B[i_3][i_2+2] + B[i_3+1][i_2-1] + 4;
        }
    }
#pragma endscop
...
kernel_2232212223_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
