...
void kernel_3422151325_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            A[i_0+1][i_1-1] = E[i_1] - F[i_1+1][i_0] * F[i_1+i_0][i_1] - F[i_0+1][i_1-1] * F[i_1+i_0-1][i_0-1] * F[i_1][i_0] + E[i_1+1] + A[i_1-1][i_0-2] + 6;
        }
    }
    for (int i_2 = 3; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < min(-i_2+PB_P+1, i_2); i_3++) {
            A[i_2+2][i_3-1] = F[i_3][i_2] - E[0] + F[i_3-1][i_2] - 4;
            for (int i_4 = 1; i_4 < -i_2+PB_P; i_4++) {
                C[i_4][i_3+i_2-1][i_2][i_4] = F[i_4-1][i_3] * G[i_3][i_2][i_2+1] - F[i_4-1][i_3] - F[i_4][0] - C[i_2-1][i_4+i_3-2][i_4][i_2-1] - C[i_3-1][i_4+i_2][i_2+1][i_3-1] + C[i_2+1][i_4+i_3-1][i_4][i_2+1] + C[i_4+2][i_3+i_2-3][i_2-2][i_4+2] * 4;
            }
        }
        for (int i_5 = 1; i_5 < PB_P; i_5++) {
            for (int i_6 = 1; i_6 < min(i_5, PB_L-1); i_6++) {
                A[i_6+1][i_5-1] = F[i_6-1][i_5] - F[i_6-1][i_6] * F[i_6][i_5] * 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3422151325_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
