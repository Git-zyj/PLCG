...
void kernel_2334352315_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        A[i_0][i_0-1] = D[i_0] * E[i_0-2] - F[i_0-2] + D[i_0] - D[i_0+2] * A[i_0+1][i_0] - A[i_0-1][i_0-2] * 3;
    }
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = PB_P - 1; i_2 >= 0; i_2--) {
            B[i_2+2] = G[i_2][1] - G[i_2+2][i_1] * H[i_1][0] * G[i_1][i_2] - B[i_2+1] + B[i_2+3] * B[i_2+1] - 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_N; i_3++) {
        A[i_3-1][i_3-2] = F[i_3] + D[i_3] * F[i_3] * D[i_3] + 6;
    }
#pragma endscop
...
kernel_2334352315_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
