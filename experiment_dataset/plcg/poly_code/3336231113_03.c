...
void kernel_3336231113_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_1] = D[i_1][i_0] + E[i_1] * A[i_1+1][i_0] + 3;
        }
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            A[i_0-1][i_2] = F[i_0-1][i_0] * E[i_2] + G[i_2][i_0][i_2] + 5;
        }
    }
    for (int i_3 = 0; i_3 < PB_N; i_3++) {
        C[i_3+1] = H[i_3] - H[i_3] + C[i_3+2] + 2;
    }
#pragma endscop
...
kernel_3336231113_03(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
