...
void kernel_4334332123_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0] + D[i_0] * D[i_0] + E[i_0] * F[i_0] + A[i_0-1] * 5;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M+1, i_1); i_2++) {
            B[i_2+i_1-1] = 2;
        }
        for (int i_3 = 3; i_3 < min(PB_L, -i_1+PB_M-2); i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                C[i_4-1][i_3][i_4+i_3-2] = G[i_4][i_3+i_1+2][0] * G[i_3][i_4][i_1] * H[i_3+2][i_4][i_1] + C[i_4-1][i_3-1][i_4+i_3-3] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4334332123_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
