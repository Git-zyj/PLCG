...
void kernel_4424132111_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_N+2, PB_M-2); i_2++) {
                for (int i_3 = 1; i_3 < min(PB_P, -i_2+PB_L-1); i_3++) {
                    A[i_0-1][i_3][i_1+1][i_1-1] = E[i_2+i_1-2][i_0][i_3+i_2+1][0] + E[i_2+2][i_0][i_3][i_1] * E[i_0][i_2+2][i_1-1][i_3-1] - 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M; i_5++) {
            B[i_4][i_5-2] = F[i_5-2][i_4+2] - F[i_5][i_4] + F[i_4-1][i_5] * 4;
        }
        for (int i_6 = i_4; i_6 < -i_4+PB_M-2; i_6++) {
            C[i_6+i_4+1][i_6+i_4-2] = G[i_4] + F[i_6][i_4] * C[i_6+i_4+2][i_6+i_4-1] * 5;
        }
    }
    for (int i_7 = 1; i_7 < PB_M; i_7++) {
        for (int i_8 = i_7; i_8 < PB_Q-1; i_8++) {
            for (int i_9 = 1; i_9 < PB_Q-1; i_9++) {
                D[i_7][i_9][i_8] = F[i_7][i_8] + H[i_8][i_8][i_7] + H[i_8][i_7-1][i_9-1] * D[i_7][i_9+1][i_8+1] - 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4424132111_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
