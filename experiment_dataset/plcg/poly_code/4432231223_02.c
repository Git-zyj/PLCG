...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_1+1][i_2] = E[i_2] + F[i_2][i_1+1] + B[i_1+1][i_2-1] + 4;
        }
        for (int i_3 = max(2, i_1); i_3 < PB_M; i_3++) {
            A[i_3] = F[i_1][i_1] + G[i_1+1][i_3-1] + H[i_3-1] + B[i_1+1][i_3-2] + 6;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < min(i_4, -i_4+PB_M); i_5++) {
            A[i_5-2] = F[i_5+i_4][i_5+1] + F[i_5][i_4] - G[i_5][i_4-1] - I[i_5+i_4][i_4][0] + G[i_5+1][i_5] + E[i_5+1] * A[i_5-2] - 2;
        }
    }
#pragma endscop
...
kernel_4432231223_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
