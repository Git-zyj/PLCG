...
void kernel_3222111211_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_P-1, PB_M+1); i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_P-2, PB_M); i_4++) {
                B[i_4][i_2+1][i_3+i_2+1] = C[i_3-1][i_4-1] - B[i_2][i_3+2][i_4+i_3+2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3222111211_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
