...
void kernel_4234152315_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = C[i_0] + C[i_0] - C[i_0] * C[0] - C[2] + A[i_0-1] * A[i_0+1] + A[i_0+1] - A[i_0-1] - 3;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_P, -i_1+PB_Q+1); i_2++) {
            for (int i_3 = 2; i_3 < min(min(PB_Q-1, -i_1+PB_M-1), -i_2+PB_Q-1); i_3++) {
                B[i_1][i_3+i_2][i_3] = D[i_3+i_1+1][i_3][i_2] + D[i_2-1][i_3-2][i_1] * D[i_3+2][i_1][i_2-1] - D[i_3][i_1][i_2] - B[i_1][i_3+i_2+1][i_3+1] - B[i_3-1][i_2+i_1-1][i_1-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4234152315_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
