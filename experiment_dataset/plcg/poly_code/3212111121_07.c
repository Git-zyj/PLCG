...
void kernel_3212111121_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M, PB_P-1); i_2++) {
                A[i_2+i_0][i_1][i_1] = C[i_2][i_1+i_0][i_2+1] + A[i_2+i_1-1][i_0][i_0] * 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_L; i_4++) {
            B[i_4] = C[i_3][i_4][i_3] * B[i_4-2] - 1;
        }
    }
#pragma endscop
...
kernel_3212111121_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
