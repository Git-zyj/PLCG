...
void kernel_4314111221_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_0-1][i_1-1][i_0] = A[i_1+1][i_0-1][i_1+2] * 1;
            }
        }
        B[i_0][i_0-1] = D[i_0+1] + 1;
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_L-3, i_3); i_4++) {
            for (int i_5 = 0; i_5 < PB_L-3; i_5++) {
                for (int i_6 = 1; i_6 < PB_P-2; i_6++) {
                    C[i_4][i_4+1][i_6] = C[i_4+2][i_4+3][i_6+2] + C[i_5+2][i_5+3][i_6-1] + C[i_4][i_4+1][i_6-1] + 1;
                }
            }
        }
    }
...
kernel_4314111221_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
