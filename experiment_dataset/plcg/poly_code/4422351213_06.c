...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4422351213_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
        ...
        for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M+1, PB_P-1); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_P-1, -i_1+PB_P+1); i_3++) {
                    A[i_0][i_1][i_3+i_2][i_1] = E[i_0-1][i_1-1][i_2-1][0] - F[i_2+i_1-1][i_0][i_3][i_2][i_1+1] + G[i_2][i_0+1][i_1+1][1] * A[i_0-1][i_2][i_3+i_1-1][i_2] + A[i_1+1][i_0][i_3+i_2+1][i_0] - 5;
                }
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < PB_P; i_5++) {
            for (int i_6 = 1; i_6 < -i_4+PB_L; i_6++) {
                for (int i_7 = 1; i_7 < -i_6+PB_P; i_7++) {
                    B[i_7][i_6][i_5] = G[i_6+1][i_4][i_7][i_6+1] * G[i_7-1][i_6-1][i_4+1][i_5] * H[i_6+1][i_5][i_7] - G[i_5][i_6+i_4][i_7+i_6][i_7] * B[i_7-1][i_6][i_4] + 3;
                }
            }
        }
        for (int i_8 = 0; i_8 < PB_M-1; i_8++) {
            D[i_4][i_8][i_4] = I[i_8] + J[i_8+1][i_4] + K[i_8][i_4-1] * K[i_4][i_8+1] * K[i_8+1][i_4+1] * 1;
        }
    }
    for (int i_9 = 1; i_9 < PB_P; i_9++) {
        for (int j_0 = 1; j_0 < min(-i_9+PB_M, PB_P-2); j_0++) {
            for (int j_1 = 1; j_1 < PB_P-2; j_1++) {
                D[i_9][j_1+1][i_9] = L[j_1][i_9][j_0-1] * M[j_1+j_0+1][i_9][j_0] * M[j_1+1][i_9][j_0] + H[j_1][j_0][i_9] - M[j_0+1][j_1][i_9] - H[j_1][j_0+i_9][i_9] + L[j_0+1][j_1-1][i_9-1] - D[i_9][j_1+2][i_9] - D[j_1][j_0+2][j_1] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4422351213_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,POLYBENCH_ARRAY(M));
