...
void kernel_2436232321_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_L-1); i_1++) {
            A[i_0][i_1+1] = E[i_0] - A[i_0+2][i_1+1] + 1;
        }
    }
    for (int i_2 = 3; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            B[i_2+2][i_3] = F[i_3] * G[i_2][i_2] - G[i_3][i_2] - F[i_3] - H[i_3+1][i_2] * B[i_2+3][i_3] + B[i_2+4][i_3] * 3;
        }
        for (int i_4 = i_2; i_4 < PB_L-2; i_4++) {
            C[i_2-1][i_4] = H[0][i_2] + G[i_2-1][i_4+1] * G[i_4][i_2+1] - C[i_2-1][i_4+1] - C[i_2-1][i_4-1] + C[i_2-3][i_4] + C[i_2-1][i_4+2] * 4;
        }
    }
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 0; i_6 < min(i_5, PB_L-2); i_6++) {
            D[i_5-1][i_6] = G[i_6][i_5+1] - B[i_5+2][i_6] * D[i_5-1][i_6+1] * D[i_5-1][i_6+2] - 5;
        }
    }
#pragma endscop
...
kernel_2436232321_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
