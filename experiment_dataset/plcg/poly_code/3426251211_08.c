...
void kernel_3426251211_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - E[i_0] * E[i_0] + E[i_0] - A[i_0+1] + A[i_0-1] - A[i_0-1] - 5;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 1; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
                D[i_1+1][i_3-1][i_3-1] = F[i_3+i_2][i_1+1][i_2] * G[i_1-1][i_3-1] - H[i_3][i_2][i_1] * H[i_3][i_2-1][i_1] * G[i_3+1][i_2+1] * F[i_2][i_3][i_1] - 2;
            }
        }
        C[i_1] = I[i_1-1] + I[i_1] - E[i_1] * A[i_1+1] + 1;
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < min(-i_4+PB_M, PB_L-1); i_5++) {
            for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
                D[i_6+1][i_5][i_5] = H[i_6][i_4][i_5+1] - F[i_4][i_6][i_5] + F[i_6+1][i_4-1][i_6-1] - H[i_6][i_5+i_4][i_4] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3426251211_08(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
