...
void kernel_2224311211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        ...
        for (int i_2 = max(1, i_0); i_2 < PB_L; i_2++) {
            B[i_0+1][i_0+1] = C[i_0][i_2][i_2-1] + A[i_2-1] * 2;
        }
    }
#pragma endscop
...
kernel_2224311211_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
