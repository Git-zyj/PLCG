...
void kernel_2426332325_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] + E[i_0] * E[i_0-2] - E[i_0] * A[i_0-1] - A[i_0-2] + 4;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 3; i_2 < PB_M-1; i_2++) {
            B[i_2+1][i_2-2] = F[i_1][i_2] * F[i_2][i_1+1] * B[i_2][i_2-3] * 6;
            B[i_2+1][i_2-2] = G[i_2+1][i_1] + H[i_2][i_1+2] * 1;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_L, PB_L-1); i_4++) {
            A[i_4] = G[i_3][i_3] - I[i_4+i_3] + A[i_4] * A[i_4+1] - 5;
        }
    }
#pragma endscop
...
kernel_2426332325_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
