...
void kernel_2312132113_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        A[i_0] = D[i_0] - D[i_0-1] - A[i_0-1] - 2;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0] = E[i_1][i_0] + E[i_1][i_0-2] - A[i_1] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_N-1, -i_2+PB_M); i_3++) {
            C[i_3+i_2] = E[i_3+1][i_2] + D[i_2] - E[i_3][i_2] + 4;
        }
    }
#pragma endscop
...
kernel_2312132113_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
