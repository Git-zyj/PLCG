...
void kernel_4222212115_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            for (int i_4 = i_2; i_4 < min(PB_P, -i_2+PB_M); i_4++) {
                B[i_4+i_2][i_3][i_3] = C[i_4-1][i_2][0] * B[i_4+i_2-1][i_3][i_3] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4222212115_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
