...
void kernel_2236312315_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_0][i_1][i_1] = C[i_1][i_0] + C[i_0+1][i_0] * 4;
        }
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            B[i_0][i_2][i_2] = B[i_0][i_2+1][i_2+1] + B[i_0+1][i_2][i_2] + B[i_0-1][i_2][i_2] + 6;
        }
    }
#pragma endscop
...
kernel_2236312315_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
