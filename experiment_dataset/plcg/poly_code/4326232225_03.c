...
void kernel_4326232225_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = D[i_0] + D[i_0] * E[i_0] + A[i_0-1] + A[i_0+2] * 3;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            for (int i_3 = 3; i_3 < min(min(i_2, PB_L-1), -i_2+PB_M); i_3++) {
                B[i_2+2][i_2-2] = F[i_2+1][i_1][i_3] - F[i_3+i_2][i_1][i_3] + G[i_3][0] + F[i_1][i_3+1][i_2] * B[i_3+1][i_3-3] + B[i_2+4][i_2] * 5;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        A[i_4-1] = D[i_4+1] - D[i_4] + 6;
    }
#pragma endscop
...
kernel_4326232225_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
