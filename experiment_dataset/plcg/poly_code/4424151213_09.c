...
void kernel_4424151213_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 0; i_3 < min(min(i_1, -i_0+PB_L), -i_2+PB_L-1); i_3++) {
                    A[i_2][i_1+i_0][i_3][i_3] = E[i_2][i_3+1][i_1+1][i_2+1] - A[i_2-1][i_1+i_0][i_3][i_3] * A[i_0][i_3+i_2+1][i_1][i_1] * A[i_2][i_3+i_0][i_1-1][i_1-1] * 4;
                }
            }
        }
        for (int i_4 = i_0; i_4 < PB_L-1; i_4++) {
            C[i_4] = F[i_4-1][0] - G[i_4][i_4][i_0] * F[i_4+1][i_0-1] + F[i_4][i_0] * F[i_4][i_0] * H[i_0] + F[i_0][i_4] * G[i_4+1][i_0+1][i_0] * 4;
        }
    }
    for (int i_5 = 1; i_5 < PB_L+1; i_5++) {
        C[i_5] = H[i_5] - H[i_5] * H[i_5] - 3;
        for (int i_6 = 0; i_6 < PB_L; i_6++) {
            C[i_6+1] = H[i_5] * H[i_6] * F[i_6][i_5] - F[i_5-1][i_6+1] + H[i_5] * 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4424151213_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
