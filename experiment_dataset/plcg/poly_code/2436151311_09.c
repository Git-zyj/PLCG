...
void kernel_2436151311_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1] = E[i_0][i_1] + A[i_1-1] - A[i_1+1] * A[i_1-1] + 3;
        }
        B[i_0] = F[i_0+1] * F[i_0] - F[i_0-1] * A[i_0] + B[i_0-1] - B[i_0+1] - B[i_0-1] - 5;
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        C[i_2] = F[i_2] * F[i_2-1] + F[i_2-1] - A[i_2] * 2;
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_M-1, i_3); i_4++) {
            D[i_3][i_4] = E[i_4+1][i_3] + E[i_3][i_3] * E[i_4][i_3] * E[i_4][i_3-1] + E[i_3][i_4+1] + E[i_3][0] - F[i_3] - E[i_3][i_4] * F[i_4-1] * E[i_4-1][i_3] - B[i_4] - D[i_3-1][i_4] * D[i_3][i_4-1] * 3;
        }
    }
#pragma endscop
...
kernel_2436151311_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
