...
void kernel_3214312311_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = C[i_0] * C[0] + A[i_0-1] + A[i_0+1] - 1;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_L, -i_1+PB_P); i_2++) {
            for (int i_3 = 0; i_3 < min(min(-i_2+PB_P+1, PB_L), -i_1+PB_P-2); i_3++) {
                B[i_3][i_2+i_1] = B[i_2][i_3+i_1+2] - B[i_1][i_3+i_2-1] - B[i_1-1][i_3+i_2-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3214312311_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
