...
void kernel_2226151323_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L, PB_M-2); i_1++) {
            A[i_1+1] = C[i_1+1][i_0] - C[i_0][i_1] + C[i_1+1][i_0+1] - C[i_0][i_1+i_0] * C[i_1][i_0] - A[i_1+2] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_L; i_3++) {
            B[i_2][i_3+1] = C[i_3+i_2][1] + C[i_2][i_3] * C[i_3][i_2] * C[i_3+i_2][0] + A[i_2+1] - B[i_2+2][i_3+1] * B[i_2-2][i_3] - B[i_2+1][i_3-1] - B[i_2-1][i_3+1] + 5;
        }
    }
#pragma endscop
...
kernel_2226151323_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
