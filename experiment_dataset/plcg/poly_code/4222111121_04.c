...
void kernel_4222111121_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M+1; i_3++) {
            for (int i_4 = max(3, i_2); i_4 < PB_L; i_4++) {
                B[i_3-1][i_2][i_4-1] = C[i_4][i_4] + B[i_3-2][i_2][i_4-1] - B[i_3-2][i_2][i_4-3] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4222111121_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
