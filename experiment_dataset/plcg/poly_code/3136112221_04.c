...
void kernel_3136112221_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_P-3, i_0); i_1++) {
            for (int i_2 = max(1, -i_1+3); i_2 < -i_1+PB_M-2; i_2++) {
                A[i_2+i_1-1][i_2-1][i_1+2] = B[i_2-1][i_0] - A[i_2+i_1+2][i_2+1][i_1+3] - A[i_2+i_1-3][i_2-1][i_1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3136112221_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
