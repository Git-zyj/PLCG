...
void kernel_3416131113_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0] * E[i_0-1] * E[i_0] * E[i_0+1] + E[i_0] - 2;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_M, PB_M-2); i_3++) {
                B[i_1+1][i_2+1] = F[i_3][i_2+1][i_1+1] - F[i_2][i_3+1][i_1+1] + F[i_3][i_3+i_1][i_2] * B[i_2+1][i_3+2] - 2;
            }
        }
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            D[i_1-1][i_4][i_4] = 4;
            for (int i_5 = 1; i_5 < min(PB_M, -i_4+PB_N); i_5++) {
                D[i_5][i_4][i_4] = G[i_5+i_4][i_1] - F[i_4][i_5][i_4+1] + D[i_5][i_4+1][i_4+1] * D[i_5-1][i_4][i_4] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3416131113_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
