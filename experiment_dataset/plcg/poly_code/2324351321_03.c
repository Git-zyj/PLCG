...
void kernel_2324351321_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_0+1][i_1] = D[i_1][i_0] + E[i_1-1] * D[i_0-1][i_1] - F[i_1-1] + G[i_1-1][i_0+1] * E[i_0] + H[i_1+1][i_0] - A[i_0+1][i_1-1] * A[i_0+3][i_1] * A[i_0-1][i_1] - A[i_1+3][i_0] - A[i_0-1][i_1-1] + 5;
        }
        B[i_0] = F[1] * I[i_0+1] + B[i_0+1] + B[i_0-1] * B[i_0-2] + 1;
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        C[i_2] = E[i_2] * I[i_2-1] - E[i_2+1] - F[i_2] * E[i_2-1] * C[i_2+1] * 1;
    }
#pragma endscop
...
kernel_2324351321_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
