...
void kernel_2434351213_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_N); i_2++) {
            B[i_1][i_2] = G[i_2+i_1][0] * H[i_2+1][i_1-1] - G[i_2][i_1+1] * I[i_2][i_1] + E[i_2] - B[i_1][i_2+1] + 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_N-1; i_3++) {
        C[i_3] = E[0] * E[1] - E[i_3] * F[1] * J[i_3-1] * E[i_3] + C[i_3+1] * D[i_3-1] - 6;
        D[i_3-1] = J[1] * F[i_3] - F[i_3] * J[i_3] * F[i_3+1] + J[i_3] - F[i_3+1] - D[i_3-2] + 2;
    }
#pragma endscop
...
kernel_2434351213_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
