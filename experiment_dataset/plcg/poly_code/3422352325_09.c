...
void kernel_3422352325_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_Q-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_M+2; i_2++) {
                B[i_0-1][i_1+2] = E[i_1-1][i_0][i_0] + B[i_2-2][i_1] * 3;
            }
            B[i_0][i_1] = F[i_1][i_0+1] * G[i_1+i_0][i_1+1] * F[i_1][i_0] - H[i_1][i_1+i_0+2] + H[i_1][0] * G[i_0-2][i_1-1] - I[i_1-2] + F[i_1][i_0] - F[i_1+i_0][1] * 6;
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_3][i_4] = H[i_4][i_0] * E[i_3][i_4][i_0] * 1;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_L; i_5++) {
        for (int i_6 = 2; i_6 < -i_5+PB_L; i_6++) {
            for (int i_7 = 2; i_7 < i_6; i_7++) {
                D[i_7+2][i_6][i_7+i_6] = J[i_7][i_6+i_5][i_7+i_6-2] - E[i_7][i_6+i_5][i_7] - E[i_7-2][i_6][i_5] * F[i_5][1] * E[i_7-2][i_6-2][i_5-2] + D[i_7+2][i_6+2][i_7+i_6+2] + D[i_7+2][i_6-1][i_7+i_6-1] + D[i_7+2][i_5-1][i_7+i_5-1] - D[i_7+1][i_6][i_7+i_6-1] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3422352325_09(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
