...
void kernel_2432251115_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0] + F[i_0] - F[i_0] - E[i_0] * F[i_0] * E[i_0] + F[i_0] + F[i_0-1] + 6;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        A[i_1+1] = F[i_1-1] * E[i_1-1] * F[i_1-1] * G[i_1][0] - F[i_1] + F[i_1] + 5;
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            A[i_2] = E[i_2] + H[i_3-1][i_2] - I[i_3][i_2][i_3+1] * G[i_3][i_2-1] - G[i_3+1][i_2-1] - 1;
            A[i_3] = G[i_3-1][i_2] * A[i_3] + 4;
        }
    }
#pragma endscop
...
kernel_2432251115_03(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
