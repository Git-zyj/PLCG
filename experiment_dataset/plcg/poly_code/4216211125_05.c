...
void kernel_4216211125_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            B[i_0][i_0-1] = C[i_0] - B[i_0+1][i_0] * 3;
        }
    }
#pragma endscop
...
kernel_4216211125_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
