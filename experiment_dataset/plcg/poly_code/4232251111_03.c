...
void kernel_4232251111_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_Q; i_2++) {
                A[i_2][i_1][i_0-1] = C[i_1][0] - D[i_2-1][i_1][i_0] + D[i_1+i_0][i_2][i_2+i_1] - D[i_1][i_1][i_2] + A[i_2+1][i_1][i_0] + A[i_2-1][i_1][i_0] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 1; i_5 < min(min(PB_P-1, -i_3+PB_M-1), -i_4+PB_Q); i_5++) {
                for (int i_6 = i_3; i_6 < min(-i_4+PB_P-1, -i_3+PB_M-1); i_6++) {
                    B[i_3][i_6+i_4+1][i_5+1][i_5+i_4] = E[i_4][i_5-1][i_3-1][i_3] * E[i_6][i_3][i_4-1][i_4] - F[i_6+i_3+1][i_6][0][i_4][i_3] + E[i_6-1][i_4][i_3][i_5-1] + D[i_4-1][i_6+i_5][i_4-1] + E[i_4][i_6][i_5+i_3+1][i_4+1] * 1;
                }
            }
        }
    }
...
kernel_4232251111_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F));
