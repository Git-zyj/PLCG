...
void kernel_4432311213_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
        ...
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < -i_0+PB_M; i_3++) {
                    A[i_3+i_0][i_2][i_3][i_1] = E[i_3][i_0][i_2][i_1] * A[i_3+i_0-1][i_2][i_3-1][i_1] * A[i_1+i_0][i_3+1][i_1][i_2] + 5;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M; i_5++) {
            for (int i_6 = 1; i_6 < i_4; i_6++) {
                B[i_5][i_4+1] = F[i_5][i_6-1][i_4] + B[i_6][i_4+2] * 4;
            }
        }
    ...
    for (int i_9 = 0; i_9 < PB_L-1; i_9++) {
        for (int j_0 = 0; j_0 < PB_N; j_0++) {
            for (int j_1 = 1; j_1 < PB_M; j_1++) {
                for (int j_2 = i_9; j_2 < PB_M; j_2++) {
                    B[j_1][j_2] = G[j_0][j_2][i_9+1][j_1-1] * 3;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4432311213_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
