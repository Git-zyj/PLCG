...
void kernel_2222311223_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
            B[i_2] = C[i_1][i_1+1] + B[i_2+1] + B[i_2-2] - 1;
        }
    }
#pragma endscop
...
kernel_2222311223_00(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
