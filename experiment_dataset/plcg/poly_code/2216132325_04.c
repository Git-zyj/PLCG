...
void kernel_2216132325_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            A[i_1][i_0+1] = C[i_1+i_0][0] - C[i_1][i_0-1] * C[i_1][i_0] - A[i_1][i_0] + A[i_1+1][i_0] + 1;
        }
        for (int i_2 = 2; i_2 < i_0; i_2++) {
            A[i_2][i_0+1] = C[i_2+1][i_0] * C[i_2-2][i_0] + A[i_0-2][i_2] * 3;
        }
    }
#pragma endscop
...
kernel_2216132325_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
