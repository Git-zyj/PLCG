...
void kernel_2424231111_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0-1] = E[i_0] * F[1] + 5;
        for (int i_1 = i_0; i_1 < min(PB_M, -i_0+PB_N-1); i_1++) {
            D[i_1] = E[i_1+i_0] + G[i_1][i_0+1] + H[i_0][i_1] - G[i_1+i_0+1][i_0] + C[i_0-1] * 1;
        }
        C[i_0-1] = F[i_0-1] - A[i_0-1] + 4;
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        D[i_2] = F[i_2+1] + E[i_2] + D[i_2+1] + 5;
    }
#pragma endscop
...
kernel_2424231111_09(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
