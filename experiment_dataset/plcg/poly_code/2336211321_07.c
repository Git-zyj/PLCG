...
void kernel_2336211321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        ...
        B[i_0+1] = B[i_0] * B[i_0+3] + B[i_0+2] - B[i_0-1] - 2;
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 3; i_3 < min(PB_N-1, -i_2+PB_L+1); i_3++) {
            C[i_3-1] = D[i_3+i_2-1][i_2] - D[i_2][i_3] * C[i_3] + C[i_3-2] - C[i_3-3] - C[i_3+1] + 3;
        }
    }
#pragma endscop
...
kernel_2336211321_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
