...
void kernel_4234151113_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < min(PB_N-2, -i_0+PB_L+1); i_2++) {
                A[i_0][i_2+1][i_1+1][i_2] = C[i_2][i_0][i_1-1] * C[i_1+1][i_0][i_2+i_0-1] + D[i_2][i_2+1] * A[i_1][i_0][i_2+2][i_0-1] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < min(min(-i_3+PB_M+1, PB_M-1), i_3); i_4++) {
            for (int i_5 = 1; i_5 < min(-i_3+PB_M, -i_4+PB_M-1); i_5++) {
                B[i_5+i_4-1][i_4] = C[i_5+i_4+1][i_5][i_3+1] * C[i_4+1][i_5+i_3][i_3] * C[i_5][i_4+i_3-1][i_3] + C[i_3][i_4][0] + C[i_5+1][i_3][i_4] + C[i_5][i_3-1][i_4] + C[i_4][i_5-1][i_4+1] * B[i_5+i_4-2][i_4-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4234151113_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
