...
void kernel_3314352323_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        A[i_0+1] = D[i_0] * D[i_0] * E[i_0-1] + A[i_0+2] * A[i_0+3] - A[i_0-1] + 1;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1; i_2 < PB_P-2; i_2++) {
            A[i_1+2] = F[i_2][i_1] + F[i_2][i_1] * G[i_2+1][i_1] * G[i_1][i_1] + F[i_1][i_2] + H[i_1+2][i_2+2] + 1;
        }
        for (int i_3 = 2; i_3 < i_1; i_3++) {
            for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
                C[i_4][i_3][i_3+1] = F[i_4+1][i_3] + I[i_4][i_3][i_1] - J[i_1][i_4-2][i_3+1] + I[i_3][i_4][i_1-2] * I[i_1-2][i_4-2][i_3] * F[i_4+2][i_1] * C[i_4+1][i_3-2][i_3-1] * C[i_1-2][i_4][i_4+1] - C[i_1-1][i_4-2][i_4-1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3314352323_06(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
