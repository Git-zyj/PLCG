...
void kernel_3226211225_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_L-1, i_1); i_2++) {
            for (int i_3 = i_2; i_3 < PB_P-2; i_3++) {
                B[i_1+1][i_2][i_3] = B[i_1][i_2][i_3] * B[i_1+1][i_2-1][i_3] * B[i_1+1][i_2+1][i_3] + B[i_1+1][i_2][i_3+2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3226211225_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
