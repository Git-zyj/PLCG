...
void kernel_3432132323_07(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < min(PB_M, -i_1+PB_P); i_2++) {
                D[i_0-2][i_2+1] = E[i_0][i_1][0][i_2] * F[i_2-2][i_0][i_1] + F[i_0+1][i_0][i_2+i_1] - F[i_2][i_0-1][i_2] * 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < PB_N; i_5++) {
                D[i_4][i_5] = G[i_5][i_4] * F[i_4][i_5][i_3] * F[i_4-1][i_3-1][i_3] + 6;
            }
        }
        C[i_3] = H[i_3] - H[0] * C[i_3-2] + C[i_3+2] * C[i_3+1] + C[i_3-1] * 3;
    }
    for (int i_6 = 1; i_6 < PB_M; i_6++) {
        for (int i_7 = 1; i_7 < PB_Q; i_7++) {
            for (int i_8 = 1; i_8 < min(min(-i_6+PB_P, -i_7+PB_Q), PB_M-2); i_8++) {
                D[i_8][i_7+1] = F[i_6][i_7][i_8+i_6] + E[i_8+i_7-1][i_6-1][i_6][i_8+i_7] + D[i_8-1][i_6-1] * D[i_8+2][i_7+2] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3432132323_07(xd,yd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
