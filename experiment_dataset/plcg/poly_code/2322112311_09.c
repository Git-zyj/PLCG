...
void kernel_2322112311_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            A[i_0][0] = A[i_0+1][0] + A[i_0-1][0] * A[i_0-1][0] - A[i_0+1][0] - 6;
        }
        for (int i_2 = 0; i_2 < min(PB_L-1, -i_0+PB_P-1); i_2++) {
            B[i_0][i_2+i_0] = D[i_2][i_0] - B[i_2+1][i_2+i_0+1] * B[i_0][i_2+i_0-1] * 5;
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M-4, PB_P); i_4++) {
            C[i_4+i_3+2][i_3+1] = D[i_4+i_3][0] + A[i_3][0] * C[i_4+i_3+4][i_3+2] * C[i_4+i_3+1][i_3+1] - 3;
        }
    }
#pragma endscop
...
kernel_2322112311_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
