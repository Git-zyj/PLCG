...
void kernel_4414231215_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            A[i_1+1][i_0] = E[i_0] - F[i_1][i_0] + G[i_1+1][i_0-1] * 5;
        }
        B[i_0-1] = E[i_0] - H[i_0] * B[i_0] - 6;
        for (int i_2 = 0; i_2 < min(-i_0+PB_L, i_0); i_2++) {
            C[i_0][i_2] = G[i_0-1][i_0] + F[i_2][i_2+i_0] - H[i_2] - F[i_2][0] - A[i_2+1][i_0] - C[i_0-1][i_2] * C[i_2][i_0+1] - 3;
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            for (int i_5 = 1; i_5 < -i_4+PB_M; i_5++) {
                A[i_4+1][i_5] = I[i_5+i_4][i_3][i_4] - F[i_5-1][i_4] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4414231215_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
