...
void kernel_4222231325_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = 2; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M-2; i_2++) {
                for (int i_3 = 0; i_3 < i_2; i_3++) {
                    A[i_2+i_0][i_2+1][i_1][i_3] = C[i_0][i_2][i_1+1][i_3] * D[i_2-1][i_3+i_2+1][i_1+i_0][i_3+i_0-1] - A[i_2+i_0-3][i_2][i_3][i_1] - A[i_2+i_0][i_2+1][i_1+1][i_3] * A[i_2+i_0+2][i_2+1][i_1-2][i_3+2] + 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        B[i_4] = E[i_4] - F[i_4] * G[i_4][i_4] + B[i_4-2] + B[i_4+1] * 4;
    }
#pragma endscop
...
kernel_4222231325_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
