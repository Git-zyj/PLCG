...
void kernel_2422352325_00(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        D[i_0-2][2] = E[i_0] * F[i_0-2] + E[i_0] - 4;
        for (int i_1 = 1; i_1 < -i_0+PB_M+1; i_1++) {
            D[i_0-3][2] = G[i_1-1][i_0] + H[i_0] - I[i_1+i_0-1][i_1] * I[i_1][i_0] * G[i_1][i_0] + E[i_1+i_0] - G[i_1+1][i_1+i_0] * F[i_0+1] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            D[i_2][2] = J[i_2][i_2] - I[i_2][i_3] - J[i_2][i_3] - 3;
        }
    }
    for (int i_4 = 3; i_4 < PB_M; i_4++) {
        D[i_4-2][2] = G[i_4][i_4+2] - E[i_4] + H[i_4] - E[i_4] + H[i_4] - E[i_4] * D[i_4][2] - D[i_4-1][2] + D[i_4-3][2] - 4;
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2422352325_00(xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
