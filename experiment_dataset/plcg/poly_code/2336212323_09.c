...
void kernel_2336212323_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            B[i_1+i_0][i_1+i_0] = B[i_1+i_0][i_1+i_0] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = i_2; i_3 < -i_2+PB_M-2; i_3++) {
            B[i_3+i_2][i_3+i_2] = B[i_3+i_2-2][i_3+i_2-2] + B[i_3+i_2+2][i_3+i_2+2] * 3;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        C[i_4] = D[i_4+1][0] * C[i_4+2] - C[i_4-1] + 5;
    }
#pragma endscop
...
kernel_2336212323_09(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
