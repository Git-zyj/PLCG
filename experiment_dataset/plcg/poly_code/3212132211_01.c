...
void kernel_3212132211_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(min(PB_Q-2, -i_1+PB_L), -i_0+PB_M+2); i_2++) {
                A[i_0][i_0-2][i_2+1] = C[i_2][i_0][0] + C[i_2][i_0][i_1] + C[i_2][i_0-1][i_1] - C[i_2+i_0-2][i_2+i_1][i_0] - A[i_0-1][i_0-3][i_2+1] + A[i_0][i_0-2][i_2+2] - 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < PB_Q+1; i_4++) {
            for (int i_5 = i_4; i_5 < PB_P-2; i_5++) {
                B[i_4][i_5+2] = C[i_4-2][i_3][i_4-1] * B[i_4+1][i_5+1] - B[i_5-1][i_4+2] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3212132211_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
