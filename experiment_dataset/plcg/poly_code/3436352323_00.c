...
void kernel_3436352323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = E[i_0] - E[i_0-1] - F[i_0] + E[i_0+1] * A[i_0+1] + A[i_0-1] * 4;
        for (int i_1 = 2; i_1 < min(PB_P+2, -i_0+PB_M+2); i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_L, PB_P-1), i_1); i_2++) {
                B[i_0][i_1-1][i_2] = G[i_1][i_0+2][i_2] + H[i_2][i_2+i_0][i_1] * I[i_0-1][i_2][i_1-2] * H[i_1][i_0][i_1+2] + G[i_1+i_0-2][i_2][i_1] - B[i_0][i_1-1][i_2+1] * B[i_0+1][i_1-1][i_2] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_N-2, i_3); i_4++) {
            C[i_4+i_3][i_4] = F[i_3] * J[i_3][i_4-2] - K[i_4+i_3+1][2] - L[i_3+2][i_4] - A[i_4] * C[i_4+i_3-1][i_4-1] * C[i_4+i_3+2][i_4+2] * 5;
        }
    }
    for (int i_5 = 1; i_5 < PB_P; i_5++) {
        for (int i_6 = 0; i_6 < PB_P; i_6++) {
            D[i_5][i_6] = L[i_6][i_5] - G[i_5][i_6][i_6] * K[1][i_5-1] - M[i_6] - A[i_6] - A[i_5] * 3;
        }
    }
#pragma endscop
