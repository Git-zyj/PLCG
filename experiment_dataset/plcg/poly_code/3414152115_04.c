...
void kernel_3414152115_04(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
            D[i_0-1][i_1+2] = E[i_0] + F[i_1][i_1+1] - E[i_0-1] + E[i_1] * F[i_1][i_0] * F[i_1][i_0] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_L-1; i_3++) {
            for (int i_4 = 1; i_4 < min(min(-i_3+PB_L, i_2), PB_L-2); i_4++) {
                D[i_3][i_4+1] = G[i_4+i_3][i_4+2][i_2] - G[i_4-1][i_3+i_2+1][i_4+1] * 3;
            }
            for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
                D[i_3+1][i_5] = G[i_3][i_2][i_2-2] * G[i_5+2][i_2][0] + H[i_2+2][i_5][i_3][i_2] - G[i_3-2][i_5][i_2] - G[i_3-1][i_5][i_3] + 2;
            }
        }
        for (int i_6 = 0; i_6 < PB_P-2; i_6++) {
            for (int i_7 = 0; i_7 < -i_2+PB_L; i_7++) {
                D[i_7][i_6+1] = G[i_7][i_2-2][i_6+1] * G[i_7+i_2][i_6][0] + F[i_7+2][i_2] - G[i_7][i_2][i_6] + D[i_7+1][i_6+2] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3414152115_04(xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
