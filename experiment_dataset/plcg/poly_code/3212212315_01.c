...
void kernel_3212212315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L-3, -i_0+PB_M-1); i_1++) {
            A[i_1] = B[i_1+i_0+1][i_1+3] * B[i_1+i_0][i_0+2] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_L-2, -i_2+PB_M-1); i_3++) {
            B[i_3+i_2][i_3+2] = C[i_2][i_3] * C[i_3][i_2-1] * A[i_2] + B[i_3+i_2+1][i_3+2] * B[i_3+i_2-1][i_3+2] + 4;
        }
    }
#pragma endscop
...
kernel_3212212315_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
