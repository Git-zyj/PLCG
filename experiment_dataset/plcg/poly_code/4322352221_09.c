...
void kernel_4322352221_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M-2; i_2++) {
                A[i_0][i_0][i_1+2] = D[i_1][i_2][i_0] * E[i_0][i_1][i_2][i_0] + F[i_2][i_2+i_1+2] + 6;
            }
        }
        for (int i_3 = 2; i_3 < PB_P; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_P, -i_0+PB_Q-1); i_4++) {
                for (int i_5 = 1; i_5 < PB_M; i_5++) {
                    B[i_0][2][i_4+i_0][i_3] = G[i_0][i_4][i_5][i_3] + H[i_5][i_4-1][i_3] * E[i_4][i_0][i_5][i_4] - E[i_5-1][i_3][i_3+1][i_0] - B[i_0+1][2][i_4+i_0+1][i_3-2] * 3;
                }
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_L-2; i_6++) {
        for (int i_7 = 2; i_7 < min(PB_L-3, -i_6+PB_L-1); i_7++) {
            for (int i_8 = 2; i_8 < -i_6+PB_P; i_8++) {
                C[i_7][i_8+2] = I[i_8][i_7+i_6+1][i_8+i_6] - I[i_7+i_6][i_6][1] * I[i_8][2][i_7+2] * D[i_8][i_6][i_7] + D[i_6-1][i_8][i_7] + D[i_8][i_6][i_7] * C[i_8-2][i_7+3] * C[i_8][i_6+1] * C[i_7-2][i_6+2] + C[i_7-2][i_8+1] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4322352221_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
