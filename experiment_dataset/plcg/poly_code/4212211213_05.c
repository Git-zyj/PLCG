...
void kernel_4212211213_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            B[i_2+i_1][i_1] = C[i_2] - C[i_1] * B[i_2+i_1][i_1+1] + 6;
        }
    }
#pragma endscop
...
kernel_4212211213_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
