...
void kernel_3132131221_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P-2, -i_0+PB_P); i_1++) {
            A[i_0][i_1][i_1+1] = B[i_1][i_0+1][i_1+i_0] + C[i_0] - C[i_1] + A[i_0-1][i_1][i_1+1] + A[i_0][i_1+1][i_1+2] - 3;
        }
    }
#pragma endscop
...
kernel_3132131221_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
