...
void kernel_3336212225_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M-2; i_2++) {
                A[i_2][i_1-1][i_2+i_0] = A[i_2+2][i_1-1][i_2+i_0+2] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-3; i_3++) {
        B[i_3+1] = D[i_3] + D[i_3] * B[i_3-1] * B[i_3+2] - B[i_3] * B[i_3+3] * B[i_3-1] * 6;
    }
...
kernel_3336212225_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
