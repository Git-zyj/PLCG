...
void kernel_2212112313_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0][i_0] = C[i_0+1] * 4;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_0][i_1][i_1] = A[i_0-1][i_0-1] * A[i_0][i_0] - A[i_0+1][i_0+1] * A[i_0+1][i_0+1] * B[i_1][i_0-1][i_0-1] + 2;
        }
    }
#pragma endscop
...
kernel_2212112313_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
