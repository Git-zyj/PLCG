...
void kernel_4336231125_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_1+i_0-1][i_1+i_0] = D[i_1] - 1;
        }
    }
        ...
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_N-1, -i_3+PB_M-1); i_4++) {
                for (int i_5 = 1; i_5 < min(-i_2+PB_L, i_3); i_5++) {
                    B[i_5+i_2][0][i_3][i_5] = E[i_3][i_5][i_2+1][0] - F[i_4+i_3+1][i_2][i_5-1][i_2] * E[i_5][i_3][i_2][i_4] + G[i_4+1][i_5-1][i_3+1] + E[i_5+1][i_4][i_5+1][0] * 2;
                }
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
        C[i_6] = D[i_6] + C[i_6+1] - 2;
    }
#pragma endscop
...
kernel_4336231125_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
