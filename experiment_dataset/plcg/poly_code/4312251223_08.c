...
void kernel_4312251223_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_P, -i_0+PB_L-1); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_P, -i_0+PB_M-3); i_2++) {
                A[i_1-1][i_2+i_0+1][i_1+1] = D[i_2][i_0][i_0] * D[i_2][i_1+i_0+1][i_0] + D[i_1-1][i_2-1][i_2] * E[i_1-1][i_1][i_2][i_0] - F[i_2][i_1][i_0] + A[i_1-1][i_2+i_0+3][i_1+1] * A[i_1-3][i_2+i_0+1][i_1-1] - A[i_1-1][i_2+i_0+3][i_1+1] * 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M; i_4++) {
            C[i_4] = G[i_3][i_4+1] + G[i_4+i_3][i_3] * G[i_4][i_3-1] * G[i_4+i_3][i_4] * H[i_4][i_3-1] + 3;
        }
        C[i_3] = I[i_3-1] + J[i_3-1] - J[i_3-1] + I[i_3-1] - J[i_3] + 4;
    }
#pragma endscop
...
kernel_4312251223_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
