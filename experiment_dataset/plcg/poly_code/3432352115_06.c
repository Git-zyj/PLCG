...
void kernel_3432352115_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            B[i_1] = E[i_1][i_0] - F[i_1+2][i_0] + G[i_1-1][i_0] - H[i_1+1] + G[i_1][i_0+2] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        B[i_2] = H[i_2] - I[i_2] * I[i_2] + B[i_2+1] - 2;
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = i_3; i_4 < min(-i_3+PB_M+2, PB_M-1); i_4++) {
            B[i_4-1] = G[i_4+i_3-2][i_4] + E[i_3-1][i_4] + F[i_4+1][i_3+2] - G[i_4-2][i_3-2] * 1;
        }
    }
    for (int i_5 = 0; i_5 < PB_L; i_5++) {
        for (int i_6 = max(1, i_5); i_6 < PB_L-1; i_6++) {
            for (int i_7 = 0; i_7 < PB_L; i_7++) {
                B[i_6+1] = J[i_7][i_5][0] - K[i_5][i_7+2][i_6-1] - K[i_5][i_6][i_7] + K[i_7][i_6][i_5+2] * J[i_7+1][i_5][i_6] - G[i_6][i_5] + L[i_7][i_5][2] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_3432352115_06(xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L)));
