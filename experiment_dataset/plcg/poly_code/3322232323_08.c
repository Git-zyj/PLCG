...
void kernel_3322232323_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+1; i_0++) {
        B[i_0-2] = D[i_0] + E[i_0][i_0-1] + F[i_0] * 1;
    }
    for (int i_1 = 4; i_1 < PB_N-1; i_1++) {
        B[i_1-2] = D[i_1] - F[i_1+1] - F[i_1-1] - B[i_1] * B[i_1-4] * B[i_1-1] + 1;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            B[i_2] = D[i_3] - E[i_3][i_2] + E[i_2][i_3] + 5;
        }
    }
#pragma endscop
...
kernel_3322232323_08(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
