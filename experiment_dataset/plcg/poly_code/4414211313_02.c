...
void kernel_4414211313_02(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        C[i_0][i_0+1] = E[i_0] - B[i_0+1] - 4;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        B[i_1+1] = F[i_1] - B[i_1+2] * B[i_1] + C[i_1+1][i_1+2] + 3;
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            C[i_2][i_2+1] = B[i_1+1] - D[i_2][i_2][i_1] + 1;
            D[i_1][i_1][i_2] = C[i_1][i_1+1] - 1;
        }
    }
#pragma endscop
...
kernel_4414211313_02(xc,yc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
