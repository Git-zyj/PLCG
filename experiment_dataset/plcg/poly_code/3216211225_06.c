...
void kernel_3216211225_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_N; i_2++) {
                B[i_2] = C[i_2+1][i_1][i_0] + 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N+1; i_3++) {
        for (int i_4 = 3; i_4 < -i_3+PB_N+1; i_4++) {
            B[i_4-1] = D[i_4+i_3-1][i_3] - B[i_4-3] * B[i_4-2] + 2;
        }
    }
#pragma endscop
...
kernel_3216211225_06(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
