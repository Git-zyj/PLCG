...
void kernel_2314211323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        ...
        for (int i_2 = 3; i_2 < i_0; i_2++) {
            B[i_2-1] = D[i_2+1][i_0] - B[i_2-3] * B[i_2+1] * B[i_2-2] + 1;
        }
    }
...
kernel_2314211323_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
