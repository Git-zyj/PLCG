...
void kernel_4232331315_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_N; i_2++) {
            for (int i_3 = 0; i_3 < min(min(-i_1+PB_P+1, PB_Q-1), -i_2+PB_M-1); i_3++) {
                B[i_1][i_3+i_2][i_1] = E[i_2+1][i_1][i_3+1] - E[i_2][i_3+i_1-1][i_3+1] * E[i_3+i_2][i_1][i_1] + B[i_1-1][i_3+i_2+1][i_1-1] - B[i_1][i_3+i_2][i_1] * B[i_2][i_3+i_1+1][i_2] + B[i_1+1][i_3+i_2-1][i_1+1] * B[i_1+1][i_3+i_2][i_1+1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4232331315_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
