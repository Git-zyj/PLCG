...
void kernel_2316131125_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        B[i_0] = D[i_0+1][0] - E[i_0] - E[i_0] + E[i_0] + 3;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_M; i_2++) {
            B[i_1+1] = D[i_2+i_1][i_1+1] - D[i_2][i_1] - B[i_2+2] + 6;
            C[i_1-1][i_2][i_1] = D[i_1][i_2-1] - E[i_1] * 2;
        }
    }
#pragma endscop
...
kernel_2316131125_05(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
