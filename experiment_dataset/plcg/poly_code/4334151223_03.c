...
void kernel_4334151223_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        A[i_0+1] = D[i_0][i_0] * E[i_0] + E[i_0] - E[i_0+1] * E[i_0] - E[i_0] * A[i_0] * 1;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M, PB_L-2); i_2++) {
            A[i_2+2] = D[i_2][i_1] - D[i_2+i_1-1][i_1] * D[i_2][i_1] + E[i_2+i_1] - 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_L, -i_3+PB_M-1); i_4++) {
            C[i_4+i_3][i_3] = D[i_3][i_4-1] * D[i_3][0] + D[i_4][i_3] + E[i_4] * C[i_4+i_3+1][i_3] + C[i_4+i_3-2][i_3] + 1;
        }
    }
#pragma endscop
...
kernel_4334151223_03(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
