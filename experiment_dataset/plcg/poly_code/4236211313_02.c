...
void kernel_4236211313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        A[i_0+1] = C[i_0] + A[i_0+2] - A[i_0] + B[0][i_0] + 3;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < PB_M; i_2++) {
            B[0][i_2] = B[0][i_2-1] * B[0][i_2+1] * B[0][i_2-1] * 5;
        }
    }
#pragma endscop
...
kernel_4236211313_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
