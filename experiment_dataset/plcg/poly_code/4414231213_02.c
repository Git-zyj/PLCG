...
void kernel_4414231213_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[0] + F[i_0-1] - A[i_0+1] * 4;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        B[i_1] = F[i_1] * F[i_1] + E[i_1] * E[i_1] + B[i_1-1] * B[i_1+1] * 3;
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 0; i_3 < PB_L; i_3++) {
                C[i_1-1][i_2+1][i_3] = G[i_2+1][i_3][i_1+1] * C[i_1-1][i_3][i_2-1] + 4;
            }
        }
        D[i_1] = F[i_1] + F[i_1] - E[i_1] - E[i_1-1] * A[i_1] - B[i_1] * D[i_1+1] + 3;
    }
#pragma endscop
...
kernel_4414231213_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
