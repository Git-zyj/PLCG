...
void kernel_2234231323_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_N-1 - 1; i_0 >= 1; i_0--) {
        A[i_0+1] = C[i_0] * D[i_0-1] + D[i_0+1] * C[i_0] + C[i_0] * A[i_0+2] * 5;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            A[i_2+1] = A[i_2+1] + A[i_2-1] - 5;
        }
    }
#pragma endscop
...
kernel_2234231323_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
