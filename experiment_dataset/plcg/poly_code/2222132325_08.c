...
void kernel_2222132325_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_1-1] = C[i_1][i_0] + B[i_1-1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_M-1; i_3++) {
            B[i_3] = C[i_3+i_2][i_2+2] + C[i_3+i_2+1][0] * C[i_3+i_2-1][i_2] - C[i_3+1][i_2] + B[i_3-2] * B[i_3+2] * 2;
        }
    }
#pragma endscop
...
kernel_2222132325_08(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
