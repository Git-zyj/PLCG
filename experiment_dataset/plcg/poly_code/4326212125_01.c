...
void kernel_4326212125_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_N-1, -i_2+PB_M); i_3++) {
            B[i_3+i_2][i_3] = D[i_2][i_3+1] + 1;
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            C[i_4][i_4] = C[i_4+2][i_4+2] * C[i_4-2][i_4-2] * 6;
        }
    }
#pragma endscop
...
kernel_4326212125_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
