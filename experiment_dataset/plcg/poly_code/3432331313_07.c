...
void kernel_3432331313_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            A[i_0][i_1] = E[i_0][0] + F[i_1][1] * A[i_0-1][i_1+1] + A[i_0+1][i_1] + A[i_1+1][i_0] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_N, -i_2+PB_M-1); i_3++) {
            for (int i_4 = 0; i_4 < PB_N; i_4++) {
                B[i_4+1][i_3][i_2] = G[i_4][i_2][i_3-1] - G[i_3][i_2-1][i_2-1] * G[i_3-1][i_3+i_2+1][i_4] * B[i_4+1][i_3+1][i_2] * B[i_4+1][i_3-1][i_2-1] - 1;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
        for (int i_6 = i_5; i_6 < PB_M-1; i_6++) {
            A[i_5+1][i_6+1] = H[i_6][i_5] - H[i_5][i_6] * 5;
        }
    }
    for (int i_7 = 0; i_7 < PB_L; i_7++) {
        for (int i_8 = 1; i_8 < PB_P; i_8++) {
            A[i_7][i_8-1] = H[i_8-1][i_7] + E[i_7][i_7] + H[i_7][i_8] - 4;
        }
    }
#pragma endscop
...
kernel_3432331313_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
