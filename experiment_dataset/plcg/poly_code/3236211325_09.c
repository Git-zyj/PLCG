...
void kernel_3236211325_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = A[i_0+1] - A[i_0-1] + A[i_0+2] * A[i_0-2] + A[i_0+1] * 4;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            for (int i_3 = max(1, i_2); i_3 < PB_N-2; i_3++) {
                B[i_1][i_2][i_1+1] = C[i_3-1][i_1][i_2] + C[i_1+1][i_3-1][0] - B[i_1][i_3+2][i_1+1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3236211325_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
