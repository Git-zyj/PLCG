...
void kernel_2424351215_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] - F[i_0+1] + E[i_0] + G[i_0-1][i_0+1] - F[i_0] + H[i_0] * A[i_0-1] - 5;
        A[i_0] = H[i_0] + E[i_0] - F[i_0] - F[i_0+1] + H[0] + A[i_0] * 3;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        C[i_1] = I[i_1][0] * F[i_1] * E[i_1] * H[i_1-1] + C[i_1-1] - C[i_1+1] * 2;
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        A[i_2-1] = E[i_2] * E[i_2-1] - E[1] * E[0] - 2;
    }
#pragma endscop
...
kernel_2424351215_01(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
