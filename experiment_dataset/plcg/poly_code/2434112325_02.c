...
void kernel_2434112325_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        A[i_0] = E[i_0+2] * A[i_0-2] + A[i_0-1] - 2;
    }
    ...
    for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            C[i_3+1][i_4][i_3] = F[i_4][i_3-2][i_3-1] - C[i_3][i_4][i_3-1] - C[i_3+3][i_4+1][i_3+2] - C[i_3-1][i_4][i_3-2] - 1;
        }
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            A[i_5-1] = G[i_3][i_5] - G[i_5][0] * 2;
        }
    }
#pragma endscop
...
kernel_2434112325_02(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
