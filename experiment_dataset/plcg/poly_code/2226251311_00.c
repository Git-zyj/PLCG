...
void kernel_2226251311_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P, -i_0+PB_N); i_1++) {
            A[i_0][i_1][i_0-1] = C[i_1+1][i_0] * D[i_1-1] + E[i_1-1][i_0] - E[i_1+i_0][i_1-1] + A[i_1+1][i_0-1][i_1] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            B[i_2][i_3] = D[i_3] + E[i_3][i_2-1] - E[i_2+1][i_3-1] - E[i_3][i_2] - E[i_3][i_2+1] + C[i_2][0] * A[i_2][i_3][i_2-1] + A[i_3][i_2][i_3-1] - A[i_2-1][i_3][i_2-2] - A[i_3][i_2+1][i_3-1] * B[i_2][i_3+1] - 3;
        }
    }
#pragma endscop
...
kernel_2226251311_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
