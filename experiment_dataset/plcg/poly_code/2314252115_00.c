...
void kernel_2314252115_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        A[i_0] = D[i_0] + D[i_0+1] * E[i_0+2] * D[i_0] * D[i_0] + 1;
        B[i_0+1] = D[i_0] + D[i_0] - D[i_0+1] * E[i_0] - B[i_0+2] * 6;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_L+2; i_2++) {
            C[i_2-2][i_1+2][i_2] = F[i_2][i_1+2] + G[i_2][i_1] + F[i_2-2][i_1] - F[i_2-1][i_1+1] + C[i_2-2][i_1+3][i_2] - 4;
        }
    }
#pragma endscop
...
kernel_2314252115_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
