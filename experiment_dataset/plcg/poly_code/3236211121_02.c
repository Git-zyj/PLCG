...
void kernel_3236211121_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        ...
        for (int i_2 = i_0; i_2 < min(PB_M-3, -i_0+PB_M-1); i_2++) {
            B[i_0][i_2+1] = C[i_2+i_0+1][i_0] + B[i_0+1][i_2+3] + 3;
        }
    }
#pragma endscop
...
kernel_3236211121_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
