...
void kernel_2234112313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N-2; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M; i_2++) {
            B[0] = C[i_2][i_1] - D[i_1+2] + B[0] + B[0] + 1;
        }
    }
#pragma endscop
...
kernel_2234112313_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
