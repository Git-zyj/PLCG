...
void kernel_2322112211_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0][i_0-2] = A[i_0-1][i_0-3] + A[i_0+1][i_0-1] * 6;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_M+2; i_2++) {
            B[0] = D[i_2+i_0-2][0] + B[0] + B[0] - B[0] * 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        C[i_3-1] = D[i_3][i_3] + C[i_3] * 6;
    }
#pragma endscop
...
kernel_2322112211_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
