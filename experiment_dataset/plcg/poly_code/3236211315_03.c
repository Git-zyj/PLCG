...
void kernel_3236211315_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            B[i_0-1][i_0+1] = C[i_1+1] - B[i_0-1][i_0+1] + 4;
        }
        for (int i_2 = 0; i_2 < PB_Q; i_2++) {
            B[i_0-1][i_0+1] = D[i_0-1][i_0] * B[i_0][i_0+2] * B[i_0-2][i_0] - 5;
        }
    }
#pragma endscop
...
kernel_3236211315_03(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
