...
void kernel_3332152221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_0+2][i_1-2] = D[i_0+1][i_1+2] * D[i_1][i_0] - E[i_1-2] + D[i_1][i_0] - D[i_1][i_0-1] - E[i_0] - A[i_0+3][i_1-2] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
            B[i_3][i_2] = D[i_2][i_3] * D[i_2][i_3-2] - D[i_2+2][i_3] - B[i_3+2][i_2-2] * B[i_3][i_2-2] * 6;
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 3; i_5 < i_4; i_5++) {
            C[i_5-1][i_4] = D[i_4][i_5] + E[0] + D[i_4-1][i_5-1] * E[i_5] + C[i_5-3][i_4] + C[i_5][i_4] + 4;
        }
    }
#pragma endscop
...
kernel_3332152221_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
