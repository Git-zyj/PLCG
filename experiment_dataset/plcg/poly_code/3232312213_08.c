...
void kernel_3232312213_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_1-1][i_0][i_0] = C[i_1][i_0+1] * 1;
        }
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            for (int i_3 = 1; i_3 < PB_M; i_3++) {
                B[i_3][i_2][i_2] = C[i_3][i_2+1] + B[i_3-1][i_0][i_0] + B[i_3][i_2-1][i_2-1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3232312213_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
