...
void kernel_3232331315_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_1] = C[i_1] - D[i_1][i_0] * E[i_0][i_1-1] - B[i_1] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = PB_M-1 - 1; i_3 >= 1; i_3--) {
            B[i_3] = E[i_3][i_2] * E[i_3][i_2] - E[i_3][i_2] - B[i_3-1] * B[i_3+1] - 6;
        }
    }
#pragma endscop
...
kernel_3232331315_04(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
