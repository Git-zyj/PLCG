...
void kernel_4314111313_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                A[i_1][i_0-1][i_2+1] = A[i_1-1][i_0-1][i_2+1] + A[i_2+1][i_0-1][i_1+2] - 6;
            }
        }
    ...
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
            for (int i_7 = 1; i_7 < -i_5+PB_M; i_7++) {
                C[i_6][i_5][i_7-1] = D[i_7+i_5][i_5] * C[i_6+1][i_5][i_7-1] - C[i_7+1][i_5+1][i_6-1] - C[i_6][i_5-1][i_7-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4314111313_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
