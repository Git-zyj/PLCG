...
void kernel_3224151111_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            A[i_1] = C[i_1+1] + D[i_1+i_0+1][i_1] + D[i_0-1][i_1] + D[i_1-1][i_0+1] * D[i_0][i_1] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = PB_L - 1; i_3 >= 0; i_3--) {
            for (int i_4 = 1; i_4 < min(PB_L, -i_2+PB_M); i_4++) {
                B[i_4][i_4] = D[i_4+1][i_4] * D[i_4+i_2][i_3] + E[i_3][i_2][i_4] - E[i_3+1][i_4-1][i_2+1] - B[i_4-1][i_4-1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3224151111_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
