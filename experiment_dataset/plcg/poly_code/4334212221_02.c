...
void kernel_4334212221_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[i_2-2][i_1+i_0][i_0+1] = D[i_1+1][i_0][0] + A[i_2-2][i_1+i_0-1][i_0-1] * 2;
            }
            for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
                B[i_0][i_3-1][i_1+i_0+2] = E[i_0][i_3][i_1-2] + C[i_1][i_3] * 1;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M+1; i_4++) {
        for (int i_5 = 0; i_5 < PB_N-2; i_5++) {
            for (int i_6 = max(1, i_5); i_6 < PB_M; i_6++) {
                C[i_5][i_6] = C[i_5+2][i_6] * C[i_5][i_4-1] * C[i_5][i_6-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4334212221_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
