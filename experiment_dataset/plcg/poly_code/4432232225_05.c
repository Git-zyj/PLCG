...
void kernel_4432232225_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = E[i_0] * E[i_0] * F[i_0+1] - F[i_0] - E[i_0] * F[i_0] * A[i_0+1] + 2;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_N, -i_1+PB_P); i_2++) {
            A[i_2-1] = G[i_2][i_1-1] + G[i_2][i_1] * H[i_1-1][i_2][i_2+i_1] - 5;
        }
        A[i_1-1] = E[i_1] + 6;
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < min(-i_3+PB_P, PB_M-4); i_4++) {
            for (int i_5 = 3; i_5 < -i_4+PB_M-4; i_5++) {
                D[i_5+i_4+2][i_5-1] = H[i_3][i_5-2][i_4] * H[i_5][i_4][i_4+i_3] + D[i_5+i_4+4][i_5-1] - D[i_5+i_4][i_5-3] + D[i_5+i_4-1][i_4-2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4432232225_05(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
