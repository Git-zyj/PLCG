...
void kernel_3214211223_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_N-1, -i_2+PB_L); i_3++) {
            B[i_3-1][i_2] = D[i_3+i_2][i_2] * B[i_3+1][i_2] + B[i_3-1][i_2+1] * 5;
        }
    }
#pragma endscop
...
kernel_3214211223_05(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
