...
void kernel_3224111121_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-4; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                A[i_1][i_2-1] = C[i_2][i_1+1][i_0] - 6;
            }
        }
        for (int i_3 = i_0; i_3 < -i_0+PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < -i_0+PB_L-4; i_4++) {
                B[i_4+i_0+1][i_3+i_0][i_4] = B[i_4+i_0+4][i_3+i_0+2][i_4+1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3224111121_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
