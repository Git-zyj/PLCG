...
void kernel_3322232313_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0+2] = D[i_0] + D[i_0] * D[i_0] + E[i_0] * D[i_0+1] + A[i_0+3] - A[i_0+1] + 6;
        A[i_0+2] = E[i_0] + A[i_0+2] * A[i_0+3] - A[i_0+1] * 6;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-2; i_2++) {
            for (int i_3 = 1; i_3 < PB_L; i_3++) {
                C[i_2+2][i_1][i_3] = F[0][i_2+2] * G[i_2][i_1-1][i_3] + C[i_2+2][i_1][i_3-1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3322232313_01(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
