...
void kernel_3332312311_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_P-3; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L+1; i_2++) {
                A[i_1][i_1] = A[i_1-1][i_1-1] * A[i_0-1][i_0-1] * A[i_2-1][i_2-1] * 1;
            }
            for (int i_3 = 2; i_3 < PB_P; i_3++) {
                B[i_1][i_3-1][i_1+2] = D[i_1-1][i_0][2] * B[i_1+1][i_3-1][i_1+3] - B[i_1-1][i_3-1][i_1+1] - B[i_1+1][i_3-2][i_1+3] * 3;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < -i_4+PB_M; i_5++) {
            for (int i_6 = 2; i_6 < PB_Q; i_6++) {
                C[i_5+i_4][i_5][i_6] = D[i_6-2][i_5][i_4-2] + A[i_6+1][i_6+1] + A[i_6][i_6] * A[i_5][i_5] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3332312311_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
