...
void kernel_3316111225_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P-2, i_0); i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, -i_1+PB_P-2); i_2++) {
                A[i_2][i_0][i_2+i_1+1] = A[i_2+1][i_0-1][i_2+i_1+2] + 5;
            }
        }
        for (int i_3 = 0; i_3 < min(i_0, -i_0+PB_P); i_3++) {
            for (int i_4 = 1; i_4 < PB_N-3; i_4++) {
                B[i_3+1][i_4-1] = D[i_0][i_4+1][i_3+i_0] * B[i_3][i_4-1] * B[i_3+1][i_4+1] - B[i_4+3][i_3] + 2;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_L; i_5++) {
        for (int i_6 = i_5; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 0; i_7 < PB_L; i_7++) {
                C[i_5][i_6][i_7] = D[i_5][i_7][i_6] + D[i_7][i_5][1] - C[i_7][i_5-2][i_6+1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3316111225_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
