...
void kernel_3312312223_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        ...
        for (int i_4 = 0; i_4 < PB_M; i_4++) {
            C[i_4][i_1] = D[i_4][i_1] * C[i_4][i_1+1] * 5;
        }
    }
#pragma endscop
...
kernel_3312312223_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
