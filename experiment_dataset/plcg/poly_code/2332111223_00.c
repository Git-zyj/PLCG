...
void kernel_2332111223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = B[i_0-1] + B[i_0-3] + B[i_0] - 2;
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            B[i_1-1] = A[i_1-1] + C[i_1+1][i_0+1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            C[i_2][i_3] = D[i_3-1][i_2+1] * C[i_2-1][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_2332111223_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
