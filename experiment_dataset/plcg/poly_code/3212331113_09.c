...
void kernel_3212331113_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            A[i_0][i_1] = C[i_0][0] * D[i_0-1][i_1-1][0] + A[i_0+1][i_1] - A[i_0][i_1+1] - 6;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_N; i_2++) {
            B[i_2+i_0][i_2] = E[i_0][i_2-1] + F[i_2-1] * E[i_2][i_0-1] + 4;
        }
    }
#pragma endscop
...
kernel_3212331113_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
