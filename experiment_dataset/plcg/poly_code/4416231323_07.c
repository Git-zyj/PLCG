...
void kernel_4416231323_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_1+1][i_2][i_0] = E[i_2][0][i_1] - E[i_2][i_0][i_1] - A[i_1-1][i_2][i_0] * A[i_1+3][i_2][i_0+1] * A[i_1+1][i_2+2][i_0+1] - 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_N-1, -i_3+PB_M); i_4++) {
            D[i_4] = F[i_4+i_3][i_3] * G[i_4+1][i_3] + H[i_4] + F[i_4][i_3-1] - H[i_4-1] * D[i_4] - 4;
        }
        for (int i_5 = 1; i_5 < PB_N-1; i_5++) {
            for (int i_6 = 0; i_6 < min(-i_3+PB_L, i_3); i_6++) {
                A[i_5+2][i_3][i_6] = E[i_5+1][i_6][i_6+i_3] - E[i_5-1][i_6][i_3] * 1;
            }
        }
        D[i_3] = I[i_3-1] - H[i_3] * H[i_3] - D[i_3-2] - 6;
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4416231323_07(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
