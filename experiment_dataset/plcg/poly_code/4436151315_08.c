...
void kernel_4436151315_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, i_0); i_1++) {
            A[i_0][i_1] = E[i_1] + F[i_1][i_0-1] - F[1][i_1+i_0] + F[i_0-1][i_1] + F[i_1-1][i_0] - G[i_1-1][i_0][i_1] + F[i_1-1][i_1] - A[i_1+1][i_0] + A[i_1-1][i_0] - 6;
        }
        for (int i_2 = 1; i_2 < min(-i_0+PB_M, i_0); i_2++) {
            A[i_2-1][i_0] = F[i_2+1][i_0] + F[i_2][i_0+1] - F[i_2+i_0][i_2] - F[i_0][i_2] - 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 0; i_5 < i_3; i_5++) {
                for (int i_6 = i_5; i_6 < min(-i_3+PB_Q+1, PB_M-1); i_6++) {
                    C[i_5][i_4][i_4-1][i_6+1] = H[i_3][i_6+1][i_5][i_4+1] * G[i_5][i_4][i_6+i_3-1] + C[i_5][i_4-1][i_4-2][i_6+2] + C[i_5][i_6+1][i_6][i_4+1] - C[i_5][i_4+1][i_4][i_6+1] * C[i_5+1][i_4][i_4-1][i_6+1] - 2;
                }
            }
        }
        for (int i_7 = i_3; i_7 < PB_M; i_7++) {
            for (int i_8 = i_7; i_8 < PB_M-1; i_8++) {
                for (int i_9 = 0; i_9 < min(min(i_3, -i_8+PB_L-1), -i_7+PB_Q-1); i_9++) {
                    C[i_7-1][i_8][i_8-1][i_9+1] = G[i_7][i_9+i_8+1][i_7-1] + H[i_3][i_9+1][i_7+1][i_8] * G[i_8][i_8][i_9] * H[i_8+1][i_9][i_9+i_7+1][i_3+1] - 6;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436151315_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
