...
void kernel_3426132223_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        A[i_0] = E[i_0-1] * E[i_0] + 2;
        for (int i_1 = max(3, i_0); i_1 < min(-i_0+PB_L, PB_P-2); i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                B[i_0-2][i_1][i_2] = F[i_2+1][i_1+i_0][0] + F[i_1+i_0][i_2][i_1+1] + B[i_0-2][i_1][i_2-1] + B[i_0-2][i_2][i_1+2] - B[i_1-3][i_2][i_0+1] + 4;
            }
            for (int i_3 = i_1; i_3 < -i_1+PB_L-2; i_3++) {
                C[i_0][i_3+i_1][i_1] = F[i_3+i_1][i_1+i_0][0] * F[i_3][i_1][i_0] - F[i_0][i_1][1] - C[i_0][i_3+i_1+2][i_3] + 1;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < min(-i_4+PB_N, PB_L); i_5++) {
            A[i_5] = G[i_5+i_4][i_5] * G[i_5+i_4][0] + A[i_5-2] + 6;
        }
    }
#pragma endscop
...
kernel_3426132223_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
