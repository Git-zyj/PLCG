...
void kernel_2432231223_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        B[i_0] = E[i_0+1] + E[i_0] * F[i_0+1] + E[i_0+1] - B[i_0] - 4;
        B[i_0] = E[i_0] - 5;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
            C[i_2+i_1][i_2+i_1] = G[i_2][0] + B[i_2] - 5;
        }
        for (int i_3 = 1; i_3 < min(-i_1+PB_M, PB_M-2); i_3++) {
            D[i_3] = G[i_3][i_1] - H[i_3+i_1][1] * F[i_3-1] - D[i_3-1] * D[i_3+2] + D[i_3+1] + 2;
        }
    }
#pragma endscop
...
kernel_2432231223_09(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
