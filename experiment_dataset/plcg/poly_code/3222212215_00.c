...
void kernel_3222212215_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                B[i_0+2][i_1][i_1+1] = C[i_1][i_0] * 6;
                B[i_0+1][i_1][i_1+1] = B[i_1][i_2][i_2+1] - B[i_0+2][i_1][i_1+1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3222212215_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
