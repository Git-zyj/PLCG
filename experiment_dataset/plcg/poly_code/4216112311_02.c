...
void kernel_4216112311_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < i_0; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < PB_N; i_2++) {
                A[i_0][i_1-2][i_2+1] = C[i_2][i_0][i_1] + C[i_1][i_2][i_0] * A[i_0][i_1-3][i_2+1] + A[i_0][i_1-2][i_2] - A[i_0-1][i_2-2][i_1+1] + 4;
            }
        }
    }
...
kernel_4216112311_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
