...
void kernel_4432252125_08(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-4; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_Q, PB_M); i_2++) {
                C[i_1+4] = E[i_2][i_2-1][i_1] * F[i_0][i_1][i_1+1] - F[i_2][i_0][i_1] * G[i_1][i_0+1] - E[i_1+2][i_0-2][i_2+i_0] * 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-3; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            C[i_3+2] = G[i_3+1][i_4-1] - H[i_3-1][i_4] - F[i_4][i_4][0] + I[i_3+1] * 2;
        }
        C[i_3+2] = J[i_3] - G[i_3][0] * J[i_3] - I[2] + J[2] * J[i_3] - C[i_3+3] * 3;
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = 0; i_6 < min(i_5, PB_M-2); i_6++) {
            for (int i_7 = 0; i_7 < PB_P; i_7++) {
                C[i_6+2] = G[i_5][i_6] * E[i_6][i_5][i_7] * E[i_5][i_7][i_6] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4432252125_08(xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
