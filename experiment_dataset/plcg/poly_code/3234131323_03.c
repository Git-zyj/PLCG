...
void kernel_3234131323_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L-2 - 1; i_0 >= 2; i_0--) {
        A[i_0] = C[i_0] + C[i_0] + C[i_0] + C[i_0-1] * A[i_0-2] + A[i_0+2] + 3;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        B[i_1+1] = C[i_1+1] + A[i_1] + A[i_1-2] * B[i_1+3] - 1;
    }
#pragma endscop
...
kernel_3234131323_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
