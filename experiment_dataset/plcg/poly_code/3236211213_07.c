...
void kernel_3236211213_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_1] = C[i_0][i_1] + A[i_0+1][i_1] * A[i_0][i_1+1] + 1;
        }
        for (int i_2 = i_0; i_2 < -i_0+PB_M+1; i_2++) {
            for (int i_3 = 1; i_3 < -i_0+PB_M; i_3++) {
                B[i_3+i_0][i_2][i_2-1] = B[i_2+i_0-1][i_3][i_3-1] * B[i_3+i_0-1][i_2][i_2-1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3236211213_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
