...
void kernel_4212212111_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_1][i_2-1] = A[i_2][i_0-2] * 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            A[i_4][i_3-2] = C[1] + C[i_4+2] - 3;
        }
    }
#pragma endscop
...
kernel_4212212111_01(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
