...
void kernel_2336352311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-2; i_0++) {
        A[i_0-2] = D[i_0] + D[i_0] * E[i_0] - E[i_0-1] - F[i_0+2][i_0-1] - E[i_0] + A[i_0-1] - A[i_0-3] - A[i_0-1] + 2;
    }
    for (int i_1 = 3; i_1 < PB_M-2; i_1++) {
        B[i_1][i_1] = E[0] * E[i_1+2] + G[i_1+1] - G[i_1] - H[i_1-2][i_1] + D[i_1] + A[i_1-2] * A[i_1-3] + 2;
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        B[i_2][i_2] = D[i_2] + D[i_2] * G[i_2] * B[i_2+1][i_2+1] * 6;
    }
#pragma endscop
...
kernel_2336352311_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
