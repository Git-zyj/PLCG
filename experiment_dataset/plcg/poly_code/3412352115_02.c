...
void kernel_3412352115_02(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            D[i_1] = E[i_1] + F[i_0+1][i_0+2] * G[i_0+1] * H[i_1][i_0+2] - G[i_1+2] * 4;
        }
        for (int i_2 = max(2, i_0); i_2 < PB_Q+2; i_2++) {
            for (int i_3 = 0; i_3 < min(PB_L-1, -i_2+PB_P); i_3++) {
                B[i_0][i_2][i_3+i_2] = I[i_3+1][i_2][i_0] * J[i_3][i_0+1][i_2-2] * H[i_2+2][i_2+2] - B[i_0-1][i_2][i_3+i_2] + 3;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < i_4; i_5++) {
            for (int i_6 = 2; i_6 < PB_Q; i_6++) {
                D[i_6] = I[i_4][i_5][i_6] * J[i_6-2][i_4+1][i_4] - F[i_4][i_5-2] - 2;
            }
        }
        for (int i_7 = 0; i_7 < PB_M-2; i_7++) {
            D[i_7] = F[i_4][i_7] * H[i_7][i_4+2] * H[i_7][i_4] * F[i_7+2][i_4] + H[i_7][i_4] + H[i_7][i_4] * K[i_4][0] * E[i_4+2] - 3;
        }
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3412352115_02(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K)));
