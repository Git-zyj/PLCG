...
void kernel_2236132113_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < min(-i_1+PB_L-1, PB_M-1); i_2++) {
            B[i_2][i_2] = C[i_2][i_1] + C[i_1][i_1] + C[i_2+i_1+1][i_2-2] - D[i_2] - C[i_1][i_2] * B[i_2+1][i_2+1] * 2;
        }
    }
#pragma endscop
...
kernel_2236132113_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
