...
void kernel_2212312221_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L - 1; i_0 >= 0; i_0--) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            A[i_1+1] = A[i_1+2] - A[i_1-1] * 2;
        }
    }
    for (int i_2 = 3; i_2 < PB_P+1; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_L+2, PB_M-2); i_3++) {
            B[i_3][i_2+1][i_2-1] = C[i_3+2][i_3+i_2-2] * B[i_3][i_2][i_2-2] - B[i_3][i_2-1][i_2-3] - 2;
        }
    }
#pragma endscop
...
kernel_2212312221_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
