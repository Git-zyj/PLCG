...
void kernel_2216131325_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-3; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_P-3); i_1++) {
            A[i_1][i_1+i_0] = C[i_0][i_1] * C[i_1][i_0+1] + D[i_1] + A[i_1+1][i_1+i_0+3] - A[i_1+1][i_1+i_0-1] * A[i_1][i_1+i_0+2] - 6;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_P+2, i_0); i_2++) {
            A[i_2][i_2+i_0-2] = D[i_2] + C[i_0][i_2+1] + D[i_0+1] * 6;
        }
    }
#pragma endscop
...
kernel_2216131325_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
