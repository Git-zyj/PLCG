...
void kernel_2412351123_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0-1] = E[i_0] + F[0] * F[i_0] * G[i_0][i_0] * E[i_0-1] - F[i_0-1] + E[i_0] * 3;
        C[i_0] = E[0] + H[i_0-1] * F[i_0] * E[i_0+1] * F[i_0] - H[i_0+1] * C[i_0] * C[i_0-2] + 3;
        C[i_0] = H[0] * 6;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = PB_M - 1; i_2 >= 0; i_2--) {
            C[i_2] = F[i_2] * G[i_2][i_1-1] + I[i_2][i_2] * G[i_1][i_2] * E[i_2] - G[i_2+1][i_1-1] - 5;
        }
    }
#pragma endscop
...
kernel_2412351123_06(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
