...
void kernel_3232312213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = C[i_0] + 3;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_Q+1, -i_0+PB_Q-1); i_2++) {
                B[i_1][i_2][i_2+i_0] = B[i_1][i_2][i_2+i_0+1] + B[i_1][i_2][i_2+i_0-1] - B[i_0][i_2-1][i_2+i_1-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3232312213_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
