...
void kernel_2412351315_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0-1] + E[i_0] - E[i_0] * F[i_0] * G[i_0] + H[i_0][i_0] * A[i_0-1] * A[i_0+1] * 6;
        A[i_0+1] = G[i_0-1] + F[1] + A[i_0] - 6;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            C[i_0+1][i_1+1][i_0-1] = I[i_1][i_1] + J[i_0][i_1] * J[i_1-1][i_0] + I[i_1][i_0] * H[i_1+1][i_0+1] + F[i_0] + A[i_1] + A[i_1+1] * D[i_1+1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        D[i_2+1] = G[i_2] - F[i_2] + I[i_2][i_2] * A[i_2] * D[i_2+2] + D[i_2] * 5;
    }
#pragma endscop
...
kernel_2412351315_05(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
