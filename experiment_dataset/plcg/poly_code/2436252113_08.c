...
void kernel_2436252113_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0-1] = E[i_0] * F[i_0+2] + F[i_0] + E[i_0-1] + G[i_0][0] - 2;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_M; i_2++) {
            A[i_2-1] = F[i_2-2] - H[i_2-2][i_1] - G[i_2][i_1-1] - 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-3; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_M; i_4++) {
            A[i_4-2] = G[i_4][i_3] - G[i_3][i_4] * G[i_3][2] * G[i_4][i_3] * H[i_3][i_4-2] * 2;
        }
        for (int i_5 = 0; i_5 < i_3; i_5++) {
            D[i_3+2][i_5] = F[i_3] * H[i_5][i_3] * I[i_5][i_5][i_3] * H[i_3][i_5] + D[i_5+2][i_3-1] + D[i_3+3][i_5] - 1;
        }
    }
#pragma endscop
...
kernel_2436252113_08(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
