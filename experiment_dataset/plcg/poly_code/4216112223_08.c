...
void kernel_4216112223_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_N; i_2++) {
                B[i_2+i_0][i_2] = C[i_1+2][i_2+2] - 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_N, i_3); i_4++) {
            B[i_4+i_3][i_3] = C[i_4+1][i_3] + B[i_4+i_3-2][i_3-2] * B[i_4+i_3-1][i_3-2] + 3;
        }
    }
#pragma endscop
...
kernel_4216112223_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
