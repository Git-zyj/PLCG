...
void kernel_4324251323_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M+2, PB_M-2); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M+1, -i_1+PB_M-2); i_2++) {
                A[i_2+i_1][i_0] = D[i_1][i_2][1] - E[i_2-1][i_1] + D[i_1][i_2][i_2-1] - D[i_1][i_2-1][i_0] * A[i_2+i_0-1][i_1+2] - A[i_2+i_1+2][i_0-2] * A[i_1+i_0-2][i_2+1] * A[i_2+i_0-2][i_1+1] + B[i_2+1] + 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_N; i_4++) {
            B[i_4] = F[i_4+1][i_3] + G[i_4] * F[i_4+i_3][0] * E[i_4][0] - E[i_4][i_3] + B[i_4+1] * 1;
        }
        for (int i_5 = 0; i_5 < i_3; i_5++) {
            for (int i_6 = 0; i_6 < min(min(-i_3+PB_M, -i_5+PB_L-1), PB_M-2); i_6++) {
                for (int i_7 = 1; i_7 < PB_M-2; i_7++) {
                    C[i_6][i_7][i_3] = H[i_5][i_7][i_3+1][i_6+1] + I[i_7][i_3][i_6][i_5] - H[i_7][i_3][0][i_6] + I[i_3-1][i_7-1][i_6+i_5+1][1] - A[i_6+i_3][i_7+1] - C[i_7+2][i_6+2][i_5] - 2;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4324251323_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
