...
void kernel_4332151311_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_Q, -i_0+PB_M+1); i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < min(min(-i_2+PB_L, -i_1+PB_M+1), i_1); i_3++) {
                    A[i_2][i_3+1][i_1+i_0-1] = D[i_3][i_3+i_2][i_0+1] - E[i_3-1][i_1+1][i_2-1][i_1] - D[i_3][i_3][i_2] * E[i_3-1][i_2-1][i_1][i_0+1] - A[i_2-1][i_0+1][i_3+i_1-1] - 3;
                }
            }
        }
    }
        ...
        for (int i_5 = i_4; i_5 < PB_Q; i_5++) {
            for (int i_6 = 0; i_6 < PB_Q-1; i_6++) {
                for (int i_7 = 1; i_7 < -i_5+PB_Q; i_7++) {
                    B[i_6][i_4-1][i_7][i_5] = E[i_4][i_6+1][i_5-1][i_7] + E[i_6][i_7][i_5][i_4] * E[i_6][i_4-1][i_7+i_5][i_7] + E[i_7][i_4-1][i_6+1][i_4] + B[i_6][i_4-1][i_7+1][i_5] * B[i_7][i_4-1][i_5-1][i_6] + B[i_6+1][i_4-1][i_7][i_5] * B[i_7][i_4-2][i_5][i_6] - B[i_6][i_7-1][i_4][i_5+1] - 4;
                }
            }
        }
    }
    for (int i_8 = 1; i_8 < PB_L-1; i_8++) {
        for (int i_9 = 1; i_9 < PB_L-1; i_9++) {
            C[i_9][i_8] = F[i_9+1] + F[i_9] - G[i_9-1][i_8+1] * F[i_9+1] - G[i_9][i_8] * G[i_9][i_9] + C[i_8][i_9+1] + C[i_9][i_8-1] + 5;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4332151311_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
