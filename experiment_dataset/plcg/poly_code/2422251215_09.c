...
void kernel_2422251215_09(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_P-1); i_1++) {
            D[i_0][i_1] = E[i_0][i_1+1] - F[i_1-1][i_0] - G[i_1][i_0][i_1+1] - E[i_1][i_0] * 1;
            D[i_1][i_0+1] = E[i_0][1] * F[i_0][i_1] - F[i_1+1][i_0] * F[i_1][i_0] + F[i_1+i_0][i_1] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        C[i_2] = H[i_2] * I[i_2] + I[i_2] + I[1] * I[1] - 5;
        for (int i_3 = 1; i_3 < PB_N; i_3++) {
            D[i_3][i_2] = F[i_3-1][i_2-1] * E[i_3+1][i_2+1] + H[i_2] + F[i_3][i_2+1] - D[i_3-1][i_2] - D[i_3][i_2-1] * D[i_3][i_2+1] * 3;
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2422251215_09(xd,yd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
