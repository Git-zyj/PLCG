...
void kernel_4222132221_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = C[i_0] * C[i_0-2] * C[i_0] - C[i_0-2] * C[i_0] - C[i_0+1] * A[i_0+1] - A[i_0+2] * A[i_0-1] * A[i_0-2] + 5;
        for (int i_1 = 0; i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < min(i_0, PB_L-2); i_2++) {
                B[i_0][i_1+i_0+1][i_2+2] = 3;
            }
        }
    }
#pragma endscop
...
kernel_4222132221_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
