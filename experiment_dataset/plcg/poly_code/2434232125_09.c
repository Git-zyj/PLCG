...
void kernel_2434232125_09(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    ...
    for (int i_1 = 4; i_1 < PB_M+2; i_1++) {
        B[i_1-2] = F[i_1] + F[i_1+1] * B[i_1-4] - 3;
    }
    for (int i_2 = 2; i_2 < PB_L+1; i_2++) {
        C[i_2+1] = F[i_2] * E[i_2-1] * F[i_2] - 3;
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            D[i_2-2][i_3] = F[i_2-1] + G[i_2-1][i_3] * H[i_3+1][i_2] * C[i_3+1] * 4;
        }
    }
#pragma endscop
...
kernel_2434232125_09(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
