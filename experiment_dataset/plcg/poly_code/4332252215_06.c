...
void kernel_4332252215_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q; i_2++) {
                for (int i_3 = 2; i_3 < min(min(-i_0+PB_M, PB_Q), -i_2+PB_P); i_3++) {
                    B[i_2][i_1] = D[i_1][i_2][i_3][1] - E[i_3][i_2][i_0][0] - E[i_1][i_3+i_0][i_0-1][i_3+i_2] - D[i_3-2][i_0+1][i_3+1][i_2] + D[i_3][i_2][i_1][i_0] * D[i_2][i_3][i_0+1][i_1-2] * 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
            for (int i_6 = 0; i_6 < min(i_4, -i_4+PB_L); i_6++) {
                B[i_5][i_6] = F[i_6+1][i_5][i_4] - G[i_4][i_5] + F[i_4+2][i_6][i_5] - H[i_5][i_6+i_4] + F[i_4][i_5+2][0] * F[i_6][i_4+1][i_5+1] + F[i_6][i_4-1][i_4-2] * B[i_5][i_6+1] * B[i_5+1][i_6] + 5;
            }
        }
    }
        ...
        for (int i_8 = 0; i_8 < PB_Q; i_8++) {
            for (int i_9 = 0; i_9 < PB_L; i_9++) {
                for (int j_0 = 0; j_0 < PB_L-2; j_0++) {
                    B[j_0][i_9] = D[i_7][i_9][j_0][i_8] * D[i_8+1][j_0+2][i_9][i_7] * 3;
                }
            }
        }
    }
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4332252215_06(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
