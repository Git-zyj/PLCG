...
void kernel_4212112211_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            for (int i_4 = 1; i_4 < min(min(-i_3+PB_L, -i_2+PB_Q-3), PB_M-1); i_4++) {
                B[i_4][i_3][i_4+i_2+2] = C[i_3-2][i_2][i_4] * C[i_4+i_3][i_2][i_3] * B[i_4-1][i_2][i_4+i_3+1] - B[i_4][i_3][i_4+i_2+3] * B[i_4+1][i_3][i_4+i_2+3] * 4;
            }
        }
    }
#pragma endscop
...
kernel_4212112211_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
