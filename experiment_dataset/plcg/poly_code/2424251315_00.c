...
void kernel_2424251315_00(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_N-1; i_1++) {
            C[i_1-1] = E[i_1+i_0] - F[i_0-1][i_1+1] * F[i_1+i_0+1][0] - G[i_0][i_0+1] * 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_N+1, PB_M-2); i_3++) {
            B[i_3+1] = F[i_2][i_3] + F[i_3+1][i_2] - F[i_3+i_2-1][0] + F[i_2][i_3] * G[i_3+1][i_2] + H[i_3] - B[i_3] + B[i_3+2] * C[i_3] * 3;
        }
        C[i_2] = E[i_2] + H[i_2] - E[i_2] * E[i_2] + H[i_2] * E[i_2] - B[i_2] - B[i_2+2] + 6;
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_M-1; i_5++) {
            D[i_4][i_5] = I[i_5+i_4+1][i_5+1][i_4+1] - G[i_5][i_4] - F[i_5][i_4] - I[i_5+i_4][i_5+1][i_4] * B[i_5] + D[i_4][i_5-1] + D[i_4+1][i_5+1] * 4;
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2424251315_00(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
