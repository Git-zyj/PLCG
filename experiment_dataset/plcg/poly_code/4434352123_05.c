...
void kernel_4434352123_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym)){
        ...
        for (int i_1 = 4; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
                    A[i_0][i_0][i_1][i_1] = E[i_2][i_3-1][i_1+1][0] + F[i_3+2][i_0-2][i_2][i_1] - E[i_1][i_2][i_3][i_2+1] * F[i_1][i_0][i_3][i_2] + E[i_1+1][i_0][i_3-2][i_2] - B[i_3][i_2][i_1-4] + 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_Q; i_4++) {
        for (int i_5 = 0; i_5 < -i_4+PB_Q; i_5++) {
            for (int i_6 = 1; i_6 < PB_M; i_6++) {
                B[i_6][i_5][i_4-2] = G[i_6-1][i_4][i_5+i_4] + B[i_6][i_5][i_4] * 5;
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_M-2; i_7++) {
        C[i_7-2] = H[i_7] * I[i_7] * I[i_7] + H[i_7+2] * H[i_7] * 3;
    }
    for (int i_8 = 0; i_8 < PB_L; i_8++) {
        for (int i_9 = 2; i_9 < min(-i_8+PB_L, i_8); i_9++) {
            C[i_9-2] = J[i_8] + K[i_9+i_8][i_9] + L[i_8][i_9] - M[i_9][i_8] - M[i_8+1][i_9] * G[i_9+2][i_8][i_8] - 4;
        }
    }
#pragma endscop
