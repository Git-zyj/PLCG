...
void kernel_4426351311_02(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-1; i_1++) {
            C[i_1+i_0][1] = E[i_1+i_0][i_0] * F[i_1][i_0] - G[i_1+i_0+1][i_1+1][0] + H[i_0+1][i_1][i_1+i_0] - I[i_0][i_1+1][i_0-1] - J[1] + K[i_1+1] + 1;
        }
        for (int i_2 = i_0; i_2 < -i_0+PB_M+1; i_2++) {
            B[i_0-1][i_2] = E[i_0-1][i_2-1] + K[i_0+1] + L[i_2][i_0-1] + L[i_2+i_0-1][i_0] - B[i_0-1][i_2-1] - B[i_0][i_2+1] + B[i_0-2][i_2] - 4;
        }
        for (int i_3 = 0; i_3 < -i_0+PB_M-2; i_3++) {
            C[i_3+i_0][1] = F[i_3][i_0] * L[i_0][i_3+1] - C[i_3+i_0-1][1] + C[i_3+i_0+2][1] + C[i_3+i_0+1][1] * C[i_3+i_0+1][1] + 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        D[i_4+1] = M[i_4] * K[i_4] - M[i_4] - K[i_4-1] - J[i_4] + M[i_4+1] - D[i_4+2] + 6;
    }
#pragma endscop
