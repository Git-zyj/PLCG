...
void kernel_2232112211_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_2][i_3] = C[i_3][i_2] * B[i_3][i_2+1] + B[i_2][i_3-1] + 2;
        }
    }
#pragma endscop
...
kernel_2232112211_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
