...
void kernel_4112311121_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 0; i_1 < PB_L-3; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_L-3; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_1+PB_P-4, i_2); i_3++) {
                    A[i_2-1][i_2+i_1+1][i_3+i_1] = B[i_3][i_1][i_2] * A[i_2-1][i_2+i_1+3][i_3+i_1+4] * 2;
                }
            }
        }
    }
...
kernel_4112311121_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
