...
void kernel_3212312125_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = PB_L+1 - 1; i_2 >= 2; i_2--) {
            for (int i_3 = 2; i_3 < PB_N; i_3++) {
                B[i_3][i_2-1] = C[i_1][i_3][0] - B[i_3-2][i_2-2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3212312125_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
