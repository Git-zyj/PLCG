...
void kernel_4226311323_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            A[i_1] = C[i_1][0] * A[i_1+2] + A[i_1+1] * A[i_1-2] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < min(min(-i_3+PB_L, -i_2+PB_L), i_2); i_4++) {
                B[i_2][i_4+i_3][i_4+i_2-1] = B[i_2][i_4+i_3-2][i_4+i_2-1] + B[i_3][i_4+i_2][i_4+i_3] - B[i_2+1][i_4+i_3-1][i_4+i_2] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4226311323_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
