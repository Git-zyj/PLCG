...
void kernel_4232151221_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        A[i_0][i_0-1] = C[i_0] * C[i_0] - C[i_0+1] * C[i_0] * C[i_0] * A[i_0-1][i_0-2] - B[0] * 5;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = -i_1+PB_L-1 - 1; i_2 >= 0; i_2--) {
            B[0] = D[i_2+i_1+1][i_2] + D[i_2][i_1-1] * D[i_1][i_2] - D[i_1][i_2] + D[i_2][i_1] - A[i_1][i_1-1] * B[0] + 3;
        }
    }
#pragma endscop
...
kernel_4232151221_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
