...
void kernel_2334351223_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0+1] - E[i_0] * F[i_0] - E[1] - D[i_0] - A[i_0+2] - 3;
        for (int i_1 = -i_0+PB_M - 1; i_1 >= 2; i_1--) {
            B[i_1][i_0] = G[i_1+i_0][0][i_0+1] - H[i_1][i_0] * F[1] * I[i_0][i_1+i_0-1] + J[i_1][i_0] * A[i_1-1] + B[i_1-2][i_0+2] + B[i_0][i_1-1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        A[i_2] = D[i_2-1] - E[i_2] + F[i_2+1] - D[i_2] * 3;
    }
#pragma endscop
...
kernel_2334351223_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
