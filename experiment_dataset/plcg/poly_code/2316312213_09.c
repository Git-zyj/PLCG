...
void kernel_2316312213_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_0][i_1] = D[i_1][i_0] * A[i_0][i_1+1] * B[i_0+1][i_1] * 4;
        }
    ...
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_M-1; i_4++) {
            C[i_3][i_4] = E[i_4-1] + C[i_3][i_4+1] + C[i_3+1][i_4-1] - 3;
        }
    }
#pragma endscop
...
kernel_2316312213_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
