...
void kernel_4234331115_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M; i_1++) {
            A[i_0+1][i_1-1] = C[i_1][i_0] + C[i_0][i_1-1] * C[i_1-1][i_0] * C[i_1][i_0] * A[i_0+1][i_1-2] * 5;
        }
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            for (int i_3 = 2; i_3 < i_0; i_3++) {
                A[i_0+2][i_3-2] = D[i_3][i_2][i_0+1] + E[i_3+1][i_0][i_2+1][i_0] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4234331115_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
