...
void kernel_2116132325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < min(min(i_0, -i_0+PB_N), PB_M-2); i_1++) {
            A[i_1] = B[0][i_1][i_0] + C[i_1] - B[i_1][i_1+i_0][1] * A[i_1+2] - A[i_1-2] * A[i_1+1] * 5;
        }
    }
#pragma endscop
...
kernel_2116132325_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
