...
void kernel_3124312311_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
    ...
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < min(min(i_0, -i_0+PB_L+1), PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < min(PB_M-1, -i_0+PB_L-1); i_2++) {
                A[i_1][i_2+i_0][i_0+2][2] = B[i_1+1][i_2] * A[i_2][i_1+i_0-1][i_1+2][2] * A[i_1][i_2+i_0+1][i_0+3][2] + A[i_2+1][i_1+i_0-1][i_0+1][2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3124312311_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
