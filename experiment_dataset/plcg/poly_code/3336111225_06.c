...
void kernel_3336111225_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_1][i_0-1] = D[i_1-1][i_0] + C[i_1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < i_2; i_4++) {
                B[i_3][i_4-1] = B[i_3-2][i_4-1] + 6;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
        C[i_5] = C[i_5+1] * C[i_5+2] + 4;
    }
#pragma endscop
...
kernel_3336111225_06(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
