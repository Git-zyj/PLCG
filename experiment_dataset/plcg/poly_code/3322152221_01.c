...
void kernel_3322152221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P+2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
                A[i_1][i_2] = D[i_2][i_0-2][i_1] * D[i_1][i_2+1][i_0] + D[i_1][i_2-2][i_0] - A[i_2][i_0+1] - A[i_2-2][i_1+2] - A[i_0][i_2-2] + A[i_0+1][i_2] + 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_P; i_4++) {
            for (int i_5 = 1; i_5 < min(-i_4+PB_M, i_3); i_5++) {
                B[i_5][i_3][i_4] = D[i_5][i_3][i_4] + D[i_5+i_4][i_3+1][0] * D[i_5+1][i_3][i_4] + B[i_5-1][i_3+1][i_4] * B[i_5][i_3+1][i_4-2] - 3;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_P; i_6++) {
        for (int i_7 = 2; i_7 < PB_P; i_7++) {
            for (int i_8 = 0; i_8 < PB_P; i_8++) {
                C[i_7][i_8+1][i_6] = D[i_7][i_8][i_6] - D[i_6][i_8][i_7] - D[i_8+1][i_6][i_7] * D[i_7][i_6][i_8] * D[i_6][i_7][i_8] + D[i_7-2][i_6][i_6+1] - D[i_6-2][i_8][i_7+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3322152221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
