...
void kernel_3322212221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
                C[i_3][i_2] = C[i_3+1][i_2+2] + C[i_2+1][i_3] - C[i_2][i_3+2] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3322212221_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
