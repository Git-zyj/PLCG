...
void kernel_4224312321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0][i_0] = C[i_0+1][i_0] * A[i_0+1][i_0+1] + A[i_0+2][i_0+2] + A[i_0-1][i_0-1] * 3;
    }
    for (int i_1 = 3; i_1 < PB_M+1; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_P-3, i_1); i_2++) {
            for (int i_3 = 0; i_3 < PB_P-2; i_3++) {
                B[i_1-1][i_2][i_3+2] = B[i_1-1][i_3+2][i_2+2] * B[i_1-1][i_3+1][i_2+3] + B[i_1-3][i_3][i_2+1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4224312321_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
