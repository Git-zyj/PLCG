...
void kernel_3212231321_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
                A[i_1][i_2][i_0+1] = C[i_0+1][i_1][i_2] + C[i_2][i_0][i_0+1] * C[i_1][i_0][i_2] + A[i_1+1][i_2-2][i_0+1] - A[i_1+2][i_2][i_0+1] - A[i_1][i_2][i_0+2] * A[i_1-1][i_2+1][i_0+2] - 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            B[i_4+1] = D[i_3+1][i_4+1] * E[i_4][i_3+1] * E[i_4+1][i_3] - B[i_4+2] * B[i_4] * 4;
        }
    }
#pragma endscop
...
kernel_3212231321_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
