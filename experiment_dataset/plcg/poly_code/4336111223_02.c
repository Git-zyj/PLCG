...
void kernel_4336111223_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 3; i_2 < -i_1+PB_N-1; i_2++) {
                A[i_1][i_2][i_2-1] = D[i_2+i_1+1][i_0][i_0] - D[i_0][i_2][i_1] + A[i_1][i_2-2][i_2-3] + 6;
            }
        }
    }
        ...
        for (int i_6 = i_3; i_6 < PB_M-2; i_6++) {
            for (int i_7 = i_6; i_7 < PB_L-2; i_7++) {
                for (int i_8 = 0; i_8 < PB_L-1; i_8++) {
                    C[i_3][i_6+1][i_7][i_8] = D[i_7][i_6][i_8] - C[i_3-1][i_6+1][i_7+2][i_8+2] * C[i_3][i_7+1][i_8+1][i_6-1] - C[i_3+1][i_6+2][i_7][i_8+2] - 5;
                }
            }
        }
    }
...
kernel_4336111223_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
