...
void kernel_4312132111_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < -i_2+PB_M; i_3++) {
                    A[i_0][i_1][i_2][i_2-1] = D[i_3+i_2][i_1+i_0][i_2][0] * D[i_0][i_3][i_1][i_2-2] * D[i_0][i_2][i_1-1][i_1] + A[i_0][i_1-1][i_2][i_2-1] * 3;
                }
            }
        }
        for (int i_4 = i_0; i_4 < PB_M; i_4++) {
            for (int i_5 = i_4; i_5 < -i_0+PB_L+2; i_5++) {
                for (int i_6 = 2; i_6 < PB_L-1; i_6++) {
                    B[i_0][i_4][i_5+i_0-2][i_6-2] = D[i_5][i_6+1][i_0][i_4] - D[i_6][i_0-2][i_4][i_5] - 6;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_L; i_7++) {
        for (int i_8 = 1; i_8 < -i_7+PB_P; i_8++) {
            for (int i_9 = 3; i_9 < PB_M-2; i_9++) {
                for (int j_0 = 1; j_0 < min(-i_8+PB_L, -i_7+PB_L+1); j_0++) {
                    B[i_8][i_7+1][j_0+i_8-1][i_9-3] = E[i_9][j_0+i_7-1][i_8] + D[i_9][i_7][i_8][j_0-1] + E[i_9+2][j_0+i_8][i_8+i_7] * 2;
                }
            }
        }
    }
...
kernel_4312132111_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
