...
void kernel_4314111215_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_N+1; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                for (int i_3 = 2; i_3 < -i_2+PB_L+1; i_3++) {
                    A[i_1-1][i_3-1][i_2][i_0] = D[i_0][i_3+i_2-1][i_1][i_0+1] + A[i_2-2][i_1-1][i_3][i_0+1] * A[i_1-2][i_3-2][i_2][i_0] - 4;
                }
            }
        }
    }
    for (int i_4 = PB_M-2 - 1; i_4 >= 0; i_4--) {
        B[i_4+1] = E[i_4] + B[i_4] - B[i_4+2] - 1;
        for (int i_5 = 0; i_5 < PB_M; i_5++) {
            B[i_4+1] = F[i_5][i_4][i_4] + 6;
        }
    }
#pragma endscop
...
kernel_4314111215_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
