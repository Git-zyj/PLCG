...
void kernel_3322152111_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_M-1); i_1++) {
            A[i_1][i_0] = D[i_1+i_0][i_1] + E[i_1] - D[i_0][i_1] - D[i_0][i_1-2] * E[i_1] - D[i_1][i_0] * A[i_1+1][i_0] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_M; i_3++) {
            for (int i_4 = 1; i_4 < -i_2+PB_Q; i_4++) {
                B[i_3+i_2][i_4] = D[i_3][0] * D[i_4][i_3-2] - F[i_3][i_2][i_3] + F[i_3][i_2][i_4-1] + D[i_4][i_3] * G[i_2-2][i_4][i_3][i_4+i_2] + 2;
            }
        }
        C[i_2][i_2-1] = E[i_2-2] * E[0] * E[i_2] * 1;
    }
#pragma endscop
...
kernel_3322152111_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
