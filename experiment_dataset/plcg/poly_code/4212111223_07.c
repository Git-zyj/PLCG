...
void kernel_4212111223_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_M, -i_2+PB_M-2); i_3++) {
                B[i_3+i_2][1] = B[i_3+i_1][1] + B[i_3+i_2+2][1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_4212111223_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
