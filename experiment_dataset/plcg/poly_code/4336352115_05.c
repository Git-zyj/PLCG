...
void kernel_4336352115_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0+2] * E[i_0-1] - E[i_0] - D[i_0] + 3;
    }
    for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            for (int i_3 = max(2, i_1); i_3 < min(-i_1+PB_P, -i_2+PB_M); i_3++) {
                for (int i_4 = 1; i_4 < PB_N; i_4++) {
                    B[i_4][i_3+i_2][i_2][i_1+2] = F[i_4][i_1][i_3-2][0] + G[i_1][i_3][i_4-1][0] - H[i_1][i_3][i_2][i_3] * H[i_2][i_1][i_4][i_3] - H[i_1+1][i_3+1][i_4][i_2-1] * G[i_4][i_2][i_1+2][i_3+i_1] * 1;
                }
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_N-2; i_5++) {
        for (int i_6 = i_5; i_6 < -i_5+PB_N-2; i_6++) {
            for (int i_7 = 3; i_7 < -i_6+PB_N; i_7++) {
                C[i_5-2][i_6-2][i_7+2] = I[i_6-2][i_5][i_7] + J[i_6+i_5+2][i_7][i_7] * I[i_7][i_5][i_6+2] * K[i_7+i_6][i_6][i_5] * C[i_6-2][i_7-3][i_5+2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4336352115_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
