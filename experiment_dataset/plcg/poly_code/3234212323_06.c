...
void kernel_3234212323_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < min(i_1, PB_L-2); i_2++) {
                A[i_0-2][i_1][i_2] = A[i_1-2][i_2+1][i_0] - A[i_0-2][i_1][i_2-1] + A[i_0-2][i_2+2][i_1] - 5;
            }
        }
    }
...
kernel_3234212323_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
