...
void kernel_2332112325_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L-2, PB_M-1); i_1++) {
            A[i_0+2][i_0] = D[i_1][i_1+i_0+2] - A[i_1+1][i_1-1] * 3;
        }
    }
    ...
    for (int i_3 = 2; i_3 < PB_N-2; i_3++) {
        for (int i_4 = 4; i_4 < PB_N; i_4++) {
            C[i_3][i_4-2] = C[i_3][i_4] - C[i_3-2][i_4-3] - C[i_3-1][i_4-1] + C[i_3+2][i_4-2] + C[i_3][i_4-4] * 5;
        }
    }
#pragma endscop
...
kernel_2332112325_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
