...
void kernel_3332212323_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_L-2); i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                A[i_0][i_1-2][i_2] = A[i_0][i_1][i_2] * A[i_0][i_1-2][i_2-2] + A[i_2-2][i_0-2][i_1+2] - 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < PB_P-3; i_4++) {
            B[i_3][i_4+2] = D[i_3][i_4] + B[i_3+1][i_4+3] - B[i_3-2][i_4+2] + 5;
        }
        for (int i_5 = 4; i_5 < PB_L; i_5++) {
            C[i_5-2] = D[i_5-2][i_3] + D[i_5][i_3] * C[i_5-1] + C[i_5-3] * C[i_5] + C[i_5-4] + 2;
        }
    }
#pragma endscop
...
kernel_3332212323_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
