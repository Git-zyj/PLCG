...
void kernel_3214111115_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(i_2, -i_2+PB_L-1); i_3++) {
            B[i_3+i_2-1][i_2] = C[i_3][i_2] * B[i_3+i_2+1][i_2+1] - 6;
        }
    }
#pragma endscop
...
kernel_3214111115_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
