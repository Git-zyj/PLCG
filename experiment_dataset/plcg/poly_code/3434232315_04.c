...
void kernel_3434232315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
    ...
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_L-1; i_2++) {
            for (int i_3 = 0; i_3 < min(PB_L, -i_1+PB_M-1); i_3++) {
                C[i_2+1][i_3] = F[i_3+i_1+1][i_3][i_2+1] + F[i_1][i_2-2][i_3] * 4;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < PB_P; i_5++) {
            C[i_4][i_5] = G[i_5][i_4-2] + G[i_5][i_4] * E[i_5-1] - C[i_4+1][i_5] * 5;
            C[i_4][i_5] = H[i_5][i_4] * G[i_5][i_4-1] * H[i_5][i_4-2] + I[i_5-1] + A[i_5] * A[i_5-1] - 5;
        }
    }
#pragma endscop
...
kernel_3434232315_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
