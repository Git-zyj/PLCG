...
void kernel_4326312313_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                A[i_1][i_2] = A[i_1-1][i_2] - A[i_2-1][i_1] + A[i_2][i_1+1] - 2;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
        C[i_3] = D[i_3+2] * C[i_3+1] * 1;
...
kernel_4326312313_06(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
