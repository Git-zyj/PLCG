...
void kernel_2434252315_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] * E[i_0+2] - F[i_0] + F[i_0-1] * A[i_0+1] * A[i_0-1] + 6;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 2; i_2 < min(PB_P, -i_1+PB_M-2); i_2++) {
            A[i_2-1] = G[i_2+i_1+2][0] * H[i_2-2][0] + G[i_1][i_2-2] * G[0][i_1-1] + H[i_2+1][i_1] * A[i_2+1] + 4;
        }
        for (int i_3 = i_1; i_3 < PB_M-1; i_3++) {
            A[i_3+1] = G[i_1-1][i_3] - H[i_3][i_1] + H[i_1-1][i_1] - H[i_3][i_1] * H[i_3][i_1] + 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < min(-i_4+PB_M, PB_M-2); i_5++) {
            A[i_4] = I[i_5+2][i_4][0] * E[i_5+i_4] - E[i_4-1] + E[i_5] * I[i_4][i_5][i_4] - G[i_4][i_5+i_4] * 5;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2434252315_06(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
