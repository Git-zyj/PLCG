...
void kernel_4226132325_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_L-1, i_1); i_2++) {
                B[i_2][i_1-1] = C[i_0][i_2][i_1-1] + C[i_1][i_2+1][i_0-2] * 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_M-2; i_4++) {
            for (int i_5 = 2; i_5 < min(-i_4+PB_M, PB_M-2); i_5++) {
                B[i_4][i_3] = C[i_5+i_4][i_3][i_4+2] * C[i_4][i_3][0] * C[i_5][i_3+2][i_4] - B[i_4+2][i_5-2] + B[i_5+2][i_4+1] + B[i_4-1][i_3] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4226132325_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
