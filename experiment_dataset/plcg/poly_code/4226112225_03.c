...
void kernel_4226112225_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_L+2; i_2++) {
            for (int i_3 = 0; i_3 < -i_1+PB_L; i_3++) {
                B[i_1][i_3+i_2] = B[i_2-1][i_3+i_1] + B[i_3][i_2+i_1-2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_4226112225_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
