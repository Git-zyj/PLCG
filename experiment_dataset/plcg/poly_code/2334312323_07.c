...
void kernel_2334312323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
        for (int i_3 = 3; i_3 < i_2; i_3++) {
            C[i_2][i_3-1] = A[i_3+2] - C[i_2][i_3-2] * C[i_3-2][i_2-1] * C[i_2][i_3-3] - 2;
        }
    }
#pragma endscop
...
kernel_2334312323_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
