...
void kernel_4314132315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_P; i_1++) {
        for (int i_2 = 3; i_2 < -i_1+PB_P; i_2++) {
            B[i_2+1][i_2+i_1-1] = D[i_2] + E[i_1-2][i_2] * A[i_2-3] + B[i_1+2][i_2+i_1] * B[i_2][i_2+i_1-2] * 3;
        }
        A[i_1-1] = D[i_1+2] + D[i_1] * D[i_1] - A[i_1-3] - 6;
    }
#pragma endscop
...
kernel_4314132315_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
