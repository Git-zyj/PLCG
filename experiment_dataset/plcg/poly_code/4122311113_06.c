...
void kernel_4122311113_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < min(PB_P, i_2); i_3++) {
                    A[i_0][i_3][i_2][i_2] = B[i_2-1][i_1+1][i_3] + A[i_0][i_1][i_3-1][i_3-1] - 2;
                }
            }
        }
    }
...
kernel_4122311113_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
