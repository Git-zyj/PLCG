...
void kernel_4222212111_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        ...
        for (int i_1 = 1; i_1 < PB_M-3; i_1++) {
            for (int i_2 = max(1, i_0); i_2 < PB_L; i_2++) {
                B[i_0+2][i_2][0] = C[i_1][i_2-1] - B[i_0+2][i_1-1][0] - B[i_1+3][i_2][0] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4222212111_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
