...
void kernel_4316212213_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
                A[i_0][i_2][i_1] = A[i_0-1][i_2+1][i_1] - A[i_0+1][i_2][i_1-1] + A[i_0+1][i_2-1][i_1] + 4;
            }
        }
        for (int i_3 = i_0; i_3 < PB_L+1; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
                for (int i_5 = 0; i_5 < i_0; i_5++) {
                    A[i_3-1][i_4][i_5] = D[i_4+2][i_0][i_5] - 4;
                }
            }
        }
    }
...
kernel_4316212213_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
