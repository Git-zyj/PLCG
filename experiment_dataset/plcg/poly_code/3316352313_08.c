...
void kernel_3316352313_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_N+1, PB_M-2); i_1++) {
            A[i_1+1][i_0] = D[i_1-1][i_0] + E[i_1] - F[i_1] + G[i_1+i_0-1][i_1+2] * H[i_0][i_1] + B[i_0] + B[i_1-1] - 3;
        }
        B[i_0] = F[i_0+2] * I[i_0] + I[i_0-1] - F[i_0+2] + B[i_0-1] + 1;
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M; i_3++) {
            B[i_3-1] = D[i_3+i_2][i_2] - D[i_3][i_2] + H[i_3-1][i_2-2] * E[i_3] - A[i_2+1][i_3] - A[i_3+1][i_2+1] - 1;
        }
    }
#pragma endscop
...
kernel_3316352313_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
