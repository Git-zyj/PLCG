...
void kernel_3422251115_09(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(min(-i_0+PB_L, i_0), PB_P-1); i_1++) {
            D[i_1-1] = E[i_1] - E[i_0] * 2;
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, -i_0+PB_P); i_2++) {
                D[i_2] = F[i_2][i_0][i_0] * F[i_2][i_1+1][i_0] + G[i_2][i_0] + F[i_1+1][i_0][i_2+i_0] + H[i_2+i_1][i_0][i_2] * G[i_2][i_1] * F[i_2][i_0][i_1] - F[i_2][i_0][i_1+1] * F[i_2][i_1+i_0][0] + 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        C[i_3] = E[i_3] - I[i_3] * D[i_3] * 3;
        D[i_3] = I[i_3] + E[i_3] + E[i_3-1] + E[i_3] + C[i_3] * 6;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3422251115_09(xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
