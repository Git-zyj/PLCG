...
void kernel_4214132223_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            A[i_0][i_1] = C[i_0] + D[i_1][i_0] * D[i_1+i_0][i_1] * 4;
        }
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_0+PB_Q, PB_P); i_3++) {
                B[i_0][i_2][i_3] = E[i_3][i_0][2] * E[i_3+2][i_2][0] * E[i_3+2][i_2][i_3+i_0] * B[i_0][i_2+2][i_3-1] + B[i_0+1][i_2][i_3] - B[i_0-1][i_2][i_3] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4214132223_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
