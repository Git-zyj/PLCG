...
void kernel_4432131215_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_0+1][i_1-1] = E[i_1] - F[i_1+1][i_1-1] + A[i_1+1][i_0-2] + A[i_0+2][i_1-1] - 3;
        }
    }
        ...
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
                for (int i_5 = 1; i_5 < PB_L; i_5++) {
                    A[i_4+1][i_5-1] = G[i_3][i_5][i_4] * H[i_4-1][i_2+1][i_5][i_2-1] - G[i_4][i_3][i_5] - 2;
                }
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
        for (int i_7 = 2; i_7 < PB_Q-2; i_7++) {
            for (int i_8 = 0; i_8 < min(-i_7+PB_L+2, -i_6+PB_L-1); i_8++) {
                for (int i_9 = 0; i_9 < -i_6+PB_L; i_9++) {
                    C[i_8+i_6-1][i_9][i_7+1][i_7] = H[i_8][i_6][i_9][i_7+1] * H[i_9+i_8][i_9][i_7][i_6-1] + H[i_7+1][i_8+i_6+1][i_7-1][i_8] - C[i_8+i_7-2][i_9][i_6+1][i_6] * C[i_9+i_6][i_8][i_7+2][i_7+1] + 6;
                }
            }
        }
        for (int j_0 = 0; j_0 < PB_L; j_0++) {
            for (int j_1 = 1; j_1 < PB_M; j_1++) {
                A[j_0][j_1-1] = G[j_0][j_1][i_6] + 3;
            }
        }
    }
#pragma endscop
...
kernel_4432131215_09(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
