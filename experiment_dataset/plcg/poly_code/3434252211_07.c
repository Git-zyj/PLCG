...
void kernel_3434252211_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        B[i_1] = F[i_1-2] - E[i_1] - F[i_1+2] + E[i_1+1] - F[i_1] - F[i_1+2] + B[i_1-1] - D[i_1+1] * 2;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_M+2); i_3++) {
            for (int i_4 = 2; i_4 < min(PB_Q, -i_2+PB_P); i_4++) {
                C[i_3+i_2-2][i_2][i_4] = G[i_4-1][i_3+2][i_2][i_3] + H[i_4][i_3] * I[i_4][i_2][i_3] - J[i_2][i_3][i_4] * I[i_4-2][i_3][i_4+i_2] * C[i_4+i_2-1][i_2][i_3-1] * 2;
            }
        }
        for (int i_5 = i_2; i_5 < min(PB_L+1, -i_2+PB_M); i_5++) {
            D[i_5] = K[i_5][i_2] - K[i_5+i_2][i_5-1] + K[i_5][i_2] - K[i_5][i_2] * H[i_5+1][i_2] * E[i_2-1] - D[i_5-1] - D[i_2+1] * 5;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3434252211_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K)));
