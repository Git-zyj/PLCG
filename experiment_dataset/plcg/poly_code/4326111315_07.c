...
void kernel_4326111315_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = D[i_0][i_1+1] + D[i_0-1][i_1-1] + A[i_1-1][i_0] + A[i_1][i_0+1] + A[i_1+1][i_0+1] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < PB_M; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_L, i_3); i_4++) {
                C[i_3][i_2+1][i_4] = A[i_3-1][i_4] + 4;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 0; i_6 < min(i_5, PB_L-2); i_6++) {
            for (int i_7 = 1; i_7 < PB_M; i_7++) {
                C[i_5][i_6+1][i_7] = E[i_7][i_6][i_5] - C[i_5+1][i_6+1][i_7] - C[i_5][i_6+2][i_7-1] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4326111315_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
