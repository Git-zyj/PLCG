...
void kernel_4434351215_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,int wk,DATA_TYPE POLYBENCH_5D(K,xK,yK,zK,vK,wK,xk,yk,zk,vk,wk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_M-1; i_2++) {
                A[i_1-1][i_2+i_0] = E[i_0-1][i_2+i_1][i_0] * F[i_2+i_1-1][i_2+i_0][i_0+1] - E[i_1][i_2+1][i_0] - E[i_2][i_2][i_0] - E[i_2][i_1][i_0] + A[i_2-1][i_1+i_0-1] * A[i_1][i_2+i_0+1] * A[i_2][i_1+i_0+1] + 2;
            }
        }
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            C[i_3][i_0] = G[i_3][i_0] - H[i_3] + 1;
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            for (int i_6 = 1; i_6 < PB_L; i_6++) {
                C[i_5][i_6+1] = I[i_4][i_6-1][i_5] + F[i_5][i_4+1][i_6] * I[i_6][i_5][i_4] * E[i_6][i_4][i_5-1] - F[i_5][i_6][i_4] + C[i_5-1][i_6+1] - 2;
            }
        }
        for (int i_7 = i_4; i_7 < min(PB_M, -i_4+PB_P+1); i_7++) {
            for (int i_8 = 1; i_8 < PB_L; i_8++) {
                for (int i_9 = 1; i_9 < i_7; i_9++) {
                    D[i_9][i_8][i_4][i_7] = J[i_8-1][i_9][i_7+i_4-1][i_4+1] - K[i_4][i_9-1][i_8+1][i_7-1][i_4] + L[i_4-1][i_8][i_9][i_9] * J[i_7][i_4+1][i_9][i_8] + M[i_8][i_9][i_4][i_7] + 1;
                }
            }
        }
    }
