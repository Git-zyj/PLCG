...
void kernel_3322111211_06(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            B[i_3+1][i_4] = B[i_3][i_4-1] - 4;
        }
    }
    for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 1; i_6 < i_5; i_6++) {
            for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
                C[i_7+1][i_6-1][i_5+1] = D[i_5][i_6][i_7-1] + D[i_7+1][i_5][i_6] - C[i_5+1][i_7][i_6] + C[i_7+1][i_6][i_5+2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3322111211_06(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
