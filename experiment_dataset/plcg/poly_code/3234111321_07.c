...
void kernel_3234111321_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 2; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            ...
            for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
                B[i_3-1][i_1] = C[i_1][i_0-1][i_0] * C[i_3+1][i_0-1][i_1] * A[i_0][i_1][i_3] + A[i_0-2][i_1][i_3+1] - B[i_1-1][i_3+2] + B[i_3][i_1] - B[i_3+1][i_1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3234111321_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
