...
void kernel_3326211323_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        ...
        for (int i_3 = 1; i_3 < min(PB_L-3, i_0); i_3++) {
            B[i_3+1][i_3+1] = D[i_3][i_0-1] + B[i_3+3][i_3+3] * B[i_3-1][i_3-1] + B[i_3+2][i_3+2] * B[i_3][i_3] - 4;
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        C[i_4+1] = C[i_4] * C[i_4-1] * 4;
    }
#pragma endscop
...
kernel_3326211323_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
