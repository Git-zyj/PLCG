...
void kernel_3424351111_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                A[i_1+i_0][i_2+i_1][i_0] = E[i_1-1][i_2-1][i_0][i_0+1] * F[i_0][i_2+1][i_0+1] - F[i_1][i_2+i_0][i_0] * G[i_2][i_0-1][i_1-1] + H[i_2][i_1][i_0] - I[i_2+i_1][i_1-1] - H[i_1][i_2][i_0] * J[i_2][i_1] * A[i_1+i_0+1][i_2+i_0][i_1] - 3;
            }
        }
        B[i_0] = K[i_0] - L[i_0] + K[i_0] * 4;
        for (int i_3 = 1; i_3 < i_0; i_3++) {
            C[i_3+1][i_0] = I[i_0][i_3] - I[i_3][i_0] - I[i_3][i_0] + 5;
            for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
                D[i_4][i_0][i_3][i_0+1] = G[i_3][i_4+1][i_0] + F[i_4][i_3][i_0+1] + F[i_4][i_0][i_3-1] + 6;
            }
        }
    }
#pragma endscop
