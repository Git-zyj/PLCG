...
void kernel_3414151113_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P; i_1++) {
            A[i_1] = E[i_1] * E[i_0] - F[i_1][i_0+1] - F[i_0][i_1] * A[i_1+1] - 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 1; i_4 < min(PB_Q+1, -i_2+PB_M); i_4++) {
                A[i_4] = G[i_2+1][i_4][i_3+1] + G[i_3][i_4+i_2][1] + H[i_3][i_2][i_4-1][i_4-1] + G[i_2][i_4+1][i_3] + G[i_3][i_2][i_2] - G[i_4][i_4][i_3-1] * G[i_4][i_2][i_3-1] * 4;
            }
        }
        C[i_2] = E[i_2+1] - E[i_2-1] * E[i_2] * E[i_2-1] * 2;
        for (int i_5 = 0; i_5 < i_2; i_5++) {
            for (int i_6 = 1; i_6 < i_5; i_6++) {
                A[i_6-1] = G[i_5][i_2+1][i_5] + G[i_2+1][i_6-1][0] - G[i_6][i_6+1][i_5] - G[i_5][i_2][i_6] * G[i_6+1][i_5+1][0] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3414151113_06(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
