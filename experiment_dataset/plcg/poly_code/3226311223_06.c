...
void kernel_3226311223_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_2 = 3; i_2 < PB_Q-1; i_2++) {
        for (int i_3 = 3; i_3 < PB_Q-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_2-1][i_2-1][i_3+1] = A[i_3-2] + B[i_3-3][i_3-3][i_2+1] - B[i_2-3][i_2-3][i_3+2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3226311223_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
