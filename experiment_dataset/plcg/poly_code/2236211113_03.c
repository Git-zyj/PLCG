...
void kernel_2236211113_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = PB_P - 1; i_2 >= 0; i_2--) {
        for (int i_3 = 0; i_3 < min(PB_P, -i_2+PB_N); i_3++) {
            B[i_3+i_2] = C[i_2][i_3] * C[i_3][i_2] + 4;
        }
    }
#pragma endscop
...
kernel_2236211113_03(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
