...
void kernel_3236112225_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            B[i_0+2][i_1+i_0] = C[i_0][i_1][0] - 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 3; i_3 < min(i_2, PB_L-1); i_3++) {
            for (int i_4 = max(1, -i_3+3); i_4 < -i_3+PB_L-1; i_4++) {
                B[i_4][i_4+i_3-2] = B[i_3+1][i_4+i_3+1] * B[i_4-1][i_4+i_3-3] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3236112225_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
