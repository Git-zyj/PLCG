...
void kernel_2226132225_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_P-2; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M; i_2++) {
            B[i_2][i_1-2] = D[i_1][i_1] - C[i_1] + A[i_1+2][i_1+2] - B[i_2-2][i_1-2] + 3;
        }
    }
#pragma endscop
...
kernel_2226132225_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
