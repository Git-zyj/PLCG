...
void kernel_3212212215_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+1; i_0++) {
        ...
        for (int i_3 = 1; i_3 < min(PB_L, -i_0+PB_Q-1); i_3++) {
            for (int i_4 = PB_L+1 - 1; i_4 >= 1; i_4--) {
                B[i_3][i_0-2][i_0+2][i_4] = C[i_3][i_4][i_0][i_3+i_0+1] - B[i_3][i_4-1][i_4+3][i_0] * B[i_3][i_0-2][i_0+2][i_4+1] + B[i_3-1][i_0-1][i_0+3][i_4] * B[i_3-1][i_0-2][i_0+2][i_4-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3212212215_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
