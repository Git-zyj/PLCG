...
void kernel_4326212323_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-3; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M-2; i_2++) {
                A[i_1+1][i_2+2] = B[i_2+i_1+2][i_2+1] - B[i_2+i_1][i_2] - 5;
            }
            B[i_1+i_0][i_1] = A[i_1+1][i_0+3] + A[i_1+1][i_0+2] * 1;
        }
    }
    for (int i_3 = 2; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            C[i_4][i_3] = D[i_4][i_3] * A[i_4+1][i_3+2] - C[i_4][i_3-2] + C[i_4-1][i_3] * 1;
        }
    }
#pragma endscop
...
kernel_4326212323_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
