...
void kernel_3424231323_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-3; i_0++) {
        A[i_0] = E[i_0] - E[i_0+1] * E[i_0] + 3;
        for (int i_1 = PB_N+1 - 1; i_1 >= i_0; i_1--) {
            B[i_0+1][i_1] = F[i_0][i_1] - F[i_1+1][i_0] - B[i_0+2][i_1] - B[i_0+3][i_1] - B[i_0][i_1] - B[i_0+1][i_1-1] * B[i_1-1][i_0+2] - 1;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M+1, PB_L-1); i_2++) {
            B[i_0+1][i_2+1] = E[i_0-1] - F[i_2+i_0-1][i_0] * A[i_2] + 5;
        }
        for (int i_3 = 1; i_3 < PB_N; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                D[i_0][i_4][i_3] = G[i_3][i_0][i_4-1] + H[i_4][i_3-1][i_3] - G[i_3][0][i_4] * H[i_4][i_0+1][i_3] + D[i_0][i_4][i_3+2] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3424231323_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
