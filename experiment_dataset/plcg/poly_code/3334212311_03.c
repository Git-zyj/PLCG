...
void kernel_3334212311_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(i_1, PB_M-1); i_2++) {
                A[i_2][i_0][i_1] = A[i_2][i_0+1][i_1] * A[i_0+1][i_2][i_1] + A[i_2][i_0+1][i_1-1] * A[i_2+1][i_0][i_1-1] + 2;
            }
        }
    }
    ...
    for (int i_4 = 0; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 1; i_5 < PB_L; i_5++) {
            for (int i_6 = 2; i_6 < min(i_5, PB_L-1); i_6++) {
                C[i_6-1][i_4+1][i_6] = C[i_6][i_4][i_6+1] + C[i_6][i_4+2][i_6+1] * C[i_5-1][i_4+2][i_5] * C[i_6-2][i_4+1][i_6-1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3334212311_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
