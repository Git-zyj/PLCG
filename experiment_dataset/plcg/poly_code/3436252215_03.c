...
void kernel_3436252215_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_L, -i_0+PB_P); i_1++) {
            A[i_0][i_1] = E[0] * F[i_1] * G[i_1-1][i_0][i_0+2] + H[i_1][i_0] - E[i_1-1] + H[i_0-1][i_0] * I[i_0+1][i_1][0] - G[i_1][i_1+i_0][i_0] * A[i_0+1][i_1] * A[i_1][i_0+1] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q+2; i_2++) {
        for (int i_3 = 2; i_3 < min(i_2, PB_L+1); i_3++) {
            for (int i_4 = i_3; i_4 < min(-i_3+PB_M+1, PB_M-2); i_4++) {
                B[i_4+1][i_2][i_3-1] = H[i_4+2][i_3+2] + J[i_4+i_3-1][i_3-2][i_2+1][i_2-2] + B[i_4+2][i_2-1][i_3-1] + 2;
            }
        }
        for (int i_5 = 0; i_5 < min(PB_L, i_2); i_5++) {
            C[i_2][i_5] = H[i_5][i_2] + K[i_5][i_2] * E[i_5] - H[i_5+2][i_2] * H[i_2][i_5] + H[i_5+1][i_2] * A[i_2-1][i_5] - 2;
        }
    }
    for (int i_6 = 1; i_6 < PB_M; i_6++) {
        for (int i_7 = 1; i_7 < min(PB_L-2, -i_6+PB_N); i_7++) {
            D[i_7+2][i_6] = K[i_6][i_7-1] * H[i_7][i_6] * K[i_7+i_6][i_6] + F[i_7] + A[i_6-1][i_7] - A[i_6][i_7-1] + A[i_6-1][i_7-1] * D[i_7+1][i_6] * 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3436252215_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K)));
