...
void kernel_2234312323_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < PB_N-2; i_1++) {
            A[i_1] = C[i_1] + A[i_1-2] + A[i_1+2] + A[i_1+1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_L-2; i_3++) {
            B[i_3+i_2][i_2-1] = B[i_3+i_2-1][i_3-2] * B[i_3+i_2+1][i_3-1] * B[i_3+i_2+2][i_2+1] + 1;
        }
    }
#pragma endscop
...
kernel_2234312323_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
