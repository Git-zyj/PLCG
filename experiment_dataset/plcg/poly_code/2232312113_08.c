...
void kernel_2232312113_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_N; i_1++) {
            A[i_1+i_0][i_1] = A[i_1+i_0-2][i_1-1] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N-1; i_3++) {
            A[i_3+i_2+1][i_3+1] = C[i_2] - 5;
        }
    }
#pragma endscop
...
kernel_2232312113_08(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
