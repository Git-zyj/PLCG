...
void kernel_2234212221_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
        for (int i_3 = 3; i_3 < PB_M; i_3++) {
            B[i_2][i_3-2] = C[i_3] - B[i_2-1][i_3-2] * B[i_2-2][i_3-1] - B[i_2][i_3-3] - B[i_2+2][i_3] * 5;
        }
    }
#pragma endscop
...
kernel_2234212221_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
