...
void kernel_4232132211_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_M-1; i_2++) {
                A[i_2-2][i_1][i_1+i_0] = C[i_1][i_2-1] + D[i_2][i_1+2][i_0] * D[i_1+2][i_2+i_0+1][i_0] - D[i_2][i_1][i_0] - A[i_2-2][i_1-1][i_1+i_0] * A[i_2-2][i_0+1][i_1+i_0+1] - A[i_2-2][i_1][i_1+i_0+1] - 4;
            }
        }
    }
        ...
        for (int i_4 = max(2, i_3); i_4 < PB_L; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q; i_5++) {
                for (int i_6 = 1; i_6 < PB_Q; i_6++) {
                    B[i_6][i_4][i_5][i_3+1] = E[i_4-2][i_5][i_3][i_6-1] + E[i_4][i_5][i_6-1][2] - 6;
                }
            }
        }
    }
...
kernel_4232132211_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
