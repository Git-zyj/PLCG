...
void kernel_2316111323_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-2; i_1++) {
            B[i_0+2][i_1+1] = D[i_0+1][i_0-1] + B[i_0][i_1+1] - B[i_0][i_1+2] * 2;
            B[i_0][i_1+1] = D[i_1][i_0-1] - 1;
        }
    }
    for (int i_2 = 3; i_2 < PB_M-1; i_2++) {
        C[i_2-1] = C[i_2-3] + C[i_2] - C[i_2+1] + 6;
    }
#pragma endscop
...
kernel_2316111323_08(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
