...
void kernel_2316152115_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_N); i_1++) {
            A[0] = D[i_1][i_0] + D[i_1+i_0][i_1] + E[i_0] - D[i_1][i_0] * D[i_1][i_0+1] + D[i_1-1][i_0] - D[i_0][i_1-1] - A[0] + A[0] + 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_M, -i_2+PB_N+2); i_3++) {
            C[i_2+2][i_2+2] = D[i_3+i_2-2][i_3] + D[i_3+1][i_2] + D[i_3][i_2] * 2;
        }
        C[i_2+2][i_2+2] = E[i_2] - E[2] + E[i_2] - E[i_2] + 3;
    }
#pragma endscop
...
kernel_2316152115_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
