...
void kernel_3234252221_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_P, -i_0+PB_L); i_1++) {
            for (int i_2 = 2; i_2 < min(i_0, -i_1+PB_L); i_2++) {
                A[i_1+i_0][i_2][i_0-2] = C[i_2+i_1][i_0][0] + D[i_2+i_1][i_1+1] * C[i_1-1][i_2][i_0] + E[i_1][i_0][i_2] + E[i_0][i_2-2][i_1] - C[i_2+i_1-2][i_0+2][i_0] * A[i_1+i_0][i_2-2][i_0-2] + A[i_1+i_0-1][i_2-1][i_0-2] - B[i_1+i_0][i_2+2][0] * 3;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_Q-2; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_M, PB_P); i_4++) {
            for (int i_5 = 0; i_5 < PB_P; i_5++) {
                B[i_4+i_3][i_5][0] = C[i_4][i_3-2][i_3+2] * D[i_5][i_4+1] * E[i_5][i_4][i_3-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3234252221_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
