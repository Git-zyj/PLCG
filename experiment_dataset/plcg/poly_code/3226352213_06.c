...
void kernel_3226352213_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = C[2] * C[i_0+1] - D[i_0] * D[i_0-2] * C[i_0] * A[i_0+1] - A[i_0-1] * 5;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            for (int i_3 = 2; i_3 < -i_2+PB_M; i_3++) {
                B[i_3][i_2-1][i_1] = E[i_3][i_1][i_2] + F[i_3][i_2][0] - F[i_3][i_1][i_2+2] - G[i_2][i_3-2][i_1] - G[i_3+i_2][i_1+1][i_2] - B[i_3+1][i_2-1][i_1] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3226352213_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
