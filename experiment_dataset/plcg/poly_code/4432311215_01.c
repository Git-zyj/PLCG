...
void kernel_4432311215_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_1][i_0][i_2][0] = A[i_1+1][i_0][i_2][0] - A[i_2+1][i_0+1][i_1][0] - 4;
            }
        }
        ...
        for (int i_5 = 0; i_5 < PB_P; i_5++) {
            for (int i_6 = 0; i_6 < PB_Q-1; i_6++) {
                for (int i_7 = 0; i_7 < PB_Q-2; i_7++) {
                    D[i_4][i_5][i_7+2][i_6+1] = 5;
                }
            }
        }
        for (int i_8 = 0; i_8 < min(PB_Q-1, -i_4+PB_Q); i_8++) {
            for (int i_9 = 0; i_9 < PB_P-1; i_9++) {
                for (int j_0 = 1; j_0 < -i_4+PB_L-1; j_0++) {
                    D[i_4][i_9][i_8+1][j_0] = E[j_0-1][i_4][i_9][i_8] + F[i_9][j_0+i_4+1][i_9+i_8][i_8] * G[i_9][j_0+1][i_8][i_9+1] + G[i_9][i_4][j_0][i_8+i_4] * D[i_4+1][j_0][i_8+1][i_9] * D[i_4][i_9+1][i_8+1][j_0-1] + 4;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4432311215_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
