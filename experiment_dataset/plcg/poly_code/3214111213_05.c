...
void kernel_3214111213_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
                A[i_1][i_2][i_2] = C[i_0][i_2][i_1] - A[i_1][i_2+1][i_2+1] * A[i_2][i_1+1][i_1+1] - 5;
            }
        }
        for (int i_3 = 0; i_3 < min(PB_P, -i_0+PB_L); i_3++) {
            A[i_0][i_3][i_3] = D[i_3+i_0][i_0] + 3;
        }
    }
#pragma endscop
...
kernel_3214111213_05(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
