...
void kernel_4234312315_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        for (int i_1 = 3; i_1 < PB_P; i_1++) {
            for (int i_2 = 2; i_2 < min(i_0, PB_L-1); i_2++) {
                A[i_2-2][i_1] = C[i_2+1][i_0+1] + A[i_1-3][i_2-1] * A[i_2-1][i_1] * 3;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            for (int i_4 = 0; i_4 < PB_P; i_4++) {
                for (int i_5 = 0; i_5 < i_4; i_5++) {
                    A[i_0-1][i_5] = A[i_0-1][i_4] * 1;
                }
            }
        }
    }
...
kernel_4234312315_01(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
