...
void kernel_3316331115_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] * 3;
        A[i_0-1] = D[i_0] - D[i_0] + E[i_0] - A[i_0+1] - 4;
    }
    for (int i_1 = 1; i_1 < PB_Q; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_P, -i_1+PB_L); i_2++) {
            for (int i_3 = max(1, i_2); i_3 < PB_P; i_3++) {
                C[i_1][i_2+1][i_2] = F[i_3][i_2+i_1] * G[i_1-1][i_3+i_2][i_3][1] * H[i_2][i_1][i_3-1] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3316331115_04(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
