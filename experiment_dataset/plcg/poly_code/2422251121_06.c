...
void kernel_2422251121_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            A[i_0][i_1-1] = E[i_0-1] - F[i_1][i_0] - G[i_0][i_0] * F[i_0][i_1+1] - H[i_1-1] - D[i_0][i_1+2] * 5;
        }
        for (int i_2 = 1; i_2 < min(-i_0+PB_M, PB_L+1); i_2++) {
            B[i_2+i_0][i_2-1] = E[i_2+i_0] - F[i_2+i_0][i_2-1] * F[i_2][i_0] * F[i_0-1][0] - G[i_2+i_0-1][0] + 4;
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M, PB_L+1); i_4++) {
            C[i_4-1][i_4] = F[i_4][i_4+i_3] + G[i_3][i_4+1] - I[i_4][i_3][i_4] + H[i_4] + 6;
        }
        for (int i_5 = max(1, i_3); i_5 < min(-i_3+PB_M, PB_P-1); i_5++) {
            D[i_3][i_5] = E[i_5] * G[i_3][i_5+1] + G[i_5+i_3][i_5-1] + 6;
        }
    }
#pragma endscop
...
kernel_2422251121_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
