...
void kernel_2422332125_06(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        C[i_0] = E[i_0+2] + F[i_0] + 5;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            B[i_2+1][i_1-2] = G[i_2+1][i_1] + H[i_2][i_1] - 5;
        }
        for (int i_3 = 0; i_3 < PB_N-1; i_3++) {
            C[i_3] = F[i_1] + F[i_1] - C[i_3+1] + 1;
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        D[i_4] = F[0] + I[i_4] - I[i_4+1] * 6;
    }
#pragma endscop
...
kernel_2422332125_06(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
