...
void kernel_3232212315_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
                A[i_1][i_2+1] = A[i_2+1][i_0] - A[i_1][i_2+2] * 3;
            }
        }
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            B[i_0+2][i_3] = C[i_3-1][i_3] + C[0][i_0] - B[i_0+1][i_3] + B[i_0+2][i_3+1] * B[i_0+3][i_3] - 1;
        }
    }
#pragma endscop
...
kernel_3232212315_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
