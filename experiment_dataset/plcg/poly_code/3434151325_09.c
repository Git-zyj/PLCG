...
void kernel_3434151325_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = E[i_0-1] - E[i_0] + E[i_0+1] * E[i_0] - E[i_0+1] * A[i_0+1] * 4;
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            A[i_1] = F[i_0][1] - F[i_1+1][i_0] * F[i_1][i_0] - C[i_0+1][i_1] * 2;
        }
        for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
            C[i_0][i_2] = F[i_2-1][i_0] + E[i_2] * F[i_0][i_2] * A[i_2] - A[i_0] * A[i_2+1] * C[i_0][i_2+2] * 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_Q; i_4++) {
            for (int i_5 = 1; i_5 < PB_Q; i_5++) {
                C[i_3+2][i_5+1] = G[i_5][i_3][i_4-1] + F[i_4-1][i_5] * H[i_4][i_5-1][i_3+1][0] - G[i_5+1][i_3+1][i_4] * G[i_4-1][1][i_3-1] + G[i_4+1][i_5][i_4] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3434151325_09(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
