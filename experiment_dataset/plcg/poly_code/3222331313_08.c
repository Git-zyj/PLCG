...
void kernel_3222331313_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L, -i_0+PB_P); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_M-1); i_2++) {
                B[i_1][i_2+1] = C[i_2+i_1][i_1][i_2] + D[i_1][i_2-1][i_1+i_0] + D[i_0][i_1+i_0][i_2+1] - D[i_0][i_2-1][i_1] + 6;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_M-1; i_4++) {
            B[i_3][i_4] = E[i_4+i_3-1][i_4+i_3+1] + B[i_3+1][i_4] * B[i_3-1][i_4] * B[i_3][i_4+1] - 2;
        }
    }
#pragma endscop
...
kernel_3222331313_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
