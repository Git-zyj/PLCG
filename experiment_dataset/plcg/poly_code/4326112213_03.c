...
void kernel_4326112213_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 3; i_2 < min(-i_0+PB_M, PB_L); i_2++) {
                A[i_0-1][i_2][i_1-2] = D[i_2+i_0][i_1] - A[i_1-1][i_0][i_2-3] + A[i_0-1][i_2][i_1-1] + 4;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 3; i_4 < i_3; i_4++) {
            B[i_3][i_4-2] = B[i_3-1][i_4-3] - 5;
        }
    }
    for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 0; i_6 < PB_Q-2; i_6++) {
            for (int i_7 = 0; i_7 < i_6; i_7++) {
                for (int i_8 = max(2, i_5); i_8 < PB_L+1; i_8++) {
                    C[i_5][i_5][i_7+2][i_8-1] = C[i_8-1][i_8-1][i_7+2][i_5] - C[i_5][i_5][i_7+2][i_8-2] * C[i_5+1][i_5+1][i_6+2][i_8-1] + 3;
                }
            }
        }
    }
...
kernel_4326112213_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
