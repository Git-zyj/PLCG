...
void kernel_2324212221_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M; i_1++) {
            A[i_1-1][i_0] = D[i_0][i_0] * A[i_0+1][i_1] + A[i_1][i_0] * 2;
        }
    }
    for (int i_2 = 3; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_P; i_3++) {
            B[i_2-1][i_2] = B[i_2-3][i_2-2] * B[i_2][i_2+1] * 6;
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < PB_M-1; i_5++) {
            C[i_5-1][i_5] = C[i_5+1][i_5+2] + C[i_5][i_5+1] + 6;
        }
    }
#pragma endscop
...
kernel_2324212221_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
