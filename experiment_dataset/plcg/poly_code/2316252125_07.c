...
void kernel_2316252125_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            B[i_1-2] = D[i_1][i_0] * E[i_1-1] + D[i_1+1][i_0] - F[i_1][i_0-1] - F[i_1][i_0] * D[i_0][i_1+1] - B[i_0] * 5;
        }
        B[i_0] = E[i_0+1] * E[i_0] - G[i_0-2] - 1;
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_2+1] = D[i_3][i_2] * G[1] * H[i_2][i_3][1] + F[i_2-2][i_2] + D[i_3][i_2] + F[i_3][i_2-2] + 6;
        }
    }
#pragma endscop
...
kernel_2316252125_07(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
