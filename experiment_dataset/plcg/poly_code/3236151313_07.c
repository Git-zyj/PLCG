...
void kernel_3236151313_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(-i_0+PB_L, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                A[i_2][i_1-1][i_2] = C[i_2][i_0][i_1] + D[i_0][i_0] - C[i_2+1][i_1][i_0] + D[i_1-1][i_2] * C[i_1][i_0][i_0] - D[i_1+1][1] * C[i_2][i_1+i_0][1] + A[i_2-1][i_1-1][i_2-1] - A[i_2][i_1][i_2] * A[i_2][i_1-2][i_2] + 4;
            }
        }
        for (int i_3 = i_0; i_3 < PB_M-1; i_3++) {
            A[i_3-1][i_0][i_3-1] = D[i_3][i_0] + D[i_3+1][i_0-1] - D[i_3][i_0] + 4;
        }
    }
#pragma endscop
...
kernel_3236151313_07(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
