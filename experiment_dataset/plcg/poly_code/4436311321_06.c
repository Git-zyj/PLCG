...
void kernel_4436311321_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < min(i_0, PB_M-2); i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < min(-i_2+PB_L, PB_L-1); i_3++) {
                    A[i_1][i_3+1][1][i_3+1] = E[i_2][i_1][i_3-1][0] + F[i_1][i_3+i_2][i_0+1][i_2+1][i_1+1] - A[i_1][i_2][1][i_2] + A[i_1+2][i_3+1][1][i_3+1] + 5;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        for (int i_5 = max(i_4, 2); i_5 < PB_M; i_5++) {
            B[i_4+1][i_5] = G[i_5][i_4] * B[i_4+1][i_5-1] * B[i_4+1][i_5+1] * B[i_4+1][i_5-2] - 1;
        }
    }
    for (int i_6 = 2; i_6 < PB_N-1; i_6++) {
        for (int i_7 = 1; i_7 < i_6; i_7++) {
            C[i_6+1][i_7-1] = G[i_7-1][i_6-1] * C[i_6-1][i_7+1] - C[i_6][i_7-1] - 4;
        }
        for (int i_8 = 2; i_8 < i_6; i_8++) {
            D[i_6+1][i_8] = D[i_6+1][i_8-2] * D[i_8+1][i_6-2] + 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4436311321_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
