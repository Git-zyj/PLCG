...
void kernel_2312352213_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1][i_0+2] = D[i_1][i_1+i_0] + E[i_1] + F[i_1+i_0+2][i_0-2] + D[i_1][i_0] * F[i_1][i_0+2] + A[i_1-1][i_0+2] * A[i_1][i_0+1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        B[i_2][0] = E[i_2+2] * G[i_2] * E[i_2] - H[i_2] + E[i_2] + B[i_2-1][0] - 3;
        for (int i_3 = 1; i_3 < -i_2+PB_L; i_3++) {
            C[i_2][i_3+i_2] = G[i_3] + D[i_2][i_3-1] + F[i_3][i_2] - F[i_2+1][i_3] * F[i_2+1][i_3] * B[i_3-1][0] * C[i_2][i_3+i_2-1] * 1;
        }
    }
#pragma endscop
...
kernel_2312352213_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
