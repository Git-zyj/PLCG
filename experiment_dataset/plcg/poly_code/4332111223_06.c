...
void kernel_4332111223_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 3; i_2 < PB_M; i_2++) {
                A[i_2][i_2-1][i_0-1] = A[i_1+1][i_1][i_0-3] - A[i_2-2][i_2-3][i_0] + 3;
            }
        }
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            B[i_0-1][i_3] = D[i_0][i_3] * B[i_0-3][i_3] * 2;
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < PB_L-2; i_5++) {
            C[i_5] = D[i_5-1][1] * D[i_4-1][i_5] * C[i_5+2] * C[i_5-1] + C[i_5+1] + 6;
        }
    }
#pragma endscop
...
kernel_4332111223_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
