...
void kernel_3234152323_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            B[i_1+i_0][0] = C[i_1][i_0] * C[i_0][i_1] - C[i_1][i_0+1] - D[i_1] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 2; i_3 < min(-i_2+PB_P+1, PB_M-3); i_3++) {
            for (int i_4 = 1; i_4 < min(min(-i_3+PB_M-3, -i_2+PB_L-2), i_2); i_4++) {
                B[i_4+i_3+1][0] = E[i_4+1][i_2][i_3+1] * E[i_3-2][i_4-1][i_2-1] - E[i_2-1][i_4+i_3][0] * E[i_4+i_2+1][i_3-2][i_3-2] - E[i_3][i_4+i_2+2][i_3-2] + E[i_3][i_4+i_2][i_3+i_2-1] + B[i_4+i_2-2][0] - B[i_4+i_3][0] + B[i_4+i_3+3][0] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3234152323_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
