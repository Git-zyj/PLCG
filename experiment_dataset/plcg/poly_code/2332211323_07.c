...
void kernel_2332211323_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0+1][i_1+1] = A[i_0][i_1] - A[i_0+3][i_1+1] * A[i_0-1][i_1+2] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-3; i_2++) {
        ...
        for (int i_4 = 0; i_4 < min(-i_2+PB_L, PB_M); i_4++) {
            C[i_2+1][i_2] = D[i_4][i_4+i_2] - C[i_2+3][i_2+2] - C[i_2][i_2-1] - 3;
        }
    }
#pragma endscop
...
kernel_2332211323_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
