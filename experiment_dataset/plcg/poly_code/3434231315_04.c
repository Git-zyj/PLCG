...
void kernel_3434231315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0+1] + F[i_0] * E[i_0] + E[i_0] * F[i_0] * A[i_0-1] - 6;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            B[i_0][i_1] = E[0] + A[i_1+1] * A[i_1] - A[i_1-1] * B[i_0][i_1+1] + B[i_1][i_0+1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            A[i_3] = G[i_3-1][i_2] + H[i_2][i_3][i_2] + 4;
        }
        A[i_2-1] = F[i_2] * E[i_2] - F[i_2] + 1;
    }
#pragma endscop
...
kernel_3434231315_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
