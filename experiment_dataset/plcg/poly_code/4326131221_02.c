...
void kernel_4326131221_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            A[i_0+1][i_1-1] = A[i_0][i_1-1] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N; i_3++) {
            A[i_3][i_2-1] = D[i_3+i_2][0] * D[i_2][i_2] * D[i_3][i_2] - D[i_2][i_3] - D[i_3][i_2] - 5;
        }
        for (int i_4 = 1; i_4 < min(i_2, -i_2+PB_P); i_4++) {
            for (int i_5 = 0; i_5 < min(-i_4+PB_L-1, -i_2+PB_L-2); i_5++) {
                C[i_4-1][i_5+i_2][i_2] = E[i_4][i_2][i_5] - E[i_5][i_2][i_4+i_2] + E[i_5+i_2][i_5+i_4+1][i_4] - C[i_4-1][i_5+i_2+2][i_2] * C[i_4-1][i_5+i_2+2][i_2+2] - C[i_4][i_5+i_2-2][i_2] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4326131221_02(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
