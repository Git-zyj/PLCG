...
void kernel_4336251115_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        A[i_0-1] = D[i_0] * E[i_0] * D[i_0] * E[i_0] - D[i_0] - D[i_0] * A[i_0] + 5;
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_Q-1); i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                B[i_1][i_2] = F[i_2][i_1][0] - F[i_1-1][i_2][i_1+i_0+1] + F[i_1][i_2][i_0] - G[1][i_1] - H[i_1][i_0][i_2] + F[i_2][i_2][i_0-1] + B[i_2][i_1-1] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_M-1, i_3); i_4++) {
            A[i_4-1] = G[i_4+1][i_3] - E[i_4-1] + G[i_4][i_3] + 5;
        }
    }
#pragma endscop
...
kernel_4336251115_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
