...
void kernel_4236212113_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[i_0][i_1-2] = C[i_0+2][i_1][i_0] * D[i_0][i_1] * A[i_0+1][i_1-2] * 4;
        }
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            for (int i_3 = i_2; i_3 < PB_Q; i_3++) {
                for (int i_4 = 0; i_4 < -i_3+PB_Q; i_4++) {
                    B[i_0][i_2][i_4+i_3][i_0+2] = 3;
                }
            }
        }
    }
...
kernel_4236212113_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
