...
void kernel_3232112223_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = min(PB_L, -i_0+PB_Q-1) - 1; i_1 >= 0; i_1--) {
            A[1][i_1+2][i_1+i_0] = C[i_0][i_1][i_0] - A[1][i_1+2][i_1+i_0-1] + A[1][i_1+3][i_1+i_0+1] * B[i_1] - 1;
        }
        B[i_0] = B[i_0-2] - 6;
    }
#pragma endscop
...
kernel_3232112223_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
