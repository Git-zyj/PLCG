...
void kernel_2214151111_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        B[i_0][i_0] = C[i_0-1][0] + D[i_0-1] - 5;
    }
    for (int i_1 = 1; i_1 < PB_N; i_1++) {
        for (int i_2 = i_1; i_2 < PB_M-1; i_2++) {
            B[i_2][i_2] = C[i_2][i_1+1] * D[i_1] * C[i_2][i_1+1] - C[i_2+1][i_1] + C[i_1][i_1-1] + C[i_2][i_1] * C[i_1][i_2] - C[i_2][i_1] * B[i_2-1][i_2-1] + 1;
        }
    }
#pragma endscop
...
kernel_2214151111_05(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
