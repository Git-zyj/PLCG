...
void kernel_3322232313_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        A[i_0-2] = D[i_0] - E[i_0-1] - A[i_0-1] * A[i_0-3] * A[i_0-1] + 5;
    }
    for (int i_1 = 2; i_1 < PB_L+2; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            B[i_1-1][i_2] = D[i_2] * F[i_2-2][i_1] + F[i_2][i_1] * G[i_1-2][0] + A[i_1-2] * A[i_2-2] * B[i_1][i_2] + 1;
        }
        for (int i_3 = 0; i_3 < i_1; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                B[i_4-1][i_3+1] = H[i_1-2][i_4][i_3] + F[i_4][i_1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3322232313_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
