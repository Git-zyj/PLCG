...
void kernel_4334132321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_1+i_0][i_2][i_0-1] = D[i_2][i_0][i_1] + A[i_1+i_0+1][i_2+2][i_0-1] * 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_M-2, i_3); i_4++) {
            for (int i_5 = max(max(2, -i_4+2), i_3); i_5 < min(PB_M+1, -i_4+PB_L+2); i_5++) {
                B[i_4][i_4+2][0] = D[i_4][i_5-2][i_3] - E[i_3][i_5+i_4-2] - B[i_4-1][i_4+1][0] * B[i_4+2][i_4+4][0] - B[i_5-1][i_5+1][0] * B[i_4+1][i_4+3][0] - 3;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_M; i_6++) {
        for (int i_7 = 2; i_7 < PB_L; i_7++) {
            for (int i_8 = 0; i_8 < min(min(PB_M, -i_6+PB_N), -i_7+PB_N+2); i_8++) {
                C[i_6][i_8][i_8] = D[i_6][i_8][i_7] * D[i_8+i_7-2][i_8+i_6][i_7] - D[i_6+1][i_7][i_8] * D[i_7][i_8][i_6+2] + E[i_8][i_7] + C[i_6-1][i_8+1][i_8+1] * C[i_6-1][i_8][i_8] * C[i_6][i_8+1][i_8+1] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4334132321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
