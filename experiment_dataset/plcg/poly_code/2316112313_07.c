...
void kernel_2316112313_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        A[i_1] = D[i_1+2] * D[i_1] * 1;
        for (int i_2 = 0; i_2 < min(-i_1+PB_L, PB_L-2); i_2++) {
            C[i_1][i_2+2] = D[i_2+i_1] * A[i_2] * C[i_1][i_2+1] + C[i_1-1][i_2+1] + C[i_1-1][i_2+2] + C[i_1+1][i_2+1] * 6;
        }
    }
#pragma endscop
...
kernel_2316112313_07(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
