...
void kernel_2226212113_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0][i_0+2] = A[i_0+1][i_0+3] + 2;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        B[i_1-2] = C[i_1] + C[i_1-1] + B[i_1-1] * 5;
    }
#pragma endscop
...
kernel_2226212113_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
