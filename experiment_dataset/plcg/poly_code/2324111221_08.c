...
void kernel_2324111221_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-1, PB_L); i_1++) {
            B[i_1+i_0+1][i_1] = D[i_0][i_1] + D[i_0][i_1] + 2;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_M-2, PB_L); i_2++) {
            B[i_2+i_0+1][i_2] = B[i_2+i_0+2][i_2] - 3;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-3; i_3++) {
        C[i_3+1] = C[i_3+2] + C[i_3+3] + C[i_3] + 6;
    }
#pragma endscop
...
kernel_2324111221_08(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
