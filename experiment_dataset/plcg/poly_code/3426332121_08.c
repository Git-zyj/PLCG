...
void kernel_3426332121_08(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_P-1; i_1++) {
        for (int i_2 = max(4, i_1); i_2 < -i_1+PB_Q; i_2++) {
            for (int i_3 = i_1; i_3 < min(-i_1+PB_P+1, -i_2+PB_L+2); i_3++) {
                B[i_2-2][i_3+i_1-1][2] = F[i_3+i_2-2][i_1][i_2+i_1] + G[i_2+2][i_3-1][i_1+1] - B[i_2-4][i_3+i_1-1][2] + B[i_2-2][i_3+i_1-3][2] * 3;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        ...
        for (int i_5 = 1; i_5 < min(i_4, -i_4+PB_L+1); i_5++) {
            D[i_4][i_4] = H[i_5][i_4-1] + H[i_5-1][i_4] - E[i_4+1] - H[i_5][i_4] - H[i_4-1][i_5] + H[i_4+1][i_5+i_4-1] * 2;
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3426332121_08(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
