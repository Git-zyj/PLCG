...
void kernel_3232112125_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
                B[i_1-1][i_2-1][i_1-1] = C[i_1][i_2+2][i_1] + C[i_2][i_0][i_1] * 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_Q; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            for (int i_5 = 3; i_5 < PB_P+1; i_5++) {
                B[i_4][i_5-1][i_4] = B[i_4][i_5-3][i_4] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3232112125_04(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
