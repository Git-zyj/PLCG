...
void kernel_4236212125_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q, PB_L-2); i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_1+2][i_2+1][i_1+i_0] = C[i_2][i_1][i_0] + A[i_1+2][i_2-1][i_1+i_0-1] + 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M; i_4++) {
            for (int i_5 = 0; i_5 < min(PB_L-2, -i_4+PB_Q); i_5++) {
                A[i_5+2][i_3+1][i_5+i_4] = D[i_4][i_5][i_3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4236212125_04(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
