...
void kernel_2314132221_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-1, PB_L-1); i_1++) {
            A[i_1+i_0][i_1+1] = D[i_0][i_1] - E[i_0-2] + D[i_0][i_1] + D[i_1][i_0] * D[i_1][i_0] * A[i_1+i_0+1][i_1+1] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_P; i_3++) {
            B[i_2][i_3] = E[i_2-1] * D[i_2-1][i_3+1] * D[i_2][i_3] + C[i_3] * C[i_3+1] + C[i_3-1] + 6;
        }
        C[i_2] = E[i_2] + 5;
    }
#pragma endscop
...
kernel_2314132221_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
