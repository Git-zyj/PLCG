...
void kernel_4222212111_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 3; i_2 < PB_P+2; i_2++) {
                for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
                    B[i_1][i_2-2][i_3+2] = B[i_3][i_2-3][i_0+2] + 2;
                }
            }
        }
    }
...
kernel_4222212111_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
