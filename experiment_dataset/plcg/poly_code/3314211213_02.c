...
void kernel_3314211213_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0+1][i_0] = D[i_0] + A[i_0+2][i_0+1] + 5;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < min(i_1, PB_L-2); i_2++) {
            B[i_1][i_2+1] = A[i_2+1][i_2] * B[i_1][i_2+2] * B[i_1][i_2] * 4;
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            C[i_1][i_3] = C[i_1+1][i_3] + 4;
        }
    }
#pragma endscop
...
kernel_3314211213_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
