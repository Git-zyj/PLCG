...
void kernel_4336111321_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
        ...
        for (int i_1 = max(1, i_0); i_1 < PB_M; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L-3; i_2++) {
                for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
                    A[i_1][i_2][i_2+1] = A[i_1][i_2+2][i_2+3] + A[i_2][i_3-2][i_3-1] + A[i_1-1][i_3][i_3+1] * 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        B[i_4+1] = D[i_4-1] - C[1] - C[1] - 4;
    }
    for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 0; i_6 < i_5; i_6++) {
            C[1] = B[i_6+1] * B[i_5+1] - C[1] + C[1] * 3;
        }
    }
#pragma endscop
...
kernel_4336111321_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
