...
void kernel_4412111211_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_M, -i_0+PB_L+1); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_M+1, -i_0+PB_L-1); i_2++) {
                A[i_1][i_2+i_0] = A[i_1][i_2+i_0-1] + A[i_1][i_2+i_0+1] + A[i_2-1][i_1+i_0-1] - 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        ...
        for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_M-1; i_5++) {
                C[i_5][i_5+i_4] = C[i_5][i_5+i_4+1] + C[i_4][i_5+i_4-1] + 2;
            }
        }
...
kernel_4412111211_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
