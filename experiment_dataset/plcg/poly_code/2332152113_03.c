...
void kernel_2332152113_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M, -i_0+PB_L); i_1++) {
            A[i_1-1] = D[i_1][i_0-2] * D[i_1][i_0-1] * D[i_1-2][i_0-2] + D[i_1][i_1+i_0] + D[i_1-2][i_0] - A[i_1] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            A[i_3-1] = E[i_3+1] - E[i_3-2] + D[i_3][i_2] * D[i_3-1][i_2] * D[i_2][i_3-1] - 1;
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < min(PB_M, -i_4+PB_Q); i_5++) {
            C[i_5][i_5+i_4] = D[i_5][i_4] + D[i_5][i_4-2] + E[i_4-1] * 2;
        }
    }
#pragma endscop
...
kernel_2332152113_03(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
