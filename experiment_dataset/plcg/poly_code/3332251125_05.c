...
void kernel_3332251125_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L+1; i_0++) {
        A[i_0+1] = D[i_0+1] - D[i_0+1] * E[i_0] - A[i_0+3] - 5;
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = i_0; i_2 < -i_1+PB_L-1; i_2++) {
                B[i_0][i_1][i_2] = F[i_1][i_2+i_1-1] + G[i_2][i_0-1][i_0] + G[i_0-1][i_1][i_2] * H[i_0-1][i_2+i_1+1] + G[i_0][i_1][i_2] + I[i_0][i_2][0] - 3;
            }
        }
        A[i_0+1] = E[i_0+1] * D[i_0-1] + E[i_0] - E[i_0] - A[i_0-1] - 2;
    }
#pragma endscop
...
kernel_3332251125_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
