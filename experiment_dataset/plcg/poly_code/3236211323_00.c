...
void kernel_3236211323_00(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, -i_0+PB_Q-1); i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_Q+1; i_2++) {
                A[i_0-1][i_1][i_2+i_0-1] = A[i_0-1][i_2][i_1+i_0+1] * A[i_0-1][i_2+2][i_1+i_0-1] - 1;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_M-1; i_3++) {
        B[i_3-1] = C[i_3] + B[i_3] - B[i_3-3] * B[i_3+1] - 4;
    }
#pragma endscop
...
kernel_3236211323_00(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
