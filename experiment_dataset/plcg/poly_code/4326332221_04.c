...
void kernel_4326332221_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0] * E[i_0-1] + A[i_0-1] * A[i_0+1] - A[i_0-2] + 5;
    }
    for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
        B[i_1+1] = E[i_1] - B[i_1-1] - 2;
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_2+PB_P-1, i_1); i_3++) {
                C[i_1-2][i_2][i_3+i_2] = F[i_3+i_2][i_1][i_3] * F[i_3+2][i_1-2][i_2] + G[i_3+1][i_2+2][i_2][i_1] - H[2][i_2+1] + F[i_3][i_3-1][i_2] * C[i_1-2][i_2][i_3+i_2+1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4326332221_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
