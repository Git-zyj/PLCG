...
void kernel_4226312211_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
        ...
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            for (int i_3 = 0; i_3 < min(i_2, PB_M-2); i_3++) {
                for (int i_4 = 1; i_4 < PB_L; i_4++) {
                    B[i_3+2][i_2][i_4] = D[i_1+2][i_2][0] + B[i_3+2][i_4-1][i_1+1] - 1;
                }
            }
        }
    }
...
kernel_4226312211_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
