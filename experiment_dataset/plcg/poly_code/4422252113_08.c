...
void kernel_4422252113_08(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_P+1; i_2++) {
                D[i_1][i_0] = E[i_0-1][i_2][i_1] + F[i_1-1][i_2+i_0-1][i_1] * G[i_2][i_1-1] + 2;
            }
        }
        B[i_0] = H[i_0] - H[i_0-1] - I[i_0] + I[i_0] + I[i_0-1] - I[i_0-2] - B[i_0-1] + 6;
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_Q; i_4++) {
            for (int i_5 = 2; i_5 < PB_L; i_5++) {
                for (int i_6 = i_5; i_6 < PB_L; i_6++) {
                    C[i_5][i_6][i_3][i_4+i_3] = J[i_5+1][0][i_6][i_4+i_3] * J[i_3][i_6-1][i_5][i_4] + K[0][i_5-2][i_6][i_5-2] * 6;
                }
            }
        }
        for (int i_7 = 2; i_7 < PB_L; i_7++) {
            D[i_3][i_7] = G[i_3][i_7] - G[i_3][i_7] + L[i_7][i_3] * G[i_3][i_7-2] + G[i_7][i_3-1] + L[i_7][i_3] * G[i_7][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_4422252113_08(xd,yd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
