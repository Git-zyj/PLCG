...
void kernel_3124132125_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M - 1; i_0 >= 2; i_0--) {
        for (int i_1 = 1; i_1 < min(min(-i_0+PB_N+1, i_0), PB_M-1); i_1++) {
            A[i_1+i_0-2][i_1+i_0-2] = B[i_1][i_1-1] - B[i_0][i_1+1] + B[i_0][i_1] + A[i_1+i_0-1][i_1+i_0-1] + 5;
        }
    }
#pragma endscop
...
kernel_3124132125_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
