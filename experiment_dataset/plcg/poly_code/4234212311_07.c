...
void kernel_4234212311_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = C[i_1][i_0+1] * A[i_0-1][i_1+1] - A[i_1-1][i_0+1] * A[i_1+1][i_0-1] * 5;
        }
        for (int i_2 = i_0; i_2 < PB_P; i_2++) {
            for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                A[i_3][i_0] = C[i_3+2][i_2] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4234212311_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
