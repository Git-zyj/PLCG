...
void kernel_3234212321_05(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            B[i_0][i_1-1][i_1] = C[i_0][i_1] + C[i_1][i_0] * 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            for (int i_4 = max(3, i_3); i_4 < PB_Q; i_4++) {
                B[i_3][i_4-1][i_4] = B[i_3+1][i_4-1][i_4] - B[i_3-1][i_4-1][i_4] - B[i_3][i_4-3][i_4-2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3234212321_05(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
