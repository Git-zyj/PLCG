...
void kernel_2332132115_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        B[i_1] = D[i_1] * D[i_1] + D[i_1+1] * 4;
        A[i_1-1] = D[i_1+1] + D[i_1-2] - D[i_1] * B[i_1+1] + 1;
    }
#pragma endscop
...
kernel_2332132115_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
