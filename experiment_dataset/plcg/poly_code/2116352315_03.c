...
void kernel_2116352315_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 3; i_1 < min(i_0, -i_0+PB_N); i_1++) {
            A[i_1-2] = B[i_1][i_0+2] - C[i_1+2][i_0-2] * D[i_1][i_0+1] * D[i_1+i_0][2] * C[i_1][i_0-2] + A[i_1-1] * A[i_1-3] - A[i_1-3] * 3;
        }
    }
#pragma endscop
...
kernel_2116352315_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
