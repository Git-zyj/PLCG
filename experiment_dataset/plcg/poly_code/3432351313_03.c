...
void kernel_3432351313_03(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                B[i_1-1][i_2+1] = E[i_1][i_2][i_2] + E[i_2+1][i_0][i_1] + F[i_0][i_1] - G[i_2][i_1][i_0] + G[i_2][i_0-1][i_1] + 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L-2; i_4++) {
            B[i_3-1][i_4+1] = E[i_3][i_4][i_3] + F[i_4][i_3-1] + H[i_4-1][i_3+1] * B[i_3-1][i_4] - B[i_3-1][i_4+2] * B[i_3-2][i_4] - 4;
        }
    }
    for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
        for (int i_6 = 0; i_6 < PB_P-1; i_6++) {
            for (int i_7 = 1; i_7 < i_6; i_7++) {
                B[i_6][i_7+1] = I[i_7-1][i_6] + E[i_6+1][i_5][i_7+1] * E[i_7+1][i_5][i_6+1] - H[i_7][0] * 2;
            }
        }
        for (int i_8 = 1; i_8 < PB_P; i_8++) {
            B[i_5-1][i_8+1] = H[i_8][i_5+1] - F[i_8+1][i_5+1] + H[i_8][i_5] + F[i_5][1] - H[i_5+1][i_8] - J[i_8-1] * K[i_8] * I[i_8][i_5] * 6;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3432351313_03(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K)));
