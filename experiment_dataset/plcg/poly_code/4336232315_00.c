...
void kernel_4336232315_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < min(-i_0+PB_Q, PB_P); i_1++) {
            for (int i_2 = 1; i_2 < min(i_0, PB_P-1); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_2+PB_L, i_2); i_3++) {
                    A[i_3][i_2][0][i_1+i_0] = D[i_0][i_3+i_2][i_1][i_2+1] - D[i_2+i_1][i_3][i_0][i_2] + A[i_3][i_0][0][i_2+i_1] * A[i_3][i_2-1][0][i_1+i_0] + 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
            B[i_4][i_5] = E[i_4+1] + F[i_5] - B[i_4+1][i_5-1] - B[i_5+1][i_4] - B[i_4][i_5+1] + 1;
        }
    }
    for (int i_6 = 0; i_6 < PB_P; i_6++) {
        for (int i_7 = 0; i_7 < PB_L-2; i_7++) {
            for (int i_8 = max(2, i_7); i_8 < min(PB_P, -i_7+PB_M-1); i_8++) {
                B[i_7+1][i_8-1] = G[i_7+2][i_8] + H[i_8+i_7+1][i_6] - I[i_7][i_7+2][i_6] + H[i_7][i_8-2] * H[i_8][i_6] - 1;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4336232315_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
