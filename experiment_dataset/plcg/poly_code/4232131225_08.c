...
void kernel_4232131225_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-3; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(PB_M, -i_0+PB_L-1); i_1++) {
            B[i_0+1][i_1] = C[i_1-1][i_0+1] * C[i_1-1][i_0] * C[i_0-1][i_1] * D[i_1] * C[i_1+i_0+1][i_1] * B[i_0+3][i_1] + B[i_0+1][i_1-2] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            B[i_2+1][i_3] = E[i_3][i_2][i_3-1] + 1;
        }
    }
#pragma endscop
...
kernel_4232131225_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
