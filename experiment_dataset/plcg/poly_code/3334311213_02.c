...
void kernel_3334311213_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        ...
        for (int i_1 = 0; i_1 < min(min(PB_P, -i_0+PB_L), i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                B[i_0+1][i_2][0] = D[i_2][i_2][i_1+i_0] * B[i_0][i_2][0] + B[i_0+1][i_2-1][0] - C[i_2+1][i_1][i_1] + 5;
            }
        }
    }
...
kernel_3334311213_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
