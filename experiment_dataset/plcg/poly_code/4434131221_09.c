...
void kernel_4434131221_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-3; i_0++) {
        A[i_0+1] = E[i_0] * E[i_0-1] * E[i_0] - A[i_0+3] * 1;
    }
    for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
        B[i_1] = E[i_1] - E[i_1] + E[0] * E[i_1] - B[i_1+1] * B[i_1-1] + 6;
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < PB_Q-1; i_3++) {
            for (int i_4 = i_3; i_4 < -i_3+PB_Q-1; i_4++) {
                C[i_2][i_3][i_4+i_3+1] = F[i_3][i_4][i_3] + F[i_4][i_2][i_3] + C[i_2][i_3-2][i_4+i_3-1] + 4;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_N-3; i_5++) {
        D[i_5] = E[i_5] + E[i_5+1] - A[i_5+3] * D[i_5+1] + 5;
    }
#pragma endscop
...
kernel_4434131221_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
