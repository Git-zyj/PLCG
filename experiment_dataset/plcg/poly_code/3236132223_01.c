...
void kernel_3236132223_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(min(PB_L, i_0), -i_0+PB_N-2); i_1++) {
            A[i_1+i_0] = C[i_1][i_0] + D[i_0] * A[i_1+i_0+2] - A[i_1+i_0+1] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_N, PB_L); i_3++) {
            A[i_3+i_2] = C[i_3][i_2] + C[i_2][i_3] + D[i_2] - 5;
        }
    }
#pragma endscop
...
kernel_3236132223_01(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
