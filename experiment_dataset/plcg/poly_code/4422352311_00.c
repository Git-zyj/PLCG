...
void kernel_4422352311_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,int ym,DATA_TYPE POLYBENCH_2D(M,xM,yM,xm,ym),int xn,int yn,DATA_TYPE POLYBENCH_2D(N,xN,yN,xn,yn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_M-2; i_2++) {
                for (int i_3 = 2; i_3 < min(-i_0+PB_M-2, -i_1+PB_L); i_3++) {
                    A[i_3+i_0+2][i_1][i_2-2][i_1] = E[i_3+i_1][i_2+1][i_0][i_0] + E[i_1-2][i_3+2][i_0][i_2] - F[i_2][i_1][i_3] - G[i_2-2][i_1+1][i_0][i_2] + H[i_3][i_2+1][i_0][i_1-1] - A[i_2+i_0+2][i_1-1][i_3-2][i_1-1] - A[i_3+i_0+2][i_1-1][i_2-2][i_1-1] * 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        B[i_4-1] = I[i_4] * J[i_4] - K[i_4] - K[i_4+1] + J[i_4] * J[2] - B[i_4-2] + B[i_4] + 5;
        for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
            for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
                C[i_4][i_5+2][i_4] = L[i_6+1][i_5] - F[i_5][i_4][i_6-2] * C[i_5-1][i_6+2][i_5-1] - C[i_4][i_5+1][i_4] * 4;
            }
        }
        for (int i_7 = 1; i_7 < -i_4+PB_M-1; i_7++) {
            B[i_4-1] = M[i_7][i_4] - M[i_7-1][i_7] - N[i_4][i_7] * N[i_7+i_4+1][i_7] - M[i_7][i_4] + B[i_7-1] * B[i_7] + 3;
        }
    }
#pragma endscop
