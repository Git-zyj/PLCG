...
void kernel_4216111115_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            for (int i_2 = PB_Q - 1; i_2 >= i_1; i_2--) {
                for (int i_3 = i_2; i_3 < -i_1+PB_M; i_3++) {
                    A[i_3+i_1][i_0][i_2][0] = A[i_3+i_1-1][i_0][i_2][0] + 1;
                }
            }
        }
...
kernel_4216111115_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
