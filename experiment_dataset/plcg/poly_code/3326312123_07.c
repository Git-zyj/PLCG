...
void kernel_3326312123_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        ...
        for (int i_1 = 1; i_1 < PB_Q-2; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_Q+1, -i_0+PB_P); i_2++) {
                B[i_0+2][i_1+1][i_2-1] = D[i_1][i_2+i_0][i_2+i_1-1] + C[i_2][i_0][i_1+2] * 5;
                C[i_2][i_0][i_1+2] = 2;
            }
        }
    }
...
kernel_3326312123_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
