...
void kernel_3324312211_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N; i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_L+1; i_2++) {
                A[i_1][i_2+i_1-1] = B[i_1][i_2-2] - 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_L; i_4++) {
            B[i_3][i_4-1] = D[i_3][1] + B[i_3-1][i_4] * B[i_3-1][i_4-2] - 4;
        }
    }
    for (int i_5 = 0; i_5 < PB_L; i_5++) {
        for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 0; i_7 < PB_M; i_7++) {
                C[i_7][i_6] = C[i_7][i_6+1] - C[i_7][i_6-1] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3324312211_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
