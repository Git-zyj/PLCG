...
void kernel_2332311311_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
            B[i_2-1] = D[i_2+1] - B[i_2-2] - B[i_2] + B[i_2-2] * 4;
        }
    }
    for (int i_3 = 2; i_3 < PB_L+1; i_3++) {
        C[i_3][i_3-1] = B[i_3-1] - B[i_3-2] - B[i_3] + 2;
    }
#pragma endscop
...
kernel_2332311311_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
