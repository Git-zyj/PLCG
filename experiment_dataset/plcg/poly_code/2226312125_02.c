...
void kernel_2226312125_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        A[i_0-2] = A[i_0-4] * A[i_0] - 2;
    }
    for (int i_1 = 1; i_1 < PB_N+1; i_1++) {
        for (int i_2 = 0; i_2 < min(PB_N-1, -i_1+PB_L-1); i_2++) {
            B[i_2+i_1+1][i_1+1] = C[i_1-1][i_2] - C[i_2+1][i_1+2] * 5;
        }
    }
#pragma endscop
...
kernel_2226312125_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
