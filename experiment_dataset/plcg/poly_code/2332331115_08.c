...
void kernel_2332331115_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            A[i_0][i_1-1] = D[i_0][i_0] * B[i_1] + 6;
        }
        B[i_0] = E[i_0] - F[i_0] * G[i_0][i_0] + E[1] + 2;
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            A[i_2][i_3-1] = H[i_3][i_2] * D[i_2][i_3] + D[i_3][i_2] - H[i_3][i_2] + A[i_3][i_2-2] * 2;
        }
    }
#pragma endscop
...
kernel_2332331115_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
