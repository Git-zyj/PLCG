...
void kernel_3334351115_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0][i_0] + E[i_0] + F[i_0+1] * F[0] * A[i_0+1] - 2;
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            A[i_1] = G[i_1][i_0] - H[i_1][i_0-1] - G[i_1+1][i_0-1] - H[i_1+1][i_0] - 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        A[i_2+1] = I[1] - F[i_2] * I[i_2-1] * I[i_2-1] + F[i_2] - I[i_2-1] + 6;
    }
#pragma endscop
...
kernel_3334351115_07(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
