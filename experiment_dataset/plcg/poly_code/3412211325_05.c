    ...
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                B[i_1][i_2] = F[i_0][i_1+1][i_2] + G[i_2][i_1-1] * B[i_1+2][i_2] * B[i_2+1][i_1+2] * 3;
            }
        }
        A[i_0-2] = A[i_0-1] * 6;
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        D[i_3] = H[i_3+1] + D[i_3-1] * D[i_3+1] + D[i_3-2] - D[i_3+2] - 5;
    }
#pragma endscop
...
kernel_3412211325_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
