...
void kernel_4216112323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 3; i_1 < PB_M-2; i_1++) {
            A[i_0][i_1-1] = C[i_0][i_1+2] * C[i_1+2][i_0] + A[i_0+2][i_1-1] * A[i_0][i_1-3] * A[i_0][i_1] + A[i_0-1][i_1+1] + 4;
        }
    }
        ...
        for (int i_3 = 0; i_3 < PB_L-4; i_3++) {
            for (int i_4 = 0; i_4 < PB_P-2; i_4++) {
                for (int i_5 = 2; i_5 < PB_Q; i_5++) {
                    B[i_3+2][i_4][i_5] = B[i_3+4][i_5-2][i_2] - B[i_3+2][i_4+2][i_5] * 3;
                }
            }
        }
    }
...
kernel_4216112323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
