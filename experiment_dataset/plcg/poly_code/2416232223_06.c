...
void kernel_2416232223_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0+1] + E[1] + E[i_0-2] - F[i_0-1] * F[i_0] - 3;
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            B[i_1][0] = G[i_1][i_0] + F[i_1] - B[i_1+1][0] - B[i_1+2][0] * B[i_1-2][0] * B[i_1-1][0] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        B[i_2+2][0] = E[i_2-1] * E[i_2] - 2;
        for (int i_3 = 0; i_3 < -i_2+PB_M+1; i_3++) {
            B[i_3][0] = H[i_3][i_2] + G[i_3][i_2] * G[i_3+i_2-1][i_2] - 3;
        }
    }
#pragma endscop
...
kernel_2416232223_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
