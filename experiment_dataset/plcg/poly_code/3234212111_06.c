...
void kernel_3234212111_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 3; i_1 < PB_L+2; i_1++) {
            A[i_0-2][i_1-2] = C[i_0][i_1-2] + A[i_0-1][i_1-3] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = max(1, i_3); i_4 < min(PB_P+1, -i_2+PB_M); i_4++) {
                B[i_4+i_2][i_4-1] = 5;
            }
        }
    }
...
kernel_3234212111_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
