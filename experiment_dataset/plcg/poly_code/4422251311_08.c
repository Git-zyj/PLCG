...
void kernel_4422251311_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
        ...
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_0+PB_P-1; i_2++) {
                for (int i_3 = 0; i_3 < min(i_0, -i_1+PB_M+1); i_3++) {
                    A[i_3+1][i_1+1][i_1][i_2-1] = E[i_3][i_0][i_3][i_1] * F[i_2][i_3+i_1-1][i_2+i_0+1][i_0][i_3+1] + G[i_3][i_0][i_2+1][i_1][i_0] * H[i_3][i_2+i_0-1][i_1][i_1] + I[i_3][i_1][i_2] + A[i_3+1][i_1+2][i_1+1][i_2-1] * A[i_3+2][i_1+1][i_1][i_2] + 6;
                }
            }
            B[i_0+1][i_1] = J[i_1][i_0] + K[i_1][i_0] - J[i_1][i_0] + J[1][i_0+1] - B[i_0+2][i_1] + B[i_0][i_1] * B[i_0+2][i_1+1] - 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < min(i_4, PB_Q-1); i_5++) {
            for (int i_6 = 1; i_6 < PB_P; i_6++) {
                C[i_5-1][i_5][i_6] = K[i_4][i_5] * L[i_4][i_5][i_6] - I[i_6][i_6][i_5+1] - C[i_5-1][i_5][i_6-1] * C[i_5-2][i_5-1][i_6] - C[i_5-2][i_5-1][i_6-1] - 5;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_L-1; i_7++) {
        for (int i_8 = 1; i_8 < min(-i_7+PB_M-1, PB_L-1); i_8++) {
            D[i_7][i_8+i_7] = M[i_8+1] + K[i_8-1][i_7] - J[i_7-1][i_8-1] * K[i_8-1][i_7+1] - J[i_7][i_8+1] + D[i_8-1][i_8+i_7-1] * D[i_8+1][i_8+i_7+1] * 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4422251311_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M));
