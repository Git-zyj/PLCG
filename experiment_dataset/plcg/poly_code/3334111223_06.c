...
void kernel_3334111223_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            A[i_0][i_0+1] = A[i_0+2][i_0+3] + 3;
        }
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            B[i_0][i_2] = B[i_0+1][i_2-1] - B[i_2-2][i_0] + 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < -i_3+PB_M-1; i_4++) {
            C[i_3][i_4] = D[i_4][i_3-1] + D[i_4+i_3+1][1] + D[i_3][0] - C[i_3+1][i_4] + C[i_3-1][i_4] * C[i_3+2][i_4-2] - 4;
        }
    }
#pragma endscop
...
kernel_3334111223_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
