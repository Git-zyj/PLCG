...
void kernel_3424232115_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_0+1][i_1-1] = E[1][i_1] * F[i_0+2][i_1] - F[i_1+2][i_0-1] * F[i_1][i_0+1] + 6;
            for (int i_2 = 1; i_2 < i_1; i_2++) {
                A[i_1+1][i_2-1] = A[i_2+1][i_1-1] * 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M, PB_L); i_4++) {
            for (int i_5 = 0; i_5 < -i_4+PB_L; i_5++) {
                A[i_4][i_3-1] = G[i_5+1][i_4+i_3][i_4] - H[i_4-1][i_4][i_5] * H[i_3-2][i_5+i_4][i_4+i_3] + F[i_4][i_5] - 6;
            }
        }
        for (int i_6 = 0; i_6 < PB_M; i_6++) {
            for (int i_7 = 0; i_7 < min(-i_6+PB_P, PB_M-1); i_7++) {
                A[i_3+1][i_7] = F[i_7+1][i_3] + H[i_6][i_3][i_7+i_6] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3424232115_06(xa,ya,POLYBENCH_ARRAY(A), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
