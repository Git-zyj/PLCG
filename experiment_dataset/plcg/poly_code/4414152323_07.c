...
void kernel_4414152323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0+2] + E[i_0] - E[i_0] + E[i_0] + E[i_0] - A[i_0-2] + A[i_0+2] * A[i_0+1] + 1;
        for (int i_1 = 3; i_1 < -i_0+PB_L-1; i_1++) {
            for (int i_2 = 2; i_2 < -i_0+PB_N; i_2++) {
                B[i_0][i_2][i_1+i_0-2] = F[i_0][i_1-1][i_2] + F[i_0][i_1][i_2] * G[i_1-2][i_0-2] * G[i_2][i_1+i_0+1] - F[i_1-2][i_2+i_0][i_2+2] * F[i_0][i_2+1][i_1] - F[i_1][i_2-2][i_0] - B[i_2-1][i_0][i_2+i_1-3] - B[i_2][i_0+1][i_2+i_1-2] - B[i_0][i_2+2][i_1+i_0-2] * B[i_0-2][i_2-1][i_1+i_0-4] + 2;
            }
        }
        for (int i_3 = 1; i_3 < PB_L-4; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_N, -i_0+PB_N); i_4++) {
                C[i_4][i_3-1][i_4+i_0] = F[i_0][i_4+i_3][i_4] * F[i_3][i_4+i_0][i_3] * G[0][i_3+1] + F[i_3][i_0][i_3] - B[i_0][i_4-1][i_3+i_0-2] + 4;
                for (int i_5 = 0; i_5 < min(i_0, -i_3+PB_L-4); i_5++) {
                    C[i_5+2][i_4-1][i_5+i_3+4] = H[i_0+1][i_4][i_3][0] + H[i_5][i_4][i_3+2][i_0-2] + C[i_4][i_3][i_5+i_4] + 2;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414152323_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
