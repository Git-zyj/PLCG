...
void kernel_4222212215_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1][i_0] = A[i_1][i_0+1] + A[i_1][i_0-1] - A[i_1-1][i_0] - A[i_1+1][i_0] * 1;
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 0; i_3 < PB_L; i_3++) {
                    B[i_0][i_1][i_2][i_1] = C[i_3][i_0][i_1][i_1] * 2;
                }
            }
        }
    }
...
kernel_4222212215_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
