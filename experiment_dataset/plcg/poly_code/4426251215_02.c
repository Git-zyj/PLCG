...
void kernel_4426251215_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        A[i_0+1][i_0] = E[i_0] + E[i_0] * E[i_0] - F[i_0] * 6;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_0+1][i_0] = G[i_0][i_0] + H[i_0-1][i_1] + I[i_0+1][i_1][0] - H[i_0+1][i_1-1] + H[i_0][i_1] + G[i_1+1][i_0] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            A[i_2+2][i_2+1] = F[i_3-1] * G[i_3+1][i_2+1] + H[i_2][i_3-1] * F[i_2+1] + G[i_3][i_2] + E[i_3] + 5;
        }
    }
    for (int i_4 = 2; i_4 < PB_P-1; i_4++) {
        for (int i_5 = 0; i_5 < -i_4+PB_P-1; i_5++) {
            D[i_4][i_5+i_4][i_4-1] = G[i_5][i_4-1] - H[i_5][i_4-1] * F[i_4] - D[i_4-1][i_5+i_4][i_4-2] - D[i_4][i_5+i_4+1][i_4-1] * D[i_4-1][i_5+i_4-1][i_4-2] + 6;
        }
    }
#pragma endscop
...
kernel_4426251215_02(xa,ya,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
