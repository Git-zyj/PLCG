...
void kernel_3414311111_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[i_0+1] = E[0] * F[i_0] - A[i_0] - 4;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            B[i_2+1] = G[i_1][0] - E[i_2] - 5;
        }
        for (int i_3 = 0; i_3 < min(PB_L, -i_1+PB_P-1); i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_P, -i_1+PB_M-1); i_4++) {
                C[i_4+i_1][i_3][i_4+i_3] = C[i_4+i_1+1][i_3][i_3+i_1+1] - 4;
            }
        }
        D[i_1] = 1;
    }
...
kernel_3414311111_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
