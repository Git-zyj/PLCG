...
void kernel_4332351221_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+i_0][i_0] = D[i_1+1] + E[i_1][i_0] + D[i_1] * F[i_1-1][i_0] - D[i_1] * A[i_1+i_0+2][i_0] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            for (int i_4 = 1; i_4 < min(-i_3+PB_P-1, -i_2+PB_P); i_4++) {
                for (int i_5 = 1; i_5 < PB_Q-1; i_5++) {
                    B[i_4-1][i_3][i_5] = G[i_5][i_4][i_2-1][i_3-1][0] * H[i_5-1][i_4+i_2][i_3][i_2-1] * I[i_4][i_5-1][i_2][i_4+i_3+1] - H[i_3][i_5][i_4][i_5+1] + J[i_2][i_4][i_3][i_5] - J[i_5-1][i_2][i_4][i_3-1] + H[i_3+1][i_5-1][i_4-1][i_5+1] + B[i_4-1][i_3][i_5-1] * B[i_4-1][i_3+2][i_5] - 3;
                }
            }
        }
        for (int i_6 = 1; i_6 < i_2; i_6++) {
            C[i_6][1] = K[i_6] + L[i_6][i_2+1][i_2] * F[i_6-1][i_2] + C[i_6-1][1] + C[i_2-1][1] * C[i_6+1][1] - 4;
        }
    }
#pragma endscop
