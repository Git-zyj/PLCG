...
void kernel_4324351325_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < min(min(-i_2+PB_Q, -i_0+PB_L), i_0); i_3++) {
                    B[i_1+2][i_3+2][i_2-1] = D[i_1][i_2-1][i_0+1] * E[i_1+i_0-1][i_3][i_2+1][i_3+i_0] * F[i_3-1][i_1+i_0][i_2][i_1][i_2] - G[i_1+1][i_0][i_2+1][i_1] + E[i_1][i_2][i_3+i_0][1] - H[i_1+1][i_0][i_1+1][i_3+i_2] - B[i_1][i_2][i_3-1] - 1;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < PB_L; i_5++) {
            for (int i_6 = 1; i_6 < min(min(PB_L, -i_4+PB_M), i_5); i_6++) {
                B[i_6][i_4][i_5-1] = I[i_5][i_6] + D[i_6][i_4][i_5-1] * D[i_5-1][i_6][i_4-1] * J[i_6+i_4][0] * I[i_6][i_5] + B[i_6-1][i_4][i_5-1] * C[i_4][i_6][i_5-2] * 5;
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_N-3; i_7++) {
        for (int i_8 = 2; i_8 < PB_L; i_8++) {
            for (int i_9 = 1; i_9 < -i_7+PB_L-1; i_9++) {
                C[i_7+1][i_8][i_9-1] = K[i_8-1][i_7][i_9-1] - L[i_8+1][i_9][i_7] - L[i_7][i_9+i_7+1][i_8] * C[i_7+3][i_8][i_9-1] + C[i_7+1][i_8-2][i_9-1] * C[i_8+1][i_9+2][i_7+1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4324351325_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L));
