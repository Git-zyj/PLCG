...
void kernel_3324112123_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P; i_0++) {
        for (int i_1 = max(i_0, 4); i_1 < min(-i_0+PB_M+3, PB_L+4); i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_M, PB_P-1), -i_1+PB_M+2); i_2++) {
                A[i_2+i_0][i_0-2][i_2+1] = A[i_1+i_0-3][i_1-4][i_0] * A[i_2+i_1-2][i_1-4][i_2+1] + 4;
            }
        }
    }
    ...
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < PB_N; i_5++) {
            for (int i_6 = 2; i_6 < i_5; i_6++) {
                C[i_5][i_4-1][i_6] = D[i_5][i_4][i_6-2] - C[i_5][i_4][i_6] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3324112123_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
