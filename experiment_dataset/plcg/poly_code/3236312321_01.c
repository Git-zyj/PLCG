...
void kernel_3236312321_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-4; i_1++) {
            A[i_1+2] = A[i_1+3] * A[i_1] + A[i_1+1] * A[i_1+4] + 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
                B[i_3][i_4][i_4] = C[i_2+2][i_3][i_4] - B[i_2][i_4-2][i_4-2] - B[i_2][i_4+2][i_4+2] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3236312321_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
