...
void kernel_4422131121_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            A[i_1+1] = E[i_1] * F[i_0][i_1] * A[i_1+3] - A[i_0+2] - 5;
        }
        B[i_0-1] = E[i_0] * E[i_0] + E[i_0] * E[i_0] * 5;
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_2+PB_L+1, PB_M-1); i_3++) {
                C[i_2+1][i_0][i_3] = G[i_3+1][i_0][i_3+i_2-1] + 2;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        D[i_4] = E[i_4] * E[i_4+1] + E[i_4] * B[i_4-1] + D[i_4-2] + 5;
    }
#pragma endscop
...
kernel_4422131121_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
