...
void kernel_3334111315_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        B[i_3] = D[i_3] * D[i_3] + D[i_3-1] * B[i_3+1] * 2;
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
            B[i_5+1] = B[i_5+1] + B[i_5] - 1;
        }
    }
#pragma endscop
...
kernel_3334111315_00(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
