...
void kernel_2322132315_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            A[0] = D[i_1] + E[i_0+2][i_1-2] + A[0] - A[0] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            A[0] = E[i_2][i_3] * E[i_2][i_3] - E[i_3-1][i_2] - C[i_3] + 2;
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        C[i_4] = D[i_4+2] + D[i_4] * A[0] * C[i_4+1] - 3;
    }
#pragma endscop
...
kernel_2322132315_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
