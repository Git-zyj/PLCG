...
void kernel_3234212323_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = C[0] * C[i_0] * A[i_0-2] * A[i_0-1] - A[i_0+2] + 2;
    }
    for (int i_1 = 2; i_1 < PB_M+2; i_1++) {
        for (int i_2 = 4; i_2 < -i_1+PB_M+2; i_2++) {
            for (int i_3 = 4; i_3 < min(-i_2+PB_M, -i_1+PB_M+2); i_3++) {
                B[i_3+i_2][i_2-2] = B[i_3+i_1-2][i_3-4] + B[i_2+i_1-2][i_2-4] * B[i_3+i_2-2][i_2-3] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3234212323_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
