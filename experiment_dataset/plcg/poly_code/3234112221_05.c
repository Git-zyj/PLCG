...
void kernel_3234112221_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_P-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_1-2][i_2][i_0] = A[i_0-4][i_2+2][i_1+1] - 3;
            }
        }
    }
    for (int i_3 = 4; i_3 < PB_M; i_3++) {
        B[i_3-2] = C[i_3] + C[i_3] - B[i_3-3] + B[i_3-4] + B[i_3] * 3;
    }
#pragma endscop
...
kernel_3234112221_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
