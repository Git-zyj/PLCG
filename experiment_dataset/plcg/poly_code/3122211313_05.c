...
void kernel_3122211313_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = PB_L-1 - 1; i_1 >= i_0; i_1--) {
            for (int i_2 = 1; i_2 < -i_0+PB_Q; i_2++) {
                A[i_1][i_0-1][i_2+i_0][i_1] = B[i_0][i_1] + A[i_1][i_0-1][i_2+i_0-1][i_1] - A[i_1+1][i_0-1][i_2+i_0-1][i_1+1] + A[i_1][i_2-1][i_2+i_0-1][i_1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3122211313_05(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
