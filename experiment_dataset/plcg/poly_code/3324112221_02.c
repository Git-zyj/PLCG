...
void kernel_3324112221_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+i_0][i_0] = D[i_1][i_0-1] + A[i_1+i_0-1][i_0] * A[i_1+i_0-2][i_0-2] + A[i_1+i_0+2][i_0+2] * 3;
        }
    }
    ...
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 0; i_4 < i_3; i_4++) {
            C[i_3][i_4][i_4] = C[i_3+2][i_4][i_4] - 3;
        }
    }
#pragma endscop
...
kernel_3324112221_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
