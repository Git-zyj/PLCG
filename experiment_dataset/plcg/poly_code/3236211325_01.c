...
void kernel_3236211325_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                B[i_2][i_0-2] = C[i_1][0] - 5;
            }
        }
        for (int i_3 = 2; i_3 < i_0; i_3++) {
            B[i_3][i_0] = B[i_3+1][i_0] + B[i_3+2][i_0] * B[i_3-2][i_0] + 2;
        }
    }
#pragma endscop
...
kernel_3236211325_01(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
