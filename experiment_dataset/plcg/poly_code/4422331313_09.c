...
void kernel_4422331313_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-1] = E[1] * E[i_0-1] * E[i_0] - F[i_0] + A[i_0-2] * A[i_0] * 4;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-2; i_2++) {
                D[i_1][i_0][i_1][i_2+2] = 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L-1; i_4++) {
            for (int i_5 = -i_4+PB_Q - 1; i_5 >= 2; i_5--) {
                for (int i_6 = i_4; i_6 < min(PB_Q-2, -i_3+PB_Q); i_6++) {
                    C[i_5-1][i_4+1][i_6+1][i_6] = G[i_4][i_3][i_5-1][i_6+1] + H[i_4][i_3][i_6+i_3][i_5+i_4] + I[i_6][i_4+1][i_5][i_3-1] * C[i_5-2][i_4+1][i_6+1][i_6] * C[i_5][i_4+2][i_6+1][i_6] + C[i_5-1][i_4][i_6+2][i_6+1] * C[i_5-2][i_4+2][i_6+1][i_6] + 1;
                }
            }
        }
    }
        ...
        for (int i_8 = 0; i_8 < -i_7+PB_Q-1; i_8++) {
            for (int i_9 = 0; i_9 < -i_7+PB_Q; i_9++) {
                for (int j_0 = 0; j_0 < PB_L; j_0++) {
                    D[i_8+1][i_9][i_8+1][i_7+1] = H[j_0][i_8+i_7+1][i_8][i_9] * I[i_8][j_0][i_7][i_9+i_7] * I[j_0+1][i_8+1][i_7][i_9] * J[j_0][i_8][i_7-1] * K[i_9][j_0][i_8] + D[i_8+1][j_0][i_8+1][i_7+2] * D[i_8][i_9][i_8][i_7+1] - 1;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4422331313_09(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
