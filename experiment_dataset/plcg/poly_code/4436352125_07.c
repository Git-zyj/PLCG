...
static void print_array(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4436352125_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_Q; i_1++) {
            for (int i_2 = 2; i_2 < PB_L; i_2++) {
                for (int i_3 = 2; i_3 < min(min(-i_0+PB_M, -i_1+PB_M), -i_2+PB_M); i_3++) {
                    B[i_3-2][i_1][i_2-1] = E[i_2][i_0][i_2][0] + F[i_3+i_0][i_2][i_1][i_1] * F[i_3][i_0][i_1][i_2] + F[i_3+i_1][i_0][i_2][i_1] + E[i_3+i_2][i_0][i_1][0] * G[i_1][i_2][i_3][i_0+2] + E[i_1][i_3][i_3+i_2-2][i_1+i_0] * E[i_2][i_3][i_0][1] * 5;
                }
            }
        }
        for (int i_4 = i_0; i_4 < PB_L; i_4++) {
            for (int i_5 = i_0; i_5 < min(-i_4+PB_L+1, -i_0+PB_P-1); i_5++) {
                B[i_0-2][i_4][i_5] = H[i_4][i_0][i_5] - H[i_5+i_4-1][i_5+i_0][i_4] + I[i_4][i_0] * J[i_4-2][i_5+i_0+1][i_0] - B[i_0-2][i_4-2][i_5] - B[i_4-1][i_0][i_5] + 1;
            }
        }
    }
    for (int i_6 = 2; i_6 < PB_L-2; i_6++) {
        for (int i_7 = 1; i_7 < i_6; i_7++) {
            for (int i_8 = 0; i_8 < PB_P-1; i_8++) {
                for (int i_9 = 0; i_9 < min(min(-i_8+PB_Q, -i_7+PB_M), PB_L-1); i_9++) {
                    C[i_6-2][i_7][i_7][i_9+i_8] = E[i_7-1][i_6+1][i_9][i_7] - G[i_9+i_7][i_6+1][i_9][i_8] + H[i_9+1][i_7][i_8+1] * 4;
                }
            }
        }
        for (int j_0 = 1; j_0 < min(i_6, -i_6+PB_N-1); j_0++) {
            D[j_0] = K[j_0+i_6+1] + L[j_0][i_6+2] * I[j_0][i_6+1] * I[j_0][i_6+2] - M[j_0+i_6] * D[j_0-1] + 3;
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_4436352125_07(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M));
