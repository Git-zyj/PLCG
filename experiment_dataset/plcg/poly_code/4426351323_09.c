...
void kernel_4426351323_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,int wd,DATA_TYPE POLYBENCH_5D(D,xD,yD,zD,vD,wD,xd,yd,zd,vd,wd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                for (int i_3 = 1; i_3 < min(min(-i_2+PB_L, PB_P-2), i_1); i_3++) {
                    A[i_2][i_1][i_3][i_1] = E[i_3][i_2-1][i_1+i_0][i_0][i_2] * F[i_1+1][i_0][i_0][i_2] + G[i_0][i_3-1][i_1][i_2] * H[i_3+i_2][i_1][i_0][i_3] - A[i_3][i_1][i_2-1][i_1] - A[i_1+1][i_3+2][i_2][i_3+2] - 2;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        B[i_4] = I[i_4+1] - I[i_4] + I[i_4+1] + I[i_4] + I[1] - B[i_4+2] * 4;
    }
    for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
        for (int i_6 = 2; i_6 < PB_P; i_6++) {
            for (int i_7 = 1; i_7 < PB_P-3; i_7++) {
                for (int i_8 = i_7; i_8 < PB_Q-1; i_8++) {
                    C[i_7][i_6][i_5+1][i_8] = H[i_7][i_8-1][i_5][i_7-1] - J[i_7][i_8+1][i_6] - G[i_8][i_7+1][i_5][i_6] + G[i_8][0][i_7][i_5-1] * C[i_8-1][i_6][i_7+3][i_5] + C[i_8][i_6][i_5+1][i_7+2] * C[i_7][i_6-1][i_5+1][i_8-1] + C[i_7+1][i_6][i_5][i_8+1] + C[i_7][i_6][i_5+1][i_8+1] * C[i_6-2][i_7][i_5][i_8] * 1;
                }
            }
        }
        for (int i_9 = 1; i_9 < i_5; i_9++) {
            for (int j_0 = 1; j_0 < min(-i_9+PB_P-1, -i_5+PB_L); j_0++) {
                for (int j_1 = 1; j_1 < PB_P; j_1++) {
                    D[i_5][i_9][i_9][j_0][0] = G[j_1][i_9][j_0-1][i_5+1] + K[j_0+i_9-1][j_1-1][0] + K[j_0][i_9+1][i_5] * G[j_1-1][j_0+i_5][i_9+1][j_0+i_9+1] + H[i_5][j_0][i_5][j_1] * H[i_5+1][j_0+i_5-1][i_9][i_9] - A[i_5][i_9+1][j_1][i_9+1] * 5;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,wd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4426351323_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,wd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,wd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
