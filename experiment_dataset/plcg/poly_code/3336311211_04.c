...
void kernel_3336311211_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_2 = 1; i_2 < PB_P; i_2++) {
            B[i_0][i_2-1] = B[i_0][i_2] * C[i_0+1] + 6;
        }
    }
    for (int i_3 = PB_M - 1; i_3 >= 0; i_3--) {
        for (int i_4 = max(1, i_3); i_4 < PB_M-1; i_4++) {
            C[i_4] = D[i_4][i_3] - C[i_4+1] + C[i_4-1] - 5;
        }
    }
#pragma endscop
...
kernel_3336311211_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
