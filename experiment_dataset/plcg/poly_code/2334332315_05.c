...
void kernel_2334332315_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        A[1] = D[i_0] - E[i_0+1] + E[i_0] * F[i_0] + D[i_0] - A[1] * A[1] - 5;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        B[i_1] = B[i_1+1] + B[i_1-1] + B[i_1+1] * 4;
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < PB_N; i_3++) {
            C[i_3] = G[i_2][i_3-2] + H[i_3-2][i_2+2] * D[i_3] * A[1] * C[i_3-1] - C[i_3+1] + C[i_3-1] * 6;
        }
    }
#pragma endscop
...
kernel_2334332315_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
