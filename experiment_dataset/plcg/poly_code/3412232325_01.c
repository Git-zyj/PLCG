...
void kernel_3412232325_01(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        D[i_0] = E[i_0] + F[i_0] + F[i_0] + D[i_0] - 4;
        for (int i_1 = 1; i_1 < -i_0+PB_L-2; i_1++) {
            B[i_1][i_0][i_0] = G[i_0][i_1] - H[i_1+2][i_0] - G[i_1+1][i_1+i_0+2] * B[i_1-1][i_0+2][i_0+2] + 2;
        }
        for (int i_2 = max(2, i_0); i_2 < PB_L-1; i_2++) {
            for (int i_3 = 0; i_3 < PB_N; i_3++) {
                B[i_0][i_2-2][i_2-2] = I[i_2+1][i_0][0] - H[i_3][i_2] - 2;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        D[i_4] = E[i_4] - E[i_4] - D[i_4+1] * D[i_4+2] * D[i_4-1] - D[i_4-2] + 2;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3412232325_01(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
