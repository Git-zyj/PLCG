...
void kernel_4214151223_04(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M+1, PB_M-1); i_1++) {
            B[i_1] = C[i_1+1][i_0] + C[i_1+1][i_0] - C[i_1+i_0-1][i_1+1] + C[i_1-1][i_0+1] - C[i_1][i_0] + C[i_1][i_0] + B[i_1] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M+1; i_3++) {
            B[i_3] = C[i_3][i_3+i_2-1] + C[i_2][i_3-1] + C[i_3][i_2+1] * C[i_3][i_3+i_2-1] + B[i_3-1] + 6;
        }
    }
#pragma endscop
...
kernel_4214151223_04(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
