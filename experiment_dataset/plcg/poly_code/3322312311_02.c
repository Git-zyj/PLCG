...
void kernel_3322312311_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                A[i_2][i_0][i_2] = A[i_2][i_0+1][i_2] - A[i_2-1][i_0][i_2-1] + 2;
            }
            for (int i_3 = 0; i_3 < min(PB_P, i_0); i_3++) {
                A[i_3][i_0][i_3] = D[i_1][i_1][i_0] + 3;
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P+2; i_4++) {
        for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_P; i_6++) {
                C[i_4+1][i_5][i_6-1] = D[i_5][i_4-2][i_5] * C[i_6+1][i_5-1][i_4-2] * C[i_4+1][i_5+1][i_6-1] + C[i_4+1][i_5][i_6] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3322312311_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
