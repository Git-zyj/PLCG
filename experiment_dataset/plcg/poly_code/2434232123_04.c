...
void kernel_2434232123_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0-2] = E[i_0] + F[i_0] + E[i_0] * A[i_0-1] * 1;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        ...
        for (int i_3 = max(1, i_1); i_3 < PB_M; i_3++) {
            B[i_3][i_1+2] = G[i_3-1][i_1] * G[i_3][i_1+2] * F[i_1] - H[i_3-1][i_1] * H[i_3][i_1] * 3;
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        D[i_4] = E[i_4] * E[i_4] - F[i_4] - 5;
    }
#pragma endscop
...
kernel_2434232123_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
