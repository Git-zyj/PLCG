...
void kernel_2232211125_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            B[i_1][i_2] = C[i_2][0] - D[i_2][i_1] - B[i_1+1][i_2] + 5;
        }
    }
#pragma endscop
...
kernel_2232211125_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
