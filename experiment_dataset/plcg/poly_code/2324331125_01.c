...
void kernel_2324331125_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        A[i_0] = D[i_0] - E[i_0+1] * 6;
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            B[i_0+1][i_1][i_1] = F[i_0][i_1] * G[i_0][i_1] - H[i_0][i_1][i_0] + G[i_1][i_0] + F[i_1][i_1] + A[i_1] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
        C[i_2-1] = D[i_2] - E[i_2] - 6;
    }
#pragma endscop
...
kernel_2324331125_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
