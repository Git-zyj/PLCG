...
void kernel_3224152125_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_P-2; i_1++) {
            B[i_0][i_1+i_0+2] = C[i_0-1][0] + C[i_1][i_1+i_0+1] * D[i_1+2] * 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_P-2; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_P-2; i_3++) {
            B[i_2][i_3+i_2+2] = D[i_3] + D[i_3-1] * D[i_3+2] * C[i_2-2][i_3+i_2] + C[i_3+2][i_2] - C[i_3-1][i_2] * B[i_2-2][i_3+i_2] - 5;
        }
    }
#pragma endscop
...
kernel_3224152125_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
