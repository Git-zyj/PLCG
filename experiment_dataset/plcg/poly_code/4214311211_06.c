...
void kernel_4214311211_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_0-1][i_1] = A[i_1-1][i_0+1] - A[i_0-2][i_1+1] + A[i_0][i_1] * 3;
        }
    }
        ...
        for (int i_3 = 1; i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < PB_L; i_4++) {
                for (int i_5 = 1; i_5 < PB_P; i_5++) {
                    B[i_3-1][0][i_4][i_5] = C[i_3-1][i_5][i_2-1][i_4] + B[i_4-1][0][i_2][i_5-1] + 4;
                }
            }
        }
    }
...
kernel_4214311211_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
