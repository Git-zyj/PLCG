...
void kernel_4334352315_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 4; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_L-1; i_2++) {
                for (int i_3 = max(max(max(-i_1+4, 1), -i_2), -i_0+1); i_3 < min(-i_0+PB_Q+1, -i_1+PB_Q+1); i_3++) {
                    A[i_2+1][i_3][i_0+1][i_3+i_1-2] = D[i_3+i_2][i_0][i_1][i_2+2] - E[i_2][i_2-1][i_1][i_3] + E[i_2+i_1+1][i_3][i_0][i_1+1] * A[i_2+1][i_3-1][i_0+2][i_3+i_1-4] * A[i_2+2][i_0+1][i_1+1][i_3+i_0-1] * A[i_2+1][i_3][i_0+1][i_3+i_1-1] * 6;
                }
            }
        }
        B[i_0] = F[i_0+2] * G[i_0] + F[0] + G[i_0-2] - B[i_0-1] * B[i_0+1] + 5;
        for (int i_4 = 0; i_4 < min(-i_0+PB_Q+2, PB_L-2); i_4++) {
            for (int i_5 = i_0; i_5 < PB_Q; i_5++) {
                for (int i_6 = 0; i_6 < PB_Q-2; i_6++) {
                    A[i_5+1][i_4][i_6+1][i_4+i_0-2] = D[i_0][i_5][i_0][i_4] + E[i_5][i_4][i_0-1][i_6] + E[i_4][i_6][i_4+1][i_0] - H[i_5][i_6][i_0][i_4][i_0] * E[i_0][i_6+2][i_6][i_5] * E[i_4+2][i_6][i_0-1][0] + I[i_5+1][i_6+1][i_4] + 3;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4334352315_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
