...
void kernel_3224311315_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = i_1; i_2 < PB_L-1; i_2++) {
                B[i_0+1][i_2+1] = C[i_2][i_1] - B[i_1+2][i_2] - B[i_1+1][i_2] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            B[i_4+1][i_3] = C[i_3][i_4] - B[i_4][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_3224311315_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
