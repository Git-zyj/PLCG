...
void kernel_3424251111_01(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            for (int i_2 = i_1; i_2 < min(-i_1+PB_M, -i_0+PB_L); i_2++) {
                C[i_2-1][i_1-1] = E[i_2+i_1][i_0-1][i_1][i_0] + F[i_2+i_1][i_2+i_0][i_0] + 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = i_4; i_5 < min(-i_4+PB_P+1, PB_L-1); i_5++) {
                C[i_4-1][i_5-1] = G[i_5][i_4] - E[i_3][i_5-1][i_4+1][i_3] - H[i_4+1][i_5][i_5] - H[i_4][i_3][i_5+1] - G[i_5+1][i_5+i_4-1] * F[i_5][i_5][i_3] - H[i_5-1][i_3][i_4] + F[i_4-1][i_3-1][i_3] - H[i_3][i_5+1][i_4] * F[i_5][i_4][i_3+1] - C[i_4-1][i_3-1] - 1;
            }
            C[i_4-1][i_3-1] = I[i_4-1][i_3] + G[i_4-1][i_3] * G[i_4][i_3] - J[i_4] + I[i_4][i_3-1] * 2;
        }
        for (int i_6 = 0; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < min(min(i_6, -i_3+PB_L-1), -i_6+PB_L); i_7++) {
                D[i_6][i_7+i_6][i_3] = H[i_6][i_7][i_3] - F[i_6+1][i_7+i_3+1][i_3-1] + D[i_3+1][i_7+i_3+1][i_6+1] - 4;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3424251111_01(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
