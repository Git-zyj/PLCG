...
void kernel_2434251115_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - F[i_0] - F[i_0] - E[i_0+1] + F[i_0+1] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            B[i_1][i_2] = G[i_1][i_2-1] + G[i_1][i_2] - E[i_2-1] * A[i_1] + 1;
            B[i_1][i_2+1] = G[i_1][i_2+1] - H[i_2][i_1] - H[i_2][i_1+1] - G[i_1][i_2] + H[i_2-1][i_1] + 6;
        }
    }
    for (int i_3 = PB_M - 1; i_3 >= 1; i_3--) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_L, PB_M-1); i_4++) {
            B[i_3-1][i_4] = F[i_4+1] * I[i_4][i_4+i_3][i_3+1] * H[i_4+1][i_3] + G[i_3][i_4] - 1;
        }
    }
#pragma endscop
...
kernel_2434251115_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
