...
void kernel_4336311311_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = A[i_0+2] + A[i_0] + A[i_0+2] + A[i_0] - A[i_0+2] + 4;
    }
    for (int i_1 = 0; i_1 < PB_Q; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 0; i_3 < -i_2+PB_L-1; i_3++) {
                B[i_3][i_2][i_3] = D[i_3+i_2+1][i_2][i_1] * 6;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < PB_Q-1; i_5++) {
            for (int i_6 = 0; i_6 < PB_L-2; i_6++) {
                C[i_6+1][i_4+1][i_5] = B[i_4][i_5-1][i_4] + C[i_4+1][i_6][i_5+1] * C[i_6][i_4+1][i_5] * C[i_6+2][i_4][i_5] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4336311311_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
