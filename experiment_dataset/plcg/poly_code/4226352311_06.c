...
void kernel_4226352311_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < PB_N-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_Q); i_2++) {
                A[i_1][i_2] = C[i_1-1][i_0][i_0] - D[i_2][i_1] - C[i_2][i_0-2][i_1] * E[0][i_0][i_1-2] * D[i_1-2][i_1] - C[i_0][i_1-1][i_2] - E[i_2+i_1][i_0+1][0] - A[i_1-1][i_2-1] * A[i_1+1][i_2] - A[i_1][i_2-1] - 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_Q; i_3++) {
        for (int i_4 = 1; i_4 < PB_Q-2; i_4++) {
            for (int i_5 = i_3; i_5 < min(-i_3+PB_L, -i_4+PB_L+1); i_5++) {
                A[i_4-1][i_3+1] = C[i_5][i_3][i_4] + F[i_4][i_5+i_3] * C[i_5+i_4-1][i_3][i_4+2] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4226352311_06(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
