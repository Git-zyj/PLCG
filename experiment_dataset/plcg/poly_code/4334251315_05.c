...
void kernel_4334251315_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L-1; i_1++) {
            A[i_0][i_1+i_0-1] = D[i_1][i_0+1] * E[i_1][i_1+1][i_0] - F[i_1][0] * F[i_1][i_0+1] + A[i_0][i_1+i_0] + A[i_0+1][i_1+i_0+1] - A[i_0][i_1+i_0-2] + 3;
        }
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = i_0; i_3 < min(-i_2+PB_L+1, -i_0+PB_L); i_3++) {
                A[i_2+1][i_3+i_2-1] = F[i_2+1][i_0-1] - E[i_2][i_0][i_0] * G[i_2][i_3+i_0][i_0] * E[i_3][i_3][1] + D[i_3][i_2-1] - 6;
            }
        }
    }
        ...
        for (int i_5 = i_4; i_5 < min(PB_Q-1, -i_4+PB_M); i_5++) {
            for (int i_6 = 1; i_6 < -i_4+PB_Q-1; i_6++) {
                for (int i_7 = 1; i_7 < PB_Q-1; i_7++) {
                    A[i_6][i_7+i_6-1] = H[i_6-1][i_4-1][i_5-1][i_7+1] - H[i_5+i_4][i_6][i_7][i_6+1] + H[i_7-1][i_5][i_6+i_4+1][i_4] * H[i_7+1][i_6][i_4][i_5] + E[i_6+1][i_5][i_6] - I[i_6][i_5-1][i_4][i_5+1] * 3;
                }
            }
        }
    }
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4334251315_05(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
