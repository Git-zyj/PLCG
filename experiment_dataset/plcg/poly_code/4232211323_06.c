...
void kernel_4232211323_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-2; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_N-3, -i_1+PB_Q+1); i_2++) {
                A[i_1+1][i_2+1] = C[i_0-1][i_2][i_2+i_1-1] * A[i_2+3][i_0+3] * A[i_2+2][i_0+1] - A[i_1+2][i_2+1] - 6;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_Q, -i_3+PB_L-2); i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                B[i_3-1][i_5][i_5+i_4+1] = C[i_4-1][i_3-1][0] - B[i_3-3][i_5][i_5+i_4+1] + B[i_3-1][i_5][i_5+i_4] - B[i_5-1][i_3+1][i_4+i_3+2] - 5;
            }
        }
    }
#pragma endscop
...
kernel_4232211323_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
