...
void kernel_4214212225_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[2] = A[2] - A[2] * A[2] * 1;
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            B[i_1-2][i_0] = C[i_0+1][i_1] * A[2] * 1;
        }
    }
#pragma endscop
...
kernel_4214212225_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
