...
void kernel_3422151321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] * E[i_0] - E[i_0] - A[i_0+2] * A[i_0+1] * A[i_0-1] + B[i_0+1] * 1;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        B[i_1] = E[1] + E[i_1] * E[i_1-1] * A[i_1] * A[i_1-1] - B[i_1-2] - 5;
        for (int i_2 = 1; i_2 < i_1; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_2+PB_N, PB_P-2); i_3++) {
                C[i_1][i_2][i_3] = F[i_3][i_1][0] + G[i_1+1][i_3+i_2][i_1+1][i_3-1] * H[i_3][i_2] * F[i_3][i_1][i_2] - H[i_2][i_3] + F[i_1][i_3][i_2] - C[i_1][i_2-1][i_3] + C[i_1][i_2][i_3+2] - 2;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        D[i_4] = E[i_4-1] * E[i_4] + H[i_4][i_4] + E[i_4] + E[i_4] - A[i_4] + B[i_4-1] + D[i_4+2] + 3;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3422151321_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
