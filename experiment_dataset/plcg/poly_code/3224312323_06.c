...
void kernel_3224312323_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            B[i_0+1][i_1][i_1] = C[i_1+2][i_0] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
            B[i_2+2][i_3][i_3] = C[i_3][i_2] * B[i_2+4][i_3][i_3] * B[i_2+2][i_3+1][i_3+1] + B[i_2+2][i_3-1][i_3-1] * 5;
        }
    }
#pragma endscop
...
kernel_3224312323_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
