...
void kernel_2332352323_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_L); i_1++) {
            B[i_1+1] = D[i_1][i_0] + E[i_1+i_0-2][i_0-1] - D[i_0+2][i_1] - F[i_1][i_1] - G[i_0+1][i_1][0] * F[i_1+i_0][i_1-2] * F[i_1][i_0] + B[i_1] - 6;
        }
        B[i_0] = H[i_0-1] * I[i_0] - I[i_0] - H[i_0-2] - B[i_0-2] - B[i_0+1] - 4;
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < i_2; i_3++) {
            B[i_3-2] = E[i_3+2][i_2+1] * F[i_3][i_2] * 6;
        }
    }
#pragma endscop
...
kernel_2332352323_07(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
