...
void kernel_4336231313_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            for (int i_2 = 1; i_2 < PB_L; i_2++) {
                for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
                    A[i_3][i_0][i_1-1][i_2] = D[i_3+1][i_0][i_2][i_1] + D[i_3][i_2][i_0][i_1] + A[i_3][i_0-1][i_1-2][i_2-1] + A[i_3-1][i_0-1][i_1][i_2] + 6;
                }
            }
        }
        B[i_0] = E[i_0-1] + E[i_0] + B[i_0+1] - B[i_0-1] * B[i_0+1] * 6;
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_L; i_5++) {
            for (int i_6 = 1; i_6 < i_5; i_6++) {
                for (int i_7 = 1; i_7 < -i_4+PB_L; i_7++) {
                    A[i_4][i_5-1][i_6-1][i_7] = F[i_6][i_7][i_7+i_4][i_5] * D[i_7][i_6-1][i_5+i_4][0] + D[i_6][i_4][0][i_5+i_4] + A[i_4][i_5-1][i_7-1][i_6-1] + 2;
                }
            }
        }
    }
...
kernel_4336231313_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
