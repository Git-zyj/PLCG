...
void kernel_2434352313_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        B[i_0-2] = E[i_0+1] * F[i_0] * E[i_0+1] + F[0] + E[i_0] + F[i_0] - B[i_0-2] * B[i_0-3] * B[i_0-1] - 3;
        for (int i_1 = 3; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            B[i_1-2] = E[i_1+i_0] - G[i_1-1][i_0] + H[i_0][i_1] * I[i_0][i_1+1][0] - G[i_1][i_1+i_0] - H[i_1][i_0+1] - G[i_1+2][i_0] * B[i_1-1] * B[i_1-3] * 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, PB_M-2); i_3++) {
            C[i_2][i_3] = G[i_3-1][i_2] * J[i_3][0] - J[i_3+2][i_2-1] + H[i_2][i_3] * G[i_3][i_2] * C[i_2+1][i_3] + C[i_2+1][i_3-1] * C[i_2+1][i_3+1] + 2;
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            D[i_5] = H[i_5][i_4+2] + G[i_4][i_5] + D[i_5+1] + 6;
        }
    }
#pragma endscop
...
kernel_2434352313_09(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
