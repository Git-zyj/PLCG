...
void kernel_4226311221_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = max(1, i_0); i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = 2; i_3 < PB_L; i_3++) {
                    A[i_1][i_3][i_1] = A[i_1-1][i_3-2][i_1-1] - 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-2; i_4++) {
        B[i_4-1][i_4] = C[i_4] - B[i_4][i_4+1] * B[i_4+1][i_4+2] - 3;
    }
#pragma endscop
...
kernel_4226311221_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
