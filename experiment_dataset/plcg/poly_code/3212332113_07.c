...
void kernel_3212332113_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = PB_L - 1; i_2 >= 0; i_2--) {
        for (int i_3 = 0; i_3 < PB_N; i_3++) {
            A[i_3][i_2] = C[i_3][i_2] + D[i_3][i_2] * D[i_3][i_2] - D[i_2][0] - 4;
        }
    }
#pragma endscop
...
kernel_3212332113_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
