...
void kernel_3312111323_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            B[i_1] = B[i_1] - B[i_0] * B[i_0-2] + 4;
        }
        B[i_0] = 1;
        for (int i_2 = 0; i_2 < min(-i_0+PB_L, PB_M-1); i_2++) {
            for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
                B[i_0] = D[i_2+1][i_3+1][i_2+i_0] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3312111323_06(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D));
