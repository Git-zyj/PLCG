...
void kernel_2314311313_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
            A[i_0-1][i_1+1] = A[i_0-1][i_1] - A[i_0-2][i_1+1] * A[i_0-1][i_1+2] - 4;
        }
    ...
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        C[i_3-1] = D[i_3+1] + C[i_3-2] + C[i_3] + 6;
    }
#pragma endscop
...
kernel_2314311313_07(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
