...
void kernel_3316212321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[0] = A[0] - A[0] * 2;
        for (int i_1 = 2; i_1 < min(min(PB_P, i_0), -i_0+PB_N-2); i_1++) {
            B[i_1+i_0][i_1-2] = A[0] + B[i_1+i_0+2][i_1] - C[i_0][i_1] - 1;
            C[i_0][i_1] = D[0][i_0-1] - A[0] - B[i_1+i_0+1][i_1-1] - 1;
        }
    }
#pragma endscop
...
kernel_3316212321_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
