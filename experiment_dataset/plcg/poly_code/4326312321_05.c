...
void kernel_4326312321_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
                A[i_2][i_1][i_0] = D[i_2][i_0][i_1+2] - A[i_0+2][i_2+2][i_1-1] - A[i_2+2][i_1+2][i_0] * A[i_2][i_1+1][i_0] * A[i_2+2][i_1][i_0-1] - A[i_2][i_1-1][i_0] * 1;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_L-4; i_3++) {
        ...
        for (int i_4 = i_3; i_4 < PB_M-1; i_4++) {
            for (int i_5 = i_4; i_5 < PB_L-2; i_5++) {
                C[i_5+2][i_3][i_3-2] = A[i_4+1][i_5][i_3+1] + C[i_3+4][i_5][i_5-2] * C[i_5+2][i_3+2][i_3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4326312321_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
