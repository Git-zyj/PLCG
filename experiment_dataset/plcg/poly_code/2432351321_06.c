...
void kernel_2432351321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N; i_0++) {
        A[i_0] = E[i_0+1] + E[i_0-1] * F[i_0] * E[i_0] - F[i_0] - A[i_0+1] - B[i_0-2] * B[i_0] * D[i_0-3] + 1;
        for (int i_1 = 2; i_1 < -i_0+PB_N; i_1++) {
            B[i_1] = E[i_1+i_0] * G[i_1][i_0] - H[i_0] - I[i_1+i_0][i_0] - I[i_1][i_0] + B[i_1+1] * D[i_1-2] - 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_L+1, PB_N-2); i_3++) {
            C[i_3] = G[i_3+i_2-1][1] + E[i_3] - G[i_2][1] - C[i_3+2] + 5;
        }
    }
    for (int i_4 = 3; i_4 < PB_M; i_4++) {
        D[i_4-1] = H[i_4] - E[i_4] + H[i_4] + F[i_4] + D[i_4-2] * D[i_4-3] + 2;
    }
#pragma endscop
...
kernel_2432351321_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
