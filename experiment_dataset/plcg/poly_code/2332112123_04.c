...
void kernel_2332112123_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 3; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            C[2][i_2-2] = D[i_3][i_2] + C[2][i_2-3] - 5;
        }
    }
#pragma endscop
...
kernel_2332112123_04(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
