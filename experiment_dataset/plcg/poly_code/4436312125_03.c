...
void kernel_4436312125_03(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_M-1; i_4++) {
            B[i_4+1][i_3] = B[i_4-1][i_3] + 5;
        }
    }
    for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
        C[i_5+2] = E[i_5] * E[i_5-2] + C[i_5] * 3;
...
kernel_4436312125_03(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E));
