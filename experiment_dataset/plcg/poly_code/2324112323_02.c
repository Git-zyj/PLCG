...
void kernel_2324112323_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-4; i_0++) {
        A[i_0+2] = D[i_0+1] - D[i_0] * A[i_0] + A[i_0+4] * 6;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            C[i_1+1][0] = C[i_1+1][0] + 6;
        }
        C[i_1+1][0] = D[i_1] + C[i_1-1][0] * C[i_1][0] - C[i_1+3][0] + 6;
    }
#pragma endscop
...
kernel_2324112323_02(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
