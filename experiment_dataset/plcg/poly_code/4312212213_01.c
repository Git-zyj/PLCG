...
void kernel_4312212213_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
        ...
        for (int i_1 = 0; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
                for (int i_3 = 1; i_3 < -i_1+PB_L; i_3++) {
                    B[i_3-1][i_0][2] = D[i_0][i_2+2][i_3+i_1][i_1+1] * 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-2; i_4++) {
        for (int i_5 = i_4; i_5 < PB_P; i_5++) {
            for (int i_6 = 2; i_6 < PB_M+1; i_6++) {
                B[i_6-1][i_4][2] = B[i_6-2][i_4][2] * 3;
            }
        }
        C[i_4] = E[i_4-1] * F[i_4+2] * C[i_4-1] + C[i_4+1] + 4;
    }
#pragma endscop
...
kernel_4312212213_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
