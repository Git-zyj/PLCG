...
void kernel_4414151223_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
...
#pragma scop
        ...
        for (int i_1 = 1; i_1 < min(min(-i_0+PB_Q, i_0), PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < PB_M; i_2++) {
                for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
                    B[i_3+1] = E[i_3][i_2][i_1+i_0][i_0-1] - E[i_2][i_1-1][i_3-1][i_0] * E[i_3][i_0-1][i_3][i_3] + E[i_1+1][i_2][i_0-1][i_3] - E[i_2][i_3][i_0][i_1] * 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_M; i_5++) {
            B[i_5+1] = F[i_5][0] * F[i_4][i_5+1] - F[i_5-1][i_4] + G[i_5+1] + F[i_5+i_4][i_4] * G[i_5] + B[i_5+2] * 3;
            for (int i_6 = i_4; i_6 < PB_L-1; i_6++) {
                B[i_4+1] = H[i_5][i_6][1] * H[i_4-1][i_6][0] * H[i_4][i_6][i_5] + E[i_5][i_4][i_6][0] + H[i_6][i_4-1][i_4] + F[i_6][i_5] * B[i_6+1] * 4;
            }
        }
        for (int i_7 = i_4; i_7 < PB_M; i_7++) {
            D[i_4][i_7] = F[i_4][i_7] - F[i_4][i_7-1] * B[i_4+1] * 5;
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414151223_03(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
