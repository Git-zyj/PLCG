...
void kernel_3334252311_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] - D[i_0] * D[i_0-1] + E[0] - E[i_0-1] - E[i_0] * A[i_0-1] + 3;
        B[i_0] = E[i_0-2] * E[i_0] + D[i_0] - D[i_0-1] - B[i_0+1] - B[i_0-1] + 1;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        for (int i_2 = i_1; i_2 < min(-i_1+PB_Q-2, PB_L); i_2++) {
            for (int i_3 = 2; i_3 < min(min(PB_L-3, -i_2+PB_P-2), -i_1+PB_P-3); i_3++) {
                C[i_1+1][i_3+2][i_3+i_1+2] = F[i_3+1][i_2+i_1+2][i_1-2] * F[i_3-2][i_2+i_1][i_2] * F[i_2][i_1-2][i_3] + G[i_1][i_3][i_2+1][i_3+2] * C[i_2+2][i_3+1][i_3+i_2+2] * C[i_1+1][i_3+1][i_3+i_1+1] * C[i_1+1][i_3+3][i_3+i_1+3] + C[i_1+2][i_3+2][i_3+i_1+3] - C[i_3+2][i_1+1][i_3+i_1+2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3334252311_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
