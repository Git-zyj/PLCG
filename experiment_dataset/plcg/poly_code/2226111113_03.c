...
void kernel_2226111113_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, PB_L-1); i_3++) {
            B[i_2][i_3+1] = C[i_3-1][i_2] + B[i_2-1][i_3] * B[i_2][i_3] * 6;
        }
    }
#pragma endscop
...
kernel_2226111113_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
