...
void kernel_4436331225_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,int wi,DATA_TYPE POLYBENCH_5D(I,xI,yI,zI,vI,wI,xi,yi,zi,vi,wi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = E[i_0] - A[i_0] - A[i_0+1] * A[i_0-3] * 2;
        for (int i_1 = 1; i_1 < -i_0+PB_Q+1; i_1++) {
            for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
                for (int i_3 = i_0; i_3 < min(-i_1+PB_Q+1, PB_L); i_3++) {
                    B[i_2][i_2][i_3-1] = F[i_2][i_0-1][i_3+i_1-1][i_0] - G[i_1][i_3-1][i_2-1][i_0] - G[i_0][i_3][i_2][i_1+i_0-1] * G[i_1][i_2+1][i_0+1][i_2-1] * H[i_2][i_0-1][i_2][i_0] + B[i_2-2][i_2-2][i_3-1] * 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_P; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M; i_5++) {
            for (int i_6 = 0; i_6 < min(i_4, PB_L); i_6++) {
                for (int i_7 = i_6; i_7 < min(PB_L, -i_5+PB_Q); i_7++) {
                    C[i_6][i_4][i_7+i_5] = G[i_5][i_7][i_7][i_4] - 4;
                }
            }
        }
    }
        ...
        for (int i_9 = max(i_8, 1); i_9 < PB_L; i_9++) {
            for (int j_0 = 1; j_0 < -i_8+PB_Q; j_0++) {
                for (int j_1 = 1; j_1 < PB_L; j_1++) {
                    B[j_1][j_1][j_0-1] = I[i_9][j_0-1][i_8][i_9-1][j_1] + H[i_9+1][j_1][i_8][j_0] - C[i_9][j_1-1][j_0+i_8] - 2;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4436331225_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,wi,POLYBENCH_ARRAY(I)));
