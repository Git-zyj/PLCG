...
void kernel_3234112325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            A[i_1] = A[i_0+1] + A[i_1-1] * A[i_1-2] + A[i_1+2] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        B[i_2+2][i_2-1] = C[i_2] - B[i_2+4][i_2+1] * 5;
    }
#pragma endscop
...
kernel_3234112325_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
