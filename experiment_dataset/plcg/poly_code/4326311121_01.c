...
void kernel_4326311121_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_1][i_0][i_2] = D[i_1][i_2+1][i_0-1] - 6;
            }
            for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
                for (int i_4 = 0; i_4 < i_0; i_4++) {
                    A[i_3+2][i_0-1][i_1+2] = 5;
                }
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_L; i_5++) {
        for (int i_6 = i_5; i_6 < PB_Q-2; i_6++) {
            for (int i_7 = i_6; i_7 < PB_N-2; i_7++) {
                C[i_7][i_5][i_6] = C[i_5][i_7][i_6+2] - C[i_7+2][i_5][i_6] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4326311121_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
