...
void kernel_3336332321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0-2] - E[i_0+1] * B[i_0-1] + B[i_0] + B[i_0+2] - B[i_0-2] * 3;
        B[i_0] = F[i_0] * E[i_0] - A[i_0] - A[i_0-1] - A[i_0+2] + A[i_0+1] - 5;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_P, i_1); i_2++) {
            for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
                C[i_1+1][i_3+i_2+1][i_2+i_1-1] = G[i_3][i_2][i_1] + H[i_2][i_3][i_1][0] - I[i_3+1][i_1][i_2] * G[i_3-2][i_2+i_1][i_3] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3336332321_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
