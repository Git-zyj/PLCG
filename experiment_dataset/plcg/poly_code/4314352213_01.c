...
void kernel_4314352213_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < min(min(-i_1+PB_M, i_0), -i_0+PB_L-1); i_2++) {
                for (int i_3 = 1; i_3 < PB_N; i_3++) {
                    A[i_3-1][i_1][i_0-2][0] = D[i_3][i_1][i_0][i_2+1] * E[i_2+i_1][i_0][i_3][i_1-2] - F[i_1+1][i_3][i_3+2] - E[i_2-2][i_3][i_1][i_0-1] * F[i_1][i_2+i_0+1][1] * A[i_3-1][i_1][i_0-1][0] - A[i_3][i_1][i_0-1][0] - A[i_3-1][i_2+1][i_0-1][0] + 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_N; i_5++) {
            B[i_5] = G[i_5+i_4][1] * G[i_4][i_5] + G[i_5+i_4][i_4] + H[i_5][i_4+1] - B[i_5-1] + 2;
        }
        for (int i_6 = 2; i_6 < min(min(i_4, PB_M-2), -i_4+PB_L); i_6++) {
            B[i_6] = H[i_6-2][i_4-2] - H[i_6+2][i_4] - H[i_6+i_4-2][i_6+i_4] + G[i_6+i_4-2][i_4] - G[i_6][i_4] * H[i_6+2][i_4+2] * 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4314352213_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
