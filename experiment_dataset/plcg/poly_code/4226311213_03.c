...
void kernel_4226311213_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = i_1; i_2 < min(-i_1+PB_M, -i_0+PB_M+1); i_2++) {
                for (int i_3 = 0; i_3 < PB_M; i_3++) {
                    A[i_2+i_1][i_2][i_0] = A[i_2+i_0-1][i_2][i_1] * 3;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-2; i_5++) {
            B[i_5+1] = C[i_5][i_4] + B[i_5+2] - B[i_5] * 6;
        }
    }
#pragma endscop
...
kernel_4226311213_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
