...
void kernel_4434251313_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_L, PB_P-1); i_2++) {
                for (int i_3 = 1; i_3 < min(-i_1+PB_L-1, -i_0+PB_P); i_3++) {
                    A[i_1][i_2][i_2+i_0][1] = E[i_2+1][i_0][i_1][i_3] * F[i_0][i_0][0] + G[i_1][i_2][i_3][i_0] * G[i_3][i_1][i_2+1][i_0] + G[i_1][i_0][i_3+i_0][i_2] - G[i_2-1][i_3][i_1][i_0] * A[i_1][i_2][i_2+i_0-1][1] - A[i_1-1][i_2-1][i_2+i_0-1][1] * A[i_1][i_3-1][i_3+i_2-1][1] * A[i_2][i_3+1][i_3+i_1+1][1] * 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 2; i_5 < min(i_4, -i_4+PB_L-2); i_5++) {
            for (int i_6 = 0; i_6 < min(min(-i_4+PB_L+1, PB_P-1), -i_5+PB_M-2); i_6++) {
                for (int i_7 = 2; i_7 < PB_L-1; i_7++) {
                    B[i_6+i_5][i_4-1][i_7-1][1] = G[i_6][i_7+1][i_4][i_5] - G[i_6][i_7][i_5][i_4] - B[i_6+i_5+2][i_4-1][i_7-1][1] - B[i_6+i_5-2][i_4-1][i_7-2][1] + B[i_6+i_5+1][i_4-1][i_7-1][1] - B[i_6+i_5][i_4-2][i_7-2][1] * 2;
                }
                for (int i_8 = 0; i_8 < PB_Q-1; i_8++) {
                    A[i_6+1][i_5+1][i_5+i_4+2][1] = E[i_8][i_4-1][i_6+1][i_4] - G[i_8][i_6+i_4-1][i_8+i_5][i_5+1] + E[i_4][i_5][i_4][i_8+1] * G[i_8][i_8+1][i_5][i_6] * G[i_6][i_4][i_8][i_6] - 6;
                }
            }
        }
    }
    for (int i_9 = 1; i_9 < PB_N; i_9++) {
        D[i_9-1] = H[i_9] * I[i_9] * I[i_9+1] + H[i_9] + D[i_9] + 2;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4434251313_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
