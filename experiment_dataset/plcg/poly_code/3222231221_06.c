...
void kernel_3222231221_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0+1] = C[i_0] - C[i_0] * C[i_0-1] - D[i_0+1] + A[i_0-1] - A[i_0+3] - A[i_0] + 3;
        for (int i_1 = i_0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = PB_Q - 1; i_2 >= 0; i_2--) {
                B[i_2][i_1][i_1+1] = E[i_1][i_0][0] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3222231221_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
