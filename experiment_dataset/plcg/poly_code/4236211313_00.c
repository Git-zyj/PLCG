...
void kernel_4236211313_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                B[i_2-1][0] = C[i_1][i_0-1][i_2] - 4;
            }
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[i_3][0] = B[i_3+1][0] - B[i_3-1][0] - B[i_3+1][0] + 1;
        }
    }
#pragma endscop
...
kernel_4236211313_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
