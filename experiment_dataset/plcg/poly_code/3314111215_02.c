...
void kernel_3314111215_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        ...
        B[i_0][i_0-1] = B[i_0-1][i_0-2] * B[i_0+1][i_0] + 4;
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_N-2, -i_2+PB_M); i_3++) {
            C[i_3+1][i_2] = D[i_3+i_2][i_2] + C[i_3+2][i_2] * C[i_3+1][i_2+1] + 4;
        }
    }
#pragma endscop
...
kernel_3314111215_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
