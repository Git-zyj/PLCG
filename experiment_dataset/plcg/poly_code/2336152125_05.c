...
void kernel_2336152125_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        B[i_0-3] = D[i_0-1] - D[i_0] - D[i_0-1] - D[i_0] + D[i_0-2] * 2;
    }
    for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
        B[i_1-1] = D[i_1+1] - D[i_1] * D[i_1+1] * D[i_1] - D[i_1+1] - D[i_1] - B[i_1-2] - 3;
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        B[i_2-1] = D[i_2] * D[i_2-1] + D[i_2] * D[i_2] + 1;
    }
#pragma endscop
...
kernel_2336152125_05(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
