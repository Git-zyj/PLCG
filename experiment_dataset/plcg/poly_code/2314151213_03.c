...
void kernel_2314151213_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        A[i_0+1] = D[i_0] + D[i_0] * D[i_0-1] * E[i_0][i_0-1] - D[i_0] * D[i_0] + A[i_0+2] - A[i_0] * 4;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            B[i_1][i_2-1] = E[i_2-1][i_1] - E[i_2][i_1] * B[i_1][i_2-2] + 6;
        }
        for (int i_3 = 2; i_3 < min(min(PB_N-2, -i_1+PB_L), i_1); i_3++) {
            A[i_3+2] = E[i_3][i_3+i_1] + E[i_3][i_1-1] * E[i_1][0] * D[1] * E[i_3][i_1-1] - D[i_3+1] + E[i_1+1][0] + B[i_1][i_3-2] + 1;
        }
    }
#pragma endscop
...
kernel_2314151213_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
