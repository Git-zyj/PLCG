...
void kernel_3412232211_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0] = E[i_0-1] + E[i_0] * F[i_0] * A[i_0+1] - 6;
        for (int i_1 = 1; i_1 < min(PB_P, -i_0+PB_Q-1); i_1++) {
            for (int i_2 = 1; i_2 < i_0; i_2++) {
                B[i_0+2][i_1][i_2] = G[i_2][i_0+2] * H[i_2-1][i_0][i_1] + I[i_1][i_2+1][i_0+1] - B[i_0+2][i_2][i_1-1] + D[i_2+1][i_0+1][i_1+i_0+1] - 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 2; i_5 < -i_4+PB_Q-2; i_5++) {
                D[i_3][i_4+1][i_5+i_4+2] = I[i_5][i_4][i_3+1] - H[i_5-2][i_3][i_3+1] * 5;
            }
        }
        for (int i_6 = 2; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < min(i_6, -i_6+PB_Q); i_7++) {
                D[i_3][i_6][i_7+i_6] = H[i_7][i_3][i_6-2] * H[i_6][i_3][i_7] + D[i_3][i_6-1][i_7+i_6-1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3412232211_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
