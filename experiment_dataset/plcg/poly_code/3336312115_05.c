...
void kernel_3336312115_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_P+2, i_0); i_1++) {
            A[i_0][i_1-2] = B[i_0+3][i_0+1] - 4;
        }
        B[i_0+2][i_0] = 4;
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = 3; i_3 < min(i_2, -i_2+PB_P); i_3++) {
            C[i_3+i_2][i_3-1] = D[i_3][i_2] - D[i_3][i_3+i_2] - A[i_2][i_3-3] - C[i_3+i_2-1][i_3-2] - 3;
        }
    }
#pragma endscop
...
kernel_3336312115_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
