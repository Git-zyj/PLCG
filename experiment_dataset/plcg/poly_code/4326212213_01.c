...
void kernel_4326212213_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_M, -i_0+PB_M-2); i_2++) {
                A[i_2+i_1][i_0] = A[i_2+i_0+2][i_1+1] - 4;
            }
        }
    ...
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        C[i_4] = D[i_4-1] - C[i_4+1] + C[i_4-1] + C[i_4-1] + 4;
    }
#pragma endscop
...
kernel_4326212213_01(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
