...
void kernel_4124312221_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 2; i_2 < min(PB_Q, -i_0+PB_P+1); i_2++) {
                A[i_2][i_1+i_0][i_1-1][i_2] = B[i_1+i_0][i_2] + A[i_2][i_1+i_0+1][i_1][i_2] + A[i_1][i_2+i_0-1][i_2-2][i_1] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4124312221_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
