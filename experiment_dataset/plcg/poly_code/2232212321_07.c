...
void kernel_2232212321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-3 - 1; i_0 >= 0; i_0--) {
        for (int i_1 = 0; i_1 < min(PB_L, -i_0+PB_M-3); i_1++) {
            A[i_1][i_1+i_0+2] = C[i_0][i_1] + A[i_1+1][i_1+i_0+3] + A[i_1][i_1+i_0] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_P-4; i_3++) {
            B[i_2][i_3+2] = C[i_3-1][i_2+2] - B[i_2][i_3+4] - B[i_2-2][i_3+2] - B[i_2+2][i_3+4] - 3;
        }
    }
#pragma endscop
...
kernel_2232212321_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
