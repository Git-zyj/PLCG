...
void kernel_3424131123_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_M; i_2++) {
            B[i_1][i_2+i_1+1] = F[i_2][i_1] - F[i_2+i_1][i_2] * 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            A[i_4] = F[i_4-1][i_3-1] * F[i_4][i_3] + G[i_3-1][i_4][i_4-1] - 1;
        }
        for (int i_5 = 2; i_5 < PB_M; i_5++) {
            for (int i_6 = 1; i_6 < PB_M; i_6++) {
                D[i_3+1][i_5][i_6] = F[i_6][i_5] + G[i_5][i_3][i_5-1] - G[i_6][i_3-1][i_5] - D[i_3][i_5][i_6-1] * D[i_3+1][i_5-2][i_6+2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_3424131123_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
