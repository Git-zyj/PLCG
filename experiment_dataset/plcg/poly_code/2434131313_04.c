...
void kernel_2434131313_04(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        C[i_0+1] = E[i_0] - E[i_0] * E[i_0] + E[i_0] - 1;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < min(-i_1+PB_M-1, i_1); i_2++) {
            B[i_1][i_2+i_1] = B[i_1][i_2+i_1+1] * C[i_2+1] + 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        C[i_3+1] = E[1] - E[i_3-1] + E[i_3] * C[i_3+2] - 3;
        C[i_3+2] = E[i_3] * E[i_3] * C[i_3] - C[i_3+1] * 4;
    }
#pragma endscop
...
kernel_2434131313_04(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E));
