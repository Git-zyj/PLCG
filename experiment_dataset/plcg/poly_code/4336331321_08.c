...
void kernel_4336331321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
                A[i_2][i_1] = D[i_0][i_2-1] + A[i_2+2][i_1-1] - A[i_0+2][i_2-2] * A[i_2][i_1-2] + 3;
            }
        }
    }
    for (int i_3 = PB_M-3 - 1; i_3 >= 1; i_3--) {
        for (int i_4 = 0; i_4 < PB_Q-2; i_4++) {
            for (int i_5 = 0; i_5 < i_4; i_5++) {
                for (int i_6 = 0; i_6 < min(min(-i_3+PB_P, -i_4+PB_Q), i_3); i_6++) {
                    B[i_6+1][i_5][i_3][i_4] = E[i_3][i_5][i_6+i_4][i_5] + F[i_5][i_6][i_6+i_3][1][i_4] - B[i_6+1][i_5+2][i_3-1][i_4] + B[i_3+3][i_6][i_5][i_4] * B[i_6+1][i_5][i_3][i_4+2] - 1;
                }
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_P; i_7++) {
        for (int i_8 = 1; i_8 < min(PB_L-1, -i_7+PB_L+1); i_8++) {
            A[i_7][i_8] = G[i_8][i_7] - H[i_8+i_7-1][i_7] + H[i_7][i_8] + G[i_8+i_7][i_8+1] - I[i_7-1][i_8][i_7-1] * G[i_7-1][i_8-1] - 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4336331321_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
