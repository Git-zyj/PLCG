...
void kernel_2436231311_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            A[i_1+1] = E[0] * F[i_1][i_0] + E[i_1+i_0] - A[i_1] * A[i_1+2] + A[i_1] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            B[i_3+1][i_3+1] = G[i_3][i_2] + G[i_3][i_2-1] + E[i_2] * A[i_3+1] * 1;
        }
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            D[i_4] = H[i_4] * G[i_4-1][i_2+1] - G[i_4][i_2-1] * A[i_2] - D[i_2] + 5;
        }
        D[i_2] = E[i_2] + H[i_2] - H[i_2] + A[i_2] * B[i_2][i_2] * 3;
    }
#pragma endscop
...
kernel_2436231311_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
