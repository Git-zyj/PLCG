...
void kernel_2312232225_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        B[i_0-1] = D[i_0-1] + 5;
        B[i_0-1] = E[i_0+2][i_0+1] * F[i_0-1] * F[i_0] * D[i_0] + F[i_0-2] + 3;
    }
    for (int i_1 = 2; i_1 < PB_L; i_1++) {
        B[i_1] = D[0] * B[i_1-1] * B[i_1-2] + 1;
    }
#pragma endscop
...
kernel_2312232225_08(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
