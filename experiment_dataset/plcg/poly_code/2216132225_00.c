...
void kernel_2216132225_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_L, i_0); i_1++) {
            A[i_1] = A[i_1-1] * A[i_0-2] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        A[i_2] = C[i_2] - C[i_2-2] + C[i_2] - C[i_2+2] + C[i_2] * 3;
    }
#pragma endscop
...
kernel_2216132225_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
