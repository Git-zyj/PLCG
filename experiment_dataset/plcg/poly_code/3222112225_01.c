...
void kernel_3222112225_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 2; i_4 < min(PB_L-1, i_3); i_4++) {
            B[i_4] = C[i_4+1][i_3+1][0] * B[i_4+1] * B[i_4-1] + B[i_4-2] - 1;
        }
    }
#pragma endscop
...
kernel_3222112225_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
