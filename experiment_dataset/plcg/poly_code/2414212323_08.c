...
void kernel_2414212323_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    for (int i_0 = 4; i_0 < PB_L; i_0++) {
        ...
        for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
            C[i_0-2][i_1] = C[i_0-2][i_1-2] * C[i_0-4][i_1+2] * C[i_1-1][i_0] - 6;
            D[i_0][i_1] = D[i_0-1][i_1] + D[i_0-1][i_1-1] * D[i_0][i_1+1] + D[i_0][i_1-1] + D[i_0+1][i_1] * 5;
        }
    }
#pragma endscop
...
kernel_2414212323_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
