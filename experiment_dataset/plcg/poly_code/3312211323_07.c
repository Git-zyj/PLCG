...
void kernel_3312211323_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1][i_0] = A[i_1][i_0-1] - A[i_1-1][i_0-2] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        ...
        for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
            C[i_4] = D[i_2] * C[i_4+2] * C[i_4-1] - C[i_4-2] + C[i_4+1] * 4;
        }
    }
#pragma endscop
...
kernel_3312211323_07(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
