...
void kernel_3216151321_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 3; i_1 < PB_L-3; i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_1+PB_M, i_0), -i_0+PB_P); i_2++) {
                A[i_0+1][i_1][i_0-1] = C[i_0][i_2][i_1] + D[i_0][i_1] - D[i_0][i_2-1] - D[i_2+i_1][i_0+1] - C[i_1][i_2][i_2+i_0] - C[i_1][i_2+1][i_0] * A[i_1+3][i_2][i_1+1] + A[i_1-1][i_0][i_1-3] - A[i_0+2][i_1][i_0] * 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 1; i_4 < min(min(-i_3+PB_M, PB_L-1), i_3); i_4++) {
            for (int i_5 = 1; i_5 < -i_3+PB_L; i_5++) {
                A[i_4+1][i_5][i_4-1] = C[i_3][i_5+i_3][0] - C[i_4+i_3][i_5][i_4] - C[i_4][i_4][i_5-1] + D[i_5][i_4+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3216151321_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
