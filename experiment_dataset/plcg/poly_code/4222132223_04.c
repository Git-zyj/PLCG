...
void kernel_4222132223_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M-1, PB_L); i_2++) {
                A[i_1+2][i_0+1][i_2+1] = C[i_0][i_2][i_2] * C[i_2][i_1][i_0] + C[i_1][i_2+i_0+1][i_0+1] * A[i_1+2][i_0+1][i_2+3] - A[i_0+2][i_1+1][i_2+3] * 3;
            }
        }
        B[i_0][0] = D[i_0+1] - D[i_0+1] * D[i_0+1] - B[i_0+2][0] * 2;
    }
#pragma endscop
...
kernel_4222132223_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
