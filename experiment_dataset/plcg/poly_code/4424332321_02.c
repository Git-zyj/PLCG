...
void kernel_4424332321_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_1+1] = E[i_1][i_0] * F[i_0-2][0] - A[i_0+2][i_1+1] * 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-3; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_M; i_3++) {
            B[i_3] = G[i_3+1][i_2][i_3] + H[i_3+i_2][i_2] * H[i_3][i_2] * B[i_3-2] - C[i_2-2][i_3+1][i_3] - 2;
            for (int i_4 = 0; i_4 < min(-i_3+PB_M, -i_2+PB_L); i_4++) {
                C[i_2-2][i_3+1][i_3] = I[i_4+i_2][i_3][0][i_4+1] + H[i_4+i_3][i_3+i_2-2] + J[i_3][i_4+i_2][i_4] + B[i_3] - C[i_2-1][i_3-1][i_3-2] * 4;
            }
        }
        for (int i_5 = 0; i_5 < PB_L-3; i_5++) {
            for (int i_6 = max(1, i_5); i_6 < PB_L; i_6++) {
                D[i_5][i_2+2][i_6] = H[i_5][i_6] + G[i_5][i_2][0] + H[i_5][i_6] + C[i_2-2][i_5+3][i_5+2] - D[i_5][i_2][i_6] + D[i_5+2][i_2+2][i_6-1] + D[i_6][i_2+1][i_5+1] - D[i_5][i_2+3][i_6] + 5;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_4424332321_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J)));
