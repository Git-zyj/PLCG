...
void kernel_2336132323_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        C[i_0] = D[i_0+2] * D[i_0-1] + D[i_0] + 3;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        C[i_1+2] = D[i_1] - 2;
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        C[i_2] = D[i_2] * D[i_2] * D[i_2-2] - C[i_2-1] * C[i_2-2] * C[i_2+2] * 3;
    }
#pragma endscop
...
kernel_2336132323_02(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
