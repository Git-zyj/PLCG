...
void kernel_4334252115_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[0] = D[i_0] * D[i_0-2] - E[i_0] * E[i_0-2] + E[i_0] - E[i_0+2] - A[0] * 6;
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            A[0] = F[i_1+1][i_0+1] - G[i_1-2][i_0+1] * F[i_0][i_1] + 4;
        }
    }
    for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 2; i_4 < -i_2+PB_M; i_4++) {
                A[0] = H[i_4-2][i_2-1][i_3] + H[i_3+1][i_3][i_2-1] - H[i_4+i_2][i_3][0] * I[i_2][i_4-2][i_3+2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4334252115_02(xa,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
