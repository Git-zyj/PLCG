...
void kernel_4322251215_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        B[i_0] = D[i_0-1] * E[i_0-1] * B[i_0+1] - B[i_0] * 4;
        B[i_0] = E[i_0] - E[i_0+1] + D[i_0] - E[i_0-1] - E[i_0] * D[i_0] + E[1] * D[i_0-1] + B[i_0+1] + 3;
    }
        ...
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            for (int i_3 = 0; i_3 < min(-i_1+PB_M, -i_2+PB_M-1); i_3++) {
                for (int i_4 = 1; i_4 < PB_L; i_4++) {
                    C[i_3+i_2+1][i_4-1][i_1-1][1] = F[i_3][i_4][i_1] + G[i_4][i_3][i_1][i_3] * H[i_4][i_3+i_1][i_4-1][i_2-1] * 3;
                }
            }
        }
    }
...
kernel_4322251215_09(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
