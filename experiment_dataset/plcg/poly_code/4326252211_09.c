...
void kernel_4326252211_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        B[i_0] = D[i_0] * D[i_0] + D[i_0-1] * D[i_0] + 2;
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            B[i_1] = E[i_1][i_0-2] + E[i_1+i_0][i_1+i_0-1] - E[i_0][i_1] * F[i_1+1][i_0] * B[i_1-1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = max(1, i_2); i_3 < min(-i_2+PB_L, PB_M-1); i_3++) {
            for (int i_4 = 0; i_4 < min(min(-i_3+PB_L, PB_M-2), -i_2+PB_M-1); i_4++) {
                for (int i_5 = 1; i_5 < min(-i_3+PB_L, PB_P-1); i_5++) {
                    C[i_2][i_5+i_4][i_5][i_3+1] = G[i_5][i_4+2][i_2+2][i_4+1] - H[i_4][i_5][i_3] + G[i_2][i_5][i_3][i_4] * G[i_5][i_4+i_2+1][i_3][0] * I[i_5-1][i_3+i_2][i_4][i_4] + G[i_3-1][i_4+2][i_2+2][i_4] * C[i_2][i_5+i_3][i_3][i_4] * C[i_2][i_4+i_3][i_3+1][i_5+1] + C[i_3+1][i_5+i_4][i_5][i_2] * 3;
                }
            }
        }
    }
...
init_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4326252211_09(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
