...
void kernel_3432112323_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M; i_2++) {
                A[i_2-2][i_1+1][i_1] = A[i_0-2][i_2-1][i_2-2] + A[i_2-2][i_1-1][i_1-2] * A[i_2][i_1+1][i_1] - 3;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_L+1; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_M; i_4++) {
            B[i_4] = E[i_3-1] - B[i_4-2] - 5;
        }
    }
    for (int i_5 = 0; i_5 < PB_M-4; i_5++) {
        for (int i_6 = 0; i_6 < PB_M-3; i_6++) {
            C[i_6+2][0] = C[i_5+3][0] + C[i_6+1][0] * C[i_6+3][0] - C[i_5+4][0] + C[i_6][0] * 5;
        }
        for (int i_7 = i_5; i_7 < PB_M-1; i_7++) {
            D[i_7+1] = D[i_7] - D[i_5] - 6;
        }
    }
#pragma endscop
...
kernel_3432112323_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
