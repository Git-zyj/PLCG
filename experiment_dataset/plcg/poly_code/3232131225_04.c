...
void kernel_3232131225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        A[i_0+1] = C[i_0+1] + C[i_0] * C[i_0] * A[i_0] * A[i_0+3] + 2;
    }
    for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            A[i_2] = C[i_1+1] - C[i_1] - 3;
        }
    }
#pragma endscop
...
kernel_3232131225_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
