...
void kernel_4234152325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_N-2); i_1++) {
            A[i_1] = C[i_1+2] - D[i_1+i_0][i_1] * C[i_0] - C[0] + A[i_1-2] * A[i_1-1] + 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_M+2, PB_N+1); i_3++) {
            A[i_2] = D[i_3][i_2] * D[i_3+i_2-2][0] - C[i_3-1] * D[i_2][0] - D[i_3][i_2] * A[i_2] * 1;
        }
    }
#pragma endscop
...
kernel_4234152325_04(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
