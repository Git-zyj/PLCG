...
void kernel_2312331325_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        A[i_0] = D[0] * E[i_0] * E[i_0+1] - A[i_0-1] + 3;
    }
    for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
            B[i_1][i_2] = F[i_2+1][i_1+1] * E[i_2] - A[i_2] * B[i_1][i_2+2] - B[i_1][i_2+1] - B[i_2+2][i_1-2] - B[i_1+1][i_2] * 4;
        }
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            B[i_1][i_3] = G[i_1-1][i_3] + D[i_3-1] * H[i_3][i_1-1] - F[i_3][i_1] - 2;
        }
    }
#pragma endscop
...
kernel_2312331325_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
