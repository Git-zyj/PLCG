...
void kernel_3324351315_08(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        C[i_0][i_0] = D[i_0] + E[i_0] * C[i_0-1][i_0-1] * 5;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            C[i_1+1][i_1+1] = F[i_1+1][i_1+1] + G[i_2][i_1] * G[i_2][i_1+1] + H[0][i_1+1] * 4;
        }
        for (int i_3 = 0; i_3 < -i_1+PB_N-1; i_3++) {
            C[i_1][i_1] = F[i_1+1][i_1+1] + E[i_3+i_1] + E[i_3+1] - F[i_3+i_1+1][i_3] - H[i_3][i_3+i_1] * G[i_3][i_1+1] + I[i_3+1] * C[i_1-1][i_1-1] - C[i_1+1][i_1+1] - 5;
        }
    }
#pragma endscop
...
kernel_3324351315_08(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
