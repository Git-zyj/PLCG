...
void kernel_4312152113_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
    ...
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_P, -i_0+PB_M-1); i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_1+PB_M, PB_P), -i_0+PB_L); i_2++) {
                A[i_2+i_1][i_1-1] = D[i_1+1][i_2][i_0+1] + D[i_2][i_0+1][i_1] + D[i_1][i_2+i_0][i_2] - D[i_2+i_1][i_0][i_1] - 3;
            }
            B[i_0-2][i_1] = E[i_1][i_0] * E[i_1+i_0+1][i_0] * D[i_0][i_1+1][i_0-1] - A[i_1+i_0][i_1-2] + 3;
        }
    }
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 2; i_4 < PB_P+2; i_4++) {
            C[i_3][1] = F[i_4+2] - F[i_3] * E[i_4][i_3] + E[i_4-2][i_3] + E[i_4][i_3+1] + E[i_3][i_4-2] - 2;
        }
    }
#pragma endscop
...
kernel_4312152113_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
