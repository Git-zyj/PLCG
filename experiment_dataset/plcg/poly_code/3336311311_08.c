...
void kernel_3336311311_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_M; i_1++) {
            A[i_1][i_0][i_1-1] = B[i_1][i_0][i_1] * 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                B[i_4][i_3][i_4] = B[i_4][i_2+1][i_4] * 3;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 1; i_6 < i_5; i_6++) {
            for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
                C[i_5][i_6][i_7-1] = D[i_7+1][i_6-1][0] + A[i_6][i_5][i_6-1] * B[i_5][i_6][i_5] + B[i_7+1][i_6][i_7+1] - C[i_5][i_6+1][i_7-1] - C[i_6][i_7+1][i_5-1] + C[i_5+1][i_6+1][i_7-1] - C[i_5][i_6][i_7] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3336311311_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
