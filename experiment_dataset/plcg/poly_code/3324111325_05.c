...
void kernel_3324111325_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
                A[i_0][i_2][i_1] = D[i_1+1][i_0][i_2] * A[i_0+1][i_2+1][i_1] * A[i_0-2][i_2][i_1] * A[i_0][i_2][i_1+2] * 2;
            }
        }
    }
    ...
    for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
        for (int i_7 = 0; i_7 < i_6; i_7++) {
            for (int i_8 = 2; i_8 < -i_6+PB_L; i_8++) {
                A[i_6+1][i_8-2][i_7] = D[i_7][i_8+i_6][i_7] + 3;
            }
        }
    }
#pragma endscop
...
kernel_3324111325_05(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
