...
void kernel_2326112113_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = PB_M-2 - 1; i_2 >= 0; i_2--) {
        for (int i_3 = 2; i_3 < PB_P+2; i_3++) {
            C[i_2+1][i_3-2] = D[i_2] - D[i_3+2] * C[i_2+2][i_3-2] * 2;
        }
    }
#pragma endscop
...
kernel_2326112113_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
