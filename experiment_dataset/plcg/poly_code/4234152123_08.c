...
void kernel_4234152123_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_N; i_1++) {
            A[i_1] = C[i_0][i_1] + C[i_1-1][i_0-2] * D[i_1] * A[i_1-2] * 3;
            A[i_1] = C[i_1][i_0] - D[i_1+1] - C[i_1+i_0][i_0+1] * C[i_1+i_0][1] * C[i_1][i_0+2] * C[i_0][i_0] * 6;
        }
    }
#pragma endscop
...
kernel_4234152123_08(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
