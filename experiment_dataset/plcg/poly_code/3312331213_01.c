...
void kernel_3312331213_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            A[i_1] = D[i_0+1][i_1][0] * A[i_1-1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_Q; i_2++) {
        A[i_2] = E[i_2-1] + F[i_2-1] - F[i_2-1] * 4;
        for (int i_3 = 1; i_3 < -i_2+PB_Q; i_3++) {
            A[i_3] = G[i_2][i_3] - D[i_2][i_3-1][i_2+1] + H[i_3][i_3+i_2] + A[i_3] * 6;
        }
    }
#pragma endscop
...
kernel_3312331213_01(xa,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
