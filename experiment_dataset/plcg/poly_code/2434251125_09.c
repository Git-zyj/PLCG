...
void kernel_2434251125_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[1] * F[i_0] + E[i_0] - E[i_0] + E[i_0] - G[i_0][i_0] + F[i_0] * F[i_0+1] * E[i_0] * A[i_0-1] * 1;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_1][1] = H[i_1-1][i_0][i_0] - G[i_1][i_1-1] + I[i_1][i_0] - B[i_1+1][1] - 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        A[i_2] = F[i_2+1] - E[0] - E[i_2+1] + F[i_2+1] * 6;
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            D[i_4+1] = I[i_4][i_3] * G[i_4][i_3] - 5;
        }
    }
#pragma endscop
...
kernel_2434251125_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
