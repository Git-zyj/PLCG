...
void kernel_4414351213_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm),int xn,int yn,int zn,int vn,DATA_TYPE POLYBENCH_4D(N,xN,yN,zN,vN,xn,yn,zn,vn)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[i_0] * E[i_0] + F[i_0] * G[i_0] + G[i_0] + H[i_0][0] * A[i_0-1] + 5;
        for (int i_1 = 0; i_1 < -i_0+PB_N; i_1++) {
            B[i_0][i_1] = I[i_1+i_0][i_1+1] + J[i_1][i_0-1] - I[i_1][i_1] * E[i_1] + A[i_1] * 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-2; i_2++) {
        for (int i_3 = PB_Q-1 - 1; i_3 >= 2; i_3--) {
            C[i_2][i_3-1] = K[i_3][i_3][i_2] * J[i_3][i_2-1] - H[i_3][i_2] * G[i_3] * B[i_3-1][i_2-1] * C[i_2][i_3-2] + C[i_2+1][i_3-1] * 1;
            for (int i_4 = 1; i_4 < -i_3+PB_M; i_4++) {
                for (int i_5 = 1; i_5 < -i_4+PB_Q; i_5++) {
                    D[i_3][i_2+1][i_5+i_4][i_2+1] = L[i_5-1][i_5][i_3][i_4] - L[i_5][i_2][i_3-1][i_4] + M[i_5-1][i_4][i_3][i_2] - N[i_4+i_3][i_5][i_2][i_4-1] * N[i_5][i_3][i_2+1][i_4] - L[i_4-1][i_5][i_2+1][i_3+1] + D[i_3][i_2+2][i_5+i_4-1][i_2+2] * D[i_3-1][i_2+1][i_5+i_4][i_2+1] - 4;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,POLYBENCH_ARRAY(N), INIT_SEED);
kernel_4414351213_06(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,vl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), xn,yn,zn,vn,POLYBENCH_ARRAY(N));
