...
void kernel_2216351321_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0-1] = C[i_0] + C[i_0-1] * D[i_0+1] - C[i_0] + E[i_0+1] + A[i_0-2] + B[i_0+2] + B[i_0] - B[i_0-1] + B[i_0-2] * 4;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = -i_1+PB_M - 1; i_2 >= max(2, i_1); i_2--) {
            B[i_2] = F[i_2][i_1-1] * G[i_2][i_1] - H[i_1][i_2] + G[i_2][i_1] - H[i_2+i_1][i_2] * B[i_2-2] * 2;
        }
    }
#pragma endscop
...
kernel_2216351321_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
