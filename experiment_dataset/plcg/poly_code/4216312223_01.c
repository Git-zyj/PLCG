...
void kernel_4216312223_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_Q-2, i_0); i_1++) {
            for (int i_2 = 0; i_2 < min(min(PB_Q-2, i_0), -i_1+PB_Q-1); i_2++) {
                for (int i_3 = 0; i_3 < -i_2+PB_Q-2; i_3++) {
                    A[i_0+1][i_0][i_3+i_2][i_3+2] = A[i_0+1][i_0][i_2+i_1+1][i_1+2] - A[i_0+1][i_0][i_3+i_2+2][i_3+2] * 4;
                }
            }
        }
        B[i_0] = C[i_0] - B[i_0-1] + 6;
    }
#pragma endscop
...
kernel_4216312223_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
