...
void kernel_2416232215_02(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        B[i_0] = E[i_0] * 2;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            B[i_1] = F[i_0][i_1+2] - G[i_1+i_0][i_1+1] - F[i_0-1][i_1] - G[i_0][i_1] + B[i_1+1] + B[i_1-1] - 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = i_2; i_3 < PB_P; i_3++) {
            B[i_3] = F[i_2][i_3] * H[i_2][i_3][2] * 1;
            B[i_2+1] = F[i_2-2][i_3-2] * F[i_2+2][i_3] + F[i_3-1][i_2] - G[i_2-2][i_3] * I[i_3] - 3;
        }
    }
#pragma endscop
...
kernel_2416232215_02(xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
