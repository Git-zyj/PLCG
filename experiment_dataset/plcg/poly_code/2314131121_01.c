...
void kernel_2314131121_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P, -i_0+PB_L); i_1++) {
            A[i_1+i_0][i_0] = D[i_1][i_0] + E[i_1] * F[i_1+i_0][i_1+1][i_0] * B[i_0][i_1+2] + B[i_0][i_1] - 4;
            B[i_0][i_1] = D[i_1][i_0] - D[i_1][i_0+1] - E[i_1+1] - D[i_0][i_1] - D[i_1+i_0][i_1] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L+1; i_3++) {
            C[i_2][i_2-1] = D[i_3+i_2-1][i_2] + C[i_2-1][i_2-2] - 6;
        }
    }
#pragma endscop
...
kernel_2314131121_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
