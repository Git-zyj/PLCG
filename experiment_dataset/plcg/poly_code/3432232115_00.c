...
void kernel_3432232115_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        A[i_0+2] = E[i_0+2] - F[i_0-1] - F[i_0-1] - E[i_0-1] + E[i_0+2] - A[i_0+3] - 3;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(i_1, -i_1+PB_Q); i_2++) {
            for (int i_3 = 2; i_3 < PB_Q+2; i_3++) {
                B[i_3+2][i_1][i_2-1] = G[i_1][i_3][i_2+i_1][i_3-2] * H[i_2][i_1][i_3] - B[i_3+2][i_1][i_2] * 1;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L+1; i_4++) {
        for (int i_5 = 2; i_5 < PB_P; i_5++) {
            for (int i_6 = 0; i_6 < PB_P; i_6++) {
                B[i_6+2][i_4-1][i_5-1] = I[i_4][i_6][i_5] + I[i_6][i_4][i_5-2] * H[i_5-1][i_6][i_4] + 5;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_P; i_7++) {
        for (int i_8 = 1; i_8 < PB_L; i_8++) {
            for (int i_9 = 0; i_9 < min(i_7, -i_7+PB_M+1); i_9++) {
                B[i_9+2][i_8][i_7] = J[i_9+i_7-1][i_8-1] * 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_3432232115_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J)));
