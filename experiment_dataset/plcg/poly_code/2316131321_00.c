...
void kernel_2316131321_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        ...
        for (int i_2 = 1; i_2 < min(-i_0+PB_M, PB_L); i_2++) {
            A[i_0-2][i_2-1] = D[i_0+1][i_2-1] - D[i_2][i_0] * D[i_2][i_0] + D[i_2+i_0][0] * 4;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_L; i_4++) {
            C[i_3+1][i_4] = D[i_4-1][i_4-1] + D[i_3+1][i_4] * D[i_4][i_3] - C[i_3+1][i_4-2] - C[i_3][i_4] + C[i_3+2][i_4] * 3;
        }
    }
#pragma endscop
...
kernel_2316131321_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
