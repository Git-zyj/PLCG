...
void kernel_3332352211_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,int vl,DATA_TYPE POLYBENCH_4D(L,xL,yL,zL,vL,xl,yl,zl,vl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0] * D[i_0] - E[i_0] + A[i_0-1] + A[i_0+1] * 1;
        for (int i_1 = 0; i_1 < -i_0+PB_P; i_1++) {
            for (int i_2 = i_0; i_2 < PB_M-2; i_2++) {
                B[i_0][i_1][i_2+1] = F[i_2+2][i_1] * F[i_2][i_1+i_0] * G[i_0-2][i_2+2][i_1] - H[i_2][i_1+i_0][i_0][i_2+i_1+1] - G[i_1][i_2-2][1] * B[i_0+1][i_1][i_2] + B[i_2][i_1+1][i_0+2] * 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = 1; i_4 < min(PB_P-1, -i_3+PB_Q); i_4++) {
            for (int i_5 = 2; i_5 < -i_4+PB_M; i_5++) {
                C[i_5+i_4][i_5] = G[i_5][i_5][i_4+i_3] + I[i_5+1][i_4][i_3] * J[i_5][i_5+i_4] * K[i_5+i_4][i_5+1][i_3] + L[i_3][i_5][i_4][i_3] * G[i_4-1][i_5-2][i_3+1] - B[i_5][i_4+1][i_3+1] - 3;
            }
        }
    }
#pragma endscop
