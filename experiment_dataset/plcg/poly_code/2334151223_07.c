...
void kernel_2334151223_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_L-2; i_1++) {
            A[i_1][i_1+i_0][1] = D[i_1+i_0][i_0] * D[i_0+1][i_1-1] - E[i_0] + D[i_0][i_1] * D[i_1+1][i_0+1] + A[i_1][i_1+i_0+2][1] * A[i_1-1][i_1+i_0-1][1] * 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-3; i_2++) {
        for (int i_3 = 1; i_3 < min(-i_2+PB_L-3, i_2); i_3++) {
            B[i_2][i_3+i_2+1] = E[i_3] * D[i_3+1][i_2] + D[i_2][i_3] + F[i_2+1][i_3-1][0] * D[i_3+1][i_2] + D[i_3][i_2] + B[i_2][i_3+i_2+3] + 1;
        }
        for (int i_4 = 1; i_4 < PB_L; i_4++) {
            C[0] = D[i_4-1][i_2] * D[i_2+1][i_4] - D[i_2-1][i_4-1] - D[i_4-1][i_2-1] + C[0] - 6;
        }
    }
#pragma endscop
...
kernel_2334151223_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
