...
void kernel_3336131323_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_N; i_0++) {
        A[i_0-1] = D[i_0-1] - D[i_0] - A[i_0-2] * A[i_0-3] * 3;
    }
    for (int i_1 = 3; i_1 < PB_N-1; i_1++) {
        C[i_1] = D[i_1] + D[i_1+1] * A[i_1-3] - 5;
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < min(i_2, -i_2+PB_M); i_3++) {
            C[i_2] = E[i_3][i_2] - E[i_3+i_2][i_3] + E[i_3][i_2] * C[i_3+2] * C[i_2-1] - C[i_3-1] + 4;
        }
    }
#pragma endscop
...
kernel_3336131323_09(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
