...
void kernel_3322252223_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        A[i_0+1] = D[i_0+2] * E[i_0] * E[i_0] + E[i_0] * D[i_0-1] + E[i_0] * A[i_0-1] * 3;
    }
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = 1; i_3 < min(PB_L, -i_1+PB_P-1); i_3++) {
                C[i_1][i_2+4][i_3+i_1+1][i_2+2] = F[i_3][0] * G[i_3][i_1+1] * F[i_3-1][i_2-2] + H[i_2][i_3+i_1][i_2] * I[i_2][i_3+i_1][i_1] * 6;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_P-2; i_4++) {
        for (int i_5 = 1; i_5 < min(PB_L, -i_4+PB_P+1); i_5++) {
            for (int i_6 = 2; i_6 < min(min(-i_5+PB_P-1, -i_4+PB_P), PB_L+1); i_6++) {
                C[i_6][i_4+2][i_6+i_5+1][i_4] = I[i_6][i_4][i_5] * H[i_6-1][i_4][i_5] + C[i_6-2][i_5+1][i_6+i_4][i_5-1] + C[i_4][i_6+1][i_5+i_4-1][i_6-1] + 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3322252223_04(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
