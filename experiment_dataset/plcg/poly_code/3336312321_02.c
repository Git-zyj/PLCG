...
void kernel_3336312321_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
    ...
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        ...
        for (int i_1 = 1; i_1 < min(i_0, PB_L-1); i_1++) {
            for (int i_2 = 2; i_2 < PB_M+1; i_2++) {
                B[i_2-2][i_1+1] = B[i_2-2][i_1] + B[i_2-2][i_1-1] - B[i_2-1][i_1+1] - 2;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < -i_3+PB_M; i_4++) {
            C[i_3][i_4] = D[i_4+i_3][i_3] * C[i_4+2][i_3] * C[i_3-2][i_4+2] * C[i_3+1][i_4] * C[i_4-1][i_3-1] + C[i_4-1][i_3] + C[i_3][i_4-2] * 4;
        }
    }
#pragma endscop
...
kernel_3336312321_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
