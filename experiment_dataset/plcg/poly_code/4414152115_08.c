...
void kernel_4414152115_08(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
        ...
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = 3; i_3 < PB_M; i_3++) {
                    D[i_3-3] = E[i_3][i_0+2][i_2][i_0-2] - F[i_3][i_1+2][i_0-1][i_0][i_2] - E[i_0][i_2][0][i_1] * 4;
                }
            }
        }
        for (int i_4 = 2; i_4 < -i_0+PB_L; i_4++) {
            D[i_4-1] = G[i_4][i_4+i_0-1] + H[i_0+1] * G[i_4+1][i_0] + H[i_4] + G[i_4-2][i_0] - G[i_4+i_0][i_0] - D[i_4-2] - 5;
        }
    }
    for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
        for (int i_6 = i_5; i_6 < PB_L; i_6++) {
            for (int i_7 = 0; i_7 < -i_6+PB_Q; i_7++) {
                D[i_5-1] = E[i_5][i_6][i_7+i_6][i_5] * I[i_7][i_5+1][i_6-2] * 5;
            }
        }
        D[i_5-2] = H[i_5] - H[i_5-1] - H[i_5-1] * H[i_5] * G[i_5][i_5] * H[i_5] + 3;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4414152115_08(xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
