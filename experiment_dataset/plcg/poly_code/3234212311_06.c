...
void kernel_3234212311_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_Q, -i_0+PB_N); i_2++) {
                B[i_2+i_0] = C[i_1][i_2][i_0+2] - 3;
            }
        }
        for (int i_3 = 0; i_3 < min(i_0, -i_0+PB_N+2); i_3++) {
            B[i_3+i_0-2] = D[i_0][i_3+2] * B[i_3+i_0-2] * B[i_3+i_0-3] + B[i_3+i_0-3] + 6;
        }
    }
#pragma endscop
...
kernel_3234212311_06(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
