...
void kernel_4436231325_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_1][i_0-1] = E[i_0] * F[i_1][i_1] - G[i_0][i_1] * H[i_1] + 3;
            for (int i_2 = 0; i_2 < -i_0+PB_P; i_2++) {
                A[i_2][i_1-1] = I[i_1][i_2+1][i_0+1] + I[i_0][i_2+i_0][i_1] - A[i_1][i_0-1] + 2;
            }
        }
        for (int i_3 = 0; i_3 < i_0; i_3++) {
            D[i_3] = F[i_3+1][i_0+1] + G[i_3][i_0+1] * G[i_3+1][i_0+1] - A[i_3][i_0-1] + 6;
        }
    }
    for (int i_4 = 2; i_4 < PB_M-2; i_4++) {
        D[i_4] = E[i_4+1] + E[i_4-1] * E[i_4] - D[i_4+2] + D[i_4-2] + D[i_4+1] - 1;
    }
#pragma endscop
...
kernel_4436231325_01(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
