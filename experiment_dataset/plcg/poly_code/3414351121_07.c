...
void kernel_3414351121_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0-1] = E[i_0] * E[i_0] * F[i_0-1] + F[i_0] + G[i_0][0] * 5;
    }
    for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
        B[i_1-1] = E[i_1+1] - H[i_1] + F[i_1-1] * H[i_1-1] + E[i_1] * E[i_1+1] * F[i_1] * E[i_1-1] + C[i_1-1] * 2;
        C[i_1] = E[1] * E[i_1] + E[i_1] * G[i_1-1][i_1] - B[i_1-1] * C[i_1+2] + 4;
        for (int i_2 = 0; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < i_1; i_3++) {
                D[i_3][i_2][i_2] = I[i_3][i_1][i_1-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3414351121_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
