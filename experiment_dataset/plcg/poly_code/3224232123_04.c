...
void kernel_3224232123_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                B[i_2][i_1+i_0+2] = C[i_1][i_2][0] - C[i_2+i_0][i_1][i_2+i_0] - D[i_2][i_0-1][i_1] - 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_M; i_4++) {
            for (int i_5 = 0; i_5 < min(min(-i_3+PB_M, PB_L-2), i_3); i_5++) {
                B[i_4][i_5+i_3] = E[i_3-1][i_5+2] + D[i_4+i_3][i_4][i_5] + D[i_4][i_5][i_5] - B[i_4-1][i_5+i_3] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3224232123_04(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
