...
void kernel_4416151215_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0+1] + E[i_0+1] - E[i_0] - E[i_0-1] + E[i_0] * E[i_0+1] * A[i_0-1] - A[i_0+1] * A[i_0+1] - 4;
        for (int i_1 = 1; i_1 < PB_Q+1; i_1++) {
            for (int i_2 = i_0; i_2 < -i_1+PB_L; i_2++) {
                B[i_0][i_1+1][i_2] = F[i_1][i_0][i_2] + F[i_0-1][i_2-1][i_2+i_1] + F[i_1+1][i_0][i_2] + B[i_0][i_1+2][i_2] + C[i_0][i_2][i_1-1] * 4;
            }
        }
        for (int i_3 = i_0; i_3 < PB_Q; i_3++) {
            for (int i_4 = 0; i_4 < min(-i_3+PB_N, -i_0+PB_L); i_4++) {
                C[i_0][i_4][i_3] = F[i_3-1][i_4][i_4+i_0] + G[i_4][i_3+1] - F[i_3][i_4+i_0][0] + F[i_4+i_0][i_3][0] - F[i_4+i_3][i_0][i_0+1] - C[i_0-1][i_4][i_3] + 2;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_L-1; i_5++) {
        for (int i_6 = i_5; i_6 < min(PB_Q, -i_5+PB_M); i_6++) {
            for (int i_7 = max(1, i_5); i_7 < min(-i_5+PB_N, PB_L+1); i_7++) {
                B[i_7][i_6+2][i_5+1] = F[i_7+i_5][i_6][i_5+1] * F[i_5+1][i_6][i_6] - F[i_7][i_6+i_5][i_7-1] * H[i_6][i_7][i_5+1][i_6] * 2;
            }
        }
    }
#pragma endscop
...
kernel_4416151215_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
