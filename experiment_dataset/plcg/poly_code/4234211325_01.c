...
void kernel_4234211325_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_M; i_2++) {
                A[i_2][i_1+1][i_0-1] = A[i_0][i_1][i_2-1] + A[i_2][i_1+1][i_0-3] + A[i_0-2][i_1+1][i_2-1] + 2;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < min(PB_L-1, i_3); i_4++) {
            for (int i_5 = 1; i_5 < PB_P+1; i_5++) {
                for (int i_6 = 0; i_6 < min(PB_M-1, -i_5+PB_M+1); i_6++) {
                    A[i_6+1][i_4+1][i_5-1] = C[i_4-1][i_6+i_5-1][i_3] + 6;
                }
            }
        }
    }
...
kernel_4234211325_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
