...
void kernel_2422231211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0] = E[1] * F[i_0] - F[i_0] - G[i_0][i_0-1] - A[i_0+1] * 6;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            C[i_2] = C[i_2-1] - C[i_1-1] + 6;
        }
        C[i_1-1] = F[i_1] - F[i_1] + E[i_1] * H[i_1+1][i_1-1] - A[i_1] + 4;
    }
    for (int i_3 = PB_P - 1; i_3 >= 0; i_3--) {
        D[i_3] = G[i_3][i_3+1] + H[i_3+1][1] + D[i_3+1] * 1;
    }
#pragma endscop
...
kernel_2422231211_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
