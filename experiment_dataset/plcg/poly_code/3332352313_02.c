...
void kernel_3332352313_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = D[i_0] - D[i_0+2] * D[i_0] + D[i_0+2] - E[i_0] + A[i_0-1] - A[i_0+1] + A[i_0+1] - A[i_0+1] - 2;
    }
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 2; i_3 < PB_M; i_3++) {
                B[i_3][i_1+1] = F[i_2][i_3][i_1] + G[i_3][i_1][i_2] - H[i_3-2][i_1][i_2-1] * F[i_3][i_1][0] + B[i_3-1][i_2+1] + 6;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_N; i_4++) {
        A[i_4+1] = E[i_4] * D[i_4] - D[i_4] + E[i_4] - A[i_4-1] - 1;
    }
#pragma endscop
...
kernel_3332352313_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
