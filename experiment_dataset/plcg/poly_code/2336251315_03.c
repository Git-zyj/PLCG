...
void kernel_2336251315_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        B[i_0] = D[i_0+1] * D[i_0] - D[i_0] + D[0] + 3;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_L+1; i_2++) {
            B[i_2] = E[i_1] - D[i_2] + F[i_1-1][i_2][0] * F[i_1][i_2][i_1] - G[i_2][i_2+i_1-1] + B[i_2-1] - B[i_2+1] + 5;
        }
        B[i_1-1] = E[i_1] * E[i_1-1] + D[i_1] - H[i_1][1] * E[i_1] * E[i_1+1] - B[i_1-1] * 4;
    }
#pragma endscop
...
kernel_2336251315_03(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
