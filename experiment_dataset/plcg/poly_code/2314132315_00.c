...
void kernel_2314132315_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L; i_0++) {
        A[i_0-2] = D[i_0] * D[i_0-2] - D[i_0] * D[i_0] - D[i_0] + D[i_0] + A[i_0-1] - A[i_0-3] - A[i_0-1] * A[i_0-3] + 2;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 0; i_2 < i_1; i_2++) {
            B[i_2][i_1] = E[i_2][i_1+1] + B[i_1-1][i_2] - 5;
        }
        for (int i_3 = i_1; i_3 < -i_1+PB_L; i_3++) {
            B[i_3][i_1+1] = D[i_3+i_1] + D[i_3] - 5;
        }
    }
#pragma endscop
...
kernel_2314132315_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
