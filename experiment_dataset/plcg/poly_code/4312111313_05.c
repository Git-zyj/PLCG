...
void kernel_4312111313_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            B[i_1][i_1+1] = D[i_1+1] * D[i_2+1] - E[i_2-1][i_2] + A[i_1+1] - 2;
        }
        C[i_1-1] = B[i_1][i_1+1] + B[i_1+1][i_1+2] - C[i_1] - C[i_1-2] * 6;
    }
#pragma endscop
...
kernel_4312111313_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
