...
void kernel_4322211221_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            B[i_3] = D[i_3][i_2-1] - B[i_3-2] + 6;
        }
        C[i_2] = B[i_2] + C[i_2+1] - C[i_2-2] + 6;
    }
#pragma endscop
...
kernel_4322211221_03(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
