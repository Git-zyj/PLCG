...
void kernel_4212312115_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(i_0, PB_M+2); i_1++) {
            for (int i_2 = max(0, -i_1+2); i_2 < min(PB_M-1, -i_1+PB_M+2); i_2++) {
                B[i_1] = C[i_2+1][i_0][i_0] + D[i_2+i_1-2][0] * 1;
            }
        }
    }
...
kernel_4212312115_08(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
