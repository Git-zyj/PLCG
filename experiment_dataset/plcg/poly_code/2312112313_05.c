...
void kernel_2312112313_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        for (int i_1 = 3; i_1 < PB_M; i_1++) {
            A[i_1-2] = D[i_0-1][i_1] + D[i_1][i_0+1] * A[i_1-3] - B[i_0][i_1-2] + 1;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 3; i_3 < PB_L-1; i_3++) {
            B[i_2][i_3-2] = A[i_2-1] - B[i_2][i_3-3] - B[i_2+1][i_3-1] + B[i_2+1][i_3-2] - 5;
            C[i_2+2][i_3] = D[i_3-2][0] * C[i_2+1][i_3-1] + C[i_2+2][i_3+1] * 5;
        }
    }
#pragma endscop
...
kernel_2312112313_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
