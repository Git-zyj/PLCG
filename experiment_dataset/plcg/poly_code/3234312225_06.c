...
void kernel_3234312225_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < min(i_1, PB_M-1); i_2++) {
            B[i_2] = C[i_2+1] - B[i_2+1] + B[i_2-2] + 3;
        }
    }
#pragma endscop
...
kernel_3234312225_06(xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
