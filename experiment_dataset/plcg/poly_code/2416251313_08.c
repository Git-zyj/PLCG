...
void kernel_2416251313_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        A[i_0] = E[i_0] + F[i_0] + E[i_0] + F[i_0] - F[i_0] - 6;
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_N); i_1++) {
            B[i_1][i_0-1] = G[i_0][i_1] - H[i_1+i_0][i_1] * G[i_0+1][0] - A[i_0-1] + B[i_1-1][i_0-1] - B[i_1][i_0] + 3;
        }
        for (int i_2 = 1; i_2 < PB_P+1; i_2++) {
            A[i_2] = H[i_2-1][i_0] * I[i_0][i_2-1][1] * H[i_2-1][i_0-1] * F[i_2] + A[i_2+1] - 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        A[i_3] = F[i_3] * E[i_3] * F[i_3] - F[i_3] + E[i_3+1] * E[i_3] * E[i_3+1] * A[i_3] - 5;
    }
#pragma endscop
...
kernel_2416251313_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
