...
void kernel_3236212211_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L+2, i_2); i_3++) {
            A[i_2-1][i_3-2] = C[i_2][i_3] - D[i_3][0] + 1;
        }
    }
#pragma endscop
...
kernel_3236212211_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
