...
void kernel_2322111323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_L, i_0); i_1++) {
            A[i_0][i_1] = A[i_0+1][i_1-1] * A[i_0-1][i_1] * A[i_0][i_1-1] * A[i_0+1][i_1] - A[i_0+2][i_1] - 4;
        }
        B[i_0] = B[i_0-2] * 2;
    }
    for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
            C[i_3] = D[i_3][i_2+1] * C[i_3-1] * C[i_3+2] + 5;
        }
    }
#pragma endscop
...
kernel_2322111323_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
