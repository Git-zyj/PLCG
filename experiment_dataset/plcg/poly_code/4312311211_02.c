...
void kernel_4312311211_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_N; i_2++) {
                A[i_2+i_1][0] = A[i_2+i_1][0] - A[i_2+i_1][0] - 1;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_P+1; i_4++) {
            for (int i_5 = 1; i_5 < PB_M; i_5++) {
                for (int i_6 = 1; i_6 < PB_P; i_6++) {
                    B[i_5][i_6][i_6] = B[i_3][i_5-1][i_5-1] + B[i_6-1][i_4-1][i_4-1] - 2;
                }
            }
        }
        for (int i_7 = 3; i_7 < i_3; i_7++) {
            for (int i_8 = max(1, -i_7+3); i_8 < min(PB_N, -i_7+PB_L+1); i_8++) {
                C[i_3][i_8+i_7-1][i_8] = D[i_8][i_7][i_3-1] - C[i_3+1][i_8+i_7-3][i_8-1] + 6;
            }
        }
    }
#pragma endscop
...
kernel_4312311211_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
