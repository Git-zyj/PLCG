...
void kernel_3232112323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L-4; i_1++) {
        for (int i_2 = 2; i_2 < i_1; i_2++) {
            B[i_1][i_2+2][i_2] = C[i_2-2] * B[i_2][i_1+4][i_1+2] * B[i_1+2][i_2+1][i_2-1] * B[i_1+1][i_2+1][i_2-1] * 5;
        }
    }
#pragma endscop
...
kernel_3232112323_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
