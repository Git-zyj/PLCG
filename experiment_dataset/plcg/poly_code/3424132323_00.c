...
void kernel_3424132323_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0][i_0] = E[i_0] + E[i_0] + E[i_0] - A[i_0-2][i_0-2] * A[i_0-1][i_0-1] - A[i_0+1][i_0+1] * 1;
    }
    for (int i_1 = 3; i_1 < PB_Q+2; i_1++) {
        for (int i_2 = 2; i_2 < min(min(-i_1+PB_M, PB_Q-2), i_1); i_2++) {
            for (int i_3 = 2; i_3 < PB_P-2; i_3++) {
                B[i_1-2][i_3][i_3] = F[i_2][i_3+2][i_2-1] * F[i_3-2][i_2+i_1-2][i_1-2] * F[i_3][i_1-2][i_2] * F[i_2+i_1][i_3][i_2+2] + B[i_1-1][i_3][i_3] - B[i_3][i_2-1][i_2-1] * 1;
            }
            C[i_1-1][i_2-2] = G[i_2+2][i_1] + G[i_2+i_1][i_1] - B[i_2-2][i_1][i_1] + B[i_1-3][i_2+1][i_2+1] + C[i_1][i_2-2] + 4;
        }
        for (int i_4 = i_1; i_4 < PB_L-1; i_4++) {
            D[i_4+1][i_1] = G[i_4][i_1-2] - E[i_4-1] * A[i_1][i_1] + D[i_4+1][i_1-1] * 6;
        }
    }
#pragma endscop
...
kernel_3424132323_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
