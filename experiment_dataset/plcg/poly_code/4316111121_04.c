...
void kernel_4316111121_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = C[i_0][i_0+1] + 4;
        B[i_0] = C[i_0][i_0+1] - 2;
        for (int i_1 = 0; i_1 < min(i_0, PB_L-2); i_1++) {
            C[i_0][i_0+1] = D[i_1][i_0] * B[i_1+2] * 6;
        }
    }
#pragma endscop
...
kernel_4316111121_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
