...
void kernel_2234132313_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, PB_M-1); i_1++) {
            A[i_1][i_0] = C[i_1+1] - A[i_0+1][i_1] * A[i_1+1][i_0] + A[i_1][i_0-1] * A[i_1-1][i_0] + 5;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            B[i_2-2][i_3] = D[i_2][i_3] - D[i_3+2][1] + D[i_3][i_2] - D[i_3+1][i_2] * A[i_3+1][i_2] - B[i_2-2][i_3-1] + 2;
        }
    }
#pragma endscop
...
kernel_2234132313_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
