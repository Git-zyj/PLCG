...
void kernel_4226112323_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_M-3, i_0); i_1++) {
            A[i_1+2] = C[i_1-2] - A[i_1+3] * A[i_1] - B[i_0-2][i_0-1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        B[i_2-1][i_2] = C[i_2] * B[i_2+1][i_2+2] * B[i_2][i_2+1] - 1;
    }
#pragma endscop
...
kernel_4226112323_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
