...
void kernel_4216252325_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[0] = C[i_0] * C[i_0] + D[i_0+1] - D[i_0] * A[0] * A[0] * 5;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        A[0] = D[i_1] - C[i_1-2] + D[i_1] * C[i_1] * D[0] * A[0] * 2;
    }
#pragma endscop
...
kernel_4216252325_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
