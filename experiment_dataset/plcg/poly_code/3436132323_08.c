...
void kernel_3436132323_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0+1] * E[i_0+2] * E[i_0] * E[i_0+1] * A[i_0+2] + D[i_0] + 4;
    }
    for (int i_1 = 1; i_1 < PB_M-3; i_1++) {
        B[i_1+1] = E[i_1+2] - B[i_1-1] * B[i_1+2] + B[i_1+3] + B[i_1] * D[i_1] + 6;
    }
    for (int i_2 = 4; i_2 < PB_Q+2; i_2++) {
        for (int i_3 = 1; i_3 < PB_P-2; i_3++) {
            for (int i_4 = 2; i_4 < min(min(PB_Q, -i_3+PB_N), -i_2+PB_M-2); i_4++) {
                C[i_4+i_2][i_3][i_4-2] = F[i_2][i_4+i_3][i_4] - F[i_4][i_2][i_3-1] * C[i_4+i_2+2][i_3][i_4] + C[i_4+i_2-4][i_3-1][i_2-4] - C[i_4+i_2-2][i_3+2][i_2-2] - 1;
            }
        }
        D[i_2] = E[i_2+1] + E[i_2+1] + E[i_2] + E[i_2-1] + G[i_2+1][1] - D[i_2+1] * D[i_2-1] - 5;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3436132323_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
