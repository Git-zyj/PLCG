...
void kernel_2432351221_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        A[i_0+1] = E[i_0] + E[i_0+1] - F[i_0-1] - F[i_0-1] + G[i_0][i_0] + B[0] + D[i_0] * 3;
    }
    for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L+1; i_2++) {
            B[0] = H[i_2-1][i_1] + H[i_1][i_2+1] - I[i_1-1][i_1] - F[i_2-1] * G[i_2-1][i_1+1] - B[0] - C[i_2-1][i_2-1] * 6;
            C[i_1+1][i_1+1] = G[i_1][i_2] - J[i_2] - H[i_2+1][i_1] - H[i_1-1][i_1] - D[i_2] * D[i_1+1] * 3;
        }
        D[i_1] = E[i_1-1] + F[i_1+1] - E[i_1-1] - J[i_1] + B[0] - C[i_1+1][i_1+1] * 4;
    }
#pragma endscop
...
kernel_2432351221_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
