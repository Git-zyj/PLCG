...
void kernel_3114252323_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < min(-i_0+PB_M, PB_L-1); i_1++) {
            A[i_0] = B[i_1+2][i_0] * C[i_1-1][i_0][i_0+1] + B[i_1+i_0][i_0-1] * B[i_1][i_0] + B[i_0][i_1+1] - A[i_1-1] - A[i_1-2] + A[i_1+1] * 3;
        }
    }
#pragma endscop
...
kernel_3114252323_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
