...
void kernel_4416232311_08(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L; i_1++) {
            D[i_0+1] = E[i_0+1][i_1] * F[i_1-1][0] - F[i_1-1][i_0] * 2;
        }
        for (int i_2 = 1; i_2 < min(PB_N-1, -i_0+PB_L); i_2++) {
            B[i_2] = G[i_2][i_2+i_0][i_0] - B[i_2-1] * B[i_2+1] + 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_N-1; i_3++) {
        D[i_3] = H[i_3] - I[i_3] * I[i_3-2] - I[2] - D[i_3] - D[i_3-1] * 3;
        D[i_3] = I[i_3] + I[i_3] + D[i_3-1] * D[i_3+1] + 5;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4416232311_08(xd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
