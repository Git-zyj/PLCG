...
void kernel_4232311321_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = A[i_0+2] - A[i_0-1] + A[i_0-2] - A[i_0+1] - 4;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
            for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
                B[i_1][i_3-1][i_2] = C[i_2][i_1][i_3] * D[i_2+1][i_3+1][i_1] * B[i_1-1][i_3-1][i_2+2] + B[i_1][i_3-2][i_2-2] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4232311321_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
