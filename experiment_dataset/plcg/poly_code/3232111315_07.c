...
void kernel_3232111315_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < PB_Q; i_1++) {
            for (int i_2 = min(-i_0+PB_Q+1, -i_1+PB_L) - 1; i_2 >= 0; i_2--) {
                B[i_2+i_1-1][1] = C[i_1-1][i_0-1][i_2+i_0-1] * B[i_2+i_1][1] - 3;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_Q-1; i_3++) {
        for (int i_4 = 1; i_4 < PB_Q; i_4++) {
            for (int i_5 = 1; i_5 < min(PB_Q, -i_4+PB_L); i_5++) {
                B[i_5+i_4][1] = C[i_4][i_5-1][i_3+1] - B[i_5+i_4-1][1] + B[i_5+i_4-1][1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3232111315_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
