...
void kernel_4414332323_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0] * E[i_0-2] - F[i_0] - F[i_0] + A[i_0-2] - A[i_0+1] - 6;
    }
    for (int i_1 = 2; i_1 < PB_P-2; i_1++) {
        for (int i_2 = i_1; i_2 < PB_P; i_2++) {
            for (int i_3 = i_2; i_3 < PB_L-2; i_3++) {
                for (int i_4 = 1; i_4 < -i_2+PB_L+1; i_4++) {
                    C[i_2][i_3+2][i_4-1] = G[i_3][i_4+i_2-1][i_2] + H[i_4+1][i_2-2][i_1][i_1] * I[i_2][i_4+1][i_1+2][1] + 6;
                }
            }
        }
        for (int i_5 = 0; i_5 < PB_L-3; i_5++) {
            for (int i_6 = i_1; i_6 < PB_L-3; i_6++) {
                C[i_1][i_5+2][i_6] = J[i_5][0][i_1+1] - K[i_5+1][i_1-2][i_6-2] - C[i_1][i_6+3][i_5] - C[i_1-2][i_5+3][i_6] * C[i_1][i_5+2][i_6-2] * 6;
            }
        }
        D[i_1] = F[i_1] + A[i_1-2] * D[i_1+2] + D[i_1+1] * D[i_1-2] * 1;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4414332323_02(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
