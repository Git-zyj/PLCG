...
void kernel_2432312325_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        A[i_0+2] = E[i_0] + A[i_0] - A[i_0+1] * 5;
    }
    for (int i_1 = 2; i_1 < PB_M-3; i_1++) {
        A[i_1+3] = 5;
        for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
            C[i_2][i_1] = F[i_1+2][i_2+2] - G[i_2][i_1] + C[i_1][i_2+1] * C[i_2-2][i_1-2] + C[i_2+1][i_1] + 5;
        }
    }
    for (int i_3 = 2; i_3 < PB_M+2; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            C[i_3-2][i_4] = H[i_4-1][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_2432312325_06(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
