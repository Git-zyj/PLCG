...
void kernel_2424212225_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-3; i_1++) {
        C[i_1+3] = C[i_1+1] * C[i_1-1] - C[i_1] + 4;
        C[i_1+1] = 3;
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 0; i_3 < min(min(PB_L, i_2), -i_2+PB_M-2); i_3++) {
            D[i_3+i_2][i_3] = E[i_3+1][i_2] - D[i_3+i_2+2][i_3] + 2;
        }
    }
#pragma endscop
...
kernel_2424212225_08(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
