...
void kernel_3232212315_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = max(1, i_1); i_2 < PB_L; i_2++) {
                A[i_0-2][i_1][i_2] = A[i_0-2][i_1][i_2-1] - A[i_0-3][i_1+1][i_2] - 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_N-2; i_3++) {
        B[i_3+1] = C[i_3+2] * B[i_3] - B[i_3+2] + B[i_3+2] + B[i_3] - 1;
    }
#pragma endscop
...
kernel_3232212315_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
