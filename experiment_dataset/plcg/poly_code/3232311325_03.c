...
void kernel_3232311325_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q+2; i_1++) {
            A[i_1-1][i_0] = A[i_1+1][i_0] - A[i_1-2][i_0] * A[i_0-1][i_1-2] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            A[i_3-1][i_2] = C[i_2][i_3] - 5;
        }
    }
#pragma endscop
...
kernel_3232311325_03(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
