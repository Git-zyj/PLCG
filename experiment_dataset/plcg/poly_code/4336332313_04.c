...
void kernel_4336332313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_L, i_1); i_2++) {
            for (int i_3 = 2; i_3 < -i_2+PB_M; i_3++) {
                for (int i_4 = 2; i_4 < min(PB_M, -i_1+PB_P); i_4++) {
                    B[i_4][i_3][i_2][i_3-2] = F[i_4][i_1][i_1][i_2-1] + G[i_3+i_2][i_4+i_1][i_2][0] + H[i_4-1][i_2+i_1][i_3][i_3+i_2] * I[i_2+i_1][i_4][i_3] * B[i_1+1][i_4][i_2][i_4-2] - B[i_4-1][i_3][i_2][i_3-2] + B[i_4][i_3+1][i_1+1][i_3-1] * B[i_4][i_3][i_2+1][i_3-2] * B[i_4][i_3][i_2-1][i_3-2] * 2;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L-2; i_5++) {
        for (int i_6 = 0; i_6 < i_5; i_6++) {
            C[i_5][i_6] = J[i_6+1] * K[i_6+2][i_5+2] * C[i_5-1][i_6] * C[i_6+1][i_5] - 6;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4336332313_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
