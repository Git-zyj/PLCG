...
void kernel_3336112225_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_M, i_0); i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                A[i_1][i_1+1] = D[i_2][i_1-1] + D[i_1+2][i_1] - 6;
            }
        }
        for (int i_3 = 0; i_3 < min(PB_M-4, i_0); i_3++) {
            B[i_3+2] = B[i_3+4] - B[i_3+3] - 5;
        }
    }
    for (int i_4 = 4; i_4 < PB_M; i_4++) {
        C[i_4-2] = C[i_4] - C[i_4-1] * C[i_4-4] * 1;
    }
#pragma endscop
...
kernel_3336112225_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
