...
void kernel_4412151223_02(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, PB_L-1); i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_L, PB_P-1); i_2++) {
                for (int i_3 = 1; i_3 < min(min(min(-i_2+PB_P+1, -i_1+PB_L), -i_0+PB_L), PB_P-1); i_3++) {
                    B[i_2][i_1-1][i_0+2][i_3+1] = E[i_2][i_1+i_0][i_3+i_2-1][1] * F[i_2+1][i_1][i_0][1][i_3-1] + E[i_2-1][i_3+i_0][i_1][i_1+1] * E[i_1][i_2+i_0][i_3-1][i_1+i_0] * E[i_0][i_1][i_2-1][i_3+1] - E[i_3][i_2][i_0][i_3] * 4;
                    B[i_2][i_1+1][i_0][i_3] = E[i_3][i_0][i_0][i_2] - E[i_2][i_0+1][i_3+1][i_1+1] * E[i_0][i_1][i_2+1][i_2] * G[i_2][i_3+i_1][i_0] * 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
            D[i_5-1] = H[i_5] * I[i_5+1][i_4] - I[i_5][i_5] - I[i_5][i_4] + D[i_5] * D[i_5+1] - 2;
        }
        D[i_4] = H[1] - H[i_4] + I[i_4][0] * D[i_4+2] - D[i_4-1] - 6;
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4412151223_02(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
