...
void kernel_3326111221_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        ...
        for (int i_2 = 2; i_2 < PB_M; i_2++) {
            B[i_2][i_0] = D[i_0][i_2] - A[i_2][i_0+1][i_2+1] * B[i_2-2][i_0] - 2;
        }
    }
...
kernel_3326111221_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
