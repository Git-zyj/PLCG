...
void kernel_3222111125_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_L, i_1); i_2++) {
                A[i_2-1][i_1][i_2+i_0] = A[i_2-1][i_1][i_2+i_0-2] - 6;
            }
        }
        for (int i_3 = 0; i_3 < PB_M; i_3++) {
            B[i_3][i_3] = C[i_0][i_3] * 3;
        }
    }
#pragma endscop
...
kernel_3222111125_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
