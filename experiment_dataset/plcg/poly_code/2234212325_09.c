...
void kernel_2234212325_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-4; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-4; i_1++) {
            A[i_0][i_1+i_0+1] = A[i_1+2][i_1+i_0+3] - A[i_0][i_1+i_0] + A[i_0+2][i_1+i_0+4] - A[i_0-1][i_1+i_0+1] * 4;
        }
    }
    for (int i_2 = 4; i_2 < PB_M; i_2++) {
        B[i_2-2] = C[i_2] + B[i_2-4] * B[i_2-1] + 2;
    }
#pragma endscop
...
kernel_2234212325_09(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
