...
void kernel_2432111311_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            A[i_0][i_1] = A[i_1-1][i_0] - A[i_0][i_1+1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            B[i_3][i_2] = E[i_3+1] + A[i_2][i_3-1] + B[i_3][i_2-1] * C[i_2+1] * 2;
        }
        C[i_2+1] = C[i_2+2] - C[i_2] * C[i_2] - C[i_2] * 4;
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            D[i_2][i_4-1] = C[i_2+1] + C[i_4+1] + C[i_4+2] + 6;
        }
    }
#pragma endscop
...
kernel_2432111311_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
