...
void kernel_3232232313_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            A[i_0][i_1][1] = C[i_0+1][i_1-1] * D[i_1] + A[i_0-1][i_1][1] - A[i_1+1][i_0][1] - A[i_0-1][i_1+1][1] * 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_P; i_3++) {
            for (int i_4 = 0; i_4 < i_3; i_4++) {
                A[i_4][i_3][1] = E[i_3+i_2][i_4+2][i_3+i_2] + E[i_2][i_4][i_3] * E[i_4][i_2][i_3-2] + 1;
            }
        }
    }
#pragma endscop
...
kernel_3232232313_08(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
