...
void kernel_4224212321_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = i_1; i_2 < PB_P-1; i_2++) {
                A[i_1][i_1+i_0][i_2][0] = A[i_1][i_1+i_0-2][i_2+1][0] * A[i_1+2][i_2+i_1][i_0+1][0] - 6;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_L; i_3++) {
        B[i_3-1] = C[i_3] - C[i_3] - B[i_3-2] + B[i_3-3] + B[i_3+1] - B[i_3] + 1;
    }
#pragma endscop
...
kernel_4224212321_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
