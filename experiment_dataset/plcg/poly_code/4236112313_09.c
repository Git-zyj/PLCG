...
void kernel_4236112313_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_1 = 0; i_1 < PB_P; i_1++) {
        for (int i_2 = 1; i_2 < min(i_1, PB_M-1); i_2++) {
            for (int i_3 = max(2, i_2); i_3 < PB_M; i_3++) {
                B[i_2][i_3] = C[i_2][i_3-2][i_1] + B[i_2+1][i_3] - B[i_2-1][i_3-1] * B[i_3-1][i_2+1] * B[i_2-1][i_3] * 6;
            }
        }
    }
#pragma endscop
...
kernel_4236112313_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
