...
void kernel_3212111221_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_N-2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_N+1, -i_1+PB_N-2); i_2++) {
                A[i_2+i_1][i_0] = A[i_2+i_0-1][i_1-2] * A[i_2+i_1+2][i_0-1] + A[i_2+i_1][i_0+1] + 6;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L-1; i_4++) {
            B[i_4+1][i_3+1] = C[i_3][i_4+1] + 2;
        }
    }
#pragma endscop
...
kernel_3212111221_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
