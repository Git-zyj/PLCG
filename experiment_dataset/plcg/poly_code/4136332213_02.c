...
void kernel_4136332213_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-4; i_1++) {
            for (int i_2 = 1; i_2 < min(min(-i_1+PB_L-2, i_0), -i_0+PB_L-1); i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_L-4; i_3++) {
                    A[i_2+i_1+2][i_3][i_3+i_2] = B[i_1][i_1+2][i_3+i_0][i_2+1] - C[i_1][i_2][i_3+i_0][0] * C[i_2][i_3+i_1-2][i_0][i_3] * A[i_2+i_0+1][i_1][i_2+i_1] + A[i_3+i_1+4][i_2-1][i_3+i_2] * 6;
                }
            }
        }
    }
...
kernel_4136332213_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
