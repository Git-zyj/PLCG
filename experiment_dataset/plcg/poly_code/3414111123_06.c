...
void kernel_3414111123_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q+1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q+1; i_2++) {
                A[i_0-1][i_0+1][i_2-1] = A[i_0-2][i_0][i_2-1] + B[i_1+2][i_2] - 3;
            }
            B[i_0+1][i_1] = E[i_0-1][i_1] - E[i_0][i_0] - A[i_1-1][i_1+1][i_0-1] * 2;
        }
        for (int i_3 = 0; i_3 < min(PB_M, -i_0+PB_L); i_3++) {
            C[i_0-1][i_3+1] = C[i_3][i_0+3] - 5;
            D[i_3] = E[i_3][i_3+i_0] * 6;
        }
    }
#pragma endscop
...
kernel_3414111123_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
