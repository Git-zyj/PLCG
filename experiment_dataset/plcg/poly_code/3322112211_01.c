...
void kernel_3322112211_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_Q; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < min(-i_0+PB_M, PB_Q); i_2++) {
                A[i_0][i_1+2][i_2] = D[i_2+i_0][i_0+2][i_1-2] - D[i_2-2][i_0-1][2] - A[i_0-1][i_1+1][i_2] + 3;
            }
        }
    }
    ...
    for (int i_6 = 1; i_6 < PB_Q; i_6++) {
        for (int i_7 = 0; i_7 < PB_Q; i_7++) {
            for (int i_8 = 0; i_8 < PB_Q; i_8++) {
                A[i_6-1][i_8+2][i_7+1] = D[i_7][i_6][0] - A[i_8][i_7+2][i_6] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3322112211_01(xa,ya,za,POLYBENCH_ARRAY(A), xd,yd,zd,POLYBENCH_ARRAY(D));
