...
void kernel_4236211121_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = i_0; i_1 < min(PB_P-1, -i_0+PB_P); i_1++) {
            for (int i_2 = i_1; i_2 < min(PB_Q, -i_0+PB_L-1); i_2++) {
                A[i_2][i_1+i_0][0] = C[i_0][i_2+1][i_1] * D[i_2+i_0+1][i_1+1] - 5;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = i_4; i_5 < PB_L-2; i_5++) {
                for (int i_6 = 1; i_6 < PB_P-1; i_6++) {
                    B[i_6-1][i_4][i_3+1] = B[i_4-1][i_5+2][i_6+1] * 4;
                }
            }
        }
    }
...
kernel_4236211121_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
