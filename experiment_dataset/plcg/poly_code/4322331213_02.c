...
void kernel_4322331213_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0+1] = D[i_0+1] + D[i_0-1] * E[i_0-1] * A[i_0] * 2;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
                for (int i_4 = 0; i_4 < PB_M; i_4++) {
                    B[i_2][i_2][i_2][i_3+1] = F[i_4][i_2+1][i_1] - G[i_2-1][i_3+1][i_4][i_1+1] + B[i_2][i_2][i_2][i_3] - 1;
                }
            }
        }
        C[i_1] = D[i_1] - D[i_1] * A[i_1] * A[i_1+1] - C[i_1-1] - 6;
    }
#pragma endscop
...
kernel_4322331213_02(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
