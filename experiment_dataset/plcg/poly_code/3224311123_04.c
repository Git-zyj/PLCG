...
void kernel_3224311123_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-4; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_Q; i_2++) {
                B[i_0-2][i_1][i_2+i_1] = 4;
            }
            for (int i_3 = 0; i_3 < min(-i_1+PB_Q, -i_0+PB_Q-4); i_3++) {
                B[i_0][i_1][i_3+i_1] = C[i_3][i_0] * B[i_1][i_0+2][i_3+i_0+4] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3224311123_04(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
