...
void kernel_3214111213_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
        for (int i_3 = 1; i_3 < PB_L-1; i_3++) {
            for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
                B[i_2-1][i_4] = C[i_3-1][i_3][i_4] + B[i_3-1][i_4+1] * B[i_4-2][i_3+1] * 2;
            }
        }
    }
#pragma endscop
...
kernel_3214111213_08(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
