        ...
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_Q-1, -i_0+PB_L-1); i_2++) {
                for (int i_3 = 1; i_3 < min(PB_P, -i_1+PB_L+1); i_3++) {
                    A[i_0][i_1-1][i_2+1][i_3-1] = D[i_3][i_2+i_0+1][i_1][i_2] - D[i_2][i_3+i_1-1][i_3][1] + E[i_1+1][i_3][i_2] * D[i_1][i_3][i_2][i_0] * 3;
                }
            }
        }
        B[i_0-1] = F[i_0] * F[0] + F[i_0-1] + F[i_0-1] - F[0] * F[i_0] * F[i_0] * B[i_0+1] * 1;
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        B[i_4-1] = F[i_4] + F[i_4] + F[i_4] - F[i_4] * 1;
    }
#pragma endscop
...
kernel_4322151125_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
