...
void kernel_4216311211_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < min(i_0, -i_2+PB_L-1); i_3++) {
                    A[i_3][i_2+i_1][i_2-1] = A[i_0][i_3+i_2+1][i_3] + A[i_1+1][i_3+i_2][i_2-1] * A[i_3-1][i_2+i_1+1][i_2-1] * 2;
                }
            }
        }
    }
...
kernel_4216311211_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
