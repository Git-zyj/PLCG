...
void kernel_3334112315_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_M-1); i_1++) {
            A[i_0][i_1] = D[i_1][i_1] * A[i_0-1][i_1] * A[i_0+1][i_1+1] * 4;
        }
        for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
            B[i_2+1] = B[i_2] - B[i_2+2] * B[i_2+2] + 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < -i_3+PB_P; i_4++) {
            for (int i_5 = 1; i_5 < PB_N; i_5++) {
                C[i_3][i_4+i_3][i_5] = A[i_5][i_3+1] - C[i_3-1][i_4+i_3-1][i_5] - C[i_3][i_4+i_3][i_5-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3334112315_06(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
