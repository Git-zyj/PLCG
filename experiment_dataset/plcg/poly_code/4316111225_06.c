...
void kernel_4316111225_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_1] = D[i_1][i_0-1] - D[i_0][i_1-1] + 2;
        }
        B[i_0] = B[i_0+2] - 2;
    }
    for (int i_2 = PB_Q-1 - 1; i_2 >= 2; i_2--) {
        for (int i_3 = i_2; i_3 < min(PB_Q, -i_2+PB_L-2); i_3++) {
            for (int i_4 = 0; i_4 < min(-i_2+PB_L, PB_M-1); i_4++) {
                C[i_3+1][i_4+i_2] = E[i_4][i_3][i_2+1] * C[i_3+1][i_4+i_2-2] - C[i_4+1][i_3+i_2+2] * C[i_4+1][i_3+i_2+1] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4316111225_06(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
