...
void kernel_3324251223_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M-1; i_0++) {
        A[i_0] = D[i_0-1] + E[1] * D[i_0+1] * 3;
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            A[i_1+1] = D[i_1-1] - F[i_0][i_1-1] * D[i_1] - G[i_1+1][i_0] * F[i_1-1][i_0+1] - G[i_1][i_1] + 4;
        }
        for (int i_2 = 2; i_2 < PB_Q-3; i_2++) {
            for (int i_3 = 1; i_3 < min(-i_0+PB_Q, -i_2+PB_Q-3); i_3++) {
                C[i_0][i_3+1][i_3+i_2] = H[i_3+1][i_0+1][i_2] + I[i_3-1][i_2][i_3+1] + F[i_2+1][i_3] * H[i_2][i_3+1][i_3-1] * C[i_0][i_3+3][i_3+i_2+3] - C[i_2-2][i_0-1][i_3+i_0-4] - C[i_2-2][i_3+1][i_3+i_0] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3324251223_02(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
