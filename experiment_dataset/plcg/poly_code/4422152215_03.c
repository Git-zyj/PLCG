...
void kernel_4422152215_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
                for (int i_3 = i_2; i_3 < min(-i_0+PB_Q+1, PB_L-1); i_3++) {
                    A[i_3][i_0-1][i_2][i_1+2] = E[i_3][i_0][i_1][i_2] - E[i_2+2][i_3+1][i_1][i_0] + F[i_1][i_2][i_3+i_0-1] + A[i_3][i_0-1][i_2-1][i_1+1] * 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_Q-1; i_4++) {
        for (int i_5 = max(i_4, 1); i_5 < PB_P+1; i_5++) {
            for (int i_6 = 1; i_6 < PB_P; i_6++) {
                D[i_6-1] = F[i_5][i_6][i_4+1] + F[i_6+1][i_5-1][i_4] * 4;
            }
        }
    }
    for (int i_7 = 2; i_7 < PB_L-1; i_7++) {
        for (int i_8 = 0; i_8 < PB_L; i_8++) {
            D[i_8] = G[i_7][i_8] * G[i_8][i_8] - G[i_7][i_8] + H[i_7] + G[i_8][i_7] * G[i_8][i_7] * D[i_7] * 3;
        }
        D[i_7] = H[i_7-2] + H[i_7+1] + H[i_7] + H[i_7+1] - H[i_7+1] - H[i_7] * D[i_7-1] + 6;
    }
#pragma endscop
...
kernel_4422152215_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
