...
void kernel_2332132315_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0+1] = D[i_0-1] * D[i_0] + D[i_0] - D[i_0] * D[i_0] - D[i_0] + A[i_0+2] - 2;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            C[i_2] = C[i_1-1] + 4;
        }
        C[i_1] = D[i_1] + C[i_1-1] * C[i_1+1] - C[i_1-1] * 2;
    }
#pragma endscop
...
kernel_2332132315_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
