...
void kernel_2414251215_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-1; i_0++) {
        A[i_0+1] = E[i_0] * E[i_0+1] * F[i_0] * F[i_0] - E[i_0+1] + A[i_0] - 4;
    }
    for (int i_1 = 2; i_1 < PB_P; i_1++) {
        B[i_1] = F[i_1] - F[i_1] + E[i_1] + F[i_1-1] * D[i_1] + D[i_1-1] - 1;
        for (int i_2 = i_1; i_2 < PB_P; i_2++) {
            A[i_2+1] = G[i_2][i_1] * G[i_2-1][i_1] * G[i_2-1][i_1] + H[i_1][i_2] * G[i_1][i_2] * I[i_1][i_2-1][i_1] * D[i_1] + D[i_1-2] - 2;
        }
        D[i_1-1] = E[i_1] - E[0] - E[i_1+1] * D[i_1-2] - 5;
    }
#pragma endscop
...
kernel_2414251215_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
