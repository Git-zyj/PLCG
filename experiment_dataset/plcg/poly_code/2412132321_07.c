...
void kernel_2412132321_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 3; i_1 < PB_M; i_1++) {
            A[i_1][i_1-1] = E[i_1+1][i_0] * A[i_1-2][i_1-3] + A[i_1+1][i_1] + 1;
        }
        for (int i_2 = 0; i_2 < min(PB_M, -i_0+PB_N); i_2++) {
            B[i_0-1][i_2] = E[i_2][i_0] * E[i_2][i_0+1] + E[i_0+1][i_2] * E[i_2+i_0][i_0] - B[i_0][i_2] - B[i_0+1][i_2] * 2;
        }
    }
    for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 2; i_4 < min(min(-i_3+PB_N, PB_P-4), i_3); i_4++) {
            C[i_3][i_4-1] = F[i_3] * E[i_3][i_4+1] * E[i_4-2][i_3] + C[i_3-2][i_4-1] * C[i_3][i_4] + C[i_4+1][i_3-1] + 1;
            D[i_4][i_3][i_4+2] = E[i_4][i_3] - E[i_3][i_4+i_3] * D[i_4-1][i_3][i_4+1] - D[i_4+2][i_3+2][i_4+4] * D[i_4+1][i_3][i_4+3] + D[i_4-2][i_3][i_4] * 3;
        }
    }
#pragma endscop
...
kernel_2412132321_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
