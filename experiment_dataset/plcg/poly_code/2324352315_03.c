...
void kernel_2324352315_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-2; i_0++) {
        A[i_0-2] = D[i_0+2] + D[i_0] - E[i_0] * F[0] - D[i_0] - F[i_0] + A[i_0-3] * A[i_0-1] - B[i_0+1] - 4;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        B[i_1] = E[i_1] + F[0] - D[i_1] * B[i_1+1] * 2;
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_M, -i_2+PB_P); i_3++) {
            B[i_3] = G[0][i_3+i_2] + D[i_2] - E[1] * G[i_3][i_2] - H[i_3][i_2] - B[i_3] * 6;
        }
    }
#pragma endscop
...
kernel_2324352315_03(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
