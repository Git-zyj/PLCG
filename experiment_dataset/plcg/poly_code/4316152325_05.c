...
void kernel_4316152325_05(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_N-3; i_2++) {
                C[i_2+3][i_1+1] = D[i_1][i_2][i_0] * D[i_2][i_0][i_1] + D[i_1-1][i_0][i_2+1] - E[i_2][i_1+1][i_0+1][i_1+1] * 6;
            }
        }
        for (int i_3 = 1; i_3 < PB_L; i_3++) {
            for (int i_4 = i_3; i_4 < PB_N-1; i_4++) {
                C[i_4+1][i_3] = D[i_4][i_3][i_0+2] * D[i_4][i_0][i_3] * D[i_4-1][i_4][i_3] * D[i_4][i_0][i_3] * F[i_0-2][i_4] - D[i_3-1][i_4+2][i_0] - 2;
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_N-1; i_5++) {
        for (int i_6 = 1; i_6 < min(-i_5+PB_L, PB_L-2); i_6++) {
            C[i_6+1][i_5] = F[i_6+i_5][0] - F[i_6+2][i_5] - F[i_6+1][i_5] + F[i_6][i_5] * G[i_6] + C[i_6+2][i_5] - C[i_6-1][i_5] * C[i_5+1][i_6+1] * 2;
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4316152325_05(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G)));
