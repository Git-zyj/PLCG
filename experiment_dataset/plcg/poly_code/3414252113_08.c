...
void kernel_3414252113_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0+2] = E[i_0+2] * E[i_0] * E[i_0-1] - F[i_0] + F[i_0] * G[i_0][i_0] * 6;
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_1][i_0] = H[i_1+1][i_0] + G[i_0+2][i_1] + G[i_1][i_0-2] * G[i_1][i_0] - A[i_1+2] - A[i_0+3] + 3;
        }
        for (int i_2 = i_0; i_2 < PB_L-1; i_2++) {
            for (int i_3 = 2; i_3 < PB_L; i_3++) {
                B[i_2-1][i_3] = I[i_2+1][i_0][i_3-2] - H[i_3][i_0-1] - 5;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < min(min(i_4, PB_L-1), -i_4+PB_L); i_5++) {
            D[i_4][i_5] = G[i_4][i_5-1] * E[i_5] + I[i_5+i_4][i_5-2][i_4+1] * H[i_5-2][i_5] + F[i_5] + D[i_4+1][i_5+1] + 6;
        }
    }
#pragma endscop
...
kernel_3414252113_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
