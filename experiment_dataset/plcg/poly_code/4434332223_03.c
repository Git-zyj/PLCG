...
void kernel_4434332223_03(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
...
#pragma scop
        ...
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                for (int i_3 = 1; i_3 < -i_0+PB_Q; i_3++) {
                    A[i_3][i_1][i_0] = E[i_0-1][i_3+1][i_1][i_1] + F[i_2][i_1][i_0+2][i_3][i_2] + G[i_1][i_0-2][i_3-1][i_2] - H[i_3][i_3+1][i_2] + G[i_2][i_3+i_0][i_1-2][i_2] * 6;
                }
            }
            for (int i_4 = 1; i_4 < min(-i_1+PB_N+2, PB_L-2); i_4++) {
                B[i_1][i_4] = E[i_0-2][i_4][i_0][i_1+1] + I[i_1-2][i_4][i_0+2] * J[i_4+i_1-2][i_0-1] * B[i_1][i_4-1] * B[i_1][i_4+2] - B[i_1+2][i_4] * 2;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M; i_5++) {
        ...
        D[i_5] = K[i_5-2] - L[i_5-1] * L[i_5] * D[i_5-2] - 4;
    }
#pragma endscop
...
kernel_4434332223_03(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
