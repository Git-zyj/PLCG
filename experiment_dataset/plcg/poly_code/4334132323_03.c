...
void kernel_4334132323_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we)){
        ...
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                for (int i_3 = 2; i_3 < -i_2+PB_L+1; i_3++) {
                    A[i_0][i_1][i_0][i_3+i_2-2] = D[i_0][i_3+i_2-1][i_1][2] + D[i_2][i_3-2][i_0-1][0] + A[i_0+1][i_1-2][i_0+1][i_3+i_2-2] + A[i_1][i_0][i_1][i_3+i_2-1] - A[i_1-2][i_0][i_1-2][i_3+i_2-2] + 1;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_P-2; i_4++) {
        for (int i_5 = 2; i_5 < i_4; i_5++) {
            for (int i_6 = 2; i_6 < PB_L+1; i_6++) {
                for (int i_7 = 0; i_7 < min(-i_6+PB_Q+2, PB_M-2); i_7++) {
                    A[i_4][i_5][i_4][i_7+i_6-2] = D[i_5-2][i_6-1][i_4][i_7+2] - D[i_7+2][i_6-1][i_4+2][i_5+2] + C[i_5][i_7][i_4] - 1;
                }
            }
        }
        for (int i_8 = i_4; i_8 < PB_L; i_8++) {
            for (int i_9 = 2; i_9 < -i_8+PB_L; i_9++) {
                for (int j_0 = i_8; j_0 < -i_8+PB_Q; j_0++) {
                    C[i_9][i_4][i_8] = D[i_4][i_9+i_8][j_0][0] * E[i_9][j_0][i_4-2][i_4][i_8] * D[j_0][i_9-2][i_4][j_0+i_8] - D[j_0+2][i_4][i_9][i_8] - C[i_8][j_0+1][i_9] * C[j_0][i_4][i_9-1] - 3;
                }
            }
        }
    }
...
kernel_4334132323_03(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E));
