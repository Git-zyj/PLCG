...
void kernel_4334212223_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(3, i_0); i_1 < PB_M+2; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
                A[i_1-2][i_2] = D[i_2+2][i_1-2] - A[i_1-3][i_2] * A[i_1-3][i_2+2] * 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            B[i_4][i_3+2] = B[i_4+1][i_3+2] * 5;
        }
    }
...
kernel_4334212223_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
