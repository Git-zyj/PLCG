...
void kernel_4222132321_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            for (int i_2 = 2; i_2 < min(-i_1+PB_Q-2, PB_M); i_2++) {
                A[i_0][i_2][i_1][i_1+i_0] = C[i_2-1][i_0][0] - C[i_2][i_0][i_1+i_0] * A[i_1-1][i_2+2][i_0][i_1+i_0-1] * A[i_0-2][i_2-2][i_1][i_1+i_0-2] - A[i_1][i_0][i_2-1][i_2+i_1-1] * A[i_2][i_0+2][i_1+2][i_2+i_1+2] + 5;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_L, PB_M); i_4++) {
            for (int i_5 = 2; i_5 < min(PB_M, -i_3+PB_L-2); i_5++) {
                B[i_4][i_5+i_3] = C[i_5-2][i_5+i_3+2][i_4] - D[i_5][i_4] + C[i_4][i_3][i_5] - B[i_5][i_4+i_3] + 2;
            }
        }
    }
#pragma endscop
...
kernel_4222132321_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
