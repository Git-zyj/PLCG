...
void kernel_2224132325_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_M, PB_N+2); i_1++) {
            A[i_1] = C[i_1+i_0] * D[i_1-2][i_0] + A[i_1+1] + A[i_1+2] - A[i_0-2] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N; i_3++) {
            A[i_3+2] = D[i_3+i_2][i_2+1] - C[i_3] - D[i_2][i_2] * 1;
        }
    }
#pragma endscop
...
kernel_2224132325_00(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
