...
void kernel_4336111313_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_0][i_0] = D[i_1+1][i_0] * A[i_0-1][i_0-1] + A[i_0+1][i_0+1] - A[i_0-1][i_0-1] + A[i_0+1][i_0+1] + 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 1; i_3 < i_2; i_3++) {
            for (int i_4 = max(0, -i_3+1); i_4 < min(-i_3+PB_P+1, PB_N); i_4++) {
                for (int i_5 = i_3; i_5 < PB_Q; i_5++) {
                    B[i_4][i_2][i_4+i_3-1][i_5] = 6;
                }
            }
        }
    }
        ...
        for (int i_7 = i_6; i_7 < PB_M-1; i_7++) {
            for (int i_8 = 0; i_8 < PB_Q-1; i_8++) {
                for (int i_9 = 0; i_9 < PB_Q-1; i_9++) {
                    C[i_7][i_8][i_8] = C[i_7][i_9+1][i_9+1] + C[i_7+1][i_8][i_8] + C[i_7][i_8+1][i_8+1] - 4;
                }
            }
        }
    }
...
kernel_4336111313_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
