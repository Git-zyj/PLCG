...
void kernel_3234112211_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        ...
        for (int i_3 = max(1, i_0); i_3 < min(-i_0+PB_Q, PB_L-1); i_3++) {
            for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
                B[i_4][i_3][i_3] = C[i_0][i_4][i_3+i_0] - C[i_3][i_4][i_4+2] + B[i_4][i_3+1][i_3+1] + B[i_4-1][i_3+1][i_3+1] * B[i_4+1][i_3-1][i_3-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3234112211_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
