...
void kernel_4236232121_09(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < min(-i_0+PB_L+1, PB_L-2); i_1++) {
            for (int i_2 = i_0; i_2 < PB_P; i_2++) {
                for (int i_3 = i_2; i_3 < min(min(-i_1+PB_L-2, -i_2+PB_L), PB_P); i_3++) {
                    B[i_3+i_1+2][0][i_2][i_0] = C[i_3+i_2][i_0][i_1][i_0] - 6;
                }
            }
            for (int i_4 = 0; i_4 < min(-i_0+PB_L, i_0); i_4++) {
                for (int i_5 = 0; i_5 < -i_1+PB_L; i_5++) {
                    B[i_5+i_1][0][i_4][i_0] = D[i_4][i_1-2][i_0+1][0] + C[i_5][i_4][i_0][i_1] * D[i_1-2][i_4+i_0][i_5][0] - E[i_1][i_4][i_5][i_0][i_4+2] + B[i_1+i_0-1][0][i_4][i_5] - 2;
                }
            }
        }
    }
...
kernel_4236232121_09(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E));
