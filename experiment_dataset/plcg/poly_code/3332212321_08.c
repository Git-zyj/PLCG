...
void kernel_3332212321_08(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        ...
        for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
            B[i_0+2][i_3] = D[i_0][i_3-2] * B[i_3+3][i_0] + B[i_3+2][i_0-2] - B[i_0+1][i_3] * B[i_0+2][i_3+2] + 2;
        }
        C[i_0] = C[i_0+1] - 3;
    }
#pragma endscop
...
kernel_3332212321_08(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
