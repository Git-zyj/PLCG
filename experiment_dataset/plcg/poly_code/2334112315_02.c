...
void kernel_2334112315_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L-3; i_1++) {
        B[i_1] = D[i_1-2] * D[i_1-2] + B[i_1+1] + 4;
        for (int i_2 = 1; i_2 < min(-i_1+PB_M-1, i_1); i_2++) {
            C[i_2+i_1][i_1+2] = D[i_1+2] + B[i_2] * B[i_1] - B[i_2-1] + C[i_2+i_1+1][i_1+3] - 6;
        }
    }
#pragma endscop
...
kernel_2334112315_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
