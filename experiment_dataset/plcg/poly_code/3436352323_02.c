...
void kernel_3436352323_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        A[i_0] = E[i_0] - E[i_0] - F[i_0] + A[i_0-2] - A[i_0-1] * A[i_0+2] - 1;
    }
    for (int i_1 = 0; i_1 < PB_N; i_1++) {
        for (int i_2 = 0; i_2 < PB_N-2; i_2++) {
            B[i_1][i_2] = A[i_2] + B[i_1][i_2+2] * 4;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M, i_3); i_4++) {
            A[i_4-2] = G[i_3][i_4] - H[i_4][i_3+2] - G[i_4][i_3-1] - I[i_4][i_3] * G[i_3-1][i_4] - G[i_3+2][i_4-2] + G[i_4][i_3] + G[1][i_3] * F[i_4+i_3] * G[i_3][i_4] * 3;
        }
    }
    for (int i_5 = 0; i_5 < PB_L; i_5++) {
        for (int i_6 = 1; i_6 < min(i_5, PB_L-2); i_6++) {
            for (int i_7 = max(1, i_5); i_7 < PB_L-2; i_7++) {
                D[i_5+2][i_7][i_6] = J[i_7][i_5][i_6+2] + G[i_7-1][i_6+2] - G[i_6][0] - K[i_6-1][i_6+1][i_7] * D[i_7+2][i_5][i_6-1] * D[i_5+2][i_7+2][i_6+1] + D[i_5+2][i_7+2][i_6] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3436352323_02(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K)));
