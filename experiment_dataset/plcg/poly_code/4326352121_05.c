...
void kernel_4326352121_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_Q, i_0); i_2++) {
                for (int i_3 = 1; i_3 < min(min(PB_N-1, -i_2+PB_L), -i_0+PB_L); i_3++) {
                    A[i_1][i_3+i_2][i_0] = D[i_2+1][i_0+2][i_1][i_3-1] - E[i_2-1][i_0-1][i_3][i_1] + F[i_1+2][i_3][i_0][i_2-1] - F[i_1+1][i_3+i_0][i_2-1][i_0] - D[i_2][i_1][i_3+1][i_0] - G[i_1][i_2][i_3][i_0+1][i_2+i_0] + F[i_1][i_3+i_0][i_0][i_2] + A[i_1][i_3+i_0][i_2] + 2;
                }
            }
        }
        for (int i_4 = 1; i_4 < -i_0+PB_M; i_4++) {
            B[i_4] = H[i_4][i_0+1] * H[i_4+i_0][1] + I[i_0] + J[i_4][i_0] - B[i_4+1] + B[i_4-1] + 1;
        }
    }
    for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
        for (int i_6 = 2; i_6 < PB_M-2; i_6++) {
            for (int i_7 = 0; i_7 < -i_5+PB_P; i_7++) {
                C[i_6+2][i_7] = H[i_5+1][i_6] + K[i_6-2][i_5][i_7+i_5] + K[i_7][i_6-2][i_5] * 3;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4326352121_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
