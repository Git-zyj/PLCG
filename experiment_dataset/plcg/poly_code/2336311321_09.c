...
void kernel_2336311321_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = PB_M-2 - 1; i_2 >= i_1; i_2--) {
            B[i_1+1][i_2] = B[i_2][i_1+2] + B[i_1+2][i_2] * B[i_1+1][i_2+2] * B[i_2+2][i_1+2] + 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_M+1; i_3++) {
        for (int i_4 = 0; i_4 < min(-i_3+PB_L, PB_M); i_4++) {
            A[i_3-1] = E[i_4][i_4+i_3] + 6;
        }
    }
#pragma endscop
...
kernel_2336311321_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
