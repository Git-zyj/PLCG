...
void kernel_4236312321_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P-2; i_1++) {
            A[i_0][i_1] = A[i_0-1][i_1+2] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < min(-i_2+PB_L, PB_L-2); i_3++) {
            for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
                B[i_2][i_3+1][i_4] = C[i_2][i_4][0] + D[i_3+i_2][i_4-2][0] - B[i_2][i_3+1][i_4+2] - B[i_2][i_3+2][i_4+1] * B[i_4+1][i_3+1][i_2] + B[i_2-1][i_3-1][i_4] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4236312321_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
