...
void kernel_2434112221_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_N; i_1++) {
            A[i_1+i_0] = E[i_0][i_1+1] - 3;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M; i_3++) {
            B[i_2-1][i_3] = E[i_3-2][i_2] + B[i_2-3][i_3-1] * B[i_2-2][i_3-2] - B[i_2][i_3] + 5;
        }
    }
    for (int i_4 = 2; i_4 < PB_M+2; i_4++) {
        C[i_4-2] = 3;
        for (int i_5 = 3; i_5 < PB_M; i_5++) {
            D[i_5] = C[i_5-1] + C[i_5-3] - D[i_5-2] + 1;
        }
    }
#pragma endscop
...
kernel_2434112221_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
