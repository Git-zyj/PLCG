...
void kernel_2236112311_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
            B[i_2][i_1][i_1+2] = C[i_2][i_1+2] - B[i_2][i_1-1][i_1+1] * B[i_2+1][i_1][i_1+2] + B[i_1][i_2+1][i_2+3] + B[i_2][i_1+1][i_1+3] + 2;
        }
    }
#pragma endscop
...
kernel_2236112311_09(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
