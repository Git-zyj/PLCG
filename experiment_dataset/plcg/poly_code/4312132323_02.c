...
void kernel_4312132323_02(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf)){
        ...
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = max(1, i_0); i_2 < PB_L-1; i_2++) {
                for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
                    A[i_0+1][i_2-1][i_1][i_3] = D[i_2-1][i_3+2][i_0][i_2] + D[i_1][i_3-1][2][i_0] * A[i_0+1][i_2-1][i_1-2][i_3+2] - A[i_2+1][i_0+1][i_1][i_3] - 4;
                }
            }
        }
    }
        ...
        for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
            for (int i_6 = 1; i_6 < PB_M; i_6++) {
                for (int i_7 = 0; i_7 < PB_P-2; i_7++) {
                    B[i_7][i_5][i_4][i_5] = B[i_6-1][i_7][i_5][i_7] + B[i_6][i_5-2][i_4][i_5-2] * B[i_6-1][i_5+1][i_7+2][i_5+1] - 6;
                }
            }
        }
        for (int i_8 = 2; i_8 < min(PB_P-2, -i_4+PB_P); i_8++) {
            for (int i_9 = 2; i_9 < -i_4+PB_L; i_9++) {
                for (int j_0 = 2; j_0 < PB_L-2; j_0++) {
                    A[i_9+1][i_4-1][i_8][j_0] = D[j_0-2][j_0+1][i_8][i_9] * E[i_8-2][i_9-2][j_0+1] - D[j_0][i_4-1][i_8+2][i_9-1] * E[i_9+i_4][i_8-2][j_0] - D[i_9+1][j_0][j_0][i_4+2] + F[i_8][j_0+2][i_9+i_4][i_8+i_4][1] - D[i_4+1][i_9][j_0-2][i_8] - 3;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), INIT_SEED);
kernel_4312132323_02(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F)));
