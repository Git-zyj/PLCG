...
void kernel_3226112223_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M-2, i_0); i_1++) {
            for (int i_2 = 2; i_2 < -i_1+PB_P+1; i_2++) {
                B[i_1+i_0][i_2-2][i_0+2] = C[i_2-1][i_0][i_1] - C[i_0][i_2+i_1-1][0] + 5;
            }
            for (int i_3 = 0; i_3 < i_1; i_3++) {
                B[i_1+i_0][i_3][i_0+2] = B[i_1+i_0+2][i_3][i_0+4] * B[i_1+i_0+2][i_3][i_1+2] * 6;
            }
        }
    }
#pragma endscop
...
kernel_3226112223_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
