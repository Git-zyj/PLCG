...
void kernel_2334132311_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        A[i_0+1] = D[i_0] - D[i_0+2] * A[i_0] + A[i_0+2] - C[i_0] - 3;
    }
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_P-3, i_1); i_2++) {
            B[i_1][i_2+2] = D[i_2] - D[0] + A[i_2+2] + B[i_1][i_2+3] + C[i_2-1] + 6;
        }
        for (int i_3 = 2; i_3 < min(-i_1+PB_M, PB_L-1); i_3++) {
            C[i_3] = E[i_3+i_1][i_3+1] - E[i_1][i_3] * E[i_3-2][i_1] + C[i_3-1] + 5;
        }
    }
#pragma endscop
...
kernel_2334132311_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
