...
void kernel_2234152221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = C[1] - D[i_0-2][i_0+2] + C[i_0-2] * C[i_0] * A[i_0-1] + A[i_0-2] * 3;
        for (int i_1 = 2; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_1+2] = D[i_1+2][i_0-2] - D[i_1+i_0+2][i_1-2] * D[i_1][i_0] * D[i_1][i_0+1] + D[i_1-1][i_0-2] * D[i_0][i_0] + 5;
        }
    }
#pragma endscop
...
kernel_2234152221_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
