...
void kernel_4214211215_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-1; i_1++) {
            for (int i_2 = i_1; i_2 < min(-i_1+PB_L-1, -i_0+PB_L); i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_2+PB_P+1, -i_1+PB_P+2), -i_0+PB_L-1); i_3++) {
                    A[i_0][i_1][i_2+i_0][i_2+i_1-1] = A[i_1+1][i_3][i_2+i_1+1][i_3+i_2-1] * A[i_0+1][i_1-1][i_3+i_0+1][i_3+i_1-2] * 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
        B[i_4+1] = C[0] * C[i_4] * B[i_4] - 2;
    }
#pragma endscop
...
kernel_4214211215_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
