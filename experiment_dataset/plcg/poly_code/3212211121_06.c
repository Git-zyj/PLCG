...
void kernel_3212211121_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
    ...
    for (int i_0 = 1; i_0 < PB_P+1; i_0++) {
        ...
        for (int i_2 = 0; i_2 < min(PB_Q-1, -i_0+PB_N); i_2++) {
            for (int i_3 = 0; i_3 < PB_P; i_3++) {
                B[i_0][i_3][i_2+1] = C[i_2+i_0][i_3][i_0-1] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3212211121_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
