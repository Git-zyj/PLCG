...
void kernel_4214152223_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        A[i_0][2] = C[i_0][0] + D[i_0] * C[i_0+1][0] + D[i_0+1] + D[i_0+1] - C[i_0][0] * A[i_0+1][2] - 2;
        for (int i_1 = 0; i_1 < PB_Q; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_L, i_1); i_2++) {
                B[i_0][i_2][0] = C[i_1][i_2] * E[i_0+1][i_2+i_1][i_0-2] * E[i_0][i_2][i_1+1] * E[i_2-1][i_0][i_1] * B[i_0][i_2+2][0] - B[i_0][i_2+1][0] * B[i_0+2][i_2][0] + 3;
            }
        }
    }
#pragma endscop
...
kernel_4214152223_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
