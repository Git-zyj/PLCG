...
void kernel_3126211213_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = min(i_0, PB_M-2) - 1; i_1 >= 0; i_1--) {
            A[i_1+1] = B[i_1+1] + A[i_1] - A[i_1+2] - 3;
        }
    }
#pragma endscop
...
kernel_3126211213_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B));
