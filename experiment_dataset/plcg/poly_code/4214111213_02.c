...
void kernel_4214111213_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1][i_0] = C[i_0] * C[i_0-1] * A[i_0][i_0+1] - 2;
    }
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 0; i_2 < PB_Q; i_2++) {
            for (int i_3 = 1; i_3 < PB_P; i_3++) {
                for (int i_4 = max(1, i_2); i_4 < PB_Q; i_4++) {
                    B[i_3][i_3-1][i_4] = B[i_3+1][i_3][i_4] - B[i_3+1][i_3][i_4-1] * 2;
                }
            }
        }
    }
...
kernel_4214111213_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
