...
void kernel_4432111321_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
        ...
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 3; i_2 < PB_M; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
                    A[i_1][i_3][i_2-1][i_0] = A[i_2][i_3+2][i_1-1][i_0+1] + A[i_3][i_1][i_2-3][i_0+2] + 4;
                }
            }
        }
        B[i_0-1] = B[i_0] - B[i_0-2] - B[i_0+1] + 1;
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < PB_L; i_5++) {
            for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
                for (int i_7 = 1; i_7 < i_6; i_7++) {
                    C[i_4][i_5][i_6+1][i_7] = C[i_4][i_5-1][i_6+1][i_7] - D[i_6+1][i_5-2][i_7-1][i_4-1] * 1;
                }
            }
        }
    }
        ...
        for (int i_9 = 1; i_9 < PB_L; i_9++) {
            for (int j_0 = 1; j_0 < PB_M-1; j_0++) {
                for (int j_1 = 0; j_1 < PB_Q; j_1++) {
                    D[j_0][i_9-1][j_1][i_8] = E[j_0-1][i_8][i_8][i_9] * D[j_0+1][i_9-1][j_1][i_8+1] - D[j_0][i_9-1][i_8+1][j_1] - D[j_0][i_9][j_1][i_8+2] - 6;
                }
            }
        }
    }
...
kernel_4432111321_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
