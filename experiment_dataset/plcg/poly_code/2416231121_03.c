...
void kernel_2416231121_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0-1] * 3;
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, PB_L-2); i_1++) {
            B[i_0][i_1] = F[i_1][i_0] + A[i_1-1] * B[i_0][i_1+1] * 5;
            B[i_0+2][i_1+2] = E[i_1+i_0] * G[i_0+1] - F[i_1][i_0] * A[i_0] + 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M; i_3++) {
            D[i_3] = F[i_2][i_2] * F[i_2-1][i_3+1] * H[i_2][i_3] + H[i_2][i_3] * 1;
        }
    }
#pragma endscop
...
kernel_2416231121_03(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
