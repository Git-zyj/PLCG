...
void kernel_3234111325_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        A[i_0] = A[i_0-1] * A[i_0-2] - A[i_0+1] - A[i_0+2] * A[i_0-2] - A[i_0+2] - 2;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                B[i_1][i_2] = C[i_2+1][i_1][i_0+1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3234111325_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
