...
void kernel_3316111311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P+1; i_1++) {
        for (int i_2 = 0; i_2 < PB_P-1; i_2++) {
            B[i_1][i_2] = 5;
            C[i_1-1][i_2] = D[i_2][i_1-1] - D[i_1+1][i_2+1] * A[i_2] + B[i_1][i_2] * B[i_1+1][i_2] + 4;
        }
    }
#pragma endscop
...
kernel_3316111311_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
