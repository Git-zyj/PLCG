...
void kernel_3414332213_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        B[i_0+2] = E[i_0-1] * F[i_0-2] - 2;
    }
    for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
            B[i_2+1] = E[i_1] + G[i_2][i_1] + E[0] - B[i_2+2] - 4;
        }
        B[i_1+1] = F[i_1-1] - E[i_1-1] - F[i_1] * 3;
        for (int i_3 = 1; i_3 < PB_Q-1; i_3++) {
            for (int i_4 = 0; i_4 < -i_1+PB_L; i_4++) {
                D[i_1+2][i_4][i_3] = H[i_3-1][i_4+i_1][i_3] + D[i_1+2][i_4+1][i_3+1] * D[i_1+2][i_4][i_3-1] - 4;
            }
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3414332213_00(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
