...
void kernel_4222212323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L; i_1++) {
            A[i_0][i_1+i_0] = C[i_1][0] + A[i_0][i_1+i_0-1] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 3; i_4 < PB_N-3; i_4++) {
                B[i_4-1][i_4+2] = B[i_4][i_4+3] + B[i_2-2][i_2+1] * B[i_4-3][i_4] - B[i_4-2][i_4+1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_4222212323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
