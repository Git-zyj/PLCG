...
void kernel_4232151225_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_P-1); i_1++) {
            A[i_1][i_1-1][i_0] = C[i_1][i_0] - C[i_1-1][i_0] - D[i_1+i_0] + C[i_1+1][i_0+1] + C[i_0][i_1] - A[i_1+2][i_1+1][i_0] - A[i_1+2][i_1+1][i_0+1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < min(min(i_2, PB_L-1), -i_2+PB_M); i_3++) {
            for (int i_4 = 1; i_4 < -i_3+PB_L; i_4++) {
                A[i_4][i_4-1][i_2] = E[i_4][i_2][i_3] - E[i_2][i_4+i_3][i_3] + E[i_4][i_2][i_3] * E[i_4][i_3+1][i_2] * E[i_3+i_2][i_4][1] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4232151225_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
