...
void kernel_3214311213_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_L-2); i_1++) {
            A[i_1][i_1] = C[i_0][i_1+1] * C[i_1+i_0][i_1] - 6;
            for (int i_2 = max(1, i_0); i_2 < PB_P; i_2++) {
                B[i_0][i_1+1][i_2] = A[i_2-1][i_2-1] + B[i_0][i_1+2][i_2-1] + B[i_0][i_1][i_2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3214311213_04(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
