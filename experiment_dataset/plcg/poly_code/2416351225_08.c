...
void kernel_2416351225_08(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_P; i_1++) {
            C[i_1-2] = E[i_1+1] - F[i_1] + E[i_1+i_0+1] + G[i_1+i_0][i_1] + H[i_1][i_0] - H[i_1][i_1] - H[i_0][i_1+i_0] + G[i_1][i_0-1] - 5;
        }
        C[i_0-3] = I[i_0] + E[i_0] * C[i_0-1] + C[i_0-2] * 1;
        for (int i_2 = i_0; i_2 < PB_L-1; i_2++) {
            C[i_2-1] = G[i_0][i_2-1] + G[i_2+1][i_0] * G[i_0+1][i_2-1] * G[i_2+1][i_0+1] - G[i_0][i_2+1] - 5;
        }
    }
    for (int i_3 = 1; i_3 < PB_N-2; i_3++) {
        D[i_3] = I[i_3+1] * F[i_3+1] * F[i_3] + D[i_3-1] + D[i_3+2] + 3;
    }
#pragma endscop
...
kernel_2416351225_08(xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
