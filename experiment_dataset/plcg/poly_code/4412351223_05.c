...
static void print_array(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm))
{
int i_0, i_1, i_2, i_3;
...
void kernel_4412351223_05(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = i_0; i_2 < min(PB_P-1, -i_1+PB_Q); i_2++) {
                for (int i_3 = 1; i_3 < min(PB_P-3, -i_2+PB_L+1); i_3++) {
                    A[i_1][i_3+1][i_2][i_0+1] = E[i_0+1][i_3+i_2-1][i_1][i_3] - F[i_3-1][i_2-1][i_1][i_3] - E[i_1][i_2][i_1][i_0+1] - G[i_1-1][i_0][i_3][i_2] - A[i_3][i_1+1][i_2][i_0] - A[i_1+1][i_0+1][i_2][i_3+3] - 2;
                }
                for (int i_4 = 1; i_4 < -i_1+PB_M; i_4++) {
                    A[i_1+1][i_4+1][i_2][i_0] = F[i_4-1][i_2-1][i_0-1][i_2+i_1] + E[i_1][i_2-1][i_0][0] * G[i_4+i_1][i_2-1][i_0][i_1] + F[i_2][i_4][i_0][i_1] * H[i_4-1][i_1][i_2] * I[i_1][i_2+1][i_4] + F[i_4+i_1][i_4][i_0][i_2] * 4;
                }
            }
        }
        for (int i_5 = 2; i_5 < -i_0+PB_P-1; i_5++) {
            C[i_0+1][i_5-1] = J[i_5] * K[i_5+1][i_0+1] - K[i_5][i_0] + L[i_5-1] * L[i_0-1] - 2;
            for (int i_6 = 3; i_6 < -i_0+PB_M; i_6++) {
                D[i_0][i_6+1][i_5+i_0+1][i_5+1] = M[i_0][i_6+i_0-1][i_5+1] + I[i_6+i_0][i_6][i_5+1] - I[i_5][i_6][i_5+i_0+1] - C[i_5+1][i_6-1] * C[i_5+1][i_6-3] * C[i_6+1][i_5-2] - C[i_5+1][i_6] + 3;
            }
        }
    }
#pragma endscop
