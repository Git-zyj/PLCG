...
void kernel_4234312321_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_P-3; i_2++) {
            for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
                B[i_3][i_2][i_2+1] = C[i_3][i_2-2] - B[i_3][i_2+2][i_2+3] + B[i_3+1][i_2][i_2+1] - B[i_3][i_2+1][i_2+2] - B[i_3-2][i_2+2][i_2+3] - 6;
            }
        }
    }
#pragma endscop
...
kernel_4234312321_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
