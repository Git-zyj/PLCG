...
void kernel_4224312211_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                B[i_1+1][i_0+1][i_1+i_0] = C[i_2+i_1][i_0][i_0-1] + C[i_1+1][i_2][i_0] + 4;
            }
        }
        for (int i_3 = 0; i_3 < min(PB_L-3, -i_0+PB_L-1); i_3++) {
            B[i_3][i_0+2][i_3+i_0] = B[i_3][i_0+3][i_3+i_0+1] * B[i_0-1][i_3+3][i_3+i_0] - 4;
        }
    }
#pragma endscop
...
kernel_4224312211_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
