...
void kernel_3214132225_08(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                B[i_0-1][i_1+3][i_2] = C[i_1+1][i_2] * D[i_1][i_2][i_0+1] + 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < -i_3+PB_Q; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q; i_5++) {
                B[i_3-1][i_4+2][i_5] = D[i_5+i_4][i_3][i_4+i_3] - D[i_4][i_3+1][i_5] + C[i_5+i_4-2][i_3+1] + D[i_3][i_5][i_4-1] + B[i_4-1][i_5+2][i_3+1] - B[i_3][i_5+3][i_4+1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_3214132225_08(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
