...
void kernel_4312352325_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-4; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_N; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_P+1, PB_Q-2); i_3++) {
                    A[i_1+1][i_1][i_0][i_3+2] = D[i_0][i_3][i_1-2][i_1] * E[i_1][i_3][i_2-1][i_0] - D[i_2-1][i_1][i_3+i_0-1][i_0] - F[i_2+i_1][i_0+1][i_3][i_2] + E[i_1+2][i_0][i_2][i_3] + G[i_3][i_1+i_0+1][i_2] - 3;
                }
            }
        }
        C[i_0+4][i_0+2] = H[i_0+1] - H[i_0] * I[0] + C[i_0+1][i_0-1] * C[i_0+2][i_0] * C[i_0][i_0-2] * 3;
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < min(PB_L-2, -i_4+PB_L); i_5++) {
            C[i_5+2][i_5] = J[i_5+i_4][0] - K[i_5][i_4] * L[i_4-1][i_5-1] - L[i_5-2][i_4] + C[i_5][i_5-2] - C[i_5+1][i_5-1] + 4;
        }
    }
#pragma endscop
