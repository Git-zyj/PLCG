...
void kernel_4436132225_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0-2][i_0] = E[i_0+2] + A[i_0][i_0+2] - A[i_0-1][i_0+1] - 4;
        }
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
                for (int i_4 = 0; i_4 < i_0; i_4++) {
                    A[i_3-2][i_3] = F[i_3-2][i_2][i_0] + F[i_0][i_2][i_3+1] * G[i_2+2][i_3+2][i_4][i_0] * 1;
                }
            }
        }
    }
    for (int i_5 = 0; i_5 < PB_M-2; i_5++) {
        C[i_5] = E[i_5] + E[i_5] - C[i_5+2] * C[i_5+1] * 6;
    }
    for (int i_6 = 0; i_6 < PB_L-2; i_6++) {
        for (int i_7 = max(i_6, 2); i_7 < PB_M; i_7++) {
            for (int i_8 = 2; i_8 < PB_M-2; i_8++) {
                A[i_8][i_8+2] = H[i_8+1][i_6] - F[i_7][i_8+2][i_7-2] + F[i_8-2][i_7][i_6+2] + H[i_8][i_7] * 2;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436132225_06(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H)));
