...
void kernel_2422251121_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1] = E[i_1-1][i_0] + F[i_1][i_1-1] * G[i_0-1] * E[i_0-1][1] + F[i_0][i_1] + H[i_1] * I[i_0-1][i_1][i_0] - E[i_1][i_0+1] - C[i_0] * 5;
        }
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            B[i_0] = E[i_0-1][i_2] - G[i_2] - J[i_2-1][i_0+1][i_0] - G[i_0] * 6;
        }
        C[i_0] = G[i_0] * H[i_0] * A[i_0] + C[i_0+2] + 1;
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = i_3; i_4 < PB_M-1; i_4++) {
            A[i_4] = F[i_4+1][1] - E[i_4+1][i_3-1] - F[i_3][i_4+1] * E[i_4][i_4] * H[i_3+1] + 2;
        }
    }
#pragma endscop
...
kernel_2422251121_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J));
