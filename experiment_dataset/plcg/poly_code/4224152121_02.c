...
void kernel_4224152121_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M-1; i_2++) {
                A[i_2+i_0+1][i_1+2][2] = C[i_1][i_0] + D[i_2][i_0][i_0] - D[i_1][i_0+2][i_0+2] * 5;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 1; i_4 < PB_M; i_4++) {
            for (int i_5 = 2; i_5 < PB_M; i_5++) {
                for (int i_6 = max(i_4, 2); i_6 < -i_3+PB_M-2; i_6++) {
                    B[i_6][i_4][i_5][i_6+2] = E[i_4][i_3][i_6][i_5] * E[i_6-2][i_5-2][i_3][i_4] * E[i_5][i_6][i_3][i_4-1] + E[i_6+i_3+2][i_4][i_4][i_5] * E[i_4][i_5][i_6][i_3+2] * E[i_6][i_3+2][i_4][i_5-2] + B[i_3-1][i_6-1][i_5][i_3+1] + 6;
                }
            }
        }
    }
...
kernel_4224152121_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
