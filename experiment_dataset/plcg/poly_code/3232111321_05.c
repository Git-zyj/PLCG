...
void kernel_3232111321_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_1 = 0; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < PB_M+1; i_2++) {
            for (int i_3 = 2; i_3 < PB_M+1; i_3++) {
                B[i_3-1][i_1] = B[i_2-1][i_3-2] * B[i_3-2][i_1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_3232111321_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
