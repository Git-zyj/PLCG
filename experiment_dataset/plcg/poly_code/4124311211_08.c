...
void kernel_4124311211_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb)){
        ...
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                for (int i_3 = 2; i_3 < i_2; i_3++) {
                    A[i_0-1][i_1][i_3][i_3-1] = B[i_2+1][i_0-1][0] + A[i_0-1][i_1][i_3-1][i_3-2] + A[i_0][i_1][i_3][i_3-1] * 1;
                }
            }
        }
    }
...
kernel_4124311211_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B));
