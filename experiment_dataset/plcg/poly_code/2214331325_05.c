...
void kernel_2214331325_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = 3; i_1 < PB_P-1; i_1++) {
            A[i_1-1] = C[i_0][i_1] * C[i_1][i_0] * D[i_1][i_1+1] + A[i_1] * A[i_1-3] + A[i_1-2] * B[i_1+1] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_P, -i_2+PB_M); i_3++) {
            B[i_3+1] = D[i_3+1][i_2] + D[i_3+i_2][0] + D[i_3][i_2] - A[i_3] - 6;
        }
    }
#pragma endscop
...
kernel_2214331325_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
