...
void kernel_2136132211_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P, -i_0+PB_M-3); i_1++) {
            A[i_1+i_0+1][0] = B[i_1+i_0][i_1+1][0] + C[i_1][i_0-1] + C[i_1][i_1] - A[i_1+i_0+2][0] + A[i_1+i_0+3][0] * 6;
        }
    }
#pragma endscop
...
kernel_2136132211_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
