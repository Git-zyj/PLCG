...
void kernel_4226311215_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < min(PB_Q, -i_0+PB_L); i_2++) {
                A[i_0][i_1+1][i_2] = C[i_2+i_0][i_1+1][i_2] * D[i_1][i_0] * A[i_0-1][i_1+1][i_2-1] + 2;
            }
        }
    }
        ...
        for (int i_4 = 0; i_4 < PB_Q-1; i_4++) {
            for (int i_5 = 0; i_5 < PB_Q-1; i_5++) {
                for (int i_6 = 0; i_6 < min(PB_Q-2, i_5); i_6++) {
                    B[i_3][i_4][i_5+1][i_6+1] = B[i_5][i_4+1][i_6+2][i_3] + B[i_3-1][i_5][i_4+1][i_6+1] - 2;
                }
            }
        }
    }
...
kernel_4226311215_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
