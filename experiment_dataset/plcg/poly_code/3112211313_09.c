...
void kernel_3112211313_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_M, -i_0+PB_P-1); i_2++) {
                A[i_1-1][i_2+i_0-1][i_1] = B[i_2][i_1] + A[i_1-1][i_2+i_0-1][i_1] + A[i_1-2][i_2+i_0+1][i_1-1] * A[i_1-1][i_2+i_0-2][i_1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3112211313_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B));
