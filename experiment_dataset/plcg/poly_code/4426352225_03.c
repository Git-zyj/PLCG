...
void kernel_4426352225_03(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,DATA_TYPE POLYBENCH_3D(M,xM,yM,zM,xm,ym,zm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 2; i_2 < min(min(-i_0+PB_M, -i_1+PB_M), PB_P-1); i_2++) {
                for (int i_3 = 0; i_3 < -i_0+PB_P; i_3++) {
                    A[i_3+i_2-2][i_1][i_2][i_0] = E[i_2+i_0][i_3][i_1+1][0] + F[i_2][i_3+i_0][i_0][i_1] * F[i_1-2][i_3][i_0][i_2] + G[i_2+i_1][i_3+i_0][i_3+i_2][i_0] * G[i_1][i_2+1][i_3][i_0-2] - A[i_3+i_0-1][i_1][i_0][i_2+1] + 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_Q-1; i_4++) {
        for (int i_5 = 2; i_5 < PB_L-2; i_5++) {
            B[i_4][i_5] = H[i_4][i_5-2] + I[i_5+2][i_4+1] * J[i_4+2][i_4] * B[i_4-2][i_5] + B[i_4][i_5-2] - 1;
        }
        for (int i_6 = 0; i_6 < min(PB_Q-2, -i_4+PB_M); i_6++) {
            for (int i_7 = 2; i_7 < PB_Q+1; i_7++) {
                for (int i_8 = 0; i_8 < min(i_6, -i_7+PB_N+2); i_8++) {
                    A[i_8+i_7-2][i_6][i_7][i_4] = E[i_8+2][i_4][i_4][i_6] * G[i_6+i_4][i_8][i_7-1][i_4] - E[i_7][i_4+2][i_8][i_6+2] * 2;
                }
            }
        }
    }
    for (int i_9 = 1; i_9 < PB_P-1; i_9++) {
        for (int j_0 = 2; j_0 < PB_L; j_0++) {
            for (int j_1 = i_9; j_1 < min(-i_9+PB_M, PB_M-2); j_1++) {
                B[j_1][j_0] = K[i_9][j_0-1][j_1] * K[j_1][j_0][i_9] * L[j_1+2][j_0][i_9] + H[j_1][j_0-2] + K[j_1+i_9][j_0][i_9+1] - M[j_0][j_1-1][i_9-1] - L[j_1+2][j_0][i_9+1] - 4;
            }
        }
    }
#pragma endscop
