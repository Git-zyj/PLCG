...
void kernel_2332311321_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        A[i_0] = A[i_0+1] * 6;
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            C[i_0][i_0+1] = D[i_1][i_0] - A[i_0] - A[i_0+1] + 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_N-1; i_2++) {
        for (int i_3 = 3; i_3 < min(-i_2+PB_N-1, PB_L-1); i_3++) {
            C[i_3-1][i_3] = E[i_3+i_2+1] * C[i_3-3][i_3-2] - C[i_3][i_3+1] * 6;
        }
    }
#pragma endscop
...
kernel_2332311321_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
