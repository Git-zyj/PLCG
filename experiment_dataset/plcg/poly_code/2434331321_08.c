...
void kernel_2434331321_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        A[i_0] = E[i_0-1] * F[i_0] * G[i_0] - A[i_0-1] * 4;
    }
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        B[i_1] = F[i_1+1] - E[i_1-1] + B[i_1-2] + B[i_1+2] * 4;
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
            C[i_2][i_3-1][i_3] = H[i_3+1][i_2] - H[i_2][i_3] + H[i_2-1][i_3] + I[i_3+1][i_2][0] * C[i_3-2][i_2-1][i_2] * 6;
        }
    }
    for (int i_4 = 3; i_4 < PB_M-1; i_4++) {
        D[i_4-1] = F[i_4] * D[i_4+1] - D[i_4] * D[i_4-3] + D[i_4-2] + D[i_4-3] - 4;
    }
#pragma endscop
...
kernel_2434331321_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
