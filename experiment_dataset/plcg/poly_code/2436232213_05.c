...
void kernel_2436232213_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
        B[i_1+2] = F[i_1] * F[i_1] * C[i_1] * C[i_1-1] - 4;
        C[i_1] = E[i_1] - E[i_1-2] * E[i_1+2] + E[i_1] * C[i_1-1] * 2;
    }
    for (int i_2 = 1; i_2 < PB_N+1; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_N-1; i_3++) {
            C[i_3+1] = G[i_3][i_2] - H[i_2-1][i_3-2] + H[i_3][i_2] - H[i_3+1][i_3] + 5;
        }
    }
#pragma endscop
...
kernel_2436232213_05(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
