...
void kernel_2426252325_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0-2] * F[i_0] - E[i_0+1] - E[i_0] - F[i_0] + F[i_0] + A[i_0-2] * A[i_0+1] + 3;
    }
    for (int i_1 = 2; i_1 < PB_L-2; i_1++) {
        for (int i_2 = 2; i_2 < min(i_1, -i_1+PB_M+2); i_2++) {
            A[i_2-2] = G[i_2-1][i_1] - G[i_2-2][i_1] * G[i_2][i_1+1] + H[i_2+i_1-2][0] - E[i_2] + F[i_2] - 6;
        }
        for (int i_3 = 2; i_3 < min(-i_1+PB_N-1, i_1); i_3++) {
            D[i_3][i_1] = H[i_1-2][0] * H[i_3-1][i_1+2] + H[i_1-1][0] - G[i_1][i_3-2] + G[i_3][i_1+2] * 6;
            D[i_1][i_3] = I[i_1-1][i_3][0] + G[i_3+i_1+1][i_3] - E[i_1] * D[i_1][i_3-2] - D[i_1][i_3+1] - D[i_1-1][i_3] * 4;
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2426252325_03(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
