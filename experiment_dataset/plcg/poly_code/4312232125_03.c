...
void kernel_4312232125_03(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        C[i_0][i_0] = D[i_0+1] * E[1] * E[i_0+2] * 4;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_N-2, i_1); i_2++) {
            for (int i_3 = 0; i_3 < PB_M; i_3++) {
                C[i_3][i_3] = F[i_2-1][i_3][i_1] - 6;
            }
            C[i_2][i_2] = G[i_2+2][i_1] - G[i_1-1][i_2] - H[i_2][i_1] + E[i_1] - C[i_2+1][i_2+1] - 6;
        }
    }
#pragma endscop
...
kernel_4312232125_03(xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
