...
void kernel_4422152225_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + E[i_0-1] + E[i_0] * E[i_0] + E[i_0] * A[i_0+2] - A[i_0-2] - 1;
        B[i_0+1] = E[i_0-1] + E[i_0] - E[i_0+1] + E[i_0-2] - E[i_0] * A[i_0+2] + B[i_0-1] + 4;
    }
    for (int i_1 = 1; i_1 < PB_N-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_N-2; i_2++) {
            for (int i_3 = 0; i_3 < min(min(PB_N-2, -i_2+PB_M+1), -i_1+PB_Q); i_3++) {
                D[i_3+1][i_2][i_1+2][i_1-1] = F[i_2+2][i_1][i_3+i_1-1] + G[i_3+i_2-1][i_1] * F[i_1+2][i_3][i_2-2] - G[i_2-1][i_1+2] + H[i_2][i_3][i_1-1][i_2-1] + G[0][i_2] + H[i_3][i_2+1][i_3+i_1][2] - F[i_3+2][i_3][i_1+1] * F[i_3][i_2+2][1] * 4;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M; i_4++) {
        for (int i_5 = 2; i_5 < PB_L+2; i_5++) {
            for (int i_6 = 0; i_6 < PB_M; i_6++) {
                for (int i_7 = max(i_4, 1); i_7 < PB_P-2; i_7++) {
                    D[i_4][i_5-2][i_7+2][i_7-1] = 5;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422152225_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H)));
