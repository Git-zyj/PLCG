...
void kernel_4436352311_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl),int xm,DATA_TYPE POLYBENCH_1D(M,xM,xm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 3; i_2 < min(-i_0+PB_L+2, PB_L-1); i_2++) {
                for (int i_3 = 2; i_3 < i_1; i_3++) {
                    A[i_0][i_3][i_2-2][i_0+1] = E[i_3-1][i_1+1][i_2][i_0] * F[i_0][i_3][i_1][i_2] * G[0][i_2][i_3][i_0-1] * G[i_2-2][i_0][i_3][i_1] - F[i_0][i_2+1][i_3-2][i_1] * E[i_2+1][i_2+i_0-2][i_3+2][i_1] + G[i_0][i_3][i_1][i_3+1] - A[i_0][i_3][i_2-3][i_0+1] * A[i_0-1][i_1-1][i_2-3][i_0] * B[i_3-1][i_2] - 6;
                }
            }
        }
        for (int i_4 = 0; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 2; i_5 < min(i_0, PB_L-1); i_5++) {
                B[i_5-1][i_0] = H[i_5][i_0] + I[i_4][i_5][i_0+1] + J[i_4+2][i_5+1][i_0] - I[i_4+1][i_5-1][i_4] + B[i_5-2][i_4+1] * 6;
            }
        }
        C[i_0-1] = K[i_0] - L[i_0] + M[i_0] + H[i_0][i_0] - L[i_0] * C[i_0-2] * C[i_0] - D[i_0+1] + 1;
    }
    for (int i_6 = 1; i_6 < PB_L; i_6++) {
        D[i_6] = L[i_6] + K[i_6] * C[i_6] + D[i_6-1] + D[i_6+1] * 1;
    }
#pragma endscop
...
kernel_4436352311_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), xm,POLYBENCH_ARRAY(M));
