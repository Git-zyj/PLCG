...
void kernel_4226152211_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(min(i_0, -i_0+PB_M+1), PB_L-2); i_1++) {
            B[i_0+1][i_1] = C[i_1+i_0-1][0] + C[i_1+i_0-1][i_1] + C[i_0-1][i_1] + C[i_0+2][i_1] * C[i_0][i_1+2] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_P-1; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            B[i_2][i_3] = C[i_3][i_2-1] - D[i_3+i_2] * C[i_3][i_2] * C[i_3][i_2] - C[i_2][1] - B[i_2-1][i_3] + B[i_3][i_2+1] * 5;
        }
    }
#pragma endscop
...
kernel_4226152211_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
