...
void kernel_3436232225_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = E[i_0] + A[i_0+1] * A[i_0+2] + A[i_0-1] - 4;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_L-1; i_2++) {
            for (int i_3 = i_2; i_3 < -i_1+PB_L; i_3++) {
                B[i_2+1][i_3+i_1][i_2] = F[i_3][i_1][i_2] + G[i_2][i_3][i_2-2][i_1] + F[i_3+i_1][i_2+2][i_3-1] + H[i_2][i_3][i_1-2] + B[i_3-1][i_2+i_1+1][i_3-2] * 6;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 3; i_5 < -i_4+PB_L; i_5++) {
            for (int i_6 = 0; i_6 < i_5; i_6++) {
                C[i_6+1][i_4][i_4-1] = I[i_6][i_4] * I[i_4-1][i_6] - C[i_6+1][i_5-2][i_5-3] * 5;
            }
            for (int i_7 = 0; i_7 < PB_M-1; i_7++) {
                C[i_7+1][i_4+1][i_4] = H[i_4][i_7][i_5] - H[i_7+1][i_5+i_4][0] * 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_3436232225_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
