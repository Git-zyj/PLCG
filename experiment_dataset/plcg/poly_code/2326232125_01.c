...
void kernel_2326232125_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        B[i_0-1] = D[i_0-2] - D[i_0] + 6;
        B[i_0-1] = D[i_0-2] - E[i_0] + B[i_0] - 2;
    }
    for (int i_1 = 4; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(PB_Q-1, i_1); i_2++) {
            C[i_1][i_1-2] = F[i_1][i_2] * G[i_2][i_1][i_2+1] * H[i_1][i_2-1] - F[i_1-1][0] * C[i_1-2][i_1-4] - 3;
        }
    }
#pragma endscop
...
kernel_2326232125_01(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
