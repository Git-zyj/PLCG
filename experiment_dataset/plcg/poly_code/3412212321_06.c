...
void kernel_3412212321_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_L+2, PB_M-1); i_1++) {
            for (int i_2 = 1; i_2 < PB_P-2; i_2++) {
                A[i_2][i_1+1][i_2] = E[i_1][i_2+2] + E[i_1+i_0-2][i_2-1] * A[i_1][i_2][i_1] + 5;
            }
        }
        B[i_0] = B[i_0-2] - B[i_0-1] + 1;
        for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
                C[i_0][i_3][i_4] = A[i_3+2][i_4+2][i_3+2] + 6;
            }
        }
    }
    for (int i_5 = 4; i_5 < PB_M+2; i_5++) {
        for (int i_6 = 4; i_6 < PB_L; i_6++) {
            D[i_5-2][i_6-2] = D[i_5-2][i_6] + D[i_5-3][i_6-2] - D[i_5-4][i_6-4] - D[i_5-3][i_6] - D[i_5-4][i_6-2] * 1;
        }
    }
#pragma endscop
...
kernel_3412212321_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
