...
void kernel_4316112123_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_M; i_1++) {
            A[i_0][i_1] = D[i_1][i_0] + A[i_0][i_1-2] + A[i_0-1][i_1] - 2;
        }
    }
        ...
        for (int i_3 = 0; i_3 < min(-i_2+PB_Q, i_2); i_3++) {
            for (int i_4 = 0; i_4 < i_3; i_4++) {
                for (int i_5 = 0; i_5 < PB_Q-1; i_5++) {
                    B[i_3][i_3][i_2-1][i_5+1] = 5;
                }
                for (int i_6 = 0; i_6 < PB_M-2; i_6++) {
                    C[i_6+2][i_4+2][i_3+i_2] = 6;
                }
            }
        }
    }
...
kernel_4316112123_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
