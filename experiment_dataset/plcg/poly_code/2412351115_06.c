...
void kernel_2412351115_06(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        D[i_0+1] = E[i_0] * F[i_0] + G[i_0-1][0] * E[i_0] * F[i_0] - F[i_0] + 4;
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            B[i_0][i_1] = H[i_1][i_0-1] + F[i_1-1] - I[i_1][i_0-1] - B[i_0][i_1+1] + 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        for (int i_3 = i_2; i_3 < min(PB_L, -i_2+PB_M-1); i_3++) {
            D[i_2] = J[i_2+1] + I[i_3+i_2+1][0] + F[i_2] + D[i_3] + 6;
        }
        D[i_2] = F[i_2+1] * F[i_2] - J[i_2] + F[i_2] + F[i_2+1] - E[i_2] - E[i_2+1] + F[i_2-1] - 3;
    }
#pragma endscop
...
kernel_2412351115_06(xd,POLYBENCH_ARRAY(D), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
