...
void kernel_4414232115_02(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M, -i_1+PB_P); i_2++) {
                for (int i_3 = i_0; i_3 < min(PB_M-2, -i_1+PB_L+2); i_3++) {
                    D[i_2][i_3-1][i_3+1][i_1] = E[i_2+i_0][i_3+i_1-2][i_2+i_1][0] - E[i_3+2][i_2][i_1][i_0] - 5;
                }
            }
        }
        B[i_0] = F[i_0-2] - G[i_0+1] * G[i_0-2] + G[i_0-1] * G[i_0] - 1;
        for (int i_4 = i_0; i_4 < PB_M; i_4++) {
            for (int i_5 = 2; i_5 < PB_M-1; i_5++) {
                for (int i_6 = i_4; i_6 < PB_M-1; i_6++) {
                    C[i_5-2][i_4][i_5+1][i_0] = H[i_5][i_6+1][i_0] + 2;
                }
            }
            for (int i_7 = 2; i_7 < PB_L-3; i_7++) {
                for (int i_8 = 0; i_8 < PB_P; i_8++) {
                    D[i_0][i_7][i_7+2][i_8] = E[i_4][i_0][i_8][i_7-2] + D[i_0][i_7+1][i_7+3][i_8] - D[i_0-1][i_7][i_7+2][i_8+1] - 6;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414232115_02(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
