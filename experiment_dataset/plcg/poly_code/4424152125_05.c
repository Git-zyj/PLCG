...
void kernel_4424152125_05(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_M+2; i_1++) {
            D[i_0][i_1-2] = E[i_1][i_1+1] - E[i_0][i_1+1] + F[i_1-1][i_0][1] + E[i_1][i_0-1] + E[i_1-2][i_0] + E[i_1+i_0-2][i_0] - 1;
        }
        for (int i_2 = 0; i_2 < min(-i_0+PB_L+2, PB_M); i_2++) {
            for (int i_3 = 0; i_3 < PB_M; i_3++) {
                D[i_2][i_0-2] = F[i_3][i_0-2][i_2] + F[i_2][i_3][i_2] - E[i_2][i_3] * F[i_2][i_3][i_2+i_0-2] * F[i_2][i_3][i_0] * 6;
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 0; i_6 < PB_M; i_6++) {
                for (int i_7 = 0; i_7 < PB_Q-2; i_7++) {
                    C[i_4][i_6][i_5][i_7+1] = G[i_4][i_6][i_5][i_7+2] * G[i_7][i_4-1][i_5][i_6] - G[i_5][i_7][i_4-2][i_6+1] + C[i_6][i_4+1][i_5][i_7+1] - 3;
                }
            }
        }
        for (int i_8 = 2; i_8 < PB_N-2; i_8++) {
            D[i_4][i_8-2] = E[i_8][i_4] * E[i_4-2][i_8] * H[i_8+2] + D[i_4+1][i_8-2] * 5;
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4424152125_05(xd,yd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
