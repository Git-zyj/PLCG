...
void kernel_2322311311_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_L+1; i_1++) {
            A[i_0][i_1-1] = D[i_0] - A[i_0+1][i_1-2] * A[i_0+1][i_1-1] - 2;
        }
        B[i_0+1] = B[i_0] + B[i_0+2] + B[i_0] - B[i_0+2] + 5;
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 0; i_3 < i_2; i_3++) {
            C[i_3][i_2+1] = C[i_2+1][i_3+2] * C[i_2][i_3+2] * 6;
        }
    }
#pragma endscop
...
kernel_2322311311_08(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
