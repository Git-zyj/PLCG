...
void kernel_4326152323_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
        ...
        for (int i_1 = i_0; i_1 < -i_0+PB_Q; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                for (int i_3 = 0; i_3 < -i_1+PB_Q; i_3++) {
                    A[i_3][i_3][i_2][i_0-2] = D[i_1][i_3][i_0][i_2+1] * D[i_2][i_0-2][i_1][i_1+i_0] + D[i_0+2][i_3][i_1][i_2] - D[i_2][i_0][i_3][i_3+i_1] * E[i_3][i_2][i_1+2] - A[i_1][i_1][i_3][i_2] * 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
        B[i_4] = F[i_4] * F[i_4+2] + F[i_4-2] * B[i_4+2] + B[i_4+1] * B[i_4-2] * 4;
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = 1; i_6 < min(PB_L, -i_5+PB_M); i_6++) {
            B[i_6] = G[i_6][i_5] * F[i_5] - G[i_6+i_5][i_6] + G[i_6+1][i_5] * G[i_5][i_6] * B[i_6-1] + 6;
        }
    }
#pragma endscop
...
kernel_4326152323_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
