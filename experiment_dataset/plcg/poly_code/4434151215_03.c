...
static void print_array(int xd,int yd,int zd,int vd,int wd,DATA_TYPE POLYBENCH_5D(D,xD,yD,zD,vD,wD,xd,yd,zd,vd,wd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh))
{
int i_0, i_1, i_2, i_3, i_4;
...
void kernel_4434151215_03(int xd,int yd,int zd,int vd,int wd,DATA_TYPE POLYBENCH_5D(D,xD,yD,zD,vD,wD,xd,yd,zd,vd,wd),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < min(min(-i_0+PB_Q, i_1), PB_L-1); i_2++) {
                for (int i_3 = 1; i_3 < min(PB_Q-2, -i_0+PB_P); i_3++) {
                    D[i_0][i_2][i_1-1][i_3+2][1] = E[i_2+1][i_0][i_1+1][i_3] + E[i_3-1][i_1][i_2+i_0][0] + E[i_1-1][i_0][i_3+1][0] - E[i_1+1][i_2][i_0+1][i_3+1] - E[i_1][i_3+i_0][i_2+1][i_0] - 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < PB_L; i_5++) {
            B[i_5-1] = F[i_5][i_4-1] * G[1] - B[i_5] + B[i_5-2] * 6;
        }
    }
    for (int i_6 = 2; i_6 < PB_L; i_6++) {
        B[i_6-2] = G[i_6] - G[i_6] - G[i_6-1] - G[0] * G[i_6] - G[i_6] + 1;
    }
    for (int i_7 = 1; i_7 < PB_M; i_7++) {
        for (int i_8 = 1; i_8 < PB_L-1; i_8++) {
            for (int i_9 = 1; i_9 < PB_L; i_9++) {
                for (int j_0 = i_9; j_0 < min(-i_8+PB_P+1, PB_L-1); j_0++) {
                    D[i_8][j_0][i_7][i_9+1][1] = E[j_0][i_7][i_8][1] - H[0][i_8][j_0+i_8-1] * E[i_8+1][j_0-1][i_9-1][i_7+1] + E[i_8][i_7][j_0][i_7] + E[i_9][j_0][0][j_0+1] - D[i_7-1][j_0+1][i_8+1][i_9+1][1] - D[i_8-1][j_0-1][i_7][i_9+1][1] * 2;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,wd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4434151215_03(xd,yd,zd,vd,wd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xd,yd,zd,vd,wd,POLYBENCH_ARRAY(D), xb,POLYBENCH_ARRAY(B), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
