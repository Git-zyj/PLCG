...
void kernel_3224131223_02(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_N-1; i_1++) {
            B[i_0-1][i_1] = C[i_1][i_0] * C[i_1-1][0] + D[i_1] * C[i_1+1][i_0] + 5;
        }
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            B[i_0-1][i_2] = C[i_2][i_0] + B[i_0-1][i_2-2] + B[i_0-3][i_2] - 3;
        }
    }
#pragma endscop
...
kernel_3224131223_02(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
