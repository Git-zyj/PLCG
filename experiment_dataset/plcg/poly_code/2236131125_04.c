...
void kernel_2236131125_04(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            B[i_0-1][i_0-1] = C[0] + D[i_0][0] + D[i_1][i_0-1] + C[i_1] + 4;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 3; i_3 < min(i_2, PB_N+1); i_3++) {
            B[i_3-1][i_3-1] = C[0] + C[i_3+1] * B[i_3-3][i_3-3] - 1;
        }
    }
#pragma endscop
...
kernel_2236131125_04(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
