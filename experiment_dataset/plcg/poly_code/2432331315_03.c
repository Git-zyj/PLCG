...
void kernel_2432331315_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        B[i_0] = E[i_0-1] + E[i_0] + F[i_0] - B[i_0] - 4;
        B[i_0] = F[i_0-1] + B[i_0+1] + B[i_0-1] + 2;
    }
    for (int i_1 = 2; i_1 < PB_M; i_1++) {
        for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
            C[i_1-1][i_2] = G[i_2+1][i_2] - F[i_2-1] + G[i_2-1][1] * H[i_2][i_2+1] * C[i_1-2][i_2-1] + 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_M-1, i_3); i_4++) {
            D[i_3][i_4] = G[i_4+i_3][i_3] + I[i_3][i_4][i_3-1] + G[i_4+i_3+1][i_4+1] - G[i_3-1][i_4] * B[i_4] - D[i_3-1][i_4] * D[i_3][i_4+1] + D[i_3+1][i_4-1] * 1;
        }
    }
#pragma endscop
...
kernel_2432331315_03(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
