...
void kernel_3214312121_02(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_Q+2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_L+3; i_2++) {
                B[i_0-2][i_1][i_2+i_0-3] = 3;
            }
        }
        for (int i_3 = 1; i_3 < min(-i_0+PB_Q+2, i_0); i_3++) {
            for (int i_4 = 1; i_4 < -i_0+PB_L-1; i_4++) {
                B[i_0][i_3][i_4+i_0-1] = C[i_3][i_4][i_3+i_0-2] - C[i_3][i_4-1][i_0-2] - B[i_0+2][i_3-1][i_4+i_0+1] + 2;
            }
        }
    }
#pragma endscop
...
kernel_3214312121_02(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
