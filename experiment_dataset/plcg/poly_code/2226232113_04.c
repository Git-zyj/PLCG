...
void kernel_2226232113_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-2 - 1; i_0 >= 0; i_0--) {
        for (int i_1 = 2; i_1 < -i_0+PB_M-2; i_1++) {
            A[i_0][i_1] = C[i_1-2][i_0+2] * C[i_0+2][i_1+2] * D[i_1+i_0+2] * A[i_0+1][i_1] + B[i_1] + 3;
        }
        B[i_0] = E[i_0] * D[i_0+2] * D[i_0] - 2;
    }
#pragma endscop
...
kernel_2226232113_04(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
