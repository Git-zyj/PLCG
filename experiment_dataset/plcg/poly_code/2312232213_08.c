...
void kernel_2312232213_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_M; i_2++) {
            B[i_2+2] = F[i_2][i_1] * E[i_2+i_1] * E[i_1] - B[i_2+3] * B[i_2+1] * 1;
        }
        for (int i_3 = 1; i_3 < min(PB_P-2, -i_1+PB_N+1); i_3++) {
            C[0][i_1][i_3-1] = D[i_3] * G[i_3+i_1-1][i_1][i_3+2] * A[i_3-1] + B[i_1+2] * C[0][i_1+1][i_3-1] - 1;
        }
    }
#pragma endscop
...
kernel_2312232213_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
