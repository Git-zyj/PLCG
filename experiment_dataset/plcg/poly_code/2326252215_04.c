...
void kernel_2326252215_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0] = D[i_0-1] + D[i_0+1] + E[i_0+1] + D[i_0] - E[i_0] + D[i_0] * A[i_0+1] * 2;
        A[i_0-1] = D[i_0-2] + E[i_0-1] - D[i_0+2] + D[i_0] * 1;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = max(1, i_1); i_2 < -i_1+PB_L-2; i_2++) {
            C[i_1][i_2] = F[i_2][i_1] - G[i_2][i_2][i_1] * F[i_2][i_1] - H[i_1+2][i_2+i_1+2] - I[i_1][i_2-1][i_1] - A[i_1] * C[i_1+1][i_2] * 1;
        }
    }
#pragma endscop
...
kernel_2326252215_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
