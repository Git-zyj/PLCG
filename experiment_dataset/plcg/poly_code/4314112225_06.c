...
void kernel_4314112225_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        A[i_0+1] = D[i_0+1] - A[i_0] * A[i_0+3] + 3;
    }
    for (int i_1 = 2; i_1 < PB_N+2; i_1++) {
        B[i_1-2] = 2;
        for (int i_2 = 0; i_2 < PB_P-2; i_2++) {
            for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
                C[i_2][i_3] = C[i_3+1][i_2] - C[i_2][i_3-2] - C[i_3-2][i_2+2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_4314112225_06(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
