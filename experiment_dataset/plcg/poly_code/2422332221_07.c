...
void kernel_2422332221_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_P-1); i_1++) {
            A[i_0-2][i_1] = E[i_1] - F[i_1] * G[i_1][i_0] - G[i_0][i_1] + H[i_0][i_1+1] * A[i_0-2][i_1+2] * A[i_0-3][i_1] - A[i_0-2][i_1+1] + A[i_0-1][i_1] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
        B[i_2][i_2] = E[i_2] - I[i_2] * B[i_2+1][i_2+1] - 2;
    }
    for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 2; i_4 < -i_3+PB_M-1; i_4++) {
            C[i_3][i_4] = I[i_4+i_3+1] * C[i_3][i_4-2] * 4;
        }
        for (int i_5 = 0; i_5 < -i_3+PB_M; i_5++) {
            D[i_3+1][i_3] = H[i_3][i_5] - H[i_5][i_3] + F[i_3] - H[i_5+i_3][i_5] * D[i_3][i_3-1] + 5;
        }
    }
#pragma endscop
...
kernel_2422332221_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
