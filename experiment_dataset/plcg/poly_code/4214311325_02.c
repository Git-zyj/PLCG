...
void kernel_4214311325_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_P-2, i_0); i_1++) {
            for (int i_2 = 2; i_2 < PB_Q; i_2++) {
                A[i_0][i_1+1][i_2-1] = A[i_0][i_1+1][i_2-2] + A[i_0][i_1+2][i_2-2] - A[i_0][i_1+2][i_2] + 3;
            }
            for (int i_3 = 1; i_3 < PB_M; i_3++) {
                for (int i_4 = 0; i_4 < min(-i_3+PB_M, i_3); i_4++) {
                    A[i_0][i_4+1][i_3-1] = C[i_4+i_3][i_1][i_0] * 3;
                }
            }
        }
    }
...
kernel_4214311325_02(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
