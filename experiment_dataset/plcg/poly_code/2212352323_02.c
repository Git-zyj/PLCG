...
void kernel_2212352323_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_1][i_0] = C[i_1][i_0] - C[i_1][i_0] + C[i_1][i_0] + A[i_1-1][i_0] - A[i_0-1][i_1] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
            B[i_2][i_3+1] = C[i_2][i_3] * D[i_3][i_2] * C[i_3][i_2] + C[i_3-2][i_2-2] + D[i_3+1][0] - E[i_3] - B[i_2][i_3-1] + B[i_2][i_3+3] + B[i_2+1][i_3+1] - 2;
        }
    }
#pragma endscop
...
kernel_2212352323_02(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
