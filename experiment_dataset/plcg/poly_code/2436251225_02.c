...
void kernel_2436251225_02(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        C[i_0+1] = E[i_0-1] * F[i_0] + G[i_0][i_0] * 6;
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            C[i_1+1] = G[i_1+1][i_0-1] + H[i_0][i_0+1] - H[i_0][i_1] * G[i_1+i_0][i_0+1] + E[i_1] + G[i_1][i_0+1] * C[i_1+3] - 5;
            C[i_1+1] = H[i_0+1][0] + G[i_1][i_0] + G[i_1][i_0+1] - F[0] * 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_M-3, PB_L); i_3++) {
            D[i_3+i_2][0] = I[i_3][i_2][0] - H[i_3][i_2+1] + H[i_3][i_2] + F[i_3+1] - H[i_2][i_2] + D[i_3+i_2+3][0] + D[i_3+i_2][0] - 6;
        }
    }
#pragma endscop
...
init_array(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2436251225_02(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I)));
