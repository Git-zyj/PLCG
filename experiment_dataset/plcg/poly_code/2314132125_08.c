...
void kernel_2314132125_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N; i_1++) {
            A[i_1][i_0-2] = D[i_0-1] - E[i_1][i_0] + E[i_0][i_1] * A[i_1-1][i_0-1] + 4;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_P; i_2++) {
            C[i_0][i_2] = E[i_0][i_2] + E[0][i_2+i_0] * D[i_0-2] - E[i_2][1] + 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            C[i_3][i_4] = E[i_3][0] * E[i_3][i_4] - C[i_3-1][i_4] + 5;
        }
    }
#pragma endscop
...
kernel_2314132125_08(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
