...
void kernel_4334331325_00(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            C[i_0][i_1] = D[i_1][i_0+1] + E[i_1] + F[i_1] * E[i_1] - 3;
        }
        for (int i_2 = 0; i_2 < i_0; i_2++) {
            for (int i_3 = 1; i_3 < PB_M; i_3++) {
                for (int i_4 = 1; i_4 < min(min(-i_3+PB_L, i_2), PB_M-1); i_4++) {
                    C[i_4][i_3-1] = G[i_4+1][i_3][i_2][i_0] * H[i_3][i_0][i_3] + I[i_3][i_2][i_0-1][i_4] - G[i_2][i_4+i_3][i_4-1][i_0] - 6;
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_L+1; i_5++) {
        for (int i_6 = 2; i_6 < PB_M; i_6++) {
            for (int i_7 = 0; i_7 < PB_M; i_7++) {
                C[i_6][i_7] = C[i_6-2][i_7] + C[i_7][i_5-1] * C[i_6-1][i_7] + 2;
            }
        }
    }
#pragma endscop
...
init_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4334331325_00(xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
