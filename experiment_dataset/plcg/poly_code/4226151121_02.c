...
void kernel_4226151121_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        ...
        for (int i_1 = max(2, i_0); i_1 < PB_L; i_1++) {
            A[i_1-2] = D[i_0+1][i_1-1] + D[i_0-1][i_1] + D[i_1][i_0] * 6;
        }
    }
#pragma endscop
...
kernel_4226151121_02(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
