...
void kernel_3326251113_06(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0-1] = D[i_0] * D[i_0] * D[i_0] * D[i_0-1] * D[i_0] * E[i_0] * E[i_0+1] * A[i_0-2] + A[i_0] - 5;
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M, i_0); i_2++) {
                B[i_1-1][i_2-1][i_0] = F[i_1][i_2+1] - G[i_2-1][i_1][i_2] + H[i_1+1][i_2+i_0][i_0+1] * H[i_0][i_1+1][0] * 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M; i_3++) {
        A[i_3-2] = D[i_3] + E[i_3] + D[i_3] + D[i_3] * 1;
    }
#pragma endscop
...
kernel_3326251113_06(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
