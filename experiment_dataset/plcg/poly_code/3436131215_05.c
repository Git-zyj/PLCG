...
void kernel_3436131215_05(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        B[i_0-1][i_0-1] = E[i_0] * E[i_0+1] + E[i_0] - B[i_0][i_0] - 3;
        for (int i_1 = i_0; i_1 < -i_0+PB_M; i_1++) {
            B[i_0][i_0] = F[i_1-1][i_0] + E[i_1+i_0] - B[i_0-1][i_0-1] - 1;
        }
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            B[i_0][i_0] = F[i_2-1][i_0] - F[i_2-1][i_2] + F[i_2][i_0] + 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_N, -i_3+PB_Q-1); i_4++) {
            for (int i_5 = 0; i_5 < i_4; i_5++) {
                B[i_4][i_4] = G[0][i_5+1][i_3+1][i_4+i_3+1] + F[i_5][i_4+1] * H[i_5+i_4][i_3][i_5+1] + 6;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_3436131215_05(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
