...
void kernel_2422251315_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        B[i_1] = F[i_1-1] + G[i_1+1][1] * E[i_1] - B[i_1-1] + 2;
        for (int i_2 = 1; i_2 < -i_1+PB_M; i_2++) {
            B[i_2] = G[i_2-1][i_1-1] - G[i_2+i_1][0] + H[i_2][i_1-1] * H[i_2-1][i_1] + A[i_2] * B[i_1] + B[i_1+1] + D[i_1][i_2-1] * 4;
        }
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            D[i_1-1][i_3-1] = E[i_3+1] * F[i_3-1] - E[i_3-1] - E[i_3] * G[i_1][i_3] + G[i_3+1][i_3] * I[i_3-1][i_1][i_3] * G[i_3][i_1] + B[i_3+1] + B[i_1] - D[i_1][i_3-1] + 5;
        }
    }
#pragma endscop
...
kernel_2422251315_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
