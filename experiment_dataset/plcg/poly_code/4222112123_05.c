...
void kernel_4222112123_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < PB_N; i_4++) {
            B[i_4] = C[i_4-1][i_3] * C[i_4][i_3] * B[i_4-1] * 1;
        }
    }
#pragma endscop
...
kernel_4222112123_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
