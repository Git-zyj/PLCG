...
void kernel_4422251111_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            B[i_0-1] = E[i_1][1] - F[i_0-1][i_1-1] * G[i_1+1] + H[i_0-1] - 1;
        }
        B[i_0-1] = G[0] + H[i_0-1] * G[i_0+1] - G[i_0] * G[i_0] * B[i_0] - B[i_0-2] + 2;
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_N; i_3++) {
            for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
                C[i_3+i_2][i_4+1][i_4] = F[i_4][i_3+1] - E[i_4][i_3+1] * F[i_4-1][i_3] * C[i_3+i_2][i_4+1][i_4] * 3;
            }
        }
        for (int i_5 = 1; i_5 < -i_2+PB_M; i_5++) {
            D[i_5] = F[i_5][i_2] - E[i_5][i_2] * H[i_5] - G[i_2+1] * E[i_5][i_2] + H[i_5] - E[i_5+i_2][0] - F[i_5-1][i_2] + 5;
        }
    }
#pragma endscop
...
init_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4422251111_00(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
