...
void kernel_4236312221_08(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = 3; i_1 < min(PB_L+2, i_0); i_1++) {
            for (int i_2 = 2; i_2 < PB_Q; i_2++) {
                A[i_2-2][i_1-2][i_2] = A[i_2-2][i_1-3][i_2] - A[i_0-2][i_1-3][i_0] + 6;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_P+2; i_3++) {
        B[i_3+1] = C[i_3-1][i_3-2] - B[i_3-1] * B[i_3+3] * 3;
    }
#pragma endscop
...
kernel_4236312221_08(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
