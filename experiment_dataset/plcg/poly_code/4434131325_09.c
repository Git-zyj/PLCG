...
void kernel_4434131325_09(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < PB_M; i_2++) {
                A[i_2][i_0][i_1][0] = E[i_0][i_1-1][i_2][i_0+1] * F[i_2-1][i_1-1] * A[i_2][i_0-2][i_1-1][0] + A[i_2-2][i_1][i_0-2][0] * 4;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
            for (int i_5 = 3; i_5 < -i_4+PB_L-1; i_5++) {
                for (int i_6 = 1; i_6 < PB_Q-1; i_6++) {
                    B[i_6][i_4+1][i_6][i_5-1] = E[i_5+i_4+1][i_6+1][i_3][i_3] * A[i_4][i_5][i_6][0] * B[i_6][i_4-1][i_6][i_5-1] - B[i_6-1][i_4+1][i_6-1][i_5-1] - B[i_6][i_4+1][i_6][i_5-3] - 6;
                }
            }
        }
        D[i_3-1] = G[i_3] * G[i_3+1] * 3;
    }
    for (int i_7 = 1; i_7 < PB_L; i_7++) {
        for (int i_8 = 3; i_8 < min(-i_7+PB_M, i_7); i_8++) {
            D[i_7-1] = F[i_8][i_7] + F[i_8][i_7] + F[i_8+i_7][i_8] * F[i_8][i_7-1] * D[i_8-3] + D[i_8] + 3;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4434131325_09(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G)));
