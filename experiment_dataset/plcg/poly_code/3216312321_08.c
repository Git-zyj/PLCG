...
void kernel_3216312321_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = i_0; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_M; i_2++) {
                A[i_1+2][i_2] = C[i_2+i_1][1] - A[i_1+1][i_2] - 1;
            }
        }
    }
    for (int i_3 = 4; i_3 < PB_M+2; i_3++) {
        for (int i_4 = 4; i_4 < min(PB_M-2, i_3); i_4++) {
            B[i_4-2][i_4] = D[i_3-2][i_4+2] * B[i_4-4][i_4-2] * B[i_3-4][i_3-2] + B[i_4][i_4+2] * B[i_4-3][i_4-1] + 1;
        }
    }
#pragma endscop
...
kernel_3216312321_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
