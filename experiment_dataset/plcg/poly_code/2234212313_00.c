...
void kernel_2234212313_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        for (int i_1 = PB_P+2 - 1; i_1 >= 3; i_1--) {
            A[i_1+1] = A[i_1] * A[i_1+2] + 4;
            B[i_0+2][i_1+1][i_1-2] = C[i_1][i_0] + B[i_0+3][i_1+1][i_1-2] - B[i_0+2][i_1][i_1-3] + B[i_0+1][i_1+1][i_1-2] - 5;
        }
    }
#pragma endscop
...
kernel_2234212313_00(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
