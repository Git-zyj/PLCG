...
void kernel_3324112223_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 0; i_1 < min(i_0, PB_M-2); i_1++) {
            A[i_0-1][i_1+1] = B[i_1+2][i_0-1] + 5;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M+2; i_2++) {
            B[i_2][i_0] = D[i_2+i_0-2][i_0-2] + D[i_0-1][i_2] * D[i_2][i_0] + B[i_0-1][i_2-1] * B[i_2][i_0+1] * 5;
        }
        C[0] = C[0] + C[0] - 1;
    }
#pragma endscop
...
kernel_3324112223_06(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
