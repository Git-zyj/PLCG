...
void kernel_2436252321_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M-2; i_1++) {
            A[i_0][i_1] = E[i_1] + F[i_1][i_0-2] - G[i_0+2][0] * H[i_0][i_1][0] - F[i_1][i_0] * A[i_0][i_1-1] + A[i_0-1][i_1-2] + A[i_0+1][i_1] * A[i_0+2][i_1+2] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-1; i_2++) {
        B[i_2-2] = F[i_2+1][i_2-1] * I[i_2+1] + E[i_2] * I[i_2-2] + F[i_2][i_2] * B[i_2] - 2;
    }
    for (int i_3 = 3; i_3 < PB_M-2; i_3++) {
        C[i_3-2] = I[i_3] - E[2] + I[i_3] + I[i_3] * I[i_3] * I[i_3] + C[i_3-3] + D[i_3+1] * D[i_3] - D[i_3+2] * 4;
        D[i_3] = E[i_3] * E[i_3+1] * I[i_3+1] - B[i_3-2] * C[i_3-2] - D[i_3-1] * 2;
    }
#pragma endscop
...
kernel_2436252321_01(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
