...
void kernel_3314212325_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        ...
        B[i_0+2] = D[i_0] * B[i_0+1] + B[i_0] - B[i_0+3] * 6;
    }
    for (int i_3 = 0; i_3 < PB_M; i_3++) {
        for (int i_4 = min(PB_M-2, i_3) - 1; i_4 >= 0; i_4--) {
            B[i_4+2] = 2;
        }
    }
#pragma endscop
...
kernel_3314212325_08(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D));
