...
void kernel_3224111115_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_N-1; i_1++) {
            A[i_1][i_0][i_0] = A[i_1+1][i_0][i_0] + 5;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_N, i_2); i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                A[i_3][i_2][i_2] = C[i_4][i_3][i_3] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3224111115_04(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
