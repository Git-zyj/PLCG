...
void kernel_3212311123_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    for (int i_0 = 3; i_0 < PB_P-1; i_0++) {
        ...
        for (int i_3 = 0; i_3 < -i_0+PB_M; i_3++) {
            B[i_3+i_0] = C[i_3][i_0+1] - B[i_3+i_0-1] * B[i_3+i_0-3] * 1;
        }
    }
#pragma endscop
...
kernel_3212311123_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
