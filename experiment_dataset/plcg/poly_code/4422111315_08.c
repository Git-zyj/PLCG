...
void kernel_4422111315_08(int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
            C[i_1][i_0-1][i_1+1] = C[i_0-1][i_1-1][i_0] * C[i_1-2][i_0][i_1-1] - 6;
        }
    }
        ...
        for (int i_3 = 2; i_3 < PB_L; i_3++) {
            for (int i_4 = 0; i_4 < PB_L; i_4++) {
                for (int i_5 = 0; i_5 < PB_M; i_5++) {
                    C[i_3-2][i_2][i_3-1] = E[i_5][i_4][i_3] + 5;
                }
            }
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                C[i_2-1][i_6][i_2] = E[i_3][i_2-1][i_6] * C[i_2-2][i_6+1][i_2-1] - 5;
            }
        }
        D[i_2+1] = F[i_2-1] - D[i_2+2] * D[i_2] - 2;
    }
#pragma endscop
...
kernel_4422111315_08(xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
