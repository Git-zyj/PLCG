...
void kernel_3416351323_08(int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk),int xl,int yl,int zl,DATA_TYPE POLYBENCH_3D(L,xL,yL,zL,xl,yl,zl),int xm,int ym,int zm,int vm,DATA_TYPE POLYBENCH_4D(M,xM,yM,zM,vM,xm,ym,zm,vm)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_N-1; i_1++) {
            D[i_0-2][i_1+2] = E[i_1+i_0] * F[i_0][i_1] + F[i_1+i_0+1][0] * G[i_1][i_0+1] + D[i_0][i_1+1] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = i_2; i_3 < PB_M-1; i_3++) {
            D[i_2][i_3] = G[i_3][i_2] + H[i_3+1] - E[i_3] * G[i_2][i_3] * I[i_3][i_2+1] * 5;
        }
        for (int i_4 = 0; i_4 < min(PB_P, -i_2+PB_M-1); i_4++) {
            for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
                C[i_5+1][i_2] = J[i_5+i_4][i_2+1][i_5-1] + K[i_2][i_4+1][i_5] - K[i_5][i_4][i_2] - L[i_4][i_2][i_5+1] + M[i_2][i_5][i_4][0] * C[i_5+1][i_2+1] + 6;
            }
            D[i_2][i_4] = I[i_4][i_2] * H[i_2+1] * F[i_2-1][i_4+i_2+1] + I[i_4+i_2-1][0] * D[i_2-1][i_4] * D[i_2][i_4+1] - D[i_2+2][i_4] * 5;
        }
    }
#pragma endscop
...
init_array(xd,yd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M), INIT_SEED);
kernel_3416351323_08(xd,yd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M));
polybench_prevent_dce(print_array(xd,yd,POLYBENCH_ARRAY(D), xc,yc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K), xl,yl,zl,POLYBENCH_ARRAY(L), xm,ym,zm,vm,POLYBENCH_ARRAY(M)));
