...
void kernel_4324352221_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,DATA_TYPE POLYBENCH_1D(K,xK,xk),int xl,DATA_TYPE POLYBENCH_1D(L,xL,xl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = max(2, i_0); i_2 < -i_0+PB_L; i_2++) {
                for (int i_3 = i_0; i_3 < -i_0+PB_M; i_3++) {
                    A[i_2-2][i_3][i_1][i_0-1] = D[i_1][i_0][i_3][i_2+1] * E[i_3-1][i_1][i_2+i_0][i_2] - E[i_1-1][i_0-1][i_2][i_0] * E[i_2][i_3+i_0][i_1][i_0] * D[i_2-2][i_3][i_1][i_0+2] + A[i_2-2][i_0-1][i_3][i_1-1] - 6;
                }
            }
        }
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 0; i_5 < min(i_4, -i_4+PB_P+1); i_5++) {
                B[i_5][i_4] = F[i_4-1][i_0] + G[i_5][i_0][i_5+i_4-1] * H[i_4][i_5][i_0-1] * B[i_5+2][i_4] * B[i_5+1][i_4] + B[i_5+2][i_4+1] + B[i_4+2][i_5] - 3;
            }
        }
    }
    for (int i_6 = 0; i_6 < PB_M-1; i_6++) {
        C[i_6] = I[i_6] + J[i_6][i_6] + K[i_6] + L[i_6] * K[i_6+1] * I[i_6] - 1;
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L), INIT_SEED);
kernel_4324352221_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,POLYBENCH_ARRAY(K), xl,POLYBENCH_ARRAY(L));
