...
void kernel_3436351315_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_P); i_1++) {
            A[i_1] = E[i_1-1] * F[i_0+1][i_0+1] * G[i_0][i_0+1] + H[i_0][i_1+i_0] * G[i_0][i_1] - I[i_1] - A[i_1+1] * A[i_1-1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_N-2; i_2++) {
        B[i_2+1] = I[i_2] + E[1] * I[i_2-1] + B[i_2+2] * B[i_2] + D[i_2+1] - 3;
        B[i_2] = I[i_2+1] + J[i_2] * H[i_2][i_2+1] - J[i_2+1] + I[i_2] - B[i_2+1] - D[i_2+1] + 6;
        D[i_2] = J[i_2+1] * E[i_2-1] * E[i_2] + E[i_2] - A[i_2] - B[i_2] + 5;
    }
#pragma endscop
...
kernel_3436351315_01(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
