...
void kernel_4234152313_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            B[i_1-1] = C[i_0+1][i_1] * C[i_1+1][i_1] + C[i_1-1][i_0-2] * C[i_0][i_1] - C[i_1][i_0+2] - 3;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 2; i_3 < min(PB_L, -i_2+PB_M); i_3++) {
            B[i_3] = C[i_3][i_2] - C[i_3+i_2][i_3] - C[i_2][0] + C[i_3][i_2] + C[i_3-2][i_2] - B[i_3-1] - B[i_3+1] - B[i_3+1] - 4;
        }
    }
#pragma endscop
...
kernel_4234152313_00(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
