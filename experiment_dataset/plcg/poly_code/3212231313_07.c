...
void kernel_3212231313_07(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_Q, PB_M-1); i_2++) {
                B[i_1][i_2+i_0+2][i_1-1][i_0] = C[i_1][i_2+i_0][1] * D[i_2+1][i_1][i_0] - D[i_1][i_2+1][i_0] * D[i_2+1][i_0][i_1] - 1;
            }
        }
    }
    for (int i_3 = 1; i_3 < PB_P-1; i_3++) {
        for (int i_4 = 1; i_4 < min(-i_3+PB_L-1, PB_M-1); i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_L-2, PB_M-2); i_5++) {
                B[i_4+1][i_5+i_3+1][i_4][i_3] = D[i_5][i_4][i_3-1] - B[i_4+1][i_5+i_3+2][i_4][i_3+1] - B[i_5+2][i_4+i_3+1][i_5+1][i_3] - B[i_5+2][i_4+i_3][i_5+1][i_4-1] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3212231313_07(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
