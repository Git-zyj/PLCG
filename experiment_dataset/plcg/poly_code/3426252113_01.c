...
void kernel_3426252113_01(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-3; i_0++) {
        B[i_0+3][0] = E[0] + F[i_0] - F[i_0] + 6;
    }
    for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_P+2; i_2++) {
            B[i_1+2][0] = G[i_2][i_1+1] - H[i_2][i_2-1] + G[i_2][i_1] + G[i_2+2][i_1] * H[i_2-2][i_1] + 3;
            for (int i_3 = 1; i_3 < min(PB_P, -i_1+PB_L+2); i_3++) {
                B[i_1+1][0] = I[i_1][i_2+2][i_2-2] + I[i_3][i_2][i_1-1] - I[i_3+1][i_1][i_2] * J[i_3][i_1][i_2-1] + I[i_2][i_3][0] * J[i_3-1][i_3+i_1-2][i_2] + J[i_3][i_3][i_2+i_1-2] - B[i_3+3][0] + 6;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_P; i_4++) {
        for (int i_5 = 1; i_5 < PB_P-2; i_5++) {
            for (int i_6 = 2; i_6 < min(min(i_4, PB_P-2), -i_5+PB_M+1); i_6++) {
                B[i_6+2][0] = G[i_6-2][i_4] - G[i_6][i_5+2] * J[i_6][i_5][i_6+2] + J[i_6+i_5-1][i_4][i_4] - K[i_5][i_6][i_5][i_4] + 3;
            }
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_3426252113_01(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K)));
