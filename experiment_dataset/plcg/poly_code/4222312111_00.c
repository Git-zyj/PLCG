...
void kernel_4222312111_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = i_1; i_2 < -i_1+PB_P; i_2++) {
                A[i_0][i_2+i_1][i_2][i_1-2] = C[i_1][i_2] - A[i_0+1][i_2+i_1][i_2][i_1-2] + 3;
            }
        }
        B[i_0] = B[i_0-1] * 1;
    }
...
kernel_4222312111_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
