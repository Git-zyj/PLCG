...
void kernel_4424251223_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        A[i_0] = E[i_0] - F[i_0] + F[i_0+1] * F[i_0] - C[i_0] - 1;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
            for (int i_3 = max(2, i_1); i_3 < PB_M; i_3++) {
                for (int i_4 = 1; i_4 < PB_Q-2; i_4++) {
                    B[i_3-1][i_1][i_4][i_2] = G[i_2-1][i_4-1][i_3][i_1] + G[i_3][i_1+1][i_4][i_2] + B[i_3-2][i_1][i_4][i_2+1] - B[i_3-1][i_1][i_4+2][i_2+2] + B[i_3-1][i_1][i_4+2][i_2+1] * 6;
                }
            }
            C[i_2] = H[i_2-1][i_1] * I[i_1][i_2+1] - H[i_2-1][i_1] - H[i_2+1][i_1] * C[i_1+2] * C[i_2-1] - 1;
        }
    }
    for (int i_5 = 1; i_5 < PB_L-1; i_5++) {
        A[i_5] = F[0] + E[i_5-1] - F[i_5] * F[i_5] * E[i_5+1] * F[i_5] * F[i_5] * 6;
    }
#pragma endscop
...
init_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4424251223_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
