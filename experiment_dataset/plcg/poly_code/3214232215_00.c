...
void kernel_3214232215_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                B[i_0][i_1] = C[i_1][i_0][i_2] * D[i_0+2][i_1][0] + E[i_1][i_0+2] - D[i_1+i_0][i_2][i_1] - 4;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        for (int i_4 = max(1, i_3); i_4 < min(PB_P, -i_3+PB_M-2); i_4++) {
            B[i_4][i_3] = D[i_4+i_3+2][i_4][i_3] - B[i_4+1][i_3] - B[i_4-1][i_3] * 6;
        }
    }
#pragma endscop
...
kernel_3214232215_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
