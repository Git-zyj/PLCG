...
void kernel_2326351121_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-1; i_0++) {
        B[i_0+1] = D[i_0] - E[i_0+1] * D[i_0] - E[i_0+1] + D[i_0] - E[i_0] * 2;
    }
    for (int i_1 = 1; i_1 < PB_M; i_1++) {
        B[i_1] = E[i_1] - E[i_1] + E[i_1] * D[i_1-1] * B[i_1+2] + 5;
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < PB_M-1; i_3++) {
            C[i_3+1][i_2] = D[i_3] + F[i_2][i_3] - G[i_3-1] - H[i_2][i_3-1] + 6;
        }
    }
#pragma endscop
...
kernel_2326351121_00(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
