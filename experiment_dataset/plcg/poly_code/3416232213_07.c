...
void kernel_3416232213_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_L-1); i_1++) {
            A[i_0][i_1] = E[i_1] - F[i_1+i_0][1] - E[i_1+1] * A[i_0-1][i_1] + A[i_0+1][i_1] * 4;
            B[0][i_1] = G[i_1+i_0][i_1] * F[i_1][i_0] * A[i_0+1][i_1+1] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_L; i_2++) {
        B[0][i_2-1] = E[i_2] * H[0] * G[i_2][i_2] * H[i_2] * B[0][i_2+1] - 4;
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            for (int i_4 = i_3; i_4 < -i_3+PB_L; i_4++) {
                A[i_3][i_4] = F[i_2+1][i_4+i_3] * I[i_3][i_4-1][i_2-1] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3416232213_07(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
