...
void kernel_2434351225_07(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(i_0, -i_0+PB_M+1); i_1++) {
            C[i_1] = E[i_0+1] * F[i_1-1][i_0] + G[i_1][i_0+1] * H[i_1][i_0] + G[i_1][i_0] - H[i_1+i_0-1][i_0+1] * B[i_0][i_1-1] + 2;
        }
        for (int i_2 = 3; i_2 < -i_0+PB_M-1; i_2++) {
            B[i_0][i_2-1] = F[i_2+1][i_0] * F[i_2+i_0+1][i_2] + B[i_0][i_2-2] * B[i_0][i_2-3] + 6;
        }
        C[i_0] = I[i_0] - I[i_0] + E[i_0+1] + I[i_0] + I[i_0-1] + E[i_0] + 1;
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        C[i_3-2] = J[i_3] - G[i_3-1][i_3] * E[i_3+1] + J[i_3] + E[i_3-1] + 2;
    }
#pragma endscop
...
kernel_2434351225_07(xc,POLYBENCH_ARRAY(C), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
