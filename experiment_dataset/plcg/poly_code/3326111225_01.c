...
void kernel_3326111225_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_P+1); i_2++) {
                A[i_1][i_0-1][i_2] = D[i_0+1][i_2-1][i_1-1] + D[i_2+i_1][i_0][i_0] * A[i_1+1][i_0-1][i_2+2] - A[i_1][i_0-2][i_2+2] - 6;
            }
        }
    ...
    for (int i_4 = 3; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < i_4; i_5++) {
            for (int i_6 = 0; i_6 < PB_Q-2; i_6++) {
                C[i_4-1][i_4][i_6] = C[i_4-3][i_4-2][i_6+2] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3326111225_01(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
