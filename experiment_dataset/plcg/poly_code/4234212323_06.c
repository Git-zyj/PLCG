...
void kernel_4234212323_06(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_M, i_1); i_2++) {
                B[i_2] = C[i_0+2][i_1+1][i_2] + C[i_2][i_0][i_1] - 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = i_3; i_4 < PB_L-2; i_4++) {
            B[i_4+1] = B[i_4+2] * B[i_4] - B[i_3] * 6;
        }
    }
#pragma endscop
...
kernel_4234212323_06(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
