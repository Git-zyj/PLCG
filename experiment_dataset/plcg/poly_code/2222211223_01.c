...
void kernel_2222211223_01(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N-3; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_N-3; i_1++) {
            A[i_1+i_0][i_1-1] = A[i_1+i_0+3][i_1+1] + A[i_1+i_0+1][i_1+1] - 6;
        }
    }
    for (int i_2 = 3; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_N+1, -i_2+PB_M); i_3++) {
            B[i_3][i_2-1] = C[i_3-1][i_3+i_2] + B[i_3][i_2-3] - B[i_3+1][i_2] - 3;
        }
    }
#pragma endscop
...
kernel_2222211223_01(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
