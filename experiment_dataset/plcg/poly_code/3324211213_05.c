...
void kernel_3324211213_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = 1; i_2 < -i_1+PB_P-1; i_2++) {
            C[i_1][i_2+i_1] = D[i_1-1][i_2-1] - D[i_1][0] * C[i_2+1][i_2+i_1+1] - C[i_2-1][i_2+i_1] + 6;
        }
    }
#pragma endscop
...
kernel_3324211213_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
