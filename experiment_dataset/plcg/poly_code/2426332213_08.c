...
void kernel_2426332213_08(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, i_0); i_1++) {
            B[i_1] = E[i_1+1][i_0] - F[i_1+i_0] - G[i_0][i_1+1] * B[i_1] - 4;
        }
        B[i_0] = F[i_0+2] * H[i_0] * I[i_0] * B[i_0-1] - 5;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        D[i_2-2][i_2] = I[i_2] - I[i_2+2] - 5;
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            D[i_2-2][i_2] = G[i_3][i_2-2] * E[i_2][i_3] - B[i_3+1] - 1;
        }
    }
#pragma endscop
...
kernel_2426332213_08(xb,POLYBENCH_ARRAY(B), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
