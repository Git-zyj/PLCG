...
void kernel_2314112325_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_P-2; i_1++) {
            B[i_1+1] = C[i_1+1][i_1+i_0+2][i_1+i_0+2] + C[i_0+2][i_1+i_0+1][i_1+i_0+1] + C[i_1-1][i_1+i_0-2][i_1+i_0-2] - 2;
        }
    }
    for (int i_2 = 0; i_2 < PB_N; i_2++) {
        B[i_2+1] = 2;
        for (int i_3 = 1; i_3 < -i_2+PB_P; i_3++) {
            C[i_3+1][i_3+i_2][i_3+i_2] = D[i_2][0] - B[i_2+1] * B[i_3-1] + B[i_3+3] * 1;
        }
    }
#pragma endscop
...
kernel_2314112325_09(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
