...
void kernel_3334232223_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M+1, PB_L-2); i_1++) {
            A[i_0][i_1] = D[i_1+i_0-1][i_1+2] * D[i_1-1][i_0] - D[i_1+1][i_0+1] + A[i_0][i_1-1] * A[i_0][i_1+1] - 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_M-2; i_2++) {
        for (int i_3 = max(4, i_2); i_3 < PB_M; i_3++) {
            for (int i_4 = 1; i_4 < PB_M; i_4++) {
                C[i_4-1][i_3-4][i_2-2] = E[i_3][i_2+2][i_2] + F[i_4][1] * 2;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = 3; i_6 < PB_L-1; i_6++) {
            for (int i_7 = 2; i_7 < -i_5+PB_M; i_7++) {
                C[i_5-1][i_6-2][i_7-2] = E[i_6][i_7+i_5][i_7+1] - E[i_7][i_5][i_6+1] * E[i_6][i_5][i_6] * D[i_7][i_6] * C[i_5-1][i_6-3][i_7-2] - 6;
            }
        }
    }
#pragma endscop
...
kernel_3334232223_00(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
