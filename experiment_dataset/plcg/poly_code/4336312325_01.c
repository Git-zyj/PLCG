...
void kernel_4336312325_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
        ...
        for (int i_1 = 0; i_1 < -i_0+PB_M-1; i_1++) {
            for (int i_2 = 2; i_2 < PB_P-4; i_2++) {
                for (int i_3 = i_2; i_3 < -i_2+PB_L; i_3++) {
                    A[i_1+i_0][i_2][i_3+i_2][i_2+2] = A[i_1+i_0+1][i_2][i_3+i_2][i_2+2] * A[i_1+i_0-1][i_2-2][i_3+i_2-2][i_2] - A[i_1+i_0][i_2+2][i_3+i_2][i_2+4] * 5;
                }
            }
        }
        for (int i_4 = i_0; i_4 < PB_L-1; i_4++) {
            B[i_0][i_4] = D[i_4][i_0] - B[i_0][i_4-1] * B[i_0][i_4+1] + 3;
        }
    }
    for (int i_5 = 2; i_5 < PB_L; i_5++) {
        for (int i_6 = 2; i_6 < i_5; i_6++) {
            for (int i_7 = 2; i_7 < min(-i_5+PB_P, -i_6+PB_P-1); i_7++) {
                C[i_5][i_6-2][i_7+i_5] = B[i_7-2][i_5] * C[i_5-1][i_6][i_7+i_5-2] - C[i_6+1][i_5-2][i_7+i_6+1] * C[i_6][i_5-2][i_7+i_6+1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4336312325_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
