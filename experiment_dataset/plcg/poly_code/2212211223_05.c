...
void kernel_2212211223_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_M; i_1++) {
            A[i_0][i_1+i_0] = C[i_1+i_0][i_0] - A[i_1][i_1+i_0-2] * A[i_0][i_1+i_0+2] + 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < min(-i_2+PB_P+2, PB_M-1); i_3++) {
            A[i_2][i_3+i_2-2] = C[i_3+1][i_2] * 6;
        }
    }
#pragma endscop
...
kernel_2212211223_05(xa,ya,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C));
