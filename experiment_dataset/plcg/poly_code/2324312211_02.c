...
void kernel_2324312211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N; i_0++) {
        A[i_0] = A[i_0-1] - B[i_0] * B[i_0-1] + 6;
        B[i_0] = D[i_0-2] - D[i_0] + D[i_0] + B[i_0+1] - 6;
    }
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        for (int i_2 = 0; i_2 < PB_M; i_2++) {
            C[i_1][i_1-1] = C[i_1-1][i_1-2] - C[i_1+1][i_1] * 3;
        }
    }
#pragma endscop
...
kernel_2324312211_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
