...
void kernel_3422151323_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L-1 - 1; i_0 >= 0; i_0--) {
        B[i_0+2] = E[i_0+1][i_0+1] - F[i_0] - F[i_0] + F[0] + F[i_0] - D[i_0+1] * 1;
        B[i_0] = F[i_0] * E[i_0][i_0] + F[i_0+1] + D[i_0] + 2;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        D[i_1] = F[i_1] - F[i_1+1] - F[i_1] * F[i_1] * F[i_1] - F[i_1-1] * F[i_1] * D[i_1-1] * 1;
        D[i_1] = F[i_1] + F[i_1] * F[i_1-1] * F[i_1+1] * F[i_1-1] - B[i_1] * D[i_1-1] - 1;
    }
#pragma endscop
...
kernel_3422151323_05(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
