...
void kernel_3412352325_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0+1][i_0] = E[i_0] * F[i_1][1] + F[i_0][i_1] + G[i_0-1][i_1] * A[i_0+3][i_0+2] - 1;
        }
    }
    for (int i_2 = 2; i_2 < PB_N; i_2++) {
        D[i_2+2] = H[i_2] + H[i_2] - G[i_2-1][2] - I[i_2-2] * H[i_2-2] * D[i_2+2] * 1;
        for (int i_3 = 1; i_3 < -i_2+PB_N; i_3++) {
            A[i_2+1][i_2] = H[i_3] * G[i_3+i_2][i_3] * J[i_2+2][i_3+2] * I[i_3-1] * G[i_3][0] * J[0][i_2] + H[i_2] + 1;
        }
        D[i_2+2] = I[i_2-1] - A[i_2+1][i_2] - D[i_2] + D[i_2+4] * 5;
    }
#pragma endscop
...
kernel_3412352325_06(xa,ya,POLYBENCH_ARRAY(A), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
