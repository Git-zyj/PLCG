...
void kernel_2326151223_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0+1] + D[i_0] * D[i_0+1] + D[1] * D[0] + A[i_0+1] + B[i_0][i_0] - 4;
    }
    for (int i_1 = 1; i_1 < PB_L; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_M, PB_L); i_2++) {
            B[i_1][i_1] = E[i_2+i_1][0] + E[i_2][i_1] + E[i_2-1][i_1] + A[i_1] - 3;
        }
        B[i_1][i_1] = D[i_1] - D[i_1-1] * D[i_1-1] * D[i_1+1] - D[i_1-1] + D[i_1] + D[i_1+1] - A[i_1] - 1;
    }
#pragma endscop
...
kernel_2326151223_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
