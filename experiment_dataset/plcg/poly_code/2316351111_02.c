...
void kernel_2316351111_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0-1] = D[i_0+1] - D[0] * D[i_0-1] - E[i_0] + F[i_0+1] + F[i_0] * 6;
    }
    for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_M; i_2++) {
            B[i_2] = G[i_1][i_2+1] * H[i_1][i_2-1] + H[i_1][i_2-1] + G[i_1][i_2+1] * G[i_2-1][i_1] * F[i_1] * E[1] * A[i_2-1] * 3;
        }
        for (int i_3 = 1; i_3 < -i_1+PB_M; i_3++) {
            C[i_3-1] = G[i_3+i_1][i_3+i_1] + G[i_1+1][i_3+1] * C[i_3] * 1;
        }
    }
#pragma endscop
...
kernel_2316351111_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
