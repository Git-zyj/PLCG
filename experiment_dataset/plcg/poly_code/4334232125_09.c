...
void kernel_4334232125_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M+2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L, -i_0+PB_M+2); i_1++) {
            A[i_1+i_0-2][i_1] = D[i_1] + 5;
        }
    }
        ...
        for (int i_3 = 2; i_3 < PB_L-2; i_3++) {
            for (int i_4 = 0; i_4 < i_3; i_4++) {
                for (int i_5 = 1; i_5 < PB_M-2; i_5++) {
                    B[i_5][i_2][i_4][i_3] = E[i_4][i_5][i_2][0] - F[i_2][i_5+2][i_3-2][i_4] + G[i_4+1][i_3+2][i_5-1] * B[i_3][i_2+1][i_5][i_4] + B[i_5][i_2][i_4][i_3-2] - 4;
                }
            }
        }
        C[i_2-2] = D[i_2+2] - D[i_2] + D[i_2-1] - C[i_2-4] - 4;
    }
#pragma endscop
...
kernel_4334232125_09(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
