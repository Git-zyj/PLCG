...
void kernel_3324132113_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_1] = D[i_1][i_0+2] * D[i_0][0] + 5;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_M+2; i_2++) {
            B[i_2] = D[i_0][i_2+i_0-2] - D[i_0][i_2] * D[i_2][i_0] * D[i_2][i_0] * B[i_2-1] * 5;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_L; i_4++) {
            C[i_3+1][i_4] = D[i_4][i_3] * 3;
        }
    }
#pragma endscop
...
kernel_3324132113_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
