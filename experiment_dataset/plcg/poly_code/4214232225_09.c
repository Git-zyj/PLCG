...
void kernel_4214232225_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        A[i_0-2] = C[0] + C[i_0] + D[i_0] * A[i_0-3] + A[i_0] + A[i_0-4] * 5;
    }
    for (int i_1 = 4; i_1 < PB_L; i_1++) {
        for (int i_2 = i_1; i_2 < min(PB_L, -i_1+PB_M+2); i_2++) {
            B[i_1-2][i_2] = D[i_2] + E[i_2+i_1-2][0] + D[i_2] * B[i_1-4][i_2] - 2;
        }
    }
#pragma endscop
...
kernel_4214232225_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
