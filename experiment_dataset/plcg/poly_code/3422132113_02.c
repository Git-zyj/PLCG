...
void kernel_3422132113_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_L, PB_M-1); i_1++) {
            for (int i_2 = 2; i_2 < min(min(-i_0+PB_L, -i_1+PB_L), PB_M-2); i_2++) {
                A[i_2+2][i_1+i_0+1][i_2] = E[i_0][i_2+i_1] + F[i_2-2][i_2+i_0][i_1] * F[i_2+i_1-1][i_0][i_2] + A[i_0+2][i_2+i_1+2][i_0] + 2;
            }
            for (int i_3 = 0; i_3 < -i_1+PB_M-1; i_3++) {
                B[i_3][i_1+1][i_1] = F[i_1][i_3][i_0] - F[i_1+i_0][i_3+1][2] * E[i_3+i_1+1][i_0] + 4;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = 0; i_5 < PB_L; i_5++) {
            for (int i_6 = 0; i_6 < PB_L; i_6++) {
                C[i_6][i_5][i_5] = E[i_6][i_5] - E[i_6][i_5] + 5;
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_L; i_7++) {
        for (int i_8 = 2; i_8 < min(PB_Q-2, -i_7+PB_L); i_8++) {
            for (int i_9 = 2; i_9 < -i_8+PB_M; i_9++) {
                D[i_7][i_9][i_8-2][i_8] = E[i_9-2][i_8+1] - F[i_8][i_9-2][i_7] * F[i_9][i_8+i_7][i_7+2] * G[i_9+i_8][i_7][i_9][i_8+2] * 6;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_3422132113_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G)));
