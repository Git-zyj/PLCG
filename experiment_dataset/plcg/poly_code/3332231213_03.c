...
void kernel_3332231213_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_0-1][i_1-1] = A[i_0][i_1] * A[i_0-2][i_1-1] + A[i_0-1][i_1] + 3;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_P; i_3++) {
            B[i_2][i_3-1] = D[i_3-1][i_2] * D[i_2][i_3] * D[i_3+1][i_3+i_2] + B[i_2-1][i_3-1] + 5;
        }
    }
    for (int i_4 = 1; i_4 < PB_Q; i_4++) {
        for (int i_5 = i_4; i_5 < min(PB_P, -i_4+PB_L-1); i_5++) {
            C[i_5+i_4][i_4] = D[i_5-1][0] + D[i_5][i_4] - D[i_5+1][i_4] - D[i_4-1][i_5] + C[i_5+i_4+1][i_4] - 2;
        }
    }
#pragma endscop
...
kernel_3332231213_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
