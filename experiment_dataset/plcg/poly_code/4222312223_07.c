...
void kernel_4222312223_07(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-3; i_1++) {
            B[i_1+1][i_1+3] = C[i_0][i_0-2] * B[i_0][i_0+2] + 4;
        }
        for (int i_2 = i_0; i_2 < PB_M-3; i_2++) {
            B[i_2][i_2+2] = C[i_0][i_2] * B[i_2+1][i_2+3] * 4;
        }
    }
#pragma endscop
...
kernel_4222312223_07(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
