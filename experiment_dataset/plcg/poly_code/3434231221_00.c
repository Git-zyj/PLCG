...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_L-1; i_1++) {
        B[i_1-1] = F[i_1-1] + F[i_1+1] + F[1] * G[i_1+1][i_1] * B[i_1+1] - B[i_1-2] * 5;
    }
    for (int i_2 = 2; i_2 < PB_P-1; i_2++) {
        C[i_2] = G[i_2][1] - F[i_2+1] - E[i_2-1] - E[i_2] - C[i_2+1] * 3;
        for (int i_3 = min(PB_P-1, -i_2+PB_M) - 1; i_3 >= 2; i_3--) {
            D[i_2][i_3] = H[i_3][i_3+i_2] + I[i_3][i_2+1][i_2] * C[i_3+2] - D[i_2-2][i_3-2] + D[i_2][i_3+1] - 6;
        }
    }
#pragma endscop
...
kernel_3434231221_00(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
