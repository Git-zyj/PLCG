...
void kernel_4226131311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = C[i_0-1] + C[1] + C[i_0] + A[i_0+1] - A[i_0-1] - A[i_0-1] + 3;
    }
    for (int i_1 = 1; i_1 < PB_Q+1; i_1++) {
        for (int i_2 = 2; i_2 < -i_1+PB_Q+1; i_2++) {
            for (int i_3 = 1; i_3 < -i_2+PB_Q; i_3++) {
                B[i_1][i_2-1][i_3][i_3+i_2] = D[i_3-1][i_1] - E[i_1-1][i_2][i_2] * B[i_3+1][i_2-2][i_1][i_2+i_1-1] - B[i_1][i_2-1][i_3-1][i_3+i_2-1] + 4;
            }
        }
    }
#pragma endscop
...
kernel_4226131311_08(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
