...
void kernel_2424351221_07(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N+1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_P-2; i_1++) {
            B[i_1+2][i_0][i_1+2] = E[i_1] - F[i_0][i_1+1] + G[i_1][i_0-1] * F[i_1][i_0+1] + H[i_0-1][i_1+1] - 2;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_N+1; i_2++) {
            B[i_2][i_0][i_2] = F[i_0][i_2] + G[i_2][i_0+1] * G[i_2][i_0-1] - I[i_2+i_0-1] * H[i_0+1][i_2+1] + 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_N; i_3++) {
        C[i_3] = E[i_3+1] - E[i_3] * E[i_3-1] * I[i_3] * C[i_3+1] + 3;
    }
    for (int i_4 = 2; i_4 < PB_P; i_4++) {
        for (int i_5 = i_4; i_5 < PB_P; i_5++) {
            D[i_4][1] = J[i_5-1] - J[i_4] + G[i_5][i_4] + H[i_5][i_4] - G[i_5+1][i_4] - F[i_5][i_4] * D[i_4-2][1] * D[i_4-1][1] * D[i_4+2][1] - 6;
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), INIT_SEED);
kernel_2424351221_07(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J)));
