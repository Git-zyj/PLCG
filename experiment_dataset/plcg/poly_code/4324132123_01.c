...
void kernel_4324132123_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-2; i_0++) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            for (int i_2 = 0; i_2 < min(min(min(-i_1+PB_M, PB_M-4), i_0), -i_0+PB_P-1); i_2++) {
                for (int i_3 = i_1; i_3 < -i_2+PB_M-4; i_3++) {
                    A[i_2+i_1][i_0-2][i_3+1] = D[i_3][i_2][i_0+2][i_1] * E[i_3+1][i_1-2][i_0] * D[i_1][i_2+i_0+1][i_3][0] - A[i_3+i_2+4][i_1][i_0+1] * 3;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        B[i_4] = F[i_4-1] + F[i_4] - F[i_4] + B[i_4+1] - B[i_4-1] * 6;
        for (int i_5 = 0; i_5 < PB_L-2; i_5++) {
            C[i_4][i_5] = F[1] + G[i_5][i_4-2] + G[i_4][i_5+2] - 1;
        }
    }
#pragma endscop
...
kernel_4324132123_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
