...
void kernel_2432351121_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_L-2; i_1++) {
            A[i_1][i_1] = E[i_1+1] + F[i_1][i_0-1] * G[i_1][i_0] + G[i_1-1][i_0+1] + A[i_1+2][i_1+2] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_M; i_2++) {
        B[i_2][i_2] = H[i_2] * H[i_2] * 5;
    }
    for (int i_3 = 1; i_3 < PB_Q; i_3++) {
        C[i_3] = E[i_3] - E[i_3+1] + H[0] * H[i_3-1] - E[i_3-1] + H[i_3] + C[i_3-1] - D[i_3] + 2;
        D[i_3] = E[i_3] * I[i_3] - H[i_3] * J[i_3][i_3] * I[i_3] - 6;
    }
#pragma endscop
...
kernel_2432351121_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J));
