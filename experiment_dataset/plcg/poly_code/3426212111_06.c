...
void kernel_3426212111_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 2; i_1 < min(PB_L, i_0); i_1++) {
            for (int i_2 = 3; i_2 < -i_0+PB_Q; i_2++) {
                A[i_0-2][i_1-2][i_2] = E[i_1][i_2+i_0][i_1] * A[i_0-3][i_2-3][i_1-1] - 2;
            }
        }
        for (int i_3 = i_0; i_3 < PB_M+1; i_3++) {
            for (int i_4 = 2; i_4 < -i_3+PB_M+1; i_4++) {
                A[i_0-3][i_4-2][i_3+1] = C[i_4+i_3-1][i_4] + 4;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M; i_5++) {
        for (int i_6 = i_5; i_6 < -i_5+PB_M; i_6++) {
            C[i_6+i_5-1][i_6] = C[i_6+i_5][i_5] - 3;
        }
        D[i_5] = 6;
    }
#pragma endscop
...
kernel_3426212111_06(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E));
