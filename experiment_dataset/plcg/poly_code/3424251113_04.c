...
void kernel_3424251113_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = E[i_0] * F[1] - F[i_0-1] + F[i_0] * 5;
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_N+1); i_1++) {
            B[i_0][i_1] = G[i_1-1][i_0] * G[i_0][i_1+1] - H[i_1][i_1+i_0-1][0] * I[i_1+i_0][i_1+1] + G[i_0][i_0-1] - A[i_0] + A[i_1] - 2;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            for (int i_4 = 1; i_4 < min(min(PB_P, i_2), -i_2+PB_L-1); i_4++) {
                B[i_3][i_4-1] = H[i_4][i_2][i_3] + H[i_3][i_4+i_2-1][0] - I[i_4][0] + H[i_3][i_4+i_2+1][i_2] * B[i_3][i_4] + 5;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_N; i_5++) {
        D[i_5] = E[i_5] * F[i_5-1] + F[i_5] * E[i_5] - F[i_5] + 6;
    }
#pragma endscop
...
kernel_3424251113_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
