...
void kernel_4234111123_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
    ...
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 0; i_5 < PB_M-1; i_5++) {
            for (int i_6 = max(i_4, 2); i_6 < PB_L+1; i_6++) {
                B[i_4][i_6-1][0] = C[i_4-1][i_5+1] * B[i_4][i_6-2][0] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4234111123_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
