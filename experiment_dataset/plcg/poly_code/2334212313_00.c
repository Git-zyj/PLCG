...
void kernel_2334212313_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        ...
        for (int i_4 = i_2; i_4 < PB_L-1; i_4++) {
            C[i_2][i_4] = C[i_2-1][i_4] - C[i_2][i_4+1] + C[i_2+1][i_4] * C[i_2+1][i_4+1] - C[i_2][i_4-1] - C[i_2+1][i_4-1] * 5;
        }
    }
#pragma endscop
...
kernel_2334212313_00(xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
