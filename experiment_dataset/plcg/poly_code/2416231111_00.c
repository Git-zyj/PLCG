...
void kernel_2416231111_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q; i_0++) {
        A[i_0] = E[i_0] + F[i_0] + E[i_0] + F[i_0] * C[i_0] - 6;
        for (int i_1 = i_0; i_1 < -i_0+PB_Q; i_1++) {
            B[i_1][i_1+i_0-1] = G[i_0+1][i_1+i_0] * E[i_0] * H[i_1][0] - 6;
        }
        C[i_0] = E[i_0] - F[i_0+1] + F[i_0-1] * 5;
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_Q, -i_2+PB_M); i_3++) {
            D[i_3-1] = I[i_3+i_2][i_3+i_2][1] + 1;
        }
    }
#pragma endscop
...
kernel_2416231111_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I));
