...
void kernel_3226212211_02(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_1 = 3; i_1 < PB_M+2; i_1++) {
        for (int i_2 = i_1; i_2 < -i_1+PB_M+2; i_2++) {
            B[i_2+i_1-2] = A[i_2] + B[i_2+i_1-3] + B[i_2+i_1-3] - 1;
        }
    }
#pragma endscop
...
kernel_3226212211_02(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
