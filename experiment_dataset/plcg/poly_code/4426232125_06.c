...
void kernel_4426232125_06(int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        C[i_0-1] = E[i_0] + F[i_0] - E[i_0] * E[i_0] * 5;
    }
    for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
        C[i_1+1] = E[i_1] * E[i_1] * E[i_1] * 4;
        C[i_1-1] = E[i_1] + 3;
    }
    for (int i_2 = 0; i_2 < PB_L-4; i_2++) {
        for (int i_3 = 2; i_3 < min(-i_2+PB_L-4, i_2); i_3++) {
            for (int i_4 = 0; i_4 < -i_2+PB_L; i_4++) {
                for (int i_5 = 0; i_5 < i_2; i_5++) {
                    D[i_2][i_5+i_3+2][i_3-2][i_4+i_2] = G[i_5][i_2+1][i_3][0] - H[i_2][i_4][i_5] - I[i_5][i_2][i_3][i_4] * I[i_3][i_4+i_2][i_5+i_3][0] * D[i_4][i_3+i_2+4][i_3-2][i_5+i_4] + 2;
                }
            }
        }
    }
...
init_array(xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4426232125_06(xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I)));
