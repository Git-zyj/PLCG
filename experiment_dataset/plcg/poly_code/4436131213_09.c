...
void kernel_4436131213_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M+2, i_1); i_2++) {
                for (int i_3 = 0; i_3 < min(min(min(-i_1+PB_M, -i_2+PB_M), PB_L-2), -i_0+PB_P-1); i_3++) {
                    A[i_3+i_1-1][i_2+1][i_3+i_2] = E[i_2-1][i_0][i_3][i_1+1] - F[i_3+i_2][i_1+1][i_0+1] * F[i_0][i_2][i_1] - E[i_2][i_0][i_1-1][0] - A[i_3+i_1][i_2+1][i_3+i_2] + A[i_2+i_0-2][i_3+2][i_3+i_0+1] * A[i_3+i_1-1][i_2][i_3+i_2-1] + 4;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L; i_4++) {
        for (int i_5 = 1; i_5 < i_4; i_5++) {
            for (int i_6 = 0; i_6 < -i_5+PB_Q; i_6++) {
                B[i_4][i_6][i_5] = G[i_4-1][i_5] - F[i_5+1][i_4][i_6+i_5] + F[i_6+1][i_4][i_5] - 2;
            }
            C[i_5-1][i_4][i_4] = H[1] - G[i_4][i_4] - C[i_5][i_4-1][i_4-1] - 6;
        }
        for (int i_7 = 1; i_7 < PB_L+1; i_7++) {
            for (int i_8 = 1; i_8 < PB_L; i_8++) {
                D[i_4][i_8][i_4] = F[i_7][i_8][i_4] - F[i_7][i_4][i_8] * F[i_7-1][i_4][i_8] + D[i_7-1][i_8-1][i_7-1] + 4;
            }
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4436131213_09(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H)));
