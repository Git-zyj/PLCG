...
void kernel_4412252221_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
        ...
        for (int i_1 = 1; i_1 < PB_N-2; i_1++) {
            for (int i_2 = 2; i_2 < i_1; i_2++) {
                for (int i_3 = 2; i_3 < PB_L-1; i_3++) {
                    A[i_2][i_3][i_0][i_0] = E[i_2][i_3][i_1-1][i_0-2] + E[i_3-2][i_2][i_1][i_0+1] + F[i_2][i_0+2][i_1-1][i_3] * A[i_2-2][i_3+1][i_0][i_0] - 2;
                }
            }
            for (int i_4 = 2; i_4 < -i_0+PB_L+1; i_4++) {
                B[i_4][i_1][i_0] = G[i_4][i_1] * H[i_1][i_0+1][i_4] * I[i_1][i_4-2][i_0-1] - F[i_4][i_0][i_4+1][i_1] * H[i_4-2][i_4+i_0-1][i_1] - I[i_1+2][i_4+i_0][i_1] - B[i_4][i_1+2][i_0] * B[i_1][i_4][i_0-1] + 5;
            }
        }
        C[i_0] = J[i_0] - K[i_0][2] * C[i_0-1] - 4;
    }
    for (int i_5 = 2; i_5 < PB_M-2; i_5++) {
        for (int i_6 = 1; i_6 < min(-i_5+PB_M-2, i_5); i_6++) {
            D[i_6] = K[i_5+2][i_6] + I[i_6][i_6+i_5+2][i_5] * G[i_6][i_5-2] + K[i_5][i_6] * G[i_6-1][i_5] * K[i_6+1][i_5] * D[i_6+1] + 4;
        }
    }
#pragma endscop
