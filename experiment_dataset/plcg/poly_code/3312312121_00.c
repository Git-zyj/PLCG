...
void kernel_3312312121_00(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
...
#pragma scop
    ...
    for (int i_2 = 0; i_2 < PB_P; i_2++) {
        for (int i_3 = 0; i_3 < PB_P; i_3++) {
            B[i_3+2][i_2] = D[i_3][i_3] * 5;
        }
        for (int i_4 = 2; i_4 < PB_L-2; i_4++) {
            C[i_4-2] = B[i_4+2][i_2] + 3;
        }
    }
#pragma endscop
...
kernel_3312312121_00(xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
