...
void kernel_2336312213_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
...
#pragma scop
    ...
    for (int i_1 = 2; i_1 < PB_N+2; i_1++) {
        for (int i_2 = 1; i_2 < min(-i_1+PB_N+2, i_1); i_2++) {
            C[i_1][i_2-1] = D[i_2+i_1-2][i_2+i_1+1] + E[i_1] * 2;
        }
    }
    for (int i_3 = 1; i_3 < PB_M; i_3++) {
        for (int i_4 = i_3; i_4 < PB_P-1; i_4++) {
            C[i_3][i_4] = C[i_3][i_4-1] * C[i_3-1][i_4+1] * C[i_3-1][i_4] + C[i_3][i_4+1] + 5;
        }
    }
#pragma endscop
...
kernel_2336312213_04(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
