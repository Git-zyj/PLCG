...
void kernel_2234312115_00(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        A[i_0] = C[i_0][0] * A[i_0+1] + 5;
    }
    for (int i_1 = 1; i_1 < PB_P; i_1++) {
        for (int i_2 = 0; i_2 < -i_1+PB_P; i_2++) {
            B[i_2][i_1] = D[i_2+i_1-1][i_2+i_1] + 3;
        }
    }
#pragma endscop
...
kernel_2234312115_00(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
