...
void kernel_4334351213_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,int zh,int vh,DATA_TYPE POLYBENCH_4D(H,xH,yH,zH,vH,xh,yh,zh,vh),int xi,int yi,int zi,DATA_TYPE POLYBENCH_3D(I,xI,yI,zI,xi,yi,zi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M; i_2++) {
                for (int i_3 = 1; i_3 < PB_M; i_3++) {
                    A[i_0][i_2+i_0][i_1][i_2] = D[i_0][i_2][i_3][i_1] - E[i_1][i_3-1][i_2][i_0-1] + F[i_3-1][i_1+1][i_2] - G[i_2][i_1][i_0-1][i_3][i_0+1] + H[i_3][i_1][i_0][i_2] - E[i_1][i_3][i_2][i_1] + H[i_2][i_0][i_3][i_2+1] * D[i_3][i_0][i_3-1][i_2] * 3;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-2; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_L; i_5++) {
            for (int i_6 = 1; i_6 < min(min(PB_N-1, -i_5+PB_L), -i_4+PB_M+1); i_6++) {
                B[i_6][i_4+1][i_5] = I[i_6-1][i_5+i_4][0] + J[i_6+1][i_5] - I[i_6][i_5][i_4-1] - K[i_6+1][i_5+1][i_5+i_4] - D[i_4][i_6+i_5][i_6+1][i_5+1] * A[i_6-1][i_6+i_4-1][i_5][i_4] * A[i_5-1][i_5+i_4-1][i_6][i_4] - A[i_5-1][i_6+i_5-1][i_4][i_6] * B[i_6+1][i_4+1][i_5] + 3;
            }
        }
        for (int i_7 = 1; i_7 < -i_4+PB_M-1; i_7++) {
            C[i_7+i_4+1][i_4+1] = J[i_7][i_4] + J[i_7-1][i_4] - C[i_7+i_4][i_4] * C[i_7+i_4+1][i_4+2] - 6;
        }
    }
#pragma endscop
...
kernel_4334351213_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,POLYBENCH_ARRAY(H), xi,yi,zi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
