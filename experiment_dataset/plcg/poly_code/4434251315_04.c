...
void kernel_4434251315_04(int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        D[i_0-1] = E[i_0+1] - E[i_0-1] * E[i_0] * E[i_0] - E[i_0] - E[i_0] - F[i_0+1] - 2;
    }
    for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
        for (int i_2 = i_1; i_2 < min(-i_1+PB_L-2, PB_P-1); i_2++) {
            for (int i_3 = max(1, i_2); i_3 < PB_P; i_3++) {
                B[i_3][i_2+i_1+1][i_3] = G[i_2+1][i_3] * H[i_1][i_2][i_2+1] - I[i_2][i_3] * B[i_3-1][i_2+i_1+2][i_3-1] - B[i_3][i_2+i_1+2][i_3] + 5;
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_M; i_4++) {
        for (int i_5 = 1; i_5 < PB_M; i_5++) {
            D[i_5-1] = I[i_5][i_4] - G[i_4][i_4] * G[i_5-1][i_5] * G[i_5-1][i_4-1] * 3;
        }
    }
    for (int i_6 = 2; i_6 < PB_M-1; i_6++) {
        D[i_6-1] = F[i_6+1] + F[i_6] - I[i_6][1] - E[i_6] - D[i_6] + D[i_6-2] + D[i_6] - 5;
    }
#pragma endscop
...
init_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4434251315_04(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xd,POLYBENCH_ARRAY(D), xb,yb,zb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I)));
