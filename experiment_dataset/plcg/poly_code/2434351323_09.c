...
void kernel_2434351323_09(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 3; i_1 < min(-i_0+PB_M, PB_L-2); i_1++) {
            A[i_1-1][i_0] = E[i_1+i_0] - F[i_1][0] - G[i_1+1] * F[i_1-1][i_0-1] - H[i_1][i_0] - A[i_1-3][i_0] + A[i_0-1][i_1+2] * A[i_1-3][i_0+1] - A[i_1][i_0-1] * 1;
        }
        C[i_0] = I[i_0] * I[i_0] - G[i_0] * E[i_0] + G[i_0] - E[i_0+1] - G[i_0] + 5;
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        C[i_2] = E[i_2] + I[0] * E[i_2+1] * C[i_2+2] - C[i_2+1] + 2;
    }
    for (int i_3 = 2; i_3 < PB_M-1; i_3++) {
        C[i_3-2] = G[i_3+1] * E[i_3] + E[i_3+1] * 2;
    }
#pragma endscop
...
init_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_2434351323_09(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
