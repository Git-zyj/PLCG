...
void kernel_3434231215_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_N; i_0++) {
        A[i_0] = E[0] - E[i_0-1] * F[i_0] - A[i_0+1] + 1;
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            B[i_1][i_0-1] = F[i_1] - G[i_1][i_0] - F[i_1] + A[i_1-1] * B[i_1][i_0] + 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_N; i_2++) {
        A[i_2-1] = F[i_2] + E[0] + F[i_2+1] + F[i_2] * 4;
    }
    for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
        for (int i_4 = 0; i_4 < PB_Q; i_4++) {
            for (int i_5 = 1; i_5 < PB_M-1; i_5++) {
                B[i_4][i_5-1] = H[i_5+1][i_3-1][i_4] * 4;
            }
        }
    }
#pragma endscop
...
kernel_3434231215_05(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
