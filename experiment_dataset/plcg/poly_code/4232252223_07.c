...
void kernel_4232252223_07(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P; i_0++) {
        for (int i_1 = 2; i_1 < PB_P-1; i_1++) {
            for (int i_2 = 2; i_2 < min(min(-i_0+PB_M, PB_Q-2), -i_1+PB_M-1); i_2++) {
                for (int i_3 = i_0; i_3 < min(-i_1+PB_P, -i_0+PB_L-1); i_3++) {
                    A[i_2-1][i_2+i_0][i_3+i_1][i_0] = C[i_0][i_2][i_3+i_1][i_2] * C[i_1+1][i_3][i_0][i_2+2] + D[i_3][i_1][i_0][i_2-1] * D[i_1][i_2][i_3+i_0-2][i_2-2] - D[i_3+1][i_2][i_0][i_1] * C[i_3][i_0][i_1][i_2+i_1-1] - A[i_2-1][i_2+i_0][i_3+i_1+1][i_0] * A[i_2-1][i_2+i_0][i_3+i_1-2][i_0] - A[i_2-1][i_2+i_1+1][i_3+i_0+1][i_1+1] * 2;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
        for (int i_5 = 1; i_5 < -i_4+PB_M-2; i_5++) {
            B[2] = E[i_4][i_5][i_5-1] + F[i_5+i_4+2][2] * G[i_5+1][i_4] - E[i_4][i_5][0] - B[2] - 1;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4232252223_07(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
