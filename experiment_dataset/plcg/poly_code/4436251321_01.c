...
void kernel_4436251321_01(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk),int xl,int yl,DATA_TYPE POLYBENCH_2D(L,xL,yL,xl,yl)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_L; i_0++) {
        for (int i_1 = 2; i_1 < PB_L; i_1++) {
            for (int i_2 = max(3, i_1); i_2 < min(-i_1+PB_N, -i_0+PB_N); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_1+PB_M, -i_2+PB_N-1); i_3++) {
                    A[i_2+i_0][i_3+i_1][i_0-1][i_2-1] = E[i_3][i_0+1][i_2][i_1] * F[i_3+i_2+1][i_1][0][i_0] - G[i_2+i_1][i_3][i_0+1] + F[i_3+i_1+1][i_0][i_2-1][0] + H[i_2][i_1-1][i_1][i_3][i_0-1] * A[i_2+i_0-4][i_3+i_1][i_2-3][i_0-3] * A[i_2+i_0-1][i_3+i_1-2][i_0-2][i_2-1] * 6;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_L-1; i_4++) {
        B[i_4+1] = I[i_4] - I[i_4+1] - J[i_4] - J[i_4] - C[i_4-1] - C[i_4] * 5;
        C[i_4-1] = J[i_4] - J[i_4] - I[i_4-1] + I[i_4-1] * B[i_4+1] - B[i_4-1] - 4;
    }
    for (int i_5 = 2; i_5 < PB_L-1; i_5++) {
        for (int i_6 = 1; i_6 < min(PB_M-3, i_5); i_6++) {
            D[i_5][i_6+1] = K[i_6][i_5-1] + L[i_6][i_5+1] - K[i_6][i_5] + K[i_6-1][1] * B[i_6+1] - D[i_5+2][i_6+1] + D[i_5][i_6+2] + D[i_5-2][i_6] + D[i_5][i_6+3] - 4;
        }
    }
#pragma endscop
...
kernel_4436251321_01(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), xl,yl,POLYBENCH_ARRAY(L));
