...
void kernel_2212312311_03(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_L-3, i_0); i_1++) {
            A[i_0][i_1+2] = C[i_1][i_0-2] + A[i_0+1][i_1+3] - A[i_0+1][i_1+2] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_L-1; i_2++) {
        B[i_2][i_2] = D[i_2] + B[i_2+1][i_2+1] * B[i_2-1][i_2-1] * B[i_2+1][i_2+1] - 5;
    }
#pragma endscop
...
kernel_2212312311_03(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
