...
void kernel_4326311325_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
    ...
    for (int i_1 = 1; i_1 < PB_P-1; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < PB_P-2; i_2++) {
            for (int i_3 = i_2; i_3 < -i_1+PB_P-1; i_3++) {
                B[i_2][i_3-1][i_3+i_1] = D[i_1][i_3+1][i_1+1] + B[i_2][i_3-1][i_3+i_1+1] * 5;
            }
            for (int i_4 = 2; i_4 < PB_M+1; i_4++) {
                for (int i_5 = i_2; i_5 < PB_L; i_5++) {
                    C[i_4-1][i_5][i_2] = C[i_1-1][i_5][i_2+2] - C[i_4-1][i_2-2][i_5] + C[i_5-2][i_1][i_2] - C[i_4-2][i_5][i_2] * 5;
                }
            }
        }
    }
...
kernel_4326311325_01(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
