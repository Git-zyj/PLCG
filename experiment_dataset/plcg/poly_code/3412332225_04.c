...
void kernel_3412332225_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,int yi,DATA_TYPE POLYBENCH_2D(I,xI,yI,xi,yi),int xj,int yj,int zj,DATA_TYPE POLYBENCH_3D(J,xJ,yJ,zJ,xj,yj,zj),int xk,int yk,int zk,DATA_TYPE POLYBENCH_3D(K,xK,yK,zK,xk,yk,zk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M; i_0++) {
        A[i_0-2] = E[i_0] - F[i_0] * A[i_0-3] + 4;
        for (int i_1 = 0; i_1 < -i_0+PB_M+1; i_1++) {
            B[i_1][i_1+i_0-1] = G[i_1][i_0] - 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        for (int i_3 = 2; i_3 < -i_2+PB_M; i_3++) {
            B[i_3][i_3+i_2-1] = E[i_3] + H[i_3-1][i_2] * E[i_3+i_2] - A[i_3-2] + 6;
        }
        for (int i_4 = 1; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 1; i_5 < -i_2+PB_M; i_5++) {
                D[i_4][i_5-1][i_4] = I[i_5+i_2][i_4] - J[i_4][i_5][i_2] - K[i_4][i_2][i_2] - D[i_4-1][i_5-1][i_4-1] * D[i_4+2][i_5-1][i_4+2] - 3;
            }
        }
    }
#pragma endscop
...
kernel_3412332225_04(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,yi,POLYBENCH_ARRAY(I), xj,yj,zj,POLYBENCH_ARRAY(J), xk,yk,zk,POLYBENCH_ARRAY(K));
