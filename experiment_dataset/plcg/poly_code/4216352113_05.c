...
void kernel_4216352113_05(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            for (int i_2 = 2; i_2 < min(-i_0+PB_P+2, -i_1+PB_M-1); i_2++) {
                for (int i_3 = 1; i_3 < min(PB_P-1, -i_1+PB_L); i_3++) {
                    B[i_3][i_2+i_0-2][i_1][i_3-1] = C[i_1][i_3+1][i_2][i_0-2] * D[i_2+1][i_1+1][i_3][i_0] - D[i_2+i_1+1][i_2][i_3+i_1][i_0] * D[i_1][i_2-2][i_3][i_0] + 5;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < PB_Q-2; i_4++) {
        for (int i_5 = 2; i_5 < -i_4+PB_Q; i_5++) {
            for (int i_6 = 2; i_6 < min(min(-i_5+PB_P, PB_Q-2), -i_4+PB_P); i_6++) {
                for (int i_7 = 0; i_7 < -i_5+PB_L-1; i_7++) {
                    B[i_4][i_6+i_5][i_7][i_4-1] = D[i_6][i_7+1][i_4][i_5+i_4] * C[i_7+i_5][i_6+i_4][i_5-2][0] * E[i_6-2][i_7+1][i_5] + F[i_6-2][i_5][i_4+1][i_4+2] + G[i_5-2][i_7+i_5+1][i_4-1][i_6+2][i_6+2] + B[i_4][i_6+i_5][i_7+1][i_4-1] + 3;
                }
            }
        }
    }
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4216352113_05(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G)));
