...
void kernel_2422151111_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            A[i_0][i_1] = E[i_1+1] - F[i_0][i_1] * E[i_1] + F[i_1][i_0] * E[i_1] * 5;
        }
    }
    for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_M-1; i_3++) {
            B[i_3+i_2][i_2-1] = F[i_2][i_3-1] - F[i_2][i_3-1] + B[i_3+i_2+1][i_2] - B[i_3+i_2][i_2] * 2;
        }
        for (int i_4 = 0; i_4 < -i_2+PB_M; i_4++) {
            C[i_2-1][i_4] = F[i_4+i_2][i_2+1] * F[i_2][i_4] * E[i_4] - F[i_2][i_2] + C[i_2][i_4] - 3;
        }
    }
    for (int i_5 = 0; i_5 < PB_M; i_5++) {
        for (int i_6 = 1; i_6 < PB_M-1; i_6++) {
            D[i_6][i_5] = E[1] - F[i_6][i_5] * F[i_6-1][0] + E[i_6-1] - F[i_6+1][1] * F[i_6][i_5] * F[i_5][1] * E[i_5] - 2;
        }
    }
#pragma endscop
...
kernel_2422151111_05(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
