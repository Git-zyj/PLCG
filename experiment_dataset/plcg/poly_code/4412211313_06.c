...
void kernel_4412211313_06(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-1, PB_L-1); i_1++) {
            for (int i_2 = i_0; i_2 < min(-i_0+PB_M-2, -i_1+PB_M-1); i_2++) {
                A[i_1+i_0+1][i_2+1][0] = A[i_2+i_0+2][i_1+1][0] + A[i_2+i_1+1][i_0+2][0] - 2;
            }
        }
    }
        ...
        for (int i_4 = 1; i_4 < i_3; i_4++) {
            for (int i_5 = 1; i_5 < -i_3+PB_L+1; i_5++) {
                for (int i_6 = 1; i_6 < PB_P; i_6++) {
                    B[i_3][i_5][i_6][i_4] = E[i_6][i_5+i_3-1][i_4-1][i_4] - B[i_3][i_5][i_6-1][i_4] + B[i_3-1][i_5-1][i_6-1][i_4+1] + B[i_3][i_5-1][i_6][i_4] - B[i_3][i_5][i_6][i_4+1] - B[i_6][i_5][i_3][i_4+1] + 2;
                }
            }
        }
        C[i_3+1] = F[i_3-1] * C[i_3] * C[i_3+2] * 1;
        for (int i_7 = 0; i_7 < PB_N-1; i_7++) {
            C[i_7+1] = G[i_7][i_3+1] * 4;
        }
    }
#pragma endscop
...
init_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4412211313_06(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G)));
