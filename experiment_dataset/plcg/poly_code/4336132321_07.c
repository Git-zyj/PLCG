...
void kernel_4336132321_07(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-2, i_0); i_1++) {
            C[i_0-2][i_1] = D[i_1] - E[0][i_0-1] * D[i_1+i_0+2] + 2;
        }
    }
    for (int i_2 = 3; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 1; i_3 < -i_2+PB_L+2; i_3++) {
            for (int i_4 = 4; i_4 < min(i_2, -i_2+PB_M); i_4++) {
                B[i_4-2][i_3][i_2] = E[i_3][i_4] + F[i_2][i_4][i_3+i_2-2] * F[i_2][i_4-1][i_3] - F[i_4+i_2][i_3][i_2+2] * B[i_4][i_3][i_2] - B[i_4-3][i_2][i_3] - B[i_4-4][i_3-1][i_2] * B[i_2-3][i_3][i_4-2] - 6;
            }
        }
    }
    for (int i_5 = 2; i_5 < PB_M; i_5++) {
        for (int i_6 = 0; i_6 < PB_L-2; i_6++) {
            C[i_5][i_6] = E[i_6][i_5] + C[i_5-1][i_6] - C[i_5-2][i_6+2] * 2;
        }
    }
#pragma endscop
...
kernel_4336132321_07(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
