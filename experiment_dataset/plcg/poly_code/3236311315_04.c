...
void kernel_3236311315_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
    ...
    for (int i_1 = 0; i_1 < PB_Q-1; i_1++) {
        for (int i_2 = 1; i_2 < PB_L; i_2++) {
            for (int i_3 = 0; i_3 < PB_M; i_3++) {
                B[i_3][i_2][i_1+1] = B[i_3][i_2-1][i_1+1] + B[i_3][i_2][i_1] * 3;
            }
        }
    }
#pragma endscop
...
kernel_3236311315_04(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
