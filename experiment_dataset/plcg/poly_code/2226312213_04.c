...
void kernel_2226312213_04(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M; i_1++) {
            A[i_0+1][i_0] = A[i_1][i_1-1] * A[i_0+2][i_0+1] - 6;
        }
    }
    for (int i_2 = 0; i_2 < PB_M-2; i_2++) {
        A[i_2+1][i_2] = C[i_2+2] * 1;
    }
#pragma endscop
...
kernel_2226312213_04(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C));
