...
void kernel_3336212223_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
    ...
    for (int i_1 = 0; i_1 < PB_M; i_1++) {
        for (int i_2 = 4; i_2 < PB_M+4; i_2++) {
            for (int i_3 = 2; i_3 < PB_M; i_3++) {
                C[i_3-2][i_2] = C[i_3][i_2] * C[i_2-4][i_3] - 2;
            }
        }
    }
#pragma endscop
...
kernel_3336212223_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
