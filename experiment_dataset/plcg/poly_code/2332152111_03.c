...
void kernel_2332152111_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        A[i_0] = D[i_0][i_0] + E[i_0+2] + A[i_0+1] + 1;
    }
    for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
        for (int i_2 = i_1; i_2 < PB_L; i_2++) {
            A[i_2+1] = D[i_2+2][i_1+1] + D[i_1+2][i_2] + D[i_2][i_1] * E[i_1] * D[i_1+2][i_2] - D[i_2+2][i_1+1] - 1;
        }
    }
    for (int i_3 = 1; i_3 < PB_L-2; i_3++) {
        for (int i_4 = i_3; i_4 < -i_3+PB_L; i_4++) {
            C[i_3][i_4+i_3] = D[i_4][i_3-1] + D[i_3][i_4+1] * D[i_4+i_3+2][i_3+1] * D[i_3][i_4] + D[i_4][i_3+2] * D[i_4][i_3-1] - D[i_4][i_3+1] - C[i_3][i_4+i_3-1] - 2;
        }
    }
#pragma endscop
...
kernel_2332152111_03(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
