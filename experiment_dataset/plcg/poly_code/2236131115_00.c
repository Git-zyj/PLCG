...
void kernel_2236131115_00(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            B[i_1-1] = C[i_0][1] + C[i_1-1][i_0+1] - C[i_1][i_0-1] - 6;
        }
    }
    for (int i_2 = 1; i_2 < PB_P; i_2++) {
        for (int i_3 = max(2, i_2); i_3 < PB_N+1; i_3++) {
            B[i_3-1] = C[i_3][i_2-1] - C[i_3][i_2] * B[i_3-2] + 1;
        }
    }
#pragma endscop
...
kernel_2236131115_00(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
