...
void kernel_3214212111_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            for (int i_2 = PB_M-1 - 1; i_2 >= max(1, i_1); i_2--) {
                A[i_0+1][i_2+2][i_0] = A[i_2][i_0+2][i_2-1] * A[i_2+1][i_1+1][i_2] - 1;
            }
        }
        B[i_0-2] = C[i_0] - C[0] - 1;
    }
...
kernel_3214212111_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
