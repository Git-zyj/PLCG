...
void kernel_4434332111_04(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,int we,DATA_TYPE POLYBENCH_5D(E,xE,yE,zE,vE,wE,xe,ye,ze,ve,we),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-2; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_Q+1; i_2++) {
                for (int i_3 = 0; i_3 < min(min(-i_1+PB_M, PB_P), -i_2+PB_Q); i_3++) {
                    A[i_1+2][i_3][i_0][i_2-1] = E[i_0+1][i_3][i_2][i_0][i_2+i_1-1] * F[i_3+i_1][i_0][i_2][i_1] + G[i_1+1][i_0+2][i_3+i_2][i_3] * A[i_1+2][i_3][i_0-1][i_2-1] + 2;
                }
            }
        }
        B[i_0] = H[i_0] + I[0] * H[i_0] + B[i_0-1] * 4;
        for (int i_4 = 0; i_4 < PB_P; i_4++) {
            for (int i_5 = 2; i_5 < PB_P-2; i_5++) {
                for (int i_6 = 0; i_6 < min(PB_P-1, -i_4+PB_P); i_6++) {
                    C[i_5-2][i_0][i_6+i_4][i_4] = F[i_4][i_0][i_5+2][i_6+1] - F[i_5][i_6][i_0-2][i_4] * 2;
                }
            }
        }
    }
        ...
        for (int i_8 = 0; i_8 < min(PB_P-1, i_7); i_8++) {
            for (int i_9 = 1; i_9 < PB_Q; i_9++) {
                for (int j_0 = 0; j_0 < PB_Q; j_0++) {
                    D[0][i_8][i_9][i_8] = G[i_8][i_7][j_0][i_7] + D[0][i_8+1][i_7][i_8+1] - D[0][i_8+1][i_9-1][i_8+1] - 1;
                }
            }
        }
    }
...
init_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4434332111_04(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,we,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
