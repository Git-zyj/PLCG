...
void kernel_3314212123_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_M; i_0++) {
        for (int i_1 = i_0; i_1 < min(-i_0+PB_M, PB_N-1); i_1++) {
            A[i_1] = A[i_1+2] * C[i_1+1][i_1+i_0] - 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_Q; i_2++) {
        B[i_2] = B[i_2-2] * 6;
        for (int i_3 = 0; i_3 < min(PB_N-2, -i_2+PB_M); i_3++) {
            C[i_3+2][i_3+i_2] = D[i_3][i_2] * 1;
        }
    }
#pragma endscop
...
kernel_3314212123_08(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
