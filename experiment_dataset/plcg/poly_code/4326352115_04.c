...
void kernel_4326352115_04(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg)){
...
#pragma scop
        ...
        for (int i_1 = 0; i_1 < PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < i_1; i_2++) {
                for (int i_3 = 0; i_3 < min(i_0, -i_2+PB_P); i_3++) {
                    B[i_3][i_1+1][i_2+1] = D[i_0][i_1+2][i_3+i_2][i_0-2] * D[i_1][i_0+2][i_3][2] + 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L-1; i_4++) {
        for (int i_5 = i_4; i_5 < -i_4+PB_M-1; i_5++) {
            for (int i_6 = 2; i_6 < min(min(i_4, -i_5+PB_Q), PB_L-2); i_6++) {
                for (int i_7 = 2; i_7 < -i_5+PB_M; i_7++) {
                    B[i_7][i_4+1][i_6] = E[i_7][i_5+2][i_4][i_4+1] - F[i_6+2][i_5][i_7+1][i_6+i_5] * E[i_7-1][i_4][i_6-1][i_7] - E[i_5][i_7][i_4][i_6-2] - D[i_5+i_4+1][i_6][i_4][i_7] - G[i_6][i_5][i_6] + F[i_7-2][i_4-2][i_6][i_5-1] * E[i_6][i_7+i_5][i_4][i_4] + D[i_7][i_4][i_7-1][i_6] + B[i_6][i_7+1][i_5+1] * 5;
                }
            }
        }
        for (int i_8 = 0; i_8 < min(i_4, -i_4+PB_M+2); i_8++) {
            for (int i_9 = max(i_8, 2); i_9 < PB_M-1; i_9++) {
                for (int j_0 = 0; j_0 < -i_4+PB_L; j_0++) {
                    B[i_9+1][i_4][j_0] = G[i_9-2][j_0][i_8+2] + F[j_0+i_4][i_9][i_4][i_8] * E[i_8+i_4-2][i_9-1][j_0][j_0] - 1;
                }
            }
        }
    }
...
init_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), INIT_SEED);
kernel_4326352115_04(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G));
polybench_prevent_dce(print_array(xb,yb,zb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G)));
