...
void kernel_2436351123_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,DATA_TYPE POLYBENCH_1D(J,xJ,xj)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-1; i_0++) {
        for (int i_1 = 1; i_1 < min(-i_0+PB_M, PB_P); i_1++) {
            A[i_1] = E[i_1] + F[i_1][i_0-1] * G[i_1][i_0+1] * F[i_1+i_0][0] * G[i_1][i_0-1] * A[i_1-1] + D[i_1] - 3;
        }
        for (int i_2 = 1; i_2 < min(PB_P+1, -i_0+PB_L-1); i_2++) {
            B[i_2-1][i_2+i_0+1] = G[i_2-1][i_0+1] * F[i_2][i_0] * F[i_2][i_0+1] - H[i_0-1][i_2-1] + E[i_2+1] + H[i_2+i_0+1][i_0+1] * I[i_0] + 6;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        C[i_3+1] = I[i_3] + J[i_3] - J[i_3-1] - J[i_3] * 1;
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        D[i_4] = J[i_4] - I[0] - C[i_4+1] * 2;
    }
#pragma endscop
...
kernel_2436351123_09(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,POLYBENCH_ARRAY(J));
