...
void kernel_3232132223_09(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < -i_0+PB_M-3; i_2++) {
                A[i_2+i_0+1][i_1][i_0] = C[i_2][i_0][i_1] - C[i_1+1][i_2][i_0] + A[i_2+i_0+3][i_1][i_0+2] - A[i_2+i_0+3][i_1][i_0] + 2;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_L; i_3++) {
        for (int i_4 = 2; i_4 < min(-i_3+PB_M-1, i_3); i_4++) {
            for (int i_5 = 0; i_5 < min(-i_3+PB_N-1, PB_L); i_5++) {
                A[i_4+i_3+1][i_5][i_4] = C[i_5+i_3+1][i_4-2][0] * C[i_5][i_4][i_3] * C[i_3][i_4][i_4-2] + 5;
            }
        }
    }
#pragma endscop
...
kernel_3232132223_09(xa,ya,za,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C));
