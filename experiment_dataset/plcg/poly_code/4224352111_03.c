...
void kernel_4224352111_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = C[i_0] - D[i_0] - D[i_0+1] + D[i_0] + D[i_0] * A[i_0-1] - 2;
    }
    for (int i_1 = 1; i_1 < PB_P-2; i_1++) {
        for (int i_2 = 2; i_2 < PB_L; i_2++) {
            for (int i_3 = 2; i_3 < min(i_2, PB_L-2); i_3++) {
                for (int i_4 = 2; i_4 < PB_P-2; i_4++) {
                    B[i_1][i_3+2][i_4-1][i_2-2] = E[i_2-1][i_2][i_4-2][i_1] * F[i_3-2][i_4-1][i_2][i_1] + F[i_4][i_3+2][i_1-1][i_1+2] * G[i_4][i_2][i_4+2][i_1+2] + F[0][i_4-1][i_1][i_3] * 5;
                }
            }
        }
    }
...
kernel_4224352111_03(xa,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G));
