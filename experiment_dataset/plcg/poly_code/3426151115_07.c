...
void kernel_3426151115_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,DATA_TYPE POLYBENCH_1D(G,xG,xg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M; i_1++) {
            A[i_0-1][i_1] = E[i_1][i_0+1] * E[i_1][i_0-1] * E[i_0][i_1] - 5;
        }
        for (int i_2 = 2; i_2 < min(-i_0+PB_M, i_0); i_2++) {
            A[i_2-2][i_0] = E[i_2+1][i_0-1] + E[i_2][i_0+1] * E[i_2+i_0][0] - E[i_2][i_0-1] + E[i_2+1][i_0-1] * E[i_2+i_0-1][0] + E[i_0][i_2] - E[i_0][i_0] - E[i_0-1][0] + 4;
        }
        for (int i_3 = 1; i_3 < min(i_0, -i_0+PB_N); i_3++) {
            for (int i_4 = 1; i_4 < PB_M-1; i_4++) {
                C[i_3][i_4][i_0] = F[i_3+1][i_4][i_0] * E[i_4-1][i_3] + F[i_3+i_0][i_4+1][i_3+1] * C[i_3][i_4][i_0+1] * C[i_3-1][i_4][i_0] - 4;
            }
        }
    }
    for (int i_5 = 1; i_5 < PB_M+1; i_5++) {
        for (int i_6 = 0; i_6 < PB_M; i_6++) {
            A[i_5-1][i_6] = E[i_5-1][i_6+1] * E[i_6][i_5] + G[i_6] - E[i_6][i_5] + 5;
        }
    }
#pragma endscop
...
kernel_3426151115_07(xa,ya,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,POLYBENCH_ARRAY(G));
