...
void kernel_3216132111_07(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 2; i_2 < PB_M+2; i_2++) {
                A[i_2-2][i_0] = C[i_0-2][i_1][i_2] * 6;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_L-1; i_3++) {
            B[i_3] = D[i_3][i_0] * D[i_3][i_3+1] * D[i_3+2][0] * D[i_3+i_0-1][i_3+i_0+1] * B[i_3+1] - 2;
        }
    }
#pragma endscop
...
kernel_3216132111_07(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
