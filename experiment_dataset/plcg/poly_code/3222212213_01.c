...
void kernel_3222212213_01(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P-2; i_0++) {
        for (int i_1 = 0; i_1 < -i_0+PB_L+1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_M-2, PB_P-1); i_2++) {
                B[i_2+i_0+2][i_1+i_0-1][i_2+1] = C[i_1][i_0+2][i_2] + 5;
            }
        }
        for (int i_3 = 0; i_3 < -i_0+PB_L-1; i_3++) {
            for (int i_4 = 0; i_4 < min(PB_P, -i_0+PB_M-2); i_4++) {
                B[i_4+i_0+1][i_3+i_0][i_4] = B[i_4+i_0+2][i_3+i_0+1][i_4] + B[i_4+i_0+1][i_3+i_0+1][i_4] - 5;
            }
        }
    }
#pragma endscop
...
kernel_3222212213_01(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
