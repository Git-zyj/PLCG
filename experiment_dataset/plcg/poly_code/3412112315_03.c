...
void kernel_3412112315_03(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = A[i_0+1] * A[i_0-1] - A[i_0-1] * 2;
        for (int i_1 = i_0; i_1 < PB_L+1; i_1++) {
            A[i_1] = E[i_0][i_1-1] * A[i_1] + 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L; i_2++) {
        for (int i_3 = 0; i_3 < PB_Q-2; i_3++) {
            for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
                D[i_2+2][i_3] = F[i_4][i_2][i_3+2] + D[i_4+2][i_3] * 4;
            }
            D[i_2+2][i_3] = E[i_3+2][i_2] + D[i_2+2][i_3+1] * 4;
        }
    }
#pragma endscop
...
kernel_3412112315_03(xa,POLYBENCH_ARRAY(A), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
