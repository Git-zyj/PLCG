...
void kernel_4426132313_07(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,int vh,int wh,DATA_TYPE POLYBENCH_5D(H,xH,yH,zH,vH,wH,xh,yh,zh,vh,wh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_L-2; i_0++) {
        B[i_0-2] = E[i_0+1] + B[i_0-2] - 4;
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            B[i_1-2] = F[i_1][i_0] - B[i_1-1] * B[i_1-3] + 4;
        }
        B[i_0-1] = E[i_0] - E[i_0+1] * E[i_0+2] - E[i_0] + E[0] * B[i_0-3] + 3;
    }
    for (int i_2 = 0; i_2 < PB_L-2; i_2++) {
        for (int i_3 = 1; i_3 < PB_P+1; i_3++) {
            for (int i_4 = i_3; i_4 < min(PB_P, -i_2+PB_Q); i_4++) {
                for (int i_5 = 0; i_5 < -i_4+PB_P; i_5++) {
                    D[i_2][i_3][i_5+i_4][i_4] = G[i_4][i_2][i_3-1][i_5] - G[i_4][i_2][i_5][i_3] - H[i_5+i_4][i_2+2][i_3][i_3][i_4+i_2] - D[i_2][i_3-1][i_5+i_4-1][i_4] - D[i_2+1][i_3][i_5+i_4-1][i_4-1] + 1;
                }
            }
        }
    }
...
init_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4426132313_07(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,vh,wh,POLYBENCH_ARRAY(H)));
