...
void kernel_4434112211_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 0; i_1 < i_0; i_1++) {
            A[i_1+2][i_0+1][i_1] = E[i_1+2][i_0] + A[i_1+2][i_0+2][i_1] - A[i_0+1][i_1+2][i_0-1] + 6;
        }
        for (int i_2 = 0; i_2 < PB_P; i_2++) {
            for (int i_3 = i_2; i_3 < -i_2+PB_P; i_3++) {
                B[i_0][i_3+i_2][i_0-1] = 6;
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_P-3; i_4++) {
        for (int i_5 = 1; i_5 < PB_P-2; i_5++) {
            for (int i_6 = 0; i_6 < min(-i_5+PB_L, -i_4+PB_L-1); i_6++) {
                C[i_4][i_6+i_5][i_5+2][i_4] = C[i_5-1][i_6+i_4+1][i_4+3][i_5-1] - 3;
            }
        }
    }
    for (int i_7 = 0; i_7 < PB_Q; i_7++) {
        for (int i_8 = 1; i_8 < PB_P-2; i_8++) {
            for (int i_9 = 0; i_9 < min(i_8, -i_8+PB_L-1); i_9++) {
                D[i_7][i_8][i_7] = C[i_7][i_9+i_8+1][i_8+2][i_7] + D[i_7][i_8-1][i_7] - 4;
            }
        }
    }
#pragma endscop
...
kernel_4434112211_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
