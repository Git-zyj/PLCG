...
void kernel_3232252311_01(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < -i_0+PB_N-1; i_1++) {
            for (int i_2 = 1; i_2 < -i_1+PB_N-2; i_2++) {
                A[i_1+i_0][i_1][i_2] = C[i_1][i_2][i_1+i_0-2] - D[i_0][i_1][i_2+i_1] - E[i_1-1][i_2] + D[i_1][i_2-1][i_0+2] - D[i_1][i_2+2][i_0] - C[i_2][i_0-2][i_1+1] + A[i_1+i_0+1][i_1][i_2+1] + A[i_1+i_0+1][i_1][i_2] * A[i_2+i_1+2][i_2+1][i_0-1] - 5;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_L-2; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-1; i_4++) {
            for (int i_5 = 1; i_5 < -i_3+PB_L-2; i_5++) {
                B[i_4][i_5][i_3-2] = C[i_4+1][i_5+i_3+2][i_3+2] + C[i_5][i_4][i_3] * C[i_4][i_5+i_3][i_5-1] * C[i_5][i_3+1][i_5] + B[i_5][i_4][i_3-3] - B[i_4][i_5-1][i_3-1] * B[i_4][i_5-1][i_3-2] + 6;
            }
        }
    }
#pragma endscop
...
kernel_3232252311_01(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E));
