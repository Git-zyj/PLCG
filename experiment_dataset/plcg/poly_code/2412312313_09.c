...
void kernel_2412312313_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-3; i_2++) {
        C[i_2+2] = A[i_2-1] * A[i_2] * C[i_2+3] + 2;
        for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
            D[i_2][i_3] = B[i_3+1] - C[i_3+1] + 4;
        }
    }
#pragma endscop
...
kernel_2412312313_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
