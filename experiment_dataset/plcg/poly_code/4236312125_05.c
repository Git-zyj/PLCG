...
void kernel_4236312125_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_M-1 - 1; i_0 >= 1; i_0--) {
        A[i_0] = 4;
        for (int i_1 = max(2, i_0); i_1 < -i_0+PB_M-1; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_0+PB_P+1, PB_L-1); i_2++) {
                B[i_1+i_0][i_2][i_0-1] = C[i_1-2][i_2+i_0-1][i_0+2] * C[i_2][i_2][i_1-1] - B[i_1+i_0+1][i_2+1][i_0-1] * 1;
            }
        }
    }
#pragma endscop
...
kernel_4236312125_05(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
