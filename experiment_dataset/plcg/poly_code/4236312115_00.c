...
void kernel_4236312115_00(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_Q-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M+1; i_1++) {
            for (int i_2 = 0; i_2 < min(PB_P, i_0); i_2++) {
                B[i_0-1][i_1-1][i_2] = C[i_1-1][i_2][i_0+1] - 3;
            }
        }
        for (int i_3 = 0; i_3 < PB_P; i_3++) {
            for (int i_4 = 0; i_4 < PB_P-1; i_4++) {
                B[i_0][i_3][i_4] = B[i_0][i_4+1][i_3] * 3;
            }
        }
    }
#pragma endscop
...
kernel_4236312115_00(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
