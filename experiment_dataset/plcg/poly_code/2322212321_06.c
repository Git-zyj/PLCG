...
void kernel_2322212321_06(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        A[i_0][1] = A[i_0+1][1] + A[i_0-2][1] + A[i_0+2][1] + A[i_0-1][1] + 6;
    }
    for (int i_1 = 0; i_1 < PB_M-2; i_1++) {
        C[i_1+1] = D[i_1+2] + D[i_1] - 5;
    }
    for (int i_2 = 2; i_2 < PB_L; i_2++) {
        C[i_2+1] = D[2] + A[i_2-2][1] - 1;
    }
#pragma endscop
...
kernel_2322212321_06(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D));
