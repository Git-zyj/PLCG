...
void kernel_2214132121_09(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_N; i_0++) {
        for (int i_1 = 2; i_1 < min(min(PB_N-1, i_0), -i_0+PB_N); i_1++) {
            B[i_1-2] = C[i_1+1][i_0] + C[i_1][i_1+1] - C[i_1][i_1+i_0] * C[i_1][i_0] + 1;
        }
    }
    for (int i_2 = 3; i_2 < PB_M+3; i_2++) {
        for (int i_3 = 2; i_3 < PB_N; i_3++) {
            B[i_3-2] = C[i_3][i_3-1] + B[i_2-3] - 4;
        }
    }
#pragma endscop
...
kernel_2214132121_09(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
