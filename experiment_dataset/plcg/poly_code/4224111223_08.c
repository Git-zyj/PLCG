...
void kernel_4224111223_08(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
        ...
        for (int i_1 = 1; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_L; i_2++) {
                for (int i_3 = 0; i_3 < PB_L-1; i_3++) {
                    A[i_0-1][i_2][i_1][i_3] = A[i_1-1][i_3+1][i_2][i_0] - 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M; i_4++) {
        B[i_4][i_4] = C[i_4] * B[i_4-2][i_4-2] - B[i_4-1][i_4-1] - 5;
    }
#pragma endscop
...
kernel_4224111223_08(xa,ya,za,va,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
