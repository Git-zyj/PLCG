...
void kernel_3226112221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M-2; i_0++) {
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            A[i_0+2] = C[i_0+2][i_1] - A[i_0+4] + A[i_1] * 5;
        }
        for (int i_2 = i_0; i_2 < min(-i_0+PB_L, PB_M-1); i_2++) {
            B[i_2][i_0][i_2+i_0-1] = C[i_2][i_0] * B[i_2][i_0-1][i_2+i_0-2] + B[i_2+1][i_0][i_2+i_0] * 2;
        }
    }
#pragma endscop
...
kernel_3226112221_07(xa,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
