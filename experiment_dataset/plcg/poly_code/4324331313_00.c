...
void kernel_4324331313_00(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,int wf,DATA_TYPE POLYBENCH_5D(F,xF,yF,zF,vF,wF,xf,yf,zf,vf,wf),int xg,int yg,int zg,int vg,DATA_TYPE POLYBENCH_4D(G,xG,yG,zG,vG,xg,yg,zg,vg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
...
#pragma scop
        ...
        for (int i_1 = i_0; i_1 < PB_Q-1; i_1++) {
            for (int i_2 = 1; i_2 < PB_Q-1; i_2++) {
                for (int i_3 = 0; i_3 < min(-i_0+PB_M, -i_2+PB_M-1); i_3++) {
                    A[i_3+i_0][i_1][i_2-1][i_1+1] = D[i_3][i_1][i_0][i_2] - E[i_3][i_2][i_0][0] + F[i_2][i_1-1][i_3][0][i_2+1] * D[i_3+i_2+1][i_0-1][i_1+1][i_2] + 4;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_M-1; i_4++) {
        for (int i_5 = 2; i_5 < PB_N; i_5++) {
            for (int i_6 = 1; i_6 < min(PB_Q-1, i_5); i_6++) {
                for (int i_7 = 1; i_7 < min(-i_6+PB_M, -i_4+PB_M+1); i_7++) {
                    A[i_7+i_4-1][i_6][i_5-2][i_6+1] = G[i_5][i_4+1][i_6-1][i_4] - E[i_4][i_7+i_6][i_5][0] + H[i_7-1][i_6-1][i_5-1] - 2;
                }
            }
        }
        C[i_4-1] = I[i_4] - C[i_4] * C[i_4-2] - C[i_4-2] * C[i_4-2] * C[i_4-2] * 2;
    }
#pragma endscop
...
kernel_4324331313_00(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,wf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
