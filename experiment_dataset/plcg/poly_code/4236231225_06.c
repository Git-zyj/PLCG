...
void kernel_4236231225_06(int xa,int ya,int za,int va,DATA_TYPE POLYBENCH_4D(A,xA,yA,zA,vA,xa,ya,za,va),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_P; i_0++) {
        for (int i_1 = 1; i_1 < min(PB_P-3, -i_0+PB_L+1); i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_P-3; i_2++) {
                for (int i_3 = 2; i_3 < i_0; i_3++) {
                    A[i_3][i_1][i_2+i_1][1] = C[i_2][i_1+i_0-1][i_3] * D[i_3][i_0][i_0][i_1] - C[i_2+1][i_1-1][i_3] * A[i_3+1][i_1+2][i_2+i_1+3][1] * A[i_3-2][i_1][i_2+i_1][1] - 5;
                }
            }
        }
    }
        ...
        for (int i_5 = 0; i_5 < PB_P-1; i_5++) {
            for (int i_6 = 1; i_6 < min(i_4, -i_5+PB_P-1); i_6++) {
                for (int i_7 = 1; i_7 < min(i_4, PB_M-2); i_7++) {
                    A[i_7+2][i_5][i_6+i_5+1][1] = E[i_7-1][i_4][i_5+1][i_6-1] - D[i_7][i_7][i_4][i_6] - 6;
                }
            }
        }
    }
...
kernel_4236231225_06(xa,ya,za,va,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E));
