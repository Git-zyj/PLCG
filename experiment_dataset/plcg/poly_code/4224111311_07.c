...
void kernel_4224111311_07(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = max(2, i_0); i_1 < PB_L-1; i_1++) {
            for (int i_2 = 0; i_2 < -i_1+PB_L-1; i_2++) {
                A[i_0][i_2+1][i_2+i_1] = C[i_0][i_2+i_1+1] * A[i_0][i_2][i_2+i_1-1] + A[i_0+1][i_2+2][i_2+i_1+1] * A[i_0-1][i_2][i_2+i_1-2] * 1;
            }
        }
    }
        ...
        for (int i_4 = i_3; i_4 < PB_L-2; i_4++) {
            for (int i_5 = 1; i_5 < PB_P-1; i_5++) {
                for (int i_6 = 0; i_6 < PB_L-2; i_6++) {
                    B[i_4+1][i_6][i_3+1] = D[i_6][i_5-1][i_4+1] + B[i_6+2][i_5][i_3] * B[i_4+2][i_6][i_5+1] * B[i_4+1][i_6+1][i_5+1] + 5;
                }
            }
        }
    }
...
kernel_4224111311_07(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
