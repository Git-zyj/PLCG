...
void kernel_2324132221_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-2; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M, PB_P-3); i_1++) {
            A[i_0][i_1+1] = D[i_0][i_1] * D[i_1+i_0][2] * A[i_0-2][i_1+1] * A[i_0+2][i_1+3] * 2;
        }
        for (int i_2 = 1; i_2 < min(PB_P+1, -i_0+PB_M+1); i_2++) {
            B[i_0+2][i_2] = D[i_2-1][i_0] * D[i_2+i_0-1][i_2-1] + D[i_2+2][i_2-1] + 6;
        }
    }
    for (int i_3 = 0; i_3 < PB_M-2; i_3++) {
        C[i_3] = E[0] - E[i_3+2] + E[i_3] + E[i_3] - C[i_3+2] * C[i_3+1] * 4;
    }
#pragma endscop
...
kernel_2324132221_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
