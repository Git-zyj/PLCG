...
void kernel_3222312221_02(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < min(-i_0+PB_M-1, i_0); i_1++) {
            for (int i_2 = 1; i_2 < PB_P; i_2++) {
                A[i_1+i_0][i_0+1][i_2] = A[i_1+i_0+1][i_0+1][i_2-1] - 4;
            }
        }
    }
    for (int i_3 = 2; i_3 < PB_M-2; i_3++) {
        for (int i_4 = 2; i_4 < PB_M; i_4++) {
            B[i_3][i_4] = C[i_4] * B[i_3-2][i_4] + B[i_3-2][i_4-2] * B[i_3+2][i_4-2] * 1;
        }
    }
#pragma endscop
...
kernel_3222312221_02(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
