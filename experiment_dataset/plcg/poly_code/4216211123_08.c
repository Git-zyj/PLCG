...
void kernel_4216211123_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 3; i_0 < PB_M-1; i_0++) {
        A[i_0-1][i_0-1] = A[i_0-3][i_0-3] - A[i_0-2][i_0-2] * 2;
        for (int i_1 = i_0; i_1 < PB_L; i_1++) {
            for (int i_2 = 0; i_2 < PB_Q-1; i_2++) {
                for (int i_3 = 0; i_3 < min(PB_P, i_2); i_3++) {
                    B[i_0+1][i_3][i_2+1][0] = C[i_0][i_1][i_2][i_3] + 2;
                }
            }
        }
    }
...
kernel_4216211123_08(xa,ya,POLYBENCH_ARRAY(A), xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,yc,zc,vc,POLYBENCH_ARRAY(C));
