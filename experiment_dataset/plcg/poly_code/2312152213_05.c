...
void kernel_2312152213_05(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = PB_L - 1; i_0 >= 1; i_0--) {
        for (int i_1 = 2; i_1 < PB_M; i_1++) {
            A[i_1] = D[i_0][i_1-2] * D[i_1][i_1] + D[i_0][i_1-1] + D[i_1][i_0] - D[i_0][i_1] + D[i_0][i_1] * E[i_0] - A[i_1-1] - C[i_0][i_0-1] + 4;
        }
        A[i_0-1] = E[i_0] - E[i_0-1] + D[i_0][i_0-1] + E[i_0] + 1;
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        for (int i_3 = 0; i_3 < PB_L; i_3++) {
            C[i_2][i_2-1] = D[i_3][i_2] + E[i_2+1] + E[i_3] * C[i_2+1][i_2] * C[i_2-1][i_2-2] - 3;
        }
    }
#pragma endscop
...
kernel_2312152213_05(xa,POLYBENCH_ARRAY(A), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E));
