...
void kernel_2416251325_08(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            A[i_0][i_1+1] = E[i_1] - F[i_1-1] - G[i_1][i_0] + H[i_1-1][i_0] - A[i_0][i_1] - A[i_0-2][i_1-1] - 6;
        }
        for (int i_2 = 1; i_2 < -i_0+PB_L; i_2++) {
            C[i_2-1] = G[i_2][i_0] * F[i_2+i_0] + H[i_2][i_0] - H[i_0][i_2] * 3;
        }
    }
    for (int i_3 = 1; i_3 < PB_L; i_3++) {
        C[i_3] = E[i_3+1] - F[i_3] * F[i_3] * E[i_3] * E[i_3] + H[i_3][i_3-1] - E[0] + F[i_3] - C[i_3+1] + D[i_3] + D[i_3+1] + 4;
        D[i_3] = E[i_3+1] + E[i_3-1] + E[i_3] - E[i_3] + C[i_3] + C[i_3+2] + 2;
    }
#pragma endscop
...
kernel_2416251325_08(xa,ya,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H));
