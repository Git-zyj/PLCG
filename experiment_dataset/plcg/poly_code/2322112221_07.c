...
void kernel_2322112221_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        A[i_0+2] = 6;
        for (int i_1 = 1; i_1 < min(PB_L-4, -i_0+PB_M-2); i_1++) {
            B[i_1+i_0][i_0-1] = D[i_1][i_0] - A[i_1+4] + B[i_1+i_0-1][i_0-1] + B[i_1+i_0+2][i_0-1] - B[i_1+i_0-2][i_1-1] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_L-3; i_2++) {
        for (int i_3 = 0; i_3 < -i_2+PB_L-3; i_3++) {
            C[i_3+i_2+1][i_3] = C[i_3+i_2+2][i_3] - C[i_3+i_2+3][i_3+2] - 5;
        }
    }
#pragma endscop
...
kernel_2322112221_07(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
