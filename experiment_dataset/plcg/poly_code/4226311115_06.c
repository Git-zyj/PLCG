...
void kernel_4226311115_06(int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 1; i_1 < i_0; i_1++) {
            for (int i_2 = 0; i_2 < PB_M-1; i_2++) {
                B[i_0+2][i_1-1][i_1] = C[i_2+1][i_2+1][i_0] * 1;
            }
        }
    }
    for (int i_3 = 0; i_3 < PB_M-1; i_3++) {
        for (int i_4 = 0; i_4 < PB_M-2; i_4++) {
            for (int i_5 = 1; i_5 < PB_P; i_5++) {
                B[i_3+1][i_5-1][i_5] = B[i_4+2][i_5-1][i_5] * 5;
            }
        }
    }
#pragma endscop
...
kernel_4226311115_06(xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C));
