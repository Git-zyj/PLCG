...
void kernel_4414152223_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-1; i_0++) {
        A[i_0] = E[i_0-2] * E[i_0] + E[i_0] + E[i_0] - E[i_0] - E[i_0] - A[i_0+2] * A[i_0-1] + 3;
        for (int i_1 = 0; i_1 < PB_P; i_1++) {
            for (int i_2 = 0; i_2 < PB_P; i_2++) {
                for (int i_3 = 2; i_3 < PB_P; i_3++) {
                    D[i_3-2][i_0+1][i_2][i_0+1] = F[i_1][i_0+1][i_3][i_2] + F[i_1][i_3][i_2][1] + F[i_1][i_2][i_3][i_3] - F[i_3-1][i_2][i_1][i_0+1] + D[i_3-1][i_2][i_0][i_2] * 5;
                }
            }
        }
    }
    for (int i_4 = 2; i_4 < PB_L; i_4++) {
        for (int i_5 = 2; i_5 < min(i_4, PB_P-2); i_5++) {
            A[i_5+2] = E[i_5+2] - E[i_4] + G[i_5-1][i_4] + E[i_5] + G[i_4-2][i_5-2] * 3;
            for (int i_6 = 1; i_6 < -i_5+PB_P-2; i_6++) {
                for (int i_7 = 2; i_7 < PB_P-1; i_7++) {
                    D[i_6][i_5][i_4][i_5] = F[i_4][i_7-2][i_6+i_5+2][i_5] - F[i_7][i_6][i_5][0] + H[i_5][i_7+i_6-1][0] + F[i_5][i_4][i_7+1][i_6+2] - D[i_4][i_7][i_5-2][i_7] - 4;
                }
            }
        }
    }
...
init_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), INIT_SEED);
kernel_4414152223_04(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
polybench_prevent_dce(print_array(xa,POLYBENCH_ARRAY(A), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H)));
