...
void kernel_4226212221_01(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
...
#pragma scop
    ...
    for (int i_3 = 2; i_3 < PB_L+2; i_3++) {
        for (int i_4 = 0; i_4 < min(PB_M-1, i_3); i_4++) {
            B[i_4+1] = C[i_4+1][i_3-2] * B[i_4+3] + 4;
        }
    }
#pragma endscop
...
kernel_4226212221_01(xb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
