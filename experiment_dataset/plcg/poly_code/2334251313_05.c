...
void kernel_2334251313_05(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        B[i_0] = D[i_0] + E[i_0+1][i_0] * F[i_0-1] * F[i_0] - 2;
    }
    for (int i_1 = 1; i_1 < PB_L+1; i_1++) {
        B[i_1] = D[i_1] * F[i_1] - D[i_1] * D[i_1] - E[i_1][i_1-1] + D[0] - D[1] - B[i_1+1] + B[i_1-1] + B[i_1-1] + 2;
    }
    for (int i_2 = PB_M - 1; i_2 >= 1; i_2--) {
        for (int i_3 = i_2; i_3 < PB_L; i_3++) {
            B[i_3+1] = F[i_3+1] + F[i_3] * F[i_2] + E[i_2-1][i_3] - 2;
        }
    }
#pragma endscop
...
kernel_2334251313_05(xb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
