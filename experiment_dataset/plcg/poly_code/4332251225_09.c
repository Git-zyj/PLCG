...
void kernel_4332251225_09(int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,int vg,int wg,DATA_TYPE POLYBENCH_5D(G,xG,yG,zG,vG,wG,xg,yg,zg,vg,wg),int xh,int yh,DATA_TYPE POLYBENCH_2D(H,xH,yH,xh,yh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 1; i_1 < PB_M-1; i_1++) {
            for (int i_2 = 1; i_2 < min(-i_0+PB_M-1, -i_1+PB_P); i_2++) {
                for (int i_3 = 1; i_3 < min(min(-i_0+PB_Q+1, PB_L-1), -i_2+PB_M); i_3++) {
                    B[i_2][i_0+1] = D[i_3][i_3][i_0][i_2-1] * E[i_2+i_1][i_3][i_3+i_0-1][i_2] * F[i_1-1][i_3][i_2] - D[i_2+i_0+1][i_3][i_1+1][i_1] * G[i_3+1][i_2-1][i_2+i_1][i_3][i_0] + D[i_3+1][i_2+1][i_1][i_0] + E[i_3+i_2][i_2][i_0][i_0] * D[i_3+1][i_3-1][i_1][i_0-1] * 4;
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < PB_L; i_4++) {
        for (int i_5 = i_4; i_5 < PB_M-1; i_5++) {
            for (int i_6 = PB_L-2 - 1; i_6 >= 1; i_6--) {
                B[i_4][i_6+1] = H[i_4][i_6] * D[i_6][i_5][i_5+1][i_4] * B[i_5][i_6-1] + B[i_5][i_6+2] * 6;
            }
        }
    }
    for (int i_7 = 1; i_7 < PB_M-1; i_7++) {
        for (int i_8 = 0; i_8 < -i_7+PB_M-1; i_8++) {
            B[i_7-1][i_8+1] = H[i_8+i_7+1][i_8+i_7] - H[i_7][1] + I[i_8] - 1;
        }
    }
#pragma endscop
...
init_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), INIT_SEED);
kernel_4332251225_09(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I));
polybench_prevent_dce(print_array(xb,yb,POLYBENCH_ARRAY(B), xd,yd,zd,vd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,vg,wg,POLYBENCH_ARRAY(G), xh,yh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I)));
