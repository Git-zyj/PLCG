...
void kernel_2232111123_01(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_N-1; i_0++) {
        for (int i_1 = i_0; i_1 < PB_N-2; i_1++) {
            A[i_1] = C[i_1+1][i_0] * A[i_1+2] * 3;
        }
        for (int i_2 = 0; i_2 < -i_0+PB_N-1; i_2++) {
            B[i_2+i_0+1][i_0] = B[i_2+i_0][i_0-2] + 2;
        }
    }
#pragma endscop
...
kernel_2232111123_01(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
