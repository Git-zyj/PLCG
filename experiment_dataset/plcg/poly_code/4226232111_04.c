...
void kernel_4226232111_04(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,int vf,DATA_TYPE POLYBENCH_4D(F,xF,yF,zF,vF,xf,yf,zf,vf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        for (int i_1 = 2; i_1 < i_0; i_1++) {
            for (int i_2 = 1; i_2 < PB_M-1; i_2++) {
                A[i_0-1][i_1][i_2] = C[i_2][i_0][i_1-1] - D[i_2-1][i_0][i_1] + C[i_2][i_2][i_0+1] * 4;
                for (int i_3 = 1; i_3 < min(min(-i_1+PB_P+2, PB_M), -i_2+PB_N); i_3++) {
                    B[i_3][i_2][i_0] = E[i_0][i_2][0][i_3+i_1-2] + F[i_3+i_2][i_0+1][i_0][i_1] + E[i_1-2][i_0][i_2][i_0+2] - A[i_1-1][i_3-1][i_2+1] - B[i_3][i_2-1][i_1] + 2;
                }
            }
        }
    }
...
kernel_4226232111_04(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,zb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,vf,POLYBENCH_ARRAY(F));
