...
void kernel_4222232225_03(int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L-2; i_0++) {
        for (int i_1 = 2; i_1 < -i_0+PB_L-2; i_1++) {
            for (int i_2 = 0; i_2 < i_0; i_2++) {
                B[i_1] = C[i_0][i_2+i_1-2][i_0-2] - 6;
            }
            B[i_1] = D[i_1][i_0+2] * C[i_0][i_1][i_0] * D[i_1+i_0+2][i_1] + D[i_1+2][i_0-2] * B[i_0+1] * B[i_1-1] + 1;
        }
    }
#pragma endscop
...
kernel_4222232225_03(xb,POLYBENCH_ARRAY(B), xc,yc,zc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
