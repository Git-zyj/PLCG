...
void kernel_2426311311_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_L; i_0++) {
        for (int i_1 = max(1, i_0); i_1 < PB_L-1; i_1++) {
            A[i_1+1] = E[i_1+1][i_0] * D[i_0][i_0+1] + D[i_1-1][i_1] * 4;
        }
    }
    for (int i_2 = 2; i_2 < PB_L-1; i_2++) {
        B[i_2-1] = A[i_2+1] - B[i_2-2] - 1;
        C[i_2] = F[0] - B[i_2-1] + B[i_2] + 3;
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_M; i_4++) {
            D[i_4-1][i_4] = B[i_4-1] * D[i_4][i_4+1] - D[i_4-2][i_4-1] * D[i_4][i_4+1] + D[i_4][i_4+1] + 5;
        }
    }
#pragma endscop
...
kernel_2426311311_07(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
