...
void kernel_4414231215_03(int xb,int yb,int zb,int vb,DATA_TYPE POLYBENCH_4D(B,xB,yB,zB,vB,xb,yb,zb,vb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xe,int ye,int ze,int ve,DATA_TYPE POLYBENCH_4D(E,xE,yE,zE,vE,xe,ye,ze,ve),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,int zg,DATA_TYPE POLYBENCH_3D(G,xG,yG,zG,xg,yg,zg),int xh,DATA_TYPE POLYBENCH_1D(H,xH,xh),int xi,DATA_TYPE POLYBENCH_1D(I,xI,xi),int xj,int yj,DATA_TYPE POLYBENCH_2D(J,xJ,yJ,xj,yj),int xk,int yk,DATA_TYPE POLYBENCH_2D(K,xK,yK,xk,yk)){
        ...
        for (int i_1 = 1; i_1 < -i_0+PB_Q+2; i_1++) {
            for (int i_2 = 0; i_2 < min(-i_1+PB_Q+1, i_0); i_2++) {
                for (int i_3 = 0; i_3 < min(-i_1+PB_P+1, i_0); i_3++) {
                    B[i_2][i_2+i_1-1][i_1+i_0-2][1] = E[i_1][i_0-1][i_2+1][i_3+i_1-1] + F[i_3][i_1+1][i_1] - 4;
                }
            }
        }
        for (int i_4 = 1; i_4 < -i_0+PB_P-1; i_4++) {
            for (int i_5 = 0; i_5 < i_0; i_5++) {
                B[i_5][i_5+i_4-1][i_4+i_0-1][1] = F[i_5][i_0-1][i_0+1] * G[i_4][i_5][i_4+i_0+1] + G[i_0][i_5][i_4] - 2;
            }
        }
    }
    for (int i_6 = 1; i_6 < PB_L-1; i_6++) {
        C[i_6] = H[i_6] * I[i_6-1] + C[i_6+1] - 2;
        for (int i_7 = 1; i_7 < min(-i_6+PB_M, i_6); i_7++) {
            C[i_7] = J[i_7-1][i_6] * K[i_7+i_6][i_7+1] - C[i_7+1] * C[i_7] * 6;
        }
    }
#pragma endscop
...
init_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4414231215_03(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xb,yb,zb,vb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xe,ye,ze,ve,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,zg,POLYBENCH_ARRAY(G), xh,POLYBENCH_ARRAY(H), xi,POLYBENCH_ARRAY(I), xj,yj,POLYBENCH_ARRAY(J), xk,yk,POLYBENCH_ARRAY(K)));
