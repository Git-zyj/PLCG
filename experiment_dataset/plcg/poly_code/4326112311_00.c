...
void kernel_4326112311_00(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,int zd,DATA_TYPE POLYBENCH_3D(D,xD,yD,zD,xd,yd,zd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L; i_0++) {
        for (int i_1 = 1; i_1 < PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < min(min(-i_1+PB_M, -i_0+PB_M+1), PB_L-2); i_2++) {
                A[i_2+i_1][i_1] = D[i_1][i_0][i_2+2] - A[i_2+i_0-1][i_0] * A[i_2+i_1-1][i_2-1] - 6;
            }
        }
    }
    for (int i_3 = 3; i_3 < PB_M+2; i_3++) {
        for (int i_4 = 3; i_4 < PB_M+1; i_4++) {
            B[i_4-2][i_3-2] = B[i_4-1][i_3-2] + B[i_3-2][i_4-1] - B[i_4-3][i_3-2] - B[i_4-2][i_3-3] * 6;
        }
    }
...
kernel_4326112311_00(xa,ya,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,zd,POLYBENCH_ARRAY(D));
