...
void kernel_2222252111_09(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,DATA_TYPE POLYBENCH_1D(F,xF,xf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_L+2; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            A[i_1] = C[i_0-2] + D[i_1][i_1] + E[i_1+1][i_0] - D[i_1][i_0-2] - 5;
        }
    }
    for (int i_2 = 3; i_2 < PB_L; i_2++) {
        B[i_2-2] = C[i_2-1] + F[i_2+1] - C[i_2] - F[i_2] * C[i_2] * C[i_2] * B[i_2-3] + 2;
    }
#pragma endscop
...
kernel_2222252111_09(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,POLYBENCH_ARRAY(F));
