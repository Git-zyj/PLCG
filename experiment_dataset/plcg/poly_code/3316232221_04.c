...
void kernel_3316232221_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,int yc,int zc,DATA_TYPE POLYBENCH_3D(C,xC,yC,zC,xc,yc,zc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-2; i_0++) {
        A[i_0] = D[i_0+2] * E[i_0] - D[i_0] + A[i_0-1] + 6;
    }
    for (int i_1 = 0; i_1 < PB_P-2; i_1++) {
        for (int i_2 = max(2, i_1); i_2 < min(-i_1+PB_M, PB_M-1); i_2++) {
            A[i_2] = F[i_2][i_1+1] - F[i_1][i_2-2] - G[i_2][i_1+2] - 4;
            for (int i_3 = 0; i_3 < PB_L-2; i_3++) {
                C[i_2][i_3][i_1] = F[i_2+i_1][i_3] * C[i_2][i_3][i_1+1] - C[i_2+1][i_3+2][i_1] - 1;
            }
        }
    }
#pragma endscop
...
kernel_3316232221_04(xa,POLYBENCH_ARRAY(A), xc,yc,zc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G));
