...
void kernel_4414111221_01(int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc),int xb,int yb,int zb,DATA_TYPE POLYBENCH_3D(B,xB,yB,zB,xb,yb,zb),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_M-1; i_1++) {
            C[i_1][i_0] = E[i_1+1][i_0-1] + E[i_1+1][0] - C[i_1][i_0] - 3;
            for (int i_2 = i_1; i_2 < min(-i_0+PB_M, -i_1+PB_M); i_2++) {
                B[i_2+i_0][i_1][i_1] = F[i_1][i_2][i_1] + B[i_2+i_0-1][i_1][i_1] - B[i_2+i_0-2][i_1][i_1] + B[i_2+i_1][i_0-2][i_0-2] * 4;
            }
...
kernel_4414111221_01(xc,yc,POLYBENCH_ARRAY(C), xb,yb,zb,POLYBENCH_ARRAY(B), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F));
