...
void kernel_2432311313_04(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,DATA_TYPE POLYBENCH_1D(D,xD,xd),int xe,int ye,DATA_TYPE POLYBENCH_2D(E,xE,yE,xe,ye),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_L-1; i_0++) {
        for (int i_1 = 0; i_1 < PB_L-1; i_1++) {
            A[i_0+1] = E[i_0-1][i_1+1] - B[i_1] + B[i_1+1] + 3;
        }
        B[i_0] = A[i_0+1] - A[i_0] - 1;
    }
    for (int i_2 = 1; i_2 < PB_M; i_2++) {
        C[i_2] = F[i_2+1][0] + C[i_2-1] * C[i_2+1] + C[i_2-1] - 5;
    }
    for (int i_3 = 0; i_3 < PB_P; i_3++) {
        for (int i_4 = max(2, i_3); i_4 < PB_L; i_4++) {
            D[i_4-1] = B[i_4] - D[i_4-2] - 2;
        }
    }
#pragma endscop
...
kernel_2432311313_04(xa,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C), xd,POLYBENCH_ARRAY(D), xe,ye,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F));
