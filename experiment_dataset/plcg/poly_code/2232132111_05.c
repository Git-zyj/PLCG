...
void kernel_2232132111_05(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc)){
...
#pragma scop
    ...
    for (int i_2 = 1; i_2 < PB_M-2; i_2++) {
        B[i_2] = C[i_2-1] - C[i_2] * C[i_2+2] + C[i_2] - C[i_2+1] - C[i_2] + B[i_2+1] + 5;
    }
#pragma endscop
...
kernel_2232132111_05(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xc,POLYBENCH_ARRAY(C));
