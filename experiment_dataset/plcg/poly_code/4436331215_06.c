...
void kernel_4436331215_06(int xd,int yd,int zd,int vd,DATA_TYPE POLYBENCH_4D(D,xD,yD,zD,vD,xd,yd,zd,vd),int xc,int yc,int zc,int vc,DATA_TYPE POLYBENCH_4D(C,xC,yC,zC,vC,xc,yc,zc,vc),int xe,int ye,int ze,DATA_TYPE POLYBENCH_3D(E,xE,yE,zE,xe,ye,ze),int xf,int yf,int zf,DATA_TYPE POLYBENCH_3D(F,xF,yF,zF,xf,yf,zf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh),int xi,int yi,int zi,int vi,DATA_TYPE POLYBENCH_4D(I,xI,yI,zI,vI,xi,yi,zi,vi),int xj,int yj,int zj,int vj,DATA_TYPE POLYBENCH_4D(J,xJ,yJ,zJ,vJ,xj,yj,zj,vj),int xk,int yk,int zk,int vk,DATA_TYPE POLYBENCH_4D(K,xK,yK,zK,vK,xk,yk,zk,vk)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 0; i_1 < PB_L; i_1++) {
            for (int i_2 = i_0; i_2 < PB_L; i_2++) {
                D[i_2-1][i_1][i_0-2][i_0-1] = E[i_2][i_0][i_1] * F[i_2][i_0][i_1] + G[i_0][i_1] - 2;
            }
        }
        for (int i_3 = i_0; i_3 < PB_M-1; i_3++) {
            for (int i_4 = i_3; i_4 < PB_L-1; i_4++) {
                D[i_3-1][i_4+1][i_0-1][i_0] = H[i_3+1][i_4][i_0] * I[i_4][i_3][i_0-1][i_3] - 4;
            }
        }
    }
        ...
        for (int i_6 = 1; i_6 < i_5; i_6++) {
            for (int i_7 = max(1, i_5); i_7 < PB_L-1; i_7++) {
                for (int i_8 = 1; i_8 < PB_L; i_8++) {
                    C[i_5][i_7][i_8][i_6] = I[i_5][i_8][i_7+1][i_6] + J[i_7][i_6-1][i_5][i_8-1] * C[i_5][i_7-1][i_6][i_8] * C[i_5][i_7][i_8][i_6+1] + 6;
                }
            }
        }
    }
    for (int i_9 = 1; i_9 < PB_L; i_9++) {
        for (int j_0 = 2; j_0 < min(i_9, PB_M-1); j_0++) {
            for (int j_1 = 1; j_1 < PB_M-1; j_1++) {
                for (int j_2 = 0; j_2 < PB_P; j_2++) {
                    D[i_9-1][j_1][j_0-1][j_0] = F[j_1][j_0+1][j_2] * K[j_1+1][i_9][i_9-1][j_2] + I[j_1][i_9][i_9][j_0] + D[i_9-1][j_1-1][j_0-1][j_0] - D[i_9][j_1][j_0-2][j_0-1] + 6;
                }
            }
        }
    }
...
init_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K), INIT_SEED);
kernel_4436331215_06(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K));
polybench_prevent_dce(print_array(xd,yd,zd,vd,POLYBENCH_ARRAY(D), xc,yc,zc,vc,POLYBENCH_ARRAY(C), xe,ye,ze,POLYBENCH_ARRAY(E), xf,yf,zf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H), xi,yi,zi,vi,POLYBENCH_ARRAY(I), xj,yj,zj,vj,POLYBENCH_ARRAY(J), xk,yk,zk,vk,POLYBENCH_ARRAY(K)));
