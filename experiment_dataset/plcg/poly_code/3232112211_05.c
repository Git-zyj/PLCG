...
void kernel_3232112211_05(int xa,int ya,int za,DATA_TYPE POLYBENCH_3D(A,xA,yA,zA,xa,ya,za),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 0; i_0 < PB_Q; i_0++) {
        for (int i_1 = 0; i_1 < min(PB_Q, -i_0+PB_L); i_1++) {
            A[i_1+i_0][i_0][0] = B[i_0+1][i_1+1] - 1;
        }
    }
    for (int i_2 = 0; i_2 < PB_Q; i_2++) {
        for (int i_3 = 1; i_3 < min(PB_Q, -i_2+PB_L); i_3++) {
            B[i_2+1][i_3+2] = C[i_2][i_3-1] - A[i_3+i_2][i_3][0] + B[i_2][i_3+3] - B[i_2][i_3+2] - 6;
        }
    }
#pragma endscop
...
kernel_3232112211_05(xa,ya,za,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
