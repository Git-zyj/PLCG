...
void kernel_4234132325_07(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xc,DATA_TYPE POLYBENCH_1D(C,xC,xc),int xd,int yd,DATA_TYPE POLYBENCH_2D(D,xD,yD,xd,yd)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 4; i_0 < PB_M; i_0++) {
        A[i_0-2] = C[i_0] - C[i_0] + C[i_0] + A[i_0-1] + A[i_0-4] * A[i_0-3] - 5;
    }
    for (int i_1 = 0; i_1 < PB_Q; i_1++) {
        for (int i_2 = 2; i_2 < min(min(-i_1+PB_L, PB_M+2), i_1); i_2++) {
            A[i_2-2] = D[i_2+i_1][i_2] + D[i_2+i_1][1] + 2;
        }
    }
#pragma endscop
...
kernel_4234132325_07(xa,POLYBENCH_ARRAY(A), xc,POLYBENCH_ARRAY(C), xd,yd,POLYBENCH_ARRAY(D));
