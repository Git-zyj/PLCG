...
void kernel_2236212311_08(int xa,DATA_TYPE POLYBENCH_1D(A,xA,xa),int xb,int yb,DATA_TYPE POLYBENCH_2D(B,xB,yB,xb,yb),int xc,int yc,DATA_TYPE POLYBENCH_2D(C,xC,yC,xc,yc)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 1; i_0 < PB_M-1; i_0++) {
        A[i_0] = A[i_0+1] + A[i_0-1] + A[i_0-1] + 3;
        for (int i_1 = i_0; i_1 < min(PB_N, -i_0+PB_M-1); i_1++) {
            B[i_1+i_0][i_0] = C[i_1][i_0] - B[i_1+i_0][i_0-1] + B[i_1+i_0-1][i_0] * B[i_1+i_0+1][i_0+1] + 5;
        }
    }
#pragma endscop
...
kernel_2236212311_08(xa,POLYBENCH_ARRAY(A), xb,yb,POLYBENCH_ARRAY(B), xc,yc,POLYBENCH_ARRAY(C));
