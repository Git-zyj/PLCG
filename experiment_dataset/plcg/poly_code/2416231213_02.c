...
void kernel_2416231213_02(int xa,int ya,DATA_TYPE POLYBENCH_2D(A,xA,yA,xa,ya),int xb,DATA_TYPE POLYBENCH_1D(B,xB,xb),int xe,DATA_TYPE POLYBENCH_1D(E,xE,xe),int xf,int yf,DATA_TYPE POLYBENCH_2D(F,xF,yF,xf,yf),int xg,int yg,DATA_TYPE POLYBENCH_2D(G,xG,yG,xg,yg),int xh,int yh,int zh,DATA_TYPE POLYBENCH_3D(H,xH,yH,zH,xh,yh,zh)){
polybench_start_instruments;
#pragma scop
    for (int i_0 = 2; i_0 < PB_M; i_0++) {
        for (int i_1 = 1; i_1 < -i_0+PB_M; i_1++) {
            A[i_1][i_1+i_0-1] = E[i_1] * F[i_0][i_1] * F[i_1][i_0] + G[i_0][i_1] + B[i_1+1] * 4;
            B[i_1] = F[i_1+i_0][i_1] * A[i_1-1][i_1+i_0-2] + B[i_1+1] * B[i_1-1] * 6;
        }
    }
    for (int i_2 = 2; i_2 < PB_M; i_2++) {
        for (int i_3 = 0; i_3 < min(PB_M, -i_2+PB_L-1); i_3++) {
            A[i_2][i_3+i_2-1] = 6;
            A[i_2-1][i_3+i_2-2] = E[i_3] - H[i_3][i_3+i_2+1][i_2] - E[i_3] - F[i_2-1][i_3] + G[i_3][i_2-1] + F[i_2-1][i_2] * 2;
        }
    }
#pragma endscop
...
kernel_2416231213_02(xa,ya,POLYBENCH_ARRAY(A), xb,POLYBENCH_ARRAY(B), xe,POLYBENCH_ARRAY(E), xf,yf,POLYBENCH_ARRAY(F), xg,yg,POLYBENCH_ARRAY(G), xh,yh,zh,POLYBENCH_ARRAY(H));
