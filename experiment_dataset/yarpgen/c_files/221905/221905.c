/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221905
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221905 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221905
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 += var_11;
    var_20 = var_2;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 22;i_2 += 1)
            {
                {
                    var_21 = (max(var_21, (arr_3 [i_2])));
                    var_22 &= ((~(min((arr_6 [i_0] [i_1 - 1] [i_2 - 1] [i_2]), (((-7872240470203049498 + 9223372036854775807) >> 1))))));
                    var_23 = (((((((((arr_0 [i_1] [i_1]) > (arr_4 [i_0] [i_1] [1])))) ^ (var_0 && var_9)))) <= ((0 ? -3224 : -7872240470203049498))));
                    var_24 = (arr_4 [i_2 - 4] [i_1] [i_2 - 3]);
                }
            }
        }
    }
    #pragma endscop
}
