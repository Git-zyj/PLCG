/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246145
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246145 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246145
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 ^= var_3;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (arr_0 [i_0]);
        var_11 = ((3211450625 ^ (var_4 || var_7)));
        arr_3 [i_0] = (var_3 >= 1083516658);
        var_12 |= ((var_0 ? (arr_0 [i_0]) : (arr_1 [i_0])));
    }
    var_13 += (max(((min((0 | 54495), (max(3211450626, -2047375556))))), var_2));
    var_14 = ((((((var_0 + var_7) * var_1))) ? (max(11086, 9223372036854775786)) : var_7));
    var_15 = 11035;
    #pragma endscop
}
