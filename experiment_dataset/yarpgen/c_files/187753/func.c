/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187753
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187753 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187753
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 2; i_0 < 19; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 20; i_1 += 4) 
        {
            {
                var_18 = ((((unsigned long long int) ((6423988837948873427ULL) | (17047856446988732587ULL)))) % (((((/* implicit */_Bool) 3856229743628618710ULL)) ? (17047856446988732587ULL) : (6559620035417851162ULL))));
                var_19 = (-(min((arr_3 [i_1 - 2]), (arr_4 [i_0 + 1] [i_1 - 2]))));
            }
        } 
    } 
    var_20 = (~(var_0));
}
