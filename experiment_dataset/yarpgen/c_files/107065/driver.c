#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

unsigned long long int var_0 = 8224437724620225439ULL;
unsigned short var_1 = (unsigned short)10247;
unsigned char var_4 = (unsigned char)33;
_Bool var_6 = (_Bool)1;
short var_7 = (short)-18620;
unsigned char var_10 = (unsigned char)238;
int zero = 0;
unsigned short var_11 = (unsigned short)61906;
unsigned char var_12 = (unsigned char)234;
unsigned char var_13 = (unsigned char)30;
signed char var_14 = (signed char)-63;
void init() {
}

void checksum() {
    hash(&seed, var_11);
    hash(&seed, var_12);
    hash(&seed, var_13);
    hash(&seed, var_14);
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
