/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 173251
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=173251 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/173251
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = 194;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_12 = (max(var_12, ((((arr_3 [11] [1] [i_0]) < ((247 ? 2095335850 : var_0)))))));
                /* LoopNest 2 */
                for (int i_2 = 4; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        {
                            var_13 = (min(var_13, (~161)));
                            arr_11 [i_3] [i_2] [i_2 + 4] [i_1] [i_0] = (+-6925);
                            var_14 = 6924;
                            var_15 -= (min(((~((3127430743 ? -16 : 24064795)))), (arr_6 [11] [i_1] [18])));
                            var_16 |= (min(((((arr_0 [i_0]) || (arr_10 [i_0] [i_0] [i_2] [11])))), var_7));
                        }
                    }
                }
            }
        }
    }
    var_17 = var_7;
    #pragma endscop
}
