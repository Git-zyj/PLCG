/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27692
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27692 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27692
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (4092 <= -1);
    var_13 = ((1 % (~15360)));
    var_14 = (min(var_14, var_10));
    var_15 = (max(var_15, (((!((((((max(15360, -28)))) / (36011204832919552 * var_4)))))))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = ((~(arr_0 [i_0] [i_0])));
                var_16 = ((((((((0 + (arr_5 [i_0])))) ? ((min((arr_5 [i_1]), 37354))) : (((arr_2 [i_0] [i_1] [i_1]) ? var_2 : var_9))))) ? (-8758086291415701810 - 2147483647) : 8));
            }
        }
    }
    #pragma endscop
}
