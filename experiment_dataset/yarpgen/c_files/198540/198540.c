/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198540
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198540 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198540
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((((((var_5 ? var_7 : 65535)))) ? (~var_10) : var_9));

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        arr_4 [i_0] = 0;
        var_16 = ((-47449 ? (((41585 ? 53833 : 39078))) : (max(26449, (arr_3 [2] [i_0])))));

        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            var_17 = (39065 | -39067);
            arr_8 [i_0] = (39069 >= 26442);
            arr_9 [i_0] = ((((min((arr_0 [i_0]), (arr_0 [i_0])))) ? var_6 : (arr_0 [i_0])));
        }
    }
    var_18 = -16;
    #pragma endscop
}
