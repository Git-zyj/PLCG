/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82230
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82230 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82230
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, ((max(var_11, var_0)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 4; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 18;i_4 += 1)
                        {
                            {
                                var_20 = (min(var_20, ((((~(min(var_5, (arr_8 [12] [12] [i_2] [i_2] [i_1] [i_0])))))))));
                                arr_13 [i_4] [4] [4] [i_3 + 3] [17] &= ((+(((-77 + 2147483647) >> (((var_12 ^ var_13) - 136))))));
                            }
                        }
                    }
                }
                arr_14 [i_0] [1] = ((((((((arr_9 [i_1] [i_1] [i_0] [i_0] [i_0] [i_0]) ^ var_8))) ? 44404 : (arr_6 [i_0] [1]))) ^ var_5));
            }
        }
    }
    #pragma endscop
}
