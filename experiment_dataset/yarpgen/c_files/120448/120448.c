/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120448
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120448 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120448
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_1;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        arr_2 [i_0] = (((arr_0 [i_0]) % var_4));
        arr_3 [i_0] = var_0;
        arr_4 [0] [i_0] = var_10;
        var_17 = ((21564 / ((var_15 ? var_9 : 796476751))));
    }
    var_18 = var_2;
    /* LoopNest 3 */
    for (int i_1 = 0; i_1 < 19;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 19;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 19;i_3 += 1)
            {
                {
                    arr_12 [i_1] [i_1] [i_3] [i_1] = (((((((max((arr_9 [i_1] [i_1]), (arr_5 [i_1])))) ? (max(var_0, (arr_6 [i_3]))) : (((-796476752 ? -796476747 : 32640)))))) && ((max(951218868, -796476747)))));
                    arr_13 [i_1] [i_1] [i_3] = 32658;
                }
            }
        }
    }
    #pragma endscop
}
