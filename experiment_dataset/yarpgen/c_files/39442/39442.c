/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 39442
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=39442 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/39442
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max(var_20, (((12 ? -12 : 0)))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_21 = (min(var_21, var_4));
                var_22 = (min(var_22, (((((max(11, 1))) ? ((11 / ((((arr_3 [i_0] [i_1]) + 1))))) : (arr_4 [i_0 - 3] [i_0 + 1]))))));
                var_23 |= -39;
                var_24 = (max(var_24, (((((min((arr_1 [i_0 + 1]), (arr_1 [i_0 - 2])))) ? (arr_4 [i_0] [7]) : 15194721971081498556)))));
            }
        }
    }
    var_25 = (max((((var_3 ? (~var_3) : var_18))), var_16));
    var_26 ^= var_1;
    #pragma endscop
}
