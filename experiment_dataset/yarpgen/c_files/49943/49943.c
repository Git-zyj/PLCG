/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49943
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49943 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49943
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    var_12 = var_0;
                    arr_7 [i_0] [i_0] [i_2 - 1] [i_2] = ((var_5 ? (65529 & 29) : (((((29 ? var_5 : var_8))) ? (var_5 / 3501850565458797366) : var_4))));
                }
            }
        }
    }
    var_13 = ((var_3 ? var_0 : ((46403 ? ((var_9 ? 18446744073709551615 : 7)) : (((-30 ? var_5 : 58)))))));
    #pragma endscop
}
