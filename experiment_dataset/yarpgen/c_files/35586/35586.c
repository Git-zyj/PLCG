/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 35586
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=35586 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/35586
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_7;
    var_18 = -var_0;

    /* vectorizable */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        var_19 = (min(var_19, (arr_0 [i_0])));
        var_20 = (((~2930763543) - (((-(arr_1 [i_0]))))));

        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            var_21 = (min(var_21, ((((var_0 % -195956930200555040) << (((var_7 % 5944340848874561529) - 4464610658722267094)))))));
            var_22 = ((~((7619903096157603721 << (((arr_2 [i_0]) - 47000))))));
            arr_4 [i_0] [i_1] = ((~((var_5 ? (arr_3 [i_0] [i_0]) : var_7))));
        }
        var_23 = (((arr_1 [i_0 - 1]) | (arr_1 [i_0 - 2])));
    }
    var_24 = var_0;
    #pragma endscop
}
