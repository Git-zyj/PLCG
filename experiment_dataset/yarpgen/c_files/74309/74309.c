/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74309
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74309 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74309
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((max(var_3, var_14)) > (((((var_5 ? var_14 : var_2)) << (var_15 + 1996543564))))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    var_17 = var_9;

                    for (int i_3 = 4; i_3 < 9;i_3 += 1)
                    {
                        var_18 -= (arr_7 [i_1] [i_3] [i_1]);
                        var_19 = (max(var_19, ((min(((max(((max((arr_6 [i_1] [i_3] [i_2] [i_1]), (arr_0 [i_0])))), (max((arr_2 [i_0]), (arr_10 [i_3] [2] [i_2] [i_2])))))), (arr_8 [i_0] [i_0 + 1] [i_0]))))));
                    }
                }
            }
        }
    }
    var_20 = var_3;
    #pragma endscop
}
