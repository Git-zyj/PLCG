/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 225781
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=225781 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/225781
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] [i_0] = (!var_13);
                arr_5 [9] [i_0] = (var_13 + var_4);
                var_15 = (max(var_15, (arr_0 [i_1])));
                var_16 = (((arr_2 [13] [13] [i_1]) % ((max(var_12, var_9)))));
                var_17 = ((-18671 ? 17 : -17384));
            }
        }
    }
    var_18 = ((var_11 ? (max(((var_11 ? var_13 : var_13)), (var_13 - var_12))) : (min(-var_8, 1))));
    #pragma endscop
}
