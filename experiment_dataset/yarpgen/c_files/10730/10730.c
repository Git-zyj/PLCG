/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 10730
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=10730 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/10730
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (var_0 | ((var_13 ? 1 : -4498899042974233950)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_7 [i_2] [i_2] [i_0] = var_7;
                    var_18 = ((-4287981358922416827 ? 4287981358922416826 : (var_16 != -4287981358922416834)));
                    arr_8 [i_0] [i_2] [i_0] [i_0] = (((arr_6 [i_0] [i_0] [i_0] [i_0]) >> (arr_1 [i_0] [3])));
                }
            }
        }
    }
    #pragma endscop
}
