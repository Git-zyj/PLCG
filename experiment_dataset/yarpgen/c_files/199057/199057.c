/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199057
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199057 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199057
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_7;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_18 = (max((max((1788375677234223535 / var_8), -424490531546096533)), ((min((min(var_7, var_14)), (arr_0 [i_0] [i_1]))))));
                var_19 = ((var_3 ? (max((arr_6 [i_1]), var_8)) : ((min(((min((arr_0 [i_0] [i_0]), (arr_6 [i_0])))), (((arr_1 [5] [5]) ? (arr_0 [i_0] [0]) : -34)))))));
            }
        }
    }
    #pragma endscop
}
