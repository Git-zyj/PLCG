/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149873
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149873 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149873
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 += ((/* implicit */unsigned short) var_1);
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 13; i_0 += 3) 
    {
        for (unsigned short i_1 = 0; i_1 < 13; i_1 += 2) 
        {
            {
                var_21 = ((/* implicit */signed char) ((((/* implicit */int) ((short) (unsigned short)0))) / (((((/* implicit */int) (short)1)) | ((~(((/* implicit */int) (unsigned short)10812))))))));
                var_22 -= ((/* implicit */signed char) max(((_Bool)0), ((!(((/* implicit */_Bool) arr_5 [i_0]))))));
            }
        } 
    } 
    var_23 = ((/* implicit */signed char) ((unsigned short) var_16));
    var_24 = ((/* implicit */unsigned short) ((var_14) - (var_14)));
    var_25 = ((/* implicit */long long int) var_5);
}
