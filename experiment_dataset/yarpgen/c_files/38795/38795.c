/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38795
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38795 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38795
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [i_0] = arr_2 [6] [i_0];
                arr_7 [i_0] [i_1] [4] = min((((arr_1 [i_0]) ? (arr_1 [i_0]) : 98)), var_1);
            }
        }
    }
    var_10 = (min(var_0, 5415760960486203286));
    var_11 = (min(-1, (min((var_5 ^ 1090715534753792), 68))));
    #pragma endscop
}
