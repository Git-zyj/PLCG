/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 9608
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=9608 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/9608
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */signed char) var_2);
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 1; i_0 < 23; i_0 += 2) 
    {
        for (_Bool i_1 = 1; i_1 < 1; i_1 += 1) 
        {
            for (signed char i_2 = 0; i_2 < 25; i_2 += 2) 
            {
                {
                    var_18 = ((/* implicit */long long int) (short)-822);
                    var_19 -= ((/* implicit */long long int) (-(((((((/* implicit */unsigned long long int) var_8)) ^ (var_5))) % (((/* implicit */unsigned long long int) ((((/* implicit */_Bool) (signed char)-16)) ? (((/* implicit */int) var_15)) : (((/* implicit */int) var_10)))))))));
                }
            } 
        } 
    } 
}
