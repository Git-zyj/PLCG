/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 113645
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=113645 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/113645
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max(var_20, ((min(((var_1 ? ((max(var_5, var_1))) : var_5)), var_10)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 12;i_1 += 1)
        {
            {
                arr_6 [i_0] |= (max((((arr_5 [i_1 + 1] [i_1]) ? (arr_5 [i_1 + 1] [i_0]) : (arr_5 [i_1 - 1] [i_1]))), ((max((arr_5 [i_1 - 2] [i_0]), (arr_5 [i_1 + 2] [i_1]))))));
                var_21 ^= (min((0 == 6270812877442789119), ((-(arr_1 [i_1 - 2])))));
                var_22 ^= (min(-0, (arr_4 [i_1 - 2] [i_1 + 1])));
                var_23 = (arr_5 [i_0] [i_0]);
            }
        }
    }
    #pragma endscop
}
