/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90688
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90688 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90688
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 18;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] [i_0] = (arr_1 [i_0] [i_0]);
        arr_3 [i_0] = ((!(((-(arr_1 [i_0] [i_0]))))));
        arr_4 [i_0] = var_15;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 18;i_1 += 1) /* same iter space */
    {
        var_18 = (((-(arr_0 [i_1] [i_1]))) / (arr_0 [i_1] [i_1]));
        arr_9 [i_1] [i_1] = (arr_8 [i_1]);
    }
    for (int i_2 = 2; i_2 < 8;i_2 += 1)
    {
        arr_13 [i_2] = (~1002920628);
        /* LoopNest 3 */
        for (int i_3 = 1; i_3 < 8;i_3 += 1)
        {
            for (int i_4 = 3; i_4 < 9;i_4 += 1)
            {
                for (int i_5 = 0; i_5 < 10;i_5 += 1)
                {
                    {
                        arr_21 [i_5] [i_5] [i_5] [i_2] |= ((-(((arr_20 [i_3 + 2] [i_3]) | var_15))));

                        for (int i_6 = 0; i_6 < 10;i_6 += 1)
                        {
                            arr_26 [i_6] [i_4 + 1] [i_4] [i_5] [i_3] [i_2 + 1] [i_3] = (!var_5);
                            var_19 = (min(var_19, ((((arr_17 [i_3] [i_3 - 1] [i_3 + 2]) || ((((!(arr_11 [i_2]))))))))));
                            arr_27 [i_2] [i_3] [i_4] [i_4 + 1] [i_5] [i_3] [i_3] = (arr_5 [i_2]);
                        }
                    }
                }
            }
        }
        arr_28 [i_2 + 1] = (arr_25 [i_2] [i_2] [i_2 + 2] [i_2] [i_2]);
    }
    for (int i_7 = 0; i_7 < 15;i_7 += 1)
    {
        arr_31 [i_7] = (arr_30 [i_7]);
        arr_32 [i_7] = var_1;
        var_20 = ((var_15 % (min(var_0, var_3))));
    }
    var_21 ^= (var_14 && var_3);
    var_22 = (min(var_5, var_3));
    #pragma endscop
}
