/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105816
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105816 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105816
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_11 [i_0] [i_0] [i_0] = (((min(61446, (arr_4 [i_0] [i_2]))) + -810159261761704879));
                    arr_12 [i_0] = 76;
                    arr_13 [i_1] [i_1] [i_2] [i_0] = (min((min(((28 ? -787760639 : -810159261761704897)), ((((arr_9 [i_1] [i_2]) ? 124 : var_7))))), (arr_2 [i_0] [i_0])));
                    arr_14 [i_0] [i_1] [i_0] [i_0] = ((!((((127 / 124) - (arr_8 [i_0]))))));
                }
            }
        }
    }
    var_20 = 0;
    var_21 = var_8;
    var_22 = var_6;
    #pragma endscop
}
