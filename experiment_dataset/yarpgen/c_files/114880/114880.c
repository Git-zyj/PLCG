/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114880
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114880 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114880
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_16 = -var_1;
                var_17 += (max(((-(arr_3 [i_1] [i_1] [i_0 + 1]))), (var_1 && 100)));
                arr_4 [12] [i_1] [i_0 + 2] = ((-(((arr_0 [i_0]) ? (max(var_0, (arr_0 [i_1]))) : var_8))));
                /* LoopNest 3 */
                for (int i_2 = 1; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 21;i_4 += 1)
                        {
                            {
                                var_18 = (max(100, ((20 ? 127 : 11))));
                                var_19 = (arr_12 [i_0] [i_1] [i_1] [i_3] [i_3] [i_4]);
                            }
                        }
                    }
                }
            }
        }
    }
    var_20 = (max(((var_0 ? var_0 : var_12)), (0 || 2711)));
    #pragma endscop
}
