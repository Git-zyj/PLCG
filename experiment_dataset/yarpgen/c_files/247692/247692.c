/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247692
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247692 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247692
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (~var_4);
    var_12 = (min(var_12, (((((var_10 << (var_7 <= var_10))) != (!0))))));
    var_13 = var_4;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 13;i_2 += 1)
            {
                {
                    var_14 = (((min((arr_5 [i_2 + 2] [i_0 + 1]), (arr_5 [i_2 + 2] [i_0 - 1]))) >> (-20494 + 20512)));
                    arr_8 [8] [i_2] [i_1] [8] &= ((!(!161)));

                    /* vectorizable */
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        var_15 += 5031118689900151650;
                        var_16 -= (arr_6 [i_3] [0] [0] [i_0]);
                        var_17 = var_5;
                        var_18 = (var_4 ? -38262 : (arr_2 [i_2 - 2]));
                    }
                    var_19 = (min(var_19, var_4));
                }
            }
        }
    }
    #pragma endscop
}
