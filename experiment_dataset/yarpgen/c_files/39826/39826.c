/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 39826
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=39826 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/39826
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_17 = (-var_9 ^ var_5);
                                var_18 = ((!((max((arr_10 [i_0] [i_0 - 1] [i_0 + 1] [i_3] [i_0]), (arr_10 [9] [i_0 - 2] [i_0 + 1] [9] [i_0 - 2]))))));
                                arr_13 [i_0] [i_0] [i_0] [i_3] [9] = 2758407488;
                            }
                        }
                    }
                }
                var_19 = ((min(((max(var_13, (arr_11 [12] [3] [i_1] [12] [i_1])))), (max((arr_8 [i_0] [i_1] [i_0]), var_14)))));
            }
        }
    }
    var_20 = 1536559819;
    #pragma endscop
}
