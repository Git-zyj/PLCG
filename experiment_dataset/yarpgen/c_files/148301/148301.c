/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148301
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148301 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148301
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_19 = (max(var_19, ((max(((4294967295 ? (arr_0 [i_0]) : 4630621235637122456)), (((!(arr_0 [0])))))))));

        /* vectorizable */
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            var_20 = (min(var_20, var_13));
            /* LoopNest 2 */
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 14;i_3 += 1)
                {
                    {
                        var_21 = ((var_9 ? var_3 : (arr_8 [i_3] [i_1] [i_1] [i_1] [i_0] [i_0])));
                        arr_9 [13] [i_2] [i_2] [i_1] [i_1] [i_0] = ((-90 | (arr_7 [i_3] [i_2] [i_1] [i_0])));
                        var_22 = (max(var_22, (((-(!var_14))))));
                    }
                }
            }
        }
    }
    var_23 = var_5;
    #pragma endscop
}
