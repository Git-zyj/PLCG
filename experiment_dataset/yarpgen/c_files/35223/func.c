/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 35223
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=35223 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/35223
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 18; i_0 += 3) 
    {
        for (unsigned short i_1 = 2; i_1 < 18; i_1 += 4) 
        {
            for (unsigned int i_2 = 0; i_2 < 21; i_2 += 1) 
            {
                {
                    arr_11 [i_0] = ((/* implicit */unsigned short) ((((/* implicit */int) arr_7 [20] [i_1 + 3] [i_0])) >= (((/* implicit */int) var_0))));
                    arr_12 [i_0] [i_1] [i_1] = ((/* implicit */int) ((long long int) arr_3 [i_1 - 2]));
                }
            } 
        } 
    } 
    var_12 = ((/* implicit */unsigned int) max(((+(((/* implicit */int) var_4)))), (((((/* implicit */int) ((((/* implicit */_Bool) 1189369196)) && (((/* implicit */_Bool) 3752053885U))))) ^ (((/* implicit */int) var_4))))));
}
