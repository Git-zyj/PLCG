/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29617
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29617 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29617
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_7 [i_0] = ((((((var_1 ? var_12 : var_6)) & ((-(arr_3 [i_0] [i_1] [i_1]))))) >= (max(((var_0 ? -16396 : -28185)), ((-(arr_4 [7] [i_1] [i_0])))))));
                var_16 = (max((min((max((arr_6 [i_1]), var_10)), (arr_6 [i_0 + 3]))), ((((var_1 ? 16389 : (arr_4 [i_0 + 3] [i_0] [10])))))));
                var_17 ^= (min(58820, -16378));
            }
        }
    }
    var_18 = (min(var_18, var_5));
    #pragma endscop
}
