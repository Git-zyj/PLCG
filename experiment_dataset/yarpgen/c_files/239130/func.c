/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239130
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239130 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239130
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */long long int) (-((-(((/* implicit */int) min(((short)32757), (((/* implicit */short) var_2)))))))));
    var_14 = ((/* implicit */unsigned short) (-((-(((/* implicit */int) var_9))))));
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 24; i_0 += 1) 
    {
        arr_2 [i_0] = ((/* implicit */long long int) max(((!(arr_0 [i_0] [i_0]))), (((((/* implicit */int) var_0)) < (((/* implicit */int) (!(((/* implicit */_Bool) arr_1 [i_0] [i_0])))))))));
        var_15 = ((/* implicit */_Bool) var_3);
    }
    var_16 = ((/* implicit */long long int) max((var_16), (((/* implicit */long long int) (short)32757))));
}
