/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 115196
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=115196 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/115196
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_14 -= (max((var_12 && var_1), (arr_1 [i_0 + 2] [4])));
                arr_5 [i_0] [i_0] = ((-((var_0 ? (arr_4 [i_0 - 1] [i_0] [i_0]) : ((18446744073709551615 ? 18446744073709551615 : 64))))));
                var_15 = (max(var_15, var_7));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 12;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 12;i_3 += 1)
        {
            {
                var_16 = (min(var_16, (((var_8 >= (-1 ^ var_13))))));
                var_17 = (max((1 && 65526), (!994)));
            }
        }
    }
    #pragma endscop
}
