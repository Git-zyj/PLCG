/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116855
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116855 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116855
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        {
                            var_11 = (((min(-1, (arr_7 [1] [19] [i_2] [i_3])))));
                            var_12 &= (((var_6 * 1) / 4194304));
                            var_13 = ((+((((arr_5 [i_0 - 1]) || var_4)))));
                        }
                    }
                }
                var_14 -= var_6;
            }
        }
    }
    /* LoopNest 2 */
    for (int i_4 = 2; i_4 < 11;i_4 += 1)
    {
        for (int i_5 = 0; i_5 < 15;i_5 += 1)
        {
            {
                var_15 = (!-var_3);
                var_16 = 1;
                /* LoopNest 2 */
                for (int i_6 = 0; i_6 < 15;i_6 += 1)
                {
                    for (int i_7 = 4; i_7 < 11;i_7 += 1)
                    {
                        {
                            var_17 = ((((min(((var_9 ? var_6 : 7)), (arr_12 [i_6])))) ? ((min((arr_11 [1] [1] [i_6] [i_6]), (!-6272949780431872559)))) : (arr_13 [i_4 + 3])));
                            arr_25 [11] [11] [i_6] [i_6] [1] = (arr_16 [i_4] [i_4]);
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
