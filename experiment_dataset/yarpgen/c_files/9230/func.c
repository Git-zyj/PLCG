/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 9230
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=9230 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/9230
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */signed char) 9223372036854775807LL);
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 0; i_0 < 20; i_0 += 1) 
    {
        /* LoopNest 2 */
        for (unsigned long long int i_1 = 3; i_1 < 19; i_1 += 1) 
        {
            for (signed char i_2 = 2; i_2 < 19; i_2 += 1) 
            {
                {
                    var_11 += ((/* implicit */int) var_4);
                    arr_6 [i_0] [i_0] [i_0] [i_1] = min(((unsigned char)62), (((/* implicit */unsigned char) (signed char)-9)));
                    var_12 = ((/* implicit */unsigned long long int) max((((/* implicit */unsigned int) ((((/* implicit */int) arr_2 [i_2 + 1] [i_1 - 2])) | (((/* implicit */int) (signed char)105))))), (((((/* implicit */unsigned int) var_9)) | (((((/* implicit */_Bool) var_0)) ? (var_0) : (((/* implicit */unsigned int) ((/* implicit */int) (signed char)-9)))))))));
                }
            } 
        } 
        var_13 = ((/* implicit */signed char) arr_5 [i_0] [i_0]);
    }
}
