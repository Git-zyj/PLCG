/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123723
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123723 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123723
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_13 = (max(var_13, (((!((((~0) ? var_1 : ((var_0 & (arr_0 [2])))))))))));
                var_14 = 4294967295;
                var_15 ^= (((arr_4 [10] [i_1]) ? (((var_8 ? (arr_3 [i_1] [6] [i_0]) : (arr_1 [i_0 + 1])))) : (((arr_1 [i_0 + 4]) ? (arr_4 [8] [8]) : (arr_4 [0] [i_1])))));
                var_16 *= (((((39046 ? var_11 : var_9)))));
            }
        }
    }
    #pragma endscop
}
