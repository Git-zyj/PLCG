/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 35972
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=35972 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/35972
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            {
                                var_16 = (max(var_16, 16093583900985635047));
                                var_17 |= (((max(0, 5))));
                            }
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_5 = 0; i_5 < 12;i_5 += 1)
                {
                    for (int i_6 = 0; i_6 < 12;i_6 += 1)
                    {
                        {
                            arr_19 [i_0] [2] = ((var_14 ? var_9 : var_7));
                            var_18 = -1;
                        }
                    }
                }
                arr_20 [i_0] [i_0] [10] = (((((-(arr_11 [i_0 - 1])))) ? 1 : ((max((arr_14 [i_0 - 1] [i_0 - 1] [1] [i_0]), var_11)))));
            }
        }
    }
    var_19 = (((max(var_0, (max(16093583900985635047, 1))))) || var_3);
    #pragma endscop
}
