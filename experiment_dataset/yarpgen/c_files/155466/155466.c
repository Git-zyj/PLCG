/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155466
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155466 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155466
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_11 = -1;
                var_12 += (!-8245);

                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    var_13 = ((((max((arr_3 [i_0 + 1] [i_2]), (arr_3 [i_1] [i_2])))) && (!var_8)));
                    var_14 = (((((var_7 ? ((8246 ? 0 : 1)) : var_3))) ? (((!var_10) ? ((var_10 ? var_5 : var_1)) : var_5)) : (((arr_1 [i_1]) ? (arr_1 [i_0 - 1]) : (arr_1 [i_2])))));
                }
            }
        }
    }
    var_15 = (((((((var_2 ? var_9 : var_4))) ? var_8 : ((max(var_9, 32749)))))) ? ((1 ? var_0 : var_6)) : ((((min(0, var_4))) ? (1 && 0) : (~var_0))));
    #pragma endscop
}
