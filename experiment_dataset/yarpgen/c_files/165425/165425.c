/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165425
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165425 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165425
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_12 &= (arr_1 [8]);
        var_13 += ((!79) ? (186 - var_10) : (((arr_0 [i_0]) ? var_1 : var_0)));
    }
    for (int i_1 = 0; i_1 < 12;i_1 += 1)
    {
        var_14 = (max(var_14, ((max((arr_4 [i_1]), ((((arr_2 [i_1] [i_1]) >> (((((arr_3 [i_1]) ? var_10 : var_11)) - 15823))))))))));
        arr_6 [i_1] = (arr_3 [i_1]);
        var_15 &= (((((min((arr_3 [i_1]), -14151)))) ? ((min(var_0, var_5))) : (!78)));
    }
    var_16 = ((var_7 ? (((var_3 & var_9) | 190)) : var_2));
    #pragma endscop
}
