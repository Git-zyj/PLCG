/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4226
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4226 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4226
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_16, var_3));
    var_17 = (max(var_17, var_3));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 6;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 8;i_2 += 1)
            {
                {
                    var_18 |= (max(var_14, (arr_7 [9] [i_1] [9] [5])));
                    var_19 = min(((((min(124, 0)) > (((arr_6 [i_1]) ? var_12 : 18356))))), (((arr_7 [i_2] [i_2] [i_2] [i_2]) ? (arr_7 [i_1 - 2] [6] [i_2] [i_1 - 2]) : 31134)));
                    var_20 = (min(var_20, (((1 ? 1 : 1)))));
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 1; i_3 < 1;i_3 += 1)
    {
        for (int i_4 = 2; i_4 < 13;i_4 += 1)
        {
            {
                var_21 += (!((min(var_10, (arr_10 [i_3] [i_3])))));
                arr_13 [i_4] [i_3] = min((arr_9 [i_3 - 1] [i_3 - 1]), (((!(arr_10 [i_3] [12])))));
            }
        }
    }
    #pragma endscop
}
