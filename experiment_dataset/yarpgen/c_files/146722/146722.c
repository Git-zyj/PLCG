/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146722
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146722 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146722
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((((min(var_4, ((var_5 ? -88 : var_3))))) == var_1));
    var_20 = (max((((!(22155072988915337 != 0)))), (min((!-68), var_3))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 21;i_1 += 1)
        {
            {
                var_21 = (arr_4 [i_0] [i_0]);
                var_22 = -68;
                var_23 = (arr_2 [i_0 + 3]);
                var_24 = ((-((((max((arr_3 [i_0]), 2147483645))) / (arr_0 [i_0 + 1])))));
                arr_6 [i_0] [i_0] [3] = ((-(arr_0 [i_0 + 1])));
            }
        }
    }
    #pragma endscop
}
