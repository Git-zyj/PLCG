/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227834
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227834 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227834
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_18;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    arr_8 [i_2] [i_1] [i_0] = ((21 ? (arr_6 [i_0] [i_1] [i_2]) : (((!(((211 ? 23 : 234))))))));
                    var_20 = ((8 ? (((2786 <= var_17) ? ((min((arr_7 [i_0] [i_0] [i_2]), var_3))) : (arr_0 [i_0]))) : ((min(var_1, var_16)))));
                    arr_9 [i_0] [i_1] [i_0] [i_1] = 21;
                }
            }
        }
    }
    var_21 = 1445251593007996286;
    var_22 -= -2780;
    #pragma endscop
}
