/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105886
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105886 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105886
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 ^= ((/* implicit */signed char) var_11);
    /* LoopNest 2 */
    for (signed char i_0 = 0; i_0 < 16; i_0 += 1) 
    {
        for (unsigned short i_1 = 3; i_1 < 15; i_1 += 4) 
        {
            {
                arr_4 [i_0] = ((/* implicit */unsigned short) ((unsigned long long int) 536870911LL));
                arr_5 [i_1] = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) (((~(((/* implicit */int) arr_1 [i_0])))) & (((/* implicit */int) ((short) arr_1 [i_0])))))) ? ((((~(((/* implicit */int) arr_1 [i_1 + 1])))) ^ (((/* implicit */int) var_11)))) : ((~(((/* implicit */int) (unsigned short)25603))))));
            }
        } 
    } 
}
