/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83599
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83599 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83599
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 ^= (min(((-(var_17 - var_13))), var_12));
    var_21 = ((-(min(var_17, ((var_9 ? 4156199500 : var_2))))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_22 = (arr_4 [i_0] [i_1]);
                arr_6 [i_0 - 2] [i_0] [i_0 - 3] = ((((min((arr_5 [i_0 - 3] [i_0] [i_1]), 101))) + (((arr_5 [i_0 + 1] [i_1] [i_1]) ? (arr_5 [i_0 - 3] [i_1] [i_1]) : (arr_5 [i_0 - 1] [i_0 - 1] [15])))));
            }
        }
    }
    var_23 = var_4;
    #pragma endscop
}
