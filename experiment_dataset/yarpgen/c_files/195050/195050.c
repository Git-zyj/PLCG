/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195050
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195050 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195050
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = 25199;
    var_14 = ((+(((1 * var_9) * (!15143134932729256343)))));

    for (int i_0 = 2; i_0 < 17;i_0 += 1)
    {
        var_15 = arr_2 [i_0];
        var_16 *= (arr_0 [i_0]);
    }
    for (int i_1 = 0; i_1 < 11;i_1 += 1)
    {
        var_17 = ((!((39502 && (arr_2 [i_1])))));
        /* LoopNest 2 */
        for (int i_2 = 2; i_2 < 10;i_2 += 1)
        {
            for (int i_3 = 3; i_3 < 8;i_3 += 1)
            {
                {
                    arr_11 [8] [8] &= ((var_11 ? 26035 : 0));
                    arr_12 [i_1] [i_1] = (max(var_0, (((arr_0 [i_3]) >> (48544 - 48541)))));
                    var_18 = ((max(var_3, ((1 ? 0 : 15143134932729256343)))));
                }
            }
        }
        arr_13 [i_1] = (arr_6 [i_1] [i_1]);
    }
    #pragma endscop
}
