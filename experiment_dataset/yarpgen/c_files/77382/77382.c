/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77382
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77382 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77382
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    arr_7 [i_0] [9] [i_2] = (!((min((((!(arr_3 [i_0] [i_0])))), (arr_2 [i_1])))));
                    arr_8 [5] [i_1] [i_1] [5] = ((!((((arr_1 [i_1]) ? var_5 : (arr_4 [i_1]))))));
                }
            }
        }
    }
    var_14 = ((-23857 ? -23838 : ((min(0, 15469)))));
    var_15 = ((((max((~234), 34382))) ? var_7 : 3885971374));
    var_16 = 673821217;
    #pragma endscop
}
