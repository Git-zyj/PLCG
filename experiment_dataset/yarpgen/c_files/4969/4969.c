/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4969
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4969 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4969
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_13 *= ((arr_2 [i_0] [i_0]) < (1 + 1));
        var_14 = (min(var_14, ((max((min(1, (((arr_3 [i_0]) ? var_10 : (arr_0 [1]))))), (((max(var_11, var_6)))))))));
        var_15 = max(((0 == (arr_1 [i_0]))), (arr_2 [i_0] [i_0]));
    }
    var_16 = (((((~(~var_0)))) ? ((max(var_6, (1 >= var_5)))) : var_9));
    var_17 = (((((!(max(1, var_4))))) + var_8));
    #pragma endscop
}
