/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233613
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233613 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233613
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        var_10 = (((((27123 < (arr_1 [i_0] [i_0])))) + -7117));
        var_11 -= ((arr_0 [i_0] [i_0 - 1]) ? -7117 : (arr_0 [i_0] [i_0 - 1]));
    }
    var_12 = (max(var_12, ((min(-5496374446597477822, var_0)))));

    /* vectorizable */
    for (int i_1 = 0; i_1 < 25;i_1 += 1)
    {
        arr_6 [i_1] = ((arr_5 [i_1]) >> 28);
        arr_7 [i_1] [i_1] = ((-(arr_3 [17] [6])));
        var_13 ^= ((~((4177920 ? (arr_3 [i_1] [i_1]) : (arr_4 [16])))));
    }
    #pragma endscop
}
