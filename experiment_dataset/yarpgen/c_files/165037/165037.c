/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165037
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165037 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165037
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(var_16, var_11));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_1] [i_2] [i_2] = var_14;

                    /* vectorizable */
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        arr_9 [i_0] [i_1] [i_0] [i_0] = (-85 == 107);
                        var_17 = ((var_12 >= (arr_7 [i_0] [i_0] [i_2])));
                        var_18 = (((arr_0 [i_0]) ? var_5 : (arr_0 [i_0])));
                        var_19 = (min(var_19, (arr_5 [i_0] [12] [i_2])));
                        arr_10 [i_0] [15] [i_0] [i_0] [i_0] [i_0] = (-6 || 90);
                    }
                    arr_11 [i_0] [i_1] [i_0] = (((max(((max(var_1, -4))), ((-13 ? (arr_4 [i_0] [i_0] [i_0] [1]) : (arr_8 [i_2] [i_0] [i_0]))))) >= (((arr_7 [i_0] [i_1] [i_0]) / (arr_7 [i_0] [i_2] [i_2])))));
                }
            }
        }
    }
    #pragma endscop
}
