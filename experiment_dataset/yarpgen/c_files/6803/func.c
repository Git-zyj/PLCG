/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6803
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6803 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6803
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (int i_0 = 1; i_0 < 14; i_0 += 1) 
    {
        arr_4 [i_0] = ((/* implicit */unsigned char) (((~(((/* implicit */int) (_Bool)1)))) | (((/* implicit */int) ((signed char) var_11)))));
        var_18 = ((/* implicit */_Bool) arr_1 [i_0]);
    }
    var_19 = ((/* implicit */unsigned char) (-(0LL)));
    var_20 = ((/* implicit */unsigned int) (~(((((/* implicit */_Bool) (unsigned short)49527)) ? (((/* implicit */int) (_Bool)1)) : (1499903927)))));
}
