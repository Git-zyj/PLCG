/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162075
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162075 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162075
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 |= (min((((var_3 ? var_10 : var_4))), ((var_4 ? ((8589934591 >> (var_4 - 199))) : var_8))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [i_0] [17] = min(var_5, (arr_3 [i_1]));
                arr_5 [i_0] &= var_8;
                var_12 = (max(((((max(8589934591, 2499340220)) < (arr_0 [i_0] [i_0])))), 8589934591));
            }
        }
    }
    var_13 += ((((((var_2 ? var_0 : var_5)) > (0 || var_2))) ? var_0 : ((max(183, (max(var_8, var_6)))))));
    #pragma endscop
}
