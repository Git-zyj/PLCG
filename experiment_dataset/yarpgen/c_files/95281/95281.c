/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95281
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95281 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95281
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (var_5 >> ((var_3 & (var_4 & 2114519033))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    var_14 = var_3;
                    var_15 -= (((((((-2114519033 + 2147483647) >> var_4)) >> (-2114519034 + 2114519052))) >> ((((2114519033 ? var_1 : 4455745672347597652)) - 31662))));
                    var_16 = ((var_6 ? (((((1 ? 10 : var_1))) ? ((var_3 ? var_12 : 883022041672396862)) : 218)) : var_1));
                }
            }
        }
    }
    #pragma endscop
}
