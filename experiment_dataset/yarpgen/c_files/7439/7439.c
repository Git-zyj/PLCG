/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7439
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7439 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7439
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_15 = 29052;
                var_16 |= (max((~var_8), ((-(arr_3 [i_0] [i_0] [i_1])))));
            }
        }
    }

    for (int i_2 = 0; i_2 < 21;i_2 += 1) /* same iter space */
    {
        var_17 = (max(var_17, (((var_2 / ((max((arr_4 [i_2]), var_10))))))));
        var_18 = (min((~var_3), 15143));
    }
    /* vectorizable */
    for (int i_3 = 0; i_3 < 21;i_3 += 1) /* same iter space */
    {
        var_19 = (((arr_4 [i_3]) & -15143));
        var_20 = (max(var_20, (var_7 | var_5)));
    }
    for (int i_4 = 0; i_4 < 21;i_4 += 1) /* same iter space */
    {
        var_21 ^= (min(((2305561534236983296 ? (arr_5 [i_4] [i_4]) : (arr_6 [i_4]))), ((~(((arr_8 [i_4] [2]) ? 1572717744 : (arr_6 [i_4])))))));
        var_22 = ((!((!(arr_9 [i_4])))));
        arr_11 [i_4] = (arr_8 [i_4] [10]);
        var_23 += (arr_7 [20]);
    }
    #pragma endscop
}
