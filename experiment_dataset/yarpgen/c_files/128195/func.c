/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 128195
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=128195 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/128195
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned int) max((((_Bool) ((_Bool) var_2))), (((_Bool) ((int) var_17)))));
    var_21 = ((/* implicit */long long int) ((_Bool) var_6));
    /* LoopNest 3 */
    for (unsigned int i_0 = 4; i_0 < 22; i_0 += 2) 
    {
        for (long long int i_1 = 0; i_1 < 23; i_1 += 2) 
        {
            for (long long int i_2 = 0; i_2 < 23; i_2 += 4) 
            {
                {
                    var_22 -= ((/* implicit */long long int) ((_Bool) min((((/* implicit */int) var_8)), (var_19))));
                    arr_9 [i_0] [i_0] [i_2] |= ((/* implicit */unsigned int) ((int) ((_Bool) var_17)));
                }
            } 
        } 
    } 
}
