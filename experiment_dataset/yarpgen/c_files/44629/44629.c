/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44629
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44629 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44629
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        arr_2 [i_0 + 2] = ((-((var_14 ? var_14 : 4199927088))));
        arr_3 [i_0] [i_0] = (arr_1 [i_0 + 1]);
        arr_4 [i_0] [i_0] = (var_13 ? (arr_1 [i_0 + 2]) : (((((arr_1 [i_0]) && var_8)))));
        arr_5 [i_0 - 1] = (var_13 >> (arr_0 [i_0 + 2]));
    }
    var_15 = (((var_8 / (~var_11))));
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        for (int i_2 = 1; i_2 < 12;i_2 += 1)
        {
            {
                var_16 = (arr_11 [i_2 + 1] [i_2 - 1] [i_2 + 1]);
                arr_12 [i_2] [i_2] [i_2] = var_10;
            }
        }
    }
    #pragma endscop
}
