/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72486
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72486 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72486
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 12;i_0 += 1)
    {
        var_13 = (max(var_13, (((411525480 > (174 && 14283))))));
        var_14 = ((((((arr_0 [i_0]) > 25))) / ((min((arr_1 [i_0 + 1] [i_0 - 3]), var_1)))));
        arr_2 [i_0] = var_12;
        var_15 += arr_0 [i_0];
        arr_3 [i_0] = ((((((~0) & ((max(-14284, 161)))))) ? (47 ^ 58996) : (((arr_1 [i_0 + 2] [i_0 + 2]) ? (arr_1 [i_0 + 3] [i_0 + 2]) : -17771))));
    }
    var_16 = var_7;
    var_17 = var_1;
    var_18 = (max(var_18, ((min(31, (-32767 - 1))))));
    var_19 = var_10;
    #pragma endscop
}
