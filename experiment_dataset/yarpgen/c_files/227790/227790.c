/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227790
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227790 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227790
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 0;i_2 += 1)
            {
                {
                    arr_6 [i_1] [i_1] [i_2] = ((var_13 * (!2004448127)));
                    var_16 = (max(var_16, (((((var_6 / (arr_5 [i_2] [i_2 + 1]))) >= (((arr_5 [i_2] [i_2 + 1]) * var_5)))))));
                }
            }
        }
    }
    var_17 = ((((((var_0 == (~var_1))))) < (55 * var_7)));
    var_18 &= ((1251409766 && (255 > 1)));
    var_19 = ((0 ? var_12 : 39086));
    var_20 = var_12;
    #pragma endscop
}
