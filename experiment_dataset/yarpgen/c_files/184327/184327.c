/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184327
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184327 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184327
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (min((-2007070475627584889 + 284436475), var_8));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_20 = var_2;
                    arr_11 [i_0] [i_1] [i_0] = ((((min((arr_8 [i_0] [i_1] [i_2] [i_1 - 2]), (arr_8 [i_0 + 2] [i_1] [1] [i_2])))) ? 0 : (!var_12)));
                    var_21 &= 1;
                }
            }
        }
    }
    var_22 = var_14;

    for (int i_3 = 0; i_3 < 15;i_3 += 1)
    {
        arr_16 [i_3] = (arr_13 [i_3]);
        arr_17 [i_3] = (~var_17);
        var_23 = (min((arr_15 [i_3]), (((((min(var_12, 1))) + 1)))));
        var_24 = (((max(1, (arr_14 [i_3]))) ^ (~var_10)));
        var_25 *= ((~(((!0) / (var_16 || 4294967295)))));
    }
    var_26 -= var_3;
    #pragma endscop
}
