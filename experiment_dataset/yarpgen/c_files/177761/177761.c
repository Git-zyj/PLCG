/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177761
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177761 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177761
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (((((var_12 ? (max(var_7, var_5)) : ((var_0 ? 765024659 : var_7))))) ? ((var_13 ? var_6 : var_11)) : ((~(var_7 <= var_10)))));
    var_16 = max(1, 1);
    var_17 = ((((((~var_1) & 1))) ? var_12 : (((~((max(var_9, -26402))))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        var_18 = (((arr_0 [i_0]) ^ (arr_1 [i_0] [i_0])));
        /* LoopNest 2 */
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 16;i_2 += 1)
            {
                {
                    var_19 = (arr_4 [i_2 + 2] [i_2 + 1]);
                    var_20 = ((var_0 + ((var_1 ? (arr_3 [i_0]) : (arr_6 [i_1] [10] [9] [8])))));
                }
            }
        }
    }
    var_21 = var_7;
    #pragma endscop
}
