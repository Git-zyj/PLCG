/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 31264
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=31264 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/31264
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned int) max((var_10), (((/* implicit */unsigned char) var_5))));
    var_15 = ((/* implicit */unsigned int) ((long long int) var_1));
    var_16 = ((/* implicit */_Bool) ((min((min((((/* implicit */unsigned int) var_8)), (var_7))), (min((((/* implicit */unsigned int) (signed char)-90)), (var_6))))) * (((/* implicit */unsigned int) ((/* implicit */int) var_10)))));
    /* LoopNest 2 */
    for (unsigned int i_0 = 1; i_0 < 10; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 11; i_1 += 3) 
        {
            {
                var_17 = ((/* implicit */unsigned int) var_13);
                var_18 = ((/* implicit */unsigned long long int) arr_4 [i_0] [i_0]);
                arr_6 [i_0 - 1] [i_0] = ((/* implicit */long long int) var_1);
            }
        } 
    } 
}
