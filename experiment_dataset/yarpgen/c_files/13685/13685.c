/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 13685
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=13685 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/13685
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 13;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] [i_1] = (((arr_1 [i_0 + 1]) < (min((arr_1 [9]), (arr_2 [i_0])))));
                var_12 = (((min(var_1, (((arr_5 [i_0] [i_1] [i_1]) ? (arr_4 [i_0]) : (arr_5 [i_0] [i_1] [i_1]))))) > (arr_2 [i_0 - 2])));
                arr_7 [i_1] [i_0] [i_1 - 3] = (min(4338, 43));
            }
        }
    }
    var_13 = var_0;
    var_14 = (min(var_3, (((((var_2 ? 28955 : var_0))) * (max(var_11, var_9))))));
    #pragma endscop
}
