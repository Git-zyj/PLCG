/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71414
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71414 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71414
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [i_0] |= (((arr_3 [i_0] [i_0] [i_0]) >= var_6));
                arr_7 [i_1] = ((~((32767 / (arr_3 [i_0] [i_1] [i_1])))));
                arr_8 [i_0] [i_0] = ((((max(105, var_1))) - (((arr_2 [i_0]) ? (arr_5 [i_0] [i_0]) : (arr_3 [i_0] [i_0] [i_1])))));
            }
        }
    }
    var_10 = (-9223372036854775807 - 1);
    var_11 = (((((var_7 << (3 % var_5)))) ? var_6 : (max((~4611686018427387648), (var_0 == var_3)))));
    var_12 = var_0;
    var_13 *= var_0;
    #pragma endscop
}
