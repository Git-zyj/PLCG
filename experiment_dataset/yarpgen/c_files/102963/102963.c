/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102963
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102963 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102963
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] [i_0] = (((((14 ? 0 : 4294967281))) ? (arr_3 [i_1] [i_1]) : var_3));
                arr_5 [i_0] [i_0] [i_0] = ((((!(arr_2 [i_0 + 1]))) ? 4294967274 : (max((((var_2 ? -2147483642 : 4294967282))), (0 % 1)))));
                var_13 = (((arr_1 [i_0 + 1]) ? -83 : (arr_1 [i_1])));
                arr_6 [i_0] [i_1] [i_0] |= var_3;
            }
        }
    }
    var_14 = min(134, 1);
    #pragma endscop
}
