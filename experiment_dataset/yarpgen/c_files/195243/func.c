/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195243
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195243 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195243
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = max((var_7), (((/* implicit */long long int) (~(var_6)))));
    var_17 *= ((/* implicit */unsigned int) var_6);
    var_18 = ((/* implicit */int) var_14);
    var_19 = ((/* implicit */short) var_2);
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_20 = ((/* implicit */short) (-(arr_2 [i_0] [i_0])));
        var_21 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) (+(arr_1 [i_0] [i_0])))) && (((/* implicit */_Bool) ((arr_0 [i_0]) << (((arr_0 [i_0]) - (3621135670U))))))));
    }
}
