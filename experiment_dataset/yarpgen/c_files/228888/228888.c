/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 228888
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=228888 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/228888
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((-(min(var_4, -var_10))));
    var_21 = (min(((((min(var_17, var_11)) != var_10))), (min(((max(var_4, var_13))), (~var_19)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    var_22 = (min((min((min(12582912, (arr_4 [i_0]))), var_6)), ((((var_0 >= var_1) <= (arr_6 [i_0] [i_1] [i_0]))))));
                    var_23 &= ((!(~var_8)));
                    var_24 = -8192;
                }
            }
        }
    }
    #pragma endscop
}
