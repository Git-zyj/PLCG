/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85069
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85069 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85069
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, ((((!var_16) || ((min((((var_6 ? 11 : var_8))), var_7))))))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 4; i_2 < 8;i_2 += 1)
                {
                    for (int i_3 = 4; i_3 < 8;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_20 = var_6;
                                var_21 = ((((var_15 ? ((max((arr_11 [i_0] [i_0]), 56))) : (arr_6 [i_0 + 1] [i_0] [i_0]))) != (min(1, (max((arr_11 [i_1] [i_3]), var_5))))));
                            }
                        }
                    }
                }
                var_22 = max((((arr_12 [i_0] [i_1] [i_1] [i_0]) ? (arr_11 [i_0 + 1] [i_1]) : var_1)), var_17);
                var_23 = var_12;
            }
        }
    }

    for (int i_5 = 0; i_5 < 14;i_5 += 1)
    {
        var_24 = (arr_13 [i_5]);
        var_25 = (arr_14 [0] [i_5]);
        var_26 = -1108654786260748060;
    }
    #pragma endscop
}
