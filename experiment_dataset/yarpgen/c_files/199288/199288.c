/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199288
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199288 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199288
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_7;

    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 3; i_1 < 8;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                for (int i_3 = 4; i_3 < 8;i_3 += 1)
                {
                    {
                        var_11 = arr_5 [i_1] [i_0];
                        var_12 = (((arr_5 [1] [i_1]) ? ((min(1, 231))) : var_8));
                        var_13 = (min(((min((min(var_8, 1)), ((max((arr_3 [i_1] [i_1]), var_0)))))), (arr_3 [i_1 - 2] [i_1])));
                    }
                }
            }
        }
        arr_11 [i_0] = min((((min((arr_5 [1] [i_0]), (arr_6 [6]))))), var_9);
        arr_12 [i_0] [i_0] = ((-(((arr_6 [0]) ? (min((arr_1 [i_0] [i_0]), var_5)) : (~var_1)))));
    }
    #pragma endscop
}
