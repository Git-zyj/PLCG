/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179426
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179426 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179426
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = (max((arr_1 [i_0]), (min((arr_1 [i_0 + 1]), (arr_0 [i_0] [i_0 + 1])))));
        arr_4 [i_0] [i_0] = ((!((max((max(15, 238)), var_8)))));
        arr_5 [i_0] = (arr_1 [i_0]);
        arr_6 [i_0] = (arr_1 [i_0]);
    }
    var_17 |= (min((-9223372036854775807 - 1), var_0));
    var_18 = (max(var_18, -15));
    var_19 = var_10;
    var_20 = var_13;
    #pragma endscop
}
