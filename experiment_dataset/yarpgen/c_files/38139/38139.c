/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38139
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38139 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38139
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 += 3800894704;
    var_17 ^= (min(((var_14 ? var_15 : var_8)), (!-18446744073709551615)));
    var_18 = (!15446174349546166023);

    /* vectorizable */
    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 7;i_2 += 1)
            {
                {
                    var_19 = (arr_1 [i_1]);

                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        var_20 = ((-(arr_0 [i_0])));

                        for (int i_4 = 0; i_4 < 10;i_4 += 1)
                        {
                            var_21 = (((arr_9 [i_0] [i_0 + 2]) ? (arr_6 [i_0] [i_0] [i_0 + 1] [i_0]) : (arr_3 [i_0 + 1])));
                            arr_10 [i_4] [i_4] [i_4] [i_4] [i_4] [i_4] [i_1] |= arr_1 [i_0 + 2];
                            arr_11 [i_0] [i_0] [i_2] [i_3] [i_3] [i_0 - 1] = (arr_6 [i_0 + 2] [i_0 + 2] [i_0 - 1] [i_2 - 1]);
                            var_22 += arr_6 [i_0] [i_1] [i_2] [i_4];
                        }
                        for (int i_5 = 0; i_5 < 10;i_5 += 1)
                        {
                            var_23 += (!var_10);
                            arr_15 [i_0] = (arr_0 [i_0]);
                        }
                        arr_16 [i_0] [i_3] = (arr_12 [i_0 + 1] [i_0 + 1] [i_1] [i_2 + 1] [i_2] [i_2 + 3]);
                    }
                }
            }
        }
        var_24 = var_2;
    }
    #pragma endscop
}
