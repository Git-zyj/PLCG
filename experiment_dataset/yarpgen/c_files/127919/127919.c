/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127919
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127919 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127919
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_16 = ((min((arr_1 [i_1]), (arr_2 [i_0] [i_1] [i_0]))) / ((((arr_3 [i_0]) ? (arr_1 [i_1]) : (arr_0 [i_0])))));
                arr_4 [i_0] [i_1] = var_11;
                var_17 = (arr_2 [6] [i_1] [i_1]);
                arr_5 [i_0] [i_0] [i_0] = -1073741823;
                arr_6 [i_0] [i_0] [i_0] = (arr_2 [i_0] [i_0] [i_1]);
            }
        }
    }
    var_18 = (((((((max(var_1, var_13))) ? var_5 : 4294967295))) ? (max(var_5, (((-39 ? 77 : var_0))))) : ((((!(((-1 ? 3041319158 : var_6)))))))));
    #pragma endscop
}
