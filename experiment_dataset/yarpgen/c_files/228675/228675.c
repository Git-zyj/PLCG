/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 228675
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=228675 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/228675
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_4;
    var_11 = (min((max(((min(var_3, var_8))), var_6)), 0));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = (((0 >> (1 <= var_9))));
                var_12 = (min((max(-1165980512, (arr_2 [14]))), ((var_3 ? (arr_2 [12]) : var_3))));
                var_13 = (max((arr_1 [i_0]), (arr_0 [i_0] [0])));
                arr_6 [6] [i_0] [11] = (min((min((arr_0 [i_0] [i_1]), (arr_4 [i_0] [i_0]))), (arr_0 [2] [i_1])));
            }
        }
    }
    var_14 = var_5;
    #pragma endscop
}
