/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158115
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158115 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158115
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 |= ((/* implicit */unsigned char) (short)-28379);
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 3; i_0 < 18; i_0 += 2) 
    {
        arr_3 [i_0] = ((/* implicit */long long int) (+((-(((/* implicit */int) (!(((/* implicit */_Bool) (signed char)111)))))))));
        arr_4 [i_0] [i_0 + 1] = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) (short)11939))));
    }
}
