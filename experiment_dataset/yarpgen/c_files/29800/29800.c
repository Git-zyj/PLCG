/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29800
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29800 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29800
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(var_14, 1239977528));
    var_15 = (((((-(~55)))) ? 210 : var_13));
    var_16 += 46;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        {
                            var_17 = -57;
                            var_18 = ((((~(arr_4 [i_0]))) * ((((arr_2 [i_0] [i_0 - 1]) + ((min(40, 218))))))));
                            arr_12 [i_0] [i_1] = ((-(9219164116931025559 / var_8)));
                        }
                    }
                }
                arr_13 [i_0] [i_0] [i_0] = (arr_3 [i_0]);
                arr_14 [i_0] [i_1 - 1] [i_0] = (min(1239977528, ((max(57, (arr_1 [i_1 + 1]))))));
            }
        }
    }
    #pragma endscop
}
