/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90137
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90137 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90137
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_1] = (86 < 49152);
                var_13 = ((-13 ? 16360 : 6));
                var_14 = (((-var_8 ? ((var_12 ? 49153 : (arr_4 [i_0 - 2] [i_1]))) : (((var_0 ? 10 : var_2))))));
                var_15 = (max(var_15, ((((((var_1 << 1) ? 24605 : var_3))) % ((var_7 ? 18446744073709551615 : var_6))))));
            }
        }
    }
    var_16 = (max(202984448, -255));
    #pragma endscop
}
