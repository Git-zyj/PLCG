/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151970
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151970 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151970
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_17;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_19 = (((!var_5) / 1972024242));
        arr_2 [i_0] = 87;
        arr_3 [i_0] [2] |= (!var_2);
        arr_4 [i_0] = arr_0 [i_0] [1];
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        arr_7 [12] = ((1787100556 * ((((arr_5 [i_1] [i_1]) != (arr_5 [i_1] [i_1]))))));
        arr_8 [i_1] = (~(max((((0 ? (arr_6 [i_1]) : -377365435973067577))), (max(0, var_17)))));
        var_20 = ((((((max((arr_6 [i_1]), (arr_6 [i_1])))) >> (59 - 51))) | ((((max(0, 180))) ? -94 : 0))));
    }
    #pragma endscop
}
