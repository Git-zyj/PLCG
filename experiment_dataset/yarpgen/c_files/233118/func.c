/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233118
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233118 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233118
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */long long int) var_6);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 2; i_0 < 15; i_0 += 3) 
    {
        for (int i_1 = 0; i_1 < 18; i_1 += 4) 
        {
            {
                var_16 = (~(min((((var_10) * (4373530685990272198ULL))), (((3180523451693372789ULL) % (((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned char)230))))))));
                var_17 ^= ((/* implicit */unsigned long long int) ((max((4373530685990272206ULL), (((/* implicit */unsigned long long int) var_2)))) != (((((/* implicit */_Bool) var_10)) ? (1215874245486078723ULL) : (var_10)))));
            }
        } 
    } 
}
