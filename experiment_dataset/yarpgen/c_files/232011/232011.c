/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 232011
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=232011 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/232011
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_9;
    var_16 = -231294885;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 19;i_1 += 1)
        {
            {
                var_17 = (min(var_17, (((-(min(((min((arr_2 [i_1]), (arr_4 [i_1])))), (((arr_5 [i_0] [i_1]) ? 80 : (arr_4 [i_0]))))))))));
                var_18 = (min(var_18, ((max((((~var_6) ^ var_13)), (min((arr_5 [i_0] [i_0]), (((arr_4 [i_1]) ? (arr_4 [i_0]) : -7597)))))))));
                arr_6 [i_1] [i_1] [i_1 - 2] = ((~(((arr_3 [i_1 - 3]) >> (((!(arr_5 [i_0] [i_0]))))))));
            }
        }
    }
    #pragma endscop
}
