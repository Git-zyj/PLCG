/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219914
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219914 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219914
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_15 = max((arr_6 [i_0 + 1]), (((((min(-282080074, var_3)) + 2147483647)) >> (!var_6))));
                var_16 = ((min(31855, 627541999)));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 2; i_2 < 16;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 19;i_3 += 1)
        {
            {
                arr_12 [i_2] [i_2] = (max(((min((arr_5 [i_3] [i_2]), (arr_3 [i_3])))), 1048575));
                arr_13 [i_2] [i_2] [i_3] = (arr_9 [i_2] [i_3]);
            }
        }
    }
    #pragma endscop
}
