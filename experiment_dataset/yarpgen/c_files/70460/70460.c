/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70460
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70460 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70460
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((min(var_8, 0)));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_12 = (max(((min((22 > 1), 479650305))), (((min(10640820282517921343, (arr_0 [i_0 + 1]))) | (~240)))));
                var_13 = ((!(((var_3 & (1 == var_2))))));
                arr_4 [i_0] = ((-(((((arr_1 [i_1]) != (arr_3 [i_0]))) ? (-9305 & 255) : -5))));
            }
        }
    }
    var_14 = (max(var_14, ((((max(-273243238, 25)) >= (~var_5))))));
    #pragma endscop
}
