/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14450
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14450 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14450
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_3;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 12;i_1 += 1)
        {
            {
                var_17 = ((((((arr_3 [i_0] [i_0] [i_0]) ? 20 : (arr_2 [i_1 - 3] [i_1 + 1] [i_1 - 4])))) ? (arr_1 [i_0] [i_0]) : (((arr_3 [7] [7] [i_0]) * ((-28563 ? (arr_5 [i_0]) : (arr_4 [i_0])))))));
                arr_6 [i_0] [i_0] [i_1] = 20;
                var_18 = arr_2 [i_0] [6] [i_1];
                /* LoopNest 2 */
                for (int i_2 = 3; i_2 < 9;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 10;i_3 += 1)
                    {
                        {
                            var_19 += 45;
                            arr_11 [i_0] [9] [i_0] [9] [i_3] = (max(25422, 0));
                            var_20 -= arr_1 [i_1] [i_2 + 1];
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
