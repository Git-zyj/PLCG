/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98625
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98625 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98625
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 10; i_0 += 3) 
    {
        for (unsigned char i_1 = 0; i_1 < 10; i_1 += 3) 
        {
            {
                arr_7 [i_0] [i_0] [i_0] = ((/* implicit */short) (-(((/* implicit */int) ((((/* implicit */long long int) ((/* implicit */int) (unsigned char)184))) > (var_13))))));
                arr_8 [i_0] = ((/* implicit */unsigned char) (-((-(((/* implicit */int) (signed char)34))))));
            }
        } 
    } 
    var_18 = ((/* implicit */unsigned char) (!(((/* implicit */_Bool) var_6))));
}
