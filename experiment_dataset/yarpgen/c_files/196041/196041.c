/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196041
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196041 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196041
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    arr_7 [i_0] [2] [i_2] = 1274;
                    arr_8 [i_0] [i_0] = (min((var_11 && var_16), var_11));
                }
            }
        }
    }
    var_18 |= ((((var_17 ? var_12 : (((var_2 + 2147483647) << (1 - 1))))) - (((var_12 > (!var_11))))));
    var_19 = ((-((var_14 + ((-19248 ? var_5 : var_17))))));
    var_20 = (!var_15);
    var_21 = 11790589028784298506;
    #pragma endscop
}
