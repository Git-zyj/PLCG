/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95425
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95425 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95425
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = max(-524288, ((524314 ? var_2 : var_4)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 10;i_2 += 1)
            {
                {
                    var_14 = (min((max((max(524288, 524314)), (arr_6 [i_0] [i_0] [i_0]))), var_0));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 10;i_4 += 1)
                        {
                            {
                                var_15 = var_1;
                                var_16 = (((((-1681118030075940645 ? 65024 : (((min(3968, 127))))))) ? (arr_2 [i_0]) : ((max(((min(15, 255))), (min(0, 9223372036854775807)))))));
                                var_17 = -1;
                                var_18 += ((-105 ? 250 : 134213632));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
