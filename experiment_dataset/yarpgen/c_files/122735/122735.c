/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122735
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122735 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122735
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_12 |= 3;
                arr_6 [i_0 - 1] [i_1] [i_0 - 1] = (((((arr_3 [i_0 - 2]) + 4398046511088))) ? (((arr_0 [i_0] [i_0]) ? (arr_5 [i_0] [i_0 + 1]) : (arr_3 [i_1]))) : (((arr_4 [i_0] [i_0 + 1] [i_0 - 2]) ? (arr_1 [i_0 - 2]) : (arr_1 [i_1]))));
            }
        }
    }

    for (int i_2 = 0; i_2 < 18;i_2 += 1)
    {
        var_13 = (--18446739675663040530);
        arr_9 [i_2] = (min((((arr_8 [i_2]) ^ 7539929384813102769)), (arr_8 [i_2])));
        arr_10 [i_2] [i_2] &= arr_8 [i_2];
    }
    #pragma endscop
}
