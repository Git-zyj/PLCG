/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 1863
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=1863 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/1863
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (var_11 + -var_14);

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        var_17 = ((-(arr_1 [i_0] [7])));
        var_18 = ((~(arr_1 [i_0] [i_0])));
        var_19 *= (arr_0 [i_0] [i_0]);
        var_20 = (arr_0 [i_0] [i_0]);
    }
    var_21 = ((((max(var_0, var_9))) >> var_7));
    /* LoopNest 2 */
    for (int i_1 = 3; i_1 < 9;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 11;i_2 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_3 = 2; i_3 < 10;i_3 += 1)
                {
                    for (int i_4 = 0; i_4 < 11;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 11;i_5 += 1)
                        {
                            {
                                arr_12 [1] [i_1] [i_3] [i_1] [i_1] = ((~((((arr_8 [i_1] [i_2] [i_1] [i_1] [i_5]) >= (((arr_7 [i_1 - 3] [i_1 - 3] [i_3] [i_4]) % var_3)))))));
                                var_22 ^= (((arr_9 [i_1] [i_1] [6] [5] [i_3] [i_3 - 1]) ? ((((min((arr_10 [i_5] [i_4] [i_3] [i_2] [i_1]), 32767))) * var_7)) : (((!(~5986570867811201048))))));
                                var_23 = var_11;
                            }
                        }
                    }
                }
                var_24 = (arr_6 [i_1] [i_1] [i_2] [9]);
            }
        }
    }
    #pragma endscop
}
