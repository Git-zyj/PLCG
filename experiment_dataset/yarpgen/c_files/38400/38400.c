/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38400
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38400 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38400
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 += ((1432930419 ? (((-32767 - 1) ? -5952275209865546888 : (min(0, var_14)))) : var_10));
    var_17 = ((var_10 >> (((min(var_2, var_12)) - 61516))));
    var_18 = (var_0 + (var_1 + 1020));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = ((!(((1029 / (arr_1 [i_1]))))));
                var_19 += (((var_2 ? (arr_2 [i_1] [i_0] [i_0]) : (arr_2 [i_0] [i_1] [i_1]))));
                var_20 += var_7;
            }
        }
    }
    #pragma endscop
}
