/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 58996
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=58996 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/58996
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((((((-1 * var_1) ? ((var_8 ? 16579993761629214023 : var_4)) : 1))) ? var_10 : (((((var_9 ? 2400869341 : var_6))) ? 44920 : ((1866750312080337578 ? 16579993761629214007 : var_1))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_15 -= var_2;
                var_16 = (min(var_16, (((1838842315 ? (arr_5 [i_0]) : (((arr_6 [i_0] [i_0]) ? (arr_6 [i_0] [i_0]) : (arr_0 [i_1]))))))));
            }
        }
    }
    #pragma endscop
}
