/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143578
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143578 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143578
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((unsigned short) var_1);
    /* LoopNest 3 */
    for (signed char i_0 = 0; i_0 < 24; i_0 += 2) 
    {
        for (unsigned short i_1 = 0; i_1 < 24; i_1 += 1) 
        {
            for (unsigned char i_2 = 0; i_2 < 24; i_2 += 4) 
            {
                {
                    arr_8 [i_0] [i_2] = ((/* implicit */signed char) var_7);
                    arr_9 [i_2] [i_1] [i_1] = arr_4 [i_1] [i_1] [i_1];
                }
            } 
        } 
    } 
    var_14 = ((/* implicit */long long int) ((min((((var_12) - (((/* implicit */unsigned int) ((/* implicit */int) (short)-17841))))), (((/* implicit */unsigned int) ((((/* implicit */unsigned int) ((/* implicit */int) var_9))) > (var_11)))))) == (((/* implicit */unsigned int) ((/* implicit */int) max(((unsigned char)153), ((unsigned char)152)))))));
}
