/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159084
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159084 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159084
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 ^= var_5;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 6;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 9;i_1 += 1)
        {
            {
                var_12 = (max(var_12, ((min((arr_1 [i_0] [i_0]), (arr_1 [i_0] [i_0]))))));
                var_13 = (arr_0 [i_1]);
                var_14 *= (arr_1 [i_0] [i_1 + 1]);
                /* LoopNest 2 */
                for (int i_2 = 3; i_2 < 9;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            arr_8 [i_0 - 1] [2] [3] = (-9223372036854775807 - 1);

                            for (int i_4 = 1; i_4 < 7;i_4 += 1)
                            {
                                var_15 = -9223372036854775807;
                                var_16 = -1;
                                var_17 = (min(var_8, ((~(arr_9 [i_0] [i_1] [i_1] [i_4 + 3] [i_4])))));
                            }
                            var_18 = (arr_2 [9]);
                        }
                    }
                }
            }
        }
    }

    /* vectorizable */
    for (int i_5 = 1; i_5 < 1;i_5 += 1)
    {
        var_19 = (min(var_19, (((arr_15 [i_5 - 1]) ? (-9223372036854775807 - 1) : 9223372036854775807))));
        arr_16 [i_5] = (arr_13 [i_5 - 1]);
    }
    var_20 = (max(var_20, var_5));
    #pragma endscop
}
