/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94436
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94436 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94436
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_20 = (min(var_20, 58));
                /* LoopNest 3 */
                for (int i_2 = 1; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 14;i_4 += 1)
                        {
                            {
                                arr_16 [i_1] [i_2 - 1] [i_2] [i_3] [i_0] = ((((((((5 << (var_17 - 3939355992535785672)))) ? (arr_5 [i_0] [i_2 - 1] [i_4 + 1]) : (((arr_14 [i_4 - 1] [i_2] [i_1]) * (arr_12 [i_0] [i_0] [i_0] [2] [2] [i_0] [2])))))) ? (~2868800604) : var_18));
                                var_21 = (max(var_21, (arr_7 [i_0] [i_1])));
                            }
                        }
                    }
                }
            }
        }
    }
    var_22 |= ((-(~var_18)));
    #pragma endscop
}
