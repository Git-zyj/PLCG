/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155075
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155075 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155075
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 12;i_2 += 1)
            {
                {
                    var_17 = 115;
                    arr_9 [8] [8] [i_1] [i_2] = (((arr_2 [i_0] [i_0]) >> (((arr_3 [i_0] [i_2] [i_2]) - 23873))));
                    arr_10 [i_0] [i_1] = (((max((arr_8 [i_0] [i_1 + 1] [i_0] [i_2 - 2]), (arr_2 [i_0] [i_1 - 1])))) ? (arr_8 [13] [i_1 + 1] [i_2] [i_2 + 1]) : (arr_2 [4] [i_1 - 1]));
                }
            }
        }
    }

    for (int i_3 = 3; i_3 < 16;i_3 += 1)
    {
        var_18 = ((((min(var_11, 104))) ? (((1 != (arr_11 [i_3 + 1])))) : 6));
        var_19 = 1;
    }
    /* vectorizable */
    for (int i_4 = 0; i_4 < 1;i_4 += 1)
    {
        var_20 = (arr_5 [14] [i_4]);
        arr_16 [i_4] [4] = (arr_0 [1]);
    }
    #pragma endscop
}
