/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 107508
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=107508 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/107508
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = min((((/* implicit */int) var_6)), (var_11));
    var_18 = ((/* implicit */int) (!(((/* implicit */_Bool) max((((/* implicit */unsigned int) min((var_16), (var_2)))), (((unsigned int) var_6)))))));
    /* LoopSeq 1 */
    for (long long int i_0 = 0; i_0 < 17; i_0 += 1) 
    {
        var_19 = ((/* implicit */int) var_0);
        var_20 = ((/* implicit */short) max((var_20), (((/* implicit */short) (!(((/* implicit */_Bool) ((arr_1 [i_0] [i_0]) ? (arr_0 [i_0]) : (arr_0 [i_0])))))))));
    }
}
