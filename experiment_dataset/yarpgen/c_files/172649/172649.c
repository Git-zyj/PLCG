/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172649
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172649 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172649
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = 338984907;
        var_17 = 1071644672;
        arr_5 [i_0] [i_0] = (19 > -11870);
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 10;i_1 += 1)
    {
        var_18 = ((-19 < ((-1 ? -11870 : var_3))));
        arr_8 [i_1] = 338984900;
    }
    var_19 = (max(var_19, -338984908));
    var_20 = (~14987);
    var_21 = ((((min(((-23342 ? -23332 : 689787332)), 1))) ? 2147483647 : 3732968867));
    #pragma endscop
}
