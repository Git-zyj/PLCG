/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156758
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156758 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156758
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 |= (-32767 - 1);

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_16 -= (-32767 - 1);
        arr_2 [i_0] = 3927079080827733262;
        var_17 = 15;

        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            arr_5 [10] &= (-127 - 1);

            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                arr_9 [i_0] = 0;
                var_18 = (-32767 - 1);
                arr_10 [i_2] [i_0] [i_0] = 32758;
            }
            /* vectorizable */
            for (int i_3 = 0; i_3 < 14;i_3 += 1)
            {
                arr_15 [i_0] [i_3] [i_3] [i_0] |= 8556;
                arr_16 [i_3] [i_3] &= -6862647704488600422;
            }
            /* vectorizable */
            for (int i_4 = 2; i_4 < 12;i_4 += 1)
            {
                arr_19 [i_0] [i_1] = 32753;
                arr_20 [i_0] [i_1] [i_0] = -32756;
            }
        }
        for (int i_5 = 1; i_5 < 10;i_5 += 1)
        {
            var_19 = 1;
            var_20 = -8555;
        }
        arr_24 [i_0] = -15374;
    }
    var_21 = 246;
    #pragma endscop
}
