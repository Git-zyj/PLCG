/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 188800
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=188800 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/188800
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 19;i_1 += 1)
        {
            {
                var_10 = (min(var_10, ((max(((max(3352434951018288613, -2049492912))), (~18446744073709551599))))));
                arr_6 [i_0 - 1] [i_1] [i_0 - 1] |= ((~(~var_1)));
            }
        }
    }
    var_11 = (min(-3352434951018288611, 39));

    for (int i_2 = 1; i_2 < 9;i_2 += 1)
    {
        arr_9 [i_2] [i_2] = ((-34 % ((((arr_0 [i_2]) && (((-(arr_0 [i_2 + 1])))))))));
        var_12 += (arr_3 [i_2] [i_2] [i_2]);
    }
    #pragma endscop
}
