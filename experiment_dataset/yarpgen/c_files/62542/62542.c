/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62542
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62542 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62542
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 -= (var_7 + 928047764);
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_15 += ((-((~(arr_2 [i_0 + 1] [i_0 - 2])))));
                var_16 = min(((min(var_11, (arr_2 [i_0] [i_0])))), (((arr_4 [i_1] [i_1] [i_0]) ? (arr_3 [i_1] [i_1] [i_0]) : (arr_4 [i_0 - 1] [i_0] [i_1]))));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 17;i_4 += 1)
                        {
                            {
                                var_17 = ((min((arr_6 [i_0 - 1]), var_5)) ? 1 : ((min((arr_8 [i_1]), (max(var_10, var_6))))));
                                var_18 = (min(var_18, (-163113323 || 8005910681414013832)));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
