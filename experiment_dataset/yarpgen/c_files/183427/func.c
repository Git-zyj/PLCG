/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 183427
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=183427 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/183427
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */int) min((var_14), (((/* implicit */int) (+(var_11))))));
    /* LoopNest 3 */
    for (long long int i_0 = 0; i_0 < 16; i_0 += 1) 
    {
        for (int i_1 = 3; i_1 < 15; i_1 += 2) 
        {
            for (unsigned char i_2 = 1; i_2 < 14; i_2 += 3) 
            {
                {
                    var_15 |= ((/* implicit */signed char) (+(((/* implicit */int) (!(arr_3 [14] [(unsigned char)8]))))));
                    var_16 = ((/* implicit */long long int) (!(((/* implicit */_Bool) (signed char)-18))));
                }
            } 
        } 
    } 
    var_17 = var_2;
    var_18 = ((/* implicit */int) max((var_18), (((/* implicit */int) ((var_6) > (((var_8) * (((/* implicit */int) (!(((/* implicit */_Bool) var_0))))))))))));
}
