/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38344
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38344 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38344
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */unsigned char) var_3);
    /* LoopNest 2 */
    for (signed char i_0 = 1; i_0 < 12; i_0 += 3) 
    {
        for (_Bool i_1 = 1; i_1 < 1; i_1 += 1) 
        {
            {
                var_18 = ((/* implicit */int) (!(((/* implicit */_Bool) arr_2 [i_0 + 4] [i_1 - 1]))));
                arr_7 [13] [i_1] [i_1] = ((/* implicit */unsigned int) max((((21) > (((/* implicit */int) var_9)))), (((((/* implicit */unsigned long long int) ((/* implicit */int) arr_0 [i_0 + 3]))) <= (((unsigned long long int) (short)21398))))));
            }
        } 
    } 
    var_19 = ((/* implicit */unsigned char) min((var_19), (((/* implicit */unsigned char) var_9))));
}
