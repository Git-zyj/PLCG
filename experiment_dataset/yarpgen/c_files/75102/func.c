/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75102
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75102 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75102
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned int) ((((/* implicit */int) (!(((/* implicit */_Bool) (+(var_13))))))) > (((/* implicit */int) var_6))));
    var_21 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) ((int) min((var_11), (((/* implicit */unsigned int) var_15))))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 0; i_0 < 22; i_0 += 2) 
    {
        var_22 = ((/* implicit */int) (-((-(var_19)))));
        arr_3 [i_0] = ((((/* implicit */_Bool) arr_1 [i_0])) ? (((/* implicit */int) arr_1 [i_0])) : (var_5));
    }
}
