/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52019
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52019 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52019
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_13 = (((8908 ? 205428650232807319 : 336636008)));
                arr_4 [i_1] = ((668001200 <= ((max((arr_1 [i_0] [i_0]), (arr_1 [i_1] [5]))))));
            }
        }
    }
    var_14 = (min(((max(2706668854, var_0))), ((min(56599, 2706668854)))));
    var_15 = (min(2706668859, (((((var_4 ? var_12 : var_12))) % var_9))));
    #pragma endscop
}
