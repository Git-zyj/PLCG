/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14102
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14102 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14102
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 23;i_1 += 1)
        {
            {
                arr_4 [i_1] [i_0 - 2] [i_0 - 2] = 2255;
                /* LoopNest 2 */
                for (int i_2 = 3; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        {
                            var_20 = (arr_2 [i_0 + 1]);
                            arr_9 [i_2] [i_2 + 1] [i_2 - 3] = (!((((arr_2 [i_1]) ? (((((arr_3 [i_0] [i_0]) == (arr_8 [i_0] [i_2] [i_1] [i_0]))))) : (arr_2 [i_2])))));
                        }
                    }
                }
                var_21 = (max(var_21, ((((arr_6 [i_1]) >> (((arr_1 [i_0 + 1]) + 64)))))));
            }
        }
    }
    var_22 ^= (min(var_1, 20739));
    var_23 += ((+(min((~var_5), ((min(44796, 44796)))))));
    #pragma endscop
}
