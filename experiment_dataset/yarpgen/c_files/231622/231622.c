/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231622
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231622 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231622
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = 218481476;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_17 += var_8;
                var_18 = (max(var_18, (arr_1 [i_1] [i_0])));
                var_19 &= min(((((max(var_13, var_12))) ? (arr_0 [i_0 - 1] [i_0 - 1]) : (max(1238046716, 3056920579)))), ((((var_9 || (arr_3 [16] [i_0] [i_0]))) ? ((891824535 ? var_9 : var_5)) : (arr_1 [i_0] [i_1]))));
            }
        }
    }
    var_20 ^= (17456 && var_6);
    #pragma endscop
}
