/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4424
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4424 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4424
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 &= ((/* implicit */unsigned char) var_9);
    /* LoopSeq 1 */
    for (signed char i_0 = 1; i_0 < 21; i_0 += 1) 
    {
        var_14 = ((/* implicit */unsigned short) max((var_14), (((/* implicit */unsigned short) max((-189163882), (189163882))))));
        arr_2 [i_0] = ((/* implicit */signed char) min((((/* implicit */long long int) ((((/* implicit */_Bool) ((9223372036854775807LL) | (((/* implicit */long long int) arr_0 [i_0]))))) ? (((/* implicit */unsigned int) ((/* implicit */int) (unsigned char)149))) : (arr_1 [i_0] [i_0])))), (11790032259531255LL)));
    }
    var_15 = ((/* implicit */signed char) var_1);
}
