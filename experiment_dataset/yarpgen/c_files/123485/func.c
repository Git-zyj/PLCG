/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123485
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123485 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123485
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */unsigned char) var_10);
    var_14 = ((/* implicit */unsigned long long int) ((unsigned short) (+(((/* implicit */int) ((unsigned char) (unsigned char)255))))));
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 0; i_0 < 24; i_0 += 3) 
    {
        for (int i_1 = 1; i_1 < 23; i_1 += 3) 
        {
            for (int i_2 = 0; i_2 < 24; i_2 += 3) 
            {
                {
                    var_15 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) arr_6 [i_0] [i_0] [i_1 - 1] [(unsigned short)9]))));
                    var_16 = ((/* implicit */int) var_4);
                }
            } 
        } 
    } 
}
