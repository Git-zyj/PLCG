/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 35754
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=35754 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/35754
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (var_10 / (((~16125) ? 255 : (24 && 19))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_7 [i_1] = (min((max(33014, (max((arr_1 [i_0]), (arr_3 [i_0] [i_0]))))), (min((arr_1 [i_0]), (((arr_6 [i_1] [i_1]) ? var_0 : var_6))))));
                var_18 = ((~((4294967291 ? (33027 & 49) : 28))));
            }
        }
    }
    var_19 = (max((((226 ? var_3 : var_9))), ((~(118 <= 4294967294)))));
    #pragma endscop
}
