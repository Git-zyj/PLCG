/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241500
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241500 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241500
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_19 += ((((~(arr_1 [i_1] [i_0]))) % (arr_0 [i_0 - 1])));
                var_20 *= ((104 || ((((((arr_0 [i_1]) && (arr_1 [i_1] [14]))) ? (((var_16 ? 2498220814 : var_2))) : (arr_4 [i_0] [i_1]))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 24;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        {
                            arr_12 [i_0] [i_2] = (max((min(1, (arr_6 [i_2]))), ((2413815980 ? (arr_9 [i_3] [16] [i_0]) : (arr_6 [i_2])))));
                            var_21 = (var_14 & 1072395957);
                        }
                    }
                }
            }
        }
    }
    var_22 = (max(var_22, var_17));
    var_23 = (((!var_16) ? var_6 : var_10));
    #pragma endscop
}
