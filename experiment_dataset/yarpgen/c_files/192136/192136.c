/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 192136
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=192136 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/192136
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((~(!var_8)));
    var_14 = var_10;
    var_15 = ((((max((38941 >= var_2), (max(var_8, var_11))))) == var_6));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    arr_7 [1] [1] [1] [i_2] = ((~(arr_0 [i_0])));
                    var_16 = (min((max((((var_5 ? 23336 : (arr_3 [i_0] [i_0] [i_0])))), (arr_0 [i_2]))), ((-(arr_0 [i_0])))));
                    var_17 |= ((!(((var_6 ? (arr_2 [i_1 + 1]) : (arr_2 [i_1 + 1]))))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                arr_14 [i_2] [i_2] [i_2] [i_2] = -23337;
                                arr_15 [12] = (arr_4 [17]);
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
