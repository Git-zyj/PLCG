/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212543
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212543 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212543
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(var_14, ((((14 ? 51727 : 134217727))))));
    var_15 = (min((max(var_9, 13810)), var_7));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 14;i_2 += 1)
            {
                {
                    var_16 = 99;
                    var_17 = (max(var_17, (!13794)));
                }
            }
        }
    }
    var_18 = (min(var_2, ((16814 ? var_11 : ((max(var_1, var_12)))))));
    var_19 = ((((var_2 | (((var_0 ? var_10 : -90))))) * (!var_11)));
    #pragma endscop
}
