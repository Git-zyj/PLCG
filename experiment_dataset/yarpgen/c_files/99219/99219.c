/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99219
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99219 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99219
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 -= var_4;

    for (int i_0 = 4; i_0 < 8;i_0 += 1)
    {
        var_15 *= (arr_0 [i_0]);
        var_16 -= (min((((((7560771007510998192 ? 13835058055282163712 : 13835058055282163712))) ? 1 : 13835058055282163736)), (~0)));
        arr_2 [i_0] = (arr_1 [i_0]);
    }
    var_17 = ((((var_5 / var_0) ? (var_11 > var_4) : ((13835058055282163743 << (4611686018427387879 - 4611686018427387859))))));
    #pragma endscop
}
