/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95709
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95709 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95709
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            {
                var_19 = ((~((~(arr_0 [i_1 - 1])))));
                arr_5 [i_0] [i_0] = ((+(((((arr_1 [i_0] [i_0]) && (arr_2 [i_0] [4]))) ? (!var_14) : ((min((arr_4 [i_1]), (arr_0 [i_0]))))))));
                arr_6 [i_0] [i_0] = (((((var_10 && 1) / (~4014816241))) * ((((!(arr_3 [i_1 + 1])))))));
                var_20 += (((((~((~(arr_4 [i_0])))))) ? (arr_0 [i_1 + 1]) : ((~(arr_3 [i_1 + 1])))));
            }
        }
    }
    var_21 = (max(var_21, -var_18));
    var_22 -= var_17;
    var_23 = var_8;
    #pragma endscop
}
