/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 176741
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=176741 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/176741
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [i_1] [i_1] [i_0] = ((+(((((var_7 ? (arr_3 [i_0] [i_1] [i_1]) : (arr_3 [i_0] [2] [i_0])))) ? ((max((arr_3 [i_0] [i_1] [i_1]), -33))) : -27809))));
                var_13 = (min(((((arr_3 [i_1] [i_0 + 1] [i_0 + 1]) && (arr_2 [i_0])))), ((((((arr_0 [i_1] [i_0]) ? (arr_0 [i_0] [i_1]) : 155))) & (min(var_9, (arr_3 [i_0] [i_1] [i_1])))))));
            }
        }
    }
    var_14 &= var_9;
    var_15 = (min(var_4, var_0));
    #pragma endscop
}
