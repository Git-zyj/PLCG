/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76954
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76954 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76954
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_10 = (max((~134217472), var_9));
                arr_5 [i_1] = 17682488435289885671;
                arr_6 [i_1] [i_0] [13] = ((~((((((var_6 ? (arr_1 [i_0]) : var_9))) || (764255638419665945 & 11888802722407879063))))));
            }
        }
    }

    /* vectorizable */
    for (int i_2 = 1; i_2 < 1;i_2 += 1)
    {
        var_11 ^= ((var_9 <= (arr_3 [i_2 - 1])));
        var_12 *= ((!(arr_4 [i_2 - 1])));
    }
    #pragma endscop
}
