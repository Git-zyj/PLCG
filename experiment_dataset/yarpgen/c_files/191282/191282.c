/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191282
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191282 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191282
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (~((-3337902869276032185 ? (((var_11 << (-86 + 113)))) : -3337902869276032184)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_13 = (min((min(16486, 9223372036854775807)), ((min((arr_1 [i_0]), ((max((arr_2 [i_0]), (arr_5 [i_0])))))))));
                arr_8 [1] [i_1] = (-5243692185330670334 ^ (((~(arr_4 [i_0] [i_1])))));
            }
        }
    }
    #pragma endscop
}
