/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180940
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180940 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180940
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_16 = ((~(arr_3 [i_1] [i_0])));
                var_17 = min(((((!-17950) <= var_6))), (max((var_3 - -286859459), -30313)));
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 8;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 6;i_3 += 1)
                    {
                        {
                            var_18 = (max((-127 - 1), -1588218980));
                            var_19 = ((min((~0), 0)));
                            arr_10 [i_0] [i_0] = (((((max(var_15, var_2)) | (max(var_1, var_9))))));
                            var_20 = (max(var_20, (((var_4 ? (var_1 + 1) : var_2)))));
                        }
                    }
                }
            }
        }
    }
    var_21 = (max((max(var_9, (min(1886752708, (-2147483647 - 1))))), (1 == 0)));
    #pragma endscop
}
