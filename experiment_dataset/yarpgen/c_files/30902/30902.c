/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30902
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30902 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30902
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 20;i_1 += 1)
        {
            {
                arr_5 [i_0] = ((((min(var_4, var_6))) != ((max((arr_3 [i_1 - 1] [i_1 + 2]), (arr_3 [i_1 - 1] [i_1 + 2]))))));
                arr_6 [i_0] [i_0] = (min((((((var_0 ? (arr_4 [i_1]) : (arr_1 [i_1]))) == ((min(-1, (arr_3 [i_0] [i_0]))))))), (min(((~(arr_3 [i_1] [i_0]))), (arr_3 [i_0] [i_1 + 2])))));
            }
        }
    }
    var_10 = ((!((((var_2 % var_9) ? (max(-1405970302545822069, (-127 - 1))) : var_9)))));
    var_11 = var_4;
    var_12 = var_8;
    #pragma endscop
}
