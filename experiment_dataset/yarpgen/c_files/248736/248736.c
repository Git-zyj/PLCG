/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 248736
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=248736 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/248736
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] |= ((((((max(0, var_0)) | (((1 - (arr_4 [i_1]))))))) ? 1 : (min(var_4, -1))));
                arr_6 [i_0 + 2] = (((((var_1 ? ((var_8 + (arr_1 [i_0 + 2]))) : ((((arr_4 [i_0]) < 64)))))) ? ((max(((1 ? 19332 : (arr_2 [i_0] [i_0]))), ((min((arr_3 [8]), (arr_4 [i_0 - 1]))))))) : (((arr_4 [i_0]) ^ 0))));
                var_10 = (arr_1 [i_0 - 1]);
            }
        }
    }
    var_11 = -1;
    #pragma endscop
}
