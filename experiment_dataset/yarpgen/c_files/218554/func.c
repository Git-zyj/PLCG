/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218554
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218554 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218554
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned char) var_1);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 1; i_0 < 11; i_0 += 3) 
    {
        for (unsigned short i_1 = 3; i_1 < 11; i_1 += 1) 
        {
            {
                var_16 += ((/* implicit */short) (-(max((arr_1 [i_0 + 2]), (arr_1 [i_0 - 1])))));
                var_17 = ((/* implicit */unsigned char) (((-2147483647 - 1)) & (((/* implicit */int) arr_3 [i_0] [i_1]))));
                arr_5 [i_0] [i_0] [i_0] = ((/* implicit */unsigned short) arr_0 [i_0]);
                var_18 = ((/* implicit */short) min(((+(arr_1 [i_0]))), (((/* implicit */long long int) arr_4 [i_0]))));
            }
        } 
    } 
}
