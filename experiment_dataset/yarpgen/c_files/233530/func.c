/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233530
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233530 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233530
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 += ((/* implicit */unsigned long long int) min((((/* implicit */unsigned int) ((((/* implicit */_Bool) (+(var_3)))) ? (((/* implicit */int) ((unsigned short) var_0))) : (((((/* implicit */_Bool) var_0)) ? (((/* implicit */int) var_2)) : (((/* implicit */int) var_6))))))), (2241555064U)));
    var_11 = var_4;
    var_12 = ((/* implicit */unsigned int) var_6);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 1; i_0 < 11; i_0 += 1) 
    {
        for (unsigned int i_1 = 1; i_1 < 12; i_1 += 1) 
        {
            {
                var_13 = ((/* implicit */signed char) ((((/* implicit */int) (!(((/* implicit */_Bool) 3935179034U))))) <= (max((-2079247218), (((/* implicit */int) (signed char)-21))))));
                var_14 = ((/* implicit */long long int) var_3);
                var_15 = ((/* implicit */signed char) arr_4 [i_0 + 1] [6ULL] [i_0]);
            }
        } 
    } 
}
