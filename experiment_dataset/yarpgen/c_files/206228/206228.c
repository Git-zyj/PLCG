/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206228
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206228 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206228
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_13, (max(var_17, (~var_16)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        {
                            var_20 = var_12;
                            arr_10 [10] = var_15;
                            arr_11 [i_1] = var_10;
                            arr_12 [i_0] [i_0] [i_0] [2] = (arr_5 [i_3] [i_2] [8] [7]);
                        }
                    }
                }
                var_21 &= min((max((arr_6 [i_0]), (arr_6 [i_0]))), (arr_6 [i_0]));
            }
        }
    }
    var_22 = ((~((var_2 ^ ((2250916369869443616 ? 2250916369869443626 : var_5))))));
    var_23 = var_6;
    #pragma endscop
}
