/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172611
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172611 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172611
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (min(var_17, (((((min(var_2, var_8))) && ((min((min(var_9, var_16)), var_7))))))));
    var_18 = ((var_6 ? var_6 : var_0));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_19 = ((~(((var_4 ^ 255) << (((((arr_3 [i_0]) ? (arr_0 [i_0 + 1] [i_1]) : (arr_2 [i_0]))) - 1894665159))))));
                arr_4 [i_0] = (~-1349070710);
                var_20 = (arr_1 [i_0 + 1] [i_1]);
            }
        }
    }
    #pragma endscop
}
