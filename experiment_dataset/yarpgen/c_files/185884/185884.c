/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 185884
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=185884 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/185884
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_11, var_7));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    var_12 = (max(45985, 268435456));
                    /* LoopNest 2 */
                    for (int i_3 = 4; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 17;i_4 += 1)
                        {
                            {
                                arr_16 [4] [i_3] [i_2] = (min(((~(arr_14 [i_3] [i_3] [i_3] [i_3] [i_3 - 4]))), (arr_10 [i_0] [i_1])));
                                var_13 = var_0;
                            }
                        }
                    }
                    var_14 = (min(var_14, (((165 <= ((((min((arr_6 [i_0 - 1] [i_0 - 1]), var_8)) != (arr_10 [i_0 - 1] [i_0 - 1])))))))));
                    var_15 = ((var_9 ? ((~(arr_3 [i_0 - 1] [i_2]))) : (arr_5 [i_0] [i_2])));
                }
            }
        }
    }
    #pragma endscop
}
