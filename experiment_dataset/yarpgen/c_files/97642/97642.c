/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97642
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97642 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97642
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            {
                var_12 = (((((var_5 ? 0 : (arr_2 [i_1 - 1] [i_1] [i_1])))) * (((+((((arr_3 [14]) < 0))))))));
                /* LoopNest 2 */
                for (int i_2 = 3; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 13;i_3 += 1)
                    {
                        {
                            var_13 = ((!(((-246 ? (((arr_5 [i_0] [i_2 - 1] [i_1]) ? (arr_7 [i_0] [i_0] [i_0] [i_0] [4] [i_0]) : 246)) : (arr_7 [i_0] [i_1] [i_1] [2] [i_2] [7]))))));
                            var_14 = 218;
                        }
                    }
                }
            }
        }
    }
    var_15 = (+((((var_0 < var_1) < (!var_7)))));
    #pragma endscop
}
