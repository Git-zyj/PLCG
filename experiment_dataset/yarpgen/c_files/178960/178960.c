/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 178960
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=178960 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/178960
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    var_11 = (((((-var_5 ? 330888359 : (arr_3 [i_2])))) ? ((-(arr_1 [i_1]))) : (arr_3 [i_1 - 2])));
                    var_12 = (min(var_12, ((1 ? var_6 : ((max((arr_2 [i_0] [i_1 - 1]), var_4)))))));
                    arr_6 [i_0] [i_1] [i_0] [i_0] &= (((((min(var_1, var_10)))) && ((((~3302163334029654024) ? (max(2147483647, 0)) : (arr_5 [i_0]))))));
                }
            }
        }
    }
    var_13 = var_8;
    var_14 = var_1;
    #pragma endscop
}
