/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 25911
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=25911 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/25911
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        arr_3 [i_0] = (arr_0 [i_0] [i_0]);

        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            var_15 = ((((max(((((arr_2 [i_0]) / 42845))), ((var_0 * (arr_5 [i_0] [i_1])))))) && (((((21510 ? var_7 : var_1)) - (arr_1 [i_0] [i_0]))))));
            arr_6 [i_0] = (arr_2 [i_1]);
        }
    }
    var_16 *= var_8;
    var_17 = (((((var_14 || var_12) && (!var_10))) && 59574));
    var_18 = 6037310645989546212;

    for (int i_2 = 1; i_2 < 21;i_2 += 1)
    {
        arr_9 [i_2] = 0;
        arr_10 [i_2] [i_2 + 2] = (((arr_8 [i_2 - 1]) >= 7));
    }
    #pragma endscop
}
