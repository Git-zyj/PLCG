/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169653
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169653 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169653
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_12 = (max(var_0, -var_0));
        var_13 = -0;
        var_14 = ((1 | (((arr_0 [i_0] [i_0]) ? 1 : var_4))));
        var_15 = ((~(min(var_1, (arr_1 [i_0] [i_0])))));
        var_16 -= (((((((arr_1 [i_0] [i_0]) <= 1)) ? (arr_0 [i_0] [i_0]) : (arr_1 [i_0] [i_0]))) * var_2));
    }
    var_17 = (((var_9 >> ((var_5 ? var_8 : var_6)))) == (min((!1), -0)));
    #pragma endscop
}
