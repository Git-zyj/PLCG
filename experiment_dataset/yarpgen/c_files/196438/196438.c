/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196438
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196438 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196438
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_0;
    var_20 = var_11;
    var_21 = (min((min(var_15, -var_11)), ((4194303 - (var_4 + var_12)))));

    /* vectorizable */
    for (int i_0 = 3; i_0 < 15;i_0 += 1)
    {
        var_22 = var_2;
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 14;i_3 += 1)
                {
                    {
                        var_23 = ((!(((var_3 * (arr_3 [i_1] [i_0]))))));
                        var_24 = (1361558187 << (var_7 - 125));
                    }
                }
            }
        }
        var_25 = ((!(((var_16 << (((arr_3 [i_0] [i_0]) + 104)))))));
    }
    for (int i_4 = 0; i_4 < 19;i_4 += 1)
    {
        arr_10 [i_4] = min((((((arr_9 [i_4]) ? 0 : (arr_9 [1]))) & (arr_8 [i_4]))), (arr_8 [i_4]));
        arr_11 [i_4] = var_16;
        arr_12 [i_4] = (arr_9 [i_4]);
    }
    #pragma endscop
}
