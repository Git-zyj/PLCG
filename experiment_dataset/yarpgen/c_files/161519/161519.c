/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 161519
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=161519 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/161519
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 6;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 8;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 8;i_3 += 1)
                    {
                        {
                            arr_8 [i_3] = (arr_5 [i_3]);
                            var_15 = (max(var_15, ((max((((~(arr_2 [i_1] [i_1])))), (arr_4 [i_2] [i_2]))))));
                            var_16 = (arr_7 [i_2] [i_2 + 2] [i_3] [i_2 + 1] [i_2 - 2] [i_2 - 1]);
                        }
                    }
                }
                var_17 = (max(var_17, (((((arr_3 [i_1 - 1] [i_1 - 1] [i_1 + 1]) ? var_1 : (arr_3 [i_1 - 3] [i_1 - 1] [i_1])))))));
            }
        }
    }
    var_18 = (min(var_3, var_5));
    #pragma endscop
}
