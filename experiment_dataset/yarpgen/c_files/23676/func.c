/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23676
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23676 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23676
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */int) max((((/* implicit */unsigned int) var_16)), (4294967295U)));
    var_18 ^= ((/* implicit */long long int) (!(((/* implicit */_Bool) var_15))));
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (short i_1 = 1; i_1 < 20; i_1 += 2) 
        {
            for (int i_2 = 2; i_2 < 19; i_2 += 4) 
            {
                {
                    arr_7 [i_1] = ((((((/* implicit */unsigned int) ((/* implicit */int) ((signed char) 536870912)))) & (min((((/* implicit */unsigned int) var_1)), (19U))))) != (min((((/* implicit */unsigned int) -536870900)), (19U))));
                    var_19 *= ((/* implicit */unsigned char) max((25U), (((/* implicit */unsigned int) (!(((/* implicit */_Bool) var_15)))))));
                    var_20 = ((/* implicit */_Bool) ((short) var_16));
                }
            } 
        } 
    } 
    var_21 = ((/* implicit */long long int) (+(((/* implicit */int) var_11))));
}
