/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116990
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116990 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116990
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        arr_3 [i_0] = ((var_3 ? (!var_9) : (((((min(var_8, var_2))) != ((max(-13082, 13074))))))));
        var_11 = (((((var_9 >> (57344 - 57334)))) ? ((~(255 ^ -13094))) : (max((~13082), ((max(-13082, 14211)))))));
    }
    var_12 = (min(var_12, var_0));
    var_13 -= ((-(min((((-13082 ? var_6 : var_6))), ((var_1 ? var_1 : var_0))))));
    var_14 = (max(2016, 15));
    #pragma endscop
}
