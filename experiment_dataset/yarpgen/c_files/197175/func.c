/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197175
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197175 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197175
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned long long int) var_7);
    /* LoopSeq 1 */
    for (signed char i_0 = 3; i_0 < 11; i_0 += 2) 
    {
        var_16 = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) (signed char)-39))));
        arr_2 [i_0] = ((/* implicit */_Bool) 4840340879153391249LL);
    }
    var_17 = ((/* implicit */short) 137304735744ULL);
}
