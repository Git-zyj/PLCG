/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158474
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158474 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158474
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13; i_0 += 1) 
    {
        for (int i_1 = 0; i_1 < 13; i_1 += 3) 
        {
            {
                var_17 = (+(min(((+(7340032))), (var_6))));
                var_18 = ((/* implicit */unsigned int) var_1);
                var_19 *= ((/* implicit */unsigned int) ((int) var_10));
            }
        } 
    } 
    var_20 = ((/* implicit */int) min((var_15), (((/* implicit */unsigned int) (~(1073709056))))));
    var_21 &= ((/* implicit */unsigned int) var_1);
    var_22 = ((/* implicit */int) (!(((/* implicit */_Bool) min((var_3), (((((/* implicit */unsigned int) var_4)) * (var_14))))))));
}
