/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110650
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110650 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110650
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_14 = (min(var_14, (arr_2 [i_0])));

        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            arr_6 [i_1] [i_1] = ((~(arr_2 [i_1])));
            arr_7 [i_1] = ((0 ? (arr_2 [i_0]) : 0));
        }
        arr_8 [i_0] [i_0] = var_8;
        arr_9 [i_0] &= 27216;
        arr_10 [i_0] [i_0] = (!1);
    }
    for (int i_2 = 0; i_2 < 13;i_2 += 1)
    {
        arr_13 [i_2] = ((((((arr_12 [i_2]) ? 1 : (arr_12 [i_2])))) || (arr_12 [i_2])));
        var_15 *= 9;
        var_16 = (((max((((!(arr_11 [i_2] [i_2])))), ((3858484176 ? 436483119 : (arr_12 [i_2]))))) >> (-1 + 9)));
    }
    for (int i_3 = 0; i_3 < 13;i_3 += 1)
    {
        var_17 -= (((arr_16 [i_3] [i_3]) << (min(0, ((var_5 >> (2401230051209697431 - 2401230051209697413)))))));
        arr_17 [i_3] = (arr_14 [i_3] [i_3]);
        var_18 = 1;
    }
    var_19 = (!var_3);
    #pragma endscop
}
