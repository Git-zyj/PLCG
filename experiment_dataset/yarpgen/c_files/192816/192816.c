/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 192816
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=192816 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/192816
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 &= 15;

    for (int i_0 = 3; i_0 < 15;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (arr_0 [i_0]);
        var_11 = (max((arr_1 [i_0]), ((~(arr_1 [i_0])))));
        var_12 = -1;
        var_13 = ((var_7 ? var_3 : ((((((arr_1 [i_0]) ? var_7 : (-127 - 1)))) ? var_1 : (arr_1 [i_0])))));
    }
    for (int i_1 = 0; i_1 < 11;i_1 += 1)
    {
        arr_5 [i_1] [i_1] &= 0;
        var_14 = (min(var_14, ((max(-var_0, (1 && -7))))));
        arr_6 [i_1] &= ((-(min(3089616052, (arr_4 [i_1] [i_1])))));
        arr_7 [1] &= (!-2147483647);
        var_15 = (max(var_15, (((!(((max((arr_3 [i_1] [8]), (arr_4 [0] [0]))) < var_2)))))));
    }
    var_16 = var_3;
    #pragma endscop
}
