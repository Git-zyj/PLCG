/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7613
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7613 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7613
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (min(var_17, var_8));
    var_18 = (!307614918);
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            {
                var_19 = (((307614918 ? ((((arr_3 [i_1] [i_1]) ? var_6 : (arr_4 [i_0])))) : (((arr_2 [6]) ? (arr_3 [i_1] [10]) : (arr_0 [8] [i_1]))))));
                arr_5 [i_1] [i_1] = ((~(arr_2 [2])));
                var_20 += ((((((arr_3 [10] [i_1]) ? (((arr_1 [i_0]) ? -2917688013213241094 : var_6)) : ((((arr_1 [i_0]) ? (arr_1 [10]) : var_11)))))) ? -307614918 : ((-8573190282623997626 ? (arr_4 [i_0 + 4]) : (arr_0 [3] [i_1])))));
            }
        }
    }
    var_21 *= var_1;
    #pragma endscop
}
