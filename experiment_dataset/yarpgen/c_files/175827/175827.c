/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175827
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175827 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175827
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((min(var_3, (!var_12))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 10;i_2 += 1)
            {
                {
                    var_18 = (((((1958265481734944241 ? (arr_1 [i_0]) : var_0))) ? 2782710682752028182 : 1610944179270351554));
                    arr_6 [i_2] &= (max(((~(arr_2 [i_2 - 2]))), (((((~(arr_3 [i_1] [4] [i_1])))) ? (((~(arr_5 [0] [i_0] [i_1] [i_0])))) : (-2782710682752028201 & 2085733599)))));
                    var_19 = (min(var_19, (-32767 - 1)));
                }
            }
        }
    }
    #pragma endscop
}
