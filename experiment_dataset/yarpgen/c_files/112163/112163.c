/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112163
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112163 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112163
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            {
                var_12 = ((((((arr_2 [i_0] [i_0]) ? (arr_2 [i_0] [i_0]) : (arr_2 [i_0] [i_0])))) ? (arr_2 [i_0] [i_0]) : ((((arr_2 [i_0] [i_0]) ? (arr_1 [i_1 + 1] [i_1 + 2]) : (arr_3 [i_1 - 2] [i_1 + 3]))))));
                var_13 = ((!(arr_0 [i_0])));
                var_14 |= (((arr_3 [i_1 + 1] [i_1 - 2]) && (!-359554504)));
            }
        }
    }
    var_15 = var_8;
    var_16 = ((65535 ? 1684508510 : var_0));
    var_17 = -1684508510;
    #pragma endscop
}
