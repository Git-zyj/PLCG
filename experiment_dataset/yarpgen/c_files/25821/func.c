/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 25821
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=25821 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/25821
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */long long int) max((var_13), (((/* implicit */long long int) ((((/* implicit */_Bool) var_2)) ? ((-(((/* implicit */int) var_1)))) : (((/* implicit */int) var_0)))))));
    var_14 = ((/* implicit */unsigned int) var_6);
    /* LoopSeq 1 */
    for (int i_0 = 1; i_0 < 11; i_0 += 2) 
    {
        var_15 = 18446744073709551609ULL;
        var_16 = ((/* implicit */long long int) (~(min((((/* implicit */int) var_0)), (((((/* implicit */int) (signed char)-70)) - (((/* implicit */int) var_0))))))));
    }
}
