/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231318
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231318 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231318
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned short i_0 = 2; i_0 < 19; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 21; i_1 += 1) 
        {
            {
                arr_4 [i_0] = ((/* implicit */unsigned long long int) (((-(max((((/* implicit */unsigned long long int) arr_2 [12] [12] [(unsigned char)1])), (arr_0 [i_0] [9U]))))) <= (((/* implicit */unsigned long long int) (+(((/* implicit */int) arr_1 [i_0])))))));
                var_14 = ((/* implicit */int) arr_0 [i_1 + 1] [2LL]);
            }
        } 
    } 
    var_15 = ((/* implicit */unsigned short) -1730044071);
    var_16 = ((/* implicit */unsigned int) 1730044094);
}
