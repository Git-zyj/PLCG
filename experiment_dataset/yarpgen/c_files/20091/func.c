/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20091
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20091 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20091
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned char i_0 = 0; i_0 < 10; i_0 += 4) 
    {
        var_16 = ((/* implicit */unsigned short) min((var_16), (((/* implicit */unsigned short) -30LL))));
        arr_4 [(unsigned short)0] = ((/* implicit */unsigned short) ((signed char) ((unsigned int) var_7)));
    }
    var_17 = ((/* implicit */short) max((var_17), (((/* implicit */short) (~(449593694))))));
    var_18 = (-(var_10));
}
