/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 161563
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=161563 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/161563
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {

                    /* vectorizable */
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        arr_10 [i_0] [i_0] [1] [i_1] [i_3] = 4972732555152502527;
                        var_13 |= 25584;
                    }
                    arr_11 [i_1] [i_1] [i_1] = 65524;
                    /* LoopNest 2 */
                    for (int i_4 = 0; i_4 < 13;i_4 += 1)
                    {
                        for (int i_5 = 4; i_5 < 12;i_5 += 1)
                        {
                            {
                                arr_16 [i_1] [i_1] = 63;
                                arr_17 [1] [i_1] [i_1] [i_1] [1] [11] [i_1] = (max(var_3, (min((min(1048448, var_10)), var_10))));
                                var_14 = (min(((max(var_10, var_9))), -var_9));
                            }
                        }
                    }
                }
            }
        }
    }
    var_15 = 0;
    #pragma endscop
}
