/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164772
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164772 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164772
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (((var_10 + 2147483647) >> (244 - 236)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_1] [i_0] = (min((arr_5 [i_2] [i_1]), 8388607));
                    arr_9 [i_0] [i_1] [i_0] [18] = ((((1 ? (arr_6 [i_1] [i_1]) : (arr_1 [18])))));
                }
            }
        }
    }
    var_15 = ((((-102 > var_9) ? ((-15 ? 2031110349 : 7079660936356982443)) : (((var_12 ? var_4 : 1))))));
    #pragma endscop
}
