/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165818
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165818 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165818
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (var_6 + var_2);

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_20 = (min((((max((arr_2 [i_0]), (arr_2 [i_0]))))), var_11));
        arr_3 [i_0] [i_0] = (min(((((arr_0 [i_0] [i_0]) ? (arr_1 [i_0]) : (arr_2 [i_0])))), (max(var_12, (((-4543038753344346614 + 9223372036854775807) >> (var_14 - 1736014000176823676)))))));

        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            var_21 ^= (var_7 < 133);
            var_22 = (min(var_22, (((var_4 * (!var_1))))));
        }
    }
    var_23 = var_1;
    #pragma endscop
}
