/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103647
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103647 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103647
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((0 ? 800480437 : 4294967295));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_21 = (((((arr_3 [i_0] [8] [i_1]) && (arr_3 [i_0] [i_1] [i_0]))) || ((max(15, (arr_3 [i_0] [i_0] [i_1]))))));
                var_22 = (((var_10 & var_12) ? (var_4 >= var_3) : (((arr_3 [i_0] [5] [i_0]) ? var_2 : var_17))));
            }
        }
    }
    var_23 = (max(var_23, 1));
    var_24 += ((var_5 != (((153 & (min(-1055109694, 76)))))));
    #pragma endscop
}
