/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40481
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40481 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40481
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 4; i_0 < 22;i_0 += 1)
    {
        var_19 = (min(var_19, (arr_1 [i_0] [i_0])));
        arr_4 [i_0] = (max(var_18, ((9671679826899176940 ? (var_1 & var_14) : (arr_1 [i_0] [i_0 - 1])))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 25;i_1 += 1)
    {
        arr_7 [i_1] [i_1] = (arr_6 [i_1]);
        arr_8 [i_1] = (arr_6 [i_1]);
        arr_9 [i_1] = (arr_6 [i_1]);
    }
    var_20 = ((~(min((max(2470044661, -8253313267921068997)), (-32767 - 1)))));
    var_21 = (max(var_21, ((((114688 ? (~2470044661) : (min(2470044673, var_2))))))));
    var_22 = (max(((var_2 ? (var_10 & var_15) : ((max(7, var_5))))), var_13));
    #pragma endscop
}
