/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229768
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229768 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229768
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 ^= ((26729 ? var_5 : var_0));
    var_17 = (max(var_17, (((var_1 ? var_0 : ((~(min(var_8, var_11)))))))));

    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        var_18 = var_4;
        var_19 = (max(var_19, (((((min(538, -1060582884))) ? ((max((arr_0 [12]), (arr_0 [0])))) : (((arr_0 [12]) ? (arr_0 [8]) : (arr_0 [16]))))))));
        arr_2 [i_0] = (arr_1 [i_0]);
    }
    #pragma endscop
}
