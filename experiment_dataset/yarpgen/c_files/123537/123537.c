/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123537
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123537 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123537
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((((var_6 ? var_13 : ((var_4 ? var_1 : var_15))))) ? var_10 : var_16);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 18;i_1 += 1)
        {
            {
                arr_4 [i_0] = (((((+(((arr_0 [i_0] [i_0]) ? var_10 : (arr_3 [i_1])))))) ? ((((arr_0 [i_1 - 2] [i_0]) || var_12))) : 147));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        {
                            arr_10 [i_0] [i_0] = (max(((~(arr_1 [i_1 - 2]))), ((min((arr_1 [i_1 - 2]), (arr_1 [i_1 + 1]))))));
                            var_18 = (max(var_18, 106));
                            var_19 &= -7179622689045140765;
                            var_20 = (max(var_20, 1414094181));
                        }
                    }
                }
                var_21 = var_14;
                var_22 &= (98 - 139);
            }
        }
    }
    var_23 -= var_14;
    #pragma endscop
}
