/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123163
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123163 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123163
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (var_2 ? (((!var_13) | (min(var_8, 21607)))) : (max(((min(var_5, var_13))), (var_9 | var_3))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_0] [i_0] [i_0] &= (arr_4 [i_0]);
                    arr_8 [i_2] = (~var_9);
                }
            }
        }
    }
    var_16 = (max((min(((max(43928, 3033409422))), ((var_3 ? var_6 : var_11)))), var_10));
    var_17 = ((~(min(4294967285, var_9))));
    #pragma endscop
}
