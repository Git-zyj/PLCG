/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40667
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40667 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40667
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_10;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 16;i_2 += 1)
            {
                {
                    var_14 = (arr_3 [3] [i_1] [10]);
                    arr_9 [i_0] [i_0] [i_2 - 2] [i_1] = ((var_11 ? (max((((var_10 + 9223372036854775807) >> (1266340253 - 1266340252))), ((((arr_2 [i_0]) >= (arr_2 [i_0])))))) : var_11));
                }
            }
        }
    }
    var_15 = (((((((max(var_3, var_2))) ? -var_8 : var_11))) ? ((((min(var_6, var_12))) ? var_8 : var_4)) : -1745299807));
    #pragma endscop
}
