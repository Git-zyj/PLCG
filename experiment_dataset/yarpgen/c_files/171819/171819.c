/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171819
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171819 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171819
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_17 = ((0 ? (((arr_2 [i_1]) ? 18446744073709551615 : (var_7 - 1))) : ((((((var_2 ? var_1 : var_0))) ? (18446744073709551615 && var_8) : ((94 ? -1853389036 : (arr_5 [i_1] [i_1] [i_0]))))))));
                arr_6 [i_0] [i_0] = (((((arr_1 [i_1]) % (((arr_4 [i_0] [i_1]) ? (arr_2 [i_0]) : (arr_5 [i_0] [i_0] [i_1])))))));
            }
        }
    }
    var_18 = -var_16;
    #pragma endscop
}
