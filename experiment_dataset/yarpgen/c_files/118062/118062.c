/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 118062
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=118062 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/118062
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 11;i_0 += 1) /* same iter space */
    {
        arr_3 [i_0] = var_13;
        var_17 += (arr_1 [i_0]);
        var_18 += ((+((((((arr_1 [i_0]) + var_3))) ? 2288307293635131262 : (((((arr_0 [i_0] [6]) || var_6))))))));
    }
    for (int i_1 = 0; i_1 < 11;i_1 += 1) /* same iter space */
    {
        var_19 += var_0;
        var_20 = (min(var_20, (arr_4 [i_1])));
        arr_8 [i_1] = var_3;
        arr_9 [i_1] = (arr_4 [i_1]);
        var_21 += (arr_6 [6] [i_1]);
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 19;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 1;i_3 += 1)
        {
            {
                arr_17 [i_2] [i_2] [i_2] = (((arr_12 [i_3]) != (min((((!(arr_11 [i_2])))), ((-(arr_13 [i_2] [i_2])))))));
                var_22 += (min(((-(min((arr_13 [i_2] [12]), (arr_13 [i_2] [12]))))), (((-(!1))))));
                var_23 = ((arr_13 [i_2] [i_2]) || (2288307293635131262 != var_16));
            }
        }
    }
    #pragma endscop
}
