/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184622
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184622 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184622
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        arr_3 [i_0] = ((363536406 ? (arr_2 [i_0]) : ((4080 ? 1846930452 : 4194303))));
        arr_4 [i_0] [i_0] = ((var_5 ? (~var_5) : (((~(arr_0 [i_0]))))));
        var_10 ^= 65535;
    }
    var_11 ^= ((~(0 <= 1)));
    var_12 = (((~128) ? ((-((var_1 ? 1 : var_0)))) : (((((max(var_9, var_7))) ? ((var_8 ? var_4 : 3369621925)) : var_2)))));
    #pragma endscop
}
