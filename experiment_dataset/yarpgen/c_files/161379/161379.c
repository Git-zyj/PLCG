/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 161379
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=161379 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/161379
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_1;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 16;i_2 += 1)
            {
                {
                    var_14 = (max(var_14, ((min((((10874 ? (arr_3 [14] [i_2]) : 3895494093))), (((!(((3895494091 ? (arr_6 [i_0 + 1] [i_1 - 1] [17]) : (-9223372036854775807 - 1))))))))))));
                    arr_8 [1] [i_1] [i_1] [i_0] = 122;
                    arr_9 [i_0] [0] [i_1] = 9223372036854775807;
                }
            }
        }
    }
    #pragma endscop
}
