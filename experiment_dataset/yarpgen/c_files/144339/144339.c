/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144339
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144339 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144339
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (((((1452672464 ? (var_2 == 9223372036854775807) : 1))) ? (((var_9 && ((max(48, 1)))))) : var_14));
    var_19 = var_13;

    for (int i_0 = 0; i_0 < 13;i_0 += 1) /* same iter space */
    {
        arr_3 [i_0] [i_0] = (((!(((3849077036 ? (arr_2 [i_0]) : 12274))))));
        var_20 = (arr_2 [i_0]);
        arr_4 [i_0] [i_0] |= (arr_2 [i_0]);
    }
    for (int i_1 = 0; i_1 < 13;i_1 += 1) /* same iter space */
    {
        var_21 = ((var_8 - (min((min(var_15, 41178)), 6610))));
        var_22 = -1658613079529157633;
    }
    #pragma endscop
}
