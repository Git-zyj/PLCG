/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60987
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60987 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60987
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_7;
    var_12 = 1003331364;
    var_13 = 0;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    arr_6 [i_2] [i_2] [i_1] [i_0] = ((((32767 ? 0 : (((767761586 == (arr_4 [i_0] [i_1])))))) ^ (max((arr_0 [i_0] [i_1]), (((arr_5 [i_2]) ? 0 : (arr_0 [i_0] [i_0])))))));
                    var_14 = ((((0 ? -126 : 13849300442140769163)) ^ ((0 & (~536870911)))));
                }
            }
        }
    }
    var_15 = (min(var_3, ((var_9 ? var_5 : var_1))));
    #pragma endscop
}
