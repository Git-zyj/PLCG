/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51002
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51002 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51002
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 6;i_1 += 1)
        {
            {
                var_17 = ((((((((arr_2 [i_0]) - (arr_1 [8] [i_0]))) | (arr_2 [i_0])))) > 10660072103954583359));
                var_18 = (min(var_18, ((((((((arr_2 [i_1]) + 9223372036854775807)) << (((arr_2 [i_0]) - 22346))))) < ((((((arr_2 [i_0]) ? (arr_2 [i_0]) : (arr_2 [i_0])))) - (arr_2 [i_0 + 1])))))));
            }
        }
    }
    #pragma endscop
}
