/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3001
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3001 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3001
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_5;
    var_16 ^= var_9;
    var_17 = (max(var_17, 2197692000));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_11 [i_0] [i_3] [i_0] [i_0] |= var_2;
                                arr_12 [i_4] [i_3] [i_2] [i_1] [i_0] = ((((!(arr_10 [i_0] [i_0] [i_1] [i_2] [i_0] [i_4]))) && ((!(arr_10 [i_0] [1] [i_2] [i_2] [i_2] [i_3])))));
                                var_18 -= (arr_4 [i_2] [i_2]);
                                var_19 = arr_1 [i_1];
                            }
                        }
                    }
                    arr_13 [i_1] = (((((var_1 & (arr_6 [i_0] [i_1])))) && ((((arr_6 [i_2] [i_2]) ? (arr_6 [i_0] [i_2]) : var_4)))));
                }
            }
        }
    }
    #pragma endscop
}
