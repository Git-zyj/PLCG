/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73844
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73844 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73844
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 ^= (((var_7 || (!-4012843560575455336))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_15 -= (~(((arr_3 [i_1]) ? 43 : (max((arr_1 [2]), var_5)))));
                var_16 = (min(var_16, ((max(var_9, ((((~(arr_1 [4]))))))))));
                var_17 = (arr_2 [i_0] [i_0] [i_1]);
                var_18 = ((~(arr_0 [i_0])));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_19 = (min(var_19, ((((arr_4 [i_3]) + (((max(3957154204, var_10)))))))));
                                var_20 = -4012843560575455317;
                                var_21 = var_2;
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
