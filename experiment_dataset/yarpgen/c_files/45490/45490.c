/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45490
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45490 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45490
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((var_2 || (((-var_6 ? var_4 : var_5)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_12 = (max(var_12, ((((max(((min(1, (arr_0 [i_0] [i_0])))), (arr_7 [10] [10] [10] [i_2] [i_3] [i_2]))) % (arr_8 [4] [1] [4] [i_0]))))));
                                var_13 = (min(4294967295, (((arr_7 [1] [1] [i_1] [i_1] [13] [8]) - ((1 + (arr_3 [i_1] [i_3] [i_4])))))));
                            }
                        }
                    }
                }
                arr_12 [i_0] [i_0] = var_5;
                var_14 = (max(var_14, (arr_9 [i_0] [i_0] [i_0] [i_0] [9])));
            }
        }
    }
    #pragma endscop
}
