/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184667
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184667 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184667
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = 213;
        arr_4 [i_0] = 255;
        /* LoopNest 3 */
        for (int i_1 = 4; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 1;i_3 += 1)
                {
                    {

                        for (int i_4 = 0; i_4 < 14;i_4 += 1)
                        {
                            arr_17 [i_0] [i_0] [i_1 - 4] [i_2] [i_3] [i_3] [i_4] |= ((16 ? (((!(10 & 245)))) : var_18));
                            var_20 |= (max(9, 520093696));
                            arr_18 [i_0] [i_1 - 3] [i_2] [i_3] [i_4] = ((1 ? 245 : 520093696));
                        }
                        for (int i_5 = 0; i_5 < 1;i_5 += 1)
                        {
                            var_21 = (min(var_21, (((-(((0 / var_19) ? -251 : ((var_11 ? 239 : 3774873599)))))))));
                            arr_22 [i_0] [i_1] [i_2] [i_3] [i_5] [i_0] [i_0] |= var_1;
                            var_22 -= (!110);
                            var_23 |= (((var_5 ? 146 : var_17)));
                            arr_23 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0] = var_15;
                        }
                        var_24 = -114;
                    }
                }
            }
        }
        arr_24 [i_0] = ((~(--1)));
        arr_25 [i_0] = (-240 ? var_10 : 0);
    }
    var_25 = 141;
    #pragma endscop
}
