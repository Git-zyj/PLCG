/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241367
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241367 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241367
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 *= ((short) ((unsigned char) var_9));
    /* LoopNest 3 */
    for (long long int i_0 = 0; i_0 < 22; i_0 += 4) 
    {
        for (long long int i_1 = 0; i_1 < 22; i_1 += 1) 
        {
            for (unsigned char i_2 = 4; i_2 < 19; i_2 += 1) 
            {
                {
                    arr_8 [i_2] [i_2] = ((/* implicit */unsigned short) ((((/* implicit */_Bool) ((var_2) << (((((/* implicit */int) (unsigned char)125)) - (82)))))) ? (((/* implicit */unsigned int) ((/* implicit */int) ((signed char) ((unsigned long long int) var_0))))) : (0U)));
                    arr_9 [i_0] [i_1] [i_2] = arr_6 [i_2] [i_1] [(unsigned short)5];
                }
            } 
        } 
    } 
}
