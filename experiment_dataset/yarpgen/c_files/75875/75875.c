/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75875
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75875 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75875
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((var_8 / (((!35172) + (min(var_9, -925227102223844634))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 18;i_1 += 1)
        {
            {
                var_11 = (min(var_11, ((min((min(var_9, ((var_3 ? 1 : var_1)))), (arr_5 [i_0] [i_1 - 1] [i_1]))))));
                var_12 &= (min((-76 <= 2730768988), 1));
            }
        }
    }
    var_13 = ((var_3 >> (var_4 - 759095527)));
    #pragma endscop
}
