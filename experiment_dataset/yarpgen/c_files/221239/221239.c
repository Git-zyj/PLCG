/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221239
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221239 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221239
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        arr_2 [i_0] &= (var_10 ? ((4294967295 ? (arr_1 [i_0] [i_0]) : (arr_0 [i_0]))) : 100);
        var_20 = (~var_14);
        var_21 -= ((((14724740804430451758 ? 27 : -32))) ? ((-(arr_1 [i_0] [i_0]))) : (((!(arr_1 [3] [i_0])))));
        var_22 = (max(var_22, (((!(arr_1 [i_0] [i_0]))))));
        var_23 |= ((!(((14724740804430451762 >> (((arr_1 [i_0] [i_0]) - 1703)))))));
    }
    var_24 = (var_11 / -var_17);
    var_25 &= ((-1 < (((var_8 ? 376964363 : var_12)))));
    var_26 = ((!(((-1 & 161) < var_12))));
    #pragma endscop
}
