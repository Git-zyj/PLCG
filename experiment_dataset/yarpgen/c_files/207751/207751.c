/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 207751
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=207751 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/207751
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(var_16, -32756));

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = 2147483647;

        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            arr_6 [i_0] [i_0] [i_0] = var_10;
            var_17 -= 1;
        }
        for (int i_2 = 3; i_2 < 23;i_2 += 1) /* same iter space */
        {
            var_18 = var_15;

            for (int i_3 = 0; i_3 < 24;i_3 += 1)
            {
                var_19 += 40;
                arr_11 [i_3] [i_2] [i_0] = (arr_9 [i_0] [i_2] [i_3] [16]);
                arr_12 [i_0] [i_2] [i_3] = -1;
            }
            arr_13 [i_2 - 2] [i_2] [i_0] = (arr_9 [i_0] [i_0] [i_2 + 1] [i_2 - 2]);
        }
        for (int i_4 = 3; i_4 < 23;i_4 += 1) /* same iter space */
        {
            var_20 = var_14;
            arr_16 [i_0] [i_0] [i_4 - 1] = 255;
        }
    }
    var_21 = -32758;
    #pragma endscop
}
