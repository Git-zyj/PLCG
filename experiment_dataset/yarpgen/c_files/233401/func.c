/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233401
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233401 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233401
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 1; i_0 < 12; i_0 += 2) 
    {
        var_19 = ((/* implicit */short) ((unsigned short) var_8));
        var_20 = ((/* implicit */unsigned long long int) min((var_20), (((/* implicit */unsigned long long int) ((((var_7) == (((/* implicit */unsigned int) ((/* implicit */int) var_3))))) || (((/* implicit */_Bool) ((signed char) var_2))))))));
    }
    var_21 = ((/* implicit */short) var_17);
    var_22 = ((/* implicit */signed char) var_5);
}
