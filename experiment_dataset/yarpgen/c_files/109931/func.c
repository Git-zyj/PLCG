/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 109931
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=109931 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/109931
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (long long int i_0 = 0; i_0 < 24; i_0 += 1) 
    {
        for (unsigned char i_1 = 1; i_1 < 23; i_1 += 2) 
        {
            for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
            {
                {
                    var_11 = ((/* implicit */long long int) min((var_11), (-754502643348512678LL)));
                    var_12 = arr_6 [i_0] [i_0] [i_0];
                }
            } 
        } 
    } 
    /* LoopNest 2 */
    for (unsigned short i_3 = 0; i_3 < 16; i_3 += 4) 
    {
        for (long long int i_4 = 0; i_4 < 16; i_4 += 1) 
        {
            {
                /* LoopSeq 1 */
                for (_Bool i_5 = 0; i_5 < 1; i_5 += 1) 
                {
                    var_13 &= ((/* implicit */long long int) ((((/* implicit */int) max(((unsigned short)62006), (((/* implicit */unsigned short) (signed char)-9))))) & (((/* implicit */int) (unsigned short)59811))));
                    var_14 ^= ((/* implicit */unsigned long long int) arr_12 [i_3]);
                }
                var_15 ^= (!(((/* implicit */_Bool) (~(((/* implicit */int) arr_10 [i_3]))))));
            }
        } 
    } 
    var_16 = ((/* implicit */unsigned char) var_5);
}
