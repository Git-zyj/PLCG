/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82794
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82794 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82794
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {

                    for (int i_3 = 2; i_3 < 9;i_3 += 1)
                    {

                        for (int i_4 = 2; i_4 < 9;i_4 += 1)
                        {
                            var_11 = (((var_4 - var_9) != var_9));
                            arr_13 [i_0] [i_0] [i_0] [i_2] [i_0] [i_2] [i_0] = ((((var_1 | var_9) != var_2)));
                            var_12 -= (var_4 != var_3);
                        }
                        arr_14 [i_1] &= ((16388083985505136339 != ((((6 ^ var_3) | 1047232304)))));

                        /* vectorizable */
                        for (int i_5 = 0; i_5 < 12;i_5 += 1)
                        {
                            arr_19 [i_1] [i_1] [i_1] [i_3] [i_1] [i_2] = (var_2 != var_9);
                            var_13 = (max(var_13, var_9));
                        }
                    }
                    var_14 = var_6;
                }
                arr_20 [i_0] [i_1] [i_0] = var_3;
                arr_21 [i_0] [i_0] [i_0] = ((var_4 | (52597 | 0)));
            }
        }
    }
    var_15 = (var_2 != var_5);
    #pragma endscop
}
