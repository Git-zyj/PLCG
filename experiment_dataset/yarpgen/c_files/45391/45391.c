/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45391
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45391 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45391
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 ^= var_10;
    var_16 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_1] = var_8;
                arr_6 [i_0] = var_12;
                var_17 = (min(var_17, var_6));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 24;i_4 += 1)
                        {
                            {
                                var_18 *= ((((max(var_11, var_11))) ? (var_9 & var_8) : var_5));
                                var_19 *= (max(((var_10 ? var_5 : var_8)), ((var_10 ? var_3 : var_6))));
                            }
                        }
                    }
                }
                var_20 = (-((min(var_5, (max(var_10, var_5))))));
            }
        }
    }
    var_21 = (min(var_21, (((((min((((var_13 ? 1 : 7))), (max(var_11, var_0))))) ? ((min((!var_3), ((var_3 ? var_8 : var_13))))) : (((max(var_11, 0)) | (~var_3))))))));
    #pragma endscop
}
