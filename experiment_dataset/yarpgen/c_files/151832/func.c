/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151832
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151832 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151832
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */short) var_9);
    /* LoopSeq 2 */
    for (short i_0 = 0; i_0 < 23; i_0 += 2) /* same iter space */
    {
        var_19 = ((/* implicit */unsigned short) (-(((((/* implicit */_Bool) var_7)) ? (var_14) : (((/* implicit */int) var_12))))));
        arr_2 [i_0] = ((/* implicit */signed char) var_17);
    }
    /* vectorizable */
    for (short i_1 = 0; i_1 < 23; i_1 += 2) /* same iter space */
    {
        var_20 = ((/* implicit */unsigned short) (~(((/* implicit */int) var_12))));
        var_21 = ((/* implicit */_Bool) ((((/* implicit */_Bool) var_12)) ? (((/* implicit */unsigned long long int) ((((/* implicit */_Bool) var_10)) ? (var_14) : (((/* implicit */int) var_10))))) : (((((/* implicit */_Bool) var_4)) ? (var_8) : (((/* implicit */unsigned long long int) ((/* implicit */int) var_15)))))));
        var_22 -= ((/* implicit */unsigned short) (!(((/* implicit */_Bool) var_10))));
    }
    var_23 = min((((/* implicit */unsigned short) ((short) (~(((/* implicit */int) (unsigned short)28964)))))), (var_2));
}
