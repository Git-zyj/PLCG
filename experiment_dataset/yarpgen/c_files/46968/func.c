/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46968
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46968 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46968
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */short) var_2);
    var_18 = ((/* implicit */unsigned int) var_6);
    var_19 = ((/* implicit */unsigned int) var_16);
    var_20 = ((/* implicit */long long int) var_7);
    /* LoopSeq 1 */
    for (unsigned int i_0 = 3; i_0 < 20; i_0 += 4) 
    {
        var_21 = ((/* implicit */short) (unsigned short)13974);
        var_22 = ((/* implicit */unsigned short) 3544777483955118301LL);
        var_23 = ((/* implicit */unsigned char) min((var_23), (((/* implicit */unsigned char) (unsigned short)43456))));
    }
}
