/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 53856
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=53856 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/53856
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */signed char) ((4294967288U) <= (18U)));
    /* LoopSeq 1 */
    for (int i_0 = 2; i_0 < 12; i_0 += 3) 
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 15; i_1 += 1) 
        {
            for (short i_2 = 0; i_2 < 15; i_2 += 1) 
            {
                for (long long int i_3 = 0; i_3 < 15; i_3 += 2) 
                {
                    {
                        arr_11 [i_0] [i_2] [i_2] [i_3] [i_0 - 1] = min((((/* implicit */unsigned int) arr_3 [i_0] [i_0 + 2])), (4294967288U));
                        var_16 = ((/* implicit */unsigned short) min((((18014396361998336LL) / (((/* implicit */long long int) ((/* implicit */int) (short)27522))))), (((/* implicit */long long int) (-(((/* implicit */int) (unsigned char)21)))))));
                        var_17 = ((/* implicit */int) var_11);
                    }
                } 
            } 
        } 
        /* LoopNest 2 */
        for (signed char i_4 = 0; i_4 < 15; i_4 += 1) 
        {
            for (unsigned int i_5 = 1; i_5 < 13; i_5 += 2) 
            {
                {
                    var_18 = ((/* implicit */unsigned short) var_13);
                    var_19 = ((/* implicit */unsigned long long int) max((4294967291U), (10U)));
                }
            } 
        } 
    }
    /* LoopNest 2 */
    for (unsigned int i_6 = 0; i_6 < 17; i_6 += 4) 
    {
        for (unsigned char i_7 = 4; i_7 < 16; i_7 += 3) 
        {
            {
                var_20 = ((/* implicit */long long int) (+(1U)));
                var_21 = ((/* implicit */long long int) min(((-(arr_19 [i_7 + 1]))), ((-(arr_19 [i_7 - 2])))));
                var_22 *= ((/* implicit */unsigned long long int) (+(4294967289U)));
            }
        } 
    } 
    var_23 = ((/* implicit */int) var_10);
}
