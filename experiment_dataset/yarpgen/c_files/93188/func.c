/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93188
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93188 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93188
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */signed char) (+(var_12)));
    var_19 = ((/* implicit */signed char) ((int) 680807817U));
    /* LoopSeq 1 */
    for (long long int i_0 = 3; i_0 < 9; i_0 += 4) 
    {
        arr_3 [i_0] = -1110114994;
        arr_4 [i_0] [(signed char)8] |= ((signed char) (-(((/* implicit */int) arr_0 [i_0 + 1]))));
        arr_5 [i_0] = ((/* implicit */_Bool) 15811629556498815226ULL);
    }
}
