/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165551
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165551 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165551
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (max((min(((var_6 ? var_9 : var_10)), ((max(var_1, var_10))))), ((((min(var_6, var_4)) <= var_8)))));
    var_12 = var_10;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 23;i_1 += 1)
        {
            {
                var_13 |= (((max((max(var_6, var_10)), (var_7 || var_8))) >> ((((max(var_0, var_7))) - 61135))));
                var_14 = (max(var_14, ((((max(var_5, var_9)) ^ ((var_4 ? var_4 : var_3)))))));
                arr_5 [i_1 + 1] = var_4;
            }
        }
    }
    #pragma endscop
}
