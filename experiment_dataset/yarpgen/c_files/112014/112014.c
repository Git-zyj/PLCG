/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112014
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112014 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112014
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] [6] [i_1] = ((((((arr_4 [i_0] [i_0]) << (((arr_3 [i_0] [19] [i_1]) - 11127479292934402211))))) || (((((arr_7 [1] [i_1] [i_1] [i_2]) && (arr_3 [i_0] [1] [i_2])))))));
                    var_11 = (arr_6 [20] [18] [18] [i_2]);
                    arr_9 [i_0] [i_0] [18] = (arr_2 [i_0] [i_0] [11]);
                }
            }
        }
    }
    var_12 = min(var_8, ((((min(var_3, var_2))) + (min(25547, (-9223372036854775807 - 1))))));
    var_13 = ((min(72, 0)));
    var_14 = (max(var_14, var_3));
    #pragma endscop
}
