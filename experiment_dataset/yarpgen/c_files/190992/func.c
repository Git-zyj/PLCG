/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190992
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190992 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190992
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = var_14;
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned short i_0 = 0; i_0 < 11; i_0 += 3) 
    {
        var_17 = ((/* implicit */_Bool) ((((/* implicit */int) arr_1 [i_0])) + (((/* implicit */int) arr_1 [i_0]))));
        var_18 |= ((/* implicit */int) arr_0 [i_0] [i_0]);
        var_19 = ((((((/* implicit */_Bool) var_2)) ? (arr_0 [i_0] [i_0]) : (arr_0 [i_0] [i_0]))) < (arr_0 [i_0] [i_0]));
    }
    var_20 = ((/* implicit */unsigned short) (+(min((((/* implicit */int) var_6)), ((-(((/* implicit */int) var_2))))))));
}
