/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70613
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70613 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70613
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(177953343420468811, (((var_8 ? var_16 : var_13)))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_18 ^= var_15;
        arr_2 [i_0] [i_0] = ((var_12 ? ((var_15 ? (arr_1 [i_0] [i_0]) : var_5)) : (arr_0 [i_0])));
        arr_3 [i_0] = var_1;
    }
    for (int i_1 = 0; i_1 < 23;i_1 += 1)
    {
        var_19 = ((!(arr_5 [i_1])));
        arr_6 [i_1] &= ((((~var_11) ? var_2 : var_13)));

        for (int i_2 = 0; i_2 < 23;i_2 += 1)
        {
            arr_10 [i_1] = var_7;
            arr_11 [i_1] [i_2] [i_1] = (max(var_6, (arr_9 [i_1] [i_2] [i_2])));
            arr_12 [i_2] [i_2] = var_9;
            var_20 = (min(var_20, ((max(((var_16 ? var_3 : var_5)), (arr_0 [i_1]))))));
        }
    }
    #pragma endscop
}
