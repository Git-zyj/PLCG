/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66417
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66417 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66417
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */short) max((var_14), (((/* implicit */short) var_5))));
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned short i_1 = 2; i_1 < 13; i_1 += 3) 
        {
            {
                var_15 = ((/* implicit */unsigned int) max((var_15), (((/* implicit */unsigned int) (((!(((/* implicit */_Bool) 3816186981684114225ULL)))) ? (((/* implicit */int) ((unsigned char) (!(((/* implicit */_Bool) var_8)))))) : (((/* implicit */int) (!(((/* implicit */_Bool) var_2))))))))));
                arr_6 [i_0] [i_0] = ((/* implicit */int) (unsigned char)236);
            }
        } 
    } 
    var_16 -= ((/* implicit */_Bool) var_12);
}
