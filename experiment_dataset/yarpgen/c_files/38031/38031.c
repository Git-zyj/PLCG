/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38031
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38031 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38031
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = 7781093294518042833;
    var_13 = ((((max(((2089115202 ? var_2 : 31)), 13399))) ? (((var_6 ? ((min(var_9, var_10))) : var_10))) : (var_0 ^ 1052120168142232891)));
    var_14 = -1943927198;

    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        arr_2 [9] |= (arr_1 [i_0 - 2]);
        arr_3 [i_0] = ((((arr_0 [i_0] [i_0 - 1]) ? 52136 : var_0)));
    }
    #pragma endscop
}
