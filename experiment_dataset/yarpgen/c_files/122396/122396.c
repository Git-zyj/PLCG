/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122396
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122396 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122396
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_1] = (((((((((arr_4 [i_1 - 1]) ? (arr_1 [i_0] [i_0]) : (arr_3 [i_1])))) ? var_3 : (arr_0 [13] [i_1])))) < ((1 + (arr_1 [i_1 - 2] [i_1])))));
                arr_6 [i_0] = ((((((((var_5 ? var_12 : var_16))) ? (~18394002303980937185) : ((max(1184041150, -69944763334504868)))))) ? 1184041166 : (arr_4 [i_1 - 2])));
            }
        }
    }
    var_19 = var_1;
    var_20 = (min(var_20, var_9));
    #pragma endscop
}
