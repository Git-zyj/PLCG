/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67774
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67774 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67774
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_10;

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 14;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 15;i_3 += 1)
                {
                    {
                        var_20 *= (~3953452534274791539);
                        arr_10 [i_0] [i_1] [i_0] [i_3] = (((arr_4 [i_3 + 2] [i_1] [i_2 + 2]) ? (arr_4 [i_3 + 2] [i_1] [i_2 + 2]) : 566));
                    }
                }
            }
        }
        var_21 = (max(var_21, (arr_2 [i_0] [i_0])));
        var_22 *= (arr_4 [i_0] [i_0] [i_0]);
        arr_11 [i_0] = (((arr_3 [1] [i_0] [i_0]) ? (arr_7 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0]) : (arr_4 [i_0] [6] [i_0])));
        arr_12 [i_0] [i_0] = (((((((0 ? var_4 : -574) + 2147483647)) << (((arr_0 [i_0]) - 146))))));
    }
    var_23 *= var_3;
    var_24 = var_6;
    #pragma endscop
}
