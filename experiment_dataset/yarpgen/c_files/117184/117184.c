/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117184
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117184 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117184
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = (arr_1 [i_0]);
        var_15 = (min(var_15, ((((30494 || 41) % (arr_1 [i_0]))))));

        /* vectorizable */
        for (int i_1 = 2; i_1 < 16;i_1 += 1)
        {
            var_16 = (arr_4 [i_1 + 1] [i_1 - 1]);
            arr_6 [i_0] [i_0] [i_1 - 1] = (((arr_2 [i_0]) | 1));
            arr_7 [8] = ((30579 & (arr_0 [i_0] [i_1 + 1])));
            arr_8 [i_0] [i_1] [i_1] = (arr_5 [i_0] [i_0] [i_1]);
        }
        var_17 ^= (arr_4 [i_0] [15]);
    }
    /* vectorizable */
    for (int i_2 = 4; i_2 < 17;i_2 += 1)
    {
        var_18 = (((arr_9 [i_2 - 1]) + (arr_9 [i_2 + 1])));
        var_19 = (((arr_10 [i_2 - 4]) == (arr_10 [i_2 - 4])));
        var_20 ^= (arr_9 [i_2 + 1]);
    }
    var_21 = var_5;
    #pragma endscop
}
