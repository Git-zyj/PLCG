/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106654
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106654 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106654
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 22; i_0 += 4) 
    {
        arr_2 [i_0] = ((/* implicit */_Bool) arr_1 [i_0]);
        var_17 = ((/* implicit */unsigned long long int) ((3045160301U) % (1249806994U)));
    }
    var_18 = ((/* implicit */_Bool) ((((/* implicit */_Bool) max((((/* implicit */unsigned short) var_14)), (var_2)))) ? (max(((-(var_15))), (((/* implicit */long long int) ((-1787211195) / (728925270)))))) : (((/* implicit */long long int) var_10))));
    var_19 = ((/* implicit */unsigned short) var_15);
    var_20 = ((/* implicit */unsigned long long int) 2639420001U);
}
