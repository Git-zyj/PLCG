/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 140413
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=140413 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/140413
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 |= var_2;
    var_20 = (max(var_20, ((min((((((var_1 ? var_7 : 1))) ? (((var_10 ? 1 : var_13))) : (var_2 ^ var_16))), ((min(20257, ((3292563773 ? var_6 : 1))))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_21 |= var_12;
                var_22 ^= (((arr_5 [5] [i_1] [i_0]) - (arr_3 [1] [i_0])));
                var_23 = (((arr_5 [i_0] [i_1] [i_0]) / (((((max(1, 0))) == ((max(0, 1))))))));
            }
        }
    }
    #pragma endscop
}
