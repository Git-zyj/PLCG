/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179649
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179649 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179649
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((-((var_9 ? var_9 : (((min(var_7, var_7))))))));
    var_11 = (min((var_6 >= 14510380680978929002), var_2));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_12 = (max(-424187610, -1712387412));
                    arr_9 [i_1] [i_0] = (~((((!(arr_0 [i_2]))) ? ((240552284 ? 2118989826 : -424187610)) : -240552284)));
                }
            }
        }
    }
    #pragma endscop
}
