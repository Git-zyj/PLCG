/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177138
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177138 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177138
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((var_4 <= ((((var_12 <= var_9) % (min(var_6, var_13)))))));
    var_15 = var_9;
    var_16 = (((((var_11 <= ((var_12 ? var_0 : var_5))))) % ((((var_13 ? var_11 : var_3))))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] = var_13;
                var_17 = ((((((var_11 ? var_4 : var_7))) && var_7)));
            }
        }
    }
    var_18 = min((((max(var_13, var_1)))), ((min(var_10, var_2))));
    #pragma endscop
}
