/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136360
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136360 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136360
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_16 = (min((!var_7), (((arr_7 [i_1 - 1] [i_1] [i_1] [i_2]) ? var_15 : var_3))));
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_17 = (((((((max(-32759, (arr_2 [0] [i_0])))) ? 18446744073709551615 : 63))) ? (max((arr_11 [i_1 - 1] [i_1 - 1]), 4294967295)) : (((((~var_4) || 35291))))));
                                arr_13 [i_2] [9] [i_3] [i_2] [i_1] [i_1] [i_0] = var_12;
                            }
                        }
                    }
                }
            }
        }
    }
    var_18 = (!var_12);
    var_19 = (min(var_6, (!var_0)));
    #pragma endscop
}
