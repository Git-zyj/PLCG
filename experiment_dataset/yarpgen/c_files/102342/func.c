/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102342
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102342 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102342
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */short) var_13);
    var_15 -= ((/* implicit */unsigned short) (-(((/* implicit */int) (unsigned char)255))));
    var_16 = ((((((/* implicit */_Bool) min((var_11), (((/* implicit */long long int) (_Bool)1))))) ? (var_6) : (min((var_0), (var_6))))) | (var_4));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned char i_0 = 0; i_0 < 24; i_0 += 4) 
    {
        arr_2 [i_0] = ((/* implicit */_Bool) arr_1 [i_0] [i_0]);
        var_17 = ((/* implicit */unsigned char) ((var_6) + (((/* implicit */unsigned int) ((((/* implicit */_Bool) arr_1 [i_0] [i_0])) ? (((/* implicit */int) var_8)) : (((/* implicit */int) (signed char)32)))))));
        var_18 -= ((/* implicit */unsigned short) (!(((/* implicit */_Bool) var_1))));
        arr_3 [i_0] = ((/* implicit */int) (!(((((/* implicit */_Bool) var_9)) || (((/* implicit */_Bool) var_4))))));
        var_19 = var_10;
    }
    var_20 = ((/* implicit */unsigned short) var_12);
}
