/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49517
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49517 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49517
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max(((min((((var_7 ? 79 : var_8))), var_13))), (((var_3 & var_13) ? var_11 : var_3))));
    var_16 = (((((var_3 << (var_2 - 1603114007)) & var_1))));
    var_17 = var_12;
    var_18 -= (((max((~var_1), var_8)) * (((var_2 * var_6) ? var_10 : (var_5 <= var_6)))));

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_19 = ((~(((arr_1 [i_0]) ? (arr_1 [i_0]) : (arr_1 [i_0])))));
        var_20 = ((+(((arr_2 [i_0]) / (max(6970704388216151503, var_10))))));
    }
    #pragma endscop
}
