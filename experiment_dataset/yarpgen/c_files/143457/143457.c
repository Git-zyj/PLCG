/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143457
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143457 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143457
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_4 [1] = ((((min((arr_3 [i_0]), 1))) * var_12));
        var_19 = (min(var_19, 15));
        var_20 *= (arr_1 [i_0] [i_0]);
        var_21 = (min(var_21, 128));
    }
    for (int i_1 = 0; i_1 < 18;i_1 += 1)
    {
        /* LoopNest 3 */
        for (int i_2 = 1; i_2 < 14;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 18;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 18;i_4 += 1)
                {
                    {
                        var_22 = 1879048192;
                        arr_16 [i_1] [14] [i_2] [i_2] [i_3] [i_4] = var_8;
                        var_23 = (max(var_23, (~var_3)));
                    }
                }
            }
        }
        var_24 = (((arr_2 [i_1]) / var_9));
        var_25 = (max(var_25, (arr_8 [i_1] [i_1])));
    }
    var_26 = (max(var_26, (((-3297157431272998550 * ((10028081162311807568 | (-32767 - 1))))))));
    #pragma endscop
}
