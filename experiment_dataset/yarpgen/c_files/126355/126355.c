/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 126355
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=126355 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/126355
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_14 = (((arr_1 [i_0]) && ((max((((!(arr_0 [i_0] [i_1])))), var_0)))));
                var_15 = (max((min((((36028797018931200 < (arr_0 [i_0 - 2] [i_0 - 2])))), (min(var_2, (arr_0 [i_0] [i_1]))))), ((-18006 ? (arr_0 [i_0 - 1] [i_0 - 2]) : var_9))));
                var_16 ^= (min((((arr_4 [i_0 - 2] [i_0 - 1]) ? (arr_4 [i_0 - 2] [i_0 - 2]) : var_11)), (min(-49987947, var_5))));
            }
        }
    }
    var_17 = 55999;
    #pragma endscop
}
