/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220807
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220807 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220807
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_6 [i_0] [11] = ((+(((arr_2 [i_0 + 1] [i_0 + 1]) ? var_7 : (arr_5 [i_0 - 1] [i_0 - 1])))));
                var_12 = ((((min((arr_2 [i_0] [i_0 - 1]), var_5))) ? (((arr_5 [i_1] [i_0]) ? 483 : 0)) : ((var_10 >> ((((arr_5 [i_1] [1]) == var_11)))))));
                var_13 = (max(var_13, (arr_0 [3])));
                var_14 = (max(var_14, ((max(var_8, ((min((arr_4 [i_1] [i_0 - 1]), (arr_0 [i_0 + 1])))))))));
            }
        }
    }
    var_15 = var_5;
    #pragma endscop
}
