/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206505
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206505 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206505
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_6;
    var_20 ^= ((var_10 || (var_3 < var_5)));

    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        arr_3 [i_0 + 1] = ((((arr_0 [i_0 - 1]) != (arr_2 [i_0]))));
        arr_4 [i_0 + 1] = (arr_2 [i_0]);
        var_21 = (min(var_21, ((((arr_1 [i_0] [i_0]) > (arr_0 [i_0]))))));
    }
    for (int i_1 = 0; i_1 < 12;i_1 += 1) /* same iter space */
    {
        arr_7 [4] = (arr_1 [10] [i_1]);
        var_22 = ((((((((arr_6 [i_1] [i_1]) < (arr_0 [i_1])))) != (((arr_6 [i_1] [i_1]) << (((arr_0 [i_1]) + 123)))))) && (arr_6 [i_1] [1])));
        arr_8 [i_1] = (((((arr_1 [i_1] [i_1]) & (arr_2 [i_1])))));
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 12;i_2 += 1) /* same iter space */
    {
        arr_12 [i_2] = (arr_11 [1]);
        arr_13 [i_2] = (arr_0 [i_2]);
        var_23 = (arr_6 [i_2] [i_2]);
    }
    #pragma endscop
}
