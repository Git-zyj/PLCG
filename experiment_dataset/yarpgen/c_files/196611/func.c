/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196611
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196611 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196611
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 -= ((/* implicit */signed char) ((((/* implicit */int) (_Bool)0)) * (((/* implicit */int) (short)-16910))));
    var_13 -= ((/* implicit */_Bool) var_0);
    /* LoopNest 2 */
    for (unsigned char i_0 = 0; i_0 < 22; i_0 += 3) 
    {
        for (signed char i_1 = 0; i_1 < 22; i_1 += 2) 
        {
            {
                var_14 -= ((/* implicit */long long int) (+(((/* implicit */int) (short)-16937))));
                arr_7 [i_0] [i_0] = ((/* implicit */signed char) (+(((((/* implicit */unsigned int) ((/* implicit */int) var_2))) * (var_7)))));
            }
        } 
    } 
}
