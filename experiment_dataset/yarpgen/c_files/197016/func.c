/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197016
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197016 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197016
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned int) (+(var_10)));
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 1; i_0 < 14; i_0 += 2) 
    {
        for (int i_1 = 1; i_1 < 11; i_1 += 4) 
        {
            for (signed char i_2 = 0; i_2 < 15; i_2 += 2) 
            {
                {
                    var_17 = ((/* implicit */unsigned char) max((((/* implicit */unsigned int) (~(((/* implicit */int) (unsigned char)65))))), (var_3)));
                    arr_7 [i_0] = ((/* implicit */signed char) ((((unsigned int) (signed char)103)) / ((-(var_5)))));
                }
            } 
        } 
    } 
    var_18 = ((/* implicit */_Bool) (-((-(((/* implicit */int) var_13))))));
}
