/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 9422
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=9422 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/9422
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = 1297097276738546356;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_15 = (max(var_15, 0));
                var_16 = (min(var_16, (((-((12 ? (arr_0 [i_0]) : 1)))))));
                var_17 += (((arr_0 [17]) ? -1 : (((arr_4 [i_0] [i_0]) ? (arr_5 [i_0]) : (!0)))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 23;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 23;i_3 += 1)
        {
            {
                var_18 = (arr_11 [i_2] [i_2] [i_2]);
                var_19 -= 292714542;
            }
        }
    }
    var_20 = (max(1, 3730182469533648644));
    #pragma endscop
}
