/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48584
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48584 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48584
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (min((((((min(220, var_16))) || (10165861858078871572 && var_6)))), ((-(max(var_8, var_4))))));

    /* vectorizable */
    for (int i_0 = 4; i_0 < 13;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] [i_0] = (((var_1 + 9223372036854775807) << ((((20 << (var_6 + 26262))) - 83886080))));
        var_18 = var_10;
    }
    /* vectorizable */
    for (int i_1 = 4; i_1 < 13;i_1 += 1) /* same iter space */
    {
        arr_5 [1] = var_9;
        var_19 = (max(var_19, var_9));
        arr_6 [5] = ((-var_15 / (((arr_1 [i_1]) ? var_2 : var_6))));
    }
    var_20 = ((var_5 ? (!var_2) : (var_14 + var_13)));
    #pragma endscop
}
