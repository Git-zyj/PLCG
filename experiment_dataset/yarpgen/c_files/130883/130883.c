/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130883
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130883 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130883
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_15;
    var_20 = (((((var_17 ? var_16 : (var_5 | var_8)))) ? ((var_17 ? (min(var_10, var_12)) : var_15)) : var_12));
    var_21 = ((var_8 ? (64 * 64) : 9223372036854775797));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] [i_0] [i_2] = (min(7884109612764162529, ((min(0, -1591066968)))));
                    var_22 = (arr_4 [2]);
                }
            }
        }
    }
    var_23 = (max((var_2 | var_7), var_17));
    #pragma endscop
}
