/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 39546
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=39546 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/39546
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 10;i_2 += 1)
            {
                {
                    var_12 -= (arr_0 [i_0]);
                    var_13 = ((!(((((~(arr_4 [i_0] [i_1] [i_0])))) || (arr_7 [i_0] [i_2])))));

                    for (int i_3 = 0; i_3 < 0;i_3 += 1)
                    {
                        var_14 -= ((!(arr_3 [i_2 + 1] [i_0] [i_3 + 1])));
                        var_15 *= (arr_5 [1] [i_1] [i_1]);
                        var_16 -= ((!((min(-10725711619748231001, (arr_3 [i_3 + 1] [i_0] [i_0]))))));
                        arr_11 [i_0] [2] [i_1] [i_3] = var_10;
                    }
                    var_17 = max((arr_9 [i_2 + 2] [i_1] [i_2 - 2] [i_2 - 1] [i_2 + 3] [i_2 + 3]), var_11);
                }
            }
        }
    }
    var_18 = var_11;
    #pragma endscop
}
