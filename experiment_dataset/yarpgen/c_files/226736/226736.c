/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226736
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226736 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226736
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((((min((~var_0), var_10))) & ((var_0 ? (((-32767 - 1) ? -32745 : var_8)) : var_3))));

    for (int i_0 = 2; i_0 < 11;i_0 += 1) /* same iter space */
    {
        arr_3 [i_0 - 2] = var_1;
        arr_4 [1] = (((~32592) ? 3167364194 : 3167364194));
        /* LoopNest 2 */
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    var_16 = ((~((~(~var_14)))));
                    arr_9 [i_0] [i_1 + 2] [i_1] = ((~(((arr_5 [i_0 - 1]) ? var_0 : (arr_5 [i_0 - 2])))));
                    arr_10 [i_1] = 1;
                    var_17 = (min((max((arr_2 [i_0]), var_4)), (arr_8 [i_0] [1])));
                }
            }
        }
    }
    for (int i_3 = 2; i_3 < 11;i_3 += 1) /* same iter space */
    {
        var_18 = (arr_1 [i_3] [i_3]);
        var_19 = var_10;
        arr_13 [i_3] = (((arr_12 [i_3]) ? (((((((arr_12 [1]) / var_3))) ? (arr_12 [i_3 - 1]) : var_5))) : (max(((((arr_11 [2]) <= (arr_5 [i_3 - 2])))), (arr_2 [4])))));
    }
    var_20 = (((~1) ? var_8 : var_0));
    #pragma endscop
}
