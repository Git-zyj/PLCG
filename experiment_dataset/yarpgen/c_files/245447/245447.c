/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 245447
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=245447 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/245447
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 -= (((var_5 & ((var_1 ? var_3 : 1400412660784795492)))));
    var_17 = (((var_7 * 255) - (((((var_5 ? var_11 : var_10))) ? var_14 : ((var_8 ? var_8 : 18446744073709551615))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 9;i_2 += 1)
            {
                {
                    var_18 += (arr_5 [i_2] [i_2 + 1] [i_2 + 1]);
                    arr_9 [i_0] [i_0] [i_0] = (((235 - 126) * (((219 > (var_11 < 7))))));
                }
            }
        }
    }
    #pragma endscop
}
