/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208835
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208835 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208835
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            {
                arr_4 [i_0] = ((((((arr_2 [i_0] [i_0]) ? (arr_2 [i_0] [i_0]) : var_17))) ? (arr_2 [i_0] [i_0]) : ((~((~(arr_1 [i_0] [i_1])))))));
                var_20 -= (arr_1 [6] [i_1 + 3]);
            }
        }
    }
    var_21 = (min(var_21, (((((max(((46 ? (-2147483647 - 1) : var_13)), ((2147483647 ? var_5 : var_13))))) ? (min((var_2 && var_16), (max(var_0, var_17)))) : var_10)))));
    #pragma endscop
}
