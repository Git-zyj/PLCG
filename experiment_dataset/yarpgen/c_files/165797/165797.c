/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165797
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165797 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165797
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [4] = ((~(((arr_2 [i_0]) ? (arr_2 [i_0]) : (arr_2 [i_1])))));
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        {
                            var_18 = ((+((((((arr_2 [i_0]) ? (arr_3 [i_0] [2] [i_2 - 2]) : (arr_10 [i_0] [i_0] [i_0] [0])))) ? (((!(arr_1 [i_0])))) : (min((arr_1 [i_3]), (arr_2 [i_0])))))));
                            var_19 &= (arr_1 [i_1]);
                        }
                    }
                }
            }
        }
    }
    var_20 = (min(var_20, var_13));
    var_21 |= (((((((var_1 && var_10) >= var_8)))) | var_11));
    #pragma endscop
}
