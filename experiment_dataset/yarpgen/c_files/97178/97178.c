/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97178
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97178 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97178
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_20 &= (arr_1 [i_0]);

        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            /* LoopNest 3 */
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 11;i_3 += 1)
                {
                    for (int i_4 = 1; i_4 < 12;i_4 += 1)
                    {
                        {
                            var_21 = (min((arr_11 [i_3] [i_3 - 1] [i_3 + 2] [i_3] [i_3]), (((((arr_11 [i_3] [i_3 - 1] [i_3 + 2] [i_3] [i_3]) + 2147483647)) >> var_4))));
                            var_22 = var_15;
                        }
                    }
                }
            }
            var_23 = (min(var_23, (arr_4 [i_0])));
        }
        var_24 = (((((max(var_11, var_3))) ? ((((arr_0 [i_0] [i_0]) == var_2))) : (var_19 >= var_13))));
        var_25 = var_1;
    }

    for (int i_5 = 2; i_5 < 13;i_5 += 1)
    {
        var_26 = (((65535 ? -var_17 : (((arr_13 [i_5]) ? var_13 : var_13)))));

        for (int i_6 = 0; i_6 < 0;i_6 += 1)
        {
            var_27 &= ((!(((2 ? 159 : 17088685130380903307)))));
            var_28 = (max(var_28, (1 && 1)));
        }
    }
    #pragma endscop
}
