/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63084
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63084 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63084
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            {
                var_12 = var_6;
                arr_6 [i_0] [i_0] [i_1] = ((~(arr_2 [i_0] [i_1 - 1])));
                var_13 = var_10;
                arr_7 [i_0] [i_0] = var_1;
            }
        }
    }

    for (int i_2 = 3; i_2 < 19;i_2 += 1)
    {
        var_14 = ((-(((arr_9 [10]) ? (!var_2) : (((var_11 > (arr_8 [i_2] [1]))))))));
        arr_10 [i_2] [i_2 - 3] = (((((var_6 % (~var_9)))) ? (var_0 << var_10) : (arr_8 [i_2 + 1] [i_2 + 1])));
        arr_11 [i_2] = (((!var_5) ? ((((((-(arr_9 [i_2])))) || ((((arr_8 [i_2] [i_2]) + var_4)))))) : ((var_7 ^ (arr_8 [i_2] [i_2])))));
    }
    #pragma endscop
}
