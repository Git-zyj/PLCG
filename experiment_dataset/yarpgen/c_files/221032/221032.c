/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221032
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221032 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221032
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_3;
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 20;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 21;i_4 += 1)
                        {
                            {
                                var_11 = ((-(((~var_9) ? (arr_7 [i_0 - 2] [i_1] [i_0 - 2] [i_3]) : ((7806 ? (arr_2 [i_2]) : var_1))))));
                                var_12 -= var_9;
                                var_13 = ((((!(!var_0))) ? (arr_11 [16] [i_4]) : (((!(!var_7))))));
                            }
                        }
                    }
                }
                var_14 -= (arr_7 [17] [i_1] [i_1] [i_0]);
                var_15 = (arr_1 [i_0]);
            }
        }
    }
    var_16 = (max(var_16, (((~((-(23161 < -1869141250762382295))))))));
    #pragma endscop
}
