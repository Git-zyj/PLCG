/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33688
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33688 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33688
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 ^= ((/* implicit */long long int) var_13);
    var_16 ^= ((/* implicit */unsigned char) (+(((long long int) var_4))));
    /* LoopNest 2 */
    for (unsigned char i_0 = 3; i_0 < 13; i_0 += 1) 
    {
        for (long long int i_1 = 0; i_1 < 15; i_1 += 4) 
        {
            {
                arr_5 [i_0] [(unsigned char)13] [10LL] = ((/* implicit */long long int) (~(var_11)));
                var_17 = ((/* implicit */unsigned int) arr_2 [i_0] [i_0]);
                var_18 = ((/* implicit */long long int) arr_0 [i_0 - 1] [i_0 + 1]);
            }
        } 
    } 
    var_19 = ((/* implicit */signed char) ((unsigned char) (unsigned short)0));
    var_20 = ((/* implicit */long long int) (!(((/* implicit */_Bool) var_11))));
}
