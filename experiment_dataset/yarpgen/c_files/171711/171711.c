/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171711
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171711 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171711
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_0;

    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        var_18 = (~var_15);
        var_19 ^= ((((-(arr_2 [i_0 + 1]))) / (arr_2 [i_0 + 1])));
        arr_3 [i_0] = (arr_2 [i_0 + 1]);

        /* vectorizable */
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            arr_7 [i_0] = ((+((var_13 ? (arr_5 [i_1]) : (arr_2 [i_0])))));
            var_20 *= var_4;
        }
    }
    for (int i_2 = 0; i_2 < 20;i_2 += 1)
    {
        arr_12 [i_2] = var_6;
        var_21 *= ((((-(arr_9 [i_2] [i_2])))) ? -7597627375583372797 : ((-(arr_8 [i_2]))));
    }
    for (int i_3 = 1; i_3 < 11;i_3 += 1)
    {
        var_22 += ((+(((arr_0 [i_3 - 1]) ^ ((-(arr_6 [i_3] [i_3] [i_3 + 1])))))));
        var_23 = var_5;
        arr_15 [i_3] [i_3] = ((~var_6) * (((((arr_5 [i_3 - 1]) && (arr_9 [i_3 + 1] [i_3 + 1]))))));
    }
    var_24 *= -var_11;
    var_25 = ((var_9 * (min(var_10, var_1))));
    var_26 *= var_16;
    #pragma endscop
}
