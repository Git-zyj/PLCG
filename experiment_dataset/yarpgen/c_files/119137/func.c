/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119137
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119137 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119137
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 *= ((/* implicit */unsigned long long int) var_4);
    var_14 = ((/* implicit */long long int) ((((/* implicit */unsigned long long int) (~(var_4)))) & (var_5)));
    var_15 = var_8;
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 1; i_0 < 16; i_0 += 3) 
    {
        var_16 = ((/* implicit */unsigned short) ((((/* implicit */int) (unsigned short)39990)) > (((arr_0 [i_0]) ? (((/* implicit */int) (unsigned char)38)) : (((/* implicit */int) (unsigned char)156))))));
        arr_2 [i_0] = (+(((unsigned int) var_8)));
    }
}
