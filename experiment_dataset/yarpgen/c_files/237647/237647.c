/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237647
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237647 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237647
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 ^= (min(1, 5345019300464622010));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        var_15 = var_9;
                        var_16 = ((((((var_11 <= var_13) || ((max(var_5, var_8)))))) ^ ((-var_9 / (max(var_6, var_1))))));
                    }
                    arr_13 [i_2] [0] [i_2] [3] |= (min((((var_7 & ((min(var_9, var_11)))))), ((var_13 & (min(var_12, var_7))))));
                }
            }
        }
    }
    #pragma endscop
}
