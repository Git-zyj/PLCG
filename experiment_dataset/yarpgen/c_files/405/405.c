/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 405
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=405 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/405
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(var_9, var_8));

    /* vectorizable */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        arr_3 [i_0] = (arr_1 [i_0 + 1] [i_0 + 1]);
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    arr_8 [i_2] = var_8;
                    arr_9 [i_2] = ((-((2047 & (arr_4 [i_0] [i_0] [i_0])))));
                    var_15 = (((arr_2 [i_0 + 4]) ? (var_4 * 28818) : -19));
                }
            }
        }
        arr_10 [i_0] = (arr_5 [i_0 + 1] [7]);
    }
    for (int i_3 = 0; i_3 < 1;i_3 += 1)
    {
        var_16 = 54315;
        arr_15 [i_3] = (min((max((arr_5 [i_3] [7]), (arr_5 [i_3] [i_3]))), (!-1)));
        /* LoopNest 3 */
        for (int i_4 = 1; i_4 < 11;i_4 += 1)
        {
            for (int i_5 = 1; i_5 < 11;i_5 += 1)
            {
                for (int i_6 = 0; i_6 < 1;i_6 += 1)
                {
                    {
                        var_17 = ((63 && (arr_4 [i_6] [i_5] [i_4 + 2])));
                        var_18 = ((!((min(5623443182671808456, 30)))));
                    }
                }
            }
        }
        arr_25 [i_3] = (arr_0 [i_3] [i_3]);
    }
    #pragma endscop
}
