/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239645
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239645 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239645
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            {
                var_19 = ((((max(2611556571616044186, (arr_4 [i_0 + 1])))) ? (((18446744073692774400 >= (arr_4 [i_0 + 4])))) : (((50735 != (arr_4 [i_0 + 1]))))));
                var_20 &= (arr_4 [i_0 + 2]);
                arr_5 [i_1 + 2] [i_1] [i_1 + 1] &= 26328;
                arr_6 [i_0 + 3] [i_0 + 3] [i_0 + 3] = 19;
            }
        }
    }
    var_21 += (min((~var_2), var_12));
    #pragma endscop
}
