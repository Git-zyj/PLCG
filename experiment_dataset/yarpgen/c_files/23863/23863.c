/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23863
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23863 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23863
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((((min(664, ((min(29, 126))))))) ^ var_3));
    var_17 = var_3;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            {
                var_18 += ((~(((arr_3 [i_0] [i_1] [i_0]) ^ (arr_0 [0])))));
                var_19 += min((((((arr_2 [i_0] [i_0]) ? 6213809399070886446 : -5325)))), (min((arr_0 [i_1 - 1]), 8796093022207)));
                var_20 = (min(var_20, ((((-(max((arr_1 [i_0] [i_0]), 9823064125986579407))))))));
            }
        }
    }
    var_21 = var_12;
    var_22 = var_2;
    #pragma endscop
}
