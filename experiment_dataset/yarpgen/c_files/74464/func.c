/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74464
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74464 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74464
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 += ((/* implicit */short) var_7);
    var_13 = ((/* implicit */int) max((var_13), (((/* implicit */int) var_11))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 21; i_0 += 4) 
    {
        for (unsigned short i_1 = 3; i_1 < 20; i_1 += 1) 
        {
            for (signed char i_2 = 1; i_2 < 22; i_2 += 1) 
            {
                {
                    arr_8 [i_2] [i_1] [i_0] = ((/* implicit */long long int) ((((((/* implicit */int) arr_1 [i_0 + 2])) >= (((/* implicit */int) arr_1 [i_0 + 1])))) || (((/* implicit */_Bool) min(((-9223372036854775807LL - 1LL)), (((/* implicit */long long int) ((((/* implicit */int) (_Bool)1)) <= (((/* implicit */int) (unsigned short)28428))))))))));
                    var_14 ^= ((/* implicit */long long int) var_3);
                    var_15 = ((/* implicit */unsigned short) min((var_15), (((/* implicit */unsigned short) var_6))));
                    var_16 |= ((/* implicit */unsigned char) (unsigned short)28428);
                    arr_9 [i_2] = ((((/* implicit */long long int) ((/* implicit */int) arr_0 [i_0]))) != (-9223372036854775783LL));
                }
            } 
        } 
    } 
}
