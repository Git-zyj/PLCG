/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87831
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87831 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87831
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */signed char) ((short) var_1));
    /* LoopSeq 1 */
    for (unsigned short i_0 = 4; i_0 < 10; i_0 += 4) 
    {
        var_11 = ((/* implicit */unsigned int) arr_0 [i_0]);
        var_12 = ((/* implicit */long long int) 2147483647);
        arr_2 [i_0] [i_0 + 1] |= max((arr_1 [i_0]), (((/* implicit */unsigned long long int) ((((/* implicit */_Bool) max((((/* implicit */unsigned int) arr_0 [i_0])), (var_1)))) ? (((/* implicit */int) arr_0 [i_0 - 4])) : (((((/* implicit */_Bool) (unsigned char)1)) ? (((/* implicit */int) (unsigned char)227)) : (((/* implicit */int) (unsigned char)255))))))));
    }
}
