#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

unsigned int var_1 = 3156727101U;
unsigned char var_4 = (unsigned char)253;
unsigned char var_5 = (unsigned char)198;
unsigned char var_6 = (unsigned char)103;
unsigned char var_7 = (unsigned char)166;
unsigned char var_8 = (unsigned char)27;
short var_9 = (short)-1699;
int zero = 0;
unsigned short var_11 = (unsigned short)57317;
int var_12 = -1737775829;
short var_13 = (short)-8903;
int arr_5 [16] ;
void init() {
    for (size_t i_0 = 0; i_0 < 16; ++i_0) 
        arr_5 [i_0] = (i_0 % 2 == 0) ? 559144785 : -1756322924;
}

void checksum() {
    hash(&seed, var_11);
    hash(&seed, var_12);
    hash(&seed, var_13);
    for (size_t i_0 = 0; i_0 < 16; ++i_0) 
        hash(&seed, arr_5 [i_0] );
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
