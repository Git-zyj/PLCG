/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244546
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244546 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244546
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((((var_5 ? var_5 : (((min(var_13, var_12)))))) == var_14));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_20 = (max(var_20, (((+(min((arr_6 [i_0]), (arr_0 [i_0] [i_1]))))))));
                    var_21 = (min(var_21, (arr_0 [i_2] [i_1])));
                }
            }
        }
    }
    var_22 = ((var_3 ? 7 : ((var_0 ? ((var_1 ? var_17 : var_13)) : var_3))));
    var_23 = (!(~var_6));
    #pragma endscop
}
