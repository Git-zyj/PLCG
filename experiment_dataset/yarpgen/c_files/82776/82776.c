/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82776
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82776 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82776
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (((max(255, 21))) ? (((var_0 ? 16 : var_4))) : ((var_9 ? (-16 / var_14) : (var_2 * var_5))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            {
                var_19 = (arr_2 [i_0] [i_0]);
                arr_6 [i_0] = -var_10;
                arr_7 [i_0] = 27907;
                arr_8 [i_0] [i_0] = (arr_2 [i_0] [i_0]);
                var_20 = (min(((min(-26, (arr_4 [i_0] [5] [i_0])))), ((var_15 ? (arr_4 [i_1 + 1] [i_1 - 2] [i_0]) : 134217724))));
            }
        }
    }
    var_21 = ((var_4 ? var_6 : -var_0));
    var_22 = var_10;
    var_23 = var_6;
    #pragma endscop
}
