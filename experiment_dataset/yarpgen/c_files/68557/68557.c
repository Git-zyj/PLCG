/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68557
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68557 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68557
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        arr_3 [i_0] = (arr_1 [2] [i_0]);
        arr_4 [i_0] = ((~(arr_0 [i_0] [i_0])));

        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            arr_7 [i_0] [i_1] = ((-(arr_6 [i_0])));
            var_16 = (arr_5 [i_0 + 1] [1]);
            arr_8 [i_0] [i_0] = (0 ^ 1);
            var_17 = (var_12 < (((arr_2 [i_0]) ? var_11 : (arr_2 [i_0]))));
            var_18 = (min(var_18, (((0 ? 1 : 0)))));
        }
    }
    var_19 += var_15;
    var_20 = (min(var_20, (((!(((var_5 ? ((var_7 ? var_10 : var_13)) : (min(4184523517, var_14))))))))));
    #pragma endscop
}
