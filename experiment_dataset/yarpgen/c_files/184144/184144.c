/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184144
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184144 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184144
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 23;i_1 += 1)
        {
            {
                var_18 &= (((arr_2 [20] [i_1]) < (min((arr_4 [i_1 - 4]), (((arr_1 [i_0]) ? var_11 : -106))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 24;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        {
                            arr_13 [i_1 - 1] = -15;
                            var_19 = (((((8562429886679695992 ? 2342762019 : (!var_17)))) ? (((9394 >> (2342762019 - 2342761995)))) : (1952205277 * 1952205282)));
                        }
                    }
                }
                arr_14 [i_0] [i_0] |= (+-0);
            }
        }
    }
    var_20 = (min((~32767), var_5));
    #pragma endscop
}
