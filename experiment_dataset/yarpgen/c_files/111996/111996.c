/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111996
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111996 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111996
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((~(max(var_11, var_5))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_14 = var_7;
                    arr_6 [i_0] [i_0] [i_1] [i_2] = (max(((108 ? var_8 : (arr_2 [i_2]))), var_5));
                    var_15 |= (max(((((var_6 ? 8 : 0)) ^ ((4 ? -35 : (arr_0 [8] [i_0]))))), ((1 + ((max(var_2, var_7)))))));
                    arr_7 [i_0] [i_0] [i_0] [i_1] = (((arr_5 [i_0] [i_1] [i_2]) <= ((((arr_5 [i_0] [i_1] [i_2]) & (arr_0 [1] [1]))))));
                }
            }
        }
    }
    #pragma endscop
}
