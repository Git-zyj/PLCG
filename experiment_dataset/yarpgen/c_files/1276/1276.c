/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 1276
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=1276 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/1276
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 -= (-122 & -99);

    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        var_15 = (max(((-47 / (arr_0 [i_0]))), ((~(arr_0 [i_0])))));
        var_16 = ((((((var_13 - var_8) <= var_0))) > 120));
        arr_2 [i_0] [i_0] = ((-((~(var_3 != -120)))));
        arr_3 [i_0] &= var_6;
    }
    var_17 = (((((min(102, 1654161165978922365)) | (((max(119, var_5)))))) - (min(((var_0 ? var_7 : var_13)), (~var_1)))));
    var_18 = ((var_1 & ((((max(var_10, 46)))))));
    #pragma endscop
}
