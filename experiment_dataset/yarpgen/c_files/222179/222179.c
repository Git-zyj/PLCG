/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222179
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222179 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222179
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        var_17 = (~(arr_1 [i_0 - 1]));
        var_18 ^= (((((arr_1 [i_0 - 2]) ? var_9 : var_10)) ^ ((((arr_0 [i_0 - 2]) << (var_12 - 7654660834259544781))))));
        arr_2 [i_0] = 1;
    }
    var_19 = (var_13 >= (((max(var_0, var_13)) & var_16)));
    var_20 = var_15;
    /* LoopNest 3 */
    for (int i_1 = 1; i_1 < 23;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 25;i_3 += 1)
            {
                {
                    var_21 |= ((-((~(((arr_4 [i_1]) ? (arr_8 [i_2] [i_2]) : (arr_9 [i_3] [i_2] [i_1])))))));
                    var_22 = var_9;
                    var_23 |= ((max(var_12, var_15)));
                }
            }
        }
    }
    #pragma endscop
}
