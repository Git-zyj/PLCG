/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14223
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14223 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14223
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        var_10 += ((((((arr_3 [i_0] [i_0]) || (arr_3 [i_0] [i_0])))) <= (arr_2 [i_0])));
        var_11 += ((((((((-(arr_1 [i_0] [i_0])))) ? (784800415 - 21804) : (arr_0 [i_0] [i_0])))) ? (min((1 / var_3), (arr_3 [i_0] [i_0]))) : (arr_2 [i_0])));
        var_12 += (arr_3 [2] [i_0]);
    }
    for (int i_1 = 0; i_1 < 17;i_1 += 1)
    {
        var_13 += ((((-(arr_2 [i_1]))) > (arr_3 [i_1] [i_1])));
        var_14 = (arr_3 [i_1] [i_1]);
        var_15 += (((~43734) < (-3146 != 1)));
    }
    var_16 = var_5;
    var_17 = var_5;
    var_18 = (((((-(max(var_2, var_1))))) ? var_9 : var_4));
    #pragma endscop
}
