/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218175
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218175 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218175
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_1 + 1] = ((~((~(((arr_3 [i_0] [i_0] [i_0 + 3]) ? (arr_1 [i_0]) : var_2))))));
                var_15 *= ((+((var_5 - (min((arr_2 [i_1]), var_11))))));
                var_16 = (arr_0 [11] [i_0]);
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 3; i_2 < 11;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 14;i_3 += 1)
        {
            {
                arr_11 [i_2] [i_3] [i_3] = (min(var_3, -var_9));
                var_17 += ((var_0 ? -142 : (min(((var_6 ? (arr_7 [1]) : (arr_7 [i_3]))), ((-(arr_10 [i_2] [i_3] [4])))))));
                arr_12 [i_2 - 3] = (min(((max(1, 255))), 1551683638));
                var_18 = (arr_6 [i_3]);
            }
        }
    }
    #pragma endscop
}
