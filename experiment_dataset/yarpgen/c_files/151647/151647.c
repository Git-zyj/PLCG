/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151647
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151647 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151647
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_20 ^= (((((arr_5 [i_1 - 4] [i_1]) & (arr_5 [i_1 - 1] [i_1]))) << (((arr_4 [i_1 - 4]) - 449215340))));
                    var_21 = (((var_16 >= var_19) ? var_7 : (((arr_1 [i_1 - 1]) ? ((10797666069695644974 ? var_1 : var_12)) : (max(930773694, var_13))))));
                }
            }
        }
    }
    var_22 = ((((min(-1425450283, var_2)) ? (((-6529126940546501180 / (-2147483647 - 1)))) : var_7)));
    #pragma endscop
}
