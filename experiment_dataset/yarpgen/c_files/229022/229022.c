/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229022
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229022 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229022
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 += ((!(!var_9)));

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        var_15 += ((!((min(7, 4294967294)))));
        arr_3 [20] |= ((((max((arr_0 [i_0]), (arr_0 [i_0])))) ? (((var_4 + var_6) ? (((arr_1 [i_0]) ? var_1 : (arr_1 [i_0]))) : (arr_2 [i_0]))) : (((((max((arr_2 [i_0]), (arr_0 [i_0])))) ? ((var_12 ? (arr_0 [i_0]) : (arr_0 [i_0]))) : (((arr_2 [i_0]) ? var_10 : (arr_0 [i_0]))))))));
    }
    var_16 = ((var_1 ? var_8 : var_4));
    #pragma endscop
}
