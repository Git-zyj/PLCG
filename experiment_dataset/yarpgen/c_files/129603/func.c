/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 129603
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=129603 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/129603
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned int) -1328519702);
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned char i_0 = 1; i_0 < 11; i_0 += 1) 
    {
        var_13 = ((/* implicit */_Bool) max((var_13), (((/* implicit */_Bool) (+(((((/* implicit */_Bool) var_7)) ? (var_4) : (((/* implicit */unsigned int) ((/* implicit */int) (unsigned char)94))))))))));
        /* LoopNest 3 */
        for (short i_1 = 0; i_1 < 14; i_1 += 3) 
        {
            for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
            {
                for (short i_3 = 0; i_3 < 14; i_3 += 3) 
                {
                    {
                        var_14 = var_3;
                        var_15 = ((/* implicit */int) 122880LL);
                        var_16 = ((/* implicit */unsigned char) max((var_16), (var_6)));
                        arr_10 [i_0] [i_1] = ((/* implicit */short) var_10);
                    }
                } 
            } 
        } 
    }
    var_17 = ((/* implicit */unsigned long long int) min((((/* implicit */unsigned int) var_6)), (117440512U)));
}
