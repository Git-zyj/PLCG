/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 137210
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=137210 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/137210
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_1;

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_21 = (max(var_21, (((1 | (((arr_3 [i_0]) ? (min((arr_3 [i_0]), var_1)) : -1)))))));
        var_22 = (arr_2 [i_0]);
        var_23 = (((arr_3 [i_0]) == (1 % 1)));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    var_24 = var_8;
                    var_25 = (1 > 4009094866);
                }
            }
        }
    }
    /* vectorizable */
    for (int i_3 = 0; i_3 < 1;i_3 += 1)
    {
        var_26 = (max(var_26, (((-((var_17 | (arr_7 [i_3] [i_3] [2] [i_3]))))))));
        var_27 = ((6921624150301104701 % (((1479191485 ? 1 : var_14)))));
        var_28 = (((arr_8 [i_3]) < var_9));
    }
    #pragma endscop
}
