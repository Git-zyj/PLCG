/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90520
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90520 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90520
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_0] = (((arr_1 [i_0] [8]) ? (max(((var_9 ? 11231941436685774306 : (arr_5 [i_0] [i_0] [i_0]))), ((max(1, (arr_1 [i_0] [i_0])))))) : (((max(var_16, var_4))))));
                var_17 = (min(var_17, (((~((var_16 & (arr_4 [i_0] [i_0] [i_0]))))))));
            }
        }
    }
    var_18 = ((var_4 * (min(var_7, -var_0))));
    #pragma endscop
}
