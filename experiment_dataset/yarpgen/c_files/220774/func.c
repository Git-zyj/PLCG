/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220774
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220774 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220774
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (short i_0 = 3; i_0 < 21; i_0 += 3) 
    {
        var_17 += ((((/* implicit */_Bool) (+(((/* implicit */int) var_11))))) ? (((/* implicit */int) (!(((/* implicit */_Bool) var_11))))) : (((/* implicit */int) var_12)));
        var_18 ^= ((/* implicit */_Bool) (+(((/* implicit */int) ((var_5) != (((/* implicit */int) (unsigned char)7)))))));
    }
    var_19 = ((/* implicit */unsigned char) (+(var_0)));
}
