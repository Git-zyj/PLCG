/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72409
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72409 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72409
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((min(1, (14348070973489691269 - 4613587725712812878))));
    var_16 |= var_7;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_4 [i_0] [1] |= (min(((2219160440 ? (arr_3 [i_0] [i_1]) : ((1 ? 3809634691695073699 : 3700659182)))), (min(((~(arr_3 [i_0] [i_0]))), ((~(arr_1 [i_0] [i_1])))))));
                arr_5 [i_1] = 1;
            }
        }
    }
    #pragma endscop
}
