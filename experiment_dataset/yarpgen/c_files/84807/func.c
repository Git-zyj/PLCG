/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84807
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84807 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84807
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 &= ((/* implicit */int) (_Bool)1);
    /* LoopNest 2 */
    for (short i_0 = 1; i_0 < 15; i_0 += 4) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_19 = ((/* implicit */int) ((((/* implicit */int) (unsigned short)7050)) < (((((/* implicit */int) arr_4 [i_0] [i_1])) - (arr_0 [i_0] [i_1])))));
                var_20 = ((/* implicit */short) max((((/* implicit */int) ((unsigned short) ((arr_3 [3]) | (((/* implicit */int) var_4)))))), (((((/* implicit */int) arr_1 [14])) / (((((/* implicit */int) var_9)) * (((/* implicit */int) (unsigned short)7064))))))));
            }
        } 
    } 
    var_21 &= ((/* implicit */unsigned int) (unsigned short)58478);
    var_22 = ((/* implicit */int) var_2);
    var_23 = ((/* implicit */unsigned long long int) var_1);
}
