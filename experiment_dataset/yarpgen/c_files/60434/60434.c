/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60434
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60434 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60434
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_1;
    var_19 = var_11;
    var_20 = var_8;

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        arr_2 [1] = (arr_0 [3]);

        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            var_21 = (max(var_21, ((min(0, (((190 ? var_12 : var_10))))))));

            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                var_22 |= (((var_11 ? (arr_0 [i_1]) : (arr_0 [i_0]))));
                var_23 -= ((!(~171)));
            }
        }
        var_24 = ((((((min(var_16, (arr_4 [3] [3] [i_0]))) > ((11 ? (arr_4 [i_0] [i_0] [i_0]) : var_8)))))) - (min((3585159546 * 0), (arr_5 [i_0]))));
    }
    var_25 -= var_6;
    #pragma endscop
}
