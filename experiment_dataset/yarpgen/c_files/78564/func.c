/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78564
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78564 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78564
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 &= ((/* implicit */unsigned short) ((long long int) (((_Bool)0) && (((/* implicit */_Bool) min((-1868724461), (((/* implicit */int) (signed char)(-127 - 1)))))))));
    var_20 = ((/* implicit */signed char) (short)-8192);
    /* LoopSeq 1 */
    /* vectorizable */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_21 = ((/* implicit */long long int) ((((/* implicit */int) (unsigned char)31)) > (((/* implicit */int) (short)-8192))));
        var_22 = ((/* implicit */signed char) ((unsigned long long int) 0ULL));
    }
}
