/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217645
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217645 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217645
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [i_0] = (arr_2 [i_0]);
                arr_6 [i_0] [i_1] = ((((~0) ^ (((arr_3 [i_0] [i_1]) ^ 18185940820611862763)))) & ((min(((65532 ? 1 : var_12)), ((4294967274 ? (arr_1 [i_0]) : var_12))))));
                arr_7 [i_1] |= ((-((((min(var_14, var_13))) ? 1 : ((min(-7, var_1)))))));
            }
        }
    }
    var_18 = (((((852532187 ? 48421 : var_15))) ? (((((6375973363412401695 ? var_16 : -15010))))) : 6375973363412401688));
    #pragma endscop
}
