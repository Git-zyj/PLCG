/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 57711
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=57711 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/57711
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        arr_3 [i_0] = 416715422;
        arr_4 [i_0] [i_0] = ((-416715422 | (((!(arr_1 [i_0]))))));
    }
    var_17 = -416715422;
    /* LoopNest 3 */
    for (int i_1 = 2; i_1 < 20;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 22;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 22;i_3 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_4 = 0; i_4 < 0;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 22;i_5 += 1)
                        {
                            {
                                arr_17 [i_2] = ((((((-118 + 2147483647) >> (9834 - 9814)))) ? 2849731229362560306 : 9845));
                                arr_18 [i_2] [15] [i_3] [15] [i_2] = (arr_6 [i_1 + 1]);
                            }
                        }
                    }
                    arr_19 [i_1] [i_1] = ((~(((min(var_15, (arr_14 [i_1] [i_2] [i_3] [i_3])))))));
                }
            }
        }
    }
    #pragma endscop
}
