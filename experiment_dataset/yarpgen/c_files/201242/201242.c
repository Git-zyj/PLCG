/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 201242
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=201242 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/201242
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            {
                arr_6 [i_0] = max(((((arr_2 [i_1] [i_0]) ? (arr_4 [i_0]) : (arr_2 [i_0] [i_1])))), (arr_3 [i_0]));
                arr_7 [i_0] [i_0] [i_0] = (arr_0 [i_1]);
            }
        }
    }
    var_17 = var_2;
    var_18 = ((var_9 ? ((((min(var_5, var_9))) ? (max(var_8, var_2)) : var_15)) : ((((max(var_6, var_7))) ? (max(var_2, var_10)) : var_9))));
    var_19 &= var_3;
    var_20 = var_15;
    #pragma endscop
}
