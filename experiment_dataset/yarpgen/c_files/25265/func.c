/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 25265
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=25265 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/25265
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */int) var_1);
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 20; i_0 += 2) 
    {
        arr_3 [(short)19] = ((/* implicit */short) ((((/* implicit */_Bool) ((arr_1 [i_0]) / ((-(var_18)))))) && ((!(((/* implicit */_Bool) arr_0 [i_0]))))));
        var_20 = ((/* implicit */short) max((((((/* implicit */_Bool) max((((/* implicit */int) (_Bool)1)), (var_14)))) ? (((/* implicit */int) (!(((/* implicit */_Bool) (short)-14107))))) : (1876892205))), (((/* implicit */int) ((_Bool) (!(var_4)))))));
    }
}
