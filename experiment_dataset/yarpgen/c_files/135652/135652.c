/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135652
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135652 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135652
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            {
                var_12 = ((3405251628 ? ((127 ? (max(-7339646078812031582, 1)) : -52)) : (((min(1, (min(0, 1))))))));
                arr_6 [13] [i_0] [i_1] = -7339646078812031597;
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 18;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 18;i_3 += 1)
        {
            {
                var_13 &= (max(16756, (min((((1 ? 4096 : 1))), 4773804321035512372))));
                var_14 = (min(var_14, ((((94 <= 2254654632) > (70 && 94))))));
                arr_13 [i_2] [i_3] [i_3] = ((min(-7, -23507)));
            }
        }
    }
    #pragma endscop
}
