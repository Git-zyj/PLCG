/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47204
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47204 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47204
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 15;i_1 += 1)
        {
            {
                var_11 += (((arr_5 [i_0] [i_1] [i_1]) ? (((!0) ? (((min((arr_0 [i_1]), (arr_2 [i_1]))))) : (((arr_6 [i_1 - 2] [i_0]) ? (arr_1 [i_1]) : (arr_3 [i_1]))))) : (!255)));
                var_12 = (max(var_12, ((max((((((arr_4 [i_0] [i_1]) ? var_4 : 1)) << (arr_6 [9] [i_1 + 3]))), (!var_3))))));
            }
        }
    }
    var_13 = var_5;
    var_14 = var_0;
    var_15 += ((((((var_8 ? var_2 : -918047169))) ? var_3 : 1)));
    #pragma endscop
}
