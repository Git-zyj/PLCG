/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14125
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14125 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14125
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_8;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_13 = ((max((((arr_2 [i_0] [i_0]) + var_6)), (arr_1 [i_0 + 1] [i_1 - 1]))));
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 9;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        {
                            var_14 = -11631;
                            arr_12 [i_0] [i_2] [i_0] [i_0] [i_2] [i_2] = ((min((((var_8 ? var_8 : (arr_4 [i_3] [i_3] [i_3])))), (max((arr_6 [i_2] [i_1] [i_1] [i_2]), -6066978149585622786)))));
                        }
                    }
                }
            }
        }
    }
    var_15 = -30620;
    #pragma endscop
}
