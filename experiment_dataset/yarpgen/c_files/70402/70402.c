/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70402
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70402 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70402
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_3 [13] [i_0] = (arr_1 [i_0] [9]);
        arr_4 [i_0] = (2401162587 ? (752175791919230752 == 752175791919230752) : 752175791919230752);
    }
    var_19 = (((max(752175791919230752, 76)) < var_18));
    var_20 = max((((2896472062 ? 0 : 2896472062))), var_0);
    #pragma endscop
}
