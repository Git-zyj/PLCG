/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 193782
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=193782 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/193782
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 |= (max(3436, 281200098803712));
    var_19 -= (min((~-116), var_8));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_20 = -116;
                    arr_7 [i_0] [i_2] [i_2] = (((max((arr_2 [9] [i_2]), (max((arr_0 [i_1] [i_2]), (arr_2 [i_0] [i_1]))))) + (arr_5 [i_0] [i_0])));
                }
            }
        }
    }
    var_21 = (max(-125, 93));
    var_22 = (((-127 - 1) ? 549755813887 : 18050807694406063281));
    #pragma endscop
}
