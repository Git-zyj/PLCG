/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222430
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222430 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222430
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_15;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_18 = ((((arr_5 [i_0] [i_1] [i_2]) * (arr_4 [i_0] [i_0]))));
                    var_19 = (max(((((((var_14 == (arr_1 [1] [10]))))) * var_8)), (arr_7 [i_0] [i_1] [i_0])));
                    arr_9 [i_0] [i_1] [i_0] = ((((min(((var_14 ? (arr_8 [i_0]) : (arr_6 [i_2] [i_1] [i_1] [6]))), var_9))) && (((arr_5 [i_0] [i_1] [i_0]) >= (arr_4 [i_0] [i_0])))));
                }
            }
        }
    }
    var_20 = (min(var_20, ((min((~26), var_8)))));
    var_21 = var_5;
    #pragma endscop
}
