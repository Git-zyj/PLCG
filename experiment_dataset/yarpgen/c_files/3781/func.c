/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3781
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3781 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3781
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 12; i_0 += 4) 
    {
        var_17 = ((/* implicit */_Bool) ((((/* implicit */long long int) (+(((((/* implicit */_Bool) var_8)) ? (((/* implicit */int) arr_0 [i_0] [i_0])) : (((/* implicit */int) (unsigned char)0))))))) ^ (((((/* implicit */_Bool) min((((/* implicit */long long int) var_14)), (var_10)))) ? (((/* implicit */long long int) ((/* implicit */int) var_9))) : (-7870626424343418242LL)))));
        var_18 = ((/* implicit */long long int) ((signed char) (~((~(var_0))))));
    }
    var_19 = ((/* implicit */int) max((var_19), (((/* implicit */int) var_10))));
    var_20 = ((/* implicit */unsigned char) var_6);
}
