/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234729
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234729 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234729
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */int) min((var_16), (((/* implicit */int) var_9))));
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 20; i_1 += 4) 
        {
            for (unsigned long long int i_2 = 0; i_2 < 20; i_2 += 3) 
            {
                {
                    var_17 = ((/* implicit */signed char) ((((((/* implicit */int) arr_4 [i_1])) << (((arr_2 [i_0] [i_1] [i_1]) - (1589626926))))) * (((/* implicit */int) min((arr_6 [i_1] [i_0]), (((/* implicit */unsigned char) var_15)))))));
                    var_18 = ((((/* implicit */_Bool) ((unsigned char) ((((/* implicit */_Bool) (signed char)10)) && (((/* implicit */_Bool) var_5)))))) ? (var_5) : (((/* implicit */unsigned int) ((/* implicit */int) arr_4 [i_0]))));
                    var_19 = ((/* implicit */signed char) 62914560);
                }
            } 
        } 
    } 
    var_20 ^= ((/* implicit */_Bool) 0U);
    var_21 = ((/* implicit */_Bool) var_6);
    var_22 |= ((/* implicit */short) var_1);
}
