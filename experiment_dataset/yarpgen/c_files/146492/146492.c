/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146492
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146492 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146492
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            {
                var_20 |= (min(((1 ? 3541480293 : var_5)), var_11));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 13;i_4 += 1)
                        {
                            {
                                arr_12 [i_2] [i_0] |= var_16;
                                arr_13 [i_3] = var_4;
                                arr_14 [i_0] [i_0] [i_1] [i_0] [i_3] [i_3] = ((4024528358 ? 2147483638 : 1));
                            }
                        }
                    }
                }
            }
        }
    }
    var_21 |= var_14;
    var_22 = (((min(32755, var_4))) ? var_0 : ((min((1 >> var_16), var_9))));
    var_23 = var_0;
    #pragma endscop
}
