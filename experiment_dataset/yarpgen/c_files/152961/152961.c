/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152961
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152961 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152961
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_0;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 25;i_4 += 1)
                        {
                            {
                                var_17 = (((((59406 ? var_0 : 59406))) ? ((-64 ? var_12 : var_9)) : 63230));
                                var_18 -= ((var_1 ? ((var_15 ? var_13 : var_13)) : ((var_13 ? var_12 : var_6))));
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 25;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 25;i_6 += 1)
                        {
                            {
                                var_19 = var_5;
                                var_20 = var_2;
                                var_21 = (max(var_21, ((((((var_7 ? var_5 : var_7))) ? ((var_6 ? var_14 : var_4)) : (((var_12 ? var_13 : var_10))))))));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
