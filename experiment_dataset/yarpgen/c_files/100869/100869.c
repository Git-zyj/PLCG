/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100869
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100869 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100869
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1) /* same iter space */
    {
        var_15 = (max(var_15, (arr_0 [i_0] [i_0])));
        var_16 = (arr_1 [i_0] [i_0]);
        arr_2 [3] |= ((((((arr_0 [i_0] [i_0]) / (arr_0 [14] [i_0])))) * (((((arr_1 [i_0] [7]) ? 281474976710655 : (arr_0 [i_0] [i_0]))) / -var_0))));
        var_17 ^= (((~((((arr_0 [i_0] [i_0]) != (arr_1 [i_0] [i_0])))))) > (((((arr_0 [i_0] [i_0]) / (arr_0 [0] [i_0]))) / -var_10)));
    }
    for (int i_1 = 0; i_1 < 25;i_1 += 1) /* same iter space */
    {
        arr_5 [i_1] = var_0;
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 0;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 25;i_3 += 1)
            {
                {
                    arr_11 [i_1] [i_1] [0] [i_1] &= (arr_7 [24]);
                    var_18 += ((~((-((-(arr_10 [i_3] [i_2 + 1])))))));
                }
            }
        }
        var_19 = (arr_0 [i_1] [15]);
        var_20 = (arr_10 [i_1] [i_1]);
    }
    var_21 |= (!var_10);
    var_22 -= var_2;
    #pragma endscop
}
