/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102910
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102910 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102910
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        arr_2 [i_0] = (arr_1 [i_0]);
        /* LoopNest 2 */
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_13 = (max(var_13, 1764985507316710781));
                                arr_14 [i_0] [i_0] [i_0] [i_0] [i_0] &= (((arr_8 [i_4 + 2] [i_1 + 1]) / (arr_8 [i_4 + 3] [i_1 + 1])));
                                var_14 += (arr_5 [i_4 + 3]);
                            }
                        }
                    }
                    arr_15 [i_2] [i_2] [i_1] [i_0] = ((var_3 || (arr_6 [i_0] [i_1 + 2])));
                    arr_16 [i_0] [i_1] [i_1] [i_0] &= arr_3 [i_0] [i_0] [i_2];
                }
            }
        }
        var_15 = (arr_9 [i_0] [i_0] [i_0]);
        var_16 *= 1981389854609481224;
    }
    var_17 = ((var_7 ? var_4 : var_4));
    #pragma endscop
}
