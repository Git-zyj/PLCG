/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 10022
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=10022 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/10022
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_13 += (~-0);
        var_14 = ((-((41466 ? 24068 : -41470))));
        var_15 += ((+(((((arr_2 [i_0]) + 2147483647)) >> (((((arr_1 [0] [i_0]) ? (arr_1 [i_0] [i_0]) : -27021)) - 604894732))))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 25;i_1 += 1)
    {
        var_16 = (max(var_16, (((-1679937837 ^ (arr_3 [i_1] [i_1]))))));
        var_17 += ((arr_4 [i_1]) << (arr_5 [i_1] [i_1]));
    }
    var_18 = (min(var_18, var_7));
    #pragma endscop
}
