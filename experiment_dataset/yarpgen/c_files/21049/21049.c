/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21049
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21049 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21049
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_2] [i_2] [i_0] = (arr_5 [i_0] [i_1]);
                    arr_10 [i_0] = 39664;
                    arr_11 [i_0] [i_0] [i_2] [i_0] = (max((arr_3 [i_0] [i_2]), (max(((max((arr_6 [i_0] [i_1 - 2] [i_1 + 1] [i_2]), var_6))), (max((arr_5 [i_0] [i_1 - 2]), var_13))))));
                }
            }
        }
    }
    var_17 = var_12;
    var_18 = (-2147483647 - 1);
    var_19 = -var_10;
    var_20 = (-(((var_5 >= (8276771727543223831 * 1)))));
    #pragma endscop
}
