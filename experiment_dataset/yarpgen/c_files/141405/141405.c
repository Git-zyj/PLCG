/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 141405
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=141405 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/141405
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_1;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_1] [i_1] = ((6866892173665380823 >> (14173935821488130392 - 14173935821488130373)));
                    arr_8 [14] [i_1] [14] = ((14173935821488130413 <= (arr_0 [i_0])));
                }
            }
        }
    }
    var_11 = -14173935821488130392;
    #pragma endscop
}
