/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29079
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29079 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29079
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(var_18, ((((max(0, 0)) ? ((max((min(var_4, var_2)), ((max(var_6, var_14)))))) : ((min(1, 0))))))));
    var_19 = (max(var_19, 0));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_20 = (min(var_20, ((((((((var_4 ? (arr_3 [i_1]) : (arr_4 [6])))) || (((var_1 ? var_16 : 1)))))) >= ((max(var_1, (arr_4 [i_0]))))))));
                var_21 |= var_17;
            }
        }
    }
    var_22 = (max(var_22, (0 ^ 122)));
    #pragma endscop
}
