/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235122
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235122 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235122
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 21;i_2 += 1)
            {
                {
                    var_12 = (1201558508 ? (((arr_5 [i_2 + 3] [i_2 + 3] [i_2] [i_2 + 1]) ? var_1 : (arr_6 [i_1] [i_2 + 1] [i_2] [i_2]))) : (arr_6 [i_1] [i_2 + 2] [i_2 + 2] [i_2]));
                    var_13 = (((4223702841444556080 ? (arr_6 [i_0] [i_0 + 1] [i_2 - 1] [i_2]) : (arr_6 [i_0] [i_0 + 1] [i_2 + 2] [i_2]))));
                }
            }
        }
    }
    #pragma endscop
}
