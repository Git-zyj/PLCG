/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164466
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164466 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164466
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 13;i_2 += 1)
            {
                {
                    var_11 -= ((((((-113 - (arr_0 [i_0]))) - (((arr_6 [i_0] [i_0] [i_2] [i_0]) - (arr_3 [i_0] [10]))))) + 42460));
                    var_12 |= ((var_10 + (arr_7 [i_0] [i_0] [i_0] [i_0])));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 12;i_4 += 1)
                        {
                            {
                                var_13 = (arr_6 [i_0] [i_1] [i_3] [i_4]);
                                var_14 = (10202 >> (10224489903942068818 - 10224489903942068794));
                                var_15 = 0;
                            }
                        }
                    }
                }
            }
        }
    }
    var_16 |= (!var_2);
    var_17 = (!var_6);
    #pragma endscop
}
