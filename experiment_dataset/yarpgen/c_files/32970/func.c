/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32970
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32970 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32970
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 2; i_0 < 9; i_0 += 2) 
    {
        for (short i_1 = 1; i_1 < 7; i_1 += 3) 
        {
            {
                var_17 = ((/* implicit */signed char) (~((+(((/* implicit */int) max((var_16), (arr_2 [10ULL] [i_1 - 1] [i_1]))))))));
                var_18 = ((/* implicit */signed char) (!(((/* implicit */_Bool) ((max((((/* implicit */int) (short)-17461)), (-2020919484))) % ((~(2020919484))))))));
                var_19 = ((/* implicit */_Bool) (-(((/* implicit */int) (!(((((/* implicit */int) (signed char)117)) <= (((/* implicit */int) (signed char)-118)))))))));
            }
        } 
    } 
    var_20 = ((/* implicit */signed char) (-(((/* implicit */int) (_Bool)0))));
    var_21 = ((/* implicit */signed char) (!((!(((var_15) < (((/* implicit */int) var_16))))))));
    var_22 += (!((!(((/* implicit */_Bool) (-(((/* implicit */int) var_7))))))));
}
