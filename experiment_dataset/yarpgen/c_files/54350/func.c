/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54350
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54350 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54350
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 17; i_0 += 2) 
    {
        for (unsigned short i_1 = 0; i_1 < 17; i_1 += 2) 
        {
            {
                var_20 = ((/* implicit */unsigned long long int) (~(((/* implicit */int) var_11))));
                var_21 = ((/* implicit */short) min((var_21), (((/* implicit */short) max((((/* implicit */long long int) min((((/* implicit */short) var_11)), (arr_0 [2])))), (max((((/* implicit */long long int) arr_3 [i_0] [i_1])), (min((var_16), (((/* implicit */long long int) var_3)))))))))));
            }
        } 
    } 
    var_22 = max((var_17), (((/* implicit */long long int) min((var_10), (var_10)))));
}
