/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77184
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77184 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77184
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        var_18 = 235;
        var_19 = (min(var_19, ((((((((!(arr_1 [1]))) ? (arr_1 [2]) : (arr_1 [16])))) && 20)))));
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        var_20 *= (!456509146);
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 14;i_3 += 1)
            {
                {
                    var_21 = ((-(1881590401 <= 235)));
                    var_22 *= ((((((((var_10 >= (arr_10 [i_3]))))) + (((arr_3 [i_3]) * (arr_0 [i_2]))))) << (((((min(235, (arr_7 [i_3] [1] [i_3]))))) - 24))));
                }
            }
        }
    }
    /* vectorizable */
    for (int i_4 = 0; i_4 < 11;i_4 += 1)
    {
        arr_13 [i_4] = var_8;
        /* LoopNest 2 */
        for (int i_5 = 1; i_5 < 9;i_5 += 1)
        {
            for (int i_6 = 2; i_6 < 8;i_6 += 1)
            {
                {
                    var_23 = var_4;
                    var_24 = var_5;
                }
            }
        }
        var_25 = var_16;
    }
    var_26 = ((-(min((var_14 * 1), ((var_2 ? var_16 : var_0))))));
    #pragma endscop
}
