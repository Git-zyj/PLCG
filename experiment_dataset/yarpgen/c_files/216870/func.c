/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 216870
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=216870 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/216870
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned char) max((min((((/* implicit */unsigned long long int) (signed char)117)), (var_5))), (((/* implicit */unsigned long long int) (~((-(var_13))))))));
    var_16 = ((/* implicit */unsigned long long int) max(((-(((/* implicit */int) (signed char)-118)))), (var_11)));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 2; i_0 < 11; i_0 += 1) 
    {
        arr_3 [i_0] = ((/* implicit */long long int) var_11);
        arr_4 [i_0] = ((/* implicit */unsigned int) ((((/* implicit */_Bool) arr_1 [i_0 + 2] [i_0])) || (((/* implicit */_Bool) var_7))));
    }
}
