/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89238
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89238 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89238
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 -= var_0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_18 -= (~((((!56027) != (!var_16)))));
                arr_4 [i_1] [i_0] = (max((((56027 ? (arr_3 [i_0] [i_1]) : var_2))), (max(((9508 ? var_10 : var_11)), (max((arr_2 [i_1] [i_1] [i_0]), (arr_1 [i_0] [i_1])))))));
                arr_5 [i_1] [i_1] = (((arr_1 [i_0] [i_0]) ? ((((arr_1 [i_1] [i_0]) == var_1))) : (~var_12)));
                arr_6 [i_1] [i_0] [0] = var_2;
            }
        }
    }
    #pragma endscop
}
