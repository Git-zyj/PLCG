/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 205574
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=205574 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/205574
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned short i_0 = 4; i_0 < 23; i_0 += 2) 
    {
        var_18 = ((/* implicit */signed char) (~(var_15)));
        var_19 = ((/* implicit */unsigned int) (-(((((/* implicit */_Bool) arr_0 [i_0 - 1])) ? (((/* implicit */unsigned long long int) ((/* implicit */int) arr_0 [i_0]))) : (((unsigned long long int) 16211520668014153052ULL))))));
        var_20 = ((/* implicit */_Bool) ((((/* implicit */_Bool) ((((/* implicit */int) (signed char)-25)) & (((/* implicit */int) (unsigned char)240))))) ? (((/* implicit */int) (signed char)-3)) : (((arr_1 [i_0 + 2]) & (((/* implicit */int) (short)-2462))))));
    }
    var_21 = ((/* implicit */unsigned long long int) var_9);
    var_22 = ((/* implicit */unsigned short) var_8);
    var_23 = ((/* implicit */unsigned short) min((var_23), (((/* implicit */unsigned short) var_13))));
    var_24 = (-(max((var_4), (((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) var_16))))))));
}
