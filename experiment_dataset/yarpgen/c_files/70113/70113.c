/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70113
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70113 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70113
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 *= min(16223965279445857525, 3184752430);
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_15 = (min(var_15, ((max(var_11, (((!(((5542183224489838730 ? 3365390790 : (arr_2 [i_1] [i_0]))))))))))));
                var_16 = (min(var_16, ((min((max(var_6, var_6)), ((var_11 >> (((~var_11) + 196)))))))));
            }
        }
    }
    #pragma endscop
}
