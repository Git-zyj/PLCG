/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221136
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221136 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221136
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = -var_11;
    var_16 = ((~(min((min(var_9, -4241373085271096447)), var_11))));

    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {

        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            var_17 = (min((min((arr_0 [i_1 + 2]), var_10)), (!0)));
            var_18 = ((((((arr_4 [i_0] [i_1 + 2] [1]) ? (max((arr_2 [i_1]), var_7)) : 0))) ? var_10 : (var_1 && -1)));
        }
        arr_5 [i_0] = ((((((max(0, var_5))))) >= ((((var_7 ^ (arr_2 [i_0]))) >> ((((arr_0 [i_0]) <= var_10)))))));
    }
    #pragma endscop
}
