/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120526
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120526 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120526
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_19 -= (+-88604982);
        arr_2 [i_0] = -1483708247;
        var_20 = var_7;
        var_21 = 1483708247;
        arr_3 [1] [1] &= 123145302310912;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_22 &= var_14;
        arr_6 [i_1] [i_1] = var_7;
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 10;i_2 += 1)
    {
        arr_9 [i_2] = (!5755679381852037494);
        var_23 = (8907645955349869945 > var_12);
    }
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 24;i_3 += 1)
    {
        for (int i_4 = 2; i_4 < 23;i_4 += 1)
        {
            {
                arr_15 [i_3] [i_4 - 2] = (((max(var_18, 30187))));
                arr_16 [i_3] = ((!(32767 < var_3)));
            }
        }
    }
    var_24 |= var_18;
    var_25 = (max(var_25, (225462002139117487 || 713181175)));
    #pragma endscop
}
