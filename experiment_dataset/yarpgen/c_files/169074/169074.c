/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169074
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169074 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169074
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((min(var_3, (min(var_12, var_7)))) <= var_4));
    var_17 = (min((min(var_7, var_15)), ((min((6923019746865836542 <= 0), var_1)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_18 = (min((arr_2 [i_0]), ((min((arr_4 [i_0]), (arr_4 [i_1]))))));
                var_19 &= (min(((min((arr_3 [i_0] [i_1] [i_1]), (arr_3 [i_0] [i_0] [8])))), (arr_2 [i_0])));
            }
        }
    }
    #pragma endscop
}
