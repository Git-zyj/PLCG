/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60717
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60717 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60717
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = 1;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    arr_9 [i_0] = (max((((arr_8 [i_0] [i_0] [i_0]) * (arr_8 [i_0] [i_1 + 3] [i_2]))), var_2));
                    var_19 *= var_14;
                    arr_10 [i_0] [i_2] [i_2] = ((max(4164406226, (arr_4 [i_1 - 1] [i_1] [i_1 - 1]))));
                    var_20 = (max(var_20, (((~((1369991932 ? (arr_6 [i_1 + 1] [i_2] [i_2] [i_2]) : 1)))))));
                    var_21 = ((~(32667 % -14240)));
                }
            }
        }
    }
    var_22 = var_12;
    var_23 ^= (((--14700) ? var_15 : (~14702)));
    #pragma endscop
}
