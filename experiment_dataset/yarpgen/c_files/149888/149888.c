/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149888
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149888 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149888
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_4;
    var_18 = 2124587413;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 20;i_1 += 1)
        {
            {
                var_19 = ((!((((arr_4 [i_1] [i_1 + 2]) ? (arr_4 [i_1] [i_1]) : (arr_4 [i_1] [i_1 + 2]))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 21;i_3 += 1)
                    {
                        {
                            var_20 = (max(var_20, 4000504448033550905));
                            arr_12 [i_0] [i_0] [i_1] [i_1] [i_2] [i_1] = (((((var_10 ? 30775 : (arr_4 [i_1] [14])))) ? (arr_0 [i_0] [i_1 + 1]) : (((var_14 ? 81 : 6)))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
