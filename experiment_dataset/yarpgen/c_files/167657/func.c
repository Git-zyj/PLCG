/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 167657
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=167657 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/167657
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */short) var_8);
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 22; i_0 += 3) 
    {
        arr_4 [i_0] = ((/* implicit */int) ((((/* implicit */_Bool) (-(((/* implicit */int) arr_2 [i_0] [i_0]))))) && (((/* implicit */_Bool) arr_0 [i_0] [i_0]))));
        var_19 = ((/* implicit */unsigned long long int) (~((~(((/* implicit */int) arr_1 [i_0] [i_0]))))));
        var_20 ^= ((/* implicit */_Bool) arr_0 [i_0] [i_0]);
        var_21 += ((/* implicit */unsigned long long int) max((((arr_0 [i_0] [i_0]) * (((arr_0 [i_0] [5U]) * (arr_0 [i_0] [i_0]))))), (((/* implicit */unsigned int) arr_2 [i_0] [i_0]))));
    }
}
