/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74452
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74452 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74452
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] = 4294967295;
                var_17 = var_1;
                var_18 = (min(var_18, ((min((arr_1 [i_0]), (!var_13))))));
                var_19 = (max((((!(!1)))), (arr_4 [i_0] [i_0] [i_1])));
                var_20 = max(((4294967274 ? (~4294967285) : ((1 ? 1 : 0)))), (arr_3 [i_1] [i_1] [i_0]));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 1;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 25;i_3 += 1)
        {
            {
                var_21 = (arr_8 [i_2]);
                var_22 = (arr_9 [i_2] [i_2] [i_3]);
            }
        }
    }
    #pragma endscop
}
