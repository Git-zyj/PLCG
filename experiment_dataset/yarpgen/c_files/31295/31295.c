/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 31295
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=31295 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/31295
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_1] = (((var_16 % (arr_3 [i_0 - 2]))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 13;i_3 += 1)
                    {
                        {
                            var_20 = ((~(((arr_10 [i_0] [i_3 - 2] [i_2] [i_3]) & (arr_10 [i_0] [i_3 - 3] [i_0] [2])))));
                            arr_11 [i_3] = ((((!var_8) * ((62443 ? var_0 : var_5)))));
                            arr_12 [6] [i_1] [i_2] [i_3 - 2] = (((min((!16), (!41255))) ? (((arr_10 [i_0 + 2] [13] [i_0 + 1] [i_0]) ^ (((var_14 ? (arr_4 [i_0]) : var_18))))) : (((~(arr_0 [i_2]))))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
