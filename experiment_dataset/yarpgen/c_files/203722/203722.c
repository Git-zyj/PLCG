/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203722
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203722 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203722
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_6;

    /* vectorizable */
    for (int i_0 = 3; i_0 < 14;i_0 += 1)
    {
        arr_3 [i_0 + 1] [i_0] = (!382795287);
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                for (int i_3 = 4; i_3 < 13;i_3 += 1)
                {
                    {
                        var_16 = (!2996);
                        arr_10 [i_0] [i_0] [i_2] [i_3] |= ((!(arr_9 [i_0] [i_1] [i_2] [i_0 + 1] [i_1] [i_3 + 2])));
                    }
                }
            }
        }
        var_17 = (((arr_6 [i_0 - 3] [i_0 - 3] [i_0 - 3]) ? (arr_0 [i_0] [i_0]) : (((arr_4 [i_0] [i_0]) ? 0 : 3912172008))));
    }
    #pragma endscop
}
