/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212975
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212975 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212975
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_5;

    for (int i_0 = 4; i_0 < 21;i_0 += 1)
    {
        var_16 = ((2147483647 ? -47 : 1932245952));
        var_17 &= (((var_6 ? (arr_1 [i_0 - 4]) : (arr_0 [i_0 + 3] [i_0 + 3]))));
        /* LoopNest 3 */
        for (int i_1 = 3; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 24;i_3 += 1)
                {
                    {
                        var_18 = ((((max((arr_5 [i_1] [i_0 - 1]), (min(var_1, (arr_6 [i_0] [i_0] [i_2] [i_0])))))) ? var_12 : (((((1 % var_1) && ((1 <= (arr_7 [i_0 - 4] [i_2])))))))));
                        var_19 = (arr_1 [i_0]);
                        var_20 = (((((max(var_7, 19316)) % 6049318183418353255))));
                    }
                }
            }
        }
    }
    #pragma endscop
}
