/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 101069
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=101069 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/101069
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 ^= ((/* implicit */short) max((-3197566791189562461LL), (1125899906842623LL)));
    /* LoopNest 3 */
    for (long long int i_0 = 4; i_0 < 19; i_0 += 4) 
    {
        for (long long int i_1 = 0; i_1 < 20; i_1 += 1) 
        {
            for (short i_2 = 0; i_2 < 20; i_2 += 4) 
            {
                {
                    var_13 = ((/* implicit */long long int) ((((/* implicit */int) (short)-19)) > (((/* implicit */int) (short)32762))));
                    var_14 = ((/* implicit */long long int) (!(((/* implicit */_Bool) min(((short)-32759), ((short)31801))))));
                }
            } 
        } 
    } 
    var_15 &= var_8;
}
