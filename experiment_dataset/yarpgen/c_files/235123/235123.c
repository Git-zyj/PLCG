/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235123
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235123 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235123
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 15;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] |= 16777216;
        arr_3 [7] [i_0] = (((arr_0 [i_0] [i_0]) <= (arr_0 [i_0] [i_0])));
        arr_4 [i_0] = (((arr_0 [i_0] [i_0]) ? ((~(arr_1 [i_0] [i_0]))) : (arr_1 [i_0] [i_0])));
        arr_5 [i_0] = (arr_0 [i_0] [i_0]);
    }
    for (int i_1 = 0; i_1 < 15;i_1 += 1) /* same iter space */
    {
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 15;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 15;i_3 += 1)
            {
                {
                    arr_15 [4] = (max((min((min((arr_1 [8] [i_1]), var_7)), 6109290261118394864)), 0));
                    arr_16 [i_1] [i_2] [i_2] [i_2] = var_0;
                }
            }
        }
        arr_17 [i_1] [i_1] = arr_14 [i_1] [i_1] [i_1] [i_1];
    }
    var_10 = var_7;
    var_11 = ((!(((var_2 ? var_6 : (((var_6 ? 0 : var_1))))))));
    #pragma endscop
}
