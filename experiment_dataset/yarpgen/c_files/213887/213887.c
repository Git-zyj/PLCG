/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213887
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213887 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213887
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_16 = (max((((arr_0 [i_0]) & var_15)), ((((arr_3 [i_0]) / var_11)))));
                arr_6 [i_0] = var_9;
            }
        }
    }
    var_17 = ((((max((-108 - 108), var_13))) ? 108 : var_13));
    var_18 = (((var_10 / var_4) ? var_2 : var_12));
    var_19 = (((((var_14 ? (var_2 <= var_8) : (min(var_11, var_1))))) && (var_13 ^ var_9)));
    #pragma endscop
}
