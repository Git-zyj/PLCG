/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 92453
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=92453 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/92453
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_3 [i_0] = (((0 ? 14063 : 14051)));
        var_15 = var_13;
        arr_4 [i_0] [i_0] = ((((((max(-29574, 4706))))) ? (((var_14 ? (((arr_0 [i_0] [i_0]) + var_12)) : (arr_0 [i_0] [i_0])))) : (arr_1 [5])));
        arr_5 [i_0] [i_0] = (arr_2 [i_0]);
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 18;i_1 += 1)
    {
        var_16 = (((arr_2 [i_1]) > (((-7 ? 107 : 1)))));
        arr_8 [i_1] |= (((arr_7 [i_1]) ? (arr_6 [i_1] [i_1]) : (arr_0 [i_1] [i_1])));
        arr_9 [i_1] [i_1] = arr_2 [i_1];
    }

    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        var_17 = (arr_10 [i_2]);
        arr_13 [i_2] [i_2] = (arr_11 [i_2]);
    }
    var_18 = var_11;
    #pragma endscop
}
