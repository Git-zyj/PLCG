/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102999
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102999 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102999
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 18;i_1 += 1)
        {
            {
                var_13 = (min(var_13, (arr_0 [i_1])));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_14 = (min(var_14, ((max(((var_12 ? (((max(0, var_12)))) : (min(var_4, (arr_9 [i_0]))))), (((((max(var_1, var_5))) ? var_1 : (!var_11)))))))));
                                var_15 = (arr_2 [i_1]);
                                arr_12 [i_0] [i_1] [i_2] [i_3] [10] = (arr_8 [i_0] [i_1] [i_1] [i_1 - 2]);
                                var_16 |= (arr_6 [i_1 - 1] [i_1 - 2] [i_4] [i_4]);
                            }
                        }
                    }
                }
            }
        }
    }
    var_17 = (~var_12);
    var_18 = 10744;
    var_19 ^= var_3;
    #pragma endscop
}
