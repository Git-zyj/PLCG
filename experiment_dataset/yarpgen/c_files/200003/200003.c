/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200003
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200003 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200003
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_19 -= ((((min(var_6, ((var_16 ? var_8 : 58758))))) - (max((50535 > var_7), ((var_13 ? 4066567055 : -32740))))));
                var_20 = ((((((3 / var_1) * (max(65535, var_9))))) ? 15000 : ((113 ? (((min(15001, -114)))) : 15196257425796576516))));
                var_21 = (max(4294967295, (arr_4 [11])));
            }
        }
    }
    var_22 = var_13;
    #pragma endscop
}
