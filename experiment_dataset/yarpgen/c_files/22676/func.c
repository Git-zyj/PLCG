/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22676
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22676 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22676
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */_Bool) max((var_15), ((!(((/* implicit */_Bool) max((((/* implicit */unsigned short) ((((/* implicit */_Bool) -4872114098980610742LL)) && (((/* implicit */_Bool) var_14))))), (var_13))))))));
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 17; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 17; i_1 += 4) 
        {
            {
                arr_6 [15ULL] [14ULL] = 4872114098980610731LL;
                arr_7 [i_1] = var_1;
            }
        } 
    } 
}
