/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114189
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114189 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114189
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (min(var_19, ((-7890802772816260660 ? 1212740859 : 0))));

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        arr_3 [i_0] = (min((((min(var_14, var_11)))), ((118 ? -1212740859 : -6442156257612712781))));
        var_20 &= (((129 | (arr_0 [i_0]))));
        var_21 = (arr_1 [i_0]);
    }
    var_22 &= (min(-7890802772816260660, var_11));
    var_23 = var_0;
    var_24 = var_8;
    #pragma endscop
}
