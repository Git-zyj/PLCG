/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230563
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230563 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230563
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */long long int) ((((/* implicit */int) (!(((/* implicit */_Bool) var_1))))) < (((/* implicit */int) var_8))));
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 12; i_0 += 2) 
    {
        var_21 *= ((/* implicit */signed char) max((((/* implicit */int) (unsigned short)32626)), ((-(((/* implicit */int) max(((unsigned char)0), (((/* implicit */unsigned char) (_Bool)0)))))))));
        var_22 ^= ((/* implicit */unsigned int) (_Bool)1);
        arr_2 [i_0] = ((/* implicit */signed char) (-(((/* implicit */int) var_1))));
        var_23 = ((/* implicit */short) (!((_Bool)0)));
    }
    var_24 += ((/* implicit */_Bool) var_17);
    var_25 = ((/* implicit */signed char) -3907452775619439516LL);
}
