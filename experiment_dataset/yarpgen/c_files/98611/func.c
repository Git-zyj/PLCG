/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98611
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98611 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98611
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 &= ((/* implicit */unsigned short) ((((/* implicit */_Bool) var_6)) && (((/* implicit */_Bool) var_18))));
    /* LoopNest 3 */
    for (short i_0 = 3; i_0 < 9; i_0 += 3) 
    {
        for (unsigned int i_1 = 3; i_1 < 9; i_1 += 3) 
        {
            for (unsigned short i_2 = 0; i_2 < 10; i_2 += 3) 
            {
                {
                    arr_9 [i_0] [i_1] &= ((/* implicit */short) 1630409768);
                    arr_10 [i_0] [i_1 - 2] [i_2] [i_1] = ((/* implicit */_Bool) (~((~(((((/* implicit */_Bool) var_6)) ? (((/* implicit */int) arr_4 [i_0 + 1])) : (((/* implicit */int) arr_3 [i_0] [i_1] [i_2]))))))));
                }
            } 
        } 
    } 
}
