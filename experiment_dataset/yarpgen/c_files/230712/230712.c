/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230712
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230712 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230712
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    var_10 = ((((var_2 || (arr_4 [i_0] [i_1] [15]))) && (arr_3 [i_0] [i_1 + 1])));
                    var_11 = ((((max(var_6, 32126) << (var_8 + 19659)))));
                    var_12 = (((arr_4 [i_1] [i_1] [5]) ? (((((65517 & (arr_4 [i_0] [i_1] [i_2])))) ? (min(var_0, var_2)) : (((arr_3 [i_0] [i_1]) ? 1859775575 : (arr_9 [i_1] [i_1] [4]))))) : ((min(1, 24058)))));
                }
            }
        }
    }
    var_13 = var_5;
    #pragma endscop
}
