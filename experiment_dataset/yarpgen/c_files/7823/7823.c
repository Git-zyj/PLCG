/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7823
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7823 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7823
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max((((!(~var_13)))), var_9));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    arr_7 [i_2] [i_0] [i_0] [i_0] = ((!(((arr_6 [i_0] [i_0] [i_0]) == (arr_6 [4] [4] [i_0])))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 10;i_4 += 1)
                        {
                            {
                                var_16 = (arr_11 [i_0]);
                                arr_13 [i_0] [i_1] [i_0] [i_3] [i_0] = var_2;
                                arr_14 [i_0] [i_1] [1] [i_3] [1] = (((arr_3 [i_4 - 2] [i_4]) == ((min((arr_9 [i_4 - 2]), (arr_6 [i_4 - 2] [i_4] [i_0]))))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_17 = (min(var_17, var_2));
    #pragma endscop
}
