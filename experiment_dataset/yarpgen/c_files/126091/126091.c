/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 126091
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=126091 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/126091
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_16, ((((~2041541886) <= var_10)))));

    for (int i_0 = 2; i_0 < 17;i_0 += 1) /* same iter space */
    {
        arr_2 [5] = -58;

        for (int i_1 = 3; i_1 < 18;i_1 += 1)
        {
            var_17 -= (arr_4 [i_0 - 1] [i_0 + 1] [i_0 + 1]);
            var_18 = (max(var_18, 1427779094));
            var_19 = (((((arr_1 [i_0 + 2]) / 2253425410))) ? (arr_4 [i_0] [i_0] [14]) : (arr_3 [i_1]));
        }
        for (int i_2 = 3; i_2 < 18;i_2 += 1)
        {
            var_20 = (8447471128762953681 ^ ((((var_0 - (arr_6 [1] [12]))))));
            var_21 = (min(var_21, ((((-127 - 1) ? (-8447471128762953683 / 507207674) : (((-14 ? -6 : var_5))))))));
        }
    }
    for (int i_3 = 2; i_3 < 17;i_3 += 1) /* same iter space */
    {
        arr_14 [12] |= var_2;
        /* LoopNest 2 */
        for (int i_4 = 0; i_4 < 19;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 1;i_5 += 1)
            {
                {
                    var_22 = var_4;
                    var_23 = ((1427779112 ? var_13 : -180));
                }
            }
        }
    }
    /* vectorizable */
    for (int i_6 = 2; i_6 < 17;i_6 += 1) /* same iter space */
    {
        arr_22 [7] [5] |= -var_5;
        var_24 = var_1;
    }
    var_25 = var_12;
    #pragma endscop
}
