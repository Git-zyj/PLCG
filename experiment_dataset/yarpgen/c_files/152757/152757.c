/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152757
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152757 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152757
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [9] = ((var_9 ? ((var_14 ? (arr_0 [i_1]) : (arr_2 [i_0]))) : (((arr_4 [i_0] [i_0]) >> (((arr_1 [i_0]) - 3231046671089489934))))));
                var_16 = (((((arr_5 [i_0]) ? 9223372036854775807 : 60335)) / var_3));
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 1; i_2 < 15;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 19;i_3 += 1)
        {
            for (int i_4 = 2; i_4 < 17;i_4 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 1;i_5 += 1)
                    {
                        for (int i_6 = 2; i_6 < 17;i_6 += 1)
                        {
                            {
                                var_17 = (min(var_17, var_8));
                                var_18 = (max((arr_11 [1]), (arr_9 [0])));
                            }
                        }
                    }
                    var_19 = (min(var_19, var_10));
                }
            }
        }
    }
    var_20 = var_2;
    #pragma endscop
}
