/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102737
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102737 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102737
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = -1;
    var_19 = -var_8;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    arr_9 [i_2] = ((((((min(var_16, var_15))) | ((max((arr_7 [i_0] [i_1] [i_0]), var_8)))))) - (min((var_2 != var_1), (max(var_8, var_14)))));
                    arr_10 [i_2] [i_2] [i_1] [i_0] = ((((max((max(var_10, (arr_0 [i_2]))), ((max((arr_3 [i_0]), (arr_3 [i_0]))))))) * ((min((127 || 127), (!4294967294))))));
                    var_20 = ((((!(arr_4 [i_0] [i_2])))));
                }
            }
        }
    }
    #pragma endscop
}
