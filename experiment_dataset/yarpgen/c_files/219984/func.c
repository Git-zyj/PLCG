/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219984
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219984 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219984
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */short) ((((/* implicit */_Bool) ((((/* implicit */_Bool) ((((/* implicit */_Bool) var_9)) ? (((/* implicit */unsigned int) ((/* implicit */int) var_6))) : (var_1)))) ? (((((/* implicit */_Bool) var_8)) ? (858068661U) : (var_5))) : (var_7)))) ? (((/* implicit */unsigned int) ((/* implicit */int) var_13))) : (var_9)));
    var_15 = ((/* implicit */unsigned char) var_6);
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 15; i_0 += 2) 
    {
        arr_2 [(short)0] = ((/* implicit */unsigned int) var_10);
        var_16 = var_9;
    }
}
