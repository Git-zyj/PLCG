/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180644
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180644 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180644
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((63463 ? var_4 : var_3));

    for (int i_0 = 0; i_0 < 22;i_0 += 1) /* same iter space */
    {
        var_13 = (((arr_1 [i_0] [i_0]) > (arr_1 [i_0] [i_0])));
        arr_2 [i_0] = (arr_1 [i_0] [10]);
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    var_14 = (min(var_14, (arr_6 [i_0] [10] [i_0])));
                    arr_9 [i_1] = -65523;
                    var_15 = var_11;
                }
            }
        }
    }
    /* vectorizable */
    for (int i_3 = 0; i_3 < 22;i_3 += 1) /* same iter space */
    {
        var_16 ^= (arr_7 [i_3] [i_3]);

        for (int i_4 = 0; i_4 < 22;i_4 += 1)
        {
            var_17 = ((arr_0 [i_4] [0]) ? (arr_0 [i_3] [i_3]) : (arr_14 [i_3] [i_4]));
            var_18 = (min(var_18, (((32 ? (arr_11 [i_3]) : (arr_11 [i_4]))))));
            var_19 = (arr_1 [i_3] [i_4]);
        }
        for (int i_5 = 0; i_5 < 22;i_5 += 1)
        {
            /* LoopNest 2 */
            for (int i_6 = 0; i_6 < 22;i_6 += 1)
            {
                for (int i_7 = 0; i_7 < 22;i_7 += 1)
                {
                    {
                        var_20 += ((~(arr_23 [i_3] [i_5] [i_7])));
                        arr_24 [i_3] [i_3] [i_6] [i_6] = (arr_5 [i_6] [5]);
                        var_21 *= -var_6;
                        var_22 = (max(var_22, ((((arr_19 [i_3] [8] [i_5]) || var_8)))));
                    }
                }
            }
            var_23 = 63463;
        }
    }
    #pragma endscop
}
