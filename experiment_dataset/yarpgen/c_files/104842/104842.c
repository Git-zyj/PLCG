/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104842
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104842 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104842
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max(var_15, var_7));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            {
                var_16 = (max(9223372036854775807, (max((arr_2 [i_0] [i_1 - 2] [6]), (arr_2 [i_0] [i_1 - 1] [i_1 - 1])))));
                var_17 -= ((~((1 | (-2147483647 - 1)))));
                var_18 |= ((!(arr_1 [i_0] [i_1])));
            }
        }
    }
    var_19 ^= (((~57282) ? (max(var_4, var_8)) : -var_7));
    var_20 = ((var_0 == (((((var_5 ? 6464597036888211981 : var_6))) ? var_7 : (var_7 ^ var_1)))));
    #pragma endscop
}
