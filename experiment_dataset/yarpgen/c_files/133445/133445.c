/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133445
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133445 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133445
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_0] = (((((+((8160 ? (-9223372036854775807 - 1) : 32750))))) ? (((var_8 ? var_1 : (arr_3 [i_0])))) : ((var_2 ? var_6 : (!var_1)))));
                arr_7 [i_0] = ((!(~var_3)));
                var_11 = ((((((arr_0 [i_0 - 2]) ? -1719030130122998211 : 1))) ? (~1) : ((var_2 ? (arr_1 [i_0 + 1] [i_0]) : (arr_0 [i_0 + 1])))));
            }
        }
    }
    var_12 = var_5;
    var_13 = ((var_2 ? (~7932931034619373702) : var_9));
    #pragma endscop
}
