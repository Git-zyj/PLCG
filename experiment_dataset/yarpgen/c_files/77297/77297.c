/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77297
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77297 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77297
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (var_8 / var_7);

    /* vectorizable */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        arr_3 [i_0] = (((((-(arr_0 [i_0 + 1]))) + 2147483647)) << (((1 % 4) - 1)));
        arr_4 [i_0] = 1;
    }
    for (int i_1 = 0; i_1 < 12;i_1 += 1)
    {
        var_15 = ((3 ? 233 : 233));
        var_16 = (min(var_16, (((~(((arr_7 [i_1]) >> (2936163490218829348 - 2936163490218829333))))))));
    }
    for (int i_2 = 0; i_2 < 10;i_2 += 1)
    {
        arr_10 [i_2] [i_2] = (min((!31), (((arr_7 [1]) || var_5))));
        var_17 = (min(var_17, (((((((10078 << (4472 - 4472)))) != (arr_6 [i_2])))))));
    }
    var_18 = var_5;
    var_19 = var_2;
    var_20 = var_0;
    #pragma endscop
}
