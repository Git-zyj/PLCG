/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163899
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163899 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163899
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 ^= ((/* implicit */unsigned char) var_9);
    var_15 = ((/* implicit */long long int) max((var_15), (((/* implicit */long long int) ((((/* implicit */_Bool) var_1)) || (((/* implicit */_Bool) min((((/* implicit */short) (_Bool)0)), ((short)14730)))))))));
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 13; i_0 += 2) 
    {
        for (int i_1 = 2; i_1 < 11; i_1 += 4) 
        {
            {
                arr_6 [i_0] = ((/* implicit */unsigned long long int) var_3);
                var_16 = ((/* implicit */unsigned char) max((((((/* implicit */int) (short)-16487)) - (((/* implicit */int) (_Bool)1)))), ((-((+(arr_2 [i_0] [(unsigned char)2])))))));
            }
        } 
    } 
    var_17 = ((/* implicit */int) var_3);
}
