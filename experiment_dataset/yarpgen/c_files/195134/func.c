/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195134
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195134 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195134
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = 164375698U;
    /* LoopNest 3 */
    for (short i_0 = 4; i_0 < 9; i_0 += 4) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 10; i_1 += 4) 
        {
            for (unsigned char i_2 = 0; i_2 < 10; i_2 += 1) 
            {
                {
                    /* LoopNest 2 */
                    for (unsigned short i_3 = 0; i_3 < 10; i_3 += 4) 
                    {
                        for (long long int i_4 = 0; i_4 < 10; i_4 += 4) 
                        {
                            {
                                var_14 = ((/* implicit */long long int) (+((-(((/* implicit */int) var_9))))));
                                arr_14 [i_0 + 1] = ((/* implicit */long long int) arr_8 [i_4] [i_4] [i_2] [i_3] [i_4] [i_3]);
                            }
                        } 
                    } 
                    arr_15 [i_2] = ((/* implicit */signed char) ((((/* implicit */unsigned int) ((/* implicit */int) (!(((/* implicit */_Bool) (~(((/* implicit */int) (short)-12263))))))))) * (((unsigned int) (signed char)-121))));
                    /* LoopNest 2 */
                    for (short i_5 = 0; i_5 < 10; i_5 += 3) 
                    {
                        for (_Bool i_6 = 0; i_6 < 1; i_6 += 1) 
                        {
                            {
                                arr_23 [6LL] [i_1] [i_5] = ((/* implicit */unsigned int) max(((!(arr_12 [i_0] [i_0 + 1] [i_0 - 4] [i_0] [i_0 - 1]))), ((!((!((_Bool)0)))))));
                                var_15 = ((/* implicit */long long int) (-(arr_11 [i_0] [i_1] [5LL] [i_5] [i_6])));
                            }
                        } 
                    } 
                }
            } 
        } 
    } 
}
