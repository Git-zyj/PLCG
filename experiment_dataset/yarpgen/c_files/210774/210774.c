/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210774
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210774 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210774
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                var_11 = (arr_3 [i_0 + 1]);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 16;i_3 += 1)
                    {
                        {
                            var_12 = ((-(arr_2 [i_1] [i_1])));
                            var_13 = ((!(arr_6 [i_3 + 1] [i_1 + 1] [i_1 + 1] [i_1 + 2] [i_1 + 2] [i_1 + 2])));
                        }
                    }
                }
                var_14 = var_6;
                /* LoopNest 2 */
                for (int i_4 = 0; i_4 < 1;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 19;i_5 += 1)
                    {
                        {
                            arr_12 [i_4] [i_4] [i_4] [1] &= arr_9 [i_1 + 2] [i_1 + 2] [i_0 + 4];
                            var_15 &= ((((min(24, (arr_3 [i_0 - 1])))) ? (arr_3 [i_0 + 4]) : (arr_3 [i_0 - 1])));
                        }
                    }
                }
            }
        }
    }
    var_16 *= ((var_0 > (var_3 - -var_9)));
    #pragma endscop
}
