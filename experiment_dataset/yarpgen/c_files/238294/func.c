/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 238294
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=238294 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/238294
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 |= ((/* implicit */signed char) var_6);
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 0; i_0 += 1) 
    {
        for (short i_1 = 0; i_1 < 13; i_1 += 2) 
        {
            for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
            {
                {
                    /* LoopSeq 1 */
                    for (long long int i_3 = 0; i_3 < 13; i_3 += 4) 
                    {
                        var_11 -= ((/* implicit */signed char) 3534854694965792126LL);
                        /* LoopSeq 1 */
                        /* vectorizable */
                        for (_Bool i_4 = 0; i_4 < 1; i_4 += 1) 
                        {
                            arr_10 [i_0] [i_2] [i_2] [8ULL] [i_0] = 32767LL;
                            var_12 = ((/* implicit */_Bool) min((var_12), (((/* implicit */_Bool) -32768LL))));
                            var_13 = ((/* implicit */unsigned long long int) max((var_13), (((/* implicit */unsigned long long int) (_Bool)0))));
                            var_14 = ((/* implicit */long long int) max((var_14), (((/* implicit */long long int) (_Bool)1))));
                        }
                        arr_11 [i_1] [i_2] [i_1] [9U] [12LL] [i_1] = ((/* implicit */unsigned short) (_Bool)1);
                    }
                    arr_12 [i_2] [i_2] = ((/* implicit */unsigned short) (_Bool)1);
                }
            } 
        } 
    } 
    var_15 = (_Bool)1;
    var_16 = ((/* implicit */_Bool) min((var_16), (((/* implicit */_Bool) var_3))));
    var_17 = ((/* implicit */unsigned long long int) (_Bool)0);
}
