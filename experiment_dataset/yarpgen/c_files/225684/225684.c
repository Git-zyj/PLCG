/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 225684
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=225684 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/225684
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((((!(((3200773773719131306 ? 25892 : var_15)))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_0] [i_1] [i_2] = ((((var_10 && (((var_4 ? 255 : var_14))))) ? var_15 : ((var_0 ? var_0 : var_11))));
                    arr_8 [i_1] = ((var_7 ? ((681 ? (arr_5 [i_2] [i_1] [i_0]) : var_12)) : var_11));
                }
            }
        }
    }
    #pragma endscop
}
