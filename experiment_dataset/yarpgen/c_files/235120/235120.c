/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235120
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235120 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235120
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 22;i_2 += 1)
            {
                {
                    var_10 = (max((((!(arr_5 [i_0] [i_2 + 2] [i_1 - 2])))), (max((arr_2 [i_0] [i_2 + 1]), 1005994762381347009))));
                    arr_10 [i_0] [i_1] [i_2] [i_0] = (227135856699403281 - -27);
                    arr_11 [i_0] [i_0] [i_0] = -0;
                    arr_12 [i_0] = (max(85, (min(227135856699403293, 141))));
                }
            }
        }
    }
    var_11 = ((16472149079999177872 & (((min(9772, -33))))));
    #pragma endscop
}
