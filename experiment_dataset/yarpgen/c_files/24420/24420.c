/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24420
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24420 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24420
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            {
                var_17 |= (((arr_3 [i_0] [i_1 + 1]) ? (((((max((arr_2 [4] [i_1]), 9024079316475655051))) || -3215854383267756776))) : var_6));
                var_18 = (((!(arr_0 [i_1 - 1] [i_1 - 2]))) ? ((((min(-977401965, 247))) ? (((!(arr_3 [i_0] [i_0])))) : (arr_2 [20] [21]))) : (((arr_0 [i_1 - 1] [17]) * (!var_7))));
            }
        }
    }
    var_19 = var_11;
    var_20 *= ((!(((var_9 ? (!var_4) : -var_11)))));
    var_21 = var_13;
    #pragma endscop
}
