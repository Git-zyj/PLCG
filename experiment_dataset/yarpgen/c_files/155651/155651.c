/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155651
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155651 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155651
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_20 = (((((((min((arr_0 [i_0]), (arr_7 [i_1]))) != (arr_2 [i_0] [i_1]))))) < 15912941322061372177));
                    var_21 = (1 < 2533802751648179438);
                }
            }
        }
    }
    var_22 = (((((var_3 ? var_7 : 1))) ? 2533802751648179441 : 15912941322061372191));
    #pragma endscop
}
