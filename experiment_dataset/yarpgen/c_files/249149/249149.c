/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249149
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249149 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249149
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_10;
    var_18 = ((var_5 ? var_3 : var_7));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_19 = (max(var_19, var_12));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 0;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            {
                                var_20 *= ((0 ? (arr_0 [i_4] [i_4]) : (arr_7 [i_0] [i_0] [i_0] [i_2 + 1] [i_3] [8])));
                                var_21 = var_11;
                                var_22 = (min(var_22, (((((max(var_2, var_11))) ? ((-(arr_5 [i_2 + 1] [i_2 + 1] [i_2 + 1] [i_2 + 1]))) : (arr_1 [7] [i_1]))))));
                            }
                        }
                    }
                }
                var_23 = (+((((arr_7 [i_0] [i_0] [i_1] [i_1] [i_1] [i_1]) >= (arr_7 [i_0] [1] [i_0] [1] [i_1] [i_1])))));
                var_24 ^= 0;
            }
        }
    }
    var_25 = (~var_10);
    #pragma endscop
}
