/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 141972
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=141972 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/141972
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_18 = (((((((min(var_7, (arr_3 [i_0 + 3] [i_0] [i_0])))) - (((arr_4 [13]) + var_4))))) ? (((65517 - var_3) ? ((max(18, -32760))) : 19)) : (((((32760 ? 32767 : var_6))) ? (arr_3 [i_0] [i_0] [i_0]) : 65497))));
                arr_5 [i_0 - 4] &= (~var_6);
            }
        }
    }
    var_19 = (max(var_19, (((var_3 ? (((((min(15515, 1))) || ((min(4260354650, var_9)))))) : var_8)))));
    var_20 = (!var_16);
    #pragma endscop
}
