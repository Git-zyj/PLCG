/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233582
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233582 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233582
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_11 = (max(-1415614384, 1415614383));
        arr_2 [i_0] [i_0] = ((-(((arr_1 [i_0 - 1]) ? (arr_1 [i_0 - 1]) : (arr_1 [i_0 - 1])))));
    }
    for (int i_1 = 0; i_1 < 25;i_1 += 1)
    {
        arr_5 [i_1] = (+-1184084630);
        var_12 = (max(var_12, (((!(arr_0 [i_1]))))));
        var_13 = (min((arr_0 [i_1]), (arr_4 [i_1])));
    }
    var_14 = ((max(((30808 ? -1184084631 : 4294967268), var_2))));
    var_15 = var_8;
    var_16 = var_6;
    #pragma endscop
}
