/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 225628
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=225628 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/225628
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (!15);
    var_20 = (max(var_20, ((((max((~var_5), var_11)) <= var_3)))));
    var_21 = var_17;

    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = ((((max(var_9, (-551754705735609659 ^ -551754705735609636)))) ? (var_6 & -551754705735609636) : 4294967290));
        var_22 &= 161;
        arr_3 [i_0] = ((((551754705735609623 || (arr_1 [i_0]))) || (((34765 ? (arr_1 [i_0]) : 65535)))));
    }
    #pragma endscop
}
