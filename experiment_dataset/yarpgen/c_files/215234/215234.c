/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215234
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215234 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215234
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_4;
    var_18 ^= var_15;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    var_19 = (arr_3 [i_2]);
                    var_20 = (min(var_20, (max(((max(((var_15 ? var_14 : (arr_1 [i_0]))), (arr_6 [i_0] [i_1])))), 758067656564506970))));
                    var_21 = (+(((arr_2 [i_1]) - var_3)));
                    var_22 = ((((((var_6 ? 1 : var_3)))) < (~var_8)));
                }
            }
        }
    }
    var_23 = max(4926632833966255830, 17);
    #pragma endscop
}
