/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110418
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110418 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110418
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, ((((var_2 | -var_8) ? ((((var_6 ? var_1 : var_8)))) : var_0)))));
    var_11 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_12 = (((((224 ? 32 : 0))) ? ((-(((arr_1 [i_0]) - (arr_1 [i_0]))))) : (arr_2 [i_0])));
                var_13 = (max(var_13, ((((((((((!(arr_4 [i_0]))))) + ((15 ? var_3 : var_4))))) ? ((((((var_6 ? (arr_1 [8]) : (arr_2 [0]))) != (!-1))))) : (max((~8232678046622813994), 8232678046622813994)))))));
            }
        }
    }
    #pragma endscop
}
