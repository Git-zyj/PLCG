/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194309
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194309 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194309
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned int i_1 = 0; i_1 < 11; i_1 += 4) 
        {
            {
                var_10 = ((/* implicit */_Bool) (signed char)19);
                arr_4 [i_1] = ((/* implicit */signed char) ((arr_2 [i_0] [i_1]) ^ (((arr_2 [i_0] [(signed char)2]) | (arr_2 [i_0] [i_1])))));
            }
        } 
    } 
    var_11 = ((/* implicit */unsigned long long int) (-(((/* implicit */int) (signed char)36))));
    var_12 = ((/* implicit */unsigned char) var_0);
}
