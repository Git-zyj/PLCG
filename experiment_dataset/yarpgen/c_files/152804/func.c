/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152804
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152804 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152804
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 12; i_0 += 1) 
    {
        arr_3 [i_0] = ((/* implicit */signed char) ((unsigned char) arr_1 [i_0]));
        arr_4 [i_0] = ((/* implicit */unsigned long long int) arr_1 [i_0]);
        arr_5 [i_0] = ((/* implicit */unsigned char) ((long long int) max((((/* implicit */unsigned int) var_14)), (arr_2 [i_0]))));
    }
    var_15 = ((/* implicit */int) min((var_15), (((/* implicit */int) ((unsigned long long int) max(((+(((/* implicit */int) var_12)))), (((/* implicit */int) var_13))))))));
}
