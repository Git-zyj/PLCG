/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94301
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94301 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94301
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((!((max((((var_15 >> (var_8 + 6040)))), (min(var_6, var_12)))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 12;i_2 += 1)
            {
                {
                    var_19 = ((max(0, 6094)));
                    var_20 = ((min(16429724161412098352, -117)));
                    var_21 ^= (((((arr_3 [i_0]) ? (~var_4) : ((max(var_10, -4678650063280795843)))))) ? (arr_6 [i_1 + 1]) : (arr_5 [i_0] [i_0]));
                }
            }
        }
    }
    #pragma endscop
}
