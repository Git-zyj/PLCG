/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157745
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157745 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157745
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((((min(var_2, var_2))) ? var_17 : var_16));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_19 -= ((((((1048568 || (arr_8 [i_2] [i_1] [i_2] [i_0]))) ? (4293918708 / var_13) : (((~(arr_2 [i_2]))))))) ? ((((max(33554431, (arr_6 [i_0])))) ? (arr_8 [i_0 - 1] [i_0] [i_0 - 1] [i_0 - 1]) : (((arr_2 [i_1]) | 127)))) : (((arr_2 [i_1]) ? (arr_2 [i_0 - 1]) : var_13)));
                    var_20 -= (arr_3 [i_0] [i_2]);
                }
            }
        }
    }
    #pragma endscop
}
