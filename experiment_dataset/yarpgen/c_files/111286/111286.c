/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111286
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111286 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111286
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_14 = (max(var_14, (96 - 6006)));
                var_15 = (min(var_15, (arr_5 [i_1] [i_1])));
                var_16 = ((((!(arr_1 [i_0 - 3]))) ? (((arr_1 [i_0 - 4]) ^ (arr_1 [i_0 + 1]))) : (((arr_1 [i_0 - 3]) >> (((arr_1 [i_0 - 3]) - 99))))));
            }
        }
    }
    var_17 = ((var_12 ? (((var_7 ? ((3052389972 ? -728695021 : var_4)) : var_8))) : ((var_5 ? ((-6008 ? 1 : 1242577324)) : (((var_4 ? var_13 : var_3)))))));
    var_18 = var_7;
    #pragma endscop
}
