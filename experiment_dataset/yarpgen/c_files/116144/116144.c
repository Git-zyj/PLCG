/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116144
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116144 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116144
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(((var_10 & (-84 && 4294967295))), (((!((min(-84, -1048576))))))));
    var_15 = var_1;
    var_16 = (max(var_16, 83));

    for (int i_0 = 4; i_0 < 15;i_0 += 1)
    {
        var_17 = (min(((((arr_1 [i_0 + 4]) && (arr_1 [i_0 + 3])))), (((arr_1 [i_0 - 4]) ? var_4 : 6))));
        arr_4 [i_0] = (var_2 <= -7);
        var_18 = (max(var_18, (arr_2 [i_0] [i_0])));
        var_19 = ((~(((arr_1 [i_0 - 3]) ? (max(134217664, (-127 - 1))) : (arr_1 [i_0])))));
    }
    var_20 = var_3;
    #pragma endscop
}
