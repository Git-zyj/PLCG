/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112435
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112435 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112435
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = -2433;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    var_15 = -65535;
                    var_16 = ((min((min(32, 5502401719132157041)), (min((-9223372036854775807 - 1), (arr_3 [i_0]))))));
                    var_17 = (min(var_17, (((((max((arr_6 [1] [i_2 - 1] [i_2 - 1]), -10080))) + ((var_1 ? ((min((-2147483647 - 1), 8))) : (max((arr_2 [i_0] [i_0]), 9128149501277888584)))))))));
                    arr_10 [i_0] [i_0] = (arr_9 [i_0] [i_0] [i_0]);
                }
            }
        }
    }
    #pragma endscop
}
