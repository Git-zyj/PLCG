/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184638
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184638 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184638
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_16 = (min(var_16, (arr_2 [i_0] [i_0])));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    arr_7 [i_0] [9] [i_0] = (arr_3 [i_0] [i_0]);
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 6;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 10;i_4 += 1)
                        {
                            {
                                arr_14 [i_0] [7] [i_2] [2] [4] = (arr_0 [i_0]);
                                arr_15 [i_0] [i_0] [i_2] [i_1] [i_0] = var_2;
                                var_17 ^= var_12;
                            }
                        }
                    }
                    arr_16 [i_0] [i_0] [7] [i_0] = (arr_4 [i_0 - 1] [i_0 - 1] [i_0 - 1]);
                    arr_17 [i_0] = (((arr_5 [i_0 - 1]) ? (arr_5 [i_0 - 1]) : (arr_5 [i_0 - 1])));
                }
            }
        }
    }
    var_18 = (min((37 || 12), var_14));
    #pragma endscop
}
