/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 153911
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=153911 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/153911
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_19 = ((((arr_3 [i_1 + 1]) ? ((min((arr_6 [i_0] [i_0]), (arr_3 [i_0])))) : (((arr_1 [i_0] [12]) ? (arr_4 [i_0] [i_0] [i_0]) : (arr_2 [0] [i_1]))))));
                var_20 = (((((arr_4 [i_0] [i_1 + 1] [i_0]) ? (((arr_1 [i_0] [i_1]) ? (arr_6 [i_0] [1]) : var_11)) : var_2)) ^ 33));
            }
        }
    }
    var_21 = var_16;
    var_22 = (max(var_22, 1));

    for (int i_2 = 0; i_2 < 1;i_2 += 1)
    {
        var_23 = (+-1);
        var_24 = ((var_13 ? ((+(((arr_8 [i_2]) ? var_15 : 33)))) : (((max(1, -65))))));
    }
    #pragma endscop
}
