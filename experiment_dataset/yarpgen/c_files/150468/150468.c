/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150468
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150468 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150468
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 20;i_2 += 1)
            {
                {
                    arr_8 [i_0] [18] [i_2] [10] = ((-255 ? ((((max((arr_4 [i_0] [i_0] [i_0]), (arr_7 [i_0] [i_1 + 2] [i_0] [i_2])))) ? 3094957899279364937 : (arr_7 [i_0] [i_1] [i_2] [i_1]))) : 2147483647));
                    arr_9 [i_0] [i_0] [i_1] [i_0] = (arr_3 [i_2]);
                }
            }
        }
    }
    var_19 = min(((min(-23746, (min(48, 0))))), var_14);
    #pragma endscop
}
