/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34178
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34178 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34178
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((((((181 ? 153 : 0)) - (var_5 - var_2)))) ? (var_3 >= var_15) : -var_11));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = arr_0 [i_0];
                var_17 = (max(var_17, (arr_4 [0])));
                arr_6 [i_0] [i_1] [i_1] = (min((((65535 ? 1653662925 : -1))), (max((((arr_1 [i_0]) ? (arr_2 [i_0] [i_1]) : var_13)), (var_8 / var_6)))));
                arr_7 [i_0] = ((max((((var_3 ? var_15 : var_4))), (((arr_2 [i_0 - 1] [i_0 - 1]) ? var_6 : (arr_0 [i_0]))))));
            }
        }
    }
    #pragma endscop
}
