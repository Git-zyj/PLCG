/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60650
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60650 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60650
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */long long int) (+(((/* implicit */int) var_4))));
    var_16 ^= ((/* implicit */unsigned short) ((-1) + (((var_13) / ((~(((/* implicit */int) var_11))))))));
    var_17 |= ((/* implicit */unsigned short) ((((/* implicit */_Bool) (signed char)-55)) ? (var_6) : (((/* implicit */long long int) ((/* implicit */int) (unsigned short)31986)))));
    var_18 |= ((/* implicit */int) var_6);
    /* LoopSeq 2 */
    for (long long int i_0 = 1; i_0 < 16; i_0 += 1) 
    {
        arr_3 [i_0] [i_0] = var_0;
        arr_4 [i_0] = ((/* implicit */signed char) min((((/* implicit */int) (!(((/* implicit */_Bool) arr_2 [i_0 - 1]))))), (min((-1), (((/* implicit */int) (signed char)-106))))));
    }
    for (long long int i_1 = 1; i_1 < 13; i_1 += 2) 
    {
        var_19 -= ((/* implicit */long long int) var_12);
        arr_9 [i_1] = ((/* implicit */signed char) ((long long int) (!(((/* implicit */_Bool) var_13)))));
    }
}
