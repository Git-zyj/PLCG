/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138485
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138485 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138485
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((var_9 + var_6) / -var_14));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            {
                var_20 = (((arr_1 [i_0]) ? ((~(218 | 1))) : (~37706)));
                var_21 = ((var_16 ? 229 : (((arr_2 [i_0 + 1] [i_1 + 1] [i_1]) ? (arr_2 [i_0 - 2] [i_1 - 1] [i_0 - 2]) : (arr_2 [i_0 - 2] [i_1 + 1] [i_1 - 1])))));
            }
        }
    }
    var_22 = (min(var_5, (min((min(984637, var_1)), var_18))));
    #pragma endscop
}
