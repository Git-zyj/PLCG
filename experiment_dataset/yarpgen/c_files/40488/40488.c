/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40488
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40488 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40488
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_12 = (min(var_12, (((((((arr_0 [i_0 - 1]) ? var_4 : (arr_0 [i_0 - 1])))) ? (max(4294967295, var_4)) : var_4)))));
        var_13 = (-23027 / 14837366764776844080);
        var_14 = (((arr_1 [5]) ? (arr_2 [i_0]) : (arr_2 [i_0])));
    }
    var_15 = var_7;
    var_16 = var_2;
    var_17 = (((((~var_9) % var_9))) ? var_2 : (!107));
    #pragma endscop
}
