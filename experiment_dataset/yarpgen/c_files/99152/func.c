/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99152
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99152 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99152
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 11; i_0 += 3) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_16 = ((/* implicit */int) ((signed char) arr_0 [i_0]));
                arr_6 [i_1] [i_0] [i_0] = ((/* implicit */unsigned int) (signed char)51);
            }
        } 
    } 
    var_17 = ((/* implicit */signed char) (~((~(min((((/* implicit */unsigned int) (_Bool)1)), (var_1)))))));
    var_18 = ((/* implicit */_Bool) var_7);
    var_19 *= ((/* implicit */short) max((var_8), (((/* implicit */long long int) (~(2622339456U))))));
}
