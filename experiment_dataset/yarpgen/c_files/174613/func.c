/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174613
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174613 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174613
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */_Bool) ((((/* implicit */_Bool) min(((+(var_5))), (((/* implicit */unsigned int) var_1))))) ? ((-(((((/* implicit */unsigned long long int) ((/* implicit */int) var_10))) + (19ULL))))) : (((/* implicit */unsigned long long int) var_6))));
    var_12 = ((/* implicit */unsigned long long int) var_0);
    /* LoopSeq 1 */
    for (_Bool i_0 = 1; i_0 < 1; i_0 += 1) 
    {
        var_13 += ((/* implicit */int) arr_0 [8ULL]);
        arr_3 [i_0] [i_0] = ((/* implicit */unsigned short) var_5);
    }
    var_14 = ((/* implicit */unsigned int) max((var_14), (((((/* implicit */_Bool) var_1)) ? (((/* implicit */unsigned int) ((/* implicit */int) var_0))) : (var_5)))));
}
