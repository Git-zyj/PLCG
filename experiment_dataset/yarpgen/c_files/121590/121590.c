/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 121590
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=121590 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/121590
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [i_0] [12] = ((var_6 < (arr_2 [i_0] [i_1] [i_1])));
                arr_5 [15] [i_1] = ((+(max((arr_1 [i_0] [1]), var_5))));
                arr_6 [i_0] = (min((min(((min(4073819645, 3291972824))), (arr_3 [i_0]))), (arr_0 [i_1])));
            }
        }
    }
    var_16 = var_4;
    var_17 = ((var_0 + ((max((var_15 | var_6), (((var_12 + 2147483647) >> (-1 + 29))))))));
    #pragma endscop
}
