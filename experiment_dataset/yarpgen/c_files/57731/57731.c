/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 57731
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=57731 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/57731
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_12 -= ((1 ? ((((-206944894 && (min(1, 1)))))) : -6021441160807093119));
        var_13 *= ((((arr_1 [i_0] [i_0]) ? (arr_0 [i_0]) : 206944894)));
        arr_2 [i_0] = ((((-((32376 ? var_2 : (arr_1 [i_0] [i_0]))))) - ((min((arr_0 [i_0]), (arr_0 [i_0]))))));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_14 = (max((8877200271775587383 * 32381), (!-8)));
                    arr_7 [i_1] [i_0] [i_0] [i_2] &= 33156;
                }
            }
        }
    }
    var_15 = var_2;
    #pragma endscop
}
