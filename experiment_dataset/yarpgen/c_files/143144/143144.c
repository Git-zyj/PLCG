/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143144
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143144 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143144
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 ^= ((~(((var_16 || var_16) ? 65408 : var_12))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    var_21 *= (!(((((arr_2 [i_0] [i_0]) || 66584576)) || (1 == 28596))));
                    var_22 = (((min((arr_3 [i_0]), (arr_3 [i_0]))) << ((((var_19 ? (arr_3 [i_0]) : (arr_3 [i_2]))) - 6746428176426805161))));
                    var_23 = ((!((((120 && var_15) - (arr_2 [i_2] [3]))))));
                    arr_8 [i_0] [9] [i_1] [i_2] = 8764696694917994526;
                }
            }
        }
    }
    #pragma endscop
}
