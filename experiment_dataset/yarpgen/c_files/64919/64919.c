/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64919
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64919 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64919
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 16;i_2 += 1)
            {
                {
                    var_11 = (min(var_11, (arr_4 [i_2 - 4] [4] [i_2 - 4])));
                    var_12 = (max(var_12, ((((arr_4 [i_0] [12] [i_0]) || ((max(((196810230 ? 29912 : -28225)), ((min((arr_1 [i_2 - 1]), (arr_6 [i_0] [i_0] [i_2 - 2]))))))))))));
                    arr_9 [i_0] [i_1] [i_0] = ((((var_8 * (arr_2 [i_2 - 3]))) * (arr_5 [i_2 - 2])));
                }
            }
        }
    }
    var_13 = var_0;
    #pragma endscop
}
