/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70464
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70464 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70464
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (signed char i_0 = 2; i_0 < 17; i_0 += 3) 
    {
        for (signed char i_1 = 1; i_1 < 20; i_1 += 1) 
        {
            {
                var_17 += ((/* implicit */unsigned char) ((((/* implicit */unsigned int) ((/* implicit */int) (!((_Bool)1))))) != (arr_2 [i_0] [i_1 + 1])));
                var_18 |= ((/* implicit */signed char) var_0);
                arr_5 [i_1] = ((((((((/* implicit */_Bool) var_14)) ? (arr_0 [i_1 + 1] [i_0]) : (((/* implicit */unsigned int) ((/* implicit */int) var_14))))) - (((/* implicit */unsigned int) ((/* implicit */int) var_13))))) % (((/* implicit */unsigned int) ((/* implicit */int) ((short) arr_2 [i_0] [i_0])))));
            }
        } 
    } 
    var_19 |= ((/* implicit */unsigned short) var_6);
}
