/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 113379
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=113379 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/113379
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (((max((max(var_7, var_8)), var_8)) ^ var_7));
    var_14 = ((var_4 ? (-1326789588 - var_5) : (((var_5 ? (var_2 > var_4) : var_10)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            {
                arr_5 [i_0] = (((((((arr_2 [i_0] [i_1 - 1]) < (arr_3 [i_0]))) ? var_0 : (max(var_8, (arr_2 [i_0] [i_1]))))) < ((((((arr_0 [i_0] [i_1 + 2]) / (arr_4 [i_0] [i_0])))) ? (~var_11) : ((var_9 ? var_12 : var_1))))));
                arr_6 [i_0] = var_6;
            }
        }
    }
    #pragma endscop
}
