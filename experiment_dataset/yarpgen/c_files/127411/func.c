/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127411
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127411 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127411
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned int) (!(var_5)));
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 2; i_0 < 17; i_0 += 2) 
    {
        var_17 = (((!(((/* implicit */_Bool) ((((/* implicit */_Bool) (unsigned short)43749)) ? (((/* implicit */int) (unsigned short)21786)) : (((/* implicit */int) (unsigned short)43750))))))) ? (((/* implicit */unsigned long long int) ((/* implicit */int) (!(((/* implicit */_Bool) var_4)))))) : (((((/* implicit */_Bool) (unsigned short)43749)) ? (((/* implicit */unsigned long long int) ((/* implicit */int) arr_2 [i_0 - 2]))) : (var_3))));
        var_18 = ((/* implicit */short) min((var_18), (((/* implicit */short) var_8))));
        arr_3 [3ULL] = ((/* implicit */signed char) (-((+((-(((/* implicit */int) (_Bool)0))))))));
    }
}
