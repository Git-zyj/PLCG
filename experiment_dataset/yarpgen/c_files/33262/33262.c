/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33262
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33262 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33262
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((((var_3 || 9614502444984848946) << var_3))) | 959702347));
    var_13 = var_0;
    var_14 = (var_10 ? ((3335264965 ? 959702355 : 1)) : (43 & var_6));
    var_15 = 1711;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_16 = ((!((max((arr_2 [i_0]), var_2)))));
                var_17 = (max((((((1711 ? -1 : -112)) != 1))), ((58 ? 1 : 2))));
                arr_5 [i_1] = ((var_1 & ((~(~42)))));
                arr_6 [i_0] = ((((arr_2 [i_0]) ? (arr_0 [i_1]) : 1)));
            }
        }
    }
    #pragma endscop
}
