/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 107758
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=107758 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/107758
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_3;
    var_16 &= var_12;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 15;i_0 += 1) /* same iter space */
    {
        var_17 = (arr_0 [i_0]);
        arr_2 [i_0] [i_0] = ((var_6 ? (((arr_1 [i_0] [i_0]) >> (var_5 - 2070089107781306477))) : (arr_0 [i_0])));
        var_18 = ((-28 ? ((var_6 ? (arr_0 [i_0]) : var_2)) : var_8));
        var_19 *= ((~((2147483647 ? (arr_1 [i_0] [i_0]) : (arr_1 [i_0] [i_0])))));
    }
    for (int i_1 = 0; i_1 < 15;i_1 += 1) /* same iter space */
    {
        var_20 = (32767 ^ 0);
        var_21 = (max(var_21, (((+(min((arr_3 [i_1]), var_5)))))));
    }
    #pragma endscop
}
