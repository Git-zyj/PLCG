/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21337
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21337 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21337
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((((((var_3 == var_8) ? (max(var_1, 12363719848917486850)) : var_3))) && var_12));

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 19;i_3 += 1)
                {
                    {
                        var_15 ^= ((((((32066 == -28878) << var_4))) ? var_1 : ((var_0 ? var_11 : (32080 & 6083024224792064766)))));
                        var_16 = 12363719848917486850;
                    }
                }
            }
        }
        var_17 = 8885;
    }
    /* vectorizable */
    for (int i_4 = 0; i_4 < 13;i_4 += 1)
    {
        arr_11 [i_4] [i_4] = ((var_5 >= (!var_11)));
        arr_12 [i_4] = 522240;
        var_18 = -0;
    }
    #pragma endscop
}
