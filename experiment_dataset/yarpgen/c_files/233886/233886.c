/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233886
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233886 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233886
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        var_19 = ((((1 ? -18092 : 4197011986243876038))));
        var_20 = ((~(max((arr_0 [i_0 + 2]), (min(var_16, var_2))))));
        var_21 = (arr_3 [i_0]);
        var_22 = ((18092 ? (arr_1 [i_0 + 2]) : ((-((105 >> (var_14 - 29351)))))));
        var_23 = ((((((arr_1 [i_0]) ? (arr_0 [i_0]) : 2915467050)) >= (arr_0 [i_0]))));
    }
    var_24 = (min((113 && var_14), ((var_18 ? 14882 : 15761169109844449509))));
    #pragma endscop
}
