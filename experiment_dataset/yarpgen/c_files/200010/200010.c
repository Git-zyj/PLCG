/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200010
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200010 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200010
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = -4088;
    var_13 = (17 < 81);
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    for (int i_3 = 4; i_3 < 22;i_3 += 1)
                    {
                        {
                            arr_8 [i_3] [i_0] [i_1] [i_3 + 1] [i_2] |= (~4146934277);
                            arr_9 [i_0] [i_0] [i_1] [i_1] [i_3] &= ((((arr_4 [i_0 - 2] [i_2] [i_3 - 3]) && (((-(arr_7 [i_3] [i_1] [i_2])))))));
                            arr_10 [i_0] [i_2] [i_1] [i_0 - 1] = var_4;
                        }
                    }
                }
                var_14 = var_8;
                var_15 *= 3452361351;
                var_16 = (((min(((0 ? -10809578 : (arr_6 [i_0] [i_0 - 2] [i_1] [i_1]))), -2)) - var_5));
                arr_11 [i_1] = var_2;
            }
        }
    }
    var_17 = var_3;
    #pragma endscop
}
