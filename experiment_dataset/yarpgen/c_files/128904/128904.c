/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 128904
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=128904 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/128904
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((((88 ? 50 : 856953253) <= var_3)))));
    var_12 = var_5;

    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (min((((arr_0 [i_0 + 1] [i_0 + 2]) - (arr_1 [i_0 + 1]))), (((arr_0 [i_0 + 1] [i_0 + 2]) - (arr_1 [i_0 + 1])))));
        var_13 += ((((min((-123 > 1), (arr_1 [i_0])))) > ((((-9223372036854775807 - 1) <= var_10)))));
    }
    #pragma endscop
}
