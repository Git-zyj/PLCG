/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106875
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106875 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106875
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_19 += ((!(arr_0 [i_0 - 1])));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    var_20 += (((arr_1 [i_0]) ? (arr_1 [i_0 - 1]) : (arr_2 [i_0] [16])));
                    var_21 = (max(var_21, (~103)));
                    var_22 -= (!688634965);
                    var_23 *= (arr_7 [12] [12] [i_0 - 1] [12]);
                }
            }
        }
        arr_8 [i_0 - 1] = (((arr_4 [i_0] [i_0 - 1]) - (((arr_4 [i_0] [i_0]) - 113))));
        arr_9 [i_0] = (((arr_1 [i_0 - 1]) ? (arr_2 [1] [1]) : ((((arr_2 [i_0] [i_0]) >= (arr_3 [i_0] [i_0]))))));
    }
    var_24 += var_15;
    var_25 -= var_18;
    #pragma endscop
}
