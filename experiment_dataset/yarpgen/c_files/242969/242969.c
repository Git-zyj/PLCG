/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242969
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242969 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242969
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_6 [14] = (var_5 <= (((-(arr_1 [i_0])))));
                arr_7 [i_0] [i_0] = (~var_9);
                var_20 = ((((min(1, (((arr_3 [i_0]) >> (var_19 - 13052)))))) ? var_3 : 8));
            }
        }
    }
    var_21 = (max(-11084051232000539496, ((var_6 ? var_12 : (max(-107316348, var_5))))));
    var_22 = (min(var_22, (((var_17 ? (var_4 | var_18) : 34477)))));
    #pragma endscop
}
