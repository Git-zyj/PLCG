/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231814
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231814 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231814
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned int) 2912249575311739900ULL);
    var_16 = ((/* implicit */long long int) (unsigned char)66);
    /* LoopNest 2 */
    for (unsigned int i_0 = 1; i_0 < 21; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 22; i_1 += 3) 
        {
            {
                var_17 = ((/* implicit */unsigned char) 15534494498397811715ULL);
                var_18 = ((/* implicit */signed char) 15534494498397811716ULL);
            }
        } 
    } 
}
