/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 147277
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=147277 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/147277
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        arr_2 [i_0] = (((((~((~(arr_0 [i_0])))))) ? ((((min((arr_1 [i_0] [i_0]), (arr_0 [i_0])))) ? (((max((arr_0 [i_0]), (arr_0 [i_0]))))) : ((-(arr_1 [i_0] [i_0]))))) : (arr_0 [i_0])));
        arr_3 [i_0] [i_0] = arr_0 [i_0];
    }
    var_17 = (((((-var_15 ? (~var_13) : ((var_2 ? var_4 : var_2))))) ? var_12 : (min(-var_12, ((max(var_6, var_1)))))));
    var_18 |= var_2;
    var_19 = (((((min(var_14, var_3)) ? var_14 : (min(var_8, var_11))))));
    #pragma endscop
}
