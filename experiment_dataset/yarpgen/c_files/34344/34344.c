/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34344
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34344 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34344
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (max(((((((36253 ? var_6 : var_15))) ? var_14 : var_1))), (max(var_8, var_1))));
    var_19 = var_12;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_20 = ((arr_0 [i_0]) ? (((arr_4 [i_0] [i_1] [i_0]) ? (arr_4 [6] [i_1] [i_0]) : (arr_2 [i_1] [i_0] [i_0]))) : ((~(max((arr_2 [i_0] [i_0] [i_1]), var_11)))));
                arr_5 [i_0] [i_0] [i_0] = (min((((~(arr_0 [i_1])))), ((var_11 ? (arr_1 [i_0]) : (!var_17)))));
            }
        }
    }
    #pragma endscop
}
