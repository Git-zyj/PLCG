/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 223430
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=223430 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/223430
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        var_17 = var_0;
        arr_3 [i_0] = 18;
    }
    for (int i_1 = 4; i_1 < 23;i_1 += 1)
    {
        var_18 = ((var_12 * (arr_5 [i_1 - 4])));
        arr_7 [i_1] [i_1 + 1] = (min((arr_4 [i_1]), ((var_13 ? (~var_6) : 84459208))));
        arr_8 [2] [i_1] = ((-(max(var_1, (4734079270500790413 + var_7)))));
    }
    var_19 = ((((((var_8 && 9223372036854775807) ? var_0 : var_1))) ? 7499684939129233214 : var_5));
    var_20 = var_11;
    var_21 ^= var_15;
    #pragma endscop
}
