/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45875
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45875 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45875
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 20;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] [4] [i_2] = 3884805500;
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 21;i_4 += 1)
                        {
                            {
                                var_12 = (max(var_12, 26183));
                                arr_13 [i_2] [i_1 - 1] [i_2 + 3] [i_1 + 1] [i_4 + 1] = -28753;
                                arr_14 [i_0] &= 28753;
                                arr_15 [i_0] [1] [i_2] [i_2] [i_0] [i_4] = 4644086766780744027;
                                arr_16 [i_0] [i_1] [i_2 + 3] [i_3] [8] = 4294967295;
                            }
                        }
                    }
                }
            }
        }
    }
    var_13 = 17635511952417430441;
    #pragma endscop
}
