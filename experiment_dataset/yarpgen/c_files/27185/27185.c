/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27185
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27185 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27185
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_3;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 10;i_2 += 1)
            {
                {
                    var_20 = (arr_1 [i_0] [i_1]);
                    var_21 = (min((((arr_3 [i_2] [i_1] [i_2]) / 742066306)), (arr_1 [i_0] [i_0])));
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 12;i_3 += 1)
    {
        for (int i_4 = 1; i_4 < 9;i_4 += 1)
        {
            {
                var_22 = (min((((arr_3 [i_3] [i_4 + 3] [i_4 - 1]) ? (arr_3 [i_4] [i_4 + 3] [i_4 - 1]) : var_9)), (((-(arr_3 [8] [i_4 + 3] [i_4 - 1]))))));
                /* LoopNest 3 */
                for (int i_5 = 0; i_5 < 12;i_5 += 1)
                {
                    for (int i_6 = 1; i_6 < 11;i_6 += 1)
                    {
                        for (int i_7 = 0; i_7 < 12;i_7 += 1)
                        {
                            {
                                var_23 = ((var_0 ? ((-87 ? 742066306 : 1)) : (arr_8 [i_6] [i_6] [i_5] [i_3])));
                                var_24 = (((arr_4 [i_3]) | var_4));
                            }
                        }
                    }
                }
            }
        }
    }
    var_25 = var_8;
    #pragma endscop
}
