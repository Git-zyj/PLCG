/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 19118
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=19118 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/19118
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */short) max((var_14), (var_10)));
    /* LoopSeq 1 */
    /* vectorizable */
    for (short i_0 = 0; i_0 < 16; i_0 += 3) 
    {
        var_15 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) var_4))));
        var_16 = ((/* implicit */unsigned short) ((((/* implicit */_Bool) (unsigned short)60909)) ? (((/* implicit */int) (unsigned short)4641)) : (((/* implicit */int) (unsigned short)4624))));
        var_17 = ((/* implicit */_Bool) 1862386591);
    }
}
