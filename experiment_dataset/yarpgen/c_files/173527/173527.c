/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 173527
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=173527 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/173527
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (-127 - 1);
    var_16 = (max(var_1, 2982350260294619834));

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        var_17 = ((((max(105, 37615))) || (arr_1 [i_0] [i_0])));
        arr_2 [i_0] [i_0] = ((((((var_10 > (arr_0 [3]))) ^ (min(var_4, var_1))))));
    }
    /* LoopNest 2 */
    for (int i_1 = 1; i_1 < 7;i_1 += 1)
    {
        for (int i_2 = 1; i_2 < 8;i_2 += 1)
        {
            {
                arr_7 [i_1] [i_1] = ((~((2982350260294619834 ? 2982350260294619813 : 81))));
                arr_8 [2] |= (((-(max((arr_0 [i_1]), 2982350260294619834)))));
            }
        }
    }
    var_18 = var_1;
    #pragma endscop
}
