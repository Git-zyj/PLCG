/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219032
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219032 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219032
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(((((min(61, var_5))) ? var_1 : var_7)), var_3));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 21;i_1 += 1)
        {
            {

                /* vectorizable */
                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    arr_9 [i_2] = var_3;
                    var_11 = (arr_3 [i_2]);
                }
                /* LoopNest 3 */
                for (int i_3 = 0; i_3 < 23;i_3 += 1)
                {
                    for (int i_4 = 3; i_4 < 21;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 1;i_5 += 1)
                        {
                            {
                                var_12 = (max(var_12, (((~(min(((var_8 ? (-9223372036854775807 - 1) : var_7)), 63)))))));
                                var_13 = ((~((-((var_6 * (arr_14 [i_5] [i_0] [i_3])))))));
                            }
                        }
                    }
                }
                var_14 = (min(var_14, (~71)));
            }
        }
    }
    var_15 ^= var_4;
    #pragma endscop
}
