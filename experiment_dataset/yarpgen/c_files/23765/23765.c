/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23765
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23765 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23765
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((min(14136, ((var_0 >> (-5287272314301921868 + 5287272314301921921))))));

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        arr_4 [i_0] = (max(((((((var_9 ? var_8 : 32767))) && (((var_0 ? var_13 : 51408)))))), (min((14128 * var_6), 28672))));
        var_19 = (((~135) <= (min(-121, (65520 / 30)))));
    }
    #pragma endscop
}
