/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164657
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164657 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164657
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 24;i_2 += 1)
            {
                {
                    var_18 = ((var_12 ? ((-var_6 ? (~var_12) : var_2)) : (~var_17)));
                    var_19 = ((~(arr_4 [i_0] [i_1] [i_2 + 1])));
                    arr_9 [i_0] [i_1] = (((-32767 - 1) + (!var_1)));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 25;i_4 += 1)
                        {
                            {
                                var_20 = ((((((-1586383296 + 2147483647) << (9223372036854775807 - 9223372036854775807)))) || (!var_12)));
                                var_21 = ((max((0 / 17), (arr_12 [i_2 - 1] [i_2 - 1] [i_3]))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_22 = ((~((~(var_17 - 32767)))));
    #pragma endscop
}
