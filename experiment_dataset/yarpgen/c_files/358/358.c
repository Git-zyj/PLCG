/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 358
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=358 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/358
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_9;

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        arr_2 [i_0] &= (arr_1 [i_0]);
        arr_3 [11] = min((~var_3), (((arr_1 [i_0]) ? ((((arr_1 [i_0]) ? (arr_0 [1]) : 1))) : ((1 ? 1 : 1223425657744553420)))));
        arr_4 [i_0] = 707218166;
        arr_5 [i_0] = (arr_1 [i_0]);
        arr_6 [i_0] = (-112 == 98);
    }
    var_20 = 17223318415964998196;
    var_21 = (((var_5 | 2250628015) ? (((((var_2 << (var_1 - 17688)))) ? (max(17223318415964998196, 1)) : var_11)) : var_17));
    #pragma endscop
}
