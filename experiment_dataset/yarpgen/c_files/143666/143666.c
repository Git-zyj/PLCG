/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143666
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143666 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143666
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [i_0 + 1] [i_0 + 3] [i_0 + 1] = ((var_8 ? ((((!var_9) && (((var_6 ? var_3 : (arr_3 [i_1] [i_1] [i_1]))))))) : (!0)));
                arr_5 [7] [i_0] |= var_4;
            }
        }
    }
    var_12 = var_2;
    var_13 = ((var_11 ? var_2 : ((((var_0 ? var_11 : var_10)) | (var_3 - var_3)))));
    var_14 = var_11;
    var_15 = ((var_2 ? (((~var_11) >> var_9)) : (((var_7 / var_2) ? var_9 : (var_11 * var_9)))));
    #pragma endscop
}
