/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102235
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102235 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102235
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = -3417014031170904418;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        {
                            var_17 = (((3417014031170904409 == ((max(0, (arr_1 [i_1])))))) ? (arr_0 [i_2] [i_2 - 1]) : (((~((0 >> (var_14 + 14746)))))));
                            var_18 = (min(var_18, ((((((!(arr_1 [i_1])))) * (1 == 3417014031170904418))))));
                        }
                    }
                }
                var_19 = (max((min(var_4, 239)), ((((var_9 / var_9) < (arr_5 [i_1] [i_1]))))));
            }
        }
    }
    #pragma endscop
}
