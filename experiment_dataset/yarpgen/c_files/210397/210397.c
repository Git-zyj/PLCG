/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210397
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210397 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210397
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        var_15 = 571957152676052992;
        var_16 = (max(var_16, (((var_11 ? ((((~10) || (arr_0 [i_0])))) : (arr_3 [i_0]))))));
    }
    var_17 = (max(((((!-122) || (~var_9)))), var_13));
    var_18 |= (min((((var_0 - var_8) - (0 + 126))), ((-((17530311962986718648 ? var_1 : var_4))))));
    var_19 = (~var_10);
    #pragma endscop
}
