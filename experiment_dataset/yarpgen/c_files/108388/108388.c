/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 108388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=108388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/108388
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 23;i_1 += 1)
        {
            {
                arr_6 [i_0] = (((((((0 && 252) && ((((arr_1 [i_0]) ^ (arr_0 [i_1] [i_0])))))))) <= ((((((arr_4 [i_0]) | (arr_0 [i_0] [i_1])))) ? ((((arr_3 [i_0] [i_1]) << (((((arr_4 [i_0]) + 901565658)) - 23))))) : (((arr_4 [i_0]) ^ (arr_0 [i_1] [i_0])))))));
                arr_7 [i_0] = (arr_1 [8]);
            }
        }
    }
    var_16 = (((((var_15 || var_7) ? (((1 ? var_4 : 341394166))) : ((var_15 ? var_15 : var_1)))) >= var_6));
    var_17 = var_8;
    #pragma endscop
}
