/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236775
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236775 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236775
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */unsigned short) 4044476983736132326LL);
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 0; i_0 < 16; i_0 += 3) 
    {
        var_14 = (signed char)-95;
        var_15 = ((/* implicit */unsigned char) (signed char)44);
        arr_3 [i_0] [i_0] = ((/* implicit */unsigned short) 16166291470967293664ULL);
        /* LoopNest 2 */
        for (unsigned long long int i_1 = 2; i_1 < 14; i_1 += 3) 
        {
            for (unsigned short i_2 = 4; i_2 < 15; i_2 += 2) 
            {
                {
                    arr_9 [i_0] [i_2] [i_0] = ((/* implicit */_Bool) (short)32767);
                    var_16 -= ((/* implicit */long long int) (signed char)127);
                }
            } 
        } 
    }
}
