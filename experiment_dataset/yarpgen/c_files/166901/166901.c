/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166901
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166901 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166901
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min((+-434712922), var_5));
    var_19 = var_3;
    var_20 = (max(var_20, var_3));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    var_21 = (min(var_21, (18542 > -6294738440563110364)));
                    var_22 = (min(var_22, ((min((max((arr_6 [5]), ((119 ? 0 : 10675017824304626059)))), (-94 * 3669551324))))));
                    arr_8 [i_0] [i_1] [i_1] [i_2] = ((~((~(arr_0 [i_0 - 1])))));
                }
            }
        }
    }
    #pragma endscop
}
