/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 193645
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=193645 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/193645
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    var_12 = (1146819761118072414 ? 6191807754124941823 : 18126084481567689107);
                    arr_8 [i_0] [i_0] [i_0] = var_4;
                    arr_9 [i_0] [i_0] [i_0 + 1] [i_0] = (arr_6 [15] [i_0 + 1] [15] [i_2]);
                }
            }
        }
    }
    var_13 = var_9;
    var_14 = (((max(var_8, (var_3 + 320659592141862509))) * 18126084481567689107));
    #pragma endscop
}
