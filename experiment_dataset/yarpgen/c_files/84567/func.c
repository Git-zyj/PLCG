/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84567
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84567 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84567
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned int) min((var_12), (((/* implicit */unsigned int) ((((unsigned long long int) var_1)) == (((/* implicit */unsigned long long int) ((/* implicit */int) var_10))))))));
    var_13 = ((/* implicit */unsigned long long int) var_8);
    /* LoopNest 3 */
    for (long long int i_0 = 0; i_0 < 17; i_0 += 3) 
    {
        for (unsigned int i_1 = 4; i_1 < 16; i_1 += 2) 
        {
            for (unsigned long long int i_2 = 0; i_2 < 17; i_2 += 1) 
            {
                {
                    var_14 = ((/* implicit */long long int) ((9223372036854775783LL) > (((/* implicit */long long int) 251752838U))));
                    var_15 = ((((unsigned long long int) arr_1 [i_1 - 4] [i_1])) * (((/* implicit */unsigned long long int) 0U)));
                }
            } 
        } 
    } 
    var_16 = ((/* implicit */unsigned long long int) min((var_16), (((/* implicit */unsigned long long int) var_9))));
}
