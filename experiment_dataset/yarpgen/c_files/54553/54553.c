/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54553
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54553 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54553
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [i_0] = var_13;
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_16 = (max(var_16, -var_15));
                                var_17 -= (2320105067 == 1934816139);
                            }
                        }
                    }
                }
            }
        }
    }
    var_18 = var_13;
    /* LoopNest 2 */
    for (int i_5 = 0; i_5 < 17;i_5 += 1)
    {
        for (int i_6 = 2; i_6 < 16;i_6 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_7 = 0; i_7 < 17;i_7 += 1)
                {
                    for (int i_8 = 0; i_8 < 17;i_8 += 1)
                    {
                        for (int i_9 = 2; i_9 < 16;i_9 += 1)
                        {
                            {
                                arr_32 [i_5] [i_6 - 2] [i_6] [i_9] [i_8] [i_8] [i_9 - 2] = -1;
                                var_19 = -2;
                                arr_33 [i_5] [i_9] [i_9 - 2] [i_8] = var_2;
                            }
                        }
                    }
                }
                arr_34 [i_5] [i_5] [i_5] = ((var_14 && ((((~var_10) ? var_10 : -30468)))));
                var_20 = ((var_3 ? (((~(var_12 - var_8)))) : ((((-2147483647 - 1) | 65535)))));
                arr_35 [i_5] [i_5] [i_5] = ((((min(var_4, var_13)))) - 2161727821137838080);
            }
        }
    }
    var_21 = var_2;
    #pragma endscop
}
