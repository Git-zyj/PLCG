/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174955
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174955 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174955
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 += ((~(((((-127 - 1) ^ var_10)) & (var_14 < var_4)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_21 = (arr_1 [i_0]);
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_22 = (min(var_22, (((~((-((max(0, 1))))))))));
                                var_23 = ((~((max((arr_8 [i_0] [0] [i_3] [i_4]), var_17)))));
                                var_24 = (((((!1073741822) ^ ((min(1, (arr_6 [6])))))) < 1));
                            }
                        }
                    }
                }
                var_25 = arr_6 [i_0];
                var_26 = (max(((((arr_0 [i_0]) != var_14))), (arr_2 [i_0])));
            }
        }
    }
    #pragma endscop
}
