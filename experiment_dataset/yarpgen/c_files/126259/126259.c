/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 126259
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=126259 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/126259
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 16;i_2 += 1)
            {
                {
                    var_10 += (arr_6 [i_0] [i_1] [1] [0]);
                    var_11 = (min(var_11, ((min((((arr_5 [6] [i_1] [i_2 + 1]) ? var_9 : (arr_5 [1] [i_1] [i_2 - 1]))), (arr_2 [i_0] [i_1] [7]))))));
                    var_12 = (~((-111 ? 1786818136 : var_5)));
                    var_13 ^= max((max((arr_4 [0]), (arr_4 [14]))), (max((arr_4 [10]), (arr_4 [0]))));
                }
            }
        }
    }
    var_14 = max(var_3, (var_6 | var_6));
    var_15 ^= (-46 * -53);
    var_16 = var_2;
    #pragma endscop
}
