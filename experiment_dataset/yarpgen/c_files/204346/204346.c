/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204346
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204346 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204346
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 14;i_2 += 1)
            {
                {
                    var_12 *= ((((30973 < 34581) ? (((arr_3 [10] [10]) - 6)) : (var_5 > var_7))));
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 11;i_4 += 1)
                        {
                            {
                                arr_11 [i_0] [i_1] [i_2] [i_2] [i_0] = (arr_2 [10] [i_1]);
                                var_13 *= ((var_1 ? (((arr_0 [8]) ? (arr_0 [4]) : (arr_9 [4] [i_2] [i_3] [i_4]))) : ((max(var_3, (max(var_7, var_9)))))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_14 = (min(var_14, var_0));
    #pragma endscop
}
