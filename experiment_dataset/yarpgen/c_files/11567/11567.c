/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11567
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11567 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11567
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 20;i_3 += 1)
                    {
                        {
                            var_18 = (min(-100, ((-((-8406 ? 1278263624153980794 : (arr_6 [i_1 + 1] [i_1 + 1] [i_1 + 1])))))));
                            arr_10 [i_0] [0] = (((((max((arr_1 [i_0 - 2] [i_0]), 519517587587550212)))) ? (arr_0 [i_2 - 1] [i_3]) : (((!(arr_9 [i_3] [i_3 - 2] [i_3] [i_3 - 2]))))));
                        }
                    }
                }
                var_19 = var_12;
            }
        }
    }
    var_20 = (max(var_20, var_9));
    var_21 = var_16;
    var_22 = (~var_17);
    var_23 = (min(0, 17693));
    #pragma endscop
}
