/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 81131
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=81131 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/81131
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_12;

    for (int i_0 = 1; i_0 < 24;i_0 += 1)
    {
        arr_4 [i_0] = (((!-1391627940) / ((((121 ? var_14 : var_19))))));
        var_21 = ((((max((arr_0 [24]), var_4))) && ((min((arr_3 [i_0] [i_0 - 1]), var_16)))));
        var_22 = (((+-2047) / -3075963618809551273));
        var_23 = -268435456;
        var_24 = (max(var_8, (max((!3991447257), ((var_13 ? -23320 : (arr_2 [i_0] [17])))))));
    }
    #pragma endscop
}
