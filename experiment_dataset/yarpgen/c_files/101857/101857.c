/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 101857
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=101857 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/101857
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((((min(var_16, var_2))) > (((((var_1 ? var_7 : var_3))) ? var_7 : ((var_16 ? var_6 : var_2))))));
    var_20 = var_11;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_21 ^= (max(3285525904321033877, (arr_3 [i_0])));
                    var_22 = (max(var_22, ((((max(((var_18 ? var_17 : (arr_3 [i_0]))), (var_5 + var_11))) | 0)))));
                    arr_9 [i_2] = (arr_4 [i_2] [i_2]);
                    var_23 += (((arr_2 [i_2]) ? ((max((arr_2 [i_0]), var_7))) : (arr_2 [i_0])));
                }
            }
        }
    }
    #pragma endscop
}
