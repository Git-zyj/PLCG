/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133742
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133742 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133742
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */unsigned long long int) (-(((/* implicit */int) ((var_11) < (((/* implicit */unsigned long long int) var_8)))))));
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 24; i_0 += 3) 
    {
        for (short i_1 = 0; i_1 < 24; i_1 += 1) 
        {
            {
                arr_5 [i_1] = ((/* implicit */unsigned short) arr_0 [i_0]);
                var_20 = ((/* implicit */unsigned short) (!((!(((_Bool) arr_4 [i_1] [10LL] [i_0]))))));
            }
        } 
    } 
    var_21 = ((/* implicit */unsigned char) var_7);
    var_22 = ((/* implicit */int) max((var_22), (((/* implicit */int) ((((/* implicit */_Bool) (+(var_14)))) ? (((/* implicit */unsigned long long int) (+(var_13)))) : ((-(var_15))))))));
}
