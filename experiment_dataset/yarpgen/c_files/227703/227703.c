/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227703
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227703 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227703
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 -= (((((((min(1, var_8))) ? (((var_13 ? var_10 : var_11))) : var_7))) ? var_16 : (var_10 ^ 1)));
    var_20 = var_10;
    var_21 = (min(var_21, var_8));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] = (((~1) ? var_10 : ((-var_7 * (((arr_2 [i_1]) ? (arr_1 [i_0]) : 1))))));
                var_22 = (((((arr_4 [i_0 + 2]) ? (arr_4 [i_0 + 2]) : (arr_4 [i_0 + 2]))) != (((arr_4 [i_0 + 2]) ? (arr_4 [i_0 + 2]) : var_8))));
                arr_6 [i_0] = 0;
                var_23 = ((~var_11) ? (~var_8) : ((min((arr_2 [i_0]), var_0))));
            }
        }
    }
    #pragma endscop
}
