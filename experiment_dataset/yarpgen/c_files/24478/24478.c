/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24478
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24478 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24478
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_14 = (+(((arr_1 [i_0]) % -4299955944540337125)));
        arr_2 [i_0] &= ((~(((-2490494716 <= (arr_1 [i_0]))))));
        arr_3 [i_0] = ((((max((arr_0 [i_0] [i_0]), (arr_0 [i_0] [i_0])))) ^ (arr_0 [i_0] [i_0])));
        var_15 -= (arr_1 [i_0]);
    }
    var_16 = ((var_5 ? ((var_6 ? ((63662 ? var_7 : var_7)) : (~1))) : var_4));
    #pragma endscop
}
