/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120362
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120362 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120362
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (var_2 > var_9);
    var_18 = ((0 && (((var_12 << (var_13 - 7117007490361599191))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_19 = 2695024097;
                var_20 = (max(var_20, (1599943212 % 7)));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        {
                            var_21 ^= 12288;
                            var_22 = (max(var_22, ((((((((34359738352 - 2090767272) - 34359738358)) + 9223372036854775807)) >> (34359738352 != 1599943198))))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
