/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33859
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33859 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33859
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_11 = (min(var_11, ((max((max(((var_5 * (arr_0 [i_0] [i_0]))), var_8)), ((((max((arr_3 [i_0] [16]), var_4)) / var_6))))))));
                arr_4 [10] [i_1] |= (arr_1 [i_1]);
            }
        }
    }
    var_12 = (min(var_12, ((min((((var_6 ? var_2 : var_3))), ((var_2 ? (max(var_10, var_8)) : var_7)))))));
    var_13 = (min(var_13, (((((max(var_8, var_6)) + (((7 ? 6 : 32767)))))))));
    #pragma endscop
}
