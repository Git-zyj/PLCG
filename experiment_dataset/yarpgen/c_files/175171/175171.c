/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175171
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175171 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175171
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 17;i_2 += 1)
            {
                {
                    arr_8 [i_1] [i_1] [i_2] = var_11;
                    var_12 = (((!var_1) % (min((arr_7 [i_2 + 2] [i_2 - 2] [i_2]), (arr_7 [i_2 + 2] [i_1] [i_0])))));
                    var_13 = (max(var_13, ((max((arr_1 [i_0] [2]), (arr_6 [i_0] [i_0] [i_0] [i_0]))))));
                }
            }
        }
    }
    var_14 = (min(var_14, ((min(((((var_11 % var_11) >= (max(var_11, 11419163072340333370))))), -101)))));
    #pragma endscop
}
