/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23273
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23273 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23273
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 &= ((var_0 ? (max(var_1, ((var_10 ? var_8 : var_4)))) : (max(((var_3 ? var_8 : var_5)), ((var_9 ? var_9 : var_5))))));
    var_13 ^= max(var_6, (min((min(1, 1)), 1)));

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_14 = arr_1 [i_0 - 1];
        var_15 = (min(((((((arr_1 [i_0]) ^ (arr_0 [1])))) ? (((arr_0 [i_0]) ? (arr_1 [1]) : (arr_0 [i_0]))) : (arr_0 [i_0]))), (arr_0 [i_0 - 1])));
    }
    #pragma endscop
}
