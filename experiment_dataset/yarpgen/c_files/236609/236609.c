/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236609
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236609 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236609
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min(var_13, (((var_9 ? var_8 : var_4)))));
    var_14 ^= ((var_12 ? var_0 : (((min(var_5, var_5))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_15 = (arr_1 [i_0]);
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 14;i_3 += 1)
                {
                    {
                        var_16 = 55004;
                        var_17 = (max(var_17, ((((arr_9 [i_0] [i_0] [i_0]) ? (arr_3 [i_0] [i_3] [i_2]) : (arr_2 [i_1]))))));
                    }
                }
            }
        }
        var_18 = (-8602394189204364806 ? (arr_3 [i_0] [i_0] [i_0]) : 50);
    }
    var_19 = (min(var_19, var_0));
    #pragma endscop
}
