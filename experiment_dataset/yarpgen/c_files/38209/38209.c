/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38209
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38209 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38209
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((var_13 - ((((((var_6 ? var_7 : (-2147483647 - 1)))) ? ((max(var_6, var_2))) : (~37284))))));

    /* vectorizable */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        arr_2 [i_0] = ((~(((arr_1 [1] [i_0]) + 62))));
        var_18 = (arr_0 [i_0 + 1]);
        var_19 = (((arr_1 [i_0 + 1] [i_0 - 2]) % 28250));
        var_20 = (arr_1 [i_0 + 3] [i_0 - 2]);
    }
    var_21 = ((var_4 ? (((!var_7) ? ((var_4 ? 6519485 : var_8)) : (!337941423))) : ((max(var_5, var_1)))));
    #pragma endscop
}
