/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 248866
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=248866 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/248866
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (((~(-2916199331872190780 && 10828442419609129440))) < var_16);
    var_18 = (((-2916199331872190780 + 9223372036854775807) << (((-1 + 25) - 24))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_19 = (~1207483064);
                var_20 = (min(var_20, (((((((arr_4 [i_1 - 1] [i_1 - 1] [i_1]) <= (var_12 != var_11))))) % (((arr_1 [i_1 - 1]) ? (min(1, -7803055754147736725)) : (arr_5 [i_1 - 1])))))));
            }
        }
    }
    var_21 = (min(var_21, var_5));
    #pragma endscop
}
