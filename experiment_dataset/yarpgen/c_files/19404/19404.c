/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 19404
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=19404 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/19404
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_11 = var_3;
                arr_8 [i_0] = ((var_5 < (((-(((arr_4 [i_0]) + 2147483642)))))));
            }
        }
    }
    var_12 = var_4;
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 16;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 16;i_3 += 1)
        {
            {
                arr_13 [i_2] [14] [11] = ((32 ? (((~(arr_12 [i_2])))) : (((var_0 ? ((var_2 ? 210 : 10215)) : (-32767 - 1))))));
                /* LoopNest 3 */
                for (int i_4 = 1; i_4 < 15;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 16;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 16;i_6 += 1)
                        {
                            {
                                var_13 = var_10;
                                var_14 *= 32;
                            }
                        }
                    }
                }
                var_15 = (min(var_15, var_8));
            }
        }
    }
    var_16 = ((var_8 * (19988 < 3673375414)));
    var_17 = (((((var_6 ? var_0 : var_1))) < ((var_8 ? var_9 : var_8))));
    #pragma endscop
}
