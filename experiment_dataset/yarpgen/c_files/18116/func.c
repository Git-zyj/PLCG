/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18116
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18116 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18116
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (signed char i_0 = 2; i_0 < 23; i_0 += 3) 
    {
        var_18 = ((/* implicit */unsigned long long int) 3439269223U);
        var_19 = ((/* implicit */_Bool) ((((/* implicit */_Bool) (+((-(((/* implicit */int) arr_1 [i_0] [i_0]))))))) ? (var_14) : (((/* implicit */unsigned long long int) (-(-1006493493))))));
        arr_3 [i_0] = ((/* implicit */short) (+((+((-(((/* implicit */int) var_15))))))));
    }
    var_20 = ((/* implicit */long long int) var_4);
}
