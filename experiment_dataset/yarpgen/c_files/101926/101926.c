/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 101926
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=101926 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/101926
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 23;i_0 += 1)
    {
        var_17 = var_3;
        arr_2 [i_0] [i_0] = (((arr_0 [i_0]) ? 125 : var_6));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 24;i_2 += 1)
            {
                {
                    var_18 &= ((((var_10 + (arr_4 [i_0] [2] [i_0])))) < (-1683540462 < 1));
                    var_19 += arr_7 [i_0] [6] [6] [20];
                    arr_9 [i_1] [i_0] = (1710883684 / 1000325255);
                    arr_10 [i_2] [i_2] [i_0] = (arr_5 [i_0] [i_0]);
                }
            }
        }
        var_20 = var_4;
    }
    var_21 = ((var_16 - ((var_14 + ((var_3 ? var_5 : var_8))))));
    #pragma endscop
}
