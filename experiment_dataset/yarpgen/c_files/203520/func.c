/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203520
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203520 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203520
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (long long int i_0 = 1; i_0 < 9; i_0 += 1) 
    {
        for (unsigned char i_1 = 1; i_1 < 10; i_1 += 2) 
        {
            {
                var_14 = ((/* implicit */_Bool) (~(((/* implicit */int) (!(((/* implicit */_Bool) 9455814094002899297ULL)))))));
                arr_6 [i_0] [i_1] = ((/* implicit */unsigned int) (-((+(268435455ULL)))));
            }
        } 
    } 
    var_15 = ((/* implicit */unsigned long long int) var_6);
    var_16 = 18446744073441116151ULL;
}
