/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 57820
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=57820 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/57820
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                arr_4 [i_0] = arr_2 [i_0] [i_0] [i_1];
                var_18 = (max((1 > -1854226347), (arr_0 [i_1])));
                arr_5 [i_0] = ((((min((arr_3 [i_0] [9] [i_0]), (min(1, (arr_3 [i_0] [i_0] [i_0])))))) ? ((max((arr_3 [i_0] [i_1] [i_1]), (arr_2 [i_0] [i_1] [9])))) : ((~((1 ? (arr_2 [i_0] [i_0] [7]) : var_7))))));
            }
        }
    }
    var_19 -= (((var_2 >= var_12) && ((max((((1 ? var_2 : 1771751624))), ((1 ? 1 : var_0)))))));
    #pragma endscop
}
