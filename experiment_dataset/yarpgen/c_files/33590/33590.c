/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33590
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33590 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33590
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 6;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 9;i_2 += 1)
            {
                {
                    var_17 = (19768 >= 45767);
                    arr_8 [i_2 - 1] [4] [i_0] = -118;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_13 [i_0 - 3] [3] [i_2 + 1] [i_3] [i_4] = (-111 ? -1 : 19768);
                                var_18 &= 1;
                                arr_14 [i_4] [8] [1] = (max(-63, var_16));
                                var_19 = (arr_0 [i_0] [0]);
                                var_20 ^= ((((arr_1 [9] [i_1]) ^ ((19754 ? var_15 : -114)))));
                            }
                        }
                    }
                    arr_15 [i_0] [i_1] [i_1] [i_2] = (118 >= -111);
                }
            }
        }
    }
    var_21 += var_10;
    var_22 = var_7;
    var_23 = ((max(var_4, (((var_1 ? var_13 : var_8))))));
    #pragma endscop
}
