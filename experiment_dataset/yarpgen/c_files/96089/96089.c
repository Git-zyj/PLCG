/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 96089
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=96089 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/96089
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = (min((((-var_11 == ((var_10 + (arr_5 [i_1] [i_0] [i_0])))))), (arr_0 [i_0])));
                arr_7 [i_0] [i_0] = ((53 ? (((-9223372036854775807 - 1) ? 102 : 4294967040)) : (((((min(4294967011, 4294967040))) ? (((18446744069414584576 ? (arr_3 [i_0] [i_1]) : var_13))) : (((arr_1 [i_1] [i_1]) ^ (arr_4 [i_1]))))))));
            }
        }
    }
    var_20 = -var_12;
    var_21 = min((min(var_5, var_4)), var_9);
    var_22 = (var_13 << -var_12);
    #pragma endscop
}
