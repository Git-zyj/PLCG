/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143745
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143745 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143745
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (long long int i_0 = 4; i_0 < 11; i_0 += 3) 
    {
        var_13 = ((/* implicit */unsigned long long int) ((signed char) arr_2 [i_0 - 2]));
        var_14 = ((/* implicit */short) ((((/* implicit */_Bool) arr_3 [i_0 + 2])) ? (((/* implicit */unsigned int) ((/* implicit */int) arr_3 [i_0 - 2]))) : (3899519765U)));
        var_15 = ((/* implicit */unsigned char) ((((/* implicit */int) var_3)) / (((((/* implicit */int) arr_3 [i_0])) | (arr_0 [i_0])))));
    }
    var_16 = ((/* implicit */long long int) var_7);
    var_17 &= var_12;
    var_18 = ((/* implicit */unsigned int) var_0);
}
