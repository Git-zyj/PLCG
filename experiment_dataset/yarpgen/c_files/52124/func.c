/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52124
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52124 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52124
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = min((((/* implicit */long long int) var_5)), (max((((((/* implicit */_Bool) var_6)) ? (((/* implicit */long long int) ((/* implicit */int) var_4))) : (var_11))), (((/* implicit */long long int) min((((/* implicit */short) var_1)), ((short)-3818)))))));
    var_14 = ((/* implicit */short) max((var_14), (((/* implicit */short) var_9))));
    /* LoopNest 2 */
    for (long long int i_0 = 2; i_0 < 18; i_0 += 4) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 19; i_1 += 1) 
        {
            {
                arr_5 [i_1] = arr_1 [(short)7];
                var_15 = ((/* implicit */long long int) (-(((/* implicit */int) var_12))));
            }
        } 
    } 
}
