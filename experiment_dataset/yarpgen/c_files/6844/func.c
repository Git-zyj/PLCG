/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6844
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6844 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6844
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 1; i_0 < 7; i_0 += 4) 
    {
        arr_3 [i_0] [(signed char)10] = ((((/* implicit */_Bool) var_16)) ? (((/* implicit */int) (signed char)7)) : (((/* implicit */int) arr_1 [i_0 + 2])));
        var_18 = ((/* implicit */int) (signed char)-1);
    }
    var_19 = ((/* implicit */unsigned long long int) (signed char)57);
    var_20 = ((/* implicit */unsigned char) min((((/* implicit */int) (signed char)0)), (-1362848730)));
    var_21 = ((/* implicit */signed char) min((min((var_11), ((~(((/* implicit */int) (_Bool)1)))))), (((/* implicit */int) var_1))));
}
