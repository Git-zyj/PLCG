/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220363
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220363 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220363
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        {
                            arr_9 [i_0 - 1] [i_0 - 1] [i_0 - 1] [1] = (max(((((((576179277326712832 != (arr_5 [i_0] [i_1] [i_2] [i_1]))))) - 3246167351342703902)), (arr_2 [10])));
                            var_13 = (var_7 % var_7);
                        }
                    }
                }
                var_14 = (((((1 ? -32759 : 13))) && 1));
            }
        }
    }
    var_15 = ((var_10 ? (((((16775168 || 22) < (max(3246167351342703895, var_4)))))) : ((-(max(18014398492704768, 1756060509))))));
    var_16 = (min(var_16, var_11));
    var_17 = var_1;
    var_18 = ((0 ? 24592 : -4006992188742153809));
    #pragma endscop
}
