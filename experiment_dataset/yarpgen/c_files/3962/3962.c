/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3962
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3962 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3962
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_17 = (min(var_17, ((var_5 ? (arr_0 [i_0]) : (((-1920 + 2147483647) << (10160036086729280831 - 10160036086729280831)))))));
        var_18 = (((-(arr_0 [i_0]))));
    }
    for (int i_1 = 3; i_1 < 22;i_1 += 1)
    {
        var_19 = (max(var_19, (min((((564 ? 50366 : 581))), ((-(arr_5 [i_1 - 2])))))));
        var_20 = ((-2134301052 ^ (((arr_5 [i_1 + 1]) / ((var_14 ? 569 : var_7))))));
        /* LoopNest 2 */
        for (int i_2 = 1; i_2 < 21;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 25;i_3 += 1)
            {
                {
                    var_21 = arr_11 [i_1] [i_2] [18] [i_1];
                    var_22 = (!207);
                }
            }
        }
    }
    var_23 = var_1;
    #pragma endscop
}
