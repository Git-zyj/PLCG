/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 9251
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=9251 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/9251
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 |= ((/* implicit */signed char) var_10);
    var_13 = ((520192U) << (((520173U) - (520151U))));
    /* LoopNest 2 */
    for (short i_0 = 2; i_0 < 9; i_0 += 1) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_14 -= var_10;
                arr_7 [i_0] = ((((/* implicit */_Bool) max((((/* implicit */long long int) (_Bool)1)), (((((/* implicit */_Bool) var_1)) ? (arr_5 [11] [i_0]) : (((/* implicit */long long int) ((/* implicit */int) arr_2 [i_0] [(short)2])))))))) ? (max((arr_0 [i_1]), (((/* implicit */unsigned int) arr_3 [i_0])))) : ((~(arr_0 [7LL]))));
            }
        } 
    } 
}
