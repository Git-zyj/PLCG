/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131587
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131587 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131587
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (-var_1 + var_11);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    arr_11 [i_2] = ((arr_7 [i_1 + 1] [i_0 + 1] [i_2]) * (((arr_5 [i_0] [5] [i_2]) / (((arr_6 [i_0] [i_1]) / (arr_4 [12] [i_1 + 1]))))));
                    arr_12 [i_2] = ((!((((((arr_4 [i_0 + 1] [i_0]) | (arr_1 [i_2]))) | (arr_10 [i_1 + 1]))))));
                    var_14 *= (arr_9 [i_1 + 1] [i_0 + 1] [1]);
                    var_15 = (((arr_5 [i_0 + 1] [i_1 - 2] [i_2]) - (arr_2 [i_0] [i_2])));
                }
            }
        }
    }
    var_16 = var_5;
    #pragma endscop
}
