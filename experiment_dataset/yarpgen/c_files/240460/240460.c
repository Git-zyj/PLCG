/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 240460
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=240460 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/240460
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_11 = (((((arr_1 [i_1]) | (arr_3 [i_0] [i_0] [4]))) <= ((max((arr_2 [i_1] [i_0]), (arr_1 [2]))))));
                var_12 *= ((min((arr_1 [i_0]), (3978120906 & -226170139))) != (1157181075320255176 + 149));
                arr_4 [i_0] [i_0] [i_1] = 226170145;
            }
        }
    }
    var_13 = -18446744073709551608;
    var_14 = (((var_6 + 2147483647) << (((var_4 != -1) - 1))));
    #pragma endscop
}
