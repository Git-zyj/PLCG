/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98097
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98097 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98097
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (_Bool i_0 = 1; i_0 < 1; i_0 += 1) 
    {
        for (unsigned int i_1 = 0; i_1 < 17; i_1 += 4) 
        {
            {
                var_15 = ((/* implicit */short) var_14);
                var_16 = ((/* implicit */unsigned char) min((((arr_4 [i_0] [i_0]) ? (((/* implicit */unsigned int) (-2147483647 - 1))) : (min((((/* implicit */unsigned int) var_1)), (var_7))))), (((/* implicit */unsigned int) arr_1 [i_0 - 1]))));
            }
        } 
    } 
    var_17 -= ((/* implicit */unsigned int) var_13);
    var_18 *= ((/* implicit */unsigned int) var_3);
    var_19 = ((/* implicit */_Bool) min((var_19), (((((/* implicit */_Bool) (unsigned char)10)) && (((/* implicit */_Bool) max((2147483647), (((/* implicit */int) (_Bool)1)))))))));
}
