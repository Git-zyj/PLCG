/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51303
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51303 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51303
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            {
                var_11 = ((~(arr_2 [i_1 + 3] [i_1 - 1] [i_1 + 2])));
                var_12 = (arr_0 [i_1 + 2] [i_1 + 3]);
                arr_5 [i_0] = (((((1 << (((arr_2 [i_1 + 3] [i_1 + 2] [i_1 + 1]) - 686704543))))) ? (((((arr_2 [i_1 + 2] [i_1 + 1] [i_1 + 2]) == 1)))) : var_7));
                arr_6 [i_1] = (((((((1 ? (arr_3 [i_1]) : -32)) * ((min((arr_2 [i_1] [i_1] [i_0]), 1)))))) / ((1 ? ((-(arr_1 [i_0]))) : (!var_4)))));
            }
        }
    }
    #pragma endscop
}
