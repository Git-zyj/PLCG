/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 42024
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=42024 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/42024
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_14;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_18 -= ((var_12 ? var_5 : (((((var_0 * (arr_0 [i_0 + 2] [8])))) - ((34659 ? (arr_2 [i_0] [7] [i_0 - 1]) : var_4))))));
                arr_4 [i_1] = (((var_8 ^ 8455323349153139660) | (~51087)));
                arr_5 [i_1] [i_1] [i_0 + 2] = (((((0 ^ 2706192824) ? (((arr_3 [i_1]) ? var_15 : 0)) : var_16)) << ((((var_6 ? (((min(var_12, var_9)))) : (arr_3 [i_1]))) - 56939))));
                arr_6 [i_0] [i_1] [8] = -111;
            }
        }
    }
    #pragma endscop
}
