/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99244
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99244 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99244
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((var_7 < ((((!((min(var_11, var_6)))))))));
    var_15 &= (((((((129 ? 127 : 1)) || (-8408154137626759536 || var_11))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_2] = (max((var_4 + 127), ((18149278553716838198 ? var_3 : var_6))));
                    arr_8 [i_0] [i_1] [i_1] [1] = ((!((((min(var_2, var_5)) % var_11)))));
                    arr_9 [i_1] [i_2] = (min((-32767 - 1), -15025));
                }
            }
        }
    }
    var_16 = var_9;
    var_17 = var_3;
    #pragma endscop
}
