/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 243915
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=243915 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/243915
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_12, (((!((max(var_13, 0))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = ((!(arr_3 [i_0] [i_0] [i_0])));
                var_17 = (max(var_17, 137));
                var_18 = (max(var_18, (((max((((-9223372036854775807 - 1) + (arr_1 [i_0]))), ((max(16, (arr_2 [i_0] [i_0])))))) - (((8955526380688947831 ? ((max(1, -19))) : var_8)))))));
                arr_5 [i_0] [i_1] [i_1] = (max(1, (max(var_10, 1))));
                arr_6 [i_0] = 1;
            }
        }
    }
    #pragma endscop
}
