/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133322
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133322 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133322
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (453644249571438442 ? -7 : -5677928043024462657);
    var_11 = -1828508997;
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_12 = (max(var_12, ((((arr_0 [12]) ? ((((!(((var_9 ? (arr_4 [i_2]) : var_9))))))) : (max((((arr_7 [10]) ? -1828508997 : (arr_6 [i_2] [i_1]))), var_4)))))));
                    var_13 = (arr_2 [i_0 + 1] [8]);
                }
            }
        }
    }
    #pragma endscop
}
