/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59793
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59793 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59793
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned char) (+(((/* implicit */int) var_7))));
    var_16 ^= ((/* implicit */unsigned short) max((((/* implicit */unsigned int) (unsigned char)128)), (2566725804U)));
    var_17 += ((/* implicit */_Bool) ((((/* implicit */_Bool) var_4)) ? (var_12) : (((/* implicit */long long int) ((/* implicit */int) var_7)))));
    /* LoopNest 3 */
    for (long long int i_0 = 2; i_0 < 8; i_0 += 1) 
    {
        for (signed char i_1 = 1; i_1 < 8; i_1 += 2) 
        {
            for (signed char i_2 = 1; i_2 < 9; i_2 += 1) 
            {
                {
                    arr_9 [i_0] [i_1 + 1] [i_2] = ((/* implicit */long long int) ((_Bool) (_Bool)1));
                    var_18 += ((/* implicit */long long int) (-(arr_5 [i_0 - 2] [i_1 + 2] [i_2])));
                }
            } 
        } 
    } 
}
