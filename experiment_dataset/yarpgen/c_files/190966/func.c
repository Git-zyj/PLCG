/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190966
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190966 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190966
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = var_0;
    /* LoopSeq 1 */
    for (unsigned int i_0 = 3; i_0 < 12; i_0 += 1) 
    {
        arr_4 [i_0 + 1] = ((/* implicit */signed char) (~((+(((/* implicit */int) (signed char)18))))));
        var_19 = ((/* implicit */unsigned int) (((_Bool)1) ? (((/* implicit */unsigned long long int) ((/* implicit */int) (signed char)-19))) : (7799618259470792850ULL)));
    }
    var_20 = ((/* implicit */signed char) min((var_20), (((/* implicit */signed char) var_6))));
    var_21 = ((/* implicit */int) var_6);
}
