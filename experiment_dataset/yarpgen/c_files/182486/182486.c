/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182486
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182486 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182486
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (!-7153341361246033520);
    var_11 = (~-var_6);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    var_12 = (((min((max((arr_5 [i_1] [i_0]), -1)), var_9))) ? (~var_7) : ((-((var_6 ? 4147152453 : var_9)))));
                    var_13 = (((min((arr_0 [i_2]), (arr_0 [i_0])))) ? (min((arr_0 [4]), (arr_0 [i_0]))) : (arr_0 [i_0]));
                }
            }
        }
    }
    #pragma endscop
}
