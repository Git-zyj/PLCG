/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 37504
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=37504 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/37504
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 18; i_0 += 1) 
    {
        for (int i_1 = 0; i_1 < 18; i_1 += 2) 
        {
            {
                var_15 = max((((867001980) & (((/* implicit */int) arr_1 [i_1])))), (((/* implicit */int) ((_Bool) arr_1 [i_0]))));
                arr_7 [i_0] [i_0] = ((((/* implicit */int) (_Bool)1)) + (((/* implicit */int) (_Bool)1)));
            }
        } 
    } 
    var_16 = ((/* implicit */short) 289965711U);
}
