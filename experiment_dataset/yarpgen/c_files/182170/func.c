/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182170
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182170 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182170
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 2 */
    /* vectorizable */
    for (unsigned short i_0 = 0; i_0 < 12; i_0 += 4) 
    {
        var_17 = (~(((/* implicit */int) (!(((/* implicit */_Bool) var_3))))));
        var_18 = ((((/* implicit */int) var_7)) & ((-(((/* implicit */int) (unsigned char)243)))));
    }
    for (unsigned int i_1 = 0; i_1 < 15; i_1 += 3) 
    {
        arr_6 [i_1] [i_1] = ((/* implicit */_Bool) var_7);
        arr_7 [i_1] = ((unsigned char) arr_3 [(_Bool)1] [i_1]);
    }
    var_19 = ((unsigned char) max((((/* implicit */int) max((((/* implicit */unsigned char) var_12)), ((unsigned char)235)))), (((((/* implicit */int) (unsigned char)248)) << (((1311833345U) - (1311833338U)))))));
}
