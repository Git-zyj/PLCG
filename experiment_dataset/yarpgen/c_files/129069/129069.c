/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 129069
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=129069 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/129069
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (max((var_1 >= var_3), 251));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    var_12 *= (max((min((((var_9 * (arr_3 [i_0])))), var_6)), 1));
                    var_13 = (min(var_1, (max(-4952284911873440001, ((min(-14790, -9137)))))));
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 17;i_4 += 1)
                        {
                            {
                                arr_15 [i_0] [2] [i_0] [1] [i_3] [i_4] [1] = (arr_0 [i_0]);
                                arr_16 [i_0] [i_0] [10] [i_0] [i_0] [i_0] = ((~(arr_0 [i_0])));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
