/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130614
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130614 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130614
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 -= ((/* implicit */unsigned char) var_13);
    /* LoopNest 3 */
    for (long long int i_0 = 0; i_0 < 18; i_0 += 2) 
    {
        for (int i_1 = 0; i_1 < 18; i_1 += 2) 
        {
            for (unsigned char i_2 = 0; i_2 < 18; i_2 += 3) 
            {
                {
                    /* LoopSeq 1 */
                    for (unsigned long long int i_3 = 0; i_3 < 18; i_3 += 4) 
                    {
                        arr_12 [i_0] [i_0] [i_2] [i_0] [i_0] = ((/* implicit */unsigned char) 1981900592U);
                        var_16 = ((/* implicit */unsigned char) (~(max((((((/* implicit */_Bool) arr_4 [i_0] [i_0] [i_0])) ? (((/* implicit */unsigned int) ((/* implicit */int) var_11))) : (2528574045U))), (((/* implicit */unsigned int) arr_1 [i_0]))))));
                    }
                    var_17 = ((/* implicit */int) (~(max((var_5), (((/* implicit */unsigned int) (((_Bool)1) ? (((/* implicit */int) (_Bool)1)) : (((/* implicit */int) (unsigned char)181)))))))));
                }
            } 
        } 
    } 
    var_18 = ((/* implicit */_Bool) (+(((/* implicit */int) (_Bool)0))));
}
