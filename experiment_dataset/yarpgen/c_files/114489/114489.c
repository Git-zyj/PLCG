/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114489
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114489 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114489
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((~((min(var_7, var_9)))));

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = (arr_0 [i_0]);

        /* vectorizable */
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            arr_6 [i_0] = (((arr_4 [i_1 + 2] [i_0] [11]) ? (arr_5 [i_1 + 1] [i_1 + 2] [12]) : var_1));
            var_11 = ((var_8 ? var_1 : (arr_4 [i_0] [i_0] [2])));
        }
        for (int i_2 = 0; i_2 < 15;i_2 += 1)
        {
            var_12 = (min(var_12, (((((max(((((arr_1 [i_2]) == var_1))), (min((arr_4 [i_2] [i_2] [i_0]), (arr_5 [i_0] [i_0] [i_0])))))) ? (min((arr_5 [2] [i_2] [i_0]), (arr_7 [i_2]))) : 15406)))));
            var_13 -= ((~(min((arr_7 [i_2]), var_5))));
        }
    }
    #pragma endscop
}
