/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217642
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217642 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217642
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((-(9056671141738669600 / 33368)));

    /* vectorizable */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        arr_3 [i_0] = (arr_2 [i_0] [i_0]);
        var_11 = ((((var_7 ? (arr_1 [2] [23]) : var_4)) == 255));
        arr_4 [i_0 - 1] = 1779777302;
        var_12 = (~(arr_0 [11]));
    }
    /* LoopNest 2 */
    for (int i_1 = 1; i_1 < 21;i_1 += 1)
    {
        for (int i_2 = 1; i_2 < 20;i_2 += 1)
        {
            {
                var_13 += (min(2515189998, ((((arr_8 [i_1 + 1]) > -255)))));
                var_14 = (max(((+(min((arr_6 [i_1] [i_2]), var_7)))), (((~(max(0, 234881024)))))));
            }
        }
    }
    var_15 = (max(var_8, (min((var_9 <= -471401183128726955), (-2147483647 - 1)))));
    var_16 = var_5;
    #pragma endscop
}
