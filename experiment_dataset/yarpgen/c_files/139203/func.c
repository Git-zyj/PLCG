/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 139203
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=139203 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/139203
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 2 */
    for (short i_0 = 0; i_0 < 23; i_0 += 4) 
    {
        var_14 = ((/* implicit */unsigned long long int) ((((/* implicit */int) (!(((/* implicit */_Bool) -1))))) == (((/* implicit */int) arr_0 [i_0] [i_0]))));
        var_15 = ((/* implicit */unsigned long long int) arr_2 [i_0]);
        var_16 = ((/* implicit */short) min((var_16), (((/* implicit */short) (-(max((((/* implicit */unsigned int) arr_1 [i_0] [i_0])), (max((((/* implicit */unsigned int) arr_1 [i_0] [i_0])), (arr_2 [i_0]))))))))));
    }
    for (unsigned long long int i_1 = 0; i_1 < 21; i_1 += 3) 
    {
        var_17 *= ((/* implicit */unsigned long long int) var_4);
        var_18 = ((/* implicit */unsigned long long int) (_Bool)0);
    }
    var_19 = ((/* implicit */unsigned short) var_0);
}
