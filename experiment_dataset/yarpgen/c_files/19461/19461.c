/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 19461
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=19461 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/19461
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_2, var_1));
    var_11 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_12 = (max(var_12, ((max((max((arr_3 [i_0 - 1] [i_0 + 1]), 6899002579367381456)), ((max((arr_3 [i_0 + 1] [i_0 - 1]), (arr_3 [i_0 - 1] [i_0 - 1])))))))));
                    var_13 -= (max(((min(20262572, var_3))), (min((arr_9 [i_0 + 1] [i_0 - 1] [i_0 + 1] [i_0 - 1]), (arr_9 [i_0 + 1] [i_0 - 1] [i_0 + 1] [i_0 - 1])))));
                }
            }
        }
    }
    var_14 = var_7;
    #pragma endscop
}
