/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 17559
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=17559 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/17559
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_12;

    for (int i_0 = 4; i_0 < 19;i_0 += 1)
    {
        arr_4 [i_0] = (max(((max((arr_3 [i_0]), (arr_1 [i_0])))), (arr_1 [i_0 + 2])));
        var_19 |= (var_14 ? (arr_3 [i_0 + 1]) : (min((var_7 * -747030182), 2775906948)));
    }
    var_20 ^= min(((var_17 ? var_7 : var_16)), ((var_14 ^ (var_0 | var_7))));
    #pragma endscop
}
