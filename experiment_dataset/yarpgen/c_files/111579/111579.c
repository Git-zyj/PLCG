/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111579
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111579 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111579
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((((((((-32760 ? var_0 : var_2))) ? 1 : var_6))) ? var_7 : (max(0, (var_7 & var_7)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_1] = var_3;
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_15 [i_1] [1] = 30905;
                                arr_16 [i_1] [i_1] [i_3] = ((((min(192, 1)))));
                                arr_17 [i_1] [i_1] = arr_6 [i_0] [i_0] [i_0];
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 24;i_5 += 1)
                    {
                        for (int i_6 = 2; i_6 < 23;i_6 += 1)
                        {
                            {
                                arr_23 [i_0] [i_0] [i_2] [i_0] [i_1] = ((arr_12 [i_0] [i_1] [i_1] [i_1] [i_1]) <= (131 < 64));
                                arr_24 [i_0] [i_6 - 1] [i_2] [i_1] [i_0] = 57;
                            }
                        }
                    }
                    var_11 = (max(var_11, 224));
                }
            }
        }
    }
    var_12 = var_8;
    var_13 -= var_8;
    #pragma endscop
}
