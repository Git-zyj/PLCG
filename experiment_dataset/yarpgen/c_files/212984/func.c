/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212984
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212984 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212984
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 -= ((/* implicit */short) (unsigned char)205);
    var_21 = ((/* implicit */_Bool) (+(((/* implicit */int) (signed char)93))));
    /* LoopSeq 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        arr_2 [i_0] = ((/* implicit */unsigned char) (-(((/* implicit */int) var_11))));
        var_22 = ((/* implicit */short) min((((/* implicit */unsigned int) (-(((/* implicit */int) arr_0 [i_0] [i_0]))))), (var_5)));
    }
    /* vectorizable */
    for (unsigned char i_1 = 0; i_1 < 11; i_1 += 2) 
    {
        var_23 = ((/* implicit */long long int) arr_4 [i_1]);
        arr_6 [i_1] = var_1;
        arr_7 [i_1] [i_1] = ((/* implicit */signed char) arr_1 [i_1] [i_1]);
        arr_8 [i_1] = ((/* implicit */short) (!(((/* implicit */_Bool) (~(((/* implicit */int) arr_5 [i_1])))))));
    }
    var_24 = ((/* implicit */unsigned short) var_9);
}
