/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62555
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62555 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62555
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 10;i_2 += 1)
            {
                {
                    var_18 -= (arr_6 [i_0] [4] [i_0]);
                    var_19 = (((((var_2 * var_12) || (((1 ? var_6 : (arr_7 [i_2] [i_1] [i_2 + 1])))))) ? 1 : (min((~var_1), (arr_3 [i_0] [i_1 + 2] [i_2 - 1])))));
                }
            }
        }
    }
    var_20 -= min(var_1, ((((min(var_9, var_15))) ? (var_5 / var_14) : -var_11)));
    #pragma endscop
}
