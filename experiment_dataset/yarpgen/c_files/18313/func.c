/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18313
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18313 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18313
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */long long int) var_1);
    /* LoopNest 3 */
    for (signed char i_0 = 0; i_0 < 21; i_0 += 2) 
    {
        for (int i_1 = 2; i_1 < 17; i_1 += 4) 
        {
            for (unsigned int i_2 = 0; i_2 < 21; i_2 += 2) 
            {
                {
                    var_11 += ((/* implicit */short) ((min((((/* implicit */long long int) (-(((/* implicit */int) arr_3 [i_0] [i_1]))))), (((((/* implicit */_Bool) 2840614117U)) ? (((/* implicit */long long int) ((/* implicit */int) arr_1 [11]))) : (0LL))))) <= (((/* implicit */long long int) ((/* implicit */int) ((short) -303273234))))));
                    var_12 ^= ((/* implicit */short) (unsigned char)86);
                }
            } 
        } 
    } 
    var_13 |= ((/* implicit */unsigned int) var_0);
}
