/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 167656
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=167656 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/167656
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */short) var_11);
    /* LoopSeq 2 */
    /* vectorizable */
    for (int i_0 = 2; i_0 < 17; i_0 += 2) 
    {
        arr_3 [i_0] |= ((/* implicit */unsigned long long int) var_9);
        arr_4 [i_0] [i_0] = ((/* implicit */_Bool) var_5);
        var_14 = ((/* implicit */unsigned char) max((var_14), (((/* implicit */unsigned char) var_1))));
        var_15 = ((/* implicit */long long int) var_12);
        var_16 = ((/* implicit */signed char) var_8);
    }
    /* vectorizable */
    for (unsigned int i_1 = 0; i_1 < 18; i_1 += 2) 
    {
        var_17 = ((/* implicit */unsigned long long int) var_2);
        arr_8 [i_1] [i_1] = ((/* implicit */unsigned int) var_7);
    }
    var_18 ^= var_2;
    var_19 = ((/* implicit */int) var_9);
}
