/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 92055
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=92055 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/92055
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((~(((!(((var_8 ? 291893268626851767 : var_5))))))));
    var_11 = 222;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 16;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_2 + 2] [3] = ((-(((arr_5 [i_2 - 1] [i_2 - 1] [i_2]) ? (arr_5 [i_2 + 1] [i_2 - 1] [i_2]) : (arr_5 [i_2 + 2] [i_2 - 1] [i_2])))));
                    var_12 &= ((var_3 >> ((((1332088175 ? (arr_5 [i_2] [i_1] [i_0]) : ((var_3 | (arr_4 [i_0] [i_0] [i_0]))))) - 160))));
                }
            }
        }
    }
    #pragma endscop
}
