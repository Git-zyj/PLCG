/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149243
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149243 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149243
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (((((((max(var_0, var_7))) > 4)))) * var_15);
    var_21 = (min(var_1, var_2));
    var_22 = (max(var_22, var_18));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_23 = (min(var_15, -var_0));
                    var_24 = (max(16949740812021831760, 194));
                    var_25 = (min(var_25, (arr_0 [i_0])));

                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {

                        /* vectorizable */
                        for (int i_4 = 0; i_4 < 13;i_4 += 1)
                        {
                            var_26 = (arr_10 [1] [i_0]);
                            var_27 = -var_12;
                            var_28 = (arr_8 [i_4] [i_3] [1] [i_0]);
                            var_29 = (arr_6 [3] [i_1] [7] [7]);
                        }
                        var_30 = (max((~604162223), ((((arr_5 [i_1] [i_1 + 1]) <= (arr_5 [i_1] [i_1 - 1]))))));
                    }
                    /* vectorizable */
                    for (int i_5 = 0; i_5 < 13;i_5 += 1)
                    {
                        var_31 = (arr_13 [i_1 + 1] [i_5]);
                        var_32 = var_15;
                        var_33 = 200;
                        var_34 = ((~(((arr_12 [i_0] [i_0]) & (arr_1 [10] [2])))));
                    }
                    var_35 = var_10;
                }
            }
        }
    }
    #pragma endscop
}
