/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202037
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202037 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202037
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (((((43318 > -103) == (~var_0))) || ((((3340183352 ? 81 : -984385025))))));

    for (int i_0 = 4; i_0 < 21;i_0 += 1)
    {
        var_18 = var_9;
        arr_3 [i_0] = (arr_0 [i_0 - 1] [i_0]);
    }

    for (int i_1 = 0; i_1 < 16;i_1 += 1)
    {
        var_19 &= ((((~((((arr_2 [i_1] [1]) || (arr_4 [i_1] [i_1])))))) == var_8));

        for (int i_2 = 1; i_2 < 15;i_2 += 1)
        {
            arr_11 [i_1] [4] = -984385025;
            var_20 = (min(var_20, var_12));
            var_21 = 954783961;
            arr_12 [i_1] [i_1] = (arr_5 [i_2 - 1] [i_2 + 1]);
        }

        for (int i_3 = 0; i_3 < 1;i_3 += 1)
        {
            var_22 = ((((((arr_5 [1] [i_3]) ? 1 : var_9))) ? (min(-984385025, (arr_5 [i_1] [13]))) : var_11));
            var_23 = 186;
        }
    }
    #pragma endscop
}
