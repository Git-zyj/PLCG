/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71168
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71168 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71168
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = 32755;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] = var_10;
                var_20 = (arr_2 [i_1] [i_0 + 3]);
                var_21 = (min(var_21, (((((((((min(32755, 32725)))) * (min(var_0, (arr_2 [7] [i_0])))))) ? (max((((arr_3 [i_0] [0] [i_0 + 4]) ? var_11 : var_5)), (((var_17 ? (arr_1 [i_0]) : (arr_2 [i_0 + 4] [13])))))) : var_14)))));
            }
        }
    }
    #pragma endscop
}
