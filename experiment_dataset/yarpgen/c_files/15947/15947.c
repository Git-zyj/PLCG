/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15947
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15947 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15947
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (((((min(1183554799, 37)))) ? var_2 : var_0));
    var_14 = (((~((var_5 ? var_8 : 25)))));
    var_15 = var_1;

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_16 = ((+((3447712453 ? (arr_0 [i_0]) : (arr_0 [i_0])))));
        arr_2 [i_0] [i_0] = (((--30) ? (((arr_0 [i_0]) ? (arr_1 [i_0] [8]) : (arr_1 [i_0] [i_0]))) : ((((!(((var_4 ? (arr_0 [i_0]) : -37)))))))));
    }
    #pragma endscop
}
