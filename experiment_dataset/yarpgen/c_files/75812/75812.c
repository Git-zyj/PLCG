/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75812
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75812 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75812
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        var_16 = var_0;
        var_17 = (arr_2 [i_0]);
    }
    for (int i_1 = 1; i_1 < 1;i_1 += 1)
    {
        arr_5 [i_1] [i_1 - 1] = (min((((var_11 < (arr_4 [i_1] [i_1])))), (((((var_1 ? (arr_3 [i_1]) : var_6))) ? (var_6 > var_6) : var_6))));
        arr_6 [i_1 - 1] = (((((min(var_14, var_12)) * (((arr_3 [i_1]) & var_3)))) ^ (((!(((var_15 ? (arr_1 [i_1]) : var_15))))))));
        var_18 = ((((((min(var_4, var_9))) && ((!(arr_3 [i_1]))))) ? ((var_0 ? (((arr_1 [i_1 - 1]) | var_3)) : var_12)) : var_4));
    }
    var_19 -= var_12;
    var_20 = var_12;
    #pragma endscop
}
