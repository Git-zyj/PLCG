/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 28384
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=28384 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/28384
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(var_14, var_8));
    var_15 = ((-9033 ? ((min((var_3 & 31743), -1490207431))) : var_8));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 9;i_2 += 1)
            {
                {
                    arr_8 [i_2] [i_0] [i_1] [i_2] = (min((((arr_6 [i_2 - 2] [i_2] [i_2 + 1]) ? (arr_4 [i_2 - 2] [i_2 - 2] [i_2 + 1]) : (arr_6 [i_2 - 3] [i_2] [i_2 - 3]))), (((-12977 ? 65535 : (arr_4 [i_2 + 1] [i_2 - 3] [i_2 - 3]))))));
                    arr_9 [i_1] &= (min((!var_6), (((arr_6 [i_0] [i_0] [i_0]) ^ (arr_6 [i_0] [i_0] [i_2])))));
                }
            }
        }
    }
    #pragma endscop
}
