/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175241
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175241 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175241
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 ^= ((((((var_0 > var_13) ^ var_3))) ? (((((23 ? var_8 : var_9))) * (max(var_0, var_3)))) : (((((var_14 ^ var_4) == -0))))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            {
                var_16 = ((-(((arr_0 [i_1 + 1] [i_1]) ? (arr_3 [i_0 - 1]) : (arr_3 [i_0 - 2])))));
                var_17 ^= var_12;
                arr_6 [i_0] [4] [i_0] = (((var_3 * var_10) || (arr_0 [i_1 - 1] [i_1 + 1])));
            }
        }
    }
    var_18 = var_12;
    #pragma endscop
}
