/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136696
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136696 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136696
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_2;
    var_11 = ((max(var_5, -var_1)));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_12 = ((-2912 ? 2147450880 : 2147450880));
                var_13 = (arr_0 [i_0]);
                arr_5 [i_0] [i_1] [i_1] = 2147450868;
                arr_6 [i_0] = (min((((arr_4 [i_0 + 1] [i_0 + 4] [10]) ? var_5 : (((arr_0 [i_0]) ? (arr_2 [i_0] [i_1] [i_1]) : (arr_3 [1]))))), var_2));
                var_14 = 214;
            }
        }
    }
    var_15 = var_2;
    #pragma endscop
}
