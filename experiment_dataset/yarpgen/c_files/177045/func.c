/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177045
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177045 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177045
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (long long int i_0 = 0; i_0 < 10; i_0 += 4) 
    {
        for (unsigned char i_1 = 1; i_1 < 9; i_1 += 4) 
        {
            for (unsigned long long int i_2 = 1; i_2 < 8; i_2 += 4) 
            {
                {
                    var_10 = ((/* implicit */unsigned long long int) var_9);
                    arr_7 [i_0] [i_1] [i_1] [i_2] = ((/* implicit */short) ((((/* implicit */long long int) ((/* implicit */int) ((signed char) arr_2 [i_1])))) / (((((/* implicit */_Bool) (-(16231826347990932482ULL)))) ? (max((((/* implicit */long long int) -610928001)), (arr_6 [i_2]))) : (((/* implicit */long long int) var_0))))));
                }
            } 
        } 
    } 
    var_11 = ((/* implicit */int) ((var_4) < (((/* implicit */unsigned int) ((/* implicit */int) (unsigned char)252)))));
}
