/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235061
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235061 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235061
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_17 = (min(var_17, 22770));
                    var_18 = (min(var_18, (((var_9 == ((((arr_3 [i_0] [22] [i_2]) ? ((var_8 ? -86 : (arr_6 [i_1] [i_1] [i_0]))) : (((arr_7 [1] [1] [i_2]) ? (arr_7 [4] [4] [i_2]) : (arr_6 [i_2] [i_2] [i_2])))))))))));
                    arr_8 [i_0] = (arr_1 [i_2]);
                    var_19 = ((((~(arr_5 [i_0] [i_0] [i_0] [i_0]))) > -86));
                }
            }
        }
    }
    var_20 = (max(var_20, -87));
    #pragma endscop
}
