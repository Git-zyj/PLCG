/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82695
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82695 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82695
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 -= ((/* implicit */short) var_15);
    /* LoopNest 2 */
    for (unsigned char i_0 = 2; i_0 < 23; i_0 += 4) 
    {
        for (unsigned int i_1 = 1; i_1 < 24; i_1 += 4) 
        {
            {
                var_18 *= ((/* implicit */unsigned int) arr_0 [i_1 + 1] [i_1 - 1]);
                var_19 = ((/* implicit */_Bool) ((((/* implicit */int) (unsigned short)0)) / (((/* implicit */int) (unsigned char)8))));
                var_20 = ((/* implicit */long long int) (unsigned char)230);
            }
        } 
    } 
    /* LoopSeq 1 */
    /* vectorizable */
    for (short i_2 = 3; i_2 < 16; i_2 += 4) 
    {
        var_21 = ((/* implicit */unsigned char) arr_1 [i_2 - 2]);
        var_22 = ((/* implicit */long long int) max((var_22), (((/* implicit */long long int) var_7))));
    }
    var_23 -= ((/* implicit */unsigned char) var_12);
}
