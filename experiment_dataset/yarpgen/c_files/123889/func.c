/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123889
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123889 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123889
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = var_14;
    var_17 = ((/* implicit */unsigned long long int) var_1);
    /* LoopSeq 2 */
    for (long long int i_0 = 1; i_0 < 13; i_0 += 2) /* same iter space */
    {
        arr_4 [i_0] = ((/* implicit */short) arr_1 [i_0] [i_0]);
        var_18 = (short)-6;
    }
    /* vectorizable */
    for (long long int i_1 = 1; i_1 < 13; i_1 += 2) /* same iter space */
    {
        var_19 = ((/* implicit */short) max((var_19), (((/* implicit */short) (-(((/* implicit */int) ((((/* implicit */_Bool) var_10)) && (((/* implicit */_Bool) var_6))))))))));
        arr_8 [i_1] = ((/* implicit */unsigned short) (+(((/* implicit */int) arr_7 [i_1 - 1] [i_1 - 1]))));
        arr_9 [i_1] [i_1] = ((/* implicit */unsigned int) (+(var_9)));
    }
}
