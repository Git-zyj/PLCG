/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172104
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172104 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172104
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] [17] = (max((((arr_2 [i_1]) | (arr_2 [i_0]))), ((var_11 ? (arr_2 [i_1]) : (arr_2 [i_0])))));
                arr_7 [i_0] = ((((arr_2 [i_1]) ? (arr_0 [i_1]) : var_8)));
                var_16 &= var_0;
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 4; i_2 < 17;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 17;i_3 += 1)
        {
            {
                var_17 ^= (((((min(var_15, 828140682)) != var_9)) ? (((((0 ^ (arr_2 [i_3])))) ? -var_14 : 4294967295)) : ((min(((~(arr_4 [i_3] [i_2] [i_2 + 2]))), ((((arr_9 [i_2] [i_3]) || var_2))))))));
                var_18 += ((!(((~(arr_1 [i_2 + 1]))))));
            }
        }
    }
    #pragma endscop
}
