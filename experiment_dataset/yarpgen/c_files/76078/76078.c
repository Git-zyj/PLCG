/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76078
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76078 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76078
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_5 [i_1] = (arr_4 [i_1]);
                arr_6 [i_0] [i_1] [i_1] = 0;
            }
        }
    }

    for (int i_2 = 1; i_2 < 9;i_2 += 1)
    {
        var_19 = (max(var_19, (((((((arr_3 [i_2 + 2] [10] [i_2 + 2]) ? (arr_3 [i_2 + 2] [0] [i_2]) : (arr_3 [i_2 + 2] [14] [10])))) ? (((max((arr_3 [i_2 + 2] [12] [i_2]), 226)))) : (arr_3 [i_2 + 2] [10] [10]))))));
        arr_9 [0] [i_2 - 1] &= (((arr_2 [i_2 + 2]) ? (arr_2 [i_2 - 1]) : (arr_3 [i_2 + 2] [12] [i_2 + 1])));
        var_20 = arr_2 [i_2];
    }
    var_21 &= var_14;
    var_22 = ((var_14 ? var_5 : var_18));
    var_23 &= var_4;
    #pragma endscop
}
