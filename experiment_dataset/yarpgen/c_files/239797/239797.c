/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239797
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239797 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239797
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 &= max(var_9, var_0);
    var_12 = var_8;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        var_13 *= var_4;
        arr_2 [i_0] [i_0] = (((arr_1 [i_0]) ^ (arr_1 [i_0])));
        var_14 = arr_1 [i_0];
    }
    for (int i_1 = 0; i_1 < 24;i_1 += 1)
    {
        arr_7 [i_1] = ((!(arr_5 [i_1])));
        var_15 += ((117 ? 3438757282 : var_1));
        var_16 = ((~(((250 < (max(var_2, var_3)))))));
        arr_8 [i_1] = (((arr_4 [i_1] [i_1]) ? ((min(var_10, var_9))) : ((-(arr_3 [i_1] [i_1])))));
    }
    for (int i_2 = 2; i_2 < 18;i_2 += 1)
    {
        var_17 ^= var_3;
        var_18 = (max(var_18, ((+((var_3 ? (arr_5 [12]) : (arr_6 [0] [i_2 + 2])))))));
    }
    var_19 = var_4;
    var_20 = 0;
    #pragma endscop
}
