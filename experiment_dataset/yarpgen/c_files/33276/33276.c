/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33276
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33276 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33276
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 -= var_1;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [i_1] = (((arr_5 [i_0 - 1] [i_1]) ? (((((((arr_3 [i_1] [i_1] [i_1]) ? (arr_5 [i_0] [i_2]) : 17281))) < ((-5963195358825544926 ? (arr_1 [3] [i_1]) : (arr_1 [i_0 + 4] [i_0])))))) : ((~(arr_3 [i_2] [i_1] [i_0 + 4])))));
                    var_14 -= ((~(arr_2 [i_2])));
                    arr_7 [i_0 + 4] [i_0 + 4] [i_0] [i_1] = ((~(arr_2 [i_0 + 3])));
                    var_15 = ((-(arr_3 [13] [i_1] [i_2])));
                }
            }
        }
    }
    #pragma endscop
}
