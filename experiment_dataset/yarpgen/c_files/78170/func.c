/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78170
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78170 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78170
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */_Bool) var_11);
    var_15 = ((/* implicit */_Bool) (~(((/* implicit */int) (unsigned short)32768))));
    var_16 = min((min((((/* implicit */int) (unsigned char)192)), (470741752))), (((/* implicit */int) ((((((/* implicit */_Bool) var_7)) ? (470741743) : (((/* implicit */int) var_10)))) <= (((/* implicit */int) var_1))))));
    /* LoopSeq 1 */
    for (short i_0 = 3; i_0 < 17; i_0 += 1) 
    {
        arr_3 [i_0] [i_0] = ((/* implicit */unsigned short) (+(((/* implicit */int) max((((/* implicit */unsigned short) arr_1 [i_0])), (arr_2 [i_0] [i_0 + 2]))))));
        var_17 += ((/* implicit */_Bool) arr_2 [i_0 + 1] [i_0 + 2]);
        var_18 = ((((-470741752) + (2147483647))) >> (((-470741753) + (470741753))));
    }
}
