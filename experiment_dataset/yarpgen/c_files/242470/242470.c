/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242470
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242470 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242470
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_12;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_5 [i_0] |= (((+(((-2147483647 - 1) - -15076)))));
                arr_6 [i_0] = (max((max((min(var_7, var_2)), ((max((-127 - 1), 248))))), var_7));
                arr_7 [i_0] |= (((!(arr_1 [i_0]))) ? ((~(arr_3 [i_0] [i_0] [i_0]))) : (1481685425 - 122));
                arr_8 [i_1] = ((((min((arr_4 [i_0] [i_0]), ((((arr_0 [i_1]) || var_6)))))) ? (!106) : var_5));
            }
        }
    }
    var_20 = var_8;
    #pragma endscop
}
