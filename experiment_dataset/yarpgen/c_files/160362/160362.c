/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160362
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160362 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160362
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (max(67553994410557440, (min(67553994410557459, var_6))));
    var_12 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            {
                var_13 = ((arr_1 [i_0]) ? (arr_0 [i_0] [i_0]) : (((arr_3 [i_1 - 2] [i_1 + 1]) * 4294967295)));
                arr_4 [i_0] [i_1] [i_1 + 1] = (~-67553994410557431);
                arr_5 [i_0] [i_1] [i_1] = ((((-(((arr_1 [i_0]) ? 3010325186823697065 : var_10)))) * (arr_0 [i_0] [i_0])));
            }
        }
    }
    #pragma endscop
}
