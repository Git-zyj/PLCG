#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

unsigned char var_1 = (unsigned char)117;
unsigned char var_5 = (unsigned char)194;
long long int var_7 = 6386233390062129023LL;
unsigned char var_11 = (unsigned char)196;
long long int var_13 = -4048023917943120250LL;
int zero = 0;
unsigned char var_14 = (unsigned char)248;
unsigned char var_15 = (unsigned char)142;
long long int var_16 = -8918057864554390341LL;
long long int var_17 = -222840314572055500LL;
void init() {
}

void checksum() {
    hash(&seed, var_14);
    hash(&seed, var_15);
    hash(&seed, var_16);
    hash(&seed, var_17);
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
