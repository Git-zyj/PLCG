/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226646
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226646 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226646
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_19 = (max(var_19, ((max((min(((~(arr_1 [i_0] [i_0]))), (~1))), -21728)))));
        arr_2 [i_0] = (((((var_16 ? -1948407599 : -1948407587))) && (((-1948407599 ? -6123980893687122117 : ((((-1948407599 <= (-2147483647 - 1))))))))));
    }
    var_20 = (max(var_20, (((!(((var_13 ? -1948407599 : (min(123, 2796802100))))))))));
    #pragma endscop
}
