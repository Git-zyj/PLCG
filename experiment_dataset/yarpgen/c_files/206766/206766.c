/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206766
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206766 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206766
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((~((var_1 ^ ((min(var_9, var_5)))))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_0] = var_5;
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        {
                            var_11 &= (((arr_0 [i_0]) ? (((((var_0 ? var_2 : var_8))) ? (((arr_11 [1] [i_1] [i_2 + 1]) ? (arr_2 [i_3]) : var_5)) : (var_4 || var_3))) : var_7));
                            var_12 = (arr_8 [i_0 - 1] [i_2 + 1] [i_3]);
                            var_13 = (~var_3);
                            var_14 = (min((((max(var_3, var_2)))), ((var_1 ? ((var_8 ? var_5 : var_8)) : var_3))));
                            var_15 = ((~(((arr_11 [i_2 + 1] [i_0 + 1] [i_0]) ? var_1 : (arr_11 [i_2 + 1] [i_0 - 1] [i_0])))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
