/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94229
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94229 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94229
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (((max(((var_12 ? var_18 : 11561)), var_5)) == var_16));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    arr_7 [i_2] [i_2] [i_1] [i_0] |= (((((~(arr_1 [i_1])))) ? 9187343239835811840 : (arr_2 [i_0] [i_1])));
                    arr_8 [i_2] [i_2] [i_2] = ((-(max(var_8, ((112 ? 32768 : 150))))));
                }
            }
        }
    }
    var_21 = ((430735480832909862 ? -1 : var_5));
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 13;i_3 += 1)
    {
        for (int i_4 = 4; i_4 < 12;i_4 += 1)
        {
            {
                var_22 = 32768;
                var_23 = (arr_10 [i_3] [i_4]);
            }
        }
    }
    #pragma endscop
}
