/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172403
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172403 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172403
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1) /* same iter space */
    {
        var_12 = (max(var_12, ((((var_1 & (254 ^ 8191)))))));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    arr_10 [i_0] [i_1] [10] = ((!(((!249) && 67))));
                    var_13 = (max(var_13, (arr_2 [10])));
                    var_14 = -1529370264;
                }
            }
        }
        arr_11 [i_0] = (1529370264 * 1529370264);
    }
    for (int i_3 = 0; i_3 < 13;i_3 += 1) /* same iter space */
    {
        var_15 = (max(var_15, 2765597005));
        arr_14 [i_3] = (arr_9 [i_3] [i_3] [i_3]);
    }
    var_16 -= ((var_7 ? var_7 : -2008597527));
    #pragma endscop
}
