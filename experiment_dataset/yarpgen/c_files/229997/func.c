/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229997
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229997 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229997
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned short) ((((/* implicit */_Bool) ((((((/* implicit */int) (_Bool)1)) > (-549857266))) ? (((/* implicit */int) var_2)) : (((/* implicit */int) (unsigned short)25150))))) ? (((/* implicit */int) (_Bool)0)) : (((/* implicit */int) var_2))));
    /* LoopNest 2 */
    for (unsigned short i_0 = 1; i_0 < 24; i_0 += 3) 
    {
        for (int i_1 = 2; i_1 < 24; i_1 += 4) 
        {
            {
                var_15 = ((/* implicit */int) arr_3 [i_0 - 1]);
                var_16 = ((/* implicit */short) arr_3 [i_1]);
                var_17 = ((/* implicit */_Bool) min((var_17), (((/* implicit */_Bool) var_13))));
            }
        } 
    } 
}
