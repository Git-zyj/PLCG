/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209121
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209121 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209121
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 18;i_2 += 1)
            {
                {
                    var_15 = (arr_3 [i_1] [i_2 - 1]);
                    arr_6 [i_0] [i_1] [i_0] = (((((-(((arr_5 [i_0] [i_1]) + (arr_3 [i_0] [i_2 - 1])))))) + (((535822336 < -3191365562305580367) ? 36028795945222144 : 15568226928522967088))));
                }
            }
        }
    }
    var_16 = var_2;
    var_17 = (var_6 + 3191365562305580347);
    var_18 = (((max(-3191365562305580342, 3759144950)) + var_11));
    #pragma endscop
}
