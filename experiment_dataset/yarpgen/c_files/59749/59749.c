/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59749
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59749 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59749
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_12;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_5 [i_0] = (arr_0 [i_0 + 4] [i_0 - 2]);
                arr_6 [i_0] = -9104061412962856204;
                var_19 = (max((((arr_2 [i_0]) ? (arr_2 [i_0]) : 104)), (((((var_6 ? var_8 : (arr_4 [i_0])))) ? 151 : (arr_3 [i_0] [i_0])))));
            }
        }
    }
    var_20 = (((!var_13) ? var_11 : var_8));
    #pragma endscop
}
