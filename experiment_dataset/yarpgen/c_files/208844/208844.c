/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208844
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208844 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208844
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_6;
    var_13 = ((((var_5 << (var_8 - 1616704821714407306)))));
    var_14 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] [2] = (min((((((!(arr_3 [8]))) ? (((~(arr_0 [i_0 + 2])))) : (arr_2 [i_1] [4])))), -var_8));
                var_15 = ((+(min(((var_2 << (((((arr_0 [i_1]) + 19368)) - 19)))), var_5))));
            }
        }
    }
    var_16 = ((((min((var_4 > var_9), var_10))) ? (((((-10 ? 44 : 60))) ? var_11 : var_11)) : 508));
    #pragma endscop
}
