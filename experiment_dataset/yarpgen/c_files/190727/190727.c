/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190727
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190727 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190727
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_16, ((((var_12 <= var_12) % var_8)))));
    var_17 = var_6;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 7;i_2 += 1)
            {
                {
                    var_18 = (max(var_18, (var_14 % var_2)));
                    var_19 += ((3 >= (62 % 220)));
                    arr_6 [i_0] [i_1] [i_0] [i_0] = (arr_4 [i_2 - 2] [i_2 - 1] [i_2 + 2]);
                }
            }
        }
    }

    for (int i_3 = 0; i_3 < 17;i_3 += 1)
    {
        var_20 = (min(var_20, ((((133 ^ 255) >= (((((((arr_9 [i_3]) <= var_9))) >= var_8))))))));
        var_21 = (((136 % 11) >= ((28 % (arr_9 [i_3])))));
    }
    #pragma endscop
}
