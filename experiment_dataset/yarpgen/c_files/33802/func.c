/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33802
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33802 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33802
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (short i_1 = 2; i_1 < 22; i_1 += 3) 
        {
            {
                var_10 &= ((/* implicit */signed char) ((unsigned char) ((_Bool) ((int) 747418078))));
                var_11 += ((/* implicit */unsigned short) ((unsigned int) ((((/* implicit */int) (short)-25812)) >= (((/* implicit */int) arr_1 [i_0])))));
            }
        } 
    } 
    var_12 = ((/* implicit */unsigned int) var_7);
}
