/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 109744
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=109744 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/109744
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 22;i_1 += 1)
        {
            {
                var_13 = (max(var_13, (((var_12 ? (arr_3 [i_0]) : ((((0 != -4080986640082198909) ? ((1 ? -7847529342692336451 : var_4)) : (arr_2 [i_1])))))))));
                var_14 = (max(var_14, var_12));
                var_15 = (max(var_15, ((min(22688, (((((var_10 ? (arr_3 [i_0]) : 3885445028))) ? ((((arr_5 [i_0] [8]) || -22702))) : (((arr_0 [i_1]) / (arr_1 [11]))))))))));
            }
        }
    }
    var_16 = (~var_7);
    var_17 = var_5;
    #pragma endscop
}
