/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33821
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33821 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33821
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 |= var_5;
    var_21 = ((var_7 ? ((74 ? 64 : -34)) : 20));
    var_22 = ((~((((1 != var_10) || ((max(59, var_5))))))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    var_23 = ((34 ? -34 : 1));
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 19;i_4 += 1)
                        {
                            {
                                arr_14 [1] [i_3] [i_3] [i_1 - 2] [15] = 1526035409527749154;
                                arr_15 [i_3] [i_1] [0] [i_1 - 1] = (max(((((-30330 ? 18093 : 88)) << (arr_1 [i_1 - 1]))), (arr_7 [i_3 - 2] [i_3 + 4])));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
