/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36179
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36179 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36179
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 *= (!(((/* implicit */_Bool) (+(((/* implicit */int) ((384) == (var_18))))))));
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 23; i_0 += 3) 
    {
        for (long long int i_1 = 0; i_1 < 23; i_1 += 1) 
        {
            {
                /* LoopNest 3 */
                for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
                {
                    for (int i_3 = 0; i_3 < 23; i_3 += 3) 
                    {
                        for (unsigned long long int i_4 = 4; i_4 < 21; i_4 += 2) 
                        {
                            {
                                arr_16 [(signed char)4] [22LL] [i_4] |= ((/* implicit */unsigned long long int) (_Bool)0);
                                arr_17 [i_0] [i_1] = ((/* implicit */long long int) (!(((/* implicit */_Bool) (signed char)0))));
                            }
                        } 
                    } 
                } 
                var_20 = (((+(((/* implicit */int) arr_9 [i_0] [i_1] [i_1])))) + (((/* implicit */int) arr_9 [i_0] [21LL] [(unsigned short)20])));
            }
        } 
    } 
    var_21 = var_12;
    var_22 = ((/* implicit */signed char) (~(var_5)));
}
