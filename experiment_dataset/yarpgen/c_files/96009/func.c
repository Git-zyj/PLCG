/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 96009
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=96009 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/96009
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 |= ((/* implicit */signed char) (-(((/* implicit */int) var_3))));
    var_13 = ((/* implicit */unsigned short) var_7);
    var_14 = ((/* implicit */signed char) min((var_14), (((/* implicit */signed char) var_0))));
    var_15 = ((/* implicit */unsigned char) (-(((/* implicit */int) (signed char)-86))));
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (signed char i_1 = 0; i_1 < 19; i_1 += 4) 
        {
            {
                arr_4 [i_0] [i_0] = ((/* implicit */signed char) ((((/* implicit */_Bool) max((((/* implicit */int) arr_0 [i_0] [i_0])), ((+(((/* implicit */int) (unsigned char)95))))))) && (((/* implicit */_Bool) (-(((/* implicit */int) arr_1 [i_1])))))));
                /* LoopNest 2 */
                for (signed char i_2 = 3; i_2 < 18; i_2 += 4) 
                {
                    for (unsigned short i_3 = 0; i_3 < 19; i_3 += 4) 
                    {
                        {
                            var_16 *= ((/* implicit */_Bool) (-(((/* implicit */int) var_11))));
                            var_17 |= ((/* implicit */_Bool) (+(min((((/* implicit */unsigned int) arr_8 [i_0] [i_1] [i_2] [i_1])), (((((/* implicit */unsigned int) ((/* implicit */int) var_1))) + (arr_5 [i_2] [i_3] [i_2] [i_3])))))));
                        }
                    } 
                } 
                var_18 |= ((/* implicit */unsigned int) arr_8 [i_0] [(_Bool)1] [(_Bool)1] [i_1]);
            }
        } 
    } 
}
