/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150628
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150628 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150628
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (((4294967295 || var_7) <= (((!var_11) % (var_6 > 9)))));

    for (int i_0 = 3; i_0 < 21;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] = ((~((var_6 << (var_2 != 31744)))));
        var_15 = (+((7884828240200401954 << (((!var_8) - 1)))));
    }
    /* vectorizable */
    for (int i_1 = 3; i_1 < 21;i_1 += 1) /* same iter space */
    {
        var_16 = (min(var_16, ((((~var_2) < ((~(arr_1 [i_1]))))))));
        arr_6 [i_1] = (((!3862552118) ^ 68));
    }
    var_17 += ((-(var_13 * var_11)));
    var_18 = ((var_4 * (432415178 <= 54)));
    #pragma endscop
}
