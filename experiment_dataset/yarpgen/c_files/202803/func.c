/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202803
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202803 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202803
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_16 &= ((/* implicit */signed char) var_8);
        arr_2 [(short)3] = ((/* implicit */short) max((((/* implicit */unsigned short) min((((/* implicit */signed char) (_Bool)1)), ((signed char)-119)))), (max((((/* implicit */unsigned short) (_Bool)1)), ((unsigned short)13454)))));
    }
    var_17 -= ((/* implicit */unsigned short) var_3);
}
