/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 145080
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=145080 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/145080
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned int) var_13);
    /* LoopNest 3 */
    for (short i_0 = 0; i_0 < 20; i_0 += 4) 
    {
        for (unsigned char i_1 = 1; i_1 < 16; i_1 += 3) 
        {
            for (long long int i_2 = 1; i_2 < 18; i_2 += 2) 
            {
                {
                    arr_9 [10] [i_1] [i_2 + 1] [i_2 + 1] = ((/* implicit */signed char) arr_6 [i_1] [i_1 + 4] [i_1]);
                    var_21 = ((/* implicit */unsigned long long int) (short)-3736);
                    var_22 = ((/* implicit */_Bool) max(((-(((/* implicit */int) arr_8 [i_0] [i_1 + 3] [(unsigned short)11] [i_2 + 2])))), ((-(((/* implicit */int) arr_8 [i_0] [i_1 + 2] [i_2] [i_2 - 1]))))));
                    var_23 = ((/* implicit */unsigned char) ((long long int) var_15));
                }
            } 
        } 
    } 
    var_24 = ((/* implicit */int) var_16);
    var_25 = ((/* implicit */_Bool) 0);
    var_26 = ((/* implicit */unsigned char) (-(-22)));
}
