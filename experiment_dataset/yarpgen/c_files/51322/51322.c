/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51322
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51322 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51322
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_14;
    var_18 -= ((!((((var_8 % var_0) * (!1895))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    var_19 ^= var_16;
                    var_20 = (min(var_20, ((((arr_7 [i_2]) > (((var_2 ? var_14 : (arr_1 [i_1] [i_0])))))))));

                    for (int i_3 = 2; i_3 < 16;i_3 += 1)
                    {
                        var_21 = ((2147483647 ? (((-(((arr_9 [i_0] [9]) ? var_8 : var_8))))) : var_15));
                        var_22 = (((var_12 != ((min(var_4, (arr_11 [i_3])))))));
                    }
                    var_23 = var_5;
                }
            }
        }
    }
    var_24 = var_16;
    var_25 = var_13;
    #pragma endscop
}
