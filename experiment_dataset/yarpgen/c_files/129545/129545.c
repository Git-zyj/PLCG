/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 129545
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=129545 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/129545
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_7;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            {
                var_20 = ((((((((arr_2 [i_1]) ^ var_3)) >> (var_2 - 167143335592460504)))) ? ((((((arr_3 [i_0 + 1]) << (((arr_4 [2]) - 10612636126530803385))))) ? ((((arr_4 [i_0]) ? var_10 : (-2147483647 - 1)))) : (max(0, var_2)))) : (((((2147483647 ? (arr_1 [i_0]) : (arr_2 [i_1]))) & (arr_0 [12]))))));
                var_21 = (max(var_21, (((!((((2147483647 & var_5) ? 1 : 1))))))));
            }
        }
    }
    #pragma endscop
}
