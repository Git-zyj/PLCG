/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44548
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44548 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44548
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_0, var_5));
    var_12 = (min((!var_3), var_4));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_6 [13] [13] [i_0] = ((((arr_2 [i_0]) ? ((min(var_2, 225))) : (40 - var_6))));
                arr_7 [i_0] [i_1] [i_1] = (((((arr_0 [i_0 - 3]) ? (arr_0 [i_0 - 1]) : (arr_0 [i_0 - 2])))) ? (arr_3 [i_0] [i_0]) : (min(-2051400840505060527, 59)));
            }
        }
    }
    #pragma endscop
}
