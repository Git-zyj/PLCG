/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249630
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249630 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249630
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((32765 >> (var_8 + 3408044075898265068)));

    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        var_14 = -7506;
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_15 ^= (~(arr_6 [i_0] [i_1] [i_1] [i_1]));
                    arr_8 [i_0] [i_1] [i_0] = (((arr_7 [i_0 - 1] [i_0 + 1] [i_0]) ? ((max((arr_7 [i_0 + 1] [i_0 - 2] [i_0 - 2]), -32762))) : (((!(arr_7 [i_0 - 1] [i_0 + 1] [i_0]))))));
                }
            }
        }
    }
    for (int i_3 = 0; i_3 < 19;i_3 += 1)
    {
        arr_11 [i_3] [i_3] = (arr_10 [15]);
        var_16 = (!var_3);
    }
    #pragma endscop
}
