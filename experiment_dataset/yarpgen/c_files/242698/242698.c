/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242698
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242698 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242698
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((!(--65530)));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_20 -= ((((min((min((arr_5 [i_0 - 1]), -2985)), (2989 < 17628018756169510825)))) << ((((32765 & 32765) || ((min(var_12, 19297))))))));
                var_21 = (max(var_21, (((!((((46254 / 65535) | ((~(arr_3 [i_0] [i_0] [i_0])))))))))));
                var_22 = (+(((~4915) | ((((arr_1 [i_1] [10]) > var_8))))));
            }
        }
    }
    var_23 += (-((-(min(1098858100, 18101868908196816637)))));
    #pragma endscop
}
