/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14295
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14295 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14295
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 15;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0 + 1] [i_0 + 1] = var_10;
        var_20 = ((!((((((var_0 ? (arr_1 [i_0]) : var_1))) ? var_15 : (arr_0 [i_0]))))));
        var_21 = (((arr_0 [i_0 - 1]) >= (((((var_16 ? 14 : 12137))) & (arr_0 [i_0])))));

        /* vectorizable */
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {

            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                var_22 = (arr_7 [i_0] [i_1] [i_0]);
                var_23 = ((-(arr_3 [i_0] [i_1] [i_2])));
            }
            var_24 = var_3;
            arr_8 [i_1] [i_1] [i_1] = (((var_3 ? var_6 : var_4)));
            arr_9 [i_0 - 1] [i_1] [i_0 - 1] = (~242);
        }
    }
    /* vectorizable */
    for (int i_3 = 1; i_3 < 15;i_3 += 1) /* same iter space */
    {

        for (int i_4 = 0; i_4 < 17;i_4 += 1)
        {
            var_25 ^= ((83 << (78 - 69)));
            arr_14 [i_3] = ((var_14 ^ (var_17 - 167)));
        }
        for (int i_5 = 1; i_5 < 16;i_5 += 1)
        {
            var_26 = (arr_13 [i_3] [i_3 - 1] [i_3 - 1]);
            var_27 = (min(var_27, ((((arr_1 [i_3 + 1]) ? (arr_4 [13] [i_3 + 1]) : (arr_0 [i_5 + 1]))))));
        }
        arr_18 [i_3] = (arr_15 [i_3]);
    }
    var_28 &= ((-((-((var_15 ? var_19 : var_14))))));
    #pragma endscop
}
