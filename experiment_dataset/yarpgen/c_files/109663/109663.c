/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 109663
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=109663 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/109663
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_8;
    var_20 |= (((1 - 32752) * (96 + 39415)));

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        arr_4 [i_0] = (arr_1 [i_0]);
        var_21 = ((1350774472 ? (((((arr_3 [i_0]) / (arr_1 [i_0]))) + var_14)) : ((((((arr_1 [i_0]) / var_4))) / (((arr_2 [i_0]) ? (arr_0 [i_0] [14]) : var_8))))));
        var_22 = 4294967291;
        arr_5 [i_0] [i_0] = ((((-(arr_2 [i_0]))) > (((((arr_2 [i_0]) || var_18))))));
    }
    var_23 ^= (var_18 * var_15);
    #pragma endscop
}
