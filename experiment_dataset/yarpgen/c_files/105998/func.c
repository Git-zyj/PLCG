/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105998
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105998 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105998
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */unsigned int) ((((/* implicit */_Bool) var_16)) && (((/* implicit */_Bool) ((int) (signed char)95)))));
    var_18 = ((/* implicit */short) var_9);
    var_19 = ((/* implicit */unsigned int) (-(var_2)));
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 2; i_0 < 10; i_0 += 1) 
    {
        for (unsigned int i_1 = 3; i_1 < 9; i_1 += 1) 
        {
            {
                arr_6 [i_0] [i_1] [i_1] = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) (+((~(var_4))))))));
                var_20 = ((/* implicit */short) min((((/* implicit */long long int) min(((!(((/* implicit */_Bool) 7707550466373155577LL)))), ((!(((/* implicit */_Bool) (signed char)127))))))), (7707550466373155577LL)));
            }
        } 
    } 
}
