/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120130
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120130 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120130
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 24;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        {
                            var_18 = (min(var_18, (4262743221 - -1)));
                            arr_10 [i_2] [i_1] [20] [i_1] [i_1] [i_1] |= var_15;
                            var_19 = ((((((!(((var_11 % (-2147483647 - 1)))))))) == var_6));
                        }
                    }
                }
                arr_11 [i_0] [i_0] = ((-108 ? (((0 == 99) ? ((102 ? -32761 : -31645)) : -98)) : ((7 ? (~99) : -1268419607))));
                var_20 = (min(((((!var_10) | var_6))), var_3));
            }
        }
    }
    var_21 = var_0;
    #pragma endscop
}
