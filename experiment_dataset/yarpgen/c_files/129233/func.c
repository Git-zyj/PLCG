/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 129233
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=129233 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/129233
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */long long int) var_2);
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 11; i_0 += 1) 
    {
        for (_Bool i_1 = 1; i_1 < 1; i_1 += 1) 
        {
            {
                var_21 = ((/* implicit */_Bool) arr_0 [i_0]);
                var_22 = ((/* implicit */long long int) arr_2 [i_0]);
                var_23 = ((/* implicit */unsigned short) (+(((((/* implicit */_Bool) ((((/* implicit */unsigned int) var_13)) ^ (4294967284U)))) ? (7U) : (1873736058U)))));
                var_24 = ((/* implicit */_Bool) ((((/* implicit */_Bool) min((((/* implicit */unsigned long long int) arr_4 [i_1 - 1] [i_1])), (arr_1 [i_1 - 1] [i_1])))) ? (arr_1 [i_1 - 1] [i_1]) : (((4420928172274460786ULL) - (((/* implicit */unsigned long long int) 0LL))))));
            }
        } 
    } 
    var_25 += ((/* implicit */int) var_15);
}
