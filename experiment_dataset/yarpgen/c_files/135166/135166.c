/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135166
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135166 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135166
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = -3864510385907176963;

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        arr_3 [i_0] = ((((((arr_1 [i_0]) ? (arr_2 [i_0]) : var_1))) ? ((((((arr_0 [i_0] [i_0]) + (arr_0 [i_0] [i_0])))) ? 12253866950345005489 : 16196553787803945881)) : (((-(arr_1 [i_0]))))));
        var_14 = ((!((((arr_2 [i_0]) ? 2250190285905605751 : var_1)))));
        var_15 ^= ((!(arr_2 [i_0])));
    }
    #pragma endscop
}
