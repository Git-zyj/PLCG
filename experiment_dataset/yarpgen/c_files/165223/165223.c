/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165223
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165223 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165223
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_13 = (min(var_13, 18446744073709551615));
                arr_6 [i_0] = arr_1 [i_0] [i_0 + 1];
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 1; i_2 < 20;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 24;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 24;i_4 += 1)
            {
                {
                    arr_13 [i_3] [12] |= (((((2592823665597717056 ? ((max(-2592823665597717057, -2592823665597717059))) : (min(23, var_6))))) ? (max(((var_7 ? -2135636399843335318 : 18446744073709551592)), 9821627121030595431)) : 673544016154841819));
                    var_14 *= ((arr_12 [i_2 + 1] [i_2 + 4] [i_2 + 2] [i_2]) ? (~-2135636399843335318) : (((arr_10 [i_3] [i_3]) & 8625116952678956190)));
                }
            }
        }
    }
    #pragma endscop
}
