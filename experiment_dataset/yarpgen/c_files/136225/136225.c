/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136225
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136225 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136225
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_15 = ((((((((118 <= (arr_3 [i_0]))))) * (max((arr_3 [9]), var_4)))) == (((arr_2 [i_1]) * (arr_3 [i_1])))));
                var_16 = (((((var_9 ? (arr_2 [i_1]) : var_13))) ? 1797702431 : ((((min(11030, 1164813717))) ? (arr_1 [i_0]) : ((var_1 ? 1164813716 : 139611588448485376))))));
            }
        }
    }
    var_17 = var_0;
    #pragma endscop
}
