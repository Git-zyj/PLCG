/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197612
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197612 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197612
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */long long int) (+(((/* implicit */int) var_0))));
    var_12 = ((/* implicit */unsigned short) var_4);
    var_13 = ((/* implicit */signed char) (+(min((max((((/* implicit */long long int) (unsigned short)35990)), (var_8))), (((/* implicit */long long int) var_5))))));
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 15; i_0 += 1) 
    {
        var_14 = ((/* implicit */unsigned long long int) max((((/* implicit */int) ((short) arr_1 [i_0]))), ((-(((/* implicit */int) (unsigned char)8))))));
        var_15 = ((/* implicit */unsigned long long int) (+(min((((/* implicit */long long int) (+(arr_1 [i_0])))), (((long long int) 0U))))));
    }
    var_16 = ((/* implicit */unsigned long long int) (~(var_2)));
}
