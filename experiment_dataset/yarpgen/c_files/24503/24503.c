/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24503
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24503 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24503
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_10 = var_3;
        var_11 = (((arr_1 [i_0]) ? (arr_1 [i_0]) : (arr_0 [i_0])));
        var_12 = -2642;
        var_13 = (((2642 - 3) < var_6));
        arr_2 [i_0] = var_1;
    }
    /* LoopNest 3 */
    for (int i_1 = 3; i_1 < 14;i_1 += 1)
    {
        for (int i_2 = 2; i_2 < 14;i_2 += 1)
        {
            for (int i_3 = 3; i_3 < 14;i_3 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_4 = 1; i_4 < 13;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 16;i_5 += 1)
                        {
                            {
                                var_14 = ((!((((-2104593845 + 2147483647) >> (((arr_3 [i_4 + 3] [i_2]) + 8788274147697795585)))))));
                                arr_17 [7] [i_2] [i_3 + 2] [i_4] [i_5] = ((min(9223372036854775789, 125)));
                                var_15 += arr_14 [i_1] [i_2] [i_3] [i_2] [i_5];
                                arr_18 [i_1] [i_1 + 2] [i_1] [5] [i_5] = (min((~4294967295), ((((var_7 + 2147483647) << (var_6 - 32))))));
                            }
                        }
                    }
                    var_16 = (arr_7 [i_3 - 3] [11] [i_1 - 2]);
                    var_17 -= (81 << 4);
                }
            }
        }
    }
    var_18 = var_9;
    #pragma endscop
}
