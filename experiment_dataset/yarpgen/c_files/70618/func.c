/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70618
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70618 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70618
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 += ((/* implicit */_Bool) min((((/* implicit */int) ((((/* implicit */_Bool) var_8)) && (((((/* implicit */int) var_13)) > (((/* implicit */int) var_13))))))), (((((var_7) ? (((/* implicit */int) (_Bool)1)) : (((/* implicit */int) var_6)))) - (((/* implicit */int) var_4))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18; i_0 += 1) 
    {
        for (int i_1 = 0; i_1 < 18; i_1 += 4) 
        {
            {
                var_15 = ((/* implicit */_Bool) (+(-1000186533)));
                var_16 = ((/* implicit */int) var_11);
            }
        } 
    } 
    var_17 = ((/* implicit */unsigned int) var_5);
    var_18 = ((int) ((((/* implicit */int) var_9)) > (var_2)));
}
