/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144869
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144869 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144869
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_9;
    var_16 = var_7;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_17 ^= ((((-((18446744073709551615 ? 3492911968 : 2047)))) / (arr_1 [i_1] [i_1])));
                var_18 = (var_10 ? (!511) : (((~1) ? (((arr_5 [i_0] [i_0] [i_0]) & (arr_0 [i_0] [i_1]))) : (arr_2 [i_0]))));
                var_19 -= (18446744073709551615 * -var_3);
                arr_6 [i_0] = (arr_5 [i_0] [i_0] [i_0]);
            }
        }
    }
    #pragma endscop
}
