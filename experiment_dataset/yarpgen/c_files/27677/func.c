/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27677
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27677 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27677
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 += ((/* implicit */unsigned char) (-(var_8)));
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 21; i_0 += 1) 
    {
        var_19 += ((/* implicit */short) arr_0 [i_0]);
        var_20 = ((/* implicit */int) max((((/* implicit */unsigned int) min(((short)992), (((/* implicit */short) var_12))))), (arr_0 [i_0])));
        arr_2 [i_0] = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) -9223372036854775795LL))));
    }
}
