/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18665
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18665 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18665
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = var_10;
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 15; i_0 += 2) 
    {
        for (int i_1 = 1; i_1 < 11; i_1 += 1) 
        {
            {
                var_21 = ((/* implicit */long long int) (+(arr_3 [i_0] [i_1])));
                var_22 = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) arr_0 [i_1 + 4]))));
                var_23 = ((/* implicit */long long int) ((((/* implicit */int) (short)10705)) << (((((/* implicit */int) (signed char)72)) - (67)))));
            }
        } 
    } 
    var_24 = ((/* implicit */unsigned short) min((var_24), (((/* implicit */unsigned short) var_14))));
}
