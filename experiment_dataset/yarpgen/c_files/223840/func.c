/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 223840
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=223840 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/223840
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 -= ((/* implicit */short) ((((/* implicit */unsigned int) ((int) (!(((/* implicit */_Bool) var_6)))))) > (min((((/* implicit */unsigned int) var_2)), (var_4)))));
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 18; i_0 += 4) 
    {
        var_11 -= ((/* implicit */long long int) var_2);
        var_12 = ((/* implicit */signed char) min((var_12), (((/* implicit */signed char) (-(((/* implicit */int) (_Bool)0)))))));
    }
    var_13 = ((/* implicit */long long int) (unsigned short)23292);
    var_14 = ((/* implicit */signed char) ((unsigned int) var_5));
    var_15 = ((/* implicit */unsigned short) var_7);
}
