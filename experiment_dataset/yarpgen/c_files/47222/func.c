/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47222
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47222 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47222
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned int) var_1);
    /* LoopNest 2 */
    for (unsigned char i_0 = 1; i_0 < 18; i_0 += 2) 
    {
        for (signed char i_1 = 0; i_1 < 20; i_1 += 1) 
        {
            {
                var_13 = ((/* implicit */unsigned char) var_0);
                var_14 = ((/* implicit */short) max((((/* implicit */long long int) (((_Bool)0) ? (((/* implicit */unsigned int) ((/* implicit */int) (unsigned char)8))) : (3361537982U)))), ((~(arr_1 [i_0 + 2])))));
            }
        } 
    } 
    /* LoopNest 2 */
    for (unsigned short i_2 = 1; i_2 < 15; i_2 += 2) 
    {
        for (int i_3 = 3; i_3 < 12; i_3 += 4) 
        {
            {
                arr_8 [i_2] [i_2] = 118321805803921872LL;
                var_15 = ((/* implicit */_Bool) min((arr_4 [i_2 + 1]), ((+(arr_4 [i_3])))));
            }
        } 
    } 
}
