/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239221
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239221 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239221
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */int) (-(var_0)));
    var_17 &= ((/* implicit */unsigned long long int) var_13);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 15; i_0 += 3) 
    {
        for (short i_1 = 0; i_1 < 15; i_1 += 2) 
        {
            {
                var_18 = ((/* implicit */unsigned short) min((((/* implicit */long long int) (+((-(((/* implicit */int) var_12))))))), (9223372036854775807LL)));
                arr_6 [i_1] [i_0] = ((/* implicit */unsigned long long int) arr_0 [i_0] [i_0]);
                arr_7 [i_0] [i_0] = ((/* implicit */short) (!((!(((/* implicit */_Bool) var_1))))));
            }
        } 
    } 
}
