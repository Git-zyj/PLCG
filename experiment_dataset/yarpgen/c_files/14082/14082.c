/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14082
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14082 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14082
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 -= var_4;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 18;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_1] [0] = var_2;
                    var_15 |= (((2098813892 || var_0) > (var_7 > var_8)));
                    var_16 = var_6;
                    var_17 = (((((var_1 ? var_13 : var_4))) ? (var_9 == var_11) : (var_4 >= var_1)));
                    var_18 = (min(var_18, ((var_7 <= (((var_12 >= 5596410269063146159) ? (165 < 9052038088266282199) : var_9))))));
                }
            }
        }
    }
    var_19 = (((149 == var_8) < var_0));
    #pragma endscop
}
