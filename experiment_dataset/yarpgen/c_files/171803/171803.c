/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171803
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171803 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171803
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_14 = (arr_2 [i_0]);
        var_15 ^= (((((((max(var_2, (arr_2 [i_0])))) ? -var_11 : var_5))) & ((var_9 ^ (min((arr_2 [i_0]), var_5))))));
    }
    var_16 &= ((((min(((var_4 >> (var_0 + 132))), (~var_5)))) ? (min((~var_13), ((max(var_5, var_12))))) : var_8));
    var_17 = ((var_1 < (((var_13 > var_0) | ((var_9 ? var_8 : var_2))))));
    #pragma endscop
}
