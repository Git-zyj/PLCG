/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152944
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152944 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152944
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        arr_3 [i_0] = (((170 ? 31129 : (arr_1 [i_0]))));
        var_17 -= (((((arr_1 [i_0]) ? (arr_1 [i_0]) : (arr_1 [i_0]))) << ((((~(arr_1 [i_0]))) - 1181794879))));
    }

    for (int i_1 = 0; i_1 < 23;i_1 += 1)
    {
        var_18 = (((arr_5 [i_1]) * (((((41552 ? 15808026939156172424 : (arr_5 [i_1]))) < 23)))));
        var_19 = (-((min(40, var_5))));
        arr_8 [i_1] = 12105723091875009514;
        var_20 = 1;
    }
    #pragma endscop
}
