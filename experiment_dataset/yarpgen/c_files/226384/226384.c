/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226384
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226384 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226384
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = min(var_3, var_10);

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        arr_3 [i_0] &= (((((((arr_0 [i_0]) ? 2147483647 : 18446744073709551603))) ? (max(-2061646922, var_11)) : -var_11)));
        var_15 = (~(arr_0 [i_0]));

        for (int i_1 = 4; i_1 < 11;i_1 += 1)
        {
            arr_7 [i_1 - 3] = var_4;
            arr_8 [i_0] = ((-1883914469 && (~2061646932)));
            var_16 = var_13;
        }
        for (int i_2 = 0; i_2 < 13;i_2 += 1)
        {
            arr_11 [7] [i_0] [i_2] = 58720256;
            arr_12 [i_0] [i_2] = 16175439258829124201;
        }
    }
    var_17 = (~2147483647);
    #pragma endscop
}
