/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77213
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77213 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77213
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_10 = (max((!28329), (arr_3 [i_0 - 2])));
                var_11 = (((2783413611 % 1703426750637981841) < (max(18446744073709551615, (arr_1 [i_0] [i_1])))));
                var_12 |= (2783413581 - 62);
                arr_5 [i_0] [i_1] = var_5;
            }
        }
    }
    var_13 = (min(var_13, ((max(((((max(var_3, var_2)) < (2783413591 != var_1)))), ((((1 ? -13 : var_5)) / (~0))))))));
    #pragma endscop
}
