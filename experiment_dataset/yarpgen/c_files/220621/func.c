/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220621
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220621 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220621
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 += ((/* implicit */unsigned char) (~(max((((/* implicit */int) (signed char)-99)), ((~(((/* implicit */int) (signed char)-99))))))));
    /* LoopNest 2 */
    for (short i_0 = 1; i_0 < 17; i_0 += 4) 
    {
        for (short i_1 = 1; i_1 < 18; i_1 += 1) 
        {
            {
                var_20 = ((/* implicit */short) (_Bool)0);
                var_21 = ((/* implicit */unsigned char) (~(((/* implicit */int) min((((/* implicit */unsigned short) (signed char)-99)), (arr_3 [i_0]))))));
            }
        } 
    } 
    /* LoopNest 3 */
    for (unsigned short i_2 = 0; i_2 < 11; i_2 += 4) 
    {
        for (unsigned short i_3 = 2; i_3 < 8; i_3 += 1) 
        {
            for (unsigned long long int i_4 = 0; i_4 < 11; i_4 += 3) 
            {
                {
                    var_22 = ((/* implicit */_Bool) (+(((/* implicit */int) max(((unsigned short)32640), (((/* implicit */unsigned short) arr_2 [i_2] [i_3 - 2] [i_4])))))));
                    var_23 = ((/* implicit */signed char) 9223372036854775807LL);
                }
            } 
        } 
    } 
}
