/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62503
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62503 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62503
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 -= var_2;

    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        var_17 &= (arr_2 [i_0 + 1]);
        var_18 -= (max((min((max(var_5, (arr_0 [i_0]))), ((max(var_4, var_15))))), ((max(2147483648, (((arr_1 [7] [i_0]) & (arr_1 [i_0] [i_0]))))))));
        var_19 = (((arr_2 [i_0]) < (((arr_0 [i_0]) ? (min(var_11, (arr_0 [i_0 + 1]))) : (arr_0 [i_0 + 2])))));
    }
    #pragma endscop
}
