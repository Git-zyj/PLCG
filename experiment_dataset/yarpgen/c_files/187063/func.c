/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187063
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187063 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187063
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */signed char) var_2);
    var_15 = ((/* implicit */unsigned char) var_6);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10; i_0 += 4) 
    {
        for (int i_1 = 0; i_1 < 10; i_1 += 3) 
        {
            {
                var_16 = ((/* implicit */int) max((var_16), (((/* implicit */int) arr_4 [(signed char)6] [i_0] [7]))));
                var_17 = ((long long int) max((arr_2 [i_1]), (arr_4 [(signed char)0] [i_0] [6LL])));
                arr_5 [i_1] [i_1] = ((/* implicit */unsigned char) ((((/* implicit */_Bool) ((unsigned int) (+(18446744073709551615ULL))))) ? ((-((-(((/* implicit */int) arr_3 [i_1])))))) : ((+(((/* implicit */int) arr_3 [i_1]))))));
            }
        } 
    } 
    var_18 = ((/* implicit */signed char) min((var_18), (((/* implicit */signed char) var_3))));
}
