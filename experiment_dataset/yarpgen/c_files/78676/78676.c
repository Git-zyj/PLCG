/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78676
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78676 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78676
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min(var_2, var_2));

    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_14 = ((((arr_4 [i_0 + 1] [i_2] [i_2] [i_2]) ? (arr_4 [i_0 - 1] [i_2] [i_0 - 1] [i_2]) : (arr_5 [i_0 + 1] [i_0] [i_2] [i_2]))));
                    arr_7 [i_0] [i_1] [i_0] [i_2] = ((-(((arr_3 [i_2] [i_0] [i_0]) ? (arr_2 [i_0 + 1]) : (arr_1 [i_0])))));
                }
            }
        }
        var_15 = (max(var_15, ((((!((((arr_1 [i_0]) ? (arr_1 [i_0]) : var_11))))) && (((((var_9 || (arr_2 [i_0]))) ? (!var_9) : (arr_3 [i_0 - 2] [i_0 - 2] [i_0 - 2]))))))));
    }
    #pragma endscop
}
