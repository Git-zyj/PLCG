/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 207618
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=207618 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/207618
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (min(var_4, (((!var_3) << (var_9 + 76)))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_21 = ((!((((arr_0 [i_0 - 1] [i_0]) ? var_11 : (((-38 && (arr_4 [i_0] [i_0] [i_0])))))))));
                arr_6 [i_0] [i_0] [i_0] = ((((max(var_13, ((var_19 ? var_19 : (arr_0 [i_1] [i_0])))))) ? (var_12 || var_9) : var_18));
                arr_7 [i_0] [i_1] = -5261;
            }
        }
    }
    var_22 = var_0;
    var_23 = var_7;
    #pragma endscop
}
