/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106938
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106938 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106938
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 24;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 24;i_4 += 1)
                        {
                            {
                                var_19 = (min(((((!18446744073709551615) ? (~0) : -1023321956241499017))), (((((~(arr_0 [i_2] [1])))) ? 18446744073709551615 : (-127 - 1)))));
                                var_20 = (arr_1 [i_0]);
                            }
                        }
                    }
                }
                arr_16 [i_0] [11] = arr_6 [i_0] [20] [i_1] [i_0];
            }
        }
    }
    var_21 = ((max((var_5 || var_3), (min(var_10, var_0)))));
    #pragma endscop
}
