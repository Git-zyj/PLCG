/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224082
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224082 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224082
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 7;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_17 = (min(var_17, ((max(var_7, (!var_10))))));
                    arr_9 [8] [i_1] [i_2] [i_0] |= (min(var_16, ((((arr_2 [i_1]) ^ (arr_7 [i_0] [i_0] [i_2]))))));
                    arr_10 [i_0] = 2821;
                }
            }
        }
    }

    for (int i_3 = 0; i_3 < 17;i_3 += 1)
    {
        var_18 = (((((2790 ? 2758 : 2821))) ? (((((8191 * 205) > -46)))) : (arr_11 [i_3] [11])));
        arr_13 [7] [i_3] = (~62772);
        arr_14 [i_3] &= 4294959105;
    }
    #pragma endscop
}
