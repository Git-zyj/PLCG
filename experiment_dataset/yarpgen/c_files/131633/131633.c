/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131633
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131633 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131633
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_21 += ((-(((((var_14 ? (arr_3 [i_1] [i_0]) : (arr_0 [i_0 + 1] [i_0 + 1])))) ? var_10 : ((1208135842 ? (arr_2 [i_0] [i_1] [i_1]) : var_12))))));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 13;i_4 += 1)
                        {
                            {
                                arr_13 [i_4] [i_2] [1] [i_0] [i_0] [i_2] [i_0] = (arr_4 [i_4]);
                                var_22 = (arr_10 [i_1] [1] [4]);
                            }
                        }
                    }
                }
            }
        }
    }
    var_23 = var_11;
    #pragma endscop
}
