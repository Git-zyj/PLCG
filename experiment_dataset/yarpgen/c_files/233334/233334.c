/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233334
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233334 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233334
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [i_1] = (i_1 % 2 == 0) ? (((6 >> (((arr_2 [i_0] [i_0] [i_1]) - 10481752557264484209))))) : (((6 >> (((((arr_2 [i_0] [i_0] [i_1]) - 10481752557264484209)) - 6285809301146323207)))));
                var_16 *= ((18446744073709551610 ? 6 : 8000233189181241133));
                var_17 = (max(var_17, var_15));
            }
        }
    }
    var_18 = (!var_0);
    #pragma endscop
}
