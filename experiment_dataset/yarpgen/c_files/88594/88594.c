/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88594
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88594 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88594
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            {
                var_20 = ((var_5 << (17097 - 17091)));

                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    var_21 += (!14);
                    var_22 = (min(((1857876579 != (arr_4 [i_1 + 1]))), (((arr_6 [i_1 - 2] [i_1 - 1] [i_1 - 1]) >= (max((arr_5 [i_2]), (arr_8 [i_0] [i_1 + 1] [i_2])))))));
                    var_23 = (arr_1 [i_1 + 1]);
                    var_24 = max(((((arr_3 [i_1 - 2] [i_1 - 1]) < (arr_3 [i_1 - 2] [i_1 - 1])))), (arr_3 [i_1 - 2] [i_1 - 1]));
                    arr_9 [i_2] [i_1 - 2] [i_0] = (((-127 - 1) | ((~(arr_2 [i_1 + 1])))));
                }
            }
        }
    }
    var_25 = (!var_7);
    #pragma endscop
}
