/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89326
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89326 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89326
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_14 = (min(((max((max((arr_2 [i_0] [i_0]), -25149)), 45484))), (min((((-(arr_1 [i_0])))), var_9))));
        /* LoopNest 2 */
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    var_15 = ((((arr_6 [i_1 + 1]) ? (arr_6 [i_1 + 2]) : var_7)));
                    /* LoopNest 2 */
                    for (int i_3 = 4; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                var_16 = (max(241, 127));
                                arr_14 [i_0] [i_2] [i_0] |= (arr_13 [i_1 + 1] [i_3 + 1] [i_3 - 3] [i_3 + 1] [i_3 - 4] [i_3 + 2]);
                            }
                        }
                    }
                    var_17 = 3;
                }
            }
        }
        var_18 = var_8;
    }
    var_19 = (~var_7);
    #pragma endscop
}
