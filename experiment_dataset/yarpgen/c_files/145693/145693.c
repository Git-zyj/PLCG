/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 145693
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=145693 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/145693
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 4; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 18;i_4 += 1)
                        {
                            {
                                var_19 = ((~(min(-7369387424285923061, 7991935549024363511))));
                                var_20 *= ((((max(1, 7369387424285923061))) ? 7991935549024363514 : (arr_1 [i_2])));
                            }
                        }
                    }
                }
                arr_14 [i_0 - 1] [i_1] = -var_6;
            }
        }
    }
    var_21 = (((~var_6) ? var_14 : ((var_18 ? var_7 : (((7369387424285923061 ? var_18 : 52)))))));
    #pragma endscop
}
