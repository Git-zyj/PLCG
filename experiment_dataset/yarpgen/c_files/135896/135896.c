/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135896
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135896 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135896
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 += var_2;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_9 [i_0] [1] [i_1] [i_0] = (((((((min((-32767 - 1), 63))) ? (arr_4 [i_2] [i_0]) : (arr_4 [i_2] [i_0])))) ? ((1 ? 88 : (((arr_2 [i_2] [i_2] [i_1]) | 1)))) : (((var_1 / (arr_8 [i_2] [i_1] [i_0]))))));
                    var_11 ^= var_6;
                    var_12 = (((((arr_3 [i_0] [i_1] [i_0]) * (arr_7 [i_2] [i_0]))) == var_5));
                }
            }
        }
    }
    #pragma endscop
}
