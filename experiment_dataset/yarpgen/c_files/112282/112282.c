/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112282
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112282 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112282
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((!((max(13929945300224402136, -8443))))));
    var_20 = (max(var_2, var_7));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_21 = ((((((arr_0 [i_0]) ? (min(var_7, (arr_3 [i_0] [i_0] [3]))) : ((var_15 ? var_3 : var_4))))) || (((-(var_7 && var_10))))));
                var_22 = (arr_3 [i_0] [i_1] [i_1]);
            }
        }
    }
    var_23 = ((-var_10 || (((min(var_3, var_5)) < var_7))));
    var_24 = var_2;
    #pragma endscop
}
