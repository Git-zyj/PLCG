/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 592
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=592 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/592
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 ^= var_4;

    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        var_13 = min((((((arr_0 [i_0 + 2] [i_0]) > -31962)) ? (arr_0 [i_0 - 1] [i_0 + 1]) : (arr_2 [i_0]))), (arr_2 [i_0]));
        var_14 ^= ((((max((arr_2 [i_0 + 1]), (arr_2 [i_0 + 1])))) > (min((arr_0 [i_0] [i_0]), (((arr_2 [i_0]) ? var_3 : (arr_2 [i_0])))))));
        arr_3 [i_0 + 1] = 32166;
        var_15 += (((arr_2 [i_0 + 2]) ? (((!(arr_2 [i_0 - 1])))) : ((min((arr_2 [i_0 + 1]), (arr_2 [i_0 + 1]))))));
    }
    var_16 &= -var_4;
    #pragma endscop
}
