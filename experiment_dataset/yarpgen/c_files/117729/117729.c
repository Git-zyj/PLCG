/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117729
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117729 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117729
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = 89;
    var_16 = -90;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        arr_10 [i_2] [i_1] = 12481891448495254803;
                        var_17 = (min(var_17, ((((arr_1 [i_2]) ? 255 : -5154696)))));
                        arr_11 [i_0] [3] [i_2] [i_0] [i_0] = var_10;
                    }
                    arr_12 [i_2] [i_1] [10] [1] = (((arr_4 [i_0] [i_1]) ? 2 : (max(398680379, ((var_0 & (arr_3 [i_2] [i_1] [i_0])))))));
                }
            }
        }
    }
    var_18 = (min(var_18, (var_1 && var_11)));
    #pragma endscop
}
