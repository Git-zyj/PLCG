/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179425
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179425 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179425
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(((((var_10 ? 1 : 3205883814)))), ((var_2 ? var_3 : var_10))));
    var_17 = -var_15;
    var_18 = (~var_13);
    var_19 = (min(var_19, 1));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_20 = (((arr_3 [i_0 + 1]) < var_9));

                for (int i_2 = 0; i_2 < 19;i_2 += 1) /* same iter space */
                {
                    var_21 += 1;
                    var_22 -= (((((1 ? (arr_4 [i_2]) : ((~(arr_3 [1])))))) ? (((~((1 >> (var_10 + 7416)))))) : (arr_2 [i_2] [i_1])));
                }
                /* vectorizable */
                for (int i_3 = 0; i_3 < 19;i_3 += 1) /* same iter space */
                {
                    /* LoopNest 2 */
                    for (int i_4 = 0; i_4 < 1;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 19;i_5 += 1)
                        {
                            {
                                arr_15 [i_1] [i_1] = (arr_3 [i_0 + 1]);
                                var_23 = ((1 | (arr_3 [i_0 + 1])));
                                var_24 = (((arr_8 [i_5]) < (!-25206)));
                                arr_16 [i_1] [i_1] = 1;
                                arr_17 [13] [i_3] [i_3] [i_1] [i_3] [i_3] [i_3] = (((~(arr_10 [i_1]))));
                            }
                        }
                    }
                    var_25 = (arr_10 [i_1]);
                }
            }
        }
    }
    #pragma endscop
}
