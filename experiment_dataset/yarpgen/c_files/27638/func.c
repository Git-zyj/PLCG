/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27638
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27638 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27638
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned char i_0 = 0; i_0 < 18; i_0 += 3) 
    {
        for (unsigned short i_1 = 0; i_1 < 18; i_1 += 3) 
        {
            {
                var_18 = ((/* implicit */short) min((var_18), (((/* implicit */short) ((((/* implicit */_Bool) (short)15354)) && (((/* implicit */_Bool) (short)-15355)))))));
                var_19 = ((/* implicit */unsigned char) (-((~((-2147483647 - 1))))));
            }
        } 
    } 
    var_20 = ((/* implicit */short) ((min((((((/* implicit */int) var_9)) ^ (((/* implicit */int) (_Bool)1)))), (((/* implicit */int) (signed char)64)))) - (((/* implicit */int) var_0))));
}
