/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242282
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242282 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242282
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 |= var_7;
    var_12 = var_7;
    var_13 *= ((((((-915293794317849218 ? 4294967295 : var_5)) < (min(115, 2527026027)))) ? var_7 : (min(915293794317849217, 915293794317849218))));

    for (int i_0 = 1; i_0 < 7;i_0 += 1)
    {
        var_14 *= ((((~((((arr_1 [i_0]) < 0)))))) >= -915293794317849199);
        var_15 = 2008044618;
        var_16 = (max((((arr_1 [i_0]) ? (((-(arr_1 [i_0])))) : -var_10)), (min(915293794317849216, 1))));
    }
    #pragma endscop
}
