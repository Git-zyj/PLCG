/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 186308
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=186308 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/186308
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */unsigned int) var_11);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16; i_0 += 1) 
    {
        for (int i_1 = 0; i_1 < 16; i_1 += 4) 
        {
            for (signed char i_2 = 0; i_2 < 16; i_2 += 4) 
            {
                {
                    var_20 = (unsigned char)4;
                    var_21 = ((/* implicit */int) (-9223372036854775807LL - 1LL));
                }
            } 
        } 
    } 
    var_22 &= ((/* implicit */int) var_7);
    /* LoopNest 2 */
    for (unsigned char i_3 = 0; i_3 < 21; i_3 += 2) 
    {
        for (int i_4 = 2; i_4 < 20; i_4 += 4) 
        {
            {
                var_23 = ((/* implicit */int) (signed char)-64);
                var_24 = ((/* implicit */unsigned int) -1402384213);
                var_25 = ((/* implicit */unsigned int) 1402384229);
                var_26 = ((/* implicit */signed char) 1402384206);
            }
        } 
    } 
}
