/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211588
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211588 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211588
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 *= var_0;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 24;i_1 += 1)
        {
            {
                var_11 = ((((var_4 >= (((arr_2 [i_0] [i_1]) ? var_2 : var_9)))) ? (((((max(var_7, var_6))) ? ((var_0 ? (arr_3 [i_0] [i_0] [i_1]) : (arr_0 [i_0] [7]))) : var_3))) : (max(-8848112242678290380, 224))));
                var_12 *= -77;
                arr_5 [i_1] [i_1] [i_0] = ((((((arr_0 [i_1] [i_0]) ? (!var_5) : ((((-9223372036854775807 - 1) || var_6)))))) <= ((-35 ? 3049066844 : var_3))));
            }
        }
    }
    #pragma endscop
}
