/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 141053
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=141053 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/141053
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        var_13 = ((1 ? (arr_1 [i_0] [i_0]) : (((((arr_0 [i_0]) <= (arr_0 [i_0])))))));
        arr_2 [i_0] [i_0] = ((~((0 ? 281474976710655 : 1))));
        /* LoopNest 2 */
        for (int i_1 = 3; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    var_14 -= (((((18446462598732840956 + (102 - -1)))) ? (((18446462598732840961 ? (((arr_6 [i_1]) / 63)) : ((-(arr_4 [i_0] [i_0] [i_2])))))) : ((8495 + (max((arr_3 [i_0] [i_1 + 1] [i_0]), (arr_5 [i_2] [i_1] [i_1 - 2] [i_0])))))));
                    var_15 += (54 + 1);
                    var_16 = (max(var_16, 253));
                }
            }
        }
    }
    var_17 = 0;
    var_18 = var_9;
    #pragma endscop
}
