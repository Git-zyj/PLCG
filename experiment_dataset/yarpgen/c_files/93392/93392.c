/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93392
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93392 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93392
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 &= var_0;
    var_13 = -193514817;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] = var_4;
                    arr_9 [i_1] [i_1] [i_2] = (((max(((65536 ? (arr_4 [i_0] [i_1]) : (arr_7 [i_0] [i_0] [i_2] [5]))), var_8)) != ((max((var_10 & 1428495544), 65532)))));
                    var_14 = (min(var_14, ((max(var_10, (min(2191060563, (arr_7 [i_0] [i_1] [i_1] [i_0]))))))));
                }
            }
        }
    }
    var_15 = ((((min(var_7, 1287664838)))) ? var_11 : (~3007302483));
    #pragma endscop
}
