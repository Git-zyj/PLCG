/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38681
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38681 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38681
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 20;i_3 += 1)
                    {
                        {
                            var_15 *= (min(var_12, (arr_3 [i_1 + 1] [i_1])));
                            arr_9 [i_0] [i_0] [i_0] [i_0] = var_1;
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_4 = 2; i_4 < 22;i_4 += 1)
                {
                    for (int i_5 = 3; i_5 < 22;i_5 += 1)
                    {
                        {
                            arr_15 [i_4] [i_1] [i_1] [i_5] = (min(((((3977791033 ? var_6 : var_8)) - (((0 >> (((arr_8 [i_5] [i_1] [i_4 - 2]) - 633861377901181158))))))), (arr_14 [i_0] [i_1 + 2] [i_1] [i_4 - 2] [i_5])));
                            arr_16 [i_5] = ((min(1, 3977791033)));
                        }
                    }
                }
            }
        }
    }
    var_16 = (max(var_16, ((max(var_5, var_3)))));
    var_17 = ((~((~(~1280660005)))));
    #pragma endscop
}
