/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155566
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155566 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155566
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */short) max((((/* implicit */unsigned int) ((int) 2675691781U))), (((((/* implicit */_Bool) (~(var_2)))) ? (2005339754U) : ((~(1923005321U)))))));
    /* LoopNest 3 */
    for (short i_0 = 2; i_0 < 21; i_0 += 4) 
    {
        for (long long int i_1 = 0; i_1 < 24; i_1 += 4) 
        {
            for (short i_2 = 1; i_2 < 22; i_2 += 1) 
            {
                {
                    arr_8 [i_2] [i_2] [i_2] = ((/* implicit */short) 2371961975U);
                    var_15 = ((/* implicit */long long int) var_10);
                    arr_9 [i_2] [i_2] [i_2] = ((/* implicit */long long int) (short)11699);
                    arr_10 [i_2] = ((/* implicit */int) max((((/* implicit */unsigned int) var_10)), (1923005321U)));
                }
            } 
        } 
    } 
}
