/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206127
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206127 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206127
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */int) 25U);
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 23; i_0 += 4) 
    {
        var_20 = ((/* implicit */unsigned short) 4294967270U);
        arr_2 [i_0] [i_0] = ((/* implicit */unsigned char) ((_Bool) var_4));
        var_21 = ((/* implicit */unsigned int) max((var_21), (((unsigned int) (signed char)64))));
    }
}
