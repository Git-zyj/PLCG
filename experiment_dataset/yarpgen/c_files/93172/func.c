/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93172
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93172 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93172
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 2; i_0 < 13; i_0 += 2) 
    {
        arr_3 [i_0] = ((int) var_6);
        arr_4 [i_0] = ((/* implicit */unsigned long long int) ((unsigned char) ((unsigned short) -1)));
    }
    var_15 = ((/* implicit */unsigned short) max((var_15), (((/* implicit */unsigned short) ((((/* implicit */unsigned long long int) -2)) * (3ULL))))));
}
