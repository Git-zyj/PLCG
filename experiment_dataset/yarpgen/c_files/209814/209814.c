/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209814
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209814 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209814
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_1;
    var_11 = (min(var_3, var_1));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_1] = (((((((min((arr_4 [i_0]), var_9))) ^ 0))) ? (min((min(1, (arr_3 [i_0]))), (arr_0 [i_0]))) : (((min(((min(0, -5154))), 2584))))));
                arr_6 [i_0] [i_0] [i_1] = (max((arr_1 [i_0]), ((((((arr_3 [i_0]) & -1719211168))) ? var_4 : (arr_2 [i_0])))));
            }
        }
    }
    #pragma endscop
}
