/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224385
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224385 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224385
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_0] [i_2] [i_1] = (arr_0 [i_0]);
                    var_17 += var_0;
                    var_18 = (min(9, -7338416459443019279));
                }
            }
        }
    }
    var_19 = (var_5 || var_2);
    var_20 = var_6;
    var_21 = ((var_5 ? (((((4294967292 ? var_11 : var_16))) ? (var_16 & var_14) : (((var_4 ? var_4 : var_16))))) : (max((((var_7 ? var_12 : var_13))), ((var_14 ? 8 : 0))))));
    #pragma endscop
}
