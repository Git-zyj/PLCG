/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20478
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20478 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20478
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (!((((!var_5) + 6431158885954876374))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_17 ^= ((((((arr_7 [i_0] [8] [i_2]) + (arr_7 [i_0] [i_1] [i_0])))) ? (((arr_7 [i_0] [i_1] [i_1]) ? (arr_7 [i_0] [i_1] [i_1]) : -480143299)) : ((7952442316288464102 ? (arr_7 [i_0] [i_1] [i_2]) : (arr_7 [i_0] [i_1] [i_2])))));
                    var_18 = ((~((-(arr_1 [i_2])))));
                    var_19 = 8659;
                }
            }
        }
    }
    #pragma endscop
}
