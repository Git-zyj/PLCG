/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72609
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72609 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72609
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 &= ((1 << (((!(2147483638 < 2147483638))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_1] = (arr_1 [i_0]);
                var_13 = (max((((arr_1 [4]) && -2147483644)), ((5052 < (arr_3 [0] [i_1] [i_1])))));
            }
        }
    }
    var_14 = (-2147483643 ? (max(((max(913561127, 1))), var_11)) : (((((max(var_6, 6))) * ((1 ? var_0 : 2147483646))))));
    #pragma endscop
}
