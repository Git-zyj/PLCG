/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88134
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88134 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88134
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 += var_3;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_20 ^= ((((((((arr_4 [1] [i_1]) != (arr_5 [i_0])))) - ((5119 ? 1 : var_18))))) ? (arr_3 [i_1]) : ((61185 ? ((min(var_12, var_14))) : (arr_7 [i_0 - 1] [i_1] [i_1]))));
                    arr_9 [i_0] [i_1] [i_2] = (min(((max(0, (((arr_0 [i_0]) >= (arr_7 [i_0] [i_1] [i_2])))))), (arr_0 [i_0 - 1])));
                }
            }
        }
    }
    var_21 = (~1);
    #pragma endscop
}
