/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234854
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234854 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234854
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (int i_0 = 1; i_0 < 21; i_0 += 3) 
    {
        arr_3 [i_0] [i_0 + 1] |= ((/* implicit */short) var_8);
        var_19 = ((/* implicit */unsigned long long int) arr_0 [i_0 + 1]);
        var_20 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) (short)-19967))));
        var_21 = ((/* implicit */unsigned int) (short)19979);
    }
    var_22 = ((/* implicit */unsigned char) (short)19980);
}
