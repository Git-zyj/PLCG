/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 192397
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=192397 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/192397
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_14 = (max(var_14, ((~(arr_0 [i_0])))));
        /* LoopNest 2 */
        for (int i_1 = 4; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 22;i_2 += 1)
            {
                {
                    var_15 = ((-1459732302 ? (((arr_2 [i_0] [i_0]) ? (-2147483647 - 1) : var_11)) : ((var_6 << (var_12 - 12758223329759546024)))));
                    var_16 = -28407;
                }
            }
        }
        var_17 |= -2147483641;
    }
    var_18 = var_7;
    var_19 = (((((~(max(28406, var_3))))) ? (((-((var_11 << (var_2 + 89)))))) : ((var_8 ? var_6 : 3224525786087555891))));
    #pragma endscop
}
