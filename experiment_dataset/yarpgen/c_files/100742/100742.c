/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100742
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100742 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100742
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        var_17 ^= (!28669);
        var_18 ^= (max((arr_1 [i_0]), -4869573654935945666));

        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            var_19 = (!807913362);
            arr_4 [i_0] [5] [10] = ((-(min(-4869573654935945666, 67))));
        }
        /* LoopNest 3 */
        for (int i_2 = 0; i_2 < 15;i_2 += 1)
        {
            for (int i_3 = 1; i_3 < 13;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 15;i_4 += 1)
                {
                    {
                        var_20 = 804529969;
                        var_21 = (max(var_21, 612468070));
                        var_22 = arr_9 [i_0] [i_2] [2] [i_4];
                    }
                }
            }
        }
    }
    var_23 = (((((-(var_8 > var_10)))) ? (var_11 > var_0) : 438381894));
    #pragma endscop
}
