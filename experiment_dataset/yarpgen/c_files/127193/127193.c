/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127193
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127193 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127193
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (((max(var_4, (var_4 * var_1))) % var_11));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_13 = (min((arr_1 [i_0]), ((max(2147483645, (max(255, (arr_6 [i_1] [i_1]))))))));
                arr_7 [i_0] &= (min((min(var_9, -22389)), ((max(203, 65535)))));
                var_14 = ((!(arr_2 [i_0])));
                arr_8 [1] [i_1] = ((!var_7) >> ((((max((arr_1 [i_1]), 8))) - 71)));
            }
        }
    }
    var_15 = var_11;
    var_16 = (!18446744073709551599);
    #pragma endscop
}
