/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83008
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83008 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83008
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_13 = (min(var_13, (3220283095 != -507430144)));
                var_14 = (min(((var_11 ? (arr_3 [i_0 + 2]) : (arr_3 [i_0 + 2]))), (arr_3 [6])));
                var_15 -= (((max(((((arr_1 [i_1]) ? (arr_3 [i_0]) : (arr_0 [i_1])))), ((var_11 ? 0 : (arr_1 [i_0]))))) & (0 ^ var_2)));
            }
        }
    }
    var_16 = -18997;
    var_17 = (max(-1074684200, (((((var_0 ? var_6 : var_8)) <= (var_0 == var_10))))));
    #pragma endscop
}
