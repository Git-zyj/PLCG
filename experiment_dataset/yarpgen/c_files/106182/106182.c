/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106182
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106182 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106182
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (((((var_8 ? var_9 : (!6091)))) * ((var_5 ? 965041635 : var_1))));
    var_13 = (min(var_13, ((min(((((((var_11 ? 28 : 3606820737))) ? (~var_8) : var_4))), -5013401771551508907)))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 24;i_0 += 1) /* same iter space */
    {
        arr_2 [5] [23] = (-var_5 == var_0);
        arr_3 [i_0] = -53;
        var_14 ^= ((((((arr_0 [i_0]) == var_3))) != (arr_0 [i_0])));
        arr_4 [i_0] = (((arr_0 [i_0]) ? (arr_1 [i_0]) : (arr_1 [i_0])));
        var_15 = (min(var_15, -28));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 24;i_1 += 1) /* same iter space */
    {
        var_16 = ((!(arr_0 [i_1])));
        arr_7 [i_1] = ((!(arr_6 [i_1])));
    }
    #pragma endscop
}
