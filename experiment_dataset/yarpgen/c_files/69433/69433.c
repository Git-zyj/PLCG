/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 69433
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=69433 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/69433
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [i_0] [21] [i_1] = 2055;
                arr_6 [4] [4] |= (max(((((min(0, (arr_0 [i_0])))) ? (arr_1 [i_0 + 1]) : 18446744073709551609)), ((((arr_0 [i_0 - 1]) && (arr_0 [i_0 - 1]))))));
                var_11 = (min((arr_2 [i_0 - 2]), (min((arr_2 [i_0 - 2]), (arr_2 [i_0 + 1])))));
                var_12 ^= -var_4;
            }
        }
    }
    var_13 = ((var_1 > (!var_8)));
    #pragma endscop
}
