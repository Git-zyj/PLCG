/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 53737
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=53737 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/53737
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 -= ((/* implicit */signed char) (-(((/* implicit */int) var_1))));
    var_11 = ((/* implicit */signed char) ((((/* implicit */_Bool) max((((((/* implicit */int) var_2)) + (var_4))), (((/* implicit */int) var_3))))) ? ((-(var_5))) : (max((((/* implicit */unsigned long long int) var_7)), (var_5)))));
    /* LoopNest 2 */
    for (unsigned short i_0 = 0; i_0 < 14; i_0 += 3) 
    {
        for (long long int i_1 = 4; i_1 < 11; i_1 += 3) 
        {
            {
                arr_5 [i_0] [i_0] = ((/* implicit */_Bool) (~((~(((/* implicit */int) (unsigned char)26))))));
                var_12 = (!(((/* implicit */_Bool) (+(((arr_4 [i_0]) * (arr_4 [i_0])))))));
            }
        } 
    } 
}
