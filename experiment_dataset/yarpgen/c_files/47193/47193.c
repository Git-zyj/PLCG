/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47193
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47193 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47193
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (!var_14);
    var_20 = (min(1, 95));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                arr_5 [i_1] &= (((((~(arr_3 [i_1]))) ^ (arr_2 [i_0] [21] [i_0]))));
                var_21 ^= (!var_10);
                var_22 ^= (((((min(((max((arr_3 [i_0]), 0))), var_5)))) + var_17));
                var_23 *= ((-(((arr_3 [1]) ? (arr_3 [i_0]) : (arr_4 [i_1] [i_0])))));
            }
        }
    }
    var_24 = (((((var_3 ? (max(var_13, var_5)) : var_13))) ? var_16 : (min(var_2, var_6))));
    #pragma endscop
}
