/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 65958
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=65958 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/65958
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_5 [i_0] = (((arr_3 [i_0]) / (((((var_1 ? (arr_0 [i_1]) : 63040))) ? var_17 : var_9))));
                arr_6 [i_0] = (max(((((arr_4 [i_0] [i_1] [i_0]) && var_6))), (max(var_6, ((-(arr_0 [i_0])))))));
                arr_7 [i_0] = (((!217) <= (((arr_1 [i_0] [i_0]) + 3695548721193283584))));
            }
        }
    }
    var_18 = var_13;
    var_19 = (min(var_19, var_17));
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 10;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 10;i_3 += 1)
        {
            {
                var_20 = (~((-(arr_11 [i_3] [i_3] [4]))));
                var_21 = (((~((~(arr_2 [i_3]))))));
                var_22 = (min(var_22, var_11));
            }
        }
    }
    #pragma endscop
}
