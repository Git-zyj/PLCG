/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38281
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38281 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38281
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    arr_8 [i_0] [i_1 + 1] [i_2] = ((+(((arr_4 [i_0] [i_0] [i_0]) ^ (~var_9)))));
                    var_17 *= (min((((arr_2 [i_1 + 1] [i_1 + 1]) ? (arr_2 [i_1 + 1] [i_1 + 1]) : var_6)), (arr_2 [i_1 + 1] [i_1 + 1])));
                }

                for (int i_3 = 0; i_3 < 21;i_3 += 1)
                {
                    arr_11 [i_0] [i_1] = (arr_3 [i_0]);
                    /* LoopNest 2 */
                    for (int i_4 = 0; i_4 < 21;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 21;i_5 += 1)
                        {
                            {
                                arr_17 [i_0] [i_1] [i_4] [i_5] = (((((-((min(var_11, 50)))))) ? (var_3 * 50) : (min(var_5, ((3985284873275788328 ? var_12 : var_7))))));
                                var_18 *= -54;
                            }
                        }
                    }
                }
            }
        }
    }
    var_19 = var_15;
    #pragma endscop
}
