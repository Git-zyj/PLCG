/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 243528
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=243528 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/243528
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned short i_0 = 3; i_0 < 15; i_0 += 4) 
    {
        for (short i_1 = 1; i_1 < 16; i_1 += 3) 
        {
            {
                var_19 *= max((max((((/* implicit */unsigned short) ((((/* implicit */_Bool) -9085102068040714218LL)) || (((/* implicit */_Bool) 9085102068040714218LL))))), (((unsigned short) arr_1 [i_0 - 1])))), (((/* implicit */unsigned short) arr_0 [i_1])));
                var_20 *= ((long long int) ((((/* implicit */int) arr_1 [i_0])) % (((/* implicit */int) arr_1 [i_0]))));
            }
        } 
    } 
    var_21 = var_3;
}
