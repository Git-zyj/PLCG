/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94327
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94327 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94327
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = ((((((((arr_0 [i_0]) <= 32))) - (arr_1 [i_0]))) - ((837493465 ? var_4 : (arr_0 [i_0])))));
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 4; i_3 < 9;i_3 += 1)
                    {
                        {
                            arr_12 [i_3 - 3] [i_2] [i_2] [i_1] [i_0] = 0;
                            var_19 = (((1 && 3457473827) ? 837493467 : (((127 ? (arr_7 [i_0] [i_2] [i_2 - 1] [i_0]) : var_9)))));
                            var_20 = (((((-(arr_7 [i_0] [i_2] [i_0] [i_3])))) && (~var_12)));
                        }
                    }
                }
            }
        }
    }
    var_21 = 3457473821;
    #pragma endscop
}
