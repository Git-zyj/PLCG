/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27039
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27039 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27039
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned short) var_14);
    /* LoopNest 2 */
    for (unsigned short i_0 = 1; i_0 < 11; i_0 += 3) 
    {
        for (long long int i_1 = 0; i_1 < 12; i_1 += 3) 
        {
            {
                var_17 = ((max((((/* implicit */unsigned long long int) (unsigned char)155)), (1152921487426977792ULL))) << (((((/* implicit */_Bool) var_8)) ? (((/* implicit */unsigned int) ((/* implicit */int) var_13))) : (var_14))));
                var_18 = min((((/* implicit */long long int) var_6)), (min((var_3), (((/* implicit */long long int) var_10)))));
            }
        } 
    } 
}
