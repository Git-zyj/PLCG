/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196953
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196953 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196953
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_14 = (((arr_2 [i_0] [1]) | (((((min(var_4, var_8)))) & (((arr_0 [i_0]) & 4576626732128752535))))));
        arr_3 [i_0] = ((!((((arr_0 [i_0]) ? (arr_0 [i_0]) : (-32767 - 1))))));
        arr_4 [10] [i_0] = ((var_13 / var_7) != (((!(arr_2 [i_0] [i_0])))));
    }
    var_15 = ((-32767 - 1) ? ((-32745 / (-32767 - 1))) : -23610);
    #pragma endscop
}
