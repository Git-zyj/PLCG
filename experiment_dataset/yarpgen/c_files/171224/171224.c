/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171224
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171224 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171224
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    var_20 = ((+(((arr_4 [i_0 + 2] [i_0 - 1] [i_0 + 2]) << (((arr_5 [i_0 + 2] [i_0 + 1] [i_0 + 2] [i_2]) - 7122647979741304039))))));
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_21 = (min(var_21, (~var_0)));
                                arr_11 [i_0] [12] [i_2] [i_0] [i_2] [6] [i_1] = ((7123923329173321880 + (((((110 - (arr_1 [i_0]))) - (((arr_10 [i_0] [i_1] [8] [i_2] [i_4]) + 0)))))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_22 = var_1;
    #pragma endscop
}
