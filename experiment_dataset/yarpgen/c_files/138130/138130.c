/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138130
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138130 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138130
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min((var_9 + 192), ((min(var_8, var_0)))));
    var_12 -= 1;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_13 = (arr_2 [i_0]);
        arr_4 [i_0] = ((((8576507489304841429 ? 211 : 752314629182701748)) - ((((arr_1 [i_0] [i_0]) - -14401)))));
        arr_5 [i_0] = (((((-32767 - 1) || 1))) - (arr_3 [i_0]));
        var_14 = (max(var_14, ((((arr_1 [i_0] [2]) << (arr_1 [i_0] [i_0]))))));
    }
    var_15 = (~65535);
    #pragma endscop
}
