/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20905
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20905 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20905
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((var_4 ? var_6 : var_8)) & ((min((var_0 == var_9), (-32767 - 1))))));

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {

        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            var_12 *= (((arr_3 [i_1 - 1] [i_0]) >> ((((min((arr_3 [i_1 + 2] [i_0]), (arr_3 [i_1 + 2] [i_0])))) - 45))));
            var_13 *= (arr_2 [4] [i_1 + 2] [i_1 + 1]);
        }
        arr_5 [5] = ((((var_3 >= (arr_4 [i_0] [i_0] [i_0]))) || (((((arr_4 [2] [12] [i_0]) < (arr_1 [1])))))));
    }
    #pragma endscop
}
