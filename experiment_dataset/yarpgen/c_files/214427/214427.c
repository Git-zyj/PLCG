/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214427
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214427 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214427
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 ^= ((var_1 ? (4860143452906221094 | var_5) : 71));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1 + 1] |= (((!(((1 ? 1 : (arr_2 [i_1] [i_1 + 3])))))));
                var_14 ^= ((((((arr_1 [i_1 + 3]) % (arr_1 [i_1 + 3])))) ? 2498450976 : (var_2 + -var_7)));
                arr_5 [i_1 + 3] [i_0] |= var_8;
                var_15 ^= 92;
            }
        }
    }
    var_16 |= var_6;
    var_17 += (!var_5);
    #pragma endscop
}
