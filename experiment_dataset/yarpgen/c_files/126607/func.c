/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 126607
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=126607 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/126607
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_14 -= ((/* implicit */_Bool) max(((~(((long long int) var_13)))), (((/* implicit */long long int) max(((+(((/* implicit */int) (short)-23183)))), ((-(((/* implicit */int) var_1)))))))));
        arr_3 [i_0] = ((/* implicit */int) (short)0);
    }
    /* vectorizable */
    for (unsigned int i_1 = 1; i_1 < 10; i_1 += 2) 
    {
        var_15 &= ((/* implicit */long long int) var_11);
        arr_6 [i_1] [(unsigned char)3] = ((/* implicit */signed char) (~(var_7)));
    }
    var_16 = ((/* implicit */short) ((long long int) var_9));
}
