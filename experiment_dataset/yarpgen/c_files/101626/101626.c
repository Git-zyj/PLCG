/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 101626
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=101626 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/101626
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((((max(var_0, var_6))) ? 461053319431174992 : var_8)));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        var_12 = (((arr_0 [i_0]) ? 461053319431174992 : (arr_0 [i_0])));

        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            arr_4 [i_0] [i_0] [i_1 - 1] = 461053319431174992;
            var_13 = 17985690754278376623;
            var_14 = ((!(!1)));
        }
        /* LoopNest 3 */
        for (int i_2 = 2; i_2 < 18;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 20;i_3 += 1)
            {
                for (int i_4 = 4; i_4 < 18;i_4 += 1)
                {
                    {
                        var_15 = (arr_2 [i_0] [i_0]);
                        var_16 = (arr_2 [i_2 - 2] [i_2 - 1]);
                    }
                }
            }
        }
        var_17 = (((arr_10 [i_0] [i_0]) + (arr_14 [i_0] [i_0] [i_0])));
        var_18 = (min(var_18, (((255 ? ((~(arr_9 [13]))) : -1529121539)))));
    }
    #pragma endscop
}
