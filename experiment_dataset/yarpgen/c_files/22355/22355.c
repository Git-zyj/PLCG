/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22355
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22355 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22355
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 8;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 9;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 9;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 10;i_4 += 1)
                        {
                            {
                                arr_18 [i_0] [i_0] [i_0] [i_4] [i_3] [i_2 + 1] = var_2;
                                var_15 = (~var_4);
                                arr_19 [i_0] [i_2] [i_0] [1] = var_0;
                                arr_20 [i_4] [i_4] [i_1 + 2] [i_4] [i_0] [i_0] = var_12;
                                var_16 = ((!(~var_14)));
                            }
                        }
                    }

                    /* vectorizable */
                    for (int i_5 = 1; i_5 < 9;i_5 += 1) /* same iter space */
                    {
                        var_17 = ((!(!var_3)));
                        arr_25 [i_5] [i_0] [i_0] [i_0] = -var_0;
                        var_18 = var_0;
                    }
                    for (int i_6 = 1; i_6 < 9;i_6 += 1) /* same iter space */
                    {
                        arr_28 [i_1] [i_0] [i_0] [i_0] = var_13;
                        arr_29 [i_0] = (~var_10);
                    }
                    var_19 = (min(var_19, -var_10));
                    var_20 = var_3;
                }
            }
        }
    }
    var_21 = (!var_5);
    #pragma endscop
}
