/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 12264
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=12264 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/12264
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        arr_2 [i_0] [11] = (((((arr_1 [i_0] [i_0]) + (var_4 + var_9))) > (((max(var_12, var_8))))));
        var_15 &= var_11;
        var_16 = var_5;
    }
    var_17 = (min(var_17, ((((((((var_5 ? var_10 : var_4)) ^ (var_6 > var_3)))) ? (((min(var_10, (var_13 > var_9))))) : ((-var_3 ? var_5 : (max(var_4, var_5)))))))));
    #pragma endscop
}
