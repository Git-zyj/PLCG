/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236984
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236984 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236984
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        var_16 = (max(var_16, ((((arr_1 [i_0]) ? ((-((var_3 % (arr_0 [13]))))) : (((((min((arr_0 [i_0]), -28))) && 214))))))));
        var_17 = ((~(min((arr_1 [i_0]), 11324555623314621448))));
        var_18 = (arr_1 [i_0]);
        arr_2 [i_0] |= -30083;
        arr_3 [i_0] = (((min(((var_11 ? 11470 : var_8)), ((min(var_12, (arr_1 [i_0])))))) >> ((((~(((arr_0 [i_0]) / var_13)))) + 64))));
    }
    var_19 = 7122188450394930168;
    #pragma endscop
}
