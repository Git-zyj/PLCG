/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 25850
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=25850 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/25850
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 17; i_0 += 3) 
    {
        var_18 *= ((/* implicit */int) (!((!(((/* implicit */_Bool) arr_1 [i_0]))))));
        arr_3 [i_0] = ((/* implicit */signed char) (~(((/* implicit */int) ((short) (!(((/* implicit */_Bool) (signed char)64))))))));
    }
    var_19 = ((/* implicit */_Bool) (-(max(((~(488820806824318880LL))), (((/* implicit */long long int) var_17))))));
    var_20 = ((/* implicit */unsigned long long int) ((unsigned int) var_11));
}
