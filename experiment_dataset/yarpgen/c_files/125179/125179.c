/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125179
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125179 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125179
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_3;
    var_12 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 12;i_1 += 1)
        {
            {
                var_13 = ((((((arr_2 [i_0 + 1] [i_1 + 1]) ^ ((((arr_0 [i_0 + 2]) + (arr_0 [i_0 + 1]))))))) ? ((((((arr_4 [i_0] [i_1]) * (arr_1 [i_0] [i_0])))) ? (arr_4 [i_1 - 1] [i_0 + 3]) : (((max((arr_3 [i_1 - 2] [i_1 - 2] [i_1 - 2]), (arr_3 [i_0] [i_1] [i_0 + 3]))))))) : ((((arr_1 [i_0] [i_0]) ? (arr_1 [i_0 - 1] [i_0]) : (((((arr_4 [i_0] [i_0]) && (arr_0 [i_1 - 2]))))))))));
                var_14 &= (min(1, 18446744073709551615));
            }
        }
    }
    #pragma endscop
}
