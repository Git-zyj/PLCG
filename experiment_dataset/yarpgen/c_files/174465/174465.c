/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174465
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174465 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174465
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    arr_7 [i_0] [11] [i_1] [i_2] = (max(((min(109, var_11))), (arr_0 [i_0] [i_0])));
                    var_19 = ((-(152 | var_14)));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                var_20 = ((+(((arr_11 [i_4] [i_3]) << (-3639333699700614951 + 3639333699700614972)))));
                                var_21 = ((max(883415867, 114)));
                            }
                        }
                    }
                    arr_12 [i_0] [i_0] [i_1] [i_2] = (!var_14);
                }
            }
        }
    }
    var_22 = (var_16 - var_10);
    #pragma endscop
}
