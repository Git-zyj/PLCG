/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103635
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103635 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103635
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_12, 223));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_17 = (((((878504673 ? var_7 : 87))) >> (32927 - 32914)));
                var_18 = ((~(arr_1 [i_0])));
                arr_7 [i_0] = var_10;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 11;i_3 += 1)
                    {
                        {
                            var_19 = ((!(((max(var_13, var_6))))));
                            arr_13 [i_2] = (((((-87 != ((-(arr_10 [i_3 + 1])))))) <= (!var_15)));
                        }
                    }
                }
                var_20 = (65509 ? (((+(((arr_0 [i_0]) ? var_7 : (arr_1 [i_0])))))) : (((arr_2 [i_0]) / (arr_1 [i_1]))));
            }
        }
    }
    var_21 = (min(var_21, ((min(((!(!var_1))), ((max(var_5, -88))))))));
    #pragma endscop
}
