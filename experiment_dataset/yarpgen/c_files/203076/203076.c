/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203076
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203076 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203076
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((21219 ? -21219 : -21219));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_15 += (((arr_3 [i_1]) ? ((~(arr_3 [i_0 + 3]))) : (((!21219) & ((-(arr_6 [17] [16])))))));
                arr_7 [4] |= 260088018;
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 18;i_4 += 1)
                        {
                            {
                                var_16 *= (min(var_11, ((~((var_8 ? (arr_15 [i_0] [i_0] [1] [i_0 + 3] [6]) : (arr_0 [1])))))));
                                var_17 = (max(var_17, ((((~((-(arr_6 [i_0] [3])))))))));
                                var_18 = (min(-21219, 0));
                                var_19 = ((!((max(-21219, 485)))));
                                var_20 = (min(var_20, var_4));
                            }
                        }
                    }
                }
            }
        }
    }
    var_21 += var_7;
    var_22 += ((+(((var_3 == var_5) ? (var_1 != var_5) : (var_9 ^ var_2)))));
    #pragma endscop
}
