/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 414
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=414 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/414
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */signed char) var_6);
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 18; i_0 += 1) 
    {
        for (short i_1 = 3; i_1 < 15; i_1 += 4) 
        {
            {
                var_20 = ((/* implicit */signed char) (+(((/* implicit */int) (unsigned short)12128))));
                var_21 = ((/* implicit */unsigned short) var_9);
                var_22 -= ((/* implicit */signed char) arr_0 [i_0] [i_0]);
                var_23 *= ((/* implicit */signed char) ((((/* implicit */int) var_3)) >> (((/* implicit */int) min(((unsigned short)65527), ((unsigned short)8))))));
            }
        } 
    } 
    var_24 = ((/* implicit */unsigned short) var_15);
    var_25 = var_16;
}
