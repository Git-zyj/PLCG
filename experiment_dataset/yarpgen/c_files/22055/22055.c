/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22055
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22055 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22055
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        var_15 = var_10;
        var_16 = ((((arr_2 [i_0] [i_0 + 1]) ? var_3 : 79)));
        var_17 = (max(var_8, ((((max(70876096, 2684773759))) ? var_3 : ((-125 ? -87 : (arr_2 [9] [i_0])))))));
        arr_3 [i_0] = (min((((29 < 68719476735) ? 68719476735 : (((1 ? 2764961264 : (arr_0 [i_0 + 1])))))), -1));
    }
    var_18 &= 18446744004990074881;
    #pragma endscop
}
