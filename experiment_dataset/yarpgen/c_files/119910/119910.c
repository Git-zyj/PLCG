/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119910
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119910 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119910
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 ^= (((~var_7) <= (~236)));

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        var_19 -= 98;
        arr_3 [14] = ((+(((arr_1 [i_0]) ? (-98 - -834863285072452546) : (!9776)))));
        arr_4 [i_0] [i_0] = (arr_2 [i_0]);
        arr_5 [20] [i_0] = ((((((((arr_1 [i_0]) < -114)) ? ((-3 ? 166 : (-127 - 1))) : (~19518)))) ? ((min((arr_2 [i_0]), (arr_2 [18])))) : (-24 < 95)));
    }
    var_20 = ((min(var_11, ((var_9 ? var_12 : 0)))) >= ((((((min(45501, 52538))) >= var_4)))));
    #pragma endscop
}
