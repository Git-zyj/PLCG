/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82959
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82959 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82959
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 13;i_3 += 1)
                    {
                        {
                            var_19 |= arr_1 [1];
                            arr_8 [i_0] [i_1] [13] [i_3] [i_1] = ((((((~(arr_4 [i_1] [i_1] [i_2] [i_3 + 2]))))) ^ (((!((max(0, 16943339929534426857))))))));
                        }
                    }
                }
                var_20 = (max(var_20, (((max(209, ((-112 ? 46 : 15))))))));
                arr_9 [i_0] [i_0] [i_1] = (max(((max(20, 209))), (~4336005622360082065)));
            }
        }
    }
    var_21 = 145;
    var_22 = ((((((((max(113, 12)))) ^ (113 - 1793845715)))) ? var_16 : -92));
    #pragma endscop
}
