/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131005
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131005 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131005
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */short) var_10);
    var_18 = ((/* implicit */_Bool) (~(5557608560291387047ULL)));
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 10; i_0 += 4) 
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 10; i_1 += 1) 
        {
            for (long long int i_2 = 0; i_2 < 10; i_2 += 1) 
            {
                for (unsigned int i_3 = 0; i_3 < 10; i_3 += 2) 
                {
                    {
                        var_19 -= ((/* implicit */unsigned long long int) (~(231509900512749258LL)));
                        var_20 = ((/* implicit */short) arr_4 [i_1] [i_1] [i_1]);
                    }
                } 
            } 
        } 
        arr_8 [i_0] = ((/* implicit */short) (((+(((unsigned long long int) (_Bool)1)))) - (((/* implicit */unsigned long long int) ((unsigned int) var_6)))));
    }
}
