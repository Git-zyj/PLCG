/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72953
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72953 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72953
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    var_14 = (max(var_0, (max((((arr_4 [8]) << (((((-9223372036854775807 - 1) - -9223372036854775775)) + 43)))), var_0))));
                    arr_8 [i_2] [8] [16] [12] |= ((((min((arr_4 [i_0 + 2]), (max(var_9, (arr_7 [i_0] [i_2] [0])))))) ? ((min(9223372036854775789, (-9223372036854775807 - 1)))) : (arr_4 [i_1])));
                    var_15 = -9223372036854775807;
                }
            }
        }
    }
    var_16 = (min(var_16, var_4));
    #pragma endscop
}
