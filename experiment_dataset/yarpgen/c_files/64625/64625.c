/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64625
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64625 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64625
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((var_10 ? var_16 : var_18));

    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        arr_4 [i_0] = (((arr_1 [i_0] [i_0]) != (min((arr_3 [i_0 - 1]), (arr_3 [i_0 + 1])))));
        var_21 = ((~(((266389219 || 2868) ? (!-1462878494) : (((arr_1 [i_0 - 1] [15]) / var_9))))));
        var_22 = ((min(((var_16 ^ (arr_0 [i_0] [i_0]))), -671714509063056732)) | -1462878481);
    }
    #pragma endscop
}
