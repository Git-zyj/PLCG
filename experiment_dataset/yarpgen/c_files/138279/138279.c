/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138279
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138279 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138279
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_7 [i_1] [i_0] [i_0] = ((((min((arr_6 [i_1]), (var_7 ^ var_13)))) ? (min((arr_2 [i_1] [i_1]), (var_1 + var_14))) : ((max((277263525 ^ var_11), -1058101635)))));
                arr_8 [6] [i_0] [i_1] = (((((max((arr_6 [i_0]), (arr_6 [i_0]))))) & (var_12 * -var_13)));
            }
        }
    }
    var_19 = var_17;
    var_20 = 255;
    #pragma endscop
}
