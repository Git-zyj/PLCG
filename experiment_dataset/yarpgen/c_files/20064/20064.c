/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20064
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20064 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20064
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (~((~(~var_0))));

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 0;i_4 += 1)
                        {
                            {
                                var_13 ^= ((-((var_6 ? var_3 : (arr_3 [i_4 + 1])))));
                                var_14 = (arr_7 [i_3] [i_1] [i_0]);
                                arr_13 [i_2] [i_3] = ((-((min(var_10, var_2)))));
                            }
                        }
                    }
                    var_15 = ((!(arr_6 [i_0])));
                    var_16 = (min(var_10, (((arr_12 [i_0] [i_2] [6] [i_0] [i_0]) ? (arr_12 [0] [i_2] [i_1] [8] [i_1]) : (arr_12 [i_0] [i_2] [i_2] [i_1] [i_1])))));
                }
            }
        }
        var_17 = ((~(arr_9 [i_0])));
    }
    #pragma endscop
}
