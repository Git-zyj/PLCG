/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3147
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3147 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3147
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */long long int) ((((/* implicit */_Bool) var_9)) ? (((/* implicit */int) ((signed char) 14631062098363526095ULL))) : (((/* implicit */int) var_13))));
    /* LoopSeq 1 */
    for (_Bool i_0 = 1; i_0 < 1; i_0 += 1) 
    {
        var_16 *= ((/* implicit */unsigned int) (_Bool)1);
        arr_2 [i_0] [i_0] = ((/* implicit */unsigned int) (unsigned char)113);
    }
}
