/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111566
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111566 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111566
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 &= var_4;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] = var_3;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 10;i_4 += 1)
                        {
                            {
                                var_11 = (((arr_7 [i_1] [i_2] [i_3] [i_4]) ? (arr_9 [i_0] [i_1]) : (((arr_3 [i_1]) ? (arr_5 [i_2] [i_4]) : (min((arr_4 [i_0] [i_3]), var_4))))));
                                var_12 = (((arr_4 [i_4] [i_1]) >= (max((((arr_3 [i_0]) ? (arr_13 [i_0] [i_1]) : (arr_10 [i_0] [i_1] [i_2] [i_3]))), (arr_2 [i_3] [i_4])))));
                            }
                        }
                    }
                    var_13 = (arr_12 [i_0] [i_1] [i_1] [i_1] [i_2]);
                }
            }
        }
    }
    #pragma endscop
}
