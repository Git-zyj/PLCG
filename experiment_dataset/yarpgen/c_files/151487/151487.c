/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151487
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151487 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151487
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max(var_20, var_7));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_21 = (max(var_21, (((arr_3 [i_1]) ? var_2 : (((arr_2 [i_0] [i_1]) ? (arr_2 [i_1] [i_0]) : (arr_2 [i_0] [i_1])))))));
                var_22 = (min(var_22, (((((min(8192, var_2))) ? (max((arr_1 [i_0]), var_12)) : -586049871)))));
                var_23 = (max(var_23, (~-8192)));
            }
        }
    }
    #pragma endscop
}
