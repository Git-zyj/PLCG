/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135672
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135672 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135672
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_6 [14] [i_1] [6] = (~2803609934891320703);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        {
                            arr_13 [i_0] [i_0] = ((((var_11 ? 0 : ((min(92364395, var_16))))) >= ((((min(var_2, var_6)))))));
                            arr_14 [i_0] [i_0] [i_1] [i_1] [i_2] [i_0] = (!10110967866442144304);
                            arr_15 [i_0] [i_0] [i_2] [i_3] = var_9;
                        }
                    }
                }
            }
        }
    }
    var_17 = var_5;
    #pragma endscop
}
