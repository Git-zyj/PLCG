/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 2207
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=2207 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/2207
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */short) var_6);
    var_21 = ((/* implicit */unsigned short) min((var_10), (((((/* implicit */_Bool) ((((/* implicit */_Bool) 17717110351053373831ULL)) ? (var_6) : (((/* implicit */int) var_13))))) ? (var_10) : (((/* implicit */unsigned int) ((((/* implicit */_Bool) var_18)) ? (((/* implicit */int) (short)6350)) : (((/* implicit */int) (short)-6371)))))))));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 1; i_0 < 7; i_0 += 4) 
    {
        var_22 = ((/* implicit */unsigned int) (signed char)-1);
        arr_4 [i_0] = ((/* implicit */unsigned char) 7837647742547869585LL);
    }
}
