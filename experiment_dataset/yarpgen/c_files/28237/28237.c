/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 28237
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=28237 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/28237
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [4] [4] &= var_6;
                arr_6 [6] [i_0] = (((27904 && 27904) ? ((-2147483629 ? (((arr_0 [i_1] [i_1]) * var_9)) : (((max(27915, 176)))))) : 3261742343507511791));
            }
        }
    }
    var_11 += (((-var_0 + 37605) + var_6));
    var_12 = (~613283519);
    var_13 -= ((~(15185001730202039824 & 2438471588)));
    #pragma endscop
}
