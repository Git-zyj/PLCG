/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71712
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71712 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71712
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 ^= (((((-6085234335681817168 ? ((max(0, 1))) : -var_7))) ? ((max(((var_11 ? 0 : var_2)), var_2))) : (max((var_9 * 12366808719712514467), var_9))));
    var_14 = var_7;
    var_15 -= 0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_16 |= var_12;
                var_17 = (((((arr_1 [i_0]) ? var_8 : -67))) ? var_9 : ((((4095 <= (arr_0 [9] [i_0]))))));
                arr_5 [i_1] [i_0] = 1;
            }
        }
    }
    #pragma endscop
}
