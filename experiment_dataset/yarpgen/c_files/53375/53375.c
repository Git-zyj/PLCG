/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 53375
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=53375 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/53375
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_12;
    var_16 = (max(409721434, (((~((55 ? var_3 : 57)))))));

    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        var_17 += (arr_0 [i_0 + 1] [18]);

        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            arr_8 [i_1] |= ((~((((min(var_3, (arr_0 [i_1] [i_1])))) ? ((var_1 ? (arr_4 [i_0 + 1] [i_0] [i_1]) : 1)) : (arr_4 [i_0 + 1] [i_0 + 1] [i_0 + 1])))));
            var_18 = (min(((~(arr_5 [i_1 - 2] [i_1 - 1]))), (arr_5 [i_1 - 1] [i_1])));
        }
        for (int i_2 = 3; i_2 < 20;i_2 += 1)
        {
            arr_12 [i_0] [i_0 + 1] [i_0] = var_13;
            var_19 = var_10;
        }
    }
    #pragma endscop
}
