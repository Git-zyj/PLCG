/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191660
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191660 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191660
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (_Bool i_0 = 0; i_0 < 0; i_0 += 1) 
    {
        var_11 = ((/* implicit */int) ((((/* implicit */_Bool) ((arr_0 [i_0]) ? (var_9) : (((/* implicit */long long int) ((/* implicit */int) var_5)))))) ? (((((/* implicit */_Bool) var_2)) ? (1421850599U) : (2873116697U))) : (((unsigned int) 2873116690U))));
        var_12 = ((/* implicit */unsigned long long int) (~(((/* implicit */int) arr_1 [i_0 + 1]))));
    }
    var_13 = ((/* implicit */_Bool) min((var_13), (((/* implicit */_Bool) var_6))));
}
