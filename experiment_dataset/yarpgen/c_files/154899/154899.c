/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 154899
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=154899 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/154899
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_12, (var_8 > var_4)));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 11;i_2 += 1)
            {
                {
                    var_13 = ((((~(arr_0 [i_2 - 3] [i_2 - 1]))) != ((min(var_5, ((max(var_6, (arr_1 [i_0])))))))));
                    var_14 = (((((var_7 ? 0 : var_6))) ? ((((var_8 == (arr_0 [i_2 - 2] [i_0 - 1]))))) : (max(1, var_4))));
                    var_15 |= (max((min(0, 15484130071377641388)), 11));
                }
            }
        }
    }
    var_16 += var_5;
    #pragma endscop
}
