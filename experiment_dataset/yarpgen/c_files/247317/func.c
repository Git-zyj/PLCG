/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247317
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247317 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247317
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_11 ^= (-(arr_2 [i_0] [i_0]));
        var_12 = ((/* implicit */unsigned short) (-(((/* implicit */int) (unsigned short)44318))));
    }
    for (unsigned short i_1 = 0; i_1 < 20; i_1 += 2) 
    {
        var_13 = ((/* implicit */long long int) arr_3 [i_1]);
        var_14 -= ((/* implicit */unsigned char) max((max((((/* implicit */unsigned short) ((2474808093U) != (((/* implicit */unsigned int) ((/* implicit */int) arr_4 [i_1])))))), (arr_3 [i_1]))), (arr_3 [i_1])));
        var_15 += ((/* implicit */short) ((((unsigned int) arr_3 [i_1])) << (((/* implicit */int) (!(((/* implicit */_Bool) ((unsigned int) arr_3 [i_1]))))))));
    }
    var_16 = var_8;
    var_17 *= ((/* implicit */unsigned short) var_2);
}
