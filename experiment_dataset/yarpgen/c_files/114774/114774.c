/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114774
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114774 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114774
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 16;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_2] [i_2] [i_0 + 1] = (((~1931425143) ? ((max(106, -116))) : (((~106) ? var_7 : 101))));
                    var_10 = -102;
                }
            }
        }
    }
    var_11 = (min(((((((101 ? var_1 : var_3))) ? ((var_1 ? var_4 : var_7)) : ((12683472508574069179 ? var_0 : var_2))))), (max(var_3, -var_1))));
    #pragma endscop
}
