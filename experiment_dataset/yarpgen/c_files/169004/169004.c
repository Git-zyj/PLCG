/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169004
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169004 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169004
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 11;i_0 += 1) /* same iter space */
    {
        var_19 = (((((var_1 | var_2) / (arr_0 [i_0 + 2]))) >= (arr_2 [i_0 - 1] [i_0 + 1])));
        var_20 *= (((((107 ? (arr_1 [i_0 - 1]) : (arr_1 [i_0 + 2])))) ? (((!((((arr_3 [i_0 - 2]) ? var_11 : var_16)))))) : ((((1 || (arr_3 [i_0]))) ? (arr_1 [12]) : ((-(arr_3 [i_0])))))));
        var_21 = -1;
    }
    for (int i_1 = 3; i_1 < 11;i_1 += 1) /* same iter space */
    {
        var_22 = (max(var_22, ((((arr_1 [i_1 + 2]) ? (((arr_1 [i_1 + 2]) ? (arr_1 [i_1 + 2]) : -1040339745)) : ((~(arr_1 [i_1 + 2]))))))));
        var_23 = var_12;
    }
    var_24 &= -6533229644066936001;
    #pragma endscop
}
