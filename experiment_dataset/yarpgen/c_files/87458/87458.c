/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87458
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87458 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87458
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (98 * 98);
    var_19 = ((var_16 >> (((((82 % 18266) | (var_17 * var_1))) - 1051549784445164150))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_20 = (min(var_14, ((+(max((arr_3 [i_0]), var_8))))));
                arr_6 [i_1] [i_0] = (arr_0 [i_1]);
                arr_7 [i_1] [i_1] = ((~(((((((arr_4 [i_1]) << (((var_9 + 1715164148) - 16))))) == 17789704113531630205)))));
            }
        }
    }
    var_21 = -1;
    #pragma endscop
}
