/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229501
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229501 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229501
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 -= ((((((var_12 + var_15) << (197 - 197)))) ? var_14 : (((((3255646732 ? var_14 : 127)))))));
    var_20 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 14;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 14;i_4 += 1)
                        {
                            {
                                var_21 = (min(var_21, -24));
                                var_22 = ((~(min((arr_10 [i_0] [i_4] [i_0] [i_3 + 1] [9]), (((arr_0 [i_3] [i_3]) ? (arr_9 [i_0] [i_3]) : (arr_6 [i_0])))))));
                                var_23 *= (min(18446744073709551603, 126));
                                var_24 ^= max(124, -83);
                            }
                        }
                    }
                }
                var_25 = ((-((max(-28, (arr_8 [i_1 + 1]))))));
            }
        }
    }
    #pragma endscop
}
