/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117526
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117526 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117526
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_4;
    var_15 = var_5;
    var_16 = (max((((max(var_6, var_9)) - var_9)), var_11));

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            /* LoopNest 2 */
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 21;i_3 += 1)
                {
                    {
                        var_17 = ((var_5 != (var_2 + var_11)));
                        arr_12 [i_0] [i_0] [i_2] [2] = min(((max(631713961, -1482009616))), var_1);
                        var_18 |= var_8;
                    }
                }
            }
            arr_13 [i_1] [i_1] [i_1] |= -12;
        }
        var_19 = (max((arr_0 [i_0]), (arr_7 [i_0] [i_0] [2])));
    }
    #pragma endscop
}
