/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 240717
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=240717 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/240717
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned short i_0 = 0; i_0 < 25; i_0 += 3) 
    {
        var_12 = ((/* implicit */unsigned short) max((var_12), (((/* implicit */unsigned short) (!((_Bool)1))))));
        var_13 = ((/* implicit */unsigned short) min((var_13), (((/* implicit */unsigned short) var_0))));
    }
    /* LoopSeq 1 */
    for (int i_1 = 0; i_1 < 18; i_1 += 1) 
    {
        var_14 += ((/* implicit */unsigned int) min((((/* implicit */long long int) ((var_11) <= (((/* implicit */long long int) ((/* implicit */int) (_Bool)1)))))), (min((min((9223372036854775807LL), (((/* implicit */long long int) (_Bool)1)))), (((/* implicit */long long int) var_10))))));
        var_15 ^= ((/* implicit */unsigned short) (((_Bool)1) ? (((/* implicit */int) (_Bool)1)) : (((/* implicit */int) (_Bool)1))));
        var_16 *= ((unsigned char) (!(((/* implicit */_Bool) (unsigned char)137))));
    }
    var_17 = ((/* implicit */long long int) max((var_17), (((/* implicit */long long int) var_9))));
    var_18 = ((/* implicit */int) max((var_18), (((/* implicit */int) ((long long int) var_11)))));
}
