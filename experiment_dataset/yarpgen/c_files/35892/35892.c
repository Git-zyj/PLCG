/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 35892
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=35892 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/35892
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_20 = (arr_1 [i_0]);
        arr_4 [i_0] |= ((7837 * ((~(((arr_3 [24]) ? (arr_3 [i_0]) : 5738043793120736436))))));
        arr_5 [i_0] = ((((min(154, 5906334786002461217))) ? (((!(-32767 - 1)))) : (arr_1 [i_0])));
    }
    var_21 = (min(var_21, var_12));
    var_22 -= (min((-32752 * 4673141057083156677), (((min(202, var_2))))));
    #pragma endscop
}
