/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233593
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233593 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233593
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_18;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 13;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_0] = ((arr_7 [i_2 + 1] [i_1] [i_1 + 1] [i_0]) ^ (((arr_0 [i_1 + 1] [i_2 - 1]) << (((arr_0 [i_1 + 2] [i_2 + 3]) - 50526)))));
                    var_21 = (max((((!(arr_5 [i_1 - 1])))), ((15 ? (1 != 240) : 140))));
                    arr_9 [i_1] [i_2] = var_18;
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                var_22 = (min(var_22, var_2));
                                arr_15 [i_0] [i_1] [i_1] [i_3] [i_4] = (arr_4 [i_0] [i_0]);
                                var_23 ^= (((arr_11 [i_2 + 2] [i_2 + 1] [5] [i_3 - 1]) > (arr_11 [i_2 - 2] [i_2 - 2] [i_3] [i_3 + 1])));
                            }
                        }
                    }
                }
            }
        }
    }
    var_24 = (((((244 + 1) == var_5))) + 163);
    #pragma endscop
}
