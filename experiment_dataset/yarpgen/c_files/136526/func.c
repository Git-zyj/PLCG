/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136526
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136526 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136526
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */signed char) (-((+(((var_2) - (((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned short)6609)))))))));
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 18; i_0 += 3) 
    {
        arr_2 [i_0] [i_0] = ((/* implicit */int) ((unsigned int) var_13));
        var_16 += ((/* implicit */_Bool) arr_1 [16LL] [i_0]);
    }
}
