/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 139780
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=139780 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/139780
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((!var_6) / var_8);
    var_21 = var_2;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_10 [i_1] [i_1] = (((max(213, 677489909)) ^ (var_12 == var_5)));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 24;i_4 += 1)
                        {
                            {
                                arr_15 [i_0] [i_1] [i_0] [i_0] = min(3, 0);
                                var_22 ^= ((-(((!(arr_0 [i_4]))))));
                                arr_16 [i_1] = var_3;
                            }
                        }
                    }
                    var_23 = (min((((min((arr_5 [4]), (arr_0 [i_0]))) ? 127 : ((239 ? 231 : (arr_12 [i_1] [i_1] [i_1]))))), (((((arr_4 [i_1] [13]) + var_19))))));
                }
            }
        }
    }
    #pragma endscop
}
