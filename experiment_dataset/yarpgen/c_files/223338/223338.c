/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 223338
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=223338 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/223338
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_15;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    var_20 = (min(var_20, (((!(((231 | (arr_0 [i_1])))))))));
                    arr_7 [i_0 + 1] [i_1] [i_0] = (max(((((((arr_0 [i_0]) / (arr_4 [i_0])))) ? 23 : (arr_1 [i_0]))), ((16383 ? 2471568758 : 47))));
                    var_21 = 250;
                    var_22 = (arr_1 [i_0]);
                    arr_8 [i_0] [i_1] [6] [i_1] = (max(((((arr_0 [i_0]) ? var_6 : var_17))), (max((~var_12), -16373))));
                }
            }
        }
    }
    var_23 = 16384;
    var_24 = var_1;
    var_25 = var_18;
    #pragma endscop
}
