/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104990
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104990 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104990
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max(var_13, ((!((((1987775550958290983 ? 3775807535 : -149168281)) != var_5))))));

    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        var_14 += (((((arr_1 [i_0 - 2]) != var_7)) ? (var_1 || var_10) : ((~(arr_1 [i_0 + 2])))));
        arr_3 [i_0] [i_0] = var_1;
        arr_4 [i_0] [i_0 - 1] = ((((((arr_2 [i_0 + 1]) * var_0))) ? ((((arr_2 [i_0 + 2]) >= (var_4 + var_0)))) : ((var_2 ? var_12 : var_7))));
    }
    var_15 = (-var_12 < var_6);
    var_16 = var_3;
    #pragma endscop
}
