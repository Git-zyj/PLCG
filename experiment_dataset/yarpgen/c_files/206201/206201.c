/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206201
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206201 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206201
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = (~var_9);
                arr_6 [i_0] = ((-(arr_3 [i_0])));
                arr_7 [i_0] [i_1] [i_0] = (~(min(var_6, (arr_0 [i_0]))));
                var_12 = (max(((~(max((arr_0 [i_1]), var_1)))), ((((arr_1 [i_0]) && (arr_4 [i_0]))))));
            }
        }
    }
    var_13 = (max(9, 3330644972));
    var_14 = ((((((~var_1) ? var_7 : (~var_11)))) | var_2));
    var_15 = ((-(max(var_9, (max(var_4, var_8))))));
    #pragma endscop
}
