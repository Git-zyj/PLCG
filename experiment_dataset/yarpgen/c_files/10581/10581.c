/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 10581
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=10581 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/10581
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = 1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_12 = 17261647453496764701;
                var_13 = ((~(((arr_2 [i_1] [i_0]) ^ (arr_4 [i_1] [i_0] [i_0])))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 22;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 22;i_3 += 1)
        {
            {
                arr_11 [i_2] [10] [i_2] = (arr_5 [i_2]);
                var_14 &= ((~(arr_8 [i_2])));
                arr_12 [i_3] [2] &= (max((!5601427056473038012), (((!5601427056473038012) ? var_9 : -10413))));
            }
        }
    }
    #pragma endscop
}
