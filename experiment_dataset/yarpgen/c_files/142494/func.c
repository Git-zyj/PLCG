/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 142494
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=142494 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/142494
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 |= ((/* implicit */long long int) (-(((((/* implicit */_Bool) var_1)) ? (((/* implicit */int) (_Bool)0)) : ((-(((/* implicit */int) var_7))))))));
    var_13 = ((/* implicit */short) (+(var_3)));
    var_14 &= ((/* implicit */signed char) var_7);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16; i_0 += 4) 
    {
        for (unsigned char i_1 = 0; i_1 < 16; i_1 += 4) 
        {
            for (unsigned short i_2 = 0; i_2 < 16; i_2 += 2) 
            {
                {
                    var_15 = ((/* implicit */long long int) ((arr_2 [i_1]) <= (((int) (+(((/* implicit */int) var_7)))))));
                    var_16 = ((/* implicit */unsigned short) ((unsigned char) (+(((/* implicit */int) ((((/* implicit */int) (short)-5153)) > (((/* implicit */int) var_6))))))));
                }
            } 
        } 
    } 
}
