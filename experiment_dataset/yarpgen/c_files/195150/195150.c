/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195150
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195150 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195150
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        arr_4 [i_0] = (((arr_2 [i_0] [i_0]) ? var_0 : var_1));
        arr_5 [i_0] = ((1 ? var_1 : 1842170718));
        var_12 = ((-((~(arr_0 [i_0] [i_0])))));
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 11;i_3 += 1)
                {
                    {
                        var_13 = (max(var_13, var_10));
                        var_14 = ((var_0 ? var_5 : var_6));
                        arr_12 [3] [i_3] [i_3] [i_2] [i_3] [i_3] = ((~(~var_9)));
                    }
                }
            }
        }
    }
    /* vectorizable */
    for (int i_4 = 1; i_4 < 16;i_4 += 1)
    {
        var_15 = 1;
        var_16 = 521877359;
        var_17 |= (arr_16 [i_4 - 1]);
    }
    var_18 = var_2;

    /* vectorizable */
    for (int i_5 = 0; i_5 < 14;i_5 += 1)
    {
        var_19 = (((arr_3 [i_5]) ? (((var_6 + (arr_15 [i_5])))) : ((var_8 ? (arr_18 [5]) : var_10))));
        var_20 = (min(var_20, (((var_5 ? (arr_8 [i_5] [i_5] [i_5] [i_5]) : (((1 ? 1 : 23246))))))));
    }
    #pragma endscop
}
