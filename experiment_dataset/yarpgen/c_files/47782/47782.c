/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47782
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47782 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47782
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        arr_3 [i_0] = ((~((((var_0 ? -1274586553 : 10866)) + (((arr_1 [i_0] [i_0]) ? var_15 : (arr_2 [i_0])))))));
        arr_4 [i_0] = ((arr_1 [i_0] [i_0]) >= (arr_1 [i_0] [i_0]));
        var_19 = (1 <= 10859);
    }
    var_20 = (((((~10859) ? ((var_13 ? var_5 : var_8)) : (((4294967288 ? 1274586555 : 8191))))) == var_16));
    var_21 = (min(var_21, ((max(var_4, ((var_13 ? (var_5 != var_17) : var_13)))))));
    var_22 = var_15;
    var_23 = var_2;
    #pragma endscop
}
