/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52604
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52604 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52604
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            {
                var_15 = ((var_7 & (arr_2 [i_0] [i_1] [i_1 + 1])));
                var_16 = ((((((arr_2 [i_0] [i_1] [i_1 + 2]) ? (arr_2 [i_0] [i_1] [i_1 + 2]) : var_11))) ? ((-(arr_2 [6] [i_1] [i_1 + 2]))) : var_0));

                /* vectorizable */
                for (int i_2 = 3; i_2 < 17;i_2 += 1)
                {
                    var_17 = (max(var_17, ((((arr_5 [i_2 + 1] [i_2 + 1]) ? var_2 : var_7)))));
                    var_18 = (arr_5 [i_2 + 3] [i_1 + 1]);
                    var_19 = (arr_0 [i_1 + 1]);
                }
                var_20 = var_13;
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 18;i_3 += 1)
    {
        for (int i_4 = 1; i_4 < 17;i_4 += 1)
        {
            {
                var_21 = 255;
                var_22 = (var_12 ? ((((var_5 | var_3) - var_4))) : ((-1620258188 ? var_11 : var_6)));
                var_23 = var_8;
            }
        }
    }
    #pragma endscop
}
