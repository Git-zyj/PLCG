/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45397
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45397 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45397
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_16;
    var_21 = ((max(var_9, (var_0 == 95))));
    var_22 = -var_9;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_23 = (max(((-(((arr_0 [i_0] [i_1]) ? (arr_1 [i_0]) : var_10)))), ((var_14 ? var_6 : (((!(arr_3 [i_0] [0] [0]))))))));
                arr_4 [i_0] [i_1] = var_10;
                arr_5 [i_1] [i_1] = (((((8173811107951622547 ? var_9 : 8173811107951622547))) ? 9 : -7));
            }
        }
    }
    #pragma endscop
}
