/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71380
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71380 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71380
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_3;

    for (int i_0 = 0; i_0 < 23;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] &= (min((!var_5), (((arr_1 [i_0]) ? 63 : (arr_0 [i_0])))));
        var_18 = -71;
    }
    for (int i_1 = 0; i_1 < 23;i_1 += 1) /* same iter space */
    {
        var_19 = (min((~-86), ((max(-74, 1)))));
        var_20 *= var_11;
        var_21 *= (min((((~var_12) ? var_15 : (((((arr_3 [i_1]) == -1915307554)))))), ((min((arr_5 [i_1]), var_14)))));
    }
    var_22 = ((-((((min(var_4, 1033))) ? 253 : (169 >> 0)))));
    var_23 = var_7;
    var_24 = 0;
    #pragma endscop
}
