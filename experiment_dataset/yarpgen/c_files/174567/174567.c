/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174567
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174567 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174567
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = -22803;
    var_19 = (max((0 + var_15), ((min(((min(var_14, var_15))), var_9)))));
    var_20 -= 38;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [8] [i_1] = (arr_4 [i_0] [i_1] [i_2]);

                    /* vectorizable */
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        var_21 = (min(var_21, 17592));
                        arr_9 [i_1] [3] [i_1] [i_1] = ((37193 ? (arr_4 [i_1] [i_1] [i_1]) : (arr_8 [i_1] [1] [i_1] [i_1])));
                    }
                }
            }
        }
    }
    var_22 = (!var_16);
    #pragma endscop
}
