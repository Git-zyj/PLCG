/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130292
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130292 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130292
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */unsigned int) var_4);
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 20; i_1 += 3) 
        {
            {
                var_11 -= ((/* implicit */unsigned char) 18446744073709551600ULL);
                var_12 ^= ((/* implicit */unsigned char) var_2);
            }
        } 
    } 
    var_13 = ((((/* implicit */_Bool) var_3)) ? (((/* implicit */unsigned long long int) var_5)) : (max((max((138613975111481399ULL), (18446744073709551600ULL))), (((/* implicit */unsigned long long int) var_0)))));
    var_14 = ((/* implicit */unsigned int) ((short) var_1));
}
