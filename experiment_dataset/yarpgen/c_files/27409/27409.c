/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27409
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27409 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27409
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    arr_8 [i_0] = 1;
                    arr_9 [i_0] [i_0] [i_1] [i_2] = var_11;
                    var_16 -= ((((min((min(var_15, (arr_3 [i_0] [i_2] [i_2]))), (arr_4 [i_1 - 1] [i_1] [i_1 + 1] [i_1 + 1])))) ? (min((((arr_5 [i_0] [i_0] [i_1 + 1] [i_2]) ? var_12 : var_11)), (min(var_7, (arr_1 [i_1]))))) : var_11));
                }
            }
        }
    }
    var_17 = ((!((min(var_6, (min(var_7, var_1)))))));
    var_18 |= (min((min((min(var_1, var_1)), var_1)), var_12));
    #pragma endscop
}
