/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215493
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215493 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215493
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((((-var_3 ? (min(var_18, var_12)) : (-16 / var_16)))) / 15));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_0] = (min((arr_5 [i_0]), (((min(var_1, var_15))))));
                    var_20 = ((min((arr_7 [i_2] [i_2 + 1] [i_2 + 2]), (arr_7 [i_2] [i_2 + 1] [i_2 + 1]))));
                    arr_9 [i_1] = (arr_7 [i_0] [12] [8]);
                }
            }
        }
    }
    #pragma endscop
}
