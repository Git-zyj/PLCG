/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36205
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36205 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36205
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 += var_3;
    var_15 = var_4;

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 13;i_3 += 1)
                {
                    {
                        arr_12 [i_0] [i_1] [i_1] [i_0] [i_3] [i_0] = var_4;
                        var_16 = (arr_8 [13] [i_0] [i_1] [i_3 + 2] [11] [i_2]);
                    }
                }
            }
        }
        var_17 -= -1914185849243157266;
    }
    /* vectorizable */
    for (int i_4 = 0; i_4 < 1;i_4 += 1)
    {
        var_18 = (((arr_13 [i_4]) ? 1 : (arr_15 [i_4])));
        arr_16 [i_4] = var_3;
        /* LoopNest 2 */
        for (int i_5 = 0; i_5 < 22;i_5 += 1)
        {
            for (int i_6 = 2; i_6 < 20;i_6 += 1)
            {
                {
                    arr_21 [20] [i_4] [i_6] [i_6 + 2] = 4149;
                    var_19 = ((var_1 % (((18446744073709551615 ? var_12 : -199631926)))));
                }
            }
        }
    }
    #pragma endscop
}
