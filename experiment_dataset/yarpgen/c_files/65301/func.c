/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 65301
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=65301 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/65301
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned short i_0 = 0; i_0 < 22; i_0 += 3) 
    {
        var_16 = ((/* implicit */signed char) min((((/* implicit */int) var_6)), (((((/* implicit */int) var_7)) + (((int) (_Bool)1))))));
        arr_3 [i_0] = ((/* implicit */unsigned char) (+(((((((/* implicit */_Bool) var_9)) ? (((/* implicit */unsigned int) ((/* implicit */int) (_Bool)1))) : (var_2))) + (((/* implicit */unsigned int) ((/* implicit */int) var_3)))))));
    }
    var_17 ^= ((/* implicit */unsigned int) (+(((/* implicit */int) var_14))));
    var_18 = ((/* implicit */long long int) var_11);
}
