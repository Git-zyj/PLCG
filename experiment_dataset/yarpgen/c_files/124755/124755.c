/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124755
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124755 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124755
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max((!var_6), ((((max(var_13, var_0))) ? var_7 : var_0))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        {
                            var_16 -= (arr_1 [i_3]);
                            var_17 = (((((~(arr_3 [i_0] [i_2])))) ? 0 : (((max(127, var_13))))));
                        }
                    }
                }
                arr_9 [i_0] [i_1] = ((+(((arr_7 [i_1]) ^ (arr_7 [i_1])))));
                arr_10 [i_0] = (((((min(1, (arr_2 [i_0] [i_0]))) >= (var_8 ^ 1))) ? (((var_3 ? (arr_4 [i_0]) : (arr_1 [i_0])))) : (((min((max(106, 1)), (arr_1 [i_0])))))));
            }
        }
    }
    var_18 = var_14;
    #pragma endscop
}
