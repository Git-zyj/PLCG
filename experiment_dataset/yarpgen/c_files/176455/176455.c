/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 176455
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=176455 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/176455
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_19 = ((((((-(arr_5 [10] [i_0] [i_2] [i_0]))))) | (((min(var_15, var_13)) - (arr_0 [i_0])))));
                    var_20 = (min(var_20, (((((((((4 ^ (arr_6 [i_0] [i_1] [i_2])))) ? (((arr_0 [i_2]) ^ var_7)) : (arr_2 [i_2] [0] [6])))) ? -var_11 : var_17)))));
                }
            }
        }
    }
    var_21 = ((99 | (min((-96 ^ 77), 77))));
    #pragma endscop
}
