/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219078
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219078 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219078
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((max((max(var_9, var_3)), (var_2 < 0))) == (((max(var_8, var_8))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_12 = (min(var_12, ((max(((((((arr_3 [i_0] [7] [i_0]) | -970083573)) < ((max((arr_2 [6] [i_1] [i_0]), (arr_6 [i_0]))))))), (max(var_0, (((970083572 / (arr_5 [i_2] [i_2] [i_2])))))))))));
                    var_13 += arr_4 [i_0];
                }
            }
        }
    }
    #pragma endscop
}
