/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51516
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51516 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51516
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */unsigned char) ((var_4) & (((/* implicit */unsigned int) ((/* implicit */int) var_5)))));
    /* LoopNest 2 */
    for (signed char i_0 = 3; i_0 < 13; i_0 += 2) 
    {
        for (long long int i_1 = 3; i_1 < 14; i_1 += 2) 
        {
            {
                var_14 = ((/* implicit */unsigned char) var_3);
                var_15 *= ((/* implicit */unsigned long long int) (~(((/* implicit */int) (unsigned short)56683))));
                arr_4 [i_0] [i_1 + 1] [i_1] = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) ((unsigned int) ((((/* implicit */int) var_0)) % (((/* implicit */int) (signed char)-53))))))));
            }
        } 
    } 
    var_16 = ((/* implicit */unsigned int) var_1);
    var_17 = ((/* implicit */unsigned long long int) min((var_17), (((/* implicit */unsigned long long int) (~(var_10))))));
}
