/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15779
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15779 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15779
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 19;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] = (arr_1 [i_0] [i_0]);
        var_20 = -1868085304;
        var_21 = ((!(arr_0 [i_0] [i_0])));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 19;i_1 += 1) /* same iter space */
    {
        var_22 = ((-((-(arr_6 [i_1] [6])))));
        arr_7 [i_1] = ((((((arr_3 [i_1] [i_1]) & var_3))) ? ((-1868085298 ? (arr_4 [i_1]) : var_11)) : (arr_1 [16] [1])));
        arr_8 [i_1] = (arr_6 [17] [i_1]);
    }
    var_23 = ((((var_11 + (max(1, 10158125974751364677))))) ? (max(var_15, (~23561))) : ((~(var_9 & 1))));
    #pragma endscop
}
