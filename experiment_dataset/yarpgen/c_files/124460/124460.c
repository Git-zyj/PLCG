/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124460
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124460 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124460
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_1;

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        arr_2 [i_0] = (((((arr_1 [i_0]) | (arr_1 [i_0])))) ? (!2147483647) : var_7);
        var_18 |= ((var_7 ? var_5 : ((var_2 ^ (arr_0 [i_0])))));
    }
    var_19 = ((10131720030526980145 * ((((~4294967291) * 116)))));
    var_20 = ((!(((var_11 ? (~var_10) : (max(764694417, var_5)))))));
    #pragma endscop
}
