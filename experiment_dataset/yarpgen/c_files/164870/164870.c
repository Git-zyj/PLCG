/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164870
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164870 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164870
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((((-(max(var_8, var_5))))) ? ((((var_7 ? 51520 : var_4)))) : (~-1858069799));
    var_19 = var_12;
    var_20 = ((-((1474576476551408352 ? -32753 : 13989))));

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_3 [i_0] = 1;
        var_21 = ((51520 ? (min(-958, 2603643053681620789)) : ((((arr_1 [i_0] [i_0]) * (-32767 - 1))))));
    }
    #pragma endscop
}
