/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 207566
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=207566 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/207566
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_11 = (arr_2 [i_1]);

                for (int i_2 = 1; i_2 < 17;i_2 += 1)
                {
                    var_12 |= (min(((min((arr_3 [i_0] [i_1 + 1] [i_2 + 3]), (((!(arr_0 [i_1]))))))), (((((arr_4 [i_0]) + 9223372036854775807)) << (var_8 - 92)))));
                    var_13 = 9223372036854775807;
                    var_14 = var_10;
                    var_15 += 9223372036854775807;
                }
                for (int i_3 = 0; i_3 < 20;i_3 += 1)
                {
                    var_16 |= (min(var_5, (arr_4 [i_3])));
                    /* LoopNest 2 */
                    for (int i_4 = 0; i_4 < 0;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 20;i_5 += 1)
                        {
                            {
                                var_17 = var_10;
                                var_18 = (min((max((arr_15 [i_0] [i_1] [i_3] [i_4] [i_5]), (arr_3 [i_3] [i_3] [i_5]))), (min((arr_9 [i_3] [i_1 + 1] [i_4 + 1] [i_1 + 1]), var_4))));
                                var_19 += (-9223372036854775807 - 1);
                                var_20 = arr_11 [i_5] [i_1] [i_4] [i_4] [i_0] [i_0];
                            }
                        }
                    }
                }
                var_21 = (arr_11 [i_1 + 1] [i_1] [i_0] [i_0] [i_0] [i_0]);
            }
        }
    }
    var_22 = (var_2 / (min(var_6, var_9)));
    var_23 = var_1;
    var_24 = (min(var_24, var_2));
    #pragma endscop
}
