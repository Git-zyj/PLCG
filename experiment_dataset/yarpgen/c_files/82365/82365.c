/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82365
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82365 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82365
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [i_1] = (arr_5 [i_0] [i_1] [i_0] [i_2]);
                    arr_7 [i_0 + 1] [i_1] = (!var_3);
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 16;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 16;i_4 += 1)
        {
            {
                var_20 = (4521529362373741817 || -3535092495074081218);
                var_21 -= 54932;
            }
        }
    }
    var_22 = min((var_6 & 209), var_14);
    var_23 = ((((-2091235680 - var_2) || (var_6 >= 57))));
    var_24 = var_15;
    #pragma endscop
}
