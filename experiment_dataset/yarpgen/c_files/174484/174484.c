/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174484
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174484 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174484
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((!((min(52714, ((var_4 ? 12792300697346833161 : 0)))))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 17;i_2 += 1)
            {
                {
                    var_12 -= (4142 - -96);
                    var_13 &= var_1;
                    var_14 = (max(14, ((((arr_3 [i_0 + 2] [i_0]) ? (arr_3 [i_0] [i_0]) : (arr_3 [i_0 - 1] [i_0]))))));
                    var_15 = var_8;
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_16 = (min(var_16, ((min((max(6958519037217344673, 16128)), ((((~(arr_4 [i_0] [i_0] [i_0]))))))))));
                                var_17 = ((var_0 ? (arr_5 [i_0 + 2] [i_0] [i_0 - 1]) : 12821));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
