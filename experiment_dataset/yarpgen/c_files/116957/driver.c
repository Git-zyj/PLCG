#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

short var_1 = (short)-12511;
signed char var_3 = (signed char)-104;
unsigned char var_4 = (unsigned char)82;
signed char var_5 = (signed char)35;
signed char var_6 = (signed char)-121;
unsigned long long int var_7 = 157650743027512617ULL;
unsigned char var_8 = (unsigned char)189;
unsigned char var_9 = (unsigned char)31;
int zero = 0;
int var_10 = 1909244848;
int var_11 = 316724517;
signed char var_12 = (signed char)91;
unsigned long long int var_13 = 3570695409548723050ULL;
void init() {
}

void checksum() {
    hash(&seed, var_10);
    hash(&seed, var_11);
    hash(&seed, var_12);
    hash(&seed, var_13);
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
