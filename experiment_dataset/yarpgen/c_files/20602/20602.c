/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20602
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20602 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20602
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    var_18 = (((-32767 - 1) ? var_2 : (min(((((arr_7 [i_2]) < var_16))), -5667230177110065617))));
                    var_19 -= (((arr_7 [i_0]) ? (max((((arr_8 [i_2] [i_0]) ? 9153773789490220518 : (arr_2 [i_0]))), var_4)) : var_12));
                    arr_9 [i_2] = 5724;
                }
            }
        }
    }
    var_20 = var_12;
    var_21 = ((var_3 ? var_9 : var_10));
    var_22 = var_11;
    #pragma endscop
}
