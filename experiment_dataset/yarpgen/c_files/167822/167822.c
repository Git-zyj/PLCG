/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 167822
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=167822 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/167822
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_13 |= 1616977569;
                arr_7 [i_0] [i_1] [i_1] |= 440221802;
            }
        }
    }
    var_14 = (max(var_14, (((var_2 ? (max(((var_7 ? 3602135088 : var_7)), (3602135095 + var_9))) : var_6)))));
    var_15 = (!var_9);
    var_16 = var_10;
    var_17 |= (((((((3602135093 ? var_0 : var_3))) ? (~var_1) : (~var_5))) <= (((var_4 ? var_3 : var_0)))));
    #pragma endscop
}
