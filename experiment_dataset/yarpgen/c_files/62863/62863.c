/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62863
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62863 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62863
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_1;
    var_11 = (min(var_11, (17448635161399859541 != 17448635161399859511)));
    var_12 = var_8;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 9;i_2 += 1)
            {
                {
                    var_13 = (arr_3 [i_0]);
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_14 = (min(var_14, (~18446744073709551605)));
                                arr_13 [i_0] [i_3] [8] [i_3] [4] [i_1] = (arr_6 [i_1] [i_2 + 2] [i_2 + 2]);
                                var_15 = (~998108912309692084);
                                arr_14 [i_0] [i_1] [i_3] = 998108912309692104;
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
