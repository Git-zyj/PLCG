/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133701
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133701 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133701
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 ^= ((/* implicit */signed char) ((((/* implicit */_Bool) var_19)) ? ((-(var_3))) : (((/* implicit */int) ((unsigned short) ((signed char) var_11))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (int i_0 = 0; i_0 < 19; i_0 += 1) 
    {
        var_21 = ((/* implicit */_Bool) ((unsigned int) (!(((/* implicit */_Bool) -6696872850797246368LL)))));
        var_22 = ((/* implicit */signed char) ((((/* implicit */unsigned long long int) ((/* implicit */int) (short)6285))) ^ (72057594037927936ULL)));
        var_23 = ((/* implicit */_Bool) ((unsigned long long int) arr_1 [i_0] [i_0]));
        var_24 = ((/* implicit */signed char) (~(var_13)));
        var_25 += ((/* implicit */signed char) (+(((/* implicit */int) var_0))));
    }
    var_26 = ((/* implicit */_Bool) min((var_26), (((/* implicit */_Bool) var_12))));
}
