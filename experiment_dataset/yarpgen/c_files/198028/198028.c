/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198028
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198028 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198028
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_16, ((((((var_8 >> (var_5 - 18596)))) ? (((((min(var_8, var_15))) < (~var_1)))) : var_10)))));
    var_17 = (max(1687670108214396513, 1));

    for (int i_0 = 0; i_0 < 0;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] = var_11;
        arr_3 [i_0] = -var_8;
    }
    for (int i_1 = 0; i_1 < 0;i_1 += 1) /* same iter space */
    {
        var_18 = (min(var_18, ((!(arr_4 [i_1] [2])))));
        var_19 ^= ((((((arr_0 [i_1 + 1]) ? (arr_4 [i_1] [i_1 + 1]) : 1687670108214396513))) ? (min(-1687670108214396504, 8388607)) : ((((arr_5 [2]) ? (arr_5 [i_1 + 1]) : (arr_0 [i_1]))))));
    }
    #pragma endscop
}
