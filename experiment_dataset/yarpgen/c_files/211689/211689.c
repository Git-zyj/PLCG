/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211689
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211689 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211689
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min((~var_1), ((~((24871 ? -27 : var_1))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = ((-49 ? (((-(arr_3 [i_0])))) : (arr_3 [i_0])));
        arr_5 [i_0] [i_0] = (((arr_2 [i_0]) ? ((~(arr_0 [i_0]))) : ((~(arr_0 [i_0])))));
        arr_6 [i_0] [i_0] = (arr_2 [i_0]);
    }
    var_19 = (min(1, ((var_4 ? (min(var_8, var_14)) : (~var_12)))));
    #pragma endscop
}
