/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85913
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85913 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85913
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = min((((/* implicit */unsigned int) var_5)), (min((((/* implicit */unsigned int) var_8)), (max((var_10), (((/* implicit */unsigned int) var_0)))))));
    var_18 = ((/* implicit */unsigned char) max((var_18), (((/* implicit */unsigned char) var_10))));
    /* LoopNest 3 */
    for (unsigned int i_0 = 0; i_0 < 15; i_0 += 4) 
    {
        for (int i_1 = 0; i_1 < 15; i_1 += 1) 
        {
            for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
            {
                {
                    var_19 = ((/* implicit */int) var_0);
                    var_20 = ((/* implicit */unsigned char) max((var_20), (((/* implicit */unsigned char) (+(((/* implicit */int) (!(((/* implicit */_Bool) arr_3 [i_2] [i_1] [i_2]))))))))));
                }
            } 
        } 
    } 
    var_21 = ((/* implicit */unsigned char) max((var_21), (((/* implicit */unsigned char) var_4))));
    /* LoopNest 2 */
    for (short i_3 = 0; i_3 < 13; i_3 += 4) 
    {
        for (unsigned char i_4 = 0; i_4 < 13; i_4 += 1) 
        {
            {
                arr_13 [i_3] = ((/* implicit */int) ((unsigned int) ((((/* implicit */_Bool) arr_12 [i_3])) ? (((/* implicit */unsigned long long int) ((/* implicit */int) arr_8 [i_3]))) : (arr_2 [i_3] [i_3]))));
                var_22 = ((/* implicit */short) (-(((/* implicit */int) (!(var_8))))));
            }
        } 
    } 
}
