/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117132
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117132 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117132
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_19 = ((~(((arr_0 [i_0]) << 0))));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_20 = ((-(arr_12 [0] [i_1] [0] [i_1] [i_1] [i_1])));
                                arr_15 [i_1] [i_1] [i_2] [i_2] [i_4] [i_0] [i_0] = -3;
                            }
                        }
                    }
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_5 = 0; i_5 < 20;i_5 += 1)
    {
        for (int i_6 = 0; i_6 < 20;i_6 += 1)
        {
            {
                var_21 = (((((((((arr_17 [i_5]) ? 90 : 8))) ? (((arr_0 [i_6]) ? (arr_8 [10] [i_6] [i_5] [i_6]) : (arr_9 [0] [i_6] [i_5] [i_5]))) : 45928))) & (((!127) | ((-(arr_5 [i_5] [i_5] [i_5])))))));
                arr_22 [i_5] [i_6] = (arr_13 [i_6] [i_5] [i_5]);
                var_22 = -1;
            }
        }
    }
    #pragma endscop
}
