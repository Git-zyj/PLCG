/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 37952
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=37952 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/37952
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (~(min((~var_0), (max(var_11, var_9)))));
    var_14 *= ((min(-11065, (var_7 != 692))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 21;i_4 += 1)
                        {
                            {
                                var_15 = ((!((max(960, (arr_3 [i_0] [i_4]))))));
                                arr_14 [0] [22] [1] [i_0] [i_3] [19] [i_4] = (arr_8 [i_0] [0]);
                                var_16 = (arr_5 [i_0] [i_1]);
                            }
                        }
                    }
                    var_17 = -238;
                    var_18 *= (arr_12 [21] [i_0] [i_2]);
                }
            }
        }
    }
    #pragma endscop
}
