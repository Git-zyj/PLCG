/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 121113
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=121113 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/121113
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_11;
    var_20 = (max(var_20, (var_8 || 17181325478414012484)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] [i_1] = (max(17181325478414012463, -1576493451));
                    arr_9 [i_1] [i_0] = (((((min((arr_7 [i_0] [i_1] [i_2]), (arr_0 [i_0] [i_0]))))) * (arr_3 [i_1] [i_2])));
                }
            }
        }
    }
    #pragma endscop
}
