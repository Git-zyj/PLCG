/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 35720
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=35720 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/35720
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned short) (-((+(((/* implicit */int) var_13))))));
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 13; i_0 += 3) 
    {
        for (unsigned char i_1 = 1; i_1 < 11; i_1 += 3) 
        {
            {
                var_16 = ((/* implicit */unsigned long long int) min((((((/* implicit */_Bool) min((((/* implicit */int) arr_1 [i_0])), (var_6)))) ? ((+(((/* implicit */int) var_10)))) : ((~(((/* implicit */int) (unsigned char)105)))))), (((/* implicit */int) min(((unsigned char)118), (((/* implicit */unsigned char) arr_3 [i_1] [i_0] [i_1])))))));
                var_17 = ((/* implicit */unsigned int) var_7);
                var_18 = ((/* implicit */unsigned short) max((var_18), (((/* implicit */unsigned short) ((((/* implicit */_Bool) 7900359968652491333LL)) ? (arr_2 [6U]) : ((~((+(((/* implicit */int) var_1)))))))))));
            }
        } 
    } 
    var_19 = ((/* implicit */unsigned char) var_2);
}
