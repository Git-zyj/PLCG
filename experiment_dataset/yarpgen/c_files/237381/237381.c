/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237381
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237381 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237381
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = -6466111398728317426;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_13 = (max(var_9, ((((arr_5 [i_0] [i_0] [i_2]) - 0)))));
                    arr_8 [i_0] [i_0] [i_1] [i_2] = ((!(((var_10 ? -21198 : -9223372036854775789)))));
                }
            }
        }
    }
    var_14 += var_8;
    /* LoopNest 3 */
    for (int i_3 = 0; i_3 < 13;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 13;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 13;i_5 += 1)
            {
                {
                    arr_18 [i_3] [i_3] [i_3] = 0;
                    arr_19 [12] [i_4] = arr_17 [i_3] [i_4] [i_4];
                    arr_20 [i_4] [i_4] = (min((arr_11 [10]), var_0));
                }
            }
        }
    }
    #pragma endscop
}
