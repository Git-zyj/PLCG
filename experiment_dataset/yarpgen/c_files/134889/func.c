/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 134889
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=134889 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/134889
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (signed char i_0 = 1; i_0 < 9; i_0 += 4) 
    {
        for (int i_1 = 1; i_1 < 6; i_1 += 2) 
        {
            {
                var_17 = ((/* implicit */unsigned short) (-(((/* implicit */int) (unsigned char)0))));
                var_18 = ((/* implicit */unsigned int) 72057594037927935LL);
                var_19 = ((/* implicit */unsigned short) (~(((/* implicit */int) (!(((/* implicit */_Bool) (~(((/* implicit */int) (unsigned char)182))))))))));
            }
        } 
    } 
    var_20 = (unsigned char)255;
}
