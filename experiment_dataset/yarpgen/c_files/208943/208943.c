/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208943
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208943 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208943
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (((max((min(var_0, 1)), ((max(-861629390, var_8))))) + var_13));
    var_16 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_17 -= (min(-17530290139386329211, ((min((arr_2 [i_1]), 10179049)))));

                /* vectorizable */
                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    arr_6 [i_0] [i_1] [i_2] [i_2] = (arr_1 [i_0]);
                    arr_7 [i_2] [i_2] |= (((arr_2 [i_1]) >= (arr_2 [i_2])));
                    var_18 = (1926705472 != -103);
                    var_19 = (arr_1 [i_0]);
                }
            }
        }
    }
    var_20 = (1 ^ 96);
    #pragma endscop
}
