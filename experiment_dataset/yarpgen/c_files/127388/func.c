/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127388
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned long long int) ((((((var_8) ^ (var_10))) <= (var_2))) ? (572982542847721848LL) : (((/* implicit */long long int) ((/* implicit */int) ((((/* implicit */int) ((unsigned char) var_4))) <= (((/* implicit */int) (unsigned char)0))))))));
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 16; i_0 += 3) 
    {
        for (unsigned int i_1 = 4; i_1 < 15; i_1 += 4) 
        {
            {
                var_17 = ((/* implicit */int) arr_0 [i_0] [i_0]);
                var_18 = ((/* implicit */long long int) arr_2 [i_1]);
            }
        } 
    } 
    var_19 *= ((/* implicit */unsigned char) ((((((/* implicit */long long int) ((((/* implicit */_Bool) 16865759761648522508ULL)) ? (((/* implicit */int) (unsigned char)255)) : (((/* implicit */int) (unsigned char)0))))) <= (var_8))) ? (((/* implicit */long long int) var_4)) : (var_2)));
    var_20 *= ((/* implicit */unsigned long long int) var_9);
}
