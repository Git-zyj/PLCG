/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 192066
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=192066 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/192066
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 0;i_2 += 1)
            {
                {
                    var_15 &= 6;
                    var_16 = (min((arr_7 [i_1] [i_2 + 1] [i_2 + 1]), 45387077));
                    var_17 = (min(var_17, ((min(((~(min((arr_0 [i_1]), (arr_6 [i_0] [i_1] [i_2]))))), (((((min((arr_7 [i_1] [i_1] [i_0]), var_2))) || ((((arr_4 [i_0] [i_0]) & var_12)))))))))));
                }
            }
        }
    }
    var_18 = (min(var_18, ((min(var_9, 14)))));
    #pragma endscop
}
