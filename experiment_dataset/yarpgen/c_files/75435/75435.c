/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75435
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75435 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75435
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_4;
    var_18 = var_11;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    var_19 *= (min((arr_0 [i_2]), ((~((min(33, (-32767 - 1))))))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 21;i_4 += 1)
                        {
                            {
                                var_20 *= ((~(arr_0 [i_0])));
                                var_21 = ((((!((!(arr_5 [2] [i_2] [i_4]))))) ? (~var_0) : (arr_5 [i_1] [i_3] [i_1])));
                            }
                        }
                    }
                    var_22 = (((((((var_8 < (-32767 - 1))) ? (arr_8 [i_0] [i_0] [i_2] [i_1] [i_1] [i_1]) : (arr_9 [i_0] [i_1] [i_0] [i_0] [i_0] [i_0] [i_0])))) != (max(((((arr_4 [i_2] [i_2] [i_1] [4]) ? 33 : (arr_6 [i_1])))), (arr_3 [3] [3])))));
                }
            }
        }
    }
    #pragma endscop
}
