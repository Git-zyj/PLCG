/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177552
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177552 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177552
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_2;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 14;i_4 += 1)
                        {
                            {
                                var_11 *= (max(3, 6));
                                arr_13 [i_0] [i_2] [i_0 + 1] = (arr_11 [i_1] [i_1] [i_2] [i_3] [i_3]);
                                var_12 = (max((arr_4 [i_0 - 1]), (arr_4 [i_0 - 1])));
                                arr_14 [i_2] [i_3] = (((-9223372036854775802 ? var_6 : (((arr_3 [i_4]) ? (arr_3 [i_2]) : var_6)))));
                            }
                        }
                    }
                    arr_15 [i_2] [i_1] [i_2] = ((3929 <= (((((arr_4 [i_0]) ? 9409195459483725996 : var_5))))));
                }
            }
        }
    }
    var_13 *= var_8;
    #pragma endscop
}
