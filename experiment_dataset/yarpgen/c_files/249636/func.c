/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249636
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249636 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249636
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned short i_0 = 0; i_0 < 16; i_0 += 1) 
    {
        var_17 = ((/* implicit */_Bool) min((((((/* implicit */_Bool) 18446744073709551613ULL)) ? (-7358551297830338048LL) : (-1144834570929000598LL))), (((((/* implicit */long long int) ((/* implicit */int) (!(((/* implicit */_Bool) (signed char)-121)))))) | (((8896035376946434143LL) - (((/* implicit */long long int) ((/* implicit */int) arr_1 [i_0] [i_0])))))))));
        var_18 += ((/* implicit */unsigned short) var_2);
    }
    var_19 = var_14;
}
