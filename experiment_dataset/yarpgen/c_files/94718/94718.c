/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94718
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94718 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94718
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = (((max((!18), 0)) ? ((((((arr_1 [i_0] [i_1]) * 101))) ? var_16 : (arr_4 [i_1]))) : (((arr_2 [i_1]) ? (-92 || -92) : ((0 ? var_16 : (arr_4 [i_0])))))));
                var_18 = (((((~(0 % var_15)))) ? -255 : ((((min(2493302567, 24))) ? (arr_4 [i_0]) : ((0 ? var_3 : 7))))));
                arr_7 [i_0] [i_1] = (min(var_2, ((-8245395991364613273 - (arr_0 [i_0])))));
            }
        }
    }
    var_19 = var_4;
    #pragma endscop
}
