/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206499
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206499 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206499
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_14 = (((((((arr_4 [i_0] [i_0]) * (arr_5 [i_0] [i_1])))) ? (arr_4 [10] [i_0]) : ((min((arr_4 [i_0] [i_1]), (arr_3 [i_0] [i_1])))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        {
                            var_15 *= (((((arr_8 [i_0] [i_0] [i_0]) ? var_6 : (arr_8 [i_0] [i_1] [i_1])))) ? ((((arr_9 [i_3]) > (arr_9 [i_3])))) : (arr_0 [i_3]));
                            var_16 = (arr_0 [i_1]);
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
