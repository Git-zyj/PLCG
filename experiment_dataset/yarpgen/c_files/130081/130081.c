/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130081
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130081 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130081
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_16;

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        arr_2 [i_0] = ((((var_3 && (arr_0 [i_0] [i_0]))) ? -1 : (arr_1 [i_0] [i_0])));
        arr_3 [i_0] &= ((1 ? ((-1566313242 ? (var_1 - 1) : (arr_1 [i_0] [12]))) : -118));
        arr_4 [i_0] [i_0] = (((-7143880733684330827 ^ ((((arr_0 [i_0] [i_0]) * var_10))))));
        arr_5 [i_0] |= var_12;
    }
    var_20 = (max(var_18, 1746912044100054791));
    var_21 = var_12;
    #pragma endscop
}
