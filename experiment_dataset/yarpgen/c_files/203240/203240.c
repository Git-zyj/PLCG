/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203240
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203240 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203240
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, ((((var_3 ? 1022 : (min(4294966255, -2709516200925599697))))))));
    var_11 = var_2;

    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        var_12 = (min(var_12, (min((((((arr_1 [i_0 - 2] [i_0 + 1]) + 9223372036854775807)) >> (((arr_1 [i_0] [i_0 + 1]) + 1280360853596699936)))), ((var_6 ? (arr_1 [14] [i_0 + 1]) : (arr_1 [4] [i_0 + 1])))))));
        var_13 = (~var_0);
        arr_2 [i_0 - 2] = var_3;
    }
    /* vectorizable */
    for (int i_1 = 2; i_1 < 14;i_1 += 1)
    {
        /* LoopNest 3 */
        for (int i_2 = 1; i_2 < 15;i_2 += 1)
        {
            for (int i_3 = 2; i_3 < 15;i_3 += 1)
            {
                for (int i_4 = 4; i_4 < 12;i_4 += 1)
                {
                    {
                        var_14 = ((var_7 ? var_7 : var_1));
                        var_15 = ((var_6 ? var_1 : (arr_9 [i_1])));
                        var_16 = 255;

                        for (int i_5 = 1; i_5 < 13;i_5 += 1)
                        {
                            var_17 = (var_1 && var_3);
                            var_18 = (max(var_18, var_4));
                        }
                        arr_17 [i_2] [4] &= (!16890);
                    }
                }
            }
        }
        var_19 &= (arr_3 [i_1] [i_1]);
    }
    #pragma endscop
}
