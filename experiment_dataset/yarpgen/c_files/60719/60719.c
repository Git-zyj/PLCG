/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60719
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60719 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60719
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((!((min(((var_8 ? 0 : 0)), ((var_4 ? -43 : 1)))))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_13 = (-(max((min((arr_1 [7] [7]), (arr_2 [i_0]))), (((0 ? 12 : (arr_3 [i_0] [i_1] [i_1])))))));
                arr_6 [i_0] [i_0 - 1] = ((~(max((max((arr_2 [i_0]), (arr_1 [i_1] [i_1]))), 65535))));
                var_14 = (((max(1, 562949953421280)) == (((min((max(1, 32519)), (((!(arr_0 [i_0]))))))))));
            }
        }
    }
    var_15 = 32519;
    #pragma endscop
}
