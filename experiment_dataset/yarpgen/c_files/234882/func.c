/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234882
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234882 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234882
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned char) var_4);
    /* LoopNest 3 */
    for (unsigned char i_0 = 1; i_0 < 23; i_0 += 3) 
    {
        for (unsigned char i_1 = 0; i_1 < 25; i_1 += 1) 
        {
            for (unsigned char i_2 = 0; i_2 < 25; i_2 += 2) 
            {
                {
                    var_12 = ((/* implicit */unsigned char) (~((+(((/* implicit */int) (unsigned short)43164))))));
                    var_13 *= ((/* implicit */unsigned char) min(((+((+(((/* implicit */int) var_9)))))), (min(((~(((/* implicit */int) var_5)))), (((/* implicit */int) (!(((/* implicit */_Bool) arr_3 [i_1])))))))));
                }
            } 
        } 
    } 
    var_14 = ((/* implicit */unsigned char) max((var_14), (((/* implicit */unsigned char) (~((-(((/* implicit */int) var_5)))))))));
}
