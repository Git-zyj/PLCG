/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229188
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229188 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229188
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = -var_3;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            {
                var_21 = (((((48 ? 1920 : var_13))) ? (arr_0 [i_0]) : (((max((arr_3 [6] [i_0]), -64))))));
                arr_4 [i_0] = ((~((min((max((arr_3 [i_0] [i_0]), (arr_3 [11] [i_0]))), var_4)))));
                var_22 = (min((arr_1 [1]), (arr_0 [i_1 + 2])));
                arr_5 [i_0] [i_1] = (!((min(1, var_12))));
            }
        }
    }
    var_23 = (max((((-253272439 ? 130048 : 48))), var_19));
    var_24 = var_4;
    var_25 = (!var_15);
    #pragma endscop
}
