/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68908
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68908 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68908
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_11, (((!((((max(var_1, 1308401823))) || var_8)))))));
    var_12 *= var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = (arr_1 [i_0]);
                var_13 &= ((~((max(var_9, (arr_1 [i_1]))))));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 23;i_4 += 1)
                        {
                            {
                                arr_13 [i_1] [i_1] [i_1] [i_1] [i_0] = (arr_12 [i_0] [i_0] [i_0] [i_0] [i_0]);
                                arr_14 [i_0] [i_1] [i_0] [i_4] &= 331450220;
                                arr_15 [i_0] = var_8;
                            }
                        }
                    }
                }
            }
        }
    }
    var_14 = var_1;
    #pragma endscop
}
