/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76544
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76544 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76544
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_6 [i_0] [12] [i_0] = (((((var_9 ? var_3 : 3342554556808081256))) && (5238919267361010604 * var_8)));
                arr_7 [i_0] = ((((4205821650271343621 ? 16383 : (((-9223372036854775807 - 1) / 22412))))) % (((var_6 ? 2777222391468285337 : 18237009857949016493))));
            }
        }
    }
    var_20 = var_17;
    #pragma endscop
}
