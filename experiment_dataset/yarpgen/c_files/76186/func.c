/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76186
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76186 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76186
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 ^= ((/* implicit */int) var_15);
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 14; i_0 += 4) 
    {
        for (unsigned char i_1 = 1; i_1 < 15; i_1 += 4) 
        {
            for (short i_2 = 3; i_2 < 16; i_2 += 4) 
            {
                {
                    arr_8 [9U] [i_1] [i_1] [i_2 - 3] = ((((((/* implicit */int) var_2)) + (((/* implicit */int) arr_4 [i_0 - 3])))) >= (max((((/* implicit */int) (!(((/* implicit */_Bool) arr_3 [i_1 + 2] [i_1 + 2]))))), (var_6))));
                    var_17 = ((/* implicit */unsigned short) (+(((/* implicit */int) ((unsigned char) min((((/* implicit */signed char) var_10)), (arr_4 [i_2])))))));
                }
            } 
        } 
    } 
}
