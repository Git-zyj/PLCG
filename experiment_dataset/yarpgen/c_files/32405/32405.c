/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32405
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32405 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32405
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 -= (max((min((~var_13), (max(var_0, var_6)))), (min(((max(var_5, var_1))), (~var_11)))));
    var_16 = min(-2305843009213693952, 189);
    var_17 = (min((max(((min(var_6, var_3))), (~var_12))), (~var_5)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 20;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 24;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 24;i_4 += 1)
                        {
                            {
                                var_18 = (max(var_18, (!var_7)));
                                var_19 = (min(var_19, ((max((max(var_1, (min(var_6, var_7)))), (min((max(var_9, var_2)), var_4)))))));
                            }
                        }
                    }
                }
                var_20 = var_1;
                var_21 += var_3;
            }
        }
    }
    var_22 = -var_5;
    #pragma endscop
}
