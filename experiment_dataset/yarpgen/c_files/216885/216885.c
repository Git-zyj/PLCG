/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 216885
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=216885 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/216885
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = -var_2;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] [i_1] = var_8;
                arr_5 [i_1] = ((-(((577125740 >> (4942 - 4918))))));
                arr_6 [i_0] [i_1] [i_0] |= ((((!(arr_3 [i_1 + 2] [i_1 + 3]))) ? var_8 : ((((arr_3 [i_1 + 2] [i_1]) ? -8267558579531729003 : (arr_3 [i_1 + 2] [i_1 + 2]))))));
                arr_7 [i_0] &= (((((-577125741 ? ((-4942 ? 219 : 0)) : (arr_2 [i_0] [i_0])))) ? (-219 / 28782) : var_6));
            }
        }
    }
    #pragma endscop
}
