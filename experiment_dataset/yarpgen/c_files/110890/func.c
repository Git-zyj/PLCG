/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110890
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110890 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110890
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 |= ((/* implicit */int) var_9);
    var_21 = ((/* implicit */int) var_16);
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 17; i_0 += 1) 
    {
        for (unsigned int i_1 = 0; i_1 < 17; i_1 += 1) 
        {
            {
                var_22 = ((/* implicit */unsigned long long int) min((((/* implicit */long long int) (unsigned short)60024)), (-5076587332800853709LL)));
                arr_8 [i_0] [(short)14] = ((/* implicit */signed char) ((short) min((arr_1 [i_0]), (((/* implicit */signed char) (!(((/* implicit */_Bool) -1139632758))))))));
            }
        } 
    } 
}
