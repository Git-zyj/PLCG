/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 232332
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=232332 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/232332
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_8;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            {
                var_12 = (((max(6390980424514233488, var_8)) == (((var_5 ? ((((arr_1 [i_0]) != (arr_0 [i_0])))) : (arr_0 [i_0]))))));
                arr_4 [2] [i_1] [i_0] &= var_3;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 8;i_3 += 1)
                    {
                        {
                            arr_9 [i_0] [i_1] [i_2] = 32;
                            var_13 += (((~(6390980424514233488 ^ var_4))));
                            var_14 = arr_8 [i_0 - 1] [i_1] [i_0] [i_3];
                        }
                    }
                }
            }
        }
    }
    var_15 += ((var_0 ? 442 : ((-(22 % -8149008705019023488)))));
    var_16 = var_4;
    #pragma endscop
}
