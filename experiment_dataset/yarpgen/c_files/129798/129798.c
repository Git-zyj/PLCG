/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 129798
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=129798 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/129798
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_11 = (arr_0 [8]);
        var_12 ^= (var_3 && var_10);
        var_13 = (min(var_13, (arr_1 [i_0])));
    }
    for (int i_1 = 1; i_1 < 10;i_1 += 1) /* same iter space */
    {
        arr_4 [i_1] [i_1] = (min(-22, 65));
        var_14 = (min(var_14, (((~(~var_8))))));
    }
    /* vectorizable */
    for (int i_2 = 1; i_2 < 10;i_2 += 1) /* same iter space */
    {
        var_15 &= -115;
        var_16 *= -89;
        var_17 = ((-7136 % 121) ? (arr_5 [i_2] [i_2 - 1]) : (!var_3));
    }
    for (int i_3 = 0; i_3 < 24;i_3 += 1)
    {
        var_18 = (max((arr_10 [i_3]), var_3));
        var_19 = (min(var_19, ((max(121, 9223372036854775807)))));
        arr_11 [i_3] [i_3] = (arr_10 [i_3]);
        var_20 += (arr_9 [i_3]);
    }
    var_21 = (min(var_21, 1));
    var_22 -= ((((((min(var_8, 6))) | var_1)) >= (min(var_8, var_5))));
    var_23 -= ((max(var_8, ((max(var_6, var_6))))));
    #pragma endscop
}
