/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14854
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14854 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14854
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_11 = var_3;
                var_12 = ((((min((arr_3 [i_0 - 1] [3]), var_1))) && (((var_2 ? var_1 : var_4)))));
                arr_5 [i_1] = ((var_10 ? (arr_1 [i_1] [i_0]) : var_3));
            }
        }
    }
    var_13 = (max(var_13, (!var_1)));

    /* vectorizable */
    for (int i_2 = 0; i_2 < 1;i_2 += 1)
    {
        arr_9 [i_2] = var_1;
        var_14 *= ((!(arr_8 [i_2])));
    }
    for (int i_3 = 0; i_3 < 18;i_3 += 1)
    {
        arr_12 [i_3] = ((((!((max(var_9, var_1))))) ? ((min(var_5, var_9))) : ((~((~(arr_11 [i_3])))))));
        arr_13 [i_3] [4] = var_8;
        var_15 = ((-161 ? (var_3 * var_7) : var_2));
        arr_14 [i_3] = ((~((max(var_7, (arr_11 [i_3]))))));
    }
    #pragma endscop
}
