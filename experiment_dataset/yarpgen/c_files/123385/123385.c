/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123385
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123385 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123385
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_9;
    var_14 = -1;
    var_15 = (((58403 ^ -928065984) ? (min(var_8, var_3)) : (!var_1)));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_16 = (min(var_16, (((((((var_2 >= (min(var_5, (arr_2 [i_0]))))))) - -0)))));
                arr_6 [i_0 + 1] [6] [i_0] = min(((var_12 ? (arr_2 [i_0]) : var_0)), var_9);
                var_17 -= ((((((var_5 / var_4) ? var_2 : ((var_6 * (arr_3 [1])))))) ? var_12 : (((arr_4 [i_0 - 3] [i_0 - 1]) ^ var_4))));
            }
        }
    }
    #pragma endscop
}
