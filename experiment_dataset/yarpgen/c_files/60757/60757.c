/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60757
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60757 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60757
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((min(var_3, (max(14395482671684706209, 179)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    arr_9 [11] = (((~var_4) ? 469762048 : ((((((arr_8 [i_2] [i_0]) ? 14395482671684706203 : var_6))) ? (max(65535, 14395482671684706214)) : (((max((arr_0 [i_2]), var_11))))))));
                    arr_10 [i_2] = 247;
                    arr_11 [i_0] [i_1] [i_2] |= (max(((1 ? (arr_6 [i_0] [i_1] [i_0]) : 1)), var_3));
                }
            }
        }
    }
    #pragma endscop
}
