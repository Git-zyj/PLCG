/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217983
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217983 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217983
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 -= ((/* implicit */unsigned char) var_1);
    /* LoopSeq 1 */
    for (unsigned short i_0 = 1; i_0 < 15; i_0 += 2) 
    {
        arr_2 [i_0] = ((/* implicit */long long int) (-(((/* implicit */int) (_Bool)1))));
        var_14 = ((((/* implicit */_Bool) min((arr_0 [i_0] [i_0 - 1]), (arr_0 [i_0 - 1] [i_0 + 2])))) ? (max((5441820636050437377ULL), (((/* implicit */unsigned long long int) (_Bool)1)))) : (((/* implicit */unsigned long long int) ((/* implicit */int) max((((/* implicit */unsigned short) (_Bool)0)), (arr_0 [i_0 + 2] [i_0 - 1]))))));
    }
}
