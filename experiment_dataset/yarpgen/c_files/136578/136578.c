/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136578
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136578 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136578
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_13 = ((var_12 ? (min(var_11, (arr_3 [i_0] [i_0] [i_1]))) : (arr_5 [i_1])));
                arr_6 [i_0] [3] [15] = ((~(((!(arr_2 [i_0] [3] [i_1]))))));
                var_14 &= (((min((max(var_9, var_9)), (arr_5 [8]))) / ((max(17082, 2761973674)))));
                arr_7 [i_0] = (18446744073709551615 / 4611686018427387903);
            }
        }
    }
    var_15 = (max(var_15, var_0));
    #pragma endscop
}
