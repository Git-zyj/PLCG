/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208085
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208085 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208085
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_3;
    var_21 = (min(var_21, var_11));
    var_22 = (((((var_18 ? var_8 : var_11))) ? var_10 : var_11));
    var_23 = ((var_0 ? var_14 : (((((var_0 ? var_18 : var_0))) ? var_18 : var_17))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            var_24 = (((arr_0 [i_0]) ? (arr_0 [i_1]) : (arr_1 [18] [i_1])));
            var_25 = var_9;
            arr_6 [i_0] [i_0] = (arr_4 [i_0] [i_0]);
            var_26 = (-57263157 ? -30747 : 1);
        }
        for (int i_2 = 3; i_2 < 23;i_2 += 1)
        {
            var_27 = (((arr_3 [i_2 - 3]) ? var_17 : (arr_5 [19] [i_2] [i_2 + 1])));
            arr_11 [i_0] [i_2] = var_3;
        }
        arr_12 [i_0] = var_15;
        arr_13 [i_0] = (arr_3 [i_0]);
    }
    #pragma endscop
}
