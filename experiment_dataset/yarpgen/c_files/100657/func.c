/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100657
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100657 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100657
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */long long int) min((var_11), (max((((((/* implicit */long long int) ((/* implicit */int) var_10))) + (var_8))), (var_3)))));
    /* LoopNest 3 */
    for (long long int i_0 = 2; i_0 < 10; i_0 += 1) 
    {
        for (long long int i_1 = 1; i_1 < 10; i_1 += 2) 
        {
            for (long long int i_2 = 3; i_2 < 11; i_2 += 3) 
            {
                {
                    arr_8 [i_0] [i_1] [i_2] = ((/* implicit */signed char) (+(((((/* implicit */int) var_2)) | (((/* implicit */int) (signed char)-1))))));
                    var_12 = ((/* implicit */long long int) max((var_12), (((/* implicit */long long int) (signed char)63))));
                }
            } 
        } 
    } 
}
