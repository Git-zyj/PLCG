/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 57379
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=57379 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/57379
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((-var_10 && var_18) >> 12));
    var_20 = var_3;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] = (min((min((arr_0 [i_0]), ((min(var_4, (arr_3 [i_0] [i_0])))))), var_14));
                var_21 = ((~(((max(var_11, (arr_1 [i_0])))))));
                arr_5 [i_1] [i_0] = (var_7 <= -var_11);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 20;i_3 += 1)
                    {
                        {
                            var_22 = var_2;
                            var_23 = (min(var_23, (((!(((-(arr_2 [i_3 + 1] [i_3 - 2] [1])))))))));
                        }
                    }
                }
                var_24 = 12;
            }
        }
    }
    var_25 = var_11;
    #pragma endscop
}
