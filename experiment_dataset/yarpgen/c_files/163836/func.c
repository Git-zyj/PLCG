/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163836
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163836 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163836
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */short) ((((/* implicit */int) (signed char)90)) ^ (((/* implicit */int) (signed char)90))));
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 1; i_0 < 22; i_0 += 2) 
    {
        for (long long int i_1 = 0; i_1 < 25; i_1 += 1) 
        {
            {
                var_14 = ((/* implicit */short) ((_Bool) var_3));
                var_15 = ((/* implicit */unsigned char) (-(((/* implicit */int) min((arr_0 [i_0 + 1] [i_0 + 3]), (((/* implicit */unsigned short) var_11)))))));
            }
        } 
    } 
}
