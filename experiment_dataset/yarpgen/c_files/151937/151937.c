/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151937
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151937 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151937
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_0] = (((((1 ? 41183 : (arr_4 [i_1])))) ? (arr_3 [i_0]) : -0));
                arr_7 [i_1] = 48444;

                for (int i_2 = 0; i_2 < 19;i_2 += 1)
                {
                    var_15 *= (arr_5 [i_0] [1] [i_0]);
                    arr_12 [i_0] [i_1] [i_1] [i_2] = arr_4 [i_1];
                }
                var_16 = var_7;
                var_17 = (min(var_17, (((((-(arr_10 [i_0] [i_0]))) - (arr_9 [i_0] [i_0] [15]))))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 1; i_3 < 12;i_3 += 1)
    {
        for (int i_4 = 2; i_4 < 14;i_4 += 1)
        {
            {
                var_18 = 0;
                arr_18 [i_3] [i_3] [i_3] = 11817974825452895985;
            }
        }
    }
    var_19 = (!var_11);
    #pragma endscop
}
