/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138773
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138773 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138773
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_1;

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        var_13 = ((-(arr_1 [i_0] [i_0])));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    var_14 = (max((((arr_0 [i_2]) ? -16383 : (((arr_3 [i_1]) ? 18446744073709551615 : -9223372036854775792)))), ((((arr_6 [i_0] [i_1] [i_2]) ? var_6 : (arr_6 [i_0] [i_1] [3]))))));
                    var_15 += (~(((arr_5 [i_0] [i_2]) ? (((arr_5 [i_0] [i_0]) ? var_4 : 1)) : -18446744073709551615)));
                    var_16 *= var_3;
                }
            }
        }
    }
    var_17 ^= var_3;
    var_18 = 65508;
    var_19 = var_4;
    #pragma endscop
}
