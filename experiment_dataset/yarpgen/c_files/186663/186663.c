/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 186663
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=186663 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/186663
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((var_5 ? (var_7 + var_5) : -var_4)) <= var_0));

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        arr_2 [9] = (-38034933716924958 - (max((arr_0 [i_0]), (arr_1 [i_0] [i_0]))));
        arr_3 [i_0] = ((max(((((arr_1 [i_0] [i_0]) && (arr_1 [i_0] [i_0])))), (arr_0 [i_0]))) | (arr_1 [i_0] [i_0]));
    }
    for (int i_1 = 0; i_1 < 10;i_1 += 1)
    {
        arr_8 [i_1] [i_1] = (arr_5 [i_1]);
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 10;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 10;i_3 += 1)
            {
                {
                    arr_15 [i_1] [4] [i_1] = ((+((((arr_12 [i_1]) >= (arr_12 [i_1]))))));
                    arr_16 [i_3] [i_3] [i_1] = ((arr_1 [i_2] [i_1]) | 1);

                    /* vectorizable */
                    for (int i_4 = 2; i_4 < 8;i_4 += 1) /* same iter space */
                    {
                        arr_19 [i_1] [i_1] = ((var_8 < (arr_17 [i_2])));
                        arr_20 [2] [i_2] [i_1] [i_1] = arr_4 [8];
                        arr_21 [i_1] [i_2] [i_1] [i_4] [i_4 + 2] = (arr_13 [i_4 + 2] [i_4] [i_4] [i_4]);
                    }
                    /* vectorizable */
                    for (int i_5 = 2; i_5 < 8;i_5 += 1) /* same iter space */
                    {
                        arr_25 [i_1] [i_1] = (arr_22 [i_5] [i_5 + 1] [i_5 + 2] [i_5] [i_5] [i_5 - 2]);
                        arr_26 [i_1] = arr_14 [i_2] [i_2] [i_5];
                        arr_27 [i_1] [i_2] = -var_1;
                        arr_28 [i_5] [i_5] [i_5] [i_5 - 2] = (-(arr_10 [i_5] [i_1] [i_5]));
                        arr_29 [i_1] [i_2] [i_3] [i_1] [i_5] = (((arr_13 [i_5 - 1] [i_5 + 1] [i_5 + 2] [i_5 - 1]) == var_5));
                    }
                }
            }
        }
    }
    #pragma endscop
}
