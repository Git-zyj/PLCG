/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133672
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133672 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133672
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 *= ((-(!120)));
    var_12 -= ((((((9450 * -102) || ((min(22044, var_1)))))) & (((!(123 / var_10))))));
    var_13 = ((((-((max(var_2, 99))))) & var_7));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_4 [i_0] = (124 == 1);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 11;i_3 += 1)
                    {
                        {
                            arr_12 [i_0] [i_1] = ((((-105 < (arr_0 [i_3] [i_3 + 3])))));

                            for (int i_4 = 1; i_4 < 11;i_4 += 1)
                            {
                                var_14 = (max(var_14, ((max(var_7, (-109 * 39739))))));
                                arr_15 [i_0] [i_1] [i_4] = (((max(var_1, (-2147483647 - 1))) * (((((min(1, var_7))) * 1)))));
                            }
                            arr_16 [i_0] [i_0] = (-105 > 156);
                        }
                    }
                }
            }
        }
    }
    var_15 = var_6;
    #pragma endscop
}
