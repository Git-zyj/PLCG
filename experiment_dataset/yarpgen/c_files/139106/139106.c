/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 139106
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=139106 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/139106
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_7;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = var_9;
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        var_14 = (max(var_14, (arr_4 [i_0] [i_0])));
                        arr_11 [i_0] [i_0] [i_3] [i_3] = (((arr_7 [i_0]) ? (arr_10 [i_1] [i_3]) : (arr_0 [i_0])));
                        arr_12 [i_3] [i_1] [4] = var_12;
                        var_15 -= ((-1 ? 9223372036854775807 : 10045692835786789961));
                    }
                    var_16 = ((8401051237922761651 ? (arr_4 [i_0] [i_1]) : -1));
                }
            }
        }
    }
    #pragma endscop
}
