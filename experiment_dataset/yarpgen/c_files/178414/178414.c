/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 178414
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=178414 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/178414
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((min(17504469521722194938, var_10))) % (((((var_3 ? var_6 : var_3))) ? (var_4 - var_7) : var_5))));

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_12 = ((var_5 >> (17504469521722194924 - 17504469521722194919)));
        arr_2 [i_0] = ((+((+(((arr_0 [i_0]) ? (arr_1 [1]) : 80))))));
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_13 = var_5;
        var_14 *= (arr_0 [i_1]);
    }
    var_15 = (!var_7);
    #pragma endscop
}
