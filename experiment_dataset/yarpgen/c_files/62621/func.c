/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62621
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62621 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62621
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned int i_1 = 3; i_1 < 21; i_1 += 3) 
        {
            {
                var_11 = ((/* implicit */unsigned long long int) (~(((/* implicit */int) ((_Bool) 562949952372736ULL)))));
                var_12 = ((short) arr_3 [1ULL] [1ULL]);
                var_13 = ((/* implicit */_Bool) (-(((((/* implicit */unsigned int) ((/* implicit */int) arr_2 [i_1 + 3] [i_1 + 3]))) | (var_6)))));
            }
        } 
    } 
    var_14 = ((/* implicit */unsigned long long int) var_5);
    var_15 |= ((/* implicit */unsigned char) var_7);
}
