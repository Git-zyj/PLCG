/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212405
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212405 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212405
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        var_18 = (min(var_18, ((((max((arr_1 [i_0] [i_0 + 1]), (((arr_1 [13] [i_0]) ^ (arr_0 [0]))))) == (((((arr_0 [i_0 + 2]) == (arr_0 [i_0 + 1]))))))))));
        var_19 = (min(var_19, ((max(((-3956 ? 1922344113 : 5674001082614885715)), (arr_1 [i_0 - 1] [i_0 + 2]))))));

        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            var_20 = (min(var_20, (arr_3 [i_1 + 1] [i_1])));
            var_21 = var_14;
        }
    }
    var_22 += ((!(((281474976710655 ? -1165156770884031991 : ((4019955438441904198 ? -3200448077166130877 : 0)))))));
    #pragma endscop
}
