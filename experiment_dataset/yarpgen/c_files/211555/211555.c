/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211555
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211555 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211555
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            {
                arr_4 [i_0] = 536870896;
                arr_5 [i_1 - 2] = (min(8380416, 110));
                arr_6 [i_0 - 2] [i_0 - 2] = (max((((+(((arr_0 [i_1]) ? 40634 : 65))))), (max((arr_2 [i_0 - 4] [i_0] [i_0]), 3251410124))));
                arr_7 [i_0 + 1] [i_0 + 2] [i_1] = ((min(-95, (arr_0 [i_1 - 1]))));
            }
        }
    }
    var_11 = 780205336820073877;
    var_12 = ((~(max(((min(59, 65))), (59 << var_10)))));
    #pragma endscop
}
