/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62472
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62472 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62472
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 11; i_0 += 1) 
    {
        arr_4 [i_0] = ((/* implicit */unsigned long long int) max(((+(((/* implicit */int) var_8)))), (((/* implicit */int) (!(((/* implicit */_Bool) var_16)))))));
        arr_5 [i_0] |= ((/* implicit */unsigned short) (+(((/* implicit */int) var_6))));
    }
    var_17 = ((/* implicit */unsigned char) ((max(((-(var_2))), (((/* implicit */int) max((((/* implicit */short) var_7)), (var_9)))))) << (((((((/* implicit */int) var_9)) & (((/* implicit */int) var_11)))) ^ (((/* implicit */int) var_10))))));
}
