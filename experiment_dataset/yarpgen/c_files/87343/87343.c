/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87343
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87343 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87343
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_16 -= (((!-var_12) ? ((((arr_0 [i_0 - 1] [i_1]) ? (arr_0 [i_0] [i_1]) : (arr_0 [i_0 - 1] [i_1])))) : (4398046510976 - var_15)));
                arr_4 [i_1] = (((((~(var_2 | var_4)))) >= ((109 ? (-9223372036854775807 - 1) : ((var_15 % (arr_1 [i_1])))))));
                var_17 = (((8596138300843895229 ? 536870656 : -11)));
            }
        }
    }

    /* vectorizable */
    for (int i_2 = 3; i_2 < 15;i_2 += 1)
    {
        var_18 -= (~var_9);
        var_19 ^= 23012;
    }
    #pragma endscop
}
