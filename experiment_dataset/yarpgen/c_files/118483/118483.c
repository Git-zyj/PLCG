/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 118483
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=118483 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/118483
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 13;i_0 += 1) /* same iter space */
    {
        var_19 *= 1;
        var_20 |= ((((((((95 ? 66584576 : 11))) ? (arr_0 [i_0 + 2]) : 1))) >= (((arr_0 [i_0]) ? (((arr_0 [i_0]) ? var_14 : var_13)) : -20))));
    }
    for (int i_1 = 1; i_1 < 13;i_1 += 1) /* same iter space */
    {
        var_21 = (max(var_21, (((((var_13 ? (arr_3 [i_1 - 1]) : ((((arr_5 [i_1] [i_1]) ? -5 : 35384))))) < (arr_1 [i_1 + 1]))))));
        var_22 = var_16;
    }
    var_23 = ((-(min(((-1 ? var_3 : var_16)), ((-113 ? var_0 : var_16))))));
    var_24 = var_16;
    var_25 = 0;
    #pragma endscop
}
