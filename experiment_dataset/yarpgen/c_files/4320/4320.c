/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4320
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4320 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4320
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 &= var_15;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    arr_9 [i_2] |= (((~var_10) & (max(var_6, var_17))));
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 19;i_4 += 1)
                        {
                            {
                                var_20 = ((((min(var_7, var_18))) ? var_3 : -var_4));
                                var_21 = var_12;
                                var_22 = var_5;
                                var_23 &= var_16;
                                var_24 = (min(var_24, ((var_10 ? var_17 : (var_13 <= var_2)))));
                            }
                        }
                    }
                    arr_17 [i_0] [i_1] [i_2] [i_2] = ((~(~var_5)));
                    var_25 = (min(var_25, (((!((((42855 > -57) ? var_4 : var_10))))))));
                    arr_18 [i_2] = var_16;
                }
            }
        }
    }
    #pragma endscop
}
