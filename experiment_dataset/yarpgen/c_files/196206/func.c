/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196206
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196206 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196206
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 ^= ((/* implicit */_Bool) (+(((/* implicit */int) var_6))));
    var_13 = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) max((var_3), (((/* implicit */unsigned int) var_10))))) ? (((/* implicit */int) var_6)) : (((/* implicit */int) var_11))));
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned short i_1 = 1; i_1 < 19; i_1 += 2) 
        {
            {
                var_14 |= ((/* implicit */unsigned short) var_7);
                arr_5 [i_0] [14U] |= ((/* implicit */_Bool) ((((/* implicit */_Bool) (signed char)1)) ? (3866536717633274065LL) : (-1LL)));
            }
        } 
    } 
}
