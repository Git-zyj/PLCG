/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 201593
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=201593 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/201593
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 18;i_2 += 1)
            {
                {
                    arr_8 [i_1] [i_0] = (((arr_6 [4] [i_1] [1] [i_2 + 1]) ^ (((max((arr_6 [i_0] [i_0] [i_0] [i_0]), var_9)) % (((max(var_5, (arr_2 [i_2])))))))));
                    arr_9 [i_0] [16] &= (arr_2 [i_1]);
                }
            }
        }
    }
    var_10 = (-380888495 != var_8);
    var_11 += (((max(var_9, (31050 ^ var_6))) >> (-var_2 - 2038483013)));
    #pragma endscop
}
