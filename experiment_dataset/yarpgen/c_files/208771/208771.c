/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208771
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208771 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208771
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 19;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_2] [i_2] = (((((((min(var_8, 4160749568))) ? (!var_9) : ((19 ? (arr_4 [i_1]) : var_10))))) ? (arr_4 [i_2 + 2]) : ((-(((arr_3 [i_0] [i_1]) - var_12))))));
                    var_14 = (var_1 - -7887);
                }
            }
        }
    }
    var_15 = ((var_12 ? var_6 : (((max(var_0, 65503))))));
    var_16 = ((var_11 ? var_10 : ((((var_5 > (min(var_12, var_1))))))));
    var_17 = var_11;
    #pragma endscop
}
