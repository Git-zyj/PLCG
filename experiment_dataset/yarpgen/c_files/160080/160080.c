/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160080
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160080 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160080
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_20 += (288212783965667328 < 0);
                arr_5 [1] [i_1] |= (max(131071, 32036));
                arr_6 [i_0] [i_0] [6] |= ((((min(0, var_6))) * ((((arr_1 [i_0 + 1]) ? (arr_2 [i_0 + 1]) : (arr_4 [i_0 - 1] [i_0 - 3]))))));
            }
        }
    }
    var_21 = ((min(var_4, (min(var_13, var_3)))));
    #pragma endscop
}
