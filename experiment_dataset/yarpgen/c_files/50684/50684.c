/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 50684
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=50684 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/50684
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 -= ((!(123 >= var_11)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 19;i_2 += 1)
            {
                {
                    var_21 ^= ((+(((arr_2 [i_2 + 1]) * var_16))));
                    arr_8 [i_0] [i_0] [i_1] [i_0] = 510540087;
                    var_22 |= ((((!(~-1651841117))) ? (min((((arr_4 [i_0] [i_1] [i_2]) / var_13)), (!var_19))) : ((max(((60 | (arr_6 [i_0] [18]))), (arr_5 [i_2 - 1] [i_2 - 1] [i_2]))))));
                    var_23 = ((!((!((min((arr_5 [i_2 - 1] [i_2] [i_2]), 135)))))));
                    var_24 = var_3;
                }
            }
        }
    }
    #pragma endscop
}
