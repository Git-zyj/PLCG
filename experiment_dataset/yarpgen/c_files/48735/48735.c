/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48735
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48735 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48735
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 -= var_6;
    var_18 = (max(var_18, ((max((min(var_11, var_0)), (((~(-127 <= var_6)))))))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_19 = (((arr_3 [i_0 + 1] [15] [i_1]) * var_13));
                arr_6 [i_1] = (min(var_13, (arr_3 [i_0] [i_0] [i_0 - 3])));
                var_20 = 841666936;
                arr_7 [i_0] [i_1] [15] = ((((min(((1 ? 96 : 11)), (~17)))) - var_4));
            }
        }
    }
    #pragma endscop
}
