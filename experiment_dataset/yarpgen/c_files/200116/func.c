/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200116
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200116 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200116
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */long long int) var_3);
    var_14 = var_5;
    /* LoopNest 2 */
    for (unsigned char i_0 = 0; i_0 < 13; i_0 += 1) 
    {
        for (int i_1 = 0; i_1 < 13; i_1 += 2) 
        {
            {
                var_15 = min((min((arr_1 [i_0] [i_0]), (arr_1 [i_0] [i_0]))), (max((((/* implicit */long long int) var_3)), (arr_1 [i_0] [i_0]))));
                var_16 = ((/* implicit */int) 1377839048U);
            }
        } 
    } 
    var_17 = ((((/* implicit */_Bool) ((max((var_6), (((/* implicit */int) var_3)))) >> (((var_11) + (5164136510729899184LL)))))) ? (((((/* implicit */_Bool) max((7302983590978478727LL), (var_2)))) ? (134184960) : (var_6))) : (var_10));
    var_18 = ((/* implicit */unsigned int) var_5);
}
