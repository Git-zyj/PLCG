/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 189219
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=189219 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/189219
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = var_7;
    /* LoopSeq 1 */
    for (unsigned char i_0 = 2; i_0 < 22; i_0 += 1) 
    {
        var_20 = ((/* implicit */unsigned long long int) ((signed char) min((((/* implicit */int) var_6)), ((~(((/* implicit */int) (short)7936)))))));
        arr_2 [i_0] [i_0] = ((/* implicit */long long int) (-(((/* implicit */int) var_8))));
    }
    var_21 &= ((/* implicit */short) var_2);
    var_22 = var_2;
}
