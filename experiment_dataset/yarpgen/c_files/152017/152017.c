/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152017
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152017 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152017
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_14 = (arr_3 [i_1] [i_1] [i_0]);
                arr_5 [i_1] [i_1] [i_0] = arr_2 [i_0] [i_1];
                /* LoopNest 3 */
                for (int i_2 = 3; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 18;i_4 += 1)
                        {
                            {
                                arr_15 [i_4] [i_3] [i_2] [i_0] [i_0] = (arr_3 [i_1] [i_2] [i_1]);
                                arr_16 [i_0] [i_1] [i_2 - 1] [i_3] [i_4] |= (((arr_7 [i_0] [i_2] [i_0] [10]) < ((((!(!1145372445)))))));
                                var_15 = ((((-((var_12 ? (arr_11 [i_0] [i_0] [i_1] [i_0]) : 6646)))) - (arr_1 [i_1])));
                            }
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_5 = 1; i_5 < 15;i_5 += 1)
                {
                    for (int i_6 = 1; i_6 < 16;i_6 += 1)
                    {
                        {
                            var_16 |= 27762;
                            arr_22 [i_6] [9] = arr_13 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0];
                        }
                    }
                }
                var_17 = (arr_6 [i_1]);
            }
        }
    }
    var_18 = ((((27762 << (((min(65535, 4294967295)) - 65526)))) < (!127)));
    var_19 = (!var_8);
    var_20 = 4294967295;
    #pragma endscop
}
