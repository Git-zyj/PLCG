/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236879
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236879 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236879
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, (!var_17)));
    var_20 = (max(var_20, var_14));
    var_21 -= ((((var_2 > (max(4473522747268265610, 4294967295)))) ? var_6 : ((max(((0 ? 0 : 143)), var_5)))));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_1] [i_0] = 109;
                    var_22 = var_9;
                    var_23 = (max(var_23, (((var_12 ? ((min(0, 192))) : (max(((var_11 / (arr_0 [i_2] [i_0]))), var_7)))))));
                    var_24 = (min(var_24, ((((((arr_0 [i_0] [i_0 - 2]) > (arr_3 [i_0] [i_0 + 1] [i_1 - 2]))) ? (arr_3 [i_0] [i_0 - 2] [i_1 - 1]) : ((var_12 + (arr_0 [i_0 - 1] [i_0 - 1]))))))));
                }
            }
        }
    }
    #pragma endscop
}
