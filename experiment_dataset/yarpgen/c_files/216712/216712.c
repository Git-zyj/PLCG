/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 216712
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=216712 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/216712
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_15 = var_6;
                var_16 = (var_3 ? (arr_2 [i_1]) : (~1));
                var_17 *= 0;
                arr_4 [i_1] [i_1] = (((((1 ? 1 : ((var_14 ? var_10 : -604000036))))) ? (arr_1 [i_0]) : (((arr_3 [i_1] [i_1]) ? 26532 : (arr_0 [i_1])))));
            }
        }
    }
    var_18 = (max(var_18, ((max(var_9, var_4)))));
    var_19 ^= (min(1, 4177920));
    var_20 = ((~((((~var_12) > var_13)))));
    #pragma endscop
}
