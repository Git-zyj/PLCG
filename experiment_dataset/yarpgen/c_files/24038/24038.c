/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24038
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24038 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24038
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((((((((min(var_14, var_14))) ? var_10 : (((-127 - 1) / 255)))) + 2147483647)) << (((-67 | 1) - 1))));

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        var_17 = (((((((arr_0 [i_0]) ^ var_5))) | ((var_9 ? (arr_0 [i_0]) : (arr_3 [i_0] [i_0]))))));
        var_18 += ((var_2 && (1 - 23208)));
        var_19 = (((arr_1 [i_0]) * (((!((min(var_14, var_2))))))));
        var_20 = var_0;
        var_21 ^= (arr_3 [i_0] [i_0]);
    }
    #pragma endscop
}
