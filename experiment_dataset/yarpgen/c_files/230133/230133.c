/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230133
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230133 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230133
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] = -3229099059969088710;
                var_16 = ((((max(var_1, (arr_2 [i_0] [i_1] [i_1])))) + (arr_2 [3] [8] [i_1])));
                var_17 = (max(var_17, ((((((var_4 + (arr_2 [i_0] [i_1] [i_1])))) ? (max((arr_0 [i_0]), (arr_0 [i_0]))) : (((arr_0 [i_0]) << (8117454174169887163 - 8117454174169887140))))))));
                var_18 = ((max(101, 5964550036734912)));
            }
        }
    }
    var_19 = (max(var_19, -var_8));
    #pragma endscop
}
