/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 178788
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=178788 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/178788
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((-((-var_11 ? 0 : 2252470881))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            var_21 = (arr_2 [i_1]);
            var_22 += (arr_3 [i_1] [i_1]);
        }
        for (int i_2 = 0; i_2 < 11;i_2 += 1)
        {
            var_23 = ((var_8 ? var_14 : (arr_6 [i_0] [i_0])));
            var_24 &= ((~((var_7 ? 1 : 2380602312))));
        }
        var_25 = (!var_15);
        var_26 = ((~var_2) ? var_6 : var_13);
    }
    var_27 = max(var_10, (max((245 <= var_1), (max(var_9, var_16)))));
    #pragma endscop
}
