/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168951
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168951 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168951
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_11 = ((255 < (arr_5 [i_0 + 1] [i_0] [i_1])));
                var_12 = ((75 - (max(65535, (arr_0 [i_0])))));
                /* LoopNest 3 */
                for (int i_2 = 1; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 16;i_4 += 1)
                        {
                            {
                                arr_13 [i_0] [i_0] [i_2] [i_2] [i_4] = (~-15);
                                var_13 -= (((arr_12 [i_3] [i_3 - 2] [i_2] [i_3] [i_3 - 2] [i_4 + 1] [i_0 + 1]) | (var_6 < var_2)));
                            }
                        }
                    }
                }
            }
        }
    }
    var_14 = ((var_0 ? (((!var_6) ? ((75 << (29548 - 29536))) : 219)) : var_8));
    #pragma endscop
}
