/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158182
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158182 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158182
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, (~var_9)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_20 = ((((((((var_11 - (arr_4 [i_0] [i_0] [i_0])))) ? (((!(arr_3 [i_0] [i_0] [i_0])))) : (var_10 - 16384)))) || -109));
                var_21 = max((((arr_1 [i_0]) ? (arr_1 [i_0]) : (arr_2 [i_0]))), (arr_2 [i_0]));
                arr_5 [i_0] = ((((min(var_18, (arr_3 [i_0] [i_0] [0])))) >> 0));
            }
        }
    }
    var_22 = var_5;
    #pragma endscop
}
