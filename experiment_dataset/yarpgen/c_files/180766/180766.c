/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180766
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180766 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180766
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = 8480;
    var_17 = -1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_18 *= var_11;
                arr_4 [i_0] [i_1] [i_1] = ((!(((-(~51813))))));
                /* LoopNest 2 */
                for (int i_2 = 4; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        {
                            arr_12 [i_0] [i_0] [i_0] [i_1] [i_0] [i_1] = ((((max((arr_9 [i_0] [i_0] [i_1] [i_1] [i_2 - 4] [i_2 + 4]), 1))) + 13730));
                            arr_13 [i_0] = (((arr_10 [i_0] [i_0] [i_1] [i_2] [i_3]) > (((!(arr_11 [i_2 - 1] [i_2 + 4] [i_2 - 1] [i_2 - 3] [i_2 - 2] [i_2 - 4]))))));
                            var_19 = ((!(arr_3 [i_0] [i_2 - 4])));
                        }
                    }
                }
                arr_14 [i_0] [i_0] [i_1] |= var_8;
            }
        }
    }
    var_20 = (0 / var_14);
    #pragma endscop
}
