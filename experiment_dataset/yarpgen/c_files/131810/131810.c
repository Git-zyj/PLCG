/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131810
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131810 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131810
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 23;i_4 += 1)
                        {
                            {
                                arr_15 [i_0] [i_1] [i_2] = var_9;
                                arr_16 [i_0] [i_0] [i_0] = (min(var_1, (min(8992, 9223372036854775807))));
                            }
                        }
                    }

                    for (int i_5 = 1; i_5 < 22;i_5 += 1)
                    {
                        var_11 &= ((((max(var_4, var_6))) ? (min(var_1, var_0)) : ((min(var_10, var_9)))));
                        arr_19 [i_0] [i_5] = var_3;
                    }
                }
            }
        }
    }
    var_12 = var_5;
    #pragma endscop
}
