/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47040
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47040 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47040
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 ^= var_13;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        arr_9 [i_0] [i_1] [i_3] [i_3] |= -var_10;
                        arr_10 [i_0] [i_1] [i_2] [i_3] |= var_6;
                        var_16 |= var_8;
                    }
                    /* vectorizable */
                    for (int i_4 = 1; i_4 < 19;i_4 += 1)
                    {
                        var_17 = ((-(arr_0 [i_0 + 3] [i_1])));
                        var_18 = (arr_5 [i_4 - 1] [i_0 + 3] [i_0 + 3] [i_0 + 3]);
                        var_19 = (arr_11 [i_0] [i_1] [i_2] [i_2]);
                    }
                    /* vectorizable */
                    for (int i_5 = 0; i_5 < 21;i_5 += 1)
                    {
                        var_20 = ((((-(arr_2 [i_5]))) <= (arr_14 [i_0] [i_2] [i_5])));
                        arr_16 [i_0] [i_0] [i_0] [i_0] |= (!(!var_0));
                    }
                    var_21 = var_7;
                }
            }
        }
    }
    #pragma endscop
}
