/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229187
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229187 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229187
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((-((var_14 ? (var_11 || var_6) : (var_5 || var_6)))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_0] [i_0] = (((((var_14 >= ((3988274921330600419 ? var_12 : 12))))) >= (arr_1 [i_1 + 1] [i_2])));
                    var_17 = ((((((arr_0 [i_0 - 1]) | 1))) & (max((arr_5 [i_0 - 1]), (arr_0 [i_0 - 1])))));
                    var_18 = (min(var_18, (arr_2 [i_0] [i_1])));
                }
            }
        }
    }
    #pragma endscop
}
