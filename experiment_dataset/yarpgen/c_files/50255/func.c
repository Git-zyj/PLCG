/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 50255
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=50255 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/50255
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned int) (~(((/* implicit */int) var_10))));
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 3; i_1 < 11; i_1 += 4) 
        {
            for (unsigned int i_2 = 1; i_2 < 12; i_2 += 4) 
            {
                {
                    var_21 = ((/* implicit */unsigned char) min((var_21), (((/* implicit */unsigned char) ((2147483647) / (((/* implicit */int) (signed char)111)))))));
                    var_22 = ((/* implicit */long long int) ((int) (~(arr_6 [i_2 + 1] [i_2 + 1] [i_2 + 1]))));
                }
            } 
        } 
    } 
}
