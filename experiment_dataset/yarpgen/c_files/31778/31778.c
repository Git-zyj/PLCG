/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 31778
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=31778 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/31778
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((~(max(var_6, var_10))));
    var_13 = (min(var_10, var_8));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 16;i_1 += 1)
        {
            {
                arr_4 [6] [i_1] [i_0] = (min((max((arr_2 [i_0 - 1] [i_1 - 1]), (arr_3 [i_0 + 1]))), ((min(var_5, var_5)))));
                var_14 = ((!((max(1, ((59 ? var_1 : var_6)))))));
                arr_5 [i_1] [i_1 + 1] [i_0] = ((var_2 ? (min(((min((arr_3 [i_0]), var_11))), ((var_2 ? var_10 : (arr_2 [i_0] [i_1]))))) : (min(((max((arr_1 [i_0]), (arr_3 [i_0])))), (max(var_1, var_0))))));
            }
        }
    }
    #pragma endscop
}
