/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98897
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98897 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98897
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(3453799884, 41002967));
    var_12 ^= ((((max(var_10, 4233353399))) == ((max((max(268435328, var_8)), ((var_6 ? var_8 : 41002966)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_13 = (((3434178842 / 4294967295) ? (arr_1 [i_0]) : 1591372922));
                var_14 = -88;
            }
        }
    }
    #pragma endscop
}
