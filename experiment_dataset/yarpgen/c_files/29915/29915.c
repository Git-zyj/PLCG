/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29915
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29915 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29915
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_11, (~var_9)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    arr_9 [i_0] = (max(((~((179 ? var_0 : 179)))), (~1241899529)));
                    arr_10 [i_0] [i_0] = (var_2 - var_0);
                    arr_11 [i_0] [i_1] [i_0] = (((arr_8 [i_0] [i_1] [i_0] [i_0]) ? (max(((22275 ? 179 : var_10)), ((-97151513 ? (arr_5 [i_1] [i_1]) : (arr_3 [i_2] [i_1]))))) : var_9));

                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        var_12 &= var_8;
                        arr_14 [i_0] [i_1] [i_2] [i_2] [i_3] [i_2] = var_6;
                    }
                    var_13 = var_2;
                }
            }
        }
    }
    #pragma endscop
}
