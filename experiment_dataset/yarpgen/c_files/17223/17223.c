/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 17223
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=17223 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/17223
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(var_13, (min(var_6, (((254022711 ? var_14 : var_17)))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 15;i_2 += 1)
            {
                {
                    arr_10 [i_0] [11] = (~101);
                    arr_11 [i_0] [i_0] [i_2] [11] = (max((~1986841594), var_0));
                    arr_12 [i_0] [i_0] = (max(((((!var_9) & (arr_6 [i_0] [i_1] [i_2])))), (((~var_7) | ((var_6 ? var_14 : (arr_1 [14] [i_1])))))));
                    arr_13 [15] [i_0] [i_2] [15] = 4250561951;
                }
            }
        }
    }
    #pragma endscop
}
