/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 243689
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=243689 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/243689
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_16;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = (((((((min((arr_0 [i_1]), (arr_1 [i_0]))) < (arr_2 [i_1]))))) % (arr_1 [i_1])));
                arr_7 [i_0] = (max((((((min(16383, -116))) || (((101 & (arr_3 [i_0] [0]))))))), (arr_2 [i_1])));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 14;i_3 += 1)
                    {
                        {
                            var_21 = arr_11 [i_0] [i_0] [i_0] [i_2] [i_3];
                            arr_13 [i_0] [i_0] [i_2] [i_2] = (arr_10 [i_1] [i_2] [i_2]);
                            var_22 = (arr_3 [i_0 - 1] [i_0 - 1]);
                            arr_14 [i_1] [i_1] [i_1] [i_3 - 1] [1] [1] = (max(1679762219, (((116 >= ((min(16, (arr_3 [i_0 - 1] [i_0 - 1])))))))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
