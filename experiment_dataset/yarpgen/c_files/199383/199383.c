/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199383
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199383 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199383
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_4 [i_0] = ((12 ? 1 : -342055456));
                var_18 += ((!(((-(arr_1 [i_1]))))));
            }
        }
    }

    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        var_19 *= (((arr_8 [i_2]) ? (((arr_6 [i_2]) ? (arr_6 [i_2]) : (arr_6 [i_2]))) : (((((arr_6 [i_2]) < (arr_5 [i_2] [i_2])))))));
        arr_9 [i_2] [i_2] = ((((((arr_7 [i_2] [i_2]) && (((arr_5 [i_2] [i_2]) == (arr_7 [i_2] [i_2])))))) << ((((224 ? 24837 : 242026009)) - 24830))));
        var_20 = (((((1 ? 1 : -731))) >= (min((arr_6 [i_2]), (arr_6 [i_2])))));
    }
    #pragma endscop
}
