/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 142658
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=142658 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/142658
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 += ((((-(min(var_1, var_8)))) != -var_4));
    var_19 = (max(((((min(var_15, var_10))) ? var_11 : (((185 ? -12 : 0))))), (((!(((var_17 ? var_0 : 4294967295))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] = (15032 && 0);
                var_20 = (((var_0 + 9223372036854775807) >> (((min(((var_2 ? var_10 : var_2)), 33708)) - 33708))));
            }
        }
    }
    var_21 = ((-(((4294967290 > (10 == 0))))));
    #pragma endscop
}
