/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164479
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164479 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164479
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_10 = (max(var_10, ((max((((arr_6 [i_0] [i_0]) ? -53078891674968164 : (arr_2 [i_0]))), ((var_6 + (arr_6 [i_0] [i_0]))))))));
                var_11 = (min(var_11, ((((-var_4 ? (((-127 - 1) ? 1858389269 : var_2)) : (arr_5 [12] [i_1]))) / var_0))));
            }
        }
    }
    var_12 = (max(var_12, (((((((((max(3310, 54629)))) & ((var_5 ? var_4 : 0))))) ^ var_0)))));
    #pragma endscop
}
