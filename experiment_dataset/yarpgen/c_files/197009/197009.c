/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197009
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197009 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197009
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 23;i_2 += 1)
            {
                {
                    arr_7 [i_2 + 1] [i_1] = (max((15 ^ var_0), ((min((arr_0 [i_2] [i_0 + 1]), (arr_6 [i_2] [i_2 + 2] [i_2 - 1]))))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 24;i_4 += 1)
                        {
                            {
                                var_11 = (max(var_11, (arr_6 [i_2 + 1] [i_2 + 1] [i_4])));
                                arr_15 [i_0 - 2] [23] [23] [i_2] [i_2] [i_4] [i_4] = (min(26, 15));
                            }
                        }
                    }
                }
            }
        }
    }
    var_12 = ((var_8 ? (~var_4) : (0 ^ var_9)));
    var_13 = (max(var_13, (-23913 || 1)));
    #pragma endscop
}
