/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111202
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111202 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111202
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 += var_0;
    var_12 = var_1;

    /* vectorizable */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        var_13 = ((arr_1 [i_0]) % (arr_1 [i_0]));
        var_14 = (max(var_14, ((((arr_0 [i_0 + 2]) >= var_6)))));
        var_15 = (arr_0 [i_0 - 1]);
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 15;i_1 += 1)
    {
        arr_4 [i_1] = (((var_0 ? var_10 : var_1)));
        var_16 = (arr_1 [i_1]);
        arr_5 [i_1] = (((((arr_0 [i_1]) ? var_7 : (arr_3 [i_1] [i_1]))) != var_0));

        for (int i_2 = 1; i_2 < 14;i_2 += 1)
        {
            /* LoopNest 2 */
            for (int i_3 = 4; i_3 < 12;i_3 += 1)
            {
                for (int i_4 = 1; i_4 < 14;i_4 += 1)
                {
                    {
                        var_17 = (arr_6 [i_2 - 1] [i_2] [i_2]);
                        var_18 += 20983;
                    }
                }
            }
            var_19 &= 20987;
        }
    }
    /* vectorizable */
    for (int i_5 = 0; i_5 < 13;i_5 += 1)
    {
        var_20 = (arr_0 [15]);
        var_21 *= (!(arr_2 [i_5]));
        var_22 = (max(var_22, (4895320595727670 != 25298)));
    }
    var_23 = var_2;
    #pragma endscop
}
