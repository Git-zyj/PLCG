/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 134197
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=134197 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/134197
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 -= 438560886;
    var_11 -= var_2;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [i_0 - 1] = (max((max(2098972082, (-32767 - 1))), (min(((min((arr_3 [i_0] [i_0 - 2] [i_0 + 1]), var_1))), ((-(arr_0 [i_0])))))));
                arr_7 [i_0] = (arr_4 [i_0] [i_0]);
                var_12 = ((arr_5 [i_0 + 1] [i_0 - 1] [i_0]) ? 36 : ((~(((arr_3 [i_0 - 2] [i_1] [i_1]) ? (arr_2 [i_0] [i_1]) : (arr_4 [i_0] [i_1]))))));
            }
        }
    }
    #pragma endscop
}
