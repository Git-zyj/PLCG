/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34859
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34859 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34859
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 |= var_0;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 18;i_3 += 1)
                    {
                        {
                            arr_10 [i_0 - 1] [i_0] [i_2] [i_0 - 2] [i_0] = var_2;
                            var_13 = ((~(arr_3 [i_1] [7])));
                        }
                    }
                }
                arr_11 [i_0] = ((((((!var_10) || -var_2))) ^ var_6));
                /* LoopNest 2 */
                for (int i_4 = 0; i_4 < 19;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 19;i_5 += 1)
                    {
                        {
                            var_14 = (max(var_14, ((((arr_1 [i_4]) > (min((arr_0 [i_0 - 2]), (((arr_12 [i_5]) ? var_7 : var_9)))))))));
                            arr_17 [i_0] &= var_11;
                        }
                    }
                }
            }
        }
    }
    var_15 = (var_3 <= ((((!((max(var_3, var_3))))))));
    #pragma endscop
}
