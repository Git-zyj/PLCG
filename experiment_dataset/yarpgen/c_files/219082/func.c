/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219082
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219082 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219082
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */long long int) (((-(((/* implicit */int) max((((/* implicit */unsigned short) var_11)), (var_6)))))) != (((/* implicit */int) var_7))));
    var_19 |= ((/* implicit */unsigned int) var_12);
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned char i_0 = 0; i_0 < 15; i_0 += 2) 
    {
        var_20 -= ((/* implicit */unsigned short) var_13);
        arr_2 [i_0] [i_0] = ((/* implicit */long long int) ((((/* implicit */int) (unsigned short)1023)) > (((/* implicit */int) (unsigned short)1036))));
    }
    var_21 = ((/* implicit */short) var_9);
}
