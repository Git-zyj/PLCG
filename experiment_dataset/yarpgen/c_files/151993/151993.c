/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151993
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151993 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151993
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_1] = ((-(max(var_1, ((max(2080374784, 2080374784)))))));
                arr_6 [6] &= (arr_3 [12]);
                arr_7 [i_1] [i_1] [i_0] = ((82 ? (!-84) : 2147483647));
                arr_8 [i_1] [i_1] = (max((!14617), (arr_0 [i_0] [i_0])));
            }
        }
    }
    var_12 = ((14605 ? (min((min(var_10, -760481856)), (~28))) : var_3));
    #pragma endscop
}
