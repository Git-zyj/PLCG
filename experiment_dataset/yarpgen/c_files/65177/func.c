/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 65177
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=65177 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/65177
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */signed char) ((unsigned long long int) -5714882843518083075LL));
    var_16 = ((/* implicit */signed char) max((var_16), (((/* implicit */signed char) max((((/* implicit */unsigned short) var_5)), ((unsigned short)45263))))));
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (signed char i_1 = 2; i_1 < 22; i_1 += 1) 
        {
            {
                arr_4 [(signed char)4] &= ((/* implicit */_Bool) (signed char)-33);
                var_17 = ((/* implicit */unsigned int) (_Bool)1);
                arr_5 [i_0] = var_4;
            }
        } 
    } 
}
