/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52679
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52679 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52679
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 19;i_0 += 1) /* same iter space */
    {
        var_10 = ((((-(62750 ^ -44636741))) >= -31));
        var_11 ^= (arr_1 [7] [7]);
    }
    for (int i_1 = 0; i_1 < 19;i_1 += 1) /* same iter space */
    {
        /* LoopNest 3 */
        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 19;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 19;i_4 += 1)
                {
                    {
                        var_12 = ((1891264725 ? (arr_2 [i_1]) : 62750));
                        arr_12 [i_1] [i_3] [i_2] [i_2] [i_1] |= (+-6287456421824420448);
                    }
                }
            }
        }
        arr_13 [i_1] |= (((255 ? -1213175755977784236 : 62750)));
        var_13 = 255;
        var_14 = ((~((max((arr_1 [i_1] [i_1]), (arr_1 [i_1] [i_1]))))));
    }
    var_15 = (max(var_4, var_8));
    #pragma endscop
}
