/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40221
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40221 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40221
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        {
                            var_19 = (arr_6 [i_0] [i_0] [i_2] [i_2]);
                            arr_8 [i_0] [i_0] [i_0] [i_3] [i_0] = (((((((!(arr_3 [i_0] [i_0])))) != 65535)) ? (max((arr_4 [i_0] [i_1] [i_1] [i_3]), ((max(var_15, var_0))))) : 32396));
                        }
                    }
                }
                var_20 *= (max(((max((arr_1 [i_0]), 103))), var_0));
                var_21 = (min(var_21, 3469132744837035071));
            }
        }
    }
    #pragma endscop
}
