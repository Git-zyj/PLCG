/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110371
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110371 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110371
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = -var_3;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = (arr_5 [i_0] [i_0] [i_1]);
                arr_7 [i_0] [i_0] |= (((((-(!33554431)))) % ((((!(arr_4 [i_1]))) ? -33554436 : ((((arr_0 [i_0]) ? (arr_3 [i_1] [i_0]) : (arr_3 [i_0] [i_0]))))))));
            }
        }
    }
    var_15 += var_1;
    var_16 = (((((var_2 ? 3336369886 : var_12))) ? (((min(var_12, var_3)))) : var_8));
    #pragma endscop
}
