/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103491
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103491 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103491
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 |= var_4;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 20;i_1 += 1)
        {
            {
                var_18 = var_2;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        {
                            var_19 = (((82 >= 1) ? ((min((arr_6 [9] [i_0] [i_2]), ((var_16 ? var_6 : 1))))) : ((1099509530624 ? ((((arr_3 [i_0] [i_0] [i_0]) ? (arr_8 [i_0] [i_0] [i_0] [i_0] [i_0]) : -18368))) : var_15))));
                            var_20 *= 1;
                        }
                    }
                }
            }
        }
    }
    var_21 = (max((min(0, -7117920834053478323)), 1099509530624));
    #pragma endscop
}
