/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77154
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77154 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77154
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    var_13 = (max((arr_4 [i_0] [i_0]), (max((((~(arr_1 [i_0])))), (~-1)))));
                    arr_8 [i_0] [i_0] [i_2] = ((((~(~var_0))) * (((var_12 + ((((var_0 || (arr_5 [i_0] [i_1] [17]))))))))));
                    var_14 = (max(var_0, ((((~var_9) != (arr_1 [i_0]))))));
                    var_15 |= ((max(-var_7, (arr_1 [i_0]))));
                }
            }
        }
    }
    var_16 &= (max(var_1, (((((min(var_0, var_4))) ? var_10 : var_9)))));
    #pragma endscop
}
