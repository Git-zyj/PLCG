/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 16881
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=16881 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/16881
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(var_13, var_10));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_13 [i_1] [i_1] [i_2] [i_3] [i_2] = (max((arr_2 [i_1] [i_1]), (arr_8 [i_0] [i_3] [14] [i_1])));
                                arr_14 [i_0] [i_1] [i_1] [i_3] [i_1] = var_8;
                                var_19 = (min(((((((var_9 ? (arr_9 [i_2] [i_2] [i_2] [1] [i_1] [i_2]) : var_13))) >= var_11))), 234));
                            }
                        }
                    }
                    var_20 *= (min(4095, var_17));
                }
            }
        }
    }
    var_21 = var_6;
    #pragma endscop
}
