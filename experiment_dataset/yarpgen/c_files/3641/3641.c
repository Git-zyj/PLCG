/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3641
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3641 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3641
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(var_15, (((((var_1 ? (-2 ^ var_9) : (125 == var_12))) / ((((max(var_13, var_11)) <= -2))))))));
    var_16 = (((13408 << 1) != (((((-1 ? var_12 : var_3))) | (max(var_8, 13388))))));
    var_17 = ((!((((var_10 + 2147483647) << (var_1 / 2))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_18 = (min(var_18, ((((arr_1 [i_0]) ? (min(((((arr_0 [i_0]) || var_6))), (((arr_3 [i_0] [i_1]) ? 1 : 4294967288)))) : var_3)))));
                var_19 ^= var_9;
                arr_6 [i_0] = ((!(arr_3 [2] [0])));
                var_20 = -1617352446;
            }
        }
    }
    var_21 = (min(var_21, var_0));
    #pragma endscop
}
