/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194064
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194064 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194064
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (var_0 != var_5);
    var_11 = ((!(((var_6 / (min(var_3, var_8)))))));
    var_12 = (min(var_12, ((((min(var_1, ((min(var_8, var_5))))) * var_9)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_13 = var_5;
                var_14 = ((((min((arr_4 [i_0] [i_1]), (arr_3 [i_1])))) && ((((arr_4 [i_0] [2]) - (arr_4 [i_1] [i_1]))))));
                var_15 &= ((-((var_2 << (((var_5 && (arr_0 [i_1]))))))));
            }
        }
    }
    #pragma endscop
}
