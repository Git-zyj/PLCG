/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77634
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77634 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77634
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = var_8;
    /* LoopNest 3 */
    for (unsigned char i_0 = 3; i_0 < 8; i_0 += 4) 
    {
        for (unsigned short i_1 = 0; i_1 < 12; i_1 += 2) 
        {
            for (int i_2 = 0; i_2 < 12; i_2 += 3) 
            {
                {
                    var_19 = ((/* implicit */long long int) min((var_19), (((/* implicit */long long int) min((((((/* implicit */_Bool) arr_2 [i_1])) ? (((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned char)24))) : (arr_2 [i_1]))), (((/* implicit */unsigned long long int) var_7)))))));
                    var_20 = ((/* implicit */unsigned char) var_7);
                    var_21 = ((/* implicit */long long int) arr_7 [i_1]);
                }
            } 
        } 
    } 
    var_22 = ((/* implicit */unsigned long long int) (-(((((/* implicit */_Bool) (unsigned short)11291)) ? (((/* implicit */unsigned int) ((/* implicit */int) var_8))) : (var_11)))));
}
