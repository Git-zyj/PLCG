/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130456
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130456 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130456
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_12;
    var_18 ^= ((!((max((max(var_12, 1)), var_7)))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_19 = 118;
                arr_6 [i_1] [i_0] [i_0] = 1;
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_20 = var_12;
                                var_21 = ((((((arr_7 [i_3] [i_1] [i_0 - 1]) ? var_12 : var_0))) ? (min(5, var_16)) : (arr_11 [i_3] [i_3] [5] [i_3])));
                                arr_14 [i_0] [i_0] [i_3] [3] = (((((~(arr_12 [i_0 + 1] [i_0 + 2] [i_0 + 2] [i_0 + 2])))) ? (((max(var_9, 1)))) : var_5));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
