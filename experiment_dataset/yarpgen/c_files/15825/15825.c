/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15825
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15825 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15825
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 -= ((((((max(var_12, var_10)) | var_1))) ? 19 : ((var_14 ? (var_6 ^ var_14) : var_15))));
    var_20 = (max(var_20, var_14));
    var_21 = ((253188236 | 0) % var_14);
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_0] = (arr_0 [i_0]);
                arr_5 [i_0 + 1] [i_0] [i_1] = (((((var_5 ? (arr_2 [i_0 - 1] [i_0] [i_0 - 4]) : (arr_2 [i_0] [i_0] [i_0 - 4])))) ? ((((arr_2 [i_0] [i_0] [i_0 - 2]) <= (arr_2 [i_0 - 1] [i_0] [i_0 - 2])))) : (((arr_2 [i_1] [i_0] [i_0 - 3]) ? (arr_2 [i_0 + 2] [i_0] [i_0 - 2]) : var_5))));
            }
        }
    }
    #pragma endscop
}
