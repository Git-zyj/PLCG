/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136395
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136395 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136395
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */unsigned int) var_16);
    /* LoopNest 3 */
    for (long long int i_0 = 4; i_0 < 9; i_0 += 2) 
    {
        for (int i_1 = 0; i_1 < 11; i_1 += 3) 
        {
            for (unsigned short i_2 = 3; i_2 < 8; i_2 += 2) 
            {
                {
                    var_20 = ((/* implicit */unsigned int) ((((/* implicit */long long int) ((/* implicit */int) ((unsigned short) (_Bool)0)))) <= (((((long long int) var_14)) / (((/* implicit */long long int) var_1))))));
                    var_21 = 8313700145328527994ULL;
                    var_22 = ((/* implicit */unsigned short) var_9);
                }
            } 
        } 
    } 
}
