/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41826
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41826 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41826
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 11;i_1 += 1)
        {
            {
                arr_4 [i_1] [i_1] [i_1] = (((min((max(18446744073709551601, var_7)), (((-(arr_2 [i_0 - 1] [i_0 - 1]))))))) ? (arr_2 [i_0 + 1] [i_0 - 2]) : 18446744073709551615);
                arr_5 [i_1] [i_1] = (max(var_4, ((((arr_2 [i_0 - 1] [i_0 - 1]) < (arr_2 [i_0 + 1] [i_0 + 1]))))));
            }
        }
    }
    var_10 = (min(var_10, (((var_8 | var_0) & var_2))));
    #pragma endscop
}
