/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52030
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52030 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52030
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [i_0] = (-2147483647 - 1);
                    var_13 *= ((((((((arr_2 [i_0] [i_0]) <= -2147483637)) ? 37 : ((((arr_4 [i_0]) && (arr_1 [i_0] [11]))))))) ? ((((((arr_3 [7] [7] [7]) ? var_12 : (arr_5 [i_0] [i_0] [i_1])))) ? 0 : (arr_7 [0] [i_1] [i_2]))) : ((((!(arr_7 [i_0] [i_1] [i_0])))))));
                }
            }
        }
    }
    var_14 -= ((var_11 ? (!1176707661562361169) : (20975 + var_9)));
    var_15 = var_0;
    #pragma endscop
}
