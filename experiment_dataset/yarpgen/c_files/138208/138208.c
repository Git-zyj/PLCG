/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138208
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138208 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138208
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 *= (220 < 7846877926717238399);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                arr_17 [i_0] [i_1] [i_0] [i_3] [i_3] = arr_3 [i_0] [i_0];
                                arr_18 [i_0] [i_1] [i_2] [i_3] = (min(((192 ? -64 : 243)), (max(-37, ((((arr_8 [i_0] [i_1] [i_3]) || var_14)))))));
                                var_16 = max((arr_2 [i_3]), (((arr_10 [i_2]) ? ((((arr_0 [i_0] [i_1]) ? var_2 : 203))) : (arr_12 [i_0] [i_1] [i_1] [i_2] [i_3] [i_4]))));
                            }
                        }
                    }
                    arr_19 [i_2] = ((var_0 ? 273549268 : (arr_8 [i_0] [i_1] [i_2])));
                }
            }
        }
    }
    #pragma endscop
}
