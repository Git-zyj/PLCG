/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242605
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242605 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242605
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */unsigned int) var_15);
    var_19 = ((/* implicit */signed char) max((var_19), (((/* implicit */signed char) ((max((((/* implicit */unsigned long long int) var_11)), (844424930131968ULL))) >= (((/* implicit */unsigned long long int) (+(((/* implicit */int) var_6))))))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 0; i_0 < 22; i_0 += 3) 
    {
        var_20 = ((/* implicit */unsigned char) var_16);
        var_21 = ((/* implicit */_Bool) min((var_21), (((/* implicit */_Bool) (unsigned char)95))));
    }
}
