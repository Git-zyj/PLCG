/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246777
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246777 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246777
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 1; i_0 < 15; i_0 += 2) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_16 = ((/* implicit */int) ((((/* implicit */long long int) min((((/* implicit */unsigned int) ((unsigned short) arr_3 [i_1] [i_1] [(short)3]))), (arr_2 [i_0])))) >= (max((((/* implicit */long long int) arr_2 [i_0 + 1])), (arr_0 [i_0] [i_1])))));
                var_17 += ((/* implicit */long long int) ((((int) ((arr_0 [i_0 + 1] [i_0 + 1]) == (arr_0 [i_0] [i_1])))) | (((int) ((int) arr_1 [i_0])))));
            }
        } 
    } 
    var_18 = ((/* implicit */unsigned int) var_0);
}
