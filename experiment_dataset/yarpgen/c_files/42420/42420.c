/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 42420
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=42420 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/42420
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 *= 226;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] = ((+(((arr_4 [i_0] [1] [i_0]) / (((var_4 || (arr_3 [i_0]))))))));
                var_15 *= (min(((max((arr_4 [i_0] [i_1] [i_1]), (arr_4 [6] [i_0] [5])))), (arr_4 [i_0] [0] [i_1])));
            }
        }
    }
    var_16 = ((var_10 / (max((-2762449535482828274 / 9223372036854775807), var_13))));
    var_17 = var_10;
    var_18 = var_2;
    #pragma endscop
}
