/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 228470
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=228470 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/228470
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_10;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0 - 1] [i_1] = ((((max((1 - 1), var_1))) ? ((((arr_4 [i_0]) - (((arr_2 [i_0] [i_1] [i_1]) ? (arr_3 [i_0] [5] [i_1 - 1]) : (arr_0 [i_0])))))) : (((3234476225 - -415618595) - (27953 - var_14)))));
                arr_6 [i_0] [i_0 + 1] [12] = (((((arr_0 [i_1]) - (min(var_0, var_14)))) - ((max((((arr_0 [i_1]) - var_2)), (arr_3 [i_0] [i_0] [i_1 - 1]))))));
            }
        }
    }
    #pragma endscop
}
