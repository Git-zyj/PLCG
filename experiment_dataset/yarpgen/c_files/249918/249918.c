/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249918
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249918 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249918
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_13 += ((233 >> (var_6 - 26608)));
                var_14 += ((((max(22, (min(var_3, (arr_2 [i_0])))))) ? 233 : (((!(((13 ? var_9 : (arr_0 [i_1] [i_1])))))))));
                var_15 ^= var_4;
                var_16 = (max((((arr_2 [i_0 + 1]) ? 222 : (arr_2 [i_0 + 2]))), var_5));
            }
        }
    }
    var_17 = var_4;
    var_18 = 205;
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 17;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 17;i_3 += 1)
        {
            {
                var_19 = var_11;
                arr_11 [i_2] [i_2] = var_0;
            }
        }
    }
    /* LoopNest 2 */
    for (int i_4 = 0; i_4 < 13;i_4 += 1)
    {
        for (int i_5 = 2; i_5 < 12;i_5 += 1)
        {
            {
                var_20 = ((-((max((arr_8 [i_5 - 2] [i_5 - 1]), var_5)))));
                arr_18 [i_4] = ((((min(var_0, (arr_3 [i_5] [i_4] [i_5])))) ? (arr_3 [i_4] [i_4] [i_4]) : 23));
            }
        }
    }
    #pragma endscop
}
