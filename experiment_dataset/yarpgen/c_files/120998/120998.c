/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120998
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120998 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120998
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 12;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 15;i_4 += 1)
                        {
                            {
                                arr_12 [i_0] [4] [i_2] [i_2] [i_4] = (((arr_10 [i_2] [i_2] [i_4]) ? (((((((((arr_9 [i_0]) != 0)))) >= ((0 ? 147 : 28)))))) : -29));
                                arr_13 [i_0] [i_1] [14] [i_1] [i_4] &= -28;
                            }
                        }
                    }
                    var_10 ^= 416304637;
                    var_11 = (((((3119569434700141785 & 42098) + (~749125906))) > (arr_5 [10] [i_0 + 4] [i_2])));
                    var_12 &= 80;
                }
            }
        }
    }
    var_13 = (min((min(var_1, (-441399908 / 112))), 17942));
    #pragma endscop
}
