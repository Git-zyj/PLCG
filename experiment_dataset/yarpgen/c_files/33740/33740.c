/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33740
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33740 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33740
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(705718305285418329, var_2));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_9 [i_0 + 1] [i_0 + 1] [23] [23] = 1;
                    var_19 = 16426;
                    arr_10 [i_0] [i_1] [i_1] [i_2] = (min(((((arr_1 [i_2] [1]) >> ((16430 ? 1 : var_8))))), (min((var_0 & var_7), var_14))));
                    var_20 ^= (min((arr_6 [11] [i_1 + 1] [i_2]), var_9));
                    var_21 = (max(1, -69));
                }
            }
        }
    }
    #pragma endscop
}
