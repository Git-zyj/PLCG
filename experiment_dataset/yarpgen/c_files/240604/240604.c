/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 240604
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=240604 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/240604
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(((((var_1 == var_1) - (74 ^ 3253070281)))), (min(4797279520970249786, 63))));
    var_11 = (min(17181, -3828));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_1] = ((!((!((max((arr_1 [i_1 + 3]), (arr_3 [i_0] [i_1] [i_1]))))))));
                arr_6 [i_0] [i_0] = (min(((((((~(arr_1 [i_0])))) && ((max(var_0, var_2)))))), ((-((264241152 >> (72 - 43)))))));
            }
        }
    }
    #pragma endscop
}
