/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165772
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165772 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165772
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {

                /* vectorizable */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    var_12 = ((var_7 ? var_10 : 1));
                    arr_8 [i_1] |= ((!(!var_3)));
                }
                var_13 = var_8;
                arr_9 [10] = (max(var_0, ((((arr_4 [i_0] [i_0] [i_1]) != 63)))));
                var_14 += (~var_4);
                var_15 *= min((min((((arr_1 [i_0] [i_0]) ? var_3 : 51357)), (~var_0))), var_4);
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 1; i_3 < 12;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 14;i_4 += 1)
        {
            {
                arr_16 [i_3] [i_3] [i_3 + 1] = ((~(((var_5 == 3395982067) ? (((arr_15 [1]) ? var_10 : var_2)) : var_8))));
                arr_17 [i_3] [i_3] = (!var_8);
            }
        }
    }
    #pragma endscop
}
