/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143132
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143132 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143132
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_3;
    var_16 = var_13;
    var_17 = var_8;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_18 = (arr_4 [i_0 + 1] [i_0 + 1] [i_1]);
                var_19 = ((-(arr_1 [i_0] [1])));
                var_20 = ((arr_0 [i_0]) ? var_1 : ((192 ? -7132065605184951638 : (((22398232 ? (arr_0 [i_0]) : var_13))))));
                /* LoopNest 3 */
                for (int i_2 = 1; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 14;i_4 += 1)
                        {
                            {
                                arr_13 [i_0] = ((-(arr_12 [i_0] [i_1] [i_0])));
                                arr_14 [i_0] [i_0] [i_0 + 1] [i_0] [i_0] [i_0] = (arr_12 [i_0] [i_2 + 3] [i_0]);
                                arr_15 [12] [0] [1] [12] [i_0] = 246;
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
