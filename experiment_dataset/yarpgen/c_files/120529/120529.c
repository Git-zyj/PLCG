/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120529
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120529 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120529
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((33007 ? ((min(48, (min(-1, 0))))) : 1));
    var_13 &= min((!var_1), (min(2006, 1)));
    var_14 = (max(var_14, var_10));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_15 = (max(var_15, ((((((arr_4 [i_1 - 1]) >= (((min(var_2, var_6)))))) ? ((((var_9 < (arr_1 [i_1 - 3] [i_0 - 1]))))) : (max(var_2, ((-(arr_1 [i_1 - 3] [i_2]))))))))));
                    var_16 = (min(var_16, ((max(((((arr_0 [i_0 - 1]) ? (arr_4 [i_0 + 2]) : (arr_1 [i_0] [i_0])))), (arr_7 [i_0]))))));
                }
            }
        }
    }
    #pragma endscop
}
