/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 109141
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=109141 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/109141
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        {
                            var_12 = (max(-107, 97));
                            arr_12 [i_1] [i_1] = (((((-(arr_11 [i_0] [i_0])))) ? (-40 % var_10) : (((max((arr_4 [i_1]), 23245))))));
                        }
                    }
                }
                var_13 = (max(((98 ? 33974 : (arr_4 [i_1]))), (((arr_5 [i_0] [i_0]) ? (arr_6 [i_0] [i_0] [i_1]) : (~-107)))));
                arr_13 [i_1] [i_1] = (((arr_10 [i_0]) * (arr_1 [i_0])));
                arr_14 [i_1] [i_1] = -110;
            }
        }
    }
    #pragma endscop
}
