/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125807
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125807 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125807
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 &= var_2;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 11;i_2 += 1)
            {
                {
                    var_13 -= (arr_8 [i_0] [i_1] [i_2]);
                    var_14 *= var_5;
                    var_15 ^= (((((((((arr_4 [2] [i_0]) + 239))) ? ((64776 ? (arr_3 [i_0]) : (arr_8 [i_0] [i_0] [i_0]))) : 47925))) ? (arr_8 [4] [4] [i_2 + 1]) : ((min(((var_8 ? 239 : (arr_0 [11]))), ((((arr_1 [i_1]) < (arr_8 [i_0] [i_0] [i_0])))))))));
                    arr_9 [i_0] [i_0] = (((arr_1 [i_0]) ? ((max(var_0, (arr_5 [i_0])))) : (((arr_7 [i_2 + 1] [i_1] [10] [i_2]) ? (min((arr_8 [8] [i_0] [i_0]), (arr_0 [i_0]))) : (var_7 && 239)))));
                }
            }
        }
    }
    var_16 ^= 63;
    #pragma endscop
}
