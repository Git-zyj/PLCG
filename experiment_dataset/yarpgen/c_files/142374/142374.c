/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 142374
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=142374 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/142374
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max((((max(-609605432852822571, var_8)) + ((max(var_4, var_0))))), var_5));
    var_11 = (~9131);
    var_12 = var_0;

    for (int i_0 = 1; i_0 < 7;i_0 += 1)
    {
        arr_3 [i_0] = (arr_2 [i_0 - 1] [i_0 - 1]);
        arr_4 [i_0 + 2] = (((((min(-258058059, 0)) * ((var_3 ? (arr_2 [i_0] [i_0]) : (arr_1 [i_0]))))) ^ (min(18446744073709551615, (6016379263580787817 ^ 2024985910)))));
    }
    #pragma endscop
}
