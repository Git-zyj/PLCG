/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151860
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151860 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151860
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = var_9;
    /* LoopSeq 1 */
    for (signed char i_0 = 2; i_0 < 10; i_0 += 3) 
    {
        var_16 = ((((/* implicit */_Bool) (signed char)7)) ? (arr_1 [i_0] [i_0]) : (((/* implicit */unsigned long long int) ((((/* implicit */_Bool) arr_1 [i_0] [i_0 - 1])) ? (262965943) : (((/* implicit */int) var_6))))));
        var_17 = ((((/* implicit */long long int) ((/* implicit */int) (short)5067))) + (-8153986129820860676LL));
    }
}
