/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 140378
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=140378 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/140378
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = 0;
    var_13 = var_0;

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_14 *= 1;
        arr_2 [i_0] [i_0] = 1;
        arr_3 [i_0] = 1;
        var_15 = ((min((!var_10), (max(1, (arr_0 [i_0] [i_0]))))) && 1);
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_16 = ((var_5 * (arr_5 [i_1] [i_1])));
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 1;i_3 += 1)
            {
                {
                    arr_13 [i_3] = ((var_5 ? 1 : var_11));
                    /* LoopNest 2 */
                    for (int i_4 = 0; i_4 < 1;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 1;i_5 += 1)
                        {
                            {
                                arr_20 [i_1] [i_2] [i_3] [i_4] [i_5] = (max(0, ((~(arr_17 [i_3] [i_4] [i_3])))));
                                var_17 += 1;
                                var_18 = ((((-(arr_10 [i_5] [i_4] [i_3]))) ^ 1));
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_6 = 0; i_6 < 0;i_6 += 1)
                    {
                        for (int i_7 = 1; i_7 < 1;i_7 += 1)
                        {
                            {
                                var_19 = (max(((~((min(1, 1))))), (((1 < (arr_7 [i_1] [i_7 - 1]))))));
                                var_20 = (max(var_20, var_3));
                                arr_25 [i_2] [i_3] = max(var_10, (arr_24 [i_3] [i_2] [i_3] [i_7 - 1]));
                                var_21 = min((!1), (min(var_4, (max((arr_10 [i_6 + 1] [i_2] [i_1]), (arr_7 [i_1] [i_6 + 1]))))));
                                arr_26 [i_1] [i_1] [i_1] [i_1] [i_3] [i_1] = arr_24 [i_3] [i_6 + 1] [i_3] [i_6 + 1];
                            }
                        }
                    }
                    var_22 *= 1;
                }
            }
        }
        var_23 += (!((min((0 - var_5), 1))));
    }
    #pragma endscop
}
