/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84931
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84931 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84931
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_18 = ((var_13 ? ((63 ? 5237660004251418046 : 5237660004251418046)) : (arr_2 [i_0])));
        arr_3 [i_0] [i_0] = var_12;

        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            var_19 = (min(var_19, (var_11 & var_17)));
            arr_6 [i_0] [i_1] = var_14;
        }
    }
    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        var_20 = ((((var_4 && (arr_8 [i_2]))) && (((((4570 ? var_4 : 12241))) && var_10))));
        var_21 = (((((arr_8 [i_2]) * var_5))) ? -4 : 938398063);
        var_22 = (arr_8 [i_2]);
    }

    /* vectorizable */
    for (int i_3 = 0; i_3 < 19;i_3 += 1)
    {
        var_23 = (arr_11 [i_3]);
        arr_12 [i_3] [i_3] = var_2;
    }
    #pragma endscop
}
