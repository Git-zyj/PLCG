/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23432
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23432 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23432
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned long long int) (((!(((/* implicit */_Bool) (((_Bool)1) ? (((/* implicit */int) var_11)) : (var_12)))))) ? (((((/* implicit */_Bool) (+(var_12)))) ? (var_2) : (var_4))) : (((/* implicit */int) var_8))));
    var_17 = ((/* implicit */unsigned short) var_10);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 1; i_0 < 13; i_0 += 4) 
    {
        for (signed char i_1 = 3; i_1 < 14; i_1 += 1) 
        {
            {
                var_18 = ((/* implicit */short) max((min((((/* implicit */int) (unsigned char)163)), (var_4))), (((((/* implicit */int) (_Bool)1)) + ((-(((/* implicit */int) (_Bool)1))))))));
                var_19 = ((/* implicit */unsigned long long int) -1LL);
            }
        } 
    } 
}
