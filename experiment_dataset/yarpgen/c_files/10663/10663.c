/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 10663
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=10663 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/10663
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] = ((1310549851 ? ((min(77, (min(37075, 54661))))) : (26327 ^ 11052)));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 10;i_3 += 1)
                    {
                        {
                            var_17 = (~var_14);
                            var_18 = (max(var_1, (((((var_11 + 2147483647) >> (var_15 - 83))) | (arr_2 [i_0] [i_1] [i_0])))));
                            var_19 = ((((((arr_1 [i_0 + 1]) << (((arr_1 [i_3 + 2]) - 7460))))) ? (max(var_10, ((min(10863, -26315))))) : (((((arr_0 [i_2] [i_3 - 1]) >= (arr_7 [i_0] [i_0])))))));
                        }
                    }
                }
            }
        }
    }
    var_20 = (-1 | -2006386304);
    var_21 = -var_7;
    #pragma endscop
}
