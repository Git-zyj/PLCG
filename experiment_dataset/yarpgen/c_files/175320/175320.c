/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175320
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175320 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175320
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 &= ((((((1 ? var_0 : 1)))) % (max((max(var_17, var_10)), ((var_16 ? var_17 : var_8))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 8;i_1 += 1)
        {
            {
                var_20 &= (((((arr_2 [i_1 + 1]) & (max((arr_0 [2]), 485811252))))) && (((((min((arr_0 [i_1 + 4]), (arr_0 [i_0])))) ? (((arr_2 [i_0]) ? 129 : 1)) : (1 && 3416059792)))));
                var_21 = ((0 << (17646094981287106844 != 17646094981287106844)));
            }
        }
    }
    #pragma endscop
}
