/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210976
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210976 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210976
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (~-1248456226);
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_6 [i_0] = (max((((+(min((arr_3 [13] [i_1]), var_13))))), ((~(max(var_10, 7899124603066549825))))));
                arr_7 [i_0] = (arr_2 [17] [i_0 + 1]);
                arr_8 [i_0] [5] [0] |= ((-(arr_0 [i_0])));
            }
        }
    }
    var_16 = var_6;
    var_17 = ((var_4 ? var_12 : (min(var_10, -7796551685689040370))));
    #pragma endscop
}
