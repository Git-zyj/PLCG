/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244453
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244453 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244453
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((var_9 % (min(var_2, (~var_5)))));
    var_12 = 186;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_13 = 11;
                    var_14 = ((((arr_1 [i_0 + 1] [i_0 + 1]) ? (var_9 != 73) : (((arr_1 [4] [i_0]) >> (((arr_1 [4] [i_2]) - 12215)))))));
                    var_15 = (max((((max(var_6, (arr_0 [i_0]))) * ((((var_10 > (arr_5 [i_2] [i_2] [i_1] [i_0]))))))), (((arr_3 [2] [i_0] [i_0]) ? (arr_4 [2] [i_1] [i_2]) : ((((arr_3 [i_0] [i_0] [i_0]) ? 611123022 : 60)))))));
                }
            }
        }
    }
    var_16 = var_10;
    #pragma endscop
}
