/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48491
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48491 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48491
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 4; i_0 < 9;i_0 += 1)
    {
        arr_2 [i_0] = (((+(((arr_1 [9] [9]) ? 245 : 237)))) ^ 10812);
        arr_3 [i_0] = (((-9223372036854775807 - 1) == var_6));
        var_18 = min((min(((44 ? var_12 : (arr_0 [i_0]))), (9223372036854775807 == 248))), var_14);
        var_19 -= (!2147483647);
    }
    for (int i_1 = 0; i_1 < 24;i_1 += 1)
    {
        var_20 -= (((arr_5 [i_1]) != -134));
        arr_6 [i_1] = 245;
    }
    var_21 = (min(var_15, var_6));
    #pragma endscop
}
