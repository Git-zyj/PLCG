/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 1373
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=1373 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/1373
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_7 [i_1] [i_1] [i_1] = (arr_3 [i_1] [i_0]);
                var_15 = (max(27, 10789));
                arr_8 [i_0] [i_1] [i_1] = ((((!(arr_0 [1] [i_0]))) ? ((min((arr_0 [i_1] [i_1]), (arr_0 [i_0] [i_1])))) : (((!(arr_0 [i_0] [i_1]))))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 2; i_2 < 13;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 14;i_3 += 1)
        {
            {
                var_16 = (((((arr_12 [i_2 + 1] [2] [i_2]) || (arr_12 [i_2 + 1] [i_2] [i_2]))) || ((!(arr_12 [i_2 + 1] [i_2 + 1] [i_3])))));
                var_17 = (max(var_17, var_13));
            }
        }
    }
    #pragma endscop
}
