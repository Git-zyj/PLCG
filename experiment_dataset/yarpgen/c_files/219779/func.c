/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219779
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219779 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219779
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 ^= ((/* implicit */signed char) min((((/* implicit */int) var_9)), ((-(((/* implicit */int) (_Bool)1))))));
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 1; i_0 < 13; i_0 += 1) 
    {
        for (unsigned int i_1 = 0; i_1 < 17; i_1 += 4) 
        {
            for (unsigned short i_2 = 3; i_2 < 13; i_2 += 1) 
            {
                {
                    /* LoopNest 2 */
                    for (unsigned char i_3 = 0; i_3 < 17; i_3 += 4) 
                    {
                        for (unsigned int i_4 = 0; i_4 < 17; i_4 += 3) 
                        {
                            {
                                var_12 = ((/* implicit */unsigned char) min((((/* implicit */unsigned int) (~(((/* implicit */int) min((((/* implicit */unsigned short) var_3)), (arr_10 [i_0 + 1] [(unsigned char)7] [i_1] [i_2] [16ULL] [i_4]))))))), (3452255628U)));
                                arr_11 [i_2] [i_1] = max((var_1), (((/* implicit */unsigned long long int) var_9)));
                                var_13 = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) (signed char)127))));
                            }
                        } 
                    } 
                    var_14 = ((/* implicit */unsigned int) (~((-(((/* implicit */int) var_8))))));
                    var_15 = ((/* implicit */long long int) var_6);
                }
            } 
        } 
    } 
    var_16 &= ((/* implicit */unsigned int) var_10);
    var_17 &= ((/* implicit */unsigned short) (!(((/* implicit */_Bool) (-(var_4))))));
}
