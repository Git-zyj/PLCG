/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34380
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34380 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34380
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_5;

    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        arr_2 [i_0] = ((!((max(var_7, 24)))));
        arr_3 [13] = (((((((((arr_0 [i_0 - 2] [i_0 - 2]) & (arr_1 [i_0])))) ? (((155 & (arr_1 [i_0 + 3])))) : (arr_0 [i_0] [i_0])))) ? (((~(((arr_0 [i_0] [i_0]) ? var_6 : (arr_0 [i_0 + 2] [i_0 - 1])))))) : ((var_4 ? -4679291839540111210 : 4679291839540111210))));
    }
    #pragma endscop
}
