/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 167410
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=167410 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/167410
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max(var_13, var_11));
    var_14 = (min(var_14, (0 && var_7)));
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = 0;
                var_15 = (max(var_15, ((((var_2 > ((16597020782932279290 ? (arr_1 [i_0]) : 3447727309566049950)))) ? ((-(arr_4 [i_0 + 3]))) : (((min(var_5, (arr_2 [i_0] [i_1])))))))));
                var_16 = ((-(min((arr_3 [i_0 - 2] [i_0 + 3] [i_0 - 1]), (arr_3 [i_0 - 3] [i_0 - 3] [i_0 - 3])))));
                var_17 = (min(var_17, 29479));
            }
        }
    }
    var_18 = var_7;
    #pragma endscop
}
