/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174445
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174445 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174445
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = var_10;
                arr_5 [9] = var_2;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        {
                            arr_11 [4] [5] [1] [i_2] [4] [10] = (max(((min((arr_3 [i_2] [i_3]), (arr_3 [i_1] [i_1])))), (((arr_3 [i_1] [i_3]) ? 6331555163728455721 : (arr_3 [i_1] [i_2])))));
                            var_14 = var_10;
                        }
                    }
                }
                arr_12 [i_0] [8] = (var_2 * ((var_3 << (var_9 - 4015117447449665544))));
            }
        }
    }
    #pragma endscop
}
