/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72196
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72196 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72196
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_21 |= 6490;
                                arr_11 [i_0] [i_0] [i_1] [i_2] [i_2] [i_2] [i_1] = ((-(min((arr_10 [16] [i_1] [11] [i_3] [i_3] [i_4]), -var_7))));
                                arr_12 [i_4] [i_3] [i_1] [i_1] [9] = (max((((var_19 + 0) >> (((arr_8 [i_1] [i_0 - 1] [i_0 + 1] [i_0] [i_0 - 1] [19] [i_0]) - 9411)))), ((max(var_19, 6482)))));
                            }
                        }
                    }
                    arr_13 [i_1] = (~96);
                }
            }
        }
    }
    #pragma endscop
}
