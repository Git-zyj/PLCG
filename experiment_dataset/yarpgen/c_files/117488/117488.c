/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117488
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117488 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117488
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_6;
    var_17 = (((max((~var_10), var_2)) | (((-(var_12 <= var_10))))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_1] [i_1] = ((~((((max(15, 3))) ? (((min(0, var_12)))) : (max(var_1, var_10))))));
                arr_7 [i_1] = (((max(3250201043879381594, 3128865517)) ^ (((max(1, -64))))));
            }
        }
    }
    var_18 = ((((((var_6 + var_3) - (var_6 & var_0)))) % ((min(var_9, var_8)))));
    #pragma endscop
}
