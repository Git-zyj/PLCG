/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103142
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103142 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103142
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = -2;
    var_13 = (max((!-511), var_7));

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_14 = (((-20017 + 2147483647) >> ((((-1569247816 ? (arr_1 [i_0]) : 719685560)) - 758139605))));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_15 *= (((((!((((arr_6 [7] [i_1] [i_0]) >> (var_11 - 111))))))) % (((3968 || ((max(3992, 3968))))))));
                    arr_8 [i_2] [i_1] [i_0] [i_0] = (!-1);
                }
            }
        }
    }
    #pragma endscop
}
