/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105742
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105742 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105742
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (min(32767, -95));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_6 [i_1] = (max((arr_5 [i_0] [i_1]), (((min((arr_3 [i_1]), 4085554978662083851))))));
                arr_7 [0] = (((((94 & (arr_5 [i_0 + 2] [i_1])))) ? ((-(var_11 - var_5))) : (arr_2 [i_0] [i_1])));
                arr_8 [i_1] = (((((max(var_17, 95)))) - (((!(arr_1 [i_0 - 1]))))));
                arr_9 [i_1] = max(var_3, var_9);
            }
        }
    }
    var_21 = var_13;
    var_22 = var_0;
    #pragma endscop
}
