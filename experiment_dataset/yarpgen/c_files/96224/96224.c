/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 96224
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=96224 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/96224
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        var_18 -= ((var_2 + ((616821277 ? (arr_2 [1] [i_2] [i_1]) : ((min((arr_1 [i_1]), var_1)))))));
                        var_19 = (min(var_19, var_5));
                    }
                    var_20 = ((!((((!(arr_9 [9] [9] [9] [i_0]))) || ((min((arr_6 [i_0] [i_1] [i_1] [i_2]), -9093)))))));
                }
            }
        }
    }
    var_21 = (max(var_21, ((((((-(!var_9)))) ? (max((~-14812), ((max(14812, 0))))) : -var_8)))));
    #pragma endscop
}
