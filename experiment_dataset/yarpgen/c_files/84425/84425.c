/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84425
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84425 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84425
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 ^= -6137738969490506201;
    var_17 = (max(-95470500, var_4));
    var_18 |= ((~((var_14 | (max(var_3, var_5))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    var_19 = (arr_0 [i_1] [i_0]);
                    var_20 = (max(var_20, (((((((((var_3 ? var_2 : -49))) ? var_0 : var_7))) && ((max(((-(arr_4 [i_2] [i_2]))), ((var_10 ? 255 : (arr_6 [i_2])))))))))));
                    var_21 = (arr_1 [i_1]);
                }
            }
        }
    }
    var_22 = var_13;
    #pragma endscop
}
