/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68745
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68745 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68745
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 &= var_2;
    var_13 = (((((max(2730495243, var_11)))) + 2730495243));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_14 -= (((!1) + ((min((arr_2 [i_0]), (arr_2 [i_0]))))));
                var_15 = 62914560;
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 0; i_2 < 18;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 18;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 18;i_4 += 1)
            {
                {
                    arr_12 [i_3] [i_4] [i_4] = ((62914567 ? -3423 : 2730495254));
                    var_16 = (min(var_5, ((-(((arr_10 [i_2] [i_2] [i_4]) ? var_11 : var_8))))));
                }
            }
        }
    }
    #pragma endscop
}
