/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135868
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135868 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135868
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */int) min((var_13), (((/* implicit */int) var_6))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 9; i_0 += 1) 
    {
        for (unsigned int i_1 = 0; i_1 < 11; i_1 += 1) 
        {
            {
                arr_4 [4U] [i_0] = ((/* implicit */long long int) ((((/* implicit */_Bool) var_4)) && (((/* implicit */_Bool) var_6))));
                arr_5 [i_0] [(signed char)7] [i_0] = ((17275318685357101916ULL) << (((17275318685357101915ULL) - (17275318685357101900ULL))));
                var_14 = ((/* implicit */short) min((((/* implicit */int) var_6)), (arr_3 [i_1])));
            }
        } 
    } 
}
