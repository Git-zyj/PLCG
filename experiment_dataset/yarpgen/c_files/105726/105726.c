/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105726
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105726 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105726
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_19 *= ((-(arr_5 [i_1] [i_1] [i_0 + 1])));
                arr_6 [i_0] = 1311706935;
                arr_7 [i_0] [i_0 + 1] = var_9;
            }
        }
    }
    var_20 += (((((((max(8, -7006096415844924825))) ? var_4 : 205))) != (max(-881874952919113834, var_5))));
    var_21 = (!var_6);
    var_22 = -1311706946;
    var_23 = (max(((~(~var_10))), (max(1, (~0)))));
    #pragma endscop
}
