/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73334
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73334 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73334
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned short i_0 = 0; i_0 < 20; i_0 += 4) 
    {
        for (signed char i_1 = 3; i_1 < 17; i_1 += 3) 
        {
            {
                arr_6 [i_0] [(short)4] = ((/* implicit */short) max(((+(2700557012727164730ULL))), (((/* implicit */unsigned long long int) min((((/* implicit */long long int) var_13)), (arr_0 [i_1 + 1]))))));
                var_17 = ((/* implicit */unsigned int) var_13);
            }
        } 
    } 
    var_18 &= ((/* implicit */_Bool) 4294967292U);
    var_19 = ((/* implicit */unsigned short) var_7);
}
