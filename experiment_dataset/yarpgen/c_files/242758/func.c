/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242758
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242758 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242758
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned short) min(((signed char)-92), (((/* implicit */signed char) (_Bool)1))));
    /* LoopNest 2 */
    for (unsigned short i_0 = 1; i_0 < 11; i_0 += 4) 
    {
        for (long long int i_1 = 0; i_1 < 13; i_1 += 2) 
        {
            {
                arr_5 [i_1] [i_0] [i_0] = ((/* implicit */_Bool) ((((((/* implicit */int) (short)-14448)) > (((/* implicit */int) arr_1 [i_0 - 1])))) ? (var_6) : (((/* implicit */unsigned int) -333932935))));
                arr_6 [i_0 + 1] [i_1] = ((/* implicit */unsigned short) arr_1 [i_0 - 1]);
            }
        } 
    } 
}
