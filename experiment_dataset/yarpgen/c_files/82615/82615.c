/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82615
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82615 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82615
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = max(var_6, var_14);
    var_19 &= 24;
    var_20 = (max(var_15, (max(1, ((var_15 ? var_4 : var_14))))));
    var_21 = 1;

    for (int i_0 = 0; i_0 < 25;i_0 += 1) /* same iter space */
    {
        var_22 = 1;
        arr_2 [i_0] = (arr_0 [i_0]);
        arr_3 [i_0] [i_0] = 1;
    }
    for (int i_1 = 0; i_1 < 25;i_1 += 1) /* same iter space */
    {
        arr_6 [i_1] = (0 < 18446744073709551606);
        var_23 = (((((176246155264613803 + var_2) - 1)) < 1));
        var_24 ^= (((min(((1 - (arr_4 [i_1] [i_1]))), (min(var_1, var_16)))) > (((arr_4 [16] [i_1]) ? var_10 : ((2147483647 ? var_12 : 57976))))));
        arr_7 [i_1] [i_1] = (arr_1 [5] [5]);
    }
    #pragma endscop
}
