/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97313
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97313 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97313
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_1;
    var_13 = max(((1 ? 60901 : (var_4 % 32762))), ((var_6 + (!6190775121774261121))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                arr_5 [18] = ((((var_5 ? var_2 : (arr_3 [i_0] [i_0 - 1] [i_0 + 1]))) >> (var_3 < var_2)));
                arr_6 [i_0 - 1] = (min(var_6, -559251982));
                var_14 = arr_1 [15];
                arr_7 [i_1 + 3] [i_0 + 3] [13] = ((15208 << (((~var_7) + 27198))));
            }
        }
    }
    #pragma endscop
}
