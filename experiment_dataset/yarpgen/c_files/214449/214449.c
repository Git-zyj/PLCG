/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214449
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214449 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214449
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_10 = ((((var_1 + 9223372036854775807) << (var_5 - 206061887))));
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 22;i_3 += 1)
                    {
                        {
                            var_11 = ((~1) != ((+((((arr_6 [i_3] [i_0] [i_1] [i_0]) && var_2))))));
                            arr_11 [i_3] [18] [i_0] [i_0] = (((((-(((arr_5 [i_1] [i_2] [i_3]) + (arr_4 [i_0] [i_1] [i_2])))))) ? ((var_8 ? var_9 : var_9)) : (2147483625 != 2147483647)));
                        }
                    }
                }
            }
        }
    }
    var_12 = (var_3 << ((-1 * (1 - 1))));
    var_13 = ((((((var_2 ? var_7 : 65535))) && var_5)));
    #pragma endscop
}
