/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224848
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224848 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224848
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (min((min((max((arr_1 [i_0]), var_11)), (max(-9, var_14)))), (arr_0 [i_0])));
        arr_3 [i_0] = (((min((var_9 ^ var_0), ((((arr_1 [i_0]) ? (arr_1 [i_0]) : var_12))))) >= (arr_0 [i_0])));
        var_19 = ((max((arr_0 [i_0]), (min((arr_0 [i_0]), (arr_1 [i_0]))))));
    }
    var_20 = (var_2 ^ var_6);
    var_21 = (min(var_18, -var_1));
    #pragma endscop
}
