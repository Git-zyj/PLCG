/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70388
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_14 = ((~((~((var_2 ? 1 : -8401335761657093635))))));
                var_15 = (18470 * -18484);
                var_16 = (min(var_16, ((((((~var_2) + 2147483647)) << ((((~((30961 ? var_0 : 18502)))) - 1406630677)))))));
                arr_5 [i_0] [i_0] = (((arr_0 [i_0 + 1] [i_0 + 1]) <= (min((arr_3 [i_0 - 1] [i_0 - 1] [i_0]), (arr_0 [i_0 + 1] [i_0 - 1])))));
            }
        }
    }
    var_17 = ((!(((-18476 ? var_1 : (~var_5))))));
    var_18 = var_10;
    #pragma endscop
}
