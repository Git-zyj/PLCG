/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 107967
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=107967 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/107967
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((0 ? 4294967295 : 65533));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_13 = (min(var_11, var_10));
                var_14 ^= (min(var_10, ((0 << (-1571962225 + 1571962246)))));
                var_15 = (min(var_15, (((((max(4754, (4096 && var_2)))) << (2713738367421025641 - 2713738367421025639))))));
                arr_4 [i_1] = (((((var_6 < (arr_1 [i_0] [1]))))) >= ((~(-2147483647 - 1))));
                var_16 = (arr_3 [i_0] [i_1]);
            }
        }
    }
    var_17 = (min(var_17, (var_1 ^ -391410666)));
    #pragma endscop
}
