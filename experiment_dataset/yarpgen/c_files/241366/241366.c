/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241366
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241366 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241366
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    var_10 = (((~1) & ((var_6 ? 3506442519 : 4277813558))));
                    arr_10 [5] [5] [2] = var_6;
                    arr_11 [i_2] [i_1] [i_0] = (arr_3 [i_0]);
                }
            }
        }
        var_11 = (arr_1 [i_0] [i_0]);
        var_12 = arr_1 [i_0] [i_0];
        arr_12 [i_0] = ((arr_7 [i_0] [i_0] [i_0]) ? (arr_7 [i_0] [i_0] [i_0]) : (arr_4 [i_0] [i_0]));
        arr_13 [i_0] = ((var_4 + (arr_8 [i_0])));
    }
    var_13 = ((((max(var_2, var_7))) * (min(0, 5007525140668048146))));
    var_14 |= var_1;
    var_15 = ((((12552946083506093581 && (var_3 || var_8))) ? 91 : 48818625));
    #pragma endscop
}
