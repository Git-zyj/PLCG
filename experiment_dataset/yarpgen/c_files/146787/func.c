/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146787
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146787 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146787
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */short) ((max((((/* implicit */int) ((((/* implicit */int) var_8)) >= (-12)))), (var_10))) < (((/* implicit */int) var_8))));
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 2; i_0 < 11; i_0 += 4) 
    {
        for (unsigned short i_1 = 0; i_1 < 12; i_1 += 3) 
        {
            for (unsigned char i_2 = 2; i_2 < 11; i_2 += 3) 
            {
                {
                    var_15 = ((/* implicit */_Bool) arr_0 [i_0 - 1] [i_0 - 2]);
                    var_16 = ((/* implicit */_Bool) max((arr_2 [i_2 + 1] [i_2 - 1] [i_2 - 1]), (arr_1 [i_0 - 2])));
                }
            } 
        } 
    } 
}
