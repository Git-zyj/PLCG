/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40395
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40395 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40395
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 &= (!var_2);
    var_11 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_12 = ((-(((arr_0 [i_1]) ? (var_2 || 127) : 24))));
                var_13 = (min(var_13, (((min(40, 29))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 24;i_3 += 1)
                    {
                        {
                            var_14 -= ((((var_0 ? 2374130274 : var_7)) / (max(var_7, 24949))));
                            arr_13 [i_3] [i_1] [i_3] [i_3] = (min(((((var_4 ? var_1 : var_4)) * (!var_3))), (((-37 ? 2374130279 : 3291)))));
                        }
                    }
                }
                var_15 ^= 1;
            }
        }
    }
    #pragma endscop
}
