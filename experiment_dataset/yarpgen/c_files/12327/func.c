/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 12327
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=12327 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/12327
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = (short)-26217;
    /* LoopNest 3 */
    for (unsigned int i_0 = 0; i_0 < 23; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 20; i_1 += 1) 
        {
            for (long long int i_2 = 0; i_2 < 23; i_2 += 1) 
            {
                {
                    var_13 = ((/* implicit */short) (!(((/* implicit */_Bool) ((unsigned long long int) 11864257872100506839ULL)))));
                    arr_8 [i_0] [i_1 + 3] [i_2] [(unsigned short)6] = ((unsigned short) (unsigned short)40048);
                }
            } 
        } 
    } 
    var_14 -= ((/* implicit */short) ((int) var_9));
    var_15 &= ((/* implicit */short) var_6);
    var_16 ^= var_11;
}
