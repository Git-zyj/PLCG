/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197795
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197795 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197795
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 ^= (max(((var_7 ? (max(var_0, var_3)) : var_6)), 52842));

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_13 ^= (~12694);
        arr_3 [2] = ((((((52842 != (arr_0 [i_0]))) || 226)) ? ((((((~(arr_0 [7])))) ? (arr_2 [i_0]) : -8787952124334445265))) : (((arr_2 [i_0]) ? ((var_6 ^ (arr_1 [i_0] [i_0]))) : (((52845 ? 12696 : 1)))))));
    }
    #pragma endscop
}
