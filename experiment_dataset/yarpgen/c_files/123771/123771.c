/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123771
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123771 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123771
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((~var_12) * (var_3 * var_9)));
    var_17 = var_7;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_10 [i_0] [i_0] [i_0] = ((((~var_9) ? ((((arr_6 [i_0] [i_0] [i_2]) + (arr_0 [i_0])))) : (((arr_3 [i_2] [i_0]) % 4053969820)))) != (((((var_11 | 240997476) && var_1)))));
                    arr_11 [i_0 + 3] [i_1] [i_0] [i_2] = -4053969821;
                }
            }
        }
    }
    var_18 = (42670 % -93);
    #pragma endscop
}
