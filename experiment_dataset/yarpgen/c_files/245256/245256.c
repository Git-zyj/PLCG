/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 245256
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=245256 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/245256
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (min((((min(var_11, var_17)) * var_9)), var_12));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                var_20 |= (min(0, 27));
                var_21 = (min(var_21, ((((arr_3 [8] [i_1]) & (arr_1 [i_0]))))));
                arr_4 [i_1] [i_1] [i_1] = ((min((arr_3 [i_1] [i_0]), ((((arr_0 [i_0]) <= (arr_3 [i_1] [i_1])))))) << (((~10) ? (((63 ? 4 : (arr_1 [i_1])))) : 2012944302398572018)));
            }
        }
    }
    var_22 = var_4;
    #pragma endscop
}
