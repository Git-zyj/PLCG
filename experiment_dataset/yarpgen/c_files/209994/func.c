/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209994
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209994 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209994
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 -= ((/* implicit */unsigned char) (+(((((/* implicit */int) ((((/* implicit */int) (signed char)-34)) >= (((/* implicit */int) (unsigned char)253))))) >> (((var_4) - (1260561378)))))));
    var_20 = (unsigned char)29;
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        arr_2 [i_0] = ((/* implicit */_Bool) (+((-(((/* implicit */int) (unsigned char)132))))));
        var_21 += ((/* implicit */short) (+((-((+(((/* implicit */int) (_Bool)1))))))));
    }
}
