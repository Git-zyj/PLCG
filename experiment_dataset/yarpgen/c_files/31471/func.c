/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 31471
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=31471 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/31471
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */int) (-(((((/* implicit */_Bool) var_1)) ? (var_9) : (((var_1) ^ (-8240848828363618468LL)))))));
    var_19 = ((/* implicit */unsigned char) min(((_Bool)1), ((_Bool)0)));
    var_20 = ((/* implicit */long long int) (+(18446744073709551615ULL)));
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 1; i_0 < 18; i_0 += 2) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_21 = ((/* implicit */long long int) (-((-(((/* implicit */int) var_15))))));
                var_22 = ((/* implicit */signed char) 1048448);
            }
        } 
    } 
}
