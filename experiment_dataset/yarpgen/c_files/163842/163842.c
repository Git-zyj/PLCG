/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163842
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163842 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163842
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (min(var_17, var_7));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {

                /* vectorizable */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    arr_7 [3] [i_0] [i_1] [i_2] = (((arr_0 [i_0] [i_0]) ? (arr_0 [i_0] [i_1]) : 3339948889));
                    var_18 = (arr_4 [i_0] [i_0] [i_0]);
                    arr_8 [i_2] [i_1] = (arr_1 [i_0]);
                }
                /* vectorizable */
                for (int i_3 = 2; i_3 < 21;i_3 += 1)
                {
                    arr_13 [i_0] [1] [i_3] [i_3] |= (arr_10 [i_0] [i_1] [i_3]);
                    arr_14 [18] [i_1] [i_0] = (((arr_1 [1]) >= (0 >= -2584987698901001933)));
                    var_19 |= -13;
                }
                var_20 = (arr_3 [17] [17]);
            }
        }
    }
    var_21 = (((((~((max(64, var_16)))))) ? (min(var_0, 0)) : ((min(var_9, var_2)))));
    #pragma endscop
}
