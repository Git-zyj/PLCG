/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82743
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82743 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82743
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 -= (max((~var_15), (((!((max(9, 5))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_21 = (max(var_21, ((!(((arr_0 [i_1]) && (!5)))))));
                var_22 *= (max((max((arr_4 [i_1] [i_1] [i_0]), (arr_2 [i_0] [8]))), (arr_0 [i_1])));
                var_23 -= ((((arr_3 [i_1]) - (arr_3 [i_1]))));
                arr_6 [0] = (arr_3 [i_0]);
            }
        }
    }
    var_24 += ((((var_14 || (((var_4 ? var_3 : var_7))))) ? var_19 : ((var_13 ? (var_16 >= var_18) : ((var_7 ? 9 : 7))))));
    #pragma endscop
}
