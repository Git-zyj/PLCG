/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 640
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=640 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/640
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((1695337279 ? (var_6 - var_4) : var_10));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] = var_9;
                var_16 ^= (min((~7834399891397970101), ((min((arr_4 [i_1] [2] [i_0]), (arr_4 [i_0] [2] [i_0]))))));
            }
        }
    }
    var_17 = ((((min(var_11, (max(10612344182311581532, 1))))) ? var_4 : var_13));
    var_18 = (min(var_18, (((-(max((min(var_13, var_2)), var_5)))))));
    #pragma endscop
}
