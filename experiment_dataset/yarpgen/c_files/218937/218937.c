/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218937
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218937 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218937
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((var_0 ? ((20339 ? ((min(32280, var_1))) : (max(-102, var_7)))) : var_6));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_5 [i_0] = ((var_0 ^ (((120 & (arr_4 [i_0] [i_0] [i_1]))))));
                arr_6 [i_0] = min(((((((1 < (arr_2 [i_0] [i_1])))) > ((((arr_2 [i_1] [i_0]) == (arr_0 [i_0] [i_0]))))))), (arr_4 [i_0] [i_1] [i_0]));
            }
        }
    }
    var_13 = (var_0 >= var_4);
    #pragma endscop
}
