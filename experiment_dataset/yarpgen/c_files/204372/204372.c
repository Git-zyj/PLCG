/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204372
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204372 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204372
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min((((((var_6 ? var_1 : var_10))) ? var_1 : var_0)), -var_0));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [i_0] = -65535;
                var_13 = arr_4 [6] [i_0] [i_1 - 2];
                var_14 = arr_1 [i_1];
                arr_6 [i_1] [i_0] [i_0] = (!var_2);
                var_15 = ((((min((arr_0 [i_0] [i_1 - 1]), (((arr_4 [i_0] [i_0] [i_1]) ? var_9 : var_2))))) ? ((max((arr_4 [i_0] [i_1] [i_1]), var_4))) : (((!(~24232))))));
            }
        }
    }
    #pragma endscop
}
