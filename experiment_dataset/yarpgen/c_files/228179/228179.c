/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 228179
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=228179 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/228179
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((var_12 && (((max(5039, 0))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_16 = var_10;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 17;i_3 += 1)
                    {
                        {
                            arr_11 [i_0] [i_3] [i_2] [i_2] = (((((((-(arr_1 [i_3]))) / var_2))) | (arr_3 [i_0] [i_0] [i_2])));
                            var_17 = (max(var_17, var_6));
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_4 = 0; i_4 < 19;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 19;i_5 += 1)
                    {
                        {
                            arr_19 [i_0] [i_1] [i_1] [i_1] [i_4] [i_5] = var_8;
                            var_18 = ((((arr_10 [i_0] [i_0] [i_0] [i_0]) ? (arr_10 [i_0] [i_1] [i_0] [i_5]) : (arr_10 [i_4] [i_1] [i_4] [i_5]))));
                        }
                    }
                }
                var_19 = (-9223372036854775807 - 1);
            }
        }
    }
    #pragma endscop
}
