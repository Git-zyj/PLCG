/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22083
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22083 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22083
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_1] [7] = (arr_2 [i_0]);
                var_20 = (((!var_15) ? ((-((var_8 ? (arr_3 [i_0] [i_0]) : var_7)))) : (max((arr_2 [i_1]), (arr_0 [i_0])))));
            }
        }
    }
    var_21 = (max(var_21, 64663));
    var_22 = (min(var_22, var_11));
    var_23 = (max(var_8, ((((max(var_4, var_9))) / ((var_19 ? var_16 : var_14))))));
    #pragma endscop
}
