/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204282
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204282 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204282
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_16 = ((((min(10772514876771545546, var_2))) ? (((((4897268247405680173 ? var_2 : (arr_5 [i_0] [i_1] [i_1]))) == (arr_1 [i_0] [i_1])))) : -7674229196938006069));
                arr_6 [i_0] = (arr_4 [i_0]);
            }
        }
    }
    var_17 = (min(var_17, var_5));
    /* LoopNest 2 */
    for (int i_2 = 4; i_2 < 11;i_2 += 1)
    {
        for (int i_3 = 2; i_3 < 11;i_3 += 1)
        {
            {
                arr_12 [i_2] |= (min((arr_4 [i_2]), ((min(var_14, (arr_5 [i_3 - 2] [i_3 + 1] [i_3]))))));
                var_18 -= (arr_7 [i_3 - 2] [i_2]);
            }
        }
    }
    var_19 = var_8;
    var_20 = var_13;
    #pragma endscop
}
