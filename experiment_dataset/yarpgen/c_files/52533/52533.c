/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52533
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52533 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52533
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_13 -= 23;
        var_14 = (arr_2 [i_0] [i_0]);
        var_15 = arr_2 [i_0] [i_0];
        var_16 = (((!(arr_1 [i_0] [i_0]))));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 19;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 16;i_4 += 1)
                        {
                            {
                                var_17 = (arr_0 [i_3] [i_4]);
                                var_18 = arr_10 [i_0] [i_1] [i_2] [i_3] [i_4] [i_0] [i_0];
                                var_19 = ((0 ? 0 : 1));
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 20;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 20;i_6 += 1)
                        {
                            {
                                var_20 = (min((((arr_7 [i_0] [i_6]) ? (arr_10 [i_0] [i_0 - 1] [i_0 - 1] [i_0] [i_0] [i_0] [i_0 - 1]) : (arr_8 [i_2] [i_2] [i_2] [i_2]))), ((min((max((arr_2 [i_0] [i_1]), 1)), (arr_7 [i_0 - 1] [i_0 - 1]))))));
                                var_21 = ((!((((~1) ? (arr_7 [i_0] [i_0 - 1]) : var_4)))));
                                var_22 = ((~(max(232, (0 + 1)))));
                                var_23 = ((max(1, 233)));
                                var_24 &= (arr_1 [i_1] [i_6]);
                            }
                        }
                    }
                    var_25 = (max(var_1, (min(-1, (arr_9 [i_2] [i_2] [i_2] [i_1] [i_0] [i_0])))));
                    var_26 = var_5;
                }
            }
        }
    }
    var_27 ^= ((var_12 ? var_6 : ((var_6 ? ((max(var_2, var_6))) : var_12))));
    #pragma endscop
}
