/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117452
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117452 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117452
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 -= var_1;
    var_16 = 34359738368;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [i_1] = (((((((((arr_0 [i_0]) != var_10))) & (arr_1 [i_0 + 1])))) != (arr_4 [i_0 - 1] [i_0 - 1])));
                var_17 = (((((max(1, 90))) ? (((arr_1 [i_0]) + (arr_2 [i_0 + 2] [i_1]))) : ((var_13 ? (arr_2 [i_0] [i_0]) : var_3)))));
            }
        }
    }
    var_18 *= ((min(((-1 ? var_7 : 215)), ((max(var_7, var_10))))));
    var_19 *= ((~((min(31, -1)))));
    #pragma endscop
}
