/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 186011
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=186011 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/186011
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_14 = 45226;
                    arr_9 [i_0] [2] [i_2] [i_0] = ((+(((((var_7 ? (arr_6 [i_0] [10] [i_2] [i_2]) : 93))) ? ((var_7 ? 0 : 20291)) : ((1 ? var_5 : (arr_0 [i_0] [i_1])))))));
                    arr_10 [i_2] [i_0] [i_0] [i_0] = (!20310);
                }
            }
        }
    }
    var_15 = var_1;
    var_16 += (((((3 | (max(45226, var_5))))) ? ((var_13 ? ((var_9 << (45226 - 45221))) : (var_8 != 29651))) : -0));
    #pragma endscop
}
