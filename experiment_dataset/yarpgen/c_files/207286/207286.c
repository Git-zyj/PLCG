/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 207286
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=207286 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/207286
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 15;i_2 += 1)
            {
                {
                    var_13 *= var_0;

                    /* vectorizable */
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        var_14 = (!var_4);
                        var_15 += 17064;
                        arr_9 [i_1] [i_1] = -5546179091762191173;
                    }
                    /* vectorizable */
                    for (int i_4 = 0; i_4 < 16;i_4 += 1)
                    {
                        arr_12 [i_0] [i_1] [i_1] [i_1] [i_4] = ((-17064 | (arr_11 [i_2] [i_2 - 2] [i_2] [i_2 - 2])));
                        arr_13 [11] [i_1] [i_2] [i_1] [i_4] [1] = ((var_12 ? 8702055729047266659 : (arr_6 [i_2 - 4])));
                    }
                    var_16 = 48462;
                    var_17 = (min(var_17, (17064 | 17074)));
                }
            }
        }
    }
    var_18 = var_4;
    #pragma endscop
}
