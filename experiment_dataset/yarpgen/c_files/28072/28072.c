/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 28072
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=28072 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/28072
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_7;
    var_13 *= (~var_9);
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_14 = (min(var_14, ((min((((arr_3 [i_0]) >> ((((126 * (arr_2 [i_0]))) - 11334)))), (((var_2 | 0) | (((!(arr_3 [i_0])))))))))));
                arr_4 [i_0] [i_0] = arr_0 [i_1];
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 0;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_15 = (arr_1 [i_2] [i_4]);
                                arr_14 [i_1] [i_1] [i_1] [i_2] [i_1] [i_4] |= (arr_13 [i_0 + 1] [i_1] [i_2] [i_3] [1]);
                                var_16 = ((-((-var_4 % ((-32765 ? var_6 : var_2))))));
                            }
                        }
                    }
                }
                arr_15 [i_0] [i_0] [i_0] = (min((!var_1), ((var_9 ? 32755 : var_3))));
            }
        }
    }
    #pragma endscop
}
