/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99372
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99372 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99372
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(var_14, var_2));
    var_15 = (max(var_15, var_12));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_16 = (max(var_16, (((((max(var_9, var_8))) ? (((min((arr_0 [i_0] [i_0 + 1]), 93)))) : var_11)))));
                var_17 |= ((var_4 - (arr_0 [i_1] [i_0 + 1])));
                var_18 = (min(var_18, ((min((max((arr_3 [i_0 + 1]), (arr_3 [i_0 + 2]))), (min((arr_3 [i_0 + 2]), (arr_3 [i_0 - 1]))))))));
            }
        }
    }
    #pragma endscop
}
