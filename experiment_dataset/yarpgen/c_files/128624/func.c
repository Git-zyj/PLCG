/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 128624
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=128624 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/128624
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */unsigned short) var_15);
    var_18 = ((/* implicit */int) var_2);
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 0; i_0 < 19; i_0 += 1) 
    {
        var_19 = arr_0 [6U];
        arr_3 [i_0] &= ((/* implicit */unsigned long long int) arr_1 [i_0] [i_0]);
        var_20 += ((/* implicit */unsigned int) (-(var_14)));
        var_21 = ((/* implicit */int) arr_2 [i_0]);
    }
    var_22 *= ((/* implicit */unsigned short) min((((/* implicit */unsigned long long int) var_0)), (min((((/* implicit */unsigned long long int) var_8)), (((((/* implicit */_Bool) var_3)) ? (((/* implicit */unsigned long long int) var_12)) : (var_5)))))));
}
