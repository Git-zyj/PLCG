/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90268
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90268 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90268
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 *= ((/* implicit */short) ((((/* implicit */_Bool) (-((+(((/* implicit */int) (signed char)1))))))) ? (((/* implicit */long long int) ((min((var_4), (((/* implicit */unsigned int) var_0)))) & (((/* implicit */unsigned int) ((/* implicit */int) (unsigned char)67)))))) : (var_3)));
    /* LoopSeq 1 */
    for (unsigned short i_0 = 0; i_0 < 10; i_0 += 3) 
    {
        var_14 = ((/* implicit */long long int) var_5);
        arr_2 [i_0] |= var_11;
        arr_3 [(short)0] = ((/* implicit */int) var_7);
    }
}
