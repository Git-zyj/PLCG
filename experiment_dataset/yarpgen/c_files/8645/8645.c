/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 8645
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=8645 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/8645
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] [i_1] = 2130706432;
                var_12 = (min(((~(arr_0 [i_0 - 2] [i_0 - 2]))), (((arr_3 [i_0] [i_0 - 2]) ? (arr_3 [i_0] [i_0 - 2]) : (arr_3 [i_0] [i_0 - 2])))));
                var_13 += var_3;
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 0; i_2 < 23;i_2 += 1)
    {
        for (int i_3 = 3; i_3 < 20;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 23;i_4 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_5 = 1; i_5 < 21;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 23;i_6 += 1)
                        {
                            {
                                var_14 = (max(1, 96));
                                var_15 = (arr_6 [12] [i_3 + 2]);
                                var_16 = var_7;
                            }
                        }
                    }
                    var_17 ^= var_4;
                }
            }
        }
    }
    var_18 |= (max(var_8, 2164260864));
    #pragma endscop
}
