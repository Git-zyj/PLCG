/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 225771
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=225771 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/225771
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((((var_16 ? (1 && var_7) : var_11))) || (!0));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] [i_1] = 1;
                arr_5 [i_0] = ((((51669 ^ ((60526 | (arr_0 [i_1]))))) > 65535));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 4; i_2 < 16;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 17;i_3 += 1)
        {
            {
                var_21 = (max(var_21, (((~((min((arr_8 [i_2] [i_3]), (arr_8 [i_2] [i_3])))))))));
                var_22 = ((!((max((arr_9 [i_2]), (arr_9 [i_2 - 4]))))));
            }
        }
    }
    var_23 = (((var_9 | var_19) || var_5));
    var_24 = 3354340366;
    #pragma endscop
}
