/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 205359
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=205359 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/205359
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_9;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                {
                    arr_6 [1] [i_1] [i_1] [i_1] = min(31078, ((2147483647 ? 68 : (arr_2 [i_0]))));
                    var_16 = (min((((arr_5 [i_2 + 2]) ? (arr_2 [i_0]) : (arr_3 [i_1 + 1]))), 161));
                }
            }
        }
    }
    var_17 = ((var_12 == ((var_5 ? (min(var_4, 18446744073709551615)) : (var_7 * var_14)))));
    #pragma endscop
}
