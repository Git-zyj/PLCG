/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 225827
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=225827 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/225827
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((8191 > ((max(255, 0)))));
    var_20 = ((0 ? ((((var_11 ? var_11 : var_3)))) : (min(var_4, var_11))));
    var_21 ^= (((((var_4 | (-1 - 36)))) ? var_17 : (((31 ? 216 : 94)))));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 10;i_2 += 1)
            {
                {
                    arr_9 [i_2 + 1] [i_1] [i_0 - 1] = (((~29) + (max(((~(arr_5 [i_0]))), 0))));
                    var_22 = (max((((arr_4 [i_1] [i_1] [i_2 + 1]) || var_13)), (((arr_6 [i_0 - 2] [i_1 + 1] [i_1]) && var_1))));
                }
            }
        }
    }
    #pragma endscop
}
