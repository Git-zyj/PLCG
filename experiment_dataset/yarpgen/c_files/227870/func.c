/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227870
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227870 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227870
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (int i_0 = 1; i_0 < 21; i_0 += 4) 
    {
        var_17 = ((/* implicit */long long int) ((unsigned short) ((unsigned int) var_14)));
        var_18 = ((/* implicit */unsigned short) min((var_18), (((/* implicit */unsigned short) var_6))));
    }
    var_19 ^= ((/* implicit */unsigned short) var_6);
    var_20 = ((/* implicit */_Bool) min((((unsigned long long int) max((var_13), (((/* implicit */unsigned long long int) var_0))))), (((/* implicit */unsigned long long int) (-(var_14))))));
}
