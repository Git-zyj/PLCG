/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175933
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175933 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175933
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 4; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        {
                            var_17 = (-(((arr_7 [i_1] [i_2 - 1] [i_3]) ? (arr_7 [i_0] [i_2 + 3] [i_0]) : (arr_7 [i_1] [i_2 + 1] [i_1]))));
                            arr_11 [i_3] [i_2] [i_1] [i_1] [i_0] [i_0] = (-(arr_5 [i_0]));
                            var_18 = 7168;
                        }
                    }
                }
                var_19 = ((arr_4 [i_0]) ? (min((arr_2 [i_0]), 3147770177698863806)) : 3147770177698863807);
            }
        }
    }
    var_20 -= (((var_1 ? var_14 : var_16)));
    #pragma endscop
}
