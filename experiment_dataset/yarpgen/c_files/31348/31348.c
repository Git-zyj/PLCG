/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 31348
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=31348 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/31348
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_20 = (~1);
        var_21 *= ((1268792861 ? ((((((var_2 != (arr_1 [i_0]))))) & (-112 / 2323675049))) : (arr_0 [i_0])));
    }
    var_22 &= (max(((((var_5 + 2147483647) << (17079074237626129493 - 17079074237626129493)))), (min(var_4, -1971292246))));
    var_23 = (min(var_23, (((249 * (var_18 && var_17))))));
    #pragma endscop
}
