/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195799
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195799 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195799
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 ^= var_6;
    var_18 = (min((((!(!var_4)))), ((var_2 ? -var_11 : (((var_10 ? 28644 : var_6)))))));
    var_19 = ((var_11 ? var_14 : (max((max(var_5, var_6)), var_0))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_20 += (((arr_0 [i_0]) ? -690624630 : (((((var_5 ? (arr_0 [i_0]) : (arr_3 [20])))) | (arr_5 [i_0] [i_1])))));
                arr_6 [i_0] = (((arr_2 [i_0] [i_0]) ? (!var_11) : ((((arr_2 [i_0] [i_0]) <= 423180930)))));
            }
        }
    }
    #pragma endscop
}
