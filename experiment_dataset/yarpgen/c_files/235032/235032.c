/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235032
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235032 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235032
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 |= 33267;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] = (max(((var_4 ? (arr_4 [i_0] [i_1]) : (((arr_0 [i_0]) ? var_11 : (arr_2 [i_1]))))), (((arr_0 [i_0]) ? (arr_0 [i_1]) : (arr_4 [i_1] [i_0])))));
                var_15 = (((((var_2 ? var_5 : (arr_3 [i_1])))) ? (~1) : (arr_3 [i_1])));
            }
        }
    }
    var_16 = (max(var_16, (((59262 > (-10529 <= 1060194133))))));
    #pragma endscop
}
