/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124045
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124045 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124045
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */long long int) (+((+(var_0)))));
    var_18 = ((/* implicit */unsigned long long int) var_11);
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 21; i_0 += 4) 
    {
        for (signed char i_1 = 0; i_1 < 21; i_1 += 4) 
        {
            {
                var_19 = ((/* implicit */unsigned char) min((((/* implicit */int) (unsigned char)178)), (((((/* implicit */int) arr_2 [i_0] [i_1] [i_1])) % (-716996644)))));
                arr_4 [i_0] = ((((/* implicit */_Bool) (~(((/* implicit */int) var_11))))) ? ((~(762645413U))) : (((((/* implicit */_Bool) ((int) var_1))) ? (((/* implicit */unsigned int) ((/* implicit */int) arr_2 [i_0] [i_1] [i_1]))) : (var_14))));
                arr_5 [i_1] = ((/* implicit */unsigned short) var_4);
            }
        } 
    } 
    var_20 += ((/* implicit */int) var_13);
    var_21 ^= ((/* implicit */unsigned char) (~(((/* implicit */int) var_16))));
}
