#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

signed char var_1 = (signed char)22;
unsigned char var_2 = (unsigned char)166;
unsigned short var_3 = (unsigned short)9873;
int var_4 = -1219142847;
unsigned char var_6 = (unsigned char)40;
unsigned short var_7 = (unsigned short)918;
unsigned int var_8 = 3287375264U;
_Bool var_10 = (_Bool)0;
int var_12 = 327609372;
int zero = 0;
signed char var_14 = (signed char)30;
signed char var_15 = (signed char)-42;
int var_16 = -692267742;
void init() {
}

void checksum() {
    hash(&seed, var_14);
    hash(&seed, var_15);
    hash(&seed, var_16);
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
