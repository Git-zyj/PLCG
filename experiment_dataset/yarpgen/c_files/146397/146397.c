/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146397
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146397 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146397
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_5;

    /* vectorizable */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        arr_2 [i_0] = (arr_0 [i_0]);
        arr_3 [i_0] [i_0 + 2] = ((var_10 && (((var_8 ? var_7 : var_10)))));
    }
    for (int i_1 = 0; i_1 < 24;i_1 += 1)
    {
        var_16 = ((!((min(var_1, var_8)))));
        var_17 *= var_0;
        arr_7 [i_1] = ((!((((234131008781162215 ? -6880923581776712216 : var_4))))));
    }
    var_18 &= (min(var_1, ((var_2 ? var_10 : (var_11 <= 117)))));
    #pragma endscop
}
