#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

unsigned char var_2 = (unsigned char)156;
unsigned char var_3 = (unsigned char)123;
signed char var_6 = (signed char)-73;
unsigned char var_8 = (unsigned char)110;
unsigned long long int var_9 = 8574382467264856706ULL;
_Bool var_11 = (_Bool)0;
unsigned short var_12 = (unsigned short)1463;
int zero = 0;
unsigned char var_14 = (unsigned char)180;
int var_15 = -1116205787;
unsigned int var_16 = 1206998042U;
unsigned int var_17 = 2876648291U;
void init() {
}

void checksum() {
    hash(&seed, var_14);
    hash(&seed, var_15);
    hash(&seed, var_16);
    hash(&seed, var_17);
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
