/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127315
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127315 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127315
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(var_7, (((var_9 >= 3568180557) >= 726786738))));

    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        arr_4 [i_0] = (((((~(~var_5)))) ? (((~var_2) ? ((((arr_0 [1]) - (arr_1 [i_0] [i_0])))) : (((arr_3 [i_0]) & (arr_3 [i_0]))))) : var_7));
        var_15 = arr_2 [i_0] [i_0 - 1];
    }
    var_16 = (32767 && 1);
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 10;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 10;i_2 += 1)
        {
            {
                var_17 = var_6;
                /* LoopNest 2 */
                for (int i_3 = 3; i_3 < 8;i_3 += 1)
                {
                    for (int i_4 = 2; i_4 < 9;i_4 += 1)
                    {
                        {
                            arr_16 [i_1] [i_1] = (arr_3 [i_4]);
                            var_18 = ((((min((arr_5 [i_1] [i_1]), (arr_5 [i_1] [i_1])))) ? (arr_5 [i_3 + 1] [i_1]) : ((((!(arr_5 [i_1] [i_1])))))));
                        }
                    }
                }
                var_19 = (max((min(((-(arr_0 [i_1]))), var_7)), 0));
            }
        }
    }
    #pragma endscop
}
