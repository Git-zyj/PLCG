/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149828
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149828 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149828
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (~((5947249618591014043 ? 3758096384 : 0)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_21 = (+(((arr_4 [i_0] [7]) ? (arr_5 [i_0]) : (max((arr_3 [i_0] [i_1]), var_13)))));
                arr_6 [i_0] [i_0] [i_1] = ((((+(((arr_1 [i_0] [i_0]) ? (arr_2 [i_0]) : var_15))))) ? (arr_2 [i_0]) : (arr_1 [i_0] [i_0]));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 16;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 16;i_3 += 1)
        {
            {
                var_22 = var_8;
                var_23 = var_15;
            }
        }
    }
    #pragma endscop
}
