/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239847
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239847 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239847
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((!((min((min(var_0, var_5)), 32)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_17 = (~65535);
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        {
                            var_18 += ((((min((arr_6 [i_1] [i_1]), var_3))) ? (min((var_2 && var_8), (var_1 * var_3))) : 844424930131968));
                            var_19 = (min(((min(((-844424930131968 ? var_7 : var_8)), ((min((arr_6 [i_1] [i_0]), 75)))))), (((((844424930131977 ? var_5 : var_15))) ^ (arr_0 [i_0])))));
                        }
                    }
                }
            }
        }
    }
    var_20 = var_1;
    #pragma endscop
}
