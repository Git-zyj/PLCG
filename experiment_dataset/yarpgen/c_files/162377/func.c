/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162377
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162377 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162377
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 &= ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) (+(((var_11) ? (((/* implicit */int) var_7)) : (((/* implicit */int) (unsigned char)0))))))) ? (var_8) : (((((/* implicit */_Bool) max((16252928U), (((/* implicit */unsigned int) var_4))))) ? (((/* implicit */int) var_1)) : ((-(((/* implicit */int) var_6))))))));
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 2; i_0 < 13; i_0 += 4) 
    {
        var_20 += ((/* implicit */short) var_0);
        var_21 = ((/* implicit */unsigned int) (signed char)-1);
    }
}
