/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 50009
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=50009 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/50009
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */unsigned long long int) var_1);
    /* LoopNest 2 */
    for (long long int i_0 = 3; i_0 < 9; i_0 += 3) 
    {
        for (unsigned short i_1 = 3; i_1 < 8; i_1 += 4) 
        {
            {
                arr_6 [i_1] [i_1] = ((/* implicit */unsigned long long int) ((((((((/* implicit */_Bool) var_1)) || (((/* implicit */_Bool) arr_0 [5ULL])))) || (((/* implicit */_Bool) var_7)))) ? (((/* implicit */int) ((((/* implicit */_Bool) min((((/* implicit */unsigned long long int) -1724827466)), (8967887886343605902ULL)))) || (((/* implicit */_Bool) arr_0 [i_0]))))) : (((/* implicit */int) ((((/* implicit */_Bool) arr_4 [i_1])) || (((/* implicit */_Bool) arr_4 [i_1])))))));
                var_11 = ((/* implicit */unsigned short) min((var_11), (((/* implicit */unsigned short) arr_1 [i_0]))));
            }
        } 
    } 
    var_12 = ((/* implicit */unsigned short) max((var_12), (((unsigned short) var_9))));
}
