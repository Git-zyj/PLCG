/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46659
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46659 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46659
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((-((max(var_0, var_1))))) != (var_11 ^ var_8)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 17;i_2 += 1)
            {
                {
                    var_13 *= -224177346891251106;
                    var_14 = (max(var_14, (((-(arr_3 [i_0] [i_1] [i_2]))))));
                    arr_8 [i_0] [i_2 + 2] = (min((((min(4294967295, (arr_0 [i_0] [i_0]))))), (arr_3 [i_0] [i_0] [i_2])));
                }
            }
        }
    }
    var_15 = (var_6 / ((2634159726 << (((var_3 & var_11) - 17706)))));
    #pragma endscop
}
