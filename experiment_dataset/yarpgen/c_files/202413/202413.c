/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202413
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202413 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202413
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = -4301159233233302568;
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 11;i_1 += 1)
        {
            {
                var_15 = (arr_4 [i_1 + 1]);
                var_16 = (((((arr_3 [i_0] [i_0 - 1] [i_1 - 2]) ? var_2 : (arr_3 [i_0 - 1] [i_1 - 1] [6]))) + (arr_3 [7] [i_0] [1])));
                var_17 -= ((var_0 ^ ((~(~1735844269767946847)))));
            }
        }
    }
    var_18 = (((((-(var_10 / 229)))) ? ((var_5 ? (var_4 | var_4) : (min(24, var_2)))) : var_0));
    #pragma endscop
}
