/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41319
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41319 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41319
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 += ((((min(((min(3992634123, -73))), 5170866602088935074))) == ((((min(var_7, -5048380702640852811))) ? ((var_3 ? var_18 : var_14)) : var_11))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_20 = (min(var_3, ((var_6 >> (var_9 && var_17)))));
                arr_6 [i_0] = (min((var_2 || var_11), (max(1, var_11))));
                var_21 = var_11;
            }
        }
    }
    #pragma endscop
}
