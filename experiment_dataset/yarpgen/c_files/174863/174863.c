/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174863
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174863 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174863
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (((((max(var_4, var_12)) * var_1)) * (var_11 * 0)));
    var_19 &= (var_3 & 224);
    var_20 = ((((!((max(1073741824, 29770))))) ? 31960 : (!-3495)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_21 = (min(var_21, 4212662861));
                arr_7 [i_1] [i_0] [15] = (((-(arr_1 [i_0] [i_0]))));
                var_22 = -12;
                arr_8 [i_0] [2] = (max((arr_1 [i_0] [i_1]), 3900719845));
            }
        }
    }
    #pragma endscop
}
