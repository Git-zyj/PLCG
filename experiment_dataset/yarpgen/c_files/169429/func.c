/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169429
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169429 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169429
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 2 */
    /* vectorizable */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) /* same iter space */
    {
        arr_3 [i_0] [i_0] = arr_2 [i_0];
        var_14 = ((/* implicit */unsigned short) ((((/* implicit */_Bool) arr_2 [i_0])) ? (((/* implicit */int) arr_2 [i_0])) : (((/* implicit */int) arr_2 [i_0]))));
        arr_4 [i_0] = ((/* implicit */unsigned int) ((int) (unsigned char)220));
    }
    /* vectorizable */
    for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) /* same iter space */
    {
        var_15 = (~(((int) arr_0 [i_1])));
        arr_7 [13U] [i_1] = ((/* implicit */unsigned int) ((arr_1 [i_1] [i_1]) < (((/* implicit */int) (unsigned char)186))));
    }
    var_16 = ((/* implicit */int) var_13);
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned short i_2 = 0; i_2 < 23; i_2 += 3) 
    {
        var_17 -= ((/* implicit */unsigned char) arr_9 [i_2]);
        arr_12 [i_2] = (unsigned short)61457;
    }
}
