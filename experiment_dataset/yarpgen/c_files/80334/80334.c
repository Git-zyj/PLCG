/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80334
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80334 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80334
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((((!var_10) | (min(1, -28350174)))) < (((var_0 >= var_0) ? 8 : ((var_6 ? var_10 : var_1))))));

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_12 *= (arr_0 [5] [i_0]);
        var_13 = var_3;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 22;i_1 += 1)
    {
        var_14 = 184;

        for (int i_2 = 0; i_2 < 22;i_2 += 1)
        {
            arr_8 [i_2] [i_2] = (~2974247710);
            var_15 = (var_6 && var_9);
            var_16 -= (((arr_2 [i_1]) <= (arr_2 [i_2])));
        }
        var_17 = ((+((var_3 ? (arr_3 [i_1]) : 73))));
        var_18 = 25;
        var_19 -= ((~((2974247710 ? (arr_4 [i_1] [1] [20]) : (arr_4 [i_1] [i_1] [i_1])))));
    }
    var_20 = (min(var_9, var_8));
    #pragma endscop
}
