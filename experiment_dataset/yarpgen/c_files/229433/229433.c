/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229433
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229433 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229433
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = (((((arr_0 [i_0] [i_0]) ? var_11 : ((42 << (38356 - 38337))))) & (((4924453217485964083 ? 1 : 38355)))));
        var_15 = (min(var_15, (arr_1 [i_0])));
        arr_4 [i_0] = ((33720 ? 20886 : ((min(38356, var_3)))));
    }
    var_16 = (min(var_16, ((max(((((206 * var_9) ? 1023 : var_5))), (var_7 * var_6))))));
    var_17 ^= var_0;
    #pragma endscop
}
