/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48793
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48793 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48793
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    var_12 = (max(((min((arr_6 [i_1 + 1] [i_1 + 1]), (arr_6 [i_1 + 1] [i_1 - 1])))), (arr_7 [i_1])));
                    var_13 = max((min((max((arr_7 [18]), (arr_4 [i_2]))), (arr_6 [i_1 + 1] [i_1 + 1]))), (max((arr_7 [i_1 - 1]), (arr_7 [i_1 + 1]))));
                }
            }
        }
    }
    var_14 = var_4;
    var_15 += ((((((var_3 - var_5) - var_9))) ? var_10 : var_11));
    #pragma endscop
}
