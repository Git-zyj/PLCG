/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 126439
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=126439 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/126439
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 |= ((/* implicit */signed char) var_10);
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 15; i_0 += 3) 
    {
        for (long long int i_1 = 0; i_1 < 18; i_1 += 3) 
        {
            for (long long int i_2 = 3; i_2 < 14; i_2 += 3) 
            {
                {
                    var_19 = ((/* implicit */unsigned int) ((long long int) (!(((/* implicit */_Bool) arr_2 [i_0])))));
                    arr_8 [i_0] [(unsigned short)5] = ((/* implicit */signed char) max((((/* implicit */unsigned short) (!(((/* implicit */_Bool) (-(((/* implicit */int) var_3)))))))), (var_3)));
                }
            } 
        } 
    } 
}
