/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27191
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27191 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27191
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */short) max(((+(var_13))), (((/* implicit */long long int) var_1))));
    var_15 = ((/* implicit */short) (!(((/* implicit */_Bool) ((((/* implicit */_Bool) 1842731889)) ? ((-(1205207777))) : (((/* implicit */int) var_9)))))));
    var_16 ^= (-(((/* implicit */int) var_7)));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 10; i_0 += 3) 
    {
        arr_3 [i_0] = ((/* implicit */unsigned int) arr_0 [i_0]);
        var_17 = ((/* implicit */unsigned int) (!(arr_1 [i_0])));
        arr_4 [i_0] &= ((/* implicit */unsigned short) (short)-10745);
        arr_5 [i_0] [i_0] = ((/* implicit */_Bool) min((max((((/* implicit */unsigned int) (unsigned char)32)), (1303012088U))), (2772598443U)));
        var_18 = (short)32767;
    }
    var_19 = ((/* implicit */short) var_3);
}
