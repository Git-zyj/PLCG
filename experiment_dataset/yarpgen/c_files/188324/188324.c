/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 188324
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=188324 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/188324
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_6 [i_0] = min((((arr_2 [i_0]) / ((max((arr_0 [i_0]), (arr_1 [i_0])))))), ((min((min((arr_0 [i_1]), (arr_1 [i_0]))), (((var_1 && (arr_0 [i_1]))))))));
                var_10 = (min(((max(((var_5 ? -7786290659505025293 : (arr_1 [i_1]))), (~var_1)))), var_7));
            }
        }
    }
    var_11 = ((var_7 * ((((((32765 ? var_7 : var_6))) ? ((var_0 ? var_0 : var_3)) : (90 | -125))))));
    var_12 = var_0;
    #pragma endscop
}
