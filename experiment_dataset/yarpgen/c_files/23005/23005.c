/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23005
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23005 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23005
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (!6);
    var_21 = (max(var_21, var_8));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_7 [i_0] [i_1] [i_1] = ((var_5 ? var_14 : (max((min(var_17, var_8)), 252))));
                var_22 = (max(var_22, (((!(((((!(arr_3 [i_0] [0] [i_1]))) ? var_4 : (arr_2 [2])))))))));
                arr_8 [i_0] = ((-(arr_5 [i_0] [i_0] [i_0])));
                arr_9 [i_0] = ((-(((((var_6 ? var_11 : 2))) ? (arr_2 [i_0]) : ((min(var_3, var_6)))))));
            }
        }
    }
    #pragma endscop
}
