/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40440
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40440 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40440
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned char i_0 = 3; i_0 < 18; i_0 += 2) 
    {
        for (int i_1 = 0; i_1 < 21; i_1 += 2) 
        {
            {
                var_18 = ((/* implicit */unsigned long long int) arr_2 [i_0]);
                arr_4 [i_0] = min((((/* implicit */int) var_7)), (((((/* implicit */_Bool) arr_3 [i_0] [i_1])) ? (((/* implicit */int) arr_3 [i_0] [i_1])) : (((/* implicit */int) (signed char)116)))));
                var_19 = ((/* implicit */unsigned char) ((unsigned int) (+(1241471537U))));
                var_20 += ((/* implicit */short) arr_3 [i_1] [i_1]);
            }
        } 
    } 
    var_21 = ((/* implicit */signed char) ((unsigned char) min((var_17), (((/* implicit */long long int) ((((/* implicit */_Bool) var_15)) ? (((/* implicit */int) var_10)) : (((/* implicit */int) (short)-23055))))))));
}
