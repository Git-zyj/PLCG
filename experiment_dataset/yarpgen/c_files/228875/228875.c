/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 228875
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=228875 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/228875
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_16 |= var_4;
                    var_17 = (max((min(((var_6 ? var_15 : var_2)), ((min(-75, (arr_6 [i_0] [i_1] [i_1] [i_1])))))), (((!(arr_1 [i_0]))))));
                    var_18 = (((~1) && 22));
                }
            }
        }
    }

    for (int i_3 = 2; i_3 < 20;i_3 += 1) /* same iter space */
    {
        var_19 = ((arr_8 [i_3 - 2] [7]) && ((22 && (max(1, 1)))));
        var_20 -= (((var_5 ? (arr_8 [i_3 - 1] [i_3 + 1]) : (max(var_6, var_1)))));
    }
    /* vectorizable */
    for (int i_4 = 2; i_4 < 20;i_4 += 1) /* same iter space */
    {
        var_21 = (((arr_8 [i_4 - 2] [i_4]) - var_6));
        var_22 += -1;
        /* LoopNest 3 */
        for (int i_5 = 0; i_5 < 21;i_5 += 1)
        {
            for (int i_6 = 0; i_6 < 21;i_6 += 1)
            {
                for (int i_7 = 2; i_7 < 18;i_7 += 1)
                {
                    {
                        var_23 = (((arr_17 [i_7 - 1]) % (arr_17 [i_7 + 1])));
                        var_24 = (var_2 > ((((var_8 < (arr_19 [i_7] [i_5] [i_5]))))));
                    }
                }
            }
        }
    }
    for (int i_8 = 2; i_8 < 20;i_8 += 1) /* same iter space */
    {
        var_25 = ((((22 ? (arr_16 [i_8 + 1] [i_8 - 2]) : (arr_16 [i_8 - 2] [i_8 + 1]))) + ((-(arr_16 [i_8 - 2] [i_8 - 2])))));
        var_26 = (arr_19 [2] [i_8] [2]);
    }
    #pragma endscop
}
