/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150484
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150484 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150484
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 += 13673;
    var_11 = ((4294967295 ? (!253952) : var_1));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 20;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] = (((((((arr_4 [i_0] [i_1] [i_2 - 2]) * (arr_4 [i_0] [i_0] [i_0]))))) ? (((arr_2 [i_2 + 3] [i_2 + 1] [i_2]) ? (arr_2 [i_0] [i_1] [i_2 - 2]) : 0)) : (min(((min((arr_1 [i_0] [i_1]), (arr_6 [i_0] [i_1] [i_2] [i_2])))), ((2670272248337515932 ? (arr_2 [i_0] [i_1] [i_2 + 2]) : (arr_6 [i_0] [0] [i_1] [i_2])))))));
                    arr_9 [i_2] [i_1] [i_0] [i_0] = arr_7 [i_0];
                }
            }
        }
    }
    var_12 = 0;
    #pragma endscop
}
