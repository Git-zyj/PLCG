/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 129321
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=129321 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/129321
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */_Bool) var_16);
    var_18 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) var_0)) ? (96220619) : (((/* implicit */int) var_3))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (int i_0 = 0; i_0 < 17; i_0 += 1) 
    {
        var_19 = ((/* implicit */_Bool) (+(96220609)));
        var_20 = ((/* implicit */unsigned short) (+(((long long int) -96220620))));
        var_21 = ((/* implicit */long long int) arr_1 [i_0]);
    }
}
