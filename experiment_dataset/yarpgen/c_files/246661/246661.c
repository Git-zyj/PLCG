/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246661
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246661 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246661
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_17 = (min(var_17, ((((((max(-48, 24078)))) - (41 / var_15))))));
                var_18 -= (max(((var_15 - (arr_5 [i_0]))), ((var_8 - (arr_0 [i_0] [i_1])))));
            }
        }
    }
    var_19 = var_12;
    var_20 = (max(var_20, 54267));
    var_21 = (min(var_21, (((((max((max(54267, 63)), (var_0 < var_8)))) ? (((max(41, var_10)))) : ((var_7 | ((max(var_3, 11269))))))))));
    #pragma endscop
}
