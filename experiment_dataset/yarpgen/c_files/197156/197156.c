/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197156
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197156 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197156
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(var_14, var_13));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_2] [i_2] = (arr_2 [i_1]);
                    var_15 *= (~-1);
                    arr_9 [i_0] [i_1] [i_1] = (((arr_0 [i_2]) ? ((((!(arr_3 [i_0] [i_1]))))) : -7435605584667511751));
                }
            }
        }
    }

    for (int i_3 = 0; i_3 < 10;i_3 += 1)
    {
        arr_12 [i_3] [i_3] = ((+((var_0 ? ((min((arr_3 [12] [i_3]), -11062))) : ((~(arr_10 [4])))))));
        arr_13 [0] [i_3] |= ((-((min((arr_5 [i_3] [i_3] [i_3] [i_3]), (arr_5 [i_3] [8] [i_3] [i_3]))))));
    }
    #pragma endscop
}
