/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15383
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15383 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15383
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (((!(~-64))));
    var_11 = (max(-800060637, ((((min(var_1, 64))) ? ((1 ? var_8 : 1)) : (((var_2 ? var_3 : var_9)))))));
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 16;i_2 += 1)
            {
                {
                    arr_8 [i_2] [i_1] [i_1] [i_0 - 3] = ((((max((arr_5 [i_1 + 1] [i_1] [i_0 + 2]), (arr_5 [i_1 + 1] [i_1] [i_0 - 2])))) && (((2147483636 ? 64 : ((var_7 ? (arr_4 [i_1] [i_1]) : 17058932149429513473)))))));
                    var_12 = 1;
                }
            }
        }
    }
    #pragma endscop
}
