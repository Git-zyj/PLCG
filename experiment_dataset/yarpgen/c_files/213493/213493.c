/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213493
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213493 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213493
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 18;i_1 += 1)
        {
            {
                var_10 = 536870911;

                for (int i_2 = 1; i_2 < 19;i_2 += 1)
                {
                    var_11 = (min(var_0, (((var_9 ? var_9 : (arr_3 [i_2] [i_1]))))));
                    var_12 = (min(var_12, (((((((((arr_2 [i_0] [2]) ? 253 : 247))) ? var_3 : (((arr_4 [i_0] [i_0]) - (arr_7 [i_0]))))) + var_6)))));
                }
                /* vectorizable */
                for (int i_3 = 0; i_3 < 20;i_3 += 1)
                {
                    arr_11 [i_3] [i_1] &= ((63 ? (((-32767 - 1) ? 145 : 0)) : var_2));
                    var_13 = (var_7 || 16368);
                }
            }
        }
    }
    var_14 = ((1 ? (max(63, var_4)) : 63));
    #pragma endscop
}
