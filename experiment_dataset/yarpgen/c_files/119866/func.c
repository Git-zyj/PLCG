/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119866
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119866 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119866
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */unsigned long long int) (-(((((/* implicit */_Bool) 978368162)) ? (((/* implicit */int) var_9)) : (((/* implicit */int) ((unsigned char) var_8)))))));
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (long long int i_1 = 1; i_1 < 11; i_1 += 1) 
        {
            for (int i_2 = 3; i_2 < 10; i_2 += 2) 
            {
                {
                    arr_8 [i_1] [i_1] [i_0] [i_1] = ((/* implicit */long long int) arr_4 [i_2 - 1] [i_2 - 3] [i_1 + 1]);
                    arr_9 [i_0] [5] [i_1] = ((/* implicit */unsigned short) arr_2 [i_1 + 2]);
                }
            } 
        } 
    } 
}
