/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 69437
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=69437 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/69437
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 ^= ((((min(var_4, var_1))) ? var_7 : (((~var_18) ? var_7 : ((min(var_18, var_14)))))));
    var_20 |= ((var_0 ? var_10 : ((var_15 ? var_10 : -var_18))));
    var_21 = var_1;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    arr_8 [i_1] = ((((arr_1 [i_2] [i_1 + 1]) | (var_0 >= var_12))));
                    arr_9 [i_1] [i_1 - 1] = var_5;
                    arr_10 [i_0] [i_1] [18] = ((~(arr_2 [i_1 + 1] [i_1 + 1])));
                }
            }
        }
    }
    var_22 = var_18;
    #pragma endscop
}
