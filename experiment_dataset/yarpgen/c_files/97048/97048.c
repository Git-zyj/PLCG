/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97048
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97048 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97048
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_20 ^= (((arr_3 [i_0] [i_1] [i_0]) ? (((((-110 + 2147483647) << (7752 - 7752))))) : ((+(((arr_3 [i_2] [i_1] [i_0]) / (arr_4 [11] [i_2])))))));
                    arr_7 [i_1 + 1] = (((((((12568679298747778957 ? 57470 : 43685149311006715)) >= (arr_6 [i_0 + 2])))) >= (arr_2 [i_0] [i_1] [i_1])));
                }
            }
        }
    }
    var_21 = (min(var_21, ((min(var_8, ((~(var_12 >= -19))))))));
    #pragma endscop
}
