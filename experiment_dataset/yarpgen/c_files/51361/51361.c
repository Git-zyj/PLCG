/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51361
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51361 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51361
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_10 [i_0] [i_0] = 12;
                    var_19 = 6;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 24;i_4 += 1)
                        {
                            {
                                arr_16 [i_0] [i_1] [i_1] [i_2] [i_3] [i_0] [i_4] = (max(3705578431, 18446744073709551615));
                                arr_17 [i_4] [i_4] [i_0] [i_2] [i_0] [i_1] [i_0] = 1837690949;
                            }
                        }
                    }
                    var_20 = (min((arr_13 [i_0 + 1] [i_0 - 1] [i_0 - 2] [i_0] [i_0] [i_0 - 2]), (~1837690949)));
                }
            }
        }
    }
    var_21 = ((var_18 ? ((((!((min(var_2, var_17))))))) : var_18));
    #pragma endscop
}
