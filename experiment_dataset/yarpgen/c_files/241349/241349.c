/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241349
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241349 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241349
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 4; i_0 < 12;i_0 += 1)
    {
        var_16 = (-2147483647 - 1);
        arr_2 [i_0] = var_14;
        arr_3 [i_0] [i_0] = ((max((arr_0 [i_0]), (arr_0 [i_0]))));

        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            arr_8 [i_0] [i_1] = max(((~(max((arr_6 [3]), var_1)))), 1389445997);
            var_17 = (min(var_17, ((((var_14 ? var_9 : (arr_1 [i_0 - 2])))))));
        }
        for (int i_2 = 3; i_2 < 13;i_2 += 1)
        {
            arr_11 [i_0] = (arr_9 [9] [i_2]);
            arr_12 [i_2] [i_0] [i_0 - 1] = (arr_10 [i_2 - 2] [i_2 + 1]);
        }
    }
    var_18 = (max(var_18, (((-((~((min(var_11, var_3))))))))));
    var_19 = ((var_0 == ((~(min(8717632205678801975, -1465734498))))));
    var_20 = var_11;
    #pragma endscop
}
