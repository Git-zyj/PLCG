/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 207259
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=207259 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/207259
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, 0));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    arr_8 [i_2] [i_2] = (arr_4 [i_2] [5] [i_2]);
                    var_11 = (min(((((-var_1 + 2147483647) >> (((arr_0 [i_1] [i_1]) - 74))))), (min((arr_7 [i_0] [i_2] [i_0] [i_0]), (min((arr_0 [i_1] [i_2]), (arr_2 [9])))))));
                    arr_9 [i_2] [1] [1] = (~3094153847412355970);
                    var_12 = (min((max((arr_2 [i_0]), (arr_3 [i_1] [i_1] [i_0]))), (var_3 | var_6)));
                }
            }
        }
    }
    #pragma endscop
}
