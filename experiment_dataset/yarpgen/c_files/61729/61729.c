/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61729
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61729 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61729
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [i_0] = (arr_4 [i_0]);
                var_20 *= (var_0 ^ (arr_3 [i_0 + 1] [i_0 - 1] [i_1 + 2]));
            }
        }
    }

    for (int i_2 = 4; i_2 < 17;i_2 += 1)
    {
        var_21 = (max((arr_8 [i_2 - 3]), -22409));
        var_22 = ((~(max((((arr_6 [i_2]) ? (arr_6 [i_2]) : (arr_6 [i_2]))), (max((arr_6 [1]), (arr_6 [i_2])))))));
    }
    var_23 = ((~(max(((var_2 ? -22416 : var_16)), (!var_7)))));
    #pragma endscop
}
