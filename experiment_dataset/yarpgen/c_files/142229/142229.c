/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 142229
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=142229 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/142229
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        {

                            for (int i_4 = 0; i_4 < 14;i_4 += 1) /* same iter space */
                            {
                                arr_13 [i_4] [7] [i_2] [7] [i_1 - 1] [i_0] = (min(((-(arr_6 [i_0 - 1] [i_1 + 1] [i_3]))), (arr_6 [i_0 - 3] [i_1 - 1] [i_1 + 1])));
                                arr_14 [i_0 - 3] = var_11;
                                var_20 = (var_5 + var_18);
                            }
                            for (int i_5 = 0; i_5 < 14;i_5 += 1) /* same iter space */
                            {
                                var_21 = (min(1, 68719476735));
                                var_22 = (((-((var_8 ? var_15 : 0)))));
                            }
                            var_23 = (max(-68719476736, -410849321));
                        }
                    }
                }
                var_24 = ((-(2147483647 + 0)));
            }
        }
    }
    var_25 = (var_14 || var_18);
    #pragma endscop
}
