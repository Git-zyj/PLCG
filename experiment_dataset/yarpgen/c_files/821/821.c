/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 821
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=821 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/821
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min((min(var_11, (!102559621))), var_5));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_15 = (min(var_15, (((((arr_2 [i_0]) ? 9223372036854775804 : (arr_1 [i_0] [i_0])))))));
        var_16 = (arr_1 [i_0] [i_0]);
        arr_3 [i_0] [i_0] = var_10;
        arr_4 [i_0] [i_0] = (((arr_2 [3]) ? (arr_0 [i_0]) : ((4398046494720 ? var_11 : 119))));
    }
    var_17 = (min(var_17, (((((((var_11 - var_0) ? var_3 : (min(var_0, var_4))))) ? var_5 : var_10)))));
    #pragma endscop
}
