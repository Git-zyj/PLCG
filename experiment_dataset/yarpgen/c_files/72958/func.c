/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72958
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72958 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72958
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 ^= ((/* implicit */unsigned char) max((((/* implicit */unsigned long long int) 7486326776244860292LL)), ((-(min((((/* implicit */unsigned long long int) var_1)), (17287384756557590592ULL)))))));
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 1; i_0 < 21; i_0 += 3) 
    {
        for (short i_1 = 2; i_1 < 20; i_1 += 3) 
        {
            {
                var_20 -= ((/* implicit */unsigned long long int) ((1159359317151961024ULL) >= (((/* implicit */unsigned long long int) ((/* implicit */int) (_Bool)1)))));
                arr_6 [18U] [i_1] &= ((/* implicit */unsigned char) var_1);
                var_21 ^= ((/* implicit */_Bool) var_10);
            }
        } 
    } 
}
