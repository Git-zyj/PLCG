/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91921
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91921 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91921
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */_Bool) ((int) ((((((/* implicit */int) var_11)) - (((/* implicit */int) var_3)))) + (((((/* implicit */int) (short)2790)) + (((/* implicit */int) (short)-2786)))))));
    /* LoopNest 2 */
    for (signed char i_0 = 1; i_0 < 17; i_0 += 3) 
    {
        for (short i_1 = 1; i_1 < 20; i_1 += 2) 
        {
            {
                var_21 = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) (-(((/* implicit */int) arr_0 [i_0 + 3])))))));
                arr_4 [i_0] = ((/* implicit */unsigned char) ((short) (+(269859019))));
                var_22 = ((/* implicit */signed char) max((var_22), (((/* implicit */signed char) ((((int) var_14)) + (((/* implicit */int) ((((/* implicit */int) arr_0 [i_1 - 1])) <= (var_2)))))))));
                var_23 ^= ((/* implicit */_Bool) arr_3 [i_0] [i_1 - 1] [i_1]);
            }
        } 
    } 
    var_24 = ((/* implicit */int) (!(((/* implicit */_Bool) var_11))));
    var_25 = ((/* implicit */unsigned short) (_Bool)1);
    var_26 -= ((/* implicit */short) var_19);
}
