/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219286
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219286 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219286
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 15;i_2 += 1)
            {
                {
                    arr_10 [i_1] [i_1 - 1] [i_0] [i_1] = (arr_4 [i_2] [i_1]);
                    var_16 = (arr_3 [i_2]);
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_17 = (max(var_17, (arr_5 [i_2])));
                                var_18 = (arr_7 [i_2] [i_3] [i_3] [i_1]);
                                arr_15 [i_1] [i_3] [i_2] [i_1] [i_1] = arr_6 [i_2 - 1] [i_2 - 1];
                                var_19 = (arr_3 [i_1]);
                            }
                        }
                    }
                    var_20 |= (arr_11 [i_0] [i_1 + 2] [i_2] [i_0]);

                    for (int i_5 = 0; i_5 < 16;i_5 += 1)
                    {
                        var_21 = (arr_3 [i_5]);
                        var_22 = (arr_7 [i_2] [i_2] [1] [i_2]);
                        var_23 = (arr_16 [i_1]);
                    }
                }
            }
        }
    }
    #pragma endscop
}
