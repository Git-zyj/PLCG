/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 216474
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=216474 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/216474
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_17 *= (arr_0 [7]);
        var_18 = (max(var_18, (((arr_1 [i_0]) ? ((var_7 ? (var_5 && var_4) : (arr_1 [i_0]))) : ((((((arr_1 [i_0]) ? (arr_1 [0]) : var_4))) ? var_11 : var_12))))));
        var_19 = min(((var_1 ? var_4 : (arr_1 [i_0]))), var_4);
        var_20 = (((((arr_1 [i_0]) & (arr_0 [i_0]))) / (((var_15 ? (arr_1 [i_0]) : (arr_1 [i_0]))))));
    }
    var_21 = var_16;
    #pragma endscop
}
