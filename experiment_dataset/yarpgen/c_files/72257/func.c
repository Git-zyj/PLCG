/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72257
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72257 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72257
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned char) var_5);
    var_12 = ((/* implicit */unsigned short) (short)29288);
    /* LoopNest 3 */
    for (signed char i_0 = 0; i_0 < 13; i_0 += 1) 
    {
        for (unsigned short i_1 = 1; i_1 < 10; i_1 += 1) 
        {
            for (unsigned long long int i_2 = 1; i_2 < 11; i_2 += 1) 
            {
                {
                    var_13 = ((/* implicit */unsigned char) (-(((/* implicit */int) (unsigned char)64))));
                    arr_8 [i_0] = var_8;
                    arr_9 [i_0] [i_2] = ((/* implicit */unsigned long long int) (signed char)-69);
                }
            } 
        } 
    } 
    var_14 = ((/* implicit */unsigned short) max((var_14), (((/* implicit */unsigned short) 7728198185822148501ULL))));
}
