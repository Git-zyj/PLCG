/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89872
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89872 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89872
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_0 + 4] = var_11;
                var_19 = (max(var_19, (min((((-(arr_2 [i_0] [i_0] [4])))), ((-3613855963295116156 ? 137438887936 : var_14))))));
            }
        }
    }
    var_20 = (((((var_9 >> (var_2 + 384554450425586916))) << (((~var_6) + 61005)))));
    var_21 = (max(-137438887949, var_11));
    #pragma endscop
}
