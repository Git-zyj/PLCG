/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44246
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44246 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44246
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 24;i_0 += 1)
    {

        /* vectorizable */
        for (int i_1 = 3; i_1 < 22;i_1 += 1)
        {
            var_17 = (~var_1);
            var_18 *= ((!(arr_0 [i_1 - 3] [i_1 + 1])));
            var_19 = (arr_3 [i_1]);
        }
        var_20 = ((!((max(((max((arr_3 [i_0]), (arr_0 [i_0] [i_0])))), (arr_5 [11]))))));
    }
    var_21 = (min(4250730614, 6963574992062434658));
    var_22 = ((((var_11 / ((var_5 ? var_12 : var_16)))) & (max(((var_9 ? var_7 : var_4)), var_9))));
    var_23 = var_9;
    #pragma endscop
}
