/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 147220
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=147220 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/147220
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned long long int) ((((/* implicit */int) var_7)) - (((((/* implicit */_Bool) 0U)) ? (((/* implicit */int) (unsigned short)52422)) : (((/* implicit */int) (short)14297))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (_Bool i_0 = 1; i_0 < 1; i_0 += 1) 
    {
        var_13 = ((/* implicit */unsigned long long int) ((((/* implicit */int) arr_1 [i_0 - 1])) % (((/* implicit */int) arr_1 [i_0 - 1]))));
        var_14 *= ((/* implicit */short) var_7);
    }
}
