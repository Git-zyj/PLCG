/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52595
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52595 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52595
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 |= 17394;
    var_16 = var_3;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 14;i_1 += 1)
        {
            {
                var_17 = (max(var_17, ((max((((arr_4 [i_0] [i_1 - 1]) ? (arr_4 [i_1 + 1] [i_1 - 2]) : (arr_4 [i_1] [i_1 - 1]))), (max(((max((-2147483647 - 1), 31994))), (arr_4 [i_0] [i_1]))))))));
                arr_5 [i_1] [i_0] [i_1] = arr_0 [i_0];
                arr_6 [i_1] [i_1] = (min((min(-121, 1)), (!2147483647)));
            }
        }
    }
    var_18 = (-9223372036854775807 - 1);
    #pragma endscop
}
