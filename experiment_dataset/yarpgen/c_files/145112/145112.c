/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 145112
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=145112 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/145112
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_12 = (max(var_12, ((((((min((arr_4 [i_0]), 28031)))) ? (((var_3 / (arr_0 [i_0])))) : ((((arr_4 [i_1]) >> (((arr_2 [i_2]) - 62))))))))));
                    var_13 -= (((((-(arr_0 [i_0])))) & 8));
                }
            }
        }
    }
    var_14 = (((((12403506966212164165 >> (var_10 - 48389)))) ? ((-1409350579 & (var_7 & var_11))) : (((((var_4 ? var_10 : 1)) >> (616748171 - 616748170))))));
    #pragma endscop
}
