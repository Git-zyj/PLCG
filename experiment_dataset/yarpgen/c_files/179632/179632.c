/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179632
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179632 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179632
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = -3270091756113097130;

    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        var_15 = (((((((32748 ? (arr_1 [19] [17]) : var_0)) ^ (arr_2 [i_0] [i_0 + 2])))) ? 144115188075855871 : (~var_10)));
        var_16 = (max(var_16, ((((min((arr_0 [i_0 + 1] [i_0 - 1]), (arr_0 [i_0] [i_0 - 1]))) / 2654317975)))));
        var_17 *= (arr_3 [i_0 - 1]);
    }
    var_18 = ((((var_0 ? -32736 : -0)) ^ var_6));
    #pragma endscop
}
