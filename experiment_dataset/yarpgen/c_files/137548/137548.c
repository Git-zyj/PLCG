/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 137548
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=137548 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/137548
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_14 = (!var_10);
                    arr_6 [14] [i_1 + 3] = (((-2122528959167547406 - 32768) ? ((((var_11 ? (arr_3 [22]) : -61)))) : var_12));
                }
            }
        }
    }
    var_15 &= ((-61 % ((-(min(var_5, var_13))))));
    var_16 = (min(var_16, ((max((((max(var_7, var_5)) / 1808344619)), ((max(-55, 57))))))));
    #pragma endscop
}
