/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148302
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148302 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148302
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_11, (((63 ? (!var_15) : (var_0 && 2992874675))))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [2] |= (-8388608 < 513833377);
                arr_6 [i_0] [i_0] = ((~(-127 - 1)));
                var_20 = (max(var_20, ((((arr_3 [i_0 - 1] [i_0 + 1] [i_0 + 1]) != var_9)))));
                var_21 = var_0;
            }
        }
    }
    var_22 = ((!((((min(123, var_11))) || (((18157705753943906510 ? 1950585219 : var_15)))))));
    var_23 &= (max(-8388608, 262142));
    #pragma endscop
}
