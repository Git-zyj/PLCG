/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 192958
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=192958 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/192958
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (!var_12);
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_2] [i_2] = ((((((arr_4 [i_0 + 3] [i_0]) % (((arr_5 [i_0] [i_1] [i_0] [i_0]) ? (arr_2 [i_1]) : var_11))))) ? ((max(((-(arr_5 [i_0] [i_1] [i_2] [i_1]))), (min((arr_3 [i_0]), (arr_3 [18])))))) : 2825116779));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 21;i_4 += 1)
                        {
                            {
                                var_19 ^= var_13;
                                var_20 = arr_8 [i_2] [i_2] [i_2] [i_1] [i_0];
                                arr_12 [i_2] [i_1] [i_1] [10] [i_1] = (((((min(var_0, -2132919278)) ? ((min(118, 118))) : (((arr_10 [i_2] [i_2] [i_2] [i_2] [i_2] [i_2]) / var_17))))));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
