/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221724
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221724 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221724
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = 0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_19 = -var_10;
                var_20 -= ((+((~(((arr_2 [i_0] [i_1]) ? var_16 : var_15))))));
                var_21 = ((((-((var_14 >> (var_5 - 3445666199))))) <= var_17));
                var_22 = ((((((arr_4 [i_0]) ? -2 : 2919307702))) ? ((((arr_1 [i_0] [i_1]) != (arr_6 [i_0] [i_1])))) : (min((min(0, var_8)), (arr_6 [i_0] [i_1])))));
            }
        }
    }
    #pragma endscop
}
