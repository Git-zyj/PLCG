/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 108903
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=108903 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/108903
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, (((-(255 && var_16))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_18 ^= (~(min((((arr_2 [i_0] [i_1]) ? 7901575076187380208 : -7901575076187380208)), (arr_0 [i_0]))));
                var_19 += (arr_1 [i_1]);
                arr_4 [i_0] [i_1] = var_11;
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            arr_10 [i_0] [i_0] [i_2] [i_3] [i_0] = ((var_8 * ((((((250 ? 30752 : 255))) ^ -1240644429537262633)))));
                            var_20 = -7901575076187380194;
                            arr_11 [i_0] [6] = (arr_9 [i_0] [i_1] [0] [7]);
                            var_21 = var_0;
                        }
                    }
                }
            }
        }
    }
    var_22 = (var_7 | var_2);
    #pragma endscop
}
