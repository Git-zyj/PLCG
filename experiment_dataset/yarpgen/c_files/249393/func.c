/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249393
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249393 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249393
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */short) max((var_11), (((/* implicit */short) (-(((/* implicit */int) (_Bool)1)))))));
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 1; i_0 < 20; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 18; i_1 += 3) 
        {
            {
                var_12 = ((/* implicit */signed char) max((var_12), (((/* implicit */signed char) max(((+(((/* implicit */int) (_Bool)1)))), (max((((/* implicit */int) arr_0 [i_0 + 1])), ((+(((/* implicit */int) arr_2 [(unsigned short)16])))))))))));
                arr_6 [i_0] = ((/* implicit */int) arr_4 [i_0]);
            }
        } 
    } 
    var_13 = ((/* implicit */unsigned long long int) ((unsigned char) var_0));
}
