/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164911
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164911 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164911
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] |= ((((max((arr_5 [i_1]), var_16))) ? var_3 : ((((arr_4 [i_0] [i_0] [i_0]) || (arr_5 [i_0]))))));
                arr_7 [i_0] = (arr_1 [i_0]);
            }
        }
    }

    /* vectorizable */
    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        arr_10 [i_2] [i_2] = (arr_9 [i_2] [i_2]);
        arr_11 [i_2] [i_2] = (((((var_4 ? 3133977815 : (arr_8 [i_2] [i_2])))) ? -21 : (((arr_8 [i_2] [i_2]) ? var_14 : (arr_8 [i_2] [i_2])))));
        arr_12 [i_2] = (((arr_9 [i_2] [i_2]) >= (arr_8 [i_2] [i_2])));
    }
    /* vectorizable */
    for (int i_3 = 3; i_3 < 12;i_3 += 1)
    {
        arr_15 [i_3] = (arr_9 [i_3 + 1] [i_3 + 1]);
        arr_16 [i_3] = -102;
        arr_17 [i_3] = ((((arr_0 [i_3] [i_3]) || -21)));
    }
    var_20 = (+((((!20) && var_2))));
    #pragma endscop
}
