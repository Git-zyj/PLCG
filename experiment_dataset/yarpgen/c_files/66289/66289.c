/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66289
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66289 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66289
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((~65535) % var_4);
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    var_14 = ((-3535704753257120888 ? 73 : (((-5944839369639466852 >= 4243474386) * -8190))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_15 = 20681;
                                arr_16 [i_1] = ((-((692008109 ? (!0) : (arr_3 [i_0])))));
                                var_16 &= ((((((((var_3 ? var_11 : var_11))) ? var_7 : (arr_7 [i_0] [i_1 - 1] [i_2] [i_2])))) || var_0));
                            }
                        }
                    }
                }
            }
        }
    }
    var_17 = (((((!448317743) ? var_5 : (!var_11)))) ? var_4 : var_12);
    var_18 = (max(var_18, var_0));
    #pragma endscop
}
