/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89094
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89094 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89094
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    arr_6 [i_1] [i_0] [i_1] = (var_10 - ((((arr_5 [i_0] [i_1] [i_1]) ? -325543920 : 325543919))));
                    arr_7 [i_1] [i_0] [i_0] [i_1] = var_8;
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_13 = (min(((((arr_2 [i_0 - 1] [i_3 - 1]) != var_9))), -var_4));
                                arr_13 [i_0 - 1] [i_0 + 2] [i_1] [i_1] [i_3] [i_4] = ((!(((-(arr_10 [i_1] [i_1] [i_3 - 1] [i_1] [i_4]))))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_14 = var_0;
    var_15 = (var_3 || var_5);
    #pragma endscop
}
