/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51551
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51551 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51551
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_17 = arr_2 [12];
                    var_18 = arr_1 [i_2];
                }
            }
        }
    }
    var_19 = 230;
    var_20 = min(1, 82);
    /* LoopNest 3 */
    for (int i_3 = 0; i_3 < 23;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 23;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 23;i_5 += 1)
            {
                {
                    var_21 = max((!163), (arr_12 [i_3] [i_5] [i_5] [i_5]));
                    var_22 = 0;
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_6 = 4; i_6 < 23;i_6 += 1)
    {
        for (int i_7 = 0; i_7 < 24;i_7 += 1)
        {
            for (int i_8 = 0; i_8 < 24;i_8 += 1)
            {
                {

                    for (int i_9 = 1; i_9 < 23;i_9 += 1)
                    {
                        var_23 = (((arr_19 [10]) > var_4));
                        arr_24 [2] [i_6 - 1] [i_6 - 1] [i_6 - 1] [i_6 - 1] = min((min(var_9, (arr_14 [i_6]))), 222);
                        var_24 = ((~((-(arr_23 [i_6 - 2] [i_6 - 2] [i_6 - 4] [i_6 - 1])))));
                    }
                    var_25 = var_6;
                    var_26 = (((arr_17 [i_6 - 2] [i_7] [i_8]) ? var_9 : (arr_17 [i_6 - 1] [i_7] [i_8])));
                }
            }
        }
    }
    #pragma endscop
}
