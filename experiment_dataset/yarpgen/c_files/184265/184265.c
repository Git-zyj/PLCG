/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184265
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184265 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184265
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 &= ((min(var_8, 24543)));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 2; i_2 < 9;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 9;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 9;i_4 += 1)
                        {
                            {
                                arr_12 [2] [i_2] [i_2] [i_2] [i_0] = (arr_0 [i_0]);
                                arr_13 [i_0 + 1] [i_2] [i_2 + 1] [i_3] [i_4] = var_7;
                                var_20 &= ((-(min(24544, (arr_4 [i_1] [i_4])))));
                            }
                        }
                    }
                }
                arr_14 [i_0] [i_0] = (~((((((arr_8 [i_0] [4] [i_0 - 2] [i_1]) >> (((arr_1 [6]) + 3570658421070995025))))) | (min(var_17, var_8)))));
            }
        }
    }
    var_21 = var_8;
    #pragma endscop
}
