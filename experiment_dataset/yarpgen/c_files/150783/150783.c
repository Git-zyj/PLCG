/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150783
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150783 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150783
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                var_17 = var_14;
                var_18 = (min(var_18, var_13));

                for (int i_2 = 3; i_2 < 16;i_2 += 1)
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 17;i_4 += 1)
                        {
                            {
                                var_19 = var_5;
                                arr_14 [i_0] [i_1] [i_1] [i_3] [i_4] |= (((arr_0 [i_0]) || var_3));
                            }
                        }
                    }
                    var_20 = -85;
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 17;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 1;i_6 += 1)
                        {
                            {
                                arr_20 [i_0] [i_5] [i_2] [i_1] [i_0] = (((arr_16 [i_0] [i_1]) != (arr_19 [i_6] [i_5] [i_2] [i_1] [i_0])));
                                var_21 += (((arr_10 [i_1 + 1] [0] [i_0] [i_2 - 2]) ? (arr_5 [i_1 - 1] [i_1 + 2] [i_2 - 3]) : var_15));
                                arr_21 [i_0] [i_1] [i_1] [i_5] [i_6] = arr_0 [i_1];
                                var_22 = (arr_0 [i_0]);
                                var_23 += var_2;
                            }
                        }
                    }
                    arr_22 [i_0] [i_0] [i_2 - 2] = ((((arr_10 [i_2 - 3] [2] [i_2 - 1] [i_1 + 1]) >= 876098149)));
                }
            }
        }
    }
    #pragma endscop
}
