/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29107
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29107 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29107
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_16 = ((((arr_0 [i_0]) ? (arr_0 [i_1]) : 214)));
                arr_4 [11] [i_1] [i_1] = ((-var_13 ? -8192 : ((-(max(-1597224770, var_5))))));
            }
        }
    }
    var_17 = (((var_14 / var_5) & ((var_12 ? (((min(var_12, var_6)))) : ((var_11 ? 5140069979031517096 : var_6))))));
    var_18 = (max(-var_15, var_10));
    #pragma endscop
}
