/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86551
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86551 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86551
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */long long int) var_5);
    /* LoopSeq 1 */
    /* vectorizable */
    for (long long int i_0 = 0; i_0 < 24; i_0 += 3) 
    {
        arr_3 [i_0] = arr_0 [i_0];
        var_17 = ((/* implicit */unsigned short) max((var_17), (((/* implicit */unsigned short) (!(((/* implicit */_Bool) 2889163032U)))))));
        var_18 = ((/* implicit */_Bool) arr_1 [i_0]);
        arr_4 [4LL] [i_0] = ((/* implicit */_Bool) ((arr_0 [i_0]) * (arr_0 [i_0])));
    }
    var_19 = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) var_4))));
    var_20 = ((/* implicit */unsigned int) (-(23ULL)));
}
