/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34580
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34580 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34580
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] = ((((((!((!(arr_1 [i_0] [i_0]))))))) * ((((((arr_0 [1]) >> var_1))) % (min(281474976702464, 6099))))));
        var_16 |= max(((+(((arr_0 [15]) / var_9)))), ((((-9223372036854775807 - 1) == 16))));
        arr_3 [i_0] [11] = 38;
    }
    for (int i_1 = 0; i_1 < 25;i_1 += 1) /* same iter space */
    {
        arr_7 [1] = 4294967295;
        var_17 = 2015172577696799367;
    }
    var_18 = var_7;
    /* LoopNest 2 */
    for (int i_2 = 4; i_2 < 9;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 12;i_3 += 1)
        {
            {
                arr_12 [i_2] [10] [i_3] |= ((-((min((arr_6 [i_2 - 2]), (arr_6 [i_2 + 2]))))));
                var_19 = (min(var_19, (((-(arr_9 [i_2] [7]))))));
            }
        }
    }
    var_20 &= (+-0);
    #pragma endscop
}
