/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74890
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74890 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74890
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 8;i_2 += 1)
            {
                {
                    var_17 = (max((arr_7 [i_0] [i_0 - 2]), (min(15387832105737879134, (arr_7 [i_0] [i_0 - 2])))));
                    var_18 = (((arr_3 [9] [9]) != (((((arr_2 [i_0] [i_0]) ? (arr_3 [5] [i_2]) : (arr_0 [i_1])))))));
                    arr_8 [i_0] [i_1] [i_0] [i_0] = (~574208952489738240);
                }
            }
        }
    }
    var_19 = var_11;
    var_20 ^= (min(((var_7 ? var_1 : 6405664167079735631)), var_2));
    #pragma endscop
}
