/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190934
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190934 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190934
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */short) var_12);
    /* LoopNest 3 */
    for (unsigned char i_0 = 0; i_0 < 12; i_0 += 1) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            for (short i_2 = 1; i_2 < 8; i_2 += 3) 
            {
                {
                    var_20 = ((/* implicit */unsigned long long int) (+(((((/* implicit */_Bool) (unsigned char)136)) ? (((/* implicit */int) (signed char)3)) : (((/* implicit */int) (unsigned char)111))))));
                    arr_10 [(unsigned char)5] [(unsigned short)3] [(short)9] = ((/* implicit */short) (-((-(((/* implicit */int) arr_0 [i_2 + 1] [i_2 + 3]))))));
                }
            } 
        } 
    } 
}
