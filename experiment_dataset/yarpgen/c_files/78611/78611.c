/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78611
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78611 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78611
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    var_16 = (!12734380568740198348);
                    var_17 = min(-var_14, (2489424481 == -var_13));
                }
            }
        }
        var_18 = ((((!0) % var_11)));
        var_19 = var_5;
    }
    for (int i_3 = 0; i_3 < 15;i_3 += 1)
    {
        var_20 = ((7168 == 0) << var_12);
        var_21 = (!3459548049);
    }
    var_22 = (var_7 / var_1);
    var_23 = ((((((var_14 ? -66477597 : 12734380568740198348))) ? (max(2954032713, 1152921504338411520)) : (30891 <= var_1))));
    #pragma endscop
}
