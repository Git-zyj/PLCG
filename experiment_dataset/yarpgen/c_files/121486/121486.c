/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 121486
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=121486 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/121486
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (min((max(var_6, var_2)), (((var_6 + (var_10 % var_16))))));
    var_21 = (min(32357276, (max(var_13, ((max(var_18, var_8)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_5 [i_1] = ((-((-2758803634298672131 ? var_17 : var_10))));
                var_22 = var_9;
            }
        }
    }
    var_23 = (min(var_4, ((var_19 + (max(var_13, var_14))))));
    #pragma endscop
}
