/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74438
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74438 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74438
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] = ((-(min(8411694388132742977, (arr_4 [i_1])))));
                arr_6 [i_1] = ((+(max((arr_0 [i_0] [i_1]), (((arr_3 [i_0] [i_1]) - (arr_3 [i_0] [i_0])))))));
                arr_7 [1] [i_1] [i_1] = (!168379617845553000);
            }
        }
    }
    var_12 = ((max(var_3, ((2390752786 ? 72 : 168379617845553000)))));
    var_13 = var_1;
    #pragma endscop
}
