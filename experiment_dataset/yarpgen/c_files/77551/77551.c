/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77551
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77551 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77551
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_9;
    var_11 = var_7;
    var_12 = ((((-1610612736 > (2607901360 + -2211087441415037326))) ? ((((-145929688 >= ((var_0 ? var_7 : var_6)))))) : 4149037615));
    var_13 = (var_2 + var_6);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    arr_7 [i_1] [i_0] |= ((~((-(arr_5 [i_0] [i_1] [i_2])))));
                    arr_8 [i_0] [i_1] [i_2] [i_0] = (~(min(var_8, (arr_4 [i_2] [i_2] [11] [i_0]))));
                    var_14 -= var_6;
                    var_15 = (min(145929686, 3698844818));
                }
            }
        }
    }
    #pragma endscop
}
