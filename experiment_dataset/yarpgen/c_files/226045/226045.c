/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226045
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226045 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226045
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((max(var_10, var_13)));
    var_21 = (max(0, var_15));
    var_22 = var_16;

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = var_18;
        var_23 = (~18446744073709551593);
        var_24 = arr_1 [i_0];
        var_25 += ((var_11 > (((max(var_14, (arr_2 [i_0] [i_0])))))));
        arr_4 [i_0] [i_0] = (((!var_1) / (((min(var_4, (arr_1 [4])))))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 10;i_1 += 1)
    {
        var_26 = var_13;
        arr_8 [5] &= arr_2 [8] [17];
    }
    for (int i_2 = 1; i_2 < 10;i_2 += 1) /* same iter space */
    {
        arr_11 [i_2] = var_6;
        arr_12 [i_2] = ((((0 << (3303 - 3241))) + (min(var_8, 4561547775810517584))));
    }
    /* vectorizable */
    for (int i_3 = 1; i_3 < 10;i_3 += 1) /* same iter space */
    {
        /* LoopNest 3 */
        for (int i_4 = 0; i_4 < 14;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 14;i_5 += 1)
            {
                for (int i_6 = 0; i_6 < 14;i_6 += 1)
                {
                    {
                        arr_23 [9] [i_3] [i_4] [i_5] [i_6] = -1;
                        var_27 *= ((((arr_20 [i_3] [13] [5] [i_6]) ^ 960)));
                    }
                }
            }
        }
        arr_24 [i_3] [i_3 + 1] = (14 >> (18446744073709551615 - 18446744073709551566));
    }
    var_28 = ((max(1073741824, 4561547775810517584)) | var_17);
    #pragma endscop
}
