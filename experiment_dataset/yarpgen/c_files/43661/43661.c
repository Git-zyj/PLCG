/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43661
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43661 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43661
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 16;i_2 += 1)
            {
                {
                    arr_6 [i_2 + 2] [i_1] [i_0] = -var_14;
                    arr_7 [i_2] [i_1] [0] [i_0] = (((((arr_3 [i_1]) ? var_1 : (arr_4 [i_0] [15]))) % (((arr_4 [i_1] [i_1 + 1]) ? (arr_4 [i_0] [i_1 - 1]) : (arr_4 [i_1] [i_1 - 1])))));
                    var_16 = var_6;
                    var_17 = ((!(((arr_4 [i_1 - 1] [i_1 - 1]) >= (((arr_5 [i_0] [i_0] [i_2] [i_0]) ? var_7 : (arr_1 [i_1])))))));
                }
            }
        }
    }
    var_18 += var_8;
    var_19 = var_2;
    var_20 = (!0);
    #pragma endscop
}
