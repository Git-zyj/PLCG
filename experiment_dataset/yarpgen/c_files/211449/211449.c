/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211449
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211449 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211449
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 += var_7;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        {
                            arr_11 [i_0] [i_1] [i_0] [i_0] [i_1] [9] = (min(((max((arr_8 [i_3] [i_3] [i_3 - 1] [i_3 - 1] [0] [i_2]), (arr_8 [i_3] [i_3] [i_3] [i_3 - 1] [i_3] [i_3])))), ((1 ? ((-43 ? 38 : -43)) : 42))));
                            var_14 = (min(var_14, var_12));
                            var_15 = (min((~28208), var_4));
                        }
                    }
                }
                var_16 = (min(((~(((arr_0 [i_0]) ? (arr_9 [1] [i_1] [i_1] [i_1] [i_0]) : (arr_9 [i_0] [i_1] [i_0] [i_0] [i_0]))))), (min((var_0 >= 1917), (~var_9)))));
            }
        }
    }
    var_17 = (((-56 != 141) < var_5));
    var_18 = var_4;
    #pragma endscop
}
