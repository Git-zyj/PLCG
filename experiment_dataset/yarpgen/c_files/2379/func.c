/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 2379
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=2379 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/2379
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned short) ((((/* implicit */_Bool) var_5)) ? (((((/* implicit */_Bool) var_11)) ? (603803976) : (((/* implicit */int) var_7)))) : (((/* implicit */int) ((signed char) var_2)))));
    /* LoopSeq 2 */
    /* vectorizable */
    for (long long int i_0 = 0; i_0 < 13; i_0 += 2) 
    {
        arr_2 [(signed char)3] = ((/* implicit */unsigned long long int) ((((/* implicit */int) (short)16459)) * (((/* implicit */int) var_9))));
        arr_3 [1LL] [i_0] = ((/* implicit */unsigned char) (~(((/* implicit */int) arr_0 [9U]))));
    }
    for (int i_1 = 2; i_1 < 14; i_1 += 2) 
    {
        var_17 *= ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) var_9))));
        var_18 = ((/* implicit */long long int) ((_Bool) var_9));
    }
    var_19 = ((/* implicit */long long int) (_Bool)1);
}
