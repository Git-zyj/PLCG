/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 92802
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=92802 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/92802
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 ^= (max(var_3, (((((max(var_3, -6914)))) ^ ((-29799 ? var_4 : 65535))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_11 = (max(var_11, ((min(201326592, (min((var_6 | 29798), (max(18446744073508225005, var_7)))))))));
                var_12 = (18446744073508225039 ^ var_9);
                var_13 = max(var_3, (arr_3 [i_0]));
            }
        }
    }
    #pragma endscop
}
