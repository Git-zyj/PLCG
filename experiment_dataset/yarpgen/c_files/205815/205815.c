/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 205815
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=205815 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/205815
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = 4294967295;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 14;i_2 += 1)
            {
                {
                    var_11 = ((var_7 != (max(((min((arr_3 [4] [i_1] [i_0]), (arr_2 [i_0] [i_0])))), (arr_3 [i_2] [15] [i_0])))));
                    var_12 ^= ((((max((max((arr_5 [i_0] [i_0]), 4294967295)), (min(4294967295, 12))))) ? ((max((-127 - 1), (arr_2 [12] [12])))) : -31498));
                }
            }
        }
    }
    #pragma endscop
}
