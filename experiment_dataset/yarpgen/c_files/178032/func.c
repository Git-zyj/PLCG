/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 178032
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=178032 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/178032
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (short i_0 = 3; i_0 < 12; i_0 += 4) 
    {
        for (int i_1 = 0; i_1 < 13; i_1 += 1) 
        {
            for (int i_2 = 0; i_2 < 13; i_2 += 1) 
            {
                {
                    var_12 = ((/* implicit */unsigned short) max((var_12), (((/* implicit */unsigned short) (+(((/* implicit */int) arr_8 [i_2] [i_1] [i_0 + 1])))))));
                    /* LoopNest 2 */
                    for (unsigned short i_3 = 0; i_3 < 13; i_3 += 2) 
                    {
                        for (unsigned char i_4 = 0; i_4 < 13; i_4 += 4) 
                        {
                            {
                                arr_14 [i_3] [i_1] [i_0] = ((/* implicit */short) arr_5 [i_2]);
                                var_13 = ((/* implicit */signed char) (-(8273592616540878640ULL)));
                                var_14 ^= ((/* implicit */unsigned long long int) 1107254453U);
                            }
                        } 
                    } 
                    var_15 = ((/* implicit */short) min((((/* implicit */unsigned int) (+(((/* implicit */int) arr_3 [i_0]))))), (3187712843U)));
                }
            } 
        } 
    } 
    var_16 = ((/* implicit */unsigned char) var_0);
    /* LoopSeq 2 */
    for (int i_5 = 0; i_5 < 11; i_5 += 4) /* same iter space */
    {
        arr_18 [i_5] [i_5] = ((/* implicit */_Bool) 3187712842U);
        arr_19 [i_5] [10] = arr_15 [i_5];
    }
    for (int i_6 = 0; i_6 < 11; i_6 += 4) /* same iter space */
    {
        var_17 ^= ((/* implicit */int) (_Bool)0);
        arr_22 [i_6] [i_6] = ((/* implicit */long long int) 3187712842U);
        var_18 += ((/* implicit */unsigned short) arr_1 [i_6]);
    }
}
