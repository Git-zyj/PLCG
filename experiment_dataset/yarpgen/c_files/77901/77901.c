/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77901
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77901 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77901
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            var_20 -= -1368556418;
                            var_21 -= (min(((~(arr_7 [i_0] [i_2 + 1] [i_2 + 1]))), (arr_7 [i_0] [i_2 + 1] [i_2 + 1])));
                        }
                    }
                }
                var_22 = (((((48 * ((9223372036854775785 ? (arr_0 [i_0]) : (arr_5 [i_0] [i_0] [i_0])))))) ? (arr_5 [i_0] [i_0] [i_0]) : ((-63 * ((var_10 ? (arr_4 [i_0] [i_0] [i_0]) : var_12))))));
            }
        }
    }
    var_23 = (min((((var_1 + var_12) / var_0)), var_13));
    var_24 = var_0;
    #pragma endscop
}
