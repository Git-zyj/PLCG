/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214943
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214943 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214943
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 4; i_0 < 24;i_0 += 1)
    {
        arr_2 [i_0] [i_0 - 1] = ((((var_7 ? 27722 : -27727))));
        arr_3 [i_0] = (((((((!(arr_0 [10] [i_0])))) / (arr_1 [i_0]))) | ((min((min(27722, 27726)), (-127 - 1))))));
    }

    /* vectorizable */
    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        arr_6 [i_1] = (((((65535 ? (arr_4 [i_1]) : var_0))) ? 145 : ((var_6 ? (-127 - 1) : (arr_1 [i_1])))));
        var_11 *= (0 < 1);
        var_12 = var_5;
    }
    var_13 = (!var_3);
    var_14 = (min(var_3, (((((-27711 ? var_4 : var_4))) ? 27723 : (var_6 - var_10)))));
    var_15 = var_0;
    #pragma endscop
}
