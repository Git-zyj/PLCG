/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237444
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237444 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237444
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((var_4 + var_11) + (((var_15 >= var_8) ? var_10 : var_5)));

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_19 = (min(var_19, var_15));
        var_20 += (min((793225360 ^ 793225350), ((var_3 ? (var_3 ^ var_1) : (max((arr_0 [i_0]), var_11))))));
    }
    var_21 = ((!((min(-57, var_5)))));
    var_22 = (max(var_22, (!19797)));
    var_23 &= (max(((max(647568193, (max(31577, 793225350))))), -8999984623071376106));
    #pragma endscop
}
