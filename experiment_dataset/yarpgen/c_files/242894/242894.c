/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242894
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242894 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242894
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [1] = (max((min(152, ((152 ? (arr_4 [i_0] [i_0]) : var_4)))), (~5061455356368148922)));
                var_16 = (max(var_16, 534773760));
                var_17 = (min(var_17, ((((min((arr_0 [i_0] [i_1]), (~var_8))) | (~49396))))));
                arr_7 [i_0] [i_0] = (arr_3 [i_1] [i_0] [i_0]);
            }
        }
    }
    var_18 = (min(var_18, 1));
    #pragma endscop
}
