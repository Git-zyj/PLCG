/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213376
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213376 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213376
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (!-var_13);
    var_16 = (max(var_16, var_10));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_17 |= var_0;
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 18;i_3 += 1)
                    {
                        {
                            var_18 &= (~15632193993350710766);
                            arr_10 [i_0] [i_0] [i_0] [i_0] = ((var_10 + (((arr_6 [6] [i_2]) ? (((~(arr_5 [i_3] [i_0] [i_2] [i_3])))) : (~var_10)))));
                            arr_11 [i_0] [i_3] [i_0] [i_1 - 1] [i_3] [i_3] = ((+((((arr_5 [i_1 - 1] [i_2 + 2] [i_3 - 1] [i_3]) && (arr_3 [i_1 - 1]))))));
                        }
                    }
                }
                arr_12 [i_0] = var_9;
                var_19 = var_4;
                arr_13 [i_0] [i_0] [i_1] = (((((-((var_6 + (arr_2 [i_0])))))) || (((((var_13 + (arr_1 [13] [i_1 - 1]))) + 146)))));
            }
        }
    }
    #pragma endscop
}
