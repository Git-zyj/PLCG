/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33162
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33162 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33162
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((var_10 ? ((var_2 ? var_4 : -19) : ((-25434 ? -66 : -25434))))));
    var_12 = var_8;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    var_13 -= ((!((max((arr_7 [i_2] [i_2] [i_1] [i_0]), (arr_5 [i_0] [i_1] [i_2] [i_2]))))));
                    arr_8 [i_2] [i_1] [i_2] = (arr_6 [i_0] [i_0] [i_0] [i_0]);
                    var_14 = ((84 || (((arr_5 [i_2] [i_1] [i_1] [i_0]) && (95 | -63)))));
                }
            }
        }
    }
    var_15 *= var_1;
    #pragma endscop
}
