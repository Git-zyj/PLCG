/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104892
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104892 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104892
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [i_0] |= (max(((~((min(var_4, (arr_4 [i_0] [i_0] [i_1 + 1])))))), (!-3650814320265891200)));
                var_19 = (max((((var_6 ? (arr_5 [i_1 - 1]) : ((-(arr_4 [i_0] [i_1 + 1] [i_0])))))), (((arr_1 [i_0] [7]) ? var_6 : (((min(-74, (arr_4 [i_0] [i_0] [i_1 + 2])))))))));
                var_20 *= (arr_2 [i_0] [i_0]);
            }
        }
    }
    var_21 ^= var_7;
    var_22 = (((((var_7 ? (-1285830778 >= var_1) : (var_13 + var_12)))) || var_5));
    #pragma endscop
}
