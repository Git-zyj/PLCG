/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 173876
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=173876 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/173876
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 ^= var_2;
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_11 = (min(var_11, ((((((arr_3 [i_1 + 1] [i_0]) ? (arr_3 [i_1 + 4] [i_0 - 3]) : (arr_3 [i_1 - 1] [i_0]))) * (((arr_3 [i_1 + 1] [1]) ? (arr_3 [i_1 + 3] [3]) : (arr_3 [i_1 + 1] [i_0 + 1]))))))));
                    var_12 = (min(15, 34091302912));
                }
            }
        }
    }
    var_13 = ((-7317715649317780509 - 18446744039618248703) - var_2);
    #pragma endscop
}
