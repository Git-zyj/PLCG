/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 232094
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=232094 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/232094
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 *= (4185051944 ^ var_6);

    /* vectorizable */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_4 [i_0] = var_5;
        var_11 = ((var_0 < (arr_1 [i_0])));
    }
    /* vectorizable */
    for (int i_1 = 1; i_1 < 14;i_1 += 1)
    {
        var_12 = var_1;

        for (int i_2 = 0; i_2 < 17;i_2 += 1)
        {
            var_13 |= var_2;
            arr_10 [i_1] [i_1] = 1435021169;
            var_14 = var_4;
            var_15 *= -var_3;
        }
        for (int i_3 = 0; i_3 < 17;i_3 += 1)
        {
            arr_15 [i_1] [i_1] [i_3] = 942829376;
            var_16 |= (arr_0 [i_3]);
        }
        for (int i_4 = 0; i_4 < 17;i_4 += 1)
        {
            var_17 = -18446744073709551604;
            var_18 *= ((var_8 ? var_8 : var_4));
        }
    }
    for (int i_5 = 2; i_5 < 16;i_5 += 1) /* same iter space */
    {
        arr_21 [i_5] = var_5;
        var_19 -= (min(((max(1185844926, var_8))), (min(((min(3815, (arr_20 [i_5] [i_5])))), 2))));
    }
    /* vectorizable */
    for (int i_6 = 2; i_6 < 16;i_6 += 1) /* same iter space */
    {
        var_20 |= (((arr_3 [18]) ? (arr_2 [i_6] [i_6]) : (arr_3 [14])));
        var_21 = var_9;
        var_22 += 438599674;
        var_23 = (min(var_23, var_7));
        var_24 = 431799056;
    }
    #pragma endscop
}
