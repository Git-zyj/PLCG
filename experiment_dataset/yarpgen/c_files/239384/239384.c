/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239384
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239384 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239384
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((-var_4 ? var_12 : (621609212 - -660685278134827503)));

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_21 = var_6;
        var_22 = ((((max((var_13 != var_2), (((arr_2 [i_0]) + (arr_1 [i_0] [i_0])))))) ? (((arr_2 [12]) ? (arr_2 [i_0]) : 63488)) : (~-31)));
        arr_3 [i_0] &= ((-71 != (((var_1 || (arr_0 [i_0]))))));

        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            var_23 = 29;
            arr_7 [i_0] [i_0] = (((((~((-(arr_6 [i_0] [i_0] [i_1])))))) ? (((arr_1 [i_1 - 2] [i_1 - 2]) ? var_2 : var_8)) : (arr_4 [6])));
        }
    }
    #pragma endscop
}
