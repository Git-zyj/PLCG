/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177002
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177002 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177002
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_11;

    for (int i_0 = 0; i_0 < 14;i_0 += 1) /* same iter space */
    {
        var_16 = (max(var_16, (arr_2 [i_0])));
        arr_4 [i_0] [4] = (max(((-92 ? 0 : (((3594 ? (arr_3 [i_0]) : var_5))))), 1023));
        arr_5 [i_0] [i_0] = ((min(91, (arr_0 [i_0]))));
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1) /* same iter space */
    {
        var_17 -= (-(arr_6 [i_1] [i_1]));
        var_18 = max((arr_7 [i_1]), ((((min(1452844890, -91))) ? ((min((arr_0 [i_1]), -12904))) : ((0 ? (arr_6 [i_1] [i_1]) : var_2)))));
        var_19 = ((~(((arr_6 [i_1] [i_1]) ? (arr_6 [i_1] [i_1]) : (arr_2 [i_1])))));
    }
    var_20 = (min(19, ((~(~1920)))));
    #pragma endscop
}
