/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 37740
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=37740 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/37740
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_13 = ((-(((min(-385634289, (arr_1 [i_0] [i_1])))))));
                arr_6 [i_0] [i_1] = (((((154 * (arr_2 [i_0])))) ? (var_3 >= var_3) : (min((arr_0 [i_0] [i_1]), (arr_0 [i_0] [i_1])))));
            }
        }
    }
    var_14 = (max((((((min(var_8, var_11))) ? ((var_12 ? var_7 : var_0)) : 8191))), (min((max(var_11, 2609425332646580858)), (89 / 1)))));
    #pragma endscop
}
