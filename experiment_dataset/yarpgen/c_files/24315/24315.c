/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24315
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24315 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24315
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    arr_9 [i_1] [i_1] [i_1] [i_1] = (arr_4 [i_0] [i_0] [i_0]);

                    for (int i_3 = 2; i_3 < 23;i_3 += 1)
                    {
                        var_13 = var_10;
                        var_14 = (min(var_14, ((min(((min((arr_7 [i_0 + 3] [i_3 - 1] [i_3 + 1]), var_5))), var_7)))));
                    }
                    arr_12 [i_0] [i_1] [i_1] = (1514250787 || var_5);
                    var_15 = (max(var_15, (arr_11 [i_0] [12] [i_1] [12])));
                }
            }
        }
    }
    var_16 = (max(var_16, (((((!(min(0, var_1)))) ? -var_8 : (120 + -32755))))));
    var_17 = (((-(min(var_4, var_8)))));
    #pragma endscop
}
