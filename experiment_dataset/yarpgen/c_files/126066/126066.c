/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 126066
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=126066 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/126066
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = (arr_1 [i_0]);
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 23;i_2 += 1)
            {
                {

                    /* vectorizable */
                    for (int i_3 = 3; i_3 < 21;i_3 += 1)
                    {
                        var_20 = 1;
                        var_21 = (!1);
                        var_22 = (((arr_11 [i_3] [i_3 + 3] [i_3 - 3] [i_3 + 3]) && (arr_11 [i_3] [i_3 - 2] [i_3] [i_3 + 2])));
                    }
                    var_23 = (((((-(arr_10 [i_0] [i_1] [i_2] [i_1])))) ? ((max((arr_0 [i_0]), var_1))) : (~1)));
                }
            }
        }

        /* vectorizable */
        for (int i_4 = 0; i_4 < 24;i_4 += 1)
        {
            var_24 = var_3;
            var_25 = ((var_18 ? var_3 : (arr_2 [i_0])));
            var_26 = 1;
            arr_14 [i_0] [i_0] [i_0] = var_14;
        }
    }
    for (int i_5 = 2; i_5 < 14;i_5 += 1)
    {
        var_27 = var_17;
        var_28 = ((((arr_15 [i_5 - 1]) ? (max(1, var_18)) : (arr_2 [i_5]))));
    }
    var_29 = var_2;
    /* LoopNest 3 */
    for (int i_6 = 4; i_6 < 22;i_6 += 1)
    {
        for (int i_7 = 0; i_7 < 23;i_7 += 1)
        {
            for (int i_8 = 1; i_8 < 20;i_8 += 1)
            {
                {
                    arr_27 [i_6] [i_8] = (~var_17);
                    /* LoopNest 2 */
                    for (int i_9 = 0; i_9 < 23;i_9 += 1)
                    {
                        for (int i_10 = 0; i_10 < 23;i_10 += 1)
                        {
                            {
                                var_30 = (!2216836364);
                                var_31 = -1;
                                var_32 = arr_29 [i_6] [i_6] [i_8] [i_6];
                            }
                        }
                    }
                    arr_33 [i_6] [i_8] [i_8 + 2] = (!150);
                }
            }
        }
    }
    var_33 = (27295 ^ var_7);
    #pragma endscop
}
