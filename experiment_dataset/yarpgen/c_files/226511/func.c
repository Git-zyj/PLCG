/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226511
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226511 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226511
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 ^= ((/* implicit */_Bool) ((((/* implicit */_Bool) 3596715742034381476ULL)) ? (((unsigned int) (-(var_1)))) : (((/* implicit */unsigned int) (+((-(((/* implicit */int) var_14)))))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 1; i_0 < 14; i_0 += 4) 
    {
        var_21 = ((/* implicit */unsigned short) max((var_21), (((/* implicit */unsigned short) (-(((/* implicit */int) arr_1 [i_0])))))));
        var_22 = ((/* implicit */long long int) max((var_22), (var_2)));
    }
}
