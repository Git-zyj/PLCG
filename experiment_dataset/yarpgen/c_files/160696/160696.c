/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160696
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160696 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160696
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, (~var_18)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 15;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 14;i_4 += 1)
                        {
                            {
                                arr_16 [i_1 + 1] [i_0] [i_3] = (!255);
                                arr_17 [i_1] [i_0] [i_0] [i_3] [i_0] [i_4] = ((-((min((arr_0 [i_0] [i_4 + 2]), (arr_0 [i_0] [i_4 - 2]))))));
                                var_20 = (((0 ? 596 : (2840001784 / var_8))));
                                var_21 = (arr_12 [i_3] [i_0] [i_0] [i_0]);
                            }
                        }
                    }
                    var_22 |= var_11;
                    arr_18 [i_0] = (255 + -16886);
                }
            }
        }
    }
    #pragma endscop
}
