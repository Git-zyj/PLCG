/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213859
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213859 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213859
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((var_3 > 0) * ((((var_14 ? var_14 : var_3)) * (~var_12))));

    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 19;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 23;i_3 += 1)
                {
                    {
                        var_16 += (arr_9 [i_0] [i_0] [i_2] [i_3]);
                        arr_10 [i_3] [i_2] [0] [i_0] = (((arr_1 [i_2 + 3]) >> (var_8 - 54900)));
                        var_17 = (arr_0 [i_0] [i_3]);
                    }
                }
            }
        }
        arr_11 [i_0] [i_0 + 1] = (arr_6 [20]);
    }
    #pragma endscop
}
