/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 186134
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=186134 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/186134
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1 + 1] [i_1] [i_0] = (((!13) ? (((var_2 % (arr_2 [i_1 + 2])))) : (min((arr_2 [i_1 + 1]), 7072765388517371408))));
                    var_19 = (max(var_19, 4055701523908440507));
                    var_20 ^= (max(3, (max(3385270518460434679, var_8))));
                }
            }
        }
    }
    var_21 = 11791499190781550794;
    #pragma endscop
}
