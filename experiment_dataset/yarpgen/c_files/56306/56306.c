/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 56306
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=56306 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/56306
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = 46;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [i_0] = (max(-2147483640, ((max((((arr_4 [i_0] [i_0] [i_1]) ? 64990 : (arr_3 [i_0]))), (arr_0 [i_0]))))));
                var_15 ^= (((((arr_4 [10] [i_0] [i_0 - 1]) ? (arr_5 [i_0] [2]) : ((548 ? 757236913 : 374428606)))) - (((18446744073709551609 ? 97 : 191)))));
            }
        }
    }
    #pragma endscop
}
