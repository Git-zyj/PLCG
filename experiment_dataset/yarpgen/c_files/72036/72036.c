/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72036
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72036 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72036
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_18 -= (((((((max(var_3, var_5)))) - -9316200683421418495))) ? 30439 : (((32099 | var_14) % (arr_3 [i_1]))));
                arr_4 [i_0] [i_0] [12] = ((+(((~123) ? (35084 - var_2) : (min((arr_0 [i_0]), var_15))))));
                arr_5 [6] [i_0] [i_0] = (max(35081, var_16));
            }
        }
    }
    var_19 = var_6;
    #pragma endscop
}
