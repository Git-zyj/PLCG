/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196484
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196484 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196484
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        arr_4 [i_0] &= ((min((arr_3 [i_0] [i_0]), ((46 ? (arr_2 [i_0] [i_0]) : 0)))));
        var_15 *= (arr_0 [i_0]);
        arr_5 [i_0] = (((((1680399842 <= 0) ? var_0 : (arr_1 [i_0] [i_0]))) / (arr_0 [i_0])));
        var_16 = (-32767 - 1);
        var_17 = (max((((arr_0 [i_0]) << (var_13 - 4637))), (~var_12)));
    }
    var_18 = ((65535 ? 962424234 : 32768));
    var_19 = ((!(((min(var_8, var_5))))));
    #pragma endscop
}
