/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171337
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171337 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171337
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_6 [i_0] = (((7279 || -35) > (min(7551565819617227470, 1264473386))));
                var_14 = ((((456347807 ? var_4 : (1 != -9223372036854775784))) % (((((9223372036854775784 ? -61 : 0)) ^ 1017954737)))));
                var_15 = (min(var_15, (252 >> 7)));
            }
        }
    }
    var_16 = ((16261 ? (((!(((-1535 ? (-9223372036854775807 - 1) : 47)))))) : var_7));
    var_17 *= (max(var_12, 7));
    var_18 = var_0;
    #pragma endscop
}
