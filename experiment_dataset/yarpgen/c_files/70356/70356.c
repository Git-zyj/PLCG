/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70356
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70356 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70356
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (((((-((var_0 ? var_2 : var_11))))) ? var_13 : (max((~var_11), ((max(var_2, var_4)))))));
    var_15 = ((-(!var_7)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] = -var_5;
                arr_5 [i_0] [i_0] [i_1] = ((((min((~var_1), ((((arr_0 [i_0]) || var_10)))))) ? ((max(-var_1, ((max(var_3, (arr_2 [i_0]))))))) : (min((!-6560), var_7))));
                var_16 ^= (min(var_4, (min((!var_10), ((var_6 ? (arr_2 [2]) : (arr_2 [i_0])))))));
            }
        }
    }
    #pragma endscop
}
