/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127599
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127599 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127599
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] = ((((max(14469912262062125996, (((var_11 ? var_7 : 104)))))) ? ((((min((arr_1 [i_1] [0]), 2018589647))) ? ((var_9 ? var_4 : -12306)) : ((1566201084 ? var_2 : var_1)))) : (~1499604399)));
                var_12 = (min((((arr_3 [i_1]) ? (arr_3 [i_0]) : (arr_3 [i_0]))), ((((arr_4 [i_0] [i_1]) - 65524)))));
            }
        }
    }
    var_13 = (((max(1, var_7))));
    #pragma endscop
}
