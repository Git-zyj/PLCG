/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215111
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215111 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215111
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 23;i_1 += 1)
        {
            {
                var_17 = max(1, ((+((1 ? (arr_0 [i_0]) : (arr_0 [i_0]))))));
                var_18 = (((((((-2035825743 ? 13917 : (arr_0 [i_1]))) ^ (~13917)))) ? (max(7516192768, (arr_4 [i_0] [i_1]))) : ((((((arr_2 [i_0]) >> (((arr_1 [i_0]) - 1082933694)))))))));
                arr_5 [i_0] |= (arr_4 [i_1] [i_0]);
            }
        }
    }
    var_19 = ((11105 | ((max((max(var_10, 1)), var_7)))));
    var_20 = -7282911672863383260;
    #pragma endscop
}
