/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 161645
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=161645 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/161645
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 |= 2147483647;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_15 = (max(var_15, ((-(((!(var_11 || -473742626))))))));
                var_16 = ((arr_2 [i_0]) ? ((~(arr_3 [i_1] [i_0]))) : (min((((arr_0 [i_0]) ? -1206480059 : (arr_3 [i_0] [i_1]))), ((var_3 ? var_5 : 1322575368)))));
                var_17 = ((min(-1322575368, 703814046)));
                var_18 = arr_3 [8] [8];
            }
        }
    }
    #pragma endscop
}
