/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130366
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130366 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130366
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 -= ((var_2 && (((var_8 << (((-27261 + 27290) - 20)))))));
    var_16 *= ((((var_14 ? var_10 : var_13)) ^ var_10));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                arr_4 [i_1] = -13605;
                arr_5 [i_0] [i_1] = var_2;
                var_17 = (i_1 % 2 == zero) ? ((min(((12649 >> (((arr_3 [i_1]) - 13030)))), ((~(arr_3 [i_1])))))) : ((min(((12649 >> (((((arr_3 [i_1]) - 13030)) - 36120)))), ((~(arr_3 [i_1]))))));
            }
        }
    }
    var_18 = (max(var_18, var_9));
    var_19 = ((524287 << (415118081 - 415118074)));
    #pragma endscop
}
