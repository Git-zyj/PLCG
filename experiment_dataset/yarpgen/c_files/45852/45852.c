/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45852
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45852 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45852
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_16 = (min((max(var_3, -var_3)), (min((((arr_4 [i_0] [i_0]) / var_2)), ((min(var_0, var_13)))))));
                var_17 = (3800555356 / 494411944);
            }
        }
    }
    var_18 = var_14;
    var_19 = ((~(min((max(494411951, 494411947)), var_8))));
    var_20 &= (((!-150427271) ? var_5 : (min(((3800555356 ? 494411940 : var_2)), ((max(var_13, var_12)))))));
    #pragma endscop
}
