/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136823
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136823 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136823
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 15;i_1 += 1)
        {
            {
                arr_4 [16] [16] = (arr_0 [i_1 - 1] [i_1 - 1]);
                arr_5 [i_0] [i_0] [i_0] = ((+((((((arr_3 [i_0] [i_1] [i_0]) ? 0 : var_8))) ? (arr_2 [i_0] [i_0]) : (arr_0 [i_1 - 2] [i_1 - 1])))));
            }
        }
    }
    var_11 = (min(7, -6410245973760851455));
    var_12 = ((((max(var_8, (min(var_9, var_1))))) > var_8));
    #pragma endscop
}
