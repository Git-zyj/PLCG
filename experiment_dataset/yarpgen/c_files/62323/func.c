/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62323
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62323 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62323
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (signed char i_0 = 0; i_0 < 12; i_0 += 4) 
    {
        for (unsigned char i_1 = 2; i_1 < 10; i_1 += 4) 
        {
            {
                arr_7 [i_0] [i_1] = (unsigned char)0;
                var_19 = ((/* implicit */long long int) arr_4 [i_1 - 2] [i_1 - 2]);
                arr_8 [i_0] [i_0] [(signed char)6] = ((/* implicit */_Bool) max((var_12), (((/* implicit */long long int) var_4))));
            }
        } 
    } 
    var_20 = ((/* implicit */unsigned short) var_15);
    var_21 = ((/* implicit */signed char) ((short) ((long long int) ((var_1) << (((var_0) - (8923602105346260276ULL)))))));
}
