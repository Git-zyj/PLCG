/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227950
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227950 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227950
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_18 ^= (min(-5469, 18446744073709551615));
                arr_4 [i_0] = var_13;
                var_19 = (max(var_19, (max(((73 != (arr_2 [i_0]))), 1))));
            }
        }
    }
    var_20 = (((((~((4161452057 ? var_14 : var_7))))) ? ((((max(var_4, var_10))) ? 1 : var_12)) : (min(var_12, (((var_4 << (var_14 - 87))))))));
    #pragma endscop
}
