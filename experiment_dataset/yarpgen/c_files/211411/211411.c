/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211411
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211411 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211411
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min(var_12, var_0));
    var_13 = (32767 | -1179367653);

    for (int i_0 = 3; i_0 < 16;i_0 += 1)
    {
        arr_4 [i_0] = (arr_1 [i_0] [i_0 + 1]);

        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            var_14 = (max(var_14, ((((arr_0 [i_0 - 3]) << (((360992706480760790 > (arr_0 [i_0 - 2])))))))));
            arr_7 [i_1] [i_0] [i_0] = (((((~(max((arr_1 [i_0 - 1] [i_1]), 3267434873))))) || (arr_1 [i_1] [i_0])));
        }
    }
    for (int i_2 = 0; i_2 < 25;i_2 += 1)
    {
        arr_12 [i_2] = (min((((9007199254740991 ^ 2546099095526841591) & 65509)), var_4));
        var_15 -= 65493;
        var_16 *= -1;
    }
    var_17 = var_0;
    #pragma endscop
}
