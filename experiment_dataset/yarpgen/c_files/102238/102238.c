/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102238
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102238 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102238
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 -= (min(-466194328, 4));

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_15 = ((((((arr_3 [i_0]) % var_6))) ? (max((arr_3 [i_0]), (((arr_0 [i_0]) ? (arr_2 [18] [i_0]) : (arr_3 [i_0]))))) : ((((max(var_10, 4294967291))) ? 4 : 4294967291))));
        var_16 ^= ((((min((min((arr_0 [18]), var_10)), (min(var_7, 8079951156572969124))))) ? (min(9047378631242156048, 4)) : var_4));
    }
    #pragma endscop
}
