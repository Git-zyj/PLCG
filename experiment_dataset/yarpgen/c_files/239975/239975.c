/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239975
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239975 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239975
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_3;
    var_18 = (min(var_18, ((min(var_15, (((((-34 ? var_1 : var_6)) == var_12))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_4 [3] [i_1] [6] = ((((min((arr_1 [i_0]), (min(-2042965746, (arr_3 [i_0])))))) < -159940314454163186));
                var_19 = (max((((arr_2 [i_0]) | -1976)), (((arr_3 [i_0]) / 159940314454163176))));
                var_20 = ((~(~var_3)));
                arr_5 [i_0] [i_0] [i_1] = var_4;
            }
        }
    }
    #pragma endscop
}
