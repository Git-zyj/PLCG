/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239161
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239161 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239161
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((143210234 ? (((((((var_3 ? -143210235 : 0))) < var_1)))) : var_14));

    /* vectorizable */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = (arr_1 [i_0 - 1]);
        var_17 = (min(var_17, ((((arr_1 [i_0 - 1]) >= var_7)))));
    }
    for (int i_1 = 0; i_1 < 22;i_1 += 1)
    {
        var_18 = (arr_5 [i_1]);
        arr_6 [i_1] = -51;
    }

    for (int i_2 = 0; i_2 < 12;i_2 += 1)
    {
        var_19 = -143210235;
        var_20 = ((((((var_1 ? (arr_5 [1]) : var_6)) < ((-143210235 ? 1 : var_7)))) ? ((~((1 ? 0 : (arr_7 [i_2] [i_2]))))) : -143210234));
    }
    #pragma endscop
}
