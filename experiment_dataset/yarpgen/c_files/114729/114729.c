/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114729
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114729 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114729
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((((min(var_0, 4020))) ? (~var_1) : (11 & var_8))) <= ((var_8 ? ((26 ? var_5 : var_0)) : var_8))));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_13 |= (23914 ? var_3 : ((((arr_2 [7]) || -4036))));
                    arr_7 [i_0] [6] [1] [i_1] = (min(4022, 4014));
                }
            }
        }
    }
    var_14 = (((var_8 + 2147483647) >> (((var_4 * 1) - 2037))));
    var_15 = (min((max(((min(var_6, var_8))), 49)), ((min(16256, var_0)))));
    #pragma endscop
}
