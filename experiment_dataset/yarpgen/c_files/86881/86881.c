/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86881
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86881 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86881
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_18 = (max(var_18, (arr_3 [i_0])));
                var_19 = (min(var_19, var_0));
                arr_5 [i_0] = ((~((var_4 ^ ((18425360799425763516 ? 18425360799425763521 : var_5))))));
                var_20 -= ((9097771802630825394 ? -320675613 : (((min((arr_0 [i_0]), var_7)) / var_14))));
            }
        }
    }
    var_21 = (((max(var_15, var_1)) / var_10));
    #pragma endscop
}
