/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 232857
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=232857 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/232857
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (((((-(var_19 && 1)))) * var_4));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_21 -= 0;
                var_22 &= var_3;
                var_23 = ((((((var_11 ? var_16 : 18361078911491926742)))) != (arr_1 [i_0] [1])));
                var_24 = (((((var_15 + 0) & (((((arr_0 [i_0] [i_1]) >= 85665162217624874)))))) >= (((((var_18 ? (arr_2 [6] [2]) : (arr_2 [i_0] [i_1])))) ? var_17 : (arr_2 [i_1] [i_0])))));
            }
        }
    }
    var_25 = var_15;
    #pragma endscop
}
