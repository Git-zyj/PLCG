/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 153253
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=153253 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/153253
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((var_2 >= (var_11 / var_0)));
    var_18 -= var_14;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 20;i_1 += 1)
        {
            {
                var_19 = (((arr_6 [i_0] [i_1 - 1] [i_0]) ? (max((arr_6 [i_1 - 1] [i_1 + 1] [i_1 + 1]), var_0)) : var_10));
                var_20 &= (max((min((arr_5 [9] [i_1 + 1] [i_1 - 1]), var_10)), (arr_6 [i_1] [i_1 - 1] [i_1 - 1])));
            }
        }
    }
    var_21 = max(var_10, (max((min(var_13, var_12)), var_6)));
    #pragma endscop
}
