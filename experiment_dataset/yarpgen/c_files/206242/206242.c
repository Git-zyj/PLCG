/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206242
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206242 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206242
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_2;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 20;i_2 += 1)
            {
                {
                    var_19 = (min(30625909, (((arr_6 [i_2 + 1] [i_2 + 2] [i_2 + 2]) ? (arr_6 [i_2 + 1] [i_2 + 2] [i_2 - 1]) : var_0))));
                    var_20 *= ((((((((arr_0 [i_0] [i_1]) ? var_4 : var_14))) ? (!-1) : ((-(arr_2 [13]))))) > 1372638610898042675));
                }
            }
        }
    }
    var_21 *= var_4;
    var_22 = var_14;
    var_23 = (max(((~(min(var_9, var_13)))), (((((max(var_6, var_2))) ? ((min(var_5, 64512))) : var_6)))));
    #pragma endscop
}
