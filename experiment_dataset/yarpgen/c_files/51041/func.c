/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51041
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51041 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51041
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = var_7;
    /* LoopNest 3 */
    for (_Bool i_0 = 1; i_0 < 1; i_0 += 1) 
    {
        for (signed char i_1 = 3; i_1 < 20; i_1 += 3) 
        {
            for (signed char i_2 = 0; i_2 < 22; i_2 += 3) 
            {
                {
                    /* LoopSeq 1 */
                    for (_Bool i_3 = 0; i_3 < 1; i_3 += 1) 
                    {
                        arr_11 [i_0 - 1] [i_0] [i_0 - 1] [i_0] = ((/* implicit */_Bool) (+(((/* implicit */int) ((_Bool) var_8)))));
                        arr_12 [i_0] = ((/* implicit */signed char) (_Bool)0);
                    }
                    arr_13 [i_0] = ((/* implicit */_Bool) ((((/* implicit */int) var_4)) >> (((/* implicit */int) (!(((/* implicit */_Bool) max((((/* implicit */signed char) (_Bool)1)), ((signed char)-33)))))))));
                }
            } 
        } 
    } 
}
