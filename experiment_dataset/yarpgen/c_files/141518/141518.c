/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 141518
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=141518 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/141518
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 -= -10537143936583008909;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_17 = (((-var_5 && (2149025242 < 1893127612))));
                arr_5 [i_0] = (min((arr_3 [i_0 + 1]), (!var_15)));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 15;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 15;i_3 += 1)
        {
            {
                var_18 = ((min(var_8, (arr_7 [i_2] [i_2]))));
                arr_10 [i_2] [i_3] = (max((arr_8 [12] [12] [12]), (((-(arr_6 [i_2] [i_3]))))));
                var_19 &= ((!((((arr_0 [i_2] [i_3]) ? 2149025242 : (arr_3 [i_3]))))));
            }
        }
    }
    #pragma endscop
}
