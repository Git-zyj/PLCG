/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3000
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3000 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3000
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_10 = (arr_3 [i_1]);
                arr_5 [i_0] [i_1] [i_1] = ((-(arr_0 [i_1] [i_1])));
                arr_6 [i_1] [i_1] [i_0] = var_1;

                for (int i_2 = 1; i_2 < 13;i_2 += 1)
                {
                    var_11 *= var_3;
                    arr_9 [13] [i_1] [i_1] [i_2] = (max(120259084288, ((max(19, -237)))));
                }
                var_12 += ((((~(min((arr_3 [i_0]), (arr_2 [i_0])))))) ? (arr_2 [i_0]) : (arr_7 [2] [i_1] [i_1]));
            }
        }
    }
    var_13 = (min(var_13, (~var_0)));
    var_14 = var_3;
    var_15 = (((var_1 ? -var_0 : var_1)));
    #pragma endscop
}
