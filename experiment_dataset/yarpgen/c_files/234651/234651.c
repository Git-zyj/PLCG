/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234651
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234651 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234651
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            {
                arr_4 [i_1] [i_1] [4] = -var_1;
                arr_5 [i_0] [1] [i_1] = -64;
                var_10 = 61802;
            }
        }
    }

    for (int i_2 = 0; i_2 < 18;i_2 += 1)
    {
        var_11 &= (((~var_5) ? ((~(arr_6 [i_2]))) : (arr_6 [i_2])));

        for (int i_3 = 0; i_3 < 18;i_3 += 1)
        {
            var_12 = (min(var_12, ((min(((((arr_10 [i_2] [i_2] [i_3]) < (arr_10 [i_2] [i_2] [i_3])))), (arr_10 [i_2] [i_3] [14]))))));
            var_13 = (min(var_13, 7936));
            arr_12 [i_3] [i_3] [i_2] = (arr_6 [i_2]);
        }
    }
    #pragma endscop
}
