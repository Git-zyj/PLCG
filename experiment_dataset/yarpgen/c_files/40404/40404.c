/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40404
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40404 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40404
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((-20980 ? 19104 : 1));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        {
                            arr_10 [i_2] [i_3] [i_2] [i_2] [i_1] [i_0] = var_2;
                            var_12 = ((((min(var_7, 33)))) ? ((var_7 ? 52 : var_6)) : (min(((var_3 ? 1 : var_1)), 274877906943)));
                            var_13 = ((((1 ? -2147483642 : 247))));
                            var_14 = (min(52, 0));
                            var_15 = (min(var_15, (((0 ? 1 : -35)))));
                        }
                    }
                }
                arr_11 [i_0] = (!-4294967295);
            }
        }
    }
    #pragma endscop
}
