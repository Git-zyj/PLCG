/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80992
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80992 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80992
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (((!var_11) == var_16));
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    var_21 *= (max((((max(-14203, 93)))), ((var_19 ? var_9 : (((var_6 * (arr_3 [13]))))))));
                    var_22 = (min(var_22, -6869476178568908039));
                    var_23 &= ((((((arr_3 [i_0]) ? ((((!(arr_1 [i_2 - 1]))))) : (arr_4 [i_0 - 1] [i_2 - 1])))) ? (arr_0 [i_0] [i_1]) : ((((arr_3 [i_0 + 3]) ? (arr_3 [i_0 - 4]) : (arr_3 [i_0 - 2]))))));
                }
            }
        }
    }
    var_24 = (max(var_24, ((max(((!((min(216736587, var_17))))), var_12)))));
    #pragma endscop
}
