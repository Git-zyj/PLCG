/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144392
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144392 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144392
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = -18094;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_11 = 192;
                var_12 = (((arr_3 [i_0]) << (((-var_7 || ((min(63684, var_2))))))));
                var_13 = (i_0 % 2 == zero) ? (((((((var_5 * var_3) >> (((arr_0 [i_0]) + 14632))))) ? 10830593695048931956 : ((var_9 ? var_2 : (arr_5 [i_0])))))) : (((((((var_5 * var_3) >> (((((arr_0 [i_0]) + 14632)) - 12356))))) ? 10830593695048931956 : ((var_9 ? var_2 : (arr_5 [i_0]))))));
            }
        }
    }
    #pragma endscop
}
