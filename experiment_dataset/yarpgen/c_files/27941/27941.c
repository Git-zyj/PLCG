/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27941
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27941 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27941
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = -var_11;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_20 = (max((!var_0), (29 < -80)));
                var_21 = (((-1632058960 + -74) <= ((-(arr_4 [i_1] [i_1] [i_0 - 2])))));
                arr_5 [5] [i_1] [i_0] = var_18;
                var_22 = (((((arr_2 [i_0 - 1] [i_0 + 1]) ? (arr_2 [i_0 - 3] [i_0 - 1]) : -1632058960)) + (((arr_2 [i_0 - 1] [i_0 - 2]) / -165628221))));
            }
        }
    }
    #pragma endscop
}
