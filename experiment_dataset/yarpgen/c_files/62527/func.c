/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62527
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62527 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62527
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */signed char) ((unsigned int) ((((((/* implicit */_Bool) (short)127)) ? (((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned short)13))) : (13010613884586057880ULL))) * (((/* implicit */unsigned long long int) 0)))));
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 23; i_0 += 3) 
    {
        var_13 = ((/* implicit */_Bool) ((((/* implicit */int) (!(((/* implicit */_Bool) var_8))))) - (12)));
        var_14 *= ((/* implicit */unsigned int) arr_1 [i_0]);
    }
}
