/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112399
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112399 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112399
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = -2;
    var_13 = 1;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 16;i_2 += 1)
            {
                {
                    var_14 = (max(((~((((arr_2 [i_0] [i_0]) != (arr_3 [i_0])))))), ((7864320 ? 0 : ((2 ? 1915404536 : 24))))));

                    for (int i_3 = 4; i_3 < 17;i_3 += 1) /* same iter space */
                    {
                        var_15 = max((arr_8 [i_0] [i_0] [i_0]), 195);
                        var_16 = ((!((max(520093696, 87)))));
                    }
                    for (int i_4 = 4; i_4 < 17;i_4 += 1) /* same iter space */
                    {
                        arr_11 [i_0] [i_0 + 1] [i_1] [i_1] [i_0] [i_0] = (max((!-6203251779834038783), (arr_3 [i_0])));
                        arr_12 [i_0] [i_0] [0] = ((max((arr_6 [i_4]), 9223372036854775807)));
                        arr_13 [i_0] [i_1] [i_0] [i_4 - 4] = (arr_10 [15]);
                    }
                }
            }
        }
    }
    var_17 = 527765581332480;
    #pragma endscop
}
