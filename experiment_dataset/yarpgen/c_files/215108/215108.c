/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215108
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215108 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215108
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_19 *= ((min((arr_2 [i_0]), (arr_0 [i_0]))));
        /* LoopNest 3 */
        for (int i_1 = 3; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 14;i_3 += 1)
                {
                    {
                        arr_10 [i_0] [i_2] = (((50367 & 50373) ? ((((min((arr_7 [8] [i_1]), var_1)) | (((min((arr_6 [i_0] [13]), 50367))))))) : (((~15198) & 10486690814704842389))));
                        arr_11 [i_0] [i_1] [i_2] [i_3] = ((~(min(((((arr_0 [i_3]) - 123116337))), 2525564384))));
                    }
                }
            }
        }
    }
    var_20 = var_0;
    var_21 = var_17;
    #pragma endscop
}
