/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209316
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209316 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209316
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (var_9 ^ 18230005765042171558);
    var_13 = (min(((2147483646 ? (~var_0) : (216738308667380055 | var_10))), var_8));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            {
                var_14 *= (max((max(3207400655, 45608)), 416159409));
                arr_6 [i_1] = (var_10 && var_7);
                var_15 = ((max((arr_1 [i_0]), (arr_5 [i_0] [i_1] [i_1]))));
            }
        }
    }
    var_16 = var_7;
    #pragma endscop
}
