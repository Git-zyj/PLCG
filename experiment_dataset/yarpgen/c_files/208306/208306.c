/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208306
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208306 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208306
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 ^= (((var_6 ? (!var_7) : var_1)));

    /* vectorizable */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        var_14 ^= (((arr_1 [i_0 - 1]) ? (((var_1 ? (arr_2 [i_0 + 1]) : (arr_0 [i_0 + 2])))) : ((194 ? 97841865 : 268435455))));
        var_15 = (arr_2 [i_0 - 1]);
        var_16 |= (((arr_2 [i_0 - 1]) < var_10));
        var_17 *= var_11;
    }

    for (int i_1 = 1; i_1 < 23;i_1 += 1)
    {
        var_18 = 1660236664;
        var_19 |= (min(((0 ? (arr_4 [i_1 + 2]) : var_4)), (!var_11)));
        var_20 = (((arr_3 [i_1]) ? 14421 : ((62082 ? 4294967295 : 222))));
    }
    var_21 = var_6;
    #pragma endscop
}
