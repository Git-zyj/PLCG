/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67119
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67119 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67119
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 |= (min(var_2, (~var_17)));
    var_19 = (~((((min(134, 112))) | var_0)));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 22;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1 + 1] [i_2 - 1] = (min((((arr_7 [i_0] [16]) + ((((arr_3 [i_1]) + var_11))))), ((min(127, -112)))));
                    var_20 = (((((-(arr_0 [i_1 + 2] [i_1 + 2])))) ? (((arr_4 [i_1 + 2] [i_1 + 2]) ? 255 : (arr_7 [i_1 + 2] [i_1 + 2]))) : (((arr_0 [i_1 + 2] [i_1 + 2]) - (arr_7 [i_1 + 2] [i_1 + 2])))));
                }
            }
        }
    }
    var_21 = (min(0, 111));
    #pragma endscop
}
