/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133311
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133311 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133311
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (short i_0 = 2; i_0 < 20; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 24; i_1 += 2) 
        {
            {
                arr_6 [i_1] = ((/* implicit */unsigned short) var_6);
                arr_7 [i_0 - 2] = ((/* implicit */int) max((((/* implicit */unsigned short) arr_1 [i_0 + 3])), (min((((/* implicit */unsigned short) var_5)), ((unsigned short)31)))));
            }
        } 
    } 
    var_12 = (signed char)-105;
    var_13 = ((/* implicit */signed char) (!(((/* implicit */_Bool) min((((/* implicit */long long int) 320295082U)), (var_3))))));
}
