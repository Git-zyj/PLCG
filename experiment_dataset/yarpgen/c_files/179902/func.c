/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179902
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179902 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179902
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned short) -1LL);
    /* LoopSeq 1 */
    /* vectorizable */
    for (short i_0 = 0; i_0 < 21; i_0 += 4) 
    {
        var_17 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) -1LL)) ? (((/* implicit */unsigned long long int) (~(((/* implicit */int) var_1))))) : ((+(arr_1 [i_0])))));
        var_18 = ((/* implicit */short) max((var_18), (((/* implicit */short) (~(var_0))))));
    }
    var_19 = ((/* implicit */long long int) var_5);
    var_20 = ((/* implicit */long long int) ((((/* implicit */_Bool) var_9)) ? (((/* implicit */int) (short)30542)) : ((+(((/* implicit */int) (unsigned char)234))))));
}
