/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 240782
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=240782 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/240782
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_11 = (arr_6 [i_0] [i_1]);

                    /* vectorizable */
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        var_12 = -2741;
                        var_13 = var_0;
                    }
                    arr_13 [i_0] [i_1] [i_2] [i_2] = var_5;
                }
            }
        }
    }
    var_14 = ((var_8 ? (min(39, var_0)) : var_8));
    var_15 = (min(((((~var_8) ? ((19457 ? var_8 : var_9)) : -20375))), ((((max(var_3, -1))) ? var_0 : (var_6 / var_8)))));
    var_16 = ((0 ? var_6 : var_4));
    #pragma endscop
}
