/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 154890
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=154890 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/154890
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = 14447;
    var_15 *= (max((var_10 >> 31), ((((!0) && var_2)))));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 8;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [i_0] = (max((((124 ? (arr_1 [i_0 + 3] [i_1 + 2]) : (arr_1 [i_0 - 1] [i_1 - 2])))), (arr_4 [i_0 + 2] [i_0 + 2])));
                    var_16 = 10905280201430183624;
                }
            }
        }
    }
    var_17 = ((var_1 ? 1 : ((50757 ? -var_10 : (max(var_2, var_5))))));
    #pragma endscop
}
