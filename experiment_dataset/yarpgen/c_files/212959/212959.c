/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212959
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212959 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212959
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(5, (-127 - 1)));
    var_11 = ((var_5 ? var_0 : (var_4 != var_4)));
    var_12 = (-122 % var_5);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [7] [i_1] = ((((min(var_9, 14479))) ? ((((arr_2 [i_0] [i_1]) ? (arr_2 [i_0] [i_1]) : (arr_2 [i_1 - 1] [i_1])))) : (min((arr_3 [i_1]), (arr_3 [i_1])))));
                arr_5 [i_0] [1] [i_0] = 2588406475;
                arr_6 [i_0] [i_1] [i_1] = (~(max(var_5, 1043673794)));
                arr_7 [i_1] [i_1] [i_1] = ((((-(arr_3 [i_1])))) | 105);
            }
        }
    }
    var_13 = var_1;
    #pragma endscop
}
