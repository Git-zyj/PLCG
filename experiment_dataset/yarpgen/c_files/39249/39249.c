/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 39249
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=39249 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/39249
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (var_8 / 32640);
    var_14 += (min(32907, var_2));

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        var_15 = ((!((!(arr_0 [i_0])))));
        arr_4 [i_0] = (((arr_3 [i_0]) ? (arr_2 [0]) : ((-1 ? 32640 : 32640))));
        var_16 = arr_3 [i_0];
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 20;i_3 += 1)
                {
                    {
                        var_17 = ((((min((arr_3 [i_3]), var_2))) && (((-((min(1, 15))))))));
                        arr_13 [i_0] [i_0] [i_0] [i_0] = (!var_3);
                        arr_14 [13] [13] [13] [0] = (((((~(arr_11 [6] [i_3 - 1] [i_3 + 1] [6])))) ? (arr_10 [i_0] [i_3 + 1] [i_3] [10]) : ((var_2 << (((arr_0 [i_0]) + 22))))));
                        var_18 += var_5;
                    }
                }
            }
        }
    }
    #pragma endscop
}
