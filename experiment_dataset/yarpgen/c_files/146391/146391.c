/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146391
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146391 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146391
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = -var_13;
    var_17 = -125829120;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 14;i_2 += 1)
            {
                {
                    var_18 = (max(var_18, var_5));
                    var_19 += ((((((((var_14 ? 1 : var_0))) ? ((-(arr_0 [9] [9]))) : var_11))) ? (+-8193944089119136386) : ((((arr_5 [5] [i_1 + 2] [i_1]) ? (((arr_6 [i_1]) ? -1086716530299907784 : (arr_4 [i_0] [i_1]))) : ((0 ? 1086716530299907768 : 4294967295)))))));
                    var_20 = (~7864320);
                }
            }
        }
    }
    var_21 = (((~((1086716530299907783 ? 32 : 7384455937286851977)))));
    var_22 |= (!0);
    #pragma endscop
}
