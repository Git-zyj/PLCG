/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222957
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222957 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222957
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_15 = 18446744073709551615;
        var_16 = ((((((var_2 != var_11) % var_5))) ? (((18446744073709551615 + var_5) % var_0)) : ((min(((var_10 >> (var_12 - 310346775))), var_6)))));
    }
    for (int i_1 = 0; i_1 < 24;i_1 += 1)
    {
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 1;i_3 += 1)
            {
                {
                    var_17 = (((!170) ? 0 : ((max((((!(-9223372036854775807 - 1)))), var_3)))));
                    var_18 = var_7;
                }
            }
        }
        var_19 -= (max((!var_1), (7947610636778956943 & 215)));
    }
    var_20 |= ((!(!33221)));
    var_21 = ((!(!var_9)));
    #pragma endscop
}
