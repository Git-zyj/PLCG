/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98482
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98482 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98482
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_16 = (((((~(arr_1 [i_0])))) ? (((var_13 ? (arr_2 [i_1] [i_1]) : var_2))) : (max((4294967293 / var_4), ((((arr_3 [i_0] [i_0] [i_0]) / var_3)))))));
                var_17 += var_2;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        {
                            var_18 += ((max((arr_6 [i_1] [i_1] [i_0]), (arr_6 [i_0] [i_0] [i_0]))));
                            var_19 *= (~((min(var_10, (var_13 > var_13)))));
                            var_20 *= ((-((min((max(0, var_9)), (arr_0 [i_0] [i_0]))))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
