/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82291
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82291 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82291
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_12, (((var_9 ? 3046578348 : var_0)))));
    var_13 = ((var_2 ? var_2 : var_0));

    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        var_14 = (~var_0);
        arr_3 [i_0] = (max((1 ^ -5), ((var_8 ? (arr_0 [i_0 - 1]) : (arr_2 [i_0 + 1])))));
        var_15 = ((!((((arr_1 [i_0] [i_0]) ? -32753 : (arr_0 [i_0 + 1]))))));
    }
    for (int i_1 = 0; i_1 < 23;i_1 += 1)
    {
        var_16 -= ((+(((arr_7 [i_1]) ? var_9 : 4213751114349401434))));
        var_17 = (arr_6 [i_1] [i_1]);
        var_18 += ((max(((~(arr_6 [i_1] [16]))), (arr_7 [7]))));
    }
    var_19 = var_9;
    var_20 = var_8;
    #pragma endscop
}
