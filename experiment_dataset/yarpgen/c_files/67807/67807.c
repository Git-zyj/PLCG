/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67807
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67807 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67807
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [i_0] = (arr_1 [i_0]);
                arr_6 [i_0] [i_0] [i_0] = (((arr_4 [i_0] [i_0]) >> (((!((max((arr_1 [i_0]), 2876492843172250113))))))));
                var_15 = (min(var_15, ((min((((arr_2 [i_0] [i_0] [i_1]) ? (((arr_0 [i_0]) ? (arr_0 [i_0]) : 6173872998089235695)) : (56581 && -6173872998089235695))), (arr_4 [i_0] [i_0]))))));
            }
        }
    }
    var_16 = var_10;
    #pragma endscop
}
