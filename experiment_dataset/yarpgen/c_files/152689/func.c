/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152689
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152689 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152689
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 1; i_0 < 24; i_0 += 2) 
    {
        arr_4 [i_0] [i_0] = ((/* implicit */int) var_6);
        var_11 = ((/* implicit */unsigned long long int) min((var_11), (((/* implicit */unsigned long long int) arr_1 [(unsigned char)16] [(unsigned char)16]))));
        var_12 = ((/* implicit */unsigned char) 134217727U);
        var_13 = ((/* implicit */long long int) ((((((/* implicit */int) var_9)) + (2147483647))) << (((arr_3 [i_0 + 1]) - (17635158660186417217ULL)))));
    }
    /* LoopSeq 1 */
    for (signed char i_1 = 0; i_1 < 18; i_1 += 4) 
    {
        arr_7 [i_1] = ((/* implicit */signed char) arr_6 [i_1]);
        var_14 = ((/* implicit */_Bool) (-(((/* implicit */int) var_7))));
    }
}
