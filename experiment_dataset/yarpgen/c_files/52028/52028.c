/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52028
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52028 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52028
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 15;i_1 += 1)
        {
            {
                arr_5 [i_1] = ((((((((arr_4 [i_0] [i_1 - 2]) & var_6))) ? (arr_0 [i_0] [10]) : (~var_9))) | (arr_3 [i_0] [i_1] [i_1 - 1])));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        {
                            var_18 *= (max((min((arr_3 [i_1 + 3] [i_1] [i_1 + 1]), ((47 & (arr_3 [i_0] [i_0] [i_0]))))), ((((((8791798054912 ? -47 : var_15))) ? var_5 : 65)))));
                            var_19 = var_11;
                        }
                    }
                }
            }
        }
    }
    var_20 *= var_9;
    var_21 = var_17;
    #pragma endscop
}
