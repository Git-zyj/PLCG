/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114072
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114072 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114072
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned short) min((((unsigned int) (unsigned char)200)), (((/* implicit */unsigned int) var_5))));
    var_12 = ((/* implicit */unsigned char) (~(var_10)));
    var_13 *= ((/* implicit */unsigned short) var_0);
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 10; i_0 += 3) 
    {
        var_14 = ((/* implicit */unsigned int) ((short) (-(var_7))));
        var_15 = ((/* implicit */unsigned char) ((arr_2 [i_0] [i_0]) | (((unsigned long long int) (unsigned char)200))));
        arr_3 [i_0] [i_0] = ((/* implicit */int) (unsigned char)68);
    }
    var_16 = ((/* implicit */unsigned long long int) var_7);
}
