/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198573
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198573 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198573
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 += 623493726;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_11 = (min((~var_6), (((arr_4 [i_0 - 3]) << (arr_4 [i_0 - 3])))));
                arr_6 [i_0] [i_0 - 3] = ((((((var_0 * var_7) * (((27 ? 623493726 : 1)))))) ? (((arr_1 [i_0 - 1]) * (arr_1 [i_0 - 2]))) : ((53716 * ((-116 * (arr_5 [i_0])))))));
                var_12 = var_9;
                var_13 = (arr_5 [i_0]);
            }
        }
    }
    var_14 = ((-(min((~4294967295), (!17)))));
    #pragma endscop
}
