/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 176297
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=176297 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/176297
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        var_15 = ((((max((min(-26280434341563408, 6940)), (631153534 & 631153538)))) ? (((((min(var_12, var_0))) & var_12))) : (min(((~(arr_1 [i_0]))), var_6))));
        arr_2 [i_0] [i_0] = (min((!1423993485), (((arr_1 [i_0]) ? (arr_1 [i_0]) : (arr_1 [i_0])))));
    }
    var_16 = var_2;
    #pragma endscop
}
