/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14625
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14625 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14625
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (-((10 ? var_0 : var_4)));
    var_12 = var_10;
    var_13 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [i_0] = (((39 / var_8) > ((((arr_3 [i_1 + 2] [i_1 + 2]) <= (arr_4 [i_1 + 1] [i_1] [i_1 + 2]))))));
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_14 = (arr_7 [i_4 + 3] [i_3 + 1] [i_3 + 1] [i_1 - 1]);
                                arr_16 [i_4] [i_3] [i_2] [i_1] [i_1] [i_3] [i_0] = (((arr_1 [6] [6]) ? 10 : (max(39, (arr_7 [i_0] [i_1] [i_3 - 1] [i_4])))));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
