/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41016
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41016 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41016
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */unsigned char) ((((((/* implicit */_Bool) var_5)) ? (((/* implicit */int) ((unsigned char) (short)-15313))) : (((var_2) / (((/* implicit */int) (_Bool)1)))))) * (((/* implicit */int) var_12))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (short i_0 = 0; i_0 < 20; i_0 += 4) 
    {
        var_14 = ((/* implicit */unsigned short) var_5);
        arr_3 [i_0] [i_0] = ((/* implicit */long long int) (~(((/* implicit */int) var_3))));
        var_15 = (+(1907262647));
        var_16 = ((/* implicit */signed char) (-(((/* implicit */int) var_7))));
        var_17 += ((/* implicit */int) var_1);
    }
    var_18 = ((/* implicit */unsigned char) max((var_18), (((/* implicit */unsigned char) ((max((max((((/* implicit */long long int) var_12)), (var_8))), (((/* implicit */long long int) ((signed char) var_12))))) >= (((/* implicit */long long int) ((/* implicit */int) (!(((/* implicit */_Bool) ((signed char) (signed char)-8))))))))))));
}
