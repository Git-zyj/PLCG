/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 245335
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=245335 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/245335
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned int i_0 = 1; i_0 < 23; i_0 += 3) 
    {
        for (unsigned short i_1 = 0; i_1 < 25; i_1 += 4) 
        {
            {
                var_17 = ((/* implicit */long long int) (!(((/* implicit */_Bool) min((((/* implicit */short) (unsigned char)136)), (arr_5 [i_1]))))));
                var_18 = ((/* implicit */int) ((((/* implicit */_Bool) (-(4156649159U)))) && (((/* implicit */_Bool) ((((/* implicit */_Bool) arr_3 [i_0] [i_1])) ? (((/* implicit */int) var_11)) : (((/* implicit */int) (unsigned char)131)))))));
            }
        } 
    } 
    var_19 = ((/* implicit */signed char) var_7);
    var_20 = ((/* implicit */long long int) (+((-(((/* implicit */int) var_10))))));
    var_21 = (!(((/* implicit */_Bool) (+(((((/* implicit */_Bool) var_9)) ? (var_2) : (((/* implicit */unsigned long long int) ((/* implicit */int) var_15)))))))));
}
