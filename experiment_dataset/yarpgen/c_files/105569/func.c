/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105569
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105569 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105569
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */_Bool) ((((/* implicit */long long int) max((((/* implicit */int) ((_Bool) var_7))), ((-(((/* implicit */int) var_5))))))) | (var_2)));
    /* LoopNest 3 */
    for (short i_0 = 0; i_0 < 11; i_0 += 4) 
    {
        for (unsigned char i_1 = 0; i_1 < 11; i_1 += 2) 
        {
            for (unsigned short i_2 = 0; i_2 < 11; i_2 += 1) 
            {
                {
                    var_11 += arr_0 [i_0] [i_2];
                    var_12 = ((/* implicit */unsigned char) (-(max((((arr_1 [(unsigned char)3]) ? (var_2) : (((/* implicit */long long int) ((/* implicit */int) var_9))))), (((/* implicit */long long int) var_5))))));
                }
            } 
        } 
    } 
}
