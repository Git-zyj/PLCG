/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106847
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106847 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106847
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] [i_1] = ((arr_2 [i_0] [i_0] [i_0]) ? var_10 : ((((arr_3 [i_0] [i_1]) ? var_8 : var_13))));
                var_19 &= (((((-(arr_4 [i_0] [i_1] [i_1])))) ? (min(-var_17, (min(var_0, var_5)))) : ((max((((5638453499197705061 && (arr_1 [i_0])))), (arr_1 [i_1]))))));
                var_20 ^= (max((arr_1 [i_0]), (max((max(var_16, var_15)), 594636526))));
            }
        }
    }
    var_21 ^= var_16;
    var_22 = (min(var_22, var_8));
    #pragma endscop
}
