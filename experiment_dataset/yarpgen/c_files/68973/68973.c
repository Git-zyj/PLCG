/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68973
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68973 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68973
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {

        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            var_10 = (max(var_10, ((~(arr_3 [i_1] [i_1 + 4])))));
            var_11 = 4611686018427387904;
        }
        arr_5 [i_0] = (!688550406);
        /* LoopNest 3 */
        for (int i_2 = 1; i_2 < 15;i_2 += 1)
        {
            for (int i_3 = 1; i_3 < 18;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 19;i_4 += 1)
                {
                    {
                        var_12 ^= (((arr_8 [i_0] [i_2 + 2] [i_3]) || (arr_9 [i_2] [i_4])));
                        arr_14 [i_0] [i_2 - 1] [i_3 - 1] [i_4] = (var_4 ? (((arr_9 [i_2] [i_2]) - var_2)) : (((!(arr_8 [i_4] [i_3] [i_2 + 1])))));
                    }
                }
            }
        }
    }
    var_13 ^= var_7;
    #pragma endscop
}
