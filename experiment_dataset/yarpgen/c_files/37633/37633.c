/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 37633
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=37633 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/37633
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((var_1 + 2147483647) << (var_7 - 17984)));

    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        arr_2 [i_0 + 4] = (arr_1 [i_0] [i_0]);
        arr_3 [i_0] [i_0 - 1] = ((((((arr_0 [i_0 + 3]) ? 226 : 39))) && ((min((arr_0 [i_0 + 4]), (arr_0 [i_0 + 1]))))));
        var_12 -= (1214013116186176258 * 204);
        var_13 = (((!52) >> ((((220 ? var_6 : (arr_0 [i_0]))) - 61118))));
        arr_4 [i_0] = ((-(arr_0 [i_0 + 2])));
    }
    #pragma endscop
}
