/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 58104
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=58104 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/58104
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_7;
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    var_11 = (min(var_11, ((((((arr_4 [i_1] [4] [i_1 - 1]) * (arr_5 [i_2] [i_0 + 1]))) / (arr_9 [i_0 + 2] [i_1] [i_1 + 1]))))));
                    arr_10 [i_0 + 2] [i_1] = ((-((min(var_8, var_0)))));
                    var_12 = ((((arr_6 [i_0 + 2] [i_0] [i_1]) ? (arr_6 [i_0 + 2] [i_0] [i_1]) : var_9)));
                    var_13 ^= (((((arr_1 [i_0]) && 3550122673)) ? (arr_6 [i_0] [i_0 - 1] [i_2]) : (~var_3)));
                }
            }
        }
    }
    #pragma endscop
}
