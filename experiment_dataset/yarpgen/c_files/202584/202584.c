/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202584
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202584 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202584
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_16 = 1;
        var_17 = (1 ^ var_14);

        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            arr_5 [i_0] = arr_3 [i_0] [i_0];
            arr_6 [i_0] [i_0] [i_1] = -var_9;
            arr_7 [i_0] = 0;
        }
    }
    for (int i_2 = 0; i_2 < 1;i_2 += 1)
    {
        arr_10 [i_2] = (((1 + 1) ? (~1) : ((-((0 ? 1 : 1))))));
        var_18 ^= (max(1, (max(((1 ? var_13 : 1)), var_9))));
    }
    for (int i_3 = 0; i_3 < 1;i_3 += 1)
    {
        var_19 = 0;
        arr_15 [i_3] [i_3] = ((1 ? (arr_1 [1]) : ((0 ? (arr_14 [i_3] [i_3]) : (arr_14 [i_3] [i_3])))));
    }
    var_20 += var_2;
    var_21 = 1;
    var_22 = ((var_6 ? (((min(1, 1)) ? var_1 : ((min(1, var_15))))) : (((var_14 * var_11) ? ((0 ? var_1 : var_1)) : (!1)))));
    #pragma endscop
}
