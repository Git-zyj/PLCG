/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221363
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221363 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221363
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = 2;

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 1;i_3 += 1)
                {
                    {
                        var_18 = (((arr_2 [23] [i_3]) % ((min(7853, 230708308)))));
                        arr_13 [i_0] = (max((arr_0 [i_1] [i_3]), ((min(1, 1)))));
                        var_19 = (arr_5 [i_0] [i_0] [i_0]);
                    }
                }
            }
        }
        var_20 -= (((min(1, 1)) ? ((-(!24107))) : 25963));
        arr_14 [i_0] = (~1);
    }
    for (int i_4 = 0; i_4 < 15;i_4 += 1)
    {
        var_21 = (max(var_21, 8656784812455241094));
        arr_18 [i_4] [i_4] = (max(-121, (((((1 ? (arr_12 [1]) : 7152250106781376860))) ? ((((arr_2 [i_4] [20]) << (127 - 117)))) : (max(435481258, (arr_12 [20])))))));
    }
    #pragma endscop
}
