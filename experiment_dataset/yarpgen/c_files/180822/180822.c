/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180822
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180822 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180822
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_6;
    var_18 = var_1;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            arr_6 [i_0] [i_0] = (((arr_1 [i_0]) ? (arr_4 [i_0] [i_1] [i_1]) : (arr_1 [i_1])));
            var_19 = (max(var_19, (((!(arr_5 [i_0] [i_1]))))));
        }
        for (int i_2 = 0; i_2 < 11;i_2 += 1)
        {
            arr_10 [i_0] = ((((((arr_8 [i_2] [i_0]) * (arr_3 [i_0])))) ? (arr_9 [i_2] [i_2]) : (arr_0 [i_0] [i_2])));
            var_20 = var_6;
        }
        var_21 -= ((!(arr_2 [i_0])));
    }
    /* vectorizable */
    for (int i_3 = 0; i_3 < 18;i_3 += 1) /* same iter space */
    {
        arr_13 [i_3] [i_3] = (((arr_11 [11] [i_3]) ? (arr_12 [i_3] [i_3]) : (arr_12 [i_3] [i_3])));
        var_22 ^= ((arr_11 [i_3] [i_3]) & ((((!(arr_12 [i_3] [i_3]))))));
    }
    /* vectorizable */
    for (int i_4 = 0; i_4 < 18;i_4 += 1) /* same iter space */
    {
        arr_16 [6] [6] = ((~(arr_15 [i_4])));
        arr_17 [i_4] = (arr_12 [7] [i_4]);
    }
    #pragma endscop
}
