/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73249
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73249 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73249
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((max(1545, (max(var_5, -73)))));
    var_11 += ((((var_1 > -1069141086666497958) + -1555394383)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_12 = (min(var_12, (arr_0 [10] [i_0])));
                    var_13 = (min(var_9, (min((((arr_5 [i_2] [i_1] [i_0]) ? (arr_5 [i_0] [i_1] [i_0]) : 1345417177365385021)), 0))));
                }
            }
        }
    }
    var_14 = var_9;
    var_15 = var_0;
    #pragma endscop
}
