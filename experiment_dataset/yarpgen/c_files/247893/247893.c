/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247893
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247893 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247893
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_12, ((var_9 ? (var_7 * var_11) : var_3))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_13 -= ((((21 ? 1 : 1)) >> ((((1 ? -105 : (1 >> 10))) + 113))));
                var_14 = (((((1 ? (((arr_1 [i_0] [i_1]) * 1)) : (arr_0 [i_0] [i_0])))) ? (arr_3 [i_0 - 1] [i_0 + 1]) : ((((((arr_2 [i_0 + 2]) - var_3))) ? (min(var_8, (arr_2 [i_1]))) : 92))));
            }
        }
    }
    #pragma endscop
}
