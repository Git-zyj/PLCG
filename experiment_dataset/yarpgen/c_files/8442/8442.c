/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 8442
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=8442 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/8442
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = (-((min((max(1, -5)), 0))));
        arr_4 [i_0] = max(((((var_9 == var_10) > var_5))), ((var_0 << (var_10 - 26867))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        arr_7 [i_1] [i_1] = ((((var_5 ? var_2 : var_7)) * (var_9 << var_5)));
        arr_8 [i_1] = var_3;
        arr_9 [i_1] [i_1] = var_2;
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 14;i_2 += 1)
    {
        var_13 = var_11;
        var_14 = (max(var_14, ((var_6 ? var_0 : var_6))));
        arr_13 [i_2] [i_2] &= var_9;
        arr_14 [9] [9] = var_2;
    }
    var_15 = max(500532101, 1);
    #pragma endscop
}
