/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194004
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194004 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194004
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 ^= (((((((var_11 ? 32643 : var_10)) == (var_17 ^ var_16)))) % (var_7 || var_5)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 7;i_1 += 1)
        {
            {
                arr_4 [i_0] [7] = (min((arr_0 [i_0]), ((((arr_3 [i_1]) > (arr_0 [i_1 + 3]))))));
                arr_5 [i_0] [i_0] [i_1] = ((min(47937, (((var_5 ? var_16 : (arr_1 [i_1 + 2])))))));
                var_19 ^= (max(var_8, ((-18446744073709551615 % (((max(32652, 32750))))))));
                arr_6 [i_0] [i_0] = var_8;
            }
        }
    }
    var_20 = 18446744073709551615;
    #pragma endscop
}
