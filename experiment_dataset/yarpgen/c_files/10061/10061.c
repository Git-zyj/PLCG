/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 10061
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=10061 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/10061
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 ^= (max(var_2, var_2));
    var_14 = (max(var_14, (((var_6 ? -var_1 : var_0)))));
    var_15 = var_2;
    var_16 = (min(var_8, (!2816515088135319059)));

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_17 = ((((((arr_1 [i_0]) ? (arr_0 [i_0]) : 12130))) ? (arr_1 [i_0]) : (((!(arr_1 [1]))))));
        var_18 += (2816515088135319059 / 126);
        var_19 += ((~(((arr_1 [i_0]) ? (arr_1 [i_0]) : (arr_1 [i_0])))));
        arr_2 [i_0] = (arr_0 [i_0]);
        arr_3 [i_0] = (min(((max((arr_1 [i_0]), (arr_1 [i_0])))), (((arr_1 [i_0]) ? 2816515088135319054 : (arr_0 [i_0])))));
    }
    #pragma endscop
}
