/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 108726
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=108726 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/108726
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 2 */
    for (short i_0 = 0; i_0 < 23; i_0 += 1) 
    {
        arr_2 [i_0] [i_0] = ((/* implicit */int) arr_1 [i_0]);
        var_12 = ((var_0) ? (((/* implicit */int) (unsigned short)7)) : (((/* implicit */int) ((arr_0 [i_0]) <= (((/* implicit */int) (!(((/* implicit */_Bool) var_8)))))))));
    }
    for (unsigned int i_1 = 0; i_1 < 21; i_1 += 4) 
    {
        var_13 = ((/* implicit */unsigned char) max((var_13), (((/* implicit */unsigned char) ((((/* implicit */_Bool) arr_1 [i_1])) ? (((/* implicit */long long int) (-(min((arr_0 [i_1]), (((/* implicit */int) (unsigned short)34277))))))) : (((long long int) min((575897802350002176LL), (1011253739266033821LL)))))))));
        var_14 = ((/* implicit */int) var_8);
    }
    var_15 = ((/* implicit */int) min((var_15), (((/* implicit */int) var_0))));
}
