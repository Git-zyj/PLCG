/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100351
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100351 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100351
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 12; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 14; i_1 += 2) 
        {
            {
                arr_4 [i_0] [12] [i_0] = ((((/* implicit */_Bool) min((((/* implicit */long long int) var_7)), (((long long int) (unsigned char)150))))) ? (((long long int) (unsigned char)150)) : (((/* implicit */long long int) (+(((/* implicit */int) (unsigned char)150))))));
                arr_5 [i_0] [i_1] = ((/* implicit */signed char) var_9);
            }
        } 
    } 
    var_15 ^= ((/* implicit */_Bool) var_6);
    var_16 = ((/* implicit */signed char) max(((_Bool)0), ((_Bool)1)));
}
