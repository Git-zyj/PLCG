/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68280
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68280 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68280
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */long long int) min((var_12), (((/* implicit */long long int) 2147483647))));
    var_13 = ((/* implicit */int) max((var_13), (var_4)));
    var_14 = ((/* implicit */signed char) var_5);
    /* LoopSeq 1 */
    for (unsigned short i_0 = 0; i_0 < 25; i_0 += 4) 
    {
        var_15 *= ((/* implicit */unsigned long long int) 2147483621);
        var_16 *= ((/* implicit */short) max((((/* implicit */unsigned long long int) 1893693596)), (max((arr_0 [i_0]), (0ULL)))));
    }
}
