/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 42236
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=42236 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/42236
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    var_17 *= (((((var_7 ? (arr_4 [i_1]) : var_12))) ? ((((arr_3 [i_0] [i_0] [i_0]) ? (arr_6 [i_0] [i_1] [i_2] [i_2]) : var_3))) : (~var_1)));
                    var_18 |= var_5;
                    var_19 = (max(var_19, ((((max((arr_0 [i_0] [i_0]), (arr_0 [i_0] [i_1]))) != ((0 ? (arr_6 [i_0] [i_0] [i_0] [16]) : (arr_4 [i_0]))))))));
                }
            }
        }
    }
    var_20 += (~var_6);
    #pragma endscop
}
