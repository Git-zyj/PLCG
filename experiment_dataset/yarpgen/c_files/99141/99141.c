/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99141
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99141 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99141
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 &= -7413306835544579798;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_13 += ((((((((arr_4 [i_0 - 1]) << (var_3 - 59084))) + (-6453428977483000614 & 3290980016)))) ? ((((((var_9 ? -6453428977483000633 : 6453428977483000634)) <= 1014673044)))) : -2205144853622985347));
                    arr_9 [i_1] = (arr_0 [i_0] [i_0]);
                }
            }
        }
    }
    var_14 = var_1;
    var_15 |= (var_5 != 2453599155);
    #pragma endscop
}
