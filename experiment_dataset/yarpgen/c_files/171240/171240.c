/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171240
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171240 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171240
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_19 = 21;
                arr_4 [i_0] [i_0] [11] = (((((var_12 ? (arr_3 [i_0]) : ((18446744073709551595 & (arr_2 [i_0] [i_0] [i_1])))))) ? var_12 : (arr_3 [i_0])));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 12;i_3 += 1)
                    {
                        {
                            arr_9 [i_0] = ((((max(18446744073709551569, ((max(var_16, 6522)))))) ? var_5 : 16));
                            arr_10 [i_0] [i_2] [i_2] [i_1] [i_0] = (arr_2 [i_2] [i_2] [i_0]);
                            arr_11 [i_0] [i_0] = var_12;
                            arr_12 [i_0 + 1] [i_1] [i_2] [i_3] |= (arr_1 [i_3 + 2]);
                        }
                    }
                }
            }
        }
    }
    var_20 = var_17;
    #pragma endscop
}
