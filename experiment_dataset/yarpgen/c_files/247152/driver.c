#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

unsigned char var_0 = (unsigned char)47;
_Bool var_2 = (_Bool)0;
int var_12 = -1525603744;
unsigned char var_13 = (unsigned char)148;
unsigned char var_15 = (unsigned char)82;
int zero = 0;
short var_16 = (short)15326;
unsigned char var_17 = (unsigned char)65;
unsigned char var_18 = (unsigned char)64;
unsigned long long int var_19 = 12385234072909669988ULL;
short var_20 = (short)24637;
short var_21 = (short)1844;
unsigned int var_22 = 441758538U;
int arr_0 [24] ;
_Bool arr_3 [24] [24] [24] ;
void init() {
    for (size_t i_0 = 0; i_0 < 24; ++i_0) 
        arr_0 [i_0] = -1170786465;
    for (size_t i_0 = 0; i_0 < 24; ++i_0) 
        for (size_t i_1 = 0; i_1 < 24; ++i_1) 
            for (size_t i_2 = 0; i_2 < 24; ++i_2) 
                arr_3 [i_0] [i_1] [i_2] = (_Bool)1;
}

void checksum() {
    hash(&seed, var_16);
    hash(&seed, var_17);
    hash(&seed, var_18);
    hash(&seed, var_19);
    hash(&seed, var_20);
    hash(&seed, var_21);
    hash(&seed, var_22);
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
