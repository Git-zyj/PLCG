/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 129088
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=129088 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/129088
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    arr_8 [i_1] = (min((((arr_1 [i_1 - 1] [i_1 - 1]) ? (arr_1 [i_1 - 1] [i_1 - 1]) : 0)), ((min((arr_1 [i_1 - 1] [i_1 - 1]), 0)))));
                    var_14 = ((((max((min(13461329367761838587, 215)), var_4))) ? ((((arr_5 [i_2]) + (arr_1 [i_0] [7])))) : (arr_7 [0] [i_1 - 1] [i_1] [i_0])));
                }
            }
        }
    }
    var_15 = 1;
    #pragma endscop
}
