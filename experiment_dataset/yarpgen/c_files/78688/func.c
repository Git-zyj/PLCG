/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78688
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78688 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78688
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 13; i_0 += 4) 
    {
        for (unsigned char i_1 = 0; i_1 < 16; i_1 += 2) 
        {
            {
                var_11 *= ((/* implicit */unsigned int) max((((/* implicit */unsigned long long int) (_Bool)1)), (max((((/* implicit */unsigned long long int) arr_3 [i_0 + 3])), (var_7)))));
                var_12 = ((/* implicit */unsigned int) (unsigned char)241);
                var_13 = ((/* implicit */unsigned char) min((var_13), (((/* implicit */unsigned char) var_8))));
                var_14 = ((/* implicit */unsigned int) arr_0 [i_0 + 1]);
            }
        } 
    } 
    var_15 = ((/* implicit */unsigned int) min(((-(((/* implicit */int) ((((/* implicit */_Bool) var_2)) || (((/* implicit */_Bool) 6423872108895690100LL))))))), (((/* implicit */int) ((_Bool) (_Bool)1)))));
}
