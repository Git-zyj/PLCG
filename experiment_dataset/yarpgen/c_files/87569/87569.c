/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87569
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87569 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87569
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 &= var_2;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_13 = (min(var_13, -7645));
                var_14 = (+(max((!var_0), ((-7626 ? var_10 : (arr_5 [i_0] [i_1]))))));
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 1; i_2 < 7;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 8;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 10;i_4 += 1)
            {
                {
                    arr_15 [i_3] [8] = ((!(arr_13 [i_3] [i_3 + 2] [i_4] [i_2 - 1])));
                    arr_16 [i_4] [i_4] |= 7645;
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 10;i_5 += 1)
                    {
                        for (int i_6 = 2; i_6 < 8;i_6 += 1)
                        {
                            {
                                var_15 = ((((arr_5 [i_2 - 1] [i_2 - 1]) <= (arr_5 [i_4] [i_6 + 1]))) ? (((((arr_5 [i_3 - 1] [i_6 - 1]) == -7646)))) : 7668);
                                arr_23 [6] [i_5] [i_5] [i_2] &= ((~(arr_9 [i_4])));
                            }
                        }
                    }
                }
            }
        }
    }
    var_16 = (var_3 - var_0);
    #pragma endscop
}
