/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99893
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99893 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99893
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_16 = (arr_3 [i_0] [i_0]);
                arr_7 [i_1] = ((((var_0 ^ (arr_3 [i_0] [i_1])))));
                arr_8 [i_0] = (min((arr_3 [i_0] [i_1]), var_8));
                var_17 = (((!(arr_3 [i_0] [i_1]))) ? (((((~(arr_0 [i_0])))))) : ((1014188739 ? (arr_1 [i_0]) : (arr_6 [i_0]))));
            }
        }
    }
    var_18 = (((!1) & (max(var_11, ((var_10 >> (var_15 + 784734154)))))));
    var_19 = (!var_5);
    var_20 = (min(var_7, var_10));
    #pragma endscop
}
