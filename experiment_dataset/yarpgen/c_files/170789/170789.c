/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 170789
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=170789 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/170789
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_5 [16] [i_0 - 1] [i_0] = ((min((arr_1 [i_1]), var_2)));
                var_10 *= (((1583936162 >= -510959075554630171) ? (arr_2 [i_0] [i_1]) : var_3));
                arr_6 [13] = ((!(((~(((arr_3 [i_1] [i_1] [i_1]) ? -21 : var_6)))))));
                arr_7 [3] [i_1] [i_1] = ((!((min(510959075554630166, (12600427914768960732 % -1397591549292787174))))));
            }
        }
    }
    var_11 += var_9;
    #pragma endscop
}
