/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 57587
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=57587 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/57587
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_20 = (min((var_16 * var_6), (arr_3 [22] [22])));
                arr_4 [i_0] = (arr_0 [i_0]);
                arr_5 [i_1] [i_0 + 2] [19] = var_11;
            }
        }
    }
    var_21 = var_10;

    for (int i_2 = 2; i_2 < 20;i_2 += 1)
    {
        arr_9 [i_2] = (arr_0 [i_2]);
        var_22 += (arr_6 [i_2] [i_2 + 1]);
        var_23 ^= ((1761945953 > ((((!(arr_2 [i_2 - 2] [i_2 - 2]))) ? var_10 : (arr_8 [i_2 - 1])))));
        arr_10 [i_2] [7] = (arr_7 [i_2]);
    }
    var_24 *= (min((((1761945950 & 7337) ? -2 : (max(var_14, var_17)))), var_11));
    #pragma endscop
}
