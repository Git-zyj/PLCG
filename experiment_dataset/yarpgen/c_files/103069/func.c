/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103069
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103069 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103069
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 ^= ((/* implicit */unsigned char) var_10);
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15; i_0 += 4) 
    {
        for (int i_1 = 0; i_1 < 16; i_1 += 4) 
        {
            {
                arr_6 [i_0 + 1] [i_0 + 1] = ((/* implicit */short) ((((/* implicit */_Bool) ((((/* implicit */_Bool) arr_3 [i_0 - 1])) ? (((/* implicit */int) arr_0 [i_0 + 1])) : (((/* implicit */int) arr_4 [i_0 - 1]))))) ? (max((((/* implicit */int) var_3)), ((-(((/* implicit */int) arr_1 [i_0 + 1])))))) : ((-(1249866267)))));
                var_20 += ((/* implicit */long long int) var_14);
            }
        } 
    } 
}
