/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21111
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21111 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21111
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (((-1658944582 + 2147483647) << (((var_11 + 31162) - 27))));
    var_15 &= var_2;
    var_16 = 2147483647;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    var_17 = (min(880352915, ((((arr_7 [i_0] [11]) ? 12838281581973381259 : (arr_5 [i_0] [i_0] [i_0]))))));
                    var_18 *= (((arr_7 [i_1] [i_0]) ? ((1189614776 ? (arr_7 [i_2] [i_1]) : 880352937)) : (((((arr_7 [i_0] [i_1]) || (arr_7 [i_1] [i_2])))))));
                }
            }
        }
    }
    #pragma endscop
}
