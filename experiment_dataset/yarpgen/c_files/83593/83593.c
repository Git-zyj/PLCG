/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83593
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83593 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83593
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        arr_2 [11] [i_0] = (min(((-(arr_0 [i_0]))), (((arr_0 [i_0]) % (arr_1 [i_0])))));
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 19;i_3 += 1)
                {
                    {
                        arr_11 [i_0] [i_1] [i_2] [i_0] = (min((var_5 && var_2), ((var_9 ? var_5 : var_1))));
                        arr_12 [i_2] [i_3] = (103 == -30);
                    }
                }
            }
        }
        var_11 -= (arr_3 [i_0] [9] [i_0]);
        arr_13 [i_0] = var_1;
    }
    var_12 = ((((((-19417 ? 0 : var_1)) << (-21 + 32))) != (((-85 + 2147483647) << (126139896 - 126139896)))));
    #pragma endscop
}
