/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43517
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43517 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43517
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = min(((var_2 ? var_3 : (-4 + var_6))), (var_4 || -var_7));

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_13 ^= (((((var_3 > (((arr_0 [2]) + (arr_1 [i_0])))))) >> ((((-(arr_1 [i_0]))) + 22316))));

        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            var_14 += var_4;
            var_15 = (((((~(arr_1 [i_0])))) - ((var_6 ? (arr_2 [i_1]) : 3328681596363719443))));
        }
        arr_5 [i_0] = (((((((((arr_1 [0]) ? (arr_0 [i_0]) : -1353346176))) ? (arr_2 [i_0]) : -var_9))) || (arr_0 [i_0])));
    }
    var_16 = var_5;
    var_17 = var_6;
    #pragma endscop
}
