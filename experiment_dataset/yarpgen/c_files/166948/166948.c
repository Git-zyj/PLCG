/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166948
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166948 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166948
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    var_12 = (min((21 | 24973), (((2132754671 < ((-28426 ? 24973 : -24952)))))));
                    var_13 |= ((-((min(var_11, 24)))));
                    var_14 = (min(var_14, ((min(2335, ((2172104945 ? var_10 : var_1)))))));
                    var_15 = ((var_3 ? -8294533279970120069 : var_2));
                }
            }
        }
    }
    var_16 = (min(var_16, (3266560862 ^ 0)));
    var_17 = (!var_8);
    var_18 = (~var_4);
    #pragma endscop
}
