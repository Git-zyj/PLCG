/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168844
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168844 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168844
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */int) (unsigned short)20950);
    /* LoopSeq 1 */
    /* vectorizable */
    for (short i_0 = 0; i_0 < 25; i_0 += 4) 
    {
        var_21 = var_16;
        var_22 = ((/* implicit */unsigned short) var_18);
        var_23 = ((/* implicit */int) (-(((3117608226U) >> (((((/* implicit */int) var_18)) + (57)))))));
        var_24 = ((/* implicit */short) min((var_24), (((/* implicit */short) (-(((/* implicit */int) arr_0 [i_0])))))));
    }
    var_25 = ((/* implicit */long long int) (signed char)-98);
}
