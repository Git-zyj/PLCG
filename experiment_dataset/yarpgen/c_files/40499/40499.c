/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40499
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40499 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40499
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 19;i_0 += 1)
    {

        for (int i_1 = 2; i_1 < 19;i_1 += 1) /* same iter space */
        {
            /* LoopNest 3 */
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 20;i_3 += 1)
                {
                    for (int i_4 = 1; i_4 < 19;i_4 += 1)
                    {
                        {
                            arr_13 [i_3] [i_3] [i_3] [i_3] [i_3] [i_3] [i_3] = (-2147483647 - 1);
                            arr_14 [i_4] [i_3] [i_2] [i_1 - 1] [i_0 - 2] = (arr_8 [i_0]);
                        }
                    }
                }
            }
            arr_15 [i_0] = (!var_11);
        }
        for (int i_5 = 2; i_5 < 19;i_5 += 1) /* same iter space */
        {
            arr_19 [i_5 - 1] = (((var_1 >= (var_5 + var_2))) ? (arr_7 [i_5 + 1] [i_5 + 1] [i_0] [i_0 - 2]) : var_5);
            arr_20 [i_5] [i_0] = var_4;
        }
        arr_21 [i_0] [i_0 - 1] = -23077;
    }
    var_12 = ((((-9223372036854775807 - 1) != (var_6 > var_3))));
    var_13 = -2147483620;
    #pragma endscop
}
