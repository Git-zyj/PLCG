/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111226
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111226 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111226
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 23;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = (arr_1 [i_0]);
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 23;i_2 += 1)
            {
                {
                    var_20 = ((((max((arr_9 [14] [i_2 + 1] [4] [i_2]), (arr_9 [i_2] [i_2 - 1] [i_0] [i_0])))) ? ((((arr_9 [i_2] [i_2 + 1] [i_2 - 1] [1]) > (arr_9 [20] [i_2 + 1] [i_1] [i_1])))) : ((max((arr_9 [i_2 + 1] [i_2 - 1] [i_1] [i_0]), -34)))));
                    var_21 |= (arr_3 [i_0] [16]);
                }
            }
        }
    }
    var_22 = ((var_8 > ((var_5 ? var_16 : var_10))));
    var_23 = 102;
    #pragma endscop
}
