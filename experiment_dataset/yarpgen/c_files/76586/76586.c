/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76586
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76586 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76586
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    var_12 = (max(var_12, (((-(((!var_4) ? var_7 : (((-9223372036854775807 - 1) | (arr_2 [i_1]))))))))));
                    arr_9 [i_0] [i_1] = (~255);
                    arr_10 [i_0] [i_1] [i_2] = (arr_2 [i_2]);
                }
            }
        }
    }
    var_13 = ((-((-var_1 ? (~var_11) : ((min(268431360, 268431361)))))));
    var_14 = (min(((~(max(var_11, var_11)))), ((((((var_2 ? var_4 : 19))) ? ((max(var_4, var_2))) : ((var_1 ? var_7 : 268431360)))))));
    var_15 = (-var_2 & var_8);
    #pragma endscop
}
