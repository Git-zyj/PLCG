/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44078
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44078 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44078
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        {
                            arr_12 [i_0] [i_0] [i_0] [i_2] = max(((var_9 & (max(9859636999442153650, 13163396792071547776)))), (!var_6));
                            var_10 = (min(var_2, var_7));
                            var_11 = (max(var_11, (min((arr_0 [i_1 - 1] [i_1]), (((arr_7 [i_1 - 2] [i_1] [i_1 + 2] [8]) ? 16300444695899179406 : var_8))))));
                        }
                    }
                }
                arr_13 [i_1 + 2] [i_0] [i_0] = (arr_11 [6] [i_1 + 1]);
            }
        }
    }
    var_12 = var_4;
    #pragma endscop
}
