/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234203
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234203 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234203
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                arr_16 [i_1] = 13373280802821187187;
                                arr_17 [i_1] [i_1] = 0;
                                arr_18 [i_0] [i_1] [i_0] [i_0] [i_0] = var_6;
                                arr_19 [i_0] [i_0] [i_0] |= var_3;
                                arr_20 [i_1] [i_1] [i_2] = (-(((arr_13 [i_0] [i_1] [i_3] [i_4]) >> (((arr_1 [i_0] [i_0]) + 2115815475)))));
                            }
                        }
                    }
                }
                arr_21 [i_0] [i_0] [i_0] = 31;
            }
        }
    }
    var_20 = -1363302489;
    var_21 = (max(-293130963, (((((var_3 ? 294348214 : 4000619081))) ? (var_2 | var_2) : (max(var_6, var_4))))));
    var_22 = ((((((~293130992) + var_1))) || -293130963));
    var_23 = var_10;
    #pragma endscop
}
