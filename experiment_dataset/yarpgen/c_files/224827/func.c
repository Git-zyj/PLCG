/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224827
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224827 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224827
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */short) (-(((((/* implicit */_Bool) ((long long int) var_11))) ? (((((/* implicit */unsigned long long int) ((/* implicit */int) var_3))) % (65535ULL))) : (((/* implicit */unsigned long long int) ((((/* implicit */_Bool) var_5)) ? (var_14) : (((/* implicit */long long int) var_4)))))))));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 20; i_0 += 2) 
    {
        arr_2 [i_0] = ((/* implicit */_Bool) var_6);
        var_16 = ((/* implicit */short) ((unsigned long long int) ((_Bool) var_13)));
    }
    var_17 = ((/* implicit */unsigned int) var_0);
    var_18 = ((/* implicit */_Bool) var_11);
}
