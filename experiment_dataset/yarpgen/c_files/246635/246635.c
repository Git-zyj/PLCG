/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246635
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246635 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246635
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        arr_2 [i_0 - 1] [i_0 - 1] |= (arr_0 [i_0 + 1]);
        /* LoopNest 2 */
        for (int i_1 = 2; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 10;i_2 += 1)
            {
                {
                    var_10 = (max(var_10, (((((var_0 << (((var_1 + 695815653) - 15))))) ? (((-2147483647 - 1) ? 0 : var_6)) : (arr_3 [i_2])))));
                    var_11 = (max(var_11, ((((arr_4 [i_0] [7]) << var_3)))));
                    var_12 = (((arr_3 [i_1]) ? (arr_6 [i_0 + 1] [i_0 - 1] [10]) : (arr_5 [i_0 + 1])));
                    arr_9 [i_1] [i_1] [1] = 65529;
                }
            }
        }
        arr_10 [i_0] = ((~(arr_1 [i_0] [i_0])));
    }
    var_13 = var_3;
    #pragma endscop
}
