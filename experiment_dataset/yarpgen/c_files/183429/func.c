/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 183429
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=183429 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/183429
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 -= ((/* implicit */int) var_10);
    var_19 = ((/* implicit */unsigned short) var_5);
    var_20 = ((/* implicit */unsigned short) min((var_20), (var_14)));
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 25; i_0 += 1) 
    {
        for (_Bool i_1 = 1; i_1 < 1; i_1 += 1) 
        {
            {
                var_21 = arr_4 [(_Bool)1] [18U];
                arr_6 [i_1 - 1] [i_1] [i_1] = ((/* implicit */unsigned int) ((signed char) ((((/* implicit */_Bool) arr_0 [i_1 - 1])) ? (((/* implicit */int) arr_0 [i_1 - 1])) : (((/* implicit */int) arr_2 [i_1 - 1] [i_1])))));
                var_22 = ((/* implicit */_Bool) var_13);
                var_23 *= arr_3 [i_0] [i_1 - 1];
            }
        } 
    } 
}
