/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49394
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49394 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49394
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_10;
    var_13 += (((-var_4 ? -0 : (!var_3))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            {
                var_14 = (min(var_14, (((!((((var_11 || (arr_1 [i_0]))) || (((arr_0 [i_0] [i_0]) && (arr_0 [i_0] [i_1]))))))))));
                var_15 = (max(((var_9 ? ((var_9 ? var_1 : var_10)) : (~var_4))), ((var_6 ? (((!(arr_2 [i_0])))) : (!1)))));
                var_16 -= (var_9 % var_4);
                var_17 ^= var_3;
            }
        }
    }
    var_18 = max(var_9, (((var_4 & var_5) ? var_0 : var_1)));
    #pragma endscop
}
