/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191592
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191592 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191592
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [1] |= 89;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 14;i_3 += 1)
                    {
                        {
                            arr_13 [i_3] [i_1] [i_2] [i_2] [i_1] [i_0 + 2] = (arr_5 [i_1] [i_3 - 1]);
                            arr_14 [i_1] [i_1] [i_2] [i_3 - 2] = (arr_2 [i_3]);
                            arr_15 [i_0 + 3] [i_1] [i_1] [i_1] = ((((min((var_12 * 2), (arr_9 [i_0] [i_0] [i_0] [i_1])))) ? (arr_10 [i_0] [i_0] [i_0 + 3] [i_1] [i_0 + 2]) : 2050524220));
                        }
                    }
                }
            }
        }
    }
    var_18 |= ((-((max(((max(-99, 2))), var_12)))));
    var_19 += var_3;
    #pragma endscop
}
