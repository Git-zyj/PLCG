/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22016
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22016 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22016
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = 0;

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_21 = ((((max((((arr_1 [i_0]) - (arr_0 [i_0]))), var_10))) ? ((19 & (arr_0 [3]))) : ((((arr_1 [i_0]) >> (((arr_1 [i_0]) - 262669540)))))));
        arr_2 [10] = (max((min((arr_0 [i_0]), (arr_1 [i_0]))), ((-2997327076061065144 ? 3763308946 : -2997327076061065144))));
    }
    #pragma endscop
}
