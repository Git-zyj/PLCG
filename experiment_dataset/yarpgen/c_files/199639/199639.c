/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199639
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199639 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199639
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((max(var_6, ((var_0 ? -3960667542252984598 : 116))))) ? (!116) : ((var_8 ? var_7 : var_0))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            {
                var_12 = ((22014 / (((arr_1 [i_1 - 2] [i_1 + 1]) ? -19 : -96))));
                var_13 += ((((max((arr_0 [i_0]), (var_5 - -72)))) ? ((min(-var_6, ((2547110458 ? -18026 : (-2147483647 - 1)))))) : (arr_3 [i_1 - 2] [i_1 + 1] [i_0])));
            }
        }
    }
    #pragma endscop
}
