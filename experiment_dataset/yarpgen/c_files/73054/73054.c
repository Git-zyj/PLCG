/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73054
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73054 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73054
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    var_17 = 126;
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 8;i_4 += 1)
                        {
                            {
                                var_18 = ((min((arr_3 [i_2 - 1] [i_2 - 1] [i_2 - 1]), 2147483647)) != ((((arr_12 [i_2] [i_2] [i_2] [i_2 - 1] [i_2] [i_2] [i_2]) >= (!var_8)))));
                                arr_14 [i_0] [i_0] [i_2] [i_1] [i_2] [i_3] [i_1] = -418;
                            }
                        }
                    }
                }
            }
        }
    }

    /* vectorizable */
    for (int i_5 = 1; i_5 < 1;i_5 += 1)
    {
        var_19 = ((-(-1 >= 12658)));
        /* LoopNest 3 */
        for (int i_6 = 1; i_6 < 23;i_6 += 1)
        {
            for (int i_7 = 1; i_7 < 21;i_7 += 1)
            {
                for (int i_8 = 2; i_8 < 22;i_8 += 1)
                {
                    {
                        arr_28 [i_5] = 40558;
                        var_20 *= -2147483647;
                        var_21 = ((!((((arr_24 [i_6] [i_7] [i_6]) / 268435424)))));
                        var_22 = var_5;
                    }
                }
            }
        }
    }
    #pragma endscop
}
