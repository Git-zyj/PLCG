/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202231
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202231 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202231
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_3;
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 22;i_4 += 1)
                        {
                            {
                                arr_15 [i_0] [i_3] [i_0] [i_1] [i_0] = var_11;
                                arr_16 [i_0 - 3] [i_0] [i_0 + 3] [i_0 + 3] [i_0 + 3] = max(((min((arr_3 [i_0] [i_0]), var_11))), ((var_7 ? (arr_8 [i_3 + 1] [i_3 - 3] [i_0] [i_0]) : var_5)));
                            }
                        }
                    }
                    var_13 ^= var_2;
                }
            }
        }
    }
    var_14 = ((+(((~var_1) ? var_0 : var_5))));
    #pragma endscop
}
