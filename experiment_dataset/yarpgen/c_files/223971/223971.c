/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 223971
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=223971 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/223971
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    var_15 *= ((1 > ((-604334539 ? (!18150) : 10832923298903158557))));
                    var_16 = (min(var_16, (((((max(-7368684911444272580, var_4))) ? (((-((var_10 ? var_5 : (arr_8 [i_0 + 1])))))) : (7613820774806393034 % 15069968086616034823))))));
                    var_17 = (arr_8 [i_0]);
                }
            }
        }
    }
    var_18 = (((((-16084 ? 7613820774806393035 : 7791))) ? var_6 : ((var_0 ? var_14 : var_7))));
    var_19 = var_4;
    var_20 = var_8;
    var_21 = (min(var_21, var_4));
    #pragma endscop
}
