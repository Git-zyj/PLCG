/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45505
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45505 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45505
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (min(-var_14, 1023));

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        arr_2 [i_0] = (min(((((((-(arr_0 [6] [10]))) + 2147483647)) >> (arr_1 [i_0]))), ((-(arr_0 [i_0] [i_0])))));
        arr_3 [i_0] = (min((arr_0 [i_0] [i_0]), -229));
        arr_4 [6] [i_0] = ((!((max((arr_1 [i_0]), (arr_0 [i_0] [i_0]))))));
    }
    for (int i_1 = 0; i_1 < 22;i_1 += 1)
    {
        arr_7 [18] = (!1);
        arr_8 [0] = ((-((~(arr_5 [i_1])))));
    }
    var_20 = var_3;
    #pragma endscop
}
