/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120760
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120760 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120760
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = (!(((/* implicit */_Bool) var_15)));
    var_21 = ((/* implicit */unsigned char) min((((/* implicit */short) ((_Bool) var_6))), (var_9)));
    var_22 &= ((/* implicit */int) var_13);
    var_23 = ((/* implicit */_Bool) ((((/* implicit */int) var_6)) * ((~(((/* implicit */int) var_17))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned char i_0 = 0; i_0 < 19; i_0 += 2) 
    {
        arr_2 [i_0] = ((/* implicit */short) (+(((((/* implicit */_Bool) var_19)) ? (((/* implicit */int) (_Bool)0)) : (((/* implicit */int) var_18))))));
        arr_3 [i_0] = ((/* implicit */unsigned char) var_18);
        arr_4 [i_0] = ((/* implicit */unsigned char) (+(((/* implicit */int) var_10))));
        var_24 = ((/* implicit */int) max((var_24), ((-(((/* implicit */int) var_19))))));
    }
}
