/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 109885
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=109885 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/109885
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 16;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 19;i_2 += 1)
                {

                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        var_19 = ((((arr_8 [i_1] [i_1 - 1] [i_1]) ? var_8 : -66)));
                        var_20 = (max(var_20, (arr_2 [7])));
                    }
                    var_21 = (arr_3 [i_1 + 2] [i_1 + 1]);
                }
                var_22 = (((arr_6 [i_0] [i_1 + 3] [i_1]) & (((arr_4 [i_0] [i_1]) | (arr_2 [i_0])))));
                var_23 = (min(((~((65 + (arr_5 [i_0]))))), (((-4590857109615042458 == (arr_3 [i_1 + 1] [i_1 + 3]))))));
            }
        }
    }
    var_24 = ((!(~var_3)));
    var_25 = var_12;
    #pragma endscop
}
