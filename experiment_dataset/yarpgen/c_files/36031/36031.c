/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36031
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36031 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36031
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = ((((0 ? (arr_1 [i_0] [i_0]) : (arr_0 [i_0]))) <= 54103));
        arr_3 [i_0] = ((((((arr_0 [i_0]) ? -557208469948585635 : (arr_0 [i_0])))) ? (arr_1 [i_0] [i_0]) : ((~(arr_1 [i_0] [i_0])))));
        arr_4 [i_0] = (47680 && -94544410127730111);
    }
    var_12 = (max(var_12, var_4));
    #pragma endscop
}
