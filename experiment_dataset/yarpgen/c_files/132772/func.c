/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132772
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132772 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132772
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (long long int i_0 = 3; i_0 < 10; i_0 += 1) 
    {
        for (long long int i_1 = 3; i_1 < 11; i_1 += 2) 
        {
            {
                var_16 = var_0;
                var_17 = ((/* implicit */long long int) ((1359916803648423451LL) >= (9223372036854775807LL)));
            }
        } 
    } 
    var_18 = ((long long int) 9223372036854775807LL);
    var_19 = ((min(((-9223372036854775807LL - 1LL)), (7686649713475513044LL))) & (((/* implicit */long long int) ((/* implicit */int) ((938878997361366050LL) >= (9223372036854775807LL))))));
}
