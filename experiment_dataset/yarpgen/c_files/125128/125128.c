/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125128
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125128 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125128
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_19 = (min(var_19, (((~((((((arr_0 [i_0] [i_0]) ? var_10 : var_11)) != (arr_4 [0] [i_1])))))))));
                var_20 = (min(var_20, ((((arr_6 [4] [4]) < ((var_1 ? (arr_4 [6] [i_1]) : (arr_4 [10] [i_1]))))))));
            }
        }
    }
    var_21 = (max(var_21, ((((max((min(var_12, var_3)), (max(var_13, var_18)))) >= (((~(min(var_5, var_15))))))))));
    var_22 = (~var_1);
    var_23 = var_17;
    #pragma endscop
}
