/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234243
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234243 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234243
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */signed char) var_8);
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 3; i_0 < 10; i_0 += 4) 
    {
        for (signed char i_1 = 2; i_1 < 9; i_1 += 1) 
        {
            for (signed char i_2 = 0; i_2 < 12; i_2 += 4) 
            {
                {
                    var_12 = ((/* implicit */short) (unsigned short)12269);
                    var_13 = ((/* implicit */unsigned int) max((var_13), (((/* implicit */unsigned int) var_5))));
                    var_14 |= ((/* implicit */unsigned long long int) (-(3981296286U)));
                }
            } 
        } 
    } 
    var_15 ^= ((/* implicit */signed char) ((unsigned short) min((((/* implicit */unsigned int) var_2)), (max((104186385U), (((/* implicit */unsigned int) var_6)))))));
}
