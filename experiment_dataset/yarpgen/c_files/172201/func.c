/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172201
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172201 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172201
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned int) var_2);
    /* LoopNest 3 */
    for (unsigned short i_0 = 2; i_0 < 17; i_0 += 4) 
    {
        for (unsigned int i_1 = 0; i_1 < 20; i_1 += 1) 
        {
            for (signed char i_2 = 0; i_2 < 20; i_2 += 1) 
            {
                {
                    var_17 = ((/* implicit */int) (unsigned short)4);
                    arr_8 [i_1] [14U] = ((/* implicit */unsigned char) ((((/* implicit */long long int) ((((var_8) - (((/* implicit */unsigned int) ((/* implicit */int) (unsigned short)10515))))) + (var_8)))) - (min(((-(609970370954984106LL))), (((/* implicit */long long int) arr_1 [i_1] [i_0]))))));
                }
            } 
        } 
    } 
}
