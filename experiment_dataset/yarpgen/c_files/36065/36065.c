/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36065
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36065 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36065
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = max(var_9, (max((~11591865392499117286), ((-612989324 ? 14941066887826318128 : -9223372036854775786)))));
    var_11 = 0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] [i_0] = min((max(((((arr_3 [i_0] [6]) <= (arr_2 [i_1] [i_1])))), (arr_2 [i_1] [i_0]))), ((~((var_7 ? var_1 : 4925448679988882428)))));
                arr_5 [i_0] = (612989324 && 9223372036854775807);
                var_12 = (arr_3 [i_0] [i_1]);
            }
        }
    }
    #pragma endscop
}
