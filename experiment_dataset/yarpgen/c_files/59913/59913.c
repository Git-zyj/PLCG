/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59913
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59913 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59913
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 |= ((((((((-32767 - 1) ^ var_0)) ^ var_8))) || ((min(var_10, ((max(var_2, var_11))))))));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 21;i_2 += 1)
            {
                {
                    arr_8 [i_0 - 3] [i_1] [i_2] [i_0] = (arr_2 [i_0]);
                    var_13 = (+-33);
                    arr_9 [i_0] = var_3;
                    var_14 = ((min(var_7, (((arr_7 [i_0 + 1] [i_2] [i_0 - 3] [i_2]) | (arr_3 [i_2] [i_2] [i_2 - 1]))))) ^ ((((-41 ? -12 : var_1)) * var_11)));
                    arr_10 [i_0] [i_1] [i_2] = var_0;
                }
            }
        }
    }
    #pragma endscop
}
