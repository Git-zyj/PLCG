/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6841
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6841 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6841
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 21;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_19 -= (((arr_9 [i_4] [i_4] [i_3] [i_3 + 1]) & (arr_12 [i_4] [i_4] [i_4] [i_3] [i_3 - 2])));
                                var_20 ^= (arr_11 [i_3 + 1] [i_2 + 2]);
                            }
                        }
                    }
                    var_21 = ((((28247 * (arr_12 [i_0] [i_1] [i_2] [i_2] [i_0])))));
                    var_22 = (((28247 ? (arr_7 [i_0] [i_1] [i_1]) : ((((arr_10 [i_2] [i_1]) ? var_1 : -782582453))))));
                }
            }
        }
    }
    var_23 = (max(var_23, ((min((((var_14 ? var_5 : var_13))), var_5)))));
    #pragma endscop
}
