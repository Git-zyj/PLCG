/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83510
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83510 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83510
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 11;i_2 += 1)
            {
                {
                    arr_8 [i_0] [8] [i_2] [i_2] = ((((((((44656 ? 20879 : 760781434))) - 0))) & (min(208, ((-9223372036854775798 ? (arr_0 [i_2 - 1]) : (arr_6 [i_0] [i_0] [i_0] [i_0])))))));
                    var_19 = (((min((arr_2 [i_0] [i_2 - 2]), (arr_3 [i_0] [i_1 + 1])))) ? ((((arr_3 [1] [i_0]) >= (arr_2 [i_2 + 3] [i_1])))) : (((!(arr_3 [i_0] [i_2 - 2])))));
                }
            }
        }
    }
    var_20 = (max(var_20, -760781435));
    #pragma endscop
}
