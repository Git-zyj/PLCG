/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 13207
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=13207 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/13207
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, ((((!var_6) ? 2603791262 : var_7)))));

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_11 = (((arr_1 [i_0]) - (11 < -71)));
        var_12 ^= (((~var_9) ? ((var_0 ? 9223372036854775807 : (arr_1 [i_0]))) : (~800265460863074486)));
        arr_2 [3] [i_0] = (var_6 <= 24);
        var_13 = (((min(14108, 914))));
    }
    #pragma endscop
}
