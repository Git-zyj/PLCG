/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169245
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169245 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169245
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_10;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_16 = (min(var_16, ((((!(arr_4 [i_0]))) ? (max((((arr_0 [5]) & var_6)), ((((arr_2 [1]) && (arr_3 [0] [9])))))) : (((+((((arr_1 [i_0]) == (arr_4 [i_1])))))))))));
                var_17 = (max(((((var_11 ? (arr_4 [0]) : var_4)) * (((((arr_2 [i_0]) == (arr_3 [i_0] [i_0]))))))), (((((var_2 ? (arr_0 [8]) : var_1)) << (arr_3 [i_1] [i_0]))))));
            }
        }
    }
    #pragma endscop
}
