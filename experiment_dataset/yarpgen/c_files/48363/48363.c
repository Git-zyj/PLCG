/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48363
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48363 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48363
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = 20893;
    var_14 = var_11;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_6 [i_0] [8] = (((((arr_1 [i_0] [i_1]) > (arr_1 [i_0] [i_1]))) ? 2290893533 : ((((((arr_1 [i_0] [i_0]) + 2147483647)) >> (((arr_2 [i_1]) + 1646)))))));
                var_15 = ((((max((arr_0 [i_0]), var_12))) ? ((-(arr_0 [i_0]))) : ((((arr_0 [i_1]) < (arr_0 [i_1]))))));
            }
        }
    }
    var_16 = (2290893543 - 2004073763);
    #pragma endscop
}
