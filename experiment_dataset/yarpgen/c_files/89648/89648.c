/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89648
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89648 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89648
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 |= var_5;

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_17 = ((min(var_0, var_10)));
        var_18 |= ((-4091 ? (var_10 >= -4070) : var_5));
        var_19 = ((max(((var_6 * (arr_0 [i_0] [i_0]))), (arr_1 [i_0]))));
    }
    for (int i_1 = 0; i_1 < 24;i_1 += 1)
    {
        var_20 = ((((56 == var_11) ? (min(76, 7)) : (((!(arr_2 [i_1] [i_1])))))));
        var_21 = (-(var_6 && var_1));
        var_22 = (arr_3 [i_1]);
    }
    var_23 = ((((min(-372343901, var_7))) ? ((((max(60, var_15))) / 70)) : (max(((var_8 ? var_7 : var_7)), var_8))));
    var_24 = var_14;
    #pragma endscop
}
