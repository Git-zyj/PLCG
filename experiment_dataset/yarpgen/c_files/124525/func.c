/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124525
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124525 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124525
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned char i_0 = 4; i_0 < 18; i_0 += 2) 
    {
        arr_2 [i_0 - 3] = ((/* implicit */signed char) var_13);
        var_14 = ((/* implicit */_Bool) ((unsigned short) ((long long int) arr_1 [i_0 + 3])));
    }
    var_15 = var_12;
    var_16 = ((/* implicit */_Bool) max((var_16), (((/* implicit */_Bool) 3862900825U))));
    var_17 = ((/* implicit */unsigned short) var_11);
}
