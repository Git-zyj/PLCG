/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 113461
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=113461 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/113461
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = var_6;
                var_14 |= (min(32763, -1214699665));
                arr_7 [i_0] = (((var_4 >= 20321) * 1));
                var_15 = (min(var_15, ((-(min((((9519181584735045919 ? -20321 : 1))), 2854892513895675310))))));
                var_16 = (max(var_16, ((((arr_1 [2]) ? ((min((arr_1 [0]), (arr_1 [14])))) : (max(101, 5204210461420055231)))))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 24;i_3 += 1)
        {
            {
                arr_15 [i_2] [i_2] = var_8;
                var_17 *= var_4;
            }
        }
    }
    var_18 = 1;
    var_19 ^= ((~(!var_13)));
    #pragma endscop
}
