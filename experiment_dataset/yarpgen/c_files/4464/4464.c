/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4464
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4464 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4464
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_13 = (-(arr_1 [i_0] [i_0]));
        var_14 = (arr_2 [i_0]);
        arr_3 [5] = (((arr_0 [i_0] [i_0]) >> (90 - 72)));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        var_15 = ((-(arr_1 [14] [i_1])));
                        arr_13 [i_0] [i_0] [i_3] = ((arr_2 [i_1]) ? (0 > -26369447420510206) : ((-(arr_0 [i_0] [i_1]))));
                    }
                    arr_14 [i_0] [i_1] [i_1] [i_2] = (2147483647 & -var_5);
                    arr_15 [i_2] [i_1] = (((((6 ? -104 : -121))) ? var_12 : 3796151733));
                }
            }
        }
        var_16 = ((-(0 & 0)));
    }
    var_17 = (max(var_1, var_0));
    #pragma endscop
}
