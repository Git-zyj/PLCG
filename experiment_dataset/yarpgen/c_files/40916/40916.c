/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40916
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40916 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40916
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((var_0 ? 80339023 : var_5)) <= var_7));

    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        var_12 ^= ((-1152045496 ? 18446744073709551606 : (-9223372036854775807 - 1)));
        var_13 = (min(var_13, ((max((~var_1), ((((arr_0 [i_0 - 2] [i_0 + 1]) ? var_8 : (arr_0 [i_0 - 1] [i_0 - 2])))))))));
    }
    #pragma endscop
}
