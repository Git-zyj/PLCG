/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 134740
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=134740 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/134740
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 21; i_0 += 3) 
    {
        for (unsigned short i_1 = 0; i_1 < 21; i_1 += 1) 
        {
            {
                arr_5 [i_0] [i_1] |= ((/* implicit */int) var_18);
                arr_6 [i_0] [i_1] = ((/* implicit */unsigned char) var_6);
            }
        } 
    } 
    var_20 = ((/* implicit */int) ((unsigned char) -491128866));
    var_21 = ((/* implicit */int) min((((unsigned char) ((var_12) == (((/* implicit */unsigned long long int) ((/* implicit */int) (_Bool)0)))))), (((/* implicit */unsigned char) (!(((/* implicit */_Bool) ((var_13) + (((/* implicit */long long int) ((/* implicit */int) var_8)))))))))));
}
