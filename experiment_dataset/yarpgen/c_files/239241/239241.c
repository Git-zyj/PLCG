/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239241
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239241 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239241
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        var_11 = (((arr_0 [i_0 - 1]) - var_3));
        var_12 = ((-(var_3 >= var_1)));
    }
    var_13 = (((((((1313419326540761458 ? var_10 : var_2))) ? var_9 : (((var_3 ? var_3 : var_7))))) % var_1));
    var_14 = var_6;
    var_15 = ((((var_3 ? (var_0 & var_6) : var_2))) ? var_6 : (min((var_6 ^ var_8), var_5)));
    var_16 = var_9;
    #pragma endscop
}
