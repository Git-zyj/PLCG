/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67052
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67052 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67052
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned int) (+(((/* implicit */int) max((min((((/* implicit */short) (unsigned char)2)), (var_8))), (((/* implicit */short) (!(((/* implicit */_Bool) -1339446353))))))))));
    /* LoopSeq 2 */
    for (unsigned char i_0 = 0; i_0 < 17; i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] [i_0] = ((/* implicit */unsigned short) 7060515904780793583ULL);
        var_21 ^= (signed char)70;
    }
    /* vectorizable */
    for (unsigned char i_1 = 0; i_1 < 17; i_1 += 1) /* same iter space */
    {
        arr_6 [i_1] = arr_3 [i_1];
        var_22 = ((/* implicit */_Bool) ((((/* implicit */int) arr_3 [i_1])) & (((/* implicit */int) arr_5 [i_1] [i_1]))));
    }
}
