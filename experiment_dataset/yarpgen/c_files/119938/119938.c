/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119938
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119938 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119938
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    arr_8 [i_1] [i_1] [i_1] = var_0;
                    arr_9 [i_0] [i_0] [i_0] [i_1] = (((((4294967295 << (4183817916 - 4183817896)))) ? ((((!2147483647) ? (arr_2 [i_1] [i_0]) : (209 & 0)))) : ((var_7 ? var_12 : (arr_6 [i_0] [i_0])))));
                }
            }
        }
    }
    var_14 = (min(((0 - ((var_10 ? var_10 : var_13)))), (((((-127 - 1) ? var_1 : var_4)) * var_11))));
    #pragma endscop
}
