/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164445
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164445 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164445
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */signed char) var_2);
    var_13 = ((_Bool) var_9);
    var_14 = ((/* implicit */long long int) var_11);
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 12; i_0 += 2) 
    {
        for (int i_1 = 1; i_1 < 12; i_1 += 3) 
        {
            for (unsigned char i_2 = 2; i_2 < 11; i_2 += 3) 
            {
                {
                    /* LoopSeq 3 */
                    for (long long int i_3 = 0; i_3 < 13; i_3 += 3) 
                    {
                        var_15 = ((/* implicit */unsigned int) ((_Bool) arr_2 [i_0]));
                        var_16 = ((/* implicit */unsigned char) max((var_16), ((unsigned char)90)));
                        var_17 = ((/* implicit */unsigned int) var_2);
                    }
                    /* vectorizable */
                    for (unsigned short i_4 = 1; i_4 < 12; i_4 += 4) 
                    {
                        var_18 = ((/* implicit */short) (unsigned char)166);
                        var_19 |= ((/* implicit */unsigned long long int) arr_11 [i_0 - 1] [i_0 - 1] [i_0 + 1] [i_1] [i_2 + 2] [i_4]);
                    }
                    for (unsigned short i_5 = 1; i_5 < 12; i_5 += 1) 
                    {
                        var_20 = ((/* implicit */short) ((unsigned int) arr_14 [i_0] [i_1] [i_2] [i_1 - 1] [i_1 - 1] [12U]));
                        var_21 -= ((/* implicit */unsigned char) var_2);
                    }
                    var_22 = ((/* implicit */signed char) arr_5 [i_1] [i_0]);
                }
            } 
        } 
    } 
}
