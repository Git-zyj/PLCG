/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68807
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68807 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68807
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */short) var_10);
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 20; i_0 += 4) 
    {
        var_13 += ((/* implicit */_Bool) ((unsigned long long int) ((((/* implicit */int) (!(((/* implicit */_Bool) var_3))))) | ((-(((/* implicit */int) var_5)))))));
        arr_2 [(unsigned short)14] = ((/* implicit */signed char) min((((/* implicit */int) (signed char)-122)), (max((2147483647), ((~(((/* implicit */int) (short)8176))))))));
    }
}
