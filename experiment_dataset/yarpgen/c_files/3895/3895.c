/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3895
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3895 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3895
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (min(var_20, var_0));
    var_21 = (((var_19 ? var_3 : var_11)));

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        var_22 = (((((((((arr_0 [i_0]) ? var_9 : (arr_1 [i_0])))) ? ((max(var_19, (arr_2 [11])))) : ((var_8 ? var_17 : var_14))))) ? (min((arr_2 [i_0]), ((var_2 ? (arr_1 [i_0]) : (arr_0 [i_0]))))) : (((var_3 ? var_10 : var_2)))));
        var_23 -= ((((min(var_4, (arr_1 [i_0])))) ? var_12 : var_17));
    }
    #pragma endscop
}
