/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 9416
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=9416 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/9416
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 += (((((4294967287 + 240) + 2656)) + var_12));
    var_14 = var_8;
    var_15 = (min(var_15, var_11));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [10] = (((arr_4 [i_0] [i_1] [i_0]) && (arr_3 [i_1] [i_0])));
                var_16 += ((arr_3 [i_0] [i_0]) && ((((arr_4 [i_1] [i_0] [i_0]) ^ (arr_4 [i_0] [i_0] [i_0])))));
                var_17 += ((var_10 && (arr_2 [i_1] [i_1])));
            }
        }
    }
    #pragma endscop
}
