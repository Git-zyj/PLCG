/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 147324
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=147324 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/147324
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 13;i_0 += 1) /* same iter space */
    {
        arr_3 [i_0] = 22383;

        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            var_12 = (min(var_12, (~var_6)));
            var_13 = ((~(arr_2 [i_1])));
            var_14 = (22383 ? (arr_6 [i_0 - 1] [i_0 - 1] [i_0]) : ((-(arr_1 [i_0]))));
        }
        var_15 = ((arr_5 [i_0 - 1]) ^ (43155 | var_9));
        arr_7 [i_0] = (arr_0 [i_0]);
        var_16 = (1609548212 | 1);
    }
    /* vectorizable */
    for (int i_2 = 1; i_2 < 13;i_2 += 1) /* same iter space */
    {
        arr_11 [i_2] = (((arr_5 [i_2 - 1]) & ((~(arr_6 [i_2] [i_2] [i_2])))));
        var_17 = -43153;
        var_18 |= var_7;
        var_19 = ((!(1 - 47340)));
        var_20 = (min(var_20, ((((arr_10 [i_2]) ? (arr_9 [i_2 + 1] [i_2 + 1]) : (1609548212 - 84))))));
    }
    /* vectorizable */
    for (int i_3 = 1; i_3 < 13;i_3 += 1) /* same iter space */
    {
        arr_15 [i_3] = (39 * 22383);
        var_21 = ((((var_3 >> (var_3 - 17202762853967658086))) << (((arr_1 [i_3 - 1]) + 60))));
    }
    var_22 = (max(var_22, 1));
    /* LoopNest 3 */
    for (int i_4 = 0; i_4 < 10;i_4 += 1)
    {
        for (int i_5 = 0; i_5 < 10;i_5 += 1)
        {
            for (int i_6 = 0; i_6 < 1;i_6 += 1)
            {
                {
                    var_23 = 15;
                    var_24 = 0;
                }
            }
        }
    }
    var_25 = (min(22383, 22383));
    #pragma endscop
}
