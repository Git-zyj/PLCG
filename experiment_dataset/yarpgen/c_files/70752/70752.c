/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70752
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70752 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70752
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (!253);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_11 = (((((var_3 / (arr_0 [i_0])))) ? var_0 : (((233 ? 65534 : 1359395002)))));
                var_12 *= (arr_4 [i_0] [i_1]);
                var_13 = var_1;
                arr_5 [i_0] = (((arr_4 [i_0] [i_1]) ? -1359395003 : (((arr_2 [i_0]) ? var_5 : ((var_2 ? -1359394988 : (arr_2 [i_1])))))));
            }
        }
    }
    var_14 = ((((max(42, (max(var_5, var_0))))) ? ((22 ? var_2 : -1359395011)) : (min(var_0, var_8))));
    #pragma endscop
}
