/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47897
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47897 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47897
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, var_8));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_20 = (max(var_20, ((((arr_5 [i_0] [i_0] [i_0]) || ((min((arr_6 [i_0] [i_1] [7]), (((var_13 - (arr_1 [i_0] [i_0]))))))))))));
                var_21 = (arr_5 [10] [i_0] [i_0]);
                arr_8 [i_0] [i_1] = (!((((arr_3 [i_1] [i_0]) | -9223372036854775793))));
                var_22 += ((~(arr_0 [6])));
                arr_9 [i_1] [i_1] [i_0] = (-9223372036854775807 - 1);
            }
        }
    }
    var_23 = (min(var_23, var_0));
    #pragma endscop
}
