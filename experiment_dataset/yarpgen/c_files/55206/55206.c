/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 55206
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=55206 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/55206
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 -= ((var_12 ? var_12 : ((((min(11019, var_0)))))));
    var_14 = var_9;

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_15 = 1;
        var_16 = (min(var_16, (((!(arr_1 [12]))))));
        arr_2 [i_0] = (min(var_12, ((1 ? (arr_0 [11] [i_0]) : 11019))));
        var_17 = ((97 ? 54529 : 11006));
        var_18 = ((47 != (arr_1 [i_0])));
    }
    for (int i_1 = 0; i_1 < 23;i_1 += 1)
    {
        var_19 = ((arr_4 [i_1] [i_1]) * (((((~var_10) < var_4)))));
        var_20 = (((((((((arr_3 [i_1]) ? (arr_4 [i_1] [i_1]) : 11014))) ? (arr_4 [i_1] [9]) : -171973311))) || (arr_3 [i_1])));
        arr_5 [i_1] = (((arr_4 [i_1] [i_1]) || (arr_4 [i_1] [i_1])));
    }
    #pragma endscop
}
