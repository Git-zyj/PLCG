/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224933
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224933 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224933
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (max(((-var_10 & ((var_6 ? var_0 : 7590946458750144831)))), var_1));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 18;i_1 += 1)
        {
            {
                var_12 = (max((arr_0 [i_1 + 3] [i_1]), (max(-4304722249589327929, (arr_4 [i_1 + 4] [i_1])))));
                var_13 -= ((+((65531 ? (arr_3 [i_0] [i_0] [i_1]) : (arr_4 [i_0] [10])))));
                arr_5 [i_0] [i_1] [i_1] = ((~(arr_1 [i_1])));
            }
        }
    }
    #pragma endscop
}
