/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 96881
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=96881 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/96881
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 16;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        var_12 = ((3 ? 4 : var_11));
                        arr_11 [i_0 - 1] [i_1 + 1] [i_0] [6] [9] [i_3] = (((((-(((arr_9 [i_0] [i_0] [i_2 - 1] [i_3]) * var_0))))) ? (var_6 < var_4) : ((min((max((arr_5 [i_0] [i_0] [i_0]), var_7)), (min(63488, -23)))))));
                    }
                    /* LoopNest 2 */
                    for (int i_4 = 3; i_4 < 14;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 17;i_5 += 1)
                        {
                            {
                                var_13 = (max(var_8, (max(221, 25120))));
                                var_14 = arr_7 [i_1 + 1] [i_2] [i_4] [6];
                            }
                        }
                    }
                }
            }
        }
    }
    var_15 -= 21889;
    #pragma endscop
}
