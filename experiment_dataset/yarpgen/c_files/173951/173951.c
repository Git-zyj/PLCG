/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 173951
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=173951 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/173951
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_7;
    var_21 += var_15;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                arr_4 [i_1] = min((((((var_3 ? (arr_2 [i_1]) : var_18)) <= ((var_7 ? var_14 : var_7))))), 107);
                arr_5 [i_0] [i_1 - 1] [i_0] &= (-((((var_1 ? 32 : var_16)))));
            }
        }
    }
    var_22 = (min(var_22, ((((((157 ? 42 : 1868064473))) ? ((1 >> ((var_2 ? var_14 : var_2)))) : var_11)))));
    #pragma endscop
}
