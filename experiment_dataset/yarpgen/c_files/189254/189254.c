/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 189254
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=189254 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/189254
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((var_7 << (var_10 + 95)));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (var_5 | -39);
        var_12 = ((!(arr_1 [i_0] [i_0])));
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        arr_7 [i_1] [i_1] = (((((((-(arr_6 [i_1]))) & -18))) ? (((arr_4 [i_1]) ? var_8 : (arr_4 [i_1]))) : -var_4));
        var_13 = ((-88 ? (((arr_4 [i_1]) ? (arr_6 [i_1]) : (arr_6 [i_1]))) : (((arr_6 [i_1]) ? (arr_6 [i_1]) : (arr_6 [i_1])))));
        var_14 = 17;
        arr_8 [i_1] [i_1] = ((-125 ? (!var_6) : ((-124 ? ((var_9 ? var_7 : (arr_4 [i_1]))) : ((31 ? var_3 : 120))))));
    }
    #pragma endscop
}
