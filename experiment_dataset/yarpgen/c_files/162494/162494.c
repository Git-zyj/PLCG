/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162494
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162494 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162494
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (-var_11 + var_3);
    var_17 = ((((((10282016188467879796 + var_1) <= (!-2071943418)))) + (max(var_4, var_12))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_0] [i_0] = (((!3241077463) ? 8 : ((2194 ? 10282016188467879787 : 1))));
                    var_18 = (min(var_18, (arr_6 [i_0])));
                    arr_9 [i_0] [i_0] [i_0] = 98;
                }
            }
        }
    }
    #pragma endscop
}
