/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136613
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136613 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136613
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 |= (var_1 ? var_7 : 1071780655);

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        var_20 = (var_6 << (((arr_1 [i_0] [i_0]) - 55)));
        var_21 = 8;
        var_22 = (((((1016 / ((((arr_1 [i_0] [i_0]) || (arr_1 [i_0] [i_0]))))))) ? (~var_7) : (((var_3 * var_11) ? var_0 : -8))));
        arr_2 [i_0] [i_0] = (var_7 >= var_18);
        arr_3 [i_0] = var_8;
    }
    var_23 = var_10;
    #pragma endscop
}
