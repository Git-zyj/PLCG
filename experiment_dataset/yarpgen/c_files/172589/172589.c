/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172589
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172589 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172589
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_13;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_19 = 18446744073709551615;
                var_20 = -var_6;
                var_21 = (5515715790091375484 % -81);
                var_22 = ((-(((((min(var_9, 2071551323))) <= 10262797780574147885)))));
                arr_5 [i_0] = ((!((!(!0)))));
            }
        }
    }
    var_23 = (min(var_23, (((min(var_13, ((839595122378580435 ? 16735 : var_0))))))));
    var_24 = (((((~((31587 ? var_13 : var_7))))) ? (((((((5154 ? 96 : 18446744073709551609))) && (((var_12 ? var_16 : var_14))))))) : var_12));
    #pragma endscop
}
