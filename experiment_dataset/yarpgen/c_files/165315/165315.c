/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165315
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165315 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165315
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 14;i_2 += 1)
            {
                {
                    var_12 = (max(var_12, (((((((((max(var_6, var_1))) != (arr_3 [i_1 - 3] [i_2 - 2]))))) < (min((((arr_6 [i_2] [i_2] [14] [i_0]) | -1217990894626449750)), (!var_10))))))));
                    arr_8 [3] [i_1] = -32760147;
                    arr_9 [i_0] [i_0] [i_1] [i_2] = 1217990894626449764;
                }
            }
        }
    }
    var_13 = (min(var_13, var_11));
    var_14 = (min(var_14, var_10));
    #pragma endscop
}
