/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24235
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24235 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24235
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 14;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                arr_12 [i_0] = -6333;
                                var_17 = (-289375501 != 1937810877360446429);
                                var_18 = (arr_5 [i_0]);
                                arr_13 [i_2] [i_1] [i_2] [i_1 + 1] [i_4] [i_1 - 1] [i_0] = (arr_6 [i_0] [i_0] [i_0]);
                            }
                        }
                    }
                    arr_14 [11] [11] [i_2] [i_2] = (((((((arr_7 [i_2] [i_2] [i_2 - 2] [i_2]) == ((-(arr_9 [i_2] [i_1] [i_0] [i_0]))))))) * 1937810877360446429));
                }
            }
        }
    }
    var_19 = (max(var_19, var_10));
    #pragma endscop
}
