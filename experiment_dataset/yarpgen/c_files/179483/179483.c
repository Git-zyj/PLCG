/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179483
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179483 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179483
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        arr_4 [i_0] = ((((((arr_0 [i_0 - 1]) ? -2147483640 : 0))) ? (((arr_0 [i_0 - 1]) ? (arr_0 [i_0 + 2]) : 4421270209310511032)) : 2147483640));
        arr_5 [i_0] = (((max(17324, (arr_0 [i_0 + 2])))) ? ((-2147483629 ? (-67 || 69) : -2147483640)) : ((((arr_1 [i_0]) == (((3 ? 1474863813 : 91)))))));
    }
    var_14 = 3630509357;
    var_15 = (min(var_15, var_9));
    #pragma endscop
}
