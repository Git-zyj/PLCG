/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85845
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85845 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85845
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */_Bool) var_0);
    var_14 = ((/* implicit */signed char) var_11);
    /* LoopNest 3 */
    for (unsigned short i_0 = 2; i_0 < 19; i_0 += 3) 
    {
        for (signed char i_1 = 0; i_1 < 22; i_1 += 2) 
        {
            for (long long int i_2 = 1; i_2 < 20; i_2 += 4) 
            {
                {
                    var_15 = ((/* implicit */signed char) ((((/* implicit */_Bool) ((((/* implicit */_Bool) 2270308181U)) ? (max((var_8), (((/* implicit */int) var_5)))) : (max((var_8), (((/* implicit */int) arr_4 [i_0] [i_0] [i_0] [i_0]))))))) ? ((+(((arr_3 [i_1]) | (((/* implicit */unsigned int) ((/* implicit */int) var_6))))))) : (((/* implicit */unsigned int) ((/* implicit */int) var_3)))));
                    var_16 += ((/* implicit */signed char) (-(((/* implicit */int) ((((/* implicit */_Bool) 2024659115U)) && (((/* implicit */_Bool) arr_6 [i_0] [i_0 + 2] [i_2 - 1] [i_2])))))));
                    var_17 |= ((/* implicit */signed char) arr_1 [i_0]);
                }
            } 
        } 
    } 
}
