/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156407
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156407 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156407
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (short i_0 = 1; i_0 < 20; i_0 += 4) 
    {
        for (unsigned int i_1 = 1; i_1 < 22; i_1 += 4) 
        {
            {
                var_14 = ((/* implicit */signed char) (~(arr_2 [i_0] [i_1] [i_0])));
                arr_4 [i_1] = var_9;
            }
        } 
    } 
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_2 = 0; i_2 < 25; i_2 += 2) 
    {
        arr_8 [(unsigned short)17] [i_2] = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) arr_7 [i_2])) ? (var_3) : (((/* implicit */unsigned int) ((/* implicit */int) arr_6 [i_2])))));
        var_15 = arr_7 [i_2];
    }
}
