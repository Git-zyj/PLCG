/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133724
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133724 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133724
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */int) var_10);
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_18 *= ((/* implicit */int) ((((/* implicit */_Bool) min((arr_1 [i_0]), (((/* implicit */unsigned short) (unsigned char)123))))) ? (((unsigned int) arr_3 [i_0] [i_0])) : (max((((/* implicit */unsigned int) var_0)), (arr_2 [i_1] [i_1] [i_0])))));
                var_19 = ((/* implicit */_Bool) ((int) 1089683488));
            }
        } 
    } 
    var_20 = ((/* implicit */unsigned int) var_10);
}
