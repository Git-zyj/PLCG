/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73220
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73220 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73220
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 17;i_2 += 1)
            {
                {
                    var_13 = ((-(max((max((arr_4 [i_0] [i_0] [i_0]), (arr_7 [i_2] [i_0] [i_0] [i_0]))), (var_5 && var_8)))));
                    var_14 = ((((((var_2 ? var_4 : (arr_6 [i_0] [i_1] [i_1] [i_2]))) - ((max((arr_7 [i_0] [i_0] [i_1] [i_2]), (arr_4 [i_0] [1] [i_2])))))) >= ((((min(18360, (arr_0 [i_1] [i_2]))))))));
                }
            }
        }
    }
    var_15 -= (max((!635800347), (min(-87, ((max(-1, -22688)))))));
    #pragma endscop
}
