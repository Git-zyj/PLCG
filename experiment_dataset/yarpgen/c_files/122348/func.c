/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122348
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122348 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122348
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */unsigned char) ((short) min((((/* implicit */short) ((unsigned char) var_9))), (max((((/* implicit */short) var_0)), (var_8))))));
    var_18 = ((/* implicit */short) var_10);
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 0; i_0 < 19; i_0 += 4) 
    {
        var_19 = ((/* implicit */unsigned long long int) var_12);
        var_20 = ((/* implicit */unsigned char) min((((/* implicit */int) ((unsigned short) var_11))), (((((/* implicit */int) (unsigned char)0)) - (((/* implicit */int) (short)1))))));
    }
}
