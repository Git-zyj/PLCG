/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 193052
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=193052 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/193052
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        var_12 = (arr_0 [9] [9]);
        var_13 &= -var_2;
        /* LoopNest 3 */
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 8;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 10;i_3 += 1)
                {
                    {
                        var_14 = (9 == -9076300866574854234);
                        arr_11 [i_0] = (((+((min((arr_1 [i_2]), var_8))))));
                    }
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < 18;i_4 += 1)
    {
        var_15 = (min(var_15, (((((var_6 ? ((3136191394 ? 1 : var_10)) : -19115)) > -46)))));
        arr_15 [i_4] [i_4] = (max(65505, (((0 != 1) + ((65505 ? (arr_13 [i_4]) : var_10))))));
    }
    var_16 = (min(var_9, (((var_11 ? ((max(0, var_7))) : var_2)))));
    #pragma endscop
}
