/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83329
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83329 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83329
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */long long int) (+(((/* implicit */int) var_6))));
    var_12 = ((/* implicit */int) var_10);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 12; i_1 += 4) 
        {
            {
                var_13 ^= ((/* implicit */unsigned int) min((((unsigned long long int) ((unsigned int) arr_3 [i_1]))), ((+((+(2835600766369854485ULL)))))));
                var_14 = ((/* implicit */unsigned short) (-(1598103305U)));
            }
        } 
    } 
    var_15 |= ((/* implicit */unsigned long long int) (~(((/* implicit */int) (short)-2133))));
}
