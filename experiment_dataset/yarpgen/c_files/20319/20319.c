/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20319
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20319 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20319
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_2;
    var_16 = (max((max(var_3, var_8)), var_11));

    for (int i_0 = 1; i_0 < 7;i_0 += 1)
    {

        for (int i_1 = 4; i_1 < 6;i_1 += 1)
        {
            arr_7 [i_0] [i_0] = ((!((!(((7343070713430286737 ? var_9 : -64)))))));
            var_17 = (min(var_17, -1913490623));
        }
        arr_8 [i_0] = ((((((min(var_10, 56))) ? 1716109581106960432 : -var_13)) < (((+(max((arr_1 [i_0 + 3]), (arr_2 [i_0]))))))));
    }
    #pragma endscop
}
