/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133241
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133241 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133241
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (long long int i_0 = 2; i_0 < 22; i_0 += 2) 
    {
        var_18 = ((/* implicit */unsigned char) min((var_18), (((/* implicit */unsigned char) ((unsigned int) (+(((/* implicit */int) arr_1 [i_0] [i_0]))))))));
        var_19 = ((/* implicit */_Bool) ((short) (!(((/* implicit */_Bool) 0ULL)))));
        arr_2 [i_0] |= ((/* implicit */short) (+(var_3)));
    }
    var_20 = ((/* implicit */signed char) ((unsigned long long int) ((long long int) var_14)));
    var_21 *= (-(((unsigned long long int) var_12)));
    var_22 = ((/* implicit */unsigned int) var_1);
    var_23 = ((/* implicit */_Bool) var_10);
}
