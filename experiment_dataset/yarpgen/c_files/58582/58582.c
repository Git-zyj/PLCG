/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 58582
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=58582 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/58582
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(var_13, var_0));
    var_17 = var_15;

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_18 = (((((((arr_1 [i_0]) && var_1)) && (arr_0 [14]))) ? 125 : -4070552811171209153));
        var_19 ^= (((1 | -1) * (arr_1 [i_0])));
        arr_2 [i_0] [i_0] = ((((var_5 ? var_9 : var_7))));
        var_20 = (20278 && 1);
    }
    var_21 = (min(1, 27));
    var_22 = (min(var_11, var_6));
    #pragma endscop
}
