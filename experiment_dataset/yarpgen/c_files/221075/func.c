/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221075
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221075 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221075
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */long long int) var_8);
    /* LoopSeq 2 */
    /* vectorizable */
    for (unsigned long long int i_0 = 1; i_0 < 21; i_0 += 3) /* same iter space */
    {
        var_12 *= ((/* implicit */long long int) ((((((/* implicit */int) var_1)) | (((/* implicit */int) var_1)))) != (((((/* implicit */int) (signed char)-63)) | (((/* implicit */int) var_5))))));
        var_13 = ((/* implicit */int) min((var_13), (((/* implicit */int) var_7))));
        arr_3 [i_0 + 1] [i_0 + 1] = ((/* implicit */unsigned char) (((-(((/* implicit */int) (signed char)90)))) & (arr_2 [i_0 - 1])));
    }
    /* vectorizable */
    for (unsigned long long int i_1 = 1; i_1 < 21; i_1 += 3) /* same iter space */
    {
        var_14 = ((/* implicit */short) min((var_14), (((/* implicit */short) var_1))));
        var_15 ^= ((/* implicit */long long int) (!(((/* implicit */_Bool) (signed char)-63))));
    }
    var_16 ^= ((/* implicit */unsigned int) ((unsigned long long int) (~((~(((/* implicit */int) (signed char)-63)))))));
}
