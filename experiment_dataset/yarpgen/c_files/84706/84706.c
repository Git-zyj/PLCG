/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84706
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84706 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84706
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 += ((!((min((min(21036, 12625747964069716638)), ((min(var_0, 1548148902))))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 22;i_2 += 1)
            {
                {
                    arr_7 [i_2 - 3] = ((~(arr_5 [i_0] [i_1] [i_2 - 2] [10])));
                    var_13 = (3883143143 ? 2746818403 : (((arr_5 [i_2 + 1] [i_1] [i_2] [i_2]) ? var_9 : 0)));
                }
            }
        }
    }
    #pragma endscop
}
