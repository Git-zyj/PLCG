/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127180
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127180 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127180
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min((((36028797018963967 ? 6902 : var_11))), (max(1761658581, 6902))));
    var_14 = 17592185520128;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_15 = 2184440619808271596;
                var_16 = ((~((((~(arr_2 [i_1]))) ^ (arr_5 [i_1] [6] [6])))));
                var_17 += ((!((((arr_4 [i_1]) * (max((arr_0 [11] [11]), (arr_2 [i_0]))))))));
            }
        }
    }
    var_18 = (0 == 16);
    #pragma endscop
}
