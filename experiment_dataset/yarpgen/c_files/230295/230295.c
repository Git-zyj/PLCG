/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230295
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230295 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230295
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_10 = (max(var_10, (arr_4 [i_0] [i_1])));
                var_11 = 14376553285099575819;
                arr_7 [i_0] [i_1] [i_1] = (min(14376553285099575824, (max((min(var_7, (arr_1 [6] [i_0]))), ((min((arr_5 [i_0] [i_1] [i_1]), (arr_2 [i_0]))))))));
            }
        }
    }
    var_12 = (min(var_12, var_8));
    #pragma endscop
}
