/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84897
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84897 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84897
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_19 = (((var_16 ? ((max(1, (arr_7 [i_0] [i_0] [i_0] [i_0])))) : (arr_0 [i_0 + 1] [i_0 - 1]))));

                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        var_20 = (((((arr_3 [i_0 - 1] [i_0 - 1]) ? (arr_3 [i_0 - 1] [i_0 - 1]) : var_17))) ? ((((arr_3 [i_0 - 1] [i_0 - 1]) >= var_13))) : (~1));
                        arr_10 [i_2] = (max(((((arr_0 [i_1] [i_1]) < (arr_9 [i_0] [10] [i_2] [i_3])))), -2014441175));
                    }
                }
            }
        }
    }
    #pragma endscop
}
