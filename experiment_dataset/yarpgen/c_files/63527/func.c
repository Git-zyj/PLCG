/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63527
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63527 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63527
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned char i_0 = 3; i_0 < 17; i_0 += 1) 
    {
        var_10 &= ((/* implicit */long long int) min(((~(((/* implicit */int) (signed char)-50)))), (((/* implicit */int) (!(((/* implicit */_Bool) arr_0 [10])))))));
        var_11 = ((/* implicit */unsigned int) ((short) min((2076106590U), (2218860679U))));
    }
    var_12 = ((/* implicit */signed char) ((unsigned int) var_0));
    var_13 = ((/* implicit */_Bool) var_7);
    var_14 = var_1;
}
