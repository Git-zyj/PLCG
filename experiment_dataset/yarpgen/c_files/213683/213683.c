/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213683
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213683 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213683
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (1855143619 >= -30798);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                arr_6 [i_1] = ((+-2147483647) ? ((min(30799, (arr_5 [i_1])))) : ((max((arr_5 [i_0]), var_9))));
                arr_7 [11] [i_1] [i_1] = ((((max((13 < 20), ((var_3 ? (arr_1 [i_1]) : 0))))) ? ((min(235, (!var_5)))) : (((arr_3 [i_0]) << (var_4 - 35553)))));
            }
        }
    }
    #pragma endscop
}
