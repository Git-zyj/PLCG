/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93872
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93872 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93872
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 *= ((/* implicit */unsigned short) var_4);
    /* LoopSeq 1 */
    for (long long int i_0 = 0; i_0 < 13; i_0 += 4) 
    {
        arr_2 [i_0] = ((/* implicit */_Bool) ((unsigned short) ((((/* implicit */_Bool) max((((/* implicit */unsigned int) (unsigned short)5815)), (var_18)))) ? (((/* implicit */int) arr_0 [i_0])) : (((/* implicit */int) max((var_7), (((/* implicit */unsigned short) var_6))))))));
        var_20 = var_0;
    }
}
