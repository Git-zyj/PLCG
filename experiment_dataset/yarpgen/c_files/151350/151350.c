/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151350
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151350 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151350
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(var_18, (((var_7 ? 1975855826 : var_9)))));
    var_19 = (min(((!(((var_7 ? 0 : 1))))), (!-1538303062)));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            {
                arr_6 [i_0] [19] [i_0] = ((((7 ? (-32767 - 1) : (arr_3 [i_0] [i_0])))) >= var_17);
                arr_7 [15] [i_1] [i_1 - 1] = ((~(min((arr_0 [i_1 + 3] [i_1 + 2]), -6599624499551409840))));
            }
        }
    }
    #pragma endscop
}
