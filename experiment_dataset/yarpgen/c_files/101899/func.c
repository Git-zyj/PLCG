/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 101899
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=101899 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/101899
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */short) ((signed char) var_18));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned char i_0 = 0; i_0 < 13; i_0 += 2) 
    {
        arr_4 [i_0] = ((/* implicit */unsigned long long int) (-(arr_2 [i_0])));
        var_20 |= ((/* implicit */unsigned short) arr_0 [i_0]);
        var_21 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) ((-438480287) + (-438480287)))) ? (var_16) : (((/* implicit */unsigned long long int) arr_2 [i_0]))));
    }
}
