/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224582
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224582 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224582
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 2; i_2 < 23;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 22;i_4 += 1)
                        {
                            {
                                var_13 = (arr_7 [i_0] [i_0] [i_0] [13] [i_0] [i_0]);
                                var_14 = var_5;
                            }
                        }
                    }
                }
                var_15 ^= (!var_12);
                var_16 += (max(var_2, (max((arr_6 [i_0] [i_0] [i_1]), ((((arr_11 [i_0] [i_1]) || var_11)))))));
            }
        }
    }
    var_17 = (max(var_17, ((((((((max(var_9, 14))) ? var_12 : (max(6026, var_11))))) ? var_2 : var_0)))));
    #pragma endscop
}
