/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 13631
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=13631 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/13631
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_18 ^= (((arr_1 [i_0] [i_0]) != (arr_1 [i_0] [i_0])));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_19 = (((arr_3 [i_1] [i_2]) > (arr_5 [i_2] [i_1] [i_2])));
                    arr_7 [i_0] [i_1] [i_1] [i_2] = ((arr_0 [i_0] [i_0]) >> (((arr_1 [i_0] [i_1]) + 4101438775098030932)));
                    arr_8 [i_2] = (arr_0 [i_2] [i_1]);
                }
            }
        }
        var_20 = (((arr_3 [i_0] [i_0]) | (arr_5 [i_0] [i_0] [i_0])));
    }
    /* vectorizable */
    for (int i_3 = 1; i_3 < 23;i_3 += 1)
    {
        arr_13 [i_3] = (arr_11 [i_3]);
        var_21 = arr_12 [i_3 + 1];
    }
    var_22 = (((var_3 && var_10) || var_3));
    #pragma endscop
}
