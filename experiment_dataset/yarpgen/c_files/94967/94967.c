/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94967
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94967 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94967
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        var_20 = min((((arr_1 [i_0 + 1]) << (var_13 - 2032980764))), ((~(arr_1 [i_0 + 1]))));
        arr_2 [i_0] = (min(var_16, (((arr_0 [i_0]) * (arr_0 [i_0 + 1])))));
    }
    var_21 = (min(((var_3 << (var_14 - 1413328388))), (var_17 - var_14)));
    /* LoopNest 2 */
    for (int i_1 = 1; i_1 < 15;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 16;i_2 += 1)
        {
            {
                arr_7 [7] [i_1] = (arr_4 [i_1] [i_2]);
                var_22 ^= (max(18342, (max(0, 16368))));
            }
        }
    }
    #pragma endscop
}
