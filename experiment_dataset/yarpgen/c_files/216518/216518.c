/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 216518
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=216518 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/216518
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 1;i_3 += 1)
                {
                    {
                        var_20 ^= ((-30466 && (((var_1 ? (arr_7 [i_0] [i_1] [i_2] [8]) : (((arr_4 [i_0] [0] [i_2]) / 1996685328)))))));
                        var_21 = (min(var_21, ((max(((((arr_4 [i_2] [4] [i_2]) >= (max((arr_0 [0]), var_2))))), var_0)))));
                        var_22 -= var_14;
                    }
                }
            }
        }
        var_23 -= (arr_3 [i_0] [i_0]);
        arr_11 [i_0] = ((((((arr_0 [i_0]) ? (arr_0 [i_0]) : var_6))) || (arr_0 [i_0])));
    }
    var_24 = var_16;
    var_25 = (var_17 <= var_4);
    #pragma endscop
}
