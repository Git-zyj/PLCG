/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159748
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159748 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159748
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_0] [i_2] [i_2] = max(((max((((arr_5 [7]) && 0)), 1))), ((-(arr_4 [i_1 + 1] [i_1 + 2]))));
                    var_18 = (max(var_18, ((max(((max((-19 >= 1), (arr_1 [i_1 + 2] [i_1 - 1])))), (max(1, (((arr_4 [i_0] [i_1 + 2]) ? (arr_0 [i_0] [i_0]) : (arr_6 [i_0] [i_1] [i_2]))))))))));
                }
            }
        }
    }
    var_19 -= var_6;
    #pragma endscop
}
