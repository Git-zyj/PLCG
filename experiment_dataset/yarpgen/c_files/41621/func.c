/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41621
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41621 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41621
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned char) min((((((/* implicit */_Bool) min((((/* implicit */long long int) var_2)), (-5528900206090025652LL)))) ? (((/* implicit */int) ((unsigned char) 13955061085636917723ULL))) : (((/* implicit */int) var_0)))), (((/* implicit */int) (unsigned char)48))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 3; i_0 < 10; i_0 += 3) 
    {
        var_17 = ((/* implicit */_Bool) arr_0 [i_0 - 2]);
        var_18 = ((/* implicit */unsigned char) ((((/* implicit */unsigned int) ((var_4) ? (((/* implicit */int) (signed char)-1)) : (((/* implicit */int) var_8))))) | (4159250972U)));
        var_19 = ((/* implicit */_Bool) arr_1 [i_0]);
    }
    var_20 = ((/* implicit */long long int) var_0);
}
