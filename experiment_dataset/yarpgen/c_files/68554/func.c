/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68554
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68554 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68554
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 |= ((/* implicit */unsigned int) max((2ULL), (((/* implicit */unsigned long long int) (_Bool)1))));
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 20; i_0 += 1) 
    {
        for (unsigned int i_1 = 0; i_1 < 20; i_1 += 1) 
        {
            {
                var_12 = ((/* implicit */short) (-(((/* implicit */int) (unsigned char)102))));
                var_13 *= ((/* implicit */short) 17887652479976490477ULL);
            }
        } 
    } 
}
