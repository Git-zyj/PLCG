/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 134926
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=134926 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/134926
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [i_1] = var_4;
                var_15 = (min((((arr_0 [i_1 + 1] [i_0 + 2]) ? 1 : (arr_0 [i_0 + 1] [i_1]))), ((((min(11181887173826855206, (arr_3 [i_0] [19] [i_1])))) ? var_13 : ((536870911 ? 12516302432517664336 : 2047))))));
                var_16 = (min(var_16, (((!(255 | 1))))));
            }
        }
    }
    var_17 = (min(2147483635, 12516302432517664343));
    #pragma endscop
}
