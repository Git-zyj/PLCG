/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74719
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74719 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74719
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max((((~(min(127, var_8))))), var_19));
    var_21 ^= var_17;
    var_22 &= var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = (~var_14);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        {
                            var_23 = var_12;
                            var_24 = (((!1) ? var_12 : (min((arr_10 [i_0] [i_0]), 2147483620))));
                        }
                    }
                }
                arr_13 [i_0] [12] [i_0] = 0;
                var_25 &= (min(-3008485414440822175, ((min(var_14, (arr_9 [i_0] [i_1] [6] [8]))))));
            }
        }
    }
    var_26 = ((min(var_6, -var_19)));
    #pragma endscop
}
