/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62644
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62644 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62644
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 0; i_0 < 18; i_0 += 4) 
    {
        var_11 |= ((/* implicit */_Bool) (+(var_6)));
        arr_2 [i_0] = ((/* implicit */unsigned long long int) ((((/* implicit */unsigned int) (+(var_4)))) & (min((arr_1 [i_0]), (arr_1 [i_0])))));
    }
    var_12 -= ((/* implicit */int) var_6);
    var_13 -= ((/* implicit */unsigned int) (!(((/* implicit */_Bool) min((((unsigned long long int) var_8)), (((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) var_2))))))))));
}
