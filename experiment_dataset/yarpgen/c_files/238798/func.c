/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 238798
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=238798 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/238798
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_0 = 0; i_0 < 23; i_0 += 2) 
    {
        arr_3 [(_Bool)1] [(_Bool)1] = ((/* implicit */long long int) ((arr_2 [i_0]) ? (20) : (((/* implicit */int) arr_2 [16LL]))));
        arr_4 [i_0] = ((/* implicit */unsigned int) var_17);
        var_19 = ((/* implicit */unsigned int) ((arr_1 [i_0]) / (arr_1 [i_0])));
    }
    var_20 = ((/* implicit */unsigned long long int) (short)-11285);
    var_21 = var_8;
    var_22 = ((/* implicit */int) var_12);
}
