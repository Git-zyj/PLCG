/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224911
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224911 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224911
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((var_4 - (var_13 && -var_7)));
    var_15 = (max(var_15, ((min(var_6, var_8)))));

    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = (arr_0 [3]);
        var_16 = (min(var_16, ((min((arr_1 [i_0 + 1]), (arr_1 [i_0 + 1]))))));
        var_17 |= (((arr_2 [i_0 + 1]) + ((~(((arr_2 [i_0]) / (arr_2 [i_0])))))));
    }
    var_18 = (min((var_13 - var_5), (min(var_2, var_4))));
    var_19 = var_0;
    #pragma endscop
}
