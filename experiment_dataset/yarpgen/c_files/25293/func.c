/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 25293
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=25293 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/25293
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */signed char) min((((/* implicit */long long int) (short)-21353)), (5654323386844335765LL)));
    var_15 = var_3;
    /* LoopNest 3 */
    for (unsigned short i_0 = 2; i_0 < 18; i_0 += 4) 
    {
        for (unsigned short i_1 = 0; i_1 < 22; i_1 += 4) 
        {
            for (unsigned long long int i_2 = 0; i_2 < 22; i_2 += 1) 
            {
                {
                    var_16 = ((/* implicit */long long int) max((var_16), (((/* implicit */long long int) max((((/* implicit */int) var_9)), (((((/* implicit */_Bool) (unsigned short)65513)) ? (((((/* implicit */int) (_Bool)0)) & (-478297221))) : (-1620336011))))))));
                    var_17 = ((/* implicit */short) var_5);
                }
            } 
        } 
    } 
    /* LoopNest 3 */
    for (long long int i_3 = 1; i_3 < 22; i_3 += 2) 
    {
        for (unsigned short i_4 = 0; i_4 < 25; i_4 += 3) 
        {
            for (long long int i_5 = 1; i_5 < 24; i_5 += 2) 
            {
                {
                    var_18 = ((/* implicit */short) 1620336023);
                    var_19 = ((/* implicit */unsigned char) max((var_19), (var_9)));
                }
            } 
        } 
    } 
}
