/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203385
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203385 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203385
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(((((var_3 / var_0) < 111))), -var_14));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_16 += (((((var_6 >> (((((arr_1 [i_0]) ^ var_5)) - 2743257479))))) ? (min((min((arr_3 [i_0] [i_0] [i_1]), var_6)), (arr_1 [i_1]))) : var_3));
                var_17 = (max(-7714771472208920970, -69));
                var_18 = var_14;
                var_19 = ((((((arr_3 [13] [i_0] [i_0]) < (arr_2 [i_0 + 1] [i_0] [i_1]))) && ((min(-18033, 15793))))));
            }
        }
    }
    #pragma endscop
}
