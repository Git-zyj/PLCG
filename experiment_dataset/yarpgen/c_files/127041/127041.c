/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127041
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127041 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127041
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        var_19 += (arr_1 [i_0]);
        arr_2 [i_0] = (((arr_0 [i_0] [i_0]) << (((max((((arr_1 [i_0 + 2]) ? (arr_0 [i_0] [i_0 + 3]) : (arr_0 [i_0] [i_0]))), var_4)) - 49))));
        arr_3 [i_0] = ((((min(((3 ? 84 : 172)), (!var_15)))) ? (((!(((arr_1 [i_0]) || (arr_0 [i_0] [i_0 + 3])))))) : (var_17 && var_0)));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 0;i_1 += 1)
    {
        arr_6 [i_1] = arr_4 [i_1 + 1];
        arr_7 [i_1] [i_1] = (arr_5 [i_1] [i_1]);
        var_20 ^= var_2;
        var_21 = (arr_4 [i_1]);
    }
    var_22 = ((((((var_10 ? 138 : var_14)) / ((max(var_0, var_10))))) - 1));
    #pragma endscop
}
