/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209488
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209488 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209488
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((((((max(var_11, var_11))) ? ((-4228657675866541413 ? var_7 : var_19)) : var_10)) * var_17));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_21 = (arr_1 [i_0] [i_0]);
        var_22 = (arr_0 [i_0]);
        arr_2 [i_0] = 20;
    }
    for (int i_1 = 1; i_1 < 12;i_1 += 1)
    {
        var_23 = arr_0 [i_1];
        var_24 = (min(var_24, (((((max((arr_4 [i_1 + 1] [i_1 + 1]), (arr_0 [i_1 - 1])))) ? ((~(arr_0 [i_1 + 1]))) : (((min(1, (arr_4 [i_1 - 1] [i_1 - 1]))))))))));
        var_25 = (min(var_25, ((((arr_3 [i_1]) - 1)))));
        arr_5 [i_1] = (arr_0 [i_1]);
    }
    var_26 = (var_15 && var_12);
    #pragma endscop
}
