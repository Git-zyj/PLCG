/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41488
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41488 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41488
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                for (int i_3 = 4; i_3 < 15;i_3 += 1)
                {
                    {
                        var_17 = ((!(((arr_9 [i_3] [i_3 - 3] [i_3] [i_3 - 2]) < (arr_9 [i_3] [i_3 - 3] [i_1] [i_1])))));
                        var_18 = ((max(0, (var_10 ^ 1))));
                    }
                }
            }
        }
        var_19 = ((((max((arr_9 [i_0] [i_0] [i_0] [i_0]), var_9))) ? (arr_5 [i_0] [i_0] [i_0] [i_0]) : (((arr_9 [i_0] [i_0] [i_0] [i_0]) / (arr_5 [i_0] [i_0] [i_0] [i_0])))));
        var_20 = (~1);
    }
    var_21 = min((59663 <= var_2), (!var_5));
    var_22 = 59663;
    #pragma endscop
}
