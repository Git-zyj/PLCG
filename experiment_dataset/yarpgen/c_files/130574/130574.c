/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130574
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130574 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130574
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = 0;
    var_11 = ((var_9 ? (1 * var_3) : var_8));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] [i_0] = (((arr_3 [i_0] [i_0] [i_0 + 1]) && (((var_8 ? var_1 : (-9223372036854775807 - 1))))));
                var_12 -= ((((((arr_0 [i_0]) + var_9))) < 1));
                var_13 = (min(((((max(var_4, var_2))) ? ((min((arr_1 [i_0]), var_3))) : (arr_3 [i_0] [i_0 + 1] [i_0 + 2]))), 0));
            }
        }
    }
    #pragma endscop
}
