/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165187
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165187 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165187
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (long long int i_1 = 0; i_1 < 18; i_1 += 1) 
        {
            {
                /* LoopNest 2 */
                for (long long int i_2 = 3; i_2 < 15; i_2 += 2) 
                {
                    for (long long int i_3 = 2; i_3 < 15; i_3 += 1) 
                    {
                        {
                            arr_8 [i_0] [i_1] [i_1] [i_3] [i_0] [(short)16] = (_Bool)1;
                            var_19 = ((/* implicit */unsigned int) (-(((/* implicit */int) (unsigned char)1))));
                            var_20 = ((/* implicit */long long int) 1757808536U);
                        }
                    } 
                } 
                arr_9 [(short)16] [i_1] &= ((/* implicit */short) (-(1757808541U)));
            }
        } 
    } 
    var_21 ^= ((/* implicit */_Bool) (-(((((/* implicit */int) max(((short)-19962), (((/* implicit */short) (_Bool)1))))) - (((/* implicit */int) ((unsigned char) var_17)))))));
    var_22 = ((/* implicit */_Bool) max((var_22), (((/* implicit */_Bool) (-((+(((/* implicit */int) (short)19936)))))))));
}
