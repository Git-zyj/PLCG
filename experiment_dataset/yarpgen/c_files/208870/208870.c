/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208870
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208870 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208870
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (max(var_11, var_6));
    var_12 = (((~var_8) ? var_3 : (((!(var_8 && -1))))));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 24;i_4 += 1)
                        {
                            {
                                var_13 = (max(var_13, (((-(max((max(var_1, -1)), 1)))))));
                                arr_14 [i_0] [i_1] [i_1] [i_0] [i_0] = 58;
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 25;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 25;i_6 += 1)
                        {
                            {
                                arr_20 [i_0] [i_0] [i_0] [i_5] [i_6] = var_2;
                                arr_21 [9] [i_5] [i_2] [i_5] [i_0] = 141;
                            }
                        }
                    }
                    var_14 -= 198;
                }
            }
        }
    }
    #pragma endscop
}
