/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247460
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247460 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247460
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */long long int) (~((-(((/* implicit */int) (short)-29874))))));
    var_13 = ((/* implicit */_Bool) min((var_13), (((/* implicit */_Bool) var_0))));
    /* LoopNest 3 */
    for (long long int i_0 = 4; i_0 < 19; i_0 += 2) 
    {
        for (int i_1 = 0; i_1 < 21; i_1 += 4) 
        {
            for (unsigned int i_2 = 0; i_2 < 21; i_2 += 4) 
            {
                {
                    var_14 = ((/* implicit */unsigned int) min((var_14), (((/* implicit */unsigned int) arr_1 [i_1]))));
                    var_15 = ((/* implicit */long long int) ((unsigned char) max((max((6755399441055744LL), (((/* implicit */long long int) (_Bool)1)))), (((/* implicit */long long int) ((unsigned int) var_3))))));
                }
            } 
        } 
    } 
}
