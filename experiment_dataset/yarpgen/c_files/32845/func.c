/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32845
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32845 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32845
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */short) ((((/* implicit */_Bool) ((signed char) (+(var_7))))) || (((/* implicit */_Bool) min((529666314U), (((/* implicit */unsigned int) (short)-21467)))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (long long int i_0 = 0; i_0 < 23; i_0 += 3) 
    {
        arr_3 [i_0] = ((/* implicit */unsigned long long int) arr_0 [i_0]);
        /* LoopNest 2 */
        for (long long int i_1 = 3; i_1 < 21; i_1 += 1) 
        {
            for (int i_2 = 0; i_2 < 23; i_2 += 2) 
            {
                {
                    var_15 = var_0;
                    var_16 = ((/* implicit */int) ((unsigned int) var_7));
                }
            } 
        } 
    }
    var_17 = ((/* implicit */unsigned int) var_10);
}
