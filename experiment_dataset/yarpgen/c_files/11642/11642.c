/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11642
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11642 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11642
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            {
                arr_5 [i_0] [13] = ((max((arr_3 [i_0]), 27)));
                /* LoopNest 3 */
                for (int i_2 = 1; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                arr_13 [i_0] = (max((((arr_2 [i_1 + 2] [i_1]) % (arr_1 [i_1 - 1] [i_0]))), (arr_2 [i_1 + 2] [i_1])));
                                arr_14 [i_2] [i_0] [i_4] = 27;
                            }
                        }
                    }
                }
                var_10 = ((-18446744073709551588 ? (arr_8 [i_0] [5] [1] [i_0]) : (((!(!22))))));
            }
        }
    }
    var_11 = var_0;
    #pragma endscop
}
