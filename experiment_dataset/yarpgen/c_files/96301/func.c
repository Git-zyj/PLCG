/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 96301
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=96301 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/96301
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (long long int i_0 = 0; i_0 < 20; i_0 += 4) 
    {
        var_10 |= arr_0 [i_0];
        var_11 = ((/* implicit */unsigned short) (((~(min((11030490629588922166ULL), (11030490629588922182ULL))))) * (((/* implicit */unsigned long long int) var_2))));
    }
    var_12 = ((/* implicit */unsigned char) ((((/* implicit */long long int) ((/* implicit */int) (_Bool)1))) ^ (-4943926811669655338LL)));
}
