/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 207235
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=207235 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/207235
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (~var_3);
    var_21 |= ((var_9 / (max(-var_7, ((var_15 ? var_19 : var_7))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] [14] = (min((((~var_7) ^ (arr_3 [i_1] [i_0] [i_0]))), ((~(((arr_3 [8] [i_0] [1]) ? (arr_0 [i_0] [i_1]) : var_6))))));
                arr_5 [i_0] = (((((arr_0 [i_0] [i_1]) && ((max((arr_0 [i_0] [i_1]), var_6))))) && ((max(var_0, (max(var_18, (arr_1 [i_0]))))))));
            }
        }
    }
    #pragma endscop
}
