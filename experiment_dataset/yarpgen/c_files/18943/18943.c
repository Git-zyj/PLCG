/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18943
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18943 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18943
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 14;i_2 += 1)
            {
                {
                    var_14 = (max((max(var_6, (max(var_8, var_0)))), (min((arr_8 [i_0] [i_0] [i_0]), (max((arr_0 [i_2]), (arr_8 [i_0] [10] [i_0])))))));
                    var_15 ^= (((((((((arr_1 [i_0]) || (arr_2 [i_1])))) / (arr_3 [i_1])))) ? (~65524) : (!-var_12)));
                    var_16 = (arr_9 [12] [i_1] [2]);
                }
            }
        }
    }
    var_17 = (~var_3);
    var_18 &= var_10;
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 16;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 16;i_4 += 1)
        {
            {
                arr_15 [i_3] = ((-((106 >> (-2108072350 + 2108072367)))));
                var_19 = (arr_12 [i_3] [i_3] [1]);
            }
        }
    }
    #pragma endscop
}
