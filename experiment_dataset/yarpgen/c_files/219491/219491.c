/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219491
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219491 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219491
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 15;i_2 += 1)
            {
                {
                    arr_9 [1] [i_2] [i_1] [6] = ((((((((967 ? -22 : 1))) ? 2088960 : 18446744073709551615))) ? (-32767 - 1) : (((((var_1 ? 0 : 9223372036854775807))) ? 38461 : (!18446744073709551615)))));
                    var_15 = var_11;
                    arr_10 [i_0] [i_1 + 1] [i_1] [i_2] = (max((arr_3 [i_2]), ((((var_2 < var_6) && -967)))));
                }
            }
        }
    }
    var_16 += ((var_4 ? (min((var_5 + -912300145), 0)) : 1));
    var_17 = (!28597);
    #pragma endscop
}
