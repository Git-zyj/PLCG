/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 115164
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=115164 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/115164
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 ^= (!(((/* implicit */_Bool) var_1)));
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 0; i_0 < 19; i_0 += 3) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            for (long long int i_2 = 3; i_2 < 17; i_2 += 1) 
            {
                {
                    var_19 = ((/* implicit */unsigned long long int) max((var_19), (((((/* implicit */unsigned long long int) min((((/* implicit */int) var_17)), (min((-459782403), (((/* implicit */int) (short)0))))))) - (var_5)))));
                    var_20 ^= ((/* implicit */long long int) ((int) (!(((/* implicit */_Bool) var_9)))));
                }
            } 
        } 
    } 
}
