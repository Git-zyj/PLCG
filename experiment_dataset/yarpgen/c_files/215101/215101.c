/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215101
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215101 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215101
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 *= (var_16 > var_18);
    var_20 = (var_15 < 1);

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 16;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 1;i_3 += 1)
                {
                    {
                        arr_11 [i_0] = (min((arr_2 [i_2] [i_2 - 3] [i_0]), (var_4 ^ 6143)));
                        arr_12 [i_0] [i_0] [i_0] = ((!((!((max(63470, 10355917806438772990)))))));
                        var_21 = (min(var_21, ((((!var_7) ^ var_5)))));
                    }
                }
            }
        }
        var_22 -= (((arr_9 [i_0] [i_0] [14] [i_0]) < (((((var_18 ? (arr_0 [i_0]) : -4724508298078120316))) % (~10355917806438772987)))));
        arr_13 [i_0] = (!((!(arr_6 [i_0] [i_0]))));
    }
    #pragma endscop
}
