/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152939
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152939 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152939
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            {
                arr_5 [i_1] = ((!((!((((arr_3 [i_1]) ? 18446744073709551615 : var_1)))))));
                arr_6 [i_1] = (-(~18446744073709551605));
                arr_7 [i_1] = var_3;
            }
        }
    }

    /* vectorizable */
    for (int i_2 = 0; i_2 < 19;i_2 += 1)
    {
        arr_11 [i_2] = (arr_2 [i_2] [i_2]);
        arr_12 [i_2] = ((var_8 >= var_9) * (arr_8 [i_2]));
        var_14 = (max(var_14, (((23881 ? var_11 : var_12)))));
    }
    #pragma endscop
}
