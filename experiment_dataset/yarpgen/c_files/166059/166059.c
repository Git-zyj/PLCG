/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166059
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166059 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166059
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_13;
    var_15 |= (min(var_11, (!var_10)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 22;i_2 += 1)
            {
                {
                    var_16 = (-(min((arr_1 [i_2 + 1]), (arr_1 [i_2 - 2]))));
                    var_17 ^= ((var_2 ? ((((0 - var_3) ? (((arr_4 [i_2 + 1] [i_1] [i_0]) & (arr_5 [i_0] [i_0] [i_0]))) : ((max((arr_1 [i_1]), (arr_0 [i_1] [i_1]))))))) : ((((!(arr_1 [16]))) ? (arr_0 [i_0] [i_0]) : (min(var_10, var_12))))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 25;i_4 += 1)
                        {
                            {
                                arr_12 [i_0] [i_0] = (arr_1 [i_3]);
                                arr_13 [i_4] [i_4] [i_3] [i_0] [i_2] [i_1] [i_0] = (arr_9 [i_1] [i_3] [i_0] [i_1] [i_1] [i_0]);
                            }
                        }
                    }
                }
            }
        }
    }
    var_18 = (min((((-var_0 && (~var_2)))), var_0));
    #pragma endscop
}
