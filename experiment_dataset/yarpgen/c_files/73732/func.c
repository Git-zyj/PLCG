/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73732
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73732 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73732
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned char) var_4);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 1; i_0 < 12; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 10; i_1 += 2) 
        {
            {
                var_17 = ((/* implicit */short) -5499772543410863024LL);
                /* LoopNest 3 */
                for (unsigned short i_2 = 0; i_2 < 13; i_2 += 4) 
                {
                    for (_Bool i_3 = 0; i_3 < 1; i_3 += 1) 
                    {
                        for (unsigned char i_4 = 0; i_4 < 13; i_4 += 4) 
                        {
                            {
                                var_18 = ((/* implicit */unsigned char) ((short) ((((/* implicit */_Bool) arr_6 [i_1 - 2] [i_1] [i_2] [i_3])) ? (-1LL) : (((/* implicit */long long int) arr_5 [i_1 - 2] [i_0] [i_3])))));
                                var_19 = ((/* implicit */long long int) arr_3 [i_2]);
                            }
                        } 
                    } 
                } 
                arr_11 [i_0 - 1] [i_0] = ((/* implicit */short) min((((/* implicit */int) (!(((/* implicit */_Bool) arr_1 [i_1 - 2] [i_1 + 1]))))), (max((((/* implicit */int) (unsigned short)0)), (2147221504)))));
            }
        } 
    } 
    var_20 = ((/* implicit */unsigned int) max((var_20), (((/* implicit */unsigned int) (_Bool)0))));
}
