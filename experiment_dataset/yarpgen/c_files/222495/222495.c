/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222495
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222495 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222495
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = (((min((arr_1 [i_1]), (arr_0 [i_1])))) || ((min((arr_2 [i_0] [1] [i_0]), (arr_0 [i_0])))));
                var_17 = (arr_0 [i_0]);
            }
        }
    }
    var_18 = var_1;

    for (int i_2 = 4; i_2 < 16;i_2 += 1)
    {
        var_19 = ((-107 ? (min((arr_7 [i_2 + 1]), (((var_1 + 9223372036854775807) << (var_2 - 41809))))) : (arr_7 [i_2])));
        var_20 = ((!(arr_6 [i_2 - 3])));
        var_21 = ((var_0 ? (var_8 >= var_15) : (((!(((10064332481196361643 ? var_11 : (arr_6 [i_2])))))))));
    }
    #pragma endscop
}
