/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49898
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49898 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49898
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 |= 0;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 10;i_2 += 1)
            {
                {
                    var_17 = 255;

                    for (int i_3 = 0; i_3 < 0;i_3 += 1) /* same iter space */
                    {
                        var_18 = (max((((~(-8437610484257639144 & 1130494273)))), (max(((-99 ? 8437610484257639144 : var_13)), 22213))));
                        arr_10 [i_1] [i_1] [i_1] [i_1] [i_3] = ((155 | ((75 ? (arr_0 [i_2] [i_2]) : 77))));
                    }
                    for (int i_4 = 0; i_4 < 0;i_4 += 1) /* same iter space */
                    {
                        arr_13 [i_4] [i_1] [i_4] [i_4] = 1;
                        var_19 = (min(-1, ((((-8633019313935136009 | var_3) || (((var_1 ? -8437610484257639137 : var_6))))))));
                        var_20 = (max(var_20, var_1));
                    }
                }
            }
        }
    }
    #pragma endscop
}
