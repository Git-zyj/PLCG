/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199090
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199090 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199090
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (!(~var_8));

    for (int i_0 = 4; i_0 < 15;i_0 += 1)
    {
        arr_4 [i_0 - 3] = (arr_1 [i_0]);
        arr_5 [i_0 - 1] = -32765;
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 14;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 16;i_3 += 1)
                {
                    {
                        var_11 = 8762231367179387136;
                        var_12 = ((-((((max(7, 253989269652223129))) ? (arr_1 [i_0]) : 11899616801646417220))));
                    }
                }
            }
        }
    }
    /* vectorizable */
    for (int i_4 = 0; i_4 < 17;i_4 += 1)
    {
        var_13 = (max(var_13, var_4));
        var_14 = (max(var_14, (((((11899616801646417219 ? (arr_14 [16]) : 13160382942931494979)) % 1)))));
        arr_16 [i_4] = var_4;
    }
    var_15 = (max(var_15, 23));
    var_16 = ((var_0 * (((var_6 / var_4) / var_0))));
    #pragma endscop
}
