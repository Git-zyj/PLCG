/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247957
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247957 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247957
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (var_9 * ((((877847217 && var_1) && 2147483647))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    var_18 ^= (((((min((arr_3 [i_0]), -8524)) / ((~(arr_3 [i_0]))))) / (((((arr_1 [i_1]) >= (arr_4 [i_2] [i_2] [i_1] [i_0])))))));
                    arr_7 [i_0] [i_1] [i_2] [i_2] = ((var_6 / (26990 < 60502)));
                }
            }
        }
    }
    var_19 = max(((((((6777038907279371189 ? var_14 : var_5))) || var_11))), var_2);
    #pragma endscop
}
