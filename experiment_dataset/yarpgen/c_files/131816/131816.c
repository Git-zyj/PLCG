/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131816
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131816 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131816
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        arr_3 [6] = (arr_0 [14] [i_0]);
        arr_4 [3] |= -9223372036854775784;
    }
    var_13 = var_10;
    /* LoopNest 3 */
    for (int i_1 = 0; i_1 < 17;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 17;i_2 += 1)
        {
            for (int i_3 = 1; i_3 < 14;i_3 += 1)
            {
                {
                    arr_13 [i_1] [i_1] [10] = (((((arr_10 [10] [i_2] [i_3] [i_3]) / -195))) ? (arr_6 [i_1]) : (((arr_12 [i_3 + 3] [i_3 + 2] [5] [i_3]) ? var_4 : (arr_12 [i_3 - 1] [i_3 + 2] [i_3] [i_3]))));
                    arr_14 [12] [i_2] [i_3] &= (~-1812336567);
                    arr_15 [2] [11] [i_1] [i_1] = (arr_1 [14]);
                }
            }
        }
    }
    #pragma endscop
}
