/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105006
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105006 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105006
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_8;
    var_15 = var_12;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_10 [i_1] = (((var_7 ? -11 : -82)));
                    var_16 = (((~281474976710655) ? (((arr_9 [i_0 - 1] [i_1] [i_0]) ? (((var_11 ? 66 : 102))) : (arr_0 [i_1] [i_2]))) : ((((!var_5) ? (arr_5 [i_2] [i_2]) : (((var_2 <= (arr_7 [i_0] [i_0] [i_1] [i_1])))))))));
                }
            }
        }
    }
    var_17 = (min(var_4, ((var_11 ? (!var_8) : var_4))));
    #pragma endscop
}
