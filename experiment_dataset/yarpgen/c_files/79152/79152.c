/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 79152
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=79152 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/79152
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = 8941342939157088899;
    var_14 = (max(((max(var_10, var_8))), var_0));
    var_15 = (((((var_12 || ((min(var_7, 0)))))) != (((min(var_4, var_1))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_16 -= ((-(((!((((arr_2 [i_0] [i_0] [i_1]) * 0))))))));
                var_17 = (((arr_0 [i_0]) ^ (((((((-8941342939157088918 ? 12262 : -1867648630))) || (((-(arr_3 [i_0]))))))))));
            }
        }
    }
    var_18 += var_5;
    #pragma endscop
}
