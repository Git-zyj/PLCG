/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172673
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172673 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172673
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */_Bool) ((short) ((unsigned long long int) (~(var_9)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12; i_0 += 3) 
    {
        for (unsigned char i_1 = 2; i_1 < 11; i_1 += 1) 
        {
            {
                arr_8 [i_1] = ((/* implicit */long long int) (!(((/* implicit */_Bool) (+((+(((/* implicit */int) (_Bool)1)))))))));
                arr_9 [3U] [3U] |= ((/* implicit */unsigned char) (~((~((~(((/* implicit */int) arr_4 [i_1] [(unsigned char)2] [i_0]))))))));
            }
        } 
    } 
}
