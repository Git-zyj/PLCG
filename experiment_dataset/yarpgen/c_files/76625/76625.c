/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76625
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76625 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76625
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, (((!(((~(!114)))))))));
    var_11 = (((~var_6) | (max(var_5, (1 | var_0)))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = (((((~((min(0, 142)))))) ? ((max(65535, (arr_3 [i_0] [5])))) : (arr_3 [i_1 - 1] [i_0 + 1])));
                arr_5 [i_0 + 2] [i_0 + 2] = (((((1 ? -17715 : 3860360606))) ? 1 : 1));
                arr_6 [i_0] [i_0] [i_0] = (arr_0 [i_0 - 2] [i_0 - 1]);
            }
        }
    }
    #pragma endscop
}
