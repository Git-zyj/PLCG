/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204902
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204902 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204902
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        var_14 |= (((((((var_1 ? var_9 : (arr_0 [20])))) ? 28242 : var_1)) >> ((((var_11 + ((52 ? (arr_0 [i_0]) : (arr_1 [i_0]))))) - 2078473290978709697))));
        arr_2 [i_0 - 1] [i_0 - 1] = (arr_1 [i_0]);
    }
    var_15 = (max((((min(32767, var_0)) ^ ((var_11 ? var_0 : var_1)))), ((var_11 ? (max(var_8, var_0)) : (var_13 < var_10)))));

    /* vectorizable */
    for (int i_1 = 4; i_1 < 7;i_1 += 1)
    {
        var_16 = ((~(-32762 <= var_8)));
        arr_6 [i_1] = var_10;
    }
    #pragma endscop
}
