/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36717
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36717 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36717
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (signed char i_0 = 1; i_0 < 8; i_0 += 2) 
    {
        for (signed char i_1 = 2; i_1 < 10; i_1 += 4) 
        {
            for (signed char i_2 = 0; i_2 < 11; i_2 += 1) 
            {
                {
                    var_14 = arr_5 [i_0] [i_1] [i_1] [(signed char)4];
                    var_15 = ((/* implicit */signed char) ((((/* implicit */_Bool) (signed char)114)) ? (((/* implicit */int) arr_4 [(signed char)4] [i_0])) : (((/* implicit */int) ((signed char) ((((/* implicit */_Bool) arr_3 [i_1 - 2])) ? (((/* implicit */int) (signed char)-107)) : (((/* implicit */int) var_12))))))));
                }
            } 
        } 
    } 
    /* LoopNest 2 */
    for (signed char i_3 = 0; i_3 < 18; i_3 += 4) 
    {
        for (signed char i_4 = 0; i_4 < 18; i_4 += 3) 
        {
            {
                var_16 = (signed char)-69;
                arr_12 [i_3] [i_4] = arr_11 [i_3] [i_4];
                var_17 = arr_11 [i_3] [i_4];
            }
        } 
    } 
}
