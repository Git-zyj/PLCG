/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 92624
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=92624 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/92624
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (((((var_4 ? 0 : -11058)))) ? ((((var_2 ? var_2 : var_3)) * var_10)) : ((max(((var_5 ? var_6 : 0)), (!var_2)))));
    var_13 = var_7;
    var_14 = (((var_4 > var_1) ? ((((min(12, var_0))) ? var_1 : -var_3)) : (var_6 + 1)));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = ((var_7 ? var_8 : (arr_2 [i_0])));
        arr_4 [i_0] [i_0] = var_2;
    }
    var_15 = 11042;
    #pragma endscop
}
