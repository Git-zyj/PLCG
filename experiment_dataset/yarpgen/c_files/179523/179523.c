/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179523
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179523 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179523
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 -= (((((~(max(var_4, var_11))))) ? (var_5 < var_11) : var_0));
    var_14 = ((!(32970 <= var_7)));
    var_15 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_16 = (arr_3 [i_0] [i_0]);
                var_17 = ((!(((12159 ? (arr_2 [i_0] [i_1]) : -12159)))));
                var_18 = (max(var_18, (arr_0 [6] [i_0])));
                var_19 = ((+(((arr_0 [i_1] [i_0]) ? (arr_0 [i_0] [i_1]) : (arr_0 [i_0] [i_0])))));
            }
        }
    }
    var_20 = (!781365048336381370);
    #pragma endscop
}
