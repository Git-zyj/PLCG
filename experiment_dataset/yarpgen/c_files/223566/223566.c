/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 223566
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=223566 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/223566
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {

        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            arr_5 [i_0 + 1] = (33554368 / -14701);
            arr_6 [19] = ((0 ? -2147483622 : 6861125948754168196));
        }
        arr_7 [i_0] = ((((!(arr_1 [i_0] [i_0 + 1])))) == ((min(((var_17 == (arr_0 [9]))), (arr_2 [i_0 - 1])))));
        arr_8 [i_0 - 3] = var_7;
    }
    var_18 = (131056 && -20687);
    var_19 = (max((2147483628 == -2147483634), 35883));
    #pragma endscop
}
