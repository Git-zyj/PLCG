/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 140985
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=140985 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/140985
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (!1347611830);
    var_14 = (max(var_14, ((max(45, (((~var_6) | var_8)))))));
    var_15 = max(((-3478 ? -11133 : -11161)), (30 && 191));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 23;i_2 += 1)
            {
                {
                    var_16 &= (arr_0 [i_1] [i_2]);
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 25;i_4 += 1)
                        {
                            {
                                var_17 ^= (arr_4 [i_4] [i_1] [i_4]);
                                var_18 = (arr_6 [i_1] [i_1] [i_2 + 2] [i_1]);
                            }
                        }
                    }
                    var_19 -= ((~(((!(arr_5 [i_1] [i_1] [i_2 + 2]))))));
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_5 = 0; i_5 < 11;i_5 += 1)
    {
        for (int i_6 = 1; i_6 < 9;i_6 += 1)
        {
            {
                var_20 += ((20 ? 1 : 191));
                var_21 = (!var_12);
            }
        }
    }
    #pragma endscop
}
