/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155498
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155498 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155498
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */short) ((((/* implicit */_Bool) var_7)) ? (((/* implicit */unsigned long long int) ((unsigned int) (-(((/* implicit */int) (short)-11857)))))) : (((((/* implicit */_Bool) var_1)) ? (6451276543641942801ULL) : (min((5379052276039280984ULL), (((/* implicit */unsigned long long int) var_14))))))));
    /* LoopNest 2 */
    for (unsigned short i_0 = 0; i_0 < 21; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 21; i_1 += 3) 
        {
            {
                var_17 = ((/* implicit */unsigned char) arr_4 [i_0] [i_1] [i_1]);
                var_18 = ((/* implicit */unsigned int) var_1);
            }
        } 
    } 
    var_19 = ((/* implicit */unsigned int) var_5);
}
