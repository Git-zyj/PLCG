/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14496
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14496 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14496
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */short) var_10);
    /* LoopNest 3 */
    for (long long int i_0 = 0; i_0 < 25; i_0 += 1) 
    {
        for (int i_1 = 0; i_1 < 25; i_1 += 1) 
        {
            for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
            {
                {
                    var_17 -= ((/* implicit */short) ((max((((/* implicit */unsigned int) (unsigned char)167)), (arr_3 [i_0]))) > (((/* implicit */unsigned int) ((/* implicit */int) (signed char)4)))));
                    var_18 = ((/* implicit */long long int) (!(((/* implicit */_Bool) (signed char)(-127 - 1)))));
                }
            } 
        } 
    } 
    var_19 = ((/* implicit */signed char) (~(((/* implicit */int) var_7))));
    var_20 = ((/* implicit */int) (short)(-32767 - 1));
    var_21 += ((/* implicit */signed char) (-(var_3)));
}
