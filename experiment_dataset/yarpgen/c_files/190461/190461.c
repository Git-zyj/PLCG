/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190461
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190461 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190461
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 11;i_1 += 1)
        {
            {
                var_18 = ((((((arr_2 [i_1] [i_1] [i_1 + 3]) ? 0 : (((!(arr_3 [i_1] [i_1]))))))) ? (max((arr_2 [i_1] [i_1 + 1] [i_1 - 2]), var_14)) : ((((((var_15 ? var_10 : var_12)) < (!var_13)))))));
                var_19 += var_6;
            }
        }
    }
    var_20 = (!var_1);
    var_21 = ((var_4 ? (min(((1 ? var_11 : var_11)), 1456843653509525847)) : (((min(2047, var_5))))));
    #pragma endscop
}
