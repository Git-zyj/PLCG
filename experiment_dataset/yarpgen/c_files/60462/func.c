/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60462
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60462 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60462
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 *= ((/* implicit */unsigned int) var_8);
    /* LoopSeq 1 */
    for (long long int i_0 = 0; i_0 < 12; i_0 += 2) 
    {
        var_13 = ((/* implicit */int) (unsigned short)29700);
        var_14 = ((/* implicit */long long int) (signed char)20);
        var_15 = ((/* implicit */unsigned long long int) arr_1 [i_0]);
        var_16 -= ((/* implicit */unsigned short) arr_1 [i_0]);
    }
    var_17 = ((/* implicit */unsigned short) var_0);
    var_18 = ((/* implicit */int) 14U);
    var_19 = ((/* implicit */unsigned short) (signed char)-89);
}
