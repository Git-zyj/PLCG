/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 819
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=819 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/819
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_7;
    var_12 = (min((min(-76, 1581464097)), var_0));

    for (int i_0 = 1; i_0 < 23;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = var_10;

        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            var_13 = (((((var_2 ? ((var_1 ? (arr_4 [i_0] [i_0]) : (arr_1 [i_0]))) : (max((arr_4 [i_0] [i_1]), (arr_1 [i_0])))))) ? (((max((min(var_0, (arr_4 [i_0] [i_1]))), var_10)))) : ((var_6 ? ((min(var_7, (arr_4 [i_1] [i_0])))) : (((arr_0 [i_0]) ? var_9 : (arr_4 [i_0] [i_1])))))));
            var_14 = (arr_1 [i_0]);
        }
    }

    /* vectorizable */
    for (int i_2 = 1; i_2 < 20;i_2 += 1)
    {
        arr_7 [16] [16] &= (arr_0 [22]);
        var_15 = var_3;
        var_16 &= var_1;
    }
    #pragma endscop
}
