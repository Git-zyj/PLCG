/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181762
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181762 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181762
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_15 -= (((((((((arr_4 [i_1] [i_0]) ? 1 : var_11))) ? 8190 : (arr_4 [12] [i_1])))) && ((min(var_1, (arr_6 [i_0 - 1]))))));
                arr_7 [i_1] = ((((((~(((((arr_1 [14]) + 2147483647)) << (((arr_6 [i_0 - 1]) - 38148)))))) + 2147483647)) << (((((arr_1 [1]) + 1332471109)) - 26))));
            }
        }
    }
    var_16 -= -0;
    var_17 = (((!(~var_5))) ? var_9 : -12);
    #pragma endscop
}
