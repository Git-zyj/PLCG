/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88372
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88372 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88372
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */long long int) max((((/* implicit */unsigned long long int) (~(((/* implicit */int) var_5))))), (max(((-(10068380256446756888ULL))), (((/* implicit */unsigned long long int) var_9))))));
    /* LoopSeq 1 */
    for (long long int i_0 = 1; i_0 < 11; i_0 += 2) 
    {
        var_15 = ((/* implicit */int) arr_1 [i_0]);
        arr_2 [i_0] = ((/* implicit */unsigned short) ((((/* implicit */_Bool) arr_1 [i_0])) ? (((/* implicit */int) arr_0 [i_0 + 1])) : (((/* implicit */int) var_5))));
    }
    var_16 = ((/* implicit */signed char) min((var_16), (((/* implicit */signed char) var_7))));
    var_17 = ((/* implicit */int) (((_Bool)1) ? (288160007407534080ULL) : (((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned short)36236)))));
    var_18 = ((/* implicit */int) var_3);
}
