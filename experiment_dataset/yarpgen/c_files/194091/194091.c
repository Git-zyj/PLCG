/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194091
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194091 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194091
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        var_15 = (min(var_15, (arr_0 [i_0 - 2] [i_0 - 2])));
        arr_2 [i_0] [7] = (((arr_0 [i_0 + 1] [i_0 - 2]) > (arr_1 [11])));
    }
    for (int i_1 = 2; i_1 < 9;i_1 += 1)
    {
        var_16 = ((((134217727 < (var_5 ^ var_14))) && var_1));
        arr_5 [i_1] = (-83842425 & 3236879447);
    }
    var_17 = ((-(((min(var_14, var_6)) - var_11))));
    var_18 |= (min((26563 & 2423045356), ((((-228362402 ? 26563 : 968221462))))));
    #pragma endscop
}
