/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 25820
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=25820 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/25820
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            {
                arr_6 [i_0] = (~var_4);
                var_10 = (max(var_2, (((253 ? 63104 : var_8)))));
                arr_7 [i_1] [i_0] = ((((!((max(var_4, var_0)))))));
                arr_8 [9] [i_0] = var_3;
            }
        }
    }
    var_11 = -24;

    for (int i_2 = 2; i_2 < 14;i_2 += 1)
    {
        var_12 *= (((arr_9 [i_2 - 1]) / -var_8));
        arr_12 [i_2] [i_2] = (max(((min((469762048 < 36647), (max(var_5, (arr_9 [i_2 + 1])))))), (arr_9 [i_2 - 1])));
        var_13 &= (var_4 >= var_0);
        var_14 = (max(28897, -7118));
    }
    #pragma endscop
}
