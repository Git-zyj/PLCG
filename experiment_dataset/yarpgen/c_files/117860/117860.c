/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117860
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117860 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117860
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_11 = (max(var_11, ((min((~-585399571), (1805418524 | 10170))))));
        var_12 = (arr_1 [i_0] [i_0]);
        var_13 = var_6;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        arr_5 [i_1] [i_1] = (-(-1805418525 + 1805418524));
        var_14 = var_0;
        var_15 = var_4;
    }
    /* vectorizable */
    for (int i_2 = 1; i_2 < 1;i_2 += 1)
    {
        arr_9 [i_2] [i_2] = 1805418533;
        arr_10 [i_2] = (arr_4 [i_2 - 1] [i_2 - 1]);
        var_16 *= (((arr_7 [i_2 - 1] [i_2 - 1]) ? (arr_7 [i_2 - 1] [i_2 - 1]) : (arr_3 [20])));
    }
    var_17 = (max(var_17, (min(-1805418540, 249))));
    #pragma endscop
}
