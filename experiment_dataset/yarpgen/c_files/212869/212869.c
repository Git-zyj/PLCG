/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212869
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212869 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212869
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_6 [i_0] |= (((arr_5 [i_1]) * var_1));

                /* vectorizable */
                for (int i_2 = 2; i_2 < 20;i_2 += 1)
                {
                    var_19 = (((10 ^ 8388607) | (~var_10)));
                    var_20 = 2265487810;
                }
                for (int i_3 = 0; i_3 < 22;i_3 += 1)
                {
                    var_21 = ((((-23347065076547975 / (((min(-28, 110)))))) * (1 / 3485146972)));
                    arr_11 [i_0] [i_0] [i_3] = 190484496;
                    var_22 = ((-3710715978941466146 * ((0 ? 797758338 : 3901855115))));
                }
            }
        }
    }
    var_23 = (min(-203959230, 826290339));
    #pragma endscop
}
