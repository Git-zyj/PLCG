/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 183464
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=183464 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/183464
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_15;
    var_20 = 144115188075851776;
    var_21 ^= ((min(18302628885633699839, 4261998730)));
    var_22 = 53593;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            {
                var_23 = ((arr_0 [i_0 - 2]) | (arr_1 [i_0]));
                var_24 = (arr_2 [i_0]);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        {
                            arr_12 [i_0] [i_1 - 3] [i_2] = var_3;
                            var_25 = min((min(-4261998732, 65535)), -32968583);
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
