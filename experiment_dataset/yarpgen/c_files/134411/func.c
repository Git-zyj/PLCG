/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 134411
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=134411 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/134411
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */long long int) ((-2015106138759832110LL) > (((/* implicit */long long int) ((/* implicit */int) (signed char)67)))));
    /* LoopNest 2 */
    for (signed char i_0 = 4; i_0 < 15; i_0 += 2) 
    {
        for (unsigned short i_1 = 1; i_1 < 15; i_1 += 3) 
        {
            {
                var_19 = ((/* implicit */unsigned short) max((var_19), (((/* implicit */unsigned short) (-(((((/* implicit */_Bool) (signed char)-34)) ? (((/* implicit */unsigned int) ((/* implicit */int) (signed char)33))) : (3048488948U))))))));
                var_20 = ((/* implicit */unsigned char) (signed char)-35);
            }
        } 
    } 
    var_21 &= ((/* implicit */unsigned char) var_6);
}
