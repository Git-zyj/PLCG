/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204008
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204008 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204008
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 *= var_8;
    var_18 &= -1024021777108704371;
    var_19 = ((var_6 ? (max(var_3, var_16)) : (var_5 >= var_14)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 17;i_2 += 1)
            {
                {
                    var_20 = ((max((arr_2 [i_1 + 1]), 1)));

                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        var_21 = 1;
                        var_22 = ((((((((arr_10 [i_0] [i_0] [i_3]) ? var_12 : var_8))) ? ((((!(arr_4 [i_0] [i_0] [i_0]))))) : (max((arr_9 [i_2]), var_6)))) != ((((-var_4 || (arr_0 [7] [i_2])))))));
                    }
                }
            }
        }
    }
    #pragma endscop
}
