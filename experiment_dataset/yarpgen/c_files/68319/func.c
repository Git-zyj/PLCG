/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68319
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68319 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68319
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */short) (~((-(max((((/* implicit */int) var_3)), (var_4)))))));
    var_13 = ((/* implicit */unsigned int) (-(((/* implicit */int) var_9))));
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 0; i_0 += 1) 
    {
        arr_3 [i_0 + 1] = ((/* implicit */unsigned short) var_0);
        /* LoopSeq 1 */
        for (unsigned long long int i_1 = 0; i_1 < 18; i_1 += 2) 
        {
            var_14 = ((/* implicit */unsigned int) var_1);
            var_15 = ((/* implicit */unsigned int) max((var_15), (((/* implicit */unsigned int) (+(var_2))))));
        }
        var_16 = (-(((/* implicit */int) (!(((/* implicit */_Bool) max((var_10), (var_1))))))));
        var_17 = (-(((/* implicit */int) var_0)));
        var_18 *= var_1;
    }
    /* LoopNest 3 */
    for (unsigned long long int i_2 = 1; i_2 < 22; i_2 += 1) 
    {
        for (unsigned short i_3 = 0; i_3 < 25; i_3 += 4) 
        {
            for (unsigned long long int i_4 = 2; i_4 < 21; i_4 += 4) 
            {
                {
                    var_19 = ((/* implicit */unsigned short) (-(((/* implicit */int) (!(((/* implicit */_Bool) var_6)))))));
                    var_20 = ((/* implicit */unsigned short) var_10);
                    var_21 = ((/* implicit */unsigned char) var_5);
                }
            } 
        } 
    } 
    var_22 = ((/* implicit */int) (-(min((((/* implicit */unsigned long long int) min((var_4), (var_10)))), ((+(var_11)))))));
}
