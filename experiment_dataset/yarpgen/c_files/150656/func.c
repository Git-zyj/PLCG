/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150656
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150656 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150656
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */unsigned short) max((var_7), (var_6)));
    var_19 = ((/* implicit */unsigned short) max((max((((/* implicit */int) min((var_11), (var_14)))), ((~(((/* implicit */int) var_15)))))), (((/* implicit */int) var_0))));
    var_20 = ((/* implicit */unsigned short) (-(((/* implicit */int) max((var_8), (max((var_3), (((/* implicit */unsigned short) var_2)))))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 1; i_0 < 10; i_0 += 4) 
    {
        var_21 = ((/* implicit */unsigned char) var_6);
        var_22 = ((/* implicit */unsigned long long int) (-(((/* implicit */int) var_11))));
    }
}
