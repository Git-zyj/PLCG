/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213862
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213862 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213862
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_5 [i_0] = (((((45821 ? (arr_4 [i_0] [i_1] [i_1]) : (arr_4 [i_0] [i_0] [0])))) ? 99629457 : ((((arr_1 [i_0] [i_1]) * ((((arr_4 [i_0] [i_0] [i_1]) > (arr_3 [i_0] [i_1] [i_0])))))))));
                arr_6 [i_0] = (((arr_2 [i_0] [6] [i_0]) ? (((!(arr_2 [i_1] [i_0] [i_0])))) : ((-626424712042731894 ? 2206 : 1))));
            }
        }
    }
    var_17 = var_13;
    var_18 = -var_16;
    var_19 = (var_13 / var_16);
    #pragma endscop
}
