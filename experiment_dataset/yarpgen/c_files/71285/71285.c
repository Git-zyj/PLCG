/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71285
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71285 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71285
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 24;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        {
                            var_13 -= (((arr_8 [i_0] [i_0]) << (((((3907745886 ? (arr_4 [1] [10] [10]) : (arr_8 [i_1] [i_2])))) - 5486))));
                            var_14 = 4194303;
                        }
                    }
                }
                arr_9 [11] [i_1] [i_1] = 18446744073705357313;
                var_15 *= ((((4194306 ? (~4194288) : (~19))) * (((((arr_6 [i_0] [20] [i_1] [i_1]) != (arr_6 [i_0] [i_0] [i_1] [i_1])))))));
                var_16 = (max(var_16, (((~((4194294 ? (arr_5 [i_0]) : (arr_5 [i_0]))))))));
                var_17 = 1;
            }
        }
    }
    var_18 = var_6;
    #pragma endscop
}
