/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45494
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45494 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45494
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 10;i_2 += 1)
            {
                {
                    var_15 = (((((arr_3 [i_2 + 2] [i_2 + 2] [i_2 - 1]) / (arr_3 [i_2 + 1] [i_2 + 3] [i_2 + 1]))) > (arr_4 [1])));
                    var_16 *= (max(((~(arr_2 [i_2 - 1] [i_2 + 3]))), (((arr_2 [i_2 + 3] [i_2 + 3]) ? (arr_2 [i_2 + 1] [i_2 - 1]) : 66))));
                    var_17 = (max(var_17, (((!(arr_5 [i_1] [i_2]))))));
                }
            }
        }
    }
    var_18 = 1;
    #pragma endscop
}
