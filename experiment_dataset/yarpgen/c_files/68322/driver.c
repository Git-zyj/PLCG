#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

unsigned char var_1 = (unsigned char)234;
_Bool var_3 = (_Bool)0;
long long int var_7 = -90187321877397269LL;
short var_10 = (short)-1545;
int zero = 0;
signed char var_15 = (signed char)46;
signed char var_16 = (signed char)-99;
long long int var_17 = 1436211684044882993LL;
unsigned int var_18 = 266863111U;
signed char var_19 = (signed char)-123;
long long int var_20 = 4759724351883077278LL;
short var_21 = (short)-23010;
unsigned int var_22 = 2170040312U;
unsigned int arr_2 [18] ;
signed char arr_3 [18] ;
unsigned int arr_5 [18] [18] [18] ;
unsigned char arr_9 [20] ;
signed char arr_10 [20] ;
void init() {
    for (size_t i_0 = 0; i_0 < 18; ++i_0) 
        arr_2 [i_0] = 3631387823U;
    for (size_t i_0 = 0; i_0 < 18; ++i_0) 
        arr_3 [i_0] = (signed char)46;
    for (size_t i_0 = 0; i_0 < 18; ++i_0) 
        for (size_t i_1 = 0; i_1 < 18; ++i_1) 
            for (size_t i_2 = 0; i_2 < 18; ++i_2) 
                arr_5 [i_0] [i_1] [i_2] = 2584507733U;
    for (size_t i_0 = 0; i_0 < 20; ++i_0) 
        arr_9 [i_0] = (unsigned char)249;
    for (size_t i_0 = 0; i_0 < 20; ++i_0) 
        arr_10 [i_0] = (signed char)-3;
}

void checksum() {
    hash(&seed, var_15);
    hash(&seed, var_16);
    hash(&seed, var_17);
    hash(&seed, var_18);
    hash(&seed, var_19);
    hash(&seed, var_20);
    hash(&seed, var_21);
    hash(&seed, var_22);
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
