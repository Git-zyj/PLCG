/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177966
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177966 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177966
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 7;i_1 += 1)
        {
            {
                arr_5 [i_1 - 1] [i_0] [i_0] = arr_3 [i_0 + 1] [i_1];
                var_17 = 4970;

                for (int i_2 = 0; i_2 < 11;i_2 += 1)
                {
                    var_18 = min((arr_8 [i_2]), ((min((max((arr_2 [i_1 + 4] [i_0 + 2]), var_5)), ((max(15, (arr_3 [i_0] [5]))))))));

                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        var_19 = ((((arr_7 [i_0] [i_0] [i_3]) == (arr_0 [i_0] [i_0]))));
                        arr_11 [i_0] [i_0] [i_0] [1] = (((~230) <= (arr_1 [i_0])));
                    }
                    for (int i_4 = 0; i_4 < 11;i_4 += 1)
                    {
                        var_20 = (arr_8 [i_0]);
                        var_21 = (arr_13 [i_0 + 2] [i_0 - 1] [i_0] [i_0 - 1]);
                    }
                }
                var_22 = ((max(var_15, (arr_0 [i_1] [i_0]))));
                var_23 = (((~(arr_13 [i_0 - 1] [i_0] [i_0] [i_0 - 1]))));
            }
        }
    }
    var_24 = var_6;
    #pragma endscop
}
