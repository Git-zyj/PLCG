/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46860
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46860 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46860
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    arr_10 [i_1] [i_1] [i_2] = ((-(max(var_7, 3285564583))));
                    arr_11 [i_0] [i_0] = ((((min(-90, var_8))) ? (((arr_2 [i_1 - 1] [i_1 - 1]) ? (arr_2 [i_1 - 1] [i_1 - 1]) : (arr_2 [i_1 - 1] [5]))) : (((!(arr_2 [i_1 - 1] [i_1 - 1]))))));
                    var_19 = ((~((~(arr_4 [i_1 - 2] [9] [i_2])))));
                    arr_12 [i_1] = (min((arr_1 [i_0]), var_15));
                }
            }
        }
    }
    var_20 += var_1;
    #pragma endscop
}
