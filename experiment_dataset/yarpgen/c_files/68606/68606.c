/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68606
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68606 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68606
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = -var_10;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = var_5;
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    arr_10 [i_0] [i_1] = var_2;
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 19;i_4 += 1)
                        {
                            {
                                var_12 = (var_7 % var_9);
                                arr_16 [16] [i_1] [i_2] [i_2] [i_4] = (((var_7 ? var_0 : var_8)));
                                var_13 = (((var_5 ? var_6 : var_7)));
                                var_14 = ((var_0 ? var_3 : (var_2 == var_7)));
                            }
                        }
                    }
                    arr_17 [i_1] [i_1] [i_2] = (var_10 ? (!var_6) : (!var_0));
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < 20;i_5 += 1)
    {
        var_15 = ((~((268435456 | ((var_4 ? var_0 : var_9))))));
        arr_20 [i_5] = var_10;
        var_16 *= var_10;
    }
    var_17 = var_8;
    var_18 = ((var_3 ? (max(-105, 66060288)) : var_6));
    var_19 &= -var_2;
    #pragma endscop
}
