/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 201678
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=201678 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/201678
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_21 = ((~(0 <= 0)));
                arr_5 [i_0] [i_0] |= ((((((((0 ^ (arr_2 [18] [i_0])))) & ((~(arr_4 [i_0] [i_0])))))) ? ((~(arr_4 [6] [i_0]))) : (arr_4 [i_0 - 3] [i_0])));
                arr_6 [8] [i_1] [i_1] = (((~((~(arr_0 [13]))))));
                var_22 = (min(var_22, (arr_3 [i_0] [i_0 - 2])));
            }
        }
    }
    var_23 ^= 254;
    var_24 = (((((var_5 == var_8) ? (~var_5) : ((var_7 ? var_3 : 1818162095)))) - var_11));
    #pragma endscop
}
