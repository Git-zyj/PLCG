/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249018
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249018 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249018
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min((max(((max(-1, var_8))), (max(35481, var_5)))), (((((max(341095330, 31778))) ? var_6 : (((1785815192 ? 121 : 33727))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 20;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_1] = var_9;
                var_14 = ((((((var_11 ? var_9 : 67108864)) >> (arr_1 [i_1 - 2] [i_1 - 2]))) <= (((arr_4 [i_0]) ? var_1 : -67108885))));
            }
        }
    }
    #pragma endscop
}
