/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 107241
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=107241 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/107241
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_3;

    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        var_17 *= var_0;
        arr_3 [i_0] [i_0] = (((arr_0 [i_0 + 3] [i_0 - 1]) || (arr_0 [i_0 + 1] [i_0])));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 10;i_2 += 1)
            {
                {
                    var_18 = ((((var_14 ^ var_14) ? (arr_4 [i_1] [i_0]) : (arr_4 [i_0 - 1] [i_0]))));

                    for (int i_3 = 3; i_3 < 8;i_3 += 1)
                    {
                        arr_12 [i_0] [i_1] [i_2 - 1] [i_3 + 1] = arr_5 [i_0];
                        arr_13 [i_0] = -var_4;
                    }
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < 20;i_4 += 1)
    {
        arr_18 [i_4] &= var_4;
        var_19 *= ((((64 ? ((min(var_4, (arr_16 [3])))) : var_1))) - (((arr_17 [i_4] [i_4]) ? var_8 : ((-(arr_15 [i_4]))))));
    }
    var_20 = -var_11;
    var_21 = (min(var_21, var_8));
    #pragma endscop
}
