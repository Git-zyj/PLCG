/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 2983
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=2983 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/2983
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_17 = ((26299 << (65530 - 65516)));
                arr_6 [i_0] [i_1] [i_0] = ((((min((-32760 & var_0), (max(281474976706560, 3))))) ? (((((arr_0 [i_0]) ? var_4 : var_0)) >> (((arr_3 [i_0]) - 18279)))) : (--63)));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 1;i_2 += 1)
    {
        for (int i_3 = 3; i_3 < 12;i_3 += 1)
        {
            {
                arr_12 [i_2] [8] [i_2] &= var_15;
                arr_13 [i_2] [i_2] [i_2] = ((-569223087 ? (!-var_11) : ((min(339, 26299)))));
            }
        }
    }
    #pragma endscop
}
