/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 107024
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=107024 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/107024
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_6;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    var_17 = (arr_3 [i_0]);
                    var_18 = var_5;
                    var_19 = ((var_6 & (arr_0 [i_2] [i_0])));
                    arr_8 [i_0] [i_1] [i_0] = var_2;
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_3 = 0; i_3 < 13;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 13;i_4 += 1)
        {
            for (int i_5 = 3; i_5 < 12;i_5 += 1)
            {
                {
                    var_20 = (arr_9 [i_3] [i_4]);
                    arr_17 [i_5] [11] [3] = (-2147483647 - 1);
                    var_21 = (((max(var_15, ((min(var_2, var_2))))) - (max((max((arr_3 [22]), var_1)), (((-2147483647 - 1) | (-2147483647 - 1)))))));
                }
            }
        }
    }
    var_22 = ((((max((var_9 > var_2), var_3))) != var_9));
    var_23 = (max(var_7, var_11));
    #pragma endscop
}
