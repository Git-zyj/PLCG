/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 58267
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=58267 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/58267
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (((((var_2 > ((var_8 ? var_7 : var_1))))) == var_9));
    var_16 = (max(var_16, ((max((max(var_10, (var_1 + var_3))), (((((1492446816 ? 14943992003912877284 : 0)) == var_3))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_17 = ((((((var_2 ? (arr_1 [i_0]) : var_11)) == var_1)) ? 4294967295 : (((((arr_4 [i_1] [i_0] [i_0]) == 4294967293))))));
                arr_5 [i_1] = (((!((!(arr_0 [i_0]))))));
            }
        }
    }
    #pragma endscop
}
