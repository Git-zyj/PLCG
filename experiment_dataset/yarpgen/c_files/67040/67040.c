/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67040
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67040 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67040
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_10 += (max(var_9, (var_6 | -821373080)));
                arr_4 [i_0] [i_0] [i_1] = -18446744073709551612;
            }
        }
    }
    var_11 = (min(-var_8, ((max(var_8, var_3)))));
    var_12 = 821373051;

    for (int i_2 = 0; i_2 < 16;i_2 += 1)
    {
        var_13 = (-(arr_7 [i_2]));
        var_14 = (max(var_14, (max((max((arr_5 [i_2] [i_2]), (arr_5 [i_2] [i_2]))), (arr_2 [i_2])))));
    }
    #pragma endscop
}
