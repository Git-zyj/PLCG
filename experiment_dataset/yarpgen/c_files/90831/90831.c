/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90831
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90831 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90831
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 16;i_1 += 1)
        {
            {
                var_17 |= var_1;
                var_18 = ((-((~(((arr_2 [i_1] [i_1]) ? var_14 : var_6))))));
                arr_5 [i_0] [i_1] |= (max(var_7, (min((arr_2 [13] [i_1 - 1]), (arr_4 [i_1 - 1] [i_1] [6])))));
            }
        }
    }
    var_19 |= ((((((var_9 | var_7) ? var_1 : var_14))) + ((var_8 ? var_11 : ((var_0 ? var_8 : var_12))))));
    var_20 &= var_10;
    #pragma endscop
}
