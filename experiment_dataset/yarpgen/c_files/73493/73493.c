/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73493
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73493 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73493
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 *= (max((-2147483647 - 1), var_9));
    var_21 -= var_7;
    var_22 = (((((27748 + ((var_12 ? var_14 : var_14))))) ? ((var_19 ? 10152 : ((var_6 ? var_10 : var_1)))) : var_9));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] = (((arr_2 [i_2 - 1] [i_1] [i_0]) >= (arr_6 [i_0] [i_0] [i_0])));
                    var_23 = (arr_6 [i_2 - 1] [i_2 - 1] [i_2 - 1]);
                }
            }
        }
    }
    var_24 = (min(var_24, -1));
    #pragma endscop
}
