/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102716
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102716 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102716
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((max(((-29363 ? var_2 : var_1), (!var_0)))));

    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        arr_2 [i_0] = ((!(min(((28685 && (arr_0 [i_0]))), (-29349 && 1690812687)))));
        arr_3 [i_0] = ((((((arr_0 [i_0 - 1]) + 24))) ? ((((~var_14) ? (min((arr_1 [i_0] [i_0]), 544392116)) : var_0))) : ((~(arr_0 [i_0 - 1])))));
        arr_4 [i_0] = (arr_0 [i_0]);
    }
    var_17 = 59569;
    #pragma endscop
}
