/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 188315
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=188315 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/188315
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_7 [18] = (max(((var_4 ? var_4 : var_6)), 5360982631800369524));

                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    var_14 = ((~((max(var_5, var_10)))));
                    var_15 = ((-(!var_13)));
                    var_16 -= ((((((var_10 * var_7) * (~12039824498547464200)))) ? (min(((var_6 ? var_2 : var_8)), ((min(var_7, var_0))))) : (var_10 + var_7)));
                }
            }
        }
    }
    var_17 = var_0;
    var_18 = (((-45288 ? (~var_0) : var_11)));
    #pragma endscop
}
