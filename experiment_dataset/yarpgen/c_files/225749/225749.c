/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 225749
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=225749 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/225749
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = var_12;
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 18;i_3 += 1)
                {
                    {
                        var_20 = (min(var_20, 32753));
                        var_21 = (((!var_16) >= (arr_7 [i_1] [i_0] [i_1] [i_0])));

                        for (int i_4 = 2; i_4 < 17;i_4 += 1)
                        {
                            arr_13 [i_3] [i_1] [i_2] [i_3] [i_4] [i_4] [i_1] |= var_10;
                            var_22 += (arr_11 [0] [i_1] [i_2] [i_3] [1]);
                            arr_14 [i_4] [i_3] [i_0] = ((63 ? -1419086141 : (!3766)));
                        }
                        arr_15 [i_0] = ((~(arr_8 [4] [i_1] [i_2] [i_3])));
                        arr_16 [7] [7] [i_2] [17] = (!var_15);
                    }
                }
            }
        }
    }
    /* vectorizable */
    for (int i_5 = 1; i_5 < 9;i_5 += 1)
    {
        var_23 = (min(var_23, var_1));
        var_24 = (arr_17 [i_5 + 1]);
    }
    var_25 = (max(var_25, ((max((!var_2), ((var_10 ? 1419086141 : var_9)))))));
    #pragma endscop
}
