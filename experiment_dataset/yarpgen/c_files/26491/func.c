/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 26491
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=26491 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/26491
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 ^= var_1;
    var_13 = ((/* implicit */unsigned short) var_0);
    var_14 = ((/* implicit */unsigned int) (_Bool)1);
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 22; i_0 += 1) 
    {
        var_15 = ((/* implicit */int) ((((/* implicit */_Bool) (-(((/* implicit */int) var_7))))) ? (((/* implicit */unsigned long long int) ((((/* implicit */_Bool) var_11)) ? (arr_0 [i_0]) : (arr_0 [i_0])))) : (((var_1) / (((/* implicit */unsigned long long int) -924438350))))));
        var_16 -= ((/* implicit */int) min(((-(var_6))), (((/* implicit */unsigned long long int) ((unsigned char) arr_3 [i_0] [i_0])))));
        var_17 &= ((/* implicit */unsigned long long int) arr_3 [i_0] [i_0]);
        arr_4 [i_0] = ((/* implicit */long long int) var_10);
    }
    var_18 = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) var_8))));
}
