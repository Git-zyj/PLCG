/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123032
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123032 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123032
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_16 = ((arr_1 [i_0]) ? (arr_2 [i_0]) : (arr_1 [i_0]));
        var_17 -= var_13;
        arr_3 [i_0] = (arr_2 [i_0]);
    }
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 23;i_1 += 1)
    {
        for (int i_2 = 3; i_2 < 20;i_2 += 1)
        {
            {
                var_18 = (max(var_18, (max((((arr_2 [i_2 + 1]) ? 0 : -4401051791654162992)), (arr_1 [i_1])))));

                for (int i_3 = 0; i_3 < 23;i_3 += 1)
                {
                    var_19 = (((((arr_4 [i_1] [i_2]) ? (-16 != var_8) : var_15)) >> 21));
                    var_20 = (min((((1 != (arr_11 [i_1] [i_2 + 3] [i_3] [i_3])))), ((16612 ? (arr_11 [i_1] [i_2 + 3] [i_3] [i_3]) : var_15))));
                    var_21 = (min(var_21, var_12));
                }
            }
        }
    }
    var_22 = -9223372036854775786;
    #pragma endscop
}
