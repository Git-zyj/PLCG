/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241945
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241945 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241945
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((var_3 ? var_4 : (((((-63 ? var_1 : var_2)) + var_5)))));
    var_14 = 4294967289;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    arr_6 [14] = (+(((arr_0 [i_2]) ? (arr_2 [i_2] [0] [i_0]) : (arr_2 [0] [i_1] [i_0]))));
                    arr_7 [i_1 + 1] [i_0] = var_0;
                    arr_8 [i_1 + 1] [i_1] [i_2] [i_1] = ((max((arr_2 [i_1] [i_1 + 2] [i_1 - 2]), var_3)) & (((var_10 ? -1 : (arr_4 [i_1 + 2])))));
                }
            }
        }
    }
    #pragma endscop
}
