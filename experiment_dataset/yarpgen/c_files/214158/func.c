/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214158
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214158 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214158
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned short) var_15);
    /* LoopSeq 1 */
    for (unsigned int i_0 = 1; i_0 < 21; i_0 += 4) 
    {
        var_21 |= ((/* implicit */unsigned short) (+(((max((((/* implicit */long long int) var_6)), (18014123631575040LL))) / (((/* implicit */long long int) ((/* implicit */int) (_Bool)1)))))));
        arr_3 [22U] = ((/* implicit */unsigned short) var_8);
    }
}
