/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49955
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49955 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49955
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 2; i_0 < 23; i_0 += 3) 
    {
        var_20 = ((/* implicit */int) arr_1 [i_0] [i_0]);
        var_21 = ((/* implicit */unsigned short) max((var_21), (((/* implicit */unsigned short) (~((((_Bool)0) ? (9168619242695409194ULL) : (((/* implicit */unsigned long long int) ((/* implicit */int) (_Bool)1))))))))));
    }
    var_22 = ((/* implicit */int) ((unsigned int) (~((~(((/* implicit */int) var_12)))))));
}
