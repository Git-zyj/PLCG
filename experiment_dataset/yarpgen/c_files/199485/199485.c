/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199485
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199485 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199485
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = 2088277117;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            {
                var_16 = (max((min((((var_0 && (arr_1 [i_0])))), (max(var_2, 1)))), var_0));
                var_17 = (max(var_17, (arr_4 [i_0 + 2] [i_1] [i_1])));
                arr_7 [i_0] [i_0] &= (min((max((arr_0 [i_0 + 2] [i_0 + 3]), -2088277117)), ((min((arr_3 [i_1 + 1] [i_1 - 1] [i_1 - 2]), 0)))));
            }
        }
    }
    var_18 = 2088277117;
    #pragma endscop
}
