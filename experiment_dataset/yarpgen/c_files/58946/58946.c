/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 58946
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=58946 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/58946
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_21 = (((((var_14 ? 1039681701528478318 : 9986)) == (arr_3 [i_0] [i_1]))));
                arr_6 [i_1] [i_1] = 524292611;
                var_22 = (max((((arr_4 [i_0]) ? var_5 : (arr_4 [i_0]))), var_18));
                var_23 = (var_5 | var_9);
                var_24 = (((((((min(55550, 0))) || ((min(-2147483618, var_2)))))) - ((-(min(-2147483619, (arr_1 [i_1])))))));
            }
        }
    }
    #pragma endscop
}
