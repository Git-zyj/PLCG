/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 861
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=861 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/861
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 22;i_2 += 1)
            {
                {
                    arr_10 [i_0] [i_2] [i_2] = (max((((arr_4 [i_0] [i_2 - 1] [i_2]) ^ 166)), (arr_6 [i_0] [i_2 - 1] [i_2 - 1])));
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 24;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 25;i_4 += 1)
                        {
                            {
                                var_10 += max((((min(var_8, var_9)))), ((-(max(var_2, 6238169180229864123)))));
                                var_11 += (min((arr_11 [i_3] [i_3] [i_3] [1] [i_3 - 1] [i_3]), (min((arr_11 [i_1] [i_1] [i_1] [i_3] [i_3 + 1] [i_3 - 1]), var_3))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_12 = 3834354062;
    #pragma endscop
}
