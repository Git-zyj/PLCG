/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66092
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66092 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66092
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 11;i_2 += 1)
            {
                {
                    var_17 = ((((var_15 - (((max(var_6, (arr_3 [i_2]))))))) + ((max(var_2, (((arr_4 [i_0] [i_0]) ^ (arr_7 [i_0] [i_1] [i_2] [i_2]))))))));
                    var_18 = (min(var_18, (arr_3 [i_1])));
                    var_19 = (((max(-var_13, var_15)) * ((((!(arr_5 [i_0] [i_0] [i_0] [i_0])))))));
                    arr_8 [i_0] = (((max((((arr_7 [i_0] [i_0] [i_0] [i_0]) & (arr_1 [i_0]))), (max(6434, (-9223372036854775807 - 1))))) >> (((-1 + 9223372036854775807) - 9223372036854775762))));
                    var_20 = (max(var_20, (arr_2 [10])));
                }
            }
        }
    }
    var_21 |= ((-(((!(~var_3))))));
    #pragma endscop
}
