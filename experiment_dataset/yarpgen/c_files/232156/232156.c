/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 232156
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=232156 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/232156
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_17 &= (arr_3 [i_2] [i_1]);
                    var_18 &= (arr_2 [10]);
                    var_19 = ((((max((arr_3 [i_2] [i_0]), (arr_3 [i_0] [i_1])))) ? (((+(((arr_6 [i_1] [i_2]) ? (arr_1 [i_0]) : 64679))))) : (((-9223372036854775807 - 1) / (arr_3 [i_0] [i_1])))));
                    var_20 = ((((max((arr_5 [i_0] [i_2]), ((-(arr_6 [i_0] [i_1])))))) ? (((arr_2 [i_0]) * (((arr_5 [i_0] [i_1]) ? (arr_4 [i_0]) : (arr_1 [2]))))) : (((max(857, (arr_3 [i_1] [i_1])))))));
                }
            }
        }
    }
    var_21 &= var_6;
    var_22 = (max(var_22, (((13 ? (-32767 - 1) : 0)))));
    #pragma endscop
}
