/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70849
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70849 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70849
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 22;i_4 += 1)
                        {
                            {
                                arr_11 [i_0] [i_1] [11] [i_3] [11] = ((((!63) ? (~var_1) : ((var_3 ? 177 : var_6)))));
                                arr_12 [i_2] [i_2] [i_2] [i_2] [i_2] [i_2] [i_2] = (((((!(arr_4 [i_0 - 1] [i_0] [i_0 - 1] [i_0 - 1])))) % var_4));
                                arr_13 [i_0] [i_0 + 2] [i_1] [i_2] [i_0] [i_3] [i_4] = (!var_1);
                            }
                        }
                    }
                    arr_14 [i_0] [i_1] [i_2] [i_2] = ((!(!65535)));
                }
            }
        }
    }
    var_10 |= (min(-var_4, (~268435455)));
    var_11 ^= 1;
    #pragma endscop
}
