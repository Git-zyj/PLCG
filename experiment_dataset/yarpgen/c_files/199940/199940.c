/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199940
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199940 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199940
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (((max(var_6, var_12)) < var_6));
    var_19 = var_9;
    var_20 = var_10;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_21 ^= max((max(0, 5831775505381818538)), ((((arr_4 [i_0] [i_1]) ? (arr_4 [i_0] [i_1]) : (arr_1 [i_0])))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 0;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        {
                            var_22 = (arr_10 [i_1] [i_1] [i_1] [i_1]);
                            var_23 = ((!((((arr_10 [i_2 + 1] [i_2 + 1] [i_2 + 1] [i_2]) ? var_13 : var_9)))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
