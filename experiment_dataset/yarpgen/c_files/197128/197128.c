/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197128
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197128 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197128
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_7;

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_12 = (((max(((var_6 || (arr_1 [i_0] [i_0])), (min(1, 1)))))));
        var_13 = (min((arr_0 [i_0]), 1));
    }
    for (int i_1 = 0; i_1 < 18;i_1 += 1)
    {
        var_14 = ((var_10 ? ((((max(8881840349531526926, 1))) ? var_2 : (max(var_10, var_5)))) : -11));

        for (int i_2 = 2; i_2 < 16;i_2 += 1)
        {
            var_15 = ((1 ? 1420622486 : 501));
            var_16 = ((10 & (min(((12 << (35046 - 35003))), (((30489 << (50122 - 50111))))))));
            var_17 ^= (((min(var_10, (max((arr_8 [14] [14]), var_8)))) / (arr_8 [i_2] [i_2 + 1])));
        }
    }
    #pragma endscop
}
