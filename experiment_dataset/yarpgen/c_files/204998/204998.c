/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204998
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204998 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204998
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 16;i_1 += 1)
        {
            {
                var_16 = (max(var_16, (((((((arr_2 [i_1 + 3] [i_1 + 2]) ? ((var_9 ? (arr_5 [14] [i_1 + 3]) : 262143)) : (0 >= 64)))) ? var_6 : ((~(((arr_0 [i_0] [6]) * (arr_1 [i_0]))))))))));
                var_17 *= (max(((7147881629851129018 ? 33372 : (arr_6 [i_0] [i_0]))), (((!(arr_5 [i_1 - 1] [i_1 + 3]))))));
                var_18 = ((58 < (arr_4 [i_0])));
            }
        }
    }
    var_19 = var_9;
    #pragma endscop
}
