/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149139
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149139 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149139
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = 4074353249844566383;
    var_19 = var_8;

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        arr_3 [i_0] = (((((arr_1 [i_0]) | (arr_0 [i_0]))) / ((19 | ((1 ? 1 : 1))))));
        arr_4 [i_0] = (10533625970622087865 << (15288966351764504920 - 15288966351764504861));
        var_20 = ((((max(var_9, 28))) / (arr_0 [i_0])));
        var_21 ^= (((((((arr_2 [1]) + var_7)) - (arr_1 [2]))) <= (arr_2 [0])));
    }
    for (int i_1 = 0; i_1 < 11;i_1 += 1)
    {
        var_22 = (var_8 < (arr_6 [i_1] [i_1]));
        var_23 = var_6;
    }
    var_24 = var_5;
    var_25 = (min(var_25, var_12));
    #pragma endscop
}
