/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 183563
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=183563 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/183563
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (((min(-1553333155, var_6)) - (((((16 - 1) && var_2))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_1] [10] [i_2] = 1;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 11;i_4 += 1)
                        {
                            {
                                arr_13 [i_0] [i_1] [i_2] = (((1 == 2147483634) / (min((arr_1 [i_0]), -var_0))));
                                arr_14 [i_4] [0] [i_2] [i_3] [i_3] = arr_2 [i_4] [i_1] [i_1];
                                arr_15 [i_0] [i_4] = (8 < 5);
                            }
                        }
                    }
                    arr_16 [i_2] [i_0] = (arr_5 [i_0] [6] [i_2] [i_2]);
                }
            }
        }
    }
    #pragma endscop
}
