/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181926
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181926 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181926
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 4; i_0 < 12;i_0 += 1)
    {
        var_17 = (arr_0 [i_0]);
        arr_3 [i_0] = var_0;
    }
    for (int i_1 = 3; i_1 < 10;i_1 += 1)
    {
        var_18 = var_5;
        arr_7 [i_1] [i_1] = ((-((((((arr_1 [8]) | (arr_6 [i_1] [i_1]))) != (~65362))))));
        arr_8 [i_1] [i_1] = var_3;
        arr_9 [i_1] = 1;
    }
    for (int i_2 = 0; i_2 < 14;i_2 += 1)
    {
        var_19 = (6777641223870406577 >> (11669102849839145045 - 11669102849839144986));
        arr_12 [i_2] = (min(((max(var_7, (arr_0 [12])))), (((arr_1 [10]) / var_15))));
        var_20 = var_1;
    }
    var_21 = (min(var_6, (!var_14)));
    var_22 = (var_11 <= var_7);
    #pragma endscop
}
