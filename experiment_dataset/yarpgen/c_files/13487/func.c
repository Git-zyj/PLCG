/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 13487
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=13487 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/13487
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 ^= ((/* implicit */unsigned char) var_17);
    var_20 += ((/* implicit */unsigned short) var_17);
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (signed char i_1 = 0; i_1 < 16; i_1 += 3) 
        {
            for (unsigned long long int i_2 = 2; i_2 < 15; i_2 += 2) 
            {
                {
                    arr_7 [i_0] = ((((/* implicit */_Bool) ((((((/* implicit */int) (short)-21895)) + (2147483647))) << ((((-2147483647 - 1)) - ((-2147483647 - 1))))))) ? (max((min((((/* implicit */int) (short)-1)), (var_13))), (2075431731))) : (((/* implicit */int) (signed char)125)));
                    var_21 = ((/* implicit */unsigned short) (short)(-32767 - 1));
                    arr_8 [i_2] [i_0] [i_0] = ((/* implicit */unsigned int) ((short) ((((/* implicit */_Bool) arr_3 [i_2 + 1] [4ULL] [i_2 - 2])) ? (((/* implicit */int) arr_5 [i_2 + 1] [i_2] [i_2 - 2] [i_2])) : (((/* implicit */int) arr_3 [i_2 + 1] [i_2 - 1] [i_2 - 2])))));
                }
            } 
        } 
    } 
}
