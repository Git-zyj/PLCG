/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89549
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89549 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89549
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    var_18 ^= (max((arr_1 [i_0] [i_1]), (((arr_7 [0] [i_1] [i_1]) ? (arr_7 [i_2] [i_1] [i_0]) : (arr_1 [i_0] [i_1])))));
                    arr_8 [i_0] = (max(-var_3, ((((arr_0 [i_0] [i_1]) != (arr_0 [i_0] [i_0]))))));
                    var_19 = ((-1020 ? ((17 ? var_1 : ((~(arr_4 [i_1] [i_2]))))) : (arr_2 [i_2] [i_2])));
                    var_20 = (arr_5 [i_2]);
                }
            }
        }
    }
    var_21 = ((((var_2 ? 65506 : 61704))));
    #pragma endscop
}
