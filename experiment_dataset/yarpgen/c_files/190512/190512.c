/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190512
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190512 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190512
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    var_14 -= ((!(var_13 * 25441)));
                    arr_9 [i_0] [i_1] [i_2] = var_1;
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_3 = 0; i_3 < 24;i_3 += 1)
    {
        for (int i_4 = 1; i_4 < 1;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 24;i_5 += 1)
            {
                {
                    arr_18 [13] [i_4] [i_4] = (~(((arr_11 [i_5]) | 1798900791)));
                    var_15 = ((((var_10 | var_2) | (1842259545552826339 | var_0))));

                    for (int i_6 = 0; i_6 < 24;i_6 += 1)
                    {
                        var_16 ^= ((253 + (-9223372036854775807 - 1)));
                        arr_22 [i_3] [i_4 - 1] [i_5] = 14;
                    }
                    var_17 = var_8;
                }
            }
        }
    }
    var_18 = var_10;
    #pragma endscop
}
