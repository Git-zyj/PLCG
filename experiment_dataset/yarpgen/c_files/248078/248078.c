/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 248078
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=248078 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/248078
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_18 = arr_4 [i_0] [i_0] [i_0];
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            var_19 = (((((var_7 - (arr_4 [i_0] [i_0] [10])))) ? 9850297445038724153 : (!var_10)));
                            arr_9 [i_3] [i_3] [10] [i_3] [i_0] = ((63535 > ((var_11 ? -125 : (max(1, 8596446628670827486))))));
                            var_20 ^= var_5;
                        }
                    }
                }
                arr_10 [i_0] [14] = (arr_7 [i_0] [i_1] [2] [14] [i_1]);
                var_21 += ((69 ? (+-1) : (arr_1 [i_1])));
            }
        }
    }
    var_22 ^= -28638;
    var_23 = (max(56, (min(var_13, (!1982)))));
    #pragma endscop
}
