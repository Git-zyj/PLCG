/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 9948
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=9948 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/9948
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(var_14, 2733057168));

    for (int i_0 = 3; i_0 < 14;i_0 += 1)
    {
        var_15 = -3187098442521820402;
        arr_2 [i_0] [i_0] = (-3944459711906797413 ? (((9007061815787520 << 0) ? (min(201326592, 12)) : 7978816509238128975)) : (((((arr_0 [i_0] [i_0]) && (arr_1 [11]))))));
    }
    #pragma endscop
}
