/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15037
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15037 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15037
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((((((9694 < var_1) % var_7))) ? var_9 : var_3));
    var_11 = ((-((~(14951 > var_2)))));
    var_12 = var_7;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_13 = (((((~(~var_5)))) ? (((((arr_1 [i_0]) << (-37 + 82))) << (-2527667125227629650 + 2527667125227629651))) : (((arr_1 [i_0]) ? -117 : (arr_1 [i_0])))));
                arr_5 [i_0] [i_1] = ((!(((12999 ? (arr_2 [i_0 + 1]) : (arr_2 [i_0 + 1]))))));
                arr_6 [i_1] [i_1] [i_0] = 18446744073709551615;
            }
        }
    }
    #pragma endscop
}
