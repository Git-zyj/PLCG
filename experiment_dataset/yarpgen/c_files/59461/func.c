/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59461
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59461 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59461
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned char) (+(((unsigned int) ((_Bool) var_9)))));
    var_12 = ((/* implicit */unsigned long long int) var_9);
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 24; i_0 += 4) 
    {
        var_13 = ((/* implicit */short) ((_Bool) arr_1 [i_0]));
        var_14 = ((/* implicit */long long int) min(((~(((((/* implicit */_Bool) arr_2 [i_0])) ? (((/* implicit */int) (_Bool)1)) : (((/* implicit */int) arr_1 [i_0])))))), (((((((/* implicit */_Bool) arr_2 [i_0])) ? (((/* implicit */int) arr_1 [i_0])) : (((/* implicit */int) var_9)))) * (((/* implicit */int) max((arr_0 [i_0]), (((/* implicit */unsigned char) arr_2 [i_0])))))))));
        var_15 = ((/* implicit */unsigned char) var_1);
        var_16 *= ((/* implicit */unsigned long long int) var_2);
    }
    var_17 = ((/* implicit */unsigned short) max((var_17), (((/* implicit */unsigned short) var_5))));
}
