/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63194
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63194 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63194
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 += var_11;
    var_20 = (min(var_20, -var_17));
    var_21 -= (((1 - 29543) * ((((((-127 - 1) ? 29543 : 6652070618173456366)) != 32767)))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 15;i_2 += 1)
            {
                {
                    var_22 = (arr_4 [i_0] [i_0] [i_0]);
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 18;i_4 += 1)
                        {
                            {
                                var_23 = (max(var_23, ((((((1 ? 0 : 18827))) / -14)))));
                                arr_14 [i_0 + 1] [i_4] [i_2] [i_3] [i_4] = (((var_2 ^ (arr_8 [i_1 + 2] [i_1 + 3] [i_1 + 3] [i_1]))));
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 18;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 18;i_6 += 1)
                        {
                            {
                                var_24 += var_3;
                                var_25 = (min(var_25, ((((-(arr_3 [i_0 - 1])))))));
                                var_26 += ((((max(var_16, var_16))) > (~var_9)));
                                var_27 -= var_17;
                            }
                        }
                    }
                    var_28 = (max(var_28, ((max((min(var_5, 51084)), (576460752303423487 < 32763))))));
                }
            }
        }
    }
    #pragma endscop
}
