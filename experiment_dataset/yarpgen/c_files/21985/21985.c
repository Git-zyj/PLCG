/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21985
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21985 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21985
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = -89;

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_8 [i_1] [i_1] [8] = ((((arr_0 [i_2] [i_0]) ? (arr_0 [i_1] [i_0]) : (arr_7 [i_0]))));
                    var_12 = (max(((((min(65, 14909615628250930295))) && 14909615628250930311)), ((((arr_7 [i_0]) ? var_5 : var_8)))));
                    var_13 = var_6;
                }
            }
        }
        arr_9 [11] = (((~(arr_5 [i_0] [i_0] [i_0] [i_0]))));
    }
    var_14 = (~var_8);
    #pragma endscop
}
