/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24222
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24222 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24222
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((((5 >> (var_8 - 3412070774)))) ? ((0 ? (-2147483647 - 1) : 1)) : var_0));
    var_17 = 6525306699640979693;

    for (int i_0 = 2; i_0 < 17;i_0 += 1)
    {
        var_18 = (min(var_18, ((((max((((arr_2 [12] [i_0]) - (arr_1 [i_0]))), (max((arr_0 [i_0 - 2]), (arr_1 [i_0]))))) >> (!1))))));
        var_19 = ((!((((((arr_0 [2]) << 0)) >> (((arr_0 [14]) - 1383472095)))))));
        var_20 = (arr_0 [i_0]);
    }
    #pragma endscop
}
