/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234499
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234499 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234499
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        {
                            arr_9 [i_2] [i_1] [i_3] [i_0 + 1] [i_0 - 1] [i_0] = 43676;
                            arr_10 [i_0] = 63;
                            var_17 = (min(var_17, var_16));
                            arr_11 [i_3] [i_0] [i_2] [1] [i_0] [i_0] = 1;
                        }
                    }
                }
                var_18 = (min(var_18, var_12));
                /* LoopNest 3 */
                for (int i_4 = 0; i_4 < 1;i_4 += 1)
                {
                    for (int i_5 = 3; i_5 < 14;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 15;i_6 += 1)
                        {
                            {
                                arr_18 [i_4] [i_0] [i_6] = (arr_17 [i_0 - 1] [i_5 + 1] [i_5] [i_5 - 2] [9]);
                                arr_19 [i_0] [i_0] [i_1] [i_4] [i_0] [i_4] = (arr_15 [i_0] [i_0 - 1] [i_5] [i_5]);
                                arr_20 [i_0] [i_1] [i_0] [i_0] [i_0] = var_7;
                            }
                        }
                    }
                }
            }
        }
    }
    var_19 = var_0;
    #pragma endscop
}
