/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46903
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46903 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46903
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 *= 33;
    var_20 = var_11;

    for (int i_0 = 3; i_0 < 19;i_0 += 1) /* same iter space */
    {
        var_21 -= (arr_1 [10] [10]);
        var_22 = (max(var_22, var_11));
    }
    for (int i_1 = 3; i_1 < 19;i_1 += 1) /* same iter space */
    {
        arr_5 [i_1 + 1] [1] = ((!((max((arr_2 [i_1 + 1]), var_9)))));
        /* LoopNest 2 */
        for (int i_2 = 2; i_2 < 18;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 20;i_3 += 1)
            {
                {
                    var_23 = (((arr_4 [i_1 - 2]) ? ((((arr_11 [8] [i_1] [i_1 - 1] [i_1]) & (arr_6 [i_2 + 2])))) : (4257269311 / 15349275894119809477)));
                    var_24 = (((min((-9857 + -45392466), (arr_9 [i_2 - 1] [i_2 - 1] [i_1 - 1] [i_1]))) + var_0));
                }
            }
        }
        var_25 = (min(var_25, ((((arr_10 [i_1 - 2] [i_1 - 1] [i_1 - 1]) ? (106 || 37697954) : (((!(arr_10 [i_1 - 1] [i_1 - 2] [i_1 - 2])))))))));
    }
    var_26 = 9469;
    #pragma endscop
}
