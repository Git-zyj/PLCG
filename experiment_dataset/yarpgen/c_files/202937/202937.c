/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202937
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202937 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202937
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((6324764899401496678 + var_1) ? (((-var_13 + 9223372036854775807) >> -var_17)) : (((((~var_11) >= (~60))))));
    var_19 = (var_8 - 45);
    var_20 = (max(var_20, (((((59 * var_12) / var_10))))));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] = (((max(60, (((-60 + (arr_5 [i_0 - 2] [i_1] [i_2])))))) != 255));
                    var_21 = (!61702);
                }
            }
        }
    }
    var_22 = var_2;
    #pragma endscop
}
