/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130127
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130127 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130127
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((((((((var_10 ? 5674438306159766812 : var_8))) ? var_6 : (((min(var_12, var_14))))))) ? (~4569) : var_11));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            {
                var_19 = (((((-1049998987 != 37) & (((arr_0 [i_1] [i_0]) - (arr_3 [i_0] [i_1])))))) ? (((((-(arr_2 [i_0])))) ? -32745 : var_12)) : (19 < var_15));
                var_20 *= ((19 ? 18295 : ((min(62, -62467476)))));
            }
        }
    }
    #pragma endscop
}
