/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133496
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133496 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133496
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((((-var_8 ? 49327 : (~var_8))) >> (((max((max(var_13, var_18)), var_4)) - 351804207))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_4 [i_0 + 1] [i_1] [i_1] = (arr_0 [i_1]);
                var_20 = (min(var_20, ((max((arr_2 [i_0]), ((-(arr_3 [16]))))))));
            }
        }
    }
    var_21 = (max(var_21, (((~(min((~var_17), (min(var_14, var_13)))))))));
    #pragma endscop
}
