/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 161743
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=161743 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/161743
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 11; i_1 += 2) 
        {
            for (unsigned int i_2 = 3; i_2 < 10; i_2 += 1) 
            {
                {
                    var_15 -= ((/* implicit */unsigned long long int) arr_4 [i_2 - 2] [i_0] [i_2 - 3]);
                    var_16 += ((/* implicit */_Bool) ((((/* implicit */long long int) 4294967295U)) / (arr_2 [i_2])));
                    arr_6 [i_2] = ((((/* implicit */_Bool) 1898626637543651924LL)) ? (((/* implicit */int) (signed char)-117)) : (-1072804666));
                }
            } 
        } 
    } 
    var_17 = ((/* implicit */unsigned char) var_13);
}
