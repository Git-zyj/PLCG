/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91787
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91787 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91787
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 -= var_10;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_21 = (arr_1 [i_0] [i_0]);
                var_22 = (max(var_22, ((((((~var_9) ? ((var_7 ? 1 : var_7)) : (arr_4 [i_1])))) ? ((((((arr_2 [i_0] [i_1]) << (var_15 - 126))) - (((-14447 + 2147483647) >> (((arr_4 [i_0]) - 12321))))))) : (arr_3 [i_0])))));
            }
        }
    }
    var_23 = var_12;
    var_24 = (max(var_6, (min((var_7 / var_3), -4456387821440728838))));
    #pragma endscop
}
