/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212609
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212609 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212609
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_1] = (max(var_8, (arr_0 [i_0])));
                arr_6 [i_0] [i_1] [i_1] = (arr_1 [i_0] [i_0 - 2]);
            }
        }
    }
    var_11 ^= var_1;
    var_12 = (max(26954, (!var_2)));
    var_13 = (max((((((min(var_3, -5363)))) == ((var_4 ? var_2 : var_6)))), (3128439232 == -124)));
    #pragma endscop
}
