/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 118405
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=118405 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/118405
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((((min(27736, var_1))) & var_13));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_15 = (~(((!(((34640 ? 2599 : 107)))))));

                for (int i_2 = 3; i_2 < 17;i_2 += 1)
                {
                    arr_8 [i_0] [i_0] [i_2 + 1] = (min((((((-(arr_1 [i_2]))) == (var_2 > 127)))), (arr_4 [i_0] [i_0] [i_0])));
                    arr_9 [i_2] [i_1] [i_0] = ((-((+((((arr_6 [i_1] [15] [i_1]) && var_9)))))));
                }
            }
        }
    }
    var_16 = ((+(((127 ^ 0) << (var_13 - 58)))));
    #pragma endscop
}
