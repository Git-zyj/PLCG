/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160318
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160318 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160318
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            arr_10 [i_0 + 2] [i_1] = (arr_3 [i_0] [i_0] [i_2]);
                            arr_11 [i_0 + 1] [i_1] [i_1] [i_2] [i_3] = ((!((((arr_7 [i_0 + 3] [i_0 - 1]) ? (arr_7 [i_0 - 1] [i_0 - 1]) : (arr_7 [i_0 + 4] [i_0 + 3]))))));
                        }
                    }
                }
                var_10 = -3174;
                var_11 = (min(var_11, ((max(((-(arr_2 [i_0 + 2] [i_0]))), (((~var_8) ? ((max((arr_7 [i_0] [i_1]), (arr_9 [i_0] [i_1])))) : (((149 || (arr_6 [i_1] [i_1] [4] [9])))))))))));
            }
        }
    }
    var_12 ^= (((min(var_3, var_6))));
    #pragma endscop
}
