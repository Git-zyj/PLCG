/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162360
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162360 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162360
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_0 = 0; i_0 < 24; i_0 += 1) 
    {
        var_17 = ((/* implicit */unsigned long long int) (-(((/* implicit */int) arr_0 [i_0]))));
        var_18 = ((/* implicit */long long int) arr_0 [i_0]);
        var_19 = ((/* implicit */unsigned short) var_10);
    }
    var_20 = ((/* implicit */unsigned long long int) max((var_20), (((/* implicit */unsigned long long int) (~(((/* implicit */int) (unsigned char)98)))))));
    var_21 = ((/* implicit */unsigned char) var_12);
    var_22 = ((unsigned long long int) min((((/* implicit */int) ((((/* implicit */unsigned int) ((/* implicit */int) (short)26846))) == (4099558775U)))), ((-(0)))));
    var_23 &= ((/* implicit */long long int) (-(((((/* implicit */_Bool) max((((/* implicit */unsigned int) var_6)), (814476685U)))) ? (var_2) : (((/* implicit */unsigned long long int) ((/* implicit */int) var_1)))))));
}
