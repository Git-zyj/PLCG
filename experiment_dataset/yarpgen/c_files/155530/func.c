/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155530
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155530 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155530
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned char) max((var_11), (((/* implicit */unsigned char) ((((/* implicit */int) var_10)) / ((~(((/* implicit */int) var_5)))))))));
    var_12 = ((/* implicit */unsigned long long int) (+(((/* implicit */int) (unsigned char)48))));
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 3; i_0 < 19; i_0 += 3) 
    {
        var_13 = ((/* implicit */unsigned char) (~(((/* implicit */int) arr_1 [i_0]))));
        arr_2 [i_0] [i_0] = ((/* implicit */unsigned short) ((unsigned int) arr_1 [i_0]));
    }
}
