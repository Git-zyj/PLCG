/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91616
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91616 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91616
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 *= var_8;
    var_18 = 1;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0] = var_0;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {

                            /* vectorizable */
                            for (int i_4 = 0; i_4 < 15;i_4 += 1)
                            {
                                var_19 = ((!(arr_12 [i_3] [i_0 + 2])));
                                arr_14 [i_0 + 1] [i_1] [i_3] [i_4] = ((var_15 ? -7785 : (arr_0 [i_1])));
                                var_20 += ((-((((arr_12 [i_0] [1]) != (arr_5 [i_1]))))));
                            }
                            var_21 ^= (arr_2 [i_1] [0]);
                        }
                    }
                }
                var_22 = var_16;
                var_23 = ((((max((arr_12 [i_0 - 2] [i_1]), (arr_10 [i_1])))) ? (arr_8 [i_1] [i_0 - 2]) : (((max((arr_11 [i_0 - 1]), (arr_10 [i_1])))))));
            }
        }
    }
    #pragma endscop
}
