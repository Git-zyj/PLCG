/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247312
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247312 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247312
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, ((min(var_4, var_18)))));
    var_20 = (~(~-13));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_21 = (arr_3 [i_0] [i_0] [i_0]);
                arr_5 [i_1] = (max(var_6, (((arr_0 [i_0] [i_1]) ? (arr_4 [i_0] [10]) : (arr_4 [i_0] [i_1])))));
                arr_6 [i_0] [i_0] [i_0] = ((+(((arr_4 [i_0] [i_1]) ? (arr_4 [i_1] [i_1]) : (arr_4 [i_0] [i_1])))));
            }
        }
    }
    #pragma endscop
}
