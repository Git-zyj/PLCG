/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249388
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 15; i_0 += 4) 
    {
        for (unsigned char i_1 = 1; i_1 < 14; i_1 += 2) 
        {
            {
                var_12 *= ((/* implicit */unsigned char) arr_4 [i_0]);
                var_13 &= ((/* implicit */unsigned char) var_6);
                var_14 = ((/* implicit */unsigned int) (~((+(arr_0 [i_1 - 1])))));
            }
        } 
    } 
    var_15 = ((/* implicit */int) var_2);
    var_16 = ((/* implicit */_Bool) var_4);
    var_17 = ((/* implicit */unsigned short) ((((/* implicit */_Bool) var_5)) ? (((/* implicit */long long int) ((/* implicit */int) var_0))) : (842334222634090549LL)));
    var_18 = ((/* implicit */signed char) min((((/* implicit */long long int) ((int) (~(var_8))))), ((~(max((((/* implicit */long long int) var_2)), (842334222634090561LL)))))));
}
