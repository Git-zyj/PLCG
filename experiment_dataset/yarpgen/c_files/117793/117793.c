/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117793
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117793 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117793
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_4;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_13 = ((var_3 ? ((((!(-9223372036854775807 - 1))) ? var_3 : var_5)) : ((((((3569312066 | (arr_0 [i_1] [2]))) == (((max(9, (arr_2 [i_0] [i_1])))))))))));
                arr_4 [i_0] [i_1] = ((((((((arr_0 [i_0] [i_1]) ? (arr_1 [4]) : var_10))) ? (var_3 == -1593522240) : -31)) <= var_6));
            }
        }
    }
    #pragma endscop
}
