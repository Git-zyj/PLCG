/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 58004
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=58004 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/58004
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (signed char i_0 = 1; i_0 < 16; i_0 += 2) 
    {
        for (unsigned int i_1 = 4; i_1 < 15; i_1 += 2) 
        {
            {
                var_13 = ((/* implicit */unsigned int) var_0);
                var_14 |= ((/* implicit */unsigned long long int) var_10);
            }
        } 
    } 
    var_15 = ((/* implicit */unsigned char) min((((long long int) min((var_1), (var_1)))), (((/* implicit */long long int) ((((/* implicit */_Bool) var_11)) && (((/* implicit */_Bool) min((((/* implicit */long long int) (_Bool)1)), (var_4)))))))));
    var_16 = ((/* implicit */signed char) (_Bool)1);
    var_17 = ((/* implicit */unsigned char) var_6);
    var_18 = ((/* implicit */int) ((var_8) < (((/* implicit */unsigned int) max((((/* implicit */int) (!(((/* implicit */_Bool) var_1))))), ((~(((/* implicit */int) var_0)))))))));
}
