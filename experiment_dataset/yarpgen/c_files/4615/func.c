/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4615
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4615 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4615
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned short i_0 = 0; i_0 < 15; i_0 += 3) 
    {
        for (int i_1 = 3; i_1 < 14; i_1 += 4) 
        {
            for (unsigned char i_2 = 2; i_2 < 11; i_2 += 4) 
            {
                {
                    var_14 |= ((/* implicit */long long int) ((((/* implicit */_Bool) (+(((/* implicit */int) (short)14905))))) && (((/* implicit */_Bool) ((((/* implicit */_Bool) var_4)) ? (((/* implicit */int) (short)-14902)) : (((/* implicit */int) var_9)))))));
                    var_15 *= ((/* implicit */unsigned char) (+(max(((+(((/* implicit */int) (short)14905)))), (((((/* implicit */int) var_10)) << (((/* implicit */int) var_2))))))));
                }
            } 
        } 
    } 
    var_16 *= ((/* implicit */unsigned int) var_6);
}
