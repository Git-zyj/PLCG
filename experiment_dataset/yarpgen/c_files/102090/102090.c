/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102090
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102090 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102090
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_1] = (arr_4 [i_1]);
                var_18 = (((arr_0 [i_0] [i_0 + 1]) ? (((arr_0 [i_0 - 1] [i_0 - 1]) ? (arr_0 [i_0] [i_0 - 3]) : (arr_0 [i_0] [i_0 + 1]))) : ((-(arr_0 [i_0] [i_0 - 2])))));
                var_19 = (arr_0 [i_0] [i_0 + 1]);
                var_20 = -1;
            }
        }
    }
    var_21 = (((var_15 ? (min(var_15, var_9)) : var_8)));
    #pragma endscop
}
