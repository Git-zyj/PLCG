/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33726
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33726 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33726
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_14;
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_20 ^= (min(((((arr_0 [i_0 - 1]) < (arr_0 [i_0 - 3])))), ((var_2 ? var_15 : -27260))));
                    var_21 = var_7;
                    var_22 ^= (arr_6 [i_0] [i_0 + 2] [i_0]);
                    var_23 = (min(var_23, (((((min((arr_6 [i_0] [i_0] [i_2]), (arr_6 [i_0] [i_2] [i_0])))) ? (arr_0 [i_0 - 3]) : ((((arr_0 [i_0 - 4]) < (arr_6 [i_0] [i_0] [i_0])))))))));
                }
            }
        }
    }
    #pragma endscop
}
