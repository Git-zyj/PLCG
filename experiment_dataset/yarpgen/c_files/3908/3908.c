/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3908
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3908 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3908
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_16 = (((arr_3 [i_0] [i_1]) ? (max((arr_1 [i_0] [i_0]), 1)) : 18446744073709551615));
                var_17 *= (arr_4 [i_1] [i_0]);
            }
        }
    }
    var_18 = (((var_10 / var_1) ? (~3560376027) : ((((max(734591294, 3560376022))) ? ((52650 ? 383300142 : 734591268)) : (27309 ^ 113)))));
    #pragma endscop
}
