/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 176334
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=176334 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/176334
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 -= var_14;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        arr_4 [3] [i_0] = (((146 != -21033) + 0));
        var_16 = ((14539563857546418740 ? -48 : -9007643807060083764));

        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            var_17 = (var_12 & var_3);
            arr_8 [i_1] [i_0] = (((((60 != (arr_5 [i_0])))) >> (!3453)));
            var_18 = (max(var_18, (((21893 ? 21040 : 38749)))));
            arr_9 [i_0] [i_0] = var_0;
        }
        for (int i_2 = 0; i_2 < 19;i_2 += 1)
        {

            for (int i_3 = 0; i_3 < 19;i_3 += 1)
            {
                var_19 = (arr_14 [i_0] [i_0]);
                var_20 = (max(var_20, (((~(~var_2))))));
            }
            var_21 = (((arr_3 [i_0]) && var_13));
        }
    }
    var_22 &= ((~(var_8 | var_2)));
    #pragma endscop
}
