/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177204
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177204 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177204
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 18;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_0] [i_0] [i_0] = var_10;
                    arr_8 [i_2] [i_2] [i_2] [i_2] = ((~(((!(arr_1 [i_2 - 4]))))));
                    arr_9 [i_2] [i_0] [i_0] [i_0] = 0;

                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        arr_14 [i_0] [i_0] [i_0] [i_0] [i_0] = (arr_13 [i_0] [i_0] [i_0]);
                        arr_15 [i_1] [i_1] [i_1] [i_1] [i_1] [i_1] = (!((!(arr_6 [i_0]))));
                    }
                }
            }
        }
    }
    var_12 = var_3;
    #pragma endscop
}
