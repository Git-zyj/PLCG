/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71598
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71598 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71598
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_0 = 4; i_0 < 9; i_0 += 4) 
    {
        arr_2 [10ULL] [i_0 + 3] = (+((~(((/* implicit */int) (signed char)1)))));
        arr_3 [i_0] [i_0] = ((/* implicit */unsigned short) ((((_Bool) (-2147483647 - 1))) ? (((/* implicit */long long int) ((var_3) / (((/* implicit */int) arr_1 [i_0 + 1]))))) : (arr_0 [i_0] [i_0 - 2])));
    }
    var_19 = ((/* implicit */signed char) (~((~((~(var_6)))))));
}
