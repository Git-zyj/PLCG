/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111449
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111449 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111449
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        {
                            var_13 = (min(var_13, -5375603951985783380));
                            arr_13 [i_0] [i_1 - 1] [i_1] [i_1] [i_2] [7] = (arr_3 [14]);
                        }
                    }
                }
                var_14 = arr_6 [i_1];
                var_15 = (arr_1 [i_0 - 1]);
            }
        }
    }
    var_16 = (min(var_16, var_11));
    var_17 = (max(var_17, (min(var_1, (max(var_8, var_2))))));
    var_18 = (min(var_18, (((((min(-5694693038708738891, ((16384 ? 16384 : 3448051591032908094))))) ? -16399 : 91)))));
    #pragma endscop
}
