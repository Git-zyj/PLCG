/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 8318
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=8318 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/8318
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 ^= ((1 ? 26548 : 768));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_19 = (max((((arr_2 [i_0 - 3]) ? ((511 ? 38988 : 4283083142)) : (((var_15 ? (arr_3 [i_0] [i_1]) : (arr_1 [13])))))), ((((arr_1 [i_0 - 3]) % ((~(arr_2 [i_0]))))))));
                var_20 = ((((((arr_2 [i_1]) ? ((var_10 ? (arr_1 [i_0]) : var_1)) : (arr_0 [i_0 - 2])))) ? var_5 : (((((var_8 ? var_17 : 23692))) ? (max((arr_4 [i_0 - 1] [i_1]), var_5)) : var_1))));
            }
        }
    }
    #pragma endscop
}
