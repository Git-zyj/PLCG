/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100366
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100366 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100366
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((((min(63, 1))) ^ (min(var_10, var_3))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_14 = var_4;
        arr_4 [i_0] &= (((arr_3 [i_0]) ^ ((var_0 ? 1984 : var_4))));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 11;i_2 += 1)
            {
                {
                    var_15 = -1;
                    arr_10 [i_2] = ((35465847065542656 ? 0 : -27310));
                }
            }
        }
        /* LoopNest 2 */
        for (int i_3 = 0; i_3 < 14;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 14;i_4 += 1)
            {
                {
                    var_16 *= ((var_3 | ((var_11 ? var_3 : var_12))));
                    var_17 += ((var_6 ? var_10 : (arr_9 [i_0] [8] [i_3] [i_4])));
                    arr_15 [i_0] [i_0] [4] &= (var_6 != var_10);
                    var_18 = var_1;
                }
            }
        }
        arr_16 [i_0] = (~0);
    }
    var_19 *= var_6;
    #pragma endscop
}
