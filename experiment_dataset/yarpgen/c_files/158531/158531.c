/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158531
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158531 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158531
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_12 = (((min((var_6 << var_5), (var_4 | var_9))) / (((((min(var_6, var_3)) > (!var_1)))))));
        var_13 = ((-var_11 + ((var_1 - (var_4 * var_1)))));
    }
    var_14 = (min(var_14, ((var_9 >> ((((max((max(-79, 56694)), (!3899350163)))) - 65451))))));
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 15;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 15;i_2 += 1)
        {
            {
                var_15 = (max((78 && 85), var_1));
                var_16 = (((max((var_7 + var_6), (88 - 12056))) + -var_11));
            }
        }
    }
    var_17 = (max(var_17, var_10));
    #pragma endscop
}
