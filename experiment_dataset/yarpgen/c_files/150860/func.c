/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150860
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150860 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150860
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned short i_0 = 0; i_0 < 10; i_0 += 4) 
    {
        for (unsigned char i_1 = 0; i_1 < 10; i_1 += 2) 
        {
            for (signed char i_2 = 0; i_2 < 10; i_2 += 2) 
            {
                {
                    var_15 = ((/* implicit */_Bool) (unsigned char)31);
                    var_16 = ((/* implicit */unsigned int) arr_2 [i_0] [i_0] [i_0]);
                    var_17 = ((/* implicit */short) (~((~(539911830)))));
                }
            } 
        } 
    } 
    var_18 = ((_Bool) ((((/* implicit */_Bool) ((539911824) >> (((var_4) - (9383134856197358181ULL)))))) ? (((/* implicit */int) max(((unsigned char)8), ((unsigned char)248)))) : (((/* implicit */int) ((unsigned short) (unsigned short)16060)))));
}
