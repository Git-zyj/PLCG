/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85681
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85681 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85681
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 23; i_0 += 4) 
    {
        arr_4 [i_0] = arr_2 [i_0];
        arr_5 [i_0] = ((/* implicit */unsigned short) ((signed char) ((unsigned short) arr_1 [i_0] [i_0])));
    }
    var_14 = ((/* implicit */short) max((var_14), (((/* implicit */short) var_9))));
    var_15 = ((/* implicit */unsigned short) ((((/* implicit */_Bool) min((8387584), (-8387591)))) ? (1369941196596326393ULL) : (((/* implicit */unsigned long long int) ((/* implicit */int) (signed char)-88)))));
}
