/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206234
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206234 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206234
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min(var_10, var_4));

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_11 = (min(var_11, (((((((-15123 == (var_3 - var_7))))) / (min((arr_0 [i_0]), (((arr_0 [i_0]) ? (arr_0 [i_0]) : (arr_0 [i_0]))))))))));
        arr_2 [i_0] [i_0] = (((((((~(arr_1 [i_0])))) >= ((15113 ? (arr_0 [i_0]) : var_8)))) ? 8064 : (max((arr_1 [i_0]), (arr_0 [i_0])))));
        var_12 = ((((max((arr_0 [i_0]), var_3))) ? (((arr_0 [i_0]) ? (arr_0 [i_0]) : (arr_0 [i_0]))) : (((arr_0 [i_0]) ? (arr_0 [i_0]) : (arr_0 [i_0])))));
    }
    var_13 = var_9;
    #pragma endscop
}
