/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45705
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45705 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45705
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */short) var_5);
    /* LoopSeq 1 */
    for (unsigned short i_0 = 2; i_0 < 18; i_0 += 2) 
    {
        arr_2 [i_0] = (+(((((/* implicit */_Bool) (~(((/* implicit */int) var_15))))) ? (((/* implicit */unsigned int) ((((/* implicit */_Bool) var_2)) ? (var_6) : (((/* implicit */int) var_4))))) : (var_2))));
        /* LoopNest 2 */
        for (signed char i_1 = 1; i_1 < 20; i_1 += 3) 
        {
            for (unsigned long long int i_2 = 1; i_2 < 20; i_2 += 1) 
            {
                {
                    var_20 = ((/* implicit */unsigned long long int) ((unsigned int) ((signed char) arr_5 [i_0 - 1])));
                    /* LoopSeq 1 */
                    for (short i_3 = 0; i_3 < 21; i_3 += 2) 
                    {
                        var_21 = ((/* implicit */unsigned short) min((var_21), (((/* implicit */unsigned short) var_9))));
                        arr_11 [i_0] [i_1 + 1] [i_2 + 1] [i_3] = ((/* implicit */unsigned int) (+(((((/* implicit */int) var_17)) / (var_0)))));
                        arr_12 [i_3] [i_2] [(short)14] [i_1] [i_0] = ((/* implicit */unsigned int) var_0);
                    }
                }
            } 
        } 
        var_22 ^= ((/* implicit */unsigned long long int) (-(max((((/* implicit */int) var_8)), ((~(((/* implicit */int) arr_10 [i_0 - 1] [11U] [i_0 - 1] [i_0 - 2]))))))));
    }
}
