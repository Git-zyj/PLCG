/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6563
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6563 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6563
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_4;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0 + 1] [i_1] = (((((!((min(109, 134)))))) * (!0)));
                arr_7 [16] [16] [16] |= (min((((arr_5 [i_0 - 1]) ? (~var_0) : ((var_10 ? 109 : var_6)))), ((((var_14 ? var_1 : var_1))))));
                var_16 = (min((1 | 18446744073709551615), ((min(var_3, (arr_2 [i_1] [i_0 + 1]))))));
            }
        }
    }
    #pragma endscop
}
