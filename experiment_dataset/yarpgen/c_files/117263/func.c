/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117263
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117263 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117263
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned short) var_14);
    var_17 = ((/* implicit */unsigned short) var_14);
    /* LoopNest 2 */
    for (unsigned char i_0 = 0; i_0 < 10; i_0 += 2) 
    {
        for (_Bool i_1 = 0; i_1 < 0; i_1 += 1) 
        {
            {
                var_18 = ((/* implicit */long long int) max((var_18), (((/* implicit */long long int) arr_1 [2] [i_1 + 1]))));
                /* LoopNest 3 */
                for (long long int i_2 = 0; i_2 < 10; i_2 += 4) 
                {
                    for (unsigned char i_3 = 2; i_3 < 7; i_3 += 3) 
                    {
                        for (long long int i_4 = 1; i_4 < 9; i_4 += 4) 
                        {
                            {
                                arr_12 [i_2] [i_2] [i_2] [i_0] &= ((/* implicit */unsigned int) (+((~(((/* implicit */int) var_0))))));
                                var_19 = ((/* implicit */short) var_6);
                                var_20 *= ((/* implicit */unsigned char) (-(((/* implicit */int) var_14))));
                            }
                        } 
                    } 
                } 
                /* LoopNest 2 */
                for (_Bool i_5 = 1; i_5 < 1; i_5 += 1) 
                {
                    for (int i_6 = 1; i_6 < 8; i_6 += 3) 
                    {
                        {
                            arr_18 [i_0] [i_0] [i_0] [i_0] &= ((/* implicit */unsigned short) var_13);
                            var_21 = ((/* implicit */unsigned char) arr_3 [i_0] [i_1] [(unsigned short)5]);
                        }
                    } 
                } 
            }
        } 
    } 
}
