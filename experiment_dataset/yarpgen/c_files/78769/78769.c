/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78769
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78769 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78769
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = 46694149;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_0] = (((((~(arr_4 [i_0] [i_1])))) ? (((arr_4 [i_1] [i_1]) ^ (arr_4 [i_0] [i_1]))) : (max((arr_4 [i_1] [i_1]), 0))));
                /* LoopNest 3 */
                for (int i_2 = 1; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 14;i_4 += 1)
                        {
                            {
                                arr_14 [i_2] [i_1] [i_2] [i_1] [i_2] = -695998167789806741;
                                var_11 = (max(var_11, var_6));
                                var_12 = (max(var_12, 0));
                            }
                        }
                    }
                }
                arr_15 [i_1] [i_0] = 65531;
                arr_16 [i_0] [i_1] [i_1] = (arr_7 [i_0] [i_0] [i_1] [10]);
            }
        }
    }
    #pragma endscop
}
