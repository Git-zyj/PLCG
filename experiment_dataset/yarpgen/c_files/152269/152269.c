/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152269
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152269 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152269
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_5;
    var_12 ^= (((max((!63161), (var_4 && var_9))) ? (var_7 / var_9) : (((-127 >= var_0) ? var_7 : (1 - 108)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_13 = (((((arr_2 [i_1] [i_0] [i_0]) ? var_8 : var_2)) | ((var_10 ? 11135 : (((arr_1 [7]) & var_8))))));
                var_14 &= ((2048 ? (arr_1 [i_1]) : var_7));
                arr_4 [i_0] = arr_0 [i_0] [i_0];
                arr_5 [i_0] [i_0] = ((((max((arr_2 [i_1] [i_1] [i_0]), var_7))) || ((!(arr_2 [i_1] [i_0] [i_0])))));
            }
        }
    }
    var_15 = var_7;
    #pragma endscop
}
