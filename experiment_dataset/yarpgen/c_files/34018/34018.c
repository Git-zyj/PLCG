/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34018
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34018 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34018
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                arr_6 [i_1] = (((((var_7 ? -1 : (arr_3 [i_1 - 1] [i_1 - 1])))) ? (min((arr_3 [i_1 - 1] [i_1 - 1]), (arr_4 [i_1 - 1] [i_1]))) : ((min((arr_5 [i_1 - 1] [i_1]), (arr_5 [i_1 - 1] [i_1]))))));
                arr_7 [i_1] = (max((~6716), -20681));
            }
        }
    }
    var_14 = (min(252, 9845301880812888136));
    var_15 = var_0;
    #pragma endscop
}
