/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214032
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214032 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214032
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (~2011618570);
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 16;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 3; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 18;i_4 += 1)
                        {
                            {
                                arr_12 [10] [i_2] [i_3] [1] [i_1] [i_2] [i_0] = (!((!((!(arr_6 [i_0] [i_1] [i_2] [i_2]))))));
                                var_19 = 4229318047640160165;
                            }
                        }
                    }
                }
                arr_13 [i_0] [2] = (arr_8 [i_0 - 3] [1] [1] [i_0 - 3] [16]);
            }
        }
    }
    var_20 = ((max((max(var_3, -2133547419), (max(1262551529, var_5))))));
    var_21 = var_9;
    #pragma endscop
}
