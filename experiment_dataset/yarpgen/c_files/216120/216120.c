/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 216120
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=216120 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/216120
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(var_16, -var_1));
    var_17 = var_2;
    var_18 = (((((-1 ? var_9 : var_10)))) != ((9223372036854775807 ? ((var_1 ? var_3 : var_9)) : var_0)));

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        var_19 = (min(((-1 ? -1 : 90405720)), ((min((arr_0 [i_0] [i_0]), (arr_0 [i_0] [i_0]))))));
        arr_3 [i_0] [i_0] = (((min(var_6, (((var_3 ? var_7 : 3739900779))))) % (((((var_9 ? var_4 : 555066492)))))));
        var_20 = (((((((((arr_1 [i_0]) || 1)))) ^ (arr_2 [i_0] [i_0]))) == ((((arr_2 [i_0] [i_0]) ? var_3 : 1)))));
    }
    #pragma endscop
}
