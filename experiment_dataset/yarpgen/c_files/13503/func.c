/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 13503
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=13503 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/13503
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 0; i_0 < 17; i_0 += 2) 
    {
        var_16 = var_3;
        /* LoopNest 3 */
        for (unsigned char i_1 = 1; i_1 < 16; i_1 += 3) 
        {
            for (unsigned int i_2 = 0; i_2 < 17; i_2 += 1) 
            {
                for (unsigned char i_3 = 1; i_3 < 16; i_3 += 1) 
                {
                    {
                        arr_10 [i_0] [i_0] [i_0] [11] [i_2] [i_1] = ((/* implicit */signed char) (-(min((5501669093477791176ULL), (((/* implicit */unsigned long long int) min((16777212U), (var_6))))))));
                        var_17 = ((/* implicit */_Bool) (-(((arr_4 [i_3 + 1] [i_1 + 1] [i_1 + 1]) ? (((/* implicit */int) arr_4 [i_3 - 1] [i_1 - 1] [i_1 + 1])) : (arr_9 [i_1 + 1] [i_1 + 1] [i_3 - 1] [i_3])))));
                    }
                } 
            } 
        } 
    }
    var_18 -= ((/* implicit */int) var_10);
    var_19 = ((/* implicit */unsigned int) max((var_19), (((/* implicit */unsigned int) var_5))));
}
