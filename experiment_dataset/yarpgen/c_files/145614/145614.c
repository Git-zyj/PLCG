/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 145614
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=145614 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/145614
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_12 = ((((((max((arr_0 [i_0]), (arr_0 [i_0])))) < (~1))) ? (((((((arr_1 [i_0] [i_0]) ? (arr_1 [i_0] [i_0]) : (arr_0 [i_0])))) ? (arr_1 [i_0] [i_0]) : (min((arr_1 [i_0] [1]), (arr_0 [1])))))) : (~-772190522383872465)));
        var_13 ^= ((~(arr_0 [i_0])));
    }
    for (int i_1 = 4; i_1 < 14;i_1 += 1)
    {
        arr_4 [i_1] [i_1] = ((var_1 ? (~var_3) : (max(209, ((~(arr_2 [15])))))));
        var_14 ^= -7;
    }
    for (int i_2 = 0; i_2 < 17;i_2 += 1)
    {
        var_15 = 10483502941651967917;
        arr_8 [i_2] = (arr_6 [i_2] [i_2]);
        var_16 = (min(var_16, var_4));
        var_17 |= (arr_6 [i_2] [i_2]);
    }
    var_18 = 1;
    #pragma endscop
}
