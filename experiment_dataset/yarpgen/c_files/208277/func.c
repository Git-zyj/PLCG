/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208277
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208277 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208277
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 -= ((/* implicit */signed char) ((int) var_9));
    var_13 = var_0;
    /* LoopSeq 2 */
    for (unsigned char i_0 = 4; i_0 < 22; i_0 += 3) 
    {
        var_14 ^= ((/* implicit */_Bool) var_6);
        var_15 *= ((/* implicit */unsigned char) ((_Bool) (-(((/* implicit */int) arr_1 [i_0])))));
    }
    /* vectorizable */
    for (signed char i_1 = 0; i_1 < 18; i_1 += 1) 
    {
        arr_4 [i_1] = (!(((/* implicit */_Bool) ((signed char) arr_1 [i_1]))));
        var_16 = ((/* implicit */unsigned int) ((((((/* implicit */int) (_Bool)0)) | (((/* implicit */int) (_Bool)1)))) >> (((/* implicit */int) arr_2 [i_1] [i_1]))));
    }
}
