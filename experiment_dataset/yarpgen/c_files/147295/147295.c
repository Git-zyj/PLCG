/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 147295
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=147295 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/147295
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min((-1 + 7759271575549920328), (min(var_9, (~7759271575549920328)))));

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        arr_2 [i_0] [10] = (((arr_1 [i_0]) ? ((((((min(var_6, var_12))) || 342216123)))) : (min(var_16, (max(3594831458, 52768))))));
        arr_3 [i_0] [i_0] = var_11;
        arr_4 [i_0] [i_0] = ((min(var_6, 7759271575549920310)));
        arr_5 [12] = ((min(var_2, (var_4 < var_6))));
    }
    #pragma endscop
}
