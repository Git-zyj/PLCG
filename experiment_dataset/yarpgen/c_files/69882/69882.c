/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 69882
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=69882 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/69882
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 16;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    var_14 += ((~(((!(arr_7 [i_1 - 2] [i_0] [i_1 - 1]))))));
                    arr_9 [i_1] [i_1 - 4] [i_2] [i_2] = var_10;
                }

                /* vectorizable */
                for (int i_3 = 1; i_3 < 16;i_3 += 1)
                {
                    arr_12 [i_1] [i_1] = (((((arr_4 [i_0] [i_1] [i_0]) ? var_1 : (arr_2 [i_0]))) / (arr_2 [i_3 - 1])));
                    var_15 = 237;
                    arr_13 [i_1] [i_1 - 2] [i_0] [i_1] = (arr_11 [i_1] [i_1]);
                }
            }
        }
    }
    var_16 = var_9;
    #pragma endscop
}
