/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 79545
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=79545 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/79545
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(var_15, ((min(((!((min(16477358820714036804, -6248))))), ((var_4 || (var_14 && -31071))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    var_16 = (arr_7 [i_2] [10] [i_0]);
                    var_17 = (min(var_17, (((!((max((arr_2 [i_0]), (((arr_3 [i_0]) * -28165))))))))));
                }
                arr_9 [i_0] [i_1] = ((!(arr_2 [i_0])));

                /* vectorizable */
                for (int i_3 = 0; i_3 < 14;i_3 += 1)
                {
                    var_18 = (arr_7 [i_1] [i_1] [11]);
                    var_19 = ((~(arr_2 [i_0])));
                }
                arr_13 [i_0] [i_0] [i_0] = ((~((min(((((arr_0 [i_1]) >= 785468160))), 252)))));
                /* LoopNest 2 */
                for (int i_4 = 1; i_4 < 10;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 14;i_5 += 1)
                    {
                        {
                            var_20 ^= arr_4 [i_5] [11] [13];
                            var_21 = (min(var_21, var_9));
                        }
                    }
                }
            }
        }
    }
    var_22 = var_4;
    #pragma endscop
}
