/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 201232
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=201232 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/201232
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max(var_20, var_19));

    for (int i_0 = 0; i_0 < 19;i_0 += 1) /* same iter space */
    {
        arr_3 [i_0] = var_14;
        arr_4 [i_0] [i_0] = (((((var_6 <= var_5) ? (~2147483647) : 1))) || (-4571541230085374164 / 2147483647));
    }
    for (int i_1 = 0; i_1 < 19;i_1 += 1) /* same iter space */
    {
        var_21 = 1;
        var_22 = var_13;
    }
    for (int i_2 = 0; i_2 < 15;i_2 += 1)
    {
        arr_10 [i_2] [i_2] = ((!(((1 * (4571541230085374163 < -4571541230085374164))))));
        arr_11 [i_2] = ((!((min(1, (arr_7 [i_2]))))));
    }
    #pragma endscop
}
