/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226633
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226633 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226633
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            arr_9 [11] [i_2] [0] [i_3] = (arr_6 [i_0] [i_0] [i_0]);
                            var_17 = (min((((((arr_3 [i_0] [i_1 - 1] [i_1 - 1]) & 0)))), (((arr_0 [i_1 - 1]) ? 1 : (arr_0 [i_1 - 1])))));
                        }
                    }
                }

                /* vectorizable */
                for (int i_4 = 0; i_4 < 15;i_4 += 1)
                {
                    var_18 |= arr_2 [i_1 - 1] [i_1 - 1];
                    arr_12 [i_0] [i_4] = (((arr_8 [i_4] [i_4] [i_1] [i_1 - 1] [i_1] [i_0]) != 0));
                    arr_13 [i_4] [i_1] [i_0] = (~1);
                }
            }
        }
    }
    var_19 = (max((max(var_10, 1)), var_15));
    #pragma endscop
}
