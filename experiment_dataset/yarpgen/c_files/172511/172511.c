/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172511
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172511 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172511
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 15;i_2 += 1)
            {
                {
                    var_17 = (arr_5 [i_0] [i_1]);
                    var_18 ^= (arr_7 [0]);
                    arr_9 [i_1] [12] [i_2] [i_1] = (min((((~((min((arr_8 [i_2] [i_1] [i_1] [i_2 - 1]), (arr_5 [i_0 - 2] [i_2]))))))), (min((min(var_2, var_8)), (arr_1 [i_0 + 1])))));
                }
            }
        }
        arr_10 [i_0] [i_0] = 40;
        var_19 += ((((((min((arr_2 [i_0]), 1))) <= (((arr_6 [4]) * (arr_6 [6]))))) ? ((~(!9738232964573932669))) : (((!((min((arr_2 [2]), var_14))))))));
        var_20 = (arr_0 [i_0]);
    }
    var_21 &= (!((max(1, 63370))));
    #pragma endscop
}
