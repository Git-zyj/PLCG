/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 153514
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=153514 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/153514
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */int) (_Bool)1);
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned short i_0 = 0; i_0 < 21; i_0 += 4) 
    {
        /* LoopSeq 2 */
        for (unsigned int i_1 = 0; i_1 < 21; i_1 += 4) 
        {
            var_14 = ((/* implicit */_Bool) min((var_14), (((/* implicit */_Bool) var_10))));
            var_15 = ((((/* implicit */_Bool) arr_2 [i_0])) ? (((/* implicit */unsigned int) (+(((/* implicit */int) (_Bool)1))))) : (var_12));
        }
        for (unsigned short i_2 = 0; i_2 < 21; i_2 += 4) 
        {
            arr_6 [i_2] [i_2] [i_2] = ((/* implicit */short) (+(((((/* implicit */_Bool) arr_5 [i_2])) ? (-1620024265475552076LL) : (((/* implicit */long long int) ((/* implicit */int) var_10)))))));
            arr_7 [i_0] [i_0] [i_0] = ((/* implicit */short) ((unsigned short) var_10));
            var_16 = ((/* implicit */signed char) (!(((/* implicit */_Bool) ((var_0) ? (((/* implicit */unsigned int) ((/* implicit */int) var_0))) : (var_2))))));
        }
        var_17 = ((/* implicit */short) (+(((/* implicit */int) (signed char)117))));
    }
}
