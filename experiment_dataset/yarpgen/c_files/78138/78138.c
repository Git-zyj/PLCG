/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78138
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78138 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78138
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            {
                var_10 = (max((max((arr_3 [i_0 + 1] [2]), (max((arr_2 [i_0]), (arr_2 [i_0 + 1]))))), (max((max((arr_3 [i_0 + 1] [7]), (arr_3 [i_0 + 1] [i_1]))), (arr_0 [13])))));
                var_11 = (max(65527, -8992));
            }
        }
    }

    /* vectorizable */
    for (int i_2 = 0; i_2 < 25;i_2 += 1)
    {
        var_12 = (min(var_12, (arr_6 [i_2])));
        var_13 = (arr_6 [11]);
    }
    var_14 = (max((max(((max(var_0, var_9))), var_1)), var_5));
    #pragma endscop
}
