/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94532
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94532 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94532
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = var_1;
    var_13 = ((/* implicit */int) var_3);
    /* LoopNest 2 */
    for (signed char i_0 = 1; i_0 < 20; i_0 += 4) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 18; i_1 += 1) 
        {
            {
                var_14 = ((/* implicit */_Bool) arr_6 [i_0] [i_1]);
                var_15 = ((/* implicit */unsigned char) max((var_15), (((/* implicit */unsigned char) (!(((/* implicit */_Bool) -3574734315357600158LL)))))));
                var_16 -= ((/* implicit */short) var_11);
                arr_7 [i_0] [i_0 - 1] [i_1] = ((/* implicit */unsigned char) ((short) arr_0 [i_0]));
            }
        } 
    } 
}
