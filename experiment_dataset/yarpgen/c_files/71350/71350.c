/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71350
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71350 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71350
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_15 = ((var_3 ? var_6 : var_7));
        arr_2 [i_0] [i_0] = var_10;
    }

    for (int i_1 = 0; i_1 < 19;i_1 += 1)
    {
        arr_5 [i_1] [i_1] = ((min((max(224, 21124)), (var_12 < var_0))));
        var_16 = (max(var_16, -var_8));
        arr_6 [i_1] [i_1] = (min(var_10, (var_1 & var_3)));
    }
    for (int i_2 = 2; i_2 < 21;i_2 += 1)
    {
        var_17 ^= (((min(((min(-700443146691412661, 2047))), var_3)) + var_6));
        arr_10 [i_2] [i_2] = (~255);
    }
    #pragma endscop
}
