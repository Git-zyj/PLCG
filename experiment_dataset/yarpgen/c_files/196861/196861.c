/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196861
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196861 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196861
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_13 = 1;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        {
                            var_14 += (((arr_11 [i_2] [i_1] [i_3]) % 1));
                            var_15 = ((((var_8 ? -6058274221420270739 : var_8)) < 158));
                            arr_13 [i_0] [i_2] [i_2] [i_2] = (max(var_7, ((var_10 ? -6058274221420270739 : var_2))));
                            var_16 = var_12;
                            arr_14 [i_3] [i_1] [i_2] [i_1] [i_0] = (min(var_4, 255));
                        }
                    }
                }
            }
        }
    }
    var_17 += 255;
    #pragma endscop
}
