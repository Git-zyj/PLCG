/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199853
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199853 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199853
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        arr_2 [i_0] = min((~var_5), (arr_0 [i_0 - 1]));
        arr_3 [7] [7] = ((+(((((((-2040058910 + 2147483647) >> (2040058910 - 2040058902)))) && ((min(252, 32230))))))));
        arr_4 [i_0 + 1] = ((((max((arr_1 [i_0 - 2]), (arr_0 [i_0 + 1])))) ? 2040058932 : ((var_10 << (-32249 + 32269)))));
        arr_5 [i_0 - 1] = ((+(min((!1702520606), (min(-2040058932, 32241))))));
    }
    var_19 ^= 3615270058;
    #pragma endscop
}
