/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 121835
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=121835 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/121835
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((min((min(var_7, var_19)), var_7)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_21 = (max((arr_1 [i_0]), (-127 - 1)));
                var_22 &= max((((((-119 ? 253 : 144114638320041984)) > (arr_5 [i_0] [i_1] [i_1])))), ((~(arr_1 [i_1]))));
                var_23 &= (((((max(var_0, var_13))) ? ((var_8 ? var_15 : (arr_0 [i_1] [i_0]))) : (min((arr_5 [i_0] [i_1] [i_1]), var_17)))));
                var_24 &= var_14;
            }
        }
    }
    #pragma endscop
}
