/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132635
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132635 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132635
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            {
                var_11 = 1938234396;
                arr_6 [i_0] [i_1] [i_0] = ((((min((arr_3 [i_1] [i_1]), var_3))) ? var_9 : ((min(var_1, (arr_1 [i_1 - 1] [i_1 + 1]))))));
            }
        }
    }
    var_12 = var_2;
    var_13 = 217;
    var_14 = max((((var_0 ? var_9 : var_3))), ((((min(1, 47980))) ? (((max(var_10, var_4)))) : (min(2992822809, 4294967295)))));
    #pragma endscop
}
