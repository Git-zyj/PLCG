/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 1494
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=1494 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/1494
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 |= (((((var_8 ? ((var_2 ? var_6 : var_14)) : var_14))) ? ((~((16433691919114758393 ? var_12 : 0)))) : ((~((max(var_8, 35371)))))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_18 = 7929615566441525818;
                arr_4 [i_0] [i_1] = (arr_1 [i_1]);
                arr_5 [i_0] [1] = ((((0 == (arr_3 [i_0 - 1]))) ? (((arr_0 [i_0 - 2]) ? (arr_0 [i_1]) : (arr_2 [i_0 + 1]))) : ((((!27428) % ((max(47, (arr_3 [i_1])))))))));
            }
        }
    }
    var_19 = var_14;
    var_20 &= 0;
    #pragma endscop
}
