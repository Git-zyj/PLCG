/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 154144
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=154144 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/154144
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_17 = (((arr_1 [i_0]) ? ((-(0 && 1))) : (((((1 ? 51770 : 51762))) ? 13761 : var_7))));
        var_18 &= ((+(max((arr_1 [i_0]), (arr_1 [i_0])))));
    }
    var_19 += ((((var_3 <= (~var_16)))));
    var_20 = ((~((((13774 ? -5197 : 0))))));
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 12;i_1 += 1)
    {
        for (int i_2 = 3; i_2 < 11;i_2 += 1)
        {
            {
                var_21 = ((((min((((arr_5 [11]) ? var_7 : (arr_2 [i_1]))), var_4))) ? ((-var_1 + (((62 ? var_2 : (arr_8 [i_1] [i_2])))))) : (((((((arr_5 [i_1]) + 4))) ? ((var_2 - (arr_8 [i_1] [i_2]))) : 32767)))));
                var_22 = (((((arr_6 [i_2 - 3] [i_2 - 3]) ? (arr_6 [i_2 - 2] [i_2 - 1]) : 1095925846)) | (min((arr_6 [i_2 + 1] [i_2 - 2]), 2147483622))));
            }
        }
    }
    #pragma endscop
}
