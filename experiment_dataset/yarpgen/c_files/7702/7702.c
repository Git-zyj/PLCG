/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7702
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7702 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7702
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 += var_4;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 14;i_2 += 1)
            {
                {
                    arr_7 [i_2] = (arr_4 [i_0] [i_0] [1] [i_0]);

                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        var_12 ^= var_8;
                        arr_11 [i_3] [i_2] [6] [i_2 - 1] [i_2] [i_0] = (((arr_4 [i_2] [i_2 + 1] [i_2 + 1] [i_2 + 1]) && (arr_4 [i_2] [i_2] [i_2 + 1] [i_2 - 1])));
                    }
                }
            }
        }
        var_13 = (min(var_13, (((arr_3 [i_0] [i_0] [i_0]) ? (arr_3 [i_0] [i_0] [i_0]) : (arr_3 [1] [4] [i_0])))));
        var_14 = (arr_8 [i_0] [i_0] [i_0] [i_0]);
    }
    #pragma endscop
}
