/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110879
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110879 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110879
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min(var_10, var_9));
    var_11 |= var_1;
    var_12 = (max(-122, var_4));
    var_13 = min(var_7, ((var_8 + (((min(29, var_5)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_14 ^= (arr_0 [i_1]);
                var_15 = ((((max((((var_7 ? (arr_1 [i_1]) : var_0))), (min(var_7, (arr_4 [i_0] [i_0] [i_1])))))) ? ((max((((arr_3 [i_1] [i_1] [i_1]) < (arr_1 [i_0]))), var_0))) : (arr_3 [i_0] [i_0] [i_1])));
                var_16 = (((arr_1 [i_0]) && -0));
                var_17 += 32738;
            }
        }
    }
    #pragma endscop
}
