/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24985
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24985 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24985
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_14 = (min(((((arr_3 [i_0]) & ((var_10 ? var_3 : 3745859640))))), (max(6859236182518331190, (arr_2 [i_1])))));
                var_15 = ((((((4762546583065283715 ? var_10 : 244))) ? 4163340706 : (((var_10 ? 0 : 12))))) / (arr_6 [i_0] [i_1] [i_1]));
            }
        }
    }
    var_16 = var_5;
    var_17 = var_5;
    #pragma endscop
}
