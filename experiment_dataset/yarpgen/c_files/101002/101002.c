/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 101002
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=101002 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/101002
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (((((~var_7) <= ((1099511627775 ? 1985298404 : -1915391712)))) && ((((var_9 ^ 144) & (1 >= -1))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_11 &= ((-255 ? 3134033209368094159 : (((((((arr_1 [6] [i_0]) && (arr_2 [10]))) && 12752992839761445053))))));
                    arr_7 [i_2] = (min((((arr_3 [i_1]) - ((255 | (arr_6 [i_0] [i_2] [i_2] [i_0]))))), (arr_4 [i_2])));
                }
            }
        }
    }
    #pragma endscop
}
