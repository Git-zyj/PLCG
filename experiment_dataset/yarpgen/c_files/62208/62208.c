/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62208
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62208 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62208
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max(var_20, var_2));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] = 1;

                    for (int i_3 = 3; i_3 < 20;i_3 += 1)
                    {
                        var_21 = (min(((21473 ^ (((arr_4 [i_0] [1] [i_2]) ? (arr_1 [i_2]) : (arr_3 [i_3] [i_1]))))), ((-(arr_2 [i_3])))));
                        arr_11 [i_0] [1] [i_0] [i_0] = 1;
                        var_22 &= var_17;
                        var_23 = (min(var_23, ((max((((arr_7 [i_1 + 1] [i_1 + 1] [i_3 - 2]) << (23534 - 23529))), -282157163)))));
                    }
                    arr_12 [i_0] [i_1] [i_0] = (min((((arr_0 [i_1 + 1] [i_1 + 1]) | (arr_2 [i_1 + 1]))), (arr_2 [i_1 + 1])));
                }
            }
        }
    }
    #pragma endscop
}
