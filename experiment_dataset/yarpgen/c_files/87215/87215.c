/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87215
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87215 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87215
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((((2 ? 11 : 254))) / var_10);

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (((((((((arr_1 [i_0] [i_0]) ? (arr_0 [i_0]) : var_6))) ? (((253 ? (arr_1 [i_0] [4]) : var_7))) : ((var_8 ? (arr_1 [i_0] [i_0]) : (arr_0 [i_0])))))) ? (arr_0 [i_0]) : (~11)));
        arr_3 [i_0] [i_0] = ((((((arr_1 [i_0] [i_0]) & (arr_1 [8] [i_0])))) ? 30720 : ((((244 && (arr_0 [i_0])))))));
        var_15 = (arr_0 [i_0]);
    }
    var_16 -= var_6;
    #pragma endscop
}
