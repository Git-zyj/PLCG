/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86169
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86169 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86169
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max((max(((var_7 << (var_10 + 7556244550626755660))), -var_3)), (((!(16383 > -1))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_14 = (max(32184, (arr_5 [i_0] [i_1] [i_2])));
                    arr_7 [i_0] [i_1] [2] [i_2] = (((max((arr_1 [i_1]), 1))) % (arr_2 [i_0] [1]));
                    arr_8 [9] [i_1] [i_2] = (((((((arr_4 [i_0] [i_1] [i_2]) ? -16 : 20)))) && ((min(15, (((~(arr_4 [i_0] [i_1] [i_2])))))))));
                }
            }
        }
    }
    var_15 ^= var_7;
    #pragma endscop
}
