/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 188498
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=188498 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/188498
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            {
                arr_6 [i_0] = ((max(634055917, (max((arr_5 [i_0] [i_0] [i_0]), (arr_1 [i_0]))))));

                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    var_14 = (((arr_7 [i_0] [i_0] [i_1] [11]) ? (arr_0 [i_0]) : (arr_1 [i_0])));
                    var_15 |= ((1 > (arr_8 [i_1 - 1] [i_1 - 1] [i_1 + 1] [i_1 + 1])));
                    var_16 = (arr_5 [i_0] [i_0] [i_0]);
                }
            }
        }
    }
    var_17 = (max(var_11, (((var_6 ? 881965172 : 1)))));
    var_18 = ((var_1 ? var_4 : (((((1 ? 1 : var_10)) / var_7)))));
    var_19 = var_9;
    var_20 &= (1 / var_5);
    #pragma endscop
}
