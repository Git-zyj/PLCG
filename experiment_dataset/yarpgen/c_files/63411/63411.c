/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63411
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63411 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63411
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 |= (max(((((var_13 ? var_10 : var_1)) << (((max(var_0, var_5)) - 3660428639161136589)))), var_3));
    var_15 = var_3;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    var_16 = (min(var_16, (((min(var_1, var_12))))));
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 24;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 23;i_4 += 1)
                        {
                            {
                                arr_12 [i_0] [i_0] = ((~(~var_8)));
                                var_17 = (!1);
                                var_18 = (63 * 45064);
                                var_19 = 1073733632;
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 25;i_5 += 1)
                    {
                        for (int i_6 = 1; i_6 < 21;i_6 += 1)
                        {
                            {
                                var_20 = (arr_13 [i_0] [i_0] [15] [i_0]);
                                var_21 = (min((((!((min(var_2, (arr_7 [i_0]))))))), (((arr_15 [i_6 + 1] [i_6] [i_6] [i_6 + 1]) ? var_7 : var_9))));
                            }
                        }
                    }
                    var_22 = ((~(((~var_0) / (((arr_4 [i_0] [i_0]) ^ var_2))))));
                }
            }
        }
    }
    #pragma endscop
}
