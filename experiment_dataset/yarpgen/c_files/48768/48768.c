/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48768
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48768 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48768
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (((((var_2 >= var_9) && (((-1734459245513910848 ? var_14 : var_5))))) ? ((((273883171035135843 <= 27) <= var_14))) : var_2));
    var_16 ^= (~var_13);
    var_17 |= var_2;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            {
                var_18 = (((arr_1 [i_0]) | 65535));
                var_19 = (((((-24197 ? (~-18) : ((~(arr_3 [i_0])))))) ? (((min((((arr_1 [i_0]) >= (arr_2 [i_0]))), (!104))))) : 3037286175));
                var_20 = (arr_4 [i_0] [i_0] [i_1 - 2]);
            }
        }
    }
    var_21 = (min(var_21, var_7));
    #pragma endscop
}
