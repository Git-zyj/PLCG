/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70168
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70168 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70168
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 &= (((((758299327 != var_5) || var_14)) ? ((1 ? ((min(1, 1))) : -10021)) : var_10));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        var_17 ^= (~32);
        var_18 = (min(var_18, 16537448674626630466));
        var_19 = (max(var_19, -27542));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        var_20 = (max(var_20, (~16)));
        var_21 &= (~14223);
        var_22 = (max(var_22, (((arr_1 [i_1]) && 104))));
        var_23 += (var_2 > var_13);
    }
    for (int i_2 = 2; i_2 < 19;i_2 += 1)
    {
        arr_10 [i_2] [i_2] |= ((1 & ((((var_12 ? (arr_6 [2]) : 1))))));
        var_24 = (max(var_24, var_10));
        var_25 |= (((-239 + 2147483647) << ((((49626781 ? ((min(23, var_15))) : -4933)) - 23))));
    }
    #pragma endscop
}
