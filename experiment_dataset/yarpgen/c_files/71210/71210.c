/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71210
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71210 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71210
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((var_0 ? var_8 : (var_12 ^ var_16)));

    /* vectorizable */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        var_21 = (min(var_21, (arr_1 [2])));
        arr_3 [i_0] = ((arr_0 [i_0 + 1] [i_0 + 1]) ? (arr_2 [i_0]) : (((arr_2 [i_0]) % -12506)));
        arr_4 [i_0 - 1] = (((9170352858711249404 ? 12505 : var_18)) ^ (arr_2 [i_0 - 1]));
    }
    #pragma endscop
}
