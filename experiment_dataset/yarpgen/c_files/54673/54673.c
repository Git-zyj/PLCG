/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54673
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54673 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54673
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(var_14, ((min((((((max(23105, 4))) | ((min(141, var_5)))))), ((var_13 ? (var_1 != var_4) : (var_7 / 141))))))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] = ((((((min(115, 140))) == (arr_2 [i_0 + 3] [i_1]))) ? (arr_2 [i_0 + 4] [12]) : (((arr_1 [i_0 - 2]) ? (arr_3 [i_0 + 3]) : ((var_13 ? (arr_3 [2]) : var_10))))));
                var_15 = (max(var_15, var_0));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 14;i_4 += 1)
                        {
                            {
                                var_16 ^= (arr_2 [i_4] [i_1]);
                                var_17 -= 135;
                                var_18 = var_3;
                            }
                        }
                    }
                }
                var_19 = ((-((~(arr_7 [i_1] [i_1] [i_0 + 2])))));
            }
        }
    }
    var_20 -= 12084;
    #pragma endscop
}
