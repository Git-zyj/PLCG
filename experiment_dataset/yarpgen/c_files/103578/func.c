/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103578
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103578 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103578
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */short) var_10);
    /* LoopNest 2 */
    for (unsigned char i_0 = 0; i_0 < 13; i_0 += 2) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_13 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) var_7)) ? (arr_0 [i_1]) : (((arr_1 [i_1] [i_0]) ? (((/* implicit */int) (_Bool)0)) : (arr_0 [i_1])))));
                var_14 = ((/* implicit */_Bool) min((max((arr_0 [i_0]), (1676588076))), (arr_4 [i_0] [i_1])));
            }
        } 
    } 
}
