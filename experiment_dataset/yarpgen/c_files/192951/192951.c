/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 192951
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=192951 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/192951
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    arr_7 [i_1] = var_9;
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_18 = (~var_7);
                                arr_14 [i_3] [1] [i_2] [i_0] [i_3] = (min((arr_9 [i_0] [i_3] [i_3] [i_3]), (arr_6 [i_3] [i_1] [i_0])));
                            }
                        }
                    }
                }
            }
        }
    }
    var_19 = var_17;
    var_20 = ((var_2 != (((var_4 > (!var_10))))));
    var_21 = (((((var_3 ? ((79 ? var_16 : 109)) : ((min(67, var_17)))))) ? (min(-var_8, var_12)) : ((-(108 * -124)))));
    var_22 = var_8;
    #pragma endscop
}
