/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131595
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131595 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131595
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = -72;
    var_15 = ((var_7 ? ((29758 ? 4925988080932884163 : 3482)) : (((((37 ? -3482 : 1))) ? (~13520755992776667454) : var_4))));

    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = ((((~115) ? var_4 : (((arr_0 [i_0]) ? 19 : 8674404929816653797)))));
        arr_3 [i_0] = (arr_1 [i_0]);
        var_16 = (((((arr_0 [i_0]) || (arr_1 [i_0 - 1]))) ? ((((!(arr_0 [i_0]))))) : (min((0 >= 176), ((~(arr_0 [i_0])))))));
    }
    var_17 = (~var_2);
    #pragma endscop
}
