/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 186487
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=186487 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/186487
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((var_0 ? ((~(~var_2))) : (((((var_5 & var_5) || (((var_10 ? var_2 : var_5)))))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        var_12 = -var_7;
        var_13 = (((~var_3) || (((var_8 ? var_7 : var_3)))));
    }
    var_14 = ((((!var_8) ? var_10 : var_0)) & -var_4);
    var_15 = (((((~((var_7 ? var_1 : var_1))))) ? (~-var_8) : ((((var_1 ? var_1 : var_3)) % ((((var_10 + 2147483647) >> (var_7 - 12398))))))));
    var_16 += var_0;
    #pragma endscop
}
