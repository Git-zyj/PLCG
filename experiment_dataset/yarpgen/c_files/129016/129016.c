/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 129016
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=129016 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/129016
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 &= 2278960412;
    var_18 = ((((min((max(2278960412, var_12)), var_15))) && ((((((4294967295 ? 0 : 11))) ? ((3144016923 ? 0 : 1566220022)) : 4294967285)))));
    var_19 = 2035478006;
    var_20 = (max(var_20, (((min(var_1, var_14)) / var_7))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_21 = max(((var_0 ? (arr_2 [i_0]) : (((arr_4 [4] [2]) + 2805589226)))), (arr_3 [i_0]));
                arr_6 [i_0] [i_0] = 2035478031;
            }
        }
    }
    #pragma endscop
}
