/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 238765
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=238765 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/238765
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 += (min(((((min(-3537046884584565936, var_7)) != (var_1 % var_8)))), ((var_8 ? var_8 : var_5))));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 24;i_2 += 1)
            {
                {
                    var_12 -= ((((arr_2 [i_2 - 2]) & 13589316836115423550)));
                    var_13 ^= (min(((-22720 ? -22747 : -22716)), ((min(-40, 22747)))));
                    var_14 = ((-17027 ? (min((arr_7 [4] [i_2 - 1] [i_2 - 4] [i_2]), (((arr_1 [i_1]) / var_1)))) : -22720));
                }
            }
        }
    }
    #pragma endscop
}
