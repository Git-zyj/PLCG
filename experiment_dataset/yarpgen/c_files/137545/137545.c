/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 137545
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=137545 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/137545
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            var_15 -= ((0 ? 2528337161201872685 : 2176402416));
            var_16 -= (((arr_3 [i_0] [i_0] [i_1]) + var_5));
        }
        var_17 = var_13;
        arr_4 [i_0] [i_0] |= (arr_1 [i_0]);
        var_18 = ((((arr_3 [i_0] [20] [i_0]) ? var_5 : 2954653466)) + (arr_3 [i_0] [i_0] [i_0]));
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 12;i_2 += 1)
    {
        var_19 = 24;
        var_20 &= (arr_3 [i_2] [i_2] [i_2]);
    }
    var_21 -= (((((var_12 / var_4) | ((var_6 ? 10392329734827135526 : var_5)))) | ((min(var_1, var_3)))));
    #pragma endscop
}
