/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41704
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41704 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41704
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        arr_2 [12] [12] &= 4095;
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1] [i_2] [i_0] = (!1);
                    var_11 = (!7903324245258890174);
                }
            }
        }
        var_12 = ((((1 || (0 % 72))) ? -79 : (((1 ? 2746760002 : -79)))));
    }
    var_13 *= ((((((((14457190143801879430 ? -79 : var_9))) ? 72 : (max(var_8, 9223372036854775807))))) ? var_3 : 14));
    #pragma endscop
}
