/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 232872
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=232872 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/232872
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_12 = (min(var_12, (arr_1 [i_0])));
        arr_2 [i_0] [1] = ((((((2763207679 ? (arr_1 [i_0]) : 2763207679)) - (arr_0 [i_0]))) << (((arr_1 [i_0]) + 122))));
        var_13 = (arr_0 [i_0]);
        var_14 = (i_0 % 2 == 0) ? ((((arr_1 [i_0]) >> (max(((((arr_0 [i_0]) >= (arr_1 [i_0])))), (min((arr_0 [i_0]), (arr_1 [i_0])))))))) : ((((arr_1 [i_0]) >> (((max(((((arr_0 [i_0]) >= (arr_1 [i_0])))), (min((arr_0 [i_0]), (arr_1 [i_0]))))) - 139)))));
    }
    var_15 = var_8;
    #pragma endscop
}
