/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 118575
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=118575 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/118575
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_12 = (max(var_12, (((((arr_3 [i_0] [i_1] [i_1]) <= (((arr_1 [i_1]) ? (arr_3 [i_0] [i_0] [i_1]) : (arr_1 [i_0])))))))));
                var_13 = (arr_0 [5] [i_0]);
                var_14 = ((arr_4 [i_0] [i_1] [i_0]) <= -613118315);
                var_15 = (max(var_15, (((((((((arr_0 [i_1] [i_0]) && var_5))) == (arr_2 [i_1] [i_0]))) ? 80 : (((arr_1 [i_0]) ? (arr_1 [i_0]) : (arr_2 [i_0] [i_1]))))))));
            }
        }
    }
    var_16 &= var_7;
    #pragma endscop
}
