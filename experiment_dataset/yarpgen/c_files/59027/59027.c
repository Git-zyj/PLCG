/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59027
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59027 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59027
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 += var_7;
    var_17 = (max(var_17, var_8));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            {
                var_18 = (max((arr_1 [i_0]), ((~(arr_1 [i_0])))));
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 18;i_3 += 1)
                    {
                        {
                            var_19 = (max(var_19, var_1));
                            arr_13 [i_0] = (arr_10 [i_0] [i_0] [i_0] [i_0] [i_0]);
                        }
                    }
                }
                var_20 = var_8;
                var_21 = (min((((arr_0 [i_0]) ? ((max((arr_12 [i_0] [i_0] [i_1] [i_1 + 1]), (arr_4 [i_0] [i_0] [i_0])))) : (var_14 && 199665067656560765))), 251));
                arr_14 [i_0] [i_1 - 1] = (arr_0 [i_0]);
            }
        }
    }
    var_22 = var_10;
    #pragma endscop
}
