/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157197
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157197 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157197
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_8;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_6 [i_1] [3] = ((0 > (var_13 - 255)));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        {
                            var_21 &= var_1;
                            arr_11 [i_0] [i_0 - 1] [i_2] [i_2] = (arr_8 [i_0] [i_0] [i_0 - 2]);
                            arr_12 [i_0] [i_1] [i_2] [i_3] [9] [1] = ((+(((((((arr_4 [7] [i_2] [i_2]) + 2147483647)) >> (((arr_3 [i_0 - 1] [i_1]) + 30343086)))) & var_14))));
                            var_22 &= var_3;
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
