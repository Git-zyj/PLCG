/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89150
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89150 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89150
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_14;

    /* vectorizable */
    for (int i_0 = 2; i_0 < 11;i_0 += 1)
    {
        var_19 = (((arr_1 [i_0 - 1] [i_0 + 1]) ? 71 : 82));
        var_20 = (max(var_20, (((9223372036854775798 ? -2455439102532002717 : -3296205758883965850)))));
    }
    var_21 = ((5054108963256687397 ? (((!-9223372036854775798) ? ((var_14 ? var_4 : 9223372036854775802)) : var_5)) : var_17));
    #pragma endscop
}
