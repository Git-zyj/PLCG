/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244061
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244061 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244061
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (!((min(((65535 ? var_13 : var_12)), ((65528 ? var_15 : var_2))))));
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_17 = (min((+-2191788017), (((var_14 ? 9 : -22748)))));
                var_18 = (min(((2191788026 ? -22742 : (((arr_4 [i_0 - 2]) & (arr_0 [i_0]))))), 6907));
                var_19 = ((((!(arr_2 [i_0 - 2] [5]))) ? (((arr_3 [i_0 - 4]) ? (arr_3 [i_0 + 1]) : (arr_3 [i_0 - 2]))) : -var_2));
            }
        }
    }
    #pragma endscop
}
