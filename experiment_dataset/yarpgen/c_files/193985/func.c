/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 193985
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=193985 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/193985
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */short) var_9);
    var_11 = var_5;
    var_12 = (-((+(var_4))));
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 25; i_0 += 1) 
    {
        /* LoopSeq 1 */
        for (unsigned char i_1 = 2; i_1 < 22; i_1 += 3) 
        {
            var_13 = ((/* implicit */int) var_3);
            arr_5 [i_0] [i_1] [i_1] = ((/* implicit */unsigned short) (((+(arr_3 [i_1 - 2]))) <= (((((/* implicit */_Bool) var_9)) ? (var_3) : (((/* implicit */unsigned int) ((/* implicit */int) var_2)))))));
        }
        arr_6 [i_0] [7LL] = ((/* implicit */_Bool) min((((((/* implicit */_Bool) ((((/* implicit */_Bool) var_9)) ? (((/* implicit */unsigned int) ((/* implicit */int) arr_4 [20U] [20U] [i_0]))) : (var_9)))) ? (((/* implicit */int) var_2)) : (var_1))), (((/* implicit */int) (!(var_8))))));
        arr_7 [i_0] [i_0] = ((/* implicit */unsigned short) var_5);
        var_14 = ((/* implicit */long long int) var_9);
    }
}
