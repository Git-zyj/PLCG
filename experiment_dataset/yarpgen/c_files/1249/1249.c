/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 1249
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=1249 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/1249
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_10 = (arr_1 [i_0]);

        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            arr_4 [i_0] = 3;
            var_11 -= (((((-1 ? var_8 : (arr_3 [i_0])))) ? ((((!(arr_3 [3]))))) : ((0 ? var_1 : 20828))));
        }
        for (int i_2 = 1; i_2 < 1;i_2 += 1) /* same iter space */
        {
            arr_8 [i_0] [i_0] [i_2] = arr_5 [i_0];
            arr_9 [i_0] = (((((!(arr_7 [i_0] [i_2] [i_0])))) * (arr_6 [i_2 - 1] [i_2 - 1] [i_0])));
        }
        for (int i_3 = 1; i_3 < 1;i_3 += 1) /* same iter space */
        {
            var_12 *= (((arr_2 [8] [8]) ? var_1 : (-9223372036854775807 - 1)));
            arr_14 [i_0] = var_7;
            arr_15 [i_0] = (((arr_12 [1] [i_0]) ? (arr_12 [i_0] [i_0]) : (arr_13 [i_0] [i_0])));
            /* LoopNest 2 */
            for (int i_4 = 2; i_4 < 10;i_4 += 1)
            {
                for (int i_5 = 0; i_5 < 12;i_5 += 1)
                {
                    {
                        arr_21 [i_0] [i_3] [i_3 - 1] [3] [i_5] = 9;
                        var_13 = var_2;
                    }
                }
            }
        }
        for (int i_6 = 1; i_6 < 1;i_6 += 1) /* same iter space */
        {
            arr_24 [i_0] [i_6] [i_6 - 1] = arr_13 [i_6 - 1] [i_0];
            var_14 = var_8;
        }
        var_15 = (((9223372036854775807 ? var_6 : 9223372036854775799)) | -369457799273509655);
    }
    var_16 = var_5;
    #pragma endscop
}
