/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220972
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220972 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220972
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 += ((/* implicit */unsigned int) var_2);
    /* LoopNest 2 */
    for (unsigned int i_0 = 4; i_0 < 15; i_0 += 4) 
    {
        for (long long int i_1 = 0; i_1 < 16; i_1 += 4) 
        {
            {
                arr_8 [i_0] = ((/* implicit */unsigned int) ((((/* implicit */_Bool) 12215300734343080581ULL)) ? (((/* implicit */unsigned long long int) 0U)) : (((((/* implicit */_Bool) ((((/* implicit */_Bool) 15303163062031274564ULL)) ? (var_7) : (((/* implicit */long long int) var_3))))) ? (((/* implicit */unsigned long long int) ((/* implicit */int) (!(((/* implicit */_Bool) var_10)))))) : (3143581011678277032ULL)))));
                var_16 = ((/* implicit */int) var_4);
            }
        } 
    } 
    var_17 = ((/* implicit */unsigned int) var_11);
    var_18 = ((/* implicit */unsigned int) var_7);
    var_19 = ((/* implicit */long long int) min((var_19), (((/* implicit */long long int) var_12))));
}
