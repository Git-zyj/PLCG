/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229682
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229682 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229682
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                arr_7 [i_0] = (((3734223774 ? ((~(arr_4 [i_0] [i_0] [i_1])) : (((arr_5 [i_0] [i_0]) ? (arr_5 [i_0] [i_0]) : 37))))));
                arr_8 [3] [3] [i_1 - 1] = (((arr_6 [i_0] [i_0] [i_1 - 1]) ? ((~(arr_5 [i_0] [i_1 - 1]))) : (arr_6 [i_0] [i_0] [i_0])));
                var_18 = ((-(max(867257420, 867257420))));
            }
        }
    }
    var_19 = (max(var_19, var_16));
    #pragma endscop
}
