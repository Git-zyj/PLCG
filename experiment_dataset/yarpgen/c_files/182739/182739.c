/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182739
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182739 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182739
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(var_18, var_17));

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_19 = (min(var_19, (arr_0 [i_0])));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    arr_7 [i_0] [1] = (!(max((((arr_1 [i_0]) || (arr_3 [i_0]))), (!var_6))));
                    arr_8 [i_0] [1] [i_2] = (max(((!(arr_6 [i_1] [i_1 + 1] [i_1 + 1]))), (max((arr_6 [i_2] [i_2] [i_1 + 1]), (arr_6 [4] [i_2] [i_1 + 1])))));
                    var_20 = (max(var_20, (arr_2 [i_0])));
                }
            }
        }
        var_21 = ((~(((~var_4) | (arr_4 [i_0])))));
    }
    var_22 = (((var_11 && 1) ? 1 : (((var_6 < 1) ? 0 : (~var_4)))));
    #pragma endscop
}
