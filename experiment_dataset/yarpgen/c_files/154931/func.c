/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 154931
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=154931 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/154931
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned int) max((var_11), (((/* implicit */unsigned int) (~(((/* implicit */int) var_9)))))));
    var_12 = ((/* implicit */unsigned int) var_7);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 16; i_0 += 2) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_13 = ((/* implicit */unsigned char) (~(((((/* implicit */_Bool) arr_3 [i_0] [i_1])) ? (((/* implicit */unsigned int) arr_3 [i_1] [i_0])) : (var_0)))));
                arr_5 [i_0] [i_0] = ((/* implicit */unsigned long long int) (short)-20494);
            }
        } 
    } 
}
