/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 189178
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=189178 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/189178
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(var_15, ((min(var_14, (-var_12 + 0))))));
    var_16 = var_4;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_0] |= ((((((min((arr_1 [i_0 - 1] [i_0 - 1]), var_3)) + (((var_0 ? var_0 : (arr_4 [i_0] [i_0] [i_1]))))))) ? (arr_1 [i_0] [i_0]) : ((((((1 ? var_14 : (arr_4 [i_0] [i_0] [i_1]))) < var_0))))));
                var_17 = (2147483648 - 60952);
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 0; i_2 < 16;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 13;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 16;i_4 += 1)
            {
                {
                    var_18 = (var_10 | var_5);
                    arr_13 [i_2] [14] [5] [i_4] = var_2;
                }
            }
        }
    }
    #pragma endscop
}
