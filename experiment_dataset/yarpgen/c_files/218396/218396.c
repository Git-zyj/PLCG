/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218396
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218396 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218396
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 |= ((!(((107 ? 114 : 2984139417)))));
    var_16 = (min(var_16, var_11));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_17 &= (((arr_1 [i_0 - 1] [i_0 + 1]) * (((2984139412 ? -39 : 255)))));
                var_18 = (4141942550 >= -var_14);
                var_19 &= (((2147483647 || 1310827878) | 1));
                var_20 = ((arr_1 [i_1] [i_1]) ? ((var_9 ? (arr_2 [i_0 + 1] [i_1]) : (arr_0 [i_0 - 1]))) : (arr_3 [i_1] [i_1] [i_0 - 1]));
            }
        }
    }
    #pragma endscop
}
