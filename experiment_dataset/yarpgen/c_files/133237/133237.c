/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133237
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133237 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133237
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 21;i_4 += 1)
                        {
                            {
                                var_16 = ((((+((19818 ? (arr_0 [i_0]) : 9)))) * ((((((arr_8 [i_0] [i_0]) ? 247 : var_4))) / (((arr_13 [i_0] [i_0] [0] [i_0 - 1] [i_0] [i_0 + 2] [i_0]) ? 36028797018701824 : -36028797018701824))))));
                                var_17 = var_6;
                                arr_14 [i_0] [7] [i_0] = var_0;
                                var_18 ^= arr_6 [i_2] [0] [i_2];
                            }
                        }
                    }
                    arr_15 [i_0] [i_0] [i_0] = (max(246, (arr_1 [i_2])));
                }
            }
        }
    }
    var_19 = var_15;
    #pragma endscop
}
