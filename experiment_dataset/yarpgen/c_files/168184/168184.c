/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168184
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168184 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168184
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_15 += (~(((!(arr_1 [i_0])))));
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 24;i_3 += 1)
                {
                    {

                        for (int i_4 = 0; i_4 < 25;i_4 += 1)
                        {
                            var_16 = ((!(((((!(arr_6 [21] [i_1] [i_2] [i_4]))))))));
                            arr_14 [i_0] [i_1] [i_1] [i_1] [0] [i_0] = ((!(arr_5 [i_0] [i_1])));
                            var_17 = ((-(arr_11 [i_2] [i_2] [i_2] [i_2] [i_2] [i_2])));
                            var_18 = (max(var_18, (arr_0 [i_4])));
                        }
                        var_19 = (~(((!(arr_7 [i_0] [i_1] [i_2])))));
                    }
                }
            }
        }
    }
    for (int i_5 = 1; i_5 < 11;i_5 += 1)
    {
        var_20 = (~(-9223372036854775807 - 1));
        /* LoopNest 2 */
        for (int i_6 = 0; i_6 < 12;i_6 += 1)
        {
            for (int i_7 = 0; i_7 < 12;i_7 += 1)
            {
                {
                    var_21 = ((-(((!(arr_2 [i_5 + 1] [i_7]))))));
                    var_22 = (arr_2 [i_5] [i_5]);
                }
            }
        }
        var_23 = (min(var_23, (arr_5 [i_5] [i_5])));
    }
    var_24 = var_4;
    #pragma endscop
}
