/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195551
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195551 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195551
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_4 [i_1] = ((min((arr_3 [i_0]), (arr_3 [i_0]))) - ((((min((arr_2 [i_0] [i_1]), var_17))) ? (((arr_0 [i_1]) & -30884)) : ((((-4211 + 2147483647) << (var_15 - 4208162115)))))));
                var_20 = (max(var_20, ((0 ? 65535 : ((-(min(0, 13036745914371783647))))))));
            }
        }
    }
    var_21 = var_15;
    #pragma endscop
}
