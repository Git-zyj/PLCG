/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68947
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68947 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68947
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min(var_11, var_9));
    var_13 = var_9;
    var_14 = var_10;
    var_15 = (((!var_3) >> (-206240672 + 206240698)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            {
                var_16 = (arr_2 [20]);
                /* LoopNest 2 */
                for (int i_2 = 4; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 0;i_3 += 1)
                    {
                        {
                            var_17 = var_1;
                            arr_13 [i_3] [i_1] [i_1] [i_1] [i_0] = var_6;
                            arr_14 [i_0] [i_1] [i_1] = var_5;
                        }
                    }
                }
                var_18 = 1705016487;
                arr_15 [i_1] [i_1 - 1] = (((((var_3 ? var_5 : var_5))) ? (((~(arr_5 [i_1] [i_1])))) : ((~(arr_4 [i_0] [i_1] [i_1 + 2])))));
                var_19 = (((var_8 && var_7) ? var_4 : (max((!var_11), var_5))));
            }
        }
    }
    #pragma endscop
}
