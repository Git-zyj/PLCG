/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52095
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52095 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52095
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 9;i_1 += 1)
        {
            {
                var_16 ^= (max((min(((var_0 ? 18446744073709551615 : 1)), (arr_2 [i_0] [i_0]))), (!var_12)));
                var_17 &= (~0);
            }
        }
    }
    var_18 = (min((max(((var_10 ? 1 : var_9)), -var_13)), ((max(var_8, ((11 ? var_12 : var_6)))))));
    var_19 = (max(((((max(var_4, var_12)) > -var_9))), var_1));
    var_20 |= var_12;
    #pragma endscop
}
