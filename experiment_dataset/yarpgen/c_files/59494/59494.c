/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59494
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59494 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59494
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(((((var_9 ? var_7 : var_16)))), var_16));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_5 [i_1] = (((-(arr_1 [i_0 - 2]))));
                arr_6 [i_1] [i_1] [i_1] = ((((min((arr_0 [i_0 + 1]), ((48 ? (arr_4 [i_1]) : 8))))) && (((arr_3 [i_1]) != (arr_2 [i_0 + 1] [i_0 - 2])))));
                arr_7 [i_1] [i_1] [i_1] = (((((!var_1) == ((~(arr_3 [i_1]))))) && ((!((max(var_18, 224)))))));
                arr_8 [i_0 + 1] [i_0 + 1] [i_1] = (var_14 && 23020);
            }
        }
    }
    var_20 = (min(var_20, (~var_14)));
    #pragma endscop
}
