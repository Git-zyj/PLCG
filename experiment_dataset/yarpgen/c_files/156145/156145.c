/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156145
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156145 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156145
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (((var_3 ? (var_1 * var_8) : var_0)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_18 = (((~(arr_2 [i_0] [i_1]))));
                var_19 ^= ((+(max(((min((arr_2 [i_1] [12]), (arr_4 [i_0])))), ((13 ? 1 : 32762))))));
                var_20 = (max(var_20, ((((((arr_3 [i_0]) || (arr_3 [i_0]))) ? (arr_0 [i_0] [i_1]) : (arr_0 [i_1] [i_0]))))));
                var_21 = (max(var_21, (((-((max((arr_4 [i_0]), (arr_4 [i_0])))))))));
            }
        }
    }
    var_22 = (min(var_22, (((var_5 ? var_3 : var_3)))));
    #pragma endscop
}
