/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64338
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64338 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64338
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = (~(7027029063117274413LL));
    /* LoopSeq 1 */
    for (long long int i_0 = 2; i_0 < 14; i_0 += 4) 
    {
        arr_2 [i_0] = ((/* implicit */unsigned int) arr_0 [i_0]);
        var_11 = ((/* implicit */unsigned int) ((((/* implicit */_Bool) max((((/* implicit */unsigned int) arr_1 [i_0 - 1])), (var_5)))) && (((/* implicit */_Bool) ((424391110U) ^ (((/* implicit */unsigned int) ((/* implicit */int) (short)22479))))))));
    }
}
