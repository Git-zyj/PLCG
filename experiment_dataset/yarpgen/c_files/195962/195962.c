/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195962
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195962 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195962
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (((!var_8) ? 4294967295 : 128));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_16 = var_6;
                var_17 = var_10;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 18;i_3 += 1)
                    {
                        {
                            var_18 = ((((var_7 ? ((445992875 ? var_2 : var_0)) : var_14)) / (max((max(var_0, 65393)), (arr_4 [i_0] [i_0] [i_1])))));
                            arr_8 [i_3] [i_2] [i_1] [i_0 - 1] [i_3] [i_3] = 65392;
                            var_19 = 1;
                            var_20 = ((!(((((arr_1 [i_1] [i_1]) % var_1))))));
                        }
                    }
                }
            }
        }
    }
    var_21 = (min((((((128 ? var_2 : var_10))) ? var_1 : var_6)), var_1));
    var_22 = ((var_8 && (!var_11)));
    #pragma endscop
}
