/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20594
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20594 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20594
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((max(((53 ? -8307511496398482555 : -30855)), 30855)) >> (var_11 - 219)));
    var_17 = 30837;

    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        var_18 -= (((arr_1 [i_0] [i_0]) || ((max((((arr_0 [i_0 - 1]) ? -30865 : (arr_0 [i_0 + 1]))), (arr_2 [10]))))));
        var_19 = (max(var_19, ((((((((arr_2 [i_0]) * 209))) ? ((~(arr_0 [12]))) : (min((arr_0 [i_0]), (arr_0 [i_0])))))))));
    }
    #pragma endscop
}
