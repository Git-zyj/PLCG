/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 5093
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=5093 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/5093
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        var_19 += ((arr_3 [i_0] [i_0]) ^ (((((arr_3 [i_0] [i_0]) < var_12)))));
        var_20 = ((((arr_0 [i_0]) ? (min((arr_1 [i_0]), var_0)) : var_3)));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 10;i_4 += 1)
                        {
                            {
                                var_21 -= 13861491566450949858;
                                var_22 = (var_17 ^ (((((arr_5 [i_0]) < (arr_5 [i_2]))))));
                                arr_14 [i_0] [i_1] [i_2] [i_3] [i_4] = ((arr_1 [i_0]) % var_14);
                            }
                        }
                    }
                    var_23 = (((arr_12 [i_0] [i_0] [i_1] [i_1] [i_2] [i_2]) ^ (arr_12 [i_0] [i_1] [i_1] [i_2] [i_0] [i_2])));
                }
            }
        }
    }
    var_24 = var_16;
    #pragma endscop
}
