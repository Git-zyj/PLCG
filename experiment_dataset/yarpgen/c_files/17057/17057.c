/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 17057
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=17057 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/17057
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    arr_9 [i_1] = (min((arr_2 [i_2] [i_1 + 1]), (max((((-9 ? 18 : 8))), -25868741))));
                    var_16 = (min((9223372036854775807 - 5), ((((arr_3 [i_0] [0] [i_2]) ^ (80 >> 0))))));
                    arr_10 [11] [i_1] [i_0] [5] = (arr_7 [2] [9] [i_2] [i_2]);
                    arr_11 [6] [3] [6] = ((min(2132214929, (arr_5 [4] [7] [i_1 + 1] [i_1 + 1]))));
                }
            }
        }
    }
    var_17 = var_13;
    #pragma endscop
}
