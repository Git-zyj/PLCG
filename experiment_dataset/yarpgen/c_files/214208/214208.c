/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214208
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214208 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214208
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 20;i_2 += 1)
            {
                {
                    var_16 = (min(var_16, (((~(arr_3 [i_2] [i_0] [i_0]))))));
                    var_17 = (arr_2 [i_0] [i_1]);
                    var_18 = arr_1 [i_0 - 4];
                    var_19 = (min((arr_3 [i_0 + 2] [i_0 - 2] [i_0]), ((1165469734 / (arr_1 [i_0 + 1])))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_20 |= (arr_12 [2] [i_0 - 4] [i_1] [i_2 - 1] [i_2] [i_2 + 3] [i_2 - 1]);
                                arr_13 [i_0] [i_0 + 2] [i_2] [i_4] [i_4] = ((~(((~(arr_3 [i_2 + 3] [15] [i_0 + 1]))))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_21 = ((min(var_6, (max(var_3, var_7)))) >= var_9);
    var_22 *= var_0;
    var_23 = -var_2;
    #pragma endscop
}
