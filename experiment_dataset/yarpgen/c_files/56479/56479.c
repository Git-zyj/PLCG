/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 56479
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=56479 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/56479
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_8 [i_1] [i_2] [i_1] [i_1] = 12515660445802642050;
                    var_10 = (min(var_10, (arr_0 [i_0])));
                    var_11 = max(((max((arr_2 [i_2]), (arr_2 [i_0])))), 30612);
                }
            }
        }
    }
    var_12 &= ((7100 <= (min(((1 ? 1749741541 : 4203875994193104414)), 14763071961242654658))));
    var_13 = var_4;
    #pragma endscop
}
