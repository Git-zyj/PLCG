/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72736
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72736 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72736
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (max(var_11, var_1));
    var_12 *= var_8;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_13 = (min(var_13, (arr_0 [i_0 - 1] [16])));
                arr_4 [i_0 - 1] [i_1] [i_0] = ((((!(arr_1 [i_0 - 1])))) > (arr_1 [i_0 - 1]));
                arr_5 [i_0] [i_0] [i_0] = (arr_3 [i_0 - 1] [i_1]);
                var_14 = (max(var_14, 1));
            }
        }
    }
    var_15 = ((-1350600799866413803 % ((-var_9 ? -1825653711 : ((var_3 ? var_1 : 4303400624612753071))))));
    #pragma endscop
}
