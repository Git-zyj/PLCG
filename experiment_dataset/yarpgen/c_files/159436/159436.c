/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159436
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159436 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159436
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_19 = (((var_8 ? ((15358 ? 8893466619614687119 : 1) : var_12))));
        var_20 = (min(var_20, -var_13));

        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            var_21 = var_6;
            /* LoopNest 2 */
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                for (int i_3 = 3; i_3 < 9;i_3 += 1)
                {
                    {
                        var_22 = (min(var_22, -var_17));
                        arr_10 [i_2] [i_2] [0] [i_2] [i_1] = ((!4294967295) ? var_10 : var_4);

                        /* vectorizable */
                        for (int i_4 = 0; i_4 < 11;i_4 += 1)
                        {
                            arr_15 [i_1] [i_3] = (~var_13);
                            var_23 = var_6;
                        }
                        for (int i_5 = 0; i_5 < 11;i_5 += 1)
                        {
                            arr_18 [i_0] [i_1] [i_2] [i_2] [i_3] [i_0] |= var_14;
                            var_24 = (min(var_24, ((((!(((var_13 ? var_0 : var_10)))))))));
                        }
                    }
                }
            }
        }
        for (int i_6 = 1; i_6 < 1;i_6 += 1)
        {
            var_25 |= -var_14;
            var_26 = ((-(~-var_0)));
        }
    }
    /* vectorizable */
    for (int i_7 = 0; i_7 < 18;i_7 += 1)
    {
        var_27 = -var_8;
        arr_25 [i_7] [i_7] = var_2;
    }
    var_28 = var_4;
    var_29 = (min(var_29, (!0)));
    #pragma endscop
}
