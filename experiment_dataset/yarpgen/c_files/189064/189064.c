/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 189064
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=189064 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/189064
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    var_20 = (min(var_20, (((-60 <= (arr_5 [i_1] [i_2]))))));
                    var_21 -= ((!(arr_2 [i_1])));
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 1; i_3 < 19;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 21;i_4 += 1)
        {
            {
                arr_13 [i_4] = (-59 != 36);
                var_22 = (max((arr_12 [i_3 + 2] [i_3] [i_3]), (((arr_0 [i_3 + 2]) * (arr_1 [i_3])))));
                var_23 = ((~(max((arr_0 [i_3]), (arr_9 [i_4] [i_3])))));
            }
        }
    }
    var_24 = var_2;
    var_25 = (((var_8 ? ((var_6 ? var_18 : -49)) : 0)) + var_16);
    #pragma endscop
}
