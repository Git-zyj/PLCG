/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 189038
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=189038 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/189038
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min(var_10, var_9));
    var_11 = (--46);
    var_12 = (((min(-45, var_6))) <= (min(var_7, (min(var_3, var_5)))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 15;i_1 += 1)
        {
            {
                arr_5 [i_0 + 2] [i_0] = ((~(((!(arr_3 [i_0]))))));
                arr_6 [i_0] = (((((arr_2 [i_0] [i_0]) ? (arr_1 [i_0]) : (arr_1 [i_0])))) ? ((((arr_2 [i_0] [i_0]) / (arr_1 [i_0])))) : (arr_0 [i_0] [i_0]));
                var_13 = (((arr_0 [i_0] [i_0 - 1]) > (arr_3 [i_0])));
            }
        }
    }
    #pragma endscop
}
