/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104770
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104770 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104770
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned int) ((min(((!(((/* implicit */_Bool) var_11)))), ((!(((/* implicit */_Bool) var_11)))))) ? (((/* implicit */int) ((((-3606795204122094848LL) != (((/* implicit */long long int) ((/* implicit */int) var_6))))) && (((/* implicit */_Bool) var_12))))) : (((/* implicit */int) (!(((/* implicit */_Bool) (unsigned short)65535)))))));
    /* LoopNest 2 */
    for (long long int i_0 = 1; i_0 < 12; i_0 += 3) 
    {
        for (long long int i_1 = 1; i_1 < 15; i_1 += 1) 
        {
            {
                arr_5 [i_1] &= ((/* implicit */signed char) ((((/* implicit */int) ((((/* implicit */int) ((short) (unsigned char)123))) != (((/* implicit */int) max((arr_1 [i_0] [i_0]), (arr_0 [i_1 - 1] [i_0 + 2]))))))) <= ((-(((/* implicit */int) (short)-8873))))));
                var_21 = ((/* implicit */unsigned char) max((var_21), (((/* implicit */unsigned char) arr_3 [i_0] [i_0]))));
            }
        } 
    } 
    var_22 = ((/* implicit */int) (short)(-32767 - 1));
    var_23 = ((/* implicit */unsigned int) var_12);
}
