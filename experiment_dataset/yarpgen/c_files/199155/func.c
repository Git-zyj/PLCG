/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199155
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199155 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199155
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */unsigned char) var_12);
    var_20 = ((/* implicit */_Bool) var_10);
    var_21 = ((/* implicit */signed char) max((var_21), (((/* implicit */signed char) var_18))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22; i_0 += 2) 
    {
        for (int i_1 = 1; i_1 < 18; i_1 += 2) 
        {
            {
                arr_5 [i_1 - 1] [4U] [i_0] = ((/* implicit */unsigned long long int) max(((signed char)-2), (((/* implicit */signed char) arr_3 [i_1 + 3] [i_1] [i_1 + 1]))));
                var_22 *= ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) max((((/* implicit */short) (_Bool)0)), ((short)-11576)))) ? (((/* implicit */int) min((var_12), (((/* implicit */signed char) arr_3 [i_1 + 1] [i_1 + 3] [i_1 - 1]))))) : (((((/* implicit */int) arr_3 [i_1] [i_1] [i_1 + 4])) % (((/* implicit */int) (unsigned short)14827))))));
            }
        } 
    } 
}
