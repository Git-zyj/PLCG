/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 19093
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=19093 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/19093
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 19;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_1] = (max((((((arr_1 [i_0]) ? 247 : (arr_0 [1]))))), (((arr_2 [i_1] [1]) ? (arr_2 [i_1] [i_1]) : (((2965621179 ? 10 : -1)))))));
                var_12 = (min(((min((arr_0 [i_0 - 1]), (arr_0 [i_0 - 1])))), ((-1320769089 ? 5057632753060396845 : 5057632753060396845))));
            }
        }
    }
    var_13 = ((((~((var_8 ? var_4 : var_5)))) % (~var_4)));
    #pragma endscop
}
