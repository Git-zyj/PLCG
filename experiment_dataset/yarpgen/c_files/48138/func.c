/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48138
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48138 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48138
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = min(((((-(var_6))) + (((/* implicit */long long int) ((/* implicit */int) ((var_3) >= (var_8))))))), ((+((-(-122175058504713543LL))))));
    var_13 = ((/* implicit */_Bool) (+(var_6)));
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 21; i_0 += 1) 
    {
        for (long long int i_1 = 3; i_1 < 20; i_1 += 1) 
        {
            {
                arr_5 [2LL] = (!(((/* implicit */_Bool) 122175058504713542LL)));
                arr_6 [i_0] = ((((/* implicit */_Bool) 2725610195145301651LL)) ? (var_5) : (-3289952665540255531LL));
            }
        } 
    } 
    var_14 = ((/* implicit */_Bool) var_3);
}
