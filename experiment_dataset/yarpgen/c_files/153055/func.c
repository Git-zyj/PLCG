/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 153055
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=153055 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/153055
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = (_Bool)1;
    /* LoopNest 2 */
    for (long long int i_0 = 2; i_0 < 21; i_0 += 1) 
    {
        for (short i_1 = 4; i_1 < 21; i_1 += 2) 
        {
            {
                var_21 = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) ((((/* implicit */_Bool) (unsigned short)49451)) ? (((/* implicit */unsigned int) ((/* implicit */int) (short)-32757))) : (min((((/* implicit */unsigned int) (signed char)-88)), (4294967295U)))))) ? (((/* implicit */int) ((unsigned short) (-(((/* implicit */int) (_Bool)1)))))) : ((-(((/* implicit */int) (_Bool)0))))));
                var_22 = (~(((/* implicit */int) (short)1)));
            }
        } 
    } 
    var_23 = ((/* implicit */unsigned short) (signed char)77);
}
