/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80868
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80868 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80868
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_11 -= (arr_0 [i_2] [i_3]);
                                arr_13 [i_0] [i_2] |= var_5;
                                arr_14 [i_0] [1] [6] [i_0] [i_0] = (var_4 <= 17592186042368);
                            }
                        }
                    }
                }
                var_12 = (max(var_12, (((((((var_9 < (arr_12 [i_0] [i_0] [4] [i_1]))))) ? var_2 : ((min(18446726481523509256, 10))))))));
            }
        }
    }
    #pragma endscop
}
