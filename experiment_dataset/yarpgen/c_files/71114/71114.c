/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71114
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71114 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71114
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((((((var_6 * var_0) >= var_9))) >= (!var_9)));
    var_14 |= var_9;

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_15 = (~(min(-0, var_4)));
        arr_2 [i_0] [16] = -2336280604050102747;
        arr_3 [1] [i_0] = ((((((!(((arr_1 [i_0]) > -1)))))) > ((max(1, -2336280604050102747)))));
    }
    /* vectorizable */
    for (int i_1 = 1; i_1 < 13;i_1 += 1)
    {
        arr_6 [i_1] [i_1] = 1;

        for (int i_2 = 0; i_2 < 14;i_2 += 1)
        {
            var_16 = (arr_5 [7] [i_2]);
            /* LoopNest 2 */
            for (int i_3 = 4; i_3 < 11;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 14;i_4 += 1)
                {
                    {
                        var_17 = (max(var_17, var_6));
                        var_18 = (min(var_18, (var_12 | var_8)));
                        arr_15 [2] [i_1] [i_3 + 2] = (((((!(arr_1 [i_2])))) & (arr_9 [i_3 + 2])));
                    }
                }
            }
            arr_16 [i_1] [i_2] [i_1] = (!-var_0);
        }
        arr_17 [i_1] = var_8;
    }
    var_19 = (((((-(0 + var_7)))) ? (max(var_10, (min(var_10, var_2)))) : (+-2494815243097729384)));
    #pragma endscop
}
