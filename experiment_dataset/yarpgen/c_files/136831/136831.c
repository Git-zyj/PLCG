/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136831
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136831 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136831
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 &= ((-(221 >> 27)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            {
                arr_7 [i_0] [i_0] [i_0] = (((((((arr_5 [4] [i_0] [i_1]) ? (arr_1 [5]) : 214)) & -4)) + 9));
                /* LoopNest 3 */
                for (int i_2 = 1; i_2 < 7;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 9;i_4 += 1)
                        {
                            {
                                arr_14 [i_0] [i_1] [i_2 + 2] [6] [i_4] = ((((((arr_2 [i_1 + 1] [i_1 + 2]) >> (((arr_2 [i_1 + 1] [i_2 + 1]) - 98))))) ? ((((38 ? (arr_12 [i_0] [i_0] [i_0] [i_0] [2] [i_0] [8]) : 64)) - 146)) : (((!(arr_11 [i_0]))))));
                                var_11 = (min(var_11, 190));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
