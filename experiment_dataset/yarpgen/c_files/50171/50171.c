/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 50171
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=50171 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/50171
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {

                /* vectorizable */
                for (int i_2 = 0; i_2 < 22;i_2 += 1)
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 22;i_4 += 1)
                        {
                            {
                                var_18 = var_7;
                                var_19 = ((!(arr_7 [i_0] [i_1] [i_2])));
                                var_20 = (arr_9 [i_3] [i_1]);
                            }
                        }
                    }
                    arr_12 [i_2] [i_1] [6] [i_0 + 1] |= (((arr_1 [i_0 - 3] [12]) ? -var_12 : (((arr_6 [i_0] [i_0 - 1] [i_0 - 1]) ? var_9 : var_0))));
                    var_21 = ((~((43 ? 512 : -29377))));
                }
                var_22 = var_7;
            }
        }
    }
    /* LoopNest 2 */
    for (int i_5 = 0; i_5 < 21;i_5 += 1)
    {
        for (int i_6 = 4; i_6 < 20;i_6 += 1)
        {
            {
                arr_18 [i_5] = var_10;
                arr_19 [i_5] = ((((((((~(arr_15 [i_5])))) ? (((10311607184900529454 ? 7193 : 29360))) : (((arr_0 [i_5] [i_6]) ^ 15))))) ? (((arr_17 [i_6 - 4] [i_6 - 2] [i_6]) ? 1 : 57256)) : var_8));
            }
        }
    }
    #pragma endscop
}
