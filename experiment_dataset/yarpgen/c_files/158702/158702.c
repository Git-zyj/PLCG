/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158702
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158702 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158702
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_16 = (max(var_16, var_11));
        arr_2 [i_0] = ((~(arr_0 [i_0])));
        arr_3 [i_0] = ((var_5 ? (((arr_0 [i_0]) ? (arr_0 [i_0]) : (arr_1 [i_0] [i_0]))) : var_5));
        arr_4 [i_0] [i_0] = var_0;
        arr_5 [i_0] [i_0] = (arr_1 [i_0] [i_0]);
    }
    for (int i_1 = 0; i_1 < 20;i_1 += 1)
    {
        var_17 += ((var_6 ? (!var_6) : var_10));
        arr_9 [i_1] = ((((((((9223372036854775807 ? (arr_6 [i_1]) : 9223372036854775807))) ? (!var_9) : (arr_6 [i_1])))) ? ((+(((arr_6 [i_1]) & var_0)))) : (arr_7 [i_1])));
        var_18 ^= ((!(arr_6 [i_1])));
    }
    var_19 += (min(var_0, var_10));
    #pragma endscop
}
