/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 147390
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=147390 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/147390
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max(var_9, var_13));
    var_16 = (((max(var_12, var_1)) | var_8));

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        var_17 += (((var_3 ? (max((arr_0 [10]), var_4)) : 7878)));

        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            /* LoopNest 2 */
            for (int i_2 = 2; i_2 < 19;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 21;i_3 += 1)
                {
                    {
                        var_18 = 1;
                        var_19 = -var_5;
                        var_20 &= 1;
                    }
                }
            }
            arr_12 [i_0] [i_0] [i_0] = ((var_0 ? 1 : (((!var_13) ^ ((max(var_14, var_1)))))));
            var_21 = ((var_14 > (var_1 + var_7)));
        }
        var_22 &= (max(((var_14 ^ (arr_4 [i_0]))), var_5));
    }
    #pragma endscop
}
