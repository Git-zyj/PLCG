/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148672
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148672 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148672
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = -3434485689243596000;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                var_13 = ((((((0 ? var_1 : var_8)))) ? var_1 : (((var_5 ? var_1 : var_6)))));
                                arr_13 [i_0] = ((((min((max(var_6, 1073741824)), (((127 ? var_0 : 43623)))))) ? var_7 : (!var_4)));
                                arr_14 [i_4] [5] [i_4] [1] [i_4] [i_0] = ((-var_6 ? -var_0 : (~var_2)));
                            }
                        }
                    }
                }
                var_14 = ((~((var_7 - (!var_6)))));
                arr_15 [i_0] [i_0] [i_1] = ((~(max(var_10, -113))));
                var_15 = ((!(~4095)));
                var_16 = var_1;
            }
        }
    }
    #pragma endscop
}
