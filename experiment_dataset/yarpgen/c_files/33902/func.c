/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33902
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33902 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33902
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */signed char) var_4);
    /* LoopNest 3 */
    for (short i_0 = 1; i_0 < 9; i_0 += 3) 
    {
        for (short i_1 = 0; i_1 < 12; i_1 += 1) 
        {
            for (unsigned int i_2 = 0; i_2 < 12; i_2 += 4) 
            {
                {
                    var_13 = ((/* implicit */short) ((((/* implicit */unsigned long long int) ((/* implicit */int) arr_1 [i_0] [i_1]))) * (((((/* implicit */unsigned long long int) ((/* implicit */int) arr_5 [i_0] [i_1]))) - (((unsigned long long int) arr_2 [i_1] [i_2]))))));
                    arr_7 [i_2] [i_1] [i_1] [i_0] = ((/* implicit */unsigned char) ((((int) arr_5 [i_0] [i_1])) % (((/* implicit */int) arr_0 [i_0]))));
                }
            } 
        } 
    } 
}
