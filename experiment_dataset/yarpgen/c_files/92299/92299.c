/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 92299
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=92299 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/92299
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_5 [i_0] [14] [14] = var_11;
                var_20 += (((((arr_4 [i_0]) >= 1))) | (((arr_2 [i_0] [i_0]) ? var_6 : ((var_17 ? (arr_3 [i_1]) : (arr_0 [i_1]))))));
                var_21 = (~((((((-127 - 1) / 508))) & (~var_19))));
                arr_6 [i_0] [i_0] [i_1] = ((((2781287959 ? 65028 : (-9223372036854775807 - 1)))) ? (arr_3 [i_1]) : (arr_1 [i_0]));
            }
        }
    }
    var_22 = var_1;
    var_23 = var_0;
    #pragma endscop
}
