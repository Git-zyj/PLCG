/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132468
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132468 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132468
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 ^= ((/* implicit */signed char) var_11);
    /* LoopNest 2 */
    for (long long int i_0 = 1; i_0 < 8; i_0 += 2) 
    {
        for (unsigned char i_1 = 1; i_1 < 9; i_1 += 2) 
        {
            {
                var_20 = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) arr_1 [i_0] [9U])) ? (max((arr_2 [i_1 + 2]), (((/* implicit */unsigned int) var_5)))) : (((/* implicit */unsigned int) ((/* implicit */int) arr_1 [i_0] [i_0])))));
                arr_4 [i_0] = ((/* implicit */_Bool) var_0);
            }
        } 
    } 
    var_21 = ((/* implicit */unsigned short) var_3);
    var_22 &= ((/* implicit */unsigned int) var_11);
}
