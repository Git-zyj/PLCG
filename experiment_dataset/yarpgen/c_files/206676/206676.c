/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206676
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206676 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206676
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_12;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_14 = (min(var_14, var_2));
                var_15 = (min(var_15, ((((min((arr_1 [12]), (((arr_3 [i_0] [1] [i_1]) ? var_11 : 345682771)))) + ((var_4 ? (max(var_8, var_10)) : (((345682771 ? (arr_0 [i_0]) : -7862157763680428598))))))))));
                arr_5 [i_0] [i_1] = ((-((var_4 ? (arr_4 [i_0]) : (arr_4 [i_0])))));
            }
        }
    }
    #pragma endscop
}
