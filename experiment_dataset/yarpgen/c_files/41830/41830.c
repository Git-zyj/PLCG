/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41830
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41830 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41830
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(var_11, (~var_7)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            {
                arr_5 [8] [i_1] [i_1] = 4294967295;
                var_19 = (min(((~((((-9223372036854775807 - 1) == (arr_4 [i_1])))))), ((min((arr_0 [i_1 - 2]), 1)))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 13;i_3 += 1)
                    {
                        {
                            arr_12 [i_0] [i_1] [i_1] [i_3 + 1] [i_3] [i_3] = ((~(min(-1, var_16))));
                            var_20 = max(-var_15, (((~(arr_11 [i_0] [i_0] [5] [i_1] [i_3 + 1])))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
