/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234996
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234996 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234996
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned short i_0 = 2; i_0 < 20; i_0 += 4) 
    {
        var_17 = ((/* implicit */_Bool) ((((/* implicit */int) (unsigned short)2600)) - ((+((+(((/* implicit */int) (unsigned short)33813))))))));
        var_18 |= ((/* implicit */unsigned int) min((min((((/* implicit */int) var_4)), (max((864422836), (1760722007))))), (((/* implicit */int) var_15))));
    }
    var_19 = ((/* implicit */unsigned int) var_1);
}
