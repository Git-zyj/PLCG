/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203625
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203625 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203625
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 17;i_2 += 1)
            {
                {
                    arr_9 [1] [1] [i_0] [i_2] &= ((((((((arr_1 [i_0]) > var_5))) >= (arr_7 [i_0] [i_1] [i_2 - 1]))) ? (arr_7 [i_2 - 1] [i_2 - 1] [i_2 - 1]) : (((((max(var_4, var_2)) == (arr_4 [i_1])))))));
                    arr_10 [i_1] = (min(((min((arr_7 [i_0] [3] [i_0]), (((var_2 + 2147483647) >> (((arr_2 [i_0] [i_0]) - 51529))))))), (arr_7 [i_0] [i_1] [i_2 + 1])));
                }
            }
        }
    }
    var_10 *= var_1;
    #pragma endscop
}
