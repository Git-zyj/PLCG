/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73973
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73973 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73973
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */signed char) var_11);
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (long long int i_1 = 1; i_1 < 10; i_1 += 4) 
        {
            {
                var_20 &= ((/* implicit */long long int) var_12);
                var_21 = ((/* implicit */unsigned short) ((((((/* implicit */unsigned int) ((var_7) ? (((/* implicit */int) arr_0 [i_0] [i_0])) : (((/* implicit */int) arr_1 [i_1 + 1]))))) ^ (max((var_16), (((/* implicit */unsigned int) var_7)))))) - (var_0)));
            }
        } 
    } 
}
