/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46410
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46410 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46410
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            {

                for (int i_2 = 1; i_2 < 14;i_2 += 1)
                {
                    arr_8 [i_2] [i_0] [i_0] [i_0] = ((((-(arr_0 [i_0] [i_1 + 1])))));
                    var_18 = (min(515382390, (var_1 || 3424768769268491673)));
                }
                var_19 -= (min((arr_7 [12] [12] [1] [12]), 286318018));

                for (int i_3 = 0; i_3 < 15;i_3 += 1)
                {
                    var_20 = (min(var_20, var_17));
                    var_21 = ((!((((arr_3 [i_1 + 1]) ? (arr_3 [i_1 + 1]) : (arr_7 [i_0] [i_0] [i_3] [2]))))));
                }
                arr_12 [i_0] [i_0] = ((-(arr_3 [i_1 - 1])));
            }
        }
    }
    var_22 = var_16;
    var_23 = (((~(~var_2))));
    #pragma endscop
}
