/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102329
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102329 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102329
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */unsigned long long int) max((var_17), (max((((/* implicit */unsigned long long int) (~((-(((/* implicit */int) var_16))))))), (2826441205035393355ULL)))));
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 2; i_0 < 18; i_0 += 3) 
    {
        var_18 ^= ((/* implicit */unsigned long long int) arr_1 [i_0 - 2] [i_0 + 1]);
        arr_3 [i_0] = ((/* implicit */unsigned long long int) arr_0 [i_0 - 1]);
        var_19 *= (unsigned short)18754;
        var_20 += ((/* implicit */unsigned short) ((int) 16737830555951024882ULL));
    }
}
