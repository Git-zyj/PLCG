/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213152
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213152 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213152
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 19;i_2 += 1)
            {
                {
                    var_13 = ((((min(var_8, (arr_5 [i_2 - 2] [i_2] [i_2 - 2])))) ? (arr_5 [i_2 - 1] [i_2 - 1] [i_2 - 1]) : (((arr_5 [i_2] [20] [i_2 - 2]) >> (((arr_5 [i_2 + 2] [i_2 - 2] [i_2 - 2]) - 5073))))));
                    var_14 = ((((min(var_3, (arr_5 [i_0] [i_0] [i_0 - 2])))) ? var_0 : (var_3 == var_2)));
                    arr_8 [i_0] [i_1] [i_0] [i_0] = (0 - 10);
                }
            }
        }
    }
    var_15 = var_10;
    var_16 = var_9;
    var_17 = var_12;
    #pragma endscop
}
