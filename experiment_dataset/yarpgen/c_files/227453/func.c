/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227453
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227453 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227453
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */short) min((var_19), (((/* implicit */short) var_5))));
    var_20 = ((/* implicit */_Bool) var_14);
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (short i_1 = 2; i_1 < 23; i_1 += 2) 
        {
            for (signed char i_2 = 0; i_2 < 24; i_2 += 3) 
            {
                {
                    arr_7 [i_0] [i_0] [i_0] = ((/* implicit */unsigned long long int) ((((/* implicit */int) arr_2 [i_1 - 2] [i_1 - 1] [i_0])) == (((/* implicit */int) var_10))));
                    var_21 = ((/* implicit */unsigned long long int) min((var_21), (((/* implicit */unsigned long long int) ((((10843036298489547895ULL) ^ (((/* implicit */unsigned long long int) ((/* implicit */int) (signed char)-41))))) <= (((/* implicit */unsigned long long int) ((/* implicit */int) (signed char)-113))))))));
                }
            } 
        } 
    } 
}
