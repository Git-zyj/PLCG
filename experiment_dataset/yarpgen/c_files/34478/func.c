/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34478
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34478 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34478
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned short i_0 = 0; i_0 < 20; i_0 += 2) 
    {
        for (long long int i_1 = 0; i_1 < 20; i_1 += 4) 
        {
            {
                var_20 += ((/* implicit */unsigned long long int) arr_7 [i_1] [8] [8]);
                var_21 = min((((/* implicit */unsigned short) (!(((/* implicit */_Bool) 1086099317U))))), (arr_7 [i_0] [i_0] [i_0]));
                var_22 = ((/* implicit */unsigned int) max((var_22), (((/* implicit */unsigned int) var_15))));
                arr_8 [i_1] = ((/* implicit */unsigned long long int) 1086099330U);
            }
        } 
    } 
    var_23 = ((/* implicit */unsigned long long int) var_7);
}
