/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61539
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61539 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61539
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_1] [i_0] = min((arr_5 [i_1 + 1] [i_1 + 3]), ((min(3, 3))));
                var_20 |= (!var_9);
            }
        }
    }
    var_21 = ((((min(var_4, 3))) ? var_19 : (max((min(0, 12)), (((var_15 ? var_2 : 127)))))));

    for (int i_2 = 0; i_2 < 22;i_2 += 1)
    {
        var_22 = arr_9 [i_2];

        for (int i_3 = 0; i_3 < 22;i_3 += 1)
        {
            var_23 = (arr_7 [i_2] [i_3]);
            var_24 = (((((arr_11 [i_2]) ? (arr_11 [i_2]) : (arr_11 [i_2]))) >> (((!var_17) >> ((((max((arr_11 [i_3]), var_4))) - 18972))))));
        }
    }
    #pragma endscop
}
