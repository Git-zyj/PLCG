/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127859
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127859 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127859
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((max((max(var_0, 73)), -var_11)) + var_11));
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0 - 1] [i_1] = (min(-13504, ((-(arr_0 [i_1])))));
                arr_6 [i_0] [i_0] [7] = (min((arr_3 [i_0] [i_0]), ((-((max(54917, (arr_0 [i_0]))))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 0;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            arr_11 [i_0] |= (((arr_0 [i_0 - 2]) ? 18446744073709551604 : (((min((arr_0 [i_0 + 2]), (arr_0 [i_0 + 3])))))));
                            arr_12 [i_2] [i_3] [i_3] [i_3] = var_2;
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
