/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47023
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47023 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47023
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (((var_11 / var_12) % var_11));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_14 = (((arr_13 [i_1] [i_0]) + (min(0, (arr_13 [i_0] [i_0])))));
                                var_15 = ((var_2 ? ((~(arr_3 [i_0]))) : (arr_10 [i_4] [i_4] [i_2])));
                                var_16 = (min(var_16, (((((((arr_13 [i_0] [i_4]) / (((arr_12 [i_4] [i_3] [i_2] [i_0] [i_0]) ? -2 : (arr_1 [i_0 + 2])))))) / (51 + -3677418138329807901))))));
                                var_17 = arr_12 [i_0] [i_0] [i_0 + 1] [i_0] [i_0 + 2];
                                arr_14 [i_0 - 1] [i_0] [i_2] [i_3] [i_4] = arr_13 [i_0] [i_0];
                            }
                        }
                    }
                    var_18 = ((max(1944952167, var_6)));
                    var_19 = (arr_3 [1]);
                }
            }
        }
    }
    var_20 = var_0;
    var_21 = (((var_11 ? -32 : (var_2 / var_3))));
    #pragma endscop
}
