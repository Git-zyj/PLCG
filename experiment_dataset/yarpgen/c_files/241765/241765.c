/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241765
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241765 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241765
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = ((((max((min(var_9, var_1)), (2147483647 <= 204)))) && (((~((min(36, -101))))))));
        var_14 = (max(var_14, 12));
        var_15 = (arr_0 [i_0]);
    }
    var_16 = -110;

    for (int i_1 = 1; i_1 < 18;i_1 += 1)
    {
        var_17 = 93;
        var_18 = ((65523 ? (arr_3 [i_1 + 1]) : ((~(arr_3 [i_1 + 2])))));
        var_19 = (min(var_19, (arr_5 [i_1 + 1] [i_1])));
    }
    for (int i_2 = 3; i_2 < 18;i_2 += 1)
    {
        arr_8 [i_2] = (arr_7 [i_2]);
        var_20 = -1;
        var_21 = (min(var_21, var_0));
        arr_9 [i_2 - 3] [i_2] = (~var_12);
        var_22 = (min((((2147483647 ? (arr_6 [i_2]) : ((var_3 ? (-2147483647 - 1) : var_2))))), (arr_6 [i_2 + 2])));
    }
    var_23 = ((min(var_5, var_1)));
    #pragma endscop
}
