/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86229
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86229 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86229
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 16; i_0 += 2) 
    {
        arr_2 [i_0] = ((/* implicit */short) (!(((/* implicit */_Bool) (-((+(((/* implicit */int) (short)32767)))))))));
        arr_3 [i_0] = var_10;
    }
    var_15 = ((/* implicit */unsigned long long int) var_10);
    /* LoopNest 3 */
    for (unsigned short i_1 = 0; i_1 < 22; i_1 += 2) 
    {
        for (signed char i_2 = 0; i_2 < 22; i_2 += 1) 
        {
            for (int i_3 = 2; i_3 < 21; i_3 += 2) 
            {
                {
                    arr_12 [i_1] [i_2] [i_1] = ((/* implicit */unsigned char) (-((-(((/* implicit */int) arr_10 [i_2] [(_Bool)1] [i_2] [i_2]))))));
                    var_16 = ((/* implicit */signed char) var_3);
                    var_17 = ((/* implicit */signed char) (-(((((/* implicit */_Bool) arr_10 [i_2] [i_3 - 1] [i_3 - 2] [i_3 - 1])) ? (((/* implicit */int) arr_10 [i_2] [i_3] [i_3 - 1] [i_3 - 1])) : (((/* implicit */int) arr_10 [i_2] [i_3 - 2] [i_3 - 2] [i_3 + 1]))))));
                }
            } 
        } 
    } 
}
