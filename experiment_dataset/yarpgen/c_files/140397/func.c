/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 140397
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=140397 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/140397
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */long long int) var_1);
    var_20 = ((/* implicit */int) max((var_20), (((/* implicit */int) var_3))));
    /* LoopNest 3 */
    for (unsigned short i_0 = 0; i_0 < 22; i_0 += 4) 
    {
        for (long long int i_1 = 3; i_1 < 21; i_1 += 1) 
        {
            for (unsigned short i_2 = 1; i_2 < 21; i_2 += 3) 
            {
                {
                    var_21 = ((/* implicit */unsigned char) max((var_21), (((/* implicit */unsigned char) var_16))));
                    var_22 += ((/* implicit */unsigned int) var_0);
                    /* LoopNest 2 */
                    for (unsigned char i_3 = 2; i_3 < 21; i_3 += 4) 
                    {
                        for (long long int i_4 = 1; i_4 < 20; i_4 += 2) 
                        {
                            {
                                var_23 = ((/* implicit */unsigned int) max((((((/* implicit */_Bool) var_6)) ? (((/* implicit */int) (unsigned short)36860)) : (((/* implicit */int) ((((/* implicit */int) var_2)) == (var_0)))))), (577571250)));
                                arr_13 [i_0] [i_0] [i_2] [i_0] [i_0] = ((/* implicit */unsigned int) ((((int) ((((/* implicit */int) var_2)) - (((/* implicit */int) var_2))))) - (((((/* implicit */int) (signed char)-95)) + (((/* implicit */int) min((((/* implicit */unsigned char) (signed char)94)), ((unsigned char)250))))))));
                                var_24 = (unsigned short)3727;
                            }
                        } 
                    } 
                }
            } 
        } 
    } 
    var_25 = ((/* implicit */unsigned char) ((((/* implicit */int) ((_Bool) min((((/* implicit */unsigned int) -719571641)), (var_3))))) * (((/* implicit */int) min((((/* implicit */unsigned short) min(((unsigned char)0), ((unsigned char)16)))), ((unsigned short)18460))))));
}
