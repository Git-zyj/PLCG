/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22144
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22144 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22144
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (~var_2);
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_19 = (~944407430806340157);
                arr_5 [i_0] [i_0 - 3] [i_0] = 1;
                var_20 = (min(var_20, (((max(1, -944407430806340157))))));
                var_21 = (min(var_21, (((((var_6 + 0) / var_11))))));
            }
        }
    }
    var_22 = (max((((-(33158 - 516096)))), var_11));
    var_23 = (max((~-var_7), (var_8 % var_16)));
    #pragma endscop
}
