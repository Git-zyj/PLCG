/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130915
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130915 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130915
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((min(var_11, ((var_7 ? var_12 : var_2)))));
    var_18 = (((min(var_11, var_5))));
    var_19 = var_3;
    var_20 = ((-(((!((max(12, 1))))))));

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        arr_2 [i_0] = (((min((arr_0 [i_0]), (arr_1 [i_0]))) - (((min((arr_0 [i_0]), (arr_0 [i_0])))))));
        var_21 = (min(((~(arr_1 [i_0]))), ((+(min((arr_1 [i_0]), (arr_1 [i_0])))))));
        arr_3 [i_0] = -1;
    }
    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        arr_6 [i_1] [i_1] = (arr_4 [i_1]);
        var_22 = (((!(((-(arr_1 [i_1])))))));
        var_23 = (((((arr_5 [i_1] [i_1]) & (arr_5 [i_1] [i_1]))) / (arr_5 [i_1] [i_1])));
        var_24 = ((!(((~(((arr_5 [i_1] [i_1]) ? (arr_0 [i_1]) : (arr_5 [i_1] [i_1]))))))));
    }
    #pragma endscop
}
