/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208114
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208114 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208114
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            {
                var_15 = (((min(var_6, (var_5 >= var_1)))) > (((min(0, -3395946008990331353)))));
                arr_5 [i_0] [0] = ((~(min(((((arr_0 [i_0]) | (arr_3 [i_0])))), var_9))));
                var_16 = (((((!(arr_0 [i_0 + 2])))) | 32758));
            }
        }
    }

    for (int i_2 = 0; i_2 < 12;i_2 += 1)
    {
        var_17 = (min(var_17, (-9223372036854775807 - 1)));
        arr_8 [11] [i_2] = (min((((500873044 >= var_8) - (arr_7 [i_2]))), ((((((arr_6 [i_2]) == 0)) && (((var_12 / (arr_3 [i_2])))))))));
    }
    #pragma endscop
}
