/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208731
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208731 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208731
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_8;
    var_19 -= ((var_7 ? var_10 : (((var_3 / var_8) ? (var_16 && var_3) : ((var_1 ? var_4 : var_2))))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 24;i_1 += 1)
        {
            {
                var_20 = (arr_4 [i_0] [i_0 - 1] [i_1]);
                var_21 = (max(var_21, ((((arr_4 [23] [i_1 - 3] [i_0]) ^ ((((-(arr_0 [i_0]))) << (((((arr_0 [i_1]) + (arr_3 [i_0] [i_0]))) - 123075090442181583)))))))));
            }
        }
    }
    var_22 = ((var_1 ? var_7 : var_14));
    #pragma endscop
}
