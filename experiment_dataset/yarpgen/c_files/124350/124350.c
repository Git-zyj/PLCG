/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124350
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124350 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124350
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        var_17 = ((max((min((arr_1 [i_0]), 8388607)), (max((arr_1 [i_0]), (arr_0 [i_0]))))));
        /* LoopNest 3 */
        for (int i_1 = 4; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 9;i_3 += 1)
                {
                    {
                        arr_12 [i_3] [i_2] [i_3] = (arr_6 [i_2]);
                        arr_13 [i_1] [i_3] = (arr_11 [i_1 - 4] [i_1] [i_1] [i_1] [i_1]);
                        arr_14 [i_3] [i_2] = max((((!(arr_11 [i_1 - 1] [i_1] [i_1 - 4] [i_1] [i_1])))), (arr_4 [i_3 - 1] [i_1 - 4] [i_1 - 2]));
                        var_18 = (max((arr_5 [i_2]), ((-(arr_0 [i_0])))));
                    }
                }
            }
        }
    }
    var_19 -= var_1;
    var_20 = (max(var_20, var_13));
    #pragma endscop
}
