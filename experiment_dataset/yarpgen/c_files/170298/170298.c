/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 170298
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=170298 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/170298
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 4; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        {
                            var_13 ^= var_4;
                            arr_11 [i_0] [i_0] [i_0] [i_0] |= (((2963 && (arr_3 [i_1] [i_2 - 3]))));
                            var_14 = ((!(((var_7 ? var_12 : (arr_0 [i_2 - 2]))))));
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_4 = 1; i_4 < 14;i_4 += 1)
                {
                    for (int i_5 = 3; i_5 < 14;i_5 += 1)
                    {
                        {
                            var_15 = arr_13 [4] [i_1] [i_4 - 1] [i_4 - 1];
                            var_16 = (arr_12 [i_0] [i_1] [i_1] [i_1]);
                        }
                    }
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_6 = 1; i_6 < 22;i_6 += 1)
    {
        for (int i_7 = 0; i_7 < 23;i_7 += 1)
        {
            for (int i_8 = 0; i_8 < 23;i_8 += 1)
            {
                {
                    arr_26 [i_6] [i_7] [i_8] = ((!((max(173, var_6)))));
                    var_17 = var_5;
                    var_18 = (44 || 144);
                }
            }
        }
    }
    #pragma endscop
}
