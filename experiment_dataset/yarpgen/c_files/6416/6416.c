/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6416
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6416 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6416
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (((((var_11 ^ var_12) ? var_6 : var_1)) >> (4095 - 4075)));

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = var_6;
        var_14 = (((4092 ? (min((arr_1 [i_0] [i_0]), 5414257346336874159)) : (((((arr_0 [i_0] [i_0]) || var_0)))))));
    }
    var_15 += ((((var_2 ? (~var_1) : (var_7 && 61423))) >> (var_6 - 5996854481565455201)));
    #pragma endscop
}
