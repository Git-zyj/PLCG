/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7204
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7204 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7204
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = max((((max(0, 1)))), (max((((~(-127 - 1)))), var_3)));

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_14 = ((~(((((((arr_0 [i_0]) && var_3))) != (arr_0 [16]))))));
        var_15 = 12646832138655324575;
        var_16 = ((max(-5280780689901317719, -0)));
    }

    for (int i_1 = 0; i_1 < 12;i_1 += 1)
    {
        var_17 *= (((((~(-127 - 1)))) ? ((((min((arr_4 [i_1] [i_1]), (arr_1 [11]))) > (~var_9)))) : (55888 & 5022)));
        var_18 += (max((((((~(arr_3 [i_1]))) >= 0))), (arr_3 [i_1])));
    }
    var_19 = var_7;
    #pragma endscop
}
