/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32507
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32507 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32507
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (~var_3);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 8;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_0] = max((((65535 / 428378085) / (((arr_7 [i_0] [i_0] [i_0] [i_2]) * (arr_6 [i_1] [i_1 - 1] [i_1] [i_0]))))), var_16);
                    var_20 *= (min(((var_17 ? ((max(var_16, (arr_6 [i_1 - 2] [i_1 - 2] [i_1 - 2] [i_1 - 2])))) : (max(var_17, var_10)))), var_1));
                }
            }
        }
    }
    #pragma endscop
}
