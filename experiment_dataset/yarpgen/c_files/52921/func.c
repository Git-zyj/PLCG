/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52921
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52921 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52921
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned char i_0 = 1; i_0 < 22; i_0 += 3) 
    {
        for (signed char i_1 = 0; i_1 < 23; i_1 += 4) 
        {
            {
                var_17 = ((((/* implicit */_Bool) (~(((/* implicit */int) (signed char)-121))))) ? (((/* implicit */int) ((signed char) var_3))) : (((/* implicit */int) ((_Bool) max(((short)15945), (((/* implicit */short) arr_0 [i_0])))))));
                var_18 = ((/* implicit */short) (((~(((/* implicit */int) min((var_4), (((/* implicit */unsigned char) (signed char)-121))))))) < (var_3)));
                var_19 = ((/* implicit */short) (~((-(var_0)))));
            }
        } 
    } 
    var_20 ^= ((/* implicit */_Bool) var_13);
}
