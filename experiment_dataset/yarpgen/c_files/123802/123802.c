/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123802
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123802 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123802
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_20 = ((var_2 ? (arr_3 [i_0] [i_0] [i_0]) : var_0));
                arr_5 [i_0] [i_0] [i_0] = ((2692968404 ? ((((59898 ? (arr_0 [i_0]) : var_10)))) : (((arr_3 [i_0] [i_0] [i_0]) ? (arr_3 [i_0] [i_0] [i_0]) : (arr_4 [i_0] [i_1] [i_1])))));
                var_21 = (((((((arr_2 [i_0]) ? (arr_3 [i_1] [i_1] [i_1]) : 1))) ? (arr_1 [i_0]) : ((var_4 ? var_15 : (arr_0 [i_1]))))));
            }
        }
    }
    var_22 ^= var_16;
    #pragma endscop
}
