/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164463
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164463 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164463
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_3;
    var_16 = var_8;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [i_2] &= ((((((arr_1 [i_0] [i_2]) ? ((((arr_2 [i_0]) ? (arr_6 [i_0] [i_0] [i_0]) : var_9))) : (((arr_0 [i_2 - 1]) ? -23581 : (arr_7 [i_0])))))) ? ((((3401815420 ? (arr_2 [i_0]) : (arr_5 [i_1]))))) : (arr_7 [9])));
                    arr_10 [i_0] [i_2] [i_0] [i_2] = -1527409574724894761;
                    var_17 &= (arr_6 [i_0] [i_0] [i_0]);
                }
            }
        }
    }
    var_18 *= ((var_9 ? var_3 : (var_5 - 404022257)));
    #pragma endscop
}
