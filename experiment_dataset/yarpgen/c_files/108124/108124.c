/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 108124
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=108124 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/108124
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((var_1 ? ((max(((1 ? 398933838 : 3896033457)), var_14))) : (((!var_7) ? (var_4 - 398933847) : ((max(var_10, 1238388283)))))));

    /* vectorizable */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        var_17 = (var_10 / (arr_1 [i_0 - 1]));
        var_18 ^= ((2356892300986155174 << (var_0 - 28)));
    }
    #pragma endscop
}
