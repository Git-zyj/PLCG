/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77120
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77120 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77120
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (~(((((54196 ? 11339 : 11344))) & 2528514963)));
    var_14 = ((-221882165 * (var_4 > var_4)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_15 = var_6;
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 15;i_4 += 1)
                        {
                            {
                                var_16 = ((var_11 << (((min(var_4, -var_11)) - 2357945719))));
                                var_17 = (((-34213 + 2147483647) >> (((((((arr_12 [i_1]) ? (arr_11 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0] [7]) : var_11)) << (var_6 != var_4))) - 390))));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
