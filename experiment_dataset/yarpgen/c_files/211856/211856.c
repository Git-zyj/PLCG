/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211856
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211856 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211856
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(3515810591, (((var_6 * var_4) / (((max(194, var_8))))))));
    var_15 += (max(var_6, var_9));
    var_16 = 15221;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_17 = (min(((175 && (((arr_4 [i_0] [i_0]) <= (arr_1 [i_1]))))), (((arr_3 [i_1] [i_1] [i_0]) || (min((arr_3 [1] [8] [i_1]), (arr_3 [i_0] [i_0] [10])))))));
                var_18 = (arr_4 [i_0] [i_1]);
                var_19 = ((((-5292776884740407029 % (((((arr_1 [i_0]) != -5292776884740407041)))))) + 5292776884740407026));
            }
        }
    }
    #pragma endscop
}
