/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152396
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152396 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152396
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_8;

    for (int i_0 = 1; i_0 < 6;i_0 += 1)
    {
        arr_2 [i_0] = ((((max((879854208 + var_8), -var_9))) && (((((((arr_0 [i_0] [i_0]) | (arr_0 [i_0] [i_0])))) ? (arr_1 [i_0]) : ((((arr_0 [i_0 + 2] [i_0]) > (arr_0 [0] [i_0])))))))));
        arr_3 [i_0 + 3] [i_0] = ((0 < (arr_0 [1] [i_0])));
    }
    /* LoopNest 3 */
    for (int i_1 = 0; i_1 < 18;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 18;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 18;i_3 += 1)
            {
                {
                    var_13 += (min(2, (((arr_7 [3] [i_2] [i_2]) + (arr_4 [11])))));
                    var_14 += (arr_9 [4] [i_2]);
                    var_15 = ((~((-(arr_4 [1])))));
                }
            }
        }
    }
    #pragma endscop
}
