/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 137306
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=137306 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/137306
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 -= (~var_9);
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    var_16 = (arr_4 [17] [i_1] [i_2] [1]);
                    arr_6 [i_0] [i_1] [i_1] [i_1] = ((-(((!(((~(arr_5 [i_0] [16] [i_1])))))))));
                    arr_7 [i_1] [i_1] = (arr_1 [i_0] [i_0]);
                }
            }
        }
    }
    var_17 ^= var_13;
    var_18 &= (((((var_1 ? var_1 : ((0 ? var_9 : 1211644671965158116))))) ? ((var_5 ? var_10 : ((var_4 ? var_8 : var_12)))) : var_11));
    var_19 = (~(((((var_1 ? var_11 : var_11))) ? var_3 : var_14)));
    #pragma endscop
}
