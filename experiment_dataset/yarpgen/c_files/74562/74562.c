/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74562
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74562 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74562
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_5 [i_0] = (arr_4 [1] [12]);

                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    var_17 = (~17);
                    var_18 = (((((var_13 ? (arr_2 [i_1] [i_2]) : (arr_0 [4] [4])))) ? (((!(arr_6 [i_0] [i_1] [i_2] [i_0])))) : 1));
                    arr_8 [i_0] [i_1] [i_2] = ((((((arr_7 [i_0]) & (arr_4 [i_1] [i_2])))) ? ((max((arr_3 [i_1] [i_2]), (arr_0 [i_0] [i_0])))) : (((((((arr_7 [i_0]) < var_8))) > -var_3)))));
                }
            }
        }
    }
    var_19 = (var_12 < var_3);
    var_20 -= var_7;
    #pragma endscop
}
