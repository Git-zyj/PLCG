/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 140409
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=140409 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/140409
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (max((((var_10 - 60) ? (((max(var_8, -86)))) : 2823670528)), (((max(var_9, var_0))))));

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_12 = (max((arr_1 [i_0] [i_0]), (max((max((arr_1 [i_0] [11]), (arr_1 [i_0] [i_0]))), (var_4 <= -22675)))));
        var_13 = var_0;
    }
    for (int i_1 = 1; i_1 < 23;i_1 += 1)
    {
        var_14 = (max(var_9, ((+((((arr_3 [i_1] [i_1]) == var_3)))))));
        var_15 = (~var_4);
        var_16 = (arr_2 [i_1]);
        var_17 = (min(var_17, (!var_1)));
        var_18 = var_6;
    }
    #pragma endscop
}
