/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138109
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138109 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138109
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_11 = (max(var_11, (((((!((((arr_8 [i_0]) ? var_2 : (arr_1 [i_4])))))) ? (arr_5 [i_4] [i_2] [i_2]) : (((((max(var_4, 28))) ? ((var_7 ? var_10 : var_8)) : (arr_11 [i_2])))))))));
                                arr_12 [i_0] [i_0] [i_0] [i_0] [8] [i_0] [i_0] = (((arr_1 [i_0]) <= (arr_1 [i_0])));
                            }
                        }
                    }
                    var_12 = (arr_4 [i_0] [i_0]);
                    var_13 = ((!(arr_1 [i_0])));
                    var_14 = (max(var_14, ((((((-(arr_0 [i_1] [i_1])))) ? (((((-(arr_1 [i_2])))) ? (arr_4 [i_0] [i_1]) : var_3)) : (arr_6 [i_2]))))));
                }
            }
        }
    }
    var_15 ^= ((0 / (var_1 ^ 1)));
    var_16 -= (var_2 ^ var_4);
    #pragma endscop
}
