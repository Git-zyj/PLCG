/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203549
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203549 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203549
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 ^= var_14;

    /* vectorizable */
    for (int i_0 = 2; i_0 < 17;i_0 += 1)
    {
        arr_4 [i_0] = arr_0 [i_0] [i_0];
        var_17 = 3708919151;
    }
    for (int i_1 = 2; i_1 < 7;i_1 += 1)
    {
        arr_8 [i_1] = (arr_1 [i_1 + 2]);
        var_18 = (max(var_18, (((586048143 << (((arr_1 [1]) - 1811873847)))))));
        arr_9 [4] |= (arr_1 [11]);
        arr_10 [i_1] [i_1] = (arr_2 [i_1] [i_1 + 2]);
    }
    var_19 ^= var_13;
    var_20 = (((~var_2) != (max(((-1091157402 ? var_8 : 0)), var_1))));
    var_21 = (min(var_21, ((((!(((1 >> (7133629590915318799 - 7133629590915318777))))))))));
    #pragma endscop
}
