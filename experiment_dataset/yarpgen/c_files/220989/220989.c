/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220989
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220989 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220989
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (!-var_16);
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_19 = (~-255);
                var_20 |= max(((1213282856 ? ((var_8 ? (arr_2 [8] [i_1]) : 9765268543520895944)) : -19)), (arr_3 [i_1]));
                var_21 = (((min((arr_3 [i_0 + 1]), var_13))) ? (((~79108678) ? (arr_5 [i_0] [i_0] [i_0]) : ((8104 ? 18446744073709551615 : 79108669)))) : ((((!(arr_4 [i_0 + 1] [i_0 + 1] [i_1]))))));
            }
        }
    }
    #pragma endscop
}
