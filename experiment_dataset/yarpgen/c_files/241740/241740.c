/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241740
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241740 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241740
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((!((var_8 && (17399 < var_4)))));

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (min((arr_1 [i_0]), (30099 > 0)));
        arr_3 [i_0] = (max(1, (((max((arr_1 [i_0]), (arr_0 [8]))) >= (((((arr_1 [i_0]) || (arr_1 [i_0])))))))));
        arr_4 [i_0] = (arr_1 [i_0]);
    }
    for (int i_1 = 0; i_1 < 21;i_1 += 1)
    {
        arr_9 [i_1] = 3;
        arr_10 [i_1] = 0;
        var_12 = var_2;
    }
    for (int i_2 = 0; i_2 < 23;i_2 += 1)
    {
        arr_15 [i_2] [i_2] = (arr_13 [i_2]);
        var_13 ^= ((((-1026162458960542173 / (-2147483647 - 1))) << (((arr_11 [i_2]) - 15))));
        var_14 -= ((~(4208 || -4208)));
    }
    #pragma endscop
}
