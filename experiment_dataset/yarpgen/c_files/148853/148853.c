/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148853
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148853 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148853
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_11;
    var_15 = ((!((!(var_5 <= var_7)))));

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_16 += ((5148027242655300968 ? ((min((arr_1 [i_0]), ((18209058041124440779 ? (arr_0 [i_0]) : (arr_1 [i_0])))))) : ((~(((arr_1 [i_0]) ? 237686032585110837 : (arr_1 [i_0])))))));

        /* vectorizable */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            var_17 = var_5;
            var_18 ^= ((var_9 ? (arr_2 [i_0]) : (arr_0 [i_0])));
        }
        arr_4 [9] [i_0] = (arr_3 [i_0] [i_0] [3]);
    }
    #pragma endscop
}
