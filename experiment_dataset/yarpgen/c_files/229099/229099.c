/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229099
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229099 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229099
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_17 = (max(var_17, var_15));
                var_18 = (((arr_3 [i_0 + 2] [i_1]) ? ((((arr_4 [i_0] [i_0]) / var_16))) : ((((max(2370813610, (arr_4 [i_0] [i_1])))) ? (arr_4 [i_0 + 2] [i_0 + 2]) : 2587178282))));
                arr_6 [15] [15] = (2370813623 * 186);
                arr_7 [i_0] [i_0] [i_1] = (+-1);
                arr_8 [i_1] [i_1] [i_1] = (max((arr_3 [i_0] [18]), (arr_2 [i_0 + 1] [i_0 + 2])));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 25;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 1;i_3 += 1)
        {
            {
                arr_13 [i_2] [i_3] [i_3] = var_5;
                arr_14 [i_3] = (arr_12 [i_2] [i_3 - 1]);
            }
        }
    }
    var_19 = 69;
    #pragma endscop
}
