/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54426
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54426 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54426
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            arr_6 [0] = min(((-(arr_4 [i_1] [i_0] [i_0]))), (((arr_0 [1]) % (arr_2 [i_0]))));
            var_11 = ((-((-(arr_2 [i_0])))));
        }
        var_12 = var_1;
        arr_7 [1] [i_0] = (((arr_2 [i_0]) <= (max((arr_5 [i_0]), (((arr_3 [i_0]) ? (arr_2 [i_0]) : -1454639225))))));
        arr_8 [8] [1] = ((~((-1453117192 ? 251 : (arr_5 [i_0])))));
        arr_9 [11] = (((~var_5) < ((min((min(var_5, var_4)), (1453117191 <= 1453117186))))));
    }
    var_13 = (max(var_13, var_5));
    var_14 = var_8;
    #pragma endscop
}
