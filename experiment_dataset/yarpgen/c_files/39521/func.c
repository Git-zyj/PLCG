/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 39521
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=39521 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/39521
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 13; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 14; i_1 += 3) 
        {
            for (long long int i_2 = 2; i_2 < 12; i_2 += 3) 
            {
                {
                    arr_9 [i_0] [i_1] [i_2] = ((/* implicit */unsigned short) (-9223372036854775807LL - 1LL));
                    arr_10 [i_0] = ((/* implicit */long long int) max((((/* implicit */unsigned int) (!(((/* implicit */_Bool) arr_2 [i_2 - 2] [i_2 - 2]))))), (((((/* implicit */_Bool) 3497168936U)) ? (((/* implicit */unsigned int) ((/* implicit */int) arr_7 [i_2] [i_2]))) : (max((arr_3 [i_2] [i_0]), (arr_6 [i_0] [i_0] [i_0])))))));
                    arr_11 [i_0] [i_0] [i_0] [i_0] = ((/* implicit */int) var_8);
                }
            } 
        } 
    } 
    var_16 = ((/* implicit */unsigned char) var_10);
}
