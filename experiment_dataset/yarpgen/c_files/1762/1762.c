/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 1762
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=1762 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/1762
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, (((var_6 ? ((((var_3 >= (max(var_8, 1175610806962700203)))))) : (max((max(-1668318607507975989, var_18)), var_6)))))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_20 = (((arr_1 [i_0] [i_1]) ? ((var_15 ? (arr_0 [i_1]) : var_17)) : var_16));
                var_21 = var_7;
            }
        }
    }
    var_22 = var_18;
    #pragma endscop
}
