/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221638
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221638 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221638
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (var_1 + var_0);
    var_11 = (max(var_11, -var_0));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 23;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_0] [i_1] = 3055201333806020290;
                    var_12 = -1;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 0;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 24;i_4 += 1)
                        {
                            {
                                var_13 = (!1);
                                var_14 ^= ((((min((9223372036854775805 * 0), -var_7))) ? (max((!-964860163), (min((arr_12 [i_0] [i_1] [i_2] [10] [i_4] [i_4] [i_2 - 1]), -2006343472)))) : ((~(arr_11 [20] [20] [i_3] [20] [20])))));
                                var_15 = (~-1);
                                var_16 = (arr_13 [i_0] [i_1] [i_2] [i_3]);
                            }
                        }
                    }
                }
            }
        }
    }
    var_17 &= var_7;
    #pragma endscop
}
