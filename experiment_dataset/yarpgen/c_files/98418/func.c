/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98418
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98418 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98418
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */unsigned int) var_4);
    var_11 = ((/* implicit */unsigned char) min((((/* implicit */unsigned long long int) var_4)), ((~(12740006450515846679ULL)))));
    /* LoopSeq 1 */
    for (unsigned char i_0 = 0; i_0 < 16; i_0 += 4) 
    {
        var_12 = ((/* implicit */short) ((((/* implicit */_Bool) min((((/* implicit */unsigned long long int) (~(-1467307389)))), (min((((/* implicit */unsigned long long int) arr_1 [i_0] [i_0])), (var_7)))))) ? (((/* implicit */unsigned int) (~((-(736727399)))))) : ((~((+(arr_1 [i_0] [i_0])))))));
        var_13 *= ((/* implicit */unsigned char) arr_0 [i_0]);
    }
    var_14 = var_3;
}
