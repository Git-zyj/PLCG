/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103571
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103571 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103571
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 20;i_2 += 1)
            {
                for (int i_3 = 4; i_3 < 17;i_3 += 1)
                {
                    {
                        arr_10 [i_1] [i_1] = ((4 ? 10 : -76));
                        arr_11 [3] [3] [i_1] [i_3 + 4] = -10;
                    }
                }
            }
        }

        for (int i_4 = 2; i_4 < 18;i_4 += 1)
        {
            /* LoopNest 3 */
            for (int i_5 = 3; i_5 < 20;i_5 += 1)
            {
                for (int i_6 = 0; i_6 < 21;i_6 += 1)
                {
                    for (int i_7 = 0; i_7 < 21;i_7 += 1)
                    {
                        {
                            arr_26 [i_0] [i_0] [i_4 - 2] [i_5] [i_6] [i_0] = 1725928162355476645;
                            arr_27 [i_7] [i_7] [i_5] [i_0] [i_5] [i_0] [i_0] = ((-(arr_23 [i_7] [i_6] [i_4 - 1] [i_4] [i_4])));
                        }
                    }
                }
            }
            var_12 = 59;
            var_13 = (min(var_13, (!-76)));
        }
    }
    var_14 = (~(var_5 % 215183142));
    var_15 = (var_0 > var_0);
    #pragma endscop
}
