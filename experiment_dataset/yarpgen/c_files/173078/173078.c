/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 173078
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=173078 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/173078
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max(var_20, (((((var_2 / (min(var_0, var_5)))) == -var_1)))));

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_21 = ((((!(arr_0 [i_0]))) ? (arr_1 [i_0] [i_0]) : (arr_0 [i_0])));
        var_22 &= arr_1 [i_0] [i_0];
    }
    for (int i_1 = 0; i_1 < 11;i_1 += 1)
    {
        arr_4 [i_1] [i_1] = ((-(((((0 ? 12081467722971873969 : -4928074285378474524))) ? (arr_1 [i_1] [i_1]) : (((arr_2 [i_1]) ? 12 : (arr_1 [i_1] [i_1])))))));
        var_23 = (max(var_23, ((max(6365276350737677647, 1)))));
    }
    var_24 = 24;
    #pragma endscop
}
