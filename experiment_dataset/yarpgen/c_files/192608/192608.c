/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 192608
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=192608 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/192608
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((var_7 ? (min(var_11, var_2)) : var_8));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_13 = (min(var_13, ((((((arr_4 [i_0] [i_1]) ? (arr_0 [i_0]) : (min((arr_1 [2] [2]), (arr_0 [15]))))) + ((min((arr_1 [18] [18]), (arr_4 [i_0] [i_1])))))))));
                var_14 = (max(var_14, ((~(min((arr_0 [i_0]), (arr_0 [i_1])))))));
            }
        }
    }
    #pragma endscop
}
