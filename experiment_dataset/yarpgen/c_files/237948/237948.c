/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237948
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237948 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237948
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        arr_3 [i_0] [0] = ((!(((18223674668017074058 ? 4294967282 : 32746)))));
        arr_4 [i_0] = ((4294967295 * (min(var_0, (var_3 == var_5)))));
        arr_5 [i_0] = (!3);
    }
    var_14 = (min(var_14, var_3));
    var_15 = (var_6 - (((~0) & 2705480915)));
    var_16 = max(((-8530 ? (max(var_4, var_7)) : (((5 ? var_11 : 2147483647))))), var_3);
    #pragma endscop
}
