/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24555
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24555 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24555
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 &= var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_12 &= 0;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        {
                            arr_11 [i_3] [i_0] [i_2] [i_1] [i_0] [i_0] = 130;

                            for (int i_4 = 0; i_4 < 23;i_4 += 1)
                            {
                                var_13 = var_10;
                                arr_16 [i_4] [i_1] [i_2] [i_4] &= (min((((arr_0 [i_2]) ? 7068527336985023398 : (arr_0 [i_3]))), (!204)));
                                var_14 *= (max((~-7221687732455974742), ((1 & (arr_9 [i_0] [i_0] [i_0] [i_0])))));
                            }
                            var_15 = ((2284 ? 0 : -90));
                        }
                    }
                }
                var_16 = ((-7068527336985023398 ? -7068527336985023398 : 1));
            }
        }
    }
    #pragma endscop
}
