/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152612
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152612 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152612
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */long long int) var_6);
    /* LoopNest 2 */
    for (unsigned char i_0 = 1; i_0 < 15; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 13; i_1 += 3) 
        {
            {
                arr_6 [i_1] [(unsigned short)4] [i_0] = ((long long int) max((((/* implicit */unsigned int) ((int) arr_4 [i_1]))), (max((3895155060U), (((/* implicit */unsigned int) arr_1 [i_0]))))));
                var_17 = ((/* implicit */long long int) 3822879368U);
            }
        } 
    } 
}
