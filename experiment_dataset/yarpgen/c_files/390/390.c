/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 390
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=390 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/390
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((((min(var_1, 4294967295)) * (var_0 == var_1)))) ? var_8 : (((var_4 ? var_3 : (var_4 - -10))))));
    var_13 = (((max(-var_9, ((var_10 ? var_8 : var_6)))) / var_0));

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        arr_3 [i_0] = var_8;
        var_14 = (max(var_14, ((min((arr_1 [i_0]), (((arr_1 [i_0]) ? (arr_1 [i_0]) : (arr_1 [i_0]))))))));
    }
    var_15 = var_11;
    #pragma endscop
}
