/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97096
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97096 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97096
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        arr_2 [i_0] = (!((((arr_1 [i_0] [i_0]) ? var_2 : (arr_0 [5] [5])))));
        arr_3 [4] [i_0] = -var_7;
        var_17 ^= var_10;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_18 = var_5;
        var_19 = var_13;
    }

    /* vectorizable */
    for (int i_2 = 0; i_2 < 14;i_2 += 1)
    {
        arr_9 [i_2] = (((var_11 ? var_10 : var_14)) * var_5);
        arr_10 [i_2] = (~(var_15 + var_8));
        var_20 = (arr_4 [i_2]);
    }
    for (int i_3 = 2; i_3 < 13;i_3 += 1)
    {
        var_21 = (min(var_21, ((((arr_0 [i_3] [i_3 - 1]) / ((-(var_4 / var_9))))))));

        /* vectorizable */
        for (int i_4 = 0; i_4 < 15;i_4 += 1)
        {
            var_22 = (-var_9 / -var_12);
            var_23 = (arr_12 [i_3] [7]);
        }
        /* vectorizable */
        for (int i_5 = 0; i_5 < 15;i_5 += 1)
        {
            var_24 ^= ((((0 ? 0 : 8825451780569222487)) % var_3));
            var_25 ^= ((~(arr_14 [i_5] [i_3 - 2])));
        }
        arr_17 [i_3 + 1] = (!var_7);
    }
    #pragma endscop
}
