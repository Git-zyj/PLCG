/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68171
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68171 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68171
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_10, 0));

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = var_9;
        var_13 = max(var_1, (arr_0 [13]));
        arr_3 [i_0] [i_0] = ((!(arr_0 [i_0 - 1])));
        arr_4 [i_0] = (min(-13, 2069385021));
        /* LoopNest 2 */
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 18;i_2 += 1)
            {
                {
                    arr_11 [i_0] [i_0] [i_0] = ((~(min((~-1), (arr_6 [i_0 - 1] [i_1 - 2])))));
                    arr_12 [i_0] [i_1] [i_1] [i_2] = (min((((arr_8 [i_2] [i_2 - 2] [i_1 - 2] [i_0 - 1]) ? var_6 : (arr_10 [i_0 - 1]))), ((19960 ? var_6 : ((((-19961 + 2147483647) >> (36720 - 36699))))))));
                }
            }
        }
    }
    #pragma endscop
}
