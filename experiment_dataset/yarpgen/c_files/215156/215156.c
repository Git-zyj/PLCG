/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215156
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215156 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215156
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        var_10 ^= var_5;
        var_11 = ((var_6 ? (arr_0 [i_0]) : (arr_0 [i_0 + 2])));

        /* vectorizable */
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            var_12 = (~243);

            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                /* LoopNest 2 */
                for (int i_3 = 0; i_3 < 20;i_3 += 1)
                {
                    for (int i_4 = 0; i_4 < 20;i_4 += 1)
                    {
                        {
                            var_13 += ((var_6 ? (arr_10 [i_2 - 1] [i_1 - 1] [i_1 - 1] [i_0 - 1]) : (arr_10 [i_2 + 2] [i_1 - 1] [i_1 - 1] [i_0 + 3])));
                            arr_13 [i_0] [i_0] = (((190 * 232) ? -130 : ((var_3 ? var_9 : (arr_12 [i_0])))));
                        }
                    }
                }
                arr_14 [2] [2] |= (~141);
            }
        }
    }
    var_14 = ((((var_5 == 23) ? var_4 : ((max(-32754, 47))))));
    #pragma endscop
}
