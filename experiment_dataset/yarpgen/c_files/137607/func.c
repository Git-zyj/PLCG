/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 137607
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=137607 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/137607
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */signed char) -3546629208322589789LL);
    var_16 &= ((/* implicit */short) (!(((/* implicit */_Bool) ((((/* implicit */_Bool) (short)-1)) ? (var_3) : (((/* implicit */unsigned int) ((/* implicit */int) var_1))))))));
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 12; i_0 += 4) 
    {
        for (signed char i_1 = 0; i_1 < 12; i_1 += 2) 
        {
            {
                var_17 = ((/* implicit */short) arr_2 [i_0]);
                var_18 += ((/* implicit */short) (~((-((-(((/* implicit */int) (unsigned short)65535))))))));
            }
        } 
    } 
}
