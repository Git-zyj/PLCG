/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236189
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236189 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236189
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(var_14, var_5));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 14;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 14;i_4 += 1)
                        {
                            {
                                arr_15 [i_2] [i_3] = (arr_1 [i_0] [i_0]);
                                var_15 &= ((min(1, (arr_7 [i_4 - 1] [i_4 - 1] [i_1 - 1] [i_3]))));
                                var_16 = (max((~1865339726), (((arr_0 [i_1 - 3]) / (arr_2 [i_0 + 1] [i_1 + 1])))));
                            }
                        }
                    }
                }
                arr_16 [i_0] = (arr_3 [i_0 - 1]);
            }
        }
    }
    var_17 = (max(var_17, ((max((min(((var_8 ? var_12 : var_7)), (var_2 - -153417680))), var_4)))));
    #pragma endscop
}
