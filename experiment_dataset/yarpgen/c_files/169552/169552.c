/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169552
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169552 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169552
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_0] = (((arr_5 [i_1]) ? ((((!16430) ? 32752 : ((-(arr_1 [i_0] [i_0])))))) : (((((var_5 ? (arr_3 [i_0]) : (arr_1 [i_0] [i_1])))) ? var_0 : (arr_2 [i_0] [i_0])))));
                arr_7 [i_0] [i_1] = ((-((-(arr_1 [i_0] [i_1])))));
                var_10 *= (~var_5);
                var_11 = (((arr_0 [i_0]) ? (939524096 - var_7) : (((arr_1 [i_0] [i_1]) ? (arr_1 [i_0] [i_1]) : (arr_1 [i_0] [i_1])))));
            }
        }
    }
    var_12 = ((var_9 ? ((-16384 ? (42140 & 42215) : var_9)) : ((var_3 ? (-32767 && 131071) : (var_1 == var_3)))));
    #pragma endscop
}
