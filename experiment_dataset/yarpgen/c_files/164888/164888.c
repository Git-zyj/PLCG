/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164888
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164888 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164888
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_18 = (arr_0 [i_2]);
                                var_19 *= (((arr_0 [i_2]) ? ((((var_16 ? (arr_5 [i_2] [i_3] [i_4]) : 0)) ^ ((max(0, -3))))) : (((((arr_11 [i_0] [i_1] [i_0] [i_2] [i_3] [i_1] [i_0]) * (arr_11 [i_0] [i_0] [i_0] [i_1] [i_2] [i_2] [i_0])))))));
                                var_20 ^= var_1;
                            }
                        }
                    }
                }
                var_21 *= (arr_1 [i_0]);
            }
        }
    }
    var_22 = ((-((var_2 - (max(0, 1533885204))))));
    #pragma endscop
}
