/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 96105
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=96105 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/96105
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = min(var_9, (min(((max(var_6, var_0))), ((var_5 ? 12196035093651302938 : var_6)))));
    var_11 = ((((max(var_3, -var_0))) ? var_5 : var_2));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        arr_2 [i_0] = ((arr_1 [i_0]) ? (arr_0 [i_0]) : 1757415944);
        var_12 = var_2;
        var_13 = (arr_0 [i_0]);
        var_14 = (arr_1 [i_0]);
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        arr_6 [i_1] = arr_5 [i_1] [i_1];
        var_15 += (-5661346059299173420 <= 7360291364426943008);
    }
    #pragma endscop
}
