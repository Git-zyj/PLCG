/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198832
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198832 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198832
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_7;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [4] = (((~((-(arr_5 [i_0] [i_1] [i_0]))))));

                /* vectorizable */
                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    var_11 = ((~(arr_2 [i_0])));
                    var_12 = (max(var_12, (arr_5 [i_2] [i_2] [i_0])));
                    var_13 = (min(var_13, var_3));
                    arr_9 [i_0] [i_1] = ((!(arr_5 [i_0] [i_2] [13])));
                }
                for (int i_3 = 0; i_3 < 17;i_3 += 1)
                {
                    arr_12 [i_0] [i_0] [i_3] [i_3] = ((~(arr_10 [i_0])));
                    var_14 = (min(var_14, var_4));
                }
                for (int i_4 = 0; i_4 < 17;i_4 += 1)
                {
                    arr_15 [i_0] [i_1] [i_0] [i_4] = ((-(((!(arr_4 [i_0]))))));
                    arr_16 [i_0] = var_4;
                }
                var_15 = (-((~(arr_5 [1] [i_1] [i_1]))));
            }
        }
    }
    #pragma endscop
}
