/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32862
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32862 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32862
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(var_18, var_11));
    var_19 = var_13;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_20 = (((((min(9223372036854775807, 0)))) > (-227574158 == var_6)));
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 22;i_3 += 1)
                    {
                        {
                            arr_11 [i_3] [i_3] [i_0] [i_3] [i_3] = max((arr_7 [i_0] [i_1]), (((((min(var_13, var_14))) || 10269))));
                            var_21 = (max(var_21, (arr_3 [i_0] [i_1] [i_3])));
                        }
                    }
                }
                var_22 = (min(var_22, (((((((arr_1 [i_0] [i_1]) / (arr_1 [i_0] [i_1])))) / (((arr_6 [i_0] [i_0] [i_0] [i_0]) ? (arr_7 [2] [i_1]) : (arr_7 [0] [i_1]))))))));
                var_23 *= -227574158;
            }
        }
    }
    #pragma endscop
}
