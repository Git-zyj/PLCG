/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21819
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21819 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21819
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 |= (max(var_15, (((!(var_4 >= var_11))))));
    var_19 = var_7;

    for (int i_0 = 0; i_0 < 10;i_0 += 1) /* same iter space */
    {
        var_20 = (min(var_20, 2072924747533272360));
        var_21 = (((255 ? 1 : (arr_0 [i_0]))));
        arr_2 [i_0] = ((!((max((arr_0 [i_0]), 211896400)))));
    }
    for (int i_1 = 0; i_1 < 10;i_1 += 1) /* same iter space */
    {
        arr_6 [i_1] = var_11;
        var_22 = (-(((arr_1 [4]) ? var_1 : (arr_5 [i_1] [i_1]))));
        var_23 = arr_4 [i_1];
        var_24 = (arr_0 [i_1]);
    }
    #pragma endscop
}
