/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34839
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34839 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34839
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((!((((~var_11) ? ((max(var_13, 60300))) : var_13)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            {
                var_19 = ((-3871 ? ((var_13 ? (((var_7 ? (arr_0 [i_0]) : var_14))) : (arr_2 [i_1 + 3] [i_1 - 1]))) : (max(((min(2542663692, 11146))), (var_2 / 32751)))));
                arr_5 [i_0] [i_0] [i_0] = ((var_5 >> (var_1 + 4467370256622794338)));
            }
        }
    }
    #pragma endscop
}
