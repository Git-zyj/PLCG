/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 8955
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=8955 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/8955
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned long long int) max((var_3), (((/* implicit */long long int) var_10))));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 4; i_0 < 19; i_0 += 2) 
    {
        arr_3 [i_0] = ((/* implicit */long long int) (unsigned char)24);
        var_13 ^= ((/* implicit */long long int) var_9);
        var_14 *= ((/* implicit */unsigned int) var_0);
        arr_4 [i_0] [(short)20] = ((var_7) ? (((/* implicit */unsigned int) (~(((/* implicit */int) var_6))))) : ((~(max((var_8), (1676915413U))))));
    }
    var_15 &= ((/* implicit */short) (~(((/* implicit */int) (signed char)-124))));
    var_16 ^= ((/* implicit */unsigned char) (-(2794610220404423389LL)));
}
