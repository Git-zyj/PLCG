/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 161191
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=161191 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/161191
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = -var_14;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_20 = (max(var_20, ((min(var_11, (((~(arr_4 [i_2])))))))));
                    var_21 &= 223;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 15;i_4 += 1)
                        {
                            {
                                var_22 = 40;
                                arr_13 [i_4] [i_3] [i_0] [i_0] [i_0] [i_0] = -41;
                                arr_14 [i_0] [i_0] [i_0] [i_2] [i_2] [i_3] [i_4] = (((!1) >> (var_11 - 3247782266)));
                            }
                        }
                    }
                    arr_15 [i_0] [i_1] [i_2] [i_2] = ((-(arr_4 [i_0])));
                }
            }
        }
    }
    var_23 -= var_10;
    /* LoopNest 2 */
    for (int i_5 = 0; i_5 < 20;i_5 += 1)
    {
        for (int i_6 = 0; i_6 < 20;i_6 += 1)
        {
            {
                arr_21 [i_5] [i_6] [i_6] = var_5;
                arr_22 [i_6] = var_16;
            }
        }
    }
    #pragma endscop
}
