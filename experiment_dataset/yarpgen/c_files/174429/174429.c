/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174429
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174429 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174429
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_0] = ((~((~((var_6 >> (arr_3 [i_0])))))));
                var_11 += (max(((((min(9223372036854775807, 0)) == 7755876653917337926))), ((var_3 << (((arr_0 [i_0 + 1]) - 4992405702441605131))))));
            }
        }
    }
    var_12 ^= var_5;
    var_13 = var_8;
    var_14 = (((min(((var_6 ? var_1 : var_6)), (var_4 | 247))) != (var_10 + var_0)));
    #pragma endscop
}
