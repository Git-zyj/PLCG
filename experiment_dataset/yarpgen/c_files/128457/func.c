/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 128457
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=128457 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/128457
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 &= ((/* implicit */int) 3053561435U);
    /* LoopNest 2 */
    for (unsigned char i_0 = 3; i_0 < 11; i_0 += 1) 
    {
        for (unsigned short i_1 = 1; i_1 < 11; i_1 += 1) 
        {
            {
                var_15 = ((/* implicit */short) max((((var_4) / (((/* implicit */int) var_8)))), (((/* implicit */int) arr_5 [i_0] [i_1]))));
                arr_6 [i_0] [i_0 - 2] [i_1] = ((/* implicit */short) (((-(((/* implicit */int) (unsigned short)65535)))) <= (((/* implicit */int) (!(((/* implicit */_Bool) arr_4 [i_0 - 1] [i_0 + 1] [i_1 + 2])))))));
            }
        } 
    } 
}
