/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237758
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237758 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237758
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] = (min((((arr_0 [i_0]) ? (min((arr_1 [i_0]), var_3)) : var_5)), (arr_2 [i_0])));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        {
                            var_10 = ((!(arr_10 [i_0] [10] [i_2] [i_0] [i_0] [i_1])));
                            arr_12 [i_0] [i_0] [i_0] [i_3] = ((((arr_0 [i_0]) || (arr_4 [i_0] [i_0]))));
                            arr_13 [i_0] [i_0] [i_0] = (arr_6 [i_0] [i_0] [i_2] [i_3]);
                            arr_14 [i_0] [4] = var_8;
                        }
                    }
                }
            }
        }
    }
    var_11 = (min(var_11, var_8));
    #pragma endscop
}
