/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 53619
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=53619 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/53619
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((((((5534835096586156442 ? 126 : 5534835096586156442))) ? 32751 : -32752)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    var_17 ^= -8239;
                    arr_9 [i_2] [i_2 - 1] [i_1] [i_0] = 32766;
                    var_18 = -1242;
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 7;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 7;i_4 += 1)
                        {
                            {
                                var_19 = (max(var_19, 6));
                                var_20 = ((32746 ? ((((((0 ? 9265776987260698367 : 26))) ? -32741 : ((4382567590863076534 ? -8304811968978338981 : 4839))))) : -32741));
                            }
                        }
                    }
                }
            }
        }
    }
    var_21 = 32743;
    #pragma endscop
}
