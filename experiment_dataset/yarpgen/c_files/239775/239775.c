/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239775
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239775 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239775
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((((min((((0 ? var_2 : var_10))), ((var_10 ? var_5 : -651720342))))) ? var_12 : var_8));
    var_15 = var_10;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0 - 2] = ((arr_4 [16] [i_1]) ? -67108863 : (arr_4 [i_0] [i_1]));
                arr_6 [i_0] [i_0] [13] = var_13;
                var_16 &= ((~(((arr_0 [i_0]) ? (min(224, (arr_1 [i_0]))) : 3462259872545077480))));
            }
        }
    }
    #pragma endscop
}
