/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80490
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80490 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80490
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_1] [i_2] = ((6063625020244315508 >= (((((arr_5 [i_0] [i_0]) >= var_4)) ? var_9 : (1 / 18446744073709551615)))));
                    arr_9 [i_0] [i_0] [16] [i_0] = 15;
                }
            }
        }
    }

    /* vectorizable */
    for (int i_3 = 0; i_3 < 10;i_3 += 1)
    {
        arr_14 [i_3] [i_3] = (((((91 ? 1 : -7447865303546624807))) ? (arr_6 [i_3] [i_3] [i_3] [i_3]) : (((arr_0 [i_3]) ? -32763 : var_10))));
        arr_15 [i_3] = (arr_0 [i_3]);
    }
    #pragma endscop
}
