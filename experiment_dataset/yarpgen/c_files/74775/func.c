/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74775
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74775 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74775
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = (+(((/* implicit */int) var_12)));
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_20 *= ((/* implicit */signed char) (!(((/* implicit */_Bool) max((((/* implicit */int) var_2)), (((arr_2 [(_Bool)1]) ? (((/* implicit */int) var_7)) : (((/* implicit */int) var_7)))))))));
        arr_3 [i_0] |= ((/* implicit */long long int) 535115196);
        var_21 += ((/* implicit */long long int) arr_2 [i_0]);
        var_22 = ((/* implicit */unsigned char) max((var_22), (((/* implicit */unsigned char) (-(((/* implicit */int) max((var_8), ((short)-32763)))))))));
    }
}
