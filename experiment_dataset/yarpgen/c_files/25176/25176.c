/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 25176
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=25176 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/25176
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] |= var_10;
                arr_6 [i_0] [i_0] = ((var_3 ? var_9 : (max((((-(arr_4 [i_0])))), (max(var_9, var_5))))));
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 2; i_2 < 11;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 10;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 12;i_4 += 1)
            {
                {
                    var_11 = (arr_9 [i_2] [i_3] [i_2]);
                    var_12 = (arr_9 [i_2 - 2] [i_3] [i_4]);
                    var_13 = ((((min(845248249, 2298065882))) ? -var_3 : ((((arr_10 [i_3 + 2]) <= (arr_10 [i_3 - 1]))))));
                }
            }
        }
    }
    #pragma endscop
}
