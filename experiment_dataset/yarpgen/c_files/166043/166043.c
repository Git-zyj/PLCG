/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166043
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166043 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166043
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_6;
    var_15 = ((((max(43, 4294967289))) ? (!239460384) : ((-(min(var_5, 1624656904))))));

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        var_16 = (min(var_16, (((((max(693334384, (!1624656912)))) ? 226 : ((((arr_0 [i_0] [i_0]) <= 221))))))));
        var_17 = 1624656912;
        var_18 = var_5;
        arr_3 [i_0] = 3468550850;
    }
    /* vectorizable */
    for (int i_1 = 4; i_1 < 12;i_1 += 1)
    {
        arr_7 [i_1] = ((+(((arr_2 [i_1]) / var_4))));
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 13;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 13;i_3 += 1)
            {
                {
                    var_19 -= ((var_13 == (-32767 - 1)));
                    var_20 += (~44);
                    var_21 = (((~var_10) ? -26 : (!-126)));
                    var_22 = ((var_5 <= (arr_10 [i_1] [i_1 - 3])));
                }
            }
        }
    }
    var_23 *= var_6;
    #pragma endscop
}
