/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 19473
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=19473 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/19473
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */short) var_3);
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 25; i_0 += 1) 
    {
        arr_4 [i_0] = ((/* implicit */signed char) (+(((/* implicit */int) (signed char)-106))));
        arr_5 [i_0] [i_0] = ((/* implicit */short) (+(((((/* implicit */unsigned long long int) 3541912850U)) - (((unsigned long long int) var_19))))));
    }
}
