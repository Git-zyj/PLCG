/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149855
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149855 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149855
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */unsigned int) var_2);
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 10; i_0 += 2) 
    {
        /* LoopNest 2 */
        for (long long int i_1 = 0; i_1 < 10; i_1 += 2) 
        {
            for (unsigned int i_2 = 0; i_2 < 10; i_2 += 2) 
            {
                {
                    /* LoopNest 2 */
                    for (unsigned int i_3 = 0; i_3 < 10; i_3 += 4) 
                    {
                        for (long long int i_4 = 0; i_4 < 10; i_4 += 2) 
                        {
                            {
                                var_14 = ((/* implicit */unsigned long long int) var_1);
                                var_15 = ((/* implicit */unsigned int) (+(((((/* implicit */long long int) ((/* implicit */int) var_7))) / (((long long int) var_2))))));
                                arr_14 [i_4] [(unsigned short)2] [i_1] [i_0] = var_5;
                                var_16 = ((/* implicit */_Bool) (+(((/* implicit */int) arr_9 [i_0] [i_0]))));
                            }
                        } 
                    } 
                    var_17 -= ((unsigned int) var_3);
                }
            } 
        } 
        /* LoopNest 3 */
        for (short i_5 = 1; i_5 < 8; i_5 += 2) 
        {
            for (unsigned int i_6 = 2; i_6 < 9; i_6 += 4) 
            {
                for (long long int i_7 = 0; i_7 < 10; i_7 += 4) 
                {
                    {
                        var_18 = ((/* implicit */unsigned long long int) (-((-(-5701921354756748865LL)))));
                        var_19 += ((/* implicit */short) var_1);
                    }
                } 
            } 
        } 
    }
    var_20 &= ((/* implicit */unsigned long long int) var_6);
}
