/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122708
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122708 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122708
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_10 |= ((((((arr_1 [i_0]) & (arr_2 [i_0])))) ? var_5 : 2035050377));
        var_11 |= (arr_0 [i_0]);
    }
    var_12 = (max(var_12, var_6));

    for (int i_1 = 0; i_1 < 11;i_1 += 1) /* same iter space */
    {
        arr_8 [6] = ((((arr_6 [i_1] [i_1]) << (2099809267 - 2099809251))));
        var_13 = (((arr_6 [6] [6]) ? (!0) : 2195158029));
        var_14 -= (arr_4 [i_1]);
        arr_9 [i_1] [i_1] = -190925072;
    }
    for (int i_2 = 0; i_2 < 11;i_2 += 1) /* same iter space */
    {
        var_15 = ((~(~var_0)));
        arr_14 [i_2] = ((((-(arr_7 [i_2] [i_2]))) | ((max((max((arr_5 [1]), 1)), (arr_1 [i_2]))))));
    }
    var_16 = (min(var_16, 2099809266));
    #pragma endscop
}
