/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199418
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199418 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199418
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 16;i_1 += 1)
        {
            {
                var_13 = (arr_1 [i_0]);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            var_14 = (arr_8 [i_0] [i_1 + 1] [i_2] [i_0]);
                            arr_9 [i_0] [i_0] [i_2] [8] = (arr_7 [0] [0] [7] [i_3] [0] [i_2]);
                            var_15 = (~1);
                            arr_10 [i_2] [i_2] [0] = (max(((-(arr_7 [i_2] [i_1 + 1] [i_1 + 1] [i_1] [i_1 - 3] [i_2]))), ((var_6 ? (arr_5 [i_0] [i_3]) : ((((arr_8 [i_0] [i_0] [i_2] [i_3]) ? 1 : 2048)))))));
                            var_16 = 1;
                        }
                    }
                }
            }
        }
    }
    var_17 = (min(var_17, ((min(var_6, (~var_2))))));
    #pragma endscop
}
