/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 205473
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=205473 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/205473
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min(((((max(var_4, 8750362453095960837))) ? (-24086 - var_7) : -41)), ((-(min(var_7, 1))))));
    var_13 = (((((var_10 ? -24062 : (max(1, 9168073141098621606))))) ? (~-127) : (((max(var_7, 42226))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_6 [i_0] [9] = (((42226 - (arr_2 [i_0] [i_2]))));
                    var_14 = -1;
                    var_15 = ((max(1, (max(var_5, 1)))) ^ ((min(((~(arr_3 [i_2] [i_0]))), (arr_3 [i_1] [11])))));
                }
            }
        }
    }
    #pragma endscop
}
