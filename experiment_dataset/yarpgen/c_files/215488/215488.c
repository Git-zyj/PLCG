/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215488
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215488 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215488
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = -26307;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 12;i_3 += 1)
                    {
                        {
                            arr_13 [i_3] [i_3] = var_7;
                            var_20 = (arr_1 [i_2] [i_2 - 1]);
                            arr_14 [i_0] [i_3] [i_2] [i_3 + 1] = (arr_0 [i_0]);
                            var_21 = (min(var_21, ((min((arr_7 [i_0] [i_1 + 1] [i_1 + 1] [i_3]), (arr_7 [i_0] [i_1] [i_1 + 1] [i_3]))))));
                        }
                    }
                }
                var_22 = (min(((max(var_9, 1))), (((arr_2 [i_1 + 1]) + (arr_2 [i_1 + 1])))));
                var_23 = ((+(((-32767 - 1) ? 114167752 : (arr_4 [i_0])))));
                arr_15 [9] = -111;
            }
        }
    }
    var_24 = var_3;
    var_25 = (max(var_25, var_8));
    #pragma endscop
}
