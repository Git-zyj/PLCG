/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 189508
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=189508 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/189508
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_20 = 1;
                arr_4 [i_0] [i_1] [i_0] = ((21874 ? (min((((arr_2 [i_1]) ? (arr_2 [i_1]) : var_15)), (arr_1 [i_1]))) : 43639));
                arr_5 [i_0] [i_0] [i_0] = ((((arr_3 [i_1 + 1]) ? (arr_3 [i_1 + 1]) : (arr_3 [i_1 + 1]))));
            }
        }
    }
    var_21 &= var_16;
    var_22 = (max((min(var_11, 12181)), var_0));
    var_23 -= ((var_9 ? var_1 : ((max(var_10, var_3)))));
    #pragma endscop
}
