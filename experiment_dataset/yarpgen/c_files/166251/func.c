/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166251
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166251 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166251
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned char) var_5);
    var_15 = ((/* implicit */unsigned char) (!(((/* implicit */_Bool) var_9))));
    /* LoopSeq 1 */
    for (signed char i_0 = 4; i_0 < 16; i_0 += 2) 
    {
        /* LoopSeq 1 */
        for (unsigned char i_1 = 1; i_1 < 15; i_1 += 2) 
        {
            var_16 = ((/* implicit */short) (~(((int) arr_4 [i_1 + 2] [i_0 + 1]))));
            var_17 = ((/* implicit */signed char) max((var_17), (((/* implicit */signed char) (~(((/* implicit */int) arr_2 [i_1 + 2] [i_0 - 3])))))));
        }
        var_18 = ((/* implicit */int) min((var_18), (((/* implicit */int) ((unsigned long long int) ((unsigned long long int) arr_1 [i_0 - 4]))))));
    }
}
