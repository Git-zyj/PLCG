/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93353
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93353 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93353
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        var_12 = (min(var_12, (((~(max(var_1, (arr_3 [i_0]))))))));

        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            var_13 = (arr_0 [i_1] [i_0]);

            for (int i_2 = 1; i_2 < 19;i_2 += 1)
            {
                var_14 = ((var_3 ? (arr_0 [0] [i_2]) : (arr_7 [i_0] [i_1] [i_0])));
                var_15 = (arr_1 [4]);
                arr_9 [i_0] [i_1] [i_2 - 1] = ((var_1 ? ((max((arr_7 [i_2 - 1] [i_2 - 1] [i_1]), var_2))) : var_0));
            }
        }
        for (int i_3 = 0; i_3 < 21;i_3 += 1)
        {

            for (int i_4 = 0; i_4 < 1;i_4 += 1)
            {
                var_16 = (min(var_16, var_10));
                var_17 = (arr_10 [i_0] [i_0]);
            }
            var_18 = ((var_2 + ((min((arr_13 [i_0] [i_3] [i_3]), (arr_13 [i_0] [i_3] [i_0]))))));
        }
        arr_14 [i_0] [i_0] = ((((min(((6 ? var_9 : var_5)), (arr_12 [i_0] [i_0])))) & (min((arr_3 [i_0]), (arr_10 [i_0] [i_0])))));
    }
    var_19 -= var_9;
    #pragma endscop
}
