/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 55153
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=55153 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/55153
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(((var_8 ? var_0 : var_0)), (26520 - 248)));
    var_12 = ((var_1 ? var_5 : (65523 * 248)));
    var_13 = ((((var_9 * (var_3 / var_10))) < var_7));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1] [i_1] [i_2] = (min((max((arr_3 [i_0]), (arr_0 [i_1 + 2]))), (var_4 & var_4)));
                    arr_10 [i_2] = var_7;
                }
            }
        }
    }
    var_14 = (((((883967930 + var_10) ^ ((var_9 ? var_6 : 99)))) & var_4));
    #pragma endscop
}
