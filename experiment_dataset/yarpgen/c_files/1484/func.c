/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 1484
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=1484 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/1484
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */short) ((unsigned long long int) var_4));
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 23; i_0 += 2) 
    {
        var_18 = ((/* implicit */unsigned short) ((((/* implicit */int) (unsigned short)35606)) <= (((/* implicit */int) arr_1 [i_0]))));
        arr_2 [i_0] = ((/* implicit */_Bool) (unsigned short)29930);
        var_19 = ((/* implicit */unsigned short) var_11);
    }
}
