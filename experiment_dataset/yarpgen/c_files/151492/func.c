/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151492
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151492 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151492
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 14; i_0 += 4) 
    {
        var_19 = ((/* implicit */signed char) arr_0 [i_0]);
        var_20 = ((/* implicit */unsigned int) min((var_20), (((/* implicit */unsigned int) 594442038))));
        arr_2 [i_0] = ((/* implicit */unsigned char) 26ULL);
    }
    var_21 -= ((/* implicit */unsigned int) (-(((/* implicit */int) var_8))));
    var_22 = ((/* implicit */short) (-(((((/* implicit */_Bool) max((var_7), (((/* implicit */unsigned char) var_13))))) ? (min((var_6), (((/* implicit */unsigned long long int) var_17)))) : (((/* implicit */unsigned long long int) ((/* implicit */int) ((signed char) var_13))))))));
}
