/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40152
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40152 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40152
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */_Bool) (~(var_3)));
    var_21 = ((/* implicit */int) max((var_21), ((~(((/* implicit */int) var_18))))));
    var_22 = ((/* implicit */unsigned long long int) min((var_22), (((/* implicit */unsigned long long int) (~((+(((/* implicit */int) var_14)))))))));
    var_23 = ((/* implicit */short) (+(var_17)));
    /* LoopNest 2 */
    for (signed char i_0 = 1; i_0 < 9; i_0 += 4) 
    {
        for (short i_1 = 0; i_1 < 11; i_1 += 1) 
        {
            {
                var_24 = ((/* implicit */unsigned int) (+((+(((/* implicit */int) var_15))))));
                /* LoopNest 2 */
                for (unsigned long long int i_2 = 0; i_2 < 11; i_2 += 2) 
                {
                    for (unsigned char i_3 = 3; i_3 < 10; i_3 += 3) 
                    {
                        {
                            var_25 = ((/* implicit */int) min((var_25), (((/* implicit */int) (!((!(((/* implicit */_Bool) (unsigned short)38388)))))))));
                            var_26 -= ((/* implicit */int) (!((!(((/* implicit */_Bool) var_11))))));
                            arr_10 [i_0 - 1] [i_0] [i_1] [(unsigned char)9] [i_2] [i_3 - 1] = ((/* implicit */short) (-(((/* implicit */int) (signed char)-59))));
                            arr_11 [i_0] [i_0] [2ULL] [4ULL] = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) (~(var_17))))));
                        }
                    } 
                } 
                var_27 = ((/* implicit */unsigned long long int) (!((!(((/* implicit */_Bool) var_8))))));
                arr_12 [i_0] = ((/* implicit */unsigned int) (-((-(((/* implicit */int) arr_1 [i_0 - 1] [i_1]))))));
            }
        } 
    } 
}
