/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70149
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70149 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70149
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_5 [i_1] = -1713761782074816717;
                arr_6 [i_0] = (((~var_14) >> (51 - 39)));
                arr_7 [i_0] [i_1] [i_1] |= (!-1713761782074816716);
            }
        }
    }
    var_16 = -30070;
    /* LoopNest 3 */
    for (int i_2 = 0; i_2 < 21;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 21;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 21;i_4 += 1)
            {
                {
                    var_17 = 109;
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 21;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 21;i_6 += 1)
                        {
                            {
                                var_18 = (max((arr_9 [0]), (min(-1713761782074816716, ((((arr_22 [i_2] [i_3] [i_4] [i_5] [i_2] [i_2]) + var_0)))))));
                                arr_23 [i_2] = ((-(max(8881, var_2))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_19 = var_0;
    #pragma endscop
}
