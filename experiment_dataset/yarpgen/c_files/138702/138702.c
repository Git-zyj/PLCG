/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138702
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138702 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138702
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 7;i_2 += 1)
            {
                {

                    for (int i_3 = 1; i_3 < 7;i_3 += 1)
                    {
                        arr_10 [i_0] [1] [i_2] [3] = ((213 ? -var_7 : ((-(arr_8 [i_3 + 3] [i_2 - 1] [i_2 - 2] [i_2] [i_3 + 2] [i_1])))));
                        arr_11 [i_2] = (!var_3);
                    }
                    /* vectorizable */
                    for (int i_4 = 0; i_4 < 1;i_4 += 1)
                    {
                        var_10 = ((511 ? (arr_4 [i_2 - 1] [7] [i_2 - 2]) : 1));
                        var_11 += ((((var_0 ? -1 : var_2)) * 8));
                    }
                    arr_14 [i_2] = (!var_2);
                }
            }
        }
    }
    var_12 = 1;
    var_13 = ((var_5 ? ((32748 ? 1 : (1 ^ 3))) : var_6));
    var_14 = var_7;
    #pragma endscop
}
