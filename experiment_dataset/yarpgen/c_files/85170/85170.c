/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85170
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85170 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85170
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (((((((min(12766, 195))) & (!32750)))) ? ((min(((var_0 ? var_1 : var_0)), var_6))) : (max(var_9, (~var_10)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 9;i_1 += 1)
        {
            {
                var_18 = (min(var_18, ((min(((min((arr_3 [i_1 - 1]), (arr_3 [i_0])))), ((-(((arr_2 [i_0] [i_1]) ? var_15 : var_6)))))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        {
                            var_19 = (max(var_19, 56));
                            var_20 = (arr_10 [i_1 - 2] [i_1]);
                            var_21 = (max(((max((arr_2 [i_1 - 3] [i_1 - 3]), (arr_2 [i_1 + 1] [i_1 - 1])))), ((-((~(arr_10 [i_0] [i_1])))))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
