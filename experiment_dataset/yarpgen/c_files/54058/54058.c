/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54058
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54058 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54058
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 -= var_7;
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 13;i_2 += 1)
            {
                {
                    var_15 = (min(9771, 1));
                    var_16 = (min(((max(32767, -32765))), 9772));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 13;i_4 += 1)
                        {
                            {
                                arr_15 [i_0] [i_0 - 1] [12] [i_1] [i_2 - 1] [0] [i_4] |= 55764;
                                var_17 = (max(((max(((max(88, 32767))), (min(46803, 1))))), (min(-8633590218991462532, 1))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_18 = var_4;
    var_19 = (min(131, 1105083719105189249));
    var_20 = var_0;
    #pragma endscop
}
