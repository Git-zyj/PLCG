/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175295
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175295 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175295
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {

        /* vectorizable */
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            var_10 = ((!(2861453560 % 2049615777)));
            var_11 = (arr_2 [i_0]);
            var_12 = (arr_2 [i_0]);
            var_13 = ((!(!2245351523)));
            var_14 = (arr_1 [i_0] [i_0]);
        }
        /* vectorizable */
        for (int i_2 = 0; i_2 < 10;i_2 += 1) /* same iter space */
        {
            var_15 = -927836312;
            var_16 = (arr_3 [i_2] [i_0]);
            /* LoopNest 2 */
            for (int i_3 = 1; i_3 < 6;i_3 += 1)
            {
                for (int i_4 = 3; i_4 < 7;i_4 += 1)
                {
                    {
                        var_17 = (min(var_17, ((((arr_9 [i_0] [i_4 + 2] [i_3 + 3]) >> (((arr_9 [i_0] [i_4 + 2] [i_3 - 1]) - 24169)))))));
                        var_18 = (((arr_4 [i_0]) ? 4252494458 : (arr_11 [i_3] [i_3 + 3] [i_3 - 1] [i_4 - 3] [i_3 - 1] [i_3])));
                    }
                }
            }
        }
        for (int i_5 = 0; i_5 < 10;i_5 += 1) /* same iter space */
        {
            var_19 = ((148 && (arr_12 [i_0] [i_5] [i_0] [i_5] [i_5])));
            var_20 *= var_6;
        }
        var_21 &= (arr_5 [4]);
        var_22 |= var_6;
    }
    for (int i_6 = 0; i_6 < 14;i_6 += 1)
    {
        var_23 = (arr_19 [i_6]);
        var_24 ^= var_1;
    }
    var_25 = var_2;
    var_26 = var_7;
    #pragma endscop
}
