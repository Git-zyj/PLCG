/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190943
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190943 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190943
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            {
                var_10 = -2024833263;
                arr_4 [i_0] [i_1 + 1] [i_1] = (max((arr_3 [i_0 + 1]), ((min(2955621350, -var_0)))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 1; i_2 < 12;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 13;i_3 += 1)
        {
            {
                arr_9 [i_2 + 1] = var_0;
                arr_10 [12] = var_3;

                /* vectorizable */
                for (int i_4 = 3; i_4 < 12;i_4 += 1)
                {
                    arr_13 [i_2] [i_4] [i_4] [i_2 - 1] = (((var_1 != var_7) <= (var_1 <= var_1)));
                    var_11 = -2024833253;
                    arr_14 [i_4] [i_4 - 2] [12] [i_3] = var_7;
                }
                var_12 = (((((min((arr_8 [i_2] [i_3]), var_6)))) != ((67108832 ? 35704 : (1 < 29806)))));
            }
        }
    }
    #pragma endscop
}
