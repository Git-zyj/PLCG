/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195832
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195832 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195832
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = 24316;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_6 [i_0] = ((~(!202)));
                var_19 = (max(((((((var_15 + 2147483647) << (((arr_3 [i_0]) - 24))))) ? (((arr_4 [i_0]) ? 202 : (arr_0 [i_0]))) : ((var_12 ? (arr_5 [i_0] [i_1] [i_1]) : (arr_3 [i_0]))))), (((!(arr_2 [i_0]))))));
            }
        }
    }
    var_20 = (min(-var_12, (((min(var_8, var_0))))));
    var_21 ^= ((-(((var_3 + var_10) ? (65 - var_3) : var_15))));
    #pragma endscop
}
