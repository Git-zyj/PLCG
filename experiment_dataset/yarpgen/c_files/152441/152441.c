/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152441
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152441 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152441
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_12;
    var_17 = (~var_13);

    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {

        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            arr_6 [i_1] [10] = ((((((arr_2 [i_1]) & var_6))) ? (((((-1 + 2147483647) << (4294967286 - 4294967286))) << (((arr_2 [i_0]) - 153)))) : (((~var_10) / ((-(arr_1 [i_0])))))));
            var_18 = (arr_1 [i_1 - 1]);
        }
        var_19 = (max((((arr_1 [i_0 + 1]) & var_0)), ((((max(var_1, -1967928685))) ? -1967928685 : -107))));
        arr_7 [i_0 + 1] [i_0 - 1] &= ((((max(2080374784, 150))) ? var_8 : (arr_1 [i_0 - 2])));
    }
    #pragma endscop
}
