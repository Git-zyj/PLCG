/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 8626
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=8626 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/8626
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    arr_10 [i_2] [i_2] = (!((max(14571998859271326889, (arr_4 [i_2] [i_0] [i_0])))));
                    var_14 = min((((-(arr_1 [7] [i_1])))), (min(((var_4 ? (arr_3 [i_0] [1] [1]) : (arr_7 [i_0] [1]))), (arr_7 [i_0] [i_2]))));
                }
            }
        }
    }
    var_15 = (((12 + var_13) ? var_8 : var_12));
    #pragma endscop
}
