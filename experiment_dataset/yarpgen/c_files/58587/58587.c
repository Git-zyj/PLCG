/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 58587
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=58587 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/58587
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_13 = ((((((!(arr_3 [i_0] [i_1]))))) ^ var_11));
                var_14 = (max(var_14, (arr_3 [i_0] [i_0])));
            }
        }
    }
    var_15 = var_1;
    var_16 = var_3;

    for (int i_2 = 0; i_2 < 25;i_2 += 1)
    {
        var_17 = (255 == 18446744073709551615);
        arr_8 [i_2] = ((-((~(arr_6 [i_2] [i_2])))));
        var_18 -= ((~((((arr_5 [i_2]) < (var_2 || 0))))));
    }
    for (int i_3 = 0; i_3 < 24;i_3 += 1)
    {
        var_19 = ((((((~var_11) <= (~var_5)))) < var_0));
        arr_11 [i_3] = 1;
    }
    for (int i_4 = 0; i_4 < 15;i_4 += 1)
    {
        arr_14 [i_4] = (((max((arr_4 [i_4]), (arr_0 [i_4] [i_4])))) == 1);
        var_20 ^= var_7;
    }
    /* LoopNest 2 */
    for (int i_5 = 0; i_5 < 11;i_5 += 1)
    {
        for (int i_6 = 0; i_6 < 11;i_6 += 1)
        {
            {
                var_21 = (0 * 1391819683);
                var_22 = (max(var_22, (((((arr_10 [i_5] [i_5]) <= var_3))))));
            }
        }
    }
    #pragma endscop
}
