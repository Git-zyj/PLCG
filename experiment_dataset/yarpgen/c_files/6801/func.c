/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6801
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6801 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6801
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned short i_0 = 3; i_0 < 9; i_0 += 1) 
    {
        for (unsigned short i_1 = 0; i_1 < 10; i_1 += 2) 
        {
            {
                var_12 -= ((/* implicit */signed char) arr_0 [i_0 - 2] [i_0 - 1]);
                var_13 ^= ((/* implicit */unsigned int) ((((/* implicit */_Bool) ((unsigned short) 11714641811334029794ULL))) ? (var_9) : (((/* implicit */int) ((signed char) arr_4 [i_0] [i_1] [i_0])))));
            }
        } 
    } 
    var_14 = ((/* implicit */int) (!(((/* implicit */_Bool) var_3))));
}
