/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66091
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66091 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66091
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((~((var_0 ? 30854 : var_4)))));
    var_20 = 207;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            {
                var_21 = (min(var_21, ((-(~0)))));
                arr_8 [i_0] [i_0] = var_13;
                var_22 = (max(0, ((max(var_8, 30100)))));
                var_23 = ((((min(-105, (arr_6 [i_1] [i_1])))) ? (((((7747580245863411633 ? 34359738367 : 23))) ? 0 : 251)) : (12674060019283350488 ^ 4432302959813252982)));
                arr_9 [i_0] [i_0] = 3994299477605899573;
            }
        }
    }
    #pragma endscop
}
