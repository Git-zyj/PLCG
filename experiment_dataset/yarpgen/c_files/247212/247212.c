/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247212
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247212 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247212
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_14;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_21 |= (arr_3 [6] [8] [i_1]);
                arr_4 [i_0] [i_0] [5] |= ((var_1 / (max((arr_2 [i_0]), 16777216))));
                var_22 = (103 | -31648);
                arr_5 [i_0] = ((1102544727 ? 684217210543066075 : 8699907872620828969));
                var_23 = ((((((arr_1 [i_1]) ? (arr_1 [i_0]) : (arr_0 [8] [i_1])))) ? (max((arr_1 [i_0]), (arr_0 [i_1] [i_0]))) : (arr_1 [i_1])));
            }
        }
    }
    var_24 = (((!153) ^ var_13));
    #pragma endscop
}
