/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83609
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83609 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83609
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            {
                var_16 -= (arr_3 [i_1 + 1]);
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 23;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 22;i_3 += 1)
                    {
                        {
                            var_17 = ((((((((max((arr_2 [i_2 - 2]), (arr_3 [9]))))) / var_7))) ^ ((min((arr_8 [4] [i_2 + 2]), (arr_1 [i_0]))))));
                            arr_9 [i_1] [i_2] [i_1] [i_1] = (((((((arr_2 [i_2]) - (arr_2 [i_0 + 2]))))) ? 36177135 : ((((arr_7 [i_3 + 1] [i_3 + 2] [i_3 + 1] [i_3 - 1]) ? (arr_8 [i_1] [i_1 + 2]) : (arr_0 [i_2 - 2]))))));
                        }
                    }
                }
                arr_10 [0] [i_1] = min((arr_6 [i_0] [i_1 - 1] [21]), 39034);
            }
        }
    }
    var_18 |= (((~(var_9 || var_9))));
    #pragma endscop
}
