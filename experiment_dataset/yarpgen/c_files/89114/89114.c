/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89114
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89114 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89114
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 19;i_1 += 1)
        {
            {
                var_12 = var_1;
                arr_6 [i_0] [i_1] = ((-((-(arr_0 [i_1 - 2])))));
                var_13 = ((!((max((arr_0 [i_1 - 3]), var_9)))));
                arr_7 [i_0] [i_1 - 3] [i_1] = (((arr_0 [19]) ? (min((arr_4 [i_1] [i_1] [i_1 + 1]), (arr_4 [i_0] [i_1 + 1] [5]))) : ((((arr_3 [i_1]) ? (arr_4 [i_0] [i_0] [3]) : var_9)))));
            }
        }
    }
    var_14 = 46;
    var_15 = (!-var_6);
    #pragma endscop
}
