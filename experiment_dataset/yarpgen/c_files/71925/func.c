/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71925
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71925 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71925
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned char i_1 = 0; i_1 < 21; i_1 += 2) 
        {
            for (unsigned int i_2 = 2; i_2 < 17; i_2 += 4) 
            {
                {
                    var_14 = ((/* implicit */unsigned char) arr_0 [i_0]);
                    var_15 = ((/* implicit */signed char) max((var_15), (((/* implicit */signed char) ((arr_7 [i_2 + 2] [i_2 + 4] [i_2 + 2]) ? (((arr_6 [i_2 + 4] [i_2 - 2] [i_2 + 4]) - (arr_6 [i_2 - 2] [i_2 - 2] [i_2 + 3]))) : (((/* implicit */long long int) ((/* implicit */int) ((((/* implicit */int) arr_7 [i_2 + 4] [i_2 + 1] [i_2])) < (((/* implicit */int) arr_7 [i_2 + 3] [i_2 + 1] [i_2])))))))))));
                }
            } 
        } 
    } 
    var_16 = ((/* implicit */unsigned short) min((var_16), (((/* implicit */unsigned short) var_13))));
}
