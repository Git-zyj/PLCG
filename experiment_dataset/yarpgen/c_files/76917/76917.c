/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76917
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76917 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76917
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_8;

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_14 ^= ((+(((arr_0 [i_0]) ? (arr_1 [i_0] [i_0]) : (arr_1 [i_0] [i_0])))));
        var_15 = (max(var_15, (((-(arr_0 [i_0]))))));
        arr_2 [i_0] = ((max((arr_1 [i_0] [i_0]), (arr_0 [i_0]))));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_16 = (max(var_16, (((+(((arr_6 [i_0] [i_1] [i_0]) ? ((((arr_6 [i_0] [i_1] [i_0]) || (arr_5 [i_0] [i_0])))) : ((max((arr_8 [i_0] [i_0] [i_2] [i_1]), (arr_3 [i_0] [i_2])))))))))));
                    var_17 = (max((max(((((arr_0 [i_0]) ? (arr_4 [i_1] [i_1]) : (arr_8 [i_0] [i_1] [i_2] [i_2])))), (max((arr_1 [i_0] [i_0]), (arr_6 [i_0] [i_1] [i_1]))))), ((max(-32281, -171522902)))));
                }
            }
        }
    }
    #pragma endscop
}
