/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220929
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220929 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220929
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 ^= var_1;
    var_12 = (min(var_9, (min(var_1, var_5))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                arr_5 [i_0] = (min(13691193302578943089, 14341833454129904695));
                var_13 += ((((max((arr_3 [i_1 + 1] [i_1 - 1]), 686735933401328189))) ? (((min(23812, (arr_2 [i_1] [i_1 + 1] [i_1 - 1]))))) : (((arr_1 [i_1 + 1]) ? 10498573474807237447 : (arr_3 [i_1 + 1] [i_1 - 1])))));
            }
        }
    }
    #pragma endscop
}
