/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99713
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99713 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99713
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_3;
    var_20 ^= 13531096415178692731;

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {

        /* vectorizable */
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            var_21 = ((((-509010530705022667 && (arr_1 [i_0] [i_0])))) != 251);
            var_22 = (max(var_22, (249 - 225)));
        }
        var_23 = (min(var_23, ((~(min(((min(255, var_15))), (30 & var_17)))))));
        arr_4 [i_0] = (((max(var_7, var_13)) + var_14));
    }
    #pragma endscop
}
