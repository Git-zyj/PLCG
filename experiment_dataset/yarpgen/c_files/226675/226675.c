/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226675
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226675 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226675
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max(var_15, (((var_11 ^ ((-1640997159 ? 110 : 139)))))));
    var_16 += 134;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 6;i_1 += 1)
        {
            {
                var_17 -= (1 ? 145 : 113);
                /* LoopNest 3 */
                for (int i_2 = 1; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 7;i_4 += 1)
                        {
                            {
                                arr_12 [i_0] [i_0] [i_0] = (((var_0 + 146) ? var_5 : var_7));
                                var_18 = ((((((((var_3 ? 2097152 : (arr_6 [i_2] [i_3])))) - ((var_14 ? (arr_1 [i_0]) : var_4))))) ? (max(32, 1523506186)) : 126));
                                arr_13 [i_0] [i_1 + 2] [i_2] [i_0] [i_0] [i_0] = var_6;
                            }
                        }
                    }
                }
            }
        }
    }
    var_19 = (max(var_19, (((-var_13 / (max(var_7, var_5)))))));
    #pragma endscop
}
