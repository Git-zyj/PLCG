/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148075
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148075 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148075
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((((882956972 ? -5655756872196643675 : var_0))));
    var_11 = (max(var_11, (~18446744073709551615)));
    var_12 = ((var_7 <= (((var_9 << ((((var_9 ? var_8 : -5655756872196643689)) + 5655756872196643705)))))));
    var_13 = (min(var_13, (-6307613674807031896 & 1925078230)));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            {
                var_14 = (min(var_14, 15539));
                arr_6 [i_1] = ((-4806 ? 21030 : 4778));
            }
        }
    }
    #pragma endscop
}
