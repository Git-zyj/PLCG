/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116659
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116659 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116659
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */short) var_2);
    var_16 = ((/* implicit */unsigned char) (-(((/* implicit */int) (short)815))));
    /* LoopSeq 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_17 = ((/* implicit */unsigned char) max((var_17), (((/* implicit */unsigned char) (~(((unsigned int) arr_1 [0])))))));
        arr_2 [i_0] [i_0] = ((/* implicit */_Bool) var_4);
    }
    for (_Bool i_1 = 0; i_1 < 0; i_1 += 1) 
    {
        arr_7 [i_1] = ((/* implicit */unsigned short) ((((/* implicit */_Bool) (signed char)67)) && (((/* implicit */_Bool) arr_6 [i_1] [i_1]))));
        var_18 *= ((/* implicit */_Bool) (+(max((((/* implicit */long long int) var_4)), (arr_3 [i_1])))));
    }
    var_19 = ((/* implicit */unsigned short) var_6);
}
