/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132890
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132890 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132890
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (((max(var_6, ((1 ? 6919210571572881897 : 2095104))))) ? ((((max(var_8, 14669))) ? (min(var_1, -2095114)) : (!var_13))) : ((var_11 ? var_4 : ((max(var_12, 83))))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    var_16 = (min(var_16, (((((arr_3 [i_1 + 2] [i_2]) ? (arr_5 [i_0 - 1] [i_1 + 2]) : (arr_5 [i_0 + 2] [i_1 + 1])))))));
                    var_17 &= var_5;
                    arr_8 [i_1] = -1948998124;
                    var_18 = ((arr_2 [i_2] [i_1 + 1]) * (((30589 ? (((!(arr_4 [i_0] [i_0])))) : (!3)))));
                    var_19 = (min(var_19, var_1));
                }
            }
        }
    }
    #pragma endscop
}
