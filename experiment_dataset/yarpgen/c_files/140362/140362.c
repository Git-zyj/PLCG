/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 140362
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=140362 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/140362
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_4 [i_0] = var_11;

                for (int i_2 = 0; i_2 < 24;i_2 += 1)
                {

                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        var_12 = arr_1 [i_0] [i_1];
                        var_13 = (min(var_13, var_8));
                        var_14 = (arr_3 [i_2] [i_1] [i_2]);

                        for (int i_4 = 3; i_4 < 22;i_4 += 1)
                        {
                            var_15 -= (max((arr_5 [i_0]), (arr_3 [i_2] [15] [i_4])));
                            var_16 = (min(var_16, var_10));
                            arr_13 [i_4] [i_0] [i_0] = var_3;
                            arr_14 [i_0] [10] [i_0] [i_3] [i_4] [i_0] = var_5;
                            var_17 = (arr_8 [6]);
                        }
                    }
                    arr_15 [i_0] [16] [i_2] [i_0] = (max(var_6, var_4));
                }
                var_18 = (min(var_18, (arr_6 [i_1] [i_1] [i_1] [i_0])));
                var_19 = (max(var_19, var_7));
            }
        }
    }
    var_20 = var_4;
    var_21 = var_2;
    #pragma endscop
}
