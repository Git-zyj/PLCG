/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122908
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122908 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122908
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(var_15, var_7));
    var_16 = var_8;
    var_17 = var_9;

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        arr_4 [i_0] = (arr_2 [i_0] [i_0]);
        var_18 = (((((((arr_2 [i_0] [i_0]) ? (arr_1 [i_0]) : (arr_1 [i_0]))) == 65535)) ? ((((var_14 || (arr_2 [i_0] [i_0]))))) : (((arr_1 [i_0]) ? var_8 : 5244))));
    }
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        for (int i_2 = 2; i_2 < 18;i_2 += 1)
        {
            {
                arr_11 [20] = ((((max((arr_10 [i_2 + 1]), var_6))) ? (max((arr_10 [i_2 + 4]), (arr_10 [i_2 - 2]))) : (min((arr_10 [i_2 + 4]), var_8))));
                var_19 = (max(var_19, ((max(5244, (~-5833))))));
            }
        }
    }
    #pragma endscop
}
