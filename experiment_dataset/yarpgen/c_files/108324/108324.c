/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 108324
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=108324 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/108324
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [6] = 8942670250594473640;
                    arr_9 [i_0] = -8942670250594473640;
                    arr_10 [i_0] [i_0] = (max((min(((min((arr_6 [i_2] [i_2] [7] [7]), var_4))), 7502726243330066740)), (8942670250594473668 & -7502726243330066741)));
                }
            }
        }
    }
    var_18 = var_15;
    #pragma endscop
}
