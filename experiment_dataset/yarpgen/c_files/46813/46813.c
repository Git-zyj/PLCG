/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46813
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46813 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46813
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_20 = (min(var_20, (max((arr_0 [i_0] [i_1]), (((arr_0 [i_0] [i_1]) ? (arr_0 [i_1] [i_1]) : var_18))))));
                var_21 = (min(var_21, (arr_2 [i_1] [i_0])));
                var_22 = (max(var_22, 18));
                arr_5 [1] [3] [i_1] = ((((arr_3 [i_0]) % var_11)));
            }
        }
    }
    var_23 = var_18;
    var_24 = (((var_1 || var_14) ? var_0 : var_8));
    var_25 = ((70368744177663 == ((((max(var_15, var_10))) ? var_6 : 70368744177664))));
    #pragma endscop
}
