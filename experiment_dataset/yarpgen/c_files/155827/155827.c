/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155827
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155827 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155827
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 24;i_0 += 1)
    {
        arr_2 [i_0] [0] = (((+((var_10 ? (arr_0 [i_0] [i_0 - 2]) : (arr_1 [i_0]))))));
        var_18 = (min(var_18, ((((-1 % 38) + (arr_1 [12]))))));
    }
    var_19 = ((((((max(var_17, var_17)) ^ 218))) ? (var_15 != 211) : (((var_10 == var_10) ? -4536690875628258989 : (((min(var_15, var_3))))))));
    #pragma endscop
}
