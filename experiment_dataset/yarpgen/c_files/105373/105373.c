/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105373
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105373 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105373
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (25165824 - var_12);
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_19 ^= ((25165824 < (((var_7 >= var_11) & (124 * 2609616405)))));

                /* vectorizable */
                for (int i_2 = 0; i_2 < 25;i_2 += 1) /* same iter space */
                {
                    arr_9 [i_0 - 1] [i_1] [i_1] = (var_14 - 13993197375436304005);
                    arr_10 [i_0] [i_1] [11] = (12631974894088956893 << var_12);
                }
                for (int i_3 = 0; i_3 < 25;i_3 += 1) /* same iter space */
                {
                    arr_13 [i_0] [i_0] [i_1] = (4293918720 ^ 39366);
                    var_20 = var_6;
                }
                for (int i_4 = 0; i_4 < 25;i_4 += 1) /* same iter space */
                {
                    var_21 *= (-87 <= var_1);
                    var_22 = var_11;
                }
            }
        }
    }
    var_23 = var_3;
    var_24 += var_16;
    #pragma endscop
}
