/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11985
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11985 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11985
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned int i_0 = 0; i_0 < 21; i_0 += 1) 
    {
        for (unsigned short i_1 = 2; i_1 < 18; i_1 += 1) 
        {
            for (unsigned char i_2 = 0; i_2 < 21; i_2 += 4) 
            {
                {
                    arr_7 [i_0] [i_0] [i_0] [i_2] = ((/* implicit */unsigned char) min((((/* implicit */unsigned long long int) max((((/* implicit */int) var_16)), ((-(((/* implicit */int) arr_5 [i_0] [i_2] [i_2]))))))), (arr_2 [i_0])));
                    var_19 ^= ((/* implicit */unsigned char) arr_6 [i_0] [i_1] [i_0] [(unsigned char)0]);
                }
            } 
        } 
    } 
    var_20 = ((/* implicit */unsigned char) (~(((/* implicit */int) var_10))));
    var_21 = ((/* implicit */signed char) var_8);
}
