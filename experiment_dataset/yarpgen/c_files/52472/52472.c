/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52472
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52472 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52472
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 0;i_2 += 1)
            {
                {
                    var_10 = (max((((!(arr_4 [i_0] [i_0] [i_2])))), (max((arr_4 [i_0 - 2] [i_0] [i_0 - 1]), (arr_4 [i_0] [i_1] [i_2])))));
                    arr_8 [i_0 - 2] [i_1] [i_1] [i_2] = var_8;
                }
            }
        }
    }
    var_11 = (min(var_11, ((max(var_8, var_8)))));
    /* LoopNest 2 */
    for (int i_3 = 2; i_3 < 16;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 20;i_4 += 1)
        {
            {
                arr_13 [i_3] = var_7;
                /* LoopNest 2 */
                for (int i_5 = 1; i_5 < 17;i_5 += 1)
                {
                    for (int i_6 = 1; i_6 < 18;i_6 += 1)
                    {
                        {
                            arr_19 [i_3] = (arr_9 [i_3]);
                            var_12 = var_7;
                            var_13 = (((var_1 ? 1286143290583986111 : (arr_11 [i_3 + 2]))));
                        }
                    }
                }
                arr_20 [i_3] = (max((arr_0 [i_3 + 4]), (arr_7 [i_3 + 4])));
                arr_21 [i_3] = arr_11 [i_3 + 1];
            }
        }
    }
    #pragma endscop
}
