/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63917
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63917 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63917
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            {
                var_18 = ((var_2 * (arr_4 [i_1 + 1] [i_1 + 1])));
                /* LoopNest 3 */
                for (int i_2 = 1; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 15;i_4 += 1)
                        {
                            {
                                var_19 = (max(var_19, 1));
                                var_20 = (max(var_20, ((((arr_15 [i_1 - 1] [i_1 - 2] [i_2 - 1] [i_2 + 2] [i_4 - 2] [i_4 + 1] [i_4 - 1]) || ((((var_0 & (arr_11 [i_0] [i_1] [i_2] [i_1]))))))))));
                                var_21 = 1006923989;
                            }
                        }
                    }
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_5 = 0; i_5 < 13;i_5 += 1)
    {
        for (int i_6 = 2; i_6 < 12;i_6 += 1)
        {
            {
                arr_22 [i_5] [i_6] = -1006923992;
                var_22 = arr_10 [i_5] [i_6];
                var_23 = ((var_16 < ((var_17 ? (max(var_13, var_12)) : ((var_3 ? (arr_3 [i_5]) : (arr_5 [i_6] [i_5] [i_5])))))));
                var_24 |= (~((var_7 ? var_10 : var_10)));
            }
        }
    }
    #pragma endscop
}
