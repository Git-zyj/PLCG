/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 108977
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=108977 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/108977
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */signed char) (((-(((/* implicit */int) var_2)))) | (((/* implicit */int) (unsigned short)7))));
    /* LoopNest 3 */
    for (short i_0 = 1; i_0 < 17; i_0 += 2) 
    {
        for (short i_1 = 0; i_1 < 19; i_1 += 2) 
        {
            for (signed char i_2 = 1; i_2 < 18; i_2 += 4) 
            {
                {
                    arr_9 [(signed char)8] [i_0] [(signed char)8] [1] = var_18;
                    var_20 = ((/* implicit */signed char) max((var_20), (((/* implicit */signed char) var_11))));
                    /* LoopSeq 1 */
                    for (_Bool i_3 = 1; i_3 < 1; i_3 += 1) 
                    {
                        arr_12 [i_0] = ((/* implicit */short) var_9);
                        arr_13 [i_3] [i_3] [i_3 - 1] [i_0] [i_3 - 1] = ((/* implicit */unsigned short) max((((((/* implicit */_Bool) -1839433086)) ? (1863563958) : (((((/* implicit */int) (short)2046)) & (((/* implicit */int) (signed char)-29)))))), (var_8)));
                    }
                }
            } 
        } 
    } 
}
