/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210906
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210906 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210906
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 11; i_0 += 1) 
    {
        arr_3 [i_0] [i_0] = ((/* implicit */unsigned int) arr_2 [i_0] [i_0]);
        arr_4 [i_0] [i_0] = ((/* implicit */short) (!(((/* implicit */_Bool) ((((/* implicit */int) ((unsigned char) (unsigned char)118))) + (((/* implicit */int) var_10)))))));
    }
    var_12 += ((/* implicit */unsigned int) (~(((/* implicit */int) (unsigned char)167))));
    var_13 = ((/* implicit */unsigned long long int) var_6);
}
