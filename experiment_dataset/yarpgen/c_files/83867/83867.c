/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83867
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83867 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83867
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_13 = var_8;
        arr_3 [i_0] = (((max((arr_1 [i_0]), (arr_0 [i_0])))) ? ((max((arr_0 [i_0]), (arr_1 [i_0])))) : ((~(arr_2 [i_0]))));
        arr_4 [i_0] [i_0] &= ((~((~(16760832 | 0)))));
        arr_5 [i_0] = (((((var_2 || (~5)))) ^ ((25214 & ((var_11 ? -60 : -17))))));
    }
    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        arr_9 [i_1] = (((!(arr_8 [i_1]))));
        var_14 |= (max((((!((max((arr_8 [i_1]), (arr_6 [i_1]))))))), 992));
    }
    var_15 = var_4;
    #pragma endscop
}
