/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 69805
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=69805 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/69805
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned short) var_10);
    var_16 = ((/* implicit */_Bool) ((signed char) (!(((/* implicit */_Bool) ((signed char) (signed char)53))))));
    var_17 = ((/* implicit */unsigned long long int) var_8);
    var_18 = ((/* implicit */_Bool) var_0);
    /* LoopNest 3 */
    for (unsigned int i_0 = 1; i_0 < 10; i_0 += 4) 
    {
        for (signed char i_1 = 2; i_1 < 11; i_1 += 3) 
        {
            for (unsigned long long int i_2 = 1; i_2 < 11; i_2 += 3) 
            {
                {
                    var_19 *= ((/* implicit */unsigned int) (+(((/* implicit */int) min((((/* implicit */unsigned short) var_8)), (var_0))))));
                    /* LoopNest 2 */
                    for (signed char i_3 = 0; i_3 < 12; i_3 += 2) 
                    {
                        for (unsigned int i_4 = 0; i_4 < 12; i_4 += 2) 
                        {
                            {
                                var_20 *= max(((signed char)-53), (((/* implicit */signed char) (_Bool)1)));
                                var_21 = ((/* implicit */unsigned int) (+(1461616051756530738ULL)));
                                var_22 = ((/* implicit */signed char) var_6);
                            }
                        } 
                    } 
                }
            } 
        } 
    } 
}
