/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 81313
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=81313 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/81313
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */short) min((var_18), (((/* implicit */short) -1533634367))));
    var_19 &= var_5;
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 16; i_0 += 1) 
    {
        arr_2 [i_0] = arr_1 [i_0];
        var_20 += ((/* implicit */signed char) max((max((((/* implicit */long long int) var_7)), (var_16))), (((long long int) arr_1 [(short)10]))));
    }
    var_21 = ((/* implicit */long long int) var_10);
}
