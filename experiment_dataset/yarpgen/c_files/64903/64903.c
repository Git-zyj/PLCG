/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64903
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64903 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64903
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min(2147483647, 0));
    var_11 = ((15 >> (((1 != ((1 ? var_7 : 1)))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 10;i_2 += 1)
            {
                {
                    arr_6 [i_1] [i_1] = (((min((!1), (((arr_1 [9] [i_1]) ? var_0 : var_9)))) >> (arr_2 [i_0] [i_1] [i_0])));
                    arr_7 [i_0] [i_1] [i_1] [i_2 + 2] = ((max(1, 88)));
                    arr_8 [i_0] [i_1] [i_2 - 1] = 390751241;
                    arr_9 [i_0] [i_1] [i_2] = ((-17 ? (arr_5 [i_2 - 2] [i_1] [i_2 - 1]) : 1));
                }
            }
        }
    }
    var_12 = ((var_9 / (var_1 & 8556380160)));
    #pragma endscop
}
