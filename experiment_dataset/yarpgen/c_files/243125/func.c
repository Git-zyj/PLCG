/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 243125
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=243125 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/243125
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned char) max((-328840912), (((/* implicit */int) (unsigned char)77))));
    var_17 = ((/* implicit */unsigned int) (~((+(((/* implicit */int) var_15))))));
    /* LoopNest 3 */
    for (long long int i_0 = 1; i_0 < 17; i_0 += 1) 
    {
        for (unsigned char i_1 = 2; i_1 < 17; i_1 += 1) 
        {
            for (unsigned long long int i_2 = 0; i_2 < 18; i_2 += 2) 
            {
                {
                    var_18 = ((/* implicit */unsigned short) var_13);
                    var_19 += ((/* implicit */signed char) ((((/* implicit */_Bool) min((4229577376U), (3597062308U)))) ? (((/* implicit */unsigned long long int) (((!(((/* implicit */_Bool) 3597062297U)))) ? (((/* implicit */long long int) 697904998U)) : (arr_1 [i_2])))) : (((((/* implicit */_Bool) var_0)) ? (((/* implicit */unsigned long long int) ((((/* implicit */int) arr_5 [i_2])) + (var_0)))) : (((((/* implicit */_Bool) arr_2 [i_2])) ? (var_1) : (var_1)))))));
                }
            } 
        } 
    } 
}
