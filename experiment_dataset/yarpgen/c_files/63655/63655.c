/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63655
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63655 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63655
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 2; i_2 < 7;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 8;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_11 = var_1;
                                arr_11 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0] = (max(((~(arr_4 [i_0]))), 54196));
                            }
                        }
                    }
                }
                var_12 = (((min((((var_4 ? -53 : var_9))), var_4)) * ((((((var_0 ? var_9 : (arr_10 [i_0] [i_0] [i_1] [i_1])))) * 0)))));
                var_13 = (min(((-(arr_9 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0]))), (min(54196, -7646891899590893506))));
            }
        }
    }
    #pragma endscop
}
