/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102593
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102593 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102593
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */short) (~(var_7)));
    /* LoopSeq 2 */
    for (unsigned long long int i_0 = 3; i_0 < 22; i_0 += 1) /* same iter space */
    {
        arr_2 [(signed char)19] = arr_0 [i_0 + 2];
        arr_3 [i_0 - 2] |= arr_1 [i_0];
        arr_4 [i_0 - 2] [i_0] = ((/* implicit */int) arr_1 [(short)10]);
    }
    for (unsigned long long int i_1 = 3; i_1 < 22; i_1 += 1) /* same iter space */
    {
        var_15 = ((/* implicit */unsigned short) ((int) 145985698));
        arr_9 [i_1] = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) 145985698)) ? (145985698) : (((/* implicit */int) (unsigned short)35537))));
        var_16 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) var_7)) ? (((/* implicit */int) arr_7 [i_1 - 1] [i_1])) : (((/* implicit */int) ((unsigned char) min((((/* implicit */long long int) var_6)), (arr_8 [i_1])))))));
    }
}
