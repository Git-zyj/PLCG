/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246569
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246569 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246569
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_0;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 11;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_12 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0] [i_0] = (arr_8 [4] [i_1] [i_2 + 2] [i_4]);
                                var_14 = (max(var_14, (arr_0 [i_4])));
                                var_15 = (max(var_15, (arr_4 [i_2])));
                            }
                        }
                    }

                    for (int i_5 = 0; i_5 < 14;i_5 += 1)
                    {
                        var_16 = (arr_4 [i_0]);
                        var_17 += (arr_4 [i_0]);
                    }
                    for (int i_6 = 0; i_6 < 1;i_6 += 1)
                    {
                        var_18 = (arr_15 [i_1] [i_1] [i_1] [i_1]);
                        arr_18 [i_0] [i_1] [i_2] [i_6] &= (arr_15 [i_0] [i_1] [i_2] [i_6]);
                    }

                    for (int i_7 = 0; i_7 < 14;i_7 += 1)
                    {
                        arr_23 [i_1] |= arr_7 [i_0] [i_0] [i_0];
                        var_19 = (arr_17 [i_7] [i_7] [i_2] [13]);
                    }
                }
            }
        }
    }
    var_20 *= var_6;
    /* LoopNest 2 */
    for (int i_8 = 0; i_8 < 1;i_8 += 1)
    {
        for (int i_9 = 0; i_9 < 1;i_9 += 1)
        {
            {
                var_21 = (max(var_21, (arr_25 [0])));
                /* LoopNest 2 */
                for (int i_10 = 0; i_10 < 21;i_10 += 1)
                {
                    for (int i_11 = 0; i_11 < 21;i_11 += 1)
                    {
                        {
                            var_22 = (arr_32 [13] [i_9] [18] [i_9] [i_9] [i_9]);
                            var_23 = arr_30 [i_10] [i_9] [i_8];
                            arr_36 [i_9] [i_9] [i_9] [i_8] = (arr_24 [i_11]);
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
