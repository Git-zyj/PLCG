/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60825
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60825 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60825
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            {
                var_12 ^= ((!((((arr_3 [i_0 - 1] [i_0 - 1] [i_1 - 2]) - (arr_3 [i_0 - 1] [i_1 - 2] [i_1 - 1]))))));
                var_13 = -5238;
                var_14 = (((((arr_2 [i_1]) > (arr_2 [i_0 - 1])))) == ((((((arr_6 [i_0] [i_0] [i_0]) ? (arr_0 [i_0]) : -1))) ? 255 : (arr_0 [i_1]))));
            }
        }
    }
    var_15 = (min(var_15, (((var_3 / ((var_6 / (max(var_1, var_3)))))))));
    #pragma endscop
}
