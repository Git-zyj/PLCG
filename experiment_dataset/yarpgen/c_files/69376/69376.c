/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 69376
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=69376 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/69376
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 &= 16778161407519054384;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_0] = var_4;
                    var_17 = (-481927587889155761 + var_9);
                    var_18 = (((((1023 % (arr_0 [i_0] [i_0])))) || (((var_1 - (arr_0 [11] [i_0]))))));
                    var_19 = -14;
                }
            }
        }
    }
    var_20 += (((((var_5 ? var_15 : (!4098351270)))) > (((((16778161407519054384 ? var_3 : var_2))) ? var_10 : 0))));
    #pragma endscop
}
