/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38985
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38985 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38985
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) var_2)) && (((/* implicit */_Bool) (+((-(915711838))))))));
    /* LoopSeq 1 */
    for (long long int i_0 = 2; i_0 < 19; i_0 += 1) 
    {
        var_14 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) ((((/* implicit */_Bool) arr_0 [i_0 + 2])) ? (((/* implicit */int) var_11)) : (((/* implicit */int) arr_0 [i_0 - 1]))))) || (((/* implicit */_Bool) (+(var_5))))));
        var_15 = ((/* implicit */unsigned short) (-(((/* implicit */int) arr_0 [i_0 + 2]))));
    }
    var_16 = ((/* implicit */int) ((((/* implicit */unsigned int) ((/* implicit */int) var_7))) + ((-(var_12)))));
}
