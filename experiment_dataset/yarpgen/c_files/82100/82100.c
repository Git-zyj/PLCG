/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82100
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82100 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82100
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 &= ((-((min(7, 18518)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_16 = ((!((((~13207) ? ((max((arr_1 [i_0] [0]), var_10))) : (((arr_3 [i_0] [i_0] [i_0]) ? var_0 : (arr_4 [i_0] [i_0] [i_0]))))))));
                var_17 = (((arr_4 [i_0] [i_0] [i_1]) - (((arr_1 [i_0] [i_0]) ? (arr_2 [12]) : -32138))));
                arr_5 [i_0] [21] = ((-32138 ? 464687502 : 129));
                var_18 &= (((((22910 <= ((0 ? 1 : 13207))))) << (((arr_1 [i_1] [i_1]) + 17959))));
            }
        }
    }
    #pragma endscop
}
