/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241939
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241939 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241939
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 &= var_12;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 20;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 0;i_3 += 1)
                    {
                        arr_11 [i_0] [i_1 - 1] [i_2] = (((arr_7 [i_1] [i_1]) + ((-(arr_5 [i_2 - 1])))));
                        var_16 = ((3110637517 >= (((min((((16128 > (arr_7 [i_0] [i_0])))), 39845))))));
                    }
                    arr_12 [i_2] [i_2] [i_2] = (min((arr_4 [i_1] [i_1] [i_2]), (arr_9 [i_0] [i_0] [i_2])));
                    var_17 = (max(var_17, (((!(19722 * 1))))));
                }
            }
        }
    }
    var_18 = var_11;
    #pragma endscop
}
