/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 5995
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=5995 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/5995
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(var_15, var_12));

    for (int i_0 = 2; i_0 < 17;i_0 += 1)
    {
        arr_2 [i_0] = -899881273;
        arr_3 [i_0] = max((arr_1 [i_0]), (((!(arr_0 [i_0 + 2] [i_0 + 1])))));
    }
    for (int i_1 = 3; i_1 < 12;i_1 += 1)
    {
        /* LoopNest 3 */
        for (int i_2 = 2; i_2 < 12;i_2 += 1)
        {
            for (int i_3 = 1; i_3 < 12;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 13;i_4 += 1)
                {
                    {
                        var_16 = (min(var_16, (-9223372036854775807 - 1)));
                        var_17 = ((-1045047843 ? (((var_12 ? (arr_0 [i_2 + 1] [i_3 - 1]) : (arr_0 [i_2 + 1] [i_3 + 1])))) : (max(((var_7 ? (arr_6 [i_1 - 2]) : (arr_1 [i_3]))), ((var_2 ? (arr_6 [i_2]) : -1945393870))))));
                    }
                }
            }
        }
        var_18 = ((!(arr_15 [i_1] [i_1 + 1])));
        arr_17 [i_1] = var_13;
    }
    #pragma endscop
}
