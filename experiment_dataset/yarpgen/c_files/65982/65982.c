/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 65982
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=65982 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/65982
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            {
                var_12 = (min(var_12, (((((((min(var_1, var_1))) ? (var_4 > var_0) : (min(var_4, var_8)))) - ((var_8 ? ((max(var_2, var_6))) : var_11)))))));
                var_13 = (min(var_13, ((((((var_4 ? ((var_2 ? var_8 : var_9)) : (var_6 & var_0)))) ? (((max((min(var_0, var_8)), (3211280201913333907 <= -22))))) : (((((var_4 ? var_11 : var_2))) ? (var_8 != var_7) : var_11)))))));
            }
        }
    }
    var_14 = (min(var_0, var_2));
    var_15 |= var_8;
    #pragma endscop
}
