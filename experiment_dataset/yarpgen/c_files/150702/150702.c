/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150702
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150702 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150702
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((var_17 / var_5) * var_7));
    var_20 = var_9;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 17;i_4 += 1)
                        {
                            {
                                var_21 |= var_16;
                                arr_14 [i_0 + 1] [12] [i_0] [i_3] [0] = (((((var_8 ? var_6 : (max(var_0, var_16))))) && ((min((var_0 | var_13), 1)))));
                            }
                        }
                    }
                    arr_15 [i_0] [i_1] [i_0] = ((var_1 ? (-127 - 1) : (arr_1 [i_0])));
                    var_22 = (max(var_3, (127 < -114)));
                    var_23 = ((arr_8 [i_0] [i_1] [i_0] [i_0]) | var_13);
                    /* LoopNest 2 */
                    for (int i_5 = 4; i_5 < 15;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 1;i_6 += 1)
                        {
                            {
                                var_24 = var_7;
                                var_25 = var_18;
                            }
                        }
                    }
                }
            }
        }
    }
    var_26 = (var_3 > var_14);
    #pragma endscop
}
