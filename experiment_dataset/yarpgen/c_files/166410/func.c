/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166410
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166410 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166410
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */int) var_12);
    var_16 = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) var_1)) ? (var_4) : (((((/* implicit */_Bool) (+(var_0)))) ? (((/* implicit */int) var_9)) : (((/* implicit */int) var_5))))));
    var_17 = ((/* implicit */short) var_0);
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 12; i_0 += 4) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 12; i_1 += 1) 
        {
            {
                var_18 = ((/* implicit */signed char) var_8);
                var_19 = ((/* implicit */int) arr_2 [i_1] [i_0] [i_0]);
            }
        } 
    } 
}
