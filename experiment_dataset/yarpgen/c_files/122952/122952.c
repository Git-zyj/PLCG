/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122952
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122952 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122952
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (~209);
    var_21 = ((var_2 & ((((max(var_0, var_8))) & (-30760 | var_5)))));
    var_22 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] [i_1] = ((((~(arr_4 [i_1 + 1] [i_1 + 1]))) & ((11 ? (arr_4 [i_1 + 2] [i_1 + 2]) : (arr_4 [i_1 + 1] [i_1 - 1])))));
                arr_7 [i_1] = ((-(56 / -21827)));
                arr_8 [i_1] [8] [i_1] = (arr_4 [i_0] [i_0]);
            }
        }
    }
    var_23 = (((max(var_12, ((var_18 ? var_2 : var_13)))) / (((-32767 - 1) * var_8))));
    #pragma endscop
}
