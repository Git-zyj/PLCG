/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156449
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156449 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156449
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0] = (((((+(((arr_3 [i_0] [i_0] [i_1]) ? (arr_2 [i_0]) : var_15))))) / 9));
                var_19 = (max((((arr_5 [i_0] [i_0 + 1] [i_0]) ? (arr_5 [i_0] [i_0 + 1] [i_0]) : (arr_5 [i_0] [i_0 + 1] [i_0 + 1]))), var_1));
                arr_7 [i_0 - 1] [i_1] [i_0] = var_14;
            }
        }
    }
    var_20 -= var_5;
    var_21 = (((((max(var_17, 82)))) ? var_10 : (((((var_13 ? 13 : -9409))) ? var_17 : var_6))));
    #pragma endscop
}
