/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 1589
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=1589 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/1589
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_18 ^= (((arr_1 [i_0] [i_0]) + (arr_0 [i_0] [1])));
        var_19 = -20;
        /* LoopNest 3 */
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 10;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 8;i_3 += 1)
                {
                    {

                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            arr_14 [i_0] [i_0] [i_2] [i_3] [i_4] = (min(-118, 117));
                            var_20 = var_12;
                        }
                        var_21 = ((5610390705491056500 ? ((-1 ? -120 : (arr_10 [8] [i_2 + 2] [i_3]))) : (arr_12 [i_2])));
                        var_22 = ((65521 ? 118 : 1));
                    }
                }
            }
        }
    }
    var_23 = (max((208 ^ var_13), 15358));
    var_24 = ((var_2 ? var_17 : var_11));
    #pragma endscop
}
