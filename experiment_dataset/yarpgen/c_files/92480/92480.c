/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 92480
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=92480 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/92480
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(var_16, (((((var_4 / ((max(var_6, var_10))))) ^ 47845)))));
    var_17 = (min(var_1, ((var_5 ? var_3 : var_2))));
    var_18 = (min(var_18, var_15));
    var_19 = var_8;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 15;i_2 += 1)
            {
                {
                    var_20 = (max(var_20, ((max((arr_3 [i_0] [i_1]), 47845)))));
                    arr_9 [i_0] [i_0] [i_2] = (max((((arr_7 [i_0] [i_1] [i_1] [i_2 - 2]) % (arr_7 [i_0] [i_0] [i_2] [i_0]))), (((max((arr_1 [i_2] [i_1]), (arr_7 [i_0] [i_1] [i_1] [i_0])))))));
                }
            }
        }
    }
    #pragma endscop
}
