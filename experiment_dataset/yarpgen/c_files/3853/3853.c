/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3853
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3853 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3853
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min((~var_3), 50904421));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 8;i_2 += 1)
            {
                {
                    var_14 ^= (((~4244062874) ? (max((((arr_4 [i_1]) ? var_8 : 4244062871)), (arr_0 [i_0 + 2] [i_0]))) : (max((arr_1 [i_0]), (arr_1 [i_1])))));
                    arr_8 [i_1] &= var_11;
                    var_15 = (max(var_15, 1662706216526772043));
                    arr_9 [i_0 + 2] [i_0 + 2] [i_1] [i_2 - 2] = (var_12 <= 50904425);
                }
            }
        }
    }
    #pragma endscop
}
