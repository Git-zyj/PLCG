/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 183520
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=183520 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/183520
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */unsigned int) max((var_10), (((/* implicit */unsigned int) (-(((((/* implicit */int) var_7)) + (((((/* implicit */int) (signed char)-115)) / (((/* implicit */int) (unsigned char)112)))))))))));
    var_11 = ((/* implicit */unsigned char) var_2);
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 15; i_0 += 1) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_12 += ((/* implicit */unsigned short) (+(-344480495)));
                var_13 = arr_2 [i_0] [9U] [9U];
            }
        } 
    } 
    var_14 *= ((/* implicit */unsigned char) var_6);
}
