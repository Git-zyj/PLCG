/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61405
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61405 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61405
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_7 [i_0] [i_0] = ((var_10 | (((arr_4 [i_1]) ? (arr_4 [i_0]) : (arr_4 [i_0])))));
                arr_8 [6] [i_0] [i_0] |= ((1 ? ((((max((arr_6 [i_0] [i_0] [i_0]), (arr_0 [i_1] [i_0])))) + ((min((arr_0 [i_0] [i_0]), (arr_3 [i_1] [i_1])))))) : ((((arr_5 [4]) >= (arr_5 [1]))))));
            }
        }
    }
    var_11 -= ((~((var_1 ? var_0 : var_1))));
    #pragma endscop
}
