/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195867
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195867 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195867
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */unsigned long long int) var_13);
    /* LoopNest 2 */
    for (unsigned int i_0 = 1; i_0 < 17; i_0 += 4) 
    {
        for (long long int i_1 = 0; i_1 < 18; i_1 += 2) 
        {
            {
                var_18 += ((/* implicit */_Bool) var_0);
                arr_4 [i_0] = ((/* implicit */short) max((arr_0 [i_0 + 1]), (((/* implicit */unsigned int) ((int) var_1)))));
            }
        } 
    } 
    /* LoopNest 3 */
    for (long long int i_2 = 2; i_2 < 18; i_2 += 2) 
    {
        for (long long int i_3 = 1; i_3 < 20; i_3 += 2) 
        {
            for (int i_4 = 0; i_4 < 21; i_4 += 2) 
            {
                {
                    arr_13 [i_3 + 1] = ((/* implicit */_Bool) max((arr_11 [i_3 + 1] [i_3 - 1] [i_2] [i_2 + 3]), (((/* implicit */long long int) min(((short)31256), (((/* implicit */short) var_6)))))));
                    var_19 *= ((/* implicit */unsigned short) (!(((((/* implicit */int) (!(((/* implicit */_Bool) var_15))))) <= (((int) arr_6 [i_2]))))));
                    var_20 = ((/* implicit */long long int) ((int) ((unsigned char) min((var_10), (((/* implicit */int) var_6))))));
                }
            } 
        } 
    } 
}
