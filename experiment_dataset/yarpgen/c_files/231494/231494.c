/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231494
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231494 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231494
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_15 *= (min((min((((arr_3 [i_0] [i_0]) ? (arr_3 [i_0] [17]) : var_5)), (var_6 == var_4))), var_10));
                arr_4 [i_1] = 0;
                var_16 |= (min(((min((~var_12), var_2))), (min((arr_1 [i_0]), (((var_6 / (arr_3 [i_0] [i_1]))))))));
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 3; i_2 < 18;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 19;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 22;i_4 += 1)
            {
                {
                    var_17 |= ((-(min(var_12, var_1))));
                    var_18 = var_14;
                }
            }
        }
    }
    #pragma endscop
}
