/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41432
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41432 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41432
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        {
                            var_14 = (!((min(var_13, -12884901888))));
                            var_15 &= ((var_0 < (~18117)));
                        }
                    }
                }
                var_16 &= ((((((~(arr_8 [i_1] [i_1] [i_0] [6] [i_1] [i_0]))) + 2147483647)) << (((!(((-18098 ? var_3 : var_12))))))));
            }
        }
    }
    var_17 = var_4;
    var_18 = (((~var_12) ? var_5 : (((max(18117, -12884901889)) & (min(18098, var_0))))));
    var_19 = (((((!((max(var_7, var_2)))))) >> (var_7 != var_10)));
    #pragma endscop
}
