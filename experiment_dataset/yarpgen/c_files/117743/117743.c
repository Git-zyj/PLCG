/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117743
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117743 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117743
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_11;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 10;i_4 += 1)
                        {
                            {
                                var_21 ^= (((arr_9 [i_3] [i_4] [i_4] [i_0]) * ((~(((arr_7 [i_0] [i_0 - 1] [i_4]) ? var_0 : (arr_1 [1])))))));
                                var_22 = (arr_9 [i_0] [i_2] [i_0] [i_0]);
                                var_23 = (min(var_23, (arr_4 [3] [i_2])));
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 1;i_5 += 1)
                    {
                        for (int i_6 = 2; i_6 < 9;i_6 += 1)
                        {
                            {
                                var_24 = var_18;
                                var_25 ^= (max((arr_1 [i_0]), var_19));
                                arr_18 [i_0] [i_0] [0] [i_5] [i_2] = ((((((!(arr_0 [i_6])) && (1 || 36))))));
                                arr_19 [i_0] [4] [i_2] [4] [6] = ((var_2 ? 224 : 199));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
