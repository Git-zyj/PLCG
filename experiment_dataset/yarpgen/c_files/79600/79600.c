/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 79600
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=79600 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/79600
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_13 = (min(var_13, ((min(-var_11, (max((max(4010981802, (arr_2 [i_0] [i_1]))), (((var_7 ? var_11 : -2107454149))))))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 15;i_3 += 1)
                    {
                        {
                            var_14 = ((~(arr_5 [i_1])));

                            for (int i_4 = 0; i_4 < 1;i_4 += 1)
                            {
                                var_15 = ((~(arr_10 [i_0] [i_3] [i_3] [i_4])));
                                var_16 |= (max((((arr_2 [i_3 - 2] [i_0 - 1]) ? var_6 : (arr_2 [i_3 - 2] [i_0 - 2]))), (((arr_2 [i_3 - 2] [i_0 - 3]) | (arr_2 [i_3 + 1] [i_0 - 2])))));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
