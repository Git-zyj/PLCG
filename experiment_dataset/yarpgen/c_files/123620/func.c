/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123620
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123620 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123620
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */unsigned long long int) min((min((max((-616430764617849792LL), (((/* implicit */long long int) var_11)))), (4016712275377024948LL))), (min((var_16), (((/* implicit */long long int) min(((_Bool)1), ((_Bool)1))))))));
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 2; i_0 < 11; i_0 += 3) 
    {
        var_18 = ((/* implicit */unsigned long long int) arr_2 [i_0]);
        var_19 = arr_0 [i_0] [i_0];
        var_20 = ((/* implicit */unsigned int) (unsigned char)176);
        var_21 = ((/* implicit */unsigned int) (_Bool)1);
    }
}
