/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198212
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198212 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198212
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 18;i_2 += 1)
            {
                {
                    var_19 = (min(var_19, (!-253883680227118534)));
                    var_20 = ((!((min((arr_3 [i_2 - 1]), (arr_8 [i_1 + 1] [0] [1] [i_1 + 1]))))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 19;i_4 += 1)
                        {
                            {
                                arr_15 [i_0] [i_1] [i_0] [i_0] [i_3] [i_4] = (!1);
                                arr_16 [i_1] [i_0] = (((arr_12 [i_1] [i_1] [6] [i_1] [i_1]) >> (((((((-1 ? var_15 : -5))) ? var_4 : ((((arr_11 [i_0] [i_0]) > var_12))))) - 10211))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_21 = var_4;
    #pragma endscop
}
