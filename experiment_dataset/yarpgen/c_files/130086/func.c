/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130086
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130086 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130086
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 *= ((/* implicit */int) (!((!(((/* implicit */_Bool) 18446744073709551602ULL))))));
    var_16 = ((/* implicit */unsigned long long int) ((unsigned int) var_3));
    var_17 = (_Bool)0;
    var_18 = ((/* implicit */long long int) var_1);
    /* LoopNest 2 */
    for (_Bool i_0 = 1; i_0 < 1; i_0 += 1) 
    {
        for (long long int i_1 = 2; i_1 < 21; i_1 += 4) 
        {
            {
                arr_7 [i_1] [i_1] = ((/* implicit */_Bool) (+(((/* implicit */int) arr_5 [i_1 + 1] [i_0 - 1] [i_0 - 1]))));
                var_19 = ((/* implicit */unsigned long long int) var_6);
                /* LoopNest 2 */
                for (short i_2 = 0; i_2 < 22; i_2 += 1) 
                {
                    for (unsigned int i_3 = 0; i_3 < 22; i_3 += 4) 
                    {
                        {
                            var_20 = ((/* implicit */_Bool) var_8);
                            var_21 = ((/* implicit */unsigned long long int) arr_6 [i_0] [8U] [i_0]);
                        }
                    } 
                } 
            }
        } 
    } 
}
