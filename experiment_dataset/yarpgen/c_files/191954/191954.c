/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191954
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191954 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191954
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 -= (var_8 | 30057);
    var_11 = (!var_2);

    /* vectorizable */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        var_12 ^= var_9;
        var_13 = (arr_1 [5]);
    }
    for (int i_1 = 0; i_1 < 16;i_1 += 1)
    {
        var_14 = (min(var_14, (arr_1 [i_1])));
        var_15 = ((((1 ? (var_6 | 30057) : (-30051 * var_8))) << (((arr_1 [i_1]) + 908402863))));
    }
    for (int i_2 = 0; i_2 < 17;i_2 += 1)
    {
        var_16 = (max(var_16, var_8));
        /* LoopNest 3 */
        for (int i_3 = 3; i_3 < 15;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 17;i_4 += 1)
            {
                for (int i_5 = 0; i_5 < 17;i_5 += 1)
                {
                    {
                        var_17 ^= (max((((((arr_11 [4] [i_4] [i_3] [i_2]) ? (arr_6 [i_2]) : 14)))), 1));
                        var_18 = (((arr_6 [i_3 + 1]) || (arr_4 [i_2] [i_3 - 2])));
                    }
                }
            }
        }
        var_19 &= 57045;
    }
    var_20 += var_6;
    var_21 = (!(((((max(1802322276, var_5))) ? var_8 : var_3))));
    #pragma endscop
}
