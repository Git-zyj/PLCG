/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163046
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163046 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163046
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_11 = var_1;
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_12 |= var_5;
                                var_13 -= var_7;
                                var_14 = var_4;
                            }
                        }
                    }
                }
            }
        }
    }
    var_15 = ((0 | (1 < var_6)));
    var_16 -= (((((((min(var_0, var_10))) / (2147483647 > 0)))) ? var_4 : var_0));
    var_17 = (min(var_17, (((((min(2147483647, (min(8468221640432906944, var_3))))) ? (((var_8 | var_7) ? ((min(var_4, var_10))) : var_9)) : var_2)))));
    var_18 = var_5;
    #pragma endscop
}
