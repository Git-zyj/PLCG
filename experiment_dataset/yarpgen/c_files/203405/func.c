/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203405
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203405 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203405
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */signed char) ((((/* implicit */_Bool) var_9)) && (var_7)));
    /* LoopNest 2 */
    for (unsigned char i_0 = 3; i_0 < 18; i_0 += 2) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_18 &= ((/* implicit */unsigned int) ((((/* implicit */_Bool) ((var_1) / (((((/* implicit */_Bool) var_1)) ? (((/* implicit */int) var_10)) : (((/* implicit */int) var_8))))))) ? (((/* implicit */int) ((short) 1779274921U))) : (arr_2 [i_1])));
                var_19 = ((/* implicit */unsigned char) arr_3 [i_1]);
                var_20 *= ((/* implicit */signed char) arr_1 [i_0] [i_0]);
            }
        } 
    } 
}
