/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194161
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194161 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194161
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = -2025772837;
    var_21 = var_19;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 17;i_4 += 1)
                        {
                            {
                                var_22 = 2082307870;
                                arr_14 [i_4] [i_2] [i_2] [i_0] [i_4] [i_2] [i_0] = 2212659425;
                                var_23 = -2025772820;
                            }
                        }
                    }
                    arr_15 [i_2] [i_2] = 13751;
                    arr_16 [i_2] [i_2] = 2082307870;
                    var_24 = (-2147483647 - 1);
                    var_25 = 379232817;
                }
            }
        }
    }

    /* vectorizable */
    for (int i_5 = 0; i_5 < 20;i_5 += 1)
    {
        arr_19 [i_5] = 2212659425;
        arr_20 [i_5] = 60017;
        arr_21 [i_5] [4] = 60017;
        var_26 = 2212659442;
        var_27 = (min(var_27, -2025772830));
    }
    /* vectorizable */
    for (int i_6 = 3; i_6 < 7;i_6 += 1)
    {
        arr_24 [i_6 + 1] = 5518;
        arr_25 [i_6] = -2025772835;
        var_28 = (min(var_28, 2025772835));
    }
    var_29 = var_3;
    #pragma endscop
}
