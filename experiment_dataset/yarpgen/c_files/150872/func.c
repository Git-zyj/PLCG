/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150872
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150872 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150872
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned char) min((var_20), (var_19)));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned char i_0 = 3; i_0 < 14; i_0 += 1) 
    {
        var_21 -= ((/* implicit */unsigned char) ((((/* implicit */int) (_Bool)0)) * (((/* implicit */int) (unsigned char)158))));
        var_22 = ((/* implicit */unsigned int) (unsigned char)114);
    }
    var_23 = ((/* implicit */long long int) var_2);
}
