/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212952
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212952 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212952
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((var_8 ? (max(var_10, ((var_7 ? var_5 : var_4)))) : var_3));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [i_2] = ((-(arr_7 [i_0] [i_0] [i_2])));
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 9;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            {
                                arr_16 [i_4] [i_0] [i_2] [i_0] [i_0] = (max((max(7947450918722594916, 114)), (((((4294967295 ? (arr_4 [i_2] [i_4]) : 1369614975))) | var_3))));
                                arr_17 [i_0] [10] [i_3] [i_2] [i_0] [i_0] = ((-(~1)));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
