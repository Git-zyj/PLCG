/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144018
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144018 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144018
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 19;i_2 += 1)
            {
                {
                    var_19 = (max(var_19, (((!((!((var_7 == (arr_6 [i_2 + 2] [i_1 - 1] [i_1 - 3] [i_0]))))))))));
                    var_20 = (((((var_1 ? (arr_2 [i_2] [i_1]) : (((arr_7 [i_0] [i_0] [i_0 - 2]) % 255))))) ? (((((max((arr_1 [i_1]), var_6)))) % (min((arr_4 [i_0] [i_0] [i_0]), var_8)))) : (var_9 < var_6)));
                    var_21 = (max(var_21, (((((~(!var_5))) + 3)))));
                }
            }
        }
    }
    #pragma endscop
}
