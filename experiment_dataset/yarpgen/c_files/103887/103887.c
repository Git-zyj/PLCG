/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103887
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103887 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103887
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (!var_4);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_19 += var_10;
                                var_20 ^= var_7;
                            }
                        }
                    }
                }
                arr_12 [i_0] [i_0] |= (((arr_9 [i_0] [i_0] [i_0] [i_1] [i_1]) ? (((((-(arr_7 [i_0] [6] [i_1])))) ? (arr_8 [i_0] [i_0] [i_0] [i_1] [i_1] [i_0]) : ((((arr_6 [i_0] [1] [i_1]) ? var_13 : (arr_0 [i_1] [i_0])))))) : (((arr_4 [i_0] [i_0] [i_0] [i_1]) ? (arr_4 [i_0] [i_0] [i_0] [22]) : var_17))));
            }
        }
    }
    var_21 = var_9;
    var_22 |= (((var_7 ? var_10 : ((max(var_10, var_10))))));
    #pragma endscop
}
