/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80725
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80725 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80725
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_6;

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_14 = ((((((arr_0 [i_0]) <= (arr_0 [i_0])))) / ((((arr_0 [i_0]) || 22)))));
        var_15 = ((((((65524 > (arr_1 [i_0]))))) * (var_5 ^ -2)));
    }
    /* vectorizable */
    for (int i_1 = 3; i_1 < 21;i_1 += 1)
    {
        var_16 = (((arr_2 [i_1 - 2] [i_1 - 1]) != 21));
        arr_5 [i_1] [i_1] = ((var_10 > (arr_4 [i_1])));
    }
    var_17 = (((((4 * var_9) * var_5)) * var_1));
    #pragma endscop
}
