#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

int var_8 = 425352618;
signed char var_10 = (signed char)-1;
signed char var_11 = (signed char)63;
unsigned char var_14 = (unsigned char)44;
signed char var_18 = (signed char)127;
int zero = 0;
unsigned long long int var_19 = 10371904995874827706ULL;
unsigned char var_20 = (unsigned char)134;
signed char var_21 = (signed char)3;
unsigned long long int var_22 = 5861050652462395226ULL;
unsigned long long int var_23 = 17298087178584541832ULL;
signed char var_24 = (signed char)-13;
unsigned long long int arr_0 [18] ;
int arr_1 [18] ;
unsigned char arr_2 [18] [18] ;
int arr_3 [18] ;
void init() {
    for (size_t i_0 = 0; i_0 < 18; ++i_0) 
        arr_0 [i_0] = 9809174483978034248ULL;
    for (size_t i_0 = 0; i_0 < 18; ++i_0) 
        arr_1 [i_0] = -596326957;
    for (size_t i_0 = 0; i_0 < 18; ++i_0) 
        for (size_t i_1 = 0; i_1 < 18; ++i_1) 
            arr_2 [i_0] [i_1] = (unsigned char)119;
    for (size_t i_0 = 0; i_0 < 18; ++i_0) 
        arr_3 [i_0] = -732830489;
}

void checksum() {
    hash(&seed, var_19);
    hash(&seed, var_20);
    hash(&seed, var_21);
    hash(&seed, var_22);
    hash(&seed, var_23);
    hash(&seed, var_24);
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
