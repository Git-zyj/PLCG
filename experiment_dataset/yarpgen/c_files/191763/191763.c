/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191763
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191763 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191763
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((max(var_8, (var_1 + 2324048986))));

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        var_15 = (min(var_15, (((~(((83 >= (arr_0 [17])))))))));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1] = (((~(max(3211375982047262904, var_5)))));
                    arr_10 [i_0] [4] [i_0] [i_0] = ((((((arr_2 [i_0] [21] [i_0]) >= (arr_0 [i_2])))) <= 15235368091662288712));
                }
            }
        }
        arr_11 [i_0] [i_0] = ((var_4 || (((-(((arr_2 [i_0] [i_0] [i_0]) ^ 67)))))));
    }
    #pragma endscop
}
