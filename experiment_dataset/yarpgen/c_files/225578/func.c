/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 225578
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=225578 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/225578
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 10; i_0 += 4) 
    {
        arr_4 [i_0] [(signed char)0] = arr_0 [i_0];
        var_20 = ((/* implicit */unsigned int) ((_Bool) arr_3 [(_Bool)1] [(_Bool)1]));
    }
    var_21 = ((/* implicit */int) max((var_21), (((((/* implicit */int) (_Bool)0)) / (((/* implicit */int) (_Bool)1))))));
}
