/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72014
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72014 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72014
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */long long int) var_13);
    /* LoopNest 2 */
    for (unsigned char i_0 = 1; i_0 < 13; i_0 += 3) 
    {
        for (unsigned int i_1 = 0; i_1 < 14; i_1 += 2) 
        {
            {
                arr_5 [i_0] [i_0] [i_0] = ((/* implicit */unsigned short) (-(((/* implicit */int) ((_Bool) var_10)))));
                arr_6 [i_0 - 1] &= ((/* implicit */int) (!(((((/* implicit */unsigned long long int) var_7)) <= (6969217759474927756ULL)))));
            }
        } 
    } 
}
