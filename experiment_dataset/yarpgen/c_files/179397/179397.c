/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179397
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179397 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179397
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                var_15 = -6770;
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 16;i_3 += 1)
                    {
                        {
                            var_16 = ((~(arr_2 [i_1] [i_0])));
                            var_17 = (max(var_17, ((((((max((arr_5 [i_1] [6]), var_13)) ^ (6770 == -21949)))) ? ((max((arr_5 [10] [10]), (arr_4 [i_2 + 2] [i_2 + 2] [i_3 - 1])))) : ((64 ? ((min(-10110, -6770))) : (~var_2)))))));
                            var_18 = 112;
                            arr_11 [i_0] [i_1 - 1] [i_2 + 2] [i_3] = (arr_9 [i_3] [i_3 + 2] [i_1 + 3] [i_1] [i_0]);
                        }
                    }
                }
            }
        }
    }
    var_19 = var_3;
    #pragma endscop
}
