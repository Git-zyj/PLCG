/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 147275
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=147275 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/147275
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            {
                var_20 = (max(var_20, (((var_6 ? (-17759 + -17759) : (((arr_5 [i_1 + 1] [i_1 - 1]) ? (arr_5 [i_1 + 1] [i_1 - 1]) : var_13)))))));
                var_21 = ((((min((arr_0 [i_1 - 2] [i_1 - 1]), (arr_5 [i_1 + 1] [i_1 + 1])))) ? (((arr_0 [i_1 - 2] [i_1 - 1]) ? (arr_0 [i_1 + 1] [i_1 + 1]) : (arr_1 [i_1 + 1]))) : (!2047)));
                var_22 = (max(var_22, ((((arr_3 [i_1 - 2]) <= ((32766 ? var_0 : var_18)))))));
            }
        }
    }
    var_23 = (max(var_23, var_9));
    #pragma endscop
}
