/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 245705
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=245705 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/245705
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max(var_20, ((max(var_8, (~var_1))))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [i_0] = (arr_1 [i_0]);
                arr_5 [15] [i_0] = var_4;
                arr_6 [1] [i_0] = (arr_0 [i_0]);
                var_21 = (((((var_16 ? (arr_1 [i_1]) : (var_5 % 1)))) | 9223372036854775807));
            }
        }
    }
    var_22 -= var_9;
    var_23 = (((var_12 + 2147483647) >> ((((var_15 ? (-9223372036854775807 - 1) : var_15)) - 9223372036854775789))));
    #pragma endscop
}
