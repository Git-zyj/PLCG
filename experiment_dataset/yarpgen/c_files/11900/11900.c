/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11900
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11900 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11900
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_11, (((!(!var_6))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {

                for (int i_2 = 1; i_2 < 18;i_2 += 1)
                {
                    var_12 = (min(var_12, ((+(max((arr_6 [i_2] [i_2 + 1] [i_2 + 2]), (((-(arr_1 [i_2]))))))))));
                    var_13 = ((-(min(32672, (arr_7 [i_2 - 1] [i_2 + 2] [i_2])))));
                }
                var_14 |= ((~(((!((max((arr_3 [i_0]), (arr_6 [i_0] [i_0] [i_1])))))))));
                var_15 ^= (min(((min(-99, (arr_3 [i_0])))), (arr_3 [i_0])));
            }
        }
    }
    #pragma endscop
}
