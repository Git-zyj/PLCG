/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83553
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83553 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83553
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 16;i_4 += 1)
                        {
                            {
                                arr_16 [i_0] [i_1] [i_0] = (arr_15 [i_1 + 2] [i_4 - 3] [i_4] [i_4 + 1] [i_4 - 1]);
                                var_13 = ((((max((0 - var_5), (((arr_12 [i_0] [i_0] [i_0] [i_0]) % (arr_2 [i_0])))))) || ((((arr_12 [6] [i_0] [6] [i_3]) ? var_0 : var_6)))));
                                var_14 += 8474166318446555971;
                            }
                        }
                    }
                    var_15 = (min(var_15, ((max(0, 1)))));
                }
            }
        }
    }
    #pragma endscop
}
