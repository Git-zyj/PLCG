/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224156
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224156 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224156
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 19;i_3 += 1)
                {
                    {

                        for (int i_4 = 0; i_4 < 19;i_4 += 1)
                        {
                            var_12 ^= (arr_3 [i_0] [6]);
                            arr_15 [8] = arr_14 [i_0] [6] [i_0] [i_0] [i_0];
                        }
                        arr_16 [i_1] [i_1] [i_1] = (((((arr_10 [0] [1] [i_1] [i_1] [i_0] [i_0]) ? (arr_11 [i_0] [i_0] [i_2]) : (arr_1 [i_0]))) <= var_1));
                    }
                }
            }
        }
        var_13 = ((-(arr_0 [i_0])));
        arr_17 [i_0] [i_0] &= (-127 - 1);
    }
    var_14 = (max(var_14, ((min((((!(-127 - 1)))), var_2)))));
    #pragma endscop
}
