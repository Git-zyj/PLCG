/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68774
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68774 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68774
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */long long int) var_15);
    var_17 = ((/* implicit */unsigned long long int) (~(((/* implicit */int) var_4))));
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 13; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 11; i_1 += 4) 
        {
            {
                var_18 = ((/* implicit */unsigned long long int) 1677484708);
                var_19 = ((/* implicit */unsigned char) ((((((/* implicit */int) arr_5 [i_1 - 1] [i_1 - 1])) & (((/* implicit */int) arr_5 [i_1 - 1] [i_1 + 1])))) - (((/* implicit */int) ((unsigned short) ((int) arr_0 [i_1]))))));
                var_20 ^= ((_Bool) max((arr_2 [i_0]), (arr_2 [i_0])));
                var_21 = ((/* implicit */unsigned long long int) arr_2 [i_1]);
            }
        } 
    } 
}
