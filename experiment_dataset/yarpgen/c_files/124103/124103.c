/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124103
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124103 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124103
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 ^= (max((((~var_2) ? var_7 : ((var_7 ? var_7 : var_9)))), -var_1));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_0] [i_0] = ((var_6 ? (var_4 ^ var_4) : ((((!(arr_3 [i_0] [i_1] [i_2])))))));
                    arr_9 [i_0] [i_1] [i_0] = ((~((((((arr_6 [i_0] [i_2]) ? var_9 : var_8))) ? (arr_2 [i_0]) : (arr_7 [i_0] [i_0] [i_1] [i_0])))));
                    var_11 += var_6;
                }
            }
        }
    }
    var_12 = (min(var_12, var_5));
    #pragma endscop
}
