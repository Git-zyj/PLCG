/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59135
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59135 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59135
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_1;
    var_12 = var_6;

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        arr_2 [12] = ((((((arr_1 [i_0]) ? var_0 : (arr_1 [i_0])))) ? ((((max(var_10, (arr_1 [i_0])))) ? var_4 : ((42772 ? (arr_1 [i_0]) : (arr_1 [i_0]))))) : var_5));
        var_13 -= (((((-82 ? 124 : (arr_1 [i_0])))) ? (arr_1 [i_0]) : ((var_5 ? (arr_1 [i_0]) : (arr_1 [i_0])))));
    }
    var_14 = ((((var_2 ? ((var_7 ? 1 : var_5)) : (2147483647 * 1))) != (((1 != ((min(var_10, var_0))))))));
    #pragma endscop
}
