/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156184
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156184 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156184
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 *= ((((((var_9 != var_1) ? var_4 : (!17535008522148694839)))) && ((min((0 * 1), (var_11 && var_10))))));
    var_14 = (max(var_14, var_5));
    var_15 = var_11;

    /* vectorizable */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            var_16 = ((-(arr_3 [i_0 - 1] [i_0 - 1])));
            arr_6 [i_0] [i_1] = 4626696098167061226;
        }
        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            var_17 = (~1);
            arr_10 [i_0] [i_0] = -15739755096433140036;
        }
        var_18 -= ((14151583308922920069 ? 274877906943 : (arr_0 [i_0 - 1])));
        var_19 = (max(var_19, ((((arr_3 [i_0] [i_0 - 2]) ? ((1 ? (arr_2 [i_0]) : (arr_7 [i_0] [i_0] [i_0]))) : (1006632960 && -8))))));
    }
    #pragma endscop
}
