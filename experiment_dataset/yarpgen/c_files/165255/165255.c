/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165255
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165255 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165255
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min(var_10, (~var_0)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_11 = (min(var_11, ((((var_3 < 1) ? (~5) : ((18446744073709551615 ? 51 : 1)))))));
                var_12 -= (((min((max(var_1, var_2)), 207))) ? 106 : (~105));
                var_13 = (min(var_13, ((((((var_3 ? var_6 : ((54 ? 207 : var_0))))) ? ((-(max(187, (-2147483647 - 1))))) : -var_3)))));
                var_14 = (min(var_14, ((((~(!9885123254974616383)))))));
            }
        }
    }
    #pragma endscop
}
