/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 216895
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=216895 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/216895
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = (-(max((((/* implicit */unsigned int) (unsigned char)212)), (1797232434U))));
    var_18 *= ((/* implicit */unsigned char) var_4);
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 23; i_0 += 4) 
    {
        arr_2 [(short)7] = ((unsigned short) ((((/* implicit */int) arr_1 [i_0] [i_0])) & (((/* implicit */int) (unsigned char)186))));
        var_19 = ((/* implicit */short) (!(((/* implicit */_Bool) (short)16383))));
    }
    var_20 = ((/* implicit */unsigned int) var_4);
}
