/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 173647
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=173647 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/173647
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        arr_3 [i_0] = (((arr_2 [i_0]) >= (((arr_2 [i_0]) ^ var_7))));
        arr_4 [i_0] = (arr_2 [i_0]);
        var_12 = (((((var_9 < (arr_2 [i_0]))))) < var_8);
        arr_5 [i_0] [i_0] = ((((arr_1 [i_0] [i_0]) << (var_8 - 3508628253444821491))));
    }
    var_13 = (max(var_1, (((var_0 * var_1) / -2360486944))));
    var_14 = (min(((98 ? (((max(var_4, 22925)))) : -4053613452659084185)), (~var_8)));
    #pragma endscop
}
