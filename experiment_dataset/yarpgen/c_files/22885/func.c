/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22885
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22885 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22885
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */unsigned long long int) var_12);
    /* LoopSeq 2 */
    /* vectorizable */
    for (long long int i_0 = 2; i_0 < 17; i_0 += 4) 
    {
        arr_3 [i_0] = var_11;
        var_19 = ((/* implicit */long long int) (((~(var_10))) < (((/* implicit */unsigned int) ((/* implicit */int) var_14)))));
        arr_4 [i_0] [i_0] = ((/* implicit */short) var_2);
    }
    for (unsigned short i_1 = 0; i_1 < 19; i_1 += 2) 
    {
        arr_7 [i_1] = ((/* implicit */unsigned int) ((((/* implicit */_Bool) 0ULL)) ? (((/* implicit */unsigned long long int) 3114604404U)) : (18446744073709551615ULL)));
        var_20 = ((/* implicit */_Bool) min((-5379625180223458820LL), (((/* implicit */long long int) 3561970200U))));
    }
}
