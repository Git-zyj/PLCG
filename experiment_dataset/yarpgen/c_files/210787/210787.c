/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210787
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210787 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210787
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 -= ((!((max((59 / var_10), var_0)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 24;i_2 += 1)
            {
                {
                    var_19 |= var_15;
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_20 = 89441854991492459;
                                var_21 &= (((((((((arr_3 [i_0]) - (arr_8 [i_2])))) & (max(var_6, var_9))))) && (((((var_12 ? 201 : -11782)) - ((var_12 / (arr_12 [i_4] [i_2] [i_0]))))))));
                            }
                        }
                    }
                    var_22 *= (((-(arr_6 [i_2 - 1] [i_2 - 1] [i_1 + 2]))));
                }
            }
        }
    }
    var_23 = ((((((201 / var_4) / (max(var_4, var_11))))) ? var_15 : (~-var_15)));
    #pragma endscop
}
