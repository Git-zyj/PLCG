/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3618
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3618 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3618
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned char) var_10);
    /* LoopNest 3 */
    for (unsigned short i_0 = 0; i_0 < 22; i_0 += 4) 
    {
        for (unsigned short i_1 = 2; i_1 < 19; i_1 += 2) 
        {
            for (unsigned long long int i_2 = 3; i_2 < 21; i_2 += 4) 
            {
                {
                    arr_6 [i_0] [i_1] [i_2] [i_2 - 2] = ((/* implicit */short) arr_4 [i_2] [i_1] [i_0] [i_0]);
                    arr_7 [i_2 + 1] [i_2] [i_2] = ((min((arr_4 [i_2 - 2] [i_2 - 2] [i_2 - 3] [i_2]), (arr_4 [i_2 - 2] [i_2 - 2] [i_2 - 1] [i_2]))) ? (((((/* implicit */_Bool) (unsigned short)32607)) ? (((/* implicit */int) (short)32767)) : (((/* implicit */int) arr_4 [i_2 + 1] [7LL] [i_2 - 3] [i_2 + 1])))) : (((((/* implicit */int) arr_4 [i_2 - 1] [i_2] [i_2 - 3] [i_2 - 1])) * (((/* implicit */int) arr_4 [i_2 + 1] [i_2] [i_2 - 1] [i_2 + 1])))));
                }
            } 
        } 
    } 
}
