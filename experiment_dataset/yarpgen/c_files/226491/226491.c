/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226491
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226491 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226491
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 17;i_2 += 1)
            {
                {
                    var_19 = (arr_1 [i_0]);
                    var_20 *= (min((arr_4 [i_0] [i_0] [i_2 + 1]), ((((arr_3 [i_0 + 2]) || (arr_5 [i_0] [i_1 + 2] [i_2]))))));
                }
            }
        }
    }

    for (int i_3 = 1; i_3 < 14;i_3 += 1)
    {
        var_21 = var_14;
        var_22 *= (((((((arr_1 [4]) ? (arr_11 [i_3]) : var_0)) % (((-(arr_11 [i_3])))))) >> (((((var_17 / var_2) / (var_7 * var_3))) - 64636))));
        var_23 = (max(((+(max((arr_10 [i_3 + 1]), var_10)))), (arr_1 [2])));
        var_24 = (~879209410);
    }
    #pragma endscop
}
