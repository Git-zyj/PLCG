/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219120
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219120 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219120
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min(var_13, ((((var_3 != var_12) | (var_12 + 226))))));
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0 - 1] = (((max(0, 59111)) >> ((((((max(-17, (arr_1 [i_0] [i_0])))) ? (29 * var_5) : (var_0 && -70))) + 76657))));
                arr_6 [i_1] &= var_12;
            }
        }
    }
    var_14 = ((-var_12 - (((var_3 ? var_3 : ((var_5 ? var_0 : 59112)))))));
    #pragma endscop
}
