/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 248599
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=248599 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/248599
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 20;i_2 += 1)
            {
                {
                    var_15 = (min(((((max(var_5, var_1))) % ((var_7 ? (arr_5 [i_2] [i_2]) : 13299040378531101903)))), (arr_0 [i_0])));
                    var_16 = ((+(((arr_6 [i_2 - 2] [i_2] [i_2 - 1] [i_2]) ? (arr_6 [i_1] [i_1] [i_2 - 2] [i_2 - 3]) : var_7))));
                }
            }
        }
    }
    var_17 = (min(-126, ((var_10 ? ((var_14 ? var_0 : var_12)) : (!1)))));
    var_18 = ((var_0 ? var_13 : var_3));
    #pragma endscop
}
