/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 26517
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=26517 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/26517
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    var_17 |= ((-((-((-5692252542534335576 ? 0 : -22))))));
                    arr_8 [i_0] [i_0] [i_2] = (((((-(arr_2 [i_1])))) ? (((!(((~(arr_0 [15] [15]))))))) : (((!(arr_1 [i_0]))))));
                    var_18 = (((max(((!(arr_5 [18]))), (((arr_6 [i_1] [i_0]) && (arr_7 [i_0] [13] [i_2] [i_2]))))) || ((!((!(arr_4 [i_0] [i_1] [i_2])))))));
                }
            }
        }
    }
    var_19 = (min(((-(!var_16))), var_2));
    #pragma endscop
}
