/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187932
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187932 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187932
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((((~(19314 & 12950537811340827355))) * (((((-3505 < var_14) < var_7))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                arr_6 [i_1] = 42019;
                var_20 = ((((-(arr_3 [i_0] [0] [i_1]))) - -1023));
                var_21 *= (7753114051375105227 + -7753114051375105227);
                arr_7 [i_0] [i_1] [i_1] = ((-(var_9 * 1014)));
                var_22 = ((-(((!(arr_4 [i_1] [i_0] [i_0]))))));
            }
        }
    }
    var_23 = ((((((var_11 + var_8) - var_2))) || var_7));
    #pragma endscop
}
