/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27897
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27897 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27897
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = 9;
    var_15 = (min((((~19) ? 38598 : var_10)), 4899247609157802262));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_16 = ((+(((arr_8 [i_0] [i_1] [i_2]) ? 0 : 4294967293))));
                    arr_9 [i_0] [i_2] = max((((min(2, var_13)))), (max((arr_5 [i_0] [i_1] [i_2] [i_0]), (arr_5 [i_2] [i_2] [i_1] [i_0]))));
                }
            }
        }
    }
    #pragma endscop
}
