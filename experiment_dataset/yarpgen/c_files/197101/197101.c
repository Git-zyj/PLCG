/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197101
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197101 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197101
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [i_0 - 1] &= var_7;
                var_19 = (((arr_2 [i_0] [i_0] [i_0 - 1]) | 4294967295));
                var_20 = (min(var_20, ((((((((arr_2 [i_0 - 1] [i_0 - 1] [i_0 - 1]) ? 1023 : (arr_0 [i_0]))) >> (var_9 - 143453301))) == 20962)))));
            }
        }
    }
    var_21 = (max(var_21, (((2147483647 ? (-809462665 - -1924504990) : var_14)))));
    var_22 |= ((((max(var_5, 4294967283))) ? var_17 : (!var_16)));
    #pragma endscop
}
