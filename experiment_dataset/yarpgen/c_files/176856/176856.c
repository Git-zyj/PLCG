/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 176856
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=176856 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/176856
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (var_6 * var_1);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_12 = ((((((arr_3 [i_0] [i_0] [i_0]) ? 1 : (arr_3 [i_0] [i_0] [i_0])))) ? ((((min((arr_0 [8] [i_0]), (arr_1 [i_0])))) ? ((max(var_2, var_8))) : var_8)) : var_8));
                var_13 = ((var_2 <= (!var_6)));
                var_14 = ((arr_1 [11]) | (~30));
                var_15 = (((((arr_2 [i_1] [i_0]) ? var_8 : (arr_0 [i_0] [i_0]))) / (((arr_2 [i_1] [i_0]) ? (arr_2 [i_0] [i_1]) : (arr_0 [i_0] [i_0])))));
                var_16 = (arr_2 [i_1] [i_0]);
            }
        }
    }
    #pragma endscop
}
