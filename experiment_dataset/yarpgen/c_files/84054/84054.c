/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84054
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84054 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84054
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_11;
    var_19 = 39467;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 18;i_2 += 1)
            {
                {
                    var_20 = (arr_1 [i_2 + 1]);
                    var_21 = ((arr_7 [i_0] [i_0]) | (max((arr_8 [i_2 - 2] [5] [i_0]), (arr_8 [i_2 - 2] [i_1] [i_2]))));
                    arr_9 [10] &= var_15;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 19;i_4 += 1)
                        {
                            {
                                arr_14 [i_1] [i_2] = (arr_13 [i_0] [i_0] [i_0] [i_2] [i_4]);
                                var_22 = (!1);
                                var_23 = 26068;
                                var_24 = ((-(arr_5 [i_4] [i_2] [i_2])));
                            }
                        }
                    }

                    for (int i_5 = 0; i_5 < 19;i_5 += 1)
                    {
                        arr_17 [i_2] [i_1] [i_1] [i_2] [i_2] = (arr_4 [i_0] [i_1]);
                        var_25 += (arr_1 [i_1]);
                        var_26 |= (arr_1 [i_0]);
                        arr_18 [i_2] [i_2] = (var_13 - var_10);
                    }
                }
            }
        }
    }
    var_27 = var_10;
    #pragma endscop
}
