/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191938
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191938 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191938
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_15 = arr_2 [i_0];
        arr_3 [i_0] [i_0] = (2035117358 == 10);
        arr_4 [i_0] = (arr_2 [i_0]);
        arr_5 [i_0] = ((45584 <= ((127 ? var_8 : (arr_1 [i_0])))));
        arr_6 [4] = (2610921827 | 176);
    }

    for (int i_1 = 0; i_1 < 12;i_1 += 1)
    {

        for (int i_2 = 0; i_2 < 12;i_2 += 1)
        {
            arr_11 [4] = (((!593313977) | 25954));

            /* vectorizable */
            for (int i_3 = 0; i_3 < 12;i_3 += 1)
            {
                /* LoopNest 2 */
                for (int i_4 = 0; i_4 < 12;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 12;i_5 += 1)
                    {
                        {
                            var_16 = var_6;
                            arr_21 [i_1] [i_2] [i_3] [i_4] [i_3] = (25950 & var_6);
                        }
                    }
                }
                var_17 |= (arr_12 [i_1]);
            }
        }
        /* vectorizable */
        for (int i_6 = 0; i_6 < 12;i_6 += 1)
        {
            var_18 -= 25934;
            arr_24 [i_6] [i_1] [1] = (4012150837 && 7723066406591721252);
        }
        var_19 = (arr_16 [7]);
    }
    #pragma endscop
}
