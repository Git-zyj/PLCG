/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 5522
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=5522 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/5522
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 6;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_10 = (min((((arr_4 [i_1] [i_1] [i_0 + 3]) + (arr_1 [i_0 + 1]))), ((max(var_4, (arr_1 [i_0]))))));
                var_11 = ((var_5 ? ((((arr_4 [i_0 + 1] [i_0 + 3] [i_1]) > var_7))) : (((var_6 <= (arr_4 [i_0 + 1] [i_0 + 4] [i_0]))))));
                arr_6 [i_0 + 3] [i_0] = var_4;
                var_12 *= ((var_8 ? (((((1599035977811959278 ? 1 : 65))) - 0)) : ((min(2093771877, 19949)))));
                arr_7 [i_0] [6] = 65535;
            }
        }
    }
    var_13 = var_7;
    var_14 = (((min(6, 1))));
    var_15 -= var_8;
    #pragma endscop
}
