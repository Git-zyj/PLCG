/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78333
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78333 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78333
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (min(var_11, (((!(~var_10))))));
    var_18 = (var_13 >= var_14);
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 2; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 14;i_4 += 1)
                        {
                            {
                                arr_14 [i_0] [0] [i_2] [i_3] [i_0] [i_0] = var_14;
                                var_19 = 1;
                                var_20 = (arr_1 [i_0] [i_0]);
                            }
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_5 = 0; i_5 < 14;i_5 += 1)
                {
                    for (int i_6 = 0; i_6 < 14;i_6 += 1)
                    {
                        {
                            var_21 = (max(var_21, (((((arr_8 [i_0] [i_0] [i_5] [i_5] [i_6] [4]) ? (arr_3 [i_0] [i_0]) : (max(var_9, (arr_1 [i_0 + 2] [3]))))) << (var_14 - 4113718235)))));
                            arr_21 [i_0] [i_0] [i_5] [13] [1] [i_1] = var_16;
                        }
                    }
                }
            }
        }
    }
    var_22 = var_1;
    var_23 = var_9;
    #pragma endscop
}
