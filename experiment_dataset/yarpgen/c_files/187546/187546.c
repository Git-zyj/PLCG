/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187546
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187546 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187546
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 &= (~40026);

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (max(((((((arr_1 [i_0]) ? (arr_0 [i_0] [i_0]) : (arr_0 [i_0] [i_0])))) ? (((15243217022408116267 ? (arr_0 [i_0] [i_0]) : 134))) : (max(var_13, (arr_1 [i_0]))))), ((max(95, (max((arr_0 [i_0] [i_0]), var_15)))))));
        arr_3 [i_0] = var_19;
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_21 = (max(var_21, var_11));
        arr_8 [i_1] [i_1] &= ((min((min(0, 481949302)), (arr_5 [i_1]))));
    }
    var_22 *= -var_0;
    #pragma endscop
}
