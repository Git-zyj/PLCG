/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 31106
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=31106 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/31106
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 += ((-(((!var_5) ? var_2 : ((0 ? var_10 : 1))))));
    var_17 = max(var_0, var_1);
    var_18 &= (min((1 & var_5), ((((var_2 ? var_9 : 1)) / var_12))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_19 = var_2;
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_20 = (((((1 && (((var_11 ? (arr_0 [i_2] [i_1]) : var_5)))))) & ((max(var_1, 9239)))));
                                var_21 *= (arr_11 [i_0] [i_1 - 1] [9] [i_1 - 1] [i_4]);
                                var_22 = (min(var_22, (arr_5 [3] [i_1 - 1] [i_1])));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
