/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211759
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211759 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211759
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 |= (max((var_4 < var_13), ((((-1126035497836894696 ? var_13 : var_8)) << var_4))));
    var_18 = -var_1;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1] = (min((min(var_5, (arr_1 [i_0] [i_0]))), ((~(arr_1 [i_2] [i_1])))));
                    var_19 = (((arr_2 [i_0] [1]) ? 9221120237041090560 : (arr_2 [i_0] [i_1])));
                }
            }
        }
    }
    var_20 = var_4;
    var_21 = ((var_3 ^ ((var_13 ? (var_14 > var_5) : var_10))));
    #pragma endscop
}
