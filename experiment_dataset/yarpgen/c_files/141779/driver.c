#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

int var_1 = 1794145865;
unsigned int var_3 = 1444200163U;
unsigned int var_4 = 2088710833U;
int var_5 = 1778778747;
signed char var_8 = (signed char)37;
int var_9 = -469975469;
int zero = 0;
unsigned long long int var_13 = 15497997776752683055ULL;
unsigned char var_14 = (unsigned char)17;
unsigned int var_15 = 1478227882U;
signed char var_16 = (signed char)-47;
signed char var_17 = (signed char)17;
int var_18 = 1803175490;
long long int arr_2 [25] ;
signed char arr_3 [25] [25] ;
signed char arr_6 [25] [25] [25] [25] ;
void init() {
    for (size_t i_0 = 0; i_0 < 25; ++i_0) 
        arr_2 [i_0] = -4587313410727255692LL;
    for (size_t i_0 = 0; i_0 < 25; ++i_0) 
        for (size_t i_1 = 0; i_1 < 25; ++i_1) 
            arr_3 [i_0] [i_1] = (signed char)81;
    for (size_t i_0 = 0; i_0 < 25; ++i_0) 
        for (size_t i_1 = 0; i_1 < 25; ++i_1) 
            for (size_t i_2 = 0; i_2 < 25; ++i_2) 
                for (size_t i_3 = 0; i_3 < 25; ++i_3) 
                    arr_6 [i_0] [i_1] [i_2] [i_3] = (signed char)-10;
}

void checksum() {
    hash(&seed, var_13);
    hash(&seed, var_14);
    hash(&seed, var_15);
    hash(&seed, var_16);
    hash(&seed, var_17);
    hash(&seed, var_18);
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
