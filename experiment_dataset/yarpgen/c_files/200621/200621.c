/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200621
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200621 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200621
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((((((min(var_16, var_6))) % var_18))) ? var_15 : var_0));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_20 = (((arr_0 [i_0 - 1] [i_1]) != ((((!var_2) || ((max(var_17, 0))))))));
                var_21 = ((((((arr_6 [i_0 - 1] [i_0 - 2] [i_0]) * var_18))) ? (var_3 / 1832062665) : 5126901720470228333));
            }
        }
    }
    var_22 = var_8;
    var_23 = ((((1 ? var_10 : var_16)) < (((~((min(97, 1))))))));
    #pragma endscop
}
