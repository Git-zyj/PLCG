/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182270
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182270 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182270
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = (+(((/* implicit */int) var_5)));
    var_17 = min((var_0), (max((((/* implicit */int) (!(((/* implicit */_Bool) var_1))))), (var_0))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (short i_0 = 0; i_0 < 14; i_0 += 1) 
    {
        var_18 = ((/* implicit */long long int) var_5);
        var_19 = var_7;
    }
    var_20 = ((/* implicit */int) ((((/* implicit */_Bool) min((var_15), (((/* implicit */int) var_2))))) && ((!(((/* implicit */_Bool) (~(((/* implicit */int) var_10)))))))));
    var_21 |= var_1;
}
