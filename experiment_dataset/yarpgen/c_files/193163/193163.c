/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 193163
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=193163 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/193163
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 += var_6;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_8 [i_0 - 1] [i_1] = (min((~var_3), (((-9223372036854775807 - 1) ? -1 : (((~(arr_4 [i_1]))))))));

                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        var_12 = ((var_5 ? ((((arr_5 [i_1] [i_1 + 3] [i_0 - 1]) < (arr_5 [i_1] [i_1 + 1] [i_0 - 1])))) : ((-(arr_5 [i_1] [i_1 + 3] [i_0 - 1])))));
                        var_13 = (~3214955314125423201);
                        arr_11 [i_1] [i_1] = var_7;
                    }
                }
            }
        }
    }
    #pragma endscop
}
