/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49704
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49704 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49704
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_10;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_7 [i_0] [i_0] [i_0] = ((var_11 ? ((((var_15 ^ (arr_2 [i_0] [i_0]))) * ((((!(arr_0 [i_0]))))))) : ((((arr_3 [i_0]) / (arr_3 [i_0]))))));
                var_20 &= (arr_3 [1]);
                var_21 = (-9050 / 4294967295);
            }
        }
    }
    var_22 = var_11;
    /* LoopNest 3 */
    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 1;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 24;i_4 += 1)
            {
                {
                    var_23 = (~28887);
                    arr_15 [i_2] [i_3] [i_4] [i_4] = (arr_9 [i_2]);
                    arr_16 [i_4] [i_3] [i_3] [i_3] = var_16;
                }
            }
        }
    }
    #pragma endscop
}
