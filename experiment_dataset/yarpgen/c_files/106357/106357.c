/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106357
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106357 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106357
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((-(19993 + -709696268)));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_11 = (min(var_11, -709696256));
        arr_2 [i_0] [5] = ((((-(arr_1 [i_0] [i_0]))) << (((((arr_1 [11] [1]) + 6846547091524503196)) - 10))));
        arr_3 [i_0] [i_0] &= (((((((arr_1 [i_0] [i_0]) < var_6)))) < 3889963040));
    }
    var_12 = (min(var_12, ((min((~-709696252), 1)))));
    #pragma endscop
}
