/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84963
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84963 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84963
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((var_6 ? var_8 : ((min(12359, 12381))))) >= var_3));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_12 -= var_3;
                    var_13 = (min(((((((arr_3 [1] [i_1] [2]) <= var_5)) ? (arr_6 [i_0] [i_0] [i_0]) : (arr_6 [2] [i_1] [i_2])))), ((((arr_1 [i_1] [i_1]) / (arr_5 [i_2] [i_1] [i_2] [1]))))));
                }
            }
        }
    }
    var_14 &= (max(((min((var_9 == var_5), (min(12349, 1))))), (min(var_8, ((max(var_9, var_5)))))));
    #pragma endscop
}
