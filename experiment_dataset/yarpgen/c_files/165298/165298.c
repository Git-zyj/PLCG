/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165298
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165298 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165298
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    arr_10 [i_2] [i_1] [i_1] [i_1] = (!var_8);
                    arr_11 [i_0] [i_2] [i_2] [i_2] = var_11;
                    arr_12 [i_2] [i_1] [i_2] = var_14;
                    arr_13 [i_2] [i_1] = -var_0;
                }
            }
        }
        arr_14 [i_0] [i_0] = (arr_0 [i_0 - 1]);
        var_16 += (arr_0 [i_0 - 1]);
    }
    for (int i_3 = 0; i_3 < 20;i_3 += 1)
    {
        var_17 = (max(var_17, (arr_15 [i_3] [i_3])));
        var_18 ^= ((+(((arr_18 [i_3]) | (arr_18 [i_3])))));
        var_19 = (min(var_19, (arr_18 [i_3])));
        /* LoopNest 3 */
        for (int i_4 = 3; i_4 < 19;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 20;i_5 += 1)
            {
                for (int i_6 = 0; i_6 < 1;i_6 += 1)
                {
                    {
                        var_20 = (arr_26 [1] [i_4] [i_4] [i_6]);
                        var_21 ^= 184;
                    }
                }
            }
        }
        var_22 = var_2;
    }
    var_23 = (var_9 & 5482);
    #pragma endscop
}
