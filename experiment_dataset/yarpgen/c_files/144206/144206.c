/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144206
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144206 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144206
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 ^= var_3;

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        arr_4 [i_0] = ((-(arr_2 [i_0] [i_0])));
        var_13 = arr_0 [i_0] [i_0];
    }
    for (int i_1 = 1; i_1 < 9;i_1 += 1)
    {
        arr_7 [i_1] [i_1] = ((((max((arr_2 [i_1 - 1] [i_1 + 1]), (arr_2 [i_1 - 1] [i_1 - 1])))) ? var_8 : (arr_2 [i_1 - 1] [i_1 - 1])));
        arr_8 [i_1] [3] = (max(333957615, 1071644672));
        arr_9 [i_1] |= var_1;
    }

    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        arr_13 [i_2] [i_2] &= (1 * 7);
        var_14 = var_0;
    }
    for (int i_3 = 0; i_3 < 25;i_3 += 1)
    {
        var_15 &= var_4;
        var_16 -= ((((max((max(4294967289, (arr_15 [i_3] [i_3]))), (arr_14 [i_3])))) ? (min(var_0, (var_10 - 0))) : ((var_11 ? 116 : 433904087))));
    }
    #pragma endscop
}
