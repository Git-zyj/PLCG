/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 31632
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=31632 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/31632
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = (arr_1 [i_0]);
        arr_3 [i_0] [i_0] = ((arr_1 [i_0]) > var_1);
        var_12 = 1;
        var_13 = (max(var_13, (((var_9 ^ ((((arr_0 [i_0]) != (arr_0 [i_0])))))))));
        var_14 = ((536870911 ? (((var_5 && (arr_0 [14])))) : (arr_0 [i_0])));
    }
    for (int i_1 = 1; i_1 < 19;i_1 += 1)
    {
        var_15 = (+-26);
        var_16 = ((+(((~536870914) & (arr_0 [i_1 + 1])))));
        var_17 = max((max(((max(var_8, (arr_4 [i_1])))), ((-(arr_1 [i_1 - 1]))))), ((((arr_4 [i_1 + 1]) >= var_7))));
    }
    /* vectorizable */
    for (int i_2 = 1; i_2 < 20;i_2 += 1)
    {
        var_18 = (-1 || 536870909);
        var_19 = (1 < 64);
    }
    for (int i_3 = 0; i_3 < 23;i_3 += 1)
    {
        arr_11 [19] = 1;
        var_20 = ((536870911 >= (-127 - 1)));
    }
    var_21 = (max(var_21, var_7));
    var_22 += var_3;
    var_23 = 1;
    #pragma endscop
}
