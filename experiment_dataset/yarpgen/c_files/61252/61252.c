/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61252
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61252 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61252
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_9;
    var_12 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_8 [i_1] = ((var_7 ? ((var_7 / (((arr_4 [i_0] [i_1]) | var_7)))) : (~var_8)));
                arr_9 [i_0] = ((-(((-1196951567 + 9223372036854775807) >> (var_10 - 182)))));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 14;i_4 += 1)
                        {
                            {
                                arr_16 [i_0] [i_1] [i_2] [i_3] [i_0] = (((arr_2 [i_0]) != (((((((var_3 + 2147483647) << (var_9 - 3764521)))) || var_5)))));
                                var_13 *= var_1;
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
