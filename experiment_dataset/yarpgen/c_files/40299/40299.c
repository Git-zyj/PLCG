/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40299
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40299 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40299
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 += var_3;
    var_16 = (max(var_6, (max(var_13, var_5))));
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        arr_8 [i_0] [6] [i_0] [i_3] = (max(((max((~var_10), var_3))), var_4));
                        var_17 = ((!((min((min(var_2, var_2)), var_10)))));
                    }
                    for (int i_4 = 0; i_4 < 19;i_4 += 1)
                    {
                        var_18 = (max(var_18, (((-(~var_3))))));
                        arr_13 [i_0] = ((-(!var_11)));
                    }
                    arr_14 [i_0] [12] [i_0] = (min(var_6, var_13));
                    arr_15 [i_0] = (max((min(var_11, var_2)), (!var_4)));
                    arr_16 [i_2] [8] [i_0 - 2] = (max(-var_6, (max(var_0, var_14))));
                }
            }
        }
    }
    var_19 = var_11;
    #pragma endscop
}
