/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191022
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191022 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191022
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max(var_13, (!13)));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_14 = (min(var_14, var_2));
                arr_5 [i_0] [i_0] [i_0] = ((4968178735957387695 == (((-(!var_1))))));
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        {
                            arr_13 [i_0] [i_0] [i_1] [i_0] [i_0] = min((min(var_0, var_8)), ((+(((arr_4 [i_0] [i_0 + 1]) ? var_2 : (-9223372036854775807 - 1))))));
                            var_15 = var_10;
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_4 = 2; i_4 < 16;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 17;i_5 += 1)
                    {
                        {
                            arr_20 [i_0] = ((-(((!-4968178735957387696) + (arr_19 [i_0 - 2] [i_4 - 2] [i_4])))));
                            var_16 = 32611;
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
