/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 137982
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=137982 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/137982
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_16 &= (((~122) ? (arr_3 [i_1]) : (max(var_13, (arr_3 [i_0])))));

                /* vectorizable */
                for (int i_2 = 1; i_2 < 23;i_2 += 1)
                {
                    arr_7 [i_0] [i_0] [6] [i_2] = ((((((arr_0 [i_0]) ? var_0 : 4322997016625492471))) ? var_1 : var_11));
                    var_17 = (4322997016625492477 | 1);
                    var_18 = (((arr_5 [i_2 + 1] [i_2 + 1] [i_2 - 1] [i_1]) ? ((var_8 ? var_10 : var_11)) : (((4070 ? var_2 : var_9)))));
                }
            }
        }
    }
    var_19 = var_0;
    #pragma endscop
}
