/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172592
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172592 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172592
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(var_14, (((-((-(max(-1, -7)))))))));
    var_15 = (min(-1, var_12));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_0] = ((1 ? 11839 : 65535));
                    var_16 = 0;
                    var_17 = (max(var_17, ((min(-7470564765332686633, 5178320419918518892)))));
                    arr_7 [i_0] [i_1] = (arr_5 [i_0 + 1]);
                }
            }
        }
    }
    var_18 = -23927;
    var_19 = ((!((max(var_9, var_2)))));
    #pragma endscop
}
