/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190711
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190711 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190711
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((var_6 ? var_5 : (((((var_0 ? var_6 : var_6))) ? (((min(var_10, var_3)))) : var_8))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 11;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 9;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            {
                                var_12 = (max(var_12, (((max((arr_7 [i_2 + 1] [i_3] [i_2 + 1] [i_3 - 1]), 67))))));
                                var_13 += (arr_3 [i_0]);
                                var_14 = var_10;
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 12;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 1;i_6 += 1)
                        {
                            {
                                var_15 *= (arr_7 [i_0] [i_5] [i_0] [i_0]);
                                var_16 = (((((max((arr_15 [i_0 + 1] [i_1] [i_2] [i_5] [6]), (arr_4 [i_0 - 1] [i_0])))) ^ (((arr_0 [i_0] [i_0]) * var_7)))));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
