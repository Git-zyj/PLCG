/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150005
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150005 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150005
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_5;
    var_14 = (min(var_14, ((max(var_10, var_2)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    var_15 = ((var_11 > (~-29227)));
                    arr_7 [i_0] [i_0] = (max((arr_4 [i_0] [i_1 + 1] [i_2] [i_1 + 1]), (((((arr_5 [i_2] [i_1] [i_0]) <= var_10)) ? (var_2 <= var_12) : (arr_3 [i_1 + 1])))));
                    var_16 = 4416763628840263733;
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_3 = 1; i_3 < 1;i_3 += 1)
    {
        for (int i_4 = 3; i_4 < 23;i_4 += 1)
        {
            for (int i_5 = 3; i_5 < 23;i_5 += 1)
            {
                {
                    arr_16 [i_4] [i_4] = var_12;
                    arr_17 [i_3] [i_3] [i_4] = ((-127 && (111 - var_2)));
                }
            }
        }
    }
    var_17 = var_8;
    #pragma endscop
}
