/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226350
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226350 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226350
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            {
                var_16 = ((73 ? (((~(arr_0 [i_1 + 2])))) : (-73 | 4294967295)));
                var_17 = (((~(((!(arr_1 [1] [i_1])))))));
            }
        }
    }
    var_18 = (min((65535 >> 3), var_1));
    /* LoopNest 2 */
    for (int i_2 = 4; i_2 < 15;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 19;i_3 += 1)
        {
            {
                var_19 = ((!((((max(9567260876350471865, (arr_7 [i_2])))) && var_4))));

                for (int i_4 = 0; i_4 < 1;i_4 += 1)
                {
                    arr_12 [i_4] [i_2] [i_4] = (((((arr_8 [i_2] [i_2 + 2] [i_2 + 2]) ? (arr_8 [i_2] [i_2 + 3] [i_2 - 4]) : var_9))) ? var_1 : var_11);
                    arr_13 [i_2] [i_2] [i_4] = (arr_10 [i_2] [i_2]);
                }
            }
        }
    }
    var_20 |= (!4294967295);
    var_21 = var_10;
    #pragma endscop
}
