/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162520
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162520 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162520
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (((min(var_7, ((var_10 ? var_6 : var_4)))) ^ ((((!(~var_5)))))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 23;i_2 += 1)
            {
                {
                    var_13 = (arr_8 [23]);

                    for (int i_3 = 1; i_3 < 21;i_3 += 1)
                    {
                        var_14 = ((-(((205 ? (arr_7 [i_3] [i_2] [i_1] [23]) : var_10)))));
                        var_15 = (max((((min(4480, var_6)) + (61055 & 4480))), (!4480)));
                        var_16 &= ((((min(((max(19345, 61055))), (var_6 % var_3)))) || (((+(min((arr_7 [i_2] [i_1 + 1] [i_1 + 1] [i_3]), (arr_10 [i_3]))))))));
                        var_17 -= (arr_4 [i_0] [i_0]);
                        var_18 = (+-var_4);
                    }
                    var_19 = (max(var_19, var_4));
                    var_20 = (1767 % 70142937);
                    var_21 = var_3;
                }
            }
        }
    }
    #pragma endscop
}
