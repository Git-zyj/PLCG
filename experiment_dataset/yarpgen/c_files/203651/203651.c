/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203651
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203651 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203651
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 11;i_2 += 1)
            {
                {
                    var_16 = (min(-4624889636274093403, 48139));
                    var_17 = var_10;
                }
            }
        }
    }

    for (int i_3 = 2; i_3 < 21;i_3 += 1)
    {
        var_18 -= var_1;
        arr_10 [i_3] = (arr_8 [i_3]);
        var_19 = (min((((((-(arr_9 [i_3] [i_3 - 1])))) || var_9)), (((arr_9 [i_3 + 2] [i_3 + 2]) >= (arr_9 [i_3] [i_3 - 2])))));
    }
    for (int i_4 = 0; i_4 < 18;i_4 += 1)
    {
        arr_14 [i_4] = (((((~var_3) ? var_15 : ((max((arr_8 [i_4]), var_1))))) > var_4));
        var_20 = (max(var_20, (((((-(1 / var_14))) & ((((17397 ? -4623 : var_6)))))))));
        arr_15 [i_4] [i_4] = var_2;
    }
    #pragma endscop
}
