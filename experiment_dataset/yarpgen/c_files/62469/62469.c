/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62469
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62469 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62469
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 ^= var_8;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_13 = (min(var_13, ((max((((arr_3 [9] [i_1]) / (min(0, (arr_1 [i_0] [i_1]))))), (arr_3 [i_0] [i_1]))))));
                var_14 += (arr_3 [i_0] [i_1]);
                var_15 = (arr_3 [i_0] [i_0]);
                var_16 = (min(((((!-4271383426075042804) || var_6))), (max(4372410343771898906, ((((arr_3 [i_0] [i_1]) || 7060337716998413407)))))));
            }
        }
    }
    var_17 |= var_11;
    #pragma endscop
}
