/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88266
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88266 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88266
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_11, ((((((var_6 & (min(var_9, var_6))))) ? (((var_3 / (0 + 4294967295)))) : -var_5)))));

    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        arr_2 [i_0] = (min((((arr_1 [i_0] [i_0]) & (arr_1 [i_0] [i_0]))), (min((arr_1 [0] [i_0]), (arr_1 [i_0] [i_0])))));
        arr_3 [i_0] = (arr_0 [i_0]);
    }
    for (int i_1 = 2; i_1 < 18;i_1 += 1)
    {
        var_12 += (((arr_4 [i_1 + 2]) >> ((((arr_4 [i_1 + 2]) > (arr_4 [i_1 - 2]))))));
        arr_6 [i_1] = (~0);
    }
    #pragma endscop
}
