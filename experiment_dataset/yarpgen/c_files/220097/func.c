/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220097
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220097 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220097
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */unsigned int) ((long long int) var_3));
    var_14 = ((/* implicit */int) var_4);
    /* LoopSeq 1 */
    for (short i_0 = 3; i_0 < 14; i_0 += 2) 
    {
        var_15 &= ((/* implicit */unsigned int) ((((/* implicit */unsigned long long int) 4294967295U)) - (arr_0 [i_0 + 2])));
        var_16 += ((/* implicit */short) var_0);
    }
    var_17 = ((short) ((((/* implicit */_Bool) var_3)) ? (((/* implicit */int) var_10)) : (((/* implicit */int) var_2))));
    var_18 -= ((/* implicit */int) var_2);
}
