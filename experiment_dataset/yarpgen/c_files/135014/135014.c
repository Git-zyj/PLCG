/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135014
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135014 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135014
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    var_18 ^= ((-2619750200008607286 ? 4 : ((2199522382 & (arr_7 [i_1] [i_1] [i_0 + 1] [i_2 - 1])))));
                    var_19 = (((((((arr_4 [i_0 + 1] [i_2] [i_0]) > ((-2619750200008607278 ? (arr_1 [i_1]) : (arr_3 [i_0]))))))) % (arr_0 [i_2 - 1])));
                    var_20 = (max(var_20, 383369592));
                }
            }
        }
    }
    var_21 = (max(var_21, var_10));
    #pragma endscop
}
