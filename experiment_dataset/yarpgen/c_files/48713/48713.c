/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48713
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48713 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48713
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_0] [6] = (max(var_11, (arr_3 [i_0] [i_1])));
                var_17 = (min(var_17, (min(((((arr_0 [i_0 + 1] [i_1 + 1]) ? (arr_0 [i_0] [i_1 + 1]) : 10820))), ((var_1 / (arr_2 [i_1] [i_1] [i_0 - 3])))))));
            }
        }
    }
    var_18 = (max(var_18, var_8));
    var_19 = (((((-(min(var_9, var_14))))) ? (min(((60618 ? -761966952 : 1152813226709672291)), var_0)) : ((min(((min(var_13, var_5))), (min(var_7, 60618)))))));
    #pragma endscop
}
