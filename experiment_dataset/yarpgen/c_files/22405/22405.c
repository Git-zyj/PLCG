/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22405
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22405 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22405
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 3; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 17;i_4 += 1)
                        {
                            {
                                arr_14 [i_1] [i_4] [i_2] [i_3] [i_4] [i_3] [5] = -1955074746;
                                arr_15 [1] [i_2] [i_2] [i_2] [i_2] [i_2] [16] = 101;
                                arr_16 [i_1] = var_5;
                            }
                        }
                    }
                }
                arr_17 [17] [i_1] [i_0] = (max((max(var_2, ((((arr_13 [i_0] [i_0] [i_1] [i_1] [i_1]) < 3))))), ((min(1, 306932186)))));
            }
        }
    }
    var_15 = var_5;
    #pragma endscop
}
