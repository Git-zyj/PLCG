/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236249
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236249 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236249
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((!(((var_14 ? var_18 : 4294967295))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            {
                var_20 += ((((((3511233139 > (arr_2 [i_0] [i_1 - 2]))))) > (((134 >> (-1183064960 + 1183064985))))));
                arr_4 [10] [i_1] |= var_13;
                var_21 &= ((~(((arr_2 [i_1 - 3] [i_1]) & var_0))));
            }
        }
    }
    var_22 = (((((max(var_13, -3290756290574427038))) ? var_13 : -9223372036854775805)));
    #pragma endscop
}
