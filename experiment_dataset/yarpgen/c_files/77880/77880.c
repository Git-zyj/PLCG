/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77880
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77880 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77880
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_10;
    var_18 = ((((max(((var_10 ? var_0 : 28040)), var_11))) ? var_4 : var_0));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] &= var_8;
                arr_7 [i_0] = (arr_4 [i_0] [i_0] [10]);
                var_19 -= (((((~(((arr_4 [i_0] [i_0] [i_1]) / 3789004978))))) ? (arr_5 [i_0] [i_0]) : ((var_13 - ((max(63, var_7)))))));
                var_20 = (arr_1 [i_0]);
                arr_8 [i_0] [i_0] [i_0] = var_15;
            }
        }
    }
    var_21 = ((var_10 ? -1 : 741366432));
    #pragma endscop
}
