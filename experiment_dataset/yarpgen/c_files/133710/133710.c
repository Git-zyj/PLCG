/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133710
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133710 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133710
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 19;i_0 += 1) /* same iter space */
    {
        var_14 = (max(var_14, (((!490245670) ? (arr_1 [i_0] [i_0]) : (~var_11)))));
        arr_2 [i_0] = (max((!-490245670), -1659578312));
    }
    for (int i_1 = 0; i_1 < 19;i_1 += 1) /* same iter space */
    {
        var_15 = (min(-1471, -1457));
        arr_5 [i_1] [i_1] = ((((min(var_5, (arr_0 [i_1])))) ? -1126064986 : -1457));
        var_16 &= ((~(((!(arr_0 [i_1]))))));
        arr_6 [i_1] = (((((3608633926 ? (arr_3 [i_1] [i_1]) : (arr_3 [i_1] [i_1])))) ? (var_13 && -1435) : 1436));
    }
    var_17 = (min(0, var_6));
    #pragma endscop
}
