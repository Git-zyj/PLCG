/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166448
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166448 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166448
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 *= (min(32236, ((1 ? 1 : ((min(25962, -7)))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_13 = ((44 ? -69 : 2));
        var_14 = -28;

        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            var_15 = (var_6 ? (arr_2 [i_1] [i_1] [i_0]) : ((var_3 ? 10452631615320071263 : (arr_4 [i_1] [i_1] [i_0]))));
            var_16 *= (((!619478432) ? (arr_0 [i_1] [i_0]) : (arr_1 [i_1])));
            var_17 = (min(var_17, var_11));
            var_18 *= ((64 ? 1 : 1));
        }
        for (int i_2 = 1; i_2 < 12;i_2 += 1) /* same iter space */
        {
            var_19 -= 3459628252;
            var_20 = ((3675488855 > (arr_5 [i_0] [i_0])));
            var_21 *= (((arr_6 [i_2 - 1]) ? (arr_1 [i_2 + 3]) : (arr_5 [i_2 + 4] [i_2 - 1])));
            arr_7 [i_0] [i_0] [i_2] = 354966236;
        }
        for (int i_3 = 1; i_3 < 12;i_3 += 1) /* same iter space */
        {
            var_22 = 6995201891962409799;
            var_23 = (arr_0 [i_0] [i_0]);
        }
        arr_10 [i_0] = -830429303;
    }
    var_24 = 2138560836396205175;
    var_25 = (((((var_2 ? var_5 : var_1))) | (!var_1)));
    #pragma endscop
}
