/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 153853
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=153853 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/153853
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_19 = (0 <= 1);
                var_20 *= ((((((arr_3 [i_0 + 4] [i_0 - 2]) ? (arr_5 [i_0 - 1] [i_0 + 1]) : -16384))) ? (27 / 1127147066) : ((((((arr_0 [i_0] [i_1]) ? 16383 : (arr_3 [i_0] [i_1])))) ? 45 : 13222954896936390333))));
            }
        }
    }
    var_21 = ((((var_13 <= (var_18 | var_10))) ? var_1 : (var_7 + var_1)));
    var_22 = var_11;
    #pragma endscop
}
