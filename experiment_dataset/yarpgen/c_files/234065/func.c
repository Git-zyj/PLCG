/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234065
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234065 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234065
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned int i_0 = 1; i_0 < 20; i_0 += 3) 
    {
        for (unsigned int i_1 = 0; i_1 < 21; i_1 += 3) 
        {
            {
                var_18 = ((/* implicit */signed char) (((+(var_17))) < ((-(var_10)))));
                var_19 += ((/* implicit */signed char) ((int) (!(((var_17) < (var_17))))));
                arr_6 [(signed char)8] &= ((/* implicit */int) (((-(var_15))) >> (((((/* implicit */_Bool) ((unsigned int) var_17))) ? (((/* implicit */int) var_14)) : (((int) var_16))))));
                arr_7 [i_0] = ((/* implicit */signed char) (~((-(var_10)))));
            }
        } 
    } 
    var_20 = ((/* implicit */unsigned int) (~(((/* implicit */int) var_6))));
    var_21 &= (((~(((/* implicit */int) ((signed char) var_16))))) / (var_7));
    var_22 &= ((/* implicit */int) (((!(((/* implicit */_Bool) var_11)))) ? (var_13) : (((/* implicit */unsigned int) var_11))));
}
