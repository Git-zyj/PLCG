/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106429
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106429 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106429
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (short i_0 = 4; i_0 < 8; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 9; i_1 += 1) 
        {
            {
                var_12 = arr_2 [i_0 + 3];
                /* LoopNest 3 */
                for (unsigned char i_2 = 0; i_2 < 11; i_2 += 1) 
                {
                    for (int i_3 = 2; i_3 < 8; i_3 += 3) 
                    {
                        for (unsigned long long int i_4 = 1; i_4 < 8; i_4 += 3) 
                        {
                            {
                                var_13 &= ((/* implicit */unsigned short) -1329550137);
                                var_14 *= ((/* implicit */short) arr_11 [i_0 - 2] [i_0 - 2] [i_2]);
                                arr_15 [i_3] [i_3] [i_1] [i_3] [2U] [i_1] = ((/* implicit */signed char) max((arr_7 [i_1] [i_1 - 1] [i_1] [i_3]), (((/* implicit */unsigned short) arr_2 [i_1]))));
                            }
                        } 
                    } 
                } 
            }
        } 
    } 
    var_15 |= ((/* implicit */unsigned short) var_4);
}
