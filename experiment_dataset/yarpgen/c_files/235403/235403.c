/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235403
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235403 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235403
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max((((-8540237324097705197 & 17603) ? (min(var_14, var_5)) : 1)), var_14));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_1] = (arr_4 [i_0 + 1] [i_0 + 1] [i_0]);
                var_20 *= ((min(var_4, var_17)));
                var_21 += (max(var_18, (arr_1 [i_0 + 1])));
            }
        }
    }
    var_22 = var_11;
    #pragma endscop
}
