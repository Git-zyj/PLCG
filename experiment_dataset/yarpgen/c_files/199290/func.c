/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199290
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199290 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199290
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */signed char) var_8);
    /* LoopSeq 1 */
    /* vectorizable */
    for (long long int i_0 = 0; i_0 < 25; i_0 += 2) 
    {
        /* LoopSeq 2 */
        for (long long int i_1 = 0; i_1 < 25; i_1 += 2) 
        {
            var_17 = (-(arr_0 [i_1]));
            var_18 = ((unsigned short) (+(var_14)));
        }
        for (unsigned char i_2 = 0; i_2 < 25; i_2 += 2) 
        {
            var_19 = ((/* implicit */_Bool) (~(((/* implicit */int) (!(((/* implicit */_Bool) var_2)))))));
            var_20 = ((/* implicit */signed char) max((var_20), (((/* implicit */signed char) var_2))));
            var_21 = ((/* implicit */short) ((((/* implicit */_Bool) 2367560635U)) ? (((/* implicit */int) (unsigned char)151)) : (((/* implicit */int) (_Bool)1))));
        }
        var_22 = ((/* implicit */long long int) (!(((/* implicit */_Bool) ((var_12) >> (((var_3) - (133058580U))))))));
        arr_6 [i_0] [i_0] = ((/* implicit */signed char) ((unsigned int) arr_0 [i_0]));
    }
    var_23 = ((/* implicit */signed char) var_3);
    var_24 = ((/* implicit */unsigned short) (~(125829120)));
}
