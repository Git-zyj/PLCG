/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45824
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45824 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45824
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 += (2083130103 <= (((min(var_0, var_4)))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                arr_4 [24] &= 532782061;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        {
                            arr_11 [i_0] = ((((((min(2083130103, 2083130101))) - (arr_8 [i_0] [i_1] [i_1] [i_1]))) / ((var_7 ? var_14 : (((arr_5 [i_0]) - (arr_1 [i_0])))))));
                            arr_12 [i_0 - 2] [i_0] [i_3] [i_3] = 532782072;
                        }
                    }
                }
                var_20 ^= (arr_10 [i_0] [i_1] [i_1] [22]);
                arr_13 [i_0] [i_0] [i_0] = var_17;
            }
        }
    }
    var_21 = var_0;
    var_22 &= var_15;
    var_23 += var_13;
    #pragma endscop
}
