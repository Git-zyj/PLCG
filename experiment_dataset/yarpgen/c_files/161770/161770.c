/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 161770
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=161770 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/161770
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 19;i_4 += 1)
                        {
                            {
                                var_14 = (69 && 100);
                                var_15 = ((((min((arr_13 [i_0] [i_2] [i_4]), (arr_5 [i_3])))) << (var_3 - 44)));
                                var_16 = ((((~(arr_16 [i_2] [i_3] [i_2] [i_1] [i_2]))) % (arr_7 [i_3])));
                                var_17 = (min(var_17, (((((min((arr_1 [i_0]), (arr_12 [i_0] [i_1] [i_2])))) & (arr_12 [i_2] [i_3] [i_4]))))));
                                var_18 = min(var_13, ((min(var_3, (arr_2 [i_4])))));
                            }
                        }
                    }
                    var_19 += var_1;
                }
            }
        }
    }
    var_20 = -var_11;
    #pragma endscop
}
