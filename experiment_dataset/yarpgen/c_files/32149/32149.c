/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32149
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32149 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32149
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((((-var_15 ? var_9 : (~18446744073709551608))) - var_0));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    var_20 -= ((((18446744073709551615 >> (5493 - 5457)))) ? (((~18446744073709551608) ? (!var_17) : var_8)) : ((((!(~11616))))));
                    arr_7 [i_0] [i_1] [i_2] = (((!var_13) + (((!18446744073709551608) ? -3 : (var_2 / 9223372036854775807)))));
                }
            }
        }
    }
    #pragma endscop
}
