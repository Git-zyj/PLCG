/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244345
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244345 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244345
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (((min((!19), var_2)) >> (((((((143 ? var_1 : var_6))) ? ((var_0 ? var_3 : var_12)) : (!var_3))) - 17357))));
    var_14 = var_4;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    var_15 ^= (max(((-((max(11, (arr_3 [i_0] [i_0] [i_2])))))), (arr_5 [i_1 - 2] [i_1 + 2] [i_2])));
                    var_16 = ((((~(arr_1 [i_1 - 1]))) <= (((arr_3 [i_1 - 1] [i_1 - 2] [i_1]) | (arr_5 [i_1 - 1] [i_1 + 2] [i_1])))));
                }
            }
        }
    }
    var_17 = ((!((((var_4 + 2147483647) >> (var_0 - 22183))))));
    #pragma endscop
}
