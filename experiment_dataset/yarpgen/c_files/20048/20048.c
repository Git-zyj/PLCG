/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20048
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20048 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20048
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_15 = (((arr_6 [i_2]) ? (((-1019715222214793382 ? 1668991246 : var_8))) : var_5));
                    arr_9 [i_0] [i_0] [i_1] [i_2] = (!2625976049);
                    var_16 = 6598812428231903693;
                }
            }
        }
    }
    var_17 = (max(var_9, (((min(52290, var_6))))));
    var_18 = (min(var_18, (((min(var_2, (min(var_12, var_4))))))));
    #pragma endscop
}
