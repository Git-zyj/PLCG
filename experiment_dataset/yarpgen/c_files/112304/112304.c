/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112304
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112304 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112304
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_6;
    var_19 = var_12;
    var_20 = (52326 ? 2147483647 : -985033488);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        {
                            arr_13 [i_0] [i_1] [i_2] [i_0] [i_0] [i_0] = ((614451914 | ((max((min((arr_5 [1]), (arr_9 [4] [i_2] [i_2] [i_2] [4]))), ((153 ? 122 : var_0)))))));
                            var_21 = (-(arr_5 [i_0]));
                            var_22 = (arr_11 [i_0] [i_1] [i_0] [i_2]);
                        }
                    }
                }
                var_23 = (max(var_23, (arr_8 [i_1] [i_1] [i_1] [i_0])));
            }
        }
    }
    #pragma endscop
}
