/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11509
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11509 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11509
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_7;

    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        arr_3 [i_0 + 3] [i_0 + 4] |= (((((var_5 ? 1 : (arr_2 [i_0 - 2])))) ? (arr_1 [i_0 + 1] [i_0]) : (((arr_2 [i_0 - 2]) - var_11))));
        var_17 ^= (((arr_1 [i_0 + 1] [i_0 - 1]) ? (max((arr_1 [i_0 + 1] [i_0 + 1]), (arr_2 [i_0 + 3]))) : (arr_2 [i_0 + 2])));
        arr_4 [i_0] [i_0] = ((!(arr_2 [i_0])));
        var_18 = ((!((((((var_1 ? var_3 : (arr_0 [i_0 - 1])))) ? var_3 : (arr_0 [i_0 + 4]))))));
    }
    var_19 = 11983;
    #pragma endscop
}
