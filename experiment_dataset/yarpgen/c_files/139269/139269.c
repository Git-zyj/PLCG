/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 139269
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=139269 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/139269
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_12;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] = (((((49734 ? 49734 : ((15802 ? 0 : 123))))) ? (((max(1, (arr_2 [i_0] [i_1]))))) : -0));
                arr_6 [i_0] [i_0] [i_0] = (max(((min((arr_1 [i_0]), -248))), (((~-32181) ? (((~(arr_4 [1] [i_0])))) : ((-7905519570422233932 ? (arr_3 [i_0]) : (arr_3 [i_0])))))));
                var_19 = ((49734 || (((0 - (~32759))))));
                arr_7 [i_0] = max((123 + -7905519570422233906), 1530776117);
            }
        }
    }
    #pragma endscop
}
