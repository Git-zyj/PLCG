/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235723
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235723 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235723
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (min((min(511, (((-1663281380 ? var_8 : var_18))))), var_6));
    var_21 = min(((2502488425100621421 ? 65535 : var_9)), (((~var_3) ? var_3 : (min(var_9, var_1)))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        arr_2 [i_0] = (var_9 / (((var_12 ? (arr_0 [i_0]) : 9954))));
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 10;i_3 += 1)
                {
                    {
                        var_22 = var_5;
                        arr_10 [i_0] = 65535;
                    }
                }
            }
        }
    }
    #pragma endscop
}
