/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234289
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234289 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234289
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (var_5 || var_16);

    for (int i_0 = 0; i_0 < 1;i_0 += 1) /* same iter space */
    {
        var_18 = (((var_3 > var_9) <= (((((var_16 | var_4) + 9223372036854775807)) << (var_3 - 1804076826)))));
        var_19 |= var_7;
        var_20 = var_15;
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1) /* same iter space */
    {
        var_21 = ((((var_4 >= (var_11 + var_5))) && (var_8 || var_8)));
        var_22 = ((var_5 << (((((var_8 + 2147483647) >> (var_15 + 1141137875))) - 55449256))));
        var_23 = (min(var_23, var_2));
    }
    #pragma endscop
}
