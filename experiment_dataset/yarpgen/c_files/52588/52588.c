/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52588
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52588 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52588
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_16 -= ((max((((var_14 + 2147483647) << (var_9 - 1))), (arr_12 [i_0] [i_1] [i_2] [i_1] [i_4]))));
                                var_17 = var_11;
                            }
                        }
                    }
                    var_18 = (min(var_18, (((((((arr_10 [i_0] [i_0] [0] [i_1] [i_0] [i_2]) ? 249 : ((max(var_2, 236)))))) & ((var_2 ? (arr_4 [i_2] [i_2]) : ((min((arr_1 [3]), 1))))))))));
                }
            }
        }
    }
    var_19 = 65535;
    var_20 = (((var_12 ? var_6 : (var_12 <= var_7))));
    var_21 -= -24636;
    #pragma endscop
}
