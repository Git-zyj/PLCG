/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105480
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105480 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105480
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 *= ((/* implicit */signed char) (unsigned short)14618);
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 16; i_0 += 4) 
    {
        for (short i_1 = 0; i_1 < 16; i_1 += 2) 
        {
            {
                var_19 = ((/* implicit */long long int) (_Bool)1);
                var_20 = ((/* implicit */unsigned int) min((var_20), (((/* implicit */unsigned int) ((unsigned short) (unsigned short)14618)))));
                var_21 = ((/* implicit */short) min((var_21), (((/* implicit */short) ((((/* implicit */int) var_4)) != ((-(((/* implicit */int) (unsigned short)14618)))))))));
            }
        } 
    } 
}
