/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227999
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227999 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227999
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_1] [i_0] |= var_15;
                    var_16 = (min(((max((((arr_0 [i_1] [i_1]) ? var_4 : var_8)), (((var_9 ? var_15 : -1014440307)))))), (~4635620024798372817)));
                    var_17 = (min((((arr_0 [i_0 - 2] [i_0 - 1]) & (arr_0 [i_0 + 3] [i_0 - 2]))), (max(3380727622, -1154431520))));
                }
            }
        }
    }
    var_18 = (min(var_18, 7981344387232461431));
    var_19 = var_1;
    #pragma endscop
}
