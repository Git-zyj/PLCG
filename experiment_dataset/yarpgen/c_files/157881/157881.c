/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157881
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157881 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157881
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_3;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            {
                arr_4 [i_0] = (((var_3 | (arr_2 [i_0 - 1]))));
                arr_5 [i_0] = var_4;
                var_11 = (((var_0 - var_6) ? ((((var_1 ? var_5 : -52)) - (arr_1 [i_1] [i_1]))) : -0));
                var_12 = (((~var_2) ? ((((18446744073709551615 - (arr_3 [i_0] [i_1]))) / (((2082337463 - (arr_2 [i_0])))))) : var_8));
            }
        }
    }
    #pragma endscop
}
