/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234799
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234799 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234799
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = 3841214031;
    var_21 = var_12;
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 18;i_2 += 1)
            {
                {
                    var_22 ^= 8817;
                    var_23 -= (65535 * 453753265);
                    arr_8 [i_0 - 4] [i_1 + 2] [i_2 - 2] [i_0] = (((arr_5 [i_2 - 1] [i_1] [i_1] [i_0]) ? (arr_3 [i_0 - 1]) : ((min((arr_4 [i_0 - 4] [i_1 - 2] [i_2 - 1] [i_2 + 1]), (arr_4 [i_0 - 4] [i_1 + 3] [5] [i_2 - 1]))))));
                }
            }
        }
    }
    var_24 = ((((max(13696507932559969179, 3841214036))) || (((1 ? 1 : 1)))));
    var_25 = var_11;
    #pragma endscop
}
