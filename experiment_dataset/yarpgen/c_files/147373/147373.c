/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 147373
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=147373 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/147373
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_7;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    arr_10 [i_1] [i_2] [i_1] [i_1] = ((!(((-1 ? var_1 : (arr_0 [i_0 + 3]))))));
                    var_16 = (max(var_16, ((min(87, 1847133756372783182)))));
                    var_17 -= ((-((((min(var_2, 666043195))) ? (arr_6 [i_2] [i_0 + 1] [i_0 + 1]) : (2147483633 / 97)))));
                }
            }
        }
    }
    var_18 -= ((((-((var_3 ? var_9 : -97)))) > (((~((-97 ? 1553945321 : var_0)))))));
    #pragma endscop
}
