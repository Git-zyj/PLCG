/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71766
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71766 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71766
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        arr_3 [6] [6] &= (((7 | 1) ? -64 : ((var_10 + (arr_1 [0])))));
        var_11 *= (min(((-8 ? (~var_1) : ((255 ? (arr_1 [4]) : var_0)))), (((4 ? 7 : 26645)))));
        var_12 = (min(var_12, ((((arr_0 [4]) ? var_9 : (arr_1 [2]))))));
    }
    var_13 = var_10;
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 25;i_1 += 1)
    {
        for (int i_2 = 4; i_2 < 22;i_2 += 1)
        {
            {
                arr_9 [i_2] [i_1] [i_1] = ((min((arr_4 [20] [20]), ((((arr_7 [i_1] [i_1] [i_1]) ? -1 : 7))))));
                arr_10 [i_1] [i_2 - 1] [i_1] = arr_8 [i_2] [20] [i_2 - 2];
            }
        }
    }
    var_14 = ((~(((var_8 >= var_7) | var_9))));
    #pragma endscop
}
