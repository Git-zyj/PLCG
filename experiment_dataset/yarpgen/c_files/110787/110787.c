/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110787
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110787 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110787
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (((var_6 ? (var_5 % var_7) : var_2)));
    var_11 -= 7375327733862872616;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_12 = (arr_2 [i_0] [24] [i_1 + 1]);
                arr_5 [i_0 - 1] = -16213;
                var_13 = (max(var_13, ((((((var_3 ? var_4 : 0))) ? ((45346 ? 85 : (arr_4 [i_1 + 1] [i_1]))) : (arr_2 [i_0 + 1] [i_1 + 1] [i_1 + 1]))))));
            }
        }
    }
    #pragma endscop
}
