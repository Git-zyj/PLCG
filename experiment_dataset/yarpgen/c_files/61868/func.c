/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61868
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61868 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61868
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 -= ((/* implicit */unsigned char) var_9);
    var_18 = ((/* implicit */signed char) (+(max((((/* implicit */int) ((short) var_9))), ((+(((/* implicit */int) var_10))))))));
    /* LoopNest 2 */
    for (long long int i_0 = 2; i_0 < 9; i_0 += 4) 
    {
        for (int i_1 = 4; i_1 < 12; i_1 += 1) 
        {
            {
                arr_4 [i_1 - 2] [(short)11] [(short)11] = ((/* implicit */short) ((int) min(((-(((/* implicit */int) var_10)))), (((int) var_0)))));
                var_19 *= ((/* implicit */short) (!(((/* implicit */_Bool) ((((/* implicit */_Bool) var_4)) ? (((/* implicit */int) var_2)) : (((/* implicit */int) var_5)))))));
            }
        } 
    } 
}
