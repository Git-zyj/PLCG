/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66357
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66357 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66357
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 *= ((/* implicit */unsigned short) (!(var_3)));
    /* LoopNest 3 */
    for (unsigned char i_0 = 0; i_0 < 10; i_0 += 4) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 10; i_1 += 1) 
        {
            for (long long int i_2 = 0; i_2 < 10; i_2 += 1) 
            {
                {
                    /* LoopNest 2 */
                    for (unsigned short i_3 = 0; i_3 < 10; i_3 += 1) 
                    {
                        for (unsigned int i_4 = 0; i_4 < 10; i_4 += 4) 
                        {
                            {
                                var_14 = ((/* implicit */_Bool) min((var_14), ((!(((/* implicit */_Bool) (~(8697724967704732544LL))))))));
                                var_15 = ((/* implicit */unsigned short) (~((-(16339539241544281654ULL)))));
                                arr_17 [i_1] [i_1] [i_2] [i_3] [i_4] = ((/* implicit */long long int) (-(11366502968970624326ULL)));
                            }
                        } 
                    } 
                    var_16 = -2444058159405013725LL;
                    var_17 = (!(((/* implicit */_Bool) (+(((/* implicit */int) (unsigned short)23990))))));
                }
            } 
        } 
    } 
}
