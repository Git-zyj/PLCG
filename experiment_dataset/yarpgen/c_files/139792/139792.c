/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 139792
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=139792 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/139792
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((var_5 ? ((((((-8634273450593440233 ? var_13 : var_13))) ? (-8634273450593440233 & var_8) : (((2 ? 146 : var_8)))))) : (max((((3375415012 ? 1 : 2))), ((var_11 ? var_13 : 10762210867243228858))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_16 ^= (arr_1 [i_0] [i_1]);
                var_17 = var_9;
                arr_5 [i_1] = (((arr_1 [i_0] [i_0]) ? (max(((4294967295 ? (arr_3 [7]) : (arr_2 [i_1] [i_0] [i_0]))), ((-(arr_3 [i_0]))))) : (((~(arr_4 [i_0] [i_0]))))));
                arr_6 [i_0] [i_0] = (arr_3 [i_0]);
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 2; i_2 < 11;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 12;i_3 += 1)
        {
            {
                var_18 = (arr_9 [i_2]);
                var_19 = (arr_4 [i_2] [i_2 + 1]);
            }
        }
    }
    #pragma endscop
}
