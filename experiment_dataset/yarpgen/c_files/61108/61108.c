/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61108
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61108 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61108
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            {
                var_17 = (((arr_3 [i_0] [i_0]) ? var_16 : ((min((arr_5 [i_1]), (min(3614541698, 3614541698)))))));
                var_18 = (arr_2 [i_0] [i_0]);
                var_19 = ((min(var_16, var_7)));
            }
        }
    }
    var_20 = (min(var_20, ((min((((!21) ? (((var_2 ? var_9 : var_0))) : ((var_6 ? var_10 : 680425609)))), var_10)))));
    #pragma endscop
}
