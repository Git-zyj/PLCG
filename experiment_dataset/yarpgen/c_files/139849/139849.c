/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 139849
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=139849 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/139849
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        var_16 = (min(var_16, var_5));
        var_17 = var_15;
    }
    var_18 = (min(var_18, (((((((var_14 - var_3) ? (~var_7) : var_4))) || (((var_8 & var_3) || (((var_5 ? var_14 : var_1))))))))));
    var_19 = ((((min(5526131078965984137, (var_2 * var_9)))) ? (((!var_2) ? var_3 : (!var_4))) : ((((var_7 != var_4) - (!var_6))))));
    #pragma endscop
}
