/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226217
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226217 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226217
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] = (max((arr_4 [i_0]), (arr_4 [i_0])));
                var_12 = (min(var_12, (((((((arr_2 [i_1]) | -26920))) || ((min((((!(arr_0 [i_1])))), (arr_2 [i_1])))))))));
                arr_6 [i_0] = ((((((-(arr_3 [i_1] [5] [i_0]))))) ? (min(0, (arr_3 [i_1] [i_1] [i_0]))) : (min((arr_0 [i_0]), ((2008484439 ? (arr_3 [i_0] [i_1] [i_0]) : (arr_4 [i_0])))))));
            }
        }
    }
    var_13 = (~var_1);
    #pragma endscop
}
