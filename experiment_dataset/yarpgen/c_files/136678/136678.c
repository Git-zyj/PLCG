/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136678
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136678 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136678
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 9;i_2 += 1)
            {
                {
                    arr_7 [i_0] = ((((((!((!(arr_0 [1] [9]))))))) > ((((var_12 ? -1 : var_5)) & (((var_2 ? (arr_4 [i_0]) : var_0)))))));
                    arr_8 [i_0] [i_0] = (((((arr_3 [i_2] [i_1] [i_0]) ? var_4 : (min(var_2, var_6)))) & (((((var_1 ? 127 : -2147483640))) | var_11))));
                    var_13 = ((((min(((((arr_0 [0] [i_1]) >= var_11))), var_9))) >> (-118 + 118)));
                }
            }
        }
    }
    var_14 &= var_4;
    var_15 &= (var_1 > var_9);
    var_16 = var_11;
    #pragma endscop
}
