/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169094
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169094 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169094
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = (((arr_0 [i_0 - 1]) <= ((((arr_0 [i_0 - 1]) - (arr_1 [i_0 - 1]))))));
        var_15 = (max(((var_11 >> (!var_5))), var_3));
        arr_3 [i_0] = (min(-116, 10030));
        arr_4 [0] [0] &= ((((var_3 - (arr_1 [i_0])))) - (((min((max(var_14, var_5)), var_12)))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 12;i_1 += 1)
    {
        var_16 = (((arr_6 [i_1]) % (arr_6 [i_1])));
        var_17 = (((arr_5 [i_1]) == (arr_5 [i_1])));
    }
    var_18 = var_8;
    var_19 = (min(var_19, var_12));
    var_20 = var_0;
    #pragma endscop
}
