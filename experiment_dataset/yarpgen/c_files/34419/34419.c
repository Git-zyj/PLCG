/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34419
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34419 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34419
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((((max(30124, ((min(var_8, var_3)))))) && (!var_2)));

    /* vectorizable */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_18 = var_3;
        /* LoopNest 2 */
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_19 += (arr_4 [i_0] [0] [i_1 - 1]);
                    var_20 = (min(var_20, ((((arr_1 [i_0 - 1]) ? (arr_3 [i_0] [i_1 - 1] [i_2]) : (arr_3 [i_0 - 1] [i_1 - 1] [i_0 - 1]))))));
                    var_21 ^= (((arr_1 [i_0 - 1]) & (arr_1 [i_0 - 1])));
                }
            }
        }
    }
    var_22 ^= (!var_15);
    #pragma endscop
}
