/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179133
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179133 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179133
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(var_16, ((max(-502770801, (~1533635996))))));

    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        var_17 = (min((((!(arr_1 [i_0 - 1])))), (arr_1 [i_0 - 1])));

        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            var_18 = (-(((arr_2 [i_0 - 1]) * (arr_2 [i_0 - 1]))));
            var_19 = (max(var_19, (((-502770801 ? 1540346906 : 10729)))));
        }
    }
    for (int i_2 = 0; i_2 < 23;i_2 += 1)
    {
        var_20 = (arr_1 [3]);
        var_21 = (max((!22428), (max((min((arr_7 [i_2] [i_2]), (arr_5 [i_2] [i_2] [i_2]))), (((arr_2 [i_2]) & var_5))))));
        arr_9 [i_2] [i_2] = (min(((((((arr_0 [i_2]) << (((arr_6 [i_2]) + 324801638717509020))))) ? ((((arr_1 [i_2]) + (arr_7 [i_2] [i_2])))) : (arr_6 [i_2]))), ((min(-405, 998040081)))));
    }
    var_22 = (max(var_22, var_3));
    #pragma endscop
}
