#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

unsigned char var_0 = (unsigned char)65;
int var_1 = 1412693174;
unsigned char var_6 = (unsigned char)89;
short var_7 = (short)-14110;
unsigned char var_9 = (unsigned char)138;
signed char var_10 = (signed char)6;
unsigned char var_14 = (unsigned char)120;
int zero = 0;
unsigned long long int var_15 = 12949133675795285276ULL;
unsigned short var_16 = (unsigned short)2297;
int var_17 = -1300862001;
signed char var_18 = (signed char)12;
unsigned char arr_4 [20] [20] ;
void init() {
    for (size_t i_0 = 0; i_0 < 20; ++i_0) 
        for (size_t i_1 = 0; i_1 < 20; ++i_1) 
            arr_4 [i_0] [i_1] = (unsigned char)230;
}

void checksum() {
    hash(&seed, var_15);
    hash(&seed, var_16);
    hash(&seed, var_17);
    hash(&seed, var_18);
    for (size_t i_0 = 0; i_0 < 20; ++i_0) 
        for (size_t i_1 = 0; i_1 < 20; ++i_1) 
            hash(&seed, arr_4 [i_0] [i_1] );
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
