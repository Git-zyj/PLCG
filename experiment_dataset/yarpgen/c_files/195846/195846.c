/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195846
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195846 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195846
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_16;
    var_18 = (min(var_18, var_5));
    var_19 &= ((((((((var_3 ? var_13 : 26973))) ? var_14 : 1))) ? (((var_14 >= 0) << (var_3 + 11317))) : ((((((var_14 ? var_14 : -1791167219)) + 2147483647)) << (((0 | 13275) - 13275))))));
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 8;i_1 += 1)
        {
            {
                arr_7 [i_1] = (arr_1 [i_0]);
                arr_8 [i_1] [i_1] = ((+((max((arr_2 [i_0 - 1]), (arr_1 [i_0]))))));
                var_20 *= ((-(!var_4)));
                var_21 = ((((((arr_1 [i_0 - 4]) | -13260))) ? (((((arr_3 [i_0] [i_1 + 1]) + 2147483647)) << (((max((arr_6 [i_0] [i_0]), (arr_2 [i_0]))) - 4294959639)))) : (+-1)));
            }
        }
    }
    #pragma endscop
}
