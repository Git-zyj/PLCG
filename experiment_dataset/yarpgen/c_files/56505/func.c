/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 56505
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=56505 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/56505
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned short) min((var_11), (((/* implicit */unsigned short) var_0))));
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 11; i_0 += 4) 
    {
        var_12 ^= ((/* implicit */short) (+(((/* implicit */int) (!(((/* implicit */_Bool) arr_0 [i_0])))))));
        arr_2 [i_0] [(unsigned short)2] = ((/* implicit */unsigned char) max((((/* implicit */unsigned long long int) max((-1051944378683630429LL), (((/* implicit */long long int) 1465972622))))), (((arr_0 [i_0]) & (((((/* implicit */_Bool) arr_0 [i_0])) ? (var_1) : (arr_0 [i_0])))))));
    }
}
