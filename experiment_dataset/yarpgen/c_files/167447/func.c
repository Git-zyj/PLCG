/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 167447
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=167447 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/167447
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 += ((/* implicit */unsigned char) ((var_5) & (((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned char)255)))));
    var_18 = ((/* implicit */unsigned char) var_15);
    var_19 = ((/* implicit */int) (!(((/* implicit */_Bool) var_3))));
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 25; i_0 += 3) 
    {
        arr_3 [i_0] = ((/* implicit */short) var_13);
        var_20 = ((/* implicit */_Bool) arr_2 [(unsigned short)14]);
        arr_4 [i_0] = ((/* implicit */signed char) (~(((/* implicit */int) ((unsigned char) (((_Bool)1) ? (((/* implicit */int) (unsigned short)23858)) : (((/* implicit */int) (unsigned short)9171))))))));
    }
}
