/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 12552
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=12552 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/12552
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, (((((min(var_1, 14404253628987606962))) - (((((9223372036854775807 << (((-6263208046980948951 + 6263208046980948955) - 4))))) ? (var_0 > var_13) : ((var_4 ? 31085 : var_4)))))))));
    var_18 = ((((var_11 < (223 <= var_2))) ? -889105748 : ((-(max(31091, 7))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 17;i_2 += 1)
            {
                {
                    var_19 = (min(var_19, (var_1 && -var_2)));
                    var_20 = (max(var_20, (arr_6 [i_0])));
                }
            }
        }
    }
    #pragma endscop
}
