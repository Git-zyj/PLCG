/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124688
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124688 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124688
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_4;

    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        var_19 += (!-1906132119);
        arr_3 [i_0] = ((((min((arr_0 [i_0 - 1]), (!91)))) ? -12188 : -1116637244));
    }
    for (int i_1 = 2; i_1 < 14;i_1 += 1) /* same iter space */
    {
        var_20 += ((((max((arr_0 [i_1 - 2]), (arr_1 [i_1 + 1])))) && ((max((arr_0 [i_1 - 1]), (arr_0 [i_1 + 1]))))));
        var_21 += var_13;
    }
    for (int i_2 = 2; i_2 < 14;i_2 += 1) /* same iter space */
    {
        arr_8 [i_2] = 165;
        var_22 += (((arr_0 [i_2 - 2]) || ((((arr_0 [i_2 - 2]) % (arr_0 [i_2 - 1]))))));
        var_23 += ((((min(56, (arr_4 [4])))) >= (((-65535 != (1906132104 + -19582))))));
    }
    /* vectorizable */
    for (int i_3 = 2; i_3 < 14;i_3 += 1) /* same iter space */
    {
        var_24 = (max(var_24, -1906132083));
        var_25 = (((arr_7 [6]) ? (arr_7 [8]) : (arr_7 [0])));
    }
    #pragma endscop
}
