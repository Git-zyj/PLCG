/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165392
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165392 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165392
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 12;i_2 += 1)
            {
                {
                    arr_9 [i_1] [i_1] [i_0 - 3] = (max(17648555160803656493, 0));
                    arr_10 [i_2] = ((+((max((arr_7 [i_0] [i_0 + 1] [i_2 - 1]), var_10)))));
                    var_20 = ((((((arr_1 [9] [i_0 - 2]) ? var_6 : (arr_1 [i_0 + 1] [i_0 - 2])))) ? 1 : (((arr_1 [i_2 - 2] [i_0 - 2]) ? (arr_1 [i_0] [i_0 - 2]) : var_7))));
                }
            }
        }
    }
    var_21 ^= var_2;
    var_22 = (max(var_22, var_11));
    #pragma endscop
}
