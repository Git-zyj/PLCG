/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138796
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138796 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138796
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 21;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 2; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 22;i_4 += 1)
                        {
                            {
                                arr_12 [i_0] [i_0] [i_0] [i_1] [12] = (arr_6 [i_3 + 2] [i_2 + 1]);
                                arr_13 [i_1] [i_1] [i_1] [i_1 + 1] [i_1 - 3] = (-32767 - 1);
                                arr_14 [i_0] [i_0] [i_1] [i_3 + 1] [i_4] = -9460;
                            }
                        }
                    }
                }
                var_18 ^= ((((((arr_4 [i_1 - 2] [i_1 + 1]) ? (arr_4 [i_1 - 2] [i_1]) : (arr_4 [i_1 - 1] [i_1])))) ? (arr_6 [i_0] [i_1]) : (min((arr_5 [i_1 - 1]), -32767))));
            }
        }
    }
    var_19 = 32764;
    #pragma endscop
}
