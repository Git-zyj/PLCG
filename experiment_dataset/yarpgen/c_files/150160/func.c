/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150160
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150160 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150160
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned long long int) ((((/* implicit */int) var_3)) >> (((var_11) - (2579472029U)))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (int i_0 = 0; i_0 < 20; i_0 += 1) 
    {
        var_13 = ((/* implicit */short) ((((/* implicit */int) var_4)) | (((((/* implicit */_Bool) (unsigned char)194)) ? (((/* implicit */int) (short)-26816)) : (((/* implicit */int) (short)26816))))));
        var_14 = ((/* implicit */unsigned int) var_10);
    }
}
