/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23313
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23313 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23313
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_4;
    var_14 = (((min(var_5, var_2)) > var_5));
    var_15 = (min(var_15, (var_11 * -var_11)));
    var_16 = var_1;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    var_17 = (arr_1 [i_2] [i_0]);
                    arr_9 [i_1] [i_1] [i_1] [i_1] = ((((min(127, 1594609487))) / (((((max((arr_0 [i_0] [i_1]), var_8))) && (arr_2 [i_1]))))));
                }
            }
        }
    }
    #pragma endscop
}
