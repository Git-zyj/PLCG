/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98010
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98010 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98010
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_6;
    var_21 -= var_7;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {

                    for (int i_3 = 2; i_3 < 9;i_3 += 1)
                    {
                        arr_12 [i_0] [0] [i_0] [i_3] [i_3] [i_3] = var_19;
                        var_22 ^= ((((var_18 == (arr_8 [i_0] [i_2]))) && var_6));
                    }
                    arr_13 [i_0] [i_2] = ((var_13 + var_10) ? (arr_10 [0] [i_2] [i_0] [i_0] [i_1] [i_0]) : -1576846351);
                    var_23 = var_11;
                    var_24 = (min(var_24, 163));
                }
            }
        }
        arr_14 [i_0] = var_19;
        var_25 = (((92 ? 0 : 1)));
    }
    var_26 = 70;
    #pragma endscop
}
