/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158433
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158433 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158433
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_8, var_15));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 8;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            {
                                arr_14 [i_1] [i_1] = max(var_11, (((arr_12 [i_1] [i_2] [i_3 + 1] [i_3 + 1] [i_3] [i_1] [i_3 + 2]) ? (arr_8 [i_2]) : var_1)));
                                var_17 = var_2;
                                var_18 *= (arr_7 [i_3 - 1] [i_0] [i_3] [i_3 + 4]);
                            }
                        }
                    }
                    var_19 -= ((~(max(var_6, (arr_4 [i_0])))));
                    var_20 = 2147483647;
                    var_21 += -18446744073709551615;
                }
            }
        }
    }
    var_22 = var_12;
    #pragma endscop
}
