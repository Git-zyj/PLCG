/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86805
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86805 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86805
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 ^= ((/* implicit */_Bool) (signed char)-1);
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 23; i_0 += 1) 
    {
        arr_2 [i_0] [i_0] = ((/* implicit */unsigned char) ((((/* implicit */unsigned int) ((/* implicit */int) var_8))) & (((4294967295U) | (((0U) | (((/* implicit */unsigned int) ((/* implicit */int) (signed char)-105)))))))));
        arr_3 [i_0] = ((/* implicit */unsigned int) (~(((arr_0 [i_0]) | (arr_0 [i_0])))));
    }
    var_12 = ((/* implicit */short) min((var_12), (((/* implicit */short) var_4))));
}
