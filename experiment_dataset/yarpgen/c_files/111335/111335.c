/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111335
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111335 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111335
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_19 = ((7 ? ((((arr_7 [i_0] [i_1] [i_2]) ? (arr_7 [i_0] [i_1] [i_2]) : (arr_7 [i_0] [i_1] [17])))) : (max(-4294967295, ((max((arr_7 [i_2] [i_1] [i_0]), 2147483647)))))));
                    var_20 ^= (arr_5 [i_0] [18] [i_0] [i_0]);
                    var_21 = (min(2147483647, 2147483647));
                    arr_8 [i_0] [i_1] [i_1] = 2147483647;
                }
            }
        }
    }
    var_22 = var_0;
    var_23 = var_9;
    #pragma endscop
}
