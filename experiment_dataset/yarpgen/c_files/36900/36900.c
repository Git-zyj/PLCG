/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36900
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36900 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36900
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        {
                            var_18 = (max((arr_7 [i_0] [i_1] [i_2] [i_2]), (min(-238, ((-(arr_2 [i_0])))))));
                            arr_12 [i_1] [i_3 - 1] = (((arr_5 [i_1]) ? ((min(1, (arr_2 [i_3 - 1])))) : var_13));
                            var_19 = (max(var_19, 4031179949));
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_4 = 0; i_4 < 24;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 24;i_5 += 1)
                    {
                        {
                            arr_18 [i_1] [i_5] = var_0;
                            var_20 = -127;
                            arr_19 [i_4] [i_1] [i_1] [i_0] = (arr_13 [i_0] [i_1] [i_4] [i_5]);
                        }
                    }
                }
            }
        }
    }
    var_21 = ((-((max(-124, 127)))));
    #pragma endscop
}
