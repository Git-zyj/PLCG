/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226018
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226018 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226018
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min((((var_3 % var_11) ? 715157466907464664 : (((-104 ? var_3 : var_9))))), 715157466907464675));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    var_13 += ((max(17731586606802086949, 0)));
                    var_14 = (((((~(((!(arr_7 [i_0] [i_0]))))))) ? (((((max(-61, (arr_5 [i_0] [i_1] [i_1] [i_2])))) < -118))) : (arr_2 [i_0] [i_2])));
                }
            }
        }
    }
    var_15 = ((~(max((-12 & 17731586606802086969), var_9))));
    var_16 = -54;
    #pragma endscop
}
