/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235344
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235344 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235344
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_6;

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_19 |= (((arr_1 [2] [i_0 - 1]) % -23));
        var_20 = (-53 > 1053);
        /* LoopNest 2 */
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 21;i_4 += 1)
                        {
                            {
                                arr_13 [i_0] [i_1 - 1] [i_0] [i_0] [i_2] [i_4] = (~var_1);
                                arr_14 [i_0] [i_0 - 1] [i_0 - 1] [i_2] [i_0] [i_0 - 1] [i_0 - 1] = (!var_15);
                                arr_15 [i_2] [i_2] [i_3] = (arr_0 [i_0 - 1] [i_0 - 1]);
                            }
                        }
                    }
                    arr_16 [i_2] [i_2] = var_12;
                    var_21 = (min(((min(56654, 10948459919749082388))), ((((min((arr_6 [i_2]), var_8)) - -var_3)))));
                }
            }
        }
    }
    #pragma endscop
}
