/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 137200
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=137200 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/137200
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_14 = (((var_13 ? var_8 : (arr_0 [i_0]))));
        var_15 = -838016760;
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 16;i_3 += 1)
                {
                    {
                        var_16 = 0;
                        var_17 = 0;
                        var_18 = (!1);
                        arr_11 [i_0] [i_2] [i_2] [i_3] = ((((((arr_3 [i_1]) ? (arr_8 [i_3]) : (max((arr_10 [i_1] [i_3]), (arr_3 [i_0])))))) ? ((((max(512, var_5))) ? ((var_6 & (-2147483647 - 1))) : 512)) : (arr_9 [i_0] [i_1] [i_2] [i_3] [i_3])));
                        var_19 = 1;
                    }
                }
            }
        }
        var_20 = (arr_10 [i_0] [i_0]);
    }
    var_21 = ((~(max(899424013, 6005))));
    #pragma endscop
}
