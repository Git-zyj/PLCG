/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127642
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127642 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127642
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_3;
    var_12 = var_1;
    var_13 = var_5;
    var_14 *= (var_2 / 16733444141076371434);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_1] [i_2] = ((((arr_2 [i_1 - 1]) % (arr_5 [i_1 - 1] [i_1 - 1] [i_1 - 1] [i_1 - 1]))));
                    var_15 -= (((arr_3 [i_1 - 1] [i_2] [i_2]) && ((((arr_3 [i_1 - 1] [i_1 - 1] [0]) ? var_4 : (arr_3 [i_1 - 1] [i_1] [i_2]))))));
                    var_16 -= ((!((max((arr_1 [i_1]), (arr_1 [i_0]))))));
                }
            }
        }
    }
    #pragma endscop
}
