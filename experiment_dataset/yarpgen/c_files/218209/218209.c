/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218209
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218209 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218209
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, ((max(8547, 9223372036854775791)))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [i_0] = ((((((arr_2 [i_0] [i_0 - 1]) ? (209 || 1561088510) : (arr_4 [i_0] [i_1])))) ? (min((((arr_3 [i_0 - 1] [i_1]) ? (arr_4 [i_0] [i_1]) : 6562765440561000776)), (arr_4 [i_0] [i_0]))) : (((+(max((arr_3 [i_0] [i_1]), 72)))))));
                var_11 = (35046933135360 % 64);
            }
        }
    }
    #pragma endscop
}
