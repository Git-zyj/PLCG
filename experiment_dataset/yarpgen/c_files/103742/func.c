/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103742
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103742 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103742
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = var_15;
    /* LoopSeq 1 */
    /* vectorizable */
    for (int i_0 = 1; i_0 < 22; i_0 += 2) 
    {
        arr_4 [i_0] = ((/* implicit */int) (!((!(((/* implicit */_Bool) 1925269278))))));
        var_18 = ((/* implicit */unsigned short) var_8);
        var_19 *= ((/* implicit */unsigned char) ((((/* implicit */_Bool) arr_2 [i_0 - 1])) ? (((/* implicit */int) arr_2 [i_0 + 2])) : (((/* implicit */int) arr_2 [i_0 - 1]))));
        arr_5 [(unsigned short)10] = ((/* implicit */unsigned short) var_15);
        var_20 ^= ((/* implicit */unsigned char) (!(((/* implicit */_Bool) 7915533264433072099ULL))));
    }
}
