/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3368
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3368 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3368
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_9;
    var_12 = var_6;
    var_13 = ((((max(18375767323497636315, 3))) ? (max(var_6, (var_0 ^ var_4))) : var_0));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {

        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            var_14 = (11996427037478636581 && 53283);
            arr_5 [i_1] [i_1 - 1] = (arr_0 [i_1]);
        }
        /* LoopNest 3 */
        for (int i_2 = 4; i_2 < 18;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 1;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 20;i_4 += 1)
                {
                    {
                        var_15 = (arr_1 [i_0] [i_0]);
                        var_16 = (((!var_0) ? 1 : (11620 / 173)));
                    }
                }
            }
        }
        var_17 = ((70976750211915313 ? (arr_11 [4] [14] [i_0] [1]) : var_8));
    }
    #pragma endscop
}
