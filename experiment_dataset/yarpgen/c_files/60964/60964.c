/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60964
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60964 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60964
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1) /* same iter space */
    {
        var_12 &= (((((var_3 << (((arr_0 [i_0]) - 68))))) || (arr_2 [i_0] [20])));
        var_13 = (max(var_13, (((-2330798610331217574 ? var_2 : (((!(arr_0 [i_0])))))))));
        arr_3 [i_0] [i_0] = (-9223372036854775807 - 1);
        var_14 = (max(var_14, (~var_5)));
        var_15 = (min(var_15, ((((((~(arr_1 [i_0])))) || 2330798610331217574)))));
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1) /* same iter space */
    {
        arr_6 [i_1] [i_1] = ((~var_11) ? (arr_5 [i_1]) : (((((((arr_5 [i_1]) == var_11)))) | var_9)));
        var_16 = (min(var_16, (arr_4 [14])));
    }
    var_17 = var_4;
    var_18 = (min((9223372036854775807 == var_2), var_4));
    var_19 = var_6;
    #pragma endscop
}
