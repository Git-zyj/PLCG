/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213342
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213342 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213342
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_6 [i_0] = (min((arr_5 [i_1] [2]), (((((((arr_3 [i_0] [i_1] [i_1]) ? (arr_4 [i_1] [i_1]) : 0))) ? (var_9 | 15) : var_4)))));
                var_12 ^= ((var_10 ? (arr_0 [i_1] [i_1]) : (((arr_1 [i_0] [i_0]) ? var_4 : (max((arr_3 [8] [i_1] [1]), var_3))))));
                var_13 = 1421191264;
                arr_7 [i_1] [i_1] = ((~(arr_2 [i_0])));
            }
        }
    }
    #pragma endscop
}
