/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 228568
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=228568 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/228568
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((var_9 ? var_7 : ((-609304552 ? var_7 : var_13))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    var_16 = (max(var_16, ((((((~var_9) + 2147483647)) << (((min(1, -1515998247)) - 1)))))));
                    var_17 = (max(var_17, (((-(arr_0 [i_0] [i_0]))))));
                    arr_7 [i_0] = arr_1 [i_0] [i_1];
                }
                for (int i_3 = 0; i_3 < 19;i_3 += 1)
                {
                    arr_12 [i_0] [16] [i_3] = (arr_5 [i_0] [i_0] [i_0] [i_0]);
                    var_18 = ((arr_4 [i_0]) ? (arr_11 [i_0] [i_0] [i_0]) : ((~(arr_3 [i_0]))));
                    arr_13 [i_0] = -1;
                }
                arr_14 [1] [i_0] = (arr_8 [0] [1] [i_1] [i_0]);
                var_19 = 1977944106;
            }
        }
    }
    #pragma endscop
}
