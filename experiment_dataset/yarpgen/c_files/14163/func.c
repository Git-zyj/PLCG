/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14163
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14163 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14163
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 11; i_0 += 4) 
    {
        arr_2 [i_0] = ((/* implicit */unsigned char) min((arr_0 [i_0]), (((/* implicit */long long int) arr_1 [i_0]))));
        arr_3 [i_0] [i_0] = ((((/* implicit */_Bool) arr_0 [i_0])) && (((/* implicit */_Bool) (+(((/* implicit */int) max(((unsigned short)65535), (((/* implicit */unsigned short) (unsigned char)7)))))))));
    }
    var_19 = ((/* implicit */unsigned int) (-(var_10)));
}
