/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104401
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104401 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104401
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_7;
    var_16 = min(((min(var_2, var_7))), (min(var_6, var_9)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 9;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_1] [i_2] = (max(var_6, (max(var_10, (max(28392, 1585858798))))));

                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        arr_11 [i_0] [i_0] [i_0] = max((min(0, (min(var_11, var_12)))), (max(var_2, 11484)));
                        arr_12 [i_0] [i_0] [i_2] [i_0] [i_3] = var_12;
                        var_17 = (min(var_17, var_4));
                        arr_13 [i_0] [i_0] [i_0] [i_0] [i_0] = var_5;
                    }
                }
            }
        }
    }
    #pragma endscop
}
