/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46607
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46607 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46607
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_14 = (min(var_14, (((-var_12 == ((((!(arr_1 [i_0] [i_0]))))))))));
        var_15 += ((var_7 ? 1 : (arr_0 [i_0])));
        arr_2 [i_0] = (arr_0 [i_0]);
    }

    for (int i_1 = 0; i_1 < 21;i_1 += 1)
    {
        var_16 = (min(((((var_5 ? (arr_3 [i_1] [i_1]) : -69)) + (arr_3 [i_1] [i_1]))), (((!((((arr_3 [i_1] [i_1]) ? var_5 : var_10))))))));
        var_17 = var_10;
        var_18 = (((-15 % var_1) | (max((((-2147483647 - 1) ? 2240365766 : 6623)), var_1))));
    }
    var_19 = 70368744177408;
    #pragma endscop
}
