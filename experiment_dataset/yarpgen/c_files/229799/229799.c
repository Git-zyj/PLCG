/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229799
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229799 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229799
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_5 [i_1] = -15;
                var_12 -= (~2147483647);
                var_13 = ((((!(arr_0 [i_0 - 2]))) ? (max((arr_1 [i_0 - 1]), (~0))) : ((((var_4 * (arr_1 [i_1]))) - var_10))));
            }
        }
    }
    var_14 = 12747392480548821167;
    var_15 = 14179;
    var_16 = (var_9 > (max(((min(1, var_11))), ((var_9 ? var_7 : var_9)))));
    var_17 = (min(var_17, ((var_9 ? var_9 : ((((var_1 <= var_8) ? -var_5 : var_11)))))));
    #pragma endscop
}
