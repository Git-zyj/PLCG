/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239162
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239162 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239162
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned short i_0 = 0; i_0 < 21; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 21; i_1 += 1) 
        {
            for (long long int i_2 = 2; i_2 < 20; i_2 += 4) 
            {
                {
                    var_18 ^= ((/* implicit */long long int) max((min((((/* implicit */unsigned long long int) (_Bool)1)), (11163567440611488856ULL))), (max((arr_4 [i_2] [i_2 + 1] [i_2]), (arr_4 [i_2] [i_2 + 1] [i_2])))));
                    arr_8 [i_0] [i_0] [i_1] = ((/* implicit */unsigned char) (~((-(18066185326454192194ULL)))));
                }
            } 
        } 
    } 
    var_19 = ((/* implicit */unsigned long long int) var_6);
}
