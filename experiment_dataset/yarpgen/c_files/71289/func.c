/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71289
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71289 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71289
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned short) var_8);
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 3; i_0 < 10; i_0 += 4) 
    {
        var_17 = (short)-27221;
        var_18 += ((/* implicit */short) max((min((((/* implicit */unsigned short) (unsigned char)146)), (var_0))), (((/* implicit */unsigned short) ((((/* implicit */_Bool) (unsigned char)107)) || (((/* implicit */_Bool) var_6)))))));
        arr_3 [i_0] = ((/* implicit */short) min(((!(((/* implicit */_Bool) var_7)))), ((!(((/* implicit */_Bool) 513438297U))))));
    }
}
