/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 176201
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=176201 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/176201
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(var_16, var_11));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 22;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_17 = (min((((((var_6 ? (arr_4 [i_2] [i_2] [1]) : (arr_3 [i_1] [8] [i_4 - 1])))) ? -7827107648360716691 : (9223372036854775800 + -25235))), (((2706505289391258567 ? 917068974 : -9575)))));
                                var_18 -= (arr_2 [i_1 + 1] [i_2 - 1] [i_3]);
                            }
                        }
                    }
                    var_19 = (arr_8 [i_0] [i_0] [i_1] [i_1] [1]);
                }
            }
        }
    }
    var_20 &= 4;
    #pragma endscop
}
