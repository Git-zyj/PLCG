/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 115079
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=115079 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/115079
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (min(var_17, var_0));
    var_18 = min(4294967278, -25849);
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 13;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_0] = (((((arr_1 [i_0]) / ((3 << (2147483647 - 2147483621))))) ^ ((~((1 << (var_9 - 53784)))))));
                arr_6 [i_1] [i_0] [i_0 + 2] = ((var_16 ? (((((-713198974 ? -713198974 : (arr_2 [i_0] [i_1] [i_1])))) ? 17 : var_16)) : 1462200704));
            }
        }
    }
    #pragma endscop
}
