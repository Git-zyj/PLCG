/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43487
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43487 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43487
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_11;
    var_13 = var_6;
    var_14 = (min((min((-32765 ^ var_3), (246 & var_0))), (((!((min(0, 176))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_4 [i_1] = ((((min(var_1, (arr_2 [i_1] [i_1])))) ^ (min(((((arr_3 [13]) ^ 2))), (min(var_2, var_5))))));
                arr_5 [i_1] [12] = ((-143 ? ((((((max(0, var_6)))) != 8102259586650167511))) : 0));
                arr_6 [8] [i_1] [1] = (min(243, (min(var_10, 576460752303390720))));
                arr_7 [i_1] [i_0] = var_4;
            }
        }
    }
    #pragma endscop
}
