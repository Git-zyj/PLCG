/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89588
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89588 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89588
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 *= ((/* implicit */unsigned short) (~(var_12)));
    var_21 = ((/* implicit */unsigned int) min((var_21), (var_15)));
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 24; i_0 += 2) 
    {
        for (short i_1 = 1; i_1 < 23; i_1 += 2) 
        {
            {
                arr_4 [i_1] = ((/* implicit */unsigned short) ((((/* implicit */unsigned long long int) ((/* implicit */int) arr_2 [i_0] [i_1] [i_0]))) <= (arr_1 [i_1] [i_0])));
                var_22 += ((/* implicit */signed char) (_Bool)1);
                var_23 = ((/* implicit */unsigned int) ((((/* implicit */_Bool) (-(arr_0 [i_1 + 1])))) ? (((/* implicit */long long int) ((((((/* implicit */_Bool) arr_1 [i_0] [i_1])) ? (arr_0 [i_0]) : (3261711304U))) + (205029606U)))) : ((-((-(var_12)))))));
            }
        } 
    } 
}
