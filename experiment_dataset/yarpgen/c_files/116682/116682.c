/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116682
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116682 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116682
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_4;
    var_13 |= ((1 ? (((!(var_11 < var_3)))) : (var_10 < 1)));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            {
                var_14 = (((((25 ? (((arr_2 [i_0]) ? var_11 : var_0)) : (~-6)))) ? (arr_3 [i_0]) : var_5));
                var_15 = var_4;
                var_16 = min((((((~var_2) + 2147483647)) << (var_11 - 16402))), ((((var_4 << (1427619691 - 1427619680))) << ((min(var_6, (arr_1 [i_1])))))));
                var_17 = (min(var_17, var_8));
            }
        }
    }
    #pragma endscop
}
