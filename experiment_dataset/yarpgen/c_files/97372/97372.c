/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97372
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97372 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97372
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_6;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_3 [i_0] = (arr_1 [i_0] [i_0]);
        arr_4 [i_0] = (((arr_2 [i_0] [i_0]) ? (arr_2 [i_0] [i_0]) : (arr_2 [i_0] [2])));
        var_18 = (((arr_2 [i_0] [i_0]) / var_0));
        arr_5 [i_0] [12] = ((~(arr_1 [i_0] [i_0])));
    }
    var_19 &= (((((-((min(var_11, var_11)))))) ? var_13 : (min(var_7, var_16))));
    var_20 ^= (((var_9 + 9223372036854775807) >> (((max(var_5, var_7)) - 11253583834325938622))));
    #pragma endscop
}
