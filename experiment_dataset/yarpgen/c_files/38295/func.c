/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38295
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38295 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38295
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */unsigned short) var_8);
    var_14 = var_3;
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 22; i_0 += 3) 
    {
        for (short i_1 = 0; i_1 < 22; i_1 += 4) 
        {
            {
                var_15 -= ((/* implicit */short) max((arr_3 [i_1]), (arr_3 [i_0])));
                var_16 = ((/* implicit */unsigned char) arr_0 [i_0]);
                var_17 &= ((/* implicit */short) (-(arr_2 [i_0] [i_1])));
                var_18 += ((/* implicit */unsigned int) min((((/* implicit */unsigned long long int) (-(arr_0 [i_1])))), (max((arr_5 [i_0] [i_1] [i_1]), (((/* implicit */unsigned long long int) max((arr_0 [i_0]), (((/* implicit */long long int) (signed char)15)))))))));
                var_19 = ((/* implicit */unsigned short) (signed char)-28);
            }
        } 
    } 
}
