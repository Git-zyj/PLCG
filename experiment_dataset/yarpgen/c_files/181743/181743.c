/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181743
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181743 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181743
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_1;

    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        arr_4 [13] [i_0] = (arr_2 [i_0]);

        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            arr_9 [i_0] = (arr_8 [i_1]);
            var_12 = (max(var_12, ((max(((~(arr_0 [8]))), (((!((max((arr_0 [8]), var_3)))))))))));
            var_13 = (min(var_13, (arr_8 [i_1])));
        }
    }
    var_14 -= var_10;
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        for (int i_3 = 2; i_3 < 22;i_3 += 1)
        {
            {
                var_15 *= ((!((max(((max((arr_12 [23]), var_10))), var_9)))));
                var_16 = (max(var_2, ((var_2 ? var_3 : ((~(arr_14 [i_2] [i_3] [i_2])))))));
            }
        }
    }
    #pragma endscop
}
