/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99584
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99584 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99584
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 22;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 2; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 22;i_4 += 1)
                        {
                            {
                                var_18 -= var_17;
                                var_19 = (arr_8 [i_0] [i_2] [i_1] [i_0]);
                            }
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_5 = 2; i_5 < 19;i_5 += 1)
                {
                    for (int i_6 = 0; i_6 < 23;i_6 += 1)
                    {
                        {
                            var_20 = (((((var_13 && 103) && 1402052946)) ? (((arr_2 [i_5 - 1]) & 2534547345372715292)) : (arr_12 [i_0] [19] [i_5] [6] [i_6])));
                            var_21 = (max((((3041573621482299011 && (arr_3 [i_0])))), 219));
                        }
                    }
                }
            }
        }
    }
    var_22 = (max(1, 31744));
    #pragma endscop
}
