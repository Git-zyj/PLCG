/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30709
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30709 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30709
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_20 = (min(var_20, 1600875386));
                var_21 = ((((max((arr_5 [i_0 - 2]), (arr_5 [i_0 - 2])))) ? (((arr_5 [i_0 - 2]) ? (arr_5 [i_0 - 1]) : (arr_5 [i_0 - 3]))) : (((arr_5 [i_0 - 3]) + (arr_5 [i_0 - 1])))));
                var_22 *= (~65530);
            }
        }
    }
    var_23 = (((((var_10 ? 96 : 43628))) ? var_18 : ((((28674 ? var_15 : var_18)) / ((var_14 ? 40521 : var_15))))));
    #pragma endscop
}
