/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158092
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158092 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158092
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_19 *= ((1580104163 ? (1018137286 || 23372) : ((((8847110910797295255 ? 5416020773719782574 : 4294967291)) << var_6))));
                    var_20 = (max((((!(((var_12 ? 4294967295 : var_14)))))), (((((var_15 ? var_8 : 26260))) ? (arr_2 [i_1] [i_0]) : (arr_7 [i_0] [i_0])))));
                }
            }
        }
    }
    var_21 |= (max((((var_16 <= (max(var_18, 26260))))), var_12));
    var_22 = (min(var_22, var_10));
    #pragma endscop
}
