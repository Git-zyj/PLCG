/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100499
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100499 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100499
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 |= var_1;
    var_17 -= var_10;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 25;i_4 += 1)
                        {
                            {
                                var_18 = (max(var_18, ((((max(var_13, ((var_3 ? 18446744073709551615 : 3185678027)))) >> ((min(var_8, var_12))))))));
                                var_19 ^= ((((max((arr_1 [i_4]), (arr_2 [i_3])))) / var_5));
                            }
                        }
                    }
                }
                var_20 = (min(var_20, var_8));
            }
        }
    }
    var_21 = (max(var_21, ((max(var_15, (min(var_0, (var_2 + 6540099766825338540))))))));
    #pragma endscop
}
