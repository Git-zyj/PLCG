/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70912
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70912 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70912
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min(var_10, (2582109665313038749 ^ 55283)));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_11 = (max(2582109665313038765, 17179869183));
                arr_6 [i_0] [11] = 56;
                var_12 *= (max(31457, (min((arr_4 [i_0 + 1]), (arr_4 [i_0 + 1])))));
                var_13 = (max(var_13, ((((0 - (arr_2 [i_0 - 2])))))));
                var_14 ^= 65535;
            }
        }
    }
    #pragma endscop
}
