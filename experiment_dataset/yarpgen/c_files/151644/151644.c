/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151644
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151644 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151644
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_16 = -6078347562168721378;
                arr_4 [i_0] [i_0] = (i_0 % 2 == 0) ? (((~(((arr_1 [i_0]) ? (((arr_2 [1] [1] [i_0]) >> (arr_3 [i_0] [i_0] [i_0]))) : (arr_3 [i_0] [i_1] [i_0])))))) : (((~(((arr_1 [i_0]) ? (((((arr_2 [1] [1] [i_0]) + 9223372036854775807)) >> (arr_3 [i_0] [i_0] [i_0]))) : (arr_3 [i_0] [i_1] [i_0]))))));
            }
        }
    }
    var_17 = 1743319646;
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 14;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 12;i_3 += 1)
        {
            {
                var_18 = (!49465);
                var_19 = (arr_5 [i_2]);
            }
        }
    }
    #pragma endscop
}
