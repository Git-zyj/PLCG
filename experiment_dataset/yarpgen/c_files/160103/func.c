/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160103
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160103 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160103
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */short) var_4);
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 19; i_0 += 1) 
    {
        var_12 *= arr_0 [i_0];
        arr_3 [(unsigned char)12] |= ((/* implicit */int) ((((/* implicit */_Bool) ((((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned short)0))) | (arr_2 [(unsigned char)14])))) && (((/* implicit */_Bool) arr_2 [(unsigned short)12]))));
        var_13 *= ((/* implicit */signed char) (~((-2147483647 - 1))));
    }
    var_14 *= ((/* implicit */unsigned int) ((signed char) (+(max((var_3), (((/* implicit */int) var_2)))))));
    var_15 = ((/* implicit */unsigned char) var_1);
}
