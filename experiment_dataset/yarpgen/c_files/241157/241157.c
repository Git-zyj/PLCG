/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241157
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241157 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241157
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 12;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    var_12 = (arr_1 [i_1] [13]);
                    var_13 = var_7;
                }
                arr_8 [i_0] [i_1] [0] = 0;
                var_14 -= min(((~((var_7 ? 604102959 : 3690864336)))), ((-3690864336 ? -2594883956 : var_11)));
                /* LoopNest 2 */
                for (int i_3 = 2; i_3 < 11;i_3 += 1)
                {
                    for (int i_4 = 0; i_4 < 15;i_4 += 1)
                    {
                        {
                            var_15 *= ((~((~(arr_4 [i_0] [i_1 - 2])))));
                            arr_14 [8] [i_1] [i_4] [i_4] [i_4] [i_4] = (min(((~(arr_11 [i_1] [i_1]))), (max((arr_10 [i_1 + 3] [i_1] [i_0] [i_0]), var_10))));
                        }
                    }
                }
            }
        }
    }
    var_16 |= -604102959;
    var_17 = 19;
    var_18 = (var_6 ? var_6 : var_8);
    #pragma endscop
}
