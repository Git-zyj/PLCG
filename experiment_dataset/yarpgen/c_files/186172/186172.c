/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 186172
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=186172 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/186172
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        arr_4 [i_0] [i_0] &= ((!(arr_2 [i_0] [i_0])));
        var_13 = (min(var_13, (arr_1 [i_0])));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 25;i_1 += 1) /* same iter space */
    {
        arr_8 [i_1] = var_0;
        arr_9 [i_1] = (!var_12);
        arr_10 [i_1] [i_1] = (((var_3 ? var_4 : 0)));
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 25;i_2 += 1) /* same iter space */
    {

        for (int i_3 = 0; i_3 < 25;i_3 += 1)
        {
            arr_15 [i_2] [i_2] [i_2] = var_7;
            arr_16 [i_2] [i_2] = ((((arr_13 [i_2]) ? -126 : 72)));
            arr_17 [i_2] [i_2] = (var_3 || var_4);
            var_14 = (max(var_14, (arr_14 [i_2])));
            var_15 = (max(var_15, var_5));
        }
        var_16 = (min(var_16, (((((((arr_13 [18]) ? (arr_5 [10]) : 14))) ? ((42 ? 62 : 99)) : (((!(arr_12 [i_2] [i_2])))))))));
    }
    var_17 = var_8;
    var_18 = (max((var_8 | 14), var_12));
    var_19 = 0;
    var_20 = var_9;
    #pragma endscop
}
