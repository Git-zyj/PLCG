/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124070
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124070 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124070
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1) /* same iter space */
    {
        var_12 = ((486324797 ? (((!(((var_10 ? (arr_2 [14]) : (arr_3 [i_0]))))))) : (max(((-18 ? 217 : -18030)), 255))));
        var_13 = (arr_0 [i_0]);
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1) /* same iter space */
    {
        arr_6 [i_1] = ((var_1 < (((((arr_0 [i_1]) != 123)) ? (((max(0, var_11)))) : 127))));
        var_14 = (((((var_10 || (arr_1 [i_1] [i_1]))) && var_0)) ? (arr_5 [i_1]) : (((((var_5 ^ (arr_2 [i_1])))) + (arr_5 [i_1]))));
    }
    var_15 -= var_2;
    #pragma endscop
}
