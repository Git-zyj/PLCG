/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49262
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49262 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49262
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */int) var_3);
    /* LoopSeq 1 */
    for (unsigned char i_0 = 1; i_0 < 9; i_0 += 3) 
    {
        var_17 = arr_0 [i_0] [i_0];
        arr_4 [i_0] = ((/* implicit */unsigned char) max(((+(min((577121629), (((/* implicit */int) var_12)))))), (max((((((/* implicit */_Bool) arr_1 [i_0] [(unsigned short)4])) ? (((/* implicit */int) arr_1 [i_0] [i_0])) : (((/* implicit */int) arr_3 [(unsigned short)4])))), (((/* implicit */int) arr_2 [(_Bool)1]))))));
    }
}
