/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75013
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75013 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75013
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (((min(((min(var_8, var_12))), ((123 ? var_4 : var_9)))) ^ var_14));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_2] [i_0] = (min((arr_1 [i_1]), (arr_5 [i_2] [i_2] [i_1] [i_0])));
                    var_19 &= (((arr_4 [i_2]) ? (arr_5 [i_2] [i_1] [i_1] [i_0]) : (arr_2 [i_0])));
                }
            }
        }
    }
    var_20 |= var_4;
    #pragma endscop
}
