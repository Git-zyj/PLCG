/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41606
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41606 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41606
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_2 [i_0] = ((!((((arr_0 [i_0]) ? 0 : var_1)))));
        var_17 = (((((arr_1 [i_0] [i_0]) ? var_15 : var_13))) ? -var_14 : var_10);
        arr_3 [i_0] [i_0] = ((+(((arr_1 [i_0] [i_0]) ? var_14 : var_7))));
    }
    var_18 = (min(var_18, -23));
    var_19 = (min(var_19, (((!(((-(!4294967294)))))))));
    #pragma endscop
}
