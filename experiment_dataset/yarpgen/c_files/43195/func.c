/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43195
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43195 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43195
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */short) var_0);
    var_12 = ((/* implicit */unsigned int) min((max((5245182128404069393ULL), (((/* implicit */unsigned long long int) var_5)))), (((/* implicit */unsigned long long int) var_8))));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 20; i_0 += 1) 
    {
        var_13 = ((/* implicit */unsigned int) (short)-29);
        var_14 *= ((/* implicit */unsigned int) ((unsigned short) ((unsigned int) arr_2 [i_0])));
    }
}
