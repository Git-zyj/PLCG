/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247381
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247381 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247381
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = -2;
    var_13 = ((!((max(var_10, (min(7716494046822436975, var_2)))))));
    var_14 = 63;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = (arr_0 [i_1 + 2] [i_1 + 2]);
                arr_7 [6] [6] = (min(((min((!2464586358658022827), -18))), (arr_4 [i_0] [i_0] [i_0])));
                arr_8 [i_0] [i_0] [12] = arr_1 [i_0] [i_1];
                arr_9 [8] [2] [8] = min((((min((arr_3 [i_0] [i_0]), (arr_0 [4] [4]))))), -76);
                arr_10 [i_0] [i_0] = arr_5 [i_0] [i_0];
            }
        }
    }
    #pragma endscop
}
