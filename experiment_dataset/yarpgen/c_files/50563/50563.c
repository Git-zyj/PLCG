/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 50563
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=50563 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/50563
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = 204;

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        arr_2 [i_0] [10] = min((arr_0 [i_0]), (((arr_1 [i_0] [i_0]) ? (arr_0 [i_0]) : (arr_0 [i_0]))));
        arr_3 [i_0] = (min((((!(((-7755096196872645912 ? 1352126536 : (arr_0 [i_0]))))))), (((arr_0 [i_0]) ? (((arr_1 [i_0] [i_0]) ? (arr_1 [i_0] [i_0]) : (arr_0 [i_0]))) : (min((arr_1 [i_0] [i_0]), 1692173833))))));
    }
    var_11 = (max((var_2 | var_1), 138));
    #pragma endscop
}
