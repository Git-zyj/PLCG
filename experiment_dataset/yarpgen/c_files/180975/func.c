/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180975
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180975 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180975
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (int i_0 = 1; i_0 < 21; i_0 += 1) 
    {
        var_12 = ((/* implicit */unsigned char) max((max((((/* implicit */unsigned long long int) min((((/* implicit */long long int) var_0)), (arr_3 [i_0])))), (max((((/* implicit */unsigned long long int) (short)-855)), (18446744073709551615ULL))))), (((/* implicit */unsigned long long int) min((min((((/* implicit */long long int) arr_1 [i_0 - 1] [i_0])), (var_10))), (min((arr_3 [i_0]), (((/* implicit */long long int) -862406273)))))))));
        var_13 = min((min((arr_0 [i_0 + 1]), (arr_0 [i_0 + 2]))), (max((arr_0 [i_0 + 1]), (arr_0 [i_0 + 2]))));
    }
    var_14 = ((/* implicit */short) var_3);
    var_15 = ((/* implicit */int) min((((/* implicit */signed char) var_2)), (var_4)));
}
