/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110040
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110040 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110040
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */int) var_4);
    /* LoopSeq 1 */
    for (unsigned int i_0 = 1; i_0 < 13; i_0 += 1) 
    {
        var_21 = ((/* implicit */short) min((var_21), (((/* implicit */short) (unsigned char)18))));
        var_22 = ((/* implicit */_Bool) ((short) 2797197471U));
    }
    var_23 = ((/* implicit */int) min((var_23), (((/* implicit */int) (short)-24116))));
}
