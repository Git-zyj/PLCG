/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 228802
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=228802 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/228802
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            {
                arr_4 [0] [i_1] |= ((((((arr_0 [2]) ? (arr_0 [10]) : 61))) ? ((195 ? (arr_1 [i_1 + 1]) : 170)) : 224));
                var_18 += (((min((((arr_0 [2]) / 197)), 169)) / 167));
                var_19 = ((+(((arr_1 [i_1 - 2]) ? (arr_1 [i_1 + 1]) : 150))));
                arr_5 [i_0] [i_0] [i_1] = ((-((~(arr_2 [i_0] [i_1] [i_1 + 1])))));
                var_20 = (max(var_20, 48));
            }
        }
    }
    var_21 |= (min(250, ((var_9 ? var_12 : ((max(var_3, 82)))))));
    #pragma endscop
}
