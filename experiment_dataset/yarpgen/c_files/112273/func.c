/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112273
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112273 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112273
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */unsigned short) min((((/* implicit */unsigned int) ((signed char) ((int) (unsigned short)126)))), (((unsigned int) (~(((/* implicit */int) var_6)))))));
    /* LoopNest 2 */
    for (unsigned short i_0 = 2; i_0 < 15; i_0 += 1) 
    {
        for (unsigned short i_1 = 3; i_1 < 14; i_1 += 1) 
        {
            {
                var_19 = ((/* implicit */unsigned long long int) min((var_19), (((/* implicit */unsigned long long int) var_11))));
                var_20 = ((/* implicit */_Bool) ((((/* implicit */_Bool) -446247225395921409LL)) ? (max((((/* implicit */int) (short)-4)), (arr_2 [i_0 - 2] [i_1]))) : (((/* implicit */int) var_9))));
                arr_7 [6LL] &= ((/* implicit */signed char) ((((/* implicit */long long int) ((/* implicit */int) ((_Bool) arr_2 [i_1 - 3] [i_0 + 1])))) <= (446247225395921409LL)));
            }
        } 
    } 
}
