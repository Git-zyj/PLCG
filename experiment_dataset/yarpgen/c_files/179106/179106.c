/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179106
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179106 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179106
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((max(var_5, -var_12)));
    var_14 = (max(var_9, (max(var_11, ((12716059992625258323 ? -114 : -2843))))));
    var_15 = ((-67 ? 4 : 144));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 21;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 24;i_4 += 1)
                        {
                            {
                                arr_12 [i_4] [i_4] [i_1] [i_1] [i_1] [10] = var_11;
                                var_16 += var_9;
                            }
                        }
                    }
                    arr_13 [i_1] [i_1] [i_2] = ((63618 ? 2842 : -4234732730529425549));
                }
            }
        }
        var_17 = ((740923382 ? 16 : (!124)));
    }
    #pragma endscop
}
