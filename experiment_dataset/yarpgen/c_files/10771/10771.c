/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 10771
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=10771 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/10771
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((!(((var_5 && var_16) || var_17))));
    var_20 += var_11;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_1] = (max((((((arr_3 [7] [i_0]) ? var_9 : var_6)) / ((max(var_13, (arr_2 [i_1] [i_0])))))), (((arr_0 [i_0] [i_0]) ^ var_18))));
                var_21 = (((((var_16 ? var_5 : var_10))) ? (max((min((arr_4 [i_0] [i_1]), (arr_0 [i_0] [i_1]))), 197)) : (((((((max((arr_0 [5] [i_0]), var_6)))) == ((29 ? 58 : (arr_4 [2] [i_1])))))))));
            }
        }
    }
    #pragma endscop
}
