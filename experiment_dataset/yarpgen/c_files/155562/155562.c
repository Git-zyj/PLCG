/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155562
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155562 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155562
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_5 [6] [i_0] [6] |= (arr_4 [i_0] [9]);
                arr_6 [i_0] = (arr_3 [i_0 - 1] [i_0 - 1] [i_0 - 1]);
                arr_7 [i_0] = (((((1 ? (arr_1 [i_0]) : (((min((arr_3 [i_0] [4] [5]), 1))))))) ? (min((max(1197242358, 1)), 1)) : (((((min(var_3, 230))) ? ((max(var_9, (arr_0 [1] [i_1])))) : (arr_4 [i_0] [i_0]))))));
            }
        }
    }
    var_13 = ((((var_0 ? (var_4 < var_11) : 15)) & (((var_10 < (230 != var_12))))));
    #pragma endscop
}
