/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 118687
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=118687 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/118687
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_17;
    var_20 = var_18;
    var_21 = (max(var_21, ((((((min(var_8, -113)))) == var_18)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 19;i_2 += 1)
            {
                {
                    var_22 += ((((max((arr_5 [i_2] [i_2] [i_2 - 2] [i_1]), (arr_5 [i_0] [i_0] [i_2 - 2] [i_1])))) ^ (-9 + var_3)));
                    var_23 = ((!(((-((((arr_4 [i_0] [i_1] [i_2]) == var_13))))))));
                }
            }
        }
    }
    var_24 = ((((((var_2 ? var_13 : var_6)) ? (max(var_17, var_9)) : var_6))));
    #pragma endscop
}
