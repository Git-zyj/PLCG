/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51210
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51210 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51210
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */int) (((((~(var_8))) ^ (((/* implicit */unsigned int) ((/* implicit */int) (_Bool)1))))) ^ ((+(var_7)))));
    /* LoopNest 2 */
    for (unsigned char i_0 = 0; i_0 < 13; i_0 += 1) 
    {
        for (long long int i_1 = 3; i_1 < 12; i_1 += 4) 
        {
            {
                var_13 = ((/* implicit */_Bool) ((max((arr_2 [i_1]), (((/* implicit */unsigned int) arr_0 [i_1 + 1])))) >> (((int) arr_5 [i_1] [i_1] [i_1 + 1]))));
                var_14 = ((/* implicit */int) arr_0 [i_0]);
                arr_6 [i_0] [i_1] [i_1] = arr_1 [i_0];
            }
        } 
    } 
}
