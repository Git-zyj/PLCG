/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 245783
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=245783 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/245783
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_3;

    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        arr_2 [i_0] = (((arr_0 [i_0]) ? ((((((arr_1 [i_0] [i_0]) ? 2648248694390078306 : 255))))) : ((+(((-127 - 1) ? (arr_1 [i_0] [i_0]) : (arr_0 [6])))))));
        var_14 = (((~(arr_0 [i_0]))));
    }

    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        var_15 = ((~((0 ? (arr_1 [i_1] [4]) : ((((arr_1 [i_1] [i_1]) ? (arr_0 [i_1]) : (arr_0 [i_1]))))))));
        arr_5 [i_1] = (min((arr_4 [i_1] [i_1]), 0));
    }
    #pragma endscop
}
