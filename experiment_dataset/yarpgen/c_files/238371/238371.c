/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 238371
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=238371 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/238371
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_10 &= ((var_8 << (var_0 - 11256)));
        arr_2 [i_0] = (5525703286146415145 / var_4);

        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {

            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                var_11 -= (var_2 * 1759635644);
                var_12 = ((203 ? ((var_8 ? var_2 : -1)) : (!65517)));
            }
            var_13 ^= (((((var_5 ? 5525703286146415141 : (arr_0 [i_0])))) ? 11302162278467171609 : (var_3 | -30)));
        }
        var_14 = (max(var_14, var_2));
    }
    var_15 = ((var_2 * (6350 | -4223)));
    #pragma endscop
}
