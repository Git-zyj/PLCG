/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 13634
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=13634 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/13634
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 17; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 15; i_1 += 3) 
        {
            {
                arr_4 [i_1] = ((/* implicit */short) ((((/* implicit */int) (short)0)) == (((/* implicit */int) (_Bool)0))));
                var_12 = ((/* implicit */short) (~(((/* implicit */int) (!(((/* implicit */_Bool) ((unsigned int) arr_0 [i_0]))))))));
            }
        } 
    } 
    var_13 = ((/* implicit */unsigned int) max((min((((/* implicit */unsigned short) ((((/* implicit */int) (signed char)2)) == (((/* implicit */int) var_5))))), (min((((/* implicit */unsigned short) (_Bool)0)), (var_6))))), (((/* implicit */unsigned short) var_11))));
    var_14 = ((/* implicit */unsigned long long int) var_4);
}
