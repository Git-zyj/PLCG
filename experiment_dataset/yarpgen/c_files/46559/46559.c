/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46559
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46559 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46559
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 23;i_1 += 1)
        {
            {
                arr_4 [i_0] = (((arr_0 [i_1 - 2]) ? (max((arr_0 [i_1 + 1]), -7521423732530220484)) : ((((arr_2 [i_1 - 1] [i_1 + 2]) % (arr_0 [i_1 + 1]))))));
                arr_5 [i_1] = (((((!(arr_3 [i_1 + 1] [i_1])))) <= (arr_2 [15] [i_1])));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_13 [i_4] [i_4] |= arr_11 [i_1 - 1] [i_1 - 1] [i_1 - 1] [i_1] [i_1 + 1] [i_1 - 1] [i_1];
                                arr_14 [17] [i_1] [11] [i_1] [i_1] [i_1] = ((var_3 % (((((0 ? 1 : (arr_8 [i_3] [11] [i_3]))) != ((((arr_2 [i_0] [i_0]) / var_13))))))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_17 = var_4;
    #pragma endscop
}
