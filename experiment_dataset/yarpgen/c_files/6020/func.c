/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6020
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6020 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6020
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((((/* implicit */_Bool) ((unsigned int) ((((/* implicit */_Bool) 536870912U)) ? (var_11) : (var_9))))) ? (((((((/* implicit */_Bool) var_9)) ? (var_11) : (var_10))) % (var_10))) : (((((/* implicit */_Bool) ((((/* implicit */_Bool) var_7)) ? (var_12) : (var_0)))) ? (2982585999U) : ((-(var_8))))));
    /* LoopNest 2 */
    for (unsigned int i_0 = 1; i_0 < 9; i_0 += 4) 
    {
        for (unsigned int i_1 = 0; i_1 < 11; i_1 += 4) 
        {
            {
                arr_5 [i_0] [i_1] = 3758096362U;
                var_14 = var_0;
            }
        } 
    } 
}
