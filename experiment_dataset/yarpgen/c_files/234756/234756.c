/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234756
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234756 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234756
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_10 = ((~(((!(2838742311 | 7608135209033476461))))));
                    arr_8 [i_0] [i_1] [i_2] [i_2] = ((~((3315007157 ? 979960138 : (arr_5 [i_0] [i_0 - 1] [i_0 + 1])))));
                }
            }
        }
    }
    var_11 = ((7356912091640399873 ? 13957 : 3315007157));
    var_12 = ((var_5 ? var_7 : var_1));
    var_13 += 9936;
    #pragma endscop
}
