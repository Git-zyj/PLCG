/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 192501
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=192501 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/192501
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 12;i_3 += 1)
                    {
                        {
                            var_13 = (min(var_3, (var_7 ^ var_10)));
                            var_14 &= (((((((~(arr_5 [i_0] [i_1] [i_1]))) <= (((!(arr_7 [i_3] [i_1] [i_3]))))))) > ((max((arr_1 [i_3 - 1]), (arr_1 [i_3 - 1]))))));
                        }
                    }
                }
                var_15 ^= ((((arr_8 [i_0] [i_1] [i_1] [i_1]) < (arr_8 [i_0] [9] [i_0] [9]))) ? (((arr_1 [i_1]) | var_6)) : (min((arr_6 [i_0] [i_1] [i_1] [i_1]), (arr_1 [i_0]))));
            }
        }
    }
    var_16 *= ((min(var_8, var_3)));
    var_17 += var_12;
    #pragma endscop
}
