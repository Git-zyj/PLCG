/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120376
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120376 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120376
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            {
                var_20 = arr_2 [5];
                var_21 = (max(var_21, ((((min((((arr_4 [i_1]) >= (arr_3 [i_1] [i_1] [0]))), (arr_3 [3] [i_1 + 1] [i_1]))) && var_19)))));
                var_22 = -4;
                var_23 = (max(var_23, ((max(((var_10 ? ((max((arr_1 [i_0]), (arr_0 [i_0])))) : -var_0)), var_19)))));
                var_24 = ((1 == (379051809 > -39153053)));
            }
        }
    }
    var_25 = var_0;
    #pragma endscop
}
