/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160804
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160804 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160804
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_10 = (var_4 + var_2);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        {
                            var_11 = (min((~4094), (((arr_6 [i_3]) - (arr_6 [i_0])))));
                            arr_9 [i_0] [i_0] [i_0] [i_0] = var_3;
                        }
                    }
                }
                arr_10 [2] [i_0] = (min(124, 2300973856));
                arr_11 [i_1] [i_0] [i_0] = (arr_0 [i_0]);
            }
        }
    }
    var_12 *= (((((var_0 == var_5) >= var_3)) ? var_6 : var_5));
    var_13 = var_9;
    #pragma endscop
}
