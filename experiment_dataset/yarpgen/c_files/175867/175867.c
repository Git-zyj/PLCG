/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175867
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175867 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175867
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 += (min(var_11, (((var_11 > var_17) ^ ((max(var_8, 1)))))));

    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        var_21 = (((min((arr_0 [i_0 + 2] [i_0 - 1]), (arr_2 [i_0 + 2])))));
        var_22 = var_8;
        arr_3 [6] |= (min((arr_2 [i_0 - 1]), (min(((min((arr_0 [i_0] [i_0 + 1]), (arr_0 [i_0 + 2] [i_0])))), (((arr_2 [i_0]) ^ var_0))))));
        var_23 = var_10;
    }
    #pragma endscop
}
