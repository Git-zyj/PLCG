/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 121280
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=121280 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/121280
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_0] [0] = ((((max((((var_0 ? -1202250242 : var_10))), (((arr_4 [i_0] [i_0]) ? -1282067971 : (arr_3 [i_0] [i_0])))))) ? (min((arr_4 [i_0] [i_1]), (arr_4 [i_0] [i_0]))) : ((-4300 ? 1 : 4199))));
                var_20 = (((((((arr_1 [i_1]) || var_18)))) >= ((-1282067965 ? -4310 : 6815051639349575330))));
            }
        }
    }
    var_21 = var_16;
    #pragma endscop
}
