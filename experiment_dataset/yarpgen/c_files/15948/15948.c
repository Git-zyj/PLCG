/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15948
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15948 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15948
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = max(var_5, var_1);
    var_13 = var_8;

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        arr_4 [2] = (((34 >> (((var_8 | var_6) - 2732670887)))));
        arr_5 [i_0] = (((((((-(arr_3 [i_0] [i_0]))) < (((var_9 ? 9 : (arr_0 [i_0] [i_0]))))))) > (((!((max(118, 9))))))));
        arr_6 [i_0] = (arr_2 [i_0]);
        arr_7 [i_0] = (min((!var_9), ((((((arr_2 [i_0]) ? (arr_3 [i_0] [i_0]) : (arr_1 [i_0])))) ? ((((var_7 || (arr_0 [i_0] [i_0]))))) : ((var_3 ? (arr_1 [i_0]) : 0))))));
    }
    var_14 = var_7;
    #pragma endscop
}
