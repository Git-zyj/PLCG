/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68936
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68936 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68936
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_12;
    var_18 = ((((var_8 ? var_14 : (!var_7))) + (!var_13)));
    var_19 = var_11;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = (((var_1 % -1429370756) < ((((var_4 && (47507 || var_15)))))));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            {
                                arr_15 [4] [i_1] [i_0] [i_1] [i_0] = var_5;
                                var_20 *= (((!var_12) ? (var_8 / var_12) : (var_1 <= var_13)));
                                var_21 = var_11;
                            }
                        }
                    }
                }
                arr_16 [i_0] [10] &= (var_4 & var_10);
            }
        }
    }
    var_22 = var_14;
    #pragma endscop
}
