/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208865
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208865 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208865
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        arr_2 [i_0 + 3] = (arr_1 [i_0 + 2]);
        arr_3 [i_0] = (arr_0 [i_0 + 2] [i_0]);
        var_18 = (min((((arr_0 [1] [i_0]) ? var_10 : -17565)), (((arr_0 [i_0 + 2] [i_0 - 1]) ? (arr_0 [i_0 + 2] [i_0 + 3]) : (arr_1 [i_0 + 2])))));
    }
    var_19 = (((((((max(var_14, (-2147483647 - 1)))) ? -var_5 : ((-2138187980 ? var_2 : var_13))))) ? 3472547382 : var_0));
    var_20 += var_4;
    var_21 = ((~(var_2 | 274640689)));
    #pragma endscop
}
