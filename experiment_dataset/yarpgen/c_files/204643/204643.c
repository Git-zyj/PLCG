/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204643
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204643 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204643
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 |= ((var_8 ? (max(0, -122)) : ((var_2 ? 23 : var_2))));

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_11 = (min(var_11, ((min((((!((min(var_2, (arr_1 [i_0]))))))), ((-((((arr_0 [i_0]) || 1228478683))))))))));
        var_12 &= var_9;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 17;i_1 += 1)
    {
        var_13 = (max(var_13, ((((var_9 + 2147483647) << (var_7 - 1228732859003908516))))));
        var_14 = (min(var_14, (arr_3 [i_1])));
        var_15 = (max(var_15, (arr_2 [i_1])));
    }
    #pragma endscop
}
