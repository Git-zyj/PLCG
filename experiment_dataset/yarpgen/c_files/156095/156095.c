/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156095
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156095 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156095
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 12;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 15;i_4 += 1)
                        {
                            {
                                arr_13 [i_2] = var_11;
                                arr_14 [i_0 - 2] [i_1] [i_2] [i_1] [i_2] = (min(-5323459550013389571, (((0 ? ((var_8 * (arr_10 [i_2] [i_1 + 1] [i_2] [i_2] [i_4]))) : var_4)))));
                            }
                        }
                    }
                    arr_15 [i_2] [i_0] [i_2] [4] &= ((~(min(((max(-14923, var_10))), -8))));
                }
            }
        }
    }

    /* vectorizable */
    for (int i_5 = 1; i_5 < 13;i_5 += 1)
    {
        var_13 *= (((arr_16 [12]) * (arr_16 [0])));
        var_14 = (max(var_14, (arr_17 [6] [6])));
    }
    #pragma endscop
}
