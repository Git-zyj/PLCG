/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 161030
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=161030 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/161030
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    var_10 = max(251, ((var_3 ? (arr_5 [i_1]) : (arr_5 [i_1]))));
                    var_11 |= (~((((-3 ? 268435456 : -12)))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 21;i_4 += 1)
                        {
                            {
                                arr_14 [i_1] [i_0] [i_4] &= (((arr_1 [i_3]) >= (max((var_4 * var_6), (((1904397818 ? 3 : 1904397818)))))));
                                arr_15 [i_0] [i_0] [i_0] [i_0] [i_1] = ((-(((~4294967295) ? -var_9 : 1))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_12 = 3;
    var_13 = (~var_3);
    #pragma endscop
}
