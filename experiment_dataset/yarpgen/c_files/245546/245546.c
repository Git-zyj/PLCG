/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 245546
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=245546 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/245546
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0] = ((((max(((max(var_12, var_6))), (arr_2 [i_0] [i_0])))) ? (((arr_1 [i_0]) ? (arr_3 [i_0] [i_0] [7]) : (var_16 > var_3))) : ((max((!2402), ((var_5 ? 1 : (arr_0 [i_0]))))))));
                arr_6 [i_1] [i_0] [i_0] = max(((max((arr_0 [i_0]), (arr_1 [i_0])))), (((arr_3 [1] [i_1] [i_0]) % (~var_3))));
                arr_7 [14] [i_1] = ((+(((arr_4 [i_1]) ? var_6 : (arr_4 [i_1])))));
            }
        }
    }
    var_17 = var_6;
    var_18 = var_1;
    #pragma endscop
}
