/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52513
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52513 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52513
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (long long int i_0 = 0; i_0 < 12; i_0 += 3) 
    {
        var_13 = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) 744099513U))));
        arr_3 [i_0] = ((/* implicit */short) var_2);
        arr_4 [i_0] = ((/* implicit */int) ((unsigned char) ((((/* implicit */_Bool) var_10)) ? (((/* implicit */long long int) ((/* implicit */int) (short)-8042))) : (var_8))));
    }
    var_14 = ((/* implicit */short) max((var_14), (((/* implicit */short) ((unsigned char) var_2)))));
}
