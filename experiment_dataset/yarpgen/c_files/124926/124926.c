/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124926
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124926 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124926
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((((185 ? var_13 : 43)) < ((var_12 ? -var_11 : (~var_15)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_18 = (arr_2 [i_0] [i_0]);
                var_19 = (min(var_19, (((((min((arr_1 [i_1] [i_0]), var_9))) ? ((-((max(var_5, 65534))))) : ((max((arr_4 [i_1] [i_1]), (arr_4 [i_1] [i_1])))))))));
                var_20 = (arr_4 [i_0] [i_0]);
            }
        }
    }
    var_21 = ((1 ? (54561 * 35138) : 25));
    #pragma endscop
}
