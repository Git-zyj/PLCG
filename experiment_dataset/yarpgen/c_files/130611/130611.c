/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130611
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130611 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130611
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_16 |= (max(((var_14 ? ((((!(arr_0 [i_0] [i_0]))))) : var_5)), ((var_7 ? (arr_1 [i_0] [i_0]) : (arr_1 [i_0] [i_0])))));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_17 = (min(var_17, (((!((((arr_2 [i_2] [i_0]) ? 65535 : (arr_3 [i_0] [i_2])))))))));
                    var_18 = ((((!(arr_5 [i_2] [i_1] [i_0] [2]))) && ((((arr_2 [i_0] [i_2]) ? var_1 : (arr_2 [i_0] [i_2]))))));
                    var_19 = (+(max((arr_0 [i_2] [i_1]), (arr_0 [i_0] [i_0]))));
                }
            }
        }
    }
    var_20 = var_13;
    var_21 = var_11;
    #pragma endscop
}
