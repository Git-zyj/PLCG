/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34285
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34285 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34285
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_4;
    var_16 = ((!((max(37810, (min(var_11, 124)))))));
    var_17 = (((var_4 ^ -var_6) == ((max(var_2, var_14)))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] [i_1] = 106;
                arr_5 [18] [i_1] [18] = ((((min(((((arr_1 [i_0] [i_0]) >= 0))), var_10))) | (((82 << (124 - 103))))));
                arr_6 [8] [i_1] = (var_12 >= var_11);
                arr_7 [i_0] [i_0] [i_0] = ((-(((((min((arr_2 [i_0 + 2] [i_1] [19]), (arr_2 [i_0 + 3] [i_0 + 1] [i_0]))))) + (max(160, -5636555794220730850))))));
            }
        }
    }
    #pragma endscop
}
