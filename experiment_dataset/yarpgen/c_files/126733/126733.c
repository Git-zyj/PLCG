/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 126733
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=126733 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/126733
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (((((var_1 ? -var_4 : var_0))) ? (((!var_3) ? var_7 : (var_2 ^ var_9))) : (var_6 & var_0)));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_4 [i_0] [10] = ((((arr_3 [i_0] [20] [20]) >= ((~(arr_0 [i_0]))))));
                var_11 = ((-85 ? (arr_1 [i_0]) : (((((min((arr_2 [i_0] [i_1]), 42))) ? ((((arr_0 [i_0]) > -111))) : ((min((arr_3 [i_0] [i_0] [i_1]), -85))))))));
            }
        }
    }
    #pragma endscop
}
