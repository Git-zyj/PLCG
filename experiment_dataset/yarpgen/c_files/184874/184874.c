/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184874
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184874 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184874
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max(var_4, var_2));
    var_14 = ((((var_12 ? var_10 : var_12)) / -var_11));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_15 = (((-65520 ? (!87) : ((-2 ? var_11 : var_9)))));
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_16 = (min(var_16, ((((arr_11 [4] [i_1 - 1] [i_1] [i_1] [i_4 - 1] [i_1]) ? (max((((127 ? (arr_7 [i_2] [i_2] [i_3 - 1] [i_3 - 1]) : var_6))), var_0)) : (((-((min(var_8, var_4)))))))))));
                                var_17 = 11;
                            }
                        }
                    }
                }
            }
        }
    }
    var_18 ^= ((var_8 ? (((((var_0 ? 65522 : var_5))) ? var_3 : var_6)) : ((((max(1, 1)))))));
    #pragma endscop
}
