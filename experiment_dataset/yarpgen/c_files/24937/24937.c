/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24937
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24937 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24937
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((((((var_17 != var_17) > (min(var_1, 14384521531185292582))))) ^ ((min(195, (var_9 <= var_4))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_19 -= arr_0 [i_0];
                var_20 = var_6;
                var_21 += (min((arr_3 [i_0] [i_0] [i_1]), (((!(arr_2 [2] [i_0]))))));
                var_22 = (((((14384521531185292560 || (73 != 14384521531185292551))))) + ((min(15120543501389978070, var_0))));
            }
        }
    }
    var_23 = (max(var_0, (~var_6)));
    #pragma endscop
}
