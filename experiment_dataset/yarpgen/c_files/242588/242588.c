/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242588
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242588 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242588
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_13;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        arr_2 [i_0] = (1107 * var_8);
        arr_3 [i_0] = (arr_1 [i_0]);
        var_21 = ((-88 / (arr_0 [i_0])));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 22;i_1 += 1)
    {
        arr_7 [i_1] [i_1] = (((arr_6 [i_1]) << (((arr_6 [i_1]) - 42))));
        var_22 = (((((((arr_6 [i_1]) >= (arr_5 [i_1]))))) - var_4));
        arr_8 [i_1] [i_1] &= ((var_2 || 97) <= (arr_5 [i_1]));
        var_23 = (min(var_23, 11327));
    }
    var_24 = ((!(((var_6 ? -75 : (!var_18))))));
    #pragma endscop
}
