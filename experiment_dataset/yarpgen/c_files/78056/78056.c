/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78056
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78056 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78056
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((((-9185 ? (-1950969627521520395 & 9170) : (-3606059006246898570 - 1))) ^ (((var_13 ? ((var_10 ? -46 : var_14)) : (~var_15))))));
    var_18 = var_4;
    var_19 = (min(var_19, var_12));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 19;i_2 += 1)
            {
                {
                    var_20 = var_5;
                    var_21 = (((((169 << (3524238976 - 3524238975))))) || 281474976710655);
                    arr_9 [i_0] [i_0] [i_1] [i_2] = (((((1094800320 & (var_14 == 226)))) ? ((var_15 ? (((202 ? var_9 : 555802985))) : var_14)) : (((((var_7 ? 1 : var_5)))))));
                }
            }
        }
    }
    var_22 = var_15;
    #pragma endscop
}
