/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33631
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33631 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33631
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] = (arr_1 [i_0 - 2]);
                arr_6 [i_0] [1] = ((-(234881024 && -1067465157)));

                /* vectorizable */
                for (int i_2 = 3; i_2 < 13;i_2 += 1)
                {
                    var_11 = ((92 | (arr_9 [i_0 - 2] [i_0 - 1] [i_2 + 1])));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                var_12 = (max(var_12, (4060086266 <= var_9)));
                                arr_15 [i_0] [i_0] [i_2] [i_3] [i_2] = (((arr_8 [i_0] [i_2]) ? 23756 : (32766 && 32751)));
                                arr_16 [i_0 - 1] [i_2] [i_2] = (var_5 * 2182353095);
                            }
                        }
                    }
                }
            }
        }
    }
    var_13 = ((((max(var_4, var_3)) ? var_6 : var_7)));
    #pragma endscop
}
