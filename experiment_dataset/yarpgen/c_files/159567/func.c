/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159567
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159567 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159567
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 += (unsigned short)63669;
    /* LoopNest 3 */
    for (short i_0 = 0; i_0 < 21; i_0 += 4) 
    {
        for (int i_1 = 2; i_1 < 19; i_1 += 4) 
        {
            for (long long int i_2 = 1; i_2 < 20; i_2 += 4) 
            {
                {
                    var_18 = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) (-(((/* implicit */int) var_1)))))));
                    arr_9 [i_0] [i_2] [i_0] [i_0] = ((/* implicit */unsigned char) (short)-10972);
                    var_19 = ((/* implicit */unsigned int) max((var_19), (((/* implicit */unsigned int) (-(((arr_0 [i_0] [i_1]) ? (((/* implicit */int) var_3)) : (((/* implicit */int) arr_3 [i_0] [i_1 - 2])))))))));
                }
            } 
        } 
    } 
    var_20 = ((/* implicit */unsigned int) (~(var_12)));
    var_21 = ((/* implicit */long long int) var_2);
}
