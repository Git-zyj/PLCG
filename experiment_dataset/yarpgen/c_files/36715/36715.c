/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36715
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36715 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36715
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            {
                var_11 = (max(var_11, (((!((((((325735516 ? 288230375614840832 : (arr_3 [i_0] [i_0] [6])))) ? (arr_2 [i_0]) : var_4))))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 20;i_3 += 1)
                    {
                        {
                            var_12 = (--102);
                            arr_12 [i_0] [i_1] [i_0] = (arr_8 [i_0 - 1] [4]);
                            arr_13 [i_0] [18] [i_1] [5] = (((((var_10 != (arr_7 [i_0 - 2] [i_2] [i_3] [i_3])))) - (arr_6 [i_1] [i_0] [i_1] [i_1])));
                        }
                    }
                }
            }
        }
    }
    var_13 = var_8;
    #pragma endscop
}
