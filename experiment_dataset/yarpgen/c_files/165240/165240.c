/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165240
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165240 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165240
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 *= var_9;
    var_12 |= ((((min((max(var_2, var_6)), ((-7980499610515819267 ? var_6 : 98))))) ? ((var_6 ? var_4 : (var_8 * 1))) : 1));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    var_13 = (min(var_13, var_0));
                    arr_9 [i_0] [i_0] [5] [i_2] = (((((-(arr_2 [i_0])))) ? (min(157, (arr_2 [i_0]))) : (arr_2 [i_0])));
                    var_14 = (((((-((max((arr_3 [i_0] [i_0]), var_3)))))) ? (7980499610515819267 | 157) : ((-(((arr_0 [15]) ? (arr_4 [i_0] [i_1] [i_2]) : 0))))));
                }
            }
        }
    }
    #pragma endscop
}
