/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105640
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105640 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105640
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            {
                var_20 = (max((((arr_1 [i_0] [i_0]) ? 5763879953828212227 : (arr_1 [i_0] [i_0]))), (-1 * var_8)));
                var_21 = (min((arr_1 [i_0] [i_0]), (arr_5 [i_0] [i_1])));
                arr_6 [i_0] [i_0] = (((var_8 & 255) ^ (arr_3 [10] [i_1 - 1])));
            }
        }
    }
    var_22 = (min(var_22, ((min(((var_6 != ((min(var_9, var_12))))), ((!((min(var_15, 255))))))))));
    var_23 ^= ((!(!24)));
    #pragma endscop
}
