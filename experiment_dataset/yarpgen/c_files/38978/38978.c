/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38978
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38978 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38978
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 += (-9223372036854775807 - 1);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 15;i_3 += 1)
                    {
                        {
                            var_20 = ((~(max(var_15, 1984))));
                            var_21 = (((min(((var_0 ? (arr_3 [i_2] [i_1] [i_0]) : var_2)), (-1 / var_11))) % 35498));
                            var_22 = ((((max((3166529346 < var_17), 47711))) % ((((((min(-126, 1))) + 2147483647)) << (23326 - 23326)))));
                            var_23 = (min(var_23, -45));
                        }
                    }
                }
                var_24 = (max(var_24, var_17));
                var_25 = (max((21931 || 10890), ((144115188075855871 && (arr_7 [i_0])))));
                var_26 = ((min(((((arr_6 [i_1]) % (arr_7 [i_0])))), ((var_10 & (arr_10 [i_0] [i_1] [i_0]))))) >= (min((arr_7 [i_0]), (arr_5 [i_0]))));
            }
        }
    }
    #pragma endscop
}
