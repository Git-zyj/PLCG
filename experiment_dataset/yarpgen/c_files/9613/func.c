/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 9613
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=9613 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/9613
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 3 */
    /* vectorizable */
    for (int i_0 = 0; i_0 < 17; i_0 += 3) /* same iter space */
    {
        arr_3 [i_0] [i_0] = 0U;
        var_14 = 887947674;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 17; i_1 += 3) /* same iter space */
    {
        var_15 = 17981881845937697721ULL;
        arr_6 [i_1] = ((/* implicit */int) (unsigned char)145);
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 17; i_2 += 3) /* same iter space */
    {
        /* LoopSeq 1 */
        for (unsigned long long int i_3 = 0; i_3 < 17; i_3 += 2) 
        {
            arr_11 [i_2] = ((/* implicit */int) (unsigned char)146);
            arr_12 [i_2] [i_3] = ((/* implicit */unsigned int) (unsigned char)146);
        }
        arr_13 [i_2] [i_2] = 1509192566;
    }
    var_16 = ((/* implicit */int) var_4);
}
