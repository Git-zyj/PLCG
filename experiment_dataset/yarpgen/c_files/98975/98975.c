/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98975
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98975 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98975
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_10;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_2] = ((((((arr_2 [i_0]) && (arr_2 [i_0])))) >> (((min((arr_4 [i_0] [i_0]), (arr_4 [i_1] [i_2]))) - 1576080391))));
                    arr_10 [i_0] [i_0] [i_0] [i_0] = ((((((0 - (arr_0 [i_1] [i_0]))) + 2147483647)) >> (96 - 84)));
                    arr_11 [i_0] [i_1] [i_1] = (arr_8 [i_0] [i_1] [i_0] [i_0]);
                }
            }
        }
    }
    var_14 = ((-var_11 < (max((max(var_7, var_3)), 703018328))));
    var_15 = var_3;
    #pragma endscop
}
