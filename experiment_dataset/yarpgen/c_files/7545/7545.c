/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7545
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7545 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7545
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_16 = (((-14 ? 1 : -14)));
                var_17 = ((~((~(arr_2 [i_1])))));
                /* LoopNest 3 */
                for (int i_2 = 2; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 17;i_4 += 1)
                        {
                            {
                                var_18 = ((!((((arr_8 [i_0] [i_0] [i_0]) | (arr_9 [i_4 + 2]))))));
                                var_19 = (!35511);
                                arr_16 [i_0] [i_1] [i_2] = (((arr_6 [i_0] [i_1]) ? ((var_4 ? 23080 : (arr_8 [i_4 - 1] [i_1] [i_1]))) : -1908993469));
                            }
                        }
                    }
                }
                arr_17 [i_0] [i_1] = (((((arr_7 [i_1] [i_1] [i_0]) && (arr_7 [i_0] [1] [i_1]))) && (arr_7 [i_1] [i_1] [i_1])));
            }
        }
    }
    var_20 |= ((var_2 ? (~var_0) : var_1));
    #pragma endscop
}
