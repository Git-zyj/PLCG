/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110111
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110111 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110111
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((((max(((max(-32748, -51))), (min(var_1, var_5))))) ? var_9 : var_10));

    for (int i_0 = 1; i_0 < 22;i_0 += 1) /* same iter space */
    {
        var_17 &= var_14;
        var_18 = ((var_9 ? ((((((arr_0 [i_0 + 1]) ? var_6 : (arr_0 [i_0])))) ? var_0 : (arr_0 [i_0]))) : (arr_1 [i_0] [i_0])));
        var_19 = (((min(((max((arr_1 [i_0 + 1] [i_0]), var_6))), (~40))) & (arr_1 [i_0 + 1] [i_0 + 1])));
    }
    for (int i_1 = 1; i_1 < 22;i_1 += 1) /* same iter space */
    {
        var_20 = ((-((var_0 ? (arr_3 [i_1]) : (arr_3 [i_1 + 1])))));
        arr_4 [i_1] = ((-((min(40, -22)))));
    }
    #pragma endscop
}
