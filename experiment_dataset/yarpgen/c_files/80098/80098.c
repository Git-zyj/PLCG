/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80098
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80098 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80098
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_12 = var_4;
        arr_3 [i_0] = ((var_6 ? ((min(var_4, var_4))) : var_10));
        var_13 *= (min((((var_0 ? var_1 : var_2))), ((min(-var_3, ((var_6 ? var_7 : var_7)))))));
    }
    for (int i_1 = 2; i_1 < 13;i_1 += 1)
    {
        var_14 -= (-((var_7 ? var_5 : (var_1 | var_2))));
        arr_6 [i_1] = (((var_5 ? var_2 : var_4)));
    }
    var_15 *= ((+(max((!var_3), -var_10))));
    var_16 = ((-(min(((var_6 ? var_11 : var_8)), (max(var_10, var_7))))));
    var_17 = max(((var_6 ? var_11 : -75)), var_3);
    #pragma endscop
}
