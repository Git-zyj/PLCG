/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21309
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21309 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21309
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max((((var_3 ? ((min(1, var_2))) : (1 || 6904121597945679704)))), (max(var_8, 4294967295))));
    var_11 = (--202);
    var_12 = ((-((((var_7 == var_4) < var_8)))));
    var_13 = max(var_0, var_2);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_14 = (min(var_6, (arr_5 [i_0])));
                var_15 -= (((arr_3 [i_0] [i_0]) ? (min(7507513053602479342, (~var_8))) : var_2));
            }
        }
    }
    #pragma endscop
}
