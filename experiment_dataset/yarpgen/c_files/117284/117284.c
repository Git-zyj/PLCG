/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117284
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117284 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117284
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        var_15 ^= ((((min((var_0 / -12401), (arr_1 [i_0])))) ? var_10 : (min((arr_0 [i_0]), (arr_1 [i_0])))));
        var_16 = 11757220466981615487;
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 21;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 23;i_3 += 1)
                {
                    {
                        var_17 = (~var_14);
                        var_18 -= (~-12401);
                        var_19 |= arr_8 [i_1] [i_1] [i_2] [i_3 + 1];
                        var_20 = (min(var_20, (arr_2 [i_3] [i_1])));
                    }
                }
            }
        }
        var_21 = (max(var_21, (((var_11 ? var_8 : var_11)))));
        arr_11 [i_0] [i_0] = ((!(arr_8 [i_0] [3] [i_0] [3])));
    }
    var_22 = (!1);
    #pragma endscop
}
