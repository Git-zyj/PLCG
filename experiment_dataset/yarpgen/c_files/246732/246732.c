/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246732
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246732 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246732
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (((var_12 / 9062987338769767904) < (((1 >> (((min(-304126318, 6036268283344048822)) - 6036268283344048793)))))));
    var_15 = var_10;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            {
                var_16 = (arr_3 [i_1 + 1] [i_1 - 1]);
                var_17 = (arr_0 [i_0]);
                var_18 ^= (min((((arr_2 [i_0 - 1] [i_1] [i_0 - 1]) * (arr_2 [i_0 + 2] [i_0 + 2] [i_1]))), (arr_2 [i_0] [i_1] [i_0])));
                var_19 = (min(var_19, ((((~-32759) <= var_8)))));
            }
        }
    }
    #pragma endscop
}
