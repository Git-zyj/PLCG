/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99208
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99208 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99208
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 7;i_0 += 1)
    {
        arr_3 [i_0] = ((-(min((arr_1 [i_0]), (arr_0 [i_0] [4])))));
        var_20 = -var_12;
        arr_4 [i_0] = (min((var_9 <= 134217728), (((arr_2 [i_0 + 3]) ? (arr_2 [i_0 + 2]) : -3601174568233662909))));
    }
    var_21 = var_10;
    var_22 = (var_3 | var_17);
    var_23 += (min(((((((var_11 ? 9223372036854775807 : var_12))) < var_17))), (((0 > -5065) * ((var_15 ? var_8 : var_6))))));
    #pragma endscop
}
