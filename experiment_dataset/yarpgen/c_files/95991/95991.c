/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95991
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95991 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95991
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_14 = (arr_0 [12]);
                var_15 = ((((((((-628079018 ? 65530 : 598855629))) ? (((arr_4 [i_0]) - 177)) : (arr_1 [i_0] [i_0])))) ? (arr_3 [i_0] [i_0]) : (((arr_2 [i_0] [i_0] [i_0]) ? 5 : (((arr_0 [i_0]) ? (arr_1 [i_1] [i_0]) : var_5))))));
                var_16 |= (((arr_0 [i_1]) + (arr_1 [5] [20])));
            }
        }
    }
    var_17 = ((var_12 ? (var_13 == var_13) : -243));
    var_18 = (!var_9);
    #pragma endscop
}
