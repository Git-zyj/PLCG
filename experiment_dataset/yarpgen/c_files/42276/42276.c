/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 42276
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=42276 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/42276
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 17;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1] [i_1] [i_2] = (((((((-(arr_4 [i_1] [i_1]))) == (((((arr_4 [i_0] [i_1]) && 1))))))) * (173 <= -0)));
                    var_12 = 8568227;
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_3 = 0; i_3 < 16;i_3 += 1)
    {
        for (int i_4 = 2; i_4 < 13;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 0;i_5 += 1)
            {
                {
                    var_13 = (((((((arr_5 [i_3] [i_4] [i_5]) / 173)))) ? (((arr_0 [i_3]) * (((-(arr_7 [i_4])))))) : var_11));
                    arr_17 [i_3] [i_4] [i_5] = ((~((-(arr_16 [i_5 + 1] [i_4 - 2] [i_4 + 1] [i_3])))));
                }
            }
        }
    }
    #pragma endscop
}
