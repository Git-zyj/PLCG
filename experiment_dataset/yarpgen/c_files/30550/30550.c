/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30550
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30550 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30550
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((var_12 * (max(((var_8 ? 178 : var_13)), var_12))));
    var_16 = (max(var_16, var_0));
    var_17 ^= var_4;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        var_18 += (~var_1);
        var_19 = ((((arr_0 [1]) ? 1316504169771166368 : (arr_0 [i_0]))) / (arr_1 [i_0]));
        var_20 = (((arr_1 [i_0]) / var_1));
    }
    var_21 = (max(var_21, ((var_7 >> (-var_4 <= var_5)))));
    #pragma endscop
}
