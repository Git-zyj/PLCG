/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15275
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15275 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15275
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */long long int) var_8);
    /* LoopNest 2 */
    for (unsigned char i_0 = 0; i_0 < 12; i_0 += 2) 
    {
        for (short i_1 = 2; i_1 < 11; i_1 += 2) 
        {
            {
                var_12 = ((/* implicit */signed char) 576459652791795712ULL);
                arr_5 [i_1] = ((/* implicit */unsigned long long int) arr_2 [i_0]);
            }
        } 
    } 
    var_13 = ((/* implicit */unsigned long long int) min((var_13), ((~(((1441648530394482205ULL) + (((/* implicit */unsigned long long int) ((/* implicit */int) max((((/* implicit */unsigned short) var_10)), ((unsigned short)65535)))))))))));
}
