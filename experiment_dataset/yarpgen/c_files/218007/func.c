/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218007
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218007 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218007
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */_Bool) ((((/* implicit */_Bool) ((((/* implicit */long long int) (~(((/* implicit */int) (short)23295))))) % (var_1)))) ? (((/* implicit */int) var_6)) : (((/* implicit */int) var_14))));
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 0; i_0 < 22; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 22; i_1 += 1) 
        {
            for (long long int i_2 = 0; i_2 < 22; i_2 += 4) 
            {
                {
                    var_17 = ((/* implicit */int) 24ULL);
                    arr_9 [i_2] |= ((/* implicit */long long int) var_8);
                }
            } 
        } 
    } 
    var_18 = ((/* implicit */long long int) var_7);
}
