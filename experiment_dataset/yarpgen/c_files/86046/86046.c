/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86046
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86046 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86046
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_16, ((min(((min(var_4, (min(var_10, var_9))))), ((7 ? 1 : 65529)))))));

    for (int i_0 = 2; i_0 < 19;i_0 += 1) /* same iter space */
    {
        var_17 ^= (((arr_0 [i_0]) >= var_3));
        arr_2 [i_0] = 2147483630;
    }
    for (int i_1 = 2; i_1 < 19;i_1 += 1) /* same iter space */
    {
        arr_5 [i_1] [1] = (arr_0 [i_1]);
        arr_6 [i_1] = (((arr_0 [19]) ? ((((min((arr_1 [i_1]), var_1))) ? -var_5 : (!var_2))) : ((((~var_15) ? (arr_3 [i_1 - 1] [i_1 + 1]) : (~var_9))))));
        arr_7 [9] = (((max(var_9, ((65527 ? (arr_3 [i_1] [2]) : (arr_4 [8]))))) / (max((min(22, var_15)), ((~(arr_3 [i_1 - 2] [i_1])))))));
    }
    var_18 = ((var_6 ? var_2 : ((var_6 ? ((var_7 ? var_5 : var_9)) : (var_4 - var_2)))));
    #pragma endscop
}
