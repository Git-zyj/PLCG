/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155064
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155064 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155064
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 7;i_1 += 1)
        {
            {
                arr_8 [i_0] [i_1] = (min((((arr_4 [i_0 + 1]) ? (arr_4 [i_0 + 1]) : (arr_4 [i_0 - 1]))), ((min((arr_4 [i_0 + 2]), (arr_4 [i_0 + 4]))))));
                arr_9 [i_1] [0] = var_3;
            }
        }
    }
    var_15 = (min(((var_13 ? (((var_11 ? var_11 : var_5))) : (var_6 ^ var_8))), (min(((var_13 ? var_1 : var_14)), (((var_2 ? var_8 : 962176430)))))));
    #pragma endscop
}
