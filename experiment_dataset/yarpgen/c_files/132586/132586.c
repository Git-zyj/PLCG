/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132586
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132586 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132586
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((!(((1023 & var_14) > var_2))));

    for (int i_0 = 2; i_0 < 14;i_0 += 1) /* same iter space */
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 16;i_2 += 1)
            {
                {
                    arr_8 [i_1] [i_1] [i_1] [i_2] = (arr_5 [i_1]);
                    arr_9 [i_1] [i_1] [i_1] [i_2] = arr_6 [i_0] [i_0] [i_0] [9];
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 15;i_4 += 1)
                        {
                            {
                                var_18 = (-32767 - 1);
                                var_19 = (arr_3 [i_1] [i_1] [i_1]);
                                var_20 = (arr_6 [i_0] [i_1] [i_2 - 2] [i_3]);
                            }
                        }
                    }
                    arr_15 [i_1] [i_1] [i_2 - 1] [i_2 - 1] = -2079086360;
                }
            }
        }
        arr_16 [i_0] = ((((((-750720533 || (arr_2 [i_0]))))) / (arr_0 [i_0])));
        var_21 = -1;
    }
    /* vectorizable */
    for (int i_5 = 2; i_5 < 14;i_5 += 1) /* same iter space */
    {
        var_22 *= (((arr_2 [i_5 + 2]) / (arr_2 [i_5 + 3])));
        arr_20 [i_5 + 2] [i_5] = ((!((((arr_18 [i_5 + 3]) ^ 20269)))));
    }
    var_23 = (((-(var_10 % 92))));
    #pragma endscop
}
