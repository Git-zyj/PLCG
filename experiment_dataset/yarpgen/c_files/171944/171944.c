/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171944
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171944 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171944
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_4;
    var_15 -= ((536870912 ? var_13 : 1));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_16 = ((!(((((arr_1 [i_0]) ? 0 : (arr_1 [i_0])))))));
                var_17 = (((((-(((arr_1 [i_1]) ? 3426058704 : (arr_3 [i_1] [i_1] [13])))))) ? (arr_3 [i_0] [i_1] [i_1]) : 2147483640));
            }
        }
    }
    var_18 += 64372;
    var_19 = (((var_12 >> (var_0 - 26015))));
    #pragma endscop
}
