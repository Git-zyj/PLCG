/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136090
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136090 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136090
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = 8916820144316090380;

    /* vectorizable */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        arr_2 [i_0 - 1] [i_0] = (arr_0 [i_0]);
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_20 ^= ((((1 >= (arr_6 [i_0] [i_1] [i_2]))) ? ((254 ? 28 : 37527)) : (arr_5 [i_0 - 2] [i_0] [i_0 + 1])));
                    var_21 = ((-((~(arr_5 [i_2] [i_1] [i_0])))));
                    var_22 = ((-((15 ? 32757 : var_13))));
                }
            }
        }
        var_23 = (!13421997884603235022);
    }
    var_24 = 6756311735341557404;
    var_25 = (var_7 > 11139);
    #pragma endscop
}
