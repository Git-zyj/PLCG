/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95332
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95332 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95332
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 += ((((-((var_3 ? var_7 : var_7))))) ? var_17 : ((~(~var_14))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    var_19 = ((-(max(0, (arr_0 [i_0] [i_1 + 2])))));
                    var_20 = (max(var_20, (((((max(7, (arr_8 [i_2] [i_0])))) ? (((max((arr_5 [i_0] [i_0] [i_0]), (arr_7 [i_0] [i_1 + 3] [i_2]))))) : var_4)))));
                }
            }
        }
    }
    var_21 |= var_0;
    #pragma endscop
}
