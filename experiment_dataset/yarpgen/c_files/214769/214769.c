/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214769
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214769 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214769
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (~106);
    var_15 = (min(var_15, 18446744073709551607));
    var_16 = (max((((((18446744073709551613 - (-127 - 1)))) ? ((var_5 ? var_13 : var_7)) : ((max(var_10, var_2))))), var_8));
    var_17 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 23;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] |= ((((((arr_3 [i_1 - 3] [i_1 - 3] [i_1]) > (arr_1 [i_1 + 1] [i_1 + 1])))) == ((-(arr_1 [i_0] [i_1 + 1])))));
                var_18 = (arr_2 [i_1 - 1]);
            }
        }
    }
    #pragma endscop
}
