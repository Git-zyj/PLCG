/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11473
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11473 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11473
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((-(-1 != var_10)));
    var_17 ^= ((((min((((var_15 ? 224 : var_1))), (max(var_4, var_2))))) && (((min(1, 4294967289))))));
    var_18 = (min(var_18, var_7));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 21;i_1 += 1)
        {
            {
                var_19 = min((max((((-(arr_2 [i_0])))), (max(1, var_3)))), ((((((arr_1 [i_0]) ? 688230302 : var_15)) & (arr_2 [i_1])))));
                var_20 = (((arr_0 [i_0]) > (min(((var_14 | (arr_2 [i_0]))), 1))));
            }
        }
    }
    #pragma endscop
}
