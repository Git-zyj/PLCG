/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209211
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209211 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209211
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_18 -= max(var_15, ((~(arr_0 [i_1]))));
                var_19 = (min(var_19, ((((((((var_12 ? var_7 : var_14))) ? (max(32512, 18446744073709551600)) : (min(0, 18446744073709551615)))) <= var_8)))));
            }
        }
    }
    var_20 = ((var_9 ? ((-5639873238012236289 ? 18446744073709551615 : 60646)) : var_10));
    var_21 |= (min(var_17, var_15));
    #pragma endscop
}
