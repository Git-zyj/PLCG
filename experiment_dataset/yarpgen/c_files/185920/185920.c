/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 185920
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=185920 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/185920
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_12 = (max(var_6, (arr_2 [i_0])));
        arr_3 [i_0] = var_1;
        var_13 = (((((arr_2 [i_0]) | ((((0 < (arr_2 [i_0]))))))) / 1));
    }
    for (int i_1 = 0; i_1 < 16;i_1 += 1)
    {
        arr_8 [i_1] = 4294967295;
        var_14 = (arr_2 [i_1]);
        /* LoopNest 3 */
        for (int i_2 = 0; i_2 < 16;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 16;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 16;i_4 += 1)
                {
                    {
                        var_15 = ((var_6 > ((((var_5 != (1 / 8323072)))))));

                        for (int i_5 = 1; i_5 < 15;i_5 += 1)
                        {
                            arr_20 [i_1] [i_1] [i_1] [1] [i_4] [1] [i_1] = ((~(~8323072)));
                            var_16 = (-(arr_14 [i_2] [i_2] [i_4] [i_5 + 1]));
                        }
                    }
                }
            }
        }
    }

    for (int i_6 = 1; i_6 < 19;i_6 += 1)
    {
        var_17 = ((var_1 ? var_6 : (((-(~-1878966452))))));
        var_18 ^= (((!((54719 && (arr_21 [i_6]))))));
    }
    #pragma endscop
}
