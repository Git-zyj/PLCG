/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 96638
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=96638 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/96638
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 ^= (max(-108, ((var_8 ? var_12 : ((var_8 ? var_10 : 127))))));

    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        var_15 = ((-((-(arr_2 [i_0] [i_0 + 1])))));
        var_16 -= (arr_1 [i_0 + 1]);
    }
    var_17 = ((var_7 ? (-119 & 33822) : 3024061829));

    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        var_18 = (min(var_18, var_4));
        arr_6 [i_1] = ((-((995206080 >> (-1 + 17)))));
    }
    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        var_19 = (max((arr_8 [i_2]), (((var_1 << (var_0 - 171))))));
        var_20 = (((-2147483647 | 1597001308864762223) ? 50373 : (2368618535 || 1)));
    }
    #pragma endscop
}
