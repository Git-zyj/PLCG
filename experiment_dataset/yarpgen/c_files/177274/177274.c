/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177274
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177274 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177274
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_3;
    var_12 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [i_1] [i_0] = (min((min((arr_3 [i_2]), (arr_1 [i_0]))), (arr_3 [i_0])));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 17;i_4 += 1)
                        {
                            {
                                var_13 = ((max(229, 2395459990)));
                                arr_15 [i_4] [i_3] [i_1] [i_2] [i_1] [i_0] [i_0] = 0;
                                arr_16 [i_0] [i_1] [i_1] [i_0] [i_2] = (arr_8 [i_0] [i_0] [i_0] [i_1]);
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
