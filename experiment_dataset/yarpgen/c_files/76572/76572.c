/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76572
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76572 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76572
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 22;i_0 += 1)
    {
        arr_3 [i_0 - 1] = (((((arr_1 [i_0 + 1]) ? var_15 : var_2))));
        arr_4 [i_0 - 3] = ((~(arr_0 [i_0 - 1])));
    }
    /* LoopNest 2 */
    for (int i_1 = 2; i_1 < 21;i_1 += 1)
    {
        for (int i_2 = 1; i_2 < 22;i_2 += 1)
        {
            {
                var_20 = (max(var_20, ((((min((arr_2 [i_2 + 1] [i_2 + 1]), (arr_7 [i_2 - 1] [i_2 - 1]))) == ((((!(arr_2 [i_2 - 1] [i_2 - 1]))))))))));
                var_21 = (max(var_21, 7762));
            }
        }
    }
    var_22 = (((var_14 || var_15) | var_0));
    #pragma endscop
}
