/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44212
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44212 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44212
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((var_2 ? var_8 : var_2));
    var_13 = (min(var_13, var_7));
    var_14 = (((((var_4 + var_0) ? 8014520870917780688 : (~var_6)))) ? var_1 : ((1 ? -2 : 8014520870917780688)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    var_15 = 7;
                    arr_9 [i_0] [i_1] [i_1] [i_0] = (((((!((max((arr_4 [i_2] [i_1]), (arr_1 [i_0]))))))) != ((~(((arr_8 [14] [i_1] [i_1] [i_2]) ? var_0 : (arr_2 [i_1])))))));
                }
            }
        }
    }
    var_16 = ((((-(var_7 / var_8)))) ? (max(113, ((max(-32279, var_2))))) : (((2 ? 611246674 : -6751263135694397979))));
    #pragma endscop
}
