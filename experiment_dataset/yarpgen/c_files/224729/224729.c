/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224729
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224729 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224729
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((((var_7 >= ((min(var_7, var_11))))) ? var_13 : ((var_14 ? -1 : (~var_7)))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_6 [i_0] [8] [i_1] = ((((min(((min((arr_1 [i_0]), (arr_1 [i_0])))), (max(var_11, 7802847681456913631))))) ? 7460995894704977277 : (((arr_2 [11] [i_0 + 3] [i_0 + 4]) ? (arr_4 [i_0 - 1] [i_0 - 1]) : 32))));
                var_17 -= (!var_7);
                var_18 = ((!(!var_0)));
            }
        }
    }
    var_19 = var_6;
    #pragma endscop
}
