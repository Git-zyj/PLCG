/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199583
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199583 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199583
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 4; i_0 < 17;i_0 += 1)
    {
        var_16 = (max((~60044), (arr_1 [i_0 - 4] [i_0 - 4])));
        arr_2 [i_0] = var_14;
        var_17 = (max(var_17, (min((var_1 + 60033), ((((-(arr_0 [i_0])))))))));
    }
    var_18 = (max(var_18, ((((var_0 ? (var_1 ^ 22624) : var_15)) / var_4))));
    var_19 = ((((var_12 ? ((var_4 ? var_11 : var_10)) : ((var_13 ? var_14 : var_9)))) | ((((var_8 <= (~var_11)))))));
    #pragma endscop
}
