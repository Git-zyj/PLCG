/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151671
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151671 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151671
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 23;i_2 += 1)
            {
                {
                    arr_6 [i_2] [i_0] [i_0 - 1] = 200;
                    var_16 = max((-2147483647 - 1), (min(68715282432, (arr_2 [i_0 - 1] [i_0 - 1]))));
                    var_17 |= var_6;
                    var_18 += ((min(var_4, (arr_1 [i_0 - 1]))));
                    arr_7 [i_0] [11] [i_1] [i_0] = (((arr_3 [i_0] [i_2]) ? (arr_3 [i_0] [i_1]) : (max((((var_11 ? (arr_2 [i_0 - 1] [i_2]) : (arr_5 [i_1] [i_0])))), var_11))));
                }
            }
        }
    }
    var_19 = (!-16384);
    #pragma endscop
}
