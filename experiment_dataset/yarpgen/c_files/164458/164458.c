/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164458
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164458 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164458
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [i_0] = (((((0 << (((min(207629296, 10763798901784984653)) - 207629271))))) ? (arr_4 [i_2]) : (min(2, -15))));
                    arr_7 [i_0] = ((((max(-114, 143))) & (((!(arr_5 [i_0] [i_2 - 1] [i_2 - 1]))))));
                    var_20 = ((0 ? 1613738156988783631 : 159));
                }
            }
        }
    }
    var_21 = (min(var_21, ((min(((((var_11 <= var_12) / (~var_18)))), (max(var_19, -var_11)))))));
    #pragma endscop
}
