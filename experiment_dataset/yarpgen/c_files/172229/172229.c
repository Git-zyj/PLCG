/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172229
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172229 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172229
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (!var_9);

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        var_11 = (((arr_1 [i_0] [i_0]) ? (((!((max(var_2, 0)))))) : (arr_0 [i_0])));
        arr_3 [i_0] = (max((((+(((arr_1 [i_0] [i_0]) >> 1))))), ((550038406801621472 >> (-10699 + 10700)))));
        var_12 = ((11194229052640522620 ? (274877841408 || 0) : (arr_0 [i_0])));
    }
    #pragma endscop
}
