/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152962
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152962 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152962
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned char) (+(((/* implicit */int) (unsigned char)255))));
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (short i_1 = 4; i_1 < 16; i_1 += 1) 
        {
            {
                arr_5 [i_1] [(short)14] [i_1] = ((/* implicit */unsigned int) ((((/* implicit */_Bool) var_0)) ? (((/* implicit */int) var_5)) : (((/* implicit */int) arr_0 [i_1] [i_1]))));
                var_13 -= ((/* implicit */unsigned char) max((arr_4 [i_0] [i_0] [i_0]), (((/* implicit */long long int) (unsigned char)10))));
            }
        } 
    } 
    var_14 = ((short) ((((/* implicit */_Bool) (unsigned short)0)) ? (((/* implicit */int) var_7)) : (((/* implicit */int) var_9))));
}
