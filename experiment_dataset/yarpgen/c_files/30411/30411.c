/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30411
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30411 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30411
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((((var_5 || var_0) ? (((var_7 ? -9 : 15))) : 16053832630889719140)) >> ((((-(var_3 - 99))) - 48))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] [i_1] [i_2] = (arr_5 [i_0] [i_0] [i_1] [i_0]);
                    arr_9 [i_1] [i_1] = ((max(-2, (arr_1 [i_1 - 1]))));
                    arr_10 [i_0] [i_1] [i_0] = (((arr_7 [1] [i_1] [i_2] [i_0]) || (((26 * (arr_3 [i_1 + 1] [i_1 - 3] [i_2]))))));
                }
            }
        }
    }
    #pragma endscop
}
