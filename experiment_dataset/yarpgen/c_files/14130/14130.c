/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14130
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14130 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14130
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_13 = (max(var_13, (~var_7)));
                    arr_9 [i_0] [i_1] = ((var_8 < (arr_8 [5] [i_1] [11] [20])));
                    var_14 = (((((!(arr_5 [i_2])))) >= ((((-(arr_2 [i_0]))) * ((var_8 / (arr_4 [1] [i_1] [14])))))));
                }
            }
        }
    }
    var_15 = (min(var_15, (((!(~var_2))))));
    var_16 = var_0;
    var_17 = ((9244545613230665214 ? 9244545613230665207 : 18446744073709551615));
    var_18 = ((var_11 << ((((~var_10) >= (10285475919411567207 <= var_4))))));
    #pragma endscop
}
