/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191885
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191885 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191885
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 12;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] = ((~(arr_0 [i_0])));
        var_19 = var_17;
        var_20 = (max(var_20, ((((arr_0 [0]) && (arr_0 [6]))))));
        var_21 = (min(var_21, (arr_1 [i_0] [i_0 - 1])));
    }
    /* vectorizable */
    for (int i_1 = 1; i_1 < 12;i_1 += 1) /* same iter space */
    {
        arr_6 [i_1] = (((arr_0 [i_1]) * var_10));
        var_22 -= (var_13 != (!var_3));
        arr_7 [i_1] = (((var_6 ? var_13 : var_3)) < var_4);
    }
    var_23 |= (((((((2213337079 ? 3794803548421498288 : 2081630224)) + var_10))) ? -2169 : ((var_0 ? (min(-2198, var_14)) : var_1))));
    var_24 = -var_1;
    #pragma endscop
}
