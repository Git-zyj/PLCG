/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117716
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117716 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117716
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */_Bool) min((var_3), (((/* implicit */unsigned long long int) (short)32697))));
    var_11 = ((/* implicit */int) min((var_11), ((-(((/* implicit */int) (_Bool)0))))));
    var_12 = ((/* implicit */short) (+((-((-(var_6)))))));
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 3; i_0 < 22; i_0 += 4) 
    {
        var_13 = ((/* implicit */unsigned char) arr_0 [i_0] [i_0]);
        var_14 = ((/* implicit */short) max((var_14), (((/* implicit */short) (-((-(((/* implicit */int) (_Bool)0)))))))));
        var_15 = ((/* implicit */long long int) var_2);
    }
    var_16 = ((/* implicit */long long int) max((var_16), ((~(((long long int) (-(var_4))))))));
}
