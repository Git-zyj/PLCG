/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29101
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29101 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29101
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (min(var_20, (min((((((min(var_12, var_8))) ? ((-9 ? -39 : -39)) : (max(2147483647, var_16))))), var_10))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_3 [i_0] = (((arr_0 [i_0]) ? (arr_2 [i_0] [i_0]) : (var_1 / 15)));
        var_21 = 2147483647;

        for (int i_1 = 3; i_1 < 20;i_1 += 1)
        {
            arr_7 [i_0] [i_0] = (((arr_0 [i_0]) <= (arr_0 [i_0])));
            var_22 = (min(var_22, (arr_2 [i_0] [i_1 + 2])));
            var_23 -= -39;
        }
        arr_8 [i_0] [i_0] = (arr_4 [i_0] [i_0]);
    }
    #pragma endscop
}
