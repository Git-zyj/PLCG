/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85786
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85786 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85786
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((((var_3 | (var_8 % var_14))) % ((var_1 + (-2147483642 | var_11)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_17 = (min(var_17, (max(((((225 > 11) >= (!250)))), (max(((~(arr_0 [i_1]))), (4 - 13)))))));
                var_18 ^= ((((((arr_3 [i_0] [i_0] [8]) - (max((arr_3 [9] [9] [i_0]), (arr_0 [i_0])))))) ? (((!(arr_2 [i_0])))) : (229 | -255)));
            }
        }
    }
    var_19 = var_10;
    var_20 = (~-var_10);
    var_21 &= max((((!(~255)))), (((var_0 > var_7) ? 243 : ((max(11, 130))))));
    #pragma endscop
}
