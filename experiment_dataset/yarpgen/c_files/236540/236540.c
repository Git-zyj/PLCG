/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236540
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236540 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236540
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = (-((((!(arr_3 [i_0]))) ? ((-(arr_3 [i_0]))) : (~15501166771784246884))));
        var_14 = ((!(((var_0 ? (arr_1 [i_0]) : var_11)))));
        arr_5 [8] [i_0] &= ((!((max((arr_3 [10]), (arr_2 [i_0]))))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 21;i_1 += 1) /* same iter space */
    {
        var_15 *= var_3;
        var_16 = -var_0;
        arr_8 [2] [i_1] |= (~(arr_7 [i_1] [i_1]));
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 21;i_2 += 1) /* same iter space */
    {
        var_17 = arr_7 [i_2] [i_2];
        var_18 ^= (~15501166771784246870);
    }
    var_19 = var_8;
    var_20 = var_1;
    var_21 |= (!var_6);
    var_22 = max(var_0, -var_8);
    #pragma endscop
}
