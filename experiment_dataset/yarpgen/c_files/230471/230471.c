/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230471
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230471 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230471
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 19;i_0 += 1)
    {
        arr_3 [i_0] [i_0 - 1] = ((-(((!(arr_1 [i_0] [i_0]))))));
        /* LoopNest 3 */
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 20;i_3 += 1)
                {
                    {
                        arr_12 [i_0] [i_3] [i_2] [i_3] = ((!(arr_2 [i_0 - 2])));
                        var_10 = (arr_8 [i_0] [i_0] [i_0] [i_3]);
                        var_11 = ((!((((arr_4 [i_0] [i_0] [i_2]) ? 0 : (arr_1 [i_1] [i_1]))))));
                    }
                }
            }
        }

        for (int i_4 = 0; i_4 < 20;i_4 += 1)
        {
            var_12 = 196;
            arr_16 [i_0] [i_4] = (arr_5 [i_0 - 2] [i_0 + 1] [i_0 + 1]);
        }
        for (int i_5 = 0; i_5 < 20;i_5 += 1)
        {
            arr_19 [i_5] [i_0] = ((56 ? ((((arr_7 [i_0] [i_5]) ? (arr_8 [i_0 + 1] [i_5] [i_5] [6]) : (arr_13 [i_5] [i_0])))) : 18446744073709551612));
            var_13 = var_9;
        }
    }
    var_14 = var_7;
    #pragma endscop
}
