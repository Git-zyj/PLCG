/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233570
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233570 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233570
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_19 = (((arr_1 [i_0]) ? 2 : (((-(arr_1 [i_0]))))));
                    var_20 |= (arr_0 [i_0]);
                    var_21 = ((-((-(((arr_3 [i_0]) | (arr_1 [i_0])))))));
                    var_22 = (((((!(((~(arr_0 [i_0]))))))) << (((arr_3 [i_0]) + 17435))));
                }
            }
        }
    }
    var_23 ^= (((var_17 | 63) ? ((~((var_8 ? -1320584284 : var_1)))) : (((~var_16) ? (~var_18) : (~1320584284)))));
    var_24 = -var_1;
    var_25 = (!var_13);
    #pragma endscop
}
