/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227047
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227047 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227047
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        var_10 = (((arr_0 [i_0]) ? -1 : ((~(arr_1 [i_0])))));
        var_11 ^= (var_9 != -1059295938);
    }
    var_12 = var_7;
    var_13 = ((1059295941 ? 1059295937 : 14892));
    var_14 = ((((((-1059295942 ? var_1 : var_7)))) ? ((((max(var_7, 3))) ? 204 : (max(var_4, var_3)))) : (((((var_9 ? var_3 : var_6))) ? var_3 : (max(var_6, 15836242854038522523))))));
    #pragma endscop
}
