/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4990
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4990 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4990
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((+(max((-1042477148 / 6636179672068914899), 1))));
    var_17 = (max(var_17, var_0));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            {
                var_18 = (arr_0 [i_0]);
                var_19 = ((1042477148 ^ ((((max(var_5, (arr_1 [i_0] [i_1])))) ? (((arr_0 [i_0]) ? var_6 : var_15)) : 1))));

                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    arr_9 [i_0] [i_1] [i_0] = (((arr_6 [i_1]) * (((!((max(6636179672068914899, (arr_1 [i_0] [1])))))))));
                    var_20 = 34448;
                }
            }
        }
    }
    #pragma endscop
}
