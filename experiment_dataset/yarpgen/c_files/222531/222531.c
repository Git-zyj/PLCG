/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222531
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222531 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222531
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(var_14, var_9));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_1] = (max(((18446744073709551602 ? 18446744073709551598 : 14)), (((+(((arr_3 [i_1] [i_0 + 1] [i_0]) ? 45151 : 478)))))));
                var_15 |= 0;
                var_16 *= -var_10;
                var_17 = (max(0, 41039));
                var_18 ^= (((!18446744073709551602) ? (max((max(632823274, 15)), ((((arr_4 [i_1] [i_1] [i_0]) >= (arr_2 [i_1])))))) : (((((arr_0 [i_0]) && 1))))));
            }
        }
    }
    var_19 = (max(var_19, 1));
    var_20 = var_1;
    #pragma endscop
}
