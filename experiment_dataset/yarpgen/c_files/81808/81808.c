/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 81808
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=81808 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/81808
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_4;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_11 &= (min(((((min(var_8, 0))))), ((12288 ? (((127 - (arr_3 [i_0] [i_1])))) : (((arr_0 [i_0]) ? var_4 : var_6))))));
                var_12 *= (arr_2 [i_0] [i_0] [i_0]);
                var_13 = (((max((min(var_3, (arr_1 [i_0]))), (arr_1 [i_1])))) && ((min((arr_0 [i_0]), (arr_0 [i_0])))));
                var_14 = ((-(((arr_1 [i_1]) ? var_4 : (((((arr_0 [i_1]) || 125))))))));
            }
        }
    }
    var_15 = (max(var_15, var_1));
    var_16 = min(-var_3, ((((var_0 ^ var_2) ? 0 : (85 && -32)))));
    var_17 = ((var_9 ? var_8 : (min(-var_9, (min(-1021749433, var_5))))));
    #pragma endscop
}
