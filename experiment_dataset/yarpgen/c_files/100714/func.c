/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100714
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100714 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100714
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */short) (-(1027883155)));
    /* LoopNest 2 */
    for (signed char i_0 = 0; i_0 < 23; i_0 += 3) 
    {
        for (int i_1 = 2; i_1 < 19; i_1 += 1) 
        {
            {
                var_21 = ((/* implicit */int) arr_2 [i_1] [i_1]);
                var_22 = ((/* implicit */signed char) ((((/* implicit */int) ((arr_1 [(short)19] [(short)4]) < (((((/* implicit */int) (signed char)127)) << (((((/* implicit */int) arr_3 [(signed char)3] [i_1])) - (74)))))))) + (970649397)));
                arr_4 [i_0] [i_1] [i_1] = arr_2 [i_1] [i_1];
            }
        } 
    } 
}
