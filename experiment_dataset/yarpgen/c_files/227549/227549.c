/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227549
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227549 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227549
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min((min(((var_2 ? var_4 : var_8)), var_13)), (~32)));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 10;i_4 += 1)
                        {
                            {
                                var_17 = (((38 ? 32 : -32)));
                                var_18 = (arr_11 [i_3] [i_4] [i_3] [i_3] [i_4 - 2]);
                                var_19 = ((((((((((arr_12 [i_3 + 2] [7]) < (arr_6 [i_4 + 1] [i_3] [i_2]))))) - (arr_7 [i_1])))) ? 25 : 72));
                            }
                        }
                    }
                    var_20 = (((-32 ? 6 : -31)));
                    var_21 = (-3 ? (arr_3 [i_0]) : ((183 ? 9223336852482686976 : 5088)));
                }
            }
        }
    }
    #pragma endscop
}
