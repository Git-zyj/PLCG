/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 92567
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=92567 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/92567
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */_Bool) ((var_6) / ((+(((/* implicit */int) (signed char)-99))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (long long int i_0 = 3; i_0 < 18; i_0 += 4) 
    {
        /* LoopSeq 1 */
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            arr_4 [i_0] = ((/* implicit */unsigned long long int) arr_2 [i_0] [i_1]);
            arr_5 [i_0] [i_1] = ((/* implicit */long long int) ((((/* implicit */long long int) ((/* implicit */int) (unsigned char)92))) > (arr_0 [i_0])));
            arr_6 [i_0 - 1] [i_0 + 4] = ((/* implicit */_Bool) 8219027474745041863LL);
        }
        var_14 &= ((/* implicit */unsigned short) (!(((/* implicit */_Bool) var_0))));
    }
}
