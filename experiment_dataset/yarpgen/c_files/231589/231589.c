/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231589
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231589 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231589
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 9;i_2 += 1)
            {
                {
                    arr_12 [i_0] = (min((arr_5 [i_0 - 2] [i_0 + 1]), (max(0, -30807))));
                    arr_13 [i_0] = (min((min((arr_9 [i_0] [i_2 - 2] [i_0]), 12)), (arr_0 [i_0] [i_0 - 1])));
                    var_16 = (arr_6 [i_0]);
                    var_17 = (min((max(0, -30807)), (((~(arr_8 [i_0 + 1] [i_2 - 2]))))));
                    arr_14 [7] [i_0] [i_2] [i_2] = (max(1064498067, 1064498067));
                }
            }
        }
    }
    var_18 = (var_15 >= var_11);
    var_19 = (min(var_19, (((-(min((-30807 | var_8), var_2)))))));
    #pragma endscop
}
