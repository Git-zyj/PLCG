/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208829
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208829 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208829
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((min(var_3, var_1))) << (var_1 - 17324315645697412597)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 13;i_2 += 1)
            {
                {
                    var_12 = ((((((arr_0 [i_1 + 1] [i_1 - 3]) ? var_6 : (arr_6 [i_1 + 1])))) ? (56 || 1) : (min((arr_3 [i_1 - 1] [i_2] [i_2 - 3]), (arr_3 [i_1 + 1] [i_1 - 3] [i_2 - 1])))));
                    var_13 = (((((~var_1) && 4294967289)) ? (arr_2 [i_0] [12]) : (((var_0 ? (((arr_6 [i_0]) ? -66 : 42)) : var_0)))));
                }
            }
        }
    }
    #pragma endscop
}
