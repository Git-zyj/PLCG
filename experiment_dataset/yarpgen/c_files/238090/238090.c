/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 238090
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=238090 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/238090
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 14;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] [i_0] = (min((((arr_3 [i_1 + 2]) / var_1)), 1637843488));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_13 [i_0] [i_0] [i_2] [i_0] [i_0] [i_2] &= -8266629967057818090;
                                var_12 = (max(((6361930577046085488 * ((1 ? (arr_0 [i_3]) : var_4)))), (arr_6 [i_2 + 2] [i_2 + 2] [i_2 + 2])));
                            }
                        }
                    }
                }
            }
        }
    }
    var_13 += (var_1 || 6361930577046085488);
    var_14 = var_5;
    #pragma endscop
}
