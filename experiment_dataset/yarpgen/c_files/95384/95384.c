/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95384
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95384 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95384
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((((max(-2047380576, 50666))) < ((var_5 ? var_2 : var_7))));
    var_16 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [i_1] = (((288230358971842560 && var_14) ? (arr_3 [i_0 - 1] [i_0 + 1] [i_1]) : (arr_0 [i_0 - 1])));
                var_17 += (min((!-288230358971842548), (arr_0 [i_1])));
            }
        }
    }
    #pragma endscop
}
