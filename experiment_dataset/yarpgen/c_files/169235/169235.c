/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169235
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169235 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169235
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            {
                var_14 = var_13;
                var_15 ^= var_10;
                var_16 = (((((+(((arr_0 [i_1]) << (((arr_4 [i_0 + 1] [1] [i_1]) - 7753171694678745165))))))) ? (((arr_4 [i_0 - 1] [i_1] [i_0]) ? ((13994 ? var_8 : 4621755532426502553)) : (min(-9223372036854775805, (arr_6 [i_0]))))) : (arr_4 [i_1 - 1] [i_1] [i_1])));
            }
        }
    }
    var_17 = ((-7155045362973112179 >= ((var_2 ? 12368301500011649453 : 63))));
    #pragma endscop
}
