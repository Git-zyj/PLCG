/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127532
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127532 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127532
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 ^= ((!((((var_0 - var_5) * var_6)))));
    var_12 = 9;
    var_13 = 1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_14 = 1;

                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 14;i_4 += 1)
                        {
                            {
                                arr_12 [i_0] [i_2] [i_0] = (((-(var_0 - var_9))) + 1);
                                var_15 = 1;
                            }
                        }
                    }

                    for (int i_5 = 3; i_5 < 12;i_5 += 1)
                    {
                        var_16 ^= 1;
                        arr_15 [i_0] [i_0] [i_0] [i_2] = (((((var_9 >= (arr_10 [i_2] [i_2] [i_1] [i_1] [i_2])))) != (((~var_0) - (((arr_2 [i_0] [i_1]) ^ (arr_14 [i_0] [1] [i_2] [i_2])))))));
                    }
                    for (int i_6 = 3; i_6 < 12;i_6 += 1)
                    {
                        arr_18 [i_0] [i_1] [i_2] [i_2] [11] = -1;
                        var_17 ^= (!26);
                    }
                }
                for (int i_7 = 0; i_7 < 14;i_7 += 1)
                {
                    arr_21 [i_0] [i_0] [i_0] [i_0] = -5604890151434738748;
                    var_18 ^= arr_4 [i_1] [i_1];
                    var_19 ^= (!-28);
                    var_20 = var_6;
                    var_21 = (arr_10 [i_0] [i_0] [i_0] [i_0] [i_0]);
                }
                arr_22 [i_0] [i_0] [i_0] = (65501 <= 1);
            }
        }
    }
    #pragma endscop
}
