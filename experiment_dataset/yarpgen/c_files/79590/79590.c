/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 79590
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=79590 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/79590
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        arr_4 [i_0] = ((max(((var_3 ? var_1 : var_6)), (13029 / 1))));
        var_17 = ((!((min((min((arr_1 [11]), 0)), var_10)))));
    }
    var_18 = ((((var_12 ? var_10 : var_1))));
    var_19 = (max((min((var_0 >= var_1), 4360267328333573424)), ((max(-1087299198, ((var_3 ? 13795 : 0)))))));
    #pragma endscop
}
