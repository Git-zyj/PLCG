/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 201183
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=201183 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/201183
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */signed char) max((((/* implicit */unsigned int) (-(var_7)))), (max((339495030U), (var_15)))));
    /* LoopNest 2 */
    for (signed char i_0 = 0; i_0 < 13; i_0 += 2) 
    {
        for (int i_1 = 2; i_1 < 12; i_1 += 4) 
        {
            {
                var_19 &= ((/* implicit */int) ((unsigned int) (-(((((/* implicit */_Bool) arr_0 [i_0])) ? (arr_3 [i_0]) : (arr_3 [i_0]))))));
                var_20 = ((/* implicit */signed char) ((((/* implicit */_Bool) 3955472269U)) ? (3949378990U) : (((/* implicit */unsigned int) -675879430))));
                var_21 = ((/* implicit */signed char) arr_0 [i_0]);
            }
        } 
    } 
}
