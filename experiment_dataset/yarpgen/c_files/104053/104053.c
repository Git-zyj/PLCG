/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104053
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104053 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104053
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_6;
    var_20 = var_2;
    var_21 = (max(var_21, (min((min(var_2, var_12)), var_0))));
    var_22 = var_9;

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = 0;
        arr_3 [i_0] = (((((((1 ? 1 : 1))) ? ((max((arr_1 [i_0]), var_4))) : 1))) && (((((var_6 >> (arr_1 [i_0]))) ^ var_15))));
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_23 += 1;
        var_24 = (min(((~(arr_5 [i_1]))), var_1));
        arr_6 [i_1] [i_1] = ((((((min((arr_5 [i_1]), 1))) > ((min(var_9, (arr_4 [i_1]))))))) > var_3);
        arr_7 [i_1] &= (((((~var_14) + 2147483647)) >> var_15));
    }
    #pragma endscop
}
