/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38660
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38660 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38660
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (((-(min(9007199250546688, var_13)))));
    var_16 = (min(var_16, (-9223372036854775807 - 1)));

    for (int i_0 = 4; i_0 < 12;i_0 += 1)
    {
        var_17 = 1;
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 12;i_2 += 1)
            {
                {
                    var_18 = var_2;
                    var_19 = (min((((arr_6 [i_2] [i_2 + 2] [i_0 - 4]) ? ((var_3 ? -5645256487697539543 : -871)) : -55)), var_13));
                    var_20 = (((((-((70368744177663 ? var_2 : 0))))) ? var_2 : (arr_2 [i_0])));
                    arr_7 [i_0] [i_0] [i_0] = (arr_3 [i_2] [i_0] [i_2 - 1]);
                }
            }
        }
        arr_8 [i_0] = (arr_6 [i_0] [i_0 + 1] [i_0]);
    }
    #pragma endscop
}
