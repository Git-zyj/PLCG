/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165803
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165803 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165803
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 3; i_0 < 21; i_0 += 1) 
    {
        var_15 = (unsigned char)128;
        var_16 = ((/* implicit */unsigned long long int) (unsigned char)139);
    }
    var_17 |= ((/* implicit */short) (unsigned char)143);
    var_18 = ((/* implicit */unsigned short) max((var_18), (((/* implicit */unsigned short) (unsigned char)140))));
}
