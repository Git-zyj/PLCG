/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98226
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98226 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98226
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (!1);
    var_14 = ((var_6 <= (((((var_12 ? var_1 : var_5))) ? 340975652 : (max(var_8, var_10))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_2] [i_0] = (((!var_8) ? var_6 : (arr_1 [i_2])));
                    var_15 = (((arr_7 [13] [i_1] [i_1] [i_1]) ? var_6 : (arr_7 [i_0] [i_0] [i_0] [i_0])));
                }
            }
        }
    }
    #pragma endscop
}
