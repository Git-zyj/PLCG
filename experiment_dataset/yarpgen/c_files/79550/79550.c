/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 79550
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=79550 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/79550
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            {
                arr_5 [i_0] [14] [i_0] = (max(((var_3 ? 1 : ((1 ? 342132760430736724 : (arr_2 [i_0]))))), ((((arr_3 [i_1 - 1] [12] [i_0]) * (arr_3 [i_1 - 1] [i_1] [i_0]))))));
                var_12 = (var_6 ? (max((-7 / 1), (((arr_3 [i_1] [i_0] [17]) ? (arr_2 [i_0]) : (arr_2 [i_1]))))) : -8536151716085049090);
            }
        }
    }
    var_13 = var_8;
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 23;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 1;i_3 += 1)
        {
            {
                var_14 = (arr_12 [i_3] [i_3]);
                arr_13 [i_3] [i_3] = 255;
            }
        }
    }
    #pragma endscop
}
