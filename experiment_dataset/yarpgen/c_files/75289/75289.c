/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75289
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75289 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75289
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (-999399486 - 3298193706);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {

                    /* vectorizable */
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        var_13 = arr_5 [i_0] [i_1 + 2] [i_3] [i_3];
                        var_14 = (min(var_14, (4119439718 | var_9)));
                        var_15 = ((!(((var_11 ? (arr_1 [i_0] [i_0]) : var_0)))));
                    }
                    for (int i_4 = 0; i_4 < 1;i_4 += 1)
                    {
                        var_16 *= (max(var_3, (max((arr_1 [i_1 - 1] [i_1 - 2]), (arr_4 [i_1] [i_1] [i_2])))));
                        var_17 *= (max((~411202517), var_6));
                        var_18 *= arr_5 [i_4] [i_2] [i_1] [i_0];
                    }
                    arr_11 [i_0] [i_0] = 4119439718;
                    var_19 = (max(var_19, (arr_4 [i_0] [i_1 + 1] [i_2])));
                }
            }
        }
    }
    #pragma endscop
}
