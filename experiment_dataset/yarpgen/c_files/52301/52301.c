/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52301
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52301 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52301
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_14;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0] = min((arr_0 [i_0 + 3]), (((arr_0 [i_0 + 1]) ^ (arr_0 [i_0 - 1]))));
                var_16 = (min(var_16, (((((((arr_0 [i_0 + 3]) ? (arr_3 [i_0 + 4] [i_0 + 4] [5]) : (arr_3 [i_0 + 1] [i_0] [i_0])))) ? (((((-1720411982 ? (arr_1 [17]) : 21581))) ? -1 : (arr_0 [i_0]))) : var_6)))));
            }
        }
    }
    #pragma endscop
}
