/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7300
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7300 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7300
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_9, var_10));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    arr_7 [i_0] = (min(var_1, ((1 ? 4 : (arr_0 [i_0])))));
                    arr_8 [i_1] [i_0] [i_1] = ((((((11 ^ 8210189058407471739) < (arr_2 [i_0])))) < (min(((70 ? 1 : 851988763)), 1))));
                    var_12 = (arr_6 [i_0] [12] [i_0]);
                }
            }
        }
    }
    var_13 = 5918177189358982562;
    #pragma endscop
}
