/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242050
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242050 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242050
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_6;
    var_14 = (min(var_14, var_11));
    var_15 = (min(var_7, 962072674304));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_16 *= (min((((arr_1 [i_0]) ? (((min(40, (arr_4 [i_0]))))) : -2715218172025800076)), -var_12));
                var_17 = (max(var_17, ((max(((min((arr_4 [i_1]), (((0 && (arr_3 [10] [i_1]))))))), (arr_0 [i_1]))))));
            }
        }
    }
    #pragma endscop
}
