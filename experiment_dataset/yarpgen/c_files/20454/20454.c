/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20454
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20454 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20454
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_8;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_11 = ((((min(-299902557, (!482681040)))) - ((((1400681753 != var_0) < -var_4)))));
                arr_4 [i_0] [i_0] = (((((((((-2147483647 - 1) ? 299902575 : 1))) ? var_8 : (arr_0 [i_0])))) ? (((arr_1 [i_0]) ^ 67108864)) : 1723923895));
                arr_5 [i_0] = (min(39908, (((arr_2 [i_0] [i_0] [i_1]) ? (arr_2 [i_0] [i_0] [i_1]) : 5999426984295570893))));
            }
        }
    }
    var_12 = 1;
    #pragma endscop
}
