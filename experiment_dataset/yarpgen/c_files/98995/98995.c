/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98995
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98995 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98995
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 -= ((((!((min(var_9, var_12))))) ? ((var_10 ? (var_12 ^ var_13) : var_11)) : var_10));
    var_18 = ((var_3 ? var_13 : 0));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_19 = ((~((min((arr_1 [i_0]), var_9)))));
                    var_20 = (arr_0 [i_0] [i_0]);
                    var_21 = (~0);
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_22 ^= (min((max(var_8, (arr_8 [i_3]))), (min((arr_6 [i_0] [i_1] [i_2] [i_4]), -113))));
                                arr_10 [i_0] [i_4] [i_4] [i_3] [i_4] [i_3] = 0;
                                arr_11 [i_0] = (!1);
                                var_23 = (min(var_23, 1));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
