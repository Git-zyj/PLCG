/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149885
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149885 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149885
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 |= (min(((((max(0, 12102310163604820868))) ? var_6 : ((((-127 - 1) ? var_5 : var_7))))), 524256));

    /* vectorizable */
    for (int i_0 = 1; i_0 < 13;i_0 += 1) /* same iter space */
    {
        arr_3 [1] = var_1;
        var_13 = (arr_2 [i_0 + 2] [i_0]);

        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            var_14 = (arr_0 [i_1] [i_1]);
            arr_6 [13] = (arr_2 [8] [i_1]);
            var_15 = (min(var_15, 37));
        }
    }
    for (int i_2 = 1; i_2 < 13;i_2 += 1) /* same iter space */
    {
        var_16 = (max(var_16, 0));
        var_17 = ((~(((arr_5 [i_2 + 1] [i_2 + 3] [i_2 + 2]) ? (arr_5 [i_2 + 2] [i_2 + 2] [12]) : 0))));
        var_18 += 928400446;
    }
    var_19 = var_11;
    #pragma endscop
}
