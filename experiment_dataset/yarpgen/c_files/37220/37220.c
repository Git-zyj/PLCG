/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 37220
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=37220 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/37220
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    var_20 = ((!((max((((!(arr_1 [i_0] [i_1])))), ((~(arr_1 [i_2] [i_0]))))))));
                    arr_8 [i_2] = (min((arr_6 [i_0] [i_0] [i_0] [i_0]), var_19));
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_3 = 0; i_3 < 1;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 11;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 11;i_5 += 1)
            {
                {
                    var_21 = (!1);
                    arr_17 [i_3] [i_4] [i_5] [6] = (arr_12 [i_3] [10]);
                    var_22 = (!var_18);
                }
            }
        }
    }
    var_23 = ((((min(var_6, (var_14 || var_13)))) << var_13));
    var_24 = ((-1 ? (((var_13 < (min(3136989157, var_8))))) : var_19));
    #pragma endscop
}
