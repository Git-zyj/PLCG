/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23341
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23341 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23341
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_11 ^= (((((var_8 ? (var_3 * 5) : (((((arr_1 [i_0] [i_1]) != var_4))))))) ? (((max(-125, 127)))) : ((((max(var_9, var_2))) ? (min(4294967295, -8777995744104683116)) : ((4294967295 ? -5706701769343849885 : 1054061475))))));
                var_12 = (arr_1 [i_0] [i_0]);
                var_13 = ((-(max(224, (var_8 * 0)))));
                arr_4 [5] = (var_7 >= (((~1503811138) % var_9)));
            }
        }
    }
    var_14 = var_6;
    #pragma endscop
}
