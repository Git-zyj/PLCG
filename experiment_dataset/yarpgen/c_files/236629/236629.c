/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236629
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236629 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236629
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_15 *= (((~var_3) ? (arr_1 [i_0]) : (~var_12)));
        var_16 = (((arr_1 [i_0]) % var_0));
    }
    for (int i_1 = 0; i_1 < 23;i_1 += 1)
    {
        /* LoopNest 3 */
        for (int i_2 = 0; i_2 < 23;i_2 += 1)
        {
            for (int i_3 = 1; i_3 < 22;i_3 += 1)
            {
                for (int i_4 = 1; i_4 < 21;i_4 += 1)
                {
                    {
                        arr_14 [i_1] [i_2] [22] [16] [i_3] [i_2] &= 1733865804;
                        arr_15 [3] [i_4] [i_3] = -var_10;
                    }
                }
            }
        }
        var_17 -= (max((max(1, (arr_7 [i_1]))), ((+(((arr_13 [i_1] [i_1] [i_1]) + 4294967282))))));
    }
    var_18 *= ((var_5 - (((var_0 ? var_13 : 13)))));
    var_19 = var_0;
    #pragma endscop
}
