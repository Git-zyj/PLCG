/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158718
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158718 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158718
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 += ((/* implicit */unsigned int) ((var_4) >= (((/* implicit */unsigned long long int) var_13))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 0; i_0 < 15; i_0 += 2) 
    {
        /* LoopSeq 1 */
        for (unsigned int i_1 = 1; i_1 < 14; i_1 += 2) 
        {
            arr_7 [i_1] = ((/* implicit */unsigned char) (signed char)121);
            var_19 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) ((((/* implicit */_Bool) (short)-23875)) ? (((/* implicit */int) var_17)) : (((/* implicit */int) (signed char)124)))))));
        }
        var_20 |= ((/* implicit */unsigned int) var_10);
        var_21 -= ((/* implicit */_Bool) (-((-(var_13)))));
        var_22 = ((/* implicit */unsigned int) (+(((/* implicit */int) (unsigned char)20))));
    }
}
