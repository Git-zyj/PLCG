/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230649
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230649 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230649
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */short) ((((/* implicit */_Bool) ((((var_8) + (2147483647))) << (((var_15) - (1346783479U)))))) ? (var_8) : (((/* implicit */int) (unsigned char)18))));
    var_18 = ((/* implicit */unsigned long long int) max((var_18), (((/* implicit */unsigned long long int) var_15))));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 2; i_0 < 23; i_0 += 1) 
    {
        var_19 = ((/* implicit */_Bool) (~(arr_2 [i_0] [i_0 - 1])));
        arr_3 [i_0] [i_0] = ((/* implicit */unsigned char) max((((((/* implicit */_Bool) arr_1 [i_0] [i_0 + 1])) ? (((/* implicit */long long int) arr_0 [i_0 - 2])) : (arr_1 [i_0] [i_0 + 1]))), (((/* implicit */long long int) 2139267213U))));
    }
}
