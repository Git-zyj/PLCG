/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104604
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104604 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104604
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_11;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 20;i_1 += 1)
        {
            {
                var_18 = ((((~((var_3 ? 5369288892590460536 : 16982195001088320309)))) & 3973997920106540953));
                arr_7 [i_1] [i_1] = (((arr_1 [i_1 - 1] [i_1 + 1]) << 1));
                arr_8 [i_1] = 3973997920106540953;
                arr_9 [i_0] [i_1] = 4294967295;
            }
        }
    }
    #pragma endscop
}
