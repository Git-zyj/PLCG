/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204123
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204123 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204123
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_7;

    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 2; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    var_11 = (max(((((arr_6 [i_0 - 1] [i_0 - 1]) + 102))), ((var_4 ? (arr_5 [i_0 + 1] [i_1 - 2]) : (arr_5 [i_0 - 2] [i_1 - 2])))));
                    var_12 = 252;
                }
            }
        }
        var_13 -= -21;
        /* LoopNest 2 */
        for (int i_3 = 0; i_3 < 0;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 13;i_4 += 1)
            {
                {
                    var_14 = (max(var_14, (((!(((16 - ((var_7 ? 5 : (arr_1 [i_0])))))))))));
                    var_15 = ((var_2 + (((((arr_13 [i_0] [i_0] [i_0]) ? var_5 : 469762048))))));
                }
            }
        }
    }
    #pragma endscop
}
