/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236728
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236728 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236728
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 -= (max((min(var_2, ((max(var_0, var_1))))), var_8));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_13 = (max(var_13, ((((((arr_1 [1]) ? (arr_3 [i_0 - 1] [17] [1]) : (arr_5 [2] [i_1]))))))));
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        {
                            var_14 -= (min((arr_11 [i_0] [i_1] [i_0] [i_3]), (((var_4 ? var_7 : var_5)))));
                            arr_12 [i_1] [i_3] = ((4294901760 - (arr_3 [i_1] [i_2] [i_3])));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
