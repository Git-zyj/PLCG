/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136340
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136340 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136340
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (var_7 ? (!0) : -29869);
    var_19 = 29869;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_1] [i_0] = ((((-53 ? 3 : 14546))));
                arr_7 [i_1] [i_1] = ((5225251720085469989 ? 3612474738 : ((18289623683956583259 ? (((1325839866256906814 ? -37 : -71))) : (min(7359438820832958191, 110))))));
            }
        }
    }
    #pragma endscop
}
