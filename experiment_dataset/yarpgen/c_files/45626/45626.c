/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45626
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45626 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45626
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_11 = (((arr_9 [i_1]) % (arr_9 [i_2])));
                                var_12 += ((-(arr_3 [1] [i_2] [i_2])));
                            }
                        }
                    }
                }
                var_13 = (min(var_13, -5449615371505618548));
                arr_12 [i_1] = (-6131500089492354578 ^ -1);
                arr_13 [i_0] = -1422006603;
                var_14 = (max(var_14, -11));
            }
        }
    }
    var_15 = (min(var_15, (((((-5339299106901181748 ? var_5 : var_1)))))));
    #pragma endscop
}
