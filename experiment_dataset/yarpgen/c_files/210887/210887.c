/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210887
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210887 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210887
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            {
                var_11 = 1;
                arr_5 [i_1] [i_1] [i_1 + 1] = (max(4162220673, 7837176807154503381));
                arr_6 [i_1] = (((max((arr_2 [i_1 - 1] [i_0]), var_1)) * -1));
            }
        }
    }
    var_12 = var_6;
    var_13 = ((((max((!var_8), ((var_1 ? var_4 : var_1))))) ? ((var_9 - (var_9 > var_4))) : var_10));
    #pragma endscop
}
