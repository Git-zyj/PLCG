/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6174
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6174 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6174
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, var_4));
    var_11 = ((149 ? 7008 : 2));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 8;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_12 = (arr_8 [i_0]);
                                var_13 *= ((((min((arr_6 [i_4] [i_4]), (arr_6 [i_3] [i_4])))) ? ((((arr_3 [i_0] [i_1]) >= (arr_5 [i_4] [i_4] [i_1])))) : ((min((arr_6 [i_1] [i_3]), -6999)))));
                                var_14 ^= ((!((!((max(1, var_8)))))));
                                var_15 = (arr_7 [i_2 + 1] [i_2 + 1] [i_2] [i_2 - 1] [i_2 + 3]);
                                var_16 *= ((((!(((-7008 ? -26860 : 2873007954)))))) != 7008);
                            }
                        }
                    }
                    var_17 = ((((-7008 ? 245 : 65535)) == (1 * 221)));
                }
            }
        }
    }
    var_18 = (max(var_18, ((min(((max(1, 1))), (min((~var_2), (~var_8))))))));
    #pragma endscop
}
