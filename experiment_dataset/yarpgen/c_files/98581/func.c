/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98581
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98581 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98581
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */short) var_12);
    /* LoopSeq 1 */
    for (unsigned short i_0 = 1; i_0 < 10; i_0 += 1) 
    {
        var_20 = ((/* implicit */_Bool) var_15);
        arr_3 [i_0] = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) var_13))));
    }
    var_21 = ((/* implicit */unsigned long long int) max((var_21), (((((/* implicit */_Bool) (((!(((/* implicit */_Bool) var_4)))) ? (((/* implicit */int) var_11)) : (((/* implicit */int) ((var_3) && (((/* implicit */_Bool) var_6)))))))) ? (var_7) : (((/* implicit */unsigned long long int) ((/* implicit */int) ((short) var_17))))))));
}
