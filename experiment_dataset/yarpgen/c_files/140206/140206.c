/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 140206
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=140206 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/140206
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 |= var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] [19] = (min((((arr_1 [i_0] [i_1]) + (arr_1 [i_1] [i_1]))), ((((arr_1 [i_0] [i_1]) > 3608305110)))));
                var_12 = ((~((((2718972885 >> (44431 - 44407))) << (233 - 216)))));
                var_13 = (((((arr_1 [i_0] [14]) & (arr_1 [i_0] [i_1]))) | (min((arr_1 [i_0] [i_0]), var_5))));
            }
        }
    }
    #pragma endscop
}
