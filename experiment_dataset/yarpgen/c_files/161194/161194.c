/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 161194
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=161194 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/161194
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_16;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 17;i_4 += 1)
                        {
                            {
                                arr_13 [i_4] [i_1] [i_2] [i_1] [i_0 - 1] = (min(5504321859152258315, 0));
                                arr_14 [i_4] [i_4] [i_2] [i_4] [i_0] = (min(-932366492, (arr_12 [i_1 + 2] [i_1 + 1])));
                                arr_15 [i_0] [i_0] [i_1] = var_1;
                            }
                        }
                    }
                    arr_16 [i_1] [i_1] = (max(((932366491 ? 1 : var_12)), ((max(0, 1)))));
                }
            }
        }
    }
    var_18 = var_1;
    #pragma endscop
}
