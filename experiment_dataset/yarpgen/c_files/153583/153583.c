/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 153583
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=153583 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/153583
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_15 = (((var_6 || 205) ? (arr_3 [4] [i_1]) : var_4));
                arr_4 [i_0 - 2] = ((!(((max((arr_3 [i_0 - 1] [i_0]), 56883))))));
                var_16 ^= (max((((+((min((arr_1 [9]), (arr_0 [i_0]))))))), (max((max(var_5, var_2)), (arr_3 [i_1] [i_0 - 2])))));
                var_17 = max(((((min((arr_3 [i_0 + 2] [i_0 + 2]), (arr_3 [i_0] [i_0])))) / ((-(arr_1 [i_0 + 3]))))), ((~((~(arr_0 [i_1]))))));
            }
        }
    }
    var_18 = var_7;
    #pragma endscop
}
