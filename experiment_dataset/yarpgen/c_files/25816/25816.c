/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 25816
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=25816 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/25816
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    var_14 = var_6;
                    /* LoopNest 2 */
                    for (int i_3 = 4; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 19;i_4 += 1)
                        {
                            {
                                var_15 = (arr_1 [i_0]);
                                var_16 = (arr_5 [i_3 - 1] [i_3 - 1] [i_4 - 2] [i_4]);
                                arr_11 [i_0] [i_0] [10] [i_0] [i_3] [i_3 - 3] [10] = -20;
                            }
                        }
                    }
                    var_17 = ((((((min((arr_9 [i_0] [i_0] [i_0] [i_1] [i_2]), (arr_8 [15] [15] [i_2] [i_2])))) ? (!var_1) : (arr_0 [i_1] [i_2])))) ? var_5 : var_11);
                }
            }
        }
    }
    var_18 = ((~((var_9 ? var_6 : var_4))));
    var_19 = var_3;
    #pragma endscop
}
