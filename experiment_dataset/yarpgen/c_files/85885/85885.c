/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85885
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85885 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85885
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 9;i_1 += 1)
        {
            {
                arr_6 [i_0] = (((arr_5 [i_1 + 2] [i_0 - 1]) ? ((min((arr_4 [i_0 + 1] [i_0 - 2] [i_1 + 1]), (arr_5 [i_1 - 2] [i_0 - 2])))) : (((arr_5 [i_1 + 1] [i_0 - 2]) % 37889))));
                arr_7 [i_1 - 1] = (min((var_13 ^ 64942), (arr_4 [i_0] [i_0] [i_0])));
                arr_8 [i_1 - 1] = ((-var_5 == ((max(14043, 1)))));
            }
        }
    }
    #pragma endscop
}
