/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 107586
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=107586 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/107586
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */int) var_6);
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 10; i_0 += 1) 
    {
        var_21 = ((/* implicit */unsigned long long int) min((var_21), (((((/* implicit */_Bool) 961511541)) ? (((/* implicit */unsigned long long int) ((/* implicit */int) (signed char)-29))) : (arr_1 [i_0] [i_0])))));
        arr_2 [i_0] = ((/* implicit */unsigned int) var_11);
    }
    var_22 = ((/* implicit */unsigned int) ((((((/* implicit */_Bool) var_9)) ? (var_11) : (((((/* implicit */_Bool) var_19)) ? (-1635566443) : (((/* implicit */int) var_13)))))) / (961511541)));
    var_23 = ((/* implicit */unsigned long long int) var_13);
}
