/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179177
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179177 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179177
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 -= ((/* implicit */unsigned int) var_1);
    var_20 *= ((/* implicit */unsigned long long int) ((((((/* implicit */_Bool) ((((/* implicit */_Bool) var_5)) ? (4) : (((/* implicit */int) var_1))))) ? (((/* implicit */int) (_Bool)1)) : (((4) + (((/* implicit */int) var_15)))))) >> (4)));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 0; i_0 < 19; i_0 += 3) 
    {
        var_21 -= ((/* implicit */unsigned long long int) (_Bool)1);
        var_22 = ((/* implicit */int) min((var_22), ((((!(((/* implicit */_Bool) -1LL)))) ? (var_5) : (((((/* implicit */int) arr_2 [i_0] [i_0])) % (((/* implicit */int) arr_1 [i_0]))))))));
        var_23 = ((/* implicit */unsigned char) (-(((/* implicit */int) arr_0 [i_0] [i_0]))));
    }
}
