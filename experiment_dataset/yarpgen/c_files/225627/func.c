/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 225627
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=225627 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/225627
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (long long int i_0 = 0; i_0 < 16; i_0 += 2) 
    {
        for (short i_1 = 0; i_1 < 16; i_1 += 1) 
        {
            for (unsigned long long int i_2 = 3; i_2 < 13; i_2 += 1) 
            {
                {
                    var_10 = ((/* implicit */unsigned long long int) (!((_Bool)1)));
                    var_11 = ((/* implicit */int) max((var_11), ((+(((/* implicit */int) arr_3 [i_0]))))));
                    var_12 -= ((/* implicit */signed char) -1215743869173965444LL);
                }
            } 
        } 
    } 
    var_13 = (unsigned char)39;
    var_14 = ((/* implicit */unsigned short) (signed char)4);
}
