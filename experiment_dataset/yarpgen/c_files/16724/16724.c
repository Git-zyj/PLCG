/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 16724
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=16724 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/16724
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_6;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_19 = var_1;
        var_20 = (((-(arr_2 [i_0] [i_0]))) + (arr_0 [i_0]));
    }
    for (int i_1 = 0; i_1 < 21;i_1 += 1)
    {
        arr_6 [i_1] [i_1] = (min(((-(arr_3 [i_1] [i_1]))), (arr_3 [i_1] [i_1])));
        /* LoopNest 3 */
        for (int i_2 = 2; i_2 < 19;i_2 += 1)
        {
            for (int i_3 = 1; i_3 < 18;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 21;i_4 += 1)
                {
                    {
                        var_21 = var_2;
                        var_22 = var_1;
                        arr_16 [i_4] [i_3] = ((!(arr_11 [i_3 + 3] [i_2 + 2] [i_3 + 3] [i_2 - 2])));
                    }
                }
            }
        }
        var_23 = (-195620306 & 1);
        var_24 = (min(((8727947539317064967 ? 754754506 : 1)), (~var_12)));
    }
    #pragma endscop
}
