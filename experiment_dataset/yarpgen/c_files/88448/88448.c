/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88448
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88448 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88448
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 21;i_2 += 1)
            {
                {
                    arr_10 [i_2] = (arr_3 [i_0]);
                    var_17 = (arr_8 [0] [i_1] [i_2] [i_2]);
                }
            }
        }
    }
    var_18 ^= var_11;

    for (int i_3 = 1; i_3 < 22;i_3 += 1)
    {
        var_19 = ((1 << (32 - 2)));
        arr_14 [i_3 + 1] = (arr_13 [i_3] [i_3]);
        var_20 = (min(var_20, ((min((arr_12 [i_3 + 1]), (arr_11 [17] [1]))))));
        var_21 = (min(var_21, (arr_13 [i_3] [i_3])));
        arr_15 [4] [i_3] = (arr_11 [i_3] [19]);
    }
    #pragma endscop
}
