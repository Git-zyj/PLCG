/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4963
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4963 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4963
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_18 = (arr_3 [i_1]);
                var_19 = (arr_1 [i_0 - 1]);
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        {
                            var_20 = (max((min((arr_8 [i_0] [i_2 + 2] [i_1] [i_0]), ((min((arr_1 [i_3]), var_4))))), var_13));
                            var_21 ^= 514539129152004796;
                            var_22 = var_10;
                            var_23 ^= (min(((min((((var_4 == (arr_1 [i_3])))), (((arr_1 [i_1]) ? (arr_1 [i_1]) : var_6))))), ((var_17 ? (((arr_8 [i_1] [i_2] [i_1] [i_0 - 1]) ^ 43748)) : -var_8))));
                        }
                    }
                }
            }
        }
    }
    var_24 |= (var_9 == var_1);
    var_25 |= (!var_10);
    #pragma endscop
}
