/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236383
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236383 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236383
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 0; i_0 < 22; i_0 += 4) 
    {
        for (short i_1 = 0; i_1 < 22; i_1 += 1) 
        {
            for (unsigned int i_2 = 2; i_2 < 19; i_2 += 1) 
            {
                {
                    arr_10 [i_1] = ((/* implicit */unsigned long long int) max((arr_4 [i_1] [i_2 + 3] [i_1]), (((/* implicit */unsigned char) (_Bool)1))));
                    arr_11 [i_1] [i_2 + 3] [i_0] [i_1] = ((/* implicit */unsigned long long int) (-((~(((/* implicit */int) arr_5 [i_0] [i_0] [i_1]))))));
                }
            } 
        } 
    } 
    var_17 = ((/* implicit */unsigned long long int) var_6);
}
