/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40847
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40847 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40847
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_19 ^= (((~65535) >= (min(1, -var_5))));

                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    var_20 = (min(var_20, 108));
                    var_21 = (arr_6 [i_0] [i_1] [i_1] [i_2]);
                }
                for (int i_3 = 1; i_3 < 16;i_3 += 1)
                {
                    var_22 = (min(var_6, (((((17190 ? (arr_3 [i_0] [i_1] [i_3]) : (arr_2 [i_0] [i_0]))) <= (arr_4 [i_0] [i_1] [i_3]))))));
                    arr_11 [i_3] &= ((731777044 < ((-(max(37882, 9895030192266710637))))));
                }
            }
        }
    }
    var_23 = var_15;
    var_24 = -7;
    #pragma endscop
}
