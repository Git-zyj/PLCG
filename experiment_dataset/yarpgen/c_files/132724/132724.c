/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132724
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132724 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132724
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 16;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [i_1] [i_0] = (((~var_8) && ((((arr_3 [i_0] [i_1] [i_2]) % 1)))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 16;i_4 += 1)
                        {
                            {
                                arr_14 [i_0] [10] [i_3] [16] [i_3] = ((-((2048 - (arr_13 [i_1] [i_2 + 2] [i_2 + 2] [i_4 + 2] [i_2 + 2])))));
                                arr_15 [i_0] [i_3] [i_2] [i_3] [i_0] [i_4] = ((-((~(((arr_3 [i_0] [i_3] [i_4]) ? (arr_2 [i_3] [i_0]) : 1))))));
                                arr_16 [i_0] [i_0] [i_2] [i_3] [i_3] = (!-255);
                            }
                        }
                    }
                }
            }
        }
    }
    var_13 = 255;
    var_14 = (max(var_14, var_12));
    var_15 = var_4;
    var_16 = 2492994406467684246;
    #pragma endscop
}
