/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95534
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95534 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95534
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = -var_4;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 20;i_3 += 1)
                    {
                        {
                            arr_14 [i_0] [i_2] [i_1] = arr_9 [i_0] [i_0] [i_0] [18];
                            var_12 = (max(var_12, (((~((1962 << (var_6 - 40717))))))));
                        }
                    }
                }
                arr_15 [i_1] = max(((~(((var_1 > (arr_11 [i_0] [i_0] [i_0])))))), ((var_3 ? var_0 : (1 == -222821855))));
                arr_16 [i_1] [i_1 - 1] = (((max((arr_4 [i_1] [i_1 - 2]), var_8)) * (arr_4 [i_1] [i_1 + 1])));
            }
        }
    }
    #pragma endscop
}
