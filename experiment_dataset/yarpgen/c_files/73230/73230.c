/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73230
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73230 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73230
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 += 42652;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_13 *= (min(18212, (((((arr_5 [i_0 + 1]) * 2840)) | (8 < var_8)))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        {
                            var_14 += ((((((max((arr_9 [i_1] [11] [11]), 1286))) ? 60704 : 45031)) < (((!(arr_12 [3] [i_1] [3] [3]))))));
                            arr_13 [i_3] = (-0 | 127);
                        }
                    }
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_4 = 0; i_4 < 12;i_4 += 1)
    {
        for (int i_5 = 3; i_5 < 11;i_5 += 1)
        {
            {
                arr_21 [i_4] [i_5] [11] = (22883 & 26242);
                var_15 = (min(var_15, 34389));
            }
        }
    }
    #pragma endscop
}
