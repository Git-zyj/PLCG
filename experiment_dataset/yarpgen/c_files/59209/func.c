/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59209
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59209 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59209
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */int) ((unsigned int) -1333701236));
    var_16 += ((/* implicit */int) var_1);
    /* LoopSeq 1 */
    /* vectorizable */
    for (int i_0 = 0; i_0 < 15; i_0 += 1) 
    {
        var_17 -= ((/* implicit */short) arr_1 [i_0]);
        /* LoopSeq 1 */
        for (int i_1 = 0; i_1 < 15; i_1 += 4) 
        {
            var_18 = ((/* implicit */int) ((arr_2 [3]) ? (arr_0 [i_0]) : (((/* implicit */long long int) ((/* implicit */int) var_6)))));
            var_19 = ((/* implicit */signed char) min((var_19), (((/* implicit */signed char) var_0))));
            var_20 = ((/* implicit */unsigned short) 1734148782);
        }
    }
}
