/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100193
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100193 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100193
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (~var_1);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_13 += var_6;
                var_14 = (min(var_14, ((((((((24 ? 0 : var_6)) * (arr_2 [i_1])))) ? (((arr_0 [i_1]) ? var_6 : (arr_1 [i_0]))) : (((2683121472 != (((arr_2 [i_0]) ? var_1 : var_6))))))))));
                arr_4 [i_0] [i_1] = (((arr_1 [12]) == (max(var_10, var_5))));
                arr_5 [i_1] = var_6;
                var_15 = (max(var_15, ((max((arr_3 [0] [i_1] [0]), var_9)))));
            }
        }
    }
    #pragma endscop
}
