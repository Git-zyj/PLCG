/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149161
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149161 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149161
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (max(var_18, var_7));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_19 = (max((((((max(var_4, 24578))) ? -115 : (arr_4 [i_1])))), (min((min((arr_6 [i_0]), var_4)), ((max(-93, var_3)))))));
                var_20 ^= 2135387072;
                var_21 = (((((var_0 ? ((16434 ? (arr_5 [i_0] [i_1] [i_1]) : 1)) : ((-(arr_5 [i_0] [i_1] [i_1])))))) ? ((((var_17 * (arr_7 [i_0]))) << (-16170 + 16173))) : var_13));
            }
        }
    }
    var_22 ^= (max(var_0, (!var_2)));
    var_23 = var_7;
    #pragma endscop
}
