/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187573
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187573 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187573
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_3;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_12 = (((max(var_1, (arr_3 [4] [i_1] [i_1]))) & ((((((((-1195451885 + 2147483647) >> (-15915 + 15929)))) <= (((arr_2 [i_0] [i_1]) ^ var_1))))))));
                arr_6 [i_0] [i_0] [i_0] = (min((((868627227 <= (arr_5 [i_0] [i_1] [i_1])))), (((var_7 / var_10) + 868627206))));
            }
        }
    }
    var_13 ^= ((((max(319679709, 2591723684690455737) ^ ((max(319679709, var_3)))))));
    var_14 = var_1;
    var_15 = ((min(var_7, var_0)));
    #pragma endscop
}
