/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125412
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125412 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125412
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 |= ((/* implicit */unsigned char) max((((/* implicit */unsigned long long int) ((((/* implicit */unsigned int) ((/* implicit */int) var_0))) >= (var_4)))), (max((((/* implicit */unsigned long long int) ((int) var_5))), (((((/* implicit */_Bool) var_0)) ? (((/* implicit */unsigned long long int) var_4)) : (var_2)))))));
    var_11 = ((/* implicit */long long int) max((var_11), (((/* implicit */long long int) var_2))));
    var_12 = ((/* implicit */long long int) var_8);
    var_13 = ((/* implicit */unsigned long long int) var_8);
    /* LoopNest 2 */
    for (unsigned short i_0 = 3; i_0 < 21; i_0 += 3) 
    {
        for (int i_1 = 3; i_1 < 18; i_1 += 2) 
        {
            {
                var_14 = ((/* implicit */int) (((-(arr_2 [i_0 - 2] [i_1 + 2] [i_1 - 3]))) - (((((/* implicit */unsigned long long int) 201538194U)) * (arr_2 [i_0 - 2] [i_1 + 2] [i_1 - 3])))));
                var_15 = ((/* implicit */unsigned long long int) var_1);
            }
        } 
    } 
}
