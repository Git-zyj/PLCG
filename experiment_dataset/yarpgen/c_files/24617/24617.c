/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24617
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24617 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24617
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_13 |= ((1 >> (((min((max((arr_0 [i_0] [i_1]), 127)), (((arr_1 [i_1] [i_0]) * var_2)))) - 18394819))));
                arr_4 [3] = (arr_2 [i_1]);
                arr_5 [i_0] [i_1] = (arr_3 [i_0] [i_1] [i_0]);
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 0; i_2 < 13;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 13;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 13;i_4 += 1)
            {
                {
                    var_14 = (max(var_14, (min((arr_2 [i_2]), (arr_0 [i_3] [i_3])))));
                    var_15 -= ((((~(arr_10 [12]))) != 1));
                    arr_16 [i_3] [i_3] = (max((((127 & (arr_14 [i_2] [i_3] [i_3] [i_4])))), (max((arr_14 [i_2] [i_3] [i_2] [i_2]), (arr_2 [i_2])))));
                    arr_17 [i_3] [i_3] [i_3] [i_2] = (var_3 | 0);
                }
            }
        }
    }
    #pragma endscop
}
