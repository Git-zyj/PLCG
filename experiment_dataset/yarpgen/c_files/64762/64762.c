/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64762
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64762 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64762
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_18 = ((-35 ? ((min(37, 1))) : ((var_4 ? 1 : (arr_1 [i_0] [i_1])))));
                var_19 = (max(var_19, ((max((min(157, var_11)), ((((arr_2 [i_1] [i_0]) >= var_11))))))));
                var_20 = arr_4 [i_0] [1] [i_1];
            }
        }
    }
    var_21 = var_6;
    var_22 += ((-1 - ((var_16 ? 916983861853882462 : (((var_10 ? var_10 : var_17)))))));
    #pragma endscop
}
