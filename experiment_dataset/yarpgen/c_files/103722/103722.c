/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103722
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103722 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103722
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (((((-(var_2 >= var_11)))) == var_5));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 14;i_1 += 1)
        {
            {
                arr_7 [i_1] = var_1;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        {
                            arr_15 [i_2] [i_1] [4] [i_0] = (((((arr_3 [i_0] [i_1 - 4]) || (arr_0 [i_0]))) ? ((~(((arr_13 [i_0] [i_3]) ? (arr_3 [i_0] [0]) : var_9)))) : (arr_9 [i_1 + 1] [i_1] [i_1 - 2] [12])));
                            arr_16 [i_3] [1] [3] [i_3] [3] = ((-((~((var_6 ? var_9 : 415626315))))));
                        }
                    }
                }
            }
        }
    }
    var_16 |= ((~(~-var_2)));
    var_17 = ((((var_4 > (var_14 % 14319))) ? var_0 : var_10));
    var_18 = -var_10;
    #pragma endscop
}
