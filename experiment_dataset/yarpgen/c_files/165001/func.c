/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165001
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165001 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165001
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned int) (_Bool)0);
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 14; i_0 += 3) 
    {
        var_21 = ((/* implicit */long long int) ((unsigned int) (_Bool)1));
        var_22 = ((/* implicit */unsigned int) max((var_22), (((/* implicit */unsigned int) max((arr_1 [6LL]), (((/* implicit */unsigned short) (_Bool)0)))))));
        var_23 = ((/* implicit */short) min((var_23), (((/* implicit */short) min((474128171), ((+(((/* implicit */int) arr_2 [2ULL] [2ULL])))))))));
    }
}
