/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211226
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211226 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211226
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = ((((((arr_4 [i_0] [i_1]) ? 55 : (arr_1 [i_1] [i_0])))) ? (max(-438288086798844702, (arr_2 [10]))) : ((((arr_4 [i_0] [i_0]) ? (arr_3 [i_0]) : -727122251)))));
                arr_6 [i_0] = ((((arr_4 [i_0] [i_0]) <= (arr_4 [i_0] [i_0]))));
                var_10 = var_9;
            }
        }
    }
    var_11 = (max(var_11, ((((max(var_4, var_2)) < ((((var_8 < var_8) ? var_2 : ((var_5 ? var_6 : 727122269))))))))));
    var_12 = -5;
    #pragma endscop
}
