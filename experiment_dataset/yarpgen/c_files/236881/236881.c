/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236881
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236881 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236881
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((~(max(42, var_1)))) & var_10));
    var_13 = (((var_11 * -var_4) > (((var_5 & (var_5 / var_2))))));
    var_14 = (((((!var_10) & var_10)) & (((((max(0, -19570))) <= (var_6 <= var_9))))));

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_15 = -30232;
        arr_4 [i_0] [i_0] = ((((((arr_3 [i_0]) - (~-1)))) ? (((arr_0 [i_0]) & (arr_3 [i_0]))) : ((min(((-(arr_0 [i_0]))), (arr_1 [i_0]))))));
    }
    var_16 *= ((var_7 / (((var_10 - var_1) & var_9))));
    #pragma endscop
}
