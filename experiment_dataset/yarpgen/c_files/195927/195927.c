/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195927
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195927 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195927
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                arr_8 [i_0] [i_0] [i_1] = (min((((((var_2 + (arr_4 [i_0] [15] [i_1])))) ? var_3 : (arr_1 [i_0]))), (((max(var_3, (arr_0 [i_0]))) / var_12))));
                var_13 += (arr_5 [i_0] [i_0]);
            }
        }
    }
    var_14 |= var_0;
    /* LoopNest 3 */
    for (int i_2 = 1; i_2 < 1;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 19;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 19;i_4 += 1)
            {
                {
                    var_15 = (max(var_15, 524287));
                    arr_19 [i_2] [i_2] [i_2] = 5060;
                }
            }
        }
    }
    var_16 = var_3;
    var_17 = (max(((((5060 ? -164472352 : 58130)))), 55727));
    #pragma endscop
}
