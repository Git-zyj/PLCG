/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247901
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247901 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247901
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_4;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_11 = ((((-(arr_2 [i_0] [i_0] [i_1]))) < 1));
                var_12 &= (min(((min(((-(arr_0 [i_0]))), (arr_4 [i_0] [i_0] [i_0])))), (arr_2 [15] [i_1] [i_0])));

                for (int i_2 = 2; i_2 < 17;i_2 += 1)
                {
                    var_13 = ((!((min(1, (arr_2 [i_2 + 2] [i_2 + 1] [i_2 + 3]))))));
                    arr_7 [i_2] [i_1] [16] = ((((min(0, var_1))) || var_3));
                    var_14 = 32767;
                }
                var_15 = ((((((var_0 ? 1 : (arr_0 [i_0]))) <= (arr_0 [i_0]))) ? ((~(1 > 1))) : ((~(arr_2 [i_0] [7] [i_0])))));
            }
        }
    }
    #pragma endscop
}
