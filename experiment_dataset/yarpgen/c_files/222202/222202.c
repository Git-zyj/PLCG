/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222202
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222202 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222202
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_5;
    var_13 = 1;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    var_14 = (min(var_14, (((!((((arr_3 [i_0] [12] [i_2]) ? (arr_3 [i_2] [i_1] [i_0 + 1]) : (arr_3 [i_0 - 1] [i_1] [i_2])))))))));
                    var_15 = ((min((~var_7), ((max((arr_7 [i_0 + 1] [20] [i_1] [i_2]), (arr_5 [13] [i_0] [i_0] [i_0])))))));
                    arr_8 [i_2] [i_2] [i_1] [5] = (((arr_6 [i_0] [3] [3] [i_0]) << (var_8 - 1686895552)));
                }
            }
        }
    }
    #pragma endscop
}
