/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180562
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180562 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180562
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        {
                            var_12 = (126 >= 545357767376896);
                            var_13 = (min(var_13, (((!((((arr_2 [i_2] [i_3]) ? (arr_0 [i_0] [i_1]) : 14567))))))));
                        }
                    }
                }
                var_14 = (min(((var_6 ? var_9 : ((var_9 / (arr_7 [i_0] [i_0] [i_0] [3]))))), ((-14579 ? 9325086900725248734 : -1305384587))));
            }
        }
    }
    var_15 = var_0;
    #pragma endscop
}
