/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165928
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165928 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165928
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 23;i_0 += 1)
    {
        var_18 -= (arr_1 [i_0] [i_0]);
        var_19 ^= 18014398505287680;
        var_20 |= (((1318445844 ^ 11753) + (((17 & (arr_0 [i_0 - 3]))))));
        var_21 -= (((arr_1 [i_0] [i_0]) < (max((var_14 - var_16), (arr_1 [i_0] [i_0])))));
    }
    for (int i_1 = 0; i_1 < 15;i_1 += 1)
    {
        arr_4 [i_1] [i_1] = 1;
        var_22 = var_12;
        var_23 &= (-(max((127 && var_5), (arr_2 [i_1]))));
    }
    for (int i_2 = 0; i_2 < 17;i_2 += 1)
    {
        arr_8 [i_2] [i_2] = (((min((arr_6 [i_2]), var_7)) ^ (((arr_6 [i_2]) / (arr_6 [i_2])))));
        var_24 *= var_15;
    }
    var_25 = var_16;
    var_26 = var_5;
    var_27 = -var_14;
    #pragma endscop
}
