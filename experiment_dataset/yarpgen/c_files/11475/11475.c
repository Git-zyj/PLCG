/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11475
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11475 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11475
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, ((max(var_5, (var_15 / var_4))))));
    var_20 = (~(2351774487996602673 | 2351774487996602670));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_1] &= (arr_1 [i_0 - 2] [i_0]);
                arr_6 [i_0] [i_0] [i_0 + 1] = -2351774487996602662;
                arr_7 [i_0] [i_0] = (max(4116413148, (arr_0 [i_0] [i_1])));
            }
        }
    }
    var_21 -= var_0;
    #pragma endscop
}
