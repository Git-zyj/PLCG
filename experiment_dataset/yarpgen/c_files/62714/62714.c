/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62714
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62714 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62714
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_17 = (max(var_17, ((((max((arr_3 [i_1] [i_1]), (arr_1 [i_0])))) * ((((arr_1 [i_0]) < (arr_3 [i_0] [i_0]))))))));
                arr_5 [i_0] [i_0] [i_1] = (((((arr_3 [i_0] [i_1]) ? 40 : -99))) ? (((-9223372036854775807 - 1) ^ ((min(var_15, (arr_1 [i_0])))))) : (arr_4 [i_0] [i_0] [i_0]));
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 0; i_2 < 17;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 17;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 17;i_4 += 1)
            {
                {
                    var_18 = (max(var_18, (((-327002024 ? var_0 : (arr_8 [i_2]))))));

                    for (int i_5 = 4; i_5 < 16;i_5 += 1)
                    {
                        var_19 *= 3682752609;
                        var_20 *= ((!((!(var_11 || 56)))));
                        var_21 ^= var_9;
                    }
                }
            }
        }
    }
    #pragma endscop
}
