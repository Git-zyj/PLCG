/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 107953
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=107953 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/107953
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 &= var_3;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_13 = var_2;
                arr_6 [i_0] = ((arr_1 [10]) ? (max((arr_3 [i_0]), (arr_3 [i_0]))) : (((~(min(var_6, 1873915359))))));
                arr_7 [i_0] = ((((max(18446744073709551615, 919935472))) ? (((max(var_3, (arr_3 [i_0]))) + var_1)) : (((arr_5 [i_0] [i_0 + 2] [8]) - (arr_5 [i_0] [i_0 + 1] [i_0 - 1])))));
            }
        }
    }
    #pragma endscop
}
