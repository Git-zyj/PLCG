/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231088
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231088 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231088
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned char i_0 = 3; i_0 < 8; i_0 += 3) 
    {
        for (short i_1 = 0; i_1 < 11; i_1 += 3) 
        {
            {
                var_13 = ((/* implicit */unsigned short) min((var_13), (((/* implicit */unsigned short) ((short) var_4)))));
                var_14 = ((/* implicit */signed char) max((var_14), (((/* implicit */signed char) ((unsigned int) ((((((/* implicit */int) (short)26606)) * (((/* implicit */int) (short)-4512)))) == (((/* implicit */int) (short)31338))))))));
            }
        } 
    } 
    var_15 = ((/* implicit */signed char) var_7);
    var_16 = ((/* implicit */unsigned short) var_1);
}
