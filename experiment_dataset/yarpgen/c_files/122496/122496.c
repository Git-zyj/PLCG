/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122496
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122496 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122496
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 14;i_0 += 1) /* same iter space */
    {
        var_19 = (2742228726 | 65024);
        var_20 = (4091512224 | 6005);
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1) /* same iter space */
    {
        var_21 = 32767;
        var_22 ^= max((min(((min((arr_3 [i_1]), 22))), (203455072 % var_18))), ((min(((-(arr_0 [i_1] [i_1]))), ((203455054 ? 6003 : (arr_6 [4] [6])))))));
    }
    var_23 = (~var_14);
    var_24 = (max((((-(min(var_8, var_10))))), ((((30290 ? 3630297043657173232 : -18752)) | var_0))));
    #pragma endscop
}
