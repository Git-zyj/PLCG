/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120802
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120802 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120802
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_13 += (max(((-(arr_1 [i_0 + 3]))), (max((0 / var_11), ((max((arr_0 [i_0 - 1]), 14)))))));
                var_14 += ((((!(arr_1 [i_0 + 3]))) || ((((arr_1 [i_0 + 3]) ? (arr_1 [i_0 + 3]) : (arr_1 [i_0 + 3]))))));
                var_15 |= (max((((~((~(arr_2 [5] [i_0])))))), (arr_0 [i_0 - 1])));
            }
        }
    }
    var_16 = 31;
    var_17 |= ((882767114 ? 0 : 12243));
    #pragma endscop
}
