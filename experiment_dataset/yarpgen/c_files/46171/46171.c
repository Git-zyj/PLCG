/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46171
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46171 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46171
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((!(((((var_0 ? var_2 : 458752)) | var_9)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                {
                    arr_10 [14] [14] [i_1] [14] &= ((((min(var_7, (arr_2 [5])) | (!var_0)))));
                    arr_11 [i_0] [i_1] [i_0] = (min((-991073433 / var_4), (((-(var_8 / 65535))))));
                }
            }
        }
    }
    var_11 = ((2663165582 && ((max(1780489143, var_2)))));
    var_12 = var_9;
    #pragma endscop
}
