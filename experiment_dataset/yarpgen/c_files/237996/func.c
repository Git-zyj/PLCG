/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237996
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237996 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237996
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned int i_0 = 4; i_0 < 14; i_0 += 1) 
    {
        arr_3 [i_0] = ((/* implicit */unsigned short) var_8);
        arr_4 [i_0] = ((/* implicit */unsigned short) (~(max(((~(((/* implicit */int) var_12)))), (((((/* implicit */_Bool) arr_1 [i_0] [i_0])) ? (((/* implicit */int) (short)5427)) : (((/* implicit */int) arr_0 [i_0] [i_0]))))))));
    }
    var_18 = ((/* implicit */unsigned short) ((((/* implicit */_Bool) var_16)) ? (var_0) : (((/* implicit */long long int) var_14))));
}
