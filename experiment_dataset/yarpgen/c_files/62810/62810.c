/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62810
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62810 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62810
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_17 -= ((((min(1, (arr_3 [i_1] [i_0] [i_0])))) ? (min((arr_3 [i_0] [i_0] [i_0]), -258048)) : var_2));
                var_18 = (max(var_18, ((max((((-(arr_3 [i_0] [i_0] [i_0])))), ((((var_1 + (arr_5 [i_0] [i_1] [i_1]))))))))));
            }
        }
    }
    var_19 = ((var_13 ^ ((((9223372036854775793 >= var_4) % var_1)))));
    var_20 = var_4;
    #pragma endscop
}
