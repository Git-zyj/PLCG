/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150201
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150201 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150201
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_20 = (((226 ? (arr_5 [i_0] [i_0] [i_1]) : var_13)));
                arr_8 [i_0] [i_1] = (((((arr_5 [i_0] [i_1] [i_0]) ? var_18 : (arr_2 [i_0]))) << (((arr_4 [i_1]) - 112))));
            }
        }
    }
    var_21 = var_18;
    var_22 = (!var_9);
    var_23 = (min(var_23, var_19));
    var_24 = ((~((((max(var_1, -513606544))) ? (var_13 ^ 0) : (-681561305 >= 822745721)))));
    #pragma endscop
}
