/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70966
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70966 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70966
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_6 [i_0 - 1] [i_0 - 1] [i_1] = min((var_3 || var_10), ((536870911 << (671971774 - 671971774))));
                arr_7 [i_0 - 1] = ((var_4 ? ((((671971799 & -671971775) > (arr_3 [i_0] [i_0] [i_0])))) : ((671971803 + ((((arr_0 [i_0 - 1] [i_0 - 1]) == -671971776)))))));
            }
        }
    }
    var_13 = 671971758;
    #pragma endscop
}
