/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60346
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60346 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60346
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((var_11 ? (((var_8 ? var_1 : var_5))) : (((((var_10 ? var_6 : var_8))) | var_3))));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    var_16 = (((min(((var_1 ? var_13 : var_14)), ((min((arr_3 [i_0] [i_1]), var_1)))))) ? (((max((arr_3 [i_0 - 2] [i_0 - 2]), var_13)))) : (((var_7 * var_4) * (var_10 < var_5))));
                    var_17 = (min(var_17, (var_9 * var_8)));
                }
            }
        }
    }
    #pragma endscop
}
