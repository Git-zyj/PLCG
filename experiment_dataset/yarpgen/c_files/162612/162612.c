/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162612
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162612 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162612
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = 255;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_0] [i_0] = (~var_0);
                var_12 = (((((-1 ? 7 : 12526))) ? (arr_1 [i_0] [i_0]) : (--13193)));
                arr_7 [i_1] = -125;
                var_13 = (min((((((arr_0 [i_1] [i_0]) ? var_10 : var_9)) | var_3)), (arr_5 [i_1])));
            }
        }
    }
    var_14 += (((((~var_7) ? ((563924224 ? var_6 : var_5)) : 1358179937)) >> ((((-(11 % var_2))) + 21))));
    #pragma endscop
}
