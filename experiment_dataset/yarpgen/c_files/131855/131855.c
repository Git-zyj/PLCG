/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131855
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131855 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131855
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max((((var_0 ? ((max(var_1, var_1))) : ((max(var_16, 15)))))), 1403250441));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 11;i_1 += 1)
        {
            {
                var_21 = (min(var_21, ((((!var_10) ? ((-(arr_1 [i_0 - 2]))) : var_13)))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 11;i_3 += 1)
                    {
                        {
                            arr_9 [i_0] [i_0] [i_0] [i_0] = 1;
                            arr_10 [i_0] [i_1] = ((((((arr_6 [i_1 + 1] [i_0] [i_0] [i_0 - 1]) + (arr_6 [i_1 + 3] [i_0] [i_0] [i_0 + 3])))) ? 58151 : ((var_18 ? (arr_6 [i_1 + 3] [i_0] [i_0] [i_0 + 2]) : (arr_8 [i_3] [i_3] [i_3 - 1] [i_1 - 3] [i_1 - 1] [i_0 - 2])))));
                        }
                    }
                }
            }
        }
    }
    var_22 = var_18;
    #pragma endscop
}
