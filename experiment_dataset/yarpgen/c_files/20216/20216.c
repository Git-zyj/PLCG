/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20216
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20216 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20216
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min(var_12, (((var_7 ? (var_7 >= var_5) : (-3795997789 && var_5))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 18;i_3 += 1)
                    {
                        {
                            var_13 = (~9450);
                            arr_11 [i_3] [i_3] [i_2] [i_1 - 3] [i_0] = ((((((var_7 ? (arr_6 [i_0] [i_1 + 1] [i_0]) : (arr_1 [17]))))) && 55398));
                        }
                    }
                }
                var_14 = 1;
            }
        }
    }
    var_15 *= ((((3795997771 ? (~var_9) : var_1)) + (((((((var_7 ? var_11 : var_3))) || var_8))))));
    #pragma endscop
}
