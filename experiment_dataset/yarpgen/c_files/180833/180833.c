/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180833
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180833 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180833
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (((max(var_12, var_3)) << (!var_4)));
    var_14 = ((~(((!(((var_0 ? var_12 : var_6))))))));

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        arr_2 [i_0] = (((arr_1 [i_0] [8]) - var_8));
        arr_3 [2] |= ((~(arr_1 [1] [i_0])));
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 1;i_3 += 1)
                {
                    {
                        arr_13 [i_1] [i_0] [i_1] [i_0] [i_0] = (max((((arr_1 [i_0] [i_3]) ? (arr_4 [i_1]) : (arr_7 [i_0]))), (((arr_5 [i_0] [i_2] [i_3]) ? ((~(arr_7 [i_0]))) : ((((arr_7 [i_0]) ? var_10 : var_0)))))));
                        arr_14 [i_0] = var_5;
                    }
                }
            }
        }
    }
    #pragma endscop
}
