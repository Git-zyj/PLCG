/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249995
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249995 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249995
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((((var_10 ? var_3 : ((var_5 ? var_6 : var_4))))) ? ((min((-51 > -7696772626061221917), var_8))) : ((min((min(-89, var_2)), (!126))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    arr_8 [i_2] [5] = ((max((10597 > 88), ((var_8 / (arr_2 [i_1] [i_1]))))));
                    var_12 -= (((arr_3 [i_0] [i_1] [i_2]) | (((arr_1 [i_0]) - (arr_1 [i_2])))));
                }
                for (int i_3 = 0; i_3 < 20;i_3 += 1)
                {
                    arr_11 [i_0] [i_1] [i_0] [i_3] = -1;
                    arr_12 [i_3] = (max(var_8, (((((((arr_0 [i_1]) >= -3607))) >= var_3)))));
                }
                var_13 = (arr_3 [i_0] [i_0] [i_1]);
            }
        }
    }
    var_14 = (max(var_14, (~var_5)));
    #pragma endscop
}
