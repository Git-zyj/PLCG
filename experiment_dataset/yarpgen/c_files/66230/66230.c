/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66230
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66230 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66230
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min((max((var_2 & var_7), (var_3 && var_1))), ((((min(-19, var_7))) ? (max(-24, var_3)) : var_8))));

    /* vectorizable */
    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        var_12 |= (((var_1 / var_0) * 23));
        arr_3 [i_0] [i_0] = (~32767);
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 22;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 20;i_3 += 1)
                {
                    {
                        var_13 = ((-30028 == (arr_6 [i_2 - 2] [i_2] [i_2 - 2])));

                        for (int i_4 = 1; i_4 < 19;i_4 += 1)
                        {
                            var_14 = (max(var_14, ((((-(arr_2 [i_3]))) / (arr_10 [i_0 + 1] [i_0 + 1])))));
                            var_15 = (max(var_15, (((3 ? 32758 : var_2)))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
