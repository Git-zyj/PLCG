/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174456
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174456 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174456
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 11;i_1 += 1)
        {
            {
                var_16 -= arr_1 [i_1 + 1];
                var_17 = (max(var_17, ((((((-var_2 ? (arr_0 [i_1 - 1] [i_1 - 3]) : (arr_1 [i_1 + 4])))) ? ((min(var_12, -var_15))) : (((!var_14) ? var_12 : ((var_13 ? var_15 : var_1)))))))));
            }
        }
    }
    var_18 = (min(((min(((var_1 ? var_5 : var_7)), var_11))), -var_4));
    var_19 = ((var_2 ? ((max((!var_4), var_7))) : (!var_9)));
    #pragma endscop
}
