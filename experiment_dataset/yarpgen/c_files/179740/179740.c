/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179740
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179740 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179740
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_20 -= (max(((((min(18313, -2203926146742196519))) ? ((max((arr_0 [8]), -38))) : (arr_0 [2]))), (((!(~var_7))))));
        arr_3 [i_0] = ((((max(var_3, 65534))) ? (min(var_12, (arr_2 [i_0]))) : ((((arr_1 [i_0] [1]) != (max(var_10, -2361095593469308173)))))));
        arr_4 [i_0] [i_0] = (min(((var_3 && (arr_2 [i_0]))), (arr_2 [i_0])));
    }
    var_21 = (-9223372036854775807 - 1);
    #pragma endscop
}
