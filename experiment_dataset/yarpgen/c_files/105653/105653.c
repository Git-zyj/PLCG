/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105653
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105653 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105653
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 7;i_0 += 1)
    {
        arr_3 [i_0] = -var_19;
        arr_4 [i_0] = var_16;

        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            var_20 = (max(var_20, (((!(max((!var_19), (255 >= 64))))))));
            arr_7 [8] = var_9;
            var_21 = (min(var_21, (((var_17 & ((min(var_14, var_4))))))));
            var_22 = (max(((~(var_4 * var_14))), ((((4294967293 ^ 0) >= var_8)))));
        }
        var_23 = (max(var_23, (((var_12 > (min(var_9, (530866729 / 13586))))))));
    }
    var_24 = (min(var_24, (var_19 ^ var_3)));
    #pragma endscop
}
