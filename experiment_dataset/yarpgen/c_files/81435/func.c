/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 81435
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=81435 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/81435
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((((/* implicit */_Bool) ((long long int) var_5))) ? ((+(((var_10) - (var_0))))) : ((-((+(var_1))))));
    var_17 ^= 12477900675692981LL;
    var_18 = max((((/* implicit */long long int) ((-7961081496562881303LL) == (max((var_4), (-1901608435709065404LL)))))), (var_13));
    var_19 -= var_11;
    /* LoopNest 3 */
    for (long long int i_0 = 3; i_0 < 19; i_0 += 3) 
    {
        for (long long int i_1 = 0; i_1 < 20; i_1 += 4) 
        {
            for (long long int i_2 = 4; i_2 < 18; i_2 += 2) 
            {
                {
                    var_20 |= -1LL;
                    var_21 = ((long long int) var_0);
                }
            } 
        } 
    } 
}
