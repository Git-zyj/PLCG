/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159378
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159378 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159378
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((max(var_3, var_4))) ? (((var_9 ? var_1 : var_6))) : var_4);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 17;i_4 += 1)
                        {
                            {
                                var_12 += (((((13328928446963973291 ? 4 : 228))) == (((((max(36, 39431))) * (!var_5))))));
                                arr_13 [i_0] [i_0] [i_2] [i_3] [9] = (arr_2 [i_0] [i_0]);
                                var_13 = (max((arr_8 [i_0] [i_1] [i_2] [i_3]), (((arr_9 [i_0]) ? 76 : ((104 ? var_6 : (arr_5 [i_2])))))));
                            }
                        }
                    }
                }
                var_14 = (((-(~4))));
            }
        }
    }
    #pragma endscop
}
