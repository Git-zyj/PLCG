/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160812
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160812 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160812
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = 1;

    for (int i_0 = 4; i_0 < 23;i_0 += 1)
    {
        var_20 |= (arr_1 [i_0]);
        var_21 = (max((min((arr_1 [i_0 - 3]), 19395)), ((+(((arr_0 [i_0]) ? (arr_1 [i_0 + 1]) : (arr_1 [i_0])))))));
        arr_2 [3] = (min(1, 1));
        var_22 = (arr_0 [i_0]);
        arr_3 [i_0] = max((max((arr_0 [i_0 + 1]), (arr_0 [0]))), ((((min(1, (arr_1 [i_0]))) != 1))));
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        arr_6 [i_1] [i_1] = ((((max(1, (arr_5 [i_1]))))));
        arr_7 [i_1] &= (max(((((arr_5 [i_1]) == 1))), (min(((0 + (-127 - 1))), 1))));
        var_23 = (((arr_1 [i_1]) ? (arr_5 [i_1]) : 1));
    }
    #pragma endscop
}
