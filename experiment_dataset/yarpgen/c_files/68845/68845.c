/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68845
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68845 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68845
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_14 = var_13;
                arr_5 [i_0] [i_0] [i_0] = (max((((~881900173337362067) ^ 1649902061)), (arr_2 [i_1 - 1])));
                arr_6 [14] [i_0] = ((!(((var_5 ? (!0) : (arr_0 [i_1 - 1] [i_1 - 1]))))));
                arr_7 [i_0] = ((((min(var_4, (arr_2 [i_1])))) ? var_8 : (((((arr_2 [i_0]) == 11649288232581006005))))));
            }
        }
    }
    var_15 = (var_0 & var_12);
    #pragma endscop
}
