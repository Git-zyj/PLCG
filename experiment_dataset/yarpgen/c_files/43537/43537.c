/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43537
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43537 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43537
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_12 *= (((arr_0 [i_0] [i_0]) ? (arr_1 [i_0]) : (arr_0 [i_0] [i_0])));
        arr_2 [i_0] [i_0] = (arr_1 [i_0]);
        arr_3 [i_0] [i_0] = (arr_1 [i_0]);
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        arr_8 [i_1] = ((-(arr_4 [i_1] [i_1])));
        var_13 = (max(((((arr_6 [i_1] [i_1]) || (arr_6 [i_1] [i_1])))), (arr_5 [i_1])));
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 15;i_2 += 1)
    {
        var_14 ^= (((arr_7 [i_2] [i_2]) - (arr_5 [i_2])));
        /* LoopNest 2 */
        for (int i_3 = 0; i_3 < 15;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 15;i_4 += 1)
            {
                {

                    for (int i_5 = 2; i_5 < 14;i_5 += 1)
                    {
                        var_15 = (((arr_16 [i_5 + 1] [i_5] [i_5]) ? (arr_10 [i_5 + 1]) : (arr_18 [i_2] [i_2] [i_5])));
                        var_16 += (((arr_12 [i_2] [i_2]) ? (arr_12 [i_2] [i_2]) : (arr_12 [i_2] [i_2])));
                    }
                    var_17 = ((var_10 || ((((arr_5 [i_3]) ? (arr_11 [i_2]) : (arr_5 [i_2]))))));
                }
            }
        }
    }
    var_18 -= ((((min(1, 1857679539))) || var_8));
    #pragma endscop
}
