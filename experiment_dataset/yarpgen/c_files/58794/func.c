/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 58794
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=58794 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/58794
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 1; i_0 < 17; i_0 += 4) 
    {
        arr_2 [i_0] = ((/* implicit */short) ((((/* implicit */_Bool) (unsigned char)244)) && (((/* implicit */_Bool) (unsigned char)9))));
        arr_3 [i_0] = ((/* implicit */_Bool) ((((/* implicit */unsigned long long int) ((/* implicit */int) var_6))) | (((((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned char)218))) % (var_5)))));
        /* LoopNest 2 */
        for (unsigned int i_1 = 4; i_1 < 16; i_1 += 3) 
        {
            for (signed char i_2 = 3; i_2 < 16; i_2 += 1) 
            {
                {
                    arr_9 [i_0] [i_0] = ((/* implicit */unsigned char) var_10);
                    arr_10 [i_0] = ((/* implicit */unsigned short) ((((/* implicit */int) ((unsigned char) var_5))) ^ ((+(-752071264)))));
                }
            } 
        } 
    }
    var_11 = ((/* implicit */short) var_7);
}
