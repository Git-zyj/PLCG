/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174128
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174128 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174128
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_10 = max((((!(arr_0 [i_0] [i_1])))), (min((((var_1 + 9223372036854775807) >> (var_2 - 1951710414297957898))), (arr_1 [i_1] [i_1]))));
                var_11 = (min(var_11, var_1));
                var_12 = ((~(arr_4 [i_0] [i_1])));
                var_13 = (((arr_4 [i_0] [i_0]) && ((((arr_2 [i_0] [i_1] [i_1]) & (arr_2 [i_1] [i_1] [i_0]))))));
            }
        }
    }
    var_14 = (max(var_14, ((((var_7 ^ 960) ? var_7 : var_4)))));
    var_15 = var_9;
    var_16 = var_4;
    #pragma endscop
}
