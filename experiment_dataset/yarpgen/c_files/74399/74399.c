/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74399
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74399 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74399
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 += (max((max(0, var_1)), 1));

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            var_12 -= ((!((((!var_8) ? ((max(1, 59))) : (!205))))));
            arr_5 [i_0] [i_1] [i_0] = ((-(((!31226) | var_1))));
        }
        var_13 ^= max(((((~(arr_2 [1]))))), (var_0 + -1));
        var_14 = -var_1;
        arr_6 [i_0] [i_0] = -var_4;
        var_15 = (min(var_15, ((max(((max(var_9, (arr_0 [i_0] [i_0])))), ((-((var_8 ? var_6 : 224)))))))));
    }
    var_16 = (min(var_16, var_2));
    #pragma endscop
}
