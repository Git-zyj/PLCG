/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165211
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165211 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165211
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (var_8 / var_4);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_6 [i_1] [i_1] [i_0] = (((((-(arr_4 [i_1 - 4] [i_1] [i_1 - 3] [i_1])))) ? 926900304 : ((((arr_5 [i_0] [1] [i_1]) && (arr_0 [i_0]))))));
                    arr_7 [i_0] [i_1] [i_1 - 2] [5] = (arr_4 [i_0] [i_1] [i_2] [i_2]);
                }
            }
        }
    }
    var_15 = (min(var_15, ((var_13 << (((((~var_9) + 2122567102634469858)) - 12))))));
    #pragma endscop
}
