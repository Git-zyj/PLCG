/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212584
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212584 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212584
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max(var_4, -1));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_14 ^= ((((((arr_2 [i_0 - 1] [14] [i_0 - 1]) <= (max(65530, (arr_1 [i_1] [i_0])))))) < ((((((5 > (arr_3 [i_1])))) <= (min((arr_3 [11]), (arr_3 [i_1]))))))));
                var_15 = (arr_3 [8]);
                var_16 = (min(var_16, ((((-(((arr_3 [i_0]) ^ -1))))))));
                var_17 = ((var_3 ? (((arr_3 [i_1]) ? ((1 ^ (arr_1 [i_0] [i_1]))) : ((((arr_0 [i_0] [i_0]) >> (((arr_0 [i_0] [i_1]) - 3725714267))))))) : (max((arr_4 [i_0 - 1] [4]), (arr_1 [12] [i_1])))));
            }
        }
    }
    #pragma endscop
}
