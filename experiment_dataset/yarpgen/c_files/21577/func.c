/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21577
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21577 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21577
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (long long int i_0 = 0; i_0 < 23; i_0 += 3) 
    {
        for (unsigned char i_1 = 0; i_1 < 23; i_1 += 3) 
        {
            for (unsigned long long int i_2 = 0; i_2 < 23; i_2 += 3) 
            {
                {
                    var_15 = ((/* implicit */unsigned long long int) max((var_15), (((/* implicit */unsigned long long int) ((unsigned int) (unsigned char)239)))));
                    var_16 = ((/* implicit */unsigned short) var_13);
                    var_17 = ((/* implicit */signed char) var_9);
                    var_18 = ((/* implicit */signed char) ((((/* implicit */unsigned int) ((/* implicit */int) min((((/* implicit */unsigned char) arr_0 [i_0])), (var_5))))) - ((-(arr_4 [i_0] [i_0] [i_0])))));
                }
            } 
        } 
    } 
    var_19 |= ((((/* implicit */_Bool) var_2)) ? (var_9) : (((/* implicit */unsigned long long int) ((/* implicit */int) var_2))));
    var_20 = ((/* implicit */unsigned char) var_12);
}
