/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54870
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54870 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54870
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 15;i_4 += 1)
                        {
                            {
                                var_13 = (max(((2607170082 ? 4194304 : var_10)), -3));
                                var_14 = var_2;
                            }
                        }
                    }
                    var_15 = ((3581729152 + ((((((var_0 ? var_10 : var_11))) ? var_3 : ((2607170103 ? (arr_0 [i_0]) : (arr_4 [i_1] [i_1] [i_1] [i_0]))))))));
                    var_16 = (min(var_16, 0));
                }
            }
        }
    }
    var_17 -= ((var_12 ? ((((var_12 / var_12) > var_6))) : var_0));
    #pragma endscop
}
