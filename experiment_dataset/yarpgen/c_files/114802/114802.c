/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114802
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114802 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114802
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 *= var_14;
    var_17 = var_15;

    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        arr_2 [i_0] = -2307400238919193051;
        var_18 += ((min(10096214820297938017, 4425644840104116606)) - (!32767));
        arr_3 [i_0] [11] = ((((min(((max((arr_1 [i_0] [i_0]), (arr_1 [i_0] [i_0])))), (((arr_0 [i_0]) ? (arr_1 [i_0] [i_0 + 2]) : (arr_1 [i_0] [i_0])))))) == (arr_0 [i_0 - 1])));
    }
    var_19 &= ((!((min(var_14, var_13)))));
    #pragma endscop
}
