/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 57139
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=57139 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/57139
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 4; i_0 < 8;i_0 += 1)
    {
        var_12 = (((((var_6 ? (arr_3 [6] [i_0]) : (arr_0 [i_0])))) ? ((-1648727140 ? -105 : var_2)) : var_0));
        var_13 = (((arr_2 [i_0 - 2]) / -1648727140));
        var_14 += (~(arr_1 [i_0] [2]));
        /* LoopNest 3 */
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 9;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 12;i_3 += 1)
                {
                    {
                        var_15 = var_5;
                        arr_14 [i_0] [i_0] [1] [i_2] [i_3] = 27123255;
                        arr_15 [0] [i_3] [i_0] [6] [i_0] &= ((-105 <= 1648727139) ? (arr_9 [i_0 - 3] [i_1 + 1]) : (arr_3 [i_0 + 1] [i_3]));
                    }
                }
            }
        }
    }
    var_16 = 52;
    var_17 = (-92 % var_6);
    #pragma endscop
}
