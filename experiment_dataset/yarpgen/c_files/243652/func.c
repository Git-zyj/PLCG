/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 243652
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=243652 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/243652
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */int) var_14);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 15; i_0 += 4) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_20 = ((/* implicit */short) min(((~(((((/* implicit */_Bool) (signed char)12)) ? (arr_3 [i_1]) : (((/* implicit */int) (unsigned char)150)))))), (((/* implicit */int) (!(((/* implicit */_Bool) ((((/* implicit */_Bool) arr_3 [i_0])) ? (arr_2 [i_0] [i_0]) : (((/* implicit */unsigned int) arr_1 [i_1]))))))))));
                var_21 = ((int) arr_1 [i_0]);
                var_22 = ((631153002) & (((int) (unsigned char)245)));
            }
        } 
    } 
    var_23 = ((/* implicit */unsigned int) var_15);
}
