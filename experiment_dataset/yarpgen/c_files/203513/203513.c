/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203513
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203513 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203513
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min((min(1, var_6)), (((~(~-8208222187688203082))))));

    for (int i_0 = 4; i_0 < 7;i_0 += 1)
    {
        var_11 = var_0;
        arr_4 [0] [i_0] |= ((((((arr_2 [i_0]) ? (arr_1 [i_0]) : var_3))) ? var_0 : (arr_3 [i_0])));
        arr_5 [i_0] = (((65535 ? (((min(37002, 128)))) : (((arr_2 [i_0]) ? (arr_3 [i_0]) : var_1)))));
        arr_6 [i_0] &= var_5;
    }
    #pragma endscop
}
