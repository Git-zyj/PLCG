/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172583
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172583 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172583
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (1114173120 * 1114173142);
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0 - 1] [i_0] = var_13;
                var_20 += (-2028374545 + var_0);
                arr_6 [i_0] [i_1] = ((((((!(var_17 || 13977395352953331088))))) < (((((var_9 ? 2028374544 : -2028374539))) ? var_4 : (var_12 != var_1)))));
            }
        }
    }
    #pragma endscop
}
