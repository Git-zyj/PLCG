/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239246
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239246 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239246
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 3 */
    /* vectorizable */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_11 = ((/* implicit */short) var_3);
        var_12 = ((/* implicit */_Bool) min((var_12), (((/* implicit */_Bool) var_7))));
        var_13 = ((/* implicit */unsigned short) min((var_13), (((/* implicit */unsigned short) var_5))));
    }
    for (signed char i_1 = 0; i_1 < 15; i_1 += 4) 
    {
        var_14 = ((/* implicit */unsigned short) var_0);
        arr_4 [i_1] [i_1] = var_10;
    }
    for (int i_2 = 0; i_2 < 17; i_2 += 2) 
    {
        arr_7 [i_2] [i_2] = ((/* implicit */unsigned char) var_3);
        arr_8 [i_2] = var_10;
        var_15 = ((/* implicit */unsigned long long int) var_8);
        var_16 = ((/* implicit */unsigned char) var_6);
    }
    var_17 = ((/* implicit */long long int) var_3);
}
