/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168666
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168666 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168666
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (max(var_18, var_6));
    var_19 += (((min(var_6, var_10)) + var_12));
    var_20 = (((-(~var_8))) << (var_15 - 892800832));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 15;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_13 [15] [i_2 + 2] [i_2 - 1] [0] [i_0] = var_4;
                                var_21 = var_8;
                            }
                        }
                    }
                    arr_14 [6] [i_1] [i_0] [11] = ((25 ? var_10 : (min((arr_1 [i_2 + 2] [3]), (((255 ? 2027617782 : -2027617755)))))));
                    arr_15 [0] [i_0] [5] [16] = (arr_9 [i_0] [i_0] [i_0] [i_2 + 1]);
                }
            }
        }
    }
    #pragma endscop
}
