/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151639
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151639 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151639
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(((((-1 <= -4512769652400819186) <= 4119957378317432201))), var_12));
    var_16 = var_0;

    /* vectorizable */
    for (int i_0 = 3; i_0 < 14;i_0 += 1) /* same iter space */
    {
        var_17 = var_10;
        arr_4 [i_0] = (((arr_1 [i_0 + 3] [i_0 + 1]) <= -5));
        arr_5 [i_0] [i_0] = (((arr_2 [i_0]) <= (arr_3 [i_0 + 1] [i_0 - 3])));
    }
    /* vectorizable */
    for (int i_1 = 3; i_1 < 14;i_1 += 1) /* same iter space */
    {
        var_18 = (min(var_18, (((2516321120 ? 1895554923 : -29)))));
        var_19 = arr_7 [i_1];
        var_20 = (arr_8 [i_1 - 3]);
    }
    #pragma endscop
}
