/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219744
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219744 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219744
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_12 += (max((max(var_1, ((min(16383, (arr_1 [i_0])))))), var_1));
                    arr_7 [i_2] = ((((max(30720, (arr_3 [i_2] [i_1] [i_0])))) ? (arr_3 [i_1] [i_1] [i_1]) : (arr_3 [i_0] [i_1] [i_2])));
                }
            }
        }
        arr_8 [i_0] [i_0] = (min((arr_6 [12]), var_5));
    }
    var_13 = ((-((((max(2047, var_9))) ? (((28 ? var_0 : var_3))) : ((var_1 ? var_5 : var_2))))));
    var_14 *= var_5;
    #pragma endscop
}
