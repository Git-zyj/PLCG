/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135798
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135798 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135798
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    var_11 = ((-((min(0, 18)))));
                    var_12 = (!var_3);
                    var_13 = min((((-(((var_7 >= (arr_3 [i_0] [i_0]))))))), ((min(2254798998, 35))));
                }
            }
        }
    }
    var_14 = 237;
    var_15 = (((((var_9 ? var_10 : ((var_7 ? var_1 : var_6))))) ? ((min(-var_3, (((var_10 ? var_10 : 160)))))) : ((var_2 ? ((-8575962222880941317 ? var_0 : var_6)) : (((224 ? 5 : 255)))))));
    var_16 = (max(-8575962222880941317, 239));
    #pragma endscop
}
