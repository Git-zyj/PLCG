/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97139
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97139 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97139
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_6;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    arr_9 [i_1] [i_1] [i_1] = var_7;
                    var_11 = (max(var_11, ((((-25500 * 11) ? (-127 - 1) : (((((25500 ? (arr_8 [i_0] [i_0]) : (arr_0 [i_0]))) <= (arr_1 [i_0])))))))));
                    arr_10 [i_1] = (arr_6 [7] [i_1] [i_1] [i_2]);
                    var_12 -= ((((!-25510) ? (((arr_4 [i_2] [i_2]) ? 3588565113 : 25500)) : (((max(-25516, -25516)))))));
                    var_13 = arr_6 [i_0 - 1] [i_0 - 1] [i_0 - 1] [i_0 - 1];
                }
            }
        }
    }
    #pragma endscop
}
