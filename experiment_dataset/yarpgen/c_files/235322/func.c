/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235322
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235322 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235322
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */unsigned int) var_8);
    /* LoopSeq 1 */
    for (short i_0 = 3; i_0 < 7; i_0 += 4) 
    {
        var_11 = ((/* implicit */int) (~(((min((((/* implicit */unsigned long long int) var_3)), (var_6))) * (((/* implicit */unsigned long long int) ((/* implicit */int) max((((/* implicit */signed char) arr_1 [i_0])), (var_5)))))))));
        var_12 = ((/* implicit */signed char) max((var_12), (((/* implicit */signed char) (~(((/* implicit */int) (short)(-32767 - 1))))))));
    }
}
