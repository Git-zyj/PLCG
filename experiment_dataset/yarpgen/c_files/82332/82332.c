/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82332
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82332 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82332
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 16;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 3; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_11 [i_0] [12] [6] [6] [i_2] = (var_9 * var_6);
                                var_11 += (max(51570, 112));
                                var_12 = (!((min(-93, (arr_1 [i_2 - 1] [i_1 - 1])))));
                                arr_12 [i_0] [i_1 - 4] [i_2] [i_2] [i_3 - 1] [i_4] = ((~(((arr_3 [i_0] [i_0] [i_0 - 1]) ? 255 : (arr_8 [i_2] [i_3 + 1] [i_3 + 1] [i_2] [i_1] [i_2])))));
                            }
                        }
                    }
                }
                var_13 = (arr_1 [11] [11]);
                arr_13 [i_0] [i_1] = -var_9;
            }
        }
    }
    var_14 += var_9;
    var_15 = var_10;
    #pragma endscop
}
