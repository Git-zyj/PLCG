/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249492
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249492 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249492
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (((~(~1242151721))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 22;i_4 += 1)
                        {
                            {
                                arr_11 [i_0] [17] [i_2] [i_2] [i_2] = (~(--1242151725));
                                var_21 = ((~(((-(arr_2 [i_0]))))));
                                var_22 += ((~(+-11560)));
                                arr_12 [i_2] [i_3] [i_0] [i_0] [i_2] = ((((!(~var_12)))));
                            }
                        }
                    }
                }
                arr_13 [i_0] [i_1] = ((!((!((!(arr_1 [6] [6])))))));
                /* LoopNest 2 */
                for (int i_5 = 0; i_5 < 22;i_5 += 1)
                {
                    for (int i_6 = 1; i_6 < 20;i_6 += 1)
                    {
                        {
                            arr_19 [i_0] [i_1] [i_5] [i_6] = ((-((-((~(arr_10 [i_0] [i_0])))))));
                            var_23 = (min(var_23, (--617038645)));
                            arr_20 [i_5] = ((~((~(!var_8)))));
                        }
                    }
                }
                var_24 = -17896517663449811238;
            }
        }
    }
    #pragma endscop
}
