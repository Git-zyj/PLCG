/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206774
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206774 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206774
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((-(min(var_8, (var_3 << var_5)))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 14;i_0 += 1) /* same iter space */
    {
        arr_3 [i_0] = ((~(arr_0 [i_0] [i_0])));
        var_16 = ((var_11 ? var_4 : (arr_0 [i_0] [i_0])));
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1) /* same iter space */
    {
        arr_8 [4] [i_1] = (arr_5 [i_1]);

        for (int i_2 = 0; i_2 < 14;i_2 += 1)
        {
            var_17 = (max((((var_10 < ((max(var_6, var_8)))))), (max(var_1, var_1))));
            arr_11 [i_1] [i_1] = ((-(((((min(var_14, var_14))) <= var_8)))));
            var_18 = (max(var_18, (((!(((~((((arr_2 [i_2]) >= var_8)))))))))));
            var_19 = (max(var_19, var_7));
        }
    }
    #pragma endscop
}
