/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95211
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95211 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95211
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 *= ((((max(((var_4 ? 1 : 17023259460099038566)), (((var_8 ? var_4 : var_3)))))) ? var_0 : (((~(!var_2))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_11 &= var_9;
                var_12 *= (~var_0);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        {
                            var_13 += var_8;
                            var_14 &= (~var_7);
                            var_15 = (min(var_15, 1));
                            arr_11 [i_0] [i_2] [i_2] [i_3] [i_0] = var_1;
                            var_16 += (((max((var_7 || var_5), (!var_0))) ? (((var_1 && var_2) ? (1 || 2364991953) : (var_3 && var_4))) : (!var_8)));
                        }
                    }
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_4 = 0; i_4 < 12;i_4 += 1)
    {
        for (int i_5 = 1; i_5 < 9;i_5 += 1)
        {
            {
                arr_18 [i_5] = var_8;
                var_17 = var_5;
            }
        }
    }
    #pragma endscop
}
