/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54833
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54833 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54833
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        arr_3 [i_0] = (max((arr_2 [i_0]), var_2));
        arr_4 [i_0] = ((var_0 ? ((1 & (arr_2 [i_0]))) : (max(((var_4 ? (arr_2 [i_0]) : -2037858678)), (~1)))));
        var_20 -= (arr_0 [i_0] [i_0]);
        arr_5 [i_0] = (max((((2251799780130816 ? (arr_1 [i_0] [i_0]) : 1))), ((var_15 ? (arr_2 [i_0]) : 1154794880))));
    }
    var_21 = var_14;
    var_22 = -803669434;
    var_23 = (max(0, ((-var_14 != (max(1773513661, var_9))))));
    var_24 = (~var_15);
    #pragma endscop
}
