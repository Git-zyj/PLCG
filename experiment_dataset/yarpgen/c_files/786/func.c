/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 786
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=786 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/786
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */unsigned long long int) ((((((/* implicit */int) var_16)) + (2147483647))) >> ((((+(((/* implicit */int) (unsigned char)246)))) - (229)))));
    /* LoopNest 2 */
    for (signed char i_0 = 1; i_0 < 20; i_0 += 3) 
    {
        for (long long int i_1 = 2; i_1 < 20; i_1 += 1) 
        {
            {
                var_19 += ((/* implicit */int) ((((/* implicit */int) arr_3 [i_0 - 1] [i_0 + 2] [16])) >= (((16711680) | (((/* implicit */int) arr_4 [i_1 - 1] [20]))))));
                var_20 -= ((/* implicit */short) (~(((/* implicit */int) ((((/* implicit */_Bool) arr_0 [i_1 - 1])) || (((/* implicit */_Bool) arr_0 [i_1 + 1])))))));
                var_21 = ((/* implicit */int) var_0);
            }
        } 
    } 
}
