/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241792
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241792 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241792
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_11;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_4 [17] [i_1] [i_1] = ((((0 ? 0 : 18446744073709551610)) + (((arr_1 [i_0 + 3]) ? (arr_1 [i_0 + 2]) : (arr_3 [i_0 - 2] [i_0] [i_1])))));
                var_13 = (((((((arr_1 [1]) == (arr_3 [i_0 + 1] [i_0 + 1] [i_0 + 1]))))) * -var_6));
            }
        }
    }
    var_14 = ((var_4 << (((var_7 == ((max(var_9, var_10))))))));
    var_15 = var_4;
    #pragma endscop
}
