/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116582
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116582 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116582
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((((min(var_11, var_12))) ? var_5 : var_0));
    var_18 = var_5;

    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        arr_2 [i_0] = (((-2147483644 / var_11) == (arr_0 [i_0 - 1])));
        arr_3 [i_0] = (((((arr_1 [i_0 - 1]) || (arr_1 [i_0 - 1]))) ? (-2147483644 || var_15) : ((((((arr_0 [i_0]) & (arr_1 [i_0 + 3])))) ? -74 : (arr_0 [i_0])))));
    }
    #pragma endscop
}
