/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206982
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206982 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206982
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] [i_0] = (16102 % -7616996732650573600);
                arr_7 [i_0] [i_1] [i_1] = (((arr_3 [i_0]) % ((((((arr_0 [i_1]) != (arr_4 [i_0] [i_1])))) - (arr_4 [i_0] [i_0])))));
                var_11 = -7616996732650573600;
            }
        }
    }
    var_12 = (min((((var_10 << var_6) ? var_9 : -var_8)), (var_9 && var_9)));
    var_13 -= var_0;
    var_14 -= var_3;
    #pragma endscop
}
