/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110985
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110985 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110985
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 25; i_0 += 1) 
    {
        for (unsigned int i_1 = 1; i_1 < 24; i_1 += 4) 
        {
            {
                arr_5 [i_0] |= ((/* implicit */_Bool) ((var_2) ^ (((/* implicit */unsigned long long int) arr_3 [i_0] [i_1 + 1]))));
                var_18 = ((/* implicit */long long int) min((var_18), ((((+(arr_3 [i_0] [i_1 + 1]))) / (((/* implicit */long long int) ((/* implicit */int) ((unsigned short) ((((/* implicit */_Bool) (unsigned char)241)) ? (((/* implicit */int) (unsigned char)18)) : (((/* implicit */int) var_11)))))))))));
            }
        } 
    } 
    var_19 *= ((/* implicit */unsigned int) var_11);
}
