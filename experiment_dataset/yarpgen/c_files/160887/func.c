/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160887
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160887 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160887
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 *= ((/* implicit */unsigned char) var_9);
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 12; i_0 += 3) 
    {
        for (long long int i_1 = 0; i_1 < 15; i_1 += 1) 
        {
            {
                var_13 = ((/* implicit */long long int) max((var_13), (min((((((/* implicit */_Bool) min((1841247614U), (2453719668U)))) ? (max((((/* implicit */long long int) var_1)), (arr_3 [i_0] [i_1]))) : (((/* implicit */long long int) ((/* implicit */int) arr_2 [i_0] [i_0] [(short)8]))))), (((long long int) -7843606588537854326LL))))));
                var_14 = ((/* implicit */unsigned short) ((unsigned long long int) var_6));
            }
        } 
    } 
    var_15 = ((/* implicit */long long int) var_5);
}
