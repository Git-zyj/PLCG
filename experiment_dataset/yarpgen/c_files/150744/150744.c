/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150744
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150744 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150744
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 3; i_0 < 22;i_0 += 1) /* same iter space */
    {
        var_20 -= (arr_2 [i_0]);
        arr_3 [14] [i_0] = -255;
        var_21 = -21838;
    }
    /* vectorizable */
    for (int i_1 = 3; i_1 < 22;i_1 += 1) /* same iter space */
    {
        arr_8 [i_1] [i_1] = (~var_9);
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 23;i_2 += 1)
        {
            for (int i_3 = 1; i_3 < 22;i_3 += 1)
            {
                {
                    var_22 = (((((arr_2 [i_2]) + 2147483647)) << (((((arr_2 [i_1 - 1]) + 5383)) - 17))));
                    var_23 = 14463036864555533234;
                }
            }
        }
    }
    /* vectorizable */
    for (int i_4 = 0; i_4 < 20;i_4 += 1)
    {
        arr_17 [6] = ((2609521035 << (((var_9 << 1) - 8408151209217600361))));
        var_24 = (max(var_24, (((arr_10 [i_4] [i_4] [i_4]) >= (arr_13 [i_4] [i_4] [i_4] [i_4])))));
        var_25 &= ((+((var_11 ? (arr_14 [i_4] [i_4]) : var_13))));
    }
    var_26 = var_15;
    #pragma endscop
}
