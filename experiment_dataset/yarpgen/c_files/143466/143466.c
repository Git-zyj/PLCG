/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143466
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143466 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143466
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min(var_12, var_7));

    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        var_13 *= ((min((arr_0 [2]), (arr_1 [i_0 + 1]))));
        arr_2 [i_0] = ((((((arr_1 [i_0 + 1]) & (arr_1 [i_0 + 1])))) ? (arr_0 [i_0]) : (((arr_1 [i_0 + 4]) | (arr_1 [i_0 + 1])))));
        var_14 *= ((((((min((arr_1 [i_0]), (arr_1 [i_0 + 3])))) ? var_11 : ((max((arr_0 [1]), (arr_0 [1])))))) > var_8));
        var_15 |= (arr_1 [i_0]);
        arr_3 [i_0] [i_0] = (min(((~(arr_1 [i_0]))), var_8));
    }
    var_16 &= (var_1 - var_10);
    var_17 &= var_8;
    #pragma endscop
}
