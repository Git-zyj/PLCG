/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177678
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177678 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177678
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(1, var_1));
    var_16 = (min(var_16, (((-((((max(var_3, var_1))) ? -var_9 : ((var_3 ? 224 : var_13)))))))));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_17 = ((((arr_4 [i_0]) <= (arr_1 [i_0 + 3]))) ? (~1) : (min(var_9, (arr_1 [i_1]))));
                    arr_7 [i_0 - 3] [i_1] [i_2] |= (max(-var_14, ((1 & (arr_2 [i_1] [i_1])))));
                    var_18 = ((-(max(var_2, (arr_1 [i_0 + 2])))));
                    arr_8 [i_0] = (max(1, 14262));
                }
            }
        }
    }
    var_19 = (((~(var_9 < var_1))));
    #pragma endscop
}
