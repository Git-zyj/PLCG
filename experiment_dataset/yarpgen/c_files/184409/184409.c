/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184409
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184409 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184409
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_8;
    var_18 = var_14;
    var_19 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            {
                var_20 ^= -12;
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_21 = -21;
                                arr_12 [i_0] [i_1] = ((-(((arr_2 [i_0]) ? var_1 : (arr_2 [i_4])))));
                            }
                        }
                    }
                }
                var_22 = (-5459067419556901704 ? (((arr_2 [i_0 + 2]) ? (arr_10 [i_1 - 2] [i_1 - 2] [i_1] [i_1] [i_0 + 1] [i_0 + 1]) : (arr_2 [i_0 + 1]))) : (min((arr_10 [i_0 + 1] [i_1 + 1] [i_1] [i_1] [i_0 + 1] [i_0 + 1]), 33)));
            }
        }
    }
    #pragma endscop
}
