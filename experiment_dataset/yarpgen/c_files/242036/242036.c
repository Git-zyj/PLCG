/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242036
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242036 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242036
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_12, var_11));
    var_13 += (var_6 - var_4);
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_14 = (min((((!((((arr_3 [i_0 + 1] [i_1] [i_1]) ? var_2 : 42061)))))), (var_9 / var_4)));
                var_15 += (arr_5 [i_0] [i_1]);
                var_16 = (((arr_5 [5] [5]) + (((((arr_4 [i_0]) - (arr_5 [i_0] [i_1]))) >> (var_3 + 66)))));
                var_17 += (min((max(var_10, -13096)), (arr_4 [11])));
            }
        }
    }
    var_18 = (max(var_10, 13096));
    #pragma endscop
}
