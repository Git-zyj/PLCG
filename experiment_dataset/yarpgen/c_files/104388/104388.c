/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104388
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, var_7));
    var_18 = 12182182534862444059;
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [i_1] [9] [i_0 + 2] = (min((arr_2 [i_0 + 1] [i_1]), -1));
                var_19 = -1;
                arr_6 [4] [i_1] = (((+-34) < (min(((var_2 ? 2495706562058704209 : 0)), ((var_8 ? 2495706562058704209 : (arr_2 [i_1] [i_0])))))));
                var_20 = ((!((((arr_1 [i_1] [i_0 - 4]) + -1653494878)))));
            }
        }
    }
    var_21 = (max(var_21, (((var_9 ? ((-((max(227, 245))))) : var_0)))));
    #pragma endscop
}
