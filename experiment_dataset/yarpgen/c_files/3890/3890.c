/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3890
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3890 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3890
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, var_4));
    var_18 = 52;

    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0 - 1] [5] [i_2] = ((4397203820424152723 ? 45 : 21401));
                    arr_9 [i_2] = (max(1, var_9));
                    var_19 = ((((((((var_4 ? (arr_4 [i_2]) : (arr_0 [i_1] [0])))) ? var_12 : (~var_13)))) ? (arr_5 [i_2] [i_2] [12]) : 15700));
                    var_20 = 0;
                }
            }
        }
        arr_10 [i_0] = (arr_0 [i_0] [i_0]);
    }

    for (int i_3 = 0; i_3 < 18;i_3 += 1)
    {
        var_21 = ((!((max((arr_12 [i_3]), (arr_11 [i_3] [i_3]))))));
        var_22 = (arr_11 [i_3] [i_3]);
    }
    #pragma endscop
}
