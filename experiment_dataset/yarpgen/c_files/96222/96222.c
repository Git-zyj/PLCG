/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 96222
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=96222 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/96222
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((min((17267393324223522979 || 706364567), ((var_11 ? var_6 : var_4)))) - var_4));
    var_17 = (var_0 < var_1);

    /* vectorizable */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 21;i_3 += 1)
                {
                    {
                        var_18 = ((!(-4588248311596417972 < 28117)));
                        arr_11 [i_0] [i_0] [i_2] [1] [i_2] = ((((var_11 * (arr_8 [i_0] [i_1 - 2] [i_2] [i_3]))) / (((-9223372036854775807 - 1) / -126))));
                    }
                }
            }
        }
        var_19 = (min(var_19, (arr_8 [i_0] [i_0] [i_0] [i_0])));
    }
    #pragma endscop
}
