/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66549
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66549 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66549
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) (unsigned char)242)) ? (((/* implicit */int) (unsigned char)135)) : (((/* implicit */int) (unsigned char)162))));
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 1; i_0 < 11; i_0 += 3) 
    {
        arr_4 [i_0] = ((/* implicit */unsigned char) var_4);
        var_12 = ((/* implicit */_Bool) ((7451210508166936948LL) * (((/* implicit */long long int) ((/* implicit */int) (_Bool)1)))));
        var_13 = ((/* implicit */_Bool) var_10);
    }
}
