/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218955
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218955 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218955
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 &= ((/* implicit */long long int) var_6);
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 20; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 4; i_1 < 18; i_1 += 2) 
        {
            {
                var_17 = ((/* implicit */unsigned long long int) max((var_17), ((-(((((/* implicit */_Bool) 627837781910041404ULL)) ? (627837781910041404ULL) : (var_1)))))));
                arr_4 [i_0] = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) -16LL))));
            }
        } 
    } 
}
