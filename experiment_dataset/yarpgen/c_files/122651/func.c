/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122651
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122651 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122651
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned char) min((((/* implicit */unsigned long long int) 6405801347933729024LL)), (var_6)));
    /* LoopSeq 1 */
    /* vectorizable */
    for (long long int i_0 = 0; i_0 < 18; i_0 += 1) 
    {
        var_13 = ((/* implicit */unsigned long long int) 6405801347933729024LL);
        var_14 = ((((/* implicit */int) arr_1 [i_0] [i_0])) | (((/* implicit */int) var_1)));
    }
}
