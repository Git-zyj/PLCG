/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211733
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211733 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211733
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 &= (max(((min(var_4, ((var_3 ? 42647184 : 181))))), var_12));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_17 = 75;
                arr_7 [i_1] [i_0] = 37;
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 19;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 19;i_3 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_4 = 0; i_4 < 19;i_4 += 1)
                {
                    for (int i_5 = 1; i_5 < 18;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 19;i_6 += 1)
                        {
                            {
                                var_18 &= (((61197 == (arr_17 [i_5 + 1] [i_5 + 1] [i_5 + 1] [10] [i_5 - 1]))));
                                var_19 *= (arr_13 [i_5 + 1] [i_5] [i_5] [i_5 - 1]);
                            }
                        }
                    }
                }
                var_20 = max((arr_19 [i_2] [i_2] [i_2] [i_3] [i_3] [i_3]), (arr_16 [i_2]));
            }
        }
    }
    #pragma endscop
}
