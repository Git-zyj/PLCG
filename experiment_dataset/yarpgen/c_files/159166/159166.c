/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159166
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159166 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159166
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (~var_1);

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_2 [i_0] = ((var_2 ? ((var_3 ? (arr_1 [i_0] [i_0]) : (((arr_1 [i_0] [8]) ? (arr_1 [i_0] [1]) : var_3)))) : ((((arr_0 [1]) && (((arr_0 [i_0]) || var_6)))))));
        var_12 = (min(var_12, (arr_0 [i_0])));
        arr_3 [i_0] = ((~(((((arr_0 [i_0]) - (arr_1 [i_0] [1])))))));
    }
    for (int i_1 = 0; i_1 < 20;i_1 += 1)
    {
        var_13 = (((arr_5 [1]) ? ((min((arr_6 [i_1]), (arr_6 [i_1])))) : ((((((arr_6 [10]) ^ (arr_7 [9])))) ? (((arr_5 [i_1]) ? (arr_6 [i_1]) : (arr_6 [6]))) : var_5))));
        arr_8 [i_1] = max((arr_1 [i_1] [i_1]), (max((min((arr_7 [i_1]), var_10)), (arr_5 [i_1]))));
        var_14 = (min(var_14, (arr_6 [i_1])));
        var_15 *= (arr_5 [i_1]);
    }
    #pragma endscop
}
