/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 12999
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=12999 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/12999
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, ((min((((max(0, 20)))), (min(var_5, (~215))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_18 = ((((max(65535, 1152579410))) ? ((34 ? (arr_4 [i_1] [i_1]) : (arr_0 [i_0]))) : ((~(((!(arr_2 [i_0] [i_1]))))))));
                arr_5 [i_0] |= (((((-(arr_4 [i_0] [i_1])))) ? (arr_1 [i_0]) : (min((arr_4 [i_0] [i_1]), (arr_2 [i_1] [i_0])))));
            }
        }
    }
    #pragma endscop
}
