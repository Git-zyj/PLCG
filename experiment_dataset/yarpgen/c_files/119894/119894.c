/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119894
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119894 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119894
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1 - 1] = var_1;
                var_18 = (max(var_18, ((max(((((arr_2 [i_0 - 1] [i_0 - 1]) ? (((arr_3 [i_1]) % var_13)) : var_8))), (min((min(var_7, var_10)), (~-1))))))));
            }
        }
    }
    var_19 = (((((var_13 ? ((var_8 ? 1965308667 : var_8)) : ((var_15 ? var_13 : 1965308667))))) ? ((-((var_11 ? var_6 : var_5)))) : ((-161787070 ? var_1 : (!var_11)))));
    #pragma endscop
}
