/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117511
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117511 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117511
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = min((max(var_0, ((var_7 ? 2931690414808166037 : var_3)))), var_5);
    var_12 = (min(15506, (~var_2)));
    var_13 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                arr_7 [i_0] [i_0] [i_0] = ((((((max((arr_0 [i_1]), -20480))) ? 6517404377986058125 : var_9)) >= var_9));
                var_14 = ((~((((arr_2 [i_0 + 1] [i_1]) || (arr_2 [i_0 - 1] [i_0 - 1]))))));
            }
        }
    }
    #pragma endscop
}
