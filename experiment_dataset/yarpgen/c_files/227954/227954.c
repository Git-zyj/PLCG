/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227954
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227954 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227954
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(var_15, var_0));

    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        var_16 = (max(var_16, (arr_0 [i_0])));
        var_17 = -1238513024;
    }
    /* vectorizable */
    for (int i_1 = 4; i_1 < 13;i_1 += 1)
    {
        var_18 &= ((!(~var_7)));
        arr_5 [i_1 - 4] = (~var_3);
        var_19 = (arr_3 [i_1 + 1]);
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 10;i_2 += 1)
    {
        var_20 ^= (var_13 & (arr_3 [i_2]));
        var_21 = (max(var_21, (arr_2 [i_2])));
    }
    /* vectorizable */
    for (int i_3 = 0; i_3 < 11;i_3 += 1)
    {
        /* LoopNest 2 */
        for (int i_4 = 2; i_4 < 8;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 11;i_5 += 1)
            {
                {
                    arr_19 [0] [i_5] [i_3] [2] = (arr_8 [i_3] [i_3]);
                    var_22 *= (arr_4 [i_3] [i_3]);
                    var_23 |= 18446744073709551615;
                    arr_20 [i_5] [i_3] [i_3] = (var_1 == var_3);
                }
            }
        }
        arr_21 [i_3] = ((((var_5 ^ (arr_17 [i_3] [i_3] [i_3] [i_3])))) ? (~var_8) : (arr_0 [i_3]));
        arr_22 [i_3] = -var_8;
        var_24 &= ((!(arr_17 [i_3] [i_3] [i_3] [i_3])));
        var_25 |= ((~(arr_1 [i_3] [i_3])));
    }
    #pragma endscop
}
