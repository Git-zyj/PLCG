/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217628
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217628 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217628
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (((var_3 ? (var_12 >= var_12) : (18446744073709551602 && var_11))));
    var_16 = (min(var_16, (min((var_13 ^ var_12), var_11))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    var_17 = 986886212;
                    var_18 = ((var_6 >> (((((~var_2) ? var_8 : (!17063006654721737232))) - 77))));
                    var_19 = (min(var_19, (((!((((arr_3 [i_0] [i_0 + 1]) ? var_11 : (arr_3 [i_0] [i_0 + 2])))))))));
                    var_20 = (~((-(arr_0 [i_0]))));
                }
            }
        }
    }
    #pragma endscop
}
