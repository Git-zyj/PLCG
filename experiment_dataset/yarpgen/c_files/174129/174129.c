/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174129
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174129 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174129
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            {
                arr_6 [i_0] = 9024;
                arr_7 [i_0] = (((((max(var_7, -9024)) == (((max((arr_2 [i_0]), (arr_1 [i_0])))))))) != (min((arr_1 [i_0 - 1]), (arr_4 [i_0] [i_0]))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 2; i_2 < 12;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 13;i_3 += 1)
        {
            {
                arr_12 [i_2] [i_2] [i_3] = arr_11 [i_2];
                arr_13 [i_2] [i_2] = (max(((~((0 ? -2112843422 : (arr_1 [i_3]))))), ((+((9024 ? (arr_3 [i_2]) : (arr_4 [i_3] [i_2])))))));
            }
        }
    }
    var_17 = (min(var_6, ((max(((var_4 ? var_4 : 9024)), ((var_10 ? var_9 : var_16)))))));
    #pragma endscop
}
