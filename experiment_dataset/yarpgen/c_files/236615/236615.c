/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236615
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236615 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236615
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        var_10 = ((~((107 ? ((((arr_1 [17] [17]) < (arr_0 [i_0] [i_0])))) : var_2))));
        arr_3 [i_0] = ((var_0 ? ((~(((arr_1 [i_0] [i_0]) ? var_8 : var_6)))) : ((((arr_2 [i_0]) >= ((var_4 ? (arr_0 [i_0] [i_0 - 1]) : (arr_2 [i_0]))))))));
    }
    var_11 = ((var_3 > ((var_1 ? var_5 : ((var_4 ? var_3 : (-9223372036854775807 - 1)))))));
    #pragma endscop
}
