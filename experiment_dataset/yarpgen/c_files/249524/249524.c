/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249524
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249524 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249524
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min(var_11, ((max(((1 ? var_0 : 1)), var_5)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] [i_1] = ((0 || 1) ? 1 : var_4);
                    var_14 ^= (((18446744073709551609 || var_4) ? ((((-(arr_5 [i_0] [i_2] [4] [i_2]))) ^ var_3)) : ((262112 ? (min(var_4, 17)) : ((((arr_1 [2]) > var_11)))))));
                    var_15 *= ((-((((!1) < 65518)))));
                }
            }
        }
    }
    var_16 &= var_5;
    var_17 = var_5;
    #pragma endscop
}
