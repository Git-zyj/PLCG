/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247614
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247614 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247614
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_9;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    arr_8 [i_2] [i_1] [i_0] = ((((min(var_2, 9801952946788712807))) ? (((arr_5 [i_0] [16] [i_1] [i_1]) ? (arr_0 [i_1]) : 1)) : (((arr_6 [i_0] [i_1] [i_1] [i_2]) ^ (arr_6 [i_0] [i_1] [i_1] [i_2])))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 19;i_4 += 1)
                        {
                            {
                                var_12 = ((((arr_4 [i_4] [i_1]) * (arr_12 [i_4] [i_4 + 1] [i_4 - 3] [i_4 + 3] [11] [i_4 - 3]))));
                                var_13 = (((-(arr_2 [i_4 - 3] [i_1]))));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
