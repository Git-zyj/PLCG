/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211695
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211695 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211695
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = var_0;
                arr_7 [i_1] [i_0] [i_0] = (((arr_4 [i_1] [i_1 + 1] [i_1 + 1]) / (max((arr_4 [i_1] [i_1 - 1] [i_1]), -1536288480))));
                var_19 = (((((arr_0 [i_1]) ? (arr_4 [i_0] [i_0] [i_0]) : (arr_4 [i_1 + 1] [i_1 + 1] [i_1 + 1])))) || ((var_13 || (arr_4 [i_0] [i_1 + 1] [i_0]))));
            }
        }
    }
    var_20 = var_1;
    var_21 = ((var_18 / (min(var_13, var_15))));
    #pragma endscop
}
