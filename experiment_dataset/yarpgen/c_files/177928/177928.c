/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177928
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177928 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177928
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_1;

    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        var_16 = var_11;
        arr_3 [i_0] = (arr_2 [i_0 + 1]);
    }
    for (int i_1 = 0; i_1 < 19;i_1 += 1)
    {
        /* LoopNest 3 */
        for (int i_2 = 0; i_2 < 19;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 19;i_3 += 1)
            {
                for (int i_4 = 3; i_4 < 17;i_4 += 1)
                {
                    {

                        for (int i_5 = 0; i_5 < 19;i_5 += 1)
                        {
                            var_17 = ((!(arr_12 [i_2] [i_1] [i_4 + 2])));
                            var_18 += (arr_7 [i_1] [i_1] [i_1]);
                            var_19 = (~var_1);
                        }
                        var_20 = (max(var_20, (arr_11 [16] [i_3] [2])));
                        var_21 = ((!(arr_4 [i_2])));
                        var_22 = -11582;
                        arr_17 [5] [i_1] [5] [i_1] [i_2] [i_1] = (arr_5 [2] [2]);
                    }
                }
            }
        }
        /* LoopNest 2 */
        for (int i_6 = 1; i_6 < 16;i_6 += 1)
        {
            for (int i_7 = 0; i_7 < 19;i_7 += 1)
            {
                {
                    var_23 = (!-33);
                    arr_23 [i_6] [i_6] [i_6] [0] = (arr_16 [i_6] [i_6]);
                    arr_24 [i_6] [i_6] [i_7] = arr_22 [i_6] [i_6 + 3];
                }
            }
        }
    }
    for (int i_8 = 3; i_8 < 20;i_8 += 1)
    {
        var_24 = (arr_25 [i_8 - 3]);
        var_25 *= 1681018766679408132;
    }
    /* vectorizable */
    for (int i_9 = 2; i_9 < 19;i_9 += 1)
    {
        var_26 += (arr_30 [8]);
        var_27 = (max(var_27, -var_4));
        arr_32 [i_9] = (arr_27 [i_9]);
        var_28 = (!(arr_31 [18] [i_9 - 1]));
    }
    /* LoopNest 2 */
    for (int i_10 = 1; i_10 < 12;i_10 += 1)
    {
        for (int i_11 = 0; i_11 < 13;i_11 += 1)
        {
            {
                var_29 = -27770;
                var_30 = (min(var_30, var_2));
            }
        }
    }
    var_31 = var_1;
    var_32 = (max(var_32, var_4));
    #pragma endscop
}
