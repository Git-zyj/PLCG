/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 205110
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=205110 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/205110
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 &= var_7;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            {
                arr_5 [i_1] = (!2147483647);
                var_14 = (max(((-(arr_1 [i_1 + 4]))), (((arr_1 [i_1 + 2]) ? (arr_1 [i_1 + 2]) : (arr_1 [i_1 + 1])))));
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        {
                            arr_11 [i_1] [i_2] [i_2] [i_1] [i_0] [i_1] = var_3;
                            var_15 = (max((arr_10 [i_2] [i_3] [i_2] [i_1] [3] [i_0]), ((-(((!(arr_0 [i_1]))))))));
                        }
                    }
                }
                arr_12 [i_1] [i_0] [i_1 + 4] = (arr_0 [i_1 + 4]);
            }
        }
    }
    var_16 = var_6;
    #pragma endscop
}
