/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135252
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135252 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135252
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((var_8 ? var_2 : var_0));
    var_11 = (min(-112, (max(((var_4 ? 76 : 2561579974499799139)), var_6))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_1] = (arr_0 [i_1] [i_0]);
                arr_6 [7] [7] [i_0] = -var_0;
            }
        }
    }
    var_12 = (max((((((4294967295 ? var_4 : -117))) ? -112 : 6407248948466775595)), ((max(4294967295, -104)))));
    #pragma endscop
}
