/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 26320
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=26320 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/26320
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 0; i_0 < 10; i_0 += 4) 
    {
        var_18 = ((/* implicit */unsigned int) min((var_18), (((((/* implicit */_Bool) arr_2 [i_0] [i_0])) ? (((/* implicit */unsigned int) ((/* implicit */int) var_10))) : (arr_2 [i_0] [i_0])))));
        var_19 = ((/* implicit */_Bool) min((var_19), (((/* implicit */_Bool) var_3))));
    }
    var_20 = ((/* implicit */_Bool) (short)32767);
    var_21 = ((/* implicit */unsigned int) (~((+(min((var_17), (((/* implicit */unsigned long long int) var_3))))))));
    var_22 = ((/* implicit */unsigned char) ((((/* implicit */int) (_Bool)1)) % (((/* implicit */int) (signed char)127))));
}
