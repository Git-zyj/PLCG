/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32572
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32572 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32572
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_0] = (((arr_2 [i_1 + 2]) >> (((max((max((arr_2 [i_0]), var_10)), ((min(var_9, (arr_4 [11] [i_1 - 2])))))) - 1527503952253538616))));
                arr_7 [i_0] [i_0] [i_1] = (((((arr_1 [i_0]) ? -16 : ((min(var_1, var_2)))))) ? var_6 : (((((min(173, 3802220582))) ? (arr_1 [i_1 + 1]) : (max((arr_5 [i_1] [i_1] [i_1 + 1]), 492746731))))));
            }
        }
    }
    var_14 = var_8;
    var_15 = 0;
    #pragma endscop
}
