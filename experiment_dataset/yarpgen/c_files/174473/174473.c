/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174473
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174473 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174473
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((var_4 * (max(((min(-15306, var_10))), ((3427 ? var_0 : 0))))));
    var_19 = (~var_13);
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 22;i_1 += 1)
        {
            {
                var_20 = (((((((arr_0 [i_0]) ? 126 : 143)))) ? (arr_3 [i_0 - 1]) : (((arr_5 [i_0 - 3] [i_1]) * (arr_5 [i_0 - 2] [i_1])))));
                var_21 = (((((arr_0 [i_0 - 1]) >= (105 & 5))) ? (((((31894 ? (arr_4 [i_1]) : (arr_1 [i_1])))) ? (((1225529904852454940 && (arr_0 [i_0])))) : (((arr_4 [i_1]) + (arr_2 [i_1 - 2]))))) : (arr_0 [i_0 - 1])));
                var_22 += (((!(arr_4 [i_0 - 2]))));
            }
        }
    }
    #pragma endscop
}
