/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88133
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88133 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88133
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = -3888554230344858537;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_14 = (max(var_14, var_2));
                var_15 = (arr_3 [i_0]);
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 25;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 25;i_3 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_4 = 2; i_4 < 23;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 25;i_5 += 1)
                    {
                        {
                            arr_19 [i_2] [i_2] [i_2] [i_5] = (-9223372036854775807 - 1);
                            var_16 |= (((((((arr_5 [i_3]) + (arr_6 [i_4]))))) ? (3888554230344858551 >= 3888554230344858537) : (((arr_14 [i_5] [i_4 + 1] [i_4] [i_4]) ? (arr_14 [i_4 + 2] [i_4 + 2] [i_3] [i_2]) : (arr_8 [i_4 + 1] [i_4] [i_4])))));
                            var_17 = var_6;
                        }
                    }
                }
                arr_20 [i_2] |= (((0 ? 0 : 1)));
            }
        }
    }
    #pragma endscop
}
