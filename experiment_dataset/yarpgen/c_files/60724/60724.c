/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60724
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60724 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60724
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_11 = (((arr_0 [i_0] [i_0]) ? (arr_0 [1] [i_0]) : (arr_0 [i_0] [i_0])));
        var_12 = var_8;

        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            var_13 = (arr_1 [i_0]);
            arr_4 [i_1] [i_0] [i_0] = (arr_1 [i_0]);
        }
    }
    for (int i_2 = 4; i_2 < 21;i_2 += 1)
    {
        var_14 = var_4;
        var_15 = (min(var_15, (((~(((((var_1 ? (arr_8 [i_2] [i_2]) : (arr_5 [i_2]))) >= (arr_5 [i_2 + 2])))))))));
    }
    var_16 = var_8;
    var_17 = (((((~(var_4 || var_5)))) ? (var_9 || var_2) : (--54)));
    #pragma endscop
}
