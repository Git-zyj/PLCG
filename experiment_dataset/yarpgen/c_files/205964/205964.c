/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 205964
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=205964 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/205964
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 += ((((min(var_11, (min(var_8, var_2))))) && (((!var_17) < var_6))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_21 = (min((3563419636484299217 / 1), var_19));
                var_22 = 562949953421311;
                arr_7 [i_0] = (min((arr_0 [i_0]), ((((arr_4 [i_0] [i_0]) && ((max((arr_6 [i_0] [i_0]), (arr_0 [i_0])))))))));
            }
        }
    }
    #pragma endscop
}
