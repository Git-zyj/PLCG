/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14273
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14273 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14273
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (max(var_18, var_8));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 17;i_4 += 1)
                        {
                            {
                                arr_14 [i_2] [i_4] [i_3 - 1] [i_2] [i_1] [i_0 + 1] [i_2] = (arr_7 [i_0 + 1]);
                                arr_15 [i_0] [i_1 - 1] [8] [i_3 - 1] [i_1] |= ((-(arr_10 [i_4])));
                                var_19 = (4294967293 / (24632 >= 197));
                            }
                        }
                    }
                }
                var_20 = (max(var_20, (((var_0 ? (arr_10 [i_1]) : (arr_11 [i_1] [i_1 + 3] [i_1 + 2] [i_0 + 2] [i_0 + 1]))))));
                var_21 = ((!((max((arr_1 [i_0 - 1]), (arr_0 [i_0 + 2]))))));
                var_22 = (max(var_22, 0));
            }
        }
    }
    var_23 &= var_13;
    #pragma endscop
}
