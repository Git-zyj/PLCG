/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 16527
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=16527 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/16527
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_9;
    var_21 *= ((((var_10 && ((min(var_2, var_3)))))));

    for (int i_0 = 2; i_0 < 11;i_0 += 1)
    {
        arr_3 [i_0] [i_0 - 1] |= ((~(!108)));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 11;i_2 += 1)
            {
                {
                    var_22 += ((~((((arr_2 [i_0 - 1] [i_0 - 2]) != (((arr_4 [i_1] [i_1] [i_0]) % (arr_6 [i_2] [i_1] [i_0] [i_0]))))))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            {
                                var_23 = ((!(arr_12 [i_0] [i_1] [i_2 - 2] [i_3] [i_3] [i_1])));
                                var_24 |= var_8;
                            }
                        }
                    }
                    var_25 = 122;
                }
            }
        }
        var_26 &= -71;
    }
    var_27 -= ((((max((~var_12), (4154140596134324456 < var_15)))) ? (((max(var_11, var_19)) & -var_3)) : var_19));
    #pragma endscop
}
