/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165506
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165506 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165506
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_11 = 5535994724274409067;
                var_12 = ((~var_6) ? (~48873) : (((arr_5 [i_1]) % (arr_5 [i_0]))));
                arr_7 [i_0] [i_1] = ((~(~-5535994724274409067)));
            }
        }
    }
    var_13 = var_7;
    var_14 = var_9;
    var_15 = (((((min(var_7, var_7)))) ? var_9 : ((((max(var_10, var_1))) ? var_3 : var_2))));
    #pragma endscop
}
