/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59966
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59966 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59966
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [0] = 71776119061217280;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 21;i_3 += 1)
                    {
                        {
                            var_17 = min((((arr_6 [9] [i_0] [8] [i_3 - 1]) | 1663074409)), 172);
                            var_18 = 160;
                        }
                    }
                }
                var_19 = min((((arr_0 [i_1 + 3]) ? (arr_3 [i_0 + 1] [i_1 + 2]) : (arr_0 [i_0 + 2]))), (((-1 > (arr_0 [i_0 + 1])))));
            }
        }
    }
    var_20 = (((max(var_10, (-1590403073238426600 && var_0)))));
    #pragma endscop
}
