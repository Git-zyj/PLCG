/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 108558
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=108558 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/108558
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */signed char) min((var_13), (((/* implicit */signed char) (~(((/* implicit */int) var_11)))))));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 2; i_0 < 22; i_0 += 2) 
    {
        var_14 = ((/* implicit */short) max((var_14), (((/* implicit */short) var_1))));
        var_15 = ((/* implicit */signed char) max((var_15), (arr_0 [i_0])));
        var_16 = ((/* implicit */signed char) max((var_16), (((/* implicit */signed char) (+((+(((((/* implicit */_Bool) 0U)) ? (((/* implicit */unsigned int) ((/* implicit */int) arr_0 [i_0]))) : (4294967295U))))))))));
    }
    var_17 = ((/* implicit */signed char) var_12);
}
