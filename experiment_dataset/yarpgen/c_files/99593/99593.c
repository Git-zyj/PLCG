/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99593
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99593 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99593
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((~((1 ? 0 : 1))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 8;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 9;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        {
                            var_14 ^= (arr_4 [i_3]);
                            var_15 = ((((-(arr_0 [i_0])))) ^ (arr_7 [i_2] [i_2 - 1] [i_2 - 1] [i_2] [i_2] [i_2]));
                            arr_11 [i_0] [i_2] = (arr_10 [i_0] [i_1] [i_2] [i_3]);
                            arr_12 [i_0 - 1] [i_0] = ((var_2 + (((-(arr_4 [i_0]))))));
                        }
                    }
                }
                arr_13 [i_0] [i_1] = ((!((((arr_3 [i_0 - 1]) >> (arr_9 [i_0 + 1] [i_0] [i_0] [2]))))));
            }
        }
    }
    #pragma endscop
}
