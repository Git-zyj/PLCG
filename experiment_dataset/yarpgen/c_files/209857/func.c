/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209857
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209857 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209857
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */_Bool) (-((+((-(var_2)))))));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 23; i_0 += 2) 
    {
        arr_2 [i_0] [i_0] = ((/* implicit */unsigned char) (~((-(((/* implicit */int) ((signed char) var_11)))))));
        arr_3 [i_0] = ((/* implicit */_Bool) min(((+(((/* implicit */int) (!(((/* implicit */_Bool) -1356983268379986141LL))))))), (((int) (unsigned short)26317))));
    }
    var_14 = ((/* implicit */int) (!((!(((/* implicit */_Bool) var_0))))));
}
