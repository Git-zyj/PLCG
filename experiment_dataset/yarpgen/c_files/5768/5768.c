/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 5768
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=5768 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/5768
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_14 = (var_6 < 1);

                for (int i_2 = 1; i_2 < 9;i_2 += 1)
                {
                    arr_6 [i_2] [i_1] [i_1] [i_1] = ((((((((var_13 ? (arr_2 [8] [i_1] [i_2 + 2]) : (arr_1 [i_0])))) || (max((arr_1 [i_1]), 1))))) > ((~(!1)))));
                    var_15 = 912461233204638159;
                    arr_7 [i_0] [i_1] [i_2] [i_2 + 2] = ((min(912461233204638135, 1813676256)));
                    var_16 = var_6;
                }
            }
        }
    }
    var_17 = var_12;
    #pragma endscop
}
