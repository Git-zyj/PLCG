/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 128702
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=128702 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/128702
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 *= ((/* implicit */signed char) (+(((((/* implicit */unsigned int) (-(((/* implicit */int) var_4))))) + (max((((/* implicit */unsigned int) -2147483647)), (4224543536U)))))));
    /* LoopSeq 3 */
    for (signed char i_0 = 2; i_0 < 24; i_0 += 3) 
    {
        var_13 ^= ((/* implicit */short) (-(((/* implicit */int) var_4))));
        var_14 += ((/* implicit */unsigned short) (-((+(min((var_6), (((/* implicit */long long int) var_8))))))));
    }
    for (int i_1 = 0; i_1 < 14; i_1 += 4) 
    {
        /* LoopNest 3 */
        for (signed char i_2 = 1; i_2 < 12; i_2 += 4) 
        {
            for (_Bool i_3 = 0; i_3 < 1; i_3 += 1) 
            {
                for (unsigned char i_4 = 1; i_4 < 11; i_4 += 4) 
                {
                    {
                        var_15 = ((/* implicit */short) (!(((/* implicit */_Bool) ((arr_12 [i_1]) * (3461865726U))))));
                        var_16 -= ((/* implicit */unsigned short) (+(((/* implicit */int) max((((/* implicit */unsigned char) ((var_8) > (var_8)))), (var_2))))));
                    }
                } 
            } 
        } 
        var_17 = ((/* implicit */short) max((var_17), (((/* implicit */short) var_9))));
    }
    /* vectorizable */
    for (short i_5 = 0; i_5 < 10; i_5 += 4) 
    {
        var_18 = ((/* implicit */short) max((var_18), (((/* implicit */short) (-(arr_4 [i_5]))))));
        var_19 += ((/* implicit */unsigned char) (+((+(((/* implicit */int) (unsigned char)0))))));
        var_20 = ((/* implicit */signed char) (~(((/* implicit */int) var_0))));
        var_21 = ((/* implicit */short) ((((var_6) + (9223372036854775807LL))) >> (((((/* implicit */int) arr_5 [i_5] [i_5])) - (16609)))));
        var_22 = ((/* implicit */short) (-(((/* implicit */int) (_Bool)0))));
    }
    var_23 ^= ((/* implicit */signed char) var_7);
}
