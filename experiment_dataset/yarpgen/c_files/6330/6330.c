/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6330
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6330 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6330
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            {
                var_20 ^= (arr_2 [i_0] [i_0]);
                var_21 = ((18446744073709551615 * (min(18446744073709551603, ((((arr_4 [i_0] [i_0]) == (arr_1 [i_0] [i_1 + 1]))))))));
                var_22 = (arr_2 [i_0 - 1] [i_1]);
                var_23 = (((arr_0 [i_0 - 1]) ? (arr_2 [i_0 + 1] [i_1]) : ((4294967288 ? (arr_3 [i_0] [i_0] [i_0]) : 8))));
            }
        }
    }
    #pragma endscop
}
