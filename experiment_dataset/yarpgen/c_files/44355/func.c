/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44355
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44355 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44355
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 -= ((/* implicit */signed char) var_8);
    /* LoopSeq 1 */
    for (int i_0 = 1; i_0 < 17; i_0 += 4) 
    {
        var_13 &= ((/* implicit */long long int) ((unsigned char) -1891978315922245738LL));
        var_14 = ((/* implicit */unsigned char) max((var_14), (((/* implicit */unsigned char) (~(7395889789521299163LL))))));
    }
}
