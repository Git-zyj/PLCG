/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214839
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214839 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214839
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_3 [1] = (arr_0 [i_0] [i_0]);
        var_16 = ((((max(var_12, var_7))) ? (min(var_5, var_13)) : var_13));
    }
    for (int i_1 = 0; i_1 < 17;i_1 += 1)
    {
        /* LoopNest 3 */
        for (int i_2 = 2; i_2 < 13;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 17;i_3 += 1)
            {
                for (int i_4 = 3; i_4 < 16;i_4 += 1)
                {
                    {
                        var_17 = ((var_13 ? var_6 : (((((arr_6 [i_1] [i_1]) ? (arr_8 [i_4] [i_1] [i_2]) : (arr_8 [i_1] [i_1] [i_1])))))));
                        arr_13 [i_1] = (arr_7 [i_4 - 2] [i_3] [i_2]);
                    }
                }
            }
        }
        var_18 = var_10;
    }
    for (int i_5 = 0; i_5 < 0;i_5 += 1)
    {
        var_19 = arr_14 [i_5 + 1];
        arr_16 [i_5 + 1] = var_12;
    }
    for (int i_6 = 3; i_6 < 7;i_6 += 1)
    {
        var_20 = var_4;
        arr_19 [4] = var_7;
    }
    var_21 = var_10;
    var_22 = var_11;
    #pragma endscop
}
