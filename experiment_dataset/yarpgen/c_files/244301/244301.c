/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244301
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244301 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244301
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 23;i_2 += 1)
            {
                {
                    var_10 = (max(((((arr_5 [i_1] [i_1] [4]) == (((var_1 ? 29182 : 36353)))))), ((var_5 ? var_5 : (!var_2)))));
                    arr_8 [i_2] [i_1 + 2] [i_0] = ((((max(((max(36354, (arr_4 [i_1])))), (((arr_7 [i_0] [i_0] [i_0] [i_1 + 1]) ? var_8 : 36354))))) % ((-(arr_1 [i_1 + 2])))));
                }
            }
        }
    }
    var_11 |= ((2126198296 ? var_4 : (-204182522 | var_2)));
    var_12 = var_0;
    #pragma endscop
}
