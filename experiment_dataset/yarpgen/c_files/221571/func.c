/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221571
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221571 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221571
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned int) ((((/* implicit */long long int) ((/* implicit */int) ((min((((/* implicit */unsigned int) var_2)), (872906873U))) > (min((((/* implicit */unsigned int) (unsigned short)13213)), (var_0))))))) > (var_8)));
    var_12 = ((/* implicit */int) ((4294967295U) != (((/* implicit */unsigned int) ((/* implicit */int) ((unsigned short) (!(var_3))))))));
    /* LoopSeq 1 */
    for (unsigned short i_0 = 0; i_0 < 17; i_0 += 3) 
    {
        arr_2 [i_0] = ((/* implicit */unsigned short) arr_0 [i_0]);
        arr_3 [i_0] [i_0] = arr_1 [i_0];
        var_13 = ((/* implicit */unsigned short) (_Bool)1);
    }
    var_14 = ((/* implicit */unsigned int) var_9);
}
