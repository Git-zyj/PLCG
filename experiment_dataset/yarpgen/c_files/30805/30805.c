/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30805
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30805 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30805
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 |= var_3;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            {
                arr_6 [i_0 - 2] = (((((1294357723 ? 1294357723 : 1294357723))) & (max(3000609573, 18446744073709551615))));
                arr_7 [i_1] [i_1] [i_0] = ((~(((arr_5 [i_0] [i_1 - 1] [i_1 + 2]) ? (arr_3 [i_0] [i_0 + 3]) : (arr_4 [i_0 + 2] [i_1 + 2])))));
            }
        }
    }
    #pragma endscop
}
