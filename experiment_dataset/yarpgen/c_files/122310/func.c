/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122310
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122310 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122310
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (int i_0 = 2; i_0 < 21; i_0 += 2) 
    {
        /* LoopSeq 1 */
        for (_Bool i_1 = 0; i_1 < 0; i_1 += 1) 
        {
            var_13 = ((/* implicit */long long int) (+((-(((/* implicit */int) var_9))))));
            var_14 = ((/* implicit */long long int) arr_0 [2] [(_Bool)1]);
            arr_6 [i_1] = ((/* implicit */_Bool) max((min((((/* implicit */int) ((_Bool) arr_4 [i_0] [9] [i_1]))), (((int) var_12)))), (arr_2 [i_1])));
        }
        var_15 = ((/* implicit */_Bool) arr_5 [i_0]);
        var_16 = var_3;
    }
    var_17 = ((/* implicit */long long int) max((var_17), (((/* implicit */long long int) (~(((/* implicit */int) var_9)))))));
}
