/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18823
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18823 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18823
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min(var_4, var_4));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1 - 2] [i_2] [i_2] = (((arr_6 [i_0]) ? (arr_0 [i_0]) : 0));
                    var_11 = 18446744073709551615;
                    var_12 -= (max(var_3, (max((max((arr_2 [i_0] [14] [i_2]), (arr_3 [i_2] [i_1]))), ((max((arr_4 [i_2]), (arr_6 [i_1]))))))));
                    var_13 -= (max((arr_5 [i_0] [i_0]), (((arr_5 [i_2] [i_1 + 1]) / (arr_5 [i_2] [i_0])))));
                }
            }
        }
    }
    var_14 = var_0;
    #pragma endscop
}
