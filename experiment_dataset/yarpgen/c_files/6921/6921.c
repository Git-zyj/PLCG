/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6921
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6921 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6921
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(var_16, (((var_8 ? (min(var_7, (var_15 % var_9))) : ((min(var_11, var_10))))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                {
                    var_17 = (max(-1, (((~(arr_6 [i_2] [i_2] [i_1] [i_0]))))));
                    var_18 = ((-(arr_0 [i_0] [i_0])));
                    arr_7 [i_2 + 1] = ((!((max((5243 && var_13), (min((arr_0 [i_0] [i_0]), (arr_3 [i_1]))))))));
                }
            }
        }
    }
    #pragma endscop
}
