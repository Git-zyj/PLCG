/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218831
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218831 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218831
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    arr_9 [i_1] = (((((!((min(var_10, var_2)))))) >> (var_9 - 1395947755)));
                    arr_10 [i_0] [i_1] [i_2] = ((!((1 != ((100 >> (100 - 70)))))));
                    var_18 = 287104476244869120;
                    arr_11 [1] [i_1] [1] = var_11;
                }
            }
        }
    }
    var_19 = var_17;
    var_20 = (5200447784267297205 == 156);
    var_21 = -94;
    #pragma endscop
}
