/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32732
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32732 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32732
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 |= var_1;
    var_11 |= ((((((65535 ? var_8 : var_5)))) ? ((max(var_0, var_7))) : 1));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_0] = ((65535 << ((((((1 << (var_7 - 40717))) << (3859341067526646624 - 3859341067526646623))) - 2097141))));
                var_12 = (max(var_12, (((((((arr_0 [i_1]) ? (arr_0 [i_0]) : (arr_4 [i_0])))) ? (((arr_4 [i_0]) * (arr_3 [i_0]))) : (arr_4 [i_0]))))));
            }
        }
    }
    #pragma endscop
}
