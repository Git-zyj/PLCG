/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 141829
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=141829 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/141829
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned char i_0 = 3; i_0 < 17; i_0 += 3) 
    {
        var_16 = ((/* implicit */unsigned int) min((var_16), (((/* implicit */unsigned int) 0ULL))));
        arr_3 [i_0] [i_0] = ((/* implicit */long long int) (_Bool)1);
        arr_4 [i_0] = ((long long int) max((arr_1 [i_0]), (arr_1 [i_0])));
    }
    var_17 = ((/* implicit */unsigned char) (~(max((((/* implicit */unsigned long long int) ((long long int) 18ULL))), ((~(9927334171331351558ULL)))))));
}
