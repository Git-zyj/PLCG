/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 128296
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=128296 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/128296
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_11 &= ((((var_10 || ((!(arr_2 [i_0] [i_0]))))) ? (((((1 | (arr_0 [i_0])))) ? var_10 : (arr_0 [i_0]))) : (((((min((arr_4 [i_0] [i_0]), (arr_3 [i_1]))) <= (~var_8)))))));
                var_12 = (max(var_12, (((-((-(arr_4 [i_0] [i_1]))))))));
                var_13 = (max((arr_3 [i_0]), (((((((arr_2 [i_1] [i_1]) <= var_8))) >= var_0)))));
            }
        }
    }
    var_14 = var_6;
    #pragma endscop
}
