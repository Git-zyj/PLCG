/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63094
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63094 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63094
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (var_9 | -var_0);

    /* vectorizable */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        arr_2 [i_0] = (arr_1 [i_0] [i_0]);
        arr_3 [i_0] = (~(arr_0 [i_0]));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_11 [i_0] [1] [i_2] = (!131);
                    var_13 = ((var_3 / var_0) ? var_4 : (arr_6 [i_1]));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_14 = 1;
                                arr_17 [i_2] = -4099312466276747837;
                                arr_18 [i_0] [5] [5] [14] [i_2] [i_2] [i_4] = 2389052177793085583;
                            }
                        }
                    }
                    arr_19 [i_2] [i_1] = ((var_0 << (((((arr_15 [i_0] [i_1] [14] [i_2] [i_1] [i_1]) ? var_1 : (arr_13 [i_0] [i_1] [i_2]))) - 214))));
                }
            }
        }
    }
    #pragma endscop
}
