/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6715
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6715 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6715
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 ^= (((((var_10 ? var_7 : (((var_6 ? var_0 : var_6)))))) ? ((-(min(var_2, var_10)))) : var_7));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                arr_5 [i_1] = (!208);
                arr_6 [i_0] = (((((((max(var_5, var_7))) ? var_5 : (min(var_4, var_12))))) ? ((((((var_1 ? var_7 : var_9))) ? (var_10 || var_11) : (var_9 && var_9)))) : ((((min(10, 20943))) ? 1 : ((-10 ? 10122425368020151307 : 1))))));
            }
        }
    }
    var_14 = var_5;
    #pragma endscop
}
