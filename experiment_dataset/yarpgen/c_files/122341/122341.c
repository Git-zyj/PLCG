/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122341
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122341 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122341
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_0;
    var_13 = (var_4 - var_8);

    for (int i_0 = 3; i_0 < 14;i_0 += 1)
    {
        var_14 = ((!(arr_3 [i_0 - 1] [i_0])));
        var_15 = (-2147483629 && 130048);
    }
    for (int i_1 = 0; i_1 < 18;i_1 += 1) /* same iter space */
    {
        var_16 += ((~(((arr_7 [i_1] [i_1]) ? (arr_7 [i_1] [i_1]) : (arr_7 [i_1] [i_1])))));
        arr_8 [i_1] = 2147483617;
    }
    for (int i_2 = 0; i_2 < 18;i_2 += 1) /* same iter space */
    {
        var_17 = (arr_9 [i_2] [i_2]);
        var_18 |= (((arr_5 [i_2] [i_2]) ^ ((-(arr_5 [i_2] [i_2])))));
    }
    for (int i_3 = 0; i_3 < 19;i_3 += 1)
    {
        var_19 = (arr_13 [i_3]);
        var_20 = ((-2147483629 % (arr_13 [i_3])));
    }
    var_21 = var_11;
    #pragma endscop
}
