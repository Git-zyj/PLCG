/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123916
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123916 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123916
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_18 ^= (((!var_14) & (min((arr_1 [i_0]), 57760))));
        /* LoopNest 3 */
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 14;i_3 += 1)
                {
                    {
                        var_19 = (7776 + -2097152);
                        var_20 = 57760;
                        arr_10 [i_0] [i_2] [i_0] [i_0] [i_2] [i_0] = (min(((((arr_6 [i_0] [i_1 - 1] [i_2]) || (arr_4 [i_1 + 1] [i_1 + 1])))), (arr_9 [i_1] [i_2] [i_2] [i_3])));
                        var_21 *= ((((((arr_7 [i_0] [i_1 - 1] [i_1 - 1]) ? var_1 : 57759))) ? 7776 : ((29332 ? 1 : 255))));
                    }
                }
            }
        }
    }
    var_22 *= var_7;
    var_23 = 1;
    var_24 = ((~(((~1) ? 13054362373812742230 : (~1)))));
    #pragma endscop
}
