/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229865
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229865 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229865
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 ^= ((/* implicit */unsigned short) (+(((4294967168U) + (((/* implicit */unsigned int) (+(((/* implicit */int) var_13)))))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 4; i_0 < 8; i_0 += 3) 
    {
        var_15 = ((/* implicit */unsigned char) var_11);
        var_16 ^= ((/* implicit */unsigned short) ((((/* implicit */_Bool) arr_0 [i_0 - 2])) ? (((/* implicit */long long int) 128U)) : (arr_1 [i_0 - 1] [i_0 - 4])));
    }
}
