/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40608
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40608 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40608
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(((min(((753652672 ? 1 : var_6)), (4294967269 || var_6)))), (~var_5)));
    var_16 = 4294967269;

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        arr_2 [i_0] = ((-((var_5 ? ((27 ? (arr_1 [i_0] [1]) : (arr_0 [0] [i_0]))) : -22485))));
        arr_3 [i_0] [i_0] = (((max((arr_1 [i_0] [i_0]), var_1)) * 396515499));
    }
    #pragma endscop
}
