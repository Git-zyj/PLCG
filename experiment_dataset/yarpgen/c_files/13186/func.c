/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 13186
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=13186 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/13186
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned int) max((var_15), (((/* implicit */unsigned int) (!((_Bool)1))))));
    var_16 = min((min(((!(((/* implicit */_Bool) var_10)))), (var_12))), ((!(((/* implicit */_Bool) (~(var_6)))))));
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 0; i_0 += 1) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            for (signed char i_2 = 0; i_2 < 24; i_2 += 1) 
            {
                {
                    arr_7 [i_0 + 1] [i_0] [(_Bool)1] = ((/* implicit */_Bool) min((((/* implicit */unsigned long long int) 4294967294U)), (var_4)));
                    var_17 = ((/* implicit */signed char) ((unsigned int) ((((/* implicit */int) arr_2 [i_0 + 1] [i_1] [i_2])) % (((/* implicit */int) ((_Bool) (unsigned char)170))))));
                    /* LoopNest 2 */
                    for (signed char i_3 = 2; i_3 < 22; i_3 += 2) 
                    {
                        for (_Bool i_4 = 1; i_4 < 1; i_4 += 1) 
                        {
                            {
                                var_18 = ((/* implicit */unsigned long long int) max((var_18), (((/* implicit */unsigned long long int) (~(((/* implicit */int) (_Bool)1)))))));
                                var_19 = ((/* implicit */unsigned long long int) arr_6 [i_4 - 1] [i_3] [(unsigned char)23]);
                                var_20 &= ((/* implicit */unsigned int) (unsigned char)254);
                            }
                        } 
                    } 
                }
            } 
        } 
    } 
}
