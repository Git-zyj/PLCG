/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221545
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221545 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221545
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 9;i_2 += 1)
            {
                {
                    var_16 = 10920223357739098809;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 8;i_4 += 1)
                        {
                            {
                                arr_13 [i_0] [i_1] [i_3] = (-9223372036854775807 - 1);
                                var_17 = (arr_6 [i_0] [i_2] [i_4]);
                                arr_14 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0] = var_0;
                                arr_15 [i_0] [i_1] [i_1] [i_1] [i_1] = min((arr_0 [i_0]), ((((arr_0 [i_0]) < (-9223372036854775807 - 1)))));
                            }
                        }
                    }
                    arr_16 [i_0] [i_0] [i_0] = ((var_5 < (arr_3 [2])));
                }
            }
        }
    }
    var_18 = var_6;
    var_19 = (min(var_6, (6589223401693747690 < 9223372036854775807)));
    #pragma endscop
}
