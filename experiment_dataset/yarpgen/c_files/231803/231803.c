/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231803
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231803 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231803
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (1549027920 || var_3);
    var_14 = var_11;
    var_15 = ((var_11 ? var_5 : (((min(var_8, var_12))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            var_16 = arr_1 [i_0];
            var_17 = ((!(arr_5 [i_0])));
            var_18 |= (((arr_5 [i_0]) != var_1));
        }
        arr_6 [i_0] = arr_1 [i_0];
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 15;i_2 += 1)
    {
        var_19 = (((arr_2 [i_2]) ? (arr_1 [i_2]) : (arr_2 [14])));
        arr_9 [i_2] = arr_0 [i_2];
    }
    for (int i_3 = 0; i_3 < 15;i_3 += 1)
    {
        var_20 = ((arr_4 [i_3]) | (min((arr_7 [i_3]), (((arr_2 [i_3]) ? (arr_1 [i_3]) : 22481)))));
        arr_13 [i_3] = ((min((arr_8 [i_3]), (arr_4 [i_3]))));
    }
    #pragma endscop
}
