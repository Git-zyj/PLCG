/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54460
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54460 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54460
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 |= ((492794011 ? -492794011 : 62487));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {

                for (int i_2 = 2; i_2 < 19;i_2 += 1)
                {
                    arr_10 [i_0] = (min(1, ((26172 <= ((min(255, 141)))))));
                    var_11 ^= 112;
                }
                for (int i_3 = 2; i_3 < 19;i_3 += 1)
                {
                    arr_13 [i_0 + 1] [i_0] [i_3] [i_0] = -var_2;
                    arr_14 [i_1] [i_0] [i_1] [i_1] = 200;
                }
                arr_15 [i_0] = 112;
            }
        }
    }

    for (int i_4 = 1; i_4 < 21;i_4 += 1)
    {
        arr_19 [i_4] = (!3197633275067803091);
        var_12 = (-(((!((min(112, 1)))))));

        for (int i_5 = 0; i_5 < 1;i_5 += 1)
        {
            var_13 = (min((max(34343, 1)), (arr_16 [1])));
            arr_23 [i_4] [i_4] [i_5] = -var_9;
        }
    }
    #pragma endscop
}
