/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236247
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236247 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236247
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 *= ((((max(var_3, ((max(var_7, var_9)))))) ? ((((min(var_17, var_10))) ? (2564074642 % var_0) : (!var_11))) : var_11));
    var_19 = var_3;
    var_20 = (max(var_20, var_7));
    var_21 = (min(var_21, (((((max(var_12, ((min(var_17, var_14)))))) ? var_14 : ((min(var_12, var_0))))))));

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_3 [15] [15] |= -12;
        arr_4 [i_0] = (arr_0 [i_0] [3]);
    }
    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        var_22 = (arr_7 [i_1] [i_1]);
        var_23 = -var_8;
        var_24 = var_13;
        var_25 = var_0;
    }
    #pragma endscop
}
