/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125773
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125773 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125773
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned int) min((var_16), (var_2)));
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 18; i_0 += 3) 
    {
        for (unsigned int i_1 = 0; i_1 < 18; i_1 += 3) 
        {
            {
                var_17 = arr_4 [i_0] [i_0] [i_1];
                var_18 = ((/* implicit */unsigned int) max((var_18), (((((/* implicit */_Bool) 837740611U)) ? (536346624U) : (4294967295U)))));
                arr_7 [i_0] = var_1;
                arr_8 [i_0] [i_1] [i_1] = ((((/* implicit */_Bool) ((var_13) << (((arr_1 [i_0]) - (2490396985U)))))) ? (((((/* implicit */_Bool) var_13)) ? (min((var_7), (2605630655U))) : (var_0))) : (min((932589405U), (4155547962U))));
            }
        } 
    } 
}
