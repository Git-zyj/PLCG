/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 113822
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=113822 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/113822
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max(-var_12, -0));

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        arr_3 [i_0] = ((((var_13 >= (arr_2 [i_0])))));
        var_16 = (((((var_11 * (arr_0 [i_0] [i_0]))))) ? (((arr_1 [i_0] [i_0]) / 517978892)) : ((min((arr_1 [i_0] [i_0]), (arr_1 [i_0] [i_0])))));
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        arr_7 [6] [i_1] = (min(((-(((-2147483647 - 1) ? 1 : 0)))), ((((~var_13) > (min(1, 3532569039)))))));
        var_17 = (max(var_17, (arr_5 [i_1] [i_1])));
    }
    #pragma endscop
}
