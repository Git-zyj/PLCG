/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163339
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163339 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163339
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            arr_7 [i_1] [i_1] [i_0] = (min((arr_1 [i_0] [i_1]), ((((arr_6 [i_1]) || (arr_3 [i_0]))))));
            var_17 |= (!780189961);
            var_18 = ((~(arr_5 [i_0] [i_0])));
            arr_8 [i_0] [i_0] |= ((-(((-(arr_2 [i_1]))))));
        }
        arr_9 [i_0] = ((-((min(0, 780189961)))));
        var_19 |= ((((min((arr_6 [i_0]), (arr_6 [i_0])))) && ((!(arr_0 [i_0] [i_0])))));
    }
    var_20 &= (min(((((!var_1) && ((min(var_6, var_12)))))), (min((min(var_3, 656)), var_0))));
    var_21 = var_9;
    #pragma endscop
}
