/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219771
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219771 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219771
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_10 ^= ((var_4 ? ((var_4 ? var_7 : var_6)) : var_9));
                var_11 = var_9;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 11;i_3 += 1)
                    {
                        {
                            var_12 *= var_2;
                            arr_9 [i_0] [i_0] [6] [i_3] |= var_0;
                        }
                    }
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_4 = 0; i_4 < 16;i_4 += 1)
    {
        for (int i_5 = 0; i_5 < 1;i_5 += 1)
        {
            {
                arr_16 [i_5] = (((max(var_8, (var_2 ^ var_5))) & (~57344)));
                var_13 = (~var_6);
                arr_17 [i_4] [i_4] |= ((((var_0 % ((var_0 ? var_0 : var_4))))) ? var_5 : ((((max(var_5, var_7))) ? ((var_1 ? var_3 : var_0)) : (var_0 > var_5))));
            }
        }
    }
    var_14 = var_5;
    var_15 = (max(var_15, var_2));
    #pragma endscop
}
