/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49605
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49605 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49605
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, -14276));

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_18 = (max(var_18, var_3));
                    var_19 = ((max(-19, var_15)));
                }
            }
        }
        var_20 ^= ((var_7 ? (max(-6, -14276)) : (((-1120015112 ? -4 : ((max(10350, 10350))))))));
        var_21 = (((((-(arr_2 [i_0] [i_0] [i_0])))) ? ((var_6 ? var_3 : (max((arr_3 [i_0] [i_0]), 13307)))) : (~55177)));
        var_22 *= var_10;
    }
    #pragma endscop
}
