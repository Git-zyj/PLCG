/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36074
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36074 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36074
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = -150529489;
    var_11 |= (max(var_6, ((150529488 ? var_2 : (min(var_7, var_6))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    var_12 = (min((((!(arr_2 [i_2] [i_0])))), (min((arr_2 [i_2] [i_2]), (arr_2 [i_0] [i_1])))));
                    var_13 = (((arr_3 [i_0] [i_1] [i_2]) ? (arr_3 [i_2] [i_1] [i_0]) : (arr_3 [i_0] [i_1] [i_2])));
                }
            }
        }
    }
    var_14 = (~var_2);
    #pragma endscop
}
