/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135607
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135607 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135607
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 16;i_1 += 1)
        {
            {
                arr_7 [i_0] [i_1] [i_1] = ((((((var_4 ? 0 : 0)))) ? 2763216538 : (arr_0 [i_0])));
                arr_8 [i_0] = (((0 ? (min((arr_0 [i_0]), (arr_1 [i_1] [11]))) : (~212532691))));
            }
        }
    }
    var_11 = ((var_4 ? var_3 : (((((var_8 ? 40198467391278357 : 5046))) ? var_9 : (var_5 / var_6)))));
    #pragma endscop
}
