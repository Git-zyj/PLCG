/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132985
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132985 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132985
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((var_4 ? (min(var_5, var_11) : var_13)));
    var_17 = var_7;
    var_18 = var_0;
    var_19 = (var_11 ? ((18446744073709551615 / (((0 ? var_15 : var_13))))) : var_1);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_20 = (34 * (min((arr_0 [i_0]), ((var_11 ^ (arr_3 [i_0]))))));
                arr_5 [i_1] [6] [6] = ((((min((arr_4 [i_1]), var_1)) - -25118)) + ((((((var_10 ? 0 : (arr_3 [i_0])))) ? -99939247 : (((max(var_15, var_15))))))));
            }
        }
    }
    #pragma endscop
}
