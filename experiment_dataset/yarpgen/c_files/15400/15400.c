/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15400
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15400 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15400
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 += var_8;
    var_11 = var_6;
    var_12 = (8 >= 4088);
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 9;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 10;i_4 += 1)
                        {
                            {
                                var_13 = arr_10 [i_1];
                                var_14 = ((((((arr_0 [i_3 + 1] [i_0 - 3]) ? (arr_0 [i_3 - 1] [i_0 - 3]) : (arr_0 [i_3 - 1] [i_0 - 2]))) + 2147483647)) >> (((((((arr_0 [i_3 + 1] [i_0 - 3]) + 2147483647)) << (((((arr_0 [i_3 + 1] [i_0 - 3]) + 97)) - 15)))) - 2147483549)));
                            }
                        }
                    }
                    arr_14 [i_0] [i_0] [i_1] [i_1] = (arr_2 [i_1] [i_2]);
                }
            }
        }
    }
    var_15 = ((var_4 ? var_9 : var_2));
    #pragma endscop
}
