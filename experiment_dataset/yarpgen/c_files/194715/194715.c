/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194715
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194715 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194715
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        arr_4 [i_0] = (((arr_3 [i_0] [i_0]) <= 8243500099412436441));
        arr_5 [i_0] = (2750056484470391710 ^ 461689613106634510);
        arr_6 [i_0] = ((~(arr_3 [i_0] [i_0])));
    }
    for (int i_1 = 0; i_1 < 23;i_1 += 1)
    {
        var_14 = -26742;
        arr_10 [i_1] [i_1] = ((15696687589239159922 | ((((arr_8 [i_1]) >> (((arr_8 [i_1]) - 238)))))));
    }
    var_15 = ((~((-(32 >= var_1)))));
    var_16 = var_7;
    #pragma endscop
}
