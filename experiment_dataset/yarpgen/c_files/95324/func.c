/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95324
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95324 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95324
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 14; i_0 += 4) 
    {
        var_14 ^= ((/* implicit */unsigned short) ((0) >> (((((/* implicit */int) (unsigned char)35)) - (28)))));
        arr_2 [i_0] = ((/* implicit */_Bool) (unsigned char)247);
    }
    var_15 = ((/* implicit */short) max((var_15), (((/* implicit */short) (unsigned char)200))));
}
