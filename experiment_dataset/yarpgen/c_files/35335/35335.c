/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 35335
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=35335 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/35335
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((var_0 ? var_6 : ((((var_7 ? 2 : var_8))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_11 -= ((!(!-26)));
                var_12 = (var_5 ? ((((!(((-26 ? 2 : var_7))))))) : (((arr_0 [i_0] [i_1]) ^ ((var_4 ? (arr_0 [i_0] [i_1]) : (arr_5 [i_0] [i_1] [i_1]))))));
                arr_6 [i_1] [i_0] = (min((((arr_5 [i_1] [i_1] [i_0]) ? (((arr_3 [i_0]) - var_9)) : -var_4)), (arr_4 [i_0] [i_0] [i_0])));
            }
        }
    }
    #pragma endscop
}
