/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217674
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217674 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217674
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min((min(var_13, (!var_1))), (min((max(0, var_6)), var_1))));
    var_16 = var_12;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                arr_6 [i_0] = (((~var_12) + (min((max(var_3, (arr_3 [i_1] [i_0]))), var_6))));
                var_17 = (max(var_17, (((~((~(arr_2 [1] [22]))))))));
            }
        }
    }
    var_18 = (max(var_18, ((min(((~(~var_12))), (min(((var_14 ? var_2 : 85)), var_1)))))));
    #pragma endscop
}
