/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233544
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233544 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233544
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((((((var_1 ? 22130 : var_12)) & var_0)) >> var_6));
    var_15 = var_13;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 15;i_2 += 1)
            {
                {
                    var_16 = (max(var_16, (((-(6638926152119514619 % 11807817921590036996))))));
                    var_17 = (max(var_17, (((min((arr_4 [i_2 - 1] [i_2 + 1] [i_2] [i_2 - 1]), (arr_4 [i_2] [i_2 - 1] [i_2] [i_2 - 2])))))));
                }
            }
        }
    }
    var_18 = var_5;
    #pragma endscop
}
