/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112902
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112902 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112902
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_12, 8521215115264));
    var_13 = (~var_5);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            {
                var_14 |= (((!(((1 ? var_10 : var_3))))) ? (((arr_5 [i_1 + 1] [i_0] [i_0]) ^ (((125829120 && (arr_4 [i_0] [1] [i_1])))))) : 110);
                var_15 ^= ((var_10 ? (arr_2 [i_0] [i_0]) : ((((!(((-(arr_5 [i_0] [i_1] [i_1]))))))))));
                arr_6 [i_0] [i_0] = ((((((!var_7) / (arr_3 [i_1] [i_1] [8])))) || (~1)));
            }
        }
    }
    var_16 = (min(-15470, 179));
    #pragma endscop
}
