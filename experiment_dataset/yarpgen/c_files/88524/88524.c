/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88524
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88524 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88524
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 22;i_0 += 1) /* same iter space */
    {
        var_12 = (min(var_12, var_9));
        arr_2 [i_0] = (min(((~(!-13046))), ((((var_8 && 51250) != 14274)))));
    }
    for (int i_1 = 0; i_1 < 22;i_1 += 1) /* same iter space */
    {
        arr_6 [i_1] |= (((((var_11 ? (arr_1 [i_1]) : (arr_0 [i_1])))) ? -1003938571171624326 : ((max((((arr_0 [2]) ? var_5 : (arr_5 [i_1]))), ((max((arr_4 [i_1]), var_8))))))));
        arr_7 [i_1] = ((~((~(arr_0 [i_1])))));
        arr_8 [i_1] = var_6;
        var_13 = arr_1 [i_1];
    }
    for (int i_2 = 0; i_2 < 14;i_2 += 1)
    {
        var_14 = ((8924437471652818825 ? 13045 : 2288119439));
        var_15 = arr_1 [i_2];
        var_16 += (arr_9 [12]);
    }
    var_17 = (max(var_10, var_8));
    var_18 = -8924437471652818825;
    #pragma endscop
}
