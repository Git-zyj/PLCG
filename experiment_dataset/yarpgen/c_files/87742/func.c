/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87742
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87742 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87742
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned int i_0 = 2; i_0 < 15; i_0 += 1) 
    {
        for (short i_1 = 0; i_1 < 17; i_1 += 4) 
        {
            {
                var_18 |= ((/* implicit */unsigned long long int) (~(((/* implicit */int) (unsigned short)65535))));
                var_19 = ((/* implicit */unsigned char) (+(((max((-1883971804), (((/* implicit */int) (unsigned short)17849)))) * (((/* implicit */int) arr_2 [i_1] [i_0 - 1]))))));
            }
        } 
    } 
    var_20 = ((/* implicit */unsigned int) var_11);
}
