/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70722
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70722 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70722
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, ((var_2 ? var_1 : ((((-779326464 ? (-2147483647 - 1) : var_5)) & ((var_7 << (var_0 - 1749576092)))))))));
    var_11 = var_4;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_12 = ((arr_4 [i_1] [i_2]) ? (arr_7 [i_2] [i_1] [i_0]) : (arr_7 [i_0] [i_1] [i_2]));
                    arr_9 [i_0] [i_0] [i_2] = (min((min((-2147483647 - 1), var_9)), (arr_4 [i_1] [i_2])));
                }
            }
        }
    }
    var_13 = var_3;
    #pragma endscop
}
