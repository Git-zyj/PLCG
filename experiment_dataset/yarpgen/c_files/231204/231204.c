/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231204
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231204 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231204
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            {
                var_19 ^= (-1783 ? ((min(184, 2771579290))) : 12);
                arr_5 [i_0] [i_0] = (((arr_0 [i_0] [i_1 - 1]) ? ((((arr_3 [i_1 + 1] [i_1 - 1]) == (((arr_3 [i_0] [i_1 + 1]) & 1065151889408))))) : 1));
                arr_6 [2] = (((1 % 16383) ? 179 : ((((-1502090863 > (arr_1 [i_0])))))));
                var_20 += (((max((arr_4 [i_0]), var_10)) ^ ((((((var_6 > (arr_1 [i_0])))) < -1)))));
            }
        }
    }
    var_21 = var_15;
    #pragma endscop
}
