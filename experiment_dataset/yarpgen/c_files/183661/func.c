/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 183661
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=183661 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/183661
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        /* LoopSeq 1 */
        for (unsigned short i_1 = 0; i_1 < 24; i_1 += 2) 
        {
            var_14 = (+(((arr_3 [i_0]) ? (((/* implicit */int) var_12)) : (((/* implicit */int) arr_3 [i_0])))));
            var_15 = ((/* implicit */unsigned short) (_Bool)0);
        }
        /* LoopNest 2 */
        for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
        {
            for (int i_3 = 1; i_3 < 22; i_3 += 1) 
            {
                {
                    var_16 = ((/* implicit */unsigned short) (signed char)1);
                    var_17 = ((/* implicit */unsigned long long int) ((signed char) (unsigned short)21893));
                }
            } 
        } 
        var_18 += ((/* implicit */int) -5870030645874920222LL);
    }
    var_19 = ((/* implicit */int) var_11);
    var_20 = ((/* implicit */int) var_13);
}
