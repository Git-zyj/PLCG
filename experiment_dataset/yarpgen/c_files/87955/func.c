/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87955
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87955 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87955
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (short i_0 = 1; i_0 < 16; i_0 += 2) 
    {
        for (signed char i_1 = 2; i_1 < 15; i_1 += 1) 
        {
            for (long long int i_2 = 1; i_2 < 16; i_2 += 3) 
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 16; i_3 += 2) 
                    {
                        for (signed char i_4 = 4; i_4 < 16; i_4 += 3) 
                        {
                            {
                                var_17 *= ((/* implicit */unsigned char) arr_0 [i_3]);
                                arr_14 [i_0] [i_1 + 2] [i_2 - 1] [i_3] [i_4 - 1] = ((/* implicit */unsigned short) var_11);
                            }
                        } 
                    } 
                    var_18 = (~(((/* implicit */int) (signed char)-125)));
                    var_19 *= ((/* implicit */unsigned int) ((((/* implicit */int) min((((/* implicit */short) (_Bool)1)), ((short)4065)))) / (((/* implicit */int) (signed char)-126))));
                }
            } 
        } 
    } 
    var_20 = ((/* implicit */unsigned char) var_2);
}
