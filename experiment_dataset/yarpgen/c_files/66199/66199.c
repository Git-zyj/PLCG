/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66199
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66199 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66199
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_18;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    var_20 ^= (arr_3 [i_1]);
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 14;i_4 += 1)
                        {
                            {
                                arr_13 [i_0] [i_1] [i_2] [i_0] [i_1] = (((min(1, 18446744073709551594)) == (arr_5 [i_0] [i_1] [i_3])));
                                arr_14 [i_1] [i_2] [i_1] = (((((min((arr_10 [i_1]), var_2)) <= (((var_7 ? (arr_6 [i_2] [i_2]) : 51539607552))))) ? (arr_11 [i_1] [i_1] [i_1] [i_1]) : var_3));
                                var_21 = var_17;
                            }
                        }
                    }
                    var_22 = (min((arr_6 [i_1] [i_1]), (arr_6 [i_0] [i_1])));
                }
            }
        }
    }
    #pragma endscop
}
