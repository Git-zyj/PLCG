/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217692
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217692 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217692
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 15;i_2 += 1)
            {
                {
                    var_19 -= ((254 ? (arr_3 [i_0] [i_0] [i_0]) : (((arr_5 [i_2 + 1] [i_2 - 1] [i_2]) ? -7384 : 0))));
                    arr_7 [i_1] &= min((((!((min((arr_1 [i_1]), (arr_5 [9] [i_0] [i_0]))))))), (min((((!(arr_0 [i_0] [i_2])))), (max((arr_1 [i_2]), (arr_1 [i_1]))))));
                    var_20 = ((((((arr_0 [9] [i_2]) != (arr_3 [i_2] [i_2 + 2] [i_2])))) != (arr_0 [i_0] [9])));
                }
            }
        }
    }
    var_21 = var_15;
    #pragma endscop
}
