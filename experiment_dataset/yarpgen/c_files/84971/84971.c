/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84971
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84971 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84971
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_18 = (((arr_3 [i_1] [6]) ? var_16 : (!var_1)));
                arr_4 [i_1] [i_0] [i_0] = (((!(arr_2 [i_1] [i_0] [i_0]))));
                var_19 = (min(var_19, (((1 ? (arr_3 [i_0] [i_1]) : (arr_0 [i_0]))))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 2; i_2 < 11;i_2 += 1)
    {
        for (int i_3 = 3; i_3 < 11;i_3 += 1)
        {
            {
                var_20 = (arr_2 [i_3 - 3] [i_3] [i_3 + 1]);
                var_21 = var_14;
                var_22 = ((0 ? ((var_16 ? var_15 : (!var_17))) : ((min(var_5, 0)))));
            }
        }
    }
    var_23 += 1;
    var_24 = (!0);
    #pragma endscop
}
