/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168065
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168065 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168065
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        {
                            var_15 = min(1, (((arr_3 [i_2 + 1]) | (arr_3 [i_2 + 1]))));
                            arr_11 [i_0] [i_0 + 1] [i_2] = (((arr_6 [i_3] [i_2 - 1]) != (((arr_8 [i_2 + 1] [i_0 + 2] [i_2] [i_2]) ? ((((var_3 != (arr_4 [i_0] [i_2]))))) : var_6))));
                        }
                    }
                }
                var_16 -= (((((arr_3 [i_0 - 1]) ^ 1)) | (((arr_3 [i_0 + 1]) ? var_6 : 93))));
            }
        }
    }
    var_17 = var_12;
    var_18 = (min(var_18, (~var_4)));
    var_19 = var_6;
    #pragma endscop
}
