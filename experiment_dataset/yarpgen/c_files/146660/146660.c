/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146660
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146660 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146660
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (((((var_1 ? var_15 : var_9))) && ((max(var_4, (max(var_1, 2147483647)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                arr_4 [i_1] [i_1] = ((((var_8 >= var_19) & (((arr_0 [i_1]) ? var_0 : (-2147483647 - 1))))));
                arr_5 [i_0] [i_1] = (((((14567 == var_5) ? var_8 : (arr_0 [i_1]))) > 127));
            }
        }
    }
    var_21 = var_13;
    #pragma endscop
}
