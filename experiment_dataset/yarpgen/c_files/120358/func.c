/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120358
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120358 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120358
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */int) var_14);
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 16; i_0 += 2) 
    {
        for (unsigned short i_1 = 3; i_1 < 14; i_1 += 1) 
        {
            {
                arr_4 [i_1 + 1] = ((/* implicit */int) min((((/* implicit */unsigned long long int) ((((/* implicit */_Bool) arr_2 [i_0] [i_1])) ? (((/* implicit */long long int) (-(-1450854574)))) : (var_0)))), (var_8)));
                var_17 = ((/* implicit */long long int) (-(((/* implicit */int) var_1))));
            }
        } 
    } 
    var_18 = ((/* implicit */unsigned short) max((((/* implicit */int) min(((short)6591), (((/* implicit */short) (!(((/* implicit */_Bool) var_0)))))))), ((~(((/* implicit */int) max((((/* implicit */unsigned short) var_1)), ((unsigned short)63942))))))));
}
