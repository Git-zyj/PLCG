/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226592
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226592 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226592
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 18;i_0 += 1) /* same iter space */
    {
        var_16 = (max(var_16, -14899340));
        arr_4 [i_0] &= ((-(arr_0 [i_0 + 1])));
        var_17 |= var_3;
        arr_5 [i_0] = -60;
        var_18 += 59;
    }
    for (int i_1 = 1; i_1 < 18;i_1 += 1) /* same iter space */
    {
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 21;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 21;i_3 += 1)
            {
                {
                    var_19 = ((-(arr_3 [i_1 + 1] [i_2])));
                    var_20 -= ((((((arr_9 [i_1 + 3]) % (arr_9 [i_1 + 3])))) ? (min(59, (arr_1 [i_2]))) : var_6));
                    arr_15 [i_3] [i_2] [i_3] [i_1 + 3] = 19259;
                }
            }
        }
        arr_16 [i_1 + 3] = (((arr_0 [i_1]) ? (((((71 ? (arr_8 [i_1 + 2]) : -60))) ? (arr_3 [i_1 + 1] [i_1 + 1]) : (arr_13 [i_1 + 3] [i_1] [i_1 - 1]))) : var_13));
    }
    var_21 -= -59;
    #pragma endscop
}
