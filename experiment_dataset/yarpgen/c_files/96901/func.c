/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 96901
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=96901 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/96901
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned int) var_6);
    /* LoopNest 2 */
    for (long long int i_0 = 1; i_0 < 16; i_0 += 3) 
    {
        for (unsigned char i_1 = 0; i_1 < 18; i_1 += 4) 
        {
            {
                var_16 = ((/* implicit */unsigned char) min((var_16), (((/* implicit */unsigned char) (((+(((/* implicit */int) arr_4 [i_0 + 2] [i_0 + 2])))) + (((/* implicit */int) min((arr_4 [i_0 + 2] [i_0 + 2]), (arr_4 [i_1] [i_0 + 1])))))))));
                var_17 = ((/* implicit */signed char) arr_5 [i_1]);
                var_18 ^= ((/* implicit */unsigned short) ((signed char) arr_4 [i_0 - 1] [i_0 + 2]));
                arr_6 [i_0] [12ULL] [i_0] = ((/* implicit */unsigned short) (short)25964);
            }
        } 
    } 
    var_19 = ((/* implicit */unsigned short) ((((/* implicit */_Bool) var_8)) || (((/* implicit */_Bool) 325195642862868887ULL))));
    var_20 = ((/* implicit */_Bool) max((var_20), (((/* implicit */_Bool) var_4))));
}
