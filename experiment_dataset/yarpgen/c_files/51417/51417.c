/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51417
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51417 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51417
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(var_15, 7));

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_16 = (arr_1 [i_0]);

        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            var_17 = (arr_5 [i_1]);
            arr_7 [i_0] [i_1] [i_1] = (~var_14);
            var_18 = (max(var_18, (((-((max((arr_6 [i_0]), (arr_5 [i_1])))))))));
            arr_8 [i_0] [i_0] [i_0] = (((~5234612421326037437) ? ((+(((arr_2 [i_0]) ? (arr_6 [i_0]) : (arr_4 [i_0] [i_0] [i_0]))))) : (arr_1 [i_0])));
            var_19 ^= (~5234612421326037455);
        }
        var_20 ^= -var_3;
    }
    #pragma endscop
}
