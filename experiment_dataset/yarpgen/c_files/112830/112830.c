/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112830
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112830 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112830
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((var_2 ? var_2 : (var_1 >= var_4)));
    var_18 = var_13;

    for (int i_0 = 3; i_0 < 20;i_0 += 1)
    {
        arr_2 [i_0] = (((arr_0 [i_0]) <= (((((arr_1 [i_0 + 1]) ? (arr_1 [i_0]) : (arr_0 [i_0])))))));
        arr_3 [2] &= ((~(((arr_1 [i_0 + 1]) ? ((~(arr_1 [i_0]))) : ((((arr_0 [12]) > (arr_1 [i_0]))))))));
        /* LoopNest 3 */
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 20;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 21;i_3 += 1)
                {
                    {
                        var_19 = (((((-8584607575062419257 ^ (arr_1 [8])))) ? (((arr_4 [i_0] [i_1 - 1]) ? ((((arr_4 [i_0] [10]) || (arr_10 [14] [i_1] [i_0] [i_3])))) : var_11)) : (arr_8 [i_3] [i_1] [i_0])));
                        var_20 = (max(var_20, var_14));
                        var_21 *= var_1;
                    }
                }
            }
        }
    }
    #pragma endscop
}
