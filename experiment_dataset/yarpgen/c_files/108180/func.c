/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 108180
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=108180 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/108180
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */_Bool) var_5);
    var_19 += ((/* implicit */unsigned long long int) (signed char)-14);
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 22; i_0 += 2) 
    {
        for (unsigned int i_1 = 2; i_1 < 21; i_1 += 4) 
        {
            for (short i_2 = 0; i_2 < 23; i_2 += 4) 
            {
                {
                    var_20 = ((/* implicit */short) (~(((var_2) | (((((/* implicit */unsigned long long int) 2147483647)) ^ (5ULL)))))));
                    /* LoopSeq 1 */
                    /* vectorizable */
                    for (long long int i_3 = 3; i_3 < 22; i_3 += 1) 
                    {
                        var_21 = ((/* implicit */unsigned int) ((((((var_0) + (9223372036854775807LL))) << (((var_1) - (3294609344U))))) ^ (((/* implicit */long long int) ((/* implicit */int) var_7)))));
                        var_22 = ((short) var_7);
                    }
                }
            } 
        } 
    } 
}
