/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103464
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103464 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103464
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */short) (+(var_7)));
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 25; i_0 += 2) 
    {
        var_11 &= ((/* implicit */long long int) var_3);
        arr_4 [(short)24] = ((/* implicit */long long int) max((arr_0 [i_0] [i_0]), (((/* implicit */unsigned int) ((arr_0 [i_0] [i_0]) > (((/* implicit */unsigned int) var_1)))))));
        arr_5 [i_0] [i_0] = ((/* implicit */unsigned int) ((unsigned short) (!(((/* implicit */_Bool) arr_2 [i_0])))));
        arr_6 [i_0] = ((/* implicit */long long int) ((((/* implicit */int) (short)28600)) | (((/* implicit */int) (signed char)24))));
    }
    var_12 = ((/* implicit */signed char) var_9);
}
