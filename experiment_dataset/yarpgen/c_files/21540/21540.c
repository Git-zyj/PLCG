/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21540
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21540 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21540
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_7;
    var_16 = var_9;

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_17 = (min((min(var_3, ((var_5 ? var_10 : var_9)))), ((-((-(arr_0 [i_0])))))));
        var_18 -= var_0;
    }
    var_19 = (((((var_7 ? ((min(var_10, var_12))) : (var_6 >= var_9)))) ? (max(var_8, ((var_8 ? var_6 : var_11)))) : var_3));
    var_20 = ((((((((var_5 ? var_1 : var_6))) || ((min(var_0, var_8)))))) < (10 > 65525)));
    #pragma endscop
}
