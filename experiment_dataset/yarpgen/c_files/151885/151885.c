/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151885
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151885 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151885
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_5;
    var_20 = ((var_14 * var_17) / -9271);
    var_21 = var_1;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        var_22 = (min(var_22, (arr_0 [i_0 + 1])));
                        arr_9 [i_0] [i_2] [i_2] [i_3] [i_2] [i_1] = ((((((1 | var_9) + (29062 <= var_6)))) ? 0 : (((((1 ? (arr_2 [i_2] [i_3]) : var_14))) ? var_4 : 1))));
                    }
                    var_23 = (((((9294 | (arr_2 [i_0 + 1] [i_1 + 1])))) ? (((arr_2 [i_0 - 2] [i_1 - 2]) ? 1 : -9287)) : (((arr_2 [i_0 - 2] [i_1 + 1]) + var_9))));
                    var_24 = (max(35, 1));
                }
            }
        }
    }
    #pragma endscop
}
