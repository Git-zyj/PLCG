/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 58164
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=58164 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/58164
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_15 = ((((min((arr_3 [i_0 + 3] [i_0 + 3]), (arr_3 [i_0] [i_0 - 1])))) ? (min((arr_0 [i_0 - 1]), (arr_3 [i_0] [i_0 + 1]))) : (((arr_0 [i_0 + 3]) | (arr_3 [i_0 + 1] [i_0 - 1])))));
                var_16 = (((2935232993 ? (min(1359734282, (arr_6 [i_0]))) : (arr_3 [i_0] [i_0 + 2]))));
            }
        }
    }
    var_17 = var_14;
    var_18 = var_10;
    #pragma endscop
}
