/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222410
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222410 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222410
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 11;i_2 += 1)
            {
                {
                    arr_6 [i_1] [9] = arr_1 [i_2 - 1];
                    var_12 += (min((((-(arr_0 [i_1] [7])))), (min(2037164785, -114))));
                    var_13 = (((arr_4 [i_2 - 1] [i_2 - 1] [i_2 - 2] [1]) ? (((!(((var_1 ? var_8 : var_9)))))) : (((((arr_2 [0] [i_2]) > 2006504096))))));
                    var_14 = (((((max((!1), (max(var_10, 1)))))) - var_8));
                }
            }
        }
    }
    var_15 ^= ((var_1 == (((1 + 0) ? var_9 : ((-52 ? var_7 : -52))))));
    #pragma endscop
}
