/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76837
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76837 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76837
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_8 [i_1] [i_0] [2] |= (min((arr_4 [i_0] [16]), ((min((arr_5 [i_0]), (247 <= var_9))))));
                arr_9 [i_1] = (min(((min((arr_0 [i_0]), (arr_5 [i_1])))), (min((min((arr_6 [i_0]), (arr_7 [i_0] [i_0] [i_1]))), (!var_5)))));
            }
        }
    }
    var_10 = (((!(!177))));
    var_11 = var_0;
    var_12 = (min((((((var_3 ? 4994571167071979376 : var_8))) ? (~var_4) : (~73))), var_1));
    #pragma endscop
}
