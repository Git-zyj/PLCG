/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197386
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197386 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197386
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        arr_3 [1] = ((((((arr_1 [i_0]) ? var_2 : (arr_0 [0])))) || var_2));
        arr_4 [7] = (arr_1 [i_0]);
    }

    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        var_14 = (max(var_14, ((max(116, 1)))));

        for (int i_2 = 0; i_2 < 1;i_2 += 1) /* same iter space */
        {
            arr_11 [7] [i_1] = 0;
            var_15 = (arr_10 [i_2] [i_1]);
        }
        for (int i_3 = 0; i_3 < 1;i_3 += 1) /* same iter space */
        {
            var_16 &= var_11;
            arr_14 [i_1] [i_1] = ((1 ? 1 : 0));
            var_17 = ((((((arr_5 [i_3]) ? (min((arr_12 [i_1] [i_1]), 1)) : (arr_13 [i_1])))) ? (((max(1, 1)))) : (arr_1 [i_1])));
        }
        var_18 = (max(var_18, var_7));
    }
    #pragma endscop
}
