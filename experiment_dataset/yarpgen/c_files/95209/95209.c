/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95209
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95209 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95209
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max(var_13, ((max(((min(1, var_1))), var_9)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_14 = (min(var_14, ((max(18, 29196)))));
                arr_4 [i_0] [i_0] = (55 ? ((-1917 ? (~-32) : (arr_0 [i_0] [i_1]))) : (arr_3 [i_0]));
                var_15 = (min(var_15, (((var_4 ? ((var_5 ? 51 : (arr_0 [i_1] [i_1]))) : ((min((arr_0 [i_1] [i_1]), var_8))))))));
                var_16 -= 55;
            }
        }
    }
    #pragma endscop
}
