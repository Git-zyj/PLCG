/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 118915
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=118915 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/118915
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_3;
    var_20 = ((var_13 > (((var_18 ? var_2 : var_8)))));
    var_21 = (max(32, var_16));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_22 = (!var_10);
                arr_5 [i_0] = (min(32, 9223372036854775807));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 22;i_4 += 1)
                        {
                            {
                                arr_12 [i_0] [i_2] [i_2] [i_3 - 2] [i_3 - 2] = var_16;
                                var_23 = ((((var_18 / (arr_7 [i_2] [i_2] [i_3 - 1]))) / (((arr_7 [i_0] [i_1] [i_3 - 2]) ? var_13 : var_17))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_24 = (min(12, 3306898482));
    #pragma endscop
}
