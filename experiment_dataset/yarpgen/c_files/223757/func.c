/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 223757
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=223757 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/223757
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned short) max((var_16), (((/* implicit */unsigned short) var_2))));
    var_17 = ((/* implicit */unsigned char) var_2);
    var_18 = ((/* implicit */unsigned int) var_14);
    /* LoopNest 2 */
    for (unsigned char i_0 = 0; i_0 < 10; i_0 += 4) 
    {
        for (unsigned char i_1 = 2; i_1 < 8; i_1 += 3) 
        {
            {
                var_19 = ((/* implicit */short) min((var_19), (((/* implicit */short) max((9370854122693727671ULL), (((/* implicit */unsigned long long int) 8299905441254452300LL)))))));
                arr_6 [i_0] [i_0] [i_0] = ((/* implicit */unsigned char) (-(-8299905441254452301LL)));
                var_20 = ((/* implicit */unsigned short) ((((/* implicit */int) (((-(9223372036854775807LL))) < (3364828893844509244LL)))) - (((/* implicit */int) ((((/* implicit */long long int) 1073725440)) <= (-8299905441254452291LL))))));
            }
        } 
    } 
}
