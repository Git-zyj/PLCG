/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 13045
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=13045 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/13045
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_10;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            {
                var_17 = (arr_0 [i_0]);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        {
                            arr_11 [i_0] [14] [1] [i_1] = (arr_10 [i_1] [i_1]);
                            var_18 = (arr_5 [i_1] [i_1] [i_0]);
                            arr_12 [i_1] [i_1] [2] [i_3] [i_0] = (((((arr_9 [i_0] [i_1 - 1] [i_0] [i_3]) * (arr_5 [i_0] [i_1] [i_2]))) == (arr_7 [i_2])));
                            arr_13 [i_1] [i_0] [i_1] [6] [i_2] [i_3] = ((-((((arr_4 [i_2]) >= (arr_3 [7]))))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
