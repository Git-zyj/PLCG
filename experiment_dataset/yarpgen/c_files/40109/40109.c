/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40109
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40109 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40109
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            {
                arr_5 [7] [i_0] = ((~(((arr_4 [i_1 - 1] [i_1 - 1]) % (arr_4 [i_1 - 1] [i_1 - 2])))));
                var_19 = (((max((((arr_0 [i_1 - 2] [1]) * (arr_0 [i_0] [i_1]))), (((arr_2 [i_0] [10] [i_1 - 2]) ? (arr_1 [0]) : (arr_0 [i_0] [3]))))) * ((((arr_2 [i_0] [i_1 - 1] [11]) || (arr_2 [i_0] [i_1 - 2] [i_0]))))));
            }
        }
    }
    var_20 += var_1;
    #pragma endscop
}
