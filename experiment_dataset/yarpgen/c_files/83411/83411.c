/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83411
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83411 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83411
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_7 [i_0] = ((((((var_3 ? (arr_3 [i_0] [i_0]) : (arr_4 [i_0])))) ? ((-(arr_1 [i_0]))) : ((((arr_6 [i_0]) > 13))))));
                var_17 = ((arr_1 [i_0]) ? -3098973410366417489 : ((((var_0 ? var_6 : var_6)) / (max((arr_3 [i_0] [i_0]), 9223372036854775807)))));
            }
        }
    }
    var_18 = -12792;
    var_19 &= var_9;
    #pragma endscop
}
