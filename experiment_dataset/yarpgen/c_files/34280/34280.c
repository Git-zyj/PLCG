/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34280
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34280 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34280
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min(0, ((var_11 ? var_0 : var_3))));
    var_13 = 0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_14 = (min(var_14, ((((((var_7 << (var_3 - 164)))) ? ((((((7 ? 4 : 0)) >= var_8)))) : 8096132026945894220)))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        {
                            var_15 = (min((arr_2 [i_2]), 15306));
                            var_16 = ((4053941912 == (((-(arr_2 [i_0]))))));
                            var_17 = ((((((arr_3 [1]) > (arr_1 [i_0]))))) == ((var_3 + (arr_2 [i_0]))));
                        }
                    }
                }
                var_18 |= -1270017289;
            }
        }
    }
    #pragma endscop
}
