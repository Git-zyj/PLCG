/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196505
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196505 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196505
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */short) ((((/* implicit */unsigned int) ((/* implicit */int) var_10))) - ((+(var_6)))));
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 24; i_0 += 1) 
    {
        for (long long int i_1 = 3; i_1 < 20; i_1 += 1) 
        {
            {
                /* LoopNest 3 */
                for (long long int i_2 = 3; i_2 < 22; i_2 += 4) 
                {
                    for (long long int i_3 = 2; i_3 < 22; i_3 += 2) 
                    {
                        for (unsigned char i_4 = 2; i_4 < 22; i_4 += 2) 
                        {
                            {
                                var_17 = (!(((/* implicit */_Bool) 9049977493691330662LL)));
                                var_18 += ((/* implicit */unsigned short) (+(((((/* implicit */int) (short)-10309)) / (((/* implicit */int) (_Bool)1))))));
                            }
                        } 
                    } 
                } 
                var_19 ^= ((/* implicit */unsigned long long int) arr_12 [19] [i_0] [(unsigned char)23] [i_0] [6U] [(_Bool)1] [i_1 + 4]);
            }
        } 
    } 
}
