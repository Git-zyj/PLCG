/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63253
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63253 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63253
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_4 [i_1] [i_0] = -var_6;
                var_17 = (((arr_2 [i_1] [i_0]) >= (arr_2 [i_0] [i_0])));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 2; i_2 < 18;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 22;i_3 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_4 = 2; i_4 < 20;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 22;i_5 += 1)
                    {
                        {
                            arr_19 [i_5] [i_2] [1] [i_5] = (((~(arr_7 [i_5]))));
                            arr_20 [i_5] [i_5] [i_2] [i_5] = var_8;
                            arr_21 [i_3] [i_2] [i_3] = (arr_10 [i_2 + 3] [i_3] [i_3]);
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_6 = 0; i_6 < 1;i_6 += 1)
                {
                    for (int i_7 = 0; i_7 < 1;i_7 += 1)
                    {
                        {
                            var_18 = (((((arr_7 [i_3]) >= var_14)) ? (~38) : (arr_10 [i_2] [i_3] [i_6])));
                            var_19 = var_13;
                            var_20 = var_6;
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
