/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144489
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144489 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144489
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((-var_9 ? 4483846941963661794 : 34358689792)));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    var_12 = (((arr_7 [i_0 + 3] [i_0 + 2] [i_0 + 1]) ? (arr_7 [i_2] [i_1 + 1] [i_0]) : (max((arr_5 [i_1 - 3] [i_1 - 3] [i_1 + 3]), 2032463172))));
                    arr_8 [i_0] = (arr_1 [12]);
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_16 [i_0] [i_1] [i_2] [i_0] [i_4] = (min((((-(-127 - 1)))), ((-(arr_0 [i_1 - 1])))));
                                var_13 -= (arr_11 [i_4] [i_3] [i_2] [i_1]);
                                var_14 = ((arr_4 [i_1 - 2] [i_1 - 2]) | (min((arr_4 [i_4] [i_1 + 3]), (arr_4 [i_4] [i_2]))));
                            }
                        }
                    }
                    arr_17 [i_0] [i_1] [i_2] = -1;
                }
            }
        }
    }
    var_15 = (((var_0 < var_9) + var_7));
    #pragma endscop
}
