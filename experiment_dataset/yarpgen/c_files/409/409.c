/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 409
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=409 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/409
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        arr_2 [0] [i_0] = max((max((arr_1 [i_0]), (arr_0 [i_0]))), ((max(var_14, (arr_1 [i_0])))));
        var_16 = (min(var_16, ((min((min((arr_0 [i_0]), (arr_0 [i_0]))), ((min((arr_1 [i_0]), (arr_1 [i_0])))))))));
    }
    var_17 = ((-((-(min(var_3, var_15))))));
    var_18 = (var_14 ^ 4653291898766835730);
    #pragma endscop
}
