/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 141262
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=141262 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/141262
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min(((-((min(0, 119))))), (max(((var_1 ? var_11 : 1)), var_0))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 20;i_1 += 1)
        {
            {
                var_14 |= var_6;
                var_15 = ((~(min(((max(16384, 1))), (((arr_2 [i_0]) ? 18461 : 0))))));
                var_16 = ((~((var_10 ^ (max(var_10, var_6))))));
            }
        }
    }
    var_17 = ((var_8 ? ((((5745 ? 0 : 0)))) : ((var_6 ? var_7 : var_5))));
    var_18 = ((var_4 ? ((max(var_6, var_9))) : (2912318740 && 1)));
    #pragma endscop
}
