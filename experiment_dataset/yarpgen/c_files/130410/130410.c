/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130410
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130410 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130410
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [i_2] [i_1] [i_2] = ((1 >> (65518 - 65492)));
                    var_14 = (((((-(((arr_0 [i_2]) ? 1 : 20))))) ? ((((!(arr_2 [i_0] [i_1]))))) : (((arr_0 [1]) - ((2787628929 ? (arr_3 [i_0] [i_0] [i_0]) : 2877063540407574757))))));
                    var_15 -= -13;
                }
            }
        }
    }
    var_16 = (!-18700);
    var_17 &= ((var_4 >> ((((-27530 ^ var_2) != var_0)))));

    /* vectorizable */
    for (int i_3 = 1; i_3 < 13;i_3 += 1)
    {
        var_18 -= ((!(arr_7 [i_3 + 1])));
        var_19 -= (((arr_7 [i_3 - 1]) > (arr_8 [i_3 + 2])));
        arr_9 [2] = (arr_8 [9]);
    }
    #pragma endscop
}
