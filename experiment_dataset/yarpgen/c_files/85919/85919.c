/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85919
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85919 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85919
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        var_17 -= 18446744073709551609;
        arr_2 [i_0] = (arr_1 [i_0]);
    }
    /* vectorizable */
    for (int i_1 = 1; i_1 < 12;i_1 += 1)
    {
        var_18 -= ((1 & ((var_10 ? var_14 : 1))));

        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {

            for (int i_3 = 2; i_3 < 13;i_3 += 1)
            {
                var_19 = (max(var_19, (arr_6 [i_1 - 1])));
                var_20 -= ((arr_1 [i_3]) % var_3);
                var_21 = (((arr_3 [i_2]) * (arr_9 [i_2] [i_2] [i_2] [i_1 + 2])));
                arr_11 [i_3] [i_3] [i_3 - 1] = var_3;
                var_22 += ((((((arr_5 [i_1] [i_2]) ? var_12 : 1))) ? var_1 : var_13));
            }
            arr_12 [i_1] = arr_3 [i_1 + 2];
        }
        var_23 = (((arr_9 [i_1] [i_1] [i_1 + 1] [i_1]) >> (arr_9 [i_1] [i_1 + 1] [i_1 + 2] [i_1])));
    }
    var_24 = 1;
    #pragma endscop
}
