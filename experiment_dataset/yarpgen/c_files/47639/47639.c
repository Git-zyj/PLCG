/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47639
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47639 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47639
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max(var_13, var_6));
    var_14 = var_7;
    var_15 ^= (min((max((var_6 < var_3), var_10)), (!15541)));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_16 = ((var_3 ? (arr_0 [i_0]) : -15542));

        for (int i_1 = 1; i_1 < 22;i_1 += 1) /* same iter space */
        {
            /* LoopNest 3 */
            for (int i_2 = 0; i_2 < 0;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 23;i_3 += 1)
                {
                    for (int i_4 = 0; i_4 < 23;i_4 += 1)
                    {
                        {
                            arr_12 [15] [i_0] [i_2] [i_2] = ((((((arr_8 [i_0] [i_1 + 1] [i_2] [i_3]) ? -15530 : var_7))) ? 430379066692310497 : 248));
                            arr_13 [i_0] [i_3] [i_2 + 1] [i_1] [i_0] = ((var_5 ? (arr_5 [i_1 + 1]) : (arr_7 [i_0] [i_1 + 1] [17] [i_2 + 1] [i_2] [i_2])));
                        }
                    }
                }
            }
            var_17 = -116;
            arr_14 [i_0] = var_3;
        }
        for (int i_5 = 1; i_5 < 22;i_5 += 1) /* same iter space */
        {
            arr_17 [i_0] [i_5] = var_1;
            var_18 = ((-116 ? var_12 : (-2 ^ 173)));
        }
    }
    #pragma endscop
}
