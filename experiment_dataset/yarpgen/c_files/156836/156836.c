/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156836
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156836 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156836
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 ^= (max(var_6, ((~(((var_6 >= (-127 - 1))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [i_0] = (((arr_2 [i_0]) && ((((arr_4 [i_0]) ? (((arr_4 [i_0]) % -113)) : ((121 ? 62 : (-127 - 1))))))));
                var_19 = (min(((((arr_4 [i_0]) > var_3))), (((arr_2 [i_1]) ? (arr_3 [i_0]) : (~-81)))));
                arr_6 [i_0] = min((min(28, 79)), -49);
                var_20 ^= var_13;
            }
        }
    }
    var_21 ^= var_5;
    #pragma endscop
}
