/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 170541
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=170541 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/170541
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 *= ((/* implicit */unsigned long long int) max(((+(((/* implicit */int) var_13)))), (((/* implicit */int) var_2))));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 3; i_0 < 13; i_0 += 1) 
    {
        var_15 = ((/* implicit */short) min((var_15), (((/* implicit */short) ((unsigned int) min((((/* implicit */long long int) min((((/* implicit */unsigned short) (_Bool)1)), (arr_1 [i_0 + 2])))), ((+(-3955871254420725134LL)))))))));
        arr_3 [i_0] = ((/* implicit */unsigned char) arr_1 [i_0]);
    }
}
