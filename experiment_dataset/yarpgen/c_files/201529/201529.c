/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 201529
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=201529 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/201529
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = 937143713;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_20 = (((var_12 & var_12) && ((((arr_3 [i_0] [i_0] [i_1]) ? (1838296058122930266 & 28604) : 46835)))));
                var_21 = ((var_7 ? (arr_4 [i_0] [i_0] [i_1]) : (18685 * 7)));
            }
        }
    }
    var_22 = var_17;
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 1;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 22;i_3 += 1)
        {
            {
                arr_10 [i_2] [i_2] [i_2] = ((!(((arr_7 [i_2]) != (((arr_6 [i_2] [i_3]) | 1555918439))))));
                var_23 = (((var_16 < (arr_8 [i_2]))));
                var_24 = (arr_9 [i_2]);
            }
        }
    }
    var_25 = -var_12;
    #pragma endscop
}
