/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36741
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36741 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36741
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_17 = ((((min(var_7, var_1))) ? (((((var_8 ? var_12 : var_7))) ? ((1586070007 ? var_4 : 1)) : (var_10 ^ var_6))) : (((var_4 ? var_16 : var_2)))));
                arr_5 [i_0] [i_0 + 1] = var_8;
            }
        }
    }
    var_18 = ((var_3 ? (min((max(var_7, var_9)), var_12)) : ((min(var_14, var_13)))));
    var_19 = (((((min(var_4, var_1))) ? var_8 : (var_5 % var_15))));
    #pragma endscop
}
