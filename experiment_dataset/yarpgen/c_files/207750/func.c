/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 207750
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=207750 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/207750
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */unsigned short) var_11);
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 12; i_0 += 1) 
    {
        for (unsigned short i_1 = 1; i_1 < 10; i_1 += 2) 
        {
            {
                arr_4 [i_0] = ((/* implicit */unsigned int) (((~(((((/* implicit */int) var_6)) & (((/* implicit */int) (unsigned short)9354)))))) > (((/* implicit */int) ((short) arr_2 [i_0] [i_1] [i_1])))));
                arr_5 [i_0] [i_0] [i_0] |= ((/* implicit */unsigned short) (+(((int) (unsigned short)56181))));
            }
        } 
    } 
    var_14 = ((/* implicit */_Bool) (+(max((((/* implicit */int) var_7)), (-756826040)))));
}
