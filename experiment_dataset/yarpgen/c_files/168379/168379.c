/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168379
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168379 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168379
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_3;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 12;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_13 = ((((((max(var_3, var_0))) ? ((min(var_9, var_4))) : (1295254419 | 2147475456)))) ? (!var_5) : (min(var_11, var_0)));
                                var_14 = (max(var_14, (((-(min((((var_4 ? var_9 : var_5))), (var_10 - var_11))))))));
                            }
                        }
                    }
                    arr_13 [i_0] [i_2] = var_3;
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 14;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 14;i_6 += 1)
                        {
                            {
                                var_15 = var_11;
                                var_16 = (((!var_7) ? ((((min(2147483647, 1295254432))) ? var_2 : ((72057594037927935 ? 2147483647 : 1)))) : (((1133 ? 1 : -2147475448)))));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
