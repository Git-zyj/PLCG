/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191193
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191193 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191193
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 8;i_0 += 1)
    {
        var_14 = -2527706497172965471;
        var_15 = (arr_1 [5]);
        arr_2 [i_0] = var_8;
    }
    /* vectorizable */
    for (int i_1 = 2; i_1 < 15;i_1 += 1)
    {
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 17;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 17;i_3 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_4 = 1; i_4 < 14;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 1;i_5 += 1)
                        {
                            {
                                var_16 = ((var_7 ? (arr_12 [i_1] [i_2] [i_3] [i_1]) : ((73 ? (arr_7 [i_1 - 1] [i_2]) : var_2))));
                                var_17 = var_2;
                            }
                        }
                    }
                    arr_21 [i_1] [3] [i_1] = ((-(arr_9 [i_1 - 2] [i_1 + 2] [i_1 - 1])));
                    arr_22 [i_1] [i_2] [i_1] [1] = ((!(arr_7 [10] [i_2])));
                    var_18 = arr_17 [i_1] [i_2] [i_2] [i_2] [i_3];
                    var_19 = ((~(arr_19 [i_1 - 2] [i_1 - 1])));
                }
            }
        }
        /* LoopNest 3 */
        for (int i_6 = 1; i_6 < 15;i_6 += 1)
        {
            for (int i_7 = 0; i_7 < 17;i_7 += 1)
            {
                for (int i_8 = 3; i_8 < 13;i_8 += 1)
                {
                    {
                        var_20 = -2119442204;
                        var_21 = var_11;
                    }
                }
            }
        }
    }
    var_22 = var_9;
    var_23 = ((var_13 ? ((-73 ? var_5 : var_3)) : (max(((13149 >> (var_13 - 19))), -74))));
    #pragma endscop
}
