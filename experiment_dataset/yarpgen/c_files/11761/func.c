/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11761
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11761 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11761
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned long long int) max((var_15), (((/* implicit */unsigned long long int) var_5))));
    var_16 -= var_7;
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 25; i_0 += 4) 
    {
        var_17 = ((/* implicit */long long int) 24U);
        var_18 *= ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) ((((/* implicit */_Bool) arr_0 [i_0])) ? (arr_0 [i_0]) : (arr_0 [i_0])))) ? (((((/* implicit */_Bool) var_11)) ? (arr_0 [i_0]) : (arr_0 [i_0]))) : (((((/* implicit */_Bool) arr_1 [i_0])) ? (arr_0 [i_0]) : (var_6)))));
    }
    var_19 = ((/* implicit */int) (~(min((((/* implicit */long long int) -1819138177)), (var_7)))));
}
