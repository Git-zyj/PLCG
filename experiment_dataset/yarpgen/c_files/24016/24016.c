/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24016
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24016 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24016
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 22;i_1 += 1)
        {
            {
                var_10 = (~var_2);
                var_11 = (((~117) ? (((96 >> 1) * (arr_2 [i_0] [i_0] [5]))) : (arr_2 [i_0 + 1] [i_1] [3])));
                var_12 += max(((((arr_2 [i_0] [i_1] [i_0 + 1]) ? (arr_2 [i_0] [i_0] [i_0 + 1]) : (arr_2 [i_0] [i_1 - 2] [i_0 + 1])))), 4001786850);
            }
        }
    }
    var_13 = ((((var_5 ? (var_3 % 15) : var_7)) * (var_6 || var_2)));
    var_14 = (~var_6);
    var_15 = var_7;
    #pragma endscop
}
