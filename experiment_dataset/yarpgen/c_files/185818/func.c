/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 185818
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=185818 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/185818
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned short) min((var_11), (((/* implicit */unsigned short) min((((/* implicit */unsigned int) var_10)), (max((1273596719U), (((/* implicit */unsigned int) -1)))))))));
    var_12 = ((/* implicit */long long int) min((var_12), (((/* implicit */long long int) var_3))));
    /* LoopSeq 1 */
    for (unsigned char i_0 = 0; i_0 < 17; i_0 += 1) 
    {
        var_13 += max(((short)-17828), ((short)31644));
        arr_4 [i_0] = ((/* implicit */long long int) 251658240U);
    }
    var_14 ^= ((/* implicit */unsigned short) var_5);
}
