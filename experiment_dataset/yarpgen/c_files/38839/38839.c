/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38839
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38839 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38839
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max((0 ^ 5309554652721527786), ((((((var_5 ? -5309554652721527780 : var_10))) ? var_2 : (var_11 ^ var_5))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_1] [i_1] = ((-((min(((min((arr_1 [i_0]), var_6))), var_0)))));
                arr_6 [i_0] = var_9;
                arr_7 [i_0] [i_1] [0] = var_1;
            }
        }
    }
    #pragma endscop
}
