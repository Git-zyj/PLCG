/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202944
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202944 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202944
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */long long int) ((34359738367ULL) | (((((/* implicit */_Bool) var_5)) ? (((/* implicit */unsigned long long int) (~(((/* implicit */int) var_4))))) : (((4398046511103ULL) | (((/* implicit */unsigned long long int) var_0))))))));
    /* LoopNest 2 */
    for (unsigned short i_0 = 3; i_0 < 15; i_0 += 3) 
    {
        for (unsigned int i_1 = 2; i_1 < 15; i_1 += 4) 
        {
            {
                var_11 = ((/* implicit */unsigned int) (_Bool)1);
                var_12 *= ((/* implicit */signed char) var_9);
            }
        } 
    } 
}
