/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211457
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211457 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211457
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        arr_3 [i_0] = (!13891319652820083940);
        var_13 -= (((min(((var_6 ? (arr_2 [i_0]) : var_0)), var_2)) & (arr_0 [i_0])));
        arr_4 [i_0] = min(((((var_12 ^ var_12) ? (arr_2 [i_0]) : var_7))), (((((((arr_2 [i_0]) <= var_6)))) ^ (var_8 ^ 603995664))));
        var_14 += (~var_12);
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 11;i_1 += 1)
    {
        arr_7 [i_1] [i_1] = (((arr_5 [i_1]) ? ((((arr_5 [i_1]) ? 8 : (arr_0 [i_1])))) : (arr_6 [i_1])));
        var_15 ^= var_9;
        arr_8 [i_1] [i_1] = (((arr_6 [i_1]) ? (arr_6 [i_1]) : (arr_6 [i_1])));
        var_16 &= var_3;
        var_17 = -4032;
    }
    var_18 = (~var_5);
    var_19 = ((var_3 ? (((((min(25397, 61510)))) | (65507 ^ 3177277842))) : var_7));
    #pragma endscop
}
