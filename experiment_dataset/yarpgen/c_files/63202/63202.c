/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63202
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63202 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63202
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((var_12 / ((((max(var_6, -113))) ? (var_6 & var_13) : var_0))));
    var_15 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_16 *= (min((min((((-3 ? -102 : var_1))), var_2)), (((arr_3 [i_0] [i_0 + 1]) ? var_0 : var_8))));
                var_17 = (max(var_17, (((var_10 << (((arr_2 [i_0] [i_0]) + 6427570335892542773)))))));
                arr_5 [i_0] = var_2;
            }
        }
    }
    var_18 ^= (max(((max(var_6, var_10))), (((!(((var_7 ? var_9 : var_6))))))));
    #pragma endscop
}
