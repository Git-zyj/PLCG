/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 126974
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=126974 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/126974
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((var_5 + (var_8 * var_5)));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            {
                var_11 = ((((~(arr_1 [i_0] [i_1 + 3]))) <= (((arr_1 [10] [i_1 + 3]) ? (arr_1 [11] [i_1 + 2]) : (arr_3 [i_1 + 3])))));
                arr_6 [15] [2] = (arr_0 [i_0]);
            }
        }
    }
    var_12 = (max(var_12, ((((((var_0 ? var_6 : var_1))) >= var_3)))));
    var_13 = (min(var_13, (var_3 / var_0)));
    #pragma endscop
}
