/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102894
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102894 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102894
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (1022 / var_0);
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_19 = ((arr_1 [i_0]) ? (((22644 ^ 11) * 11)) : (arr_2 [i_0 - 1]));
                var_20 = (min(((((46763 <= 111) < (arr_5 [i_1 + 1] [i_0] [i_0 - 1])))), (~var_6)));
                var_21 = (max(((((arr_3 [i_1]) == (arr_2 [i_0 - 1])))), (((((arr_5 [i_0] [i_0] [i_0]) || (arr_2 [4]))) ? (((!(arr_2 [i_0])))) : (!0)))));
            }
        }
    }
    var_22 = -15;
    #pragma endscop
}
