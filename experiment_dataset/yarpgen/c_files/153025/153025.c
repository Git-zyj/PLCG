/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 153025
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=153025 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/153025
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((-((28269 << (var_14 && var_15)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    var_19 = 114;
                    arr_8 [i_1] [i_1] = ((-(arr_5 [2] [14] [2])));
                    arr_9 [i_1] [i_1] [i_1] = ((!((((arr_5 [i_1 - 2] [i_1 + 3] [i_1 + 1]) ? 1 : 9223372036854775807)))));
                }
            }
        }
    }
    var_20 |= ((var_2 ? var_17 : ((min(var_13, var_16)))));
    #pragma endscop
}
