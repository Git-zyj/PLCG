/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146020
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146020 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146020
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (-2147483647 - 1);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 10;i_4 += 1)
                        {
                            {
                                arr_11 [i_1 + 1] [i_2] [i_3] = 1;
                                arr_12 [i_4 - 2] [i_3] [4] [i_1 - 1] [4] |= ((!(((~(arr_2 [i_3 - 1]))))));
                            }
                        }
                    }
                    arr_13 [i_1] = (max((!var_5), ((-(max(-2191, var_10))))));
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 11;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 1;i_6 += 1)
                        {
                            {
                                var_21 += (min(1, 2191));
                                arr_18 [i_0] [i_1] [i_1] [i_5] [i_6] = (min(((16686349523644858152 ? -2183 : 24028208)), 32256));
                                var_22 -= (arr_14 [i_0] [i_0] [i_0] [i_0] [10]);
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
