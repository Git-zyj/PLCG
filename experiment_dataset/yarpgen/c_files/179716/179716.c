/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179716
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179716 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179716
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(((var_1 ? ((var_14 ? var_13 : 0)) : (max(61297, 390705565)))), var_12));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 4; i_2 < 6;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 7;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 10;i_4 += 1)
                        {
                            {
                                arr_14 [i_1] = (arr_12 [i_0] [i_2 - 2] [i_3 - 2] [i_0] [i_2 - 2] [i_4]);
                                var_16 = (arr_2 [i_0] [i_1]);
                                var_17 = max((arr_0 [i_0]), (((((var_0 ? (arr_11 [i_0] [i_0] [i_2 + 3] [i_3] [i_1]) : 0))) % (1 ^ var_12))));
                            }
                        }
                    }
                }
                arr_15 [i_0] [4] |= ((((max(30591, 30588))) ? (~var_8) : -var_10));
            }
        }
    }
    #pragma endscop
}
