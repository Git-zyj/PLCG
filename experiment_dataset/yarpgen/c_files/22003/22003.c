/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22003
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22003 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22003
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = 1313675421111109217;
    var_17 |= (max((((var_1 != ((min(var_9, var_14)))))), -var_7));
    var_18 = ((((max(((var_8 ? 1 : 104)), (~-398228086)))) & 17133068652598442399));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    var_19 &= (arr_1 [i_0]);
                    arr_7 [i_1 + 2] [i_1] [i_2] = ((~((((min(var_13, 4503582447501312))) ? (((var_6 + 9223372036854775807) >> (arr_2 [i_0] [i_0]))) : ((max(3, 3)))))));
                }
                arr_8 [i_0] [i_0] |= (~var_8);
            }
        }
    }
    #pragma endscop
}
