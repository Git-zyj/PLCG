/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74892
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74892 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74892
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned int) var_1);
    var_12 ^= ((/* implicit */_Bool) var_3);
    var_13 = ((/* implicit */int) var_1);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24; i_0 += 4) 
    {
        for (unsigned int i_1 = 4; i_1 < 21; i_1 += 2) 
        {
            {
                arr_6 [i_0] [i_1] = ((/* implicit */signed char) ((unsigned char) ((((/* implicit */_Bool) (signed char)-101)) ? (((/* implicit */int) (short)3963)) : (((/* implicit */int) (signed char)103)))));
                var_14 = ((signed char) (!(((/* implicit */_Bool) (signed char)23))));
            }
        } 
    } 
}
