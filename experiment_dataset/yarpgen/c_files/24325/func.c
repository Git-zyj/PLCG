/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24325
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24325 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24325
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (signed char i_0 = 0; i_0 < 11; i_0 += 3) 
    {
        for (unsigned short i_1 = 0; i_1 < 11; i_1 += 2) 
        {
            {
                arr_5 [i_1] [i_1] [10U] = ((/* implicit */unsigned char) var_4);
                var_10 = ((/* implicit */signed char) max((var_10), (((/* implicit */signed char) var_2))));
                var_11 = ((/* implicit */unsigned short) var_1);
            }
        } 
    } 
    var_12 = ((/* implicit */signed char) min((var_12), (((/* implicit */signed char) (((!(((/* implicit */_Bool) min((((/* implicit */unsigned short) (short)7936)), (var_8)))))) ? ((-(var_0))) : (((/* implicit */int) (!(((/* implicit */_Bool) var_0))))))))));
}
