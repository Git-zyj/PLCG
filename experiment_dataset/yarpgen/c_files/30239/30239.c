/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30239
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30239 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30239
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_20 = ((var_6 ? var_0 : ((((((2147483647 ? -77252364 : (arr_1 [2])))) ? (((arr_1 [i_0]) ? (arr_2 [i_0]) : (arr_2 [i_0]))) : (arr_1 [i_0]))))));
        arr_4 [i_0] = (arr_3 [i_0]);
    }
    var_21 -= var_8;
    /* LoopNest 3 */
    for (int i_1 = 1; i_1 < 22;i_1 += 1)
    {
        for (int i_2 = 2; i_2 < 22;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 25;i_3 += 1)
            {
                {
                    var_22 = (max(var_22, (~91)));
                    var_23 -= ((-2147483638 / (-127 - 1)));
                    arr_14 [i_1] [i_3] [i_1] [i_1] = ((!(((arr_11 [i_1 + 3]) || (arr_11 [i_1 + 2])))));
                    arr_15 [20] [i_1] [i_3] [i_1] = var_15;
                }
            }
        }
    }
    #pragma endscop
}
