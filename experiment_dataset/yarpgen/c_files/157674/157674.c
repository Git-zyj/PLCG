/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157674
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157674 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157674
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_11;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_13 = ((-((min(var_3, var_6)))));
                arr_5 [i_0] = ((max(1, -1373279829)));
                arr_6 [i_0] [i_0] = ((((((arr_4 [i_0]) ? (arr_4 [i_1]) : 11))) ? (min(var_11, (min(10, (arr_0 [i_0]))))) : (((((arr_3 [i_0] [i_0] [i_0]) > var_9))))));
                arr_7 [i_0] [i_0] |= var_1;
            }
        }
    }
    var_14 += (min(((min(var_3, (min(var_3, var_3))))), var_1));
    #pragma endscop
}
