/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 216377
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=216377 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/216377
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */signed char) min(((-(((/* implicit */int) var_11)))), (((/* implicit */int) var_13))));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 4; i_0 < 7; i_0 += 3) 
    {
        var_21 = ((/* implicit */long long int) min(((-(((/* implicit */int) (_Bool)1)))), (((/* implicit */int) ((((/* implicit */_Bool) arr_0 [i_0 - 4])) || (((/* implicit */_Bool) arr_0 [i_0 + 1])))))));
        var_22 = ((/* implicit */long long int) (!(((/* implicit */_Bool) arr_0 [(_Bool)1]))));
    }
}
