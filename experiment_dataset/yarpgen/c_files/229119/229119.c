/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229119
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229119 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229119
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 ^= ((var_5 ? (((max(var_8, 14463820366874286997)) - var_2)) : 0));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_12 = ((((((arr_2 [i_1]) ? 0 : (arr_2 [i_0])))) - (max(566391711780803058, 18446744073709551611))));
                var_13 = ((0 ? (566391711780803059 || var_3) : (max((arr_1 [i_0]), (arr_3 [i_0])))));
                var_14 = -98059243301581924;
            }
        }
    }
    #pragma endscop
}
