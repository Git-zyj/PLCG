/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 129
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=129 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/129
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_20 = 135;
                arr_4 [i_1] = var_12;
                var_21 ^= (((((7893611776365814749 ? 8188 : 8180))) * ((127 ? 1421107958 : 14141527677409757556))));
                arr_5 [i_0] [i_0] = ((max(var_3, (((var_1 >> (var_6 - 22193)))))));
                var_22 = ((~(var_8 >= var_3)));
            }
        }
    }
    var_23 *= ((~(~var_3)));
    var_24 = ((((((((var_6 ? var_17 : var_4)) != var_14)))) & (max((((2873859329 ? var_19 : var_6))), 13271358113063678801))));
    #pragma endscop
}
