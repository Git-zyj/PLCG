/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 147809
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=147809 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/147809
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 17;i_0 += 1) /* same iter space */
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 15;i_3 += 1)
                {
                    {
                        var_16 = (max(var_16, ((((~-3335066009341894387) & (~var_9))))));
                        var_17 -= (((((-(arr_6 [i_1] [i_3 - 2] [i_2] [i_3])))) ? (arr_8 [i_3] [i_1]) : ((1 ? (arr_0 [15] [i_1]) : (arr_0 [i_0] [i_0])))));
                    }
                }
            }
        }
        var_18 = 1;
    }
    for (int i_4 = 0; i_4 < 17;i_4 += 1) /* same iter space */
    {
        var_19 -= (((1 ? var_8 : var_11)));
        var_20 = (min(var_1, (((arr_2 [i_4] [i_4]) & 0))));
        var_21 = (min(var_21, ((min((((~(arr_2 [8] [i_4])))), (((arr_2 [6] [i_4]) ? var_12 : (arr_2 [2] [i_4]))))))));
        var_22 = (arr_2 [i_4] [i_4]);
    }
    var_23 = (!var_9);
    #pragma endscop
}
