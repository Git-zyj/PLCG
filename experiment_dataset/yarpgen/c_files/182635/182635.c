/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182635
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182635 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182635
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(var_16, var_4));
    var_17 ^= (0 | 1825450736);
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            {
                var_18 = ((+((((max((arr_0 [i_0] [i_0]), (arr_5 [i_0] [i_0] [i_0 - 1])))) - 14300458231289744704))));
                var_19 += (((arr_3 [i_1 + 2] [i_0]) != (arr_3 [i_0] [i_0])));
                var_20 = (((max((arr_2 [i_0 - 2] [i_1 - 1]), (arr_2 [i_0] [15])))) ? (((((arr_2 [i_0 + 3] [i_1]) || (arr_2 [i_0 - 1] [15]))))) : (arr_2 [i_0 + 3] [i_1 - 1]));
            }
        }
    }
    #pragma endscop
}
