/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 101920
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=101920 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/101920
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        arr_3 [i_0] = (arr_2 [i_0] [i_0]);
        var_16 = (arr_2 [i_0] [i_0]);
    }
    /* vectorizable */
    for (int i_1 = 1; i_1 < 10;i_1 += 1)
    {
        arr_7 [i_1] = (arr_0 [i_1]);
        arr_8 [5] = 247;
        /* LoopNest 2 */
        for (int i_2 = 4; i_2 < 10;i_2 += 1)
        {
            for (int i_3 = 2; i_3 < 9;i_3 += 1)
            {
                {
                    arr_15 [i_1] = -255;
                    /* LoopNest 2 */
                    for (int i_4 = 0; i_4 < 1;i_4 += 1)
                    {
                        for (int i_5 = 1; i_5 < 10;i_5 += 1)
                        {
                            {
                                var_17 *= (((arr_9 [i_1] [i_1 + 1]) ? (arr_19 [i_2 + 1]) : (arr_19 [i_2 - 3])));
                                var_18 |= (arr_0 [11]);
                                arr_23 [i_1 - 1] [i_2 - 2] [i_4] [i_3] [3] [i_4] [i_5 + 1] = (arr_20 [i_1 + 2] [i_1 + 2] [i_2] [i_3] [i_4] [i_5] [i_5 - 1]);
                                arr_24 [8] [i_3] [i_4] [i_3] |= 228;
                            }
                        }
                    }
                }
            }
        }
    }
    var_19 = var_8;
    #pragma endscop
}
