/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181193
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181193 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181193
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_11;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_7 [i_0] [i_1] |= ((((((arr_5 [i_1] [i_0]) ? (((arr_3 [i_0]) ? 1 : (arr_5 [i_0] [i_1]))) : (arr_1 [i_0] [i_1])))) ? (arr_4 [i_0] [i_1]) : 1));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        {
                            arr_12 [i_3] [i_3] [i_3] [i_3] |= (max((((((min(1, (arr_9 [i_0] [i_1])))) ? 1 : (arr_9 [i_0] [i_3])))), (min((var_13 >= -32745), var_10))));
                            var_19 = (var_13 == (arr_9 [i_0] [i_1]));
                            var_20 = var_17;
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
