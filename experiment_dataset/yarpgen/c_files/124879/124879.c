/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124879
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124879 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124879
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (((max(((min(var_7, 0))), var_11)) & var_5));
    var_21 *= (max((min((max(var_17, 1885328876831340995)), (((var_14 ? var_16 : var_3))))), var_2));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_22 += (((min((arr_3 [i_0 + 1] [i_0]), 0))));
                arr_4 [i_0] = var_12;
                arr_5 [i_0] [12] &= (((((arr_1 [i_0 - 1]) * 1125899906838528))));
            }
        }
    }
    #pragma endscop
}
