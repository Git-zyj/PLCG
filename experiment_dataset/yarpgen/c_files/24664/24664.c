/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24664
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24664 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24664
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 -= var_0;
    var_13 = ((var_9 ? (max(var_2, 1845139062337535002)) : 21305));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_7 [i_0] [i_0] = ((-(((-127554583 + 2147483647) << ((((((20766 ? var_1 : (arr_1 [i_0]))) + 1826193012606574725)) - 16))))));
                arr_8 [i_0] [i_0] = ((max(((max(1, (arr_3 [i_0]))), (((arr_5 [i_0]) ? var_8 : (arr_5 [i_0])))))));
            }
        }
    }
    var_14 -= var_1;
    #pragma endscop
}
