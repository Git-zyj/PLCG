/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162777
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162777 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162777
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (((((min(0, 200)))) & (min((var_1 > var_6), (var_9 * var_2)))));
    var_14 = (max(var_14, var_2));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            {
                var_15 = ((((((arr_0 [i_0] [i_0 + 1]) + 9223372036854775807)) << (((((arr_0 [i_0 + 1] [i_0 + 1]) + 9135038154622184544)) - 54)))));
                arr_5 [1] [i_0] [i_1 - 2] = (arr_2 [i_0] [i_1 + 1] [i_0 + 1]);
                var_16 = (((arr_3 [i_1 - 1]) - (((arr_3 [i_0]) - (arr_3 [i_0 + 1])))));
            }
        }
    }
    var_17 = (!var_11);
    #pragma endscop
}
