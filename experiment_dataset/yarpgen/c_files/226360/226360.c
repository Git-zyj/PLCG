/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226360
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226360 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226360
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (-((var_8 ? (var_8 + var_7) : var_6)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_2] = ((-4294967283 || ((min((max((arr_4 [i_0] [i_2]), var_2)), ((max(var_6, (arr_0 [i_0])))))))));
                    arr_7 [i_2] [i_2] [i_2] [i_0] = ((-((((arr_1 [i_1 - 3] [i_1]) < (arr_1 [i_1 - 3] [i_2]))))));
                    arr_8 [i_1] [i_1] [i_1] [i_2] = arr_5 [i_0] [i_1] [i_2] [i_2];
                    arr_9 [i_2] [i_1] [i_2] = (((arr_1 [i_1] [i_1]) + ((~(((arr_5 [i_0] [i_2] [i_2] [i_1]) ? var_7 : var_1))))));
                }
            }
        }
    }
    var_11 = var_4;
    #pragma endscop
}
