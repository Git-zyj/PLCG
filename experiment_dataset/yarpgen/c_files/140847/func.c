/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 140847
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=140847 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/140847
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 ^= ((/* implicit */int) var_7);
    /* LoopNest 2 */
    for (unsigned short i_0 = 0; i_0 < 13; i_0 += 3) 
    {
        for (short i_1 = 2; i_1 < 11; i_1 += 2) 
        {
            {
                arr_6 [i_0] = ((/* implicit */short) ((((/* implicit */_Bool) ((short) ((var_1) ? (var_8) : (((/* implicit */int) arr_1 [i_0])))))) ? (((/* implicit */int) min((arr_5 [i_1 + 1] [i_1 + 2]), ((short)23253)))) : (min(((~(((/* implicit */int) var_7)))), (((/* implicit */int) max((var_2), (var_7))))))));
                var_11 = ((/* implicit */signed char) max((((/* implicit */int) (short)-32756)), (var_8)));
            }
        } 
    } 
}
