/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149374
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149374 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149374
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */signed char) 2559028437U);
    var_17 = ((/* implicit */long long int) ((var_9) ? (((((/* implicit */_Bool) var_4)) ? (((/* implicit */unsigned int) ((int) var_0))) : (((((/* implicit */_Bool) var_13)) ? (((/* implicit */unsigned int) ((/* implicit */int) var_14))) : (var_7))))) : (((/* implicit */unsigned int) ((/* implicit */int) var_0)))));
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 23; i_0 += 4) 
    {
        var_18 += ((/* implicit */unsigned long long int) var_3);
        var_19 = ((/* implicit */unsigned short) (+(var_7)));
    }
}
