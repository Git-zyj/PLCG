/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215396
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215396 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215396
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_2, var_8));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 18;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            var_12 *= (arr_0 [i_1]);
                            var_13 = (min(((((((~(arr_2 [i_0] [i_1] [2])))) <= var_3))), (min((max((arr_7 [i_1] [i_1 + 2] [i_0] [15]), (arr_7 [i_3] [i_0] [i_0] [i_0]))), ((max((arr_1 [i_3]), var_5)))))));
                        }
                    }
                }
                var_14 = (min(var_6, ((var_4 ? (((818309059 ? var_2 : (arr_3 [i_0] [i_0] [i_1])))) : (~var_7)))));
            }
        }
    }
    #pragma endscop
}
