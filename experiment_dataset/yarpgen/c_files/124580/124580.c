/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124580
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124580 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124580
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_1;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 10;i_2 += 1)
            {
                {
                    var_11 = (min(((12360 ? (((min(var_8, (arr_5 [i_1] [i_1] [i_0]))))) : (min(2199023255520, (arr_1 [i_2 - 2]))))), (max(var_4, (arr_3 [i_2 - 3] [i_2] [i_2 - 1])))));
                    arr_6 [i_2] [i_1] [i_0] = (min(((((var_9 ? var_5 : var_0)))), ((var_2 & (arr_2 [i_2 - 1])))));
                    var_12 = (arr_1 [i_1]);
                }
            }
        }
    }
    var_13 = (max(var_1, (max(var_1, var_3))));
    #pragma endscop
}
