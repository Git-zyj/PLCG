/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157440
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157440 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157440
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max((~var_5), ((max(var_2, var_0)))));
    var_14 = var_12;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_6 [i_0] = (arr_3 [i_0]);

                for (int i_2 = 3; i_2 < 19;i_2 += 1)
                {
                    var_15 = (max(var_15, (((~(((arr_3 [i_0]) / (arr_1 [i_0]))))))));
                    arr_9 [i_2 - 2] [i_2] [4] = (arr_4 [10]);
                    arr_10 [i_2] = (((arr_4 [i_1]) == (arr_2 [i_0] [i_0] [i_0])));
                }
                var_16 = (~(((arr_3 [i_1]) ^ (min((arr_0 [i_0]), (arr_1 [3]))))));
            }
        }
    }
    var_17 = (max(-13862, var_11));
    #pragma endscop
}
