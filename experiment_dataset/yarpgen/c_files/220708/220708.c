/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220708
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220708 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220708
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 14;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_1] = (min((((arr_2 [i_1]) * (-2147483647 / 4699416770093281555))), (~146)));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 15;i_4 += 1)
                        {
                            {
                                arr_12 [i_4] [i_2] [i_1] [i_1] [i_1] [i_0] = (arr_8 [i_0] [i_0] [i_2] [i_3] [11]);
                                arr_13 [4] [1] [i_1] [i_1] [i_0] = ((-2147483647 ^ ((~(arr_7 [i_2 - 3] [i_1] [10] [i_1])))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_20 = (!-2824161515844787557);
    #pragma endscop
}
