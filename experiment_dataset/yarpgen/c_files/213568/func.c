/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213568
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213568 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213568
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) 2282823044U)) ? (((/* implicit */int) ((short) var_7))) : (((/* implicit */int) ((short) var_4)))));
    var_15 = ((/* implicit */unsigned char) max((var_7), (((/* implicit */unsigned short) var_8))));
    var_16 = ((/* implicit */unsigned short) var_3);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 17; i_0 += 3) 
    {
        for (unsigned short i_1 = 2; i_1 < 16; i_1 += 1) 
        {
            {
                arr_6 [i_1] = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) arr_5 [3U] [i_1 - 1]))));
                var_17 = ((/* implicit */int) (!(((/* implicit */_Bool) var_0))));
            }
        } 
    } 
}
