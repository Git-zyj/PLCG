/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44214
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44214 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44214
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((!(((-((var_12 ? var_1 : var_4)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_19 &= ((~((~(arr_3 [i_0] [i_1] [16])))));
                var_20 ^= (!var_15);
                arr_4 [8] |= (((((!(arr_1 [i_0])))) << ((((~((var_11 ? var_12 : var_17)))) + 56119))));
            }
        }
    }

    for (int i_2 = 0; i_2 < 15;i_2 += 1)
    {
        arr_8 [i_2] [1] = (arr_3 [20] [i_2] [i_2]);
        var_21 = ((!(((var_16 & (~var_4))))));
        var_22 = (((((~(arr_1 [i_2])))) ? (((!(arr_0 [i_2] [i_2])))) : (((arr_0 [i_2] [i_2]) ? var_16 : (arr_0 [1] [i_2])))));
        /* LoopNest 2 */
        for (int i_3 = 2; i_3 < 12;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 15;i_4 += 1)
            {
                {
                    var_23 ^= (((max((arr_3 [i_2] [i_3 - 2] [i_2]), var_2)) + (arr_3 [i_2] [i_3] [i_4])));
                    var_24 = (arr_11 [i_3] [i_3]);
                }
            }
        }
    }
    #pragma endscop
}
