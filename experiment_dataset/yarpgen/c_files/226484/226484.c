/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226484
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226484 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226484
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 *= ((((((min(var_5, var_0)))) * (27964 / var_4))) % ((((var_11 / var_11) ? var_10 : ((var_10 ? 27982 : 27983))))));

    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        var_16 = (min(var_16, (((((max((arr_0 [i_0 + 1]), (((arr_0 [i_0]) ? -27965 : var_0))))) ? var_1 : ((max((arr_1 [i_0]), (((27982 * (arr_0 [i_0]))))))))))));
        var_17 = (((~(arr_1 [i_0 - 1]))));
        var_18 = var_7;
        arr_2 [i_0] = (!var_13);
    }
    var_19 = ((((max(var_1, var_2))) && (((-(0 / 27972))))));
    #pragma endscop
}
