/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166108
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166108 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166108
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 ^= ((~((7175826979919915224 ? 213 : 134217727))));
    var_14 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_6 [7] [i_0] = (((((var_11 ? ((1 ? -7175826979919915245 : 23332)) : ((var_9 ? 142 : (arr_5 [i_0] [i_1])))))) ? (((var_7 || (var_1 && var_4)))) : ((((min(1, var_12))) ? (!var_1) : var_10))));
                var_15 += 201;
            }
        }
    }
    #pragma endscop
}
