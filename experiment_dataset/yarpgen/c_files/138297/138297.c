/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138297
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138297 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138297
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 -= var_7;
    var_11 = -var_7;
    var_12 = (max(var_12, (((!(!var_2))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 0;i_3 += 1)
                    {
                        {
                            arr_12 [i_0] [i_3] [i_3] [i_3] = max(((!(arr_11 [i_2 - 2] [i_0] [i_3 + 1] [i_3]))), (arr_8 [i_2 + 1] [i_2 + 1] [i_3 + 1] [i_3 + 1]));
                            var_13 |= ((-((0 ? ((1 ? 80 : 30)) : (arr_11 [i_0] [i_2 + 2] [i_0] [i_0])))));
                        }
                    }
                }
                var_14 = 32;
                arr_13 [i_0] = ((!(((arr_7 [i_0] [i_1]) == ((var_4 ? 223 : (arr_8 [12] [i_0] [12] [i_0])))))));
            }
        }
    }
    #pragma endscop
}
