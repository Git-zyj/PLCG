/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194407
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194407 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194407
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 14;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 15;i_4 += 1)
                        {
                            {
                                var_17 = (max(var_17, ((~(((var_6 >= var_12) ? (min(3655776832726118784, var_15)) : var_4))))));
                                var_18 = -1663136739;
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 17;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 17;i_6 += 1)
                        {
                            {
                                var_19 += ((var_2 ? 3655776832726118784 : ((~((max((arr_0 [i_2]), (arr_15 [i_0] [i_1] [i_2] [i_0] [i_5] [i_6] [i_6]))))))));
                                var_20 = 3655776832726118812;
                            }
                        }
                    }
                }
            }
        }
    }
    var_21 = ((var_14 - (min(var_15, var_5))));
    #pragma endscop
}
