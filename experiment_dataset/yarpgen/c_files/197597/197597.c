/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197597
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197597 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197597
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            {
                var_17 = 870050037;
                arr_5 [i_0 + 1] [i_1] = ((((max((arr_3 [i_0 + 1]), ((((arr_2 [i_0 + 1]) && 13167)))))) ? -13167 : ((-13176 ? 10 : -13167))));
                arr_6 [i_0] [i_0] = (min(((((arr_3 [i_0 + 1]) < (arr_3 [i_0 + 1])))), (max(64, 31))));
            }
        }
    }
    var_18 = (((13149 + var_6) ? ((((((-13167 ? var_6 : var_5))) ? var_11 : 9329))) : ((~((1 ? 0 : -13184))))));
    var_19 = -13159;
    #pragma endscop
}
