/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90603
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90603 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90603
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (signed char i_0 = 3; i_0 < 18; i_0 += 1) 
    {
        for (signed char i_1 = 0; i_1 < 19; i_1 += 1) 
        {
            {
                var_16 -= ((/* implicit */_Bool) max((var_11), (var_6)));
                var_17 = ((/* implicit */unsigned short) var_11);
                var_18 |= ((/* implicit */short) min((((/* implicit */unsigned short) (signed char)-8)), (((unsigned short) (short)-22451))));
            }
        } 
    } 
    var_19 = ((/* implicit */_Bool) min((min((((/* implicit */unsigned int) (unsigned char)127)), (var_13))), (((/* implicit */unsigned int) var_6))));
}
