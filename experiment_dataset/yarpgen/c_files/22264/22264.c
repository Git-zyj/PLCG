/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22264
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22264 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22264
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (max(var_11, -60));

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        arr_2 [i_0] = -118;
        var_12 = (max(var_12, (((127 & (-115 || -116))))));
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 12;i_3 += 1)
                {
                    {
                        arr_10 [i_3] [i_2] [0] [i_0] = (((-2 || -66) + 31));
                        arr_11 [4] [4] [9] [4] [i_1] = ((((((45 ? 59 : 90)) - ((66 ? 0 : 1)))) + (127 - -126)));
                        arr_12 [i_3] [i_2] [i_0] [i_0] = -66;
                        arr_13 [i_2] [4] [i_1] [i_3] [7] [i_1] = -60;
                    }
                }
            }
        }
    }
    #pragma endscop
}
