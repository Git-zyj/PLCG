/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219396
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219396 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219396
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((var_7 < var_1) ? var_10 : (((max(var_6, var_0)) | var_6))));
    var_12 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_13 = (max(var_13, ((((((-(arr_3 [i_0 + 1] [i_0 + 1])))) ? (arr_3 [i_0 + 1] [i_0 + 1]) : var_4)))));
                var_14 = ((arr_3 [i_0] [i_0]) <= (((((arr_3 [i_0] [i_1]) / (arr_2 [i_1] [11] [6]))))));
                var_15 ^= (((((-((max(var_5, (arr_3 [1] [i_1]))))))) ? 32752 : 34032));
            }
        }
    }
    #pragma endscop
}
