/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103667
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103667 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103667
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 19;i_0 += 1)
    {
        var_19 = (max((min((arr_0 [i_0 + 1]), (((arr_0 [i_0]) ? var_1 : -1)))), (arr_0 [i_0])));
        var_20 = var_4;
    }
    for (int i_1 = 1; i_1 < 17;i_1 += 1)
    {
        var_21 = (max(var_3, (min(0, 0))));
        /* LoopNest 2 */
        for (int i_2 = 1; i_2 < 17;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 18;i_3 += 1)
            {
                {
                    var_22 = 32;
                    arr_10 [i_1] [i_1] [i_1] = var_17;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < 19;i_4 += 1)
    {
        arr_13 [i_4] = 6;
        arr_14 [i_4] = (((-123 ? 5 : 2893705699498616930)));
    }
    var_23 = var_2;
    var_24 = var_8;
    var_25 = (min(var_25, (((var_7 >= (var_17 < var_2))))));
    #pragma endscop
}
