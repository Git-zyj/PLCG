/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218080
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218080 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218080
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = (max((((~((1934279155 ? 1934279155 : 1934279148))))), ((~(arr_2 [i_0 - 1] [i_0 + 1])))));
                arr_7 [i_1] [i_1] [i_1] = (((((((arr_4 [i_1]) + 2147483647)) >> (((~1934279151) - 2360688116)))) == ((((((arr_1 [i_1]) ? var_9 : 2360688149)) > (((((arr_0 [i_0 - 1]) == (arr_4 [i_1]))))))))));
            }
        }
    }
    var_14 = (2360688140 - 784493566);
    #pragma endscop
}
