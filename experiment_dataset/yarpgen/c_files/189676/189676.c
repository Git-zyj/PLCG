/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 189676
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=189676 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/189676
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_15 = (-((((((arr_3 [1] [3]) * var_2))) ? 1 : (((arr_1 [15] [1]) * 770006894)))));
                var_16 = (arr_4 [i_1 + 1] [i_0] [1]);
                arr_5 [i_1] [i_1 + 1] = (((((!(arr_2 [i_1 + 1] [i_1 + 1])))) | ((((1 ? (arr_3 [i_0] [i_1 + 1]) : (arr_2 [10] [6]))) | ((((arr_0 [i_1 + 1]) || (arr_0 [6]))))))));
            }
        }
    }
    var_17 = (!var_9);
    var_18 = var_9;
    var_19 = (min((min(var_1, var_13)), var_1));
    #pragma endscop
}
