/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 53406
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=53406 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/53406
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = var_1;
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 0; i_0 < 12; i_0 += 2) 
    {
        var_16 = (-(((/* implicit */int) var_6)));
        var_17 *= ((/* implicit */unsigned short) arr_0 [i_0] [i_0]);
    }
    var_18 = ((/* implicit */signed char) ((unsigned long long int) max((((/* implicit */unsigned long long int) (+(((/* implicit */int) var_9))))), (((3621798626764093910ULL) / (((/* implicit */unsigned long long int) 3693406879U)))))));
}
