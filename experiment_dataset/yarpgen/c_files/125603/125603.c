/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125603
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125603 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125603
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 0;i_2 += 1)
            {
                {
                    var_12 = -70;
                    arr_6 [i_0] [i_0] [i_0] [i_2] = ((-(((((~(arr_0 [i_0]))) != -9223372036854775800)))));
                }
            }
        }
    }
    var_13 = (var_8 != var_8);
    /* LoopNest 2 */
    for (int i_3 = 3; i_3 < 19;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 22;i_4 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_5 = 0; i_5 < 22;i_5 += 1)
                {
                    for (int i_6 = 0; i_6 < 22;i_6 += 1)
                    {
                        for (int i_7 = 3; i_7 < 19;i_7 += 1)
                        {
                            {
                                var_14 += ((((-((5379 ? var_1 : (arr_16 [i_3] [i_3] [i_6]))))) ^ (((~(max(4, -256)))))));
                                var_15 = (arr_14 [i_7] [i_7] [i_7] [i_6]);
                            }
                        }
                    }
                }
                var_16 = (((((((arr_11 [i_3] [i_4] [i_3 + 1] [i_3 - 3]) + 2147483647)) << (((((arr_11 [i_3] [i_4] [i_3 + 1] [i_3 - 2]) + 46)) - 6)))) == -91));
            }
        }
    }
    #pragma endscop
}
