/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11598
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11598 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11598
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_19;

    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        arr_2 [i_0] = (max((min((arr_0 [i_0 + 1] [i_0 + 3]), 127611469)), (((!(arr_0 [i_0 + 2] [i_0]))))));
        arr_3 [i_0] = ((min(167, (arr_1 [i_0] [i_0 - 1]))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 12;i_1 += 1)
    {
        var_21 = (((arr_4 [i_1] [i_1]) ? 1 : (arr_4 [i_1] [i_1])));
        var_22 = ((arr_0 [i_1] [i_1]) ? (arr_1 [12] [i_1]) : (arr_0 [i_1] [i_1]));
    }
    #pragma endscop
}
