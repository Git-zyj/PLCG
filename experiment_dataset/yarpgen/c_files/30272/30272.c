/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30272
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30272 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30272
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_1, (max(var_11, var_6))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                {
                    arr_8 [i_2] = (-3613859060094802464 * 1);
                    arr_9 [i_0 + 1] [21] [21] [i_2] = ((!((min(-var_5, (max(var_11, 1)))))));
                    arr_10 [i_0 - 1] [0] [4] = var_7;
                    arr_11 [i_0] [18] [i_0] |= -var_9;
                }
            }
        }
    }
    var_13 = var_8;
    #pragma endscop
}
