/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180435
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180435 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180435
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = (min((((var_15 - 6524533196601803369) ? (((arr_1 [3]) - var_2)) : ((max(var_9, var_9))))), (-51 + 8114440030784724840)));
                arr_6 [i_0] [i_0] [i_0] = var_6;
                arr_7 [i_0] [i_0] = ((1194869494 <= (--2811948619)));
                arr_8 [i_0] [i_1] [21] = ((((((var_5 ? var_8 : (arr_1 [21]))))) ? (!var_2) : -var_8));
            }
        }
    }
    var_16 = var_8;
    var_17 = ((-((-var_2 ? var_2 : (max(var_12, var_12))))));
    #pragma endscop
}
