/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88387
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88387 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88387
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        var_20 = (max(var_20, 122));
        /* LoopNest 2 */
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 23;i_2 += 1)
            {
                {
                    var_21 = (max(var_21, (((-(((1 ? var_12 : 1))))))));
                    var_22 = ((((min(0, -32748)))));
                    arr_7 [i_1] [i_1] [i_2 - 1] = var_17;
                    var_23 = ((((max(var_6, -27))) && (((var_18 ? (-32748 / var_2) : (((max((arr_4 [5] [i_1] [i_0]), -32755)))))))));
                }
            }
        }
    }
    var_24 = (min(var_24, 3159414413));
    #pragma endscop
}
