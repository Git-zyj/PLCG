/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 205346
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=205346 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/205346
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 0; i_0 += 1) 
    {
        for (int i_1 = 1; i_1 < 9; i_1 += 2) 
        {
            {
                var_18 = ((/* implicit */_Bool) ((((unsigned int) ((unsigned char) arr_3 [i_0]))) | (((/* implicit */unsigned int) ((/* implicit */int) ((signed char) var_8))))));
                var_19 = ((/* implicit */_Bool) ((((int) ((((/* implicit */_Bool) var_4)) ? (var_11) : (((/* implicit */unsigned long long int) ((/* implicit */int) (_Bool)0)))))) / (((/* implicit */int) min((((/* implicit */unsigned short) var_4)), (arr_0 [i_1 + 3]))))));
            }
        } 
    } 
    var_20 ^= ((/* implicit */_Bool) var_8);
    var_21 = ((/* implicit */unsigned char) var_5);
}
