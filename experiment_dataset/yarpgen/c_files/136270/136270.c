/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136270
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136270 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136270
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        var_20 &= ((((((arr_1 [i_0] [i_0]) ^ (arr_0 [i_0 + 1])))) ? ((max((arr_1 [i_0] [i_0 + 1]), (max((arr_1 [i_0] [i_0]), 1))))) : (((max(4241588800, var_17))))));
        var_21 = 1;
        arr_2 [i_0] [1] = 1;
        var_22 = (arr_0 [9]);
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 16;i_1 += 1)
    {
        arr_6 [i_1] = (-2147483647 - 1);
        arr_7 [i_1] = (((arr_4 [i_1]) ? (arr_5 [i_1]) : (arr_5 [i_1])));
        /* LoopNest 3 */
        for (int i_2 = 0; i_2 < 16;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 16;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 16;i_4 += 1)
                {
                    {
                        arr_16 [i_3] |= var_17;
                        var_23 = ((var_8 ? -var_4 : var_3));
                        var_24 = (arr_15 [i_1] [i_2]);
                    }
                }
            }
        }
    }
    /* vectorizable */
    for (int i_5 = 0; i_5 < 22;i_5 += 1)
    {
        arr_21 [i_5] = ((1776074771 <= (arr_20 [i_5])));
        var_25 -= ((~(-127 - 1)));
        var_26 -= var_5;
    }
    var_27 = var_1;
    var_28 += 1;
    #pragma endscop
}
