/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48206
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48206 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48206
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 ^= 18;

    /* vectorizable */
    for (int i_0 = 2; i_0 < 22;i_0 += 1) /* same iter space */
    {
        var_17 = (arr_1 [i_0]);
        var_18 = (((arr_1 [i_0 + 3]) ? (arr_1 [i_0 + 3]) : var_0));
        var_19 = (arr_0 [i_0] [i_0]);
    }
    for (int i_1 = 2; i_1 < 22;i_1 += 1) /* same iter space */
    {
        var_20 = (max((arr_2 [i_1] [i_1]), (min((~18446744073709551608), ((var_4 ? 18 : 2065102988))))));
        var_21 = (min(var_21, ((((min(((min(var_7, (arr_0 [i_1] [i_1])))), var_9))) ? ((var_11 ? 0 : (arr_0 [i_1 + 2] [i_1 + 1]))) : (arr_4 [i_1] [i_1])))));
        arr_5 [i_1] [i_1] = var_5;
        var_22 = (arr_3 [i_1]);
    }
    /* vectorizable */
    for (int i_2 = 1; i_2 < 14;i_2 += 1)
    {
        var_23 += ((~(arr_7 [i_2])));
        arr_9 [i_2] = (arr_1 [i_2]);
        arr_10 [i_2] = (((arr_2 [i_2 - 1] [i_2 + 1]) ? 1048575 : (10 | 237)));
    }
    #pragma endscop
}
