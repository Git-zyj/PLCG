/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172575
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172575 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172575
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 |= (max(var_5, (max(var_6, var_9))));
    var_12 = ((((((max(var_6, var_5))) ? var_1 : var_2)) | var_5));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = (min((((arr_3 [i_0] [i_0]) ? (((109 ? -46 : 268435455))) : (((arr_0 [i_0]) ? var_3 : (arr_3 [i_0] [i_1]))))), (arr_2 [i_0])));
                arr_6 [i_1] [i_1] = 23393;
            }
        }
    }
    var_13 = 23393;
    #pragma endscop
}
