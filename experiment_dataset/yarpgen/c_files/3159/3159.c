/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3159
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3159 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3159
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_11;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 18;i_4 += 1)
                        {
                            {
                                arr_14 [i_0] [11] [i_1] [i_2] [5] [i_1] = (~((((((!(arr_3 [i_2] [i_3])))) != var_2))));
                                var_16 *= (arr_5 [i_0] [i_2] [10]);
                                arr_15 [12] [i_1] [i_2] [i_3 - 1] [i_4] = var_3;
                                var_17 = ((var_0 & ((-(arr_10 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0])))));
                            }
                        }
                    }
                    arr_16 [i_0] [i_0] [i_1] [i_1] = ((min(var_4, (-3434866698237260311 < 202))));
                }
            }
        }
    }
    var_18 = (max(var_18, (!-var_1)));
    #pragma endscop
}
