/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 167296
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=167296 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/167296
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_11;
    var_14 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_15 = (min(var_15, ((((((20 ? ((var_3 ? var_10 : (arr_0 [16]))) : var_5))) ? (((arr_4 [i_0 - 2] [i_1]) ? (arr_1 [i_0 + 3] [12]) : (0 / 3967372061))) : (((~((max(var_4, 13485)))))))))));
                var_16 = (arr_2 [i_0] [i_0]);
            }
        }
    }
    var_17 = (((((((max(var_7, var_10))) ? (52030 != 52050) : ((36 ? 85 : var_8))))) ? var_11 : var_11));
    var_18 = (!var_1);
    #pragma endscop
}
