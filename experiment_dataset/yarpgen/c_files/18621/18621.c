/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18621
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18621 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18621
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_0] = ((!(arr_3 [i_0] [i_0])));
                var_15 += var_2;
                var_16 = ((+((((max((arr_3 [i_0] [i_0]), var_12))) ? (((arr_1 [i_1] [i_0]) / var_6)) : (~var_0)))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        {
                            arr_9 [i_0] [i_0] [i_0] [1] = (var_5 < 2147483615);
                            arr_10 [i_3] [i_2] [1] [i_0] [i_0] = 8384512;
                            var_17 = (min(var_17, var_4));
                            var_18 = ((((max(4286582784, 2097151))) || (((var_7 ? (min(1792303539, 2097159)) : ((var_0 / (arr_0 [i_0]))))))));
                        }
                    }
                }
            }
        }
    }
    var_19 = var_2;
    var_20 = (((((max(var_8, var_0))) ? (var_0 & var_12) : (((var_8 ? var_6 : var_2))))));
    #pragma endscop
}
