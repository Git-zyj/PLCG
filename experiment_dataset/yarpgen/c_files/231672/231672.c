/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231672
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231672 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231672
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(((var_2 >> (((var_3 + var_12) + 2062920275525762196)))), (((((max(var_7, -48))) / var_5)))));
    var_17 = (((((!((min(var_6, -3)))))) | var_11));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                arr_7 [i_0] [i_1] = -var_9;
                arr_8 [i_0] = (min(-1, (min((arr_3 [i_0]), (arr_6 [i_0] [i_0] [i_1])))));
                arr_9 [i_0] [i_1] [i_1] = (-3 || ((((!(arr_0 [i_0]))))));
            }
        }
    }
    #pragma endscop
}
