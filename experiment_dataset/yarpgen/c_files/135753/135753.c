/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135753
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135753 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135753
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 |= (((((var_2 ? 1 : var_11)) % var_5)));
    var_14 = var_3;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    arr_9 [14] [14] [i_2] [i_2] = var_12;
                    arr_10 [i_0] = (((((var_10 / (arr_3 [i_1 - 2] [i_1 - 1])))) ? (arr_7 [i_0] [i_1 - 1] [i_1 - 3] [i_0]) : (min((var_9 && var_1), ((-1115833287 ? (arr_8 [i_0] [i_1] [i_2]) : var_1))))));
                    var_15 = (max(var_15, -1542711962));
                }
            }
        }
    }
    #pragma endscop
}
