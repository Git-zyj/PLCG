/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 223480
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=223480 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/223480
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_10 = (max((min(var_1, var_7)), var_5));
        arr_3 [i_0] [i_0] = var_5;
    }
    for (int i_1 = 0; i_1 < 25;i_1 += 1)
    {
        arr_6 [i_1] [i_1] = (min(((~((max(var_8, var_3))))), var_5));
        /* LoopNest 3 */
        for (int i_2 = 2; i_2 < 23;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 25;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 25;i_4 += 1)
                {
                    {
                        var_11 = var_1;
                        var_12 *= (max(-var_3, var_9));
                    }
                }
            }
        }
        /* LoopNest 3 */
        for (int i_5 = 0; i_5 < 25;i_5 += 1)
        {
            for (int i_6 = 0; i_6 < 25;i_6 += 1)
            {
                for (int i_7 = 2; i_7 < 22;i_7 += 1)
                {
                    {
                        arr_22 [i_1] = ((!((max(-79, 896705326)))));
                        var_13 = var_5;
                        arr_23 [i_1] [i_1] [23] [i_7] = (~var_3);
                    }
                }
            }
        }
    }
    var_14 = min((max(var_1, var_6)), var_0);
    var_15 = (~var_8);
    #pragma endscop
}
