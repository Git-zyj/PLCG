/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95079
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95079 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95079
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_19 |= (arr_1 [i_0]);
                arr_4 [i_1] [i_0] = ((arr_1 [i_0]) / (((-28358 == 123) - (((arr_0 [i_1]) ? (arr_2 [i_0] [i_1]) : 0)))));
                arr_5 [i_0] = (min((arr_2 [i_0] [i_1]), (arr_3 [i_0] [i_0])));
            }
        }
    }
    var_20 = var_3;
    var_21 = (max((~-85), var_14));
    var_22 = (max(var_22, (var_12 - var_6)));
    var_23 = ((((max(229, (min(var_15, var_13))))) ? var_12 : (var_0 && 1485278021)));
    #pragma endscop
}
