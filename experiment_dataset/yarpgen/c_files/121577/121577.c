/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 121577
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=121577 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/121577
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_12 = (max((((arr_1 [i_0]) ? (arr_0 [i_0] [i_0]) : 864036897328180285)), (((~((max(var_7, 5508))))))));
                arr_4 [i_1] = ((((((arr_1 [i_1]) ? (arr_3 [i_0] [i_1]) : (arr_3 [i_0] [i_0])))) ? (arr_0 [i_0] [i_0]) : ((var_4 ? (arr_3 [i_0] [i_0]) : var_11))));
                var_13 = (arr_3 [i_1] [i_0]);
            }
        }
    }
    var_14 -= (!var_10);
    var_15 = max(var_11, ((~((min(-5497, -5497))))));
    #pragma endscop
}
