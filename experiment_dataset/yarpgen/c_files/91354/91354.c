/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91354
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91354 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91354
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 *= var_6;
    var_11 = (max(var_11, var_0));

    for (int i_0 = 1; i_0 < 10;i_0 += 1) /* same iter space */
    {
        var_12 = (min(var_12, (arr_1 [i_0])));
        var_13 = (max(var_13, (((-(arr_0 [i_0 + 2]))))));
    }
    for (int i_1 = 1; i_1 < 10;i_1 += 1) /* same iter space */
    {
        var_14 &= (min(1, -8000490998939314318));
        arr_6 [8] &= (((~((min((arr_4 [i_1] [i_1]), (arr_0 [i_1 + 2])))))));
    }
    for (int i_2 = 0; i_2 < 1;i_2 += 1)
    {
        var_15 = (min(var_15, (((~((-(arr_7 [i_2]))))))));
        /* LoopNest 3 */
        for (int i_3 = 0; i_3 < 14;i_3 += 1)
        {
            for (int i_4 = 3; i_4 < 13;i_4 += 1)
            {
                for (int i_5 = 0; i_5 < 14;i_5 += 1)
                {
                    {
                        var_16 &= ((((((((arr_9 [i_2] [i_4]) > (arr_11 [i_2] [8] [i_2]))) ? (((arr_7 [i_5]) ? var_2 : (arr_13 [i_2] [i_3]))) : (((arr_13 [i_2] [i_3]) ? var_1 : (arr_16 [9])))))) ? (arr_11 [i_5] [7] [0]) : ((~((var_8 | (arr_9 [2] [0])))))));
                        var_17 = ((~(arr_9 [0] [7])));
                        var_18 = (min(var_1, (arr_15 [6] [6] [i_2])));
                        var_19 += (arr_9 [i_4] [i_5]);
                    }
                }
            }
        }
        var_20 = ((!(((((((arr_12 [i_2]) ^ (arr_16 [0])))) ? (min(var_6, var_9)) : ((((!(arr_7 [1]))))))))));
        var_21 = (min(var_21, (((~(arr_10 [i_2] [i_2] [i_2]))))));
    }
    #pragma endscop
}
