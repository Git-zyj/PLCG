/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 81375
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=81375 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/81375
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_12 = (min(var_12, (arr_0 [i_0])));
        var_13 = ((1 << (((arr_0 [i_0]) - 46586))));
        arr_2 [13] = (((arr_1 [i_0] [i_0]) ? 1 : 8046059139396377482));
    }
    for (int i_1 = 1; i_1 < 22;i_1 += 1)
    {
        arr_6 [i_1] = (((((~20760) ^ (arr_4 [i_1 - 1] [i_1 - 1]))) | (min(1, 8191))));
        var_14 = ((min((arr_3 [i_1 + 1]), (arr_3 [i_1 + 1]))));
        arr_7 [i_1] [i_1] = ((min(var_8, 1776541208)));
    }
    var_15 |= var_4;
    #pragma endscop
}
