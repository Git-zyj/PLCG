/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203388
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_7;
    var_15 = (min(var_15, (var_13 / var_0)));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 9;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_16 = (max(var_16, (((var_4 ? ((var_13 ? (var_4 - var_4) : var_6)) : var_10)))));
                                arr_11 [i_0 - 1] [i_0] [i_1] [i_2] [i_3 + 2] [i_3 + 2] [1] = var_7;
                            }
                        }
                    }
                    arr_12 [i_0] [i_1] [i_0 + 1] = (var_12 & var_2);
                    var_17 = (((var_13 >= var_5) != (var_0 != var_7)));
                }
            }
        }
    }
    #pragma endscop
}
