/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124820
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124820 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124820
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_12 = ((-45 / ((13947116 ? 13619879261631314775 : -24))));
                arr_8 [i_0 - 2] [i_0] = ((~(((arr_2 [i_0 - 2] [i_0]) ^ (arr_0 [i_0])))));
            }
        }
    }
    var_13 = (max(var_13, ((((~var_10) ? ((((var_4 ? 117 : var_8)))) : (~0))))));
    var_14 = (min(var_14, ((max((((var_5 << (var_2 - 83)))), ((((max(var_7, var_5))) / var_10)))))));
    var_15 = (min(var_15, var_11));
    #pragma endscop
}
