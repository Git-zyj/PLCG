/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146765
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146765 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146765
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_19;
    var_21 = min((!var_2), ((var_17 ? var_11 : ((var_1 ? var_7 : var_18)))));
    var_22 = ((~(~var_4)));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_23 -= ((!(((~(arr_1 [i_0 + 1] [i_0 + 1]))))));
                var_24 += (((((arr_0 [i_0] [i_1]) || ((!(arr_0 [8] [i_0]))))) || 1));
                var_25 += ((((((((arr_3 [i_0] [i_0] [i_0]) ? var_12 : (arr_1 [i_0] [i_1])))) ? var_0 : -3281)) - ((-3281 ? -3281 : 0))));
            }
        }
    }
    #pragma endscop
}
