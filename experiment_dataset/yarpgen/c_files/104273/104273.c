/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104273
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104273 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104273
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_15 = (max(var_15, (((-(arr_1 [i_0]))))));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 12;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_1] [i_0] [i_0] &= (arr_5 [i_0] [i_0] [i_1] [i_2]);
                    var_16 = 1;
                    var_17 = (max(var_17, (arr_1 [i_0])));
                }
            }
        }
        arr_8 [8] [i_0] |= ((10448016773123558930 ? (~1) : (arr_5 [i_0] [i_0] [i_0] [i_0])));
    }
    var_18 = (min((max((var_0 != 3), var_3)), var_6));
    var_19 = (max(var_19, (max(var_9, -var_0))));
    var_20 |= var_1;
    #pragma endscop
}
