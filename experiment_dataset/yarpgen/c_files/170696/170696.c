/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 170696
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=170696 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/170696
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 17;i_2 += 1)
            {
                {
                    var_10 = ((~(((var_1 / (arr_5 [i_0] [i_2 - 2]))))));
                    var_11 = ((((arr_1 [i_1 - 1] [i_2 - 2]) ? ((9493 ? 0 : -68198793)) : (((((arr_4 [i_0]) + 2147483647)) >> (((arr_1 [i_0] [10]) - 149)))))));
                    var_12 -= (arr_6 [i_0] [i_1] [i_2]);
                    arr_9 [1] [i_2] [i_0] = (((~((~(arr_8 [i_0] [i_2] [i_2]))))));
                }
            }
        }
    }
    var_13 *= (((-1 | var_4) ? var_6 : ((1 ? 2158660905 : 1))));
    #pragma endscop
}
