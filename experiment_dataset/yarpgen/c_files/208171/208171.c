/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208171
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208171 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208171
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (min(var_20, var_17));
    var_21 = var_3;

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_22 = (min(1571991763, -22));
        arr_3 [i_0] [i_0] = (arr_1 [i_0] [i_0]);
        arr_4 [i_0] = (((((arr_2 [i_0]) ? (arr_2 [i_0]) : (arr_2 [i_0]))) * (min((arr_2 [i_0]), (((arr_0 [1]) ? (arr_2 [i_0]) : (arr_1 [i_0] [6])))))));
    }
    for (int i_1 = 2; i_1 < 20;i_1 += 1)
    {
        var_23 += ((((((arr_6 [i_1 - 2]) ? 254 : 261888))) ? (((min((arr_5 [i_1 - 1]), (arr_6 [i_1 - 2]))))) : (min((arr_5 [i_1 - 2]), 1))));
        arr_7 [i_1] [i_1 - 2] = (arr_5 [i_1]);
        arr_8 [i_1] = ((+(min((0 * 36657), (arr_5 [i_1 - 2])))));
        var_24 *= (min(var_1, (arr_5 [i_1 - 1])));
    }
    #pragma endscop
}
