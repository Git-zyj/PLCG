/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246784
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246784 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246784
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 8;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 10;i_2 += 1)
            {
                {
                    var_14 = (min(var_14, var_11));
                    arr_7 [i_0] [i_1] [i_2] [i_0] &= ((((var_13 ? var_11 : var_1)) >= -var_8));
                    /* LoopNest 2 */
                    for (int i_3 = 4; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_15 = (min((max((max(var_1, var_1)), var_2)), var_3));
                                var_16 = var_8;
                            }
                        }
                    }
                    arr_13 [i_0] [i_0] [i_1] [i_2] |= ((~(min((max(13404816325624753818, var_8)), (min(var_1, var_6))))));
                }
            }
        }
    }
    var_17 = var_3;
    #pragma endscop
}
