/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59506
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59506 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59506
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        var_19 = (((((var_13 ? (~var_18) : (((max((arr_2 [i_0] [i_0]), var_1))))))) ? (((!(((17 * (arr_2 [i_0] [i_0]))))))) : (max(((-(arr_2 [11] [i_0]))), (arr_0 [i_0])))));
        var_20 = (min(-581962777, (~255)));
    }
    var_21 = (max((((!(~var_10)))), (min((~797083644), (!var_4)))));
    var_22 *= (((var_4 * -var_8) / ((var_3 ? ((max(var_4, var_3))) : var_4))));
    var_23 = var_8;
    #pragma endscop
}
