/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144139
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144139 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144139
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_8;
    var_14 = (min(var_4, var_5));

    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        var_15 -= (((((17 | (((var_10 >> (((((-9223372036854775807 - 1) - -9223372036854775789)) + 27)))))))) ? (((127 == (arr_3 [i_0])))) : ((((1 - 111) >= var_12)))));
        var_16 -= (((((var_11 ? (arr_3 [i_0 + 1]) : (arr_1 [14] [i_0 + 1])))) ? (max((arr_1 [i_0] [i_0 + 2]), (arr_1 [i_0] [i_0 + 1]))) : var_4));
        var_17 = var_9;
        arr_4 [i_0] = (((141 & (arr_2 [i_0] [4]))) * ((min(0, -13))));
    }
    var_18 = ((var_7 ^ (11480 && 137)));
    #pragma endscop
}
