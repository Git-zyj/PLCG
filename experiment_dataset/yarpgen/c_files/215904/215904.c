/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215904
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215904 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215904
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(((((((1032192 ? var_2 : 1))) ? var_4 : ((max(var_4, 1)))))), ((max(var_16, var_4)))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            {
                var_20 = (max((((~(arr_4 [i_0])))), (arr_0 [i_0] [0])));
                var_21 *= (min((max((((arr_6 [1]) ? var_10 : (arr_5 [i_1 - 1] [i_0] [i_0]))), (((arr_4 [i_1]) + (arr_5 [i_0 - 2] [i_0] [i_0]))))), (((+((((arr_4 [i_1]) || (arr_1 [i_0 - 3])))))))));
            }
        }
    }
    #pragma endscop
}
