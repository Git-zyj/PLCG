/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49822
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49822 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49822
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_12;
    var_18 = var_0;
    var_19 = (((min(var_3, (~var_16)))) ? var_4 : (max(((max(var_10, var_8))), -var_13)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_1] = ((!((max(0, (((!(arr_3 [i_1] [i_1] [i_0])))))))));
                arr_7 [i_0] [i_0] = (max(-54, 0));
                arr_8 [i_1] = ((((max((arr_4 [i_0] [i_0]), (max((arr_0 [i_1]), var_7))))) ? (max((var_12 - var_13), var_13)) : ((min((arr_1 [i_0] [i_1]), var_12)))));
            }
        }
    }
    #pragma endscop
}
