/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29991
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29991 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29991
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] = ((arr_1 [i_0 + 4]) >> (((var_11 != (arr_0 [i_0 - 2])))));
                arr_5 [i_0 + 3] [i_1] = ((((((arr_0 [i_0 + 4]) < (arr_0 [i_0 + 3])))) / (arr_0 [i_0 + 4])));
                var_18 = (max(var_18, ((((max(28050, 2147483640)) * (((max(var_4, (arr_3 [i_1] [i_0]))))))))));
            }
        }
    }
    var_19 += (((var_15 && var_16) ^ var_0));
    #pragma endscop
}
