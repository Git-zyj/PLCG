/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 145814
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=145814 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/145814
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned int) min((min((-325525709942154560LL), (((/* implicit */long long int) ((((/* implicit */_Bool) (short)23264)) && (((/* implicit */_Bool) (short)7161))))))), (var_1)));
    var_17 = ((/* implicit */unsigned long long int) (-(8597822981546286052LL)));
    /* LoopNest 2 */
    for (unsigned short i_0 = 0; i_0 < 20; i_0 += 3) 
    {
        for (int i_1 = 0; i_1 < 20; i_1 += 4) 
        {
            {
                var_18 = ((/* implicit */int) 1116124821U);
                var_19 = ((/* implicit */short) var_5);
            }
        } 
    } 
    var_20 = ((/* implicit */unsigned short) var_4);
}
