/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114988
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114988 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114988
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_10;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 7;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 0;i_2 += 1)
            {
                {
                    var_19 = (min(var_19, 11));
                    var_20 = (~1);
                    var_21 = (min(var_21, ((25 * (arr_1 [i_0])))));

                    /* vectorizable */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        var_22 = (arr_4 [i_0] [i_1] [i_2 + 1] [i_3]);
                        var_23 -= ((var_0 ? (arr_2 [4] [i_1]) : (arr_7 [i_0] [i_1] [i_2] [i_2 + 1] [i_2 + 1] [i_3])));
                        arr_9 [i_0] [i_1] [i_0] = (arr_8 [i_1 + 1] [i_1] [i_1 + 1] [i_2 + 1] [i_1]);
                    }
                }
            }
        }
    }
    var_24 = var_17;
    var_25 = (min(var_25, var_8));
    /* LoopNest 3 */
    for (int i_4 = 0; i_4 < 12;i_4 += 1)
    {
        for (int i_5 = 0; i_5 < 12;i_5 += 1)
        {
            for (int i_6 = 0; i_6 < 1;i_6 += 1)
            {
                {
                    var_26 *= 1;
                    var_27 = 14569251294684197132;
                }
            }
        }
    }
    #pragma endscop
}
