/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24586
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24586 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24586
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((-446506087 ? 65535 : 1287026542833075719)) < (((var_9 * ((max(var_7, var_9))))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        var_12 = (((arr_3 [i_0] [i_0]) % ((var_2 ? (arr_1 [i_0]) : (arr_1 [i_0])))));
        arr_4 [i_0] = 7923;
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 19;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 22;i_3 += 1)
                {
                    {
                        var_13 = 129;
                        var_14 = (min(var_14, 65535));
                    }
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_4 = 2; i_4 < 19;i_4 += 1)
    {
        for (int i_5 = 0; i_5 < 23;i_5 += 1)
        {
            {
                arr_18 [i_4] [i_4] = ((((~(arr_14 [i_4] [i_4 + 3])))) && (~var_6));
                arr_19 [i_5] [i_5] [i_5] = var_3;
                arr_20 [i_4] = ((~((((((var_10 + (arr_17 [i_5] [i_4])))) || ((((arr_15 [i_4]) ? var_0 : var_8))))))));
            }
        }
    }
    #pragma endscop
}
