/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120987
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120987 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120987
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 14;i_4 += 1)
                        {
                            {
                                arr_12 [i_1] [i_2] [i_4 + 2] = ((32767 ? 1 : 32764));
                                var_16 = (max((((arr_3 [i_4 + 2] [i_4 + 1]) ? var_2 : (arr_11 [i_4] [i_4 + 3] [i_4 + 1] [i_4] [i_4 + 1]))), ((((max(2474, var_6))) ? (arr_0 [i_4 - 1]) : -10187844539871389301))));
                            }
                        }
                    }
                    var_17 = var_9;
                }
            }
        }
    }
    var_18 = (2474 ? var_2 : 32767);
    var_19 &= max((max(-2147483647, var_10)), (~var_13));
    var_20 += var_5;
    #pragma endscop
}
