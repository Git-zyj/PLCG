/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61180
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61180 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61180
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_18 -= (min(((-80 / (arr_3 [i_0] [i_1]))), (((arr_0 [i_0]) ? ((min(var_7, var_3))) : (max((arr_1 [18] [i_1]), (arr_0 [i_1])))))));
                arr_4 [i_1] |= (max((~var_7), (((-78 ? (arr_2 [i_0]) : (arr_2 [i_1]))))));
            }
        }
    }
    var_19 = (max((((min(var_1, var_15)) + var_0)), var_16));
    var_20 = var_15;
    #pragma endscop
}
