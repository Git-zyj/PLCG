/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 109872
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=109872 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/109872
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 2; i_2 < 7;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_13 = (((min(1, 13297))));
                                var_14 = arr_5 [i_4] [i_1];
                                var_15 *= ((13294 ? 13293 : ((-(var_12 - 936058006)))));
                                var_16 = var_5;
                                var_17 = (min(16383, ((min(1, -13291)))));
                            }
                        }
                    }
                }
                var_18 = (min(var_18, 16406));
            }
        }
    }

    for (int i_5 = 2; i_5 < 10;i_5 += 1) /* same iter space */
    {
        var_19 = (min(((-16383 ? 13308 : -13295)), ((~(arr_16 [i_5 - 1] [i_5 + 2])))));
        var_20 = 174;
    }
    for (int i_6 = 2; i_6 < 10;i_6 += 1) /* same iter space */
    {
        var_21 = (((min((arr_18 [i_6 - 1] [i_6 + 1]), var_7)) % (((-16387 % ((~(arr_20 [i_6] [i_6]))))))));
        var_22 = (min(48761, 32763));
        var_23 = (((-(2363647257 * 221))));
    }
    #pragma endscop
}
