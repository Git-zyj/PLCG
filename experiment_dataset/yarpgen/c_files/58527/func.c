/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 58527
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=58527 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/58527
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 *= ((/* implicit */unsigned long long int) var_0);
    /* LoopSeq 1 */
    /* vectorizable */
    for (int i_0 = 2; i_0 < 14; i_0 += 1) 
    {
        arr_4 [5ULL] = ((/* implicit */unsigned short) ((((/* implicit */_Bool) 2763295330475831173LL)) || (((/* implicit */_Bool) arr_0 [i_0]))));
        var_14 += ((/* implicit */unsigned long long int) ((arr_0 [i_0 - 1]) + (arr_0 [i_0 + 1])));
        var_15 = ((/* implicit */_Bool) (-(4294967294U)));
    }
}
