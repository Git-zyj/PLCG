/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60827
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60827 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60827
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 += var_2;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] [i_1] = (var_10 ? var_10 : ((var_10 ? var_0 : (var_1 | 1))));
                var_13 |= ((((~var_9) ? var_0 : (((arr_2 [i_0] [i_0] [i_0]) & (arr_1 [i_0]))))) | (max((arr_3 [i_0 - 1] [i_0 - 1]), (max(-14, var_2)))));
                var_14 &= 248;
            }
        }
    }
    var_15 = (((((((min(var_9, -14))) + (1 - -22315)))) ? var_10 : (max(76140339, 21))));
    var_16 = var_9;
    var_17 &= -3045574829;
    #pragma endscop
}
