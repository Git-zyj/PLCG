/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213879
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213879 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213879
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = -6735945680399540511;

    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        var_11 = (arr_1 [i_0]);
        var_12 = ((((max((((var_7 ? var_9 : 2214))), (min((arr_2 [i_0 - 2] [i_0]), var_5))))) ? 128 : 160052465));
    }
    for (int i_1 = 0; i_1 < 15;i_1 += 1)
    {
        arr_5 [i_1] = var_5;
        arr_6 [i_1] = (((arr_2 [i_1] [15]) * ((((!((max(128, (arr_3 [i_1] [i_1]))))))))));
    }
    for (int i_2 = 0; i_2 < 23;i_2 += 1)
    {
        var_13 = -1;
        var_14 ^= (+(((arr_8 [i_2] [i_2]) ? (arr_10 [i_2] [i_2]) : -16112)));
    }
    var_15 = (max(var_15, var_3));
    #pragma endscop
}
