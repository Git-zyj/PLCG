/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222544
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222544 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222544
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        var_18 = var_1;
        var_19 = 65524;
        var_20 -= ((((min((arr_0 [i_0]), (arr_1 [i_0])))) - (((var_6 >= (arr_0 [i_0]))))));
        var_21 = (min(var_21, (((((max(var_4, ((min((arr_0 [i_0]), (arr_1 [i_0]))))))) ? (max(0, (~12))) : ((max((arr_0 [i_0]), (arr_0 [i_0])))))))));
    }
    for (int i_1 = 0; i_1 < 11;i_1 += 1)
    {
        arr_6 [i_1] [i_1] = ((max((arr_0 [i_1]), -34926)));
        var_22 = (max(var_22, (-2147483647 - 1)));
    }
    var_23 = ((var_7 ? 37976 : 1952628852));
    #pragma endscop
}
