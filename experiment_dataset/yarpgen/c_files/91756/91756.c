/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91756
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91756 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91756
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_19 = (min(var_19, var_4));
                    var_20 = (~0);
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 9;i_4 += 1)
                        {
                            {
                                var_21 = (min(var_21, ((min((max(var_14, var_5)), (~var_7))))));
                                var_22 = var_2;
                                var_23 = (min(var_23, (~var_9)));
                            }
                        }
                    }
                    var_24 = var_7;
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_5 = 0; i_5 < 13;i_5 += 1)
    {
        for (int i_6 = 2; i_6 < 11;i_6 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_7 = 0; i_7 < 13;i_7 += 1)
                {
                    for (int i_8 = 1; i_8 < 12;i_8 += 1)
                    {
                        {
                            arr_24 [i_6] [i_8] [i_6] [i_6] [i_6] = (min((((~(max(var_18, var_8))))), var_10));
                            var_25 = (max(var_25, (~32767)));
                            var_26 = (max(var_26, (((-((min(var_8, var_10))))))));
                        }
                    }
                }
                var_27 = (~(~var_14));
                var_28 = (max(var_28, var_18));
            }
        }
    }
    /* LoopNest 3 */
    for (int i_9 = 0; i_9 < 12;i_9 += 1)
    {
        for (int i_10 = 4; i_10 < 9;i_10 += 1)
        {
            for (int i_11 = 1; i_11 < 1;i_11 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_12 = 1; i_12 < 1;i_12 += 1)
                    {
                        for (int i_13 = 0; i_13 < 12;i_13 += 1)
                        {
                            {
                                var_29 &= ((~((var_5 ? -var_8 : (((max(var_17, var_9))))))));
                                var_30 = (max(var_30, var_11));
                            }
                        }
                    }
                    var_31 = (min(var_31, var_1));
                    /* LoopNest 2 */
                    for (int i_14 = 0; i_14 < 12;i_14 += 1)
                    {
                        for (int i_15 = 0; i_15 < 12;i_15 += 1)
                        {
                            {
                                var_32 *= (((((var_11 ? var_16 : var_3))) ? (!12354323347787355628) : -var_12));
                                var_33 = (min(var_33, (((var_0 ? var_3 : (min(var_11, var_16)))))));
                            }
                        }
                    }
                    var_34 = min(var_4, var_5);
                    var_35 &= ((!(~var_17)));
                }
            }
        }
    }
    var_36 = var_1;
    #pragma endscop
}
