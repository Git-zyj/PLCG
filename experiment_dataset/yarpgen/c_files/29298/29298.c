/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29298
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29298 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29298
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 |= (max(var_14, ((((var_4 ^ var_9) > var_17)))));
    var_21 = var_17;
    var_22 = var_2;

    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        var_23 = (min(var_23, ((((((~(arr_1 [i_0 + 2])))) ? (((var_19 && (var_7 | 1)))) : (arr_2 [i_0]))))));
        var_24 = var_3;
    }
    for (int i_1 = 0; i_1 < 17;i_1 += 1)
    {
        arr_6 [1] = (min(((min((arr_1 [i_1]), (arr_1 [i_1])))), ((-(arr_5 [i_1])))));
        arr_7 [i_1] = ((~(arr_4 [15])));
        arr_8 [1] &= (min(((var_12 - (arr_3 [i_1]))), (arr_3 [i_1])));
        arr_9 [i_1] [i_1] = (arr_5 [i_1]);
    }
    #pragma endscop
}
