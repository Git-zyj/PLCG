/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 238131
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=238131 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/238131
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 20;i_0 += 1)
    {
        arr_4 [i_0] = var_13;
        /* LoopNest 3 */
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 22;i_3 += 1)
                {
                    {
                        var_15 = (max(var_15, (((min(((-(arr_12 [i_0] [i_3] [i_3] [i_3])), 0)))))));
                        var_16 += ((!(((-18729 ? (arr_3 [i_0]) : ((~(arr_10 [i_2] [i_3]))))))));
                    }
                }
            }
        }
        var_17 = (max((arr_13 [i_0] [i_0] [i_0]), ((-(((arr_11 [i_0 + 1] [6] [i_0]) / -24))))));
    }
    /* vectorizable */
    for (int i_4 = 1; i_4 < 19;i_4 += 1)
    {
        var_18 = var_10;
        var_19 = -20;
        var_20 = (min(var_20, (arr_9 [i_4])));
        var_21 = ((~(arr_3 [i_4])));
    }
    var_22 = (max(var_22, ((min(var_14, var_3)))));
    #pragma endscop
}
