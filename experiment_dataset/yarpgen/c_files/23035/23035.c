/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23035
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23035 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23035
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        {
                            var_14 -= ((!(arr_2 [i_0] [i_0] [i_0 + 1])));
                            var_15 = (arr_6 [i_0] [7] [i_2] [i_3] [i_0]);
                            var_16 = ((((max(34648, (arr_7 [i_0] [i_0] [i_0 + 1] [i_0 - 3])))) ? -58726 : ((-(arr_7 [i_0 - 2] [i_1] [i_2] [4])))));
                        }
                    }
                }
                var_17 = (min(1987425079, (((arr_3 [i_0 + 1]) ? 3591789724 : 1549475235261150934))));
            }
        }
    }
    var_18 = var_4;
    var_19 = var_1;
    #pragma endscop
}
