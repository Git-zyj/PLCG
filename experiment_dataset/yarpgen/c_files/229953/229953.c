/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229953
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229953 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229953
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_7;
    var_11 |= ((((((var_8 ? var_4 : var_7))) ? ((min(var_2, var_8))) : (var_2 < var_1))) * var_5);
    var_12 = var_0;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_13 = (max(var_13, (((((-(arr_6 [i_0] [i_1] [i_0])))) ? (arr_6 [i_0] [i_1] [i_0]) : (((arr_3 [i_0] [i_1] [i_2]) ? (arr_0 [i_0] [i_0]) : (arr_3 [i_0] [i_1] [i_2])))))));
                    var_14 = 46;
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 12;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 12;i_4 += 1)
        {
            {
                arr_14 [i_3] [i_3] [i_3] = ((-(arr_13 [i_4])));
                var_15 = (arr_11 [i_4]);
            }
        }
    }
    #pragma endscop
}
