/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59537
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59537 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59537
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    var_20 *= ((((0 / 32) ? ((633897273 ? 208 : -1572615245710662297)) : 208)) & (((((var_13 ^ var_17) < ((var_1 ? var_4 : var_11)))))));
                    arr_8 [i_1] &= ((min((arr_4 [i_0] [i_0]), (arr_0 [i_0] [i_1]))));
                }
            }
        }
    }
    var_21 = (min((max((min(var_9, var_19)), ((min(var_13, var_3))))), var_18));
    #pragma endscop
}
