/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163137
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163137 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163137
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_9;

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = ((((((65535 ? (arr_0 [i_0] [i_0]) : 1)))) || var_2));
        var_13 += (((((((((arr_2 [i_0]) ? var_10 : var_10))) ? (var_4 >= var_8) : ((var_6 ? 1 : var_11))))) ? (27858 ^ 172) : ((253 ? 53025 : (~1)))));
    }
    var_14 |= (max(var_1, ((max(var_1, var_10)))));
    #pragma endscop
}
