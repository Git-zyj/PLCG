/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196684
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196684 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196684
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_1 + 1] = var_4;
                var_15 ^= (max((((((-(arr_4 [8])))) ? -var_1 : ((var_12 ? (arr_0 [i_0] [i_1]) : var_5)))), var_6));
                var_16 = arr_2 [i_1];
            }
        }
    }
    var_17 |= (((((((max(8322485449805364698, 4294967286))) ? ((var_0 ? var_3 : var_13)) : var_10))) ? var_6 : ((var_3 ? ((var_7 ? var_1 : var_9)) : ((var_3 ? 59 : var_1))))));
    var_18 = ((117 ? 1 : 251));
    var_19 = var_2;
    #pragma endscop
}
