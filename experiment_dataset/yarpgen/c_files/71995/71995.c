/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71995
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71995 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71995
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    arr_8 [i_0] = -3917943615473354104;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 23;i_4 += 1)
                        {
                            {
                                arr_16 [i_0] [i_0] [i_4] [i_3] [i_4] = ((+((((arr_9 [i_4] [i_3] [i_3] [i_2] [19] [i_0 - 1]) ? (arr_12 [i_1] [i_1]) : (arr_9 [14] [i_1] [i_2] [i_3] [i_3] [i_4]))))));
                                arr_17 [i_4] = (106 > 11);
                                var_13 = (arr_0 [i_3] [i_4]);
                            }
                        }
                    }
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_5 = 1; i_5 < 22;i_5 += 1)
    {
        for (int i_6 = 1; i_6 < 21;i_6 += 1)
        {
            {
                arr_25 [i_5] = var_10;
                arr_26 [i_5] = (((arr_24 [i_6] [i_5] [i_5]) ? ((-((((arr_22 [i_5]) || (arr_18 [11] [1])))))) : (arr_21 [i_6 - 1] [22])));
            }
        }
    }
    var_14 = var_10;
    var_15 |= var_10;
    #pragma endscop
}
