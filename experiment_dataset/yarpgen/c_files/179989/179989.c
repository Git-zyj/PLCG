/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179989
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179989 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179989
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((((max(5345, (!-51)))) ? var_10 : (max((min(var_3, var_14)), var_18))));
    var_20 = var_5;
    var_21 = ((((((var_9 ? var_2 : var_15)) % var_2)) > (min(51, (min(var_5, -39))))));
    var_22 = (var_2 == var_3);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_6 [i_0] = (((((((min(var_2, var_1))) ? var_10 : (arr_0 [i_0])))) ? (arr_1 [i_1] [2]) : 2107008260));
                arr_7 [i_0] = (((((arr_5 [i_0]) ? (arr_4 [i_0] [i_0]) : (arr_1 [i_0] [i_1]))) < -51));
            }
        }
    }
    #pragma endscop
}
