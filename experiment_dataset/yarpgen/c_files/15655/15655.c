/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15655
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15655 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15655
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    arr_9 [i_0] [9] [1] = var_5;
                    arr_10 [10] [9] [10] = (((arr_2 [i_0] [i_0] [i_0]) ? (~1369868685) : ((((!(arr_2 [i_0] [i_1] [i_2])))))));
                    arr_11 [7] [i_1] [6] = (((~274877906943) ? 1757112659 : (max(((max(-7627, 0))), (~var_5)))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_12 = (((var_7 ? var_8 : var_9)));
                                var_13 = (min(var_13, ((max(((~(arr_6 [5] [3] [i_2]))), (!8192))))));
                                var_14 = max((~var_0), (arr_16 [i_3]));
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 11;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 11;i_6 += 1)
                        {
                            {
                                var_15 = (max(var_15, (((~(((((min(var_6, var_1))) && ((max((arr_7 [i_0] [7]), var_5)))))))))));
                                var_16 = var_3;
                            }
                        }
                    }
                }
            }
        }
    }
    var_17 = var_1;
    #pragma endscop
}
