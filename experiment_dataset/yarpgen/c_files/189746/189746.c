/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 189746
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=189746 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/189746
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (var_13 >= var_9);
    var_17 = ((var_12 ? var_9 : (min((2001518361 ^ var_8), var_12))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] [i_2] = (((arr_6 [i_1] [i_1] [i_1] [11]) || -106));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 23;i_4 += 1)
                        {
                            {
                                arr_15 [i_0] [i_1] [i_2] [5] [i_4] = ((var_9 ? (((arr_14 [i_4] [i_1]) + (arr_14 [i_1] [i_4]))) : (max(var_10, (arr_14 [i_0] [i_3])))));
                                var_18 = (min((var_11 + var_8), (min(var_0, var_1))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_19 = var_2;
    var_20 = var_3;
    #pragma endscop
}
