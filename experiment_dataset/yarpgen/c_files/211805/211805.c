/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211805
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211805 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211805
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 ^= var_16;
    var_20 = ((-(min(var_2, ((13343245802776992280 ? var_7 : var_4))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 7;i_2 += 1)
            {
                {
                    var_21 = arr_6 [i_2 + 2] [i_2 - 1] [i_2 - 2];

                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        arr_9 [i_0] [i_1] = (min((max((arr_6 [i_0] [i_0] [i_0]), (arr_6 [i_0] [i_1] [i_2 - 1]))), (((arr_6 [i_2 + 3] [i_0] [i_0]) ? (arr_6 [i_3] [i_3] [i_2]) : (arr_6 [i_3] [i_2 + 1] [i_0])))));
                        arr_10 [i_0] [i_1] [7] [i_0] = ((-((var_12 ? (arr_0 [i_0] [i_0]) : (((var_5 > (arr_5 [i_2] [i_0]))))))));
                    }
                }
            }
        }
    }
    var_22 = var_18;
    #pragma endscop
}
