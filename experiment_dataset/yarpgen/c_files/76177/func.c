/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76177
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76177 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76177
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned short i_0 = 0; i_0 < 22; i_0 += 1) 
    {
        arr_3 [i_0] = ((/* implicit */unsigned short) arr_2 [(unsigned short)4]);
        var_19 = ((/* implicit */short) arr_1 [i_0] [i_0]);
        var_20 = ((/* implicit */unsigned int) ((((/* implicit */int) ((signed char) (unsigned short)65535))) != ((~(((/* implicit */int) (unsigned char)88))))));
    }
    var_21 = ((/* implicit */_Bool) var_1);
}
