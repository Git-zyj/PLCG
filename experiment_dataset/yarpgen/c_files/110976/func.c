/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110976
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110976 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110976
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned int i_0 = 0; i_0 < 25; i_0 += 3) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            for (unsigned short i_2 = 0; i_2 < 25; i_2 += 2) 
            {
                {
                    arr_10 [i_0] [i_1] [i_1] [i_2] = ((/* implicit */unsigned char) (-(((int) arr_4 [i_0]))));
                    var_13 = ((/* implicit */unsigned int) (-(((/* implicit */int) ((unsigned char) arr_2 [i_0])))));
                    var_14 += ((/* implicit */unsigned short) (-(2513717906279128550ULL)));
                }
            } 
        } 
    } 
    var_15 = ((/* implicit */unsigned char) var_8);
}
