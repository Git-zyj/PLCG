/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 188639
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=188639 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/188639
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_11 = (max(var_0, ((((arr_4 [i_0 + 1]) ^ (arr_2 [i_0 + 1] [i_1 + 1]))))));
                arr_5 [i_0] [i_0] [i_1] = var_1;
            }
        }
    }
    var_12 = var_9;
    /* LoopNest 2 */
    for (int i_2 = 3; i_2 < 12;i_2 += 1)
    {
        for (int i_3 = 2; i_3 < 12;i_3 += 1)
        {
            {
                arr_10 [i_3] = (((arr_3 [i_2 + 2] [i_3 - 2]) ? (max(123, (arr_3 [i_2 - 3] [i_3 + 1]))) : (max((arr_3 [i_2 - 3] [i_3 - 1]), -123))));
                arr_11 [i_3] = var_4;
                arr_12 [i_2] [i_3] = ((min((arr_3 [i_3 - 2] [i_2 - 1]), var_4)));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_4 = 0; i_4 < 1;i_4 += 1)
    {
        for (int i_5 = 0; i_5 < 25;i_5 += 1)
        {
            {
                arr_18 [i_5] [i_4] = ((((((var_4 ? (arr_17 [i_4] [i_4]) : (arr_15 [i_5])))) ? (min(var_0, var_5)) : var_7)));
                var_13 = 4093566273352325495;
                var_14 = (max(var_14, var_5));
            }
        }
    }
    #pragma endscop
}
