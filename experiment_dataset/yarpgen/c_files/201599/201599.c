/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 201599
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=201599 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/201599
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 |= (min(var_16, ((((var_15 ? var_11 : var_6))))));
    var_20 = 2719110394;
    var_21 = var_17;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            {
                var_22 = (max((~var_0), (((var_11 ? 0 : 16)))));
                arr_4 [i_0] [i_1] [i_1 + 3] = (((var_1 && var_16) ? (arr_0 [i_0]) : ((min((arr_0 [i_0]), (arr_0 [i_1 + 1]))))));
                arr_5 [i_0] [i_1 + 2] = ((((((arr_0 [i_0]) | (arr_0 [i_1 - 1])))) > (((arr_3 [i_0]) % (arr_1 [i_1] [i_0])))));
            }
        }
    }
    #pragma endscop
}
