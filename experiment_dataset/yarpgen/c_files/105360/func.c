/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105360
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105360 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105360
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */unsigned int) var_1);
    var_20 = ((/* implicit */unsigned int) (-9223372036854775807LL - 1LL));
    var_21 = ((/* implicit */int) max((((/* implicit */long long int) var_1)), (144115188075331584LL)));
    /* LoopNest 3 */
    for (unsigned short i_0 = 0; i_0 < 18; i_0 += 3) 
    {
        for (unsigned short i_1 = 1; i_1 < 15; i_1 += 2) 
        {
            for (unsigned long long int i_2 = 0; i_2 < 18; i_2 += 3) 
            {
                {
                    var_22 = ((/* implicit */unsigned long long int) -1347635748);
                    var_23 = ((/* implicit */unsigned short) ((max((((/* implicit */int) arr_7 [i_1 - 1] [i_1 + 3] [i_2])), (-1347635747))) / (((/* implicit */int) arr_7 [i_1 + 3] [i_1] [i_1]))));
                }
            } 
        } 
    } 
}
