/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 225810
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=225810 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/225810
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (((((((var_9 ? var_0 : var_0)) - 1985904407))) ? ((min(var_8, var_2))) : var_8));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    var_13 = -8638737847095167543;
                    var_14 = (min((((arr_6 [i_2] [i_2] [i_2]) ? (arr_6 [i_2] [i_1] [i_2]) : var_1)), (((!(arr_6 [i_0] [i_1] [i_2]))))));
                }
            }
        }
    }
    var_15 -= (((((-1985904382 | var_8) & -var_8)) / ((-1985904425 ? -1985904425 : 1985904387))));
    #pragma endscop
}
