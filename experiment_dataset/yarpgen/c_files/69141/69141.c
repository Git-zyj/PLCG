/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 69141
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=69141 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/69141
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min(var_15, var_14));

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        arr_3 [4] = ((((var_11 >= (max(14942876184373824075, 32761)))) ? var_5 : (((((896 ? var_5 : 11606970738526310212))) ? ((var_10 ? var_0 : var_5)) : (!var_7)))));
        var_16 -= (max(((var_13 * (arr_0 [i_0] [i_0]))), ((((((64639 ? var_3 : var_10))) && (arr_0 [i_0] [i_0]))))));
        arr_4 [i_0] = var_5;
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        arr_8 [i_1] [i_1] = arr_7 [i_1] [9];
        arr_9 [i_1] = var_8;
    }
    var_17 = 3690480002;
    #pragma endscop
}
