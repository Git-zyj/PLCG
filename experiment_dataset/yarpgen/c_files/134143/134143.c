/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 134143
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=134143 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/134143
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_0] [i_1] [i_2] = (((((var_10 ? (arr_5 [i_0] [i_1] [i_2] [8]) : var_11))) ? ((~((-457592238 ? (arr_5 [i_0] [i_1] [i_1] [15]) : 1)))) : (arr_4 [i_0] [i_1] [i_2])));
                    arr_7 [i_1] [i_1] [i_0] [i_1] = (max(((-(max(var_2, (arr_5 [i_1] [i_1] [21] [5]))))), var_14));
                }
            }
        }
    }
    var_20 = var_14;
    var_21 = (max(169, -1));
    var_22 ^= var_9;
    #pragma endscop
}
