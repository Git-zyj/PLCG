/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152168
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152168 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152168
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_5;
    var_12 &= (max(((((max((-127 - 1), -30))) ? var_5 : (-2147483647 - 1))), 2147483647));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_13 = ((arr_0 [i_1] [i_0]) ? ((min((arr_0 [i_0] [i_0]), (arr_0 [i_0] [i_1])))) : ((max((arr_0 [i_0] [i_1]), (arr_0 [i_0] [i_1])))));
                arr_4 [i_0] [4] = max(((1 ^ (-127 - 1))), (max((-2147483647 - 1), 2147483630)));
            }
        }
    }
    #pragma endscop
}
