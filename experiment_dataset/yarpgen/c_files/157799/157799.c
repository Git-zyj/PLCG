/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157799
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157799 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157799
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 -= var_1;
    var_15 += var_2;
    var_16 &= 63;
    var_17 = (max(var_17, 255));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 23;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = -9223372036854775790;
                arr_5 [i_0] = (arr_3 [i_0]);
                /* LoopNest 2 */
                for (int i_2 = 3; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        {
                            var_18 += ((!((max(((min(var_1, 150))), 22806)))));
                            var_19 = var_11;
                            var_20 |= (arr_2 [i_2 - 3] [i_1] [22]);
                        }
                    }
                }
                var_21 = ((((((((((var_12 + 2147483647) << (((var_12 + 1956875806) - 30))))) || (((-1560 | (arr_1 [i_0] [i_0])))))))) ^ (~var_13)));
            }
        }
    }
    #pragma endscop
}
