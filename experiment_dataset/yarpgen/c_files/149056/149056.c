/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149056
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149056 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149056
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_3;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_13 = (arr_2 [i_0] [i_0]);
                arr_7 [12] [i_0] [12] = ((58648 ? 1046265875 : -2825));
            }
        }
    }

    for (int i_2 = 0; i_2 < 23;i_2 += 1)
    {
        /* LoopNest 2 */
        for (int i_3 = 0; i_3 < 23;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 23;i_4 += 1)
            {
                {
                    arr_16 [i_2] [i_2] [i_4] = (!var_5);
                    var_14 = (((arr_10 [i_4] [3] [i_4]) ? (~-1695250128) : ((((arr_12 [i_2] [i_2]) != (min((arr_12 [i_4] [i_2]), (arr_11 [i_2] [i_2]))))))));
                    var_15 = (max(var_15, (arr_10 [i_4] [16] [i_2])));
                }
            }
        }
        var_16 = (max(var_16, -127));
    }
    var_17 = (max(var_17, var_5));
    #pragma endscop
}
