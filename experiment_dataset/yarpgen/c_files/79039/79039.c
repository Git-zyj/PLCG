/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 79039
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=79039 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/79039
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (!var_4);
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 21;i_2 += 1)
            {
                {
                    arr_6 [i_1] = (min((((arr_4 [i_1] [i_2 - 1] [i_2]) - 4092661936)), (((arr_4 [i_1] [i_2 + 1] [i_2 + 1]) ? 4092661955 : -8736))));
                    arr_7 [i_0] [i_0] [i_2] = ((((((arr_4 [i_2] [i_0 - 1] [i_0 + 2]) || (arr_4 [i_2] [i_0 - 1] [i_0 + 1])))) & ((min((arr_1 [i_0 - 1]), (arr_1 [i_0 + 2]))))));
                    arr_8 [i_2] = (((var_7 && var_1) - (var_14 - 202305356)));
                    arr_9 [i_0] [i_0] [i_1] [i_2] |= var_11;
                }
            }
        }
    }
    #pragma endscop
}
