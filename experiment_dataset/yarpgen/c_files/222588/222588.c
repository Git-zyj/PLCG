/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222588
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222588 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222588
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 |= 0;
    var_11 = var_6;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    var_12 = -34;
                    arr_6 [i_0] [0] [i_0] [i_0] = 5644104949373572526;
                    var_13 = (max(var_13, (((((((((3005918736 ? 12802639124335979090 : 3555295005976356331))) ? ((((arr_3 [14]) ? (arr_0 [5] [i_2]) : (arr_5 [10] [i_1])))) : 5644104949373572526))) ? (((((var_6 ? 5595573662368008569 : (arr_1 [0]))) <= 20037))) : (-127 - 1))))));
                }
            }
        }
    }
    #pragma endscop
}
