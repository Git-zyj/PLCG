/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 96134
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=96134 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/96134
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */signed char) (-(((/* implicit */int) var_3))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10; i_0 += 2) 
    {
        for (int i_1 = 1; i_1 < 9; i_1 += 1) 
        {
            {
                var_13 = ((/* implicit */short) ((((/* implicit */_Bool) ((((/* implicit */unsigned long long int) ((/* implicit */int) var_2))) - (arr_6 [i_1 + 1] [i_0 + 1])))) ? (((((/* implicit */int) (unsigned short)16384)) - (max((var_4), (((/* implicit */int) (_Bool)1)))))) : ((+(((((/* implicit */int) (signed char)-69)) + (((/* implicit */int) (short)8192))))))));
                var_14 = ((/* implicit */_Bool) var_11);
            }
        } 
    } 
}
