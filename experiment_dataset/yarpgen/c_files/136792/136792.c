/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136792
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136792 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136792
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_15;
    var_21 = (min(var_21, ((min(1, var_4)))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        var_22 = (max(var_22, (arr_1 [i_0] [i_0])));

        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            var_23 = ((arr_3 [i_1] [1] [i_1]) ? 1 : (arr_1 [i_0] [i_1]));
            arr_4 [i_0] [10] [i_0] = (arr_1 [i_1] [i_0]);
            var_24 = (((arr_3 [10] [i_0] [10]) ? 1 : (arr_1 [i_0] [i_0])));
        }
        arr_5 [i_0] [i_0] = (arr_0 [i_0]);
    }
    for (int i_2 = 1; i_2 < 12;i_2 += 1)
    {
        arr_8 [i_2] = arr_7 [i_2 + 1] [i_2 + 1];
        var_25 = (max((arr_7 [i_2 + 2] [i_2 + 2]), (arr_7 [i_2 + 2] [i_2 + 1])));
        var_26 ^= (((arr_7 [i_2 - 1] [i_2 + 1]) ? (((arr_7 [i_2 + 1] [i_2 + 1]) ? (arr_3 [i_2 + 2] [i_2 + 2] [i_2 - 1]) : (arr_3 [i_2] [i_2 + 2] [i_2 + 1]))) : (1 / 3950796777)));
        arr_9 [i_2] = (arr_6 [i_2]);
    }
    #pragma endscop
}
