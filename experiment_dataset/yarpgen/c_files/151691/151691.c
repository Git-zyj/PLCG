/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151691
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151691 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151691
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_18 -= -1;
                arr_5 [i_1] [i_1] [1] = (((-7 | var_0) & ((~(arr_2 [i_1] [i_0])))));
                arr_6 [i_0] = ((~((~((var_13 ? 0 : var_8))))));
                arr_7 [i_0] &= ((((max((arr_3 [i_0]), (arr_3 [i_0])))) <= (0 - 961664371)));
                arr_8 [i_0] = (min((0 - -27), ((min((max(-18372, (arr_4 [i_0]))), var_6)))));
            }
        }
    }
    var_19 = (max(var_19, 255));
    var_20 -= ((var_13 ? 7 : (115 / var_13)));
    #pragma endscop
}
