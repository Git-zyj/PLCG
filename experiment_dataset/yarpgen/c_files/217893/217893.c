/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217893
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217893 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217893
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        arr_10 [i_0] [i_0] [i_0] [i_0] = (((((var_1 / var_9) + 2147483647)) >> (var_1 + 271226868)));
                        var_13 *= (((((var_5 + 2147483647) >> (var_10 + 87))) > (var_8 == var_5)));
                    }
                    arr_11 [i_2] [1] [1] [i_0] &= ((var_5 | (var_3 | var_0)));
                    var_14 = var_2;
                    arr_12 [i_0] [i_1] [i_0] [i_0] = var_6;
                }
            }
        }
    }
    var_15 |= var_10;
    #pragma endscop
}
