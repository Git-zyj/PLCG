/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120744
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120744 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120744
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                var_16 = (max((((((-20647 ? 101 : -78))) ? (arr_0 [i_0 - 4]) : (((arr_0 [i_0]) ? -22452 : var_14)))), (max((min((arr_5 [i_0] [i_1]), var_12)), (arr_5 [i_1 - 1] [i_1])))));
                var_17 = (((((((max(2147483647, var_12))) ? (!var_7) : (!var_7)))) ? var_6 : (max(var_2, (arr_2 [i_1] [i_0])))));
            }
        }
    }
    var_18 = var_6;
    #pragma endscop
}
