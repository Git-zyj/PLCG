/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 109645
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=109645 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/109645
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_4 [i_0] = arr_3 [5] [i_1];
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 13;i_3 += 1)
                    {
                        {
                            arr_10 [i_0] [i_3] = var_1;
                            arr_11 [i_3] [i_2] [i_1] [i_0] [4] [i_0] = (((var_1 ? (((arr_1 [i_0]) ? var_9 : var_0)) : (arr_1 [i_0]))));
                            var_11 = (max(var_11, 0));
                            var_12 += (((-(arr_3 [i_0] [i_3 + 3]))));
                            arr_12 [1] [i_1] &= ((~((max((arr_5 [i_0] [i_0] [i_3] [i_3 + 3]), (arr_5 [i_0] [i_1] [i_2] [i_3 + 3]))))));
                        }
                    }
                }
                arr_13 [i_0] [i_0] = 82;
                var_13 = (arr_6 [i_0] [i_1] [i_0]);
            }
        }
    }
    #pragma endscop
}
