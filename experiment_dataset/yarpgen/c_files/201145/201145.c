/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 201145
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=201145 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/201145
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 15;i_1 += 1)
        {
            {
                arr_8 [i_0] [i_1] = (min(var_4, (!var_0)));
                arr_9 [i_1] [i_0] [i_0] = ((min(((8945651926080377160 ? 1 : (arr_3 [i_0] [i_0]))), (((arr_0 [i_0]) ? var_13 : 1)))));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 14;i_4 += 1)
                        {
                            {
                                var_15 = var_1;
                                var_16 -= (arr_3 [i_0] [i_1]);
                            }
                        }
                    }
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_5 = 0; i_5 < 1;i_5 += 1)
    {
        for (int i_6 = 2; i_6 < 7;i_6 += 1)
        {
            {
                arr_24 [i_5] [2] [i_6] &= ((var_4 ? (arr_1 [i_5]) : (max((max(var_3, 0)), ((((arr_6 [i_5] [i_5] [i_6]) ? 1 : 1)))))));
                arr_25 [i_5] = var_9;
            }
        }
    }
    #pragma endscop
}
