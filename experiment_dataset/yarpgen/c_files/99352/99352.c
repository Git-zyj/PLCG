/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99352
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99352 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99352
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max(var_13, var_11));
    var_14 = 211636462;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 12;i_1 += 1)
        {
            {
                arr_4 [i_0] = max((((arr_0 [i_0]) ? (arr_0 [i_0]) : (arr_2 [i_1 - 2] [i_0 + 3] [i_0]))), ((min(var_2, (arr_3 [i_0] [i_1])))));
                var_15 &= (-211636437 ? (-2147483647 - 1) : 115441150);
            }
        }
    }
    var_16 = (min(var_16, ((((var_1 < -6777170540696863457) ? var_8 : ((var_10 ? var_1 : ((var_12 ? var_4 : 7)))))))));
    #pragma endscop
}
