/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120970
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120970 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120970
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 *= ((max(((255 ? 7941 : -7941)), var_1)));
    var_14 = (max(var_14, var_7));
    var_15 = (((((max(var_4, var_5))) ? (var_9 ^ var_12) : (max(var_0, var_8)))));

    /* vectorizable */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        var_16 &= (((var_7 ? var_12 : var_8)));
        arr_4 [i_0 - 2] = (((var_7 ? var_2 : var_12)));
        arr_5 [i_0 - 2] &= var_0;
        arr_6 [i_0 - 2] [i_0] = var_12;
    }
    #pragma endscop
}
