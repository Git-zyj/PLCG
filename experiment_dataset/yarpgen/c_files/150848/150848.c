/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 150848
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=150848 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/150848
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_11;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_17 = ((((min((2200192071 & 0), (arr_5 [i_1 - 3] [i_1 + 1] [i_1 - 3])))) ? (((~(arr_4 [i_1 - 1] [i_1 - 1] [i_1 + 1] [i_1 - 4])))) : (((max(var_12, (arr_2 [i_2] [i_1] [i_0]))) & ((((arr_1 [i_0] [i_2]) ? (arr_4 [i_0] [i_1] [i_1] [i_2]) : var_2)))))));
                    var_18 = var_8;
                    var_19 = ((-(((var_15 == (((var_9 ^ (arr_3 [i_0])))))))));
                }
            }
        }
    }
    #pragma endscop
}
