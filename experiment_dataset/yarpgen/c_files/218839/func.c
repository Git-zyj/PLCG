/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218839
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218839 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218839
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 22; i_0 += 2) 
    {
        arr_4 [i_0] = ((/* implicit */unsigned long long int) min((((unsigned int) 4133592565U)), (((unsigned int) var_6))));
        arr_5 [i_0] = ((/* implicit */unsigned char) min((((unsigned short) ((unsigned int) arr_2 [8] [i_0]))), (((/* implicit */unsigned short) ((_Bool) (unsigned char)7)))));
    }
    var_18 = ((/* implicit */unsigned int) var_12);
}
