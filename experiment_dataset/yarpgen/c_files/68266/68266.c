/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68266
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68266 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68266
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_20 = (min(var_20, (((((((arr_3 [1]) / (arr_1 [i_0 + 1])))) ? (((arr_3 [0]) ? (arr_3 [12]) : (arr_3 [1]))) : (-42 / -68))))));
                var_21 -= (((arr_0 [i_0] [i_0]) || (arr_1 [i_0])));
                var_22 *= (((((arr_1 [i_0 - 1]) ? (arr_1 [i_0 - 1]) : (arr_1 [i_0 - 1]))) >= ((((((arr_3 [6]) ? (arr_4 [1] [i_0] [i_0]) : (arr_0 [i_1] [i_1]))) % (arr_3 [2]))))));
            }
        }
    }
    var_23 &= var_7;
    var_24 = (min(var_24, ((((var_9 && var_11) ? var_12 : ((var_5 ? var_0 : (var_11 - var_2))))))));
    #pragma endscop
}
