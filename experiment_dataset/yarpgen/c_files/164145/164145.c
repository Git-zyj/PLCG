/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164145
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164145 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164145
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min(((-var_11 ? (18446744073709551615 & 0) : (((max(1, 1)))))), (((-(!var_2))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_14 = (arr_1 [i_0] [i_1]);
                arr_6 [i_0] [i_0] = 2761993009;
                arr_7 [i_0] [i_0] = (max((~-8302665451778256297), ((var_5 ? -var_4 : (((arr_2 [i_0] [i_0]) * 18446744073709551602))))));
            }
        }
    }
    var_15 = var_1;
    var_16 = 18446744073709551615;
    #pragma endscop
}
