/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83568
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83568 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83568
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_1] = (arr_0 [7]);
                var_16 -= (min(1, (-1 == 1)));
                var_17 = (min(var_17, ((((((arr_0 [i_0]) ? (arr_0 [i_1]) : (arr_0 [i_0])))) ? ((min((arr_0 [i_1]), (arr_0 [i_0])))) : (((1 || (arr_0 [i_0]))))))));
                var_18 = min(((max((min(18, var_5)), (arr_2 [i_0] [i_0])))), 2);
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 12;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 12;i_3 += 1)
        {
            {
                var_19 = ((~((min((arr_10 [i_3]), 7362)))));
                var_20 ^= -7362;
            }
        }
    }
    #pragma endscop
}
