/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244430
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244430 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244430
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 |= ((var_4 & (((var_2 <= (740027340 <= var_3))))));
    var_11 = var_0;

    for (int i_0 = 3; i_0 < 15;i_0 += 1)
    {
        arr_3 [i_0] = var_8;
        arr_4 [1] &= min(1, ((var_4 ? (arr_1 [i_0] [i_0 - 1]) : var_5)));
        var_12 += (max((var_7 & 4294967291), ((((var_8 ? var_7 : var_5))))));
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_13 = (((~(((arr_5 [i_1]) / var_4)))));
        arr_7 [i_1] = ((0 ? ((~(arr_5 [i_1]))) : ((max(var_3, (min(var_6, (arr_5 [i_1]))))))));
    }
    #pragma endscop
}
