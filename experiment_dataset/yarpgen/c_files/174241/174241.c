/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174241
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174241 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174241
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_13 -= var_5;
        var_14 = ((!(((var_7 ^ (arr_1 [i_0]))))));
        var_15 -= (~10846);
    }
    for (int i_1 = 0; i_1 < 19;i_1 += 1)
    {
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 19;i_3 += 1)
            {
                {
                    var_16 = (((max((max(8574692133116797862, 200)), ((max(var_4, var_9)))))) ? (arr_2 [10] [i_2]) : ((-(arr_6 [i_3] [i_1]))));
                    var_17 = (((!-70) + var_12));
                    arr_10 [i_2] [i_1] [i_3] = var_1;
                }
            }
        }
        var_18 = (min((arr_2 [i_1] [i_1]), ((~(arr_2 [i_1] [i_1])))));
        var_19 = var_12;
        /* LoopNest 2 */
        for (int i_4 = 0; i_4 < 1;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 19;i_5 += 1)
            {
                {
                    var_20 = var_7;
                    var_21 = 1;
                    var_22 = (min(89, (max((arr_11 [i_1] [i_4] [i_5]), (arr_11 [i_1] [i_4] [i_1])))));
                }
            }
        }
    }
    var_23 = (~-10847);
    var_24 = var_12;
    #pragma endscop
}
