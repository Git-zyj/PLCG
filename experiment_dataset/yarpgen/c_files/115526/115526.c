/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 115526
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=115526 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/115526
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 7;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_19 = (((((arr_0 [i_0] [i_0]) | (arr_3 [i_0] [i_0]))) % var_18));
                    var_20 = ((((+(max((arr_2 [i_0] [i_0] [i_0]), 3522)))) > ((((3522 <= ((-(arr_4 [i_0])))))))));
                }
            }
        }
    }
    var_21 += (max(var_0, (3519 || var_4)));
    var_22 *= (min(((((-3507 - var_1) / (~var_11)))), (min(var_10, 3522))));
    var_23 = var_3;
    #pragma endscop
}
