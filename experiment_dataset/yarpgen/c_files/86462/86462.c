/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86462
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86462 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86462
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_0] = (arr_0 [i_0] [i_0]);
                arr_5 [1] |= ((var_4 ? (((arr_2 [i_1] [i_1] [i_1]) ? var_7 : (arr_2 [i_0] [i_1] [i_1]))) : ((0 ? 1 : 9))));
                arr_6 [i_0] [i_1] = (((((var_4 ? var_8 : var_8))) ? (((arr_0 [i_0] [i_0]) + ((max(var_4, 14))))) : (((min(1, 112))))));
                var_11 *= ((((min((!1), ((1 ? 227 : -117))))) ? ((-(arr_0 [i_0] [4]))) : ((var_9 ? (((arr_0 [i_0] [2]) ? (arr_3 [i_0] [6]) : var_0)) : ((119 ^ (arr_1 [1])))))));
                var_12 = (var_2 - 1);
            }
        }
    }
    var_13 = var_6;
    var_14 = (max((((!var_3) || var_3)), var_4));
    var_15 += -var_7;
    #pragma endscop
}
