/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 101446
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=101446 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/101446
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */short) (_Bool)0);
    var_16 &= ((/* implicit */unsigned char) var_6);
    var_17 = ((/* implicit */long long int) var_7);
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 0; i_0 < 20; i_0 += 1) 
    {
        var_18 = ((/* implicit */unsigned int) (-(((((/* implicit */_Bool) max((288970225U), (((/* implicit */unsigned int) arr_0 [(short)6] [(short)6]))))) ? (var_1) : (((/* implicit */int) arr_0 [i_0] [i_0]))))));
        /* LoopNest 3 */
        for (unsigned int i_1 = 1; i_1 < 19; i_1 += 4) 
        {
            for (long long int i_2 = 0; i_2 < 20; i_2 += 2) 
            {
                for (int i_3 = 0; i_3 < 20; i_3 += 2) 
                {
                    {
                        var_19 *= ((/* implicit */unsigned long long int) max((((/* implicit */int) ((((/* implicit */int) (_Bool)1)) > ((-(((/* implicit */int) arr_6 [i_1] [i_1]))))))), ((~(((/* implicit */int) arr_2 [i_0]))))));
                        var_20 = ((/* implicit */unsigned char) min(((!(arr_0 [i_0] [i_1 + 1]))), (arr_0 [i_0] [i_1 - 1])));
                    }
                } 
            } 
        } 
    }
}
