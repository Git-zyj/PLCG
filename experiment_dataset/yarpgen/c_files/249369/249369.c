/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249369
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249369 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249369
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max(var_20, ((max(var_2, (max(-var_2, var_4)))))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_21 = (max(((((arr_3 [i_0] [i_0] [i_0]) ? (arr_1 [i_0]) : var_12))), ((max(var_18, (arr_0 [i_0]))))));
                arr_6 [i_1] [i_0] [i_0] = ((((arr_4 [i_0 - 1]) | (arr_4 [i_0 - 1]))));
                var_22 = ((!((!(arr_2 [i_0])))));
            }
        }
    }
    var_23 = (min(var_3, var_18));
    #pragma endscop
}
