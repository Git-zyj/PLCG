/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67818
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67818 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67818
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 17;i_0 += 1)
    {
        arr_2 [i_0] = (((arr_0 [i_0]) ? (arr_0 [i_0]) : (arr_0 [i_0])));
        arr_3 [i_0] = ((-(arr_1 [i_0 + 4])));
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        arr_7 [i_1] = ((((4194303 ? -42 : 0)) >= (min(var_10, (arr_4 [i_1] [i_1])))));
        arr_8 [i_1] [i_1] = (min((arr_0 [i_1]), 4290772993));
        arr_9 [i_1] = ((var_2 ? ((var_0 >> (((arr_5 [i_1]) - 10809)))) : (3178001621 & 80)));
        arr_10 [i_1] [i_1] = (((arr_4 [i_1] [i_1]) ? ((((arr_4 [i_1] [i_1]) && 1))) : var_4));
    }
    var_12 = 0;
    #pragma endscop
}
