/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 161269
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=161269 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/161269
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned int i_0 = 4; i_0 < 21; i_0 += 2) 
    {
        for (unsigned char i_1 = 0; i_1 < 23; i_1 += 2) 
        {
            for (long long int i_2 = 0; i_2 < 23; i_2 += 1) 
            {
                {
                    var_12 = ((/* implicit */long long int) var_5);
                    /* LoopNest 2 */
                    for (short i_3 = 0; i_3 < 23; i_3 += 3) 
                    {
                        for (unsigned long long int i_4 = 1; i_4 < 22; i_4 += 4) 
                        {
                            {
                                arr_12 [i_3] = ((/* implicit */_Bool) (-(var_2)));
                                arr_13 [i_1] &= ((/* implicit */short) (-(((((/* implicit */long long int) ((/* implicit */int) arr_4 [i_4 + 1]))) ^ (var_3)))));
                                arr_14 [i_0 - 4] [i_1] [i_0 - 4] [i_3] [i_4] = ((/* implicit */unsigned char) var_2);
                            }
                        } 
                    } 
                    arr_15 [i_0] [i_1] [i_2] = arr_8 [i_0] [i_0];
                }
            } 
        } 
    } 
    var_13 = (-(((/* implicit */int) var_6)));
    var_14 = ((/* implicit */unsigned char) var_7);
}
