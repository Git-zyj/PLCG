/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 69461
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=69461 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/69461
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        var_14 -= (((((arr_0 [8] [i_0]) && (arr_0 [16] [i_0]))) ? (arr_0 [1] [1]) : (((arr_2 [16]) + var_1))));

        for (int i_1 = 0; i_1 < 24;i_1 += 1) /* same iter space */
        {
            var_15 = var_6;
            var_16 += (((((arr_0 [i_1] [i_1]) && var_12)) ? ((((arr_0 [i_1] [i_1]) && var_4))) : (!var_9)));
            var_17 |= (((arr_4 [i_0] [i_0] [i_0]) - ((((var_12 ? (arr_0 [i_1] [i_1]) : var_11))))));
        }
        /* vectorizable */
        for (int i_2 = 0; i_2 < 24;i_2 += 1) /* same iter space */
        {
            var_18 |= ((((((arr_0 [i_2] [i_2]) ? (arr_4 [i_0] [i_2] [23]) : (arr_4 [i_0] [i_2] [2])))) && (!7279929558777289481)));
            var_19 = ((arr_2 [i_0]) >= (arr_2 [i_0]));
        }
    }
    var_20 -= var_9;
    #pragma endscop
}
