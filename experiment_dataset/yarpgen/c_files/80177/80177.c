/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80177
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80177 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80177
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (var_1 & var_11);
    var_21 = var_12;
    var_22 = (1280837466 > -var_11);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] [1] = (((arr_6 [i_0] [2] [2] [i_0]) & ((-(((arr_4 [i_0] [i_0] [14]) / var_6))))));
                    arr_9 [i_0] [1] |= 1572184644;
                    arr_10 [i_2] [i_1] [5] = (((1572184644 ? (arr_1 [i_1] [i_0]) : (arr_1 [i_0] [i_1]))));
                }
            }
        }
    }
    var_23 = (min(var_6, (min(((var_0 ? var_9 : 1)), var_2))));
    #pragma endscop
}
