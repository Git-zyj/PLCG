/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62781
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62781 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62781
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = max(var_5, (var_9 / 9223372036854775807));
    var_12 += (((((((-1963672162 ? var_5 : 2021514396))) && ((max(var_0, var_6))))) && var_4));
    var_13 = var_3;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_14 = (((~49424) ? (~var_0) : (((var_6 < (((min((arr_8 [i_0 + 2] [i_1] [i_1] [i_2]), (arr_1 [i_0] [i_1]))))))))));
                    arr_9 [i_0] [i_0] [i_0] [15] = -2021514393;
                }
            }
        }
    }
    #pragma endscop
}
