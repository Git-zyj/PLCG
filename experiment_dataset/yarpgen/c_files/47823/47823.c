/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47823
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47823 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47823
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_5;

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        arr_2 [i_0] [7] = var_6;

        for (int i_1 = 2; i_1 < 23;i_1 += 1)
        {
            arr_5 [i_1] [i_0] = (arr_4 [i_0] [i_1 + 1]);
            arr_6 [i_0] = var_13;
        }
        /* vectorizable */
        for (int i_2 = 3; i_2 < 20;i_2 += 1)
        {
            arr_9 [i_0] [i_2] = (((arr_7 [i_2 - 3]) > -117));
            arr_10 [i_2] [i_2] [i_2] = ((var_7 ? (arr_7 [i_0]) : var_8));
        }
        arr_11 [i_0] = ((53 ? ((((arr_3 [i_0]) < ((((arr_1 [i_0] [1]) > var_13)))))) : (((((120 < (arr_7 [i_0])))) >> (var_5 - 30735)))));
    }
    var_15 = (((var_4 * var_12) < (min(var_9, var_4))));
    #pragma endscop
}
