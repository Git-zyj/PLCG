/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75035
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75035 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75035
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((var_5 ? (var_0 * var_9) : 1));
    var_16 = -var_10;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_17 = 1;
                var_18 = (max(var_18, ((max(((~(arr_4 [12] [i_1] [i_0]))), (((arr_4 [20] [i_1] [2]) % var_2)))))));

                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    var_19 = (min(var_19, (arr_1 [i_1])));
                    var_20 = (min(var_20, -var_10));
                }
                /* vectorizable */
                for (int i_3 = 0; i_3 < 22;i_3 += 1)
                {
                    arr_12 [i_3] [i_1] [i_1] [i_0] = ((((var_11 | (arr_1 [i_0]))) != (arr_0 [i_0])));
                    var_21 = 0;
                    var_22 = 0;
                }
                var_23 = ((((0 ? (arr_7 [i_1] [i_1] [i_0] [i_0]) : var_4)) != (((min((arr_7 [i_1] [i_1] [i_1] [i_1]), (arr_0 [i_0])))))));
            }
        }
    }
    #pragma endscop
}
