/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 178439
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=178439 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/178439
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(var_14, var_6));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 8;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] [i_0] = ((2147483647 > (max((((arr_0 [0] [i_0]) ? (arr_2 [i_1]) : (arr_3 [i_0] [i_0] [i_0]))), ((min((arr_2 [i_0]), 10053)))))));
                var_15 = (((min((arr_3 [i_0] [i_0] [i_0]), (max(10038, (arr_1 [i_0])))))) / (arr_1 [i_0]));
            }
        }
    }
    var_16 = var_10;
    #pragma endscop
}
