/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177676
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177676 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177676
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            {
                var_17 = (min(var_17, ((((((141 ? (arr_4 [i_1 + 1] [i_0] [i_1 + 1]) : 17989059344439551984))) ? 20 : (((var_6 | 17989059344439551984) ? ((~(arr_2 [i_0] [i_0]))) : (((max(3, 65535)))))))))));
                arr_5 [i_1] [i_0] [i_0] &= (min((min(var_7, (arr_0 [i_1 - 1] [i_1 - 3]))), (((9223372036854775807 ? -10 : 2)))));
            }
        }
    }
    var_18 = var_1;
    #pragma endscop
}
