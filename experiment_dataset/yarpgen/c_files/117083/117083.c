/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117083
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117083 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117083
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 ^= (((-1 + 2147483647) >> (var_5 - 97)));
    var_12 = (((((((min(-76140496, var_1)) > var_0)))) <= (min(((min(var_1, var_5))), -var_8))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 23;i_1 += 1)
        {
            {
                var_13 += ((min((((arr_3 [11]) >> (((arr_1 [i_0] [21]) - 5137127892923055527)))), ((~(arr_2 [i_1 + 1]))))));
                var_14 = (max(var_14, 222));
            }
        }
    }
    #pragma endscop
}
