/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 37779
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=37779 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/37779
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, var_4));
    var_20 = var_13;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1] [i_0] = (((arr_6 [i_0] [i_2] [i_2]) ? ((3301165621 + (((-(arr_6 [i_0] [i_0] [i_0])))))) : ((max(((-(-32767 - 1))), 32767)))));
                    var_21 = (min(var_21, ((((((~(-8729523906430058374 >= 1)))) || ((((((arr_6 [i_2] [i_0] [i_0]) | 127)) * (32767 / 8392086406712397338)))))))));
                    arr_10 [i_0] [i_0] = ((~(((!((((arr_0 [i_1]) ^ -8392086406712397338))))))));
                }
            }
        }
    }
    #pragma endscop
}
