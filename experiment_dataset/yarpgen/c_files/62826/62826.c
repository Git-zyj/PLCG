/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62826
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62826 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62826
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_13;
    var_20 = (!var_13);

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_21 = ((((((1479414975 > 49477) && ((min(var_8, var_0)))))) <= (((!((((((arr_0 [9]) + 2147483647)) << (85704255 - 85704255)))))))));
        var_22 = ((((((arr_1 [i_0]) + 10246))) ? ((~(24679 >= 1))) : (!0)));
        arr_2 [i_0] = (min(0, ((-29415 ? 0 : 13))));
    }
    var_23 = (min((((-(0 * var_15)))), (min((var_4 && 85704255), var_1))));
    #pragma endscop
}
