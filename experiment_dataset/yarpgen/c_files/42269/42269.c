/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 42269
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=42269 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/42269
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_5 [i_0] [9] [i_0] = (min(var_4, (max((arr_2 [i_0 - 1]), (-2147483647 - 1)))));
                var_16 = ((9223372036854775807 ? (!209) : 0));
                var_17 = (min(var_17, (arr_2 [i_0])));
                var_18 *= (max((arr_4 [i_0] [12]), ((11830 >> ((((92 ? var_9 : 0)) - 188314662))))));
            }
        }
    }
    var_19 = ((~((var_9 >> (var_10 - 12222849892418505583)))));
    #pragma endscop
}
