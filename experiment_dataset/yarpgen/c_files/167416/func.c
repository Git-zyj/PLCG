/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 167416
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=167416 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/167416
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned long long int) min((((((/* implicit */unsigned int) ((/* implicit */int) (_Bool)1))) * (var_1))), (var_5)));
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 25; i_0 += 1) 
    {
        for (unsigned char i_1 = 1; i_1 < 24; i_1 += 2) 
        {
            {
                arr_4 [i_1] [i_1] = ((((/* implicit */unsigned int) ((((/* implicit */int) arr_1 [i_1 + 1] [i_1])) ^ (((/* implicit */int) arr_1 [i_1 - 1] [i_1 - 1]))))) >= (((var_1) * (((/* implicit */unsigned int) ((/* implicit */int) (!(((/* implicit */_Bool) var_9)))))))));
                arr_5 [i_1] = arr_0 [i_0] [i_1];
                var_15 = ((/* implicit */unsigned short) 0U);
                var_16 = ((/* implicit */unsigned long long int) var_10);
            }
        } 
    } 
}
