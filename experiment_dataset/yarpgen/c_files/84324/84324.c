/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84324
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84324 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84324
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_10 = arr_3 [13];
                var_11 = (((2473786562 << ((((min((arr_1 [i_1]), 27292))) - 132)))) & 27306);
                var_12 *= (((((8 ? (arr_2 [i_0 - 1]) : 16231809691859689631)) << (-81 - 18446744073709551485))));
            }
        }
    }
    var_13 = ((var_6 ? var_4 : -1512265113790902218));
    var_14 = 12426696699351443554;
    var_15 = var_7;
    var_16 = (min(var_16, var_4));
    #pragma endscop
}
