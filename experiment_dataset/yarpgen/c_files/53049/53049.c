/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 53049
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=53049 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/53049
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min(var_6, 224));
    var_11 = (min(var_5, (((min(var_9, var_6)) * ((min(510917220, 1)))))));
    var_12 = (!var_7);
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_13 = (min(var_13, 2963687375));
                    var_14 *= var_0;
                    arr_7 [i_1 + 2] [i_0] [i_2] &= ((((((max((arr_0 [i_1] [i_1]), 510917220))))) - (max(510917220, -109))));
                    arr_8 [i_0] [i_1 - 1] [i_1 - 1] [i_1] = ((!((max((arr_3 [i_0] [i_1 + 1]), var_2)))));
                }
            }
        }
    }
    #pragma endscop
}
