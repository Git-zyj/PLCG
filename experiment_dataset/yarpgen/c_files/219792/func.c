/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219792
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219792 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219792
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned int) 17736359773795745043ULL);
    /* LoopNest 3 */
    for (short i_0 = 0; i_0 < 15; i_0 += 2) 
    {
        for (_Bool i_1 = 0; i_1 < 0; i_1 += 1) 
        {
            for (unsigned char i_2 = 0; i_2 < 15; i_2 += 2) 
            {
                {
                    /* LoopNest 2 */
                    for (unsigned short i_3 = 1; i_3 < 13; i_3 += 1) 
                    {
                        for (unsigned long long int i_4 = 1; i_4 < 13; i_4 += 2) 
                        {
                            {
                                var_12 ^= ((/* implicit */unsigned short) ((((/* implicit */int) var_5)) / (((int) var_7))));
                                var_13 -= ((/* implicit */long long int) ((((unsigned int) var_2)) >> (((710384299913806574ULL) - (710384299913806570ULL)))));
                            }
                        } 
                    } 
                    arr_13 [i_0] [i_1 + 1] [4] = var_0;
                    var_14 = ((/* implicit */_Bool) max((var_14), (((/* implicit */_Bool) ((710384299913806581ULL) >> (((17736359773795745041ULL) - (17736359773795744994ULL))))))));
                }
            } 
        } 
    } 
}
