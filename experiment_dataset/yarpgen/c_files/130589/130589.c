/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130589
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130589 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130589
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 20;i_1 += 1)
        {
            {
                var_17 = ((-(((-9575 < var_6) << ((((min(119, var_14))) - 90))))));
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        {
                            arr_10 [i_1] [i_2 + 2] [i_1] [i_1] = ((~(~1)));
                            arr_11 [4] [i_1] [i_1] [i_3] [i_3] = ((((!(arr_9 [i_1] [i_1 + 1] [i_2] [i_2 - 1] [i_3]))) ? (32764 > 0) : ((~(arr_9 [i_1] [i_1 + 3] [i_1] [i_2 + 2] [i_3])))));
                        }
                    }
                }
            }
        }
    }
    var_18 = var_5;
    var_19 |= ((~(-1 <= var_11)));
    var_20 *= ((((1085009233 ? (1 != 1) : var_12)) << (((((max(var_7, var_14))) || var_2)))));
    #pragma endscop
}
