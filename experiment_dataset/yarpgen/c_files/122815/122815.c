/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122815
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122815 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122815
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_18 = ((arr_5 [14] [i_1]) == ((min((arr_0 [i_0]), ((var_15 ? var_4 : var_16))))));
                    arr_11 [i_0] [i_1] [i_0] = (((((((((arr_1 [i_0]) ? (arr_9 [1] [i_1] [4]) : var_14))) | (min(var_3, var_7))))) ? (arr_8 [i_0] [i_0] [i_0] [i_0]) : (((var_8 & var_8) & (~var_12)))));
                    arr_12 [i_0] [i_0] [i_0] = ((min(0, (min(261931477, 1)))));
                }
            }
        }
    }
    var_19 = (((~(~var_1))) | var_1);
    #pragma endscop
}
