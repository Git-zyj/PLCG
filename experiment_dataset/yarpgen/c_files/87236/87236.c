/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87236
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87236 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87236
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 6;i_0 += 1)
    {
        arr_3 [i_0] = var_7;
        arr_4 [i_0] [i_0] = (max((max((arr_2 [i_0 + 1] [i_0 - 1]), (arr_2 [i_0 + 3] [i_0 - 1]))), 123));
        var_17 = -126;
    }
    for (int i_1 = 1; i_1 < 20;i_1 += 1)
    {
        arr_8 [i_1 + 3] = (max((((arr_7 [i_1 - 1] [i_1 + 3]) ? var_9 : (max((arr_6 [i_1]), var_5)))), 1));
        var_18 *= ((((((~0) ? 1 : ((130 ? 1 : (arr_5 [i_1] [i_1])))))) ? (min((~132), (1 >= var_0))) : (arr_5 [i_1] [i_1 + 1])));
    }
    var_19 = var_1;
    #pragma endscop
}
