/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116644
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116644 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116644
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 22;i_0 += 1) /* same iter space */
    {
        var_13 = (3190657023 < 1102094907543088129);
        arr_2 [i_0 + 2] = ((16777152 ? 4294967288 : 46));
    }
    for (int i_1 = 1; i_1 < 22;i_1 += 1) /* same iter space */
    {
        arr_5 [i_1] = ((-530767324 ? ((((arr_1 [i_1 - 1]) || (arr_1 [9])))) : 0));
        var_14 &= (+((0 ? (((arr_4 [4] [4]) ? 3079 : 1126670322)) : (~var_4))));
    }
    var_15 = ((~(max((max(var_12, var_12)), (~1)))));
    #pragma endscop
}
