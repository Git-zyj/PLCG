/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 28728
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=28728 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/28728
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (var_15 | var_15);

    /* vectorizable */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = var_16;
        var_21 = (min(var_21, ((((-(arr_1 [i_0])))))));
    }
    for (int i_1 = 0; i_1 < 24;i_1 += 1) /* same iter space */
    {
        var_22 = var_9;

        for (int i_2 = 1; i_2 < 22;i_2 += 1)
        {
            var_23 = -12762032464290818095;
            /* LoopNest 2 */
            for (int i_3 = 2; i_3 < 20;i_3 += 1)
            {
                for (int i_4 = 1; i_4 < 1;i_4 += 1)
                {
                    {
                        arr_15 [i_1] [i_2] [i_2 + 1] [i_1] [i_4] = (var_0 ? var_0 : (arr_14 [i_1] [i_2 + 2] [i_3] [i_4 - 1]));
                        var_24 = var_4;
                        var_25 = (arr_9 [i_1] [i_1]);
                    }
                }
            }
        }
    }
    for (int i_5 = 0; i_5 < 24;i_5 += 1) /* same iter space */
    {
        arr_19 [i_5] [i_5] = (((min(var_14, (arr_10 [i_5] [i_5] [i_5])))) && ((((arr_18 [i_5]) % 6))));
        var_26 |= ((!((((var_18 && (arr_9 [i_5] [i_5]))) || (((21 ? 127 : (arr_17 [i_5]))))))));
        var_27 = var_9;
    }
    #pragma endscop
}
