/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27134
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27134 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27134
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((((!((max(var_8, var_9))))))) > var_9));

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_13 = (min(var_13, ((min(((!(arr_2 [i_0]))), (((arr_2 [i_0]) >= (arr_0 [8]))))))));
        var_14 = (min(var_14, (arr_1 [i_0])));
        var_15 = (min(var_15, (max((arr_1 [1]), (((!((max(var_1, (arr_1 [i_0])))))))))));
        var_16 = (max(var_16, (((var_9 ? var_2 : 16869822259608744484)))));
    }
    var_17 = var_6;
    #pragma endscop
}
