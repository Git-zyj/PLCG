/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236525
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236525 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236525
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (!-var_8);
    var_21 = (((min(var_4, 2147483641))) ? 6 : ((var_0 ? var_13 : var_14)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_22 = (max(var_22, ((((arr_2 [i_0]) && ((min(var_7, var_10))))))));
                    arr_8 [i_0] [i_1] [i_1] [i_0] = (min(((var_13 ^ (arr_6 [i_0] [i_1] [i_2] [i_1]))), ((~(arr_6 [i_2] [i_2] [i_1] [i_0])))));
                }
            }
        }
    }
    #pragma endscop
}
