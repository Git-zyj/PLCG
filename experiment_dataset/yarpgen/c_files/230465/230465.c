/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230465
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230465 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230465
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max(var_14, (((!(var_4 && var_10))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] [i_0] = (arr_1 [i_0]);
        var_21 *= (((~4503599627370495) ? ((((arr_1 [i_0]) && (arr_0 [8])))) : (arr_0 [0])));
        var_22 = (((arr_0 [i_0]) ? var_3 : (arr_0 [i_0])));
        var_23 = (max(var_23, (!193)));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 1;i_1 += 1) /* same iter space */
    {
        var_24 = ((-(arr_3 [i_1] [i_1])));
        arr_6 [8] [i_1] = var_14;
    }
    var_25 = (max(var_0, (min(41128, 5380648443518605168))));
    var_26 = (((-(~1))));
    var_27 = ((((min(var_6, (min(-6036107388806724787, var_15))))) || var_17));
    #pragma endscop
}
