/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 153222
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=153222 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/153222
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(var_7, ((var_2 + (min(var_5, var_7))))));
    var_15 = (max((min((min(var_3, var_10)), ((max(var_3, var_0))))), ((min((var_7 >= var_10), (max(var_12, var_5)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 9;i_1 += 1)
        {
            {
                arr_6 [i_0] [0] [i_0] = (min((max(((var_13 | (arr_3 [i_0]))), (arr_2 [i_0]))), (arr_3 [i_0])));
                var_16 = (max((arr_1 [i_0] [i_1 + 4]), var_9));
            }
        }
    }
    #pragma endscop
}
