/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23682
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23682 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23682
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned int) var_4);
    var_16 = ((/* implicit */int) var_13);
    var_17 = ((/* implicit */short) max((((/* implicit */long long int) var_14)), (var_7)));
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 0; i_0 < 14; i_0 += 3) 
    {
        arr_2 [11U] [(_Bool)1] = ((/* implicit */unsigned int) ((((/* implicit */unsigned int) ((/* implicit */int) var_9))) >= (arr_1 [i_0])));
        var_18 = ((/* implicit */short) ((unsigned int) ((((/* implicit */_Bool) arr_1 [i_0])) ? (((/* implicit */long long int) var_3)) : (arr_0 [(_Bool)1]))));
    }
    var_19 *= ((/* implicit */_Bool) var_3);
}
