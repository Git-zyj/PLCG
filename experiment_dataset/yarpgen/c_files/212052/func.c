/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212052
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212052 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212052
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) (~(((((/* implicit */_Bool) (unsigned char)0)) ? (((/* implicit */unsigned long long int) ((/* implicit */int) var_3))) : (10005536342887507843ULL)))))) ? (var_5) : (((/* implicit */int) var_6))));
    /* LoopSeq 1 */
    for (long long int i_0 = 0; i_0 < 10; i_0 += 1) 
    {
        var_12 &= ((/* implicit */unsigned short) (!(((/* implicit */_Bool) var_9))));
        var_13 = 232235492;
        var_14 = ((/* implicit */unsigned char) min((var_14), (var_2)));
        var_15 = ((/* implicit */unsigned short) arr_2 [7ULL]);
    }
    var_16 ^= ((/* implicit */unsigned short) var_7);
}
