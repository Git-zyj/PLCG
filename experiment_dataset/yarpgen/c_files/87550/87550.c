/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87550
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87550 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87550
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_1] = -24;
                /* LoopNest 3 */
                for (int i_2 = 1; i_2 < 9;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 9;i_4 += 1)
                        {
                            {
                                arr_14 [i_0] [i_0] [i_1] [i_2 + 1] [i_3] [i_1] = (min(((((18 | -18) | ((-31 & (arr_9 [3] [3] [3] [i_4 - 1])))))), var_10));
                                var_14 = ((-32 ? ((max(-31, 18))) : -31));
                                arr_15 [i_0] [i_0] [i_2 + 1] [i_3] [i_1] = ((~(((var_1 ? 35 : -13)))));
                                var_15 = (max(-18, (min(((min(23, 23))), (arr_10 [i_0] [i_1] [i_2 + 1] [i_1] [i_4])))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_16 = var_13;
    #pragma endscop
}
