/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 13187
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=13187 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/13187
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_13 = ((!(!288230376151711743)));
        var_14 = (max(var_14, ((min((arr_0 [10]), (arr_0 [6]))))));
    }
    for (int i_1 = 0; i_1 < 20;i_1 += 1)
    {
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 20;i_3 += 1)
            {
                {
                    var_15 = (!255);
                    arr_9 [i_3] [1] [i_3] = ((min((max((arr_3 [i_1]), (arr_5 [1]))), (var_10 + var_12))) << (((~255) + 305)));
                    arr_10 [2] [i_2] [12] [2] &= ((((((0 ? (arr_2 [i_1] [i_1]) : 15229726771046218399)))) ? var_6 : (((max(-71, 255))))));
                    arr_11 [i_3] = arr_4 [i_1] [i_1] [i_1];
                }
            }
        }
        arr_12 [2] = -12824;
        var_16 = (max(3599047846, 3));
    }
    var_17 = var_11;
    var_18 = (-12 - var_11);
    var_19 = ((var_4 ? var_3 : var_0));
    #pragma endscop
}
