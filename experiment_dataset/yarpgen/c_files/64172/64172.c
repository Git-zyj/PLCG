/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64172
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64172 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64172
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_8 [i_0] [i_0] [i_0] = var_6;
                var_13 -= (max((((var_1 ? (-2147483647 - 1) : var_5))), (((var_10 ? 9223372036854775807 : var_0)))));
                /* LoopNest 2 */
                for (int i_2 = 4; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 13;i_3 += 1)
                    {
                        {
                            var_14 *= ((-(((((((arr_7 [i_0] [i_1]) - 9223372036854775807))) && ((((arr_10 [i_0] [i_0] [i_2 - 4]) ? (arr_4 [i_0]) : (arr_0 [i_0])))))))));
                            arr_16 [1] [i_1] [i_1] [i_1] [1] = var_1;
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
