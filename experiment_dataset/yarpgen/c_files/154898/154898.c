/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 154898
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=154898 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/154898
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_7;
    var_15 = (max(var_15, var_3));
    var_16 = var_11;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [5] [i_0] = (max((((((arr_0 [i_1] [i_1]) ? (arr_4 [i_1] [6]) : var_6)) + (((((arr_0 [i_0] [i_0 - 2]) != (arr_0 [i_0 + 1] [1]))))))), (((!(arr_4 [i_1] [i_0 - 2]))))));
                /* LoopNest 2 */
                for (int i_2 = 3; i_2 < 8;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        {
                            var_17 = var_13;
                            var_18 = (arr_10 [4] [i_0 + 1] [i_2] [i_0 - 2]);
                            var_19 = (min(var_19, var_9));
                        }
                    }
                }
                arr_11 [i_0 - 2] [i_1] = (max((max((arr_6 [1] [i_0] [2]), (arr_2 [i_0]))), ((+(((arr_1 [i_0]) ? (arr_10 [i_1] [i_0] [i_0] [i_0 - 1]) : -8054690944667972646))))));
            }
        }
    }
    #pragma endscop
}
