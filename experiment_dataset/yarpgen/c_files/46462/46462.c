/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46462
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46462 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46462
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 += (min(var_5, (max(var_4, var_9))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    var_15 -= (((((arr_7 [i_0 + 1]) ? (arr_3 [i_0 + 1] [i_0 + 1] [i_0 + 2]) : var_13))) ? (~9001) : var_3);
                    var_16 = min(((((arr_6 [10]) ? (arr_8 [i_0] [i_0 + 2] [i_0]) : (arr_6 [i_0])))), (-5686775256166496274 | -10));
                }
            }
        }
    }
    var_17 |= -15318592544744672057;
    #pragma endscop
}
