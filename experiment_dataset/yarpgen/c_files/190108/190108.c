/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190108
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190108 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190108
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = min((min(var_10, (min(var_15, var_5)))), var_4);

    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        var_20 = (min((arr_1 [i_0] [i_0]), (arr_0 [i_0] [i_0])));
        arr_3 [i_0] = (arr_0 [i_0 - 1] [i_0]);
        var_21 = (((((((arr_0 [i_0] [i_0]) & (arr_1 [i_0] [i_0]))) - (((min((arr_2 [6]), (arr_1 [i_0] [i_0]))))))) % (((arr_0 [i_0] [i_0]) / (((((arr_2 [i_0]) >= (arr_1 [1] [i_0])))))))));
        arr_4 [i_0] = ((((((arr_2 [i_0]) != (arr_2 [i_0])))) < ((((arr_0 [i_0] [i_0]) > (arr_0 [i_0] [i_0]))))));
    }
    for (int i_1 = 0; i_1 < 12;i_1 += 1)
    {
        arr_8 [i_1] = (max((arr_2 [i_1]), (arr_7 [i_1])));
        var_22 ^= (arr_1 [8] [i_1]);
        arr_9 [4] = (((((min((arr_7 [i_1]), (arr_5 [i_1] [i_1]))))) <= (arr_0 [i_1] [i_1])));
    }
    var_23 |= var_17;
    var_24 = var_3;
    #pragma endscop
}
