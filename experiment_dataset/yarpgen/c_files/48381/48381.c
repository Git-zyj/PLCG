/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48381
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48381 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48381
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 22;i_1 += 1)
        {
            {

                /* vectorizable */
                for (int i_2 = 3; i_2 < 22;i_2 += 1)
                {
                    /* LoopNest 2 */
                    for (int i_3 = 4; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_12 += ((((((arr_4 [i_0] [i_1] [i_0] [i_3 + 1]) & 57474))) ? (arr_2 [i_0] [i_4]) : ((-(arr_4 [i_0] [i_0] [i_0] [i_0])))));
                                var_13 *= 4294967279;
                            }
                        }
                    }
                    var_14 -= (!40450);
                }
                var_15 = var_6;
            }
        }
    }
    var_16 = var_4;
    var_17 = (min((~1801998662118097186), var_2));
    var_18 = (max(var_1, ((!(((-8401293373510980307 ? var_9 : 25627)))))));
    #pragma endscop
}
