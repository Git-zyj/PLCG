/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14926
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14926 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14926
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_1] = ((((63 || (-127 - 1))) ? ((((var_8 ? var_3 : 463638550)) * 463638550)) : ((((~var_9) ? var_9 : (((arr_0 [i_0]) & var_10)))))));
                var_13 = (min(var_13, (arr_4 [i_1 + 1] [i_1 + 1])));
            }
        }
    }
    var_14 = (((((3831328727 * 118) * var_12)) * ((((var_9 * 1) / var_0)))));
    var_15 *= var_6;
    #pragma endscop
}
