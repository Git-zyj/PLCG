/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169205
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169205 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169205
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    arr_7 [i_2] = (((((!(arr_3 [i_0])))) & (arr_3 [i_0])));
                    var_11 = 221;
                }
            }
        }
        arr_8 [i_0] = ((!((max((min(var_1, 822895220)), 118)))));
    }
    var_12 = (min(var_0, var_10));
    var_13 = (((var_9 + var_4) ? (max(var_1, var_9)) : ((var_10 * (min(var_7, var_9))))));
    var_14 = ((!(((~(min(var_5, 7923564686510133742)))))));
    #pragma endscop
}
