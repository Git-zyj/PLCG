/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242002
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242002 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242002
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max(1, var_2));
    var_16 = var_4;

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        var_17 = ((-(arr_2 [i_0] [i_0])));
        arr_3 [i_0] [2] = ((max((arr_2 [0] [i_0]), (max((arr_2 [i_0] [i_0]), (arr_0 [i_0]))))) > (arr_2 [i_0] [i_0]));
        arr_4 [i_0] = (((((arr_0 [i_0]) % (arr_1 [i_0])))) & (max(0, 117)));
        var_18 = (((min((arr_2 [i_0] [i_0]), (arr_1 [i_0]))) ^ (((((arr_2 [i_0] [i_0]) + 9223372036854775807)) << 0))));
    }
    #pragma endscop
}
