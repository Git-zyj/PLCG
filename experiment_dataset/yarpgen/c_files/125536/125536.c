/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125536
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125536 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125536
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_14 ^= ((~((max((var_10 != 226), (var_5 || 236))))));
                var_15 = (((((arr_0 [i_0 + 1]) / 3)) * (max((~var_9), (var_11 && var_4)))));
                var_16 |= ((~(((((var_10 ? (arr_0 [i_1]) : 1))) ? (244 * 0) : (((arr_0 [i_1]) ^ var_3))))));
            }
        }
    }
    var_17 = (!var_9);
    var_18 = ((var_9 < (max((!var_1), (var_13 / var_13)))));
    #pragma endscop
}
