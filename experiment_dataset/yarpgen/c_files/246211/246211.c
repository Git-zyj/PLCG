/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246211
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246211 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246211
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (((var_8 & var_19) ^ 13121470295524754056));
    var_21 = var_2;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    arr_10 [i_0] [i_0] [i_1] [0] &= (arr_6 [i_0] [i_2] [i_0]);
                    arr_11 [i_0] [i_1] [i_0] = (max((((arr_0 [i_1 + 1]) ? var_6 : (arr_0 [i_1 - 2]))), ((((arr_2 [i_1 + 1]) ^ (arr_2 [i_1 - 2]))))));
                    var_22 = (min(var_22, (((((var_5 + (arr_4 [i_0]))))))));
                }
            }
        }
    }
    #pragma endscop
}
