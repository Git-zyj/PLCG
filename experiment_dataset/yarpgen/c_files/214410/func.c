/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214410
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214410 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214410
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */short) min((var_17), (((/* implicit */short) var_5))));
    var_18 = ((/* implicit */unsigned int) var_13);
    /* LoopNest 2 */
    for (signed char i_0 = 4; i_0 < 10; i_0 += 3) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            {
                var_19 = ((/* implicit */unsigned long long int) ((((/* implicit */unsigned long long int) max((max((arr_4 [i_1] [i_0]), (arr_4 [(short)9] [7]))), (((/* implicit */int) var_13))))) < (((unsigned long long int) (signed char)-57))));
                arr_5 [i_0] [i_0] = ((/* implicit */short) (!(((/* implicit */_Bool) max((((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) 0ULL))))), (var_11))))));
                var_20 = ((/* implicit */_Bool) 10507548192555978121ULL);
                arr_6 [i_0] [i_1] = ((/* implicit */unsigned long long int) var_9);
            }
        } 
    } 
    var_21 = ((/* implicit */unsigned long long int) var_13);
    var_22 = ((/* implicit */signed char) var_15);
}
