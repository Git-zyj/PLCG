/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 19147
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=19147 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/19147
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        var_15 *= var_10;
        var_16 |= (((((var_1 ? var_11 : var_4))) || (((0 ? (16259 && var_13) : (-23643 && -7443409930775618729))))));
        var_17 = (min(var_17, ((((~var_5) == ((max((max(var_11, var_8)), ((min(var_4, 0)))))))))));
        arr_4 [i_0] [i_0] = (-4123880334217800285 * 1);
    }
    var_18 |= var_2;
    #pragma endscop
}
