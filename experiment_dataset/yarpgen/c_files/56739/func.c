/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 56739
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=56739 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/56739
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (long long int i_0 = 3; i_0 < 13; i_0 += 3) 
    {
        var_20 &= ((/* implicit */long long int) arr_3 [i_0]);
        var_21 = ((/* implicit */unsigned char) (_Bool)0);
    }
    var_22 = ((/* implicit */long long int) ((signed char) (((!(((/* implicit */_Bool) (unsigned char)64)))) ? (((/* implicit */long long int) ((((/* implicit */_Bool) 3118499680U)) ? (30720U) : (var_19)))) : (max((((/* implicit */long long int) -536870912)), (-5315385889359495231LL))))));
    var_23 = ((/* implicit */unsigned char) (!(((/* implicit */_Bool) 5315385889359495230LL))));
}
