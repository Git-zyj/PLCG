/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 113801
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=113801 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/113801
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_16 ^= (((((((max((arr_3 [7] [i_1] [i_0]), (arr_3 [i_2] [i_1] [i_0])))) ? (arr_1 [i_0]) : (((arr_5 [0] [i_1]) - (arr_4 [i_0] [i_0] [i_0])))))) ? (arr_2 [i_2] [i_1] [i_0]) : (((((((arr_4 [i_2] [i_1] [i_0]) ? (arr_3 [i_0] [i_1] [i_0]) : (arr_5 [i_0] [10]))) >= ((((!(arr_0 [i_0])))))))))));
                    arr_7 [i_2] [i_0] [i_0] [9] = (arr_4 [i_0] [i_0] [i_2]);
                    arr_8 [i_0] [i_1] [i_0] = -var_9;
                }
            }
        }
    }
    var_17 = (max(var_17, var_13));
    var_18 = (max(var_18, var_0));
    #pragma endscop
}
