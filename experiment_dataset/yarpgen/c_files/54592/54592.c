/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54592
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54592 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54592
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 17;i_4 += 1)
                        {
                            {
                                var_17 = (((-((min(var_1, var_10))))));
                                var_18 |= var_8;
                                var_19 = (arr_4 [i_1] [i_1]);
                                arr_12 [i_0] [i_1] [i_0] [i_0] [i_3] [i_0] [i_4] = (!(((var_5 ? (arr_6 [i_0] [i_0] [i_3] [i_3]) : (arr_10 [i_0])))));
                            }
                        }
                    }
                    var_20 = (max((((!(((-15067 ? var_3 : 1)))))), (min(var_13, (15067 ^ 32766)))));
                }
            }
        }
    }
    var_21 = (min(((11571075643482321792 ? -87 : 6722612470206122580)), ((((!var_13) ? -15064 : (var_6 || var_14))))));
    #pragma endscop
}
