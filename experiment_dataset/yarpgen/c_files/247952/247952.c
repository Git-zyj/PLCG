/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247952
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247952 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247952
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_14 = (min(var_14, (((((min(-23823, 23822))) || ((min((arr_7 [i_0] [i_1 - 2] [i_0]), ((max(1, 23823)))))))))));
                    arr_8 [i_0] [i_1] [i_0] [i_0] = ((((max((arr_6 [i_2] [i_1 - 2] [i_2] [i_0]), ((255 & (arr_2 [i_0] [i_0])))))) < (((10298257491205672841 * (arr_0 [i_0]))))));
                    var_15 = (arr_6 [i_0] [i_0] [i_1 + 3] [i_2]);
                }
            }
        }
    }
    var_16 = (max(var_16, var_0));
    var_17 = (!23822);
    #pragma endscop
}
