/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67225
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67225 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67225
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            {
                var_12 = (arr_3 [i_0] [i_0]);
                arr_4 [i_0] [i_1] = var_8;
                var_13 = (max(var_1, ((min(var_3, (arr_1 [i_0] [i_0]))))));
                var_14 = ((((((arr_3 [i_1 + 1] [i_1 + 1]) ? var_6 : (arr_3 [i_1 + 1] [i_1 + 1])))) ? ((min((arr_3 [i_1 + 1] [i_1 + 1]), (arr_3 [i_1 + 1] [i_1 + 1])))) : (arr_3 [i_1 + 1] [i_1 + 1])));
            }
        }
    }
    var_15 = (min(((min(var_8, var_10))), (~var_2)));
    var_16 = var_6;
    #pragma endscop
}
