/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 56056
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=56056 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/56056
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned int) var_6);
    /* LoopNest 2 */
    for (unsigned int i_0 = 1; i_0 < 20; i_0 += 2) 
    {
        for (long long int i_1 = 0; i_1 < 21; i_1 += 1) 
        {
            {
                var_16 = ((/* implicit */unsigned char) arr_1 [i_0]);
                /* LoopNest 2 */
                for (short i_2 = 0; i_2 < 21; i_2 += 1) 
                {
                    for (unsigned short i_3 = 1; i_3 < 20; i_3 += 2) 
                    {
                        {
                            var_17 = ((/* implicit */unsigned char) (!(((/* implicit */_Bool) arr_8 [i_0 + 1]))));
                            var_18 = ((/* implicit */long long int) max((((/* implicit */unsigned long long int) (~(((/* implicit */int) arr_0 [i_0 - 1]))))), (((arr_3 [9] [i_1] [18LL]) ^ (((/* implicit */unsigned long long int) ((/* implicit */int) var_10)))))));
                            arr_10 [i_0] [i_3] [i_3] = ((/* implicit */unsigned long long int) 123871917U);
                        }
                    } 
                } 
            }
        } 
    } 
}
