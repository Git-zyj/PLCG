/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89400
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89400 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89400
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((var_14 <= (min(((min(27644, var_14))), ((var_4 ? var_4 : var_12))))));
    var_17 = 7641;
    var_18 = ((((max(var_4, var_13)))));
    var_19 ^= var_7;

    /* vectorizable */
    for (int i_0 = 3; i_0 < 16;i_0 += 1) /* same iter space */
    {
        var_20 = (((arr_1 [i_0] [i_0]) & (arr_0 [i_0])));
        var_21 = (i_0 % 2 == zero) ? (((+(((arr_1 [i_0] [i_0]) >> (((arr_0 [i_0]) - 56570))))))) : (((+(((arr_1 [i_0] [i_0]) >> (((((arr_0 [i_0]) - 56570)) + 40915)))))));
        arr_2 [i_0] = ((!(arr_1 [i_0] [i_0])));
    }
    for (int i_1 = 3; i_1 < 16;i_1 += 1) /* same iter space */
    {
        arr_7 [i_1] = (((!103) ? var_13 : ((min(var_11, (arr_5 [i_1]))))));
        var_22 = (max(var_22, (((((arr_6 [i_1 - 1]) ? var_11 : (arr_0 [1])))))));
    }
    #pragma endscop
}
