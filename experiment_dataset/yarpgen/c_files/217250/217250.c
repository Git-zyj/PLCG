/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217250
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217250 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217250
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 8;i_0 += 1)
    {
        arr_3 [i_0] = (((arr_0 [i_0]) != (min((arr_2 [i_0 - 2]), (((arr_2 [i_0]) ? (arr_0 [i_0]) : 536805376))))));
        var_16 = (arr_2 [i_0 + 2]);
        arr_4 [i_0] = ((((((min((arr_2 [i_0]), (arr_1 [i_0]))) == (arr_1 [i_0])))) == ((((((min(var_12, (arr_0 [i_0]))))) == ((var_3 ? (-32767 - 1) : 8444249301319680)))))));
        var_17 = (arr_2 [i_0]);
    }
    var_18 += var_5;
    #pragma endscop
}
