/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 2182
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=2182 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/2182
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) (+(((/* implicit */int) var_4)))))));
    var_20 = ((/* implicit */unsigned long long int) min((var_20), (((/* implicit */unsigned long long int) var_18))));
    /* LoopSeq 1 */
    for (unsigned short i_0 = 0; i_0 < 13; i_0 += 1) 
    {
        arr_2 [i_0] = ((/* implicit */unsigned long long int) (~((~(var_11)))));
        /* LoopSeq 1 */
        /* vectorizable */
        for (unsigned long long int i_1 = 2; i_1 < 12; i_1 += 4) 
        {
            /* LoopSeq 1 */
            for (int i_2 = 3; i_2 < 11; i_2 += 3) 
            {
                var_21 = ((/* implicit */unsigned char) (!(((/* implicit */_Bool) (unsigned char)152))));
                var_22 = ((/* implicit */_Bool) (~(2490345817U)));
            }
            /* LoopSeq 1 */
            for (unsigned short i_3 = 0; i_3 < 13; i_3 += 1) 
            {
                var_23 *= (!(((/* implicit */_Bool) arr_8 [i_1] [i_1 + 1])));
                var_24 = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) arr_7 [i_0] [i_0] [i_0] [i_3]))));
            }
            var_25 = ((/* implicit */unsigned int) max((var_25), (((/* implicit */unsigned int) (!((_Bool)1))))));
            var_26 = ((/* implicit */unsigned char) (~(arr_8 [i_1 - 2] [i_1 - 2])));
        }
        var_27 = ((/* implicit */int) max((var_27), ((-(((/* implicit */int) (!(arr_4 [i_0] [i_0]))))))));
        var_28 = ((/* implicit */unsigned int) (~((~(var_15)))));
        var_29 = ((/* implicit */unsigned int) (~(((/* implicit */int) (!(((/* implicit */_Bool) var_5)))))));
    }
}
