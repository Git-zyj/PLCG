/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75825
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75825 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75825
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */int) ((((unsigned long long int) var_10)) >= (var_18)));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_0 = 2; i_0 < 15; i_0 += 4) 
    {
        arr_4 [i_0] = ((/* implicit */unsigned short) ((var_6) ? (1472040062) : (((/* implicit */int) (unsigned short)28))));
        var_21 = ((/* implicit */unsigned int) ((arr_2 [i_0]) + (((/* implicit */long long int) -1472040063))));
    }
}
