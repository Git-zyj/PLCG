/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54292
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54292 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54292
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 13;i_3 += 1)
                {
                    {
                        arr_12 [i_1] [i_1] = (arr_1 [i_1 + 1] [i_3 - 1]);
                        arr_13 [i_0] [i_1] [1] [i_0] [i_0] [10] = ((-19920 ^ ((((arr_3 [i_0] [i_2]) || var_1)))));
                    }
                }
            }
        }
        arr_14 [i_0] = var_11;
    }
    for (int i_4 = 1; i_4 < 10;i_4 += 1)
    {
        var_14 = (!-19920);
        var_15 = ((~((min((arr_11 [i_4 + 2]), (arr_11 [i_4 + 2]))))));
    }
    var_16 = var_7;
    var_17 = ((-(((-19920 != -27097) ? var_8 : 19917))));
    var_18 = 7829270899732571742;
    #pragma endscop
}
