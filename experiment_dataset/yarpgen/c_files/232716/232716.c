/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 232716
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=232716 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/232716
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (var_8 < 30);
    var_21 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_22 = ((!(arr_2 [i_1])));
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 6;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 10;i_4 += 1)
                        {
                            {
                                var_23 = (((var_17 >> var_11) % (arr_1 [i_0] [i_0])));
                                arr_10 [i_4] [i_1] [i_2] [i_3 + 3] [i_1 - 2] &= (((((((((arr_4 [i_2] [i_3 + 2] [i_4]) / 240)) > 41547)))) == ((((arr_1 [i_0] [i_0]) != (arr_1 [i_4] [i_4]))))));
                            }
                        }
                    }
                    var_24 = (max((arr_0 [i_0]), (max(23985, 4953435027865765378))));
                    arr_11 [i_0] [i_1] [i_2] = ((((var_17 > 54) >> (((arr_2 [i_0]) - 7249)))));
                    arr_12 [i_0] [i_1] [i_2] = 201;
                }
            }
        }
    }
    #pragma endscop
}
