/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187746
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187746 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187746
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */long long int) var_16);
    var_19 = ((/* implicit */long long int) var_6);
    var_20 = ((/* implicit */int) var_17);
    var_21 = ((/* implicit */unsigned short) var_12);
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 18; i_0 += 1) 
    {
        for (long long int i_1 = 0; i_1 < 18; i_1 += 4) 
        {
            {
                var_22 = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) (+(((((/* implicit */_Bool) var_16)) ? (-7038244872673894735LL) : (var_15)))))) ? (var_7) : (((/* implicit */unsigned int) ((/* implicit */int) var_14)))));
                var_23 = ((/* implicit */unsigned long long int) max((var_23), (((/* implicit */unsigned long long int) ((((/* implicit */_Bool) min((arr_0 [i_1]), (arr_0 [i_1])))) ? (((/* implicit */long long int) (+((+(var_4)))))) : (min((var_15), (var_13))))))));
            }
        } 
    } 
}
