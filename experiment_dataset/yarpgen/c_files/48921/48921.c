/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48921
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48921 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48921
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_14, ((min((max(6065, 71)), 6065)))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_20 = (min(var_20, 1663064394));
                var_21 -= (min(((((max(572734319, (arr_1 [i_1])))) ? (min((arr_5 [i_0] [i_1] [9]), (arr_2 [i_0 - 2]))) : 1725174305)), ((((var_14 != var_0) ? ((-(arr_5 [i_0] [1] [i_1]))) : (!var_0))))));
            }
        }
    }
    var_22 = ((200 ? 65535 : 74));
    #pragma endscop
}
