/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169622
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169622 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169622
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        var_20 = var_10;
        arr_3 [1] [i_0 - 1] = (arr_1 [16] [16]);
        arr_4 [i_0] [i_0] = arr_2 [i_0 - 1] [i_0];
        arr_5 [i_0] = arr_1 [12] [i_0];
        arr_6 [i_0] = arr_2 [i_0] [i_0];
    }
    var_21 = var_5;
    var_22 = (max((min(var_0, (min(var_5, var_4)))), ((min((min(var_6, var_19)), var_6)))));

    for (int i_1 = 2; i_1 < 21;i_1 += 1)
    {
        /* LoopNest 2 */
        for (int i_2 = 2; i_2 < 22;i_2 += 1)
        {
            for (int i_3 = 4; i_3 < 22;i_3 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_4 = 0; i_4 < 23;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 23;i_5 += 1)
                        {
                            {
                                var_23 *= var_15;
                                arr_21 [i_1] [i_1] [i_2] [i_3] [i_2] [i_2] = var_8;
                                arr_22 [i_3 - 2] [i_2] [i_2] [i_3 - 2] [i_3 - 2] [i_2] = var_17;
                                var_24 = (arr_13 [i_2] [i_4]);
                            }
                        }
                    }
                    var_25 = var_16;
                }
            }
        }
        arr_23 [i_1 + 2] [i_1] = (max((min((max((arr_10 [i_1] [i_1]), (arr_19 [16] [i_1] [i_1] [i_1] [i_1] [i_1]))), ((max((arr_7 [i_1 + 1] [19]), (arr_14 [i_1 + 1] [i_1 - 1] [i_1])))))), var_15));
        var_26 ^= var_16;
    }
    /* vectorizable */
    for (int i_6 = 0; i_6 < 1;i_6 += 1)
    {
        arr_27 [i_6] = (arr_12 [i_6] [i_6] [i_6]);
        var_27 = (arr_10 [i_6] [i_6]);
    }
    #pragma endscop
}
