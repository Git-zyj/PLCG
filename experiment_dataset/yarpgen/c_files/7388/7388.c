/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7388
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((((((var_8 ? 252 : var_8))) ^ (~var_9)))) ? (((((1008 ? var_10 : var_9))) ? (~1) : var_2)) : (((((var_10 ? var_6 : var_6))) ? 991 : (min(var_2, 252))))));
    var_13 = var_8;

    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        var_14 = (arr_1 [i_0]);
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 22;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 20;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_15 = (~-26483);
                                arr_13 [i_0] [i_0] [i_2] [i_3] [i_2] = arr_9 [i_0] [i_0] [i_0] [i_0];
                            }
                        }
                    }
                    var_16 ^= (arr_4 [i_0]);
                    arr_14 [i_0] [i_1] [i_2] = (arr_1 [i_1]);
                    arr_15 [i_0] [i_2] [i_2] = (arr_7 [i_0] [i_1] [i_0]);
                }
            }
        }
    }
    var_17 ^= ((!((max(var_7, 46432)))));
    #pragma endscop
}
