/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41917
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41917 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41917
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned char) var_5);
    /* LoopNest 2 */
    for (short i_0 = 3; i_0 < 9; i_0 += 2) 
    {
        for (unsigned short i_1 = 0; i_1 < 11; i_1 += 4) 
        {
            {
                arr_5 [i_0] [i_0] [i_0] = ((/* implicit */signed char) ((((/* implicit */_Bool) min((var_7), (((/* implicit */long long int) arr_0 [i_0 - 2]))))) ? (arr_1 [(_Bool)1] [i_0 + 1]) : (((((/* implicit */int) (unsigned char)128)) | (((/* implicit */int) (unsigned short)31754))))));
                arr_6 [i_0] = ((/* implicit */short) ((((/* implicit */int) (unsigned char)117)) >> ((((-(((/* implicit */int) (unsigned char)120)))) + (139)))));
            }
        } 
    } 
    var_15 = ((/* implicit */_Bool) var_4);
}
