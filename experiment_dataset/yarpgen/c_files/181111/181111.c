/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181111
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181111 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181111
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 += (((-4950581666653228697 | (min(4950581666653228716, 4950581666653228708)))));
    var_14 = ((var_8 >> (var_5 + 31)));

    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (arr_1 [i_0]);
        var_15 = ((((((-(arr_1 [22])) >= ((max(4822, -28851))))))));
        var_16 = ((~(arr_0 [i_0] [i_0 + 1])));
        arr_3 [i_0] = (var_12 != ((((arr_1 [i_0 + 1]) ? -1 : (0 & 3)))));
    }
    #pragma endscop
}
