/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 243232
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=243232 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/243232
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 -= ((/* implicit */_Bool) ((long long int) ((unsigned char) (_Bool)1)));
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 3; i_0 < 10; i_0 += 2) 
    {
        for (short i_1 = 3; i_1 < 11; i_1 += 2) 
        {
            for (long long int i_2 = 0; i_2 < 12; i_2 += 2) 
            {
                {
                    var_21 = ((/* implicit */unsigned int) (unsigned short)28046);
                    var_22 *= ((/* implicit */unsigned int) (signed char)-23);
                    var_23 = ((/* implicit */int) max((((unsigned int) (~((-9223372036854775807LL - 1LL))))), (((/* implicit */unsigned int) ((unsigned char) 3101511903U)))));
                }
            } 
        } 
    } 
}
