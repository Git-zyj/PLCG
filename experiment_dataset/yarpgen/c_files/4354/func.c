/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4354
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4354 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4354
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned short) min((((long long int) (_Bool)1)), (((/* implicit */long long int) (_Bool)1))));
    /* LoopSeq 1 */
    for (signed char i_0 = 2; i_0 < 15; i_0 += 4) 
    {
        var_15 = ((/* implicit */signed char) var_3);
        /* LoopNest 2 */
        for (unsigned int i_1 = 0; i_1 < 17; i_1 += 4) 
        {
            for (unsigned short i_2 = 3; i_2 < 14; i_2 += 3) 
            {
                {
                    arr_8 [i_0] [i_0] [i_0] [i_0 - 1] &= ((/* implicit */signed char) var_8);
                    arr_9 [10LL] &= ((/* implicit */signed char) ((((/* implicit */_Bool) ((short) min((var_10), (((/* implicit */unsigned short) (_Bool)1)))))) ? (((/* implicit */int) (!((!(((/* implicit */_Bool) (unsigned char)1))))))) : (((/* implicit */int) arr_3 [i_0]))));
                }
            } 
        } 
    }
}
