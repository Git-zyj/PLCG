/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168566
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168566 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168566
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (max((~22707), (var_14 != var_8)));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 9;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 9;i_4 += 1)
                        {
                            {
                                var_19 = (arr_14 [1] [i_3 + 2] [i_3 + 1] [i_3 + 2] [i_3]);
                                var_20 = (min(var_20, 210));
                            }
                        }
                    }
                }
                var_21 = (((~var_1) == (((!(arr_3 [i_0 - 1] [i_0 - 2]))))));
                var_22 += ((42828 <= ((18446744073709551615 ? (((27182 != (arr_9 [i_1] [i_1] [i_0] [i_0])))) : var_10))));
            }
        }
    }
    #pragma endscop
}
