/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87659
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87659 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87659
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_7;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            {
                var_20 *= ((((((var_7 ? (arr_3 [i_1] [i_1]) : (arr_2 [i_0] [i_1]))))) >> (18446744073709551615 - 18446744073709551594)));
                arr_4 [i_1] = (arr_0 [i_0 + 1]);
                arr_5 [i_1] [i_1 + 1] = (arr_3 [i_1] [i_0]);
                var_21 = (!((min(7574499139868120422, (arr_1 [i_1 - 1] [i_0 - 2])))));
                var_22 &= var_8;
            }
        }
    }
    #pragma endscop
}
