/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122549
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122549 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122549
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (var_9 ^ ((var_14 ^ (var_2 / 13))));
    var_19 = var_10;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_20 = (((((~var_11) ? (max(13, (arr_1 [i_0] [i_1]))) : (((arr_4 [i_0] [i_1]) ? (arr_1 [i_0] [i_1]) : var_13)))) >= ((((13 ^ var_11) ? (arr_3 [i_1]) : var_10)))));
                var_21 = (((((-9223372036854775807 - 1) | (arr_2 [i_0] [i_0])))) ? ((~(arr_2 [i_0] [i_1]))) : (arr_2 [i_0] [i_0]));
            }
        }
    }
    #pragma endscop
}
