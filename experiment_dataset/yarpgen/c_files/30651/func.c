/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30651
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30651 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30651
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */short) 1435948525538297723ULL);
    var_15 = ((/* implicit */unsigned char) ((signed char) var_3));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 3; i_0 < 19; i_0 += 4) 
    {
        var_16 = ((/* implicit */unsigned int) ((((/* implicit */_Bool) (~(((/* implicit */int) (signed char)95))))) || (((/* implicit */_Bool) arr_0 [i_0 - 3]))));
        arr_2 [i_0] = ((/* implicit */unsigned char) ((((/* implicit */int) var_9)) * (((/* implicit */int) ((((/* implicit */int) var_6)) == (((/* implicit */int) (signed char)-113)))))));
        arr_3 [(unsigned char)6] = ((/* implicit */_Bool) (-(arr_0 [i_0 - 1])));
    }
}
