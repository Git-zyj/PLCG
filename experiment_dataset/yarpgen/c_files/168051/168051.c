/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168051
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168051 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168051
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 &= var_14;
    var_18 = (~1151232608);
    var_19 = (((((max(var_5, var_13))) ? var_3 : (var_0 - var_16))));

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        arr_3 [i_0] = (!var_12);
        var_20 = ((((min((arr_0 [i_0]), (arr_0 [i_0])))) - (max(((min(var_15, 1))), (max((arr_1 [i_0] [i_0]), 10))))));
        var_21 += (arr_2 [i_0]);
    }
    var_22 = (((((var_15 ? 18446744073709551605 : (var_8 | var_2)))) ? (((!(((var_14 ? var_3 : var_9)))))) : var_10));
    #pragma endscop
}
