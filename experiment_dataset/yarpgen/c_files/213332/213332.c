/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213332
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213332 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213332
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    var_16 += (max((arr_2 [1]), -37));
                    var_17 += -13;
                }
            }
        }
    }
    var_18 = (((((var_15 ? 1 : var_5))) >= ((((((var_15 << (var_7 - 29788)))) < var_4)))));
    var_19 = (((((max(4134771761840064347, var_5)))) ? ((~((1 << (7263666651199879567 - 7263666651199879551))))) : ((((var_12 ? 8759 : var_2))))));
    var_20 = (max((var_11 * 5365167545460596576), 9007199254740991));
    #pragma endscop
}
