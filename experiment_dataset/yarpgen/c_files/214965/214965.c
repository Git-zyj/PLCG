/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214965
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214965 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214965
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max((min(35006, -34)), 33));
    var_15 = 34;
    var_16 = 29;

    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 16;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 17;i_3 += 1)
                {
                    {
                        var_17 = ((-(((arr_8 [i_2] [i_2] [i_2 + 1]) * (arr_8 [i_2] [i_2] [i_2 + 1])))));
                        var_18 = ((-((min((arr_10 [i_2 - 2] [i_0 - 2] [i_3] [i_0 + 1]), (max(9005, var_0)))))));
                        var_19 &= (max(var_6, (min((arr_5 [i_2 - 1] [i_1] [i_2 - 3]), -var_2))));
                    }
                }
            }
        }
        var_20 = ((-(((arr_6 [i_0] [i_0] [i_0] [i_0]) / var_8))));
    }
    #pragma endscop
}
