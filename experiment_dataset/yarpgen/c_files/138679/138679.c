/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138679
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138679 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138679
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = (min((((arr_1 [i_1]) ? (((arr_0 [i_1] [i_0]) ? (arr_4 [i_0] [i_1] [i_1]) : (arr_5 [i_1]))) : ((((arr_5 [i_1]) & var_6))))), ((min((var_5 | var_7), ((~(arr_3 [i_0] [i_1] [8]))))))));
                arr_7 [i_0] |= ((arr_5 [i_0]) * (min(83, 4768399558331694041)));
                arr_8 [i_1] [i_1] [3] = (arr_2 [i_1] [i_1]);
            }
        }
    }
    var_13 = var_1;
    #pragma endscop
}
