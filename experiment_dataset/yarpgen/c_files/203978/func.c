/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203978
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203978 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203978
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned char i_0 = 1; i_0 < 23; i_0 += 2) 
    {
        for (short i_1 = 0; i_1 < 25; i_1 += 2) 
        {
            for (short i_2 = 0; i_2 < 25; i_2 += 3) 
            {
                {
                    var_17 = ((/* implicit */unsigned char) var_5);
                    arr_8 [i_2] [i_1] [i_2] [8] = ((/* implicit */unsigned char) ((((/* implicit */_Bool) max((arr_1 [i_0 + 1]), (arr_4 [i_0 - 1] [i_1] [i_0])))) ? (arr_2 [i_0] [i_2]) : ((+(arr_0 [i_0 + 1] [i_0 + 2])))));
                }
            } 
        } 
    } 
    var_18 = ((/* implicit */unsigned char) var_4);
    var_19 = ((/* implicit */short) max(((~(((/* implicit */int) var_8)))), (((/* implicit */int) var_13))));
}
