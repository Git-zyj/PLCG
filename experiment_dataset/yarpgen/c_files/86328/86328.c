/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86328
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86328 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86328
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_2;
    var_13 = var_9;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {

                    for (int i_3 = 1; i_3 < 9;i_3 += 1)
                    {

                        for (int i_4 = 4; i_4 < 8;i_4 += 1)
                        {
                            var_14 = ((-1246452982 / ((min(104, 104)))));
                            var_15 += ((((min((((var_5 ? var_3 : -1441381324))), var_6))) ? var_6 : ((((((max(var_5, -105))) && (!var_7)))))));
                            var_16 = (min(var_16, 101));
                            var_17 *= -var_6;
                        }
                        var_18 = (var_6 - var_7);
                        arr_16 [i_2] = var_11;
                        var_19 = ((((min(var_4, var_6))) <= 164));
                    }
                    var_20 = ((~((-105 | (max(-32759, var_11))))));
                }
            }
        }
    }
    #pragma endscop
}
