/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34517
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34517 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34517
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((-(((((var_10 ? var_0 : 246954916))) ? (!-9) : ((min(var_7, var_13)))))));
    var_19 = (max(var_19, var_14));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_20 -= ((-5788541380358205868 ? var_7 : (min((!512), (975491919 / 975491919)))));
                arr_4 [i_0] [i_1] [i_0] = (arr_0 [i_1]);
            }
        }
    }
    var_21 = ((-((-var_9 ? (max(var_1, var_8)) : var_3))));
    #pragma endscop
}
