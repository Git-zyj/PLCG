/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210245
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210245 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210245
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (min(var_15, var_7));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 18;i_1 += 1)
        {
            {
                arr_6 [i_1] = (max(((((max(var_3, var_3))) >> (((((arr_5 [i_0] [1]) ? var_15 : var_0)) - 167)))), (((arr_5 [i_1 + 1] [i_1 - 3]) << (((arr_4 [i_0] [i_1] [i_0]) - 10863))))));
                var_20 = (min(((-225 * (min((arr_1 [i_0]), 29)))), (arr_0 [i_0])));
                var_21 = 1;
            }
        }
    }
    #pragma endscop
}
