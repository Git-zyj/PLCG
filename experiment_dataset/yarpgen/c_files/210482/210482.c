/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210482
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210482 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210482
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        {
                            var_16 = (((arr_1 [i_2 + 1] [i_0 + 1]) / (max((arr_1 [i_2 + 2] [i_0 + 3]), var_7))));
                            arr_12 [i_0] |= (((~62310) + 72));
                        }
                    }
                }
                var_17 = ((!(~-73)));
                var_18 = (-72 + (min((min((arr_2 [i_1] [i_0 + 3]), (-2147483647 - 1))), (!var_8))));
                arr_13 [i_1] [i_1] = 1;
                arr_14 [i_0 + 2] [i_0] [8] |= (min((arr_11 [i_0 + 2] [i_0 + 3] [i_0 + 2] [i_0]), (((arr_11 [i_0 - 1] [i_0 - 2] [i_0 - 2] [i_1]) << (((arr_11 [i_0 + 2] [i_0 + 3] [12] [1]) - 60735))))));
            }
        }
    }
    var_19 = 1;
    #pragma endscop
}
