/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208536
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208536 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208536
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = -var_4;

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_17 = (max(var_17, (((~(var_4 - var_13))))));
        arr_3 [i_0] = (((!(arr_2 [i_0]))));
        var_18 = (min(var_18, ((((arr_2 [i_0]) ? (arr_2 [i_0]) : ((min(1, -170))))))));
        var_19 = (var_2 ? (arr_0 [i_0] [i_0]) : (arr_1 [i_0]));
        var_20 = var_3;
    }
    for (int i_1 = 0; i_1 < 18;i_1 += 1)
    {
        arr_6 [i_1] = (min((arr_5 [i_1] [i_1]), ((var_7 ? (((!(arr_4 [i_1] [i_1])))) : ((var_11 ? var_8 : (arr_4 [i_1] [i_1])))))));
        arr_7 [i_1] = (arr_4 [i_1] [i_1]);
        var_21 = (((var_11 ? ((var_15 ? var_1 : var_6)) : (arr_4 [i_1] [i_1]))));
    }
    var_22 = (var_6 > var_11);
    var_23 = (!var_15);
    var_24 = var_1;
    #pragma endscop
}
