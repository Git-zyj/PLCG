/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136465
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136465 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136465
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 += var_7;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_21 = (min(var_21, (arr_2 [3])));

        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            arr_5 [19] [i_1] = (arr_4 [24] [24] [24]);
            var_22 += ((arr_3 [i_0]) ? (arr_1 [i_0]) : ((-21734 ? (arr_2 [19]) : 21721)));
        }
    }
    for (int i_2 = 0; i_2 < 10;i_2 += 1)
    {
        var_23 = -21735;
        var_24 += (arr_7 [i_2]);
        var_25 = 21721;
    }
    for (int i_3 = 0; i_3 < 18;i_3 += 1)
    {
        var_26 = -21734;
        arr_11 [i_3] |= 1867;
    }
    for (int i_4 = 0; i_4 < 19;i_4 += 1)
    {
        var_27 = (min(var_27, (((((((arr_1 [i_4]) ? ((-2 ? 45 : 21733)) : 8846594437449947845))) ? (arr_4 [i_4] [i_4] [i_4]) : (((arr_12 [i_4] [i_4]) ? 21740 : (arr_2 [14]))))))));
        arr_14 [i_4] = (arr_1 [i_4]);
    }
    #pragma endscop
}
