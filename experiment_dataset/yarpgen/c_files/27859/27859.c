/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27859
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27859 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27859
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((~((var_13 ? ((min(var_14, 0))) : (min(var_11, 1))))));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    arr_7 [i_2] [i_2] [i_2] [i_2] = (min((((var_5 != (arr_2 [i_0 + 2] [i_1] [i_0 + 2])))), 114));
                    arr_8 [i_0] = (!((((arr_2 [i_1] [i_0] [i_2]) ? var_15 : (arr_1 [i_1])))));
                    arr_9 [i_0] [i_0] [i_0] = ((~((min(-36, (arr_3 [i_0 - 3]))))));
                }
            }
        }
    }
    var_18 = (min(var_3, (((var_5 ? var_8 : var_4)))));
    #pragma endscop
}
