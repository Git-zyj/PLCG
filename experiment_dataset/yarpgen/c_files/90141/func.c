/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90141
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90141 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90141
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */int) min((var_20), (((/* implicit */int) var_8))));
    /* LoopSeq 1 */
    for (long long int i_0 = 3; i_0 < 13; i_0 += 3) 
    {
        var_21 = ((/* implicit */int) ((((/* implicit */long long int) ((((/* implicit */int) arr_2 [i_0 - 1])) ^ (((/* implicit */int) var_3))))) ^ (((((/* implicit */_Bool) arr_2 [i_0 - 3])) ? (((/* implicit */long long int) ((/* implicit */int) arr_2 [i_0 + 2]))) : (8375565514316110851LL)))));
        arr_3 [(short)5] = ((/* implicit */short) ((((/* implicit */unsigned int) ((/* implicit */int) arr_1 [i_0 - 3]))) & (var_16)));
    }
}
