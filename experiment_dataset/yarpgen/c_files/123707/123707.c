/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123707
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123707 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123707
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    arr_10 [i_0] = (max((((2304717109306851328 ? 37924 : (arr_8 [i_0] [i_1] [i_1] [i_2]))), (-2304717109306851328 & -2304717109306851329))));
                    var_18 = (min(((-(arr_1 [i_1] [i_0]))), 1898609968));
                    var_19 = (min((((var_1 ? var_3 : (arr_3 [i_0] [i_0])))), (((27620 <= (arr_2 [i_2] [i_1]))))));
                }
            }
        }
    }
    var_20 += var_11;
    #pragma endscop
}
