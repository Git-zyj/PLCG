/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91146
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91146 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91146
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 += ((/* implicit */_Bool) (~((~((~(var_2)))))));
    var_13 = ((/* implicit */unsigned short) (+(3236625087090200159ULL)));
    /* LoopSeq 1 */
    /* vectorizable */
    for (long long int i_0 = 3; i_0 < 18; i_0 += 3) 
    {
        var_14 = ((/* implicit */_Bool) (~((+(((/* implicit */int) var_1))))));
        var_15 = ((/* implicit */short) (-((+(((/* implicit */int) arr_0 [i_0]))))));
        var_16 = ((/* implicit */unsigned long long int) (-(((/* implicit */int) (!(((/* implicit */_Bool) 1532022501U)))))));
        var_17 = ((/* implicit */unsigned char) min((var_17), (((/* implicit */unsigned char) (-((+(((/* implicit */int) (unsigned char)45)))))))));
        var_18 = (~((-(-303966116))));
    }
}
