/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102918
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102918 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102918
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (148 + ((min((max(-15078, var_14)), ((max(var_14, var_3)))))));
    var_16 = ((15066 ? var_5 : (min(((var_6 ? var_2 : 15077)), (((var_11 ? -15078 : 15072)))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_1] [i_2] [i_0] = ((~(arr_6 [i_1] [i_0])));
                    arr_8 [i_0] [i_0] [i_0] [i_0] = ((((arr_3 [i_0] [i_1] [3]) ? ((min((arr_6 [i_0] [i_0]), var_9))) : (arr_6 [i_0] [i_0]))));
                }
            }
        }
    }
    #pragma endscop
}
