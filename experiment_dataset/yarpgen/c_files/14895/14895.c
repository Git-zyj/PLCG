/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14895
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14895 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14895
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (((min(1, var_1))));
    var_14 = (max(var_14, var_9));
    var_15 ^= var_4;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0] = ((!(~9869438953096353393)));
                var_16 = (min(((max((arr_3 [i_0] [i_0]), 1))), ((2091 ? (arr_3 [i_1 - 1] [i_0]) : 1))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 1; i_2 < 9;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 12;i_3 += 1)
        {
            {
                var_17 = max(4, 10000);
                var_18 = 252;
                var_19 = (((((17770939033918576224 ? (arr_3 [i_2] [i_2 + 2]) : 66))) % ((min(9051180404125699395, 57)))));
            }
        }
    }
    #pragma endscop
}
