#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

unsigned char var_4 = (unsigned char)6;
int var_8 = 710509991;
int var_9 = 854064372;
signed char var_10 = (signed char)-124;
int var_14 = -919217959;
unsigned char var_15 = (unsigned char)69;
short var_17 = (short)-30249;
int zero = 0;
int var_19 = 1191448641;
unsigned short var_20 = (unsigned short)19337;
long long int var_21 = 7461412136715564660LL;
unsigned char var_22 = (unsigned char)48;
signed char var_23 = (signed char)86;
signed char arr_4 [25] [25] ;
void init() {
    for (size_t i_0 = 0; i_0 < 25; ++i_0) 
        for (size_t i_1 = 0; i_1 < 25; ++i_1) 
            arr_4 [i_0] [i_1] = (i_0 % 2 == 0) ? (signed char)63 : (signed char)-72;
}

void checksum() {
    hash(&seed, var_19);
    hash(&seed, var_20);
    hash(&seed, var_21);
    hash(&seed, var_22);
    hash(&seed, var_23);
    for (size_t i_0 = 0; i_0 < 25; ++i_0) 
        for (size_t i_1 = 0; i_1 < 25; ++i_1) 
            hash(&seed, arr_4 [i_0] [i_1] );
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
