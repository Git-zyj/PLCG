/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166590
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166590 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166590
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_3;
    var_19 = (((var_10 & var_9) & (((var_6 << (var_6 - 38049))))));
    var_20 = var_16;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_6 [i_0] = (((((arr_5 [i_0]) * (((arr_3 [i_0]) ? 268435455 : (arr_5 [6])))))) ? 8766670917269004000 : (min(268435455, 161)));
                arr_7 [i_0] [i_1] = (arr_0 [i_0]);
                arr_8 [i_0] [i_0] = (((((-268435455 && 255) || ((var_0 && (arr_0 [i_0]))))) || ((((821605202410544605 && (arr_4 [i_0] [i_1] [2]))) || 268435455))));
                var_21 = arr_3 [i_0];
            }
        }
    }
    #pragma endscop
}
