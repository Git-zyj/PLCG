/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124164
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124164 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124164
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 2; i_0 < 7; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 10; i_1 += 2) 
        {
            {
                var_17 ^= ((/* implicit */unsigned long long int) ((_Bool) (!(((/* implicit */_Bool) 3987484286U)))));
                arr_5 [i_0] = ((/* implicit */int) ((signed char) ((_Bool) 1195282118)));
            }
        } 
    } 
    var_18 -= ((/* implicit */_Bool) max((((/* implicit */unsigned int) (-(((1528290564) / (1195282139)))))), (2080374784U)));
}
