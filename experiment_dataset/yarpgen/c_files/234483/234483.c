/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234483
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234483 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234483
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_15 -= (((var_10 ? var_8 : (arr_2 [i_0] [3]))));
        var_16 = ((~(arr_2 [i_0 - 1] [i_0 - 1])));
        var_17 = (((arr_2 [i_0] [i_0 - 1]) ? var_1 : (arr_2 [i_0] [i_0 - 1])));
    }
    var_18 = (min(var_18, var_5));

    for (int i_1 = 2; i_1 < 15;i_1 += 1)
    {
        var_19 = arr_3 [i_1];
        var_20 = (max(var_12, (((((8191 ^ (arr_3 [i_1])))) ? var_12 : (arr_3 [i_1])))));
    }
    /* vectorizable */
    for (int i_2 = 1; i_2 < 10;i_2 += 1)
    {
        arr_7 [i_2] [i_2] = (((-40 + 2147483647) >> (((arr_3 [i_2 + 2]) - 56))));
        arr_8 [i_2] = -63;

        for (int i_3 = 2; i_3 < 9;i_3 += 1)
        {
            var_21 = -28;
            var_22 *= (arr_3 [i_3]);
        }
        var_23 ^= var_3;
    }
    var_24 += var_13;
    #pragma endscop
}
