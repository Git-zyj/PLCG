/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130167
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130167 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130167
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, var_16));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    var_20 = ((var_11 * ((((!(arr_6 [i_0 - 1] [i_1 + 1] [i_1 + 1] [i_1 + 1])))))));
                    arr_7 [7] [i_0] [13] [i_2] = ((((((arr_2 [i_0 - 1]) > 1))) >> (((max((((var_11 << (((arr_4 [i_2]) - 1859370507))))), (((arr_6 [i_0] [i_0] [i_0] [i_2]) / 1)))) - 9708158948960447231))));
                }
            }
        }
    }
    #pragma endscop
}
