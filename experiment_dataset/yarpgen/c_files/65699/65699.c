/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 65699
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=65699 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/65699
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 ^= var_7;

    /* vectorizable */
    for (int i_0 = 3; i_0 < 15;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 16;i_2 += 1)
            {
                {
                    var_14 = (max(var_14, (arr_2 [1])));
                    var_15 = (min(var_15, (arr_2 [0])));
                    var_16 = (min(var_16, (((var_11 == (!-886855749))))));
                }
            }
        }
        var_17 = ((((var_0 ? 1 : var_4)) < ((0 >> (var_12 - 3384638164996197974)))));
        var_18 = (((arr_6 [i_0 + 2] [15] [i_0 - 3]) ? 204 : (arr_2 [i_0 + 2])));
    }
    var_19 = var_10;
    #pragma endscop
}
