/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32863
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32863 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32863
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(-23875, 1));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_11 = (max((max(1, 4635375464239922122)), (min(var_2, 1152886320234758144))));
                    var_12 *= (max(2039066562, ((min(0, 125)))));
                    var_13 = (max(var_13, ((max(((max(1087814267, var_1))), (max(var_5, 15)))))));
                }
            }
        }
    }
    var_14 = var_3;
    #pragma endscop
}
