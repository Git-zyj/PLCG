/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48395
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48395 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48395
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = 24;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_21 = (max(var_21, (((var_1 >> ((var_10 ? var_18 : (arr_1 [i_0]))))))));
        /* LoopNest 3 */
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 10;i_3 += 1)
                {
                    {
                        var_22 |= var_7;
                        var_23 = -24;
                    }
                }
            }
        }
        arr_10 [i_0] = arr_0 [i_0];
    }
    for (int i_4 = 2; i_4 < 18;i_4 += 1)
    {

        /* vectorizable */
        for (int i_5 = 0; i_5 < 19;i_5 += 1)
        {
            /* LoopNest 2 */
            for (int i_6 = 1; i_6 < 17;i_6 += 1)
            {
                for (int i_7 = 1; i_7 < 18;i_7 += 1)
                {
                    {
                        var_24 ^= (~var_14);
                        var_25 = (min(var_25, 6787821586542386980));
                    }
                }
            }
            var_26 -= var_2;
        }
        var_27 = ((18350397795425500476 ? ((-(arr_18 [i_4 + 1] [i_4 - 2] [i_4] [i_4]))) : (arr_15 [i_4 + 1])));
    }
    var_28 = var_13;
    var_29 = var_13;
    #pragma endscop
}
