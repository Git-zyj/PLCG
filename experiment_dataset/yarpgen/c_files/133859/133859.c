/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133859
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133859 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133859
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (max(var_11, (((var_6 << (max(((min(var_8, var_9))), ((1 ? 1 : 26232)))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 6;i_1 += 1)
        {
            {
                var_12 ^= var_2;
                var_13 = (((~1) ? (((((-(arr_0 [i_0] [i_1])))) ? ((max((arr_3 [i_1 + 1]), 1))) : (((arr_4 [i_0] [i_0]) ? var_1 : var_10)))) : (arr_4 [i_0] [i_1 + 4])));
                var_14 = (max(1, ((~(((arr_4 [i_0] [i_0]) ? var_1 : var_3))))));
            }
        }
    }
    #pragma endscop
}
