/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215792
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215792 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215792
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (var_15 % var_10);
    var_19 &= var_13;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                arr_4 [i_0] = 43;
                var_20 = ((((max(582716202, 1))) ? ((23 ? ((18446744073709551592 - (arr_2 [i_0]))) : (arr_3 [i_0] [i_1]))) : ((18446744073709551592 ^ (((max(1, 1))))))));
                arr_5 [i_1] [i_0] = ((~(arr_3 [i_0] [1])));
                arr_6 [i_0] [i_0] = (min(1816079608, 143));
            }
        }
    }
    var_21 += (((((min(var_11, 524286))) ? (~474511550) : 0)));
    var_22 = var_17;
    #pragma endscop
}
