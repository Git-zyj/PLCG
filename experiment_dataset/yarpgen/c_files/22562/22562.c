/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22562
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22562 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22562
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 |= 96;
    var_12 = (1 + (max(-114, (1 - var_8))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 19;i_2 += 1)
            {
                {
                    var_13 += (arr_3 [i_1]);
                    var_14 -= var_1;
                    var_15 *= (~var_8);
                }
            }
        }
    }

    for (int i_3 = 0; i_3 < 18;i_3 += 1)
    {
        var_16 = (max(var_16, ((((arr_5 [i_3] [i_3] [i_3]) + (arr_0 [i_3]))))));
        arr_10 [i_3] = 173;
    }
    for (int i_4 = 0; i_4 < 25;i_4 += 1)
    {
        arr_13 [i_4] &= ((((max((arr_11 [i_4] [i_4]), 37267))) + (arr_11 [i_4] [i_4])));
        var_17 *= (arr_12 [i_4]);
        var_18 *= ((var_1 >> (var_3 + 90)));
        /* LoopNest 2 */
        for (int i_5 = 1; i_5 < 24;i_5 += 1)
        {
            for (int i_6 = 0; i_6 < 25;i_6 += 1)
            {
                {
                    var_19 = var_6;
                    var_20 ^= (arr_15 [1] [1] [24]);
                }
            }
        }
    }
    #pragma endscop
}
