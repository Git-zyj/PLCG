/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200215
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200215 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200215
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 16;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        {
                            arr_11 [i_2] [i_2] = ((((arr_10 [i_1 - 1] [i_1 - 1] [i_1 - 1] [i_1] [i_1 - 1]) ? (arr_10 [i_1 - 1] [i_1] [i_1 - 1] [i_1 - 1] [i_1 - 1]) : (arr_10 [i_1 - 1] [i_1] [i_1] [i_1] [i_1 - 1]))) < var_6);
                            var_18 = (arr_3 [3] [3] [1]);
                        }
                    }
                }
                var_19 = ((+(((arr_5 [i_0] [2] [2]) ? (max((arr_0 [7]), (arr_2 [i_0] [i_0]))) : (arr_2 [i_0] [17])))));
                var_20 = (max(var_20, (arr_4 [i_1 - 1] [i_1 - 1] [i_1])));
            }
        }
    }
    var_21 *= var_0;
    #pragma endscop
}
