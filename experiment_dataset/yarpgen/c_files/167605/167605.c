/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 167605
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=167605 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/167605
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (~0);
    var_11 = var_5;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 23;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_1] [13] [i_1] = (arr_0 [i_1] [i_0]);
                    arr_8 [i_1] [3] [i_2] = (min(220, 4021037213));
                    arr_9 [i_0] [i_1] [i_2 + 1] [i_2] = (((((max(-21348, 1))) ? (arr_2 [9]) : var_3)));
                    arr_10 [i_0] [23] [i_1] = (((!var_7) - ((max((arr_5 [i_2 + 1]), var_1)))));
                }
            }
        }
    }
    var_12 = 780;
    #pragma endscop
}
