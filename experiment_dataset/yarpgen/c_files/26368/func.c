/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 26368
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=26368 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/26368
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */short) (-(var_11)));
    /* LoopNest 2 */
    for (unsigned int i_0 = 2; i_0 < 14; i_0 += 3) 
    {
        for (short i_1 = 0; i_1 < 16; i_1 += 2) 
        {
            {
                var_19 &= ((/* implicit */short) ((_Bool) 9417290571486489912ULL));
                arr_6 [i_0] [i_0] = ((/* implicit */_Bool) (signed char)54);
                var_20 ^= ((_Bool) var_5);
            }
        } 
    } 
    var_21 *= var_2;
    var_22 = ((/* implicit */int) min((var_22), (max((((((/* implicit */_Bool) var_16)) ? (var_10) : (((/* implicit */int) ((short) var_5))))), (((/* implicit */int) var_2))))));
}
