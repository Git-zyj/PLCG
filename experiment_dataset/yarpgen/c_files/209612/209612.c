/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209612
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209612 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209612
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_10 = (((((arr_0 [i_0] [i_0]) ? var_8 : 8278380306453818375)) ^ (arr_0 [i_0] [i_0])));
        arr_2 [i_0] = var_2;
        var_11 -= (max(-8278380306453818375, 1998913245));
        arr_3 [0] &= (((((var_8 ? var_3 : (~0)))) ? (arr_1 [i_0] [i_0]) : ((max((arr_1 [i_0] [i_0]), (arr_1 [i_0] [i_0]))))));
    }
    var_12 |= (((max(-413831991, (((var_7 ? 0 : var_6))))) >= var_2));
    var_13 ^= var_9;
    #pragma endscop
}
