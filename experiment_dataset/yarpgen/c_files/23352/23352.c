/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23352
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23352 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23352
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 += ((((var_4 ? (!var_5) : (min(var_0, var_4)))) - var_7));

    /* vectorizable */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = (arr_3 [i_0]);

        for (int i_1 = 4; i_1 < 13;i_1 += 1)
        {
            arr_8 [i_0 + 2] [i_1] [10] &= ((~(((!(arr_3 [1]))))));
            arr_9 [i_0] [i_1] [i_1] = ((((((arr_7 [i_0] [i_1 + 1]) ? (arr_1 [i_0]) : (arr_6 [i_0] [i_0] [i_1])))) ? (((arr_7 [i_0] [7]) ? (arr_2 [i_0] [i_0]) : (arr_0 [i_0]))) : (arr_0 [i_1])));
            arr_10 [i_0] [i_0] [5] = (arr_1 [i_0]);
        }
        arr_11 [i_0] = (0 < 1);
    }
    var_17 = var_2;
    var_18 = var_2;
    #pragma endscop
}
