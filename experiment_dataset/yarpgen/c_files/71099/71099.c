/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71099
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71099 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71099
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 15;i_4 += 1)
                        {
                            {
                                var_16 = (((arr_3 [i_0] [i_0]) ? (arr_6 [i_0] [i_1 - 2] [i_2] [i_0]) : 0));
                                var_17 = var_12;
                            }
                        }
                    }
                    arr_12 [i_0] [i_0] [i_1] = (arr_2 [i_1] [i_0]);
                    var_18 ^= (((((24 ? var_4 : ((max((arr_0 [i_1]), 117)))))) ? (arr_3 [i_0] [i_0]) : (arr_3 [i_0] [i_0])));
                }
            }
        }
    }
    var_19 = (((min(var_11, var_8)) >> -1010736610));
    var_20 = var_11;
    #pragma endscop
}
